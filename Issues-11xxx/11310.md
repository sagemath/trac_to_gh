# Issue 11310: Monkey-patch catchall `except:` statements so they at least don't catch `KeyboardInterrupt` errors

archive/issues_011138.json:
```json
{
    "assignees": [],
    "body": "There are a lot places in the Sage library which use\n\n```\nexcept:\n```\nThis is bad because it catches non-errors like `KeyboardInterrupt`.\n\n---\n\nThen the patch [attachment: 11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz) has ben auto-generated to replace all instances of\n\n```\nexcept:\n```\nby\n\n```\nexcept StandardError:\n```\nand all instances of\n\n```\nraise Exception\n```\nby\n\n```\nraise RuntimeError\n```\n\n---\n\n**Apply** [and [attachment: 11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz) and [attachment: 11310_psage.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_psage.patch.gz)(https://github.com/sagemath/sage/files/ticket11310/e5d245395abd8a7387695d22926f8f23.gz).\n\nDepends on #13109\n\nAssignee: **@jasongrout**\n\nCC:  @kini\n\nReviewer: **Keshav Kini**\n\nAuthor: **Jeroen Demeyer, Volker Braun**\n\nMerged: **sage-5.3.beta1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11310_\n\n",
    "closed_at": "2012-08-12T18:58:35Z",
    "created_at": "2011-05-07T10:10:30Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Monkey-patch catchall `except:` statements so they at least don't catch `KeyboardInterrupt` errors",
    "type": "issue",
    "updated_at": "2014-01-02T15:58:42Z",
    "url": "https://github.com/sagemath/sage/issues/11310",
    "user": "https://github.com/jdemeyer"
}
```
There are a lot places in the Sage library which use

```
except:
```
This is bad because it catches non-errors like `KeyboardInterrupt`.

---

Then the patch [attachment: 11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz) has ben auto-generated to replace all instances of

```
except:
```
by

```
except StandardError:
```
and all instances of

```
raise Exception
```
by

```
raise RuntimeError
```

---

**Apply** [and [attachment: 11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz) and [attachment: 11310_psage.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_psage.patch.gz)(https://github.com/sagemath/sage/files/ticket11310/e5d245395abd8a7387695d22926f8f23.gz).

Depends on #13109

Assignee: **@jasongrout**

CC:  @kini

Reviewer: **Keshav Kini**

Author: **Jeroen Demeyer, Volker Braun**

Merged: **sage-5.3.beta1**

_Issue created by migration from https://trac.sagemath.org/ticket/11310_





---

archive/issue_events_132473.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-07T10:10:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "milestone_number": null,
    "milestone_title": "sage-5.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11310#event-132473"
}
```



---

archive/issue_events_132474.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-07T10:10:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11310#event-132474"
}
```



---

archive/issue_events_132475.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-07T10:10:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11310#event-132475"
}
```



---

archive/issue_comments_115639.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nWhat's the problem with this ticket? I think it is a small enough change that it can be at least set for review.",
    "created_at": "2012-05-15T09:08:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115639",
    "user": "https://github.com/ppurka"
}
```

<a id='comment:2'>Comment 2:</a>
What's the problem with this ticket? I think it is a small enough change that it can be at least set for review.



---

archive/issue_comments_115640.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nTests pass on 5.0. Jeroen, is this ready for review, or do you want to add more stuff?",
    "created_at": "2012-05-16T05:22:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115640",
    "user": "https://github.com/kini"
}
```

<a id='comment:3'>Comment 3:</a>
Tests pass on 5.0. Jeroen, is this ready for review, or do you want to add more stuff?



---

archive/issue_comments_115641.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nI would like to fix the following from `devel/sage/sage/structure/parent.pyx`, starting at line 2577:\n\n```\n        try:\n            return super(Parent, self)._an_element_()\n        except EmptySetError:\n            raise\n        except:\n            _record_exception()\n            pass\n\n        try:\n            return self.gen(0)\n        except:\n            _record_exception()\n            pass\n\n        try:\n            return self.gen()\n        except:\n            _record_exception()\n            pass\n```",
    "created_at": "2012-05-16T07:21:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115641",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:4'>Comment 4:</a>
I would like to fix the following from `devel/sage/sage/structure/parent.pyx`, starting at line 2577:

```
        try:
            return super(Parent, self)._an_element_()
        except EmptySetError:
            raise
        except:
            _record_exception()
            pass

        try:
            return self.gen(0)
        except:
            _record_exception()
            pass

        try:
            return self.gen()
        except:
            _record_exception()
            pass
```



---

archive/issue_comments_115642.json:
```json
{
    "body": "Author: **Jeroen Demeyer**",
    "created_at": "2012-06-21T07:59:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115642",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Jeroen Demeyer**



---

archive/issue_comments_115643.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,31 @@\n+There are a lot places in the Sage library which use\n \n+```\n+except:\n+```\n+This is bad because it catches non-errors like `KeyboardInterrupt` and `StopIteration`.\n+\n+The patch [attachent:11310_auto.patch] has ben auto-generated by replacing all instances of\n+\n+```\n+except:\n+```\n+by\n+\n+```\n+except StandardError:\n+```\n+and all instances of\n+\n+```\n+raise Exeption\n+```\n+by\n+\n+```\n+raise RuntimeError\n+```\n+\n+Then the patch [attachment: 11310.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310.patch.gz) does some small miscellaneous fixes.\n+\n+**Apply** all patches.\n``````\n",
    "created_at": "2012-06-21T07:59:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115643",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,31 @@
+There are a lot places in the Sage library which use
 
+```
+except:
+```
+This is bad because it catches non-errors like `KeyboardInterrupt` and `StopIteration`.
+
+The patch [attachent:11310_auto.patch] has ben auto-generated by replacing all instances of
+
+```
+except:
+```
+by
+
+```
+except StandardError:
+```
+and all instances of
+
+```
+raise Exeption
+```
+by
+
+```
+raise RuntimeError
+```
+
+Then the patch [attachment: 11310.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310.patch.gz) does some small miscellaneous fixes.
+
+**Apply** all patches.
``````




---

archive/issue_comments_115644.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -5,7 +5,9 @@\n ```\n This is bad because it catches non-errors like `KeyboardInterrupt` and `StopIteration`.\n \n-The patch [attachent:11310_auto.patch] has ben auto-generated by replacing all instances of\n+---\n+\n+The patch [attachment: 11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz) has ben auto-generated by replacing all instances of\n \n ```\n except:\n@@ -18,7 +20,7 @@\n and all instances of\n \n ```\n-raise Exeption\n+raise Exception\n ```\n by\n \n@@ -28,4 +30,6 @@\n \n Then the patch [attachment: 11310.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310.patch.gz) does some small miscellaneous fixes.\n \n-**Apply** all patches.\n+---\n+\n+**Apply** [and [attachment: 11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz)(https://github.com/sagemath/sage/files/ticket11310/6c0e4b5ddb9fedd780d94f484c9d3b42.gz)\n``````\n",
    "created_at": "2012-06-21T08:01:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115644",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -5,7 +5,9 @@
 ```
 This is bad because it catches non-errors like `KeyboardInterrupt` and `StopIteration`.
 
-The patch [attachent:11310_auto.patch] has ben auto-generated by replacing all instances of
+---
+
+The patch [attachment: 11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz) has ben auto-generated by replacing all instances of
 
 ```
 except:
@@ -18,7 +20,7 @@
 and all instances of
 
 ```
-raise Exeption
+raise Exception
 ```
 by
 
@@ -28,4 +30,6 @@
 
 Then the patch [attachment: 11310.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310.patch.gz) does some small miscellaneous fixes.
 
-**Apply** all patches.
+---
+
+**Apply** [and [attachment: 11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz)(https://github.com/sagemath/sage/files/ticket11310/6c0e4b5ddb9fedd780d94f484c9d3b42.gz)
``````




---

archive/issue_comments_115645.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nChanging these bare except statements to `except StandardError` seems like not enough, don't you think? Except statements should always be written to accept the narrowest possible expected exception class only.",
    "created_at": "2012-06-21T08:08:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115645",
    "user": "https://github.com/kini"
}
```

<a id='comment:7'>Comment 7:</a>
Changing these bare except statements to `except StandardError` seems like not enough, don't you think? Except statements should always be written to accept the narrowest possible expected exception class only.



---

archive/issue_comments_115646.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nReplying to [@kini](#comment%3A7):\n> Changing these bare except statements to `except StandardError` seems like not enough, don't you think? Except statements should always be written to accept the narrowest possible expected exception class only.\n\nThen what should it be? According to the [hierarchy of exceptions](http://docs.python.org/library/exceptions.html#exception-hierarchy) I think `StandardError` encapsulates most of what can go wrong. It is at least better than a blank `except:`.\n\nAbout the `RuntimeError`s I think some of them should be `ValueError` and I spotted one which should be an `AttributeError`.",
    "created_at": "2012-06-21T08:26:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115646",
    "user": "https://github.com/ppurka"
}
```

<a id='comment:8'>Comment 8:</a>
Replying to [@kini](#comment%3A7):
> Changing these bare except statements to `except StandardError` seems like not enough, don't you think? Except statements should always be written to accept the narrowest possible expected exception class only.

Then what should it be? According to the [hierarchy of exceptions](http://docs.python.org/library/exceptions.html#exception-hierarchy) I think `StandardError` encapsulates most of what can go wrong. It is at least better than a blank `except:`.

About the `RuntimeError`s I think some of them should be `ValueError` and I spotted one which should be an `AttributeError`.



---

archive/issue_comments_115647.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nI mean that the code should be inspected and the appropriate exception class should be used in each case. Of course, this is not easy, considering that the autogenerated patch is more than 100 KB. But still, it should be done.",
    "created_at": "2012-06-21T08:32:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115647",
    "user": "https://github.com/kini"
}
```

<a id='comment:9'>Comment 9:</a>
I mean that the code should be inspected and the appropriate exception class should be used in each case. Of course, this is not easy, considering that the autogenerated patch is more than 100 KB. But still, it should be done.



---

archive/issue_comments_115648.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nI'm currently going though the auto-generated patch and fixing the obvious things by hand.  Note that the title of this ticket isn't \"totally fix all exceptions inside Sage\".\n\nThe main point is to not catch `KeyboardInterrupt` errors.",
    "created_at": "2012-06-21T08:34:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115648",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:10'>Comment 10:</a>
I'm currently going though the auto-generated patch and fixing the obvious things by hand.  Note that the title of this ticket isn't "totally fix all exceptions inside Sage".

The main point is to not catch `KeyboardInterrupt` errors.



---

archive/issue_comments_115649.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nReplying to [@jdemeyer](#comment%3A10):\n> The main point is to not catch `KeyboardInterrupt` errors.\n\nOther than some of the `RuntimeError`s, I think the patch is fine, if the objective was this.",
    "created_at": "2012-06-21T08:40:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115649",
    "user": "https://github.com/ppurka"
}
```

<a id='comment:11'>Comment 11:</a>
Replying to [@jdemeyer](#comment%3A10):
> The main point is to not catch `KeyboardInterrupt` errors.

Other than some of the `RuntimeError`s, I think the patch is fine, if the objective was this.



---

archive/issue_comments_115650.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nReplying to [@jdemeyer](#comment%3A10):\n> I'm currently going though the auto-generated patch and fixing the obvious things by hand.  Note that the title of this ticket isn't \"totally fix all exceptions inside Sage\".\n> \n> The main point is to not catch `KeyboardInterrupt` errors.\n\nWell, the title is a bit misleading I guess. Sure, it's not \"totally fix all exceptions inside Sage\", but it does seem like \"totally fix some exceptions inside Sage\" :)",
    "created_at": "2012-06-21T08:46:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115650",
    "user": "https://github.com/kini"
}
```

<a id='comment:12'>Comment 12:</a>
Replying to [@jdemeyer](#comment%3A10):
> I'm currently going though the auto-generated patch and fixing the obvious things by hand.  Note that the title of this ticket isn't "totally fix all exceptions inside Sage".
> 
> The main point is to not catch `KeyboardInterrupt` errors.

Well, the title is a bit misleading I guess. Sure, it's not "totally fix all exceptions inside Sage", but it does seem like "totally fix some exceptions inside Sage" :)



---

archive/issue_events_132476.json:
```json
{
    "actor": "https://github.com/kini",
    "created_at": "2012-06-21T08:46:55Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "title_is": "Monkey-patch catchall `except:` statements so they at least don't catch `KeyboardInterrupt` errors",
    "title_was": "Get rid of some bare \"except:\" statements",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11310#event-132476"
}
```



---

archive/issue_comments_115651.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -3,7 +3,7 @@\n ```\n except:\n ```\n-This is bad because it catches non-errors like `KeyboardInterrupt` and `StopIteration`.\n+This is bad because it catches non-errors like `KeyboardInterrupt`.\n \n ---\n \n``````\n",
    "created_at": "2012-06-21T10:39:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115651",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -3,7 +3,7 @@
 ```
 except:
 ```
-This is bad because it catches non-errors like `KeyboardInterrupt` and `StopIteration`.
+This is bad because it catches non-errors like `KeyboardInterrupt`.
 
 ---
 
``````




---

archive/issue_comments_115652.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nOkay, I changed a lot of things by hand, needs_review.",
    "created_at": "2012-06-21T13:56:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115652",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:14'>Comment 14:</a>
Okay, I changed a lot of things by hand, needs_review.



---

archive/issue_events_132477.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-21T13:56:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11310#event-132477"
}
```



---

archive/issue_events_132478.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-21T13:56:07Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "milestone_number": null,
    "milestone_title": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11310#event-132478"
}
```



---

archive/issue_events_132479.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-21T13:56:07Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "milestone_number": null,
    "milestone_title": "sage-5.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11310#event-132479"
}
```



---

archive/issue_comments_115653.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -7,7 +7,9 @@\n \n ---\n \n-The patch [attachment: 11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz) has ben auto-generated by replacing all instances of\n+Then the patch [attachment: 11310.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310.patch.gz) fixes some of these.\n+\n+Then the patch [attachment: 11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz) has ben auto-generated to replace all remaining instances of\n \n ```\n except:\n@@ -28,8 +30,6 @@\n raise RuntimeError\n ```\n \n-Then the patch [attachment: 11310.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310.patch.gz) does some small miscellaneous fixes.\n-\n ---\n \n **Apply** [and [attachment: 11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz)(https://github.com/sagemath/sage/files/ticket11310/6c0e4b5ddb9fedd780d94f484c9d3b42.gz)\n``````\n",
    "created_at": "2012-06-21T14:39:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115653",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -7,7 +7,9 @@
 
 ---
 
-The patch [attachment: 11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz) has ben auto-generated by replacing all instances of
+Then the patch [attachment: 11310.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310.patch.gz) fixes some of these.
+
+Then the patch [attachment: 11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz) has ben auto-generated to replace all remaining instances of
 
 ```
 except:
@@ -28,8 +30,6 @@
 raise RuntimeError
 ```
 
-Then the patch [attachment: 11310.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310.patch.gz) does some small miscellaneous fixes.
-
 ---
 
 **Apply** [and [attachment: 11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz)(https://github.com/sagemath/sage/files/ticket11310/6c0e4b5ddb9fedd780d94f484c9d3b42.gz)
``````




---

archive/issue_comments_115654.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nPatchbot says trailing whitespace :P",
    "created_at": "2012-06-21T15:51:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115654",
    "user": "https://github.com/kini"
}
```

<a id='comment:16'>Comment 16:</a>
Patchbot says trailing whitespace :P



---

archive/issue_comments_115655.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nHmm.. a lot of doctests are failing:\n\n```\nThe following tests failed:\n                           \n\tsage -t  --long -force_lib devel/sage/doc/en/thematic_tutorials/lie/weight_ring.rst # 2 doctests failed\n\tsage -t  --long -force_lib devel/sage/sage/rings/polynomial/pbori.pyx # 3 doctests failed\n\tsage -t  --long -force_lib devel/sage/sage/combinat/free_module.py # 3 doctests failed\n\tsage -t  --long -force_lib devel/sage/sage/combinat/posets/poset_examples.py # 1 doctests failed\n\tsage -t  --long -force_lib devel/sage/sage/combinat/root_system/weyl_characters.py # 11 doctests failed\n\tsage -t  --long -force_lib devel/sage/sage/matrix/matrix_double_dense.pyx # 1 doctests failed\n\tsage -t  --long -force_lib devel/sage/sage/modules/vector_double_dense.pyx # 1 doctests failed\n\tsage -t  --long -force_lib devel/sage/sage/graphs/isgci.py # 2 doctests failed\n\tsage -t  --long -force_lib devel/sage/sage/graphs/generic_graph.py # 3 doctests failed\n\tsage -t  --long -force_lib devel/sage/sage/graphs/graph_decompositions/rankwidth.pyx # 1 doctests failed\n\tsage -t  --long -force_lib devel/sage/sage/graphs/graph_generators.py # 2 doctests failed\n\tsage -t  --long -force_lib devel/sage/sage/interfaces/chomp.py # 3 doctests failed\n\tsage -t  --long -force_lib devel/sage/sage/interfaces/expect.py # 1 doctests failed\n\tsage -t  --long -force_lib devel/sage/sage/interfaces/psage.py # Time out\n\tsage -t  --long -force_lib devel/sage/sage/homology/simplicial_complex_morphism.py # 8 doctests failed\n\tsage -t  --long -force_lib devel/sage/sage/homology/delta_complex.py # 5 doctests failed\n\tsage -t  --long -force_lib devel/sage/sage/homology/cell_complex.py # 5 doctests failed\n\tsage -t  --long -force_lib devel/sage/sage/homology/simplicial_complex_homset.py # 7 doctests failed\n\tsage -t  --long -force_lib devel/sage/sage/homology/simplicial_complex.py # 33 doctests failed\n\tsage -t  --long -force_lib devel/sage/sage/homology/examples.py # 8 doctests failed\n```",
    "created_at": "2012-06-22T04:18:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115655",
    "user": "https://github.com/ppurka"
}
```

<a id='comment:17'>Comment 17:</a>
Hmm.. a lot of doctests are failing:

```
The following tests failed:
                           
	sage -t  --long -force_lib devel/sage/doc/en/thematic_tutorials/lie/weight_ring.rst # 2 doctests failed
	sage -t  --long -force_lib devel/sage/sage/rings/polynomial/pbori.pyx # 3 doctests failed
	sage -t  --long -force_lib devel/sage/sage/combinat/free_module.py # 3 doctests failed
	sage -t  --long -force_lib devel/sage/sage/combinat/posets/poset_examples.py # 1 doctests failed
	sage -t  --long -force_lib devel/sage/sage/combinat/root_system/weyl_characters.py # 11 doctests failed
	sage -t  --long -force_lib devel/sage/sage/matrix/matrix_double_dense.pyx # 1 doctests failed
	sage -t  --long -force_lib devel/sage/sage/modules/vector_double_dense.pyx # 1 doctests failed
	sage -t  --long -force_lib devel/sage/sage/graphs/isgci.py # 2 doctests failed
	sage -t  --long -force_lib devel/sage/sage/graphs/generic_graph.py # 3 doctests failed
	sage -t  --long -force_lib devel/sage/sage/graphs/graph_decompositions/rankwidth.pyx # 1 doctests failed
	sage -t  --long -force_lib devel/sage/sage/graphs/graph_generators.py # 2 doctests failed
	sage -t  --long -force_lib devel/sage/sage/interfaces/chomp.py # 3 doctests failed
	sage -t  --long -force_lib devel/sage/sage/interfaces/expect.py # 1 doctests failed
	sage -t  --long -force_lib devel/sage/sage/interfaces/psage.py # Time out
	sage -t  --long -force_lib devel/sage/sage/homology/simplicial_complex_morphism.py # 8 doctests failed
	sage -t  --long -force_lib devel/sage/sage/homology/delta_complex.py # 5 doctests failed
	sage -t  --long -force_lib devel/sage/sage/homology/cell_complex.py # 5 doctests failed
	sage -t  --long -force_lib devel/sage/sage/homology/simplicial_complex_homset.py # 7 doctests failed
	sage -t  --long -force_lib devel/sage/sage/homology/simplicial_complex.py # 33 doctests failed
	sage -t  --long -force_lib devel/sage/sage/homology/examples.py # 8 doctests failed
```



---

archive/issue_comments_115656.json:
```json
{
    "body": "<a id='comment:18'>Comment 18:</a>\nI think something changed in the patches between the last time I ran `make ptestlong` (I had to leave it running and go home) and the new patches uploaded 12h ago. At present, running the doctest on only these few files gives no errors.",
    "created_at": "2012-06-22T05:30:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115656",
    "user": "https://github.com/ppurka"
}
```

<a id='comment:18'>Comment 18:</a>
I think something changed in the patches between the last time I ran `make ptestlong` (I had to leave it running and go home) and the new patches uploaded 12h ago. At present, running the doctest on only these few files gives no errors.



---

archive/issue_events_132480.json:
```json
{
    "actor": "https://github.com/kini",
    "created_at": "2012-06-22T05:32:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11310#event-132480"
}
```



---

archive/issue_events_132481.json:
```json
{
    "actor": "https://github.com/kini",
    "created_at": "2012-06-22T05:32:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11310#event-132481"
}
```



---

archive/issue_comments_115657.json:
```json
{
    "body": "<a id='comment:19'>Comment 19:</a>\nPatchbot says it's fine over the whole library. Looks good to me.",
    "created_at": "2012-06-22T05:32:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115657",
    "user": "https://github.com/kini"
}
```

<a id='comment:19'>Comment 19:</a>
Patchbot says it's fine over the whole library. Looks good to me.



---

archive/issue_comments_115658.json:
```json
{
    "body": "Reviewer: **Keshav Kini**",
    "created_at": "2012-06-22T05:32:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115658",
    "user": "https://github.com/kini"
}
```

Reviewer: **Keshav Kini**



---

archive/issue_comments_115659.json:
```json
{
    "body": "Attachment: **[11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz)**",
    "created_at": "2012-06-29T12:11:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115659",
    "user": "https://github.com/jdemeyer"
}
```

Attachment: **[11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz)**



---

archive/issue_comments_115660.json:
```json
{
    "body": "<a id='comment:20'>Comment 20:</a>\nRebased to #11143 (where one of the `except:` statements was removed).",
    "created_at": "2012-06-29T12:12:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115660",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:20'>Comment 20:</a>
Rebased to #11143 (where one of the `except:` statements was removed).



---

archive/issue_comments_115661.json:
```json
{
    "body": "Dependencies: **#11143**",
    "created_at": "2012-06-29T17:13:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115661",
    "user": "https://github.com/kini"
}
```

Dependencies: **#11143**



---

archive/issue_comments_115662.json:
```json
{
    "body": "<a id='comment:22'>Comment 22:</a>\nNot really, I just *removed* a hunk to resolve the conflict.",
    "created_at": "2012-06-29T21:13:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115662",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:22'>Comment 22:</a>
Not really, I just *removed* a hunk to resolve the conflict.



---

archive/issue_comments_115663.json:
```json
{
    "body": "Changed dependencies from **#11143** to none",
    "created_at": "2012-06-29T21:13:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115663",
    "user": "https://github.com/jdemeyer"
}
```

Changed dependencies from **#11143** to none



---

archive/issue_events_132482.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-06-30T18:41:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11310#event-132482"
}
```



---

archive/issue_events_132483.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-06-30T18:41:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11310#event-132483"
}
```



---

archive/issue_comments_115664.json:
```json
{
    "body": "Dependencies: **#13109**",
    "created_at": "2012-06-30T18:41:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115664",
    "user": "https://github.com/vbraun"
}
```

Dependencies: **#13109**



---

archive/issue_comments_115665.json:
```json
{
    "body": "Rediffed patch",
    "created_at": "2012-06-30T18:44:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115665",
    "user": "https://github.com/vbraun"
}
```

Rediffed patch



---

archive/issue_events_132484.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-06-30T18:45:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11310#event-132484"
}
```



---

archive/issue_events_132485.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-06-30T18:45:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11310#event-132485"
}
```



---

archive/issue_comments_115666.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -32,4 +32,4 @@\n \n ---\n \n-**Apply** [and [attachment: 11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz)(https://github.com/sagemath/sage/files/ticket11310/6c0e4b5ddb9fedd780d94f484c9d3b42.gz)\n+**Apply** [and [attachment: 11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz)(https://github.com/sagemath/sage/files/ticket11310/e5d245395abd8a7387695d22926f8f23.gz)\n``````\n",
    "created_at": "2012-06-30T18:45:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115666",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -32,4 +32,4 @@
 
 ---
 
-**Apply** [and [attachment: 11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz)(https://github.com/sagemath/sage/files/ticket11310/6c0e4b5ddb9fedd780d94f484c9d3b42.gz)
+**Apply** [and [attachment: 11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz)(https://github.com/sagemath/sage/files/ticket11310/e5d245395abd8a7387695d22926f8f23.gz)
``````




---

archive/issue_comments_115667.json:
```json
{
    "body": "Changed author from **Jeroen Demeyer** to **Jeroen Demeyer, Volker Braun**",
    "created_at": "2012-06-30T18:45:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115667",
    "user": "https://github.com/vbraun"
}
```

Changed author from **Jeroen Demeyer** to **Jeroen Demeyer, Volker Braun**



---

archive/issue_comments_115668.json:
```json
{
    "body": "<a id='comment:24'>Comment 24:</a>\nAttachment: **[trac_11310.patch.gz](https://github.com/sagemath/sage/files/ticket11310/trac_11310.patch.gz)**\n\nThe new patch just fixes some fuzz 2 with #13109, back to positive review.",
    "created_at": "2012-06-30T18:45:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115668",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:24'>Comment 24:</a>
Attachment: **[trac_11310.patch.gz](https://github.com/sagemath/sage/files/ticket11310/trac_11310.patch.gz)**

The new patch just fixes some fuzz 2 with #13109, back to positive review.



---

archive/issue_events_132486.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-01T18:51:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "milestone_number": null,
    "milestone_title": "sage-5.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11310#event-132486"
}
```



---

archive/issue_events_132487.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-01T18:51:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "label": "https://github.com/sagemath/sage/labels/pending",
    "label_color": "008080",
    "label_name": "pending",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11310#event-132487"
}
```



---

archive/issue_events_132488.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-07-09T15:02:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "milestone_number": null,
    "milestone_title": "sage-5.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11310#event-132488"
}
```



---

archive/issue_events_132489.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-07-09T15:02:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "label": "https://github.com/sagemath/sage/labels/pending",
    "label_color": "008080",
    "label_name": "pending",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11310#event-132489"
}
```



---

archive/issue_comments_115669.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -7,8 +7,7 @@\n \n ---\n \n-Then the patch [attachment: 11310.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310.patch.gz) fixes some of these.\n-\n+The patch [attachment: trac_11310.patch.gz](https://github.com/sagemath/sage/files/ticket11310/trac_11310.patch.gz) fixes some of these.\n Then the patch [attachment: 11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz) has ben auto-generated to replace all remaining instances of\n \n ```\n``````\n",
    "created_at": "2012-08-01T12:40:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115669",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -7,8 +7,7 @@
 
 ---
 
-Then the patch [attachment: 11310.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310.patch.gz) fixes some of these.
-
+The patch [attachment: trac_11310.patch.gz](https://github.com/sagemath/sage/files/ticket11310/trac_11310.patch.gz) fixes some of these.
 Then the patch [attachment: 11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz) has ben auto-generated to replace all remaining instances of
 
 ```
``````




---

archive/issue_comments_115670.json:
```json
{
    "body": "Additional patch",
    "created_at": "2012-08-02T12:57:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115670",
    "user": "https://github.com/jdemeyer"
}
```

Additional patch



---

archive/issue_comments_115671.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -7,8 +7,7 @@\n \n ---\n \n-The patch [attachment: trac_11310.patch.gz](https://github.com/sagemath/sage/files/ticket11310/trac_11310.patch.gz) fixes some of these.\n-Then the patch [attachment: 11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz) has ben auto-generated to replace all remaining instances of\n+Then the patch [attachment: 11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz) has ben auto-generated to replace all instances of\n \n ```\n except:\n@@ -31,4 +30,4 @@\n \n ---\n \n-**Apply** [and [attachment: 11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz)(https://github.com/sagemath/sage/files/ticket11310/e5d245395abd8a7387695d22926f8f23.gz)\n+**Apply** [and [attachment: 11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz) and [attachment: 11310_psage.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_psage.patch.gz)(https://github.com/sagemath/sage/files/ticket11310/e5d245395abd8a7387695d22926f8f23.gz).\n``````\n",
    "created_at": "2012-08-02T12:58:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115671",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -7,8 +7,7 @@
 
 ---
 
-The patch [attachment: trac_11310.patch.gz](https://github.com/sagemath/sage/files/ticket11310/trac_11310.patch.gz) fixes some of these.
-Then the patch [attachment: 11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz) has ben auto-generated to replace all remaining instances of
+Then the patch [attachment: 11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz) has ben auto-generated to replace all instances of
 
 ```
 except:
@@ -31,4 +30,4 @@
 
 ---
 
-**Apply** [and [attachment: 11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz)(https://github.com/sagemath/sage/files/ticket11310/e5d245395abd8a7387695d22926f8f23.gz)
+**Apply** [and [attachment: 11310_auto.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_auto.patch.gz) and [attachment: 11310_psage.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_psage.patch.gz)(https://github.com/sagemath/sage/files/ticket11310/e5d245395abd8a7387695d22926f8f23.gz).
``````




---

archive/issue_events_132490.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-08-02T12:58:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11310#event-132490"
}
```



---

archive/issue_events_132491.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-08-02T12:58:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11310#event-132491"
}
```



---

archive/issue_comments_115672.json:
```json
{
    "body": "<a id='comment:28'>Comment 28:</a>\nAttachment: **[11310_psage.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_psage.patch.gz)**\n\nAdditional patch [attachment: 11310_psage.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_psage.patch.gz) needs review (there were sporadic doctest failures in `psage.py`).",
    "created_at": "2012-08-02T12:58:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115672",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:28'>Comment 28:</a>
Attachment: **[11310_psage.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_psage.patch.gz)**

Additional patch [attachment: 11310_psage.patch.gz](https://github.com/sagemath/sage/files/ticket11310/11310_psage.patch.gz) needs review (there were sporadic doctest failures in `psage.py`).



---

archive/issue_events_132492.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-08-02T12:58:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11310#event-132492"
}
```



---

archive/issue_events_132493.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-08-02T12:58:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11310#event-132493"
}
```



---

archive/issue_events_132494.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-08-02T13:05:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11310#event-132494"
}
```



---

archive/issue_events_132495.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-08-02T13:05:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11310#event-132495"
}
```



---

archive/issue_comments_115673.json:
```json
{
    "body": "<a id='comment:30'>Comment 30:</a>\nLooks good! \n\nIn a perfect world `ExceptionPexpect` would derive from `StandardError` and not the raw exceptions, but oh well.",
    "created_at": "2012-08-02T13:05:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115673",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:30'>Comment 30:</a>
Looks good! 

In a perfect world `ExceptionPexpect` would derive from `StandardError` and not the raw exceptions, but oh well.



---

archive/issue_comments_115674.json:
```json
{
    "body": "<a id='comment:31'>Comment 31:</a>\npsage is a weird beast. I have seen doctest errors in psage which change from version to version. I couldn't explain it, nor do I know the fix. See #12061.",
    "created_at": "2012-08-02T13:13:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115674",
    "user": "https://github.com/ppurka"
}
```

<a id='comment:31'>Comment 31:</a>
psage is a weird beast. I have seen doctest errors in psage which change from version to version. I couldn't explain it, nor do I know the fix. See #12061.



---

archive/issue_comments_115675.json:
```json
{
    "body": "Merged: **sage-5.3.beta1**",
    "created_at": "2012-08-12T18:58:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115675",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.3.beta1**



---

archive/issue_events_132496.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-08-12T18:58:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11310#event-132496"
}
```



---

archive/issue_events_132497.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-08-12T18:58:35Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11310#event-132497"
}
```



---

archive/issue_comments_115676.json:
```json
{
    "body": "<a id='comment:33'>Comment 33:</a>\nThis causes issues with Python 3. See #15620.",
    "created_at": "2014-01-02T15:58:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11310",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11310#issuecomment-115676",
    "user": "https://github.com/ohanar"
}
```

<a id='comment:33'>Comment 33:</a>
This causes issues with Python 3. See #15620.
