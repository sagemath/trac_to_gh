# Issue 11231: Sparse and dense polynomials hash differently, but are equal.

archive/issues_011059.json:
```json
{
    "assignees": [
        "https://github.com/jasongrout"
    ],
    "body": "\n```\nsage: R = PolynomialRing(ZZ, 'x', sparse=False)\nsage: S = PolynomialRing(ZZ, 'x', sparse=True)\nsage: R\nUnivariate Polynomial Ring in x over Integer Ring\nsage: S\nSparse Univariate Polynomial Ring in x over Integer Ring\nsage: R == S\nTrue\nsage: hash(R) == hash(S)\nFalse\n```\n\nIt's unclear what equality really means here... \n\nReviewer: **Christian Nassau**\n\nComponent: **misc**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11231_\n\n",
    "closed_at": "2012-12-21T22:48:41Z",
    "created_at": "2011-04-21T09:26:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/worksforme"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Sparse and dense polynomials hash differently, but are equal.",
    "type": "issue",
    "updated_at": "2012-12-21T22:48:41Z",
    "url": "https://github.com/sagemath/sage/issues/11231",
    "user": "https://github.com/robertwb"
}
```

```
sage: R = PolynomialRing(ZZ, 'x', sparse=False)
sage: S = PolynomialRing(ZZ, 'x', sparse=True)
sage: R
Univariate Polynomial Ring in x over Integer Ring
sage: S
Sparse Univariate Polynomial Ring in x over Integer Ring
sage: R == S
True
sage: hash(R) == hash(S)
False
```

It's unclear what equality really means here... 

Reviewer: **Christian Nassau**

Component: **misc**

_Issue created by migration from https://trac.sagemath.org/ticket/11231_





---

archive/issue_events_148748.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2011-04-21T09:26:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11231",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11231#event-148748"
}
```



---

archive/issue_events_148749.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2011-04-21T09:26:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11231",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11231#event-148749"
}
```



---

archive/issue_events_148750.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2011-04-21T09:26:36Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/11231",
    "subject": "https://github.com/robertwb",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11231#event-148750"
}
```



---

archive/issue_events_148751.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2012-12-10T21:34:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11231",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11231#event-148751"
}
```



---

archive/issue_events_148752.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2012-12-10T21:34:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11231",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11231#event-148752"
}
```



---

archive/issue_events_148753.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2012-12-10T21:34:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11231",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11231#event-148753"
}
```



---

archive/issue_comments_112181.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nIn `5.5.rc0`, I get the following:\n\n```\nsage: R = PolynomialRing(ZZ, 'x', sparse=False)\nsage: S = PolynomialRing(ZZ, 'x', sparse=True)\nsage: R\nUnivariate Polynomial Ring in x over Integer Ring\nsage: S\nSparse Univariate Polynomial Ring in x over Integer Ring\nsage: R == S\nFalse\nsage: hash(R) == hash(S) # on a 32-bit VM\nTrue\n```\nHowever this isn't a problem.",
    "created_at": "2012-12-10T21:34:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11231",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11231#issuecomment-112181",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:1" align="right">comment:1</div>

In `5.5.rc0`, I get the following:

```
sage: R = PolynomialRing(ZZ, 'x', sparse=False)
sage: S = PolynomialRing(ZZ, 'x', sparse=True)
sage: R
Univariate Polynomial Ring in x over Integer Ring
sage: S
Sparse Univariate Polynomial Ring in x over Integer Ring
sage: R == S
False
sage: hash(R) == hash(S) # on a 32-bit VM
True
```
However this isn't a problem.



---

archive/issue_comments_112182.json:
```json
{
    "body": "Reviewer: **cnassau**",
    "created_at": "2012-12-11T18:16:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11231",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11231#issuecomment-112182",
    "user": "https://github.com/cnassau"
}
```

Reviewer: **cnassau**



---

archive/issue_comments_112183.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nI've checked that, indeed, already Sage 5.4.1 gives this answer (different objects, same hash). And I would love to give a ticket a positive review (my first), so here we go...",
    "created_at": "2012-12-11T18:16:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11231",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11231#issuecomment-112183",
    "user": "https://github.com/cnassau"
}
```

<div id="comment:2" align="right">comment:2</div>

I've checked that, indeed, already Sage 5.4.1 gives this answer (different objects, same hash). And I would love to give a ticket a positive review (my first), so here we go...



---

archive/issue_events_148754.json:
```json
{
    "actor": "https://github.com/cnassau",
    "created_at": "2012-12-11T18:16:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11231",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11231#event-148754"
}
```



---

archive/issue_events_148755.json:
```json
{
    "actor": "https://github.com/cnassau",
    "created_at": "2012-12-11T18:16:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11231",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11231#event-148755"
}
```



---

archive/issue_comments_112184.json:
```json
{
    "body": "Changed reviewer from **cnassau** to **Christian Nassau**",
    "created_at": "2012-12-21T22:48:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11231",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11231#issuecomment-112184",
    "user": "https://github.com/jdemeyer"
}
```

Changed reviewer from **cnassau** to **Christian Nassau**



---

archive/issue_events_148756.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-12-21T22:48:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11231",
    "label": "https://github.com/sagemath/sage/labels/worksforme",
    "label_color": "a6a6a6",
    "label_name": "worksforme",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11231#event-148756"
}
```



---

archive/issue_events_148757.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-12-21T22:48:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11231",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11231#event-148757"
}
```



---

archive/issue_events_148758.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-12-21T22:48:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11231",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11231#event-148758"
}
```
