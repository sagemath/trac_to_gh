...
```json
{
    "body": "The problem isn't really R itself, but our readline spkg.  (Cf. discussions on sage-release where this `configure` error was reported for Ubuntu 11.10 systems that have libreadline-dev installed.)\n\nHere's a preliminary new readline spkg:\n\n [http://sage.math.washington.edu/home/leif/Sage/spkgs/readline-6.2.p2.spkg](http://sage.math.washington.edu/home/leif/Sage/spkgs/readline-6.2.p2.spkg)",
...
```json
{
    "body": "Btw., since Sage also ships a termcap package, we should perhaps also build a *shared* library of that as well, at least if the systems lacks one.\n\nOr, if the system *does* have a \"devel version\" of libtermcap (headers, static library and an \"unversioned\" `.so`), don't build / install Sage's at all.",
...
```json
{
    "body": "As discussed on irc, the readline package compiled, and I'm now managing to compile R with even with the previously problematic headers installed.",
...
```json
{
    "body": "I used Leif's spkg above with 4.8.alpha4, and although the build finished, Sage doesn't start; I get a segfault from IPython. I'm using Ubuntu 11.10. Is anyone else seeing this?",
...
```json
{
    "body": "These issues with segfaults on startup are being addressed at #12157.",
...
```json
{
    "body": "**bump** Leif: Can you merge the one-line change from that readline into a .p3 (starting from #12131) and clean it up for review?",
...
```json
{
    "body": "I've posted an updated version of Leif's spkg, although without the changes to `spkg-check`.",
    "created_at": "2012-01-03T22:28:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11970",
    "type": "issue_comment",
...
Attachment [trac_11970-readline.patch](tarball://root/attachments/some-uuid/ticket11970/trac_11970-readline.patch) by @JohnCremona created at 2012-01-05 11:37:16
...
```json
{
    "body": "The spkg looks good on first sight.  I can of course not test it on every system.  So I'm willing to give it **positive_review** if somebody can confirm that it actually fixes the problem on Ubuntu 11.10.  I asked Paul Zimmermann to do this.",
...
```json
{
    "body": "Deleting the file `$SAGE_ROOT/local/lib/libtermcap.a` solves the problem.  Somehow that static-only library confuses the linker.  Not sure whether this is a linker bug or feature.",
...
```json
{
    "body": "Note: the output of `readline-6.2.p3.log` is exactly the same whether or not `local/lib/libtermcap.a` exists.  From my observations above, I would expect the readline build to fail when `local/bin/libtermcap.a` exists.  But it doesn't, so maybe it finds a broken `tgetent()` somewhere somehow.",
...
```json
{
    "body": "That's it for now, going to sleep...",
...
```json
{
    "body": "FWIW, I've made a p4 (changes not yet committed) that unsets `LIBRARY_PATH`:\n\n  http://sage.math.washington.edu/home/leif/Sage/spkgs/readline-6.2.p4.spkg",
...
```json
{
    "body": "I think I got to the bottom of the `tgetent()` segmentation fault: it is a bug in the termcap library.  Under normal circumstances, `strcmp()` can be called with a NULL pointer argument.",
...
```json
{
    "body": "See #12282 for the termcap issue.",
...
```json
{
    "body": "Attachment [readline-6.2.p3-p4.diff](tarball://root/attachments/some-uuid/ticket11970/readline-6.2.p3-p4.diff) by @nexttime created at 2012-01-09 19:34:24\n\nI've attached a diff between the p3 and the p4; the changes aren't yet committed though.",
...
Attachment [readline-6.2.p3-p4.diff](tarball://root/attachments/some-uuid/ticket11970/readline-6.2.p3-p4.diff) by @nexttime created at 2012-01-09 19:34:24
...
```json
{
    "body": "thanks Leif, I missed the last step (reinstalling the python spkg). It works now.\n\nPaul",
