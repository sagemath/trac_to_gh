# Issue 11650: Make 'convert' the standard way to produce animated gifs (again)

archive/issues_011478.json:
```json
{
    "body": "In ticket #11170, code was added so that if ffmpeg is present, it gets used by default to produce animated gifs.  These can be much larger than the ones produced by 'convert', plus there may be other problems with ffmpeg.  The attached patch makes 'convert' the default again.\n\n\nAssignee: jason, was\n\nKeywords: animate convert ffmpeg\n\nResolution: fixed\n\nDependencies: #11170\n\nAuthor: John Palmieri\n\nReviewer: Dan Drake\n\nMerged: sage-4.8.alpha3\n\nIssue created by migration from https://trac.sagemath.org/ticket/11650\n\n",
    "closed_at": "2011-11-23T13:25:13Z",
    "created_at": "2011-08-04T17:03:03Z",
    "labels": [
        "component: graphics",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.8",
    "title": "Make 'convert' the standard way to produce animated gifs (again)",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/11650",
    "user": "https://github.com/jhpalmieri"
}
```
In ticket #11170, code was added so that if ffmpeg is present, it gets used by default to produce animated gifs.  These can be much larger than the ones produced by 'convert', plus there may be other problems with ffmpeg.  The attached patch makes 'convert' the default again.


Assignee: jason, was

Keywords: animate convert ffmpeg

Resolution: fixed

Dependencies: #11170

Author: John Palmieri

Reviewer: Dan Drake

Merged: sage-4.8.alpha3

Issue created by migration from https://trac.sagemath.org/ticket/11650





---

archive/issue_comments_156708.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2011-08-04T17:04:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11650",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11650#issuecomment-156708",
    "user": "https://github.com/jhpalmieri"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_156709.json:
```json
{
    "body": "Changing dependencies from \"\" to \"11170\"",
    "created_at": "2011-08-05T06:21:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11650",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11650#issuecomment-156709",
    "user": "https://github.com/kini"
}
```

Changing dependencies from "" to "11170"



---

archive/issue_comments_156710.json:
```json
{
    "body": "<a id='comment:2'></a>Just by the by, what do you think about bundling GraphicsMagick (a fork of ImageMagick by a group of its original developers) with Sage? Apparently it is faster than ImageMagick (I'm told, by a user, that it is faster by a factor of three for these animation type things). It's also X11-licensed, so if I understand correctly it is possible to bundle it, unlike ImageMagick. The source occupies ~38 MB.",
    "created_at": "2011-08-05T06:21:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11650",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11650#issuecomment-156710",
    "user": "https://github.com/kini"
}
```

<a id='comment:2'></a>Just by the by, what do you think about bundling GraphicsMagick (a fork of ImageMagick by a group of its original developers) with Sage? Apparently it is faster than ImageMagick (I'm told, by a user, that it is faster by a factor of three for these animation type things). It's also X11-licensed, so if I understand correctly it is possible to bundle it, unlike ImageMagick. The source occupies ~38 MB.



---

archive/issue_comments_156711.json:
```json
{
    "body": "Changing dependencies from \"11170\" to \"#11170\"",
    "created_at": "2011-08-05T06:28:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11650",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11650#issuecomment-156711",
    "user": "https://github.com/kini"
}
```

Changing dependencies from "11170" to "#11170"



---

archive/issue_comments_156712.json:
```json
{
    "body": "<a id='comment:4'></a>I've never used GraphicsMagick, but I'll take a look some time.  It would be nice to include it, if it's that small.  Does it need libjpeg or anything else like that?",
    "created_at": "2011-08-10T19:22:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11650",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11650#issuecomment-156712",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:4'></a>I've never used GraphicsMagick, but I'll take a look some time.  It would be nice to include it, if it's that small.  Does it need libjpeg or anything else like that?



---

archive/issue_comments_156713.json:
```json
{
    "body": "<a id='comment:5'></a>This looks good. Two notes:\n\n* You can't pass `use_ffmpeg=True` when using `.save()`. If it's easy to do, can `.save()` pass that onto `.gif()`? Or at least change the docstring in `.save()` to tell users to go directly to `.gif()` if they want to use ffmpeg.\n\n* Somehow, the `delay` parameter is now a string:\n\n```\nsage: a.gif('bar2.gif', use_ffmpeg=True)\n---------------------------------------------------------------------------\nTypeError                                 Traceback (most recent call last)\n\n/home/drake/s/sage-4.7.2.alpha2/<ipython console> in <module>()\n\n/home/drake/s/sage-4.7.2.alpha2/local/lib/python2.6/site-packages/sage/plot/animate.pyc in gif(self, delay, savefile, iterations, show_path, use_ffmpeg)\n    361                 self.ffmpeg(savefile=savefile, show_path=show_path,\n    362                             output_format='gif', delay=delay,\n--> 363                             iterations=iterations)\n    364             else:\n    365                 if not have_convert:\n\n/home/drake/s/sage-4.7.2.alpha2/local/lib/python2.6/site-packages/sage/plot/animate.pyc in ffmpeg(self, savefile, show_path, output_format, ffmpeg_options, delay, iterations, verbose)\n    566                 ffmpeg_options += ' -pix_fmt rgb24 -loop_output %s ' % iterations\n    567             if delay is not None and output_format != 'mpeg' and output_format != 'mpg':\n--> 568                 early_options += ' -r %s -g 3 ' % int(100/delay)\n    569             savefile = os.path.abspath(savefile)\n    570             pngdir = self.png()\n\nTypeError: unsupported operand type(s) for /: 'int' and 'str'\n```\nCalling `.ffmpeg()` without the patch here works fine.",
    "created_at": "2011-08-31T03:16:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11650",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11650#issuecomment-156713",
    "user": "https://github.com/dandrake"
}
```

<a id='comment:5'></a>This looks good. Two notes:

* You can't pass `use_ffmpeg=True` when using `.save()`. If it's easy to do, can `.save()` pass that onto `.gif()`? Or at least change the docstring in `.save()` to tell users to go directly to `.gif()` if they want to use ffmpeg.

* Somehow, the `delay` parameter is now a string:

```
sage: a.gif('bar2.gif', use_ffmpeg=True)
---------------------------------------------------------------------------
TypeError                                 Traceback (most recent call last)

/home/drake/s/sage-4.7.2.alpha2/<ipython console> in <module>()

/home/drake/s/sage-4.7.2.alpha2/local/lib/python2.6/site-packages/sage/plot/animate.pyc in gif(self, delay, savefile, iterations, show_path, use_ffmpeg)
    361                 self.ffmpeg(savefile=savefile, show_path=show_path,
    362                             output_format='gif', delay=delay,
--> 363                             iterations=iterations)
    364             else:
    365                 if not have_convert:

/home/drake/s/sage-4.7.2.alpha2/local/lib/python2.6/site-packages/sage/plot/animate.pyc in ffmpeg(self, savefile, show_path, output_format, ffmpeg_options, delay, iterations, verbose)
    566                 ffmpeg_options += ' -pix_fmt rgb24 -loop_output %s ' % iterations
    567             if delay is not None and output_format != 'mpeg' and output_format != 'mpg':
--> 568                 early_options += ' -r %s -g 3 ' % int(100/delay)
    569             savefile = os.path.abspath(savefile)
    570             pngdir = self.png()

TypeError: unsupported operand type(s) for /: 'int' and 'str'
```
Calling `.ffmpeg()` without the patch here works fine.



---

archive/issue_comments_156714.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2011-08-31T03:16:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11650",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11650#issuecomment-156714",
    "user": "https://github.com/dandrake"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_156715.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2011-09-08T20:25:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11650",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11650#issuecomment-156715",
    "user": "https://github.com/jhpalmieri"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_156716.json:
```json
{
    "body": "<a id='comment:6'></a>Here's a new patch; the only changes are the following:\n\n```diff\ndiff --git a/sage/plot/animate.py b/sage/plot/animate.py\n--- a/sage/plot/animate.py\n+++ b/sage/plot/animate.py\n@@ -587,7 +587,7 @@ please install it and try again.\"\"\"\n                 print \"Error running ffmpeg.\"\n                 raise\n \n-    def save(self, filename=None, show_path=False):\n+    def save(self, filename=None, show_path=False, use_ffmpeg=False):\n         \"\"\"\n         Save this animation.\n \n@@ -598,6 +598,10 @@ please install it and try again.\"\"\"\n         -  ``show_path`` - boolean (default: False); if True,\n            print the path to the saved file\n \n+        - ``use_ffmpeg`` - boolean (default: False); if True, use\n+          'ffmpeg' by default instead of 'convert' when creating GIF\n+          files.\n+\n         If filename is None, then in notebook mode, display the\n         animation; otherwise, save the animation to a GIF file.  If\n         filename ends in '.sobj', save to an sobj file.  Otherwise,\n@@ -632,7 +636,8 @@ please install it and try again.\"\"\"\n                 suffix = '.gif'\n \n         if filename is None or suffix == '.gif':\n-            self.gif(savefile=filename, show_path=show_path)\n+            self.gif(savefile=filename, show_path=show_path,\n+                     use_ffmpeg=use_ffmpeg)\n             return\n         elif suffix == '.sobj':\n             SageObject.save(self, filename)\n```\nThis allows you to do `a.save('foobar.gif', use_ffmpeg=True)`.\n\nI'm not seeing the issue with `delay` being a string.  Can you please try again?",
    "created_at": "2011-09-08T20:25:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11650",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11650#issuecomment-156716",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:6'></a>Here's a new patch; the only changes are the following:

```diff
diff --git a/sage/plot/animate.py b/sage/plot/animate.py
--- a/sage/plot/animate.py
+++ b/sage/plot/animate.py
@@ -587,7 +587,7 @@ please install it and try again."""
                 print "Error running ffmpeg."
                 raise
 
-    def save(self, filename=None, show_path=False):
+    def save(self, filename=None, show_path=False, use_ffmpeg=False):
         """
         Save this animation.
 
@@ -598,6 +598,10 @@ please install it and try again."""
         -  ``show_path`` - boolean (default: False); if True,
            print the path to the saved file
 
+        - ``use_ffmpeg`` - boolean (default: False); if True, use
+          'ffmpeg' by default instead of 'convert' when creating GIF
+          files.
+
         If filename is None, then in notebook mode, display the
         animation; otherwise, save the animation to a GIF file.  If
         filename ends in '.sobj', save to an sobj file.  Otherwise,
@@ -632,7 +636,8 @@ please install it and try again."""
                 suffix = '.gif'
 
         if filename is None or suffix == '.gif':
-            self.gif(savefile=filename, show_path=show_path)
+            self.gif(savefile=filename, show_path=show_path,
+                     use_ffmpeg=use_ffmpeg)
             return
         elif suffix == '.sobj':
             SageObject.save(self, filename)
```
This allows you to do `a.save('foobar.gif', use_ffmpeg=True)`.

I'm not seeing the issue with `delay` being a string.  Can you please try again?



---

archive/issue_comments_156717.json:
```json
{
    "body": "Attachment [trac_11650-animate.patch](tarball://root/attachments/some-uuid/ticket11650/trac_11650-animate.patch) by @jhpalmieri created at 2011-09-08 20:25:47",
    "created_at": "2011-09-08T20:25:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11650",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11650#issuecomment-156717",
    "user": "https://github.com/jhpalmieri"
}
```

Attachment [trac_11650-animate.patch](tarball://root/attachments/some-uuid/ticket11650/trac_11650-animate.patch) by @jhpalmieri created at 2011-09-08 20:25:47



---

archive/issue_comments_156718.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2011-11-18T04:38:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11650",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11650#issuecomment-156718",
    "user": "https://github.com/dandrake"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_156719.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Dan Drake\"",
    "created_at": "2011-11-18T04:38:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11650",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11650#issuecomment-156719",
    "user": "https://github.com/dandrake"
}
```

Changing reviewer from "" to "Dan Drake"



---

archive/issue_comments_156720.json:
```json
{
    "body": "<a id='comment:7'></a>Now this looks good. Passes all doctests and works properly. I don't know what happened with the `delay` option; it now works.",
    "created_at": "2011-11-18T04:38:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11650",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11650#issuecomment-156720",
    "user": "https://github.com/dandrake"
}
```

<a id='comment:7'></a>Now this looks good. Passes all doctests and works properly. I don't know what happened with the `delay` option; it now works.



---

archive/issue_events_030582.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-23T13:25:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/11650",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11650#event-30582"
}
```



---

archive/issue_comments_156721.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2011-11-23T13:25:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11650",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11650#issuecomment-156721",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed



---

archive/issue_comments_156722.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-4.8.alpha3\"",
    "created_at": "2011-11-23T13:25:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11650",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11650#issuecomment-156722",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-4.8.alpha3"
