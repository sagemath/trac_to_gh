# Issue 11919: Issue when pickling a formal function

archive/issues_011747.json:
```json
{
    "assignees": [
        "https://github.com/burcin"
    ],
    "body": "There is a problem in symbolic function pickling:\n\n```\nsage: x = var('x'); f = function('f',x) ; s = dumps(f) ; loads(s)\nRuntimeError: unknown function 'f' in archive\n```\n\nThe error was not present in Sage 4.7 but it is in newer versions.\nIt was suggested (http://groups.google.com/group/sage-support/browse_thread/thread/b439844f2fa0b675) that it is related to Pynac.\nPynac was updated to 0.2.2 (sage-4.7.1.alpha1) and then to 0.2.3 \n(sage-4.7.1.alpha4) according to sage 4.7.1 changelog.\n\nCC:  @jpflori @burcin @jasongrout @sagetrac-dsm @orlitzky\n\nKeywords: **pynac, pickling**\n\nReviewer: **Nils Bruin, Burcin Erocal**\n\nAuthor: **Michael Orlitzky**\n\nMerged: **sage-5.1.beta1**\n\nComponent: **symbolics**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11919_\n\n",
    "closed_at": "2012-05-23T21:31:59Z",
    "created_at": "2011-10-13T15:52:44Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Issue when pickling a formal function",
    "type": "issue",
    "updated_at": "2012-09-11T14:14:06Z",
    "url": "https://github.com/sagemath/sage/issues/11919",
    "user": "https://github.com/sagetrac-cdsousa"
}
```
There is a problem in symbolic function pickling:

```
sage: x = var('x'); f = function('f',x) ; s = dumps(f) ; loads(s)
RuntimeError: unknown function 'f' in archive
```

The error was not present in Sage 4.7 but it is in newer versions.
It was suggested (http://groups.google.com/group/sage-support/browse_thread/thread/b439844f2fa0b675) that it is related to Pynac.
Pynac was updated to 0.2.2 (sage-4.7.1.alpha1) and then to 0.2.3 
(sage-4.7.1.alpha4) according to sage 4.7.1 changelog.

CC:  @jpflori @burcin @jasongrout @sagetrac-dsm @orlitzky

Keywords: **pynac, pickling**

Reviewer: **Nils Bruin, Burcin Erocal**

Author: **Michael Orlitzky**

Merged: **sage-5.1.beta1**

Component: **symbolics**

_Issue created by migration from https://trac.sagemath.org/ticket/11919_





---

archive/issue_events_160830.json:
```json
{
    "actor": "https://github.com/sagetrac-cdsousa",
    "created_at": "2011-10-13T15:52:44Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-160830"
}
```



---

archive/issue_events_160831.json:
```json
{
    "actor": "https://github.com/sagetrac-cdsousa",
    "created_at": "2011-10-13T15:52:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20pickling",
    "label_color": "000080",
    "label_name": "c: pickling",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-160831"
}
```



---

archive/issue_events_160832.json:
```json
{
    "actor": "https://github.com/sagetrac-cdsousa",
    "created_at": "2011-10-13T15:52:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-160832"
}
```



---

archive/issue_events_160833.json:
```json
{
    "actor": "https://github.com/sagetrac-cdsousa",
    "created_at": "2011-10-13T15:52:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-160833"
}
```



---

archive/issue_events_160834.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-10-13T15:52:44Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "subject": "https://github.com/sagetrac-cdsousa",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-160834"
}
```



---

archive/issue_comments_125762.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -8,4 +8,4 @@\n The error was not present in Sage 4.7 but it is in newer versions.\n It was suggested (http://groups.google.com/group/sage-support/browse_thread/thread/b439844f2fa0b675) that it is related to Pynac.\n Pynac was updated to 0.2.2 (sage-4.7.1.alpha1) and then to 0.2.3 \n-(sage-4.7.1.alpha4) according to sage 7.4.1 changelog.\n+(sage-4.7.1.alpha4) according to sage 4.7.1 changelog.\n``````\n",
    "created_at": "2011-10-14T12:34:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-125762",
    "user": "https://github.com/kcrisman"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -8,4 +8,4 @@
 The error was not present in Sage 4.7 but it is in newer versions.
 It was suggested (http://groups.google.com/group/sage-support/browse_thread/thread/b439844f2fa0b675) that it is related to Pynac.
 Pynac was updated to 0.2.2 (sage-4.7.1.alpha1) and then to 0.2.3 
-(sage-4.7.1.alpha4) according to sage 7.4.1 changelog.
+(sage-4.7.1.alpha4) according to sage 4.7.1 changelog.
``````




---

archive/issue_comments_125763.json:
```json
{
    "body": "Changed keywords from none to **pynac, pickling**",
    "created_at": "2011-10-15T19:45:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-125763",
    "user": "https://github.com/nbruin"
}
```

Changed keywords from none to **pynac, pickling**



---

archive/issue_events_160835.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-10-15T19:45:46Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "subject": "https://github.com/nbruin",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-160835"
}
```



---

archive/issue_events_160836.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2011-10-15T19:45:46Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "subject": "https://github.com/nbruin",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-160836"
}
```



---

archive/issue_events_160837.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2011-10-15T19:45:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20pickling",
    "label_color": "000080",
    "label_name": "c: pickling",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-160837"
}
```



---

archive/issue_events_160838.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2011-10-15T19:45:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "c: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-160838"
}
```



---

archive/issue_comments_125764.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nI hit this independently using deepcopy(). In sage-4.7:\n\n```\nsage: f = function('f',x)\nsage: deepcopy(f)\nf(x)\n```\n\nand in 4.8.alpha5:\n\n```\nsage: f = function('f',x)\nsage: deepcopy(f)\n...\nRuntimeError: unknown function 'f' in archive\n```",
    "created_at": "2011-12-26T21:51:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-125764",
    "user": "https://github.com/orlitzky"
}
```

<div id="comment:5" align="right">comment:5</div>

I hit this independently using deepcopy(). In sage-4.7:

```
sage: f = function('f',x)
sage: deepcopy(f)
f(x)
```

and in 4.8.alpha5:

```
sage: f = function('f',x)
sage: deepcopy(f)
...
RuntimeError: unknown function 'f' in archive
```



---

archive/issue_comments_125765.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nInterestingly, if I take the pynac spkg from 4.7 and install it on 5.0.beta7, it doesn't fix the crash.",
    "created_at": "2012-03-11T02:02:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-125765",
    "user": "https://github.com/orlitzky"
}
```

<div id="comment:6" align="right">comment:6</div>

Interestingly, if I take the pynac spkg from 4.7 and install it on 5.0.beta7, it doesn't fix the crash.



---

archive/issue_comments_125766.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nI built a copy of 4.7.1, and the crash does happen. So the change was introduced between 4.7 and 4.7.1.",
    "created_at": "2012-03-12T18:05:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-125766",
    "user": "https://github.com/orlitzky"
}
```

<div id="comment:7" align="right">comment:7</div>

I built a copy of 4.7.1, and the crash does happen. So the change was introduced between 4.7 and 4.7.1.



---

archive/issue_comments_125767.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nOkay, this broke with #9240. I can fix it with a one-line change in 4.7.1.alpha4:\n\n```\ndiff --git a/sage/symbolic/function.pyx b/sage/symbolic/function.pyx\n--- a/sage/symbolic/function.pyx\n+++ b/sage/symbolic/function.pyx\n@@ -127,6 +127,7 @@\n     cdef _register_function(self):\n         cdef GFunctionOpt opt\n         opt = g_function_options_args(self._name, self._nargs)\n+        opt.set_python_func()\n\n         if hasattr(self, '_eval_'):\n             opt.eval_func(self)\n```\n\nBut there's no longer a `set_python_func()` method on GFunctionOpt, so I don't know how to fix it in 5.0.beta7. The adventure continues...",
    "created_at": "2012-03-13T02:56:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-125767",
    "user": "https://github.com/orlitzky"
}
```

<div id="comment:8" align="right">comment:8</div>

Okay, this broke with #9240. I can fix it with a one-line change in 4.7.1.alpha4:

```
diff --git a/sage/symbolic/function.pyx b/sage/symbolic/function.pyx
--- a/sage/symbolic/function.pyx
+++ b/sage/symbolic/function.pyx
@@ -127,6 +127,7 @@
     cdef _register_function(self):
         cdef GFunctionOpt opt
         opt = g_function_options_args(self._name, self._nargs)
+        opt.set_python_func()

         if hasattr(self, '_eval_'):
             opt.eval_func(self)
```

But there's no longer a `set_python_func()` method on GFunctionOpt, so I don't know how to fix it in 5.0.beta7. The adventure continues...



---

archive/issue_comments_125768.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nReplying to [@orlitzky](#comment:8):\n> But there's no longer a `set_python_func()` method on GFunctionOpt, so I don't know how to fix it in 5.0.beta7. The adventure continues...\n\nDisregard that, I'm a moron. Testing a patch now.",
    "created_at": "2012-03-13T03:23:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-125768",
    "user": "https://github.com/orlitzky"
}
```

<div id="comment:9" align="right">comment:9</div>

Replying to [@orlitzky](#comment:8):
> But there's no longer a `set_python_func()` method on GFunctionOpt, so I don't know how to fix it in 5.0.beta7. The adventure continues...

Disregard that, I'm a moron. Testing a patch now.



---

archive/issue_comments_125769.json:
```json
{
    "body": "Attachment: **[sage-trac_11919.patch.gz](https://github.com/sagemath/sage/files/ticket11919/sage-trac_11919.patch.gz)**",
    "created_at": "2012-03-13T04:09:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-125769",
    "user": "https://github.com/orlitzky"
}
```

Attachment: **[sage-trac_11919.patch.gz](https://github.com/sagemath/sage/files/ticket11919/sage-trac_11919.patch.gz)**



---

archive/issue_events_160839.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2012-03-13T04:11:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-160839"
}
```



---

archive/issue_comments_125770.json:
```json
{
    "body": "Author: **Michael Orlitzky**",
    "created_at": "2012-03-13T04:11:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-125770",
    "user": "https://github.com/orlitzky"
}
```

Author: **Michael Orlitzky**



---

archive/issue_comments_125771.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">comment:11</div>\n\nI had the same problem on Sage 4.7.1. I applied the patch described above but it didn't work (same error message). I then uninstalled sage and installed version 4.8 instead. I applied the patch again and it still didn't work. Any suggestions?",
    "created_at": "2012-05-09T22:00:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-125771",
    "user": "https://github.com/sagetrac-abid-naqvi83"
}
```

<div id="comment:11" align="right">comment:11</div>

I had the same problem on Sage 4.7.1. I applied the patch described above but it didn't work (same error message). I then uninstalled sage and installed version 4.8 instead. I applied the patch again and it still didn't work. Any suggestions?



---

archive/issue_comments_125772.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">comment:12</div>\n\nReplying to [@sagetrac-abid-naqvi83](#comment:11):\n> I had the same problem on Sage 4.7.1. I applied the patch described above but it didn't work (same error message). I then uninstalled sage and installed version 4.8 instead. I applied the patch again and it still didn't work. Any suggestions?\n\nDid you remember to run `sage -b` after applying the patch?\n\nI don't remember which version I created the patch against... Does it at least apply cleanly against 4.8?",
    "created_at": "2012-05-09T22:46:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-125772",
    "user": "https://github.com/orlitzky"
}
```

<div id="comment:12" align="right">comment:12</div>

Replying to [@sagetrac-abid-naqvi83](#comment:11):
> I had the same problem on Sage 4.7.1. I applied the patch described above but it didn't work (same error message). I then uninstalled sage and installed version 4.8 instead. I applied the patch again and it still didn't work. Any suggestions?

Did you remember to run `sage -b` after applying the patch?

I don't remember which version I created the patch against... Does it at least apply cleanly against 4.8?



---

archive/issue_events_160840.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2012-05-09T22:49:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-160840"
}
```



---

archive/issue_events_160841.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2012-05-09T22:49:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-160841"
}
```



---

archive/issue_comments_125773.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">comment:13</div>\n\nFix confirmed! indeed, the patch simply adds back a line that disappeared in #9240 without any reason stated, so probably that was just an accident.\n\nPositive review!\n\n(for abid_naqvi83: note that the patchbot is happy and that the feature is actually doctested. If the fix has no effect, perhaps you forgot to run \"sage -b\"?)",
    "created_at": "2012-05-09T22:49:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-125773",
    "user": "https://github.com/nbruin"
}
```

<div id="comment:13" align="right">comment:13</div>

Fix confirmed! indeed, the patch simply adds back a line that disappeared in #9240 without any reason stated, so probably that was just an accident.

Positive review!

(for abid_naqvi83: note that the patchbot is happy and that the feature is actually doctested. If the fix has no effect, perhaps you forgot to run "sage -b"?)



---

archive/issue_comments_125774.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">comment:14</div>\n\nThat was it. Didn't know about \"sage -b\". Sorry for the faux pas and thanks for the fix. Working now.",
    "created_at": "2012-05-10T04:29:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-125774",
    "user": "https://github.com/sagetrac-abid-naqvi83"
}
```

<div id="comment:14" align="right">comment:14</div>

That was it. Didn't know about "sage -b". Sorry for the faux pas and thanks for the fix. Working now.



---

archive/issue_events_160842.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2012-05-10T07:53:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-160842"
}
```



---

archive/issue_events_160843.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2012-05-10T07:53:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-160843"
}
```



---

archive/issue_comments_125775.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">comment:15</div>\n\nReplying to [@nbruin](#comment:13):\n> Fix confirmed! indeed, the patch simply adds back a line that disappeared in #9240 without any reason stated, so probably that was just an accident.\n\nIt was not an accident. The python_func flag stored in pynac is not a bool any more. It is a bitmask that marks which custom functions are implemented in Python. The rest, if they exist are C++ functions.\n\n> Positive review!\n\nThis patch doesn't fix the problem. It might actually lead to crashes, since pynac will look for a python function to call for evaluation, differentiation, etc. if some bits in `python_func` are set.\n\nThe correct fix will be to get pynac to create a new dummy symbolic function at the point where it raises an error with \"unknown function 'f' in archive.\" Relevant code can be found in `ginac/function.{h,cpp}` in pynac sources.",
    "created_at": "2012-05-10T07:53:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-125775",
    "user": "https://github.com/burcin"
}
```

<div id="comment:15" align="right">comment:15</div>

Replying to [@nbruin](#comment:13):
> Fix confirmed! indeed, the patch simply adds back a line that disappeared in #9240 without any reason stated, so probably that was just an accident.

It was not an accident. The python_func flag stored in pynac is not a bool any more. It is a bitmask that marks which custom functions are implemented in Python. The rest, if they exist are C++ functions.

> Positive review!

This patch doesn't fix the problem. It might actually lead to crashes, since pynac will look for a python function to call for evaluation, differentiation, etc. if some bits in `python_func` are set.

The correct fix will be to get pynac to create a new dummy symbolic function at the point where it raises an error with "unknown function 'f' in archive." Relevant code can be found in `ginac/function.{h,cpp}` in pynac sources.



---

archive/issue_events_160844.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2012-05-10T11:28:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-160844"
}
```



---

archive/issue_events_160845.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2012-05-10T11:28:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-160845"
}
```



---

archive/issue_comments_125776.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">comment:16</div>\n\nI take it back. This is a good workaround, since constructors of `function_options` takes care to clear the function pointers used to store references to the custom methods which can be defined for evalutation, conjugation, etc. Before trying to evaluate any of these custom methods, we check if the pointer is NULL, so a crash can never happen.",
    "created_at": "2012-05-10T11:28:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-125776",
    "user": "https://github.com/burcin"
}
```

<div id="comment:16" align="right">comment:16</div>

I take it back. This is a good workaround, since constructors of `function_options` takes care to clear the function pointers used to store references to the custom methods which can be defined for evalutation, conjugation, etc. Before trying to evaluate any of these custom methods, we check if the pointer is NULL, so a crash can never happen.



---

archive/issue_comments_125777.json:
```json
{
    "body": "Reviewer: **Nils Bruin, Burcin Erocal**",
    "created_at": "2012-05-10T11:28:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-125777",
    "user": "https://github.com/burcin"
}
```

Reviewer: **Nils Bruin, Burcin Erocal**



---

archive/issue_events_160846.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-11T11:32:44Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-160846"
}
```



---

archive/issue_events_160847.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-11T11:32:44Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "milestone_number": null,
    "milestone_title": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-160847"
}
```



---

archive/issue_comments_125778.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">comment:18</div>\n\nPynac 0.2.4 from #12950 fixes this, without the patch attached to this ticket. I don't know how long it will take to get that reviewed and merged, so I'm not opposed to merging this first.",
    "created_at": "2012-05-15T22:40:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-125778",
    "user": "https://github.com/burcin"
}
```

<div id="comment:18" align="right">comment:18</div>

Pynac 0.2.4 from #12950 fixes this, without the patch attached to this ticket. I don't know how long it will take to get that reviewed and merged, so I'm not opposed to merging this first.



---

archive/issue_events_160848.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-23T21:31:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-160848"
}
```



---

archive/issue_events_160849.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-23T21:31:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-160849"
}
```



---

archive/issue_comments_125779.json:
```json
{
    "body": "Merged: **sage-5.1.beta1**",
    "created_at": "2012-05-23T21:31:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-125779",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.1.beta1**



---

archive/issue_comments_125780.json:
```json
{
    "body": "<div id=\"comment:20\" align=\"right\">comment:20</div>\n\nReplying to [@burcin](#comment:18):\n> Pynac 0.2.4 from #12950 fixes this, without the patch attached to this ticket. I don't know how long it will take to get that reviewed and merged, so I'm not opposed to merging this first. \n\nNow that #12950 is in, should we remove the line again (but keep the test)?",
    "created_at": "2012-09-10T13:36:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-125780",
    "user": "https://github.com/orlitzky"
}
```

<div id="comment:20" align="right">comment:20</div>

Replying to [@burcin](#comment:18):
> Pynac 0.2.4 from #12950 fixes this, without the patch attached to this ticket. I don't know how long it will take to get that reviewed and merged, so I'm not opposed to merging this first. 

Now that #12950 is in, should we remove the line again (but keep the test)?



---

archive/issue_comments_125781.json:
```json
{
    "body": "<div id=\"comment:21\" align=\"right\">comment:21</div>\n\nReplying to [@orlitzky](#comment:20):\n> Replying to [@burcin](#comment:18):\n> > Pynac 0.2.4 from #12950 fixes this, without the patch attached to this ticket. I don't know how long it will take to get that reviewed and merged, so I'm not opposed to merging this first. \n\n> \n> Now that #12950 is in, should we remove the line again (but keep the test)?\n\nYes, though it is mostly harmless AFAICT.",
    "created_at": "2012-09-10T13:42:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-125781",
    "user": "https://github.com/burcin"
}
```

<div id="comment:21" align="right">comment:21</div>

Replying to [@orlitzky](#comment:20):
> Replying to [@burcin](#comment:18):
> > Pynac 0.2.4 from #12950 fixes this, without the patch attached to this ticket. I don't know how long it will take to get that reviewed and merged, so I'm not opposed to merging this first. 

> 
> Now that #12950 is in, should we remove the line again (but keep the test)?

Yes, though it is mostly harmless AFAICT.



---

archive/issue_comments_125782.json:
```json
{
    "body": "<div id=\"comment:22\" align=\"right\">comment:22</div>\n\nI've removed it at #13446 (needs review if anyone wants it).",
    "created_at": "2012-09-11T14:14:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-125782",
    "user": "https://github.com/orlitzky"
}
```

<div id="comment:22" align="right">comment:22</div>

I've removed it at #13446 (needs review if anyone wants it).
