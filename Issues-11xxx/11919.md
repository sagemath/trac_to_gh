# Issue 11919: Issue when pickling a formal function

archive/issues_011747.json:
```json
{
    "assignees": [],
    "body": "There is a problem in symbolic function pickling:\n\n```\nsage: x = var('x'); f = function('f',x) ; s = dumps(f) ; loads(s)\nRuntimeError: unknown function 'f' in archive\n```\n\nThe error was not present in Sage 4.7 but it is in newer versions.\nIt was suggested (http://groups.google.com/group/sage-support/browse_thread/thread/b439844f2fa0b675) that it is related to Pynac.\nPynac was updated to 0.2.2 (sage-4.7.1.alpha1) and then to 0.2.3 \n(sage-4.7.1.alpha4) according to sage 4.7.1 changelog.\n\nAssignee: **@burcin**\n\nCC:  jpflori @burcin @jasongrout dsm @orlitzky\n\nKeywords: **pynac, pickling**\n\nReviewer: **Nils Bruin, Burcin Erocal**\n\nAuthor: **Michael Orlitzky**\n\nMerged: **sage-5.1.beta1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11919_\n\n",
    "closed_at": "2012-05-23T21:31:59Z",
    "created_at": "2011-10-13T15:52:44Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Issue when pickling a formal function",
    "type": "issue",
    "updated_at": "2012-09-11T14:14:06Z",
    "url": "https://github.com/sagemath/sage/issues/11919",
    "user": "https://github.com/sagetrac-cdsousa"
}
```
There is a problem in symbolic function pickling:

```
sage: x = var('x'); f = function('f',x) ; s = dumps(f) ; loads(s)
RuntimeError: unknown function 'f' in archive
```

The error was not present in Sage 4.7 but it is in newer versions.
It was suggested (http://groups.google.com/group/sage-support/browse_thread/thread/b439844f2fa0b675) that it is related to Pynac.
Pynac was updated to 0.2.2 (sage-4.7.1.alpha1) and then to 0.2.3 
(sage-4.7.1.alpha4) according to sage 4.7.1 changelog.

Assignee: **@burcin**

CC:  jpflori @burcin @jasongrout dsm @orlitzky

Keywords: **pynac, pickling**

Reviewer: **Nils Bruin, Burcin Erocal**

Author: **Michael Orlitzky**

Merged: **sage-5.1.beta1**

_Issue created by migration from https://trac.sagemath.org/ticket/11919_





---

archive/issue_events_147002.json:
```json
{
    "actor": "https://github.com/sagetrac-cdsousa",
    "created_at": "2011-10-13T15:52:44Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-147002"
}
```



---

archive/issue_events_147003.json:
```json
{
    "actor": "https://github.com/sagetrac-cdsousa",
    "created_at": "2011-10-13T15:52:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20pickling",
    "label_color": "000080",
    "label_name": "component: pickling",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-147003"
}
```



---

archive/issue_events_147004.json:
```json
{
    "actor": "https://github.com/sagetrac-cdsousa",
    "created_at": "2011-10-13T15:52:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-147004"
}
```



---

archive/issue_events_147005.json:
```json
{
    "actor": "https://github.com/sagetrac-cdsousa",
    "created_at": "2011-10-13T15:52:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-147005"
}
```



---

archive/issue_comments_127952.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -8,4 +8,4 @@\n The error was not present in Sage 4.7 but it is in newer versions.\n It was suggested (http://groups.google.com/group/sage-support/browse_thread/thread/b439844f2fa0b675) that it is related to Pynac.\n Pynac was updated to 0.2.2 (sage-4.7.1.alpha1) and then to 0.2.3 \n-(sage-4.7.1.alpha4) according to sage 7.4.1 changelog.\n+(sage-4.7.1.alpha4) according to sage 4.7.1 changelog.\n``````\n",
    "created_at": "2011-10-14T12:34:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-127952",
    "user": "https://github.com/kcrisman"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -8,4 +8,4 @@
 The error was not present in Sage 4.7 but it is in newer versions.
 It was suggested (http://groups.google.com/group/sage-support/browse_thread/thread/b439844f2fa0b675) that it is related to Pynac.
 Pynac was updated to 0.2.2 (sage-4.7.1.alpha1) and then to 0.2.3 
-(sage-4.7.1.alpha4) according to sage 7.4.1 changelog.
+(sage-4.7.1.alpha4) according to sage 4.7.1 changelog.
``````




---

archive/issue_comments_127953.json:
```json
{
    "body": "Changed keywords from **** to **pynac, pickling**",
    "created_at": "2011-10-15T19:45:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-127953",
    "user": "https://github.com/nbruin"
}
```

Changed keywords from **** to **pynac, pickling**



---

archive/issue_comments_127954.json:
```json
{
    "body": "Changed assignee from **@williamstein** to **@burcin**",
    "created_at": "2011-10-15T19:45:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-127954",
    "user": "https://github.com/nbruin"
}
```

Changed assignee from **@williamstein** to **@burcin**



---

archive/issue_events_147006.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2011-10-15T19:45:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20pickling",
    "label_color": "000080",
    "label_name": "component: pickling",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-147006"
}
```



---

archive/issue_events_147007.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2011-10-15T19:45:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-147007"
}
```



---

archive/issue_comments_127955.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nI hit this independently using deepcopy(). In sage-4.7:\n\n```\nsage: f = function('f',x)\nsage: deepcopy(f)\nf(x)\n```\n\nand in 4.8.alpha5:\n\n```\nsage: f = function('f',x)\nsage: deepcopy(f)\n...\nRuntimeError: unknown function 'f' in archive\n```",
    "created_at": "2011-12-26T21:51:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-127955",
    "user": "https://github.com/orlitzky"
}
```

<a id='comment:5'>Comment 5:</a>
I hit this independently using deepcopy(). In sage-4.7:

```
sage: f = function('f',x)
sage: deepcopy(f)
f(x)
```

and in 4.8.alpha5:

```
sage: f = function('f',x)
sage: deepcopy(f)
...
RuntimeError: unknown function 'f' in archive
```



---

archive/issue_comments_127956.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nInterestingly, if I take the pynac spkg from 4.7 and install it on 5.0.beta7, it doesn't fix the crash.",
    "created_at": "2012-03-11T02:02:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-127956",
    "user": "https://github.com/orlitzky"
}
```

<a id='comment:6'>Comment 6:</a>
Interestingly, if I take the pynac spkg from 4.7 and install it on 5.0.beta7, it doesn't fix the crash.



---

archive/issue_comments_127957.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nI built a copy of 4.7.1, and the crash does happen. So the change was introduced between 4.7 and 4.7.1.",
    "created_at": "2012-03-12T18:05:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-127957",
    "user": "https://github.com/orlitzky"
}
```

<a id='comment:7'>Comment 7:</a>
I built a copy of 4.7.1, and the crash does happen. So the change was introduced between 4.7 and 4.7.1.



---

archive/issue_comments_127958.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nOkay, this broke with #9240. I can fix it with a one-line change in 4.7.1.alpha4:\n\n```\ndiff --git a/sage/symbolic/function.pyx b/sage/symbolic/function.pyx\n--- a/sage/symbolic/function.pyx\n+++ b/sage/symbolic/function.pyx\n@@ -127,6 +127,7 @@\n     cdef _register_function(self):\n         cdef GFunctionOpt opt\n         opt = g_function_options_args(self._name, self._nargs)\n+        opt.set_python_func()\n\n         if hasattr(self, '_eval_'):\n             opt.eval_func(self)\n```\n\nBut there's no longer a `set_python_func()` method on GFunctionOpt, so I don't know how to fix it in 5.0.beta7. The adventure continues...",
    "created_at": "2012-03-13T02:56:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-127958",
    "user": "https://github.com/orlitzky"
}
```

<a id='comment:8'>Comment 8:</a>
Okay, this broke with #9240. I can fix it with a one-line change in 4.7.1.alpha4:

```
diff --git a/sage/symbolic/function.pyx b/sage/symbolic/function.pyx
--- a/sage/symbolic/function.pyx
+++ b/sage/symbolic/function.pyx
@@ -127,6 +127,7 @@
     cdef _register_function(self):
         cdef GFunctionOpt opt
         opt = g_function_options_args(self._name, self._nargs)
+        opt.set_python_func()

         if hasattr(self, '_eval_'):
             opt.eval_func(self)
```

But there's no longer a `set_python_func()` method on GFunctionOpt, so I don't know how to fix it in 5.0.beta7. The adventure continues...



---

archive/issue_comments_127959.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nReplying to [@orlitzky](#comment%3A8):\n> But there's no longer a `set_python_func()` method on GFunctionOpt, so I don't know how to fix it in 5.0.beta7. The adventure continues...\n\nDisregard that, I'm a moron. Testing a patch now.",
    "created_at": "2012-03-13T03:23:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-127959",
    "user": "https://github.com/orlitzky"
}
```

<a id='comment:9'>Comment 9:</a>
Replying to [@orlitzky](#comment%3A8):
> But there's no longer a `set_python_func()` method on GFunctionOpt, so I don't know how to fix it in 5.0.beta7. The adventure continues...

Disregard that, I'm a moron. Testing a patch now.



---

archive/issue_comments_127960.json:
```json
{
    "body": "Attachment: **[sage-trac_11919.patch.gz](https://github.com/sagemath/sage/files/ticket11919/sage-trac_11919.patch.gz)**",
    "created_at": "2012-03-13T04:09:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-127960",
    "user": "https://github.com/orlitzky"
}
```

Attachment: **[sage-trac_11919.patch.gz](https://github.com/sagemath/sage/files/ticket11919/sage-trac_11919.patch.gz)**



---

archive/issue_events_147008.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2012-03-13T04:11:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-147008"
}
```



---

archive/issue_comments_127961.json:
```json
{
    "body": "Author: **Michael Orlitzky**",
    "created_at": "2012-03-13T04:11:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-127961",
    "user": "https://github.com/orlitzky"
}
```

Author: **Michael Orlitzky**



---

archive/issue_comments_127962.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nI had the same problem on Sage 4.7.1. I applied the patch described above but it didn't work (same error message). I then uninstalled sage and installed version 4.8 instead. I applied the patch again and it still didn't work. Any suggestions?",
    "created_at": "2012-05-09T22:00:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-127962",
    "user": "https://github.com/sagetrac-abid-naqvi83"
}
```

<a id='comment:11'>Comment 11:</a>
I had the same problem on Sage 4.7.1. I applied the patch described above but it didn't work (same error message). I then uninstalled sage and installed version 4.8 instead. I applied the patch again and it still didn't work. Any suggestions?



---

archive/issue_comments_127963.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nReplying to [abid_naqvi83](#comment%3A11):\n> I had the same problem on Sage 4.7.1. I applied the patch described above but it didn't work (same error message). I then uninstalled sage and installed version 4.8 instead. I applied the patch again and it still didn't work. Any suggestions?\n\nDid you remember to run `sage -b` after applying the patch?\n\nI don't remember which version I created the patch against... Does it at least apply cleanly against 4.8?",
    "created_at": "2012-05-09T22:46:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-127963",
    "user": "https://github.com/orlitzky"
}
```

<a id='comment:12'>Comment 12:</a>
Replying to [abid_naqvi83](#comment%3A11):
> I had the same problem on Sage 4.7.1. I applied the patch described above but it didn't work (same error message). I then uninstalled sage and installed version 4.8 instead. I applied the patch again and it still didn't work. Any suggestions?

Did you remember to run `sage -b` after applying the patch?

I don't remember which version I created the patch against... Does it at least apply cleanly against 4.8?



---

archive/issue_events_147009.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2012-05-09T22:49:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-147009"
}
```



---

archive/issue_events_147010.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2012-05-09T22:49:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-147010"
}
```



---

archive/issue_comments_127964.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nFix confirmed! indeed, the patch simply adds back a line that disappeared in #9240 without any reason stated, so probably that was just an accident.\n\nPositive review!\n\n(for abid_naqvi83: note that the patchbot is happy and that the feature is actually doctested. If the fix has no effect, perhaps you forgot to run \"sage -b\"?)",
    "created_at": "2012-05-09T22:49:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-127964",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:13'>Comment 13:</a>
Fix confirmed! indeed, the patch simply adds back a line that disappeared in #9240 without any reason stated, so probably that was just an accident.

Positive review!

(for abid_naqvi83: note that the patchbot is happy and that the feature is actually doctested. If the fix has no effect, perhaps you forgot to run "sage -b"?)



---

archive/issue_comments_127965.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nThat was it. Didn't know about \"sage -b\". Sorry for the faux pas and thanks for the fix. Working now.",
    "created_at": "2012-05-10T04:29:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-127965",
    "user": "https://github.com/sagetrac-abid-naqvi83"
}
```

<a id='comment:14'>Comment 14:</a>
That was it. Didn't know about "sage -b". Sorry for the faux pas and thanks for the fix. Working now.



---

archive/issue_events_147011.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2012-05-10T07:53:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-147011"
}
```



---

archive/issue_events_147012.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2012-05-10T07:53:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-147012"
}
```



---

archive/issue_comments_127966.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nReplying to [@nbruin](#comment%3A13):\n> Fix confirmed! indeed, the patch simply adds back a line that disappeared in #9240 without any reason stated, so probably that was just an accident.\n\nIt was not an accident. The python_func flag stored in pynac is not a bool any more. It is a bitmask that marks which custom functions are implemented in Python. The rest, if they exist are C++ functions.\n\n> Positive review!\n\nThis patch doesn't fix the problem. It might actually lead to crashes, since pynac will look for a python function to call for evaluation, differentiation, etc. if some bits in `python_func` are set.\n\nThe correct fix will be to get pynac to create a new dummy symbolic function at the point where it raises an error with \"unknown function 'f' in archive.\" Relevant code can be found in `ginac/function.{h,cpp}` in pynac sources.",
    "created_at": "2012-05-10T07:53:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-127966",
    "user": "https://github.com/burcin"
}
```

<a id='comment:15'>Comment 15:</a>
Replying to [@nbruin](#comment%3A13):
> Fix confirmed! indeed, the patch simply adds back a line that disappeared in #9240 without any reason stated, so probably that was just an accident.

It was not an accident. The python_func flag stored in pynac is not a bool any more. It is a bitmask that marks which custom functions are implemented in Python. The rest, if they exist are C++ functions.

> Positive review!

This patch doesn't fix the problem. It might actually lead to crashes, since pynac will look for a python function to call for evaluation, differentiation, etc. if some bits in `python_func` are set.

The correct fix will be to get pynac to create a new dummy symbolic function at the point where it raises an error with "unknown function 'f' in archive." Relevant code can be found in `ginac/function.{h,cpp}` in pynac sources.



---

archive/issue_events_147013.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2012-05-10T11:28:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-147013"
}
```



---

archive/issue_events_147014.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2012-05-10T11:28:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-147014"
}
```



---

archive/issue_comments_127967.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nI take it back. This is a good workaround, since constructors of `function_options` takes care to clear the function pointers used to store references to the custom methods which can be defined for evalutation, conjugation, etc. Before trying to evaluate any of these custom methods, we check if the pointer is NULL, so a crash can never happen.",
    "created_at": "2012-05-10T11:28:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-127967",
    "user": "https://github.com/burcin"
}
```

<a id='comment:16'>Comment 16:</a>
I take it back. This is a good workaround, since constructors of `function_options` takes care to clear the function pointers used to store references to the custom methods which can be defined for evalutation, conjugation, etc. Before trying to evaluate any of these custom methods, we check if the pointer is NULL, so a crash can never happen.



---

archive/issue_comments_127968.json:
```json
{
    "body": "Reviewer: **Nils Bruin, Burcin Erocal**",
    "created_at": "2012-05-10T11:28:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-127968",
    "user": "https://github.com/burcin"
}
```

Reviewer: **Nils Bruin, Burcin Erocal**



---

archive/issue_events_147015.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-11T11:32:44Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-147015"
}
```



---

archive/issue_events_147016.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-11T11:32:44Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "milestone_number": null,
    "milestone_title": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-147016"
}
```



---

archive/issue_comments_127969.json:
```json
{
    "body": "<a id='comment:18'>Comment 18:</a>\nPynac 0.2.4 from #12950 fixes this, without the patch attached to this ticket. I don't know how long it will take to get that reviewed and merged, so I'm not opposed to merging this first.",
    "created_at": "2012-05-15T22:40:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-127969",
    "user": "https://github.com/burcin"
}
```

<a id='comment:18'>Comment 18:</a>
Pynac 0.2.4 from #12950 fixes this, without the patch attached to this ticket. I don't know how long it will take to get that reviewed and merged, so I'm not opposed to merging this first.



---

archive/issue_events_147017.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-23T21:31:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-147017"
}
```



---

archive/issue_events_147018.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-23T21:31:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11919#event-147018"
}
```



---

archive/issue_comments_127970.json:
```json
{
    "body": "Merged: **sage-5.1.beta1**",
    "created_at": "2012-05-23T21:31:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-127970",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.1.beta1**



---

archive/issue_comments_127971.json:
```json
{
    "body": "<a id='comment:20'>Comment 20:</a>\nReplying to [@burcin](#comment%3A18):\n> Pynac 0.2.4 from #12950 fixes this, without the patch attached to this ticket. I don't know how long it will take to get that reviewed and merged, so I'm not opposed to merging this first. \n\nNow that #12950 is in, should we remove the line again (but keep the test)?",
    "created_at": "2012-09-10T13:36:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-127971",
    "user": "https://github.com/orlitzky"
}
```

<a id='comment:20'>Comment 20:</a>
Replying to [@burcin](#comment%3A18):
> Pynac 0.2.4 from #12950 fixes this, without the patch attached to this ticket. I don't know how long it will take to get that reviewed and merged, so I'm not opposed to merging this first. 

Now that #12950 is in, should we remove the line again (but keep the test)?



---

archive/issue_comments_127972.json:
```json
{
    "body": "<a id='comment:21'>Comment 21:</a>\nReplying to [@orlitzky](#comment%3A20):\n> Replying to [@burcin](#comment%3A18):\n> > Pynac 0.2.4 from #12950 fixes this, without the patch attached to this ticket. I don't know how long it will take to get that reviewed and merged, so I'm not opposed to merging this first. \n\n> \n> Now that #12950 is in, should we remove the line again (but keep the test)?\n\nYes, though it is mostly harmless AFAICT.",
    "created_at": "2012-09-10T13:42:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-127972",
    "user": "https://github.com/burcin"
}
```

<a id='comment:21'>Comment 21:</a>
Replying to [@orlitzky](#comment%3A20):
> Replying to [@burcin](#comment%3A18):
> > Pynac 0.2.4 from #12950 fixes this, without the patch attached to this ticket. I don't know how long it will take to get that reviewed and merged, so I'm not opposed to merging this first. 

> 
> Now that #12950 is in, should we remove the line again (but keep the test)?

Yes, though it is mostly harmless AFAICT.



---

archive/issue_comments_127973.json:
```json
{
    "body": "<a id='comment:22'>Comment 22:</a>\nI've removed it at #13446 (needs review if anyone wants it).",
    "created_at": "2012-09-11T14:14:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11919",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11919#issuecomment-127973",
    "user": "https://github.com/orlitzky"
}
```

<a id='comment:22'>Comment 22:</a>
I've removed it at #13446 (needs review if anyone wants it).
