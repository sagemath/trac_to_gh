# Issue 11639: conductor of simple curve over Q(cube root 3) takes forever

archive/issues_011467.json:
```json
{
    "assignees": [
        "https://github.com/JohnCremona"
    ],
    "body": "\n```\nsage: x=var('x'); K.<a> = NumberField(x^3 - 2); EllipticCurve([0,a]).conductor()\n[[ wait forever! ]]\n```\n\nThe correct answer has norm 322486272, and should be instant:\n\n```\nsage: magma(E).Conductor()\nIdeal\nBasis:\n[864   0   0]\n[  0 864   0]\n[  0   0 432]\nsage: magma(E).Conductor().Norm()\n322486272\n```\n\nTrying with `proof.all(False)` doesn't help, by the way, so I don't think it is just some complexity issue... and yet, I just tried control-c after letting \"E.conductor()\" sit there for a while, then typed \"%debug\", then \"u\", then saw that two ideals of the integers of the cubic field were being multiplied.  I printed the gens of one ideal and got\n\n```\nipdb> u\n> /Users/wstein/sage/install/current/local/lib/python2.6/site-packages/sage/rings/ideal.py(857)__mul__()\n    856             other = self.ring().ideal(other)\n--> 857         return self.ring().ideal([x*y for x in self.gens() for y in other.gens()])\n    858 \n\nipdb> print self.gens()\n(16384, 8192*a, 8192*a, 4096*a^2, 8192*a, 4096*a^2,  ... goes on redundantly for many pages!)\n```\n\nFixed by #13958.\n\nReviewer: **Jeroen Demeyer**\n\nComponent: **elliptic curves**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11639_\n\n",
    "closed_at": "2013-05-08T09:46:28Z",
    "created_at": "2011-08-01T23:44:45Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20elliptic%20curves",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "conductor of simple curve over Q(cube root 3) takes forever",
    "type": "issue",
    "updated_at": "2013-05-08T09:46:28Z",
    "url": "https://github.com/sagemath/sage/issues/11639",
    "user": "https://github.com/williamstein"
}
```

```
sage: x=var('x'); K.<a> = NumberField(x^3 - 2); EllipticCurve([0,a]).conductor()
[[ wait forever! ]]
```

The correct answer has norm 322486272, and should be instant:

```
sage: magma(E).Conductor()
Ideal
Basis:
[864   0   0]
[  0 864   0]
[  0   0 432]
sage: magma(E).Conductor().Norm()
322486272
```

Trying with `proof.all(False)` doesn't help, by the way, so I don't think it is just some complexity issue... and yet, I just tried control-c after letting "E.conductor()" sit there for a while, then typed "%debug", then "u", then saw that two ideals of the integers of the cubic field were being multiplied.  I printed the gens of one ideal and got

```
ipdb> u
> /Users/wstein/sage/install/current/local/lib/python2.6/site-packages/sage/rings/ideal.py(857)__mul__()
    856             other = self.ring().ideal(other)
--> 857         return self.ring().ideal([x*y for x in self.gens() for y in other.gens()])
    858 

ipdb> print self.gens()
(16384, 8192*a, 8192*a, 4096*a^2, 8192*a, 4096*a^2,  ... goes on redundantly for many pages!)
```

Fixed by #13958.

Reviewer: **Jeroen Demeyer**

Component: **elliptic curves**

_Issue created by migration from https://trac.sagemath.org/ticket/11639_





---

archive/issue_events_155966.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-08-01T23:44:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11639",
    "milestone_number": null,
    "milestone_title": "sage-5.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11639#event-155966"
}
```



---

archive/issue_events_155967.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-08-01T23:44:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11639",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20elliptic%20curves",
    "label_color": "0000ff",
    "label_name": "c: elliptic curves",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11639#event-155967"
}
```



---

archive/issue_events_155968.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-08-01T23:44:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11639",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11639#event-155968"
}
```



---

archive/issue_events_155969.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-08-01T23:44:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11639",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11639#event-155969"
}
```



---

archive/issue_events_155970.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2011-08-01T23:44:45Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/11639",
    "subject": "https://github.com/williamstein",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11639#event-155970"
}
```



---

archive/issue_comments_119883.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,7 +1,8 @@\n-{{\n+\n+```\n sage: x=var('x'); K.<a> = NumberField(x^3 - 2); EllipticCurve([0,a]).conductor()\n-[This is the Trac macro *wait forever! * that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#wait forever! -macro)\n-}}}\n+[[ wait forever! ]]\n+```\n \n The correct answer has norm 322486272, and should be instant:\n \n``````\n",
    "created_at": "2011-08-01T23:45:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11639",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11639#issuecomment-119883",
    "user": "https://github.com/williamstein"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,7 +1,8 @@
-{{
+
+```
 sage: x=var('x'); K.<a> = NumberField(x^3 - 2); EllipticCurve([0,a]).conductor()
-[This is the Trac macro *wait forever! * that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#wait forever! -macro)
-}}}
+[[ wait forever! ]]
+```
 
 The correct answer has norm 322486272, and should be instant:
 
``````




---

archive/issue_comments_119884.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThis is the heart of the problem\n\n```\nsage: K.<a> = NumberField(x^3 - 2); E = EllipticCurve([0, a])\nsage: P, Q = K.prime_factors(E.discriminant())\nsage: P, P.gens()\n(Fractional ideal (a), (2, a))\nsage: P2 = P*P; P2, P2.gens()\n(Fractional ideal (a^2), (4, 2*a, 2*a, a^2))\n```\nThe conductor is computed as `P<sup>14*Q</sup>9`, which is defined by 2<sup>23</sup> (highly redundant) generators. Multiplication of ideals avoids reducing the generators, which can be time-consuming (though not in this case), but it could at least eliminate repeats.^^",
    "created_at": "2011-08-03T07:24:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11639",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11639#issuecomment-119884",
    "user": "https://github.com/sagetrac-fwclarke"
}
```

<div id="comment:2" align="right">comment:2</div>

This is the heart of the problem

```
sage: K.<a> = NumberField(x^3 - 2); E = EllipticCurve([0, a])
sage: P, Q = K.prime_factors(E.discriminant())
sage: P, P.gens()
(Fractional ideal (a), (2, a))
sage: P2 = P*P; P2, P2.gens()
(Fractional ideal (a^2), (4, 2*a, 2*a, a^2))
```
The conductor is computed as `P<sup>14*Q</sup>9`, which is defined by 2<sup>23</sup> (highly redundant) generators. Multiplication of ideals avoids reducing the generators, which can be time-consuming (though not in this case), but it could at least eliminate repeats.^^



---

archive/issue_comments_119885.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nHow hard would it be to eliminate repeated gens after any ideal multiplication?  And would that make this particular example run in reasonable time?\n\nNote that in a Dedekind domain if I = (a,b) then I^n = (a<sup>n,b</sup>n) [exercise!] which is surely a special case worth catching!\n\nOtherwise I suppose we could have an option to return the conductor  in factored form.  When I compute conductors over number fields I almost always factor it right away (which is a bit silly when that poor ideal has only just been multiplied together), and take its norm and factor that;  rarely would I want to see the conductor as an ideal unless it were principal.",
    "created_at": "2011-08-03T08:20:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11639",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11639#issuecomment-119885",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:3" align="right">comment:3</div>

How hard would it be to eliminate repeated gens after any ideal multiplication?  And would that make this particular example run in reasonable time?

Note that in a Dedekind domain if I = (a,b) then I^n = (a<sup>n,b</sup>n) [exercise!] which is surely a special case worth catching!

Otherwise I suppose we could have an option to return the conductor  in factored form.  When I compute conductors over number fields I almost always factor it right away (which is a bit silly when that poor ideal has only just been multiplied together), and take its norm and factor that;  rarely would I want to see the conductor as an ideal unless it were principal.



---

archive/issue_comments_119886.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\n> \n> Note that in a Dedekind domain if I = (a,b) then I^n = (a<sup>n,b</sup>n) [exercise!] which is surely a special case worth catching!\n\nThat should read: if `I = (a,b)` then `I^n = (a<sup>n,b</sup>n)`",
    "created_at": "2011-08-03T08:24:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11639",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11639#issuecomment-119886",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:4" align="right">comment:4</div>

> 
> Note that in a Dedekind domain if I = (a,b) then I^n = (a<sup>n,b</sup>n) [exercise!] which is surely a special case worth catching!

That should read: if `I = (a,b)` then `I^n = (a<sup>n,b</sup>n)`



---

archive/issue_comments_119887.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2013-05-06T12:43:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11639",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11639#issuecomment-119887",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_comments_119888.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nThis works now:\n\n```\nsage: K.<a> = NumberField(x^3 - 2)\nsage: %time EllipticCurve([0,a]).conductor().norm()\nCPU times: user 0.14 s, sys: 0.01 s, total: 0.15 s\nWall time: 0.14 s\n322486272\n```",
    "created_at": "2013-05-06T12:43:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11639",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11639#issuecomment-119888",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:5" align="right">comment:5</div>

This works now:

```
sage: K.<a> = NumberField(x^3 - 2)
sage: %time EllipticCurve([0,a]).conductor().norm()
CPU times: user 0.14 s, sys: 0.01 s, total: 0.15 s
Wall time: 0.14 s
322486272
```



---

archive/issue_events_155971.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-06T12:43:24Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11639",
    "milestone_number": null,
    "milestone_title": "sage-5.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11639#event-155971"
}
```



---

archive/issue_events_155972.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-06T12:43:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11639",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11639#event-155972"
}
```



---

archive/issue_events_155973.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-06T12:43:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11639",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11639#event-155973"
}
```



---

archive/issue_events_155974.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-06T12:43:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11639",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11639#event-155974"
}
```



---

archive/issue_events_155975.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-06T12:43:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11639",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11639#event-155975"
}
```



---

archive/issue_events_155976.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-06T12:43:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11639",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11639#event-155976"
}
```



---

archive/issue_comments_119889.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nAs there is no patch here, do we know what changed to make this work ok now?",
    "created_at": "2013-05-06T12:55:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11639",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11639#issuecomment-119889",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:7" align="right">comment:7</div>

As there is no patch here, do we know what changed to make this work ok now?



---

archive/issue_comments_119890.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nReplying to [@JohnCremona](#comment:7):\n> As there is no patch here, do we know what changed to make this work ok now?\n\nI know the multiplication of number field ideals got improved, which was essentially the problem this ticket is about.",
    "created_at": "2013-05-06T12:58:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11639",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11639#issuecomment-119890",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:8" align="right">comment:8</div>

Replying to [@JohnCremona](#comment:7):
> As there is no patch here, do we know what changed to make this work ok now?

I know the multiplication of number field ideals got improved, which was essentially the problem this ticket is about.



---

archive/issue_comments_119891.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nReplying to [@jdemeyer](#comment:8):\n> Replying to [@JohnCremona](#comment:7):\n> > As there is no patch here, do we know what changed to make this work ok now?\n\n> I know the multiplication of number field ideals got improved, which was essentially the problem this ticket is about.\n\nOK, fine.  We could add a doctest referencing this ticket.  Is that worth while?",
    "created_at": "2013-05-06T13:11:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11639",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11639#issuecomment-119891",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:9" align="right">comment:9</div>

Replying to [@jdemeyer](#comment:8):
> Replying to [@JohnCremona](#comment:7):
> > As there is no patch here, do we know what changed to make this work ok now?

> I know the multiplication of number field ideals got improved, which was essentially the problem this ticket is about.

OK, fine.  We could add a doctest referencing this ticket.  Is that worth while?



---

archive/issue_comments_119892.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nReplying to [@JohnCremona](#comment:9):\n> We could add a doctest referencing this ticket.  Is that worth while?\n\nNot really, since the actual issue is fixed and doctested in #13958.",
    "created_at": "2013-05-06T13:26:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11639",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11639#issuecomment-119892",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:10" align="right">comment:10</div>

Replying to [@JohnCremona](#comment:9):
> We could add a doctest referencing this ticket.  Is that worth while?

Not really, since the actual issue is fixed and doctested in #13958.



---

archive/issue_comments_119893.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">comment:11</div>\n\nReplying to [@jdemeyer](#comment:10):\n> Replying to [@JohnCremona](#comment:9):\n> > We could add a doctest referencing this ticket.  Is that worth while?\n\n> Not really, since the actual issue is fixed and doctested in #13958.\n\nThen I am happy, especially since this cross-reference is now on this ticket!",
    "created_at": "2013-05-06T14:27:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11639",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11639#issuecomment-119893",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:11" align="right">comment:11</div>

Replying to [@jdemeyer](#comment:10):
> Replying to [@JohnCremona](#comment:9):
> > We could add a doctest referencing this ticket.  Is that worth while?

> Not really, since the actual issue is fixed and doctested in #13958.

Then I am happy, especially since this cross-reference is now on this ticket!



---

archive/issue_events_155977.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-08T09:46:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11639",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11639#event-155977"
}
```



---

archive/issue_events_155978.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-08T09:46:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11639",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11639#event-155978"
}
```



---

archive/issue_events_155979.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-08T09:46:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11639",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11639#event-155979"
}
```



---

archive/issue_comments_119894.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -30,4 +30,4 @@\n (16384, 8192*a, 8192*a, 4096*a^2, 8192*a, 4096*a^2,  ... goes on redundantly for many pages!)\n ```\n \n-So, maybe ideals aren't being properly reduced, which is causing huge trouble.\n+Fixed by #13958.\n``````\n",
    "created_at": "2013-05-08T09:46:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11639",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11639#issuecomment-119894",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -30,4 +30,4 @@
 (16384, 8192*a, 8192*a, 4096*a^2, 8192*a, 4096*a^2,  ... goes on redundantly for many pages!)
 ```
 
-So, maybe ideals aren't being properly reduced, which is causing huge trouble.
+Fixed by #13958.
``````

