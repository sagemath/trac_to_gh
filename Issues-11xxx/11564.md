# Issue 11564: Implement polyhedron unfolding (i.e net) for 3-polytopes

archive/issues_011392.json:
```json
{
    "body": "An unfolding or a net of a polyhedron is an arrangement of edge-joined polygons in the plane which can be folded (along edges) to become the faces of the polyhedron. \n\nThere might be an overlap between polygons (this is an open problem).\n\nThis ticket implement ...\n\nAssignee: mhampton\n\nCC:  @mo271 @jplab @kliem\n\nKeywords: unfolding, net\n\nReviewer: Jean-Philippe Labb\u00e9\n\nAuthor: Mara Kortenkamp\n\nBranch: u/gh-marakortenkamp/poly_net\n\nStatus: new\n\nCommit: 9d4f22788cc1feafd12a03131f10a6390a9ec40e\n\nIssue created by migration from https://trac.sagemath.org/ticket/11564\n\n",
    "created_at": "2011-07-02T23:19:34Z",
    "labels": [
        "component: geometry"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.8",
    "title": "Implement polyhedron unfolding (i.e net) for 3-polytopes",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/11564",
    "user": "https://trac.sagemath.org/admin/accounts/users/QuantumKing"
}
```
An unfolding or a net of a polyhedron is an arrangement of edge-joined polygons in the plane which can be folded (along edges) to become the faces of the polyhedron. 

There might be an overlap between polygons (this is an open problem).

This ticket implement ...

Assignee: mhampton

CC:  @mo271 @jplab @kliem

Keywords: unfolding, net

Reviewer: Jean-Philippe Labbé

Author: Mara Kortenkamp

Branch: u/gh-marakortenkamp/poly_net

Status: new

Commit: 9d4f22788cc1feafd12a03131f10a6390a9ec40e

Issue created by migration from https://trac.sagemath.org/ticket/11564





---

archive/issue_comments_154481.json:
```json
{
    "body": "Attachment [polyhedron_unfold.py](tarball://root/attachments/some-uuid/ticket11564/polyhedron_unfold.py) by QuantumKing created at 2011-07-02 23:20:04",
    "created_at": "2011-07-02T23:20:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11564#issuecomment-154481",
    "user": "https://trac.sagemath.org/admin/accounts/users/QuantumKing"
}
```

Attachment [polyhedron_unfold.py](tarball://root/attachments/some-uuid/ticket11564/polyhedron_unfold.py) by QuantumKing created at 2011-07-02 23:20:04



---

archive/issue_comments_154482.json:
```json
{
    "body": "<a id='comment:1'></a>\nFunny timing, I was about to create a ticket with solution for this same issue.  It was the outcome of an undergraduate research project.  I will try to review your solution, maybe it is possible to merge the two approaches.",
    "created_at": "2011-07-03T12:55:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11564#issuecomment-154482",
    "user": "https://trac.sagemath.org/admin/accounts/users/mhampton"
}
```

<a id='comment:1'></a>
Funny timing, I was about to create a ticket with solution for this same issue.  It was the outcome of an undergraduate research project.  I will try to review your solution, maybe it is possible to merge the two approaches.



---

archive/issue_comments_154483.json:
```json
{
    "body": "<a id='comment:2'></a>\nIt would be great also to handle higher dimensions, i.e. unfolding a 4-d polytope into 3-d...",
    "created_at": "2011-07-04T20:04:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11564#issuecomment-154483",
    "user": "https://github.com/novoselt"
}
```

<a id='comment:2'></a>
It would be great also to handle higher dimensions, i.e. unfolding a 4-d polytope into 3-d...



---

archive/issue_comments_154484.json:
```json
{
    "body": "<a id='comment:3'></a>\nHow does the code look? Any ideas to make it better would be appreciated",
    "created_at": "2011-07-19T17:58:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11564#issuecomment-154484",
    "user": "https://trac.sagemath.org/admin/accounts/users/QuantumKing"
}
```

<a id='comment:3'></a>
How does the code look? Any ideas to make it better would be appreciated



---

archive/issue_comments_154485.json:
```json
{
    "body": "<a id='comment:4'></a>\nSorry, I am traveling right now and its hard to find time for Sage development.  I do have some comments though.  \n\nNot all the functions in your patch have documentation/doctests.  These are mandatory for every function, so they need to be added.\n\nI think it will make sense to merge our efforts.  I haven't completely fixed up my student's effort but I am attaching the current state so you can take a look at what we did.  There are different strengths and weaknesses in our efforts.  Ours can handle tougher problems, but we don't have a function to check for overlaps.\n\nIf you haven't looked at the master's thesis of Schlickenrieder (http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.28.2631&rep=rep1&type=pdf)\nI highly recommend it.  It was based on his work that my student choose to implement the steepest-edge algorithm.\n\nA good test problem is what Schlickenrieder calls \"turtles\", for example:\n\n```\ntpoints = [[i,j,i^2+j^2] for i in srange(-5,6,1) for j in srange(-5,6,1)]\ntp = Polyhedron(vertices=tpoints)\n```",
    "created_at": "2011-07-21T17:13:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11564#issuecomment-154485",
    "user": "https://trac.sagemath.org/admin/accounts/users/mhampton"
}
```

<a id='comment:4'></a>
Sorry, I am traveling right now and its hard to find time for Sage development.  I do have some comments though.  

Not all the functions in your patch have documentation/doctests.  These are mandatory for every function, so they need to be added.

I think it will make sense to merge our efforts.  I haven't completely fixed up my student's effort but I am attaching the current state so you can take a look at what we did.  There are different strengths and weaknesses in our efforts.  Ours can handle tougher problems, but we don't have a function to check for overlaps.

If you haven't looked at the master's thesis of Schlickenrieder (http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.28.2631&rep=rep1&type=pdf)
I highly recommend it.  It was based on his work that my student choose to implement the steepest-edge algorithm.

A good test problem is what Schlickenrieder calls "turtles", for example:

```
tpoints = [[i,j,i^2+j^2] for i in srange(-5,6,1) for j in srange(-5,6,1)]
tp = Polyhedron(vertices=tpoints)
```



---

archive/issue_comments_154486.json:
```json
{
    "body": "Attachment [trac_11564_preliminary_effort.patch](tarball://root/attachments/some-uuid/ticket11564/trac_11564_preliminary_effort.patch) by mhampton created at 2011-07-21 17:15:23\n\nJust for comparison with other effort, don't review",
    "created_at": "2011-07-21T17:15:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11564#issuecomment-154486",
    "user": "https://trac.sagemath.org/admin/accounts/users/mhampton"
}
```

Attachment [trac_11564_preliminary_effort.patch](tarball://root/attachments/some-uuid/ticket11564/trac_11564_preliminary_effort.patch) by mhampton created at 2011-07-21 17:15:23

Just for comparison with other effort, don't review



---

archive/issue_comments_154487.json:
```json
{
    "body": "<a id='comment:5'></a>\nWhat's the status on this thing? I think it would be nice to have in sage...",
    "created_at": "2012-12-06T15:28:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11564#issuecomment-154487",
    "user": "https://trac.sagemath.org/admin/accounts/users/QuantumKing"
}
```

<a id='comment:5'></a>
What's the status on this thing? I think it would be nice to have in sage...



---

archive/issue_comments_154488.json:
```json
{
    "body": "<a id='comment:6'></a>\nMy status is that I am too busy with other duties to finish this off, although I would really like to.  \n\nThe code in my \"trac_11564_preliminary_effort.patch\" patch is from a student project.  It works, but it could really use some refactoring.\n\nIts possible I would find time to put more effort into this over my winter vacation, but don't count on it.  Otherwise, if no one else works on it I would hope to tackle it in the spring or summer.\n\nAs I mentioned before, your code mainly needed full documentation (doctests).  While it would be nice if it could handle some tough cases, for this kind of functionality something is better than nothing so feel free to finish your version up and I should be able to find time to review it.",
    "created_at": "2012-12-06T17:03:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11564#issuecomment-154488",
    "user": "https://trac.sagemath.org/admin/accounts/users/mhampton"
}
```

<a id='comment:6'></a>
My status is that I am too busy with other duties to finish this off, although I would really like to.  

The code in my "trac_11564_preliminary_effort.patch" patch is from a student project.  It works, but it could really use some refactoring.

Its possible I would find time to put more effort into this over my winter vacation, but don't count on it.  Otherwise, if no one else works on it I would hope to tackle it in the spring or summer.

As I mentioned before, your code mainly needed full documentation (doctests).  While it would be nice if it could handle some tough cases, for this kind of functionality something is better than nothing so feel free to finish your version up and I should be able to find time to review it.



---

archive/issue_events_030278.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11564#event-30278"
}
```



---

archive/issue_events_030279.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11564#event-30279"
}
```



---

archive/issue_events_030280.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11564#event-30280"
}
```



---

archive/issue_events_030281.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11564#event-30281"
}
```



---

archive/issue_events_030282.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11564#event-30282"
}
```



---

archive/issue_events_030283.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11564#event-30283"
}
```



---

archive/issue_events_030284.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11564#event-30284"
}
```



---

archive/issue_comments_154489.json:
```json
{
    "body": "Changing branch from \"\" to \"u/gh-marakortenkamp/poly_net\"",
    "created_at": "2020-10-15T13:14:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11564#issuecomment-154489",
    "user": "https://github.com/marakortenkamp"
}
```

Changing branch from "" to "u/gh-marakortenkamp/poly_net"



---

archive/issue_comments_154490.json:
```json
{
    "body": "Changing author from \"QuantumKing\" to \"Mara Kortenkamp\"",
    "created_at": "2020-10-15T13:14:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11564#issuecomment-154490",
    "user": "https://github.com/marakortenkamp"
}
```

Changing author from "QuantumKing" to "Mara Kortenkamp"



---

archive/issue_comments_154491.json:
```json
{
    "body": "Changing commit from \"\" to \"c353bce99541d6e992ea134744bf130e32ffaf18\"",
    "created_at": "2020-10-15T13:14:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11564#issuecomment-154491",
    "user": "https://github.com/marakortenkamp"
}
```

Changing commit from "" to "c353bce99541d6e992ea134744bf130e32ffaf18"



---

archive/issue_comments_154492.json:
```json
{
    "body": "<a id='comment:13'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                 |\n|-------------------------------------------------------------------------------------------------------------------------------------------|-----------------|\n|[6f9867c](https://git.sagemath.org/sage.git/commit/?id=6f9867c697296d36f54a1db23c5f31cd5280592c)|`Unfolding class`|",
    "created_at": "2020-10-15T15:09:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11564#issuecomment-154492",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:13'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                 |
|-------------------------------------------------------------------------------------------------------------------------------------------|-----------------|
|[6f9867c](https://git.sagemath.org/sage.git/commit/?id=6f9867c697296d36f54a1db23c5f31cd5280592c)|`Unfolding class`|



---

archive/issue_comments_154493.json:
```json
{
    "body": "Changing commit from \"c353bce99541d6e992ea134744bf130e32ffaf18\" to \"6f9867c697296d36f54a1db23c5f31cd5280592c\"",
    "created_at": "2020-10-15T15:09:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11564#issuecomment-154493",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "c353bce99541d6e992ea134744bf130e32ffaf18" to "6f9867c697296d36f54a1db23c5f31cd5280592c"



---

archive/issue_comments_154494.json:
```json
{
    "body": "<a id='comment:14'></a>\nHere are a few things to fix:\n\n- Is the function `faceorder` used? It doesn't seem so.\n- In the description of `faceorder I would say:\n\n```\nReturn a linear ordering of the edges in a spanning tree for the facet adjacency graph of `poly`.\n```\n\n- Many comments are in German: it would be good to have them in English.\n- Many OUTPUT and EXAMPLES are missing.\n- Many lines contain (likely old) \"Versuch\". They should be deleted once every works well...\n- Class Unfolding needs docstring for the class and its methods\n- Some conventions should be checked (spaces around `=` -> ` = ` and such...\n\nI know it is a bit premature for me to give feedback, but it can be used later for a checklist.\n\nI guess also there should be a method in `base.py` called something like `def unfolding(...):` that will return the picture?",
    "created_at": "2020-11-03T16:02:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11564#issuecomment-154494",
    "user": "https://github.com/jplab"
}
```

<a id='comment:14'></a>
Here are a few things to fix:

- Is the function `faceorder` used? It doesn't seem so.
- In the description of `faceorder I would say:

```
Return a linear ordering of the edges in a spanning tree for the facet adjacency graph of `poly`.
```

- Many comments are in German: it would be good to have them in English.
- Many OUTPUT and EXAMPLES are missing.
- Many lines contain (likely old) "Versuch". They should be deleted once every works well...
- Class Unfolding needs docstring for the class and its methods
- Some conventions should be checked (spaces around `=` -> ` = ` and such...

I know it is a bit premature for me to give feedback, but it can be used later for a checklist.

I guess also there should be a method in `base.py` called something like `def unfolding(...):` that will return the picture?



---

archive/issue_events_030285.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-11-03T16:04:30Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11564#event-30285"
}
```



---

archive/issue_events_030286.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-11-03T16:04:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11564#event-30286"
}
```



---

archive/issue_comments_154495.json:
```json
{
    "body": "<a id='comment:15'></a>\nOnce the ticket is more advanced, it would be good to complete the description...",
    "created_at": "2020-11-03T16:04:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11564#issuecomment-154495",
    "user": "https://github.com/jplab"
}
```

<a id='comment:15'></a>
Once the ticket is more advanced, it would be good to complete the description...



---

archive/issue_comments_154496.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Jean-Philippe Labb\u00e9\"",
    "created_at": "2020-11-03T16:04:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11564#issuecomment-154496",
    "user": "https://github.com/jplab"
}
```

Changing reviewer from "" to "Jean-Philippe Labbé"



---

archive/issue_comments_154497.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,5 +1,5 @@\n-An unfolding or a net of a polyhedron is an arrangement of edge-joined polygons in the plane which can be folded (along edges) to become the faces of the polyhedron. There is no overlap between the polygons.\n+An unfolding or a net of a polyhedron is an arrangement of edge-joined polygons in the plane which can be folded (along edges) to become the faces of the polyhedron. \n \n-I've attached code which does this. It will attempt to unfold without overlap a 3D polyhedron defined by the class <class 'sage.geometry.polyhedra.Polyhedron'>.\n+There might be an overlap between polygons (this is an open problem).\n \n-Eric\n+This ticket implement ...\n``````\n",
    "created_at": "2020-11-03T16:04:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11564#issuecomment-154497",
    "user": "https://github.com/jplab"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,5 +1,5 @@
-An unfolding or a net of a polyhedron is an arrangement of edge-joined polygons in the plane which can be folded (along edges) to become the faces of the polyhedron. There is no overlap between the polygons.
+An unfolding or a net of a polyhedron is an arrangement of edge-joined polygons in the plane which can be folded (along edges) to become the faces of the polyhedron. 
 
-I've attached code which does this. It will attempt to unfold without overlap a 3D polyhedron defined by the class <class 'sage.geometry.polyhedra.Polyhedron'>.
+There might be an overlap between polygons (this is an open problem).
 
-Eric
+This ticket implement ...
``````




---

archive/issue_events_030287.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11564#event-30287"
}
```



---

archive/issue_events_030288.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11564#event-30288"
}
```



---

archive/issue_comments_154498.json:
```json
{
    "body": "<a id='comment:16'></a>\nSetting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11564#issuecomment-154498",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:16'></a>
Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_comments_154499.json:
```json
{
    "body": "Changing commit from \"6f9867c697296d36f54a1db23c5f31cd5280592c\" to \"9d4f22788cc1feafd12a03131f10a6390a9ec40e\"",
    "created_at": "2021-04-05T15:15:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11564#issuecomment-154499",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "6f9867c697296d36f54a1db23c5f31cd5280592c" to "9d4f22788cc1feafd12a03131f10a6390a9ec40e"



---

archive/issue_comments_154500.json:
```json
{
    "body": "<a id='comment:17'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                |\n|-------------------------------------------------------------------------------------------------------------------------------------------|----------------|\n|[9d4f227](https://git.sagemath.org/sage.git/commit/?id=9d4f22788cc1feafd12a03131f10a6390a9ec40e)|`added examples`|",
    "created_at": "2021-04-05T15:15:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11564#issuecomment-154500",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:17'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                |
|-------------------------------------------------------------------------------------------------------------------------------------------|----------------|
|[9d4f227](https://git.sagemath.org/sage.git/commit/?id=9d4f22788cc1feafd12a03131f10a6390a9ec40e)|`added examples`|



---

archive/issue_events_030289.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:16:42Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11564#event-30289"
}
```



---

archive/issue_events_030290.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:16:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11564#event-30290"
}
```



---

archive/issue_events_030291.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11564#event-30291"
}
```



---

archive/issue_events_030292.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11564#event-30292"
}
```



---

archive/issue_events_030293.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-01T21:16:35Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11564#event-30293"
}
```



---

archive/issue_events_030294.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-01T21:16:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11564#event-30294"
}
```



---

archive/issue_events_030295.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T05:25:02Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11564#event-30295"
}
```



---

archive/issue_events_030296.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T05:25:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11564",
    "milestone": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11564#event-30296"
}
```
