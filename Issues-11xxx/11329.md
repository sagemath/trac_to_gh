# Issue 11329: make f2c spkg respect CC environment variable

archive/issues_011157.json:
```json
{
    "assignees": [],
    "body": "fixed in #7027\n\nAssignee: **Mariah Lenox**\n\nReviewer: **R. Andrew Ohana**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11329_\n\n",
    "closed_at": "2012-03-29T09:12:18Z",
    "created_at": "2011-05-11T19:57:31Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "make f2c spkg respect CC environment variable",
    "type": "issue",
    "updated_at": "2012-03-29T09:12:18Z",
    "url": "https://github.com/sagemath/sage/issues/11329",
    "user": "https://github.com/sagetrac-mariah"
}
```
fixed in #7027

Assignee: **Mariah Lenox**

Reviewer: **R. Andrew Ohana**

_Issue created by migration from https://trac.sagemath.org/ticket/11329_





---

archive/issue_events_137334.json:
```json
{
    "actor": "https://github.com/sagetrac-mariah",
    "created_at": "2011-05-11T19:57:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11329",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11329#event-137334"
}
```



---

archive/issue_events_137335.json:
```json
{
    "actor": "https://github.com/sagetrac-mariah",
    "created_at": "2011-05-11T19:57:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11329",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11329#event-137335"
}
```



---

archive/issue_events_137336.json:
```json
{
    "actor": "https://github.com/sagetrac-mariah",
    "created_at": "2011-05-11T19:57:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11329",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11329#event-137336"
}
```



---

archive/issue_events_137337.json:
```json
{
    "actor": "https://github.com/sagetrac-mariah",
    "created_at": "2011-05-11T19:57:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11329",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11329#event-137337"
}
```



---

archive/issue_events_137338.json:
```json
{
    "actor": "https://github.com/sagetrac-mariah",
    "created_at": "2011-05-11T20:06:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11329",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11329#event-137338"
}
```



---

archive/issue_comments_116018.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,5 @@\n f2c spkg does not respect the CC environment variable.\n \n+Fix: [http://boxen.math.washington.edu/home/mariah/spkg/f2c-20070816.p3.spkg](http://boxen.math.washington.edu/home/mariah/spkg/f2c-20070816.p3.spkg)\n+\n See Ticket #2999.\n``````\n",
    "created_at": "2011-05-11T20:06:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11329",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11329#issuecomment-116018",
    "user": "https://github.com/sagetrac-mariah"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,5 @@
 f2c spkg does not respect the CC environment variable.
 
+Fix: [http://boxen.math.washington.edu/home/mariah/spkg/f2c-20070816.p3.spkg](http://boxen.math.washington.edu/home/mariah/spkg/f2c-20070816.p3.spkg)
+
 See Ticket #2999.
``````




---

archive/issue_comments_116019.json:
```json
{
    "body": "Diff for the f2c spkg, for reviewing only.",
    "created_at": "2011-05-11T20:07:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11329",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11329#issuecomment-116019",
    "user": "https://github.com/sagetrac-mariah"
}
```

Diff for the f2c spkg, for reviewing only.



---

archive/issue_comments_116020.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nAttachment: **[f2c-20070816.p2-p3.diff.gz](https://github.com/sagemath/sage/files/ticket11329/f2c-20070816.p2-p3.diff.gz)**\n\nfixed in #7027",
    "created_at": "2012-03-22T09:54:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11329",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11329#issuecomment-116020",
    "user": "https://github.com/ohanar"
}
```

<a id='comment:2'>Comment 2:</a>
Attachment: **[f2c-20070816.p2-p3.diff.gz](https://github.com/sagemath/sage/files/ticket11329/f2c-20070816.p2-p3.diff.gz)**

fixed in #7027



---

archive/issue_events_137339.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2012-03-22T09:54:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11329",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11329#event-137339"
}
```



---

archive/issue_events_137340.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2012-03-22T09:54:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11329",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11329#event-137340"
}
```



---

archive/issue_comments_116021.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,5 +1 @@\n-f2c spkg does not respect the CC environment variable.\n-\n-Fix: [http://boxen.math.washington.edu/home/mariah/spkg/f2c-20070816.p3.spkg](http://boxen.math.washington.edu/home/mariah/spkg/f2c-20070816.p3.spkg)\n-\n-See Ticket #2999.\n+fixed in #7027\n``````\n",
    "created_at": "2012-03-22T09:54:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11329",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11329#issuecomment-116021",
    "user": "https://github.com/ohanar"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,5 +1 @@
-f2c spkg does not respect the CC environment variable.
-
-Fix: [http://boxen.math.washington.edu/home/mariah/spkg/f2c-20070816.p3.spkg](http://boxen.math.washington.edu/home/mariah/spkg/f2c-20070816.p3.spkg)
-
-See Ticket #2999.
+fixed in #7027
``````




---

archive/issue_comments_116022.json:
```json
{
    "body": "Reviewer: **R. Andrew Ohana**",
    "created_at": "2012-03-22T09:54:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11329",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11329#issuecomment-116022",
    "user": "https://github.com/ohanar"
}
```

Reviewer: **R. Andrew Ohana**



---

archive/issue_events_137341.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2012-03-22T09:54:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11329",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11329#event-137341"
}
```



---

archive/issue_events_137342.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2012-03-22T09:54:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11329",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11329#event-137342"
}
```



---

archive/issue_events_137343.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2012-03-22T09:54:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11329",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11329#event-137343"
}
```



---

archive/issue_events_137344.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-03-29T09:12:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11329",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11329#event-137344"
}
```



---

archive/issue_events_137345.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-03-29T09:12:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11329",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11329#event-137345"
}
```



---

archive/issue_events_137346.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-03-29T09:12:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11329",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11329#event-137346"
}
```
