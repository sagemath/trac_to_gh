# Issue 11924: lazy_import_cache.get_cache_file doesn't work well if DOT_SAGE doesn't end with a slash

archive/issues_011752.json:
```json
{
    "body": "The function `get_cache_file` in sage/misc/lazy_import_cache.py uses this code:\n\n```python\nreturn \"%s%s-lazy_import_cache.pickle\" % (\n       os.environ['DOT_SAGE'], \n       os.path.realpath(os.path.join(os.environ['SAGE_ROOT'], 'devel', 'sage')).replace(os.sep, '_'))\n```\nSince this just appends to DOT_SAGE, it returns a file in a different directory depending on whether DOT_SAGE ends with a slash or not: if DOT_SAGE is /home/user/.sage/, then this gives \n\n```\n/home/user/.sage/_mangled_SAGE_ROOT_...\n```\nwhereas if DOT_SAGE is /home/user/.sage, then it gives\n\n```\n/home/user/.sage_mangled_SAGE_ROOT_...\n```\nwhich is a file in the directory /home/user/, not in .sage/.\n\nThe attached patch uses os.path.join to combine the parts of the path correctly. (I also wonder if it would be a good idea to store the cache file in a different directory, say `DOT_SAGE/cache/`, so it's hidden more than it is currently?)\n\n\nAssignee: tbd\n\nResolution: fixed\n\nAuthor: John Palmieri\n\nReviewer: Jeroen Demeyer\n\nMerged: sage-4.8.alpha3\n\nIssue created by migration from https://trac.sagemath.org/ticket/11924\n\n",
    "closed_at": "2011-11-21T10:45:22Z",
    "created_at": "2011-10-14T18:40:56Z",
    "labels": [
        "component: misc",
        "critical",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.8",
    "title": "lazy_import_cache.get_cache_file doesn't work well if DOT_SAGE doesn't end with a slash",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/11924",
    "user": "https://github.com/jhpalmieri"
}
```
The function `get_cache_file` in sage/misc/lazy_import_cache.py uses this code:

```python
return "%s%s-lazy_import_cache.pickle" % (
       os.environ['DOT_SAGE'], 
       os.path.realpath(os.path.join(os.environ['SAGE_ROOT'], 'devel', 'sage')).replace(os.sep, '_'))
```
Since this just appends to DOT_SAGE, it returns a file in a different directory depending on whether DOT_SAGE ends with a slash or not: if DOT_SAGE is /home/user/.sage/, then this gives 

```
/home/user/.sage/_mangled_SAGE_ROOT_...
```
whereas if DOT_SAGE is /home/user/.sage, then it gives

```
/home/user/.sage_mangled_SAGE_ROOT_...
```
which is a file in the directory /home/user/, not in .sage/.

The attached patch uses os.path.join to combine the parts of the path correctly. (I also wonder if it would be a good idea to store the cache file in a different directory, say `DOT_SAGE/cache/`, so it's hidden more than it is currently?)


Assignee: tbd

Resolution: fixed

Author: John Palmieri

Reviewer: Jeroen Demeyer

Merged: sage-4.8.alpha3

Issue created by migration from https://trac.sagemath.org/ticket/11924





---

archive/issue_comments_163878.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2011-10-14T18:42:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11924",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11924#issuecomment-163878",
    "user": "https://github.com/jhpalmieri"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_163879.json:
```json
{
    "body": "<a id='comment:2'></a>I've given this \"major\" priority because any user who sets their DOT_SAGE to a string without a trailing slash ends up getting their home directory polluted with Sage lazy import cache files.  For example, on skynet, I have\n\n```\nexport DOT_SAGE=\"/home/palmieri/.sage_$HOSTNAME\"\n```\nand I end up with many files in /home/palmieri: one for each host on which I build and Sage, one for each version of Sage.  Fortunately the files start with a dot so they're hidden, but still, it's not a good situation.\n\n(I can fix this for my situation by adding a trailing slash, but I shouldn't have to.)",
    "created_at": "2011-10-17T00:37:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11924",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11924#issuecomment-163879",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:2'></a>I've given this "major" priority because any user who sets their DOT_SAGE to a string without a trailing slash ends up getting their home directory polluted with Sage lazy import cache files.  For example, on skynet, I have

```
export DOT_SAGE="/home/palmieri/.sage_$HOSTNAME"
```
and I end up with many files in /home/palmieri: one for each host on which I build and Sage, one for each version of Sage.  Fortunately the files start with a dot so they're hidden, but still, it's not a good situation.

(I can fix this for my situation by adding a trailing slash, but I shouldn't have to.)



---

archive/issue_comments_163880.json:
```json
{
    "body": "Changing component from PLEASE CHANGE to misc.",
    "created_at": "2011-10-17T00:37:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11924",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11924#issuecomment-163880",
    "user": "https://github.com/jhpalmieri"
}
```

Changing component from PLEASE CHANGE to misc.



---

archive/issue_comments_163881.json:
```json
{
    "body": "Attachment [trac_11924-lazy.patch](tarball://root/attachments/some-uuid/ticket11924/trac_11924-lazy.patch) by @jhpalmieri created at 2011-10-30 03:15:20",
    "created_at": "2011-10-30T03:15:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11924",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11924#issuecomment-163881",
    "user": "https://github.com/jhpalmieri"
}
```

Attachment [trac_11924-lazy.patch](tarball://root/attachments/some-uuid/ticket11924/trac_11924-lazy.patch) by @jhpalmieri created at 2011-10-30 03:15:20



---

archive/issue_comments_163882.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2011-11-18T12:18:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11924",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11924#issuecomment-163882",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_163883.json:
```json
{
    "body": "Changing priority from major to critical.",
    "created_at": "2011-11-18T12:18:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11924",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11924#issuecomment-163883",
    "user": "https://github.com/jdemeyer"
}
```

Changing priority from major to critical.



---

archive/issue_comments_163884.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Jeroen Demeyer\"",
    "created_at": "2011-11-18T12:18:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11924",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11924#issuecomment-163884",
    "user": "https://github.com/jdemeyer"
}
```

Changing reviewer from "" to "Jeroen Demeyer"



---

archive/issue_comments_163885.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2011-11-21T10:45:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11924",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11924#issuecomment-163885",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed



---

archive/issue_events_031538.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-21T10:45:22Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/11924",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11924#event-31538"
}
```



---

archive/issue_comments_163886.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-4.8.alpha3\"",
    "created_at": "2011-11-21T10:45:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11924",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11924#issuecomment-163886",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-4.8.alpha3"
