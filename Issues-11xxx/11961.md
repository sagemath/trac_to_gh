# Issue 11961: Fixes a bug in is_chordal -- two algorithms

archive/issues_011789.json:
```json
{
    "assignees": [
        "https://github.com/jasongrout",
        "https://github.com/nathanncohen",
        "https://github.com/rlmill"
    ],
    "body": "This ticket follows #11735, which fixes the bug reported by Jan on sage-devel [1]. It makes his alternative algorithm available in the function, so that we now have two different versions available, and still double-check the values it returns `:-)`\n\nRequires : #11735\n\nApply : [attachment: trac_11961.patch](https://github.com/sagemath/sage/files/ticket11961/trac_11961.patch.gz)\n\n[1] https://groups.google.com/d/topic/sage-support/rU1VTz1Ou_I/discussion\n\nComponent: **graph theory**\n\nAuthor: **Nathann Cohen, Jan Elffers**\n\nReviewer: **David Coudert**\n\nMerged: **sage-4.8.alpha5**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11961_\n\n",
    "closed_at": "2011-12-14T22:28:53Z",
    "created_at": "2011-10-29T10:11:03Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fixes a bug in is_chordal -- two algorithms",
    "type": "issue",
    "updated_at": "2011-12-14T22:28:53Z",
    "url": "https://github.com/sagemath/sage/issues/11961",
    "user": "https://github.com/nathanncohen"
}
```
This ticket follows #11735, which fixes the bug reported by Jan on sage-devel [1]. It makes his alternative algorithm available in the function, so that we now have two different versions available, and still double-check the values it returns `:-)`

Requires : #11735

Apply : [attachment: trac_11961.patch](https://github.com/sagemath/sage/files/ticket11961/trac_11961.patch.gz)

[1] https://groups.google.com/d/topic/sage-support/rU1VTz1Ou_I/discussion

Component: **graph theory**

Author: **Nathann Cohen, Jan Elffers**

Reviewer: **David Coudert**

Merged: **sage-4.8.alpha5**

_Issue created by migration from https://trac.sagemath.org/ticket/11961_





---

archive/issue_events_161662.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-10-29T10:11:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11961",
    "milestone_number": null,
    "milestone_title": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11961#event-161662"
}
```



---

archive/issue_events_161663.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-10-29T10:11:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11961",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "c: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11961#event-161663"
}
```



---

archive/issue_events_161664.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-10-29T10:11:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11961",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11961#event-161664"
}
```



---

archive/issue_events_161665.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-10-29T10:11:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11961",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11961#event-161665"
}
```



---

archive/issue_events_161666.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2011-10-29T10:11:03Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/11961",
    "subject": "https://github.com/nathanncohen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11961#event-161666"
}
```



---

archive/issue_events_161667.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-10-29T10:11:03Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/11961",
    "subject": "https://github.com/nathanncohen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11961#event-161667"
}
```



---

archive/issue_events_161668.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2011-10-29T10:11:03Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/11961",
    "subject": "https://github.com/nathanncohen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11961#event-161668"
}
```



---

archive/issue_events_161669.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2011-10-29T10:12:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11961",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11961#event-161669"
}
```



---

archive/issue_comments_126802.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,6 +2,6 @@\n \n Requires : #11735\n \n-Apply : [attachment:]\n+Apply : [attachment: trac_11961.patch](https://github.com/sagemath/sage/files/ticket11961/trac_11961.patch.gz)\n \n [1] https://groups.google.com/d/topic/sage-support/rU1VTz1Ou_I/discussion\n``````\n",
    "created_at": "2011-10-29T10:12:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11961#issuecomment-126802",
    "user": "https://github.com/nathanncohen"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,6 +2,6 @@
 
 Requires : #11735
 
-Apply : [attachment:]
+Apply : [attachment: trac_11961.patch](https://github.com/sagemath/sage/files/ticket11961/trac_11961.patch.gz)
 
 [1] https://groups.google.com/d/topic/sage-support/rU1VTz1Ou_I/discussion
``````




---

archive/issue_comments_126803.json:
```json
{
    "body": "Milestone sage-4.7.3 deleted",
    "created_at": "2011-11-03T16:14:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11961#issuecomment-126803",
    "user": "https://github.com/jdemeyer"
}
```

Milestone sage-4.7.3 deleted



---

archive/issue_events_161670.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-03T16:14:43Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11961",
    "milestone_number": null,
    "milestone_title": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11961#event-161670"
}
```



---

archive/issue_comments_126804.json:
```json
{
    "body": "Attachment: **[trac_11961.patch.gz](https://github.com/sagemath/sage/files/ticket11961/trac_11961.patch.gz)**",
    "created_at": "2011-11-06T20:26:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11961#issuecomment-126804",
    "user": "https://github.com/nathanncohen"
}
```

Attachment: **[trac_11961.patch.gz](https://github.com/sagemath/sage/files/ticket11961/trac_11961.patch.gz)**



---

archive/issue_events_161671.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2011-12-10T19:32:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11961",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11961#event-161671"
}
```



---

archive/issue_events_161672.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2011-12-10T19:32:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11961",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11961#event-161672"
}
```



---

archive/issue_events_161673.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2011-12-10T19:32:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11961",
    "milestone_number": null,
    "milestone_title": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11961#event-161673"
}
```



---

archive/issue_comments_126805.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nI had no problem when installing the patch. The doc is also correctly generated.\n\nAll tests I have performed are consistent: both algorithms return the same boolean value. The certificate are not the same when several certificates are possible.\n\nAlgorithm B is a bit slower than algorithm A.\n\nAs I already said for patch #11735, significant running time improvements are possible for these linear time algorithms, but at least we have some algorithms ;)\n\nThank you Nathann.",
    "created_at": "2011-12-10T19:32:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11961#issuecomment-126805",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:3" align="right">comment:3</div>

I had no problem when installing the patch. The doc is also correctly generated.

All tests I have performed are consistent: both algorithms return the same boolean value. The certificate are not the same when several certificates are possible.

Algorithm B is a bit slower than algorithm A.

As I already said for patch #11735, significant running time improvements are possible for these linear time algorithms, but at least we have some algorithms ;)

Thank you Nathann.



---

archive/issue_comments_126806.json:
```json
{
    "body": "Reviewer: **David Coudert**",
    "created_at": "2011-12-10T19:32:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11961#issuecomment-126806",
    "user": "https://github.com/dcoudert"
}
```

Reviewer: **David Coudert**



---

archive/issue_comments_126807.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nThank you for the review ! `:-)`\n\n> Algorithm B is a bit slower than algorithm A.\n\nYep. That's why it is good to have both `:-)`\n\n> As I already said for patch #11735, significant running time improvements are possible for these linear time algorithms, but at least we have some algorithms ;)\n\nThese codes are really standard. It would really be a shame not to implement them properly in C/C++ at some point.\n\nNathann",
    "created_at": "2011-12-10T19:41:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11961#issuecomment-126807",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:4" align="right">comment:4</div>

Thank you for the review ! `:-)`

> Algorithm B is a bit slower than algorithm A.

Yep. That's why it is good to have both `:-)`

> As I already said for patch #11735, significant running time improvements are possible for these linear time algorithms, but at least we have some algorithms ;)

These codes are really standard. It would really be a shame not to implement them properly in C/C++ at some point.

Nathann



---

archive/issue_comments_126808.json:
```json
{
    "body": "Merged: **sage-4.8.alpha5**",
    "created_at": "2011-12-14T22:28:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11961#issuecomment-126808",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.8.alpha5**



---

archive/issue_events_161674.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-12-14T22:28:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11961",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11961#event-161674"
}
```



---

archive/issue_events_161675.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-12-14T22:28:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11961",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11961#event-161675"
}
```
