# Issue 11165: core and quotient of partitions should be partitions

archive/issues_011012.json:
```json
{
    "assignees": [],
    "body": "Currently the methods core and quotient returns respectively a list and a list\nof list:\n\n```\nsage: Partition([7,7,5,3,3,3,1]).core(3)\n[1, 1]\nsage: type(Partition([7,7,5,3,3,3,1]).core(3))\n<type 'list'>\n\nsage:  sage: Partition([7,7,5,3,3,3,1]).quotient(3)\n[[2], [1], [2, 2, 2]]\nsage:  type(Partition([7,7,5,3,3,3,1]).quotient(3)[0])\n<type 'list'>\n```\nMathematically, those lists are partitions and moreover list are not\nhashable. As a consequence, in most use we have no choice but to change them\nto partitions. So after the patch, core should returns a partition and\nquotient a tuple of partitions.\n\n\n\nAssignee: **sage-combinat**\n\nCC:  sage-combinat\n\nKeywords: **partitions core quotient**\n\nAuthor: **Florent Hivert**\n\nReviewer: **Mike Hansen**\n\nMerged: **sage-4.7.1.alpha3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11165_\n\n",
    "closed_at": "2011-06-08T07:13:29Z",
    "created_at": "2011-04-10T08:23:05Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "core and quotient of partitions should be partitions",
    "type": "issue",
    "updated_at": "2011-06-08T07:13:29Z",
    "url": "https://github.com/sagemath/sage/issues/11165",
    "user": "https://github.com/hivert"
}
```
Currently the methods core and quotient returns respectively a list and a list
of list:

```
sage: Partition([7,7,5,3,3,3,1]).core(3)
[1, 1]
sage: type(Partition([7,7,5,3,3,3,1]).core(3))
<type 'list'>

sage:  sage: Partition([7,7,5,3,3,3,1]).quotient(3)
[[2], [1], [2, 2, 2]]
sage:  type(Partition([7,7,5,3,3,3,1]).quotient(3)[0])
<type 'list'>
```
Mathematically, those lists are partitions and moreover list are not
hashable. As a consequence, in most use we have no choice but to change them
to partitions. So after the patch, core should returns a partition and
quotient a tuple of partitions.



Assignee: **sage-combinat**

CC:  sage-combinat

Keywords: **partitions core quotient**

Author: **Florent Hivert**

Reviewer: **Mike Hansen**

Merged: **sage-4.7.1.alpha3**

_Issue created by migration from https://trac.sagemath.org/ticket/11165_





---

archive/issue_events_130641.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2011-04-10T08:23:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11165",
    "milestone_number": null,
    "milestone_title": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11165#event-130641"
}
```



---

archive/issue_events_130642.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2011-04-10T08:23:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11165",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11165#event-130642"
}
```



---

archive/issue_events_130643.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2011-04-10T08:23:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11165",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11165#event-130643"
}
```



---

archive/issue_events_130644.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2011-04-10T08:23:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11165",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11165#event-130644"
}
```



---

archive/issue_events_130645.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2011-04-10T08:56:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11165",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11165#event-130645"
}
```



---

archive/issue_comments_113208.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[trac_11165-partitions_core_quotient_fix-fh.patch.gz](https://github.com/sagemath/sage/files/ticket11165/trac_11165-partitions_core_quotient_fix-fh.patch.gz)**\n\nPatch ready for review",
    "created_at": "2011-04-10T08:56:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11165",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11165#issuecomment-113208",
    "user": "https://github.com/hivert"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[trac_11165-partitions_core_quotient_fix-fh.patch.gz](https://github.com/sagemath/sage/files/ticket11165/trac_11165-partitions_core_quotient_fix-fh.patch.gz)**

Patch ready for review



---

archive/issue_events_130646.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2011-05-27T22:09:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11165",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11165#event-130646"
}
```



---

archive/issue_events_130647.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2011-05-27T22:09:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11165",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11165#event-130647"
}
```



---

archive/issue_comments_113209.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nLooks good to me.",
    "created_at": "2011-05-27T22:09:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11165",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11165#issuecomment-113209",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:2'>Comment 2:</a>
Looks good to me.



---

archive/issue_comments_113210.json:
```json
{
    "body": "Reviewer: **Mike Hansen**",
    "created_at": "2011-05-27T22:09:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11165",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11165#issuecomment-113210",
    "user": "https://github.com/mwhansen"
}
```

Reviewer: **Mike Hansen**



---

archive/issue_comments_113211.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nthe construction of partitions starting with a core and a finite sequence of quotients is broken, please look at #11412 for a fix on top of this patch",
    "created_at": "2011-05-31T12:59:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11165",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11165#issuecomment-113211",
    "user": "https://github.com/sagetrac-pdehaye"
}
```

<a id='comment:3'>Comment 3:</a>
the construction of partitions starting with a core and a finite sequence of quotients is broken, please look at #11412 for a fix on top of this patch



---

archive/issue_comments_113212.json:
```json
{
    "body": "Merged: **sage-4.7.1.alpha3**",
    "created_at": "2011-06-08T07:13:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11165",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11165#issuecomment-113212",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.7.1.alpha3**



---

archive/issue_events_130648.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-08T07:13:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11165",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11165#event-130648"
}
```



---

archive/issue_events_130649.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-08T07:13:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11165",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11165#event-130649"
}
```
