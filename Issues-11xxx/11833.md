# Issue 11833: Class for ternary quadratic forms

archive/issues_011661.json:
```json
{
    "assignees": [],
    "body": "New class of ternary quadratic forms\n\nAssignee: **@sagetrac-justin**\n\nReviewer: **Leif Leonhardy**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11833_\n\n",
    "closed_at": "2011-09-24T12:27:52Z",
    "created_at": "2011-09-22T20:09:35Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20quadratic%20forms",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Class for ternary quadratic forms",
    "type": "issue",
    "updated_at": "2011-09-24T12:27:52Z",
    "url": "https://github.com/sagemath/sage/issues/11833",
    "user": "https://github.com/sagetrac-gdrama"
}
```
New class of ternary quadratic forms

Assignee: **@sagetrac-justin**

Reviewer: **Leif Leonhardy**

_Issue created by migration from https://trac.sagemath.org/ticket/11833_





---

archive/issue_events_145539.json:
```json
{
    "actor": "https://github.com/sagetrac-gdrama",
    "created_at": "2011-09-22T20:09:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11833",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20quadratic%20forms",
    "label_color": "0000ff",
    "label_name": "component: quadratic forms",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11833#event-145539"
}
```



---

archive/issue_events_145540.json:
```json
{
    "actor": "https://github.com/sagetrac-gdrama",
    "created_at": "2011-09-22T20:09:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11833",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11833#event-145540"
}
```



---

archive/issue_events_145541.json:
```json
{
    "actor": "https://github.com/sagetrac-gdrama",
    "created_at": "2011-09-22T20:09:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11833",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11833#event-145541"
}
```



---

archive/issue_events_145542.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2011-09-24T12:27:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11833",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11833#event-145542"
}
```



---

archive/issue_events_145543.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2011-09-24T12:27:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11833",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11833#event-145543"
}
```



---

archive/issue_comments_125937.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nClosing this as a duplicate of #11835.",
    "created_at": "2011-09-24T12:27:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11833",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11833#issuecomment-125937",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:1" align="right">Comment 1</div>

Closing this as a duplicate of #11835.



---

archive/issue_comments_125938.json:
```json
{
    "body": "Reviewer: **Leif Leonhardy**",
    "created_at": "2011-09-24T12:27:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11833",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11833#issuecomment-125938",
    "user": "https://github.com/nexttime"
}
```

Reviewer: **Leif Leonhardy**



---

archive/issue_events_145544.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2011-09-24T12:27:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11833",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11833#event-145544"
}
```
