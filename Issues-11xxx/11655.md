# Issue 11655: Maxima missing rectform simplification after integral()

archive/issues_011483.json:
```json
{
    "body": "```\nvar('a b c x', domain='real')\nA = (sin(a) * x^2+sin(b) *x + sin(c)) * exp(-x^2)\nAint = A.integrate(x,-infinity,infinity)\n```\n\nA.imag() is 0\n\nAint.imag() is a long expression, which doesn't simplify to 0. This surprising for the user.\n\nA slightly different example, actually a generalization of the one above, just works:\n\n```\nvar('a b c x', domain='real')\nB = (a * x^2+b *x + c) * exp(-x^2)\nBint = B.integrate(x,-infinity,infinity)\n```\n\nExamples can be found here: http://demo.sagenb.org/home/pub/179\n\n\nAssignee: @burcin\n\nCC:  @kcrisman\n\nStatus: new\n\nIssue created by migration from https://trac.sagemath.org/ticket/11655\n\n",
    "created_at": "2011-08-06T19:11:39Z",
    "labels": [
        "component: symbolics",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.4",
    "title": "Maxima missing rectform simplification after integral()",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/11655",
    "user": "https://trac.sagemath.org/admin/accounts/users/jan"
}
```
```
var('a b c x', domain='real')
A = (sin(a) * x^2+sin(b) *x + sin(c)) * exp(-x^2)
Aint = A.integrate(x,-infinity,infinity)
```

A.imag() is 0

Aint.imag() is a long expression, which doesn't simplify to 0. This surprising for the user.

A slightly different example, actually a generalization of the one above, just works:

```
var('a b c x', domain='real')
B = (a * x^2+b *x + c) * exp(-x^2)
Bint = B.integrate(x,-infinity,infinity)
```

Examples can be found here: http://demo.sagenb.org/home/pub/179


Assignee: @burcin

CC:  @kcrisman

Status: new

Issue created by migration from https://trac.sagemath.org/ticket/11655





---

archive/issue_events_030605.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11655",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11655#event-30605"
}
```



---

archive/issue_events_030606.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11655",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11655#event-30606"
}
```



---

archive/issue_events_030607.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11655",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11655#event-30607"
}
```



---

archive/issue_events_030608.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11655",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11655#event-30608"
}
```



---

archive/issue_events_030609.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11655",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11655#event-30609"
}
```



---

archive/issue_events_030610.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11655",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11655#event-30610"
}
```



---

archive/issue_events_030611.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11655",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11655#event-30611"
}
```



---

archive/issue_comments_156793.json:
```json
{
    "body": "<a id='comment:6'></a>This seems to be a missing rectform simplification because\n\n```\nsage: Aint.expand().simplify_rectform()\n1/2*sqrt(pi)*sin(a) + sqrt(pi)*sin(c)\n```\nThe bug is then rather that Maxima does it not in all cases.",
    "created_at": "2015-01-31T09:25:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11655",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11655#issuecomment-156793",
    "user": "https://github.com/rwst"
}
```

<a id='comment:6'></a>This seems to be a missing rectform simplification because

```
sage: Aint.expand().simplify_rectform()
1/2*sqrt(pi)*sin(a) + sqrt(pi)*sin(c)
```
The bug is then rather that Maxima does it not in all cases.
