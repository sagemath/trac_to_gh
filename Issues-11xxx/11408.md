# Issue 11408: Make sig_on() and sig_off() really macros

archive/issues_011236.json:
```json
{
    "assignees": [],
    "body": "Currently, sig_on() and sig_off() are partially macros and partially inline functions.  I plan to make them completely macros and check whether that increases performance.\n\nAssignee: **@jdemeyer**\n\nReviewer: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11408_\n\n",
    "closed_at": "2013-04-08T09:51:53Z",
    "created_at": "2011-05-31T08:33:43Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20c%20lib",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make sig_on() and sig_off() really macros",
    "type": "issue",
    "updated_at": "2013-04-08T09:51:53Z",
    "url": "https://github.com/sagemath/sage/issues/11408",
    "user": "https://github.com/jdemeyer"
}
```
Currently, sig_on() and sig_off() are partially macros and partially inline functions.  I plan to make them completely macros and check whether that increases performance.

Assignee: **@jdemeyer**

Reviewer: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/11408_





---

archive/issue_events_138572.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-31T08:33:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11408",
    "milestone_number": null,
    "milestone_title": "sage-5.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11408#event-138572"
}
```



---

archive/issue_events_138573.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-31T08:33:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11408",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20c%20lib",
    "label_color": "000080",
    "label_name": "component: c lib",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11408#event-138573"
}
```



---

archive/issue_events_138574.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-31T08:33:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11408",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11408#event-138574"
}
```



---

archive/issue_events_138575.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-31T08:33:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11408",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11408#event-138575"
}
```



---

archive/issue_comments_117512.json:
```json
{
    "body": "Attachment: **[11408_sig_on_macro.gz](https://github.com/sagemath/sage/files/ticket11408/11408_sig_on_macro.gz)**",
    "created_at": "2012-04-09T08:42:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11408",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11408#issuecomment-117512",
    "user": "https://github.com/jdemeyer"
}
```

Attachment: **[11408_sig_on_macro.gz](https://github.com/sagemath/sage/files/ticket11408/11408_sig_on_macro.gz)**



---

archive/issue_comments_117513.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2013-04-08T09:51:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11408",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11408#issuecomment-117513",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_comments_117514.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nThis isn't really an issue.",
    "created_at": "2013-04-08T09:51:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11408",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11408#issuecomment-117514",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:1" align="right">Comment 1</div>

This isn't really an issue.



---

archive/issue_events_138576.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-04-08T09:51:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11408",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11408#event-138576"
}
```



---

archive/issue_events_138577.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-04-08T09:51:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11408",
    "milestone_number": null,
    "milestone_title": "sage-5.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11408#event-138577"
}
```



---

archive/issue_events_138578.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-04-08T09:51:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11408",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11408#event-138578"
}
```



---

archive/issue_events_138579.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-04-08T09:51:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11408",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11408#event-138579"
}
```
