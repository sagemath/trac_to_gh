# Issue 11908: Fix tree plotting again

archive/issues_011736.json:
```json
{
    "assignees": [],
    "body": "I wrote some code that produces fairly nice-looking tree plots in #6747, and, I might point out, worked great.  For some reason, this was obliterated in #7004 with code that almost does the right thing, but fails to draw the tree crossing-free with high probability.\n\nThe easy thing is to add a parameter `shuffle` to `GenericGraph.layout_ranked`, so `GenericGraph.layout_tree` could just call `return self.layout_ranked(heights_dict,shuffle=False)` to avoid the (somewhat baffling) behavior of shuffling the vertices along the heights.  IMHO, the default should be to not shuffle... but the primary focus of this ticket is to fix the damned tree plots.  Again.\n\nMy preference is to bring back my code from #6747, since the results are pretty, and I find the \"shrink-wrapped\" look of layout_ranked to be incomprehensible for large-ish trees.  A good compromise is to add options: layout = \"tree\", \"tree:hang\", \"tree:shrinkwrap\", \"tree:circle\", etc.\n\nNote to reviewers:  look at the plots produced in the doctests before giving a positive review.\n\nApply:\n\n* [attachment: trac_11908-fc.patch.gz](https://github.com/sagemath/sage/files/ticket11908/trac_11908-fc.patch.gz)\n* [attachment: trac_11908-rev.patch.gz](https://github.com/sagemath/sage/files/ticket11908/trac_11908-rev.patch.gz)\n\nAssignee: **@jasongrout, @nathanncohen, @rlmill**\n\nCC:  @nthiery @videlec @nathanncohen\n\nKeywords: **tree, plot**\n\nReviewer: **Nathann Cohen**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\nMerged: **sage-5.10.beta3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11908_\n\n",
    "closed_at": "2013-05-13T13:26:13Z",
    "created_at": "2011-10-08T19:37:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.10",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix tree plotting again",
    "type": "issue",
    "updated_at": "2013-05-13T13:26:13Z",
    "url": "https://github.com/sagemath/sage/issues/11908",
    "user": "https://github.com/sagetrac-boothby"
}
```
I wrote some code that produces fairly nice-looking tree plots in #6747, and, I might point out, worked great.  For some reason, this was obliterated in #7004 with code that almost does the right thing, but fails to draw the tree crossing-free with high probability.

The easy thing is to add a parameter `shuffle` to `GenericGraph.layout_ranked`, so `GenericGraph.layout_tree` could just call `return self.layout_ranked(heights_dict,shuffle=False)` to avoid the (somewhat baffling) behavior of shuffling the vertices along the heights.  IMHO, the default should be to not shuffle... but the primary focus of this ticket is to fix the damned tree plots.  Again.

My preference is to bring back my code from #6747, since the results are pretty, and I find the "shrink-wrapped" look of layout_ranked to be incomprehensible for large-ish trees.  A good compromise is to add options: layout = "tree", "tree:hang", "tree:shrinkwrap", "tree:circle", etc.

Note to reviewers:  look at the plots produced in the doctests before giving a positive review.

Apply:

* [attachment: trac_11908-fc.patch.gz](https://github.com/sagemath/sage/files/ticket11908/trac_11908-fc.patch.gz)
* [attachment: trac_11908-rev.patch.gz](https://github.com/sagemath/sage/files/ticket11908/trac_11908-rev.patch.gz)

Assignee: **@jasongrout, @nathanncohen, @rlmill**

CC:  @nthiery @videlec @nathanncohen

Keywords: **tree, plot**

Reviewer: **Nathann Cohen**

Author: **Frédéric Chapoton**

Merged: **sage-5.10.beta3**

_Issue created by migration from https://trac.sagemath.org/ticket/11908_





---

archive/issue_events_146807.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2011-10-08T19:37:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "milestone_number": null,
    "milestone_title": "sage-5.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11908#event-146807"
}
```



---

archive/issue_events_146808.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2011-10-08T19:37:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11908#event-146808"
}
```



---

archive/issue_events_146809.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2011-10-08T19:37:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11908#event-146809"
}
```



---

archive/issue_events_146810.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2011-10-08T19:37:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11908#event-146810"
}
```



---

archive/issue_comments_127720.json:
```json
{
    "body": "Author: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2013-05-06T19:20:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11908#issuecomment-127720",
    "user": "https://github.com/fchapoton"
}
```

Author: **Frédéric Chapoton**



---

archive/issue_events_146811.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-06T19:20:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11908#event-146811"
}
```



---

archive/issue_comments_127721.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nhello there, here is a patch, needs review !",
    "created_at": "2013-05-06T19:20:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11908#issuecomment-127721",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:1'>Comment 1:</a>
hello there, here is a patch, needs review !



---

archive/issue_comments_127722.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nHellooooooo Frederic !\n\nLooks like your patch does not apply cleanly on 5.10.beta1. This being said, what do you want to achieve with this patch ? What are the bad properties of the current implementation that you want to fix (I really have no idea, I never used that code) ? Do you want the drawing to be planar, for instance (no edge crossings) ?\n\nNathann",
    "created_at": "2013-05-07T16:45:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11908#issuecomment-127722",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:3'>Comment 3:</a>
Hellooooooo Frederic !

Looks like your patch does not apply cleanly on 5.10.beta1. This being said, what do you want to achieve with this patch ? What are the bad properties of the current implementation that you want to fix (I really have no idea, I never used that code) ? Do you want the drawing to be planar, for instance (no edge crossings) ?

Nathann



---

archive/issue_events_146812.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-05-07T16:45:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11908#event-146812"
}
```



---

archive/issue_events_146813.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-05-07T16:45:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11908#event-146813"
}
```



---

archive/issue_comments_127723.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nOh, and what about using \"g.center()\" as the root when none is provided ? It should minimize the tree's height.\n\nNathann",
    "created_at": "2013-05-07T16:50:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11908#issuecomment-127723",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:4'>Comment 4:</a>
Oh, and what about using "g.center()" as the root when none is provided ? It should minimize the tree's height.

Nathann



---

archive/issue_comments_127724.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nYes, the main aim is to have a drawing of a tree without crossings. With the current algorithm, this is not the case\n\n```\ng = graphs.RandomTree(80)\ng.plot(layout='tree')\n```\ngives a very messy picture, which does not look like a tree at all.\n\nThanks for the idea of using the center, I will take care of that as soon as I can do the rebase.",
    "created_at": "2013-05-07T19:44:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11908#issuecomment-127724",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:5'>Comment 5:</a>
Yes, the main aim is to have a drawing of a tree without crossings. With the current algorithm, this is not the case

```
g = graphs.RandomTree(80)
g.plot(layout='tree')
```
gives a very messy picture, which does not look like a tree at all.

Thanks for the idea of using the center, I will take care of that as soon as I can do the rebase.



---

archive/issue_comments_127725.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nhere is a rebased patch on 5.10.beta1\n\nI have used the idea of the center as default root",
    "created_at": "2013-05-08T09:32:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11908#issuecomment-127725",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:6'>Comment 6:</a>
here is a rebased patch on 5.10.beta1

I have used the idea of the center as default root



---

archive/issue_events_146814.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-08T09:32:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11908#event-146814"
}
```



---

archive/issue_events_146815.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-08T09:32:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11908#event-146815"
}
```



---

archive/issue_comments_127726.json:
```json
{
    "body": "Changed keywords from **** to **tree, plot**",
    "created_at": "2013-05-08T09:33:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11908#issuecomment-127726",
    "user": "https://github.com/fchapoton"
}
```

Changed keywords from **** to **tree, plot**



---

archive/issue_comments_127727.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\n(while testing the patch)\n\nAhahahah. Yeah, it is indeed considerably better than what we currently have `:-D`\n\nNathann",
    "created_at": "2013-05-08T10:20:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11908#issuecomment-127727",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:8'>Comment 8:</a>
(while testing the patch)

Ahahahah. Yeah, it is indeed considerably better than what we currently have `:-D`

Nathann



---

archive/issue_comments_127728.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\none moment please, I have to correct a typo in the doc",
    "created_at": "2013-05-08T10:21:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11908#issuecomment-127728",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:9'>Comment 9:</a>
one moment please, I have to correct a typo in the doc



---

archive/issue_comments_127729.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nAttachment: **[trac_11908-fc.patch.gz](https://github.com/sagemath/sage/files/ticket11908/trac_11908-fc.patch.gz)**\n\ntypo corrected, done",
    "created_at": "2013-05-08T10:23:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11908#issuecomment-127729",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:10'>Comment 10:</a>
Attachment: **[trac_11908-fc.patch.gz](https://github.com/sagemath/sage/files/ticket11908/trac_11908-fc.patch.gz)**

typo corrected, done



---

archive/issue_comments_127730.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nTwo questions while I read and try too understand the algorithm.\n\n* Why don't you treat the orientation up/down as you do right/left, i.e. at the end of the algorithm ?\n\n* Why is `obstruction[y] = max(x+1, obstruction[y]) ` not `max(x+dx, obstruction[y])` ? You probably have a nice reason for that, as the plot are very pretty `:-)`\n\nNathann",
    "created_at": "2013-05-08T10:43:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11908#issuecomment-127730",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:11'>Comment 11:</a>
Two questions while I read and try too understand the algorithm.

* Why don't you treat the orientation up/down as you do right/left, i.e. at the end of the algorithm ?

* Why is `obstruction[y] = max(x+1, obstruction[y]) ` not `max(x+dx, obstruction[y])` ? You probably have a nice reason for that, as the plot are very pretty `:-)`

Nathann



---

archive/issue_comments_127731.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\n* Well, this is not my algorithm, it comes directly from Tom Boothby's code in #6747. I have not tried to understand how it works.\n\n* I have introduced right and left orientations, in a simple-minded way. Maybe one can refactor the code.",
    "created_at": "2013-05-08T10:47:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11908#issuecomment-127731",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:12'>Comment 12:</a>
* Well, this is not my algorithm, it comes directly from Tom Boothby's code in #6747. I have not tried to understand how it works.

* I have introduced right and left orientations, in a simple-minded way. Maybe one can refactor the code.



---

archive/issue_comments_127732.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -5,3 +5,8 @@\n My preference is to bring back my code from #6747, since the results are pretty, and I find the \"shrink-wrapped\" look of layout_ranked to be incomprehensible for large-ish trees.  A good compromise is to add options: layout = \"tree\", \"tree:hang\", \"tree:shrinkwrap\", \"tree:circle\", etc.\n \n Note to reviewers:  look at the plots produced in the doctests before giving a positive review.\n+\n+Apply:\n+\n+* [attachment: trac_11908-fc.patch.gz](https://github.com/sagemath/sage/files/ticket11908/trac_11908-fc.patch.gz)\n+* [attachment: trac_11908-rev.patch.gz](https://github.com/sagemath/sage/files/ticket11908/trac_11908-rev.patch.gz)\n``````\n",
    "created_at": "2013-05-08T12:02:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11908#issuecomment-127732",
    "user": "https://github.com/nathanncohen"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -5,3 +5,8 @@
 My preference is to bring back my code from #6747, since the results are pretty, and I find the "shrink-wrapped" look of layout_ranked to be incomprehensible for large-ish trees.  A good compromise is to add options: layout = "tree", "tree:hang", "tree:shrinkwrap", "tree:circle", etc.
 
 Note to reviewers:  look at the plots produced in the doctests before giving a positive review.
+
+Apply:
+
+* [attachment: trac_11908-fc.patch.gz](https://github.com/sagemath/sage/files/ticket11908/trac_11908-fc.patch.gz)
+* [attachment: trac_11908-rev.patch.gz](https://github.com/sagemath/sage/files/ticket11908/trac_11908-rev.patch.gz)
``````




---

archive/issue_comments_127733.json:
```json
{
    "body": "Reviewer: **Nathann Cohen**",
    "created_at": "2013-05-08T12:02:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11908#issuecomment-127733",
    "user": "https://github.com/nathanncohen"
}
```

Reviewer: **Nathann Cohen**



---

archive/issue_comments_127734.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nAttachment: **[trac_11908-rev.patch.gz](https://github.com/sagemath/sage/files/ticket11908/trac_11908-rev.patch.gz)**\n\nHellooooooooooooo !\n\nIt took me some time to understand it, but it's done. I added comments, to help the next one who will have to read that.\n\nIf you agree with those changes, you can set the ticket to `positive_review` ! Nice layout `:-)`\n\nNathann",
    "created_at": "2013-05-08T12:02:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11908#issuecomment-127734",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:13'>Comment 13:</a>
Attachment: **[trac_11908-rev.patch.gz](https://github.com/sagemath/sage/files/ticket11908/trac_11908-rev.patch.gz)**

Hellooooooooooooo !

It took me some time to understand it, but it's done. I added comments, to help the next one who will have to read that.

If you agree with those changes, you can set the ticket to `positive_review` ! Nice layout `:-)`

Nathann



---

archive/issue_events_146816.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-08T13:05:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11908#event-146816"
}
```



---

archive/issue_events_146817.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-08T13:05:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11908#event-146817"
}
```



---

archive/issue_comments_127735.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nok, looks good to me. Positive review.",
    "created_at": "2013-05-08T13:05:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11908#issuecomment-127735",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:14'>Comment 14:</a>
ok, looks good to me. Positive review.



---

archive/issue_comments_127736.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nSorry to pipe up now... but can one of you add in the following test, or something similar?\n\n```\n    sage: T = graphs.RandomTree(1000)\n    sage: T.plot(layout='tree',save_pos=True)\n    sage: T.is_drawn_free_of_edge_crossings()\n    True\n```",
    "created_at": "2013-05-09T16:46:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11908#issuecomment-127736",
    "user": "https://github.com/sagetrac-boothby"
}
```

<a id='comment:15'>Comment 15:</a>
Sorry to pipe up now... but can one of you add in the following test, or something similar?

```
    sage: T = graphs.RandomTree(1000)
    sage: T.plot(layout='tree',save_pos=True)
    sage: T.is_drawn_free_of_edge_crossings()
    True
```



---

archive/issue_events_146818.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2013-05-09T16:46:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11908#event-146818"
}
```



---

archive/issue_events_146819.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2013-05-09T16:46:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11908#event-146819"
}
```



---

archive/issue_comments_127737.json:
```json
{
    "body": "Attachment: **[trac_11908-more_test.patch.gz](https://github.com/sagemath/sage/files/ticket11908/trac_11908-more_test.patch.gz)**",
    "created_at": "2013-05-09T17:49:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11908#issuecomment-127737",
    "user": "https://github.com/fchapoton"
}
```

Attachment: **[trac_11908-more_test.patch.gz](https://github.com/sagemath/sage/files/ticket11908/trac_11908-more_test.patch.gz)**



---

archive/issue_comments_127738.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nok, done. If you are satisfied, please consider putting back the positive review.",
    "created_at": "2013-05-09T17:50:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11908#issuecomment-127738",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:16'>Comment 16:</a>
ok, done. If you are satisfied, please consider putting back the positive review.



---

archive/issue_events_146820.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-09T17:50:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11908#event-146820"
}
```



---

archive/issue_events_146821.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-09T17:50:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11908#event-146821"
}
```



---

archive/issue_comments_127739.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nThis is exactly the thing asked for, so I think you are safe in setting it to positive review as long as patchbot agrees...",
    "created_at": "2013-05-09T19:21:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11908#issuecomment-127739",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:17'>Comment 17:</a>
This is exactly the thing asked for, so I think you are safe in setting it to positive review as long as patchbot agrees...



---

archive/issue_comments_127740.json:
```json
{
    "body": "<a id='comment:18'>Comment 18:</a>\nGreen light !\n\nNathann",
    "created_at": "2013-05-10T06:55:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11908#issuecomment-127740",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:18'>Comment 18:</a>
Green light !

Nathann



---

archive/issue_events_146822.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-05-10T06:55:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11908#event-146822"
}
```



---

archive/issue_events_146823.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-05-10T06:55:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11908#event-146823"
}
```



---

archive/issue_comments_127741.json:
```json
{
    "body": "Merged: **sage-5.10.beta3**",
    "created_at": "2013-05-13T13:26:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11908#issuecomment-127741",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.10.beta3**



---

archive/issue_events_146824.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-13T13:26:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11908#event-146824"
}
```



---

archive/issue_events_146825.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-13T13:26:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11908",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11908#event-146825"
}
```
