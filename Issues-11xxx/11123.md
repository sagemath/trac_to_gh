# Issue 11123: Plot connects parts of function over non-domain area

archive/issues_010984.json:
```json
{
    "assignees": [],
    "body": "From [this ask.sagemath](http://ask.sagemath.org/question/479/plot-sqrtsinx) question:\n\n```\n\n----------------------------------------------------------------------\n| Sage Version 4.6.2, Release Date: 2011-02-25                       |\n| Type notebook() for the GUI, and license() for information.        |\n----------------------------------------------------------------------\nsage: plot(sqrt(sin(x)),(0,10))\nverbose 0 (4069: plot.py, generate_plot_points) WARNING: When plotting, failed to evaluate function at 74 points.\nverbose 0 (4069: plot.py, generate_plot_points) Last error message: ''\n```\nThe picture is attached.  \n\nIdeally, we would want plot to just not connect those pieces.  With just sqrt(x) over -1 to 1, it's not a problem, but this really does look bad.\n\nAssignee: **@jasongrout, @williamstein**\n\nCC:  @jondo\n\nKeywords: **domain plot**\n\nReviewer: **Karl-Dieter Crisman, Robert Pollak**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11123_\n\n",
    "closed_at": "2013-10-05T09:37:05Z",
    "created_at": "2011-04-04T14:53:40Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20graphics",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Plot connects parts of function over non-domain area",
    "type": "issue",
    "updated_at": "2013-10-05T09:37:05Z",
    "url": "https://github.com/sagemath/sage/issues/11123",
    "user": "https://github.com/kcrisman"
}
```
From [this ask.sagemath](http://ask.sagemath.org/question/479/plot-sqrtsinx) question:

```

----------------------------------------------------------------------
| Sage Version 4.6.2, Release Date: 2011-02-25                       |
| Type notebook() for the GUI, and license() for information.        |
----------------------------------------------------------------------
sage: plot(sqrt(sin(x)),(0,10))
verbose 0 (4069: plot.py, generate_plot_points) WARNING: When plotting, failed to evaluate function at 74 points.
verbose 0 (4069: plot.py, generate_plot_points) Last error message: ''
```
The picture is attached.  

Ideally, we would want plot to just not connect those pieces.  With just sqrt(x) over -1 to 1, it's not a problem, but this really does look bad.

Assignee: **@jasongrout, @williamstein**

CC:  @jondo

Keywords: **domain plot**

Reviewer: **Karl-Dieter Crisman, Robert Pollak**

_Issue created by migration from https://trac.sagemath.org/ticket/11123_





---

archive/issue_events_134633.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-04-04T14:53:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11123",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graphics",
    "label_color": "0000ff",
    "label_name": "component: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11123#event-134633"
}
```



---

archive/issue_events_134634.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-04-04T14:53:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11123",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11123#event-134634"
}
```



---

archive/issue_events_134635.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-04-04T14:53:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11123",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11123#event-134635"
}
```



---

archive/issue_comments_112611.json:
```json
{
    "body": "Attachment: **[tmp_0.png](https://github.com/sagemath/sage/files/ticket11123/tmp_0.png)**\n\nShows the problem",
    "created_at": "2011-04-04T14:54:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11123",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11123#issuecomment-112611",
    "user": "https://github.com/kcrisman"
}
```

Attachment: **[tmp_0.png](https://github.com/sagemath/sage/files/ticket11123/tmp_0.png)**

Shows the problem



---

archive/issue_comments_112612.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nThis should be resolved as duplicate of #13246. This ticket is older, but the other one has discussion and a patch.\n\n(It seems I cannot do this by myself.)",
    "created_at": "2013-09-13T12:36:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11123",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11123#issuecomment-112612",
    "user": "https://github.com/jondo"
}
```

<div id="comment:1" align="right">Comment 1</div>

This should be resolved as duplicate of #13246. This ticket is older, but the other one has discussion and a patch.

(It seems I cannot do this by myself.)



---

archive/issue_comments_112613.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nAgreed.\n\n(It's true you can't do this by yourself, but you *can* change the status and then the release manager can close it.)",
    "created_at": "2013-09-13T14:25:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11123",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11123#issuecomment-112613",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:2" align="right">Comment 2</div>

Agreed.

(It's true you can't do this by yourself, but you *can* change the status and then the release manager can close it.)



---

archive/issue_events_134636.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-09-13T14:25:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11123",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11123#event-134636"
}
```



---

archive/issue_events_134637.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-09-13T14:25:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11123",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11123#event-134637"
}
```



---

archive/issue_events_134638.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-09-13T14:25:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11123",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11123#event-134638"
}
```



---

archive/issue_comments_112614.json:
```json
{
    "body": "Reviewer: **Karl-Dieter Crisman**",
    "created_at": "2013-09-13T14:30:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11123",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11123#issuecomment-112614",
    "user": "https://github.com/kcrisman"
}
```

Reviewer: **Karl-Dieter Crisman**



---

archive/issue_events_134639.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-09-13T14:30:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11123",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11123#event-134639"
}
```



---

archive/issue_events_134640.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-09-13T14:30:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11123",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11123#event-134640"
}
```



---

archive/issue_comments_112615.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nAnd now just add your real name to the list of reviewers (I couldn't find it easily on Trac).",
    "created_at": "2013-09-13T14:30:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11123",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11123#issuecomment-112615",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:3" align="right">Comment 3</div>

And now just add your real name to the list of reviewers (I couldn't find it easily on Trac).



---

archive/issue_comments_112616.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThank you.\n\n(Ah, as a newbie I had not noticed that this special milestone is used instead of having 'resolved' and 'closed' as separate steps.)",
    "created_at": "2013-09-13T19:56:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11123",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11123#issuecomment-112616",
    "user": "https://github.com/jondo"
}
```

<div id="comment:4" align="right">Comment 4</div>

Thank you.

(Ah, as a newbie I had not noticed that this special milestone is used instead of having 'resolved' and 'closed' as separate steps.)



---

archive/issue_comments_112617.json:
```json
{
    "body": "Changed reviewer from **Karl-Dieter Crisman** to **Karl-Dieter Crisman, Robert Pollak**",
    "created_at": "2013-09-13T19:56:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11123",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11123#issuecomment-112617",
    "user": "https://github.com/jondo"
}
```

Changed reviewer from **Karl-Dieter Crisman** to **Karl-Dieter Crisman, Robert Pollak**



---

archive/issue_events_134641.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-05T09:37:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11123",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11123#event-134641"
}
```



---

archive/issue_events_134642.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-05T09:37:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11123",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11123#event-134642"
}
```



---

archive/issue_events_134643.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-05T09:37:05Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11123",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11123#event-134643"
}
```
