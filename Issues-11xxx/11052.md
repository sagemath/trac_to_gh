# Issue 11052: Bad shell logic in root-spkg-install

archive/issues_010944.json:
```json
{
    "assignees": [],
    "body": "The following piece of `root-spkg-install` doesn't work as intented, because the `$?` refers to the exit status of the first `if` instead of `hg incoming`:\n\n```\n    # Merge the repository, rather than overwrite changes that the\n    # user may have made.\n    hg incoming \"$CUR\" 1> /dev/null\n    if [ $? -eq 1 ]; then\n        # No changes to pull\n        exit 0\n    fi\n    if [ $? -ne 0 ]; then\n        echo \"Error with Sage root repository: 'hg incoming' failed.\"\n        exit 1\n    fi\n```\n\nCC:  @vbraun\n\nKeywords: **sage_root**\n\nAuthor: **Jeroen Demeyer**\n\nReviewer: **Volker Braun**\n\nMerged: **sage-4.7.alpha3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11052_\n\n",
    "closed_at": "2011-03-28T19:26:07Z",
    "created_at": "2011-03-26T19:52:33Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20scripts",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Bad shell logic in root-spkg-install",
    "type": "issue",
    "updated_at": "2011-03-28T19:26:07Z",
    "url": "https://github.com/sagemath/sage/issues/11052",
    "user": "https://github.com/jdemeyer"
}
```
The following piece of `root-spkg-install` doesn't work as intented, because the `$?` refers to the exit status of the first `if` instead of `hg incoming`:

```
    # Merge the repository, rather than overwrite changes that the
    # user may have made.
    hg incoming "$CUR" 1> /dev/null
    if [ $? -eq 1 ]; then
        # No changes to pull
        exit 0
    fi
    if [ $? -ne 0 ]; then
        echo "Error with Sage root repository: 'hg incoming' failed."
        exit 1
    fi
```

CC:  @vbraun

Keywords: **sage_root**

Author: **Jeroen Demeyer**

Reviewer: **Volker Braun**

Merged: **sage-4.7.alpha3**

_Issue created by migration from https://trac.sagemath.org/ticket/11052_





---

archive/issue_events_133805.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-26T19:52:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11052",
    "milestone_number": null,
    "milestone_title": "sage-4.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11052#event-133805"
}
```



---

archive/issue_events_133806.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-26T19:52:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11052",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20scripts",
    "label_color": "000080",
    "label_name": "component: scripts",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11052#event-133806"
}
```



---

archive/issue_events_133807.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-26T19:52:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11052",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11052#event-133807"
}
```



---

archive/issue_events_133808.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-26T19:52:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11052",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11052#event-133808"
}
```



---

archive/issue_comments_111057.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[11052.patch.gz](https://github.com/sagemath/sage/files/ticket11052/11052.patch.gz)**",
    "created_at": "2011-03-26T19:55:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11052",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11052#issuecomment-111057",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[11052.patch.gz](https://github.com/sagemath/sage/files/ticket11052/11052.patch.gz)**



---

archive/issue_events_133809.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-26T19:55:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11052",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11052#event-133809"
}
```



---

archive/issue_events_133810.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2011-03-28T17:25:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11052",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11052#event-133810"
}
```



---

archive/issue_events_133811.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2011-03-28T17:25:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11052",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11052#event-133811"
}
```



---

archive/issue_comments_111058.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nGood catch!",
    "created_at": "2011-03-28T17:25:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11052",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11052#issuecomment-111058",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:2'>Comment 2:</a>
Good catch!



---

archive/issue_comments_111059.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2011-03-28T19:26:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11052",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11052#issuecomment-111059",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Volker Braun**



---

archive/issue_comments_111060.json:
```json
{
    "body": "Merged: **sage-4.7.alpha3**",
    "created_at": "2011-03-28T19:26:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11052",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11052#issuecomment-111060",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.7.alpha3**



---

archive/issue_events_133812.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-28T19:26:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11052",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11052#event-133812"
}
```



---

archive/issue_events_133813.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-28T19:26:07Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11052",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11052#event-133813"
}
```



---

archive/issue_comments_111061.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThanks Volker.",
    "created_at": "2011-03-28T19:26:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11052",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11052#issuecomment-111061",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:3'>Comment 3:</a>
Thanks Volker.
