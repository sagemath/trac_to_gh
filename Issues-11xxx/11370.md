# Issue 11370: permutation.to_standard() breaks on empty permutations

archive/issues_011198.json:
```json
{
    "assignees": [],
    "body": "Yet another empty object problem\n\n```\nsage: sage.combinat.permutation.to_standard([])\n---------------------------------------------------------------------------\nValueError                                Traceback (most recent call last)\n\n/home/data/Sage-Install/sage-4.6.2/devel/sage-combinat/sage/combinat/<ipython console> in <module>()\n\n/home/florent/src/Sage/sage/local/lib/python2.6/site-packages/sage/combinat/permutation.pyc in to_standard(p)\n   4243 \n   4244     s = p[:]\n-> 4245     biggest = max(p) + 1\n   4246     i = 1\n   4247     for _ in range(len(p)):\n\nValueError: max() arg is an empty sequence\n```\nshould be \n\n```\n[]\n```\n\nI also fixed the return type of {{{from_reduced_word([])}}.\n\nAssignee: **@sagetrac-sage-combinat**\n\nCC:  @sagetrac-sage-combinat\n\nKeywords: **Standard permutation**\n\nReviewer: **Mike Hansen**\n\nAuthor: **Florent Hivert**\n\nMerged: **sage-4.7.1.alpha3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11370_\n\n",
    "closed_at": "2011-06-08T07:14:18Z",
    "created_at": "2011-05-23T12:44:34Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "permutation.to_standard() breaks on empty permutations",
    "type": "issue",
    "updated_at": "2023-01-14T16:14:43Z",
    "url": "https://github.com/sagemath/sage/issues/11370",
    "user": "https://github.com/hivert"
}
```
Yet another empty object problem

```
sage: sage.combinat.permutation.to_standard([])
---------------------------------------------------------------------------
ValueError                                Traceback (most recent call last)

/home/data/Sage-Install/sage-4.6.2/devel/sage-combinat/sage/combinat/<ipython console> in <module>()

/home/florent/src/Sage/sage/local/lib/python2.6/site-packages/sage/combinat/permutation.pyc in to_standard(p)
   4243 
   4244     s = p[:]
-> 4245     biggest = max(p) + 1
   4246     i = 1
   4247     for _ in range(len(p)):

ValueError: max() arg is an empty sequence
```
should be 

```
[]
```

I also fixed the return type of {{{from_reduced_word([])}}.

Assignee: **@sagetrac-sage-combinat**

CC:  @sagetrac-sage-combinat

Keywords: **Standard permutation**

Reviewer: **Mike Hansen**

Author: **Florent Hivert**

Merged: **sage-4.7.1.alpha3**

_Issue created by migration from https://trac.sagemath.org/ticket/11370_





---

archive/issue_events_138039.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2011-05-23T12:44:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11370",
    "milestone_number": null,
    "milestone_title": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11370#event-138039"
}
```



---

archive/issue_events_138040.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2011-05-23T12:44:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11370",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11370#event-138040"
}
```



---

archive/issue_events_138041.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2011-05-23T12:44:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11370",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11370#event-138041"
}
```



---

archive/issue_events_138042.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2011-05-23T12:44:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11370",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11370#event-138042"
}
```



---

archive/issue_comments_116933.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -21,3 +21,5 @@\n ```\n []\n ```\n+\n+I also fixed the return type of {{{from_reduced_word([])}}.\n``````\n",
    "created_at": "2011-05-23T12:49:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11370",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11370#issuecomment-116933",
    "user": "https://github.com/hivert"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -21,3 +21,5 @@
 ```
 []
 ```
+
+I also fixed the return type of {{{from_reduced_word([])}}.
``````




---

archive/issue_comments_116934.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nShould be ready for review.",
    "created_at": "2011-05-23T12:58:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11370",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11370#issuecomment-116934",
    "user": "https://github.com/hivert"
}
```

<div id="comment:2" align="right">Comment 2</div>

Should be ready for review.



---

archive/issue_comments_116935.json:
```json
{
    "body": "Attachment: **[trac_11370-perm_empty_standard_fix-fh.patch.gz](https://github.com/sagemath/sage/files/ticket11370/trac_11370-perm_empty_standard_fix-fh.patch.gz)**",
    "created_at": "2011-05-23T12:59:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11370",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11370#issuecomment-116935",
    "user": "https://github.com/hivert"
}
```

Attachment: **[trac_11370-perm_empty_standard_fix-fh.patch.gz](https://github.com/sagemath/sage/files/ticket11370/trac_11370-perm_empty_standard_fix-fh.patch.gz)**



---

archive/issue_comments_116936.json:
```json
{
    "body": "Reviewer: **Mike Hansen**",
    "created_at": "2011-05-23T20:33:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11370",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11370#issuecomment-116936",
    "user": "https://github.com/mwhansen"
}
```

Reviewer: **Mike Hansen**



---

archive/issue_events_138043.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2011-05-23T20:33:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11370",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11370#event-138043"
}
```



---

archive/issue_comments_116937.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLooks good to me.",
    "created_at": "2011-05-23T20:33:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11370",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11370#issuecomment-116937",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:3" align="right">Comment 3</div>

Looks good to me.



---

archive/issue_events_138044.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2011-05-23T20:33:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11370",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11370#event-138044"
}
```



---

archive/issue_events_138045.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2011-05-23T20:33:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11370",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11370#event-138045"
}
```



---

archive/issue_comments_116938.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThanks Mike.",
    "created_at": "2011-05-23T21:15:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11370",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11370#issuecomment-116938",
    "user": "https://github.com/hivert"
}
```

<div id="comment:5" align="right">Comment 5</div>

Thanks Mike.



---

archive/issue_comments_116939.json:
```json
{
    "body": "Merged: **sage-4.7.1.alpha3**",
    "created_at": "2011-06-08T07:14:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11370",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11370#issuecomment-116939",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.7.1.alpha3**



---

archive/issue_events_138046.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-08T07:14:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11370",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11370#event-138046"
}
```



---

archive/issue_events_138047.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-08T07:14:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11370",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11370#event-138047"
}
```
