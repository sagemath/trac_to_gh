# Issue 11390: NTL error while creating finite field homomorphism

archive/issues_011218.json:
```json
{
    "assignees": [],
    "body": "On my computer I get an unhandled NTL error by just doing:\n\n```\nsage: K = GF(1<<16,'a'); L = GF(1<<32,'b')\nsage: K.Hom(L)[0]\n\n```\nSee http://groups.google.com/group/sage-support/browse_thread/thread/b9714926d491e905 for further description and potential fix.\n\nI'm setting it as blocker as Simon suggested, feel free to change.\n\nThere seem to be some initialization problem with NTL and GF2X.\n\nAssignee: **@aghitza**\n\nCC:  @simon-king-jena jpflori @malb\n\nAuthor: **Jean-Pierre Flori**\n\nReviewer: **Simon King, Martin Albrecht**\n\nMerged: **sage-4.7.1.alpha2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11390_\n\n",
    "closed_at": "2011-06-01T07:15:35Z",
    "created_at": "2011-05-27T13:52:46Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "NTL error while creating finite field homomorphism",
    "type": "issue",
    "updated_at": "2011-06-01T07:15:35Z",
    "url": "https://github.com/sagemath/sage/issues/11390",
    "user": "https://github.com/sagetrac-jpflori"
}
```
On my computer I get an unhandled NTL error by just doing:

```
sage: K = GF(1<<16,'a'); L = GF(1<<32,'b')
sage: K.Hom(L)[0]

```
See http://groups.google.com/group/sage-support/browse_thread/thread/b9714926d491e905 for further description and potential fix.

I'm setting it as blocker as Simon suggested, feel free to change.

There seem to be some initialization problem with NTL and GF2X.

Assignee: **@aghitza**

CC:  @simon-king-jena jpflori @malb

Author: **Jean-Pierre Flori**

Reviewer: **Simon King, Martin Albrecht**

Merged: **sage-4.7.1.alpha2**

_Issue created by migration from https://trac.sagemath.org/ticket/11390_





---

archive/issue_events_138326.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2011-05-27T13:52:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11390",
    "milestone_number": null,
    "milestone_title": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11390#event-138326"
}
```



---

archive/issue_events_138327.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2011-05-27T13:52:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11390",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11390#event-138327"
}
```



---

archive/issue_events_138328.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2011-05-27T13:52:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11390",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11390#event-138328"
}
```



---

archive/issue_events_138329.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2011-05-27T13:52:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11390",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11390#event-138329"
}
```



---

archive/issue_comments_117302.json:
```json
{
    "body": "Potential fix",
    "created_at": "2011-05-27T13:54:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11390#issuecomment-117302",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Potential fix



---

archive/issue_comments_117303.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[trac_11390-fix.patch.gz](https://github.com/sagemath/sage/files/ticket11390/trac_11390-fix.patch.gz)**\n\nI'll provide another patch for doctesting the problem in a few moments.",
    "created_at": "2011-05-27T13:55:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11390#issuecomment-117303",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[trac_11390-fix.patch.gz](https://github.com/sagemath/sage/files/ticket11390/trac_11390-fix.patch.gz)**

I'll provide another patch for doctesting the problem in a few moments.



---

archive/issue_comments_117304.json:
```json
{
    "body": "Doctest",
    "created_at": "2011-05-27T15:01:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11390#issuecomment-117304",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Doctest



---

archive/issue_events_138330.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2011-05-27T15:02:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11390",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11390#event-138330"
}
```



---

archive/issue_comments_117305.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nAttachment: **[trac_11390-doctest.patch.gz](https://github.com/sagemath/sage/files/ticket11390/trac_11390-doctest.patch.gz)**",
    "created_at": "2011-05-27T15:02:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11390#issuecomment-117305",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<a id='comment:2'>Comment 2:</a>
Attachment: **[trac_11390-doctest.patch.gz](https://github.com/sagemath/sage/files/ticket11390/trac_11390-doctest.patch.gz)**



---

archive/issue_comments_117306.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nIt seems to me that the problem is solved. However, if the original author (Martin Albrecht, I just cc'ed him) has time to look at you patch, it might be better to know his opinion first.",
    "created_at": "2011-05-28T07:19:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11390#issuecomment-117306",
    "user": "https://github.com/simon-king-jena"
}
```

<a id='comment:3'>Comment 3:</a>
It seems to me that the problem is solved. However, if the original author (Martin Albrecht, I just cc'ed him) has time to look at you patch, it might be better to know his opinion first.



---

archive/issue_comments_117307.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nPatch looks good.",
    "created_at": "2011-05-28T08:42:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11390#issuecomment-117307",
    "user": "https://github.com/malb"
}
```

<a id='comment:4'>Comment 4:</a>
Patch looks good.



---

archive/issue_comments_117308.json:
```json
{
    "body": "Reviewer: **Simon King, Martin Albrecht**",
    "created_at": "2011-05-28T08:42:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11390#issuecomment-117308",
    "user": "https://github.com/malb"
}
```

Reviewer: **Simon King, Martin Albrecht**



---

archive/issue_events_138331.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2011-05-28T08:42:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11390",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11390#event-138331"
}
```



---

archive/issue_events_138332.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2011-05-28T08:42:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11390",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11390#event-138332"
}
```



---

archive/issue_events_138333.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-31T09:41:06Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/11390",
    "title_is": "NTL error while creating finite field homomorphism",
    "title_was": "Segfault while creating finite field homomorphism",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11390#event-138333"
}
```



---

archive/issue_comments_117309.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-On my computer I get a segfault just doing:\n+On my computer I get an unhandled NTL error by just doing:\n \n ```\n sage: K = GF(1<<16,'a'); L = GF(1<<32,'b')\n``````\n",
    "created_at": "2011-05-31T09:41:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11390#issuecomment-117309",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-On my computer I get a segfault just doing:
+On my computer I get an unhandled NTL error by just doing:
 
 ```
 sage: K = GF(1<<16,'a'); L = GF(1<<32,'b')
``````




---

archive/issue_comments_117310.json:
```json
{
    "body": "Merged: **sage-4.7.1.alpha2**",
    "created_at": "2011-06-01T07:15:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11390#issuecomment-117310",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.7.1.alpha2**



---

archive/issue_events_138334.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-01T07:15:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11390",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11390#event-138334"
}
```



---

archive/issue_events_138335.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-01T07:15:35Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11390",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11390#event-138335"
}
```
