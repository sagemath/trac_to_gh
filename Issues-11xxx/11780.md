...
```json
{
    "body": "<a id='comment:1'></a>I suggest to call the polynomial ring constructor (which might actually be a little faster, since it is cached) and to catch the `TypeError` that would occur if the resulting ring happens to not be `MPolynomialRing_libsingular`.",
...
<a id='comment:1'></a>I suggest to call the polynomial ring constructor (which might actually be a little faster, since it is cached) and to catch the `TypeError` that would occur if the resulting ring happens to not be `MPolynomialRing_libsingular`.
...
<a id='comment:2'></a>I did not run tests yet, and I am not able to provide a doc test that shows that the bug is fixed. But it turns out that the patch that I just attached solves the problem I met while working at #10667, and thus I change it into "needs review".
...
```json
{
    "body": "<a id='comment:3'></a>Why would `PolynomialRing` throw a `TypeError` and `MPolynomialRing_libsingular` would accept these parameters?",
...
<a id='comment:3'></a>Why would `PolynomialRing` throw a `TypeError` and `MPolynomialRing_libsingular` would accept these parameters?
...
```json
{
    "body": "<a id='comment:5'></a>Ah, got it. Thanks, that makes sense. The patch reads good, I haven't applied & tested it though.",
...
<a id='comment:5'></a>Ah, got it. Thanks, that makes sense. The patch reads good, I haven't applied & tested it though.
...
<a id='comment:6'></a>Doctests pass. However, I gave the TypeError thing another thought. If somebody changes the class for polynomials over the base field (prime field or rational field), couldn't that again lead to problems with non-identical parents? I.e. should we cause the TypeError such that future developers will note there's a potential problem?
...
<a id='comment:7'></a>I fixed another bug caused by a rogue creation of a polynomial ring: It was in the numerator() method of libsingular polynomials.
...
<a id='comment:8'></a>Replying to [malb](#comment%3A6):
> Doctests pass. However, I gave the TypeError thing another thought. If somebody changes the class for polynomials over the base field (prime field or rational field), couldn't that again lead to problems with non-identical parents? I.e. should we cause the TypeError such that future developers will note there's a potential problem?
...
```json
{
    "body": "<a id='comment:9'></a>Also, small typo at the end of this line:\n\n```\n # the integers using the (cached) polynomial ring constructor:@@@ \n```\n\n:)",
...
<a id='comment:9'></a>Also, small typo at the end of this line:
...
<a id='comment:10'></a>I removed the typo (actually it is not a typo but a mark, in order to more easily find the spot that I was working on).
...
```json
{
    "body": "<a id='comment:11'></a>The patchbot does not find actual errors (even though the blob is yellow). Review, anyone?",
...
<a id='comment:11'></a>The patchbot does not find actual errors (even though the blob is yellow). Review, anyone?
...
```json
{
    "body": "<a id='comment:12'></a>Martin, do you think you find the time to complete your review?",
...
<a id='comment:12'></a>Martin, do you think you find the time to complete your review?
...
```json
{
    "body": "<a id='comment:13'></a>I'll have to rebase, because #11339 (which is merged) creates a conflict.",
    "created_at": "2011-10-28T13:45:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11780",
    "type": "issue_comment",
...
<a id='comment:13'></a>I'll have to rebase, because #11339 (which is merged) creates a conflict.
...
```json
{
    "body": "<a id='comment:16'></a>This looks fine to me, and all tests pass under 4.8.alpha5.",
...
<a id='comment:16'></a>This looks fine to me, and all tests pass under 4.8.alpha5.
