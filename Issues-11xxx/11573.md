# Issue 11573: ELGamal Algorithm

archive/issues_011401.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mvngu"
    ],
    "body": "In cryptography, the ElGamal encryption system is an asymmetric key encryption algorithm for public-key cryptography which is based on the Diffie\u2013Hellman key exchange. It was described by Taher Elgamal in 1984. ElGamal encryption is used in the free GNU Privacy Guard software, recent versions of PGP, and other cryptosystems.\n\nCC:  @sagetrac-Minh @sagetrac-Nguyen\n\nKeywords: **Elgamal**\n\nWork Issues: **missing docstrings**\n\nReviewer: **Julian Rueth**\n\nAuthor: **Sam Scott**\n\nComponent: **cryptography**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11573_\n\n",
    "created_at": "2011-07-04T14:56:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20cryptography",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/needs%20work"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "ELGamal Algorithm",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/11573",
    "user": "https://github.com/sagetrac-charlsmathew"
}
```
In cryptography, the ElGamal encryption system is an asymmetric key encryption algorithm for public-key cryptography which is based on the Diffie–Hellman key exchange. It was described by Taher Elgamal in 1984. ElGamal encryption is used in the free GNU Privacy Guard software, recent versions of PGP, and other cryptosystems.

CC:  @sagetrac-Minh @sagetrac-Nguyen

Keywords: **Elgamal**

Work Issues: **missing docstrings**

Reviewer: **Julian Rueth**

Author: **Sam Scott**

Component: **cryptography**

_Issue created by migration from https://trac.sagemath.org/ticket/11573_





---

archive/issue_events_154774.json:
```json
{
    "actor": "https://github.com/sagetrac-charlsmathew",
    "created_at": "2011-07-04T14:56:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11573",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11573#event-154774"
}
```



---

archive/issue_events_154775.json:
```json
{
    "actor": "https://github.com/sagetrac-charlsmathew",
    "created_at": "2011-07-04T14:56:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11573",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11573#event-154775"
}
```



---

archive/issue_comments_118350.json:
```json
{
    "body": "Changed author from **Charls Mathew** to none",
    "created_at": "2012-04-06T07:39:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11573",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11573#issuecomment-118350",
    "user": "https://github.com/loefflerd"
}
```

Changed author from **Charls Mathew** to none



---

archive/issue_events_154776.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2012-04-06T07:39:36Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/11573",
    "subject": "https://github.com/loefflerd",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11573#event-154776"
}
```



---

archive/issue_events_154777.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-04-06T07:39:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11573",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20cryptography",
    "label_color": "0000ff",
    "label_name": "c: cryptography",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11573#event-154777"
}
```



---

archive/issue_events_154778.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-04-06T07:39:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11573",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11573#event-154778"
}
```



---

archive/issue_comments_118351.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nI've started working on a basic implementation of this based on \"Handbook of Applied Cryptography\" - Menezes et al.\n\nI'm intending to implement the generic version or ElGamal over a group G, where the default implementation will let G = F_p where p is a prime with approximately 128 bits of security.",
    "created_at": "2012-12-17T22:31:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11573",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11573#issuecomment-118351",
    "user": "https://github.com/sagetrac-scotts"
}
```

<div id="comment:2" align="right">comment:2</div>

I've started working on a basic implementation of this based on "Handbook of Applied Cryptography" - Menezes et al.

I'm intending to implement the generic version or ElGamal over a group G, where the default implementation will let G = F_p where p is a prime with approximately 128 bits of security.



---

archive/issue_comments_118352.json:
```json
{
    "body": "Attachment: **[17437.patch.gz](https://github.com/sagemath/sage/files/ticket11573/17437.patch.gz)**\n\nImplementation of the ElGamal algorithm",
    "created_at": "2013-01-12T12:48:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11573",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11573#issuecomment-118352",
    "user": "https://github.com/sagetrac-scotts"
}
```

Attachment: **[17437.patch.gz](https://github.com/sagemath/sage/files/ticket11573/17437.patch.gz)**

Implementation of the ElGamal algorithm



---

archive/issue_comments_118353.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nSo I attached my implementation of the ElGamal algorithm. I would appreciate it if someone would have a look through it. Hopefully there wont be any issues.\n\nThanks, \n\nSam",
    "created_at": "2013-01-12T12:49:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11573",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11573#issuecomment-118353",
    "user": "https://github.com/sagetrac-scotts"
}
```

<div id="comment:3" align="right">comment:3</div>

So I attached my implementation of the ElGamal algorithm. I would appreciate it if someone would have a look through it. Hopefully there wont be any issues.

Thanks, 

Sam



---

archive/issue_events_154779.json:
```json
{
    "actor": "https://github.com/sagetrac-scotts",
    "created_at": "2013-01-12T12:49:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11573",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11573#event-154779"
}
```



---

archive/issue_comments_118354.json:
```json
{
    "body": "Author: **Sam Scott**",
    "created_at": "2013-03-04T18:52:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11573",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11573#issuecomment-118354",
    "user": "https://github.com/saraedum"
}
```

Author: **Sam Scott**



---

archive/issue_comments_118355.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nMany of the methods lack doctests. Most of the methods that are not doctested are quite trivial, still they should have doctets.",
    "created_at": "2013-03-04T18:57:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11573",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11573#issuecomment-118355",
    "user": "https://github.com/saraedum"
}
```

<div id="comment:5" align="right">comment:5</div>

Many of the methods lack doctests. Most of the methods that are not doctested are quite trivial, still they should have doctets.



---

archive/issue_events_154780.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2013-03-04T18:57:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11573",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11573#event-154780"
}
```



---

archive/issue_events_154781.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2013-03-04T18:57:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11573",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11573#event-154781"
}
```



---

archive/issue_comments_118356.json:
```json
{
    "body": "Reviewer: **Julian Rueth**",
    "created_at": "2013-03-04T18:57:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11573",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11573#issuecomment-118356",
    "user": "https://github.com/saraedum"
}
```

Reviewer: **Julian Rueth**



---

archive/issue_events_154782.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11573",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11573#event-154782"
}
```



---

archive/issue_events_154783.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11573",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11573#event-154783"
}
```



---

archive/issue_events_154784.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11573",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11573#event-154784"
}
```



---

archive/issue_events_154785.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11573",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11573#event-154785"
}
```



---

archive/issue_events_154786.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11573",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11573#event-154786"
}
```



---

archive/issue_events_154787.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11573",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11573#event-154787"
}
```



---

archive/issue_events_154788.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11573",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11573#event-154788"
}
```



---

archive/issue_events_154789.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11573",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11573#event-154789"
}
```



---

archive/issue_comments_118357.json:
```json
{
    "body": "Work Issues: **missing docstrings**",
    "created_at": "2017-06-05T23:33:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11573",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11573#issuecomment-118357",
    "user": "https://github.com/saraedum"
}
```

Work Issues: **missing docstrings**



---

archive/issue_events_154790.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11573",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11573#event-154790"
}
```
