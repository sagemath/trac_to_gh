...
<a id='comment:1'></a>Attaching `config.log` in this case wouldn't be bad.
...
<a id='comment:2'></a>Yes, I tried installing a newer version.  It did not go well, and
I have abandoned the effort for now.  (Other things are demanding my attention.)  Perhaps someone else will have better luck!
...
```json
{
    "body": "<a id='comment:5'></a>For the record: Macaulay2 1.5 is about to be released.",
...
<a id='comment:5'></a>For the record: Macaulay2 1.5 is about to be released.
...
<a id='comment:6'></a>To review #12335 I need to install the spkg `macaulay2-1.1-r7221.p0` into sage-5.0 but when I try I get:
...
<a id='comment:7'></a>Replying to [benjaminfjones](#comment%3A6):
> To review #12335 I need to install the spkg `macaulay2-1.1-r7221.p0` into sage-5.0 but when I try I get:
> 
> 
> ```
...
<a id='comment:8'></a>I am trying to build M2 1.4 from source - it would be way better to upgrade spkg. My diary:
* I've installed autoconf on my system since it does not seem like Sage includes it in any way.
* bison and flex are available as experimental packages (why not optional?) and installed fine for me.
* I have MAKE="make -j2" and configure complains that make is not GNU, changing it to just "make" works fine.
* Next it complains about libcurses and libncurses. I have libncurses installed, but it is necessary to install development versions. I've done that.
...
<a id='comment:9'></a>I am not sure where fat source is, but I have downloaded files one by one until there were no complaints from here:
http://www.math.uiuc.edu/Macaulay2/Downloads/OtherSourceCode/1.4/index.html
I had to get 21 out of 29 currently posted there:
...
<a id='comment:10'></a>The end of make's output:
...
<a id='comment:11'></a>I made a new spkg based on Macaulay2-1.4, so far it fails to build because of 4ti2 and gfan. Both are installed in Sage (4ti2 as optional), but not detected by M2 - I've asked M2 developers what can be done. Including these into M2 package breaks during gfan installation, so I have no idea yet how close I am to a working package.
...
<a id='comment:12'></a>Here is an spkg that tries to build (building 4ti2 and gfan is prohibited):
...
```json
{
    "body": "<a id='comment:13'></a>The trunk version has succeeded, although my system now has autoconf, lib(n)curses and libxml2.",
...
<a id='comment:13'></a>The trunk version has succeeded, although my system now has autoconf, lib(n)curses and libxml2.
...
<a id='comment:15'></a>Sure, here is a link:
...
```json
{
    "body": "<a id='comment:17'></a>I think there is an option to build without xml, but I didn't try it yet. In any case since I was not able to build 1.4 due to errors during compilation of M2 itself, I'd like to wait till 1.5 is officially released, hopefully with proper packages for Normaliz etc. Then we can see how to cut down on dependencies.",
...
<a id='comment:17'></a>I think there is an option to build without xml, but I didn't try it yet. In any case since I was not able to build 1.4 due to errors during compilation of M2 itself, I'd like to wait till 1.5 is officially released, hopefully with proper packages for Normaliz etc. Then we can see how to cut down on dependencies.
...
```json
{
    "body": "<a id='comment:18'></a>I encountered a complaint about flex, too. Trying another rebuild...",
...
<a id='comment:18'></a>I encountered a complaint about flex, too. Trying another rebuild...
...
```json
{
    "body": "<a id='comment:19'></a>Interesting - so this complain was late in the build process, M2 didn't detect its absence? I've updated 1.5 spkg to have checks for gdbm, 4ti2, bison, and flex spkgs. The rest of dependencies has to be dropped somehow.",
...
<a id='comment:19'></a>Interesting - so this complain was late in the build process, M2 didn't detect its absence? I've updated 1.5 spkg to have checks for gdbm, 4ti2, bison, and flex spkgs. The rest of dependencies has to be dropped somehow.
...
<a id='comment:20'></a>As far as I know, M2 didn't detect its absence, no. It does detect some other things.
...
<a id='comment:21'></a>You can try to run "sage -M2" to get M2 prompt and see if it works (worked for me showing version 1.4.0.1). Also I see it in "sage -i" - the list of installed packages. Maybe to see it in optional ones it has to be registered somehow.
...
<a id='comment:22'></a>Replying to [novoselt](#comment%3A21):
> You can try to run "sage -M2" to get M2 prompt and see if it works (worked for me showing version 1.4.0.1).
...
<a id='comment:23'></a>One automatic thing to try is (in Sage root)
...
<a id='comment:31'></a>(Not sure what Authors and Reviewers means here, as there doesn't seem to be any code / patch / branch.)
...
```json
{
    "body": "<a id='comment:33'></a>I think this can be closed as won't fix. M2 needs many patched, for Boehm GC compatibility, packages (e.g. Flint, NTL) , and it's very bulky this way.",
    "created_at": "2017-09-13T15:41:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11710",
    "type": "issue_comment",
...
<a id='comment:33'></a>I think this can be closed as won't fix. M2 needs many patched, for Boehm GC compatibility, packages (e.g. Flint, NTL) , and it's very bulky this way.
...
```json
{
    "body": "<a id='comment:35'></a>I agree, and furthermore it is not even listed under the experimental packages anymore.",
...
<a id='comment:35'></a>I agree, and furthermore it is not even listed under the experimental packages anymore.
