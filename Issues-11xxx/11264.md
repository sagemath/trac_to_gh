# Issue 11264: sage/combinat/iet/strata.py is the last place in sage to use the deprecated type.__cmp__ operator, easy fix.

archive/issues_011092.json:
```json
{
    "assignees": [],
    "body": "type.__cmp__ has been removed from python-2.6.5 and later and is only used in one place in sage. It can be safely replaced by cmp. This will be needed to upgrade python.\n\nApply:\n* [attachment: trac_11264-replace_typecmp_in_strata.patch](https://github.com/sagemath/sage/files/ticket11264/trac_11264-replace_typecmp_in_strata.patch.gz)\n\nAssignee: **@sagetrac-sage-combinat**\n\nAuthor: **Fran\u00e7ois Bissey**\n\nReviewer: **Nicolas M. Thi\u00e9ry**\n\nMerged: **sage-4.7.1.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11264_\n\n",
    "closed_at": "2011-05-01T10:56:55Z",
    "created_at": "2011-04-28T03:39:52Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage/combinat/iet/strata.py is the last place in sage to use the deprecated type.__cmp__ operator, easy fix.",
    "type": "issue",
    "updated_at": "2011-05-01T10:56:55Z",
    "url": "https://github.com/sagemath/sage/issues/11264",
    "user": "https://github.com/kiwifb"
}
```
type.__cmp__ has been removed from python-2.6.5 and later and is only used in one place in sage. It can be safely replaced by cmp. This will be needed to upgrade python.

Apply:
* [attachment: trac_11264-replace_typecmp_in_strata.patch](https://github.com/sagemath/sage/files/ticket11264/trac_11264-replace_typecmp_in_strata.patch.gz)

Assignee: **@sagetrac-sage-combinat**

Author: **François Bissey**

Reviewer: **Nicolas M. Thiéry**

Merged: **sage-4.7.1.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/11264_





---

archive/issue_events_136313.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2011-04-28T03:39:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11264",
    "milestone_number": null,
    "milestone_title": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11264#event-136313"
}
```



---

archive/issue_events_136314.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2011-04-28T03:39:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11264",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11264#event-136314"
}
```



---

archive/issue_events_136315.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2011-04-28T03:39:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11264",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11264#event-136315"
}
```



---

archive/issue_events_136316.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2011-04-28T03:39:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11264",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11264#event-136316"
}
```



---

archive/issue_comments_115010.json:
```json
{
    "body": "Attachment: **[trac_11264-replace_typecmp_in_strata.patch.gz](https://github.com/sagemath/sage/files/ticket11264/trac_11264-replace_typecmp_in_strata.patch.gz)**\n\nreplace type.__cmp__ by cmp in strata.py",
    "created_at": "2011-04-28T03:44:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11264",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11264#issuecomment-115010",
    "user": "https://github.com/kiwifb"
}
```

Attachment: **[trac_11264-replace_typecmp_in_strata.patch.gz](https://github.com/sagemath/sage/files/ticket11264/trac_11264-replace_typecmp_in_strata.patch.gz)**

replace type.__cmp__ by cmp in strata.py



---

archive/issue_events_136317.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2011-04-28T03:46:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11264",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11264#event-136317"
}
```



---

archive/issue_comments_115011.json:
```json
{
    "body": "Author: **Fran\u00e7ois Bissey**",
    "created_at": "2011-04-28T03:46:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11264",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11264#issuecomment-115011",
    "user": "https://github.com/kiwifb"
}
```

Author: **François Bissey**



---

archive/issue_comments_115012.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,4 @@\n type.__cmp__ has been removed from python-2.6.5 and later and is only used in one place in sage. It can be safely replaced by cmp. This will be needed to upgrade python.\n+\n+Apply:\n+* [attachment: trac_11264-replace_typecmp_in_strata.patch](https://github.com/sagemath/sage/files/ticket11264/trac_11264-replace_typecmp_in_strata.patch.gz)\n``````\n",
    "created_at": "2011-04-28T03:46:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11264",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11264#issuecomment-115012",
    "user": "https://github.com/kiwifb"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,4 @@
 type.__cmp__ has been removed from python-2.6.5 and later and is only used in one place in sage. It can be safely replaced by cmp. This will be needed to upgrade python.
+
+Apply:
+* [attachment: trac_11264-replace_typecmp_in_strata.patch](https://github.com/sagemath/sage/files/ticket11264/trac_11264-replace_typecmp_in_strata.patch.gz)
``````




---

archive/issue_events_136318.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2011-04-28T05:15:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11264",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11264#event-136318"
}
```



---

archive/issue_events_136319.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2011-04-28T05:15:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11264",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11264#event-136319"
}
```



---

archive/issue_comments_115013.json:
```json
{
    "body": "Reviewer: **Nicolas M. Thi\u00e9ry**",
    "created_at": "2011-04-28T05:15:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11264",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11264#issuecomment-115013",
    "user": "https://github.com/nthiery"
}
```

Reviewer: **Nicolas M. Thiéry**



---

archive/issue_comments_115014.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nMerci.",
    "created_at": "2011-04-28T09:30:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11264",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11264#issuecomment-115014",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:3" align="right">Comment 3</div>

Merci.



---

archive/issue_comments_115015.json:
```json
{
    "body": "Merged: **sage-4.7.1.alpha0**",
    "created_at": "2011-05-01T10:56:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11264",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11264#issuecomment-115015",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.7.1.alpha0**



---

archive/issue_events_136320.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-01T10:56:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11264",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11264#event-136320"
}
```



---

archive/issue_events_136321.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-01T10:56:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11264",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11264#event-136321"
}
```
