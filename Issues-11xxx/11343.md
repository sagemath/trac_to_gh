...
```json
{
    "body": "Okay, here is where one would have to at least change this - `sagenb/data/sage/html/accounts/registration.html`.  The other place is in `twist.py`.",
...
```json
{
    "body": "Okay, I've got this temporarily disabled.    Re-enabling this is #11470.\n\nPlease test whether this breaks existing usernames with `@`!!!",
    "created_at": "2011-06-14T00:01:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11343",
    "type": "issue_comment",
...
```json
{
    "body": "This is reported upstream at [this link](http://code.google.com/p/sagenb/issues/detail?id=26).",
...
```json
{
    "body": "What's the status of this?  If the Flask notebook isn't going to be done soon, this should really have been in 4.7.1 :( especially as people upgrade their servers.",
...
```json
{
    "body": "At the very least, we should take \"`@`\" out of the explicitly-listed good characters for notebook names on the registration page!",
...
```json
{
    "body": "Ah, I see I just volunteered to review this :)",
...
```json
{
    "body": "Same problem.   See attached screenshot.  You can check this out with username `abc`@`abc` with password `123123`.",
    "created_at": "2011-09-29T13:28:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11343",
    "type": "issue_comment",
...
```json
{
    "body": "Attachment [test.sagenb-failure.png](tarball://root/attachments/some-uuid/ticket11343/test.sagenb-failure.png) by @kcrisman created at 2011-09-29 13:30:31\n\nI'm making this a blocker for 4.7.2.  This is totally unacceptable that we do not at least change the login screen to avoid ``@`` until this is fixed.  Sorry, Leif :)",
...
```json
{
    "body": "Basu, care to try this on Opera?",
...
```json
{
    "body": "I created a fresh /tmp/a.sagenb directory and ran sage -n on that directory. The bug is there in opera-11.51 with sage-4.7.1, amd64 Gentoo Linux. Sorry for all the noise I created earlier.\n\nI believe my ~/.sage directory was being written to by both sage-4.7 (with rkirov-flask notebook) and sage-4.7.1 (with old notebook) and so I wasn't seeing this bug earlier.",
...
```json
{
    "body": "This ticket is listed as blocker, what is the current status?  It seems like disallowing users to register usernames with `@` could be a quick and eays solution.  But they should still be able to *log in* with any existing username.",
...
```json
{
    "body": "Patch does the job.  New registrations with `@` are not possible, existing ones can still log in.",
    "created_at": "2011-10-14T18:03:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11343",
    "type": "issue_comment",
