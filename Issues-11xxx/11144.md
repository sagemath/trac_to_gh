# Issue 11144: trivial typo messes up interact doc

archive/issues_011001.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mvngu"
    ],
    "body": "The doc for `is_auto_update` at [this page](http://www.sagemath.org/doc/reference/sagenb/notebook/interact.html) shows that there is an underscore that is formatting a 'u' incorrectly.  Probably this is a one-or-two-character fix, though I haven't checked it out.  Most likely someone used <code>\\`\\`</code> instead of <code>\\`</code>?\n\nApply [attachment: 11144_sagenb.patch](https://github.com/sagemath/sage/files/ticket11144/11144_sagenb.patch.gz) to sagenb.\n\nCC:  @sagetrac-mvngu\n\nKeywords: **notebook**\n\nComponent: **notebook**\n\nAuthor: **Ivo Hedtke**\n\nReviewer: **John Palmieri**\n\nMerged: **sage-4.7.2.alpha1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11144_\n\n",
    "closed_at": "2011-07-28T10:24:41Z",
    "created_at": "2011-04-07T02:04:11Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p%3A%205%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "trivial typo messes up interact doc",
    "type": "issue",
    "updated_at": "2011-07-28T10:24:41Z",
    "url": "https://github.com/sagemath/sage/issues/11144",
    "user": "https://github.com/kcrisman"
}
```
The doc for `is_auto_update` at [this page](http://www.sagemath.org/doc/reference/sagenb/notebook/interact.html) shows that there is an underscore that is formatting a 'u' incorrectly.  Probably this is a one-or-two-character fix, though I haven't checked it out.  Most likely someone used <code>\`\`</code> instead of <code>\`</code>?

Apply [attachment: 11144_sagenb.patch](https://github.com/sagemath/sage/files/ticket11144/11144_sagenb.patch.gz) to sagenb.

CC:  @sagetrac-mvngu

Keywords: **notebook**

Component: **notebook**

Author: **Ivo Hedtke**

Reviewer: **John Palmieri**

Merged: **sage-4.7.2.alpha1**

_Issue created by migration from https://trac.sagemath.org/ticket/11144_





---

archive/issue_events_147832.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-04-07T02:04:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11144#event-147832"
}
```



---

archive/issue_events_147833.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-04-07T02:04:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "label": "https://github.com/sagemath/sage/labels/p%3A%205%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p: 5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11144#event-147833"
}
```



---

archive/issue_events_147834.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-04-07T02:04:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11144#event-147834"
}
```



---

archive/issue_events_147835.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-04-07T02:04:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "label": "https://github.com/sagemath/sage/labels/good%20first%20issue",
    "label_color": "7057ff",
    "label_name": "good first issue",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11144#event-147835"
}
```



---

archive/issue_events_147836.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2011-04-07T02:04:11Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "subject": "https://github.com/kcrisman",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11144#event-147836"
}
```



---

archive/issue_comments_111008.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_11144.patch.gz](https://github.com/sagemath/sage/files/ticket11144/trac_11144.patch.gz)**",
    "created_at": "2011-07-19T21:41:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11144#issuecomment-111008",
    "user": "https://github.com/hedtke"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_11144.patch.gz](https://github.com/sagemath/sage/files/ticket11144/trac_11144.patch.gz)**



---

archive/issue_events_147837.json:
```json
{
    "actor": "https://github.com/hedtke",
    "created_at": "2011-07-19T21:41:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11144#event-147837"
}
```



---

archive/issue_events_147838.json:
```json
{
    "actor": "https://github.com/hedtke",
    "created_at": "2011-07-19T21:49:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "milestone_number": null,
    "milestone_title": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11144#event-147838"
}
```



---

archive/issue_comments_111009.json:
```json
{
    "body": "Reviewer: **John Palmieri**",
    "created_at": "2011-07-23T17:44:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11144#issuecomment-111009",
    "user": "https://github.com/jhpalmieri"
}
```

Reviewer: **John Palmieri**



---

archive/issue_events_147839.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2011-07-23T17:44:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11144#event-147839"
}
```



---

archive/issue_events_147840.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2011-07-23T17:44:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11144#event-147840"
}
```



---

archive/issue_comments_111010.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,5 @@\n The doc for `is_auto_update` at [this page](http://www.sagemath.org/doc/reference/sagenb/notebook/interact.html) shows that there is an underscore that is formatting a 'u' incorrectly.  Probably this is a one-or-two-character fix, though I haven't checked it out.  Most likely someone used <code>\\`\\`</code> instead of <code>\\`</code>?\n+\n+---\n+\n+Do not apply the patch here.  This is a sagenb issue.\n``````\n",
    "created_at": "2011-07-23T17:44:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11144#issuecomment-111010",
    "user": "https://github.com/jhpalmieri"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,5 @@
 The doc for `is_auto_update` at [this page](http://www.sagemath.org/doc/reference/sagenb/notebook/interact.html) shows that there is an underscore that is formatting a 'u' incorrectly.  Probably this is a one-or-two-character fix, though I haven't checked it out.  Most likely someone used <code>\`\`</code> instead of <code>\`</code>?
+
+---
+
+Do not apply the patch here.  This is a sagenb issue.
``````




---

archive/issue_comments_111011.json:
```json
{
    "body": "Author: **Ivo Hedtke**",
    "created_at": "2011-07-23T17:44:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11144#issuecomment-111011",
    "user": "https://github.com/jhpalmieri"
}
```

Author: **Ivo Hedtke**



---

archive/issue_comments_111012.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nYou've patched the wrong file, `SAGE_ROOT/devel/sage/sage/server/notebook/interact.py`.  Note that this file says at the top\n\n```\n# This file is part of the OLD Sage notebook and is NOT actively developed,\n# maintained, or supported.  As of Sage v4.1.2, all notebook development has\n# moved to the separate Sage Notebook project:\n#\n# http://nb.sagemath.org/\n```\nI've posted it instead as a patch for the sagenb project instead: see [https://code.google.com/p/sagenb/issues/detail?id=41](https://code.google.com/p/sagenb/issues/detail?id=41).\n\nTo the release manager: I think we should close this ticket as invalid, since it's for the Sage notebook, not the Sage library.  Do not apply the patch here.",
    "created_at": "2011-07-23T17:44:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11144#issuecomment-111012",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:3" align="right">comment:3</div>

You've patched the wrong file, `SAGE_ROOT/devel/sage/sage/server/notebook/interact.py`.  Note that this file says at the top

```
# This file is part of the OLD Sage notebook and is NOT actively developed,
# maintained, or supported.  As of Sage v4.1.2, all notebook development has
# moved to the separate Sage Notebook project:
#
# http://nb.sagemath.org/
```
I've posted it instead as a patch for the sagenb project instead: see [https://code.google.com/p/sagenb/issues/detail?id=41](https://code.google.com/p/sagenb/issues/detail?id=41).

To the release manager: I think we should close this ticket as invalid, since it's for the Sage notebook, not the Sage library.  Do not apply the patch here.



---

archive/issue_comments_111013.json:
```json
{
    "body": "Changed keywords from none to **notebook**",
    "created_at": "2011-07-23T17:44:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11144#issuecomment-111013",
    "user": "https://github.com/jhpalmieri"
}
```

Changed keywords from none to **notebook**



---

archive/issue_events_147841.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2011-07-23T17:44:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "label": "https://github.com/sagemath/sage/labels/good%20first%20issue",
    "label_color": "7057ff",
    "label_name": "good first issue",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11144#event-147841"
}
```



---

archive/issue_events_147842.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-24T11:26:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11144#event-147842"
}
```



---

archive/issue_events_147843.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-24T11:26:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11144#event-147843"
}
```



---

archive/issue_comments_111014.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nReplying to [@jhpalmieri](#comment:3):\n> To the release manager: I think we should close this ticket as invalid, since it's for the Sage notebook, not the Sage library.  Do not apply the patch here.\n\nIt is still not at all clear to me whether there is even something as a \"sagenb project\" unfortunately.  I will attach a proper `sagenb` patch.",
    "created_at": "2011-07-24T11:26:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11144#issuecomment-111014",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:4" align="right">comment:4</div>

Replying to [@jhpalmieri](#comment:3):
> To the release manager: I think we should close this ticket as invalid, since it's for the Sage notebook, not the Sage library.  Do not apply the patch here.

It is still not at all clear to me whether there is even something as a "sagenb project" unfortunately.  I will attach a proper `sagenb` patch.



---

archive/issue_comments_111015.json:
```json
{
    "body": "Attachment: **[11144_sagenb.patch.gz](https://github.com/sagemath/sage/files/ticket11144/11144_sagenb.patch.gz)**\n\nPatch for SAGENB",
    "created_at": "2011-07-24T11:28:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11144#issuecomment-111015",
    "user": "https://github.com/jdemeyer"
}
```

Attachment: **[11144_sagenb.patch.gz](https://github.com/sagemath/sage/files/ticket11144/11144_sagenb.patch.gz)**

Patch for SAGENB



---

archive/issue_events_147844.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-24T11:29:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11144#event-147844"
}
```



---

archive/issue_events_147845.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-24T11:29:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11144#event-147845"
}
```



---

archive/issue_events_147846.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-24T11:29:41Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "milestone_number": null,
    "milestone_title": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11144#event-147846"
}
```



---

archive/issue_events_147847.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-24T11:29:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "milestone_number": null,
    "milestone_title": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11144#event-147847"
}
```



---

archive/issue_events_147848.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-24T11:29:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11144#event-147848"
}
```



---

archive/issue_events_147849.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-24T11:29:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11144#event-147849"
}
```



---

archive/issue_comments_111016.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,5 +1,3 @@\n The doc for `is_auto_update` at [this page](http://www.sagemath.org/doc/reference/sagenb/notebook/interact.html) shows that there is an underscore that is formatting a 'u' incorrectly.  Probably this is a one-or-two-character fix, though I haven't checked it out.  Most likely someone used <code>\\`\\`</code> instead of <code>\\`</code>?\n \n----\n-\n-Do not apply the patch here.  This is a sagenb issue.\n+Apply [attachment: 11144_sagenb.patch](https://github.com/sagemath/sage/files/ticket11144/11144_sagenb.patch.gz) to sagenb.\n``````\n",
    "created_at": "2011-07-24T11:29:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11144#issuecomment-111016",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,5 +1,3 @@
 The doc for `is_auto_update` at [this page](http://www.sagemath.org/doc/reference/sagenb/notebook/interact.html) shows that there is an underscore that is formatting a 'u' incorrectly.  Probably this is a one-or-two-character fix, though I haven't checked it out.  Most likely someone used <code>\`\`</code> instead of <code>\`</code>?
 
----
-
-Do not apply the patch here.  This is a sagenb issue.
+Apply [attachment: 11144_sagenb.patch](https://github.com/sagemath/sage/files/ticket11144/11144_sagenb.patch.gz) to sagenb.
``````




---

archive/issue_comments_111017.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nSee also #11409 for avoiding people to patch to wrong files.",
    "created_at": "2011-07-24T11:31:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11144#issuecomment-111017",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:6" align="right">comment:6</div>

See also #11409 for avoiding people to patch to wrong files.



---

archive/issue_comments_111018.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nReplying to [@jhpalmieri](#comment:3):\n> I've posted it instead as a patch for the sagenb project instead: see [https://code.google.com/p/sagenb/issues/detail?id=41](https://code.google.com/p/sagenb/issues/detail?id=41).\n\nI am waiting for clarification on this situation at http://groups.google.com/group/sage-notebook/browse_thread/thread/5f8ee7606e0d94bd\n\nThe patch itself obviously gets positive_review.",
    "created_at": "2011-07-24T11:47:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11144#issuecomment-111018",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:7" align="right">comment:7</div>

Replying to [@jhpalmieri](#comment:3):
> I've posted it instead as a patch for the sagenb project instead: see [https://code.google.com/p/sagenb/issues/detail?id=41](https://code.google.com/p/sagenb/issues/detail?id=41).

I am waiting for clarification on this situation at http://groups.google.com/group/sage-notebook/browse_thread/thread/5f8ee7606e0d94bd

The patch itself obviously gets positive_review.



---

archive/issue_events_147850.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-24T11:47:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11144#event-147850"
}
```



---

archive/issue_events_147851.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-24T11:47:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11144#event-147851"
}
```



---

archive/issue_events_147852.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-24T21:52:37Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "milestone_number": null,
    "milestone_title": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11144#event-147852"
}
```



---

archive/issue_events_147853.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-24T21:52:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "label": "https://github.com/sagemath/sage/labels/pending",
    "label_color": "008080",
    "label_name": "pending",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11144#event-147853"
}
```



---

archive/issue_events_147854.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-25T13:22:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "milestone_number": null,
    "milestone_title": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11144#event-147854"
}
```



---

archive/issue_events_147855.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-25T13:22:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "label": "https://github.com/sagemath/sage/labels/pending",
    "label_color": "008080",
    "label_name": "pending",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11144#event-147855"
}
```



---

archive/issue_events_147856.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-28T10:24:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11144#event-147856"
}
```



---

archive/issue_events_147857.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-28T10:24:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11144#event-147857"
}
```



---

archive/issue_comments_111019.json:
```json
{
    "body": "Merged: **sage-4.7.2.alpha1**",
    "created_at": "2011-07-28T10:24:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11144",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11144#issuecomment-111019",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.7.2.alpha1**
