# Issue 11792: Clean up the FLINT QS spkg

archive/issues_011620.json:
```json
{
    "assignees": [],
    "body": "This is a follow-up to #11351; see [comment:ticket:11351:2 this]\n\nSee also #12428.\n\nCC:  @jpflori @SnarkBoojum\n\nKeywords: **quadratic sieve flintqs**\n\nReviewer: **Fran\u00e7ois Bissey**\n\nComponent: **packages: standard**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11792_\n\n",
    "closed_at": "2017-01-21T18:03:11Z",
    "created_at": "2011-09-12T18:54:57Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Clean up the FLINT QS spkg",
    "type": "issue",
    "updated_at": "2017-01-21T18:03:11Z",
    "url": "https://github.com/sagemath/sage/issues/11792",
    "user": "https://github.com/nexttime"
}
```
This is a follow-up to #11351; see [comment:ticket:11351:2 this]

See also #12428.

CC:  @jpflori @SnarkBoojum

Keywords: **quadratic sieve flintqs**

Reviewer: **François Bissey**

Component: **packages: standard**

_Issue created by migration from https://trac.sagemath.org/ticket/11792_





---

archive/issue_events_158598.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2011-09-12T18:54:57Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11792#event-158598"
}
```



---

archive/issue_events_158599.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2011-09-12T18:54:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11792#event-158599"
}
```



---

archive/issue_events_158600.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2011-09-12T18:54:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11792#event-158600"
}
```



---

archive/issue_events_158601.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2011-09-12T18:54:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11792#event-158601"
}
```



---

archive/issue_comments_123224.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nSee also #9544 and #12855, for reference.",
    "created_at": "2012-06-21T13:10:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11792#issuecomment-123224",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:1" align="right">comment:1</div>

See also #9544 and #12855, for reference.



---

archive/issue_comments_123225.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nSee also #12173, upgrading FLINT to 2.3.  Does the qsieve there do what we need?\n\n---\n\nAlso note #14202, which is orthogonal but also about qsieve, so it should be pointed out.",
    "created_at": "2013-03-26T15:12:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11792#issuecomment-123225",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:3" align="right">comment:3</div>

See also #12173, upgrading FLINT to 2.3.  Does the qsieve there do what we need?

---

Also note #14202, which is orthogonal but also about qsieve, so it should be pointed out.



---

archive/issue_comments_123226.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nPuttins as need-info, since there's a pending question: does flint 2.3 make it possible to get flintqs out?",
    "created_at": "2013-04-16T12:59:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11792#issuecomment-123226",
    "user": "https://github.com/SnarkBoojum"
}
```

<div id="comment:4" align="right">comment:4</div>

Puttins as need-info, since there's a pending question: does flint 2.3 make it possible to get flintqs out?



---

archive/issue_events_158602.json:
```json
{
    "actor": "https://github.com/SnarkBoojum",
    "created_at": "2013-04-16T12:59:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11792#event-158602"
}
```



---

archive/issue_comments_123227.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nI think FLINT 2.3 provides a quadratic sieve implementation but only for two limbs integers.\nSee:\n* https://github.com/wbhart/flint2/blob/trunk/qsieve/doc/qsieve.txt\nSo I guess the answer is currently no.\nWe'll have to wait until Bill or someone else ports the old code.\n\n(FLINT 1.6 included a quadratic sieve to build as a separate executable, see:\n* http://www.flintlib.org/flint-1.6.pdf, section 18, page 64;\nas did FLINT 1.5.2 (latest version included in Sage):\n* http://www.flintlib.org/flint-1.5.2.pdf, section 14, page 57.)",
    "created_at": "2013-04-16T13:11:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11792#issuecomment-123227",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:5" align="right">comment:5</div>

I think FLINT 2.3 provides a quadratic sieve implementation but only for two limbs integers.
See:
* https://github.com/wbhart/flint2/blob/trunk/qsieve/doc/qsieve.txt
So I guess the answer is currently no.
We'll have to wait until Bill or someone else ports the old code.

(FLINT 1.6 included a quadratic sieve to build as a separate executable, see:
* http://www.flintlib.org/flint-1.6.pdf, section 18, page 64;
as did FLINT 1.5.2 (latest version included in Sage):
* http://www.flintlib.org/flint-1.5.2.pdf, section 14, page 57.)



---

archive/issue_events_158603.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11792#event-158603"
}
```



---

archive/issue_events_158604.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11792#event-158604"
}
```



---

archive/issue_events_158605.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11792#event-158605"
}
```



---

archive/issue_events_158606.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11792#event-158606"
}
```



---

archive/issue_events_158607.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11792#event-158607"
}
```



---

archive/issue_events_158608.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11792#event-158608"
}
```



---

archive/issue_events_158609.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11792#event-158609"
}
```



---

archive/issue_events_158610.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11792#event-158610"
}
```



---

archive/issue_events_158611.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-11-13T14:11:29Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "title_is": "Clean up the FLINT QS spkg",
    "title_was": "Clean up or remove the FLINT QS spkg",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11792#event-158611"
}
```



---

archive/issue_comments_123228.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1 @@\n-This is a follow-up to #11351; see [comment:ticket:11351:2 this] and [comment:ticket:11351:8 this] comment there.\n-\n-At least FLINT 1.x ships with a (presumably more recent) version of the quadratic sieve program, so rather than cleanung up the separate `flintqs` package, one could change the FLINT spkg to also build and install this program.\n-\n+This is a follow-up to #11351; see [comment:ticket:11351:2 this]\n``````\n",
    "created_at": "2014-11-13T14:11:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11792#issuecomment-123228",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1 @@
-This is a follow-up to #11351; see [comment:ticket:11351:2 this] and [comment:ticket:11351:8 this] comment there.
-
-At least FLINT 1.x ships with a (presumably more recent) version of the quadratic sieve program, so rather than cleanung up the separate `flintqs` package, one could change the FLINT spkg to also build and install this program.
-
+This is a follow-up to #11351; see [comment:ticket:11351:2 this]
``````




---

archive/issue_comments_123229.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n This is a follow-up to #11351; see [comment:ticket:11351:2 this]\n+\n+See also #12428.\n``````\n",
    "created_at": "2014-11-13T14:12:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11792#issuecomment-123229",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
 This is a follow-up to #11351; see [comment:ticket:11351:2 this]
+
+See also #12428.
``````




---

archive/issue_comments_123230.json:
```json
{
    "body": "Reviewer: **Fran\u00e7ois Bissey**",
    "created_at": "2016-09-17T07:26:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11792#issuecomment-123230",
    "user": "https://github.com/kiwifb"
}
```

Reviewer: **François Bissey**



---

archive/issue_events_158612.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-09-17T07:26:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11792#event-158612"
}
```



---

archive/issue_events_158613.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-09-17T07:26:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11792#event-158613"
}
```



---

archive/issue_comments_123231.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">comment:12</div>\n\nObsolete, we have a new autotooled `flintQS`",
    "created_at": "2016-09-17T07:26:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11792#issuecomment-123231",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:12" align="right">comment:12</div>

Obsolete, we have a new autotooled `flintQS`



---

archive/issue_events_158614.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-09-17T07:26:48Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11792#event-158614"
}
```



---

archive/issue_events_158615.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-09-17T07:26:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11792#event-158615"
}
```



---

archive/issue_events_158616.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-09-17T07:26:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11792#event-158616"
}
```



---

archive/issue_events_158617.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-01-21T18:03:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11792#event-158617"
}
```



---

archive/issue_events_158618.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-01-21T18:03:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11792",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11792#event-158618"
}
```
