# Issue 11345: Period method for quadratic irrationals

archive/issues_011173.json:
```json
{
    "body": "We propose implementing a `period` method for quadratic irrational elements of the field of continued fractions. \n\nSee the post at [http://ask.sagemath.org/question/545/continued-fraction-expansion-of-quadratic](http://ask.sagemath.org/question/545/continued-fraction-expansion-of-quadratic) for an algorithm.\n\nAssignee: @loefflerd\n\nCC:  @kcrisman\n\nKeywords: continued fraction, arithmetic\n\nReviewer: Benjamin Jones, Vincent Delecroix\n\nDependencies: #14567\n\nResolution: duplicate\n\nIssue created by migration from https://trac.sagemath.org/ticket/11345\n\n",
    "closed_at": "2013-10-05T09:46:29Z",
    "created_at": "2011-05-17T16:56:29Z",
    "labels": [
        "component: number fields",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "Period method for quadratic irrationals",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/11345",
    "user": "https://github.com/benjaminfjones"
}
```
We propose implementing a `period` method for quadratic irrational elements of the field of continued fractions. 

See the post at [http://ask.sagemath.org/question/545/continued-fraction-expansion-of-quadratic](http://ask.sagemath.org/question/545/continued-fraction-expansion-of-quadratic) for an algorithm.

Assignee: @loefflerd

CC:  @kcrisman

Keywords: continued fraction, arithmetic

Reviewer: Benjamin Jones, Vincent Delecroix

Dependencies: #14567

Resolution: duplicate

Issue created by migration from https://trac.sagemath.org/ticket/11345





---

archive/issue_comments_149564.json:
```json
{
    "body": "<a id='comment:1'></a>Well, an algorithm isn't a patch :)  but this is a promising start.  \n\n**However**, one would want to make sure there was a way to check whether this was actually a quadratic surd before using this method.  I think that the poster's comment about the [Maple documentation](http://www.maplesoft.com/support/help/Maple/view.aspx?path=numtheory/cfrac) for this could provide some helpful thoughts, though I don't think we'd want to slavishly follow the syntax or anything, especially as we have the field of continued fractions to help us out.\n\nOne more random thought; is it possible to hack out the code from GAP for this in some way, so as not to reinvent the wheel?",
    "created_at": "2011-05-17T20:05:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11345#issuecomment-149564",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:1'></a>Well, an algorithm isn't a patch :)  but this is a promising start.  

**However**, one would want to make sure there was a way to check whether this was actually a quadratic surd before using this method.  I think that the poster's comment about the [Maple documentation](http://www.maplesoft.com/support/help/Maple/view.aspx?path=numtheory/cfrac) for this could provide some helpful thoughts, though I don't think we'd want to slavishly follow the syntax or anything, especially as we have the field of continued fractions to help us out.

One more random thought; is it possible to hack out the code from GAP for this in some way, so as not to reinvent the wheel?



---

archive/issue_comments_149565.json:
```json
{
    "body": "<a id='comment:2'></a>I intend to provide a patch, I just thought I'd get the ball rolling. Maybe I should have waited to file the ticket, though. I'll be on vacation until June 6.\n\nI'm not familiar with the term \"surd\", but I guess that means an algebraic number, or maybe just an n-th root?",
    "created_at": "2011-05-17T21:10:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11345#issuecomment-149565",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:2'></a>I intend to provide a patch, I just thought I'd get the ball rolling. Maybe I should have waited to file the ticket, though. I'll be on vacation until June 6.

I'm not familiar with the term "surd", but I guess that means an algebraic number, or maybe just an n-th root?



---

archive/issue_comments_149566.json:
```json
{
    "body": "<a id='comment:3'></a>Replying to [benjaminfjones](#comment%3A2):\n> I intend to provide a patch, I just thought I'd get the ball rolling. Maybe I should have waited to file the ticket, though. I'll be on vacation until June 6.\n\nNo, always file a ticket - then others know about it, and so forth.  Many, many, *many* tickets are filed that don't have patches for a long time, but do have ideas.\n> I'm not familiar with the term \"surd\", but I guess that means an algebraic number, or maybe just an n-th root? \n\n[See here](http://mathworld.wolfram.com/QuadraticSurd.html), for example. It's an old term for (quadratic) irrationals, more or less.  You see it a lot in books written before about 1960, esp. number theory ones.",
    "created_at": "2011-05-18T01:48:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11345#issuecomment-149566",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:3'></a>Replying to [benjaminfjones](#comment%3A2):
> I intend to provide a patch, I just thought I'd get the ball rolling. Maybe I should have waited to file the ticket, though. I'll be on vacation until June 6.

No, always file a ticket - then others know about it, and so forth.  Many, many, *many* tickets are filed that don't have patches for a long time, but do have ideas.
> I'm not familiar with the term "surd", but I guess that means an algebraic number, or maybe just an n-th root? 

[See here](http://mathworld.wolfram.com/QuadraticSurd.html), for example. It's an old term for (quadratic) irrationals, more or less.  You see it a lot in books written before about 1960, esp. number theory ones.



---

archive/issue_comments_149567.json:
```json
{
    "body": "<a id='comment:4'></a>Hi,\n\nThis is a long standing ticket and I guess nobody is working on it anymore. I will close it as a duplicate as soon as #14567 is finished. Any comment or contribution welcome!\n\nVincent",
    "created_at": "2013-05-12T18:24:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11345#issuecomment-149567",
    "user": "https://github.com/videlec"
}
```

<a id='comment:4'></a>Hi,

This is a long standing ticket and I guess nobody is working on it anymore. I will close it as a duplicate as soon as #14567 is finished. Any comment or contribution welcome!

Vincent



---

archive/issue_comments_149568.json:
```json
{
    "body": "<a id='comment:5'></a>Agreed. Nice work on #14567.",
    "created_at": "2013-05-13T16:54:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11345#issuecomment-149568",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:5'></a>Agreed. Nice work on #14567.



---

archive/issue_comments_149569.json:
```json
{
    "body": "Changing author from \"Benjamin Jones, Seth Dutter\" to \"\"",
    "created_at": "2013-05-13T17:40:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11345#issuecomment-149569",
    "user": "https://github.com/kcrisman"
}
```

Changing author from "Benjamin Jones, Seth Dutter" to ""



---

archive/issue_comments_149570.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2013-05-13T17:40:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11345#issuecomment-149570",
    "user": "https://github.com/kcrisman"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_149571.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Benjamin Jones, Vincent Delecrois\"",
    "created_at": "2013-05-13T17:40:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11345#issuecomment-149571",
    "user": "https://github.com/kcrisman"
}
```

Changing reviewer from "" to "Benjamin Jones, Vincent Delecrois"



---

archive/issue_events_029426.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-05-13T17:40:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11345",
    "milestone": "sage-pending",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11345#event-29426"
}
```



---

archive/issue_comments_149572.json:
```json
{
    "body": "<a id='comment:7'></a>To release manager: when #14567 is merged, this should become sage-duplicate.",
    "created_at": "2013-05-13T17:41:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11345#issuecomment-149572",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:7'></a>To release manager: when #14567 is merged, this should become sage-duplicate.



---

archive/issue_comments_149573.json:
```json
{
    "body": "Changing reviewer from \"Benjamin Jones, Vincent Delecrois\" to \"Benjamin Jones, Vincent Delecroix\"",
    "created_at": "2013-05-13T17:41:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11345#issuecomment-149573",
    "user": "https://github.com/kcrisman"
}
```

Changing reviewer from "Benjamin Jones, Vincent Delecrois" to "Benjamin Jones, Vincent Delecroix"



---

archive/issue_comments_149574.json:
```json
{
    "body": "Changing dependencies from \"\" to \"#14567\"",
    "created_at": "2013-05-13T17:41:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11345#issuecomment-149574",
    "user": "https://github.com/kcrisman"
}
```

Changing dependencies from "" to "#14567"



---

archive/issue_comments_149575.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2013-05-13T17:41:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11345#issuecomment-149575",
    "user": "https://github.com/kcrisman"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_events_029427.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-13T17:44:54Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11345",
    "milestone": "sage-pending",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11345#event-29427"
}
```



---

archive/issue_events_029428.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-13T17:44:54Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11345",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11345#event-29428"
}
```



---

archive/issue_comments_149576.json:
```json
{
    "body": "Resolution: duplicate",
    "created_at": "2013-10-05T09:46:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11345#issuecomment-149576",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: duplicate



---

archive/issue_events_029429.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-05T09:46:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/11345",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11345#event-29429"
}
```
