# Issue 11586: bug in p-adic extension norm method

archive/issues_011414.json:
```json
{
    "assignees": [],
    "body": "\n```\nR.<x> = QQ[]\nf = x^2 + 3*x + 1\nK = Qp(7)\nM.<a> = K.extension(f)\nM(7).norm()\n```\nThe above outputs `1 + O(7^20)`, which is completely wrong.  The norm should be `7^2 + O(7^20)`, I think -- anyways, it should be divisible by 7. \n\nThe problem is that the actual code for the norm, which ones sees by typing `a.norm??` assumes that the generator for M is in fact a generator for the maximal ideal of the ring of integers.  However, this assumption is just completely false. \n\nTyping `a.norm???` we see\n\n```\n...\n    norm_of_uniformizer = (-1)**self.parent().degree() * self.parent().defining_polynomial()[0]\n    if self.valuation() == 0:\n        return self.parent().ground_ring()(self.unit_part().matrix_mod_pn().det())\n    else:\n        return self.parent().ground_ring()(self.unit_part().matrix_mod_pn().det()) * norm_of_uniformizer**self.valuation()\n```    \n\nThe above is wrong as mentioned above.  Moreover, it is nonoptimal in that norm_of_uniformizer is computed but never used in the case when `self.valuation()==0`.\n\nThis bug caused some confusion when computing with p-adic L-series for a research paper...\n\n---\n\nApply [attachment: 11586.patch.gz](https://github.com/sagemath/sage/files/ticket11586/11586.patch.gz) to the Sage library.\n\n\nDepends on #8241\n\nAssignee: **@roed314**\n\nKeywords: **padic**\n\nReviewer: **Jennifer Balakrishnan**\n\nAuthor: **David Roe**\n\nMerged: **sage-5.0.beta10**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11586_\n\n",
    "closed_at": "2012-03-23T15:19:21Z",
    "created_at": "2011-07-11T11:04:02Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20padics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "bug in p-adic extension norm method",
    "type": "issue",
    "updated_at": "2012-03-23T15:19:21Z",
    "url": "https://github.com/sagemath/sage/issues/11586",
    "user": "https://github.com/williamstein"
}
```

```
R.<x> = QQ[]
f = x^2 + 3*x + 1
K = Qp(7)
M.<a> = K.extension(f)
M(7).norm()
```
The above outputs `1 + O(7^20)`, which is completely wrong.  The norm should be `7^2 + O(7^20)`, I think -- anyways, it should be divisible by 7. 

The problem is that the actual code for the norm, which ones sees by typing `a.norm??` assumes that the generator for M is in fact a generator for the maximal ideal of the ring of integers.  However, this assumption is just completely false. 

Typing `a.norm???` we see

```
...
    norm_of_uniformizer = (-1)**self.parent().degree() * self.parent().defining_polynomial()[0]
    if self.valuation() == 0:
        return self.parent().ground_ring()(self.unit_part().matrix_mod_pn().det())
    else:
        return self.parent().ground_ring()(self.unit_part().matrix_mod_pn().det()) * norm_of_uniformizer**self.valuation()
```    

The above is wrong as mentioned above.  Moreover, it is nonoptimal in that norm_of_uniformizer is computed but never used in the case when `self.valuation()==0`.

This bug caused some confusion when computing with p-adic L-series for a research paper...

---

Apply [attachment: 11586.patch.gz](https://github.com/sagemath/sage/files/ticket11586/11586.patch.gz) to the Sage library.


Depends on #8241

Assignee: **@roed314**

Keywords: **padic**

Reviewer: **Jennifer Balakrishnan**

Author: **David Roe**

Merged: **sage-5.0.beta10**

_Issue created by migration from https://trac.sagemath.org/ticket/11586_





---

archive/issue_events_136910.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-07-11T11:04:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11586",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11586#event-136910"
}
```



---

archive/issue_events_136911.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-07-11T11:04:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11586",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20padics",
    "label_color": "0000ff",
    "label_name": "component: padics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11586#event-136911"
}
```



---

archive/issue_events_136912.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-07-11T11:04:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11586",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11586#event-136912"
}
```



---

archive/issue_events_136913.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-07-11T11:04:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11586",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11586#event-136913"
}
```



---

archive/issue_comments_120958.json:
```json
{
    "body": "Attachment: **[11586.patch.gz](https://github.com/sagemath/sage/files/ticket11586/11586.patch.gz)**",
    "created_at": "2011-07-13T06:18:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11586",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11586#issuecomment-120958",
    "user": "https://github.com/roed314"
}
```

Attachment: **[11586.patch.gz](https://github.com/sagemath/sage/files/ticket11586/11586.patch.gz)**



---

archive/issue_events_136914.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2011-07-13T06:20:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11586",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11586#event-136914"
}
```



---

archive/issue_comments_120959.json:
```json
{
    "body": "Author: **David Roe**",
    "created_at": "2011-07-13T06:20:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11586",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11586#issuecomment-120959",
    "user": "https://github.com/roed314"
}
```

Author: **David Roe**



---

archive/issue_comments_120960.json:
```json
{
    "body": "Changed keywords from **** to **padic**",
    "created_at": "2011-07-13T06:20:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11586",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11586#issuecomment-120960",
    "user": "https://github.com/roed314"
}
```

Changed keywords from **** to **padic**



---

archive/issue_comments_120961.json:
```json
{
    "body": "Dependencies: **8241**",
    "created_at": "2011-07-13T06:20:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11586",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11586#issuecomment-120961",
    "user": "https://github.com/roed314"
}
```

Dependencies: **8241**



---

archive/issue_comments_120962.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nSorry about that.  The attached patch should fix the problem (note that you need to apply #8241 first).",
    "created_at": "2011-07-13T06:20:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11586",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11586#issuecomment-120962",
    "user": "https://github.com/roed314"
}
```

<a id='comment:1'>Comment 1:</a>
Sorry about that.  The attached patch should fix the problem (note that you need to apply #8241 first).



---

archive/issue_events_136915.json:
```json
{
    "actor": "https://github.com/jbalakrishnan",
    "created_at": "2011-08-24T19:09:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11586",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11586#event-136915"
}
```



---

archive/issue_events_136916.json:
```json
{
    "actor": "https://github.com/jbalakrishnan",
    "created_at": "2011-08-24T19:09:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11586",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11586#event-136916"
}
```



---

archive/issue_comments_120963.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nGreat; this fixes the bug. All tests pass!",
    "created_at": "2011-08-24T19:09:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11586",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11586#issuecomment-120963",
    "user": "https://github.com/jbalakrishnan"
}
```

<a id='comment:2'>Comment 2:</a>
Great; this fixes the bug. All tests pass!



---

archive/issue_comments_120964.json:
```json
{
    "body": "Changed dependencies from **8241** to **#8241**",
    "created_at": "2011-08-24T22:01:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11586",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11586#issuecomment-120964",
    "user": "https://github.com/jdemeyer"
}
```

Changed dependencies from **8241** to **#8241**



---

archive/issue_comments_120965.json:
```json
{
    "body": "Reviewer: **Jennifer Balakrishnan**",
    "created_at": "2011-08-24T22:01:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11586",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11586#issuecomment-120965",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jennifer Balakrishnan**



---

archive/issue_comments_120966.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nMake someone review #8241...",
    "created_at": "2011-09-08T14:37:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11586",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11586#issuecomment-120966",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:4'>Comment 4:</a>
Make someone review #8241...



---

archive/issue_comments_120967.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -25,3 +25,7 @@\n \n This bug caused some confusion when computing with p-adic L-series for a research paper...\n \n+---\n+\n+Apply [attachment: http://11586.patch.gz](https://github.com/sagemath/sage/files/ticket11586/9dd247c831070f19a56fadf4610ab0b5.gz) to the Sage library.\n+\n``````\n",
    "created_at": "2011-09-08T14:37:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11586",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11586#issuecomment-120967",
    "user": "https://github.com/nexttime"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -25,3 +25,7 @@
 
 This bug caused some confusion when computing with p-adic L-series for a research paper...
 
+---
+
+Apply [attachment: http://11586.patch.gz](https://github.com/sagemath/sage/files/ticket11586/9dd247c831070f19a56fadf4610ab0b5.gz) to the Sage library.
+
``````




---

archive/issue_comments_120968.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -27,5 +27,5 @@\n \n ---\n \n-Apply [attachment: http://11586.patch.gz](https://github.com/sagemath/sage/files/ticket11586/9dd247c831070f19a56fadf4610ab0b5.gz) to the Sage library.\n+Apply [attachment: 11586.patch.gz](https://github.com/sagemath/sage/files/ticket11586/11586.patch.gz) to the Sage library.\n \n``````\n",
    "created_at": "2011-09-08T14:38:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11586",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11586#issuecomment-120968",
    "user": "https://github.com/nexttime"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -27,5 +27,5 @@
 
 ---
 
-Apply [attachment: http://11586.patch.gz](https://github.com/sagemath/sage/files/ticket11586/9dd247c831070f19a56fadf4610ab0b5.gz) to the Sage library.
+Apply [attachment: 11586.patch.gz](https://github.com/sagemath/sage/files/ticket11586/11586.patch.gz) to the Sage library.
 
``````




---

archive/issue_events_136917.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2011-09-19T09:47:59Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11586",
    "milestone_number": null,
    "milestone_title": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11586#event-136917"
}
```



---

archive/issue_events_136918.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2011-09-19T09:47:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11586",
    "label": "https://github.com/sagemath/sage/labels/pending",
    "label_color": "008080",
    "label_name": "pending",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11586#event-136918"
}
```



---

archive/issue_comments_120969.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nIsn't this what we have a milestone \"sage-pending\" for?",
    "created_at": "2011-09-19T09:47:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11586",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11586#issuecomment-120969",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:6'>Comment 6:</a>
Isn't this what we have a milestone "sage-pending" for?



---

archive/issue_comments_120970.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nReplying to [@loefflerd](#comment%3A6):\n> Isn't this what we have a milestone \"sage-pending\" for?\n\nI personally don't use \"sage-pending\" for *that* situation, since my script does dependency checking, so won't attempt to merge it, but considers it for inclusion, i.e., I'll see that it has positive review, and all of its dependencies with their state.\n\nMy interpretation (or use) of \"sage-pending\" is to postpone tickets for *other* reasons, so I currently only see that they're postponed, and don't consider them further.\n\nI thought this was the original intent, to have some \"formal\" way to plan which tickets get merged when during management of a specific release, also since we don't have milestones for devel releases.",
    "created_at": "2011-09-19T14:10:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11586",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11586#issuecomment-120970",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:7'>Comment 7:</a>
Replying to [@loefflerd](#comment%3A6):
> Isn't this what we have a milestone "sage-pending" for?

I personally don't use "sage-pending" for *that* situation, since my script does dependency checking, so won't attempt to merge it, but considers it for inclusion, i.e., I'll see that it has positive review, and all of its dependencies with their state.

My interpretation (or use) of "sage-pending" is to postpone tickets for *other* reasons, so I currently only see that they're postponed, and don't consider them further.

I thought this was the original intent, to have some "formal" way to plan which tickets get merged when during management of a specific release, also since we don't have milestones for devel releases.



---

archive/issue_events_136919.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-03-21T14:15:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11586",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11586#event-136919"
}
```



---

archive/issue_events_136920.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-03-21T14:15:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11586",
    "label": "https://github.com/sagemath/sage/labels/pending",
    "label_color": "008080",
    "label_name": "pending",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11586#event-136920"
}
```



---

archive/issue_comments_120971.json:
```json
{
    "body": "Merged: **sage-5.0.beta10**",
    "created_at": "2012-03-23T15:19:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11586",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11586#issuecomment-120971",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.0.beta10**



---

archive/issue_events_136921.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-03-23T15:19:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11586",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11586#event-136921"
}
```



---

archive/issue_events_136922.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-03-23T15:19:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11586",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11586#event-136922"
}
```
