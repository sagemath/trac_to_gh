# Issue 11278: singular 3-1-1-4.p8 fails on Mac OS X 10.4

archive/issues_011106.json:
```json
{
    "body": "On my Mac OS X 10.4 PPC 32-bit system, singular (#11084) fails to compile with -O2 while it *does* work with -O3.  This is the relevant part from the log file:\n\n```\ng++ -O2 -g -fPIC -pipe -fno-implicit-templates -I. -I../kernel -I/Users/jdemeyer/sage-4.7.rc0/local/include -I/Users/jdemeyer/sage-4.7.rc0/local/include -I/Users/jdemeyer/sage-4.7.rc0/local/include  -DNDEBUG -DOM_NDEBUG -DppcMac_darwin -DHAVE_CONFIG_H -c mpsr_Tok.cc\nlibtool -dynamic -twolevel_namespace -weak_reference_mismatches weak -undefined dynamic_lookup -single_module -o libsingular.dylib \\\nlibsingular-tesths.o iparith.o mpsr_Tok.o claptmpl.o \\\ngrammar.o scanner.o attrib.o eigenval_ip.o extra.o fehelp.o feOpt.o ipassign.o ipconv.o ipid.o iplib.o ipprint.o ipshell.o lists.o sdb.o fglm.o interpolation.o silink.o subexpr.o janet.o wrapper.o libparse.o sing_win.o gms.o pcv.o maps_ip.o walk.o walk_ip.o cntrlc.o misc_ip.o calcSVD.o Minor.o MinorProcessor.o MinorInterface.o  slInit_Static.o mpsr_Put.o mpsr_PutPoly.o mpsr_GetPoly.o mpsr_sl.o mpsr_Get.o mpsr_GetMisc.o mpsr_Error.o  ndbm.o sing_dbm.o -lkernel -L../kernel -L../factory -L../libfac -L/Users/jdemeyer/sage-4.7.rc0/local/lib -lsingfac -lsingcf -lntl -lreadline -lgmp -lomalloc\nld: for architecture ppc\nld: ../kernel/libkernel.a(matpol.o) has external relocation entries in non-writable section (__TEXT,__text) for symbols:\nrestFP\nsaveFP\nlibtool: internal link edit command failed\nmake[3]: *** [libsingular] Error 1\nmake[2]: *** [libsingular] Error 2\nUnable to build Singular.\n```\n\n[Googling for restFP/saveFP](http://www.google.be/search?q=restFP+saveFP) seems to indicate it is a known problem when mixing compilers, see for example [http://lists.apple.com/archives/fortran-dev/2004/jul/msg00036.html](http://lists.apple.com/archives/fortran-dev/2004/jul/msg00036.html)\n\nNew spkg using -O3 on gcc 4.0.x: [http://boxen.math.washington.edu/home/jdemeyer/spkg/singular-3-1-1-4.p9.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/singular-3-1-1-4.p9.spkg)\n\nAssignee: tbd\n\nResolution: fixed\n\nAuthor: Jeroen Demeyer\n\nReviewer: Karl-Dieter Crisman\n\nMerged: sage-4.7.rc2\n\nIssue created by migration from https://trac.sagemath.org/ticket/11278\n\n",
    "closed_at": "2011-05-11T19:15:48Z",
    "created_at": "2011-04-30T20:35:18Z",
    "labels": [
        "component: packages: standard",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.7",
    "title": "singular 3-1-1-4.p8 fails on Mac OS X 10.4",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/11278",
    "user": "https://github.com/jdemeyer"
}
```
On my Mac OS X 10.4 PPC 32-bit system, singular (#11084) fails to compile with -O2 while it *does* work with -O3.  This is the relevant part from the log file:

```
g++ -O2 -g -fPIC -pipe -fno-implicit-templates -I. -I../kernel -I/Users/jdemeyer/sage-4.7.rc0/local/include -I/Users/jdemeyer/sage-4.7.rc0/local/include -I/Users/jdemeyer/sage-4.7.rc0/local/include  -DNDEBUG -DOM_NDEBUG -DppcMac_darwin -DHAVE_CONFIG_H -c mpsr_Tok.cc
libtool -dynamic -twolevel_namespace -weak_reference_mismatches weak -undefined dynamic_lookup -single_module -o libsingular.dylib \
libsingular-tesths.o iparith.o mpsr_Tok.o claptmpl.o \
grammar.o scanner.o attrib.o eigenval_ip.o extra.o fehelp.o feOpt.o ipassign.o ipconv.o ipid.o iplib.o ipprint.o ipshell.o lists.o sdb.o fglm.o interpolation.o silink.o subexpr.o janet.o wrapper.o libparse.o sing_win.o gms.o pcv.o maps_ip.o walk.o walk_ip.o cntrlc.o misc_ip.o calcSVD.o Minor.o MinorProcessor.o MinorInterface.o  slInit_Static.o mpsr_Put.o mpsr_PutPoly.o mpsr_GetPoly.o mpsr_sl.o mpsr_Get.o mpsr_GetMisc.o mpsr_Error.o  ndbm.o sing_dbm.o -lkernel -L../kernel -L../factory -L../libfac -L/Users/jdemeyer/sage-4.7.rc0/local/lib -lsingfac -lsingcf -lntl -lreadline -lgmp -lomalloc
ld: for architecture ppc
ld: ../kernel/libkernel.a(matpol.o) has external relocation entries in non-writable section (__TEXT,__text) for symbols:
restFP
saveFP
libtool: internal link edit command failed
make[3]: *** [libsingular] Error 1
make[2]: *** [libsingular] Error 2
Unable to build Singular.
```

[Googling for restFP/saveFP](http://www.google.be/search?q=restFP+saveFP) seems to indicate it is a known problem when mixing compilers, see for example [http://lists.apple.com/archives/fortran-dev/2004/jul/msg00036.html](http://lists.apple.com/archives/fortran-dev/2004/jul/msg00036.html)

New spkg using -O3 on gcc 4.0.x: [http://boxen.math.washington.edu/home/jdemeyer/spkg/singular-3-1-1-4.p9.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/singular-3-1-1-4.p9.spkg)

Assignee: tbd

Resolution: fixed

Author: Jeroen Demeyer

Reviewer: Karl-Dieter Crisman

Merged: sage-4.7.rc2

Issue created by migration from https://trac.sagemath.org/ticket/11278





---

archive/issue_comments_147859.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,16 @@\n-On my Mac OS X 10.4 PPC 32-bit system, singular (#11084) fails to compile with -O2 while it *does* work with -O3.  I have narrowed it down to the option `-finline-functions` which must be enabled for the build to work on this system.\n+On my Mac OS X 10.4 PPC 32-bit system, singular (#11084) fails to compile with -O2 while it *does* work with -O3.  This is the relevant part from the log file:\n+\n+```\n+g++ -O3 -g -fPIC -pipe -fno-implicit-templates -I. -I../kernel -I/Users/jdemeyer/sage-4.7.rc0/local/include -I/Users/jdemeyer/sage-4.7.rc0/local/include -I/Users/jdemeyer/sage-4.7.rc0/local/include  -DNDEBUG -DOM_NDEBUG -DppcMac_darwin -DHAVE_CONFIG_H -c mpsr_Tok.cc\n+libtool -dynamic -twolevel_namespace -weak_reference_mismatches weak -undefined dynamic_lookup -single_module -o libsingular.dylib \\\n+libsingular-tesths.o iparith.o mpsr_Tok.o claptmpl.o \\\n+grammar.o scanner.o attrib.o eigenval_ip.o extra.o fehelp.o feOpt.o ipassign.o ipconv.o ipid.o iplib.o ipprint.o ipshell.o lists.o sdb.o fglm.o interpolation.o silink.o subexpr.o janet.o wrapper.o libparse.o sing_win.o gms.o pcv.o maps_ip.o walk.o walk_ip.o cntrlc.o misc_ip.o calcSVD.o Minor.o MinorProcessor.o MinorInterface.o  slInit_Static.o mpsr_Put.o mpsr_PutPoly.o mpsr_GetPoly.o mpsr_sl.o mpsr_Get.o mpsr_GetMisc.o mpsr_Error.o  ndbm.o sing_dbm.o -lkernel -L../kernel -L../factory -L../libfac -L/Users/jdemeyer/sage-4.7.rc0/local/lib -lsingfac -lsingcf -lntl -lreadline -lgmp -lomalloc\n+ld: for architecture ppc\n+ld: ../kernel/libkernel.a(matpol.o) has external relocation entries in non-writable section (__TEXT,__text) for symbols:\n+restFP\n+saveFP\n+libtool: internal link edit command failed\n+make[3]: *** [libsingular] Error 1\n+make[2]: *** [libsingular] Error 2\n+Unable to build Singular.\n+```\n``````\n",
    "created_at": "2011-04-30T21:14:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147859",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,16 @@
-On my Mac OS X 10.4 PPC 32-bit system, singular (#11084) fails to compile with -O2 while it *does* work with -O3.  I have narrowed it down to the option `-finline-functions` which must be enabled for the build to work on this system.
+On my Mac OS X 10.4 PPC 32-bit system, singular (#11084) fails to compile with -O2 while it *does* work with -O3.  This is the relevant part from the log file:
+
+```
+g++ -O3 -g -fPIC -pipe -fno-implicit-templates -I. -I../kernel -I/Users/jdemeyer/sage-4.7.rc0/local/include -I/Users/jdemeyer/sage-4.7.rc0/local/include -I/Users/jdemeyer/sage-4.7.rc0/local/include  -DNDEBUG -DOM_NDEBUG -DppcMac_darwin -DHAVE_CONFIG_H -c mpsr_Tok.cc
+libtool -dynamic -twolevel_namespace -weak_reference_mismatches weak -undefined dynamic_lookup -single_module -o libsingular.dylib \
+libsingular-tesths.o iparith.o mpsr_Tok.o claptmpl.o \
+grammar.o scanner.o attrib.o eigenval_ip.o extra.o fehelp.o feOpt.o ipassign.o ipconv.o ipid.o iplib.o ipprint.o ipshell.o lists.o sdb.o fglm.o interpolation.o silink.o subexpr.o janet.o wrapper.o libparse.o sing_win.o gms.o pcv.o maps_ip.o walk.o walk_ip.o cntrlc.o misc_ip.o calcSVD.o Minor.o MinorProcessor.o MinorInterface.o  slInit_Static.o mpsr_Put.o mpsr_PutPoly.o mpsr_GetPoly.o mpsr_sl.o mpsr_Get.o mpsr_GetMisc.o mpsr_Error.o  ndbm.o sing_dbm.o -lkernel -L../kernel -L../factory -L../libfac -L/Users/jdemeyer/sage-4.7.rc0/local/lib -lsingfac -lsingcf -lntl -lreadline -lgmp -lomalloc
+ld: for architecture ppc
+ld: ../kernel/libkernel.a(matpol.o) has external relocation entries in non-writable section (__TEXT,__text) for symbols:
+restFP
+saveFP
+libtool: internal link edit command failed
+make[3]: *** [libsingular] Error 1
+make[2]: *** [libsingular] Error 2
+Unable to build Singular.
+```
``````




---

archive/issue_comments_147860.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,7 +1,7 @@\n On my Mac OS X 10.4 PPC 32-bit system, singular (#11084) fails to compile with -O2 while it *does* work with -O3.  This is the relevant part from the log file:\n \n ```\n-g++ -O3 -g -fPIC -pipe -fno-implicit-templates -I. -I../kernel -I/Users/jdemeyer/sage-4.7.rc0/local/include -I/Users/jdemeyer/sage-4.7.rc0/local/include -I/Users/jdemeyer/sage-4.7.rc0/local/include  -DNDEBUG -DOM_NDEBUG -DppcMac_darwin -DHAVE_CONFIG_H -c mpsr_Tok.cc\n+g++ -O2 -g -fPIC -pipe -fno-implicit-templates -I. -I../kernel -I/Users/jdemeyer/sage-4.7.rc0/local/include -I/Users/jdemeyer/sage-4.7.rc0/local/include -I/Users/jdemeyer/sage-4.7.rc0/local/include  -DNDEBUG -DOM_NDEBUG -DppcMac_darwin -DHAVE_CONFIG_H -c mpsr_Tok.cc\n libtool -dynamic -twolevel_namespace -weak_reference_mismatches weak -undefined dynamic_lookup -single_module -o libsingular.dylib \\\n libsingular-tesths.o iparith.o mpsr_Tok.o claptmpl.o \\\n grammar.o scanner.o attrib.o eigenval_ip.o extra.o fehelp.o feOpt.o ipassign.o ipconv.o ipid.o iplib.o ipprint.o ipshell.o lists.o sdb.o fglm.o interpolation.o silink.o subexpr.o janet.o wrapper.o libparse.o sing_win.o gms.o pcv.o maps_ip.o walk.o walk_ip.o cntrlc.o misc_ip.o calcSVD.o Minor.o MinorProcessor.o MinorInterface.o  slInit_Static.o mpsr_Put.o mpsr_PutPoly.o mpsr_GetPoly.o mpsr_sl.o mpsr_Get.o mpsr_GetMisc.o mpsr_Error.o  ndbm.o sing_dbm.o -lkernel -L../kernel -L../factory -L../libfac -L/Users/jdemeyer/sage-4.7.rc0/local/lib -lsingfac -lsingcf -lntl -lreadline -lgmp -lomalloc\n``````\n",
    "created_at": "2011-04-30T21:17:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147860",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,7 +1,7 @@
 On my Mac OS X 10.4 PPC 32-bit system, singular (#11084) fails to compile with -O2 while it *does* work with -O3.  This is the relevant part from the log file:
 
 ```
-g++ -O3 -g -fPIC -pipe -fno-implicit-templates -I. -I../kernel -I/Users/jdemeyer/sage-4.7.rc0/local/include -I/Users/jdemeyer/sage-4.7.rc0/local/include -I/Users/jdemeyer/sage-4.7.rc0/local/include  -DNDEBUG -DOM_NDEBUG -DppcMac_darwin -DHAVE_CONFIG_H -c mpsr_Tok.cc
+g++ -O2 -g -fPIC -pipe -fno-implicit-templates -I. -I../kernel -I/Users/jdemeyer/sage-4.7.rc0/local/include -I/Users/jdemeyer/sage-4.7.rc0/local/include -I/Users/jdemeyer/sage-4.7.rc0/local/include  -DNDEBUG -DOM_NDEBUG -DppcMac_darwin -DHAVE_CONFIG_H -c mpsr_Tok.cc
 libtool -dynamic -twolevel_namespace -weak_reference_mismatches weak -undefined dynamic_lookup -single_module -o libsingular.dylib \
 libsingular-tesths.o iparith.o mpsr_Tok.o claptmpl.o \
 grammar.o scanner.o attrib.o eigenval_ip.o extra.o fehelp.o feOpt.o ipassign.o ipconv.o ipid.o iplib.o ipprint.o ipshell.o lists.o sdb.o fglm.o interpolation.o silink.o subexpr.o janet.o wrapper.o libparse.o sing_win.o gms.o pcv.o maps_ip.o walk.o walk_ip.o cntrlc.o misc_ip.o calcSVD.o Minor.o MinorProcessor.o MinorInterface.o  slInit_Static.o mpsr_Put.o mpsr_PutPoly.o mpsr_GetPoly.o mpsr_sl.o mpsr_Get.o mpsr_GetMisc.o mpsr_Error.o  ndbm.o sing_dbm.o -lkernel -L../kernel -L../factory -L../libfac -L/Users/jdemeyer/sage-4.7.rc0/local/lib -lsingfac -lsingcf -lntl -lreadline -lgmp -lomalloc
``````




---

archive/issue_comments_147861.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -14,3 +14,5 @@\n make[2]: *** [libsingular] Error 2\n Unable to build Singular.\n ```\n+\n+[Googling for restFP/saveFP](http://www.google.be/search?q=restFP+saveFP) seems to indicate it is a known problem when mixing compilers, see for example [http://lists.apple.com/archives/fortran-dev/2004/jul/msg00036.html](http://lists.apple.com/archives/fortran-dev/2004/jul/msg00036.html)\n``````\n",
    "created_at": "2011-04-30T21:29:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147861",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -14,3 +14,5 @@
 make[2]: *** [libsingular] Error 2
 Unable to build Singular.
 ```
+
+[Googling for restFP/saveFP](http://www.google.be/search?q=restFP+saveFP) seems to indicate it is a known problem when mixing compilers, see for example [http://lists.apple.com/archives/fortran-dev/2004/jul/msg00036.html](http://lists.apple.com/archives/fortran-dev/2004/jul/msg00036.html)
``````




---

archive/issue_comments_147862.json:
```json
{
    "body": "<a id='comment:5'></a>What version of gcc is this failing with? I assume it is not 4.6.0, but can you confirm that? \n\nThis error message reminds me of ones that existed with R and ECL on Solaris. The ECL bug has been fixed, but not the R bug (#9040), so R will not build 64-bit on Solaris x86 with gcc. The R bug is caused by a shared library being built with position dependent code. There are some conditions that can cause non-PIC code to be generated even when the flag -fPIC is given. \n\nI would check if -fPIC is being passed first. I had a long battle with the Singular developers to get them to accept that -fPIC should be used on Solaris. I think I'd convinced them on Linux too, but never tried on OS X. It could be that there's no -fPIC flag and it just happens this is ok with -O3 but not with -O2. \n\nTake a look at \n\nhttp://blogs.sun.com/rie/entry/my_relocations_don_t_fit\n\nwhich discusses this issue. It is a Sun blog aimed at Solaris, but the error message seems remarkably similar. Which makes me think this might be a PIC/non-PIC issue.\n\nTime for bed here - it is 0300 local time!!\n\ndave",
    "created_at": "2011-05-01T02:01:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147862",
    "user": "https://trac.sagemath.org/admin/accounts/users/drkirkby"
}
```

<a id='comment:5'></a>What version of gcc is this failing with? I assume it is not 4.6.0, but can you confirm that? 

This error message reminds me of ones that existed with R and ECL on Solaris. The ECL bug has been fixed, but not the R bug (#9040), so R will not build 64-bit on Solaris x86 with gcc. The R bug is caused by a shared library being built with position dependent code. There are some conditions that can cause non-PIC code to be generated even when the flag -fPIC is given. 

I would check if -fPIC is being passed first. I had a long battle with the Singular developers to get them to accept that -fPIC should be used on Solaris. I think I'd convinced them on Linux too, but never tried on OS X. It could be that there's no -fPIC flag and it just happens this is ok with -O3 but not with -O2. 

Take a look at 

http://blogs.sun.com/rie/entry/my_relocations_don_t_fit

which discusses this issue. It is a Sun blog aimed at Solaris, but the error message seems remarkably similar. Which makes me think this might be a PIC/non-PIC issue.

Time for bed here - it is 0300 local time!!

dave



---

archive/issue_comments_147863.json:
```json
{
    "body": "<a id='comment:6'></a>Dave, it's clear from reading various posts about this problem that it is an Apple-specific issue, so I doubt the Solaris stuff you linked to is relevant.\n\nI'm using these compilers:\n\n```\n$ gcc --version\npowerpc-apple-darwin8-gcc-4.0.1 (GCC) 4.0.1 (Apple Computer, Inc. build 5367)\nCopyright (C) 2005 Free Software Foundation, Inc.\nThis is free software; see the source for copying conditions.  There is NO\nwarranty; not even for MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.\n```\n\n```\n$ g++ --version\npowerpc-apple-darwin8-g++-4.0.1 (GCC) 4.0.1 (Apple Computer, Inc. build 5367)\nCopyright (C) 2005 Free Software Foundation, Inc.\nThis is free software; see the source for copying conditions.  There is NO\nwarranty; not even for MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.\n```",
    "created_at": "2011-05-01T09:37:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147863",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:6'></a>Dave, it's clear from reading various posts about this problem that it is an Apple-specific issue, so I doubt the Solaris stuff you linked to is relevant.

I'm using these compilers:

```
$ gcc --version
powerpc-apple-darwin8-gcc-4.0.1 (GCC) 4.0.1 (Apple Computer, Inc. build 5367)
Copyright (C) 2005 Free Software Foundation, Inc.
This is free software; see the source for copying conditions.  There is NO
warranty; not even for MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
```

```
$ g++ --version
powerpc-apple-darwin8-g++-4.0.1 (GCC) 4.0.1 (Apple Computer, Inc. build 5367)
Copyright (C) 2005 Free Software Foundation, Inc.
This is free software; see the source for copying conditions.  There is NO
warranty; not even for MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
```



---

archive/issue_comments_147864.json:
```json
{
    "body": "Changing status from new to needs_info.",
    "created_at": "2011-05-01T11:41:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147864",
    "user": "https://github.com/vbraun"
}
```

Changing status from new to needs_info.



---

archive/issue_comments_147865.json:
```json
{
    "body": "<a id='comment:7'></a>Google suggests that this is xcode-2.4.1. Can you try with a later xcode release? I think anything up to the latest xcode-3 supports PPC OSX 10.4.",
    "created_at": "2011-05-01T11:41:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147865",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:7'></a>Google suggests that this is xcode-2.4.1. Can you try with a later xcode release? I think anything up to the latest xcode-3 supports PPC OSX 10.4.



---

archive/issue_comments_147866.json:
```json
{
    "body": "<a id='comment:8'></a>Hi,\n\nfor Mac OS X 10.4 (Tiger), one should update to the latest OS version (10.4.11), and use the latest XCode version which is supported by OS X 10.4, this is XCode v2.5 (already some years old). It comes with a \"build 5370\" Apple version of GCC 4.0.1. (Any later XCode versions are not supported for OS X 10.4, AFAIR.)\n\nI'll give Sage-4.7.rc1 a try on both my OS X 10.4 MacIntel and MacPPC boxes, but the latter will literally need days to give results (it's got a 7440 G4 `@` 550MHz CPU).",
    "created_at": "2011-05-01T18:05:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147866",
    "user": "https://trac.sagemath.org/admin/accounts/users/GeorgSWeber"
}
```

<a id='comment:8'></a>Hi,

for Mac OS X 10.4 (Tiger), one should update to the latest OS version (10.4.11), and use the latest XCode version which is supported by OS X 10.4, this is XCode v2.5 (already some years old). It comes with a "build 5370" Apple version of GCC 4.0.1. (Any later XCode versions are not supported for OS X 10.4, AFAIR.)

I'll give Sage-4.7.rc1 a try on both my OS X 10.4 MacIntel and MacPPC boxes, but the latter will literally need days to give results (it's got a 7440 G4 `@` 550MHz CPU).



---

archive/issue_comments_147867.json:
```json
{
    "body": "<a id='comment:9'></a>Replying to [jdemeyer](#comment%3A6):\n> Dave, it's clear from reading various posts about this problem that it is an Apple-specific issue, so I doubt the Solaris stuff you linked to is relevant.\n\n\nI would still not dismiss the possibility that there might be non-PIC code being generated. \n\nDave",
    "created_at": "2011-05-01T18:32:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147867",
    "user": "https://trac.sagemath.org/admin/accounts/users/drkirkby"
}
```

<a id='comment:9'></a>Replying to [jdemeyer](#comment%3A6):
> Dave, it's clear from reading various posts about this problem that it is an Apple-specific issue, so I doubt the Solaris stuff you linked to is relevant.


I would still not dismiss the possibility that there might be non-PIC code being generated. 

Dave



---

archive/issue_comments_147868.json:
```json
{
    "body": "<a id='comment:10'></a>Replying to [GeorgSWeber](#comment%3A8):\n> I'll give Sage-4.7.rc1 a try on both my OS X 10.4 MacIntel and MacPPC boxes, but the latter will literally need days to give results (it's got a 7440 G4 `@` 550MHz CPU).\n\n\nThanks!  Those will be useful data points.",
    "created_at": "2011-05-02T10:19:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147868",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:10'></a>Replying to [GeorgSWeber](#comment%3A8):
> I'll give Sage-4.7.rc1 a try on both my OS X 10.4 MacIntel and MacPPC boxes, but the latter will literally need days to give results (it's got a 7440 G4 `@` 550MHz CPU).


Thanks!  Those will be useful data points.



---

archive/issue_comments_147869.json:
```json
{
    "body": "<a id='comment:11'></a>Just got in to work, I am now downloading and it will probably get to Singular sooner rather than later, we'll see.\n\nGeorg is correct that no version >2.5 is supported on 10.4.  I also only have build 5367/Xcode 2.4.1, but will see if I can track down Xcode 2.5 later - that might not mean immediately.",
    "created_at": "2011-05-02T12:56:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147869",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:11'></a>Just got in to work, I am now downloading and it will probably get to Singular sooner rather than later, we'll see.

Georg is correct that no version >2.5 is supported on 10.4.  I also only have build 5367/Xcode 2.4.1, but will see if I can track down Xcode 2.5 later - that might not mean immediately.



---

archive/issue_comments_147870.json:
```json
{
    "body": "<a id='comment:12'></a>On the system where the problem was discovered: -O0 also does work.\n\nSo here is a proposal: on \"old\" (4.0.x) versions of gcc: use -O3, otherwise -O2.  That would fix the problem on this ticket.",
    "created_at": "2011-05-02T15:24:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147870",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:12'></a>On the system where the problem was discovered: -O0 also does work.

So here is a proposal: on "old" (4.0.x) versions of gcc: use -O3, otherwise -O2.  That would fix the problem on this ticket.



---

archive/issue_comments_147871.json:
```json
{
    "body": "Changing status from needs_info to needs_review.",
    "created_at": "2011-05-03T08:19:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147871",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_info to needs_review.



---

archive/issue_comments_147872.json:
```json
{
    "body": "Changing author from \"\" to \"Jeroen Demeyer\"",
    "created_at": "2011-05-03T08:19:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147872",
    "user": "https://github.com/jdemeyer"
}
```

Changing author from "" to "Jeroen Demeyer"



---

archive/issue_comments_147873.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -16,3 +16,5 @@\n ```\n \n [Googling for restFP/saveFP](http://www.google.be/search?q=restFP+saveFP) seems to indicate it is a known problem when mixing compilers, see for example [http://lists.apple.com/archives/fortran-dev/2004/jul/msg00036.html](http://lists.apple.com/archives/fortran-dev/2004/jul/msg00036.html)\n+\n+New spkg using -O3 on gcc 4.0.x: [http://boxen.math.washington.edu/home/jdemeyer/spkg/singular-3-1-1-4.p9.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/singular-3-1-1-4.p9.spkg)\n``````\n",
    "created_at": "2011-05-03T08:19:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147873",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -16,3 +16,5 @@
 ```
 
 [Googling for restFP/saveFP](http://www.google.be/search?q=restFP+saveFP) seems to indicate it is a known problem when mixing compilers, see for example [http://lists.apple.com/archives/fortran-dev/2004/jul/msg00036.html](http://lists.apple.com/archives/fortran-dev/2004/jul/msg00036.html)
+
+New spkg using -O3 on gcc 4.0.x: [http://boxen.math.washington.edu/home/jdemeyer/spkg/singular-3-1-1-4.p9.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/singular-3-1-1-4.p9.spkg)
``````




---

archive/issue_comments_147874.json:
```json
{
    "body": "Diff for the singular spkg, for reviewing only",
    "created_at": "2011-05-03T08:20:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147874",
    "user": "https://github.com/jdemeyer"
}
```

Diff for the singular spkg, for reviewing only



---

archive/issue_comments_147875.json:
```json
{
    "body": "<a id='comment:14'></a>Attachment [singular-3-1-1-4.p8-p9.diff](tarball://root/attachments/some-uuid/ticket11278/singular-3-1-1-4.p8-p9.diff) by @kcrisman created at 2011-05-05 13:08:33\n\nJust a comment (hopefully I'll be able to review at all - even with the Xcode update, I still can't get past PolyBoRi) - any reason for checking this, rather than uname=Darwin and then the Xcode version/gcc version? Have we had any complaints on other platforms for gcc 4.0.x?",
    "created_at": "2011-05-05T13:08:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147875",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:14'></a>Attachment [singular-3-1-1-4.p8-p9.diff](tarball://root/attachments/some-uuid/ticket11278/singular-3-1-1-4.p8-p9.diff) by @kcrisman created at 2011-05-05 13:08:33

Just a comment (hopefully I'll be able to review at all - even with the Xcode update, I still can't get past PolyBoRi) - any reason for checking this, rather than uname=Darwin and then the Xcode version/gcc version? Have we had any complaints on other platforms for gcc 4.0.x?



---

archive/issue_comments_147876.json:
```json
{
    "body": "<a id='comment:15'></a>Replying to [kcrisman](#comment%3A14):\n> Have we had any complaints on other platforms for gcc 4.0.x?\n\n\nThere were no complaints on other platforms as far as I know.  However, you can consider the situation with gcc 4.0.x a status-quo: it used to be -O3 before rc0, and it is now again -O3.  Since there were no problems with gcc 4.0.x before rc0, I don't think it is a problem to use -O3 with gcc 4.0.x on all platforms.",
    "created_at": "2011-05-05T13:43:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147876",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:15'></a>Replying to [kcrisman](#comment%3A14):
> Have we had any complaints on other platforms for gcc 4.0.x?


There were no complaints on other platforms as far as I know.  However, you can consider the situation with gcc 4.0.x a status-quo: it used to be -O3 before rc0, and it is now again -O3.  Since there were no problems with gcc 4.0.x before rc0, I don't think it is a problem to use -O3 with gcc 4.0.x on all platforms.



---

archive/issue_comments_147877.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Karl-Dieter Crisman\"",
    "created_at": "2011-05-05T20:03:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147877",
    "user": "https://github.com/kcrisman"
}
```

Changing reviewer from "" to "Karl-Dieter Crisman"



---

archive/issue_comments_147878.json:
```json
{
    "body": "<a id='comment:16'></a>Replying to [jdemeyer](#comment%3A15):\n> Replying to [kcrisman](#comment%3A14):\n> > Have we had any complaints on other platforms for gcc 4.0.x?\n\n> \n> There were no complaints on other platforms as far as I know.  However, you can consider the situation with gcc 4.0.x a status-quo: it used to be -O3 before rc0, and it is now again -O3.  Since there were no problems with gcc 4.0.x before rc0, I don't think it is a problem to use -O3 with gcc 4.0.x on all platforms.\n\n\nOkay, that makes sense.  Positive review on the diff, and presumably this would fix it if it worked before; however, it will still take a while until I can test it because of the unrelated build issue mentioned above, so if someone else beats me to it, by all means. \n\nI am going to try to build rc0, not rc1; if I can get *that* to build up to Singular, then replace with this, I think that would be enough for positive review.",
    "created_at": "2011-05-05T20:03:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147878",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:16'></a>Replying to [jdemeyer](#comment%3A15):
> Replying to [kcrisman](#comment%3A14):
> > Have we had any complaints on other platforms for gcc 4.0.x?

> 
> There were no complaints on other platforms as far as I know.  However, you can consider the situation with gcc 4.0.x a status-quo: it used to be -O3 before rc0, and it is now again -O3.  Since there were no problems with gcc 4.0.x before rc0, I don't think it is a problem to use -O3 with gcc 4.0.x on all platforms.


Okay, that makes sense.  Positive review on the diff, and presumably this would fix it if it worked before; however, it will still take a while until I can test it because of the unrelated build issue mentioned above, so if someone else beats me to it, by all means. 

I am going to try to build rc0, not rc1; if I can get *that* to build up to Singular, then replace with this, I think that would be enough for positive review.



---

archive/issue_comments_147879.json:
```json
{
    "body": "<a id='comment:17'></a>Replying to [kcrisman](#comment%3A16):\n> I am going to try to build rc0, not rc1; if I can get *that* to build up to Singular, then replace with this, I think that would be enough for positive review. \n\n\nHow is it working out?",
    "created_at": "2011-05-08T10:22:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147879",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:17'></a>Replying to [kcrisman](#comment%3A16):
> I am going to try to build rc0, not rc1; if I can get *that* to build up to Singular, then replace with this, I think that would be enough for positive review. 


How is it working out?



---

archive/issue_comments_147880.json:
```json
{
    "body": "<a id='comment:18'></a>Replying to [jdemeyer](#comment%3A17):\n> Replying to [kcrisman](#comment%3A16):\n> > I am going to try to build rc0, not rc1; if I can get *that* to build up to Singular, then replace with this, I think that would be enough for positive review. \n\n> \n> How is it working out?\n\n\nI think I said somewhere on the rc1 sage-release list that rc0 failed at the same place in PolyBoRi.  I will try to look into that more (the sed thing) when I'm done with classes today, but until then there isn't much I can do with this, except to ./sage -f it into an alpha5 and check that it builds.   \n\nMaybe I could try to ./sage -f the 'bad' one into my alpha5, check it fails, and then ./sage -f this one into my alpha5, check that it builds.  Would that be sufficient?  I think I *do* have the ability to try that.",
    "created_at": "2011-05-09T12:59:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147880",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:18'></a>Replying to [jdemeyer](#comment%3A17):
> Replying to [kcrisman](#comment%3A16):
> > I am going to try to build rc0, not rc1; if I can get *that* to build up to Singular, then replace with this, I think that would be enough for positive review. 

> 
> How is it working out?


I think I said somewhere on the rc1 sage-release list that rc0 failed at the same place in PolyBoRi.  I will try to look into that more (the sed thing) when I'm done with classes today, but until then there isn't much I can do with this, except to ./sage -f it into an alpha5 and check that it builds.   

Maybe I could try to ./sage -f the 'bad' one into my alpha5, check it fails, and then ./sage -f this one into my alpha5, check that it builds.  Would that be sufficient?  I think I *do* have the ability to try that.



---

archive/issue_comments_147881.json:
```json
{
    "body": "<a id='comment:19'></a>Replying to [kcrisman](#comment%3A18):\n> Maybe I could try to ./sage -f the 'bad' one into my alpha5, check it fails, and then ./sage -f this one into my alpha5, check that it builds.  Would that be sufficient?  I think I *do* have the ability to try that.\n\n\nLet's try that then.",
    "created_at": "2011-05-09T21:43:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147881",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:19'></a>Replying to [kcrisman](#comment%3A18):
> Maybe I could try to ./sage -f the 'bad' one into my alpha5, check it fails, and then ./sage -f this one into my alpha5, check that it builds.  Would that be sufficient?  I think I *do* have the ability to try that.


Let's try that then.



---

archive/issue_comments_147882.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2011-05-11T14:30:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147882",
    "user": "https://github.com/kcrisman"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_147883.json:
```json
{
    "body": "<a id='comment:20'></a>Replying to [jdemeyer](#comment%3A19):\n> Replying to [kcrisman](#comment%3A18):\n> > Maybe I could try to ./sage -f the 'bad' one into my alpha5, check it fails, and then ./sage -f this one into my alpha5, check that it builds.  Would that be sufficient?  I think I *do* have the ability to try that.\n\n> \n> Let's try that then.\n\n\nSuccess!  Failed with p8, worked with p9.  I'm now waiting on ./sage -b, but don't anticipate any problems if you didn't have any.  I'll revert the positive review if I do come up with any problems, otherwise it's good to go.",
    "created_at": "2011-05-11T14:30:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147883",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:20'></a>Replying to [jdemeyer](#comment%3A19):
> Replying to [kcrisman](#comment%3A18):
> > Maybe I could try to ./sage -f the 'bad' one into my alpha5, check it fails, and then ./sage -f this one into my alpha5, check that it builds.  Would that be sufficient?  I think I *do* have the ability to try that.

> 
> Let's try that then.


Success!  Failed with p8, worked with p9.  I'm now waiting on ./sage -b, but don't anticipate any problems if you didn't have any.  I'll revert the positive review if I do come up with any problems, otherwise it's good to go.



---

archive/issue_comments_147884.json:
```json
{
    "body": "<a id='comment:21'></a>Yeah, relevant tests are all passing.  Good sleuthing.",
    "created_at": "2011-05-11T15:21:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147884",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:21'></a>Yeah, relevant tests are all passing.  Good sleuthing.



---

archive/issue_comments_147885.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2011-05-11T19:15:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147885",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed



---

archive/issue_comments_147886.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-4.7.rc2\"",
    "created_at": "2011-05-11T19:15:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147886",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-4.7.rc2"



---

archive/issue_events_029213.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-11T19:15:48Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11278#event-29213"
}
```



---

archive/issue_comments_147887.json:
```json
{
    "body": "<a id='comment:23'></a>Sorry for the delay, just to let you know:\n\nOn my MacPPC system, OS X 10.4.11 with XCode 2.5, with an older G4 CPU with 550 MHz and 768 MB RAM, Sage-4.7.rc2 (with singular 3-1-1-4.p9) builds fine! For some (more or less unrelated) details about polybori, see my comments trac #11331.",
    "created_at": "2011-05-14T20:57:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11278",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11278#issuecomment-147887",
    "user": "https://trac.sagemath.org/admin/accounts/users/GeorgSWeber"
}
```

<a id='comment:23'></a>Sorry for the delay, just to let you know:

On my MacPPC system, OS X 10.4.11 with XCode 2.5, with an older G4 CPU with 550 MHz and 768 MB RAM, Sage-4.7.rc2 (with singular 3-1-1-4.p9) builds fine! For some (more or less unrelated) details about polybori, see my comments trac #11331.
