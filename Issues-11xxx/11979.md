...
```json
{
    "body": "here is a patch for the first file. To be continued",
...
```json
{
    "body": "Attachment [trac_11979_divided_power.patch](tarball://root/attachments/some-uuid/ticket11979/trac_11979_divided_power.patch) by @fchapoton created at 2013-08-21 07:44:31\n\nHere is a patch including both files",
...
```json
{
    "body": "Minor fixes made. Sorry, not a review. Don't have the time or skills for that (an example should highlight all that one needs to know about the respective categories; I'm not the right person to judge...).\n\nPlease note the branch change!\n\n---\nNew commits:",
...
```json
{
    "body": "I have corrected the doctest, and also cleaned a little bit (up to pep8 standard).",
...
```json
{
    "body": "Actually these imports that caused the conflicts were probably temporary, made for easier testing. The examples in the doctests all begin with an import instruction, so I suspect these algebras were not supposed to be in the global scope. So I've removed them now. Was this a good idea?",
...
```json
{
    "body": "Looks like there are some failed tests and triggered deprecation warnings...",
    "created_at": "2017-02-05T05:58:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11979",
    "type": "issue_comment",
