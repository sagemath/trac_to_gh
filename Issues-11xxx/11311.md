# Issue 11311: engine="pdflatex" in view is ignored

archive/issues_011139.json:
```json
{
    "body": "The view command states that the pdflatex argument is deprecated and that one should use the engine argument instead.\n\nBut if one sets engine=\"pdflatex\", it is overwritten, and so one currently needs to use the deprecated pdflatex option.\n\n---\n\nApply [attachment:trac_11311-5.0.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311-5.0.patch).\n\n**Assignee:** @jasongrout, @williamstein\n\n**CC:**  @jhpalmieri\n\n**Keywords:** latex, pdflatex, days30\n\n**Reviewer:** Karl-Dieter Crisman, Franco Saliola, John Palmieri\n\n**Author:** Franco Saliola, John Palmieri\n\n**Merged:** sage-5.1.beta4\n\nIssue created by migration from https://trac.sagemath.org/ticket/11311\n\n",
    "closed_at": "2012-06-14T06:38:04Z",
    "created_at": "2011-05-08T03:40:16Z",
    "labels": [
        "component: graphics",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.1",
    "title": "engine=\"pdflatex\" in view is ignored",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/11311",
    "user": "https://github.com/saliola"
}
```
The view command states that the pdflatex argument is deprecated and that one should use the engine argument instead.

But if one sets engine="pdflatex", it is overwritten, and so one currently needs to use the deprecated pdflatex option.

---

Apply [attachment:trac_11311-5.0.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311-5.0.patch).

**Assignee:** @jasongrout, @williamstein

**CC:**  @jhpalmieri

**Keywords:** latex, pdflatex, days30

**Reviewer:** Karl-Dieter Crisman, Franco Saliola, John Palmieri

**Author:** Franco Saliola, John Palmieri

**Merged:** sage-5.1.beta4

Issue created by migration from https://trac.sagemath.org/ticket/11311





---

archive/issue_events_088671.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2011-05-08T04:33:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11311#event-88671"
}
```



---

archive/attachments_015350.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_11311_engine_pdflatex_view-fs.patch",
    "asset_url": "tarball://root/attachments/ticket11311/trac_11311_engine_pdflatex_view-fs.patch",
    "created_at": "2011-05-08T04:33:25Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket11311/trac_11311_engine_pdflatex_view-fs.patch",
    "user": "https://github.com/saliola"
}
```



---

archive/issue_comments_115652.json:
```json
{
    "body": "<a id='comment:1'></a>\n**Attachment:** [trac_11311_engine_pdflatex_view-fs.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311_engine_pdflatex_view-fs.patch)",
    "created_at": "2011-05-08T04:33:25Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115652",
    "user": "https://github.com/saliola"
}
```

<a id='comment:1'></a>
**Attachment:** [trac_11311_engine_pdflatex_view-fs.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311_engine_pdflatex_view-fs.patch)



---

archive/issue_comments_115653.json:
```json
{
    "body": "**Changing keywords** from \"latex, pdflatex\" to \"latex, pdflatex, days30\".",
    "created_at": "2011-05-08T18:29:12Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115653",
    "user": "https://github.com/saliola"
}
```

**Changing keywords** from "latex, pdflatex" to "latex, pdflatex, days30".



---

archive/issue_comments_115654.json:
```json
{
    "body": "**Author:** Franco Saliola",
    "created_at": "2011-05-08T19:41:16Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115654",
    "user": "https://github.com/saliola"
}
```

**Author:** Franco Saliola



---

archive/issue_comments_115655.json:
```json
{
    "body": "<a id='comment:5'></a>\nYou should modify the header of your patch : replace\n\ntrac_11311: fix view to not overwrite engine=\"pdflatex\"\n\nby\n\n#11311: fix view to not overwrite engine=\"pdflatex\"\n\nThen the buildbot will be more happy.",
    "created_at": "2011-06-01T13:34:11Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115655",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:5'></a>
You should modify the header of your patch : replace

trac_11311: fix view to not overwrite engine="pdflatex"

by

#11311: fix view to not overwrite engine="pdflatex"

Then the buildbot will be more happy.



---

archive/attachments_015351.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_11311_engine_pdflatex_view-fs.2.patch",
    "asset_url": "tarball://root/attachments/ticket11311/trac_11311_engine_pdflatex_view-fs.2.patch",
    "created_at": "2011-06-01T15:49:39Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket11311/trac_11311_engine_pdflatex_view-fs.2.patch",
    "user": "https://github.com/saliola"
}
```



---

archive/issue_comments_115656.json:
```json
{
    "body": "<a id='comment:6'></a>\n**Attachment:** [trac_11311_engine_pdflatex_view-fs.2.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311_engine_pdflatex_view-fs.2.patch)\n\nThanks for catching this. I corrected this in the new patch.",
    "created_at": "2011-06-01T15:49:39Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115656",
    "user": "https://github.com/saliola"
}
```

<a id='comment:6'></a>
**Attachment:** [trac_11311_engine_pdflatex_view-fs.2.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311_engine_pdflatex_view-fs.2.patch)

Thanks for catching this. I corrected this in the new patch.



---

archive/issue_comments_115657.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,3 +1,5 @@\n The view command states that the pdflatex argument is deprecated and that one should use the engine argument instead.\n \n But if one sets engine=\"pdflatex\", it is overwritten, and so one currently needs to use the deprecated pdflatex option.\n+\n+Apply trac_11311_engine_pdflatex_view-fs.2.patch\n``````\n",
    "created_at": "2011-06-01T15:49:39Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115657",
    "user": "https://github.com/saliola"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,3 +1,5 @@
 The view command states that the pdflatex argument is deprecated and that one should use the engine argument instead.
 
 But if one sets engine="pdflatex", it is overwritten, and so one currently needs to use the deprecated pdflatex option.
+
+Apply trac_11311_engine_pdflatex_view-fs.2.patch
``````




---

archive/issue_comments_115658.json:
```json
{
    "body": "<a id='comment:7'></a>\nApply [attachment:trac_11311_engine_pdflatex_view-fs.2.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311_engine_pdflatex_view-fs.2.patch).",
    "created_at": "2011-06-16T03:44:42Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115658",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:7'></a>
Apply [attachment:trac_11311_engine_pdflatex_view-fs.2.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311_engine_pdflatex_view-fs.2.patch).



---

archive/issue_comments_115659.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -2,4 +2,6 @@\n \n But if one sets engine=\"pdflatex\", it is overwritten, and so one currently needs to use the deprecated pdflatex option.\n \n-Apply trac_11311_engine_pdflatex_view-fs.2.patch\n+---\n+\n+Apply [attachment:trac_11311_engine_pdflatex_view-fs.2.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311_engine_pdflatex_view-fs.2.patch).\n``````\n",
    "created_at": "2011-06-16T03:44:42Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115659",
    "user": "https://github.com/kcrisman"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -2,4 +2,6 @@
 
 But if one sets engine="pdflatex", it is overwritten, and so one currently needs to use the deprecated pdflatex option.
 
-Apply trac_11311_engine_pdflatex_view-fs.2.patch
+---
+
+Apply [attachment:trac_11311_engine_pdflatex_view-fs.2.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311_engine_pdflatex_view-fs.2.patch).
``````




---

archive/issue_comments_115660.json:
```json
{
    "body": "<a id='comment:8'></a>\nFranco, you need to rebase this against your own patch at #11315 which was recently merged :)\n\nI may do this if the patch behaves as promised.",
    "created_at": "2011-06-16T03:48:11Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115660",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:8'></a>
Franco, you need to rebase this against your own patch at #11315 which was recently merged :)

I may do this if the patch behaves as promised.



---

archive/issue_comments_115661.json:
```json
{
    "body": "<a id='comment:9'></a>\nQuestion:  when I do\n\n```\nsage: view(2,engine='pdflatex')\n```\nit opens in my pdf viewer, not in my TeX program.  This seems wrong, based on the current doc, but I could be wrong. \n\nI'm also attaching a rebased patch.",
    "created_at": "2011-06-16T04:11:35Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115661",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:9'></a>
Question:  when I do

```
sage: view(2,engine='pdflatex')
```
it opens in my pdf viewer, not in my TeX program.  This seems wrong, based on the current doc, but I could be wrong. 

I'm also attaching a rebased patch.



---

archive/issue_comments_115662.json:
```json
{
    "body": "**Reviewer:** Karl-Dieter Crisman",
    "created_at": "2011-06-16T04:11:35Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115662",
    "user": "https://github.com/kcrisman"
}
```

**Reviewer:** Karl-Dieter Crisman



---

archive/issue_events_088672.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-06-16T04:11:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11311#event-88672"
}
```



---

archive/issue_events_088673.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-06-16T04:11:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "label": "needs info",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11311#event-88673"
}
```



---

archive/attachments_015352.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_11311-rebased.patch",
    "asset_url": "tarball://root/attachments/ticket11311/trac_11311-rebased.patch",
    "created_at": "2011-06-16T04:13:05Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket11311/trac_11311-rebased.patch",
    "user": "https://github.com/kcrisman"
}
```



---

archive/issue_comments_115663.json:
```json
{
    "body": "<a id='comment:10'></a>\n**Attachment:** [trac_11311-rebased.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311-rebased.patch)\n\nApply [attachment:trac_11311-rebased.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311-rebased.patch).",
    "created_at": "2011-06-16T04:13:05Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115663",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:10'></a>
**Attachment:** [trac_11311-rebased.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311-rebased.patch)

Apply [attachment:trac_11311-rebased.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311-rebased.patch).



---

archive/issue_comments_115664.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -4,4 +4,4 @@\n \n ---\n \n-Apply [attachment:trac_11311_engine_pdflatex_view-fs.2.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311_engine_pdflatex_view-fs.2.patch).\n+Apply [attachment:trac_11311-rebased.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311-rebased.patch).\n``````\n",
    "created_at": "2011-06-16T04:13:05Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115664",
    "user": "https://github.com/kcrisman"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -4,4 +4,4 @@
 
 ---
 
-Apply [attachment:trac_11311_engine_pdflatex_view-fs.2.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311_engine_pdflatex_view-fs.2.patch).
+Apply [attachment:trac_11311-rebased.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311-rebased.patch).
``````




---

archive/issue_comments_115665.json:
```json
{
    "body": "<a id='comment:11'></a>\nApply trac_11311-rebased.patch",
    "created_at": "2011-06-22T08:08:52Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115665",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:11'></a>
Apply trac_11311-rebased.patch



---

archive/issue_comments_115666.json:
```json
{
    "body": "<a id='comment:12'></a>\nWith the current patch, we have a little code duplication, so we could make the following change:\n\n```diff\ndiff --git a/sage/misc/latex.py b/sage/misc/latex.py\n--- a/sage/misc/latex.py\n+++ b/sage/misc/latex.py\n@@ -1851,6 +1851,10 @@ def view(objects, title='SAGE', debug=Fa\n         else:\n             latex_options = {}\n         s = _latex_file_(objects, title=title, sep=sep, tiny=tiny, debug=debug, **latex_options)\n+    if engine is None:\n+        engine = _Latex_prefs._option[\"engine\"]\n+    if pdflatex or (viewer == \"pdf\" and engine == \"latex\"):\n+        engine = \"pdflatex\"\n     # notebook\n     if EMBEDDED_MODE and viewer is None:\n         jsMath_okay = True\n@@ -1862,10 +1866,6 @@ def view(objects, title='SAGE', debug=Fa\n         if jsMath_okay:\n             print JSMath().eval(objects, mode=mode)  # put comma at end of line?\n         else:\n-            if engine is None:\n-                engine = _Latex_prefs._option[\"engine\"]\n-            if pdflatex or (viewer == \"pdf\" and engine == \"latex\"):\n-                engine = \"pdflatex\"\n             base_dir = os.path.abspath(\"\")\n             png_file = graphics_filename(ext='png')\n             png_link = \"cell://\" + png_file\n@@ -1874,10 +1874,6 @@ def view(objects, title='SAGE', debug=Fa\n             print '<html><img src=\"%s\"></html>'%png_link  # put comma at end of line?\n         return\n     # command line or notebook with viewer\n-    if engine is None:\n-        engine = _Latex_prefs._option[\"engine\"]\n-    if pdflatex or (viewer == \"pdf\" and engine == \"latex\"):\n-        engine = \"pdflatex\"\n     tmp = tmp_dir('sage_viewer')\n     tex_file = os.path.join(tmp, \"sage.tex\")\n     open(tex_file,'w').write(s)\n```\nOtherwise, I think it looks good.\n\nAs far as opening in a pdf viewer rather than in a TeX program, hasn't that been the behavior for a while?  (Sage basically opens up the file, blah.pdf or blah.dvi, using some sensible default depending on the OS -- see the file misc/viewer.py.  On OS X, for example, this just calls \"open blah.pdf\" or \"open blah.dvi\", depending on which file is produced.)",
    "created_at": "2011-06-29T21:32:57Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115666",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:12'></a>
With the current patch, we have a little code duplication, so we could make the following change:

```diff
diff --git a/sage/misc/latex.py b/sage/misc/latex.py
--- a/sage/misc/latex.py
+++ b/sage/misc/latex.py
@@ -1851,6 +1851,10 @@ def view(objects, title='SAGE', debug=Fa
         else:
             latex_options = {}
         s = _latex_file_(objects, title=title, sep=sep, tiny=tiny, debug=debug, **latex_options)
+    if engine is None:
+        engine = _Latex_prefs._option["engine"]
+    if pdflatex or (viewer == "pdf" and engine == "latex"):
+        engine = "pdflatex"
     # notebook
     if EMBEDDED_MODE and viewer is None:
         jsMath_okay = True
@@ -1862,10 +1866,6 @@ def view(objects, title='SAGE', debug=Fa
         if jsMath_okay:
             print JSMath().eval(objects, mode=mode)  # put comma at end of line?
         else:
-            if engine is None:
-                engine = _Latex_prefs._option["engine"]
-            if pdflatex or (viewer == "pdf" and engine == "latex"):
-                engine = "pdflatex"
             base_dir = os.path.abspath("")
             png_file = graphics_filename(ext='png')
             png_link = "cell://" + png_file
@@ -1874,10 +1874,6 @@ def view(objects, title='SAGE', debug=Fa
             print '<html><img src="%s"></html>'%png_link  # put comma at end of line?
         return
     # command line or notebook with viewer
-    if engine is None:
-        engine = _Latex_prefs._option["engine"]
-    if pdflatex or (viewer == "pdf" and engine == "latex"):
-        engine = "pdflatex"
     tmp = tmp_dir('sage_viewer')
     tex_file = os.path.join(tmp, "sage.tex")
     open(tex_file,'w').write(s)
```
Otherwise, I think it looks good.

As far as opening in a pdf viewer rather than in a TeX program, hasn't that been the behavior for a while?  (Sage basically opens up the file, blah.pdf or blah.dvi, using some sensible default depending on the OS -- see the file misc/viewer.py.  On OS X, for example, this just calls "open blah.pdf" or "open blah.dvi", depending on which file is produced.)



---

archive/issue_comments_115667.json:
```json
{
    "body": "<a id='comment:13'></a>\nI'm attaching a referee patch which removes the code duplication referred to above.\n\nKarl-Dieter: the program \"sage-open\", as defined in sage/misc/viewer.py, should be used to open the output file.  Before the patch here, it was called with `output_file` set to \"sage.dvi\" or something like that.  After the patch, it is called with `output_file` set to \"sage.pdf\".  So any difference in what program is being used is because this is producing a pdf file, not a dvi file.\n\nI'd like to change this from \"needs info\" to \"positive review\".   Is that okay?",
    "created_at": "2011-10-26T21:23:54Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115667",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:13'></a>
I'm attaching a referee patch which removes the code duplication referred to above.

Karl-Dieter: the program "sage-open", as defined in sage/misc/viewer.py, should be used to open the output file.  Before the patch here, it was called with `output_file` set to "sage.dvi" or something like that.  After the patch, it is called with `output_file` set to "sage.pdf".  So any difference in what program is being used is because this is producing a pdf file, not a dvi file.

I'd like to change this from "needs info" to "positive review".   Is that okay?



---

archive/issue_comments_115668.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -4,4 +4,4 @@\n \n ---\n \n-Apply [attachment:trac_11311-rebased.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311-rebased.patch).\n+Apply [[attachment:trac_11311-ref.patch](https://github.com/sagemath/sage/files/ticket11311/7ee749641223cd2460971c5819e00012.patch](https://github.com/sagemath/sage/files/ticket11311/c036bc7f8e1f6e08fc32b3d95a97b086.patch),).\n``````\n",
    "created_at": "2011-10-26T21:23:54Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115668",
    "user": "https://github.com/jhpalmieri"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -4,4 +4,4 @@
 
 ---
 
-Apply [attachment:trac_11311-rebased.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311-rebased.patch).
+Apply [[attachment:trac_11311-ref.patch](https://github.com/sagemath/sage/files/ticket11311/7ee749641223cd2460971c5819e00012.patch](https://github.com/sagemath/sage/files/ticket11311/c036bc7f8e1f6e08fc32b3d95a97b086.patch),).
``````




---

archive/issue_events_088674.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2011-10-26T21:30:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "label": "needs info",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11311#event-88674"
}
```



---

archive/issue_events_088675.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2011-10-26T21:30:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11311#event-88675"
}
```



---

archive/issue_comments_115669.json:
```json
{
    "body": "<a id='comment:14'></a>\nLooks good to me.",
    "created_at": "2011-10-26T21:30:15Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115669",
    "user": "https://github.com/saliola"
}
```

<a id='comment:14'></a>
Looks good to me.



---

archive/issue_events_088676.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2011-10-26T21:30:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11311#event-88676"
}
```



---

archive/issue_events_088677.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2011-10-26T21:30:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11311#event-88677"
}
```



---

archive/issue_events_088678.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-10-27T08:10:16Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "milestone": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11311#event-88678"
}
```



---

archive/issue_events_088679.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-10-27T08:10:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "milestone": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11311#event-88679"
}
```



---

archive/issue_comments_115670.json:
```json
{
    "body": "**Changing author** from \"Franco Saliola\" to \"Franco Saliola, John Palmieri\".",
    "created_at": "2011-10-27T15:28:22Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115670",
    "user": "https://github.com/kcrisman"
}
```

**Changing author** from "Franco Saliola" to "Franco Saliola, John Palmieri".



---

archive/issue_comments_115671.json:
```json
{
    "body": "**Changing reviewer** from \"Karl-Dieter Crisman\" to \"Karl-Dieter Crisman, Franco Saliola\".",
    "created_at": "2011-10-27T15:28:22Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115671",
    "user": "https://github.com/kcrisman"
}
```

**Changing reviewer** from "Karl-Dieter Crisman" to "Karl-Dieter Crisman, Franco Saliola".



---

archive/issue_comments_115672.json:
```json
{
    "body": "**Changing reviewer** from \"Karl-Dieter Crisman, Franco Saliola\" to \"Karl-Dieter Crisman, Franco Saliola, John Palmieri\".",
    "created_at": "2011-10-27T16:59:53Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115672",
    "user": "https://github.com/jhpalmieri"
}
```

**Changing reviewer** from "Karl-Dieter Crisman, Franco Saliola" to "Karl-Dieter Crisman, Franco Saliola, John Palmieri".



---

archive/issue_comments_115673.json:
```json
{
    "body": "Milestone sage-4.7.3 deleted",
    "created_at": "2011-11-03T16:14:43Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115673",
    "user": "https://github.com/jdemeyer"
}
```

Milestone sage-4.7.3 deleted



---

archive/issue_events_088680.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-03T16:14:43Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "milestone": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11311#event-88680"
}
```



---

archive/issue_events_088681.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-07T10:11:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11311#event-88681"
}
```



---

archive/issue_events_088682.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-07T10:11:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11311#event-88682"
}
```



---

archive/issue_events_088683.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-07T10:11:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "milestone": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11311#event-88683"
}
```



---

archive/issue_comments_115674.json:
```json
{
    "body": "**Merged:** sage-4.8.alpha1",
    "created_at": "2011-11-07T10:11:13Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115674",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-4.8.alpha1



---

archive/issue_comments_115675.json:
```json
{
    "body": "<a id='comment:21'></a>\nI get doctest failures on a system which does not have `latex` installed:\n\n```\nsage -t  -long -force_lib devel/sage/sage/misc/latex.py\n**********************************************************************\nFile \"/Users/jdemeyer/sage-4.8.alpha1/devel/sage-main/sage/misc/latex.py\", line 1877:\n    sage: view(4, engine=\"garbage\")\nExpected:\n    Traceback (most recent call last):\n    ...\n    ValueError: Unsupported LaTeX engine.\nGot:\n    Error: XeLaTeX does not seem to be installed.  Download it from\n    ctan.org and try again.\n    Latex error\n**********************************************************************\nFile \"/Users/jdemeyer/sage-4.8.alpha1/devel/sage-main/sage/misc/latex.py\", line 1882:\n    sage: view(4, engine=\"garbage\", viewer=\"pdf\")\nExpected:\n    Traceback (most recent call last):\n    ...\n    ValueError: Unsupported LaTeX engine.\nGot:\n    Error: XeLaTeX does not seem to be installed.  Download it from\n    ctan.org and try again.\n    Latex error\n**********************************************************************\n```\n\nFixing this would mean adding \"# optional - latex\" to the doctest.\n\nBut it also makes me wonder: why check the existence of XeLaTeX when `engine=\"garbage\"`?  This doesn't make much sense to me.",
    "created_at": "2011-11-08T15:02:47Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115675",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:21'></a>
I get doctest failures on a system which does not have `latex` installed:

```
sage -t  -long -force_lib devel/sage/sage/misc/latex.py
**********************************************************************
File "/Users/jdemeyer/sage-4.8.alpha1/devel/sage-main/sage/misc/latex.py", line 1877:
    sage: view(4, engine="garbage")
Expected:
    Traceback (most recent call last):
    ...
    ValueError: Unsupported LaTeX engine.
Got:
    Error: XeLaTeX does not seem to be installed.  Download it from
    ctan.org and try again.
    Latex error
**********************************************************************
File "/Users/jdemeyer/sage-4.8.alpha1/devel/sage-main/sage/misc/latex.py", line 1882:
    sage: view(4, engine="garbage", viewer="pdf")
Expected:
    Traceback (most recent call last):
    ...
    ValueError: Unsupported LaTeX engine.
Got:
    Error: XeLaTeX does not seem to be installed.  Download it from
    ctan.org and try again.
    Latex error
**********************************************************************
```

Fixing this would mean adding "# optional - latex" to the doctest.

But it also makes me wonder: why check the existence of XeLaTeX when `engine="garbage"`?  This doesn't make much sense to me.



---

archive/issue_comments_115676.json:
```json
{
    "body": "**Changing merged** from \"sage-4.8.alpha1\" to \"\".",
    "created_at": "2011-11-08T15:02:47Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115676",
    "user": "https://github.com/jdemeyer"
}
```

**Changing merged** from "sage-4.8.alpha1" to "".



---

archive/issue_comments_115677.json:
```json
{
    "body": "<a id='comment:22'></a>\nThis seems to have been introduced (or expanded, at least) in #8486.\n\n```\n    if engine and not have_pdflatex() and not have_xelatex(): \n \t        print \"Error: %s does not seem to be installed.  Download it from\" % _Latex_prefs._option[\"engine_name\"] \n```\nso `engine` is certainly `True` since it's nonempty, and we definitely don't have any of those things.  And it first checks whether one has LaTeX, before it checks the engines.  \n\nNow we have \n\n```\nsage: sage.misc.latex._Latex_prefs._option\n{'engine': 'latex', 'matrix_delimiters': ['(', ')'], 'jsmath_avoid': [], 'engine_name': 'LaTeX', 'blackboard_bold': False, 'macros': '', 'vector_delimiters': ['(', ')'], 'preamble': ''}\n```\nin a regular install.  The question is whether there is anywhere `engine` gets reset...  \n\nAha!  [Line 1404](http://hg.sagemath.org/sage-main/file/9e29a3d84c48/sage/misc/latex.py#l1404) (of the current 4.7.2) shows that this is set to `xelatex` and then not reset.   Apparently the doctest framework doesn't clear this, I'm not sure exactly where this all lives.\n\nSolution should be:\n* Fix the reset of the engine\n* Leave the error message and `# random` it.   Or even include both error messages so that the documentation shows exactly what we expect with and without a working LaTeX installation.\n\nWhat do you think?",
    "created_at": "2011-11-08T16:00:32Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115677",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:22'></a>
This seems to have been introduced (or expanded, at least) in #8486.

```
    if engine and not have_pdflatex() and not have_xelatex(): 
 	        print "Error: %s does not seem to be installed.  Download it from" % _Latex_prefs._option["engine_name"] 
```
so `engine` is certainly `True` since it's nonempty, and we definitely don't have any of those things.  And it first checks whether one has LaTeX, before it checks the engines.  

Now we have 

```
sage: sage.misc.latex._Latex_prefs._option
{'engine': 'latex', 'matrix_delimiters': ['(', ')'], 'jsmath_avoid': [], 'engine_name': 'LaTeX', 'blackboard_bold': False, 'macros': '', 'vector_delimiters': ['(', ')'], 'preamble': ''}
```
in a regular install.  The question is whether there is anywhere `engine` gets reset...  

Aha!  [Line 1404](http://hg.sagemath.org/sage-main/file/9e29a3d84c48/sage/misc/latex.py#l1404) (of the current 4.7.2) shows that this is set to `xelatex` and then not reset.   Apparently the doctest framework doesn't clear this, I'm not sure exactly where this all lives.

Solution should be:
* Fix the reset of the engine
* Leave the error message and `# random` it.   Or even include both error messages so that the documentation shows exactly what we expect with and without a working LaTeX installation.

What do you think?



---

archive/issue_comments_115678.json:
```json
{
    "body": "<a id='comment:23'></a>\nReplying to [kcrisman](#comment%3A22):\n> What do you think?\n\nI think we should try to find a solution which leaves all doctests as they are (including the new ones on this ticket): change the code, not the tests.\n\nMaybe the code should be more like:\n\n```\nif engine is None:\n    engine = \"latex\"\nif (engine==\"latex\" and not have_latex()) or (engine==\"pdflatex\" and not have_pdflatex()) or (engine==\"xelatex\" and not have_xelatex):\n    raise RuntimeError,...\n```",
    "created_at": "2011-11-08T17:14:50Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115678",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:23'></a>
Replying to [kcrisman](#comment%3A22):
> What do you think?

I think we should try to find a solution which leaves all doctests as they are (including the new ones on this ticket): change the code, not the tests.

Maybe the code should be more like:

```
if engine is None:
    engine = "latex"
if (engine=="latex" and not have_latex()) or (engine=="pdflatex" and not have_pdflatex()) or (engine=="xelatex" and not have_xelatex):
    raise RuntimeError,...
```



---

archive/attachments_015353.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_11311-ref.patch",
    "asset_url": "tarball://root/attachments/ticket11311/trac_11311-ref.patch",
    "created_at": "2011-11-23T22:18:27Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket11311/trac_11311-ref.patch",
    "user": "https://github.com/jhpalmieri"
}
```



---

archive/issue_comments_115679.json:
```json
{
    "body": "**Attachment:** [trac_11311-ref.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311-ref.patch)\n\napply on top of other patch",
    "created_at": "2011-11-23T22:18:27Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115679",
    "user": "https://github.com/jhpalmieri"
}
```

**Attachment:** [trac_11311-ref.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311-ref.patch)

apply on top of other patch



---

archive/issue_comments_115680.json:
```json
{
    "body": "<a id='comment:24'></a>\nHere's a new referee patch.  This just moves the code which checks the value of 'engine' earlier, so if 'engine' is set to a bad value, then that error gets caught first.  For me, on an OS X box (with a pretty good TeX installation), on sage.math (with a mediocre TeX installation (no xetex or xelatex), and on the skynet machine eno (no TeX at all), the following passes all tests, repeatedly:\n\n```\nsage -t  --randorder devel/sage/sage/misc/latex.py\n```\n(The patch also adds a line to one example to reset the latex preamble, so that tests pass when the order of the tests is randomized.)",
    "created_at": "2011-11-23T22:24:07Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115680",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:24'></a>
Here's a new referee patch.  This just moves the code which checks the value of 'engine' earlier, so if 'engine' is set to a bad value, then that error gets caught first.  For me, on an OS X box (with a pretty good TeX installation), on sage.math (with a mediocre TeX installation (no xetex or xelatex), and on the skynet machine eno (no TeX at all), the following passes all tests, repeatedly:

```
sage -t  --randorder devel/sage/sage/misc/latex.py
```
(The patch also adds a line to one example to reset the latex preamble, so that tests pass when the order of the tests is randomized.)



---

archive/issue_events_088684.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2011-11-23T22:24:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11311#event-88684"
}
```



---

archive/issue_comments_115681.json:
```json
{
    "body": "<a id='comment:25'></a>\nHere's an alternate version, which just consolidates some code (while checking whether `engine == 'pdflatex'`, check whether pdflatex is installed, for example).",
    "created_at": "2011-11-23T22:33:40Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115681",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:25'></a>
Here's an alternate version, which just consolidates some code (while checking whether `engine == 'pdflatex'`, check whether pdflatex is installed, for example).



---

archive/issue_comments_115682.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -4,4 +4,4 @@\n \n ---\n \n-Apply [[attachment:trac_11311-ref.patch](https://github.com/sagemath/sage/files/ticket11311/7ee749641223cd2460971c5819e00012.patch](https://github.com/sagemath/sage/files/ticket11311/c036bc7f8e1f6e08fc32b3d95a97b086.patch),).\n+Apply [ Then apply either [attachment:trac_11311-ref.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311-ref.patch) or [attachment:trac_11311-ref.v2.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311-ref.v2.patch)(https://github.com/sagemath/sage/files/ticket11311/738e79b48d3e4caa24f526608caa9116).\n``````\n",
    "created_at": "2011-11-23T22:33:40Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115682",
    "user": "https://github.com/jhpalmieri"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -4,4 +4,4 @@
 
 ---
 
-Apply [[attachment:trac_11311-ref.patch](https://github.com/sagemath/sage/files/ticket11311/7ee749641223cd2460971c5819e00012.patch](https://github.com/sagemath/sage/files/ticket11311/c036bc7f8e1f6e08fc32b3d95a97b086.patch),).
+Apply [ Then apply either [attachment:trac_11311-ref.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311-ref.patch) or [attachment:trac_11311-ref.v2.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311-ref.v2.patch)(https://github.com/sagemath/sage/files/ticket11311/738e79b48d3e4caa24f526608caa9116).
``````




---

archive/issue_comments_115683.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -4,4 +4,4 @@\n \n ---\n \n-Apply [ Then apply either [attachment:trac_11311-ref.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311-ref.patch) or [attachment:trac_11311-ref.v2.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311-ref.v2.patch)(https://github.com/sagemath/sage/files/ticket11311/738e79b48d3e4caa24f526608caa9116).\n+Apply [and [attachment:trac_11311-ref.v2.patch](https://github.com/sagemath/sage/files/ticket11311/24570fcd47ef06042152a6157cb5e15e.patch](https://github.com/sagemath/sage/files/ticket11311/f1037c542c36a8dbea0e39d15911c860.patch)).\n``````\n",
    "created_at": "2012-04-26T07:46:40Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115683",
    "user": "https://github.com/fchapoton"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -4,4 +4,4 @@
 
 ---
 
-Apply [ Then apply either [attachment:trac_11311-ref.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311-ref.patch) or [attachment:trac_11311-ref.v2.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311-ref.v2.patch)(https://github.com/sagemath/sage/files/ticket11311/738e79b48d3e4caa24f526608caa9116).
+Apply [and [attachment:trac_11311-ref.v2.patch](https://github.com/sagemath/sage/files/ticket11311/24570fcd47ef06042152a6157cb5e15e.patch](https://github.com/sagemath/sage/files/ticket11311/f1037c542c36a8dbea0e39d15911c860.patch)).
``````




---

archive/issue_comments_115684.json:
```json
{
    "body": "<a id='comment:26'></a>\nTrying to help the buildbot ...",
    "created_at": "2012-04-26T07:46:40Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115684",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:26'></a>
Trying to help the buildbot ...



---

archive/issue_comments_115685.json:
```json
{
    "body": "<a id='comment:27'></a>\n**ping**\n\nWhat is the status of this patch ?\n\nThe bot has not understood which patch to apply. Let us try again.\n\nApply trac_11311-rebased.patch  and trac_11311-ref.v2.patch",
    "created_at": "2012-05-10T19:30:03Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115685",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:27'></a>
**ping**

What is the status of this patch ?

The bot has not understood which patch to apply. Let us try again.

Apply trac_11311-rebased.patch  and trac_11311-ref.v2.patch



---

archive/issue_comments_115686.json:
```json
{
    "body": "<a id='comment:28'></a>\nHere is a rebased version of the \"v2\" patch.",
    "created_at": "2012-05-10T20:07:42Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115686",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:28'></a>
Here is a rebased version of the "v2" patch.



---

archive/issue_comments_115687.json:
```json
{
    "body": "apply on top of rebased patch",
    "created_at": "2012-05-10T20:08:06Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115687",
    "user": "https://github.com/jhpalmieri"
}
```

apply on top of rebased patch



---

archive/attachments_015354.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_11311-ref.v2.patch",
    "asset_url": "tarball://root/attachments/ticket11311/trac_11311-ref.v2.patch",
    "created_at": "2012-05-20T23:18:22Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket11311/trac_11311-ref.v2.patch",
    "user": "https://github.com/saliola"
}
```



---

archive/issue_comments_115688.json:
```json
{
    "body": "<a id='comment:29'></a>\n**Attachment:** [trac_11311-ref.v2.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311-ref.v2.patch)\n\nThanks for the review patch. Positive review.",
    "created_at": "2012-05-20T23:18:22Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115688",
    "user": "https://github.com/saliola"
}
```

<a id='comment:29'></a>
**Attachment:** [trac_11311-ref.v2.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311-ref.v2.patch)

Thanks for the review patch. Positive review.



---

archive/issue_events_088685.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2012-05-20T23:18:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11311#event-88685"
}
```



---

archive/issue_events_088686.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2012-05-20T23:18:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11311#event-88686"
}
```



---

archive/issue_comments_115689.json:
```json
{
    "body": "<a id='comment:30'></a>\nI just noticed that randomizing the doctests fails sometimes, with errors of the following form:\n\n```\nsage -t  --randorder devel/sage/sage/misc/latex.py\n...\nFile \"/Users/saliola/Applications/sage-5.0/devel/sage/sage/misc/latex.py\", line 221:\n    sage: builtin_constant_function(NotImplemented)\nExpected:\n    '\\\\mbox{\\\\rm NotImplemented}'\nGot:\n    '{\\\\rm NotImplemented}'\n...\n```\nSince this isn't cause by this patch (in fact, this patch improves things), I created a new ticket #12986 for this.",
    "created_at": "2012-05-21T00:44:27Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115689",
    "user": "https://github.com/saliola"
}
```

<a id='comment:30'></a>
I just noticed that randomizing the doctests fails sometimes, with errors of the following form:

```
sage -t  --randorder devel/sage/sage/misc/latex.py
...
File "/Users/saliola/Applications/sage-5.0/devel/sage/sage/misc/latex.py", line 221:
    sage: builtin_constant_function(NotImplemented)
Expected:
    '\\mbox{\\rm NotImplemented}'
Got:
    '{\\rm NotImplemented}'
...
```
Since this isn't cause by this patch (in fact, this patch improves things), I created a new ticket #12986 for this.



---

archive/issue_events_088687.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-21T22:33:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11311#event-88687"
}
```



---

archive/issue_events_088688.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-21T22:33:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11311#event-88688"
}
```



---

archive/issue_comments_115690.json:
```json
{
    "body": "<a id='comment:31'></a>\nThe patch only applies with fuzz 2 and therefore should be rebased to sage-5.1.beta0.",
    "created_at": "2012-05-21T22:33:13Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115690",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:31'></a>
The patch only applies with fuzz 2 and therefore should be rebased to sage-5.1.beta0.



---

archive/attachments_015355.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_11311-5.0.patch",
    "asset_url": "tarball://root/attachments/ticket11311/trac_11311-5.0.patch",
    "created_at": "2012-05-22T01:51:44Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket11311/trac_11311-5.0.patch",
    "user": "https://github.com/jhpalmieri"
}
```



---

archive/issue_comments_115691.json:
```json
{
    "body": "**Attachment:** [trac_11311-5.0.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311-5.0.patch)\n\nrebased to Sage 5.0",
    "created_at": "2012-05-22T01:51:44Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115691",
    "user": "https://github.com/jhpalmieri"
}
```

**Attachment:** [trac_11311-5.0.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311-5.0.patch)

rebased to Sage 5.0



---

archive/issue_events_088689.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-05-22T01:52:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11311#event-88689"
}
```



---

archive/issue_events_088690.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-05-22T01:52:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11311#event-88690"
}
```



---

archive/issue_comments_115692.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -4,4 +4,4 @@\n \n ---\n \n-Apply [and [attachment:trac_11311-ref.v2.patch](https://github.com/sagemath/sage/files/ticket11311/24570fcd47ef06042152a6157cb5e15e.patch](https://github.com/sagemath/sage/files/ticket11311/f1037c542c36a8dbea0e39d15911c860.patch)).\n+Apply [attachment:trac_11311-5.0.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311-5.0.patch).\n``````\n",
    "created_at": "2012-05-22T01:52:25Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115692",
    "user": "https://github.com/jhpalmieri"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -4,4 +4,4 @@
 
 ---
 
-Apply [and [attachment:trac_11311-ref.v2.patch](https://github.com/sagemath/sage/files/ticket11311/24570fcd47ef06042152a6157cb5e15e.patch](https://github.com/sagemath/sage/files/ticket11311/f1037c542c36a8dbea0e39d15911c860.patch)).
+Apply [attachment:trac_11311-5.0.patch](https://github.com/sagemath/sage/files/ticket11311/trac_11311-5.0.patch).
``````




---

archive/issue_comments_115693.json:
```json
{
    "body": "<a id='comment:32'></a>\nHere's a rebased patch (combining the old two patches).  (Despite its name and my comment when I attached it, it was actually rebased to Sage 5.1.beta0.)",
    "created_at": "2012-05-22T01:52:25Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115693",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:32'></a>
Here's a rebased patch (combining the old two patches).  (Despite its name and my comment when I attached it, it was actually rebased to Sage 5.1.beta0.)



---

archive/issue_comments_115694.json:
```json
{
    "body": "**Merged:** sage-5.1.beta1",
    "created_at": "2012-05-23T21:33:02Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115694",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-5.1.beta1



---

archive/issue_events_088691.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-23T21:33:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11311#event-88691"
}
```



---

archive/issue_events_088692.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-23T21:33:02Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11311#event-88692"
}
```



---

archive/issue_comments_115695.json:
```json
{
    "body": "**Changing merged** from \"sage-5.1.beta1\" to \"\".",
    "created_at": "2012-05-23T21:35:02Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115695",
    "user": "https://github.com/jdemeyer"
}
```

**Changing merged** from "sage-5.1.beta1" to "".



---

archive/issue_comments_115696.json:
```json
{
    "body": "<a id='comment:34'></a>\nNothing against this patch, just didn't mean to merge it now (probably next beta).",
    "created_at": "2012-05-23T21:35:02Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115696",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:34'></a>
Nothing against this patch, just didn't mean to merge it now (probably next beta).



---

archive/issue_events_088693.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-23T21:35:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11311#event-88693"
}
```



---

archive/issue_events_088694.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-23T21:35:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11311#event-88694"
}
```



---

archive/issue_events_088695.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-23T21:35:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11311#event-88695"
}
```



---

archive/issue_events_088696.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-14T06:38:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11311#event-88696"
}
```



---

archive/issue_events_088697.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-14T06:38:04Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11311#event-88697"
}
```



---

archive/issue_comments_115697.json:
```json
{
    "body": "**Merged:** sage-5.1.beta4",
    "created_at": "2012-06-14T06:38:04Z",
    "issue": "https://github.com/sagemath/sage/issues/11311",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11311#issuecomment-115697",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-5.1.beta4
