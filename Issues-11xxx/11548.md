# Issue 11548: Basic implementation of Harley's point counting for elliptic curve.

archive/issues_011376.json:
```json
{
    "assignees": [],
    "body": "This is a basic implementation of Harley's point counting algorithm in characteristic 2 as presented in Vercauteren thesis (so with what is called there Norm Computation I).\n\nIt uses Sage's interface to Pari library for computations in Z_q.\n\nIt is absolutely not optimized.\n\nA simple optimization would be to cythonize it and use direct calls to Pari or NTL/GF2X libraries.\n\nIt is faster than the basic FGH algo implementation of ticket #11448 :\n\n```\nsage: from sage.schemes.elliptic_curves.fgh_algo import compute_trace_char2\nsage: from sage.schemes.elliptic_curves.harley_algo import harley_2\nsage: K = GF(1<<160,'t')\nsage: a = K.random_element(); time print harley_2(a); time print compute_trace_char2(a); \n-735100818784167142776219\nTime: CPU 7.37 s, Wall: 7.38 s\n-735100818784167142776219\nTime: CPU 37.33 s, Wall: 37.36 s\n```\n\nAssignee: **@JohnCremona**\n\nCC:  @sagetrac-jpflori @defeo\n\nKeywords: **point counting, satoh, elliptic curve, canonical lift, robert harley**\n\nAuthor: **Jean-Pierre Flori**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11548_\n\n",
    "closed_at": "2016-08-30T13:32:25Z",
    "created_at": "2011-06-27T13:41:17Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20elliptic%20curves",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Basic implementation of Harley's point counting for elliptic curve.",
    "type": "issue",
    "updated_at": "2016-08-30T13:32:25Z",
    "url": "https://github.com/sagemath/sage/issues/11548",
    "user": "https://github.com/sagetrac-jpflori"
}
```
This is a basic implementation of Harley's point counting algorithm in characteristic 2 as presented in Vercauteren thesis (so with what is called there Norm Computation I).

It uses Sage's interface to Pari library for computations in Z_q.

It is absolutely not optimized.

A simple optimization would be to cythonize it and use direct calls to Pari or NTL/GF2X libraries.

It is faster than the basic FGH algo implementation of ticket #11448 :

```
sage: from sage.schemes.elliptic_curves.fgh_algo import compute_trace_char2
sage: from sage.schemes.elliptic_curves.harley_algo import harley_2
sage: K = GF(1<<160,'t')
sage: a = K.random_element(); time print harley_2(a); time print compute_trace_char2(a); 
-735100818784167142776219
Time: CPU 7.37 s, Wall: 7.38 s
-735100818784167142776219
Time: CPU 37.33 s, Wall: 37.36 s
```

Assignee: **@JohnCremona**

CC:  @sagetrac-jpflori @defeo

Keywords: **point counting, satoh, elliptic curve, canonical lift, robert harley**

Author: **Jean-Pierre Flori**

_Issue created by migration from https://trac.sagemath.org/ticket/11548_





---

archive/issue_events_140926.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2011-06-27T13:41:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11548",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11548#event-140926"
}
```



---

archive/issue_events_140927.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2011-06-27T13:41:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11548",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20elliptic%20curves",
    "label_color": "0000ff",
    "label_name": "component: elliptic curves",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11548#event-140927"
}
```



---

archive/issue_events_140928.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2011-06-27T13:41:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11548",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11548#event-140928"
}
```



---

archive/issue_events_140929.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2011-06-27T13:41:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11548",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11548#event-140929"
}
```



---

archive/issue_comments_119972.json:
```json
{
    "body": "Attachment: **[trac_11548-harley-pari.patch.gz](https://github.com/sagemath/sage/files/ticket11548/trac_11548-harley-pari.patch.gz)**\n\nBasic implementation of Harley's point counting algo.",
    "created_at": "2011-06-27T13:43:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11548",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11548#issuecomment-119972",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Attachment: **[trac_11548-harley-pari.patch.gz](https://github.com/sagemath/sage/files/ticket11548/trac_11548-harley-pari.patch.gz)**

Basic implementation of Harley's point counting algo.



---

archive/issue_comments_119973.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nAs I've just discovered a lot of wonderfull point counting methods have been implemented in PARI using the new finite fields structures.\nThis includes SEA over non prime fields and Harley's algo over binary fields and (although I've not tested them yet) will without doubt make this ticket and #11448 completely disgusting and useless.",
    "created_at": "2012-09-06T20:41:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11548",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11548#issuecomment-119973",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<div id="comment:2" align="right">Comment 2</div>

As I've just discovered a lot of wonderfull point counting methods have been implemented in PARI using the new finite fields structures.
This includes SEA over non prime fields and Harley's algo over binary fields and (although I've not tested them yet) will without doubt make this ticket and #11448 completely disgusting and useless.



---

archive/issue_comments_119974.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nOr not yet...\nAfter making F2xq_elltrace_Harley exported, I got\n\n```\n? install(\"F2xq_elltrace_Harley\",\"GG\",\"F2t\",\"libpari.so\")\n? T = ffinit(2,50)\ntime = 4 ms.\n%4 = Mod(1, 2)*x^50 + Mod(1, 2)*x^49 + Mod(1, 2)*x^48 + Mod(1, 2)*x^46 + Mod(1, 2)*x^45 + Mod(1, 2)*x^44 + Mod(1, 2)*x^43 + Mod(1, 2)*x^42 + Mod(1, 2)*x^41 + Mod(1, 2)*x^40 + Mod(1, 2)*x^39 + Mod(1, 2)*x^35 + Mod(1, 2)*x^34 + Mod(1, 2)*x^33 + Mod(1, 2)*x^26 + Mod(1, 2)*x^25 + Mod(1, 2)*x^22 + Mod(1, 2)*x^21 + Mod(1, 2)*x^19 + Mod(1, 2)*x^15 + Mod(1, 2)*x^11 + Mod(1, 2)*x^10 + Mod(1, 2)*x^5 + Mod(1, 2)*x^4 + Mod(1, 2)*x^3 + Mod(1, 2)*x^2 + Mod(1, 2)\n? y = ffgen(T, 'y)\ntime = 0 ms.\n%5 = y\n? F2t(y,T)\ntime = 39,851 ms.\n%6 = \n```\nThat's strange, I may have broken something in the way.",
    "created_at": "2012-09-07T22:49:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11548",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11548#issuecomment-119974",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<div id="comment:3" align="right">Comment 3</div>

Or not yet...
After making F2xq_elltrace_Harley exported, I got

```
? install("F2xq_elltrace_Harley","GG","F2t","libpari.so")
? T = ffinit(2,50)
time = 4 ms.
%4 = Mod(1, 2)*x^50 + Mod(1, 2)*x^49 + Mod(1, 2)*x^48 + Mod(1, 2)*x^46 + Mod(1, 2)*x^45 + Mod(1, 2)*x^44 + Mod(1, 2)*x^43 + Mod(1, 2)*x^42 + Mod(1, 2)*x^41 + Mod(1, 2)*x^40 + Mod(1, 2)*x^39 + Mod(1, 2)*x^35 + Mod(1, 2)*x^34 + Mod(1, 2)*x^33 + Mod(1, 2)*x^26 + Mod(1, 2)*x^25 + Mod(1, 2)*x^22 + Mod(1, 2)*x^21 + Mod(1, 2)*x^19 + Mod(1, 2)*x^15 + Mod(1, 2)*x^11 + Mod(1, 2)*x^10 + Mod(1, 2)*x^5 + Mod(1, 2)*x^4 + Mod(1, 2)*x^3 + Mod(1, 2)*x^2 + Mod(1, 2)
? y = ffgen(T, 'y)
time = 0 ms.
%5 = y
? F2t(y,T)
time = 39,851 ms.
%6 = 
```
That's strange, I may have broken something in the way.



---

archive/issue_comments_119975.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThe above timing is indeed wrong. Bill reports\n\n```\n? a=ffgen(ffinit(2,1024),'a);ellffinit([1,0,0,0,a],a,1);\n? ##\n ***   last result computed in 11,482 ms.\n```\nAnd I get similar results on my computer.\n\nAnd indeed the trace looks a little too large for smtg over F<sub>2</sub><sup>50</sup>, so I badly used the library function from gp, not sure how to do this properly though.",
    "created_at": "2012-10-10T07:42:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11548",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11548#issuecomment-119975",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<div id="comment:4" align="right">Comment 4</div>

The above timing is indeed wrong. Bill reports

```
? a=ffgen(ffinit(2,1024),'a);ellffinit([1,0,0,0,a],a,1);
? ##
 ***   last result computed in 11,482 ms.
```
And I get similar results on my computer.

And indeed the trace looks a little too large for smtg over F<sub>2</sub><sup>50</sup>, so I badly used the library function from gp, not sure how to do this properly though.



---

archive/issue_events_140930.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11548",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11548#event-140930"
}
```



---

archive/issue_events_140931.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11548",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11548#event-140931"
}
```



---

archive/issue_events_140932.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11548",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11548#event-140932"
}
```



---

archive/issue_events_140933.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11548",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11548#event-140933"
}
```



---

archive/issue_events_140934.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11548",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11548#event-140934"
}
```



---

archive/issue_events_140935.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11548",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11548#event-140935"
}
```



---

archive/issue_events_140936.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11548",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11548#event-140936"
}
```



---

archive/issue_events_140937.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11548",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11548#event-140937"
}
```



---

archive/issue_events_140938.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2016-08-17T07:55:09Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11548",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11548#event-140938"
}
```



---

archive/issue_events_140939.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2016-08-17T07:55:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11548",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11548#event-140939"
}
```



---

archive/issue_events_140940.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2016-08-17T07:55:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11548",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11548#event-140940"
}
```



---

archive/issue_comments_119976.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nSuperseded by #16931 and new PARI/GP functionalities.",
    "created_at": "2016-08-17T07:55:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11548",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11548#issuecomment-119976",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<div id="comment:9" align="right">Comment 9</div>

Superseded by #16931 and new PARI/GP functionalities.



---

archive/issue_events_140941.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2016-08-17T07:55:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11548",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11548#event-140941"
}
```



---

archive/issue_events_140942.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2016-08-17T07:55:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11548",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11548#event-140942"
}
```



---

archive/issue_events_140943.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2016-08-17T07:55:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11548",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11548#event-140943"
}
```



---

archive/issue_events_140944.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-08-30T13:32:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11548",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11548#event-140944"
}
```



---

archive/issue_events_140945.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-08-30T13:32:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11548",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11548#event-140945"
}
```



---

archive/issue_events_140946.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-08-30T13:32:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11548",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11548#event-140946"
}
```



---

archive/issue_comments_119977.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nDetermined to be invalid/duplicate/wontfix (closing as \"wontfix\" as a catch-all resolution).",
    "created_at": "2016-08-30T13:32:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11548",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11548#issuecomment-119977",
    "user": "https://github.com/embray"
}
```

<div id="comment:11" align="right">Comment 11</div>

Determined to be invalid/duplicate/wontfix (closing as "wontfix" as a catch-all resolution).
