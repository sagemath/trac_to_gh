# Issue 11159: Corrections to the  "Install from Source Code" section of the Sage Installation Guide

archive/issues_011006.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-drkirkby"
    ],
    "body": "#11081 added a number of changes to the section of the Sage Installation Guide dealing with building from source code. \n\nDespite correcting a large number of errors in the previous version of the manual, some further changes are needed:\n1. The manual stated GNU tar is needed, but BSD tar also works (at least on OS X).\n2. We should clearly state that gcc 4.6.0 is not yet supported.\n\nDepends on #11081.\n\n---\n\nApply [attachment: 11159.patch](https://github.com/sagemath/sage/files/ticket11159/11159.patch.gz), [attachment: 11159-finally-fix-GNU-tar-issue.2.patch](https://github.com/sagemath/sage/files/ticket11159/11159-finally-fix-GNU-tar-issue.2.patch.gz), and [attachment: 11159_review.patch](https://github.com/sagemath/sage/files/ticket11159/11159_review.patch.gz).\n\nCC:  @dimpase\n\nComponent: **documentation**\n\nAuthor: **David Kirkby**\n\nReviewer: **Jeroen Demeyer, John Palmieri**\n\nMerged: **sage-4.7.rc0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11159_\n\n",
    "closed_at": "2011-04-21T19:35:37Z",
    "created_at": "2011-04-08T15:56:45Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Corrections to the  \"Install from Source Code\" section of the Sage Installation Guide",
    "type": "issue",
    "updated_at": "2011-04-22T00:31:40Z",
    "url": "https://github.com/sagemath/sage/issues/11159",
    "user": "https://github.com/sagetrac-drkirkby"
}
```
#11081 added a number of changes to the section of the Sage Installation Guide dealing with building from source code. 

Despite correcting a large number of errors in the previous version of the manual, some further changes are needed:
1. The manual stated GNU tar is needed, but BSD tar also works (at least on OS X).
2. We should clearly state that gcc 4.6.0 is not yet supported.

Depends on #11081.

---

Apply [attachment: 11159.patch](https://github.com/sagemath/sage/files/ticket11159/11159.patch.gz), [attachment: 11159-finally-fix-GNU-tar-issue.2.patch](https://github.com/sagemath/sage/files/ticket11159/11159-finally-fix-GNU-tar-issue.2.patch.gz), and [attachment: 11159_review.patch](https://github.com/sagemath/sage/files/ticket11159/11159_review.patch.gz).

CC:  @dimpase

Component: **documentation**

Author: **David Kirkby**

Reviewer: **Jeroen Demeyer, John Palmieri**

Merged: **sage-4.7.rc0**

_Issue created by migration from https://trac.sagemath.org/ticket/11159_





---

archive/issue_events_146417.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2011-04-08T15:56:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "milestone_number": null,
    "milestone_title": "sage-4.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11159#event-146417"
}
```



---

archive/issue_events_146418.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2011-04-08T15:56:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11159#event-146418"
}
```



---

archive/issue_events_146419.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2011-04-08T15:56:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11159#event-146419"
}
```



---

archive/issue_events_146420.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2011-04-08T15:56:45Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "subject": "https://github.com/sagetrac-drkirkby",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11159#event-146420"
}
```



---

archive/issue_events_146421.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2011-04-08T15:56:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11159#event-146421"
}
```



---

archive/issue_comments_110994.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,5 +1,7 @@\n #11081 added a number of changes to the section of the Sage Installation Guide dealing with building from source code. \n \n-Despite correcting a large number of errors in the previous version of the manual, one error was introduced by #11081. The manual stated GNU tar is needed, but this is not so on OS X. \n+Despite correcting a large number of errors in the previous version of the manual, some further changes are needed:\n+1. The manual stated GNU tar is needed, but BSD tar also works (at least on OS X).\n+2. We should clearly state that gcc 4.6.0 is not yet supported.\n \n-Dave \n+Depends on #11081.\n``````\n",
    "created_at": "2011-04-18T18:40:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11159#issuecomment-110994",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,5 +1,7 @@
 #11081 added a number of changes to the section of the Sage Installation Guide dealing with building from source code. 
 
-Despite correcting a large number of errors in the previous version of the manual, one error was introduced by #11081. The manual stated GNU tar is needed, but this is not so on OS X. 
+Despite correcting a large number of errors in the previous version of the manual, some further changes are needed:
+1. The manual stated GNU tar is needed, but BSD tar also works (at least on OS X).
+2. We should clearly state that gcc 4.6.0 is not yet supported.
 
-Dave 
+Depends on #11081.
``````




---

archive/issue_events_146422.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-04-18T18:40:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20major%20/%203",
    "label_color": "ffbb00",
    "label_name": "p: major / 3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11159#event-146422"
}
```



---

archive/issue_events_146423.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-04-18T18:40:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
    "label_color": "ff0000",
    "label_name": "p: blocker / 1",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11159#event-146423"
}
```



---

archive/issue_events_146424.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-04-18T18:44:38Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "title_is": "Corrections to the  \"Install from Source Code\" section of the Sage Installation Guide",
    "title_was": "Correction about GNU tar in the  \"Install from Source Code\" section of the Sage Installation Guide",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11159#event-146424"
}
```



---

archive/issue_comments_110995.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nI'll sort this out in the next 15 minutes or so.",
    "created_at": "2011-04-18T19:25:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11159#issuecomment-110995",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

<div id="comment:4" align="right">comment:4</div>

I'll sort this out in the next 15 minutes or so.



---

archive/issue_events_146425.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2011-04-18T19:25:15Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "subject": "https://github.com/sagetrac-drkirkby",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11159#event-146425"
}
```



---

archive/issue_events_146426.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2011-04-18T19:25:15Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "subject": "https://github.com/sagetrac-drkirkby",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11159#event-146426"
}
```



---

archive/issue_comments_110996.json:
```json
{
    "body": "Author: **David Kirkby**",
    "created_at": "2011-04-18T19:35:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11159#issuecomment-110996",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

Author: **David Kirkby**



---

archive/issue_events_146427.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2011-04-18T19:35:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11159#event-146427"
}
```



---

archive/issue_comments_110997.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nI messed uo the patch - will have to rebuild sage from scratch and redo it!! It will take an hour or so. \n\nDave",
    "created_at": "2011-04-18T19:57:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11159#issuecomment-110997",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

<div id="comment:6" align="right">comment:6</div>

I messed uo the patch - will have to rebuild sage from scratch and redo it!! It will take an hour or so. 

Dave



---

archive/issue_events_146428.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2011-04-18T19:57:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11159#event-146428"
}
```



---

archive/issue_events_146429.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2011-04-18T19:57:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11159#event-146429"
}
```



---

archive/issue_comments_110998.json:
```json
{
    "body": "Attachment: **[11159.patch.gz](https://github.com/sagemath/sage/files/ticket11159/11159.patch.gz)**\n\nCorrects error about GNU make being required, and adds the fact gcc 4.6.0 will not build Sage",
    "created_at": "2011-04-18T20:16:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11159#issuecomment-110998",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

Attachment: **[11159.patch.gz](https://github.com/sagemath/sage/files/ticket11159/11159.patch.gz)**

Corrects error about GNU make being required, and adds the fact gcc 4.6.0 will not build Sage



---

archive/issue_events_146430.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2011-04-18T20:18:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11159#event-146430"
}
```



---

archive/issue_events_146431.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2011-04-18T20:18:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11159#event-146431"
}
```



---

archive/issue_comments_110999.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2011-04-18T20:35:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11159#issuecomment-110999",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_comments_111000.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nI would drop the reference to a specific Sage version in the manual, reviewer patch added.  Otherwise positive review.",
    "created_at": "2011-04-18T20:35:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11159#issuecomment-111000",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:8" align="right">comment:8</div>

I would drop the reference to a specific Sage version in the manual, reviewer patch added.  Otherwise positive review.



---

archive/issue_events_146432.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2011-04-18T20:43:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11159#event-146432"
}
```



---

archive/issue_events_146433.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2011-04-18T20:43:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11159#event-146433"
}
```



---

archive/issue_comments_111001.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nMy reason for stating 4.7.0 was problematic was because if someone has the latest gcc in their linux distribution, they do have the option of building Sage 4.6.2. \n\nBut I'm not too bothered either way. Let's hope we can get a fix for gcc 4.6.0 quickly. A quick Sage 4.7.1 was just the gcc-4.6.0 patches, would be quite helpful. \n\nDave",
    "created_at": "2011-04-18T20:43:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11159#issuecomment-111001",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

<div id="comment:9" align="right">comment:9</div>

My reason for stating 4.7.0 was problematic was because if someone has the latest gcc in their linux distribution, they do have the option of building Sage 4.6.2. 

But I'm not too bothered either way. Let's hope we can get a fix for gcc 4.6.0 quickly. A quick Sage 4.7.1 was just the gcc-4.6.0 patches, would be quite helpful. 

Dave



---

archive/issue_comments_111002.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nReplying to [@sagetrac-drkirkby](#comment:9):\n A quick Sage 4.7.1 was just the gcc-4.6.0 patches, would be quite helpful. \n> \n> Dave \n\nI mean:\n\n*A quick Sage 4.7.1 **with** just the gcc-4.6.0 patches, would be quite helpful*",
    "created_at": "2011-04-18T20:45:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11159#issuecomment-111002",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

<div id="comment:10" align="right">comment:10</div>

Replying to [@sagetrac-drkirkby](#comment:9):
 A quick Sage 4.7.1 was just the gcc-4.6.0 patches, would be quite helpful. 
> 
> Dave 

I mean:

*A quick Sage 4.7.1 **with** just the gcc-4.6.0 patches, would be quite helpful*



---

archive/issue_events_146434.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2011-04-18T21:29:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11159#event-146434"
}
```



---

archive/issue_events_146435.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2011-04-18T21:29:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11159#event-146435"
}
```



---

archive/issue_comments_111003.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">comment:11</div>\n\nDo we have a meta-ticket for gcc 4.6.0 issues?  Once they're sorted out, we need to make sure to fix the installation guide accordingly.\n\nRegarding the actual patch here, the description says something about GNU tar vs. BSD tar, but the patch discusses GNU make vs. BSD make.  On my OS X box, I have GNU make and BSD tar, so is the patch saying what it's supposed to?\n\n```\n$ which make # (or \"command -v make\")\n/usr/bin/make\n$ make --version\nGNU Make 3.81\nCopyright (C) 2006  Free Software Foundation, Inc.\nThis is free software; see the source for copying conditions.\nThere is NO warranty; not even for MERCHANTABILITY or FITNESS FOR A\nPARTICULAR PURPOSE.\n\nThis program built for i386-apple-darwin10.0\n```\nand\n\n```\n$ which tar\n/usr/bin/tar\n$ tar --version\nbsdtar 2.6.2 - libarchive 2.6.2\n```\nI also have a program installed called \"bsdmake\".  When I tried building Sage with `export MAKE=\"bsdmake -j2\"`, I got an error building boehm_gc:\n\n```\n...\nconfig.status: creating tests/Makefile\nconfig.status: creating src/config.h\nconfig.status: executing depfiles commands\nconfig.status: executing default commands\n(null): illegal option -- -\nusage: make [-BPSXeiknqrstv] [-C directory] [-D variable]\n\t[-d flags] [-E variable] [-f makefile] [-I directory]\n\t[-j max_jobs] [-m directory] [-V variable]\n\t[variable=value] [target ...]\nmake[1]: *** [all-recursive] Error 1\nError building BoehmGC.\n```",
    "created_at": "2011-04-18T21:29:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11159#issuecomment-111003",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:11" align="right">comment:11</div>

Do we have a meta-ticket for gcc 4.6.0 issues?  Once they're sorted out, we need to make sure to fix the installation guide accordingly.

Regarding the actual patch here, the description says something about GNU tar vs. BSD tar, but the patch discusses GNU make vs. BSD make.  On my OS X box, I have GNU make and BSD tar, so is the patch saying what it's supposed to?

```
$ which make # (or "command -v make")
/usr/bin/make
$ make --version
GNU Make 3.81
Copyright (C) 2006  Free Software Foundation, Inc.
This is free software; see the source for copying conditions.
There is NO warranty; not even for MERCHANTABILITY or FITNESS FOR A
PARTICULAR PURPOSE.

This program built for i386-apple-darwin10.0
```
and

```
$ which tar
/usr/bin/tar
$ tar --version
bsdtar 2.6.2 - libarchive 2.6.2
```
I also have a program installed called "bsdmake".  When I tried building Sage with `export MAKE="bsdmake -j2"`, I got an error building boehm_gc:

```
...
config.status: creating tests/Makefile
config.status: creating src/config.h
config.status: executing depfiles commands
config.status: executing default commands
(null): illegal option -- -
usage: make [-BPSXeiknqrstv] [-C directory] [-D variable]
	[-d flags] [-E variable] [-f makefile] [-I directory]
	[-j max_jobs] [-m directory] [-V variable]
	[variable=value] [target ...]
make[1]: *** [all-recursive] Error 1
Error building BoehmGC.
```



---

archive/issue_events_146436.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2011-04-19T03:10:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11159#event-146436"
}
```



---

archive/issue_events_146437.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2011-04-19T03:10:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11159#event-146437"
}
```



---

archive/issue_comments_111004.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">comment:12</div>\n\nReplying to [@jhpalmieri](#comment:11):\n> Do we have a meta-ticket for gcc 4.6.0 issues?  Once they're sorted out, we need to make sure to fix the installation guide accordingly.\n\nWe did not, but I have just created one (#11216). I've listed all the gcc 4.6.0 issues, including the resolved ones, so we have a record. To my knowledge there were 4 issues, 3 of which have been resolved with one (Singular) outstanding. But there may be others I'm unaware of, so others should edit the description if I missed any. \n\nThe ticket I created does mention both the Wiki and the Installation Guide will need updating when these issues are resolved. \n \n> Regarding the actual patch here, the description says something about GNU tar vs. BSD tar, but the patch discusses GNU make vs. BSD make. \n\nI was sitting in bed thinking the exact same thing. I was aware I'd put the wrong information in the patch. Still, the point about needing GNU Make on Solaris is true so I just need to add the information about 'tar' \n\n> I also have a program installed called \"bsdmake\".  When I tried building Sage with `export MAKE=\"bsdmake -j2\"`, I got an error building boehm_gc:\n\n> I think it's probably true you need GNU make too,\n\nYes, it probably is true you need GNU make, though I think Solaris and OpenSolaris are the only systems where that version of 'make' is not the default. \n\nI will apply a patch to correct the 'tar' issue. As such, I'm marking this as \"needs work\" for now. \n\nDave",
    "created_at": "2011-04-19T03:10:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11159#issuecomment-111004",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

<div id="comment:12" align="right">comment:12</div>

Replying to [@jhpalmieri](#comment:11):
> Do we have a meta-ticket for gcc 4.6.0 issues?  Once they're sorted out, we need to make sure to fix the installation guide accordingly.

We did not, but I have just created one (#11216). I've listed all the gcc 4.6.0 issues, including the resolved ones, so we have a record. To my knowledge there were 4 issues, 3 of which have been resolved with one (Singular) outstanding. But there may be others I'm unaware of, so others should edit the description if I missed any. 

The ticket I created does mention both the Wiki and the Installation Guide will need updating when these issues are resolved. 
 
> Regarding the actual patch here, the description says something about GNU tar vs. BSD tar, but the patch discusses GNU make vs. BSD make. 

I was sitting in bed thinking the exact same thing. I was aware I'd put the wrong information in the patch. Still, the point about needing GNU Make on Solaris is true so I just need to add the information about 'tar' 

> I also have a program installed called "bsdmake".  When I tried building Sage with `export MAKE="bsdmake -j2"`, I got an error building boehm_gc:

> I think it's probably true you need GNU make too,

Yes, it probably is true you need GNU make, though I think Solaris and OpenSolaris are the only systems where that version of 'make' is not the default. 

I will apply a patch to correct the 'tar' issue. As such, I'm marking this as "needs work" for now. 

Dave



---

archive/issue_comments_111005.json:
```json
{
    "body": "Attachment: **[11159-finally-fix-GNU-tar-issue.patch.gz](https://github.com/sagemath/sage/files/ticket11159/11159-finally-fix-GNU-tar-issue.patch.gz)**\n\nState that on OS X, the BSD tar supplied is acceptable.",
    "created_at": "2011-04-19T03:51:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11159#issuecomment-111005",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

Attachment: **[11159-finally-fix-GNU-tar-issue.patch.gz](https://github.com/sagemath/sage/files/ticket11159/11159-finally-fix-GNU-tar-issue.patch.gz)**

State that on OS X, the BSD tar supplied is acceptable.



---

archive/issue_comments_111006.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">comment:13</div>\n\nI've fixed the tar issue, so are marking this for review. \n\nI actually think another update of this section of the Installation Guide should split up the build instructions on each platform into different sections. i.e have sections for:\n\n* Linux\n* OS X\n* Solaris or OpenSolaris\n* AIX\n* HP-UX\n\nEach platform has it own set of requirements, with no two platforms being exactly the same. Having different sections should cut down the amount a user needs to read. \n\nDave",
    "created_at": "2011-04-19T04:00:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11159#issuecomment-111006",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

<div id="comment:13" align="right">comment:13</div>

I've fixed the tar issue, so are marking this for review. 

I actually think another update of this section of the Installation Guide should split up the build instructions on each platform into different sections. i.e have sections for:

* Linux
* OS X
* Solaris or OpenSolaris
* AIX
* HP-UX

Each platform has it own set of requirements, with no two platforms being exactly the same. Having different sections should cut down the amount a user needs to read. 

Dave



---

archive/issue_events_146438.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2011-04-19T04:00:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11159#event-146438"
}
```



---

archive/issue_events_146439.json:
```json
{
    "actor": "https://github.com/sagetrac-drkirkby",
    "created_at": "2011-04-19T04:00:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11159#event-146439"
}
```



---

archive/issue_comments_111007.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">comment:14</div>\n\nReplying to [@sagetrac-drkirkby](#comment:9):\n> My reason for stating 4.7.0 was problematic was because if someone has the latest gcc in their linux distribution, they do have the option of building Sage 4.6.2. \n\nThat was a factually incorrect statement, as I doubt any version of Sage would build with gcc 4.6.0, so Jeroen was certainly correct to remove the comment about Sage 4.7.0.\n\nDave",
    "created_at": "2011-04-19T04:05:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11159#issuecomment-111007",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

<div id="comment:14" align="right">comment:14</div>

Replying to [@sagetrac-drkirkby](#comment:9):
> My reason for stating 4.7.0 was problematic was because if someone has the latest gcc in their linux distribution, they do have the option of building Sage 4.6.2. 

That was a factually incorrect statement, as I doubt any version of Sage would build with gcc 4.6.0, so Jeroen was certainly correct to remove the comment about Sage 4.7.0.

Dave



---

archive/issue_comments_111008.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">comment:15</div>\n\nAttachment: **[11159_review.patch.gz](https://github.com/sagemath/sage/files/ticket11159/11159_review.patch.gz)**\n\nNew reviewer patch, remove the statement that `m4` is needed (I have built Sage completely from scratch on a GNU/Linux system *without* m4 and it worked).",
    "created_at": "2011-04-19T09:37:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11159#issuecomment-111008",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:15" align="right">comment:15</div>

Attachment: **[11159_review.patch.gz](https://github.com/sagemath/sage/files/ticket11159/11159_review.patch.gz)**

New reviewer patch, remove the statement that `m4` is needed (I have built Sage completely from scratch on a GNU/Linux system *without* m4 and it worked).



---

archive/issue_comments_111009.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">comment:16</div>\n\nAll three patches look good to me.  Dave, the comma starting line 129 in your second patch should actually be at the end of line 128; could you fix that?",
    "created_at": "2011-04-20T19:02:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11159#issuecomment-111009",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:16" align="right">comment:16</div>

All three patches look good to me.  Dave, the comma starting line 129 in your second patch should actually be at the end of line 128; could you fix that?



---

archive/issue_comments_111010.json:
```json
{
    "body": "Attachment: **[11159-finally-fix-GNU-tar-issue.2.patch.gz](https://github.com/sagemath/sage/files/ticket11159/11159-finally-fix-GNU-tar-issue.2.patch.gz)**\n\nApply this instead, fixes comma",
    "created_at": "2011-04-21T00:53:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11159#issuecomment-111010",
    "user": "https://github.com/kcrisman"
}
```

Attachment: **[11159-finally-fix-GNU-tar-issue.2.patch.gz](https://github.com/sagemath/sage/files/ticket11159/11159-finally-fix-GNU-tar-issue.2.patch.gz)**

Apply this instead, fixes comma



---

archive/issue_comments_111011.json:
```json
{
    "body": "Changed reviewer from **Jeroen Demeyer** to **Jeroen Demeyer, John Palmieri**",
    "created_at": "2011-04-21T00:54:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11159#issuecomment-111011",
    "user": "https://github.com/kcrisman"
}
```

Changed reviewer from **Jeroen Demeyer** to **Jeroen Demeyer, John Palmieri**



---

archive/issue_comments_111012.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -5,3 +5,7 @@\n 2. We should clearly state that gcc 4.6.0 is not yet supported.\n \n Depends on #11081.\n+\n+---\n+\n+Apply [attachment: 11159.patch](https://github.com/sagemath/sage/files/ticket11159/11159.patch.gz), [attachment: 11159-finally-fix-GNU-tar-issue.2.patch](https://github.com/sagemath/sage/files/ticket11159/11159-finally-fix-GNU-tar-issue.2.patch.gz), and [attachment: 11159_review.patch](https://github.com/sagemath/sage/files/ticket11159/11159_review.patch.gz).\n``````\n",
    "created_at": "2011-04-21T00:54:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11159#issuecomment-111012",
    "user": "https://github.com/kcrisman"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -5,3 +5,7 @@
 2. We should clearly state that gcc 4.6.0 is not yet supported.
 
 Depends on #11081.
+
+---
+
+Apply [attachment: 11159.patch](https://github.com/sagemath/sage/files/ticket11159/11159.patch.gz), [attachment: 11159-finally-fix-GNU-tar-issue.2.patch](https://github.com/sagemath/sage/files/ticket11159/11159-finally-fix-GNU-tar-issue.2.patch.gz), and [attachment: 11159_review.patch](https://github.com/sagemath/sage/files/ticket11159/11159_review.patch.gz).
``````




---

archive/issue_comments_111013.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">comment:18</div>\n\nThis fixes, that, and everything else looks nice in the doc.",
    "created_at": "2011-04-21T00:58:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11159#issuecomment-111013",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:18" align="right">comment:18</div>

This fixes, that, and everything else looks nice in the doc.



---

archive/issue_events_146440.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-04-21T00:58:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11159#event-146440"
}
```



---

archive/issue_events_146441.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-04-21T00:58:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11159#event-146441"
}
```



---

archive/issue_events_146442.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-04-21T19:35:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11159#event-146442"
}
```



---

archive/issue_events_146443.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-04-21T19:35:37Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11159#event-146443"
}
```



---

archive/issue_comments_111014.json:
```json
{
    "body": "Merged: **sage-4.7.rc0**",
    "created_at": "2011-04-21T19:35:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11159#issuecomment-111014",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.7.rc0**



---

archive/issue_comments_111015.json:
```json
{
    "body": "<div id=\"comment:20\" align=\"right\">comment:20</div>\n\nReplying to [@jhpalmieri](#comment:16):\n> All three patches look good to me.  Dave, the comma starting line 129 in your second patch should actually be at the end of line 128; could you fix that?\n\nI've been beaten to that, and this is now merged. \n\nWhilst I believe the changes have significantly improved this section of the documentation, I still believe it could be improved somewhat more. Most of the errors have been removed, but it's not wonderfully clear and a bit confusing in places. I've created a new ticket (#11237 )to make some further improvements. Feel free to make any comments on #11237.",
    "created_at": "2011-04-22T00:31:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11159",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11159#issuecomment-111015",
    "user": "https://github.com/sagetrac-drkirkby"
}
```

<div id="comment:20" align="right">comment:20</div>

Replying to [@jhpalmieri](#comment:16):
> All three patches look good to me.  Dave, the comma starting line 129 in your second patch should actually be at the end of line 128; could you fix that?

I've been beaten to that, and this is now merged. 

Whilst I believe the changes have significantly improved this section of the documentation, I still believe it could be improved somewhat more. Most of the errors have been removed, but it's not wonderfully clear and a bit confusing in places. I've created a new ticket (#11237 )to make some further improvements. Feel free to make any comments on #11237.
