# Issue 11495: Trac #1092 small memleaks exposed by ntl_ZZ_pE (from 2.8.12.alpha0)

archive/issues_011323.json:
```json
{
    "assignees": [],
    "body": "The patch for ticket #1092 has been reverted by commit 8503, so the (not so) small memleaks are back.\n\nAssignee: **@rlmill**\n\nCC:  mabshoff @wjp jpflori bober\n\nKeywords: **memleak**\n\nAuthor: **Willem Jan Palenstijn**\n\nReviewer: **Jonathan Bober**\n\nMerged: **sage-4.7.2.alpha1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11495_\n\n",
    "closed_at": "2011-07-28T09:56:37Z",
    "created_at": "2011-06-16T00:51:05Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20memleak",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Trac #1092 small memleaks exposed by ntl_ZZ_pE (from 2.8.12.alpha0)",
    "type": "issue",
    "updated_at": "2011-07-28T09:56:37Z",
    "url": "https://github.com/sagemath/sage/issues/11495",
    "user": "https://github.com/sagetrac-jpflori"
}
```
The patch for ticket #1092 has been reverted by commit 8503, so the (not so) small memleaks are back.

Assignee: **@rlmill**

CC:  mabshoff @wjp jpflori bober

Keywords: **memleak**

Author: **Willem Jan Palenstijn**

Reviewer: **Jonathan Bober**

Merged: **sage-4.7.2.alpha1**

_Issue created by migration from https://trac.sagemath.org/ticket/11495_





---

archive/issue_events_139921.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2011-06-16T00:51:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11495",
    "milestone_number": null,
    "milestone_title": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11495#event-139921"
}
```



---

archive/issue_events_139922.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2011-06-16T00:51:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11495",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20memleak",
    "label_color": "000080",
    "label_name": "component: memleak",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11495#event-139922"
}
```



---

archive/issue_events_139923.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2011-06-16T00:51:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11495",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11495#event-139923"
}
```



---

archive/issue_events_139924.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2011-06-16T00:51:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11495",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11495#event-139924"
}
```



---

archive/issue_comments_118861.json:
```json
{
    "body": "original patch for #1092",
    "created_at": "2011-06-16T00:51:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11495",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11495#issuecomment-118861",
    "user": "https://github.com/sagetrac-jpflori"
}
```

original patch for #1092



---

archive/issue_events_139925.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2011-06-24T10:35:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11495",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11495#event-139925"
}
```



---

archive/issue_comments_118862.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[memleak_ZZ_pE_to_ZZ_pX.patch.gz](https://github.com/sagemath/sage/files/ticket11495/memleak_ZZ_pE_to_ZZ_pX.patch.gz)**",
    "created_at": "2011-06-24T10:35:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11495",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11495#issuecomment-118862",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[memleak_ZZ_pE_to_ZZ_pX.patch.gz](https://github.com/sagemath/sage/files/ticket11495/memleak_ZZ_pE_to_ZZ_pX.patch.gz)**



---

archive/issue_comments_118863.json:
```json
{
    "body": "Reviewer: **Jonathan Bober**",
    "created_at": "2011-07-25T19:21:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11495",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11495#issuecomment-118863",
    "user": "https://github.com/sagetrac-bober"
}
```

Reviewer: **Jonathan Bober**



---

archive/issue_events_139926.json:
```json
{
    "actor": "https://github.com/sagetrac-bober",
    "created_at": "2011-07-25T19:21:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11495",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11495#event-139926"
}
```



---

archive/issue_events_139927.json:
```json
{
    "actor": "https://github.com/sagetrac-bober",
    "created_at": "2011-07-25T19:21:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11495",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11495#event-139927"
}
```



---

archive/issue_comments_118864.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThis looks good to me. It is a simple -1 line change that removes the creation of an unnecessary that is never deleted.",
    "created_at": "2011-07-25T19:21:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11495",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11495#issuecomment-118864",
    "user": "https://github.com/sagetrac-bober"
}
```

<a id='comment:2'>Comment 2:</a>
This looks good to me. It is a simple -1 line change that removes the creation of an unnecessary that is never deleted.



---

archive/issue_events_139928.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-27T13:35:44Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11495",
    "milestone_number": null,
    "milestone_title": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11495#event-139928"
}
```



---

archive/issue_events_139929.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-27T13:35:44Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11495",
    "milestone_number": null,
    "milestone_title": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11495#event-139929"
}
```



---

archive/issue_comments_118865.json:
```json
{
    "body": "Changed author from **wjp** to **Willem Jan Palenstijn**",
    "created_at": "2011-07-28T09:56:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11495",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11495#issuecomment-118865",
    "user": "https://github.com/jdemeyer"
}
```

Changed author from **wjp** to **Willem Jan Palenstijn**



---

archive/issue_comments_118866.json:
```json
{
    "body": "Merged: **sage-4.7.2.alpha1**",
    "created_at": "2011-07-28T09:56:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11495",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11495#issuecomment-118866",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.7.2.alpha1**



---

archive/issue_events_139930.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-28T09:56:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11495",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11495#event-139930"
}
```



---

archive/issue_events_139931.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-28T09:56:37Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11495",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11495#event-139931"
}
```
