# Issue 11079: SIGSEGV test throws SIGBUS instead on certain systems

archive/issues_010956.json:
```json
{
    "body": "In the file `sage/tests/interrupt.pyx` there is a test for a segmentation fault by explicitly dereferencing a NULL pointer.  However, on some systems (Mac OS X 10.4 PPC, Mac OS X 10.4 Intel, and Mac OS X 10.5.8 Intel) this generates a SIGBUS instead of a SIGSEGV.\n\nAssignee: mvngu\n\nCC:  @kcrisman\n\nResolution: fixed\n\nAuthor: Jeroen Demeyer\n\nReviewer: Karl-Dieter Crisman\n\nMerged: sage-4.7.alpha4\n\nIssue created by migration from https://trac.sagemath.org/ticket/11079\n\n",
    "closed_at": "2011-04-05T11:59:32Z",
    "created_at": "2011-03-29T13:11:40Z",
    "labels": [
        "component: doctest coverage",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.7",
    "title": "SIGSEGV test throws SIGBUS instead on certain systems",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/11079",
    "user": "https://github.com/jdemeyer"
}
```
In the file `sage/tests/interrupt.pyx` there is a test for a segmentation fault by explicitly dereferencing a NULL pointer.  However, on some systems (Mac OS X 10.4 PPC, Mac OS X 10.4 Intel, and Mac OS X 10.5.8 Intel) this generates a SIGBUS instead of a SIGSEGV.

Assignee: mvngu

CC:  @kcrisman

Resolution: fixed

Author: Jeroen Demeyer

Reviewer: Karl-Dieter Crisman

Merged: sage-4.7.alpha4

Issue created by migration from https://trac.sagemath.org/ticket/11079





---

archive/issue_comments_143213.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2011-03-29T13:27:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11079",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11079#issuecomment-143213",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_143214.json:
```json
{
    "body": "<a id='comment:3'></a>This patch looks correct, and\n\n```\nDasher-03:~/Desktop/sage-4.7.alpha2 student$ ./sage -t -long devel/sage/sage/tests/interrupt.pyx \nsage -t -long \"devel/sage/sage/tests/interrupt.pyx\"         \n         [436.2 s]\n \n----------------------------------------------------------------------\nAll tests passed!\nTotal time for all tests: 436.4 seconds\n```\nPositive review.  Nice (better) way to name those functions, too; now I know what they actually do!",
    "created_at": "2011-03-29T19:16:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11079",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11079#issuecomment-143214",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:3'></a>This patch looks correct, and

```
Dasher-03:~/Desktop/sage-4.7.alpha2 student$ ./sage -t -long devel/sage/sage/tests/interrupt.pyx 
sage -t -long "devel/sage/sage/tests/interrupt.pyx"         
         [436.2 s]
 
----------------------------------------------------------------------
All tests passed!
Total time for all tests: 436.4 seconds
```
Positive review.  Nice (better) way to name those functions, too; now I know what they actually do!



---

archive/issue_comments_143215.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Karl-Dieter Crisman\"",
    "created_at": "2011-03-29T19:16:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11079",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11079#issuecomment-143215",
    "user": "https://github.com/kcrisman"
}
```

Changing reviewer from "" to "Karl-Dieter Crisman"



---

archive/issue_comments_143216.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2011-03-29T19:16:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11079",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11079#issuecomment-143216",
    "user": "https://github.com/kcrisman"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_143217.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-In the file `sage/tests/interrupt.pyx` there is a test for a segmentation fault by explicitly dereferencing a NULL pointer.  However, on some systems (at least Mac OS X 10.4 PPC) this generates a SIGBUS instead of a SIGSEGV.\n+In the file `sage/tests/interrupt.pyx` there is a test for a segmentation fault by explicitly dereferencing a NULL pointer.  However, on some systems (Mac OS X 10.4 PPC and Mac OS X 10.5.8 Intel) this generates a SIGBUS instead of a SIGSEGV.\n \n Author: Jeroen Demeyer\n \n``````\n",
    "created_at": "2011-04-04T09:31:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11079",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11079#issuecomment-143217",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-In the file `sage/tests/interrupt.pyx` there is a test for a segmentation fault by explicitly dereferencing a NULL pointer.  However, on some systems (at least Mac OS X 10.4 PPC) this generates a SIGBUS instead of a SIGSEGV.
+In the file `sage/tests/interrupt.pyx` there is a test for a segmentation fault by explicitly dereferencing a NULL pointer.  However, on some systems (Mac OS X 10.4 PPC and Mac OS X 10.5.8 Intel) this generates a SIGBUS instead of a SIGSEGV.
 
 Author: Jeroen Demeyer
 
``````




---

archive/issue_comments_143218.json:
```json
{
    "body": "<a id='comment:5'></a>I see this on MacIntel OS X 10.4.11, too, so I updated the description. The patch looks superb (although I didn't apply and test it myself); hopefully it gets into the next alpha/rc release.",
    "created_at": "2011-04-04T18:28:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11079",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11079#issuecomment-143218",
    "user": "https://trac.sagemath.org/admin/accounts/users/GeorgSWeber"
}
```

<a id='comment:5'></a>I see this on MacIntel OS X 10.4.11, too, so I updated the description. The patch looks superb (although I didn't apply and test it myself); hopefully it gets into the next alpha/rc release.



---

archive/issue_comments_143219.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-\n+In the file `sage/tests/interrupt.pyx` there is a test for a segmentation fault by explicitly dereferencing a NULL pointer.  However, on some systems (Mac OS X 10.4 PPC, Mac OS X 10.4 Intel, and Mac OS X 10.5.8 Intel) this generates a SIGBUS instead of a SIGSEGV.\n \n Author: Jeroen Demeyer\n \n``````\n",
    "created_at": "2011-04-04T18:28:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11079",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11079#issuecomment-143219",
    "user": "https://trac.sagemath.org/admin/accounts/users/GeorgSWeber"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-
+In the file `sage/tests/interrupt.pyx` there is a test for a segmentation fault by explicitly dereferencing a NULL pointer.  However, on some systems (Mac OS X 10.4 PPC, Mac OS X 10.4 Intel, and Mac OS X 10.5.8 Intel) this generates a SIGBUS instead of a SIGSEGV.
 
 Author: Jeroen Demeyer
 
``````




---

archive/issue_comments_143220.json:
```json
{
    "body": "<a id='comment:6'></a>Attachment [11079_sigsegv_test.patch](tarball://root/attachments/some-uuid/ticket11079/11079_sigsegv_test.patch) by @jdemeyer created at 2011-04-04 20:46:44\n\nUpdated patch.  Only change is this comment (changed to reflect that various Mac OS X systems have this issue):\n\n```\n# Dereference a NULL pointer on purpose. This signals a SIGSEGV on most\n# systems, but on older Mac OS X and possibly other systems, this\n# signals a SIGBUS instead. In any case, this should give some signal.\n```",
    "created_at": "2011-04-04T20:46:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11079",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11079#issuecomment-143220",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:6'></a>Attachment [11079_sigsegv_test.patch](tarball://root/attachments/some-uuid/ticket11079/11079_sigsegv_test.patch) by @jdemeyer created at 2011-04-04 20:46:44

Updated patch.  Only change is this comment (changed to reflect that various Mac OS X systems have this issue):

```
# Dereference a NULL pointer on purpose. This signals a SIGSEGV on most
# systems, but on older Mac OS X and possibly other systems, this
# signals a SIGBUS instead. In any case, this should give some signal.
```



---

archive/issue_comments_143221.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2011-04-05T11:59:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11079",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11079#issuecomment-143221",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed



---

archive/issue_events_028740.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-04-05T11:59:32Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/11079",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11079#event-28740"
}
```



---

archive/issue_comments_143222.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-4.7.alpha4\"",
    "created_at": "2011-04-05T11:59:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11079",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11079#issuecomment-143222",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-4.7.alpha4"
