...
```json
{
    "body": "Can we just write a very lightweight wrapper around the python min() function which behaves symbolically if given symbolic input, but otherwise is just like the python function?  Similar to how we have a lightweight wrapper around the sum function?",
...
```json
{
    "body": "Seems okay, though I haven't tested this in three years, as you can see :)",
...
```json
{
    "body": "panda314, I can't find your handle on the wiki homepage.",
    "created_at": "2017-03-14T21:17:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11258",
    "type": "issue_comment",
...
```json
{
    "body": "The point is that we still need an actual \"real author\" name to put on this, sorry for the confusion.",
...
```json
{
    "body": "Sorry for causing this confusion. It is customary to put the real names of all the authors in that field, not just the handles. It would also be nice if you could add the handles and real names to the list of names on the wiki homepage.",
...
```json
{
    "body": "Yes, you could set it back to needs review. However, your changes now contain some merge markers `<<<<< HEAD`. I guess that someone you did not fully resolve merge conflicts.",
    "created_at": "2017-03-15T16:09:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11258",
    "type": "issue_comment",
...
```json
{
    "body": "if i click on the branch (`u/asutosh7hota/add_documentation_for_plotting_max_and_min_functions`), it displays like a huge number of deletions in `plot.py`. why? doesn't look correct, right?",
...
```json
{
    "body": "also there is a plot block (`.. PLOT::`) which allows to display plots in the doc ([example](http://doc.sagemath.org/html/en/reference/plotting/sage/plot/plot.html)). perhaps this shall be used too in your new examples at `min_max.py`??",
...
```json
{
    "body": "one naive question: is there a context where Python's built-in function is used for 1 or more symbolic arguments? i mean, the 1st example in the class doc refers to: `max(x, x^2) = x`. is it a bug or a feature?",
