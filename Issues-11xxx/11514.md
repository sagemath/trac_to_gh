# Issue 11514: Add lots of tests for RealDistributions

archive/issues_011342.json:
```json
{
    "assignees": [],
    "body": "We need more tests in `gsl/probability_distribution.pyx`.\n\nSee for instance #9080 - there are lots of methods which have minimal tests, and lots of error messages not tested.  This should be fixed.\n\nDepends on #11572\n\nAssignee: **@sagetrac-mvngu**\n\nCC:  @kwankyu @sagetrac-Kamhamea\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11514_\n\n",
    "created_at": "2011-06-17T00:34:19Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add lots of tests for RealDistributions",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/11514",
    "user": "https://github.com/kcrisman"
}
```
We need more tests in `gsl/probability_distribution.pyx`.

See for instance #9080 - there are lots of methods which have minimal tests, and lots of error messages not tested.  This should be fixed.

Depends on #11572

Assignee: **@sagetrac-mvngu**

CC:  @kwankyu @sagetrac-Kamhamea

_Issue created by migration from https://trac.sagemath.org/ticket/11514_





---

archive/issue_events_140261.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-06-17T00:34:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11514",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11514#event-140261"
}
```



---

archive/issue_events_140262.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-06-17T00:34:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11514",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11514#event-140262"
}
```



---

archive/issue_events_140263.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-06-17T00:34:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11514",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11514#event-140263"
}
```



---

archive/issue_events_140264.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-06-17T00:35:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11514",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11514#event-140264"
}
```



---

archive/issue_events_140265.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-06-17T00:35:51Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/11514",
    "title_is": "Add lots of tests for RealDistributions",
    "title_was": "Add lots of tests for !RealDistributions",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11514#event-140265"
}
```



---

archive/issue_comments_119208.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nIt seems like at least some of this is implemented at #11572, though probably not all.  In particular, the TypeErrors raised are not tested.",
    "created_at": "2011-07-05T13:30:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11514",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11514#issuecomment-119208",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:3" align="right">Comment 3</div>

It seems like at least some of this is implemented at #11572, though probably not all.  In particular, the TypeErrors raised are not tested.



---

archive/issue_comments_119209.json:
```json
{
    "body": "Dependencies: **#11572**",
    "created_at": "2013-02-03T17:36:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11514",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11514#issuecomment-119209",
    "user": "https://github.com/KPanComputes"
}
```

Dependencies: **#11572**



---

archive/issue_comments_119210.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nReplying to [@kcrisman](#comment%3A3):\n> It seems like at least some of this is implemented at #11572, though probably not all.  In particular, the TypeErrors raised are not tested.\n\nI have added the mentioned ticket as a dependency so that work done is not duplicated.",
    "created_at": "2013-02-03T17:37:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11514",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11514#issuecomment-119210",
    "user": "https://github.com/KPanComputes"
}
```

<div id="comment:5" align="right">Comment 5</div>

Replying to [@kcrisman](#comment%3A3):
> It seems like at least some of this is implemented at #11572, though probably not all.  In particular, the TypeErrors raised are not tested.

I have added the mentioned ticket as a dependency so that work done is not duplicated.



---

archive/issue_events_140266.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11514",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11514#event-140266"
}
```



---

archive/issue_events_140267.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11514",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11514#event-140267"
}
```



---

archive/issue_events_140268.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11514",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11514#event-140268"
}
```



---

archive/issue_events_140269.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11514",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11514#event-140269"
}
```



---

archive/issue_events_140270.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11514",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11514#event-140270"
}
```



---

archive/issue_events_140271.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11514",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11514#event-140271"
}
```



---

archive/issue_events_140272.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11514",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11514#event-140272"
}
```



---

archive/issue_events_140273.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11514",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11514#event-140273"
}
```



---

archive/issue_events_140274.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11514",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11514#event-140274"
}
```
