# Issue 11417: binomial of polynomial is not polynomial

archive/issues_011245.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: P=PolynomialRing(RR,'z')\nsage: z=P.gen()\nsage: binomial(z,2)                       \n(z^2 - z)/2.00000000000000\n```\nOne would expect here to get the polynomial `0.5*z^2-0.5*z`\n\nApply [attachment: 11417.3.patch.gz](https://github.com/sagemath/sage/files/ticket11417/11417.3.patch.gz)\n\nAssignee: **@aghitza**\n\nKeywords: **sd35**\n\nAuthor: **Sam Scott**\n\nReviewer: **Johan Bosman**\n\nMerged: **sage-4.8.alpha6**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11417_\n\n",
    "closed_at": "2011-12-29T07:02:58Z",
    "created_at": "2011-05-31T20:39:05Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "binomial of polynomial is not polynomial",
    "type": "issue",
    "updated_at": "2012-01-08T23:52:37Z",
    "url": "https://github.com/sagemath/sage/issues/11417",
    "user": "https://github.com/sagetrac-Henryk-Trappmann"
}
```

```
sage: P=PolynomialRing(RR,'z')
sage: z=P.gen()
sage: binomial(z,2)                       
(z^2 - z)/2.00000000000000
```
One would expect here to get the polynomial `0.5*z^2-0.5*z`

Apply [attachment: 11417.3.patch.gz](https://github.com/sagemath/sage/files/ticket11417/11417.3.patch.gz)

Assignee: **@aghitza**

Keywords: **sd35**

Author: **Sam Scott**

Reviewer: **Johan Bosman**

Merged: **sage-4.8.alpha6**

_Issue created by migration from https://trac.sagemath.org/ticket/11417_





---

archive/issue_events_138700.json:
```json
{
    "actor": "https://github.com/sagetrac-Henryk-Trappmann",
    "created_at": "2011-05-31T20:39:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11417",
    "milestone_number": null,
    "milestone_title": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11417#event-138700"
}
```



---

archive/issue_events_138701.json:
```json
{
    "actor": "https://github.com/sagetrac-Henryk-Trappmann",
    "created_at": "2011-05-31T20:39:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11417",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "component: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11417#event-138701"
}
```



---

archive/issue_events_138702.json:
```json
{
    "actor": "https://github.com/sagetrac-Henryk-Trappmann",
    "created_at": "2011-05-31T20:39:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11417",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11417#event-138702"
}
```



---

archive/issue_events_138703.json:
```json
{
    "actor": "https://github.com/sagetrac-Henryk-Trappmann",
    "created_at": "2011-05-31T20:39:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11417",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11417#event-138703"
}
```



---

archive/issue_comments_117678.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,7 +1,8 @@\n-sage: P=PolynomialRing(RR,'z')                           \n-sage: z=P.gen()                                \n-sage: binomial(z,2)                      \n+\n+```\n+sage: P=PolynomialRing(RR,'z')\n+sage: z=P.gen()\n+sage: binomial(z,2)                       \n (z^2 - z)/2.00000000000000\n-\n-On would expect here to get the polynomial\n-0.5*z^2-0.5*z\n+```\n+One would expect here to get the polynomial `0.5*z^2-0.5*z`\n``````\n",
    "created_at": "2011-06-01T05:56:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11417",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11417#issuecomment-117678",
    "user": "https://github.com/sagetrac-fwclarke"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,7 +1,8 @@
-sage: P=PolynomialRing(RR,'z')                           
-sage: z=P.gen()                                
-sage: binomial(z,2)                      
+
+```
+sage: P=PolynomialRing(RR,'z')
+sage: z=P.gen()
+sage: binomial(z,2)                       
 (z^2 - z)/2.00000000000000
-
-On would expect here to get the polynomial
-0.5*z^2-0.5*z
+```
+One would expect here to get the polynomial `0.5*z^2-0.5*z`
``````




---

archive/issue_comments_117679.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nContinuing,\n\n```\nsage: b = binomial(z, 2)\nsage: b in P\nTrue\nsage: b.parent()\nFraction Field of Univariate Polynomial Ring in z over Real Field with 53 bits of precision\nsage: P(b)\n0.500000000000000*z^2 - 0.500000000000000*z\n```",
    "created_at": "2011-06-01T05:56:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11417",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11417#issuecomment-117679",
    "user": "https://github.com/sagetrac-fwclarke"
}
```

<a id='comment:1'>Comment 1:</a>
Continuing,

```
sage: b = binomial(z, 2)
sage: b in P
True
sage: b.parent()
Fraction Field of Univariate Polynomial Ring in z over Real Field with 53 bits of precision
sage: P(b)
0.500000000000000*z^2 - 0.500000000000000*z
```



---

archive/issue_comments_117680.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThe attached patch changes functionality of the binomial function of a polynomial to return a polynomial over a fraction field of the base ring.\n\nWith the patch, we get the following behaviour:\n\n```\nsage: P=PolynomialRing(RR,'z')\nsage: z=P.gen()\nsage: binomial(z,2)                       \n0.500000000000000*z^2 - 0.500000000000000*z\n\nsage: R.<x> = ZZ[]\nsage: binomial(x,3)\n1/6*x^3 - 1/2*x^2 + 1/3*x\nbinomial(x,3).parent()\nUnivariate Polynomial Ring in x over Rational Field\n}}\n\nWhich is more like what we would expect.",
    "created_at": "2011-12-18T18:02:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11417",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11417#issuecomment-117680",
    "user": "https://github.com/sagetrac-scotts"
}
```

<a id='comment:2'>Comment 2:</a>
The attached patch changes functionality of the binomial function of a polynomial to return a polynomial over a fraction field of the base ring.

With the patch, we get the following behaviour:

```
sage: P=PolynomialRing(RR,'z')
sage: z=P.gen()
sage: binomial(z,2)                       
0.500000000000000*z^2 - 0.500000000000000*z

sage: R.<x> = ZZ[]
sage: binomial(x,3)
1/6*x^3 - 1/2*x^2 + 1/3*x
binomial(x,3).parent()
Univariate Polynomial Ring in x over Rational Field
}}

Which is more like what we would expect.



---

archive/issue_events_138704.json:
```json
{
    "actor": "https://github.com/sagetrac-scotts",
    "created_at": "2011-12-18T18:11:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11417",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11417#event-138704"
}
```



---

archive/issue_comments_117681.json:
```json
{
    "body": "Attachment: **[11417.patch.gz](https://github.com/sagemath/sage/files/ticket11417/11417.patch.gz)**",
    "created_at": "2011-12-18T20:42:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11417",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11417#issuecomment-117681",
    "user": "https://github.com/sagetrac-scotts"
}
```

Attachment: **[11417.patch.gz](https://github.com/sagemath/sage/files/ticket11417/11417.patch.gz)**



---

archive/issue_comments_117682.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\n(Patch was uploaded again due to issues with the Windows line-break character...)",
    "created_at": "2011-12-18T20:43:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11417",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11417#issuecomment-117682",
    "user": "https://github.com/sagetrac-scotts"
}
```

<a id='comment:4'>Comment 4:</a>
(Patch was uploaded again due to issues with the Windows line-break character...)



---

archive/issue_comments_117683.json:
```json
{
    "body": "Attachment: **[11417.2.patch.gz](https://github.com/sagemath/sage/files/ticket11417/11417.2.patch.gz)**",
    "created_at": "2011-12-20T01:40:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11417",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11417#issuecomment-117683",
    "user": "https://github.com/sagetrac-scotts"
}
```

Attachment: **[11417.2.patch.gz](https://github.com/sagemath/sage/files/ticket11417/11417.2.patch.gz)**



---

archive/issue_comments_117684.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nMuch simpler patch, same effect but much quicker.",
    "created_at": "2011-12-20T01:41:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11417",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11417#issuecomment-117684",
    "user": "https://github.com/sagetrac-scotts"
}
```

<a id='comment:5'>Comment 5:</a>
Much simpler patch, same effect but much quicker.



---

archive/issue_comments_117685.json:
```json
{
    "body": "Author: **Sam Scott**",
    "created_at": "2011-12-20T13:25:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11417",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11417#issuecomment-117685",
    "user": "https://github.com/sagetrac-scotts"
}
```

Author: **Sam Scott**



---

archive/issue_events_138705.json:
```json
{
    "actor": "https://github.com/sagetrac-johanbosman",
    "created_at": "2011-12-21T13:49:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11417",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11417#event-138705"
}
```



---

archive/issue_events_138706.json:
```json
{
    "actor": "https://github.com/sagetrac-johanbosman",
    "created_at": "2011-12-21T13:49:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11417",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11417#event-138706"
}
```



---

archive/issue_comments_117686.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -6,3 +6,5 @@\n (z^2 - z)/2.00000000000000\n ```\n One would expect here to get the polynomial `0.5*z^2-0.5*z`\n+\n+Apply [attachment: 11417.2.patch.gz](https://github.com/sagemath/sage/files/ticket11417/11417.2.patch.gz)\n``````\n",
    "created_at": "2011-12-21T13:49:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11417",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11417#issuecomment-117686",
    "user": "https://github.com/sagetrac-johanbosman"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -6,3 +6,5 @@
 (z^2 - z)/2.00000000000000
 ```
 One would expect here to get the polynomial `0.5*z^2-0.5*z`
+
+Apply [attachment: 11417.2.patch.gz](https://github.com/sagemath/sage/files/ticket11417/11417.2.patch.gz)
``````




---

archive/issue_comments_117687.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nIn the documentation, the line\n\n```\nTest symbolic and uni/multivariate polynomials:\n```\nshould end in two colons.  This is used to generate nice html code.",
    "created_at": "2011-12-21T13:49:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11417",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11417#issuecomment-117687",
    "user": "https://github.com/sagetrac-johanbosman"
}
```

<a id='comment:7'>Comment 7:</a>
In the documentation, the line

```
Test symbolic and uni/multivariate polynomials:
```
should end in two colons.  This is used to generate nice html code.



---

archive/issue_comments_117688.json:
```json
{
    "body": "Changed keywords from **** to **sd35**",
    "created_at": "2011-12-21T15:02:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11417",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11417#issuecomment-117688",
    "user": "https://github.com/sagetrac-johanbosman"
}
```

Changed keywords from **** to **sd35**



---

archive/issue_comments_117689.json:
```json
{
    "body": "Attachment: **[11417.3.patch.gz](https://github.com/sagemath/sage/files/ticket11417/11417.3.patch.gz)**\n\nreviewer patch",
    "created_at": "2011-12-21T21:29:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11417",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11417#issuecomment-117689",
    "user": "https://github.com/sagetrac-johanbosman"
}
```

Attachment: **[11417.3.patch.gz](https://github.com/sagemath/sage/files/ticket11417/11417.3.patch.gz)**

reviewer patch



---

archive/issue_comments_117690.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -7,4 +7,4 @@\n ```\n One would expect here to get the polynomial `0.5*z^2-0.5*z`\n \n-Apply [attachment: 11417.2.patch.gz](https://github.com/sagemath/sage/files/ticket11417/11417.2.patch.gz)\n+Apply [attachment: 11417.3.patch.gz](https://github.com/sagemath/sage/files/ticket11417/11417.3.patch.gz)\n``````\n",
    "created_at": "2011-12-21T21:34:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11417",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11417#issuecomment-117690",
    "user": "https://github.com/sagetrac-johanbosman"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -7,4 +7,4 @@
 ```
 One would expect here to get the polynomial `0.5*z^2-0.5*z`
 
-Apply [attachment: 11417.2.patch.gz](https://github.com/sagemath/sage/files/ticket11417/11417.2.patch.gz)
+Apply [attachment: 11417.3.patch.gz](https://github.com/sagemath/sage/files/ticket11417/11417.3.patch.gz)
``````




---

archive/issue_events_138707.json:
```json
{
    "actor": "https://github.com/sagetrac-johanbosman",
    "created_at": "2011-12-21T21:34:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11417",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11417#event-138707"
}
```



---

archive/issue_events_138708.json:
```json
{
    "actor": "https://github.com/sagetrac-johanbosman",
    "created_at": "2011-12-21T21:34:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11417",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11417#event-138708"
}
```



---

archive/issue_comments_117691.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nHaving added this change, I give it a positive review.",
    "created_at": "2011-12-21T21:34:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11417",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11417#issuecomment-117691",
    "user": "https://github.com/sagetrac-johanbosman"
}
```

<a id='comment:9'>Comment 9:</a>
Having added this change, I give it a positive review.



---

archive/issue_comments_117692.json:
```json
{
    "body": "Reviewer: **Johan Bosman**",
    "created_at": "2011-12-22T09:15:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11417",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11417#issuecomment-117692",
    "user": "https://github.com/sagetrac-johanbosman"
}
```

Reviewer: **Johan Bosman**



---

archive/issue_comments_117693.json:
```json
{
    "body": "Merged: **sage-4.8.alpha6**",
    "created_at": "2011-12-29T07:02:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11417",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11417#issuecomment-117693",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.8.alpha6**



---

archive/issue_events_138709.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-12-29T07:02:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11417",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11417#event-138709"
}
```



---

archive/issue_events_138710.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-12-29T07:02:58Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11417",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11417#event-138710"
}
```



---

archive/issue_comments_117694.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nThanks for that. Sorry for not replying, I was away without an internet connection.",
    "created_at": "2012-01-08T23:52:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11417",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11417#issuecomment-117694",
    "user": "https://github.com/sagetrac-scotts"
}
```

<a id='comment:12'>Comment 12:</a>
Thanks for that. Sorry for not replying, I was away without an internet connection.
