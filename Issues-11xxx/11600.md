# Issue 11600: Add alias for free module morphism lift method

archive/issues_011428.json:
```json
{
    "assignees": [],
    "body": "Adds `preimage_representative` as an alias for `lift` on free module morphisms.  Detailed discussion at:\n\nhttp://groups.google.com/group/sage-devel/browse_thread/thread/ae16e379460cc9a\n\n**Apply**:\n1. [attachment: trac_11600-alias-for-morphism-lift.patch.gz](https://github.com/sagemath/sage/files/ticket11600/trac_11600-alias-for-morphism-lift.patch.gz)\n\nAssignee: **@jasongrout, @williamstein**\n\nAuthor: **Rob Beezer**\n\nReviewer: **Martin Raum**\n\nMerged: **sage-4.7.2.alpha2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11600_\n\n",
    "closed_at": "2011-08-18T22:04:41Z",
    "created_at": "2011-07-14T23:07:47Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/good%20first%20issue"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add alias for free module morphism lift method",
    "type": "issue",
    "updated_at": "2011-08-18T22:04:41Z",
    "url": "https://github.com/sagemath/sage/issues/11600",
    "user": "https://github.com/rbeezer"
}
```
Adds `preimage_representative` as an alias for `lift` on free module morphisms.  Detailed discussion at:

http://groups.google.com/group/sage-devel/browse_thread/thread/ae16e379460cc9a

**Apply**:
1. [attachment: trac_11600-alias-for-morphism-lift.patch.gz](https://github.com/sagemath/sage/files/ticket11600/trac_11600-alias-for-morphism-lift.patch.gz)

Assignee: **@jasongrout, @williamstein**

Author: **Rob Beezer**

Reviewer: **Martin Raum**

Merged: **sage-4.7.2.alpha2**

_Issue created by migration from https://trac.sagemath.org/ticket/11600_





---

archive/issue_events_141873.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-07-14T23:07:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11600",
    "milestone_number": null,
    "milestone_title": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11600#event-141873"
}
```



---

archive/issue_events_141874.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-07-14T23:07:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11600",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11600#event-141874"
}
```



---

archive/issue_events_141875.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-07-14T23:07:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11600",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11600#event-141875"
}
```



---

archive/issue_events_141876.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-07-14T23:07:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11600",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11600#event-141876"
}
```



---

archive/issue_comments_121352.json:
```json
{
    "body": "Attachment: **[trac_11600-alias-for-morphism-lift.patch.gz](https://github.com/sagemath/sage/files/ticket11600/trac_11600-alias-for-morphism-lift.patch.gz)**",
    "created_at": "2011-07-14T23:10:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11600",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11600#issuecomment-121352",
    "user": "https://github.com/rbeezer"
}
```

Attachment: **[trac_11600-alias-for-morphism-lift.patch.gz](https://github.com/sagemath/sage/files/ticket11600/trac_11600-alias-for-morphism-lift.patch.gz)**



---

archive/issue_events_141877.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-07-14T23:15:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11600",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11600#event-141877"
}
```



---

archive/issue_comments_121353.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,6 @@\n Adds `preimage_representative` as an alias for `lift` on free module morphisms.  Detailed discussion at:\n \n http://groups.google.com/group/sage-devel/browse_thread/thread/ae16e379460cc9a\n+\n+**Apply**:\n+1. [attachment: trac_11600-alias-for-morphism-lift.patch.gz](https://github.com/sagemath/sage/files/ticket11600/trac_11600-alias-for-morphism-lift.patch.gz)\n``````\n",
    "created_at": "2011-07-14T23:15:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11600",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11600#issuecomment-121353",
    "user": "https://github.com/rbeezer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,6 @@
 Adds `preimage_representative` as an alias for `lift` on free module morphisms.  Detailed discussion at:
 
 http://groups.google.com/group/sage-devel/browse_thread/thread/ae16e379460cc9a
+
+**Apply**:
+1. [attachment: trac_11600-alias-for-morphism-lift.patch.gz](https://github.com/sagemath/sage/files/ticket11600/trac_11600-alias-for-morphism-lift.patch.gz)
``````




---

archive/issue_comments_121354.json:
```json
{
    "body": "Author: **Rob Beezer**",
    "created_at": "2011-07-14T23:15:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11600",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11600#issuecomment-121354",
    "user": "https://github.com/rbeezer"
}
```

Author: **Rob Beezer**



---

archive/issue_events_141878.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-07-14T23:15:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11600",
    "label": "https://github.com/sagemath/sage/labels/good%20first%20issue",
    "label_color": "7057ff",
    "label_name": "good first issue",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11600#event-141878"
}
```



---

archive/issue_comments_121355.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nThe summary docstring for this method is wrong, and the rest of the docstring could use some help.  I have that on another patch I am preparing with some miscellaneous fixes.  So I have tried to keep this focused on the name addition.",
    "created_at": "2011-07-14T23:15:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11600",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11600#issuecomment-121355",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:1'>Comment 1:</a>
The summary docstring for this method is wrong, and the rest of the docstring could use some help.  I have that on another patch I am preparing with some miscellaneous fixes.  So I have tried to keep this focused on the name addition.



---

archive/issue_comments_121356.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThis is nice and everything is correct.",
    "created_at": "2011-08-16T17:58:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11600",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11600#issuecomment-121356",
    "user": "https://github.com/sagetrac-mraum"
}
```

<a id='comment:2'>Comment 2:</a>
This is nice and everything is correct.



---

archive/issue_comments_121357.json:
```json
{
    "body": "Reviewer: **Martin Raum**",
    "created_at": "2011-08-16T17:58:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11600",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11600#issuecomment-121357",
    "user": "https://github.com/sagetrac-mraum"
}
```

Reviewer: **Martin Raum**



---

archive/issue_events_141879.json:
```json
{
    "actor": "https://github.com/sagetrac-mraum",
    "created_at": "2011-08-16T17:58:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11600",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11600#event-141879"
}
```



---

archive/issue_events_141880.json:
```json
{
    "actor": "https://github.com/sagetrac-mraum",
    "created_at": "2011-08-16T17:58:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11600",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11600#event-141880"
}
```



---

archive/issue_events_141881.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-08-18T22:04:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11600",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11600#event-141881"
}
```



---

archive/issue_events_141882.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-08-18T22:04:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11600",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11600#event-141882"
}
```



---

archive/issue_comments_121358.json:
```json
{
    "body": "Merged: **sage-4.7.2.alpha2**",
    "created_at": "2011-08-18T22:04:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11600",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11600#issuecomment-121358",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.7.2.alpha2**
