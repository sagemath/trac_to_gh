# Issue 11432: LLL function for quadratic forms is wrong

archive/issues_011260.json:
```json
{
    "assignees": [],
    "body": "The function Q.lll() where Q is a QuadraticForm is a one-line wrapper around LLL_gram(), which in turn calls Pari.\n\nThe description of Q.lll() is that it returns an LLL-reduced quadratic form. However, the function LLL_gram() returns a change of basis matrix, not the resulting quadratic form.\n\nIn my opinion this requires a one-line patch, which I can do if someone confirms that this is the problem.\n\nAssignee: **@mmasdeu**\n\nCC:  @tornaria\n\nKeywords: **LLL, quadratic form**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11432_\n\n",
    "closed_at": "2014-06-23T20:35:56Z",
    "created_at": "2011-06-06T11:42:48Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20quadratic%20forms",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "LLL function for quadratic forms is wrong",
    "type": "issue",
    "updated_at": "2014-06-23T20:35:56Z",
    "url": "https://github.com/sagemath/sage/issues/11432",
    "user": "https://github.com/mmasdeu"
}
```
The function Q.lll() where Q is a QuadraticForm is a one-line wrapper around LLL_gram(), which in turn calls Pari.

The description of Q.lll() is that it returns an LLL-reduced quadratic form. However, the function LLL_gram() returns a change of basis matrix, not the resulting quadratic form.

In my opinion this requires a one-line patch, which I can do if someone confirms that this is the problem.

Assignee: **@mmasdeu**

CC:  @tornaria

Keywords: **LLL, quadratic form**

_Issue created by migration from https://trac.sagemath.org/ticket/11432_





---

archive/issue_events_138947.json:
```json
{
    "actor": "https://github.com/mmasdeu",
    "created_at": "2011-06-06T11:42:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11432",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11432#event-138947"
}
```



---

archive/issue_events_138948.json:
```json
{
    "actor": "https://github.com/mmasdeu",
    "created_at": "2011-06-06T11:42:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11432",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20quadratic%20forms",
    "label_color": "0000ff",
    "label_name": "component: quadratic forms",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11432#event-138948"
}
```



---

archive/issue_events_138949.json:
```json
{
    "actor": "https://github.com/mmasdeu",
    "created_at": "2011-06-06T11:42:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11432",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11432#event-138949"
}
```



---

archive/issue_events_138950.json:
```json
{
    "actor": "https://github.com/mmasdeu",
    "created_at": "2011-06-06T11:42:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11432",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11432#event-138950"
}
```



---

archive/issue_events_138951.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11432",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11432#event-138951"
}
```



---

archive/issue_events_138952.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11432",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11432#event-138952"
}
```



---

archive/issue_events_138953.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11432",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11432#event-138953"
}
```



---

archive/issue_events_138954.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11432",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11432#event-138954"
}
```



---

archive/issue_comments_117880.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nI was wrong. When (3 years ago!) I opened the ticket, I failed to realize that one can evaluate a quadratic form on a matrix, and indeed this is what the lll() (rightly) does. I am closing the ticket.",
    "created_at": "2014-03-27T14:51:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11432#issuecomment-117880",
    "user": "https://github.com/mmasdeu"
}
```

<div id="comment:3" align="right">Comment 3</div>

I was wrong. When (3 years ago!) I opened the ticket, I failed to realize that one can evaluate a quadratic form on a matrix, and indeed this is what the lll() (rightly) does. I am closing the ticket.



---

archive/issue_comments_117881.json:
```json
{
    "body": "Changed assignee from **@sagetrac-justin** to **@mmasdeu**",
    "created_at": "2014-03-27T14:53:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11432#issuecomment-117881",
    "user": "https://github.com/mmasdeu"
}
```

Changed assignee from **@sagetrac-justin** to **@mmasdeu**



---

archive/issue_events_138955.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11432",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11432#event-138955"
}
```



---

archive/issue_events_138956.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11432",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11432#event-138956"
}
```



---

archive/issue_events_138957.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-06-23T20:35:56Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11432",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11432#event-138957"
}
```



---

archive/issue_events_138958.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-06-23T20:35:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11432",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11432#event-138958"
}
```



---

archive/issue_events_138959.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-06-23T20:35:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11432",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11432#event-138959"
}
```
