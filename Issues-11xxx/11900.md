...
```json
{
    "body": "<a id='comment:25'></a>\nImplementing the new coercion for libsingular will not be easy, but it seems that it will pay off: It seems to me that some tests in `_coerce_c_`impl are done each time an element is coerced, rather than once, just for the parent of that element.",
