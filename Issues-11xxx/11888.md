# Issue 11888: Sage is missing the lambert_w function

archive/issues_011716.json:
```json
{
    "assignees": [],
    "body": "Maxima returns solutions to some exponential equations in terms of the `lambert_w` function. Sage is missing a conversion for this function:\n\n```\n\nsage: solve(e^(5*x)+x==0, x, to_poly_solve=True)\n[x == -1/5*lambert_w(5)]\nsage: S = solve(e^(5*x)+x==0, x, to_poly_solve=True)\nsage: z = S[0].rhs()\nsage: z\n-1/5*lambert_w(5)\nsage: N(z)\n---------------------------------------------------------------------------\nTypeError                                 Traceback (most recent call last)\n\n/Users/jonesbe/sage/sage-4.7.2.alpha2/devel/sage-test/sage/<ipython console> in <module>()\n\n/Users/jonesbe/sage/latest/local/lib/python2.6/site-packages/sage/misc/functional.pyc in numerical_approx(x, prec, digits)\n   1264             prec = int((digits+1) * 3.32192) + 1\n   1265     try:\n-> 1266         return x._numerical_approx(prec)\n   1267     except AttributeError:\n   1268         from sage.rings.complex_double import is_ComplexDoubleElement\n\n/Users/jonesbe/sage/latest/local/lib/python2.6/site-packages/sage/symbolic/expression.so in sage.symbolic.expression.Expression._numerical_approx (sage/symbolic/expression.cpp:17950)()\n\nTypeError: cannot evaluate symbolic expression numerically\nsage: lambert_w(5)\n---------------------------------------------------------------------------\nNameError                                 Traceback (most recent call last)\n\n/Users/jonesbe/sage/sage-4.7.2.alpha2/devel/sage-test/sage/<ipython console> in <module>()\n\nNameError: name 'lambert_w' is not defined\nsage: \n```\n\n`mpmath` can evaluate the `lambert_w` function, so it should be easy to add a new symbolic function to Sage that will fix this issue. \n\n---\n\nApply:\n\n* [attachment: trac_11888_v8.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v8.patch.gz) to `$SAGE_ROOT/devel/sage`\n\nAssignee: **@burcin**\n\nCC:  @kcrisman ktkohl\n\nKeywords: **lambert_w symbolics conversion maxima sd35.5 sd40.5**\n\nReviewer: **Keshav Kini, Karl-Dieter Crisman, Fredrik Johansson, Burcin Erocal, Douglas McNeil, William Stein**\n\nAuthor: **Benjamin Jones**\n\nMerged: **sage-5.1.beta4**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11888_\n\n",
    "closed_at": "2012-06-14T06:38:22Z",
    "created_at": "2011-10-01T23:04:08Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Sage is missing the lambert_w function",
    "type": "issue",
    "updated_at": "2012-06-14T06:38:22Z",
    "url": "https://github.com/sagemath/sage/issues/11888",
    "user": "https://github.com/benjaminfjones"
}
```
Maxima returns solutions to some exponential equations in terms of the `lambert_w` function. Sage is missing a conversion for this function:

```

sage: solve(e^(5*x)+x==0, x, to_poly_solve=True)
[x == -1/5*lambert_w(5)]
sage: S = solve(e^(5*x)+x==0, x, to_poly_solve=True)
sage: z = S[0].rhs()
sage: z
-1/5*lambert_w(5)
sage: N(z)
---------------------------------------------------------------------------
TypeError                                 Traceback (most recent call last)

/Users/jonesbe/sage/sage-4.7.2.alpha2/devel/sage-test/sage/<ipython console> in <module>()

/Users/jonesbe/sage/latest/local/lib/python2.6/site-packages/sage/misc/functional.pyc in numerical_approx(x, prec, digits)
   1264             prec = int((digits+1) * 3.32192) + 1
   1265     try:
-> 1266         return x._numerical_approx(prec)
   1267     except AttributeError:
   1268         from sage.rings.complex_double import is_ComplexDoubleElement

/Users/jonesbe/sage/latest/local/lib/python2.6/site-packages/sage/symbolic/expression.so in sage.symbolic.expression.Expression._numerical_approx (sage/symbolic/expression.cpp:17950)()

TypeError: cannot evaluate symbolic expression numerically
sage: lambert_w(5)
---------------------------------------------------------------------------
NameError                                 Traceback (most recent call last)

/Users/jonesbe/sage/sage-4.7.2.alpha2/devel/sage-test/sage/<ipython console> in <module>()

NameError: name 'lambert_w' is not defined
sage: 
```

`mpmath` can evaluate the `lambert_w` function, so it should be easy to add a new symbolic function to Sage that will fix this issue. 

---

Apply:

* [attachment: trac_11888_v8.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v8.patch.gz) to `$SAGE_ROOT/devel/sage`

Assignee: **@burcin**

CC:  @kcrisman ktkohl

Keywords: **lambert_w symbolics conversion maxima sd35.5 sd40.5**

Reviewer: **Keshav Kini, Karl-Dieter Crisman, Fredrik Johansson, Burcin Erocal, Douglas McNeil, William Stein**

Author: **Benjamin Jones**

Merged: **sage-5.1.beta4**

_Issue created by migration from https://trac.sagemath.org/ticket/11888_





---

archive/issue_events_146436.json:
```json
{
    "actor": "https://github.com/benjaminfjones",
    "created_at": "2011-10-01T23:04:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "milestone_number": null,
    "milestone_title": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11888#event-146436"
}
```



---

archive/issue_events_146437.json:
```json
{
    "actor": "https://github.com/benjaminfjones",
    "created_at": "2011-10-01T23:04:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11888#event-146437"
}
```



---

archive/issue_events_146438.json:
```json
{
    "actor": "https://github.com/benjaminfjones",
    "created_at": "2011-10-01T23:04:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11888#event-146438"
}
```



---

archive/issue_events_146439.json:
```json
{
    "actor": "https://github.com/benjaminfjones",
    "created_at": "2011-10-01T23:04:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11888#event-146439"
}
```



---

archive/issue_comments_126978.json:
```json
{
    "body": "Attachment: **[trac_11888.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888.patch.gz)**\n\nadd lambert_w symbolic function",
    "created_at": "2012-01-10T02:41:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-126978",
    "user": "https://github.com/benjaminfjones"
}
```

Attachment: **[trac_11888.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888.patch.gz)**

add lambert_w symbolic function



---

archive/issue_comments_126979.json:
```json
{
    "body": "Changed keywords from **lambert_w symbolics conversion maxima** to **lambert_w symbolics conversion maxima sd35.5**",
    "created_at": "2012-01-10T02:44:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-126979",
    "user": "https://github.com/benjaminfjones"
}
```

Changed keywords from **lambert_w symbolics conversion maxima** to **lambert_w symbolics conversion maxima sd35.5**



---

archive/issue_events_146440.json:
```json
{
    "actor": "https://github.com/benjaminfjones",
    "created_at": "2012-01-10T02:44:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11888#event-146440"
}
```



---

archive/issue_comments_126980.json:
```json
{
    "body": "Author: **Benjamin Jones**",
    "created_at": "2012-01-10T02:44:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-126980",
    "user": "https://github.com/benjaminfjones"
}
```

Author: **Benjamin Jones**



---

archive/issue_comments_126981.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nPreliminary patch needs review.  The function has been added using the template developed as part of #11143. The issue described in the description is addressed in one of the doctests.",
    "created_at": "2012-01-10T02:44:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-126981",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:2'>Comment 2:</a>
Preliminary patch needs review.  The function has been added using the template developed as part of #11143. The issue described in the description is addressed in one of the doctests.



---

archive/issue_comments_126982.json:
```json
{
    "body": "apply to $SAGE_ROOT/devel/sage",
    "created_at": "2012-01-10T11:37:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-126982",
    "user": "https://github.com/kini"
}
```

apply to $SAGE_ROOT/devel/sage



---

archive/issue_comments_126983.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nAttachment: **[trac_11888-doctests.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888-doctests.patch.gz)**\n\nRunning `make ptestlong` now. I fixed a couple of doctests that broke, and fixed some typos and rST syntax problems in your docstring.",
    "created_at": "2012-01-10T11:39:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-126983",
    "user": "https://github.com/kini"
}
```

<a id='comment:3'>Comment 3:</a>
Attachment: **[trac_11888-doctests.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888-doctests.patch.gz)**

Running `make ptestlong` now. I fixed a couple of doctests that broke, and fixed some typos and rST syntax problems in your docstring.



---

archive/issue_comments_126984.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -35,3 +35,10 @@\n ```\n \n `mpmath` can evaluate the `lambert_w` function, so it should be easy to add a new symbolic function to Sage that will fix this issue. \n+\n+---\n+\n+Apply:\n+\n+1. [attachment: trac_11888.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888.patch.gz) to `$SAGE_ROOT/devel/sage`\n+2. [attachment: trac_11888-doctests.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888-doctests.patch.gz) to `$SAGE_ROOT/devel/sage`\n``````\n",
    "created_at": "2012-01-10T11:39:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-126984",
    "user": "https://github.com/kini"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -35,3 +35,10 @@
 ```
 
 `mpmath` can evaluate the `lambert_w` function, so it should be easy to add a new symbolic function to Sage that will fix this issue. 
+
+---
+
+Apply:
+
+1. [attachment: trac_11888.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888.patch.gz) to `$SAGE_ROOT/devel/sage`
+2. [attachment: trac_11888-doctests.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888-doctests.patch.gz) to `$SAGE_ROOT/devel/sage`
``````




---

archive/issue_comments_126985.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nAll tests pass.",
    "created_at": "2012-01-10T12:12:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-126985",
    "user": "https://github.com/kini"
}
```

<a id='comment:4'>Comment 4:</a>
All tests pass.



---

archive/issue_comments_126986.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nThanks for the fixes, kini. I've run `make ptestlong` with the patches applied and verified that all tests pass. Maybe I can get @kcrisman to finish a review this afternoon.",
    "created_at": "2012-01-10T20:05:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-126986",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:6'>Comment 6:</a>
Thanks for the fixes, kini. I've run `make ptestlong` with the patches applied and verified that all tests pass. Maybe I can get @kcrisman to finish a review this afternoon.



---

archive/issue_comments_126987.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nI don't see any obvious problems, but the random expression usually doesn't change much with these new functions and this one is *really* different.  \n\nIt's also spread across many lines, and I'm not sure if this is appropriate (just in this one case, of course).",
    "created_at": "2012-01-10T20:24:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-126987",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:7'>Comment 7:</a>
I don't see any obvious problems, but the random expression usually doesn't change much with these new functions and this one is *really* different.  

It's also spread across many lines, and I'm not sure if this is appropriate (just in this one case, of course).



---

archive/issue_comments_126988.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nI spread it across lines because 1) I was trying to keep within the recommended PEP 8 guidelines for line length, and 2) because of this\n\n```\n[2012-01-10 22:54:53] <kini> while I was fixing the second doctest, some weird\nstuff started happening to vim\n[2012-01-10 22:55:02] <kini> I thought my terminal had frozen or something\n[2012-01-10 22:56:02] <kini> but it turns out that apparently opening a new line\nafter a line with a 1800-character-long Sage symbolic expression on it causes\nvim to take a full 12 seconds to compute the correct indentation level for the\nnext line\n[2012-01-10 22:56:20] <benjaminfjones> ha!\n[2012-01-10 22:56:30] <kini> on a 4.5 GHz Core i5-2500K and utilizing three\ncores!\n[2012-01-10 22:56:39] <benjaminfjones> wow\n```\n\nWhat is inappropriate about adding line breaks?\n\nAs for the length of the expression, it seems to be a fluke. With the patches applied, starting with random seeds other than `2` gives expressions of a more \"normal\" length.",
    "created_at": "2012-01-11T01:56:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-126988",
    "user": "https://github.com/kini"
}
```

<a id='comment:8'>Comment 8:</a>
I spread it across lines because 1) I was trying to keep within the recommended PEP 8 guidelines for line length, and 2) because of this

```
[2012-01-10 22:54:53] <kini> while I was fixing the second doctest, some weird
stuff started happening to vim
[2012-01-10 22:55:02] <kini> I thought my terminal had frozen or something
[2012-01-10 22:56:02] <kini> but it turns out that apparently opening a new line
after a line with a 1800-character-long Sage symbolic expression on it causes
vim to take a full 12 seconds to compute the correct indentation level for the
next line
[2012-01-10 22:56:20] <benjaminfjones> ha!
[2012-01-10 22:56:30] <kini> on a 4.5 GHz Core i5-2500K and utilizing three
cores!
[2012-01-10 22:56:39] <benjaminfjones> wow
```

What is inappropriate about adding line breaks?

As for the length of the expression, it seems to be a fluke. With the patches applied, starting with random seeds other than `2` gives expressions of a more "normal" length.



---

archive/issue_comments_126989.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nI agree, it looks like a fluke that the expression grows so large. I did some testing of `random_expr` and found that it \"normally\" produces output around 200 - 400 character long, but occasionally the outputs can be 10 times that (I saw a few around 2500 characters long!)",
    "created_at": "2012-01-11T03:30:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-126989",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:9'>Comment 9:</a>
I agree, it looks like a fluke that the expression grows so large. I did some testing of `random_expr` and found that it "normally" produces output around 200 - 400 character long, but occasionally the outputs can be 10 times that (I saw a few around 2500 characters long!)



---

archive/issue_comments_126990.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nI strongly recommend implementing the general version of the Lambert W function (taking a branch parameter).",
    "created_at": "2012-01-11T15:26:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-126990",
    "user": "https://github.com/fredrik-johansson"
}
```

<a id='comment:10'>Comment 10:</a>
I strongly recommend implementing the general version of the Lambert W function (taking a branch parameter).



---

archive/issue_comments_126991.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\n> I strongly recommend implementing the general version of the Lambert W function (taking a branch parameter).\n\nCan you be more specific?  (Is this standard with other multivalued functions in Sage?)  Maybe this could be a separate ticket, unless the change was really easy.",
    "created_at": "2012-01-11T15:43:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-126991",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:11'>Comment 11:</a>
> I strongly recommend implementing the general version of the Lambert W function (taking a branch parameter).

Can you be more specific?  (Is this standard with other multivalued functions in Sage?)  Maybe this could be a separate ticket, unless the change was really easy.



---

archive/issue_comments_126992.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nThe change should be simple. mpmath implements the a branch `W_k(z)` for each integer `k`. It's just a matter of adding a second parameter to the wrapper and putting in some tests. I'm sitting on the train from Beverly MA to Logan airport now, I'll see if I can get it uploaded before the train stops (or my battery dies).",
    "created_at": "2012-01-11T15:55:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-126992",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:12'>Comment 12:</a>
The change should be simple. mpmath implements the a branch `W_k(z)` for each integer `k`. It's just a matter of adding a second parameter to the wrapper and putting in some tests. I'm sitting on the train from Beverly MA to Logan airport now, I'll see if I can get it uploaded before the train stops (or my battery dies).



---

archive/issue_comments_126993.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nSweet, I didn't realize it was that quick.  I love doing Sage development on that train :)  There is also free wifi at Logan, I believe.",
    "created_at": "2012-01-11T15:57:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-126993",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:13'>Comment 13:</a>
Sweet, I didn't realize it was that quick.  I love doing Sage development on that train :)  There is also free wifi at Logan, I believe.



---

archive/issue_comments_126994.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nPing.  I'd love to review this but sounds like Fredrik's point is good and if it's pretty easy for you to add that, we might as well.",
    "created_at": "2012-02-03T03:23:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-126994",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:14'>Comment 14:</a>
Ping.  I'd love to review this but sounds like Fredrik's point is good and if it's pretty easy for you to add that, we might as well.



---

archive/issue_comments_126995.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nYes, it should be easy; just add an optional branch parameter, lambertw(z, branch=0).\n\nAnother suggestion is to use scipy.special.lambertw for evaluation over RDF and CDF. The SciPy implementation is a Cython translation of the double precision version in mpmath; it supports all branches and has excellent numerical stability, and runs quite a bit faster.\n\n```\nimport scipy.special\nimport mpmath\ntimeit(\"mpmath.lambertw(-35.0r+4.6jr,2r)\")\ntimeit(\"mpmath.fp.lambertw(-35.0r+4.6jr,2r)\")\ntimeit(\"scipy.special.lambertw(-35.0r+4.6jr,2r)\")\nprint repr(complex(mpmath.lambertw(-35.0r+4.6jr,2r)))\nprint repr(mpmath.fp.lambertw(-35.0r+4.6jr,2r))\nprint repr(scipy.special.lambertw(-35.0r+4.6jr,2r))\n\n625 loops, best of 3: 301 \u00b5s per loop\n625 loops, best of 3: 65.1 \u00b5s per loop\n625 loops, best of 3: 6.75 \u00b5s per loop\n(0.91763023745202721+14.071606637742889j)\n(0.91763023745202721+14.071606637742889j)\n(0.91763023745202721+14.071606637742889j)\n```",
    "created_at": "2012-02-03T10:14:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-126995",
    "user": "https://github.com/fredrik-johansson"
}
```

<a id='comment:15'>Comment 15:</a>
Yes, it should be easy; just add an optional branch parameter, lambertw(z, branch=0).

Another suggestion is to use scipy.special.lambertw for evaluation over RDF and CDF. The SciPy implementation is a Cython translation of the double precision version in mpmath; it supports all branches and has excellent numerical stability, and runs quite a bit faster.

```
import scipy.special
import mpmath
timeit("mpmath.lambertw(-35.0r+4.6jr,2r)")
timeit("mpmath.fp.lambertw(-35.0r+4.6jr,2r)")
timeit("scipy.special.lambertw(-35.0r+4.6jr,2r)")
print repr(complex(mpmath.lambertw(-35.0r+4.6jr,2r)))
print repr(mpmath.fp.lambertw(-35.0r+4.6jr,2r))
print repr(scipy.special.lambertw(-35.0r+4.6jr,2r))

625 loops, best of 3: 301 µs per loop
625 loops, best of 3: 65.1 µs per loop
625 loops, best of 3: 6.75 µs per loop
(0.91763023745202721+14.071606637742889j)
(0.91763023745202721+14.071606637742889j)
(0.91763023745202721+14.071606637742889j)
```



---

archive/issue_comments_126996.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nNice; I wonder if there are more places we are beginning to default to mpmath where SciPy could be useful for the double fields.",
    "created_at": "2012-02-03T15:58:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-126996",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:16'>Comment 16:</a>
Nice; I wonder if there are more places we are beginning to default to mpmath where SciPy could be useful for the double fields.



---

archive/issue_comments_126997.json:
```json
{
    "body": "Work Issues: **add second parameter, RDF/CDF stuff**",
    "created_at": "2012-02-03T15:58:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-126997",
    "user": "https://github.com/kcrisman"
}
```

Work Issues: **add second parameter, RDF/CDF stuff**



---

archive/issue_comments_126998.json:
```json
{
    "body": "Reviewer: **Keshav Kini, Karl-Dieter Crisman, Fredrik Johansson**",
    "created_at": "2012-02-03T15:58:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-126998",
    "user": "https://github.com/kcrisman"
}
```

Reviewer: **Keshav Kini, Karl-Dieter Crisman, Fredrik Johansson**



---

archive/issue_events_146441.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-02-03T15:58:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11888#event-146441"
}
```



---

archive/issue_events_146442.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-02-03T15:58:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11888#event-146442"
}
```



---

archive/issue_comments_126999.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nThanks for the ping. I'm still here (and I have a patch pretty much ready to go) I just got buried under teaching. I'll try to upload a patch this evening.",
    "created_at": "2012-02-03T17:25:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-126999",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:17'>Comment 17:</a>
Thanks for the ping. I'm still here (and I have a patch pretty much ready to go) I just got buried under teaching. I'll try to upload a patch this evening.



---

archive/issue_comments_127000.json:
```json
{
    "body": "<a id='comment:18'>Comment 18:</a>\nAfter looking at this a bit more, it may be more involved than I initially envisioned to implement arbitrary branches of the lambert_w function in one symbolic function. Right now, the patch from SD 35.5 implements a subclass of !`BuiltinFunction`. The underlying assumptions about subclasses of BuiltinFunction include: (from sage/symbolic/function.pyx)\n\n```\nWe assume that each subclass of this class will define one symbolic function.\n```\n\nOne issue is that there isn't a way (as far as I can see) to pass a branch parameter to !`BuiltinFunction`'s `_call_` method. (Perhaps burcin or other authority on Sage symbolics can comment on this.)\n\nChanging the evaluation numerical eval to use SciPy would be an easy change, that's for sure. I can do that quickly and upload a patch that implements the principle branch only. \n\nAnother idea I just had was to do something like what we have for the Bessel functions, in particular the !`Bessel` class in sage/functions/special.py which is just a basic python class returning one of the Bessel (I,J,Y) functions of a given order.",
    "created_at": "2012-02-04T01:10:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127000",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:18'>Comment 18:</a>
After looking at this a bit more, it may be more involved than I initially envisioned to implement arbitrary branches of the lambert_w function in one symbolic function. Right now, the patch from SD 35.5 implements a subclass of !`BuiltinFunction`. The underlying assumptions about subclasses of BuiltinFunction include: (from sage/symbolic/function.pyx)

```
We assume that each subclass of this class will define one symbolic function.
```

One issue is that there isn't a way (as far as I can see) to pass a branch parameter to !`BuiltinFunction`'s `_call_` method. (Perhaps burcin or other authority on Sage symbolics can comment on this.)

Changing the evaluation numerical eval to use SciPy would be an easy change, that's for sure. I can do that quickly and upload a patch that implements the principle branch only. 

Another idea I just had was to do something like what we have for the Bessel functions, in particular the !`Bessel` class in sage/functions/special.py which is just a basic python class returning one of the Bessel (I,J,Y) functions of a given order.



---

archive/issue_comments_127001.json:
```json
{
    "body": "<a id='comment:19'>Comment 19:</a>\nOk, that makes sense.   I feel like there should be a way to do that nonetheless - see `incomplete_gamma`, with\n\n```\n        BuiltinFunction.__init__(self, \"gamma\", nargs=2, latex_name=r\"\\Gamma\",\n                conversions={'maxima':'gamma_incomplete', 'mathematica':'Gamma',\n                    'maple':'GAMMA'})\n```\nand then use `_eval_` and `_evalf_`, but I don't have time to try looking into whether that would work here now.\n\nBased on Fredrik's comment, make sure to only use SciPy for RDF/CDF - hopefully there is a good model elsewhere to use for that.",
    "created_at": "2012-02-04T01:30:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127001",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:19'>Comment 19:</a>
Ok, that makes sense.   I feel like there should be a way to do that nonetheless - see `incomplete_gamma`, with

```
        BuiltinFunction.__init__(self, "gamma", nargs=2, latex_name=r"\Gamma",
                conversions={'maxima':'gamma_incomplete', 'mathematica':'Gamma',
                    'maple':'GAMMA'})
```
and then use `_eval_` and `_evalf_`, but I don't have time to try looking into whether that would work here now.

Based on Fredrik's comment, make sure to only use SciPy for RDF/CDF - hopefully there is a good model elsewhere to use for that.



---

archive/issue_comments_127002.json:
```json
{
    "body": "proof of concept patch (not ready for review)",
    "created_at": "2012-02-06T05:24:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127002",
    "user": "https://github.com/benjaminfjones"
}
```

proof of concept patch (not ready for review)



---

archive/issue_comments_127003.json:
```json
{
    "body": "<a id='comment:20'>Comment 20:</a>\nAttachment: **[trac_11888_v2.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v2.patch.gz)**\n\nOK, I made a second attempt. The patch isn't complete (I need to fix and add docstrings and do more testing) and is *not* ready for review, but if the reviewers will take a look at the basic implementation and give me feedback, I'd appreciate it.\n\nIn [attachment: trac_11888_v2.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v2.patch.gz) there is a new symbolic function  `lambert_w_branch` which takes two arguments, a complex number `z` and an integer branch `n`. This is implemented using scipy.special.lambertw for RDF/CDF arguments z and using mpmath otherwise. \n\nThere is also a wrapper function `lambert_w` that accepts either one or two arguments. For one argument it returns the principle branch `lambert_w_branch(z,0)`, for two it returns `lambert_w_branch(z,n)`. I still need to add the conversion from Maxima (by hand now, since `lambert_w` doesn't inherit from BuiltinFunction any more).",
    "created_at": "2012-02-06T05:26:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127003",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:20'>Comment 20:</a>
Attachment: **[trac_11888_v2.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v2.patch.gz)**

OK, I made a second attempt. The patch isn't complete (I need to fix and add docstrings and do more testing) and is *not* ready for review, but if the reviewers will take a look at the basic implementation and give me feedback, I'd appreciate it.

In [attachment: trac_11888_v2.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v2.patch.gz) there is a new symbolic function  `lambert_w_branch` which takes two arguments, a complex number `z` and an integer branch `n`. This is implemented using scipy.special.lambertw for RDF/CDF arguments z and using mpmath otherwise. 

There is also a wrapper function `lambert_w` that accepts either one or two arguments. For one argument it returns the principle branch `lambert_w_branch(z,0)`, for two it returns `lambert_w_branch(z,n)`. I still need to add the conversion from Maxima (by hand now, since `lambert_w` doesn't inherit from BuiltinFunction any more).



---

archive/issue_comments_127004.json:
```json
{
    "body": "<a id='comment:21'>Comment 21:</a>\nI fixed the doctests and added lambert_w to the symbol table. I verified that all tests pass including the random_tests.py ones. The patch [attachment: trac_11888_v3.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v3.patch.gz) is ready for review.",
    "created_at": "2012-02-11T22:07:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127004",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:21'>Comment 21:</a>
I fixed the doctests and added lambert_w to the symbol table. I verified that all tests pass including the random_tests.py ones. The patch [attachment: trac_11888_v3.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v3.patch.gz) is ready for review.



---

archive/issue_comments_127005.json:
```json
{
    "body": "Changed work issues from **add second parameter, RDF/CDF stuff** to none",
    "created_at": "2012-02-11T22:07:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127005",
    "user": "https://github.com/benjaminfjones"
}
```

Changed work issues from **add second parameter, RDF/CDF stuff** to none



---

archive/issue_events_146443.json:
```json
{
    "actor": "https://github.com/benjaminfjones",
    "created_at": "2012-02-11T22:07:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11888#event-146443"
}
```



---

archive/issue_events_146444.json:
```json
{
    "actor": "https://github.com/benjaminfjones",
    "created_at": "2012-02-11T22:07:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11888#event-146444"
}
```



---

archive/issue_comments_127006.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -40,5 +40,4 @@\n \n Apply:\n \n-1. [attachment: trac_11888.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888.patch.gz) to `$SAGE_ROOT/devel/sage`\n-2. [attachment: trac_11888-doctests.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888-doctests.patch.gz) to `$SAGE_ROOT/devel/sage`\n+1. [attachment: trac_11888_v3.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v3.patch.gz) to `$SAGE_ROOT/devel/sage`\n``````\n",
    "created_at": "2012-02-11T22:08:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127006",
    "user": "https://github.com/benjaminfjones"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -40,5 +40,4 @@
 
 Apply:
 
-1. [attachment: trac_11888.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888.patch.gz) to `$SAGE_ROOT/devel/sage`
-2. [attachment: trac_11888-doctests.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888-doctests.patch.gz) to `$SAGE_ROOT/devel/sage`
+1. [attachment: trac_11888_v3.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v3.patch.gz) to `$SAGE_ROOT/devel/sage`
``````




---

archive/issue_comments_127007.json:
```json
{
    "body": "<a id='comment:23'>Comment 23:</a>\nprinciple -> principal, branchs -> branches\n\nOtherwise, from looking at the patch, seems good.",
    "created_at": "2012-02-11T22:27:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127007",
    "user": "https://github.com/fredrik-johansson"
}
```

<a id='comment:23'>Comment 23:</a>
principle -> principal, branchs -> branches

Otherwise, from looking at the patch, seems good.



---

archive/issue_comments_127008.json:
```json
{
    "body": "<a id='comment:24'>Comment 24:</a>\nThanks for looking at the patch, Fredrik. I've fixed the mistakes and replaced the latest patch.",
    "created_at": "2012-02-12T00:28:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127008",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:24'>Comment 24:</a>
Thanks for looking at the patch, Fredrik. I've fixed the mistakes and replaced the latest patch.



---

archive/issue_comments_127009.json:
```json
{
    "body": "adds lambert_w and lambert_w_branch functions",
    "created_at": "2012-02-12T00:29:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127009",
    "user": "https://github.com/benjaminfjones"
}
```

adds lambert_w and lambert_w_branch functions



---

archive/issue_comments_127010.json:
```json
{
    "body": "<a id='comment:25'>Comment 25:</a>\nAttachment: **[trac_11888_v3.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v3.patch.gz)**\n\n* typo\n\n```\n  SciPy is used to evalute\n```\n* Will this conflict with the beta function patch when it comes to the random test?\n* I'm wondering whether we should add a couple conversions to Mma/Maple in the init (apparently Maxima is working fine, though see [this possible enhancement](http://www.ma.utexas.edu/pipermail/maxima/2011/025861.html)).  Mathematica apparently calls it ProductLog...",
    "created_at": "2012-02-12T03:45:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127010",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:25'>Comment 25:</a>
Attachment: **[trac_11888_v3.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v3.patch.gz)**

* typo

```
  SciPy is used to evalute
```
* Will this conflict with the beta function patch when it comes to the random test?
* I'm wondering whether we should add a couple conversions to Mma/Maple in the init (apparently Maxima is working fine, though see [this possible enhancement](http://www.ma.utexas.edu/pipermail/maxima/2011/025861.html)).  Mathematica apparently calls it ProductLog...



---

archive/issue_comments_127011.json:
```json
{
    "body": "<a id='comment:26'>Comment 26:</a>\n* I'll fix the typo (tomorrow)\n  * I'll make the beta function ticket a dependency so the random test will come out correctly after the two patches are merged in order\n  * ProductLog in Mathematica puts the branch parameter first. I guess it makes sense to be consistent with that convention as well as the discussion on the Maxima list. I can't figure out whether the generalized lambert function ever made it into Maxima..",
    "created_at": "2012-02-12T06:48:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127011",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:26'>Comment 26:</a>
* I'll fix the typo (tomorrow)
  * I'll make the beta function ticket a dependency so the random test will come out correctly after the two patches are merged in order
  * ProductLog in Mathematica puts the branch parameter first. I guess it makes sense to be consistent with that convention as well as the discussion on the Maxima list. I can't figure out whether the generalized lambert function ever made it into Maxima..



---

archive/issue_comments_127012.json:
```json
{
    "body": "addressed reviewer issues, changed order of arguments to be consistant with Mma/Maple",
    "created_at": "2012-02-12T20:49:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127012",
    "user": "https://github.com/benjaminfjones"
}
```

addressed reviewer issues, changed order of arguments to be consistant with Mma/Maple



---

archive/issue_comments_127013.json:
```json
{
    "body": "Attachment: **[trac_11888_v4.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v4.patch.gz)**\n\nfixes random tests after rebasing against #9130",
    "created_at": "2012-02-12T20:49:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127013",
    "user": "https://github.com/benjaminfjones"
}
```

Attachment: **[trac_11888_v4.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v4.patch.gz)**

fixes random tests after rebasing against #9130



---

archive/issue_comments_127014.json:
```json
{
    "body": "<a id='comment:27'>Comment 27:</a>\nAttachment: **[trac_11888-random-tests.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888-random-tests.patch.gz)**",
    "created_at": "2012-02-12T20:51:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127014",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:27'>Comment 27:</a>
Attachment: **[trac_11888-random-tests.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888-random-tests.patch.gz)**



---

archive/issue_comments_127015.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -40,4 +40,6 @@\n \n Apply:\n \n-1. [attachment: trac_11888_v3.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v3.patch.gz) to `$SAGE_ROOT/devel/sage`\n+* Patches at #9130 \n+* [attachment: trac_11888_v4.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v4.patch.gz) to `$SAGE_ROOT/devel/sage`\n+* [attachment: trac_11888-random-tests.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888-random-tests.patch.gz) to `$SAGE_ROOT/devel/sage`\n``````\n",
    "created_at": "2012-02-12T20:51:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127015",
    "user": "https://github.com/benjaminfjones"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -40,4 +40,6 @@
 
 Apply:
 
-1. [attachment: trac_11888_v3.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v3.patch.gz) to `$SAGE_ROOT/devel/sage`
+* Patches at #9130 
+* [attachment: trac_11888_v4.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v4.patch.gz) to `$SAGE_ROOT/devel/sage`
+* [attachment: trac_11888-random-tests.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888-random-tests.patch.gz) to `$SAGE_ROOT/devel/sage`
``````




---

archive/issue_comments_127016.json:
```json
{
    "body": "Dependencies: **#9130**",
    "created_at": "2012-02-12T20:51:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127016",
    "user": "https://github.com/benjaminfjones"
}
```

Dependencies: **#9130**



---

archive/issue_comments_127017.json:
```json
{
    "body": "Changed reviewer from **Keshav Kini, Karl-Dieter Crisman, Fredrik Johansson** to **Keshav Kini, Karl-Dieter Crisman, Fredrik Johansson, Burcin Erocal**",
    "created_at": "2012-02-13T10:21:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127017",
    "user": "https://github.com/burcin"
}
```

Changed reviewer from **Keshav Kini, Karl-Dieter Crisman, Fredrik Johansson** to **Keshav Kini, Karl-Dieter Crisman, Fredrik Johansson, Burcin Erocal**



---

archive/issue_comments_127018.json:
```json
{
    "body": "<a id='comment:28'>Comment 28:</a>\nDo we really want to call this function `lambert_w_branch()`? Can we name it `lambert_w()`? I would even suggest to add custom printing methods (`_print_()` and `_print_latex_()`) to avoid printing the branch argument if it is 0.\n\nIf the function is named lambert_w, you can remove the wrapper function `lambert_w()` and the manual manipulation of the symbol table. In this case, a custom `__call__()` method would take the place of the wrapper method.\n\nBTW, we should either open a new ticket to add known exact evaluations to `_eval_()` or do this here:\n- 0 -> 0\n- e -> 1\n- -1/e -> -1",
    "created_at": "2012-02-13T10:21:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127018",
    "user": "https://github.com/burcin"
}
```

<a id='comment:28'>Comment 28:</a>
Do we really want to call this function `lambert_w_branch()`? Can we name it `lambert_w()`? I would even suggest to add custom printing methods (`_print_()` and `_print_latex_()`) to avoid printing the branch argument if it is 0.

If the function is named lambert_w, you can remove the wrapper function `lambert_w()` and the manual manipulation of the symbol table. In this case, a custom `__call__()` method would take the place of the wrapper method.

BTW, we should either open a new ticket to add known exact evaluations to `_eval_()` or do this here:
- 0 -> 0
- e -> 1
- -1/e -> -1



---

archive/issue_events_146445.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2012-02-13T10:21:14Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "title_is": "Sage is missing the lambert_w function",
    "title_was": "Sage is missing the lambert_w function conversion from Maxima",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11888#event-146445"
}
```



---

archive/issue_comments_127019.json:
```json
{
    "body": "<a id='comment:29'>Comment 29:</a>\nI have one more comment. Sorry for multiple emails.\n\nYou should check if the parent is `RDF` or `CDF` using `is`, not `==`. In this context, `parent` is an argument to the `_evalf_()` method, which overrides the `parent()` function imported from `sage.structure.coerce`. I suggest naming the argument `parent_d` instead of `parent`. Then you can do:\n\n```\nR = parent_d or parent(z)\nif R is float or R is complex or R is RDF or R is CDF: \n    import scipy.special \n    return scipy.special.lambertw(z, n) \nelse: \n    import mpmath \n    return mpmath_utils.call(mpmath.lambertw, z, n, parent=parent) \n```",
    "created_at": "2012-02-13T10:29:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127019",
    "user": "https://github.com/burcin"
}
```

<a id='comment:29'>Comment 29:</a>
I have one more comment. Sorry for multiple emails.

You should check if the parent is `RDF` or `CDF` using `is`, not `==`. In this context, `parent` is an argument to the `_evalf_()` method, which overrides the `parent()` function imported from `sage.structure.coerce`. I suggest naming the argument `parent_d` instead of `parent`. Then you can do:

```
R = parent_d or parent(z)
if R is float or R is complex or R is RDF or R is CDF: 
    import scipy.special 
    return scipy.special.lambertw(z, n) 
else: 
    import mpmath 
    return mpmath_utils.call(mpmath.lambertw, z, n, parent=parent) 
```



---

archive/issue_events_146446.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2012-02-13T10:29:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11888#event-146446"
}
```



---

archive/issue_events_146447.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2012-02-13T10:29:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11888#event-146447"
}
```



---

archive/issue_comments_127020.json:
```json
{
    "body": "<a id='comment:30'>Comment 30:</a>\nReplying to [@burcin](#comment%3A28):\n> Do we really want to call this function `lambert_w_branch()`? Can we name it `lambert_w()`? I would even suggest to add custom printing methods (`_print_()` and `_print_latex_()`) to avoid printing the branch argument if it is 0.\n\nThat's a great idea.",
    "created_at": "2012-02-13T18:27:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127020",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:30'>Comment 30:</a>
Replying to [@burcin](#comment%3A28):
> Do we really want to call this function `lambert_w_branch()`? Can we name it `lambert_w()`? I would even suggest to add custom printing methods (`_print_()` and `_print_latex_()`) to avoid printing the branch argument if it is 0.

That's a great idea.



---

archive/issue_comments_127021.json:
```json
{
    "body": "Changed dependencies from **#9130** to **#12507**",
    "created_at": "2012-02-18T06:10:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127021",
    "user": "https://github.com/benjaminfjones"
}
```

Changed dependencies from **#9130** to **#12507**



---

archive/issue_comments_127022.json:
```json
{
    "body": "<a id='comment:31'>Comment 31:</a>\nI've written a new patch that includes significant changes compared to the last one. I think I've included all of burcin's suggestions and I think it's much improved now. All tests pass with the patch applied on 5.0.beta4 + #12507.\n\nOne thing I haven't managed to figure out is how to get integration to work, e.g.\n\n```\nsage: integrate(lambert_w(x), x)\n...\nRuntimeError: ECL says: Error executing code in Maxima: lambert_w: wrong number of arguments.\n```\n\nI guess that's because there isn't a two-argument version of lambert_w defined in maxima. The conversion maxima -> Sage works (as shown in one of the doctests) but it looks like the other way doesn't. Another example:\n\n```\nsage: maxima(lambert_w(5))\nMaxima ERROR:\n\t\nlambert_w: wrong number of arguments.\n -- an error. To debug this try: debugmode(true);\n```\n\nQ: How do I get around this?\n\nNumerical integration also fails unless I pass a lambda function:\n\n```\nsage: numerical_integral(lambert_w(x), 0, 1)\nException TypeError: \"function not supported for these types, and can't coerce safely to supported types\" in 'sage.gsl.integration.c_ff' ignored\n...\n(0.0, 0.0)\n```\n\nbut ....\n\n```\nsage: numerical_integral(lambda x: lambert_w(x), 0, 1)\n(0.33036612476168054, 3.667800782666048e-15)\n```\n\nQ: How do I fix this?",
    "created_at": "2012-02-18T06:10:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127022",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:31'>Comment 31:</a>
I've written a new patch that includes significant changes compared to the last one. I think I've included all of burcin's suggestions and I think it's much improved now. All tests pass with the patch applied on 5.0.beta4 + #12507.

One thing I haven't managed to figure out is how to get integration to work, e.g.

```
sage: integrate(lambert_w(x), x)
...
RuntimeError: ECL says: Error executing code in Maxima: lambert_w: wrong number of arguments.
```

I guess that's because there isn't a two-argument version of lambert_w defined in maxima. The conversion maxima -> Sage works (as shown in one of the doctests) but it looks like the other way doesn't. Another example:

```
sage: maxima(lambert_w(5))
Maxima ERROR:
	
lambert_w: wrong number of arguments.
 -- an error. To debug this try: debugmode(true);
```

Q: How do I get around this?

Numerical integration also fails unless I pass a lambda function:

```
sage: numerical_integral(lambert_w(x), 0, 1)
Exception TypeError: "function not supported for these types, and can't coerce safely to supported types" in 'sage.gsl.integration.c_ff' ignored
...
(0.0, 0.0)
```

but ....

```
sage: numerical_integral(lambda x: lambert_w(x), 0, 1)
(0.33036612476168054, 3.667800782666048e-15)
```

Q: How do I fix this?



---

archive/issue_comments_127023.json:
```json
{
    "body": "adds lambert_w function",
    "created_at": "2012-02-18T06:11:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127023",
    "user": "https://github.com/benjaminfjones"
}
```

adds lambert_w function



---

archive/issue_comments_127024.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -40,6 +40,5 @@\n \n Apply:\n \n-* Patches at #9130 \n-* [attachment: trac_11888_v4.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v4.patch.gz) to `$SAGE_ROOT/devel/sage`\n-* [attachment: trac_11888-random-tests.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888-random-tests.patch.gz) to `$SAGE_ROOT/devel/sage`\n+* Patch at #12507\n+* [attachment: trac_11888_v5.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v5.patch.gz) to `$SAGE_ROOT/devel/sage`\n``````\n",
    "created_at": "2012-02-18T06:11:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127024",
    "user": "https://github.com/benjaminfjones"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -40,6 +40,5 @@
 
 Apply:
 
-* Patches at #9130 
-* [attachment: trac_11888_v4.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v4.patch.gz) to `$SAGE_ROOT/devel/sage`
-* [attachment: trac_11888-random-tests.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888-random-tests.patch.gz) to `$SAGE_ROOT/devel/sage`
+* Patch at #12507
+* [attachment: trac_11888_v5.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v5.patch.gz) to `$SAGE_ROOT/devel/sage`
``````




---

archive/issue_comments_127025.json:
```json
{
    "body": "<a id='comment:32'>Comment 32:</a>\nAttachment: **[trac_11888_v5.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v5.patch.gz)**",
    "created_at": "2012-02-18T06:11:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127025",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:32'>Comment 32:</a>
Attachment: **[trac_11888_v5.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v5.patch.gz)**



---

archive/issue_comments_127026.json:
```json
{
    "body": "<a id='comment:33'>Comment 33:</a>\nReplying to [@benjaminfjones](#comment%3A31):\n> I've written a new patch that includes significant changes compared to the last one. I think I've included all of burcin's suggestions and I think it's much improved now. All tests pass with the patch applied on 5.0.beta4 + #12507.\n\nThanks! The patch looks really good. When checking if the input is 0 in `_eval_`, you might want to `return z` instead of `Integer(0)` to preserve the type of the input. Similarly, we should return `parent(z)(1)` or `parent(z)(-1)` in the other branches.\n\n<snip>\n> I guess that's because there isn't a two-argument version of lambert_w defined in maxima. The conversion maxima -> Sage works (as shown in one of the doctests) but it looks like the other way doesn't. Another example:\n> \n> ```\n> sage: maxima(lambert_w(5))\n> Maxima ERROR:\n> \t\n> lambert_w: wrong number of arguments.\n>  -- an error. To debug this try: debugmode(true);\n> ```\n> \n> Q: How do I get around this?\n\nYou need to define `_maxima_init_evaled_()`. See line 895 of `sage/fuctions/other.py`:\n\nhttp://hg.sagemath.org/sage-main/file/c239be1054e0/sage/functions/other.py#l895",
    "created_at": "2012-02-18T16:39:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127026",
    "user": "https://github.com/burcin"
}
```

<a id='comment:33'>Comment 33:</a>
Replying to [@benjaminfjones](#comment%3A31):
> I've written a new patch that includes significant changes compared to the last one. I think I've included all of burcin's suggestions and I think it's much improved now. All tests pass with the patch applied on 5.0.beta4 + #12507.

Thanks! The patch looks really good. When checking if the input is 0 in `_eval_`, you might want to `return z` instead of `Integer(0)` to preserve the type of the input. Similarly, we should return `parent(z)(1)` or `parent(z)(-1)` in the other branches.

<snip>
> I guess that's because there isn't a two-argument version of lambert_w defined in maxima. The conversion maxima -> Sage works (as shown in one of the doctests) but it looks like the other way doesn't. Another example:
> 
> ```
> sage: maxima(lambert_w(5))
> Maxima ERROR:
> 	
> lambert_w: wrong number of arguments.
>  -- an error. To debug this try: debugmode(true);
> ```
> 
> Q: How do I get around this?

You need to define `_maxima_init_evaled_()`. See line 895 of `sage/fuctions/other.py`:

http://hg.sagemath.org/sage-main/file/c239be1054e0/sage/functions/other.py#l895



---

archive/issue_comments_127027.json:
```json
{
    "body": "<a id='comment:34'>Comment 34:</a>\nReplying to [@burcin](#comment%3A33):\n> \n> You need to define `_maxima_init_evaled_()`. See line 895 of `sage/fuctions/other.py`:\n> \n> http://hg.sagemath.org/sage-main/file/c239be1054e0/sage/functions/other.py#l895\n> \n\nIt seems that adding `_maxima_init_evaled_()` solves one issue, converting to Maxima with `_maxima_()`,\n\n```\nsage: lambert_w(x)._maxima_()\nlambert_w(x)\nsage: lambert_w(1,x)._maxima_()\n...\nNotImplementedError: Non-principal branch lambert_w[1](x) is not implemented in Maxima\n```\n\nbut integration still doesn't work (same error is raised as before). Looking closer it seems that the issue is here:\n\n```\nsage: z = lambert_w(x)\nsage: z.operands()\n[0, x]\nsage: z.operator()\nlambert_w\n```\n\nbecause when `sr_to_max` is called in the integration code, I get:\n\n```\nsage: from sage.interfaces.maxima_lib import sr_to_max\nsage: sr_to_max(lambert_w(x))\n<ECL: ((%LAMBERT_W) 0 $X)>\nsage: sr_to_max(lambert_w(1, x))\n<ECL: ((%LAMBERT_W) 1 $X)>\n```\n\nand Maxima barfs because it doesn't know what to do with `((%LAMBERT_W) 0 $X)`.",
    "created_at": "2012-02-22T18:55:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127027",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:34'>Comment 34:</a>
Replying to [@burcin](#comment%3A33):
> 
> You need to define `_maxima_init_evaled_()`. See line 895 of `sage/fuctions/other.py`:
> 
> http://hg.sagemath.org/sage-main/file/c239be1054e0/sage/functions/other.py#l895
> 

It seems that adding `_maxima_init_evaled_()` solves one issue, converting to Maxima with `_maxima_()`,

```
sage: lambert_w(x)._maxima_()
lambert_w(x)
sage: lambert_w(1,x)._maxima_()
...
NotImplementedError: Non-principal branch lambert_w[1](x) is not implemented in Maxima
```

but integration still doesn't work (same error is raised as before). Looking closer it seems that the issue is here:

```
sage: z = lambert_w(x)
sage: z.operands()
[0, x]
sage: z.operator()
lambert_w
```

because when `sr_to_max` is called in the integration code, I get:

```
sage: from sage.interfaces.maxima_lib import sr_to_max
sage: sr_to_max(lambert_w(x))
<ECL: ((%LAMBERT_W) 0 $X)>
sage: sr_to_max(lambert_w(1, x))
<ECL: ((%LAMBERT_W) 1 $X)>
```

and Maxima barfs because it doesn't know what to do with `((%LAMBERT_W) 0 $X)`.



---

archive/issue_comments_127028.json:
```json
{
    "body": "Attachment: **[trac_11888_v6.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v6.patch.gz)**\n\nadded custom latex printing and Maxima initialization",
    "created_at": "2012-03-13T19:49:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127028",
    "user": "https://github.com/benjaminfjones"
}
```

Attachment: **[trac_11888_v6.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v6.patch.gz)**

added custom latex printing and Maxima initialization



---

archive/issue_comments_127029.json:
```json
{
    "body": "<a id='comment:35'>Comment 35:</a>\nI've posted my latest patch in case anyone wants to play around with getting integration of `lambert_w` to work. \n\nThis issue could be a new ticket. One solution I can see is to add `lambert_w` to the `special_sage_to_max` dictionary in `sage/intefaces/maxima_lib.py`. There are a few other special functions listed there (like `Ei` and `polylog`) that need special conversions to maxima.\n\nSo, I propose that we either:\na. Review patch [attachment: trac_11888_v6.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v6.patch.gz) and open a new ticket for the integration issue\nb. Agree on a simple workaround like adding `lambert_w` to `special_sage_to_max` and I'll add it to the patch.",
    "created_at": "2012-03-13T21:08:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127029",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:35'>Comment 35:</a>
I've posted my latest patch in case anyone wants to play around with getting integration of `lambert_w` to work. 

This issue could be a new ticket. One solution I can see is to add `lambert_w` to the `special_sage_to_max` dictionary in `sage/intefaces/maxima_lib.py`. There are a few other special functions listed there (like `Ei` and `polylog`) that need special conversions to maxima.

So, I propose that we either:
a. Review patch [attachment: trac_11888_v6.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v6.patch.gz) and open a new ticket for the integration issue
b. Agree on a simple workaround like adding `lambert_w` to `special_sage_to_max` and I'll add it to the patch.



---

archive/issue_events_146448.json:
```json
{
    "actor": "https://github.com/benjaminfjones",
    "created_at": "2012-03-13T21:08:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11888#event-146448"
}
```



---

archive/issue_events_146449.json:
```json
{
    "actor": "https://github.com/benjaminfjones",
    "created_at": "2012-03-13T21:08:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11888#event-146449"
}
```



---

archive/issue_comments_127030.json:
```json
{
    "body": "<a id='comment:36'>Comment 36:</a>\nI think that b. makes sense.  You'd also have to add `max_lambert_w` at about [this spot](http://hg.sagemath.org/sage-main/file/c239be1054e0/sage/interfaces/maxima_lib.py#l1093) but having numerical integrals would be worth it.  \n\nI assume that this is a pretty easy change?  If not, I guess we could just document that this doesn't work yet.  In either case something should be documented, though, since half of our bug reports seem to be people using new, cool functionality who then expect that new functionality to be fully featured as well - i.e., it's not really a bug report at all, but a feature request.  Having good doc for what we *don't* do will help with that.",
    "created_at": "2012-03-14T12:50:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127030",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:36'>Comment 36:</a>
I think that b. makes sense.  You'd also have to add `max_lambert_w` at about [this spot](http://hg.sagemath.org/sage-main/file/c239be1054e0/sage/interfaces/maxima_lib.py#l1093) but having numerical integrals would be worth it.  

I assume that this is a pretty easy change?  If not, I guess we could just document that this doesn't work yet.  In either case something should be documented, though, since half of our bug reports seem to be people using new, cool functionality who then expect that new functionality to be fully featured as well - i.e., it's not really a bug report at all, but a feature request.  Having good doc for what we *don't* do will help with that.



---

archive/issue_events_146450.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-03-16T02:12:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11888#event-146450"
}
```



---

archive/issue_events_146451.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-03-16T02:12:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11888#event-146451"
}
```



---

archive/issue_comments_127031.json:
```json
{
    "body": "<a id='comment:37'>Comment 37:</a>\n'Needs work' for b.",
    "created_at": "2012-03-16T02:12:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127031",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:37'>Comment 37:</a>
'Needs work' for b.



---

archive/issue_events_146452.json:
```json
{
    "actor": "https://github.com/benjaminfjones",
    "created_at": "2012-03-19T19:58:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11888#event-146452"
}
```



---

archive/issue_events_146453.json:
```json
{
    "actor": "https://github.com/benjaminfjones",
    "created_at": "2012-03-19T19:58:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11888#event-146453"
}
```



---

archive/issue_comments_127032.json:
```json
{
    "body": "<a id='comment:38'>Comment 38:</a>\nSuccess! \n\nSymbolic and numerical integration now work as expected for the principle branch. I added doctests to indicate what is and is not implemented. \n\n---\n\nOne other comment, to indicate what causes errors, I want to add doctests to `lambert_w` such as: \n\n```\nsage: integral(lambert_w(1,x), x)                                                                   \nERROR: An unexpected error occurred while tokenizing input                                          \n...                                                                                                 \nRuntimeError: ECL says: Error executing code in Maxima: lambert_w: expected exactly 1 arguments.   \n```\n\nand \n\n```\nsage: numerical_integral(lambert_w(x), 0, 1)                                                        \nException TypeError: \"function not supported for these types, and can't coerce safely to supported types\" in 'sage.gsl.integration.c_ff' ignored                                                    \n...                                                                                                 \n(0.0, 0.0)\n```\n\nbut the doctest framework doesn't recognize the `Exception TypeError` and it seems to automatically fail if a `RuntimeError` is raised. If I put the latter 4 lines in the docstring for `lambert_w`, it fails doctesting, the framework only sees the `(0.0, 0.0)` part at the end. Is there a way around either of these issues?",
    "created_at": "2012-03-19T19:58:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127032",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:38'>Comment 38:</a>
Success! 

Symbolic and numerical integration now work as expected for the principle branch. I added doctests to indicate what is and is not implemented. 

---

One other comment, to indicate what causes errors, I want to add doctests to `lambert_w` such as: 

```
sage: integral(lambert_w(1,x), x)                                                                   
ERROR: An unexpected error occurred while tokenizing input                                          
...                                                                                                 
RuntimeError: ECL says: Error executing code in Maxima: lambert_w: expected exactly 1 arguments.   
```

and 

```
sage: numerical_integral(lambert_w(x), 0, 1)                                                        
Exception TypeError: "function not supported for these types, and can't coerce safely to supported types" in 'sage.gsl.integration.c_ff' ignored                                                    
...                                                                                                 
(0.0, 0.0)
```

but the doctest framework doesn't recognize the `Exception TypeError` and it seems to automatically fail if a `RuntimeError` is raised. If I put the latter 4 lines in the docstring for `lambert_w`, it fails doctesting, the framework only sees the `(0.0, 0.0)` part at the end. Is there a way around either of these issues?



---

archive/issue_comments_127033.json:
```json
{
    "body": "adds lambert_w function, including integration",
    "created_at": "2012-03-19T19:59:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127033",
    "user": "https://github.com/benjaminfjones"
}
```

adds lambert_w function, including integration



---

archive/issue_comments_127034.json:
```json
{
    "body": "<a id='comment:39'>Comment 39:</a>\nAttachment: **[trac_11888_v7.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v7.patch.gz)**",
    "created_at": "2012-03-19T20:00:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127034",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:39'>Comment 39:</a>
Attachment: **[trac_11888_v7.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v7.patch.gz)**



---

archive/issue_comments_127035.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -40,5 +40,4 @@\n \n Apply:\n \n-* Patch at #12507\n-* [attachment: trac_11888_v5.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v5.patch.gz) to `$SAGE_ROOT/devel/sage`\n+* [attachment: trac_11888_v7.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v7.patch.gz) to `$SAGE_ROOT/devel/sage`\n``````\n",
    "created_at": "2012-03-19T20:00:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127035",
    "user": "https://github.com/benjaminfjones"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -40,5 +40,4 @@
 
 Apply:
 
-* Patch at #12507
-* [attachment: trac_11888_v5.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v5.patch.gz) to `$SAGE_ROOT/devel/sage`
+* [attachment: trac_11888_v7.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v7.patch.gz) to `$SAGE_ROOT/devel/sage`
``````




---

archive/issue_comments_127036.json:
```json
{
    "body": "<a id='comment:40'>Comment 40:</a>\nApply trac_11888_v7.patch\n\n(for patchbot, which is trying to apply all nine patches at once)",
    "created_at": "2012-03-26T07:10:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127036",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:40'>Comment 40:</a>
Apply trac_11888_v7.patch

(for patchbot, which is trying to apply all nine patches at once)



---

archive/issue_comments_127037.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -40,4 +40,4 @@\n \n Apply:\n \n-* [attachment: trac_11888_v7.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v7.patch.gz) to `$SAGE_ROOT/devel/sage`\n+* [attachment: trac_11888_v7.2.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v7.2.patch.gz) to `$SAGE_ROOT/devel/sage`\n``````\n",
    "created_at": "2012-04-06T18:45:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127037",
    "user": "https://github.com/benjaminfjones"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -40,4 +40,4 @@
 
 Apply:
 
-* [attachment: trac_11888_v7.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v7.patch.gz) to `$SAGE_ROOT/devel/sage`
+* [attachment: trac_11888_v7.2.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v7.2.patch.gz) to `$SAGE_ROOT/devel/sage`
``````




---

archive/issue_comments_127038.json:
```json
{
    "body": "Changed dependencies from **#12507** to none",
    "created_at": "2012-04-07T22:47:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127038",
    "user": "https://github.com/benjaminfjones"
}
```

Changed dependencies from **#12507** to none



---

archive/issue_comments_127039.json:
```json
{
    "body": "<a id='comment:42'>Comment 42:</a>\nI removed #12507 from dependencies since it was merged in 5.0.beta5 and now the patchbot is getting confused trying to apply #12507 to 5.0.beta12 before testing.\n\nI verified that [attachment: trac_11888_v7.2.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v7.2.patch.gz) applies cleanly to 5.0.beta12 and I'm rerunning a patchbot instance on it.",
    "created_at": "2012-04-07T22:47:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127039",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:42'>Comment 42:</a>
I removed #12507 from dependencies since it was merged in 5.0.beta5 and now the patchbot is getting confused trying to apply #12507 to 5.0.beta12 before testing.

I verified that [attachment: trac_11888_v7.2.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v7.2.patch.gz) applies cleanly to 5.0.beta12 and I'm rerunning a patchbot instance on it.



---

archive/issue_comments_127040.json:
```json
{
    "body": "<a id='comment:43'>Comment 43:</a>\nI'm sure we can finish this off next week in Seattle.  Meanwhile, an interesting update from the Maxima developers about coming attractions:\n\n```\n\nMessage: 4\nDate: Thu, 17 May 2012 04:31:13 +0000 (UTC)\nFrom: Robert Dodier <robert.dodier@gmail.com>\nTo: maxima@math.utexas.edu\nSubject: Re: [Maxima] Generalized Lambert W function - premature\n       commit\nMessage-ID: <jp1uuh$jv8$1@dough.gmane.org>\n\nOn 2012-05-17, David Billinghurst <dbmaxima@gmail.com> wrote:\n\n> Oops. I have accidentally committed some code for Generalized Lambert\n> W function to src/specfn.lisp.  Still getting my head around git.\n\n> The code seems functionally correct, and passes tests in\n> tests/rtest_lambert_w.mac, but I hadn't finished polishing it and it\n> is still undocumented.  Unless anyone objects, I will leave it in\n> place for the time being.\n\nNo problem, OK by me.\n\n> There is a new function generalized_lambert_w(k,z) that returns the\n> kth branch W_k(z).  There are float and bigfloat routines for complex\n> z.  generalized_lambert_w(0,z) is not (yet) simplified to\n> lambert_w(z), as I hadn't decided if this should be done\n> unconditionally or controlled by a flag.  Thoughts?\n\nIs it more convenient to simplify W_0(z) instead of W(z) ? If not, then\nit seems reasonable to just go ahead and simplify it.\n\nIf you decide against automatically simplifying W_0(z) to W(z), I guess\nI hope you don't make it controlled by a flag; flags cause trouble,\nbecause one can't guess by looking at some code how it's going to turn\nout. How about a function to carry out the simplification.\n\n```",
    "created_at": "2012-05-17T12:28:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127040",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:43'>Comment 43:</a>
I'm sure we can finish this off next week in Seattle.  Meanwhile, an interesting update from the Maxima developers about coming attractions:

```

Message: 4
Date: Thu, 17 May 2012 04:31:13 +0000 (UTC)
From: Robert Dodier <robert.dodier@gmail.com>
To: maxima@math.utexas.edu
Subject: Re: [Maxima] Generalized Lambert W function - premature
       commit
Message-ID: <jp1uuh$jv8$1@dough.gmane.org>

On 2012-05-17, David Billinghurst <dbmaxima@gmail.com> wrote:

> Oops. I have accidentally committed some code for Generalized Lambert
> W function to src/specfn.lisp.  Still getting my head around git.

> The code seems functionally correct, and passes tests in
> tests/rtest_lambert_w.mac, but I hadn't finished polishing it and it
> is still undocumented.  Unless anyone objects, I will leave it in
> place for the time being.

No problem, OK by me.

> There is a new function generalized_lambert_w(k,z) that returns the
> kth branch W_k(z).  There are float and bigfloat routines for complex
> z.  generalized_lambert_w(0,z) is not (yet) simplified to
> lambert_w(z), as I hadn't decided if this should be done
> unconditionally or controlled by a flag.  Thoughts?

Is it more convenient to simplify W_0(z) instead of W(z) ? If not, then
it seems reasonable to just go ahead and simplify it.

If you decide against automatically simplifying W_0(z) to W(z), I guess
I hope you don't make it controlled by a flag; flags cause trouble,
because one can't guess by looking at some code how it's going to turn
out. How about a function to carry out the simplification.

```



---

archive/issue_comments_127041.json:
```json
{
    "body": "<a id='comment:44'>Comment 44:</a>\nThat will be good; should allow us to integrate the non-principle branch. Although, in Sage 5.0, we're still a major version behind the current Maxima release.",
    "created_at": "2012-05-17T20:37:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127041",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:44'>Comment 44:</a>
That will be good; should allow us to integrate the non-principle branch. Although, in Sage 5.0, we're still a major version behind the current Maxima release.



---

archive/issue_comments_127042.json:
```json
{
    "body": "<a id='comment:45'>Comment 45:</a>\nI am reviewing an expository paper about the Lambert W function and it says \"Maple this\" and \"Maple that\".  Let's get this into Sage and stay competitive with the M's!  ;-)\n\nRob, in cheerleader mode",
    "created_at": "2012-05-18T04:09:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127042",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:45'>Comment 45:</a>
I am reviewing an expository paper about the Lambert W function and it says "Maple this" and "Maple that".  Let's get this into Sage and stay competitive with the M's!  ;-)

Rob, in cheerleader mode



---

archive/issue_comments_127043.json:
```json
{
    "body": "removed trailing whitespace",
    "created_at": "2012-05-18T04:40:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127043",
    "user": "https://github.com/benjaminfjones"
}
```

removed trailing whitespace



---

archive/issue_comments_127044.json:
```json
{
    "body": "<a id='comment:46'>Comment 46:</a>\nAttachment: **[trac_11888_v7.2.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v7.2.patch.gz)**\n\nI agree! In that spirit, here is a rebase of the patch for Sage-5.0.",
    "created_at": "2012-05-18T04:41:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127044",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:46'>Comment 46:</a>
Attachment: **[trac_11888_v7.2.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v7.2.patch.gz)**

I agree! In that spirit, here is a rebase of the patch for Sage-5.0.



---

archive/issue_comments_127045.json:
```json
{
    "body": "<a id='comment:47'>Comment 47:</a>\nthis ticket is pointed out on the SD40.5 wiki page. Is there any particular thing one should review?\n\nPaul",
    "created_at": "2012-05-25T09:27:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127045",
    "user": "https://github.com/zimmermann6"
}
```

<a id='comment:47'>Comment 47:</a>
this ticket is pointed out on the SD40.5 wiki page. Is there any particular thing one should review?

Paul



---

archive/issue_comments_127046.json:
```json
{
    "body": "<a id='comment:48'>Comment 48:</a>\nIt looks like a few principle/principal mixups made it through.",
    "created_at": "2012-05-25T12:25:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127046",
    "user": "https://github.com/sagetrac-dsm"
}
```

<a id='comment:48'>Comment 48:</a>
It looks like a few principle/principal mixups made it through.



---

archive/issue_comments_127047.json:
```json
{
    "body": "<a id='comment:49'>Comment 49:</a>\n> this ticket is pointed out on the SD40.5 wiki page. Is there any particular thing one should review?\n\nI think that just checking everything still works and that syntax is proper (and spelling, thanks dsm) is good.  Checking some random values against another program would be helpful, especially for the branches.  Making sure documentation builds and looks good.  But this has been looked at by a lot of eyes, so I don't think it needs to be gone over completely from scratch, especially since I think Ben has doctested a lot of the issues raised in previous comments (which are worth scanning).",
    "created_at": "2012-05-25T13:54:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127047",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:49'>Comment 49:</a>
> this ticket is pointed out on the SD40.5 wiki page. Is there any particular thing one should review?

I think that just checking everything still works and that syntax is proper (and spelling, thanks dsm) is good.  Checking some random values against another program would be helpful, especially for the branches.  Making sure documentation builds and looks good.  But this has been looked at by a lot of eyes, so I don't think it needs to be gone over completely from scratch, especially since I think Ben has doctested a lot of the issues raised in previous comments (which are worth scanning).



---

archive/issue_comments_127048.json:
```json
{
    "body": "Changed keywords from **lambert_w symbolics conversion maxima sd35.5** to **lambert_w symbolics conversion maxima sd35.5 sd40.5**",
    "created_at": "2012-05-25T18:10:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127048",
    "user": "https://github.com/benjaminfjones"
}
```

Changed keywords from **lambert_w symbolics conversion maxima sd35.5** to **lambert_w symbolics conversion maxima sd35.5 sd40.5**



---

archive/issue_comments_127049.json:
```json
{
    "body": "<a id='comment:51'>Comment 51:</a>\nI can make a quick spelling fix patch, but I'll wait and see if anyone else has changes to suggest.",
    "created_at": "2012-05-25T18:18:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127049",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:51'>Comment 51:</a>
I can make a quick spelling fix patch, but I'll wait and see if anyone else has changes to suggest.



---

archive/issue_comments_127050.json:
```json
{
    "body": "<a id='comment:52'>Comment 52:</a>\nAfter some real-world discussions, I'd prefer to avoid falling into Python ints for (some of) the special values:\n\n```\nsage: parent(lambert_w(0))\nInteger Ring\nsage: parent(lambert_w(e))\n<type 'int'>\nsage: parent(lambert_w(-1/e))\n<type 'int'>\nsage: parent(lambert_w(SR(-1/e)))\n<type 'int'>\nsage: parent(lambert_w(SR(0)))\nInteger Ring\n```\n\nMysteriously enough, instrumenting it reveals that `_eval_` is actually returning SR(1) which then in some part of the code I don't understand becomes int(1) before we get it back.  If we explicitly return Integer(1) then it seems to stay as Integer(1).  This isn't the biggest deal in the world but there have been several bug reports caused by something falling out of Sagespace into Pythonspace.",
    "created_at": "2012-05-26T07:14:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127050",
    "user": "https://github.com/sagetrac-dsm"
}
```

<a id='comment:52'>Comment 52:</a>
After some real-world discussions, I'd prefer to avoid falling into Python ints for (some of) the special values:

```
sage: parent(lambert_w(0))
Integer Ring
sage: parent(lambert_w(e))
<type 'int'>
sage: parent(lambert_w(-1/e))
<type 'int'>
sage: parent(lambert_w(SR(-1/e)))
<type 'int'>
sage: parent(lambert_w(SR(0)))
Integer Ring
```

Mysteriously enough, instrumenting it reveals that `_eval_` is actually returning SR(1) which then in some part of the code I don't understand becomes int(1) before we get it back.  If we explicitly return Integer(1) then it seems to stay as Integer(1).  This isn't the biggest deal in the world but there have been several bug reports caused by something falling out of Sagespace into Pythonspace.



---

archive/issue_comments_127051.json:
```json
{
    "body": "<a id='comment:53'>Comment 53:</a>\nIt looks like whatever happens after `_eval_` might \"dereference\" the SR; the call seems to give Integer(1) if `_eval_` returns SR(Integer(1)).  Probably someone who actually knows what's going on could explain it in one line.",
    "created_at": "2012-05-26T20:45:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127051",
    "user": "https://github.com/sagetrac-dsm"
}
```

<a id='comment:53'>Comment 53:</a>
It looks like whatever happens after `_eval_` might "dereference" the SR; the call seems to give Integer(1) if `_eval_` returns SR(Integer(1)).  Probably someone who actually knows what's going on could explain it in one line.



---

archive/issue_comments_127052.json:
```json
{
    "body": "<a id='comment:54'>Comment 54:</a>\nOne solution is to change the return statements in these special cases where the automatic simplification returns an integer to just explicitly return `Integer(1)`, etc..\n\nHow does that sound?",
    "created_at": "2012-05-27T06:13:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127052",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:54'>Comment 54:</a>
One solution is to change the return statements in these special cases where the automatic simplification returns an integer to just explicitly return `Integer(1)`, etc..

How does that sound?



---

archive/issue_comments_127053.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -40,4 +40,4 @@\n \n Apply:\n \n-* [attachment: trac_11888_v7.2.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v7.2.patch.gz) to `$SAGE_ROOT/devel/sage`\n+* [attachment: trac_11888_v8.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v8.patch.gz) to `$SAGE_ROOT/devel/sage`\n``````\n",
    "created_at": "2012-05-27T18:33:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127053",
    "user": "https://github.com/benjaminfjones"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -40,4 +40,4 @@
 
 Apply:
 
-* [attachment: trac_11888_v7.2.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v7.2.patch.gz) to `$SAGE_ROOT/devel/sage`
+* [attachment: trac_11888_v8.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v8.patch.gz) to `$SAGE_ROOT/devel/sage`
``````




---

archive/issue_comments_127054.json:
```json
{
    "body": "<a id='comment:55'>Comment 55:</a>\nNew patch is ready for review. I hope this can be the final revision! \n\nPatchbot: apply [This is the Trac macro *attachment:trac_11888_v8.patch* that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#attachment:trac_11888_v8.patch-macro) to $SAGE_ROOT/devel/sage",
    "created_at": "2012-05-27T18:33:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127054",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:55'>Comment 55:</a>
New patch is ready for review. I hope this can be the final revision! 

Patchbot: apply [This is the Trac macro *attachment:trac_11888_v8.patch* that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#attachment:trac_11888_v8.patch-macro) to $SAGE_ROOT/devel/sage



---

archive/issue_comments_127055.json:
```json
{
    "body": "<a id='comment:56'>Comment 56:</a>\nLooking at it now.",
    "created_at": "2012-05-27T20:02:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127055",
    "user": "https://github.com/sagetrac-dsm"
}
```

<a id='comment:56'>Comment 56:</a>
Looking at it now.



---

archive/issue_comments_127056.json:
```json
{
    "body": "fixed spelling / grammer mistakes, returned parent(Integer(...)) for special values",
    "created_at": "2012-05-27T20:41:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127056",
    "user": "https://github.com/benjaminfjones"
}
```

fixed spelling / grammer mistakes, returned parent(Integer(...)) for special values



---

archive/issue_comments_127057.json:
```json
{
    "body": "<a id='comment:57'>Comment 57:</a>\nAttachment: **[trac_11888_v8.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v8.patch.gz)**\n\nOkay, this looks good.  Two copyedits, an extra doc describing the behaviours of the derivative function, some tests making sure we can't differentiate with respect to the branch number, and the addition of lambert_W(-pi/2) = pi/2*I as a special value.\n\nI give positive review to the preexisting parts of v8; if the new bits of v9 look okay I think we're good to go.",
    "created_at": "2012-05-27T21:18:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127057",
    "user": "https://github.com/sagetrac-dsm"
}
```

<a id='comment:57'>Comment 57:</a>
Attachment: **[trac_11888_v8.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v8.patch.gz)**

Okay, this looks good.  Two copyedits, an extra doc describing the behaviours of the derivative function, some tests making sure we can't differentiate with respect to the branch number, and the addition of lambert_W(-pi/2) = pi/2*I as a special value.

I give positive review to the preexisting parts of v8; if the new bits of v9 look okay I think we're good to go.



---

archive/issue_comments_127058.json:
```json
{
    "body": "Attachment: **[trac_11888_v9.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v9.patch.gz)**\n\npost-review version of lambertw sf",
    "created_at": "2012-05-27T21:18:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127058",
    "user": "https://github.com/sagetrac-dsm"
}
```

Attachment: **[trac_11888_v9.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v9.patch.gz)**

post-review version of lambertw sf



---

archive/issue_comments_127059.json:
```json
{
    "body": "<a id='comment:58'>Comment 58:</a>\nNew changes look good; good catch about the derivative w.r.t. branch. All relavent tests pass for me on sage-5.0. I would say this is ready to go in! Thanks for the very thorough review, Doug.",
    "created_at": "2012-05-27T22:06:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127059",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:58'>Comment 58:</a>
New changes look good; good catch about the derivative w.r.t. branch. All relavent tests pass for me on sage-5.0. I would say this is ready to go in! Thanks for the very thorough review, Doug.



---

archive/issue_events_146454.json:
```json
{
    "actor": "https://github.com/sagetrac-dsm",
    "created_at": "2012-05-27T22:09:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11888#event-146454"
}
```



---

archive/issue_events_146455.json:
```json
{
    "actor": "https://github.com/sagetrac-dsm",
    "created_at": "2012-05-27T22:09:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11888#event-146455"
}
```



---

archive/issue_comments_127060.json:
```json
{
    "body": "Changed reviewer from **Keshav Kini, Karl-Dieter Crisman, Fredrik Johansson, Burcin Erocal** to **Keshav Kini, Karl-Dieter Crisman, Fredrik Johansson, Burcin Erocal, Douglas McNeil**",
    "created_at": "2012-05-27T22:09:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127060",
    "user": "https://github.com/sagetrac-dsm"
}
```

Changed reviewer from **Keshav Kini, Karl-Dieter Crisman, Fredrik Johansson, Burcin Erocal** to **Keshav Kini, Karl-Dieter Crisman, Fredrik Johansson, Burcin Erocal, Douglas McNeil**



---

archive/issue_comments_127061.json:
```json
{
    "body": "<a id='comment:60'>Comment 60:</a>\n+1.  Tx for the work!",
    "created_at": "2012-05-27T22:10:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127061",
    "user": "https://github.com/sagetrac-dsm"
}
```

<a id='comment:60'>Comment 60:</a>
+1.  Tx for the work!



---

archive/issue_comments_127062.json:
```json
{
    "body": "Changed reviewer from **Keshav Kini, Karl-Dieter Crisman, Fredrik Johansson, Burcin Erocal, Douglas McNeil** to **Keshav Kini, Karl-Dieter Crisman, Fredrik Johansson, Burcin Erocal, Douglas McNeil, William Stein**",
    "created_at": "2012-05-27T22:18:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127062",
    "user": "https://github.com/williamstein"
}
```

Changed reviewer from **Keshav Kini, Karl-Dieter Crisman, Fredrik Johansson, Burcin Erocal, Douglas McNeil** to **Keshav Kini, Karl-Dieter Crisman, Fredrik Johansson, Burcin Erocal, Douglas McNeil, William Stein**



---

archive/issue_events_146456.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-05-27T22:18:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11888#event-146456"
}
```



---

archive/issue_events_146457.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-05-27T22:18:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11888#event-146457"
}
```



---

archive/issue_comments_127063.json:
```json
{
    "body": "<a id='comment:61'>Comment 61:</a>\nThis patch is awesome!  It's also a great example of how to make a well-documented new symbolic function that illustrates many issues. Here are a few trivial nitpicks:\n\n* What is \"simplication\"?\n\n```\nWhen automatic simplication occurs, the parent of the output value should be \n```\n\n* This docstring should start with r\" since it contains a backslash:\n\n```\n        646\t        \"\"\" \n \t647\t        The derivative of `W_n(x)` is `W_n(x)/(x \\cdot W_n(x) + x)`. \n```\n(check for similar instances throughout).\n\n* Don't use periods at the end of exceptions (also don't capitalize).  Many instances of this being wrong, e.g., \n\n```\n \t679\t            raise ValueError(\"Derivative not defined with respect to the branch number.\") \n```\nHere's a good example of what an exception string should look like (built into python):\n\n```\n>>> 1/0\nTraceback (most recent call last):\n  File \"<stdin>\", line 1, in <module>\nZeroDivisionError: integer division or modulo by zero\n```",
    "created_at": "2012-05-27T22:18:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127063",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:61'>Comment 61:</a>
This patch is awesome!  It's also a great example of how to make a well-documented new symbolic function that illustrates many issues. Here are a few trivial nitpicks:

* What is "simplication"?

```
When automatic simplication occurs, the parent of the output value should be 
```

* This docstring should start with r" since it contains a backslash:

```
        646	        """ 
 	647	        The derivative of `W_n(x)` is `W_n(x)/(x \cdot W_n(x) + x)`. 
```
(check for similar instances throughout).

* Don't use periods at the end of exceptions (also don't capitalize).  Many instances of this being wrong, e.g., 

```
 	679	            raise ValueError("Derivative not defined with respect to the branch number.") 
```
Here's a good example of what an exception string should look like (built into python):

```
>>> 1/0
Traceback (most recent call last):
  File "<stdin>", line 1, in <module>
ZeroDivisionError: integer division or modulo by zero
```



---

archive/issue_comments_127064.json:
```json
{
    "body": "<a id='comment:62'>Comment 62:</a>\nUpdated version taking into account comments of was.",
    "created_at": "2012-05-27T22:33:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127064",
    "user": "https://github.com/sagetrac-dsm"
}
```

<a id='comment:62'>Comment 62:</a>
Updated version taking into account comments of was.



---

archive/issue_events_146458.json:
```json
{
    "actor": "https://github.com/sagetrac-dsm",
    "created_at": "2012-05-27T22:34:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11888#event-146458"
}
```



---

archive/issue_events_146459.json:
```json
{
    "actor": "https://github.com/sagetrac-dsm",
    "created_at": "2012-05-27T22:34:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11888#event-146459"
}
```



---

archive/issue_comments_127065.json:
```json
{
    "body": "minor edits",
    "created_at": "2012-05-27T22:46:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127065",
    "user": "https://github.com/sagetrac-dsm"
}
```

minor edits



---

archive/issue_comments_127066.json:
```json
{
    "body": "<a id='comment:64'>Comment 64:</a>\nAttachment: **[trac_11888_v10.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v10.patch.gz)**",
    "created_at": "2012-05-27T22:49:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127066",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:64'>Comment 64:</a>
Attachment: **[trac_11888_v10.patch.gz](https://github.com/sagemath/sage/files/ticket11888/trac_11888_v10.patch.gz)**



---

archive/issue_events_146460.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-05-27T22:49:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11888#event-146460"
}
```



---

archive/issue_events_146461.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-05-27T22:49:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11888#event-146461"
}
```



---

archive/issue_events_146462.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-14T06:38:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11888#event-146462"
}
```



---

archive/issue_events_146463.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-14T06:38:22Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11888#event-146463"
}
```



---

archive/issue_comments_127067.json:
```json
{
    "body": "Merged: **sage-5.1.beta4**",
    "created_at": "2012-06-14T06:38:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11888#issuecomment-127067",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.1.beta4**
