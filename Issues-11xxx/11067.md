# Issue 11067: make it possible to use lstlisting instead of verbose in latex document without breaking "sage -t"

archive/issues_010949.json:
```json
{
    "assignees": [],
    "body": "Right now example code has to be in \\begin{verbatim}/\\end{verbatim} blocks.  However, lstlisting looks much better.  With this patch one can write latex documents and have \"sage -t\" work on them, if they use one or the other type of blocks.\n\nIt's probably not optimal -- it would be better to maybe make this customizable or something.  I don't know.   \n\nAssignee: **mvngu**\n\nKeywords: **doctest, latex**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11067_\n\n",
    "closed_at": "2014-08-20T20:38:00Z",
    "created_at": "2011-03-27T22:47:50Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20doctest%20framework",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "make it possible to use lstlisting instead of verbose in latex document without breaking \"sage -t\"",
    "type": "issue",
    "updated_at": "2014-08-20T20:38:00Z",
    "url": "https://github.com/sagemath/sage/issues/11067",
    "user": "https://github.com/williamstein"
}
```
Right now example code has to be in \begin{verbatim}/\end{verbatim} blocks.  However, lstlisting looks much better.  With this patch one can write latex documents and have "sage -t" work on them, if they use one or the other type of blocks.

It's probably not optimal -- it would be better to maybe make this customizable or something.  I don't know.   

Assignee: **mvngu**

Keywords: **doctest, latex**

_Issue created by migration from https://trac.sagemath.org/ticket/11067_





---

archive/issue_events_133864.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-03-27T22:47:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11067",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11067#event-133864"
}
```



---

archive/issue_events_133865.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-03-27T22:47:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11067",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11067#event-133865"
}
```



---

archive/issue_events_133866.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-03-27T22:47:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11067",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11067#event-133866"
}
```



---

archive/issue_events_133867.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-03-27T22:47:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11067",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11067#event-133867"
}
```



---

archive/issue_comments_111084.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[trac_11067.patch.gz](https://github.com/sagemath/sage/files/ticket11067/trac_11067.patch.gz)**",
    "created_at": "2011-03-27T22:48:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11067",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11067#issuecomment-111084",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[trac_11067.patch.gz](https://github.com/sagemath/sage/files/ticket11067/trac_11067.patch.gz)**



---

archive/issue_events_133868.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-03-27T22:48:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11067",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11067#event-133868"
}
```



---

archive/issue_comments_111085.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThis ticket may be related to #10146 (SageTeX with support for lstlistings).",
    "created_at": "2011-03-28T13:50:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11067",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11067#issuecomment-111085",
    "user": "https://github.com/novoselt"
}
```

<a id='comment:2'>Comment 2:</a>
This ticket may be related to #10146 (SageTeX with support for lstlistings).



---

archive/issue_comments_111086.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nPlease fill in your real name as Author.",
    "created_at": "2012-07-27T20:42:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11067",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11067#issuecomment-111086",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:3'>Comment 3:</a>
Please fill in your real name as Author.



---

archive/issue_events_133869.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2013-03-28T23:25:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11067",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11067#event-133869"
}
```



---

archive/issue_events_133870.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2013-03-28T23:25:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11067",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20doctest%20framework",
    "label_color": "000080",
    "label_name": "component: doctest framework",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11067#event-133870"
}
```



---

archive/issue_events_133871.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11067",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11067#event-133871"
}
```



---

archive/issue_events_133872.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11067",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11067#event-133872"
}
```



---

archive/issue_events_133873.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11067",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11067#event-133873"
}
```



---

archive/issue_events_133874.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11067",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11067#event-133874"
}
```



---

archive/issue_events_133875.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11067",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11067#event-133875"
}
```



---

archive/issue_events_133876.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11067",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11067#event-133876"
}
```



---

archive/issue_comments_111087.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nI propose to close this as a duplicate of #3912 (which needs review)",
    "created_at": "2014-07-25T11:47:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11067",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11067#issuecomment-111087",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:8'>Comment 8:</a>
I propose to close this as a duplicate of #3912 (which needs review)



---

archive/issue_comments_111088.json:
```json
{
    "body": "Changed keywords from **** to **doctest, latex**",
    "created_at": "2014-07-25T11:47:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11067",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11067#issuecomment-111088",
    "user": "https://github.com/fchapoton"
}
```

Changed keywords from **** to **doctest, latex**



---

archive/issue_events_133877.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-07-25T11:47:16Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11067",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11067#event-133877"
}
```



---

archive/issue_events_133878.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-07-25T11:47:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11067",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11067#event-133878"
}
```



---

archive/issue_events_133879.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-07-25T11:47:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11067",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11067#event-133879"
}
```



---

archive/issue_events_133880.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2014-07-26T13:04:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11067",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11067#event-133880"
}
```



---

archive/issue_events_133881.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2014-07-26T13:04:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11067",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11067#event-133881"
}
```



---

archive/issue_comments_111089.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nThanks a lot ! Would you maybe have time to have a look at #3912 ?",
    "created_at": "2014-07-26T19:15:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11067",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11067#issuecomment-111089",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:10'>Comment 10:</a>
Thanks a lot ! Would you maybe have time to have a look at #3912 ?



---

archive/issue_events_133882.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-08-20T20:38:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11067",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11067#event-133882"
}
```



---

archive/issue_events_133883.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-08-20T20:38:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11067",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11067#event-133883"
}
```



---

archive/issue_events_133884.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-08-20T20:38:00Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11067",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11067#event-133884"
}
```
