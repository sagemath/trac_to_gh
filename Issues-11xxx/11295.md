# Issue 11295: FiniteField_givaroElement: make some methods private

archive/issues_011123.json:
```json
{
    "assignees": [],
    "body": "The methods `log_to_int`, `int_repr`, `log_repr` and `poly_repr` should probably not be used by user code. Make them private (prepend an underscore) and deprecate the non-underscored versions.\n\nAssignee: **@ClementPernet**\n\nBranch/Commit: **[fff80ed](https://github.com/sagemath/sagetrac-mirror/commit/fff80ed2653f9e0ac5f791a034b72cc04f495bb0)**\n\nReviewer: **Peter Bruin**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11295_\n\n",
    "closed_at": "2015-01-02T22:12:28Z",
    "created_at": "2011-05-05T01:36:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20finite%20rings",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "FiniteField_givaroElement: make some methods private",
    "type": "issue",
    "updated_at": "2015-01-02T22:12:28Z",
    "url": "https://github.com/sagemath/sage/issues/11295",
    "user": "https://github.com/nbruin"
}
```
The methods `log_to_int`, `int_repr`, `log_repr` and `poly_repr` should probably not be used by user code. Make them private (prepend an underscore) and deprecate the non-underscored versions.

Assignee: **@ClementPernet**

Branch/Commit: **[fff80ed](https://github.com/sagemath/sagetrac-mirror/commit/fff80ed2653f9e0ac5f791a034b72cc04f495bb0)**

Reviewer: **Peter Bruin**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/11295_





---

archive/issue_events_136756.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2011-05-05T01:36:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11295#event-136756"
}
```



---

archive/issue_events_136757.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2011-05-05T01:36:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11295#event-136757"
}
```



---

archive/issue_events_136758.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2011-05-05T01:36:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11295#event-136758"
}
```



---

archive/issue_events_136759.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2011-05-05T01:36:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11295#event-136759"
}
```



---

archive/issue_events_136760.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11295#event-136760"
}
```



---

archive/issue_events_136761.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11295#event-136761"
}
```



---

archive/issue_events_136762.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11295#event-136762"
}
```



---

archive/issue_events_136763.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11295#event-136763"
}
```



---

archive/issue_events_136764.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11295#event-136764"
}
```



---

archive/issue_events_136765.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11295#event-136765"
}
```



---

archive/issue_events_136766.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11295#event-136766"
}
```



---

archive/issue_events_136767.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11295#event-136767"
}
```



---

archive/issue_comments_115401.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,11 +1 @@\n-\n-```\n-sage: F.<a>=GF(2^2)\n-sage: a.log_to_int?\n-...\n-       Returns the int representation of self, as a Sage integer.   Use\n-       int(self) to directly get a Python int.\n-...\n-sage: int(a)\n-TypeError: Cannot coerce element to an integer.\n-```\n+The methods `log_to_int`, `int_repr`, `log_repr` and `poly_repr` should probably not be used by user code. Make them private (prepend an underscore) and deprecate the non-underscored versions.\n``````\n",
    "created_at": "2014-11-25T13:52:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11295#issuecomment-115401",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,11 +1 @@
-
-```
-sage: F.<a>=GF(2^2)
-sage: a.log_to_int?
-...
-       Returns the int representation of self, as a Sage integer.   Use
-       int(self) to directly get a Python int.
-...
-sage: int(a)
-TypeError: Cannot coerce element to an integer.
-```
+The methods `log_to_int`, `int_repr`, `log_repr` and `poly_repr` should probably not be used by user code. Make them private (prepend an underscore) and deprecate the non-underscored versions.
``````




---

archive/issue_events_136768.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-11-25T13:52:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11295#event-136768"
}
```



---

archive/issue_events_136769.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-11-25T13:52:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20finite%20rings",
    "label_color": "0000ff",
    "label_name": "component: finite rings",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11295#event-136769"
}
```



---

archive/issue_comments_115402.json:
```json
{
    "body": "Author: **Jeroen Demeyer**",
    "created_at": "2014-11-25T13:52:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11295#issuecomment-115402",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Jeroen Demeyer**



---

archive/issue_comments_115403.json:
```json
{
    "body": "Changed assignee from **@sagetrac-mvngu** to **@ClementPernet**",
    "created_at": "2014-11-25T13:52:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11295#issuecomment-115403",
    "user": "https://github.com/jdemeyer"
}
```

Changed assignee from **@sagetrac-mvngu** to **@ClementPernet**



---

archive/issue_events_136770.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-11-25T13:52:28Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "title_is": "FiniteField_givaroElement: make some methods private",
    "title_was": "FiniteField_givaroElement.log_to_int documentation",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11295#event-136770"
}
```



---

archive/issue_events_136771.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-11-25T13:52:28Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11295#event-136771"
}
```



---

archive/issue_events_136772.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-11-25T13:52:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "milestone_number": null,
    "milestone_title": "sage-6.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11295#event-136772"
}
```



---

archive/issue_comments_115404.json:
```json
{
    "body": "Branch: **[u/jdemeyer/ticket/11295](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/11295)**",
    "created_at": "2014-11-25T16:14:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11295#issuecomment-115404",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/ticket/11295](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/11295)**



---

archive/issue_comments_115405.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fff80ed2653f9e0ac5f791a034b72cc04f495bb0\">fff80ed</a></td><td><code>Make some methods private</code></td></tr></table>\n",
    "created_at": "2014-11-25T16:14:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11295#issuecomment-115405",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:7" align="right">Comment 7</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fff80ed2653f9e0ac5f791a034b72cc04f495bb0">fff80ed</a></td><td><code>Make some methods private</code></td></tr></table>




---

archive/issue_comments_115406.json:
```json
{
    "body": "Commit: **[fff80ed](https://github.com/sagemath/sagetrac-mirror/commit/fff80ed2653f9e0ac5f791a034b72cc04f495bb0)**",
    "created_at": "2014-11-25T16:14:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11295#issuecomment-115406",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[fff80ed](https://github.com/sagemath/sagetrac-mirror/commit/fff80ed2653f9e0ac5f791a034b72cc04f495bb0)**



---

archive/issue_events_136773.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-11-25T16:14:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11295#event-136773"
}
```



---

archive/issue_comments_115407.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nLooks good and passes tests.",
    "created_at": "2014-12-23T11:08:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11295#issuecomment-115407",
    "user": "https://github.com/pjbruin"
}
```

<div id="comment:8" align="right">Comment 8</div>

Looks good and passes tests.



---

archive/issue_comments_115408.json:
```json
{
    "body": "Reviewer: **Peter Bruin**",
    "created_at": "2014-12-23T11:08:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11295#issuecomment-115408",
    "user": "https://github.com/pjbruin"
}
```

Reviewer: **Peter Bruin**



---

archive/issue_events_136774.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2014-12-23T11:08:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11295#event-136774"
}
```



---

archive/issue_events_136775.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2014-12-23T11:08:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11295#event-136775"
}
```



---

archive/issue_comments_115409.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/ticket/11295](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/11295)** to **[fff80ed](https://github.com/sagemath/sagetrac-mirror/commit/fff80ed2653f9e0ac5f791a034b72cc04f495bb0)**",
    "created_at": "2015-01-02T22:12:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11295#issuecomment-115409",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/ticket/11295](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/11295)** to **[fff80ed](https://github.com/sagemath/sagetrac-mirror/commit/fff80ed2653f9e0ac5f791a034b72cc04f495bb0)**



---

archive/issue_events_136776.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-01-02T22:12:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11295#event-136776"
}
```



---

archive/issue_events_136777.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "00bcd7dadbe6d1adf644ca61b54c00926002e693",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-01-02T22:12:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11295",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11295#event-136777"
}
```
