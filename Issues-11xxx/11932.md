# Issue 11932: Allow running Sage with a temporary DOT_SAGE directory

archive/issues_011760.json:
```json
{
    "body": "Version which deals with matplotlib issue: apply \n\n- [attachment:trac_11932-root.v2.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-root.v2.patch) to the Sage root repository\n- [attachment:trac_11932-scripts.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-scripts.patch) to the local/bin repository\n- [attachment:trac_11932-doc.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-doc.patch) to the Sage repository\n- [attachment:trac_11932-sage.v2.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-sage.v2.patch) to the Sage repository\n\n\n\nDepends on #11926\n\n**Assignee:** @nexttime\n\n**CC:**  @jhpalmieri\n\n**Reviewer:** Jeroen Demeyer\n\n**Author:** John Palmieri\n\n**Merged:** sage-5.1.beta0\n\nIssue created by migration from https://trac.sagemath.org/ticket/11932\n\n",
    "closed_at": "2012-05-13T21:32:43Z",
    "created_at": "2011-10-17T18:29:51Z",
    "labels": [
        "component: scripts",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.1",
    "title": "Allow running Sage with a temporary DOT_SAGE directory",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/11932",
    "user": "https://github.com/jdemeyer"
}
```
Version which deals with matplotlib issue: apply 

- [attachment:trac_11932-root.v2.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-root.v2.patch) to the Sage root repository
- [attachment:trac_11932-scripts.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-scripts.patch) to the local/bin repository
- [attachment:trac_11932-doc.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-doc.patch) to the Sage repository
- [attachment:trac_11932-sage.v2.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-sage.v2.patch) to the Sage repository



Depends on #11926

**Assignee:** @nexttime

**CC:**  @jhpalmieri

**Reviewer:** Jeroen Demeyer

**Author:** John Palmieri

**Merged:** sage-5.1.beta0

Issue created by migration from https://trac.sagemath.org/ticket/11932





---

archive/issue_comments_128222.json:
```json
{
    "body": "<a id='comment:1'></a>\nI copied some patches from #11926 (and make some small changes to John's patch).",
    "created_at": "2011-10-17T18:41:39Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128222",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:1'></a>
I copied some patches from #11926 (and make some small changes to John's patch).



---

archive/issue_comments_128223.json:
```json
{
    "body": "**Dependencies:** #11926",
    "created_at": "2011-10-17T18:41:50Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128223",
    "user": "https://github.com/jdemeyer"
}
```

**Dependencies:** #11926



---

archive/issue_comments_128224.json:
```json
{
    "body": "<a id='comment:3'></a>\nI'd rename the function (as it creates a temporary *directory*), and not use a global variable for the \"return value\".\n\nThe function could either take the variable name as its parameter (such that `sage_make_tmp_dir DIR` assigns the directory name to `DIR`), or just \"echo\" the result (as `mktemp` already does), such that one writes `DIR=sage_make_tmp_dir` or whatever.",
    "created_at": "2011-10-17T21:55:47Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128224",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:3'></a>
I'd rename the function (as it creates a temporary *directory*), and not use a global variable for the "return value".

The function could either take the variable name as its parameter (such that `sage_make_tmp_dir DIR` assigns the directory name to `DIR`), or just "echo" the result (as `mktemp` already does), such that one writes `DIR=sage_make_tmp_dir` or whatever.



---

archive/issue_comments_128225.json:
```json
{
    "body": "<a id='comment:4'></a>\nReplying to [leif](#comment%3A3):\n> or just \"echo\" the result (as `mktemp` already does), such that one writes `DIR=sage_make_tmp_dir` or whatever.\n\n\n<code>DIR=\\`sage_make_tmp_dir\\`</code> of course.",
    "created_at": "2011-10-18T06:06:37Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128225",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:4'></a>
Replying to [leif](#comment%3A3):
> or just "echo" the result (as `mktemp` already does), such that one writes `DIR=sage_make_tmp_dir` or whatever.


<code>DIR=\`sage_make_tmp_dir\`</code> of course.



---

archive/issue_comments_128226.json:
```json
{
    "body": "<a id='comment:5'></a>\nHere's a version in which the function echoes the path of the temp dir.\n\nThis works best with the patch from #11924: without applying the patch there, the parent directory of the temporary .sage directory gets polluted with pickle cache files.",
    "created_at": "2011-10-18T18:49:56Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128226",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:5'></a>
Here's a version in which the function echoes the path of the temp dir.

This works best with the patch from #11924: without applying the patch there, the parent directory of the temporary .sage directory gets polluted with pickle cache files.



---

archive/issue_comments_128227.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1 +1,5 @@\n+Apply \n \n+- [attachment:11932_sage_root.patch](https://github.com/sagemath/sage/files/ticket11932/11932_sage_root.patch) to the Sage root repository\n+- [attachment:trac_11932-scripts.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-scripts.patch) to the scripts repository\n+\n``````\n",
    "created_at": "2011-10-18T18:49:56Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128227",
    "user": "https://github.com/jhpalmieri"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1 +1,5 @@
+Apply 
 
+- [attachment:11932_sage_root.patch](https://github.com/sagemath/sage/files/ticket11932/11932_sage_root.patch) to the Sage root repository
+- [attachment:trac_11932-scripts.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-scripts.patch) to the scripts repository
+
``````




---

archive/issue_events_096167.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2011-10-18T18:49:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11932#event-96167"
}
```



---

archive/issue_comments_128228.json:
```json
{
    "body": "Milestone sage-4.7.3 deleted",
    "created_at": "2011-11-03T16:14:43Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128228",
    "user": "https://github.com/jdemeyer"
}
```

Milestone sage-4.7.3 deleted



---

archive/issue_events_096168.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-11-03T16:14:43Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "milestone": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11932#event-96168"
}
```



---

archive/issue_events_096169.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-02-28T22:16:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "milestone": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11932#event-96169"
}
```



---

archive/issue_comments_128229.json:
```json
{
    "body": "<a id='comment:7'></a>\nRebased to #11073.  Documentation added.",
    "created_at": "2012-02-28T22:16:29Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128229",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:7'></a>
Rebased to #11073.  Documentation added.



---

archive/issue_comments_128230.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,5 +1,5 @@\n Apply \n \n-- [attachment:11932_sage_root.patch](https://github.com/sagemath/sage/files/ticket11932/11932_sage_root.patch) to the Sage root repository\n-- [attachment:trac_11932-scripts.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-scripts.patch) to the scripts repository\n+- [and [attachment:trac_11932-root.patch](https://github.com/sagemath/sage/files/ticket11932/56187dc07b57e5f564bc337567067372.patch](https://github.com/sagemath/sage/files/ticket11932/a8e54123402fe4e8a866664457cdb37e.patch)) to the Sage root repository\n+- [attachment:trac_11932-doc.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-doc.patch) to the Sage repository\n \n``````\n",
    "created_at": "2012-02-28T22:16:29Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128230",
    "user": "https://github.com/jhpalmieri"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,5 +1,5 @@
 Apply 
 
-- [attachment:11932_sage_root.patch](https://github.com/sagemath/sage/files/ticket11932/11932_sage_root.patch) to the Sage root repository
-- [attachment:trac_11932-scripts.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-scripts.patch) to the scripts repository
+- [and [attachment:trac_11932-root.patch](https://github.com/sagemath/sage/files/ticket11932/56187dc07b57e5f564bc337567067372.patch](https://github.com/sagemath/sage/files/ticket11932/a8e54123402fe4e8a866664457cdb37e.patch)) to the Sage root repository
+- [attachment:trac_11932-doc.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-doc.patch) to the Sage repository
 
``````




---

archive/issue_events_096170.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-29T09:13:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11932#event-96170"
}
```



---

archive/issue_events_096171.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-29T09:13:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11932#event-96171"
}
```



---

archive/issue_comments_128231.json:
```json
{
    "body": "**Reviewer:** Jeroen Demeyer",
    "created_at": "2012-02-29T09:13:44Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128231",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** Jeroen Demeyer



---

archive/issue_comments_128232.json:
```json
{
    "body": "<a id='comment:8'></a>\nThe following seems to be portable:\n\n```\nmktemp -d ${TMPDIR:-/tmp}/dotsageXXXXXX\n```\nso I would use this instead of `sage_mktempdir`.\n\n`sage-sage` needs to be replaced by `sage`.\n\nFinally, why not simply run\n\n```\n./sage --norc </dev/null >/dev/null 2>/dev/null\n```\nin `spkg-install`?",
    "created_at": "2012-02-29T09:13:44Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128232",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:8'></a>
The following seems to be portable:

```
mktemp -d ${TMPDIR:-/tmp}/dotsageXXXXXX
```
so I would use this instead of `sage_mktempdir`.

`sage-sage` needs to be replaced by `sage`.

Finally, why not simply run

```
./sage --norc </dev/null >/dev/null 2>/dev/null
```
in `spkg-install`?



---

archive/issue_comments_128233.json:
```json
{
    "body": "**Author:** John Palmieri",
    "created_at": "2012-02-29T09:13:44Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128233",
    "user": "https://github.com/jdemeyer"
}
```

**Author:** John Palmieri



---

archive/issue_comments_128234.json:
```json
{
    "body": "<a id='comment:9'></a>\nI meant \"in `spkg/install`\".",
    "created_at": "2012-02-29T09:19:57Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128234",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:9'></a>
I meant "in `spkg/install`".



---

archive/issue_comments_128235.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,5 +1,6 @@\n Apply \n \n-- [and [attachment:trac_11932-root.patch](https://github.com/sagemath/sage/files/ticket11932/56187dc07b57e5f564bc337567067372.patch](https://github.com/sagemath/sage/files/ticket11932/a8e54123402fe4e8a866664457cdb37e.patch)) to the Sage root repository\n+- [attachment:trac_11932-root.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-root.patch) to the Sage root repository\n+- [attachment:trac_11932-scripts.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-scripts.patch) to the local/bin repository\n - [attachment:trac_11932-doc.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-doc.patch) to the Sage repository\n \n``````\n",
    "created_at": "2012-02-29T18:14:49Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128235",
    "user": "https://github.com/jhpalmieri"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,5 +1,6 @@
 Apply 
 
-- [and [attachment:trac_11932-root.patch](https://github.com/sagemath/sage/files/ticket11932/56187dc07b57e5f564bc337567067372.patch](https://github.com/sagemath/sage/files/ticket11932/a8e54123402fe4e8a866664457cdb37e.patch)) to the Sage root repository
+- [attachment:trac_11932-root.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-root.patch) to the Sage root repository
+- [attachment:trac_11932-scripts.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-scripts.patch) to the local/bin repository
 - [attachment:trac_11932-doc.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-doc.patch) to the Sage repository
 
``````




---

archive/issue_comments_128236.json:
```json
{
    "body": "<a id='comment:10'></a>\nFixed. The patch to spkg/install is no longer valid; instead, we need to patch the sage-starts script.",
    "created_at": "2012-02-29T18:14:49Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128236",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:10'></a>
Fixed. The patch to spkg/install is no longer valid; instead, we need to patch the sage-starts script.



---

archive/issue_events_096172.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-02-29T18:14:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11932#event-96172"
}
```



---

archive/issue_events_096173.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-02-29T18:14:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11932#event-96173"
}
```



---

archive/issue_events_096174.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-02-29T22:14:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11932#event-96174"
}
```



---

archive/issue_events_096175.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-02-29T22:14:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11932#event-96175"
}
```



---

archive/issue_comments_128237.json:
```json
{
    "body": "<a id='comment:11'></a>\nThis is not working, and I'm probably making some bash error in the `sage` script. When I run\n\n```\nsage -c 'print \"hello\"'\n```\nit works as it should: no banner, just \"hello\", then a new shell prompt.  When I run\n\n```\nsage --norc -c 'print \"hello\"'\n```\nit starts up Sage, prints a banner, prints \"hello\", and then gives the Sage prompt. As a consequence, the `sage-starts` script is not working right. What am I doing wrong?",
    "created_at": "2012-02-29T22:14:56Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128237",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:11'></a>
This is not working, and I'm probably making some bash error in the `sage` script. When I run

```
sage -c 'print "hello"'
```
it works as it should: no banner, just "hello", then a new shell prompt.  When I run

```
sage --norc -c 'print "hello"'
```
it starts up Sage, prints a banner, prints "hello", and then gives the Sage prompt. As a consequence, the `sage-starts` script is not working right. What am I doing wrong?



---

archive/issue_events_096176.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-02-29T22:34:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11932#event-96176"
}
```



---

archive/issue_events_096177.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-02-29T22:34:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11932#event-96177"
}
```



---

archive/issue_comments_128238.json:
```json
{
    "body": "<a id='comment:12'></a>\nNever mind, I've found various work-arounds. The best seems to be to replace `sage \"@$\"` with `source sage \"@$\"`.",
    "created_at": "2012-02-29T22:34:13Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128238",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:12'></a>
Never mind, I've found various work-arounds. The best seems to be to replace `sage "@$"` with `source sage "@$"`.



---

archive/issue_comments_128239.json:
```json
{
    "body": "<a id='comment:13'></a>\nNo, that doesn't work: it doesn't clean up the temporary directory afterward. Using `\"$SAGE_ROOT/sage\" \"$@\"` seems to work, but is that a good solution?",
    "created_at": "2012-02-29T23:37:41Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128239",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:13'></a>
No, that doesn't work: it doesn't clean up the temporary directory afterward. Using `"$SAGE_ROOT/sage" "$@"` seems to work, but is that a good solution?



---

archive/issue_comments_128240.json:
```json
{
    "body": "<a id='comment:14'></a>\nReplying to [jhpalmieri](#comment%3A13):\n> Using `\"$SAGE_ROOT/sage\" \"$@\"` seems to work, but is that a good solution?\n\nI don't see how calling `sage` could be different from calling `$SAGE_ROOT/sage`, are you sure the former doesn't work (I haven't tried).",
    "created_at": "2012-03-14T20:00:03Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128240",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:14'></a>
Replying to [jhpalmieri](#comment%3A13):
> Using `"$SAGE_ROOT/sage" "$@"` seems to work, but is that a good solution?

I don't see how calling `sage` could be different from calling `$SAGE_ROOT/sage`, are you sure the former doesn't work (I haven't tried).



---

archive/issue_comments_128241.json:
```json
{
    "body": "<a id='comment:15'></a>\nCalling `sage` at this point in the script will call the script `spkg/bin/sage`, because that directory is now in the path, so it is conceivably different from calling `$SAGE_ROOT/sage`. In practice, it does behave differently, as I mentioned above: using `$SAGE_ROOT/sage` in the script, the command `sage --norc -c 'print \"hello\"'` works as expected, while using just `sage`, the command prints the Sage banner, prints \"hello\", and then you end up at a Sage prompt, running Sage interactively.",
    "created_at": "2012-03-14T20:41:21Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128241",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:15'></a>
Calling `sage` at this point in the script will call the script `spkg/bin/sage`, because that directory is now in the path, so it is conceivably different from calling `$SAGE_ROOT/sage`. In practice, it does behave differently, as I mentioned above: using `$SAGE_ROOT/sage` in the script, the command `sage --norc -c 'print "hello"'` works as expected, while using just `sage`, the command prints the Sage banner, prints "hello", and then you end up at a Sage prompt, running Sage interactively.



---

archive/issue_comments_128242.json:
```json
{
    "body": "<a id='comment:16'></a>\nI confirm your observation but I still don't understand what's going on.  I'm investigating...",
    "created_at": "2012-03-14T20:50:46Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128242",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:16'></a>
I confirm your observation but I still don't understand what's going on.  I'm investigating...



---

archive/issue_events_096178.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-03-14T20:50:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11932#event-96178"
}
```



---

archive/issue_events_096179.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-03-14T20:50:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11932#event-96179"
}
```



---

archive/issue_comments_128243.json:
```json
{
    "body": "<a id='comment:17'></a>\nOkay, found it.  \"sage\" refers to the shell function\n\n```\nsage() {\n    sage_setup\n    sage-ipython \"$@\" -i\n}\n```\nWe should probably rename this function to `run_sage` or so...\n\nIn the mean time, replace `$SAGE_ROOT/sage \"$@\"` by\n\n```\ncommand sage \"$@\"\n```\n\nHelp of command:\n\n```\ncommand: command [-pVv] command [arg ...]\n    Execute a simple command or display information about commands.\n\n    Runs COMMAND with ARGS suppressing  shell function lookup, or display\n    information about the specified COMMANDs.  Can be used to invoke commands\n    on disk when a function with the same name exists.\n\n    Options:\n      -p        use a default value for PATH that is guaranteed to find all of\n        the standard utilities\n      -v        print a description of COMMAND similar to the `type' builtin\n      -V        print a more verbose description of each COMMAND\n\n    Exit Status:\n    Returns exit status of COMMAND, or failure if COMMAND is not found.\n```",
    "created_at": "2012-03-14T21:02:57Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128243",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:17'></a>
Okay, found it.  "sage" refers to the shell function

```
sage() {
    sage_setup
    sage-ipython "$@" -i
}
```
We should probably rename this function to `run_sage` or so...

In the mean time, replace `$SAGE_ROOT/sage "$@"` by

```
command sage "$@"
```

Help of command:

```
command: command [-pVv] command [arg ...]
    Execute a simple command or display information about commands.

    Runs COMMAND with ARGS suppressing  shell function lookup, or display
    information about the specified COMMANDs.  Can be used to invoke commands
    on disk when a function with the same name exists.

    Options:
      -p        use a default value for PATH that is guaranteed to find all of
        the standard utilities
      -v        print a description of COMMAND similar to the `type' builtin
      -V        print a more verbose description of each COMMAND

    Exit Status:
    Returns exit status of COMMAND, or failure if COMMAND is not found.
```



---

archive/issue_events_096180.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-03-14T21:13:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11932#event-96180"
}
```



---

archive/issue_events_096181.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-03-14T21:13:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11932#event-96181"
}
```



---

archive/issue_comments_128244.json:
```json
{
    "body": "<a id='comment:18'></a>\nAh, good, thank you. I agree, having a function named \"sage\" is probably a bad idea, but changing it should be dealt with elsewhere. (It would require a number of changes in the `sage` script, and a correspondingly large number of tests to make sure everything works.)",
    "created_at": "2012-03-14T21:13:52Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128244",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:18'></a>
Ah, good, thank you. I agree, having a function named "sage" is probably a bad idea, but changing it should be dealt with elsewhere. (It would require a number of changes in the `sage` script, and a correspondingly large number of tests to make sure everything works.)



---

archive/issue_comments_128245.json:
```json
{
    "body": "<a id='comment:19'></a>\nOn an entirely different topic: I don't like the `--norc` option name.  In the light of #12647, I would expect that option to suppress the reading of the `sagerc` script (and/or the `init.sage` script).  I would only support `--nodotsage`.",
    "created_at": "2012-03-14T21:41:24Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128245",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:19'></a>
On an entirely different topic: I don't like the `--norc` option name.  In the light of #12647, I would expect that option to suppress the reading of the `sagerc` script (and/or the `init.sage` script).  I would only support `--nodotsage`.



---

archive/issue_comments_128246.json:
```json
{
    "body": "<a id='comment:20'></a>\nHere are new patches which just use `--nodotsage`.  (I also considered `--tmpdotsage`, but it doesn't matter too much, as long as it's documented.)",
    "created_at": "2012-03-19T17:46:16Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128246",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:20'></a>
Here are new patches which just use `--nodotsage`.  (I also considered `--tmpdotsage`, but it doesn't matter too much, as long as it's documented.)



---

archive/attachments_015878.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_11932-scripts.patch",
    "asset_url": "tarball://root/attachments/ticket11932/trac_11932-scripts.patch",
    "created_at": "2012-03-19T17:47:28Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket11932/trac_11932-scripts.patch",
    "user": "https://github.com/jhpalmieri"
}
```



---

archive/issue_comments_128247.json:
```json
{
    "body": "Attachment [trac_11932-scripts.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-scripts.patch) by @jhpalmieri created at 2012-03-19 17:47:28\n\nscripts repo",
    "created_at": "2012-03-19T17:47:28Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128247",
    "user": "https://github.com/jhpalmieri"
}
```

Attachment [trac_11932-scripts.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-scripts.patch) by @jhpalmieri created at 2012-03-19 17:47:28

scripts repo



---

archive/issue_comments_128248.json:
```json
{
    "body": "documentation patch (Sage library)",
    "created_at": "2012-03-19T17:49:22Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128248",
    "user": "https://github.com/jhpalmieri"
}
```

documentation patch (Sage library)



---

archive/attachments_015879.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_11932-doc.patch",
    "asset_url": "tarball://root/attachments/ticket11932/trac_11932-doc.patch",
    "created_at": "2012-03-19T18:11:16Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket11932/trac_11932-doc.patch",
    "user": "https://github.com/nexttime"
}
```



---

archive/issue_comments_128249.json:
```json
{
    "body": "<a id='comment:21'></a>\nAttachment [trac_11932-doc.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-doc.patch) by @nexttime created at 2012-03-19 18:11:16\n\nIt would be nice to also have `--nosagerc`, and let `--nodotsage` by default *not* bypass `[$DOT_SAGE/]sagerc` (if it exists; of course no directory should be created if it doesn't).",
    "created_at": "2012-03-19T18:11:16Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128249",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:21'></a>
Attachment [trac_11932-doc.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-doc.patch) by @nexttime created at 2012-03-19 18:11:16

It would be nice to also have `--nosagerc`, and let `--nodotsage` by default *not* bypass `[$DOT_SAGE/]sagerc` (if it exists; of course no directory should be created if it doesn't).



---

archive/issue_comments_128250.json:
```json
{
    "body": "<a id='comment:22'></a>\nPerhaps on another ticket...",
    "created_at": "2012-03-19T18:35:15Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128250",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:22'></a>
Perhaps on another ticket...



---

archive/issue_comments_128251.json:
```json
{
    "body": "<a id='comment:23'></a>\nUnfortunately, this doesn't quite work properly since `DOT_SAGE` is changed *after* reading `sage-env`.  The following variables still refer to `$HOME/.sage`:\n\n```\njdemeyer@arcanis:/usr/local/src/sage-5.0.beta13$ ./sage --nodotsage --sh -c env | fgrep \"$HOME/.sage\"\nMPLCONFIGDIR=/home/jdemeyer/.sage//matplotlib-1.1.0\nSAGE_STARTUP_FILE=/home/jdemeyer/.sage//init.sage\nSAGE_TESTDIR=/home/jdemeyer/.sage//tmp\n```\n\nI suggest moving the `--nodotsage` check before the sourcing of `sage-env`.",
    "created_at": "2012-04-09T19:39:09Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128251",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:23'></a>
Unfortunately, this doesn't quite work properly since `DOT_SAGE` is changed *after* reading `sage-env`.  The following variables still refer to `$HOME/.sage`:

```
jdemeyer@arcanis:/usr/local/src/sage-5.0.beta13$ ./sage --nodotsage --sh -c env | fgrep "$HOME/.sage"
MPLCONFIGDIR=/home/jdemeyer/.sage//matplotlib-1.1.0
SAGE_STARTUP_FILE=/home/jdemeyer/.sage//init.sage
SAGE_TESTDIR=/home/jdemeyer/.sage//tmp
```

I suggest moving the `--nodotsage` check before the sourcing of `sage-env`.



---

archive/issue_events_096182.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-09T19:39:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11932#event-96182"
}
```



---

archive/issue_events_096183.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-09T19:39:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11932#event-96183"
}
```



---

archive/issue_comments_128252.json:
```json
{
    "body": "<a id='comment:25'></a>\nOkay, here's a new root patch.",
    "created_at": "2012-04-13T20:21:46Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128252",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:25'></a>
Okay, here's a new root patch.



---

archive/issue_events_096184.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-04-13T20:21:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11932#event-96184"
}
```



---

archive/issue_events_096185.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-04-13T20:21:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11932#event-96185"
}
```



---

archive/issue_comments_128253.json:
```json
{
    "body": "<a id='comment:26'></a>\nJust one more issue: now that you don't source `sage-env`, the `command sage` looks for `sage` in the **system** `PATH`.  I suggest to replace\n\n```\ncommand sage \"$@\"\n```\nby\n\n```\ncommand \"$0\" \"$@\"\n```\nIf you make this change, you can give this ticket positive review.\n\nFurthermore, I would like to point out that `$HOME/.sage/matplotlib-1.1.0` is still created by `sage-env` when building Sage.  But that shouldn't stop this ticket.",
    "created_at": "2012-04-26T08:09:46Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128253",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:26'></a>
Just one more issue: now that you don't source `sage-env`, the `command sage` looks for `sage` in the **system** `PATH`.  I suggest to replace

```
command sage "$@"
```
by

```
command "$0" "$@"
```
If you make this change, you can give this ticket positive review.

Furthermore, I would like to point out that `$HOME/.sage/matplotlib-1.1.0` is still created by `sage-env` when building Sage.  But that shouldn't stop this ticket.



---

archive/issue_comments_128254.json:
```json
{
    "body": "root repo",
    "created_at": "2012-04-30T17:08:05Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128254",
    "user": "https://github.com/jhpalmieri"
}
```

root repo



---

archive/attachments_015880.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_11932-root.patch",
    "asset_url": "tarball://root/attachments/ticket11932/trac_11932-root.patch",
    "created_at": "2012-04-30T17:08:28Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket11932/trac_11932-root.patch",
    "user": "https://github.com/jhpalmieri"
}
```



---

archive/issue_comments_128255.json:
```json
{
    "body": "Attachment [trac_11932-root.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-root.patch) by @jhpalmieri created at 2012-04-30 17:08:28",
    "created_at": "2012-04-30T17:08:28Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128255",
    "user": "https://github.com/jhpalmieri"
}
```

Attachment [trac_11932-root.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-root.patch) by @jhpalmieri created at 2012-04-30 17:08:28



---

archive/issue_comments_128256.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,6 +1,16 @@\n-Apply \n+Version which ignores matplotlib issue: apply \n \n - [attachment:trac_11932-root.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-root.patch) to the Sage root repository\n - [attachment:trac_11932-scripts.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-scripts.patch) to the local/bin repository\n - [attachment:trac_11932-doc.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-doc.patch) to the Sage repository\n \n+---\n+\n+Version which deals with matplotlib issue: apply \n+\n+- [attachment:trac_11932-root.v2.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-root.v2.patch) to the Sage root repository\n+- [attachment:trac_11932-scripts.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-scripts.patch) to the local/bin repository\n+- [attachment:trac_11932-doc.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-doc.patch) to the Sage repository\n+- [attachment:trac_11932-sage.v2.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-sage.v2.patch) to the Sage repository\n+\n+\n``````\n",
    "created_at": "2012-04-30T17:13:51Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128256",
    "user": "https://github.com/jhpalmieri"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,6 +1,16 @@
-Apply 
+Version which ignores matplotlib issue: apply 
 
 - [attachment:trac_11932-root.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-root.patch) to the Sage root repository
 - [attachment:trac_11932-scripts.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-scripts.patch) to the local/bin repository
 - [attachment:trac_11932-doc.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-doc.patch) to the Sage repository
 
+---
+
+Version which deals with matplotlib issue: apply 
+
+- [attachment:trac_11932-root.v2.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-root.v2.patch) to the Sage root repository
+- [attachment:trac_11932-scripts.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-scripts.patch) to the local/bin repository
+- [attachment:trac_11932-doc.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-doc.patch) to the Sage repository
+- [attachment:trac_11932-sage.v2.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-sage.v2.patch) to the Sage repository
+
+
``````




---

archive/attachments_015881.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_11932-root.v2.patch",
    "asset_url": "tarball://root/attachments/ticket11932/trac_11932-root.v2.patch",
    "created_at": "2012-04-30T17:13:51Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket11932/trac_11932-root.v2.patch",
    "user": "https://github.com/jhpalmieri"
}
```



---

archive/issue_comments_128257.json:
```json
{
    "body": "<a id='comment:27'></a>\nAttachment [trac_11932-root.v2.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-root.v2.patch) by @jhpalmieri created at 2012-04-30 17:13:51\n\nHere are two versions. If we want the easy root, the first version just makes the change\n\n```diff\ndiff --git a/spkg/bin/sage b/spkg/bin/sage\n--- a/spkg/bin/sage\n+++ b/spkg/bin/sage\n@@ -238,7 +238,7 @@ cd \"$SAGE_ROOT\"\n if [ \"$1\" = '--nodotsage' ]; then\n     export DOT_SAGE=`mktemp -d ${TMPDIR:-/tmp}/dotsageXXXXXX`\n     shift\n-    command sage \"$@\"\n+    command \"$0\" \"$@\"\n     status=$?\n     rm -rf \"$DOT_SAGE\"\n     exit $status\n```\nThe second version deals (I think) with the matplotlib issue, creating the config directory in sage.misc.misc, where other subdirectories of `DOT_SAGE` are created. It's not a very significant difference: the file [attachment:trac_11932-sage.v2.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-sage.v2.patch) and these changes to the root patch:\n\n```diff\ndiff --git a/spkg/bin/sage-env b/spkg/bin/sage-env\n--- a/spkg/bin/sage-env\n+++ b/spkg/bin/sage-env\n@@ -281,8 +281,9 @@ fi\n \n if [ \"$DOT_SAGE\" = \"\" ]; then\n     # It is *not* an error if this directory does not exist, it will\n-    # be created in spkg/bin/sage.  This also works if $HOME/.sage is a\n-    # symbolic link to a non-existing directory.\n+    # be created in spkg/bin/sage or devel/sage/sage/misc/misc.py.\n+    # This also works if $HOME/.sage is a symbolic link to a\n+    # non-existing directory.\n     DOT_SAGE=`resolvelinks \"$HOME/.sage\"`\n \n     # In theory, DOT_SAGE is not required to have a trailing slash.\n@@ -340,8 +341,7 @@ if [ -f \"$MPLINITFILE\" ]; then\n         MPLCONFIGDIR=\"$DOT_SAGE/matplotlib\"\n     fi\n     export MPLCONFIGDIR\n-    # Create the directory, because matplotlib doesn't seem to be happy otherwi\n-    mkdir -p \"$MPLCONFIGDIR\"\n+    # The directory is created when Sage starts (see sage.misc.misc).\n fi\n \n # Add some directories to $LD_LIBRARY_PATH:\n```\nIf you have time to look at the second version, please consider it. If you would rather deal with just the first version here, we can move the other version to a new ticket.",
    "created_at": "2012-04-30T17:13:51Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128257",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:27'></a>
Attachment [trac_11932-root.v2.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-root.v2.patch) by @jhpalmieri created at 2012-04-30 17:13:51

Here are two versions. If we want the easy root, the first version just makes the change

```diff
diff --git a/spkg/bin/sage b/spkg/bin/sage
--- a/spkg/bin/sage
+++ b/spkg/bin/sage
@@ -238,7 +238,7 @@ cd "$SAGE_ROOT"
 if [ "$1" = '--nodotsage' ]; then
     export DOT_SAGE=`mktemp -d ${TMPDIR:-/tmp}/dotsageXXXXXX`
     shift
-    command sage "$@"
+    command "$0" "$@"
     status=$?
     rm -rf "$DOT_SAGE"
     exit $status
```
The second version deals (I think) with the matplotlib issue, creating the config directory in sage.misc.misc, where other subdirectories of `DOT_SAGE` are created. It's not a very significant difference: the file [attachment:trac_11932-sage.v2.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-sage.v2.patch) and these changes to the root patch:

```diff
diff --git a/spkg/bin/sage-env b/spkg/bin/sage-env
--- a/spkg/bin/sage-env
+++ b/spkg/bin/sage-env
@@ -281,8 +281,9 @@ fi
 
 if [ "$DOT_SAGE" = "" ]; then
     # It is *not* an error if this directory does not exist, it will
-    # be created in spkg/bin/sage.  This also works if $HOME/.sage is a
-    # symbolic link to a non-existing directory.
+    # be created in spkg/bin/sage or devel/sage/sage/misc/misc.py.
+    # This also works if $HOME/.sage is a symbolic link to a
+    # non-existing directory.
     DOT_SAGE=`resolvelinks "$HOME/.sage"`
 
     # In theory, DOT_SAGE is not required to have a trailing slash.
@@ -340,8 +341,7 @@ if [ -f "$MPLINITFILE" ]; then
         MPLCONFIGDIR="$DOT_SAGE/matplotlib"
     fi
     export MPLCONFIGDIR
-    # Create the directory, because matplotlib doesn't seem to be happy otherwi
-    mkdir -p "$MPLCONFIGDIR"
+    # The directory is created when Sage starts (see sage.misc.misc).
 fi
 
 # Add some directories to $LD_LIBRARY_PATH:
```
If you have time to look at the second version, please consider it. If you would rather deal with just the first version here, we can move the other version to a new ticket.



---

archive/issue_comments_128258.json:
```json
{
    "body": "<a id='comment:28'></a>\nFor the Sage patch, what about:\n\n```\ntry:\n    # Create the matplotlib config directory\n    sage_makedirs(os.environ[\"MPLCONFIGDIR\"])\nexcept KeyError:\n    pass\n```\nI don't see a reason to create a directory when the environment variable doesn't exist.",
    "created_at": "2012-05-07T13:57:58Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128258",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:28'></a>
For the Sage patch, what about:

```
try:
    # Create the matplotlib config directory
    sage_makedirs(os.environ["MPLCONFIGDIR"])
except KeyError:
    pass
```
I don't see a reason to create a directory when the environment variable doesn't exist.



---

archive/issue_events_096186.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-07T13:57:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "milestone": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11932#event-96186"
}
```



---

archive/issue_events_096187.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-07T13:57:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "milestone": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11932#event-96187"
}
```



---

archive/issue_comments_128259.json:
```json
{
    "body": "<a id='comment:29'></a>\nSince we set `MPLCONFIGDIR` in sage-env, there is no reason for it not to be set (unless someone does something risky and unsupported like try to import the Sage library using the system's Python). But I made the change anyway.",
    "created_at": "2012-05-07T17:11:32Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128259",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:29'></a>
Since we set `MPLCONFIGDIR` in sage-env, there is no reason for it not to be set (unless someone does something risky and unsupported like try to import the Sage library using the system's Python). But I made the change anyway.



---

archive/attachments_015882.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_11932-sage.v2.patch",
    "asset_url": "tarball://root/attachments/ticket11932/trac_11932-sage.v2.patch",
    "created_at": "2012-05-08T10:24:02Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket11932/trac_11932-sage.v2.patch",
    "user": "https://github.com/jdemeyer"
}
```



---

archive/issue_comments_128260.json:
```json
{
    "body": "<a id='comment:30'></a>\nAttachment [trac_11932-sage.v2.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-sage.v2.patch) by @jdemeyer created at 2012-05-08 10:24:02\n\nLooks good and works well.\n\nI'm currently running\n\n```\nsage --nodotsage -tp 2 devel/sage/sage\n```",
    "created_at": "2012-05-08T10:24:02Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128260",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:30'></a>
Attachment [trac_11932-sage.v2.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-sage.v2.patch) by @jdemeyer created at 2012-05-08 10:24:02

Looks good and works well.

I'm currently running

```
sage --nodotsage -tp 2 devel/sage/sage
```



---

archive/issue_comments_128261.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,11 +1,3 @@\n-Version which ignores matplotlib issue: apply \n-\n-- [attachment:trac_11932-root.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-root.patch) to the Sage root repository\n-- [attachment:trac_11932-scripts.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-scripts.patch) to the local/bin repository\n-- [attachment:trac_11932-doc.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-doc.patch) to the Sage repository\n-\n----\n-\n Version which deals with matplotlib issue: apply \n \n - [attachment:trac_11932-root.v2.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-root.v2.patch) to the Sage root repository\n``````\n",
    "created_at": "2012-05-08T10:24:02Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128261",
    "user": "https://github.com/jdemeyer"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,11 +1,3 @@
-Version which ignores matplotlib issue: apply 
-
-- [attachment:trac_11932-root.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-root.patch) to the Sage root repository
-- [attachment:trac_11932-scripts.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-scripts.patch) to the local/bin repository
-- [attachment:trac_11932-doc.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-doc.patch) to the Sage repository
-
----
-
 Version which deals with matplotlib issue: apply 
 
 - [attachment:trac_11932-root.v2.patch](https://github.com/sagemath/sage/files/ticket11932/trac_11932-root.v2.patch) to the Sage root repository
``````




---

archive/issue_events_096188.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-08T11:28:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11932#event-96188"
}
```



---

archive/issue_events_096189.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-08T11:28:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11932#event-96189"
}
```



---

archive/issue_comments_128262.json:
```json
{
    "body": "<a id='comment:31'></a>\n\n```\nAll tests passed!\n```",
    "created_at": "2012-05-08T11:28:53Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128262",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:31'></a>

```
All tests passed!
```



---

archive/issue_comments_128263.json:
```json
{
    "body": "**Merged:** sage-5.1.beta0",
    "created_at": "2012-05-13T21:32:43Z",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11932#issuecomment-128263",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-5.1.beta0



---

archive/issue_events_096190.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-13T21:32:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11932#event-96190"
}
```



---

archive/issue_events_096191.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-13T21:32:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11932",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11932#event-96191"
}
```
