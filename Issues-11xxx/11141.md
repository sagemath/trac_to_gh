# Issue 11141: update SAGE_ROOT/local/bin/.hgignore

archive/issues_010998.json:
```json
{
    "body": "The new PolyBoRi spkg (#10797) and the new cython spkg (#10493) add the files `PolyGUI` and `cygdb` to SAGE_ROOT/local/bin/.  The attached patch adds those files to the .hgignore file.\n\nI'm marking this as a blocker because we should distribute a clean repo when we release Sage, but it should be pretty easy to review...\n\n\nAssignee: drkirkby\n\nResolution: fixed\n\nAuthor: John Palmieri\n\nReviewer: David Kirkby\n\nMerged: sage-4.7.alpha5\n\nIssue created by migration from https://trac.sagemath.org/ticket/11141\n\n",
    "closed_at": "2011-04-11T19:16:26Z",
    "created_at": "2011-04-06T20:17:28Z",
    "labels": [
        "component: scripts",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.7",
    "title": "update SAGE_ROOT/local/bin/.hgignore",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/11141",
    "user": "https://github.com/jhpalmieri"
}
```
The new PolyBoRi spkg (#10797) and the new cython spkg (#10493) add the files `PolyGUI` and `cygdb` to SAGE_ROOT/local/bin/.  The attached patch adds those files to the .hgignore file.

I'm marking this as a blocker because we should distribute a clean repo when we release Sage, but it should be pretty easy to review...


Assignee: drkirkby

Resolution: fixed

Author: John Palmieri

Reviewer: David Kirkby

Merged: sage-4.7.alpha5

Issue created by migration from https://trac.sagemath.org/ticket/11141





---

archive/issue_comments_145034.json:
```json
{
    "body": "Attachment [trac_11141.patch](tarball://root/attachments/some-uuid/ticket11141/trac_11141.patch) by @jhpalmieri created at 2011-04-06 20:18:15\n\nscripts repo",
    "created_at": "2011-04-06T20:18:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11141#issuecomment-145034",
    "user": "https://github.com/jhpalmieri"
}
```

Attachment [trac_11141.patch](tarball://root/attachments/some-uuid/ticket11141/trac_11141.patch) by @jhpalmieri created at 2011-04-06 20:18:15

scripts repo



---

archive/issue_comments_145035.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2011-04-06T20:18:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11141#issuecomment-145035",
    "user": "https://github.com/jhpalmieri"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_145036.json:
```json
{
    "body": "Set assignee to drkirkby.",
    "created_at": "2011-04-10T20:51:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11141#issuecomment-145036",
    "user": "https://trac.sagemath.org/admin/accounts/users/drkirkby"
}
```

Set assignee to drkirkby.



---

archive/issue_comments_145037.json:
```json
{
    "body": "Changing reviewer from \"\" to \"David Kirkby\"",
    "created_at": "2011-04-10T20:51:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11141#issuecomment-145037",
    "user": "https://trac.sagemath.org/admin/accounts/users/drkirkby"
}
```

Changing reviewer from "" to "David Kirkby"



---

archive/issue_comments_145038.json:
```json
{
    "body": "<a id='comment:2'></a>Works fine. Before the patch:\n\n```\ndrkirkby@hawk:~/sage-4.7.alpha3/local/bin$ hg status\n? PolyGUI\n? cygdb\n```\n\nbut the patch corrects it.",
    "created_at": "2011-04-10T20:51:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11141#issuecomment-145038",
    "user": "https://trac.sagemath.org/admin/accounts/users/drkirkby"
}
```

<a id='comment:2'></a>Works fine. Before the patch:

```
drkirkby@hawk:~/sage-4.7.alpha3/local/bin$ hg status
? PolyGUI
? cygdb
```

but the patch corrects it.



---

archive/issue_comments_145039.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2011-04-10T22:04:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11141#issuecomment-145039",
    "user": "https://trac.sagemath.org/admin/accounts/users/drkirkby"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_145040.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2011-04-11T19:16:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11141#issuecomment-145040",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed



---

archive/issue_events_028885.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-04-11T19:16:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/11141",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11141#event-28885"
}
```



---

archive/issue_comments_145041.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-4.7.alpha5\"",
    "created_at": "2011-04-11T19:16:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11141#issuecomment-145041",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-4.7.alpha5"
