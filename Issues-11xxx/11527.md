# Issue 11527: Add revolution_plot3d to reference manual

archive/issues_011355.json:
```json
{
    "assignees": [],
    "body": "Somehow, it's not in there!  \n\nApply\n\n- [attachment: trac_11527.patch.gz](https://github.com/sagemath/sage/files/ticket11527/trac_11527.patch.gz)\n\n\nAssignee: **mvngu**\n\nCC:  mvngu @jasongrout ryan\n\nKeywords: **graphics, revolution plot3d 3d plot beginner**\n\nAuthor: **Karl-Dieter Crisman**\n\nReviewer: **John Palmieri, Ivo Hedtke**\n\nMerged: **sage-4.7.2.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11527_\n\n",
    "closed_at": "2011-07-27T13:41:07Z",
    "created_at": "2011-06-21T16:40:40Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add revolution_plot3d to reference manual",
    "type": "issue",
    "updated_at": "2011-07-27T13:41:07Z",
    "url": "https://github.com/sagemath/sage/issues/11527",
    "user": "https://github.com/kcrisman"
}
```
Somehow, it's not in there!  

Apply

- [attachment: trac_11527.patch.gz](https://github.com/sagemath/sage/files/ticket11527/trac_11527.patch.gz)


Assignee: **mvngu**

CC:  mvngu @jasongrout ryan

Keywords: **graphics, revolution plot3d 3d plot beginner**

Author: **Karl-Dieter Crisman**

Reviewer: **John Palmieri, Ivo Hedtke**

Merged: **sage-4.7.2.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/11527_





---

archive/issue_events_140524.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-06-21T16:40:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11527",
    "milestone_number": null,
    "milestone_title": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11527#event-140524"
}
```



---

archive/issue_events_140525.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-06-21T16:40:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11527",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11527#event-140525"
}
```



---

archive/issue_events_140526.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-06-21T16:40:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11527",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11527#event-140526"
}
```



---

archive/issue_events_140527.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-06-21T16:42:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11527",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11527#event-140527"
}
```



---

archive/issue_comments_119544.json:
```json
{
    "body": "Attachment: **[trac_11527-revolution.patch.gz](https://github.com/sagemath/sage/files/ticket11527/trac_11527-revolution.patch.gz)**",
    "created_at": "2011-06-24T02:52:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11527",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11527#issuecomment-119544",
    "user": "https://github.com/kcrisman"
}
```

Attachment: **[trac_11527-revolution.patch.gz](https://github.com/sagemath/sage/files/ticket11527/trac_11527-revolution.patch.gz)**



---

archive/issue_events_140528.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-06-24T02:53:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11527",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11527#event-140528"
}
```



---

archive/issue_comments_119545.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nTo test this, use `./sage -docbuild reference html` and make sure all looks right in the 3d plotting area.  Let me know if there is a problem with the formatting.",
    "created_at": "2011-06-24T02:53:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11527",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11527#issuecomment-119545",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:2'>Comment 2:</a>
To test this, use `./sage -docbuild reference html` and make sure all looks right in the 3d plotting area.  Let me know if there is a problem with the formatting.



---

archive/issue_comments_119546.json:
```json
{
    "body": "Author: **Karl-Dieter Crisman**",
    "created_at": "2011-06-24T02:53:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11527",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11527#issuecomment-119546",
    "user": "https://github.com/kcrisman"
}
```

Author: **Karl-Dieter Crisman**



---

archive/issue_comments_119547.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nLooks okay except that the revolution_plot3d page has a missing title, because the py file doesn't have a docstring at the top.  I'm adding this in a new patch, along with a \"delta\" patch so you can see the difference.  If you're happy with my changes, please switch to \"positive review\".",
    "created_at": "2011-07-17T03:56:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11527",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11527#issuecomment-119547",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:3'>Comment 3:</a>
Looks okay except that the revolution_plot3d page has a missing title, because the py file doesn't have a docstring at the top.  I'm adding this in a new patch, along with a "delta" patch so you can see the difference.  If you're happy with my changes, please switch to "positive review".



---

archive/issue_comments_119548.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,6 @@\n Somehow, it's not in there!  \n+\n+Apply\n+\n+- [attachment: trac_11527.patch.gz](https://github.com/sagemath/sage/files/ticket11527/trac_11527.patch.gz)\n+\n``````\n",
    "created_at": "2011-07-17T03:56:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11527",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11527#issuecomment-119548",
    "user": "https://github.com/jhpalmieri"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,6 @@
 Somehow, it's not in there!  
+
+Apply
+
+- [attachment: trac_11527.patch.gz](https://github.com/sagemath/sage/files/ticket11527/trac_11527.patch.gz)
+
``````




---

archive/issue_comments_119549.json:
```json
{
    "body": "Reviewer: **John Palmieri**",
    "created_at": "2011-07-17T03:56:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11527",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11527#issuecomment-119549",
    "user": "https://github.com/jhpalmieri"
}
```

Reviewer: **John Palmieri**



---

archive/issue_comments_119550.json:
```json
{
    "body": "for reference only; do not apply",
    "created_at": "2011-07-17T03:57:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11527",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11527#issuecomment-119550",
    "user": "https://github.com/jhpalmieri"
}
```

for reference only; do not apply



---

archive/issue_comments_119551.json:
```json
{
    "body": "Attachment: **[trac_11527-delta.patch.gz](https://github.com/sagemath/sage/files/ticket11527/trac_11527-delta.patch.gz)**\n\nAttachment: **[trac_11527.patch.gz](https://github.com/sagemath/sage/files/ticket11527/trac_11527.patch.gz)**\n\napply only this patch",
    "created_at": "2011-07-17T03:57:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11527",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11527#issuecomment-119551",
    "user": "https://github.com/jhpalmieri"
}
```

Attachment: **[trac_11527-delta.patch.gz](https://github.com/sagemath/sage/files/ticket11527/trac_11527-delta.patch.gz)**

Attachment: **[trac_11527.patch.gz](https://github.com/sagemath/sage/files/ticket11527/trac_11527.patch.gz)**

apply only this patch



---

archive/issue_comments_119552.json:
```json
{
    "body": "Changed reviewer from **John Palmieri** to **John Palmieri, Ivo Hedtke**",
    "created_at": "2011-07-23T20:40:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11527",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11527#issuecomment-119552",
    "user": "https://github.com/hedtke"
}
```

Changed reviewer from **John Palmieri** to **John Palmieri, Ivo Hedtke**



---

archive/issue_comments_119553.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\npatch works. html doc compiles and looks good. same for pdf doc.",
    "created_at": "2011-07-23T20:40:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11527",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11527#issuecomment-119553",
    "user": "https://github.com/hedtke"
}
```

<a id='comment:4'>Comment 4:</a>
patch works. html doc compiles and looks good. same for pdf doc.



---

archive/issue_events_140529.json:
```json
{
    "actor": "https://github.com/hedtke",
    "created_at": "2011-07-23T20:40:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11527",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11527#event-140529"
}
```



---

archive/issue_events_140530.json:
```json
{
    "actor": "https://github.com/hedtke",
    "created_at": "2011-07-23T20:40:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11527",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11527#event-140530"
}
```



---

archive/issue_events_140531.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-24T11:19:27Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11527",
    "milestone_number": null,
    "milestone_title": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11527#event-140531"
}
```



---

archive/issue_events_140532.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-24T11:19:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11527",
    "milestone_number": null,
    "milestone_title": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11527#event-140532"
}
```



---

archive/issue_events_140533.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-27T13:41:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11527",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11527#event-140533"
}
```



---

archive/issue_events_140534.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-07-27T13:41:07Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11527",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11527#event-140534"
}
```



---

archive/issue_comments_119554.json:
```json
{
    "body": "Merged: **sage-4.7.2.alpha0**",
    "created_at": "2011-07-27T13:41:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11527",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11527#issuecomment-119554",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.7.2.alpha0**
