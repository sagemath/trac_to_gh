# Issue 11801: Ill-formed documentation of HilbertClassPolynomialDatabase

archive/issues_011629.json:
```json
{
    "assignees": [],
    "body": "there is a formatting issue:\n\n```\nsage: HilbertClassPolynomialDatabase?\n...\n       EXAMPLES:\n          sage: db = HilbertClassPolynomialDatabase() sage: db[-4]\n          # optional Kohel database required x - 1728 sage: db[-7]\n          # optional x + 3375 sage: f = db[-23]; f         # optional x^3\n          + 3491750*x^2 - 5151296875*x + 12771880859375 sage:\n          f.discriminant().factor()    # optional -1 * 5^18 * 7^12 * 11^4\n          * 17^2 * 19^2 * 23 sage: db[-23]                      # optional\n          x^3 + 3491750*x^2 - 5151296875*x + 12771880859375\n```\n\n---\n\nApply [attachment: trac_11801.patch.gz](https://github.com/sagemath/sage/files/ticket11801/trac_11801.patch.gz) to the Sage library.\n\n\nAssignee: **mvngu**\n\nKeywords: **ecc2011**\n\nAuthor: **Jean-Pierre Flori**\n\nReviewer: **Paul Zimmermann**\n\nMerged: **sage-4.7.2.alpha3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11801_\n\n",
    "closed_at": "2011-09-17T18:07:35Z",
    "created_at": "2011-09-15T14:37:08Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Ill-formed documentation of HilbertClassPolynomialDatabase",
    "type": "issue",
    "updated_at": "2011-09-17T18:07:35Z",
    "url": "https://github.com/sagemath/sage/issues/11801",
    "user": "https://github.com/zimmermann6"
}
```
there is a formatting issue:

```
sage: HilbertClassPolynomialDatabase?
...
       EXAMPLES:
          sage: db = HilbertClassPolynomialDatabase() sage: db[-4]
          # optional Kohel database required x - 1728 sage: db[-7]
          # optional x + 3375 sage: f = db[-23]; f         # optional x^3
          + 3491750*x^2 - 5151296875*x + 12771880859375 sage:
          f.discriminant().factor()    # optional -1 * 5^18 * 7^12 * 11^4
          * 17^2 * 19^2 * 23 sage: db[-23]                      # optional
          x^3 + 3491750*x^2 - 5151296875*x + 12771880859375
```

---

Apply [attachment: trac_11801.patch.gz](https://github.com/sagemath/sage/files/ticket11801/trac_11801.patch.gz) to the Sage library.


Assignee: **mvngu**

Keywords: **ecc2011**

Author: **Jean-Pierre Flori**

Reviewer: **Paul Zimmermann**

Merged: **sage-4.7.2.alpha3**

_Issue created by migration from https://trac.sagemath.org/ticket/11801_





---

archive/issue_events_145034.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2011-09-15T14:37:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11801",
    "milestone_number": null,
    "milestone_title": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11801#event-145034"
}
```



---

archive/issue_events_145035.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2011-09-15T14:37:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11801",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11801#event-145035"
}
```



---

archive/issue_events_145036.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2011-09-15T14:37:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11801",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11801#event-145036"
}
```



---

archive/issue_events_145037.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2011-09-15T14:37:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11801",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11801#event-145037"
}
```



---

archive/issue_comments_125543.json:
```json
{
    "body": "Fix.",
    "created_at": "2011-09-16T09:15:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11801",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11801#issuecomment-125543",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Fix.



---

archive/issue_events_145038.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2011-09-16T09:16:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11801",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11801#event-145038"
}
```



---

archive/issue_comments_125544.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[trac_11801.patch.gz](https://github.com/sagemath/sage/files/ticket11801/trac_11801.patch.gz)**",
    "created_at": "2011-09-16T09:16:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11801",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11801#issuecomment-125544",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[trac_11801.patch.gz](https://github.com/sagemath/sage/files/ticket11801/trac_11801.patch.gz)**



---

archive/issue_comments_125545.json:
```json
{
    "body": "Author: **Jean-Pierre Flori**",
    "created_at": "2011-09-16T09:16:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11801",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11801#issuecomment-125545",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Author: **Jean-Pierre Flori**



---

archive/issue_events_145039.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2011-09-16T09:29:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11801",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11801#event-145039"
}
```



---

archive/issue_events_145040.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2011-09-16T09:29:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11801",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11801#event-145040"
}
```



---

archive/issue_comments_125546.json:
```json
{
    "body": "Reviewer: **Paul Zimmermann**",
    "created_at": "2011-09-16T09:29:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11801",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11801#issuecomment-125546",
    "user": "https://github.com/zimmermann6"
}
```

Reviewer: **Paul Zimmermann**



---

archive/issue_comments_125547.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\ngood work Jean-Pierre. I also checked the examples work with Kohel's database.\n\nPaul",
    "created_at": "2011-09-16T09:29:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11801",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11801#issuecomment-125547",
    "user": "https://github.com/zimmermann6"
}
```

<a id='comment:2'>Comment 2:</a>
good work Jean-Pierre. I also checked the examples work with Kohel's database.

Paul



---

archive/issue_comments_125548.json:
```json
{
    "body": "Changed keywords from **** to **ecc2011**",
    "created_at": "2011-09-16T12:34:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11801",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11801#issuecomment-125548",
    "user": "https://github.com/zimmermann6"
}
```

Changed keywords from **** to **ecc2011**



---

archive/issue_comments_125549.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -12,3 +12,8 @@\n           * 17^2 * 19^2 * 23 sage: db[-23]                      # optional\n           x^3 + 3491750*x^2 - 5151296875*x + 12771880859375\n ```\n+\n+---\n+\n+Apply [attachment: trac_11801.patch.gz](https://github.com/sagemath/sage/files/ticket11801/trac_11801.patch.gz) to the Sage library.\n+\n``````\n",
    "created_at": "2011-09-17T06:13:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11801",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11801#issuecomment-125549",
    "user": "https://github.com/nexttime"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -12,3 +12,8 @@
           * 17^2 * 19^2 * 23 sage: db[-23]                      # optional
           x^3 + 3491750*x^2 - 5151296875*x + 12771880859375
 ```
+
+---
+
+Apply [attachment: trac_11801.patch.gz](https://github.com/sagemath/sage/files/ticket11801/trac_11801.patch.gz) to the Sage library.
+
``````




---

archive/issue_events_145041.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2011-09-17T16:26:11Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/11801",
    "title_is": "Ill-formed documentation of HilbertClassPolynomialDatabase",
    "title_was": "ill-formed documentation of HilbertClassPolynomialDatabase",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11801#event-145041"
}
```



---

archive/issue_events_145042.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2011-09-17T18:07:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11801",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11801#event-145042"
}
```



---

archive/issue_events_145043.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2011-09-17T18:07:35Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11801",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11801#event-145043"
}
```



---

archive/issue_comments_125550.json:
```json
{
    "body": "Merged: **sage-4.7.2.alpha3**",
    "created_at": "2011-09-17T18:07:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11801",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11801#issuecomment-125550",
    "user": "https://github.com/nexttime"
}
```

Merged: **sage-4.7.2.alpha3**
