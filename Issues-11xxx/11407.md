# Issue 11407: Add normalization to clonable lists

archive/issues_011235.json:
```json
{
    "assignees": [],
    "body": "The patch adds a new support class in `list_clone.pyx` for lists with a normalization methods. The normalization method is called both after creation and clone/modification. This will be needed for rooted trees.\n\nI also take the opportunity of cleaning up the bad usage of `assert` in\nthe examples and replaced them by `if ... : raise ValueError`.\n\n**Apply**\n\n* [attachment: trac_11407-list_clone_improve-fh.patch.gz](https://github.com/sagemath/sage/files/ticket11407/trac_11407-list_clone_improve-fh.patch.gz)\n\nDepends on #11308\n\nAssignee: **@hivert**\n\nCC:  sage-combinat @tscrim\n\nKeywords: **clone normalization Cernay2012, days49**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Florent Hivert**\n\nMerged: **sage-5.12.beta2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11407_\n\n",
    "closed_at": "2013-08-16T21:17:22Z",
    "created_at": "2011-05-30T21:46:13Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.12",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add normalization to clonable lists",
    "type": "issue",
    "updated_at": "2013-08-16T21:17:22Z",
    "url": "https://github.com/sagemath/sage/issues/11407",
    "user": "https://github.com/hivert"
}
```
The patch adds a new support class in `list_clone.pyx` for lists with a normalization methods. The normalization method is called both after creation and clone/modification. This will be needed for rooted trees.

I also take the opportunity of cleaning up the bad usage of `assert` in
the examples and replaced them by `if ... : raise ValueError`.

**Apply**

* [attachment: trac_11407-list_clone_improve-fh.patch.gz](https://github.com/sagemath/sage/files/ticket11407/trac_11407-list_clone_improve-fh.patch.gz)

Depends on #11308

Assignee: **@hivert**

CC:  sage-combinat @tscrim

Keywords: **clone normalization Cernay2012, days49**

Reviewer: **Travis Scrimshaw**

Author: **Florent Hivert**

Merged: **sage-5.12.beta2**

_Issue created by migration from https://trac.sagemath.org/ticket/11407_





---

archive/issue_events_138554.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2011-05-30T21:46:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11407#event-138554"
}
```



---

archive/issue_events_138555.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2011-05-30T21:46:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11407#event-138555"
}
```



---

archive/issue_events_138556.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2011-05-30T21:46:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11407#event-138556"
}
```



---

archive/issue_events_138557.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2011-05-30T21:46:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11407#event-138557"
}
```



---

archive/issue_comments_117489.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n+Patch in preparation on sage-combinat queue\n \n+Depend on #11308\n``````\n",
    "created_at": "2011-05-30T21:48:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11407#issuecomment-117489",
    "user": "https://github.com/hivert"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
+Patch in preparation on sage-combinat queue
 
+Depend on #11308
``````




---

archive/issue_comments_117490.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,2 @@\n Patch in preparation on sage-combinat queue\n \n-Depend on #11308\n``````\n",
    "created_at": "2011-05-31T14:50:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11407#issuecomment-117490",
    "user": "https://github.com/hivert"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,2 @@
 Patch in preparation on sage-combinat queue
 
-Depend on #11308
``````




---

archive/issue_comments_117491.json:
```json
{
    "body": "Dependencies: **#11308**",
    "created_at": "2011-05-31T14:50:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11407#issuecomment-117491",
    "user": "https://github.com/hivert"
}
```

Dependencies: **#11308**



---

archive/issue_comments_117492.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,2 +1,4 @@\n-Patch in preparation on sage-combinat queue\n+The patch adds a new support class in `list_clone.pyx` for lists with a normalization methods. The normalization method is called both after creation and clone/modification. This will be needed for rooted trees.\n \n+**Apply** [attachment: trac_11407-list_clone_improve-fh.patch]\n+\n``````\n",
    "created_at": "2011-11-21T15:26:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11407#issuecomment-117492",
    "user": "https://github.com/hivert"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,2 +1,4 @@
-Patch in preparation on sage-combinat queue
+The patch adds a new support class in `list_clone.pyx` for lists with a normalization methods. The normalization method is called both after creation and clone/modification. This will be needed for rooted trees.
 
+**Apply** [attachment: trac_11407-list_clone_improve-fh.patch]
+
``````




---

archive/issue_events_138558.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2011-11-21T15:26:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11407#event-138558"
}
```



---

archive/issue_comments_117493.json:
```json
{
    "body": "Changed keywords from **clone normalization** to **clone normalization Cernay2012**",
    "created_at": "2012-02-06T14:54:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11407#issuecomment-117493",
    "user": "https://github.com/hivert"
}
```

Changed keywords from **clone normalization** to **clone normalization Cernay2012**



---

archive/issue_comments_117494.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nI just pushed a new patch which fixes the failing doctests",
    "created_at": "2012-03-22T18:29:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11407#issuecomment-117494",
    "user": "https://github.com/hivert"
}
```

<a id='comment:5'>Comment 5:</a>
I just pushed a new patch which fixes the failing doctests



---

archive/issue_events_138559.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-03-31T08:22:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11407#event-138559"
}
```



---

archive/issue_events_138560.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-03-31T08:22:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11407#event-138560"
}
```



---

archive/issue_comments_117495.json:
```json
{
    "body": "Work Issues: **rebase to current beta**",
    "created_at": "2012-03-31T08:22:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11407#issuecomment-117495",
    "user": "https://github.com/loefflerd"
}
```

Work Issues: **rebase to current beta**



---

archive/issue_comments_117496.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nPatch does not apply to 5.0.beta11, I'm afraid",
    "created_at": "2012-03-31T08:22:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11407#issuecomment-117496",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:6'>Comment 6:</a>
Patch does not apply to 5.0.beta11, I'm afraid



---

archive/issue_comments_117497.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nMessage for Florent:\n\nFor information, your patch does not commute with mine about enumeration modulo the action of a permgroup. I try to push back my patch in the queue but i didn't manage to do it without breaking the queue... Sorry for that...",
    "created_at": "2012-04-05T19:00:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11407#issuecomment-117497",
    "user": "https://github.com/sagetrac-nborie"
}
```

<a id='comment:7'>Comment 7:</a>
Message for Florent:

For information, your patch does not commute with mine about enumeration modulo the action of a permgroup. I try to push back my patch in the queue but i didn't manage to do it without breaking the queue... Sorry for that...



---

archive/issue_comments_117498.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nHello Florent,\n\nI rebased the patch on 5.8 but then sage fails to compile with the following messages.\n\n```\nsage/structure/list_clone.pyx:1826:10: Overriding final methods is not allowed\n\nsage/structure/list_clone.pyx:1826:10: Only final types can have final Python (def/cpdef) methods\n```\n\nIt seems that the problem is with the `__exit__` method.",
    "created_at": "2013-04-18T19:51:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11407#issuecomment-117498",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:8'>Comment 8:</a>
Hello Florent,

I rebased the patch on 5.8 but then sage fails to compile with the following messages.

```
sage/structure/list_clone.pyx:1826:10: Overriding final methods is not allowed

sage/structure/list_clone.pyx:1826:10: Only final types can have final Python (def/cpdef) methods
```

It seems that the problem is with the `__exit__` method.



---

archive/issue_comments_117499.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n The patch adds a new support class in `list_clone.pyx` for lists with a normalization methods. The normalization method is called both after creation and clone/modification. This will be needed for rooted trees.\n \n-**Apply** [attachment: trac_11407-list_clone_improve-fh.patch]\n+**Apply** [attachment: trac_11407-list_clone_improve-fh-rebased.patch.gz](https://github.com/sagemath/sage/files/ticket11407/trac_11407-list_clone_improve-fh-rebased.patch.gz)\n \n``````\n",
    "created_at": "2013-04-18T19:52:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11407#issuecomment-117499",
    "user": "https://github.com/fchapoton"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
 The patch adds a new support class in `list_clone.pyx` for lists with a normalization methods. The normalization method is called both after creation and clone/modification. This will be needed for rooted trees.
 
-**Apply** [attachment: trac_11407-list_clone_improve-fh.patch]
+**Apply** [attachment: trac_11407-list_clone_improve-fh-rebased.patch.gz](https://github.com/sagemath/sage/files/ticket11407/trac_11407-list_clone_improve-fh-rebased.patch.gz)
 
``````




---

archive/issue_comments_117500.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nAttachment: **[trac_11407-list_clone_improve-fh-rebased.patch.gz](https://github.com/sagemath/sage/files/ticket11407/trac_11407-list_clone_improve-fh-rebased.patch.gz)**",
    "created_at": "2013-04-18T19:52:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11407#issuecomment-117500",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:9'>Comment 9:</a>
Attachment: **[trac_11407-list_clone_improve-fh-rebased.patch.gz](https://github.com/sagemath/sage/files/ticket11407/trac_11407-list_clone_improve-fh-rebased.patch.gz)**



---

archive/issue_events_138561.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2013-06-21T13:32:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11407#event-138561"
}
```



---

archive/issue_events_138562.json:
```json
{
    "actor": "https://github.com/hivert",
    "created_at": "2013-06-21T13:32:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11407#event-138562"
}
```



---

archive/issue_comments_117501.json:
```json
{
    "body": "Changed keywords from **clone normalization Cernay2012** to **clone normalization Cernay2012, days49**",
    "created_at": "2013-06-21T13:32:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11407#issuecomment-117501",
    "user": "https://github.com/hivert"
}
```

Changed keywords from **clone normalization Cernay2012** to **clone normalization Cernay2012, days49**



---

archive/issue_comments_117502.json:
```json
{
    "body": "Changed work issues from **rebase to current beta** to none",
    "created_at": "2013-06-21T13:32:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11407#issuecomment-117502",
    "user": "https://github.com/hivert"
}
```

Changed work issues from **rebase to current beta** to none



---

archive/issue_comments_117503.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,7 @@\n The patch adds a new support class in `list_clone.pyx` for lists with a normalization methods. The normalization method is called both after creation and clone/modification. This will be needed for rooted trees.\n \n-**Apply** [attachment: trac_11407-list_clone_improve-fh-rebased.patch.gz](https://github.com/sagemath/sage/files/ticket11407/trac_11407-list_clone_improve-fh-rebased.patch.gz)\n+I also take the opportunity of cleaning up the bad usage of `assert` in\n+the examples and replaced them by `if ... : raise ValueError`.\n \n+**Apply** [attachment: trac_11407-list_clone_improve-fh.patch.gz](https://github.com/sagemath/sage/files/ticket11407/trac_11407-list_clone_improve-fh.patch.gz)\n+\n``````\n",
    "created_at": "2013-06-21T13:32:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11407#issuecomment-117503",
    "user": "https://github.com/hivert"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,7 @@
 The patch adds a new support class in `list_clone.pyx` for lists with a normalization methods. The normalization method is called both after creation and clone/modification. This will be needed for rooted trees.
 
-**Apply** [attachment: trac_11407-list_clone_improve-fh-rebased.patch.gz](https://github.com/sagemath/sage/files/ticket11407/trac_11407-list_clone_improve-fh-rebased.patch.gz)
+I also take the opportunity of cleaning up the bad usage of `assert` in
+the examples and replaced them by `if ... : raise ValueError`.
 
+**Apply** [attachment: trac_11407-list_clone_improve-fh.patch.gz](https://github.com/sagemath/sage/files/ticket11407/trac_11407-list_clone_improve-fh.patch.gz)
+
``````




---

archive/issue_comments_117504.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nfor the bot\n\napply trac_11407-list_clone_improve-fh.patch",
    "created_at": "2013-06-21T14:58:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11407#issuecomment-117504",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:11'>Comment 11:</a>
for the bot

apply trac_11407-list_clone_improve-fh.patch



---

archive/issue_comments_117505.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -3,5 +3,7 @@\n I also take the opportunity of cleaning up the bad usage of `assert` in\n the examples and replaced them by `if ... : raise ValueError`.\n \n-**Apply** [attachment: trac_11407-list_clone_improve-fh.patch.gz](https://github.com/sagemath/sage/files/ticket11407/trac_11407-list_clone_improve-fh.patch.gz)\n+**Apply**\n \n+* [attachment: trac_11407-list_clone_improve-fh.patch.gz](https://github.com/sagemath/sage/files/ticket11407/trac_11407-list_clone_improve-fh.patch.gz)\n+* [attachment: trac_11407-review-ts.patch.gz](https://github.com/sagemath/sage/files/ticket11407/trac_11407-review-ts.patch.gz)\n``````\n",
    "created_at": "2013-06-22T16:19:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11407#issuecomment-117505",
    "user": "https://github.com/tscrim"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -3,5 +3,7 @@
 I also take the opportunity of cleaning up the bad usage of `assert` in
 the examples and replaced them by `if ... : raise ValueError`.
 
-**Apply** [attachment: trac_11407-list_clone_improve-fh.patch.gz](https://github.com/sagemath/sage/files/ticket11407/trac_11407-list_clone_improve-fh.patch.gz)
+**Apply**
 
+* [attachment: trac_11407-list_clone_improve-fh.patch.gz](https://github.com/sagemath/sage/files/ticket11407/trac_11407-list_clone_improve-fh.patch.gz)
+* [attachment: trac_11407-review-ts.patch.gz](https://github.com/sagemath/sage/files/ticket11407/trac_11407-review-ts.patch.gz)
``````




---

archive/issue_comments_117506.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nAttachment: **[trac_11407-review-ts.patch.gz](https://github.com/sagemath/sage/files/ticket11407/trac_11407-review-ts.patch.gz)**\n\nHey Florent,\n\nLooks good minus the one missed change in the review patch I've uploaded. After you give it a quick check, and you set this to positive review.\n\nBest,\n\nTravis\n\nFor patchbot:\n\nApply: trac_11407-list_clone_improve-fh.patch trac_11407-review-ts.patch",
    "created_at": "2013-06-22T16:19:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11407#issuecomment-117506",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:12'>Comment 12:</a>
Attachment: **[trac_11407-review-ts.patch.gz](https://github.com/sagemath/sage/files/ticket11407/trac_11407-review-ts.patch.gz)**

Hey Florent,

Looks good minus the one missed change in the review patch I've uploaded. After you give it a quick check, and you set this to positive review.

Best,

Travis

For patchbot:

Apply: trac_11407-list_clone_improve-fh.patch trac_11407-review-ts.patch



---

archive/issue_comments_117507.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2013-06-22T16:19:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11407#issuecomment-117507",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_117508.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nAttachment: **[trac_11407-list_clone_improve-fh.patch.gz](https://github.com/sagemath/sage/files/ticket11407/trac_11407-list_clone_improve-fh.patch.gz)**",
    "created_at": "2013-06-28T13:19:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11407#issuecomment-117508",
    "user": "https://github.com/hivert"
}
```

<a id='comment:13'>Comment 13:</a>
Attachment: **[trac_11407-list_clone_improve-fh.patch.gz](https://github.com/sagemath/sage/files/ticket11407/trac_11407-list_clone_improve-fh.patch.gz)**



---

archive/issue_comments_117509.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -6,4 +6,3 @@\n **Apply**\n \n * [attachment: trac_11407-list_clone_improve-fh.patch.gz](https://github.com/sagemath/sage/files/ticket11407/trac_11407-list_clone_improve-fh.patch.gz)\n-* [attachment: trac_11407-review-ts.patch.gz](https://github.com/sagemath/sage/files/ticket11407/trac_11407-review-ts.patch.gz)\n``````\n",
    "created_at": "2013-06-28T13:19:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11407#issuecomment-117509",
    "user": "https://github.com/hivert"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -6,4 +6,3 @@
 **Apply**
 
 * [attachment: trac_11407-list_clone_improve-fh.patch.gz](https://github.com/sagemath/sage/files/ticket11407/trac_11407-list_clone_improve-fh.patch.gz)
-* [attachment: trac_11407-review-ts.patch.gz](https://github.com/sagemath/sage/files/ticket11407/trac_11407-review-ts.patch.gz)
``````




---

archive/issue_comments_117510.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nHi travis,\n\nI folded your patch and fixed the multiline doctests. I'm ok with you changes so please doublecheck and se positive review if everything is ok.\n\nFlorent",
    "created_at": "2013-06-28T13:20:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11407#issuecomment-117510",
    "user": "https://github.com/hivert"
}
```

<a id='comment:14'>Comment 14:</a>
Hi travis,

I folded your patch and fixed the multiline doctests. I'm ok with you changes so please doublecheck and se positive review if everything is ok.

Florent



---

archive/issue_events_138563.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-06-28T14:18:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11407#event-138563"
}
```



---

archive/issue_events_138564.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-06-28T14:18:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11407#event-138564"
}
```



---

archive/issue_comments_117511.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nLooks good to me. Thanks Florent.\n\nFor patchbot:\n\nApply: trac_11407-list_clone_improve-fh.patch",
    "created_at": "2013-06-28T14:18:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11407#issuecomment-117511",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:15'>Comment 15:</a>
Looks good to me. Thanks Florent.

For patchbot:

Apply: trac_11407-list_clone_improve-fh.patch



---

archive/issue_comments_117512.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nThanks for the review.\n\nCheers,\n\nFlorent",
    "created_at": "2013-06-28T22:01:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11407#issuecomment-117512",
    "user": "https://github.com/hivert"
}
```

<a id='comment:16'>Comment 16:</a>
Thanks for the review.

Cheers,

Florent



---

archive/issue_events_138565.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-07-21T21:50:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11407#event-138565"
}
```



---

archive/issue_events_138566.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-07-21T21:50:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11407#event-138566"
}
```



---

archive/issue_events_138567.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-16T21:17:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11407#event-138567"
}
```



---

archive/issue_events_138568.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-16T21:17:22Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11407#event-138568"
}
```



---

archive/issue_comments_117513.json:
```json
{
    "body": "Merged: **sage-5.12.beta2**",
    "created_at": "2013-08-16T21:17:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11407",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11407#issuecomment-117513",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.12.beta2**
