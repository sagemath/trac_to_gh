...
```json
{
    "body": "<a id='comment:1'></a>I've attached a screenshot of the traceback - the best I can do in Cygwin with my limited experience.",
...
<a id='comment:1'></a>I've attached a screenshot of the traceback - the best I can do in Cygwin with my limited experience.
...
<a id='comment:2'></a>Can you please attach your `sage/rings/real_mpfr.c`?
...
```json
{
    "body": "<a id='comment:6'></a>As another (possibly unrelated) data point, #6743 has two patches which change the behavior of sage/rings/complex_double.pyx to get Sage to start (well, a year or two ago).",
...
<a id='comment:6'></a>As another (possibly unrelated) data point, #6743 has two patches which change the behavior of sage/rings/complex_double.pyx to get Sage to start (well, a year or two ago).
...
<a id='comment:7'></a>I put in print statements at every conceivable place.  Here is as far as it gets:
...
<a id='comment:8'></a>Attachment [Screen shot 2011-06-29 at 9.19.14 PM.png](tarball://root/attachments/some-uuid/ticket11551/Screen shot 2011-06-29 at 9.19.14 PM.png) by @kcrisman created at 2011-06-30 01:37:09
...
```json
{
    "body": "<a id='comment:10'></a>Trying even `cgetg(4,t_REAL)` raises a similar error.  Pari seems to not be able to allocate anything - I don't know whether there is anything before this in initialization of Sage that has a problem.\n\n---\n\nAnother data point: `sage -gp` works fine.  Something in libpari might be off.  How might I test that without actually starting Sage?",
...
<a id='comment:10'></a>Trying even `cgetg(4,t_REAL)` raises a similar error.  Pari seems to not be able to allocate anything - I don't know whether there is anything before this in initialization of Sage that has a problem.
...
```json
{
    "body": "<a id='comment:11'></a>I have now confirmed this with the *released* 4.7.1.alpha3 on both XP and Win7.  It is very reproducible, always the same place.",
...
<a id='comment:11'></a>I have now confirmed this with the *released* 4.7.1.alpha3 on both XP and Win7.  It is very reproducible, always the same place.
...
<a id='comment:12'></a>Another update: commenting out everything about initializing the Pynac I doesn't help, because there is another place in initialization this is used:
...
<a id='comment:13'></a>Some things to try:
...
<a id='comment:14'></a>I tried 2. first.  Not very exciting.
...
```json
{
    "body": "<a id='comment:15'></a>I can't get 1. to install on Cygwin.  Seems like a linking order error or something, see #11130.",
...
<a id='comment:15'></a>I can't get 1. to install on Cygwin.  Seems like a linking order error or something, see #11130.
...
<a id='comment:17'></a>I have no idea why `real_mpfr`[`.pyx`] shouldn't initialize [the] PARI [library] (i.e., the `pari_instance` variable defined in `sage/libs/pari/gen.pyx`), but you (Karl-Dieter) could verify it gets initialized by putting some print statement(s) into `PariInstance`'s `__init__()`, preferably (also) around `pari_init_opts()`, to make sure the latter really gets called, because of
...
<a id='comment:22'></a>Attachment [Pari-2.5.0Segfault.png](tarball://root/attachments/some-uuid/ticket11551/Pari-2.5.0Segfault.png) by @kcrisman created at 2011-08-03 11:47:49
...
<a id='comment:23'></a>Okay, inserting appropriate print statements gives
...
<a id='comment:24'></a>I also put in a print statement for bot and added
...
<a id='comment:25'></a>Here is something else that Mike Hansen mentioned on [a sage-devel thread](http://groups.google.com/group/sage-devel/browse_thread/thread/3fb9d33037670185) I also linked to in[comment:4](#comment%3A4).
...
```json
{
    "body": "<a id='comment:28'></a>It's somewhat disturbing how much stuff happens so early in sage/all.py just because we import sage/misc/functional.py in there.   Pari is initialized and I get the integer.pyx `bot=0`, all within that.  \n\nMy bisecting skills are getting a lot of practice now...",
...
<a id='comment:28'></a>It's somewhat disturbing how much stuff happens so early in sage/all.py just because we import sage/misc/functional.py in there.   Pari is initialized and I get the integer.pyx `bot=0`, all within that.  
...
<a id='comment:29'></a>The magic all happens in 
...
<a id='comment:30'></a>Okay, here is what happens in a normal Sage build (OS X, Sage 4.7):
...
<a id='comment:32'></a>> You're "allowed" to print `bot` in some of the Cython files because they include `.../pari/decl.pxi` which declares it, e.g.:
...
<a id='comment:33'></a>Just curious, do you run Cygwin on a 32-bit Windows / machine?
...
```json
{
    "body": "<a id='comment:34'></a>Oh, just noticed it's not that bad as I thought, as PARI actually defines `pari_sp` to `ulong` rather than `long*`.\n\n(If it wouldn't, `top` and `avma` would point to `bot + `**`sizeof(long) *`** `size`, i.e. far beyond what gets allocated.)",
...
<a id='comment:34'></a>Oh, just noticed it's not that bad as I thought, as PARI actually defines `pari_sp` to `ulong` rather than `long*`.
...
<a id='comment:40'></a>As for the printing:
* after initializing the stack, `bot`, `top`, and `avma` are all 2121924616
* in integer.pyx, they are all **zero**.
So, I guess it's possible that `x > (0 - 0) /sizeof(long) = 0`.  That doesn't explain how these are corrupted/reset, though.  I think I will post to sage-devel asking someone exactly what is happening (that is, the sequence of files gone to) in that import of CDF.  It's happening somewhere in there.
...
<a id='comment:42'></a>> > As for the printing:
> > * after initializing the stack, `bot`, `top`, and `avma` are all 2121924616
...
<a id='comment:44'></a>Well, some random pointer might corrupt PARI's stack variables as well.
...
<a id='comment:45'></a>Just before the `0`s appear, the 'deep copy to Python heap' is called and avma goes down to ...600 instead of 616.  The others are the same.  
...
```json
{
    "body": "<a id='comment:46'></a>This has reappeared in the discussion to #12104.  Leif has a patch to get Cython files to say where they have problems importing somewhere, hopefully will be of use.",
...
<a id='comment:46'></a>This has reappeared in the discussion to #12104.  Leif has a patch to get Cython files to say where they have problems importing somewhere, hopefully will be of use.
...
<a id='comment:49'></a>Still a problem.  But:
...
```json
{
    "body": "<a id='comment:50'></a>`@`jpflori - any ideas on this one?",
...
<a id='comment:50'></a>`@`jpflori - any ideas on this one?
...
```json
{
    "body": "<a id='comment:51'></a>I remember doing something with the symbolic i some time ago, potentially while updating pynac.\nNot sure this is related, but I'll begin with finding traces of that.",
...
<a id='comment:51'></a>I remember doing something with the symbolic i some time ago, potentially while updating pynac.
Not sure this is related, but I'll begin with finding traces of that.
...
```json
{
    "body": "<a id='comment:52'></a>This was here http://trac.sagemath.org/sage_trac/ticket/12950#comment:11 but seems unrelated at first sight.",
...
<a id='comment:52'></a>This was here http://trac.sagemath.org/sage_trac/ticket/12950#comment:11 but seems unrelated at first sight.
...
```json
{
    "body": "<a id='comment:53'></a>No, this is quite different, I think.  You might as well read the long list of updates here first - it was quite an education even doing all this, though I was ultimately woefully unsuccessful.",
...
<a id='comment:53'></a>No, this is quite different, I think.  You might as well read the long list of updates here first - it was quite an education even doing all this, though I was ultimately woefully unsuccessful.
...
```json
{
    "body": "<a id='comment:54'></a>This is not showing on on Cygwin on XP with the current status of building, nor apparently on Windows 7 (JP, can you confirm this).  Maybe we should close this, though it's frustrating not to know what the problem was.",
...
<a id='comment:54'></a>This is not showing on on Cygwin on XP with the current status of building, nor apparently on Windows 7 (JP, can you confirm this).  Maybe we should close this, though it's frustrating not to know what the problem was.
...
```json
{
    "body": "<a id='comment:55'></a>No, I did not got that error on my Windows 7 install.\nMaybe this is related to #11116?",
...
<a id='comment:55'></a>No, I did not got that error on my Windows 7 install.
Maybe this is related to #11116?
...
```json
{
    "body": "<a id='comment:56'></a>I doubt it.  Since neither of us is seeing this currently, I'll mark it to close, though if it ever happens again at least this info is here for posterity!",
...
<a id='comment:56'></a>I doubt it.  Since neither of us is seeing this currently, I'll mark it to close, though if it ever happens again at least this info is here for posterity!
...
<a id='comment:58'></a>The bug in #11116 only happens on one arch, and potentially with only a few versions of Sage.
Maybe it's the same here.
You were unlucky enough to try out a particular version of Sage on a particular machine where the problems of initialization order lead to a segfault.
But a innocent looking chenge since anywhere in the Sage library might have made the problem disappear.
...
<a id='comment:59'></a>> But a innocent looking chenge since anywhere in the Sage library might have made the problem disappear.
...
```json
{
    "body": "<a id='comment:63'></a>I've encountered exactly this issue trying to import sage, which I finally got to build after a number of other fixes (not all of which I've posted patches for yet).\n\nSince this ticket has already been closed and is quite long, should I open a new one?  Or just reopen this one?  It appears to be the exact same issue--I'm getting a segfault at the line\n\n```\npari_float = cgetr(2 + rounded_prec / wordsize)\n```\nin real_mpfr.pyx.",
...
<a id='comment:63'></a>I've encountered exactly this issue trying to import sage, which I finally got to build after a number of other fixes (not all of which I've posted patches for yet).
...
```json
{
    "body": "<a id='comment:64'></a>I suggest to open a new ticket.",
...
<a id='comment:64'></a>I suggest to open a new ticket.
