# Issue 11017: GP blocks files when reallocating memory; Workaround

archive/issues_010924.json:
```json
{
    "assignees": [],
    "body": "If a request in _eval_line of the gp interface cannot be executed due to lack of stack memory, the tmp file used to transfer results is blocked.\n\nThis is a workaround. We simple create a new file and trust in the system when it comes to cleaning the environement after GP has terminated.\n\nSee #11018 for a ticket for removing this workaround after finding a better solution (without reevaluating the GP code!)\n\nAssignee: **@williamstein**\n\nAuthor: **Martin Raum**\n\nReviewer: **John Cremona**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11017_\n\n",
    "created_at": "2011-03-24T20:27:00Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/needs%20info"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "GP blocks files when reallocating memory; Workaround",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/11017",
    "user": "https://github.com/sagetrac-mraum"
}
```
If a request in _eval_line of the gp interface cannot be executed due to lack of stack memory, the tmp file used to transfer results is blocked.

This is a workaround. We simple create a new file and trust in the system when it comes to cleaning the environement after GP has terminated.

See #11018 for a ticket for removing this workaround after finding a better solution (without reevaluating the GP code!)

Assignee: **@williamstein**

Author: **Martin Raum**

Reviewer: **John Cremona**

_Issue created by migration from https://trac.sagemath.org/ticket/11017_





---

archive/issue_events_133482.json:
```json
{
    "actor": "https://github.com/sagetrac-mraum",
    "created_at": "2011-03-24T20:27:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11017",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11017#event-133482"
}
```



---

archive/issue_events_133483.json:
```json
{
    "actor": "https://github.com/sagetrac-mraum",
    "created_at": "2011-03-24T20:27:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11017",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "component: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11017#event-133483"
}
```



---

archive/issue_events_133484.json:
```json
{
    "actor": "https://github.com/sagetrac-mraum",
    "created_at": "2011-03-24T20:27:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11017",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11017#event-133484"
}
```



---

archive/issue_events_133485.json:
```json
{
    "actor": "https://github.com/sagetrac-mraum",
    "created_at": "2011-03-24T20:27:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11017",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11017#event-133485"
}
```



---

archive/issue_comments_110682.json:
```json
{
    "body": "Attachment: **[trac-11017-gp_file_block.patch.gz](https://github.com/sagemath/sage/files/ticket11017/trac-11017-gp_file_block.patch.gz)**",
    "created_at": "2011-03-24T20:34:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11017",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11017#issuecomment-110682",
    "user": "https://github.com/sagetrac-mraum"
}
```

Attachment: **[trac-11017-gp_file_block.patch.gz](https://github.com/sagemath/sage/files/ticket11017/trac-11017-gp_file_block.patch.gz)**



---

archive/issue_events_133486.json:
```json
{
    "actor": "https://github.com/sagetrac-mraum",
    "created_at": "2011-03-24T20:36:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11017",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11017#event-133486"
}
```



---

archive/issue_comments_110683.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,4 +2,4 @@\n \n This is a workaround. We simple create a new file and trust in the system when it comes to cleaning the environement after GP has terminated.\n \n-See #???? for a ticket for removing this workaround after finding a better solution (without reevaluating the GP code!)\n+See #11018 for a ticket for removing this workaround after finding a better solution (without reevaluating the GP code!)\n``````\n",
    "created_at": "2011-03-24T20:36:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11017",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11017#issuecomment-110683",
    "user": "https://github.com/sagetrac-mraum"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,4 +2,4 @@
 
 This is a workaround. We simple create a new file and trust in the system when it comes to cleaning the environement after GP has terminated.
 
-See #???? for a ticket for removing this workaround after finding a better solution (without reevaluating the GP code!)
+See #11018 for a ticket for removing this workaround after finding a better solution (without reevaluating the GP code!)
``````




---

archive/issue_comments_110684.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThe patch looks ok -- testing now.",
    "created_at": "2011-03-25T16:45:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11017",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11017#issuecomment-110684",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:2'>Comment 2:</a>
The patch looks ok -- testing now.



---

archive/issue_comments_110685.json:
```json
{
    "body": "Reviewer: **John Cremona**",
    "created_at": "2011-03-25T17:07:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11017",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11017#issuecomment-110685",
    "user": "https://github.com/JohnCremona"
}
```

Reviewer: **John Cremona**



---

archive/issue_events_133487.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2011-03-25T17:07:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11017",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11017#event-133487"
}
```



---

archive/issue_events_133488.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2011-03-25T17:07:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11017",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11017#event-133488"
}
```



---

archive/issue_comments_110686.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nReplying to [@JohnCremona](#comment%3A2):\n> The patch looks ok -- testing now.\n\nAll tests pass, so positive review (and on to #11024)",
    "created_at": "2011-03-25T17:07:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11017",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11017#issuecomment-110686",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:3'>Comment 3:</a>
Replying to [@JohnCremona](#comment%3A2):
> The patch looks ok -- testing now.

All tests pass, so positive review (and on to #11024)



---

archive/issue_events_133489.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-26T20:40:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11017",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11017#event-133489"
}
```



---

archive/issue_events_133490.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-03-26T20:40:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11017",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11017#event-133490"
}
```



---

archive/issue_comments_110687.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nPlease give an example of the problem that this ticket is addressing.",
    "created_at": "2011-03-26T20:40:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11017",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11017#issuecomment-110687",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:4'>Comment 4:</a>
Please give an example of the problem that this ticket is addressing.



---

archive/issue_comments_110688.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nA typical example is Dokchitser calculator, that can easily provoke a stack overflow in GP. Then the stack will be increase but the temporary file cannot be read anymore. More specifically, the test on line 760 after applying trac-11024-dokchitser.patch fails for this very reason.",
    "created_at": "2011-03-26T23:10:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11017",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11017#issuecomment-110688",
    "user": "https://github.com/sagetrac-mraum"
}
```

<a id='comment:5'>Comment 5:</a>
A typical example is Dokchitser calculator, that can easily provoke a stack overflow in GP. Then the stack will be increase but the temporary file cannot be read anymore. More specifically, the test on line 760 after applying trac-11024-dokchitser.patch fails for this very reason.



---

archive/issue_comments_110689.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nAllright, I will have a look and try to understand the problem.",
    "created_at": "2011-03-27T10:14:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11017",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11017#issuecomment-110689",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:6'>Comment 6:</a>
Allright, I will have a look and try to understand the problem.



---

archive/issue_events_133491.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11017",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11017#event-133491"
}
```



---

archive/issue_events_133492.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11017",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11017#event-133492"
}
```



---

archive/issue_events_133493.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11017",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11017#event-133493"
}
```



---

archive/issue_events_133494.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11017",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11017#event-133494"
}
```



---

archive/issue_events_133495.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11017",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11017#event-133495"
}
```



---

archive/issue_events_133496.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11017",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11017#event-133496"
}
```



---

archive/issue_events_133497.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11017",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11017#event-133497"
}
```



---

archive/issue_events_133498.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11017",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11017#event-133498"
}
```



---

archive/issue_events_133499.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11017",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11017#event-133499"
}
```
