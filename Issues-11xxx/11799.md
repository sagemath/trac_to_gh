# Issue 11799: typo in tiny_integrals

archive/issues_011627.json:
```json
{
    "assignees": [],
    "body": "In the documentation of the function `tiny_integrals` one can\nfind:\n\n```\nsage: K = pAdicField(17, 5)\nsage: E = EllipticCurve(K, [-31/3, -2501/108])\nsage: E.tiny_integrals?\n...\n       P and Q MUST be in the same residue disk for this result to make\n```\nHere \"disk\" should read \"disc\".\n\nAssignee: **mvngu**\n\nKeywords: **ecc2011**\n\nAuthor: **Andr\u00e9 Apitzsch**\n\nReviewer: **Julian Rueth**\n\nMerged: **sage-4.8.alpha5**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11799_\n\n",
    "closed_at": "2011-12-09T10:21:17Z",
    "created_at": "2011-09-14T12:09:12Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "typo in tiny_integrals",
    "type": "issue",
    "updated_at": "2011-12-13T09:45:46Z",
    "url": "https://github.com/sagemath/sage/issues/11799",
    "user": "https://github.com/zimmermann6"
}
```
In the documentation of the function `tiny_integrals` one can
find:

```
sage: K = pAdicField(17, 5)
sage: E = EllipticCurve(K, [-31/3, -2501/108])
sage: E.tiny_integrals?
...
       P and Q MUST be in the same residue disk for this result to make
```
Here "disk" should read "disc".

Assignee: **mvngu**

Keywords: **ecc2011**

Author: **André Apitzsch**

Reviewer: **Julian Rueth**

Merged: **sage-4.8.alpha5**

_Issue created by migration from https://trac.sagemath.org/ticket/11799_





---

archive/issue_events_145008.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2011-09-14T12:09:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11799",
    "milestone_number": null,
    "milestone_title": "sage-4.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11799#event-145008"
}
```



---

archive/issue_events_145009.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2011-09-14T12:09:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11799",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11799#event-145009"
}
```



---

archive/issue_events_145010.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2011-09-14T12:09:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11799",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11799#event-145010"
}
```



---

archive/issue_events_145011.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2011-09-14T12:09:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11799",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11799#event-145011"
}
```



---

archive/issue_comments_125501.json:
```json
{
    "body": "Changed keywords from **** to **ecc2011**",
    "created_at": "2011-09-16T12:32:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11799#issuecomment-125501",
    "user": "https://github.com/zimmermann6"
}
```

Changed keywords from **** to **ecc2011**



---

archive/issue_comments_125502.json:
```json
{
    "body": "Attachment: **[trac_11799.patch.gz](https://github.com/sagemath/sage/files/ticket11799/trac_11799.patch.gz)**",
    "created_at": "2011-11-17T15:22:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11799#issuecomment-125502",
    "user": "https://github.com/a-andre"
}
```

Attachment: **[trac_11799.patch.gz](https://github.com/sagemath/sage/files/ticket11799/trac_11799.patch.gz)**



---

archive/issue_comments_125503.json:
```json
{
    "body": "Author: **Andr\u00e9 Apitzsch**",
    "created_at": "2011-11-17T15:24:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11799#issuecomment-125503",
    "user": "https://github.com/a-andre"
}
```

Author: **André Apitzsch**



---

archive/issue_events_145012.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2011-11-17T15:24:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11799",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11799#event-145012"
}
```



---

archive/issue_comments_125504.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\ntested against 4.8.alpha3.",
    "created_at": "2011-12-08T17:18:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11799#issuecomment-125504",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:3'>Comment 3:</a>
tested against 4.8.alpha3.



---

archive/issue_events_145013.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2011-12-08T17:18:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11799",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11799#event-145013"
}
```



---

archive/issue_events_145014.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2011-12-08T17:18:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11799",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11799#event-145014"
}
```



---

archive/issue_comments_125505.json:
```json
{
    "body": "Reviewer: **Julian Rueth**",
    "created_at": "2011-12-08T17:18:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11799#issuecomment-125505",
    "user": "https://github.com/saraedum"
}
```

Reviewer: **Julian Rueth**



---

archive/issue_events_145015.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-12-09T10:21:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11799",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11799#event-145015"
}
```



---

archive/issue_events_145016.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-12-09T10:21:17Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11799",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11799#event-145016"
}
```



---

archive/issue_comments_125506.json:
```json
{
    "body": "Merged: **sage-4.8.alpha4**",
    "created_at": "2011-12-09T10:21:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11799#issuecomment-125506",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.8.alpha4**



---

archive/issue_comments_125507.json:
```json
{
    "body": "Changed merged from **sage-4.8.alpha4** to **sage-4.8.alpha5**",
    "created_at": "2011-12-13T09:45:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11799",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11799#issuecomment-125507",
    "user": "https://github.com/jdemeyer"
}
```

Changed merged from **sage-4.8.alpha4** to **sage-4.8.alpha5**
