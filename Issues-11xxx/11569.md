# Issue 11569: Schmidt Samoa Cryptosystem

archive/issues_011397.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mvngu"
    ],
    "body": "The Schmidt\u2013Samoa cryptosystem is an asymmetric cryptographic technique, whose security, like Rabin depends on the difficulty of integer factorization. Unlike Rabin this algorithm does not produce an ambiguity in the decryption at a cost of encryption speed.\n\nCC:  nguyenminh2@gmail.com\n\nKeywords: **Schmidt Samoa, public key, signature scheme**\n\nComponent: **cryptography**\n\nAuthor: **faisaltp**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11569_\n\n",
    "created_at": "2011-07-03T18:38:59Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20cryptography",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Schmidt Samoa Cryptosystem",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/11569",
    "user": "https://github.com/sagetrac-tevinjoseph"
}
```
The Schmidt–Samoa cryptosystem is an asymmetric cryptographic technique, whose security, like Rabin depends on the difficulty of integer factorization. Unlike Rabin this algorithm does not produce an ambiguity in the decryption at a cost of encryption speed.

CC:  nguyenminh2@gmail.com

Keywords: **Schmidt Samoa, public key, signature scheme**

Component: **cryptography**

Author: **faisaltp**

_Issue created by migration from https://trac.sagemath.org/ticket/11569_





---

archive/issue_events_154715.json:
```json
{
    "actor": "https://github.com/sagetrac-tevinjoseph",
    "created_at": "2011-07-03T18:38:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11569",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11569#event-154715"
}
```



---

archive/issue_events_154716.json:
```json
{
    "actor": "https://github.com/sagetrac-tevinjoseph",
    "created_at": "2011-07-03T18:38:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11569",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20cryptography",
    "label_color": "0000ff",
    "label_name": "c: cryptography",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11569#event-154716"
}
```



---

archive/issue_events_154717.json:
```json
{
    "actor": "https://github.com/sagetrac-tevinjoseph",
    "created_at": "2011-07-03T18:38:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11569",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11569#event-154717"
}
```



---

archive/issue_events_154718.json:
```json
{
    "actor": "https://github.com/sagetrac-tevinjoseph",
    "created_at": "2011-07-03T18:38:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11569",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11569#event-154718"
}
```



---

archive/issue_events_154719.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2011-07-03T18:38:59Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/11569",
    "subject": "https://github.com/sagetrac-tevinjoseph",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11569#event-154719"
}
```



---

archive/issue_events_154720.json:
```json
{
    "actor": "https://github.com/sagetrac-tevinjoseph",
    "created_at": "2011-07-03T18:42:46Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/11569",
    "title_is": "Schmidt Samoa Cryptosystem",
    "title_was": "ElGamal Cryptosystem",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11569#event-154720"
}
```



---

archive/issue_comments_118325.json:
```json
{
    "body": "Changed author from **charlsmathew** to **faisaltp**",
    "created_at": "2011-07-03T18:42:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11569",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11569#issuecomment-118325",
    "user": "https://github.com/sagetrac-tevinjoseph"
}
```

Changed author from **charlsmathew** to **faisaltp**



---

archive/issue_comments_118326.json:
```json
{
    "body": "Changed keywords from **Elgamal, public key, signature scheme** to **Schmidt Samoa, public key, signature scheme**",
    "created_at": "2011-07-03T18:42:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11569",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11569#issuecomment-118326",
    "user": "https://github.com/sagetrac-tevinjoseph"
}
```

Changed keywords from **Elgamal, public key, signature scheme** to **Schmidt Samoa, public key, signature scheme**



---

archive/issue_comments_118327.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-In cryptography, the ElGamal encryption system is an asymmetric key encryption algorithm for public-key cryptography which is based on the Diffie-Hellman key exchange. It was described by Taher Elgamal in 1984. The security of the ElGamal scheme depends on the properties of the underlying group G as well as any padding scheme used on the messages.\n+The Schmidt\u2013Samoa cryptosystem is an asymmetric cryptographic technique, whose security, like Rabin depends on the difficulty of integer factorization. Unlike Rabin this algorithm does not produce an ambiguity in the decryption at a cost of encryption speed.\n``````\n",
    "created_at": "2011-07-03T18:42:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11569",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11569#issuecomment-118327",
    "user": "https://github.com/sagetrac-tevinjoseph"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-In cryptography, the ElGamal encryption system is an asymmetric key encryption algorithm for public-key cryptography which is based on the Diffie-Hellman key exchange. It was described by Taher Elgamal in 1984. The security of the ElGamal scheme depends on the properties of the underlying group G as well as any padding scheme used on the messages.
+The Schmidt–Samoa cryptosystem is an asymmetric cryptographic technique, whose security, like Rabin depends on the difficulty of integer factorization. Unlike Rabin this algorithm does not produce an ambiguity in the decryption at a cost of encryption speed.
``````




---

archive/issue_events_154721.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11569",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11569#event-154721"
}
```



---

archive/issue_events_154722.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11569",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11569#event-154722"
}
```



---

archive/issue_events_154723.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11569",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11569#event-154723"
}
```



---

archive/issue_events_154724.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11569",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11569#event-154724"
}
```



---

archive/issue_events_154725.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11569",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11569#event-154725"
}
```



---

archive/issue_events_154726.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11569",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11569#event-154726"
}
```



---

archive/issue_events_154727.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11569",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11569#event-154727"
}
```



---

archive/issue_events_154728.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11569",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11569#event-154728"
}
```



---

archive/issue_events_154729.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11569",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11569#event-154729"
}
```
