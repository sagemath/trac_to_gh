# Issue 11901: Fix more noise on MacOS X 10.4 PPC

archive/issues_011729.json:
```json
{
    "body": "This is a follow-up to #7852 (and #11897).\n\nAs [reported on sage-release](http://groups.google.com/group/sage-release/msg/e885e53316c6880f), some doctests in Sage 4.7.2.alpha3 still failed on a PowerPC running MacOS X 10.4 due to numerical noise (or slightly different behaviour, giving correct but varying results).\n\nThe patch here further increases the threshold for noisy zero terms of polynomials slightly.\n\n---\n\nApply\n1. [attachment:trac_11901-adjust_threshold_for_noisy_zero_terms_in_polynomial_element.patch]\nto the Sage library.\n\n\nAssignee: jason, was\n\nCC:  @kcrisman @rbeezer\n\nKeywords: noisy zero epsilon threshold polynomial term\n\nResolution: fixed\n\nAuthor: Leif Leonhardy\n\nReviewer: Karl-Dieter Crisman, Rob Beezer\n\nMerged: sage-4.7.2.alpha4\n\nIssue created by migration from https://trac.sagemath.org/ticket/11901\n\n",
    "closed_at": "2011-10-08T09:39:14Z",
    "created_at": "2011-10-06T19:19:11Z",
    "labels": [
        "component: linear algebra",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.7.2",
    "title": "Fix more noise on MacOS X 10.4 PPC",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/11901",
    "user": "https://github.com/nexttime"
}
```
This is a follow-up to #7852 (and #11897).

As [reported on sage-release](http://groups.google.com/group/sage-release/msg/e885e53316c6880f), some doctests in Sage 4.7.2.alpha3 still failed on a PowerPC running MacOS X 10.4 due to numerical noise (or slightly different behaviour, giving correct but varying results).

The patch here further increases the threshold for noisy zero terms of polynomials slightly.

---

Apply
1. [attachment:trac_11901-adjust_threshold_for_noisy_zero_terms_in_polynomial_element.patch]
to the Sage library.


Assignee: jason, was

CC:  @kcrisman @rbeezer

Keywords: noisy zero epsilon threshold polynomial term

Resolution: fixed

Author: Leif Leonhardy

Reviewer: Karl-Dieter Crisman, Rob Beezer

Merged: sage-4.7.2.alpha4

Issue created by migration from https://trac.sagemath.org/ticket/11901





---

archive/issue_comments_163294.json:
```json
{
    "body": "Sage library patch. Based on Sage 4.7.2.alpha3 / #7852.",
    "created_at": "2011-10-06T19:26:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11901#issuecomment-163294",
    "user": "https://github.com/nexttime"
}
```

Sage library patch. Based on Sage 4.7.2.alpha3 / #7852.



---

archive/issue_comments_163295.json:
```json
{
    "body": "<a id='comment:1'></a>Attachment [trac_11901-adjust_threshold_for_noisy_zero_terms_in_polynomial_element.patch](tarball://root/attachments/some-uuid/ticket11901/trac_11901-adjust_threshold_for_noisy_zero_terms_in_polynomial_element.patch) by @nexttime created at 2011-10-06 19:32:26\n\nA single-character patch to be tested (by \"student\") and reviewed...",
    "created_at": "2011-10-06T19:32:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11901#issuecomment-163295",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:1'></a>Attachment [trac_11901-adjust_threshold_for_noisy_zero_terms_in_polynomial_element.patch](tarball://root/attachments/some-uuid/ticket11901/trac_11901-adjust_threshold_for_noisy_zero_terms_in_polynomial_element.patch) by @nexttime created at 2011-10-06 19:32:26

A single-character patch to be tested (by "student") and reviewed...



---

archive/issue_comments_163296.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2011-10-06T19:32:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11901#issuecomment-163296",
    "user": "https://github.com/nexttime"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_163297.json:
```json
{
    "body": "Changing author from \"\" to \"Leif Leonhardy\"",
    "created_at": "2011-10-06T19:32:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11901#issuecomment-163297",
    "user": "https://github.com/nexttime"
}
```

Changing author from "" to "Leif Leonhardy"



---

archive/issue_comments_163298.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -4,3 +4,9 @@\n \n The patch here further increases the threshold for noisy zero terms of polynomials slightly.\n \n+---\n+\n+Apply\n+1. [attachment:trac_11901-adjust_threshold_for_noisy_zero_terms_in_polynomial_element.patch]\n+to the Sage library.\n+\n``````\n",
    "created_at": "2011-10-06T19:32:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11901#issuecomment-163298",
    "user": "https://github.com/nexttime"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -4,3 +4,9 @@
 
 The patch here further increases the threshold for noisy zero terms of polynomials slightly.
 
+---
+
+Apply
+1. [attachment:trac_11901-adjust_threshold_for_noisy_zero_terms_in_polynomial_element.patch]
+to the Sage library.
+
``````




---

archive/issue_comments_163299.json:
```json
{
    "body": "<a id='comment:2'></a>This looks good to me, but I think we need to see if it does the job on Mac PowerPC?",
    "created_at": "2011-10-07T04:29:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11901#issuecomment-163299",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:2'></a>This looks good to me, but I think we need to see if it does the job on Mac PowerPC?



---

archive/issue_comments_163300.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2011-10-07T17:06:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11901#issuecomment-163300",
    "user": "https://github.com/kcrisman"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_163301.json:
```json
{
    "body": "<a id='comment:3'></a>I know this is sort of silly, but it is nice to have all tests pass :)\n\nWorks fine.\n\n```\nsage -t -long \"devel/sage/sage/rings/polynomial/polynomial_element.pyx\"\n         [156.8 s]\n \n----------------------------------------------------------------------\nAll tests passed!\nTotal time for all tests: 157.4 seconds\n```",
    "created_at": "2011-10-07T17:06:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11901#issuecomment-163301",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:3'></a>I know this is sort of silly, but it is nice to have all tests pass :)

Works fine.

```
sage -t -long "devel/sage/sage/rings/polynomial/polynomial_element.pyx"
         [156.8 s]
 
----------------------------------------------------------------------
All tests passed!
Total time for all tests: 157.4 seconds
```



---

archive/issue_comments_163302.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Karl-Dieter Crisman, Rob Beezer\"",
    "created_at": "2011-10-07T17:06:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11901#issuecomment-163302",
    "user": "https://github.com/kcrisman"
}
```

Changing reviewer from "" to "Karl-Dieter Crisman, Rob Beezer"



---

archive/issue_comments_163303.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-4.7.2.alpha4\"",
    "created_at": "2011-10-08T09:39:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11901#issuecomment-163303",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-4.7.2.alpha4"



---

archive/issue_comments_163304.json:
```json
{
    "body": "<a id='comment:4'></a>Thanks!",
    "created_at": "2011-10-08T09:39:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11901#issuecomment-163304",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:4'></a>Thanks!



---

archive/issue_comments_163305.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2011-10-08T09:39:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11901#issuecomment-163305",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed



---

archive/issue_events_031466.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-10-08T09:39:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/11901",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11901#event-31466"
}
```



---

archive/issue_comments_163306.json:
```json
{
    "body": "<a id='comment:5'></a>Replying to [comment:4 jdemeyer]:\n> Thanks!\n\n\nSo Moufang is still alive?",
    "created_at": "2011-10-08T13:44:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11901#issuecomment-163306",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:5'></a>Replying to [comment:4 jdemeyer]:
> Thanks!


So Moufang is still alive?



---

archive/issue_comments_163307.json:
```json
{
    "body": "<a id='comment:6'></a>Replying to [comment:5 leif]:\n> So Moufang is still alive?\n\nOf course.  Did I ever say it wasn't?  I don't actually *use* this machine, only to test Sage.",
    "created_at": "2011-10-08T14:01:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11901#issuecomment-163307",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:6'></a>Replying to [comment:5 leif]:
> So Moufang is still alive?

Of course.  Did I ever say it wasn't?  I don't actually *use* this machine, only to test Sage.
