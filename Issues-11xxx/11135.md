# Issue 11135: Crashes when constructing algebraic numbers using Matrix.eigenvectors

archive/issues_010993.json:
```json
{
    "body": "With certain constructions of algebraic numbers, the numbers exhibit very weird behaviour and crashes in almost any interaction.\n\n```\n\nsage: M = matrix(3, [0,0,1,1,0,1,0,1,0]) \nsage: y = M.eigenvectors_left()[1][1][0]\nsage: a = abs(y[1])\nsage: a\n0.868836961832710?\nsage: type(a)\n<class 'sage.rings.qqbar.AlgebraicReal'>\nsage: a.minpoly()\n...\nTypeError: Unable to convert number to real interval.\nsage: a*a == a*a\n...\nTypeError: Unable to convert number to real interval.\n\n```\n\nWhen constructing the above number by more direct methods, it seems to be alright:\n\n```\nsage: R.<x> = QQbar[x]\nsage: exp = x^3-x-1\nsage: y1 = exp.roots()[1][0]\nsage: a = abs(y1)\nsage: a\n0.868836961832710?\nsage: a.minpoly()\nx^6 + x^4 - 1\nsage: a*a == a*a\nTrue\n\n```\n\nI don't know if the error is pertaining to eigenvectors, AlgebraicNumber, AlgebraicReal, abs, or a combination of these.\n\nAssignee: @aghitza\n\nCC:  tjolivet\n\nKeywords: matrix, eigenvectors\n\nReviewer: Johan Sebastian Rosenkilde Nielsen\n\nResolution: worksforme\n\nIssue created by migration from https://trac.sagemath.org/ticket/11135\n\n",
    "closed_at": "2013-10-05T09:44:21Z",
    "created_at": "2011-04-06T10:44:56Z",
    "labels": [
        "component: algebra",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "Crashes when constructing algebraic numbers using Matrix.eigenvectors",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/11135",
    "user": "https://github.com/johanrosenkilde"
}
```
With certain constructions of algebraic numbers, the numbers exhibit very weird behaviour and crashes in almost any interaction.

```

sage: M = matrix(3, [0,0,1,1,0,1,0,1,0]) 
sage: y = M.eigenvectors_left()[1][1][0]
sage: a = abs(y[1])
sage: a
0.868836961832710?
sage: type(a)
<class 'sage.rings.qqbar.AlgebraicReal'>
sage: a.minpoly()
...
TypeError: Unable to convert number to real interval.
sage: a*a == a*a
...
TypeError: Unable to convert number to real interval.

```

When constructing the above number by more direct methods, it seems to be alright:

```
sage: R.<x> = QQbar[x]
sage: exp = x^3-x-1
sage: y1 = exp.roots()[1][0]
sage: a = abs(y1)
sage: a
0.868836961832710?
sage: a.minpoly()
x^6 + x^4 - 1
sage: a*a == a*a
True

```

I don't know if the error is pertaining to eigenvectors, AlgebraicNumber, AlgebraicReal, abs, or a combination of these.

Assignee: @aghitza

CC:  tjolivet

Keywords: matrix, eigenvectors

Reviewer: Johan Sebastian Rosenkilde Nielsen

Resolution: worksforme

Issue created by migration from https://trac.sagemath.org/ticket/11135





---

archive/issue_comments_144944.json:
```json
{
    "body": "<a id='comment:2'></a>Works now (sage-5.11.rc0). Probably a duplicate of #12727.",
    "created_at": "2013-08-06T09:59:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11135",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11135#issuecomment-144944",
    "user": "https://github.com/mezzarobba"
}
```

<a id='comment:2'></a>Works now (sage-5.11.rc0). Probably a duplicate of #12727.



---

archive/issue_events_028876.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2013-08-06T09:59:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11135",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11135#event-28876"
}
```



---

archive/issue_comments_144945.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2013-08-06T09:59:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11135",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11135#issuecomment-144945",
    "user": "https://github.com/mezzarobba"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_144946.json:
```json
{
    "body": "<a id='comment:3'></a>Indeed, this has been solved.",
    "created_at": "2013-09-22T18:30:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11135",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11135#issuecomment-144946",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:3'></a>Indeed, this has been solved.



---

archive/issue_comments_144947.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2013-09-22T18:30:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11135",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11135#issuecomment-144947",
    "user": "https://github.com/fchapoton"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_144948.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Johan S. R. Nielsen\"",
    "created_at": "2013-10-05T09:44:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11135",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11135#issuecomment-144948",
    "user": "https://github.com/jdemeyer"
}
```

Changing reviewer from "" to "Johan S. R. Nielsen"



---

archive/issue_events_028877.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-05T09:44:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/11135",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11135#event-28877"
}
```



---

archive/issue_comments_144949.json:
```json
{
    "body": "Changing author from \"Johan S. R. Nielsen\" to \"\"",
    "created_at": "2013-10-05T09:44:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11135",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11135#issuecomment-144949",
    "user": "https://github.com/jdemeyer"
}
```

Changing author from "Johan S. R. Nielsen" to ""



---

archive/issue_comments_144950.json:
```json
{
    "body": "Resolution: worksforme",
    "created_at": "2013-10-05T09:44:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11135",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11135#issuecomment-144950",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: worksforme



---

archive/issue_comments_144951.json:
```json
{
    "body": "Changing reviewer from \"Johan S. R. Nielsen\" to \"Johan Sebastian Rosenkilde Nielsen\"",
    "created_at": "2013-10-09T20:27:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11135",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11135#issuecomment-144951",
    "user": "https://github.com/jdemeyer"
}
```

Changing reviewer from "Johan S. R. Nielsen" to "Johan Sebastian Rosenkilde Nielsen"
