# Issue 11773: Elliptic curve constructor with no arguments gives an obscure error message

archive/issues_011601.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: EllipticCurve()\n```\nleads to an assertion error instead of a sensible error message.\n\nThis has been fixed as part of #13100.  This ticket can be closed.\n\nAssignee: **@JohnCremona**\n\nKeywords: **elliptic curve**\n\nReviewer: **John Cremona**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11773_\n\n",
    "closed_at": "2012-06-19T13:33:10Z",
    "created_at": "2011-09-01T16:34:38Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20elliptic%20curves",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Elliptic curve constructor with no arguments gives an obscure error message",
    "type": "issue",
    "updated_at": "2012-06-19T13:33:10Z",
    "url": "https://github.com/sagemath/sage/issues/11773",
    "user": "https://github.com/JohnCremona"
}
```

```
sage: EllipticCurve()
```
leads to an assertion error instead of a sensible error message.

This has been fixed as part of #13100.  This ticket can be closed.

Assignee: **@JohnCremona**

Keywords: **elliptic curve**

Reviewer: **John Cremona**

_Issue created by migration from https://trac.sagemath.org/ticket/11773_





---

archive/issue_events_144573.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2011-09-01T16:34:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11773",
    "milestone_number": null,
    "milestone_title": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11773#event-144573"
}
```



---

archive/issue_events_144574.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2011-09-01T16:34:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11773",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20elliptic%20curves",
    "label_color": "0000ff",
    "label_name": "component: elliptic curves",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11773#event-144574"
}
```



---

archive/issue_events_144575.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2011-09-01T16:34:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11773",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11773#event-144575"
}
```



---

archive/issue_events_144576.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2011-09-01T16:34:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11773",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11773#event-144576"
}
```



---

archive/issue_comments_125036.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nThis has been fixed as part of #13100.",
    "created_at": "2012-06-10T16:43:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11773",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11773#issuecomment-125036",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:1" align="right">Comment 1</div>

This has been fixed as part of #13100.



---

archive/issue_comments_125037.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -3,3 +3,5 @@\n sage: EllipticCurve()\n ```\n leads to an assertion error instead of a sensible error message.\n+\n+This has been fixed as part of #13100.  This ticket can be closed.\n``````\n",
    "created_at": "2012-06-10T16:43:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11773",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11773#issuecomment-125037",
    "user": "https://github.com/JohnCremona"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -3,3 +3,5 @@
 sage: EllipticCurve()
 ```
 leads to an assertion error instead of a sensible error message.
+
+This has been fixed as part of #13100.  This ticket can be closed.
``````




---

archive/issue_events_144577.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-06-11T16:23:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11773",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11773#event-144577"
}
```



---

archive/issue_events_144578.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-06-11T16:24:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11773",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11773#event-144578"
}
```



---

archive/issue_events_144579.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-06-11T16:24:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11773",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11773#event-144579"
}
```



---

archive/issue_comments_125038.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\n#13100 has a positive review",
    "created_at": "2012-06-11T16:24:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11773",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11773#issuecomment-125038",
    "user": "https://github.com/roed314"
}
```

<div id="comment:3" align="right">Comment 3</div>

#13100 has a positive review



---

archive/issue_events_144580.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-06-11T16:24:08Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11773",
    "milestone_number": null,
    "milestone_title": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11773#event-144580"
}
```



---

archive/issue_events_144581.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-06-11T16:24:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11773",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11773#event-144581"
}
```



---

archive/issue_events_144582.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-06-11T16:24:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11773",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11773#event-144582"
}
```



---

archive/issue_comments_125039.json:
```json
{
    "body": "Reviewer: **John Cremona**",
    "created_at": "2012-06-19T13:33:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11773",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11773#issuecomment-125039",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **John Cremona**



---

archive/issue_events_144583.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-19T13:33:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11773",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11773#event-144583"
}
```



---

archive/issue_events_144584.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-19T13:33:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11773",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11773#event-144584"
}
```



---

archive/issue_events_144585.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-19T13:33:10Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11773",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11773#event-144585"
}
```
