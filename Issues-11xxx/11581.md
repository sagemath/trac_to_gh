...
```json
{
    "body": "Hi, I tried building Sage (the release, not the alpha) on the Core 2 machine. It solved the problem. I also tried the alpha on the other machine; it also solved the problem and performance seems to be better as well. Thanks for the help.",
...
```json
{
    "body": "I just realized I misinterpreted the report.  The problematic version is sage-4.7, so you should try the most recent alpha version instead.  Currently, this is [sage-4.7.1.alpha4](http://boxen.math.washington.edu/home/release/sage-4.7.1.alpha)",
...
```json
{
    "body": "Another segfault presumably caused by ATLAS with large matrices (this time NumPy, 512x512), and this time clearly in `libpthread`: #11674",
    "created_at": "2011-08-10T05:34:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11581",
    "type": "issue_comment",
...
```json
{
    "body": "ping because trac notifications seem to work again.",
...
```json
{
    "body": "Let's assume this is fixed...",
...
```json
{
    "body": "I think this was also just caused by the binary distribution being built on redhawk, which ran the obsolete Ubuntu 10.04.1 LTS, with an apparently incompatible / broken libpthread.  (It's now at 10.04.4, so this should no longer happen.)",
