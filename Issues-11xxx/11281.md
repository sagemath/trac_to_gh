# Issue 11281: .. math:: environment ignored in the live documentation

archive/issues_011109.json:
```json
{
    "assignees": [
        "https://github.com/jasongrout",
        "https://github.com/qed777",
        "https://github.com/williamstein"
    ],
    "body": ".. math:: environments seem to be ignored in the live documentation. See e.g. the second method `random_element` in http://localhost:8080/doc/live/reference/quat_algebras/sage/algebras/quatalg/quaternion_algebra.html (with the notebook running on your machine).\n\nThe inline math environments work fine. The same .. math environment work fine in the static documentation\n\n\nCC:  @sagetrac-sage-combinat\n\nKeywords: **days30**\n\nUpstream: **Reported upstream. Developers acknowledge bug.**\n\nReviewer: **John Palmieri, Keshav Kini, Karl-Dieter Crisman**\n\nComponent: **notebook**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11281_\n\n",
    "closed_at": "2014-10-27T16:26:09Z",
    "created_at": "2011-05-02T17:57:10Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": ".. math:: environment ignored in the live documentation",
    "type": "issue",
    "updated_at": "2014-10-27T16:26:09Z",
    "url": "https://github.com/sagemath/sage/issues/11281",
    "user": "https://github.com/nthiery"
}
```
.. math:: environments seem to be ignored in the live documentation. See e.g. the second method `random_element` in http://localhost:8080/doc/live/reference/quat_algebras/sage/algebras/quatalg/quaternion_algebra.html (with the notebook running on your machine).

The inline math environments work fine. The same .. math environment work fine in the static documentation


CC:  @sagetrac-sage-combinat

Keywords: **days30**

Upstream: **Reported upstream. Developers acknowledge bug.**

Reviewer: **John Palmieri, Keshav Kini, Karl-Dieter Crisman**

Component: **notebook**

_Issue created by migration from https://trac.sagemath.org/ticket/11281_





---

archive/issue_events_149600.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2011-05-02T17:57:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "milestone_number": null,
    "milestone_title": "sage-5.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149600"
}
```



---

archive/issue_events_149601.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2011-05-02T17:57:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149601"
}
```



---

archive/issue_events_149602.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2011-05-02T17:57:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149602"
}
```



---

archive/issue_events_149603.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2011-05-02T17:57:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149603"
}
```



---

archive/issue_events_149604.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2011-05-02T17:57:10Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "subject": "https://github.com/nthiery",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149604"
}
```



---

archive/issue_events_149605.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2011-05-02T17:57:10Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "subject": "https://github.com/nthiery",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149605"
}
```



---

archive/issue_events_149606.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2011-05-02T17:57:10Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "subject": "https://github.com/nthiery",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149606"
}
```



---

archive/issue_comments_113157.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nHmm.  This works for me (both 4.6 and 4.7.rc1, OS X 10.6.7 Intel, both Safari and Firefox).  Can you give more information about your set-up?  Has it worked for you with any version of Sage?  Are you sure it wasn't still \"Processing Math\"?",
    "created_at": "2011-05-02T18:50:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11281#issuecomment-113157",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:1" align="right">comment:1</div>

Hmm.  This works for me (both 4.6 and 4.7.rc1, OS X 10.6.7 Intel, both Safari and Firefox).  Can you give more information about your set-up?  Has it worked for you with any version of Sage?  Are you sure it wasn't still "Processing Math"?



---

archive/issue_comments_113158.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,4 @@\n-.. math:: environments seem to be ignored in the live documentation. See e.g. http://localhost:8000/doc/live/reference/sage/algebras/quatalg/quaternion_algebra.html (with the notebook running on your machine).\n+.. math:: environments seem to be ignored in the live documentation. See e.g. the second method `random_element` in http://localhost:8000/doc/live/reference/sage/algebras/quatalg/quaternion_algebra.html (with the notebook running on your machine).\n+\n+The inline math environments work fine. The same .. math environment work fine in the static documentation\n+\n``````\n",
    "created_at": "2011-05-02T19:11:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11281#issuecomment-113158",
    "user": "https://github.com/nthiery"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,4 @@
-.. math:: environments seem to be ignored in the live documentation. See e.g. http://localhost:8000/doc/live/reference/sage/algebras/quatalg/quaternion_algebra.html (with the notebook running on your machine).
+.. math:: environments seem to be ignored in the live documentation. See e.g. the second method `random_element` in http://localhost:8000/doc/live/reference/sage/algebras/quatalg/quaternion_algebra.html (with the notebook running on your machine).
+
+The inline math environments work fine. The same .. math environment work fine in the static documentation
+
``````




---

archive/issue_comments_113159.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nReplying to [@jhpalmieri](#comment:1):\n\nThanks for your quick answer!\n\n> Hmm.  This works for me (both 4.6 and 4.7.rc1, OS X 10.6.7 Intel, both Safari and Firefox).  Can you give more information about your set-up? \n\nOups, I was not specific about where in the file the issue wasx. I just added info to the description.\n\nSage 4.6.2 vanilla, Firefox 3.6.16, Ubuntu and OpenSuse. \n\n> Has it worked for you with any version of Sage?\n\nSorry, I don't know.\n\n> Are you sure it wasn't still \"Processing Math\"?\n\nNope.\n\nCheers,",
    "created_at": "2011-05-02T19:11:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11281#issuecomment-113159",
    "user": "https://github.com/nthiery"
}
```

<div id="comment:2" align="right">comment:2</div>

Replying to [@jhpalmieri](#comment:1):

Thanks for your quick answer!

> Hmm.  This works for me (both 4.6 and 4.7.rc1, OS X 10.6.7 Intel, both Safari and Firefox).  Can you give more information about your set-up? 

Oups, I was not specific about where in the file the issue wasx. I just added info to the description.

Sage 4.6.2 vanilla, Firefox 3.6.16, Ubuntu and OpenSuse. 

> Has it worked for you with any version of Sage?

Sorry, I don't know.

> Are you sure it wasn't still "Processing Math"?

Nope.

Cheers,



---

archive/issue_comments_113160.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nJust to clarify, both of the \".. math\" environments in this file (in random_element and theta_series) work for me.",
    "created_at": "2011-05-02T19:32:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11281#issuecomment-113160",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:3" align="right">comment:3</div>

Just to clarify, both of the ".. math" environments in this file (in random_element and theta_series) work for me.



---

archive/issue_events_149607.json:
```json
{
    "actor": "https://github.com/kini",
    "created_at": "2013-03-27T22:42:34Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "milestone_number": null,
    "milestone_title": "sage-5.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149607"
}
```



---

archive/issue_events_149608.json:
```json
{
    "actor": "https://github.com/kini",
    "created_at": "2013-03-27T22:42:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149608"
}
```



---

archive/issue_events_149609.json:
```json
{
    "actor": "https://github.com/kini",
    "created_at": "2013-03-27T22:42:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149609"
}
```



---

archive/issue_events_149610.json:
```json
{
    "actor": "https://github.com/kini",
    "created_at": "2013-03-27T22:42:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149610"
}
```



---

archive/issue_comments_113161.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nPull request: https://github.com/sagemath/sagenb/issues/145",
    "created_at": "2013-03-27T22:42:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11281#issuecomment-113161",
    "user": "https://github.com/kini"
}
```

<div id="comment:4" align="right">comment:4</div>

Pull request: https://github.com/sagemath/sagenb/issues/145



---

archive/issue_comments_113162.json:
```json
{
    "body": "Reviewer: **Keshav Kini**",
    "created_at": "2013-03-27T22:42:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11281#issuecomment-113162",
    "user": "https://github.com/kini"
}
```

Reviewer: **Keshav Kini**



---

archive/issue_events_149611.json:
```json
{
    "actor": "https://github.com/kini",
    "created_at": "2013-03-27T22:42:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149611"
}
```



---

archive/issue_events_149612.json:
```json
{
    "actor": "https://github.com/kini",
    "created_at": "2013-03-27T22:42:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149612"
}
```



---

archive/issue_comments_113163.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nI have changed this to \"reported upstream, developers acknowledge bug\" (see #8482 for rationale).",
    "created_at": "2013-03-28T14:00:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11281#issuecomment-113163",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:6" align="right">comment:6</div>

I have changed this to "reported upstream, developers acknowledge bug" (see #8482 for rationale).



---

archive/issue_events_149613.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-03-28T14:00:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149613"
}
```



---

archive/issue_events_149614.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-03-28T14:00:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149614"
}
```



---

archive/issue_comments_113164.json:
```json
{
    "body": "Upstream: **Reported upstream. Developers acknowledge bug.**",
    "created_at": "2013-03-28T14:00:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11281#issuecomment-113164",
    "user": "https://github.com/kcrisman"
}
```

Upstream: **Reported upstream. Developers acknowledge bug.**



---

archive/issue_comments_113165.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nYup, OK.",
    "created_at": "2013-03-28T14:13:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11281#issuecomment-113165",
    "user": "https://github.com/kini"
}
```

<div id="comment:7" align="right">comment:7</div>

Yup, OK.



---

archive/issue_events_149615.json:
```json
{
    "actor": "https://github.com/kini",
    "created_at": "2013-03-28T14:18:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "milestone_number": null,
    "milestone_title": "sage-5.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149615"
}
```



---

archive/issue_events_149616.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149616"
}
```



---

archive/issue_events_149617.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149617"
}
```



---

archive/issue_events_149618.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149618"
}
```



---

archive/issue_events_149619.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149619"
}
```



---

archive/issue_events_149620.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149620"
}
```



---

archive/issue_events_149621.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149621"
}
```



---

archive/issue_events_149622.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149622"
}
```



---

archive/issue_events_149623.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149623"
}
```



---

archive/issue_comments_113166.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-.. math:: environments seem to be ignored in the live documentation. See e.g. the second method `random_element` in http://localhost:8000/doc/live/reference/sage/algebras/quatalg/quaternion_algebra.html (with the notebook running on your machine).\n+.. math:: environments seem to be ignored in the live documentation. See e.g. the second method `random_element` in http://localhost:8080/doc/live/reference/quat_algebras/sage/algebras/quatalg/quaternion_algebra.html (with the notebook running on your machine).\n \n The inline math environments work fine. The same .. math environment work fine in the static documentation\n \n``````\n",
    "created_at": "2014-10-22T15:41:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11281#issuecomment-113166",
    "user": "https://github.com/kcrisman"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-.. math:: environments seem to be ignored in the live documentation. See e.g. the second method `random_element` in http://localhost:8000/doc/live/reference/sage/algebras/quatalg/quaternion_algebra.html (with the notebook running on your machine).
+.. math:: environments seem to be ignored in the live documentation. See e.g. the second method `random_element` in http://localhost:8080/doc/live/reference/quat_algebras/sage/algebras/quatalg/quaternion_algebra.html (with the notebook running on your machine).
 
 The inline math environments work fine. The same .. math environment work fine in the static documentation
 
``````




---

archive/issue_comments_113167.json:
```json
{
    "body": "Changed reviewer from **Keshav Kini** to **John Palmieri, Keshav Kini, Karl-Dieter Crisman**",
    "created_at": "2014-10-22T15:52:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11281#issuecomment-113167",
    "user": "https://github.com/kcrisman"
}
```

Changed reviewer from **Keshav Kini** to **John Palmieri, Keshav Kini, Karl-Dieter Crisman**



---

archive/issue_events_149624.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-10-22T15:52:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149624"
}
```



---

archive/issue_events_149625.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-10-22T15:52:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149625"
}
```



---

archive/issue_events_149626.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-10-22T15:52:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149626"
}
```



---

archive/issue_comments_113168.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">comment:14</div>\n\nI cannot reproduce this either, see [comment:3](#comment:3), on Safari or FF.  Mathjax seems to be fine.   I've closed the upstream ticket, please open a different upstream ticket if you still find this a problem.  (There are a number of changes that might have made this happen, including upgrading sagenb.org.)",
    "created_at": "2014-10-22T15:52:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11281#issuecomment-113168",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:14" align="right">comment:14</div>

I cannot reproduce this either, see [comment:3](#comment:3), on Safari or FF.  Mathjax seems to be fine.   I've closed the upstream ticket, please open a different upstream ticket if you still find this a problem.  (There are a number of changes that might have made this happen, including upgrading sagenb.org.)



---

archive/issue_comments_113169.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">comment:15</div>\n\nSeems indeed to work fine on my machine. Kuddos to whoever fixed this :-)",
    "created_at": "2014-10-22T16:00:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11281#issuecomment-113169",
    "user": "https://github.com/nthiery"
}
```

<div id="comment:15" align="right">comment:15</div>

Seems indeed to work fine on my machine. Kuddos to whoever fixed this :-)



---

archive/issue_comments_113170.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">comment:16</div>\n\nIt will forever remain a mystery!",
    "created_at": "2014-10-22T16:03:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11281#issuecomment-113170",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:16" align="right">comment:16</div>

It will forever remain a mystery!



---

archive/issue_events_149627.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-27T16:26:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149627"
}
```



---

archive/issue_events_149628.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-27T16:26:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11281",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11281#event-149628"
}
```
