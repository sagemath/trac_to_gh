# Issue 11629: Evaluating the order of an ideal class fails

archive/issues_011457.json:
```json
{
    "assignees": [],
    "body": "The following does not work\n\n```\nsage: K.<t> = NumberField(x^5-9*x^4+22*x-71)\nsage: v = K.ideal(2, t^3 - 8*t^2 - 7*t - 3)\nsage: c = K.class_group()(v)\nsage: c.order()\n```\n\ngiving a type-error. While it should give back 3.\n\nAssignee: **@loefflerd**\n\nKeywords: **class group**\n\nReviewer: **Francis Clarke**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11629_\n\n",
    "closed_at": "2011-08-18T09:37:59Z",
    "created_at": "2011-07-26T14:06:37Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Evaluating the order of an ideal class fails",
    "type": "issue",
    "updated_at": "2011-08-18T09:37:59Z",
    "url": "https://github.com/sagemath/sage/issues/11629",
    "user": "https://github.com/categorie"
}
```
The following does not work

```
sage: K.<t> = NumberField(x^5-9*x^4+22*x-71)
sage: v = K.ideal(2, t^3 - 8*t^2 - 7*t - 3)
sage: c = K.class_group()(v)
sage: c.order()
```

giving a type-error. While it should give back 3.

Assignee: **@loefflerd**

Keywords: **class group**

Reviewer: **Francis Clarke**

_Issue created by migration from https://trac.sagemath.org/ticket/11629_





---

archive/issue_events_142299.json:
```json
{
    "actor": "https://github.com/categorie",
    "created_at": "2011-07-26T14:06:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11629",
    "milestone_number": null,
    "milestone_title": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11629#event-142299"
}
```



---

archive/issue_events_142300.json:
```json
{
    "actor": "https://github.com/categorie",
    "created_at": "2011-07-26T14:06:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11629",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
    "label_color": "0000ff",
    "label_name": "component: number fields",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11629#event-142300"
}
```



---

archive/issue_events_142301.json:
```json
{
    "actor": "https://github.com/categorie",
    "created_at": "2011-07-26T14:06:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11629",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11629#event-142301"
}
```



---

archive/issue_events_142302.json:
```json
{
    "actor": "https://github.com/categorie",
    "created_at": "2011-07-26T14:06:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11629",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11629#event-142302"
}
```



---

archive/issue_comments_121838.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAs aconsequence, one can not do `c^4`.",
    "created_at": "2011-07-26T14:07:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11629",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11629#issuecomment-121838",
    "user": "https://github.com/categorie"
}
```

<a id='comment:1'>Comment 1:</a>
As aconsequence, one can not do `c^4`.



---

archive/issue_comments_121839.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThe problem was sorted out in #11234, which was merged in sage-4.7.1.alpha0.   So I think this ticket can be closed as a duplicate.",
    "created_at": "2011-07-27T06:27:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11629",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11629#issuecomment-121839",
    "user": "https://github.com/sagetrac-fwclarke"
}
```

<a id='comment:2'>Comment 2:</a>
The problem was sorted out in #11234, which was merged in sage-4.7.1.alpha0.   So I think this ticket can be closed as a duplicate.



---

archive/issue_comments_121840.json:
```json
{
    "body": "Reviewer: **Francis Clarke**",
    "created_at": "2011-07-27T07:44:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11629",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11629#issuecomment-121840",
    "user": "https://github.com/loefflerd"
}
```

Reviewer: **Francis Clarke**



---

archive/issue_events_142303.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2011-07-27T07:44:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11629",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11629#event-142303"
}
```



---

archive/issue_events_142304.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2011-07-27T07:45:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11629",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11629#event-142304"
}
```



---

archive/issue_events_142305.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2011-07-27T07:45:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11629",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11629#event-142305"
}
```



---

archive/issue_comments_121841.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nYes, this is fixed. Release manager: please close as duplicate.",
    "created_at": "2011-07-27T07:45:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11629",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11629#issuecomment-121841",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:4'>Comment 4:</a>
Yes, this is fixed. Release manager: please close as duplicate.



---

archive/issue_events_142306.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-08-17T19:34:50Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11629",
    "milestone_number": null,
    "milestone_title": "sage-4.7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11629#event-142306"
}
```



---

archive/issue_events_142307.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-08-17T19:34:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11629",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11629#event-142307"
}
```



---

archive/issue_events_142308.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-08-17T19:34:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11629",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11629#event-142308"
}
```



---

archive/issue_events_142309.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-08-18T09:37:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11629",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11629#event-142309"
}
```



---

archive/issue_events_142310.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-08-18T09:37:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11629",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11629#event-142310"
}
```



---

archive/issue_events_142311.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-08-18T09:37:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11629",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11629#event-142311"
}
```
