# Issue 11958: cplex needs another symbolic link

archive/issues_011786.json:
```json
{
    "body": "In order to get a current CPLEX to work with Sage, I had to add one more symbolic link to `SAGE_ROOT/local/include/` than the directions at http://www.sagemath.org/doc/thematic_tutorials/linear_programming.html#installing-cplex indicated:\n\n```\nln -s /path/to/include/cpxconst.h .\n```\n\nWithout this, `sage -b` complained about that file missing.  With this new symbolic link, `sage -b` succeeds and I can solve an example problem with cplex.\n\nSo the docs probably should be updated.\n\nAssignee: tbd\n\nCC:  @nathanncohen\n\nKeywords: Cernay2012\n\nReviewer: David Coudert\n\nAuthor: Nathann Cohen, Jason Grout\n\nMerged: sage-5.0.beta12\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/11958\n\n",
    "closed_at": "2012-04-02T15:24:31Z",
    "created_at": "2011-10-27T16:02:29Z",
    "labels": [
        "component: linear programming",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-5.0",
    "title": "cplex needs another symbolic link",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/11958",
    "user": "https://github.com/jasongrout"
}
```
In order to get a current CPLEX to work with Sage, I had to add one more symbolic link to `SAGE_ROOT/local/include/` than the directions at http://www.sagemath.org/doc/thematic_tutorials/linear_programming.html#installing-cplex indicated:

```
ln -s /path/to/include/cpxconst.h .
```

Without this, `sage -b` complained about that file missing.  With this new symbolic link, `sage -b` succeeds and I can solve an example problem with cplex.

So the docs probably should be updated.

Assignee: tbd

CC:  @nathanncohen

Keywords: Cernay2012

Reviewer: David Coudert

Author: Nathann Cohen, Jason Grout

Merged: sage-5.0.beta12

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/11958





---

archive/issue_comments_164948.json:
```json
{
    "body": "<a id='comment:1'></a>`O_o`\n\nI have been using CPLEX for (almost) years and I never met that. `O_o`\n\nWhich kind of version of it do you use ? `O_o`\n\nNathann",
    "created_at": "2011-10-27T17:07:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11958#issuecomment-164948",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:1'></a>`O_o`

I have been using CPLEX for (almost) years and I never met that. `O_o`

Which kind of version of it do you use ? `O_o`

Nathann



---

archive/issue_comments_164949.json:
```json
{
    "body": "<a id='comment:2'></a>\n```\n\nIBM ILOG CPLEX Optimization Studio Academic Research Edition\nWelcome to IBM(R) ILOG(R) CPLEX(R) Interactive Optimizer 12.3.0.0\n  with Simplex, Mixed Integer & Barrier Optimizers\n5725-A06 5725-A29 5724-Y48 5724-Y49 5724-Y54 5724-Y55\nCopyright IBM Corp. 1988, 2011.  All Rights Reserved.\n```",
    "created_at": "2011-10-27T18:40:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11958#issuecomment-164949",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:2'></a>
```

IBM ILOG CPLEX Optimization Studio Academic Research Edition
Welcome to IBM(R) ILOG(R) CPLEX(R) Interactive Optimizer 12.3.0.0
  with Simplex, Mixed Integer & Barrier Optimizers
5725-A06 5725-A29 5724-Y48 5724-Y49 5724-Y54 5724-Y55
Copyright IBM Corp. 1988, 2011.  All Rights Reserved.
```



---

archive/issue_comments_164950.json:
```json
{
    "body": "<a id='comment:3'></a>Sigh... I'm using version 12.1.0 `:-)`\n\nNathann",
    "created_at": "2011-10-27T18:42:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11958#issuecomment-164950",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:3'></a>Sigh... I'm using version 12.1.0 `:-)`

Nathann



---

archive/issue_comments_164951.json:
```json
{
    "body": "Milestone sage-4.7.3 deleted",
    "created_at": "2011-11-03T16:14:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11958#issuecomment-164951",
    "user": "https://github.com/jdemeyer"
}
```

Milestone sage-4.7.3 deleted



---

archive/issue_comments_164952.json:
```json
{
    "body": "<a id='comment:5'></a>Attachment [trac_11958.patch](tarball://root/attachments/some-uuid/ticket11958/trac_11958.patch) by @nathanncohen created at 2012-02-10 19:58:31\n\nHere it is ! Sorry for the delay ! `:-)`\n\nNathann",
    "created_at": "2012-02-10T19:58:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11958#issuecomment-164952",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:5'></a>Attachment [trac_11958.patch](tarball://root/attachments/some-uuid/ticket11958/trac_11958.patch) by @nathanncohen created at 2012-02-10 19:58:31

Here it is ! Sorry for the delay ! `:-)`

Nathann



---

archive/issue_comments_164953.json:
```json
{
    "body": "Changing component from PLEASE CHANGE to linear programming.",
    "created_at": "2012-02-10T19:58:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11958#issuecomment-164953",
    "user": "https://github.com/nathanncohen"
}
```

Changing component from PLEASE CHANGE to linear programming.



---

archive/issue_events_031657.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2012-02-10T19:58:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11958",
    "milestone": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11958#event-31657"
}
```



---

archive/issue_comments_164954.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2012-02-10T19:58:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11958#issuecomment-164954",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_164955.json:
```json
{
    "body": "Changing keywords from \"\" to \"Cernay2012\".",
    "created_at": "2012-02-10T20:08:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11958#issuecomment-164955",
    "user": "https://github.com/nathanncohen"
}
```

Changing keywords from "" to "Cernay2012".



---

archive/issue_comments_164956.json:
```json
{
    "body": "<a id='comment:7'></a>Hello,\n\nI'm able to install correctly the patch on sage-5.0.beta9, and it is very useful when you want to use cplex 12.4.\n\nIt took me some time to understand that I add to type `sage -docbuild thematic_tutorials html` for building the doc.\n\nThanks for the patch!\n\nDavid.",
    "created_at": "2012-03-23T19:37:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11958#issuecomment-164956",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:7'></a>Hello,

I'm able to install correctly the patch on sage-5.0.beta9, and it is very useful when you want to use cplex 12.4.

It took me some time to understand that I add to type `sage -docbuild thematic_tutorials html` for building the doc.

Thanks for the patch!

David.



---

archive/issue_comments_164957.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2012-03-23T19:37:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11958#issuecomment-164957",
    "user": "https://github.com/dcoudert"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_164958.json:
```json
{
    "body": "Changing reviewer from \"\" to \"David Coudert\"",
    "created_at": "2012-03-23T19:37:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11958#issuecomment-164958",
    "user": "https://github.com/dcoudert"
}
```

Changing reviewer from "" to "David Coudert"



---

archive/issue_comments_164959.json:
```json
{
    "body": "Changing author from \"\" to \"Nathann Cohen\"",
    "created_at": "2012-03-29T14:58:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11958#issuecomment-164959",
    "user": "https://github.com/jdemeyer"
}
```

Changing author from "" to "Nathann Cohen"



---

archive/issue_comments_164960.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-5.0.beta12\"",
    "created_at": "2012-04-02T15:24:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11958#issuecomment-164960",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-5.0.beta12"



---

archive/issue_events_031658.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-04-02T15:24:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/11958",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11958#event-31658"
}
```



---

archive/issue_comments_164961.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2012-04-02T15:24:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11958#issuecomment-164961",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed



---

archive/issue_comments_164962.json:
```json
{
    "body": "<a id='comment:10'></a>I hope it's not too selfish to add myself as an author, as it did take a (short) while to track down the problem and solution...",
    "created_at": "2012-04-02T20:27:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11958#issuecomment-164962",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:10'></a>I hope it's not too selfish to add myself as an author, as it did take a (short) while to track down the problem and solution...



---

archive/issue_comments_164963.json:
```json
{
    "body": "Changing author from \"Nathann Cohen\" to \"Nathann Cohen, Jason Grout\"",
    "created_at": "2012-04-02T20:27:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11958#issuecomment-164963",
    "user": "https://github.com/jasongrout"
}
```

Changing author from "Nathann Cohen" to "Nathann Cohen, Jason Grout"



---

archive/issue_comments_164964.json:
```json
{
    "body": "<a id='comment:11'></a>> I hope it's not too selfish to add myself as an author, as it did take a (short) while to track down the problem and solution...\n\n\nAhahah, be welcome in the \"authors\" field, there's room for everybody here so let us have a drink `:-)`\n\nAnyway there clearly was more work involved in finding the bug than in writing this patch !\n\nNathann",
    "created_at": "2012-04-04T18:33:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11958",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11958#issuecomment-164964",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:11'></a>> I hope it's not too selfish to add myself as an author, as it did take a (short) while to track down the problem and solution...


Ahahah, be welcome in the "authors" field, there's room for everybody here so let us have a drink `:-)`

Anyway there clearly was more work involved in finding the bug than in writing this patch !

Nathann
