# Issue 11858: Power series doesn't work with the tuple

archive/issues_011686.json:
```json
{
    "body": "\n```\nsage: PS = ZZ[['t']]\nsage: PS( [1,2,3,4] )\n1 + 2*t + 3*t^2 + 4*t^3\nsage: PS( (1,2,3,4) )\n---------------------------------------------------------------------------\nTypeError                                 Traceback (most recent call last)\n....\n\nTypeError: unable to coerce <type 'tuple'> to an integer\n```\n\n**Assignee:** @malb\n\n**CC:**  @burcin\n\n**Keywords:** sd34\n\n**Status:** new\n\nIssue created by migration from https://trac.sagemath.org/ticket/11858\n\n",
    "created_at": "2011-09-27T14:57:08Z",
    "labels": [
        "component: commutative algebra",
        "trivial",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.4",
    "title": "Power series doesn't work with the tuple",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/11858",
    "user": "https://trac.sagemath.org/admin/accounts/users/dangtuanhiep"
}
```

```
sage: PS = ZZ[['t']]
sage: PS( [1,2,3,4] )
1 + 2*t + 3*t^2 + 4*t^3
sage: PS( (1,2,3,4) )
---------------------------------------------------------------------------
TypeError                                 Traceback (most recent call last)
....

TypeError: unable to coerce <type 'tuple'> to an integer
```

**Assignee:** @malb

**CC:**  @burcin

**Keywords:** sd34

**Status:** new

Issue created by migration from https://trac.sagemath.org/ticket/11858





---

archive/issue_comments_155666.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-sage: PS = ZZ[This is the Trac macro *'t'* that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#'t'-macro)\n+[This is the Trac macro *Image* that was inherited from the migration called with arguments (sage: PS = ZZ[['t')](https://trac.sagemath.org/wiki/WikiMacros#Image-macro)\n sage: PS( [1,2,3,4] )\n 1 + 2*t + 3*t^2 + 4*t^3\n sage: PS( (1,2,3,4) )\n@@ -8,4 +8,4 @@\n ....\n \n TypeError: unable to coerce <type 'tuple'> to an integer\n-\n+)]]\n``````\n",
    "created_at": "2011-09-27T15:07:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11858#issuecomment-155666",
    "user": "https://trac.sagemath.org/admin/accounts/users/dangtuanhiep"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-sage: PS = ZZ[This is the Trac macro *'t'* that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#'t'-macro)
+[This is the Trac macro *Image* that was inherited from the migration called with arguments (sage: PS = ZZ[['t')](https://trac.sagemath.org/wiki/WikiMacros#Image-macro)
 sage: PS( [1,2,3,4] )
 1 + 2*t + 3*t^2 + 4*t^3
 sage: PS( (1,2,3,4) )
@@ -8,4 +8,4 @@
 ....
 
 TypeError: unable to coerce <type 'tuple'> to an integer
-
+)]]
``````




---

archive/issue_comments_155667.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,11 +1,12 @@\n-[This is the Trac macro *Image* that was inherited from the migration called with arguments (sage: PS = ZZ[['t')](https://trac.sagemath.org/wiki/WikiMacros#Image-macro)\n+\n+```\n+sage: PS = ZZ[['t']]\n sage: PS( [1,2,3,4] )\n 1 + 2*t + 3*t^2 + 4*t^3\n sage: PS( (1,2,3,4) )\n-\n----\n+---------------------------------------------------------------------------\n TypeError                                 Traceback (most recent call last)\n ....\n \n TypeError: unable to coerce <type 'tuple'> to an integer\n-)]]\n+```\n``````\n",
    "created_at": "2011-09-27T15:10:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11858#issuecomment-155667",
    "user": "https://trac.sagemath.org/admin/accounts/users/dangtuanhiep"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,11 +1,12 @@
-[This is the Trac macro *Image* that was inherited from the migration called with arguments (sage: PS = ZZ[['t')](https://trac.sagemath.org/wiki/WikiMacros#Image-macro)
+
+```
+sage: PS = ZZ[['t']]
 sage: PS( [1,2,3,4] )
 1 + 2*t + 3*t^2 + 4*t^3
 sage: PS( (1,2,3,4) )
-
----
+---------------------------------------------------------------------------
 TypeError                                 Traceback (most recent call last)
 ....
 
 TypeError: unable to coerce <type 'tuple'> to an integer
-)]]
+```
``````




---

archive/issue_comments_155668.json:
```json
{
    "body": "**Changing author** from \"Hiep Dang, Burcin Erocal\" to \"\".",
    "created_at": "2011-09-27T15:14:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11858#issuecomment-155668",
    "user": "https://trac.sagemath.org/admin/accounts/users/dangtuanhiep"
}
```

**Changing author** from "Hiep Dang, Burcin Erocal" to "".



---

archive/issue_events_037178.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/11858",
    "rename": {
        "from": "Power series",
        "to": "Power series doesn't work with the tuple"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11858#event-37178"
}
```



---

archive/issue_events_037179.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11858",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11858#event-37179"
}
```



---

archive/issue_events_037180.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11858",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11858#event-37180"
}
```



---

archive/issue_events_037181.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11858",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11858#event-37181"
}
```



---

archive/issue_events_037182.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11858",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11858#event-37182"
}
```



---

archive/issue_events_037183.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11858",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11858#event-37183"
}
```



---

archive/issue_events_037184.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11858",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11858#event-37184"
}
```



---

archive/issue_events_037185.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11858",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11858#event-37185"
}
```
