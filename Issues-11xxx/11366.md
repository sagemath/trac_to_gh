# Issue 11366: Incorrect documentation for set difference

archive/issues_011194.json:
```json
{
    "assignees": [],
    "body": "See [this documentation](http://www.sagemath.org/doc/reference/sage/sets/set.html#sage.sets.set.Set_object.difference), where the definition for set difference and set intersection is the same. \n\nThe actual doctests are correct, I think, it's just some sloppy copying or something in the definition.  See [enumerated sets](http://www.sagemath.org/doc/reference/sage/sets/set.html#sage.sets.set.Set_object_enumerated.difference) for what seems to be correct.\n\nAssignee: **mvngu**\n\nReviewer: **Julian Rueth**\n\nAuthor: **Travis Scrimshaw**\n\nMerged: **sage-5.6.beta1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11366_\n\n",
    "closed_at": "2012-12-21T09:30:25Z",
    "created_at": "2011-05-21T02:55:58Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Incorrect documentation for set difference",
    "type": "issue",
    "updated_at": "2012-12-21T09:30:25Z",
    "url": "https://github.com/sagemath/sage/issues/11366",
    "user": "https://github.com/kcrisman"
}
```
See [this documentation](http://www.sagemath.org/doc/reference/sage/sets/set.html#sage.sets.set.Set_object.difference), where the definition for set difference and set intersection is the same. 

The actual doctests are correct, I think, it's just some sloppy copying or something in the definition.  See [enumerated sets](http://www.sagemath.org/doc/reference/sage/sets/set.html#sage.sets.set.Set_object_enumerated.difference) for what seems to be correct.

Assignee: **mvngu**

Reviewer: **Julian Rueth**

Author: **Travis Scrimshaw**

Merged: **sage-5.6.beta1**

_Issue created by migration from https://trac.sagemath.org/ticket/11366_





---

archive/issue_events_137974.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-05-21T02:55:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11366",
    "milestone_number": null,
    "milestone_title": "sage-5.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11366#event-137974"
}
```



---

archive/issue_events_137975.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-05-21T02:55:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11366",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11366#event-137975"
}
```



---

archive/issue_events_137976.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-05-21T02:55:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11366",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11366#event-137976"
}
```



---

archive/issue_events_137977.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-05-21T02:55:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11366",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11366#event-137977"
}
```



---

archive/issue_events_137978.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-05-21T02:56:52Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/11366",
    "title_is": "Incorrect documentation for set difference",
    "title_was": "Incorrect documentation for enumerated set difference",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11366#event-137978"
}
```



---

archive/issue_comments_116901.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n See [this documentation](http://www.sagemath.org/doc/reference/sage/sets/set.html#sage.sets.set.Set_object.difference), where the definition for set difference and set intersection is the same. \n \n-The actual doctests are correct, I think, it's just some sloppy copying or something in the definition.\n+The actual doctests are correct, I think, it's just some sloppy copying or something in the definition.  See [enumerated sets](http://www.sagemath.org/doc/reference/sage/sets/set.html#sage.sets.set.Set_object_enumerated.difference) for what seems to be correct.\n``````\n",
    "created_at": "2011-05-21T02:56:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11366",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11366#issuecomment-116901",
    "user": "https://github.com/kcrisman"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 See [this documentation](http://www.sagemath.org/doc/reference/sage/sets/set.html#sage.sets.set.Set_object.difference), where the definition for set difference and set intersection is the same. 
 
-The actual doctests are correct, I think, it's just some sloppy copying or something in the definition.
+The actual doctests are correct, I think, it's just some sloppy copying or something in the definition.  See [enumerated sets](http://www.sagemath.org/doc/reference/sage/sets/set.html#sage.sets.set.Set_object_enumerated.difference) for what seems to be correct.
``````




---

archive/issue_comments_116902.json:
```json
{
    "body": "Author: **Travis Scrimshaw**",
    "created_at": "2012-11-17T07:50:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11366",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11366#issuecomment-116902",
    "user": "https://github.com/tscrim"
}
```

Author: **Travis Scrimshaw**



---

archive/issue_events_137979.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2012-11-17T07:50:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11366",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11366#event-137979"
}
```



---

archive/issue_comments_116903.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nMade the quick change. Easy to review.",
    "created_at": "2012-11-17T07:50:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11366",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11366#issuecomment-116903",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:2'>Comment 2:</a>
Made the quick change. Easy to review.



---

archive/issue_events_137980.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2012-11-22T13:36:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11366",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11366#event-137980"
}
```



---

archive/issue_events_137981.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2012-11-22T13:36:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11366",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11366#event-137981"
}
```



---

archive/issue_comments_116904.json:
```json
{
    "body": "Reviewer: **Julian Rueth**",
    "created_at": "2012-11-22T13:36:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11366",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11366#issuecomment-116904",
    "user": "https://github.com/saraedum"
}
```

Reviewer: **Julian Rueth**



---

archive/issue_comments_116905.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nA very minor problem, but I believe that the subject line of your patch should read: `Trac #11366: Doc change for set difference in Set.`.",
    "created_at": "2012-11-22T13:36:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11366",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11366#issuecomment-116905",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:3'>Comment 3:</a>
A very minor problem, but I believe that the subject line of your patch should read: `Trac #11366: Doc change for set difference in Set.`.



---

archive/issue_comments_116906.json:
```json
{
    "body": "Changes to commit message",
    "created_at": "2012-11-22T16:52:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11366",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11366#issuecomment-116906",
    "user": "https://github.com/tscrim"
}
```

Changes to commit message



---

archive/issue_events_137982.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2012-11-22T16:53:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11366",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11366#event-137982"
}
```



---

archive/issue_events_137983.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2012-11-22T16:53:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11366",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11366#event-137983"
}
```



---

archive/issue_comments_116907.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nAttachment: **[trac_11366-set_documentation-ts.patch.gz](https://github.com/sagemath/sage/files/ticket11366/trac_11366-set_documentation-ts.patch.gz)**\n\nChanged.",
    "created_at": "2012-11-22T16:53:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11366",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11366#issuecomment-116907",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:4'>Comment 4:</a>
Attachment: **[trac_11366-set_documentation-ts.patch.gz](https://github.com/sagemath/sage/files/ticket11366/trac_11366-set_documentation-ts.patch.gz)**

Changed.



---

archive/issue_events_137984.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2012-12-14T17:13:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11366",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11366#event-137984"
}
```



---

archive/issue_events_137985.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2012-12-14T17:13:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11366",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11366#event-137985"
}
```



---

archive/issue_comments_116908.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nThank you for the review.",
    "created_at": "2012-12-16T00:43:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11366",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11366#issuecomment-116908",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:6'>Comment 6:</a>
Thank you for the review.



---

archive/issue_comments_116909.json:
```json
{
    "body": "Merged: **sage-5.6.beta1**",
    "created_at": "2012-12-21T09:30:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11366",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11366#issuecomment-116909",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.6.beta1**



---

archive/issue_events_137986.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-12-21T09:30:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11366",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11366#event-137986"
}
```



---

archive/issue_events_137987.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-12-21T09:30:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11366",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11366#event-137987"
}
```
