# Issue 11195: LU decomposition for exact matrices

archive/issues_011025.json:
```json
{
    "assignees": [],
    "body": "This was meant for instructional purposes, but for some exact fields (eg finite fields) it is twice as fast as echelon form when Cythonized, as theory would predict.\n\nSo it could be the basis of a fast alternative algorithm for `solve_right` and friends.\n\nAssignee: **@jasongrout, @williamstein**\n\nReviewer: **Rob Beezer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11195_\n\n",
    "closed_at": "2011-04-27T07:59:44Z",
    "created_at": "2011-04-14T05:05:41Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "LU decomposition for exact matrices",
    "type": "issue",
    "updated_at": "2011-05-01T12:18:47Z",
    "url": "https://github.com/sagemath/sage/issues/11195",
    "user": "https://github.com/rbeezer"
}
```
This was meant for instructional purposes, but for some exact fields (eg finite fields) it is twice as fast as echelon form when Cythonized, as theory would predict.

So it could be the basis of a fast alternative algorithm for `solve_right` and friends.

Assignee: **@jasongrout, @williamstein**

Reviewer: **Rob Beezer**

_Issue created by migration from https://trac.sagemath.org/ticket/11195_





---

archive/issue_events_135292.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-04-14T05:05:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11195",
    "milestone_number": null,
    "milestone_title": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11195#event-135292"
}
```



---

archive/issue_events_135293.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-04-14T05:05:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11195",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11195#event-135293"
}
```



---

archive/issue_events_135294.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-04-14T05:05:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11195",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11195#event-135294"
}
```



---

archive/issue_events_135295.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2011-04-14T05:05:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11195",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11195#event-135295"
}
```



---

archive/issue_comments_113796.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nAttachment: **[trac_11195-LU-decomposition-exact.patch.gz](https://github.com/sagemath/sage/files/ticket11195/trac_11195-LU-decomposition-exact.patch.gz)**\n\nPatch is here for safe-keeping, but not ready for review.  It should run just fine, needs caching perhaps, and needs docstring and doctests.",
    "created_at": "2011-04-14T05:09:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11195",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11195#issuecomment-113796",
    "user": "https://github.com/rbeezer"
}
```

<div id="comment:1" align="right">Comment 1</div>

Attachment: **[trac_11195-LU-decomposition-exact.patch.gz](https://github.com/sagemath/sage/files/ticket11195/trac_11195-LU-decomposition-exact.patch.gz)**

Patch is here for safe-keeping, but not ready for review.  It should run just fine, needs caching perhaps, and needs docstring and doctests.



---

archive/issue_comments_113797.json:
```json
{
    "body": "Author: **Rob Beezer**",
    "created_at": "2011-04-14T05:09:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11195",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11195#issuecomment-113797",
    "user": "https://github.com/rbeezer"
}
```

Author: **Rob Beezer**



---

archive/issue_comments_113798.json:
```json
{
    "body": "Changed author from **Rob Beezer** to none",
    "created_at": "2011-04-27T07:59:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11195",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11195#issuecomment-113798",
    "user": "https://github.com/jdemeyer"
}
```

Changed author from **Rob Beezer** to none



---

archive/issue_comments_113799.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nDuplicate of #11259.",
    "created_at": "2011-04-27T07:59:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11195",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11195#issuecomment-113799",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

Duplicate of #11259.



---

archive/issue_events_135296.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-04-27T07:59:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11195",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11195#event-135296"
}
```



---

archive/issue_events_135297.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-04-27T07:59:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11195",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11195#event-135297"
}
```



---

archive/issue_events_135298.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-04-27T07:59:44Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11195",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11195#event-135298"
}
```



---

archive/issue_comments_113800.json:
```json
{
    "body": "Reviewer: **Rob Beezer**",
    "created_at": "2011-04-27T07:59:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11195",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11195#issuecomment-113800",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Rob Beezer**



---

archive/issue_events_135299.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2011-05-01T12:18:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11195",
    "milestone_number": null,
    "milestone_title": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11195#event-135299"
}
```
