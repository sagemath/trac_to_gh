# Issue 11269: Correct docstring formatting of dir_with_other_class

archive/issues_011097.json:
```json
{
    "assignees": [],
    "body": "The docstring of dir_with_other_class lacks a double colon, and it contains backslashes, so that it ought to be a raw string.\n\nThe trivial patch takes care of it.\n\nApply [attachment: trac11269-docformat_dir_with_other_class_rebased.patch](https://github.com/sagemath/sage/files/ticket11269/trac11269-docformat_dir_with_other_class_rebased.patch.gz) on top of sage-4.7.1.alpha1\n\nAssignee: **@sagetrac-mvngu**\n\nAuthor: **Simon King**\n\nReviewer: **Mariah Lenox**\n\nMerged: **sage-4.7.1.alpha2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11269_\n\n",
    "closed_at": "2011-05-31T17:06:16Z",
    "created_at": "2011-04-28T12:27:37Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.7.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Correct docstring formatting of dir_with_other_class",
    "type": "issue",
    "updated_at": "2011-05-31T17:06:16Z",
    "url": "https://github.com/sagemath/sage/issues/11269",
    "user": "https://github.com/simon-king-jena"
}
```
The docstring of dir_with_other_class lacks a double colon, and it contains backslashes, so that it ought to be a raw string.

The trivial patch takes care of it.

Apply [attachment: trac11269-docformat_dir_with_other_class_rebased.patch](https://github.com/sagemath/sage/files/ticket11269/trac11269-docformat_dir_with_other_class_rebased.patch.gz) on top of sage-4.7.1.alpha1

Assignee: **@sagetrac-mvngu**

Author: **Simon King**

Reviewer: **Mariah Lenox**

Merged: **sage-4.7.1.alpha2**

_Issue created by migration from https://trac.sagemath.org/ticket/11269_





---

archive/issue_events_136369.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2011-04-28T12:27:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11269",
    "milestone_number": null,
    "milestone_title": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11269#event-136369"
}
```



---

archive/issue_events_136370.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2011-04-28T12:27:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11269",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11269#event-136370"
}
```



---

archive/issue_events_136371.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2011-04-28T12:27:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11269",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11269#event-136371"
}
```



---

archive/issue_events_136372.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2011-04-28T12:27:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11269",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11269#event-136372"
}
```



---

archive/issue_comments_115053.json:
```json
{
    "body": "Correct the format of one doc string",
    "created_at": "2011-04-28T12:29:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11269",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11269#issuecomment-115053",
    "user": "https://github.com/simon-king-jena"
}
```

Correct the format of one doc string



---

archive/issue_events_136373.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2011-04-28T12:29:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11269",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11269#event-136373"
}
```



---

archive/issue_comments_115054.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nAttachment: **[trac11269-docformat_dir_with_other_class.patch.gz](https://github.com/sagemath/sage/files/ticket11269/trac11269-docformat_dir_with_other_class.patch.gz)**",
    "created_at": "2011-04-28T12:29:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11269",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11269#issuecomment-115054",
    "user": "https://github.com/simon-king-jena"
}
```

<div id="comment:1" align="right">Comment 1</div>

Attachment: **[trac11269-docformat_dir_with_other_class.patch.gz](https://github.com/sagemath/sage/files/ticket11269/trac11269-docformat_dir_with_other_class.patch.gz)**



---

archive/issue_comments_115055.json:
```json
{
    "body": "Reviewer: **Mariah Lenox**",
    "created_at": "2011-05-19T17:28:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11269",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11269#issuecomment-115055",
    "user": "https://github.com/sagetrac-mariah"
}
```

Reviewer: **Mariah Lenox**



---

archive/issue_events_136374.json:
```json
{
    "actor": "https://github.com/sagetrac-mariah",
    "created_at": "2011-05-19T17:28:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11269",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11269#event-136374"
}
```



---

archive/issue_events_136375.json:
```json
{
    "actor": "https://github.com/sagetrac-mariah",
    "created_at": "2011-05-19T17:28:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11269",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11269#event-136375"
}
```



---

archive/issue_comments_115056.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nApplied patch, regenerated docs, and verified that the patch\nnow correctly shows the docstring.  Positive review.",
    "created_at": "2011-05-19T17:28:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11269",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11269#issuecomment-115056",
    "user": "https://github.com/sagetrac-mariah"
}
```

<div id="comment:2" align="right">Comment 2</div>

Applied patch, regenerated docs, and verified that the patch
now correctly shows the docstring.  Positive review.



---

archive/issue_comments_115057.json:
```json
{
    "body": "Attachment: **[trac11269-docformat_dir_with_other_class_rebased.patch.gz](https://github.com/sagemath/sage/files/ticket11269/trac11269-docformat_dir_with_other_class_rebased.patch.gz)**",
    "created_at": "2011-05-31T13:00:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11269",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11269#issuecomment-115057",
    "user": "https://github.com/jdemeyer"
}
```

Attachment: **[trac11269-docformat_dir_with_other_class_rebased.patch.gz](https://github.com/sagemath/sage/files/ticket11269/trac11269-docformat_dir_with_other_class_rebased.patch.gz)**



---

archive/issue_comments_115058.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,5 @@\n The docstring of dir_with_other_class lacks a double colon, and it contains backslashes, so that it ought to be a raw string.\n \n The trivial patch takes care of it.\n+\n+Apply [attachment: trac11269-docformat_dir_with_other_class_rebased.patch](https://github.com/sagemath/sage/files/ticket11269/trac11269-docformat_dir_with_other_class_rebased.patch.gz) on top of sage-4.7.1.alpha1\n``````\n",
    "created_at": "2011-05-31T13:01:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11269",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11269#issuecomment-115058",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,5 @@
 The docstring of dir_with_other_class lacks a double colon, and it contains backslashes, so that it ought to be a raw string.
 
 The trivial patch takes care of it.
+
+Apply [attachment: trac11269-docformat_dir_with_other_class_rebased.patch](https://github.com/sagemath/sage/files/ticket11269/trac11269-docformat_dir_with_other_class_rebased.patch.gz) on top of sage-4.7.1.alpha1
``````




---

archive/issue_comments_115059.json:
```json
{
    "body": "Merged: **sage-4.7.1.alpha2**",
    "created_at": "2011-05-31T17:06:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11269",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11269#issuecomment-115059",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-4.7.1.alpha2**



---

archive/issue_events_136376.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-31T17:06:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11269",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11269#event-136376"
}
```



---

archive/issue_events_136377.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-05-31T17:06:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11269",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11269#event-136377"
}
```
