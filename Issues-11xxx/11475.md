...
```json
{
    "body": "The [link(s)](http://wstein.org/home/ohanar/papers/abelian_prime_counting/main.pdf) to [your paper](http://wstein.org/home/ohanar/senior_thesis/main.pdf) is (currently) dead. (I wouldn't call it 'main' btw. ;-) )\n\nHaven't yet looked a the rest...",
...
Attachment [trac_11475v2.patch](tarball://root/attachments/some-uuid/ticket11475/trac_11475v2.patch) by @nexttime created at 2011-06-15 17:14:15
...
```json
{
    "body": "Apply only trac_11475v2.patch",
...
```json
{
    "body": "Apply only trac_11475v3.patch",
...
```json
{
    "body": "Unfortunate, but kind of expected. It'll probably take a day or two for me to set up a 32-bit vm to try to debug the issue (it isn't blatantly obvious to me what is going wrong).",
...
```json
{
    "body": "Attachment [trac_11475v6.patch](tarball://root/attachments/some-uuid/ticket11475/trac_11475v6.patch) by @ohanar created at 2011-06-19 08:44:48",
    "created_at": "2011-06-19T08:44:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11475",
    "type": "issue_comment",
...
Attachment [trac_11475v6.patch](tarball://root/attachments/some-uuid/ticket11475/trac_11475v6.patch) by @ohanar created at 2011-06-19 08:44:48
...
```json
{
    "body": "Hopefully that was the only issue with 32-bit systems. Basically gmp wants a 32-bit int for mpz_set_ui on 32-bit systems, so I gave it what it wanted (this was also causing the issue with the `__sieve` function, since I use the `mpz_sqrt` function to calculate the argument).\n\nI am currently doing some more thorough testing with large values on a 64-bit machine, so that we can be sure there isn't any more breakage on the stable platform.",
...
```json
{
    "body": "Apply only trac_11475v6.patch",
...
```json
{
    "body": "Apply only trac_11475v8.patch",
...
```json
{
    "body": "Apply trac_11475mk2.patch\n\n(for patchbot)",
...
```json
{
    "body": "Ok, both issues should be resolved -- it should now work properly in the symbolic ring.",
...
```json
{
    "body": "Patchbot: Apply trac_11475mk2.patch and trac_11475-reviewer.patch.",
...
```json
{
    "body": "William pointed out that this didn't fix #6876 quite how we want, so just fixing the typo.",
...
```json
{
    "body": "On hawk (OpenSolaris i386), it actually Segmentation Faults.",
...
```json
{
    "body": "patchbot: apply trac_11475mk2.patch trac_11475-reviewer.patch",
...
```json
{
    "body": "Jeroen: now it passes on arando. Can you check on hawk?",
    "created_at": "2012-06-16T01:59:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11475",
    "type": "issue_comment",
...
```json
{
    "body": "Why does this ticket make random whitespace changes to `sage/functions/all.py`? This is total BS. This just breaks patches on trac and wastes other people's time. For example \n#11143 doesn't apply any more.",
...
```json
{
    "body": "Replying to [comment:74 ohanar]:\n> Do either of these cause a segfault?:\n> \n> ```\n> #!python\n> sage: prime_pi(2**15)\n> 3512\n> sage: prime_pi(2**10-1)\n> 172\n> ```\n\nNo.",
...
```json
{
    "body": "Attachment [trac_11475mk2.patch](tarball://root/attachments/some-uuid/ticket11475/trac_11475mk2.patch) by @ohanar created at 2012-10-07 05:49:39\n\nOk, OSX issue should now be fixed (all tests pass for me on bsd.math now).",
...
Attachment [trac_11475mk2.patch](tarball://root/attachments/some-uuid/ticket11475/trac_11475mk2.patch) by @ohanar created at 2012-10-07 05:49:39
...
```json
{
    "body": "this patch has bit-rotted, I'll upload a git branch in a moment",
