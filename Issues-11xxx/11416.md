# Issue 11416: binomial does not work on floats

archive/issues_011244.json:
```json
{
    "assignees": [],
    "body": "sage: binomial(1.3r,2)                   \n\n---\nAttributeError                            Traceback (most recent call last)\n\n/opt/sage-4.7-linux-32bit-ubuntu_10.04_lts-i686-Linux/<ipython console> in <module>()\n\n/opt/sage-4.7-linux-32bit-ubuntu_10.04_lts-i686-Linux/local/lib/python2.6/site-packages/sage/rings/arith.pyc in binomial(x, m, **kwds)\n   2978     if isinstance(x, (float, sage.rings.real_mpfr.RealNumber,\n   2979                       sage.rings.real_mpfr.RealLiteral)):\n-> 2980         P = x.parent()\n   2981         if m < 0:\n   2982             return P(0)\n\nAttributeError: 'float' object has no attribute 'parent'\n\n\nAssignee: **@aghitza**\n\nReviewer: **Douglas McNeil, Karl-Dieter Crisman**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/11416_\n\n",
    "closed_at": "2011-06-07T09:09:01Z",
    "created_at": "2011-05-31T20:36:14Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "binomial does not work on floats",
    "type": "issue",
    "updated_at": "2011-07-28T10:12:44Z",
    "url": "https://github.com/sagemath/sage/issues/11416",
    "user": "https://github.com/sagetrac-Henryk-Trappmann"
}
```
sage: binomial(1.3r,2)                   

---
AttributeError                            Traceback (most recent call last)

/opt/sage-4.7-linux-32bit-ubuntu_10.04_lts-i686-Linux/<ipython console> in <module>()

/opt/sage-4.7-linux-32bit-ubuntu_10.04_lts-i686-Linux/local/lib/python2.6/site-packages/sage/rings/arith.pyc in binomial(x, m, **kwds)
   2978     if isinstance(x, (float, sage.rings.real_mpfr.RealNumber,
   2979                       sage.rings.real_mpfr.RealLiteral)):
-> 2980         P = x.parent()
   2981         if m < 0:
   2982             return P(0)

AttributeError: 'float' object has no attribute 'parent'


Assignee: **@aghitza**

Reviewer: **Douglas McNeil, Karl-Dieter Crisman**

_Issue created by migration from https://trac.sagemath.org/ticket/11416_





---

archive/issue_events_138688.json:
```json
{
    "actor": "https://github.com/sagetrac-Henryk-Trappmann",
    "created_at": "2011-05-31T20:36:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/11416",
    "milestone_number": null,
    "milestone_title": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11416#event-138688"
}
```



---

archive/issue_events_138689.json:
```json
{
    "actor": "https://github.com/sagetrac-Henryk-Trappmann",
    "created_at": "2011-05-31T20:36:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11416",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "component: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11416#event-138689"
}
```



---

archive/issue_events_138690.json:
```json
{
    "actor": "https://github.com/sagetrac-Henryk-Trappmann",
    "created_at": "2011-05-31T20:36:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11416",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11416#event-138690"
}
```



---

archive/issue_events_138691.json:
```json
{
    "actor": "https://github.com/sagetrac-Henryk-Trappmann",
    "created_at": "2011-05-31T20:36:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11416",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11416#event-138691"
}
```



---

archive/issue_comments_117674.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nI think this was fixed by #9633 in sage-4.7.1.alpha0.",
    "created_at": "2011-06-01T08:49:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11416",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11416#issuecomment-117674",
    "user": "https://github.com/sagetrac-dsm"
}
```

<a id='comment:1'>Comment 1:</a>
I think this was fixed by #9633 in sage-4.7.1.alpha0.



---

archive/issue_comments_117675.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nReplying to [dsm](#comment%3A1):\n> I think this was fixed by #9633 in sage-4.7.1.alpha0.\n> \n\nYou are correct. \n\nAnd considering the same person reported both tickets... :)  Well, I've done that as well.",
    "created_at": "2011-06-02T03:46:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11416",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11416#issuecomment-117675",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:2'>Comment 2:</a>
Replying to [dsm](#comment%3A1):
> I think this was fixed by #9633 in sage-4.7.1.alpha0.
> 

You are correct. 

And considering the same person reported both tickets... :)  Well, I've done that as well.



---

archive/issue_comments_117676.json:
```json
{
    "body": "Reviewer: **Douglas S. McNeil, Karl-Dieter Crisman**",
    "created_at": "2011-06-02T03:46:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11416",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11416#issuecomment-117676",
    "user": "https://github.com/kcrisman"
}
```

Reviewer: **Douglas S. McNeil, Karl-Dieter Crisman**



---

archive/issue_events_138692.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-06-02T03:46:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11416",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11416#event-138692"
}
```



---

archive/issue_events_138693.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-06-02T03:46:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11416",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11416#event-138693"
}
```



---

archive/issue_events_138694.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2011-06-02T03:46:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11416",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11416#event-138694"
}
```



---

archive/issue_events_138695.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-07T09:09:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11416",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11416#event-138695"
}
```



---

archive/issue_events_138696.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-07T09:09:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/11416",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11416#event-138696"
}
```



---

archive/issue_events_138697.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-07T09:09:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/11416",
    "milestone_number": null,
    "milestone_title": "sage-4.7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11416#event-138697"
}
```



---

archive/issue_events_138698.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-07T09:09:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/11416",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11416#event-138698"
}
```



---

archive/issue_events_138699.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-07T09:09:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/11416",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/11416#event-138699"
}
```



---

archive/issue_comments_117677.json:
```json
{
    "body": "Changed reviewer from **Douglas S. McNeil, Karl-Dieter Crisman** to **Douglas McNeil, Karl-Dieter Crisman**",
    "created_at": "2011-07-28T10:12:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/11416",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/11416#issuecomment-117677",
    "user": "https://github.com/jdemeyer"
}
```

Changed reviewer from **Douglas S. McNeil, Karl-Dieter Crisman** to **Douglas McNeil, Karl-Dieter Crisman**
