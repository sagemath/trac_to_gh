# Issue 11553: Generate random elements of the algebraic field

archive/issues_011553.json:
```json
{
    "body": "Assignee: AlexGhitza\n\nCC:  spice\n\nRandom elements of QQbar default to the method implemented for the integers.  This patch creates algebraic numbers via roots of random polynomials with integer coefficients.\n\nImplementation notes:\n1.  Degree 2 is the default so that some complex numbers are generated.\n2.  Might create constant polynomials, so a monic term (of random degree) may be added.\n3.  Could be some efficiencies to be gained by not creating all the roots and choosing just one, but this should be a good start.\n4.  Real goal was random vectors over `QQbar`.  A doctest demonstrates how control of the integer coefficients can be passed from `random_vector()` to the `QQbar.random_element()` to `ZZ.random_element().\n\nIssue created by migration from https://trac.sagemath.org/ticket/11725\n\n",
    "created_at": "2011-08-23T23:47:05Z",
    "labels": [
        "algebra",
        "minor",
        "enhancement"
    ],
    "title": "Generate random elements of the algebraic field",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/11553",
    "user": "rbeezer"
}
```
Assignee: AlexGhitza

CC:  spice

Random elements of QQbar default to the method implemented for the integers.  This patch creates algebraic numbers via roots of random polynomials with integer coefficients.

Implementation notes:
1.  Degree 2 is the default so that some complex numbers are generated.
2.  Might create constant polynomials, so a monic term (of random degree) may be added.
3.  Could be some efficiencies to be gained by not creating all the roots and choosing just one, but this should be a good start.
4.  Real goal was random vectors over `QQbar`.  A doctest demonstrates how control of the integer coefficients can be passed from `random_vector()` to the `QQbar.random_element()` to `ZZ.random_element().

Issue created by migration from https://trac.sagemath.org/ticket/11725





---

archive/issue_comments_129895.json:
```json
{
    "body": "Attachment [trac_11725-random-algebraic-numbers.patch](tarball://root/attachments/some-uuid/ticket11725/trac_11725-random-algebraic-numbers.patch) by rbeezer created at 2011-08-23 23:56:12",
    "created_at": "2011-08-23T23:56:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11553#issuecomment-129895",
    "user": "rbeezer"
}
```

Attachment [trac_11725-random-algebraic-numbers.patch](tarball://root/attachments/some-uuid/ticket11725/trac_11725-random-algebraic-numbers.patch) by rbeezer created at 2011-08-23 23:56:12



---

archive/issue_comments_129896.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2011-08-23T23:56:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11553#issuecomment-129896",
    "user": "rbeezer"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_129897.json:
```json
{
    "body": "Replaces previous patch",
    "created_at": "2011-08-24T21:26:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11553#issuecomment-129897",
    "user": "spice"
}
```

Replaces previous patch



---

archive/issue_comments_129898.json:
```json
{
    "body": "Attachment [trac_11725-random-algebraic-numbers2.patch](tarball://root/attachments/some-uuid/ticket11725/trac_11725-random-algebraic-numbers2.patch) by spice created at 2011-08-24 21:30:20\n\nSeems mostly fine to me. It would be easy to get bogged down as to what the default behaviour of `QQbar.random_element()` should be, but as implemented here this seems a reasonable first step. I did a test and found that with default parameters `QQbar.random_element()` returns zero about 15% of the time, nonzero real numbers about 60% of the time, and nonzero imaginary numbers about 7% of the time:\n\n```\nsage: L = []\nsage: for j in range(5000):\n....:     L.append(QQbar.random_element())\n....:     \nsage: RDF(L.count(0))/len(L)\n0.133\nsage: C = [r.imag() == 0 and r != 0 for r in L]\nsage: RDF(C.count(True))/len(C)\n0.6194\nsage: D = [r.real() == 0 and r != 0 for r in L]\nsage: RDF(D.count(True))/len(D)\n0.066\n```\n\n\nThe doctests all work and illustrate the new behaviour well. I fixed a few spelling errors, one grammatical error and added extra info in one other place:\n\n```\n- ``poly_degree`` - default: 2 - degree of the random \n   polynomial over the integers that the algebraic number \n   is a root of.\n```\n\nbecomes\n\n```\n- ``poly_degree`` - default: 2 - degree of the random                                                     \n    polynomial over the integers of which the returned                                                      \n    algebraic number is a root.\n```\n\nAnd\n\n```\nFor example if we do not include zero as a possible \ncoefficient, there will never be a zero constant term, and \nthus never a zero root. ::\n```\n\nbecomes\n\n```\nFor example, current default behaviour of this method                                      \nreturns zero about 15% of the time; if we do not include zero as a                                        \npossible coefficient, there will never be a zero constant term, and                                       \nthus never a zero root. ::\n```\n\n\nI've uploaded the updated patch. These are trivial changes, but I suppose this should be re-reviewed before it can be given a positive review.",
    "created_at": "2011-08-24T21:30:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11553#issuecomment-129898",
    "user": "spice"
}
```

Attachment [trac_11725-random-algebraic-numbers2.patch](tarball://root/attachments/some-uuid/ticket11725/trac_11725-random-algebraic-numbers2.patch) by spice created at 2011-08-24 21:30:20

Seems mostly fine to me. It would be easy to get bogged down as to what the default behaviour of `QQbar.random_element()` should be, but as implemented here this seems a reasonable first step. I did a test and found that with default parameters `QQbar.random_element()` returns zero about 15% of the time, nonzero real numbers about 60% of the time, and nonzero imaginary numbers about 7% of the time:

```
sage: L = []
sage: for j in range(5000):
....:     L.append(QQbar.random_element())
....:     
sage: RDF(L.count(0))/len(L)
0.133
sage: C = [r.imag() == 0 and r != 0 for r in L]
sage: RDF(C.count(True))/len(C)
0.6194
sage: D = [r.real() == 0 and r != 0 for r in L]
sage: RDF(D.count(True))/len(D)
0.066
```


The doctests all work and illustrate the new behaviour well. I fixed a few spelling errors, one grammatical error and added extra info in one other place:

```
- ``poly_degree`` - default: 2 - degree of the random 
   polynomial over the integers that the algebraic number 
   is a root of.
```

becomes

```
- ``poly_degree`` - default: 2 - degree of the random                                                     
    polynomial over the integers of which the returned                                                      
    algebraic number is a root.
```

And

```
For example if we do not include zero as a possible 
coefficient, there will never be a zero constant term, and 
thus never a zero root. ::
```

becomes

```
For example, current default behaviour of this method                                      
returns zero about 15% of the time; if we do not include zero as a                                        
possible coefficient, there will never be a zero constant term, and                                       
thus never a zero root. ::
```


I've uploaded the updated patch. These are trivial changes, but I suppose this should be re-reviewed before it can be given a positive review.



---

archive/issue_comments_129899.json:
```json
{
    "body": "Changing keywords from \"\" to \"random, QQbar\".",
    "created_at": "2011-08-24T21:30:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11553#issuecomment-129899",
    "user": "spice"
}
```

Changing keywords from "" to "random, QQbar".



---

archive/issue_comments_129900.json:
```json
{
    "body": "Replaces previous patch",
    "created_at": "2011-08-24T23:24:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11553#issuecomment-129900",
    "user": "spice"
}
```

Replaces previous patch



---

archive/issue_comments_129901.json:
```json
{
    "body": "Attachment [trac_11725-random-algebraic-numbers3.patch](tarball://root/attachments/some-uuid/ticket11725/trac_11725-random-algebraic-numbers3.patch) by spice created at 2011-08-24 23:26:35\n\nNew patch uploaded in which the following if statement is removed (since it never triggers):\n\n```\nif len(roots) == 0: \n    print \"FUBAR\", p\n```\n",
    "created_at": "2011-08-24T23:26:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11553#issuecomment-129901",
    "user": "spice"
}
```

Attachment [trac_11725-random-algebraic-numbers3.patch](tarball://root/attachments/some-uuid/ticket11725/trac_11725-random-algebraic-numbers3.patch) by spice created at 2011-08-24 23:26:35

New patch uploaded in which the following if statement is removed (since it never triggers):

```
if len(roots) == 0: 
    print "FUBAR", p
```




---

archive/issue_comments_129902.json:
```json
{
    "body": "All the changes look good to me.  I'll let you flip this to positive review.\n\nThanks for your help with this one - it'll be good to have available.\n\nRob",
    "created_at": "2011-08-24T23:44:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11553#issuecomment-129902",
    "user": "rbeezer"
}
```

All the changes look good to me.  I'll let you flip this to positive review.

Thanks for your help with this one - it'll be good to have available.

Rob



---

archive/issue_comments_129903.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2011-08-24T23:55:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11553#issuecomment-129903",
    "user": "spice"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_129904.json:
```json
{
    "body": "No prob\n\n:-)\nSimon",
    "created_at": "2011-08-24T23:55:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11553#issuecomment-129904",
    "user": "spice"
}
```

No prob

:-)
Simon



---

archive/issue_comments_129905.json:
```json
{
    "body": "Changing keywords from \"random, QQbar\" to \"random, QQbar, sd32\".",
    "created_at": "2011-08-25T18:56:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11553#issuecomment-129905",
    "user": "rbeezer"
}
```

Changing keywords from "random, QQbar" to "random, QQbar, sd32".



---

archive/issue_comments_129906.json:
```json
{
    "body": "This is easily fixed:\n\n```\nsage -t -long -force_lib \"devel/sage/sage/rings/polynomial/multi_polynomial_element.py\"\n**********************************************************************\nFile \".../sage-4.7.2.alpha3-pre2/devel/sage/sage/rings/polynomial/multi_polynomial_element.py\", line 101:\n    sage: x + QQbar.random_element() # indirect doctest\nExpected:\n    x - 2\nGot:\n    x + 4\n**********************************************************************\n1 items had failures:\n   1 of   5 in __main__.example_2\n***Test Failed*** 1 failures.\n```\n\n\n\n\n\nI also get two Sphinx warnings for `qqbar.py`; as far as I can see (regarding the line numbers) these originate *from another patch*:\n\n```\n.../sage-4.7.2.alpha3-pre2/local/lib/python2.6/site-packages/sage/rings/qqbar.py:docstring of sage.rings.qqbar:476: (WARNING/2) Literal block expected; none found.\n\n.../sage-4.7.2.alpha3-pre2/local/lib/python2.6/site-packages/sage/rings/qqbar.py:docstring of sage.rings.qqbar.AlgebraicReal:5: (WARNING/2) Literal block expected; none found.\n\n```\n\n\nWould be nice if *you* could fix these *here* as well though; my \"patch queue\" for `qqbar.py` is:\n\n```\nchangeset:   16112:799c68fc8e9b\nuser:        Rob Beezer <beezer@ups.edu>\ndate:        Tue Aug 23 16:49:09 2011 -0700\nsummary:     11725: random elements of QQbar\n\nchangeset:   16084:2d91134795c5\nuser:        Simon King <simon.king@uni-jena.de>\ndate:        Tue Apr 19 10:12:21 2011 +0200\nsummary:     #9138: Use the category framework for rings.\n\nchangeset:   16075:9baeb151c9a7\nuser:        William Stein <wstein@gmail.com>\ndate:        Tue Aug 23 21:36:19 2011 -0700\nsummary:     trac 10981 -- buggy arithmetic in AA, and other issues\n\n...\n\n[from previous Sage releases]\n```\n\n\nSo apparently one of the two others (#10981, #9138) causes them, but you could (or should) base your patch(es) on these anyway.",
    "created_at": "2011-09-16T23:20:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11553#issuecomment-129906",
    "user": "leif"
}
```

This is easily fixed:

```
sage -t -long -force_lib "devel/sage/sage/rings/polynomial/multi_polynomial_element.py"
**********************************************************************
File ".../sage-4.7.2.alpha3-pre2/devel/sage/sage/rings/polynomial/multi_polynomial_element.py", line 101:
    sage: x + QQbar.random_element() # indirect doctest
Expected:
    x - 2
Got:
    x + 4
**********************************************************************
1 items had failures:
   1 of   5 in __main__.example_2
***Test Failed*** 1 failures.
```





I also get two Sphinx warnings for `qqbar.py`; as far as I can see (regarding the line numbers) these originate *from another patch*:

```
.../sage-4.7.2.alpha3-pre2/local/lib/python2.6/site-packages/sage/rings/qqbar.py:docstring of sage.rings.qqbar:476: (WARNING/2) Literal block expected; none found.

.../sage-4.7.2.alpha3-pre2/local/lib/python2.6/site-packages/sage/rings/qqbar.py:docstring of sage.rings.qqbar.AlgebraicReal:5: (WARNING/2) Literal block expected; none found.

```


Would be nice if *you* could fix these *here* as well though; my "patch queue" for `qqbar.py` is:

```
changeset:   16112:799c68fc8e9b
user:        Rob Beezer <beezer@ups.edu>
date:        Tue Aug 23 16:49:09 2011 -0700
summary:     11725: random elements of QQbar

changeset:   16084:2d91134795c5
user:        Simon King <simon.king@uni-jena.de>
date:        Tue Apr 19 10:12:21 2011 +0200
summary:     #9138: Use the category framework for rings.

changeset:   16075:9baeb151c9a7
user:        William Stein <wstein@gmail.com>
date:        Tue Aug 23 21:36:19 2011 -0700
summary:     trac 10981 -- buggy arithmetic in AA, and other issues

...

[from previous Sage releases]
```


So apparently one of the two others (#10981, #9138) causes them, but you could (or should) base your patch(es) on these anyway.



---

archive/issue_comments_129907.json:
```json
{
    "body": "Changing status from positive_review to needs_work.",
    "created_at": "2011-09-16T23:20:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11553#issuecomment-129907",
    "user": "leif"
}
```

Changing status from positive_review to needs_work.



---

archive/issue_comments_129908.json:
```json
{
    "body": "Replying to [comment:8 leif]:\n> I also get two Sphinx warnings for `qqbar.py`; as far as I can see (regarding the line numbers) these originate *from another patch*:\n\n```\n.../sage-4.7.2.alpha3-pre2/local/lib/python2.6/site-packages/sage/rings/qqbar.py:docstring of sage.rings.qqbar:476: (WARNING/2) Literal block expected; none found.\n\n.../sage-4.7.2.alpha3-pre2/local/lib/python2.6/site-packages/sage/rings/qqbar.py:docstring of sage.rings.qqbar.AlgebraicReal:5: (WARNING/2) Literal block expected; none found.\n\n```\n\n> [...]\n> So apparently one of the two others (#10981, #9138) causes them, but you could (or should) base your patch(es) on these anyway.\n\nAh, yes, [attachment:ticket:10981:trac_10981.patch the BDFL's patch] at #10981 causes these; the double colons after `TESTS` have to be changed to single ones twice.",
    "created_at": "2011-09-16T23:45:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11553#issuecomment-129908",
    "user": "leif"
}
```

Replying to [comment:8 leif]:
> I also get two Sphinx warnings for `qqbar.py`; as far as I can see (regarding the line numbers) these originate *from another patch*:

```
.../sage-4.7.2.alpha3-pre2/local/lib/python2.6/site-packages/sage/rings/qqbar.py:docstring of sage.rings.qqbar:476: (WARNING/2) Literal block expected; none found.

.../sage-4.7.2.alpha3-pre2/local/lib/python2.6/site-packages/sage/rings/qqbar.py:docstring of sage.rings.qqbar.AlgebraicReal:5: (WARNING/2) Literal block expected; none found.

```

> [...]
> So apparently one of the two others (#10981, #9138) causes them, but you could (or should) base your patch(es) on these anyway.

Ah, yes, [attachment:ticket:10981:trac_10981.patch the BDFL's patch] at #10981 causes these; the double colons after `TESTS` have to be changed to single ones twice.



---

archive/issue_comments_129909.json:
```json
{
    "body": "Replying to [comment:8 leif]:\n> Would be nice if *you* could fix these *here* as well though [...]\n\nForget that; I've now fixed them at #10981.",
    "created_at": "2011-09-17T00:50:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11553#issuecomment-129909",
    "user": "leif"
}
```

Replying to [comment:8 leif]:
> Would be nice if *you* could fix these *here* as well though [...]

Forget that; I've now fixed them at #10981.



---

archive/issue_comments_129910.json:
```json
{
    "body": "Attachment [trac_11725-fix_doctest_error.reviewer.patch](tarball://root/attachments/some-uuid/ticket11725/trac_11725-fix_doctest_error.reviewer.patch) by leif created at 2011-09-17 01:04:21\n\nReviewer patch. Fixes doctest error in `multi_polynomial_element.py`. Apply on top of the other patch.",
    "created_at": "2011-09-17T01:04:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11553#issuecomment-129910",
    "user": "leif"
}
```

Attachment [trac_11725-fix_doctest_error.reviewer.patch](tarball://root/attachments/some-uuid/ticket11725/trac_11725-fix_doctest_error.reviewer.patch) by leif created at 2011-09-17 01:04:21

Reviewer patch. Fixes doctest error in `multi_polynomial_element.py`. Apply on top of the other patch.



---

archive/issue_comments_129911.json:
```json
{
    "body": "Changing status from needs_work to positive_review.",
    "created_at": "2011-09-17T01:07:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11553#issuecomment-129911",
    "user": "leif"
}
```

Changing status from needs_work to positive_review.



---

archive/issue_comments_129912.json:
```json
{
    "body": "Attached reviewer patch fixes the doctest error.",
    "created_at": "2011-09-17T01:07:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11553#issuecomment-129912",
    "user": "leif"
}
```

Attached reviewer patch fixes the doctest error.



---

archive/issue_comments_129913.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2011-09-17T05:18:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11553",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11553#issuecomment-129913",
    "user": "leif"
}
```

Resolution: fixed
