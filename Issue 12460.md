# Issue 12460: bug comparing trivial Dirichlet characters

archive/issues_012460.json:
```json
{
    "body": "Assignee: was\n\n\n```\nsage: G1 = DirichletGroup(4)\nsage: G2 = DirichletGroup(1)\nsage: G2[0] == G1[0].primitive_character()\n[...]\nZeroDivisionError: Inverse does not exist.\n```\n\n\n(The answer should be `True`.)\n\nWhat happens, I think, is that `G1[0].primitive_character().parent().base_ring()` is not the same as `G2.base_ring()` and something somewhere tries to do some coercion, and fails:\n\n\n```\nsage: G1[0].primitive_character().parent().base_ring()\nCyclotomic Field of order 2 and degree 1\nsage: G2.base_ring()\nCyclotomic Field of order 1 and degree 1\n```\n\n\nIssue created by migration from https://trac.sagemath.org/ticket/12632\n\n",
    "created_at": "2012-03-06T01:53:11Z",
    "labels": [
        "number theory",
        "major"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-5.0",
    "title": "bug comparing trivial Dirichlet characters",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/12460",
    "user": "bober"
}
```
Assignee: was


```
sage: G1 = DirichletGroup(4)
sage: G2 = DirichletGroup(1)
sage: G2[0] == G1[0].primitive_character()
[...]
ZeroDivisionError: Inverse does not exist.
```


(The answer should be `True`.)

What happens, I think, is that `G1[0].primitive_character().parent().base_ring()` is not the same as `G2.base_ring()` and something somewhere tries to do some coercion, and fails:


```
sage: G1[0].primitive_character().parent().base_ring()
Cyclotomic Field of order 2 and degree 1
sage: G2.base_ring()
Cyclotomic Field of order 1 and degree 1
```


Issue created by migration from https://trac.sagemath.org/ticket/12632





---

archive/issue_comments_147537.json:
```json
{
    "body": "Changing type from PLEASE CHANGE to defect.",
    "created_at": "2012-03-06T01:53:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12460#issuecomment-147537",
    "user": "bober"
}
```

Changing type from PLEASE CHANGE to defect.



---

archive/issue_comments_147538.json:
```json
{
    "body": "Attachment [trac_12632-cyclo_coerce_bug.patch](tarball://root/attachments/some-uuid/ticket12632/trac_12632-cyclo_coerce_bug.patch) by davidloeffler created at 2012-03-06 09:06:27\n\nThis was indeed a coercion issue -- the basic error is this:\n\n```\nsage: K1 = CyclotomicField(1)\nsage: K2 = CyclotomicField(2)\nsage: K1.has_coerce_map_from(K2) # boom!\n```\n\nI've uploaded a patch which deals summarily with this special case by adding a couple of lines to the cyclotomic field coercion code, and adds a doctest to prove it.",
    "created_at": "2012-03-06T09:06:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12460#issuecomment-147538",
    "user": "davidloeffler"
}
```

Attachment [trac_12632-cyclo_coerce_bug.patch](tarball://root/attachments/some-uuid/ticket12632/trac_12632-cyclo_coerce_bug.patch) by davidloeffler created at 2012-03-06 09:06:27

This was indeed a coercion issue -- the basic error is this:

```
sage: K1 = CyclotomicField(1)
sage: K2 = CyclotomicField(2)
sage: K1.has_coerce_map_from(K2) # boom!
```

I've uploaded a patch which deals summarily with this special case by adding a couple of lines to the cyclotomic field coercion code, and adds a doctest to prove it.



---

archive/issue_comments_147539.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2012-03-06T09:06:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12460#issuecomment-147539",
    "user": "davidloeffler"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_147540.json:
```json
{
    "body": "Changing component from number theory to number fields.",
    "created_at": "2012-03-06T09:06:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12460#issuecomment-147540",
    "user": "davidloeffler"
}
```

Changing component from number theory to number fields.



---

archive/issue_comments_147541.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2012-03-06T09:59:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12460#issuecomment-147541",
    "user": "bober"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_147542.json:
```json
{
    "body": "Looks good (and all tests pass).",
    "created_at": "2012-03-06T09:59:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12460#issuecomment-147542",
    "user": "bober"
}
```

Looks good (and all tests pass).



---

archive/issue_comments_147543.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2012-03-13T08:25:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/12460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/12460#issuecomment-147543",
    "user": "jdemeyer"
}
```

Resolution: fixed
