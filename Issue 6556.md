# Issue 6556: notebook: document keyboard shortcuts

archive/issues_006556.json:
```json
{
    "body": "Assignee: tba\n\nKeywords: notebook, keyboard shortcuts\n\nThis issue was raised in this [sage-support](http://groups.google.com/group/sage-support/browse_thread/thread/58284efb2a15856e) thread. All key combinations and keyboard shortcuts for the notebook should be documented in the tutorial and the reference manual. The tutorial should also inform readers about the Help link on the notebook. That link contains useful key combinations when using the Sage notebook. The file\n\nSAGE_ROOT/devel/sage-main/sage/server/notebook/config.py\n\ncontains all known key combinations for the notebook.\n\nIssue created by migration from https://trac.sagemath.org/ticket/6556\n\n",
    "created_at": "2009-07-18T20:41:45Z",
    "labels": [
        "documentation",
        "major",
        "enhancement"
    ],
    "title": "notebook: document keyboard shortcuts",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/6556",
    "user": "mvngu"
}
```
Assignee: tba

Keywords: notebook, keyboard shortcuts

This issue was raised in this [sage-support](http://groups.google.com/group/sage-support/browse_thread/thread/58284efb2a15856e) thread. All key combinations and keyboard shortcuts for the notebook should be documented in the tutorial and the reference manual. The tutorial should also inform readers about the Help link on the notebook. That link contains useful key combinations when using the Sage notebook. The file

SAGE_ROOT/devel/sage-main/sage/server/notebook/config.py

contains all known key combinations for the notebook.

Issue created by migration from https://trac.sagemath.org/ticket/6556





---

archive/issue_comments_053460.json:
```json
{
    "body": "Attachment [help_page.patch](tarball://root/attachments/some-uuid/ticket6556/help_page.patch) by hgranath created at 2009-08-07 03:40:35\n\nReorganize notebook help page",
    "created_at": "2009-08-07T03:40:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6556#issuecomment-53460",
    "user": "hgranath"
}
```

Attachment [help_page.patch](tarball://root/attachments/some-uuid/ticket6556/help_page.patch) by hgranath created at 2009-08-07 03:40:35

Reorganize notebook help page



---

archive/issue_comments_053461.json:
```json
{
    "body": "I think it would be helpful for users if the info on the notebook\nHelp link was more organized. Here is a suggestion to group\ntopics, especially about key bindings, to make them easier to\nfind.\n\nA few questions:\n\nI changed \"Delete Cell\" form ctrl-backspace to backspace. Is there\nany reason not to?\n\nThe < and > indentation bindings do not work form me in firefox\n3.0.13 (tab and shift-tab do).\n\nI added Comment/Uncomment Blocks bindings. Which bindings are\npreferred, ctrl-./ctrl-, or ctrl-3/ctrl-4 ?",
    "created_at": "2009-08-07T03:43:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6556#issuecomment-53461",
    "user": "hgranath"
}
```

I think it would be helpful for users if the info on the notebook
Help link was more organized. Here is a suggestion to group
topics, especially about key bindings, to make them easier to
find.

A few questions:

I changed "Delete Cell" form ctrl-backspace to backspace. Is there
any reason not to?

The < and > indentation bindings do not work form me in firefox
3.0.13 (tab and shift-tab do).

I added Comment/Uncomment Blocks bindings. Which bindings are
preferred, ctrl-./ctrl-, or ctrl-3/ctrl-4 ?



---

archive/issue_comments_053462.json:
```json
{
    "body": "Updates help page, tutorial and reference manual",
    "created_at": "2009-08-17T17:41:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6556#issuecomment-53462",
    "user": "hgranath"
}
```

Updates help page, tutorial and reference manual



---

archive/issue_comments_053463.json:
```json
{
    "body": "Attachment [trac_6556.patch](tarball://root/attachments/some-uuid/ticket6556/trac_6556.patch) by hgranath created at 2009-08-17 17:47:07",
    "created_at": "2009-08-17T17:47:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6556#issuecomment-53463",
    "user": "hgranath"
}
```

Attachment [trac_6556.patch](tarball://root/attachments/some-uuid/ticket6556/trac_6556.patch) by hgranath created at 2009-08-17 17:47:07



---

archive/issue_comments_053464.json:
```json
{
    "body": "Works perfectly, and the new documentation is great. Nice work.\n\nNote: Only apply `trac_6556.patch`.",
    "created_at": "2009-08-30T19:38:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6556#issuecomment-53464",
    "user": "timdumol"
}
```

Works perfectly, and the new documentation is great. Nice work.

Note: Only apply `trac_6556.patch`.



---

archive/issue_comments_053465.json:
```json
{
    "body": "Merged `trac_6556.patch`.",
    "created_at": "2009-08-31T07:00:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6556#issuecomment-53465",
    "user": "mvngu"
}
```

Merged `trac_6556.patch`.



---

archive/issue_comments_053466.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-08-31T07:00:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6556#issuecomment-53466",
    "user": "mvngu"
}
```

Resolution: fixed
