# Issue 1467: make it possible to upload/download worksheets as plain text

archive/issues_001467.json:
```json
{
    "body": "Assignee: boothby\n\nmake it possible to upload/download worksheets as plain text\n\nIssue created by migration from https://trac.sagemath.org/ticket/1467\n\n",
    "created_at": "2007-12-12T06:23:58Z",
    "labels": [
        "component: notebook"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "make it possible to upload/download worksheets as plain text",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/1467",
    "user": "https://github.com/williamstein"
}
```
Assignee: boothby

make it possible to upload/download worksheets as plain text

Issue created by migration from https://trac.sagemath.org/ticket/1467





---

archive/issue_events_001618.json:
```json
{
    "actor": "@jasongrout",
    "created_at": "2008-03-31T15:52:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/1467",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1467#event-1618"
}
```



---

archive/issue_comments_009417.json:
```json
{
    "body": "Resolution: duplicate",
    "created_at": "2008-03-31T15:52:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1467",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1467#issuecomment-9417",
    "user": "https://github.com/jasongrout"
}
```

Resolution: duplicate



---

archive/issue_comments_009418.json:
```json
{
    "body": "Since it is already possible to \"download\" plain text worksheets (i.e., copy/paste), this is a dup of #637.",
    "created_at": "2008-03-31T15:52:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1467",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1467#issuecomment-9418",
    "user": "https://github.com/jasongrout"
}
```

Since it is already possible to "download" plain text worksheets (i.e., copy/paste), this is a dup of #637.
