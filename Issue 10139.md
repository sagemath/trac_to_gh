# Issue 10139: Base sage.geometry.cone on the Parma Polyhedra Library (PPL)

Issue created by migration from https://trac.sagemath.org/ticket/10140

Original creator: vbraun

Original creation time: 2010-10-17 20:51:47
...
Comment by vbraun created at 2010-10-17 20:59:25
...
Comment by novoselt created at 2010-10-17 22:18:56
...
Little picks at the patch:
 ...
 1. I think that new possible input `C_Polyhedron` should be documented in the INPUT section rather than a note in the end.
 1. Is it possible to make `C_Polyhedrons` immutable (on demand)? 
 1. I prefer all line generators to be put in the end of ray list for non-strictly convex cones, if they were determined automatically. Because if one works with faces of such a cone, then all these generators are the same and, therefore, others are more "important". That's very minor, of course.
 1. I am trying to follow PEP8 style guide http://www.python.org/dev/peps/pep-0008/ which says: When raising an exception, use "raise `ValueError`('message')" instead of the older form "raise `ValueError`, 'message'". New line 1167 tries to change this recommended form to an old one ;-)
 1. Looking at what you have done in facet normals, maybe we should change `cone.dual_lattice()` method to always return ZZ<sup>n</sup> if there is no dual method for the lattice? Thinking of the class group situation, it seems like a more sensible default.
 1. I think cones should not be printed explicitly (i.e. using "print") when `ValueError` is raised due to attempt to intersect cones in lattices of different dimension. The reason is that maybe some users want to intercept this exception and deal with it in their own way without showing any output. Also, I think that the check should be done not for equality of lattice dimensions, but for equality of lattices themselves, because cones of different lattices should not be intersected.


---

Comment by vbraun created at 2010-10-18 10:48:28
...
Comment by novoselt created at 2010-10-18 15:22:15
...
Comment by jdemeyer created at 2010-11-02 17:40:16
...
Comment by mhampton created at 2010-11-02 18:35:00
...
Comment by vbraun created at 2010-11-25 09:24:33
...
Comment by vbraun created at 2010-12-31 20:51:35
...
Comment by vbraun created at 2011-01-12 03:04:42
...
Comment by fbissey created at 2011-01-25 10:12:14
...
Comment by novoselt created at 2011-01-25 15:04:54
...
Comment by fbissey created at 2011-01-25 18:00:47
...
Comment by novoselt created at 2011-01-26 02:57:15
...
Comment by vbraun created at 2011-01-26 14:02:58
...
Comment by novoselt created at 2011-01-26 14:36:57
...
Comment by fbissey created at 2011-01-27 08:43:43
...
Comment by fbissey created at 2011-01-31 09:12:37
...
Comment by novoselt created at 2011-03-15 19:22:23
...
Comment by mhampton created at 2011-03-15 19:53:21
...
Comment by novoselt created at 2011-03-15 19:57:11
...
Comment by novoselt created at 2011-03-16 05:21:11
...
Comment by novoselt created at 2011-03-16 05:43:42
...
Comment by novoselt created at 2011-03-16 14:04:46
...
Comment by vbraun created at 2011-03-16 14:56:38
...
Comment by vbraun created at 2011-03-16 16:01:38
...
Comment by novoselt created at 2011-03-16 17:05:28
...
Comment by vbraun created at 2011-03-17 22:07:53
...
Comment by novoselt created at 2011-03-18 02:32:59
...
Some more personal experience:
 ...
 1. The second annoying issue with PALP was that the i-th face of dimension 0 could be generated by j-th vertex for i != j. While in principle vertices as points of the ambient space and vertices as elements of the face lattice are different, this discrepancy can be quite inconvenient: you need to remember that it exists and you need to insert extra code to do translation from one to another. So eventually I added sorting for 0-dimensional faces.
 1. Faces of reflexive polytopes and their polars are in bijection. I find it very convenient to write things like
 {{{
...
Somewhat related behaviour of other parts of Sage:
 ...
 1. `QQ["x, y"] == QQ["y, x"]` evaluates to False, so while these rings are generated by the same variables the order matters. (To my surprise, one can also define `QQ["x, x"]` and it will be a ring in two variables with the same name...)
...
Comment by novoselt created at 2011-03-18 02:35:15
...
Comment by mhampton created at 2011-03-18 03:38:29
...
Comment by vbraun created at 2011-03-18 15:22:34
...
Comment by novoselt created at 2011-03-18 18:29:15
...
Comment by novoselt created at 2011-03-18 18:43:05
...
Comment by vbraun created at 2011-03-19 15:17:43
...
Comment by novoselt created at 2011-03-19 16:29:30
...
If not, I propose the following:
 ...
 1. Add a function `sage.geometry.cone.keep_order(True/False)`, without importing it into the global name space, that will switch the default behaviour for the current session, so that if users always want `keep_order=True`, they don't have to repeat it all the time. Mention this function in the description of `keep_order` parameter in `Cone`.
 1. Perhaps in the documentation of that function we may mention that if users feel strongly that it should be the default always, they can explain it on the above sage-combinat discussion. (I suppose it is OK to include such links in docstrings.)
 1. In the documentation examples where the ray order is important, use `keep_order=True` instead of `check=False` (there are some examples in the patch where you have added this option).
...
Comment by vbraun created at 2011-03-19 22:30:49
...
Comment by novoselt created at 2011-03-19 22:58:36
...
Comment by vbraun created at 2011-03-19 23:24:09
...
Comment by novoselt created at 2011-04-19 15:26:55
...
So instead of `keep_order` I would like to add the above check. Advantages:
 ...
 1. It allows easy writing of doctests which explicitly show rays without worrying that the order will change in future releases and a bunch of trivial adjustments will be required.
 1. It constructs the same cones with `check=True` and `check=False` options in cases when it is indeed OK to use `check=False` option.
 1. It is a bit more intuitive to the user and in particular nice for tutorials and live demonstrations.
 1. It makes me happy ;-)
...
Comment by novoselt created at 2011-04-20 00:12:22
...
Comment by novoselt created at 2011-04-20 00:57:09
...
Comment by novoselt created at 2011-04-20 21:33:31
...
First of all, what I have done with Volker's patches:
 ...
 1. Removed doctest fixes related to the ray order as it is restored by the reviewer patch.
 1. Removed the modification to cone-from-polyhedron code since the old one still works fine and makes all the checks.
 1. Left the original of the line mentioned in http://trac.sagemath.org/sage_trac/ticket/10140#comment:40 since it seems to be the correct one to me.

The reviewer patch does the following:
 1. Preserves ray order for strictly convex cones given by the minimal generating set of rays, _without performance penalty._
 1. Removes `self.dual_lattice() is self.lattice()` check as `dual_lattice` now returns `ZZ^n` if there is no "honest dual". (This was not the case when this ticket was created.)
 1. Modifies the intersection code (discussed above) slightly. This is *not* its final version, I need to play a bit with lattice intersection but I think it is the way to go.
...
Comment by vbraun created at 2011-04-20 21:57:07
...
Comment by novoselt created at 2011-04-20 22:13:17
...
Comment by vbraun created at 2011-04-20 22:25:50
...
Comment by novoselt created at 2011-04-21 21:57:58
...
Comment by novoselt created at 2011-04-24 17:41:02
...
Comment by vbraun created at 2011-05-05 11:48:39
...
Comment by jdemeyer created at 2011-05-08 11:31:08
...
Comment by novoselt created at 2011-05-08 14:45:03
...
Comment by jdemeyer created at 2011-05-27 12:01:34
...
Comment by vbraun created at 2011-05-28 16:03:55
...
Comment by vbraun created at 2011-05-28 16:29:54
...
Comment by dimpase created at 2012-01-10 06:24:29
...
Comment by novoselt created at 2012-01-10 06:29:25
