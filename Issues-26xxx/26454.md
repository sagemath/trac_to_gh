# Issue 26454: Do not check for random output of cusps_nf.py

archive/issues_026217.json:
```json
{
    "assignees": [],
    "body": "The `ABmatrix` of a cusp of a number field is random as the docstring states, however, we are checking for the exact output in one place which is unfortunate as that output changes on some setups, e.g., within conda.\n\nWe should instead verify that the `ABmatrix` is correct.\n\nKeywords: **conda**\n\nBranch/Commit: **[u/vklein/8ef2f10e55a7fe0d5dff8b7c0de938b3ba0bfde6](https://github.com/sagemath/sagetrac-mirror/tree/u/vklein/8ef2f10e55a7fe0d5dff8b7c0de938b3ba0bfde6) @ [8ef2f10](https://github.com/sagemath/sagetrac-mirror/commit/8ef2f10e55a7fe0d5dff8b7c0de938b3ba0bfde6)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Julian R\u00fcth**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26454_\n\n",
    "closed_at": "2018-10-22T22:40:56Z",
    "created_at": "2018-10-10T10:58:21Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20distribution",
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Do not check for random output of cusps_nf.py",
    "type": "issue",
    "updated_at": "2018-10-28T14:52:23Z",
    "url": "https://github.com/sagemath/sage/issues/26454",
    "user": "https://github.com/saraedum"
}
```
The `ABmatrix` of a cusp of a number field is random as the docstring states, however, we are checking for the exact output in one place which is unfortunate as that output changes on some setups, e.g., within conda.

We should instead verify that the `ABmatrix` is correct.

Keywords: **conda**

Branch/Commit: **[u/vklein/8ef2f10e55a7fe0d5dff8b7c0de938b3ba0bfde6](https://github.com/sagemath/sagetrac-mirror/tree/u/vklein/8ef2f10e55a7fe0d5dff8b7c0de938b3ba0bfde6) @ [8ef2f10](https://github.com/sagemath/sagetrac-mirror/commit/8ef2f10e55a7fe0d5dff8b7c0de938b3ba0bfde6)**

Reviewer: **Travis Scrimshaw**

Author: **Julian Rüth**

_Issue created by migration from https://trac.sagemath.org/ticket/26454_





---

archive/issue_events_346395.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-10-10T10:58:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26454",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26454#event-346395"
}
```



---

archive/issue_events_346396.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-10-10T10:58:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26454",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20distribution",
    "label_color": "000080",
    "label_name": "component: distribution",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26454#event-346396"
}
```



---

archive/issue_events_346397.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-10-10T10:58:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26454",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26454#event-346397"
}
```



---

archive/issue_events_346398.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-10-10T10:58:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26454",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26454#event-346398"
}
```



---

archive/issue_comments_410780.json:
```json
{
    "body": "Branch: **[u/saraedum/26454](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/26454)**",
    "created_at": "2018-10-10T11:00:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26454",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26454#issuecomment-410780",
    "user": "https://github.com/saraedum"
}
```

Branch: **[u/saraedum/26454](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/26454)**



---

archive/issue_comments_410781.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/08d403ae6c9a9b2368f162ec665a496cba2c3de7\">08d403a</a></td><td><code>Do not check for exact value of ABmatrix since it's random</code></td></tr></table>\n",
    "created_at": "2018-10-10T11:01:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26454",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26454#issuecomment-410781",
    "user": "https://github.com/saraedum"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/08d403ae6c9a9b2368f162ec665a496cba2c3de7">08d403a</a></td><td><code>Do not check for exact value of ABmatrix since it's random</code></td></tr></table>




---

archive/issue_events_346399.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-10-10T11:01:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26454",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26454#event-346399"
}
```



---

archive/issue_comments_410782.json:
```json
{
    "body": "Commit: **[08d403a](https://github.com/sagemath/sagetrac-mirror/commit/08d403ae6c9a9b2368f162ec665a496cba2c3de7)**",
    "created_at": "2018-10-10T11:01:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26454",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26454#issuecomment-410782",
    "user": "https://github.com/saraedum"
}
```

Commit: **[08d403a](https://github.com/sagemath/sagetrac-mirror/commit/08d403ae6c9a9b2368f162ec665a496cba2c3de7)**



---

archive/issue_comments_410783.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2018-10-10T23:36:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26454",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26454#issuecomment-410783",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_410784.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNitpick, but we usually just use `# random` instead of `# random output`. LGTM otherwise and you can set a positive review on my behalf once tweaked.",
    "created_at": "2018-10-10T23:36:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26454",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26454#issuecomment-410784",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:3" align="right">Comment 3</div>

Nitpick, but we usually just use `# random` instead of `# random output`. LGTM otherwise and you can set a positive review on my behalf once tweaked.



---

archive/issue_comments_410785.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8ef2f10e55a7fe0d5dff8b7c0de938b3ba0bfde6\">8ef2f10</a></td><td><code>Mark doctest as # random not as # random output</code></td></tr></table>\n",
    "created_at": "2018-10-11T11:19:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26454",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26454#issuecomment-410785",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8ef2f10e55a7fe0d5dff8b7c0de938b3ba0bfde6">8ef2f10</a></td><td><code>Mark doctest as # random not as # random output</code></td></tr></table>




---

archive/issue_comments_410786.json:
```json
{
    "body": "Changed commit from **[08d403a](https://github.com/sagemath/sagetrac-mirror/commit/08d403ae6c9a9b2368f162ec665a496cba2c3de7)** to **[8ef2f10](https://github.com/sagemath/sagetrac-mirror/commit/8ef2f10e55a7fe0d5dff8b7c0de938b3ba0bfde6)**",
    "created_at": "2018-10-11T11:19:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26454",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26454#issuecomment-410786",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[08d403a](https://github.com/sagemath/sagetrac-mirror/commit/08d403ae6c9a9b2368f162ec665a496cba2c3de7)** to **[8ef2f10](https://github.com/sagemath/sagetrac-mirror/commit/8ef2f10e55a7fe0d5dff8b7c0de938b3ba0bfde6)**



---

archive/issue_comments_410787.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nFixed. Thanks for the review.\n  \n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8ef2f10e55a7fe0d5dff8b7c0de938b3ba0bfde6\">8ef2f10</a></td><td><code>Mark doctest as # random not as # random output</code></td></tr></table>\n",
    "created_at": "2018-10-11T11:20:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26454",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26454#issuecomment-410787",
    "user": "https://github.com/saraedum"
}
```

<div id="comment:5" align="right">Comment 5</div>

Fixed. Thanks for the review.
  
---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8ef2f10e55a7fe0d5dff8b7c0de938b3ba0bfde6">8ef2f10</a></td><td><code>Mark doctest as # random not as # random output</code></td></tr></table>




---

archive/issue_events_346400.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-10-11T11:20:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26454",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26454#event-346400"
}
```



---

archive/issue_events_346401.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-10-11T11:20:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26454",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26454#event-346401"
}
```



---

archive/issue_comments_410788.json:
```json
{
    "body": "Changed branch from **[u/saraedum/26454](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/26454)** to **[8ef2f10](https://github.com/sagemath/sagetrac-mirror/commit/8ef2f10e55a7fe0d5dff8b7c0de938b3ba0bfde6)**",
    "created_at": "2018-10-22T22:40:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26454",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26454#issuecomment-410788",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/saraedum/26454](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/26454)** to **[8ef2f10](https://github.com/sagemath/sagetrac-mirror/commit/8ef2f10e55a7fe0d5dff8b7c0de938b3ba0bfde6)**



---

archive/issue_events_346402.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-10-22T22:40:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26454",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26454#event-346402"
}
```



---

archive/issue_events_346403.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "30bfdb5347a96f613336a29684cfac9cb2a54901",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-10-22T22:40:56Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26454",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26454#event-346403"
}
```



---

archive/issue_comments_410789.json:
```json
{
    "body": "Changed branch from **[8ef2f10](https://github.com/sagemath/sagetrac-mirror/commit/8ef2f10e55a7fe0d5dff8b7c0de938b3ba0bfde6)** to **[u/vklein/8ef2f10e55a7fe0d5dff8b7c0de938b3ba0bfde6](https://github.com/sagemath/sagetrac-mirror/tree/u/vklein/8ef2f10e55a7fe0d5dff8b7c0de938b3ba0bfde6)**",
    "created_at": "2018-10-25T14:48:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26454",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26454#issuecomment-410789",
    "user": "https://github.com/vinklein"
}
```

Changed branch from **[8ef2f10](https://github.com/sagemath/sagetrac-mirror/commit/8ef2f10e55a7fe0d5dff8b7c0de938b3ba0bfde6)** to **[u/vklein/8ef2f10e55a7fe0d5dff8b7c0de938b3ba0bfde6](https://github.com/sagemath/sagetrac-mirror/tree/u/vklein/8ef2f10e55a7fe0d5dff8b7c0de938b3ba0bfde6)**



---

archive/issue_events_346404.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-10-28T14:52:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/26454",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26454#event-346404"
}
```



---

archive/issue_events_346405.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-10-28T14:52:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26454",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26454#event-346405"
}
```



---

archive/issue_comments_410790.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nThis should be re-targeted for 8.5.",
    "created_at": "2018-10-28T14:52:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26454",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26454#issuecomment-410790",
    "user": "https://github.com/embray"
}
```

<div id="comment:8" align="right">Comment 8</div>

This should be re-targeted for 8.5.
