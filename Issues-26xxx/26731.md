# Issue 26731: Cartesian product of polynomial quotient rings no longer works

archive/issues_026494.json:
```json
{
    "assignees": [],
    "body": "The following used to work in SageMath 8.3, but in SageMath 8.4 one gets an error:\n\n```\nfrom sage.categories.all import cartesian_product, CommutativeRings\nL.<a> = Qp(17, prec=8).extension(x^2+17, prec=8)\nR.<x> = L[]\ncategory = CommutativeRings().CartesianProducts()\ncartesian_product([R.quotient(x), R.quotient(x)], category=category)\nTraceback (most recent call last):\n...\nTypeError: <class 'sage.rings.polynomial.polynomial_quotient_ring.PolynomialQuotientRing_generic_with_category'> is not hashable and does not implement _cache_key()\n```\n\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26731_\n\n",
    "created_at": "2018-11-21T15:28:06Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20categories",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Cartesian product of polynomial quotient rings no longer works",
    "type": "issue",
    "updated_at": "2022-12-29T01:43:14Z",
    "url": "https://github.com/sagemath/sage/issues/26731",
    "user": "https://github.com/pjbruin"
}
```
The following used to work in SageMath 8.3, but in SageMath 8.4 one gets an error:

```
from sage.categories.all import cartesian_product, CommutativeRings
L.<a> = Qp(17, prec=8).extension(x^2+17, prec=8)
R.<x> = L[]
category = CommutativeRings().CartesianProducts()
cartesian_product([R.quotient(x), R.quotient(x)], category=category)
Traceback (most recent call last):
...
TypeError: <class 'sage.rings.polynomial.polynomial_quotient_ring.PolynomialQuotientRing_generic_with_category'> is not hashable and does not implement _cache_key()
```


_Issue created by migration from https://trac.sagemath.org/ticket/26731_





---

archive/issue_events_349429.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2018-11-21T15:28:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26731",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26731#event-349429"
}
```



---

archive/issue_events_349430.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2018-11-21T15:28:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26731",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20categories",
    "label_color": "0000ff",
    "label_name": "component: categories",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26731#event-349430"
}
```



---

archive/issue_events_349431.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2018-11-21T15:28:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26731",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26731#event-349431"
}
```



---

archive/issue_events_349432.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2018-11-21T15:28:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26731",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26731#event-349432"
}
```



---

archive/issue_comments_414512.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nSee also #28681.",
    "created_at": "2019-10-31T11:58:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26731",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26731#issuecomment-414512",
    "user": "https://github.com/mwageringel"
}
```

<div id="comment:1" align="right">Comment 1</div>

See also #28681.



---

archive/issue_events_349433.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:43:14Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/26731",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26731#event-349433"
}
```
