...
```json
{
    "body": "Sorry, I didn't fully read the first part of your message.  I thought for some reason you were explicitly trying to use Tkinter directly.  Regardless, the solution is still basically the same: It shouldn't be importing `_tkinter` at all unless you have some bad `PYTHONPATH` or something.  \n\nAlso maybe a `~/.config/matplotlib` setting the TkAgg backend by default?  I feel like Sage shouldn't be loading the default matplotlib config as it could conflict with other matplotlibs on the system.",
...
```json
{
    "body": "Oddly enough, the bug appears without me manually importing tkinter. Just trying to plot a graph, on a clean sage install (compiled from the source tarball).\n\nMaybe at some point matplotlib (or some other component) tries to use tkinter to show plots?",
...
```json
{
    "body": "(In fact, `sage-env` does set `export MPLCONFIGDIR=\"$DOT_SAGE/matplotlib-1.5.1\"`.  Probably the `sage.env` module should do the same if `MPLCONFIGDIR` is not otherwise set.)",
...
```json
{
    "body": "The latter isn't a real problem; it's expected.  The former is strange but I think it's something odd on your system configuration.  Starting with the fact that you can apparently import an `_tkinter` module at all.  If you do:\n\n```\nsage -python -c 'import _tkinter; print(_tkinter)'\n```\n\nwhat does it show?",
...
```json
{
    "body": "Does anyone know why Sage packages libfreetype in the first place?  Because that's probably the source of the problem here.",
