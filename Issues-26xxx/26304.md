# Issue 26304: doctests in src/sage/repl/attach.py and src/sage/repl/load.py fail when test.py is present in folder from where doctests are run

archive/issues_026067.json:
```json
{
    "assignees": [],
    "body": "First reported at https://groups.google.com/d/msg/sage-release/eL75kEvaCeA/T6_kmPNCCQAJ.\n\nIf an empty `test.py` is present in SAGE_ROOT then\n\n```\nsage -t --long src/sage/repl/attach.py\n**********************************************************************\nFile \"src/sage/repl/attach.py\", line 171, in sage.repl.attach.load_attach_path\nFailed example:\n    attach('test.py')\nExpected:\n    Traceback (most recent call last):\n    ...\n    IOError: did not find file 'test.py' to load or attach\nGot:\n    <BLANKLINE>\n**********************************************************************\nFile \"src/sage/repl/attach.py\", line 176, in sage.repl.attach.load_attach_path\nFailed example:\n    attach('test.py')\nExpected:\n    111\nGot:\n    <BLANKLINE>\n**********************************************************************\nFile \"src/sage/repl/attach.py\", line 178, in sage.repl.attach.load_attach_path\nFailed example:\n    attached_files() == [fullpath]\nExpected:\n    True\nGot:\n    False\n**********************************************************************\nFile \"src/sage/repl/attach.py\", line 183, in sage.repl.attach.load_attach_path\nFailed example:\n    load('test.py')\nExpected:\n    Traceback (most recent call last):\n    ...\n    IOError: did not find file 'test.py' to load or attach\nGot:\n    <BLANKLINE>\n**********************************************************************\nFile \"src/sage/repl/attach.py\", line 436, in sage.repl.attach.detach\nFailed example:\n    attach('test.py')\nExpected:\n    111\nGot:\n    <BLANKLINE>\n**********************************************************************\nFile \"src/sage/repl/attach.py\", line 438, in sage.repl.attach.detach\nFailed example:\n    attached_files() == [os.path.normpath(fullpath)]\nExpected:\n    True\nGot:\n    False\n**********************************************************************\nFile \"src/sage/repl/attach.py\", line 443, in sage.repl.attach.detach\nFailed example:\n    attach('test.py')\nExpected:\n    111\nGot:\n    <BLANKLINE>\n**********************************************************************\n2 items had failures:\n   3 of  25 in sage.repl.attach.detach\n   4 of  20 in sage.repl.attach.load_attach_path\n    [127 tests, 7 failures, 2.61 s]\n----------------------------------------------------------------------\nsage -t --long src/sage/repl/attach.py  # 7 doctests failed\n```\n\nand\n\n```\nsage -t --long src/sage/repl/load.py\n**********************************************************************\nFile \"src/sage/repl/load.py\", line 180, in sage.repl.load.load\nFailed example:\n    attach(fname)\nExpected:\n    111\nGot:\n    <BLANKLINE>\n**********************************************************************\n1 item had failures:\n   1 of  34 in sage.repl.load.load\n    [42 tests, 1 failure, 1.90 s]\n----------------------------------------------------------------------\nsage -t --long src/sage/repl/load.py  # 1 doctest failed\n```\n\n\nBranch/Commit: **[224ab7b](https://github.com/sagemath/sagetrac-mirror/commit/224ab7b2f79670c71b70f7f86b9ce9603a380ba8)**\n\nReviewer: **Erik Bray**\n\nAuthor: **Steven Trogdon**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26304_\n\n",
    "closed_at": "2019-01-24T18:18:02Z",
    "created_at": "2018-09-18T03:10:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20doctest%20framework",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "doctests in src/sage/repl/attach.py and src/sage/repl/load.py fail when test.py is present in folder from where doctests are run",
    "type": "issue",
    "updated_at": "2019-01-24T18:18:02Z",
    "url": "https://github.com/sagemath/sage/issues/26304",
    "user": "https://github.com/strogdon"
}
```
First reported at https://groups.google.com/d/msg/sage-release/eL75kEvaCeA/T6_kmPNCCQAJ.

If an empty `test.py` is present in SAGE_ROOT then

```
sage -t --long src/sage/repl/attach.py
**********************************************************************
File "src/sage/repl/attach.py", line 171, in sage.repl.attach.load_attach_path
Failed example:
    attach('test.py')
Expected:
    Traceback (most recent call last):
    ...
    IOError: did not find file 'test.py' to load or attach
Got:
    <BLANKLINE>
**********************************************************************
File "src/sage/repl/attach.py", line 176, in sage.repl.attach.load_attach_path
Failed example:
    attach('test.py')
Expected:
    111
Got:
    <BLANKLINE>
**********************************************************************
File "src/sage/repl/attach.py", line 178, in sage.repl.attach.load_attach_path
Failed example:
    attached_files() == [fullpath]
Expected:
    True
Got:
    False
**********************************************************************
File "src/sage/repl/attach.py", line 183, in sage.repl.attach.load_attach_path
Failed example:
    load('test.py')
Expected:
    Traceback (most recent call last):
    ...
    IOError: did not find file 'test.py' to load or attach
Got:
    <BLANKLINE>
**********************************************************************
File "src/sage/repl/attach.py", line 436, in sage.repl.attach.detach
Failed example:
    attach('test.py')
Expected:
    111
Got:
    <BLANKLINE>
**********************************************************************
File "src/sage/repl/attach.py", line 438, in sage.repl.attach.detach
Failed example:
    attached_files() == [os.path.normpath(fullpath)]
Expected:
    True
Got:
    False
**********************************************************************
File "src/sage/repl/attach.py", line 443, in sage.repl.attach.detach
Failed example:
    attach('test.py')
Expected:
    111
Got:
    <BLANKLINE>
**********************************************************************
2 items had failures:
   3 of  25 in sage.repl.attach.detach
   4 of  20 in sage.repl.attach.load_attach_path
    [127 tests, 7 failures, 2.61 s]
----------------------------------------------------------------------
sage -t --long src/sage/repl/attach.py  # 7 doctests failed
```

and

```
sage -t --long src/sage/repl/load.py
**********************************************************************
File "src/sage/repl/load.py", line 180, in sage.repl.load.load
Failed example:
    attach(fname)
Expected:
    111
Got:
    <BLANKLINE>
**********************************************************************
1 item had failures:
   1 of  34 in sage.repl.load.load
    [42 tests, 1 failure, 1.90 s]
----------------------------------------------------------------------
sage -t --long src/sage/repl/load.py  # 1 doctest failed
```


Branch/Commit: **[224ab7b](https://github.com/sagemath/sagetrac-mirror/commit/224ab7b2f79670c71b70f7f86b9ce9603a380ba8)**

Reviewer: **Erik Bray**

Author: **Steven Trogdon**

_Issue created by migration from https://trac.sagemath.org/ticket/26304_





---

archive/issue_events_344503.json:
```json
{
    "actor": "https://github.com/strogdon",
    "created_at": "2018-09-18T03:10:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26304#event-344503"
}
```



---

archive/issue_events_344504.json:
```json
{
    "actor": "https://github.com/strogdon",
    "created_at": "2018-09-18T03:10:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20doctest%20framework",
    "label_color": "000080",
    "label_name": "component: doctest framework",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26304#event-344504"
}
```



---

archive/issue_events_344505.json:
```json
{
    "actor": "https://github.com/strogdon",
    "created_at": "2018-09-18T03:10:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26304#event-344505"
}
```



---

archive/issue_events_344506.json:
```json
{
    "actor": "https://github.com/strogdon",
    "created_at": "2018-09-18T03:10:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26304#event-344506"
}
```



---

archive/issue_comments_408363.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nIn addition it would seem that `attach()` and `load_attach_path()` from `src/sage/repl/attach.py` are not working as intended. The following segments of code produce the same result when a `test.py` is present in the folder from where tests are run.\n\nWith an empty `test.py` present in SAGE_ROOT\n\n```\nsage: load_attach_path()\n['.']\nsage: t_dir = tmp_dir()\nsage: fname = 'test.py'\nsage: fullpath = os.path.join(t_dir, fname)\nsage: with open(fullpath, 'w') as f:\n....:     _ = f.write(\"print(37 * 3)\")\n....:     \nsage: load_attach_path(t_dir)\nsage: attach(fname)\nsage:\n```\n\nand without the `load_attach_path(t_dir)`\n\n```\nsage: load_attach_path()\n['.']\nsage: t_dir = tmp_dir()\nsage: fname = 'test.py'\nsage: fullpath = os.path.join(t_dir, fname)\nsage: with open(fullpath, 'w') as f:\n....:     _ = f.write(\"print(37 * 3)\")\n....:     \nsage: attach(fname)\nsage:\n```\nAt least in the first instance we should get\n\n```\nsage: attach(fname)\n111\n```\nwhich can be confirmed by running the code with `test.py` removed.",
    "created_at": "2018-09-18T03:25:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26304#issuecomment-408363",
    "user": "https://github.com/strogdon"
}
```

<div id="comment:1" align="right">Comment 1</div>

In addition it would seem that `attach()` and `load_attach_path()` from `src/sage/repl/attach.py` are not working as intended. The following segments of code produce the same result when a `test.py` is present in the folder from where tests are run.

With an empty `test.py` present in SAGE_ROOT

```
sage: load_attach_path()
['.']
sage: t_dir = tmp_dir()
sage: fname = 'test.py'
sage: fullpath = os.path.join(t_dir, fname)
sage: with open(fullpath, 'w') as f:
....:     _ = f.write("print(37 * 3)")
....:     
sage: load_attach_path(t_dir)
sage: attach(fname)
sage:
```

and without the `load_attach_path(t_dir)`

```
sage: load_attach_path()
['.']
sage: t_dir = tmp_dir()
sage: fname = 'test.py'
sage: fullpath = os.path.join(t_dir, fname)
sage: with open(fullpath, 'w') as f:
....:     _ = f.write("print(37 * 3)")
....:     
sage: attach(fname)
sage:
```
At least in the first instance we should get

```
sage: attach(fname)
111
```
which can be confirmed by running the code with `test.py` removed.



---

archive/issue_comments_408364.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nThe test should probably change directories into the temp directory.",
    "created_at": "2018-09-18T17:40:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26304#issuecomment-408364",
    "user": "https://github.com/embray"
}
```

<div id="comment:2" align="right">Comment 2</div>

The test should probably change directories into the temp directory.



---

archive/issue_comments_408365.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nReplying to [@embray](#comment%3A2):\n> The test should probably change directories into the temp directory.\n\nI just noticed that `load_attach_path()` has two arguments. Perhaps part of the problem is that we should have\n\n```\nload_attach_path(t_dir, replace='True')\n```\nThe default is `replace='False'` and the paths are appended.",
    "created_at": "2018-09-18T17:57:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26304#issuecomment-408365",
    "user": "https://github.com/strogdon"
}
```

<div id="comment:3" align="right">Comment 3</div>

Replying to [@embray](#comment%3A2):
> The test should probably change directories into the temp directory.

I just noticed that `load_attach_path()` has two arguments. Perhaps part of the problem is that we should have

```
load_attach_path(t_dir, replace='True')
```
The default is `replace='False'` and the paths are appended.



---

archive/issue_comments_408366.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ca919364a4f91a0b1c44db325a90ec2e31f8ee17\">ca91936</a></td><td><code>changes to src/sage/repl/attach.py and src/sage/repl/load.py to correct</code></td></tr></table>\n",
    "created_at": "2018-09-19T00:19:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26304#issuecomment-408366",
    "user": "https://github.com/strogdon"
}
```

<div id="comment:4" align="right">Comment 4</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ca919364a4f91a0b1c44db325a90ec2e31f8ee17">ca91936</a></td><td><code>changes to src/sage/repl/attach.py and src/sage/repl/load.py to correct</code></td></tr></table>




---

archive/issue_comments_408367.json:
```json
{
    "body": "Commit: **[ca91936](https://github.com/sagemath/sagetrac-mirror/commit/ca919364a4f91a0b1c44db325a90ec2e31f8ee17)**",
    "created_at": "2018-09-19T00:19:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26304#issuecomment-408367",
    "user": "https://github.com/strogdon"
}
```

Commit: **[ca91936](https://github.com/sagemath/sagetrac-mirror/commit/ca919364a4f91a0b1c44db325a90ec2e31f8ee17)**



---

archive/issue_comments_408368.json:
```json
{
    "body": "Branch: **[u/strogdon/attach_load_path_fix](https://github.com/sagemath/sagetrac-mirror/tree/u/strogdon/attach_load_path_fix)**",
    "created_at": "2018-09-19T00:19:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26304#issuecomment-408368",
    "user": "https://github.com/strogdon"
}
```

Branch: **[u/strogdon/attach_load_path_fix](https://github.com/sagemath/sagetrac-mirror/tree/u/strogdon/attach_load_path_fix)**



---

archive/issue_events_344507.json:
```json
{
    "actor": "https://github.com/strogdon",
    "created_at": "2018-09-19T00:20:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26304#event-344507"
}
```



---

archive/issue_comments_408369.json:
```json
{
    "body": "Author: **Steven Trogdon**",
    "created_at": "2018-09-19T00:20:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26304#issuecomment-408369",
    "user": "https://github.com/strogdon"
}
```

Author: **Steven Trogdon**



---

archive/issue_comments_408370.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nFor testing purposes make sure there is a `test.py` file in SAGE_ROOT. I hope I have retained the original intent of the code.",
    "created_at": "2018-09-19T00:20:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26304#issuecomment-408370",
    "user": "https://github.com/strogdon"
}
```

<div id="comment:5" align="right">Comment 5</div>

For testing purposes make sure there is a `test.py` file in SAGE_ROOT. I hope I have retained the original intent of the code.



---

archive/issue_comments_408371.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nI will make the following revision since `replace=True` was not needed for the doctest to pass.\n\n```diff\n--- a/src/sage/repl/attach.py\n+++ b/src/sage/repl/attach.py\n@@ -174,7 +174,7 @@ def load_attach_path(path=None, replace=False):\n         Traceback (most recent call last):\n         ...\n         IOError: did not find file 'test.py' to load or attach\n-        sage: load_attach_path(t_dir, replace=True)\n+        sage: load_attach_path(t_dir)\n         sage: attach('test.py')\n         111\n         sage: attached_files() == [fullpath]\n```",
    "created_at": "2018-09-20T15:06:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26304#issuecomment-408371",
    "user": "https://github.com/strogdon"
}
```

<div id="comment:6" align="right">Comment 6</div>

I will make the following revision since `replace=True` was not needed for the doctest to pass.

```diff
--- a/src/sage/repl/attach.py
+++ b/src/sage/repl/attach.py
@@ -174,7 +174,7 @@ def load_attach_path(path=None, replace=False):
         Traceback (most recent call last):
         ...
         IOError: did not find file 'test.py' to load or attach
-        sage: load_attach_path(t_dir, replace=True)
+        sage: load_attach_path(t_dir)
         sage: attach('test.py')
         111
         sage: attached_files() == [fullpath]
```



---

archive/issue_comments_408372.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ef281aad923f73d6d8db4fd6d1c8335ec6daf39c\">ef281aa</a></td><td><code>remove replace=True in one location</code></td></tr></table>\n",
    "created_at": "2018-09-20T15:08:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26304#issuecomment-408372",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:7" align="right">Comment 7</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ef281aad923f73d6d8db4fd6d1c8335ec6daf39c">ef281aa</a></td><td><code>remove replace=True in one location</code></td></tr></table>




---

archive/issue_comments_408373.json:
```json
{
    "body": "Changed commit from **[ca91936](https://github.com/sagemath/sagetrac-mirror/commit/ca919364a4f91a0b1c44db325a90ec2e31f8ee17)** to **[ef281aa](https://github.com/sagemath/sagetrac-mirror/commit/ef281aad923f73d6d8db4fd6d1c8335ec6daf39c)**",
    "created_at": "2018-09-20T15:08:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26304#issuecomment-408373",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[ca91936](https://github.com/sagemath/sagetrac-mirror/commit/ca919364a4f91a0b1c44db325a90ec2e31f8ee17)** to **[ef281aa](https://github.com/sagemath/sagetrac-mirror/commit/ef281aad923f73d6d8db4fd6d1c8335ec6daf39c)**



---

archive/issue_comments_408374.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nThere is one additional failure not listed above in doctesting `src/sage/repl/attach.py` that does not appear with the above branch applied. \n\nThe failure with an empty\n`test2.py` present in `SAGE_ROOT`\n\n```\n./sage -t --long src/sage/repl/attach.py\n\n**********************************************************************\nFile \"src/sage/repl/attach.py\", line 447, in sage.repl.attach.detach\nFailed example:\n    attach('test2.py')\nExpected:\n    3\nGot:\n    <BLANKLINE>\n**********************************************************************\n```\n\nTo properly test have an empty `test.py` and `test2.py` in `SAGE_ROOT` when testing from `SAGE_ROOT`.",
    "created_at": "2018-10-08T20:06:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26304#issuecomment-408374",
    "user": "https://github.com/strogdon"
}
```

<div id="comment:8" align="right">Comment 8</div>

There is one additional failure not listed above in doctesting `src/sage/repl/attach.py` that does not appear with the above branch applied. 

The failure with an empty
`test2.py` present in `SAGE_ROOT`

```
./sage -t --long src/sage/repl/attach.py

**********************************************************************
File "src/sage/repl/attach.py", line 447, in sage.repl.attach.detach
Failed example:
    attach('test2.py')
Expected:
    3
Got:
    <BLANKLINE>
**********************************************************************
```

To properly test have an empty `test.py` and `test2.py` in `SAGE_ROOT` when testing from `SAGE_ROOT`.



---

archive/issue_events_344508.json:
```json
{
    "actor": "https://github.com/strogdon",
    "created_at": "2018-10-08T20:08:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26304#event-344508"
}
```



---

archive/issue_events_344509.json:
```json
{
    "actor": "https://github.com/strogdon",
    "created_at": "2018-10-08T20:08:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26304#event-344509"
}
```



---

archive/issue_comments_408375.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nTechnically this is a doctest failure, though only in corner cases. So, perhaps it should be a blocker?",
    "created_at": "2018-10-08T20:08:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26304#issuecomment-408375",
    "user": "https://github.com/strogdon"
}
```

<div id="comment:9" align="right">Comment 9</div>

Technically this is a doctest failure, though only in corner cases. So, perhaps it should be a blocker?



---

archive/issue_comments_408376.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nI don't think this is a blocker.  It does not, for example, cause any buildbots to fail.",
    "created_at": "2018-10-09T10:45:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26304#issuecomment-408376",
    "user": "https://github.com/embray"
}
```

<div id="comment:10" align="right">Comment 10</div>

I don't think this is a blocker.  It does not, for example, cause any buildbots to fail.



---

archive/issue_events_344510.json:
```json
{
    "actor": "https://github.com/strogdon",
    "created_at": "2018-10-09T12:54:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26304#event-344510"
}
```



---

archive/issue_events_344511.json:
```json
{
    "actor": "https://github.com/strogdon",
    "created_at": "2018-10-09T12:54:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26304#event-344511"
}
```



---

archive/issue_comments_408377.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nOK, I'll remove that.",
    "created_at": "2018-10-09T12:54:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26304#issuecomment-408377",
    "user": "https://github.com/strogdon"
}
```

<div id="comment:11" align="right">Comment 11</div>

OK, I'll remove that.



---

archive/issue_events_344512.json:
```json
{
    "actor": "https://github.com/strogdon",
    "created_at": "2018-11-12T22:51:41Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26304#event-344512"
}
```



---

archive/issue_events_344513.json:
```json
{
    "actor": "https://github.com/strogdon",
    "created_at": "2018-11-12T22:51:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26304#event-344513"
}
```



---

archive/issue_events_344514.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-01-23T12:27:07Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26304#event-344514"
}
```



---

archive/issue_events_344515.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-01-23T12:27:07Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "milestone_number": null,
    "milestone_title": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26304#event-344515"
}
```



---

archive/issue_comments_408378.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nI'm not sure the `os.path.join`s are necessary here, but I'll have a look.  I also want to squash the commits, but LGTM otherwise.",
    "created_at": "2019-01-23T12:29:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26304#issuecomment-408378",
    "user": "https://github.com/embray"
}
```

<div id="comment:14" align="right">Comment 14</div>

I'm not sure the `os.path.join`s are necessary here, but I'll have a look.  I also want to squash the commits, but LGTM otherwise.



---

archive/issue_comments_408379.json:
```json
{
    "body": "Changed commit from **[ef281aa](https://github.com/sagemath/sagetrac-mirror/commit/ef281aad923f73d6d8db4fd6d1c8335ec6daf39c)** to **[224ab7b](https://github.com/sagemath/sagetrac-mirror/commit/224ab7b2f79670c71b70f7f86b9ce9603a380ba8)**",
    "created_at": "2019-01-23T13:48:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26304#issuecomment-408379",
    "user": "https://github.com/embray"
}
```

Changed commit from **[ef281aa](https://github.com/sagemath/sagetrac-mirror/commit/ef281aad923f73d6d8db4fd6d1c8335ec6daf39c)** to **[224ab7b](https://github.com/sagemath/sagetrac-mirror/commit/224ab7b2f79670c71b70f7f86b9ce9603a380ba8)**



---

archive/issue_comments_408380.json:
```json
{
    "body": "Reviewer: **Erik Bray**",
    "created_at": "2019-01-23T13:48:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26304#issuecomment-408380",
    "user": "https://github.com/embray"
}
```

Reviewer: **Erik Bray**



---

archive/issue_comments_408381.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">Comment 15</div>\n\nFixed up my own nitpicks and made one additional small fix.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/223a1fe3c30e03d546031ee3295b5ac46c97dd2d\">223a1fe</a></td><td><code>Trac #26304: Improve these tests so that they don't fail if a file named</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/224ab7b2f79670c71b70f7f86b9ce9603a380ba8\">224ab7b</a></td><td><code>invert the sense of this check so that load_attach_path can take non-string objects that are nonetheless ducktype-able as paths</code></td></tr></table>\n",
    "created_at": "2019-01-23T13:48:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26304#issuecomment-408381",
    "user": "https://github.com/embray"
}
```

<div id="comment:15" align="right">Comment 15</div>

Fixed up my own nitpicks and made one additional small fix.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/223a1fe3c30e03d546031ee3295b5ac46c97dd2d">223a1fe</a></td><td><code>Trac #26304: Improve these tests so that they don't fail if a file named</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/224ab7b2f79670c71b70f7f86b9ce9603a380ba8">224ab7b</a></td><td><code>invert the sense of this check so that load_attach_path can take non-string objects that are nonetheless ducktype-able as paths</code></td></tr></table>




---

archive/issue_comments_408382.json:
```json
{
    "body": "Changed branch from **[u/strogdon/attach_load_path_fix](https://github.com/sagemath/sagetrac-mirror/tree/u/strogdon/attach_load_path_fix)** to **[public/ticket-26304](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket-26304)**",
    "created_at": "2019-01-23T13:48:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26304#issuecomment-408382",
    "user": "https://github.com/embray"
}
```

Changed branch from **[u/strogdon/attach_load_path_fix](https://github.com/sagemath/sagetrac-mirror/tree/u/strogdon/attach_load_path_fix)** to **[public/ticket-26304](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket-26304)**



---

archive/issue_events_344516.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-01-23T13:48:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26304#event-344516"
}
```



---

archive/issue_events_344517.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-01-23T13:48:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26304#event-344517"
}
```



---

archive/issue_comments_408383.json:
```json
{
    "body": "Changed branch from **[public/ticket-26304](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket-26304)** to **[224ab7b](https://github.com/sagemath/sagetrac-mirror/commit/224ab7b2f79670c71b70f7f86b9ce9603a380ba8)**",
    "created_at": "2019-01-24T18:18:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26304#issuecomment-408383",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/ticket-26304](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket-26304)** to **[224ab7b](https://github.com/sagemath/sagetrac-mirror/commit/224ab7b2f79670c71b70f7f86b9ce9603a380ba8)**



---

archive/issue_events_344518.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-01-24T18:18:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26304#event-344518"
}
```



---

archive/issue_events_344519.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "f22d8d8ef8a6981147da70fdf58fba18297b4394",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-01-24T18:18:02Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26304",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26304#event-344519"
}
```
