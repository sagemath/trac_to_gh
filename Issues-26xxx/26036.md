# Issue 26036: Lie algebra morphism defined by values on a Lie generating subset

archive/issues_025799.json:
```json
{
    "body": "Implementation of a method to define a Lie algebra morphism by giving the images on any Lie-generating subset which need not be the entire basis.\n\nCC:  @tscrim\n\nKeywords: Lie algebras, morphisms\n\nBranch/Commit: ed5b1e9eb2553ea3628a1117101a01cad7446dfc\n\nReviewer: Travis Scrimshaw\n\nAuthor: Eero Hakavuori\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/26036\n\n",
    "closed_at": "2018-09-01T09:09:47Z",
    "created_at": "2018-08-09T16:41:35Z",
    "labels": [
        "component: algebra",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.4",
    "title": "Lie algebra morphism defined by values on a Lie generating subset",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/26036",
    "user": "https://github.com/ehaka"
}
```
Implementation of a method to define a Lie algebra morphism by giving the images on any Lie-generating subset which need not be the entire basis.

CC:  @tscrim

Keywords: Lie algebras, morphisms

Branch/Commit: ed5b1e9eb2553ea3628a1117101a01cad7446dfc

Reviewer: Travis Scrimshaw

Author: Eero Hakavuori

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/26036





---

archive/issue_comments_500293.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2018-08-09T16:45:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26036",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26036#issuecomment-500293",
    "user": "https://github.com/ehaka"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_500294.json:
```json
{
    "body": "<a id='comment:2'></a>\nThank you for implementing this! I will have some minor tweaks, but mostly it looks like a good first step. However, we should make this into an actual `Morphism` subclass (so we can define, e.g., compositions and coercions) and we should add a hook to a finite-dimensional Lie algebra so that it is exposed to the users. It probably a good idea to subclass `LieAlgebraHomomorphism_im_gens` for this. I can do this too if you would want or on a followup.",
    "created_at": "2018-08-09T22:48:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26036",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26036#issuecomment-500294",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:2'></a>
Thank you for implementing this! I will have some minor tweaks, but mostly it looks like a good first step. However, we should make this into an actual `Morphism` subclass (so we can define, e.g., compositions and coercions) and we should add a hook to a finite-dimensional Lie algebra so that it is exposed to the users. It probably a good idea to subclass `LieAlgebraHomomorphism_im_gens` for this. I can do this too if you would want or on a followup.



---

archive/issue_comments_500295.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Travis Scrimshaw\"",
    "created_at": "2018-08-09T22:48:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26036",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26036#issuecomment-500295",
    "user": "https://github.com/tscrim"
}
```

Changing reviewer from "" to "Travis Scrimshaw"



---

archive/issue_comments_500296.json:
```json
{
    "body": "<a id='comment:3'></a>\nThanks for the comment, I am still figuring out the morphism framework, so this is very helpful information. By a hook I take it you mean adding a method named for example `lie_algebra_morphism` in `sage.categories.lie_algebras.LieAlgebras.FiniteDimensional`? I will add this and change `LieAlgebraMorphism_from_generators` to a subclass of `LieAlgebraHomomorphism_im_gens` as you suggested.\n\nOn another note, I noticed that the test suites skip `_test_elements`. Would it be valid to append\n\n```python\nif not issubclass(self.__class__, parent._abstract_element_class):\n    self.__class__ = parent.__make_element_class__(self.__class__)\n```\nat the end of initialization as in `sage.modules.with_basis.morphism.ModuleMorphism.__init__` to make the morphisms proper subclasses of the parent Hom-space, or would this cause some problems?",
    "created_at": "2018-08-10T03:58:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26036",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26036#issuecomment-500296",
    "user": "https://github.com/ehaka"
}
```

<a id='comment:3'></a>
Thanks for the comment, I am still figuring out the morphism framework, so this is very helpful information. By a hook I take it you mean adding a method named for example `lie_algebra_morphism` in `sage.categories.lie_algebras.LieAlgebras.FiniteDimensional`? I will add this and change `LieAlgebraMorphism_from_generators` to a subclass of `LieAlgebraHomomorphism_im_gens` as you suggested.

On another note, I noticed that the test suites skip `_test_elements`. Would it be valid to append

```python
if not issubclass(self.__class__, parent._abstract_element_class):
    self.__class__ = parent.__make_element_class__(self.__class__)
```
at the end of initialization as in `sage.modules.with_basis.morphism.ModuleMorphism.__init__` to make the morphisms proper subclasses of the parent Hom-space, or would this cause some problems?



---

archive/issue_comments_500297.json:
```json
{
    "body": "<a id='comment:4'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                                                                                               |\n|-------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------|\n|[544750b](https://git.sagemath.org/sage.git/commit/?id=544750ba8f8807aeaeb51035a0bcfe48308dc068)|`Converted morphism creator into a subclass and added a hook into the finite dimensional Lie algebras category`|",
    "created_at": "2018-08-10T05:18:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26036",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26036#issuecomment-500297",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:4'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                                                                                               |
|-------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------|
|[544750b](https://git.sagemath.org/sage.git/commit/?id=544750ba8f8807aeaeb51035a0bcfe48308dc068)|`Converted morphism creator into a subclass and added a hook into the finite dimensional Lie algebras category`|



---

archive/issue_comments_500298.json:
```json
{
    "body": "Changing commit from \"084e213d4252162e4382774d36280add4daa901e\" to \"544750ba8f8807aeaeb51035a0bcfe48308dc068\"",
    "created_at": "2018-08-10T05:18:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26036",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26036#issuecomment-500298",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "084e213d4252162e4382774d36280add4daa901e" to "544750ba8f8807aeaeb51035a0bcfe48308dc068"



---

archive/issue_comments_500299.json:
```json
{
    "body": "<a id='comment:5'></a>\nI converted the function to a subclass and added the hook into `sage.categories.lie_algebras.LieAlgebras.FiniteDimensional.ParentMethods`. However, the test suite for `_test_pickling` of the new class fails since the class is creating a parent `LieAlgebraHomset`, which does not have unique behavior. I opened a new ticket #26039 about this, since it seems to be an issue unrelated to the new code.",
    "created_at": "2018-08-10T05:44:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26036",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26036#issuecomment-500299",
    "user": "https://github.com/ehaka"
}
```

<a id='comment:5'></a>
I converted the function to a subclass and added the hook into `sage.categories.lie_algebras.LieAlgebras.FiniteDimensional.ParentMethods`. However, the test suite for `_test_pickling` of the new class fails since the class is creating a parent `LieAlgebraHomset`, which does not have unique behavior. I opened a new ticket #26039 about this, since it seems to be an issue unrelated to the new code.



---

archive/issue_comments_500300.json:
```json
{
    "body": "<a id='comment:6'></a>\nThank you for making it into a class. I am planning to change the hook (yes, you interpreted correctly) to just `morphism` since we already know it is a Lie algebra. :)\n\nAs I mentioned on #26039, you should be using `Hom` to create the parent, not directly constructing `LieAlgebraHomset` (I know that is a stupid pitfall, but there are [annoying] reasons for it). I will be doing some reviewer modifications now, including fixing that. (Also, the morphism framework probably could use some larger scale refactoring now that we have been adding more types of morphisms, but that is my opinion and a large-scale project.)\n\nIn regards to your question about setting the `__class__`, that might work, but I would be very worried that will cause problems when the homset does not have an explicit `Element` set (because it can have many different types of elements).",
    "created_at": "2018-08-10T06:18:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26036",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26036#issuecomment-500300",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:6'></a>
Thank you for making it into a class. I am planning to change the hook (yes, you interpreted correctly) to just `morphism` since we already know it is a Lie algebra. :)

As I mentioned on #26039, you should be using `Hom` to create the parent, not directly constructing `LieAlgebraHomset` (I know that is a stupid pitfall, but there are [annoying] reasons for it). I will be doing some reviewer modifications now, including fixing that. (Also, the morphism framework probably could use some larger scale refactoring now that we have been adding more types of morphisms, but that is my opinion and a large-scale project.)

In regards to your question about setting the `__class__`, that might work, but I would be very worried that will cause problems when the homset does not have an explicit `Element` set (because it can have many different types of elements).



---

archive/issue_comments_500301.json:
```json
{
    "body": "Changing branch from \"u/gh-ehaka/lie_morphism_on_gens\" to \"public/lie_algebras/morphism_on_generators-26036\"",
    "created_at": "2018-08-10T10:48:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26036",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26036#issuecomment-500301",
    "user": "https://github.com/tscrim"
}
```

Changing branch from "u/gh-ehaka/lie_morphism_on_gens" to "public/lie_algebras/morphism_on_generators-26036"



---

archive/issue_comments_500302.json:
```json
{
    "body": "Changing commit from \"544750ba8f8807aeaeb51035a0bcfe48308dc068\" to \"4d9b050261bfe024177107a028a4bcff5000352f\"",
    "created_at": "2018-08-10T10:48:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26036",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26036#issuecomment-500302",
    "user": "https://github.com/tscrim"
}
```

Changing commit from "544750ba8f8807aeaeb51035a0bcfe48308dc068" to "4d9b050261bfe024177107a028a4bcff5000352f"



---

archive/issue_comments_500303.json:
```json
{
    "body": "<a id='comment:7'></a>\nOkay, this fixes the pickling issue(s), as well as gives a better interface. Now you can also use the morphism on you Lie algebra elements. If my changes are good, then you can set a positive review.\n\n---\nNew commits:\n|                                                                                                                                          |                                                                                       |\n|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------|\n|[24d0fd7](https://git.sagemath.org/sage.git/commit?id=24d0fd77f24ef0579538ab5ebf34d945f87ea62b)|`Renaming and moving lie_algebra_morphism() to morphism(); fixing some other problems.`|\n|[3c34d3d](https://git.sagemath.org/sage.git/commit?id=3c34d3d17ebcd6ac8acf455ca52011e10b539ce8)|`Removed trailing whitespace.`|\n|[bfe5c97](https://git.sagemath.org/sage.git/commit?id=bfe5c970cb94fd3175faaa3ef9b40bdf8e5b3474)|`Making the Homset be able to construct these morphisms and other improvements.`|\n|[4d9b050](https://git.sagemath.org/sage.git/commit?id=4d9b050261bfe024177107a028a4bcff5000352f)|`Add richcmp to morphisms (rather than use the default).`|",
    "created_at": "2018-08-10T10:48:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26036",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26036#issuecomment-500303",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:7'></a>
Okay, this fixes the pickling issue(s), as well as gives a better interface. Now you can also use the morphism on you Lie algebra elements. If my changes are good, then you can set a positive review.

---
New commits:
|                                                                                                                                          |                                                                                       |
|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------|
|[24d0fd7](https://git.sagemath.org/sage.git/commit?id=24d0fd77f24ef0579538ab5ebf34d945f87ea62b)|`Renaming and moving lie_algebra_morphism() to morphism(); fixing some other problems.`|
|[3c34d3d](https://git.sagemath.org/sage.git/commit?id=3c34d3d17ebcd6ac8acf455ca52011e10b539ce8)|`Removed trailing whitespace.`|
|[bfe5c97](https://git.sagemath.org/sage.git/commit?id=bfe5c970cb94fd3175faaa3ef9b40bdf8e5b3474)|`Making the Homset be able to construct these morphisms and other improvements.`|
|[4d9b050](https://git.sagemath.org/sage.git/commit?id=4d9b050261bfe024177107a028a4bcff5000352f)|`Add richcmp to morphisms (rather than use the default).`|



---

archive/issue_comments_500304.json:
```json
{
    "body": "Changing commit from \"4d9b050261bfe024177107a028a4bcff5000352f\" to \"ed5b1e9eb2553ea3628a1117101a01cad7446dfc\"",
    "created_at": "2018-08-10T13:08:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26036",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26036#issuecomment-500304",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "4d9b050261bfe024177107a028a4bcff5000352f" to "ed5b1e9eb2553ea3628a1117101a01cad7446dfc"



---

archive/issue_comments_500305.json:
```json
{
    "body": "<a id='comment:8'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                                                                      |\n|-------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------|\n|[ed5b1e9](https://git.sagemath.org/sage.git/commit/?id=ed5b1e9eb2553ea3628a1117101a01cad7446dfc)|`Fixed doctest to refer to new name of function and new punctuation in error message.`|",
    "created_at": "2018-08-10T13:08:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26036",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26036#issuecomment-500305",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:8'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                                                                      |
|-------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------|
|[ed5b1e9](https://git.sagemath.org/sage.git/commit/?id=ed5b1e9eb2553ea3628a1117101a01cad7446dfc)|`Fixed doctest to refer to new name of function and new punctuation in error message.`|



---

archive/issue_comments_500306.json:
```json
{
    "body": "<a id='comment:9'></a>\nLooks good to me. I fixed the doctest in `finite_dimensional_lie_algebras_with_basis` to use the correct method name and the punctuation change in the error message.",
    "created_at": "2018-08-10T13:12:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26036",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26036#issuecomment-500306",
    "user": "https://github.com/ehaka"
}
```

<a id='comment:9'></a>
Looks good to me. I fixed the doctest in `finite_dimensional_lie_algebras_with_basis` to use the correct method name and the punctuation change in the error message.



---

archive/issue_comments_500307.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2018-08-10T13:12:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26036",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26036#issuecomment-500307",
    "user": "https://github.com/ehaka"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_500308.json:
```json
{
    "body": "<a id='comment:10'></a>\nWhoops, forgot to test that file too. Thank you!",
    "created_at": "2018-08-10T22:05:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26036",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26036#issuecomment-500308",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:10'></a>
Whoops, forgot to test that file too. Thank you!



---

archive/issue_comments_500309.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2018-09-01T09:09:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26036",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26036#issuecomment-500309",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_events_065036.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-09-01T09:09:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/26036",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/26036#event-65036"
}
```



---

archive/issue_comments_500310.json:
```json
{
    "body": "Changing branch from \"public/lie_algebras/morphism_on_generators-26036\" to \"ed5b1e9eb2553ea3628a1117101a01cad7446dfc\"",
    "created_at": "2018-09-01T09:09:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26036",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26036#issuecomment-500310",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "public/lie_algebras/morphism_on_generators-26036" to "ed5b1e9eb2553ea3628a1117101a01cad7446dfc"
