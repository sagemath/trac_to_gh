# Issue 26674: py3: fix doctests in weak dict

archive/issues_026437.json:
```json
{
    "assignees": [],
    "body": "where hash should be int\n\nCC:  @tscrim\n\nBranch/Commit: **[810e6a8](https://github.com/sagemath/sagetrac-mirror/commit/810e6a8030417a81f400f3c00f5e0087d21ec856)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26674_\n\n",
    "closed_at": "2018-11-12T21:11:24Z",
    "created_at": "2018-11-10T13:56:55Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20python3",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "py3: fix doctests in weak dict",
    "type": "issue",
    "updated_at": "2018-11-12T21:11:24Z",
    "url": "https://github.com/sagemath/sage/issues/26674",
    "user": "https://github.com/fchapoton"
}
```
where hash should be int

CC:  @tscrim

Branch/Commit: **[810e6a8](https://github.com/sagemath/sagetrac-mirror/commit/810e6a8030417a81f400f3c00f5e0087d21ec856)**

Reviewer: **Travis Scrimshaw**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/26674_





---

archive/issue_events_336392.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-11-10T13:56:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26674",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26674#event-336392"
}
```



---

archive/issue_events_336393.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-11-10T13:56:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26674",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20python3",
    "label_color": "000080",
    "label_name": "component: python3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26674#event-336393"
}
```



---

archive/issue_events_336394.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-11-10T13:56:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26674",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26674#event-336394"
}
```



---

archive/issue_events_336395.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-11-10T13:56:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26674",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26674#event-336395"
}
```



---

archive/issue_comments_413817.json:
```json
{
    "body": "Commit: **[38b8e5b](https://github.com/sagemath/sagetrac-mirror/commit/38b8e5b07c4201920b3042fc19c51df3911ecb02)**",
    "created_at": "2018-11-10T13:57:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26674",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26674#issuecomment-413817",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[38b8e5b](https://github.com/sagemath/sagetrac-mirror/commit/38b8e5b07c4201920b3042fc19c51df3911ecb02)**



---

archive/issue_comments_413818.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9135406961383c752e3a2ffc47d305070ce5baf2\">9135406</a></td><td><code>improve determinism in Huffman codings</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9e3f7bc7fb03a1dba8676aa464915bf2fdbcaeb8\">9e3f7bc</a></td><td><code>different algorithm without using heap queues</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7c2de4ffc0fdcb9c8c40f0490845a4b1778620db\">7c2de4f</a></td><td><code>remove stray line in test</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d41dbe25f3480c7853556c012bde80c15741334a\">d41dbe2</a></td><td><code>explicitly raise an error when trying to make a Huffman code for fewer than 2 symbols (for which the code in this class breaks)</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1c94d3cd81846a41acbe59414957b77ce604e702\">1c94d3c</a></td><td><code>Merge branch 'u/embray/python3/sage-coding-source_coding-huffman' in 8.5.b2</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e913f5df63139d7e1000adf3dcf7aa51deddeb1f\">e913f5d</a></td><td><code>trac 26026 fix trivial failing doctest</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/38b8e5b07c4201920b3042fc19c51df3911ecb02\">38b8e5b</a></td><td><code>py3 fix doctests in weak dict</code></td></tr></table>\n",
    "created_at": "2018-11-10T13:57:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26674",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26674#issuecomment-413818",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9135406961383c752e3a2ffc47d305070ce5baf2">9135406</a></td><td><code>improve determinism in Huffman codings</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9e3f7bc7fb03a1dba8676aa464915bf2fdbcaeb8">9e3f7bc</a></td><td><code>different algorithm without using heap queues</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7c2de4ffc0fdcb9c8c40f0490845a4b1778620db">7c2de4f</a></td><td><code>remove stray line in test</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d41dbe25f3480c7853556c012bde80c15741334a">d41dbe2</a></td><td><code>explicitly raise an error when trying to make a Huffman code for fewer than 2 symbols (for which the code in this class breaks)</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1c94d3cd81846a41acbe59414957b77ce604e702">1c94d3c</a></td><td><code>Merge branch 'u/embray/python3/sage-coding-source_coding-huffman' in 8.5.b2</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e913f5df63139d7e1000adf3dcf7aa51deddeb1f">e913f5d</a></td><td><code>trac 26026 fix trivial failing doctest</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/38b8e5b07c4201920b3042fc19c51df3911ecb02">38b8e5b</a></td><td><code>py3 fix doctests in weak dict</code></td></tr></table>




---

archive/issue_events_336396.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-11-10T13:57:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26674",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26674#event-336396"
}
```



---

archive/issue_comments_413819.json:
```json
{
    "body": "Branch: **[u/chapoton/26674](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/26674)**",
    "created_at": "2018-11-10T13:57:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26674",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26674#issuecomment-413819",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/26674](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/26674)**



---

archive/issue_comments_413820.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/810e6a8030417a81f400f3c00f5e0087d21ec856\">810e6a8</a></td><td><code>py3 fix doctests in weak dict</code></td></tr></table>\n",
    "created_at": "2018-11-10T13:58:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26674",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26674#issuecomment-413820",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/810e6a8030417a81f400f3c00f5e0087d21ec856">810e6a8</a></td><td><code>py3 fix doctests in weak dict</code></td></tr></table>




---

archive/issue_comments_413821.json:
```json
{
    "body": "Changed commit from **[38b8e5b](https://github.com/sagemath/sagetrac-mirror/commit/38b8e5b07c4201920b3042fc19c51df3911ecb02)** to **[810e6a8](https://github.com/sagemath/sagetrac-mirror/commit/810e6a8030417a81f400f3c00f5e0087d21ec856)**",
    "created_at": "2018-11-10T13:58:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26674",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26674#issuecomment-413821",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[38b8e5b](https://github.com/sagemath/sagetrac-mirror/commit/38b8e5b07c4201920b3042fc19c51df3911ecb02)** to **[810e6a8](https://github.com/sagemath/sagetrac-mirror/commit/810e6a8030417a81f400f3c00f5e0087d21ec856)**



---

archive/issue_comments_413822.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nbot is morally green, please review",
    "created_at": "2018-11-10T21:07:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26674",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26674#issuecomment-413822",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:3'>Comment 3:</a>
bot is morally green, please review



---

archive/issue_comments_413823.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2018-11-11T12:53:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26674",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26674#issuecomment-413823",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_336397.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-11-11T12:53:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26674",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26674#event-336397"
}
```



---

archive/issue_events_336398.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-11-11T12:53:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26674",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26674#event-336398"
}
```



---

archive/issue_comments_413824.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nLGTM.",
    "created_at": "2018-11-11T12:53:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26674",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26674#issuecomment-413824",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:4'>Comment 4:</a>
LGTM.



---

archive/issue_events_336399.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-11-12T21:11:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26674",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26674#event-336399"
}
```



---

archive/issue_events_336400.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "dee1f88fef50623bf260f6cc98682f29aa718c9d",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-11-12T21:11:24Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26674",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26674#event-336400"
}
```



---

archive/issue_comments_413825.json:
```json
{
    "body": "Changed branch from **[u/chapoton/26674](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/26674)** to **[810e6a8](https://github.com/sagemath/sagetrac-mirror/commit/810e6a8030417a81f400f3c00f5e0087d21ec856)**",
    "created_at": "2018-11-12T21:11:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26674",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26674#issuecomment-413825",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/26674](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/26674)** to **[810e6a8](https://github.com/sagemath/sagetrac-mirror/commit/810e6a8030417a81f400f3c00f5e0087d21ec856)**
