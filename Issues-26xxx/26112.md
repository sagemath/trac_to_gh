# Issue 26112: Implement finite complex reflection groups G(m,p,n) as permutation groups

archive/issues_025875.json:
```json
{
    "assignees": [],
    "body": "We can use an m-copy folding of S<sub>n</sub> plus a connector to obtain G(m,1,n) and then an embedding of G(m,p,n) into G(m,1,n) to construct these groups as permutation groups.\n\nCC:  stumpc5\n\nKeywords: **complex reflection group**\n\nBranch/Commit: **[890c53b](https://github.com/sagemath/sagetrac-mirror/commit/890c53b47c54b94ecfc7bc5fb5c92c6db86c5392)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26112_\n\n",
    "closed_at": "2018-12-23T23:41:04Z",
    "created_at": "2018-08-23T05:53:37Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20group%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Implement finite complex reflection groups G(m,p,n) as permutation groups",
    "type": "issue",
    "updated_at": "2018-12-23T23:41:04Z",
    "url": "https://github.com/sagemath/sage/issues/26112",
    "user": "https://github.com/tscrim"
}
```
We can use an m-copy folding of S<sub>n</sub> plus a connector to obtain G(m,1,n) and then an embedding of G(m,p,n) into G(m,1,n) to construct these groups as permutation groups.

CC:  stumpc5

Keywords: **complex reflection group**

Branch/Commit: **[890c53b](https://github.com/sagemath/sagetrac-mirror/commit/890c53b47c54b94ecfc7bc5fb5c92c6db86c5392)**

Reviewer: **Frédéric Chapoton**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/26112_





---

archive/issue_events_342403.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-08-23T05:53:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26112#event-342403"
}
```



---

archive/issue_events_342404.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-08-23T05:53:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20group%20theory",
    "label_color": "0000ff",
    "label_name": "component: group theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26112#event-342404"
}
```



---

archive/issue_events_342405.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-08-23T05:53:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26112#event-342405"
}
```



---

archive/issue_events_342406.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-08-23T05:53:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26112#event-342406"
}
```



---

archive/issue_comments_405110.json:
```json
{
    "body": "Branch: **[public/group_theory/complex_refl_gp_perm_gp-26112](https://github.com/sagemath/sagetrac-mirror/tree/public/group_theory/complex_refl_gp_perm_gp-26112)**",
    "created_at": "2018-08-23T05:54:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26112#issuecomment-405110",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/group_theory/complex_refl_gp_perm_gp-26112](https://github.com/sagemath/sagetrac-mirror/tree/public/group_theory/complex_refl_gp_perm_gp-26112)**



---

archive/issue_comments_405111.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4378d9a0c80014be19be4bba03bb3dddeea824fc\">4378d9a</a></td><td><code>Implementing G(m,p,n) as a permutation group.</code></td></tr></table>\n",
    "created_at": "2018-08-23T05:54:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26112#issuecomment-405111",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4378d9a0c80014be19be4bba03bb3dddeea824fc">4378d9a</a></td><td><code>Implementing G(m,p,n) as a permutation group.</code></td></tr></table>




---

archive/issue_comments_405112.json:
```json
{
    "body": "Commit: **[4378d9a](https://github.com/sagemath/sagetrac-mirror/commit/4378d9a0c80014be19be4bba03bb3dddeea824fc)**",
    "created_at": "2018-08-23T05:54:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26112#issuecomment-405112",
    "user": "https://github.com/tscrim"
}
```

Commit: **[4378d9a](https://github.com/sagemath/sagetrac-mirror/commit/4378d9a0c80014be19be4bba03bb3dddeea824fc)**



---

archive/issue_events_342407.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-08-23T05:54:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26112#event-342407"
}
```



---

archive/issue_comments_405113.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\n\n```\nsage -t --long src/sage/combinat/colored_permutations.py\n**********************************************************************\nFile \"src/sage/combinat/colored_permutations.py\", line 636, in sage.combinat.colored_permutations.ColoredPermutations.as_permutation_group\nFailed example:\n    C.as_permutation_group()\nExpected:\n    Permutation Group with generators [(2,3)(5,6)(8,9)(11,12), (1,2)(4,5)(7,8)(10,11), (1,4,7,10)]\nGot:\n    Complex reflection group G(4, 1, 3) as a permutation group\n```",
    "created_at": "2018-08-23T09:58:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26112#issuecomment-405113",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:2'>Comment 2:</a>

```
sage -t --long src/sage/combinat/colored_permutations.py
**********************************************************************
File "src/sage/combinat/colored_permutations.py", line 636, in sage.combinat.colored_permutations.ColoredPermutations.as_permutation_group
Failed example:
    C.as_permutation_group()
Expected:
    Permutation Group with generators [(2,3)(5,6)(8,9)(11,12), (1,2)(4,5)(7,8)(10,11), (1,4,7,10)]
Got:
    Complex reflection group G(4, 1, 3) as a permutation group
```



---

archive/issue_comments_405114.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7934f2044f530d24b838b3b5dfb09842e523691b\">7934f20</a></td><td><code>Implementing G(m,p,n) as a permutation group.</code></td></tr></table>\n",
    "created_at": "2018-08-23T23:01:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26112#issuecomment-405114",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:3'>Comment 3:</a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7934f2044f530d24b838b3b5dfb09842e523691b">7934f20</a></td><td><code>Implementing G(m,p,n) as a permutation group.</code></td></tr></table>




---

archive/issue_comments_405115.json:
```json
{
    "body": "Changed commit from **[4378d9a](https://github.com/sagemath/sagetrac-mirror/commit/4378d9a0c80014be19be4bba03bb3dddeea824fc)** to **[7934f20](https://github.com/sagemath/sagetrac-mirror/commit/7934f2044f530d24b838b3b5dfb09842e523691b)**",
    "created_at": "2018-08-23T23:01:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26112#issuecomment-405115",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[4378d9a](https://github.com/sagemath/sagetrac-mirror/commit/4378d9a0c80014be19be4bba03bb3dddeea824fc)** to **[7934f20](https://github.com/sagemath/sagetrac-mirror/commit/7934f2044f530d24b838b3b5dfb09842e523691b)**



---

archive/issue_comments_405116.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nWhoops, forgot to change that. Fixed.",
    "created_at": "2018-08-23T23:01:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26112#issuecomment-405116",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:4'>Comment 4:</a>
Whoops, forgot to change that. Fixed.



---

archive/issue_comments_405117.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nI am a little puzzles with this implementation. What is a complex reflection group for you?\n\nBy definition, it is a subgroup of GL(V) generated by complex reflections. For me, you implement subgroups of the group of colored permutations where the sums of the colors satisfy a certain divisibility.\n\nAlso, what about the exceptional complex reflection groups? Shouldn't an class called `ComplexReflectionGroup` be able to initiate any complex reflection group?",
    "created_at": "2018-08-24T13:06:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26112#issuecomment-405117",
    "user": "https://github.com/sagetrac-stumpc5"
}
```

<a id='comment:5'>Comment 5:</a>
I am a little puzzles with this implementation. What is a complex reflection group for you?

By definition, it is a subgroup of GL(V) generated by complex reflections. For me, you implement subgroups of the group of colored permutations where the sums of the colors satisfy a certain divisibility.

Also, what about the exceptional complex reflection groups? Shouldn't an class called `ComplexReflectionGroup` be able to initiate any complex reflection group?



---

archive/issue_comments_405118.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/934c45bae26dcc569c05f20ef8de2c961106d661\">934c45b</a></td><td><code>Adding exceptional CRGs to API but currently raises a NotImplementedError.</code></td></tr></table>\n",
    "created_at": "2018-08-24T21:53:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26112#issuecomment-405118",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:6'>Comment 6:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/934c45bae26dcc569c05f20ef8de2c961106d661">934c45b</a></td><td><code>Adding exceptional CRGs to API but currently raises a NotImplementedError.</code></td></tr></table>




---

archive/issue_comments_405119.json:
```json
{
    "body": "Changed commit from **[7934f20](https://github.com/sagemath/sagetrac-mirror/commit/7934f2044f530d24b838b3b5dfb09842e523691b)** to **[934c45b](https://github.com/sagemath/sagetrac-mirror/commit/934c45bae26dcc569c05f20ef8de2c961106d661)**",
    "created_at": "2018-08-24T21:53:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26112#issuecomment-405119",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[7934f20](https://github.com/sagemath/sagetrac-mirror/commit/7934f2044f530d24b838b3b5dfb09842e523691b)** to **[934c45b](https://github.com/sagemath/sagetrac-mirror/commit/934c45bae26dcc569c05f20ef8de2c961106d661)**



---

archive/issue_comments_405120.json:
```json
{
    "body": "Changed commit from **[934c45b](https://github.com/sagemath/sagetrac-mirror/commit/934c45bae26dcc569c05f20ef8de2c961106d661)** to **[c6d8df2](https://github.com/sagemath/sagetrac-mirror/commit/c6d8df2e908984452cec6286152bbe788f44b6a7)**",
    "created_at": "2018-08-24T21:56:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26112#issuecomment-405120",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[934c45b](https://github.com/sagemath/sagetrac-mirror/commit/934c45bae26dcc569c05f20ef8de2c961106d661)** to **[c6d8df2](https://github.com/sagemath/sagetrac-mirror/commit/c6d8df2e908984452cec6286152bbe788f44b6a7)**



---

archive/issue_comments_405121.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c6d8df2e908984452cec6286152bbe788f44b6a7\">c6d8df2</a></td><td><code>A little bit more documentation.</code></td></tr></table>\n",
    "created_at": "2018-08-24T21:56:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26112#issuecomment-405121",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:7'>Comment 7:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c6d8df2e908984452cec6286152bbe788f44b6a7">c6d8df2</a></td><td><code>A little bit more documentation.</code></td></tr></table>




---

archive/issue_comments_405122.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nI made it so the API for `ComplexReflectionGroup` explicitly declares it will one day be able to handle an exceptional CRG, but currently raises a `NotImplementedError`.",
    "created_at": "2018-08-24T21:58:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26112#issuecomment-405122",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:8'>Comment 8:</a>
I made it so the API for `ComplexReflectionGroup` explicitly declares it will one day be able to handle an exceptional CRG, but currently raises a `NotImplementedError`.



---

archive/issue_comments_405123.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\ntypo: `The convenion`\n\nalso there is a \"degrees\" in the doc of codegrees\n\nOtherwise looks good",
    "created_at": "2018-12-15T08:10:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26112#issuecomment-405123",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:9'>Comment 9:</a>
typo: `The convenion`

also there is a "degrees" in the doc of codegrees

Otherwise looks good



---

archive/issue_comments_405124.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f4c35aee88d1bd1f76358f62609413288dd483ef\">f4c35ae</a></td><td><code>Merge branch 'public/group_theory/complex_refl_gp_perm_gp-26112' of git://trac.sagemath.org/sage into public/group_theory/complex_refl_gp_perm_gp-26112</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/890c53b47c54b94ecfc7bc5fb5c92c6db86c5392\">890c53b</a></td><td><code>Fixing typos.</code></td></tr></table>\n",
    "created_at": "2018-12-15T08:26:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26112#issuecomment-405124",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:10'>Comment 10:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f4c35aee88d1bd1f76358f62609413288dd483ef">f4c35ae</a></td><td><code>Merge branch 'public/group_theory/complex_refl_gp_perm_gp-26112' of git://trac.sagemath.org/sage into public/group_theory/complex_refl_gp_perm_gp-26112</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/890c53b47c54b94ecfc7bc5fb5c92c6db86c5392">890c53b</a></td><td><code>Fixing typos.</code></td></tr></table>




---

archive/issue_comments_405125.json:
```json
{
    "body": "Changed commit from **[c6d8df2](https://github.com/sagemath/sagetrac-mirror/commit/c6d8df2e908984452cec6286152bbe788f44b6a7)** to **[890c53b](https://github.com/sagemath/sagetrac-mirror/commit/890c53b47c54b94ecfc7bc5fb5c92c6db86c5392)**",
    "created_at": "2018-12-15T08:26:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26112#issuecomment-405125",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[c6d8df2](https://github.com/sagemath/sagetrac-mirror/commit/c6d8df2e908984452cec6286152bbe788f44b6a7)** to **[890c53b](https://github.com/sagemath/sagetrac-mirror/commit/890c53b47c54b94ecfc7bc5fb5c92c6db86c5392)**



---

archive/issue_comments_405126.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nFixed.",
    "created_at": "2018-12-15T08:27:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26112#issuecomment-405126",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:11'>Comment 11:</a>
Fixed.



---

archive/issue_events_342408.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-12-15T08:27:20Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26112#event-342408"
}
```



---

archive/issue_events_342409.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-12-15T08:27:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "milestone_number": null,
    "milestone_title": "sage-8.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26112#event-342409"
}
```



---

archive/issue_comments_405127.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nok, let it be",
    "created_at": "2018-12-15T12:27:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26112#issuecomment-405127",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:12'>Comment 12:</a>
ok, let it be



---

archive/issue_comments_405128.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2018-12-15T12:27:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26112#issuecomment-405128",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_events_342410.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-12-15T12:27:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26112#event-342410"
}
```



---

archive/issue_events_342411.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-12-15T12:27:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26112#event-342411"
}
```



---

archive/issue_events_342412.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-12-23T23:41:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26112#event-342412"
}
```



---

archive/issue_events_342413.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "9c89911b604120ef82ad0bccc5364448dd43748d",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-12-23T23:41:04Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26112#event-342413"
}
```



---

archive/issue_comments_405129.json:
```json
{
    "body": "Changed branch from **[public/group_theory/complex_refl_gp_perm_gp-26112](https://github.com/sagemath/sagetrac-mirror/tree/public/group_theory/complex_refl_gp_perm_gp-26112)** to **[890c53b](https://github.com/sagemath/sagetrac-mirror/commit/890c53b47c54b94ecfc7bc5fb5c92c6db86c5392)**",
    "created_at": "2018-12-23T23:41:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26112",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26112#issuecomment-405129",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/group_theory/complex_refl_gp_perm_gp-26112](https://github.com/sagemath/sagetrac-mirror/tree/public/group_theory/complex_refl_gp_perm_gp-26112)** to **[890c53b](https://github.com/sagemath/sagetrac-mirror/commit/890c53b47c54b94ecfc7bc5fb5c92c6db86c5392)**
