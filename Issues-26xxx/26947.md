# Issue 26947: UnionOfIntervals.__contains__ doctest tests str

archive/issues_026710.json:
```json
{
    "assignees": [],
    "body": "It is documented that the input of `__contains__` should be a real number, yet there is this doctest:\n\n```\n            sage: 'a' in A\n            False\n```\nThis will break after #22029.\n\nSince this test is meaningless, it should just be removed.\n\nCC:  @JohnCremona\n\nBranch/Commit: **[6a8b25b](https://github.com/sagemath/sagetrac-mirror/commit/6a8b25b9462acd4a0b609351d66402888549d527)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26947_\n\n",
    "closed_at": "2018-12-30T09:15:30Z",
    "created_at": "2018-12-23T14:52:34Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20coercion",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "UnionOfIntervals.__contains__ doctest tests str",
    "type": "issue",
    "updated_at": "2018-12-30T09:15:30Z",
    "url": "https://github.com/sagemath/sage/issues/26947",
    "user": "https://github.com/jdemeyer"
}
```
It is documented that the input of `__contains__` should be a real number, yet there is this doctest:

```
            sage: 'a' in A
            False
```
This will break after #22029.

Since this test is meaningless, it should just be removed.

CC:  @JohnCremona

Branch/Commit: **[6a8b25b](https://github.com/sagemath/sagetrac-mirror/commit/6a8b25b9462acd4a0b609351d66402888549d527)**

Reviewer: **Frédéric Chapoton**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/26947_





---

archive/issue_events_352135.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-12-23T14:52:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26947",
    "milestone_number": null,
    "milestone_title": "sage-8.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26947#event-352135"
}
```



---

archive/issue_events_352136.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-12-23T14:52:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26947",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20coercion",
    "label_color": "0000ff",
    "label_name": "component: coercion",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26947#event-352136"
}
```



---

archive/issue_events_352137.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-12-23T14:52:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26947",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26947#event-352137"
}
```



---

archive/issue_events_352138.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-12-23T14:52:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26947",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26947#event-352138"
}
```



---

archive/issue_comments_418299.json:
```json
{
    "body": "Branch: **[u/jdemeyer/unionofintervals___contains___doctest_tests_str](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/unionofintervals___contains___doctest_tests_str)**",
    "created_at": "2018-12-23T14:59:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26947",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26947#issuecomment-418299",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/unionofintervals___contains___doctest_tests_str](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/unionofintervals___contains___doctest_tests_str)**



---

archive/issue_comments_418300.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6a8b25b9462acd4a0b609351d66402888549d527\">6a8b25b</a></td><td><code>Remove UnionOfIntervals.__contains__ test with str input</code></td></tr></table>\n",
    "created_at": "2018-12-23T15:00:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26947",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26947#issuecomment-418300",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6a8b25b9462acd4a0b609351d66402888549d527">6a8b25b</a></td><td><code>Remove UnionOfIntervals.__contains__ test with str input</code></td></tr></table>




---

archive/issue_comments_418301.json:
```json
{
    "body": "Commit: **[6a8b25b](https://github.com/sagemath/sagetrac-mirror/commit/6a8b25b9462acd4a0b609351d66402888549d527)**",
    "created_at": "2018-12-23T15:00:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26947",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26947#issuecomment-418301",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[6a8b25b](https://github.com/sagemath/sagetrac-mirror/commit/6a8b25b9462acd4a0b609351d66402888549d527)**



---

archive/issue_comments_418302.json:
```json
{
    "body": "Author: **Jeroen Demeyer**",
    "created_at": "2018-12-23T15:00:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26947",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26947#issuecomment-418302",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Jeroen Demeyer**



---

archive/issue_events_352139.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-12-23T15:00:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26947",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26947#event-352139"
}
```



---

archive/issue_events_352140.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-12-28T17:47:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26947",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26947#event-352140"
}
```



---

archive/issue_events_352141.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-12-28T17:47:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26947",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26947#event-352141"
}
```



---

archive/issue_comments_418303.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nok",
    "created_at": "2018-12-28T17:47:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26947",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26947#issuecomment-418303",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:3'>Comment 3:</a>
ok



---

archive/issue_comments_418304.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2018-12-28T17:47:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26947",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26947#issuecomment-418304",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_events_352142.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-12-30T09:15:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26947",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26947#event-352142"
}
```



---

archive/issue_events_352143.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "3d2fa91b15b616704ad61517c10f03576b0d6c51",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-12-30T09:15:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26947",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26947#event-352143"
}
```



---

archive/issue_comments_418305.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/unionofintervals___contains___doctest_tests_str](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/unionofintervals___contains___doctest_tests_str)** to **[6a8b25b](https://github.com/sagemath/sagetrac-mirror/commit/6a8b25b9462acd4a0b609351d66402888549d527)**",
    "created_at": "2018-12-30T09:15:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26947",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26947#issuecomment-418305",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/unionofintervals___contains___doctest_tests_str](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/unionofintervals___contains___doctest_tests_str)** to **[6a8b25b](https://github.com/sagemath/sagetrac-mirror/commit/6a8b25b9462acd4a0b609351d66402888549d527)**
