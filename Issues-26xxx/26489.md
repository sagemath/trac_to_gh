...
```json
{
    "body": "Somewhere the wrong `libgcc_s` is being loaded first; by the time `libSingular` wants to load the correct one (inside Sage) its too late and the version mismatch occurs.",
