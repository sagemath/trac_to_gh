# Issue 26277: py3: MInor sorting issue with Stanley-Reisner ring

archive/issues_026040.json:
```json
{
    "assignees": [],
    "body": "Fix one doctest in Python 3 by sorting the generators of the polynomial ring used to define the Stanley-Reisner ring of a simplicial complex.\n\nCC:  @fchapoton @tscrim\n\nBranch/Commit: **[694fe31](https://github.com/sagemath/sagetrac-mirror/commit/694fe31ac3289eb1acac580d15ed984fa5a0a08b)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **John Palmieri**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26277_\n\n",
    "closed_at": "2018-09-15T09:04:58Z",
    "created_at": "2018-09-13T19:01:20Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20python3",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "py3: MInor sorting issue with Stanley-Reisner ring",
    "type": "issue",
    "updated_at": "2018-09-15T09:04:58Z",
    "url": "https://github.com/sagemath/sage/issues/26277",
    "user": "https://github.com/jhpalmieri"
}
```
Fix one doctest in Python 3 by sorting the generators of the polynomial ring used to define the Stanley-Reisner ring of a simplicial complex.

CC:  @fchapoton @tscrim

Branch/Commit: **[694fe31](https://github.com/sagemath/sagetrac-mirror/commit/694fe31ac3289eb1acac580d15ed984fa5a0a08b)**

Reviewer: **Frédéric Chapoton**

Author: **John Palmieri**

_Issue created by migration from https://trac.sagemath.org/ticket/26277_





---

archive/issue_events_344203.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2018-09-13T19:01:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26277",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26277#event-344203"
}
```



---

archive/issue_events_344204.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2018-09-13T19:01:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26277",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20python3",
    "label_color": "000080",
    "label_name": "component: python3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26277#event-344204"
}
```



---

archive/issue_events_344205.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2018-09-13T19:01:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26277",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26277#event-344205"
}
```



---

archive/issue_events_344206.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2018-09-13T19:01:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26277",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26277#event-344206"
}
```



---

archive/issue_comments_407985.json:
```json
{
    "body": "Branch: **[u/jhpalmieri/stanley-reisner-sorting](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/stanley-reisner-sorting)**",
    "created_at": "2018-09-13T19:02:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26277",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26277#issuecomment-407985",
    "user": "https://github.com/jhpalmieri"
}
```

Branch: **[u/jhpalmieri/stanley-reisner-sorting](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/stanley-reisner-sorting)**



---

archive/issue_events_344207.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2018-09-13T19:02:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26277",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26277#event-344207"
}
```



---

archive/issue_comments_407986.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/694fe31ac3289eb1acac580d15ed984fa5a0a08b\">694fe31</a></td><td><code>trac 26277: sort the generators in _stanley_reisner_base_ring.</code></td></tr></table>\n",
    "created_at": "2018-09-13T19:02:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26277",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26277#issuecomment-407986",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/694fe31ac3289eb1acac580d15ed984fa5a0a08b">694fe31</a></td><td><code>trac 26277: sort the generators in _stanley_reisner_base_ring.</code></td></tr></table>




---

archive/issue_comments_407987.json:
```json
{
    "body": "Commit: **[694fe31](https://github.com/sagemath/sagetrac-mirror/commit/694fe31ac3289eb1acac580d15ed984fa5a0a08b)**",
    "created_at": "2018-09-13T19:02:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26277",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26277#issuecomment-407987",
    "user": "https://github.com/jhpalmieri"
}
```

Commit: **[694fe31](https://github.com/sagemath/sagetrac-mirror/commit/694fe31ac3289eb1acac580d15ed984fa5a0a08b)**



---

archive/issue_comments_407988.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBy the way, there are other Python 3 failures with Stanley-Reisner rings, of the form\n\n```\nFile \"src/sage/homology/simplicial_complex.py\", line 86, in sage.homology.simplicial_complex\nFailed example:\n    X.stanley_reisner_ring()\nExpected:\n    Quotient of Multivariate Polynomial Ring in x0, x1, x2, x3 over Integer Ring by the ideal (x1*x3, x0*x2)\nGot:\n    Quotient of Multivariate Polynomial Ring in x0, x1, x2, x3 over Integer Ring by the ideal (x0*x2, x1*x3)\n```\nThese have to do with how ideals for polynomial rings are printed, so\u00a0it's coming from code in sage.rings.",
    "created_at": "2018-09-13T19:07:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26277",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26277#issuecomment-407988",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:3" align="right">Comment 3</div>

By the way, there are other Python 3 failures with Stanley-Reisner rings, of the form

```
File "src/sage/homology/simplicial_complex.py", line 86, in sage.homology.simplicial_complex
Failed example:
    X.stanley_reisner_ring()
Expected:
    Quotient of Multivariate Polynomial Ring in x0, x1, x2, x3 over Integer Ring by the ideal (x1*x3, x0*x2)
Got:
    Quotient of Multivariate Polynomial Ring in x0, x1, x2, x3 over Integer Ring by the ideal (x0*x2, x1*x3)
```
These have to do with how ideals for polynomial rings are printed, so it's coming from code in sage.rings.



---

archive/issue_comments_407989.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2018-09-14T06:50:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26277",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26277#issuecomment-407989",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_407990.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nok",
    "created_at": "2018-09-14T06:50:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26277",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26277#issuecomment-407990",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:4" align="right">Comment 4</div>

ok



---

archive/issue_events_344208.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-09-14T06:50:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26277",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26277#event-344208"
}
```



---

archive/issue_events_344209.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-09-14T06:50:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26277",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26277#event-344209"
}
```



---

archive/issue_events_344210.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-09-15T09:04:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26277",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26277#event-344210"
}
```



---

archive/issue_events_344211.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "635333d2dae10b99c6f6de5a13d6f0d98dc8383a",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-09-15T09:04:58Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26277",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26277#event-344211"
}
```



---

archive/issue_comments_407991.json:
```json
{
    "body": "Changed branch from **[u/jhpalmieri/stanley-reisner-sorting](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/stanley-reisner-sorting)** to **[694fe31](https://github.com/sagemath/sagetrac-mirror/commit/694fe31ac3289eb1acac580d15ed984fa5a0a08b)**",
    "created_at": "2018-09-15T09:04:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26277",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26277#issuecomment-407991",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jhpalmieri/stanley-reisner-sorting](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/stanley-reisner-sorting)** to **[694fe31](https://github.com/sagemath/sagetrac-mirror/commit/694fe31ac3289eb1acac580d15ed984fa5a0a08b)**
