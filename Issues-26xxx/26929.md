...
```json
{
    "body": "Seems to be the same in 8.3.",
...
```json
{
    "body": "8.3.beta0 works.  Next trying 8.3.beta4",
...
```json
{
    "body": "8.3.beta4 works, next trying 8.3.beta8",
...
```json
{
    "body": "8.3.beta8 works, next trying 8.3.rc2",
...
```json
{
    "body": "8.3.rc2 fails, next trying 8.3.rc0",
...
```json
{
    "body": "The parent does not print the `t` when you ask about the string representation but it knows about it. That's normal behaviour for many parents.",
...
```json
{
    "body": "I think the problem is that the type of `PQ` changed from a `PolynomialQuotientRing` to a `QuotientRing`, probably because some operation thrown an error and then the fallback is used.",
...
```json
{
    "body": "deleted comment because of nonsense.",
...
```json
{
    "body": "inherit from `sage.rings.ring.CommutativeRing`, just as `MPolynomialRing_base` does? (and then `MPolynomial(CommutativeRingElement)`)",
...
```json
{
    "body": "Anyway, I guess whatever works is fine.",
...
```json
{
    "body": "I think it would be more correct to adapt `polynomial_quotient_ring.py` for multivariate polynomials.\n\n---\nNew commits:",
...
```json
{
    "body": "In fact, I would like to inherit from `QuotientRing_nc`, but it seems that this is not possible ('not an extension type').  What would be the proper way to do this?  For the moment, I'll just duplicate the definitions, but that's stupid.",
...
```json
{
    "body": "It's a stupid fix, but it seems to work, at least essentially.",
...
```json
{
    "body": "Actually, it doesn't work.  There are failing doctests in `sage.crypto`.",
...
```json
{
    "body": "Yes, that works. This also works by using the `list()` method:\n\n```\nxa = Baff(x.list())\n```\nPreviously, also something like this worked:\n\n```\ny = t^2 + t + 1\nxint = ZZ(y.list(), 2)\n```\nwhich now fails.\nAny idea?",
...
```json
{
    "body": "What should the last statement do?  (I've never seen a two-parameter call to `ZZ`)",
...
```json
{
    "body": "The second parameter gives a base for the transformation of the list of digits given as the first argument. In the example `y.list()` is `[1, 1, 1, 0]` which should be interpreted as bits to give decimal 7 (big endian). See `ZZ?`.",
...
```json
{
    "body": "Yes, I think that's a consequence of the problems with conversion.  The elements of `y.lift().list()` are Boolean polynomials, and with my fix they lost the conversion to `ZZ`.\n\nIt's great and helpful that you tested all this!",
...
```json
{
    "body": "This bug was also [encountered on Ask SageMath](https://ask.sagemath.org/question/45146/declaring-symbols-in-a-field/?answer=45165#post-id-45165). The workaround to restore the old behavior is\n\n```\nfrom sage.rings.polynomial.polynomial_quotient_ring import PolynomialQuotientRing_generic\nPQ = PolynomialQuotientRing_generic(P, X^4+X+1, 't')\nt = PQ.gen()\n```\n\nbut of course the underlying issue should be addressed.",
...
```json
{
    "body": "Is there any interest in this ticket?  Should I rebase?",
...
```json
{
    "body": "Current branch has conflict with the current develop branch of Sage.",
