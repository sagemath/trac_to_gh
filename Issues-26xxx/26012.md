# Issue 26012: Fix bug concerning restrictions of tensorfields

archive/issues_025775.json:
```json
{
    "assignees": [],
    "body": "The attributes `_restrictions_graph` and `_extensions_graph`, introduced in #25417 were not affected by the method `_del_derived`, whereas `_restrictions` was, causing some issues.\n\nCC:  @egourgoulhon\n\nKeywords: **restriction, extension, tensorfield, manifold**\n\nBranch/Commit: **[043f084](https://github.com/sagemath/sagetrac-mirror/commit/043f08436dd36cd853d47a3855a260afd19420b5)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Florentin Jaffredo**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26012_\n\n",
    "closed_at": "2018-08-09T21:27:32Z",
    "created_at": "2018-08-06T15:19:49Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix bug concerning restrictions of tensorfields",
    "type": "issue",
    "updated_at": "2018-08-09T21:27:32Z",
    "url": "https://github.com/sagemath/sage/issues/26012",
    "user": "https://github.com/FlorentinJ"
}
```
The attributes `_restrictions_graph` and `_extensions_graph`, introduced in #25417 were not affected by the method `_del_derived`, whereas `_restrictions` was, causing some issues.

CC:  @egourgoulhon

Keywords: **restriction, extension, tensorfield, manifold**

Branch/Commit: **[043f084](https://github.com/sagemath/sagetrac-mirror/commit/043f08436dd36cd853d47a3855a260afd19420b5)**

Reviewer: **Travis Scrimshaw**

Author: **Florentin Jaffredo**

_Issue created by migration from https://trac.sagemath.org/ticket/26012_





---

archive/issue_events_341235.json:
```json
{
    "actor": "https://github.com/FlorentinJ",
    "created_at": "2018-08-06T15:19:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26012",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26012#event-341235"
}
```



---

archive/issue_events_341236.json:
```json
{
    "actor": "https://github.com/FlorentinJ",
    "created_at": "2018-08-06T15:19:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26012",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "component: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26012#event-341236"
}
```



---

archive/issue_events_341237.json:
```json
{
    "actor": "https://github.com/FlorentinJ",
    "created_at": "2018-08-06T15:19:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26012",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26012#event-341237"
}
```



---

archive/issue_events_341238.json:
```json
{
    "actor": "https://github.com/FlorentinJ",
    "created_at": "2018-08-06T15:19:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26012",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26012#event-341238"
}
```



---

archive/issue_events_341239.json:
```json
{
    "actor": "https://github.com/FlorentinJ",
    "created_at": "2018-08-06T15:20:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26012",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26012#event-341239"
}
```



---

archive/issue_comments_403475.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nDo you have an explicit example where something was going wrong? If so, could you add it as a test to some `TESTS::` block with a message saying something like <code>\\`\\`_restrictions_graph\\`\\` and \\`\\`_extensions_graph\\`\\` was not changed (see :trac:\\`26012\\`)::</code>",
    "created_at": "2018-08-06T22:21:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26012#issuecomment-403475",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:2" align="right">Comment 2</div>

Do you have an explicit example where something was going wrong? If so, could you add it as a test to some `TESTS::` block with a message saying something like <code>\`\`_restrictions_graph\`\` and \`\`_extensions_graph\`\` was not changed (see :trac:\`26012\`)::</code>



---

archive/issue_comments_403476.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/043f08436dd36cd853d47a3855a260afd19420b5\">043f084</a></td><td><code>Added TESTS:: block, typo</code></td></tr></table>\n",
    "created_at": "2018-08-08T10:05:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26012#issuecomment-403476",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/043f08436dd36cd853d47a3855a260afd19420b5">043f084</a></td><td><code>Added TESTS:: block, typo</code></td></tr></table>




---

archive/issue_comments_403477.json:
```json
{
    "body": "Changed commit from **[e5787ad](https://github.com/sagemath/sagetrac-mirror/commit/e5787adef1d5c47f63190f4c478b0414cae7c389)** to **[043f084](https://github.com/sagemath/sagetrac-mirror/commit/043f08436dd36cd853d47a3855a260afd19420b5)**",
    "created_at": "2018-08-08T10:05:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26012#issuecomment-403477",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[e5787ad](https://github.com/sagemath/sagetrac-mirror/commit/e5787adef1d5c47f63190f4c478b0414cae7c389)** to **[043f084](https://github.com/sagemath/sagetrac-mirror/commit/043f08436dd36cd853d47a3855a260afd19420b5)**



---

archive/issue_events_341240.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-08-08T10:32:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26012",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26012#event-341240"
}
```



---

archive/issue_events_341241.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-08-08T10:32:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26012",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26012#event-341241"
}
```



---

archive/issue_comments_403478.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThanks. LGTM.",
    "created_at": "2018-08-08T10:32:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26012#issuecomment-403478",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:4" align="right">Comment 4</div>

Thanks. LGTM.



---

archive/issue_comments_403479.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2018-08-08T10:32:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26012#issuecomment-403479",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_403480.json:
```json
{
    "body": "Changed branch from **[public/manifolds/better_restrictions_bugfix](https://github.com/sagemath/sagetrac-mirror/tree/public/manifolds/better_restrictions_bugfix)** to **[043f084](https://github.com/sagemath/sagetrac-mirror/commit/043f08436dd36cd853d47a3855a260afd19420b5)**",
    "created_at": "2018-08-09T21:27:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26012#issuecomment-403480",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/manifolds/better_restrictions_bugfix](https://github.com/sagemath/sagetrac-mirror/tree/public/manifolds/better_restrictions_bugfix)** to **[043f084](https://github.com/sagemath/sagetrac-mirror/commit/043f08436dd36cd853d47a3855a260afd19420b5)**



---

archive/issue_events_341242.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-08-09T21:27:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26012",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26012#event-341242"
}
```



---

archive/issue_events_341243.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "2274331e6eb480928bc42b00ded416735e867fc4",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-08-09T21:27:32Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26012",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26012#event-341243"
}
```
