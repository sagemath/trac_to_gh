...
```json
{
    "body": "<a id='comment:54'></a>\n`xcrun` has been there since OSX 10.9, so it's pretty safe to use, at least for [SageMath](../wiki/SageMath). Note that Python's info on the headers on OSX is outdated.\nhttps://devguide.python.org/setup/#macos-and-os-x\n\nShould we open a bug with them?",
