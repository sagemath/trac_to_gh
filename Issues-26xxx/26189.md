# Issue 26189: Intersection of modular abelian varieties gives wrong answer

archive/issues_025952.json:
```json
{
    "assignees": [],
    "body": "The intersection code is sometimes wrong. Here's an example:\n\nThis is the setup:\n\n```\nsage: d1 = J0(11).degeneracy_map(22, 1)\nsage: d2 = J0(11).degeneracy_map(22, 2)\nsage: A = (d1-d2).image()\nsage: J = J0(22)\n```\n\nThis is correct:\n\n```\nsage: J.intersection(A)\n(Finite subgroup with invariants [] over QQbar of Abelian variety J0(22) of dimension 2,\n Abelian subvariety of dimension 1 of J0(22))\n```\n\nThis is wrong:\n\n```\nsage: A.intersection(J)\n(Finite subgroup with invariants [2] over QQbar of Abelian subvariety of dimension 1 of J0(22),\n Abelian subvariety of dimension 1 of J0(22))\n```\n\n\n\nBranch/Commit: **[94b9fdd](https://github.com/sagemath/sagetrac-mirror/commit/94b9fddbaa4a883e88ad4c3e089332506692a8c5)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Kevin Lui**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26189_\n\n",
    "closed_at": "2019-05-17T11:45:56Z",
    "created_at": "2018-09-04T20:48:01Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20modular%20forms",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Intersection of modular abelian varieties gives wrong answer",
    "type": "issue",
    "updated_at": "2019-05-17T11:45:56Z",
    "url": "https://github.com/sagemath/sage/issues/26189",
    "user": "https://github.com/kevinywlui"
}
```
The intersection code is sometimes wrong. Here's an example:

This is the setup:

```
sage: d1 = J0(11).degeneracy_map(22, 1)
sage: d2 = J0(11).degeneracy_map(22, 2)
sage: A = (d1-d2).image()
sage: J = J0(22)
```

This is correct:

```
sage: J.intersection(A)
(Finite subgroup with invariants [] over QQbar of Abelian variety J0(22) of dimension 2,
 Abelian subvariety of dimension 1 of J0(22))
```

This is wrong:

```
sage: A.intersection(J)
(Finite subgroup with invariants [2] over QQbar of Abelian subvariety of dimension 1 of J0(22),
 Abelian subvariety of dimension 1 of J0(22))
```



Branch/Commit: **[94b9fdd](https://github.com/sagemath/sagetrac-mirror/commit/94b9fddbaa4a883e88ad4c3e089332506692a8c5)**

Reviewer: **Travis Scrimshaw**

Author: **Kevin Lui**

_Issue created by migration from https://trac.sagemath.org/ticket/26189_





---

archive/issue_events_343235.json:
```json
{
    "actor": "https://github.com/kevinywlui",
    "created_at": "2018-09-04T20:48:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26189",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26189#event-343235"
}
```



---

archive/issue_events_343236.json:
```json
{
    "actor": "https://github.com/kevinywlui",
    "created_at": "2018-09-04T20:48:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26189",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20modular%20forms",
    "label_color": "0000ff",
    "label_name": "component: modular forms",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26189#event-343236"
}
```



---

archive/issue_events_343237.json:
```json
{
    "actor": "https://github.com/kevinywlui",
    "created_at": "2018-09-04T20:48:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26189",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26189#event-343237"
}
```



---

archive/issue_events_343238.json:
```json
{
    "actor": "https://github.com/kevinywlui",
    "created_at": "2018-09-04T20:48:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26189",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26189#event-343238"
}
```



---

archive/issue_comments_406087.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\n#6392 ?",
    "created_at": "2018-09-05T06:01:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26189#issuecomment-406087",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:1" align="right">Comment 1</div>

#6392 ?



---

archive/issue_comments_406088.json:
```json
{
    "body": "Branch: **[u/klui/intersection](https://github.com/sagemath/sagetrac-mirror/tree/u/klui/intersection)**",
    "created_at": "2019-05-05T03:19:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26189#issuecomment-406088",
    "user": "https://github.com/kevinywlui"
}
```

Branch: **[u/klui/intersection](https://github.com/sagemath/sagetrac-mirror/tree/u/klui/intersection)**



---

archive/issue_comments_406089.json:
```json
{
    "body": "Commit: **[f8f3723](https://github.com/sagemath/sagetrac-mirror/commit/f8f372378591c234e574b6dd3e99e768ae8da1f8)**",
    "created_at": "2019-05-05T03:26:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26189#issuecomment-406089",
    "user": "https://github.com/kevinywlui"
}
```

Commit: **[f8f3723](https://github.com/sagemath/sagetrac-mirror/commit/f8f372378591c234e574b6dd3e99e768ae8da1f8)**



---

archive/issue_events_343239.json:
```json
{
    "actor": "https://github.com/kevinywlui",
    "created_at": "2019-05-05T03:26:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26189",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26189#event-343239"
}
```



---

archive/issue_comments_406090.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNo code change here. Just a documentation update.\n\nThe intersection of 2 abelian varieties, X and Y, is G+A, where G is a finite subgroup with an element in each component and A is the identity component. I don't think there's a canonical way to choose G so that it's a subgroup of X or Y. \n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f8f372378591c234e574b6dd3e99e768ae8da1f8\">f8f3723</a></td><td><code>Added documentation explaining what intersection does.</code></td></tr></table>\n",
    "created_at": "2019-05-05T03:26:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26189#issuecomment-406090",
    "user": "https://github.com/kevinywlui"
}
```

<div id="comment:3" align="right">Comment 3</div>

No code change here. Just a documentation update.

The intersection of 2 abelian varieties, X and Y, is G+A, where G is a finite subgroup with an element in each component and A is the identity component. I don't think there's a canonical way to choose G so that it's a subgroup of X or Y. 

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f8f372378591c234e574b6dd3e99e768ae8da1f8">f8f3723</a></td><td><code>Added documentation explaining what intersection does.</code></td></tr></table>




---

archive/issue_comments_406091.json:
```json
{
    "body": "Changed commit from **[f8f3723](https://github.com/sagemath/sagetrac-mirror/commit/f8f372378591c234e574b6dd3e99e768ae8da1f8)** to **[320ffbb](https://github.com/sagemath/sagetrac-mirror/commit/320ffbbfcf8596bf40ef5c5426e061886f26baef)**",
    "created_at": "2019-05-05T03:29:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26189#issuecomment-406091",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[f8f3723](https://github.com/sagemath/sagetrac-mirror/commit/f8f372378591c234e574b6dd3e99e768ae8da1f8)** to **[320ffbb](https://github.com/sagemath/sagetrac-mirror/commit/320ffbbfcf8596bf40ef5c5426e061886f26baef)**



---

archive/issue_comments_406092.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/320ffbbfcf8596bf40ef5c5426e061886f26baef\">320ffbb</a></td><td><code>doc error</code></td></tr></table>\n",
    "created_at": "2019-05-05T03:29:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26189#issuecomment-406092",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/320ffbbfcf8596bf40ef5c5426e061886f26baef">320ffbb</a></td><td><code>doc error</code></td></tr></table>




---

archive/issue_comments_406093.json:
```json
{
    "body": "Author: **Kevin Lui**",
    "created_at": "2019-05-05T03:56:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26189#issuecomment-406093",
    "user": "https://github.com/kevinywlui"
}
```

Author: **Kevin Lui**



---

archive/issue_events_343240.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-05-13T00:33:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/26189",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26189#event-343240"
}
```



---

archive/issue_events_343241.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-05-13T00:33:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26189",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26189#event-343241"
}
```



---

archive/issue_comments_406094.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2019-05-13T00:33:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26189#issuecomment-406094",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_406095.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nA little tweak to get the punctuation correct:\n\n```diff\n-        canonical. (see :trac:`26189`). In this following example, ``B`` is a\n-        subvariety of ``J``.::\n+        canonical (see :trac:`26189`). In this following example, ``B`` is a\n+        subvariety of ``J``::\n```\nOnce done, you can set a positive review on my behalf.",
    "created_at": "2019-05-13T00:33:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26189#issuecomment-406095",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:6" align="right">Comment 6</div>

A little tweak to get the punctuation correct:

```diff
-        canonical. (see :trac:`26189`). In this following example, ``B`` is a
-        subvariety of ``J``.::
+        canonical (see :trac:`26189`). In this following example, ``B`` is a
+        subvariety of ``J``::
```
Once done, you can set a positive review on my behalf.



---

archive/issue_comments_406096.json:
```json
{
    "body": "Changed commit from **[320ffbb](https://github.com/sagemath/sagetrac-mirror/commit/320ffbbfcf8596bf40ef5c5426e061886f26baef)** to **[94b9fdd](https://github.com/sagemath/sagetrac-mirror/commit/94b9fddbaa4a883e88ad4c3e089332506692a8c5)**",
    "created_at": "2019-05-13T04:53:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26189#issuecomment-406096",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[320ffbb](https://github.com/sagemath/sagetrac-mirror/commit/320ffbbfcf8596bf40ef5c5426e061886f26baef)** to **[94b9fdd](https://github.com/sagemath/sagetrac-mirror/commit/94b9fddbaa4a883e88ad4c3e089332506692a8c5)**



---

archive/issue_comments_406097.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/94b9fddbaa4a883e88ad4c3e089332506692a8c5\">94b9fdd</a></td><td><code>punctuation fix as suggested by Travis Scrimshaw</code></td></tr></table>\n",
    "created_at": "2019-05-13T04:53:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26189#issuecomment-406097",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:7" align="right">Comment 7</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/94b9fddbaa4a883e88ad4c3e089332506692a8c5">94b9fdd</a></td><td><code>punctuation fix as suggested by Travis Scrimshaw</code></td></tr></table>




---

archive/issue_events_343242.json:
```json
{
    "actor": "https://github.com/kevinywlui",
    "created_at": "2019-05-13T04:55:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26189",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26189#event-343242"
}
```



---

archive/issue_events_343243.json:
```json
{
    "actor": "https://github.com/kevinywlui",
    "created_at": "2019-05-13T04:55:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26189",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26189#event-343243"
}
```



---

archive/issue_comments_406098.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nFixed. Thanks for the review!",
    "created_at": "2019-05-13T04:55:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26189#issuecomment-406098",
    "user": "https://github.com/kevinywlui"
}
```

<div id="comment:8" align="right">Comment 8</div>

Fixed. Thanks for the review!



---

archive/issue_comments_406099.json:
```json
{
    "body": "Changed branch from **[u/klui/intersection](https://github.com/sagemath/sagetrac-mirror/tree/u/klui/intersection)** to **[94b9fdd](https://github.com/sagemath/sagetrac-mirror/commit/94b9fddbaa4a883e88ad4c3e089332506692a8c5)**",
    "created_at": "2019-05-17T11:45:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26189",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26189#issuecomment-406099",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/klui/intersection](https://github.com/sagemath/sagetrac-mirror/tree/u/klui/intersection)** to **[94b9fdd](https://github.com/sagemath/sagetrac-mirror/commit/94b9fddbaa4a883e88ad4c3e089332506692a8c5)**



---

archive/issue_events_343244.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-05-17T11:45:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26189",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26189#event-343244"
}
```



---

archive/issue_events_343245.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "a353731012bca521ae465dabc2b9fdf646e9e12b",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-05-17T11:45:56Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26189",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26189#event-343245"
}
```
