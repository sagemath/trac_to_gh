# Issue 26021: tides does not build on 32-bit systems

archive/issues_025784.json:
```json
{
    "assignees": [],
    "body": "It seems that the `configure` script for tides unconditionally adds `-m64` to the compiler flags, even on 32-bit systems.\n\nCC:  @miguelmarco\n\nKeywords: **sdl**\n\nBranch/Commit: **[342440d](https://github.com/sagemath/sagetrac-mirror/commit/342440d419aafe23039516203175e7918e1c62ad)**\n\nReviewer: **Julian R\u00fcth**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26021_\n\n",
    "closed_at": "2018-08-09T21:28:05Z",
    "created_at": "2018-08-07T13:01:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "tides does not build on 32-bit systems",
    "type": "issue",
    "updated_at": "2019-08-27T19:45:19Z",
    "url": "https://github.com/sagemath/sage/issues/26021",
    "user": "https://github.com/jdemeyer"
}
```
It seems that the `configure` script for tides unconditionally adds `-m64` to the compiler flags, even on 32-bit systems.

CC:  @miguelmarco

Keywords: **sdl**

Branch/Commit: **[342440d](https://github.com/sagemath/sagetrac-mirror/commit/342440d419aafe23039516203175e7918e1c62ad)**

Reviewer: **Julian Rüth**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/26021_





---

archive/issue_events_341360.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-08-07T13:01:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26021",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26021#event-341360"
}
```



---

archive/issue_events_341361.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-08-07T13:01:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26021",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
    "label_color": "000080",
    "label_name": "component: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26021#event-341361"
}
```



---

archive/issue_events_341362.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-08-07T13:01:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26021",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26021#event-341362"
}
```



---

archive/issue_events_341363.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-08-07T13:01:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26021",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26021#event-341363"
}
```



---

archive/issue_comments_403616.json:
```json
{
    "body": "Author: **Jeroen Demeyer**",
    "created_at": "2018-08-07T13:31:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26021",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26021#issuecomment-403616",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Jeroen Demeyer**



---

archive/issue_comments_403617.json:
```json
{
    "body": "Branch: **[u/jdemeyer/tides_does_not_build_on_32_bit_systems](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/tides_does_not_build_on_32_bit_systems)**",
    "created_at": "2018-08-07T13:32:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26021",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26021#issuecomment-403617",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/tides_does_not_build_on_32_bit_systems](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/tides_does_not_build_on_32_bit_systems)**



---

archive/issue_comments_403618.json:
```json
{
    "body": "Commit: **[342440d](https://github.com/sagemath/sagetrac-mirror/commit/342440d419aafe23039516203175e7918e1c62ad)**",
    "created_at": "2018-08-07T13:44:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26021",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26021#issuecomment-403618",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[342440d](https://github.com/sagemath/sagetrac-mirror/commit/342440d419aafe23039516203175e7918e1c62ad)**



---

archive/issue_events_341364.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-08-07T13:44:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26021",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26021#event-341364"
}
```



---

archive/issue_comments_403619.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/342440d419aafe23039516203175e7918e1c62ad\">342440d</a></td><td><code>Fix build of TIDES on 32-bit systems</code></td></tr></table>\n",
    "created_at": "2018-08-07T13:44:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26021",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26021#issuecomment-403619",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:3" align="right">Comment 3</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/342440d419aafe23039516203175e7918e1c62ad">342440d</a></td><td><code>Fix build of TIDES on 32-bit systems</code></td></tr></table>




---

archive/issue_comments_403620.json:
```json
{
    "body": "Reviewer: **Julian R\u00fcth**",
    "created_at": "2018-08-07T16:45:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26021",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26021#issuecomment-403620",
    "user": "https://github.com/saraedum"
}
```

Reviewer: **Julian Rüth**



---

archive/issue_events_341365.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-08-07T16:45:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26021",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26021#event-341365"
}
```



---

archive/issue_events_341366.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-08-07T16:45:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26021",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26021#event-341366"
}
```



---

archive/issue_comments_403621.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nNot reported upstream since there is no clear upstream. Maybe @miguelmarco knows?",
    "created_at": "2018-08-08T09:21:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26021",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26021#issuecomment-403621",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:5" align="right">Comment 5</div>

Not reported upstream since there is no clear upstream. Maybe @miguelmarco knows?



---

archive/issue_comments_403622.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/tides_does_not_build_on_32_bit_systems](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/tides_does_not_build_on_32_bit_systems)** to **[342440d](https://github.com/sagemath/sagetrac-mirror/commit/342440d419aafe23039516203175e7918e1c62ad)**",
    "created_at": "2018-08-09T21:28:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26021",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26021#issuecomment-403622",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/tides_does_not_build_on_32_bit_systems](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/tides_does_not_build_on_32_bit_systems)** to **[342440d](https://github.com/sagemath/sagetrac-mirror/commit/342440d419aafe23039516203175e7918e1c62ad)**



---

archive/issue_events_341367.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-08-09T21:28:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26021",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26021#event-341367"
}
```



---

archive/issue_events_341368.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "f1ef83ac07bc8262c41efdebb6a77ea081c3c068",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-08-09T21:28:05Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26021",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26021#event-341368"
}
```



---

archive/issue_comments_403623.json:
```json
{
    "body": "Changed keywords from none to **sdl**",
    "created_at": "2019-08-27T19:45:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26021",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26021#issuecomment-403623",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Changed keywords from none to **sdl**
