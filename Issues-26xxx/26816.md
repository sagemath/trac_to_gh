# Issue 26816: Specify subgroups of Galois groups using generators

archive/issues_026579.json:
```json
{
    "assignees": [],
    "body": "It should be possible to construct a subgroup of the Galois group of a number field from a set of generators, just as for permutation groups.  This is already supported to some extent, but the list of elements of such a subgroup is wrong.\n\n```\nsage: R.<x> = QQ[]\nsage: f = x^6 - 6*x^4 + 9*x^2 + 23\nsage: K.<a> = NumberField(f)\nsage: G = K.galois_group()\nsage: list(G)\n[(),\n (1,2,3)(4,5,6),\n (1,3,2)(4,6,5),\n (1,4)(2,6)(3,5),\n (1,5)(2,4)(3,6),\n (1,6)(2,5)(3,4)]\nsage: g = G[1]\nsage: h = G[3]\n```\nBefore:\n\n```\nsage: list(G.subgroup([]))\n[]\nsage: list(G.subgroup([g]))\n[(1,2,3)(4,5,6)]\nsage: list(G.subgroup([h]))\n[(1,4)(2,6)(3,5)]\nsage: list(G.subgroup([g,h]))\n[(1,2,3)(4,5,6), (1,4)(2,6)(3,5)]\n```\nAfter:\n\n```\nsage: list(G.subgroup([]))\n[()]\nsage: list(G.subgroup([g]))\n[(), (1,2,3)(4,5,6), (1,3,2)(4,6,5)]\nsage: list(G.subgroup([h]))\n[(), (1,4)(2,6)(3,5)]\nsage: list(G.subgroup([g,h]))\n[(),\n (1,2,3)(4,5,6),\n (1,3,2)(4,6,5),\n (1,4)(2,6)(3,5),\n (1,5)(2,4)(3,6),\n (1,6)(2,5)(3,4)]\n```\nThere is one user-visible change: the special case where applying `subgroup()` to the list of all elements of `G` returns `G` itself has been removed.  Besides simplifying the code, this has the advantage that the object returned by `subgroup()` now always has type `GaloisGroup_subgroup`, and in particular has a method `fixed_field()`.\n\nKeywords: **Galois group**\n\nBranch/Commit: **[1c199f4](https://github.com/sagemath/sagetrac-mirror/commit/1c199f427f90c4013742fcef77bd3077e7893bf6)**\n\nReviewer: **David Roe**\n\nAuthor: **Peter Bruin**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26816_\n\n",
    "closed_at": "2019-11-30T13:36:02Z",
    "created_at": "2018-12-04T15:56:38Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Specify subgroups of Galois groups using generators",
    "type": "issue",
    "updated_at": "2019-11-30T13:36:02Z",
    "url": "https://github.com/sagemath/sage/issues/26816",
    "user": "https://github.com/pjbruin"
}
```
It should be possible to construct a subgroup of the Galois group of a number field from a set of generators, just as for permutation groups.  This is already supported to some extent, but the list of elements of such a subgroup is wrong.

```
sage: R.<x> = QQ[]
sage: f = x^6 - 6*x^4 + 9*x^2 + 23
sage: K.<a> = NumberField(f)
sage: G = K.galois_group()
sage: list(G)
[(),
 (1,2,3)(4,5,6),
 (1,3,2)(4,6,5),
 (1,4)(2,6)(3,5),
 (1,5)(2,4)(3,6),
 (1,6)(2,5)(3,4)]
sage: g = G[1]
sage: h = G[3]
```
Before:

```
sage: list(G.subgroup([]))
[]
sage: list(G.subgroup([g]))
[(1,2,3)(4,5,6)]
sage: list(G.subgroup([h]))
[(1,4)(2,6)(3,5)]
sage: list(G.subgroup([g,h]))
[(1,2,3)(4,5,6), (1,4)(2,6)(3,5)]
```
After:

```
sage: list(G.subgroup([]))
[()]
sage: list(G.subgroup([g]))
[(), (1,2,3)(4,5,6), (1,3,2)(4,6,5)]
sage: list(G.subgroup([h]))
[(), (1,4)(2,6)(3,5)]
sage: list(G.subgroup([g,h]))
[(),
 (1,2,3)(4,5,6),
 (1,3,2)(4,6,5),
 (1,4)(2,6)(3,5),
 (1,5)(2,4)(3,6),
 (1,6)(2,5)(3,4)]
```
There is one user-visible change: the special case where applying `subgroup()` to the list of all elements of `G` returns `G` itself has been removed.  Besides simplifying the code, this has the advantage that the object returned by `subgroup()` now always has type `GaloisGroup_subgroup`, and in particular has a method `fixed_field()`.

Keywords: **Galois group**

Branch/Commit: **[1c199f4](https://github.com/sagemath/sagetrac-mirror/commit/1c199f427f90c4013742fcef77bd3077e7893bf6)**

Reviewer: **David Roe**

Author: **Peter Bruin**

_Issue created by migration from https://trac.sagemath.org/ticket/26816_





---

archive/issue_events_350404.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2018-12-04T15:56:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26816",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26816#event-350404"
}
```



---

archive/issue_events_350405.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2018-12-04T15:56:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26816",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
    "label_color": "0000ff",
    "label_name": "component: number fields",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26816#event-350405"
}
```



---

archive/issue_events_350406.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2018-12-04T15:56:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26816",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26816#event-350406"
}
```



---

archive/issue_events_350407.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2018-12-04T15:56:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26816",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26816#event-350407"
}
```



---

archive/issue_events_350408.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2018-12-04T16:13:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26816",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26816#event-350408"
}
```



---

archive/issue_comments_415805.json:
```json
{
    "body": "Branch: **[u/pbruin/26816-GaloisGroup_subgroup](https://github.com/sagemath/sagetrac-mirror/tree/u/pbruin/26816-GaloisGroup_subgroup)**",
    "created_at": "2018-12-04T16:13:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26816#issuecomment-415805",
    "user": "https://github.com/pjbruin"
}
```

Branch: **[u/pbruin/26816-GaloisGroup_subgroup](https://github.com/sagemath/sagetrac-mirror/tree/u/pbruin/26816-GaloisGroup_subgroup)**



---

archive/issue_comments_415806.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -44,4 +44,4 @@\n  (1,5)(2,4)(3,6),\n  (1,6)(2,5)(3,4)]\n ```\n-\n+There is one user-visible change: the special case where applying `subgroup()` to the list of all elements of `G` returns `G` itself has been removed.  Besides simplifying the code, this has the advantage that the object returned by `subgroup()` now always has type `GaloisGroup_subgroup`, and in particular has a method `fixed_field()`.\n``````\n",
    "created_at": "2018-12-04T16:13:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26816#issuecomment-415806",
    "user": "https://github.com/pjbruin"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -44,4 +44,4 @@
  (1,5)(2,4)(3,6),
  (1,6)(2,5)(3,4)]
 ```
-
+There is one user-visible change: the special case where applying `subgroup()` to the list of all elements of `G` returns `G` itself has been removed.  Besides simplifying the code, this has the advantage that the object returned by `subgroup()` now always has type `GaloisGroup_subgroup`, and in particular has a method `fixed_field()`.
``````




---

archive/issue_comments_415807.json:
```json
{
    "body": "Commit: **[eed9893](https://github.com/sagemath/sagetrac-mirror/commit/eed989369d086451145434d9e9b49841ce745332)**",
    "created_at": "2018-12-04T16:13:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26816#issuecomment-415807",
    "user": "https://github.com/pjbruin"
}
```

Commit: **[eed9893](https://github.com/sagemath/sagetrac-mirror/commit/eed989369d086451145434d9e9b49841ce745332)**



---

archive/issue_comments_415808.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1c199f427f90c4013742fcef77bd3077e7893bf6\">1c199f4</a></td><td><code>Merge branch 'develop' into ticket/26816-GaloisGroup_subgroup</code></td></tr></table>\n",
    "created_at": "2019-06-01T16:41:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26816#issuecomment-415808",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1c199f427f90c4013742fcef77bd3077e7893bf6">1c199f4</a></td><td><code>Merge branch 'develop' into ticket/26816-GaloisGroup_subgroup</code></td></tr></table>




---

archive/issue_comments_415809.json:
```json
{
    "body": "Changed commit from **[eed9893](https://github.com/sagemath/sagetrac-mirror/commit/eed989369d086451145434d9e9b49841ce745332)** to **[1c199f4](https://github.com/sagemath/sagetrac-mirror/commit/1c199f427f90c4013742fcef77bd3077e7893bf6)**",
    "created_at": "2019-06-01T16:41:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26816#issuecomment-415809",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[eed9893](https://github.com/sagemath/sagetrac-mirror/commit/eed989369d086451145434d9e9b49841ce745332)** to **[1c199f4](https://github.com/sagemath/sagetrac-mirror/commit/1c199f427f90c4013742fcef77bd3077e7893bf6)**



---

archive/issue_comments_415810.json:
```json
{
    "body": "Reviewer: **David Roe**",
    "created_at": "2019-11-20T22:12:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26816#issuecomment-415810",
    "user": "https://github.com/roed314"
}
```

Reviewer: **David Roe**



---

archive/issue_comments_415811.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nLooks good to me.  Recent patchbot failures due to transition to Python 3.",
    "created_at": "2019-11-20T22:12:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26816#issuecomment-415811",
    "user": "https://github.com/roed314"
}
```

<a id='comment:3'>Comment 3:</a>
Looks good to me.  Recent patchbot failures due to transition to Python 3.



---

archive/issue_events_350409.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2019-11-20T22:12:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26816",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26816#event-350409"
}
```



---

archive/issue_events_350410.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2019-11-20T22:12:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26816",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26816#event-350410"
}
```



---

archive/issue_events_350411.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-11-21T14:18:55Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/26816",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26816#event-350411"
}
```



---

archive/issue_events_350412.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-11-21T14:18:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26816",
    "milestone_number": null,
    "milestone_title": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26816#event-350412"
}
```



---

archive/issue_comments_415812.json:
```json
{
    "body": "Changed branch from **[u/pbruin/26816-GaloisGroup_subgroup](https://github.com/sagemath/sagetrac-mirror/tree/u/pbruin/26816-GaloisGroup_subgroup)** to **[1c199f4](https://github.com/sagemath/sagetrac-mirror/commit/1c199f427f90c4013742fcef77bd3077e7893bf6)**",
    "created_at": "2019-11-30T13:36:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26816",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26816#issuecomment-415812",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/pbruin/26816-GaloisGroup_subgroup](https://github.com/sagemath/sagetrac-mirror/tree/u/pbruin/26816-GaloisGroup_subgroup)** to **[1c199f4](https://github.com/sagemath/sagetrac-mirror/commit/1c199f427f90c4013742fcef77bd3077e7893bf6)**



---

archive/issue_events_350413.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-11-30T13:36:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26816",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26816#event-350413"
}
```



---

archive/issue_events_350414.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "7a740579e3309a7f05529df8dc9876b7843ae99f",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-11-30T13:36:02Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26816",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26816#event-350414"
}
```
