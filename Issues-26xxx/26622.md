# Issue 26622: avoid using .vertices() in convexity_properties.pyx

archive/issues_026385.json:
```json
{
    "assignees": [],
    "body": "Now that we can pass an ordering to `c_distances_all_pairs`, we don't need to use the rdering of `.vertices()` anymore\n\nCC:  @tscrim @fchapoton\n\nKeywords: **py3, graph**\n\nBranch/Commit: **[a48d860](https://github.com/sagemath/sagetrac-mirror/commit/a48d860ca14aa0c63d31f74f47d6c4354e459239)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26622_\n\n",
    "closed_at": "2018-11-07T11:27:51Z",
    "created_at": "2018-11-03T01:18:11Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "avoid using .vertices() in convexity_properties.pyx",
    "type": "issue",
    "updated_at": "2018-11-07T11:27:51Z",
    "url": "https://github.com/sagemath/sage/issues/26622",
    "user": "https://github.com/dcoudert"
}
```
Now that we can pass an ordering to `c_distances_all_pairs`, we don't need to use the rdering of `.vertices()` anymore

CC:  @tscrim @fchapoton

Keywords: **py3, graph**

Branch/Commit: **[a48d860](https://github.com/sagemath/sagetrac-mirror/commit/a48d860ca14aa0c63d31f74f47d6c4354e459239)**

Reviewer: **Frédéric Chapoton**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/26622_





---

archive/issue_events_348246.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-03T01:18:11Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26622",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26622#event-348246"
}
```



---

archive/issue_events_348247.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-03T01:18:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26622",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26622#event-348247"
}
```



---

archive/issue_events_348248.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-03T01:18:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26622",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26622#event-348248"
}
```



---

archive/issue_events_348249.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-03T01:18:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26622",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26622#event-348249"
}
```



---

archive/issue_comments_413091.json:
```json
{
    "body": "Commit: **[a48d860](https://github.com/sagemath/sagetrac-mirror/commit/a48d860ca14aa0c63d31f74f47d6c4354e459239)**",
    "created_at": "2018-11-03T01:19:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26622",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26622#issuecomment-413091",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[a48d860](https://github.com/sagemath/sagetrac-mirror/commit/a48d860ca14aa0c63d31f74f47d6c4354e459239)**



---

archive/issue_events_348250.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-03T01:19:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26622",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26622#event-348250"
}
```



---

archive/issue_comments_413092.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a48d860ca14aa0c63d31f74f47d6c4354e459239\">a48d860</a></td><td><code>trac #26622: avoid using .vertices in convexity_properties.pyx</code></td></tr></table>\n",
    "created_at": "2018-11-03T01:19:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26622",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26622#issuecomment-413092",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a48d860ca14aa0c63d31f74f47d6c4354e459239">a48d860</a></td><td><code>trac #26622: avoid using .vertices in convexity_properties.pyx</code></td></tr></table>




---

archive/issue_comments_413093.json:
```json
{
    "body": "Branch: **[public/26622_convexity_properties](https://github.com/sagemath/sagetrac-mirror/tree/public/26622_convexity_properties)**",
    "created_at": "2018-11-03T01:19:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26622",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26622#issuecomment-413093",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[public/26622_convexity_properties](https://github.com/sagemath/sagetrac-mirror/tree/public/26622_convexity_properties)**



---

archive/issue_comments_413094.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2018-11-03T10:59:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26622",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26622#issuecomment-413094",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_413095.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nok, good. Please review #26527",
    "created_at": "2018-11-03T10:59:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26622",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26622#issuecomment-413095",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

ok, good. Please review #26527



---

archive/issue_events_348251.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-11-03T10:59:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26622",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26622#event-348251"
}
```



---

archive/issue_events_348252.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-11-03T10:59:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26622",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26622#event-348252"
}
```



---

archive/issue_comments_413096.json:
```json
{
    "body": "Changed branch from **[public/26622_convexity_properties](https://github.com/sagemath/sagetrac-mirror/tree/public/26622_convexity_properties)** to **[a48d860](https://github.com/sagemath/sagetrac-mirror/commit/a48d860ca14aa0c63d31f74f47d6c4354e459239)**",
    "created_at": "2018-11-07T11:27:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26622",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26622#issuecomment-413096",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/26622_convexity_properties](https://github.com/sagemath/sagetrac-mirror/tree/public/26622_convexity_properties)** to **[a48d860](https://github.com/sagemath/sagetrac-mirror/commit/a48d860ca14aa0c63d31f74f47d6c4354e459239)**



---

archive/issue_events_348253.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-11-07T11:27:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26622",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26622#event-348253"
}
```



---

archive/issue_events_348254.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "748cced040156908dbc88e022ad7a47e6037a1e8",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-11-07T11:27:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26622",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26622#event-348254"
}
```
