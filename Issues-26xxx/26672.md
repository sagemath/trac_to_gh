# Issue 26672: clean generic_graph.py (part 10) - degree

archive/issues_026435.json:
```json
{
    "assignees": [],
    "body": "Here we improve the documentation and doctests of methods `degree`, `average_degree`, `degree_histogram`, `degree_iterator`, `degree_sequence`, `is_regular`.\n\nIn particular, we make it clear that a call to `.degree()` returns a list of the degrees of vertex degrees in the order of `list(G)` and not of `.vertices()`.\n\n\nCC:  @tscrim\n\nKeywords: **py3, graph**\n\nBranch/Commit: **[675e67b](https://github.com/sagemath/sagetrac-mirror/commit/675e67b94dd5343f5cc9c5d2d47e1606afd93511)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26672_\n\n",
    "closed_at": "2018-11-12T21:11:29Z",
    "created_at": "2018-11-10T12:08:09Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "clean generic_graph.py (part 10) - degree",
    "type": "issue",
    "updated_at": "2018-11-12T21:11:29Z",
    "url": "https://github.com/sagemath/sage/issues/26672",
    "user": "https://github.com/dcoudert"
}
```
Here we improve the documentation and doctests of methods `degree`, `average_degree`, `degree_histogram`, `degree_iterator`, `degree_sequence`, `is_regular`.

In particular, we make it clear that a call to `.degree()` returns a list of the degrees of vertex degrees in the order of `list(G)` and not of `.vertices()`.


CC:  @tscrim

Keywords: **py3, graph**

Branch/Commit: **[675e67b](https://github.com/sagemath/sagetrac-mirror/commit/675e67b94dd5343f5cc9c5d2d47e1606afd93511)**

Reviewer: **Travis Scrimshaw**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/26672_





---

archive/issue_events_348793.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-10T12:08:09Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26672",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26672#event-348793"
}
```



---

archive/issue_events_348794.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-10T12:08:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26672",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26672#event-348794"
}
```



---

archive/issue_events_348795.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-10T12:08:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26672",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26672#event-348795"
}
```



---

archive/issue_events_348796.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-10T12:08:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26672",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26672#event-348796"
}
```



---

archive/issue_comments_413801.json:
```json
{
    "body": "Commit: **[675e67b](https://github.com/sagemath/sagetrac-mirror/commit/675e67b94dd5343f5cc9c5d2d47e1606afd93511)**",
    "created_at": "2018-11-10T12:10:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26672",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26672#issuecomment-413801",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[675e67b](https://github.com/sagemath/sagetrac-mirror/commit/675e67b94dd5343f5cc9c5d2d47e1606afd93511)**



---

archive/issue_events_348797.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-10T12:10:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26672",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26672#event-348797"
}
```



---

archive/issue_comments_413802.json:
```json
{
    "body": "Branch: **[public/26672_generic_graph_part_10_degree](https://github.com/sagemath/sagetrac-mirror/tree/public/26672_generic_graph_part_10_degree)**",
    "created_at": "2018-11-10T12:10:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26672",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26672#issuecomment-413802",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[public/26672_generic_graph_part_10_degree](https://github.com/sagemath/sagetrac-mirror/tree/public/26672_generic_graph_part_10_degree)**



---

archive/issue_comments_413803.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/675e67b94dd5343f5cc9c5d2d47e1606afd93511\">675e67b</a></td><td><code>trac #26672: methods related to degree in generic_graph.py</code></td></tr></table>\n",
    "created_at": "2018-11-10T12:10:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26672",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26672#issuecomment-413803",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/675e67b94dd5343f5cc9c5d2d47e1606afd93511">675e67b</a></td><td><code>trac #26672: methods related to degree in generic_graph.py</code></td></tr></table>




---

archive/issue_comments_413804.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n Here we improve the documentation and doctests of methods `degree`, `average_degree`, `degree_histogram`, `degree_iterator`, `degree_sequence`, `is_regular`.\n \n-In particular, we make it clear that a call to `.degree()` returns a list of the degrees of the vertices in the order of `list(G)` and not of `.vertices()`. \n+In particular, we make it clear that a call to `.degree()` returns a list of the degrees of vertex degrees in the order of `list(G)` and not of `.vertices()`.\n \n``````\n",
    "created_at": "2018-11-10T12:10:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26672",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26672#issuecomment-413804",
    "user": "https://github.com/dcoudert"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
 Here we improve the documentation and doctests of methods `degree`, `average_degree`, `degree_histogram`, `degree_iterator`, `degree_sequence`, `is_regular`.
 
-In particular, we make it clear that a call to `.degree()` returns a list of the degrees of the vertices in the order of `list(G)` and not of `.vertices()`. 
+In particular, we make it clear that a call to `.degree()` returns a list of the degrees of vertex degrees in the order of `list(G)` and not of `.vertices()`.
 
``````




---

archive/issue_events_348798.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-11-11T12:59:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26672",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26672#event-348798"
}
```



---

archive/issue_events_348799.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-11-11T12:59:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26672",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26672#event-348799"
}
```



---

archive/issue_comments_413805.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nLGTM.",
    "created_at": "2018-11-11T12:59:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26672",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26672#issuecomment-413805",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:2" align="right">Comment 2</div>

LGTM.



---

archive/issue_comments_413806.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2018-11-11T12:59:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26672",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26672#issuecomment-413806",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_413807.json:
```json
{
    "body": "Changed branch from **[public/26672_generic_graph_part_10_degree](https://github.com/sagemath/sagetrac-mirror/tree/public/26672_generic_graph_part_10_degree)** to **[675e67b](https://github.com/sagemath/sagetrac-mirror/commit/675e67b94dd5343f5cc9c5d2d47e1606afd93511)**",
    "created_at": "2018-11-12T21:11:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26672",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26672#issuecomment-413807",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/26672_generic_graph_part_10_degree](https://github.com/sagemath/sagetrac-mirror/tree/public/26672_generic_graph_part_10_degree)** to **[675e67b](https://github.com/sagemath/sagetrac-mirror/commit/675e67b94dd5343f5cc9c5d2d47e1606afd93511)**



---

archive/issue_events_348800.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-11-12T21:11:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26672",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26672#event-348800"
}
```



---

archive/issue_events_348801.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "92759f1f2f310315d687752977cfa621efe2ae8d",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-11-12T21:11:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26672",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26672#event-348801"
}
```
