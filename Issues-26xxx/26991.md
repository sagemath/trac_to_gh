# Issue 26991: Refactor function field code

archive/issues_026754.json:
```json
{
    "assignees": [],
    "body": "Several methods for elements in the new function field code (Trac #22982) can be profitably moved to the `FunctionFieldElement` superclass:\n\n- divisor, divisor_of_zeros, divisor_of_poles, zeros, and poles\n\n\nKeywords: **function field**\n\nBranch/Commit: **[3f38d0e](https://github.com/sagemath/sagetrac-mirror/commit/3f38d0ecb5b484c340eb319cd595cc4946a6c295)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Brent Baccala**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26991_\n\n",
    "closed_at": "2019-06-06T22:26:29Z",
    "created_at": "2019-01-02T03:52:51Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Refactor function field code",
    "type": "issue",
    "updated_at": "2019-06-06T22:26:29Z",
    "url": "https://github.com/sagemath/sage/issues/26991",
    "user": "https://github.com/BrentBaccala"
}
```
Several methods for elements in the new function field code (Trac #22982) can be profitably moved to the `FunctionFieldElement` superclass:

- divisor, divisor_of_zeros, divisor_of_poles, zeros, and poles


Keywords: **function field**

Branch/Commit: **[3f38d0e](https://github.com/sagemath/sagetrac-mirror/commit/3f38d0ecb5b484c340eb319cd595cc4946a6c295)**

Reviewer: **Travis Scrimshaw**

Author: **Brent Baccala**

_Issue created by migration from https://trac.sagemath.org/ticket/26991_





---

archive/issue_events_352737.json:
```json
{
    "actor": "https://github.com/BrentBaccala",
    "created_at": "2019-01-02T03:52:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "milestone_number": null,
    "milestone_title": "sage-8.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26991#event-352737"
}
```



---

archive/issue_events_352738.json:
```json
{
    "actor": "https://github.com/BrentBaccala",
    "created_at": "2019-01-02T03:52:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26991#event-352738"
}
```



---

archive/issue_events_352739.json:
```json
{
    "actor": "https://github.com/BrentBaccala",
    "created_at": "2019-01-02T03:52:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26991#event-352739"
}
```



---

archive/issue_events_352740.json:
```json
{
    "actor": "https://github.com/BrentBaccala",
    "created_at": "2019-01-02T03:52:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26991#event-352740"
}
```



---

archive/issue_comments_418971.json:
```json
{
    "body": "Branch: **[public/26991](https://github.com/sagemath/sagetrac-mirror/tree/public/26991)**",
    "created_at": "2019-01-02T04:20:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26991#issuecomment-418971",
    "user": "https://github.com/BrentBaccala"
}
```

Branch: **[public/26991](https://github.com/sagemath/sagetrac-mirror/tree/public/26991)**



---

archive/issue_comments_418972.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d3b7833a14ea5a304b1aafd6240ee66c188a3302\">d3b7833</a></td><td><code>Add global function fields</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f59d184f26de251eeed948c8603485cb3195b384\">f59d184</a></td><td><code>Trac #26991: refactor divisor methods</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d5d2ab067599533c6ac0ee02f8537fd02b066538\">d5d2ab0</a></td><td><code>Trac #26991: refactor 'place' method into ideal super class</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/413aa01afc0684b0b2293ffd307ca167a3e25ab5\">413aa01</a></td><td><code>Trac #26991: refactor 'space_of_differentials' method in FunctionField</code></td></tr></table>\n",
    "created_at": "2019-01-02T04:20:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26991#issuecomment-418972",
    "user": "https://github.com/BrentBaccala"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d3b7833a14ea5a304b1aafd6240ee66c188a3302">d3b7833</a></td><td><code>Add global function fields</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f59d184f26de251eeed948c8603485cb3195b384">f59d184</a></td><td><code>Trac #26991: refactor divisor methods</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d5d2ab067599533c6ac0ee02f8537fd02b066538">d5d2ab0</a></td><td><code>Trac #26991: refactor 'place' method into ideal super class</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/413aa01afc0684b0b2293ffd307ca167a3e25ab5">413aa01</a></td><td><code>Trac #26991: refactor 'space_of_differentials' method in FunctionField</code></td></tr></table>




---

archive/issue_comments_418973.json:
```json
{
    "body": "Commit: **[413aa01](https://github.com/sagemath/sagetrac-mirror/commit/413aa01afc0684b0b2293ffd307ca167a3e25ab5)**",
    "created_at": "2019-01-02T04:20:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26991#issuecomment-418973",
    "user": "https://github.com/BrentBaccala"
}
```

Commit: **[413aa01](https://github.com/sagemath/sagetrac-mirror/commit/413aa01afc0684b0b2293ffd307ca167a3e25ab5)**



---

archive/issue_comments_418974.json:
```json
{
    "body": "Changed dependencies from **22982** to **u/klee/22982_stable**",
    "created_at": "2019-01-02T04:20:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26991#issuecomment-418974",
    "user": "https://github.com/BrentBaccala"
}
```

Changed dependencies from **22982** to **u/klee/22982_stable**



---

archive/issue_comments_418975.json:
```json
{
    "body": "Changed dependencies from **u/klee/22982_stable** to **#22982**",
    "created_at": "2019-01-03T20:32:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26991#issuecomment-418975",
    "user": "https://github.com/BrentBaccala"
}
```

Changed dependencies from **u/klee/22982_stable** to **#22982**



---

archive/issue_comments_418976.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nRetarging tickets optimistically to the next milestone.  If you are responsible for this ticket (either its reporter or owner) and don't believe you are likely to complete this ticket before the next release (8.7) please retarget this ticket's milestone to sage-pending or sage-wishlist.",
    "created_at": "2019-01-15T18:15:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26991#issuecomment-418976",
    "user": "https://github.com/embray"
}
```

<div id="comment:3" align="right">Comment 3</div>

Retarging tickets optimistically to the next milestone.  If you are responsible for this ticket (either its reporter or owner) and don't believe you are likely to complete this ticket before the next release (8.7) please retarget this ticket's milestone to sage-pending or sage-wishlist.



---

archive/issue_events_352741.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-01-15T18:15:21Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "milestone_number": null,
    "milestone_title": "sage-8.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26991#event-352741"
}
```



---

archive/issue_events_352742.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-01-15T18:15:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "milestone_number": null,
    "milestone_title": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26991#event-352742"
}
```



---

archive/issue_events_352743.json:
```json
{
    "actor": "https://github.com/BrentBaccala",
    "created_at": "2019-01-17T02:13:00Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "milestone_number": null,
    "milestone_title": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26991#event-352743"
}
```



---

archive/issue_events_352744.json:
```json
{
    "actor": "https://github.com/BrentBaccala",
    "created_at": "2019-01-17T02:13:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "label": "https://github.com/sagemath/sage/labels/pending",
    "label_color": "008080",
    "label_name": "pending",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26991#event-352744"
}
```



---

archive/issue_comments_418977.json:
```json
{
    "body": "Changed commit from **[413aa01](https://github.com/sagemath/sagetrac-mirror/commit/413aa01afc0684b0b2293ffd307ca167a3e25ab5)** to **[a2b07a4](https://github.com/sagemath/sagetrac-mirror/commit/a2b07a4f1804656ee1e6daae75e7f0092b0785d6)**",
    "created_at": "2019-04-09T21:10:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26991#issuecomment-418977",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[413aa01](https://github.com/sagemath/sagetrac-mirror/commit/413aa01afc0684b0b2293ffd307ca167a3e25ab5)** to **[a2b07a4](https://github.com/sagemath/sagetrac-mirror/commit/a2b07a4f1804656ee1e6daae75e7f0092b0785d6)**



---

archive/issue_comments_418978.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a2b07a4f1804656ee1e6daae75e7f0092b0785d6\">a2b07a4</a></td><td><code>Trac #26991: refactor methods for function field divisors</code></td></tr></table>\n",
    "created_at": "2019-04-09T21:10:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26991#issuecomment-418978",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:5" align="right">Comment 5</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a2b07a4f1804656ee1e6daae75e7f0092b0785d6">a2b07a4</a></td><td><code>Trac #26991: refactor methods for function field divisors</code></td></tr></table>




---

archive/issue_comments_418979.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nMost of the changes I proposed in the ticket description have already been done, but the element methods haven't been refactored, so that's all this ticket is now.  It's important to get it done, though, because I have another ticket that depends on this refactorization.\n\nI didn't make any changes to `is_prime`; even though I think it should throw `NotImplementedError` for classes where it isn't implemented.  Maybe that's another ticket.",
    "created_at": "2019-04-09T21:15:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26991#issuecomment-418979",
    "user": "https://github.com/BrentBaccala"
}
```

<div id="comment:6" align="right">Comment 6</div>

Most of the changes I proposed in the ticket description have already been done, but the element methods haven't been refactored, so that's all this ticket is now.  It's important to get it done, though, because I have another ticket that depends on this refactorization.

I didn't make any changes to `is_prime`; even though I think it should throw `NotImplementedError` for classes where it isn't implemented.  Maybe that's another ticket.



---

archive/issue_events_352745.json:
```json
{
    "actor": "https://github.com/BrentBaccala",
    "created_at": "2019-04-09T21:15:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26991#event-352745"
}
```



---

archive/issue_events_352746.json:
```json
{
    "actor": "https://github.com/BrentBaccala",
    "created_at": "2019-04-09T21:15:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "label": "https://github.com/sagemath/sage/labels/pending",
    "label_color": "008080",
    "label_name": "pending",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26991#event-352746"
}
```



---

archive/issue_comments_418980.json:
```json
{
    "body": "Changed dependencies from **#22982** to none",
    "created_at": "2019-04-09T21:15:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26991#issuecomment-418980",
    "user": "https://github.com/BrentBaccala"
}
```

Changed dependencies from **#22982** to none



---

archive/issue_events_352747.json:
```json
{
    "actor": "https://github.com/BrentBaccala",
    "created_at": "2019-04-09T21:15:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26991#event-352747"
}
```



---

archive/issue_comments_418981.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3f38d0ecb5b484c340eb319cd595cc4946a6c295\">3f38d0e</a></td><td><code>Merge tag '8.8.beta7' into public/26991</code></td></tr></table>\n",
    "created_at": "2019-06-04T13:43:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26991#issuecomment-418981",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:7" align="right">Comment 7</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3f38d0ecb5b484c340eb319cd595cc4946a6c295">3f38d0e</a></td><td><code>Merge tag '8.8.beta7' into public/26991</code></td></tr></table>




---

archive/issue_comments_418982.json:
```json
{
    "body": "Changed commit from **[a2b07a4](https://github.com/sagemath/sagetrac-mirror/commit/a2b07a4f1804656ee1e6daae75e7f0092b0785d6)** to **[3f38d0e](https://github.com/sagemath/sagetrac-mirror/commit/3f38d0ecb5b484c340eb319cd595cc4946a6c295)**",
    "created_at": "2019-06-04T13:43:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26991#issuecomment-418982",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[a2b07a4](https://github.com/sagemath/sagetrac-mirror/commit/a2b07a4f1804656ee1e6daae75e7f0092b0785d6)** to **[3f38d0e](https://github.com/sagemath/sagetrac-mirror/commit/3f38d0ecb5b484c340eb319cd595cc4946a6c295)**



---

archive/issue_comments_418983.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nPlease update the ticket description to reflect what is done and set the author name. Once done, you can set a positive review.",
    "created_at": "2019-06-04T23:09:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26991#issuecomment-418983",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:8" align="right">Comment 8</div>

Please update the ticket description to reflect what is done and set the author name. Once done, you can set a positive review.



---

archive/issue_comments_418984.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2019-06-04T23:09:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26991#issuecomment-418984",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_418985.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,8 +1,4 @@\n-Several methods in the new function field code (Trac #22982) can be profitably moved to superclasses, including:\n+Several methods for elements in the new function field code (Trac #22982) can be profitably moved to the `FunctionFieldElement` superclass:\n \n-- divisor, divisor_of_zeros, divisor_of_poles methods for both elements and ideals\n+- divisor, divisor_of_zeros, divisor_of_poles, zeros, and poles\n \n-- place and is_prime methods in ideals\n-\n-- space_of_differentials method in FunctionField\n-\n``````\n",
    "created_at": "2019-06-05T01:38:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26991#issuecomment-418985",
    "user": "https://github.com/BrentBaccala"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,8 +1,4 @@
-Several methods in the new function field code (Trac #22982) can be profitably moved to superclasses, including:
+Several methods for elements in the new function field code (Trac #22982) can be profitably moved to the `FunctionFieldElement` superclass:
 
-- divisor, divisor_of_zeros, divisor_of_poles methods for both elements and ideals
+- divisor, divisor_of_zeros, divisor_of_poles, zeros, and poles
 
-- place and is_prime methods in ideals
-
-- space_of_differentials method in FunctionField
-
``````




---

archive/issue_events_352748.json:
```json
{
    "actor": "https://github.com/BrentBaccala",
    "created_at": "2019-06-05T01:38:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26991#event-352748"
}
```



---

archive/issue_events_352749.json:
```json
{
    "actor": "https://github.com/BrentBaccala",
    "created_at": "2019-06-05T01:38:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26991#event-352749"
}
```



---

archive/issue_comments_418986.json:
```json
{
    "body": "Author: **Brent Baccala**",
    "created_at": "2019-06-05T01:38:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26991#issuecomment-418986",
    "user": "https://github.com/BrentBaccala"
}
```

Author: **Brent Baccala**



---

archive/issue_events_352750.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-06-06T22:26:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26991#event-352750"
}
```



---

archive/issue_events_352751.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "37a5846af9f96ecab676dc6559c2dea9f33806b2",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-06-06T22:26:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26991#event-352751"
}
```



---

archive/issue_comments_418987.json:
```json
{
    "body": "Changed branch from **[public/26991](https://github.com/sagemath/sagetrac-mirror/tree/public/26991)** to **[3f38d0e](https://github.com/sagemath/sagetrac-mirror/commit/3f38d0ecb5b484c340eb319cd595cc4946a6c295)**",
    "created_at": "2019-06-06T22:26:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26991#issuecomment-418987",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/26991](https://github.com/sagemath/sagetrac-mirror/tree/public/26991)** to **[3f38d0e](https://github.com/sagemath/sagetrac-mirror/commit/3f38d0ecb5b484c340eb319cd595cc4946a6c295)**
