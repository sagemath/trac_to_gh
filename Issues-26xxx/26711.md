# Issue 26711: avoid .vertices() in graph_coloring.py

archive/issues_026474.json:
```json
{
    "assignees": [],
    "body": "After #26484, one call to `.vertices()` was remaining in method `edge_coloring`. As it is used for the relabeling of a clique, we can safely replace it with `list(h)`.\n\nIn this ticket, we also do another round of PEP8 cleaning and improvements of math formula for the html doc.\n\nCC:  @tscrim\n\nKeywords: **py3, graph**\n\nBranch/Commit: **[80f42b3](https://github.com/sagemath/sagetrac-mirror/commit/80f42b3e44d0752c509a43482b0da99aea43934a)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26711_\n\n",
    "closed_at": "2018-11-21T19:54:44Z",
    "created_at": "2018-11-18T08:37:00Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "avoid .vertices() in graph_coloring.py",
    "type": "issue",
    "updated_at": "2018-11-21T19:54:44Z",
    "url": "https://github.com/sagemath/sage/issues/26711",
    "user": "https://github.com/dcoudert"
}
```
After #26484, one call to `.vertices()` was remaining in method `edge_coloring`. As it is used for the relabeling of a clique, we can safely replace it with `list(h)`.

In this ticket, we also do another round of PEP8 cleaning and improvements of math formula for the html doc.

CC:  @tscrim

Keywords: **py3, graph**

Branch/Commit: **[80f42b3](https://github.com/sagemath/sagetrac-mirror/commit/80f42b3e44d0752c509a43482b0da99aea43934a)**

Reviewer: **Frédéric Chapoton**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/26711_





---

archive/issue_events_349187.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-18T08:37:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26711",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26711#event-349187"
}
```



---

archive/issue_events_349188.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-18T08:37:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26711",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26711#event-349188"
}
```



---

archive/issue_events_349189.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-18T08:37:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26711",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26711#event-349189"
}
```



---

archive/issue_events_349190.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-18T08:37:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26711",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26711#event-349190"
}
```



---

archive/issue_events_349191.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-18T08:38:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26711",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26711#event-349191"
}
```



---

archive/issue_comments_414218.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3e1f0dee2e30efc099764ae9db1a23eaa65f98ec\">3e1f0de</a></td><td><code>trac #26711: further cleaning in graph_coloring.py</code></td></tr></table>\n",
    "created_at": "2018-11-18T08:38:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26711#issuecomment-414218",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3e1f0dee2e30efc099764ae9db1a23eaa65f98ec">3e1f0de</a></td><td><code>trac #26711: further cleaning in graph_coloring.py</code></td></tr></table>




---

archive/issue_comments_414219.json:
```json
{
    "body": "Changed keywords from **** to **py3, graph**",
    "created_at": "2018-11-18T08:38:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26711#issuecomment-414219",
    "user": "https://github.com/dcoudert"
}
```

Changed keywords from **** to **py3, graph**



---

archive/issue_comments_414220.json:
```json
{
    "body": "Commit: **[3e1f0de](https://github.com/sagemath/sagetrac-mirror/commit/3e1f0dee2e30efc099764ae9db1a23eaa65f98ec)**",
    "created_at": "2018-11-18T08:38:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26711#issuecomment-414220",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[3e1f0de](https://github.com/sagemath/sagetrac-mirror/commit/3e1f0dee2e30efc099764ae9db1a23eaa65f98ec)**



---

archive/issue_comments_414221.json:
```json
{
    "body": "Branch: **[public/26711_graph_coloring](https://github.com/sagemath/sagetrac-mirror/tree/public/26711_graph_coloring)**",
    "created_at": "2018-11-18T08:38:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26711#issuecomment-414221",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[public/26711_graph_coloring](https://github.com/sagemath/sagetrac-mirror/tree/public/26711_graph_coloring)**



---

archive/issue_comments_414222.json:
```json
{
    "body": "Changed branch from **[public/26711_graph_coloring](https://github.com/sagemath/sagetrac-mirror/tree/public/26711_graph_coloring)** to **[public/26711_graph_coloring_2](https://github.com/sagemath/sagetrac-mirror/tree/public/26711_graph_coloring_2)**",
    "created_at": "2018-11-18T09:35:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26711#issuecomment-414222",
    "user": "https://github.com/dcoudert"
}
```

Changed branch from **[public/26711_graph_coloring](https://github.com/sagemath/sagetrac-mirror/tree/public/26711_graph_coloring)** to **[public/26711_graph_coloring_2](https://github.com/sagemath/sagetrac-mirror/tree/public/26711_graph_coloring_2)**



---

archive/issue_comments_414223.json:
```json
{
    "body": "Changed commit from **[3e1f0de](https://github.com/sagemath/sagetrac-mirror/commit/3e1f0dee2e30efc099764ae9db1a23eaa65f98ec)** to **[50374e7](https://github.com/sagemath/sagetrac-mirror/commit/50374e7b98d94610c3025c80aa4b87b78471b33a)**",
    "created_at": "2018-11-18T09:35:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26711#issuecomment-414223",
    "user": "https://github.com/dcoudert"
}
```

Changed commit from **[3e1f0de](https://github.com/sagemath/sagetrac-mirror/commit/3e1f0dee2e30efc099764ae9db1a23eaa65f98ec)** to **[50374e7](https://github.com/sagemath/sagetrac-mirror/commit/50374e7b98d94610c3025c80aa4b87b78471b33a)**



---

archive/issue_comments_414224.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nI thought I was in a new branch, but my changes were in `develop`... I need another coffee... Should be OK now.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/50374e7b98d94610c3025c80aa4b87b78471b33a\">50374e7</a></td><td><code>trac #26711: more cleaning in graph_coloring.py</code></td></tr></table>\n",
    "created_at": "2018-11-18T09:35:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26711#issuecomment-414224",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:2'>Comment 2:</a>
I thought I was in a new branch, but my changes were in `develop`... I need another coffee... Should be OK now.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/50374e7b98d94610c3025c80aa4b87b78471b33a">50374e7</a></td><td><code>trac #26711: more cleaning in graph_coloring.py</code></td></tr></table>




---

archive/issue_comments_414225.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\n\n```\n+        sage: sorted(d.keys())\n```\ncould be just\n\n```\n+        sage: sorted(d)\n```\nOtherwise, positive review.",
    "created_at": "2018-11-19T14:30:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26711#issuecomment-414225",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:3'>Comment 3:</a>

```
+        sage: sorted(d.keys())
```
could be just

```
+        sage: sorted(d)
```
Otherwise, positive review.



---

archive/issue_comments_414226.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2018-11-19T14:30:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26711#issuecomment-414226",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_414227.json:
```json
{
    "body": "Changed commit from **[50374e7](https://github.com/sagemath/sagetrac-mirror/commit/50374e7b98d94610c3025c80aa4b87b78471b33a)** to **[80f42b3](https://github.com/sagemath/sagetrac-mirror/commit/80f42b3e44d0752c509a43482b0da99aea43934a)**",
    "created_at": "2018-11-20T11:14:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26711#issuecomment-414227",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[50374e7](https://github.com/sagemath/sagetrac-mirror/commit/50374e7b98d94610c3025c80aa4b87b78471b33a)** to **[80f42b3](https://github.com/sagemath/sagetrac-mirror/commit/80f42b3e44d0752c509a43482b0da99aea43934a)**



---

archive/issue_comments_414228.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/53a47e8358c16b503d145ad7df8d6fa91a1ffc39\">53a47e8</a></td><td><code>trac #26711: Merged with 8.5.beta4</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/80f42b3e44d0752c509a43482b0da99aea43934a\">80f42b3</a></td><td><code>#26711: reviewer's comments</code></td></tr></table>\n",
    "created_at": "2018-11-20T11:14:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26711#issuecomment-414228",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/53a47e8358c16b503d145ad7df8d6fa91a1ffc39">53a47e8</a></td><td><code>trac #26711: Merged with 8.5.beta4</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/80f42b3e44d0752c509a43482b0da99aea43934a">80f42b3</a></td><td><code>#26711: reviewer's comments</code></td></tr></table>




---

archive/issue_comments_414229.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nI have implemented you comment.",
    "created_at": "2018-11-20T11:15:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26711#issuecomment-414229",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:5'>Comment 5:</a>
I have implemented you comment.



---

archive/issue_events_349192.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-11-20T16:52:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26711",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26711#event-349192"
}
```



---

archive/issue_events_349193.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-11-20T16:52:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26711",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26711#event-349193"
}
```



---

archive/issue_comments_414230.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nok, then",
    "created_at": "2018-11-20T16:52:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26711#issuecomment-414230",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:6'>Comment 6:</a>
ok, then



---

archive/issue_events_349194.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-11-21T19:54:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26711",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26711#event-349194"
}
```



---

archive/issue_events_349195.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "cbe26c565f0bb0649ec87f137e7deba3875d5d52",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-11-21T19:54:44Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26711",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26711#event-349195"
}
```



---

archive/issue_comments_414231.json:
```json
{
    "body": "Changed branch from **[public/26711_graph_coloring_2](https://github.com/sagemath/sagetrac-mirror/tree/public/26711_graph_coloring_2)** to **[80f42b3](https://github.com/sagemath/sagetrac-mirror/commit/80f42b3e44d0752c509a43482b0da99aea43934a)**",
    "created_at": "2018-11-21T19:54:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26711#issuecomment-414231",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/26711_graph_coloring_2](https://github.com/sagemath/sagetrac-mirror/tree/public/26711_graph_coloring_2)** to **[80f42b3](https://github.com/sagemath/sagetrac-mirror/commit/80f42b3e44d0752c509a43482b0da99aea43934a)**
