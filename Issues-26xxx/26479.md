# Issue 26479: padic printer does not support latex names

archive/issues_026242.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: K.<pi> = Qp(2).extension(x^3 - 2)\nsage: latex(pi)\n'pi + O(pi^61)'\n```\n\nCC:  @roed314\n\nBranch/Commit: **[fb3bbaa](https://github.com/sagemath/sagetrac-mirror/commit/fb3bbaa1e333112e4ee34bec5712b45a845841ac)**\n\nReviewer: **David Roe, Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Xavier Caruso**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26479_\n\n",
    "closed_at": "2018-10-26T22:15:58Z",
    "created_at": "2018-10-12T15:59:48Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20padics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "padic printer does not support latex names",
    "type": "issue",
    "updated_at": "2018-10-28T14:52:23Z",
    "url": "https://github.com/sagemath/sage/issues/26479",
    "user": "https://github.com/xcaruso"
}
```

```
sage: K.<pi> = Qp(2).extension(x^3 - 2)
sage: latex(pi)
'pi + O(pi^61)'
```

CC:  @roed314

Branch/Commit: **[fb3bbaa](https://github.com/sagemath/sagetrac-mirror/commit/fb3bbaa1e333112e4ee34bec5712b45a845841ac)**

Reviewer: **David Roe, Frédéric Chapoton**

Author: **Xavier Caruso**

_Issue created by migration from https://trac.sagemath.org/ticket/26479_





---

archive/issue_events_346666.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2018-10-12T15:59:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26479",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26479#event-346666"
}
```



---

archive/issue_events_346667.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2018-10-12T15:59:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26479",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20padics",
    "label_color": "0000ff",
    "label_name": "component: padics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26479#event-346667"
}
```



---

archive/issue_events_346668.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2018-10-12T15:59:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26479",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26479#event-346668"
}
```



---

archive/issue_events_346669.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2018-10-12T15:59:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26479",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26479#event-346669"
}
```



---

archive/issue_events_346670.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2018-10-13T20:10:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26479",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26479#event-346670"
}
```



---

archive/issue_comments_411072.json:
```json
{
    "body": "Branch: **[u/caruso/padic_printer_latex](https://github.com/sagemath/sagetrac-mirror/tree/u/caruso/padic_printer_latex)**",
    "created_at": "2018-10-13T20:10:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26479",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26479#issuecomment-411072",
    "user": "https://github.com/xcaruso"
}
```

Branch: **[u/caruso/padic_printer_latex](https://github.com/sagemath/sagetrac-mirror/tree/u/caruso/padic_printer_latex)**



---

archive/issue_comments_411073.json:
```json
{
    "body": "Commit: **[8b18164](https://github.com/sagemath/sagetrac-mirror/commit/8b1816472114ac456a509e11cdd974274041fe4b)**",
    "created_at": "2018-10-13T20:10:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26479",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26479#issuecomment-411073",
    "user": "https://github.com/xcaruso"
}
```

Commit: **[8b18164](https://github.com/sagemath/sagetrac-mirror/commit/8b1816472114ac456a509e11cdd974274041fe4b)**



---

archive/issue_comments_411074.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nPlease undo all changes from `\\ZZ` to `\\Bold{Z}` in the documentation.\n\nFor the uniformity of sage documentation, it is better to always use `\\ZZ`.",
    "created_at": "2018-10-17T18:00:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26479",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26479#issuecomment-411074",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

Please undo all changes from `\ZZ` to `\Bold{Z}` in the documentation.

For the uniformity of sage documentation, it is better to always use `\ZZ`.



---

archive/issue_comments_411075.json:
```json
{
    "body": "Changed commit from **[8b18164](https://github.com/sagemath/sagetrac-mirror/commit/8b1816472114ac456a509e11cdd974274041fe4b)** to **[008d7fc](https://github.com/sagemath/sagetrac-mirror/commit/008d7fc45c055246740f55d524fca084f19914e0)**",
    "created_at": "2018-10-19T08:18:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26479",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26479#issuecomment-411075",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[8b18164](https://github.com/sagemath/sagetrac-mirror/commit/8b1816472114ac456a509e11cdd974274041fe4b)** to **[008d7fc](https://github.com/sagemath/sagetrac-mirror/commit/008d7fc45c055246740f55d524fca084f19914e0)**



---

archive/issue_comments_411076.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5ca2bd09636fa7bf6648558dd9310fa1430dacf3\">5ca2bd0</a></td><td><code>Replace back \\Bold{Z} by \\ZZ, etc. in the documentation</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/83b48e368317c0b57063e5c3091539267ee30c2d\">83b48e3</a></td><td><code>Replace \\mathbb{Z}, \\mathbb{Q} by \\ZZ, \\QQ respectively</code></td></tr><tr><td>[008d7fc](https://github.com/sagemath/sagetrac-mirror/commit/008d7fc45c055246740f55d524fca084f19914e0)</td><td><code>`Replace \\mathbb{f} by \\Bold{F`}</code></td></tr></table>\n",
    "created_at": "2018-10-19T08:18:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26479",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26479#issuecomment-411076",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5ca2bd09636fa7bf6648558dd9310fa1430dacf3">5ca2bd0</a></td><td><code>Replace back \Bold{Z} by \ZZ, etc. in the documentation</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/83b48e368317c0b57063e5c3091539267ee30c2d">83b48e3</a></td><td><code>Replace \mathbb{Z}, \mathbb{Q} by \ZZ, \QQ respectively</code></td></tr><tr><td>[008d7fc](https://github.com/sagemath/sagetrac-mirror/commit/008d7fc45c055246740f55d524fca084f19914e0)</td><td><code>`Replace \mathbb{f} by \Bold{F`}</code></td></tr></table>




---

archive/issue_comments_411077.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nReplying to [@fchapoton](#comment%3A2):\n> Please undo all changes from `\\ZZ` to `\\Bold{Z}` in the documentation.\n\n>\n> For the uniformity of sage documentation, it is better to always use `\\ZZ`.\n\nAh OK. It's a kind of trap because we cannot use `\\ZZ` in the notebook for instance.\nAnyway, I made the changes and also replaced `\\mathbb{Z}` by `\\ZZ`, etc. Please tell me if it's not appropriate.",
    "created_at": "2018-10-19T08:20:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26479",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26479#issuecomment-411077",
    "user": "https://github.com/xcaruso"
}
```

<div id="comment:4" align="right">Comment 4</div>

Replying to [@fchapoton](#comment%3A2):
> Please undo all changes from `\ZZ` to `\Bold{Z}` in the documentation.

>
> For the uniformity of sage documentation, it is better to always use `\ZZ`.

Ah OK. It's a kind of trap because we cannot use `\ZZ` in the notebook for instance.
Anyway, I made the changes and also replaced `\mathbb{Z}` by `\ZZ`, etc. Please tell me if it's not appropriate.



---

archive/issue_comments_411078.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nI think we also have `\\GF` for finite fields. But all these cosmetic changes should really be done IMHO in another ticket, because they currently hide the important changes in this ticket, which get lost in the middle of all the changes made.",
    "created_at": "2018-10-19T10:01:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26479",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26479#issuecomment-411078",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:5" align="right">Comment 5</div>

I think we also have `\GF` for finite fields. But all these cosmetic changes should really be done IMHO in another ticket, because they currently hide the important changes in this ticket, which get lost in the middle of all the changes made.



---

archive/issue_comments_411079.json:
```json
{
    "body": "Changed commit from **[008d7fc](https://github.com/sagemath/sagetrac-mirror/commit/008d7fc45c055246740f55d524fca084f19914e0)** to **[9e85f91](https://github.com/sagemath/sagetrac-mirror/commit/9e85f914c9bbfbb45c0123b241512ffba24bb757)**",
    "created_at": "2018-10-19T11:53:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26479",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26479#issuecomment-411079",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[008d7fc](https://github.com/sagemath/sagetrac-mirror/commit/008d7fc45c055246740f55d524fca084f19914e0)** to **[9e85f91](https://github.com/sagemath/sagetrac-mirror/commit/9e85f914c9bbfbb45c0123b241512ffba24bb757)**



---

archive/issue_comments_411080.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e83713c07c7d1232682288deb77a2836ead2367a\">e83713c</a></td><td><code>Revert \"Replace \\mathbb{f} by \\Bold{F}\"</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2e41efde9b71e5a9277a8ee7a2208a428aab3f59\">2e41efd</a></td><td><code>Revert \"Replace \\mathbb{Z}, \\mathbb{Q} by \\ZZ, \\QQ respectively\"</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9e85f914c9bbfbb45c0123b241512ffba24bb757\">9e85f91</a></td><td><code>Small fixes</code></td></tr></table>\n",
    "created_at": "2018-10-19T11:53:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26479",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26479#issuecomment-411080",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e83713c07c7d1232682288deb77a2836ead2367a">e83713c</a></td><td><code>Revert "Replace \mathbb{f} by \Bold{F}"</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2e41efde9b71e5a9277a8ee7a2208a428aab3f59">2e41efd</a></td><td><code>Revert "Replace \mathbb{Z}, \mathbb{Q} by \ZZ, \QQ respectively"</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9e85f914c9bbfbb45c0123b241512ffba24bb757">9e85f91</a></td><td><code>Small fixes</code></td></tr></table>




---

archive/issue_comments_411081.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nOK, if you insist, I split this ticket into two parts.",
    "created_at": "2018-10-19T11:54:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26479",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26479#issuecomment-411081",
    "user": "https://github.com/xcaruso"
}
```

<div id="comment:7" align="right">Comment 7</div>

OK, if you insist, I split this ticket into two parts.



---

archive/issue_comments_411082.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fb3bbaa1e333112e4ee34bec5712b45a845841ac\">fb3bbaa</a></td><td><code>Fix doctests</code></td></tr></table>\n",
    "created_at": "2018-10-20T09:19:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26479",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26479#issuecomment-411082",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:8" align="right">Comment 8</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fb3bbaa1e333112e4ee34bec5712b45a845841ac">fb3bbaa</a></td><td><code>Fix doctests</code></td></tr></table>




---

archive/issue_comments_411083.json:
```json
{
    "body": "Changed commit from **[9e85f91](https://github.com/sagemath/sagetrac-mirror/commit/9e85f914c9bbfbb45c0123b241512ffba24bb757)** to **[fb3bbaa](https://github.com/sagemath/sagetrac-mirror/commit/fb3bbaa1e333112e4ee34bec5712b45a845841ac)**",
    "created_at": "2018-10-20T09:19:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26479",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26479#issuecomment-411083",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[9e85f91](https://github.com/sagemath/sagetrac-mirror/commit/9e85f914c9bbfbb45c0123b241512ffba24bb757)** to **[fb3bbaa](https://github.com/sagemath/sagetrac-mirror/commit/fb3bbaa1e333112e4ee34bec5712b45a845841ac)**



---

archive/issue_comments_411084.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nLooks good!  I ran long tests and they pass.",
    "created_at": "2018-10-25T21:26:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26479",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26479#issuecomment-411084",
    "user": "https://github.com/roed314"
}
```

<div id="comment:9" align="right">Comment 9</div>

Looks good!  I ran long tests and they pass.



---

archive/issue_comments_411085.json:
```json
{
    "body": "Reviewer: **David Roe, Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2018-10-25T21:26:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26479",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26479#issuecomment-411085",
    "user": "https://github.com/roed314"
}
```

Reviewer: **David Roe, Frédéric Chapoton**



---

archive/issue_events_346671.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2018-10-25T21:26:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26479",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26479#event-346671"
}
```



---

archive/issue_events_346672.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2018-10-25T21:26:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26479",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26479#event-346672"
}
```



---

archive/issue_comments_411086.json:
```json
{
    "body": "Author: **Xavier Caruso**",
    "created_at": "2018-10-25T21:26:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26479",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26479#issuecomment-411086",
    "user": "https://github.com/roed314"
}
```

Author: **Xavier Caruso**



---

archive/issue_comments_411087.json:
```json
{
    "body": "Changed branch from **[u/caruso/padic_printer_latex](https://github.com/sagemath/sagetrac-mirror/tree/u/caruso/padic_printer_latex)** to **[fb3bbaa](https://github.com/sagemath/sagetrac-mirror/commit/fb3bbaa1e333112e4ee34bec5712b45a845841ac)**",
    "created_at": "2018-10-26T22:15:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26479",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26479#issuecomment-411087",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/caruso/padic_printer_latex](https://github.com/sagemath/sagetrac-mirror/tree/u/caruso/padic_printer_latex)** to **[fb3bbaa](https://github.com/sagemath/sagetrac-mirror/commit/fb3bbaa1e333112e4ee34bec5712b45a845841ac)**



---

archive/issue_events_346673.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-10-26T22:15:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26479",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26479#event-346673"
}
```



---

archive/issue_events_346674.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "022c7514fc6006a0b8f3e0ea09ed45bbed008852",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-10-26T22:15:58Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26479",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26479#event-346674"
}
```



---

archive/issue_events_346675.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-10-28T14:52:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/26479",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26479#event-346675"
}
```



---

archive/issue_events_346676.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-10-28T14:52:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26479",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26479#event-346676"
}
```



---

archive/issue_comments_411088.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nThis should be re-targeted for 8.5.",
    "created_at": "2018-10-28T14:52:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26479",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26479#issuecomment-411088",
    "user": "https://github.com/embray"
}
```

<div id="comment:11" align="right">Comment 11</div>

This should be re-targeted for 8.5.
