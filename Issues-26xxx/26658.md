# Issue 26658: clean generic_graph.py (part 7) - planarity

archive/issues_026421.json:
```json
{
    "assignees": [],
    "body": "Here we clean methods related to planarity: `is_planar`, `is_circular_planar`, `layout_planar`, `is_drawn_free_of_edge_crossings`, `genus`, `crossing_number`, `faces`, `num_faces`, `planar_dual`.\n- PEP8 cleaning\n- avoid using `.edges`, or use `.edges(sort=False)`\n\nNot done:\n- `set_planar_positions`: it is written inside the method: `This method is deprecated since Sage-4.4.1.alpha2. Please use instead `.layout`. However, there is no clear deprecation warning. We can either add a proper deprecation warning and remove it in 1 year, or simply remove it as it has been marked as deprecated for so many years... A specific ticket is needed anyway.\n\nCC:  @tscrim @fchapoton\n\nKeywords: **py3, graph**\n\nBranch/Commit: **[b79c678](https://github.com/sagemath/sagetrac-mirror/commit/b79c6784602c6deb2f3b3b15b7fa07068f963732)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26658_\n\n",
    "closed_at": "2018-11-11T22:19:53Z",
    "created_at": "2018-11-07T14:09:38Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "clean generic_graph.py (part 7) - planarity",
    "type": "issue",
    "updated_at": "2018-11-11T22:19:53Z",
    "url": "https://github.com/sagemath/sage/issues/26658",
    "user": "https://github.com/dcoudert"
}
```
Here we clean methods related to planarity: `is_planar`, `is_circular_planar`, `layout_planar`, `is_drawn_free_of_edge_crossings`, `genus`, `crossing_number`, `faces`, `num_faces`, `planar_dual`.
- PEP8 cleaning
- avoid using `.edges`, or use `.edges(sort=False)`

Not done:
- `set_planar_positions`: it is written inside the method: `This method is deprecated since Sage-4.4.1.alpha2. Please use instead `.layout`. However, there is no clear deprecation warning. We can either add a proper deprecation warning and remove it in 1 year, or simply remove it as it has been marked as deprecated for so many years... A specific ticket is needed anyway.

CC:  @tscrim @fchapoton

Keywords: **py3, graph**

Branch/Commit: **[b79c678](https://github.com/sagemath/sagetrac-mirror/commit/b79c6784602c6deb2f3b3b15b7fa07068f963732)**

Reviewer: **Travis Scrimshaw**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/26658_





---

archive/issue_events_348651.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-07T14:09:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26658",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26658#event-348651"
}
```



---

archive/issue_events_348652.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-07T14:09:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26658",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26658#event-348652"
}
```



---

archive/issue_events_348653.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-07T14:09:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26658",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26658#event-348653"
}
```



---

archive/issue_events_348654.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-07T14:09:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26658",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26658#event-348654"
}
```



---

archive/issue_comments_413661.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/720f95f974d958bf893be1b14bb47f70ff15d292\">720f95f</a></td><td><code>trac #26658: clean generic_graph.py part 7 - planarity methods</code></td></tr></table>\n",
    "created_at": "2018-11-07T14:14:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26658",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26658#issuecomment-413661",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/720f95f974d958bf893be1b14bb47f70ff15d292">720f95f</a></td><td><code>trac #26658: clean generic_graph.py part 7 - planarity methods</code></td></tr></table>




---

archive/issue_events_348655.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-07T14:14:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26658",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26658#event-348655"
}
```



---

archive/issue_comments_413662.json:
```json
{
    "body": "Branch: **[public/26658_generic_graph_part_7_planarity](https://github.com/sagemath/sagetrac-mirror/tree/public/26658_generic_graph_part_7_planarity)**",
    "created_at": "2018-11-07T14:14:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26658",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26658#issuecomment-413662",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[public/26658_generic_graph_part_7_planarity](https://github.com/sagemath/sagetrac-mirror/tree/public/26658_generic_graph_part_7_planarity)**



---

archive/issue_comments_413663.json:
```json
{
    "body": "Commit: **[720f95f](https://github.com/sagemath/sagetrac-mirror/commit/720f95f974d958bf893be1b14bb47f70ff15d292)**",
    "created_at": "2018-11-07T14:14:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26658",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26658#issuecomment-413663",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[720f95f](https://github.com/sagemath/sagetrac-mirror/commit/720f95f974d958bf893be1b14bb47f70ff15d292)**



---

archive/issue_comments_413664.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2018-11-08T14:38:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26658",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26658#issuecomment-413664",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_413665.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nAnother nitpick and while-we-are-at-it: we should not use contractions in error messages (`can't`) to be more formal. However, if you do not want to change that, you can set a positive review.",
    "created_at": "2018-11-08T14:38:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26658",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26658#issuecomment-413665",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:2" align="right">Comment 2</div>

Another nitpick and while-we-are-at-it: we should not use contractions in error messages (`can't`) to be more formal. However, if you do not want to change that, you can set a positive review.



---

archive/issue_comments_413666.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b79c6784602c6deb2f3b3b15b7fa07068f963732\">b79c678</a></td><td><code>trac #26658: can't -> cannot</code></td></tr></table>\n",
    "created_at": "2018-11-08T17:01:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26658",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26658#issuecomment-413666",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b79c6784602c6deb2f3b3b15b7fa07068f963732">b79c678</a></td><td><code>trac #26658: can't -> cannot</code></td></tr></table>




---

archive/issue_comments_413667.json:
```json
{
    "body": "Changed commit from **[720f95f](https://github.com/sagemath/sagetrac-mirror/commit/720f95f974d958bf893be1b14bb47f70ff15d292)** to **[b79c678](https://github.com/sagemath/sagetrac-mirror/commit/b79c6784602c6deb2f3b3b15b7fa07068f963732)**",
    "created_at": "2018-11-08T17:01:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26658",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26658#issuecomment-413667",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[720f95f](https://github.com/sagemath/sagetrac-mirror/commit/720f95f974d958bf893be1b14bb47f70ff15d292)** to **[b79c678](https://github.com/sagemath/sagetrac-mirror/commit/b79c6784602c6deb2f3b3b15b7fa07068f963732)**



---

archive/issue_comments_413668.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nI was not sure if `can't` was ok or not. Now I know and so I followed your recommendation.",
    "created_at": "2018-11-08T17:03:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26658",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26658#issuecomment-413668",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:4" align="right">Comment 4</div>

I was not sure if `can't` was ok or not. Now I know and so I followed your recommendation.



---

archive/issue_events_348656.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-08T17:03:26Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/26658",
    "title_is": "clean generic_graph.py (part 7) - planarity",
    "title_was": "clean connectivity.pyx (part 7) - planarity",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26658#event-348656"
}
```



---

archive/issue_comments_413669.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nI don't think there is an official policy, but it just makes sense in terms of formality (e.g., you would not write it in a paper). Thank you for also fixing it.",
    "created_at": "2018-11-08T19:38:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26658",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26658#issuecomment-413669",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:5" align="right">Comment 5</div>

I don't think there is an official policy, but it just makes sense in terms of formality (e.g., you would not write it in a paper). Thank you for also fixing it.



---

archive/issue_events_348657.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-11-08T19:38:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26658",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26658#event-348657"
}
```



---

archive/issue_events_348658.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-11-08T19:38:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26658",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26658#event-348658"
}
```



---

archive/issue_comments_413670.json:
```json
{
    "body": "Changed branch from **[public/26658_generic_graph_part_7_planarity](https://github.com/sagemath/sagetrac-mirror/tree/public/26658_generic_graph_part_7_planarity)** to **[b79c678](https://github.com/sagemath/sagetrac-mirror/commit/b79c6784602c6deb2f3b3b15b7fa07068f963732)**",
    "created_at": "2018-11-11T22:19:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26658",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26658#issuecomment-413670",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/26658_generic_graph_part_7_planarity](https://github.com/sagemath/sagetrac-mirror/tree/public/26658_generic_graph_part_7_planarity)** to **[b79c678](https://github.com/sagemath/sagetrac-mirror/commit/b79c6784602c6deb2f3b3b15b7fa07068f963732)**



---

archive/issue_events_348659.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-11-11T22:19:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26658",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26658#event-348659"
}
```



---

archive/issue_events_348660.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "3c1e3bf8423a5bed4844d5b4666c136c2781d01d",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-11-11T22:19:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26658",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26658#event-348660"
}
```
