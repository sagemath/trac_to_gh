# Issue 26547: clean spanning_tree.pyx

archive/issues_026310.json:
```json
{
    "assignees": [],
    "body": "Done in this ticket:\n- rewrite/simply `kruskal` using proper `DisjointSet`\n- avoid comparison of vertex labels in `boruvka` using `frozenset`\n- PEP8 cleaning\n\nSome `sort` operations remain in `kruskal`.\n- Edges must be visited by increasing weights. However, there is no reason for sorting by vertex labels as well\n- the output list of edges is sorted. There is no reason for this, especially since other methods don't do that, e.g., `boruvka`\nSo we can decide to change that as well in this ticket.\n\nCC:  @tscrim @fchapoton\n\nKeywords: **py3, graph**\n\nBranch/Commit: **[dce10d8](https://github.com/sagemath/sagetrac-mirror/commit/dce10d89f35d9b6ff89791781ff75521e6f146eb)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26547_\n\n",
    "closed_at": "2018-10-25T06:46:56Z",
    "created_at": "2018-10-24T12:52:48Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "clean spanning_tree.pyx",
    "type": "issue",
    "updated_at": "2018-10-25T06:46:56Z",
    "url": "https://github.com/sagemath/sage/issues/26547",
    "user": "https://github.com/dcoudert"
}
```
Done in this ticket:
- rewrite/simply `kruskal` using proper `DisjointSet`
- avoid comparison of vertex labels in `boruvka` using `frozenset`
- PEP8 cleaning

Some `sort` operations remain in `kruskal`.
- Edges must be visited by increasing weights. However, there is no reason for sorting by vertex labels as well
- the output list of edges is sorted. There is no reason for this, especially since other methods don't do that, e.g., `boruvka`
So we can decide to change that as well in this ticket.

CC:  @tscrim @fchapoton

Keywords: **py3, graph**

Branch/Commit: **[dce10d8](https://github.com/sagemath/sagetrac-mirror/commit/dce10d89f35d9b6ff89791781ff75521e6f146eb)**

Reviewer: **Travis Scrimshaw**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/26547_





---

archive/issue_events_347450.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-10-24T12:52:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26547",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26547#event-347450"
}
```



---

archive/issue_events_347451.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-10-24T12:52:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26547",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26547#event-347451"
}
```



---

archive/issue_events_347452.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-10-24T12:52:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26547",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26547#event-347452"
}
```



---

archive/issue_events_347453.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-10-24T12:52:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26547",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26547#event-347453"
}
```



---

archive/issue_comments_412060.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/dce10d89f35d9b6ff89791781ff75521e6f146eb\">dce10d8</a></td><td><code>trac #26547: clean spanning_tree.pyx</code></td></tr></table>\n",
    "created_at": "2018-10-24T12:54:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26547",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26547#issuecomment-412060",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/dce10d89f35d9b6ff89791781ff75521e6f146eb">dce10d8</a></td><td><code>trac #26547: clean spanning_tree.pyx</code></td></tr></table>




---

archive/issue_comments_412061.json:
```json
{
    "body": "Branch: **[public/26547_spanning_tree](https://github.com/sagemath/sagetrac-mirror/tree/public/26547_spanning_tree)**",
    "created_at": "2018-10-24T12:54:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26547",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26547#issuecomment-412061",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[public/26547_spanning_tree](https://github.com/sagemath/sagetrac-mirror/tree/public/26547_spanning_tree)**



---

archive/issue_comments_412062.json:
```json
{
    "body": "Commit: **[dce10d8](https://github.com/sagemath/sagetrac-mirror/commit/dce10d89f35d9b6ff89791781ff75521e6f146eb)**",
    "created_at": "2018-10-24T12:54:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26547",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26547#issuecomment-412062",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[dce10d8](https://github.com/sagemath/sagetrac-mirror/commit/dce10d89f35d9b6ff89791781ff75521e6f146eb)**



---

archive/issue_events_347454.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-10-24T14:40:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26547",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26547#event-347454"
}
```



---

archive/issue_comments_412063.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2018-10-24T20:31:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26547",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26547#issuecomment-412063",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_347455.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-10-24T20:31:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26547",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26547#event-347455"
}
```



---

archive/issue_events_347456.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-10-24T20:31:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26547",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26547#event-347456"
}
```



---

archive/issue_comments_412064.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLGTM.",
    "created_at": "2018-10-24T20:31:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26547",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26547#issuecomment-412064",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:3" align="right">Comment 3</div>

LGTM.



---

archive/issue_events_347457.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-10-25T06:46:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26547",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26547#event-347457"
}
```



---

archive/issue_events_347458.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "e23129c018af24a01a96e7a739a8702b7e3a4dc5",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-10-25T06:46:56Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26547",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26547#event-347458"
}
```



---

archive/issue_comments_412065.json:
```json
{
    "body": "Changed branch from **[public/26547_spanning_tree](https://github.com/sagemath/sagetrac-mirror/tree/public/26547_spanning_tree)** to **[dce10d8](https://github.com/sagemath/sagetrac-mirror/commit/dce10d89f35d9b6ff89791781ff75521e6f146eb)**",
    "created_at": "2018-10-25T06:46:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26547",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26547#issuecomment-412065",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/26547_spanning_tree](https://github.com/sagemath/sagetrac-mirror/tree/public/26547_spanning_tree)** to **[dce10d8](https://github.com/sagemath/sagetrac-mirror/commit/dce10d89f35d9b6ff89791781ff75521e6f146eb)**
