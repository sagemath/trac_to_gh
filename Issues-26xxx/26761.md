# Issue 26761: py3: fix BlanusaSecondSnarkGraph

archive/issues_026524.json:
```json
{
    "assignees": [],
    "body": "We fix this generator for Python 3 by avoiding to get both vertices labeled with an integer and vertices labeled with tuples.\n\nCC:  @tscrim @fchapoton\n\nKeywords: **py3, graph**\n\nBranch/Commit: **[aa4221f](https://github.com/sagemath/sagetrac-mirror/commit/aa4221f0ec1d3ab459a071476490e418df89c9be)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26761_\n\n",
    "closed_at": "2018-11-27T23:47:50Z",
    "created_at": "2018-11-26T08:51:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "py3: fix BlanusaSecondSnarkGraph",
    "type": "issue",
    "updated_at": "2018-11-27T23:47:50Z",
    "url": "https://github.com/sagemath/sage/issues/26761",
    "user": "https://github.com/dcoudert"
}
```
We fix this generator for Python 3 by avoiding to get both vertices labeled with an integer and vertices labeled with tuples.

CC:  @tscrim @fchapoton

Keywords: **py3, graph**

Branch/Commit: **[aa4221f](https://github.com/sagemath/sagetrac-mirror/commit/aa4221f0ec1d3ab459a071476490e418df89c9be)**

Reviewer: **Frédéric Chapoton**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/26761_





---

archive/issue_events_349733.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-26T08:51:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26761",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26761#event-349733"
}
```



---

archive/issue_events_349734.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-26T08:51:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26761",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26761#event-349734"
}
```



---

archive/issue_events_349735.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-26T08:51:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26761",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26761#event-349735"
}
```



---

archive/issue_events_349736.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-26T08:51:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26761",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26761#event-349736"
}
```



---

archive/issue_comments_414927.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-We fix this generator by avoiding to get both vertices labeled with an integer and vertices labeled with tuples.\n+We fix this generator for Python 3 by avoiding to get both vertices labeled with an integer and vertices labeled with tuples.\n``````\n",
    "created_at": "2018-11-26T08:56:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26761#issuecomment-414927",
    "user": "https://github.com/dcoudert"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-We fix this generator by avoiding to get both vertices labeled with an integer and vertices labeled with tuples.
+We fix this generator for Python 3 by avoiding to get both vertices labeled with an integer and vertices labeled with tuples.
``````




---

archive/issue_comments_414928.json:
```json
{
    "body": "Branch: **[public/26761_blanusa_second_snark](https://github.com/sagemath/sagetrac-mirror/tree/public/26761_blanusa_second_snark)**",
    "created_at": "2018-11-26T08:56:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26761#issuecomment-414928",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[public/26761_blanusa_second_snark](https://github.com/sagemath/sagetrac-mirror/tree/public/26761_blanusa_second_snark)**



---

archive/issue_events_349737.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-26T08:56:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26761",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26761#event-349737"
}
```



---

archive/issue_comments_414929.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/aa4221f0ec1d3ab459a071476490e418df89c9be\">aa4221f</a></td><td><code>trac #26761: fix BlanusaSecondSnarkGraph</code></td></tr></table>\n",
    "created_at": "2018-11-26T08:56:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26761#issuecomment-414929",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/aa4221f0ec1d3ab459a071476490e418df89c9be">aa4221f</a></td><td><code>trac #26761: fix BlanusaSecondSnarkGraph</code></td></tr></table>




---

archive/issue_events_349738.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-26T08:56:31Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/26761",
    "title_is": "py3: fix BlanusaSecondSnarkGraph",
    "title_was": "py3: fix BlanusaFirstSnarkGraph",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26761#event-349738"
}
```



---

archive/issue_comments_414930.json:
```json
{
    "body": "Commit: **[aa4221f](https://github.com/sagemath/sagetrac-mirror/commit/aa4221f0ec1d3ab459a071476490e418df89c9be)**",
    "created_at": "2018-11-26T08:56:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26761#issuecomment-414930",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[aa4221f](https://github.com/sagemath/sagetrac-mirror/commit/aa4221f0ec1d3ab459a071476490e418df89c9be)**



---

archive/issue_comments_414931.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nok",
    "created_at": "2018-11-26T13:19:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26761#issuecomment-414931",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

ok



---

archive/issue_comments_414932.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2018-11-26T13:19:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26761#issuecomment-414932",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_events_349739.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-11-26T13:19:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26761",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26761#event-349739"
}
```



---

archive/issue_events_349740.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-11-26T13:19:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26761",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26761#event-349740"
}
```



---

archive/issue_events_349741.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-11-27T23:47:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26761",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26761#event-349741"
}
```



---

archive/issue_events_349742.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "c291f5ff1d4b82d600bd3f8b02587da8c1fe39d2",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-11-27T23:47:50Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26761",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26761#event-349742"
}
```



---

archive/issue_comments_414933.json:
```json
{
    "body": "Changed branch from **[public/26761_blanusa_second_snark](https://github.com/sagemath/sagetrac-mirror/tree/public/26761_blanusa_second_snark)** to **[aa4221f](https://github.com/sagemath/sagetrac-mirror/commit/aa4221f0ec1d3ab459a071476490e418df89c9be)**",
    "created_at": "2018-11-27T23:47:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26761",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26761#issuecomment-414933",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/26761_blanusa_second_snark](https://github.com/sagemath/sagetrac-mirror/tree/public/26761_blanusa_second_snark)** to **[aa4221f](https://github.com/sagemath/sagetrac-mirror/commit/aa4221f0ec1d3ab459a071476490e418df89c9be)**
