# Issue 26642: Errors when copying files ignored

archive/issues_026405.json:
```json
{
    "assignees": [],
    "body": "This piece in `sage-spkg` is wrong:\n\n```\n    # Copy files into $SAGE_LOCAL\n    $SAGE_SUDO cp -Rp \"$PREFIX/.\" \"$SAGE_LOCAL\"\n    rm -rf \"$SAGE_DESTDIR\"\n    if [ $? -ne 0 ]; then\n        error_msg \"Error copying files for $PKG_NAME.\"\n        exit 1\n    fi\n```\nThis is checking for errors from `rm -rf` instead of from the copy operation.\n\nCC:  @embray\n\nBranch/Commit: **[2c2df8d](https://github.com/sagemath/sagetrac-mirror/commit/2c2df8d2df48ba36537209e5c1cf3608feb24772)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26642_\n\n",
    "closed_at": "2018-11-07T11:28:18Z",
    "created_at": "2018-11-05T13:21:51Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Errors when copying files ignored",
    "type": "issue",
    "updated_at": "2018-11-07T11:28:18Z",
    "url": "https://github.com/sagemath/sage/issues/26642",
    "user": "https://github.com/jdemeyer"
}
```
This piece in `sage-spkg` is wrong:

```
    # Copy files into $SAGE_LOCAL
    $SAGE_SUDO cp -Rp "$PREFIX/." "$SAGE_LOCAL"
    rm -rf "$SAGE_DESTDIR"
    if [ $? -ne 0 ]; then
        error_msg "Error copying files for $PKG_NAME."
        exit 1
    fi
```
This is checking for errors from `rm -rf` instead of from the copy operation.

CC:  @embray

Branch/Commit: **[2c2df8d](https://github.com/sagemath/sagetrac-mirror/commit/2c2df8d2df48ba36537209e5c1cf3608feb24772)**

Reviewer: **Frédéric Chapoton**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/26642_





---

archive/issue_events_348505.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-11-05T13:21:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26642",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26642#event-348505"
}
```



---

archive/issue_events_348506.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-11-05T13:21:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26642",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26642#event-348506"
}
```



---

archive/issue_events_348507.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-11-05T13:21:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26642",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26642#event-348507"
}
```



---

archive/issue_events_348508.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-11-05T13:21:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26642",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26642#event-348508"
}
```



---

archive/issue_comments_413543.json:
```json
{
    "body": "Branch: **[u/jdemeyer/errors_when_copying_files_ignored](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/errors_when_copying_files_ignored)**",
    "created_at": "2018-11-05T13:25:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26642#issuecomment-413543",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/errors_when_copying_files_ignored](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/errors_when_copying_files_ignored)**



---

archive/issue_events_348509.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-11-05T13:46:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26642",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26642#event-348509"
}
```



---

archive/issue_comments_413544.json:
```json
{
    "body": "Commit: **[2c2df8d](https://github.com/sagemath/sagetrac-mirror/commit/2c2df8d2df48ba36537209e5c1cf3608feb24772)**",
    "created_at": "2018-11-05T13:46:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26642#issuecomment-413544",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[2c2df8d](https://github.com/sagemath/sagetrac-mirror/commit/2c2df8d2df48ba36537209e5c1cf3608feb24772)**



---

archive/issue_comments_413545.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2c2df8d2df48ba36537209e5c1cf3608feb24772\">2c2df8d</a></td><td><code>Properly check errors when copying files</code></td></tr></table>\n",
    "created_at": "2018-11-05T13:46:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26642#issuecomment-413545",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2c2df8d2df48ba36537209e5c1cf3608feb24772">2c2df8d</a></td><td><code>Properly check errors when copying files</code></td></tr></table>




---

archive/issue_comments_413546.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nok, looks good",
    "created_at": "2018-11-05T20:38:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26642#issuecomment-413546",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">Comment 3</div>

ok, looks good



---

archive/issue_events_348510.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-11-05T20:38:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26642",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26642#event-348510"
}
```



---

archive/issue_events_348511.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-11-05T20:38:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26642",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26642#event-348511"
}
```



---

archive/issue_comments_413547.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2018-11-05T20:38:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26642#issuecomment-413547",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_413548.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/errors_when_copying_files_ignored](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/errors_when_copying_files_ignored)** to **[2c2df8d](https://github.com/sagemath/sagetrac-mirror/commit/2c2df8d2df48ba36537209e5c1cf3608feb24772)**",
    "created_at": "2018-11-07T11:28:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26642#issuecomment-413548",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/errors_when_copying_files_ignored](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/errors_when_copying_files_ignored)** to **[2c2df8d](https://github.com/sagemath/sagetrac-mirror/commit/2c2df8d2df48ba36537209e5c1cf3608feb24772)**



---

archive/issue_events_348512.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-11-07T11:28:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26642",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26642#event-348512"
}
```



---

archive/issue_events_348513.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "4ccc06303e0fb31cd3075aa1ceed024ced740390",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-11-07T11:28:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26642",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26642#event-348513"
}
```
