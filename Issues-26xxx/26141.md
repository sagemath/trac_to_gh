# Issue 26141: fix bad code quality for recently merged #24837

archive/issues_025904.json:
```json
{
    "body": "In #24837 the method `repr_pretty_Hrepresentation` was changed to `Hrepresentation_str` but unfortunately the quality of the code does not meet Python's and SageMath's standards. The issues (relating to #24837, https://git.sagemath.org/sage.git/commit?id=c868d2c4a4e8b78a8221291741e878d468ddf367):\n1. There is no parameter setting which gives back the old default behavior, which is inequalities separated by commas. Using the separator, one only gets strings containing a lot of blanks, which simple cannot be read properly.\n2. The default values changed without a deprecation. The deprecation just aliases the old and new methods, but does not take care of the parameter setting. (I understand that some believe that this is a minor issue as the output is \"only\" a string represented to the user and not an (intermediate) result of a computation. However, one could have done this more smoothly)\n3. PEP8 is not satisfied, in particular some too long lines and a lot of missing blanks after commas.\n4. The code in some lines/blocks is considered bad in a Pythonic sense:\n    {{{\n    shift = any([pretty_hs[index][2][0] == '-' for index in range(len(pretty_hs))])\n    }}}\n   should be something like\n    {{{\n    shift = any(pretty_h[2][0] == '-' for pretty_h in pretty_hs)\n    }}}\n   or even better\n    {{{\n    shift = any(pretty_h[2].startswith('-') for pretty_h in pretty_hs)\n    }}}\n   Also get rid of the second `for index in range(len(...))`\n5. The line\n    {{{\n    pretty_print = pretty_print[:-1] # Removing the last return\n    }}}\n   only works if the separator has length 1. Hint: The Pythonic way to do this kind of concatentation is to use `separator.join(...)`.\n6. I cannot think of any reason, why the output has an additional blank at each end of line. Please remove this. (I personally would also get rid of all the blanks at the end of the line, i.e. the padding on the right. This maybe is a matter of taste, so should not bother us too much and probably people will find a lot of arguments why this is something wanted, so I just comment this here. ;) )\n7. Why\n    {{{\n    product = -1*(coeffs[1:]*vars[1:])\n    }}}\n   and not simply\n    {{{\n    product = -(...)\n    }}}\n   or even\n    {{{\n    product = -...\n    }}}\n    ?\n8. There is quite a lot of code doubling at the end of the patch of #24837:\n     {{{\n     if not split:\n         some code\n     else:\n         almost identical code with very few exceptions\n     }}}\n   This is very hard too read as one has to compare both almost identical blocks to find the differences. I am certain that a better solution can be found for this.\n9. Docstring of `style`-parameter: Shouldn't the commas be set differently: `either ..., or ... or ...` because the latter two should be grouped and not the former two?\n10. Docstring\n     {{{\n     ``latex`` -- a boolean. Default is ``None``\n     }}}\n   So what does `None` mean here? Is it set automatically according to the input? If static, probably its `False` but who knows. Why using `None` at all and not `False` as the default then?\n21. The default of the `style`-parameter is `'>='` in base.py and `'positive'` in representation.py. Having different default values within the same module for methods doing \"the same\" is usually not a good idea.\n32. The variable `split` needs a docstring explaining what it does as this is not obvious. (And, from a technical point of view, one could consider using its inverse `join`, as it is more a `' '.join(...)` than a `....split(' ')`, but this is highly debatable.)\n\nCC:  @jplab @tscrim @mo271 @vbraun @videlec\n\nBranch/Commit: 31765a5b15e3f7611c55d184857376a9b2350491\n\nReviewer: Daniel Krenn, Jean-Philippe Labb\u00e9\n\nAuthor: Jean-Philippe Labb\u00e9, Daniel Krenn\n\nDependencies: #24837\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/26141\n\n",
    "closed_at": "2018-09-03T23:29:35Z",
    "created_at": "2018-08-27T18:28:49Z",
    "labels": [
        "component: geometry",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.4",
    "title": "fix bad code quality for recently merged #24837",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/26141",
    "user": "https://github.com/dkrenn"
}
```
In #24837 the method `repr_pretty_Hrepresentation` was changed to `Hrepresentation_str` but unfortunately the quality of the code does not meet Python's and SageMath's standards. The issues (relating to #24837, https://git.sagemath.org/sage.git/commit?id=c868d2c4a4e8b78a8221291741e878d468ddf367):
1. There is no parameter setting which gives back the old default behavior, which is inequalities separated by commas. Using the separator, one only gets strings containing a lot of blanks, which simple cannot be read properly.
2. The default values changed without a deprecation. The deprecation just aliases the old and new methods, but does not take care of the parameter setting. (I understand that some believe that this is a minor issue as the output is "only" a string represented to the user and not an (intermediate) result of a computation. However, one could have done this more smoothly)
3. PEP8 is not satisfied, in particular some too long lines and a lot of missing blanks after commas.
4. The code in some lines/blocks is considered bad in a Pythonic sense:
    {{{
    shift = any([pretty_hs[index][2][0] == '-' for index in range(len(pretty_hs))])
    }}}
   should be something like
    {{{
    shift = any(pretty_h[2][0] == '-' for pretty_h in pretty_hs)
    }}}
   or even better
    {{{
    shift = any(pretty_h[2].startswith('-') for pretty_h in pretty_hs)
    }}}
   Also get rid of the second `for index in range(len(...))`
5. The line
    {{{
    pretty_print = pretty_print[:-1] # Removing the last return
    }}}
   only works if the separator has length 1. Hint: The Pythonic way to do this kind of concatentation is to use `separator.join(...)`.
6. I cannot think of any reason, why the output has an additional blank at each end of line. Please remove this. (I personally would also get rid of all the blanks at the end of the line, i.e. the padding on the right. This maybe is a matter of taste, so should not bother us too much and probably people will find a lot of arguments why this is something wanted, so I just comment this here. ;) )
7. Why
    {{{
    product = -1*(coeffs[1:]*vars[1:])
    }}}
   and not simply
    {{{
    product = -(...)
    }}}
   or even
    {{{
    product = -...
    }}}
    ?
8. There is quite a lot of code doubling at the end of the patch of #24837:
     {{{
     if not split:
         some code
     else:
         almost identical code with very few exceptions
     }}}
   This is very hard too read as one has to compare both almost identical blocks to find the differences. I am certain that a better solution can be found for this.
9. Docstring of `style`-parameter: Shouldn't the commas be set differently: `either ..., or ... or ...` because the latter two should be grouped and not the former two?
10. Docstring
     {{{
     ``latex`` -- a boolean. Default is ``None``
     }}}
   So what does `None` mean here? Is it set automatically according to the input? If static, probably its `False` but who knows. Why using `None` at all and not `False` as the default then?
21. The default of the `style`-parameter is `'>='` in base.py and `'positive'` in representation.py. Having different default values within the same module for methods doing "the same" is usually not a good idea.
32. The variable `split` needs a docstring explaining what it does as this is not obvious. (And, from a technical point of view, one could consider using its inverse `join`, as it is more a `' '.join(...)` than a `....split(' ')`, but this is highly debatable.)

CC:  @jplab @tscrim @mo271 @vbraun @videlec

Branch/Commit: 31765a5b15e3f7611c55d184857376a9b2350491

Reviewer: Daniel Krenn, Jean-Philippe Labbé

Author: Jean-Philippe Labbé, Daniel Krenn

Dependencies: #24837

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/26141





---

archive/issue_comments_502292.json:
```json
{
    "body": "<a id='comment:1'></a>I won't comment on this specific case, but you could also consider reverting #24837 and then fixing these issues. In any case, it would be good to have one of these 3 things **soon**:\n\n1. Agreement that the code is not so bad after all.\n\n2. A fix for the code.\n\n3. A revert of the code.",
    "created_at": "2018-08-27T19:28:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26141#issuecomment-502292",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:1'></a>I won't comment on this specific case, but you could also consider reverting #24837 and then fixing these issues. In any case, it would be good to have one of these 3 things **soon**:

1. Agreement that the code is not so bad after all.

2. A fix for the code.

3. A revert of the code.



---

archive/issue_comments_502293.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -50,9 +50,3 @@\n    So what does `None` mean here? Is it set automatically according to the input? If static, probably its `False` but who knows. Why using `None` at all and not `False` as the default then?\n 21. The default of the `style`-parameter is `'>='` in base.py and `'positive'` in representation.py. Having different default values within the same module for methods doing \"the same\" is usually not a good idea.\n 32. The variable `split` needs a docstring explaining what it does as this is not obvious. (And, from a technical point of view, one could consider using its inverse `join`, as it is more a `' '.join(...)` than a `....split(' ')`, but this is highly debatable.)\n-\n-#24837 has gone in 8.4.beta0 and no sign so far for 8.4, so there is still time to fix this. As it is code that should not have been beta-released and there is clearly something to do before the next release, I consider this a blocker ;)\n-\n-Therefore, dear authors and reviewers of #24837, please fix this until the next stable release. (I am happy to do the reviewing.)\n-\n-PS: If someone finds that I am overacting feel free to bring me down to earth; it won't be too hard, i promise ;)\n``````\n",
    "created_at": "2018-08-27T22:48:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26141#issuecomment-502293",
    "user": "https://github.com/tscrim"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -50,9 +50,3 @@
    So what does `None` mean here? Is it set automatically according to the input? If static, probably its `False` but who knows. Why using `None` at all and not `False` as the default then?
 21. The default of the `style`-parameter is `'>='` in base.py and `'positive'` in representation.py. Having different default values within the same module for methods doing "the same" is usually not a good idea.
 32. The variable `split` needs a docstring explaining what it does as this is not obvious. (And, from a technical point of view, one could consider using its inverse `join`, as it is more a `' '.join(...)` than a `....split(' ')`, but this is highly debatable.)
-
-#24837 has gone in 8.4.beta0 and no sign so far for 8.4, so there is still time to fix this. As it is code that should not have been beta-released and there is clearly something to do before the next release, I consider this a blocker ;)
-
-Therefore, dear authors and reviewers of #24837, please fix this until the next stable release. (I am happy to do the reviewing.)
-
-PS: If someone finds that I am overacting feel free to bring me down to earth; it won't be too hard, i promise ;)
``````




---

archive/issue_comments_502294.json:
```json
{
    "body": "<a id='comment:2'></a>Dear Daniel, none of these issues are blockers to me. Please don't throw such strong statements around carelessly. Also, since a lot of these issues are simple, why are you asking others to do it? Why can't you take care of them yourself? If you have time to review, you have time to code.\n\nAlso, code does not need to be perfect to get into Sage (something about the enemy of good comes to mind). I am happy to delegate work to subsequent tickets.\n\nSome technical notes. PEP8 are guidelines, good ones that should be followed as much as possible, but guidelines all the same. Same for \"Pythonic\" code (also you can get a speed bonus for doing these things, not that it is particularly useful here). Sometimes you have to duplicate code or you have to make things somewhat convoluted to not do the duplication. Minor differences are differences. It is also *really* annoying to deprecate default value changes.",
    "created_at": "2018-08-27T22:48:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26141#issuecomment-502294",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:2'></a>Dear Daniel, none of these issues are blockers to me. Please don't throw such strong statements around carelessly. Also, since a lot of these issues are simple, why are you asking others to do it? Why can't you take care of them yourself? If you have time to review, you have time to code.

Also, code does not need to be perfect to get into Sage (something about the enemy of good comes to mind). I am happy to delegate work to subsequent tickets.

Some technical notes. PEP8 are guidelines, good ones that should be followed as much as possible, but guidelines all the same. Same for "Pythonic" code (also you can get a speed bonus for doing these things, not that it is particularly useful here). Sometimes you have to duplicate code or you have to make things somewhat convoluted to not do the duplication. Minor differences are differences. It is also *really* annoying to deprecate default value changes.



---

archive/issue_comments_502295.json:
```json
{
    "body": "Changing priority from blocker to minor.",
    "created_at": "2018-08-27T22:48:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26141#issuecomment-502295",
    "user": "https://github.com/tscrim"
}
```

Changing priority from blocker to minor.



---

archive/issue_comments_502296.json:
```json
{
    "body": "<a id='comment:3'></a>Replying to [tscrim](#comment%3A2):\n> Dear Daniel, none of these issues are blockers to me. Please don't throw such strong statements around carelessly. Also, since a lot of these issues are simple, why are you asking others to do it? Why can't you take care of them yourself? If you have time to review, you have time to code.\n> \n> Also, code does not need to be perfect to get into Sage (something about the enemy of good comes to mind). I am happy to delegate work to subsequent tickets.\n\n\nIt is true that each of the issues above is not a blocker, but by finding that many issues in one simple patch one gets the impression that the code was written sloppy and that the review was done sloppy. And this is something we do not want (as [SageMath](SageMath) is sometimes advertised with high coding standards and a peer-reviewing process); this is the actual issue here.\n\n> Some technical notes. PEP8 are guidelines, good ones that should be followed as much as possible, but guidelines all the same.\n\n\nI wouldn't have brought PEP8 up alone (and I do not mind if there is a too long line from time to time), but the code itself is very inconsistently written, so this is way it came up.\n\n> Same for \"Pythonic\" code (also you can get a speed bonus for doing these things, not that it is particularly useful here).\n\n\nI do not insist on this either, but as I have re-reviewed the ticket, I brought it up.\n\n> Sometimes you have to duplicate code or you have to make things somewhat convoluted to not do the duplication. Minor differences are differences.\n\n\nIn this particular case it is very easy to avoid the doubling: Use the complete code from the else-block but instead of return save the content in a variable. Then\n\n```\nif not split:\n    return ' '.join(variable)\nelse:\n    return variable\n```\n\n> It is also *really* annoying to deprecate default value changes.\n\n\nYes, but the major issue here is that the original behavior is simply not possible anymore; this is annoying.",
    "created_at": "2018-08-28T06:45:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26141#issuecomment-502296",
    "user": "https://github.com/dkrenn"
}
```

<a id='comment:3'></a>Replying to [tscrim](#comment%3A2):
> Dear Daniel, none of these issues are blockers to me. Please don't throw such strong statements around carelessly. Also, since a lot of these issues are simple, why are you asking others to do it? Why can't you take care of them yourself? If you have time to review, you have time to code.
> 
> Also, code does not need to be perfect to get into Sage (something about the enemy of good comes to mind). I am happy to delegate work to subsequent tickets.


It is true that each of the issues above is not a blocker, but by finding that many issues in one simple patch one gets the impression that the code was written sloppy and that the review was done sloppy. And this is something we do not want (as [SageMath](SageMath) is sometimes advertised with high coding standards and a peer-reviewing process); this is the actual issue here.

> Some technical notes. PEP8 are guidelines, good ones that should be followed as much as possible, but guidelines all the same.


I wouldn't have brought PEP8 up alone (and I do not mind if there is a too long line from time to time), but the code itself is very inconsistently written, so this is way it came up.

> Same for "Pythonic" code (also you can get a speed bonus for doing these things, not that it is particularly useful here).


I do not insist on this either, but as I have re-reviewed the ticket, I brought it up.

> Sometimes you have to duplicate code or you have to make things somewhat convoluted to not do the duplication. Minor differences are differences.


In this particular case it is very easy to avoid the doubling: Use the complete code from the else-block but instead of return save the content in a variable. Then

```
if not split:
    return ' '.join(variable)
else:
    return variable
```

> It is also *really* annoying to deprecate default value changes.


Yes, but the major issue here is that the original behavior is simply not possible anymore; this is annoying.



---

archive/issue_comments_502297.json:
```json
{
    "body": "<a id='comment:4'></a>Hello,\n\nThank you very much for your 12 points-deep review of this recent ticket.\n\nI am on it.",
    "created_at": "2018-08-30T06:58:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26141#issuecomment-502297",
    "user": "https://github.com/jplab"
}
```

<a id='comment:4'></a>Hello,

Thank you very much for your 12 points-deep review of this recent ticket.

I am on it.



---

archive/issue_comments_502298.json:
```json
{
    "body": "<a id='comment:5'></a>Replying to [jipilab](#comment%3A4):\n> Thank you very much for your 12 points-deep review of this recent ticket.\n> \n> I am on it.\n\n\nGreat :) If you have any questions, don't hesitate to ask :)",
    "created_at": "2018-08-30T07:49:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26141#issuecomment-502298",
    "user": "https://github.com/dkrenn"
}
```

<a id='comment:5'></a>Replying to [jipilab](#comment%3A4):
> Thank you very much for your 12 points-deep review of this recent ticket.
> 
> I am on it.


Great :) If you have any questions, don't hesitate to ask :)



---

archive/issue_comments_502299.json:
```json
{
    "body": "Changing commit from \"\" to \"d7ec556f1ce336f2efcff32f77b7aa1ecb57a4cd\"",
    "created_at": "2018-08-30T15:56:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26141#issuecomment-502299",
    "user": "https://github.com/jplab"
}
```

Changing commit from "" to "d7ec556f1ce336f2efcff32f77b7aa1ecb57a4cd"



---

archive/issue_comments_502300.json:
```json
{
    "body": "<a id='comment:6'></a>> 1. There is no parameter setting which gives back the old default behavior, which is inequalities separated by commas. Using the separator, one only gets strings containing a lot of blanks, which simple cannot be read properly.\n> 2. The default values changed without a deprecation. The deprecation just aliases the old and new methods, but does not take care of the parameter setting. (I understand that some believe that this is a minor issue as the output is \"only\" a string represented to the user and not an (intermediate) result of a computation. However, one could have done this more smoothly)\n\n\nI agree that changing the default behavior is a sensible issue, and that the transition could have been more smooth, see https://xkcd.com/1172 . The code now gives:\n\n```\nsage: c = polytopes.cube()\nsage: c.repr_pretty_Hrepresentation(separator=', ',style='positive')\n/home/jplabbe/sage/src/bin/sage-ipython:1: DeprecationWarning: repr_pretty_Hrepresentation is deprecated. Please use Hrepresentation_str instead.\nSee https://trac.sagemath.org/24837 for details.\n  #!/usr/bin/env sage-python23\n'1 >= x2, 1 >= x1, 1 >= x0, x0 + 1 >= 0, x2 + 1 >= 0, x1 + 1 >= 0'\n```\n\nso it is possible to get back the previous behavior.\n\n> 3. PEP8 is not satisfied, in particular some too long lines and a lot of missing blanks after commas.\n\n\nThe ticket #24837 introduced 10 too long lines and 12 missing spaces after commas. I added the necessary spaces after the commas in all the file and I left the other four hundred fifty E501s alone.\n\n> 4. The code in some lines/blocks is considered bad in a Pythonic sense:\n>     {{{\n>     shift = any([pretty_hs[index][2][0] == '-' for index in range(len(pretty_hs))])\n>     }}}\n>    should be something like\n>     {{{\n>     shift = any(pretty_h[2][0] == '-' for pretty_h in pretty_hs)\n>     }}}\n>    or even better\n>     {{{\n>     shift = any(pretty_h[2].startswith('-') for pretty_h in pretty_hs)\n>     }}}\n>    Also get rid of the second `for index in range(len(...))`\n\n\nThanks for mentioning, forgot to look over that.\n\n> 5. The line\n>     {{{\n>     pretty_print = pretty_print[:-1] # Removing the last return\n>     }}}\n>    only works if the separator has length 1. Hint: The Pythonic way to do this kind of concatentation is to use `separator.join(...)`.\n\n\nDone.\n\n> 6. I cannot think of any reason, why the output has an additional blank at each end of line. Please remove this. (I personally would also get rid of all the blanks at the end of the line, i.e. the padding on the right. This maybe is a matter of taste, so should not bother us too much and probably people will find a lot of arguments why this is something wanted, so I just comment this here. ;) )\n\n\nThe reason was the shifting of coefficients appearing on the right column. This column appearing on the right of the inequality sign is left justified. When a minus sign appears the coefficient start 1 character more to the left. When there is no minus sign before the coefficient, it therefore appears shifted towards the left. In other to deal with this alignment issue, we insert one character more to the right column to place the \"-\". This had the side effect that when no minus sign appears (which is the case when \"style=positive\") then, there is a space left at the end of an equation.\n\nThis is now taken care of.\n\n> 7. Why\n>     {{{\n>     product = -1*(coeffs[1:]*vars[1:])\n>     }}}\n>    and not simply\n>     {{{\n>     product = -(...)\n>     }}}\n>    or even\n>     {{{\n>     product = -...\n>     }}}\n>     ?\n\n\nDone.\n\n> 8. There is quite a lot of code doubling at the end of the patch of #24837:\n>      {{{\n>      if not split:\n>          some code\n>      else:\n>          almost identical code with very few exceptions\n>      }}}\n>    This is very hard too read as one has to compare both almost identical blocks to find the differences. I am certain that a better solution can be found for this.\n  \nDone.\n\n> 9. Docstring of `style`-parameter: Shouldn't the commas be set differently: `either ..., or ... or ...` because the latter two should be grouped and not the former two?\n\n\nJedem Tierchen sein Pl\u00e4sierchen. I do not argue on this.\n\n> 10. Docstring\n>      {{{\n>      ``latex`` -- a boolean. Default is ``None``\n>      }}}\n>    So what does `None` mean here? Is it set automatically according to the input? If static, probably its `False` but who knows. Why using `None` at all and not `False` as the default then?\n\n\nDone.\n\n> 11. The default of the `style`-parameter is `'>='` in base.py and `'positive'` in representation.py. Having different default values within the same module for methods doing \"the same\" is usually not a good idea.\n\n\nForgot to change it. Thanks, they are now both `'>='`.\n\n> 12. The variable `split` needs a docstring explaining what it does as this is not obvious. (And, from a technical point of view, one could consider using its inverse `join`, as it is more a `' '.join(...)` than a `....split(' ')`, but this is highly debatable.)\n\n\nNo opinion here.\n\n---\nNew commits:",
    "created_at": "2018-08-30T15:56:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26141#issuecomment-502300",
    "user": "https://github.com/jplab"
}
```

<a id='comment:6'></a>> 1. There is no parameter setting which gives back the old default behavior, which is inequalities separated by commas. Using the separator, one only gets strings containing a lot of blanks, which simple cannot be read properly.
> 2. The default values changed without a deprecation. The deprecation just aliases the old and new methods, but does not take care of the parameter setting. (I understand that some believe that this is a minor issue as the output is "only" a string represented to the user and not an (intermediate) result of a computation. However, one could have done this more smoothly)


I agree that changing the default behavior is a sensible issue, and that the transition could have been more smooth, see https://xkcd.com/1172 . The code now gives:

```
sage: c = polytopes.cube()
sage: c.repr_pretty_Hrepresentation(separator=', ',style='positive')
/home/jplabbe/sage/src/bin/sage-ipython:1: DeprecationWarning: repr_pretty_Hrepresentation is deprecated. Please use Hrepresentation_str instead.
See https://trac.sagemath.org/24837 for details.
  #!/usr/bin/env sage-python23
'1 >= x2, 1 >= x1, 1 >= x0, x0 + 1 >= 0, x2 + 1 >= 0, x1 + 1 >= 0'
```

so it is possible to get back the previous behavior.

> 3. PEP8 is not satisfied, in particular some too long lines and a lot of missing blanks after commas.


The ticket #24837 introduced 10 too long lines and 12 missing spaces after commas. I added the necessary spaces after the commas in all the file and I left the other four hundred fifty E501s alone.

> 4. The code in some lines/blocks is considered bad in a Pythonic sense:
>     {{{
>     shift = any([pretty_hs[index][2][0] == '-' for index in range(len(pretty_hs))])
>     }}}
>    should be something like
>     {{{
>     shift = any(pretty_h[2][0] == '-' for pretty_h in pretty_hs)
>     }}}
>    or even better
>     {{{
>     shift = any(pretty_h[2].startswith('-') for pretty_h in pretty_hs)
>     }}}
>    Also get rid of the second `for index in range(len(...))`


Thanks for mentioning, forgot to look over that.

> 5. The line
>     {{{
>     pretty_print = pretty_print[:-1] # Removing the last return
>     }}}
>    only works if the separator has length 1. Hint: The Pythonic way to do this kind of concatentation is to use `separator.join(...)`.


Done.

> 6. I cannot think of any reason, why the output has an additional blank at each end of line. Please remove this. (I personally would also get rid of all the blanks at the end of the line, i.e. the padding on the right. This maybe is a matter of taste, so should not bother us too much and probably people will find a lot of arguments why this is something wanted, so I just comment this here. ;) )


The reason was the shifting of coefficients appearing on the right column. This column appearing on the right of the inequality sign is left justified. When a minus sign appears the coefficient start 1 character more to the left. When there is no minus sign before the coefficient, it therefore appears shifted towards the left. In other to deal with this alignment issue, we insert one character more to the right column to place the "-". This had the side effect that when no minus sign appears (which is the case when "style=positive") then, there is a space left at the end of an equation.

This is now taken care of.

> 7. Why
>     {{{
>     product = -1*(coeffs[1:]*vars[1:])
>     }}}
>    and not simply
>     {{{
>     product = -(...)
>     }}}
>    or even
>     {{{
>     product = -...
>     }}}
>     ?


Done.

> 8. There is quite a lot of code doubling at the end of the patch of #24837:
>      {{{
>      if not split:
>          some code
>      else:
>          almost identical code with very few exceptions
>      }}}
>    This is very hard too read as one has to compare both almost identical blocks to find the differences. I am certain that a better solution can be found for this.
  
Done.

> 9. Docstring of `style`-parameter: Shouldn't the commas be set differently: `either ..., or ... or ...` because the latter two should be grouped and not the former two?


Jedem Tierchen sein Pläsierchen. I do not argue on this.

> 10. Docstring
>      {{{
>      ``latex`` -- a boolean. Default is ``None``
>      }}}
>    So what does `None` mean here? Is it set automatically according to the input? If static, probably its `False` but who knows. Why using `None` at all and not `False` as the default then?


Done.

> 11. The default of the `style`-parameter is `'>='` in base.py and `'positive'` in representation.py. Having different default values within the same module for methods doing "the same" is usually not a good idea.


Forgot to change it. Thanks, they are now both `'>='`.

> 12. The variable `split` needs a docstring explaining what it does as this is not obvious. (And, from a technical point of view, one could consider using its inverse `join`, as it is more a `' '.join(...)` than a `....split(' ')`, but this is highly debatable.)


No opinion here.

---
New commits:



---

archive/issue_comments_502301.json:
```json
{
    "body": "Changing branch from \"\" to \"u/jipilab/26141\"",
    "created_at": "2018-08-30T15:56:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26141#issuecomment-502301",
    "user": "https://github.com/jplab"
}
```

Changing branch from "" to "u/jipilab/26141"



---

archive/issue_comments_502302.json:
```json
{
    "body": "Changing author from \"\" to \"Jean-Philippe Labb\u00e9\"",
    "created_at": "2018-08-30T15:56:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26141#issuecomment-502302",
    "user": "https://github.com/jplab"
}
```

Changing author from "" to "Jean-Philippe Labbé"



---

archive/issue_comments_502303.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2018-08-30T15:57:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26141#issuecomment-502303",
    "user": "https://github.com/jplab"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_502304.json:
```json
{
    "body": "Changing branch from \"u/jipilab/26141\" to \"u/dkrenn/26141\"",
    "created_at": "2018-08-31T06:46:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26141#issuecomment-502304",
    "user": "https://github.com/dkrenn"
}
```

Changing branch from "u/jipilab/26141" to "u/dkrenn/26141"



---

archive/issue_comments_502305.json:
```json
{
    "body": "Changing commit from \"d7ec556f1ce336f2efcff32f77b7aa1ecb57a4cd\" to \"e83534113d56ef5d80f3c02a517c8e3bee572e96\"",
    "created_at": "2018-08-31T06:49:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26141#issuecomment-502305",
    "user": "https://github.com/dkrenn"
}
```

Changing commit from "d7ec556f1ce336f2efcff32f77b7aa1ecb57a4cd" to "e83534113d56ef5d80f3c02a517c8e3bee572e96"



---

archive/issue_comments_502306.json:
```json
{
    "body": "<a id='comment:9'></a>Replying to [jipilab](#comment%3A6):\n> I agree that changing the default behavior is a sensible issue, and that the transition could have been more smooth, see https://xkcd.com/1172 .\n\n\nI know :)))\n\n> The code now gives: [...]\n> so it is possible to get back the previous behavior.\n\n\nGreat. Thanks.\n\n> > 3. PEP8 is not satisfied, in particular some too long lines and a lot of missing blanks after commas.\n  \n> \n> The ticket #24837 introduced 10 too long lines and 12 missing spaces after commas. I added the necessary spaces after the commas in all the file and I left the other four hundred fifty E501s alone.\n\n\nThank you for doing all these changes. I've added two more space after a comma.\n\n> > 4. The code in some lines/blocks is considered bad in a Pythonic sense:\n> > [...]\n> >   or even better\n> >     {{{\n> >     shift = any(pretty_h[2].startswith('-') for pretty_h in pretty_hs)\n> >     }}}\n> >   Also get rid of the second `for index in range(len(...))`\n\n> \n> Thanks for mentioning, forgot to look over that.\n\n\nLGTM.\n\n> > 5. The line\n> >     {{{\n> >     pretty_print = pretty_print[:-1] # Removing the last return\n> >     }}}\n> >    only works if the separator has length 1. Hint: The Pythonic way to do this kind of concatentation is to use `separator.join(...)`.\n  \n> \n> Done.\n\n\nLGTM. I did a minor rewrite to hopefully improve readablity.\n\n> > 6. I cannot think of any reason, why the output has an additional blank at each end of line. Please remove this. (I personally would also get rid of all the blanks at the end of the line, i.e. the padding on the right. This maybe is a matter of taste, so should not bother us too much and probably people will find a lot of arguments why this is something wanted, so I just comment this here. ;) )\n  \n> \n> The reason was the shifting of coefficients appearing on the right column. This column appearing on the right of the inequality sign is left justified. When a minus sign appears the coefficient start 1 character more to the left. When there is no minus sign before the coefficient, it therefore appears shifted towards the left. In other to deal with this alignment issue, we insert one character more to the right column to place the \"-\". This had the side effect that when no minus sign appears (which is the case when \"style=positive\") then, there is a space left at the end of an equation.\n> \n> This is now taken care of.\n\n\nThank you for the clearification and for taking care of.\n \n> > 7. Why\n> >     {{{\n> >     product = -1*(coeffs[1:]*vars[1:])\n> >     }}}\n> >    and not simply\n> >     {{{\n> >     product = -(...)\n> >     }}}\n> >    or even\n> >     {{{\n> >     product = -...\n> >     }}}\n> >     ?\n  \n> \n> Done.\n\n\nLGTM.\n\n> > 8. There is quite a lot of code doubling at the end of the patch of #24837:\n> >      {{{\n> >      if not split:\n> >          some code\n> >      else:\n> >          almost identical code with very few exceptions\n> >      }}}\n> >    This is very hard too read as one has to compare both almost identical blocks to find the differences. I am certain that a better solution can be found for this.\n  \n> Done.\n\nMuch better now, thank you.\n\n> > 9. Docstring of `style`-parameter: Shouldn't the commas be set differently: `either ..., or ... or ...` because the latter two should be grouped and not the former two?\n  \n> \n> Jedem Tierchen sein Pl\u00e4sierchen. I do not argue on this.\n\n\nI've changed this now. Please veto, if you it doesn't work for you.\n\n> > 10. Docstring\n> >      {{{\n> >      ``latex`` -- a boolean. Default is ``None``\n> >      }}}\n> >    So what does `None` mean here? Is it set automatically according to the input? If static, probably its `False` but who knows. Why using `None` at all and not `False` as the default then?\n  \n> \n> Done.\n\n\nThanks.\n\n> > 11. The default of the `style`-parameter is `'>='` in base.py and `'positive'` in representation.py. Having different default values within the same module for methods doing \"the same\" is usually not a good idea.\n  \n> \n> Forgot to change it. Thanks, they are now both `'>='`.\n\n\nOk. (FWIW, I would be happy with any of the two being the default.)\n\n> > 12. The variable `split` needs a docstring explaining what it does as this is not obvious. (And, from a technical point of view, one could consider using its inverse `join`, as it is more a `' '.join(...)` than a `....split(' ')`, but this is highly debatable.)\n\n\nI've shortend the OUTPUT-block to avoid the double-description of the style-parameter.\n\n---\nNew commits:",
    "created_at": "2018-08-31T06:49:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26141#issuecomment-502306",
    "user": "https://github.com/dkrenn"
}
```

<a id='comment:9'></a>Replying to [jipilab](#comment%3A6):
> I agree that changing the default behavior is a sensible issue, and that the transition could have been more smooth, see https://xkcd.com/1172 .


I know :)))

> The code now gives: [...]
> so it is possible to get back the previous behavior.


Great. Thanks.

> > 3. PEP8 is not satisfied, in particular some too long lines and a lot of missing blanks after commas.
  
> 
> The ticket #24837 introduced 10 too long lines and 12 missing spaces after commas. I added the necessary spaces after the commas in all the file and I left the other four hundred fifty E501s alone.


Thank you for doing all these changes. I've added two more space after a comma.

> > 4. The code in some lines/blocks is considered bad in a Pythonic sense:
> > [...]
> >   or even better
> >     {{{
> >     shift = any(pretty_h[2].startswith('-') for pretty_h in pretty_hs)
> >     }}}
> >   Also get rid of the second `for index in range(len(...))`

> 
> Thanks for mentioning, forgot to look over that.


LGTM.

> > 5. The line
> >     {{{
> >     pretty_print = pretty_print[:-1] # Removing the last return
> >     }}}
> >    only works if the separator has length 1. Hint: The Pythonic way to do this kind of concatentation is to use `separator.join(...)`.
  
> 
> Done.


LGTM. I did a minor rewrite to hopefully improve readablity.

> > 6. I cannot think of any reason, why the output has an additional blank at each end of line. Please remove this. (I personally would also get rid of all the blanks at the end of the line, i.e. the padding on the right. This maybe is a matter of taste, so should not bother us too much and probably people will find a lot of arguments why this is something wanted, so I just comment this here. ;) )
  
> 
> The reason was the shifting of coefficients appearing on the right column. This column appearing on the right of the inequality sign is left justified. When a minus sign appears the coefficient start 1 character more to the left. When there is no minus sign before the coefficient, it therefore appears shifted towards the left. In other to deal with this alignment issue, we insert one character more to the right column to place the "-". This had the side effect that when no minus sign appears (which is the case when "style=positive") then, there is a space left at the end of an equation.
> 
> This is now taken care of.


Thank you for the clearification and for taking care of.
 
> > 7. Why
> >     {{{
> >     product = -1*(coeffs[1:]*vars[1:])
> >     }}}
> >    and not simply
> >     {{{
> >     product = -(...)
> >     }}}
> >    or even
> >     {{{
> >     product = -...
> >     }}}
> >     ?
  
> 
> Done.


LGTM.

> > 8. There is quite a lot of code doubling at the end of the patch of #24837:
> >      {{{
> >      if not split:
> >          some code
> >      else:
> >          almost identical code with very few exceptions
> >      }}}
> >    This is very hard too read as one has to compare both almost identical blocks to find the differences. I am certain that a better solution can be found for this.
  
> Done.

Much better now, thank you.

> > 9. Docstring of `style`-parameter: Shouldn't the commas be set differently: `either ..., or ... or ...` because the latter two should be grouped and not the former two?
  
> 
> Jedem Tierchen sein Pläsierchen. I do not argue on this.


I've changed this now. Please veto, if you it doesn't work for you.

> > 10. Docstring
> >      {{{
> >      ``latex`` -- a boolean. Default is ``None``
> >      }}}
> >    So what does `None` mean here? Is it set automatically according to the input? If static, probably its `False` but who knows. Why using `None` at all and not `False` as the default then?
  
> 
> Done.


Thanks.

> > 11. The default of the `style`-parameter is `'>='` in base.py and `'positive'` in representation.py. Having different default values within the same module for methods doing "the same" is usually not a good idea.
  
> 
> Forgot to change it. Thanks, they are now both `'>='`.


Ok. (FWIW, I would be happy with any of the two being the default.)

> > 12. The variable `split` needs a docstring explaining what it does as this is not obvious. (And, from a technical point of view, one could consider using its inverse `join`, as it is more a `' '.join(...)` than a `....split(' ')`, but this is highly debatable.)


I've shortend the OUTPUT-block to avoid the double-description of the style-parameter.

---
New commits:



---

archive/issue_comments_502307.json:
```json
{
    "body": "<a id='comment:10'></a>Replying to [dkrenn](#comment%3A9):\n> |                                                                                                                                          |                                               |\n> |------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------|\n> |[9ecefa0](https://git.sagemath.org/sage.git/commit?id=9ecefa04a7b33e52c7205d166e5cb0e4ba7335e0)|`Trac #26141: code simplification split/length`|\n\n\nI did a small rewrite to simplify the code.\n\n> |                                                                                                                                          |                                  |\n> |------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------|\n> |[91fa86e](https://git.sagemath.org/sage.git/commit?id=91fa86e7556158f55d7bcf6c3e45d439fa8c16e7)|`Trac #26141: new parameter align`|\n\n\nI've introduced a new parameter to set alignment. (I hereby think of other separators than newline.)\n\n> |                                                                                                                                          |                                                      |\n> |------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------|\n> |[e447b5e](https://git.sagemath.org/sage.git/commit?id=e447b5e35bd729710a5dfd2ba97a38505db82078)|`Trac #26141: trying to improve readability even more`|\n\n\nI've factored out the padding in a separate function to obtain easier code-reading, I hope.\n\nPlease cross-review these changes.",
    "created_at": "2018-08-31T06:52:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26141#issuecomment-502307",
    "user": "https://github.com/dkrenn"
}
```

<a id='comment:10'></a>Replying to [dkrenn](#comment%3A9):
> |                                                                                                                                          |                                               |
> |------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------|
> |[9ecefa0](https://git.sagemath.org/sage.git/commit?id=9ecefa04a7b33e52c7205d166e5cb0e4ba7335e0)|`Trac #26141: code simplification split/length`|


I did a small rewrite to simplify the code.

> |                                                                                                                                          |                                  |
> |------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------|
> |[91fa86e](https://git.sagemath.org/sage.git/commit?id=91fa86e7556158f55d7bcf6c3e45d439fa8c16e7)|`Trac #26141: new parameter align`|


I've introduced a new parameter to set alignment. (I hereby think of other separators than newline.)

> |                                                                                                                                          |                                                      |
> |------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------|
> |[e447b5e](https://git.sagemath.org/sage.git/commit?id=e447b5e35bd729710a5dfd2ba97a38505db82078)|`Trac #26141: trying to improve readability even more`|


I've factored out the padding in a separate function to obtain easier code-reading, I hope.

Please cross-review these changes.



---

archive/issue_comments_502308.json:
```json
{
    "body": "Changing commit from \"e83534113d56ef5d80f3c02a517c8e3bee572e96\" to \"31765a5b15e3f7611c55d184857376a9b2350491\"",
    "created_at": "2018-08-31T06:56:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26141#issuecomment-502308",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "e83534113d56ef5d80f3c02a517c8e3bee572e96" to "31765a5b15e3f7611c55d184857376a9b2350491"



---

archive/issue_comments_502309.json:
```json
{
    "body": "<a id='comment:11'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2018-08-31T06:56:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26141#issuecomment-502309",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:11'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_502310.json:
```json
{
    "body": "<a id='comment:12'></a>So, I am done; let's also see what the patchbot says ;)",
    "created_at": "2018-08-31T06:57:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26141#issuecomment-502310",
    "user": "https://github.com/dkrenn"
}
```

<a id='comment:12'></a>So, I am done; let's also see what the patchbot says ;)



---

archive/issue_comments_502311.json:
```json
{
    "body": "Changing reviewer from \"Daniel Krenn\" to \"Daniel Krenn, Jean-Philippe Labb\u00e9\"",
    "created_at": "2018-09-01T08:13:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26141#issuecomment-502311",
    "user": "https://github.com/jplab"
}
```

Changing reviewer from "Daniel Krenn" to "Daniel Krenn, Jean-Philippe Labbé"



---

archive/issue_comments_502312.json:
```json
{
    "body": "<a id='comment:13'></a>The bot seems happy, and I am too.\n\nThanks for further improvements. I set it to positive review as the issues seem to be fixed.\n\nBest,\nJP",
    "created_at": "2018-09-01T08:13:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26141#issuecomment-502312",
    "user": "https://github.com/jplab"
}
```

<a id='comment:13'></a>The bot seems happy, and I am too.

Thanks for further improvements. I set it to positive review as the issues seem to be fixed.

Best,
JP



---

archive/issue_comments_502313.json:
```json
{
    "body": "Changing author from \"Jean-Philippe Labb\u00e9\" to \"Jean-Philippe Labb\u00e9, Daniel Krenn\"",
    "created_at": "2018-09-01T08:13:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26141#issuecomment-502313",
    "user": "https://github.com/jplab"
}
```

Changing author from "Jean-Philippe Labbé" to "Jean-Philippe Labbé, Daniel Krenn"



---

archive/issue_comments_502314.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2018-09-01T08:13:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26141#issuecomment-502314",
    "user": "https://github.com/jplab"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_502315.json:
```json
{
    "body": "<a id='comment:14'></a>Thank you very much for your work here on this ticket and on the original #24837; I appreciate it very much and like the improvements.",
    "created_at": "2018-09-01T09:00:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26141#issuecomment-502315",
    "user": "https://github.com/dkrenn"
}
```

<a id='comment:14'></a>Thank you very much for your work here on this ticket and on the original #24837; I appreciate it very much and like the improvements.



---

archive/issue_events_065180.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-09-03T23:29:35Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/26141",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/26141#event-65180"
}
```



---

archive/issue_comments_502316.json:
```json
{
    "body": "Changing branch from \"u/dkrenn/26141\" to \"31765a5b15e3f7611c55d184857376a9b2350491\"",
    "created_at": "2018-09-03T23:29:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26141#issuecomment-502316",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/dkrenn/26141" to "31765a5b15e3f7611c55d184857376a9b2350491"



---

archive/issue_comments_502317.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2018-09-03T23:29:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26141#issuecomment-502317",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
