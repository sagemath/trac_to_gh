# Issue 26266: py3: @interact is broken

archive/issues_026029.json:
```json
{
    "body": "Jupyter output with version 8.4.beta4 on Python3 :\n\n```\n@interact(x=True, y=1.0)\ndef g(x, y):\n    return (x, y)\n----------------------------------------------------------------------------\nTypeError                                 Traceback (most recent call last)\n<ipython-input-6-72d2e695a5da> in <module>()\n----> 1 @interact(x=True, y=RealNumber('1.0'))\n      2 def g(x, y):\n      3     return (x, y)\n\n.../local/lib/python3.6/site-packages/ipywidgets/widgets/interaction.py in __call__(self, _InteractFactory__interact_f, **kwargs)\n    521         #    def f(*args, **kwargs):\n    522         #        ...\n--> 523         w = self.widget(f)\n    524         try:\n    525             f.widget = w\n\n.../local/lib/python3.6/site-packages/ipywidgets/widgets/interaction.py in widget(self, f)\n    437             The function to which the interactive widgets are tied.\n    438         \"\"\"\n--> 439         return self.cls(f, self.opts, **self.kwargs)\n    440 \n    441     def __call__(self, __interact_f=None, **kwargs):\n\n.../local/lib/python3.6/site-packages/sage/repl/ipython_kernel/interact.py in __init__(*args, **kwds)\n     89         # Check for auto_update in signature\n     90         sig = signature(f)\n---> 91         params = copy(sig.parameters)\n     92         try:\n     93             p_auto_update = params.pop(\"auto_update\")\n\n.../local/lib/python3.6/copy.py in copy(x)\n     94         reductor = getattr(x, \"__reduce_ex__\", None)\n     95         if reductor:\n---> 96             rv = reductor(4)\n     97         else:\n     98             reductor = getattr(x, \"__reduce__\", None)\n\nTypeError: can't pickle mappingproxy objects\n```\n\n\nAssignee: @embray\n\nCC:  @embray\n\nKeywords: jupyter interact\n\nBranch/Commit: 3fd00ba517364f765e13e281fbb1b35b5aa5ec29\n\nReviewer: Odile B\u00e9nassy\n\nAuthor: Erik Bray, Fr\u00e9d\u00e9ric Chapoton\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/26266\n\n",
    "closed_at": "2018-10-20T11:58:52Z",
    "created_at": "2018-09-13T08:01:42Z",
    "labels": [
        "component: python3",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.5",
    "title": "py3: @interact is broken",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/26266",
    "user": "https://trac.sagemath.org/admin/accounts/users/zerline"
}
```
Jupyter output with version 8.4.beta4 on Python3 :

```
@interact(x=True, y=1.0)
def g(x, y):
    return (x, y)
----------------------------------------------------------------------------
TypeError                                 Traceback (most recent call last)
<ipython-input-6-72d2e695a5da> in <module>()
----> 1 @interact(x=True, y=RealNumber('1.0'))
      2 def g(x, y):
      3     return (x, y)

.../local/lib/python3.6/site-packages/ipywidgets/widgets/interaction.py in __call__(self, _InteractFactory__interact_f, **kwargs)
    521         #    def f(*args, **kwargs):
    522         #        ...
--> 523         w = self.widget(f)
    524         try:
    525             f.widget = w

.../local/lib/python3.6/site-packages/ipywidgets/widgets/interaction.py in widget(self, f)
    437             The function to which the interactive widgets are tied.
    438         """
--> 439         return self.cls(f, self.opts, **self.kwargs)
    440 
    441     def __call__(self, __interact_f=None, **kwargs):

.../local/lib/python3.6/site-packages/sage/repl/ipython_kernel/interact.py in __init__(*args, **kwds)
     89         # Check for auto_update in signature
     90         sig = signature(f)
---> 91         params = copy(sig.parameters)
     92         try:
     93             p_auto_update = params.pop("auto_update")

.../local/lib/python3.6/copy.py in copy(x)
     94         reductor = getattr(x, "__reduce_ex__", None)
     95         if reductor:
---> 96             rv = reductor(4)
     97         else:
     98             reductor = getattr(x, "__reduce__", None)

TypeError: can't pickle mappingproxy objects
```


Assignee: @embray

CC:  @embray

Keywords: jupyter interact

Branch/Commit: 3fd00ba517364f765e13e281fbb1b35b5aa5ec29

Reviewer: Odile Bénassy

Author: Erik Bray, Frédéric Chapoton

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/26266





---

archive/issue_comments_505132.json:
```json
{
    "body": "<a id='comment:3'></a>Turns out I already have this fixed in my Python 3 branch; I just never pushed the fixes.",
    "created_at": "2018-09-13T10:03:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26266",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26266#issuecomment-505132",
    "user": "https://github.com/embray"
}
```

<a id='comment:3'></a>Turns out I already have this fixed in my Python 3 branch; I just never pushed the fixes.



---

archive/issue_comments_505133.json:
```json
{
    "body": "Set assignee to @embray.",
    "created_at": "2018-09-13T10:05:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26266",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26266#issuecomment-505133",
    "user": "https://github.com/embray"
}
```

Set assignee to @embray.



---

archive/issue_comments_505134.json:
```json
{
    "body": "<a id='comment:4'></a>FWIW this is the patch I used\n\n```diff\ndiff --git a/src/sage/repl/ipython_kernel/interact.py b/src/sage/repl/ipython_kernel/interact.py\nindex 9372426..d49af6f 100644\n--- a/src/sage/repl/ipython_kernel/interact.py\n+++ b/src/sage/repl/ipython_kernel/interact.py\n@@ -35,8 +35,7 @@ EXAMPLES::\n\n from ipywidgets.widgets import SelectionSlider, ValueWidget, ToggleButtons\n from ipywidgets.widgets.interaction import interactive, signature\n-from copy import copy\n-from collections import Iterable, Iterator\n+from collections import Iterable, Iterator, OrderedDict\n from .widgets import EvalText, SageColorPicker\n from sage.structure.element import parent\n from sage.symbolic.ring import SR\n@@ -88,7 +87,7 @@ class sage_interactive(interactive):\n\n         # Check for auto_update in signature\n         sig = signature(f)\n-        params = copy(sig.parameters)\n+        params = OrderedDict(sig.parameters)\n         try:\n             p_auto_update = params.pop(\"auto_update\")\n         except KeyError:\n\n```\n\nI will make a branch in a bit unless you want to do it first.",
    "created_at": "2018-09-13T10:05:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26266",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26266#issuecomment-505134",
    "user": "https://github.com/embray"
}
```

<a id='comment:4'></a>FWIW this is the patch I used

```diff
diff --git a/src/sage/repl/ipython_kernel/interact.py b/src/sage/repl/ipython_kernel/interact.py
index 9372426..d49af6f 100644
--- a/src/sage/repl/ipython_kernel/interact.py
+++ b/src/sage/repl/ipython_kernel/interact.py
@@ -35,8 +35,7 @@ EXAMPLES::

 from ipywidgets.widgets import SelectionSlider, ValueWidget, ToggleButtons
 from ipywidgets.widgets.interaction import interactive, signature
-from copy import copy
-from collections import Iterable, Iterator
+from collections import Iterable, Iterator, OrderedDict
 from .widgets import EvalText, SageColorPicker
 from sage.structure.element import parent
 from sage.symbolic.ring import SR
@@ -88,7 +87,7 @@ class sage_interactive(interactive):

         # Check for auto_update in signature
         sig = signature(f)
-        params = copy(sig.parameters)
+        params = OrderedDict(sig.parameters)
         try:
             p_auto_update = params.pop("auto_update")
         except KeyError:

```

I will make a branch in a bit unless you want to do it first.



---

archive/issue_comments_505135.json:
```json
{
    "body": "<a id='comment:5'></a>Thanks! Ok then. No hurry!",
    "created_at": "2018-09-13T10:24:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26266",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26266#issuecomment-505135",
    "user": "https://trac.sagemath.org/admin/accounts/users/zerline"
}
```

<a id='comment:5'></a>Thanks! Ok then. No hurry!



---

archive/issue_comments_505136.json:
```json
{
    "body": "Changing commit from \"\" to \"ca26fcca0faebd89ac56f907f911608e81a4c294\"",
    "created_at": "2018-09-19T08:00:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26266",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26266#issuecomment-505136",
    "user": "https://github.com/fchapoton"
}
```

Changing commit from "" to "ca26fcca0faebd89ac56f907f911608e81a4c294"



---

archive/issue_comments_505137.json:
```json
{
    "body": "Changing branch from \"\" to \"u/chapoton/26266\"",
    "created_at": "2018-09-19T08:00:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26266",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26266#issuecomment-505137",
    "user": "https://github.com/fchapoton"
}
```

Changing branch from "" to "u/chapoton/26266"



---

archive/issue_comments_505138.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2018-09-19T08:00:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26266",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26266#issuecomment-505138",
    "user": "https://github.com/fchapoton"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_505139.json:
```json
{
    "body": "Changing author from \"\" to \"Fr\u00e9d\u00e9ric Chapoton\"",
    "created_at": "2018-09-19T08:00:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26266",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26266#issuecomment-505139",
    "user": "https://github.com/fchapoton"
}
```

Changing author from "" to "Frédéric Chapoton"



---

archive/issue_comments_505140.json:
```json
{
    "body": "<a id='comment:7'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2018-09-19T08:01:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26266",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26266#issuecomment-505140",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:7'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_505141.json:
```json
{
    "body": "Changing commit from \"ca26fcca0faebd89ac56f907f911608e81a4c294\" to \"3fd00ba517364f765e13e281fbb1b35b5aa5ec29\"",
    "created_at": "2018-09-19T08:01:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26266",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26266#issuecomment-505141",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "ca26fcca0faebd89ac56f907f911608e81a4c294" to "3fd00ba517364f765e13e281fbb1b35b5aa5ec29"



---

archive/issue_comments_505142.json:
```json
{
    "body": "Changing author from \"Fr\u00e9d\u00e9ric Chapoton\" to \"Erik Bray, Fr\u00e9d\u00e9ric Chapoton\"",
    "created_at": "2018-09-19T19:49:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26266",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26266#issuecomment-505142",
    "user": "https://github.com/fchapoton"
}
```

Changing author from "Frédéric Chapoton" to "Erik Bray, Frédéric Chapoton"



---

archive/issue_comments_505143.json:
```json
{
    "body": "<a id='comment:8'></a>bot is morally green, please review",
    "created_at": "2018-09-19T19:49:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26266",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26266#issuecomment-505143",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:8'></a>bot is morally green, please review



---

archive/issue_comments_505144.json:
```json
{
    "body": "<a id='comment:9'></a>It would be best if zerline could confirm the fix.",
    "created_at": "2018-09-20T11:42:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26266",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26266#issuecomment-505144",
    "user": "https://github.com/embray"
}
```

<a id='comment:9'></a>It would be best if zerline could confirm the fix.



---

archive/issue_comments_505145.json:
```json
{
    "body": "<a id='comment:10'></a>`@`zerline, please have a look",
    "created_at": "2018-10-02T09:40:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26266",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26266#issuecomment-505145",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:10'></a>`@`zerline, please have a look



---

archive/issue_comments_505146.json:
```json
{
    "body": "<a id='comment:11'></a>Replying to [chapoton](#comment%3A10):\n> `@`zerline, please have a look\n\n\nyou're right `@`chapoton, I'll check this asap",
    "created_at": "2018-10-04T14:30:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26266",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26266#issuecomment-505146",
    "user": "https://trac.sagemath.org/admin/accounts/users/zerline"
}
```

<a id='comment:11'></a>Replying to [chapoton](#comment%3A10):
> `@`zerline, please have a look


you're right `@`chapoton, I'll check this asap



---

archive/issue_comments_505147.json:
```json
{
    "body": "<a id='comment:12'></a>Yes, the fix works :)",
    "created_at": "2018-10-05T14:33:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26266",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26266#issuecomment-505147",
    "user": "https://trac.sagemath.org/admin/accounts/users/zerline"
}
```

<a id='comment:12'></a>Yes, the fix works :)



---

archive/issue_comments_505148.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2018-10-07T13:07:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26266",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26266#issuecomment-505148",
    "user": "https://trac.sagemath.org/admin/accounts/users/zerline"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_505149.json:
```json
{
    "body": "<a id='comment:14'></a>please add reviewer fullname",
    "created_at": "2018-10-11T07:23:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26266",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26266#issuecomment-505149",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:14'></a>please add reviewer fullname



---

archive/issue_comments_505150.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Odile B\u00e9nassy\"",
    "created_at": "2018-10-11T10:03:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26266",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26266#issuecomment-505150",
    "user": "https://github.com/fchapoton"
}
```

Changing reviewer from "" to "Odile Bénassy"



---

archive/issue_comments_505151.json:
```json
{
    "body": "Changing branch from \"u/chapoton/26266\" to \"3fd00ba517364f765e13e281fbb1b35b5aa5ec29\"",
    "created_at": "2018-10-20T11:58:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26266",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26266#issuecomment-505151",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/chapoton/26266" to "3fd00ba517364f765e13e281fbb1b35b5aa5ec29"



---

archive/issue_events_065386.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-10-20T11:58:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/26266",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/26266#event-65386"
}
```



---

archive/issue_comments_505152.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2018-10-20T11:58:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26266",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26266#issuecomment-505152",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_comments_505153.json:
```json
{
    "body": "<a id='comment:17'></a>This should be re-targeted for 8.5.",
    "created_at": "2018-10-28T14:52:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26266",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26266#issuecomment-505153",
    "user": "https://github.com/embray"
}
```

<a id='comment:17'></a>This should be re-targeted for 8.5.



---

archive/issue_events_065387.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-10-28T14:52:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/26266",
    "milestone": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/26266#event-65387"
}
```
