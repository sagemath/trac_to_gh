...
```json
{
    "body": "<a id='comment:2'></a>the branch is almost empty (no meaningful changes)\n\n---\nNew commits:",
    "created_at": "2018-12-13T15:40:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26887",
    "type": "issue_comment",
...
<a id='comment:2'></a>the branch is almost empty (no meaningful changes)
...
```json
{
    "body": "<a id='comment:19'></a>I'm not experienced at what point a ticket should change its status to \"needs review\". Of course there is still stuff that I can do by myself like documentation and comments.\n\nI would definitely be grateful for comments and help at this point. At this point everything should work at least I'm not aware of any bugs.",
...
<a id='comment:19'></a>I'm not experienced at what point a ticket should change its status to "needs review". Of course there is still stuff that I can do by myself like documentation and comments.
...
<a id='comment:20'></a>As a start you can write examples (see [the developer guide](http://doc.sagemath.org/html/en/developer/coding_basics.html#the-docstring-of-a-function-content)).
...
<a id='comment:22'></a>I added examples.
...
<a id='comment:28'></a>These doctests are not reproducible.
...
```json
{
    "body": "<a id='comment:29'></a>The function `CombinatorialPolyhedron::get_flag_number(PyObject* py_tuple)` and many others are very likely to crash Sage (e.g. you do not take care of any error handling). I strongly advice you to avoid any Python in C/C++. The conversion tuple -> C++ vector is very easily done in Cython which automatically handles conversions and errors.",
...
<a id='comment:29'></a>The function `CombinatorialPolyhedron::get_flag_number(PyObject* py_tuple)` and many others are very likely to crash Sage (e.g. you do not take care of any error handling). I strongly advice you to avoid any Python in C/C++. The conversion tuple -> C++ vector is very easily done in Cython which automatically handles conversions and errors.
...
<a id='comment:32'></a>This doesn't build for me on OS X:
...
<a id='comment:33'></a>Thanks for going through the effort of building that on your machine.
...
```json
{
    "body": "<a id='comment:35'></a>I should have checked first..., this doesn't work",
...
<a id='comment:35'></a>I should have checked first..., this doesn't work
...
```json
{
    "body": "<a id='comment:38'></a>++    EXAMPLE::\n\nshould be\n\n++    EXAMPLES::",
...
<a id='comment:47'></a>Still doesn't build on OS X:
...
<a id='comment:50'></a>Should be fixed.
...
<a id='comment:55'></a>Some purely technical points about the programming:
...
<a id='comment:56'></a>Replying to [gh-kliem](#comment%3A22):
> In Cython I was able to calculate this vector (or more important the edge graph) in something like 3 hours, in C it took me 20 minutes.
...
```json
{
    "body": "<a id='comment:57'></a>Another minor comment: `uint64_t_or_uint32_t` is a very clunky type name. Couldn't you use a more descriptive name? Or use a standard type like `unsigned long` or `size_t`?",
...
<a id='comment:57'></a>Another minor comment: `uint64_t_or_uint32_t` is a very clunky type name. Couldn't you use a more descriptive name? Or use a standard type like `unsigned long` or `size_t`?
...
<a id='comment:63'></a>Replying to [gh-kliem](#comment%3A62):
> Replying to [jdemeyer](#comment%3A57):
> > Another minor comment: `uint64_t_or_uint32_t` is a very clunky type name. Couldn't you use a more descriptive name? Or use a standard type like `unsigned long` or `size_t`?
...
<a id='comment:64'></a>Thanks for the comments and help.
...
```json
{
    "body": "<a id='comment:66'></a>You probably know this, but just in case: on OS X, `clang` is used, not `gcc`.",
...
<a id='comment:66'></a>You probably know this, but just in case: on OS X, `clang` is used, not `gcc`.
...
<a id='comment:67'></a>Replying to [gh-kliem](#comment%3A62):
> Also, on 64bit machines, I really need to know that this is uint64_t.
...
```json
{
    "body": "<a id='comment:68'></a>A more or less equivalent question: why do you *really* need to know that the type is `uint64_t` on a 64-bit machine and `uint32_t` on a 32-bit machine?",
...
<a id='comment:68'></a>A more or less equivalent question: why do you *really* need to know that the type is `uint64_t` on a 64-bit machine and `uint32_t` on a 32-bit machine?
...
<a id='comment:69'></a>Replying to [gh-kliem](#comment%3A64):
> A solution would be to use function multiversioning as available in GCC >= 6 (https://hannes.hauswedell.net/post/2017/12/09/fmv/).
...
<a id='comment:72'></a>Replying to [gh-kliem](#comment%3A70):
> Replying to [jdemeyer](#comment%3A68):
> > A more or less equivalent question: why do you *really* to know that the type is `uint64_t` on a 64-bit machine and `uint32_t` on a 32-bit machine?
...
<a id='comment:74'></a>Replying to [jdemeyer](#comment%3A69):
> Replying to [gh-kliem](#comment%3A64):
> > A solution would be to use function multiversioning as available in GCC >= 6 (https://hannes.hauswedell.net/post/2017/12/09/fmv/).
...
<a id='comment:77'></a>I suppose it makes sense to disable all the intrinsics for this ticket (not done yet) and enable them later (#27103). This way this ticket does not depend on #27122.
...
<a id='comment:81'></a>It seems like everything boils down to a face iterator.
...
<a id='comment:93'></a>This ticket is now pretty much where I want it to be. I still need to go through everything and make comments and clean up.
...
<a id='comment:94'></a>It might be worth adding some timings against the current state-of-the-art implementation in `polymake` to show **how fast** this implementation really is. One striking example is
...
<a id='comment:95'></a>A very general question: How much of this are you actually using stuff that is within Sage (and Cython)? In particular, I am wondering if there is a benefit to separating this out into a little standalone package that gets included in Sage with some linking. To me, you could leave the features that use parts of Sage, e.g., `ridge_graph`, in the Sage implementation part, but have a good part of it independent.
...
```json
{
    "body": "<a id='comment:98'></a>I agree with Travis that error checking could be much improved: use [error return values](https://cython.readthedocs.io/en/latest/src/userguide/language_basics.html#error-return-values).\n\nAlso, instead of using `sig_malloc` and then checking for `NULL`, use `check_malloc` which automatically does that check. Even better, replace `sig_malloc(a * sizeof(T))` by `check_allocarray(a, sizeof(T))`.",
...
<a id='comment:98'></a>I agree with Travis that error checking could be much improved: use [error return values](https://cython.readthedocs.io/en/latest/src/userguide/language_basics.html#error-return-values).
...
```json
{
    "body": "<a id='comment:99'></a>Replying to [tscrim](#comment%3A95):\n> Name conflict:\n> \n> ```\n> from cpython cimport array\n> import array\n> ```\n\n\nAs far as I can tell, this isn't even used anywhere.",
...
<a id='comment:99'></a>Replying to [tscrim](#comment%3A95):
> Name conflict:
> 
> ```
> from cpython cimport array
...
<a id='comment:101'></a>Replying to [tscrim](#comment%3A95):
> For `_data` and `_memory`, could you be more specific than a `void` pointer?
...
```json
{
    "body": "<a id='comment:2'></a>Also, this `ListOfListOfPointers` and `ListOfFaces` stuff seems to be reinventing the wheel. For example, since you're using C++ anyway, you could use `std::vector<>` or whatever. There is also `sage.ext.memory_allocator.MemoryAllocator` which seems to have some overlap with what you need.",
...
<a id='comment:2'></a>Also, this `ListOfListOfPointers` and `ListOfFaces` stuff seems to be reinventing the wheel. For example, since you're using C++ anyway, you could use `std::vector<>` or whatever. There is also `sage.ext.memory_allocator.MemoryAllocator` which seems to have some overlap with what you need.
...
<a id='comment:3'></a>I consider these small accessor methods unpythonic:
...
<a id='comment:107'></a>Replying to [jdemeyer](#comment%3A102):
> Also, this `ListOfListOfPointers` and `ListOfFaces` stuff seems to be reinventing the wheel. For example, since you're using C++ anyway, you could use `std::vector<>` or whatever. There is also `sage.ext.memory_allocator.MemoryAllocator` which seems to have some overlap with what you need.
...
<a id='comment:109'></a>Replying to [gh-kliem](#comment%3A107):
> I had the impression that its somehow nice to allocate the memory in cython instead of some extern C file.
...
<a id='comment:110'></a>Replying to [gh-kliem](#comment%3A107):
> `sage.ext.memory_allocator.MemoryAllocator` seems to be great for `ListOfPointers`. I couldn't get 32-byte aligned to work with any prebuild solution.
...
```json
{
    "body": "<a id='comment:1'></a>Speaking of alignment: why are you not using https://en.cppreference.com/w/c/memory/aligned_alloc or http://man7.org/linux/man-pages/man3/posix_memalign.3.html",
...
<a id='comment:1'></a>Speaking of alignment: why are you not using https://en.cppreference.com/w/c/memory/aligned_alloc or http://man7.org/linux/man-pages/man3/posix_memalign.3.html
...
```json
{
    "body": "<a id='comment:2'></a>Since you're using divisions, I strongly recommend to add\n\n```\nfrom __future__ import division\n```\nto make sure that your code works fine when using Python 3 divisions.",
...
<a id='comment:2'></a>Since you're using divisions, I strongly recommend to add
...
<a id='comment:113'></a>Replying to [jdemeyer](#comment%3A110):
> Replying to [gh-kliem](#comment%3A107):
> > `sage.ext.memory_allocator.MemoryAllocator` seems to be great for `ListOfPointers`. I couldn't get 32-byte aligned to work with any prebuild solution.
...
<a id='comment:118'></a>Replying to [jdemeyer](#comment%3A98):
> I agree with Travis that error checking could be much improved: use [error return values](https://cython.readthedocs.io/en/latest/src/userguide/language_basics.html#error-return-values).
> 
> Also, instead of using `sig_malloc` and then checking for `NULL`, use `check_malloc` which automatically does that check. Even better, replace `sig_malloc(a * sizeof(T))` by `check_allocarray(a, sizeof(T))`.
...
```json
{
    "body": "<a id='comment:9'></a>For anything returning a Python object, you cannot add an `except` value. That's only for C return types.",
...
<a id='comment:9'></a>For anything returning a Python object, you cannot add an `except` value. That's only for C return types.
...
<a id='comment:122'></a>Replying to [tscrim](#comment%3A95):
> You should not have `get_next_level` wrapped in a `sig_on`/`sig_off`. That should do its own signal handling.
...
<a id='comment:123'></a>Replying to [jdemeyer](#comment%3A112):
> Since you're using divisions, I strongly recommend to add
> 
> ```
> from __future__ import division
...
<a id='comment:124'></a>Replying to [gh-kliem](#comment%3A122):
> Replying to [tscrim](#comment%3A95):
> > You should not have `get_next_level` wrapped in a `sig_on`/`sig_off`. That should do its own signal handling.
...
<a id='comment:125'></a>Replying to [gh-kliem](#comment%3A123):
> Instead I should us `//` for "floor division"?
...
<a id='comment:129'></a>Replying to [jdemeyer](#comment%3A124):
> Replying to [gh-kliem](#comment%3A122):
> > Replying to [tscrim](#comment%3A95):
> > > You should not have `get_next_level` wrapped in a `sig_on`/`sig_off`. That should do its own signal handling.
...
<a id='comment:6'></a>Two general comments:
...
<a id='comment:2'></a>How come that patchbots use completely different versions of the files then claimed?
...
```json
{
    "body": "<a id='comment:3'></a>The patchbot page contains all builds for a particular ticket, even old ones. I agree that it's not obvious to see whether a patchbot run is from the current commit or an earlier one.",
...
<a id='comment:3'></a>The patchbot page contains all builds for a particular ticket, even old ones. I agree that it's not obvious to see whether a patchbot run is from the current commit or an earlier one.
...
```json
{
    "body": "<a id='comment:4'></a>This should eventually use the functions added by #27292.",
...
<a id='comment:4'></a>This should eventually use the functions added by #27292.
...
<a id='comment:5'></a>I have lots of small comments while reading this code:
...
<a id='comment:146'></a>Thanks for the comments.
...
<a id='comment:147'></a>Replying to [gh-kliem](#comment%3A146):
> So `Py_ssize_t` is a good type for an index in there?
...
<a id='comment:9'></a>Another thing I could do to make life easier: Split this ticket in a number of tickets as good as possible.
...
```json
{
    "body": "<a id='comment:1'></a>Can you still have meaningful tests here with it being split? Although I think we have mostly gone through the code currently here already. So I am not sure there will be too much merit in splitting it, but maybe someone else has a different idea.",
...
<a id='comment:1'></a>Can you still have meaningful tests here with it being split? Although I think we have mostly gone through the code currently here already. So I am not sure there will be too much merit in splitting it, but maybe someone else has a different idea.
...
<a id='comment:2'></a>Ok, I leave the stuff in there.
...
<a id='comment:4'></a>Mainly I just changed `FaceIterator` to be an iterator. This also helped my remove most cython examples.
...
```json
{
    "body": "<a id='comment:7'></a>Please do not use \n\n```\n++        sage: it.next()\n```\nbut\n`next(it)`\nfor compatibility with python3.",
...
<a id='comment:7'></a>Please do not use 
...
```json
{
    "body": "<a id='comment:8'></a>Do you recommend to also remove `next = __next__` then?",
...
<a id='comment:8'></a>Do you recommend to also remove `next = __next__` then?
...
<a id='comment:160'></a>Replying to [jdemeyer](#comment%3A158):
> Do you recommend to also remove `next = __next__` then?
...
<a id='comment:1'></a>I am going through the doc right now.
...
```json
{
    "body": "<a id='comment:2'></a>According to [the patchbots](https://patchbot.sagemath.org/ticket/26887), the build is failing on a few different platforms.",
...
<a id='comment:2'></a>According to [the patchbots](https://patchbot.sagemath.org/ticket/26887), the build is failing on a few different platforms.
...
<a id='comment:163'></a>Replying to [jhpalmieri](#comment%3A162):
> According to [the patchbots](https://patchbot.sagemath.org/ticket/26887), the build is failing on a few different platforms.
...
<a id='comment:165'></a>Thanks, went through the entire document and tried to fix all of those issues.
...
<a id='comment:166'></a>Replying to [gh-kliem](#comment%3A163):
> Replying to [jhpalmieri](#comment%3A162):
> > According to [the patchbots](https://patchbot.sagemath.org/ticket/26887), the build is failing on a few different platforms.
...
<a id='comment:167'></a>Replying to [jhpalmieri](#comment%3A166):
> Replying to [gh-kliem](#comment%3A163):
> > Replying to [jhpalmieri](#comment%3A162):
> > > According to [the patchbots](https://patchbot.sagemath.org/ticket/26887), the build is failing on a few different platforms.
...
<a id='comment:170'></a>`new[]` worked fine before, so I tried that. I hope this solves the issue.
...
<a id='comment:171'></a>Replying to [gh-kliem](#comment%3A170):
> `new[]` worked fine before, so I tried that. I hope this solves the issue.
> 
> I did some googling and got the impression, that `calloc` etc. is not really the C++ way of doing it.
...
```json
{
    "body": "<a id='comment:2'></a>One of my colleges just gave me some input on readability.\n\nI will go through the code again and improve readability.",
...
<a id='comment:2'></a>One of my colleges just gave me some input on readability.
...
<a id='comment:3'></a>As mentioned above, I will try to make the comments more readable. (Writing complete English sentences. More intuitive positions of comments. Etc.)
...
<a id='comment:174'></a>Replying to [gh-kliem](#comment%3A167):
> Replying to [jhpalmieri](#comment%3A166):
> > Replying to [gh-kliem](#comment%3A163):
> > > Replying to [jhpalmieri](#comment%3A162):
> > > > According to [the patchbots](https://patchbot.sagemath.org/ticket/26887), the build is failing on a few different platforms.
...
```json
{
    "body": "<a id='comment:5'></a>I saw this, too, and after deleting those files, this built on my OS X box. I don't know if they are supposed to be cleaned up automatically somehow or not.",
...
<a id='comment:5'></a>I saw this, too, and after deleting those files, this built on my OS X box. I don't know if they are supposed to be cleaned up automatically somehow or not.
...
<a id='comment:176'></a>There seem to be scenarios where `KeyboardInterrupt` cause sage to crash. It will not use CPU anymore, so somehow it seems to work. But it will look exactly like a failed interrupt, so nothing works anymore in that session.
...
<a id='comment:8'></a>I hope this is easier to read now. I actually went through all the files.
...
<a id='comment:1'></a>Is there anything else I should do at this point?
...
<a id='comment:182'></a>Replying to [gh-kliem](#comment%3A181):
> The C++ files are not recompiled when they change (at least not always). I can add a syntax error and the compiler will not notice.
...
<a id='comment:4'></a>I gave a talk today in the seminar of Michael Joswig's work group.
...
```json
{
    "body": "<a id='comment:6'></a>`@`jdemeyer, `@`tscrim: I would appreciate this ticket to be merged soon so that we can move forward with the project. Do you have any additional critiques to be taken into account or is this ready to go?\n\nAnyway, thanks a lot for your previous work on this ticket!",
...
<a id='comment:6'></a>`@`jdemeyer, `@`tscrim: I would appreciate this ticket to be merged soon so that we can move forward with the project. Do you have any additional critiques to be taken into account or is this ready to go?
...
```json
{
    "body": "<a id='comment:8'></a>small optimization: use `smallInteger` instead of `Integer` (this does not create new integer objects but rather uses preallocated small integer objects)",
...
<a id='comment:8'></a>small optimization: use `smallInteger` instead of `Integer` (this does not create new integer objects but rather uses preallocated small integer objects)
...
```json
{
    "body": "<a id='comment:9'></a>- `Might be faster then` -> `Might be faster than`",
...
<a id='comment:9'></a>- `Might be faster then` -> `Might be faster than`
...
<a id='comment:192'></a>This is doing a typecast, as it assumes `(signed) long` as input.
...
<a id='comment:193'></a>Replying to [vdelecroix](#comment%3A187):
...
<a id='comment:194'></a>Replying to [gh-kliem](#comment%3A193):
> Replying to [vdelecroix](#comment%3A187):
> 
> > Technical comments
> > 
...
<a id='comment:195'></a>Replying to [gh-kliem](#comment%3A192):
> This is doing a typecast, as it assumes `(signed) long` as input.
> 
> I'm using `size_t` in most cases. But I guess for my use case this is just fine.
...
<a id='comment:197'></a>Replying to [vdelecroix](#comment%3A187):
> Main comment:
> 
> - the file `base.pyx` is huge. It would be nice to split it.
...
<a id='comment:198'></a>Replying to [gh-kliem](#comment%3A197):
> > doc of `combinatorial_polyhedron/base.pyx`
> > 
> > - the first line would better be the title of the module (eg look like `rings/integer.pyx`)
...
```json
{
    "body": "<a id='comment:2'></a>How do I check if all links in the docstrings work?",
...
<a id='comment:2'></a>How do I check if all links in the docstrings work?
...
<a id='comment:3'></a>This comment concerns only `CombinatorialPolyhedron` in `base.pxd` and `base.pyx`
...
```json
{
    "body": "<a id='comment:8'></a>I moved some of the methods to `CombinatorialFace` now. This should be much nicer for a regular user to handle. The iterator returns something that one can use.",
    "created_at": "2019-05-07T18:18:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26887",
    "type": "issue_comment",
...
<a id='comment:8'></a>I moved some of the methods to `CombinatorialFace` now. This should be much nicer for a regular user to handle. The iterator returns something that one can use.
...
```json
{
    "body": "<a id='comment:9'></a>does not build, see patchbot reports",
...
<a id='comment:9'></a>does not build, see patchbot reports
...
<a id='comment:1'></a>Just some small things. I am likely to accept a positive review on next round.
...
<a id='comment:212'></a>Replying to [vdelecroix](#comment%3A211):
> Just some small things. I am likely to accept a positive review on next round.
> 
> - You use custom bitsets. Why not using the one provided in [SageMath](SageMath) `sage.data_structures/bitest.[pxi,pxd]`? Most of it already uses optimized routines from GMP.
...
<a id='comment:3'></a>> It would be good to simplfy and uniformize string representations
...
<a id='comment:214'></a>Replying to [stumpc5](#comment%3A213):
> > It would be good to simplfy and uniformize string representations
...
<a id='comment:215'></a>Replying to [gh-kliem](#comment%3A214):
> Can you be more precise? I don't get what you want.
...
<a id='comment:216'></a>Replying to [stumpc5](#comment%3A215):
> Replying to [gh-kliem](#comment%3A214):
> > Can you be more precise? I don't get what you want.
...
<a id='comment:217'></a>Replying to [vdelecroix](#comment%3A211):
> Just some small things. I am likely to accept a positive review on next round.
> 
> - The name `ListOfAllFaces` sounds really strange.
> 
...
<a id='comment:218'></a>Replying to [gh-kliem](#comment%3A217):
> Replying to [vdelecroix](#comment%3A211):
> > Just some small things. I am likely to accept a positive review on next round.
> > 
> > - The name `ListOfAllFaces` sounds really strange.
...
<a id='comment:220'></a>Replying to [vdelecroix](#comment%3A218):
> then `PolyhedronFaceLattice` is even better.
...
<a id='comment:221'></a>I now give the number of facets. This seems more natural to me, as this is how `CombinatorialPolyhedron` is constructed from the facets. 
This seems to be more meaningful for many unbounded polyhedra as well (unless one wants to count lowest dimensional proper faces as "vertices").
...
```json
{
    "body": "<a id='comment:3'></a>I think I took care of above comments.",
    "created_at": "2019-05-31T20:37:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26887",
    "type": "issue_comment",
...
<a id='comment:3'></a>I think I took care of above comments.
...
```json
{
    "body": "<a id='comment:4'></a>I propose to move forward and set to positive review. If nobody objects, I will do so later today.",
    "created_at": "2019-06-04T05:40:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26887",
    "type": "issue_comment",
...
<a id='comment:4'></a>I propose to move forward and set to positive review. If nobody objects, I will do so later today.
...
<a id='comment:5'></a>I don't want to slow this ticket down by any means. Anyways, isn't the following two times the same combinatorial polyhedron
...
<a id='comment:226'></a>The representation has changed to give the number of facets now.
So this output won't happen like this anymore.
...
<a id='comment:227'></a>I think the input arguments should be more intuitive.
I also think this should happen in a follow up ticket.
...
```json
{
    "body": "<a id='comment:9'></a>Please move any further discussion to new tickets.",
...
<a id='comment:9'></a>Please move any further discussion to new tickets.
...
<a id='comment:1'></a>Fails on e.g. Debian 8:
...
<a id='comment:232'></a>As far as I know there are two ways of solving this.
...
<a id='comment:233'></a>Replying to [gh-kliem](#comment%3A232):
> As far as I know there are two ways of solving this.
> 
> Either use the macro `__cplusplus` to determine the c++ version and not use `inline` if it is not available. (This would be a quick fix I guess).
> 
...
```json
{
    "body": "<a id='comment:5'></a>added compile argument, this should work now (also #27987 fixes already a bug in the unbounded case)\n\nThanks for all the help, btw.",
    "created_at": "2019-06-27T19:04:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26887",
    "type": "issue_comment",
...
<a id='comment:5'></a>added compile argument, this should work now (also #27987 fixes already a bug in the unbounded case)
...
<a id='comment:9'></a>This triggers a new failing doctest with python3-sage:
...
```json
{
    "body": "<a id='comment:0'></a>I have made #28153 for the py3 problem, please review.",
...
<a id='comment:0'></a>I have made #28153 for the py3 problem, please review.
...
<a id='comment:242'></a>Replying to [gh-kliem](#comment%3A241):
> [...]
