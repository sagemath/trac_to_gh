# Issue 26151: Kummer surface functionality fails

archive/issues_025914.json:
```json
{
    "assignees": [],
    "body": "Currently the creation of the Kummer surface associated to a genus 2 curve fails. All three of the final statements in the following code return the same error.\n\n```\nsage: R.<x> = QQ[]\nsage: f = x^5 + x + 1\nsage: X = HyperellipticCurve(f)\nsage: J = Jacobian(X)\n\nsage: K = KummerSurface(J)\nsage: K = J.kummer_surface()\nsage: phi = X.kummer_morphism()\n```\n\nCC:  kohel @slel\n\nKeywords: **kummer**\n\nBranch/Commit: **[3f5f59c](https://github.com/sagemath/sagetrac-mirror/commit/3f5f59c8b7814443a358abbe1d8570c71c6b71b2)**\n\nReviewer: **Samuel Leli\u00e8vre**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26151_\n\n",
    "closed_at": "2019-07-04T11:09:33Z",
    "created_at": "2018-08-28T13:12:20Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Kummer surface functionality fails",
    "type": "issue",
    "updated_at": "2019-07-04T11:09:33Z",
    "url": "https://github.com/sagemath/sage/issues/26151",
    "user": "https://github.com/JRSijsling"
}
```
Currently the creation of the Kummer surface associated to a genus 2 curve fails. All three of the final statements in the following code return the same error.

```
sage: R.<x> = QQ[]
sage: f = x^5 + x + 1
sage: X = HyperellipticCurve(f)
sage: J = Jacobian(X)

sage: K = KummerSurface(J)
sage: K = J.kummer_surface()
sage: phi = X.kummer_morphism()
```

CC:  kohel @slel

Keywords: **kummer**

Branch/Commit: **[3f5f59c](https://github.com/sagemath/sagetrac-mirror/commit/3f5f59c8b7814443a358abbe1d8570c71c6b71b2)**

Reviewer: **Samuel Lelièvre**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/26151_





---

archive/issue_events_342811.json:
```json
{
    "actor": "https://github.com/JRSijsling",
    "created_at": "2018-08-28T13:12:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26151",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26151#event-342811"
}
```



---

archive/issue_events_342812.json:
```json
{
    "actor": "https://github.com/JRSijsling",
    "created_at": "2018-08-28T13:12:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26151",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
    "label_color": "0000ff",
    "label_name": "component: algebraic geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26151#event-342812"
}
```



---

archive/issue_events_342813.json:
```json
{
    "actor": "https://github.com/JRSijsling",
    "created_at": "2018-08-28T13:12:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26151",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26151#event-342813"
}
```



---

archive/issue_events_342814.json:
```json
{
    "actor": "https://github.com/JRSijsling",
    "created_at": "2018-08-28T13:12:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26151",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26151#event-342814"
}
```



---

archive/issue_comments_405606.json:
```json
{
    "body": "Commit: **[972750f](https://github.com/sagemath/sagetrac-mirror/commit/972750ff31e42e1cae9edfc3bd9f0cd672172006)**",
    "created_at": "2019-06-26T15:41:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26151",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26151#issuecomment-405606",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[972750f](https://github.com/sagemath/sagetrac-mirror/commit/972750ff31e42e1cae9edfc3bd9f0cd672172006)**



---

archive/issue_comments_405607.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nHere is a fix.\n\nNote an important change in the check inside the definition of projective morphisms..\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/972750ff31e42e1cae9edfc3bd9f0cd672172006\">972750f</a></td><td><code>trac 26151 fix Kummer surface and add doctest</code></td></tr></table>\n",
    "created_at": "2019-06-26T15:41:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26151",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26151#issuecomment-405607",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:2'>Comment 2:</a>
Here is a fix.

Note an important change in the check inside the definition of projective morphisms..

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/972750ff31e42e1cae9edfc3bd9f0cd672172006">972750f</a></td><td><code>trac 26151 fix Kummer surface and add doctest</code></td></tr></table>




---

archive/issue_events_342815.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-06-26T15:41:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26151",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26151#event-342815"
}
```



---

archive/issue_comments_405608.json:
```json
{
    "body": "Branch: **[u/chapoton/26151](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/26151)**",
    "created_at": "2019-06-26T15:41:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26151",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26151#issuecomment-405608",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/26151](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/26151)**



---

archive/issue_comments_405609.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/82855a6d9ac74beda07cebb8df5052137a0b217f\">82855a6</a></td><td><code>Merge branch 'u/chapoton/26151' in 8.8</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3f5f59c8b7814443a358abbe1d8570c71c6b71b2\">3f5f59c</a></td><td><code>trac 26151 fix doctest in scheme morphisms</code></td></tr></table>\n",
    "created_at": "2019-06-28T14:47:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26151",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26151#issuecomment-405609",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:3'>Comment 3:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/82855a6d9ac74beda07cebb8df5052137a0b217f">82855a6</a></td><td><code>Merge branch 'u/chapoton/26151' in 8.8</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3f5f59c8b7814443a358abbe1d8570c71c6b71b2">3f5f59c</a></td><td><code>trac 26151 fix doctest in scheme morphisms</code></td></tr></table>




---

archive/issue_comments_405610.json:
```json
{
    "body": "Changed commit from **[972750f](https://github.com/sagemath/sagetrac-mirror/commit/972750ff31e42e1cae9edfc3bd9f0cd672172006)** to **[3f5f59c](https://github.com/sagemath/sagetrac-mirror/commit/3f5f59c8b7814443a358abbe1d8570c71c6b71b2)**",
    "created_at": "2019-06-28T14:47:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26151",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26151#issuecomment-405610",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[972750f](https://github.com/sagemath/sagetrac-mirror/commit/972750ff31e42e1cae9edfc3bd9f0cd672172006)** to **[3f5f59c](https://github.com/sagemath/sagetrac-mirror/commit/3f5f59c8b7814443a358abbe1d8570c71c6b71b2)**



---

archive/issue_comments_405611.json:
```json
{
    "body": "Author: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2019-06-28T15:13:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26151",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26151#issuecomment-405611",
    "user": "https://github.com/fchapoton"
}
```

Author: **Frédéric Chapoton**



---

archive/issue_comments_405612.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\ngreen bot, needs review",
    "created_at": "2019-06-30T19:05:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26151",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26151#issuecomment-405612",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:5'>Comment 5:</a>
green bot, needs review



---

archive/issue_comments_405613.json:
```json
{
    "body": "Changed keywords from **** to **kummer**",
    "created_at": "2019-06-30T20:54:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26151",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26151#issuecomment-405613",
    "user": "https://github.com/slel"
}
```

Changed keywords from **** to **kummer**



---

archive/issue_comments_405614.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nThanks for finding how to fix this.",
    "created_at": "2019-06-30T20:54:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26151",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26151#issuecomment-405614",
    "user": "https://github.com/slel"
}
```

<a id='comment:6'>Comment 6:</a>
Thanks for finding how to fix this.



---

archive/issue_events_342816.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-06-30T20:54:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26151",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26151#event-342816"
}
```



---

archive/issue_events_342817.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-06-30T20:54:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26151",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26151#event-342817"
}
```



---

archive/issue_comments_405615.json:
```json
{
    "body": "Reviewer: **Samuel Leli\u00e8vre**",
    "created_at": "2019-06-30T20:54:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26151",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26151#issuecomment-405615",
    "user": "https://github.com/slel"
}
```

Reviewer: **Samuel Lelièvre**



---

archive/issue_events_342818.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-07-01T06:40:31Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/26151",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26151#event-342818"
}
```



---

archive/issue_events_342819.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-07-01T06:40:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26151",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26151#event-342819"
}
```



---

archive/issue_comments_405616.json:
```json
{
    "body": "Changed branch from **[u/chapoton/26151](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/26151)** to **[3f5f59c](https://github.com/sagemath/sagetrac-mirror/commit/3f5f59c8b7814443a358abbe1d8570c71c6b71b2)**",
    "created_at": "2019-07-04T11:09:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26151",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26151#issuecomment-405616",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/26151](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/26151)** to **[3f5f59c](https://github.com/sagemath/sagetrac-mirror/commit/3f5f59c8b7814443a358abbe1d8570c71c6b71b2)**



---

archive/issue_events_342820.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-07-04T11:09:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26151",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26151#event-342820"
}
```



---

archive/issue_events_342821.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "5a81a99d8df380533525c6b0fbdd6de2e1bf4e51",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-07-04T11:09:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26151",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26151#event-342821"
}
```
