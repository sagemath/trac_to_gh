# Issue 26019: py3: fixes to sage.rings.polynomial.real_roots

archive/issues_025782.json:
```json
{
    "assignees": [],
    "body": "This fixes all the tests for this module on Python 3, and also serves as a small follow-up to #25247.\n\nBranch/Commit: **[db22bc7](https://github.com/sagemath/sagetrac-mirror/commit/db22bc7951f4a054166b99f5cb8f74af587f178f)**\n\nReviewer: **Jeroen Demeyer, Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Erik Bray**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26019_\n\n",
    "closed_at": "2018-08-17T21:13:54Z",
    "created_at": "2018-08-07T10:43:59Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20python3",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "py3: fixes to sage.rings.polynomial.real_roots",
    "type": "issue",
    "updated_at": "2018-08-17T21:13:54Z",
    "url": "https://github.com/sagemath/sage/issues/26019",
    "user": "https://github.com/embray"
}
```
This fixes all the tests for this module on Python 3, and also serves as a small follow-up to #25247.

Branch/Commit: **[db22bc7](https://github.com/sagemath/sagetrac-mirror/commit/db22bc7951f4a054166b99f5cb8f74af587f178f)**

Reviewer: **Jeroen Demeyer, Frédéric Chapoton**

Author: **Erik Bray**

_Issue created by migration from https://trac.sagemath.org/ticket/26019_





---

archive/issue_events_341332.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-08-07T10:43:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26019",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26019#event-341332"
}
```



---

archive/issue_events_341333.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-08-07T10:43:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26019",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20python3",
    "label_color": "000080",
    "label_name": "component: python3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26019#event-341333"
}
```



---

archive/issue_events_341334.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-08-07T10:43:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26019",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26019#event-341334"
}
```



---

archive/issue_events_341335.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-08-07T10:43:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26019",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26019#event-341335"
}
```



---

archive/issue_events_341336.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-08-07T10:44:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26019",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26019#event-341336"
}
```



---

archive/issue_comments_403604.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2018-08-07T13:48:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26019",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26019#issuecomment-403604",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_341337.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-08-07T13:48:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26019",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26019#event-341337"
}
```



---

archive/issue_events_341338.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-08-07T13:48:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26019",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26019#event-341338"
}
```



---

archive/issue_events_341339.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-08-10T12:00:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26019",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26019#event-341339"
}
```



---

archive/issue_events_341340.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-08-10T12:00:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26019",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26019#event-341340"
}
```



---

archive/issue_comments_403605.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nSee patchbot",
    "created_at": "2018-08-10T12:00:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26019",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26019#issuecomment-403605",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:3'>Comment 3:</a>
See patchbot



---

archive/issue_comments_403606.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nFor the record:\n\n```\nsage -t --long src/sage/schemes/elliptic_curves/ell_rational_field.py\n**********************************************************************\nFile \"src/sage/schemes/elliptic_curves/ell_rational_field.py\", line 3338, in sage.schemes.elliptic_curves.ell_rational_field.EllipticCurve_rational_field.elliptic_exponential\nFailed example:\n    2 * E.elliptic_exponential(z)\nExpected:\n    (-1.52184235874404 - 0.0581413944316544*I : 0.948655866506124 - 0.0381469928565030*I : 1.00000000000000)\nGot:\n    (-0.379082374337223 + 0.401554153021658*I : -1.64457248125273 + 0.436489289368578*I : 1.00000000000000)\n**********************************************************************\nFile \"src/sage/schemes/elliptic_curves/ell_rational_field.py\", line 3340, in sage.schemes.elliptic_curves.ell_rational_field.EllipticCurve_rational_field.elliptic_exponential\nFailed example:\n    E.elliptic_exponential(2 * z)\nExpected:\n    (-1.52184235874404 - 0.0581413944316562*I : 0.948655866506128 - 0.0381469928565034*I : 1.00000000000000)\nGot:\n    (-0.379082374337223 + 0.401554153021660*I : -1.64457248125273 + 0.436489289368575*I : 1.00000000000000)\n**********************************************************************\n1 item had failures:\n   2 of  21 in sage.schemes.elliptic_curves.ell_rational_field.EllipticCurve_rational_field.elliptic_exponential\n    [848 tests, 2 failures, 176.91 s]\n----------------------------------------------------------------------\nsage -t --long src/sage/schemes/elliptic_curves/ell_rational_field.py  # 2 doctests failed\n----------------------------------------------------------------------\n```\n\npossibly related to the randomization change but that's not clear yet.",
    "created_at": "2018-08-10T13:27:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26019",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26019#issuecomment-403606",
    "user": "https://github.com/embray"
}
```

<a id='comment:4'>Comment 4:</a>
For the record:

```
sage -t --long src/sage/schemes/elliptic_curves/ell_rational_field.py
**********************************************************************
File "src/sage/schemes/elliptic_curves/ell_rational_field.py", line 3338, in sage.schemes.elliptic_curves.ell_rational_field.EllipticCurve_rational_field.elliptic_exponential
Failed example:
    2 * E.elliptic_exponential(z)
Expected:
    (-1.52184235874404 - 0.0581413944316544*I : 0.948655866506124 - 0.0381469928565030*I : 1.00000000000000)
Got:
    (-0.379082374337223 + 0.401554153021658*I : -1.64457248125273 + 0.436489289368578*I : 1.00000000000000)
**********************************************************************
File "src/sage/schemes/elliptic_curves/ell_rational_field.py", line 3340, in sage.schemes.elliptic_curves.ell_rational_field.EllipticCurve_rational_field.elliptic_exponential
Failed example:
    E.elliptic_exponential(2 * z)
Expected:
    (-1.52184235874404 - 0.0581413944316562*I : 0.948655866506128 - 0.0381469928565034*I : 1.00000000000000)
Got:
    (-0.379082374337223 + 0.401554153021660*I : -1.64457248125273 + 0.436489289368575*I : 1.00000000000000)
**********************************************************************
1 item had failures:
   2 of  21 in sage.schemes.elliptic_curves.ell_rational_field.EllipticCurve_rational_field.elliptic_exponential
    [848 tests, 2 failures, 176.91 s]
----------------------------------------------------------------------
sage -t --long src/sage/schemes/elliptic_curves/ell_rational_field.py  # 2 doctests failed
----------------------------------------------------------------------
```

possibly related to the randomization change but that's not clear yet.



---

archive/issue_comments_403607.json:
```json
{
    "body": "Changed commit from **[bec801e](https://github.com/sagemath/sagetrac-mirror/commit/bec801e3b085e866a88906561c912d6b03ef52a3)** to **[db22bc7](https://github.com/sagemath/sagetrac-mirror/commit/db22bc7951f4a054166b99f5cb8f74af587f178f)**",
    "created_at": "2018-08-14T13:06:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26019",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26019#issuecomment-403607",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[bec801e](https://github.com/sagemath/sagetrac-mirror/commit/bec801e3b085e866a88906561c912d6b03ef52a3)** to **[db22bc7](https://github.com/sagemath/sagetrac-mirror/commit/db22bc7951f4a054166b99f5cb8f74af587f178f)**



---

archive/issue_comments_403608.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/db22bc7951f4a054166b99f5cb8f74af587f178f\">db22bc7</a></td><td><code>add an optional 'seed' argument to randstate.python_random</code></td></tr></table>\n",
    "created_at": "2018-08-14T13:06:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26019",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26019#issuecomment-403608",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:5'>Comment 5:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/db22bc7951f4a054166b99f5cb8f74af587f178f">db22bc7</a></td><td><code>add an optional 'seed' argument to randstate.python_random</code></td></tr></table>




---

archive/issue_comments_403609.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nThis should fix it.  A just as valid fix would have been to change the expected output of the tests that broke, which only broke due to a valid, but non-obvious, change in the PRNG sequence due to adding a call to `randstate.python_random()`, which in turn calls `ZZ.random_element()`.\n\nSo technically nothing was actually broken.  Nevertheless, it occurred to me that in this case we were seeding the Python `Random` with `ZZ.random_element()`, and then immediately re-seeding it, which could just as well be avoided by the addition of a new optional argument to `randstate.python_random()`.",
    "created_at": "2018-08-14T13:09:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26019",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26019#issuecomment-403609",
    "user": "https://github.com/embray"
}
```

<a id='comment:6'>Comment 6:</a>
This should fix it.  A just as valid fix would have been to change the expected output of the tests that broke, which only broke due to a valid, but non-obvious, change in the PRNG sequence due to adding a call to `randstate.python_random()`, which in turn calls `ZZ.random_element()`.

So technically nothing was actually broken.  Nevertheless, it occurred to me that in this case we were seeding the Python `Random` with `ZZ.random_element()`, and then immediately re-seeding it, which could just as well be avoided by the addition of a new optional argument to `randstate.python_random()`.



---

archive/issue_events_341341.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-08-14T13:09:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26019",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26019#event-341341"
}
```



---

archive/issue_events_341342.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-08-14T13:09:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26019",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26019#event-341342"
}
```



---

archive/issue_comments_403610.json:
```json
{
    "body": "Changed reviewer from **Jeroen Demeyer** to **Jeroen Demeyer, Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2018-08-15T06:57:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26019",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26019#issuecomment-403610",
    "user": "https://github.com/fchapoton"
}
```

Changed reviewer from **Jeroen Demeyer** to **Jeroen Demeyer, Frédéric Chapoton**



---

archive/issue_events_341343.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-08-15T06:57:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26019",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26019#event-341343"
}
```



---

archive/issue_events_341344.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-08-15T06:57:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26019",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26019#event-341344"
}
```



---

archive/issue_comments_403611.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nok, then",
    "created_at": "2018-08-15T06:57:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26019",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26019#issuecomment-403611",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:7'>Comment 7:</a>
ok, then



---

archive/issue_events_341345.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-08-17T21:13:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26019",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26019#event-341345"
}
```



---

archive/issue_events_341346.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "71a98b0fea20e42c545b740ec6694b58225e129d",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-08-17T21:13:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26019",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26019#event-341346"
}
```



---

archive/issue_comments_403612.json:
```json
{
    "body": "Changed branch from **[u/embray/python3/sage-rings-polynomial-real_roots/misc](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/python3/sage-rings-polynomial-real_roots/misc)** to **[db22bc7](https://github.com/sagemath/sagetrac-mirror/commit/db22bc7951f4a054166b99f5cb8f74af587f178f)**",
    "created_at": "2018-08-17T21:13:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26019",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26019#issuecomment-403612",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/embray/python3/sage-rings-polynomial-real_roots/misc](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/python3/sage-rings-polynomial-real_roots/misc)** to **[db22bc7](https://github.com/sagemath/sagetrac-mirror/commit/db22bc7951f4a054166b99f5cb8f74af587f178f)**
