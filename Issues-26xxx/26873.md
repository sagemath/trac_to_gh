# Issue 26873: Remove deprecations in Parent.__init__

archive/issues_026636.json:
```json
{
    "assignees": [],
    "body": "Remove code deprecated in #24109, #23917.\n\nAlso make it an error to call `_init_category_` with a non-`Category` and use `type(self)` instead of `self.__class__`.\n\nCC:  @fchapoton\n\nBranch/Commit: **[2431770](https://github.com/sagemath/sagetrac-mirror/commit/24317703856a197ccdb606f86a965c8ca9a76082)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26873_\n\n",
    "closed_at": "2018-12-23T23:40:03Z",
    "created_at": "2018-12-11T08:56:08Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove deprecations in Parent.__init__",
    "type": "issue",
    "updated_at": "2018-12-28T14:06:38Z",
    "url": "https://github.com/sagemath/sage/issues/26873",
    "user": "https://github.com/jdemeyer"
}
```
Remove code deprecated in #24109, #23917.

Also make it an error to call `_init_category_` with a non-`Category` and use `type(self)` instead of `self.__class__`.

CC:  @fchapoton

Branch/Commit: **[2431770](https://github.com/sagemath/sagetrac-mirror/commit/24317703856a197ccdb606f86a965c8ca9a76082)**

Reviewer: **Frédéric Chapoton**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/26873_





---

archive/issue_events_351072.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-12-11T08:56:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26873",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26873#event-351072"
}
```



---

archive/issue_events_351073.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-12-11T08:56:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26873",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26873#event-351073"
}
```



---

archive/issue_events_351074.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-12-11T08:56:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26873",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26873#event-351074"
}
```



---

archive/issue_events_351075.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-12-11T08:58:48Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/26873",
    "title_is": "Remove deprecations in Parent.__init__",
    "title_was": "Remove deprecations in parent",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26873#event-351075"
}
```



---

archive/issue_comments_416615.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-\n+Remove code deprecated in #24109, #23917.\n``````\n",
    "created_at": "2018-12-11T09:02:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26873#issuecomment-416615",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-
+Remove code deprecated in #24109, #23917.
``````




---

archive/issue_comments_416616.json:
```json
{
    "body": "Branch: **[u/jdemeyer/remove_deprecations_in_parent](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/remove_deprecations_in_parent)**",
    "created_at": "2018-12-11T09:18:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26873#issuecomment-416616",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/remove_deprecations_in_parent](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/remove_deprecations_in_parent)**



---

archive/issue_comments_416617.json:
```json
{
    "body": "Commit: **[8add14f](https://github.com/sagemath/sagetrac-mirror/commit/8add14ff10642160c6adb5b7139b73814420ac0d)**",
    "created_at": "2018-12-11T09:19:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26873#issuecomment-416617",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[8add14f](https://github.com/sagemath/sagetrac-mirror/commit/8add14ff10642160c6adb5b7139b73814420ac0d)**



---

archive/issue_comments_416618.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8add14ff10642160c6adb5b7139b73814420ac0d\">8add14f</a></td><td><code>Remove deprecated code regarding parent initialization</code></td></tr></table>\n",
    "created_at": "2018-12-11T09:19:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26873#issuecomment-416618",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:4" align="right">Comment 4</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8add14ff10642160c6adb5b7139b73814420ac0d">8add14f</a></td><td><code>Remove deprecated code regarding parent initialization</code></td></tr></table>




---

archive/issue_events_351076.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-12-11T09:19:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26873",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26873#event-351076"
}
```



---

archive/issue_comments_416619.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n Remove code deprecated in #24109, #23917.\n+\n+Also make it an error to call `_init_category_` with a non-`Category`.\n``````\n",
    "created_at": "2018-12-11T09:19:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26873#issuecomment-416619",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
 Remove code deprecated in #24109, #23917.
+
+Also make it an error to call `_init_category_` with a non-`Category`.
``````




---

archive/issue_comments_416620.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e51142df7519944ebf81ebded998468ae6386d1e\">e51142d</a></td><td><code>Remove deprecated code regarding parent initialization</code></td></tr></table>\n",
    "created_at": "2018-12-11T09:21:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26873#issuecomment-416620",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e51142df7519944ebf81ebded998468ae6386d1e">e51142d</a></td><td><code>Remove deprecated code regarding parent initialization</code></td></tr></table>




---

archive/issue_comments_416621.json:
```json
{
    "body": "Changed commit from **[8add14f](https://github.com/sagemath/sagetrac-mirror/commit/8add14ff10642160c6adb5b7139b73814420ac0d)** to **[e51142d](https://github.com/sagemath/sagetrac-mirror/commit/e51142df7519944ebf81ebded998468ae6386d1e)**",
    "created_at": "2018-12-11T09:21:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26873#issuecomment-416621",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[8add14f](https://github.com/sagemath/sagetrac-mirror/commit/8add14ff10642160c6adb5b7139b73814420ac0d)** to **[e51142d](https://github.com/sagemath/sagetrac-mirror/commit/e51142df7519944ebf81ebded998468ae6386d1e)**



---

archive/issue_comments_416622.json:
```json
{
    "body": "Changed commit from **[e51142d](https://github.com/sagemath/sagetrac-mirror/commit/e51142df7519944ebf81ebded998468ae6386d1e)** to **[2431770](https://github.com/sagemath/sagetrac-mirror/commit/24317703856a197ccdb606f86a965c8ca9a76082)**",
    "created_at": "2018-12-11T09:39:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26873#issuecomment-416622",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[e51142d](https://github.com/sagemath/sagetrac-mirror/commit/e51142df7519944ebf81ebded998468ae6386d1e)** to **[2431770](https://github.com/sagemath/sagetrac-mirror/commit/24317703856a197ccdb606f86a965c8ca9a76082)**



---

archive/issue_comments_416623.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/24317703856a197ccdb606f86a965c8ca9a76082\">2431770</a></td><td><code>Remove deprecated code regarding parent initialization</code></td></tr></table>\n",
    "created_at": "2018-12-11T09:39:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26873#issuecomment-416623",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:7" align="right">Comment 7</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/24317703856a197ccdb606f86a965c8ca9a76082">2431770</a></td><td><code>Remove deprecated code regarding parent initialization</code></td></tr></table>




---

archive/issue_comments_416624.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n Remove code deprecated in #24109, #23917.\n \n-Also make it an error to call `_init_category_` with a non-`Category`.\n+Also make it an error to call `_init_category_` with a non-`Category` and use `type(self)` instead of `self.__class__`.\n``````\n",
    "created_at": "2018-12-11T09:42:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26873#issuecomment-416624",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 Remove code deprecated in #24109, #23917.
 
-Also make it an error to call `_init_category_` with a non-`Category`.
+Also make it an error to call `_init_category_` with a non-`Category` and use `type(self)` instead of `self.__class__`.
``````




---

archive/issue_comments_416625.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nok, looks good",
    "created_at": "2018-12-11T17:05:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26873#issuecomment-416625",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:9" align="right">Comment 9</div>

ok, looks good



---

archive/issue_comments_416626.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2018-12-11T17:05:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26873#issuecomment-416626",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_events_351077.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-12-11T17:05:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26873",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26873#event-351077"
}
```



---

archive/issue_events_351078.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-12-11T17:05:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26873",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26873#event-351078"
}
```



---

archive/issue_comments_416627.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/remove_deprecations_in_parent](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/remove_deprecations_in_parent)** to **[2431770](https://github.com/sagemath/sagetrac-mirror/commit/24317703856a197ccdb606f86a965c8ca9a76082)**",
    "created_at": "2018-12-23T23:40:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26873#issuecomment-416627",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/remove_deprecations_in_parent](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/remove_deprecations_in_parent)** to **[2431770](https://github.com/sagemath/sagetrac-mirror/commit/24317703856a197ccdb606f86a965c8ca9a76082)**



---

archive/issue_events_351079.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-12-23T23:40:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26873",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26873#event-351079"
}
```



---

archive/issue_events_351080.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "2a690927bad5873b949f61ec44cd671e90d6bae1",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-12-23T23:40:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26873",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26873#event-351080"
}
```



---

archive/issue_comments_416628.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nThis tickets were closed as fixed after the Sage 8.5 release.",
    "created_at": "2018-12-28T14:06:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26873#issuecomment-416628",
    "user": "https://github.com/embray"
}
```

<div id="comment:11" align="right">Comment 11</div>

This tickets were closed as fixed after the Sage 8.5 release.



---

archive/issue_events_351081.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-12-28T14:06:38Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/26873",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26873#event-351081"
}
```



---

archive/issue_events_351082.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-12-28T14:06:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26873",
    "milestone_number": null,
    "milestone_title": "sage-8.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26873#event-351082"
}
```
