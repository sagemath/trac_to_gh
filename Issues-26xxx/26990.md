# Issue 26990: Race when copying package files

archive/issues_026753.json:
```json
{
    "body": "CC:  @embray\n\nThere seems to be a race when copying files as cp is not atomic. I'm not sure if the copying holds a lock. Either the copying or the `*.pyc` file generation by the Python interpreter in another process conflicted here:\n\n```\n[future-0.17.1] Successfully installed future-0.17.1\n[future-0.17.1] Cleaning up...\n[future-0.17.1] Removed build tracker '/tmp/pip-req-tracker-ruDJcT'\n[future-0.17.1] \n[future-0.17.1] real\t0m4.907s\n[future-0.17.1] user\t0m2.226s\n[future-0.17.1] sys\t0m0.382s\n[future-0.17.1] Copying package files from temporary location /home/chapoton/sage/local/var/tmp/sage/build/future-0.17.1/inst to /home/chapoton/sage/local\n[future-0.17.1] cp: cannot create regular file '/home/chapoton/sage/local/./lib/python2.7/site-packages/future/__init__.pyc': File exists\n[future-0.17.1] ************************************************************************\n[future-0.17.1] Error copying files for future-0.17.1.\n[future-0.17.1] ************************************************************************\n[future-0.17.1] Please email sage-devel (http://groups.google.com/group/sage-devel)\n[future-0.17.1] explaining the problem and including the log file\n[future-0.17.1]   /home/chapoton/sage/logs/pkgs/future-0.17.1.log\n[future-0.17.1] Describe your computer, operating system, etc.\n[future-0.17.1] ************************************************************************\n```\nSee also: https://unix.stackexchange.com/questions/116280/cannot-create-regular-file-filename-file-exists\n\nIssue created by migration from https://trac.sagemath.org/ticket/26990\n\n",
    "closed_at": "2019-01-03T14:21:33Z",
    "created_at": "2019-01-01T21:54:59Z",
    "labels": [
        "component: build",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "Race when copying package files",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/26990",
    "user": "https://github.com/vbraun"
}
```
CC:  @embray

There seems to be a race when copying files as cp is not atomic. I'm not sure if the copying holds a lock. Either the copying or the `*.pyc` file generation by the Python interpreter in another process conflicted here:

```
[future-0.17.1] Successfully installed future-0.17.1
[future-0.17.1] Cleaning up...
[future-0.17.1] Removed build tracker '/tmp/pip-req-tracker-ruDJcT'
[future-0.17.1] 
[future-0.17.1] real	0m4.907s
[future-0.17.1] user	0m2.226s
[future-0.17.1] sys	0m0.382s
[future-0.17.1] Copying package files from temporary location /home/chapoton/sage/local/var/tmp/sage/build/future-0.17.1/inst to /home/chapoton/sage/local
[future-0.17.1] cp: cannot create regular file '/home/chapoton/sage/local/./lib/python2.7/site-packages/future/__init__.pyc': File exists
[future-0.17.1] ************************************************************************
[future-0.17.1] Error copying files for future-0.17.1.
[future-0.17.1] ************************************************************************
[future-0.17.1] Please email sage-devel (http://groups.google.com/group/sage-devel)
[future-0.17.1] explaining the problem and including the log file
[future-0.17.1]   /home/chapoton/sage/logs/pkgs/future-0.17.1.log
[future-0.17.1] Describe your computer, operating system, etc.
[future-0.17.1] ************************************************************************
```
See also: https://unix.stackexchange.com/questions/116280/cannot-create-regular-file-filename-file-exists

Issue created by migration from https://trac.sagemath.org/ticket/26990





---

archive/issue_comments_375816.json:
```json
{
    "body": "Resolution: duplicate",
    "created_at": "2019-01-02T17:02:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26990#issuecomment-375816",
    "user": "https://github.com/embray"
}
```

Resolution: duplicate



---

archive/issue_events_066957.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-01-02T17:02:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/26990",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/26990#event-66957"
}
```



---

archive/issue_events_066958.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-01-02T17:02:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/26990",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/26990#event-66958"
}
```



---

archive/issue_comments_375817.json:
```json
{
    "body": "This is basically a duplicate of #26018, particularly comment:4:ticket:26018, though I'm not positive if the branch there fixes it for all cases.",
    "created_at": "2019-01-02T17:02:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26990#issuecomment-375817",
    "user": "https://github.com/embray"
}
```

This is basically a duplicate of #26018, particularly comment:4:ticket:26018, though I'm not positive if the branch there fixes it for all cases.



---

archive/issue_comments_375818.json:
```json
{
    "body": "Actually I'm not sure this is #26018, but it still looks like a fluke.  How could you get:\n\n```\nError copying files for future-0.17.1.\n```\n\n*after*\n\n```\nSuccessfully installed future-0.17.1\n```\n\nunless somehow the same package was being installed twice simultaneously?",
    "created_at": "2019-01-02T17:07:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26990#issuecomment-375818",
    "user": "https://github.com/embray"
}
```

Actually I'm not sure this is #26018, but it still looks like a fluke.  How could you get:

```
Error copying files for future-0.17.1.
```

*after*

```
Successfully installed future-0.17.1
```

unless somehow the same package was being installed twice simultaneously?



---

archive/issue_comments_375819.json:
```json
{
    "body": "The `Error copying files for future-0.17.1.` is just the error branch in `build/bin/sage-spkg`\n\nThe success message is written to stdout, and the error message to stderr. Both are independently buffered streams, it is to be expected that they are out of order.",
    "created_at": "2019-01-02T17:32:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26990#issuecomment-375819",
    "user": "https://github.com/vbraun"
}
```

The `Error copying files for future-0.17.1.` is just the error branch in `build/bin/sage-spkg`

The success message is written to stdout, and the error message to stderr. Both are independently buffered streams, it is to be expected that they are out of order.



---

archive/issue_comments_375820.json:
```json
{
    "body": "Changing status from closed to new.",
    "created_at": "2019-01-02T17:32:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26990#issuecomment-375820",
    "user": "https://github.com/vbraun"
}
```

Changing status from closed to new.



---

archive/issue_events_066959.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-01-02T17:32:09Z",
    "event": "reopened",
    "issue": "https://github.com/sagemath/sagetest/issues/26990",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/26990#event-66959"
}
```



---

archive/issue_comments_375821.json:
```json
{
    "body": "Resolution changed from duplicate to ",
    "created_at": "2019-01-02T17:32:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26990#issuecomment-375821",
    "user": "https://github.com/vbraun"
}
```

Resolution changed from duplicate to 



---

archive/issue_comments_375822.json:
```json
{
    "body": "I understand that, but the error case is:\n\n\n```\n 903     $SAGE_SUDO cp -Rp \"$PREFIX/.\" \"$SAGE_LOCAL\"\n 904     if [ $? -ne 0 ]; then\n 905         error_msg \"Error copying files for $PKG_NAME.\"\n 906         exit 1\n 907     fi\n```\n\ni.e. it should have exited the script before the success message was ever echoed at all.",
    "created_at": "2019-01-03T13:27:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26990#issuecomment-375822",
    "user": "https://github.com/embray"
}
```

I understand that, but the error case is:


```
 903     $SAGE_SUDO cp -Rp "$PREFIX/." "$SAGE_LOCAL"
 904     if [ $? -ne 0 ]; then
 905         error_msg "Error copying files for $PKG_NAME."
 906         exit 1
 907     fi
```

i.e. it should have exited the script before the success message was ever echoed at all.



---

archive/issue_comments_375823.json:
```json
{
    "body": "The first 'Successfully installed' is printed by pip, which is also why its before the 'Removed build tracker...'",
    "created_at": "2019-01-03T13:47:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26990#issuecomment-375823",
    "user": "https://github.com/vbraun"
}
```

The first 'Successfully installed' is printed by pip, which is also why its before the 'Removed build tracker...'



---

archive/issue_comments_375824.json:
```json
{
    "body": "Yes, I just realized that.  It's the same exact message so it's confusing.  In that case I do believe this is a duplicate.",
    "created_at": "2019-01-03T14:04:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26990#issuecomment-375824",
    "user": "https://github.com/embray"
}
```

Yes, I just realized that.  It's the same exact message so it's confusing.  In that case I do believe this is a duplicate.



---

archive/issue_comments_375825.json:
```json
{
    "body": "So this is still a duplicate of #26018.  The problem with the `future` package in particular is that it (confusingly) provides stand-ins for several packages and modules in the Python 3 stdlib, such that they end up getting imported during the build process by other packages (such as pip) that support Python 3.\n\nSo I need to update #26018 to include `future` in the list of \"python toolchain packages\" that are not installed in parallel.",
    "created_at": "2019-01-03T14:21:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26990#issuecomment-375825",
    "user": "https://github.com/embray"
}
```

So this is still a duplicate of #26018.  The problem with the `future` package in particular is that it (confusingly) provides stand-ins for several packages and modules in the Python 3 stdlib, such that they end up getting imported during the build process by other packages (such as pip) that support Python 3.

So I need to update #26018 to include `future` in the list of "python toolchain packages" that are not installed in parallel.



---

archive/issue_comments_375826.json:
```json
{
    "body": "Resolution: duplicate",
    "created_at": "2019-01-03T14:21:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26990#issuecomment-375826",
    "user": "https://github.com/embray"
}
```

Resolution: duplicate



---

archive/issue_events_066960.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-01-03T14:21:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/26990",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/26990#event-66960"
}
```



---

archive/issue_comments_375827.json:
```json
{
    "body": "#26018 seems to have stalled\n\nI'm also not convinced that its the right fix, imho we just shoudn't copy over pyc files for installation (they contain hardcoded absolute paths). Nothing good can come out of this.",
    "created_at": "2019-07-28T09:57:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26990#issuecomment-375827",
    "user": "https://github.com/vbraun"
}
```

#26018 seems to have stalled

I'm also not convinced that its the right fix, imho we just shoudn't copy over pyc files for installation (they contain hardcoded absolute paths). Nothing good can come out of this.



---

archive/issue_comments_375828.json:
```json
{
    "body": "Resolution changed from duplicate to ",
    "created_at": "2019-07-29T11:48:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26990#issuecomment-375828",
    "user": "https://github.com/vbraun"
}
```

Resolution changed from duplicate to 



---

archive/issue_comments_375829.json:
```json
{
    "body": "Changing status from closed to new.",
    "created_at": "2019-07-29T11:48:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26990#issuecomment-375829",
    "user": "https://github.com/vbraun"
}
```

Changing status from closed to new.



---

archive/issue_events_066961.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-07-29T11:48:34Z",
    "event": "reopened",
    "issue": "https://github.com/sagemath/sagetest/issues/26990",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/26990#event-66961"
}
```



---

archive/issue_comments_375830.json:
```json
{
    "body": "That's a good point about the filename pyc files, but I'm not sure that's really the issue here.  Besides, I believe there are ways (e.g., maybe with the [compileall](https://docs.python.org/3/library/compileall.html#module-compileall) module) to deal with that, though I haven't tried.\n\n#26018 doesn't appear to be stalled for any good reason.  I set it to needs_review 7 months, no one did anything, and now it needs to be rebased again.",
    "created_at": "2019-08-12T14:30:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26990#issuecomment-375830",
    "user": "https://github.com/embray"
}
```

That's a good point about the filename pyc files, but I'm not sure that's really the issue here.  Besides, I believe there are ways (e.g., maybe with the [compileall](https://docs.python.org/3/library/compileall.html#module-compileall) module) to deal with that, though I haven't tried.

#26018 doesn't appear to be stalled for any good reason.  I set it to needs_review 7 months, no one did anything, and now it needs to be rebased again.



---

archive/issue_comments_375831.json:
```json
{
    "body": "Perhaps all that DESTDIR business should only apply to non-Python modules\nif we arrange for all Python modules to be installed via `pip`.",
    "created_at": "2020-02-10T20:10:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26990#issuecomment-375831",
    "user": "https://github.com/mkoeppe"
}
```

Perhaps all that DESTDIR business should only apply to non-Python modules
if we arrange for all Python modules to be installed via `pip`.



---

archive/issue_comments_375832.json:
```json
{
    "body": "They are already installed with pip, and regardless I don't think that would particularly help.  I think it would be annoying to have some SPKGs that use a different installation method.\n\nWhen I last proposed it, #26018 solved the problem for me.",
    "created_at": "2020-02-12T17:30:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26990",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26990#issuecomment-375832",
    "user": "https://github.com/embray"
}
```

They are already installed with pip, and regardless I don't think that would particularly help.  I think it would be annoying to have some SPKGs that use a different installation method.

When I last proposed it, #26018 solved the problem for me.
