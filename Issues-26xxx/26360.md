# Issue 26360: Upgrade arb to 2.15.1

archive/issues_026123.json:
```json
{
    "assignees": [],
    "body": "Arb 2.15.1 is out and the upgrade breaks a lot (~200) doctests with changes that are not actually failures. Nearly all the changes are minor differences in the accuracy of the radius. As previously discussed in #25966, I think we should fix this once and for all. The tests are brittle and break with more or less every arb upgrade. All those \"failures\" distract from the actual failures. It makes arb upgrades painful and is a burden on distributions.\n\nI suspect the solution here (lots of `...`) will be controversial. I don't like it very much myself. But I think it is better than the status quo. What we really want are different tolerances for the \"mid\" and the \"radius\". I don't know if that is possible in the doctesting framework without explicitly testing for `.mid()` and `.radius()` in different tests each time. We may even want to test the `.accuracy()` instead of the radius.\n\nUpstream tarball: \u200bhttps://github.com/fredrik-johansson/arb/archive/2.15.1.tar.gz\n\nCC:  @fredrik-johansson @embray @mezzarobba @jdemeyer @kiwifb\n\nKeywords: **arb, upgrade**\n\nBranch/Commit: **[30cc778](https://github.com/sagemath/sagetrac-mirror/commit/30cc778d46579bd0c7537ed33e8d7a4f40fd5c31)**\n\nReviewer: **Marc Mezzarobba**\n\nAuthor: **Timo Kaufmann**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26360_\n\n",
    "closed_at": "2018-12-07T12:10:46Z",
    "created_at": "2018-09-28T15:16:48Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Upgrade arb to 2.15.1",
    "type": "issue",
    "updated_at": "2023-01-08T09:14:01Z",
    "url": "https://github.com/sagemath/sage/issues/26360",
    "user": "https://github.com/timokau"
}
```
Arb 2.15.1 is out and the upgrade breaks a lot (~200) doctests with changes that are not actually failures. Nearly all the changes are minor differences in the accuracy of the radius. As previously discussed in #25966, I think we should fix this once and for all. The tests are brittle and break with more or less every arb upgrade. All those "failures" distract from the actual failures. It makes arb upgrades painful and is a burden on distributions.

I suspect the solution here (lots of `...`) will be controversial. I don't like it very much myself. But I think it is better than the status quo. What we really want are different tolerances for the "mid" and the "radius". I don't know if that is possible in the doctesting framework without explicitly testing for `.mid()` and `.radius()` in different tests each time. We may even want to test the `.accuracy()` instead of the radius.

Upstream tarball: ​https://github.com/fredrik-johansson/arb/archive/2.15.1.tar.gz

CC:  @fredrik-johansson @embray @mezzarobba @jdemeyer @kiwifb

Keywords: **arb, upgrade**

Branch/Commit: **[30cc778](https://github.com/sagemath/sagetrac-mirror/commit/30cc778d46579bd0c7537ed33e8d7a4f40fd5c31)**

Reviewer: **Marc Mezzarobba**

Author: **Timo Kaufmann**

_Issue created by migration from https://trac.sagemath.org/ticket/26360_





---

archive/issue_events_345203.json:
```json
{
    "actor": "https://github.com/timokau",
    "created_at": "2018-09-28T15:16:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26360#event-345203"
}
```



---

archive/issue_events_345204.json:
```json
{
    "actor": "https://github.com/timokau",
    "created_at": "2018-09-28T15:16:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26360#event-345204"
}
```



---

archive/issue_events_345205.json:
```json
{
    "actor": "https://github.com/timokau",
    "created_at": "2018-09-28T15:16:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26360#event-345205"
}
```



---

archive/issue_events_345206.json:
```json
{
    "actor": "https://github.com/timokau",
    "created_at": "2018-09-28T15:16:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26360#event-345206"
}
```



---

archive/issue_comments_409222.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nIt would be nice to have some hook for doctests so that balls by default are compared in terms of overlapping and that the radii are the same within a factor 10, say.",
    "created_at": "2018-09-28T15:25:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409222",
    "user": "https://github.com/fredrik-johansson"
}
```

<div id="comment:1" align="right">Comment 1</div>

It would be nice to have some hook for doctests so that balls by default are compared in terms of overlapping and that the radii are the same within a factor 10, say.



---

archive/issue_comments_409223.json:
```json
{
    "body": "Replying to [ticket:26360 gh-timokau]:\n> I suspect the solution here (lots of `...`) will be controversial. I don't like it very much myself.\n\nI'm okay with it, but I'd like to see what Jeroen says since he insisted for exact doctests back then.\n\n> What we really want are different tolerances for the \"mid\" and the \"radius\".\n\nIs that really necessary? If you use absolute tolerances, having the same tolerance for the center and the radius isn't too bad.\n\nSomething like what Fredrik suggests would be better, of course. Since arb provides a way to parse strings of the form `[mid +/- rad]`, it is perhaps doable. (I'm not volunteering, though...)",
    "created_at": "2018-09-29T08:05:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409223",
    "user": "https://github.com/mezzarobba"
}
```

Replying to [ticket:26360 gh-timokau]:
> I suspect the solution here (lots of `...`) will be controversial. I don't like it very much myself.

I'm okay with it, but I'd like to see what Jeroen says since he insisted for exact doctests back then.

> What we really want are different tolerances for the "mid" and the "radius".

Is that really necessary? If you use absolute tolerances, having the same tolerance for the center and the radius isn't too bad.

Something like what Fredrik suggests would be better, of course. Since arb provides a way to parse strings of the form `[mid +/- rad]`, it is perhaps doable. (I'm not volunteering, though...)



---

archive/issue_comments_409224.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nReplying to [@mezzarobba](#comment%3A3):\n> Replying to [ticket:26360 gh-timokau]:\n> > I suspect the solution here (lots of `...`) will be controversial. I don't like it very much myself.\n\n> \n> I'm okay with it, but I'd like to see what Jeroen says since he insisted for exact doctests back then.\n\nJeroen, do you still insist on exact doctests here?",
    "created_at": "2018-10-20T21:24:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409224",
    "user": "https://github.com/timokau"
}
```

<div id="comment:4" align="right">Comment 4</div>

Replying to [@mezzarobba](#comment%3A3):
> Replying to [ticket:26360 gh-timokau]:
> > I suspect the solution here (lots of `...`) will be controversial. I don't like it very much myself.

> 
> I'm okay with it, but I'd like to see what Jeroen says since he insisted for exact doctests back then.

Jeroen, do you still insist on exact doctests here?



---

archive/issue_comments_409225.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\n> > What we really want are different tolerances for the \"mid\" and the \"radius\".\n\n> \n> Is that really necessary? If you use absolute tolerances, having the same tolerance for the center and the radius isn't too bad.\n\nThat might also a (while not pretty) workable solution.\n\n> Something like what Fredrik suggests would be better, of course. Since arb provides a way to parse strings of the form `[mid +/- rad]`, it is perhaps doable. (I'm not volunteering, though...)\n\nIs the doctesting framework even that flexible?",
    "created_at": "2018-10-20T21:25:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409225",
    "user": "https://github.com/timokau"
}
```

<div id="comment:5" align="right">Comment 5</div>

> > What we really want are different tolerances for the "mid" and the "radius".

> 
> Is that really necessary? If you use absolute tolerances, having the same tolerance for the center and the radius isn't too bad.

That might also a (while not pretty) workable solution.

> Something like what Fredrik suggests would be better, of course. Since arb provides a way to parse strings of the form `[mid +/- rad]`, it is perhaps doable. (I'm not volunteering, though...)

Is the doctesting framework even that flexible?



---

archive/issue_comments_409226.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nReplying to [@timokau](#comment%3A4):\n> Jeroen, do you still insist on exact doctests here?\n\nI don't recall why I originally insisted on that. So go ahead.",
    "created_at": "2018-10-21T08:30:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409226",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:6" align="right">Comment 6</div>

Replying to [@timokau](#comment%3A4):
> Jeroen, do you still insist on exact doctests here?

I don't recall why I originally insisted on that. So go ahead.



---

archive/issue_comments_409227.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nReplying to [@mezzarobba](#comment%3A3):\n> Something like what Fredrik suggests would be better, of course. Since arb provides a way to parse strings of the form `[mid +/- rad]`, it is perhaps doable. (I'm not volunteering, though...)\n\nI'm not convinced that we need to patch the doctest framework for that.",
    "created_at": "2018-10-21T08:40:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409227",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:7" align="right">Comment 7</div>

Replying to [@mezzarobba](#comment%3A3):
> Something like what Fredrik suggests would be better, of course. Since arb provides a way to parse strings of the form `[mid +/- rad]`, it is perhaps doable. (I'm not volunteering, though...)

I'm not convinced that we need to patch the doctest framework for that.



---

archive/issue_comments_409228.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nI can't really do much about the remaining failures, since I lack the subject knowledge to decide weather or not they are real errors:\n\n```\nsage -t --long src/sage/rings/complex_arb.pyx\n**********************************************************************\nFile \"src/sage/rings/complex_arb.pyx\", line 4403, in sage.rings.complex_arb.ComplexBall.elliptic_f\nFailed example:\n    (CBF.pi()/2).elliptic_f(phi)\nExpected:\n    [1.5092369540513 +/- ...e-14] + [0.6251464152027 +/- ...e-14]*I\nGot:\n    nan + nan*I\n**********************************************************************\nFile \"src/sage/rings/complex_arb.pyx\", line 4409, in sage.rings.complex_arb.ComplexBall.elliptic_f\nFailed example:\n    (CBF.pi()/2).elliptic_f(phi)\nExpected:\n    [1.3393589639094 +/- ...e-14] + [1.1104369690719 +/- ...e-14]*I\nGot:\n    nan + nan*I\n**********************************************************************\nFile \"src/sage/rings/complex_arb.pyx\", line 4441, in sage.rings.complex_arb.ComplexBall.elliptic_e_inc\nFailed example:\n    (CBF.pi()/2).elliptic_e_inc(phi)\nExpected:\n    [1.2838409578982 +/- ...e-14] + [-0.5317843366915 +/- ...e-14]*I\nGot:\n    nan + nan*I\n**********************************************************************\nFile \"src/sage/rings/complex_arb.pyx\", line 4447, in sage.rings.complex_arb.ComplexBall.elliptic_e_inc\nFailed example:\n    (CBF.pi()/2).elliptic_e_inc(phi)\nExpected:\n    [0.787564350925 +/- ...e-13] + [-0.686896129145 +/- ...e-13]*I\nGot:\n    nan + nan*I\n**********************************************************************\nFile \"src/sage/rings/complex_arb.pyx\", line 4481, in sage.rings.complex_arb.ComplexBall.elliptic_pi_inc\nFailed example:\n    n.elliptic_pi_inc(CBF.pi()/2, m)\nExpected:\n    [0.8934793755173 +/- ...e-14] + [0.95707868710750 +/- ...e-15]*I\nGot:\n    nan + nan*I\n**********************************************************************\nFile \"src/sage/rings/complex_arb.pyx\", line 4488, in sage.rings.complex_arb.ComplexBall.elliptic_pi_inc\nFailed example:\n    n.elliptic_pi_inc(CBF.pi()/2, m)\nExpected:\n    [0.2969588746419 +/- ...e-14] + [1.318879533274 +/- ...e-13]*I\nGot:\n    nan + nan*I\n**********************************************************************\n3 items had failures:\n   2 of   8 in sage.rings.complex_arb.ComplexBall.elliptic_e_inc\n   2 of   8 in sage.rings.complex_arb.ComplexBall.elliptic_f\n   2 of  10 in sage.rings.complex_arb.ComplexBall.elliptic_pi_inc\n    [622 tests, 6 failures, 11.97 s]\n```\n\n```\nsage -t --long src/sage/rings/real_arb.pyx\n**********************************************************************\nFile \"src/sage/rings/real_arb.pyx\", line 1305, in sage.rings.real_arb.RealBall.__init__\nFailed example:\n    RBF(\"2.3e10000000000000000000000 +/- ...e10000000000000000000000\")\nException raised:\n    Traceback (most recent call last):\n      File \"/home/timo/repos2/sage/local/lib/python2.7/site-packages/sage/doctest/forker.py\", line 659, in _run\n        self.compile_and_execute(example, compiler, test.globs)\n      File \"/home/timo/repos2/sage/local/lib/python2.7/site-packages/sage/doctest/forker.py\", line 1070, in compile_and_execute\n        exec(compiled, globs)\n      File \"<doctest sage.rings.real_arb.RealBall.__init__[35]>\", line 1, in <module>\n        RBF(\"2.3e10000000000000000000000 +/- ...e10000000000000000000000\")\n      File \"sage/structure/parent.pyx\", line 921, in sage.structure.parent.Parent.__call__ (build/cythonized/sage/structure/parent.c:9679)\n        return mor._call_(x)\n      File \"sage/structure/coerce_maps.pyx\", line 145, in sage.structure.coerce_maps.DefaultConvertMap_unique._call_ (build/cythonized/sage/structure/coerce_maps.c:4574)\n        raise\n      File \"sage/structure/coerce_maps.pyx\", line 140, in sage.structure.coerce_maps.DefaultConvertMap_unique._call_ (build/cythonized/sage/structure/coerce_maps.c:4462)\n        return C._element_constructor(x)\n      File \"sage/rings/real_arb.pyx\", line 512, in sage.rings.real_arb.RealBallField._element_constructor_ (build/cythonized/sage/rings/real_arb.c:5843)\n        return self.element_class(self, mid, rad)\n      File \"sage/rings/real_arb.pyx\", line 1376, in sage.rings.real_arb.RealBall.__init__ (build/cythonized/sage/rings/real_arb.c:13451)\n        raise ValueError(\"unsupported string format\")\n    ValueError: unsupported string format\n**********************************************************************\n1 item had failures:\n   1 of  48 in sage.rings.real_arb.RealBall.__init__\n    [530 tests, 1 failure, 0.78 s]\n```",
    "created_at": "2018-10-23T21:34:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409228",
    "user": "https://github.com/timokau"
}
```

<div id="comment:8" align="right">Comment 8</div>

I can't really do much about the remaining failures, since I lack the subject knowledge to decide weather or not they are real errors:

```
sage -t --long src/sage/rings/complex_arb.pyx
**********************************************************************
File "src/sage/rings/complex_arb.pyx", line 4403, in sage.rings.complex_arb.ComplexBall.elliptic_f
Failed example:
    (CBF.pi()/2).elliptic_f(phi)
Expected:
    [1.5092369540513 +/- ...e-14] + [0.6251464152027 +/- ...e-14]*I
Got:
    nan + nan*I
**********************************************************************
File "src/sage/rings/complex_arb.pyx", line 4409, in sage.rings.complex_arb.ComplexBall.elliptic_f
Failed example:
    (CBF.pi()/2).elliptic_f(phi)
Expected:
    [1.3393589639094 +/- ...e-14] + [1.1104369690719 +/- ...e-14]*I
Got:
    nan + nan*I
**********************************************************************
File "src/sage/rings/complex_arb.pyx", line 4441, in sage.rings.complex_arb.ComplexBall.elliptic_e_inc
Failed example:
    (CBF.pi()/2).elliptic_e_inc(phi)
Expected:
    [1.2838409578982 +/- ...e-14] + [-0.5317843366915 +/- ...e-14]*I
Got:
    nan + nan*I
**********************************************************************
File "src/sage/rings/complex_arb.pyx", line 4447, in sage.rings.complex_arb.ComplexBall.elliptic_e_inc
Failed example:
    (CBF.pi()/2).elliptic_e_inc(phi)
Expected:
    [0.787564350925 +/- ...e-13] + [-0.686896129145 +/- ...e-13]*I
Got:
    nan + nan*I
**********************************************************************
File "src/sage/rings/complex_arb.pyx", line 4481, in sage.rings.complex_arb.ComplexBall.elliptic_pi_inc
Failed example:
    n.elliptic_pi_inc(CBF.pi()/2, m)
Expected:
    [0.8934793755173 +/- ...e-14] + [0.95707868710750 +/- ...e-15]*I
Got:
    nan + nan*I
**********************************************************************
File "src/sage/rings/complex_arb.pyx", line 4488, in sage.rings.complex_arb.ComplexBall.elliptic_pi_inc
Failed example:
    n.elliptic_pi_inc(CBF.pi()/2, m)
Expected:
    [0.2969588746419 +/- ...e-14] + [1.318879533274 +/- ...e-13]*I
Got:
    nan + nan*I
**********************************************************************
3 items had failures:
   2 of   8 in sage.rings.complex_arb.ComplexBall.elliptic_e_inc
   2 of   8 in sage.rings.complex_arb.ComplexBall.elliptic_f
   2 of  10 in sage.rings.complex_arb.ComplexBall.elliptic_pi_inc
    [622 tests, 6 failures, 11.97 s]
```

```
sage -t --long src/sage/rings/real_arb.pyx
**********************************************************************
File "src/sage/rings/real_arb.pyx", line 1305, in sage.rings.real_arb.RealBall.__init__
Failed example:
    RBF("2.3e10000000000000000000000 +/- ...e10000000000000000000000")
Exception raised:
    Traceback (most recent call last):
      File "/home/timo/repos2/sage/local/lib/python2.7/site-packages/sage/doctest/forker.py", line 659, in _run
        self.compile_and_execute(example, compiler, test.globs)
      File "/home/timo/repos2/sage/local/lib/python2.7/site-packages/sage/doctest/forker.py", line 1070, in compile_and_execute
        exec(compiled, globs)
      File "<doctest sage.rings.real_arb.RealBall.__init__[35]>", line 1, in <module>
        RBF("2.3e10000000000000000000000 +/- ...e10000000000000000000000")
      File "sage/structure/parent.pyx", line 921, in sage.structure.parent.Parent.__call__ (build/cythonized/sage/structure/parent.c:9679)
        return mor._call_(x)
      File "sage/structure/coerce_maps.pyx", line 145, in sage.structure.coerce_maps.DefaultConvertMap_unique._call_ (build/cythonized/sage/structure/coerce_maps.c:4574)
        raise
      File "sage/structure/coerce_maps.pyx", line 140, in sage.structure.coerce_maps.DefaultConvertMap_unique._call_ (build/cythonized/sage/structure/coerce_maps.c:4462)
        return C._element_constructor(x)
      File "sage/rings/real_arb.pyx", line 512, in sage.rings.real_arb.RealBallField._element_constructor_ (build/cythonized/sage/rings/real_arb.c:5843)
        return self.element_class(self, mid, rad)
      File "sage/rings/real_arb.pyx", line 1376, in sage.rings.real_arb.RealBall.__init__ (build/cythonized/sage/rings/real_arb.c:13451)
        raise ValueError("unsupported string format")
    ValueError: unsupported string format
**********************************************************************
1 item had failures:
   1 of  48 in sage.rings.real_arb.RealBall.__init__
    [530 tests, 1 failure, 0.78 s]
```



---

archive/issue_comments_409229.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nThat's a genuine regression in Arb 2.15. I have not tried to debug it, but quite likely the original code was buggy and only worked by accident -- it works if you add CBF(\"+/- 1e-10\") to pi/2.",
    "created_at": "2018-10-24T13:38:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409229",
    "user": "https://github.com/fredrik-johansson"
}
```

<div id="comment:9" align="right">Comment 9</div>

That's a genuine regression in Arb 2.15. I have not tried to debug it, but quite likely the original code was buggy and only worked by accident -- it works if you add CBF("+/- 1e-10") to pi/2.



---

archive/issue_comments_409230.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\n> it works if you add CBF(\"+/- 1e-10\") to pi/2\n\nSo should we just do that in the doctests?",
    "created_at": "2018-10-24T13:42:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409230",
    "user": "https://github.com/timokau"
}
```

<div id="comment:10" align="right">Comment 10</div>

> it works if you add CBF("+/- 1e-10") to pi/2

So should we just do that in the doctests?



---

archive/issue_comments_409231.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nThis commit should provide a bugfix: https://github.com/fredrik-johansson/arb/commit/eb60d7ac4ee85cd6f8353dd9cf52a10bfd5967c3\n\nThe other failure (\"unsupported string format\") is due to too greedy search & replace to insert ...",
    "created_at": "2018-10-24T20:38:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409231",
    "user": "https://github.com/fredrik-johansson"
}
```

<div id="comment:11" align="right">Comment 11</div>

This commit should provide a bugfix: https://github.com/fredrik-johansson/arb/commit/eb60d7ac4ee85cd6f8353dd9cf52a10bfd5967c3

The other failure ("unsupported string format") is due to too greedy search & replace to insert ...



---

archive/issue_comments_409232.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nDo you plan to make a release with that fix included anytime soon?",
    "created_at": "2018-10-24T21:26:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409232",
    "user": "https://github.com/timokau"
}
```

<div id="comment:12" align="right">Comment 12</div>

Do you plan to make a release with that fix included anytime soon?



---

archive/issue_comments_409233.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nDone. https://github.com/fredrik-johansson/arb/archive/2.15.1.tar.gz",
    "created_at": "2018-10-25T19:15:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409233",
    "user": "https://github.com/fredrik-johansson"
}
```

<div id="comment:13" align="right">Comment 13</div>

Done. https://github.com/fredrik-johansson/arb/archive/2.15.1.tar.gz



---

archive/issue_comments_409234.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nThank you! Could you expand on what you mean by \"due to too greedy search & replace to insert\"? As far as I can see it is caused by `arb_set_str` returning false.",
    "created_at": "2018-10-26T16:19:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409234",
    "user": "https://github.com/timokau"
}
```

<div id="comment:14" align="right">Comment 14</div>

Thank you! Could you expand on what you mean by "due to too greedy search & replace to insert"? As far as I can see it is caused by `arb_set_str` returning false.



---

archive/issue_comments_409235.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">Comment 15</div>\n\nThere is \"...\" in the input string which shouldn't have been inserted there in place of the numerical value.",
    "created_at": "2018-10-26T16:21:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409235",
    "user": "https://github.com/fredrik-johansson"
}
```

<div id="comment:15" align="right">Comment 15</div>

There is "..." in the input string which shouldn't have been inserted there in place of the numerical value.



---

archive/issue_comments_409236.json:
```json
{
    "body": "Changed commit from **[00d3ccd](https://github.com/sagemath/sagetrac-mirror/commit/00d3ccd2ee230d9252b9cdc3183adf9c7f9f45ad)** to **[30cc778](https://github.com/sagemath/sagetrac-mirror/commit/30cc778d46579bd0c7537ed33e8d7a4f40fd5c31)**",
    "created_at": "2018-10-27T07:42:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409236",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[00d3ccd](https://github.com/sagemath/sagetrac-mirror/commit/00d3ccd2ee230d9252b9cdc3183adf9c7f9f45ad)** to **[30cc778](https://github.com/sagemath/sagetrac-mirror/commit/30cc778d46579bd0c7537ed33e8d7a4f40fd5c31)**



---

archive/issue_comments_409237.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/30cc778d46579bd0c7537ed33e8d7a4f40fd5c31\">30cc778</a></td><td><code>Upgrade arb to 2.15.1</code></td></tr></table>\n",
    "created_at": "2018-10-27T07:42:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409237",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:16" align="right">Comment 16</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/30cc778d46579bd0c7537ed33e8d7a4f40fd5c31">30cc778</a></td><td><code>Upgrade arb to 2.15.1</code></td></tr></table>




---

archive/issue_events_345207.json:
```json
{
    "actor": "https://github.com/timokau",
    "created_at": "2018-10-27T07:44:19Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "title_is": "Upgrade arb to 2.15.1",
    "title_was": "Upgrade arb to 2.15.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26360#event-345207"
}
```



---

archive/issue_events_345208.json:
```json
{
    "actor": "https://github.com/timokau",
    "created_at": "2018-10-27T07:44:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26360#event-345208"
}
```



---

archive/issue_comments_409238.json:
```json
{
    "body": "<div id=\"comment:17\" align=\"right\">Comment 17</div>\n\nOh, stupid mistake. Thanks.\n\nDoctests pass now.",
    "created_at": "2018-10-27T07:44:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409238",
    "user": "https://github.com/timokau"
}
```

<div id="comment:17" align="right">Comment 17</div>

Oh, stupid mistake. Thanks.

Doctests pass now.



---

archive/issue_comments_409239.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,5 +1,5 @@\n-Arb 2.15.0 is out and the upgrade breaks a lot (~200) doctests with changes that are not actually failures. Nearly all the changes are minor differences in the accuracy of the radius. As previously discussed in #25966, I think we should fix this once and for all. The tests are brittle and break with more or less every arb upgrade. All those \"failures\" distract from the actual failures. It makes arb upgrades painful and is a burden on distributions.\n+Arb 2.15.1 is out and the upgrade breaks a lot (~200) doctests with changes that are not actually failures. Nearly all the changes are minor differences in the accuracy of the radius. As previously discussed in #25966, I think we should fix this once and for all. The tests are brittle and break with more or less every arb upgrade. All those \"failures\" distract from the actual failures. It makes arb upgrades painful and is a burden on distributions.\n \n I suspect the solution here (lots of `...`) will be controversial. I don't like it very much myself. But I think it is better than the status quo. What we really want are different tolerances for the \"mid\" and the \"radius\". I don't know if that is possible in the doctesting framework without explicitly testing for `.mid()` and `.radius()` in different tests each time. We may even want to test the `.accuracy()` instead of the radius.\n \n-Upstream tarball: \u200bhttps://github.com/fredrik-johansson/arb/archive/2.15.0.tar.gz\n+Upstream tarball: \u200bhttps://github.com/fredrik-johansson/arb/archive/2.15.1.tar.gz\n``````\n",
    "created_at": "2018-10-27T07:44:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409239",
    "user": "https://github.com/timokau"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,5 +1,5 @@
-Arb 2.15.0 is out and the upgrade breaks a lot (~200) doctests with changes that are not actually failures. Nearly all the changes are minor differences in the accuracy of the radius. As previously discussed in #25966, I think we should fix this once and for all. The tests are brittle and break with more or less every arb upgrade. All those "failures" distract from the actual failures. It makes arb upgrades painful and is a burden on distributions.
+Arb 2.15.1 is out and the upgrade breaks a lot (~200) doctests with changes that are not actually failures. Nearly all the changes are minor differences in the accuracy of the radius. As previously discussed in #25966, I think we should fix this once and for all. The tests are brittle and break with more or less every arb upgrade. All those "failures" distract from the actual failures. It makes arb upgrades painful and is a burden on distributions.
 
 I suspect the solution here (lots of `...`) will be controversial. I don't like it very much myself. But I think it is better than the status quo. What we really want are different tolerances for the "mid" and the "radius". I don't know if that is possible in the doctesting framework without explicitly testing for `.mid()` and `.radius()` in different tests each time. We may even want to test the `.accuracy()` instead of the radius.
 
-Upstream tarball: ​https://github.com/fredrik-johansson/arb/archive/2.15.0.tar.gz
+Upstream tarball: ​https://github.com/fredrik-johansson/arb/archive/2.15.1.tar.gz
``````




---

archive/issue_comments_409240.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">Comment 18</div>\n\nI think rather than so many `...`, if there are changes we can make to the doctest parser to make these tests a little more natural that would be better, and probably not a big deal...",
    "created_at": "2018-10-28T11:54:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409240",
    "user": "https://github.com/embray"
}
```

<div id="comment:18" align="right">Comment 18</div>

I think rather than so many `...`, if there are changes we can make to the doctest parser to make these tests a little more natural that would be better, and probably not a big deal...



---

archive/issue_comments_409241.json:
```json
{
    "body": "<div id=\"comment:19\" align=\"right\">Comment 19</div>\n\nI wouldn't know how to do that.",
    "created_at": "2018-10-28T12:35:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409241",
    "user": "https://github.com/timokau"
}
```

<div id="comment:19" align="right">Comment 19</div>

I wouldn't know how to do that.



---

archive/issue_comments_409242.json:
```json
{
    "body": "<div id=\"comment:20\" align=\"right\">Comment 20</div>\n\nI mean, I would, but I don't know exactly what it is you need in this case.",
    "created_at": "2018-10-28T12:55:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409242",
    "user": "https://github.com/embray"
}
```

<div id="comment:20" align="right">Comment 20</div>

I mean, I would, but I don't know exactly what it is you need in this case.



---

archive/issue_comments_409243.json:
```json
{
    "body": "<div id=\"comment:21\" align=\"right\">Comment 21</div>\n\nReplying to [@timokau](#comment%3A5):\n> > > What we really want are different tolerances for the \"mid\" and the \"radius\".\n\n> > \n> > Is that really necessary? If you use absolute tolerances, having the same tolerance for the center and the radius isn't too bad.\n\n> \n> That might also a (while not pretty) workable solution.\n\nDid you ever try this?  I don't really see it as \"pretty\" or not \"pretty\".  An `# abs tol` will apply to both values, but we really only care about it in this case for the radius.  Technically it would be applied to the mid too but it doesn't matter either way for the present purpose.",
    "created_at": "2018-10-28T12:59:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409243",
    "user": "https://github.com/embray"
}
```

<div id="comment:21" align="right">Comment 21</div>

Replying to [@timokau](#comment%3A5):
> > > What we really want are different tolerances for the "mid" and the "radius".

> > 
> > Is that really necessary? If you use absolute tolerances, having the same tolerance for the center and the radius isn't too bad.

> 
> That might also a (while not pretty) workable solution.

Did you ever try this?  I don't really see it as "pretty" or not "pretty".  An `# abs tol` will apply to both values, but we really only care about it in this case for the radius.  Technically it would be applied to the mid too but it doesn't matter either way for the present purpose.



---

archive/issue_comments_409244.json:
```json
{
    "body": "<div id=\"comment:22\" align=\"right\">Comment 22</div>\n\nReplying to [@embray](#comment%3A20):\n> I mean, I would, but I don't know exactly what it is you need in this case.\n\nWe'd need to use proper comparison methods instead of just comparing the digits. Something like what Fredrik [said](#comment%3A1).\n\nReplying to [@embray](#comment%3A21):\n> Replying to [@timokau](#comment%3A5):\n> > > > What we really want are different tolerances for the \"mid\" and the \"radius\".\n\n> > > \n> > > Is that really necessary? If you use absolute tolerances, having the same tolerance for the center and the radius isn't too bad.\n\n> > \n> > That might also a (while not pretty) workable solution.\n\n> \n> Did you ever try this?  I don't really see it as \"pretty\" or not \"pretty\".  An `# abs tol` will apply to both values, but we really only care about it in this case for the radius.  Technically it would be applied to the mid too but it doesn't matter either way for the present purpose.\n\nNo I haven't. It would require either a lot of manual adding of `# abs tol` or a very clever sed script. It doesn't have high enough priority for me right now to spend the time doing that, but I'd be happy with the solution if somebody else would.",
    "created_at": "2018-10-29T08:47:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409244",
    "user": "https://github.com/timokau"
}
```

<div id="comment:22" align="right">Comment 22</div>

Replying to [@embray](#comment%3A20):
> I mean, I would, but I don't know exactly what it is you need in this case.

We'd need to use proper comparison methods instead of just comparing the digits. Something like what Fredrik [said](#comment%3A1).

Replying to [@embray](#comment%3A21):
> Replying to [@timokau](#comment%3A5):
> > > > What we really want are different tolerances for the "mid" and the "radius".

> > > 
> > > Is that really necessary? If you use absolute tolerances, having the same tolerance for the center and the radius isn't too bad.

> > 
> > That might also a (while not pretty) workable solution.

> 
> Did you ever try this?  I don't really see it as "pretty" or not "pretty".  An `# abs tol` will apply to both values, but we really only care about it in this case for the radius.  Technically it would be applied to the mid too but it doesn't matter either way for the present purpose.

No I haven't. It would require either a lot of manual adding of `# abs tol` or a very clever sed script. It doesn't have high enough priority for me right now to spend the time doing that, but I'd be happy with the solution if somebody else would.



---

archive/issue_comments_409245.json:
```json
{
    "body": "<div id=\"comment:23\" align=\"right\">Comment 23</div>\n\nI'll give it a try and see.  I'm pretty good at ridiculous regular expressions :)",
    "created_at": "2018-10-31T13:21:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409245",
    "user": "https://github.com/embray"
}
```

<div id="comment:23" align="right">Comment 23</div>

I'll give it a try and see.  I'm pretty good at ridiculous regular expressions :)



---

archive/issue_comments_409246.json:
```json
{
    "body": "<div id=\"comment:24\" align=\"right\">Comment 24</div>\n\nI suggest merging this as-is. The doctest improvement would be nice to have but it's not worth delaying the upgrade over it.",
    "created_at": "2018-11-06T09:47:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409246",
    "user": "https://github.com/fredrik-johansson"
}
```

<div id="comment:24" align="right">Comment 24</div>

I suggest merging this as-is. The doctest improvement would be nice to have but it's not worth delaying the upgrade over it.



---

archive/issue_comments_409247.json:
```json
{
    "body": "<div id=\"comment:25\" align=\"right\">Comment 25</div>\n\n@fredrik-johansson was that a review? I'm guessing @embray has different priorities right now, would be nice to get this into the next release.",
    "created_at": "2018-11-26T21:29:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409247",
    "user": "https://github.com/timokau"
}
```

<div id="comment:25" align="right">Comment 25</div>

@fredrik-johansson was that a review? I'm guessing @embray has different priorities right now, would be nice to get this into the next release.



---

archive/issue_comments_409248.json:
```json
{
    "body": "<div id=\"comment:26\" align=\"right\">Comment 26</div>\n\nIt's true I have different priorities.  I'm pretty sad about this doctest issue but not enough to do anything about it *right now* so I don't think this should be held up over it.  I'll open a ticket though.",
    "created_at": "2018-11-27T10:21:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409248",
    "user": "https://github.com/embray"
}
```

<div id="comment:26" align="right">Comment 26</div>

It's true I have different priorities.  I'm pretty sad about this doctest issue but not enough to do anything about it *right now* so I don't think this should be held up over it.  I'll open a ticket though.



---

archive/issue_comments_409249.json:
```json
{
    "body": "<div id=\"comment:27\" align=\"right\">Comment 27</div>\n\nSee #26774 with an idea for a generalized solution.",
    "created_at": "2018-11-27T10:28:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409249",
    "user": "https://github.com/embray"
}
```

<div id="comment:27" align="right">Comment 27</div>

See #26774 with an idea for a generalized solution.



---

archive/issue_comments_409250.json:
```json
{
    "body": "<div id=\"comment:28\" align=\"right\">Comment 28</div>\n\n#26774 would be really nice. In addition to arb I'm sure there are a lot of other areas where the doctests could be improved. That'll take a while however.\n\nLets get this merged for now.",
    "created_at": "2018-11-27T14:06:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409250",
    "user": "https://github.com/timokau"
}
```

<div id="comment:28" align="right">Comment 28</div>

#26774 would be really nice. In addition to arb I'm sure there are a lot of other areas where the doctests could be improved. That'll take a while however.

Lets get this merged for now.



---

archive/issue_comments_409251.json:
```json
{
    "body": "<div id=\"comment:29\" align=\"right\">Comment 29</div>\n\nReplying to [@timokau](#comment%3A28):\n> #26774 would be really nice. In addition to arb I'm sure there are a lot of other areas where the doctests could be improved. That'll take a while however.\n> \n> Lets get this merged for now.\n\nI agree.",
    "created_at": "2018-12-05T12:45:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409251",
    "user": "https://github.com/mezzarobba"
}
```

<div id="comment:29" align="right">Comment 29</div>

Replying to [@timokau](#comment%3A28):
> #26774 would be really nice. In addition to arb I'm sure there are a lot of other areas where the doctests could be improved. That'll take a while however.
> 
> Lets get this merged for now.

I agree.



---

archive/issue_events_345209.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2018-12-05T12:45:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26360#event-345209"
}
```



---

archive/issue_events_345210.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2018-12-05T12:45:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26360#event-345210"
}
```



---

archive/issue_comments_409252.json:
```json
{
    "body": "Reviewer: **Marc Mezzarobba**",
    "created_at": "2018-12-05T12:45:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409252",
    "user": "https://github.com/mezzarobba"
}
```

Reviewer: **Marc Mezzarobba**



---

archive/issue_events_345211.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-12-07T12:10:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26360#event-345211"
}
```



---

archive/issue_events_345212.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "5205bbcc097e85e181886b5bf9d05a281d66531f",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-12-07T12:10:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26360#event-345212"
}
```



---

archive/issue_comments_409253.json:
```json
{
    "body": "Changed branch from **[u/gh-timokau/arb-2.15.0](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-timokau/arb-2.15.0)** to **[30cc778](https://github.com/sagemath/sagetrac-mirror/commit/30cc778d46579bd0c7537ed33e8d7a4f40fd5c31)**",
    "created_at": "2018-12-07T12:10:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26360",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26360#issuecomment-409253",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/gh-timokau/arb-2.15.0](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-timokau/arb-2.15.0)** to **[30cc778](https://github.com/sagemath/sagetrac-mirror/commit/30cc778d46579bd0c7537ed33e8d7a4f40fd5c31)**
