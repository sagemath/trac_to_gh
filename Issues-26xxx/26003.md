# Issue 26003: Documentation of cvxopt

archive/issues_025766.json:
```json
{
    "assignees": [],
    "body": "I just tried to build the current beta and had `SAGE_SPKG_INSTALL_DOCS=yes`. cvxopt failed because\n\n```\ncp: cannot stat 'html/*': No such file or directory\n```\n\nI am not sure if the component should be \"documentation\" or \"packages: standard\"\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26003_\n\n",
    "created_at": "2018-08-05T14:54:10Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Documentation of cvxopt",
    "type": "issue",
    "updated_at": "2022-12-29T01:42:26Z",
    "url": "https://github.com/sagemath/sage/issues/26003",
    "user": "https://github.com/simon-king-jena"
}
```
I just tried to build the current beta and had `SAGE_SPKG_INSTALL_DOCS=yes`. cvxopt failed because

```
cp: cannot stat 'html/*': No such file or directory
```

I am not sure if the component should be "documentation" or "packages: standard"

_Issue created by migration from https://trac.sagemath.org/ticket/26003_





---

archive/issue_events_341139.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2018-08-05T14:54:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26003",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26003#event-341139"
}
```



---

archive/issue_events_341140.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2018-08-05T14:54:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26003",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26003#event-341140"
}
```



---

archive/issue_events_341141.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2018-08-05T14:54:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26003",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26003#event-341141"
}
```



---

archive/issue_events_341142.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2018-08-05T14:54:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26003",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26003#event-341142"
}
```



---

archive/issue_comments_403365.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nit used to work. Probably messed up during upgrades to use scripts in spkg-install...\nI'm having a look now.",
    "created_at": "2018-10-17T16:07:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26003",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26003#issuecomment-403365",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:1" align="right">Comment 1</div>

it used to work. Probably messed up during upgrades to use scripts in spkg-install...
I'm having a look now.



---

archive/issue_comments_403366.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nno, sorry, it wasn't completely working, due to MathJax issues.\nSee #14645.",
    "created_at": "2018-10-17T16:16:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26003",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26003#issuecomment-403366",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:2" align="right">Comment 2</div>

no, sorry, it wasn't completely working, due to MathJax issues.
See #14645.



---

archive/issue_events_341143.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:42:26Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/26003",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26003#event-341143"
}
```
