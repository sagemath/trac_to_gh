...
```json
{
    "body": "I don't know if PARI uses openblas in its multi-threaded mode but I wonder if this is related to #26585",
...
```json
{
    "body": "Note that the code excerpts in the last lines of the backtrace are nonsense since I was compiling an older version of the docs. Here's the \"translated\" version:\n\n\n```\n  File \"sage/misc/cachefunc.pyx\", line 1005, in sage.misc.cachefunc.CachedFunction.__call__ (build/cythonized/sage/misc/cachefunc.c:6065)\n    w = self.f(*args, **kwds)\n  File \"/usr/lib/python2.7/site-packages/sage/structure/unique_representation.py\", line 1027, in __classcall__\n    instance = typecall(cls, *args, **options)\n  File \"sage/misc/classcall_metaclass.pyx\", line 496, in sage.misc.classcall_metaclass.typecall (build/cythonized/sage/misc/classcall_metaclass.c:2148)\n    return (<PyTypeObject*>type).tp_call(cls, args, kwds)\n  File \"/usr/lib/python2.7/site-packages/sage/geometry/triangulation/point_configuration.py\", line 367, in __init__\n    PointConfiguration_base.__init__(self, points, defined_affine)\n  File \"sage/geometry/triangulation/base.pyx\", line 398, in sage.geometry.triangulation.base.PointConfiguration_base.__init__ (build/cythonized/sage/geometry/triangulation/base.cpp:4135)\n    self._init_points(points)\n  File \"sage/geometry/triangulation/base.pyx\", line 456, in sage.geometry.triangulation.base.PointConfiguration_base._init_points (build/cythonized/sage/geometry/triangulation/base.cpp:4982)\n    red = matrix([ red.row(i) for i in red.pivot_rows()])\n  File \"sage/matrix/matrix2.pyx\", line 517, in sage.matrix.matrix2.Matrix.pivot_rows (build/cythonized/sage/matrix/matrix2.c:8414)\n    v = self.transpose().pivots()\n  File \"sage/matrix/matrix_integer_dense.pyx\", line 2217, in sage.matrix.matrix_integer_dense.Matrix_integer_dense.pivots (build/cythonized/sage/matrix/matrix_integer_dense.c:19162)\n    E = self.echelon_form()\n  File \"sage/matrix/matrix_integer_dense.pyx\", line 2019, in sage.matrix.matrix_integer_dense.Matrix_integer_dense.echelon_form (build/cythonized/sage/matrix/matrix_integer_dense.c:17749)\n    H_m = self._hnf_pari(flag, include_zero_rows=include_zero_rows)\n  File \"sage/matrix/matrix_integer_dense.pyx\", line 5719, in sage.matrix.matrix_integer_dense.Matrix_integer_dense._hnf_pari (build/cythonized/sage/matrix/matrix_integer_dense.c:46635)\n    sig_on()\nSignalError: Segmentation fault \n```",
...
```json
{
    "body": "I'm not sure if it does, but it might.  I grepped the pari/gp source and it doesn't use `openblas_set_num_threads` directly, but something else might be.  I did see some reference to `omp_set_num_threads` but I don't think we compile with OpenMP by default in Sage.",
...
```json
{
    "body": "There's also some multi-threading support in FLINT which could be problematic, but I have no idea if that's relevant in this case.",
...
```json
{
    "body": "I read on the mailing list post \"It is called indirectly via matplotlib when rendering plots, see full backtrace below (btw, I had to downgrade to an old version of Sage to get a meaningful backtrace - I really dislike this trend of hiding build output, it makes it very hard to debug stuff)\"\n\nI had a very similar problem to this; it actually came from the BLAS library by way of a Numpy ufunc (I think for the \"dot\" product of a matrix in a vector, or two matrices).  I feel like I actually fixed this but now I can't remember.",
...
```json
{
    "body": "Do you know some direct, specific way to reproduce this so that I can try it?",
...
```json
{
    "body": "You know what though--I'm looking at the relevant code in openblas, and `openblas_set_num_threads(1)` might actually cause it to spin up a single thread (beyond the main thread) which is actually good enough to invoke the bug I fixed.  I'm going to try to confirm that though.",
...
```json
{
    "body": "Nevermind; that does not appear to be the case, I don't think.",
...
```json
{
    "body": "ISTM PARI/GP is not even built with multi-threading enabled unless you run its `Configure` with either `--mt=pthread` or `--mt=mpi`.  On my system anyways this is not done by default...",
...
```json
{
    "body": "*Now* if I build PARI with `--mt=pthread` I can make a child process segfault if it tries to do some multi-threaded work.  Granted, this is partly with my own *bad* code which does some things improperly.  Now I'd be curious if\n\na) Anyone getting having this problem is building PARI with `--mt=pthread` and\n\nb) Exactly what code is being run in Sage that invokes multi-threading in PARI.",
...
```json
{
    "body": "I can also make it deadlock with the right combination of evil calls.",
...
```json
{
    "body": "Even when you explicitly set `nthreads` to `1` before going forth with that evilness?",
...
```json
{
    "body": "Okay, glad you're feeling better :)\n\nLet me know if I can help.",
...
```json
{
    "body": "Totally forgot about this...",
    "created_at": "2019-01-18T13:51:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26608",
    "type": "issue_comment",
...
```json
{
    "body": "Too bad, I also forgot about this. I'm literally right now returning from a week-long [PARI/GP workshop](https://pari.math.u-bordeaux.fr/Events/PARI2019/) where I could have discussed this.",
...
```json
{
    "body": "The alternative multiprocess doc build introduced in #27490 works as a temporary workaround. I just replaced the revert with a one-line patch to enable it unconditionally  for 8.7",
...
```json
{
    "body": "see also #28242, where we enable system Pari in vanilla Sage.",
...
```json
{
    "body": "If #26002 has any effect, the pool should have just a single thread, no?",
...
```json
{
    "body": "In further digging, yes, I just found #26002 and that indeed pari has `pari_mt_nbthreads = 1` so maybe I'm barking up the wrong tree.  The `polint_chinese` call indeed does not use any threads in this case.",
...
```json
{
    "body": "hmm, maybe it still does something with thread local variables?",
...
```json
{
    "body": "Did somebody check that this problem is limited to docbuilds? Does the Sage testsuite pass (apart from doc-related tests of course)?",
...
```json
{
    "body": "Closing this ticket for now, since the immediate problem (Sage's docbuild) has been worked around by #28356.  The broader issue still should have a ticket to track it, even if it won't be resolved any time soon, so I have opened #28800.",
