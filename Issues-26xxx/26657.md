# Issue 26657: Adapt unitary burau matrix to Sage convention

archive/issues_026420.json:
```json
{
    "assignees": [],
    "body": "The unitary Burau matrix has been implemented in #25760. The choice between the two adjoined possibilities has been made according to the used literature ([Squ1984]_).\n\nUnfortunately, this is opposite to the convention used in Sage for example in :meth:`_check_matrix` of :class:`UnitaryMatrixGroup_generic`.\n\nThis discrepancy is corrected by the ticket. A deprecation of the former behavior doesn't seem to be necessary, since the new functionality has been released just a couple of months ago.\n\n\n\nCC:  @tscrim\n\nKeywords: **braid, burau, unitary**\n\nBranch/Commit: **[a05e3e4](https://github.com/sagemath/sagetrac-mirror/commit/a05e3e48f56bedf5d19d4ecca8ccf698b7907629)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Sebastian Oehms**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26657_\n\n",
    "closed_at": "2018-12-05T23:56:17Z",
    "created_at": "2018-11-06T22:01:17Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20group%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Adapt unitary burau matrix to Sage convention",
    "type": "issue",
    "updated_at": "2018-12-05T23:56:17Z",
    "url": "https://github.com/sagemath/sage/issues/26657",
    "user": "https://github.com/soehms"
}
```
The unitary Burau matrix has been implemented in #25760. The choice between the two adjoined possibilities has been made according to the used literature ([Squ1984]_).

Unfortunately, this is opposite to the convention used in Sage for example in :meth:`_check_matrix` of :class:`UnitaryMatrixGroup_generic`.

This discrepancy is corrected by the ticket. A deprecation of the former behavior doesn't seem to be necessary, since the new functionality has been released just a couple of months ago.



CC:  @tscrim

Keywords: **braid, burau, unitary**

Branch/Commit: **[a05e3e4](https://github.com/sagemath/sagetrac-mirror/commit/a05e3e48f56bedf5d19d4ecca8ccf698b7907629)**

Reviewer: **Travis Scrimshaw**

Author: **Sebastian Oehms**

_Issue created by migration from https://trac.sagemath.org/ticket/26657_





---

archive/issue_events_348638.json:
```json
{
    "actor": "https://github.com/soehms",
    "created_at": "2018-11-06T22:01:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26657",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26657#event-348638"
}
```



---

archive/issue_events_348639.json:
```json
{
    "actor": "https://github.com/soehms",
    "created_at": "2018-11-06T22:01:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26657",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20group%20theory",
    "label_color": "0000ff",
    "label_name": "component: group theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26657#event-348639"
}
```



---

archive/issue_events_348640.json:
```json
{
    "actor": "https://github.com/soehms",
    "created_at": "2018-11-06T22:01:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26657",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26657#event-348640"
}
```



---

archive/issue_events_348641.json:
```json
{
    "actor": "https://github.com/soehms",
    "created_at": "2018-11-06T22:01:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26657",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26657#event-348641"
}
```



---

archive/issue_comments_413647.json:
```json
{
    "body": "Branch: **[u/soehms/adapt_unitary_burau-26657](https://github.com/sagemath/sagetrac-mirror/tree/u/soehms/adapt_unitary_burau-26657)**",
    "created_at": "2018-11-06T22:09:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26657",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26657#issuecomment-413647",
    "user": "https://github.com/soehms"
}
```

Branch: **[u/soehms/adapt_unitary_burau-26657](https://github.com/sagemath/sagetrac-mirror/tree/u/soehms/adapt_unitary_burau-26657)**



---

archive/issue_comments_413648.json:
```json
{
    "body": "Commit: **[be4674b](https://github.com/sagemath/sagetrac-mirror/commit/be4674b23482a84ff45fbf6f6f6cfede8eab0a9f)**",
    "created_at": "2018-11-06T22:10:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26657",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26657#issuecomment-413648",
    "user": "https://github.com/soehms"
}
```

Commit: **[be4674b](https://github.com/sagemath/sagetrac-mirror/commit/be4674b23482a84ff45fbf6f6f6cfede8eab0a9f)**



---

archive/issue_comments_413649.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/be4674b23482a84ff45fbf6f6f6cfede8eab0a9f\">be4674b</a></td><td><code>initial implementation</code></td></tr></table>\n",
    "created_at": "2018-11-06T22:10:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26657",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26657#issuecomment-413649",
    "user": "https://github.com/soehms"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/be4674b23482a84ff45fbf6f6f6cfede8eab0a9f">be4674b</a></td><td><code>initial implementation</code></td></tr></table>




---

archive/issue_events_348642.json:
```json
{
    "actor": "https://github.com/soehms",
    "created_at": "2018-11-06T22:10:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26657",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26657#event-348642"
}
```



---

archive/issue_comments_413650.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nSorry, I lost track of this ticket. LGTM.",
    "created_at": "2018-12-02T06:16:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26657",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26657#issuecomment-413650",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:3" align="right">Comment 3</div>

Sorry, I lost track of this ticket. LGTM.



---

archive/issue_events_348643.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-12-02T06:16:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26657",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26657#event-348643"
}
```



---

archive/issue_events_348644.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-12-02T06:16:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26657",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26657#event-348644"
}
```



---

archive/issue_comments_413651.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2018-12-02T06:16:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26657",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26657#issuecomment-413651",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_348645.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-12-02T06:17:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26657",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26657#event-348645"
}
```



---

archive/issue_events_348646.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-12-02T06:17:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26657",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26657#event-348646"
}
```



---

archive/issue_comments_413652.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nOne little fix needed: <code>:trac:\\`25760</code> -> <code>:trac:\\`25760\\`</code>. Once done, you can set to a positive review.",
    "created_at": "2018-12-02T06:17:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26657",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26657#issuecomment-413652",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:4" align="right">Comment 4</div>

One little fix needed: <code>:trac:\`25760</code> -> <code>:trac:\`25760\`</code>. Once done, you can set to a positive review.



---

archive/issue_comments_413653.json:
```json
{
    "body": "Changed branch from **[u/soehms/adapt_unitary_burau-26657](https://github.com/sagemath/sagetrac-mirror/tree/u/soehms/adapt_unitary_burau-26657)** to **[u/soehms/adapt_unitary_burau_26657](https://github.com/sagemath/sagetrac-mirror/tree/u/soehms/adapt_unitary_burau_26657)**",
    "created_at": "2018-12-02T20:05:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26657",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26657#issuecomment-413653",
    "user": "https://github.com/soehms"
}
```

Changed branch from **[u/soehms/adapt_unitary_burau-26657](https://github.com/sagemath/sagetrac-mirror/tree/u/soehms/adapt_unitary_burau-26657)** to **[u/soehms/adapt_unitary_burau_26657](https://github.com/sagemath/sagetrac-mirror/tree/u/soehms/adapt_unitary_burau_26657)**



---

archive/issue_comments_413654.json:
```json
{
    "body": "Changed commit from **[be4674b](https://github.com/sagemath/sagetrac-mirror/commit/be4674b23482a84ff45fbf6f6f6cfede8eab0a9f)** to none",
    "created_at": "2018-12-02T20:05:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26657",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26657#issuecomment-413654",
    "user": "https://github.com/soehms"
}
```

Changed commit from **[be4674b](https://github.com/sagemath/sagetrac-mirror/commit/be4674b23482a84ff45fbf6f6f6cfede8eab0a9f)** to none



---

archive/issue_comments_413655.json:
```json
{
    "body": "Commit: **[b36eca1](https://github.com/sagemath/sagetrac-mirror/commit/b36eca19909077b44fd94fd595ef0c5c1d4e3c53)**",
    "created_at": "2018-12-02T20:06:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26657",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26657#issuecomment-413655",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[b36eca1](https://github.com/sagemath/sagetrac-mirror/commit/b36eca19909077b44fd94fd595ef0c5c1d4e3c53)**



---

archive/issue_comments_413656.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2018-12-02T20:06:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26657",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26657#issuecomment-413656",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_413657.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a05e3e48f56bedf5d19d4ecca8ccf698b7907629\">a05e3e4</a></td><td><code>correct typset error in new branch to avoid merge error</code></td></tr></table>\n",
    "created_at": "2018-12-02T20:07:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26657",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26657#issuecomment-413657",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:7" align="right">Comment 7</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a05e3e48f56bedf5d19d4ecca8ccf698b7907629">a05e3e4</a></td><td><code>correct typset error in new branch to avoid merge error</code></td></tr></table>




---

archive/issue_comments_413658.json:
```json
{
    "body": "Changed commit from **[b36eca1](https://github.com/sagemath/sagetrac-mirror/commit/b36eca19909077b44fd94fd595ef0c5c1d4e3c53)** to **[a05e3e4](https://github.com/sagemath/sagetrac-mirror/commit/a05e3e48f56bedf5d19d4ecca8ccf698b7907629)**",
    "created_at": "2018-12-02T20:07:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26657",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26657#issuecomment-413658",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[b36eca1](https://github.com/sagemath/sagetrac-mirror/commit/b36eca19909077b44fd94fd595ef0c5c1d4e3c53)** to **[a05e3e4](https://github.com/sagemath/sagetrac-mirror/commit/a05e3e48f56bedf5d19d4ecca8ccf698b7907629)**



---

archive/issue_comments_413659.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nReplying to [@tscrim](#comment%3A3):\n> Sorry, I lost track of this ticket. LGTM.\n\nNo Problem! \n\nThank you, Travis! I will be waiting for the patchbot to see if I got rid of that merge error.",
    "created_at": "2018-12-02T20:09:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26657",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26657#issuecomment-413659",
    "user": "https://github.com/soehms"
}
```

<div id="comment:8" align="right">Comment 8</div>

Replying to [@tscrim](#comment%3A3):
> Sorry, I lost track of this ticket. LGTM.

No Problem! 

Thank you, Travis! I will be waiting for the patchbot to see if I got rid of that merge error.



---

archive/issue_events_348647.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-12-02T20:11:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26657",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26657#event-348647"
}
```



---

archive/issue_events_348648.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-12-02T20:11:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26657",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26657#event-348648"
}
```



---

archive/issue_comments_413660.json:
```json
{
    "body": "Changed branch from **[u/soehms/adapt_unitary_burau_26657](https://github.com/sagemath/sagetrac-mirror/tree/u/soehms/adapt_unitary_burau_26657)** to **[a05e3e4](https://github.com/sagemath/sagetrac-mirror/commit/a05e3e48f56bedf5d19d4ecca8ccf698b7907629)**",
    "created_at": "2018-12-05T23:56:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26657",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26657#issuecomment-413660",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/soehms/adapt_unitary_burau_26657](https://github.com/sagemath/sagetrac-mirror/tree/u/soehms/adapt_unitary_burau_26657)** to **[a05e3e4](https://github.com/sagemath/sagetrac-mirror/commit/a05e3e48f56bedf5d19d4ecca8ccf698b7907629)**



---

archive/issue_events_348649.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-12-05T23:56:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26657",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26657#event-348649"
}
```



---

archive/issue_events_348650.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "a1426983a253dc76ff072b12c070952f82dc1ed9",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-12-05T23:56:17Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26657",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26657#event-348650"
}
```
