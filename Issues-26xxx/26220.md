# Issue 26220: Fix CoercionException imports

archive/issues_025983.json:
```json
{
    "assignees": [],
    "body": "The lazy imports used for `CoercionException` cause some problems with Python 3. I think they are not necessary: instead of lazily importing from `sage.structure.parent`, we can import directly from `sage.structure.coerce_exceptions`, which should be pretty fast (that file contains almost nothing).\n\nThis fixes several Python 3 doctest issues, such as the one in the ticket description for #25927.\n\nBranch/Commit: **[fbdf022](https://github.com/sagemath/sagetrac-mirror/commit/fbdf0226fc2baf8684a774fec46aed337ab76a75)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **John Palmieri**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26220_\n\n",
    "closed_at": "2018-09-09T07:38:17Z",
    "created_at": "2018-09-07T23:21:47Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20python3",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix CoercionException imports",
    "type": "issue",
    "updated_at": "2018-09-09T07:38:17Z",
    "url": "https://github.com/sagemath/sage/issues/26220",
    "user": "https://github.com/jhpalmieri"
}
```
The lazy imports used for `CoercionException` cause some problems with Python 3. I think they are not necessary: instead of lazily importing from `sage.structure.parent`, we can import directly from `sage.structure.coerce_exceptions`, which should be pretty fast (that file contains almost nothing).

This fixes several Python 3 doctest issues, such as the one in the ticket description for #25927.

Branch/Commit: **[fbdf022](https://github.com/sagemath/sagetrac-mirror/commit/fbdf0226fc2baf8684a774fec46aed337ab76a75)**

Reviewer: **Travis Scrimshaw**

Author: **John Palmieri**

_Issue created by migration from https://trac.sagemath.org/ticket/26220_





---

archive/issue_events_343558.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2018-09-07T23:21:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26220",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26220#event-343558"
}
```



---

archive/issue_events_343559.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2018-09-07T23:21:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26220",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20python3",
    "label_color": "000080",
    "label_name": "component: python3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26220#event-343559"
}
```



---

archive/issue_events_343560.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2018-09-07T23:21:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26220",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26220#event-343560"
}
```



---

archive/issue_events_343561.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2018-09-07T23:21:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26220",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26220#event-343561"
}
```



---

archive/issue_comments_406941.json:
```json
{
    "body": "Branch: **[u/jhpalmieri/coercionexception](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/coercionexception)**",
    "created_at": "2018-09-07T23:22:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26220",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26220#issuecomment-406941",
    "user": "https://github.com/jhpalmieri"
}
```

Branch: **[u/jhpalmieri/coercionexception](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/coercionexception)**



---

archive/issue_comments_406942.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fbdf0226fc2baf8684a774fec46aed337ab76a75\">fbdf022</a></td><td><code>trac 26220: change import statements for CoercionException.</code></td></tr></table>\n",
    "created_at": "2018-09-07T23:22:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26220",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26220#issuecomment-406942",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fbdf0226fc2baf8684a774fec46aed337ab76a75">fbdf022</a></td><td><code>trac 26220: change import statements for CoercionException.</code></td></tr></table>




---

archive/issue_comments_406943.json:
```json
{
    "body": "Commit: **[fbdf022](https://github.com/sagemath/sagetrac-mirror/commit/fbdf0226fc2baf8684a774fec46aed337ab76a75)**",
    "created_at": "2018-09-07T23:22:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26220",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26220#issuecomment-406943",
    "user": "https://github.com/jhpalmieri"
}
```

Commit: **[fbdf022](https://github.com/sagemath/sagetrac-mirror/commit/fbdf0226fc2baf8684a774fec46aed337ab76a75)**



---

archive/issue_events_343562.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2018-09-07T23:22:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26220",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26220#event-343562"
}
```



---

archive/issue_comments_406944.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nI don't actually know if the changes in the `combinat` files are necessary, but these import statements look cleaner than importing from `parent`.",
    "created_at": "2018-09-07T23:24:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26220",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26220#issuecomment-406944",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:3" align="right">Comment 3</div>

I don't actually know if the changes in the `combinat` files are necessary, but these import statements look cleaner than importing from `parent`.



---

archive/issue_events_343563.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-09-07T23:33:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26220",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26220#event-343563"
}
```



---

archive/issue_events_343564.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-09-07T23:33:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26220",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26220#event-343564"
}
```



---

archive/issue_comments_406945.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2018-09-07T23:33:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26220",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26220#issuecomment-406945",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_406946.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThe change to pushout was also done in #26211, but I have asked for that part to be reverted in favor of this ticket.",
    "created_at": "2018-09-07T23:33:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26220",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26220#issuecomment-406946",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:4" align="right">Comment 4</div>

The change to pushout was also done in #26211, but I have asked for that part to be reverted in favor of this ticket.



---

archive/issue_comments_406947.json:
```json
{
    "body": "Changed branch from **[u/jhpalmieri/coercionexception](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/coercionexception)** to **[fbdf022](https://github.com/sagemath/sagetrac-mirror/commit/fbdf0226fc2baf8684a774fec46aed337ab76a75)**",
    "created_at": "2018-09-09T07:38:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26220",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26220#issuecomment-406947",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jhpalmieri/coercionexception](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/coercionexception)** to **[fbdf022](https://github.com/sagemath/sagetrac-mirror/commit/fbdf0226fc2baf8684a774fec46aed337ab76a75)**



---

archive/issue_events_343565.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-09-09T07:38:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26220",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26220#event-343565"
}
```



---

archive/issue_events_343566.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "cca7bbf23066a089ea5e1077121ded3bf97d6b32",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-09-09T07:38:17Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26220",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26220#event-343566"
}
```
