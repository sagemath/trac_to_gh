# Issue 26227: Make search_doc independent of sage_setup

archive/issues_025990.json:
```json
{
    "body": "search_doc currently imports stuff from sage_setup, which shouldn't be needed by sage at runtime. It also relies on the sage doc source to be present, which is not necessarily true for distro packages.\nThis patch replaces the current exhaustive check that all translated docs have been built by a simple check that $SAGE_DOC/html exists.\n\nCC:  @timokau @kiwifb @saraedum\n\nKeywords: packaging\n\nBranch/Commit: b3abf9cc455f02c4d7b11244a863e6eb4fa607c7\n\nReviewer: Fran\u00e7ois Bissey\n\nAuthor: Antonio Rojas\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/26227\n\n",
    "closed_at": "2018-09-23T19:34:12Z",
    "created_at": "2018-09-08T18:12:44Z",
    "labels": [
        "component: documentation"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.4",
    "title": "Make search_doc independent of sage_setup",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/26227",
    "user": "https://github.com/antonio-rojas"
}
```
search_doc currently imports stuff from sage_setup, which shouldn't be needed by sage at runtime. It also relies on the sage doc source to be present, which is not necessarily true for distro packages.
This patch replaces the current exhaustive check that all translated docs have been built by a simple check that $SAGE_DOC/html exists.

CC:  @timokau @kiwifb @saraedum

Keywords: packaging

Branch/Commit: b3abf9cc455f02c4d7b11244a863e6eb4fa607c7

Reviewer: François Bissey

Author: Antonio Rojas

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/26227





---

archive/issue_comments_504202.json:
```json
{
    "body": "Changing branch from \"\" to \"u/arojas/make_search_doc_independent_of_sage_setup\"",
    "created_at": "2018-09-08T18:13:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504202",
    "user": "https://github.com/antonio-rojas"
}
```

Changing branch from "" to "u/arojas/make_search_doc_independent_of_sage_setup"



---

archive/issue_comments_504203.json:
```json
{
    "body": "Changing keywords from \"\" to \"packaging\".",
    "created_at": "2018-09-08T18:21:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504203",
    "user": "https://github.com/antonio-rojas"
}
```

Changing keywords from "" to "packaging".



---

archive/issue_comments_504204.json:
```json
{
    "body": "Changing type from PLEASE CHANGE to enhancement.",
    "created_at": "2018-09-08T18:21:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504204",
    "user": "https://github.com/antonio-rojas"
}
```

Changing type from PLEASE CHANGE to enhancement.



---

archive/issue_comments_504205.json:
```json
{
    "body": "Changing component from PLEASE CHANGE to documentation.",
    "created_at": "2018-09-08T18:21:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504205",
    "user": "https://github.com/antonio-rojas"
}
```

Changing component from PLEASE CHANGE to documentation.



---

archive/issue_comments_504206.json:
```json
{
    "body": "Changing author from \"\" to \"Antonio Rojas\"",
    "created_at": "2018-09-08T18:21:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504206",
    "user": "https://github.com/antonio-rojas"
}
```

Changing author from "" to "Antonio Rojas"



---

archive/issue_comments_504207.json:
```json
{
    "body": "<a id='comment:2'></a>New commits:",
    "created_at": "2018-09-08T18:21:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504207",
    "user": "https://github.com/antonio-rojas"
}
```

<a id='comment:2'></a>New commits:



---

archive/issue_comments_504208.json:
```json
{
    "body": "Changing commit from \"\" to \"86c3afbcfa4f004c527aea9fafd6d0efd4a858d5\"",
    "created_at": "2018-09-08T18:21:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504208",
    "user": "https://github.com/antonio-rojas"
}
```

Changing commit from "" to "86c3afbcfa4f004c527aea9fafd6d0efd4a858d5"



---

archive/issue_comments_504209.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,2 @@\n-\n+search_doc currently imports stuff from sage_setup, which shouldn't be needed by sage at runtime.\n+This patch defines the necessary variables directly so that search_doc works when sage_setup is not installed.\n``````\n",
    "created_at": "2018-09-08T18:21:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504209",
    "user": "https://github.com/antonio-rojas"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,2 @@
-
+search_doc currently imports stuff from sage_setup, which shouldn't be needed by sage at runtime.
+This patch defines the necessary variables directly so that search_doc works when sage_setup is not installed.
``````




---

archive/issue_comments_504210.json:
```json
{
    "body": "<a id='comment:3'></a>Thanks for copying me. Those bits are a part of something I just remove from sage https://github.com/cschwan/sage-on-gentoo/blob/master/sci-mathematics/sage/files/sage-8.4-sagedoc.patch . I wonder if it is time for it to disappear in sage proper too.\n\nWhy do I remove that bit? This check at runtime that the documentation has been built - it may have been helpful at some point during  early sage development but I don't believe it should still be the case. Furthermore, this effectively depends on the presence (fake or real) of sage's documentation source. And of course we disapprove of that in distro. Finally if the search fails, it suggests you do stuff that is not supported for user on a distro (`sage -docbuild`).",
    "created_at": "2018-09-08T19:13:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504210",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:3'></a>Thanks for copying me. Those bits are a part of something I just remove from sage https://github.com/cschwan/sage-on-gentoo/blob/master/sci-mathematics/sage/files/sage-8.4-sagedoc.patch . I wonder if it is time for it to disappear in sage proper too.

Why do I remove that bit? This check at runtime that the documentation has been built - it may have been helpful at some point during  early sage development but I don't believe it should still be the case. Furthermore, this effectively depends on the presence (fake or real) of sage's documentation source. And of course we disapprove of that in distro. Finally if the search fails, it suggests you do stuff that is not supported for user on a distro (`sage -docbuild`).



---

archive/issue_comments_504211.json:
```json
{
    "body": "<a id='comment:4'></a>It would be great to get rid of all that code, yes. If devs agree to that I'll happily withdraw this.",
    "created_at": "2018-09-09T13:21:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504211",
    "user": "https://github.com/antonio-rojas"
}
```

<a id='comment:4'></a>It would be great to get rid of all that code, yes. If devs agree to that I'll happily withdraw this.



---

archive/issue_comments_504212.json:
```json
{
    "body": "<a id='comment:5'></a>`@`fbissy: Is there some good way of detecting whether you are on a distro, and then conditionally evaluate the code? My view has been for a long time that a Sage installation (any software installation, really) is not complete without documentation, so I want some sort of check. (I don't think you're advocating removing that big chunk of code always, just in your distro, but if we can make things work for both settings, why not?) Can we set some environment variable, or touch some file, or do something else to indicate that it's a distro and not a from-source or from-binary installation?\n\n`@`arojas: By the way, an alternative to the branch here: rather than to duplicate code and variable definitions from sage_setup.docbuild, is to define the relevant variables in sage.misc.sagedoc or sage.env, and then import them in sage_setup.docbuild. Duplication is usually a bad idea: harder to maintain and harder to diagnose problems. (sage_setup.docbuild already has some imports from the Sage library.)",
    "created_at": "2018-09-09T17:38:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504212",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:5'></a>`@`fbissy: Is there some good way of detecting whether you are on a distro, and then conditionally evaluate the code? My view has been for a long time that a Sage installation (any software installation, really) is not complete without documentation, so I want some sort of check. (I don't think you're advocating removing that big chunk of code always, just in your distro, but if we can make things work for both settings, why not?) Can we set some environment variable, or touch some file, or do something else to indicate that it's a distro and not a from-source or from-binary installation?

`@`arojas: By the way, an alternative to the branch here: rather than to duplicate code and variable definitions from sage_setup.docbuild, is to define the relevant variables in sage.misc.sagedoc or sage.env, and then import them in sage_setup.docbuild. Duplication is usually a bad idea: harder to maintain and harder to diagnose problems. (sage_setup.docbuild already has some imports from the Sage library.)



---

archive/issue_comments_504213.json:
```json
{
    "body": "<a id='comment:6'></a>I understand the concern about the documentation (or at least some documentation - in sage-on-gentoo I allow for some language selection with the caveat that only english is complete). The code is useful for someone writing new code with the associated documentation but it is difficult to justify it at runtime for everyone.\n\nThere no current way to say we are a distro, although we could come up with one :)\n\nOverall I think the concern that documentation is present could be checked by a doctest rather than anytime you do a search. I am OK for sage's source code to be referenced during a doctest.",
    "created_at": "2018-09-09T19:11:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504213",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:6'></a>I understand the concern about the documentation (or at least some documentation - in sage-on-gentoo I allow for some language selection with the caveat that only english is complete). The code is useful for someone writing new code with the associated documentation but it is difficult to justify it at runtime for everyone.

There no current way to say we are a distro, although we could come up with one :)

Overall I think the concern that documentation is present could be checked by a doctest rather than anytime you do a search. I am OK for sage's source code to be referenced during a doctest.



---

archive/issue_comments_504214.json:
```json
{
    "body": "Changing branch from \"u/arojas/make_search_doc_independent_of_sage_setup\" to \"u/jhpalmieri/make_search_doc_independent_of_sage_setup\"",
    "created_at": "2018-09-10T00:00:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504214",
    "user": "https://github.com/jhpalmieri"
}
```

Changing branch from "u/arojas/make_search_doc_independent_of_sage_setup" to "u/jhpalmieri/make_search_doc_independent_of_sage_setup"



---

archive/issue_comments_504215.json:
```json
{
    "body": "Changing commit from \"86c3afbcfa4f004c527aea9fafd6d0efd4a858d5\" to \"41bc9de9cc5e6407e770fc84414674135b691b39\"",
    "created_at": "2018-09-10T00:01:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504215",
    "user": "https://github.com/jhpalmieri"
}
```

Changing commit from "86c3afbcfa4f004c527aea9fafd6d0efd4a858d5" to "41bc9de9cc5e6407e770fc84414674135b691b39"



---

archive/issue_comments_504216.json:
```json
{
    "body": "<a id='comment:8'></a>Okay, how about something like this? I tried to write it so it would work if `SAGE_DOC_SRC` is missing or if `sage_setup` is missing. The doctests are marked 'optional - dochtml'. I also added doctests so that we find out when the hardcoded lists of languages and documents become outdated (when building with access to the full Sage source code).\n\n---\nNew commits:",
    "created_at": "2018-09-10T00:01:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504216",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:8'></a>Okay, how about something like this? I tried to write it so it would work if `SAGE_DOC_SRC` is missing or if `sage_setup` is missing. The doctests are marked 'optional - dochtml'. I also added doctests so that we find out when the hardcoded lists of languages and documents become outdated (when building with access to the full Sage source code).

---
New commits:



---

archive/issue_comments_504217.json:
```json
{
    "body": "<a id='comment:9'></a>I am not too crazy about it, I'd like other people opinions and some reflection time before completely dismissing the approach. But my first reaction is as follow\n* I don't like `HARDCODED*` variables in particular documents.\n* Of course it make sense have it tagged `dochtml` - good thinking.\n* In sage-on-gentoo I allow installing a subset of languages (with english being compulsory if you build/install documentation at all) I am not liking the idea of a doctest failing because I don't have languages that are not relevant to me. I guess that's why I don't like `HARDCODED_DOCUMENTS`, but I could live with that.\n\nI guess more generally I didn't think doctesting that some or all documentation has been built/installed would need that much machinery that in turn needs its own problematic doctests. Is the added complexity worth it?\n\nBut thank you for showing what you think it should look like.",
    "created_at": "2018-09-10T00:20:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504217",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:9'></a>I am not too crazy about it, I'd like other people opinions and some reflection time before completely dismissing the approach. But my first reaction is as follow
* I don't like `HARDCODED*` variables in particular documents.
* Of course it make sense have it tagged `dochtml` - good thinking.
* In sage-on-gentoo I allow installing a subset of languages (with english being compulsory if you build/install documentation at all) I am not liking the idea of a doctest failing because I don't have languages that are not relevant to me. I guess that's why I don't like `HARDCODED_DOCUMENTS`, but I could live with that.

I guess more generally I didn't think doctesting that some or all documentation has been built/installed would need that much machinery that in turn needs its own problematic doctests. Is the added complexity worth it?

But thank you for showing what you think it should look like.



---

archive/issue_comments_504218.json:
```json
{
    "body": "<a id='comment:10'></a>Some more thoughts.\n \nUsing sources at runtime is forbidden but testing is ideally run before install time and certainly before packaging for binary distro. We don't expect users installing binary packages to be able to run the test suite. Apart from some rare cases it is just not present to be run anyway.\n\nSo relying on SAGE_DOC_SRC at testing is fine. Nevertheless there is a general feeling that the list of language may belong to env.py rather than somewhere in sage_setup. Something more satisfactory could be constructed there.\n\nThere are many moving parts and I am considering giving it a shot from the current branch and some thinking time.",
    "created_at": "2018-09-10T00:32:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504218",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:10'></a>Some more thoughts.
 
Using sources at runtime is forbidden but testing is ideally run before install time and certainly before packaging for binary distro. We don't expect users installing binary packages to be able to run the test suite. Apart from some rare cases it is just not present to be run anyway.

So relying on SAGE_DOC_SRC at testing is fine. Nevertheless there is a general feeling that the list of language may belong to env.py rather than somewhere in sage_setup. Something more satisfactory could be constructed there.

There are many moving parts and I am considering giving it a shot from the current branch and some thinking time.



---

archive/issue_comments_504219.json:
```json
{
    "body": "<a id='comment:11'></a>Ping to other distro guys, do you have a definition for `SAGE_DOC_SRC` in `sage/env.py` for your distro. If you do what is it? In sage-on-gentoo SAGE_DOC_SRC=SAGE_DOC at runtime (but not at build time obviously).",
    "created_at": "2018-09-10T00:37:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504219",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:11'></a>Ping to other distro guys, do you have a definition for `SAGE_DOC_SRC` in `sage/env.py` for your distro. If you do what is it? In sage-on-gentoo SAGE_DOC_SRC=SAGE_DOC at runtime (but not at build time obviously).



---

archive/issue_comments_504220.json:
```json
{
    "body": "<a id='comment:12'></a>Replying to [fbissey](#comment%3A9):\n> I guess more generally I didn't think doctesting that some or all documentation has been built/installed would need that much machinery that in turn needs its own problematic doctests. Is the added complexity worth it?\n\n\nTo really test whether the built documentation is present, you need access to `SAGE_DOC_SRC` and `sage_setup` to compile the lists of languages and documents. So if the doctests should pass without access to those, you need\n\n- sensible versions of the variables,\n- and a way to detect when those sensible versions are no longer sensible (because someone has translated the tutorial to Arabic, for example).\n\nIf you don't care about doctests passing, or if you only care about doctests passing if you have intact `SAGE_DOC_SRC` and `sage_setup`, then it doesn't need to be as complicated. (References to `sage_setup` could be avoided, but I think you need `SAGE_DOC_SRC`.) Since some people want to distribute Sage without any built documentation and have it pass doctests, I don't know what is reasonable to require for doctests to pass. Maybe that's where the discussion should start: under what circumstances should doctests pass?\n\nDoes it at least make sense to have a separate function (as in my branch) that just tests whether the documentation is built, along with a doctest?",
    "created_at": "2018-09-10T01:10:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504220",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:12'></a>Replying to [fbissey](#comment%3A9):
> I guess more generally I didn't think doctesting that some or all documentation has been built/installed would need that much machinery that in turn needs its own problematic doctests. Is the added complexity worth it?


To really test whether the built documentation is present, you need access to `SAGE_DOC_SRC` and `sage_setup` to compile the lists of languages and documents. So if the doctests should pass without access to those, you need

- sensible versions of the variables,
- and a way to detect when those sensible versions are no longer sensible (because someone has translated the tutorial to Arabic, for example).

If you don't care about doctests passing, or if you only care about doctests passing if you have intact `SAGE_DOC_SRC` and `sage_setup`, then it doesn't need to be as complicated. (References to `sage_setup` could be avoided, but I think you need `SAGE_DOC_SRC`.) Since some people want to distribute Sage without any built documentation and have it pass doctests, I don't know what is reasonable to require for doctests to pass. Maybe that's where the discussion should start: under what circumstances should doctests pass?

Does it at least make sense to have a separate function (as in my branch) that just tests whether the documentation is built, along with a doctest?



---

archive/issue_comments_504221.json:
```json
{
    "body": "<a id='comment:13'></a>Replying to [jhpalmieri](#comment%3A12):\n> Replying to [fbissey](#comment%3A9):\n> > I guess more generally I didn't think doctesting that some or all documentation has been built/installed would need that much machinery that in turn needs its own problematic doctests. Is the added complexity worth it?\n\n> \n> To really test whether the built documentation is present, you need access to `SAGE_DOC_SRC` and `sage_setup` to compile the lists of languages and documents. So if the doctests should pass without access to those, you need\n> \n> - sensible versions of the variables,\n> - and a way to detect when those sensible versions are no longer sensible (because someone has translated the tutorial to Arabic, for example).\n> \n\n\nYes.\n\n> If you don't care about doctests passing, or if you only care about doctests passing if you have intact `SAGE_DOC_SRC` and `sage_setup`, then it doesn't need to be as complicated. (References to `sage_setup` could be avoided, but I think you need `SAGE_DOC_SRC`.) Since some people want to distribute Sage without any built documentation and have it pass doctests, I don't know what is reasonable to require for doctests to pass. Maybe that's where the discussion should start: under what circumstances should doctests pass?\n> \n\n\nI must say I am puzzled by my colleagues insisting the tests should pass without the doc. I guess technically I have it run OK with just some languages of the documentation but I was not expecting it to pass with none. But having the ability to test the html documentation separately is not absurd. I agree you should at least have \"something that look like SAGE_DOC_SRC\". Again, like I said using sources for testing purposes is OK.\n\n> Does it at least make sense to have a separate function (as in my branch) that just tests whether the documentation is built, along with a doctest?\n\n\nYes, I hadn't gone as far as thinking of a new function when I brought you the idea of doctesting but it make sense for it to exist and to be the thing tested. The only issue I may have is: \"is it useful to have this available at runtime?\" Which then turns into where is it appropriate to put this.",
    "created_at": "2018-09-10T01:23:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504221",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:13'></a>Replying to [jhpalmieri](#comment%3A12):
> Replying to [fbissey](#comment%3A9):
> > I guess more generally I didn't think doctesting that some or all documentation has been built/installed would need that much machinery that in turn needs its own problematic doctests. Is the added complexity worth it?

> 
> To really test whether the built documentation is present, you need access to `SAGE_DOC_SRC` and `sage_setup` to compile the lists of languages and documents. So if the doctests should pass without access to those, you need
> 
> - sensible versions of the variables,
> - and a way to detect when those sensible versions are no longer sensible (because someone has translated the tutorial to Arabic, for example).
> 


Yes.

> If you don't care about doctests passing, or if you only care about doctests passing if you have intact `SAGE_DOC_SRC` and `sage_setup`, then it doesn't need to be as complicated. (References to `sage_setup` could be avoided, but I think you need `SAGE_DOC_SRC`.) Since some people want to distribute Sage without any built documentation and have it pass doctests, I don't know what is reasonable to require for doctests to pass. Maybe that's where the discussion should start: under what circumstances should doctests pass?
> 


I must say I am puzzled by my colleagues insisting the tests should pass without the doc. I guess technically I have it run OK with just some languages of the documentation but I was not expecting it to pass with none. But having the ability to test the html documentation separately is not absurd. I agree you should at least have "something that look like SAGE_DOC_SRC". Again, like I said using sources for testing purposes is OK.

> Does it at least make sense to have a separate function (as in my branch) that just tests whether the documentation is built, along with a doctest?


Yes, I hadn't gone as far as thinking of a new function when I brought you the idea of doctesting but it make sense for it to exist and to be the thing tested. The only issue I may have is: "is it useful to have this available at runtime?" Which then turns into where is it appropriate to put this.



---

archive/issue_comments_504222.json:
```json
{
    "body": "<a id='comment:14'></a>And if it is not needed at runtime, my reaction is to shove it into sage_setup.",
    "created_at": "2018-09-10T01:32:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504222",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:14'></a>And if it is not needed at runtime, my reaction is to shove it into sage_setup.



---

archive/issue_comments_504223.json:
```json
{
    "body": "<a id='comment:15'></a>Replying to [fbissey](#comment%3A10):\n> Some more thoughts.\n>  \n> Using sources at runtime is forbidden but testing is ideally run before install time and certainly before packaging for binary distro. We don't expect users installing binary packages to be able to run the test suite. Apart from some rare cases it is just not present to be run anyway.\n\n\nI don't agree with this. Binary packages have a different folder structure than the one in sage-the-distribution and it's important to make sure that everything works as intended, and the way to check that is by running the test suite, and it needs to be done when the package is installed and the source files are no longer available. This helps detecting runtime dependencies on source files that should be removed, and can't be detected by running the test suite on sage-the-distribution.",
    "created_at": "2018-09-10T20:30:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504223",
    "user": "https://github.com/antonio-rojas"
}
```

<a id='comment:15'></a>Replying to [fbissey](#comment%3A10):
> Some more thoughts.
>  
> Using sources at runtime is forbidden but testing is ideally run before install time and certainly before packaging for binary distro. We don't expect users installing binary packages to be able to run the test suite. Apart from some rare cases it is just not present to be run anyway.


I don't agree with this. Binary packages have a different folder structure than the one in sage-the-distribution and it's important to make sure that everything works as intended, and the way to check that is by running the test suite, and it needs to be done when the package is installed and the source files are no longer available. This helps detecting runtime dependencies on source files that should be removed, and can't be detected by running the test suite on sage-the-distribution.



---

archive/issue_comments_504224.json:
```json
{
    "body": "<a id='comment:16'></a>- Should there be a doctest to test whether the documentation has been built? If I am installing Sage myself, I know whether I tried to build the docs, and if I did, then I know whether it succeeded. So people who build from source or use a Sage binary don't need this doctest. What about for distros? If we want such a doctest, should it be in `sage_setup` or the main Sage library? If in the main Sage library, can it assume that `sage_setup` and `SAGE_DOC_SRC` are intact?\n\n- If a user tries to search the documentation, should they be warned if the documentation is missing or incomplete?",
    "created_at": "2018-09-10T20:42:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504224",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:16'></a>- Should there be a doctest to test whether the documentation has been built? If I am installing Sage myself, I know whether I tried to build the docs, and if I did, then I know whether it succeeded. So people who build from source or use a Sage binary don't need this doctest. What about for distros? If we want such a doctest, should it be in `sage_setup` or the main Sage library? If in the main Sage library, can it assume that `sage_setup` and `SAGE_DOC_SRC` are intact?

- If a user tries to search the documentation, should they be warned if the documentation is missing or incomplete?



---

archive/issue_comments_504225.json:
```json
{
    "body": "<a id='comment:17'></a>Replying to [jhpalmieri](#comment%3A16):\n> - Should there be a doctest to test whether the documentation has been built? If I am installing Sage myself, I know whether I tried to build the docs, and if I did, then I know whether it succeeded. \n\n\nThe same applies to packagers who build distro packages, so I don't see why that case should be treated differently.\n\n> - If a user tries to search the documentation, should they be warned if the documentation is missing or incomplete?\n\n\nWe ship docs as a separate, optional package, so such a warning would be quite useful.",
    "created_at": "2018-09-10T20:50:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504225",
    "user": "https://github.com/antonio-rojas"
}
```

<a id='comment:17'></a>Replying to [jhpalmieri](#comment%3A16):
> - Should there be a doctest to test whether the documentation has been built? If I am installing Sage myself, I know whether I tried to build the docs, and if I did, then I know whether it succeeded. 


The same applies to packagers who build distro packages, so I don't see why that case should be treated differently.

> - If a user tries to search the documentation, should they be warned if the documentation is missing or incomplete?


We ship docs as a separate, optional package, so such a warning would be quite useful.



---

archive/issue_comments_504226.json:
```json
{
    "body": "<a id='comment:18'></a>Replying to [arojas](#comment%3A15):\n> Replying to [fbissey](#comment%3A10):\n> > Some more thoughts.\n> >  \n> > Using sources at runtime is forbidden but testing is ideally run before install time and certainly before packaging for binary distro. We don't expect users installing binary packages to be able to run the test suite. Apart from some rare cases it is just not present to be run anyway.\n\n> \n> I don't agree with this. Binary packages have a different folder structure than the one in sage-the-distribution and it's important to make sure that everything works as intended, and the way to check that is by running the test suite, and it needs to be done when the package is installed and the source files are no longer available. This helps detecting runtime dependencies on source files that should be removed, and can't be detected by running the test suite on sage-the-distribution.\n\n\nI agree with arojas. I think testing is ideally run *after* install time, under identical conditions as if the package was installed on a users machine. Because that is what we want to test: That sage is fully functional for a user. What does it help when sage works after build but may break after installation?\n\nI personally just ship the sage source code. Why do you not want to do that? The only negative I see is the disk space, which is not *that* much.",
    "created_at": "2018-09-10T21:05:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504226",
    "user": "https://github.com/timokau"
}
```

<a id='comment:18'></a>Replying to [arojas](#comment%3A15):
> Replying to [fbissey](#comment%3A10):
> > Some more thoughts.
> >  
> > Using sources at runtime is forbidden but testing is ideally run before install time and certainly before packaging for binary distro. We don't expect users installing binary packages to be able to run the test suite. Apart from some rare cases it is just not present to be run anyway.

> 
> I don't agree with this. Binary packages have a different folder structure than the one in sage-the-distribution and it's important to make sure that everything works as intended, and the way to check that is by running the test suite, and it needs to be done when the package is installed and the source files are no longer available. This helps detecting runtime dependencies on source files that should be removed, and can't be detected by running the test suite on sage-the-distribution.


I agree with arojas. I think testing is ideally run *after* install time, under identical conditions as if the package was installed on a users machine. Because that is what we want to test: That sage is fully functional for a user. What does it help when sage works after build but may break after installation?

I personally just ship the sage source code. Why do you not want to do that? The only negative I see is the disk space, which is not *that* much.



---

archive/issue_comments_504227.json:
```json
{
    "body": "<a id='comment:19'></a>Replying to [arojas](#comment%3A17):\n> > - If a user tries to search the documentation, should they be warned if the documentation is missing or incomplete?\n \n> \n> We ship docs as a separate, optional package, so such a warning would be quite useful.\n\n\nDoes your main distribution include `SAGE_DOC_SRC`? If not, how can you meaningfully measure whether the documentation is present? My branch tries to address this; is it overkill?",
    "created_at": "2018-09-10T21:08:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504227",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:19'></a>Replying to [arojas](#comment%3A17):
> > - If a user tries to search the documentation, should they be warned if the documentation is missing or incomplete?
 
> 
> We ship docs as a separate, optional package, so such a warning would be quite useful.


Does your main distribution include `SAGE_DOC_SRC`? If not, how can you meaningfully measure whether the documentation is present? My branch tries to address this; is it overkill?



---

archive/issue_comments_504228.json:
```json
{
    "body": "<a id='comment:20'></a>Replying to [jhpalmieri](#comment%3A16):\n> - Should there be a doctest to test whether the documentation has been built? If I am installing Sage myself, I know whether I tried to build the docs, and if I did, then I know whether it succeeded. So people who build from source or use a Sage binary don't need this doctest. What about for distros? If we want such a doctest, should it be in `sage_setup` or the main Sage library? If in the main Sage library, can it assume that `sage_setup` and `SAGE_DOC_SRC` are intact?\n\n\nI think a doctest that makes sure sage can find its documentation is very valuable. However it shouldn't be mandatory to build sage. That is why I originally introduced `optional - dochtml`. Currently I'm building sage without docs and then testing it without `dochtml`. I then build the docs seperately and run only the `dochtml` tests for that. Users can decide weather or not to install the docs. Unfortunately, that will be somewhat undermined by [this change](https://trac.sagemath.org/ticket/26110#comment:15).\n\n> - If a user tries to search the documentation, should they be warned if the documentation is missing or incomplete?\n\n\nThat would be nice.",
    "created_at": "2018-09-10T21:09:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504228",
    "user": "https://github.com/timokau"
}
```

<a id='comment:20'></a>Replying to [jhpalmieri](#comment%3A16):
> - Should there be a doctest to test whether the documentation has been built? If I am installing Sage myself, I know whether I tried to build the docs, and if I did, then I know whether it succeeded. So people who build from source or use a Sage binary don't need this doctest. What about for distros? If we want such a doctest, should it be in `sage_setup` or the main Sage library? If in the main Sage library, can it assume that `sage_setup` and `SAGE_DOC_SRC` are intact?


I think a doctest that makes sure sage can find its documentation is very valuable. However it shouldn't be mandatory to build sage. That is why I originally introduced `optional - dochtml`. Currently I'm building sage without docs and then testing it without `dochtml`. I then build the docs seperately and run only the `dochtml` tests for that. Users can decide weather or not to install the docs. Unfortunately, that will be somewhat undermined by [this change](https://trac.sagemath.org/ticket/26110#comment:15).

> - If a user tries to search the documentation, should they be warned if the documentation is missing or incomplete?


That would be nice.



---

archive/issue_comments_504229.json:
```json
{
    "body": "<a id='comment:21'></a>Replying to [jhpalmieri](#comment%3A19):\n> Replying to [arojas](#comment%3A17):\n> > > - If a user tries to search the documentation, should they be warned if the documentation is missing or incomplete?\n \n> > \n> > We ship docs as a separate, optional package, so such a warning would be quite useful.\n\n> \n> Does your main distribution include `SAGE_DOC_SRC`? If not, how can you meaningfully measure whether the documentation is present? My branch tries to address this; is it overkill?\n\n\nI ship two packages: `sage` and `sageWithDoc`. The second one is just a wrapper around `sage` that sets `SAGE_DOC_SRC`.",
    "created_at": "2018-09-10T21:10:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504229",
    "user": "https://github.com/timokau"
}
```

<a id='comment:21'></a>Replying to [jhpalmieri](#comment%3A19):
> Replying to [arojas](#comment%3A17):
> > > - If a user tries to search the documentation, should they be warned if the documentation is missing or incomplete?
 
> > 
> > We ship docs as a separate, optional package, so such a warning would be quite useful.

> 
> Does your main distribution include `SAGE_DOC_SRC`? If not, how can you meaningfully measure whether the documentation is present? My branch tries to address this; is it overkill?


I ship two packages: `sage` and `sageWithDoc`. The second one is just a wrapper around `sage` that sets `SAGE_DOC_SRC`.



---

archive/issue_comments_504230.json:
```json
{
    "body": "<a id='comment:22'></a>Replying to [jhpalmieri](#comment%3A19):\n> Replying to [arojas](#comment%3A17):\n> > > - If a user tries to search the documentation, should they be warned if the documentation is missing or incomplete?\n \n> > \n> > We ship docs as a separate, optional package, so such a warning would be quite useful.\n\n> \n> Does your main distribution include `SAGE_DOC_SRC`? If not, how can you meaningfully measure whether the documentation is present? My branch tries to address this; is it overkill?\n\n\nWe do define SAGE_DOC_SRC because it is still required by Sage at runtime (ideally it shouldn't). For us, simply checking whether $SAGE_DOC/html is present would be enough.",
    "created_at": "2018-09-10T21:20:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504230",
    "user": "https://github.com/antonio-rojas"
}
```

<a id='comment:22'></a>Replying to [jhpalmieri](#comment%3A19):
> Replying to [arojas](#comment%3A17):
> > > - If a user tries to search the documentation, should they be warned if the documentation is missing or incomplete?
 
> > 
> > We ship docs as a separate, optional package, so such a warning would be quite useful.

> 
> Does your main distribution include `SAGE_DOC_SRC`? If not, how can you meaningfully measure whether the documentation is present? My branch tries to address this; is it overkill?


We do define SAGE_DOC_SRC because it is still required by Sage at runtime (ideally it shouldn't). For us, simply checking whether $SAGE_DOC/html is present would be enough.



---

archive/issue_comments_504231.json:
```json
{
    "body": "<a id='comment:23'></a>We do define it too, but just a dummy value:\n\n```\nSAGE_DOC_SRC=${SAGE_DOC_SRC_OVERRIDE:-dummy}\n```\n\nWhere the wrapper may set `SAGE_DOC_SRC_OVERRIDE`.",
    "created_at": "2018-09-10T21:54:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504231",
    "user": "https://github.com/timokau"
}
```

<a id='comment:23'></a>We do define it too, but just a dummy value:

```
SAGE_DOC_SRC=${SAGE_DOC_SRC_OVERRIDE:-dummy}
```

Where the wrapper may set `SAGE_DOC_SRC_OVERRIDE`.



---

archive/issue_comments_504232.json:
```json
{
    "body": "<a id='comment:24'></a>Maybe you should rip out all of the stuff that Fran\u00e7ois mentions in[comment:3](#comment%3A3), and instead check for the presence of `SAGE_DOC/html`, printing a warning (but no instructions about how to build the docs) if that is missing. If you want to get fancy, then do it like Sage's deprecation warnings: only print the warning once per session. That could go on a separate ticket, though.\n\nI'm assuming that distributions will either include no documentation or will include (almost) all of it. With this approach, if for some reason someone decided to only include the stuff in Italian, then `search_doc` will not print any warnings but will not work well.",
    "created_at": "2018-09-10T22:30:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504232",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:24'></a>Maybe you should rip out all of the stuff that François mentions in[comment:3](#comment%3A3), and instead check for the presence of `SAGE_DOC/html`, printing a warning (but no instructions about how to build the docs) if that is missing. If you want to get fancy, then do it like Sage's deprecation warnings: only print the warning once per session. That could go on a separate ticket, though.

I'm assuming that distributions will either include no documentation or will include (almost) all of it. With this approach, if for some reason someone decided to only include the stuff in Italian, then `search_doc` will not print any warnings but will not work well.



---

archive/issue_comments_504233.json:
```json
{
    "body": "<a id='comment:25'></a>Replying to [jhpalmieri](#comment%3A24):\n> Maybe you should rip out all of the stuff that Fran\u00e7ois mentions in[comment:3](#comment%3A3), and instead check for the presence of `SAGE_DOC/html`, printing a warning (but no instructions about how to build the docs) if that is missing. If you want to get fancy, then do it like Sage's deprecation warnings: only print the warning once per session. That could go on a separate ticket, though.\n> \n> I'm assuming that distributions will either include no documentation or will include (almost) all of it. With this approach, if for some reason someone decided to only include the stuff in Italian, then `search_doc` will not print any warnings but will not work well.\n\n\nSorry for the delay. My job had to take priority over the last few days. It seems like arojas and timokau have a different concept of what should be tested and when compared to me and I suspect Debian. That is unexpected but not a show stopper to me. People should be able to do what they think they need to do unless it is outrageous in some ways.\n\nI would totally be in favor of John's last proposal. Rip this particular code and check whether the (or some) documentation is available when searching.",
    "created_at": "2018-09-13T02:11:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504233",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:25'></a>Replying to [jhpalmieri](#comment%3A24):
> Maybe you should rip out all of the stuff that François mentions in[comment:3](#comment%3A3), and instead check for the presence of `SAGE_DOC/html`, printing a warning (but no instructions about how to build the docs) if that is missing. If you want to get fancy, then do it like Sage's deprecation warnings: only print the warning once per session. That could go on a separate ticket, though.
> 
> I'm assuming that distributions will either include no documentation or will include (almost) all of it. With this approach, if for some reason someone decided to only include the stuff in Italian, then `search_doc` will not print any warnings but will not work well.


Sorry for the delay. My job had to take priority over the last few days. It seems like arojas and timokau have a different concept of what should be tested and when compared to me and I suspect Debian. That is unexpected but not a show stopper to me. People should be able to do what they think they need to do unless it is outrageous in some ways.

I would totally be in favor of John's last proposal. Rip this particular code and check whether the (or some) documentation is available when searching.



---

archive/issue_comments_504234.json:
```json
{
    "body": "Changing branch from \"u/jhpalmieri/make_search_doc_independent_of_sage_setup\" to \"u/arojas/make_search_doc_independent_of_sage_setup\"",
    "created_at": "2018-09-19T19:41:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504234",
    "user": "https://github.com/antonio-rojas"
}
```

Changing branch from "u/jhpalmieri/make_search_doc_independent_of_sage_setup" to "u/arojas/make_search_doc_independent_of_sage_setup"



---

archive/issue_comments_504235.json:
```json
{
    "body": "Changing commit from \"41bc9de9cc5e6407e770fc84414674135b691b39\" to \"b3abf9cc455f02c4d7b11244a863e6eb4fa607c7\"",
    "created_at": "2018-09-19T19:45:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504235",
    "user": "https://github.com/antonio-rojas"
}
```

Changing commit from "41bc9de9cc5e6407e770fc84414674135b691b39" to "b3abf9cc455f02c4d7b11244a863e6eb4fa607c7"



---

archive/issue_comments_504236.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,2 +1,2 @@\n-search_doc currently imports stuff from sage_setup, which shouldn't be needed by sage at runtime.\n-This patch defines the necessary variables directly so that search_doc works when sage_setup is not installed.\n+search_doc currently imports stuff from sage_setup, which shouldn't be needed by sage at runtime. It also relies on the sage doc source to be present, which is not necessarily true for distro packages.\n+This patch replaces the current exhaustive check that all translated docs have been built by a simple check that $SAGE_DOC/html exists.\n``````\n",
    "created_at": "2018-09-19T19:45:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504236",
    "user": "https://github.com/antonio-rojas"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,2 +1,2 @@
-search_doc currently imports stuff from sage_setup, which shouldn't be needed by sage at runtime.
-This patch defines the necessary variables directly so that search_doc works when sage_setup is not installed.
+search_doc currently imports stuff from sage_setup, which shouldn't be needed by sage at runtime. It also relies on the sage doc source to be present, which is not necessarily true for distro packages.
+This patch replaces the current exhaustive check that all translated docs have been built by a simple check that $SAGE_DOC/html exists.
``````




---

archive/issue_comments_504237.json:
```json
{
    "body": "<a id='comment:27'></a>New commits:",
    "created_at": "2018-09-19T19:45:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504237",
    "user": "https://github.com/antonio-rojas"
}
```

<a id='comment:27'></a>New commits:



---

archive/issue_comments_504238.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2018-09-22T10:29:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504238",
    "user": "https://github.com/antonio-rojas"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_504239.json:
```json
{
    "body": "<a id='comment:28'></a>Done as suggested in[comment:25](#comment%3A25)",
    "created_at": "2018-09-22T10:29:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504239",
    "user": "https://github.com/antonio-rojas"
}
```

<a id='comment:28'></a>Done as suggested in[comment:25](#comment%3A25)



---

archive/issue_comments_504240.json:
```json
{
    "body": "<a id='comment:29'></a>Good enough for me.",
    "created_at": "2018-09-22T10:32:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504240",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:29'></a>Good enough for me.



---

archive/issue_comments_504241.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Fran\u00e7ois Bissey\"",
    "created_at": "2018-09-22T10:32:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504241",
    "user": "https://github.com/kiwifb"
}
```

Changing reviewer from "" to "François Bissey"



---

archive/issue_comments_504242.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2018-09-22T10:32:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504242",
    "user": "https://github.com/kiwifb"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_504243.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2018-09-23T19:34:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504243",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_comments_504244.json:
```json
{
    "body": "Changing branch from \"u/arojas/make_search_doc_independent_of_sage_setup\" to \"b3abf9cc455f02c4d7b11244a863e6eb4fa607c7\"",
    "created_at": "2018-09-23T19:34:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26227#issuecomment-504244",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/arojas/make_search_doc_independent_of_sage_setup" to "b3abf9cc455f02c4d7b11244a863e6eb4fa607c7"



---

archive/issue_events_065302.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-09-23T19:34:12Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/26227",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/26227#event-65302"
}
```
