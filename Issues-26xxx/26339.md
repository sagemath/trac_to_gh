...
```json
{
    "body": "<a id='comment:2'></a>Unexpected (-1) factor in the numerator and denominator of a `sage.rings.fraction_field_element.FractionFieldElement`.\n\nSageMath version 9.1.beta3.\n\n```\nsage: P.<a,b,c>=QQ[]\nsage: f = 1/a/(b-a)/(c-a)+1/b/(a-b)/(c-b)+1/c/(a-c)/(b-c); f\n(-1)/(-a*b*c)\nsage: f.numerator()\n-1\nsage: f.denominator()\n-a*b*c\n```",
...
<a id='comment:2'></a>Unexpected (-1) factor in the numerator and denominator of a `sage.rings.fraction_field_element.FractionFieldElement`.
