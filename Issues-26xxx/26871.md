# Issue 26871: py3: fix doctests in digraph_generators.py

archive/issues_026634.json:
```json
{
    "assignees": [],
    "body": "- fix method `tournaments_nauty` using to `bytes_to_str`\n- fix doctest in method `RandomDirectedGNC` by avoiding to print the list of edges and testing that the digraph is acyclic instead\n- fix method `RandomDirectedGNR` that was calling `networkx.random_gnc` instead of `networkx.random_gnr`... and then fix the doctests by avoiding to print the list of edges and testing instead that the digraph is acyclic and is a tree\n\nCC:  @tscrim @fchapoton\n\nKeywords: **py3, graph**\n\nBranch/Commit: **[2c066f6](https://github.com/sagemath/sagetrac-mirror/commit/2c066f6a39fb0daaeb09d42896d1c48c24589028)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26871_\n\n",
    "closed_at": "2018-12-23T23:40:05Z",
    "created_at": "2018-12-11T03:15:08Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "py3: fix doctests in digraph_generators.py",
    "type": "issue",
    "updated_at": "2018-12-28T14:06:38Z",
    "url": "https://github.com/sagemath/sage/issues/26871",
    "user": "https://github.com/dcoudert"
}
```
- fix method `tournaments_nauty` using to `bytes_to_str`
- fix doctest in method `RandomDirectedGNC` by avoiding to print the list of edges and testing that the digraph is acyclic instead
- fix method `RandomDirectedGNR` that was calling `networkx.random_gnc` instead of `networkx.random_gnr`... and then fix the doctests by avoiding to print the list of edges and testing instead that the digraph is acyclic and is a tree

CC:  @tscrim @fchapoton

Keywords: **py3, graph**

Branch/Commit: **[2c066f6](https://github.com/sagemath/sagetrac-mirror/commit/2c066f6a39fb0daaeb09d42896d1c48c24589028)**

Reviewer: **Travis Scrimshaw**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/26871_





---

archive/issue_events_351047.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-12-11T03:15:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26871",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26871#event-351047"
}
```



---

archive/issue_events_351048.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-12-11T03:15:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26871",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26871#event-351048"
}
```



---

archive/issue_events_351049.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-12-11T03:15:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26871",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26871#event-351049"
}
```



---

archive/issue_events_351050.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-12-11T03:15:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26871",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26871#event-351050"
}
```



---

archive/issue_events_351051.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-12-11T03:17:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26871",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26871#event-351051"
}
```



---

archive/issue_comments_416595.json:
```json
{
    "body": "Branch: **[public/26871_digraph_generators](https://github.com/sagemath/sagetrac-mirror/tree/public/26871_digraph_generators)**",
    "created_at": "2018-12-11T03:17:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26871",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26871#issuecomment-416595",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[public/26871_digraph_generators](https://github.com/sagemath/sagetrac-mirror/tree/public/26871_digraph_generators)**



---

archive/issue_comments_416596.json:
```json
{
    "body": "Commit: **[2c066f6](https://github.com/sagemath/sagetrac-mirror/commit/2c066f6a39fb0daaeb09d42896d1c48c24589028)**",
    "created_at": "2018-12-11T03:17:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26871",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26871#issuecomment-416596",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[2c066f6](https://github.com/sagemath/sagetrac-mirror/commit/2c066f6a39fb0daaeb09d42896d1c48c24589028)**



---

archive/issue_comments_416597.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2c066f6a39fb0daaeb09d42896d1c48c24589028\">2c066f6</a></td><td><code>trac #26871: fix digraph generators</code></td></tr></table>\n",
    "created_at": "2018-12-11T03:17:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26871",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26871#issuecomment-416597",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2c066f6a39fb0daaeb09d42896d1c48c24589028">2c066f6</a></td><td><code>trac #26871: fix digraph generators</code></td></tr></table>




---

archive/issue_comments_416598.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nLGTM.",
    "created_at": "2018-12-12T02:35:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26871",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26871#issuecomment-416598",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:2" align="right">Comment 2</div>

LGTM.



---

archive/issue_comments_416599.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2018-12-12T02:35:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26871",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26871#issuecomment-416599",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_351052.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-12-12T02:35:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26871",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26871#event-351052"
}
```



---

archive/issue_events_351053.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-12-12T02:35:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26871",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26871#event-351053"
}
```



---

archive/issue_events_351054.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-12-23T23:40:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26871",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26871#event-351054"
}
```



---

archive/issue_events_351055.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "f6380728beb268888db9ac5f670a154a18a9819f",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-12-23T23:40:05Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26871",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26871#event-351055"
}
```



---

archive/issue_comments_416600.json:
```json
{
    "body": "Changed branch from **[public/26871_digraph_generators](https://github.com/sagemath/sagetrac-mirror/tree/public/26871_digraph_generators)** to **[2c066f6](https://github.com/sagemath/sagetrac-mirror/commit/2c066f6a39fb0daaeb09d42896d1c48c24589028)**",
    "created_at": "2018-12-23T23:40:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26871",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26871#issuecomment-416600",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/26871_digraph_generators](https://github.com/sagemath/sagetrac-mirror/tree/public/26871_digraph_generators)** to **[2c066f6](https://github.com/sagemath/sagetrac-mirror/commit/2c066f6a39fb0daaeb09d42896d1c48c24589028)**



---

archive/issue_comments_416601.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThis tickets were closed as fixed after the Sage 8.5 release.",
    "created_at": "2018-12-28T14:06:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26871",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26871#issuecomment-416601",
    "user": "https://github.com/embray"
}
```

<div id="comment:4" align="right">Comment 4</div>

This tickets were closed as fixed after the Sage 8.5 release.



---

archive/issue_events_351056.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-12-28T14:06:38Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/26871",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26871#event-351056"
}
```



---

archive/issue_events_351057.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-12-28T14:06:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26871",
    "milestone_number": null,
    "milestone_title": "sage-8.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26871#event-351057"
}
```
