# Issue 26281: curl-config requires the bc command to work

archive/issues_026044.json:
```json
{
    "body": "As we found out in #24919 (see #24919#comment:63) the `curl-config` program that comes with curl, which we use to check the curl version, requires the standard `bc` command, which apparently on some systems is missing?\n\nIn a way this just means that the packaging of `curl` is broken since it should list `bc` as a runtime dependency.  But we can work around that by checking for it ourselves.\n\nThis issue *should* be independent of #24919 since we already use `curl-config` to check for curl...\n\nCC:  @infinity0\n\nUpstream: Fixed upstream, in a later stable release.\n\nStatus: new\n\nIssue created by migration from https://trac.sagemath.org/ticket/26281\n\n",
    "created_at": "2018-09-14T10:57:41Z",
    "labels": [
        "component: packages: standard",
        "minor",
        "bug"
    ],
    "title": "curl-config requires the bc command to work",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/26281",
    "user": "https://github.com/embray"
}
```
As we found out in #24919 (see #24919#comment:63) the `curl-config` program that comes with curl, which we use to check the curl version, requires the standard `bc` command, which apparently on some systems is missing?

In a way this just means that the packaging of `curl` is broken since it should list `bc` as a runtime dependency.  But we can work around that by checking for it ourselves.

This issue *should* be independent of #24919 since we already use `curl-config` to check for curl...

CC:  @infinity0

Upstream: Fixed upstream, in a later stable release.

Status: new

Issue created by migration from https://trac.sagemath.org/ticket/26281





---

archive/issue_comments_505366.json:
```json
{
    "body": "Changing upstream from \"N/A\" to \"Not yet reported upstream; Will do shortly.\"",
    "created_at": "2018-10-15T10:25:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26281",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26281#issuecomment-505366",
    "user": "https://github.com/dimpase"
}
```

Changing upstream from "N/A" to "Not yet reported upstream; Will do shortly."



---

archive/issue_comments_505367.json:
```json
{
    "body": "<a id='comment:1'></a>This looks like a Debian bug, as I see this on Debian, and they package (lib)curl.\nBy right, `curl-config` they ship must be fully functional, but it misses `bc` as a dependency.\n\nI cc one of Debian people packaging Sagemath here.",
    "created_at": "2018-10-15T10:25:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26281",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26281#issuecomment-505367",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:1'></a>This looks like a Debian bug, as I see this on Debian, and they package (lib)curl.
By right, `curl-config` they ship must be fully functional, but it misses `bc` as a dependency.

I cc one of Debian people packaging Sagemath here.



---

archive/issue_comments_505368.json:
```json
{
    "body": "<a id='comment:2'></a>discussed with upstream on https://github.com/curl/curl/issues/3143\nand  upstream fixed it in https://github.com/curl/curl/pull/3174",
    "created_at": "2018-10-28T08:39:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26281",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26281#issuecomment-505368",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:2'></a>discussed with upstream on https://github.com/curl/curl/issues/3143
and  upstream fixed it in https://github.com/curl/curl/pull/3174



---

archive/issue_comments_505369.json:
```json
{
    "body": "Changing upstream from \"Not yet reported upstream; Will do shortly.\" to \"Fixed upstream, in a later stable release.\"",
    "created_at": "2018-10-28T08:39:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26281",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26281#issuecomment-505369",
    "user": "https://github.com/dimpase"
}
```

Changing upstream from "Not yet reported upstream; Will do shortly." to "Fixed upstream, in a later stable release."



---

archive/issue_comments_505370.json:
```json
{
    "body": "<a id='comment:3'></a>Thanks.",
    "created_at": "2018-10-28T11:35:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26281",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26281#issuecomment-505370",
    "user": "https://github.com/embray"
}
```

<a id='comment:3'></a>Thanks.



---

archive/issue_comments_505371.json:
```json
{
    "body": "<a id='comment:4'></a>I'm not sure what we should do in sage, if anything.  I suppose it might still be worth adding an explicit check for the bc program when installing curl :(",
    "created_at": "2018-10-28T11:36:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26281",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26281#issuecomment-505371",
    "user": "https://github.com/embray"
}
```

<a id='comment:4'></a>I'm not sure what we should do in sage, if anything.  I suppose it might still be worth adding an explicit check for the bc program when installing curl :(



---

archive/issue_comments_505372.json:
```json
{
    "body": "<a id='comment:5'></a>Replying to [comment:4 embray]:\n> I'm not sure what we should do in sage, if anything.  I suppose it might still be worth adding an explicit check for the bc program when installing curl :(\n\n\nwe could just make `bc` required in the top `configure.ac`, calling `AC_CHECK_PROGRAM` or whatever the right macro is.",
    "created_at": "2018-10-28T21:41:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26281",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26281#issuecomment-505372",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:5'></a>Replying to [comment:4 embray]:
> I'm not sure what we should do in sage, if anything.  I suppose it might still be worth adding an explicit check for the bc program when installing curl :(


we could just make `bc` required in the top `configure.ac`, calling `AC_CHECK_PROGRAM` or whatever the right macro is.



---

archive/issue_comments_505373.json:
```json
{
    "body": "<a id='comment:6'></a>The upstream fix was merged in `curl 7.62.0`.\n\nRules for deciding whether to build curl are\ndescribed in `build/pkgs/curl/spkg-configure.m4`.\n\nCurrently the approach is based on `R`'s needs:\n\n- build `curl` unless `curl >= 7.22` is found.\n\nWhich of the following should we change it to?\n\n- build `curl` unless `curl >= 7.62` is found\n- build `curl` unless either `curl >= 7.62` is found or `curl >= 7.22` and `bc` are found",
    "created_at": "2020-09-16T00:52:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26281",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26281#issuecomment-505373",
    "user": "https://github.com/slel"
}
```

<a id='comment:6'></a>The upstream fix was merged in `curl 7.62.0`.

Rules for deciding whether to build curl are
described in `build/pkgs/curl/spkg-configure.m4`.

Currently the approach is based on `R`'s needs:

- build `curl` unless `curl >= 7.22` is found.

Which of the following should we change it to?

- build `curl` unless `curl >= 7.62` is found
- build `curl` unless either `curl >= 7.62` is found or `curl >= 7.22` and `bc` are found



---

archive/issue_comments_505374.json:
```json
{
    "body": "<a id='comment:7'></a>Currently we do not build `curl` if `curl >= 7.22` (and, implcitly, `bc` - if the curl in question does need `bc` in its `curl-config`).\n\nAdding an explicit check for `bc` (after our test for curl failed) would only improve error messages, but that's a rare event that a system has no `bc` installed.\n\nOther potential improvements here would be to get rid of somewhat dodgy `LIBCURL_CHECK_CONFIG` and just use `PKG_CHECK_MODULES` (which doesn't need `bc`), or try `PKG_CHECK_MODULES`, and otherwise fall on `LIBCURL_CHECK_CONFIG`.\n\nBut all this is very minor.",
    "created_at": "2020-09-16T08:26:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26281",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26281#issuecomment-505374",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:7'></a>Currently we do not build `curl` if `curl >= 7.22` (and, implcitly, `bc` - if the curl in question does need `bc` in its `curl-config`).

Adding an explicit check for `bc` (after our test for curl failed) would only improve error messages, but that's a rare event that a system has no `bc` installed.

Other potential improvements here would be to get rid of somewhat dodgy `LIBCURL_CHECK_CONFIG` and just use `PKG_CHECK_MODULES` (which doesn't need `bc`), or try `PKG_CHECK_MODULES`, and otherwise fall on `LIBCURL_CHECK_CONFIG`.

But all this is very minor.



---

archive/issue_comments_505375.json:
```json
{
    "body": "Changing priority from major to minor.",
    "created_at": "2020-09-16T08:26:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26281",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26281#issuecomment-505375",
    "user": "https://github.com/dimpase"
}
```

Changing priority from major to minor.



---

archive/issue_comments_505376.json:
```json
{
    "body": "<a id='comment:8'></a>So maybe:\n- check for curl >= 7.62: if found, good\n- if not found, check for curl >= 7.22 and bc: if found, good\n- if not found: build curl",
    "created_at": "2020-09-16T18:50:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26281",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26281#issuecomment-505376",
    "user": "https://github.com/slel"
}
```

<a id='comment:8'></a>So maybe:
- check for curl >= 7.62: if found, good
- if not found, check for curl >= 7.22 and bc: if found, good
- if not found: build curl



---

archive/issue_comments_505377.json:
```json
{
    "body": "<a id='comment:9'></a>Replying to [comment:8 slelievre]:\n> So maybe:\n> - check for curl >= 7.62: if found, good\n> - if not found, check for curl >= 7.22 and bc: if found, good\n> - if not found: build curl\n\n\nplease see my edits",
    "created_at": "2020-09-16T20:14:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/26281",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/26281#issuecomment-505377",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:9'></a>Replying to [comment:8 slelievre]:
> So maybe:
> - check for curl >= 7.62: if found, good
> - if not found, check for curl >= 7.22 and bc: if found, good
> - if not found: build curl


please see my edits
