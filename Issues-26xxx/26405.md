# Issue 26405: Root finding for p-adic polynomials

archive/issues_026168.json:
```json
{
    "assignees": [],
    "body": "This ticket implements root finding for polynomials defined over any `p`-adic ring or field. So far, it was implemented only over `Zp` and `Qp` (using `pari`).\n\nCC:  @roed314\n\nBranch/Commit: **[d599b0f](https://github.com/sagemath/sagetrac-mirror/commit/d599b0f13debdb9cb6ea839c10c879780132417e)**\n\nReviewer: **David Roe**\n\nAuthor: **Xavier Caruso**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26405_\n\n",
    "closed_at": "2018-11-22T21:37:09Z",
    "created_at": "2018-10-04T22:55:58Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20padics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Root finding for p-adic polynomials",
    "type": "issue",
    "updated_at": "2018-11-22T21:37:09Z",
    "url": "https://github.com/sagemath/sage/issues/26405",
    "user": "https://github.com/xcaruso"
}
```
This ticket implements root finding for polynomials defined over any `p`-adic ring or field. So far, it was implemented only over `Zp` and `Qp` (using `pari`).

CC:  @roed314

Branch/Commit: **[d599b0f](https://github.com/sagemath/sagetrac-mirror/commit/d599b0f13debdb9cb6ea839c10c879780132417e)**

Reviewer: **David Roe**

Author: **Xavier Caruso**

_Issue created by migration from https://trac.sagemath.org/ticket/26405_





---

archive/issue_events_345739.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2018-10-04T22:55:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26405",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26405#event-345739"
}
```



---

archive/issue_events_345740.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2018-10-04T22:55:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26405",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20padics",
    "label_color": "0000ff",
    "label_name": "component: padics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26405#event-345740"
}
```



---

archive/issue_events_345741.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2018-10-04T22:55:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26405",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26405#event-345741"
}
```



---

archive/issue_events_345742.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2018-10-04T22:55:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26405",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26405#event-345742"
}
```



---

archive/issue_comments_409839.json:
```json
{
    "body": "Branch: **[u/caruso/root_finding_for_p_adic_polynomials](https://github.com/sagemath/sagetrac-mirror/tree/u/caruso/root_finding_for_p_adic_polynomials)**",
    "created_at": "2018-10-05T00:45:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26405",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26405#issuecomment-409839",
    "user": "https://github.com/xcaruso"
}
```

Branch: **[u/caruso/root_finding_for_p_adic_polynomials](https://github.com/sagemath/sagetrac-mirror/tree/u/caruso/root_finding_for_p_adic_polynomials)**



---

archive/issue_comments_409840.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/56268f0ea1030f4c12575d6690a3e1e97b220171\">56268f0</a></td><td><code>Fix stupid bug</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8b79b4bb6e9c5d3c559d3f7afbecee558393f799\">8b79b4b</a></td><td><code>Doctests</code></td></tr></table>\n",
    "created_at": "2018-10-07T20:01:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26405",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26405#issuecomment-409840",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/56268f0ea1030f4c12575d6690a3e1e97b220171">56268f0</a></td><td><code>Fix stupid bug</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8b79b4bb6e9c5d3c559d3f7afbecee558393f799">8b79b4b</a></td><td><code>Doctests</code></td></tr></table>




---

archive/issue_comments_409841.json:
```json
{
    "body": "Commit: **[8b79b4b](https://github.com/sagemath/sagetrac-mirror/commit/8b79b4bb6e9c5d3c559d3f7afbecee558393f799)**",
    "created_at": "2018-10-07T20:01:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26405",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26405#issuecomment-409841",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[8b79b4b](https://github.com/sagemath/sagetrac-mirror/commit/8b79b4bb6e9c5d3c559d3f7afbecee558393f799)**



---

archive/issue_comments_409842.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1535dfa6945b0efb9d9668028cefd794922588c4\">1535dfa</a></td><td><code>Typo</code></td></tr></table>\n",
    "created_at": "2018-10-07T20:05:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26405",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26405#issuecomment-409842",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1535dfa6945b0efb9d9668028cefd794922588c4">1535dfa</a></td><td><code>Typo</code></td></tr></table>




---

archive/issue_comments_409843.json:
```json
{
    "body": "Changed commit from **[8b79b4b](https://github.com/sagemath/sagetrac-mirror/commit/8b79b4bb6e9c5d3c559d3f7afbecee558393f799)** to **[1535dfa](https://github.com/sagemath/sagetrac-mirror/commit/1535dfa6945b0efb9d9668028cefd794922588c4)**",
    "created_at": "2018-10-07T20:05:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26405",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26405#issuecomment-409843",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[8b79b4b](https://github.com/sagemath/sagetrac-mirror/commit/8b79b4bb6e9c5d3c559d3f7afbecee558393f799)** to **[1535dfa](https://github.com/sagemath/sagetrac-mirror/commit/1535dfa6945b0efb9d9668028cefd794922588c4)**



---

archive/issue_comments_409844.json:
```json
{
    "body": "Author: **Xavier Caruso**",
    "created_at": "2018-10-07T20:06:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26405",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26405#issuecomment-409844",
    "user": "https://github.com/xcaruso"
}
```

Author: **Xavier Caruso**



---

archive/issue_events_345743.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2018-10-07T20:06:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26405",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26405#event-345743"
}
```



---

archive/issue_comments_409845.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-I propose to implement a method `roots()` for polynomials defined over `p`-adic rings/fields. So far, it is implemented only over `Zp` and `Qp` (and calls Pari).\n+This ticket implements root finding for polynomials defined over any `p`-adic ring or field. So far, it was implemented only over `Zp` and `Qp` (using `pari`).\n``````\n",
    "created_at": "2018-10-07T20:06:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26405",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26405#issuecomment-409845",
    "user": "https://github.com/xcaruso"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-I propose to implement a method `roots()` for polynomials defined over `p`-adic rings/fields. So far, it is implemented only over `Zp` and `Qp` (and calls Pari).
+This ticket implements root finding for polynomials defined over any `p`-adic ring or field. So far, it was implemented only over `Zp` and `Qp` (using `pari`).
``````




---

archive/issue_comments_409846.json:
```json
{
    "body": "Changed commit from **[1535dfa](https://github.com/sagemath/sagetrac-mirror/commit/1535dfa6945b0efb9d9668028cefd794922588c4)** to **[c008a4d](https://github.com/sagemath/sagetrac-mirror/commit/c008a4da20f04b11f8f28483c17953bb8ec9eccb)**",
    "created_at": "2018-10-08T22:06:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26405",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26405#issuecomment-409846",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[1535dfa](https://github.com/sagemath/sagetrac-mirror/commit/1535dfa6945b0efb9d9668028cefd794922588c4)** to **[c008a4d](https://github.com/sagemath/sagetrac-mirror/commit/c008a4da20f04b11f8f28483c17953bb8ec9eccb)**



---

archive/issue_comments_409847.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c008a4da20f04b11f8f28483c17953bb8ec9eccb\">c008a4d</a></td><td><code>Call pari through the method factor()</code></td></tr></table>\n",
    "created_at": "2018-10-08T22:06:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26405",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26405#issuecomment-409847",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:5" align="right">Comment 5</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c008a4da20f04b11f8f28483c17953bb8ec9eccb">c008a4d</a></td><td><code>Call pari through the method factor()</code></td></tr></table>




---

archive/issue_comments_409848.json:
```json
{
    "body": "Changed commit from **[c008a4d](https://github.com/sagemath/sagetrac-mirror/commit/c008a4da20f04b11f8f28483c17953bb8ec9eccb)** to **[b8d38d9](https://github.com/sagemath/sagetrac-mirror/commit/b8d38d92d54ac3b3682e9e49a52f8b01dc358c71)**",
    "created_at": "2018-10-10T06:29:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26405",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26405#issuecomment-409848",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[c008a4d](https://github.com/sagemath/sagetrac-mirror/commit/c008a4da20f04b11f8f28483c17953bb8ec9eccb)** to **[b8d38d9](https://github.com/sagemath/sagetrac-mirror/commit/b8d38d92d54ac3b3682e9e49a52f8b01dc358c71)**



---

archive/issue_comments_409849.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b8d38d92d54ac3b3682e9e49a52f8b01dc358c71\">b8d38d9</a></td><td><code>Merge branch 'develop' into t/26405/root_finding_for_p_adic_polynomials</code></td></tr></table>\n",
    "created_at": "2018-10-10T06:29:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26405",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26405#issuecomment-409849",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b8d38d92d54ac3b3682e9e49a52f8b01dc358c71">b8d38d9</a></td><td><code>Merge branch 'develop' into t/26405/root_finding_for_p_adic_polynomials</code></td></tr></table>




---

archive/issue_comments_409850.json:
```json
{
    "body": "Changed branch from **[u/caruso/root_finding_for_p_adic_polynomials](https://github.com/sagemath/sagetrac-mirror/tree/u/caruso/root_finding_for_p_adic_polynomials)** to **[u/roed/root_finding_for_p_adic_polynomials](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/root_finding_for_p_adic_polynomials)**",
    "created_at": "2018-11-21T03:37:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26405",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26405#issuecomment-409850",
    "user": "https://github.com/roed314"
}
```

Changed branch from **[u/caruso/root_finding_for_p_adic_polynomials](https://github.com/sagemath/sagetrac-mirror/tree/u/caruso/root_finding_for_p_adic_polynomials)** to **[u/roed/root_finding_for_p_adic_polynomials](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/root_finding_for_p_adic_polynomials)**



---

archive/issue_comments_409851.json:
```json
{
    "body": "Changed commit from **[b8d38d9](https://github.com/sagemath/sagetrac-mirror/commit/b8d38d92d54ac3b3682e9e49a52f8b01dc358c71)** to **[d599b0f](https://github.com/sagemath/sagetrac-mirror/commit/d599b0f13debdb9cb6ea839c10c879780132417e)**",
    "created_at": "2018-11-21T03:39:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26405",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26405#issuecomment-409851",
    "user": "https://github.com/roed314"
}
```

Changed commit from **[b8d38d9](https://github.com/sagemath/sagetrac-mirror/commit/b8d38d92d54ac3b3682e9e49a52f8b01dc358c71)** to **[d599b0f](https://github.com/sagemath/sagetrac-mirror/commit/d599b0f13debdb9cb6ea839c10c879780132417e)**



---

archive/issue_comments_409852.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nI fixed a few typos and some pyflakes errors.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c278bf5b5884a568cf4d8c22afcc34a3c6f0b811\">c278bf5</a></td><td><code>Merge branch 'u/caruso/root_finding_for_p_adic_polynomials' of git://trac.sagemath.org/sage into t/26405/root_finding_for_p_adic_polynomials</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d599b0f13debdb9cb6ea839c10c879780132417e\">d599b0f</a></td><td><code>Fixing pyflakes, fixing a few typos</code></td></tr></table>\n",
    "created_at": "2018-11-21T03:39:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26405",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26405#issuecomment-409852",
    "user": "https://github.com/roed314"
}
```

<div id="comment:8" align="right">Comment 8</div>

I fixed a few typos and some pyflakes errors.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c278bf5b5884a568cf4d8c22afcc34a3c6f0b811">c278bf5</a></td><td><code>Merge branch 'u/caruso/root_finding_for_p_adic_polynomials' of git://trac.sagemath.org/sage into t/26405/root_finding_for_p_adic_polynomials</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d599b0f13debdb9cb6ea839c10c879780132417e">d599b0f</a></td><td><code>Fixing pyflakes, fixing a few typos</code></td></tr></table>




---

archive/issue_comments_409853.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nThis looks good.",
    "created_at": "2018-11-21T03:40:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26405",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26405#issuecomment-409853",
    "user": "https://github.com/roed314"
}
```

<div id="comment:9" align="right">Comment 9</div>

This looks good.



---

archive/issue_events_345744.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2018-11-21T03:40:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26405",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26405#event-345744"
}
```



---

archive/issue_events_345745.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2018-11-21T03:40:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26405",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26405#event-345745"
}
```



---

archive/issue_comments_409854.json:
```json
{
    "body": "Reviewer: **David Roe**",
    "created_at": "2018-11-21T03:40:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26405",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26405#issuecomment-409854",
    "user": "https://github.com/roed314"
}
```

Reviewer: **David Roe**



---

archive/issue_events_345746.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-11-22T20:03:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/26405",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26405#event-345746"
}
```



---

archive/issue_events_345747.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-11-22T20:03:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26405",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26405#event-345747"
}
```



---

archive/issue_events_345748.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-11-22T21:37:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26405",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26405#event-345748"
}
```



---

archive/issue_events_345749.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "b054b079e5a4a076c4bc53d928e3ae3163059858",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-11-22T21:37:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26405",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26405#event-345749"
}
```



---

archive/issue_comments_409855.json:
```json
{
    "body": "Changed branch from **[u/roed/root_finding_for_p_adic_polynomials](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/root_finding_for_p_adic_polynomials)** to **[d599b0f](https://github.com/sagemath/sagetrac-mirror/commit/d599b0f13debdb9cb6ea839c10c879780132417e)**",
    "created_at": "2018-11-22T21:37:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26405",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26405#issuecomment-409855",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/roed/root_finding_for_p_adic_polynomials](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/root_finding_for_p_adic_polynomials)** to **[d599b0f](https://github.com/sagemath/sagetrac-mirror/commit/d599b0f13debdb9cb6ea839c10c879780132417e)**
