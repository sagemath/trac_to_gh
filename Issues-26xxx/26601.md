# Issue 26601: py3: make all tests in src/sage/tensor passed

archive/issues_026364.json:
```json
{
    "assignees": [],
    "body": "The parallelisation of the function `a.__mul__(a)` in `src/sage/tensor/modules/comp.py` was only one test failing with `python3` in `src/sage/tensor`.\nPreviously, with `python2.7`, to create the partial result we used \na list comprehension but now it fails.\nWe have to replace it with a for loop.\n\nCC:  @egourgoulhon @fchapoton @tscrim\n\nKeywords: **python3, @parallel, pickle**\n\nBranch/Commit: **[2a481f1](https://github.com/sagemath/sagetrac-mirror/commit/2a481f131e50f8a09567f79b0d3e8a9424bdfedd)**\n\nReviewer: **Eric Gourgoulhon**\n\nAuthor: **Marco Mancini**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26601_\n\n",
    "closed_at": "2018-10-31T08:03:05Z",
    "created_at": "2018-10-30T13:41:13Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "py3: make all tests in src/sage/tensor passed",
    "type": "issue",
    "updated_at": "2018-10-31T08:03:05Z",
    "url": "https://github.com/sagemath/sage/issues/26601",
    "user": "https://github.com/man74cio"
}
```
The parallelisation of the function `a.__mul__(a)` in `src/sage/tensor/modules/comp.py` was only one test failing with `python3` in `src/sage/tensor`.
Previously, with `python2.7`, to create the partial result we used 
a list comprehension but now it fails.
We have to replace it with a for loop.

CC:  @egourgoulhon @fchapoton @tscrim

Keywords: **python3, @parallel, pickle**

Branch/Commit: **[2a481f1](https://github.com/sagemath/sagetrac-mirror/commit/2a481f131e50f8a09567f79b0d3e8a9424bdfedd)**

Reviewer: **Eric Gourgoulhon**

Author: **Marco Mancini**

_Issue created by migration from https://trac.sagemath.org/ticket/26601_





---

archive/issue_events_348013.json:
```json
{
    "actor": "https://github.com/man74cio",
    "created_at": "2018-10-30T13:41:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26601",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26601#event-348013"
}
```



---

archive/issue_events_348014.json:
```json
{
    "actor": "https://github.com/man74cio",
    "created_at": "2018-10-30T13:41:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26601",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26601#event-348014"
}
```



---

archive/issue_events_348015.json:
```json
{
    "actor": "https://github.com/man74cio",
    "created_at": "2018-10-30T13:41:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26601",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26601#event-348015"
}
```



---

archive/issue_comments_412800.json:
```json
{
    "body": "Changed commit from **[fbf07a6](https://github.com/sagemath/sagetrac-mirror/commit/fbf07a6fe28f4f89c77dbee16b75988e09de051f)** to **[2a481f1](https://github.com/sagemath/sagetrac-mirror/commit/2a481f131e50f8a09567f79b0d3e8a9424bdfedd)**",
    "created_at": "2018-10-30T13:43:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26601#issuecomment-412800",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[fbf07a6](https://github.com/sagemath/sagetrac-mirror/commit/fbf07a6fe28f4f89c77dbee16b75988e09de051f)** to **[2a481f1](https://github.com/sagemath/sagetrac-mirror/commit/2a481f131e50f8a09567f79b0d3e8a9424bdfedd)**



---

archive/issue_comments_412801.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2a481f131e50f8a09567f79b0d3e8a9424bdfedd\">2a481f1</a></td><td><code>Changed name</code></td></tr></table>\n",
    "created_at": "2018-10-30T13:43:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26601#issuecomment-412801",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:1" align="right">Comment 1</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2a481f131e50f8a09567f79b0d3e8a9424bdfedd">2a481f1</a></td><td><code>Changed name</code></td></tr></table>




---

archive/issue_comments_412802.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-The parallelisation of the function `a.__mul__(a)` was only one test failing with `python3` in tensor/modules.\n+The parallelisation of the function `a.__mul__(a)` in `src/sage/tensor/modules/comp.py` was only one test failing with `python3` in `src/sage/tensor`.\n Previously, with `python2.7`, to create the partial result we used \n a list comprehension but now it fails.\n We have to replace it with a for loop.\n``````\n",
    "created_at": "2018-10-30T14:00:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26601#issuecomment-412802",
    "user": "https://github.com/egourgoulhon"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-The parallelisation of the function `a.__mul__(a)` was only one test failing with `python3` in tensor/modules.
+The parallelisation of the function `a.__mul__(a)` in `src/sage/tensor/modules/comp.py` was only one test failing with `python3` in `src/sage/tensor`.
 Previously, with `python2.7`, to create the partial result we used 
 a list comprehension but now it fails.
 We have to replace it with a for loop.
``````




---

archive/issue_events_348016.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2018-10-30T14:00:06Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/26601",
    "title_is": "py3: make all tests in src/sage/tensor passed",
    "title_was": "Make all doctests in tensor/modules passed with Python 3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26601#event-348016"
}
```



---

archive/issue_events_348017.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2018-10-30T14:00:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26601",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "component: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26601#event-348017"
}
```



---

archive/issue_events_348018.json:
```json
{
    "actor": "https://github.com/man74cio",
    "created_at": "2018-10-30T14:05:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26601",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26601#event-348018"
}
```



---

archive/issue_events_348019.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2018-10-30T14:10:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26601",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26601#event-348019"
}
```



---

archive/issue_events_348020.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2018-10-30T14:10:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26601",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26601#event-348020"
}
```



---

archive/issue_comments_412803.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThank you Marco!\n\nI've checked the changes with both Python2 and Python3.\nIn particular, with Python3, `sage -tp --long src/sage/tensor/` returns now \"All tests passed!\".",
    "created_at": "2018-10-30T14:10:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26601#issuecomment-412803",
    "user": "https://github.com/egourgoulhon"
}
```

<div id="comment:4" align="right">Comment 4</div>

Thank you Marco!

I've checked the changes with both Python2 and Python3.
In particular, with Python3, `sage -tp --long src/sage/tensor/` returns now "All tests passed!".



---

archive/issue_comments_412804.json:
```json
{
    "body": "Reviewer: **Eric Gourgoulhon**",
    "created_at": "2018-10-30T14:10:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26601#issuecomment-412804",
    "user": "https://github.com/egourgoulhon"
}
```

Reviewer: **Eric Gourgoulhon**



---

archive/issue_events_348021.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-10-31T08:03:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26601",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26601#event-348021"
}
```



---

archive/issue_events_348022.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "fd19ff3518940dd52e323a3aa57cacfed24784b3",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-10-31T08:03:05Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26601",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26601#event-348022"
}
```



---

archive/issue_comments_412805.json:
```json
{
    "body": "Changed branch from **[public/manifolds/To_python3_parellisation_prob_in_tensor_comp](https://github.com/sagemath/sagetrac-mirror/tree/public/manifolds/To_python3_parellisation_prob_in_tensor_comp)** to **[2a481f1](https://github.com/sagemath/sagetrac-mirror/commit/2a481f131e50f8a09567f79b0d3e8a9424bdfedd)**",
    "created_at": "2018-10-31T08:03:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26601#issuecomment-412805",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/manifolds/To_python3_parellisation_prob_in_tensor_comp](https://github.com/sagemath/sagetrac-mirror/tree/public/manifolds/To_python3_parellisation_prob_in_tensor_comp)** to **[2a481f1](https://github.com/sagemath/sagetrac-mirror/commit/2a481f131e50f8a09567f79b0d3e8a9424bdfedd)**
