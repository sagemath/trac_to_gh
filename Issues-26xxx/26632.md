# Issue 26632: improve method distance_matrix

archive/issues_026395.json:
```json
{
    "assignees": [],
    "body": "1. correct the behavior of the method for digraphs. Before, `M[i, j]` was either the distance from i to j or the distance from j to i. It is now the shortest path distance from i to j.\n2. add parameter `vertices` to specify the mapping vertices to consecutive integers\n3. allow to specify the parameters of method `distance_all_pairs`\n\nCC:  @tscrim @fchapoton\n\nKeywords: **py3, graph**\n\nBranch/Commit: **[824f694](https://github.com/sagemath/sagetrac-mirror/commit/824f6946c8038c2ec1b74696fbc8a2c45dfde143)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26632_\n\n",
    "closed_at": "2018-11-07T11:27:38Z",
    "created_at": "2018-11-04T11:03:07Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "improve method distance_matrix",
    "type": "issue",
    "updated_at": "2018-11-07T11:27:38Z",
    "url": "https://github.com/sagemath/sage/issues/26632",
    "user": "https://github.com/dcoudert"
}
```
1. correct the behavior of the method for digraphs. Before, `M[i, j]` was either the distance from i to j or the distance from j to i. It is now the shortest path distance from i to j.
2. add parameter `vertices` to specify the mapping vertices to consecutive integers
3. allow to specify the parameters of method `distance_all_pairs`

CC:  @tscrim @fchapoton

Keywords: **py3, graph**

Branch/Commit: **[824f694](https://github.com/sagemath/sagetrac-mirror/commit/824f6946c8038c2ec1b74696fbc8a2c45dfde143)**

Reviewer: **Travis Scrimshaw**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/26632_





---

archive/issue_events_348395.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-04T11:03:07Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26632",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26632#event-348395"
}
```



---

archive/issue_events_348396.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-04T11:03:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26632",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26632#event-348396"
}
```



---

archive/issue_events_348397.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-04T11:03:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26632",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26632#event-348397"
}
```



---

archive/issue_events_348398.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-04T11:03:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26632",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26632#event-348398"
}
```



---

archive/issue_events_348399.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-04T11:07:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26632",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26632#event-348399"
}
```



---

archive/issue_comments_413307.json:
```json
{
    "body": "Commit: **[e037375](https://github.com/sagemath/sagetrac-mirror/commit/e0373755391f73f11d33fdacb1577ff2be2ecde1)**",
    "created_at": "2018-11-04T11:07:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26632",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26632#issuecomment-413307",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[e037375](https://github.com/sagemath/sagetrac-mirror/commit/e0373755391f73f11d33fdacb1577ff2be2ecde1)**



---

archive/issue_comments_413308.json:
```json
{
    "body": "Branch: **[public/26632_distance_matrix](https://github.com/sagemath/sagetrac-mirror/tree/public/26632_distance_matrix)**",
    "created_at": "2018-11-04T11:07:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26632",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26632#issuecomment-413308",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[public/26632_distance_matrix](https://github.com/sagemath/sagetrac-mirror/tree/public/26632_distance_matrix)**



---

archive/issue_comments_413309.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e0373755391f73f11d33fdacb1577ff2be2ecde1\">e037375</a></td><td><code>trac #26632: distance_matrix</code></td></tr></table>\n",
    "created_at": "2018-11-04T11:07:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26632",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26632#issuecomment-413309",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e0373755391f73f11d33fdacb1577ff2be2ecde1">e037375</a></td><td><code>trac #26632: distance_matrix</code></td></tr></table>




---

archive/issue_comments_413310.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\n`it's` -> `its`.\n\nTo get the correct links, you probably want <code>:meth:\\`~sage.graphs.generic_graph.GenericGraph.foo\\`</code> as `GenericGraph` is not imported into the global namespace; although since it within that class, I think you can just do <code>:meth:\\`foo\\`</code>.\n\nLGTM otherwise and you can set a positive review once fixed.",
    "created_at": "2018-11-04T12:12:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26632",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26632#issuecomment-413310",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:2" align="right">Comment 2</div>

`it's` -> `its`.

To get the correct links, you probably want <code>:meth:\`~sage.graphs.generic_graph.GenericGraph.foo\`</code> as `GenericGraph` is not imported into the global namespace; although since it within that class, I think you can just do <code>:meth:\`foo\`</code>.

LGTM otherwise and you can set a positive review once fixed.



---

archive/issue_comments_413311.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2018-11-04T12:12:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26632",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26632#issuecomment-413311",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_413312.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/824f6946c8038c2ec1b74696fbc8a2c45dfde143\">824f694</a></td><td><code>trac #26632: reviewer's comments</code></td></tr></table>\n",
    "created_at": "2018-11-04T14:29:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26632",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26632#issuecomment-413312",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/824f6946c8038c2ec1b74696fbc8a2c45dfde143">824f694</a></td><td><code>trac #26632: reviewer's comments</code></td></tr></table>




---

archive/issue_comments_413313.json:
```json
{
    "body": "Changed commit from **[e037375](https://github.com/sagemath/sagetrac-mirror/commit/e0373755391f73f11d33fdacb1577ff2be2ecde1)** to **[824f694](https://github.com/sagemath/sagetrac-mirror/commit/824f6946c8038c2ec1b74696fbc8a2c45dfde143)**",
    "created_at": "2018-11-04T14:29:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26632",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26632#issuecomment-413313",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[e037375](https://github.com/sagemath/sagetrac-mirror/commit/e0373755391f73f11d33fdacb1577ff2be2ecde1)** to **[824f694](https://github.com/sagemath/sagetrac-mirror/commit/824f6946c8038c2ec1b74696fbc8a2c45dfde143)**



---

archive/issue_comments_413314.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThanks for the review.\n\nFor the link, the form <code>:meth:\\`foo\\`</code> is working properly. I'm regularly building the html doc to check.",
    "created_at": "2018-11-04T14:30:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26632",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26632#issuecomment-413314",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:4" align="right">Comment 4</div>

Thanks for the review.

For the link, the form <code>:meth:\`foo\`</code> is working properly. I'm regularly building the html doc to check.



---

archive/issue_events_348400.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-04T14:30:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26632",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26632#event-348400"
}
```



---

archive/issue_events_348401.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-11-04T14:30:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26632",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26632#event-348401"
}
```



---

archive/issue_events_348402.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-11-07T11:27:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26632",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26632#event-348402"
}
```



---

archive/issue_events_348403.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "e2e31f3d8c01f04fc344f7322aeb11bc5e4a9321",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-11-07T11:27:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26632",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26632#event-348403"
}
```



---

archive/issue_comments_413315.json:
```json
{
    "body": "Changed branch from **[public/26632_distance_matrix](https://github.com/sagemath/sagetrac-mirror/tree/public/26632_distance_matrix)** to **[824f694](https://github.com/sagemath/sagetrac-mirror/commit/824f6946c8038c2ec1b74696fbc8a2c45dfde143)**",
    "created_at": "2018-11-07T11:27:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26632",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26632#issuecomment-413315",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/26632_distance_matrix](https://github.com/sagemath/sagetrac-mirror/tree/public/26632_distance_matrix)** to **[824f694](https://github.com/sagemath/sagetrac-mirror/commit/824f6946c8038c2ec1b74696fbc8a2c45dfde143)**
