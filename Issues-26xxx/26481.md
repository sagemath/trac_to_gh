...
```
CircleCI and [GitLab](../wikis/GitLab) CI currently both build-from-latest using `sagemath/sagemath-dev:develop`. This leads to annoying errors when building from older branches as the build process won't find the commit that `sagemath-dev:develop` was built from in that branch.
