# Issue 26940: stop sorting returned list of edges in spanning tree methods

archive/issues_026703.json:
```json
{
    "assignees": [],
    "body": "Many minimum spanning tree methods sort edges before returning the result, but not all (e.g., `boruvka`). This ticket is a trial to avoid that.\n\n\n\n\nCC:  @tscrim @fchapoton @jhpalmieri @jdemeyer\n\nKeywords: **py3, graph**\n\nBranch/Commit: **[0edec09](https://github.com/sagemath/sagetrac-mirror/commit/0edec0905bec4630d2e247fedc87f72ecfae50d6)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26940_\n\n",
    "closed_at": "2018-12-31T10:37:03Z",
    "created_at": "2018-12-22T11:02:17Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "stop sorting returned list of edges in spanning tree methods",
    "type": "issue",
    "updated_at": "2018-12-31T10:37:03Z",
    "url": "https://github.com/sagemath/sage/issues/26940",
    "user": "https://github.com/dcoudert"
}
```
Many minimum spanning tree methods sort edges before returning the result, but not all (e.g., `boruvka`). This ticket is a trial to avoid that.




CC:  @tscrim @fchapoton @jhpalmieri @jdemeyer

Keywords: **py3, graph**

Branch/Commit: **[0edec09](https://github.com/sagemath/sagetrac-mirror/commit/0edec0905bec4630d2e247fedc87f72ecfae50d6)**

Reviewer: **Frédéric Chapoton**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/26940_





---

archive/issue_events_352029.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-12-22T11:02:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26940",
    "milestone_number": null,
    "milestone_title": "sage-8.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26940#event-352029"
}
```



---

archive/issue_events_352030.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-12-22T11:02:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26940",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26940#event-352030"
}
```



---

archive/issue_events_352031.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-12-22T11:02:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26940",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26940#event-352031"
}
```



---

archive/issue_events_352032.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-12-22T11:02:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26940",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26940#event-352032"
}
```



---

archive/issue_comments_418137.json:
```json
{
    "body": "Branch: **[public/26940_no_sorting_in_spanning_tree](https://github.com/sagemath/sagetrac-mirror/tree/public/26940_no_sorting_in_spanning_tree)**",
    "created_at": "2018-12-22T11:05:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26940#issuecomment-418137",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[public/26940_no_sorting_in_spanning_tree](https://github.com/sagemath/sagetrac-mirror/tree/public/26940_no_sorting_in_spanning_tree)**



---

archive/issue_events_352033.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2018-12-22T11:05:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26940",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26940#event-352033"
}
```



---

archive/issue_comments_418138.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,4 @@\n Many minimum spanning tree methods sort edges before returning the result, but not all (e.g., `boruvka`). This ticket is a trial to avoid that.\n \n \n+\n``````\n",
    "created_at": "2018-12-22T11:05:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26940#issuecomment-418138",
    "user": "https://github.com/dcoudert"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,4 @@
 Many minimum spanning tree methods sort edges before returning the result, but not all (e.g., `boruvka`). This ticket is a trial to avoid that.
 
 
+
``````




---

archive/issue_comments_418139.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0edec0905bec4630d2e247fedc87f72ecfae50d6\">0edec09</a></td><td><code>trac #26940: stop sorting edges in minimum spanning tree</code></td></tr></table>\n",
    "created_at": "2018-12-22T11:05:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26940#issuecomment-418139",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0edec0905bec4630d2e247fedc87f72ecfae50d6">0edec09</a></td><td><code>trac #26940: stop sorting edges in minimum spanning tree</code></td></tr></table>




---

archive/issue_comments_418140.json:
```json
{
    "body": "Commit: **[0edec09](https://github.com/sagemath/sagetrac-mirror/commit/0edec0905bec4630d2e247fedc87f72ecfae50d6)**",
    "created_at": "2018-12-22T11:05:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26940#issuecomment-418140",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[0edec09](https://github.com/sagemath/sagetrac-mirror/commit/0edec0905bec4630d2e247fedc87f72ecfae50d6)**



---

archive/issue_comments_418141.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nok",
    "created_at": "2018-12-29T20:00:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26940#issuecomment-418141",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

ok



---

archive/issue_events_352034.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-12-29T20:00:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26940",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26940#event-352034"
}
```



---

archive/issue_events_352035.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-12-29T20:00:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26940",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26940#event-352035"
}
```



---

archive/issue_comments_418142.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2018-12-29T20:00:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26940#issuecomment-418142",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_418143.json:
```json
{
    "body": "Changed branch from **[public/26940_no_sorting_in_spanning_tree](https://github.com/sagemath/sagetrac-mirror/tree/public/26940_no_sorting_in_spanning_tree)** to **[0edec09](https://github.com/sagemath/sagetrac-mirror/commit/0edec0905bec4630d2e247fedc87f72ecfae50d6)**",
    "created_at": "2018-12-31T10:37:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26940",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26940#issuecomment-418143",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/26940_no_sorting_in_spanning_tree](https://github.com/sagemath/sagetrac-mirror/tree/public/26940_no_sorting_in_spanning_tree)** to **[0edec09](https://github.com/sagemath/sagetrac-mirror/commit/0edec0905bec4630d2e247fedc87f72ecfae50d6)**



---

archive/issue_events_352036.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-12-31T10:37:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/26940",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26940#event-352036"
}
```



---

archive/issue_events_352037.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "f2ca1e769a0216da99d7020f7215ddf947d366dc",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-12-31T10:37:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/26940",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26940#event-352037"
}
```
