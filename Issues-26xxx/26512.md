# Issue 26512: Piecewise linear functions of several variables

archive/issues_026275.json:
```json
{
    "assignees": [],
    "body": "Source:\n\nhttps://github.com/mkoeppe/cutgeneratingfunctionology/blob/master/cutgeneratingfunctionology/multirow/piecewise_functions.sage\n\nCC:  @yuan-zhou @videlec @dimpase @xcaruso\n\n_Issue created by migration from https://trac.sagemath.org/ticket/26512_\n\n",
    "created_at": "2018-10-20T00:35:38Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Piecewise linear functions of several variables",
    "type": "issue",
    "updated_at": "2022-12-29T01:43:14Z",
    "url": "https://github.com/sagemath/sage/issues/26512",
    "user": "https://github.com/mkoeppe"
}
```
Source:

https://github.com/mkoeppe/cutgeneratingfunctionology/blob/master/cutgeneratingfunctionology/multirow/piecewise_functions.sage

CC:  @yuan-zhou @videlec @dimpase @xcaruso

_Issue created by migration from https://trac.sagemath.org/ticket/26512_





---

archive/issue_events_347084.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2018-10-20T00:35:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/26512",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26512#event-347084"
}
```



---

archive/issue_events_347085.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2018-10-20T00:35:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26512",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "component: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26512#event-347085"
}
```



---

archive/issue_events_347086.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2018-10-20T00:35:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26512",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26512#event-347086"
}
```



---

archive/issue_events_347087.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2018-10-20T00:35:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/26512",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26512#event-347087"
}
```



---

archive/issue_comments_411641.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nAt the code on github the hash is terrible ([line 188](https://github.com/mkoeppe/cutgeneratingfunctionology/blob/master/piecewise_functions.sage#L188)) and does not coincide with the equality...",
    "created_at": "2018-10-20T07:34:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26512",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26512#issuecomment-411641",
    "user": "https://github.com/videlec"
}
```

<div id="comment:1" align="right">Comment 1</div>

At the code on github the hash is terrible ([line 188](https://github.com/mkoeppe/cutgeneratingfunctionology/blob/master/piecewise_functions.sage#L188)) and does not coincide with the equality...



---

archive/issue_comments_411642.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,5 +1,3 @@\n Source:\n \n-https://github.com/mkoeppe/cutgeneratingfunctionology/blob/master/piecewise_functions.sage \n-\n-\n+https://github.com/mkoeppe/cutgeneratingfunctionology/blob/master/cutgeneratingfunctionology/multirow/piecewise_functions.sage\n``````\n",
    "created_at": "2022-03-23T04:51:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/26512",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/26512#issuecomment-411642",
    "user": "https://github.com/yuan-zhou"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,5 +1,3 @@
 Source:
 
-https://github.com/mkoeppe/cutgeneratingfunctionology/blob/master/piecewise_functions.sage 
-
-
+https://github.com/mkoeppe/cutgeneratingfunctionology/blob/master/cutgeneratingfunctionology/multirow/piecewise_functions.sage
``````




---

archive/issue_events_347088.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:43:14Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/26512",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/26512#event-347088"
}
```
