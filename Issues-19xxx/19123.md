...
```json
{
    "body": "Quite easy one. Nathann selected as a random victim for a possible reviewer. `:=)`\n\n---\nNew commits:",
    "created_at": "2015-09-01T14:56:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19123",
    "type": "issue_comment",
...
```json
{
    "body": "Sounds good, but don't you think it may be useful to *know* where the poset splits? Also, why is it only defined for lattices? The algorithm works in all cases.\n\nI did not test it, but from the code's look I am not sure that it works for the chain of length 2, as the docstring indicates. Could you add a doctest for that?\n\nNathann",
...
```json
{
    "body": "You don't have to write this algorithm twice to make it work in all situations. Once is enough. And if you are worried of the cost of a 'if' inside of the loop, then you should not be writing Python code.\n\nFurthermore, be careful with '::' as they are not needed after an INPUT block. Build the doc to check it.\n\nNathann",
...
```json
{
    "body": "Could you also add to your docstring a reference toward a textbook that defines this notion?",
...
```json
{
    "body": "OEIS uses the definition where the two-element lattice is vertically indecomposable: https://oeis.org/A058800. Does this suffice as a base for the definition?",
...
```json
{
    "body": "return_type vs return_elements.",
...
```json
{
    "body": "OK, here is one possible way to split the functionality. Ready for review.",
...
```json
{
    "body": "Nathann, what about this. You already read the code and the 2-element lattice case is now documented. Hence there is two questions left:\n\n- Is the thematic index of functions OK in lattices?\n- Is this splitting of functionality OK?",
...
```json
{
    "body": "Err. Terminology nazi here: what you did is a 'merge'. A rebase is a difference operation which moves the commits around.\n\nNathann",
...
```json
{
    "body": "Merged. Sorry for wrong term.",
...
```json
{
    "body": "Kevin, if you are interested in docs, you might want to review this. It does some polishing together with adding little new functionality.",
...
```json
{
    "body": "I still think that formal definitions should come first, without being labelled as formal definitions, and then informal definitions can come after.",
...
```json
{
    "body": "I'm not sure if I like the backend code living in `hasse_diagram.py`. Even if the code  applies to more than lattices, you seem to still be making assumptions about the poset being bounded. Plus the initial docstring there is written in a way that implies it only works for lattices.",
...
```json
{
    "body": "`ping -c 1 Kevin`. This could be nice to have before #18511, as this modifies the index of function.\n\n(Funny. I have a poset of dependencies between poset-related tickets.)",
