# Issue 19132: Start a single process when running Sage

archive/issues_018895.json:
```json
{
    "body": "Jupyter assumes that it is running a single process, otherwise interrupts get sent to the wrong process. With Sage, this assumption is currently not true due to the \"front-end\" `$SAGE_ROOT/sage` script. If we instead use `exec` there and apply #19135, this is no longer a problem and Sage Jupyter notebooks can be interrupted properly.\n\nBranch/Commit: a281e344e30955987df1d696b25b18f452c5a218\n\nReviewer: Travis Scrimshaw\n\nAuthor: Jeroen Demeyer\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/19132\n\n",
    "closed_at": "2015-09-17T09:24:11Z",
    "created_at": "2015-09-04T12:40:30Z",
    "labels": [
        "component: interfaces",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.9",
    "title": "Start a single process when running Sage",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/19132",
    "user": "https://github.com/jdemeyer"
}
```
Jupyter assumes that it is running a single process, otherwise interrupts get sent to the wrong process. With Sage, this assumption is currently not true due to the "front-end" `$SAGE_ROOT/sage` script. If we instead use `exec` there and apply #19135, this is no longer a problem and Sage Jupyter notebooks can be interrupted properly.

Branch/Commit: a281e344e30955987df1d696b25b18f452c5a218

Reviewer: Travis Scrimshaw

Author: Jeroen Demeyer

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/19132





---

archive/issue_comments_341034.json:
```json
{
    "body": "Changing branch from \"\" to \"u/jdemeyer/fix_interrupts_in_jupyter\"",
    "created_at": "2015-09-04T14:11:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19132",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19132#issuecomment-341034",
    "user": "https://github.com/jdemeyer"
}
```

Changing branch from "" to "u/jdemeyer/fix_interrupts_in_jupyter"



---

archive/issue_comments_341035.json:
```json
{
    "body": "<a id='comment:2'></a>\nNew commits:\n|                                                                                                                                          |                           |\n|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------|\n|[af48d8b](http://git.sagemath.org/sage.git/commit/?id=af48d8b6c3c9f9e6de65628ff7fdd77ec89b2fb3)|`Fix interrupts in Jupyter`|",
    "created_at": "2015-09-04T14:31:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19132",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19132#issuecomment-341035",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'></a>
New commits:
|                                                                                                                                          |                           |
|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
|[af48d8b](http://git.sagemath.org/sage.git/commit/?id=af48d8b6c3c9f9e6de65628ff7fdd77ec89b2fb3)|`Fix interrupts in Jupyter`|



---

archive/issue_comments_341036.json:
```json
{
    "body": "Changing upstream from \"N/A\" to \"Reported upstream. No feedback yet.\"",
    "created_at": "2015-09-04T14:31:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19132",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19132#issuecomment-341036",
    "user": "https://github.com/jdemeyer"
}
```

Changing upstream from "N/A" to "Reported upstream. No feedback yet."



---

archive/issue_comments_341037.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -4,3 +4,7 @@\n factor(2^1000 + 1)\n ```\n cannot be interrupted for example.\n+\n+Upstream tickets:\n+* [https://github.com/ipython/ipykernel/pull/49](https://github.com/ipython/ipykernel/pull/49)\n+* [https://github.com/jupyter/jupyter_client/pull/68](https://github.com/jupyter/jupyter_client/pull/68)\n``````\n",
    "created_at": "2015-09-04T14:31:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19132",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19132#issuecomment-341037",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -4,3 +4,7 @@
 factor(2^1000 + 1)
 ```
 cannot be interrupted for example.
+
+Upstream tickets:
+* [https://github.com/ipython/ipykernel/pull/49](https://github.com/ipython/ipykernel/pull/49)
+* [https://github.com/jupyter/jupyter_client/pull/68](https://github.com/jupyter/jupyter_client/pull/68)
``````




---

archive/issue_comments_341038.json:
```json
{
    "body": "Changing commit from \"\" to \"af48d8b6c3c9f9e6de65628ff7fdd77ec89b2fb3\"",
    "created_at": "2015-09-04T14:31:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19132",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19132#issuecomment-341038",
    "user": "https://github.com/jdemeyer"
}
```

Changing commit from "" to "af48d8b6c3c9f9e6de65628ff7fdd77ec89b2fb3"



---

archive/issue_comments_341039.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2015-09-04T14:32:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19132",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19132#issuecomment-341039",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_341040.json:
```json
{
    "body": "Changing upstream from \"Reported upstream. No feedback yet.\" to \"None of the above - read trac for reasoning.\"",
    "created_at": "2015-09-04T21:09:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19132",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19132#issuecomment-341040",
    "user": "https://github.com/jdemeyer"
}
```

Changing upstream from "Reported upstream. No feedback yet." to "None of the above - read trac for reasoning."



---

archive/issue_comments_341041.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -6,5 +6,5 @@\n cannot be interrupted for example.\n \n Upstream tickets:\n-* [https://github.com/ipython/ipykernel/pull/49](https://github.com/ipython/ipykernel/pull/49)\n-* [https://github.com/jupyter/jupyter_client/pull/68](https://github.com/jupyter/jupyter_client/pull/68)\n+* [https://github.com/ipython/ipykernel/pull/49](https://github.com/ipython/ipykernel/pull/49) (merged upstream)\n+* [https://github.com/jupyter/jupyter_client/pull/68](https://github.com/jupyter/jupyter_client/pull/68) (no feedback yet)\n``````\n",
    "created_at": "2015-09-04T21:09:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19132",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19132#issuecomment-341041",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -6,5 +6,5 @@
 cannot be interrupted for example.
 
 Upstream tickets:
-* [https://github.com/ipython/ipykernel/pull/49](https://github.com/ipython/ipykernel/pull/49)
-* [https://github.com/jupyter/jupyter_client/pull/68](https://github.com/jupyter/jupyter_client/pull/68)
+* [https://github.com/ipython/ipykernel/pull/49](https://github.com/ipython/ipykernel/pull/49) (merged upstream)
+* [https://github.com/jupyter/jupyter_client/pull/68](https://github.com/jupyter/jupyter_client/pull/68) (no feedback yet)
``````




---

archive/issue_comments_341042.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2015-09-07T09:42:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19132",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19132#issuecomment-341042",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_341043.json:
```json
{
    "body": "Changing upstream from \"None of the above - read trac for reasoning.\" to \"N/A\"",
    "created_at": "2015-09-07T09:42:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19132",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19132#issuecomment-341043",
    "user": "https://github.com/jdemeyer"
}
```

Changing upstream from "None of the above - read trac for reasoning." to "N/A"



---

archive/issue_comments_341044.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,10 +1 @@\n-Jupyter messes with Sage's interrupt (`SIGINT`) handler, such that code like\n-\n-```\n-factor(2^1000 + 1)\n-```\n-cannot be interrupted for example.\n-\n-Upstream tickets:\n-* [https://github.com/ipython/ipykernel/pull/49](https://github.com/ipython/ipykernel/pull/49) (merged upstream)\n-* [https://github.com/jupyter/jupyter_client/pull/68](https://github.com/jupyter/jupyter_client/pull/68) (no feedback yet)\n+Jupyter assumes that it is running a single process. With Sage, this assumption is currently not true due to the \"front-end\" `$SAGE_ROOT/sage` script. If we instead use `exec` there, this is no longer a problem.\n``````\n",
    "created_at": "2015-09-07T09:42:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19132",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19132#issuecomment-341044",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,10 +1 @@
-Jupyter messes with Sage's interrupt (`SIGINT`) handler, such that code like
-
-```
-factor(2^1000 + 1)
-```
-cannot be interrupted for example.
-
-Upstream tickets:
-* [https://github.com/ipython/ipykernel/pull/49](https://github.com/ipython/ipykernel/pull/49) (merged upstream)
-* [https://github.com/jupyter/jupyter_client/pull/68](https://github.com/jupyter/jupyter_client/pull/68) (no feedback yet)
+Jupyter assumes that it is running a single process. With Sage, this assumption is currently not true due to the "front-end" `$SAGE_ROOT/sage` script. If we instead use `exec` there, this is no longer a problem.
``````




---

archive/issue_comments_341045.json:
```json
{
    "body": "<a id='comment:6'></a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n|                                                                                                                                          |                                          |\n|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------|\n|[a281e34](http://git.sagemath.org/sage.git/commit/?id=a281e344e30955987df1d696b25b18f452c5a218)|`Start a single process when running Sage`|",
    "created_at": "2015-09-07T09:46:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19132",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19132#issuecomment-341045",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:6'></a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
|                                                                                                                                          |                                          |
|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------|
|[a281e34](http://git.sagemath.org/sage.git/commit/?id=a281e344e30955987df1d696b25b18f452c5a218)|`Start a single process when running Sage`|



---

archive/issue_comments_341046.json:
```json
{
    "body": "Changing commit from \"af48d8b6c3c9f9e6de65628ff7fdd77ec89b2fb3\" to \"a281e344e30955987df1d696b25b18f452c5a218\"",
    "created_at": "2015-09-07T09:46:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19132",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19132#issuecomment-341046",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "af48d8b6c3c9f9e6de65628ff7fdd77ec89b2fb3" to "a281e344e30955987df1d696b25b18f452c5a218"



---

archive/issue_comments_341047.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-Jupyter assumes that it is running a single process. With Sage, this assumption is currently not true due to the \"front-end\" `$SAGE_ROOT/sage` script. If we instead use `exec` there, this is no longer a problem.\n+Jupyter assumes that it is running a single process. With Sage, this assumption is currently not true due to the \"front-end\" `$SAGE_ROOT/sage` script. If we instead use `exec` there and apply #19135, this is no longer a problem.\n``````\n",
    "created_at": "2015-09-07T09:47:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19132",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19132#issuecomment-341047",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-Jupyter assumes that it is running a single process. With Sage, this assumption is currently not true due to the "front-end" `$SAGE_ROOT/sage` script. If we instead use `exec` there, this is no longer a problem.
+Jupyter assumes that it is running a single process. With Sage, this assumption is currently not true due to the "front-end" `$SAGE_ROOT/sage` script. If we instead use `exec` there and apply #19135, this is no longer a problem.
``````




---

archive/issue_comments_341048.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2015-09-07T09:47:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19132",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19132#issuecomment-341048",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_341049.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-Jupyter assumes that it is running a single process. With Sage, this assumption is currently not true due to the \"front-end\" `$SAGE_ROOT/sage` script. If we instead use `exec` there and apply #19135, this is no longer a problem.\n+Jupyter assumes that it is running a single process, otherwise interrupts get sent to the wrong process. With Sage, this assumption is currently not true due to the \"front-end\" `$SAGE_ROOT/sage` script. If we instead use `exec` there and apply #19135, this is no longer a problem and Sage Jupyter notebooks can be interrupted properly.\n``````\n",
    "created_at": "2015-09-07T20:09:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19132",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19132#issuecomment-341049",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-Jupyter assumes that it is running a single process. With Sage, this assumption is currently not true due to the "front-end" `$SAGE_ROOT/sage` script. If we instead use `exec` there and apply #19135, this is no longer a problem.
+Jupyter assumes that it is running a single process, otherwise interrupts get sent to the wrong process. With Sage, this assumption is currently not true due to the "front-end" `$SAGE_ROOT/sage` script. If we instead use `exec` there and apply #19135, this is no longer a problem and Sage Jupyter notebooks can be interrupted properly.
``````




---

archive/issue_comments_341050.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2015-09-15T23:42:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19132",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19132#issuecomment-341050",
    "user": "https://github.com/tscrim"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_341051.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Travis Scrimshaw\"",
    "created_at": "2015-09-15T23:42:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19132",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19132#issuecomment-341051",
    "user": "https://github.com/tscrim"
}
```

Changing reviewer from "" to "Travis Scrimshaw"



---

archive/issue_comments_341052.json:
```json
{
    "body": "<a id='comment:9'></a>\nLGTM; we don't need the extra process from the shell running around.",
    "created_at": "2015-09-15T23:42:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19132",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19132#issuecomment-341052",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:9'></a>
LGTM; we don't need the extra process from the shell running around.



---

archive/issue_events_053155.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-17T09:24:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/19132",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/19132#event-53155"
}
```



---

archive/issue_comments_341053.json:
```json
{
    "body": "Changing branch from \"u/jdemeyer/fix_interrupts_in_jupyter\" to \"a281e344e30955987df1d696b25b18f452c5a218\"",
    "created_at": "2015-09-17T09:24:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19132",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19132#issuecomment-341053",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/jdemeyer/fix_interrupts_in_jupyter" to "a281e344e30955987df1d696b25b18f452c5a218"



---

archive/issue_comments_341054.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2015-09-17T09:24:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19132",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19132#issuecomment-341054",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
