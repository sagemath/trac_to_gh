# Issue 19557: Basic iteration functionality for products of projective spaces

archive/issues_019320.json:
```json
{
    "assignees": [],
    "body": "Implement nth_iterate and orbit functions for morphisms/points of products of projective spaces. Also implement nth_iterate_map for morphisms.\n\nCC:  @bhutz\n\nBranch/Commit: **[fded196](https://github.com/sagemath/sagetrac-mirror/commit/fded1967493b696ef91b9742b697f380700a2ae0)**\n\nReviewer: **Ben Hutz**\n\nAuthor: **Grayson Jorgenson**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19557_\n\n",
    "closed_at": "2015-11-24T13:01:20Z",
    "created_at": "2015-11-09T04:17:02Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.10",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Basic iteration functionality for products of projective spaces",
    "type": "issue",
    "updated_at": "2015-11-24T13:01:20Z",
    "url": "https://github.com/sagemath/sage/issues/19557",
    "user": "https://github.com/sagetrac-gjorgenson"
}
```
Implement nth_iterate and orbit functions for morphisms/points of products of projective spaces. Also implement nth_iterate_map for morphisms.

CC:  @bhutz

Branch/Commit: **[fded196](https://github.com/sagemath/sagetrac-mirror/commit/fded1967493b696ef91b9742b697f380700a2ae0)**

Reviewer: **Ben Hutz**

Author: **Grayson Jorgenson**

_Issue created by migration from https://trac.sagemath.org/ticket/19557_





---

archive/issue_events_259253.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2015-11-09T04:17:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "milestone_number": null,
    "milestone_title": "sage-6.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19557#event-259253"
}
```



---

archive/issue_events_259254.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2015-11-09T04:17:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
    "label_color": "0000ff",
    "label_name": "component: algebraic geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19557#event-259254"
}
```



---

archive/issue_events_259255.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2015-11-09T04:17:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19557#event-259255"
}
```



---

archive/issue_events_259256.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2015-11-09T04:17:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19557#event-259256"
}
```



---

archive/issue_comments_281161.json:
```json
{
    "body": "Branch: **[u/gjorgenson/ticket/19557](https://github.com/sagemath/sagetrac-mirror/tree/u/gjorgenson/ticket/19557)**",
    "created_at": "2015-11-10T03:04:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19557#issuecomment-281161",
    "user": "https://github.com/sagetrac-gjorgenson"
}
```

Branch: **[u/gjorgenson/ticket/19557](https://github.com/sagemath/sagetrac-mirror/tree/u/gjorgenson/ticket/19557)**



---

archive/issue_comments_281162.json:
```json
{
    "body": "Commit: **[3831d5a](https://github.com/sagemath/sagetrac-mirror/commit/3831d5a78324ade0aca890d8557a1a5c149b2819)**",
    "created_at": "2015-11-10T03:08:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19557#issuecomment-281162",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[3831d5a](https://github.com/sagemath/sagetrac-mirror/commit/3831d5a78324ade0aca890d8557a1a5c149b2819)**



---

archive/issue_comments_281163.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3831d5a78324ade0aca890d8557a1a5c149b2819\">3831d5a</a></td><td><code>19557: implementation of the iteration functionality</code></td></tr></table>\n",
    "created_at": "2015-11-10T03:08:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19557#issuecomment-281163",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3831d5a78324ade0aca890d8557a1a5c149b2819">3831d5a</a></td><td><code>19557: implementation of the iteration functionality</code></td></tr></table>




---

archive/issue_comments_281164.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/920d9970db50e891151484574748b3381aa44f0a\">920d997</a></td><td><code>19557: added example and fixed some spacing</code></td></tr></table>\n",
    "created_at": "2015-11-10T03:54:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19557#issuecomment-281164",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:3'>Comment 3:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/920d9970db50e891151484574748b3381aa44f0a">920d997</a></td><td><code>19557: added example and fixed some spacing</code></td></tr></table>




---

archive/issue_comments_281165.json:
```json
{
    "body": "Changed commit from **[3831d5a](https://github.com/sagemath/sagetrac-mirror/commit/3831d5a78324ade0aca890d8557a1a5c149b2819)** to **[920d997](https://github.com/sagemath/sagetrac-mirror/commit/920d9970db50e891151484574748b3381aa44f0a)**",
    "created_at": "2015-11-10T03:54:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19557#issuecomment-281165",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[3831d5a](https://github.com/sagemath/sagetrac-mirror/commit/3831d5a78324ade0aca890d8557a1a5c149b2819)** to **[920d997](https://github.com/sagemath/sagetrac-mirror/commit/920d9970db50e891151484574748b3381aa44f0a)**



---

archive/issue_events_259257.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2015-11-10T04:00:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19557#event-259257"
}
```



---

archive/issue_comments_281166.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nNeed to fix the merge issue.\n\nFrom looking through the commits I see a couple things to fix:\n\nin nth_iterate:\nthese checks are done in `__call__` with better coercion, so not needed here.\n        if self.codomain()!=f.domain():\n            raise TypeError(\"Point is not defined over domain of function\")\n\nuse .is_endomorphism() instead:\n        if f.domain() != f.codomain():\n            raise TypeError(\"Domain and Codomain of function not equal\")\n\nin nth_iterate_map\nuse is_endomorphism() check",
    "created_at": "2015-11-17T17:09:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19557#issuecomment-281166",
    "user": "https://github.com/bhutz"
}
```

<a id='comment:5'>Comment 5:</a>
Need to fix the merge issue.

From looking through the commits I see a couple things to fix:

in nth_iterate:
these checks are done in `__call__` with better coercion, so not needed here.
        if self.codomain()!=f.domain():
            raise TypeError("Point is not defined over domain of function")

use .is_endomorphism() instead:
        if f.domain() != f.codomain():
            raise TypeError("Domain and Codomain of function not equal")

in nth_iterate_map
use is_endomorphism() check



---

archive/issue_events_259258.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2015-11-17T17:09:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19557#event-259258"
}
```



---

archive/issue_events_259259.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2015-11-17T17:09:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19557#event-259259"
}
```



---

archive/issue_comments_281167.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1a131723fa5f1394e09f52f6116649ffaf50e2cc\">1a13172</a></td><td><code>19557: Merged with new beta to fix merge conflict.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/559811d909703ad0ca2ce678cad48e2f01aee691\">559811d</a></td><td><code>19557: changed checks</code></td></tr></table>\n",
    "created_at": "2015-11-19T03:39:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19557#issuecomment-281167",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:6'>Comment 6:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1a131723fa5f1394e09f52f6116649ffaf50e2cc">1a13172</a></td><td><code>19557: Merged with new beta to fix merge conflict.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/559811d909703ad0ca2ce678cad48e2f01aee691">559811d</a></td><td><code>19557: changed checks</code></td></tr></table>




---

archive/issue_comments_281168.json:
```json
{
    "body": "Changed commit from **[920d997](https://github.com/sagemath/sagetrac-mirror/commit/920d9970db50e891151484574748b3381aa44f0a)** to **[559811d](https://github.com/sagemath/sagetrac-mirror/commit/559811d909703ad0ca2ce678cad48e2f01aee691)**",
    "created_at": "2015-11-19T03:39:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19557#issuecomment-281168",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[920d997](https://github.com/sagemath/sagetrac-mirror/commit/920d9970db50e891151484574748b3381aa44f0a)** to **[559811d](https://github.com/sagemath/sagetrac-mirror/commit/559811d909703ad0ca2ce678cad48e2f01aee691)**



---

archive/issue_comments_281169.json:
```json
{
    "body": "Changed commit from **[559811d](https://github.com/sagemath/sagetrac-mirror/commit/559811d909703ad0ca2ce678cad48e2f01aee691)** to **[37c4e69](https://github.com/sagemath/sagetrac-mirror/commit/37c4e690fb3d452573604bcac62a62f8ca32114c)**",
    "created_at": "2015-11-19T04:07:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19557#issuecomment-281169",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[559811d](https://github.com/sagemath/sagetrac-mirror/commit/559811d909703ad0ca2ce678cad48e2f01aee691)** to **[37c4e69](https://github.com/sagemath/sagetrac-mirror/commit/37c4e690fb3d452573604bcac62a62f8ca32114c)**



---

archive/issue_comments_281170.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f114f1b27adb1d2dfd5216da96fdb2902c5e2cea\">f114f1b</a></td><td><code>19557: attempted conflict fix</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/37c4e690fb3d452573604bcac62a62f8ca32114c\">37c4e69</a></td><td><code>19557: second merge attempt</code></td></tr></table>\n",
    "created_at": "2015-11-19T04:07:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19557#issuecomment-281170",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:7'>Comment 7:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f114f1b27adb1d2dfd5216da96fdb2902c5e2cea">f114f1b</a></td><td><code>19557: attempted conflict fix</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/37c4e690fb3d452573604bcac62a62f8ca32114c">37c4e69</a></td><td><code>19557: second merge attempt</code></td></tr></table>




---

archive/issue_events_259260.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2015-11-19T04:14:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19557#event-259260"
}
```



---

archive/issue_events_259261.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2015-11-19T04:14:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19557#event-259261"
}
```



---

archive/issue_events_259262.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2015-11-19T15:57:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19557#event-259262"
}
```



---

archive/issue_events_259263.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2015-11-19T15:57:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19557#event-259263"
}
```



---

archive/issue_comments_281171.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nThat all checks out fine. Just one thing: I was wrong about `__call__` doing the coercion checking. That is true for projective spaces, but not for products. I think you should put that check back in as some weird behavior can occur.\n\nGo ahead and put that back in, then I'll mark this positive.",
    "created_at": "2015-11-19T15:57:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19557#issuecomment-281171",
    "user": "https://github.com/bhutz"
}
```

<a id='comment:9'>Comment 9:</a>
That all checks out fine. Just one thing: I was wrong about `__call__` doing the coercion checking. That is true for projective spaces, but not for products. I think you should put that check back in as some weird behavior can occur.

Go ahead and put that back in, then I'll mark this positive.



---

archive/issue_comments_281172.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fded1967493b696ef91b9742b697f380700a2ae0\">fded196</a></td><td><code>19557: added check back in</code></td></tr></table>\n",
    "created_at": "2015-11-21T05:01:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19557#issuecomment-281172",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:10'>Comment 10:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fded1967493b696ef91b9742b697f380700a2ae0">fded196</a></td><td><code>19557: added check back in</code></td></tr></table>




---

archive/issue_comments_281173.json:
```json
{
    "body": "Changed commit from **[37c4e69](https://github.com/sagemath/sagetrac-mirror/commit/37c4e690fb3d452573604bcac62a62f8ca32114c)** to **[fded196](https://github.com/sagemath/sagetrac-mirror/commit/fded1967493b696ef91b9742b697f380700a2ae0)**",
    "created_at": "2015-11-21T05:01:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19557#issuecomment-281173",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[37c4e69](https://github.com/sagemath/sagetrac-mirror/commit/37c4e690fb3d452573604bcac62a62f8ca32114c)** to **[fded196](https://github.com/sagemath/sagetrac-mirror/commit/fded1967493b696ef91b9742b697f380700a2ae0)**



---

archive/issue_events_259264.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2015-11-21T05:03:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19557#event-259264"
}
```



---

archive/issue_events_259265.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2015-11-21T05:03:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19557#event-259265"
}
```



---

archive/issue_comments_281174.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nAlright, I added the check into orbit as well.",
    "created_at": "2015-11-21T05:03:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19557#issuecomment-281174",
    "user": "https://github.com/sagetrac-gjorgenson"
}
```

<a id='comment:11'>Comment 11:</a>
Alright, I added the check into orbit as well.



---

archive/issue_events_259266.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2015-11-21T14:03:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19557#event-259266"
}
```



---

archive/issue_events_259267.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2015-11-21T14:03:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19557#event-259267"
}
```



---

archive/issue_events_259268.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-11-23T06:38:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19557#event-259268"
}
```



---

archive/issue_events_259269.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-11-23T06:38:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19557#event-259269"
}
```



---

archive/issue_comments_281175.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nreviewer name",
    "created_at": "2015-11-23T06:38:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19557#issuecomment-281175",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:13'>Comment 13:</a>
reviewer name



---

archive/issue_events_259270.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2015-11-23T12:27:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19557#event-259270"
}
```



---

archive/issue_events_259271.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2015-11-23T12:27:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19557#event-259271"
}
```



---

archive/issue_comments_281176.json:
```json
{
    "body": "Reviewer: **Ben Hutz**",
    "created_at": "2015-11-23T12:27:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19557#issuecomment-281176",
    "user": "https://github.com/bhutz"
}
```

Reviewer: **Ben Hutz**



---

archive/issue_comments_281177.json:
```json
{
    "body": "Changed branch from **[u/gjorgenson/ticket/19557](https://github.com/sagemath/sagetrac-mirror/tree/u/gjorgenson/ticket/19557)** to **[fded196](https://github.com/sagemath/sagetrac-mirror/commit/fded1967493b696ef91b9742b697f380700a2ae0)**",
    "created_at": "2015-11-24T13:01:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19557#issuecomment-281177",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/gjorgenson/ticket/19557](https://github.com/sagemath/sagetrac-mirror/tree/u/gjorgenson/ticket/19557)** to **[fded196](https://github.com/sagemath/sagetrac-mirror/commit/fded1967493b696ef91b9742b697f380700a2ae0)**



---

archive/issue_events_259272.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-11-24T13:01:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19557#event-259272"
}
```



---

archive/issue_events_259273.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "9ae918b77b3e72fd1da946402b6dfe4059660594",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-11-24T13:01:20Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19557",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19557#event-259273"
}
```
