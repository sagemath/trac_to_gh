# Issue 19433: Compilation fail because libreadline-dev isn't installed

archive/issues_019196.json:
```json
{
    "assignees": [],
    "body": "http://doc.sagemath.org/html/en/installation/source.html#linux-prerequisite-installation do not mention the need of libreadline-dev and R need it.\n\nReviewer: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19433_\n\n",
    "closed_at": "2017-07-13T07:54:31Z",
    "created_at": "2015-10-18T13:04:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Compilation fail because libreadline-dev isn't installed",
    "type": "issue",
    "updated_at": "2017-07-13T07:54:31Z",
    "url": "https://github.com/sagemath/sage/issues/19433",
    "user": "https://github.com/sagetrac-r-gaia-cs"
}
```
http://doc.sagemath.org/html/en/installation/source.html#linux-prerequisite-installation do not mention the need of libreadline-dev and R need it.

Reviewer: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/19433_





---

archive/issue_events_257687.json:
```json
{
    "actor": "https://github.com/sagetrac-r-gaia-cs",
    "created_at": "2015-10-18T13:04:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19433",
    "milestone_number": null,
    "milestone_title": "sage-6.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19433#event-257687"
}
```



---

archive/issue_events_257688.json:
```json
{
    "actor": "https://github.com/sagetrac-r-gaia-cs",
    "created_at": "2015-10-18T13:04:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19433",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20website/wiki",
    "label_color": "0075ca",
    "label_name": "component: website/wiki",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19433#event-257688"
}
```



---

archive/issue_events_257689.json:
```json
{
    "actor": "https://github.com/sagetrac-r-gaia-cs",
    "created_at": "2015-10-18T13:04:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19433",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19433#event-257689"
}
```



---

archive/issue_events_257690.json:
```json
{
    "actor": "https://github.com/sagetrac-r-gaia-cs",
    "created_at": "2015-10-18T13:04:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19433",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19433#event-257690"
}
```



---

archive/issue_comments_278775.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nPlease give more detail about the problem and provide log files (made with a non-parallel build).",
    "created_at": "2015-10-18T14:00:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19433#issuecomment-278775",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:1" align="right">Comment 1</div>

Please give more detail about the problem and provide log files (made with a non-parallel build).



---

archive/issue_events_257691.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-10-18T14:00:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19433",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20website/wiki",
    "label_color": "0075ca",
    "label_name": "component: website/wiki",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19433#event-257691"
}
```



---

archive/issue_events_257692.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-10-18T14:00:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19433",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19433#event-257692"
}
```



---

archive/issue_comments_278776.json:
```json
{
    "body": "Error log when tried to install R",
    "created_at": "2015-10-18T22:10:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19433#issuecomment-278776",
    "user": "https://github.com/sagetrac-r-gaia-cs"
}
```

Error log when tried to install R



---

archive/issue_comments_278777.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nAttachment: **[r-3.2.2.p0.log](https://github.com/sagemath/sage/files/ticket19433/r-3.2.2.p0.log)**\n\nAnd which version of Sage is this?",
    "created_at": "2015-10-19T06:18:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19433#issuecomment-278777",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

Attachment: **[r-3.2.2.p0.log](https://github.com/sagemath/sage/files/ticket19433/r-3.2.2.p0.log)**

And which version of Sage is this?



---

archive/issue_comments_278778.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nAnd which operating system version?",
    "created_at": "2015-10-19T06:20:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19433#issuecomment-278778",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:3" align="right">Comment 3</div>

And which operating system version?



---

archive/issue_comments_278779.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\n\n```\n$ uname -a\nLinux pupunha 4.1.0-1-amd64 #1 SMP Debian 4.1.3-1 (2015-08-03) x86_64 GNU/Linux\n$ git rev-parse HEAD\n2cc7f6ce9b270c1efcd07b541fe94e34a245918d\n```",
    "created_at": "2015-10-19T10:09:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19433#issuecomment-278779",
    "user": "https://github.com/sagetrac-r-gaia-cs"
}
```

<div id="comment:4" align="right">Comment 4</div>


```
$ uname -a
Linux pupunha 4.1.0-1-amd64 #1 SMP Debian 4.1.3-1 (2015-08-03) x86_64 GNU/Linux
$ git rev-parse HEAD
2cc7f6ce9b270c1efcd07b541fe94e34a245918d
```



---

archive/issue_events_257693.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-19T13:17:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19433",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19433#event-257693"
}
```



---

archive/issue_comments_278780.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2017-04-19T13:17:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19433#issuecomment-278780",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_257694.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-19T13:17:48Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/19433",
    "milestone_number": null,
    "milestone_title": "sage-6.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19433#event-257694"
}
```



---

archive/issue_events_257695.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-19T13:17:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19433",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19433#event-257695"
}
```



---

archive/issue_events_257696.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-19T13:17:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19433",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19433#event-257696"
}
```



---

archive/issue_comments_278781.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nI'm assuming that this report is obsolete. If it's not, please open a new ticket.",
    "created_at": "2017-04-19T13:17:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19433#issuecomment-278781",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:5" align="right">Comment 5</div>

I'm assuming that this report is obsolete. If it's not, please open a new ticket.



---

archive/issue_events_257697.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-19T13:17:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19433",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19433#event-257697"
}
```



---

archive/issue_events_257698.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-19T13:17:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19433",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19433#event-257698"
}
```



---

archive/issue_comments_278782.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nClosing tickets in the sage-duplicate/invalid/wontfix module with positive_review (i.e. someone has confirmed they should be closed).",
    "created_at": "2017-07-13T07:54:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19433",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19433#issuecomment-278782",
    "user": "https://github.com/embray"
}
```

<div id="comment:7" align="right">Comment 7</div>

Closing tickets in the sage-duplicate/invalid/wontfix module with positive_review (i.e. someone has confirmed they should be closed).



---

archive/issue_events_257699.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-07-13T07:54:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19433",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19433#event-257699"
}
```



---

archive/issue_events_257700.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-07-13T07:54:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19433",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19433#event-257700"
}
```



---

archive/issue_events_257701.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-07-13T07:54:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19433",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19433#event-257701"
}
```
