# Issue 19064: Equality shouldn't also be unequal

archive/issues_018827.json:
```json
{
    "assignees": [],
    "body": "\n```\nbool((1/8*sqrt(2)*(sqrt(2)*(sqrt(2) - 2) + 4*sqrt(2)) + 1/4*sqrt(2)*(sqrt(2) - 2) + 3/4*sqrt(2) - 1/2) < (1/8*sqrt(2)*(sqrt(2) + 2) + 1/4*sqrt(2) + 1/4))\n```\napparently yields `True`, though these expressions are apparently actually equal.  See [this ask.sagemath question](http://ask.sagemath.org/question/29228/unexpanded-symbolic-inequalities-yield-erroneous-results/) for details.\n\nReviewer: **Ralf Stephan**\n\nComponent: **symbolics**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19064_\n\n",
    "closed_at": "2015-09-12T14:05:05Z",
    "created_at": "2015-08-20T17:12:31Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Equality shouldn't also be unequal",
    "type": "issue",
    "updated_at": "2015-09-12T14:05:05Z",
    "url": "https://github.com/sagemath/sage/issues/19064",
    "user": "https://github.com/kcrisman"
}
```

```
bool((1/8*sqrt(2)*(sqrt(2)*(sqrt(2) - 2) + 4*sqrt(2)) + 1/4*sqrt(2)*(sqrt(2) - 2) + 3/4*sqrt(2) - 1/2) < (1/8*sqrt(2)*(sqrt(2) + 2) + 1/4*sqrt(2) + 1/4))
```
apparently yields `True`, though these expressions are apparently actually equal.  See [this ask.sagemath question](http://ask.sagemath.org/question/29228/unexpanded-symbolic-inequalities-yield-erroneous-results/) for details.

Reviewer: **Ralf Stephan**

Component: **symbolics**

_Issue created by migration from https://trac.sagemath.org/ticket/19064_





---

archive/issue_events_270609.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2015-08-20T17:12:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19064",
    "milestone_number": null,
    "milestone_title": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19064#event-270609"
}
```



---

archive/issue_events_270610.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2015-08-20T17:12:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19064",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "c: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19064#event-270610"
}
```



---

archive/issue_events_270611.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2015-08-20T17:12:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19064",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19064#event-270611"
}
```



---

archive/issue_events_270612.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2015-08-20T17:12:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19064",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19064#event-270612"
}
```



---

archive/issue_comments_268864.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nCan't reproduce this on 6.9beta (I get false for < and true for ==)",
    "created_at": "2015-08-20T23:56:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19064",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19064#issuecomment-268864",
    "user": "https://github.com/nbruin"
}
```

<div id="comment:1" align="right">comment:1</div>

Can't reproduce this on 6.9beta (I get false for < and true for ==)



---

archive/issue_comments_268865.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThat was fixed in #18896",
    "created_at": "2015-08-21T06:26:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19064",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19064#issuecomment-268865",
    "user": "https://github.com/rwst"
}
```

<div id="comment:2" align="right">comment:2</div>

That was fixed in #18896



---

archive/issue_events_270613.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-08-21T06:26:33Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/19064",
    "milestone_number": null,
    "milestone_title": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19064#event-270613"
}
```



---

archive/issue_events_270614.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-08-21T06:26:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19064",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19064#event-270614"
}
```



---

archive/issue_events_270615.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-08-21T06:26:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19064",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19064#event-270615"
}
```



---

archive/issue_events_270616.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-08-21T06:26:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19064",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19064#event-270616"
}
```



---

archive/issue_events_270617.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-08-21T06:26:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19064",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19064#event-270617"
}
```



---

archive/issue_events_270618.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-08-21T06:26:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19064",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19064#event-270618"
}
```



---

archive/issue_comments_268866.json:
```json
{
    "body": "Reviewer: **Ralf Stephan**",
    "created_at": "2015-08-21T13:32:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19064",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19064#issuecomment-268866",
    "user": "https://github.com/kcrisman"
}
```

Reviewer: **Ralf Stephan**



---

archive/issue_comments_268867.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nAwesome, thanks.  I'm sorry for not checking myself, only had time to report it before I forgot and it got lost.",
    "created_at": "2015-08-21T13:32:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19064",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19064#issuecomment-268867",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:4" align="right">comment:4</div>

Awesome, thanks.  I'm sorry for not checking myself, only had time to report it before I forgot and it got lost.



---

archive/issue_events_270619.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-12T14:05:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19064",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19064#event-270619"
}
```



---

archive/issue_events_270620.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-12T14:05:05Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19064",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19064#event-270620"
}
```
