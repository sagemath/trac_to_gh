# Issue 19483: Update bliss

archive/issues_019246.json:
```json
{
    "body": "This update also fixes the following problem:\n\n```\nsage: DiGraph('GY@OQ?OW@?O?').canonical_label()\n<crash>\n```\n\nnew tarball: http://www.steinertriples.fr/ncohen/tmp/bliss-0.73.tar.gz\n\nCC:  azi\n\nBranch/Commit: ea80cfd6f3e3e9f61d8d64a560d22428a9509dd7\n\nReviewer: Jernej Azarija, Jeroen Demeyer\n\nAuthor: Nathann Cohen\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/19483\n\n",
    "closed_at": "2015-10-28T17:59:41Z",
    "created_at": "2015-10-27T09:42:21Z",
    "labels": [
        "component: graph theory",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.10",
    "title": "Update bliss",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/19483",
    "user": "https://github.com/nathanncohen"
}
```
This update also fixes the following problem:

```
sage: DiGraph('GY@OQ?OW@?O?').canonical_label()
<crash>
```

new tarball: http://www.steinertriples.fr/ncohen/tmp/bliss-0.73.tar.gz

CC:  azi

Branch/Commit: ea80cfd6f3e3e9f61d8d64a560d22428a9509dd7

Reviewer: Jernej Azarija, Jeroen Demeyer

Author: Nathann Cohen

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/19483





---

archive/issue_comments_349121.json:
```json
{
    "body": "<a id='comment:1'></a>\nYes this is a bug in bliss (confirmed by the author) that was fixed in the recent version!\n\nSo the solution is simply to update the spkg to use the latest version from here http://www.tcs.hut.fi/Software/bliss/",
    "created_at": "2015-10-27T09:44:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349121",
    "user": "https://trac.sagemath.org/admin/accounts/users/azi"
}
```

<a id='comment:1'></a>
Yes this is a bug in bliss (confirmed by the author) that was fixed in the recent version!

So the solution is simply to update the spkg to use the latest version from here http://www.tcs.hut.fi/Software/bliss/



---

archive/issue_comments_349122.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,6 +1,8 @@\n-I traced this down to the following:\n+This update is also meant to fix the following problem:\n \n ```\n sage: DiGraph('GY@OQ?OW@?O?').canonical_label()\n <crash>\n ```\n+\n+new tarball: http://www.steinertriples.fr/ncohen/tmp/bliss-0.73.tar.gz\n``````\n",
    "created_at": "2015-10-27T10:11:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349122",
    "user": "https://github.com/nathanncohen"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,6 +1,8 @@
-I traced this down to the following:
+This update is also meant to fix the following problem:
 
 ```
 sage: DiGraph('GY@OQ?OW@?O?').canonical_label()
 <crash>
 ```
+
+new tarball: http://www.steinertriples.fr/ncohen/tmp/bliss-0.73.tar.gz
``````




---

archive/issue_comments_349123.json:
```json
{
    "body": "Changing author from \"\" to \"Nathann Cohen\"",
    "created_at": "2015-10-27T10:11:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349123",
    "user": "https://github.com/nathanncohen"
}
```

Changing author from "" to "Nathann Cohen"



---

archive/issue_comments_349124.json:
```json
{
    "body": "<a id='comment:2'></a>\nHello Jernej,\n\nI updated the package but the bug remains. Could you look into it?\n\nNathann",
    "created_at": "2015-10-27T10:11:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349124",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:2'></a>
Hello Jernej,

I updated the package but the bug remains. Could you look into it?

Nathann



---

archive/issue_comments_349125.json:
```json
{
    "body": "Changing branch from \"\" to \"u/ncohen/19483\"",
    "created_at": "2015-10-27T10:11:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349125",
    "user": "https://github.com/nathanncohen"
}
```

Changing branch from "" to "u/ncohen/19483"



---

archive/issue_comments_349126.json:
```json
{
    "body": "<a id='comment:3'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2015-10-27T10:11:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349126",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:3'></a>
Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_349127.json:
```json
{
    "body": "Changing commit from \"\" to \"3152b5c3e970c906f48c9bf1cf001aee4c2583f6\"",
    "created_at": "2015-10-27T10:11:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349127",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "" to "3152b5c3e970c906f48c9bf1cf001aee4c2583f6"



---

archive/issue_comments_349128.json:
```json
{
    "body": "<a id='comment:4'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                          |                           |\n|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------|\n|[579aed8](http://git.sagemath.org/sage.git/commit/?id=579aed862ea02a060a82e6b03ab7cd068a15f76e)|`trac #19483: Update bliss`|",
    "created_at": "2015-10-27T10:13:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349128",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:4'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                          |                           |
|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------|
|[579aed8](http://git.sagemath.org/sage.git/commit/?id=579aed862ea02a060a82e6b03ab7cd068a15f76e)|`trac #19483: Update bliss`|



---

archive/issue_comments_349129.json:
```json
{
    "body": "Changing commit from \"3152b5c3e970c906f48c9bf1cf001aee4c2583f6\" to \"579aed862ea02a060a82e6b03ab7cd068a15f76e\"",
    "created_at": "2015-10-27T10:13:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349129",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "3152b5c3e970c906f48c9bf1cf001aee4c2583f6" to "579aed862ea02a060a82e6b03ab7cd068a15f76e"



---

archive/issue_comments_349130.json:
```json
{
    "body": "<a id='comment:5'></a>\nHey there!\n\nThat is weird. As far as bliss is concerned I can reproduce the bug in bliss-0.72 but not in bliss-0.73\n\n```\nazi@bodysnatcher:~/bliss-0.72$ ./bliss -directed -can -sh=f /tmp/foo.dimacs \nbliss: graph.cc:3875: virtual bool bliss::Digraph::nucr_find_first_component(unsigned int, std::vector<unsigned int>&, unsigned int&, bliss::Partition::Cell*&): Assertion `sh_return' failed.\nAborted (core dumped)\n```\n\nAnd\n\n```\nazi@bodysnatcher:~/bliss-0.73$ ./bliss -directed -can -sh=f /tmp/foo.dimacs \nGenerator: (2,3)(6,7)\nGenerator: (3,5)(4,6)\nCanonical labeling: (1,8)(2,7)(3,6)\nNodes:          6\nLeaf nodes:     4\nBad nodes:      0\nCanrep updates: 1\nGenerators:     2\nMax level:      2\nTotal time:\t0.00 seconds\n```\n|Aut|:          6\n\nEvery other heuristic -sh works as well in bliss-0.73. So that makes me wonder, are we 100% that the spkg really installs bliss?",
    "created_at": "2015-10-27T10:34:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349130",
    "user": "https://trac.sagemath.org/admin/accounts/users/azi"
}
```

<a id='comment:5'></a>
Hey there!

That is weird. As far as bliss is concerned I can reproduce the bug in bliss-0.72 but not in bliss-0.73

```
azi@bodysnatcher:~/bliss-0.72$ ./bliss -directed -can -sh=f /tmp/foo.dimacs 
bliss: graph.cc:3875: virtual bool bliss::Digraph::nucr_find_first_component(unsigned int, std::vector<unsigned int>&, unsigned int&, bliss::Partition::Cell*&): Assertion `sh_return' failed.
Aborted (core dumped)
```

And

```
azi@bodysnatcher:~/bliss-0.73$ ./bliss -directed -can -sh=f /tmp/foo.dimacs 
Generator: (2,3)(6,7)
Generator: (3,5)(4,6)
Canonical labeling: (1,8)(2,7)(3,6)
Nodes:          6
Leaf nodes:     4
Bad nodes:      0
Canrep updates: 1
Generators:     2
Max level:      2
Total time:	0.00 seconds
```
|Aut|:          6

Every other heuristic -sh works as well in bliss-0.73. So that makes me wonder, are we 100% that the spkg really installs bliss?



---

archive/issue_comments_349131.json:
```json
{
    "body": "<a id='comment:6'></a>\nMystery cleared: that's because installing the new bliss is not enough--> one also has to touch bliss.pyx, as otherwise the code is not recompiled.\n\nNathann",
    "created_at": "2015-10-27T10:40:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349131",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:6'></a>
Mystery cleared: that's because installing the new bliss is not enough--> one also has to touch bliss.pyx, as otherwise the code is not recompiled.

Nathann



---

archive/issue_comments_349132.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2015-10-27T10:40:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349132",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_349133.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-This update is also meant to fix the following problem:\n+This update also fixes the following problem:\n \n ```\n sage: DiGraph('GY@OQ?OW@?O?').canonical_label()\n``````\n",
    "created_at": "2015-10-27T10:40:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349133",
    "user": "https://github.com/nathanncohen"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-This update is also meant to fix the following problem:
+This update also fixes the following problem:
 
 ```
 sage: DiGraph('GY@OQ?OW@?O?').canonical_label()
``````




---

archive/issue_comments_349134.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Jernej Azarija\"",
    "created_at": "2015-10-27T10:46:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349134",
    "user": "https://trac.sagemath.org/admin/accounts/users/azi"
}
```

Changing reviewer from "" to "Jernej Azarija"



---

archive/issue_comments_349135.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2015-10-27T10:46:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349135",
    "user": "https://trac.sagemath.org/admin/accounts/users/azi"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_349136.json:
```json
{
    "body": "<a id='comment:8'></a>\nThanks,\n\nNathann",
    "created_at": "2015-10-27T10:46:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349136",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:8'></a>
Thanks,

Nathann



---

archive/issue_comments_349137.json:
```json
{
    "body": "<a id='comment:9'></a>\nReplying to [ncohen](#comment%3A6):\n> Mystery cleared: that's because installing the new bliss is not enough--> one also has to touch bliss.pyx, as otherwise the code is not recompiled.\n\n\nThat's a bug.",
    "created_at": "2015-10-27T19:43:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349137",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:9'></a>
Replying to [ncohen](#comment%3A6):
> Mystery cleared: that's because installing the new bliss is not enough--> one also has to touch bliss.pyx, as otherwise the code is not recompiled.


That's a bug.



---

archive/issue_comments_349138.json:
```json
{
    "body": "Changing status from positive_review to needs_work.",
    "created_at": "2015-10-27T19:43:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349138",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from positive_review to needs_work.



---

archive/issue_comments_349139.json:
```json
{
    "body": "<a id='comment:10'></a>\nUse `cp` instead of `mv` in `spkg-install` (this is probably why you have the dependency issue).",
    "created_at": "2015-10-27T19:54:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349139",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:10'></a>
Use `cp` instead of `mv` in `spkg-install` (this is probably why you have the dependency issue).



---

archive/issue_comments_349140.json:
```json
{
    "body": "<a id='comment:11'></a>\nNote that you still need an additional run of `make` or `./sage -b` after installing `bliss`. However, that's not the fault of the package.",
    "created_at": "2015-10-27T19:56:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349140",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:11'></a>
Note that you still need an additional run of `make` or `./sage -b` after installing `bliss`. However, that's not the fault of the package.



---

archive/issue_comments_349141.json:
```json
{
    "body": "<a id='comment:12'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                          |                     |\n|------------------------------------------------------------------------------------------------------------------------------------------|---------------------|\n|[ea80cfd](http://git.sagemath.org/sage.git/commit/?id=ea80cfd6f3e3e9f61d8d64a560d22428a9509dd7)|`trac #19483: Bugfix`|",
    "created_at": "2015-10-28T08:15:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349141",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:12'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                          |                     |
|------------------------------------------------------------------------------------------------------------------------------------------|---------------------|
|[ea80cfd](http://git.sagemath.org/sage.git/commit/?id=ea80cfd6f3e3e9f61d8d64a560d22428a9509dd7)|`trac #19483: Bugfix`|



---

archive/issue_comments_349142.json:
```json
{
    "body": "Changing commit from \"579aed862ea02a060a82e6b03ab7cd068a15f76e\" to \"ea80cfd6f3e3e9f61d8d64a560d22428a9509dd7\"",
    "created_at": "2015-10-28T08:15:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349142",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "579aed862ea02a060a82e6b03ab7cd068a15f76e" to "ea80cfd6f3e3e9f61d8d64a560d22428a9509dd7"



---

archive/issue_comments_349143.json:
```json
{
    "body": "<a id='comment:13'></a>\nSorry sorry, I am a bit sick these days and with half of my brain I did not realise that indeed this 'touch' thing was a problem. I added an empty line in the file.\n\nI did not understand what you said about the mv/cp change but I did it anyway. Could you be more verbose about it? I do not know of which dependency problem you are talking.\n\nThanks, and sorry if the questions are stupid. Just slept 12 hours, and I may have to do the same tonight `:-/`\n\nNathann",
    "created_at": "2015-10-28T08:16:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349143",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:13'></a>
Sorry sorry, I am a bit sick these days and with half of my brain I did not realise that indeed this 'touch' thing was a problem. I added an empty line in the file.

I did not understand what you said about the mv/cp change but I did it anyway. Could you be more verbose about it? I do not know of which dependency problem you are talking.

Thanks, and sorry if the questions are stupid. Just slept 12 hours, and I may have to do the same tonight `:-/`

Nathann



---

archive/issue_comments_349144.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2015-10-28T08:16:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349144",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_349145.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2015-10-28T08:38:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349145",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_349146.json:
```json
{
    "body": "<a id='comment:15'></a>\nReplying to [ncohen](#comment%3A13):\n> I did not understand what you said about the mv/cp change but I did it anyway. Could you be more verbose about it? I do not know of which dependency problem you are talking.\n\n\nSorry, it wasn't clear indeed.\n\nThe \"dependency problem\" is the problem that you manually needed to touch the `.pyx` file. Let me explain more:\n* with `mv`, the file is moved *with the same timestamp*, which in practice is the timestamp the package was made. So, Cython thinks this is an old file and sees no reason to recompile `bliss.pyx`.\n* with `cp`, the new file really is a new file so it has a new timestamp. Cython will detect this file as changed and automatically recompile `bliss.pyx`.",
    "created_at": "2015-10-28T08:38:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349146",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:15'></a>
Replying to [ncohen](#comment%3A13):
> I did not understand what you said about the mv/cp change but I did it anyway. Could you be more verbose about it? I do not know of which dependency problem you are talking.


Sorry, it wasn't clear indeed.

The "dependency problem" is the problem that you manually needed to touch the `.pyx` file. Let me explain more:
* with `mv`, the file is moved *with the same timestamp*, which in practice is the timestamp the package was made. So, Cython thinks this is an old file and sees no reason to recompile `bliss.pyx`.
* with `cp`, the new file really is a new file so it has a new timestamp. Cython will detect this file as changed and automatically recompile `bliss.pyx`.



---

archive/issue_comments_349147.json:
```json
{
    "body": "Changing reviewer from \"Jernej Azarija\" to \"Jernej Azarija, Jeroen Demeyer\"",
    "created_at": "2015-10-28T08:38:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349147",
    "user": "https://github.com/jdemeyer"
}
```

Changing reviewer from "Jernej Azarija" to "Jernej Azarija, Jeroen Demeyer"



---

archive/issue_comments_349148.json:
```json
{
    "body": "<a id='comment:16'></a>\nOh, I see. Cool. Then I added this empty line uselessly. It's a good news though, it felt weird that Cython would not detect that `:-)`\n\nThanks,\n\nNathann",
    "created_at": "2015-10-28T08:41:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349148",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:16'></a>
Oh, I see. Cool. Then I added this empty line uselessly. It's a good news though, it felt weird that Cython would not detect that `:-)`

Thanks,

Nathann



---

archive/issue_comments_349149.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2015-10-28T17:59:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349149",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_comments_349150.json:
```json
{
    "body": "Changing branch from \"u/ncohen/19483\" to \"ea80cfd6f3e3e9f61d8d64a560d22428a9509dd7\"",
    "created_at": "2015-10-28T17:59:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19483#issuecomment-349150",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/ncohen/19483" to "ea80cfd6f3e3e9f61d8d64a560d22428a9509dd7"



---

archive/issue_events_053747.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-10-28T17:59:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/19483",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/19483#event-53747"
}
```
