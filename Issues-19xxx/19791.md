# Issue 19791: doctest fix for tan(complex).imag() wrong

archive/issues_019554.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: tan(2+I).n()\n-0.243458201185725 + 1.16673625724092*I\nsage: tan(2+I).imag().n()\n0.0605381745356926\nsage: tan(2+I).imag()\ntanh(1)/(tan(2)^2*tan(1)^2 + 1)\n```\n\nThere is a form that works at\nhttp://dlmf.nist.gov/4.21.E39\n\nSee https://github.com/pynac/pynac/issues/114\n\nSince this is fixed in Sage now, this ticket should provide doctests.\n\nBranch/Commit: **[7a617e3](https://github.com/sagemath/sagetrac-mirror/commit/7a617e37e6f9fab3b04c5180eab5784b78c434f9)**\n\nReviewer: **Jeroen Demeyer**\n\nAuthor: **Ralf Stephan**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19791_\n\n",
    "closed_at": "2016-01-20T10:20:01Z",
    "created_at": "2015-12-28T16:32:55Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "doctest fix for tan(complex).imag() wrong",
    "type": "issue",
    "updated_at": "2016-01-20T10:20:01Z",
    "url": "https://github.com/sagemath/sage/issues/19791",
    "user": "https://github.com/rwst"
}
```

```
sage: tan(2+I).n()
-0.243458201185725 + 1.16673625724092*I
sage: tan(2+I).imag().n()
0.0605381745356926
sage: tan(2+I).imag()
tanh(1)/(tan(2)^2*tan(1)^2 + 1)
```

There is a form that works at
http://dlmf.nist.gov/4.21.E39

See https://github.com/pynac/pynac/issues/114

Since this is fixed in Sage now, this ticket should provide doctests.

Branch/Commit: **[7a617e3](https://github.com/sagemath/sagetrac-mirror/commit/7a617e37e6f9fab3b04c5180eab5784b78c434f9)**

Reviewer: **Jeroen Demeyer**

Author: **Ralf Stephan**

_Issue created by migration from https://trac.sagemath.org/ticket/19791_





---

archive/issue_events_262085.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-12-28T16:32:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19791",
    "milestone_number": null,
    "milestone_title": "sage-7.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19791#event-262085"
}
```



---

archive/issue_events_262086.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-12-28T16:32:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19791",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19791#event-262086"
}
```



---

archive/issue_events_262087.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-12-28T16:32:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19791",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19791#event-262087"
}
```



---

archive/issue_events_262088.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-12-28T16:32:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19791",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19791#event-262088"
}
```



---

archive/issue_comments_285407.json:
```json
{
    "body": "Changed upstream from **Reported upstream. Developers acknowledge bug.** to **Fixed upstream, in a later stable release.**",
    "created_at": "2015-12-28T16:49:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19791",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19791#issuecomment-285407",
    "user": "https://github.com/rwst"
}
```

Changed upstream from **Reported upstream. Developers acknowledge bug.** to **Fixed upstream, in a later stable release.**



---

archive/issue_comments_285408.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nThat bug is in GiNaC too. Fix is in pynac git master, so will be in pynac-0.5.4.",
    "created_at": "2015-12-28T16:49:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19791",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19791#issuecomment-285408",
    "user": "https://github.com/rwst"
}
```

<div id="comment:1" align="right">Comment 1</div>

That bug is in GiNaC too. Fix is in pynac git master, so will be in pynac-0.5.4.



---

archive/issue_comments_285409.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nFixed by update to Pynac 0.6.0 at #19819?",
    "created_at": "2016-01-13T09:30:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19791",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19791#issuecomment-285409",
    "user": "https://github.com/slel"
}
```

<div id="comment:2" align="right">Comment 2</div>

Fixed by update to Pynac 0.6.0 at #19819?



---

archive/issue_comments_285410.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -12,3 +12,5 @@\n http://dlmf.nist.gov/4.21.E39\n \n See https://github.com/pynac/pynac/issues/114\n+\n+Since this is fixed in Sage now, this ticket should provide doctests.\n``````\n",
    "created_at": "2016-01-13T09:37:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19791",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19791#issuecomment-285410",
    "user": "https://github.com/rwst"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -12,3 +12,5 @@
 http://dlmf.nist.gov/4.21.E39
 
 See https://github.com/pynac/pynac/issues/114
+
+Since this is fixed in Sage now, this ticket should provide doctests.
``````




---

archive/issue_comments_285411.json:
```json
{
    "body": "Branch: **[u/rws/tan_complex__imag___wrong](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/tan_complex__imag___wrong)**",
    "created_at": "2016-01-18T07:43:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19791",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19791#issuecomment-285411",
    "user": "https://github.com/rwst"
}
```

Branch: **[u/rws/tan_complex__imag___wrong](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/tan_complex__imag___wrong)**



---

archive/issue_events_262089.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2016-01-18T07:44:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19791",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19791#event-262089"
}
```



---

archive/issue_events_262090.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2016-01-18T07:44:26Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/19791",
    "title_is": "doctest fix for tan(complex).imag() wrong",
    "title_was": "tan(complex).imag() wrong",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19791#event-262090"
}
```



---

archive/issue_comments_285412.json:
```json
{
    "body": "Author: **Ralf Stephan**",
    "created_at": "2016-01-18T07:44:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19791",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19791#issuecomment-285412",
    "user": "https://github.com/rwst"
}
```

Author: **Ralf Stephan**



---

archive/issue_comments_285413.json:
```json
{
    "body": "Changed upstream from **Fixed upstream, in a later stable release.** to none",
    "created_at": "2016-01-18T07:44:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19791",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19791#issuecomment-285413",
    "user": "https://github.com/rwst"
}
```

Changed upstream from **Fixed upstream, in a later stable release.** to none



---

archive/issue_comments_285414.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7a617e37e6f9fab3b04c5180eab5784b78c434f9\">7a617e3</a></td><td><code>19791: doctest fix for tan.imag</code></td></tr></table>\n",
    "created_at": "2016-01-18T07:44:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19791",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19791#issuecomment-285414",
    "user": "https://github.com/rwst"
}
```

<div id="comment:5" align="right">Comment 5</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7a617e37e6f9fab3b04c5180eab5784b78c434f9">7a617e3</a></td><td><code>19791: doctest fix for tan.imag</code></td></tr></table>




---

archive/issue_comments_285415.json:
```json
{
    "body": "Commit: **[7a617e3](https://github.com/sagemath/sagetrac-mirror/commit/7a617e37e6f9fab3b04c5180eab5784b78c434f9)**",
    "created_at": "2016-01-18T07:44:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19791",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19791#issuecomment-285415",
    "user": "https://github.com/rwst"
}
```

Commit: **[7a617e3](https://github.com/sagemath/sagetrac-mirror/commit/7a617e37e6f9fab3b04c5180eab5784b78c434f9)**



---

archive/issue_events_262091.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-01-18T09:50:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19791",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19791#event-262091"
}
```



---

archive/issue_events_262092.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-01-18T09:50:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19791",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19791#event-262092"
}
```



---

archive/issue_comments_285416.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2016-01-18T09:50:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19791",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19791#issuecomment-285416",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_262093.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-01-20T10:20:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19791",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19791#event-262093"
}
```



---

archive/issue_events_262094.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "8ba9f5bd5aa3d8c86818603777f8e2c71c577b05",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-01-20T10:20:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19791",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19791#event-262094"
}
```



---

archive/issue_comments_285417.json:
```json
{
    "body": "Changed branch from **[u/rws/tan_complex__imag___wrong](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/tan_complex__imag___wrong)** to **[7a617e3](https://github.com/sagemath/sagetrac-mirror/commit/7a617e37e6f9fab3b04c5180eab5784b78c434f9)**",
    "created_at": "2016-01-20T10:20:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19791",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19791#issuecomment-285417",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/rws/tan_complex__imag___wrong](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/tan_complex__imag___wrong)** to **[7a617e3](https://github.com/sagemath/sagetrac-mirror/commit/7a617e37e6f9fab3b04c5180eab5784b78c434f9)**
