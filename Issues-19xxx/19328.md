# Issue 19328: Wrap automorphism group of groups better

archive/issues_019091.json:
```json
{
    "assignees": [],
    "body": "See [this math.SX question](http://math.stackexchange.com/questions/1458109/compute-automorphism-group-using-computer-software) and [this ask question](http://ask.sagemath.org/question/29658/error-when-computing-automorphism-group/).\n\n```\nG = CyclicPermutationGroup(4)\nH = CyclicPermutationGroup(2)\nD=G.direct_product(H)\nD.AutomorphismGroup()\n<boom>\n```\nOkay, that's because you receive a tuple back.  But\n\n```\nd = D[0]\nd.AutomorphismGroup()\n<boom for different and more annoying reason>\n```\nAlso, there is no `gap()` method for such groups, only a `_gap_()` method, which most people will not know about.\n\nCC:  @nathanncohen\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19328_\n\n",
    "created_at": "2015-10-01T18:46:52Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20group%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Wrap automorphism group of groups better",
    "type": "issue",
    "updated_at": "2022-12-29T01:33:44Z",
    "url": "https://github.com/sagemath/sage/issues/19328",
    "user": "https://github.com/kcrisman"
}
```
See [this math.SX question](http://math.stackexchange.com/questions/1458109/compute-automorphism-group-using-computer-software) and [this ask question](http://ask.sagemath.org/question/29658/error-when-computing-automorphism-group/).

```
G = CyclicPermutationGroup(4)
H = CyclicPermutationGroup(2)
D=G.direct_product(H)
D.AutomorphismGroup()
<boom>
```
Okay, that's because you receive a tuple back.  But

```
d = D[0]
d.AutomorphismGroup()
<boom for different and more annoying reason>
```
Also, there is no `gap()` method for such groups, only a `_gap_()` method, which most people will not know about.

CC:  @nathanncohen

_Issue created by migration from https://trac.sagemath.org/ticket/19328_





---

archive/issue_events_256427.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2015-10-01T18:46:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19328",
    "milestone_number": null,
    "milestone_title": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19328#event-256427"
}
```



---

archive/issue_events_256428.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2015-10-01T18:46:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19328",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20group%20theory",
    "label_color": "0000ff",
    "label_name": "component: group theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19328#event-256428"
}
```



---

archive/issue_events_256429.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2015-10-01T18:46:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19328",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19328#event-256429"
}
```



---

archive/issue_events_256430.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2015-10-01T18:46:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19328",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19328#event-256430"
}
```



---

archive/issue_comments_276997.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-See [this math.SX question](http://math.stackexchange.com/questions/1458109/compute-automorphism-group-using-computer-software).  \n+See [this math.SX question](http://math.stackexchange.com/questions/1458109/compute-automorphism-group-using-computer-software) and [this ask question](http://ask.sagemath.org/question/29658/error-when-computing-automorphism-group/).\n \n ```\n G = CyclicPermutationGroup(4)\n``````\n",
    "created_at": "2015-10-01T19:39:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19328#issuecomment-276997",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-See [this math.SX question](http://math.stackexchange.com/questions/1458109/compute-automorphism-group-using-computer-software).  
+See [this math.SX question](http://math.stackexchange.com/questions/1458109/compute-automorphism-group-using-computer-software) and [this ask question](http://ask.sagemath.org/question/29658/error-when-computing-automorphism-group/).
 
 ```
 G = CyclicPermutationGroup(4)
``````




---

archive/issue_events_256431.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:33:44Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/19328",
    "milestone_number": null,
    "milestone_title": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19328#event-256431"
}
```
