# Issue 19101: Change sage -i to install with dependencies

archive/issues_018864.json:
```json
{
    "assignees": [],
    "body": "* Move the current functionality of `sage -i` to `sage -p` (without any functional changes).\n* Add a new implementation for `sage -i` which installs new-style packages with dependencies.\n* Make `sage -f` equivalent to `sage -i -f`, which installs dependencies and force-reinstalls a package.\n\nFor documentation changes, see #18859.\n\nDepends on #19103\nDepends on #19104\n\nCC:  @rwst @nathanncohen\n\nBranch: **[e856a8e](https://github.com/sagemath/sagetrac-mirror/commit/e856a8e872282fe7f059900559f5e589f1b016aa)**\n\nReviewer: **Volker Braun**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19101_\n\n",
    "closed_at": "2015-09-07T06:57:19Z",
    "created_at": "2015-08-27T14:09:47Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20scripts",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Change sage -i to install with dependencies",
    "type": "issue",
    "updated_at": "2015-09-15T20:10:17Z",
    "url": "https://github.com/sagemath/sage/issues/19101",
    "user": "https://github.com/jdemeyer"
}
```
* Move the current functionality of `sage -i` to `sage -p` (without any functional changes).
* Add a new implementation for `sage -i` which installs new-style packages with dependencies.
* Make `sage -f` equivalent to `sage -i -f`, which installs dependencies and force-reinstalls a package.

For documentation changes, see #18859.

Depends on #19103
Depends on #19104

CC:  @rwst @nathanncohen

Branch: **[e856a8e](https://github.com/sagemath/sagetrac-mirror/commit/e856a8e872282fe7f059900559f5e589f1b016aa)**

Reviewer: **Volker Braun**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/19101_





---

archive/issue_events_253331.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-08-27T14:09:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "milestone_number": null,
    "milestone_title": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19101#event-253331"
}
```



---

archive/issue_events_253332.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-08-27T14:09:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20scripts",
    "label_color": "000080",
    "label_name": "component: scripts",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19101#event-253332"
}
```



---

archive/issue_events_253333.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-08-27T14:09:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19101#event-253333"
}
```



---

archive/issue_events_253334.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-08-27T14:09:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19101#event-253334"
}
```



---

archive/issue_comments_272318.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,2 +1,3 @@\n-* Move the current functionality of `sage -i` to `sage -p` (without any functional changes)\n+* Move the current functionality of `sage -i` to `sage -p` (without any functional changes).\n+* Make `sage -f` equivalent to `sage -p -f`.\n * Add a new implementation for `sage -i` which installs new-style packages with dependencies.\n``````\n",
    "created_at": "2015-08-27T14:27:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19101#issuecomment-272318",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,2 +1,3 @@
-* Move the current functionality of `sage -i` to `sage -p` (without any functional changes)
+* Move the current functionality of `sage -i` to `sage -p` (without any functional changes).
+* Make `sage -f` equivalent to `sage -p -f`.
 * Add a new implementation for `sage -i` which installs new-style packages with dependencies.
``````




---

archive/issue_comments_272319.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n * Move the current functionality of `sage -i` to `sage -p` (without any functional changes).\n-* Make `sage -f` equivalent to `sage -p -f`.\n * Add a new implementation for `sage -i` which installs new-style packages with dependencies.\n+* Make `sage -f` equivalent to `sage -i -f`, which installs dependencies and force-reinstalls a package.\n``````\n",
    "created_at": "2015-08-27T14:57:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19101#issuecomment-272319",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 * Move the current functionality of `sage -i` to `sage -p` (without any functional changes).
-* Make `sage -f` equivalent to `sage -p -f`.
 * Add a new implementation for `sage -i` which installs new-style packages with dependencies.
+* Make `sage -f` equivalent to `sage -i -f`, which installs dependencies and force-reinstalls a package.
``````




---

archive/issue_comments_272320.json:
```json
{
    "body": "Dependencies: **#19103**",
    "created_at": "2015-08-27T15:19:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19101#issuecomment-272320",
    "user": "https://github.com/jdemeyer"
}
```

Dependencies: **#19103**



---

archive/issue_comments_272321.json:
```json
{
    "body": "Changed dependencies from **#19103** to **#19103, #19104**",
    "created_at": "2015-08-27T17:49:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19101#issuecomment-272321",
    "user": "https://github.com/jdemeyer"
}
```

Changed dependencies from **#19103** to **#19103, #19104**



---

archive/issue_comments_272322.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,5 @@\n * Move the current functionality of `sage -i` to `sage -p` (without any functional changes).\n * Add a new implementation for `sage -i` which installs new-style packages with dependencies.\n * Make `sage -f` equivalent to `sage -i -f`, which installs dependencies and force-reinstalls a package.\n+\n+For documentation changes, see #18859.\n``````\n",
    "created_at": "2015-08-27T18:57:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19101#issuecomment-272322",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,5 @@
 * Move the current functionality of `sage -i` to `sage -p` (without any functional changes).
 * Add a new implementation for `sage -i` which installs new-style packages with dependencies.
 * Make `sage -f` equivalent to `sage -i -f`, which installs dependencies and force-reinstalls a package.
+
+For documentation changes, see #18859.
``````




---

archive/issue_comments_272323.json:
```json
{
    "body": "Branch: **[u/jdemeyer/change_sage__i_to_install_with_dependencies](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/change_sage__i_to_install_with_dependencies)**",
    "created_at": "2015-08-27T19:00:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19101#issuecomment-272323",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/change_sage__i_to_install_with_dependencies](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/change_sage__i_to_install_with_dependencies)**



---

archive/issue_comments_272324.json:
```json
{
    "body": "Commit: **[e856a8e](https://github.com/sagemath/sagetrac-mirror/commit/e856a8e872282fe7f059900559f5e589f1b016aa)**",
    "created_at": "2015-08-27T20:09:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19101#issuecomment-272324",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[e856a8e](https://github.com/sagemath/sagetrac-mirror/commit/e856a8e872282fe7f059900559f5e589f1b016aa)**



---

archive/issue_events_253335.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-08-27T20:09:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19101#event-253335"
}
```



---

archive/issue_comments_272325.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/31ff6e7884b7154077abf29c66ab81c9da37a515\">31ff6e7</a></td><td><code>Parse sage-spkg options in sage-spkg</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cb05746662de891c8444009150962c1b2bfe941f\">cb05746</a></td><td><code>Add sage --location option to run sage-location</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e856a8e872282fe7f059900559f5e589f1b016aa\">e856a8e</a></td><td><code>Change sage -i to install with dependencies</code></td></tr></table>\n",
    "created_at": "2015-08-27T20:09:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19101#issuecomment-272325",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:7" align="right">Comment 7</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/31ff6e7884b7154077abf29c66ab81c9da37a515">31ff6e7</a></td><td><code>Parse sage-spkg options in sage-spkg</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cb05746662de891c8444009150962c1b2bfe941f">cb05746</a></td><td><code>Add sage --location option to run sage-location</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e856a8e872282fe7f059900559f5e589f1b016aa">e856a8e</a></td><td><code>Change sage -i to install with dependencies</code></td></tr></table>




---

archive/issue_events_253336.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-08-28T12:55:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19101#event-253336"
}
```



---

archive/issue_events_253337.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-08-28T12:55:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19101#event-253337"
}
```



---

archive/issue_comments_272326.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2015-08-28T12:55:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19101#issuecomment-272326",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_events_253338.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-08-31T12:46:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19101#event-253338"
}
```



---

archive/issue_events_253339.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "57cbc03a77809b1bd939635a71b4d5e04d76c8a2",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-08-31T12:46:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19101#event-253339"
}
```



---

archive/issue_comments_272327.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/change_sage__i_to_install_with_dependencies](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/change_sage__i_to_install_with_dependencies)** to **[e856a8e](https://github.com/sagemath/sagetrac-mirror/commit/e856a8e872282fe7f059900559f5e589f1b016aa)**",
    "created_at": "2015-08-31T12:46:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19101#issuecomment-272327",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/change_sage__i_to_install_with_dependencies](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/change_sage__i_to_install_with_dependencies)** to **[e856a8e](https://github.com/sagemath/sagetrac-mirror/commit/e856a8e872282fe7f059900559f5e589f1b016aa)**



---

archive/issue_comments_272328.json:
```json
{
    "body": "Changed branch from **[e856a8e](https://github.com/sagemath/sagetrac-mirror/commit/e856a8e872282fe7f059900559f5e589f1b016aa)** to **[u/jdemeyer/change_sage__i_to_install_with_dependencies](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/change_sage__i_to_install_with_dependencies)**",
    "created_at": "2015-09-04T18:16:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19101#issuecomment-272328",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[e856a8e](https://github.com/sagemath/sagetrac-mirror/commit/e856a8e872282fe7f059900559f5e589f1b016aa)** to **[u/jdemeyer/change_sage__i_to_install_with_dependencies](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/change_sage__i_to_install_with_dependencies)**



---

archive/issue_comments_272329.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nReopening because of #19104",
    "created_at": "2015-09-04T18:16:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19101#issuecomment-272329",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:10" align="right">Comment 10</div>

Reopening because of #19104



---

archive/issue_events_253340.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-09-04T21:33:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19101#event-253340"
}
```



---

archive/issue_events_253341.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-09-04T21:33:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19101#event-253341"
}
```



---

archive/issue_events_253342.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-09-04T21:33:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19101#event-253342"
}
```



---

archive/issue_comments_272330.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/change_sage__i_to_install_with_dependencies](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/change_sage__i_to_install_with_dependencies)** to **[e856a8e](https://github.com/sagemath/sagetrac-mirror/commit/e856a8e872282fe7f059900559f5e589f1b016aa)**",
    "created_at": "2015-09-07T06:57:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19101#issuecomment-272330",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/change_sage__i_to_install_with_dependencies](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/change_sage__i_to_install_with_dependencies)** to **[e856a8e](https://github.com/sagemath/sagetrac-mirror/commit/e856a8e872282fe7f059900559f5e589f1b016aa)**



---

archive/issue_events_253343.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-07T06:57:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19101#event-253343"
}
```



---

archive/issue_events_253344.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-07T06:57:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19101#event-253344"
}
```



---

archive/issue_comments_272331.json:
```json
{
    "body": "Changed commit from **[e856a8e](https://github.com/sagemath/sagetrac-mirror/commit/e856a8e872282fe7f059900559f5e589f1b016aa)** to none",
    "created_at": "2015-09-15T19:38:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19101#issuecomment-272331",
    "user": "https://github.com/jhpalmieri"
}
```

Changed commit from **[e856a8e](https://github.com/sagemath/sagetrac-mirror/commit/e856a8e872282fe7f059900559f5e589f1b016aa)** to none



---

archive/issue_comments_272332.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nThe new doctest added in this ticket creates a log file `logs/pkgs/sqlite.log`. This means that it will fail if the user does not have the appropriate permissions in `logs/pkgs/`. This probably needs to be fixed.",
    "created_at": "2015-09-15T19:38:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19101#issuecomment-272332",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:14" align="right">Comment 14</div>

The new doctest added in this ticket creates a log file `logs/pkgs/sqlite.log`. This means that it will fail if the user does not have the appropriate permissions in `logs/pkgs/`. This probably needs to be fixed.



---

archive/issue_comments_272333.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">Comment 15</div>\n\nIt's really #19104, but I guess you're right.",
    "created_at": "2015-09-15T20:10:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19101",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19101#issuecomment-272333",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:15" align="right">Comment 15</div>

It's really #19104, but I guess you're right.
