# Issue 19104: Parse sage-spkg options in sage-spkg

archive/issues_018867.json:
```json
{
    "assignees": [],
    "body": "Instead of parsing the options to `sage -i` both in `src/bin/sage` and in `sage-spkg`, parse them just in `sage-spkg` (without requiring a specific order).\n\nBranch/Commit: **[4eb1aa8](https://github.com/sagemath/sagetrac-mirror/commit/4eb1aa8482e435f68c9e2c4882336647f1bf233c)**\n\nReviewer: **Volker Braun**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19104_\n\n",
    "closed_at": "2015-09-06T09:41:48Z",
    "created_at": "2015-08-27T17:39:19Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20scripts",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Parse sage-spkg options in sage-spkg",
    "type": "issue",
    "updated_at": "2015-09-06T09:41:48Z",
    "url": "https://github.com/sagemath/sage/issues/19104",
    "user": "https://github.com/jdemeyer"
}
```
Instead of parsing the options to `sage -i` both in `src/bin/sage` and in `sage-spkg`, parse them just in `sage-spkg` (without requiring a specific order).

Branch/Commit: **[4eb1aa8](https://github.com/sagemath/sagetrac-mirror/commit/4eb1aa8482e435f68c9e2c4882336647f1bf233c)**

Reviewer: **Volker Braun**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/19104_





---

archive/issue_events_253367.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-08-27T17:39:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "milestone_number": null,
    "milestone_title": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19104#event-253367"
}
```



---

archive/issue_events_253368.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-08-27T17:39:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20scripts",
    "label_color": "000080",
    "label_name": "component: scripts",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19104#event-253368"
}
```



---

archive/issue_events_253369.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-08-27T17:39:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19104#event-253369"
}
```



---

archive/issue_events_253370.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-08-27T17:39:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19104#event-253370"
}
```



---

archive/issue_comments_272356.json:
```json
{
    "body": "Branch: **[u/jdemeyer/parse_sage_spkg_options_in_sage_spkg](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/parse_sage_spkg_options_in_sage_spkg)**",
    "created_at": "2015-08-27T18:03:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19104#issuecomment-272356",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/parse_sage_spkg_options_in_sage_spkg](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/parse_sage_spkg_options_in_sage_spkg)**



---

archive/issue_comments_272357.json:
```json
{
    "body": "Commit: **[31ff6e7](https://github.com/sagemath/sagetrac-mirror/commit/31ff6e7884b7154077abf29c66ab81c9da37a515)**",
    "created_at": "2015-08-27T18:40:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19104#issuecomment-272357",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[31ff6e7](https://github.com/sagemath/sagetrac-mirror/commit/31ff6e7884b7154077abf29c66ab81c9da37a515)**



---

archive/issue_comments_272358.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/31ff6e7884b7154077abf29c66ab81c9da37a515\">31ff6e7</a></td><td><code>Parse sage-spkg options in sage-spkg</code></td></tr></table>\n",
    "created_at": "2015-08-27T18:40:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19104#issuecomment-272358",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/31ff6e7884b7154077abf29c66ab81c9da37a515">31ff6e7</a></td><td><code>Parse sage-spkg options in sage-spkg</code></td></tr></table>




---

archive/issue_events_253371.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-08-27T18:41:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19104#event-253371"
}
```



---

archive/issue_comments_272359.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2015-08-28T12:56:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19104#issuecomment-272359",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_events_253372.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-08-28T12:56:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19104#event-253372"
}
```



---

archive/issue_events_253373.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-08-28T12:56:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19104#event-253373"
}
```



---

archive/issue_comments_272360.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/parse_sage_spkg_options_in_sage_spkg](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/parse_sage_spkg_options_in_sage_spkg)** to **[31ff6e7](https://github.com/sagemath/sagetrac-mirror/commit/31ff6e7884b7154077abf29c66ab81c9da37a515)**",
    "created_at": "2015-08-29T23:56:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19104#issuecomment-272360",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/parse_sage_spkg_options_in_sage_spkg](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/parse_sage_spkg_options_in_sage_spkg)** to **[31ff6e7](https://github.com/sagemath/sagetrac-mirror/commit/31ff6e7884b7154077abf29c66ab81c9da37a515)**



---

archive/issue_events_253374.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-08-29T23:56:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19104#event-253374"
}
```



---

archive/issue_events_253375.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-08-29T23:56:36Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19104#event-253375"
}
```



---

archive/issue_comments_272361.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nThis ticket breaks `sage -sdist` for me",
    "created_at": "2015-09-04T18:14:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19104#issuecomment-272361",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:6" align="right">Comment 6</div>

This ticket breaks `sage -sdist` for me



---

archive/issue_comments_272362.json:
```json
{
    "body": "Changed commit from **[31ff6e7](https://github.com/sagemath/sagetrac-mirror/commit/31ff6e7884b7154077abf29c66ab81c9da37a515)** to none",
    "created_at": "2015-09-04T18:14:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19104#issuecomment-272362",
    "user": "https://github.com/vbraun"
}
```

Changed commit from **[31ff6e7](https://github.com/sagemath/sagetrac-mirror/commit/31ff6e7884b7154077abf29c66ab81c9da37a515)** to none



---

archive/issue_comments_272363.json:
```json
{
    "body": "Changed branch from **[31ff6e7](https://github.com/sagemath/sagetrac-mirror/commit/31ff6e7884b7154077abf29c66ab81c9da37a515)** to **[u/jdemeyer/parse_sage_spkg_options_in_sage_spkg](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/parse_sage_spkg_options_in_sage_spkg)**",
    "created_at": "2015-09-04T18:15:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19104#issuecomment-272363",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[31ff6e7](https://github.com/sagemath/sagetrac-mirror/commit/31ff6e7884b7154077abf29c66ab81c9da37a515)** to **[u/jdemeyer/parse_sage_spkg_options_in_sage_spkg](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/parse_sage_spkg_options_in_sage_spkg)**



---

archive/issue_comments_272364.json:
```json
{
    "body": "Commit: **[31ff6e7](https://github.com/sagemath/sagetrac-mirror/commit/31ff6e7884b7154077abf29c66ab81c9da37a515)**",
    "created_at": "2015-09-04T18:15:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19104#issuecomment-272364",
    "user": "https://github.com/vbraun"
}
```

Commit: **[31ff6e7](https://github.com/sagemath/sagetrac-mirror/commit/31ff6e7884b7154077abf29c66ab81c9da37a515)**



---

archive/issue_comments_272365.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/31ff6e7884b7154077abf29c66ab81c9da37a515\">31ff6e7</a></td><td><code>Parse sage-spkg options in sage-spkg</code></td></tr></table>\n",
    "created_at": "2015-09-04T18:15:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19104#issuecomment-272365",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:7" align="right">Comment 7</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/31ff6e7884b7154077abf29c66ab81c9da37a515">31ff6e7</a></td><td><code>Parse sage-spkg options in sage-spkg</code></td></tr></table>




---

archive/issue_comments_272366.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nTries to compile patch and then fails with weird errors, something wrong with environment variables I guess. \n\nPS: I can compile just patch (`sage -f patch`) just fine, for the record",
    "created_at": "2015-09-04T18:23:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19104#issuecomment-272366",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:8" align="right">Comment 8</div>

Tries to compile patch and then fails with weird errors, something wrong with environment variables I guess. 

PS: I can compile just patch (`sage -f patch`) just fine, for the record



---

archive/issue_comments_272367.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nDid you test with just this ticket, or with #19101 and other tickets?",
    "created_at": "2015-09-04T20:40:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19104#issuecomment-272367",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:9" align="right">Comment 9</div>

Did you test with just this ticket, or with #19101 and other tickets?



---

archive/issue_comments_272368.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nGot it, just forgot support for `SAGE_INSTALL_FETCH_ONLY`.",
    "created_at": "2015-09-04T21:32:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19104#issuecomment-272368",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:10" align="right">Comment 10</div>

Got it, just forgot support for `SAGE_INSTALL_FETCH_ONLY`.



---

archive/issue_comments_272369.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4eb1aa8482e435f68c9e2c4882336647f1bf233c\">4eb1aa8</a></td><td><code>Restore SAGE_INSTALL_FETCH_ONLY support</code></td></tr></table>\n",
    "created_at": "2015-09-05T07:42:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19104#issuecomment-272369",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:11" align="right">Comment 11</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4eb1aa8482e435f68c9e2c4882336647f1bf233c">4eb1aa8</a></td><td><code>Restore SAGE_INSTALL_FETCH_ONLY support</code></td></tr></table>




---

archive/issue_comments_272370.json:
```json
{
    "body": "Changed commit from **[31ff6e7](https://github.com/sagemath/sagetrac-mirror/commit/31ff6e7884b7154077abf29c66ab81c9da37a515)** to **[4eb1aa8](https://github.com/sagemath/sagetrac-mirror/commit/4eb1aa8482e435f68c9e2c4882336647f1bf233c)**",
    "created_at": "2015-09-05T07:42:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19104#issuecomment-272370",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[31ff6e7](https://github.com/sagemath/sagetrac-mirror/commit/31ff6e7884b7154077abf29c66ab81c9da37a515)** to **[4eb1aa8](https://github.com/sagemath/sagetrac-mirror/commit/4eb1aa8482e435f68c9e2c4882336647f1bf233c)**



---

archive/issue_events_253376.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-09-05T08:13:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19104#event-253376"
}
```



---

archive/issue_events_253377.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-05T22:09:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19104#event-253377"
}
```



---

archive/issue_events_253378.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-05T22:09:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19104#event-253378"
}
```



---

archive/issue_comments_272371.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/parse_sage_spkg_options_in_sage_spkg](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/parse_sage_spkg_options_in_sage_spkg)** to **[4eb1aa8](https://github.com/sagemath/sagetrac-mirror/commit/4eb1aa8482e435f68c9e2c4882336647f1bf233c)**",
    "created_at": "2015-09-06T09:41:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19104#issuecomment-272371",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/parse_sage_spkg_options_in_sage_spkg](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/parse_sage_spkg_options_in_sage_spkg)** to **[4eb1aa8](https://github.com/sagemath/sagetrac-mirror/commit/4eb1aa8482e435f68c9e2c4882336647f1bf233c)**



---

archive/issue_events_253379.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-06T09:41:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19104#event-253379"
}
```



---

archive/issue_events_253380.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "81e09c340a2459f221d8afa7ab6e7e0f2d6481bc",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-09-06T09:41:48Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19104",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19104#event-253380"
}
```
