...
```json
{
    "body": "This will draw from the corresponding code for Lie algebras currently in #14901.",
...
```json
{
    "body": "I pulled some changes into #19448 an will need to rebase this over #19448.",
    "created_at": "2015-10-21T19:41:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19359",
    "type": "issue_comment",
...
```json
{
    "body": "The better way would be to just write a separate class (probably inheriting from `CombinatorialFreeModule`), implement a `lift` (and `retract`?) method which is the natural inclusion into the Steenrod algebra, have it be in the corresponding `Subobjects()` category, and make sure it knows the coercion.\n\nAlthough for graded algebras with finite-dimensional components, this should be doable in a generic fashion (and would be useful for the combinatorial Hopf algebras). Something to work on after #17367.",
