...
<a id='comment:11'></a>Replying to [comment:5 dcoudert]:
...
<a id='comment:33'></a>Replying to [comment:13 dcoudert]:
> {{{
> cdef int i
> cdef list output = list(list() for i in range(nscc))  # We cannot use [] here
> for i,v in enumerate(G.vertex_iterator()):
