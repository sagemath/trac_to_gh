...
```json
{
    "body": "<a id='comment:4'></a>And in case SMC is not quite working, an alternative link https://www.dropbox.com/s/rkrvybyyh5zuv5z/jmol-14.6.1_2016.07.11.tar.bz2?dl=0",
...
<a id='comment:4'></a>And in case SMC is not quite working, an alternative link https://www.dropbox.com/s/rkrvybyyh5zuv5z/jmol-14.6.1_2016.07.11.tar.bz2?dl=0
...
```json
{
    "body": "<a id='comment:6'></a>Since it does not have to depend on #20933 and that one needs to be updated, I've rebased.",
...
<a id='comment:6'></a>Since it does not have to depend on #20933 and that one needs to be updated, I've rebased.
...
```json
{
    "body": "<a id='comment:7'></a>The updated package seems to work for me; but #20978 still doesn't work with the new version.",
...
<a id='comment:7'></a>The updated package seems to work for me; but #20978 still doesn't work with the new version.
...
<a id='comment:10'></a>I can't download it:
`Error opening 'jmol-14.6.1_2016.07.11.tar.bz2' -- "Traceback (most recent call last):\n File \"/usr/local/bin/smc-compute\", line 9, in \n load_entry_point('smc-pyutil==1.1', 'console_scripts', 'smc-compute')()\n File \"/usr/local/lib/python2.7/dist-packages/smc_pyutil/smc_compute.py\", line 1016, in main\n args.func(args)\n File \"/usr/local/lib/python2.7/dist-packages/smc_pyutil/smc_compute.py\", line 886, in g\n result = getattr(Project(project_id=project_id, dev=args.dev, projects=args.projects, single=args.single), function)(**kwds)\n File \"/usr/local/lib/python2.7/dist-packages/smc_pyutil/smc_compute.py\", line 681, in read_file\n raise RuntimeError(\"path (=%s) must be at most %s bytes, but it is %s bytes\"%(path, maxsize, size))\nRuntimeError: path (=jmol-14.6.1_2016.07.11.tar.bz2) must be at most 20000000 bytes, but it is 38106981 bytes\n"`
...
<a id='comment:11'></a>How about this one?
https://www.dropbox.com/s/rkrvybyyh5zuv5z/jmol-14.6.1_2016.07.11.tar.bz2?dl=0
...
```json
{
    "body": "<a id='comment:12'></a>the one from dropbox is what I reviewed. The other gave me a checksum error",
...
<a id='comment:12'></a>the one from dropbox is what I reviewed. The other gave me a checksum error
...
```json
{
    "body": "<a id='comment:14'></a>I'll take the liberty of setting back to positive review since there is a confirmation that this link worked. I am already using this package in SageMathCell, so would be nice to have it in 7.3.",
...
<a id='comment:14'></a>I'll take the liberty of setting back to positive review since there is a confirmation that this link worked. I am already using this package in SageMathCell, so would be nice to have it in 7.3.
...
<a id='comment:15'></a>Apparently SageMathCloud link managed to propagate to the servers and it is now possible to "download" this new jmol with the following message in the package log
