# Issue 19493: convert the ore_algebra from a package to a part of sage

archive/issues_019256.json:
```json
{
    "assignees": [],
    "body": "This ticket aims at integrating fully the code of the package ore_algebra in sage.\n\nhttp://www.risc.jku.at/research/combinat/software/ore_algebra/\n\nCC:  @mkauers @mezzarobba\n\nKeywords: **ore_algebra**\n\nBranch/Commit: **[public/19493](https://github.com/sagemath/sagetrac-mirror/tree/public/19493) @ [fcc2f59](https://github.com/sagemath/sagetrac-mirror/commit/fcc2f59a036d04632ae17f592d8604c0e8145287)**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19493_\n\n",
    "created_at": "2015-10-28T16:11:31Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/wishlist%20item"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "convert the ore_algebra from a package to a part of sage",
    "type": "issue",
    "updated_at": "2022-06-11T07:06:33Z",
    "url": "https://github.com/sagemath/sage/issues/19493",
    "user": "https://github.com/fchapoton"
}
```
This ticket aims at integrating fully the code of the package ore_algebra in sage.

http://www.risc.jku.at/research/combinat/software/ore_algebra/

CC:  @mkauers @mezzarobba

Keywords: **ore_algebra**

Branch/Commit: **[public/19493](https://github.com/sagemath/sagetrac-mirror/tree/public/19493) @ [fcc2f59](https://github.com/sagemath/sagetrac-mirror/commit/fcc2f59a036d04632ae17f592d8604c0e8145287)**

_Issue created by migration from https://trac.sagemath.org/ticket/19493_





---

archive/issue_events_258467.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-10-28T16:11:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19493",
    "milestone_number": null,
    "milestone_title": "sage-6.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19493#event-258467"
}
```



---

archive/issue_events_258468.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-10-28T16:11:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19493",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19493#event-258468"
}
```



---

archive/issue_events_258469.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-10-28T16:11:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19493",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19493#event-258469"
}
```



---

archive/issue_comments_279809.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n This ticket aims at integrating fully the code of the package ore_algebra in sage.\n \n-\n+http://www.risc.jku.at/research/combinat/software/ore_algebra/\n``````\n",
    "created_at": "2016-01-24T10:41:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19493#issuecomment-279809",
    "user": "https://github.com/fchapoton"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 This ticket aims at integrating fully the code of the package ore_algebra in sage.
 
-
+http://www.risc.jku.at/research/combinat/software/ore_algebra/
``````




---

archive/issue_comments_279810.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nI think you need to justify why you want to do this.",
    "created_at": "2016-01-24T10:55:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19493#issuecomment-279810",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

I think you need to justify why you want to do this.



---

archive/issue_comments_279811.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nIf you do this, the code should satisfy the Sage standard of code. In particular, it should be fully doctested (which is far from the case for the current `ore_algebra` package).",
    "created_at": "2016-01-24T10:58:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19493#issuecomment-279811",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:3" align="right">Comment 3</div>

If you do this, the code should satisfy the Sage standard of code. In particular, it should be fully doctested (which is far from the case for the current `ore_algebra` package).



---

archive/issue_comments_279812.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nYes, indeed. The reason I would like to do this is because this code is useful, and including it will prevent us to break this by our numerous and rapid changes and deprecations.\n\nFor the documentation, of course, it will have to meet the requirements.",
    "created_at": "2016-01-24T12:36:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19493#issuecomment-279812",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:4" align="right">Comment 4</div>

Yes, indeed. The reason I would like to do this is because this code is useful, and including it will prevent us to break this by our numerous and rapid changes and deprecations.

For the documentation, of course, it will have to meet the requirements.



---

archive/issue_comments_279813.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThis conversion was also suggested [at #19175](https://github.com/sagemath/sage/issues/19175#comment:4).",
    "created_at": "2016-01-24T20:01:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19493#issuecomment-279813",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:5" align="right">Comment 5</div>

This conversion was also suggested [at #19175](https://github.com/sagemath/sage/issues/19175#comment:4).



---

archive/issue_comments_279814.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nI made a branch, starting from the package version 0.2, but did not change the imports, so this is probably not working yet.\n\nAnd this is seriously **lacking docs**.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8daae04dd4683df8472b97dbdf1efe1e87255e4a\">8daae04</a></td><td><code>trac #19493 initial inclusion of the Ore algebras (NEEDS DOC)</code></td></tr></table>\n",
    "created_at": "2016-01-27T14:30:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19493#issuecomment-279814",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:6" align="right">Comment 6</div>

I made a branch, starting from the package version 0.2, but did not change the imports, so this is probably not working yet.

And this is seriously **lacking docs**.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8daae04dd4683df8472b97dbdf1efe1e87255e4a">8daae04</a></td><td><code>trac #19493 initial inclusion of the Ore algebras (NEEDS DOC)</code></td></tr></table>




---

archive/issue_comments_279815.json:
```json
{
    "body": "Commit: **[8daae04](https://github.com/sagemath/sagetrac-mirror/commit/8daae04dd4683df8472b97dbdf1efe1e87255e4a)**",
    "created_at": "2016-01-27T14:30:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19493#issuecomment-279815",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[8daae04](https://github.com/sagemath/sagetrac-mirror/commit/8daae04dd4683df8472b97dbdf1efe1e87255e4a)**



---

archive/issue_comments_279816.json:
```json
{
    "body": "Branch: **[public/19493](https://github.com/sagemath/sagetrac-mirror/tree/public/19493)**",
    "created_at": "2016-01-27T14:30:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19493#issuecomment-279816",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[public/19493](https://github.com/sagemath/sagetrac-mirror/tree/public/19493)**



---

archive/issue_comments_279817.json:
```json
{
    "body": "Changed commit from **[8daae04](https://github.com/sagemath/sagetrac-mirror/commit/8daae04dd4683df8472b97dbdf1efe1e87255e4a)** to **[b974749](https://github.com/sagemath/sagetrac-mirror/commit/b974749efa7d3dfd0239e8ad2b137a8dc98c5ca2)**",
    "created_at": "2016-01-28T08:52:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19493#issuecomment-279817",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[8daae04](https://github.com/sagemath/sagetrac-mirror/commit/8daae04dd4683df8472b97dbdf1efe1e87255e4a)** to **[b974749](https://github.com/sagemath/sagetrac-mirror/commit/b974749efa7d3dfd0239e8ad2b137a8dc98c5ca2)**



---

archive/issue_comments_279818.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b974749efa7d3dfd0239e8ad2b137a8dc98c5ca2\">b974749</a></td><td><code>trac #19493 change one import, put some raise into py3, whitespace cleanup</code></td></tr></table>\n",
    "created_at": "2016-01-28T08:52:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19493#issuecomment-279818",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:7" align="right">Comment 7</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b974749efa7d3dfd0239e8ad2b137a8dc98c5ca2">b974749</a></td><td><code>trac #19493 change one import, put some raise into py3, whitespace cleanup</code></td></tr></table>




---

archive/issue_comments_279819.json:
```json
{
    "body": "Changed commit from **[b974749](https://github.com/sagemath/sagetrac-mirror/commit/b974749efa7d3dfd0239e8ad2b137a8dc98c5ca2)** to **[c5e4ed6](https://github.com/sagemath/sagetrac-mirror/commit/c5e4ed62feaa9123c7b100f1f0428cf201cbb8af)**",
    "created_at": "2016-10-04T11:55:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19493#issuecomment-279819",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[b974749](https://github.com/sagemath/sagetrac-mirror/commit/b974749efa7d3dfd0239e8ad2b137a8dc98c5ca2)** to **[c5e4ed6](https://github.com/sagemath/sagetrac-mirror/commit/c5e4ed62feaa9123c7b100f1f0428cf201cbb8af)**



---

archive/issue_comments_279820.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c5e4ed62feaa9123c7b100f1f0428cf201cbb8af\">c5e4ed6</a></td><td><code>Merge branch 'public/19493' in 7.4.b6</code></td></tr></table>\n",
    "created_at": "2016-10-04T11:55:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19493#issuecomment-279820",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:8" align="right">Comment 8</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c5e4ed62feaa9123c7b100f1f0428cf201cbb8af">c5e4ed6</a></td><td><code>Merge branch 'public/19493' in 7.4.b6</code></td></tr></table>




---

archive/issue_comments_279821.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fcc2f59a036d04632ae17f592d8604c0e8145287\">fcc2f59</a></td><td><code>turning raise to py3 syntax</code></td></tr></table>\n",
    "created_at": "2016-10-04T14:08:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19493#issuecomment-279821",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:9" align="right">Comment 9</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fcc2f59a036d04632ae17f592d8604c0e8145287">fcc2f59</a></td><td><code>turning raise to py3 syntax</code></td></tr></table>




---

archive/issue_comments_279822.json:
```json
{
    "body": "Changed commit from **[c5e4ed6](https://github.com/sagemath/sagetrac-mirror/commit/c5e4ed62feaa9123c7b100f1f0428cf201cbb8af)** to **[fcc2f59](https://github.com/sagemath/sagetrac-mirror/commit/fcc2f59a036d04632ae17f592d8604c0e8145287)**",
    "created_at": "2016-10-04T14:08:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19493#issuecomment-279822",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[c5e4ed6](https://github.com/sagemath/sagetrac-mirror/commit/c5e4ed62feaa9123c7b100f1f0428cf201cbb8af)** to **[fcc2f59](https://github.com/sagemath/sagetrac-mirror/commit/fcc2f59a036d04632ae17f592d8604c0e8145287)**



---

archive/issue_comments_279823.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nNote that `ore_algebra` is still evolving, see new version at #22870. So if we want a merge to happen (do we?), i guess it should be made in collaboration with upstream developpers. Meanwhile, perhaps should the proposed modifications directly be contributed upstream.",
    "created_at": "2017-04-25T18:05:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19493#issuecomment-279823",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

<div id="comment:10" align="right">Comment 10</div>

Note that `ore_algebra` is still evolving, see new version at #22870. So if we want a merge to happen (do we?), i guess it should be made in collaboration with upstream developpers. Meanwhile, perhaps should the proposed modifications directly be contributed upstream.



---

archive/issue_comments_279824.json:
```json
{
    "body": "Changed keywords from **** to **ore_algebra**",
    "created_at": "2019-06-04T15:37:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19493#issuecomment-279824",
    "user": "https://github.com/fchapoton"
}
```

Changed keywords from **** to **ore_algebra**



---

archive/issue_comments_279825.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nSee also #26332. Upstream is still developing their code, so we should either drop this ticket or invite them to make `ore_algebra` part of the Sage library.",
    "created_at": "2019-06-04T17:14:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19493#issuecomment-279825",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:12" align="right">Comment 12</div>

See also #26332. Upstream is still developing their code, so we should either drop this ticket or invite them to make `ore_algebra` part of the Sage library.



---

archive/issue_events_258470.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-06-11T07:06:33Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/19493",
    "milestone_number": null,
    "milestone_title": "sage-6.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19493#event-258470"
}
```



---

archive/issue_events_258471.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-06-11T07:06:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19493",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19493#event-258471"
}
```
