# Issue 19869: Faster is_less_than() for set partitions

archive/issues_019632.json:
```json
{
    "assignees": [],
    "body": "This is slightly faster version for `is_less_than` on set partitions. For example runtime for\n\n```\nS = SetPartitions(5)\nPoset((S, S.is_less_than))\n```\n\ndrops 25% and 27% on the same with 6 elements.\n\nBranch/Commit: **[90192f7](https://github.com/sagemath/sagetrac-mirror/commit/90192f771d3937976ff352406a25818333399bab)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Jori M\u00e4ntysalo**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19869_\n\n",
    "closed_at": "2016-01-12T22:07:04Z",
    "created_at": "2016-01-12T10:05:01Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Faster is_less_than() for set partitions",
    "type": "issue",
    "updated_at": "2016-01-12T22:07:04Z",
    "url": "https://github.com/sagemath/sage/issues/19869",
    "user": "https://github.com/jm58660"
}
```
This is slightly faster version for `is_less_than` on set partitions. For example runtime for

```
S = SetPartitions(5)
Poset((S, S.is_less_than))
```

drops 25% and 27% on the same with 6 elements.

Branch/Commit: **[90192f7](https://github.com/sagemath/sagetrac-mirror/commit/90192f771d3937976ff352406a25818333399bab)**

Reviewer: **Travis Scrimshaw**

Author: **Jori Mäntysalo**

_Issue created by migration from https://trac.sagemath.org/ticket/19869_





---

archive/issue_events_263047.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-01-12T10:05:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19869",
    "milestone_number": null,
    "milestone_title": "sage-7.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19869#event-263047"
}
```



---

archive/issue_events_263048.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-01-12T10:05:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19869",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19869#event-263048"
}
```



---

archive/issue_events_263049.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-01-12T10:05:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19869",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19869#event-263049"
}
```



---

archive/issue_events_263050.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-01-12T10:05:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19869",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19869#event-263050"
}
```



---

archive/issue_comments_286703.json:
```json
{
    "body": "Branch: **[u/jmantysalo/setpartitions](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/setpartitions)**",
    "created_at": "2016-01-12T10:05:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19869#issuecomment-286703",
    "user": "https://github.com/jm58660"
}
```

Branch: **[u/jmantysalo/setpartitions](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/setpartitions)**



---

archive/issue_events_263051.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-01-12T12:29:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19869",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19869#event-263051"
}
```



---

archive/issue_comments_286704.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/90192f771d3937976ff352406a25818333399bab\">90192f7</a></td><td><code>Faster is_less_than().</code></td></tr></table>\n",
    "created_at": "2016-01-12T12:29:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19869#issuecomment-286704",
    "user": "https://github.com/jm58660"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/90192f771d3937976ff352406a25818333399bab">90192f7</a></td><td><code>Faster is_less_than().</code></td></tr></table>




---

archive/issue_comments_286705.json:
```json
{
    "body": "Commit: **[90192f7](https://github.com/sagemath/sagetrac-mirror/commit/90192f771d3937976ff352406a25818333399bab)**",
    "created_at": "2016-01-12T12:29:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19869#issuecomment-286705",
    "user": "https://github.com/jm58660"
}
```

Commit: **[90192f7](https://github.com/sagemath/sagetrac-mirror/commit/90192f771d3937976ff352406a25818333399bab)**



---

archive/issue_comments_286706.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2016-01-12T12:39:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19869#issuecomment-286706",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_263052.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-01-12T12:39:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19869",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19869#event-263052"
}
```



---

archive/issue_events_263053.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-01-12T12:39:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19869",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19869#event-263053"
}
```



---

archive/issue_comments_286707.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nLGTM.",
    "created_at": "2016-01-12T12:39:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19869#issuecomment-286707",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:3'>Comment 3:</a>
LGTM.



---

archive/issue_comments_286708.json:
```json
{
    "body": "Changed branch from **[u/jmantysalo/setpartitions](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/setpartitions)** to **[90192f7](https://github.com/sagemath/sagetrac-mirror/commit/90192f771d3937976ff352406a25818333399bab)**",
    "created_at": "2016-01-12T22:07:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19869#issuecomment-286708",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jmantysalo/setpartitions](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/setpartitions)** to **[90192f7](https://github.com/sagemath/sagetrac-mirror/commit/90192f771d3937976ff352406a25818333399bab)**



---

archive/issue_events_263054.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-01-12T22:07:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19869",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19869#event-263054"
}
```



---

archive/issue_events_263055.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "7f44fd14046f927278eca13f98ab8f3eacf5528f",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-01-12T22:07:04Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19869",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19869#event-263055"
}
```
