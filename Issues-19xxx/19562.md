# Issue 19562: Fix bad caching of FiniteEnumeratedSet

archive/issues_019325.json:
```json
{
    "assignees": [],
    "body": "Currently\n\n```\nsage: F1 = FiniteEnumeratedSet((1,2,3))\nsage: F2 = FiniteEnumeratedSet((1., 2., 3.))\nsage: F2.an_element().parent()\nInteger Ring\n```\nWe fix this problem by computing a universe associated to each `FiniteEnumeratedSet`.\n\nCC:  @tscrim @nthiery @hivert\n\nBranch/Commit: **[u/vdelecroix/19562](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/19562) @ [3410bed](https://github.com/sagemath/sagetrac-mirror/commit/3410bedbe8935008a047c766f3b2905424b4751e)**\n\nWork Issues: **failing doctests**\n\nReviewer: **Julian R\u00fcth**\n\nAuthor: **Vincent Delecroix**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19562_\n\n",
    "created_at": "2015-11-09T20:31:12Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/needs%20work"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.10",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix bad caching of FiniteEnumeratedSet",
    "type": "issue",
    "updated_at": "2023-01-13T13:28:57Z",
    "url": "https://github.com/sagemath/sage/issues/19562",
    "user": "https://github.com/videlec"
}
```
Currently

```
sage: F1 = FiniteEnumeratedSet((1,2,3))
sage: F2 = FiniteEnumeratedSet((1., 2., 3.))
sage: F2.an_element().parent()
Integer Ring
```
We fix this problem by computing a universe associated to each `FiniteEnumeratedSet`.

CC:  @tscrim @nthiery @hivert

Branch/Commit: **[u/vdelecroix/19562](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/19562) @ [3410bed](https://github.com/sagemath/sagetrac-mirror/commit/3410bedbe8935008a047c766f3b2905424b4751e)**

Work Issues: **failing doctests**

Reviewer: **Julian Rüth**

Author: **Vincent Delecroix**

_Issue created by migration from https://trac.sagemath.org/ticket/19562_





---

archive/issue_events_249968.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-11-09T20:31:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19562",
    "milestone_number": null,
    "milestone_title": "sage-6.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19562#event-249968"
}
```



---

archive/issue_events_249969.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-11-09T20:31:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19562",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19562#event-249969"
}
```



---

archive/issue_events_249970.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-11-09T20:31:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19562",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19562#event-249970"
}
```



---

archive/issue_events_249971.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-11-09T20:31:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19562",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19562#event-249971"
}
```



---

archive/issue_comments_281225.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3410bedbe8935008a047c766f3b2905424b4751e\">3410bed</a></td><td><code>Trac 19562: fix caching of FiniteEnumeratedSet</code></td></tr></table>\n",
    "created_at": "2015-11-09T20:32:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19562",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19562#issuecomment-281225",
    "user": "https://github.com/videlec"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3410bedbe8935008a047c766f3b2905424b4751e">3410bed</a></td><td><code>Trac 19562: fix caching of FiniteEnumeratedSet</code></td></tr></table>




---

archive/issue_comments_281226.json:
```json
{
    "body": "Commit: **[3410bed](https://github.com/sagemath/sagetrac-mirror/commit/3410bedbe8935008a047c766f3b2905424b4751e)**",
    "created_at": "2015-11-09T20:32:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19562",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19562#issuecomment-281226",
    "user": "https://github.com/videlec"
}
```

Commit: **[3410bed](https://github.com/sagemath/sagetrac-mirror/commit/3410bedbe8935008a047c766f3b2905424b4751e)**



---

archive/issue_comments_281227.json:
```json
{
    "body": "Branch: **[u/vdelecroix/19562](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/19562)**",
    "created_at": "2015-11-09T20:32:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19562",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19562#issuecomment-281227",
    "user": "https://github.com/videlec"
}
```

Branch: **[u/vdelecroix/19562](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/19562)**



---

archive/issue_events_249972.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-11-09T20:32:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19562",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19562#event-249972"
}
```



---

archive/issue_comments_281228.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nI think raising an `ArgumentError` would be more appropriate here than a simple assert. After all, one of the arguments is invalid.",
    "created_at": "2015-11-19T19:09:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19562",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19562#issuecomment-281228",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:2'>Comment 2:</a>
I think raising an `ArgumentError` would be more appropriate here than a simple assert. After all, one of the arguments is invalid.



---

archive/issue_comments_281229.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nOther than that, feel free to set it to positive review yourself.",
    "created_at": "2015-11-19T19:12:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19562",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19562#issuecomment-281229",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:3'>Comment 3:</a>
Other than that, feel free to set it to positive review yourself.



---

archive/issue_events_249973.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2015-11-19T19:12:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19562",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19562#event-249973"
}
```



---

archive/issue_events_249974.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2015-11-19T19:12:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19562",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19562#event-249974"
}
```



---

archive/issue_comments_281230.json:
```json
{
    "body": "Reviewer: **Julian R\u00fcth**",
    "created_at": "2015-11-19T19:12:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19562",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19562#issuecomment-281230",
    "user": "https://github.com/saraedum"
}
```

Reviewer: **Julian Rüth**



---

archive/issue_comments_281231.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nAnd after figuring out why all of those doctests are now failing.",
    "created_at": "2015-11-19T19:18:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19562",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19562#issuecomment-281231",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:6'>Comment 6:</a>
And after figuring out why all of those doctests are now failing.



---

archive/issue_comments_281232.json:
```json
{
    "body": "Work Issues: **failing doctests**",
    "created_at": "2018-12-14T01:27:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19562",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19562#issuecomment-281232",
    "user": "https://github.com/saraedum"
}
```

Work Issues: **failing doctests**
