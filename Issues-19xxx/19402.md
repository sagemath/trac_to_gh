# Issue 19402: RIF: use RNDD/RNDU for mignitude()/magnitude()

archive/issues_019165.json:
```json
{
    "assignees": [],
    "body": "By analogy with `lower()` and `upper()`, a directed rounding would be more logical for these methods.\n\nBranch/Commit: **[5e34b5c](https://github.com/sagemath/sagetrac-mirror/commit/5e34b5cd33d30c38482363976ea9dd0ea3ca3988)**\n\nReviewer: **Marc Mezzarobba**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19402_\n\n",
    "closed_at": "2015-10-16T08:22:11Z",
    "created_at": "2015-10-13T13:11:25Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.10",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "RIF: use RNDD/RNDU for mignitude()/magnitude()",
    "type": "issue",
    "updated_at": "2015-10-16T08:22:11Z",
    "url": "https://github.com/sagemath/sage/issues/19402",
    "user": "https://github.com/jdemeyer"
}
```
By analogy with `lower()` and `upper()`, a directed rounding would be more logical for these methods.

Branch/Commit: **[5e34b5c](https://github.com/sagemath/sagetrac-mirror/commit/5e34b5cd33d30c38482363976ea9dd0ea3ca3988)**

Reviewer: **Marc Mezzarobba**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/19402_





---

archive/issue_events_257351.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-10-13T13:11:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19402",
    "milestone_number": null,
    "milestone_title": "sage-6.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19402#event-257351"
}
```



---

archive/issue_events_257352.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-10-13T13:11:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19402",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "component: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19402#event-257352"
}
```



---

archive/issue_events_257353.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-10-13T13:11:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19402",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19402#event-257353"
}
```



---

archive/issue_events_257354.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-10-13T13:11:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19402",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19402#event-257354"
}
```



---

archive/issue_comments_278309.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-\n+By analogy with `lower()` and `upper()`, a directed rounding would be more logical for these methods.\n``````\n",
    "created_at": "2015-10-13T13:14:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19402#issuecomment-278309",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-
+By analogy with `lower()` and `upper()`, a directed rounding would be more logical for these methods.
``````




---

archive/issue_comments_278310.json:
```json
{
    "body": "Branch: **[u/jdemeyer/rif__use_rndd_rndu_for_mignitude___magnitude__](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/rif__use_rndd_rndu_for_mignitude___magnitude__)**",
    "created_at": "2015-10-13T13:16:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19402#issuecomment-278310",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/rif__use_rndd_rndu_for_mignitude___magnitude__](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/rif__use_rndd_rndu_for_mignitude___magnitude__)**



---

archive/issue_comments_278311.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5e34b5cd33d30c38482363976ea9dd0ea3ca3988\">5e34b5c</a></td><td><code>Use more logical rounding modes for mignitude/magnitude</code></td></tr></table>\n",
    "created_at": "2015-10-13T13:17:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19402#issuecomment-278311",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:3" align="right">Comment 3</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5e34b5cd33d30c38482363976ea9dd0ea3ca3988">5e34b5c</a></td><td><code>Use more logical rounding modes for mignitude/magnitude</code></td></tr></table>




---

archive/issue_events_257355.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-10-13T13:17:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19402",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19402#event-257355"
}
```



---

archive/issue_comments_278312.json:
```json
{
    "body": "Commit: **[5e34b5c](https://github.com/sagemath/sagetrac-mirror/commit/5e34b5cd33d30c38482363976ea9dd0ea3ca3988)**",
    "created_at": "2015-10-13T13:17:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19402#issuecomment-278312",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[5e34b5c](https://github.com/sagemath/sagetrac-mirror/commit/5e34b5cd33d30c38482363976ea9dd0ea3ca3988)**



---

archive/issue_comments_278313.json:
```json
{
    "body": "Reviewer: **Marc Mezzarobba**",
    "created_at": "2015-10-15T12:24:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19402#issuecomment-278313",
    "user": "https://github.com/mezzarobba"
}
```

Reviewer: **Marc Mezzarobba**



---

archive/issue_events_257356.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2015-10-15T12:24:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19402",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19402#event-257356"
}
```



---

archive/issue_events_257357.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2015-10-15T12:24:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19402",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19402#event-257357"
}
```



---

archive/issue_events_257358.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-10-16T08:22:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19402",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19402#event-257358"
}
```



---

archive/issue_events_257359.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "fd89748db8172a250e76763889f6c2c060e8fdd7",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-10-16T08:22:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19402",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19402#event-257359"
}
```



---

archive/issue_comments_278314.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/rif__use_rndd_rndu_for_mignitude___magnitude__](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/rif__use_rndd_rndu_for_mignitude___magnitude__)** to **[5e34b5c](https://github.com/sagemath/sagetrac-mirror/commit/5e34b5cd33d30c38482363976ea9dd0ea3ca3988)**",
    "created_at": "2015-10-16T08:22:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19402#issuecomment-278314",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/rif__use_rndd_rndu_for_mignitude___magnitude__](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/rif__use_rndd_rndu_for_mignitude___magnitude__)** to **[5e34b5c](https://github.com/sagemath/sagetrac-mirror/commit/5e34b5cd33d30c38482363976ea9dd0ea3ca3988)**
