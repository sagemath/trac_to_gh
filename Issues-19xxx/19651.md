# Issue 19651: DiGraph constructor for dict, odd bug

archive/issues_019414.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: DiGraph({0:[3], 1:[2], 2:[3], 3:[4]}).edges()\n[(0, 3, None), (1, 2, None), (2, 3, None), (3, 4, None)]\nsage: DiGraph({0:[3], 1:[2], 2:[3], 3:[4], 0:[7]}).edges()\n[(0, 7, None), (1, 2, None), (2, 3, None), (3, 4, None)]\n```\n\nI.e. adding `0->7` makes `0->3` to disappear.\n\n\nReviewer: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19651_\n\n",
    "closed_at": "2015-11-30T09:21:07Z",
    "created_at": "2015-11-30T05:37:55Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "DiGraph constructor for dict, odd bug",
    "type": "issue",
    "updated_at": "2015-11-30T09:21:07Z",
    "url": "https://github.com/sagemath/sage/issues/19651",
    "user": "https://github.com/jm58660"
}
```

```
sage: DiGraph({0:[3], 1:[2], 2:[3], 3:[4]}).edges()
[(0, 3, None), (1, 2, None), (2, 3, None), (3, 4, None)]
sage: DiGraph({0:[3], 1:[2], 2:[3], 3:[4], 0:[7]}).edges()
[(0, 7, None), (1, 2, None), (2, 3, None), (3, 4, None)]
```

I.e. adding `0->7` makes `0->3` to disappear.


Reviewer: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/19651_





---

archive/issue_events_260403.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2015-11-30T05:37:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19651",
    "milestone_number": null,
    "milestone_title": "sage-6.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19651#event-260403"
}
```



---

archive/issue_events_260404.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2015-11-30T05:37:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19651",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19651#event-260404"
}
```



---

archive/issue_events_260405.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2015-11-30T05:37:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19651",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19651#event-260405"
}
```



---

archive/issue_events_260406.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2015-11-30T05:37:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19651",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19651#event-260406"
}
```



---

archive/issue_events_260407.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-11-30T08:27:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19651",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19651#event-260407"
}
```



---

archive/issue_comments_283078.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nA `dict` is a mapping, so there can be only one value for each key.",
    "created_at": "2015-11-30T08:27:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19651#issuecomment-283078",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:1'>Comment 1:</a>
A `dict` is a mapping, so there can be only one value for each key.



---

archive/issue_events_260408.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-11-30T08:27:41Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/19651",
    "milestone_number": null,
    "milestone_title": "sage-6.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19651#event-260408"
}
```



---

archive/issue_events_260409.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-11-30T08:27:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19651",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19651#event-260409"
}
```



---

archive/issue_events_260410.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-11-30T08:27:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19651",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19651#event-260410"
}
```



---

archive/issue_comments_283079.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2015-11-30T08:27:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19651#issuecomment-283079",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_260411.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-11-30T08:27:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19651",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19651#event-260411"
}
```



---

archive/issue_events_260412.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-11-30T08:27:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19651",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19651#event-260412"
}
```



---

archive/issue_events_260413.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-11-30T09:21:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19651",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19651#event-260413"
}
```



---

archive/issue_events_260414.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-11-30T09:21:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19651",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19651#event-260414"
}
```



---

archive/issue_events_260415.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-11-30T09:21:07Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19651",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19651#event-260415"
}
```
