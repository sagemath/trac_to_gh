# Issue 19807: Tides malfunctions in version 7.0beta

archive/issues_019570.json:
```json
{
    "assignees": [],
    "body": "As was pointed in #19781, tides_mpfr does not work properly in the last beta. \n\nI could trace the problem to the loading of libgmp. I attach a branch that seems to fix it, but it could be that now it is using system's libgmp/libmpfr .\n\nCan somebody test it in a platform without libgmp installed system wide?\n\nCC:  @vbraun @jdemeyer\n\nKeywords: **tides**\n\nBranch: **[0396add](https://github.com/sagemath/sagetrac-mirror/commit/0396add80d6e4b1b90d6ef37a468fb9640e3dc80)**\n\nReviewer: **Volker Braun**\n\nAuthor: **Miguel Marco**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19807_\n\n",
    "closed_at": "2015-12-30T19:27:53Z",
    "created_at": "2015-12-30T12:29:16Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Tides malfunctions in version 7.0beta",
    "type": "issue",
    "updated_at": "2020-04-16T17:47:06Z",
    "url": "https://github.com/sagemath/sage/issues/19807",
    "user": "https://github.com/miguelmarco"
}
```
As was pointed in #19781, tides_mpfr does not work properly in the last beta. 

I could trace the problem to the loading of libgmp. I attach a branch that seems to fix it, but it could be that now it is using system's libgmp/libmpfr .

Can somebody test it in a platform without libgmp installed system wide?

CC:  @vbraun @jdemeyer

Keywords: **tides**

Branch: **[0396add](https://github.com/sagemath/sagetrac-mirror/commit/0396add80d6e4b1b90d6ef37a468fb9640e3dc80)**

Reviewer: **Volker Braun**

Author: **Miguel Marco**

_Issue created by migration from https://trac.sagemath.org/ticket/19807_





---

archive/issue_events_262255.json:
```json
{
    "actor": "https://github.com/miguelmarco",
    "created_at": "2015-12-30T12:29:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "milestone_number": null,
    "milestone_title": "sage-7.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19807#event-262255"
}
```



---

archive/issue_events_262256.json:
```json
{
    "actor": "https://github.com/miguelmarco",
    "created_at": "2015-12-30T12:29:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19807#event-262256"
}
```



---

archive/issue_comments_285585.json:
```json
{
    "body": "Branch: **[u/mmarco/tides_malfunctions_in_version_7_0beta](https://github.com/sagemath/sagetrac-mirror/tree/u/mmarco/tides_malfunctions_in_version_7_0beta)**",
    "created_at": "2015-12-30T12:30:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19807#issuecomment-285585",
    "user": "https://github.com/miguelmarco"
}
```

Branch: **[u/mmarco/tides_malfunctions_in_version_7_0beta](https://github.com/sagemath/sagetrac-mirror/tree/u/mmarco/tides_malfunctions_in_version_7_0beta)**



---

archive/issue_comments_285586.json:
```json
{
    "body": "Changed keywords from **** to **tides**",
    "created_at": "2015-12-30T12:32:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19807#issuecomment-285586",
    "user": "https://github.com/miguelmarco"
}
```

Changed keywords from **** to **tides**



---

archive/issue_comments_285587.json:
```json
{
    "body": "Author: **Miguel Marco**",
    "created_at": "2015-12-30T12:32:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19807#issuecomment-285587",
    "user": "https://github.com/miguelmarco"
}
```

Author: **Miguel Marco**



---

archive/issue_comments_285588.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,5 @@\n+As was pointed in #19781, tides_mpfr does not work properly in the last beta. \n \n+I could trace the problem to the loading of libgmp. I attach a branch that seems to fix it, but it could be that now it is using system's libgmp/libmpfr .\n+\n+Can somebody test it in a platform without libgmp installed system wide?\n``````\n",
    "created_at": "2015-12-30T12:32:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19807#issuecomment-285588",
    "user": "https://github.com/miguelmarco"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,5 @@
+As was pointed in #19781, tides_mpfr does not work properly in the last beta. 
 
+I could trace the problem to the loading of libgmp. I attach a branch that seems to fix it, but it could be that now it is using system's libgmp/libmpfr .
+
+Can somebody test it in a platform without libgmp installed system wide?
``````




---

archive/issue_events_262257.json:
```json
{
    "actor": "https://github.com/miguelmarco",
    "created_at": "2015-12-30T12:32:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19807#event-262257"
}
```



---

archive/issue_events_262258.json:
```json
{
    "actor": "https://github.com/miguelmarco",
    "created_at": "2015-12-30T12:32:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19807#event-262258"
}
```



---

archive/issue_events_262259.json:
```json
{
    "actor": "https://github.com/miguelmarco",
    "created_at": "2015-12-30T12:32:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19807#event-262259"
}
```



---

archive/issue_comments_285589.json:
```json
{
    "body": "Commit: **[12c3e8a](https://github.com/sagemath/sagetrac-mirror/commit/12c3e8a2bd813d05cc051c508f4632d1d89956ad)**",
    "created_at": "2015-12-30T12:32:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19807#issuecomment-285589",
    "user": "https://github.com/miguelmarco"
}
```

Commit: **[12c3e8a](https://github.com/sagemath/sagetrac-mirror/commit/12c3e8a2bd813d05cc051c508f4632d1d89956ad)**



---

archive/issue_events_262260.json:
```json
{
    "actor": "https://github.com/miguelmarco",
    "created_at": "2015-12-30T12:32:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19807#event-262260"
}
```



---

archive/issue_comments_285590.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/12c3e8a2bd813d05cc051c508f4632d1d89956ad\">12c3e8a</a></td><td><code>Don't use -L and -I flags</code></td></tr></table>\n",
    "created_at": "2015-12-30T12:32:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19807#issuecomment-285590",
    "user": "https://github.com/miguelmarco"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/12c3e8a2bd813d05cc051c508f4632d1d89956ad">12c3e8a</a></td><td><code>Don't use -L and -I flags</code></td></tr></table>




---

archive/issue_comments_285591.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nYou most likely just need to add `$LDFLAGS` to the command line.",
    "created_at": "2015-12-30T12:40:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19807#issuecomment-285591",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:3" align="right">Comment 3</div>

You most likely just need to add `$LDFLAGS` to the command line.



---

archive/issue_events_262261.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-30T12:40:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19807#event-262261"
}
```



---

archive/issue_events_262262.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-30T12:40:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19807#event-262262"
}
```



---

archive/issue_comments_285592.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nYou mean during the compilation of the library, the compilation of each instance or at running it?",
    "created_at": "2015-12-30T12:51:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19807#issuecomment-285592",
    "user": "https://github.com/miguelmarco"
}
```

<div id="comment:5" align="right">Comment 5</div>

You mean during the compilation of the library, the compilation of each instance or at running it?



---

archive/issue_comments_285593.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nI mean in the command line that you changed in your patch.",
    "created_at": "2015-12-30T13:07:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19807#issuecomment-285593",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:6" align="right">Comment 6</div>

I mean in the command line that you changed in your patch.



---

archive/issue_comments_285594.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nYou can just test it using `ldd /path/to/binary`\n\nYou should **always** add `$LDFLAGS` when creating a binary. That includes shared libraries.\n\nStatic libraries (libtides.a) aren't executable so you don't need `$LDFLAGS`.",
    "created_at": "2015-12-30T13:07:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19807#issuecomment-285594",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:7" align="right">Comment 7</div>

You can just test it using `ldd /path/to/binary`

You should **always** add `$LDFLAGS` when creating a binary. That includes shared libraries.

Static libraries (libtides.a) aren't executable so you don't need `$LDFLAGS`.



---

archive/issue_comments_285595.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/698118474823026b5ba68e0b7416fef315ebc4c9\">6981184</a></td><td><code>Added $LDFLAGS</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0396add80d6e4b1b90d6ef37a468fb9640e3dc80\">0396add</a></td><td><code>Put -L directive before -l</code></td></tr></table>\n",
    "created_at": "2015-12-30T13:54:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19807#issuecomment-285595",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:8" align="right">Comment 8</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/698118474823026b5ba68e0b7416fef315ebc4c9">6981184</a></td><td><code>Added $LDFLAGS</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0396add80d6e4b1b90d6ef37a468fb9640e3dc80">0396add</a></td><td><code>Put -L directive before -l</code></td></tr></table>




---

archive/issue_comments_285596.json:
```json
{
    "body": "Changed commit from **[12c3e8a](https://github.com/sagemath/sagetrac-mirror/commit/12c3e8a2bd813d05cc051c508f4632d1d89956ad)** to **[0396add](https://github.com/sagemath/sagetrac-mirror/commit/0396add80d6e4b1b90d6ef37a468fb9640e3dc80)**",
    "created_at": "2015-12-30T13:54:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19807#issuecomment-285596",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[12c3e8a](https://github.com/sagemath/sagetrac-mirror/commit/12c3e8a2bd813d05cc051c508f4632d1d89956ad)** to **[0396add](https://github.com/sagemath/sagetrac-mirror/commit/0396add80d6e4b1b90d6ef37a468fb9640e3dc80)**



---

archive/issue_comments_285597.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nI think it is fixed now.",
    "created_at": "2015-12-30T13:55:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19807#issuecomment-285597",
    "user": "https://github.com/miguelmarco"
}
```

<div id="comment:9" align="right">Comment 9</div>

I think it is fixed now.



---

archive/issue_events_262263.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-12-30T14:24:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19807#event-262263"
}
```



---

archive/issue_events_262264.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-12-30T14:24:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19807#event-262264"
}
```



---

archive/issue_comments_285598.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2015-12-30T14:24:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19807#issuecomment-285598",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_events_262265.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-12-30T19:27:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19807#event-262265"
}
```



---

archive/issue_events_262266.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "18b5c49ee3f3b8eeb97b6413faeb014c4ef82055",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-12-30T19:27:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19807#event-262266"
}
```



---

archive/issue_comments_285599.json:
```json
{
    "body": "Changed branch from **[u/mmarco/tides_malfunctions_in_version_7_0beta](https://github.com/sagemath/sagetrac-mirror/tree/u/mmarco/tides_malfunctions_in_version_7_0beta)** to **[0396add](https://github.com/sagemath/sagetrac-mirror/commit/0396add80d6e4b1b90d6ef37a468fb9640e3dc80)**",
    "created_at": "2015-12-30T19:27:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19807#issuecomment-285599",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mmarco/tides_malfunctions_in_version_7_0beta](https://github.com/sagemath/sagetrac-mirror/tree/u/mmarco/tides_malfunctions_in_version_7_0beta)** to **[0396add](https://github.com/sagemath/sagetrac-mirror/commit/0396add80d6e4b1b90d6ef37a468fb9640e3dc80)**



---

archive/issue_events_262267.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-04-16T17:47:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
    "label_color": "000080",
    "label_name": "component: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19807#event-262267"
}
```



---

archive/issue_comments_285600.json:
```json
{
    "body": "Changed commit from **[0396add](https://github.com/sagemath/sagetrac-mirror/commit/0396add80d6e4b1b90d6ef37a468fb9640e3dc80)** to none",
    "created_at": "2020-04-16T17:47:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19807",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19807#issuecomment-285600",
    "user": "https://github.com/slel"
}
```

Changed commit from **[0396add](https://github.com/sagemath/sagetrac-mirror/commit/0396add80d6e4b1b90d6ef37a468fb9640e3dc80)** to none
