# Issue 19967: fix or remove cliquer

archive/issues_019730.json:
```json
{
    "body": "The installation of cliquer can fail randomly with:\n\n```\nerror: /Applications/Xcode.app/Contents/Developer/Toolchains/XcodeDefault.xctoolchain/usr/bin/install_name_tool: changing install names or rpaths can't be redone for: /Users/vbraun/Code/binary-pkg/source/SageMath/jc4b6yulaujayb9sr94ia88eourzeqip0oidmas391yaj24ng0bmdu/local/lib/libcliquer.dylib (for architecture x86_64) because larger updated load commands do not fit (the program must be relinked, and you may need to use -headerpad or -headerpad_max_install_names)\n```\nReally the current cliquer has no business being a standard package:\n* No build system to speak of\n\nget autotoolized tarball at http://users.ox.ac.uk/~coml0531/sage/cliquer-1.21.tar.gz\n\n* Errors are not caught\n  (probably another ticket?)\n\n* A libcliquer.so is installed on Darwin (in addition to libcliquer.dylib)\n  (not with the new autotoolized version)\n\n\nCC:  @dimpase @nathanncohen @williamstein\n\nBranch/Commit: 8d7f039d442e915754940c45a710dd9f4900c062\n\nUpstream: Reported upstream. Developers deny it's a bug.\n\nReviewer: Volker Braun\n\nAuthor: Dima Pasechnik\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/19967\n\n",
    "closed_at": "2016-01-28T17:14:20Z",
    "created_at": "2016-01-26T20:39:33Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-7.1",
    "title": "fix or remove cliquer",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/19967",
    "user": "https://github.com/vbraun"
}
```
The installation of cliquer can fail randomly with:

```
error: /Applications/Xcode.app/Contents/Developer/Toolchains/XcodeDefault.xctoolchain/usr/bin/install_name_tool: changing install names or rpaths can't be redone for: /Users/vbraun/Code/binary-pkg/source/SageMath/jc4b6yulaujayb9sr94ia88eourzeqip0oidmas391yaj24ng0bmdu/local/lib/libcliquer.dylib (for architecture x86_64) because larger updated load commands do not fit (the program must be relinked, and you may need to use -headerpad or -headerpad_max_install_names)
```
Really the current cliquer has no business being a standard package:
* No build system to speak of

get autotoolized tarball at http://users.ox.ac.uk/~coml0531/sage/cliquer-1.21.tar.gz

* Errors are not caught
  (probably another ticket?)

* A libcliquer.so is installed on Darwin (in addition to libcliquer.dylib)
  (not with the new autotoolized version)


CC:  @dimpase @nathanncohen @williamstein

Branch/Commit: 8d7f039d442e915754940c45a710dd9f4900c062

Upstream: Reported upstream. Developers deny it's a bug.

Reviewer: Volker Braun

Author: Dima Pasechnik

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/19967





---

archive/issue_comments_360034.json:
```json
{
    "body": "<a id='comment:2'></a>\nI'll let it burn in autohell, to begin with. Wish me luck. Hopefully this at least makes it installable everywhere we need...",
    "created_at": "2016-01-26T20:49:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360034",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:2'></a>
I'll let it burn in autohell, to begin with. Wish me luck. Hopefully this at least makes it installable everywhere we need...



---

archive/issue_comments_360035.json:
```json
{
    "body": "<a id='comment:3'></a>\nReplying to [dimpase](#comment%3A2):\n> I'll let it burn in autohell, to begin with. Wish me luck. Hopefully this at least makes it installable everywhere we need...\n\n\nluck.",
    "created_at": "2016-01-26T21:59:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360035",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:3'></a>
Replying to [dimpase](#comment%3A2):
> I'll let it burn in autohell, to begin with. Wish me luck. Hopefully this at least makes it installable everywhere we need...


luck.



---

archive/issue_comments_360036.json:
```json
{
    "body": "<a id='comment:4'></a>\nhttps://github.com/dimpase/autocliquer has the dirty stuff done; one can build shared libs on Linux and OSX and cliquer's own tests pass. I don't know how to properly hook the tests into the autohell (so that `make check` does the right thing), please advice (or submit a pull request :-))\n\n`make check` should run `test/testcases`, reading data from the same directory.\n\n what the the right Makefile.am magic for them?\n\nSimilarly, there are `include/cliquer/test*` files that should not really be installed, they only needed for `make check`\n\nI can't also figure out how to set up the proper version number on the `.so` file. It's again something for `configure.ac` or one of `Makefile.am`.",
    "created_at": "2016-01-27T00:34:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360036",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:4'></a>
https://github.com/dimpase/autocliquer has the dirty stuff done; one can build shared libs on Linux and OSX and cliquer's own tests pass. I don't know how to properly hook the tests into the autohell (so that `make check` does the right thing), please advice (or submit a pull request :-))

`make check` should run `test/testcases`, reading data from the same directory.

 what the the right Makefile.am magic for them?

Similarly, there are `include/cliquer/test*` files that should not really be installed, they only needed for `make check`

I can't also figure out how to set up the proper version number on the `.so` file. It's again something for `configure.ac` or one of `Makefile.am`.



---

archive/issue_comments_360037.json:
```json
{
    "body": "<a id='comment:5'></a>\nFor proper version number it is in `configure.ac` and that's libtool stuff. Look at eclib for a nice example https://github.com/JohnCremona/eclib/blob/master/configure.ac I would recommend 1.0.0 to start with.\nfor the rest I imagine you'll take PR if we send them.",
    "created_at": "2016-01-27T00:40:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360037",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:5'></a>
For proper version number it is in `configure.ac` and that's libtool stuff. Look at eclib for a nice example https://github.com/JohnCremona/eclib/blob/master/configure.ac I would recommend 1.0.0 to start with.
for the rest I imagine you'll take PR if we send them.



---

archive/issue_comments_360038.json:
```json
{
    "body": "<a id='comment:6'></a>\nPR sent...",
    "created_at": "2016-01-27T03:32:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360038",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:6'></a>
PR sent...



---

archive/issue_comments_360039.json:
```json
{
    "body": "<a id='comment:7'></a>\nReplying to [vbraun](#comment%3A6):\n> PR sent...\n\nWhich PR to who?",
    "created_at": "2016-01-27T09:40:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360039",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:7'></a>
Replying to [vbraun](#comment%3A6):
> PR sent...

Which PR to who?



---

archive/issue_comments_360040.json:
```json
{
    "body": "<a id='comment:8'></a>\nReplying to [jdemeyer](#comment%3A7):\n> Replying to [vbraun](#comment%3A6):\n> > PR sent...\n\n> Which PR to who?\na PR to me on the github repo https://github.com/dimpase/autocliquer\n(thanks Volker!)\n\nI have already used it and make a proper testsuite; tests pass. Now to the version thing, and it will be ready for making its way into Sage.",
    "created_at": "2016-01-27T10:00:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360040",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:8'></a>
Replying to [jdemeyer](#comment%3A7):
> Replying to [vbraun](#comment%3A6):
> > PR sent...

> Which PR to who?
a PR to me on the github repo https://github.com/dimpase/autocliquer
(thanks Volker!)

I have already used it and make a proper testsuite; tests pass. Now to the version thing, and it will be ready for making its way into Sage.



---

archive/issue_comments_360041.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2016-01-27T13:26:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360041",
    "user": "https://github.com/dimpase"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_360042.json:
```json
{
    "body": "<a id='comment:9'></a>\nNew commits:\n|                                                                                                                                          |                                           |\n|------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------|\n|[b736207](http://git.sagemath.org/sage.git/commit/?id=b7362071edbbad389529f004c59b51c96dc62e46)|`trac #19777: Three new SRGs and db update`|\n|[5aca3d8](http://git.sagemath.org/sage.git/commit/?id=5aca3d83f051123c1e3c038af798f676c33940e7)|`trac #19777: Convert the other 2-intersection set`|\n|[6696d1e](http://git.sagemath.org/sage.git/commit/?id=6696d1e1ebea1288780b6703654d5d16cc84dcc2)|`Merge branch 'develop' of git://trac.sagemath.org/sage into develop`|\n|[c5374d8](http://git.sagemath.org/sage.git/commit/?id=c5374d8fe12797b923fc10ca3d89fc003a34c09c)|`Merge branch 'develop' of git://trac.sagemath.org/sage into develop`|\n|[b88f7e8](http://git.sagemath.org/sage.git/commit/?id=b88f7e84dd750759248c16b5e08527a939ad931d)|`Merge branch 'develop' of git://trac.sagemath.org/sage into develop`|\n|[136628a](http://git.sagemath.org/sage.git/commit/?id=136628a89866a65d526bf4325392a4b9daca021a)|`Merge branch 'develop' of git://trac.sagemath.org/sage into develop`|\n|[05c5881](http://git.sagemath.org/sage.git/commit/?id=05c5881e2a7c446713128f935816e01a194a8886)|`changes for autotoolozed version`|",
    "created_at": "2016-01-27T13:26:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360042",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:9'></a>
New commits:
|                                                                                                                                          |                                           |
|------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------|
|[b736207](http://git.sagemath.org/sage.git/commit/?id=b7362071edbbad389529f004c59b51c96dc62e46)|`trac #19777: Three new SRGs and db update`|
|[5aca3d8](http://git.sagemath.org/sage.git/commit/?id=5aca3d83f051123c1e3c038af798f676c33940e7)|`trac #19777: Convert the other 2-intersection set`|
|[6696d1e](http://git.sagemath.org/sage.git/commit/?id=6696d1e1ebea1288780b6703654d5d16cc84dcc2)|`Merge branch 'develop' of git://trac.sagemath.org/sage into develop`|
|[c5374d8](http://git.sagemath.org/sage.git/commit/?id=c5374d8fe12797b923fc10ca3d89fc003a34c09c)|`Merge branch 'develop' of git://trac.sagemath.org/sage into develop`|
|[b88f7e8](http://git.sagemath.org/sage.git/commit/?id=b88f7e84dd750759248c16b5e08527a939ad931d)|`Merge branch 'develop' of git://trac.sagemath.org/sage into develop`|
|[136628a](http://git.sagemath.org/sage.git/commit/?id=136628a89866a65d526bf4325392a4b9daca021a)|`Merge branch 'develop' of git://trac.sagemath.org/sage into develop`|
|[05c5881](http://git.sagemath.org/sage.git/commit/?id=05c5881e2a7c446713128f935816e01a194a8886)|`changes for autotoolozed version`|



---

archive/issue_comments_360043.json:
```json
{
    "body": "Changing branch from \"\" to \"public/autocliquer\"",
    "created_at": "2016-01-27T13:26:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360043",
    "user": "https://github.com/dimpase"
}
```

Changing branch from "" to "public/autocliquer"



---

archive/issue_comments_360044.json:
```json
{
    "body": "Changing commit from \"\" to \"05c5881e2a7c446713128f935816e01a194a8886\"",
    "created_at": "2016-01-27T13:26:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360044",
    "user": "https://github.com/dimpase"
}
```

Changing commit from "" to "05c5881e2a7c446713128f935816e01a194a8886"



---

archive/issue_comments_360045.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -5,6 +5,12 @@\n ```\n Really the current cliquer has no business being a standard package:\n * No build system to speak of\n+\n+get autotoolized tarball at http://users.ox.ac.uk/~coml0531/sage/cliquer-1.21.tar.gz\n+\n * Errors are not caught\n+  (probably another ticket?)\n+\n * A libcliquer.so is installed on Darwin (in addition to libcliquer.dylib)\n+  (not with the new autotoolized version)\n \n``````\n",
    "created_at": "2016-01-27T13:26:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360045",
    "user": "https://github.com/dimpase"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -5,6 +5,12 @@
 ```
 Really the current cliquer has no business being a standard package:
 * No build system to speak of
+
+get autotoolized tarball at http://users.ox.ac.uk/~coml0531/sage/cliquer-1.21.tar.gz
+
 * Errors are not caught
+  (probably another ticket?)
+
 * A libcliquer.so is installed on Darwin (in addition to libcliquer.dylib)
+  (not with the new autotoolized version)
 
``````




---

archive/issue_comments_360046.json:
```json
{
    "body": "<a id='comment:10'></a>\noh, sorry, the same rotten develop branch of me --- will be fixed soon",
    "created_at": "2016-01-27T13:27:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360046",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:10'></a>
oh, sorry, the same rotten develop branch of me --- will be fixed soon



---

archive/issue_comments_360047.json:
```json
{
    "body": "<a id='comment:11'></a>\nyou may meanwhile just review the last commit (\u200b05c5881)\n\nsorry for mess",
    "created_at": "2016-01-27T13:28:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360047",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:11'></a>
you may meanwhile just review the last commit (​05c5881)

sorry for mess



---

archive/issue_comments_360048.json:
```json
{
    "body": "<a id='comment:12'></a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n|                                                                                                                                          |                                  |\n|------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------|\n|[3c17486](http://git.sagemath.org/sage.git/commit/?id=3c174863f4820970b85d2080199a80b1d87863b6)|`changes for autotoolozed version`|",
    "created_at": "2016-01-27T13:42:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360048",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:12'></a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
|                                                                                                                                          |                                  |
|------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------|
|[3c17486](http://git.sagemath.org/sage.git/commit/?id=3c174863f4820970b85d2080199a80b1d87863b6)|`changes for autotoolozed version`|



---

archive/issue_comments_360049.json:
```json
{
    "body": "Changing commit from \"05c5881e2a7c446713128f935816e01a194a8886\" to \"3c174863f4820970b85d2080199a80b1d87863b6\"",
    "created_at": "2016-01-27T13:42:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360049",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "05c5881e2a7c446713128f935816e01a194a8886" to "3c174863f4820970b85d2080199a80b1d87863b6"



---

archive/issue_comments_360050.json:
```json
{
    "body": "Changing status from needs_review to needs_info.",
    "created_at": "2016-01-27T14:13:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360050",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to needs_info.



---

archive/issue_comments_360051.json:
```json
{
    "body": "<a id='comment:13'></a>\nWhy fork cliquer? Did you even try to report this upstream?",
    "created_at": "2016-01-27T14:13:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360051",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:13'></a>
Why fork cliquer? Did you even try to report this upstream?



---

archive/issue_comments_360052.json:
```json
{
    "body": "<a id='comment:14'></a>\nReplying to [jdemeyer](#comment%3A13):\n> Why fork cliquer? Did you even try to report this upstream?\n\nI know Patric, the maintainer, for over 20 years. I sent him an email, but he is a busy guy, and I am not sure how quickly he will react. Meanwhile this is all we have.",
    "created_at": "2016-01-27T14:20:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360052",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:14'></a>
Replying to [jdemeyer](#comment%3A13):
> Why fork cliquer? Did you even try to report this upstream?

I know Patric, the maintainer, for over 20 years. I sent him an email, but he is a busy guy, and I am not sure how quickly he will react. Meanwhile this is all we have.



---

archive/issue_comments_360053.json:
```json
{
    "body": "Changing upstream from \"N/A\" to \"Reported upstream. No feedback yet.\"",
    "created_at": "2016-01-27T14:20:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360053",
    "user": "https://github.com/dimpase"
}
```

Changing upstream from "N/A" to "Reported upstream. No feedback yet."



---

archive/issue_comments_360054.json:
```json
{
    "body": "Changing status from needs_info to needs_review.",
    "created_at": "2016-01-27T14:33:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360054",
    "user": "https://github.com/dimpase"
}
```

Changing status from needs_info to needs_review.



---

archive/issue_comments_360055.json:
```json
{
    "body": "Changing status from needs_review to needs_info.",
    "created_at": "2016-01-27T14:59:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360055",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to needs_info.



---

archive/issue_comments_360056.json:
```json
{
    "body": "<a id='comment:17'></a>\nStill: why fork cliquer instead of patching the sources?",
    "created_at": "2016-01-27T14:59:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360056",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:17'></a>
Still: why fork cliquer instead of patching the sources?



---

archive/issue_comments_360057.json:
```json
{
    "body": "<a id='comment:18'></a>\nReplying to [jdemeyer](#comment%3A17):\n> Still: why fork cliquer instead of patching the sources? \n\n\nCause this is the only way I know how to fix it once and for good. And cause this is waste of anyone's time to do manual building of shared libraries instead of using appropriate tools.",
    "created_at": "2016-01-27T15:05:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360057",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:18'></a>
Replying to [jdemeyer](#comment%3A17):
> Still: why fork cliquer instead of patching the sources? 


Cause this is the only way I know how to fix it once and for good. And cause this is waste of anyone's time to do manual building of shared libraries instead of using appropriate tools.



---

archive/issue_comments_360058.json:
```json
{
    "body": "Changing status from needs_info to needs_review.",
    "created_at": "2016-01-27T15:05:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360058",
    "user": "https://github.com/dimpase"
}
```

Changing status from needs_info to needs_review.



---

archive/issue_comments_360059.json:
```json
{
    "body": "<a id='comment:20'></a>\nor do you mean to say \"why don't I create a mega-patch instead of being honest about it?\"",
    "created_at": "2016-01-27T15:08:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360059",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:20'></a>
or do you mean to say "why don't I create a mega-patch instead of being honest about it?"



---

archive/issue_comments_360060.json:
```json
{
    "body": "<a id='comment:21'></a>\nReplying to [dimpase](#comment%3A20):\n> or do you mean to say \"why don't I create a mega-patch instead of being honest about it?\"\n\nIf it is a mega-patch, you're probably doing it wrong.",
    "created_at": "2016-01-27T15:38:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360060",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:21'></a>
Replying to [dimpase](#comment%3A20):
> or do you mean to say "why don't I create a mega-patch instead of being honest about it?"

If it is a mega-patch, you're probably doing it wrong.



---

archive/issue_comments_360061.json:
```json
{
    "body": "<a id='comment:22'></a>\nReplying to [jdemeyer](#comment%3A21):\n> Replying to [dimpase](#comment%3A20):\n> > or do you mean to say \"why don't I create a mega-patch instead of being honest about it?\"\n\n> If it is a mega-patch, you're probably doing it wrong.\n\nMaybe you stop saying how wrong I am and review my changes?\nThis is getting silly. I made a sane directory layout to begin with, and created bog-standard Makefile.am and configure.ac\n\nI forwarded you the email I just got from the maintainer. Go figure. I don't think we have this option in the \"Report Upstream\" menu.",
    "created_at": "2016-01-27T15:58:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360061",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:22'></a>
Replying to [jdemeyer](#comment%3A21):
> Replying to [dimpase](#comment%3A20):
> > or do you mean to say "why don't I create a mega-patch instead of being honest about it?"

> If it is a mega-patch, you're probably doing it wrong.

Maybe you stop saying how wrong I am and review my changes?
This is getting silly. I made a sane directory layout to begin with, and created bog-standard Makefile.am and configure.ac

I forwarded you the email I just got from the maintainer. Go figure. I don't think we have this option in the "Report Upstream" menu.



---

archive/issue_comments_360062.json:
```json
{
    "body": "<a id='comment:23'></a>\nReplying to [dimpase](#comment%3A22):\n> I made a sane directory layout to begin with, and created bog-standard Makefile.am and configure.ac\n\n\nWhich is very good, I'm not complaining about that. I am only saying that *this is not a reason to fork cliquer*.",
    "created_at": "2016-01-27T16:26:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360062",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:23'></a>
Replying to [dimpase](#comment%3A22):
> I made a sane directory layout to begin with, and created bog-standard Makefile.am and configure.ac


Which is very good, I'm not complaining about that. I am only saying that *this is not a reason to fork cliquer*.



---

archive/issue_comments_360063.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Volker Braun\"",
    "created_at": "2016-01-27T19:49:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360063",
    "user": "https://github.com/vbraun"
}
```

Changing reviewer from "" to "Volker Braun"



---

archive/issue_comments_360064.json:
```json
{
    "body": "<a id='comment:24'></a>\nCan you add a `--libdir=\"$SAGE_LOCAL/lib\"` to the configure? I'm not 100% sure if its still needed now that we have a lib64->lib symlink; But some distros use a complicated multilib naming scheme and we'd still break if that gets used.",
    "created_at": "2016-01-27T19:49:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360064",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:24'></a>
Can you add a `--libdir="$SAGE_LOCAL/lib"` to the configure? I'm not 100% sure if its still needed now that we have a lib64->lib symlink; But some distros use a complicated multilib naming scheme and we'd still break if that gets used.



---

archive/issue_comments_360065.json:
```json
{
    "body": "Changing upstream from \"Reported upstream. No feedback yet.\" to \"Reported upstream. Developers deny it's a bug.\"",
    "created_at": "2016-01-27T19:49:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360065",
    "user": "https://github.com/vbraun"
}
```

Changing upstream from "Reported upstream. No feedback yet." to "Reported upstream. Developers deny it's a bug."



---

archive/issue_comments_360066.json:
```json
{
    "body": "<a id='comment:26'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                          |                            |\n|------------------------------------------------------------------------------------------------------------------------------------------|----------------------------|\n|[8d7f039](http://git.sagemath.org/sage.git/commit/?id=8d7f039d442e915754940c45a710dd9f4900c062)|`added explicit libdir path`|",
    "created_at": "2016-01-27T20:10:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360066",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:26'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                          |                            |
|------------------------------------------------------------------------------------------------------------------------------------------|----------------------------|
|[8d7f039](http://git.sagemath.org/sage.git/commit/?id=8d7f039d442e915754940c45a710dd9f4900c062)|`added explicit libdir path`|



---

archive/issue_comments_360067.json:
```json
{
    "body": "Changing commit from \"3c174863f4820970b85d2080199a80b1d87863b6\" to \"8d7f039d442e915754940c45a710dd9f4900c062\"",
    "created_at": "2016-01-27T20:10:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360067",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "3c174863f4820970b85d2080199a80b1d87863b6" to "8d7f039d442e915754940c45a710dd9f4900c062"



---

archive/issue_comments_360068.json:
```json
{
    "body": "Changing author from \"\" to \"Dima Pasechnik\"",
    "created_at": "2016-01-27T20:18:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360068",
    "user": "https://github.com/dimpase"
}
```

Changing author from "" to "Dima Pasechnik"



---

archive/issue_comments_360069.json:
```json
{
    "body": "<a id='comment:27'></a>\nOK, done",
    "created_at": "2016-01-27T20:18:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360069",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:27'></a>
OK, done



---

archive/issue_comments_360070.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2016-01-27T20:41:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360070",
    "user": "https://github.com/vbraun"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_360071.json:
```json
{
    "body": "Changing branch from \"public/autocliquer\" to \"8d7f039d442e915754940c45a710dd9f4900c062\"",
    "created_at": "2016-01-28T17:14:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360071",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "public/autocliquer" to "8d7f039d442e915754940c45a710dd9f4900c062"



---

archive/issue_events_054480.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-01-28T17:14:20Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/19967#event-54480"
}
```



---

archive/issue_comments_360072.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2016-01-28T17:14:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19967",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19967#issuecomment-360072",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
