# Issue 19170: Comparison modifications in algebras/

archive/issues_018933.json:
```json
{
    "assignees": [],
    "body": "Modify some comparisons in `sage.algebras` to use the `_cmp_` and `_richcmp_` involving the coercion framework.\n\nThis ticket was initially part of #18305.\n\nBranch/Commit: **[u/vdelecroix/19170](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/19170) @ [861cce2](https://github.com/sagemath/sagetrac-mirror/commit/861cce24b8301821e3291ae53b22dccc094d9ebc)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19170_\n\n",
    "closed_at": "2017-12-12T08:23:33Z",
    "created_at": "2015-09-09T01:01:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20python3",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Comparison modifications in algebras/",
    "type": "issue",
    "updated_at": "2017-12-12T08:23:33Z",
    "url": "https://github.com/sagemath/sage/issues/19170",
    "user": "https://github.com/videlec"
}
```
Modify some comparisons in `sage.algebras` to use the `_cmp_` and `_richcmp_` involving the coercion framework.

This ticket was initially part of #18305.

Branch/Commit: **[u/vdelecroix/19170](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/19170) @ [861cce2](https://github.com/sagemath/sagetrac-mirror/commit/861cce24b8301821e3291ae53b22dccc094d9ebc)**

Reviewer: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/19170_





---

archive/issue_events_254357.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-09-09T01:01:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "milestone_number": null,
    "milestone_title": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19170#event-254357"
}
```



---

archive/issue_events_254358.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-09-09T01:01:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20coercion",
    "label_color": "0000ff",
    "label_name": "component: coercion",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19170#event-254358"
}
```



---

archive/issue_events_254359.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-09-09T01:01:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19170#event-254359"
}
```



---

archive/issue_events_254360.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-09-09T01:01:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19170#event-254360"
}
```



---

archive/issue_comments_273859.json:
```json
{
    "body": "Commit: **[861cce2](https://github.com/sagemath/sagetrac-mirror/commit/861cce24b8301821e3291ae53b22dccc094d9ebc)**",
    "created_at": "2015-09-09T01:02:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19170#issuecomment-273859",
    "user": "https://github.com/videlec"
}
```

Commit: **[861cce2](https://github.com/sagemath/sagetrac-mirror/commit/861cce24b8301821e3291ae53b22dccc094d9ebc)**



---

archive/issue_events_254361.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-09-09T01:02:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19170#event-254361"
}
```



---

archive/issue_comments_273860.json:
```json
{
    "body": "Branch: **[u/vdelecroix/19170](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/19170)**",
    "created_at": "2015-09-09T01:02:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19170#issuecomment-273860",
    "user": "https://github.com/videlec"
}
```

Branch: **[u/vdelecroix/19170](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/19170)**



---

archive/issue_comments_273861.json:
```json
{
    "body": "Dependencies: **#18305**",
    "created_at": "2015-09-09T01:02:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19170#issuecomment-273861",
    "user": "https://github.com/videlec"
}
```

Dependencies: **#18305**



---

archive/issue_comments_273862.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a9283cb08891995faa8289b7fde3880e06b347bd\">a9283cb</a></td><td><code>Trac 18305: py_rich_to_bool(_sgn) and operators</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e8b81826a0afb19b298ccf4aa55c601e3f35f367\">e8b8182</a></td><td><code>Trac 18305: `_richcmp_` for indexed monoids and groups</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/950f3ecc4198d82c64867500b2643ecd0cf6ce47\">950f3ec</a></td><td><code>Trac 18305: `_richcmp_` for Newton polygons</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/490de4234df92ac46aeb62ceafa09ff76c6b75b9\">490de42</a></td><td><code>Trac 18305: comments about Python comparisons</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3f6ea84eb645f1e29426bedc7763c9f31997867b\">3f6ea84</a></td><td><code>Trac 18305: rephrased sentence in doc</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4949e1796d94c3ff97083cc79eb148dfad921a83\">4949e17</a></td><td><code>Trac 19170: use `_cmp_` for ideals of finite dimensional algebras</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/861cce24b8301821e3291ae53b22dccc094d9ebc\">861cce2</a></td><td><code>Trac 19170: `_richcmp_` for `finite dimensional algebra elements</code></td></tr></table>\n",
    "created_at": "2015-09-09T01:02:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19170#issuecomment-273862",
    "user": "https://github.com/videlec"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a9283cb08891995faa8289b7fde3880e06b347bd">a9283cb</a></td><td><code>Trac 18305: py_rich_to_bool(_sgn) and operators</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e8b81826a0afb19b298ccf4aa55c601e3f35f367">e8b8182</a></td><td><code>Trac 18305: `_richcmp_` for indexed monoids and groups</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/950f3ecc4198d82c64867500b2643ecd0cf6ce47">950f3ec</a></td><td><code>Trac 18305: `_richcmp_` for Newton polygons</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/490de4234df92ac46aeb62ceafa09ff76c6b75b9">490de42</a></td><td><code>Trac 18305: comments about Python comparisons</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3f6ea84eb645f1e29426bedc7763c9f31997867b">3f6ea84</a></td><td><code>Trac 18305: rephrased sentence in doc</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4949e1796d94c3ff97083cc79eb148dfad921a83">4949e17</a></td><td><code>Trac 19170: use `_cmp_` for ideals of finite dimensional algebras</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/861cce24b8301821e3291ae53b22dccc094d9ebc">861cce2</a></td><td><code>Trac 19170: `_richcmp_` for `finite dimensional algebra elements</code></td></tr></table>




---

archive/issue_events_254362.json:
```json
{
    "actor": "https://github.com/kevindilks",
    "created_at": "2015-10-11T19:30:12Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "milestone_number": null,
    "milestone_title": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19170#event-254362"
}
```



---

archive/issue_events_254363.json:
```json
{
    "actor": "https://github.com/kevindilks",
    "created_at": "2015-10-11T19:30:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "milestone_number": null,
    "milestone_title": "sage-6.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19170#event-254363"
}
```



---

archive/issue_comments_273863.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nMerge conflicts.",
    "created_at": "2015-10-11T19:30:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19170#issuecomment-273863",
    "user": "https://github.com/kevindilks"
}
```

<div id="comment:2" align="right">Comment 2</div>

Merge conflicts.



---

archive/issue_comments_273864.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\ndoes not apply",
    "created_at": "2016-12-06T20:42:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19170#issuecomment-273864",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">Comment 3</div>

does not apply



---

archive/issue_events_254364.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-12-06T20:42:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19170#event-254364"
}
```



---

archive/issue_events_254365.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-12-06T20:42:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19170#event-254365"
}
```



---

archive/issue_events_254366.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-07-18T13:23:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20coercion",
    "label_color": "0000ff",
    "label_name": "component: coercion",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19170#event-254366"
}
```



---

archive/issue_events_254367.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-07-18T13:23:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20python3",
    "label_color": "000080",
    "label_name": "component: python3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19170#event-254367"
}
```



---

archive/issue_events_254368.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-07-18T13:23:35Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "milestone_number": null,
    "milestone_title": "sage-6.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19170#event-254368"
}
```



---

archive/issue_events_254369.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-07-18T13:23:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19170#event-254369"
}
```



---

archive/issue_comments_273865.json:
```json
{
    "body": "Changed dependencies from **#18305** to none",
    "created_at": "2017-07-18T13:23:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19170#issuecomment-273865",
    "user": "https://github.com/fchapoton"
}
```

Changed dependencies from **#18305** to none



---

archive/issue_events_254370.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-07-26T06:50:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19170#event-254370"
}
```



---

archive/issue_events_254371.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-07-26T06:50:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19170#event-254371"
}
```



---

archive/issue_comments_273866.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nThis is probably obsolete.",
    "created_at": "2017-07-26T06:50:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19170#issuecomment-273866",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:6" align="right">Comment 6</div>

This is probably obsolete.



---

archive/issue_events_254372.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-08-01T08:29:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19170#event-254372"
}
```



---

archive/issue_events_254373.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-08-01T08:29:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19170#event-254373"
}
```



---

archive/issue_events_254374.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-08-01T08:29:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19170#event-254374"
}
```



---

archive/issue_comments_273867.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nOk, let us consider that this one also is way outdated.",
    "created_at": "2017-08-29T13:23:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19170#issuecomment-273867",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:8" align="right">Comment 8</div>

Ok, let us consider that this one also is way outdated.



---

archive/issue_events_254375.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-08-29T13:23:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19170#event-254375"
}
```



---

archive/issue_events_254376.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-08-29T13:23:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19170#event-254376"
}
```



---

archive/issue_comments_273868.json:
```json
{
    "body": "Changed author from **Vincent Delecroix** to none",
    "created_at": "2017-08-29T13:58:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19170#issuecomment-273868",
    "user": "https://github.com/tscrim"
}
```

Changed author from **Vincent Delecroix** to none



---

archive/issue_comments_273869.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nThere is not a `def __cmp__` in `sage/algebras/` from grepping, so this has been taken care of.",
    "created_at": "2017-08-29T13:58:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19170#issuecomment-273869",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:9" align="right">Comment 9</div>

There is not a `def __cmp__` in `sage/algebras/` from grepping, so this has been taken care of.



---

archive/issue_comments_273870.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2017-08-29T13:58:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19170#issuecomment-273870",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_273871.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nYes, I wonder by who. But there remains to handle comparison of \"ideals\" in rings. I will work on that once the current py3 tickets have been included in sage.",
    "created_at": "2017-08-29T13:59:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19170#issuecomment-273871",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:10" align="right">Comment 10</div>

Yes, I wonder by who. But there remains to handle comparison of "ideals" in rings. I will work on that once the current py3 tickets have been included in sage.



---

archive/issue_events_254377.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-12-12T08:23:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19170#event-254377"
}
```



---

archive/issue_events_254378.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-12-12T08:23:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19170#event-254378"
}
```



---

archive/issue_events_254379.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-12-12T08:23:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19170",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19170#event-254379"
}
```
