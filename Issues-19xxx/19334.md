...
```json
{
    "body": "Looks trivial enough. Lucky to catch me there (I am on leave, moving and without internet at home before at least Tuesday- catching you from the public library).",
    "created_at": "2015-10-02T22:52:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19334",
    "type": "issue_comment",
...
```json
{
    "body": "I see segfaults on the patchbot.",
...
```json
{
    "body": "Legal in plain C but probably a bad idea (good old `v < -1` fun)",
...
```json
{
    "body": "Ah nevermind didn't read far enough into the generated C code...",
...
```json
{
    "body": "The difference is that Cython previously used `-1` as C integer literal, but now uses `-1L`. By the rules of C type promotion and on machines with 32-bit ints, this means that \n* `v = -1` sets `v` to `4294967295` = `UINT32_MAX - 1`\n* `v == -1` converts `-1L` to `UINT64_MAX - 1` = `18446744073709551615` and then compares with `v`\n\nTLDR: don't use unsigned and negative numbers",
...
```json
{
    "body": "The asteroidal triple code should be careful about signed values, but I'm not going to rewrite it on this ticket.",
    "created_at": "2015-10-03T11:02:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19334",
    "type": "issue_comment",
...
```json
{
    "body": "I will improve the code of asteroidal triples in #19337.",
...
```json
{
    "body": "One question: you set dependency to #19337 in which I have set dependency to #19334 (so circular dependencies).\nShouldn't we remove the dependency from this ticket to #19337 ?",
...
```json
{
    "body": "I'm not sure to follow you since 19337 is build on top of this ticket, but as long as it is working, it's fine for me.",
