# Issue 19274: Jmol (the java one) does not display some 2d polygons embedded in 3-space.

archive/issues_019037.json:
```json
{
    "body": "Thomas Fernique reported the following bug by private email: from the console, some flat polygons in three-space lead to an empty picture with jmol (java version), for example:\n\n```\nsage: polygon([(0, 0, 0), (1, 0, 0), (1, 0, 1), (0, 0, 1)])\n```\n\nHowever, this is correctly displayed with jsmol (javascript version) from the notebook.\n\nI can reproduce this bug on Sage 6.8, Debian GNU/Linux jessie 64 bit, openjdk-7-jre.\n\n\n**CC:**  @seblabbe\n\n**Reviewer:** Travis Scrimshaw\n\nIssue created by migration from https://trac.sagemath.org/ticket/19274\n\n",
    "closed_at": "2016-08-30T13:32:25Z",
    "created_at": "2015-09-23T13:41:38Z",
    "labels": [
        "component: graphics",
        "bug",
        "wontfix"
    ],
    "title": "Jmol (the java one) does not display some 2d polygons embedded in 3-space.",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/19274",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```
Thomas Fernique reported the following bug by private email: from the console, some flat polygons in three-space lead to an empty picture with jmol (java version), for example:

```
sage: polygon([(0, 0, 0), (1, 0, 0), (1, 0, 1), (0, 0, 1)])
```

However, this is correctly displayed with jsmol (javascript version) from the notebook.

I can reproduce this bug on Sage 6.8, Debian GNU/Linux jessie 64 bit, openjdk-7-jre.


**CC:**  @seblabbe

**Reviewer:** Travis Scrimshaw

Issue created by migration from https://trac.sagemath.org/ticket/19274





---

archive/issue_comments_277482.json:
```json
{
    "body": "<a id='comment:1'></a>\nDuplicate of #20978",
    "created_at": "2016-07-17T20:25:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19274",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19274#issuecomment-277482",
    "user": "https://github.com/paulmasson"
}
```

<a id='comment:1'></a>
Duplicate of #20978



---

archive/issue_events_176988.json:
```json
{
    "actor": "https://github.com/paulmasson",
    "created_at": "2016-07-17T20:25:41Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/19274",
    "milestone": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/19274#event-176988"
}
```



---

archive/issue_events_176989.json:
```json
{
    "actor": "https://github.com/paulmasson",
    "created_at": "2016-07-17T20:25:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/19274",
    "label": "duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/19274#event-176989"
}
```



---

archive/issue_events_176990.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-07-17T23:55:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/19274",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/19274#event-176990"
}
```



---

archive/issue_events_176991.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-07-17T23:55:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/19274",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/19274#event-176991"
}
```



---

archive/issue_events_176992.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-07-17T23:55:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/19274",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/19274#event-176992"
}
```



---

archive/issue_comments_277483.json:
```json
{
    "body": "**Reviewer:** Travis Scrimshaw",
    "created_at": "2016-07-17T23:55:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19274",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19274#issuecomment-277483",
    "user": "https://github.com/tscrim"
}
```

**Reviewer:** Travis Scrimshaw



---

archive/issue_comments_277484.json:
```json
{
    "body": "<a id='comment:4'></a>\nDetermined to be invalid/duplicate/wontfix (closing as \"wontfix\" as a catch-all resolution).",
    "created_at": "2016-08-30T13:32:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19274",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19274#issuecomment-277484",
    "user": "https://github.com/embray"
}
```

<a id='comment:4'></a>
Determined to be invalid/duplicate/wontfix (closing as "wontfix" as a catch-all resolution).



---

archive/issue_events_176993.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-08-30T13:32:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/19274",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/19274#event-176993"
}
```



---

archive/issue_events_176994.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-08-30T13:32:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/19274",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/19274#event-176994"
}
```



---

archive/issue_events_176995.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-08-30T13:32:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/19274",
    "label": "wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/19274#event-176995"
}
```
