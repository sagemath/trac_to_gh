# Issue 19274: Jmol (the java one) does not display some 2d polygons embedded in 3-space.

archive/issues_019037.json:
```json
{
    "assignees": [],
    "body": "Thomas Fernique reported the following bug by private email: from the console, some flat polygons in three-space lead to an empty picture with jmol (java version), for example:\n\n```\nsage: polygon([(0, 0, 0), (1, 0, 0), (1, 0, 1), (0, 0, 1)])\n```\n\nHowever, this is correctly displayed with jsmol (javascript version) from the notebook.\n\nI can reproduce this bug on Sage 6.8, Debian GNU/Linux jessie 64 bit, openjdk-7-jre.\n\n\nCC:  @seblabbe\n\nReviewer: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19274_\n\n",
    "closed_at": "2016-08-30T13:32:25Z",
    "created_at": "2015-09-23T13:41:38Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20graphics",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Jmol (the java one) does not display some 2d polygons embedded in 3-space.",
    "type": "issue",
    "updated_at": "2016-08-30T13:32:25Z",
    "url": "https://github.com/sagemath/sage/issues/19274",
    "user": "https://github.com/sagetrac-tmonteil"
}
```
Thomas Fernique reported the following bug by private email: from the console, some flat polygons in three-space lead to an empty picture with jmol (java version), for example:

```
sage: polygon([(0, 0, 0), (1, 0, 0), (1, 0, 1), (0, 0, 1)])
```

However, this is correctly displayed with jsmol (javascript version) from the notebook.

I can reproduce this bug on Sage 6.8, Debian GNU/Linux jessie 64 bit, openjdk-7-jre.


CC:  @seblabbe

Reviewer: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/19274_





---

archive/issue_events_255762.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2015-09-23T13:41:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19274",
    "milestone_number": null,
    "milestone_title": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19274#event-255762"
}
```



---

archive/issue_events_255763.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2015-09-23T13:41:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19274",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graphics",
    "label_color": "0000ff",
    "label_name": "component: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19274#event-255763"
}
```



---

archive/issue_events_255764.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2015-09-23T13:41:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19274",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19274#event-255764"
}
```



---

archive/issue_events_255765.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2015-09-23T13:41:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19274",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19274#event-255765"
}
```



---

archive/issue_comments_275962.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nDuplicate of #20978",
    "created_at": "2016-07-17T20:25:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19274",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19274#issuecomment-275962",
    "user": "https://github.com/paulmasson"
}
```

<div id="comment:1" align="right">Comment 1</div>

Duplicate of #20978



---

archive/issue_events_255766.json:
```json
{
    "actor": "https://github.com/paulmasson",
    "created_at": "2016-07-17T20:25:41Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/19274",
    "milestone_number": null,
    "milestone_title": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19274#event-255766"
}
```



---

archive/issue_events_255767.json:
```json
{
    "actor": "https://github.com/paulmasson",
    "created_at": "2016-07-17T20:25:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19274",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19274#event-255767"
}
```



---

archive/issue_events_255768.json:
```json
{
    "actor": "https://github.com/paulmasson",
    "created_at": "2016-07-17T20:25:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19274",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19274#event-255768"
}
```



---

archive/issue_events_255769.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-07-17T23:55:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19274",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19274#event-255769"
}
```



---

archive/issue_events_255770.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-07-17T23:55:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19274",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19274#event-255770"
}
```



---

archive/issue_events_255771.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-07-17T23:55:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19274",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19274#event-255771"
}
```



---

archive/issue_comments_275963.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2016-07-17T23:55:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19274",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19274#issuecomment-275963",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_275964.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nDetermined to be invalid/duplicate/wontfix (closing as \"wontfix\" as a catch-all resolution).",
    "created_at": "2016-08-30T13:32:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19274",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19274#issuecomment-275964",
    "user": "https://github.com/embray"
}
```

<div id="comment:4" align="right">Comment 4</div>

Determined to be invalid/duplicate/wontfix (closing as "wontfix" as a catch-all resolution).



---

archive/issue_events_255772.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-08-30T13:32:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19274",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19274#event-255772"
}
```



---

archive/issue_events_255773.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-08-30T13:32:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19274",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19274#event-255773"
}
```



---

archive/issue_events_255774.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-08-30T13:32:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19274",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19274#event-255774"
}
```
