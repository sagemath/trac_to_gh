# Issue 19912: Bug in method *to_cyclotomic_field* for the UniversalCyclotomicField

archive/issues_019675.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: a = E(4); a\nE(4)\nsage: a.to_cyclotomic_field()\nzeta4\nsage: a = 1+E(4); a\n1 + E(4)\nsage: a.to_cyclotomic_field()\nzeta4\n```\n\nCC:  @videlec @fchapoton\n\nKeywords: **universal cyclotomic field**\n\nBranch/Commit: **[6f31818](https://github.com/sagemath/sagetrac-mirror/commit/6f31818f26346cc05008c6ffeafe97b16f2cd156)**\n\nReviewer: **Vincent Delecroix**\n\nAuthor: **Christian Stump**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19912_\n\n",
    "closed_at": "2016-01-28T20:09:08Z",
    "created_at": "2016-01-19T09:36:08Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Bug in method *to_cyclotomic_field* for the UniversalCyclotomicField",
    "type": "issue",
    "updated_at": "2016-01-28T20:09:08Z",
    "url": "https://github.com/sagemath/sage/issues/19912",
    "user": "https://github.com/sagetrac-stumpc5"
}
```

```
sage: a = E(4); a
E(4)
sage: a.to_cyclotomic_field()
zeta4
sage: a = 1+E(4); a
1 + E(4)
sage: a.to_cyclotomic_field()
zeta4
```

CC:  @videlec @fchapoton

Keywords: **universal cyclotomic field**

Branch/Commit: **[6f31818](https://github.com/sagemath/sagetrac-mirror/commit/6f31818f26346cc05008c6ffeafe97b16f2cd156)**

Reviewer: **Vincent Delecroix**

Author: **Christian Stump**

_Issue created by migration from https://trac.sagemath.org/ticket/19912_





---

archive/issue_events_263650.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2016-01-19T09:36:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "milestone_number": null,
    "milestone_title": "sage-7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19912#event-263650"
}
```



---

archive/issue_events_263651.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2016-01-19T09:36:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19912#event-263651"
}
```



---

archive/issue_comments_287550.json:
```json
{
    "body": "Author: **Christian Stump**",
    "created_at": "2016-01-19T09:43:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19912#issuecomment-287550",
    "user": "https://github.com/sagetrac-stumpc5"
}
```

Author: **Christian Stump**



---

archive/issue_comments_287551.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,11 @@\n \n+```\n+sage: a = E(4); a\n+E(4)\n+sage: a.to_cyclotomic_field()\n+zeta4\n+sage: a = 1+E(4); a\n+1 + E(4)\n+sage: a.to_cyclotomic_field()\n+zeta4\n+```\n``````\n",
    "created_at": "2016-01-19T09:43:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19912#issuecomment-287551",
    "user": "https://github.com/sagetrac-stumpc5"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,11 @@
 
+```
+sage: a = E(4); a
+E(4)
+sage: a.to_cyclotomic_field()
+zeta4
+sage: a = 1+E(4); a
+1 + E(4)
+sage: a.to_cyclotomic_field()
+zeta4
+```
``````




---

archive/issue_comments_287552.json:
```json
{
    "body": "Changed keywords from **** to **universal cyclotomic field**",
    "created_at": "2016-01-19T09:43:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19912#issuecomment-287552",
    "user": "https://github.com/sagetrac-stumpc5"
}
```

Changed keywords from **** to **universal cyclotomic field**



---

archive/issue_events_263652.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2016-01-19T09:43:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
    "label_color": "0000ff",
    "label_name": "component: number fields",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19912#event-263652"
}
```



---

archive/issue_events_263653.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2016-01-19T09:43:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19912#event-263653"
}
```



---

archive/issue_comments_287553.json:
```json
{
    "body": "Branch: **[u/stumpc5/19912](https://github.com/sagemath/sagetrac-mirror/tree/u/stumpc5/19912)**",
    "created_at": "2016-01-19T09:49:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19912#issuecomment-287553",
    "user": "https://github.com/sagetrac-stumpc5"
}
```

Branch: **[u/stumpc5/19912](https://github.com/sagemath/sagetrac-mirror/tree/u/stumpc5/19912)**



---

archive/issue_comments_287554.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3b2432927d90f75f642e7bdcbc988b9d8b7a6b6f\">3b24329</a></td><td><code>fixed small bug</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/683a2f7fbd19f7d7508141732a6aa6b228276c47\">683a2f7</a></td><td><code>added doctest for the bug</code></td></tr></table>\n",
    "created_at": "2016-01-19T09:50:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19912#issuecomment-287554",
    "user": "https://github.com/sagetrac-stumpc5"
}
```

<div id="comment:3" align="right">Comment 3</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3b2432927d90f75f642e7bdcbc988b9d8b7a6b6f">3b24329</a></td><td><code>fixed small bug</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/683a2f7fbd19f7d7508141732a6aa6b228276c47">683a2f7</a></td><td><code>added doctest for the bug</code></td></tr></table>




---

archive/issue_comments_287555.json:
```json
{
    "body": "Commit: **[683a2f7](https://github.com/sagemath/sagetrac-mirror/commit/683a2f7fbd19f7d7508141732a6aa6b228276c47)**",
    "created_at": "2016-01-19T09:50:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19912#issuecomment-287555",
    "user": "https://github.com/sagetrac-stumpc5"
}
```

Commit: **[683a2f7](https://github.com/sagemath/sagetrac-mirror/commit/683a2f7fbd19f7d7508141732a6aa6b228276c47)**



---

archive/issue_events_263654.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2016-01-19T09:50:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19912#event-263654"
}
```



---

archive/issue_comments_287556.json:
```json
{
    "body": "Reviewer: **Vincent Delecroix**",
    "created_at": "2016-01-19T11:44:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19912#issuecomment-287556",
    "user": "https://github.com/videlec"
}
```

Reviewer: **Vincent Delecroix**



---

archive/issue_comments_287557.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nHello Christian,\n\nIndeed! The (1,k) should be changed in other places as well\n\n```\nsage: UCF = UniversalCyclotomicField()\nsage: UCFtoQQbar = UCF.coerce_embedding()\nsage: UCFtoQQbar(UCF.gen(4)+1)\n1*I\nsage: UCFtoQQbar(UCF.gen(4))\n1*I\n```\nalso \n\n```\nsage: CC(UCF.gen(4))\n1.00000000000000*I\nsage: CC(UCF.gen(4)+1)\n1.00000000000000*I\n```\n\nVincent",
    "created_at": "2016-01-19T11:44:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19912#issuecomment-287557",
    "user": "https://github.com/videlec"
}
```

<div id="comment:4" align="right">Comment 4</div>

Hello Christian,

Indeed! The (1,k) should be changed in other places as well

```
sage: UCF = UniversalCyclotomicField()
sage: UCFtoQQbar = UCF.coerce_embedding()
sage: UCFtoQQbar(UCF.gen(4)+1)
1*I
sage: UCFtoQQbar(UCF.gen(4))
1*I
```
also 

```
sage: CC(UCF.gen(4))
1.00000000000000*I
sage: CC(UCF.gen(4)+1)
1.00000000000000*I
```

Vincent



---

archive/issue_events_263655.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2016-01-19T11:44:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19912#event-263655"
}
```



---

archive/issue_events_263656.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2016-01-19T11:44:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19912#event-263656"
}
```



---

archive/issue_comments_287558.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/20d254c003a9353678bd52c265bf333802f7d616\">20d254c</a></td><td><code>fixed the same bug in two more places</code></td></tr></table>\n",
    "created_at": "2016-01-19T12:58:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19912#issuecomment-287558",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:5" align="right">Comment 5</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/20d254c003a9353678bd52c265bf333802f7d616">20d254c</a></td><td><code>fixed the same bug in two more places</code></td></tr></table>




---

archive/issue_comments_287559.json:
```json
{
    "body": "Changed commit from **[683a2f7](https://github.com/sagemath/sagetrac-mirror/commit/683a2f7fbd19f7d7508141732a6aa6b228276c47)** to **[20d254c](https://github.com/sagemath/sagetrac-mirror/commit/20d254c003a9353678bd52c265bf333802f7d616)**",
    "created_at": "2016-01-19T12:58:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19912#issuecomment-287559",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[683a2f7](https://github.com/sagemath/sagetrac-mirror/commit/683a2f7fbd19f7d7508141732a6aa6b228276c47)** to **[20d254c](https://github.com/sagemath/sagetrac-mirror/commit/20d254c003a9353678bd52c265bf333802f7d616)**



---

archive/issue_events_263657.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2016-01-19T12:59:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19912#event-263657"
}
```



---

archive/issue_events_263658.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2016-01-19T12:59:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19912#event-263658"
}
```



---

archive/issue_comments_287560.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/20d254c003a9353678bd52c265bf333802f7d616\">20d254c</a></td><td><code>fixed the same bug in two more places</code></td></tr></table>\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/20d254c003a9353678bd52c265bf333802f7d616\">20d254c</a></td><td><code>fixed the same bug in two more places</code></td></tr></table>\n",
    "created_at": "2016-01-19T12:59:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19912#issuecomment-287560",
    "user": "https://github.com/sagetrac-stumpc5"
}
```

<div id="comment:6" align="right">Comment 6</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/20d254c003a9353678bd52c265bf333802f7d616">20d254c</a></td><td><code>fixed the same bug in two more places</code></td></tr></table>

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/20d254c003a9353678bd52c265bf333802f7d616">20d254c</a></td><td><code>fixed the same bug in two more places</code></td></tr></table>




---

archive/issue_events_263659.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2016-01-19T14:38:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19912#event-263659"
}
```



---

archive/issue_events_263660.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2016-01-19T14:38:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19912#event-263660"
}
```



---

archive/issue_comments_287561.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nDoes not apply on the last develop.",
    "created_at": "2016-01-19T14:38:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19912#issuecomment-287561",
    "user": "https://github.com/videlec"
}
```

<div id="comment:7" align="right">Comment 7</div>

Does not apply on the last develop.



---

archive/issue_comments_287562.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nI solved the conflict, and added the trac role.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d56fb17ff05647e9a3efb9775a1856c0b1eefdae\">d56fb17</a></td><td><code>Merge branch 'u/stumpc5/19912' into 7.1.b0</code></td></tr></table>\n",
    "created_at": "2016-01-27T18:18:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19912#issuecomment-287562",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:8" align="right">Comment 8</div>

I solved the conflict, and added the trac role.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d56fb17ff05647e9a3efb9775a1856c0b1eefdae">d56fb17</a></td><td><code>Merge branch 'u/stumpc5/19912' into 7.1.b0</code></td></tr></table>




---

archive/issue_comments_287563.json:
```json
{
    "body": "Changed branch from **[u/stumpc5/19912](https://github.com/sagemath/sagetrac-mirror/tree/u/stumpc5/19912)** to **[public/19912](https://github.com/sagemath/sagetrac-mirror/tree/public/19912)**",
    "created_at": "2016-01-27T18:18:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19912#issuecomment-287563",
    "user": "https://github.com/fchapoton"
}
```

Changed branch from **[u/stumpc5/19912](https://github.com/sagemath/sagetrac-mirror/tree/u/stumpc5/19912)** to **[public/19912](https://github.com/sagemath/sagetrac-mirror/tree/public/19912)**



---

archive/issue_comments_287564.json:
```json
{
    "body": "Changed commit from **[20d254c](https://github.com/sagemath/sagetrac-mirror/commit/20d254c003a9353678bd52c265bf333802f7d616)** to **[d56fb17](https://github.com/sagemath/sagetrac-mirror/commit/d56fb17ff05647e9a3efb9775a1856c0b1eefdae)**",
    "created_at": "2016-01-27T18:18:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19912#issuecomment-287564",
    "user": "https://github.com/fchapoton"
}
```

Changed commit from **[20d254c](https://github.com/sagemath/sagetrac-mirror/commit/20d254c003a9353678bd52c265bf333802f7d616)** to **[d56fb17](https://github.com/sagemath/sagetrac-mirror/commit/d56fb17ff05647e9a3efb9775a1856c0b1eefdae)**



---

archive/issue_events_263661.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-01-27T18:18:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19912#event-263661"
}
```



---

archive/issue_events_263662.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-01-27T18:18:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19912#event-263662"
}
```



---

archive/issue_comments_287565.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6f31818f26346cc05008c6ffeafe97b16f2cd156\">6f31818</a></td><td><code>trac 19912 other trac roles</code></td></tr></table>\n",
    "created_at": "2016-01-27T18:23:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19912#issuecomment-287565",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:10" align="right">Comment 10</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6f31818f26346cc05008c6ffeafe97b16f2cd156">6f31818</a></td><td><code>trac 19912 other trac roles</code></td></tr></table>




---

archive/issue_comments_287566.json:
```json
{
    "body": "Changed commit from **[d56fb17](https://github.com/sagemath/sagetrac-mirror/commit/d56fb17ff05647e9a3efb9775a1856c0b1eefdae)** to **[6f31818](https://github.com/sagemath/sagetrac-mirror/commit/6f31818f26346cc05008c6ffeafe97b16f2cd156)**",
    "created_at": "2016-01-27T18:23:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19912#issuecomment-287566",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[d56fb17](https://github.com/sagemath/sagetrac-mirror/commit/d56fb17ff05647e9a3efb9775a1856c0b1eefdae)** to **[6f31818](https://github.com/sagemath/sagetrac-mirror/commit/6f31818f26346cc05008c6ffeafe97b16f2cd156)**



---

archive/issue_comments_287567.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nok, looks good to me. Just waiting for the patchbot report to be sure.",
    "created_at": "2016-01-27T18:24:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19912#issuecomment-287567",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:11" align="right">Comment 11</div>

ok, looks good to me. Just waiting for the patchbot report to be sure.



---

archive/issue_comments_287568.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nThanks -- my machine is still compiling `7.1.beta0`...",
    "created_at": "2016-01-27T18:31:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19912#issuecomment-287568",
    "user": "https://github.com/sagetrac-stumpc5"
}
```

<div id="comment:12" align="right">Comment 12</div>

Thanks -- my machine is still compiling `7.1.beta0`...



---

archive/issue_events_263663.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2016-01-28T06:46:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19912#event-263663"
}
```



---

archive/issue_events_263664.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2016-01-28T06:46:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19912#event-263664"
}
```



---

archive/issue_comments_287569.json:
```json
{
    "body": "Changed branch from **[public/19912](https://github.com/sagemath/sagetrac-mirror/tree/public/19912)** to **[6f31818](https://github.com/sagemath/sagetrac-mirror/commit/6f31818f26346cc05008c6ffeafe97b16f2cd156)**",
    "created_at": "2016-01-28T20:09:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19912#issuecomment-287569",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/19912](https://github.com/sagemath/sagetrac-mirror/tree/public/19912)** to **[6f31818](https://github.com/sagemath/sagetrac-mirror/commit/6f31818f26346cc05008c6ffeafe97b16f2cd156)**



---

archive/issue_events_263665.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-01-28T20:09:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19912#event-263665"
}
```



---

archive/issue_events_263666.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "8b9c5dfe65e30ef9e14367de7f3054681730105b",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-01-28T20:09:08Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19912",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19912#event-263666"
}
```
