# Issue 19093: bug in units conversion

archive/issues_018856.json:
```json
{
    "body": "The .convert() function converts units to their base units, e.g.\n\n```\nsage: (units.energy.joule).convert()                            \n```\n\n```\nkilogram*meter^2/second^2\n```\nThe units of heat capacity are joule/kelvin/kilogram and since joule is not a base unit, it should be possible to convert them to base units. However, this throws an error:\n\n```\nsage: (units.energy.joule/units.temperature.kelvin/units.mass.kilogram).convert()\n```\n\n```\nTraceback (click to the left of this block for traceback)\n...\nValueError: Cannot convert\n```\n\nThis seems to be the case whenever something is divided by temperature.\n\nKeywords: units\n\nStatus: new\n\nIssue created by migration from https://trac.sagemath.org/ticket/19093\n\n",
    "created_at": "2015-08-26T13:40:47Z",
    "labels": [
        "component: symbolics",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.9",
    "title": "bug in units conversion",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/19093",
    "user": "https://trac.sagemath.org/admin/accounts/users/schymans"
}
```
The .convert() function converts units to their base units, e.g.

```
sage: (units.energy.joule).convert()                            
```

```
kilogram*meter^2/second^2
```
The units of heat capacity are joule/kelvin/kilogram and since joule is not a base unit, it should be possible to convert them to base units. However, this throws an error:

```
sage: (units.energy.joule/units.temperature.kelvin/units.mass.kilogram).convert()
```

```
Traceback (click to the left of this block for traceback)
...
ValueError: Cannot convert
```

This seems to be the case whenever something is divided by temperature.

Keywords: units

Status: new

Issue created by migration from https://trac.sagemath.org/ticket/19093





---

archive/issue_comments_257749.json:
```json
{
    "body": "<a id='comment:1'></a>Goodness, did I really create this ticket only 3 weeks ago? Just googled it again and found the same issue asked by myself 2 years ago with a proposed workaround by Eviatar Bach: \nhttp://ask.sagemath.org/question/10260/convert-derived-si-units-to-base-units/",
    "created_at": "2015-09-18T15:03:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19093",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19093#issuecomment-257749",
    "user": "https://trac.sagemath.org/admin/accounts/users/schymans"
}
```

<a id='comment:1'></a>Goodness, did I really create this ticket only 3 weeks ago? Just googled it again and found the same issue asked by myself 2 years ago with a proposed workaround by Eviatar Bach: 
http://ask.sagemath.org/question/10260/convert-derived-si-units-to-base-units/
