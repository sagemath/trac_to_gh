# Issue 19366: fixing the hidden doc in geometry

archive/issues_019129.json:
```json
{
    "assignees": [],
    "body": "A few docs of underscore methods in /geometry are broken.\n\nThey can be found using\n\nsage -docbuild -u -k reference/geometry html\n\nLet us fix that.\n\n\n\nKeywords: **hidden doc**\n\nBranch/Commit: **[2f8fe5e](https://github.com/sagemath/sagetrac-mirror/commit/2f8fe5e02cfbfaf1d1b624b29c125028c787a089)**\n\nReviewer: **Nathann Cohen**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19366_\n\n",
    "closed_at": "2015-10-14T00:59:25Z",
    "created_at": "2015-10-07T19:31:53Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fixing the hidden doc in geometry",
    "type": "issue",
    "updated_at": "2015-10-14T00:59:25Z",
    "url": "https://github.com/sagemath/sage/issues/19366",
    "user": "https://github.com/fchapoton"
}
```
A few docs of underscore methods in /geometry are broken.

They can be found using

sage -docbuild -u -k reference/geometry html

Let us fix that.



Keywords: **hidden doc**

Branch/Commit: **[2f8fe5e](https://github.com/sagemath/sagetrac-mirror/commit/2f8fe5e02cfbfaf1d1b624b29c125028c787a089)**

Reviewer: **Nathann Cohen**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/19366_





---

archive/issue_events_256907.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-10-07T19:31:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19366",
    "milestone_number": null,
    "milestone_title": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19366#event-256907"
}
```



---

archive/issue_events_256908.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-10-07T19:31:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19366",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19366#event-256908"
}
```



---

archive/issue_events_256909.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-10-07T19:31:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19366",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19366#event-256909"
}
```



---

archive/issue_events_256910.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-10-07T19:31:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19366",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19366#event-256910"
}
```



---

archive/issue_comments_277646.json:
```json
{
    "body": "Commit: **[2f8fe5e](https://github.com/sagemath/sagetrac-mirror/commit/2f8fe5e02cfbfaf1d1b624b29c125028c787a089)**",
    "created_at": "2015-10-07T19:33:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19366",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19366#issuecomment-277646",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[2f8fe5e](https://github.com/sagemath/sagetrac-mirror/commit/2f8fe5e02cfbfaf1d1b624b29c125028c787a089)**



---

archive/issue_comments_277647.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,6 +1,6 @@\n-A few docs of underscore methods are broken.\n+A few docs of underscore methods in /geometry are broken.\n \n-found using\n+They can be found using\n \n sage -docbuild -u -k reference/geometry html\n \n``````\n",
    "created_at": "2015-10-07T19:33:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19366",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19366#issuecomment-277647",
    "user": "https://github.com/fchapoton"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,6 +1,6 @@
-A few docs of underscore methods are broken.
+A few docs of underscore methods in /geometry are broken.
 
-found using
+They can be found using
 
 sage -docbuild -u -k reference/geometry html
 
``````




---

archive/issue_comments_277648.json:
```json
{
    "body": "Branch: **[public/19366](https://github.com/sagemath/sagetrac-mirror/tree/public/19366)**",
    "created_at": "2015-10-07T19:33:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19366",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19366#issuecomment-277648",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[public/19366](https://github.com/sagemath/sagetrac-mirror/tree/public/19366)**



---

archive/issue_comments_277649.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2f8fe5e02cfbfaf1d1b624b29c125028c787a089\">2f8fe5e</a></td><td><code>fixing hidden doctests in geometry folder</code></td></tr></table>\n",
    "created_at": "2015-10-07T19:33:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19366",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19366#issuecomment-277649",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2f8fe5e02cfbfaf1d1b624b29c125028c787a089">2f8fe5e</a></td><td><code>fixing hidden doctests in geometry folder</code></td></tr></table>




---

archive/issue_events_256911.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-10-07T19:33:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19366",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19366#event-256911"
}
```



---

archive/issue_events_256912.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-10-07T19:33:04Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/19366",
    "title_is": "fixing the hidden doc in geometry",
    "title_was": "fixiing hidden doc in geometry",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19366#event-256912"
}
```



---

archive/issue_comments_277650.json:
```json
{
    "body": "Reviewer: **Nathann Cohen**",
    "created_at": "2015-10-07T20:08:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19366",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19366#issuecomment-277650",
    "user": "https://github.com/nathanncohen"
}
```

Reviewer: **Nathann Cohen**



---

archive/issue_events_256913.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-10-07T20:08:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19366",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19366#event-256913"
}
```



---

archive/issue_events_256914.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-10-07T20:08:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19366",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19366#event-256914"
}
```



---

archive/issue_events_256915.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-10-14T00:59:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19366",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19366#event-256915"
}
```



---

archive/issue_events_256916.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "167e9d19aa98d749126003adf3831bdaa29b89db",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-10-14T00:59:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19366",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19366#event-256916"
}
```



---

archive/issue_comments_277651.json:
```json
{
    "body": "Changed branch from **[public/19366](https://github.com/sagemath/sagetrac-mirror/tree/public/19366)** to **[2f8fe5e](https://github.com/sagemath/sagetrac-mirror/commit/2f8fe5e02cfbfaf1d1b624b29c125028c787a089)**",
    "created_at": "2015-10-14T00:59:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19366",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19366#issuecomment-277651",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/19366](https://github.com/sagemath/sagetrac-mirror/tree/public/19366)** to **[2f8fe5e](https://github.com/sagemath/sagetrac-mirror/commit/2f8fe5e02cfbfaf1d1b624b29c125028c787a089)**
