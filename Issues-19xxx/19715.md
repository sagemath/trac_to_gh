# Issue 19715: ATLAS should skip "fast" configuration if it is not implemented

archive/issues_019478.json:
```json
{
    "assignees": [],
    "body": "ATLAS should just skip the \"fast\" option if it is not available:\n\n```\nThird attempt: use \"fast\" options.\nTraceback (most recent call last):\n  File \"./spkg-install\", line 526, in <module>\n    rc = build(arch='fast')\n  File \"./spkg-install\", line 454, in build\n    rc = configure(arch, isa_ext)\n  File \"./spkg-install\", line 254, in configure\n    arch, isa_ext, thread_limit = configure_fast()\n  File \"./spkg-install\", line 374, in configure_fast\n    raise NotImplementedError('I don\\'t know a \"fast\" configuration for your cpu.')\nNotImplementedError: I don't know a \"fast\" configuration for your cpu.\n```\n\nBranch/Commit: **[939d37b](https://github.com/sagemath/sagetrac-mirror/commit/939d37b1c24ce2eabf1dabf8d4a9591da34e1b30)**\n\nReviewer: **Volker Braun**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19715_\n\n",
    "closed_at": "2015-12-28T11:08:10Z",
    "created_at": "2015-12-15T10:26:14Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "ATLAS should skip \"fast\" configuration if it is not implemented",
    "type": "issue",
    "updated_at": "2015-12-28T11:08:10Z",
    "url": "https://github.com/sagemath/sage/issues/19715",
    "user": "https://github.com/jdemeyer"
}
```
ATLAS should just skip the "fast" option if it is not available:

```
Third attempt: use "fast" options.
Traceback (most recent call last):
  File "./spkg-install", line 526, in <module>
    rc = build(arch='fast')
  File "./spkg-install", line 454, in build
    rc = configure(arch, isa_ext)
  File "./spkg-install", line 254, in configure
    arch, isa_ext, thread_limit = configure_fast()
  File "./spkg-install", line 374, in configure_fast
    raise NotImplementedError('I don\'t know a "fast" configuration for your cpu.')
NotImplementedError: I don't know a "fast" configuration for your cpu.
```

Branch/Commit: **[939d37b](https://github.com/sagemath/sagetrac-mirror/commit/939d37b1c24ce2eabf1dabf8d4a9591da34e1b30)**

Reviewer: **Volker Braun**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/19715_





---

archive/issue_events_261132.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-15T10:26:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19715",
    "milestone_number": null,
    "milestone_title": "sage-7.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19715#event-261132"
}
```



---

archive/issue_events_261133.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-15T10:26:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19715",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19715#event-261133"
}
```



---

archive/issue_events_261134.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-15T10:26:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19715",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19715#event-261134"
}
```



---

archive/issue_events_261135.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-15T10:26:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19715",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19715#event-261135"
}
```



---

archive/issue_comments_284103.json:
```json
{
    "body": "Branch: **[u/jdemeyer/atlas_should_skip__fast__configuration_if_it_is_not_implemented](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/atlas_should_skip__fast__configuration_if_it_is_not_implemented)**",
    "created_at": "2015-12-15T10:29:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19715",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19715#issuecomment-284103",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/atlas_should_skip__fast__configuration_if_it_is_not_implemented](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/atlas_should_skip__fast__configuration_if_it_is_not_implemented)**



---

archive/issue_comments_284104.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6aea3d36ae8cab6bcb3e9212aa8822983284036b\">6aea3d3</a></td><td><code>Return rc instead of exiting when configure() fails</code></td></tr></table>\n",
    "created_at": "2015-12-15T11:07:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19715",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19715#issuecomment-284104",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6aea3d36ae8cab6bcb3e9212aa8822983284036b">6aea3d3</a></td><td><code>Return rc instead of exiting when configure() fails</code></td></tr></table>




---

archive/issue_comments_284105.json:
```json
{
    "body": "Commit: **[6aea3d3](https://github.com/sagemath/sagetrac-mirror/commit/6aea3d36ae8cab6bcb3e9212aa8822983284036b)**",
    "created_at": "2015-12-15T11:07:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19715",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19715#issuecomment-284105",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[6aea3d3](https://github.com/sagemath/sagetrac-mirror/commit/6aea3d36ae8cab6bcb3e9212aa8822983284036b)**



---

archive/issue_events_261136.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-15T11:16:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19715",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19715#event-261136"
}
```



---

archive/issue_events_261137.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-15T23:01:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19715",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19715#event-261137"
}
```



---

archive/issue_events_261138.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-15T23:01:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19715",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19715#event-261138"
}
```



---

archive/issue_comments_284106.json:
```json
{
    "body": "Changed commit from **[6aea3d3](https://github.com/sagemath/sagetrac-mirror/commit/6aea3d36ae8cab6bcb3e9212aa8822983284036b)** to **[939d37b](https://github.com/sagemath/sagetrac-mirror/commit/939d37b1c24ce2eabf1dabf8d4a9591da34e1b30)**",
    "created_at": "2015-12-17T13:40:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19715",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19715#issuecomment-284106",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[6aea3d3](https://github.com/sagemath/sagetrac-mirror/commit/6aea3d36ae8cab6bcb3e9212aa8822983284036b)** to **[939d37b](https://github.com/sagemath/sagetrac-mirror/commit/939d37b1c24ce2eabf1dabf8d4a9591da34e1b30)**



---

archive/issue_comments_284107.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/939d37b1c24ce2eabf1dabf8d4a9591da34e1b30\">939d37b</a></td><td><code>Return rc instead of exiting when configure() fails</code></td></tr></table>\n",
    "created_at": "2015-12-17T13:40:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19715",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19715#issuecomment-284107",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:5'>Comment 5:</a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/939d37b1c24ce2eabf1dabf8d4a9591da34e1b30">939d37b</a></td><td><code>Return rc instead of exiting when configure() fails</code></td></tr></table>




---

archive/issue_events_261139.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-17T19:27:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19715",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19715#event-261139"
}
```



---

archive/issue_events_261140.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-17T19:27:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19715",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19715#event-261140"
}
```



---

archive/issue_comments_284108.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nIf I remember correctly there is another issue with POWER8 and Atlas.\nConfiguration (or tuning, who knows) fails because there are too many registers on POWER8 !!!\n\nSee http://sourceforge.net/p/math-atlas/bugs/225/\nOh it was POWER7 already actually.",
    "created_at": "2015-12-23T10:20:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19715",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19715#issuecomment-284108",
    "user": "https://github.com/sagetrac-jpflori"
}
```

<a id='comment:7'>Comment 7:</a>
If I remember correctly there is another issue with POWER8 and Atlas.
Configuration (or tuning, who knows) fails because there are too many registers on POWER8 !!!

See http://sourceforge.net/p/math-atlas/bugs/225/
Oh it was POWER7 already actually.



---

archive/issue_comments_284109.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nI didn't have that problem. Either it's fixed or it occurs only on POWER7, not POWER8.",
    "created_at": "2015-12-23T11:03:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19715",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19715#issuecomment-284109",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:8'>Comment 8:</a>
I didn't have that problem. Either it's fixed or it occurs only on POWER7, not POWER8.



---

archive/issue_comments_284110.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2015-12-27T16:41:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19715",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19715#issuecomment-284110",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_events_261141.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-12-27T16:41:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19715",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19715#event-261141"
}
```



---

archive/issue_events_261142.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-12-27T16:41:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19715",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19715#event-261142"
}
```



---

archive/issue_comments_284111.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/atlas_should_skip__fast__configuration_if_it_is_not_implemented](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/atlas_should_skip__fast__configuration_if_it_is_not_implemented)** to **[939d37b](https://github.com/sagemath/sagetrac-mirror/commit/939d37b1c24ce2eabf1dabf8d4a9591da34e1b30)**",
    "created_at": "2015-12-28T11:08:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19715",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19715#issuecomment-284111",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/atlas_should_skip__fast__configuration_if_it_is_not_implemented](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/atlas_should_skip__fast__configuration_if_it_is_not_implemented)** to **[939d37b](https://github.com/sagemath/sagetrac-mirror/commit/939d37b1c24ce2eabf1dabf8d4a9591da34e1b30)**



---

archive/issue_events_261143.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-12-28T11:08:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19715",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19715#event-261143"
}
```



---

archive/issue_events_261144.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "9ffb8eceadfc74ac0294800db98a38362518e747",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-12-28T11:08:10Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19715",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19715#event-261144"
}
```
