# Issue 19664: Fix Laurent series comparison and make it more efficient

archive/issues_019427.json:
```json
{
    "assignees": [],
    "body": "This is very bad:\n\n```\nsage: R.<x> = LaurentSeriesRing(ZZ)\nsage: f = x^-2 + O(x^0); f\nx^-2 + O(1)\nsage: g = x^-1 + O(x^0); g\nx^-1 + O(1)\nsage: f == g\nTrue\nsage: f-g\nx^-2 - x^-1 + O(1)\n```\n\nAlso this is bad:\n\n```\nsage: R.<x> = LaurentSeriesRing(ZZ)\nsage: x^(10^10) > 0\n...\nMemoryError: \n```\n\nBranch/Commit: **[519bafb](https://github.com/sagemath/sagetrac-mirror/commit/519bafb901ce9304e4fa36be89fd016074dee9f6)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19664_\n\n",
    "closed_at": "2015-12-06T21:42:35Z",
    "created_at": "2015-12-03T12:50:37Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.10",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix Laurent series comparison and make it more efficient",
    "type": "issue",
    "updated_at": "2015-12-06T21:42:35Z",
    "url": "https://github.com/sagemath/sage/issues/19664",
    "user": "https://github.com/jdemeyer"
}
```
This is very bad:

```
sage: R.<x> = LaurentSeriesRing(ZZ)
sage: f = x^-2 + O(x^0); f
x^-2 + O(1)
sage: g = x^-1 + O(x^0); g
x^-1 + O(1)
sage: f == g
True
sage: f-g
x^-2 - x^-1 + O(1)
```

Also this is bad:

```
sage: R.<x> = LaurentSeriesRing(ZZ)
sage: x^(10^10) > 0
...
MemoryError: 
```

Branch/Commit: **[519bafb](https://github.com/sagemath/sagetrac-mirror/commit/519bafb901ce9304e4fa36be89fd016074dee9f6)**

Reviewer: **Travis Scrimshaw**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/19664_





---

archive/issue_events_260562.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-03T12:50:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19664",
    "milestone_number": null,
    "milestone_title": "sage-6.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19664#event-260562"
}
```



---

archive/issue_events_260563.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-03T12:50:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19664",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "component: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19664#event-260563"
}
```



---

archive/issue_events_260564.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-03T12:50:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19664",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19664#event-260564"
}
```



---

archive/issue_events_260565.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-03T12:50:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19664",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19664#event-260565"
}
```



---

archive/issue_comments_283388.json:
```json
{
    "body": "Stopgaps: **#19664**",
    "created_at": "2015-12-03T13:01:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19664#issuecomment-283388",
    "user": "https://github.com/jdemeyer"
}
```

Stopgaps: **#19664**



---

archive/issue_events_260566.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-03T13:01:10Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/19664",
    "title_is": "Fix Laurent series comparison and make it more efficient",
    "title_was": "Make Laurent series comparison more efficient",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19664#event-260566"
}
```



---

archive/issue_comments_283389.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,6 +1,18 @@\n-The `_cmp_` function for Laurent series can be improved in efficiency.\n+This is very bad:\n \n-In particular, this is bad:\n+```\n+sage: R.<x> = LaurentSeriesRing(ZZ)\n+sage: f = x^-2 + O(x^0); f\n+x^-2 + O(1)\n+sage: g = x^-1 + O(x^0); g\n+x^-1 + O(1)\n+sage: f == g\n+True\n+sage: f-g\n+x^-2 - x^-1 + O(1)\n+```\n+\n+Also this is bad:\n \n ```\n sage: R.<x> = LaurentSeriesRing(ZZ)\n``````\n",
    "created_at": "2015-12-03T13:01:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19664#issuecomment-283389",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,6 +1,18 @@
-The `_cmp_` function for Laurent series can be improved in efficiency.
+This is very bad:
 
-In particular, this is bad:
+```
+sage: R.<x> = LaurentSeriesRing(ZZ)
+sage: f = x^-2 + O(x^0); f
+x^-2 + O(1)
+sage: g = x^-1 + O(x^0); g
+x^-1 + O(1)
+sage: f == g
+True
+sage: f-g
+x^-2 - x^-1 + O(1)
+```
+
+Also this is bad:
 
 ```
 sage: R.<x> = LaurentSeriesRing(ZZ)
``````




---

archive/issue_comments_283390.json:
```json
{
    "body": "Branch: **[u/jdemeyer/make_laurent_series_comparison_more_efficient](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/make_laurent_series_comparison_more_efficient)**",
    "created_at": "2015-12-03T14:54:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19664#issuecomment-283390",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/make_laurent_series_comparison_more_efficient](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/make_laurent_series_comparison_more_efficient)**



---

archive/issue_events_260567.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-03T14:55:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19664",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19664#event-260567"
}
```



---

archive/issue_comments_283391.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/519bafb901ce9304e4fa36be89fd016074dee9f6\">519bafb</a></td><td><code>Fix Laurent series comparison and make it more efficient</code></td></tr></table>\n",
    "created_at": "2015-12-03T14:55:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19664#issuecomment-283391",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:3" align="right">Comment 3</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/519bafb901ce9304e4fa36be89fd016074dee9f6">519bafb</a></td><td><code>Fix Laurent series comparison and make it more efficient</code></td></tr></table>




---

archive/issue_comments_283392.json:
```json
{
    "body": "Commit: **[519bafb](https://github.com/sagemath/sagetrac-mirror/commit/519bafb901ce9304e4fa36be89fd016074dee9f6)**",
    "created_at": "2015-12-03T14:55:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19664#issuecomment-283392",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[519bafb](https://github.com/sagemath/sagetrac-mirror/commit/519bafb901ce9304e4fa36be89fd016074dee9f6)**



---

archive/issue_comments_283393.json:
```json
{
    "body": "Changed stopgaps from **#19664** to none",
    "created_at": "2015-12-05T16:34:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19664#issuecomment-283393",
    "user": "https://github.com/tscrim"
}
```

Changed stopgaps from **#19664** to none



---

archive/issue_comments_283394.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nLGTM.",
    "created_at": "2015-12-05T16:34:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19664#issuecomment-283394",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:4" align="right">Comment 4</div>

LGTM.



---

archive/issue_events_260568.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-12-05T16:34:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19664",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19664#event-260568"
}
```



---

archive/issue_events_260569.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-12-05T16:34:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19664",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19664#event-260569"
}
```



---

archive/issue_comments_283395.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2015-12-05T16:34:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19664#issuecomment-283395",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_283396.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/make_laurent_series_comparison_more_efficient](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/make_laurent_series_comparison_more_efficient)** to **[519bafb](https://github.com/sagemath/sagetrac-mirror/commit/519bafb901ce9304e4fa36be89fd016074dee9f6)**",
    "created_at": "2015-12-06T21:42:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19664",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19664#issuecomment-283396",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/make_laurent_series_comparison_more_efficient](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/make_laurent_series_comparison_more_efficient)** to **[519bafb](https://github.com/sagemath/sagetrac-mirror/commit/519bafb901ce9304e4fa36be89fd016074dee9f6)**



---

archive/issue_events_260570.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-12-06T21:42:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19664",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19664#event-260570"
}
```



---

archive/issue_events_260571.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "f8392e38a83b29bdaf771d888deb8aa7c93117ae",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-12-06T21:42:35Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19664",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19664#event-260571"
}
```
