# Issue 19554: Workaround for #19553 for FiniteEnumeratedSet

archive/issues_019317.json:
```json
{
    "assignees": [],
    "body": "We work around #19553 by trying directly to see if `_element_constructor_` works if `Parent.__call__` fails with a `TypeError`.\n\nAssignee: **@tscrim**\n\nCC:  @sagetrac-sage-combinat\n\nBranch: **[1e3ace7](https://github.com/sagemath/sagetrac-mirror/commit/1e3ace7e6f2b004dccb83e6f726d7bc12eda8279)**\n\nReviewer: **Daniel Krenn**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19554_\n\n",
    "closed_at": "2019-03-29T12:36:50Z",
    "created_at": "2015-11-08T22:02:15Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Workaround for #19553 for FiniteEnumeratedSet",
    "type": "issue",
    "updated_at": "2019-03-29T17:20:39Z",
    "url": "https://github.com/sagemath/sage/issues/19554",
    "user": "https://github.com/tscrim"
}
```
We work around #19553 by trying directly to see if `_element_constructor_` works if `Parent.__call__` fails with a `TypeError`.

Assignee: **@tscrim**

CC:  @sagetrac-sage-combinat

Branch: **[1e3ace7](https://github.com/sagemath/sagetrac-mirror/commit/1e3ace7e6f2b004dccb83e6f726d7bc12eda8279)**

Reviewer: **Daniel Krenn**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/19554_





---

archive/issue_events_259212.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-11-08T22:02:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "milestone_number": null,
    "milestone_title": "sage-6.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19554#event-259212"
}
```



---

archive/issue_events_259213.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-11-08T22:02:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19554#event-259213"
}
```



---

archive/issue_events_259214.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-11-08T22:02:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19554#event-259214"
}
```



---

archive/issue_events_259215.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-11-08T22:02:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19554#event-259215"
}
```



---

archive/issue_comments_281096.json:
```json
{
    "body": "Commit: **[3e7eadd](https://github.com/sagemath/sagetrac-mirror/commit/3e7eaddafebea45d5a62454bba6e34f068b8febc)**",
    "created_at": "2015-11-08T22:14:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19554#issuecomment-281096",
    "user": "https://github.com/tscrim"
}
```

Commit: **[3e7eadd](https://github.com/sagemath/sagetrac-mirror/commit/3e7eaddafebea45d5a62454bba6e34f068b8febc)**



---

archive/issue_comments_281097.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3e7eaddafebea45d5a62454bba6e34f068b8febc\">3e7eadd</a></td><td><code>A workaround for #19554.</code></td></tr></table>\n",
    "created_at": "2015-11-08T22:14:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19554#issuecomment-281097",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3e7eaddafebea45d5a62454bba6e34f068b8febc">3e7eadd</a></td><td><code>A workaround for #19554.</code></td></tr></table>




---

archive/issue_events_259216.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-11-08T22:14:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19554#event-259216"
}
```



---

archive/issue_comments_281098.json:
```json
{
    "body": "Branch: **[public/sets/workaround_FiniteEnumeratedSet-19554](https://github.com/sagemath/sagetrac-mirror/tree/public/sets/workaround_FiniteEnumeratedSet-19554)**",
    "created_at": "2015-11-08T22:14:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19554#issuecomment-281098",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/sets/workaround_FiniteEnumeratedSet-19554](https://github.com/sagemath/sagetrac-mirror/tree/public/sets/workaround_FiniteEnumeratedSet-19554)**



---

archive/issue_comments_281099.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\ndoctest fails (cf. patchbot).",
    "created_at": "2017-01-05T11:42:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19554#issuecomment-281099",
    "user": "https://github.com/cheuberg"
}
```

<div id="comment:2" align="right">Comment 2</div>

doctest fails (cf. patchbot).



---

archive/issue_events_259217.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2017-01-05T11:42:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19554#event-259217"
}
```



---

archive/issue_events_259218.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2017-01-05T11:42:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19554#event-259218"
}
```



---

archive/issue_comments_281100.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6afcb5225f6bbda0bc97635caf0c732ca3616204\">6afcb52</a></td><td><code>Merge branch 'public/sets/workaround_FiniteEnumeratedSet-19554' of git://trac.sagemath.org/sage into public/sets/workaround_FiniteEnumeratedSet-19554</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1f1ace22e35effddcde5a0626267f07ab48d2ff9\">1f1ace2</a></td><td><code>Try Parent.__call__ first and if that fails, try once more _element_constructor_.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1f3c136ccf10c4bfa214c2fe466616e35d644c27\">1f3c136</a></td><td><code>Try first Parent.__call__ then fallback on self._element_constructor_.</code></td></tr></table>\n",
    "created_at": "2017-07-11T03:30:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19554#issuecomment-281100",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6afcb5225f6bbda0bc97635caf0c732ca3616204">6afcb52</a></td><td><code>Merge branch 'public/sets/workaround_FiniteEnumeratedSet-19554' of git://trac.sagemath.org/sage into public/sets/workaround_FiniteEnumeratedSet-19554</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1f1ace22e35effddcde5a0626267f07ab48d2ff9">1f1ace2</a></td><td><code>Try Parent.__call__ first and if that fails, try once more _element_constructor_.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1f3c136ccf10c4bfa214c2fe466616e35d644c27">1f3c136</a></td><td><code>Try first Parent.__call__ then fallback on self._element_constructor_.</code></td></tr></table>




---

archive/issue_comments_281101.json:
```json
{
    "body": "Changed commit from **[3e7eadd](https://github.com/sagemath/sagetrac-mirror/commit/3e7eaddafebea45d5a62454bba6e34f068b8febc)** to **[1f3c136](https://github.com/sagemath/sagetrac-mirror/commit/1f3c136ccf10c4bfa214c2fe466616e35d644c27)**",
    "created_at": "2017-07-11T03:30:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19554#issuecomment-281101",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[3e7eadd](https://github.com/sagemath/sagetrac-mirror/commit/3e7eaddafebea45d5a62454bba6e34f068b8febc)** to **[1f3c136](https://github.com/sagemath/sagetrac-mirror/commit/1f3c136ccf10c4bfa214c2fe466616e35d644c27)**



---

archive/issue_comments_281102.json:
```json
{
    "body": "Changed commit from **[1f3c136](https://github.com/sagemath/sagetrac-mirror/commit/1f3c136ccf10c4bfa214c2fe466616e35d644c27)** to **[1e3ace7](https://github.com/sagemath/sagetrac-mirror/commit/1e3ace7e6f2b004dccb83e6f726d7bc12eda8279)**",
    "created_at": "2017-07-11T03:32:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19554#issuecomment-281102",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[1f3c136](https://github.com/sagemath/sagetrac-mirror/commit/1f3c136ccf10c4bfa214c2fe466616e35d644c27)** to **[1e3ace7](https://github.com/sagemath/sagetrac-mirror/commit/1e3ace7e6f2b004dccb83e6f726d7bc12eda8279)**



---

archive/issue_comments_281103.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/06b4b10e7b0c9b33d4bc891b4f58d2e15386e6c1\">06b4b10</a></td><td><code>A workaround for #19554.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1e3ace7e6f2b004dccb83e6f726d7bc12eda8279\">1e3ace7</a></td><td><code>Try Parent.__call__ first and if that fails, try once more _element_constructor_.</code></td></tr></table>\n",
    "created_at": "2017-07-11T03:32:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19554#issuecomment-281103",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/06b4b10e7b0c9b33d4bc891b4f58d2e15386e6c1">06b4b10</a></td><td><code>A workaround for #19554.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1e3ace7e6f2b004dccb83e6f726d7bc12eda8279">1e3ace7</a></td><td><code>Try Parent.__call__ first and if that fails, try once more _element_constructor_.</code></td></tr></table>




---

archive/issue_comments_281104.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-We work around #19553 by first trying to see if `_element_constructor_` works.\n+We work around #19553 by trying directly to see if `_element_constructor_` works if `Parent.__call__` fails with a `TypeError`.\n``````\n",
    "created_at": "2017-07-11T03:33:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19554#issuecomment-281104",
    "user": "https://github.com/tscrim"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-We work around #19553 by first trying to see if `_element_constructor_` works.
+We work around #19553 by trying directly to see if `_element_constructor_` works if `Parent.__call__` fails with a `TypeError`.
``````




---

archive/issue_events_259219.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-07-11T03:33:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "milestone_number": null,
    "milestone_title": "sage-6.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19554#event-259219"
}
```



---

archive/issue_events_259220.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-07-11T03:33:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19554#event-259220"
}
```



---

archive/issue_events_259221.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-07-11T03:33:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19554#event-259221"
}
```



---

archive/issue_events_259222.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-07-11T03:33:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19554#event-259222"
}
```



---

archive/issue_comments_281105.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nLGTM",
    "created_at": "2019-03-27T14:14:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19554#issuecomment-281105",
    "user": "https://github.com/dkrenn"
}
```

<div id="comment:6" align="right">Comment 6</div>

LGTM



---

archive/issue_events_259223.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2019-03-27T14:14:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19554#event-259223"
}
```



---

archive/issue_events_259224.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2019-03-27T14:14:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19554#event-259224"
}
```



---

archive/issue_comments_281106.json:
```json
{
    "body": "Reviewer: **Daniel Krenn**",
    "created_at": "2019-03-27T14:14:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19554#issuecomment-281106",
    "user": "https://github.com/dkrenn"
}
```

Reviewer: **Daniel Krenn**



---

archive/issue_comments_281107.json:
```json
{
    "body": "Changed branch from **[public/sets/workaround_FiniteEnumeratedSet-19554](https://github.com/sagemath/sagetrac-mirror/tree/public/sets/workaround_FiniteEnumeratedSet-19554)** to **[1e3ace7](https://github.com/sagemath/sagetrac-mirror/commit/1e3ace7e6f2b004dccb83e6f726d7bc12eda8279)**",
    "created_at": "2019-03-29T12:36:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19554#issuecomment-281107",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/sets/workaround_FiniteEnumeratedSet-19554](https://github.com/sagemath/sagetrac-mirror/tree/public/sets/workaround_FiniteEnumeratedSet-19554)** to **[1e3ace7](https://github.com/sagemath/sagetrac-mirror/commit/1e3ace7e6f2b004dccb83e6f726d7bc12eda8279)**



---

archive/issue_events_259225.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-03-29T12:36:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19554#event-259225"
}
```



---

archive/issue_events_259226.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "9b6d57f617875b72bfbc337d3ac1386f6d6975d1",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-03-29T12:36:50Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19554#event-259226"
}
```



---

archive/issue_comments_281108.json:
```json
{
    "body": "Changed commit from **[1e3ace7](https://github.com/sagemath/sagetrac-mirror/commit/1e3ace7e6f2b004dccb83e6f726d7bc12eda8279)** to none",
    "created_at": "2019-03-29T17:20:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19554#issuecomment-281108",
    "user": "https://github.com/slel"
}
```

Changed commit from **[1e3ace7](https://github.com/sagemath/sagetrac-mirror/commit/1e3ace7e6f2b004dccb83e6f726d7bc12eda8279)** to none



---

archive/issue_events_259227.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-03-29T17:20:39Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "milestone_number": null,
    "milestone_title": "sage-8.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19554#event-259227"
}
```



---

archive/issue_events_259228.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-03-29T17:20:39Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19554",
    "milestone_number": null,
    "milestone_title": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19554#event-259228"
}
```
