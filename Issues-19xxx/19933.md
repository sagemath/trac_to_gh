# Issue 19933: MemoryInfo.rlimit_address_space() should return -1 for unlimited

archive/issues_019696.json:
```json
{
    "assignees": [],
    "body": "The code in `memory_info.py` is written with the assumption that `RLIM_INFINITY = -1`. This is true on Linux, but not on OS X.\n\nBranch/Commit: **[20d6d64](https://github.com/sagemath/sagetrac-mirror/commit/20d6d64a4b465225f8677ad3ecca453d544bdc64)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19933_\n\n",
    "closed_at": "2016-01-24T12:52:32Z",
    "created_at": "2016-01-21T12:26:41Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "MemoryInfo.rlimit_address_space() should return -1 for unlimited",
    "type": "issue",
    "updated_at": "2016-01-24T12:52:32Z",
    "url": "https://github.com/sagemath/sage/issues/19933",
    "user": "https://github.com/jdemeyer"
}
```
The code in `memory_info.py` is written with the assumption that `RLIM_INFINITY = -1`. This is true on Linux, but not on OS X.

Branch/Commit: **[20d6d64](https://github.com/sagemath/sagetrac-mirror/commit/20d6d64a4b465225f8677ad3ecca453d544bdc64)**

Reviewer: **Travis Scrimshaw**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/19933_





---

archive/issue_events_263920.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-01-21T12:26:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19933",
    "milestone_number": null,
    "milestone_title": "sage-7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19933#event-263920"
}
```



---

archive/issue_events_263921.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-01-21T12:26:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19933",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19933#event-263921"
}
```



---

archive/issue_events_263922.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-01-21T12:26:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19933",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19933#event-263922"
}
```



---

archive/issue_comments_287936.json:
```json
{
    "body": "Branch: **[u/jdemeyer/memoryinfo_rlimit_address_space___should_return__1_for_unlimited](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/memoryinfo_rlimit_address_space___should_return__1_for_unlimited)**",
    "created_at": "2016-01-21T12:59:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19933#issuecomment-287936",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/memoryinfo_rlimit_address_space___should_return__1_for_unlimited](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/memoryinfo_rlimit_address_space___should_return__1_for_unlimited)**



---

archive/issue_comments_287937.json:
```json
{
    "body": "Commit: **[20d6d64](https://github.com/sagemath/sagetrac-mirror/commit/20d6d64a4b465225f8677ad3ecca453d544bdc64)**",
    "created_at": "2016-01-21T13:25:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19933#issuecomment-287937",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[20d6d64](https://github.com/sagemath/sagetrac-mirror/commit/20d6d64a4b465225f8677ad3ecca453d544bdc64)**



---

archive/issue_comments_287938.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/20d6d64a4b465225f8677ad3ecca453d544bdc64\">20d6d64</a></td><td><code>Fix unlimited memory</code></td></tr></table>\n",
    "created_at": "2016-01-21T13:25:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19933#issuecomment-287938",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/20d6d64a4b465225f8677ad3ecca453d544bdc64">20d6d64</a></td><td><code>Fix unlimited memory</code></td></tr></table>




---

archive/issue_events_263923.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-01-21T13:25:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19933",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19933#event-263923"
}
```



---

archive/issue_comments_287939.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-The code in `memory_info.h` is written with the assumption that `RLIM_INFINITY = -1`. This is true on Linux, but not on OS X.\n+The code in `memory_info.py` is written with the assumption that `RLIM_INFINITY = -1`. This is true on Linux, but not on OS X.\n``````\n",
    "created_at": "2016-01-21T13:26:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19933#issuecomment-287939",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-The code in `memory_info.h` is written with the assumption that `RLIM_INFINITY = -1`. This is true on Linux, but not on OS X.
+The code in `memory_info.py` is written with the assumption that `RLIM_INFINITY = -1`. This is true on Linux, but not on OS X.
``````




---

archive/issue_events_263924.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-01-23T20:22:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19933",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19933#event-263924"
}
```



---

archive/issue_events_263925.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-01-23T20:22:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19933",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19933#event-263925"
}
```



---

archive/issue_comments_287940.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nLGTM.",
    "created_at": "2016-01-23T20:22:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19933#issuecomment-287940",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:4" align="right">Comment 4</div>

LGTM.



---

archive/issue_comments_287941.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2016-01-23T20:22:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19933#issuecomment-287941",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_263926.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-01-24T12:52:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19933",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19933#event-263926"
}
```



---

archive/issue_events_263927.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "065a2f1c8785f3a702ccb3438d57a454c02b6f77",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-01-24T12:52:32Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19933",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19933#event-263927"
}
```



---

archive/issue_comments_287942.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/memoryinfo_rlimit_address_space___should_return__1_for_unlimited](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/memoryinfo_rlimit_address_space___should_return__1_for_unlimited)** to **[20d6d64](https://github.com/sagemath/sagetrac-mirror/commit/20d6d64a4b465225f8677ad3ecca453d544bdc64)**",
    "created_at": "2016-01-24T12:52:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19933",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19933#issuecomment-287942",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/memoryinfo_rlimit_address_space___should_return__1_for_unlimited](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/memoryinfo_rlimit_address_space___should_return__1_for_unlimited)** to **[20d6d64](https://github.com/sagemath/sagetrac-mirror/commit/20d6d64a4b465225f8677ad3ecca453d544bdc64)**
