# Issue 19653: New decoders for Generalized Reed-Solomon codes

archive/issues_019416.json:
```json
{
    "body": "This ticket introduces four new different decoders for GRS codes, namely:\n\n- `GRSBerlekampWelchDecoder`,\n- `GRSGaoDecoder`,\n- `GRSKeyEquationSyndromeDecoder` and\n- `GRSErrorErasureDecoder`.\n\nIt requires the new structure for GRS codes introduced in ticket #18928\n\nCC:  @johanrosenkilde\n\nBranch/Commit: f3b4b11582fd4c2e976d2e0450d0e8699be0272e\n\nReviewer: Julien Lavauzelle\n\nAuthor: David Lucas\n\nDependencies: #18928, #19897\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/19653\n\n",
    "closed_at": "2016-03-02T08:27:44Z",
    "created_at": "2015-12-01T12:01:03Z",
    "labels": [
        "component: coding theory"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-7.1",
    "title": "New decoders for Generalized Reed-Solomon codes",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/19653",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```
This ticket introduces four new different decoders for GRS codes, namely:

- `GRSBerlekampWelchDecoder`,
- `GRSGaoDecoder`,
- `GRSKeyEquationSyndromeDecoder` and
- `GRSErrorErasureDecoder`.

It requires the new structure for GRS codes introduced in ticket #18928

CC:  @johanrosenkilde

Branch/Commit: f3b4b11582fd4c2e976d2e0450d0e8699be0272e

Reviewer: Julien Lavauzelle

Author: David Lucas

Dependencies: #18928, #19897

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/19653





---

archive/issue_comments_353309.json:
```json
{
    "body": "Changing branch from \"\" to \"u/dlucas/grs_decoders\"",
    "created_at": "2015-12-01T12:02:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353309",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

Changing branch from "" to "u/dlucas/grs_decoders"



---

archive/issue_comments_353310.json:
```json
{
    "body": "<a id='comment:2'></a>\nI pushed the changes, it's now open for review.\nPlease note that I branched #18928 to work on this ticket.\n\n---\nNew commits:\n|                                                                                                                                          |                                                                            |\n|------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------|\n|[d854eb8](http://git.sagemath.org/sage.git/commit/?id=d854eb899cbe3affffe9e7a17a07def3fbcb37b6)|`New Generalized Reed Solomon code object in Sage, coming with two encoders`|\n|[0645f58](http://git.sagemath.org/sage.git/commit/?id=0645f589948612023c2de6d04138eebf7adb3df5)|`Updated to 6.9beta6 and fixed conflict`|\n|[dc470be](http://git.sagemath.org/sage.git/commit/?id=dc470becd64eeba63d64f328de5152c7aed83403)|`Overwrote some methods from linear_code`|\n|[f8c2d7a](http://git.sagemath.org/sage.git/commit/?id=f8c2d7afe506f08947d89ec77c2b189e3b7265e7)|`Updated to 6.10beta0 and resolved conflicts`|\n|[fa4dd71](http://git.sagemath.org/sage.git/commit/?id=fa4dd71cefb1c124739a5b06b93e004c5cf451e6)|`Update to latest beta`|\n|[1c1f182](http://git.sagemath.org/sage.git/commit/?id=1c1f1820e21b8c359279886154ac835dc96c563f)|`Merged with latest beta version and fixed conflicts`|\n|[74cc281](http://git.sagemath.org/sage.git/commit/?id=74cc281ef65d2aed2fca11dd41d6ac268ad52880)|`Four new decoders for GRS codes`|",
    "created_at": "2015-12-01T12:04:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353310",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

<a id='comment:2'></a>
I pushed the changes, it's now open for review.
Please note that I branched #18928 to work on this ticket.

---
New commits:
|                                                                                                                                          |                                                                            |
|------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------|
|[d854eb8](http://git.sagemath.org/sage.git/commit/?id=d854eb899cbe3affffe9e7a17a07def3fbcb37b6)|`New Generalized Reed Solomon code object in Sage, coming with two encoders`|
|[0645f58](http://git.sagemath.org/sage.git/commit/?id=0645f589948612023c2de6d04138eebf7adb3df5)|`Updated to 6.9beta6 and fixed conflict`|
|[dc470be](http://git.sagemath.org/sage.git/commit/?id=dc470becd64eeba63d64f328de5152c7aed83403)|`Overwrote some methods from linear_code`|
|[f8c2d7a](http://git.sagemath.org/sage.git/commit/?id=f8c2d7afe506f08947d89ec77c2b189e3b7265e7)|`Updated to 6.10beta0 and resolved conflicts`|
|[fa4dd71](http://git.sagemath.org/sage.git/commit/?id=fa4dd71cefb1c124739a5b06b93e004c5cf451e6)|`Update to latest beta`|
|[1c1f182](http://git.sagemath.org/sage.git/commit/?id=1c1f1820e21b8c359279886154ac835dc96c563f)|`Merged with latest beta version and fixed conflicts`|
|[74cc281](http://git.sagemath.org/sage.git/commit/?id=74cc281ef65d2aed2fca11dd41d6ac268ad52880)|`Four new decoders for GRS codes`|



---

archive/issue_comments_353311.json:
```json
{
    "body": "Changing author from \"\" to \"David Lucas\"",
    "created_at": "2015-12-01T12:04:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353311",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

Changing author from "" to "David Lucas"



---

archive/issue_comments_353312.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2015-12-01T12:04:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353312",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_353313.json:
```json
{
    "body": "Changing commit from \"\" to \"74cc281ef65d2aed2fca11dd41d6ac268ad52880\"",
    "created_at": "2015-12-01T12:04:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353313",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

Changing commit from "" to "74cc281ef65d2aed2fca11dd41d6ac268ad52880"



---

archive/issue_comments_353314.json:
```json
{
    "body": "Changing commit from \"74cc281ef65d2aed2fca11dd41d6ac268ad52880\" to \"b03d7b5ae8f6e725a148b4fe7bcce99043e240db\"",
    "created_at": "2015-12-03T10:45:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353314",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "74cc281ef65d2aed2fca11dd41d6ac268ad52880" to "b03d7b5ae8f6e725a148b4fe7bcce99043e240db"



---

archive/issue_comments_353315.json:
```json
{
    "body": "<a id='comment:3'></a>\nBranch pushed to git repo; I updated commit sha1. Last 10 new commits:\n|                                                                                                                                          |                                         |\n|------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------|\n|[8733405](http://git.sagemath.org/sage.git/commit/?id=8733405ab5be96dff84e72543121e79ac77e6a66)|`Some improved doc-strings and doc-tests`|\n|[229b5cf](http://git.sagemath.org/sage.git/commit/?id=229b5cf5786019a256e5c6a442572eff870f0225)|`Some code beautification`|\n|[fe7e9de](http://git.sagemath.org/sage.git/commit/?id=fe7e9debca33f40e2e9208d002d15e58ac781ff2)|`Sanity-checks on input for GRSEvaluationPolynomialEncoder`|\n|[6e7013b](http://git.sagemath.org/sage.git/commit/?id=6e7013b8666b9490ea14e349bdbd96b586f048ff)|`Small doc-string improvements in linear_code and encoder`|\n|[0e82848](http://git.sagemath.org/sage.git/commit/?id=0e828483e4674799ff0f8d6a328ccd854aef8cf1)|`More small docstring improvements`|\n|[91e2e3b](http://git.sagemath.org/sage.git/commit/?id=91e2e3b20e0a903c57073ecd30b7cc73e25012b5)|`Doc-string typesetting fix`|\n|[eabe7e0](http://git.sagemath.org/sage.git/commit/?id=eabe7e0c1bc48564540602846a77ee3744c093f4)|`Reworked coercion of evaluation pts and col mults and refined tests.`|\n|[71070de](http://git.sagemath.org/sage.git/commit/?id=71070de78200916218a7dce5d7582941c4040b31)|`Merged with 18928 and fixed conflicts`|\n|[9f4fb84](http://git.sagemath.org/sage.git/commit/?id=9f4fb841fff4b01fc6ff98f321f433e87a7ea965)|`Improved docstrings and doctests`|\n|[b03d7b5](http://git.sagemath.org/sage.git/commit/?id=b03d7b5ae8f6e725a148b4fe7bcce99043e240db)|`Extra cleaning in documentation`|",
    "created_at": "2015-12-03T10:45:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353315",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:3'></a>
Branch pushed to git repo; I updated commit sha1. Last 10 new commits:
|                                                                                                                                          |                                         |
|------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------|
|[8733405](http://git.sagemath.org/sage.git/commit/?id=8733405ab5be96dff84e72543121e79ac77e6a66)|`Some improved doc-strings and doc-tests`|
|[229b5cf](http://git.sagemath.org/sage.git/commit/?id=229b5cf5786019a256e5c6a442572eff870f0225)|`Some code beautification`|
|[fe7e9de](http://git.sagemath.org/sage.git/commit/?id=fe7e9debca33f40e2e9208d002d15e58ac781ff2)|`Sanity-checks on input for GRSEvaluationPolynomialEncoder`|
|[6e7013b](http://git.sagemath.org/sage.git/commit/?id=6e7013b8666b9490ea14e349bdbd96b586f048ff)|`Small doc-string improvements in linear_code and encoder`|
|[0e82848](http://git.sagemath.org/sage.git/commit/?id=0e828483e4674799ff0f8d6a328ccd854aef8cf1)|`More small docstring improvements`|
|[91e2e3b](http://git.sagemath.org/sage.git/commit/?id=91e2e3b20e0a903c57073ecd30b7cc73e25012b5)|`Doc-string typesetting fix`|
|[eabe7e0](http://git.sagemath.org/sage.git/commit/?id=eabe7e0c1bc48564540602846a77ee3744c093f4)|`Reworked coercion of evaluation pts and col mults and refined tests.`|
|[71070de](http://git.sagemath.org/sage.git/commit/?id=71070de78200916218a7dce5d7582941c4040b31)|`Merged with 18928 and fixed conflicts`|
|[9f4fb84](http://git.sagemath.org/sage.git/commit/?id=9f4fb841fff4b01fc6ff98f321f433e87a7ea965)|`Improved docstrings and doctests`|
|[b03d7b5](http://git.sagemath.org/sage.git/commit/?id=b03d7b5ae8f6e725a148b4fe7bcce99043e240db)|`Extra cleaning in documentation`|



---

archive/issue_comments_353316.json:
```json
{
    "body": "<a id='comment:4'></a>\nI merged in #18928 latest version and did some fixes and improvements to the documentation.",
    "created_at": "2015-12-03T10:46:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353316",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

<a id='comment:4'></a>
I merged in #18928 latest version and did some fixes and improvements to the documentation.



---

archive/issue_comments_353317.json:
```json
{
    "body": "<a id='comment:5'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                          |                                          |\n|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------|\n|[4e1a184](http://git.sagemath.org/sage.git/commit/?id=4e1a1845fdacdd3f9fe9e9adacb7c92dcc81b6cd)|`Merge branch 'develop' into grs_decoders`|",
    "created_at": "2015-12-17T13:40:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353317",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:5'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                          |                                          |
|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------|
|[4e1a184](http://git.sagemath.org/sage.git/commit/?id=4e1a1845fdacdd3f9fe9e9adacb7c92dcc81b6cd)|`Merge branch 'develop' into grs_decoders`|



---

archive/issue_comments_353318.json:
```json
{
    "body": "Changing commit from \"b03d7b5ae8f6e725a148b4fe7bcce99043e240db\" to \"4e1a1845fdacdd3f9fe9e9adacb7c92dcc81b6cd\"",
    "created_at": "2015-12-17T13:40:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353318",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "b03d7b5ae8f6e725a148b4fe7bcce99043e240db" to "4e1a1845fdacdd3f9fe9e9adacb7c92dcc81b6cd"



---

archive/issue_comments_353319.json:
```json
{
    "body": "<a id='comment:6'></a>\nUpdated to latest beta, still open for review.",
    "created_at": "2015-12-17T13:40:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353319",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

<a id='comment:6'></a>
Updated to latest beta, still open for review.



---

archive/issue_comments_353320.json:
```json
{
    "body": "<a id='comment:7'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                          |                                                                                        |\n|------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------|\n|[ffed948](http://git.sagemath.org/sage.git/commit/?id=ffed9486f0d2e5bf3d1a86dff8dfa9a9e33ec9dc)|`Merge branch 'u/dlucas/grs_decoders' of git://trac.sagemath.org/sage into grs_decoders`|",
    "created_at": "2016-01-13T09:04:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353320",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:7'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                          |                                                                                        |
|------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------|
|[ffed948](http://git.sagemath.org/sage.git/commit/?id=ffed9486f0d2e5bf3d1a86dff8dfa9a9e33ec9dc)|`Merge branch 'u/dlucas/grs_decoders' of git://trac.sagemath.org/sage into grs_decoders`|



---

archive/issue_comments_353321.json:
```json
{
    "body": "Changing commit from \"4e1a1845fdacdd3f9fe9e9adacb7c92dcc81b6cd\" to \"ffed9486f0d2e5bf3d1a86dff8dfa9a9e33ec9dc\"",
    "created_at": "2016-01-13T09:04:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353321",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "4e1a1845fdacdd3f9fe9e9adacb7c92dcc81b6cd" to "ffed9486f0d2e5bf3d1a86dff8dfa9a9e33ec9dc"



---

archive/issue_comments_353322.json:
```json
{
    "body": "<a id='comment:8'></a>\nUpdated to latest beta, still open for review.",
    "created_at": "2016-01-13T09:05:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353322",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

<a id='comment:8'></a>
Updated to latest beta, still open for review.



---

archive/issue_comments_353323.json:
```json
{
    "body": "Changing commit from \"ffed9486f0d2e5bf3d1a86dff8dfa9a9e33ec9dc\" to \"37bb7ac8775f51d774cc9edaeeb0ba457c7f2546\"",
    "created_at": "2016-01-15T16:07:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353323",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "ffed9486f0d2e5bf3d1a86dff8dfa9a9e33ec9dc" to "37bb7ac8775f51d774cc9edaeeb0ba457c7f2546"



---

archive/issue_comments_353324.json:
```json
{
    "body": "<a id='comment:9'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                          |                       |\n|------------------------------------------------------------------------------------------------------------------------------------------|-----------------------|\n|[616219c](http://git.sagemath.org/sage.git/commit/?id=616219c09ebaba8ccf1b45804bd6784617109d54)|`Update to latest beta`|\n|[baa9f20](http://git.sagemath.org/sage.git/commit/?id=baa9f20fa041ece4f2c3d5eb3f76af74334ff2ce)|`First version of the tutorial`|\n|[cb65314](http://git.sagemath.org/sage.git/commit/?id=cb653141eced7deabf3cecffe3ba28bb85498bdc)|`Update to latest beta`|\n|[2057fee](http://git.sagemath.org/sage.git/commit/?id=2057fee08a6657d419272ee0a2ecbb1b73d95d0b)|`Small fixes and improvements`|\n|[77f0629](http://git.sagemath.org/sage.git/commit/?id=77f06294a0005cdbbd7006e2e75281be5a1da352)|`Merged #19897`|\n|[37bb7ac](http://git.sagemath.org/sage.git/commit/?id=37bb7ac8775f51d774cc9edaeeb0ba457c7f2546)|`Updated introductory thematic tutorial with a paragraph on decoders and message spaces`|",
    "created_at": "2016-01-15T16:07:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353324",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:9'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                          |                       |
|------------------------------------------------------------------------------------------------------------------------------------------|-----------------------|
|[616219c](http://git.sagemath.org/sage.git/commit/?id=616219c09ebaba8ccf1b45804bd6784617109d54)|`Update to latest beta`|
|[baa9f20](http://git.sagemath.org/sage.git/commit/?id=baa9f20fa041ece4f2c3d5eb3f76af74334ff2ce)|`First version of the tutorial`|
|[cb65314](http://git.sagemath.org/sage.git/commit/?id=cb653141eced7deabf3cecffe3ba28bb85498bdc)|`Update to latest beta`|
|[2057fee](http://git.sagemath.org/sage.git/commit/?id=2057fee08a6657d419272ee0a2ecbb1b73d95d0b)|`Small fixes and improvements`|
|[77f0629](http://git.sagemath.org/sage.git/commit/?id=77f06294a0005cdbbd7006e2e75281be5a1da352)|`Merged #19897`|
|[37bb7ac](http://git.sagemath.org/sage.git/commit/?id=37bb7ac8775f51d774cc9edaeeb0ba457c7f2546)|`Updated introductory thematic tutorial with a paragraph on decoders and message spaces`|



---

archive/issue_comments_353325.json:
```json
{
    "body": "Changing dependencies from \"#18928\" to \"#18928, #19897\"",
    "created_at": "2016-01-15T16:09:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353325",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

Changing dependencies from "#18928" to "#18928, #19897"



---

archive/issue_comments_353326.json:
```json
{
    "body": "<a id='comment:10'></a>\nI merged #19897 in, and updated the tutorial.\n\nIt's still open for review.",
    "created_at": "2016-01-15T16:09:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353326",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

<a id='comment:10'></a>
I merged #19897 in, and updated the tutorial.

It's still open for review.



---

archive/issue_comments_353327.json:
```json
{
    "body": "<a id='comment:11'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                          |                                          |\n|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------|\n|[1220403](http://git.sagemath.org/sage.git/commit/?id=12204034fadec3f7fb3ac7cb8ed65a027846e1de)|`Merge branch 'develop' into grs_decoders`|\n|[4340598](http://git.sagemath.org/sage.git/commit/?id=4340598e5ae5e932abb87389764ccb6e06561c12)|`Removed deprecated import`|",
    "created_at": "2016-02-03T13:06:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353327",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:11'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                          |                                          |
|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------|
|[1220403](http://git.sagemath.org/sage.git/commit/?id=12204034fadec3f7fb3ac7cb8ed65a027846e1de)|`Merge branch 'develop' into grs_decoders`|
|[4340598](http://git.sagemath.org/sage.git/commit/?id=4340598e5ae5e932abb87389764ccb6e06561c12)|`Removed deprecated import`|



---

archive/issue_comments_353328.json:
```json
{
    "body": "Changing commit from \"37bb7ac8775f51d774cc9edaeeb0ba457c7f2546\" to \"4340598e5ae5e932abb87389764ccb6e06561c12\"",
    "created_at": "2016-02-03T13:06:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353328",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "37bb7ac8775f51d774cc9edaeeb0ba457c7f2546" to "4340598e5ae5e932abb87389764ccb6e06561c12"



---

archive/issue_comments_353329.json:
```json
{
    "body": "<a id='comment:12'></a>\nI updated this ticket to latest beta and removed a deprecated `import` statement, so broken doctests won't be broken anymore. \n\nThis is still open for review.",
    "created_at": "2016-02-03T13:08:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353329",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

<a id='comment:12'></a>
I updated this ticket to latest beta and removed a deprecated `import` statement, so broken doctests won't be broken anymore. 

This is still open for review.



---

archive/issue_comments_353330.json:
```json
{
    "body": "<a id='comment:13'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                          |                                        |\n|------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------|\n|[b840682](http://git.sagemath.org/sage.git/commit/?id=b8406820e1300efdd3ef95d8e5f326c28878a568)|`Updated to 7.1beta3 and fixed conflict`|",
    "created_at": "2016-02-16T09:32:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353330",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:13'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                          |                                        |
|------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------|
|[b840682](http://git.sagemath.org/sage.git/commit/?id=b8406820e1300efdd3ef95d8e5f326c28878a568)|`Updated to 7.1beta3 and fixed conflict`|



---

archive/issue_comments_353331.json:
```json
{
    "body": "Changing commit from \"4340598e5ae5e932abb87389764ccb6e06561c12\" to \"b8406820e1300efdd3ef95d8e5f326c28878a568\"",
    "created_at": "2016-02-16T09:32:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353331",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "4340598e5ae5e932abb87389764ccb6e06561c12" to "b8406820e1300efdd3ef95d8e5f326c28878a568"



---

archive/issue_events_053983.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/dlucas",
    "created_at": "2016-02-16T09:34:09Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "milestone": "sage-7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/19653#event-53983"
}
```



---

archive/issue_comments_353332.json:
```json
{
    "body": "<a id='comment:15'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                          |                        |\n|------------------------------------------------------------------------------------------------------------------------------------------|------------------------|\n|[d70d6aa](http://git.sagemath.org/sage.git/commit/?id=d70d6aa219a0472d1e254a23b6e0427f5cc7e1b2)|`Removed __ne__ methods`|",
    "created_at": "2016-02-16T13:53:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353332",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:15'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                          |                        |
|------------------------------------------------------------------------------------------------------------------------------------------|------------------------|
|[d70d6aa](http://git.sagemath.org/sage.git/commit/?id=d70d6aa219a0472d1e254a23b6e0427f5cc7e1b2)|`Removed __ne__ methods`|



---

archive/issue_comments_353333.json:
```json
{
    "body": "Changing commit from \"b8406820e1300efdd3ef95d8e5f326c28878a568\" to \"d70d6aa219a0472d1e254a23b6e0427f5cc7e1b2\"",
    "created_at": "2016-02-16T13:53:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353333",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "b8406820e1300efdd3ef95d8e5f326c28878a568" to "d70d6aa219a0472d1e254a23b6e0427f5cc7e1b2"



---

archive/issue_comments_353334.json:
```json
{
    "body": "<a id='comment:16'></a>\nI removed `__ne__` method in every decoder as a generic one was implemented in another ticket, so these ones are useless.\n\nNow, for the broken doctests, I'm experiencing a weird problem: while trying to build a [10, 5, 6]-GRS code (lines 1549, 1591, 1631 and 1669) the doctesting framework (`sage -t grs.py`) actually builds a [9, 5, 5]-GRS code. \n\nWhich is quite weird, because if I copy these lines in my Sage terminal, everything works perfectly...\n\nEverything else is still on the clear, so I consider this suitable for review. I'm of course working on the above.\n\nDavid",
    "created_at": "2016-02-16T13:58:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353334",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

<a id='comment:16'></a>
I removed `__ne__` method in every decoder as a generic one was implemented in another ticket, so these ones are useless.

Now, for the broken doctests, I'm experiencing a weird problem: while trying to build a [10, 5, 6]-GRS code (lines 1549, 1591, 1631 and 1669) the doctesting framework (`sage -t grs.py`) actually builds a [9, 5, 5]-GRS code. 

Which is quite weird, because if I copy these lines in my Sage terminal, everything works perfectly...

Everything else is still on the clear, so I consider this suitable for review. I'm of course working on the above.

David



---

archive/issue_comments_353335.json:
```json
{
    "body": "<a id='comment:17'></a>\nHi,\n\nWell, I don't know key equation decoding that much, so I didn't review all the ticket yet. But I can post my first comments so that you start modifying the code while I go further on key equation.\n\ncoding_theory.srt\n\n* l.125: created --> create\n* l.146: to build the [12, 6] GRS code --> to build a ...\n* in the example which follows, you give a [6, 3] GRS code (instead of [12, 6]). As your example is quite big, I think you should put the [6, 3] everywhere.\n* l.253: it seems that your decoder decodes 2 errors, whereas the [6,3] GRS minimum distance is 4. It means the standard decoder you use is not a half minimum distance one, which is quite inefficient for such a structured code. Two possibilities: either you keep this standard decoder, or you change it to Gao decoder. If you choose to change, do not forget to modify the tutorial.\n* l.337: put quotes around `EvaluationVector` and `C`\n* l.352: match --> matches\n* l.375: \"The default encoder for a code is always one with vector behaviour, so when we call...\" To me, this sentence is strange. Maybe \"always uses vectors\" or \"always has a standard vector space as message space\".\n* l.380: whose length are --> is\n* l.390: remove the \"then\"\n* l.364, 442, 446, 550: why did you write \"#random\" ?\n* l.547: \"over which has 1\" sounds weird.\n* l.567: \"For instance, we did not mention how Sage manages bounds on codes.\" Does it manage them ?\n* l.585: \"to create you own codes\" --> your\n\n\n\n\ndecoders_catalog.py\n* l.16: broken link with `grs.GRSKeyEquationSyndromeDecoder <sage.coding.grs.GRSKeyEquationDecoder>`\n\n\ngrs.py\n* l. 1083: I dislike the function naming: `precompute` is too general. I also think your documentation is not precise enough : there are many monic polynomials vanishing on the evaluation points; your function returns the only one whose zeroes are exactly the evaluation points.\n* l. 1107: `G = 1` --> `G = PolRing.one()`\n* l. 1113: Your documentation is not very clear:\n    - \"Returns the greatest common divisor of a and b.\" --> your function doesn't do that\n    - \"where d is the dimension of self.code() and  k its dimension.\" --> I think you meant the code length for d.\n    - OUTPUT: a tuple of polynomials (r, s) where r is to the xgcd of a and b and s is the Bezout coefficient of a.\n      ---> as it is written, it is not an *extended* gcd algorithm, as you don't perform the \"backward step\". It doesn't either return a gcd, but one of its multiples. And I think you should mention the Euclidean algorithm somewhere. Maybe something like: \"the algorithm performs the classical Euclidean algorithm until a remainder has degree less than (n+k)/2, and return (r,s) such that in the last step, we have r = a*s + b*t\". \n* l. 1280: Don't you need to check the `input_space` also (as in GRSGaoDecoder and GRSBerlekampWelchDecoder)?\n* l. 1383: if `e` is the number of erasures and `d` the minimum distance, then the decoding_radius is `(d-1-e)//2`.\n* l. 1372 and after: you can remove backslashes when you wrap a line \n* l.1470: `(code.base_field())(0)` --> `code.base_field().zero()`. Besides, your comment: `ValueError(\"Impossible to decode a GRS code which contains 0 amongst its evaluation points\")` is a bit wrong. I would prefer: \"This decoder can't be initialized with a GRS code whose evaluation points contains 0\"\n* l. 1525 (partial xgcd): same comments as in the partial_xgcd of Gao decoder.\n\n\nJulien",
    "created_at": "2016-02-18T17:24:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353335",
    "user": "https://github.com/jlavauzelle"
}
```

<a id='comment:17'></a>
Hi,

Well, I don't know key equation decoding that much, so I didn't review all the ticket yet. But I can post my first comments so that you start modifying the code while I go further on key equation.

coding_theory.srt

* l.125: created --> create
* l.146: to build the [12, 6] GRS code --> to build a ...
* in the example which follows, you give a [6, 3] GRS code (instead of [12, 6]). As your example is quite big, I think you should put the [6, 3] everywhere.
* l.253: it seems that your decoder decodes 2 errors, whereas the [6,3] GRS minimum distance is 4. It means the standard decoder you use is not a half minimum distance one, which is quite inefficient for such a structured code. Two possibilities: either you keep this standard decoder, or you change it to Gao decoder. If you choose to change, do not forget to modify the tutorial.
* l.337: put quotes around `EvaluationVector` and `C`
* l.352: match --> matches
* l.375: "The default encoder for a code is always one with vector behaviour, so when we call..." To me, this sentence is strange. Maybe "always uses vectors" or "always has a standard vector space as message space".
* l.380: whose length are --> is
* l.390: remove the "then"
* l.364, 442, 446, 550: why did you write "#random" ?
* l.547: "over which has 1" sounds weird.
* l.567: "For instance, we did not mention how Sage manages bounds on codes." Does it manage them ?
* l.585: "to create you own codes" --> your




decoders_catalog.py
* l.16: broken link with `grs.GRSKeyEquationSyndromeDecoder <sage.coding.grs.GRSKeyEquationDecoder>`


grs.py
* l. 1083: I dislike the function naming: `precompute` is too general. I also think your documentation is not precise enough : there are many monic polynomials vanishing on the evaluation points; your function returns the only one whose zeroes are exactly the evaluation points.
* l. 1107: `G = 1` --> `G = PolRing.one()`
* l. 1113: Your documentation is not very clear:
    - "Returns the greatest common divisor of a and b." --> your function doesn't do that
    - "where d is the dimension of self.code() and  k its dimension." --> I think you meant the code length for d.
    - OUTPUT: a tuple of polynomials (r, s) where r is to the xgcd of a and b and s is the Bezout coefficient of a.
      ---> as it is written, it is not an *extended* gcd algorithm, as you don't perform the "backward step". It doesn't either return a gcd, but one of its multiples. And I think you should mention the Euclidean algorithm somewhere. Maybe something like: "the algorithm performs the classical Euclidean algorithm until a remainder has degree less than (n+k)/2, and return (r,s) such that in the last step, we have r = a*s + b*t". 
* l. 1280: Don't you need to check the `input_space` also (as in GRSGaoDecoder and GRSBerlekampWelchDecoder)?
* l. 1383: if `e` is the number of erasures and `d` the minimum distance, then the decoding_radius is `(d-1-e)//2`.
* l. 1372 and after: you can remove backslashes when you wrap a line 
* l.1470: `(code.base_field())(0)` --> `code.base_field().zero()`. Besides, your comment: `ValueError("Impossible to decode a GRS code which contains 0 amongst its evaluation points")` is a bit wrong. I would prefer: "This decoder can't be initialized with a GRS code whose evaluation points contains 0"
* l. 1525 (partial xgcd): same comments as in the partial_xgcd of Gao decoder.


Julien



---

archive/issue_comments_353336.json:
```json
{
    "body": "<a id='comment:18'></a>\nAwesome remarks Julien.\n\nJust a small clarification\n\n> * l. 1083: I dislike the function naming: `precompute` is too general. I also think your documentation is not precise enough : there are many monic polynomials vanishing on the evaluation points; your function returns the only one whose zeroes are exactly the evaluation points.\n\n\nTo be clear: it returns the *minimal-degree* polynomial that vanishes at all the evaluation points (in the algebraic closure, that's equivalent to what you say, but perhaps not everyone would think like that).",
    "created_at": "2016-02-18T17:52:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353336",
    "user": "https://github.com/johanrosenkilde"
}
```

<a id='comment:18'></a>
Awesome remarks Julien.

Just a small clarification

> * l. 1083: I dislike the function naming: `precompute` is too general. I also think your documentation is not precise enough : there are many monic polynomials vanishing on the evaluation points; your function returns the only one whose zeroes are exactly the evaluation points.


To be clear: it returns the *minimal-degree* polynomial that vanishes at all the evaluation points (in the algebraic closure, that's equivalent to what you say, but perhaps not everyone would think like that).



---

archive/issue_comments_353337.json:
```json
{
    "body": "<a id='comment:19'></a>\nHello,\n\nThanks a lot for this thorough review work!\n\nA general remark: at the time I wrote my thematic tutorial, there was no decoder in Sage but `syndrome` and `nearest_neighbor`, which both are quite slow - hence the small codes.\nNow that we have fast decoders, I prefer to default to bigger codes than [6,3]-GRS in examples, just to illustrate that we are fast over not-so-small codes.\n\n## `grs.py`\n\n- I changed default decoder, which is now `Gao`.\n- I rewrote the documentation of `precompute`, which is now called `_polynomial_vanishing_at_alphas`. Maybe not the best name ever (`alphas`?) but it's better than `precompute`. By the way, it's now a private method, which I think is quite better.\n- I rewrote both `partial_xgcd` as you advised. I also made these private methods, which was actually written in the doc but not done in practice...\n- No need to check the `input_space` in `__eq__` as it comes directly from `code`. If you check the input of all my decoders, you'll see the user only provides `code`, and the input space is extracted from `code`. As the user has no direct way to influence this, it's not necessary to test it in equality checks.\n- Fixed my mistake in error-erasure decoder's `decoding_radius`.\n- I removed the hideous, useless backslashes which were lurking all over the place.\n- I rewrote the error message related to the construction of a key equation syndrome decoder over a GRS code including a 0 in its evaluation points. I also introduced a test to illustrate this check. \n\nBy the way, Roth's book explains well the key equation decoding (that's where I learnt it). Chp 6, pp 183-196, especially 6.3.1 and 6.3.2 related to key equations, 6.4 for the Euclidean algorithm, 6.5 for Forney's formul\u00e6, 6.6 for a summary of the algorithm.\n\n## `coding_theory.rst`\n\n- I fixed all the typos you notice.\n- I \"upgraded\" all my [6,3] codes to [12, 6] codes. Twice better! \n- With these new codes, d/2 now equals to 3, I updated the number of errors accordingly. \n- I rewrote the weird sentences you noticed.\n\nTo answer your questions:\n\n> l.567: \"For instance, we did not mention how Sage manages bounds on codes.\" Does it manage them ? \n\n\nIn a way. There's no central mechanism to take care of bounds, register the best upper- and lower-bound computed so far while working on a code. \nHowever, some bounds are implemented, and I hid them behind `codes.bounds`.\nSo, Sage can somehow compute a few bounds on minimum distance, best parameters for codes and so on. \n\n> l.364, 442, 446, 550: why did you write \"#random\" ? \n\n\nBecause I don't control the output. E.g., l507 a random element of `C` is picked, and l550 (example flagged `#random`) I use my channel to create *randomly* some errors/erasures in it.\n\nNow, there's a way to control this randomness: \n1) Use \"hardcoded\" vectors.\n2) Write `set_random_seed(42)` before any random method. In that way, we guarantee the error pattern will always be the same, and thus can write the output of our method. See `channel_construction.py`, ll 210-215 for an example.\n\nI chose to *not* use this, as it's an introductory tutorial, designed for beginners. I wanted to illustrate the ability of channels to add random errors in words, and of decoders to correct these random errors. Plus, for beginners, finding a statement such as `set_random_seed` might be confusing imho.\n\nSo I more or less sacrificed doctest purity on the altar of understandability. Which is not a big deal as these methods are properly doctested in their respective files.\n\nDavid",
    "created_at": "2016-02-19T10:31:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353337",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

<a id='comment:19'></a>
Hello,

Thanks a lot for this thorough review work!

A general remark: at the time I wrote my thematic tutorial, there was no decoder in Sage but `syndrome` and `nearest_neighbor`, which both are quite slow - hence the small codes.
Now that we have fast decoders, I prefer to default to bigger codes than [6,3]-GRS in examples, just to illustrate that we are fast over not-so-small codes.

## `grs.py`

- I changed default decoder, which is now `Gao`.
- I rewrote the documentation of `precompute`, which is now called `_polynomial_vanishing_at_alphas`. Maybe not the best name ever (`alphas`?) but it's better than `precompute`. By the way, it's now a private method, which I think is quite better.
- I rewrote both `partial_xgcd` as you advised. I also made these private methods, which was actually written in the doc but not done in practice...
- No need to check the `input_space` in `__eq__` as it comes directly from `code`. If you check the input of all my decoders, you'll see the user only provides `code`, and the input space is extracted from `code`. As the user has no direct way to influence this, it's not necessary to test it in equality checks.
- Fixed my mistake in error-erasure decoder's `decoding_radius`.
- I removed the hideous, useless backslashes which were lurking all over the place.
- I rewrote the error message related to the construction of a key equation syndrome decoder over a GRS code including a 0 in its evaluation points. I also introduced a test to illustrate this check. 

By the way, Roth's book explains well the key equation decoding (that's where I learnt it). Chp 6, pp 183-196, especially 6.3.1 and 6.3.2 related to key equations, 6.4 for the Euclidean algorithm, 6.5 for Forney's formulæ, 6.6 for a summary of the algorithm.

## `coding_theory.rst`

- I fixed all the typos you notice.
- I "upgraded" all my [6,3] codes to [12, 6] codes. Twice better! 
- With these new codes, d/2 now equals to 3, I updated the number of errors accordingly. 
- I rewrote the weird sentences you noticed.

To answer your questions:

> l.567: "For instance, we did not mention how Sage manages bounds on codes." Does it manage them ? 


In a way. There's no central mechanism to take care of bounds, register the best upper- and lower-bound computed so far while working on a code. 
However, some bounds are implemented, and I hid them behind `codes.bounds`.
So, Sage can somehow compute a few bounds on minimum distance, best parameters for codes and so on. 

> l.364, 442, 446, 550: why did you write "#random" ? 


Because I don't control the output. E.g., l507 a random element of `C` is picked, and l550 (example flagged `#random`) I use my channel to create *randomly* some errors/erasures in it.

Now, there's a way to control this randomness: 
1) Use "hardcoded" vectors.
2) Write `set_random_seed(42)` before any random method. In that way, we guarantee the error pattern will always be the same, and thus can write the output of our method. See `channel_construction.py`, ll 210-215 for an example.

I chose to *not* use this, as it's an introductory tutorial, designed for beginners. I wanted to illustrate the ability of channels to add random errors in words, and of decoders to correct these random errors. Plus, for beginners, finding a statement such as `set_random_seed` might be confusing imho.

So I more or less sacrificed doctest purity on the altar of understandability. Which is not a big deal as these methods are properly doctested in their respective files.

David



---

archive/issue_comments_353338.json:
```json
{
    "body": "Changing commit from \"d70d6aa219a0472d1e254a23b6e0427f5cc7e1b2\" to \"7cbcca2969f5a72776eaff64951318a6c3c89a84\"",
    "created_at": "2016-02-19T10:32:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353338",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "d70d6aa219a0472d1e254a23b6e0427f5cc7e1b2" to "7cbcca2969f5a72776eaff64951318a6c3c89a84"



---

archive/issue_comments_353339.json:
```json
{
    "body": "<a id='comment:20'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                          |                                                  |\n|------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------|\n|[5f22eed](http://git.sagemath.org/sage.git/commit/?id=5f22eed8d8ead8856380189497475e986fe107eb)|`Default decoder for GRS codes is now Gao decoder`|\n|[8b654ac](http://git.sagemath.org/sage.git/commit/?id=8b654ac829600347c3c9c569269586f3117f44c7)|`Renamed method precompute and changed its doc`|\n|[b60ca15](http://git.sagemath.org/sage.git/commit/?id=b60ca15874ba350d3e66e0eb1bcebbe4f2ef603b)|`Rewrote documentation of partial_xgcd and made it a private method`|\n|[5ed4ac3](http://git.sagemath.org/sage.git/commit/?id=5ed4ac3227c5d4d360be90fa5e49a340dda36eba)|`Fixed error in error-erasure's decoding radius`|\n|[03c15f3](http://git.sagemath.org/sage.git/commit/?id=03c15f3eb5b0a8327f8a801d0cc033809ce3070d)|`Removed useless backslashes`|\n|[cc3cb4d](http://git.sagemath.org/sage.git/commit/?id=cc3cb4d76ba72206943bfe1bf6f736085e90d099)|`Rewrote error message in KeyEquationSyndromeDecoder`|\n|[a7f3ee1](http://git.sagemath.org/sage.git/commit/?id=a7f3ee1101e5a1ff91bcc5212e016d3cbc08b09f)|`Fixed typos`|\n|[c8bc406](http://git.sagemath.org/sage.git/commit/?id=c8bc40645e03331fa1aa772e3922795152a205ba)|`Rewrote some examples`|\n|[7cbcca2](http://git.sagemath.org/sage.git/commit/?id=7cbcca2969f5a72776eaff64951318a6c3c89a84)|`Rewrote some sentences`|",
    "created_at": "2016-02-19T10:32:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353339",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:20'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                          |                                                  |
|------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------|
|[5f22eed](http://git.sagemath.org/sage.git/commit/?id=5f22eed8d8ead8856380189497475e986fe107eb)|`Default decoder for GRS codes is now Gao decoder`|
|[8b654ac](http://git.sagemath.org/sage.git/commit/?id=8b654ac829600347c3c9c569269586f3117f44c7)|`Renamed method precompute and changed its doc`|
|[b60ca15](http://git.sagemath.org/sage.git/commit/?id=b60ca15874ba350d3e66e0eb1bcebbe4f2ef603b)|`Rewrote documentation of partial_xgcd and made it a private method`|
|[5ed4ac3](http://git.sagemath.org/sage.git/commit/?id=5ed4ac3227c5d4d360be90fa5e49a340dda36eba)|`Fixed error in error-erasure's decoding radius`|
|[03c15f3](http://git.sagemath.org/sage.git/commit/?id=03c15f3eb5b0a8327f8a801d0cc033809ce3070d)|`Removed useless backslashes`|
|[cc3cb4d](http://git.sagemath.org/sage.git/commit/?id=cc3cb4d76ba72206943bfe1bf6f736085e90d099)|`Rewrote error message in KeyEquationSyndromeDecoder`|
|[a7f3ee1](http://git.sagemath.org/sage.git/commit/?id=a7f3ee1101e5a1ff91bcc5212e016d3cbc08b09f)|`Fixed typos`|
|[c8bc406](http://git.sagemath.org/sage.git/commit/?id=c8bc40645e03331fa1aa772e3922795152a205ba)|`Rewrote some examples`|
|[7cbcca2](http://git.sagemath.org/sage.git/commit/?id=7cbcca2969f5a72776eaff64951318a6c3c89a84)|`Rewrote some sentences`|



---

archive/issue_comments_353340.json:
```json
{
    "body": "<a id='comment:21'></a>\n*** Key equation decoder ***\nThat's fine. I'm convinced we can do more efficient (e.g. \"cache\" `alpha[j]**l`, doing matrix-vector computation, or maybe using some kind of parity-check matrix) to compute the syndrome polynomial, but that's clearly not the point of the ticket. \n\n\n*** Runs ***\nWell, I don't know if that's important to fix them, but I found some non-caught (or badly caught) running errors:\n\n- when running [`Gao`/`Key`/`Berlekamp`] decoder with a GRS code of dimension k = n (a \"full code\" with distance 1, ok that's a weird code...): the decoder is built, but when trying to `decode_to_message` a word `y`, it begins by checking if `y` is a codeword by compute its syndrome, which is done by using the parity check matrix, which doesn't exist due to dimension issues. Thus the following error is raised: `ValueError: The dimension must be a positive integer at most the length of the code.` I suggest two solutions (but the second one seems to be the best one, see remark below):\n  - we prevent the construction of a decoder for such a code (in a way, it has no sense to decode that code)\n  - the decoder returns the input word\n\n- when running [`Gao`/`Key`/`Berlekamp`] decoder with a GRS code of dimension k=n-1 (distance 2) on a noisy word with 1 error, some weird things happen (for the moment, I don't have any solution for these issues):\n  - for `KeyEquationDecoder`, it tries to decode and fails almost every time (I would say with proba (q-1)/q, as if it was random).\n  - for `GaoDecoder`, `decode_to_code` raises `ValueError: The polynomial to encode must have degree at most k-1 when trying to re-encode the message`. It means `decode_to_message` outputs a polynomial with too high degree (k).\n  - for `BerlekampDecoder`, that's a random mix of the two previous behaviour (sooo weird...).\n\nIn fact, the previous issues are more important that I thought. For example, see the following script:\n\n```\nC = codes.GeneralizedReedSolomonCode(GF(16,'a').list()[1:], 12)\nD = C.decoder('ErrorErasure')\nChan = channels.ErrorErasureChannel(C.ambient_space(), 0, 3)\nc = C.random_element()\ny,era = Chan(c)\nmp = D.decode_to_message(tuple([y, era]))\n```\nThe [15,12,4]-GRS code is clearly not a trivial code, but I still get the strange error from the \"full code\", because in the `ErasureError` decoding algorithm, we puncture the code on erasures, and call the error decoding algorithm on the punctured code (which is the full code).\n\n*** Another remark ***\nIf you set Gao decoder as the default decoder for GRS, you should also set `EvaluationPolynomial` as the default encoder, otherwise we do not have `decode_to_message(encode(x)) = x`.",
    "created_at": "2016-02-19T14:55:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353340",
    "user": "https://github.com/jlavauzelle"
}
```

<a id='comment:21'></a>
*** Key equation decoder ***
That's fine. I'm convinced we can do more efficient (e.g. "cache" `alpha[j]**l`, doing matrix-vector computation, or maybe using some kind of parity-check matrix) to compute the syndrome polynomial, but that's clearly not the point of the ticket. 


*** Runs ***
Well, I don't know if that's important to fix them, but I found some non-caught (or badly caught) running errors:

- when running [`Gao`/`Key`/`Berlekamp`] decoder with a GRS code of dimension k = n (a "full code" with distance 1, ok that's a weird code...): the decoder is built, but when trying to `decode_to_message` a word `y`, it begins by checking if `y` is a codeword by compute its syndrome, which is done by using the parity check matrix, which doesn't exist due to dimension issues. Thus the following error is raised: `ValueError: The dimension must be a positive integer at most the length of the code.` I suggest two solutions (but the second one seems to be the best one, see remark below):
  - we prevent the construction of a decoder for such a code (in a way, it has no sense to decode that code)
  - the decoder returns the input word

- when running [`Gao`/`Key`/`Berlekamp`] decoder with a GRS code of dimension k=n-1 (distance 2) on a noisy word with 1 error, some weird things happen (for the moment, I don't have any solution for these issues):
  - for `KeyEquationDecoder`, it tries to decode and fails almost every time (I would say with proba (q-1)/q, as if it was random).
  - for `GaoDecoder`, `decode_to_code` raises `ValueError: The polynomial to encode must have degree at most k-1 when trying to re-encode the message`. It means `decode_to_message` outputs a polynomial with too high degree (k).
  - for `BerlekampDecoder`, that's a random mix of the two previous behaviour (sooo weird...).

In fact, the previous issues are more important that I thought. For example, see the following script:

```
C = codes.GeneralizedReedSolomonCode(GF(16,'a').list()[1:], 12)
D = C.decoder('ErrorErasure')
Chan = channels.ErrorErasureChannel(C.ambient_space(), 0, 3)
c = C.random_element()
y,era = Chan(c)
mp = D.decode_to_message(tuple([y, era]))
```
The [15,12,4]-GRS code is clearly not a trivial code, but I still get the strange error from the "full code", because in the `ErasureError` decoding algorithm, we puncture the code on erasures, and call the error decoding algorithm on the punctured code (which is the full code).

*** Another remark ***
If you set Gao decoder as the default decoder for GRS, you should also set `EvaluationPolynomial` as the default encoder, otherwise we do not have `decode_to_message(encode(x)) = x`.



---

archive/issue_comments_353341.json:
```json
{
    "body": "<a id='comment:22'></a>\nYum, border cases! I'll investigate this.\n\nRegarding your\n> ***Another remark***\n\n\nyour example actually works. See as follows:\n\n```\nsage: C = codes.GeneralizedReedSolomonCode(GF(59).list()[:40], 12)\nsage: c = C.random_element()\nsage: Chan = channels.StaticErrorRateChannel(C.ambient_space(), C.decoder().decoding_radius())\nsage: y = Chan(c)\nsage: c == C.encode(C.decode_to_message(y))\nTrue\n```\n\nWhat might be bothering you is:\n\n```\nsage: C.decoder().connected_encoder() == C.encoder()\nFalse\n```\n\nBut actually, `AbstractLinearCode.decode_to_message` has been written such that it *always* returns the element of the message space as a vector, because it seems the more natural way of seeing messages. This generic `decode_to_message` is more designed for beginners/people who don't want to bother with all the underlying structure, thus we defaulted the the most intuitive representation.\n\nAnd that's one of the reasons for which we ask `default_encoder` to be always set to an encoder whose `message_space` is a vector space.\n\nDavid",
    "created_at": "2016-02-19T15:26:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353341",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

<a id='comment:22'></a>
Yum, border cases! I'll investigate this.

Regarding your
> ***Another remark***


your example actually works. See as follows:

```
sage: C = codes.GeneralizedReedSolomonCode(GF(59).list()[:40], 12)
sage: c = C.random_element()
sage: Chan = channels.StaticErrorRateChannel(C.ambient_space(), C.decoder().decoding_radius())
sage: y = Chan(c)
sage: c == C.encode(C.decode_to_message(y))
True
```

What might be bothering you is:

```
sage: C.decoder().connected_encoder() == C.encoder()
False
```

But actually, `AbstractLinearCode.decode_to_message` has been written such that it *always* returns the element of the message space as a vector, because it seems the more natural way of seeing messages. This generic `decode_to_message` is more designed for beginners/people who don't want to bother with all the underlying structure, thus we defaulted the the most intuitive representation.

And that's one of the reasons for which we ask `default_encoder` to be always set to an encoder whose `message_space` is a vector space.

David



---

archive/issue_comments_353342.json:
```json
{
    "body": "Changing commit from \"7cbcca2969f5a72776eaff64951318a6c3c89a84\" to \"4aebb5a23a3c571f945bb7da558e03ff4b4c1ace\"",
    "created_at": "2016-02-22T10:32:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353342",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "7cbcca2969f5a72776eaff64951318a6c3c89a84" to "4aebb5a23a3c571f945bb7da558e03ff4b4c1ace"



---

archive/issue_comments_353343.json:
```json
{
    "body": "<a id='comment:23'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                          |                                                         |\n|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------|\n|[4aebb5a](http://git.sagemath.org/sage.git/commit/?id=4aebb5a23a3c571f945bb7da558e03ff4b4c1ace)|`Fixed bug related to full codes, plus some minor tweaks`|",
    "created_at": "2016-02-22T10:32:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353343",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:23'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                          |                                                         |
|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------|
|[4aebb5a](http://git.sagemath.org/sage.git/commit/?id=4aebb5a23a3c571f945bb7da558e03ff4b4c1ace)|`Fixed bug related to full codes, plus some minor tweaks`|



---

archive/issue_comments_353344.json:
```json
{
    "body": "<a id='comment:24'></a>\nHello,\n\nSo I fixed the bug related to full codes.\n\nI added a specific test in every decoder's decoding method to check if `C.length() == C.dimension()`.\nIf that's the case, the input word is returned as is/unencoded as is (depending if it's `decode_to_code` or `decode_to_message`).\nI also added a note in these methods to document this behaviour.\n\nAs `C.decode_to_message()` was also failing in the full code case, I overrode it.\n\nFinally, I renamed `shorten_word` to `punctured_word` in `GRSErrorErasureDecoder`, and store `C.length()` and `C.dimension()` in variables when they were called multiple times (as it was the case in Berlekamp-Welch.\n\nDavid",
    "created_at": "2016-02-22T10:37:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353344",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

<a id='comment:24'></a>
Hello,

So I fixed the bug related to full codes.

I added a specific test in every decoder's decoding method to check if `C.length() == C.dimension()`.
If that's the case, the input word is returned as is/unencoded as is (depending if it's `decode_to_code` or `decode_to_message`).
I also added a note in these methods to document this behaviour.

As `C.decode_to_message()` was also failing in the full code case, I overrode it.

Finally, I renamed `shorten_word` to `punctured_word` in `GRSErrorErasureDecoder`, and store `C.length()` and `C.dimension()` in variables when they were called multiple times (as it was the case in Berlekamp-Welch.

David



---

archive/issue_comments_353345.json:
```json
{
    "body": "<a id='comment:25'></a>\nHi David,\n\nI found a weird thing in your functions `_partial_xgcd()` :\n- in Key equation decoder, you define a `stop` variable, but never use it; your halting condition is related to the polynomial `t`, contrary to what you say in your documentation.\n- in Gao decoder, you use a variable `stop` which (I think, though Gao paper is not so clear about it) isn't well-defined. You write: `stop = floor(self.code().dimension() + self.code().length()) / 2`; I think that's `stop = (self.code().dimension() + self.code().length()) // 2` instead (note the integer division).\n\nApparently, it could solve the previous \"too-high degree re-encoding\" issue.\n\n\n\n\nYou also told me to point out when you used numbers instead of field elements. Here is a small list (I don't know if all of them belong to your code):\n- l. 197: `0 in ...` --> `F.zero()`\n- l. 317: maybe the 1/... \n- l. 1702: `e.append(0)` --> `e.append(C.base_ring().zero())`. Maybe also the previous 1 and 0 in lines 1698 and 1699.\n\n\nOtherwise, the code is pretty good. I've nothing to add.\n\nJulien",
    "created_at": "2016-02-22T20:52:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353345",
    "user": "https://github.com/jlavauzelle"
}
```

<a id='comment:25'></a>
Hi David,

I found a weird thing in your functions `_partial_xgcd()` :
- in Key equation decoder, you define a `stop` variable, but never use it; your halting condition is related to the polynomial `t`, contrary to what you say in your documentation.
- in Gao decoder, you use a variable `stop` which (I think, though Gao paper is not so clear about it) isn't well-defined. You write: `stop = floor(self.code().dimension() + self.code().length()) / 2`; I think that's `stop = (self.code().dimension() + self.code().length()) // 2` instead (note the integer division).

Apparently, it could solve the previous "too-high degree re-encoding" issue.




You also told me to point out when you used numbers instead of field elements. Here is a small list (I don't know if all of them belong to your code):
- l. 197: `0 in ...` --> `F.zero()`
- l. 317: maybe the 1/... 
- l. 1702: `e.append(0)` --> `e.append(C.base_ring().zero())`. Maybe also the previous 1 and 0 in lines 1698 and 1699.


Otherwise, the code is pretty good. I've nothing to add.

Julien



---

archive/issue_comments_353346.json:
```json
{
    "body": "<a id='comment:26'></a>\nHello,\n\nI changed what you asked, except the integer division in Gao decoder. I re-read Gao's paper and it's indeed unclear.\nSo I tried the empirical method, and ran some tests.\nIn the case `/` this snippet:\n\n```\nsage: C = codes.GeneralizedReedSolomonCode(GF(59).list()[:40], 39)\nsage: D = C.decoder(\"Gao\")\nsage: Chan = channels.StaticErrorRateChannel(C.ambient_space(), 1)\nsage: for i in range(100):\n....:    c = C.random_element()\n....:    y = Chan(c)\n....:    assert c == D.decode_to_code(y)\n```\n\nindeed returns the \"too-high degree\" weird error...\n\n\nBut in the case `//`, the same snippet returns an `AssertionError`!\nI'm really stuck here. If anyone has an idea, I'll be glad to hear it.\n\nDavid",
    "created_at": "2016-02-23T09:58:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353346",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

<a id='comment:26'></a>
Hello,

I changed what you asked, except the integer division in Gao decoder. I re-read Gao's paper and it's indeed unclear.
So I tried the empirical method, and ran some tests.
In the case `/` this snippet:

```
sage: C = codes.GeneralizedReedSolomonCode(GF(59).list()[:40], 39)
sage: D = C.decoder("Gao")
sage: Chan = channels.StaticErrorRateChannel(C.ambient_space(), 1)
sage: for i in range(100):
....:    c = C.random_element()
....:    y = Chan(c)
....:    assert c == D.decode_to_code(y)
```

indeed returns the "too-high degree" weird error...


But in the case `//`, the same snippet returns an `AssertionError`!
I'm really stuck here. If anyone has an idea, I'll be glad to hear it.

David



---

archive/issue_comments_353347.json:
```json
{
    "body": "Changing commit from \"4aebb5a23a3c571f945bb7da558e03ff4b4c1ace\" to \"29b8892169f34a763c6e1be45a9d7cbba89dd811\"",
    "created_at": "2016-02-23T09:58:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353347",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "4aebb5a23a3c571f945bb7da558e03ff4b4c1ace" to "29b8892169f34a763c6e1be45a9d7cbba89dd811"



---

archive/issue_comments_353348.json:
```json
{
    "body": "<a id='comment:27'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                          |                    |\n|------------------------------------------------------------------------------------------------------------------------------------------|--------------------|\n|[4479b4d](http://git.sagemath.org/sage.git/commit/?id=4479b4de966519d7e183bcf28fc0438ffdc43977)|`Update to 7.1beta4`|\n|[b2673aa](http://git.sagemath.org/sage.git/commit/?id=b2673aaa723f0fe3c59cf8d259bc3670496d2db7)|`Replaced 0s and 1s by F.zero() and F.one()`|\n|[29b8892](http://git.sagemath.org/sage.git/commit/?id=29b8892169f34a763c6e1be45a9d7cbba89dd811)|`Removed useless stop variable`|",
    "created_at": "2016-02-23T09:58:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353348",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:27'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                          |                    |
|------------------------------------------------------------------------------------------------------------------------------------------|--------------------|
|[4479b4d](http://git.sagemath.org/sage.git/commit/?id=4479b4de966519d7e183bcf28fc0438ffdc43977)|`Update to 7.1beta4`|
|[b2673aa](http://git.sagemath.org/sage.git/commit/?id=b2673aaa723f0fe3c59cf8d259bc3670496d2db7)|`Replaced 0s and 1s by F.zero() and F.one()`|
|[29b8892](http://git.sagemath.org/sage.git/commit/?id=29b8892169f34a763c6e1be45a9d7cbba89dd811)|`Removed useless stop variable`|



---

archive/issue_comments_353349.json:
```json
{
    "body": "<a id='comment:28'></a>\n> But in the case `//`, the same snippet returns an `AssertionError`!\n> I'm really stuck here. If anyone has an idea, I'll be glad to hear it.\n\n\nThe code has minimum distance 2, so it's OK that it can't decode 1 error. Or?\n\nThe correct is `floor( (n+k)/2 )`. Note that in the current code, the floor function is around `n+k`, and not around the division by two. You can do `//` as Julien suggests, or move the floor function around the division.\n\nGao's paper is very complicated to read, though the algorithm is so simple. It's rather silly really -- it would have been much clearer to formulate the stopping criterion as `deg s + k > deg r`. That's how you would phrase it when using lattice basis reduction. It implies `deg r < floor( (n+k)/2 )` under the assumption that `deg s < floor( (n-k)/2 )`.",
    "created_at": "2016-02-23T10:34:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353349",
    "user": "https://github.com/johanrosenkilde"
}
```

<a id='comment:28'></a>
> But in the case `//`, the same snippet returns an `AssertionError`!
> I'm really stuck here. If anyone has an idea, I'll be glad to hear it.


The code has minimum distance 2, so it's OK that it can't decode 1 error. Or?

The correct is `floor( (n+k)/2 )`. Note that in the current code, the floor function is around `n+k`, and not around the division by two. You can do `//` as Julien suggests, or move the floor function around the division.

Gao's paper is very complicated to read, though the algorithm is so simple. It's rather silly really -- it would have been much clearer to formulate the stopping criterion as `deg s + k > deg r`. That's how you would phrase it when using lattice basis reduction. It implies `deg r < floor( (n+k)/2 )` under the assumption that `deg s < floor( (n-k)/2 )`.



---

archive/issue_comments_353350.json:
```json
{
    "body": "<a id='comment:29'></a>\nHello,\n\nHmmm sorry David, I wasn't so clear about what was bothering me. It wasn't the fact that the decoder raised an error (as Johan said, the decoder can't correct 1 error), but I just didn't like the kind of error it raised.\n\nHope it's clear now.\n\nJulien",
    "created_at": "2016-02-23T10:49:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353350",
    "user": "https://github.com/jlavauzelle"
}
```

<a id='comment:29'></a>
Hello,

Hmmm sorry David, I wasn't so clear about what was bothering me. It wasn't the fact that the decoder raised an error (as Johan said, the decoder can't correct 1 error), but I just didn't like the kind of error it raised.

Hope it's clear now.

Julien



---

archive/issue_comments_353351.json:
```json
{
    "body": "<a id='comment:30'></a>\n> as Johan said, the decoder can't correct 1 error\n\nOh yes, of course... Sorry for being so dumb here.\n\nI'll change `/` to `//` ASAP. I'm on something else for now.",
    "created_at": "2016-02-23T11:33:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353351",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

<a id='comment:30'></a>
> as Johan said, the decoder can't correct 1 error

Oh yes, of course... Sorry for being so dumb here.

I'll change `/` to `//` ASAP. I'm on something else for now.



---

archive/issue_comments_353352.json:
```json
{
    "body": "<a id='comment:31'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                          |                                                                                    |\n|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------|\n|[6e4b074](http://git.sagemath.org/sage.git/commit/?id=6e4b0746b3daffbd18c0c874612f957f0ef2a919)|`Replaced standard division by integer division in Gao decoder's stopping criterion`|",
    "created_at": "2016-02-23T12:15:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353352",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:31'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                          |                                                                                    |
|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------|
|[6e4b074](http://git.sagemath.org/sage.git/commit/?id=6e4b0746b3daffbd18c0c874612f957f0ef2a919)|`Replaced standard division by integer division in Gao decoder's stopping criterion`|



---

archive/issue_comments_353353.json:
```json
{
    "body": "Changing commit from \"29b8892169f34a763c6e1be45a9d7cbba89dd811\" to \"6e4b0746b3daffbd18c0c874612f957f0ef2a919\"",
    "created_at": "2016-02-23T12:15:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353353",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "29b8892169f34a763c6e1be45a9d7cbba89dd811" to "6e4b0746b3daffbd18c0c874612f957f0ef2a919"



---

archive/issue_comments_353354.json:
```json
{
    "body": "<a id='comment:32'></a>\n> I'll change `/` to `//` ASAP.\n\nDone.",
    "created_at": "2016-02-23T12:16:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353354",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

<a id='comment:32'></a>
> I'll change `/` to `//` ASAP.

Done.



---

archive/issue_comments_353355.json:
```json
{
    "body": "<a id='comment:33'></a>\nI was actually checking something completely different, but then I noticed that the key equation syndrome decoder has absolutely no sanity checks! If too many errors occurred, or you just gave it a random input, then it will always return something, and often not a codeword!\n\nIn the same vain, are you sure that the output of the other decoders will always be codewords? I don't think the divisibility test is sufficient.",
    "created_at": "2016-02-24T12:58:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353355",
    "user": "https://github.com/johanrosenkilde"
}
```

<a id='comment:33'></a>
I was actually checking something completely different, but then I noticed that the key equation syndrome decoder has absolutely no sanity checks! If too many errors occurred, or you just gave it a random input, then it will always return something, and often not a codeword!

In the same vain, are you sure that the output of the other decoders will always be codewords? I don't think the divisibility test is sufficient.



---

archive/issue_comments_353356.json:
```json
{
    "body": "Changing commit from \"6e4b0746b3daffbd18c0c874612f957f0ef2a919\" to \"6ad583f27736146eb019ba99ecfb54e67c31f5ea\"",
    "created_at": "2016-02-24T15:20:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353356",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "6e4b0746b3daffbd18c0c874612f957f0ef2a919" to "6ad583f27736146eb019ba99ecfb54e67c31f5ea"



---

archive/issue_comments_353357.json:
```json
{
    "body": "<a id='comment:34'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                          |                                                                                  |\n|------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------|\n|[2246e18](http://git.sagemath.org/sage.git/commit/?id=2246e18480fd368f033a4f8694c04ee9beeec09a)|`Better doctests for BW decoder, added sanity check on decode_to_messaage's input`|\n|[7309523](http://git.sagemath.org/sage.git/commit/?id=73095231c660a89ff4f731710bb294ab1b78ef5e)|`Better doctests for Gao decoder, added sanity check on decode_to_messaage's input`|\n|[c4ca177](http://git.sagemath.org/sage.git/commit/?id=c4ca1775ee81218edb969887a7a2da4aba0e4b61)|`Better doctests for Error-Erasure decoder, added sanity check on decode_to_messaage's input`|\n|[a074cb8](http://git.sagemath.org/sage.git/commit/?id=a074cb8a53bb85a96e6f8f7abf77a038cbe3ed25)|`Changed my helper methods into private methods`|\n|[6ad583f](http://git.sagemath.org/sage.git/commit/?id=6ad583f27736146eb019ba99ecfb54e67c31f5ea)|`Fixed bug in KeyEquation decoder's decode_to_code, added sanity checks, enhanced doctests`|",
    "created_at": "2016-02-24T15:20:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353357",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:34'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                          |                                                                                  |
|------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------|
|[2246e18](http://git.sagemath.org/sage.git/commit/?id=2246e18480fd368f033a4f8694c04ee9beeec09a)|`Better doctests for BW decoder, added sanity check on decode_to_messaage's input`|
|[7309523](http://git.sagemath.org/sage.git/commit/?id=73095231c660a89ff4f731710bb294ab1b78ef5e)|`Better doctests for Gao decoder, added sanity check on decode_to_messaage's input`|
|[c4ca177](http://git.sagemath.org/sage.git/commit/?id=c4ca1775ee81218edb969887a7a2da4aba0e4b61)|`Better doctests for Error-Erasure decoder, added sanity check on decode_to_messaage's input`|
|[a074cb8](http://git.sagemath.org/sage.git/commit/?id=a074cb8a53bb85a96e6f8f7abf77a038cbe3ed25)|`Changed my helper methods into private methods`|
|[6ad583f](http://git.sagemath.org/sage.git/commit/?id=6ad583f27736146eb019ba99ecfb54e67c31f5ea)|`Fixed bug in KeyEquation decoder's decode_to_code, added sanity checks, enhanced doctests`|



---

archive/issue_comments_353358.json:
```json
{
    "body": "<a id='comment:35'></a>\nOk, I added sanity checks on the input on every `decode_to_*` method.\nIt does not utterly fail anymore if one tries to decode something which is not a vector (or a vector which is not in the ambient space of the code).\n\nI also upgraded doctests to document this behaviour. New doctests now generate a random codeword `c`, create a channel which adds as many errors as decoder's capacity, transmit `c` to get `y` and compare `c` and `y` decoded. It's much better than a single hardcoded example.\n\nFurthermore, I added an explicative message to every call to `DecodingError`.",
    "created_at": "2016-02-24T15:25:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353358",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

<a id='comment:35'></a>
Ok, I added sanity checks on the input on every `decode_to_*` method.
It does not utterly fail anymore if one tries to decode something which is not a vector (or a vector which is not in the ambient space of the code).

I also upgraded doctests to document this behaviour. New doctests now generate a random codeword `c`, create a channel which adds as many errors as decoder's capacity, transmit `c` to get `y` and compare `c` and `y` decoded. It's much better than a single hardcoded example.

Furthermore, I added an explicative message to every call to `DecodingError`.



---

archive/issue_comments_353359.json:
```json
{
    "body": "<a id='comment:36'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                          |                                                            |\n|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------|\n|[fca099e](http://git.sagemath.org/sage.git/commit/?id=fca099ed40912d26eff991fc612ee793681000c5)|`Added new sanity check on the output of BW and Gao decoder`|",
    "created_at": "2016-02-24T15:27:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353359",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:36'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                          |                                                            |
|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------|
|[fca099e](http://git.sagemath.org/sage.git/commit/?id=fca099ed40912d26eff991fc612ee793681000c5)|`Added new sanity check on the output of BW and Gao decoder`|



---

archive/issue_comments_353360.json:
```json
{
    "body": "Changing commit from \"6ad583f27736146eb019ba99ecfb54e67c31f5ea\" to \"fca099ed40912d26eff991fc612ee793681000c5\"",
    "created_at": "2016-02-24T15:27:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353360",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "6ad583f27736146eb019ba99ecfb54e67c31f5ea" to "fca099ed40912d26eff991fc612ee793681000c5"



---

archive/issue_comments_353361.json:
```json
{
    "body": "<a id='comment:37'></a>\nAnd, (sorry, I forgot to add it...) I also made a new sanity check on the output polynomial for `BerlekmapWelch` and `Gao`: if the output polynomial is not in `F[x]`, where `F` is the base ring of the code, it raises a `DecodingError`.\n\nDavid",
    "created_at": "2016-02-24T15:29:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353361",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

<a id='comment:37'></a>
And, (sorry, I forgot to add it...) I also made a new sanity check on the output polynomial for `BerlekmapWelch` and `Gao`: if the output polynomial is not in `F[x]`, where `F` is the base ring of the code, it raises a `DecodingError`.

David



---

archive/issue_comments_353362.json:
```json
{
    "body": "<a id='comment:38'></a>\nYour new checks seem good. But they were actually not what I was talking about - I wasn't very clear. Specifically for the BW decoder, let's say that more errors than (d-1)/2 occurs. In that case, all (almost) bets are off! So the division you do could for instance go fine, but you just end up with a polynomial `h` which is (when you encode it) nowhere near the received word you got. There are currently no checks for this.\n\nIt's similar, I'm pretty sure, with the other decoders.\n\nI added exactly such a check on the Guruswami-Sudan decoder yesterday.",
    "created_at": "2016-02-24T15:37:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353362",
    "user": "https://github.com/johanrosenkilde"
}
```

<a id='comment:38'></a>
Your new checks seem good. But they were actually not what I was talking about - I wasn't very clear. Specifically for the BW decoder, let's say that more errors than (d-1)/2 occurs. In that case, all (almost) bets are off! So the division you do could for instance go fine, but you just end up with a polynomial `h` which is (when you encode it) nowhere near the received word you got. There are currently no checks for this.

It's similar, I'm pretty sure, with the other decoders.

I added exactly such a check on the Guruswami-Sudan decoder yesterday.



---

archive/issue_comments_353363.json:
```json
{
    "body": "<a id='comment:39'></a>\nTwo completely different things I just noticed:\n\n* I think the line `R(S.list_from_positions(xrange(0, l0+1)))` can, and should, be written `R(S[:l0+1])`.\n\n* In Berlekamp-Welch, and in Gao, you do a divisibility test followed by the actual division, i.e. you divide twice. You should instead use the `quo_rem` function to do the division once, and do the divisibility test by seeing if the remainder is zero.",
    "created_at": "2016-02-24T15:44:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353363",
    "user": "https://github.com/johanrosenkilde"
}
```

<a id='comment:39'></a>
Two completely different things I just noticed:

* I think the line `R(S.list_from_positions(xrange(0, l0+1)))` can, and should, be written `R(S[:l0+1])`.

* In Berlekamp-Welch, and in Gao, you do a divisibility test followed by the actual division, i.e. you divide twice. You should instead use the `quo_rem` function to do the division once, and do the divisibility test by seeing if the remainder is zero.



---

archive/issue_comments_353364.json:
```json
{
    "body": "<a id='comment:40'></a>\nOk, I found something very annoying while working on writing an actual check on the output of decoding algortihms:\n\n```\nsage: C = codes.GeneralizedReedSolomonCode(GF(59).list()[1:40+1], 12)\nsage: c1 = C.random_element()\nsage: c2 = C[1]\nsage: c1.parent() == c2.parent()\nFalse\n```\n\nSo if the input of `decode_to_code`, say `r` is generated through `random_elements`, all code like t his:\n\n```\n(r - decoded_word).hamming_weight() > self.decoding_radius():\n    raise DecodingError(\"Decoding failed because the number of errors exceeded the decoding radius\")\n```\n\nsystematically fails because one cannot properly subtract two vectors whose parents are diffrent.\n\nSo I'll fix `random_element` method...",
    "created_at": "2016-02-24T16:14:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353364",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

<a id='comment:40'></a>
Ok, I found something very annoying while working on writing an actual check on the output of decoding algortihms:

```
sage: C = codes.GeneralizedReedSolomonCode(GF(59).list()[1:40+1], 12)
sage: c1 = C.random_element()
sage: c2 = C[1]
sage: c1.parent() == c2.parent()
False
```

So if the input of `decode_to_code`, say `r` is generated through `random_elements`, all code like t his:

```
(r - decoded_word).hamming_weight() > self.decoding_radius():
    raise DecodingError("Decoding failed because the number of errors exceeded the decoding radius")
```

systematically fails because one cannot properly subtract two vectors whose parents are diffrent.

So I'll fix `random_element` method...



---

archive/issue_comments_353365.json:
```json
{
    "body": "<a id='comment:41'></a>\nDamn. It's related to this sick semantics that the parent of a vector is the subspace. That's also causing trouble in the channels. We should try to fix that at the root, i.e. experiment with the scaffolding code at linear code construction time. Didn't Vincent write something about how to do this in the old sage-devel thread where we discussed the bug?",
    "created_at": "2016-02-24T16:36:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353365",
    "user": "https://github.com/johanrosenkilde"
}
```

<a id='comment:41'></a>
Damn. It's related to this sick semantics that the parent of a vector is the subspace. That's also causing trouble in the channels. We should try to fix that at the root, i.e. experiment with the scaffolding code at linear code construction time. Didn't Vincent write something about how to do this in the old sage-devel thread where we discussed the bug?



---

archive/issue_comments_353366.json:
```json
{
    "body": "<a id='comment:42'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                          |                               |\n|------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------|\n|[57dbfbf](http://git.sagemath.org/sage.git/commit/?id=57dbfbf126d1b4743ae4e6346076d39a30f9835c)|`Rewrote random_element method`|\n|[7953d60](http://git.sagemath.org/sage.git/commit/?id=7953d60aa6a583aba7767a74bf43194a34383194)|`Proper sanity checks for output of decode_to_* methods`|\n|[8673ac5](http://git.sagemath.org/sage.git/commit/?id=8673ac587f4e8bece38fc7027c96eef9771c7eec)|`Optimized a bit polynomial division in Gao and BW`|",
    "created_at": "2016-02-24T16:39:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353366",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:42'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                          |                               |
|------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------|
|[57dbfbf](http://git.sagemath.org/sage.git/commit/?id=57dbfbf126d1b4743ae4e6346076d39a30f9835c)|`Rewrote random_element method`|
|[7953d60](http://git.sagemath.org/sage.git/commit/?id=7953d60aa6a583aba7767a74bf43194a34383194)|`Proper sanity checks for output of decode_to_* methods`|
|[8673ac5](http://git.sagemath.org/sage.git/commit/?id=8673ac587f4e8bece38fc7027c96eef9771c7eec)|`Optimized a bit polynomial division in Gao and BW`|



---

archive/issue_comments_353367.json:
```json
{
    "body": "Changing commit from \"fca099ed40912d26eff991fc612ee793681000c5\" to \"8673ac587f4e8bece38fc7027c96eef9771c7eec\"",
    "created_at": "2016-02-24T16:39:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353367",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "fca099ed40912d26eff991fc612ee793681000c5" to "8673ac587f4e8bece38fc7027c96eef9771c7eec"



---

archive/issue_comments_353368.json:
```json
{
    "body": "<a id='comment:43'></a>\n> Damn. It's related to this sick semantics that the parent of a vector is the subspace. That's also causing trouble in the channels. We should try to fix that at the root, i.e. experiment with the scaffolding code at linear code construction time. Didn't Vincent write something about how to do this in the old sage-devel thread where we discussed the bug?\n\n\nAh, in the meanwhile I changed this method to something based on encoding a random element from the message space of the default encode of a code.\nProbably not the best way to do it, I'm aware of that.\n\nI made the other changes, but  \n\n> I think the line `R(S.list_from_positions(xrange(0, l0+1)))` can, and should, be written `R(S[:l0+1])`\n\n\nwhich I tried, but it did not work.",
    "created_at": "2016-02-24T16:41:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353368",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

<a id='comment:43'></a>
> Damn. It's related to this sick semantics that the parent of a vector is the subspace. That's also causing trouble in the channels. We should try to fix that at the root, i.e. experiment with the scaffolding code at linear code construction time. Didn't Vincent write something about how to do this in the old sage-devel thread where we discussed the bug?


Ah, in the meanwhile I changed this method to something based on encoding a random element from the message space of the default encode of a code.
Probably not the best way to do it, I'm aware of that.

I made the other changes, but  

> I think the line `R(S.list_from_positions(xrange(0, l0+1)))` can, and should, be written `R(S[:l0+1])`


which I tried, but it did not work.



---

archive/issue_comments_353369.json:
```json
{
    "body": "<a id='comment:44'></a>\n> Ah, in the meanwhile I changed this method to something based on encoding a random element from the message space of the default encode of a code.\n> Probably not the best way to do it, I'm aware of that.\n\n\nThat's like pissing your pants to keep warm: it doesn't fix the problem in the gazillion other ways this could explode (such as using a `StaticErrorRateChannel`).\n\n> > I think the line `R(S.list_from_positions(xrange(0, l0+1)))` can, and should, be written `R(S[:l0+1])`\n\n\nTry harder, seriously... You need to call `list` on `S[:l0+1]` as well because the `R(...)` call is picky about getting a list and not a vector. I just did it, but it turns out it's slower than `list_from_positions`, so I guess you should just keep that though it looks horrendous.",
    "created_at": "2016-02-25T08:07:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353369",
    "user": "https://github.com/johanrosenkilde"
}
```

<a id='comment:44'></a>
> Ah, in the meanwhile I changed this method to something based on encoding a random element from the message space of the default encode of a code.
> Probably not the best way to do it, I'm aware of that.


That's like pissing your pants to keep warm: it doesn't fix the problem in the gazillion other ways this could explode (such as using a `StaticErrorRateChannel`).

> > I think the line `R(S.list_from_positions(xrange(0, l0+1)))` can, and should, be written `R(S[:l0+1])`


Try harder, seriously... You need to call `list` on `S[:l0+1]` as well because the `R(...)` call is picky about getting a list and not a vector. I just did it, but it turns out it's slower than `list_from_positions`, so I guess you should just keep that though it looks horrendous.



---

archive/issue_comments_353370.json:
```json
{
    "body": "<a id='comment:45'></a>\nReplying to [jsrn](#comment%3A44):\n> > Ah, in the meanwhile I changed this method to something based on encoding a random element from the message space of the default encode of a code.\n> > Probably not the best way to do it, I'm aware of that.\n\n> \n> That's like pissing your pants to keep warm: it doesn't fix the problem in the gazillion other ways this could explode (such as using a `StaticErrorRateChannel`).\n\n\nI totally take that back! I just looked through `LinearCode` and it seems there is no other way that we could end up with a codeword whose parent is the subvector-space. So the problem really *was* with `random_element`. Which means your fix is a *good* way to solve the problem :-) Sorry about the flame.\n\nLet's discuss another time whether there is a more efficient way to do it, and whether this bizarre passing through of `*args` and `**kwargs` to random shouldn't be deprecated (can you please note this down somewhere so we don't forget though?).\n\nBest,\nJohan",
    "created_at": "2016-02-25T08:29:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353370",
    "user": "https://github.com/johanrosenkilde"
}
```

<a id='comment:45'></a>
Replying to [jsrn](#comment%3A44):
> > Ah, in the meanwhile I changed this method to something based on encoding a random element from the message space of the default encode of a code.
> > Probably not the best way to do it, I'm aware of that.

> 
> That's like pissing your pants to keep warm: it doesn't fix the problem in the gazillion other ways this could explode (such as using a `StaticErrorRateChannel`).


I totally take that back! I just looked through `LinearCode` and it seems there is no other way that we could end up with a codeword whose parent is the subvector-space. So the problem really *was* with `random_element`. Which means your fix is a *good* way to solve the problem :-) Sorry about the flame.

Let's discuss another time whether there is a more efficient way to do it, and whether this bizarre passing through of `*args` and `**kwargs` to random shouldn't be deprecated (can you please note this down somewhere so we don't forget though?).

Best,
Johan



---

archive/issue_comments_353371.json:
```json
{
    "body": "<a id='comment:46'></a>\n(for instance, that the following works is not so good:\n\n```\n    C.random_element(monkey = 2)\n```\n\nBut, as I said, that's for another patch.)",
    "created_at": "2016-02-25T08:30:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353371",
    "user": "https://github.com/johanrosenkilde"
}
```

<a id='comment:46'></a>
(for instance, that the following works is not so good:

```
    C.random_element(monkey = 2)
```

But, as I said, that's for another patch.)



---

archive/issue_comments_353372.json:
```json
{
    "body": "Changing commit from \"8673ac587f4e8bece38fc7027c96eef9771c7eec\" to \"e1b6b091ee15d723378847bfb00e58d41b37d5ee\"",
    "created_at": "2016-02-25T15:15:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353372",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "8673ac587f4e8bece38fc7027c96eef9771c7eec" to "e1b6b091ee15d723378847bfb00e58d41b37d5ee"



---

archive/issue_comments_353373.json:
```json
{
    "body": "<a id='comment:47'></a>\nBranch pushed to git repo; I updated commit sha1. Last 10 new commits:\n|                                                                                                                                          |                                          |\n|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------|\n|[d5e5f35](http://git.sagemath.org/sage.git/commit/?id=d5e5f35921def3aee8a402b811676b61d77e4c36)|`Documentation to the root finding module`|\n|[4169e8b](http://git.sagemath.org/sage.git/commit/?id=4169e8be0936069606dcefbe3655ff9213260582)|`Fixed a bug in some code being validated in the wrong place. Improved`|\n|[5f03b11](http://git.sagemath.org/sage.git/commit/?id=5f03b11a3ef36bf3262cb5f3929429f09b748a9f)|`More doc-strings and rm a single assert`|\n|[8f2ce97](http://git.sagemath.org/sage.git/commit/?id=8f2ce9796b2f8ae0c1109d42c0bdc07fd80de3cf)|`Reflow doc-string`|\n|[0c7b80d](http://git.sagemath.org/sage.git/commit/?id=0c7b80d275d22ad900eaaaec5c92352bbdf37b8b)|`More robust testing: use sets instead of lists. Semantic tests on output. Corner case on zero input`|\n|[2b09750](http://git.sagemath.org/sage.git/commit/?id=2b0975090d03abeef343811a934646a33bcedc16)|`Make the example one where the decoding results in a list`|\n|[8133531](http://git.sagemath.org/sage.git/commit/?id=8133531f6b880b4bd246cab4ad6ae430b4f4b494)|`Fixed typo in decode_to_message`|\n|[b4bc257](http://git.sagemath.org/sage.git/commit/?id=b4bc2572e40c26757fe80e4676d139519aef159e)|`Removed #random flag from _flatten_once doctest`|\n|[29448ce](http://git.sagemath.org/sage.git/commit/?id=29448cea10786283b840b6c9576b21bd6857305b)|`Changed title of rootfinding.py`|\n|[e1b6b09](http://git.sagemath.org/sage.git/commit/?id=e1b6b091ee15d723378847bfb00e58d41b37d5ee)|`Merged now postive reviewed #19666 and fixed conflicts`|",
    "created_at": "2016-02-25T15:15:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353373",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:47'></a>
Branch pushed to git repo; I updated commit sha1. Last 10 new commits:
|                                                                                                                                          |                                          |
|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------|
|[d5e5f35](http://git.sagemath.org/sage.git/commit/?id=d5e5f35921def3aee8a402b811676b61d77e4c36)|`Documentation to the root finding module`|
|[4169e8b](http://git.sagemath.org/sage.git/commit/?id=4169e8be0936069606dcefbe3655ff9213260582)|`Fixed a bug in some code being validated in the wrong place. Improved`|
|[5f03b11](http://git.sagemath.org/sage.git/commit/?id=5f03b11a3ef36bf3262cb5f3929429f09b748a9f)|`More doc-strings and rm a single assert`|
|[8f2ce97](http://git.sagemath.org/sage.git/commit/?id=8f2ce9796b2f8ae0c1109d42c0bdc07fd80de3cf)|`Reflow doc-string`|
|[0c7b80d](http://git.sagemath.org/sage.git/commit/?id=0c7b80d275d22ad900eaaaec5c92352bbdf37b8b)|`More robust testing: use sets instead of lists. Semantic tests on output. Corner case on zero input`|
|[2b09750](http://git.sagemath.org/sage.git/commit/?id=2b0975090d03abeef343811a934646a33bcedc16)|`Make the example one where the decoding results in a list`|
|[8133531](http://git.sagemath.org/sage.git/commit/?id=8133531f6b880b4bd246cab4ad6ae430b4f4b494)|`Fixed typo in decode_to_message`|
|[b4bc257](http://git.sagemath.org/sage.git/commit/?id=b4bc2572e40c26757fe80e4676d139519aef159e)|`Removed #random flag from _flatten_once doctest`|
|[29448ce](http://git.sagemath.org/sage.git/commit/?id=29448cea10786283b840b6c9576b21bd6857305b)|`Changed title of rootfinding.py`|
|[e1b6b09](http://git.sagemath.org/sage.git/commit/?id=e1b6b091ee15d723378847bfb00e58d41b37d5ee)|`Merged now postive reviewed #19666 and fixed conflicts`|



---

archive/issue_comments_353374.json:
```json
{
    "body": "<a id='comment:48'></a>\nI merged #19666 in this branch, and fixed conflicts.\nThis will prevent merge conflicts in this branch and #19666 ship in the same Sage beta.",
    "created_at": "2016-02-25T15:16:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353374",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

<a id='comment:48'></a>
I merged #19666 in this branch, and fixed conflicts.
This will prevent merge conflicts in this branch and #19666 ship in the same Sage beta.



---

archive/issue_comments_353375.json:
```json
{
    "body": "<a id='comment:49'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                          |                    |\n|------------------------------------------------------------------------------------------------------------------------------------------|--------------------|\n|[5093dce](http://git.sagemath.org/sage.git/commit/?id=5093dce713233b65314fc6fe2ef666fbca7e1ec9)|`Update to 7.1beta5`|\n|[f3b4b11](http://git.sagemath.org/sage.git/commit/?id=f3b4b11582fd4c2e976d2e0450d0e8699be0272e)|`Fixed typo in Guruswami-Sudan doc`|",
    "created_at": "2016-02-26T12:55:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353375",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:49'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                          |                    |
|------------------------------------------------------------------------------------------------------------------------------------------|--------------------|
|[5093dce](http://git.sagemath.org/sage.git/commit/?id=5093dce713233b65314fc6fe2ef666fbca7e1ec9)|`Update to 7.1beta5`|
|[f3b4b11](http://git.sagemath.org/sage.git/commit/?id=f3b4b11582fd4c2e976d2e0450d0e8699be0272e)|`Fixed typo in Guruswami-Sudan doc`|



---

archive/issue_comments_353376.json:
```json
{
    "body": "Changing commit from \"e1b6b091ee15d723378847bfb00e58d41b37d5ee\" to \"f3b4b11582fd4c2e976d2e0450d0e8699be0272e\"",
    "created_at": "2016-02-26T12:55:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353376",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "e1b6b091ee15d723378847bfb00e58d41b37d5ee" to "f3b4b11582fd4c2e976d2e0450d0e8699be0272e"



---

archive/issue_comments_353377.json:
```json
{
    "body": "<a id='comment:50'></a>\nAs this one is not yet reviewed, I take the opportunity to fix a typo in Guruswami-Sudan documentation.\n\nStill open for review.",
    "created_at": "2016-02-26T12:56:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353377",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

<a id='comment:50'></a>
As this one is not yet reviewed, I take the opportunity to fix a typo in Guruswami-Sudan documentation.

Still open for review.



---

archive/issue_comments_353378.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Julien Lavauzelle\"",
    "created_at": "2016-02-29T21:01:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353378",
    "user": "https://github.com/jlavauzelle"
}
```

Changing reviewer from "" to "Julien Lavauzelle"



---

archive/issue_comments_353379.json:
```json
{
    "body": "<a id='comment:51'></a>\nHi David,\n\nAll my previous tests now pass. I won't add anything, so if Johan agrees (and if the patchbot \"question mark\" isn't a problem), I let you put the ticket in positive review.\n\nReally good job !\n\nJulien",
    "created_at": "2016-02-29T21:01:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353379",
    "user": "https://github.com/jlavauzelle"
}
```

<a id='comment:51'></a>
Hi David,

All my previous tests now pass. I won't add anything, so if Johan agrees (and if the patchbot "question mark" isn't a problem), I let you put the ticket in positive review.

Really good job !

Julien



---

archive/issue_comments_353380.json:
```json
{
    "body": "<a id='comment:52'></a>\nHello,\n\n> (and if the patchbot \"question mark\" isn't a problem)\n\n\nNah, it's librae which has gone and done it again, see [here](http://patchbot.sagemath.org/log/19653/Ubuntu/14.04/x86_64/3.13.0-76-generic/librae/2016-02-26%2014:15:08?short), it's a completely unrelated test, which fails only on librae, not on the other bots.\n\n> I let you put the ticket in positive review\n\n\nPlease do, I don't want to take that from you (plural) ;)\n\nDavid",
    "created_at": "2016-02-29T21:17:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353380",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

<a id='comment:52'></a>
Hello,

> (and if the patchbot "question mark" isn't a problem)


Nah, it's librae which has gone and done it again, see [here](http://patchbot.sagemath.org/log/19653/Ubuntu/14.04/x86_64/3.13.0-76-generic/librae/2016-02-26%2014:15:08?short), it's a completely unrelated test, which fails only on librae, not on the other bots.

> I let you put the ticket in positive review


Please do, I don't want to take that from you (plural) ;)

David



---

archive/issue_comments_353381.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2016-03-01T07:19:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353381",
    "user": "https://github.com/johanrosenkilde"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_353382.json:
```json
{
    "body": "Changing branch from \"u/dlucas/grs_decoders\" to \"f3b4b11582fd4c2e976d2e0450d0e8699be0272e\"",
    "created_at": "2016-03-02T08:27:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353382",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/dlucas/grs_decoders" to "f3b4b11582fd4c2e976d2e0450d0e8699be0272e"



---

archive/issue_events_053984.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-03-02T08:27:44Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/19653#event-53984"
}
```



---

archive/issue_comments_353383.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2016-03-02T08:27:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19653#issuecomment-353383",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
