# Issue 19798: fix zeta on the universal cyclotomic field

archive/issues_019561.json:
```json
{
    "assignees": [],
    "body": "All roots of unity belong to the cyclotomic field but currently\n\n```\nsage: UCF = UniversalCyclotomicField().zeta(6)\nTraceback (most recent call last):\n...\nNotImplementedError: \n```\n\nWe simply add an alias `zeta -> gen`.\n\nBranch/Commit: **[e92ca38](https://github.com/sagemath/sagetrac-mirror/commit/e92ca38eaf628c046ab912ce07f0c2f6079d6f7c)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Vincent Delecroix**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19798_\n\n",
    "closed_at": "2015-12-30T13:10:59Z",
    "created_at": "2015-12-29T17:51:48Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix zeta on the universal cyclotomic field",
    "type": "issue",
    "updated_at": "2015-12-30T13:10:59Z",
    "url": "https://github.com/sagemath/sage/issues/19798",
    "user": "https://github.com/videlec"
}
```
All roots of unity belong to the cyclotomic field but currently

```
sage: UCF = UniversalCyclotomicField().zeta(6)
Traceback (most recent call last):
...
NotImplementedError: 
```

We simply add an alias `zeta -> gen`.

Branch/Commit: **[e92ca38](https://github.com/sagemath/sagetrac-mirror/commit/e92ca38eaf628c046ab912ce07f0c2f6079d6f7c)**

Reviewer: **Frédéric Chapoton**

Author: **Vincent Delecroix**

_Issue created by migration from https://trac.sagemath.org/ticket/19798_





---

archive/issue_events_262160.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-12-29T17:51:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19798",
    "milestone_number": null,
    "milestone_title": "sage-7.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19798#event-262160"
}
```



---

archive/issue_events_262161.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-12-29T17:51:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19798",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
    "label_color": "0000ff",
    "label_name": "component: number fields",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19798#event-262161"
}
```



---

archive/issue_events_262162.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-12-29T17:51:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19798",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19798#event-262162"
}
```



---

archive/issue_events_262163.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-12-29T17:51:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19798",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19798#event-262163"
}
```



---

archive/issue_events_262164.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-12-29T17:53:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19798",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19798#event-262164"
}
```



---

archive/issue_comments_285504.json:
```json
{
    "body": "Commit: **[e92ca38](https://github.com/sagemath/sagetrac-mirror/commit/e92ca38eaf628c046ab912ce07f0c2f6079d6f7c)**",
    "created_at": "2015-12-29T17:53:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19798",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19798#issuecomment-285504",
    "user": "https://github.com/videlec"
}
```

Commit: **[e92ca38](https://github.com/sagemath/sagetrac-mirror/commit/e92ca38eaf628c046ab912ce07f0c2f6079d6f7c)**



---

archive/issue_comments_285505.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e92ca38eaf628c046ab912ce07f0c2f6079d6f7c\">e92ca38</a></td><td><code>Trac 19798: fix UCF zeta</code></td></tr></table>\n",
    "created_at": "2015-12-29T17:53:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19798",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19798#issuecomment-285505",
    "user": "https://github.com/videlec"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e92ca38eaf628c046ab912ce07f0c2f6079d6f7c">e92ca38</a></td><td><code>Trac 19798: fix UCF zeta</code></td></tr></table>




---

archive/issue_comments_285506.json:
```json
{
    "body": "Branch: **[u/vdelecroix/19798](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/19798)**",
    "created_at": "2015-12-29T17:53:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19798",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19798#issuecomment-285506",
    "user": "https://github.com/videlec"
}
```

Branch: **[u/vdelecroix/19798](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/19798)**



---

archive/issue_comments_285507.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2015-12-30T09:18:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19798",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19798#issuecomment-285507",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_events_262165.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-12-30T09:18:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19798",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19798#event-262165"
}
```



---

archive/issue_events_262166.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2015-12-30T09:18:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19798",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19798#event-262166"
}
```



---

archive/issue_comments_285508.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nlgtm",
    "created_at": "2015-12-30T09:18:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19798",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19798#issuecomment-285508",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

lgtm



---

archive/issue_comments_285509.json:
```json
{
    "body": "Changed branch from **[u/vdelecroix/19798](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/19798)** to **[e92ca38](https://github.com/sagemath/sagetrac-mirror/commit/e92ca38eaf628c046ab912ce07f0c2f6079d6f7c)**",
    "created_at": "2015-12-30T13:10:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19798",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19798#issuecomment-285509",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/vdelecroix/19798](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/19798)** to **[e92ca38](https://github.com/sagemath/sagetrac-mirror/commit/e92ca38eaf628c046ab912ce07f0c2f6079d6f7c)**



---

archive/issue_events_262167.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-12-30T13:10:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19798",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19798#event-262167"
}
```



---

archive/issue_events_262168.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "0f9756f947cdf5c926f9e29ac21a60096e307e6a",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-12-30T13:10:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19798",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19798#event-262168"
}
```
