# Issue 19688: Much fast divisors using PARI divisorsu

archive/issues_019451.json:
```json
{
    "assignees": [],
    "body": "Using PARI we get a huge speedup for integer of small range. With the branch applied\n\n```\nsage: %timeit 100.divisors(method='pari')\n1000000 loops, best of 3: 818 ns per loop\nsage: %timeit 100.divisors(method='sage')\n100000 loops, best of 3: 14.1 \u00b5s per loop\n```\n\nAs it is critical for some application, it is also available included as a `cpdef` function named `_pari_divisors_small`.\n\nBranch/Commit: **[b042e23](https://github.com/sagemath/sagetrac-mirror/commit/b042e2397f53162786185aedf1e4b78f38e2960d)**\n\nReviewer: **David Loeffler**\n\nAuthor: **Vincent Delecroix**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19688_\n\n",
    "closed_at": "2015-12-22T19:49:42Z",
    "created_at": "2015-12-10T03:01:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.10",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Much fast divisors using PARI divisorsu",
    "type": "issue",
    "updated_at": "2015-12-22T19:49:42Z",
    "url": "https://github.com/sagemath/sage/issues/19688",
    "user": "https://github.com/videlec"
}
```
Using PARI we get a huge speedup for integer of small range. With the branch applied

```
sage: %timeit 100.divisors(method='pari')
1000000 loops, best of 3: 818 ns per loop
sage: %timeit 100.divisors(method='sage')
100000 loops, best of 3: 14.1 µs per loop
```

As it is critical for some application, it is also available included as a `cpdef` function named `_pari_divisors_small`.

Branch/Commit: **[b042e23](https://github.com/sagemath/sagetrac-mirror/commit/b042e2397f53162786185aedf1e4b78f38e2960d)**

Reviewer: **David Loeffler**

Author: **Vincent Delecroix**

_Issue created by migration from https://trac.sagemath.org/ticket/19688_





---

archive/issue_events_260857.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-12-10T03:01:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19688",
    "milestone_number": null,
    "milestone_title": "sage-6.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19688#event-260857"
}
```



---

archive/issue_events_260858.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-12-10T03:01:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19688",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "component: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19688#event-260858"
}
```



---

archive/issue_events_260859.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-12-10T03:01:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19688",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19688#event-260859"
}
```



---

archive/issue_events_260860.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-12-10T03:01:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19688",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19688#event-260860"
}
```



---

archive/issue_comments_283732.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d8b30e0d4590ae957171d04bd1a97d26990ccca8\">d8b30e0</a></td><td><code>a naive (and fast for small input) divisors function</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4eda3a07894afebe046b0a53641774dcec52f025\">4eda3a0</a></td><td><code>Trac 19688: divisors using PARI</code></td></tr></table>\n",
    "created_at": "2015-12-10T03:18:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19688#issuecomment-283732",
    "user": "https://github.com/videlec"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d8b30e0d4590ae957171d04bd1a97d26990ccca8">d8b30e0</a></td><td><code>a naive (and fast for small input) divisors function</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4eda3a07894afebe046b0a53641774dcec52f025">4eda3a0</a></td><td><code>Trac 19688: divisors using PARI</code></td></tr></table>




---

archive/issue_events_260861.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-12-10T03:18:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19688",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19688#event-260861"
}
```



---

archive/issue_comments_283733.json:
```json
{
    "body": "Commit: **[4eda3a0](https://github.com/sagemath/sagetrac-mirror/commit/4eda3a07894afebe046b0a53641774dcec52f025)**",
    "created_at": "2015-12-10T03:18:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19688#issuecomment-283733",
    "user": "https://github.com/videlec"
}
```

Commit: **[4eda3a0](https://github.com/sagemath/sagetrac-mirror/commit/4eda3a07894afebe046b0a53641774dcec52f025)**



---

archive/issue_comments_283734.json:
```json
{
    "body": "Branch: **[u/vdelecroix/19688](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/19688)**",
    "created_at": "2015-12-10T03:18:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19688#issuecomment-283734",
    "user": "https://github.com/videlec"
}
```

Branch: **[u/vdelecroix/19688](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/19688)**



---

archive/issue_comments_283735.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b042e2397f53162786185aedf1e4b78f38e2960d\">b042e23</a></td><td><code>Trac 19688: divisors using PARI</code></td></tr></table>\n",
    "created_at": "2015-12-10T03:19:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19688#issuecomment-283735",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b042e2397f53162786185aedf1e4b78f38e2960d">b042e23</a></td><td><code>Trac 19688: divisors using PARI</code></td></tr></table>




---

archive/issue_comments_283736.json:
```json
{
    "body": "Changed commit from **[4eda3a0](https://github.com/sagemath/sagetrac-mirror/commit/4eda3a07894afebe046b0a53641774dcec52f025)** to **[b042e23](https://github.com/sagemath/sagetrac-mirror/commit/b042e2397f53162786185aedf1e4b78f38e2960d)**",
    "created_at": "2015-12-10T03:19:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19688#issuecomment-283736",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[4eda3a0](https://github.com/sagemath/sagetrac-mirror/commit/4eda3a07894afebe046b0a53641774dcec52f025)** to **[b042e23](https://github.com/sagemath/sagetrac-mirror/commit/b042e2397f53162786185aedf1e4b78f38e2960d)**



---

archive/issue_comments_283737.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLooks fine to me.",
    "created_at": "2015-12-14T13:49:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19688#issuecomment-283737",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:3" align="right">Comment 3</div>

Looks fine to me.



---

archive/issue_events_260862.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2015-12-14T13:49:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19688",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19688#event-260862"
}
```



---

archive/issue_events_260863.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2015-12-14T13:49:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19688",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19688#event-260863"
}
```



---

archive/issue_comments_283738.json:
```json
{
    "body": "Reviewer: **David Loeffler**",
    "created_at": "2015-12-14T13:49:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19688#issuecomment-283738",
    "user": "https://github.com/loefflerd"
}
```

Reviewer: **David Loeffler**



---

archive/issue_comments_283739.json:
```json
{
    "body": "Changed branch from **[u/vdelecroix/19688](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/19688)** to **[b042e23](https://github.com/sagemath/sagetrac-mirror/commit/b042e2397f53162786185aedf1e4b78f38e2960d)**",
    "created_at": "2015-12-22T19:49:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19688#issuecomment-283739",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/vdelecroix/19688](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/19688)** to **[b042e23](https://github.com/sagemath/sagetrac-mirror/commit/b042e2397f53162786185aedf1e4b78f38e2960d)**



---

archive/issue_events_260864.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-12-22T19:49:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19688",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19688#event-260864"
}
```



---

archive/issue_events_260865.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "1000a45e371d468e86abcdbf1f8dc630e98c6fff",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-12-22T19:49:42Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19688",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19688#event-260865"
}
```
