# Issue 19786: giacpy segfault reported

archive/issues_019549.json:
```json
{
    "assignees": [],
    "body": "part of ticket #19781 where segfault were reported for giacpy.\n\nReviewer: **Jeroen Demeyer**\n\nComponent: **packages: experimental**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19786_\n\n",
    "closed_at": "2015-12-29T09:31:47Z",
    "created_at": "2015-12-27T13:10:40Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20experimental",
        "https://github.com/sagemath/sage/labels/worksforme"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "giacpy segfault reported",
    "type": "issue",
    "updated_at": "2015-12-29T09:31:47Z",
    "url": "https://github.com/sagemath/sage/issues/19786",
    "user": "https://github.com/frederichan-IMJPRG"
}
```
part of ticket #19781 where segfault were reported for giacpy.

Reviewer: **Jeroen Demeyer**

Component: **packages: experimental**

_Issue created by migration from https://trac.sagemath.org/ticket/19786_





---

archive/issue_events_279879.json:
```json
{
    "actor": "https://github.com/frederichan-IMJPRG",
    "created_at": "2015-12-27T13:10:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19786",
    "milestone_number": null,
    "milestone_title": "sage-7.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19786#event-279879"
}
```



---

archive/issue_events_279880.json:
```json
{
    "actor": "https://github.com/frederichan-IMJPRG",
    "created_at": "2015-12-27T13:10:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19786",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20experimental",
    "label_color": "000080",
    "label_name": "c: packages: experimental",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19786#event-279880"
}
```



---

archive/issue_events_279881.json:
```json
{
    "actor": "https://github.com/frederichan-IMJPRG",
    "created_at": "2015-12-27T13:10:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19786",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19786#event-279881"
}
```



---

archive/issue_events_279882.json:
```json
{
    "actor": "https://github.com/frederichan-IMJPRG",
    "created_at": "2015-12-27T13:10:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19786",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19786#event-279882"
}
```



---

archive/issue_comments_282841.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nUp to now I was able with sage7.0.beta1 to do without problems: \nmake distclean\nmake\nmake giac\nenv SAGE_CHECK=yes make giacpy\non a 64bit debian system.\n\nCan you precise if you got segfaults during the built of giacpy or during sage tests or with something at runtime?",
    "created_at": "2015-12-27T13:19:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19786",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19786#issuecomment-282841",
    "user": "https://github.com/frederichan-IMJPRG"
}
```

<div id="comment:1" align="right">comment:1</div>

Up to now I was able with sage7.0.beta1 to do without problems: 
make distclean
make
make giac
env SAGE_CHECK=yes make giacpy
on a 64bit debian system.

Can you precise if you got segfaults during the built of giacpy or during sage tests or with something at runtime?



---

archive/issue_comments_282842.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nMy first priority was getting the list of \"bad\" packages. Now that is done, so I'll inform you about `giacpy`.",
    "created_at": "2015-12-28T10:14:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19786",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19786#issuecomment-282842",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">comment:2</div>

My first priority was getting the list of "bad" packages. Now that is done, so I'll inform you about `giacpy`.



---

archive/issue_events_279883.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-29T09:31:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19786",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19786#event-279883"
}
```



---

archive/issue_events_279884.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-29T09:31:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19786",
    "label": "https://github.com/sagemath/sage/labels/worksforme",
    "label_color": "a6a6a6",
    "label_name": "worksforme",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19786#event-279884"
}
```



---

archive/issue_events_279885.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-29T09:31:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19786",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19786#event-279885"
}
```



---

archive/issue_comments_282843.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2015-12-29T09:31:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19786",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19786#issuecomment-282843",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_279886.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-29T09:31:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/19786",
    "milestone_number": null,
    "milestone_title": "sage-7.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19786#event-279886"
}
```



---

archive/issue_comments_282844.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nI cannot reproduce the problem.",
    "created_at": "2015-12-29T09:31:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19786",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19786#issuecomment-282844",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:3" align="right">comment:3</div>

I cannot reproduce the problem.
