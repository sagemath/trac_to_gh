# Issue 19489: Documentation Change for implicit_plot3d

archive/issues_019252.json:
```json
{
    "assignees": [],
    "body": "From the documentation,\n\n> Maeder\u2019s Owl (pretty but can\u2019t find an internet reference):\n> sage: u, v = var('u,v')sage: fx = v *cos(u) - 0.5* v2 * cos(2* u)sage: fy = -v *sin(u) - 0.5* v2 * sin(2* u)sage: fz = 4 *v1.5 * cos(3 *u / 2) / 3sage: parametric_plot3d([fx, fy, fz], (u, -2*pi, 2*pi), (v, 0, 1),plot_points = [90,90], frame=False, color=\"purple\")Graphics3d Object\n\n\n According to Wolfram Mathworld, Maeder's Owl is also known as Bour's  Minimal Surface. !http://mathworld.wolfram.com/BoursMinimalSurface.html\n\nCC:  @kcrisman @darijgr @tscrim @jm58660\n\nKeywords: **implicit_plot3d**\n\nBranch/Commit: **[460cacf](https://github.com/sagemath/sagetrac-mirror/commit/460cacfd343067dd6d954c5a0f754b16ae38eb4e)**\n\nReviewer: **Paul Masson**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19489_\n\n",
    "closed_at": "2016-08-29T16:56:25Z",
    "created_at": "2015-10-27T22:02:01Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/good%20first%20issue"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Documentation Change for implicit_plot3d",
    "type": "issue",
    "updated_at": "2016-08-29T16:56:25Z",
    "url": "https://github.com/sagemath/sage/issues/19489",
    "user": "https://github.com/sagetrac-JoalHeagney"
}
```
From the documentation,

> Maeder’s Owl (pretty but can’t find an internet reference):
> sage: u, v = var('u,v')sage: fx = v *cos(u) - 0.5* v2 * cos(2* u)sage: fy = -v *sin(u) - 0.5* v2 * sin(2* u)sage: fz = 4 *v1.5 * cos(3 *u / 2) / 3sage: parametric_plot3d([fx, fy, fz], (u, -2*pi, 2*pi), (v, 0, 1),plot_points = [90,90], frame=False, color="purple")Graphics3d Object


 According to Wolfram Mathworld, Maeder's Owl is also known as Bour's  Minimal Surface. !http://mathworld.wolfram.com/BoursMinimalSurface.html

CC:  @kcrisman @darijgr @tscrim @jm58660

Keywords: **implicit_plot3d**

Branch/Commit: **[460cacf](https://github.com/sagemath/sagetrac-mirror/commit/460cacfd343067dd6d954c5a0f754b16ae38eb4e)**

Reviewer: **Paul Masson**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/19489_





---

archive/issue_events_249094.json:
```json
{
    "actor": "https://github.com/sagetrac-JoalHeagney",
    "created_at": "2015-10-27T22:02:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19489",
    "milestone_number": null,
    "milestone_title": "sage-7.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19489#event-249094"
}
```



---

archive/issue_events_249095.json:
```json
{
    "actor": "https://github.com/sagetrac-JoalHeagney",
    "created_at": "2015-10-27T22:02:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19489",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19489#event-249095"
}
```



---

archive/issue_events_249096.json:
```json
{
    "actor": "https://github.com/sagetrac-JoalHeagney",
    "created_at": "2015-10-27T22:02:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19489",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19489#event-249096"
}
```



---

archive/issue_events_249097.json:
```json
{
    "actor": "https://github.com/sagetrac-JoalHeagney",
    "created_at": "2015-10-27T22:02:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19489",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19489#event-249097"
}
```



---

archive/issue_comments_279767.json:
```json
{
    "body": "Author: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2016-08-24T18:52:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19489",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19489#issuecomment-279767",
    "user": "https://github.com/fchapoton"
}
```

Author: **Frédéric Chapoton**



---

archive/issue_comments_279768.json:
```json
{
    "body": "Commit: **[9a1e1ad](https://github.com/sagemath/sagetrac-mirror/commit/9a1e1ad1ce6d0c02e525c892e82545c7a5df4965)**",
    "created_at": "2016-08-24T18:52:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19489",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19489#issuecomment-279768",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[9a1e1ad](https://github.com/sagemath/sagetrac-mirror/commit/9a1e1ad1ce6d0c02e525c892e82545c7a5df4965)**



---

archive/issue_events_249098.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-08-24T18:52:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19489",
    "label": "https://github.com/sagemath/sage/labels/good%20first%20issue",
    "label_color": "7057ff",
    "label_name": "good first issue",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19489#event-249098"
}
```



---

archive/issue_comments_279769.json:
```json
{
    "body": "Branch: **[u/chapoton/19489](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/19489)**",
    "created_at": "2016-08-24T18:52:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19489",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19489#issuecomment-279769",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/19489](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/19489)**



---

archive/issue_events_249099.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-08-24T18:52:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19489",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19489#event-249099"
}
```



---

archive/issue_events_249100.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-08-24T18:52:22Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/19489",
    "milestone_number": null,
    "milestone_title": "sage-6.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19489#event-249100"
}
```



---

archive/issue_events_249101.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-08-24T18:52:22Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19489",
    "milestone_number": null,
    "milestone_title": "sage-7.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19489#event-249101"
}
```



---

archive/issue_comments_279770.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9a1e1ad1ce6d0c02e525c892e82545c7a5df4965\">9a1e1ad</a></td><td><code>trac 19489 better doc for Maeder\u2019s Owl (link to wikipedia)</code></td></tr></table>\n",
    "created_at": "2016-08-24T18:52:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19489",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19489#issuecomment-279770",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9a1e1ad1ce6d0c02e525c892e82545c7a5df4965">9a1e1ad</a></td><td><code>trac 19489 better doc for Maeder’s Owl (link to wikipedia)</code></td></tr></table>




---

archive/issue_comments_279771.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nPlease put a double colon after the link to Boy's surface then set positive review from me.",
    "created_at": "2016-08-24T21:10:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19489",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19489#issuecomment-279771",
    "user": "https://github.com/paulmasson"
}
```

<a id='comment:2'>Comment 2:</a>
Please put a double colon after the link to Boy's surface then set positive review from me.



---

archive/issue_comments_279772.json:
```json
{
    "body": "Reviewer: **Paul Masson**",
    "created_at": "2016-08-24T21:10:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19489",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19489#issuecomment-279772",
    "user": "https://github.com/paulmasson"
}
```

Reviewer: **Paul Masson**



---

archive/issue_comments_279773.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/460cacfd343067dd6d954c5a0f754b16ae38eb4e\">460cacf</a></td><td><code>trac 19489 one detail</code></td></tr></table>\n",
    "created_at": "2016-08-25T06:05:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19489",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19489#issuecomment-279773",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:3'>Comment 3:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/460cacfd343067dd6d954c5a0f754b16ae38eb4e">460cacf</a></td><td><code>trac 19489 one detail</code></td></tr></table>




---

archive/issue_comments_279774.json:
```json
{
    "body": "Changed commit from **[9a1e1ad](https://github.com/sagemath/sagetrac-mirror/commit/9a1e1ad1ce6d0c02e525c892e82545c7a5df4965)** to **[460cacf](https://github.com/sagemath/sagetrac-mirror/commit/460cacfd343067dd6d954c5a0f754b16ae38eb4e)**",
    "created_at": "2016-08-25T06:05:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19489",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19489#issuecomment-279774",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[9a1e1ad](https://github.com/sagemath/sagetrac-mirror/commit/9a1e1ad1ce6d0c02e525c892e82545c7a5df4965)** to **[460cacf](https://github.com/sagemath/sagetrac-mirror/commit/460cacfd343067dd6d954c5a0f754b16ae38eb4e)**



---

archive/issue_comments_279775.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\ndone, thanks for the review",
    "created_at": "2016-08-25T06:07:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19489",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19489#issuecomment-279775",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:4'>Comment 4:</a>
done, thanks for the review



---

archive/issue_events_249102.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-08-25T06:07:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19489",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19489#event-249102"
}
```



---

archive/issue_events_249103.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-08-25T06:07:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19489",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19489#event-249103"
}
```



---

archive/issue_comments_279776.json:
```json
{
    "body": "Changed branch from **[u/chapoton/19489](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/19489)** to **[460cacf](https://github.com/sagemath/sagetrac-mirror/commit/460cacfd343067dd6d954c5a0f754b16ae38eb4e)**",
    "created_at": "2016-08-29T16:56:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19489",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19489#issuecomment-279776",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/19489](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/19489)** to **[460cacf](https://github.com/sagemath/sagetrac-mirror/commit/460cacfd343067dd6d954c5a0f754b16ae38eb4e)**



---

archive/issue_events_249104.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-08-29T16:56:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19489",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19489#event-249104"
}
```



---

archive/issue_events_249105.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "5fbccc40d0dcbd282d5cfc808dd126a9cce4ca54",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-08-29T16:56:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19489",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19489#event-249105"
}
```
