# Issue 19829: Cache the results of monoid_generators for IndexedFreeMonoid

archive/issues_019592.json:
```json
{
    "assignees": [],
    "body": "We should cache this because it can be relatively expensive to compute and I have a few tight loops where this becomes a major slowdown.\n\nAssignee: **sage-combinat**\n\nCC:  sage-combinat @nthiery @darijgr\n\nBranch/Commit: **[0020e67](https://github.com/sagemath/sagetrac-mirror/commit/0020e6781706fad7329dfa8755c84fc8e3236f21)**\n\nReviewer: **Darij Grinberg**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19829_\n\n",
    "closed_at": "2016-01-12T22:07:10Z",
    "created_at": "2016-01-04T11:05:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/performance",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Cache the results of monoid_generators for IndexedFreeMonoid",
    "type": "issue",
    "updated_at": "2016-01-12T22:07:10Z",
    "url": "https://github.com/sagemath/sage/issues/19829",
    "user": "https://github.com/tscrim"
}
```
We should cache this because it can be relatively expensive to compute and I have a few tight loops where this becomes a major slowdown.

Assignee: **sage-combinat**

CC:  sage-combinat @nthiery @darijgr

Branch/Commit: **[0020e67](https://github.com/sagemath/sagetrac-mirror/commit/0020e6781706fad7329dfa8755c84fc8e3236f21)**

Reviewer: **Darij Grinberg**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/19829_





---

archive/issue_events_262585.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-01-04T11:05:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19829",
    "milestone_number": null,
    "milestone_title": "sage-7.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19829#event-262585"
}
```



---

archive/issue_events_262586.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-01-04T11:05:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19829",
    "label": "https://github.com/sagemath/sage/labels/performance",
    "label_color": "696969",
    "label_name": "performance",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19829#event-262586"
}
```



---

archive/issue_events_262587.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-01-04T11:05:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19829",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19829#event-262587"
}
```



---

archive/issue_events_262588.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-01-04T11:05:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19829",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19829#event-262588"
}
```



---

archive/issue_comments_286091.json:
```json
{
    "body": "Branch: **[public/monoids/cache_indexed_monoid_generators-19829](https://github.com/sagemath/sagetrac-mirror/tree/public/monoids/cache_indexed_monoid_generators-19829)**",
    "created_at": "2016-01-04T11:06:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19829",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19829#issuecomment-286091",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/monoids/cache_indexed_monoid_generators-19829](https://github.com/sagemath/sagetrac-mirror/tree/public/monoids/cache_indexed_monoid_generators-19829)**



---

archive/issue_comments_286092.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0020e6781706fad7329dfa8755c84fc8e3236f21\">0020e67</a></td><td><code>Cache the monoid (resp. group) generators for the indexed free monoid (resp. group).</code></td></tr></table>\n",
    "created_at": "2016-01-04T11:06:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19829",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19829#issuecomment-286092",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0020e6781706fad7329dfa8755c84fc8e3236f21">0020e67</a></td><td><code>Cache the monoid (resp. group) generators for the indexed free monoid (resp. group).</code></td></tr></table>




---

archive/issue_comments_286093.json:
```json
{
    "body": "Commit: **[0020e67](https://github.com/sagemath/sagetrac-mirror/commit/0020e6781706fad7329dfa8755c84fc8e3236f21)**",
    "created_at": "2016-01-04T11:06:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19829",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19829#issuecomment-286093",
    "user": "https://github.com/tscrim"
}
```

Commit: **[0020e67](https://github.com/sagemath/sagetrac-mirror/commit/0020e6781706fad7329dfa8755c84fc8e3236f21)**



---

archive/issue_events_262589.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-01-04T11:06:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19829",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19829#event-262589"
}
```



---

archive/issue_events_262590.json:
```json
{
    "actor": "https://github.com/darijgr",
    "created_at": "2016-01-12T13:23:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19829",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19829#event-262590"
}
```



---

archive/issue_events_262591.json:
```json
{
    "actor": "https://github.com/darijgr",
    "created_at": "2016-01-12T13:23:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19829",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19829#event-262591"
}
```



---

archive/issue_comments_286094.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nLGTM (which, at this length, is little surprising :) ).",
    "created_at": "2016-01-12T13:23:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19829",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19829#issuecomment-286094",
    "user": "https://github.com/darijgr"
}
```

<a id='comment:3'>Comment 3:</a>
LGTM (which, at this length, is little surprising :) ).



---

archive/issue_comments_286095.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nThanks!",
    "created_at": "2016-01-12T15:52:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19829",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19829#issuecomment-286095",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:4'>Comment 4:</a>
Thanks!



---

archive/issue_comments_286096.json:
```json
{
    "body": "Reviewer: **Darij Grinberg**",
    "created_at": "2016-01-12T15:52:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19829",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19829#issuecomment-286096",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Darij Grinberg**



---

archive/issue_events_262592.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-01-12T22:07:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19829",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19829#event-262592"
}
```



---

archive/issue_events_262593.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "e1776596ad9d3fa13cb3580f2c3cf46defd45793",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-01-12T22:07:10Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19829",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19829#event-262593"
}
```



---

archive/issue_comments_286097.json:
```json
{
    "body": "Changed branch from **[public/monoids/cache_indexed_monoid_generators-19829](https://github.com/sagemath/sagetrac-mirror/tree/public/monoids/cache_indexed_monoid_generators-19829)** to **[0020e67](https://github.com/sagemath/sagetrac-mirror/commit/0020e6781706fad7329dfa8755c84fc8e3236f21)**",
    "created_at": "2016-01-12T22:07:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19829",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19829#issuecomment-286097",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/monoids/cache_indexed_monoid_generators-19829](https://github.com/sagemath/sagetrac-mirror/tree/public/monoids/cache_indexed_monoid_generators-19829)** to **[0020e67](https://github.com/sagemath/sagetrac-mirror/commit/0020e6781706fad7329dfa8755c84fc8e3236f21)**
