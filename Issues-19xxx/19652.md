# Issue 19652: Fix one cryptominisat optional doctest

archive/issues_019415.json:
```json
{
    "assignees": [],
    "body": "Fix this:\n\n```\nsage -t --warn-long 27.7 src/sage/rings/polynomial/multi_polynomial_sequence.py\n**********************************************************************\nFile \"src/sage/rings/polynomial/multi_polynomial_sequence.py\", line 1381, in sage.rings.polynomial.multi_polynomial_sequence.PolynomialSequence_gf2.solve\nFailed example:\n    sol = S.solve(algorithm='sat'); sol  # optional - cryptominisat\nExpected nothing\nGot:\n    [{y: 1, z: 0, x: 0}]\n**********************************************************************\n```\n\nWas reported here:\nhttps://groups.google.com/d/msg/sage-release/bczbm92zUQ4/0sBtj-ZzCQAJ\n\nBranch/Commit: **[7b95c8c](https://github.com/sagemath/sagetrac-mirror/commit/7b95c8cc19d283de2e0e7cfc1d809a0e10abc79f)**\n\nReviewer: **Volker Braun**\n\nAuthor: **S\u00e9bastien Labb\u00e9**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19652_\n\n",
    "closed_at": "2015-12-01T17:57:29Z",
    "created_at": "2015-11-30T14:38:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20cryptography",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.10",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix one cryptominisat optional doctest",
    "type": "issue",
    "updated_at": "2015-12-01T17:57:29Z",
    "url": "https://github.com/sagemath/sage/issues/19652",
    "user": "https://github.com/seblabbe"
}
```
Fix this:

```
sage -t --warn-long 27.7 src/sage/rings/polynomial/multi_polynomial_sequence.py
**********************************************************************
File "src/sage/rings/polynomial/multi_polynomial_sequence.py", line 1381, in sage.rings.polynomial.multi_polynomial_sequence.PolynomialSequence_gf2.solve
Failed example:
    sol = S.solve(algorithm='sat'); sol  # optional - cryptominisat
Expected nothing
Got:
    [{y: 1, z: 0, x: 0}]
**********************************************************************
```

Was reported here:
https://groups.google.com/d/msg/sage-release/bczbm92zUQ4/0sBtj-ZzCQAJ

Branch/Commit: **[7b95c8c](https://github.com/sagemath/sagetrac-mirror/commit/7b95c8cc19d283de2e0e7cfc1d809a0e10abc79f)**

Reviewer: **Volker Braun**

Author: **Sébastien Labbé**

_Issue created by migration from https://trac.sagemath.org/ticket/19652_





---

archive/issue_events_260420.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2015-11-30T14:38:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19652",
    "milestone_number": null,
    "milestone_title": "sage-6.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19652#event-260420"
}
```



---

archive/issue_events_260421.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2015-11-30T14:38:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19652",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20cryptography",
    "label_color": "0000ff",
    "label_name": "component: cryptography",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19652#event-260421"
}
```



---

archive/issue_events_260422.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2015-11-30T14:38:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19652",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19652#event-260422"
}
```



---

archive/issue_events_260423.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2015-11-30T14:38:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19652",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19652#event-260423"
}
```



---

archive/issue_comments_283078.json:
```json
{
    "body": "Branch: **[u/slabbe/19652](https://github.com/sagemath/sagetrac-mirror/tree/u/slabbe/19652)**",
    "created_at": "2015-11-30T14:41:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19652",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19652#issuecomment-283078",
    "user": "https://github.com/seblabbe"
}
```

Branch: **[u/slabbe/19652](https://github.com/sagemath/sagetrac-mirror/tree/u/slabbe/19652)**



---

archive/issue_comments_283079.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7b95c8cc19d283de2e0e7cfc1d809a0e10abc79f\">7b95c8c</a></td><td><code>Trac #19652: fix cryptominisat optional broken doctest</code></td></tr></table>\n",
    "created_at": "2015-11-30T14:41:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19652",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19652#issuecomment-283079",
    "user": "https://github.com/seblabbe"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7b95c8cc19d283de2e0e7cfc1d809a0e10abc79f">7b95c8c</a></td><td><code>Trac #19652: fix cryptominisat optional broken doctest</code></td></tr></table>




---

archive/issue_comments_283080.json:
```json
{
    "body": "Commit: **[7b95c8c](https://github.com/sagemath/sagetrac-mirror/commit/7b95c8cc19d283de2e0e7cfc1d809a0e10abc79f)**",
    "created_at": "2015-11-30T14:41:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19652",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19652#issuecomment-283080",
    "user": "https://github.com/seblabbe"
}
```

Commit: **[7b95c8c](https://github.com/sagemath/sagetrac-mirror/commit/7b95c8cc19d283de2e0e7cfc1d809a0e10abc79f)**



---

archive/issue_events_260424.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2015-11-30T14:41:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19652",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19652#event-260424"
}
```



---

archive/issue_comments_283081.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nlgtm, you can set it to positive review once you have your name filled in",
    "created_at": "2015-11-30T15:08:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19652",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19652#issuecomment-283081",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:2" align="right">Comment 2</div>

lgtm, you can set it to positive review once you have your name filled in



---

archive/issue_comments_283082.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2015-11-30T15:08:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19652",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19652#issuecomment-283082",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_events_260425.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2015-11-30T15:10:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19652",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19652#event-260425"
}
```



---

archive/issue_events_260426.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2015-11-30T15:10:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19652",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19652#event-260426"
}
```



---

archive/issue_comments_283083.json:
```json
{
    "body": "Author: **S\u00e9bastien Labb\u00e9**",
    "created_at": "2015-11-30T15:10:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19652",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19652#issuecomment-283083",
    "user": "https://github.com/seblabbe"
}
```

Author: **Sébastien Labbé**



---

archive/issue_comments_283084.json:
```json
{
    "body": "Changed branch from **[u/slabbe/19652](https://github.com/sagemath/sagetrac-mirror/tree/u/slabbe/19652)** to **[7b95c8c](https://github.com/sagemath/sagetrac-mirror/commit/7b95c8cc19d283de2e0e7cfc1d809a0e10abc79f)**",
    "created_at": "2015-12-01T17:57:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19652",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19652#issuecomment-283084",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/slabbe/19652](https://github.com/sagemath/sagetrac-mirror/tree/u/slabbe/19652)** to **[7b95c8c](https://github.com/sagemath/sagetrac-mirror/commit/7b95c8cc19d283de2e0e7cfc1d809a0e10abc79f)**



---

archive/issue_events_260427.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-12-01T17:57:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19652",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19652#event-260427"
}
```



---

archive/issue_events_260428.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "bd07f76db4f0946dda16b9875598bc87fc6158f2",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-12-01T17:57:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19652",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19652#event-260428"
}
```
