# Issue 19671: Ignore all exceptions in ._sage_doc_ attribute access

archive/issues_019434.json:
```json
{
    "assignees": [],
    "body": "The current docbuilder cannot cope with `obj._sage_doc_` accesses raising `RuntimeError` for example.\n\nBranch/Commit: **[c9980ee](https://github.com/sagemath/sagetrac-mirror/commit/c9980ee58ee8c8a5464fd8f5513bfb28d7ae9ed2)**\n\nReviewer: **Fran\u00e7ois Bissey, Karl-Dieter Crisman**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19671_\n\n",
    "closed_at": "2015-12-22T19:50:44Z",
    "created_at": "2015-12-05T08:28:57Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.10",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Ignore all exceptions in ._sage_doc_ attribute access",
    "type": "issue",
    "updated_at": "2015-12-22T19:50:44Z",
    "url": "https://github.com/sagemath/sage/issues/19671",
    "user": "https://github.com/jdemeyer"
}
```
The current docbuilder cannot cope with `obj._sage_doc_` accesses raising `RuntimeError` for example.

Branch/Commit: **[c9980ee](https://github.com/sagemath/sagetrac-mirror/commit/c9980ee58ee8c8a5464fd8f5513bfb28d7ae9ed2)**

Reviewer: **François Bissey, Karl-Dieter Crisman**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/19671_





---

archive/issue_events_260622.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-05T08:28:57Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19671",
    "milestone_number": null,
    "milestone_title": "sage-6.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19671#event-260622"
}
```



---

archive/issue_events_260623.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-05T08:28:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19671",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19671#event-260623"
}
```



---

archive/issue_events_260624.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-05T08:28:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19671",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19671#event-260624"
}
```



---

archive/issue_events_260625.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-05T08:28:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19671",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19671#event-260625"
}
```



---

archive/issue_events_260626.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-05T08:32:11Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/19671",
    "title_is": "Ignore all exceptions in ._sage_obj_ attribute access",
    "title_was": "FIx docbuild for sagenb upgrade",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19671#event-260626"
}
```



---

archive/issue_comments_283493.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-The current docbuilder cannot cope with `_sage_obj_()` calls returning `RuntimeError`.\n+The current docbuilder cannot cope with `obj._sage_obj_` accesses raising `RuntimeError` for example.\n``````\n",
    "created_at": "2015-12-05T08:32:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19671",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19671#issuecomment-283493",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-The current docbuilder cannot cope with `_sage_obj_()` calls returning `RuntimeError`.
+The current docbuilder cannot cope with `obj._sage_obj_` accesses raising `RuntimeError` for example.
``````




---

archive/issue_comments_283494.json:
```json
{
    "body": "Branch: **[u/jdemeyer/fix_docbuild_for_sagenb_upgrade](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix_docbuild_for_sagenb_upgrade)**",
    "created_at": "2015-12-05T10:16:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19671",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19671#issuecomment-283494",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/fix_docbuild_for_sagenb_upgrade](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix_docbuild_for_sagenb_upgrade)**



---

archive/issue_comments_283495.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c9980ee58ee8c8a5464fd8f5513bfb28d7ae9ed2\">c9980ee</a></td><td><code>Allow all exceptions in obj._sage_obj_</code></td></tr></table>\n",
    "created_at": "2015-12-05T11:13:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19671",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19671#issuecomment-283495",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:3'>Comment 3:</a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c9980ee58ee8c8a5464fd8f5513bfb28d7ae9ed2">c9980ee</a></td><td><code>Allow all exceptions in obj._sage_obj_</code></td></tr></table>




---

archive/issue_comments_283496.json:
```json
{
    "body": "Commit: **[c9980ee](https://github.com/sagemath/sagetrac-mirror/commit/c9980ee58ee8c8a5464fd8f5513bfb28d7ae9ed2)**",
    "created_at": "2015-12-05T11:13:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19671",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19671#issuecomment-283496",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[c9980ee](https://github.com/sagemath/sagetrac-mirror/commit/c9980ee58ee8c8a5464fd8f5513bfb28d7ae9ed2)**



---

archive/issue_comments_283497.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-The current docbuilder cannot cope with `obj._sage_obj_` accesses raising `RuntimeError` for example.\n+The current docbuilder cannot cope with `obj._sage_doc_` accesses raising `RuntimeError` for example.\n``````\n",
    "created_at": "2015-12-05T11:13:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19671",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19671#issuecomment-283497",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-The current docbuilder cannot cope with `obj._sage_obj_` accesses raising `RuntimeError` for example.
+The current docbuilder cannot cope with `obj._sage_doc_` accesses raising `RuntimeError` for example.
``````




---

archive/issue_events_260627.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-05T11:13:33Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/19671",
    "title_is": "Ignore all exceptions in ._sage_doc_ attribute access",
    "title_was": "Ignore all exceptions in ._sage_obj_ attribute access",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19671#event-260627"
}
```



---

archive/issue_events_260628.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-05T11:16:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19671",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19671#event-260628"
}
```



---

archive/issue_comments_283498.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nAh, I see now why Sloane.\n\nAs I said in #19616, it does build doc successfully with these changes, though I don't have my Sage install convenient to actually try the tests.  Anyone else could review that.",
    "created_at": "2015-12-05T14:51:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19671",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19671#issuecomment-283498",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:6'>Comment 6:</a>
Ah, I see now why Sloane.

As I said in #19616, it does build doc successfully with these changes, though I don't have my Sage install convenient to actually try the tests.  Anyone else could review that.



---

archive/issue_comments_283499.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nReplying to [@kcrisman](#comment%3A6):\n> try the tests.\n\nYou mean doctests or something else? The doctests pass.",
    "created_at": "2015-12-05T15:16:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19671",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19671#issuecomment-283499",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:7'>Comment 7:</a>
Replying to [@kcrisman](#comment%3A6):
> try the tests.

You mean doctests or something else? The doctests pass.



---

archive/issue_comments_283500.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nI mean doctests, but I didn't know if one was allowed to use the patchbot to confirm this or test it oneself.  I usually try them 'by hand' and a few other things, and I won't be able to do more with that until at least Monday, probably Wednesday.  If you think this is sufficient though then I certainly trust patchbot + you and you can set it to positive review.",
    "created_at": "2015-12-05T15:53:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19671",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19671#issuecomment-283500",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:8'>Comment 8:</a>
I mean doctests, but I didn't know if one was allowed to use the patchbot to confirm this or test it oneself.  I usually try them 'by hand' and a few other things, and I won't be able to do more with that until at least Monday, probably Wednesday.  If you think this is sufficient though then I certainly trust patchbot + you and you can set it to positive review.



---

archive/issue_comments_283501.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nI think it is sufficient.",
    "created_at": "2015-12-10T07:29:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19671",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19671#issuecomment-283501",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:9'>Comment 9:</a>
I think it is sufficient.



---

archive/issue_events_260629.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2015-12-10T07:29:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19671",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19671#event-260629"
}
```



---

archive/issue_events_260630.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2015-12-10T07:29:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19671",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19671#event-260630"
}
```



---

archive/issue_comments_283502.json:
```json
{
    "body": "Reviewer: **Fran\u00e7ois Bissey**",
    "created_at": "2015-12-10T07:29:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19671",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19671#issuecomment-283502",
    "user": "https://github.com/kiwifb"
}
```

Reviewer: **François Bissey**



---

archive/issue_comments_283503.json:
```json
{
    "body": "Changed reviewer from **Fran\u00e7ois Bissey** to **Fran\u00e7ois Bissey, Karl-Dieter Crisman**",
    "created_at": "2015-12-10T14:12:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19671",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19671#issuecomment-283503",
    "user": "https://github.com/kcrisman"
}
```

Changed reviewer from **François Bissey** to **François Bissey, Karl-Dieter Crisman**



---

archive/issue_comments_283504.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nThanks Fran\u00e7ois.",
    "created_at": "2015-12-10T14:12:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19671",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19671#issuecomment-283504",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:10'>Comment 10:</a>
Thanks François.



---

archive/issue_events_260631.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-12-22T19:50:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19671",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19671#event-260631"
}
```



---

archive/issue_events_260632.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "3296bb325c4bbe61141185c0ba658e86206b57ee",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-12-22T19:50:44Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19671",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19671#event-260632"
}
```



---

archive/issue_comments_283505.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/fix_docbuild_for_sagenb_upgrade](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix_docbuild_for_sagenb_upgrade)** to **[c9980ee](https://github.com/sagemath/sagetrac-mirror/commit/c9980ee58ee8c8a5464fd8f5513bfb28d7ae9ed2)**",
    "created_at": "2015-12-22T19:50:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19671",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19671#issuecomment-283505",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/fix_docbuild_for_sagenb_upgrade](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/fix_docbuild_for_sagenb_upgrade)** to **[c9980ee](https://github.com/sagemath/sagetrac-mirror/commit/c9980ee58ee8c8a5464fd8f5513bfb28d7ae9ed2)**
