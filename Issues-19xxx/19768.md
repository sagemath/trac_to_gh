# Issue 19768: Optimize cache_key()

archive/issues_019531.json:
```json
{
    "assignees": [],
    "body": "Optimize `cached_function` and friends for unhashable keys. The main part of the optimization is done by using `cdef` functions and attributes instead of Python ones.\n\nBranch/Commit: **[6bdfee3](https://github.com/sagemath/sagetrac-mirror/commit/6bdfee355e6eb1df075724a5379283fa4827094c)**\n\nReviewer: **Volker Braun**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19768_\n\n",
    "closed_at": "2015-12-29T08:53:40Z",
    "created_at": "2015-12-23T14:30:02Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Optimize cache_key()",
    "type": "issue",
    "updated_at": "2015-12-29T08:53:40Z",
    "url": "https://github.com/sagemath/sage/issues/19768",
    "user": "https://github.com/jdemeyer"
}
```
Optimize `cached_function` and friends for unhashable keys. The main part of the optimization is done by using `cdef` functions and attributes instead of Python ones.

Branch/Commit: **[6bdfee3](https://github.com/sagemath/sagetrac-mirror/commit/6bdfee355e6eb1df075724a5379283fa4827094c)**

Reviewer: **Volker Braun**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/19768_





---

archive/issue_events_261820.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-23T14:30:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19768",
    "milestone_number": null,
    "milestone_title": "sage-7.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19768#event-261820"
}
```



---

archive/issue_events_261821.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-23T14:30:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19768",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19768#event-261821"
}
```



---

archive/issue_events_261822.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-23T14:30:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19768",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19768#event-261822"
}
```



---

archive/issue_events_261823.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-23T16:07:31Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/19768",
    "title_is": "Optimize cache_key()",
    "title_was": "Optimize cached functions",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19768#event-261823"
}
```



---

archive/issue_comments_284955.json:
```json
{
    "body": "Branch: **[u/jdemeyer/optimize_cached_functions](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/optimize_cached_functions)**",
    "created_at": "2015-12-23T18:18:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19768",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19768#issuecomment-284955",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/optimize_cached_functions](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/optimize_cached_functions)**



---

archive/issue_comments_284956.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6bdfee355e6eb1df075724a5379283fa4827094c\">6bdfee3</a></td><td><code>Optimize cache_key()</code></td></tr></table>\n",
    "created_at": "2015-12-23T18:20:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19768",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19768#issuecomment-284956",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:3" align="right">Comment 3</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6bdfee355e6eb1df075724a5379283fa4827094c">6bdfee3</a></td><td><code>Optimize cache_key()</code></td></tr></table>




---

archive/issue_comments_284957.json:
```json
{
    "body": "Commit: **[6bdfee3](https://github.com/sagemath/sagetrac-mirror/commit/6bdfee355e6eb1df075724a5379283fa4827094c)**",
    "created_at": "2015-12-23T18:20:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19768",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19768#issuecomment-284957",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[6bdfee3](https://github.com/sagemath/sagetrac-mirror/commit/6bdfee355e6eb1df075724a5379283fa4827094c)**



---

archive/issue_events_261824.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-12-23T18:20:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19768",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19768#event-261824"
}
```



---

archive/issue_comments_284958.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nLGTM but can we have a description (which will also become part of the git history)?",
    "created_at": "2015-12-28T15:01:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19768",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19768#issuecomment-284958",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:4" align="right">Comment 4</div>

LGTM but can we have a description (which will also become part of the git history)?



---

archive/issue_comments_284959.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2015-12-28T15:13:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19768",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19768#issuecomment-284959",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Volker Braun**



---

archive/issue_comments_284960.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-\n+Optimize `cached_function` and friends for unhashable keys. The main part of the optimization is done by using `cdef` functions and attributes instead of Python ones.\n``````\n",
    "created_at": "2015-12-28T15:13:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19768",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19768#issuecomment-284960",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-
+Optimize `cached_function` and friends for unhashable keys. The main part of the optimization is done by using `cdef` functions and attributes instead of Python ones.
``````




---

archive/issue_events_261825.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-12-28T15:38:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19768",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19768#event-261825"
}
```



---

archive/issue_events_261826.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-12-28T15:38:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19768",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19768#event-261826"
}
```



---

archive/issue_events_261827.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-12-29T08:53:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19768",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19768#event-261827"
}
```



---

archive/issue_events_261828.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "bc4faec0a6b06471e41aaa884686807260592c41",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-12-29T08:53:40Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19768",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19768#event-261828"
}
```



---

archive/issue_comments_284961.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/optimize_cached_functions](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/optimize_cached_functions)** to **[6bdfee3](https://github.com/sagemath/sagetrac-mirror/commit/6bdfee355e6eb1df075724a5379283fa4827094c)**",
    "created_at": "2015-12-29T08:53:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19768",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19768#issuecomment-284961",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/optimize_cached_functions](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/optimize_cached_functions)** to **[6bdfee3](https://github.com/sagemath/sagetrac-mirror/commit/6bdfee355e6eb1df075724a5379283fa4827094c)**
