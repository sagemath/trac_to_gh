# Issue 19789: Capital letters for "cartesian"

archive/issues_019552.json:
```json
{
    "assignees": [],
    "body": "This trivial but long patch only changes first letter of word \"cartesian\" to capital letter in documentation and error strings.\n\nThis is almost mechanical change,\n\n```\nfind src/sage -name '*.pxd' -or -name '*.py' -or -name '*.pyx' | xargs perl -p -e 's|(?<![_a-z.])cartesian(?!_[a-z])|Cartesian|g;' -i.bak\n```\n\nwith one manual correction.\n\n\nCC:  @fchapoton\n\nBranch/Commit: **[dacb6d1](https://github.com/sagemath/sagetrac-mirror/commit/dacb6d11137eb6d0f645a29b7433236bae5ab956)**\n\nReviewer: **Volker Braun**\n\nAuthor: **Jori M\u00e4ntysalo**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19789_\n\n",
    "closed_at": "2015-12-29T08:53:44Z",
    "created_at": "2015-12-28T07:43:42Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Capital letters for \"cartesian\"",
    "type": "issue",
    "updated_at": "2015-12-29T08:53:44Z",
    "url": "https://github.com/sagemath/sage/issues/19789",
    "user": "https://github.com/jm58660"
}
```
This trivial but long patch only changes first letter of word "cartesian" to capital letter in documentation and error strings.

This is almost mechanical change,

```
find src/sage -name '*.pxd' -or -name '*.py' -or -name '*.pyx' | xargs perl -p -e 's|(?<![_a-z.])cartesian(?!_[a-z])|Cartesian|g;' -i.bak
```

with one manual correction.


CC:  @fchapoton

Branch/Commit: **[dacb6d1](https://github.com/sagemath/sagetrac-mirror/commit/dacb6d11137eb6d0f645a29b7433236bae5ab956)**

Reviewer: **Volker Braun**

Author: **Jori Mäntysalo**

_Issue created by migration from https://trac.sagemath.org/ticket/19789_





---

archive/issue_events_262067.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2015-12-28T07:43:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19789",
    "milestone_number": null,
    "milestone_title": "sage-7.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19789#event-262067"
}
```



---

archive/issue_events_262068.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2015-12-28T07:43:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19789",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19789#event-262068"
}
```



---

archive/issue_events_262069.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2015-12-28T07:43:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19789",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19789#event-262069"
}
```



---

archive/issue_events_262070.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2015-12-28T07:43:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19789",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19789#event-262070"
}
```



---

archive/issue_comments_285389.json:
```json
{
    "body": "Branch: **[u/jmantysalo/cartesian-caps](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/cartesian-caps)**",
    "created_at": "2015-12-28T07:44:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19789",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19789#issuecomment-285389",
    "user": "https://github.com/jm58660"
}
```

Branch: **[u/jmantysalo/cartesian-caps](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/cartesian-caps)**



---

archive/issue_events_262071.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2015-12-28T07:44:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19789",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19789#event-262071"
}
```



---

archive/issue_comments_285390.json:
```json
{
    "body": "Changed branch from **[u/jmantysalo/cartesian-caps](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/cartesian-caps)** to none",
    "created_at": "2015-12-28T07:44:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19789",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19789#issuecomment-285390",
    "user": "https://github.com/jm58660"
}
```

Changed branch from **[u/jmantysalo/cartesian-caps](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/cartesian-caps)** to none



---

archive/issue_comments_285391.json:
```json
{
    "body": "Branch: **[u/jmantysalo/cartesian-caps](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/cartesian-caps)**",
    "created_at": "2015-12-28T07:45:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19789",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19789#issuecomment-285391",
    "user": "https://github.com/jm58660"
}
```

Branch: **[u/jmantysalo/cartesian-caps](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/cartesian-caps)**



---

archive/issue_comments_285392.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b5957bd90e0be09c831aff3bfec43be307475db0\">b5957bd</a></td><td><code>Capital: 'cartesian' to 'Cartesian'.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/dacb6d11137eb6d0f645a29b7433236bae5ab956\">dacb6d1</a></td><td><code>Error in src/sage/libs/polybori/decl.pxd.</code></td></tr></table>\n",
    "created_at": "2015-12-28T07:45:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19789",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19789#issuecomment-285392",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:3" align="right">Comment 3</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b5957bd90e0be09c831aff3bfec43be307475db0">b5957bd</a></td><td><code>Capital: 'cartesian' to 'Cartesian'.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/dacb6d11137eb6d0f645a29b7433236bae5ab956">dacb6d1</a></td><td><code>Error in src/sage/libs/polybori/decl.pxd.</code></td></tr></table>




---

archive/issue_comments_285393.json:
```json
{
    "body": "Commit: **[dacb6d1](https://github.com/sagemath/sagetrac-mirror/commit/dacb6d11137eb6d0f645a29b7433236bae5ab956)**",
    "created_at": "2015-12-28T07:45:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19789",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19789#issuecomment-285393",
    "user": "https://github.com/jm58660"
}
```

Commit: **[dacb6d1](https://github.com/sagemath/sagetrac-mirror/commit/dacb6d11137eb6d0f645a29b7433236bae5ab956)**



---

archive/issue_comments_285394.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2015-12-28T11:05:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19789",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19789#issuecomment-285394",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_events_262072.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-12-28T11:05:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19789",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19789#event-262072"
}
```



---

archive/issue_events_262073.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-12-28T11:05:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19789",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19789#event-262073"
}
```



---

archive/issue_comments_285395.json:
```json
{
    "body": "Changed branch from **[u/jmantysalo/cartesian-caps](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/cartesian-caps)** to **[dacb6d1](https://github.com/sagemath/sagetrac-mirror/commit/dacb6d11137eb6d0f645a29b7433236bae5ab956)**",
    "created_at": "2015-12-29T08:53:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19789",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19789#issuecomment-285395",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jmantysalo/cartesian-caps](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/cartesian-caps)** to **[dacb6d1](https://github.com/sagemath/sagetrac-mirror/commit/dacb6d11137eb6d0f645a29b7433236bae5ab956)**



---

archive/issue_events_262074.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-12-29T08:53:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19789",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19789#event-262074"
}
```



---

archive/issue_events_262075.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "a84c053ddef3c71fc64cfa9f3fa55ac9cea15f2d",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-12-29T08:53:44Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19789",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19789#event-262075"
}
```
