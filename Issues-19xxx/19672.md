...
```json
{
    "body": "<a id='comment:2'></a>You should write `| $(SAGERUNTIME)` in the dependencies. That means that `giac` will be built after the Sage runtime, but not *rebuilt* every time that the Sage runtime gets rebuilt.\n\nAnd don't forget your name as author.",
...
<a id='comment:2'></a>You should write `| $(SAGERUNTIME)` in the dependencies. That means that `giac` will be built after the Sage runtime, but not *rebuilt* every time that the Sage runtime gets rebuilt.
...
```json
{
    "body": "<a id='comment:3'></a>As an example, see the dependencies for `sagetex`.",
...
<a id='comment:3'></a>As an example, see the dependencies for `sagetex`.
...
<a id='comment:5'></a>Thank you for the Hint. I have found only sagetex depending on SAGERUNTIME, 
so should I try to remove this dependency in spkg-install?
...
<a id='comment:6'></a>Replying to [frederichan](#comment%3A5):
> Thank you for the Hint. I have found only sagetex depending on SAGERUNTIME, 
> so should I try to remove this dependency in spkg-install?
> 
> I think that the guilty  is the following:
...
```json
{
    "body": "<a id='comment:9'></a>I don't plan to add changes, but  I have not yet tested this because my sage was not up to date. So I was waiting for my fresh built to finish and then test...\nBut if you prefer to review it now it is OK for me.",
...
<a id='comment:9'></a>I don't plan to add changes, but  I have not yet tested this because my sage was not up to date. So I was waiting for my fresh built to finish and then test...
But if you prefer to review it now it is OK for me.
...
<a id='comment:10'></a>So for my first test with sage 6.10beta7 I got the following PB:
I was able to built giacpy but not with the check option because
some prettyprint in sympy was not found.
make sympy solved the problem.
...
<a id='comment:13'></a>Also giacpy should depend on maxima:
