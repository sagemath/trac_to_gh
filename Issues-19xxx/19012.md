# Issue 19012: Bug in intersection of polyhedron (generic backend)

archive/issues_018775.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: K.<a> = QuadraticField(5)\nsage: P = Polyhedron([[0,0],[0,a],[1,1]])\nsage: Q = Polyhedron(ieqs=[[-1,a,1]])\nsage: P.intersection(Q)\nTraceback (most recent call last):\n...\nTypeError: can only concatenate tuple (not \"list\") to tuple\n```\n\nCC:  @vbraun @fchapoton @nathanncohen\n\nBranch: **[f041c97](https://github.com/sagemath/sagetrac-mirror/commit/f041c9771177eda3cec6eb01d026fe7866075f47)**\n\nReviewer: **Nathann Cohen**\n\nAuthor: **Vincent Delecroix**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19012_\n\n",
    "closed_at": "2015-08-13T18:35:18Z",
    "created_at": "2015-08-12T04:48:31Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Bug in intersection of polyhedron (generic backend)",
    "type": "issue",
    "updated_at": "2023-01-08T09:12:00Z",
    "url": "https://github.com/sagemath/sage/issues/19012",
    "user": "https://github.com/videlec"
}
```

```
sage: K.<a> = QuadraticField(5)
sage: P = Polyhedron([[0,0],[0,a],[1,1]])
sage: Q = Polyhedron(ieqs=[[-1,a,1]])
sage: P.intersection(Q)
Traceback (most recent call last):
...
TypeError: can only concatenate tuple (not "list") to tuple
```

CC:  @vbraun @fchapoton @nathanncohen

Branch: **[f041c97](https://github.com/sagemath/sagetrac-mirror/commit/f041c9771177eda3cec6eb01d026fe7866075f47)**

Reviewer: **Nathann Cohen**

Author: **Vincent Delecroix**

_Issue created by migration from https://trac.sagemath.org/ticket/19012_





---

archive/issue_events_252084.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-08-12T04:48:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19012",
    "milestone_number": null,
    "milestone_title": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19012#event-252084"
}
```



---

archive/issue_events_252085.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-08-12T04:48:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19012",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "component: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19012#event-252085"
}
```



---

archive/issue_events_252086.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-08-12T04:48:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19012",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19012#event-252086"
}
```



---

archive/issue_events_252087.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-08-12T04:48:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19012",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19012#event-252087"
}
```



---

archive/issue_comments_270127.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e75cc14dacb00d5e3a831b93b1493ceecb31f5e9\">e75cc14</a></td><td><code>Trac 19012: fix intersection of polyhedra</code></td></tr></table>\n",
    "created_at": "2015-08-12T05:15:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19012#issuecomment-270127",
    "user": "https://github.com/videlec"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e75cc14dacb00d5e3a831b93b1493ceecb31f5e9">e75cc14</a></td><td><code>Trac 19012: fix intersection of polyhedra</code></td></tr></table>




---

archive/issue_events_252088.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2015-08-12T05:15:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19012",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19012#event-252088"
}
```



---

archive/issue_comments_270128.json:
```json
{
    "body": "Commit: **[e75cc14](https://github.com/sagemath/sagetrac-mirror/commit/e75cc14dacb00d5e3a831b93b1493ceecb31f5e9)**",
    "created_at": "2015-08-12T05:15:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19012#issuecomment-270128",
    "user": "https://github.com/videlec"
}
```

Commit: **[e75cc14](https://github.com/sagemath/sagetrac-mirror/commit/e75cc14dacb00d5e3a831b93b1493ceecb31f5e9)**



---

archive/issue_comments_270129.json:
```json
{
    "body": "Branch: **[u/vdelecroix/19012](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/19012)**",
    "created_at": "2015-08-12T05:15:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19012#issuecomment-270129",
    "user": "https://github.com/videlec"
}
```

Branch: **[u/vdelecroix/19012](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/19012)**



---

archive/issue_comments_270130.json:
```json
{
    "body": "Changed commit from **[e75cc14](https://github.com/sagemath/sagetrac-mirror/commit/e75cc14dacb00d5e3a831b93b1493ceecb31f5e9)** to **[f041c97](https://github.com/sagemath/sagetrac-mirror/commit/f041c9771177eda3cec6eb01d026fe7866075f47)**",
    "created_at": "2015-08-12T06:04:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19012#issuecomment-270130",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[e75cc14](https://github.com/sagemath/sagetrac-mirror/commit/e75cc14dacb00d5e3a831b93b1493ceecb31f5e9)** to **[f041c97](https://github.com/sagemath/sagetrac-mirror/commit/f041c9771177eda3cec6eb01d026fe7866075f47)**



---

archive/issue_comments_270131.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f041c9771177eda3cec6eb01d026fe7866075f47\">f041c97</a></td><td><code>Trac #19012: fix intersection of polyhedra</code></td></tr></table>\n",
    "created_at": "2015-08-12T06:04:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19012#issuecomment-270131",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f041c9771177eda3cec6eb01d026fe7866075f47">f041c97</a></td><td><code>Trac #19012: fix intersection of polyhedra</code></td></tr></table>




---

archive/issue_comments_270132.json:
```json
{
    "body": "Reviewer: **Nathann Cohen**",
    "created_at": "2015-08-12T07:19:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19012#issuecomment-270132",
    "user": "https://github.com/nathanncohen"
}
```

Reviewer: **Nathann Cohen**



---

archive/issue_events_252089.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-08-12T07:19:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19012",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19012#event-252089"
}
```



---

archive/issue_events_252090.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2015-08-12T07:19:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19012",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19012#event-252090"
}
```



---

archive/issue_comments_270133.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThanks!",
    "created_at": "2015-08-12T09:19:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19012#issuecomment-270133",
    "user": "https://github.com/videlec"
}
```

<div id="comment:4" align="right">Comment 4</div>

Thanks!



---

archive/issue_comments_270134.json:
```json
{
    "body": "Changed branch from **[u/vdelecroix/19012](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/19012)** to **[f041c97](https://github.com/sagemath/sagetrac-mirror/commit/f041c9771177eda3cec6eb01d026fe7866075f47)**",
    "created_at": "2015-08-13T18:35:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19012#issuecomment-270134",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/vdelecroix/19012](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/19012)** to **[f041c97](https://github.com/sagemath/sagetrac-mirror/commit/f041c9771177eda3cec6eb01d026fe7866075f47)**



---

archive/issue_events_252091.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-08-13T18:35:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19012",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19012#event-252091"
}
```



---

archive/issue_events_252092.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "004078259c78cc477f750019825295842ce56119",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-08-13T18:35:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19012",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19012#event-252092"
}
```



---

archive/issue_comments_270135.json:
```json
{
    "body": "Changed commit from **[f041c97](https://github.com/sagemath/sagetrac-mirror/commit/f041c9771177eda3cec6eb01d026fe7866075f47)** to none",
    "created_at": "2023-01-08T09:12:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19012",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19012#issuecomment-270135",
    "user": "https://github.com/fchapoton"
}
```

Changed commit from **[f041c97](https://github.com/sagemath/sagetrac-mirror/commit/f041c9771177eda3cec6eb01d026fe7866075f47)** to none
