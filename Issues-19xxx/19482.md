# Issue 19482: Documentation for auto-generated index of functions

archive/issues_019245.json:
```json
{
    "body": "This could have a place in the developer's manual, or it will be yet another dark feature.\n\nNathann\n\nReviewer: Jori M\u00e4ntysalo\n\nAuthor: Nathann Cohen\n\nBranch: u/ncohen/19482\n\nCommit: 757a9f4a908db4be170f28a96c000a1358ac9f2d\n\nResolution: wontfix\n\nIssue created by migration from https://trac.sagemath.org/ticket/19482\n\n",
    "closed_at": "2016-08-30T13:32:25Z",
    "created_at": "2015-10-27T09:16:03Z",
    "labels": [
        "component: documentation"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "Documentation for auto-generated index of functions",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/19482",
    "user": "https://github.com/nathanncohen"
}
```
This could have a place in the developer's manual, or it will be yet another dark feature.

Nathann

Reviewer: Jori Mäntysalo

Author: Nathann Cohen

Branch: u/ncohen/19482

Commit: 757a9f4a908db4be170f28a96c000a1358ac9f2d

Resolution: wontfix

Issue created by migration from https://trac.sagemath.org/ticket/19482





---

archive/issue_comments_349091.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2015-10-27T09:16:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349091",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_349092.json:
```json
{
    "body": "<a id='comment:1'></a>New commits:",
    "created_at": "2015-10-27T09:16:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349092",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:1'></a>New commits:



---

archive/issue_comments_349093.json:
```json
{
    "body": "Changing branch from \"\" to \"u/ncohen/19482\"",
    "created_at": "2015-10-27T09:16:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349093",
    "user": "https://github.com/nathanncohen"
}
```

Changing branch from "" to "u/ncohen/19482"



---

archive/issue_comments_349094.json:
```json
{
    "body": "Changing commit from \"\" to \"757a9f4a908db4be170f28a96c000a1358ac9f2d\"",
    "created_at": "2015-10-27T09:16:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349094",
    "user": "https://github.com/nathanncohen"
}
```

Changing commit from "" to "757a9f4a908db4be170f28a96c000a1358ac9f2d"



---

archive/issue_comments_349095.json:
```json
{
    "body": "<a id='comment:2'></a>I tried to follow instructions with `categories/finite_posets.py`, but got only\n\n```\ndocstring of sage.categories.finite_posets:11: WARNING: The \"csv-table\" directive requires content; none supplied.\n```\n\nI guess that I should change `__name__` to something. To what? And should it be told in these instructions?",
    "created_at": "2015-10-27T13:01:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349095",
    "user": "https://github.com/jm58660"
}
```

<a id='comment:2'></a>I tried to follow instructions with `categories/finite_posets.py`, but got only

```
docstring of sage.categories.finite_posets:11: WARNING: The "csv-table" directive requires content; none supplied.
```

I guess that I should change `__name__` to something. To what? And should it be told in these instructions?



---

archive/issue_comments_349096.json:
```json
{
    "body": "<a id='comment:3'></a>Yo,\n\n> I tried to follow instructions with `categories/finite_posets.py`, but got only\n> \n> \n> ```\n> docstring of sage.categories.finite_posets:11: WARNING: The \"csv-table\" directive requires content; none supplied.\n> ```\n\n\nWould I be correct to guess that you followed the instructions for an index of *functions* in a file that does not define any?\n\nIn that file, I'd say that what you want are the methods of a specific class.\n\n> I guess that I should change `__name__` to something.\n\n\nNono, this should work.\n\n> And should it be told in these instructions?\n\n\nLet us first figure out what went wrong in your test.\n\nNathann",
    "created_at": "2015-10-27T13:08:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349096",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:3'></a>Yo,

> I tried to follow instructions with `categories/finite_posets.py`, but got only
> 
> 
> ```
> docstring of sage.categories.finite_posets:11: WARNING: The "csv-table" directive requires content; none supplied.
> ```


Would I be correct to guess that you followed the instructions for an index of *functions* in a file that does not define any?

In that file, I'd say that what you want are the methods of a specific class.

> I guess that I should change `__name__` to something.


Nono, this should work.

> And should it be told in these instructions?


Let us first figure out what went wrong in your test.

Nathann



---

archive/issue_comments_349097.json:
```json
{
    "body": "<a id='comment:4'></a>Replying to [ncohen](#comment%3A3):\n\n> Would I be correct to guess that you followed the instructions for an index of *functions* in a file that does not define any?\n\n\nThere is `is_lattice()`, `is_poset_isomorphism()` etc. defined in `categories/finite_posets.py`.",
    "created_at": "2015-10-27T13:19:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349097",
    "user": "https://github.com/jm58660"
}
```

<a id='comment:4'></a>Replying to [ncohen](#comment%3A3):

> Would I be correct to guess that you followed the instructions for an index of *functions* in a file that does not define any?


There is `is_lattice()`, `is_poset_isomorphism()` etc. defined in `categories/finite_posets.py`.



---

archive/issue_comments_349098.json:
```json
{
    "body": "<a id='comment:5'></a>I just checked my terminology, but it seems correct: https://en.wikipedia.org/wiki/Method_(computer_programming)\n\nA 'method' is a function defined in a class. The functions are the functions defined in a module.\n\n```\nsage: import inspect\nsage: P = Poset()\nsage: inspect.ismethod(P.is_lattice)\nTrue\nsage: inspect.isfunction(P.is_lattice)\nFalse\n```\n\nNathann",
    "created_at": "2015-10-27T13:25:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349098",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:5'></a>I just checked my terminology, but it seems correct: https://en.wikipedia.org/wiki/Method_(computer_programming)

A 'method' is a function defined in a class. The functions are the functions defined in a module.

```
sage: import inspect
sage: P = Poset()
sage: inspect.ismethod(P.is_lattice)
True
sage: inspect.isfunction(P.is_lattice)
False
```

Nathann



---

archive/issue_comments_349099.json:
```json
{
    "body": "<a id='comment:6'></a>Replying to [ncohen](#comment%3A5):\n\n> A 'method' is a function defined in a class. The functions are the functions defined in a module.\n\n\nOK. Now I tried to read `incidence_structures.py` and copypaste it to `finite_posets.py`. It start with\n\n```\nr\"\"\"\nFinite posets\n\nList of functions:\n\n{METHODS_OF_FinitePosets}\n\"\"\"\n```\n\nand ends\n\n```\nfrom sage.misc.rest_index_of_methods import gen_rest_table_index\n__doc__ = __doc__.format(METHODS_OF_FinitePosets=gen_rest_table_index(FinitePosets))\n```\n\nThere might be no problem at all for clever people, but I do not understand.",
    "created_at": "2015-10-28T10:37:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349099",
    "user": "https://github.com/jm58660"
}
```

<a id='comment:6'></a>Replying to [ncohen](#comment%3A5):

> A 'method' is a function defined in a class. The functions are the functions defined in a module.


OK. Now I tried to read `incidence_structures.py` and copypaste it to `finite_posets.py`. It start with

```
r"""
Finite posets

List of functions:

{METHODS_OF_FinitePosets}
"""
```

and ends

```
from sage.misc.rest_index_of_methods import gen_rest_table_index
__doc__ = __doc__.format(METHODS_OF_FinitePosets=gen_rest_table_index(FinitePosets))
```

There might be no problem at all for clever people, but I do not understand.



---

archive/issue_comments_349100.json:
```json
{
    "body": "<a id='comment:7'></a>That's because of the syntax of python's format thing:\n\n```\nsage: \"a is {a} and b is {b}\".format(a=4,b=5)\n'a is 4 and b is 5'\n```\n\nThen you have to know that `__doc__` is the name of the module's docstring (the string that starts with 'Finite posets'. And then `gen_rest_table_index` does its job from the class that you give it.\n\nI don't mind your telling me that it is not clear or has to be explained in the developer's manual: it would be cool if we don't have to explain how python works there, though. \n\nBut then perhaps those details should not appear in the developer's manual but rather on the module's doc of this `rest_index_of_methods` module.\n\nNathann",
    "created_at": "2015-10-28T10:42:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349100",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:7'></a>That's because of the syntax of python's format thing:

```
sage: "a is {a} and b is {b}".format(a=4,b=5)
'a is 4 and b is 5'
```

Then you have to know that `__doc__` is the name of the module's docstring (the string that starts with 'Finite posets'. And then `gen_rest_table_index` does its job from the class that you give it.

I don't mind your telling me that it is not clear or has to be explained in the developer's manual: it would be cool if we don't have to explain how python works there, though. 

But then perhaps those details should not appear in the developer's manual but rather on the module's doc of this `rest_index_of_methods` module.

Nathann



---

archive/issue_comments_349101.json:
```json
{
    "body": "<a id='comment:8'></a>I guess the question is whether we really want widespread use of a hack that does large-ish string processing at each import (including during Sage startup).\n\nAn example for how to do things right is http://docs.astropy.org/en/stable/nddata/index.html, the auto-generated class table at the bottom is created only once while running sphinx to build the docs.",
    "created_at": "2015-11-24T22:26:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349101",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:8'></a>I guess the question is whether we really want widespread use of a hack that does large-ish string processing at each import (including during Sage startup).

An example for how to do things right is http://docs.astropy.org/en/stable/nddata/index.html, the auto-generated class table at the bottom is created only once while running sphinx to build the docs.



---

archive/issue_comments_349102.json:
```json
{
    "body": "<a id='comment:9'></a>> I guess the question is whether we really want widespread use of a hack that does large-ish string processing at each import (including during Sage startup).\n\n\nNot at all. The question here is whether you want the interface of this code to be documented, so that people can use it.\n\nHow it is implemented is a different problem, and you are most welcome to help if you think that it should be written differently.\n\nNathann",
    "created_at": "2015-11-25T07:33:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349102",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:9'></a>> I guess the question is whether we really want widespread use of a hack that does large-ish string processing at each import (including during Sage startup).


Not at all. The question here is whether you want the interface of this code to be documented, so that people can use it.

How it is implemented is a different problem, and you are most welcome to help if you think that it should be written differently.

Nathann



---

archive/issue_comments_349103.json:
```json
{
    "body": "<a id='comment:10'></a>What you say is true, however. The way they do it (through Sphinx) is much cleaner. I haven't found the list of method of a given classes so far, but it's probably doable with their code.",
    "created_at": "2015-11-25T08:42:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349103",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:10'></a>What you say is true, however. The way they do it (through Sphinx) is much cleaner. I haven't found the list of method of a given classes so far, but it's probably doable with their code.



---

archive/issue_comments_349104.json:
```json
{
    "body": "<a id='comment:11'></a>Back to this one...\n\nI would not say \"Optionally, the module's documentation can include a list - -\", but something like \"Module documentation should include a list of all functions. If there are only few of them, the list can be generated - -\".\n\nMaybe after example it could also say something like \"The list will not contain functions that are deprecated or whose name start with underscore.\" I am not sure if this is good addition or just noise.\n\nAfter \"An index of **all methods** of a specific class - -\" add \"There is no automatic way to get a list of all functions defined in a module containing several classes.\"\n\n\"When a class/module defines many methods/functions it can become interesting - -\" should be \"When a class/module defines many methods/functions the list should be grouped thematically - -\".\n\nOtherwise I think that I have now understood how to use this and what can be done with automatic indexing.",
    "created_at": "2016-01-11T10:29:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349104",
    "user": "https://github.com/jm58660"
}
```

<a id='comment:11'></a>Back to this one...

I would not say "Optionally, the module's documentation can include a list - -", but something like "Module documentation should include a list of all functions. If there are only few of them, the list can be generated - -".

Maybe after example it could also say something like "The list will not contain functions that are deprecated or whose name start with underscore." I am not sure if this is good addition or just noise.

After "An index of **all methods** of a specific class - -" add "There is no automatic way to get a list of all functions defined in a module containing several classes."

"When a class/module defines many methods/functions it can become interesting - -" should be "When a class/module defines many methods/functions the list should be grouped thematically - -".

Otherwise I think that I have now understood how to use this and what can be done with automatic indexing.



---

archive/issue_comments_349105.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Jori M\u00e4ntysalo\"",
    "created_at": "2016-01-11T10:29:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349105",
    "user": "https://github.com/jm58660"
}
```

Changing reviewer from "" to "Jori Mäntysalo"



---

archive/issue_events_053743.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-01-11T10:30:11Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "milestone": "sage-7.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/19482#event-53743"
}
```



---

archive/issue_comments_349106.json:
```json
{
    "body": "<a id='comment:14'></a>Yo !\n\n> I would not say \"Optionally, the module's documentation can include a list - -\", but something like \"Module documentation should include a list of all functions. If there are only few of them, the list can be generated - -\".\n\n\nHMmmm.. Sorry for nitpicking, but I don't allow myself to write rules in our developer's manual without a poll on sage-devel. Around 0% of our modules follow this rule at the moment, and phrasing it like that might say that we could refuse branches because they don't have it.\n\n> Maybe after example it could also say something like \"The list will not contain functions that are deprecated or whose name start with underscore.\" I am not sure if this is good addition or just noise.\n\n\nWell, to me this is more documentation specific to the reference manual. There is a link toward the rest_index_of_methods module that people can follow, and see what is there. So what you say is necessary <somewhere> but I don't personall believe it must be here.\n\n> After \"An index of **all methods** of a specific class - -\" add \"There is no automatic way to get a list of all functions defined in a module containing several classes.\"\n\n\nErrr... This sentence is 'wrong', technically. A function, by definition, does not belong to a class. A method, by definition, belongs to a class. You want to say that you cannot have one big table that mixes together the methods of different classes? That's rather expected, isn't it?.. `:-P`\n\n> \"When a class/module defines many methods/functions it can become interesting - -\" should be \"When a class/module defines many methods/functions the list should be grouped thematically - -\".\n\n\nSorry, same reason. I don't want to use 'should' here. I don't mind making tools available, though I don't want to force people to use them. Especially when some don't like them.\n\nNathann",
    "created_at": "2016-01-11T12:42:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349106",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:14'></a>Yo !

> I would not say "Optionally, the module's documentation can include a list - -", but something like "Module documentation should include a list of all functions. If there are only few of them, the list can be generated - -".


HMmmm.. Sorry for nitpicking, but I don't allow myself to write rules in our developer's manual without a poll on sage-devel. Around 0% of our modules follow this rule at the moment, and phrasing it like that might say that we could refuse branches because they don't have it.

> Maybe after example it could also say something like "The list will not contain functions that are deprecated or whose name start with underscore." I am not sure if this is good addition or just noise.


Well, to me this is more documentation specific to the reference manual. There is a link toward the rest_index_of_methods module that people can follow, and see what is there. So what you say is necessary <somewhere> but I don't personall believe it must be here.

> After "An index of **all methods** of a specific class - -" add "There is no automatic way to get a list of all functions defined in a module containing several classes."


Errr... This sentence is 'wrong', technically. A function, by definition, does not belong to a class. A method, by definition, belongs to a class. You want to say that you cannot have one big table that mixes together the methods of different classes? That's rather expected, isn't it?.. `:-P`

> "When a class/module defines many methods/functions it can become interesting - -" should be "When a class/module defines many methods/functions the list should be grouped thematically - -".


Sorry, same reason. I don't want to use 'should' here. I don't mind making tools available, though I don't want to force people to use them. Especially when some don't like them.

Nathann



---

archive/issue_comments_349107.json:
```json
{
    "body": "<a id='comment:15'></a>Warning: this slows down the module import and should at most be used for modules that are lazily imported.",
    "created_at": "2016-01-11T13:24:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349107",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:15'></a>Warning: this slows down the module import and should at most be used for modules that are lazily imported.



---

archive/issue_comments_349108.json:
```json
{
    "body": "<a id='comment:16'></a>See? \"Can be used\". Not \"should be used\" `:-P`",
    "created_at": "2016-01-11T13:25:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349108",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:16'></a>See? "Can be used". Not "should be used" `:-P`



---

archive/issue_comments_349109.json:
```json
{
    "body": "<a id='comment:17'></a>In any case, this is better with this addition than without, so I mark this as positive review. But some comments: \n\n[ By comment of vbraun in time of writing this I cancelled positive review. ]\n\nReplying to [ncohen](#comment%3A14):\n\n> > After \"An index of **all methods** of a specific class - -\" add \"There is no automatic way to get a list of all functions defined in a module containing several classes.\"\n\n>\n> Errr... This sentence is 'wrong', technically. A function, by definition, does not belong to a class. A method, by definition, belongs to a class. You want to say that you cannot have one big table that mixes together the methods of different classes? That's rather expected, isn't it?.. `:-P`\n\n\nWell, it is not that strange to think that functions, methods, classes, and whatevers, defined in one file can be shown in one index.\n\n> > I would not say \"Optionally, the module's documentation can include a list - -\", but something like \"Module documentation should include a list of all functions. If there are only few of them, the list can be generated - -\".\n\n> \n> HMmmm.. Sorry for nitpicking, but I don't allow myself to write rules in our developer's manual without a poll on sage-devel. Around 0% of our modules follow this rule at the moment, and phrasing it like that might say that we could refuse branches because they don't have it.\n\n\nActually I would like to have a kind of rule that new modules must have an index. Not necessarily made with this, but somehow.\n\nBut this has been discussed before without result. You want only rules that are really rules; I would like to have also recommendations of the form \"Do X or say why you did not do.\"",
    "created_at": "2016-01-11T13:28:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349109",
    "user": "https://github.com/jm58660"
}
```

<a id='comment:17'></a>In any case, this is better with this addition than without, so I mark this as positive review. But some comments: 

[ By comment of vbraun in time of writing this I cancelled positive review. ]

Replying to [ncohen](#comment%3A14):

> > After "An index of **all methods** of a specific class - -" add "There is no automatic way to get a list of all functions defined in a module containing several classes."

>
> Errr... This sentence is 'wrong', technically. A function, by definition, does not belong to a class. A method, by definition, belongs to a class. You want to say that you cannot have one big table that mixes together the methods of different classes? That's rather expected, isn't it?.. `:-P`


Well, it is not that strange to think that functions, methods, classes, and whatevers, defined in one file can be shown in one index.

> > I would not say "Optionally, the module's documentation can include a list - -", but something like "Module documentation should include a list of all functions. If there are only few of them, the list can be generated - -".

> 
> HMmmm.. Sorry for nitpicking, but I don't allow myself to write rules in our developer's manual without a poll on sage-devel. Around 0% of our modules follow this rule at the moment, and phrasing it like that might say that we could refuse branches because they don't have it.


Actually I would like to have a kind of rule that new modules must have an index. Not necessarily made with this, but somehow.

But this has been discussed before without result. You want only rules that are really rules; I would like to have also recommendations of the form "Do X or say why you did not do."



---

archive/issue_comments_349110.json:
```json
{
    "body": "<a id='comment:18'></a>> Well, it is not that strange to think that functions, methods, classes, and whatevers, defined in one file can be shown in one index.\n\n\nWhat do you do when several classes have methods with the same name?\n\n> Actually I would like to have a kind of rule that new modules must have an index. Not necessarily made with this, but somehow.\n> \n> But this has been discussed before without result. You want only rules that are really rules; I would like to have also recommendations of the form \"Do X or say why you did not do.\"\n\n\nHmmmm... Well, then add a commit if you like?\n\nNathann",
    "created_at": "2016-01-11T13:34:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349110",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:18'></a>> Well, it is not that strange to think that functions, methods, classes, and whatevers, defined in one file can be shown in one index.


What do you do when several classes have methods with the same name?

> Actually I would like to have a kind of rule that new modules must have an index. Not necessarily made with this, but somehow.
> 
> But this has been discussed before without result. You want only rules that are really rules; I would like to have also recommendations of the form "Do X or say why you did not do."


Hmmmm... Well, then add a commit if you like?

Nathann



---

archive/issue_comments_349111.json:
```json
{
    "body": "<a id='comment:19'></a>Replying to [ncohen](#comment%3A18):\n> > Well, it is not that strange to think that functions, methods, classes, and whatevers, defined in one file can be shown in one index.\n\n> \n> What do you do when several classes have methods with the same name?\n\n\nHave a link with class name added. ... ok, ok, bad idea.\n\n> Hmmmm... Well, then add a commit if you like?\n\n\nMaybe not for this one.\n\nBut anyways, should this be accepted or not? Volker, is a warning needed in this place, in the documentation of `gen_rest...()`, or in both? Or not at all?",
    "created_at": "2016-01-11T15:42:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349111",
    "user": "https://github.com/jm58660"
}
```

<a id='comment:19'></a>Replying to [ncohen](#comment%3A18):
> > Well, it is not that strange to think that functions, methods, classes, and whatevers, defined in one file can be shown in one index.

> 
> What do you do when several classes have methods with the same name?


Have a link with class name added. ... ok, ok, bad idea.

> Hmmmm... Well, then add a commit if you like?


Maybe not for this one.

But anyways, should this be accepted or not? Volker, is a warning needed in this place, in the documentation of `gen_rest...()`, or in both? Or not at all?



---

archive/issue_comments_349112.json:
```json
{
    "body": "<a id='comment:20'></a>Replying to [jmantysalo](#comment%3A19):\n\n> But anyways, should this be accepted or not? Volker, is a warning needed in this place, in the documentation of `gen_rest...()`, or in both? Or not at all?\n\n\nJust pinging... What should we do with this?",
    "created_at": "2016-02-10T05:42:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349112",
    "user": "https://github.com/jm58660"
}
```

<a id='comment:20'></a>Replying to [jmantysalo](#comment%3A19):

> But anyways, should this be accepted or not? Volker, is a warning needed in this place, in the documentation of `gen_rest...()`, or in both? Or not at all?


Just pinging... What should we do with this?



---

archive/issue_comments_349113.json:
```json
{
    "body": "<a id='comment:21'></a>(test request from Jori. Please ignore)",
    "created_at": "2016-02-10T06:00:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349113",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:21'></a>(test request from Jori. Please ignore)



---

archive/issue_comments_349114.json:
```json
{
    "body": "<a id='comment:22'></a>Getting back to this one... Volker, I still think that Sage with this patch is better than Sage without. As Nathann said, without this we have another dark feature.",
    "created_at": "2016-04-25T07:50:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349114",
    "user": "https://github.com/jm58660"
}
```

<a id='comment:22'></a>Getting back to this one... Volker, I still think that Sage with this patch is better than Sage without. As Nathann said, without this we have another dark feature.



---

archive/issue_comments_349115.json:
```json
{
    "body": "<a id='comment:23'></a>Startup time for Sage is also an issue, we are basically already borderline too slow to start. Doing more stuff at startup is not going to help. Related, the proposed syntax of slapping some magic stuff at the bottom of each page isn't something that you should do if you had a proper implementation. If you do it right (did you look at the astropy example?) then you wouldn't need the magic `__doc__ = ...` anyways. IMHO we should have a good implementation first before encouraging others to use it.",
    "created_at": "2016-04-25T08:47:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349115",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:23'></a>Startup time for Sage is also an issue, we are basically already borderline too slow to start. Doing more stuff at startup is not going to help. Related, the proposed syntax of slapping some magic stuff at the bottom of each page isn't something that you should do if you had a proper implementation. If you do it right (did you look at the astropy example?) then you wouldn't need the magic `__doc__ = ...` anyways. IMHO we should have a good implementation first before encouraging others to use it.



---

archive/issue_comments_349116.json:
```json
{
    "body": "<a id='comment:24'></a>Replying to [vbraun](#comment%3A23):\n> IMHO we should have a good implementation first before encouraging others to use it.\n\n\nOK. And I think that we all agree that Sphinx gives headache to everyone.\n\nThere is now five files that uses this (2 in combinat, 2 in coding, 1 in graphs). Should we add some comment to those, just before `{INDEX_OF_FUNCTIONS}` to tell developers what is going on?",
    "created_at": "2016-04-25T11:27:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349116",
    "user": "https://github.com/jm58660"
}
```

<a id='comment:24'></a>Replying to [vbraun](#comment%3A23):
> IMHO we should have a good implementation first before encouraging others to use it.


OK. And I think that we all agree that Sphinx gives headache to everyone.

There is now five files that uses this (2 in combinat, 2 in coding, 1 in graphs). Should we add some comment to those, just before `{INDEX_OF_FUNCTIONS}` to tell developers what is going on?



---

archive/issue_events_053744.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-07-04T13:10:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "milestone": "sage-7.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/19482#event-53744"
}
```



---

archive/issue_events_053745.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2016-07-04T13:10:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/19482#event-53745"
}
```



---

archive/issue_comments_349117.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2016-07-04T13:10:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349117",
    "user": "https://github.com/jm58660"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_349118.json:
```json
{
    "body": "<a id='comment:25'></a>As there does not seem to be common view, and Nathann has faded from developers (sad), I mark this as wontfix/positive. Volker, please revert if you have other opinion.",
    "created_at": "2016-07-04T13:10:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349118",
    "user": "https://github.com/jm58660"
}
```

<a id='comment:25'></a>As there does not seem to be common view, and Nathann has faded from developers (sad), I mark this as wontfix/positive. Volker, please revert if you have other opinion.



---

archive/issue_comments_349119.json:
```json
{
    "body": "Resolution: wontfix",
    "created_at": "2016-08-30T13:32:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349119",
    "user": "https://github.com/embray"
}
```

Resolution: wontfix



---

archive/issue_comments_349120.json:
```json
{
    "body": "<a id='comment:26'></a>Determined to be invalid/duplicate/wontfix (closing as \"wontfix\" as a catch-all resolution).",
    "created_at": "2016-08-30T13:32:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19482#issuecomment-349120",
    "user": "https://github.com/embray"
}
```

<a id='comment:26'></a>Determined to be invalid/duplicate/wontfix (closing as "wontfix" as a catch-all resolution).



---

archive/issue_events_053746.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-08-30T13:32:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/19482",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/19482#event-53746"
}
```
