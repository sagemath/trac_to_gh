# Issue 19377: dot product of sparse vector with dense vector: segfault

archive/issues_019140.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: v = random_vector(QQ, 5, sparse=True)\nsage: w = random_vector(QQ, 5, sparse=False)\nsage: v.dot_product(w)\n------------------------------------------------------------------------\nUnhandled SIGSEGV: A segmentation fault occurred in Sage.\nThis probably occurred because a *compiled* component of Sage has a bug\nin it and is not properly wrapped with sig_on(), sig_off().\nSage will now terminate.\n------------------------------------------------------------------------\n```\n\nBranch/Commit: **[26a0b6b](https://github.com/sagemath/sagetrac-mirror/commit/26a0b6bc624b8eee81d35dfcf0dfa85b626a6245)**\n\nReviewer: **John Palmieri, Jeroen Demeyer**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19377_\n\n",
    "closed_at": "2015-10-12T07:16:30Z",
    "created_at": "2015-10-08T21:43:42Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "dot product of sparse vector with dense vector: segfault",
    "type": "issue",
    "updated_at": "2015-10-12T07:16:30Z",
    "url": "https://github.com/sagemath/sage/issues/19377",
    "user": "https://github.com/jhpalmieri"
}
```

```
sage: v = random_vector(QQ, 5, sparse=True)
sage: w = random_vector(QQ, 5, sparse=False)
sage: v.dot_product(w)
------------------------------------------------------------------------
Unhandled SIGSEGV: A segmentation fault occurred in Sage.
This probably occurred because a *compiled* component of Sage has a bug
in it and is not properly wrapped with sig_on(), sig_off().
Sage will now terminate.
------------------------------------------------------------------------
```

Branch/Commit: **[26a0b6b](https://github.com/sagemath/sagetrac-mirror/commit/26a0b6bc624b8eee81d35dfcf0dfa85b626a6245)**

Reviewer: **John Palmieri, Jeroen Demeyer**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/19377_





---

archive/issue_events_257042.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2015-10-08T21:43:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "milestone_number": null,
    "milestone_title": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19377#event-257042"
}
```



---

archive/issue_events_257043.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2015-10-08T21:43:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19377#event-257043"
}
```



---

archive/issue_events_257044.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2015-10-08T21:43:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19377#event-257044"
}
```



---

archive/issue_events_257045.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2015-10-08T21:43:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19377#event-257045"
}
```



---

archive/issue_comments_277865.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nI'm pretty sure the problem is this line in the sparse `_dot_product_coerce_`:\n\n```\ncdef dict e = (<FreeModuleElement_generic_sparse>right)._entries\n```\nwhich assumes the right vector is a sparse vector. I will fix this and post shortly.",
    "created_at": "2015-10-10T13:59:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19377#issuecomment-277865",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:1'>Comment 1:</a>
I'm pretty sure the problem is this line in the sparse `_dot_product_coerce_`:

```
cdef dict e = (<FreeModuleElement_generic_sparse>right)._entries
```
which assumes the right vector is a sparse vector. I will fix this and post shortly.



---

archive/issue_comments_277866.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nEasy enough fix (even if it many not be the \"best\", but I would argue that if this is a bottleneck, then one needs to be more careful about dense/sparse vectors).",
    "created_at": "2015-10-10T14:08:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19377#issuecomment-277866",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:2'>Comment 2:</a>
Easy enough fix (even if it many not be the "best", but I would argue that if this is a bottleneck, then one needs to be more careful about dense/sparse vectors).



---

archive/issue_comments_277867.json:
```json
{
    "body": "Author: **Travis Scrimshaw**",
    "created_at": "2015-10-10T14:08:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19377#issuecomment-277867",
    "user": "https://github.com/tscrim"
}
```

Author: **Travis Scrimshaw**



---

archive/issue_comments_277868.json:
```json
{
    "body": "Commit: **[2f7c727](https://github.com/sagemath/sagetrac-mirror/commit/2f7c7277fbc39d44c6a0d4dd56f789c3ed9c78d4)**",
    "created_at": "2015-10-10T14:08:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19377#issuecomment-277868",
    "user": "https://github.com/tscrim"
}
```

Commit: **[2f7c727](https://github.com/sagemath/sagetrac-mirror/commit/2f7c7277fbc39d44c6a0d4dd56f789c3ed9c78d4)**



---

archive/issue_comments_277869.json:
```json
{
    "body": "Branch: **[public/modules/dot_sparse_dense-19377](https://github.com/sagemath/sagetrac-mirror/tree/public/modules/dot_sparse_dense-19377)**",
    "created_at": "2015-10-10T14:08:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19377#issuecomment-277869",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/modules/dot_sparse_dense-19377](https://github.com/sagemath/sagetrac-mirror/tree/public/modules/dot_sparse_dense-19377)**



---

archive/issue_events_257046.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-10-10T14:08:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19377#event-257046"
}
```



---

archive/issue_comments_277870.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d2f690be75c4f68df5ad10837edc1d1bdf7a4073\">d2f690b</a></td><td><code>Fix for sparse dot dense vector.</code></td></tr></table>\n",
    "created_at": "2015-10-10T14:26:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19377#issuecomment-277870",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:3'>Comment 3:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d2f690be75c4f68df5ad10837edc1d1bdf7a4073">d2f690b</a></td><td><code>Fix for sparse dot dense vector.</code></td></tr></table>




---

archive/issue_comments_277871.json:
```json
{
    "body": "Changed commit from **[2f7c727](https://github.com/sagemath/sagetrac-mirror/commit/2f7c7277fbc39d44c6a0d4dd56f789c3ed9c78d4)** to **[d2f690b](https://github.com/sagemath/sagetrac-mirror/commit/d2f690be75c4f68df5ad10837edc1d1bdf7a4073)**",
    "created_at": "2015-10-10T14:26:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19377#issuecomment-277871",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[2f7c727](https://github.com/sagemath/sagetrac-mirror/commit/2f7c7277fbc39d44c6a0d4dd56f789c3ed9c78d4)** to **[d2f690b](https://github.com/sagemath/sagetrac-mirror/commit/d2f690be75c4f68df5ad10837edc1d1bdf7a4073)**



---

archive/issue_events_257047.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-10-10T20:14:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19377#event-257047"
}
```



---

archive/issue_events_257048.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-10-10T20:14:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19377#event-257048"
}
```



---

archive/issue_comments_277872.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nI think this would be much better:\n\n```\ntry:\n    e = (<FreeModuleElement_generic_sparse?>right)._entries\nexcept TypeError:\n    ...\n```",
    "created_at": "2015-10-10T20:14:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19377#issuecomment-277872",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:4'>Comment 4:</a>
I think this would be much better:

```
try:
    e = (<FreeModuleElement_generic_sparse?>right)._entries
except TypeError:
    ...
```



---

archive/issue_comments_277873.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e92b704774396ffb2fa1a25e9ceb45e8a2c8f9f0\">e92b704</a></td><td><code>Following Jeroen's suggestion.</code></td></tr></table>\n",
    "created_at": "2015-10-10T20:50:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19377#issuecomment-277873",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:5'>Comment 5:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e92b704774396ffb2fa1a25e9ceb45e8a2c8f9f0">e92b704</a></td><td><code>Following Jeroen's suggestion.</code></td></tr></table>




---

archive/issue_comments_277874.json:
```json
{
    "body": "Changed commit from **[d2f690b](https://github.com/sagemath/sagetrac-mirror/commit/d2f690be75c4f68df5ad10837edc1d1bdf7a4073)** to **[e92b704](https://github.com/sagemath/sagetrac-mirror/commit/e92b704774396ffb2fa1a25e9ceb45e8a2c8f9f0)**",
    "created_at": "2015-10-10T20:50:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19377#issuecomment-277874",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[d2f690b](https://github.com/sagemath/sagetrac-mirror/commit/d2f690be75c4f68df5ad10837edc1d1bdf7a4073)** to **[e92b704](https://github.com/sagemath/sagetrac-mirror/commit/e92b704774396ffb2fa1a25e9ceb45e8a2c8f9f0)**



---

archive/issue_comments_277875.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nDone.",
    "created_at": "2015-10-10T20:53:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19377#issuecomment-277875",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:6'>Comment 6:</a>
Done.



---

archive/issue_events_257049.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-10-10T20:53:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19377#event-257049"
}
```



---

archive/issue_events_257050.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-10-10T20:53:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19377#event-257050"
}
```



---

archive/issue_comments_277876.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nNot `except:` but `except TypeError:`",
    "created_at": "2015-10-10T21:00:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19377#issuecomment-277876",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:7'>Comment 7:</a>
Not `except:` but `except TypeError:`



---

archive/issue_events_257051.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-10-10T21:00:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19377#event-257051"
}
```



---

archive/issue_events_257052.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-10-10T21:00:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19377#event-257052"
}
```



---

archive/issue_comments_277877.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/26a0b6bc624b8eee81d35dfcf0dfa85b626a6245\">26a0b6b</a></td><td><code>TypeError...</code></td></tr></table>\n",
    "created_at": "2015-10-10T21:01:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19377#issuecomment-277877",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:8'>Comment 8:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/26a0b6bc624b8eee81d35dfcf0dfa85b626a6245">26a0b6b</a></td><td><code>TypeError...</code></td></tr></table>




---

archive/issue_comments_277878.json:
```json
{
    "body": "Changed commit from **[e92b704](https://github.com/sagemath/sagetrac-mirror/commit/e92b704774396ffb2fa1a25e9ceb45e8a2c8f9f0)** to **[26a0b6b](https://github.com/sagemath/sagetrac-mirror/commit/26a0b6bc624b8eee81d35dfcf0dfa85b626a6245)**",
    "created_at": "2015-10-10T21:01:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19377#issuecomment-277878",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[e92b704](https://github.com/sagemath/sagetrac-mirror/commit/e92b704774396ffb2fa1a25e9ceb45e8a2c8f9f0)** to **[26a0b6b](https://github.com/sagemath/sagetrac-mirror/commit/26a0b6bc624b8eee81d35dfcf0dfa85b626a6245)**



---

archive/issue_events_257053.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-10-10T21:01:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19377#event-257053"
}
```



---

archive/issue_events_257054.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2015-10-10T21:01:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19377#event-257054"
}
```



---

archive/issue_comments_277879.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\n*facepalm* Thanks.",
    "created_at": "2015-10-10T21:01:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19377#issuecomment-277879",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:9'>Comment 9:</a>
*facepalm* Thanks.



---

archive/issue_comments_277880.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nWhat does the question mark mean in\n\n```\ne = (<FreeModuleElement_generic_sparse?>right)._entries\n```",
    "created_at": "2015-10-11T02:56:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19377#issuecomment-277880",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:10'>Comment 10:</a>
What does the question mark mean in

```
e = (<FreeModuleElement_generic_sparse?>right)._entries
```



---

archive/issue_comments_277881.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nFor checking the type. See the type checking section in http://docs.cython.org/src/reference/language_basics.html.",
    "created_at": "2015-10-11T03:10:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19377#issuecomment-277881",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:11'>Comment 11:</a>
For checking the type. See the type checking section in http://docs.cython.org/src/reference/language_basics.html.



---

archive/issue_events_257055.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2015-10-11T05:40:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19377#event-257055"
}
```



---

archive/issue_events_257056.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2015-10-11T05:40:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19377#event-257056"
}
```



---

archive/issue_comments_277882.json:
```json
{
    "body": "Reviewer: **John Palmieri**",
    "created_at": "2015-10-11T05:40:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19377#issuecomment-277882",
    "user": "https://github.com/jhpalmieri"
}
```

Reviewer: **John Palmieri**



---

archive/issue_comments_277883.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nThis fixes the problem for me. I don't know if it's the best way to do it, but I agree with [#comment:2 comment #2], so I'll give it a positive review.",
    "created_at": "2015-10-11T05:40:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19377#issuecomment-277883",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:12'>Comment 12:</a>
This fixes the problem for me. I don't know if it's the best way to do it, but I agree with [#comment:2 comment #2], so I'll give it a positive review.



---

archive/issue_comments_277884.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nJohn: essentially\n\n```\ncdef t a = <t?>b\n```\nis equivalent to\n\n```\nif not isinstance(b, t):\n    raise TypeError\ncdef t a = <t>b\n```",
    "created_at": "2015-10-11T09:37:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19377#issuecomment-277884",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:13'>Comment 13:</a>
John: essentially

```
cdef t a = <t?>b
```
is equivalent to

```
if not isinstance(b, t):
    raise TypeError
cdef t a = <t>b
```



---

archive/issue_comments_277885.json:
```json
{
    "body": "Changed reviewer from **John Palmieri** to **John Palmieri, Jeroen Demeyer**",
    "created_at": "2015-10-11T09:40:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19377#issuecomment-277885",
    "user": "https://github.com/jdemeyer"
}
```

Changed reviewer from **John Palmieri** to **John Palmieri, Jeroen Demeyer**



---

archive/issue_comments_277886.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nI didn't run doctests (I hope any of you did) but the change does indeed look good.",
    "created_at": "2015-10-11T09:40:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19377#issuecomment-277886",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:14'>Comment 14:</a>
I didn't run doctests (I hope any of you did) but the change does indeed look good.



---

archive/issue_events_257057.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-10-12T07:16:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19377#event-257057"
}
```



---

archive/issue_events_257058.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "f17c00c2d91d84ee9032e39080712aef1613f9d6",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-10-12T07:16:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19377#event-257058"
}
```



---

archive/issue_comments_277887.json:
```json
{
    "body": "Changed branch from **[public/modules/dot_sparse_dense-19377](https://github.com/sagemath/sagetrac-mirror/tree/public/modules/dot_sparse_dense-19377)** to **[26a0b6b](https://github.com/sagemath/sagetrac-mirror/commit/26a0b6bc624b8eee81d35dfcf0dfa85b626a6245)**",
    "created_at": "2015-10-12T07:16:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19377#issuecomment-277887",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/modules/dot_sparse_dense-19377](https://github.com/sagemath/sagetrac-mirror/tree/public/modules/dot_sparse_dense-19377)** to **[26a0b6b](https://github.com/sagemath/sagetrac-mirror/commit/26a0b6bc624b8eee81d35dfcf0dfa85b626a6245)**
