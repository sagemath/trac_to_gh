# Issue 19866: Undef _signals to fix compiler warnings

archive/issues_019629.json:
```json
{
    "assignees": [],
    "body": "The macro `_signals` is defined in `interrupt_api.h` and also in Cython modules when cimporting `interrupt.pxd`. Since the includes are set up in the correct order, this does no harm, but it is better to fix this anyway.\n\nNormally, the compiler warns about this, but Sage silences this warning with the `-w` option.\n\nCC:  @malb\n\nBranch/Commit: **[9e6a1fd](https://github.com/sagemath/sagetrac-mirror/commit/9e6a1fdcd2df0be8fc0c9012cf3d1e7762b25e5e)**\n\nReviewer: **Martin Albrecht**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/19866_\n\n",
    "closed_at": "2016-02-02T00:29:29Z",
    "created_at": "2016-01-11T22:34:35Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20cython",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Undef _signals to fix compiler warnings",
    "type": "issue",
    "updated_at": "2016-02-02T00:29:29Z",
    "url": "https://github.com/sagemath/sage/issues/19866",
    "user": "https://github.com/jdemeyer"
}
```
The macro `_signals` is defined in `interrupt_api.h` and also in Cython modules when cimporting `interrupt.pxd`. Since the includes are set up in the correct order, this does no harm, but it is better to fix this anyway.

Normally, the compiler warns about this, but Sage silences this warning with the `-w` option.

CC:  @malb

Branch/Commit: **[9e6a1fd](https://github.com/sagemath/sagetrac-mirror/commit/9e6a1fdcd2df0be8fc0c9012cf3d1e7762b25e5e)**

Reviewer: **Martin Albrecht**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/19866_





---

archive/issue_events_263010.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-01-11T22:34:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/19866",
    "milestone_number": null,
    "milestone_title": "sage-7.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19866#event-263010"
}
```



---

archive/issue_events_263011.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-01-11T22:34:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19866",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20cython",
    "label_color": "000080",
    "label_name": "component: cython",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19866#event-263011"
}
```



---

archive/issue_events_263012.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-01-11T22:34:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19866",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19866#event-263012"
}
```



---

archive/issue_events_263013.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-01-11T22:34:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19866",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19866#event-263013"
}
```



---

archive/issue_comments_286633.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n+The macro `_signals` is defined in `interrupt_api.h` and also in Cython modules when cimporting `interrupt.pxd`. Since the includes are set up in the correct order, this does no harm, but it is better to fix this anyway.\n \n+Normally, the compiler warns about this, but Sage silences this warning with the `-w` option.\n``````\n",
    "created_at": "2016-01-11T22:41:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19866",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19866#issuecomment-286633",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
+The macro `_signals` is defined in `interrupt_api.h` and also in Cython modules when cimporting `interrupt.pxd`. Since the includes are set up in the correct order, this does no harm, but it is better to fix this anyway.
 
+Normally, the compiler warns about this, but Sage silences this warning with the `-w` option.
``````




---

archive/issue_comments_286634.json:
```json
{
    "body": "Branch: **[u/jdemeyer/undef__signals_to_fix_compiler_warnings](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/undef__signals_to_fix_compiler_warnings)**",
    "created_at": "2016-01-11T23:20:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19866",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19866#issuecomment-286634",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/undef__signals_to_fix_compiler_warnings](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/undef__signals_to_fix_compiler_warnings)**



---

archive/issue_comments_286635.json:
```json
{
    "body": "Commit: **[9e6a1fd](https://github.com/sagemath/sagetrac-mirror/commit/9e6a1fdcd2df0be8fc0c9012cf3d1e7762b25e5e)**",
    "created_at": "2016-01-11T23:21:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19866",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19866#issuecomment-286635",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[9e6a1fd](https://github.com/sagemath/sagetrac-mirror/commit/9e6a1fdcd2df0be8fc0c9012cf3d1e7762b25e5e)**



---

archive/issue_comments_286636.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9e6a1fdcd2df0be8fc0c9012cf3d1e7762b25e5e\">9e6a1fd</a></td><td><code>Undef _signals to fix compiler warnings</code></td></tr></table>\n",
    "created_at": "2016-01-11T23:21:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19866",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19866#issuecomment-286636",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:3'>Comment 3:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9e6a1fdcd2df0be8fc0c9012cf3d1e7762b25e5e">9e6a1fd</a></td><td><code>Undef _signals to fix compiler warnings</code></td></tr></table>




---

archive/issue_events_263014.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-01-11T23:21:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19866",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19866#event-263014"
}
```



---

archive/issue_events_263015.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2016-02-01T17:29:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19866",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19866#event-263015"
}
```



---

archive/issue_events_263016.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2016-02-01T17:29:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/19866",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19866#event-263016"
}
```



---

archive/issue_comments_286637.json:
```json
{
    "body": "Reviewer: **Martin Albrecht**",
    "created_at": "2016-02-01T17:29:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19866",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19866#issuecomment-286637",
    "user": "https://github.com/malb"
}
```

Reviewer: **Martin Albrecht**



---

archive/issue_comments_286638.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/undef__signals_to_fix_compiler_warnings](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/undef__signals_to_fix_compiler_warnings)** to **[9e6a1fd](https://github.com/sagemath/sagetrac-mirror/commit/9e6a1fdcd2df0be8fc0c9012cf3d1e7762b25e5e)**",
    "created_at": "2016-02-02T00:29:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/19866",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/19866#issuecomment-286638",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/undef__signals_to_fix_compiler_warnings](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/undef__signals_to_fix_compiler_warnings)** to **[9e6a1fd](https://github.com/sagemath/sagetrac-mirror/commit/9e6a1fdcd2df0be8fc0c9012cf3d1e7762b25e5e)**



---

archive/issue_events_263017.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-02-02T00:29:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/19866",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19866#event-263017"
}
```



---

archive/issue_events_263018.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "5b9cd57502cead93fb1f442d1e1b63904c7e6112",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-02-02T00:29:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/19866",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/19866#event-263018"
}
```
