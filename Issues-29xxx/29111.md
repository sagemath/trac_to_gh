...
```json
{
    "body": "As I said on #29092, we should also move corresponding sage-the-distribution tests (such as those for `sage -sqlite`) mentioned in #29002) from `src` (which is for sagelib) to somewhere in `build` (which is for sage-the-distribution). Downstream packagers would ignore the sage-the-distribution tests.",
...
```json
{
    "body": "I've asked this question before: if Sage is built with the system's Python, should we still keep `sage --python ...`? What is the difference between `sage --python` and `sage --sqlite3` (and what about `sage --gap`, `sage --singular`, etc.)?",
...
```json
{
    "body": "By the way, `sage -twistd` doesn't work: there is no `twistd` executable since Sage 9.1. Plus we are removing the twisted package in #29754. See #29878.",
...
```json
{
    "body": "Dropping `sage -sh` is a bad idea. User packages need this",
...
```json
{
    "body": "Now, I understand what you mean. User scripts may start with `sage -sh` and what not. I completely forgot about that when I removed it - even though that was one of the reason I was keeping it around. I will re-instate it as a vanilla shell or no-op for compatibility.",
...
```json
{
    "body": "Here's a beginning of what I have in mind. \n\n---\nNew commits:",
...
```json
{
    "body": "Okay, I'm fine with keeping `sage --python`. What about `sage --singular` et al.? Should `src/bin/sage` be constructed by `./configure`, adding arguments when the corresponding packages are going to be installed by Sage? Or is that too complicated?",
...
```json
{
    "body": "We could just conditionalize these ones based on whether SAGE_SRC is set or not by sage-env...",
...
```json
{
    "body": "All, please review, or feel free to push more changes to this ticket.",
    "created_at": "2020-06-17T00:29:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29111",
    "type": "issue_comment",
...
```json
{
    "body": "There are also various options regarding notebooks that may need updating. If someone knows about the details, please push changes to the branch",
...
```json
{
    "body": "Speaking of holy wars, this has reminded me of #21, which I am thinking of trying to revive. I'll see if I have time to work on that. For this ticket, we could remove `-inotebook`, and `sagenb` is no longer a valid option for `-notebook`.",
...
```json
{
    "body": "I think that the default for `sage --notebook` should now be `jupyter`. This involves only editing `sage-notebook`, and  belongs on a separate ticket: #29885.\n\n---\nNew commits:",
...
```json
{
    "body": "> We should probably continue the notebook discussion at #29885.\n\n\nSure, it's not a big deal in any case.",
...
```json
{
    "body": "Should #29884 be a dependency for this ticket, or are we also going to manually fix `src/doc/en/reference/repl/options.rst`?",
...
```json
{
    "body": "Other comments: `--fix-pkg-checksums` was deprecated, so I removed it from the help string. `--gap3` does not need to be in the basic `sage -h` message: it comes from an experimental package. For me, `sage -?` doesn't work, because the shell tries to parse `?`. So I removed that from the `sage -h` message.",
...
```json
{
    "body": "`sage-fix-pkg-checksums` was deprecated four years ago (#19984), so I deleted it.",
...
```json
{
    "body": "It is fine. I will ship those two then. It makes sense.",
...
```json
{
    "body": "Looks great to me. Let's get this in.",
...
```json
{
    "body": "The intention of using `SAGE_SRC` from `sage-env` here is that packagers would leave it undefined if they do not package a distribution.\n\nHow about moving the handling of `-h`, `-advanced` below the sourcing of `sage-env`?",
...
```json
{
    "body": "I used `SAGE_ROOT/src`, but of course this can be changed. There were a few failing doctests  in `tests/cmdline.py` (mainly because of rewordings in the documentation), now fixed.",
...
```json
{
    "body": "I also did a lot of reorganizing of the `sage` script, trying to group the different options into sensible ways. The patch is a bit of a mess, so the file itself might be easier to browse through for changes.",
...
```json
{
    "body": "I've tried to undo my changes to `sage -p`.",
...
```json
{
    "body": "Rebased on top of the new version of #29289\n\n---\nLast 10 new commits:",
...
```json
{
    "body": "By the way, `sage-site` -- is this a good name for this script?",
...
```json
{
    "body": "`@`fbissey, could you take another look so we can finalize this ticket?",
...
```json
{
    "body": "done, please check if you have a chance",
...
```json
{
    "body": "What is `sage --axiom` supposed to do? What package installs an `axiom` executable?",
...
```json
{
    "body": "Did we drop the packaging of `axiom`? http://axiom.axiom-developer.org/",
...
```json
{
    "body": "Not as a new-style package, and I couldn't find the website with old-style packages. Does it still exist?",
...
```json
{
    "body": "There is here http://old.files.sagemath.org/spkg/archive/ which I could go from here http://files.sagemath.org/spkg/",
...
```json
{
    "body": "See #29930 for a discussion of `sage --gap` and friends.",
...
```json
{
    "body": "The Sage `axiom` package requires `clisp`. I changed the `spkg-install` file to use `ecl` instead, but it failed. I am going to out on a limb and suggest that no one is using this spkg with a recent version of Sage.",
...
```json
{
    "body": "You have my full support to remove this piece of cruft.",
...
```json
{
    "body": "Does perhaps `fricas` install itself under the name `axiom`?",
...
```json
{
    "body": "Then let's just drop `sage --axiom`. Good catch!",
...
```json
{
    "body": "That's right, there is no new-style `axiom` package.",
...
```json
{
    "body": "I'm happy with everything. Who else would like to weigh in?",
...
```json
{
    "body": "Sorry, I was busy and forgot about it. We have reached an agreement with what goes in and what's next. I think we have bashed it enough and it is ready to go.",
    "created_at": "2020-06-23T20:46:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29111",
    "type": "issue_comment",
