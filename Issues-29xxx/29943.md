# Issue 29943: do not assume R installation has Makevars.site

archive/issues_029706.json:
```json
{
    "assignees": [],
    "body": "If SAGE_LOCAL is set to the prefix of an (external, or not) R installation,  it is assumed in src/bin/sage-env that Makevars.site exists.\n\nBut it should be checked, as it need not be there in case of an external R.\n\n\nCC:  @EmmanuelCharpentier @kcrisman @slel\n\nKeywords: **rproject**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29943_\n\n",
    "created_at": "2020-06-23T10:51:30Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "do not assume R installation has Makevars.site",
    "type": "issue",
    "updated_at": "2022-09-19T18:58:47Z",
    "url": "https://github.com/sagemath/sage/issues/29943",
    "user": "https://github.com/dimpase"
}
```
If SAGE_LOCAL is set to the prefix of an (external, or not) R installation,  it is assumed in src/bin/sage-env that Makevars.site exists.

But it should be checked, as it need not be there in case of an external R.


CC:  @EmmanuelCharpentier @kcrisman @slel

Keywords: **rproject**

_Issue created by migration from https://trac.sagemath.org/ticket/29943_





---

archive/issue_events_393950.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-06-23T10:51:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29943",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29943#event-393950"
}
```



---

archive/issue_events_393951.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-06-23T10:51:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29943",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "component: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29943#event-393951"
}
```



---

archive/issue_events_393952.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-06-23T10:51:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29943",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29943#event-393952"
}
```



---

archive/issue_events_393953.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-06-23T10:51:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29943",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29943#event-393953"
}
```



---

archive/issue_comments_476634.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNote: if one wants to ignore theses variables, it is possible to launch R with a parameter.\n\n\n```\n  --no-environ          Don't read the site and user environment files\n  --no-site-file        Don't read the site-wide Rprofile\n  --no-init-file        Don't read the user R profile\n```",
    "created_at": "2020-06-23T12:52:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29943",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29943#issuecomment-476634",
    "user": "https://github.com/thierry-FreeBSD"
}
```

<div id="comment:1" align="right">Comment 1</div>

Note: if one wants to ignore theses variables, it is possible to launch R with a parameter.


```
  --no-environ          Don't read the site and user environment files
  --no-site-file        Don't read the site-wide Rprofile
  --no-init-file        Don't read the user R profile
```



---

archive/issue_comments_476635.json:
```json
{
    "body": "Changed keywords from **** to **rproject**",
    "created_at": "2020-06-24T06:14:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29943",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29943#issuecomment-476635",
    "user": "https://github.com/slel"
}
```

Changed keywords from **** to **rproject**



---

archive/issue_events_393954.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-06-24T06:14:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29943",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "component: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29943#event-393954"
}
```



---

archive/issue_events_393955.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-06-24T06:14:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29943",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "component: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29943#event-393955"
}
```



---

archive/issue_comments_476636.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nIs there an actual problem?",
    "created_at": "2020-08-14T02:02:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29943",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29943#issuecomment-476636",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:4" align="right">Comment 4</div>

Is there an actual problem?



---

archive/issue_events_393956.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29943",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29943#event-393956"
}
```



---

archive/issue_events_393957.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29943",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29943#event-393957"
}
```



---

archive/issue_comments_476637.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nSage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review.",
    "created_at": "2021-04-07T19:29:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29943",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29943#issuecomment-476637",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:6" align="right">Comment 6</div>

Sage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review.



---

archive/issue_events_393958.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-07T19:29:15Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29943",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29943#event-393958"
}
```



---

archive/issue_events_393959.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-04-07T19:29:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29943",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29943#event-393959"
}
```



---

archive/issue_events_393960.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-10T17:30:20Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29943",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29943#event-393960"
}
```



---

archive/issue_events_393961.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-10T17:30:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29943",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29943#event-393961"
}
```



---

archive/issue_events_393962.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:11:26Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29943",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29943#event-393962"
}
```



---

archive/issue_events_393963.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:11:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29943",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29943#event-393963"
}
```



---

archive/issue_events_393964.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29943",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29943#event-393964"
}
```



---

archive/issue_events_393965.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29943",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29943#event-393965"
}
```



---

archive/issue_events_393966.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29943",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29943#event-393966"
}
```



---

archive/issue_events_393967.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29943",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29943#event-393967"
}
```
