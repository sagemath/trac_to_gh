# Issue 29375: Clean up some _element_constructor_() methods

archive/issues_029138.json:
```json
{
    "assignees": [],
    "body": "The coercion framework never calls `_element_constructor_()` methods without optional or keyword arguments on an input that is already in the correct parent.  This ticket removes some unnecessary checks for this case.\n\nI checked that (the branches of) the `if` statements removed by this ticket are never called by temporarily inserting `assert` statements.  In some other cases, I added doctests showing that the respective `if` statements cannot be removed (because the `_element_constructor_()` method has a optional or keyword arguments).\n\nThe deleted code in `multi_polynomial_libsingular` is no longer needed because these cases are handled by coercion maps from/via the base ring since #29247.\n\nCC:  @tscrim\n\nBranch/Commit: **[b526945](https://github.com/sagemath/sagetrac-mirror/commit/b5269455e861e119fdf9b9182a43a4dab232efaf)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton, Travis Scrimshaw**\n\nAuthor: **Peter Bruin**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29375_\n\n",
    "closed_at": "2020-03-25T22:48:33Z",
    "created_at": "2020-03-20T07:51:15Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20coercion",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Clean up some _element_constructor_() methods",
    "type": "issue",
    "updated_at": "2020-03-25T22:48:33Z",
    "url": "https://github.com/sagemath/sage/issues/29375",
    "user": "https://github.com/pjbruin"
}
```
The coercion framework never calls `_element_constructor_()` methods without optional or keyword arguments on an input that is already in the correct parent.  This ticket removes some unnecessary checks for this case.

I checked that (the branches of) the `if` statements removed by this ticket are never called by temporarily inserting `assert` statements.  In some other cases, I added doctests showing that the respective `if` statements cannot be removed (because the `_element_constructor_()` method has a optional or keyword arguments).

The deleted code in `multi_polynomial_libsingular` is no longer needed because these cases are handled by coercion maps from/via the base ring since #29247.

CC:  @tscrim

Branch/Commit: **[b526945](https://github.com/sagemath/sagetrac-mirror/commit/b5269455e861e119fdf9b9182a43a4dab232efaf)**

Reviewer: **Frédéric Chapoton, Travis Scrimshaw**

Author: **Peter Bruin**

_Issue created by migration from https://trac.sagemath.org/ticket/29375_





---

archive/issue_events_385678.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2020-03-20T07:51:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29375",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29375#event-385678"
}
```



---

archive/issue_events_385679.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2020-03-20T07:51:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29375",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20coercion",
    "label_color": "0000ff",
    "label_name": "component: coercion",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29375#event-385679"
}
```



---

archive/issue_events_385680.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2020-03-20T07:51:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29375",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29375#event-385680"
}
```



---

archive/issue_comments_464033.json:
```json
{
    "body": "Commit: **[586f977](https://github.com/sagemath/sagetrac-mirror/commit/586f97753602be4b22bff579468cd99405f50f8a)**",
    "created_at": "2020-03-20T07:53:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29375",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29375#issuecomment-464033",
    "user": "https://github.com/pjbruin"
}
```

Commit: **[586f977](https://github.com/sagemath/sagetrac-mirror/commit/586f97753602be4b22bff579468cd99405f50f8a)**



---

archive/issue_comments_464034.json:
```json
{
    "body": "Branch: **[u/pbruin/29375-clean_up_element_constructor](https://github.com/sagemath/sagetrac-mirror/tree/u/pbruin/29375-clean_up_element_constructor)**",
    "created_at": "2020-03-20T07:53:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29375",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29375#issuecomment-464034",
    "user": "https://github.com/pjbruin"
}
```

Branch: **[u/pbruin/29375-clean_up_element_constructor](https://github.com/sagemath/sagetrac-mirror/tree/u/pbruin/29375-clean_up_element_constructor)**



---

archive/issue_events_385681.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2020-03-20T07:53:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29375",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29375#event-385681"
}
```



---

archive/issue_comments_464035.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nwould you please fix the pyflakes warning from patchbot report ?",
    "created_at": "2020-03-20T11:16:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29375",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29375#issuecomment-464035",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

would you please fix the pyflakes warning from patchbot report ?



---

archive/issue_comments_464036.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b5269455e861e119fdf9b9182a43a4dab232efaf\">b526945</a></td><td><code>Trac 29375: remove unused imports</code></td></tr></table>\n",
    "created_at": "2020-03-20T12:19:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29375",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29375#issuecomment-464036",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b5269455e861e119fdf9b9182a43a4dab232efaf">b526945</a></td><td><code>Trac 29375: remove unused imports</code></td></tr></table>




---

archive/issue_comments_464037.json:
```json
{
    "body": "Changed commit from **[586f977](https://github.com/sagemath/sagetrac-mirror/commit/586f97753602be4b22bff579468cd99405f50f8a)** to **[b526945](https://github.com/sagemath/sagetrac-mirror/commit/b5269455e861e119fdf9b9182a43a4dab232efaf)**",
    "created_at": "2020-03-20T12:19:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29375",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29375#issuecomment-464037",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[586f977](https://github.com/sagemath/sagetrac-mirror/commit/586f97753602be4b22bff579468cd99405f50f8a)** to **[b526945](https://github.com/sagemath/sagetrac-mirror/commit/b5269455e861e119fdf9b9182a43a4dab232efaf)**



---

archive/issue_comments_464038.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nReplying to [@fchapoton](#comment%3A2):\n> would you please fix the pyflakes warning from patchbot report ?\n\nDone, thanks.",
    "created_at": "2020-03-20T12:20:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29375",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29375#issuecomment-464038",
    "user": "https://github.com/pjbruin"
}
```

<div id="comment:4" align="right">Comment 4</div>

Replying to [@fchapoton](#comment%3A2):
> would you please fix the pyflakes warning from patchbot report ?

Done, thanks.



---

archive/issue_comments_464039.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nlooks good to me, and the bot is fully green.\n\nMaybe Travis, who knows better the coercion framework, could give his word ?",
    "created_at": "2020-03-20T13:26:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29375",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29375#issuecomment-464039",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:5" align="right">Comment 5</div>

looks good to me, and the bot is fully green.

Maybe Travis, who knows better the coercion framework, could give his word ?



---

archive/issue_comments_464040.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nReplying to [@fchapoton](#comment%3A5):\n> Maybe Travis, who knows better the coercion framework, could give his word ?\n\nYes, what is in the ticket description is correct (it is faster to do it there because it is Cython too). So +1 for doing this.",
    "created_at": "2020-03-20T21:36:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29375",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29375#issuecomment-464040",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:6" align="right">Comment 6</div>

Replying to [@fchapoton](#comment%3A5):
> Maybe Travis, who knows better the coercion framework, could give his word ?

Yes, what is in the ticket description is correct (it is faster to do it there because it is Cython too). So +1 for doing this.



---

archive/issue_events_385682.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-03-21T07:40:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29375",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29375#event-385682"
}
```



---

archive/issue_events_385683.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-03-21T07:40:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29375",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29375#event-385683"
}
```



---

archive/issue_events_385684.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-03-21T07:40:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29375",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29375#event-385684"
}
```



---

archive/issue_comments_464041.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nok, then let it be.",
    "created_at": "2020-03-21T07:40:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29375",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29375#issuecomment-464041",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:7" align="right">Comment 7</div>

ok, then let it be.



---

archive/issue_events_385685.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-03-21T07:40:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29375",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29375#event-385685"
}
```



---

archive/issue_events_385686.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-03-21T07:40:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29375",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29375#event-385686"
}
```



---

archive/issue_comments_464042.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton, Travis Scrimshaw**",
    "created_at": "2020-03-21T07:40:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29375",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29375#issuecomment-464042",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton, Travis Scrimshaw**



---

archive/issue_comments_464043.json:
```json
{
    "body": "Changed branch from **[u/pbruin/29375-clean_up_element_constructor](https://github.com/sagemath/sagetrac-mirror/tree/u/pbruin/29375-clean_up_element_constructor)** to **[b526945](https://github.com/sagemath/sagetrac-mirror/commit/b5269455e861e119fdf9b9182a43a4dab232efaf)**",
    "created_at": "2020-03-25T22:48:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29375",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29375#issuecomment-464043",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/pbruin/29375-clean_up_element_constructor](https://github.com/sagemath/sagetrac-mirror/tree/u/pbruin/29375-clean_up_element_constructor)** to **[b526945](https://github.com/sagemath/sagetrac-mirror/commit/b5269455e861e119fdf9b9182a43a4dab232efaf)**



---

archive/issue_events_385687.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-03-25T22:48:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29375",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29375#event-385687"
}
```



---

archive/issue_events_385688.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "9399c8688b7006b8ea5ff3d1a62e80f34a8113ef",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-03-25T22:48:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29375",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29375#event-385688"
}
```
