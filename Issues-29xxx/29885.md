# Issue 29885: Remove sagenb from "sage --notebook"

archive/issues_029648.json:
```json
{
    "assignees": [],
    "body": "- Remove `sagenb` as a documented argument to the `sage-notebook` script, and remove the code to start the SageNB server.\n- Rewrite the error when `sagenb` is passed as an argument.\n- Change the default from `SageNBExport` to `NotebookJupyter`.\n\n\nBranch/Commit: **[f5853f9](https://github.com/sagemath/sagetrac-mirror/commit/f5853f9be456b0add1c6e9cb82f3f1f1741a5cf4)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **John Palmieri**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29885_\n\n",
    "closed_at": "2020-07-02T21:29:53Z",
    "created_at": "2020-06-17T17:35:08Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20scripts",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove sagenb from \"sage --notebook\"",
    "type": "issue",
    "updated_at": "2020-07-02T21:29:53Z",
    "url": "https://github.com/sagemath/sage/issues/29885",
    "user": "https://github.com/jhpalmieri"
}
```
- Remove `sagenb` as a documented argument to the `sage-notebook` script, and remove the code to start the SageNB server.
- Rewrite the error when `sagenb` is passed as an argument.
- Change the default from `SageNBExport` to `NotebookJupyter`.


Branch/Commit: **[f5853f9](https://github.com/sagemath/sagetrac-mirror/commit/f5853f9be456b0add1c6e9cb82f3f1f1741a5cf4)**

Reviewer: **Travis Scrimshaw**

Author: **John Palmieri**

_Issue created by migration from https://trac.sagemath.org/ticket/29885_





---

archive/issue_events_393112.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2020-06-17T17:35:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29885",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29885#event-393112"
}
```



---

archive/issue_events_393113.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2020-06-17T17:35:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29885",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20scripts",
    "label_color": "000080",
    "label_name": "component: scripts",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29885#event-393113"
}
```



---

archive/issue_events_393114.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2020-06-17T17:35:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29885",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29885#event-393114"
}
```



---

archive/issue_events_393115.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2020-06-17T17:35:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29885",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29885#event-393115"
}
```



---

archive/issue_comments_475249.json:
```json
{
    "body": "Branch: **[u/jhpalmieri/no-sagenb-in-sage-notebook](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/no-sagenb-in-sage-notebook)**",
    "created_at": "2020-06-17T17:35:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29885#issuecomment-475249",
    "user": "https://github.com/jhpalmieri"
}
```

Branch: **[u/jhpalmieri/no-sagenb-in-sage-notebook](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/no-sagenb-in-sage-notebook)**



---

archive/issue_comments_475250.json:
```json
{
    "body": "Commit: **[62bfb51](https://github.com/sagemath/sagetrac-mirror/commit/62bfb51d0ffa48d4c0c80c9faf65995651002790)**",
    "created_at": "2020-06-17T17:35:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29885#issuecomment-475250",
    "user": "https://github.com/jhpalmieri"
}
```

Commit: **[62bfb51](https://github.com/sagemath/sagetrac-mirror/commit/62bfb51d0ffa48d4c0c80c9faf65995651002790)**



---

archive/issue_events_393116.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2020-06-17T17:35:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29885",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29885#event-393116"
}
```



---

archive/issue_comments_475251.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/62bfb51d0ffa48d4c0c80c9faf65995651002790\">62bfb51</a></td><td><code>trac 29885: sage-notebook: remove sagenb as an option,</code></td></tr></table>\n",
    "created_at": "2020-06-17T17:35:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29885#issuecomment-475251",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/62bfb51d0ffa48d4c0c80c9faf65995651002790">62bfb51</a></td><td><code>trac 29885: sage-notebook: remove sagenb as an option,</code></td></tr></table>




---

archive/issue_comments_475252.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nI am okay with this.  I would just add something like the last line here:\n\n```\n        print('See https://wiki.sagemath.org/Python3-Switch')\n        print('Use sage --notebook=export to export sagenb notebooks to Jupyter')\n```",
    "created_at": "2020-06-17T17:56:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29885#issuecomment-475252",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:3'>Comment 3:</a>
I am okay with this.  I would just add something like the last line here:

```
        print('See https://wiki.sagemath.org/Python3-Switch')
        print('Use sage --notebook=export to export sagenb notebooks to Jupyter')
```



---

archive/issue_comments_475253.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBy the way, see #17590 and possibly elsewhere.  But since 9.2 will (apparently) not support Py2, perhaps it's indeed better to simply remove sagenb than to keep it as an optional package that can't be launched from the command line.",
    "created_at": "2020-06-17T17:58:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29885#issuecomment-475253",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:4'>Comment 4:</a>
By the way, see #17590 and possibly elsewhere.  But since 9.2 will (apparently) not support Py2, perhaps it's indeed better to simply remove sagenb than to keep it as an optional package that can't be launched from the command line.



---

archive/issue_comments_475254.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f5853f9be456b0add1c6e9cb82f3f1f1741a5cf4\">f5853f9</a></td><td><code>trac 29885: add to the error message for \"sage -n sagenb\"</code></td></tr></table>\n",
    "created_at": "2020-06-17T23:00:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29885#issuecomment-475254",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:5'>Comment 5:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f5853f9be456b0add1c6e9cb82f3f1f1741a5cf4">f5853f9</a></td><td><code>trac 29885: add to the error message for "sage -n sagenb"</code></td></tr></table>




---

archive/issue_comments_475255.json:
```json
{
    "body": "Changed commit from **[62bfb51](https://github.com/sagemath/sagetrac-mirror/commit/62bfb51d0ffa48d4c0c80c9faf65995651002790)** to **[f5853f9](https://github.com/sagemath/sagetrac-mirror/commit/f5853f9be456b0add1c6e9cb82f3f1f1741a5cf4)**",
    "created_at": "2020-06-17T23:00:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29885#issuecomment-475255",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[62bfb51](https://github.com/sagemath/sagetrac-mirror/commit/62bfb51d0ffa48d4c0c80c9faf65995651002790)** to **[f5853f9](https://github.com/sagemath/sagetrac-mirror/commit/f5853f9be456b0add1c6e9cb82f3f1f1741a5cf4)**



---

archive/issue_comments_475256.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nHere is an expansion of the \"sage -n sagenb\" error message.",
    "created_at": "2020-06-17T23:00:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29885#issuecomment-475256",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:6'>Comment 6:</a>
Here is an expansion of the "sage -n sagenb" error message.



---

archive/issue_comments_475257.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\n> Here is an expansion of the \"sage -n sagenb\" error message.\n\nThanks.  The code looks good, someone who can test the branch should just confirm that this works, and that tests pass - I can imagine some stray doctest somewhere needing the class?",
    "created_at": "2020-06-17T23:41:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29885#issuecomment-475257",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:7'>Comment 7:</a>
> Here is an expansion of the "sage -n sagenb" error message.

Thanks.  The code looks good, someone who can test the branch should just confirm that this works, and that tests pass - I can imagine some stray doctest somewhere needing the class?



---

archive/issue_events_393117.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-06-22T05:22:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29885",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29885#event-393117"
}
```



---

archive/issue_events_393118.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-06-22T05:22:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29885",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29885#event-393118"
}
```



---

archive/issue_comments_475258.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nLGTM.",
    "created_at": "2020-06-22T05:22:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29885#issuecomment-475258",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:8'>Comment 8:</a>
LGTM.



---

archive/issue_comments_475259.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2020-06-22T05:22:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29885#issuecomment-475259",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_393119.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-07-02T21:29:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29885",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29885#event-393119"
}
```



---

archive/issue_events_393120.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "2293cadad41968442661170818c60bdc5ac9b372",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-07-02T21:29:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29885",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29885#event-393120"
}
```



---

archive/issue_comments_475260.json:
```json
{
    "body": "Changed branch from **[u/jhpalmieri/no-sagenb-in-sage-notebook](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/no-sagenb-in-sage-notebook)** to **[f5853f9](https://github.com/sagemath/sagetrac-mirror/commit/f5853f9be456b0add1c6e9cb82f3f1f1741a5cf4)**",
    "created_at": "2020-07-02T21:29:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29885#issuecomment-475260",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jhpalmieri/no-sagenb-in-sage-notebook](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/no-sagenb-in-sage-notebook)** to **[f5853f9](https://github.com/sagemath/sagetrac-mirror/commit/f5853f9be456b0add1c6e9cb82f3f1f1741a5cf4)**
