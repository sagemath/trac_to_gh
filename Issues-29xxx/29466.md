# Issue 29466: Add kwds to __classcall__ of MatrixSpace

archive/issues_029229.json:
```json
{
    "assignees": [],
    "body": "most `__classcall__` methods of parent classes pocess a generic `**kwds` in their list of arguments, but `MatrixSpace`doesn't. Thus currently, you can't use additional options constructing an instance of a class inherited from `MatrixSpace`. The aim of this ticket is to make this possible.\n\n\nCC:  @tscrim\n\nKeywords: **matrix space classcall**\n\nBranch/Commit: **[532bd2a](https://github.com/sagemath/sagetrac-mirror/commit/532bd2a40ce9772aaf7e4c27bfa37ab0498117c8)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Sebastian Oehms**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29466_\n\n",
    "closed_at": "2020-04-26T22:45:57Z",
    "created_at": "2020-04-05T10:59:15Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add kwds to __classcall__ of MatrixSpace",
    "type": "issue",
    "updated_at": "2020-05-02T02:15:55Z",
    "url": "https://github.com/sagemath/sage/issues/29466",
    "user": "https://github.com/soehms"
}
```
most `__classcall__` methods of parent classes pocess a generic `**kwds` in their list of arguments, but `MatrixSpace`doesn't. Thus currently, you can't use additional options constructing an instance of a class inherited from `MatrixSpace`. The aim of this ticket is to make this possible.


CC:  @tscrim

Keywords: **matrix space classcall**

Branch/Commit: **[532bd2a](https://github.com/sagemath/sagetrac-mirror/commit/532bd2a40ce9772aaf7e4c27bfa37ab0498117c8)**

Reviewer: **Travis Scrimshaw**

Author: **Sebastian Oehms**

_Issue created by migration from https://trac.sagemath.org/ticket/29466_





---

archive/issue_events_387005.json:
```json
{
    "actor": "https://github.com/soehms",
    "created_at": "2020-04-05T10:59:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29466#event-387005"
}
```



---

archive/issue_events_387006.json:
```json
{
    "actor": "https://github.com/soehms",
    "created_at": "2020-04-05T10:59:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29466#event-387006"
}
```



---

archive/issue_events_387007.json:
```json
{
    "actor": "https://github.com/soehms",
    "created_at": "2020-04-05T10:59:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29466#event-387007"
}
```



---

archive/issue_events_387008.json:
```json
{
    "actor": "https://github.com/soehms",
    "created_at": "2020-04-05T10:59:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29466#event-387008"
}
```



---

archive/issue_comments_466093.json:
```json
{
    "body": "Branch: **[u/soehms/classcall_matrix_space_29466](https://github.com/sagemath/sagetrac-mirror/tree/u/soehms/classcall_matrix_space_29466)**",
    "created_at": "2020-04-05T11:08:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29466#issuecomment-466093",
    "user": "https://github.com/soehms"
}
```

Branch: **[u/soehms/classcall_matrix_space_29466](https://github.com/sagemath/sagetrac-mirror/tree/u/soehms/classcall_matrix_space_29466)**



---

archive/issue_comments_466094.json:
```json
{
    "body": "Commit: **[8005001](https://github.com/sagemath/sagetrac-mirror/commit/8005001685f0babe1e9121ab061843b0c57a97d8)**",
    "created_at": "2020-04-05T11:10:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29466#issuecomment-466094",
    "user": "https://github.com/soehms"
}
```

Commit: **[8005001](https://github.com/sagemath/sagetrac-mirror/commit/8005001685f0babe1e9121ab061843b0c57a97d8)**



---

archive/issue_events_387009.json:
```json
{
    "actor": "https://github.com/soehms",
    "created_at": "2020-04-05T11:10:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29466#event-387009"
}
```



---

archive/issue_comments_466095.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8005001685f0babe1e9121ab061843b0c57a97d8\">8005001</a></td><td><code>29466 initial</code></td></tr></table>\n",
    "created_at": "2020-04-05T11:10:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29466#issuecomment-466095",
    "user": "https://github.com/soehms"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8005001685f0babe1e9121ab061843b0c57a97d8">8005001</a></td><td><code>29466 initial</code></td></tr></table>




---

archive/issue_comments_466096.json:
```json
{
    "body": "Author: **Sebastian Oehms**",
    "created_at": "2020-04-05T11:10:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29466#issuecomment-466096",
    "user": "https://github.com/soehms"
}
```

Author: **Sebastian Oehms**



---

archive/issue_comments_466097.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4e6f3c8adae5f6d5711180933eddacf912c83d03\">4e6f3c8</a></td><td><code>29466: doc fixes in tutorial</code></td></tr></table>\n",
    "created_at": "2020-04-05T12:25:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29466#issuecomment-466097",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:3'>Comment 3:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4e6f3c8adae5f6d5711180933eddacf912c83d03">4e6f3c8</a></td><td><code>29466: doc fixes in tutorial</code></td></tr></table>




---

archive/issue_comments_466098.json:
```json
{
    "body": "Changed commit from **[8005001](https://github.com/sagemath/sagetrac-mirror/commit/8005001685f0babe1e9121ab061843b0c57a97d8)** to **[4e6f3c8](https://github.com/sagemath/sagetrac-mirror/commit/4e6f3c8adae5f6d5711180933eddacf912c83d03)**",
    "created_at": "2020-04-05T12:25:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29466#issuecomment-466098",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[8005001](https://github.com/sagemath/sagetrac-mirror/commit/8005001685f0babe1e9121ab061843b0c57a97d8)** to **[4e6f3c8](https://github.com/sagemath/sagetrac-mirror/commit/4e6f3c8adae5f6d5711180933eddacf912c83d03)**



---

archive/issue_events_387010.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29466#event-387010"
}
```



---

archive/issue_events_387011.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29466#event-387011"
}
```



---

archive/issue_comments_466099.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBatch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.",
    "created_at": "2020-04-14T19:41:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29466#issuecomment-466099",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:4'>Comment 4:</a>
Batch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.



---

archive/issue_events_387012.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-04-19T22:26:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29466#event-387012"
}
```



---

archive/issue_events_387013.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-04-19T22:26:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29466#event-387013"
}
```



---

archive/issue_comments_466100.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nLGTM.",
    "created_at": "2020-04-19T22:26:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29466#issuecomment-466100",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:5'>Comment 5:</a>
LGTM.



---

archive/issue_comments_466101.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2020-04-19T22:26:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29466#issuecomment-466101",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_466102.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nThanks!",
    "created_at": "2020-04-20T07:26:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29466#issuecomment-466102",
    "user": "https://github.com/soehms"
}
```

<a id='comment:6'>Comment 6:</a>
Thanks!



---

archive/issue_comments_466103.json:
```json
{
    "body": "Changed branch from **[u/soehms/classcall_matrix_space_29466](https://github.com/sagemath/sagetrac-mirror/tree/u/soehms/classcall_matrix_space_29466)** to **[4e6f3c8](https://github.com/sagemath/sagetrac-mirror/commit/4e6f3c8adae5f6d5711180933eddacf912c83d03)**",
    "created_at": "2020-04-23T22:32:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29466#issuecomment-466103",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/soehms/classcall_matrix_space_29466](https://github.com/sagemath/sagetrac-mirror/tree/u/soehms/classcall_matrix_space_29466)** to **[4e6f3c8](https://github.com/sagemath/sagetrac-mirror/commit/4e6f3c8adae5f6d5711180933eddacf912c83d03)**



---

archive/issue_events_387014.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-04-23T22:32:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29466#event-387014"
}
```



---

archive/issue_events_387015.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-04-23T22:32:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29466#event-387015"
}
```



---

archive/issue_comments_466104.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nOn Python 2:\n\n```\nFile \"src/sage/matrix/matrix_space.py\", line 487, in sage.matrix.matrix_space.MatrixSpace.__classcall__\nFailed example:\n    MS2 = MyMatrixSpace(ZZ, 2, my_option=False)\nException raised:\n    Traceback (most recent call last):\n      File \"/var/lib/buildbot/slave/sage2_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py\", line 681, in _run\n        self.compile_and_execute(example, compiler, test.globs)\n      File \"/var/lib/buildbot/slave/sage2_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py\", line 1123, in compile_and_execute\n        exec(compiled, globs)\n      File \"<doctest sage.matrix.matrix_space.MatrixSpace.__classcall__[11]>\", line 1, in <module>\n        MS2 = MyMatrixSpace(ZZ, Integer(2), my_option=False)\n      File \"sage/misc/classcall_metaclass.pyx\", line 334, in sage.misc.classcall_metaclass.ClasscallMetaclass.__call__ (build/cythonized/sage/misc/classcall_metaclass.c:1741)\n        return cls.classcall(cls, *args, **kwds)\n    TypeError: unbound method __classcall__() must be called with MyMatrixSpace instance as first argument (got ClasscallMetaclass instance instead)\n```",
    "created_at": "2020-04-24T22:27:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29466#issuecomment-466104",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:8'>Comment 8:</a>
On Python 2:

```
File "src/sage/matrix/matrix_space.py", line 487, in sage.matrix.matrix_space.MatrixSpace.__classcall__
Failed example:
    MS2 = MyMatrixSpace(ZZ, 2, my_option=False)
Exception raised:
    Traceback (most recent call last):
      File "/var/lib/buildbot/slave/sage2_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py", line 681, in _run
        self.compile_and_execute(example, compiler, test.globs)
      File "/var/lib/buildbot/slave/sage2_git/build/local/lib/python2.7/site-packages/sage/doctest/forker.py", line 1123, in compile_and_execute
        exec(compiled, globs)
      File "<doctest sage.matrix.matrix_space.MatrixSpace.__classcall__[11]>", line 1, in <module>
        MS2 = MyMatrixSpace(ZZ, Integer(2), my_option=False)
      File "sage/misc/classcall_metaclass.pyx", line 334, in sage.misc.classcall_metaclass.ClasscallMetaclass.__call__ (build/cythonized/sage/misc/classcall_metaclass.c:1741)
        return cls.classcall(cls, *args, **kwds)
    TypeError: unbound method __classcall__() must be called with MyMatrixSpace instance as first argument (got ClasscallMetaclass instance instead)
```



---

archive/issue_comments_466105.json:
```json
{
    "body": "Changed branch from **[4e6f3c8](https://github.com/sagemath/sagetrac-mirror/commit/4e6f3c8adae5f6d5711180933eddacf912c83d03)** to **[u/soehms/classcall_matrix_space_29466](https://github.com/sagemath/sagetrac-mirror/tree/u/soehms/classcall_matrix_space_29466)**",
    "created_at": "2020-04-24T22:27:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29466#issuecomment-466105",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[4e6f3c8](https://github.com/sagemath/sagetrac-mirror/commit/4e6f3c8adae5f6d5711180933eddacf912c83d03)** to **[u/soehms/classcall_matrix_space_29466](https://github.com/sagemath/sagetrac-mirror/tree/u/soehms/classcall_matrix_space_29466)**



---

archive/issue_comments_466106.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nActually, I am more surprised that is not an issue on Python3.\n\nHere is the fix:\n\n```diff\n             sage: class MyMatrixSpace(MatrixSpace):\n+            ....:     @staticmethod\n             ....:     def __classcall__(cls, base_ring, nrows, ncols=None, my_option=True, sparse=False, implementation=None):\n```\nAlso, I missed this on the first go-around:\n\n```diff\n-        Check that :`29466`is fixed::\n+        Check that :trac:`29466`is fixed::\n```\nYou can set it back to a positive review once done Sebastian.",
    "created_at": "2020-04-24T22:31:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29466#issuecomment-466106",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:9'>Comment 9:</a>
Actually, I am more surprised that is not an issue on Python3.

Here is the fix:

```diff
             sage: class MyMatrixSpace(MatrixSpace):
+            ....:     @staticmethod
             ....:     def __classcall__(cls, base_ring, nrows, ncols=None, my_option=True, sparse=False, implementation=None):
```
Also, I missed this on the first go-around:

```diff
-        Check that :`29466`is fixed::
+        Check that :trac:`29466`is fixed::
```
You can set it back to a positive review once done Sebastian.



---

archive/issue_comments_466107.json:
```json
{
    "body": "Changed commit from **[4e6f3c8](https://github.com/sagemath/sagetrac-mirror/commit/4e6f3c8adae5f6d5711180933eddacf912c83d03)** to **[532bd2a](https://github.com/sagemath/sagetrac-mirror/commit/532bd2a40ce9772aaf7e4c27bfa37ab0498117c8)**",
    "created_at": "2020-04-25T06:39:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29466#issuecomment-466107",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[4e6f3c8](https://github.com/sagemath/sagetrac-mirror/commit/4e6f3c8adae5f6d5711180933eddacf912c83d03)** to **[532bd2a](https://github.com/sagemath/sagetrac-mirror/commit/532bd2a40ce9772aaf7e4c27bfa37ab0498117c8)**



---

archive/issue_comments_466108.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/532bd2a40ce9772aaf7e4c27bfa37ab0498117c8\">532bd2a</a></td><td><code>insert missing @staticmethod and fix typo</code></td></tr></table>\n",
    "created_at": "2020-04-25T06:39:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29466#issuecomment-466108",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:10'>Comment 10:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/532bd2a40ce9772aaf7e4c27bfa37ab0498117c8">532bd2a</a></td><td><code>insert missing @staticmethod and fix typo</code></td></tr></table>




---

archive/issue_events_387016.json:
```json
{
    "actor": "https://github.com/soehms",
    "created_at": "2020-04-25T06:43:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29466#event-387016"
}
```



---

archive/issue_events_387017.json:
```json
{
    "actor": "https://github.com/soehms",
    "created_at": "2020-04-25T06:46:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29466#event-387017"
}
```



---

archive/issue_events_387018.json:
```json
{
    "actor": "https://github.com/soehms",
    "created_at": "2020-04-25T06:46:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29466#event-387018"
}
```



---

archive/issue_comments_466109.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nTest with Python 2 is working, right now.\nThanks again!",
    "created_at": "2020-04-25T06:46:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29466#issuecomment-466109",
    "user": "https://github.com/soehms"
}
```

<a id='comment:12'>Comment 12:</a>
Test with Python 2 is working, right now.
Thanks again!



---

archive/issue_comments_466110.json:
```json
{
    "body": "Changed branch from **[u/soehms/classcall_matrix_space_29466](https://github.com/sagemath/sagetrac-mirror/tree/u/soehms/classcall_matrix_space_29466)** to **[532bd2a](https://github.com/sagemath/sagetrac-mirror/commit/532bd2a40ce9772aaf7e4c27bfa37ab0498117c8)**",
    "created_at": "2020-04-26T22:45:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29466#issuecomment-466110",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/soehms/classcall_matrix_space_29466](https://github.com/sagemath/sagetrac-mirror/tree/u/soehms/classcall_matrix_space_29466)** to **[532bd2a](https://github.com/sagemath/sagetrac-mirror/commit/532bd2a40ce9772aaf7e4c27bfa37ab0498117c8)**



---

archive/issue_events_387019.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-04-26T22:45:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29466#event-387019"
}
```



---

archive/issue_events_387020.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "715458fd4a5479bc3fb457de6f57a17584df9429",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-04-26T22:45:57Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29466#event-387020"
}
```



---

archive/issue_events_387021.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-05-02T02:15:55Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29466#event-387021"
}
```



---

archive/issue_events_387022.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-05-02T02:15:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29466",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29466#event-387022"
}
```
