# Issue 29440: Improve doctests of Degenerate Submanifold

archive/issues_029203.json:
```json
{
    "assignees": [],
    "body": "The doctests in `sage -t src/sage/manifolds/differentiable/degenerate_submanifold.py` have problems with time outs.\n\nSee for example:\n- [[#29104 comment:61](https://github.com/sagemath/sage/issues/29104#comment:61)]\n- [https://groups.google.com/d/msg/sage-release/eMQIlhglN98/bTwDGD9HBQAJ](https://groups.google.com/d/msg/sage-release/eMQIlhglN98/bTwDGD9HBQAJ)\n- [[#27122 comment:33](https://github.com/sagemath/sage/issues/27122#comment:33)]\n\nPart of the problem might be, that the file makes no use of `long time` flags (testing takes 86 seconds on a fast computer without `--long`).\n\nWe add those flags and some smaller examples to display functionality of the module. This brings down testing time without `--long`` to less than 30 seconds.\n\nCC:  @sagetrac-Dicolevrai @egourgoulhon @mkoeppe\n\nKeywords: **degenerate manifolds, doctests**\n\nBranch/Commit: **[848fec0](https://github.com/sagemath/sagetrac-mirror/commit/848fec03b635a9f044b4f3daf39f45538a3f421e)**\n\nReviewer: **Hans Fotsing Tetsing**\n\nAuthor: **Jonathan Kliem**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29440_\n\n",
    "closed_at": "2020-04-09T22:44:31Z",
    "created_at": "2020-03-31T19:53:11Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Improve doctests of Degenerate Submanifold",
    "type": "issue",
    "updated_at": "2020-04-09T22:44:31Z",
    "url": "https://github.com/sagemath/sage/issues/29440",
    "user": "https://github.com/kliem"
}
```
The doctests in `sage -t src/sage/manifolds/differentiable/degenerate_submanifold.py` have problems with time outs.

See for example:
- [[#29104 comment:61](https://github.com/sagemath/sage/issues/29104#comment:61)]
- [https://groups.google.com/d/msg/sage-release/eMQIlhglN98/bTwDGD9HBQAJ](https://groups.google.com/d/msg/sage-release/eMQIlhglN98/bTwDGD9HBQAJ)
- [[#27122 comment:33](https://github.com/sagemath/sage/issues/27122#comment:33)]

Part of the problem might be, that the file makes no use of `long time` flags (testing takes 86 seconds on a fast computer without `--long`).

We add those flags and some smaller examples to display functionality of the module. This brings down testing time without `--long`` to less than 30 seconds.

CC:  @sagetrac-Dicolevrai @egourgoulhon @mkoeppe

Keywords: **degenerate manifolds, doctests**

Branch/Commit: **[848fec0](https://github.com/sagemath/sagetrac-mirror/commit/848fec03b635a9f044b4f3daf39f45538a3f421e)**

Reviewer: **Hans Fotsing Tetsing**

Author: **Jonathan Kliem**

_Issue created by migration from https://trac.sagemath.org/ticket/29440_





---

archive/issue_events_386660.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-03-31T19:53:11Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29440",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29440#event-386660"
}
```



---

archive/issue_events_386661.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-03-31T19:53:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29440",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "component: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29440#event-386661"
}
```



---

archive/issue_events_386662.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-03-31T19:53:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29440",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29440#event-386662"
}
```



---

archive/issue_events_386663.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-03-31T19:53:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29440",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29440#event-386663"
}
```



---

archive/issue_comments_465509.json:
```json
{
    "body": "Branch: **[public/29440](https://github.com/sagemath/sagetrac-mirror/tree/public/29440)**",
    "created_at": "2020-03-31T19:54:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29440#issuecomment-465509",
    "user": "https://github.com/kliem"
}
```

Branch: **[public/29440](https://github.com/sagemath/sagetrac-mirror/tree/public/29440)**



---

archive/issue_comments_465510.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ef94ca59cc368c858958e408dcfa27c6ea05cd0c\">ef94ca5</a></td><td><code>use # long time and add a smaller example</code></td></tr></table>\n",
    "created_at": "2020-03-31T19:54:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29440#issuecomment-465510",
    "user": "https://github.com/kliem"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ef94ca59cc368c858958e408dcfa27c6ea05cd0c">ef94ca5</a></td><td><code>use # long time and add a smaller example</code></td></tr></table>




---

archive/issue_comments_465511.json:
```json
{
    "body": "Commit: **[ef94ca5](https://github.com/sagemath/sagetrac-mirror/commit/ef94ca59cc368c858958e408dcfa27c6ea05cd0c)**",
    "created_at": "2020-03-31T19:54:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29440#issuecomment-465511",
    "user": "https://github.com/kliem"
}
```

Commit: **[ef94ca5](https://github.com/sagemath/sagetrac-mirror/commit/ef94ca59cc368c858958e408dcfa27c6ea05cd0c)**



---

archive/issue_events_386664.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-03-31T19:54:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29440",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29440#event-386664"
}
```



---

archive/issue_comments_465512.json:
```json
{
    "body": "Changed commit from **[ef94ca5](https://github.com/sagemath/sagetrac-mirror/commit/ef94ca59cc368c858958e408dcfa27c6ea05cd0c)** to **[848fec0](https://github.com/sagemath/sagetrac-mirror/commit/848fec03b635a9f044b4f3daf39f45538a3f421e)**",
    "created_at": "2020-04-01T09:02:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29440#issuecomment-465512",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[ef94ca5](https://github.com/sagemath/sagetrac-mirror/commit/ef94ca59cc368c858958e408dcfa27c6ea05cd0c)** to **[848fec0](https://github.com/sagemath/sagetrac-mirror/commit/848fec03b635a9f044b4f3daf39f45538a3f421e)**



---

archive/issue_comments_465513.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/848fec03b635a9f044b4f3daf39f45538a3f421e\">848fec0</a></td><td><code>removed redundant import</code></td></tr></table>\n",
    "created_at": "2020-04-01T09:02:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29440#issuecomment-465513",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/848fec03b635a9f044b4f3daf39f45538a3f421e">848fec0</a></td><td><code>removed redundant import</code></td></tr></table>




---

archive/issue_comments_465514.json:
```json
{
    "body": "Replying to [ticket:29440 gh-kliem]:\n> The doctests in `sage -t src/sage/manifolds/differentiable/degenerate_submanifold.py` have problems with time outs.\n> \n> See for example:\n> - [[#29104 comment:61](https://github.com/sagemath/sage/issues/29104#comment:61)]\n> - [https://groups.google.com/d/msg/sage-release/eMQIlhglN98/bTwDGD9HBQAJ](https://groups.google.com/d/msg/sage-release/eMQIlhglN98/bTwDGD9HBQAJ)\n> - [[#27122 comment:33](https://github.com/sagemath/sage/issues/27122#comment:33)]\n> \n> Part of the problem might be, that the file makes no use of `long time` flags (testing takes 86 seconds on a fast computer without `--long`).\n> \n> We add those flags and some smaller examples to display functionality of the module. This brings down testing time without `--long`` to less than 30 seconds.\n\nThank you so much for the improvement and the example!",
    "created_at": "2020-04-02T14:01:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29440#issuecomment-465514",
    "user": "https://github.com/sagetrac-Dicolevrai"
}
```

Replying to [ticket:29440 gh-kliem]:
> The doctests in `sage -t src/sage/manifolds/differentiable/degenerate_submanifold.py` have problems with time outs.
> 
> See for example:
> - [[#29104 comment:61](https://github.com/sagemath/sage/issues/29104#comment:61)]
> - [https://groups.google.com/d/msg/sage-release/eMQIlhglN98/bTwDGD9HBQAJ](https://groups.google.com/d/msg/sage-release/eMQIlhglN98/bTwDGD9HBQAJ)
> - [[#27122 comment:33](https://github.com/sagemath/sage/issues/27122#comment:33)]
> 
> Part of the problem might be, that the file makes no use of `long time` flags (testing takes 86 seconds on a fast computer without `--long`).
> 
> We add those flags and some smaller examples to display functionality of the module. This brings down testing time without `--long`` to less than 30 seconds.

Thank you so much for the improvement and the example!



---

archive/issue_events_386665.json:
```json
{
    "actor": "https://github.com/sagetrac-Dicolevrai",
    "created_at": "2020-04-02T14:01:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29440",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29440#event-386665"
}
```



---

archive/issue_events_386666.json:
```json
{
    "actor": "https://github.com/sagetrac-Dicolevrai",
    "created_at": "2020-04-02T14:01:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29440",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29440#event-386666"
}
```



---

archive/issue_comments_465515.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nplease add missing reviewer full name",
    "created_at": "2020-04-02T18:19:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29440#issuecomment-465515",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:5" align="right">Comment 5</div>

please add missing reviewer full name



---

archive/issue_comments_465516.json:
```json
{
    "body": "Changed author from **Jonathan Kliem** to none",
    "created_at": "2020-04-03T00:27:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29440#issuecomment-465516",
    "user": "https://github.com/sagetrac-Dicolevrai"
}
```

Changed author from **Jonathan Kliem** to none



---

archive/issue_comments_465517.json:
```json
{
    "body": "Reviewer: **Hans Fotsing Tetsing**",
    "created_at": "2020-04-03T00:27:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29440#issuecomment-465517",
    "user": "https://github.com/sagetrac-Dicolevrai"
}
```

Reviewer: **Hans Fotsing Tetsing**



---

archive/issue_comments_465518.json:
```json
{
    "body": "Author: **Jonathan Kliem**",
    "created_at": "2020-04-03T05:36:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29440#issuecomment-465518",
    "user": "https://github.com/kliem"
}
```

Author: **Jonathan Kliem**



---

archive/issue_events_386667.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-04-09T22:44:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29440",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29440#event-386667"
}
```



---

archive/issue_events_386668.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "8d4771eb277778e6ed971a8b4178bdef8a20f638",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-04-09T22:44:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29440",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29440#event-386668"
}
```



---

archive/issue_comments_465519.json:
```json
{
    "body": "Changed branch from **[public/29440](https://github.com/sagemath/sagetrac-mirror/tree/public/29440)** to **[848fec0](https://github.com/sagemath/sagetrac-mirror/commit/848fec03b635a9f044b4f3daf39f45538a3f421e)**",
    "created_at": "2020-04-09T22:44:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29440#issuecomment-465519",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/29440](https://github.com/sagemath/sagetrac-mirror/tree/public/29440)** to **[848fec0](https://github.com/sagemath/sagetrac-mirror/commit/848fec03b635a9f044b4f3daf39f45538a3f421e)**
