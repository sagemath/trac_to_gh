# Issue 29688: Finite numbers in the signed InfinityRing ironically don't have a sign

archive/issues_029451.json:
```json
{
    "body": "Keywords: InfinityRing,Infinity,sign\n\n```\nsage: sign(Infinity)\n1\nsage: sign(-Infinity)\n-1\nsage: sign(InfinityRing(3))\n(...)\nNotImplementedError: \n```\n\nThis is unfortunate because it would be convenient in an implementation of [Sturm's theorem](https://en.wikipedia.org/wiki/Sturm%27s_theorem).\n\nIssue created by migration from https://trac.sagemath.org/ticket/29688\n\n",
    "closed_at": "2020-07-10T19:34:07Z",
    "created_at": "2020-05-14T10:05:51Z",
    "labels": [
        "component: basic arithmetic",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.2",
    "title": "Finite numbers in the signed InfinityRing ironically don't have a sign",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/29688",
    "user": "https://github.com/rburing"
}
```
Keywords: InfinityRing,Infinity,sign

```
sage: sign(Infinity)
1
sage: sign(-Infinity)
-1
sage: sign(InfinityRing(3))
(...)
NotImplementedError: 
```

This is unfortunate because it would be convenient in an implementation of [Sturm's theorem](https://en.wikipedia.org/wiki/Sturm%27s_theorem).

Issue created by migration from https://trac.sagemath.org/ticket/29688





---

archive/issue_comments_417519.json:
```json
{
    "body": "How would you make a ring with signed finite numbers ?",
    "created_at": "2020-05-14T11:49:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29688#issuecomment-417519",
    "user": "https://github.com/fchapoton"
}
```

How would you make a ring with signed finite numbers ?



---

archive/issue_comments_417520.json:
```json
{
    "body": "By definition [InfinityRing](http://doc.sagemath.org/html/en/reference/rings/sage/rings/infinity.html) consists of five elements; all their signs are known, but apparently the `sign` function does not know them all.",
    "created_at": "2020-05-14T12:12:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29688#issuecomment-417520",
    "user": "https://github.com/rburing"
}
```

By definition [InfinityRing](http://doc.sagemath.org/html/en/reference/rings/sage/rings/infinity.html) consists of five elements; all their signs are known, but apparently the `sign` function does not know them all.



---

archive/issue_comments_417521.json:
```json
{
    "body": "ok, so this is not a ring, still in the category of Rings. Sigh.",
    "created_at": "2020-05-14T14:01:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29688#issuecomment-417521",
    "user": "https://github.com/fchapoton"
}
```

ok, so this is not a ring, still in the category of Rings. Sigh.



---

archive/issue_comments_417522.json:
```json
{
    "body": "I think this PR solves the problem.\n\n```\nsage: sign(InfinityRing(3))\n1\nsage: sign(InfinityRing(0))\n0\nsage: sign(InfinityRing(-3))\n-1\nsage: sign(UnsignedInfinityRing(3))\n...\nNotImplementedError: sign of number < oo is not well defined\n```\nI'm not entirely happy about raising a `NotImplementedError` in the unsigned case, but I couldn't think of anything better. `ValueError` and `SignError` don't work, because the `BuiltinFunction` `sign` doesn't take either of those as the final word, and tries other ways to calculate the value, which results in an error message that is confusing.\n\n---\nNew commits:",
    "created_at": "2020-05-15T04:15:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29688#issuecomment-417522",
    "user": "https://github.com/DaveWitteMorris"
}
```

I think this PR solves the problem.

```
sage: sign(InfinityRing(3))
1
sage: sign(InfinityRing(0))
0
sage: sign(InfinityRing(-3))
-1
sage: sign(UnsignedInfinityRing(3))
...
NotImplementedError: sign of number < oo is not well defined
```
I'm not entirely happy about raising a `NotImplementedError` in the unsigned case, but I couldn't think of anything better. `ValueError` and `SignError` don't work, because the `BuiltinFunction` `sign` doesn't take either of those as the final word, and tries other ways to calculate the value, which results in an error message that is confusing.

---
New commits:



---

archive/issue_comments_417523.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2020-05-15T04:15:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29688#issuecomment-417523",
    "user": "https://github.com/DaveWitteMorris"
}
```

Changing status from new to needs_review.



---

archive/issue_events_075908.json:
```json
{
    "actor": "https://github.com/DaveWitteMorris",
    "created_at": "2020-05-15T04:15:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/29688",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29688#event-75908"
}
```



---

archive/issue_comments_417524.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2020-07-04T19:59:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29688#issuecomment-417524",
    "user": "https://github.com/mkoeppe"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_417525.json:
```json
{
    "body": "Thanks!",
    "created_at": "2020-07-04T20:14:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29688#issuecomment-417525",
    "user": "https://github.com/DaveWitteMorris"
}
```

Thanks!



---

archive/issue_events_075909.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-07-10T19:34:07Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/29688",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29688#event-75909"
}
```



---

archive/issue_comments_417526.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2020-07-10T19:34:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29688#issuecomment-417526",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
