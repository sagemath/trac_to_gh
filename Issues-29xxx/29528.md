# Issue 29528: Singular: segmentation fault in plural.pyx

archive/issues_029291.json:
```json
{
    "body": "This ticket tracks a random segmentation fault in `plural.pyx` that occasionally appears with Singular \u2265 4.1.1p3. The corresponding doctest is disabled in #25993.\n\n```\nFile \"src/sage/rings/polynomial/plural.pyx\", line 400, in sage.rings.polynomial.plural.NCPolynomialRing_plural.__dealloc__\nFailed example:\n    R2 = A2.g_algebra({y*x:x*y-z, z*x:x*z+2*x, z*y:y*z-2*y}, order=TermOrder('degrevlex', 2))\nException raised:\n    Traceback (most recent call last):\n      File \"/amd/compute/mwagerin/git/sage_compute/python3/local/lib/python3.7/site-packages/sage/doctest/forker.py\", line 681, in _run\n        self.compile_and_execute(example, compiler, test.globs)\n      File \"/amd/compute/mwagerin/git/sage_compute/python3/local/lib/python3.7/site-packages/sage/doctest/forker.py\", line 1123, in compile_and_execute\n        exec(compiled, globs)\n      File \"<doctest sage.rings.polynomial.plural.NCPolynomialRing_plural.__dealloc__[6]>\", line 1, in <module>\n        R2 = A2.g_algebra({y*x:x*y-z, z*x:x*z+Integer(2)*x, z*y:y*z-Integer(2)*y}, order=TermOrder('degrevlex', Integer(2)))\n      File \"/amd/compute/mwagerin/git/sage_compute/python3/local/lib/python3.7/site-packages/sage/algebras/free_algebra.py\", line 887, in g_algebra\n        order=order, check=check)\n      File \"sage/structure/factory.pyx\", line 369, in sage.structure.factory.UniqueFactory.__call__ (build/cythonized/sage/structure/factory.c:2240)\n        return self.get_object(version, key, kwds)\n      File \"sage/structure/factory.pyx\", line 412, in sage.structure.factory.UniqueFactory.get_object (build/cythonized/sage/structure/factory.c:2631)\n        obj = self.create_object(version, key, **extra_args)\n      File \"sage/rings/polynomial/plural.pyx\", line 175, in sage.rings.polynomial.plural.G_AlgFactory.create_object (build/cythonized/sage/rings/polynomial/plural.cpp:5432)\n        return NCPolynomialRing_plural(base_ring, names, c, d, order,\n      File \"sage/rings/polynomial/plural.pyx\", line 355, in sage.rings.polynomial.plural.NCPolynomialRing_plural.__init__ (build/cythonized/sage/rings/polynomial/plural.cpp:6707)\n        test = ff.nctools__lib.ndcond(ring = self)\n      File \"sage/libs/singular/function.pyx\", line 1334, in sage.libs.singular.function.SingularFunction.__call__ (build/cythonized/sage/libs/singular/function.cpp:14977)\n        return call_function(self, args, ring, interruptible, attributes)\n      File \"sage/libs/singular/function.pyx\", line 1516, in sage.libs.singular.function.call_function (build/cythonized/sage/libs/singular/function.cpp:16809)\n        with opt_ctx: # we are preserving the global options state here\n      File \"sage/libs/singular/function.pyx\", line 1518, in sage.libs.singular.function.call_function (build/cythonized/sage/libs/singular/function.cpp:16721)\n        sig_on()\n    cysignals.signals.SignalError: Segmentation fault\n```\n\nIf this error occurs when running the tests, it is usually persistent and reproducible, but isolating the problem is difficult, as the corresponding doctest does not fail when run standalone.\n\nSee #25993 for more details.\n\n\nKeywords: singular, libsingular\n\nStatus: new\n\nDependencies: #13447\n\nIssue created by migration from https://trac.sagemath.org/ticket/29528\n\n",
    "created_at": "2020-04-18T17:19:42Z",
    "labels": [
        "component: interfaces",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.8",
    "title": "Singular: segmentation fault in plural.pyx",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/29528",
    "user": "https://github.com/mwageringel"
}
```
This ticket tracks a random segmentation fault in `plural.pyx` that occasionally appears with Singular ≥ 4.1.1p3. The corresponding doctest is disabled in #25993.

```
File "src/sage/rings/polynomial/plural.pyx", line 400, in sage.rings.polynomial.plural.NCPolynomialRing_plural.__dealloc__
Failed example:
    R2 = A2.g_algebra({y*x:x*y-z, z*x:x*z+2*x, z*y:y*z-2*y}, order=TermOrder('degrevlex', 2))
Exception raised:
    Traceback (most recent call last):
      File "/amd/compute/mwagerin/git/sage_compute/python3/local/lib/python3.7/site-packages/sage/doctest/forker.py", line 681, in _run
        self.compile_and_execute(example, compiler, test.globs)
      File "/amd/compute/mwagerin/git/sage_compute/python3/local/lib/python3.7/site-packages/sage/doctest/forker.py", line 1123, in compile_and_execute
        exec(compiled, globs)
      File "<doctest sage.rings.polynomial.plural.NCPolynomialRing_plural.__dealloc__[6]>", line 1, in <module>
        R2 = A2.g_algebra({y*x:x*y-z, z*x:x*z+Integer(2)*x, z*y:y*z-Integer(2)*y}, order=TermOrder('degrevlex', Integer(2)))
      File "/amd/compute/mwagerin/git/sage_compute/python3/local/lib/python3.7/site-packages/sage/algebras/free_algebra.py", line 887, in g_algebra
        order=order, check=check)
      File "sage/structure/factory.pyx", line 369, in sage.structure.factory.UniqueFactory.__call__ (build/cythonized/sage/structure/factory.c:2240)
        return self.get_object(version, key, kwds)
      File "sage/structure/factory.pyx", line 412, in sage.structure.factory.UniqueFactory.get_object (build/cythonized/sage/structure/factory.c:2631)
        obj = self.create_object(version, key, **extra_args)
      File "sage/rings/polynomial/plural.pyx", line 175, in sage.rings.polynomial.plural.G_AlgFactory.create_object (build/cythonized/sage/rings/polynomial/plural.cpp:5432)
        return NCPolynomialRing_plural(base_ring, names, c, d, order,
      File "sage/rings/polynomial/plural.pyx", line 355, in sage.rings.polynomial.plural.NCPolynomialRing_plural.__init__ (build/cythonized/sage/rings/polynomial/plural.cpp:6707)
        test = ff.nctools__lib.ndcond(ring = self)
      File "sage/libs/singular/function.pyx", line 1334, in sage.libs.singular.function.SingularFunction.__call__ (build/cythonized/sage/libs/singular/function.cpp:14977)
        return call_function(self, args, ring, interruptible, attributes)
      File "sage/libs/singular/function.pyx", line 1516, in sage.libs.singular.function.call_function (build/cythonized/sage/libs/singular/function.cpp:16809)
        with opt_ctx: # we are preserving the global options state here
      File "sage/libs/singular/function.pyx", line 1518, in sage.libs.singular.function.call_function (build/cythonized/sage/libs/singular/function.cpp:16721)
        sig_on()
    cysignals.signals.SignalError: Segmentation fault
```

If this error occurs when running the tests, it is usually persistent and reproducible, but isolating the problem is difficult, as the corresponding doctest does not fail when run standalone.

See #25993 for more details.


Keywords: singular, libsingular

Status: new

Dependencies: #13447

Issue created by migration from https://trac.sagemath.org/ticket/29528





---

archive/issue_comments_577089.json:
```json
{
    "body": "<a id='comment:1'></a>#13447 might solve this.",
    "created_at": "2020-06-06T12:39:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29528",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29528#issuecomment-577089",
    "user": "https://github.com/mwageringel"
}
```

<a id='comment:1'></a>#13447 might solve this.



---

archive/issue_comments_577090.json:
```json
{
    "body": "Changing dependencies from \"\" to \"#13447\"",
    "created_at": "2020-06-06T12:39:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29528",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29528#issuecomment-577090",
    "user": "https://github.com/mwageringel"
}
```

Changing dependencies from "" to "#13447"



---

archive/issue_comments_577091.json:
```json
{
    "body": "<a id='comment:2'></a>Also spotted here: https://github.com/mkoeppe/sage/runs/759322667?check_suite_focus=true",
    "created_at": "2020-06-14T01:30:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29528",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29528#issuecomment-577091",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:2'></a>Also spotted here: https://github.com/mkoeppe/sage/runs/759322667?check_suite_focus=true



---

archive/issue_comments_577092.json:
```json
{
    "body": "<a id='comment:3'></a>Thanks for the report. This confirms that this problem also occurs with Singular 4.1.1p2 \u2013 the version currently in Sage.",
    "created_at": "2020-06-14T11:04:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29528",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29528#issuecomment-577092",
    "user": "https://github.com/mwageringel"
}
```

<a id='comment:3'></a>Thanks for the report. This confirms that this problem also occurs with Singular 4.1.1p2 – the version currently in Sage.



---

archive/issue_events_075321.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/29528",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29528#event-75321"
}
```



---

archive/issue_comments_577093.json:
```json
{
    "body": "<a id='comment:5'></a>see also #30745 where this raises again, with Flint 2.6.3. \n\nBut interestingly with #25993 (with these tests reinstated) everything works.",
    "created_at": "2020-11-20T23:57:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29528",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29528#issuecomment-577093",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:5'></a>see also #30745 where this raises again, with Flint 2.6.3. 

But interestingly with #25993 (with these tests reinstated) everything works.



---

archive/issue_events_075322.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-05-10T17:42:09Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/29528",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29528#event-75322"
}
```



---

archive/issue_events_075323.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-05-10T17:42:09Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/29528",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29528#event-75323"
}
```



---

archive/issue_comments_577094.json:
```json
{
    "body": "<a id='comment:6'></a>Moving to 9.4, as 9.3 has been released.",
    "created_at": "2021-05-10T17:42:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29528",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29528#issuecomment-577094",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:6'></a>Moving to 9.4, as 9.3 has been released.



---

archive/issue_events_075324.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-22T20:35:10Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/29528",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29528#event-75324"
}
```



---

archive/issue_events_075325.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-22T20:35:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/29528",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29528#event-75325"
}
```



---

archive/issue_events_075326.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T20:12:32Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/29528",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29528#event-75326"
}
```



---

archive/issue_events_075327.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T20:12:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/29528",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29528#event-75327"
}
```



---

archive/issue_events_075328.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/29528",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29528#event-75328"
}
```



---

archive/issue_events_075329.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/29528",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29528#event-75329"
}
```



---

archive/issue_events_075330.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/29528",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29528#event-75330"
}
```



---

archive/issue_events_075331.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/29528",
    "milestone": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29528#event-75331"
}
```
