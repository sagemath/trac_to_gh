# Issue 29981: Make documentation doctests ready for random seeds

archive/issues_029744.json:
```json
{
    "assignees": [],
    "body": "This ticket makes\n\n```\nsage -t --long --random-seed=n src/doc/\n```\npass for different values `n` than just `0`.\n\nDepends on #29962\n\nBranch/Commit: **[0e062d5](https://github.com/sagemath/sagetrac-mirror/commit/0e062d5987166d71ce925d19aba21026fa9191ab)**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Jonathan Kliem**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29981_\n\n",
    "closed_at": "2021-06-19T20:58:42Z",
    "created_at": "2020-06-24T21:48:06Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20doctest%20framework",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make documentation doctests ready for random seeds",
    "type": "issue",
    "updated_at": "2021-06-19T20:58:42Z",
    "url": "https://github.com/sagemath/sage/issues/29981",
    "user": "https://github.com/kliem"
}
```
This ticket makes

```
sage -t --long --random-seed=n src/doc/
```
pass for different values `n` than just `0`.

Depends on #29962

Branch/Commit: **[0e062d5](https://github.com/sagemath/sagetrac-mirror/commit/0e062d5987166d71ce925d19aba21026fa9191ab)**

Reviewer: **Dima Pasechnik**

Author: **Jonathan Kliem**

_Issue created by migration from https://trac.sagemath.org/ticket/29981_





---

archive/issue_events_394556.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-06-24T21:48:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29981#event-394556"
}
```



---

archive/issue_events_394557.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-06-24T21:48:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20doctest%20framework",
    "label_color": "000080",
    "label_name": "component: doctest framework",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29981#event-394557"
}
```



---

archive/issue_events_394558.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-06-24T21:48:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29981#event-394558"
}
```



---

archive/issue_events_394559.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-06-24T21:48:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29981#event-394559"
}
```



---

archive/issue_comments_477453.json:
```json
{
    "body": "Author: **Jonathan Kliem**",
    "created_at": "2020-06-24T21:56:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29981#issuecomment-477453",
    "user": "https://github.com/kliem"
}
```

Author: **Jonathan Kliem**



---

archive/issue_comments_477454.json:
```json
{
    "body": "Branch: **[public/29981](https://github.com/sagemath/sagetrac-mirror/tree/public/29981)**",
    "created_at": "2020-06-24T21:56:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29981#issuecomment-477454",
    "user": "https://github.com/kliem"
}
```

Branch: **[public/29981](https://github.com/sagemath/sagetrac-mirror/tree/public/29981)**



---

archive/issue_comments_477455.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/da1c6beec7d0bb6b972d88ecf9fb4eccbdf285a5\">da1c6be</a></td><td><code>start from a \"random\" random seed for doctesting</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b7b836d789e034433d62f331f33fbc1821c0deaa\">b7b836d</a></td><td><code>make random seed reproducible</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/eedbe5ebea9c2b9da79404013c566fe17d0a3e20\">eedbe5e</a></td><td><code>document random_seed</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/998b1b94ce1289ea92451a86e5f6191c37eaeb5a\">998b1b9</a></td><td><code>default random seed 0 for now</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1d7b00e3fc2ebc1dc9982a2df91d15e3f12e9432\">1d7b00e</a></td><td><code>dash instead of underscore for command line options</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f1066b4c06830c25a78c5742a497bd8a6987dfba\">f1066b4</a></td><td><code>make documentation fuzz ready</code></td></tr></table>\n",
    "created_at": "2020-06-24T21:56:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29981#issuecomment-477455",
    "user": "https://github.com/kliem"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/da1c6beec7d0bb6b972d88ecf9fb4eccbdf285a5">da1c6be</a></td><td><code>start from a "random" random seed for doctesting</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b7b836d789e034433d62f331f33fbc1821c0deaa">b7b836d</a></td><td><code>make random seed reproducible</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/eedbe5ebea9c2b9da79404013c566fe17d0a3e20">eedbe5e</a></td><td><code>document random_seed</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/998b1b94ce1289ea92451a86e5f6191c37eaeb5a">998b1b9</a></td><td><code>default random seed 0 for now</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1d7b00e3fc2ebc1dc9982a2df91d15e3f12e9432">1d7b00e</a></td><td><code>dash instead of underscore for command line options</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f1066b4c06830c25a78c5742a497bd8a6987dfba">f1066b4</a></td><td><code>make documentation fuzz ready</code></td></tr></table>




---

archive/issue_events_394560.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-06-24T21:56:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29981#event-394560"
}
```



---

archive/issue_comments_477456.json:
```json
{
    "body": "Commit: **[f1066b4](https://github.com/sagemath/sagetrac-mirror/commit/f1066b4c06830c25a78c5742a497bd8a6987dfba)**",
    "created_at": "2020-06-24T21:56:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29981#issuecomment-477456",
    "user": "https://github.com/kliem"
}
```

Commit: **[f1066b4](https://github.com/sagemath/sagetrac-mirror/commit/f1066b4c06830c25a78c5742a497bd8a6987dfba)**



---

archive/issue_comments_477457.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/90cb2f3a1cd63ad7205e60181c6f9af27cbe93b6\">90cb2f3</a></td><td><code>Merge branch 'develop' of git://trac.sagemath.org/sage into public/29981</code></td></tr></table>\n",
    "created_at": "2020-06-29T12:05:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29981#issuecomment-477457",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/90cb2f3a1cd63ad7205e60181c6f9af27cbe93b6">90cb2f3</a></td><td><code>Merge branch 'develop' of git://trac.sagemath.org/sage into public/29981</code></td></tr></table>




---

archive/issue_comments_477458.json:
```json
{
    "body": "Changed commit from **[f1066b4](https://github.com/sagemath/sagetrac-mirror/commit/f1066b4c06830c25a78c5742a497bd8a6987dfba)** to **[90cb2f3](https://github.com/sagemath/sagetrac-mirror/commit/90cb2f3a1cd63ad7205e60181c6f9af27cbe93b6)**",
    "created_at": "2020-06-29T12:05:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29981#issuecomment-477458",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[f1066b4](https://github.com/sagemath/sagetrac-mirror/commit/f1066b4c06830c25a78c5742a497bd8a6987dfba)** to **[90cb2f3](https://github.com/sagemath/sagetrac-mirror/commit/90cb2f3a1cd63ad7205e60181c6f9af27cbe93b6)**



---

archive/issue_comments_477459.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ac45e07c18462ec9e11b32c35387da3229700479\">ac45e07</a></td><td><code>random event is valid now</code></td></tr></table>\n",
    "created_at": "2020-06-29T12:53:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29981#issuecomment-477459",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ac45e07c18462ec9e11b32c35387da3229700479">ac45e07</a></td><td><code>random event is valid now</code></td></tr></table>




---

archive/issue_comments_477460.json:
```json
{
    "body": "Changed commit from **[90cb2f3](https://github.com/sagemath/sagetrac-mirror/commit/90cb2f3a1cd63ad7205e60181c6f9af27cbe93b6)** to **[ac45e07](https://github.com/sagemath/sagetrac-mirror/commit/ac45e07c18462ec9e11b32c35387da3229700479)**",
    "created_at": "2020-06-29T12:53:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29981#issuecomment-477460",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[90cb2f3](https://github.com/sagemath/sagetrac-mirror/commit/90cb2f3a1cd63ad7205e60181c6f9af27cbe93b6)** to **[ac45e07](https://github.com/sagemath/sagetrac-mirror/commit/ac45e07c18462ec9e11b32c35387da3229700479)**



---

archive/issue_comments_477461.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nMerge conflict.\n\nI also need to go through it again and check I respected the design decision in #29935.",
    "created_at": "2020-07-12T18:16:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29981#issuecomment-477461",
    "user": "https://github.com/kliem"
}
```

<div id="comment:4" align="right">Comment 4</div>

Merge conflict.

I also need to go through it again and check I respected the design decision in #29935.



---

archive/issue_events_394561.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-07-12T18:16:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29981#event-394561"
}
```



---

archive/issue_events_394562.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-07-12T18:16:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29981#event-394562"
}
```



---

archive/issue_events_394563.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-05T19:21:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29981#event-394563"
}
```



---

archive/issue_events_394564.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-05T19:21:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29981#event-394564"
}
```



---

archive/issue_events_394565.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29981#event-394565"
}
```



---

archive/issue_events_394566.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29981#event-394566"
}
```



---

archive/issue_comments_477462.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nSetting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29981#issuecomment-477462",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:6" align="right">Comment 6</div>

Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_comments_477463.json:
```json
{
    "body": "Changed commit from **[ac45e07](https://github.com/sagemath/sagetrac-mirror/commit/ac45e07c18462ec9e11b32c35387da3229700479)** to **[0e062d5](https://github.com/sagemath/sagetrac-mirror/commit/0e062d5987166d71ce925d19aba21026fa9191ab)**",
    "created_at": "2021-06-01T09:57:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29981#issuecomment-477463",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[ac45e07](https://github.com/sagemath/sagetrac-mirror/commit/ac45e07c18462ec9e11b32c35387da3229700479)** to **[0e062d5](https://github.com/sagemath/sagetrac-mirror/commit/0e062d5987166d71ce925d19aba21026fa9191ab)**



---

archive/issue_comments_477464.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0e062d5987166d71ce925d19aba21026fa9191ab\">0e062d5</a></td><td><code>make documentation ready for random seeds</code></td></tr></table>\n",
    "created_at": "2021-06-01T09:57:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29981#issuecomment-477464",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:7" align="right">Comment 7</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0e062d5987166d71ce925d19aba21026fa9191ab">0e062d5</a></td><td><code>make documentation ready for random seeds</code></td></tr></table>




---

archive/issue_events_394567.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2021-06-01T09:57:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29981#event-394567"
}
```



---

archive/issue_events_394568.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2021-06-01T09:57:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29981#event-394568"
}
```



---

archive/issue_comments_477465.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2021-06-03T17:26:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29981#issuecomment-477465",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_comments_477466.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nlgtm",
    "created_at": "2021-06-03T17:26:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29981#issuecomment-477466",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:9" align="right">Comment 9</div>

lgtm



---

archive/issue_events_394569.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-06-03T17:26:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29981#event-394569"
}
```



---

archive/issue_events_394570.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-06-03T17:26:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29981#event-394570"
}
```



---

archive/issue_comments_477467.json:
```json
{
    "body": "Changed branch from **[public/29981](https://github.com/sagemath/sagetrac-mirror/tree/public/29981)** to **[0e062d5](https://github.com/sagemath/sagetrac-mirror/commit/0e062d5987166d71ce925d19aba21026fa9191ab)**",
    "created_at": "2021-06-19T20:58:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29981#issuecomment-477467",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/29981](https://github.com/sagemath/sagetrac-mirror/tree/public/29981)** to **[0e062d5](https://github.com/sagemath/sagetrac-mirror/commit/0e062d5987166d71ce925d19aba21026fa9191ab)**



---

archive/issue_events_394571.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-06-19T20:58:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29981#event-394571"
}
```



---

archive/issue_events_394572.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "5f4b61407246ccfe47c4cebeffe48e008ecc4123",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-06-19T20:58:42Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29981",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29981#event-394572"
}
```
