...
<a id='comment:5'></a>Thank you so much for you input.
...
<a id='comment:7'></a>Replying to [dcoudert](#comment%3A3):
> A few comments:
> - please document your methods
> - use standard notation `n` instead of `V`.
> - you can use `UINT32_MAX`
...
```json
{
    "body": "<a id='comment:13'></a>You must add doctests for your algorithms. Also, benchmarks would be welcome.",
...
<a id='comment:13'></a>You must add doctests for your algorithms. Also, benchmarks would be welcome.
...
<a id='comment:15'></a>I have been experimenting and it seems this method is more efficient when the graph is sparse.
It should happen because the method creates a short_digraph with the reverse edges, something that iFUB does not have to do.
...
```json
{
    "body": "<a id='comment:19'></a>FYI, #29346 also proposes an implementation of the same algorithm. Authors could join forces to get a better code.",
...
<a id='comment:19'></a>FYI, #29346 also proposes an implementation of the same algorithm. Authors could join forces to get a better code.
...
```json
{
    "body": "<a id='comment:21'></a>May be with a proper documentation and an appropriate parameter, the same method could be used for both definitions, no?",
...
<a id='comment:21'></a>May be with a proper documentation and an appropriate parameter, the same method could be used for both definitions, no?
...
```json
{
    "body": "<a id='comment:24'></a>red flag",
