# Issue 29326: Improvements for projection into affine hull

archive/issues_029089.json:
```json
{
    "body": "This ticket renames `affine_hull` of `Polyhedron_base` to `affine_hull_projection` and adds a deprecation warning.\n\nBefore, this function was hard to recognize, as by the name one expected to obtain the affine hull and not the polyhedron projected into it.\n\nAlso this ticket makes use of #28724 to simplify construction: We determine a transformation matrix `A` and a vector `b` and basically return `A*P - b`, where `P` is the polyhedron.\n\nCC:  @jplab @laisrast\n\nKeywords: polyhedra, affine hull\n\nBranch/Commit: 32e59c0ca60078ff7ed469ddc32686a1bb932542\n\nReviewer: Laith Rastanawi\n\nAuthor: Jonathan Kliem\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/29326\n\n",
    "closed_at": "2020-04-05T08:30:41Z",
    "created_at": "2020-03-13T16:36:11Z",
    "labels": [
        "component: geometry"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.1",
    "title": "Improvements for projection into affine hull",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/29326",
    "user": "https://github.com/kliem"
}
```
This ticket renames `affine_hull` of `Polyhedron_base` to `affine_hull_projection` and adds a deprecation warning.

Before, this function was hard to recognize, as by the name one expected to obtain the affine hull and not the polyhedron projected into it.

Also this ticket makes use of #28724 to simplify construction: We determine a transformation matrix `A` and a vector `b` and basically return `A*P - b`, where `P` is the polyhedron.

CC:  @jplab @laisrast

Keywords: polyhedra, affine hull

Branch/Commit: 32e59c0ca60078ff7ed469ddc32686a1bb932542

Reviewer: Laith Rastanawi

Author: Jonathan Kliem

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/29326





---

archive/issue_comments_572021.json:
```json
{
    "body": "Changing branch from \"\" to \"public/29326\"",
    "created_at": "2020-03-13T17:04:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29326",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29326#issuecomment-572021",
    "user": "https://github.com/kliem"
}
```

Changing branch from "" to "public/29326"



---

archive/issue_comments_572022.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2020-03-13T17:04:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29326",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29326#issuecomment-572022",
    "user": "https://github.com/kliem"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_572023.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,5 +1,5 @@\n-This ticket renames `affine_hull` of `Polyhedron_base` to `in_affine_hull` and adds a deprecation warning.\n+This ticket renames `affine_hull` of `Polyhedron_base` to `affine_hull_projection` and adds a deprecation warning.\n \n-Before, this function was hard to find, as an affine hull of a polyhedron usually defined to be the smallest affine subspace containing the polyhedron.\n+Before, this function was hard to recognize, as by the name one expected to obtain the affine hull and not the polyhedron projected into it.\n \n-Also this ticket makes use of #28724 to simplify this construction: We determine a transformation matrix `A` and a vector `b` and basically return `A*P - b`, where `P` is the polyhedron.\n+Also this ticket makes use of #28724 to simplify construction: We determine a transformation matrix `A` and a vector `b` and basically return `A*P - b`, where `P` is the polyhedron.\n``````\n",
    "created_at": "2020-03-13T17:04:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29326",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29326#issuecomment-572023",
    "user": "https://github.com/kliem"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,5 +1,5 @@
-This ticket renames `affine_hull` of `Polyhedron_base` to `in_affine_hull` and adds a deprecation warning.
+This ticket renames `affine_hull` of `Polyhedron_base` to `affine_hull_projection` and adds a deprecation warning.
 
-Before, this function was hard to find, as an affine hull of a polyhedron usually defined to be the smallest affine subspace containing the polyhedron.
+Before, this function was hard to recognize, as by the name one expected to obtain the affine hull and not the polyhedron projected into it.
 
-Also this ticket makes use of #28724 to simplify this construction: We determine a transformation matrix `A` and a vector `b` and basically return `A*P - b`, where `P` is the polyhedron.
+Also this ticket makes use of #28724 to simplify construction: We determine a transformation matrix `A` and a vector `b` and basically return `A*P - b`, where `P` is the polyhedron.
``````




---

archive/issue_comments_572024.json:
```json
{
    "body": "<a id='comment:1'></a>New commits:",
    "created_at": "2020-03-13T17:04:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29326",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29326#issuecomment-572024",
    "user": "https://github.com/kliem"
}
```

<a id='comment:1'></a>New commits:



---

archive/issue_comments_572025.json:
```json
{
    "body": "Changing commit from \"\" to \"6d12f79f482bf942d4602e7d8cb0bae896b0c7c4\"",
    "created_at": "2020-03-13T17:04:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29326",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29326#issuecomment-572025",
    "user": "https://github.com/kliem"
}
```

Changing commit from "" to "6d12f79f482bf942d4602e7d8cb0bae896b0c7c4"



---

archive/issue_comments_572026.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Laith Rastanawi\"",
    "created_at": "2020-03-27T17:08:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29326",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29326#issuecomment-572026",
    "user": "https://github.com/LaisRast"
}
```

Changing reviewer from "" to "Laith Rastanawi"



---

archive/issue_comments_572027.json:
```json
{
    "body": "<a id='comment:2'></a>* In `polyhedra_quickref.rst`:\n\n```diff\n-    :meth:`~sage.geometry.polyhedron.base.Polyhedron_base.affine_hull_projection` | constructs an affinely equivalent full dimensional polyhedra\n+    :meth:`~sage.geometry.polyhedron.base.Polyhedron_base.affine_hull_projection` | constructs an affinely equivalent full-dimensional polyhedron\n```\n\n* Make the second paragraph in the docstring look like a one paragraph.\n* In the \"OUTPUT\" section:\n\n```diff\n-        A full-dimensional polyhedron or a linear transformation,\n+        A full-dimensional polyhedron or an affine transformation,\n        depending on the parameter ``as_affine_map``.\n```\n* Remove the following line from \"TODO\" section:\n\n```diff\n-            - allow to return ``as_affine_map=True`` for default setting\n```\n* The new name `affine_hull_projection` is definitively better than the old name, but I am not sure if this is the best name. How about `full_dimensional_copy`? I am also not sure about this suggestion. So maybe we need another opinion.",
    "created_at": "2020-03-27T17:08:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29326",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29326#issuecomment-572027",
    "user": "https://github.com/LaisRast"
}
```

<a id='comment:2'></a>* In `polyhedra_quickref.rst`:

```diff
-    :meth:`~sage.geometry.polyhedron.base.Polyhedron_base.affine_hull_projection` | constructs an affinely equivalent full dimensional polyhedra
+    :meth:`~sage.geometry.polyhedron.base.Polyhedron_base.affine_hull_projection` | constructs an affinely equivalent full-dimensional polyhedron
```

* Make the second paragraph in the docstring look like a one paragraph.
* In the "OUTPUT" section:

```diff
-        A full-dimensional polyhedron or a linear transformation,
+        A full-dimensional polyhedron or an affine transformation,
        depending on the parameter ``as_affine_map``.
```
* Remove the following line from "TODO" section:

```diff
-            - allow to return ``as_affine_map=True`` for default setting
```
* The new name `affine_hull_projection` is definitively better than the old name, but I am not sure if this is the best name. How about `full_dimensional_copy`? I am also not sure about this suggestion. So maybe we need another opinion.



---

archive/issue_comments_572028.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2020-03-27T17:08:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29326",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29326#issuecomment-572028",
    "user": "https://github.com/LaisRast"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_572029.json:
```json
{
    "body": "<a id='comment:3'></a>The name has been discussed in https://groups.google.com/forum/#!topic/sage-devel/MEmAIPDPcHY\nIf you want to propose a different name, please do so there, so that everyone is involved.",
    "created_at": "2020-03-27T17:57:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29326",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29326#issuecomment-572029",
    "user": "https://github.com/kliem"
}
```

<a id='comment:3'></a>The name has been discussed in https://groups.google.com/forum/#!topic/sage-devel/MEmAIPDPcHY
If you want to propose a different name, please do so there, so that everyone is involved.



---

archive/issue_comments_572030.json:
```json
{
    "body": "<a id='comment:4'></a>Replying to [gh-kliem](#comment%3A3):\n> The name has been discussed in https://groups.google.com/forum/#!topic/sage-devel/MEmAIPDPcHY\n> If you want to propose a different name, please do so there, so that everyone is involved.\n\nI just wanted to hear some other opinions, and since many people already agreed for the name, I am also fine with it.",
    "created_at": "2020-03-27T18:14:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29326",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29326#issuecomment-572030",
    "user": "https://github.com/LaisRast"
}
```

<a id='comment:4'></a>Replying to [gh-kliem](#comment%3A3):
> The name has been discussed in https://groups.google.com/forum/#!topic/sage-devel/MEmAIPDPcHY
> If you want to propose a different name, please do so there, so that everyone is involved.

I just wanted to hear some other opinions, and since many people already agreed for the name, I am also fine with it.



---

archive/issue_comments_572031.json:
```json
{
    "body": "<a id='comment:5'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2020-03-27T18:33:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29326",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29326#issuecomment-572031",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:5'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_572032.json:
```json
{
    "body": "Changing commit from \"6d12f79f482bf942d4602e7d8cb0bae896b0c7c4\" to \"32e59c0ca60078ff7ed469ddc32686a1bb932542\"",
    "created_at": "2020-03-27T18:33:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29326",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29326#issuecomment-572032",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "6d12f79f482bf942d4602e7d8cb0bae896b0c7c4" to "32e59c0ca60078ff7ed469ddc32686a1bb932542"



---

archive/issue_comments_572033.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2020-03-27T19:09:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29326",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29326#issuecomment-572033",
    "user": "https://github.com/kliem"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_572034.json:
```json
{
    "body": "<a id='comment:7'></a>It is good to go.",
    "created_at": "2020-03-27T20:53:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29326",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29326#issuecomment-572034",
    "user": "https://github.com/LaisRast"
}
```

<a id='comment:7'></a>It is good to go.



---

archive/issue_comments_572035.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2020-03-27T20:53:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29326",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29326#issuecomment-572035",
    "user": "https://github.com/LaisRast"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_events_074659.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-04-05T08:30:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/29326",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29326#event-74659"
}
```



---

archive/issue_comments_572036.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2020-04-05T08:30:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29326",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29326#issuecomment-572036",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_comments_572037.json:
```json
{
    "body": "Changing branch from \"public/29326\" to \"32e59c0ca60078ff7ed469ddc32686a1bb932542\"",
    "created_at": "2020-04-05T08:30:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29326",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29326#issuecomment-572037",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "public/29326" to "32e59c0ca60078ff7ed469ddc32686a1bb932542"
