# Issue 29404: Fix build failures of pynac on macos with system python3

archive/issues_029167.json:
```json
{
    "assignees": [],
    "body": "https://github.com/mkoeppe/sage/runs/531095486\n\npynac:\n\n```\nchecking whether sage-python23 version is >= 2.7... yes\nchecking for sage-python23 version... 3.7\nchecking for sage-python23 platform... darwin\nchecking for sage-python23 script directory... ${prefix}/lib/python3.7/site-packages\nchecking for sage-python23 extension module directory... ${exec_prefix}/lib/python3.7/site-packages\nchecking for style of include used by make... GNU\nchecking for gcc... gcc\nchecking whether the C compiler works... yes\nchecking for C compiler default output file name... a.out\nchecking for suffix of executables... \nchecking whether we are cross compiling... no\nchecking for suffix of object files... o\nchecking whether we are using the GNU C compiler... yes\nchecking whether gcc accepts -g... yes\nchecking for gcc option to accept ISO C89... none needed\nchecking whether gcc understands -c and -o together... yes\nchecking dependency style of gcc... none\nchecking for python3.7... no\nconfigure: error: Cannot find python3.7 in your system path\n```\n\nFix in https://github.com/pynac/pynac/pull/351, https://github.com/mkoeppe/pynac/releases/tag/pynac-0.7.26.sage-2020-03-28\n\nThe \"temporary upstream\" tarball URL is in `checksums.ini`. To test the update on this branch: Use `make SAGE_SPKG=\"sage-spkg -o\" pynac-clean pynac`; this will download the new upstream package automatically.\n\n\n\n\nCC:  @dimpase @kiwifb @videlec @saraedum @isuruf @embray\n\nBranch: **[`7344e2d`](https://github.com/sagemath/sagetrac-mirror/commit/7344e2df8bebecdab19924a14ff4dbe51cc514f9)**\n\nUpstream: **Completely fixed; Fix reported upstream**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Matthias Koeppe**\n\nComponent: **packages: standard**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29404_\n\n",
    "closed_at": "2020-04-16T22:33:49Z",
    "created_at": "2020-03-25T02:58:16Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix build failures of pynac on macos with system python3",
    "type": "issue",
    "updated_at": "2020-06-16T11:41:30Z",
    "url": "https://github.com/sagemath/sage/issues/29404",
    "user": "https://github.com/mkoeppe"
}
```
https://github.com/mkoeppe/sage/runs/531095486

pynac:

```
checking whether sage-python23 version is >= 2.7... yes
checking for sage-python23 version... 3.7
checking for sage-python23 platform... darwin
checking for sage-python23 script directory... ${prefix}/lib/python3.7/site-packages
checking for sage-python23 extension module directory... ${exec_prefix}/lib/python3.7/site-packages
checking for style of include used by make... GNU
checking for gcc... gcc
checking whether the C compiler works... yes
checking for C compiler default output file name... a.out
checking for suffix of executables... 
checking whether we are cross compiling... no
checking for suffix of object files... o
checking whether we are using the GNU C compiler... yes
checking whether gcc accepts -g... yes
checking for gcc option to accept ISO C89... none needed
checking whether gcc understands -c and -o together... yes
checking dependency style of gcc... none
checking for python3.7... no
configure: error: Cannot find python3.7 in your system path
```

Fix in https://github.com/pynac/pynac/pull/351, https://github.com/mkoeppe/pynac/releases/tag/pynac-0.7.26.sage-2020-03-28

The "temporary upstream" tarball URL is in `checksums.ini`. To test the update on this branch: Use `make SAGE_SPKG="sage-spkg -o" pynac-clean pynac`; this will download the new upstream package automatically.




CC:  @dimpase @kiwifb @videlec @saraedum @isuruf @embray

Branch: **[`7344e2d`](https://github.com/sagemath/sagetrac-mirror/commit/7344e2df8bebecdab19924a14ff4dbe51cc514f9)**

Upstream: **Completely fixed; Fix reported upstream**

Reviewer: **Dima Pasechnik**

Author: **Matthias Koeppe**

Component: **packages: standard**

_Issue created by migration from https://trac.sagemath.org/ticket/29404_





---

archive/issue_events_406330.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-25T02:58:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406330"
}
```



---

archive/issue_events_406331.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-25T02:58:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406331"
}
```



---

archive/issue_events_406332.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-25T02:58:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406332"
}
```



---

archive/issue_events_406333.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-25T02:58:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406333"
}
```



---

archive/issue_comments_461567.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\npynac's python discovery should be fixed, this looks completely broken",
    "created_at": "2020-03-25T03:27:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461567",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:1" align="right">comment:1</div>

pynac's python discovery should be fixed, this looks completely broken



---

archive/issue_events_406334.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-25T03:31:07Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "title_is": "Fix build failures of pplpy and pynac on macos with system python3",
    "title_was": "Fix build failures of pplpy on macos with system python3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406334"
}
```



---

archive/issue_comments_461568.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nThis should be probably split into two tickets.\n\npplpy upstream is https://gitlab.com/videlec/pplpy - also in CC:",
    "created_at": "2020-03-25T03:48:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461568",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:3" align="right">comment:3</div>

This should be probably split into two tickets.

pplpy upstream is https://gitlab.com/videlec/pplpy - also in CC:



---

archive/issue_comments_461569.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nYes for the split. I am surprised that problem shows now in pynac. I would have expected it to happen much earlier. The AX_PYTHON_DEVEL macro is 5 years old - which probably doesn't help.",
    "created_at": "2020-03-25T03:51:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461569",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:4" align="right">comment:4</div>

Yes for the split. I am surprised that problem shows now in pynac. I would have expected it to happen much earlier. The AX_PYTHON_DEVEL macro is 5 years old - which probably doesn't help.



---

archive/issue_comments_461570.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nNote that I don't think the problem is in the macro, but something in there may trigger. I suspect the problem is in `AM_PATH_PYTHON` and that `PYTHON` needs to be defined when calling configure in `spkg-install`.",
    "created_at": "2020-03-25T03:54:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461570",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:5" align="right">comment:5</div>

Note that I don't think the problem is in the macro, but something in there may trigger. I suspect the problem is in `AM_PATH_PYTHON` and that `PYTHON` needs to be defined when calling configure in `spkg-install`.



---

archive/issue_comments_461571.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nWhich is already done. So I am not sure what happens to override it. Access to config.log would be helpful.",
    "created_at": "2020-03-25T03:58:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461571",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:6" align="right">comment:6</div>

Which is already done. So I am not sure what happens to override it. Access to config.log would be helpful.



---

archive/issue_comments_461572.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nYes, please go ahead with splitting the ticket.",
    "created_at": "2020-03-25T04:02:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461572",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:7" align="right">comment:7</div>

Yes, please go ahead with splitting the ticket.



---

archive/issue_comments_461573.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nIt looks from the log as here could be some mixing up happening between system python3 and Homebrew's python3 (unless Homebrew's python3 is using system's python3 headers by design).",
    "created_at": "2020-03-25T06:22:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461573",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:8" align="right">comment:8</div>

It looks from the log as here could be some mixing up happening between system python3 and Homebrew's python3 (unless Homebrew's python3 is using system's python3 headers by design).



---

archive/issue_comments_461574.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nReplying to [@dimpase](#comment:8):\n> It looks from the log as here could be some mixing up happening between system python3 and Homebrew's python3 (unless Homebrew's python3 is using system's python3 headers by design).\n\nShouldn't happen. We pass `PYTHON=sage-python23` explicitly to configure. That's what should be tested.\nhttps://github.com/sagemath/sage/blob/develop/build/pkgs/pynac/spkg-install.in#L14\n\nMy gut feeling is that something funny happened to that line during processing. But I need config.log to establish it.",
    "created_at": "2020-03-25T06:44:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461574",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:9" align="right">comment:9</div>

Replying to [@dimpase](#comment:8):
> It looks from the log as here could be some mixing up happening between system python3 and Homebrew's python3 (unless Homebrew's python3 is using system's python3 headers by design).

Shouldn't happen. We pass `PYTHON=sage-python23` explicitly to configure. That's what should be tested.
https://github.com/sagemath/sage/blob/develop/build/pkgs/pynac/spkg-install.in#L14

My gut feeling is that something funny happened to that line during processing. But I need config.log to establish it.



---

archive/issue_comments_461575.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nI don't have MacOS machine, but anyone with it may install Homebrew and try.",
    "created_at": "2020-03-25T07:05:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461575",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:10" align="right">comment:10</div>

I don't have MacOS machine, but anyone with it may install Homebrew and try.



---

archive/issue_comments_461576.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">comment:11</div>\n\nIt is more subtle than my implied gut feeling. The macro AX_PYTHON_DEVEL gets the right value for PYTHON and does its stuff based on that. I think `AM_PATH_PYTHON` doesn't like python with exotic names because it looks like it goes through a pre-defined list.",
    "created_at": "2020-03-25T07:23:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461576",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:11" align="right">comment:11</div>

It is more subtle than my implied gut feeling. The macro AX_PYTHON_DEVEL gets the right value for PYTHON and does its stuff based on that. I think `AM_PATH_PYTHON` doesn't like python with exotic names because it looks like it goes through a pre-defined list.



---

archive/issue_comments_461577.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">comment:12</div>\n\nisn't the location of Sage's Python exotic enough to choke it, if it were the case?",
    "created_at": "2020-03-25T07:27:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461577",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:12" align="right">comment:12</div>

isn't the location of Sage's Python exotic enough to choke it, if it were the case?



---

archive/issue_comments_461578.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">comment:13</div>\n\nIt would be :) \n\nCode inspection says I have it backwards. `AM_PATH_PYTHON` works and `AX_PYTHON_DEVEL` is the one failing.",
    "created_at": "2020-03-25T07:43:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461578",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:13" align="right">comment:13</div>

It would be :) 

Code inspection says I have it backwards. `AM_PATH_PYTHON` works and `AX_PYTHON_DEVEL` is the one failing.



---

archive/issue_comments_461579.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">comment:14</div>\n\nToxic. This is with an old sage I have around\n\n```\n[pynac-0.7.14] checking whether sage-python23 version is >= 2.7... yes\n[pynac-0.7.14] checking for sage-python23 version... 2.7\n[pynac-0.7.14] checking for sage-python23 platform... linux2\n[pynac-0.7.14] checking for sage-python23 script directory... ${prefix}/lib/python2.7/site-packages\n[pynac-0.7.14] checking for sage-python23 extension module directory... ${exec_prefix}/lib/python2.7/site-packages\n[pynac-0.7.14] checking for style of include used by make... GNU\n[pynac-0.7.14] checking for gcc... gcc\n[pynac-0.7.14] checking whether the C compiler works... yes\n[pynac-0.7.14] checking for C compiler default output file name... a.out\n[pynac-0.7.14] checking for suffix of executables... \n[pynac-0.7.14] checking whether we are cross compiling... no\n[pynac-0.7.14] checking for suffix of object files... o\n[pynac-0.7.14] checking whether we are using the GNU C compiler... yes\n[pynac-0.7.14] checking whether gcc accepts -g... yes\n[pynac-0.7.14] checking for gcc option to accept ISO C89... none needed\n[pynac-0.7.14] checking whether gcc understands -c and -o together... yes\n[pynac-0.7.14] checking dependency style of gcc... gcc3\n[pynac-0.7.14] checking for python2.7... /home/fbissey/sagemath/sage/local/bin/python2.7\n[pynac-0.7.14] checking for a version of Python >= '2.1.0'... yes\n[pynac-0.7.14] checking for the distutils Python package... yes\n[pynac-0.7.14] checking for Python include path... -I/home/fbissey/sagemath/sage/local/include/python2.7\n[pynac-0.7.14] checking for Python library path... -L/home/fbissey/sagemath/sage/local/lib -lpython2.7\n[pynac-0.7.14] checking for Python site-packages path... /home/fbissey/sagemath/sage/local/lib/python2.7/site-packages\n```\nAs you can see, the first test (`AM_PATH_PYTHON`) use `sage-python23` properly. But not the second that goes directly for `pythonX.Y`. Worse, that's by design of the macro.\n\n```\n\tAC_PATH_PROG([PYTHON],[python[$PYTHON_VERSION]])\n\tif test -z \"$PYTHON\"; then\n\t   AC_MSG_ERROR([Cannot find python$PYTHON_VERSION in your system path])\n\t   PYTHON_VERSION=\"\"\n\tfi\n```\nAnd for the cherry on the top of the cake, I believe `PYTHON_VERSION` is set by `AM_PATH_PYTHON`. Conclusion `AX_PYTHON_DEVEL` doesn't do exotic python name, and you better have a regular python executable in the PATH. Now, I'd like to know more about that setup that has `sage-python23` in the PATH but not the matching `pythonX.Y`. Did something happen while I wasn't looking?",
    "created_at": "2020-03-25T08:04:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461579",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:14" align="right">comment:14</div>

Toxic. This is with an old sage I have around

```
[pynac-0.7.14] checking whether sage-python23 version is >= 2.7... yes
[pynac-0.7.14] checking for sage-python23 version... 2.7
[pynac-0.7.14] checking for sage-python23 platform... linux2
[pynac-0.7.14] checking for sage-python23 script directory... ${prefix}/lib/python2.7/site-packages
[pynac-0.7.14] checking for sage-python23 extension module directory... ${exec_prefix}/lib/python2.7/site-packages
[pynac-0.7.14] checking for style of include used by make... GNU
[pynac-0.7.14] checking for gcc... gcc
[pynac-0.7.14] checking whether the C compiler works... yes
[pynac-0.7.14] checking for C compiler default output file name... a.out
[pynac-0.7.14] checking for suffix of executables... 
[pynac-0.7.14] checking whether we are cross compiling... no
[pynac-0.7.14] checking for suffix of object files... o
[pynac-0.7.14] checking whether we are using the GNU C compiler... yes
[pynac-0.7.14] checking whether gcc accepts -g... yes
[pynac-0.7.14] checking for gcc option to accept ISO C89... none needed
[pynac-0.7.14] checking whether gcc understands -c and -o together... yes
[pynac-0.7.14] checking dependency style of gcc... gcc3
[pynac-0.7.14] checking for python2.7... /home/fbissey/sagemath/sage/local/bin/python2.7
[pynac-0.7.14] checking for a version of Python >= '2.1.0'... yes
[pynac-0.7.14] checking for the distutils Python package... yes
[pynac-0.7.14] checking for Python include path... -I/home/fbissey/sagemath/sage/local/include/python2.7
[pynac-0.7.14] checking for Python library path... -L/home/fbissey/sagemath/sage/local/lib -lpython2.7
[pynac-0.7.14] checking for Python site-packages path... /home/fbissey/sagemath/sage/local/lib/python2.7/site-packages
```
As you can see, the first test (`AM_PATH_PYTHON`) use `sage-python23` properly. But not the second that goes directly for `pythonX.Y`. Worse, that's by design of the macro.

```
	AC_PATH_PROG([PYTHON],[python[$PYTHON_VERSION]])
	if test -z "$PYTHON"; then
	   AC_MSG_ERROR([Cannot find python$PYTHON_VERSION in your system path])
	   PYTHON_VERSION=""
	fi
```
And for the cherry on the top of the cake, I believe `PYTHON_VERSION` is set by `AM_PATH_PYTHON`. Conclusion `AX_PYTHON_DEVEL` doesn't do exotic python name, and you better have a regular python executable in the PATH. Now, I'd like to know more about that setup that has `sage-python23` in the PATH but not the matching `pythonX.Y`. Did something happen while I wasn't looking?



---

archive/issue_comments_461580.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">comment:15</div>\n\nUpdating `AX_PYTHON_DEVEL` would still give the same result. There is probably something smarter to do.",
    "created_at": "2020-03-25T08:07:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461580",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:15" align="right">comment:15</div>

Updating `AX_PYTHON_DEVEL` would still give the same result. There is probably something smarter to do.



---

archive/issue_comments_461581.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">comment:16</div>\n\nAll the stuff found by `AX_PYTHON_DEVEL` could be found by snipping bits of it and using the results from `AM_PATH_PYTHON`. This is a substantial rewrite of upstream's `configure.ac` that we are looking at. We only need to lift the bits that figure `PYTHON_CPPFLAGS` and `PYTHON_LDFLAGS`  (`PYTHON_LIBS` if lifted from the latest version of `AX_PYTHON_DEVEL`). The rest is provided by `AM_PATH_PYTHON`.",
    "created_at": "2020-03-25T09:06:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461581",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:16" align="right">comment:16</div>

All the stuff found by `AX_PYTHON_DEVEL` could be found by snipping bits of it and using the results from `AM_PATH_PYTHON`. This is a substantial rewrite of upstream's `configure.ac` that we are looking at. We only need to lift the bits that figure `PYTHON_CPPFLAGS` and `PYTHON_LDFLAGS`  (`PYTHON_LIBS` if lifted from the latest version of `AX_PYTHON_DEVEL`). The rest is provided by `AM_PATH_PYTHON`.



---

archive/issue_comments_461582.json:
```json
{
    "body": "<div id=\"comment:17\" align=\"right\">comment:17</div>\n\nReplying to [@kiwifb](#comment:4):\n> Yes for the split. I am surprised that problem shows now in pynac. I would have expected it to happen much earlier. The AX_PYTHON_DEVEL macro is 5 years old - which probably doesn't help.\n\nIt shows now because this is a test with system python3 (#27824) instead of building sage's own python3. On macOS, there is `/usr/bin/python3` (which is Python 3.7) but no `/usr/bin/python3.7`.",
    "created_at": "2020-03-25T11:53:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461582",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:17" align="right">comment:17</div>

Replying to [@kiwifb](#comment:4):
> Yes for the split. I am surprised that problem shows now in pynac. I would have expected it to happen much earlier. The AX_PYTHON_DEVEL macro is 5 years old - which probably doesn't help.

It shows now because this is a test with system python3 (#27824) instead of building sage's own python3. On macOS, there is `/usr/bin/python3` (which is Python 3.7) but no `/usr/bin/python3.7`.



---

archive/issue_comments_461583.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">comment:18</div>\n\nwould it work with Homebrew's python3 ?\nhttps://formulae.brew.sh/formula/python\n\n\nPython shipped with MacOS is dodgy, IMHO.",
    "created_at": "2020-03-25T14:58:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461583",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:18" align="right">comment:18</div>

would it work with Homebrew's python3 ?
https://formulae.brew.sh/formula/python


Python shipped with MacOS is dodgy, IMHO.



---

archive/issue_comments_461584.json:
```json
{
    "body": "<div id=\"comment:19\" align=\"right\">comment:19</div>\n\nReplying to [@kiwifb](#comment:16):\n> All the stuff found by `AX_PYTHON_DEVEL` could be found by snipping bits of it and using the results from `AM_PATH_PYTHON`. This is a substantial rewrite of upstream's `configure.ac` that we are looking at. We only need to lift the bits that figure `PYTHON_CPPFLAGS` and `PYTHON_LDFLAGS`  (`PYTHON_LIBS` if lifted from the latest version of `AX_PYTHON_DEVEL`). The rest is provided by `AM_PATH_PYTHON`.\n\nYes, this sounds like the correct approach.",
    "created_at": "2020-03-25T17:29:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461584",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:19" align="right">comment:19</div>

Replying to [@kiwifb](#comment:16):
> All the stuff found by `AX_PYTHON_DEVEL` could be found by snipping bits of it and using the results from `AM_PATH_PYTHON`. This is a substantial rewrite of upstream's `configure.ac` that we are looking at. We only need to lift the bits that figure `PYTHON_CPPFLAGS` and `PYTHON_LDFLAGS`  (`PYTHON_LIBS` if lifted from the latest version of `AX_PYTHON_DEVEL`). The rest is provided by `AM_PATH_PYTHON`.

Yes, this sounds like the correct approach.



---

archive/issue_comments_461585.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,16 +1,4 @@\n https://github.com/mkoeppe/sage/runs/531095486\n-\n-pplpy:\n-\n-```\n-    g++ -sdk macosx clang -bundle -undefined dynamic_lookup -L/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/local/lib -Wl,-rpath,/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/local/lib -L/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/homebrew/opt/readline/lib -L/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/homebrew/lib -I/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/homebrew/opt/readline/include -I/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/homebrew/include build/temp.macosx-10.14-x86_64-3.7/ppl/linear_algebra.o build/temp.macosx-10.14-x86_64-3.7/ppl/ppl_shim.o -lgmp -lgmpxx -lppl -lm -o build/lib.macosx-10.14-x86_64-3.7/ppl/linear_algebra.cpython-37m-darwin.so\n-    clang: error: unknown argument: '-sdk'\n-    clang: error: no such file or directory: 'macosx'\n-    clang: error: no such file or directory: 'clang'\n-    error: command 'g++' failed with exit status 1\n-    Running setup.py install for pplpy: finished with status 'error'\n-Cleaning up...\n-```\n \n pynac:\n \n``````\n",
    "created_at": "2020-03-26T02:11:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461585",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,16 +1,4 @@
 https://github.com/mkoeppe/sage/runs/531095486
-
-pplpy:
-
-```
-    g++ -sdk macosx clang -bundle -undefined dynamic_lookup -L/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/local/lib -Wl,-rpath,/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/local/lib -L/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/homebrew/opt/readline/lib -L/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/homebrew/lib -I/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/homebrew/opt/readline/include -I/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/homebrew/include build/temp.macosx-10.14-x86_64-3.7/ppl/linear_algebra.o build/temp.macosx-10.14-x86_64-3.7/ppl/ppl_shim.o -lgmp -lgmpxx -lppl -lm -o build/lib.macosx-10.14-x86_64-3.7/ppl/linear_algebra.cpython-37m-darwin.so
-    clang: error: unknown argument: '-sdk'
-    clang: error: no such file or directory: 'macosx'
-    clang: error: no such file or directory: 'clang'
-    error: command 'g++' failed with exit status 1
-    Running setup.py install for pplpy: finished with status 'error'
-Cleaning up...
-```
 
 pynac:
 
``````




---

archive/issue_events_406335.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-26T02:11:04Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "title_is": "Fix build failures of pynac on macos with system python3",
    "title_was": "Fix build failures of pplpy and pynac on macos with system python3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406335"
}
```



---

archive/issue_comments_461586.json:
```json
{
    "body": "<div id=\"comment:21\" align=\"right\">comment:21</div>\n\nReplying to [@dimpase](#comment:3):\n> This should be probably split into two tickets.\n> \n> pplpy upstream is https://gitlab.com/videlec/pplpy - also in CC:\n\npplpy is now #29408",
    "created_at": "2020-03-26T02:12:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461586",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:21" align="right">comment:21</div>

Replying to [@dimpase](#comment:3):
> This should be probably split into two tickets.
> 
> pplpy upstream is https://gitlab.com/videlec/pplpy - also in CC:

pplpy is now #29408



---

archive/issue_comments_461587.json:
```json
{
    "body": "<div id=\"comment:22\" align=\"right\">comment:22</div>\n\nSome links for `AX_PYTHON_DEVEL`:\n- current version https://github.com/autoconf-archive/autoconf-archive/blob/master/m4/ax_python_devel.m4\n- some patch for it https://0xacab.org/schleuder/rpm-gpgme/blob/epel7-schleuder/0001-fix-stupid-ax_python_devel.patch\n- replacement using python-config https://trac.macports.org/attachment/ticket/39223/python.m4",
    "created_at": "2020-03-26T02:15:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461587",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:22" align="right">comment:22</div>

Some links for `AX_PYTHON_DEVEL`:
- current version https://github.com/autoconf-archive/autoconf-archive/blob/master/m4/ax_python_devel.m4
- some patch for it https://0xacab.org/schleuder/rpm-gpgme/blob/epel7-schleuder/0001-fix-stupid-ax_python_devel.patch
- replacement using python-config https://trac.macports.org/attachment/ticket/39223/python.m4



---

archive/issue_comments_461588.json:
```json
{
    "body": "<div id=\"comment:23\" align=\"right\">comment:23</div>\n\nprobably we should try `replacement using python-config`- except that the name should not start with `AC_`...",
    "created_at": "2020-03-26T02:17:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461588",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:23" align="right">comment:23</div>

probably we should try `replacement using python-config`- except that the name should not start with `AC_`...



---

archive/issue_comments_461589.json:
```json
{
    "body": "<div id=\"comment:24\" align=\"right\">comment:24</div>\n\n... and except that we do not have `sage-python23-config`",
    "created_at": "2020-03-26T02:53:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461589",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:24" align="right">comment:24</div>

... and except that we do not have `sage-python23-config`



---

archive/issue_comments_461590.json:
```json
{
    "body": "<div id=\"comment:25\" align=\"right\">comment:25</div>\n\nIn the same bag, you could be tempted to use `python-X.Y.pc`, `AM_PATH_PYTHON` identifies the python version correctly. I eliminated it from my solutions yesterday but that may be viable if it is shipped consistently. If not, the `AX_PYTHON_DEVEL` macro just runs python commands to get at sysconfig. It is a fairly valid strategy.",
    "created_at": "2020-03-26T03:10:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461590",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:25" align="right">comment:25</div>

In the same bag, you could be tempted to use `python-X.Y.pc`, `AM_PATH_PYTHON` identifies the python version correctly. I eliminated it from my solutions yesterday but that may be viable if it is shipped consistently. If not, the `AX_PYTHON_DEVEL` macro just runs python commands to get at sysconfig. It is a fairly valid strategy.



---

archive/issue_comments_461591.json:
```json
{
    "body": "<div id=\"comment:26\" align=\"right\">comment:26</div>\n\nNo python pkgconfig on macOS without homebrew",
    "created_at": "2020-03-26T03:12:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461591",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:26" align="right">comment:26</div>

No python pkgconfig on macOS without homebrew



---

archive/issue_comments_461592.json:
```json
{
    "body": "<div id=\"comment:27\" align=\"right\">comment:27</div>\n\nKeep forgetting about that.",
    "created_at": "2020-03-26T03:14:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461592",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:27" align="right">comment:27</div>

Keep forgetting about that.



---

archive/issue_comments_461593.json:
```json
{
    "body": "<div id=\"comment:28\" align=\"right\">comment:28</div>\n\nAnd on my mac I have python3  but not python-config for it, just python 2.7. So Mac support (without homebrew) needs to probe the python interpreter directly - at the time of writing.",
    "created_at": "2020-03-26T03:21:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461593",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:28" align="right">comment:28</div>

And on my mac I have python3  but not python-config for it, just python 2.7. So Mac support (without homebrew) needs to probe the python interpreter directly - at the time of writing.



---

archive/issue_comments_461594.json:
```json
{
    "body": "<div id=\"comment:29\" align=\"right\">comment:29</div>\n\nlet us try with Homebrew Python first. One can always add moar insanity later",
    "created_at": "2020-03-26T03:28:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461594",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:29" align="right">comment:29</div>

let us try with Homebrew Python first. One can always add moar insanity later



---

archive/issue_comments_461595.json:
```json
{
    "body": "<div id=\"comment:30\" align=\"right\">comment:30</div>\n\nThat's the path of less resistance but I consider `AX_PYTHON_DEVEL` broken (including the latest one) and in need of replacement.",
    "created_at": "2020-03-26T03:35:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461595",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:30" align="right">comment:30</div>

That's the path of less resistance but I consider `AX_PYTHON_DEVEL` broken (including the latest one) and in need of replacement.



---

archive/issue_comments_461596.json:
```json
{
    "body": "<div id=\"comment:31\" align=\"right\">comment:31</div>\n\nReplying to [@dimpase](#comment:29):\n> let us try with Homebrew Python first. One can always add moar insanity later\n\nNothing try here. We already know precisely how it fails and what needs fixing: the use of that macro.",
    "created_at": "2020-03-26T03:41:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461596",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:31" align="right">comment:31</div>

Replying to [@dimpase](#comment:29):
> let us try with Homebrew Python first. One can always add moar insanity later

Nothing try here. We already know precisely how it fails and what needs fixing: the use of that macro.



---

archive/issue_comments_461597.json:
```json
{
    "body": "<div id=\"comment:33\" align=\"right\">comment:33</div>\n\nI have a first version of a patch for pynac at https://github.com/pynac/pynac/pull/351",
    "created_at": "2020-03-28T04:58:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461597",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:33" align="right">comment:33</div>

I have a first version of a patch for pynac at https://github.com/pynac/pynac/pull/351



---

archive/issue_comments_461598.json:
```json
{
    "body": "Branch: **[u/mkoeppe/fix_build_failures_of_pynac_on_macos_with_system_python3](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/fix_build_failures_of_pynac_on_macos_with_system_python3)**",
    "created_at": "2020-03-28T15:10:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461598",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/fix_build_failures_of_pynac_on_macos_with_system_python3](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/fix_build_failures_of_pynac_on_macos_with_system_python3)**



---

archive/issue_comments_461599.json:
```json
{
    "body": "<div id=\"comment:35\"></div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c97807a2131a7c3d00db3451c18b1bfdfdbe95cd\"><code>c97807a</code></a></td><td><code>build/pkgs/pynac: Use 0.7.26.sage-2020-03-28</code></td></tr></table>\n",
    "created_at": "2020-03-28T15:12:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461599",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:35"></div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c97807a2131a7c3d00db3451c18b1bfdfdbe95cd"><code>c97807a</code></a></td><td><code>build/pkgs/pynac: Use 0.7.26.sage-2020-03-28</code></td></tr></table>




---

archive/issue_comments_461600.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2020-03-28T15:12:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461600",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_comments_461601.json:
```json
{
    "body": "Commit: **[`c97807a`](https://github.com/sagemath/sagetrac-mirror/commit/c97807a2131a7c3d00db3451c18b1bfdfdbe95cd)**",
    "created_at": "2020-03-28T15:12:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461601",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[`c97807a`](https://github.com/sagemath/sagetrac-mirror/commit/c97807a2131a7c3d00db3451c18b1bfdfdbe95cd)**



---

archive/issue_comments_461602.json:
```json
{
    "body": "Upstream: **Completely fixed; Fix reported upstream**",
    "created_at": "2020-03-28T15:12:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461602",
    "user": "https://github.com/mkoeppe"
}
```

Upstream: **Completely fixed; Fix reported upstream**



---

archive/issue_events_406336.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-28T15:16:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406336"
}
```



---

archive/issue_comments_461603.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -24,3 +24,9 @@\n configure: error: Cannot find python3.7 in your system path\n ```\n \n+Fix in https://github.com/pynac/pynac/pull/351, https://github.com/mkoeppe/pynac/releases/tag/pynac-0.7.26.sage-2020-03-28\n+\n+The \"temporary upstream\" tarball URL is in `checksums.ini`. To test the update on this branch: Use `make SAGE_SPKG=\"sage-spkg -o\" pynac-clean pynac`; this will download the new upstream package automatically.\n+\n+\n+\n``````\n",
    "created_at": "2020-03-28T15:16:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461603",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -24,3 +24,9 @@
 configure: error: Cannot find python3.7 in your system path
 ```
 
+Fix in https://github.com/pynac/pynac/pull/351, https://github.com/mkoeppe/pynac/releases/tag/pynac-0.7.26.sage-2020-03-28
+
+The "temporary upstream" tarball URL is in `checksums.ini`. To test the update on this branch: Use `make SAGE_SPKG="sage-spkg -o" pynac-clean pynac`; this will download the new upstream package automatically.
+
+
+
``````




---

archive/issue_events_406337.json:
```json
{
    "actor": "https://github.com/isuruf",
    "created_at": "2020-03-28T15:30:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406337"
}
```



---

archive/issue_events_406338.json:
```json
{
    "actor": "https://github.com/isuruf",
    "created_at": "2020-03-28T15:30:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406338"
}
```



---

archive/issue_comments_461604.json:
```json
{
    "body": "<div id=\"comment:37\" align=\"right\">comment:37</div>\n\nYou shouldn't link to the shared python library. If you do, you'll run into https://github.com/pynac/pynac/issues/337",
    "created_at": "2020-03-28T15:30:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461604",
    "user": "https://github.com/isuruf"
}
```

<div id="comment:37" align="right">comment:37</div>

You shouldn't link to the shared python library. If you do, you'll run into https://github.com/pynac/pynac/issues/337



---

archive/issue_comments_461605.json:
```json
{
    "body": "<div id=\"comment:38\" align=\"right\">comment:38</div>\n\nHelp with this is very welcome! I have to work on something else this weekend",
    "created_at": "2020-03-28T15:53:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461605",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:38" align="right">comment:38</div>

Help with this is very welcome! I have to work on something else this weekend



---

archive/issue_comments_461606.json:
```json
{
    "body": "<div id=\"comment:39\"></div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6f2b3362bf0fe885916f7a233c1bb2cac7100186\"><code>6f2b336</code></a></td><td><code>build/pkgs/pynac: Use 0.7.26.sage-2020-03-28a</code></td></tr></table>\n",
    "created_at": "2020-03-29T00:05:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461606",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:39"></div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6f2b3362bf0fe885916f7a233c1bb2cac7100186"><code>6f2b336</code></a></td><td><code>build/pkgs/pynac: Use 0.7.26.sage-2020-03-28a</code></td></tr></table>




---

archive/issue_comments_461607.json:
```json
{
    "body": "Changed commit from **[`c97807a`](https://github.com/sagemath/sagetrac-mirror/commit/c97807a2131a7c3d00db3451c18b1bfdfdbe95cd)** to **[`6f2b336`](https://github.com/sagemath/sagetrac-mirror/commit/6f2b3362bf0fe885916f7a233c1bb2cac7100186)**",
    "created_at": "2020-03-29T00:05:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461607",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[`c97807a`](https://github.com/sagemath/sagetrac-mirror/commit/c97807a2131a7c3d00db3451c18b1bfdfdbe95cd)** to **[`6f2b336`](https://github.com/sagemath/sagetrac-mirror/commit/6f2b3362bf0fe885916f7a233c1bb2cac7100186)**



---

archive/issue_comments_461608.json:
```json
{
    "body": "<div id=\"comment:40\" align=\"right\">comment:40</div>\n\nFound a moment for this after all",
    "created_at": "2020-03-29T00:05:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461608",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:40" align="right">comment:40</div>

Found a moment for this after all



---

archive/issue_events_406339.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-29T00:33:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406339"
}
```



---

archive/issue_events_406340.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-29T00:33:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406340"
}
```



---

archive/issue_comments_461609.json:
```json
{
    "body": "<div id=\"comment:42\" align=\"right\">comment:42</div>\n\nTests ran at https://github.com/mkoeppe/sage/actions/runs/65556636",
    "created_at": "2020-03-29T13:47:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461609",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:42" align="right">comment:42</div>

Tests ran at https://github.com/mkoeppe/sage/actions/runs/65556636



---

archive/issue_events_406341.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-29T13:51:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406341"
}
```



---

archive/issue_events_406342.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-29T13:51:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/p%3A%202%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p: 2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406342"
}
```



---

archive/issue_comments_461610.json:
```json
{
    "body": "<div id=\"comment:43\" align=\"right\">comment:43</div>\n\nThis seems to work well according to the tests. Needs review!",
    "created_at": "2020-03-29T13:51:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461610",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:43" align="right">comment:43</div>

This seems to work well according to the tests. Needs review!



---

archive/issue_comments_461611.json:
```json
{
    "body": "<div id=\"comment:44\" align=\"right\">comment:44</div>\n\nscipy build in macos-homebrew-standard fails with a similar error:\n\n```\n2020-03-29T03:07:22.1134510Z     g++ -sdk macosx clang -bundle -undefined dynamic_lookup -bundle -undefined dynamic_lookup -L/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/local/lib -Wl,-rpath,/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/local/lib -L/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/homebrew/opt/readline/lib -L/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/homebrew/lib -D__ACCELERATE__ -I/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/homebrew/opt/readline/include -I/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/homebrew/include build/temp.macosx-10.14-x86_64-3.7/scipy/interpolate/src/_interpolate.o -Lbuild/temp.macosx-10.14-x86_64-3.7 -o build/lib.macosx-10.14-x86_64-3.7/scipy/interpolate/_interpolate.cpython-37m-darwin.so\n2020-03-29T03:07:22.1135020Z     clang: error: unknown argument: '-sdk'\n2020-03-29T03:07:22.1135400Z     clang: error: no such file or directory: 'macosx'\n2020-03-29T03:07:22.1135790Z     clang: error: no such file or directory: 'clang'\n2020-03-29T03:07:22.1137100Z     error: Command \"g++ -sdk macosx clang -bundle -undefined dynamic_lookup -bundle -undefined dynamic_lookup -L/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/local/lib -Wl,-rpath,/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/local/lib -L/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/homebrew/opt/readline/lib -L/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/homebrew/lib -D__ACCELERATE__ -I/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/homebrew/opt/readline/include -I/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/homebrew/include build/temp.macosx-10.14-x86_64-3.7/scipy/interpolate/src/_interpolate.o -Lbuild/temp.macosx-10.14-x86_64-3.7 -o build/lib.macosx-10.14-x86_64-3.7/scipy/interpolate/_interpolate.cpython-37m-darwin.so\" failed with exit status 1\n2020-03-29T03:07:22.1137680Z     Running setup.py install for scipy: finished with status 'error'\n```",
    "created_at": "2020-03-30T15:22:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461611",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:44" align="right">comment:44</div>

scipy build in macos-homebrew-standard fails with a similar error:

```
2020-03-29T03:07:22.1134510Z     g++ -sdk macosx clang -bundle -undefined dynamic_lookup -bundle -undefined dynamic_lookup -L/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/local/lib -Wl,-rpath,/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/local/lib -L/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/homebrew/opt/readline/lib -L/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/homebrew/lib -D__ACCELERATE__ -I/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/homebrew/opt/readline/include -I/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/homebrew/include build/temp.macosx-10.14-x86_64-3.7/scipy/interpolate/src/_interpolate.o -Lbuild/temp.macosx-10.14-x86_64-3.7 -o build/lib.macosx-10.14-x86_64-3.7/scipy/interpolate/_interpolate.cpython-37m-darwin.so
2020-03-29T03:07:22.1135020Z     clang: error: unknown argument: '-sdk'
2020-03-29T03:07:22.1135400Z     clang: error: no such file or directory: 'macosx'
2020-03-29T03:07:22.1135790Z     clang: error: no such file or directory: 'clang'
2020-03-29T03:07:22.1137100Z     error: Command "g++ -sdk macosx clang -bundle -undefined dynamic_lookup -bundle -undefined dynamic_lookup -L/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/local/lib -Wl,-rpath,/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/local/lib -L/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/homebrew/opt/readline/lib -L/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/homebrew/lib -D__ACCELERATE__ -I/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/homebrew/opt/readline/include -I/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-homebrew-macos-standard/homebrew/include build/temp.macosx-10.14-x86_64-3.7/scipy/interpolate/src/_interpolate.o -Lbuild/temp.macosx-10.14-x86_64-3.7 -o build/lib.macosx-10.14-x86_64-3.7/scipy/interpolate/_interpolate.cpython-37m-darwin.so" failed with exit status 1
2020-03-29T03:07:22.1137680Z     Running setup.py install for scipy: finished with status 'error'
```



---

archive/issue_events_406343.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-03-30T15:22:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406343"
}
```



---

archive/issue_events_406344.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-03-30T15:22:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406344"
}
```



---

archive/issue_comments_461612.json:
```json
{
    "body": "<div id=\"comment:45\" align=\"right\">comment:45</div>\n\nReplying to [@dimpase](#comment:44):\n> scipy build in macos-homebrew-standard fails with a similar error\n\nWrong ticket, that's #29408.",
    "created_at": "2020-03-30T15:41:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461612",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:45" align="right">comment:45</div>

Replying to [@dimpase](#comment:44):
> scipy build in macos-homebrew-standard fails with a similar error

Wrong ticket, that's #29408.



---

archive/issue_events_406345.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-30T15:41:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406345"
}
```



---

archive/issue_events_406346.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-30T15:41:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406346"
}
```



---

archive/issue_comments_461613.json:
```json
{
    "body": "<div id=\"comment:46\" align=\"right\">comment:46</div>\n\nwhat do you mean, wrong ticket? I looked at the test results at [comment:42](#comment:42)\nabove (https://github.com/mkoeppe/sage/actions/runs/65556636)",
    "created_at": "2020-03-30T15:55:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461613",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:46" align="right">comment:46</div>

what do you mean, wrong ticket? I looked at the test results at [comment:42](#comment:42)
above (https://github.com/mkoeppe/sage/actions/runs/65556636)



---

archive/issue_comments_461614.json:
```json
{
    "body": "<div id=\"comment:47\" align=\"right\">comment:47</div>\n\nThis ticket is about pynac failing with `configure: error: Cannot find python3.7 in your system path`.",
    "created_at": "2020-03-30T15:56:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461614",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:47" align="right">comment:47</div>

This ticket is about pynac failing with `configure: error: Cannot find python3.7 in your system path`.



---

archive/issue_comments_461615.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2020-03-30T16:38:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461615",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_comments_461616.json:
```json
{
    "body": "<div id=\"comment:48\" align=\"right\">comment:48</div>\n\nOK, this looks good. Sorry for confusion.",
    "created_at": "2020-03-30T16:38:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461616",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:48" align="right">comment:48</div>

OK, this looks good. Sorry for confusion.



---

archive/issue_events_406347.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-03-30T16:38:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406347"
}
```



---

archive/issue_events_406348.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-03-30T16:38:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406348"
}
```



---

archive/issue_events_406349.json:
```json
{
    "actor": "https://github.com/isuruf",
    "created_at": "2020-03-30T16:44:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406349"
}
```



---

archive/issue_events_406350.json:
```json
{
    "actor": "https://github.com/isuruf",
    "created_at": "2020-03-30T16:44:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406350"
}
```



---

archive/issue_comments_461617.json:
```json
{
    "body": "<div id=\"comment:49\" align=\"right\">comment:49</div>\n\nThanks for looking into this. This needs more work due to https://github.com/pynac/pynac/pull/351#discussion_r399846249.",
    "created_at": "2020-03-30T16:44:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461617",
    "user": "https://github.com/isuruf"
}
```

<div id="comment:49" align="right">comment:49</div>

Thanks for looking into this. This needs more work due to https://github.com/pynac/pynac/pull/351#discussion_r399846249.



---

archive/issue_comments_461618.json:
```json
{
    "body": "<div id=\"comment:50\" align=\"right\">comment:50</div>\n\nWhether this is good enough for upstream is another question - and one that will not be answered soon.\nBut the fix works for us.",
    "created_at": "2020-03-30T16:46:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461618",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:50" align="right">comment:50</div>

Whether this is good enough for upstream is another question - and one that will not be answered soon.
But the fix works for us.



---

archive/issue_comments_461619.json:
```json
{
    "body": "<div id=\"comment:51\" align=\"right\">comment:51</div>\n\nDoes it though? I assume that warning gets triggered in osx system python.",
    "created_at": "2020-03-30T16:47:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461619",
    "user": "https://github.com/isuruf"
}
```

<div id="comment:51" align="right">comment:51</div>

Does it though? I assume that warning gets triggered in osx system python.



---

archive/issue_comments_461620.json:
```json
{
    "body": "<div id=\"comment:52\" align=\"right\">comment:52</div>\n\nThis is using the version with assert, and it's not failing. See logs above.",
    "created_at": "2020-03-30T16:49:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461620",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:52" align="right">comment:52</div>

This is using the version with assert, and it's not failing. See logs above.



---

archive/issue_events_406351.json:
```json
{
    "actor": "https://github.com/isuruf",
    "created_at": "2020-03-30T16:56:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406351"
}
```



---

archive/issue_events_406352.json:
```json
{
    "actor": "https://github.com/isuruf",
    "created_at": "2020-03-30T16:56:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406352"
}
```



---

archive/issue_comments_461621.json:
```json
{
    "body": "<div id=\"comment:53\" align=\"right\">comment:53</div>\n\nI don't understand how it's going to work, but not going to hold up this ticket.",
    "created_at": "2020-03-30T16:56:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461621",
    "user": "https://github.com/isuruf"
}
```

<div id="comment:53" align="right">comment:53</div>

I don't understand how it's going to work, but not going to hold up this ticket.



---

archive/issue_comments_461622.json:
```json
{
    "body": "<div id=\"comment:54\" align=\"right\">comment:54</div>\n\nReplying to [@isuruf](#comment:53):\n> I don't understand how it's going to work\n\nJust in case you'd like another look, here's the log from https://github.com/mkoeppe/sage/actions/runs/66127777, logs-commit-3fc71b184bd15f5b10e48334eccb73be63dd3033-tox-local-homebrew-macos-standard.zip .\nNote that this is NOT using the change from #29408.\n\n```\npynac-0.7.26.sage-2020-03-28a\n====================================================\nSetting up build directory for pynac-0.7.26.sage-2020-03-28a\nFinished extraction\nNo patch files found in ../patches\n****************************************************\nHost system:\nDarwin Mac-1306.local 19.3.0 Darwin Kernel Version 19.3.0: Thu Jan  9 20:58:23 PST 2020; root:xnu-6153.81.5~1/RELEASE_X86_64 x86_64\n****************************************************\nC compiler: x86_64-apple-darwin13.4.0-clang\nC compiler version:\nclang version 9.0.1 \nTarget: x86_64-apple-darwin13.4.0\nThread model: posix\nInstalledDir: /Users/runner/runners/2.165.2/work/sage/sage/.tox/local-conda-forge-macos-standard/conda/bin\n****************************************************\nPackage 'pynac' is currently not installed\nNo legacy uninstaller found for 'pynac'; nothing to do\nStarting build...\nRunning build_pynac...\nConfiguring pynac-0.7.26.sage-2020-03-28a\n...\nchecking whether sage-python23 version is >= 2.7... yes\nchecking for sage-python23 version... 3.7\nchecking for sage-python23 platform... darwin\nchecking for sage-python23 script directory... ${prefix}/lib/python3.7/site-packages\nchecking for sage-python23 extension module directory... ${exec_prefix}/lib/python3.7/site-packages\nchecking for Python preprocessor flags... -I/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-conda-forge-macos-standard/conda/include/python3.7m -I/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-conda-forge-macos-standard/conda/include/python3.7m -fno-strict-aliasing -Wsign-compare -Wunreachable-code -DNDEBUG -fwrapv -O3 -Wall -Wstrict-prototypes -march=core2 -mtune=haswell -mssse3 -ftree-vectorize -fPIC -fPIE -fstack-protector-strong -O3 -pipe -fdebug-prefix-map=${SRC_DIR}=/usr/local/src/conda/${PKG_NAME}-${PKG_VERSION} -fdebug-prefix-map=/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-conda-forge-macos-standard/conda=/usr/local/src/conda-prefix -flto -Wl,-export_dynamic -march=core2 -mtune=haswell -mssse3 -ftree-vectorize -fPIC -fPIE -fstack-protector-strong -O3\nchecking for Python library flags... -bundle -undefined dynamic_lookup -Wl,-pie -Wl,-headerpad_max_install_names -Wl,-dead_strip_dylibs -Wl,-rpath,/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-conda-forge-macos-standard/conda/lib -L/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-conda-forge-macos-standard/conda/lib -flto -Wl,-export_dynamic -Wl,-pie -Wl,-headerpad_max_install_names -Wl,-dead_strip_dylibs -Wl,-rpath,/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-conda-forge-macos-standard/conda/lib -L/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-conda-forge-macos-standard/conda/lib -L/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-conda-forge-macos-standard/local/lib -Wl,-rpath,/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-conda-forge-macos-standard/local/lib -Wl,-rpath,/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-conda-forge-macos-standard/conda/lib -L/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-conda-forge-macos-standard/conda/lib -march=core2 -mtune=haswell -mssse3 -ftree-vectorize -fPIC -fPIE -fstack-protector-strong -O2 -pipe -isystem /Users/runner/runners/2.165.2/work/sage/sage/.tox/local-conda-forge-macos-standard/conda/include -D_FORTIFY_SOURCE=2 -mmacosx-version-min=10.9 -isystem /Users/runner/runners/2.165.2/work/sage/sage/.tox/local-conda-forge-macos-standard/conda/include\nchecking build system type... x86_64-apple-darwin19.3.0\n```",
    "created_at": "2020-03-30T17:42:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461622",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:54" align="right">comment:54</div>

Replying to [@isuruf](#comment:53):
> I don't understand how it's going to work

Just in case you'd like another look, here's the log from https://github.com/mkoeppe/sage/actions/runs/66127777, logs-commit-3fc71b184bd15f5b10e48334eccb73be63dd3033-tox-local-homebrew-macos-standard.zip .
Note that this is NOT using the change from #29408.

```
pynac-0.7.26.sage-2020-03-28a
====================================================
Setting up build directory for pynac-0.7.26.sage-2020-03-28a
Finished extraction
No patch files found in ../patches
****************************************************
Host system:
Darwin Mac-1306.local 19.3.0 Darwin Kernel Version 19.3.0: Thu Jan  9 20:58:23 PST 2020; root:xnu-6153.81.5~1/RELEASE_X86_64 x86_64
****************************************************
C compiler: x86_64-apple-darwin13.4.0-clang
C compiler version:
clang version 9.0.1 
Target: x86_64-apple-darwin13.4.0
Thread model: posix
InstalledDir: /Users/runner/runners/2.165.2/work/sage/sage/.tox/local-conda-forge-macos-standard/conda/bin
****************************************************
Package 'pynac' is currently not installed
No legacy uninstaller found for 'pynac'; nothing to do
Starting build...
Running build_pynac...
Configuring pynac-0.7.26.sage-2020-03-28a
...
checking whether sage-python23 version is >= 2.7... yes
checking for sage-python23 version... 3.7
checking for sage-python23 platform... darwin
checking for sage-python23 script directory... ${prefix}/lib/python3.7/site-packages
checking for sage-python23 extension module directory... ${exec_prefix}/lib/python3.7/site-packages
checking for Python preprocessor flags... -I/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-conda-forge-macos-standard/conda/include/python3.7m -I/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-conda-forge-macos-standard/conda/include/python3.7m -fno-strict-aliasing -Wsign-compare -Wunreachable-code -DNDEBUG -fwrapv -O3 -Wall -Wstrict-prototypes -march=core2 -mtune=haswell -mssse3 -ftree-vectorize -fPIC -fPIE -fstack-protector-strong -O3 -pipe -fdebug-prefix-map=${SRC_DIR}=/usr/local/src/conda/${PKG_NAME}-${PKG_VERSION} -fdebug-prefix-map=/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-conda-forge-macos-standard/conda=/usr/local/src/conda-prefix -flto -Wl,-export_dynamic -march=core2 -mtune=haswell -mssse3 -ftree-vectorize -fPIC -fPIE -fstack-protector-strong -O3
checking for Python library flags... -bundle -undefined dynamic_lookup -Wl,-pie -Wl,-headerpad_max_install_names -Wl,-dead_strip_dylibs -Wl,-rpath,/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-conda-forge-macos-standard/conda/lib -L/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-conda-forge-macos-standard/conda/lib -flto -Wl,-export_dynamic -Wl,-pie -Wl,-headerpad_max_install_names -Wl,-dead_strip_dylibs -Wl,-rpath,/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-conda-forge-macos-standard/conda/lib -L/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-conda-forge-macos-standard/conda/lib -L/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-conda-forge-macos-standard/local/lib -Wl,-rpath,/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-conda-forge-macos-standard/local/lib -Wl,-rpath,/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-conda-forge-macos-standard/conda/lib -L/Users/runner/runners/2.165.2/work/sage/sage/.tox/local-conda-forge-macos-standard/conda/lib -march=core2 -mtune=haswell -mssse3 -ftree-vectorize -fPIC -fPIE -fstack-protector-strong -O2 -pipe -isystem /Users/runner/runners/2.165.2/work/sage/sage/.tox/local-conda-forge-macos-standard/conda/include -D_FORTIFY_SOURCE=2 -mmacosx-version-min=10.9 -isystem /Users/runner/runners/2.165.2/work/sage/sage/.tox/local-conda-forge-macos-standard/conda/include
checking build system type... x86_64-apple-darwin19.3.0
```



---

archive/issue_comments_461623.json:
```json
{
    "body": "<div id=\"comment:55\" align=\"right\">comment:55</div>\n\nFails on cygwin (https://github.com/mkoeppe/sage/runs/548082943):\n\n```\n  [pynac-0.7.26.sage-2020-03-28a]   /cygdrive/d/a/sage/sage/local/var/tmp/sage/build/pynac-0.7.26.sage-2020-03-28a/src/ginac/numeric.cpp:175: undefined reference to `PyErr_Occurred'\n  [pynac-0.7.26.sage-2020-03-28a]   /usr/lib/gcc/x86_64-pc-cygwin/9.3.0/../../../../x86_64-pc-cygwin/bin/ld: .libs/libpynac_la-numeric.o:/cygdrive/d/a/sage/sage/local/var/tmp/sage/build/pynac-0.7.26.sage-2020-03-28a/src/ginac/numeric.cpp:502: undefined reference to `PyLong_FromLong'\n  [pynac-0.7.26.sage-2020-03-28a]   /usr/lib/gcc/x86_64-pc-cygwin/9.3.0/../../../../x86_64-pc-cygwin/bin/ld: .libs/libpynac_la-numeric.o: in function `__static_initialization_and_destruction_0':\n  [pynac-0.7.26.sage-2020-03-28a]   /cygdrive/d/a/sage/sage/local/var/tmp/sage/build/pynac-0.7.26.sage-2020-03-28a/src/ginac/numeric.cpp:503: undefined reference to `PyLong_FromLong'\n  [pynac-0.7.26.sage-2020-03-28a]   /usr/lib/gcc/x86_64-pc-cygwin/9.3.0/../../../../x86_64-pc-cygwin/bin/ld: /cygdrive/d/a/sage/sage/local/var/tmp/sage/build/pynac-0.7.26.sage-2020-03-28a/src/ginac/numeric.cpp:504: undefined reference to `PyLong_FromLong'\n  [pynac-0.7.26.sage-2020-03-28a]   collect2: error: ld returned 1 exit status\n  [pynac-0.7.26.sage-2020-03-28a]   make[6]: *** [Makefile:558: libpynac.la] Error 1\n```",
    "created_at": "2020-03-31T14:09:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461623",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:55" align="right">comment:55</div>

Fails on cygwin (https://github.com/mkoeppe/sage/runs/548082943):

```
  [pynac-0.7.26.sage-2020-03-28a]   /cygdrive/d/a/sage/sage/local/var/tmp/sage/build/pynac-0.7.26.sage-2020-03-28a/src/ginac/numeric.cpp:175: undefined reference to `PyErr_Occurred'
  [pynac-0.7.26.sage-2020-03-28a]   /usr/lib/gcc/x86_64-pc-cygwin/9.3.0/../../../../x86_64-pc-cygwin/bin/ld: .libs/libpynac_la-numeric.o:/cygdrive/d/a/sage/sage/local/var/tmp/sage/build/pynac-0.7.26.sage-2020-03-28a/src/ginac/numeric.cpp:502: undefined reference to `PyLong_FromLong'
  [pynac-0.7.26.sage-2020-03-28a]   /usr/lib/gcc/x86_64-pc-cygwin/9.3.0/../../../../x86_64-pc-cygwin/bin/ld: .libs/libpynac_la-numeric.o: in function `__static_initialization_and_destruction_0':
  [pynac-0.7.26.sage-2020-03-28a]   /cygdrive/d/a/sage/sage/local/var/tmp/sage/build/pynac-0.7.26.sage-2020-03-28a/src/ginac/numeric.cpp:503: undefined reference to `PyLong_FromLong'
  [pynac-0.7.26.sage-2020-03-28a]   /usr/lib/gcc/x86_64-pc-cygwin/9.3.0/../../../../x86_64-pc-cygwin/bin/ld: /cygdrive/d/a/sage/sage/local/var/tmp/sage/build/pynac-0.7.26.sage-2020-03-28a/src/ginac/numeric.cpp:504: undefined reference to `PyLong_FromLong'
  [pynac-0.7.26.sage-2020-03-28a]   collect2: error: ld returned 1 exit status
  [pynac-0.7.26.sage-2020-03-28a]   make[6]: *** [Makefile:558: libpynac.la] Error 1
```



---

archive/issue_events_406353.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-31T14:14:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406353"
}
```



---

archive/issue_events_406354.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-31T14:14:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406354"
}
```



---

archive/issue_comments_461624.json:
```json
{
    "body": "<div id=\"comment:58\"></div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/db5074a5487edaa6ce153d5a7812387539fc8c7a\"><code>db5074a</code></a></td><td><code>Use pynac-0.7.26.sage-2020-03-31a</code></td></tr></table>\n",
    "created_at": "2020-04-01T03:10:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461624",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:58"></div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/db5074a5487edaa6ce153d5a7812387539fc8c7a"><code>db5074a</code></a></td><td><code>Use pynac-0.7.26.sage-2020-03-31a</code></td></tr></table>




---

archive/issue_comments_461625.json:
```json
{
    "body": "Changed commit from **[`6f2b336`](https://github.com/sagemath/sagetrac-mirror/commit/6f2b3362bf0fe885916f7a233c1bb2cac7100186)** to **[`db5074a`](https://github.com/sagemath/sagetrac-mirror/commit/db5074a5487edaa6ce153d5a7812387539fc8c7a)**",
    "created_at": "2020-04-01T03:10:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461625",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[`6f2b336`](https://github.com/sagemath/sagetrac-mirror/commit/6f2b3362bf0fe885916f7a233c1bb2cac7100186)** to **[`db5074a`](https://github.com/sagemath/sagetrac-mirror/commit/db5074a5487edaa6ce153d5a7812387539fc8c7a)**



---

archive/issue_events_406355.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-01T04:14:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406355"
}
```



---

archive/issue_events_406356.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-01T04:14:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406356"
}
```



---

archive/issue_comments_461626.json:
```json
{
    "body": "<div id=\"comment:59\" align=\"right\">comment:59</div>\n\nNew tests at https://github.com/mkoeppe/sage/runs/550765335 and https://github.com/mkoeppe/sage/actions/runs/67915244",
    "created_at": "2020-04-01T04:14:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461626",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:59" align="right">comment:59</div>

New tests at https://github.com/mkoeppe/sage/runs/550765335 and https://github.com/mkoeppe/sage/actions/runs/67915244



---

archive/issue_events_406357.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-03T00:19:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406357"
}
```



---

archive/issue_events_406358.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-03T00:19:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406358"
}
```



---

archive/issue_comments_461627.json:
```json
{
    "body": "<div id=\"comment:61\"></div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cb15a592f3149dc709ad8b7f85bfe154d5211bc2\"><code>cb15a59</code></a></td><td><code>Use pynac-0.7.26.sage-2020-04-02</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/deb717fb4c53b2851a04fff610f98ca0ff80f84a\"><code>deb717f</code></a></td><td><code>use pynac-0.7.26.sage-2020-04-03</code></td></tr></table>\n",
    "created_at": "2020-04-03T19:49:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461627",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:61"></div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cb15a592f3149dc709ad8b7f85bfe154d5211bc2"><code>cb15a59</code></a></td><td><code>Use pynac-0.7.26.sage-2020-04-02</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/deb717fb4c53b2851a04fff610f98ca0ff80f84a"><code>deb717f</code></a></td><td><code>use pynac-0.7.26.sage-2020-04-03</code></td></tr></table>




---

archive/issue_comments_461628.json:
```json
{
    "body": "Changed commit from **[`db5074a`](https://github.com/sagemath/sagetrac-mirror/commit/db5074a5487edaa6ce153d5a7812387539fc8c7a)** to **[`deb717f`](https://github.com/sagemath/sagetrac-mirror/commit/deb717fb4c53b2851a04fff610f98ca0ff80f84a)**",
    "created_at": "2020-04-03T19:49:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461628",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[`db5074a`](https://github.com/sagemath/sagetrac-mirror/commit/db5074a5487edaa6ce153d5a7812387539fc8c7a)** to **[`deb717f`](https://github.com/sagemath/sagetrac-mirror/commit/deb717fb4c53b2851a04fff610f98ca0ff80f84a)**



---

archive/issue_events_406359.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-03T20:24:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406359"
}
```



---

archive/issue_events_406360.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-03T20:24:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406360"
}
```



---

archive/issue_comments_461629.json:
```json
{
    "body": "<div id=\"comment:62\" align=\"right\">comment:62</div>\n\nThis fixes the build on cygwin.",
    "created_at": "2020-04-03T20:24:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461629",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:62" align="right">comment:62</div>

This fixes the build on cygwin.



---

archive/issue_comments_461630.json:
```json
{
    "body": "<div id=\"comment:63\" align=\"right\">comment:63</div>\n\nplease squash the commits",
    "created_at": "2020-04-04T01:01:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461630",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:63" align="right">comment:63</div>

please squash the commits



---

archive/issue_comments_461631.json:
```json
{
    "body": "Changed commit from **[`deb717f`](https://github.com/sagemath/sagetrac-mirror/commit/deb717fb4c53b2851a04fff610f98ca0ff80f84a)** to **[`7344e2d`](https://github.com/sagemath/sagetrac-mirror/commit/7344e2df8bebecdab19924a14ff4dbe51cc514f9)**",
    "created_at": "2020-04-04T01:48:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461631",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[`deb717f`](https://github.com/sagemath/sagetrac-mirror/commit/deb717fb4c53b2851a04fff610f98ca0ff80f84a)** to **[`7344e2d`](https://github.com/sagemath/sagetrac-mirror/commit/7344e2df8bebecdab19924a14ff4dbe51cc514f9)**



---

archive/issue_comments_461632.json:
```json
{
    "body": "<div id=\"comment:64\"></div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7344e2df8bebecdab19924a14ff4dbe51cc514f9\"><code>7344e2d</code></a></td><td><code>Use temporary upstream version pynac-0.7.26.sage-2020-04-03</code></td></tr></table>\n",
    "created_at": "2020-04-04T01:48:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461632",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:64"></div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7344e2df8bebecdab19924a14ff4dbe51cc514f9"><code>7344e2d</code></a></td><td><code>Use temporary upstream version pynac-0.7.26.sage-2020-04-03</code></td></tr></table>




---

archive/issue_comments_461633.json:
```json
{
    "body": "<div id=\"comment:65\" align=\"right\">comment:65</div>\n\nsquashed, needs review",
    "created_at": "2020-04-05T17:18:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461633",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:65" align="right">comment:65</div>

squashed, needs review



---

archive/issue_comments_461634.json:
```json
{
    "body": "<div id=\"comment:66\" align=\"right\">comment:66</div>\n\nCorrect builds can be seen at https://github.com/mkoeppe/sage/actions/runs/74117913",
    "created_at": "2020-04-10T04:27:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461634",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:66" align="right">comment:66</div>

Correct builds can be seen at https://github.com/mkoeppe/sage/actions/runs/74117913



---

archive/issue_comments_461635.json:
```json
{
    "body": "<div id=\"comment:67\" align=\"right\">comment:67</div>\n\nIt's good on OS X, built with various different Pythons (homebrew, system, Sage).",
    "created_at": "2020-04-10T18:41:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461635",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:67" align="right">comment:67</div>

It's good on OS X, built with various different Pythons (homebrew, system, Sage).



---

archive/issue_comments_461636.json:
```json
{
    "body": "<div id=\"comment:68\" align=\"right\">comment:68</div>\n\nLet's get this one into the next beta please",
    "created_at": "2020-04-12T00:41:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461636",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:68" align="right">comment:68</div>

Let's get this one into the next beta please



---

archive/issue_comments_461637.json:
```json
{
    "body": "<div id=\"comment:69\" align=\"right\">comment:69</div>\n\nlooks good.",
    "created_at": "2020-04-12T00:47:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461637",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:69" align="right">comment:69</div>

looks good.



---

archive/issue_events_406361.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-04-12T00:47:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406361"
}
```



---

archive/issue_events_406362.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-04-12T00:47:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406362"
}
```



---

archive/issue_comments_461638.json:
```json
{
    "body": "<div id=\"comment:70\" align=\"right\">comment:70</div>\n\nThanks!",
    "created_at": "2020-04-12T00:55:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461638",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:70" align="right">comment:70</div>

Thanks!



---

archive/issue_events_406363.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-15T03:16:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/p%3A%202%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p: 2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406363"
}
```



---

archive/issue_events_406364.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-15T03:16:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p: 1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406364"
}
```



---

archive/issue_comments_461639.json:
```json
{
    "body": "Attachment: **[pynac-0.7.26.sage-2020-04-03.log](https://github.com/sagemath/sage/files/ticket29404/pynac-0.7.26.sage-2020-04-03.log)**\n\npynac did not download on Mac OS 10.15.3",
    "created_at": "2020-04-15T06:24:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461639",
    "user": "https://github.com/DaveWitteMorris"
}
```

Attachment: **[pynac-0.7.26.sage-2020-04-03.log](https://github.com/sagemath/sage/files/ticket29404/pynac-0.7.26.sage-2020-04-03.log)**

pynac did not download on Mac OS 10.15.3



---

archive/issue_comments_461640.json:
```json
{
    "body": "<div id=\"comment:72\" align=\"right\">comment:72</div>\n\nI apologize if this is noise (I may have messed up my installation somehow), but I thought I should report that I had trouble building pynac in 9.1rc0 on Mac OS 10.15.3, even with this ticket:\n\n```\nAttempting to download from https://github.com/mkoeppe/pynac/releases/download/pynac-0.7.26.sage-2020-04-03/pynac-0.7.26.sage-2020-04-03.tar.bz2\n[xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx]\nERROR [transfer|run:135]: [Errno socket error] [SSL: CERTIFICATE_VERIFY_FAILED] certificate verify failed: unable to get local issuer certificate (_ssl.c:1056)\n************************************************************************\n```\nI merged #29408 and this ticket, and ran `make build`, then realized that the description of this ticket has a different make command, so I tried that, too. But pynac did not download. I am attaching the `pynac-0.7.26.sage-2020-04-03.log` log file from the more complicated build command, in case it is of use.\n\nThe build went smoothly after I manually downloaded pynac and put it in the upstream directory, so I want to say thanks for these patches! I never would have figured out how to fix the build errors on my own.",
    "created_at": "2020-04-15T06:35:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461640",
    "user": "https://github.com/DaveWitteMorris"
}
```

<div id="comment:72" align="right">comment:72</div>

I apologize if this is noise (I may have messed up my installation somehow), but I thought I should report that I had trouble building pynac in 9.1rc0 on Mac OS 10.15.3, even with this ticket:

```
Attempting to download from https://github.com/mkoeppe/pynac/releases/download/pynac-0.7.26.sage-2020-04-03/pynac-0.7.26.sage-2020-04-03.tar.bz2
[xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx]
ERROR [transfer|run:135]: [Errno socket error] [SSL: CERTIFICATE_VERIFY_FAILED] certificate verify failed: unable to get local issuer certificate (_ssl.c:1056)
************************************************************************
```
I merged #29408 and this ticket, and ran `make build`, then realized that the description of this ticket has a different make command, so I tried that, too. But pynac did not download. I am attaching the `pynac-0.7.26.sage-2020-04-03.log` log file from the more complicated build command, in case it is of use.

The build went smoothly after I manually downloaded pynac and put it in the upstream directory, so I want to say thanks for these patches! I never would have figured out how to fix the build errors on my own.



---

archive/issue_comments_461641.json:
```json
{
    "body": "<div id=\"comment:73\" align=\"right\">comment:73</div>\n\nthis might be due to (system) python, more precisely its urllib module, being unable to provide correct certs...\n\ndoes installing python 3.7 from python.org and also making sure it installs certs (there is a 2nd step in installation, iirc) fixes this?",
    "created_at": "2020-04-15T06:41:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461641",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:73" align="right">comment:73</div>

this might be due to (system) python, more precisely its urllib module, being unable to provide correct certs...

does installing python 3.7 from python.org and also making sure it installs certs (there is a 2nd step in installation, iirc) fixes this?



---

archive/issue_comments_461642.json:
```json
{
    "body": "<div id=\"comment:74\" align=\"right\">comment:74</div>\n\nYes, that solved the problem. After installing python and certifi, pynac downloaded from mkoeppe's directory (and the build succeeded) when I used `make SAGE_SPKG=\"sage-spkg -o\" pynac-clean pynac`. Thanks!",
    "created_at": "2020-04-15T07:53:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461642",
    "user": "https://github.com/DaveWitteMorris"
}
```

<div id="comment:74" align="right">comment:74</div>

Yes, that solved the problem. After installing python and certifi, pynac downloaded from mkoeppe's directory (and the build succeeded) when I used `make SAGE_SPKG="sage-spkg -o" pynac-clean pynac`. Thanks!



---

archive/issue_events_406365.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-04-16T22:33:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406365"
}
```



---

archive/issue_events_406366.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "bf8c78179973be44ccfd8b4290d06a5e7956380c",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-04-16T22:33:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29404#event-406366"
}
```



---

archive/issue_comments_461643.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/fix_build_failures_of_pynac_on_macos_with_system_python3](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/fix_build_failures_of_pynac_on_macos_with_system_python3)** to **[`7344e2d`](https://github.com/sagemath/sagetrac-mirror/commit/7344e2df8bebecdab19924a14ff4dbe51cc514f9)**",
    "created_at": "2020-04-16T22:33:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461643",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/fix_build_failures_of_pynac_on_macos_with_system_python3](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/fix_build_failures_of_pynac_on_macos_with_system_python3)** to **[`7344e2d`](https://github.com/sagemath/sagetrac-mirror/commit/7344e2df8bebecdab19924a14ff4dbe51cc514f9)**



---

archive/issue_comments_461644.json:
```json
{
    "body": "Changed commit from **[`7344e2d`](https://github.com/sagemath/sagetrac-mirror/commit/7344e2df8bebecdab19924a14ff4dbe51cc514f9)** to none",
    "created_at": "2020-06-16T11:41:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461644",
    "user": "https://github.com/mkoeppe"
}
```

Changed commit from **[`7344e2d`](https://github.com/sagemath/sagetrac-mirror/commit/7344e2df8bebecdab19924a14ff4dbe51cc514f9)** to none



---

archive/issue_comments_461645.json:
```json
{
    "body": "<div id=\"comment:76\" align=\"right\">comment:76</div>\n\nFollow up: #29872",
    "created_at": "2020-06-16T11:41:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29404",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29404#issuecomment-461645",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:76" align="right">comment:76</div>

Follow up: #29872
