# Issue 29479: Forward Breadth first search

archive/issues_029242.json:
```json
{
    "assignees": [],
    "body": "So I have been working on an algorithm, which was described in a research paper, where it makes use of the forward breadthfirstsearch, a new type of bfs algorithm, which simply traverses from one vertex in a 'forward' directed sense, while I am working on this proper implementation, I am currently stuck at the point because sage math can return in and out vertices as a list, but won't that process for each vertex be simply too tedious and longer in the run-time?\nIn a nutshell, is there any method of traversing in a directed sense beginning from a particular vertex?\n\nReviewer: **Vipul Gupta**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29479_\n\n",
    "closed_at": "2020-04-13T16:34:50Z",
    "created_at": "2020-04-08T13:15:57Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Forward Breadth first search",
    "type": "issue",
    "updated_at": "2020-04-13T16:34:50Z",
    "url": "https://github.com/sagemath/sage/issues/29479",
    "user": "https://github.com/Shlokatadistance"
}
```
So I have been working on an algorithm, which was described in a research paper, where it makes use of the forward breadthfirstsearch, a new type of bfs algorithm, which simply traverses from one vertex in a 'forward' directed sense, while I am working on this proper implementation, I am currently stuck at the point because sage math can return in and out vertices as a list, but won't that process for each vertex be simply too tedious and longer in the run-time?
In a nutshell, is there any method of traversing in a directed sense beginning from a particular vertex?

Reviewer: **Vipul Gupta**

_Issue created by migration from https://trac.sagemath.org/ticket/29479_





---

archive/issue_events_387205.json:
```json
{
    "actor": "https://github.com/Shlokatadistance",
    "created_at": "2020-04-08T13:15:57Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29479",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29479#event-387205"
}
```



---

archive/issue_events_387206.json:
```json
{
    "actor": "https://github.com/Shlokatadistance",
    "created_at": "2020-04-08T13:15:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29479",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29479#event-387206"
}
```



---

archive/issue_events_387207.json:
```json
{
    "actor": "https://github.com/Shlokatadistance",
    "created_at": "2020-04-08T13:15:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29479",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29479#event-387207"
}
```



---

archive/issue_comments_466273.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nIf I understand the ticket description clearly, you are asking if we can traverse from a particular vertex in fixed direction(that is to say either towards its in-neighbors or out-neighbors). right?\n\nIf that's the case you can call `g.breadth_first_search(source, neighbors = neighbors=g.neighbors_out)` for Forward_BFS and `g.breadth_first_search(source, neighbors = neighbors=g.neighbors_in)` for Backward_BFS",
    "created_at": "2020-04-09T13:56:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29479",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29479#issuecomment-466273",
    "user": "https://github.com/vipul79321"
}
```

<div id="comment:1" align="right">Comment 1</div>

If I understand the ticket description clearly, you are asking if we can traverse from a particular vertex in fixed direction(that is to say either towards its in-neighbors or out-neighbors). right?

If that's the case you can call `g.breadth_first_search(source, neighbors = neighbors=g.neighbors_out)` for Forward_BFS and `g.breadth_first_search(source, neighbors = neighbors=g.neighbors_in)` for Backward_BFS



---

archive/issue_comments_466274.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nI see, that was the issue, because the direction flag in my case would turn void, especially if both forwards and backwards path were present between a pair of vertices.\nReplying to [@vipul79321](#comment%3A1):\n> If I understand the ticket description clearly, you are asking if we can traverse from a particular vertex in fixed direction(that is to say either towards its in-neighbors or out-neighbors). right?\n> \n> If that's the case you can call `g.breadth_first_search(source, neighbors = neighbors=g.neighbors_out)` for Forward_BFS and `g.breadth_first_search(source, neighbors = neighbors=g.neighbors_in)` for Backward_BFS",
    "created_at": "2020-04-10T05:19:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29479",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29479#issuecomment-466274",
    "user": "https://github.com/Shlokatadistance"
}
```

<div id="comment:2" align="right">Comment 2</div>

I see, that was the issue, because the direction flag in my case would turn void, especially if both forwards and backwards path were present between a pair of vertices.
Replying to [@vipul79321](#comment%3A1):
> If I understand the ticket description clearly, you are asking if we can traverse from a particular vertex in fixed direction(that is to say either towards its in-neighbors or out-neighbors). right?
> 
> If that's the case you can call `g.breadth_first_search(source, neighbors = neighbors=g.neighbors_out)` for Forward_BFS and `g.breadth_first_search(source, neighbors = neighbors=g.neighbors_in)` for Backward_BFS



---

archive/issue_comments_466275.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nReplying to [@Shlokatadistance](#comment%3A2):\n> I see, that was the issue, because the direction flag in my case would turn void, especially if both forwards and backwards path were present between a pair of vertices.\n> Replying to [@vipul79321](#comment%3A1):\n> > If I understand the ticket description clearly, you are asking if we can traverse from a particular vertex in fixed direction(that is to say either towards its in-neighbors or out-neighbors). right?\n> > \n> > If that's the case you can call `g.breadth_first_search(source, neighbors = neighbors=g.neighbors_out)` for Forward_BFS and `g.breadth_first_search(source, neighbors = neighbors=g.neighbors_in)` for Backward_BFS\n\nI guess that solves it. \\\\\nPlease change milestone to `invalid/duplicate/wontfix`",
    "created_at": "2020-04-10T09:43:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29479",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29479#issuecomment-466275",
    "user": "https://github.com/vipul79321"
}
```

<div id="comment:3" align="right">Comment 3</div>

Replying to [@Shlokatadistance](#comment%3A2):
> I see, that was the issue, because the direction flag in my case would turn void, especially if both forwards and backwards path were present between a pair of vertices.
> Replying to [@vipul79321](#comment%3A1):
> > If I understand the ticket description clearly, you are asking if we can traverse from a particular vertex in fixed direction(that is to say either towards its in-neighbors or out-neighbors). right?
> > 
> > If that's the case you can call `g.breadth_first_search(source, neighbors = neighbors=g.neighbors_out)` for Forward_BFS and `g.breadth_first_search(source, neighbors = neighbors=g.neighbors_in)` for Backward_BFS

I guess that solves it. \\
Please change milestone to `invalid/duplicate/wontfix`



---

archive/issue_events_387208.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2020-04-10T11:17:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29479",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29479#event-387208"
}
```



---

archive/issue_events_387209.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2020-04-10T11:17:04Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29479",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29479#event-387209"
}
```



---

archive/issue_events_387210.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2020-04-10T11:17:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29479",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29479#event-387210"
}
```



---

archive/issue_events_387211.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2020-04-10T11:17:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29479",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29479#event-387211"
}
```



---

archive/issue_comments_466276.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nPlease use https://ask.sagemath.org/ to ask questions. This is the appropriate place.",
    "created_at": "2020-04-10T11:17:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29479",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29479#issuecomment-466276",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:4" align="right">Comment 4</div>

Please use https://ask.sagemath.org/ to ask questions. This is the appropriate place.



---

archive/issue_comments_466277.json:
```json
{
    "body": "Reviewer: **Vipul Gupta**",
    "created_at": "2020-04-10T11:28:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29479",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29479#issuecomment-466277",
    "user": "https://github.com/vipul79321"
}
```

Reviewer: **Vipul Gupta**



---

archive/issue_events_387212.json:
```json
{
    "actor": "https://github.com/vipul79321",
    "created_at": "2020-04-10T11:28:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29479",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29479#event-387212"
}
```



---

archive/issue_events_387213.json:
```json
{
    "actor": "https://github.com/vipul79321",
    "created_at": "2020-04-10T11:28:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29479",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29479#event-387213"
}
```



---

archive/issue_events_387214.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-04-13T16:34:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29479",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29479#event-387214"
}
```



---

archive/issue_events_387215.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-04-13T16:34:50Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29479",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29479#event-387215"
}
```
