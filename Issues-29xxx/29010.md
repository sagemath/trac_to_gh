# Issue 29010: Split off sage.rings.number_field.homset from sage.rings.number_field.morphism

archive/issues_028773.json:
```json
{
    "assignees": [],
    "body": "The module `sage.rings.number_field.morphism` contains classes for homsets as well as homomorphisms.  It would be good to split off the homsets as a new module similar to `sage.rings.homset`.  One advantage is that this will make the patch for #28869 more readable.\n\nBranch/Commit: **[fa2a465](https://github.com/sagemath/sagetrac-mirror/commit/fa2a465eeb91e5176f0aa960f8e17f590ef2084d)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Peter Bruin**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29010_\n\n",
    "closed_at": "2020-01-20T21:17:34Z",
    "created_at": "2020-01-14T14:15:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Split off sage.rings.number_field.homset from sage.rings.number_field.morphism",
    "type": "issue",
    "updated_at": "2020-01-20T21:17:34Z",
    "url": "https://github.com/sagemath/sage/issues/29010",
    "user": "https://github.com/pjbruin"
}
```
The module `sage.rings.number_field.morphism` contains classes for homsets as well as homomorphisms.  It would be good to split off the homsets as a new module similar to `sage.rings.homset`.  One advantage is that this will make the patch for #28869 more readable.

Branch/Commit: **[fa2a465](https://github.com/sagemath/sagetrac-mirror/commit/fa2a465eeb91e5176f0aa960f8e17f590ef2084d)**

Reviewer: **Travis Scrimshaw**

Author: **Peter Bruin**

_Issue created by migration from https://trac.sagemath.org/ticket/29010_





---

archive/issue_events_380333.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2020-01-14T14:15:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29010",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29010#event-380333"
}
```



---

archive/issue_events_380334.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2020-01-14T14:15:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29010",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
    "label_color": "0000ff",
    "label_name": "component: number fields",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29010#event-380334"
}
```



---

archive/issue_events_380335.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2020-01-14T14:15:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29010",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29010#event-380335"
}
```



---

archive/issue_events_380336.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2020-01-14T14:32:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29010",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29010#event-380336"
}
```



---

archive/issue_comments_456106.json:
```json
{
    "body": "Branch: **[u/pbruin/29010-number_field_homset](https://github.com/sagemath/sagetrac-mirror/tree/u/pbruin/29010-number_field_homset)**",
    "created_at": "2020-01-14T14:32:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29010",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29010#issuecomment-456106",
    "user": "https://github.com/pjbruin"
}
```

Branch: **[u/pbruin/29010-number_field_homset](https://github.com/sagemath/sagetrac-mirror/tree/u/pbruin/29010-number_field_homset)**



---

archive/issue_comments_456107.json:
```json
{
    "body": "Commit: **[1a66fed](https://github.com/sagemath/sagetrac-mirror/commit/1a66fedd32a9e9a4cb9ecc31c5626087608ce44c)**",
    "created_at": "2020-01-14T14:32:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29010",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29010#issuecomment-456107",
    "user": "https://github.com/pjbruin"
}
```

Commit: **[1a66fed](https://github.com/sagemath/sagetrac-mirror/commit/1a66fedd32a9e9a4cb9ecc31c5626087608ce44c)**



---

archive/issue_comments_456108.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nLGTM except I think we should have a deprecation redirect (which you can do with a `lazy_import` IIRC).",
    "created_at": "2020-01-14T17:12:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29010",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29010#issuecomment-456108",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:2'>Comment 2:</a>
LGTM except I think we should have a deprecation redirect (which you can do with a `lazy_import` IIRC).



---

archive/issue_comments_456109.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fa2a465eeb91e5176f0aa960f8e17f590ef2084d\">fa2a465</a></td><td><code>Trac 29010: add lazy import with deprecation warning for moved classes</code></td></tr></table>\n",
    "created_at": "2020-01-15T10:42:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29010",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29010#issuecomment-456109",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:3'>Comment 3:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fa2a465eeb91e5176f0aa960f8e17f590ef2084d">fa2a465</a></td><td><code>Trac 29010: add lazy import with deprecation warning for moved classes</code></td></tr></table>




---

archive/issue_comments_456110.json:
```json
{
    "body": "Changed commit from **[1a66fed](https://github.com/sagemath/sagetrac-mirror/commit/1a66fedd32a9e9a4cb9ecc31c5626087608ce44c)** to **[fa2a465](https://github.com/sagemath/sagetrac-mirror/commit/fa2a465eeb91e5176f0aa960f8e17f590ef2084d)**",
    "created_at": "2020-01-15T10:42:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29010",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29010#issuecomment-456110",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[1a66fed](https://github.com/sagemath/sagetrac-mirror/commit/1a66fedd32a9e9a4cb9ecc31c5626087608ce44c)** to **[fa2a465](https://github.com/sagemath/sagetrac-mirror/commit/fa2a465eeb91e5176f0aa960f8e17f590ef2084d)**



---

archive/issue_comments_456111.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nReplying to [@tscrim](#comment%3A2):\n> LGTM except I think we should have a deprecation redirect (which you can do with a `lazy_import` IIRC).\n\nDone, thanks for the hint.",
    "created_at": "2020-01-15T10:42:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29010",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29010#issuecomment-456111",
    "user": "https://github.com/pjbruin"
}
```

<a id='comment:4'>Comment 4:</a>
Replying to [@tscrim](#comment%3A2):
> LGTM except I think we should have a deprecation redirect (which you can do with a `lazy_import` IIRC).

Done, thanks for the hint.



---

archive/issue_comments_456112.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nThank you.",
    "created_at": "2020-01-15T16:12:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29010",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29010#issuecomment-456112",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:5'>Comment 5:</a>
Thank you.



---

archive/issue_comments_456113.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2020-01-15T16:12:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29010",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29010#issuecomment-456113",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_380337.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-01-15T16:12:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29010",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29010#event-380337"
}
```



---

archive/issue_events_380338.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-01-15T16:12:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29010",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29010#event-380338"
}
```



---

archive/issue_events_380339.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-01-20T21:17:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29010",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29010#event-380339"
}
```



---

archive/issue_events_380340.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "64438cef9b51b9ebed352a41fb21f4bd8299f080",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-01-20T21:17:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29010",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29010#event-380340"
}
```



---

archive/issue_comments_456114.json:
```json
{
    "body": "Changed branch from **[u/pbruin/29010-number_field_homset](https://github.com/sagemath/sagetrac-mirror/tree/u/pbruin/29010-number_field_homset)** to **[fa2a465](https://github.com/sagemath/sagetrac-mirror/commit/fa2a465eeb91e5176f0aa960f8e17f590ef2084d)**",
    "created_at": "2020-01-20T21:17:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29010",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29010#issuecomment-456114",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/pbruin/29010-number_field_homset](https://github.com/sagemath/sagetrac-mirror/tree/u/pbruin/29010-number_field_homset)** to **[fa2a465](https://github.com/sagemath/sagetrac-mirror/commit/fa2a465eeb91e5176f0aa960f8e17f590ef2084d)**
