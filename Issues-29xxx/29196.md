# Issue 29196: Make Ehrhart series related function cached

archive/issues_028959.json:
```json
{
    "assignees": [],
    "body": "Currently, the construction for `ehrhart_series` and other related methods in `backend_normaliz` are not cached, which means that they need to be recreated every time they are called.\n\nThese functions should be cached.\n\nCC:  @sophiasage @kliem\n\nKeywords: **ehrhart, h star vector**\n\nBranch/Commit: **[b397c07](https://github.com/sagemath/sagetrac-mirror/commit/b397c07108566c042aa2932ce12b6e58f79e109a)**\n\nReviewer: **Matthias Koeppe**\n\nAuthor: **Jean-Philippe Labb\u00e9, Jonathan Kliem**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29196_\n\n",
    "closed_at": "2020-06-27T09:26:43Z",
    "created_at": "2020-02-14T12:29:51Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make Ehrhart series related function cached",
    "type": "issue",
    "updated_at": "2020-06-27T09:26:43Z",
    "url": "https://github.com/sagemath/sage/issues/29196",
    "user": "https://github.com/jplab"
}
```
Currently, the construction for `ehrhart_series` and other related methods in `backend_normaliz` are not cached, which means that they need to be recreated every time they are called.

These functions should be cached.

CC:  @sophiasage @kliem

Keywords: **ehrhart, h star vector**

Branch/Commit: **[b397c07](https://github.com/sagemath/sagetrac-mirror/commit/b397c07108566c042aa2932ce12b6e58f79e109a)**

Reviewer: **Matthias Koeppe**

Author: **Jean-Philippe Labbé, Jonathan Kliem**

_Issue created by migration from https://trac.sagemath.org/ticket/29196_





---

archive/issue_events_383156.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-02-14T12:29:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29196#event-383156"
}
```



---

archive/issue_events_383157.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-02-14T12:29:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "component: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29196#event-383157"
}
```



---

archive/issue_events_383158.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-02-14T12:29:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29196#event-383158"
}
```



---

archive/issue_events_383159.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-02-14T12:29:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29196#event-383159"
}
```



---

archive/issue_comments_460787.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nBatch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.",
    "created_at": "2020-04-14T19:41:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460787",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:1'>Comment 1:</a>
Batch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.



---

archive/issue_events_383160.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29196#event-383160"
}
```



---

archive/issue_events_383161.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29196#event-383161"
}
```



---

archive/issue_events_383162.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-05-20T13:00:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29196#event-383162"
}
```



---

archive/issue_comments_460788.json:
```json
{
    "body": "Author: **Jean-Philippe Labb\u00e9**",
    "created_at": "2020-05-20T13:00:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460788",
    "user": "https://github.com/jplab"
}
```

Author: **Jean-Philippe Labbé**



---

archive/issue_comments_460789.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nFirst version. Did I miss something?\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b70549bdad06de4b0625b559d672652cfcdc7484\">b70549b</a></td><td><code>First version of pickled ehrhart stuff</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0a45ea8aab5d414faacf71500ea27df1d37c9a8f\">0a45ea8</a></td><td><code>pep8</code></td></tr></table>\n",
    "created_at": "2020-05-20T13:00:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460789",
    "user": "https://github.com/jplab"
}
```

<a id='comment:2'>Comment 2:</a>
First version. Did I miss something?

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b70549bdad06de4b0625b559d672652cfcdc7484">b70549b</a></td><td><code>First version of pickled ehrhart stuff</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0a45ea8aab5d414faacf71500ea27df1d37c9a8f">0a45ea8</a></td><td><code>pep8</code></td></tr></table>




---

archive/issue_comments_460790.json:
```json
{
    "body": "Commit: **[0a45ea8](https://github.com/sagemath/sagetrac-mirror/commit/0a45ea8aab5d414faacf71500ea27df1d37c9a8f)**",
    "created_at": "2020-05-20T13:00:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460790",
    "user": "https://github.com/jplab"
}
```

Commit: **[0a45ea8](https://github.com/sagemath/sagetrac-mirror/commit/0a45ea8aab5d414faacf71500ea27df1d37c9a8f)**



---

archive/issue_comments_460791.json:
```json
{
    "body": "Changed keywords from **** to **ehrhart, h star vector**",
    "created_at": "2020-05-20T13:00:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460791",
    "user": "https://github.com/jplab"
}
```

Changed keywords from **** to **ehrhart, h star vector**



---

archive/issue_comments_460792.json:
```json
{
    "body": "Branch: **[public/29196](https://github.com/sagemath/sagetrac-mirror/tree/public/29196)**",
    "created_at": "2020-05-20T13:00:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460792",
    "user": "https://github.com/jplab"
}
```

Branch: **[public/29196](https://github.com/sagemath/sagetrac-mirror/tree/public/29196)**



---

archive/issue_events_383163.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-05-20T13:02:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29196#event-383163"
}
```



---

archive/issue_events_383164.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-05-20T13:02:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29196#event-383164"
}
```



---

archive/issue_comments_460793.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nThis is the first try. The tests are not passing. It needs some fixing.",
    "created_at": "2020-05-20T13:09:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460793",
    "user": "https://github.com/jplab"
}
```

<a id='comment:4'>Comment 4:</a>
This is the first try. The tests are not passing. It needs some fixing.



---

archive/issue_comments_460794.json:
```json
{
    "body": "Work Issues: **Failing doctests, hilbert series is not hashable**",
    "created_at": "2020-05-20T13:09:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460794",
    "user": "https://github.com/jplab"
}
```

Work Issues: **Failing doctests, hilbert series is not hashable**



---

archive/issue_comments_460795.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b894494ff997620b704ad582558b3b385b253b4b\">b894494</a></td><td><code>make hilbert series arguments cachable</code></td></tr></table>\n",
    "created_at": "2020-05-20T14:53:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460795",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:5'>Comment 5:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b894494ff997620b704ad582558b3b385b253b4b">b894494</a></td><td><code>make hilbert series arguments cachable</code></td></tr></table>




---

archive/issue_comments_460796.json:
```json
{
    "body": "Changed commit from **[0a45ea8](https://github.com/sagemath/sagetrac-mirror/commit/0a45ea8aab5d414faacf71500ea27df1d37c9a8f)** to **[b894494](https://github.com/sagemath/sagetrac-mirror/commit/b894494ff997620b704ad582558b3b385b253b4b)**",
    "created_at": "2020-05-20T14:53:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460796",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[0a45ea8](https://github.com/sagemath/sagetrac-mirror/commit/0a45ea8aab5d414faacf71500ea27df1d37c9a8f)** to **[b894494](https://github.com/sagemath/sagetrac-mirror/commit/b894494ff997620b704ad582558b3b385b253b4b)**



---

archive/issue_comments_460797.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nAccording to the documentation [http://doc.sagemath.org/html/en/reference/misc/sage/misc/cachefunc.html](http://doc.sagemath.org/html/en/reference/misc/sage/misc/cachefunc.html), this should do it.",
    "created_at": "2020-05-20T14:53:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460797",
    "user": "https://github.com/kliem"
}
```

<a id='comment:6'>Comment 6:</a>
According to the documentation [http://doc.sagemath.org/html/en/reference/misc/sage/misc/cachefunc.html](http://doc.sagemath.org/html/en/reference/misc/sage/misc/cachefunc.html), this should do it.



---

archive/issue_comments_460798.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/db10c95385525f766b2ada5a6a4d4580b9cdcbcb\">db10c95</a></td><td><code>Merge branch 'public/29196' of git://trac.sagemath.org/sage into public/29196-reb</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3ba66bd374dcfad3ba6214acfdd1df0b31322819\">3ba66bd</a></td><td><code>fixing doctests</code></td></tr></table>\n",
    "created_at": "2020-06-06T08:20:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460798",
    "user": "https://github.com/kliem"
}
```

<a id='comment:7'>Comment 7:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/db10c95385525f766b2ada5a6a4d4580b9cdcbcb">db10c95</a></td><td><code>Merge branch 'public/29196' of git://trac.sagemath.org/sage into public/29196-reb</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3ba66bd374dcfad3ba6214acfdd1df0b31322819">3ba66bd</a></td><td><code>fixing doctests</code></td></tr></table>




---

archive/issue_comments_460799.json:
```json
{
    "body": "Changed branch from **[public/29196](https://github.com/sagemath/sagetrac-mirror/tree/public/29196)** to **[public/29196-reb](https://github.com/sagemath/sagetrac-mirror/tree/public/29196-reb)**",
    "created_at": "2020-06-06T08:20:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460799",
    "user": "https://github.com/kliem"
}
```

Changed branch from **[public/29196](https://github.com/sagemath/sagetrac-mirror/tree/public/29196)** to **[public/29196-reb](https://github.com/sagemath/sagetrac-mirror/tree/public/29196-reb)**



---

archive/issue_comments_460800.json:
```json
{
    "body": "Changed commit from **[b894494](https://github.com/sagemath/sagetrac-mirror/commit/b894494ff997620b704ad582558b3b385b253b4b)** to **[3ba66bd](https://github.com/sagemath/sagetrac-mirror/commit/3ba66bd374dcfad3ba6214acfdd1df0b31322819)**",
    "created_at": "2020-06-06T08:20:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460800",
    "user": "https://github.com/kliem"
}
```

Changed commit from **[b894494](https://github.com/sagemath/sagetrac-mirror/commit/b894494ff997620b704ad582558b3b385b253b4b)** to **[3ba66bd](https://github.com/sagemath/sagetrac-mirror/commit/3ba66bd374dcfad3ba6214acfdd1df0b31322819)**



---

archive/issue_comments_460801.json:
```json
{
    "body": "Changed author from **Jean-Philippe Labb\u00e9** to **Jean-Philippe Labb\u00e9, Jonathan Kliem**",
    "created_at": "2020-06-06T08:20:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460801",
    "user": "https://github.com/kliem"
}
```

Changed author from **Jean-Philippe Labbé** to **Jean-Philippe Labbé, Jonathan Kliem**



---

archive/issue_events_383165.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-06-06T08:20:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29196#event-383165"
}
```



---

archive/issue_events_383166.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-06-06T08:20:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29196#event-383166"
}
```



---

archive/issue_comments_460802.json:
```json
{
    "body": "Changed commit from **[3ba66bd](https://github.com/sagemath/sagetrac-mirror/commit/3ba66bd374dcfad3ba6214acfdd1df0b31322819)** to **[6fa511d](https://github.com/sagemath/sagetrac-mirror/commit/6fa511dd53a95ba271f2a71b07214db8e8caddcb)**",
    "created_at": "2020-06-06T08:42:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460802",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[3ba66bd](https://github.com/sagemath/sagetrac-mirror/commit/3ba66bd374dcfad3ba6214acfdd1df0b31322819)** to **[6fa511d](https://github.com/sagemath/sagetrac-mirror/commit/6fa511dd53a95ba271f2a71b07214db8e8caddcb)**



---

archive/issue_comments_460803.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6fa511dd53a95ba271f2a71b07214db8e8caddcb\">6fa511d</a></td><td><code>use anonymous function for simplification</code></td></tr></table>\n",
    "created_at": "2020-06-06T08:42:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460803",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:8'>Comment 8:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6fa511dd53a95ba271f2a71b07214db8e8caddcb">6fa511d</a></td><td><code>use anonymous function for simplification</code></td></tr></table>




---

archive/issue_comments_460804.json:
```json
{
    "body": "Changed branch from **[public/29196-reb](https://github.com/sagemath/sagetrac-mirror/tree/public/29196-reb)** to **[public/29196-reb2](https://github.com/sagemath/sagetrac-mirror/tree/public/29196-reb2)**",
    "created_at": "2020-06-17T09:10:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460804",
    "user": "https://github.com/kliem"
}
```

Changed branch from **[public/29196-reb](https://github.com/sagemath/sagetrac-mirror/tree/public/29196-reb)** to **[public/29196-reb2](https://github.com/sagemath/sagetrac-mirror/tree/public/29196-reb2)**



---

archive/issue_comments_460805.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7bee618888a5d535ca40553ff6d023bd59193089\">7bee618</a></td><td><code>First version of pickled ehrhart stuff</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/370c12c22431ac7f6f7790997be5053930cfa8f3\">370c12c</a></td><td><code>pep8</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c0d0d9dcab8165828672ba3b51d33228610b2b0f\">c0d0d9d</a></td><td><code>use key to make arguments cachable</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/721a1e714bfc2bf4ab540cc3abf7f9560afc6178\">721a1e7</a></td><td><code>fix doctest</code></td></tr></table>\n",
    "created_at": "2020-06-17T09:10:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460805",
    "user": "https://github.com/kliem"
}
```

<a id='comment:9'>Comment 9:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7bee618888a5d535ca40553ff6d023bd59193089">7bee618</a></td><td><code>First version of pickled ehrhart stuff</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/370c12c22431ac7f6f7790997be5053930cfa8f3">370c12c</a></td><td><code>pep8</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c0d0d9dcab8165828672ba3b51d33228610b2b0f">c0d0d9d</a></td><td><code>use key to make arguments cachable</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/721a1e714bfc2bf4ab540cc3abf7f9560afc6178">721a1e7</a></td><td><code>fix doctest</code></td></tr></table>




---

archive/issue_comments_460806.json:
```json
{
    "body": "Changed commit from **[6fa511d](https://github.com/sagemath/sagetrac-mirror/commit/6fa511dd53a95ba271f2a71b07214db8e8caddcb)** to **[721a1e7](https://github.com/sagemath/sagetrac-mirror/commit/721a1e714bfc2bf4ab540cc3abf7f9560afc6178)**",
    "created_at": "2020-06-17T09:10:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460806",
    "user": "https://github.com/kliem"
}
```

Changed commit from **[6fa511d](https://github.com/sagemath/sagetrac-mirror/commit/6fa511dd53a95ba271f2a71b07214db8e8caddcb)** to **[721a1e7](https://github.com/sagemath/sagetrac-mirror/commit/721a1e714bfc2bf4ab540cc3abf7f9560afc6178)**



---

archive/issue_comments_460807.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nAre the \"work issues\" resolved?",
    "created_at": "2020-06-19T16:52:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460807",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:10'>Comment 10:</a>
Are the "work issues" resolved?



---

archive/issue_comments_460808.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nObviously :-) We have a green bot.",
    "created_at": "2020-06-19T16:53:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460808",
    "user": "https://github.com/kliem"
}
```

<a id='comment:11'>Comment 11:</a>
Obviously :-) We have a green bot.



---

archive/issue_comments_460809.json:
```json
{
    "body": "Changed work issues from **Failing doctests, hilbert series is not hashable** to none",
    "created_at": "2020-06-19T16:53:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460809",
    "user": "https://github.com/kliem"
}
```

Changed work issues from **Failing doctests, hilbert series is not hashable** to none



---

archive/issue_comments_460810.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nWell, ok, that doesn't mean anything if the bot doesn't have normaliz, which appears to be the case.\n\nBut I claim they are resolved anyway.",
    "created_at": "2020-06-19T16:55:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460810",
    "user": "https://github.com/kliem"
}
```

<a id='comment:12'>Comment 12:</a>
Well, ok, that doesn't mean anything if the bot doesn't have normaliz, which appears to be the case.

But I claim they are resolved anyway.



---

archive/issue_comments_460811.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2020-06-19T18:23:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460811",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_events_383167.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-19T18:23:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29196#event-383167"
}
```



---

archive/issue_events_383168.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-19T18:23:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29196#event-383168"
}
```



---

archive/issue_comments_460812.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nThank you.",
    "created_at": "2020-06-19T19:00:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460812",
    "user": "https://github.com/kliem"
}
```

<a id='comment:14'>Comment 14:</a>
Thank you.



---

archive/issue_comments_460813.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nIf latte_int is installed:\n\n```\nFile \"src/sage/geometry/polyhedron/base_QQ.py\", line 547, in sage.geometry.polyhedron.base_QQ.Polyhedron_QQ.?\nFailed example:\n    Q.ehrhart_quasipolynomial.is_in_cache()         # optional - latte_int\nExpected:\n    True\nGot:\n    False\n**********************************************************************\n1 item had failures:\n   1 of  67 in sage.geometry.polyhedron.base_QQ.Polyhedron_QQ.?\n    [76 tests, 1 failure, 0.56 s]\n```",
    "created_at": "2020-06-22T22:33:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460813",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:15'>Comment 15:</a>
If latte_int is installed:

```
File "src/sage/geometry/polyhedron/base_QQ.py", line 547, in sage.geometry.polyhedron.base_QQ.Polyhedron_QQ.?
Failed example:
    Q.ehrhart_quasipolynomial.is_in_cache()         # optional - latte_int
Expected:
    True
Got:
    False
**********************************************************************
1 item had failures:
   1 of  67 in sage.geometry.polyhedron.base_QQ.Polyhedron_QQ.?
    [76 tests, 1 failure, 0.56 s]
```



---

archive/issue_events_383169.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-06-22T22:33:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29196#event-383169"
}
```



---

archive/issue_events_383170.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-06-22T22:33:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29196#event-383170"
}
```



---

archive/issue_comments_460814.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b397c07108566c042aa2932ce12b6e58f79e109a\">b397c07</a></td><td><code>fix doctest</code></td></tr></table>\n",
    "created_at": "2020-06-23T05:59:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460814",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:16'>Comment 16:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b397c07108566c042aa2932ce12b6e58f79e109a">b397c07</a></td><td><code>fix doctest</code></td></tr></table>




---

archive/issue_comments_460815.json:
```json
{
    "body": "Changed commit from **[721a1e7](https://github.com/sagemath/sagetrac-mirror/commit/721a1e714bfc2bf4ab540cc3abf7f9560afc6178)** to **[b397c07](https://github.com/sagemath/sagetrac-mirror/commit/b397c07108566c042aa2932ce12b6e58f79e109a)**",
    "created_at": "2020-06-23T05:59:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460815",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[721a1e7](https://github.com/sagemath/sagetrac-mirror/commit/721a1e714bfc2bf4ab540cc3abf7f9560afc6178)** to **[b397c07](https://github.com/sagemath/sagetrac-mirror/commit/b397c07108566c042aa2932ce12b6e58f79e109a)**



---

archive/issue_events_383171.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-06-23T06:00:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29196#event-383171"
}
```



---

archive/issue_events_383172.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-06-23T06:00:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29196#event-383172"
}
```



---

archive/issue_comments_460816.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nSorry, I missed that. Didn't have `latte_int` installed on my laptop. You need to give `is_in_cache` the same arguments of course.",
    "created_at": "2020-06-23T06:00:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460816",
    "user": "https://github.com/kliem"
}
```

<a id='comment:17'>Comment 17:</a>
Sorry, I missed that. Didn't have `latte_int` installed on my laptop. You need to give `is_in_cache` the same arguments of course.



---

archive/issue_events_383173.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-26T16:45:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29196#event-383173"
}
```



---

archive/issue_events_383174.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-26T16:45:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29196#event-383174"
}
```



---

archive/issue_comments_460817.json:
```json
{
    "body": "<a id='comment:19'>Comment 19:</a>\nThank you.",
    "created_at": "2020-06-26T20:37:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460817",
    "user": "https://github.com/kliem"
}
```

<a id='comment:19'>Comment 19:</a>
Thank you.



---

archive/issue_comments_460818.json:
```json
{
    "body": "Changed branch from **[public/29196-reb2](https://github.com/sagemath/sagetrac-mirror/tree/public/29196-reb2)** to **[b397c07](https://github.com/sagemath/sagetrac-mirror/commit/b397c07108566c042aa2932ce12b6e58f79e109a)**",
    "created_at": "2020-06-27T09:26:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29196#issuecomment-460818",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/29196-reb2](https://github.com/sagemath/sagetrac-mirror/tree/public/29196-reb2)** to **[b397c07](https://github.com/sagemath/sagetrac-mirror/commit/b397c07108566c042aa2932ce12b6e58f79e109a)**



---

archive/issue_events_383175.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-06-27T09:26:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29196#event-383175"
}
```



---

archive/issue_events_383176.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "07eb677abb72607b8cbca8226812ace4a7c49e4c",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-06-27T09:26:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29196",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29196#event-383176"
}
```
