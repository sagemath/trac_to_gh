# Issue 29312: Fix attribute lookup in _coerce_map_from_base_ring() for Cython classes

archive/issues_029075.json:
```json
{
    "assignees": [],
    "body": "Reported by Markus Wageringel in [#29247 comment:14](https://github.com/sagemath/sage/issues/29247#comment:14):\n\n```\nsage: F.<x,y,z> = FreeAlgebra(QQ, implementation='letterplace')\nsage: F._coerce_map_from_base_ring()\n...\nAttributeError: type object 'sage.algebras.letterplace.free_algebra_letterplace' has no attribute 'from_base_ring'\n```\n\n\nCC:  @mwageringel @tscrim\n\nBranch/Commit: **[c4631b6](https://github.com/sagemath/sagetrac-mirror/commit/c4631b6b2ff70da6dc329785a521c9a29dc1eade)**\n\nReviewer: **Markus Wageringel**\n\nAuthor: **Peter Bruin**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29312_\n\n",
    "closed_at": "2020-03-12T22:48:12Z",
    "created_at": "2020-03-11T07:32:47Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20coercion",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix attribute lookup in _coerce_map_from_base_ring() for Cython classes",
    "type": "issue",
    "updated_at": "2020-03-12T22:48:12Z",
    "url": "https://github.com/sagemath/sage/issues/29312",
    "user": "https://github.com/pjbruin"
}
```
Reported by Markus Wageringel in [#29247 comment:14](https://github.com/sagemath/sage/issues/29247#comment:14):

```
sage: F.<x,y,z> = FreeAlgebra(QQ, implementation='letterplace')
sage: F._coerce_map_from_base_ring()
...
AttributeError: type object 'sage.algebras.letterplace.free_algebra_letterplace' has no attribute 'from_base_ring'
```


CC:  @mwageringel @tscrim

Branch/Commit: **[c4631b6](https://github.com/sagemath/sagetrac-mirror/commit/c4631b6b2ff70da6dc329785a521c9a29dc1eade)**

Reviewer: **Markus Wageringel**

Author: **Peter Bruin**

_Issue created by migration from https://trac.sagemath.org/ticket/29312_





---

archive/issue_events_384736.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2020-03-11T07:32:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29312",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29312#event-384736"
}
```



---

archive/issue_events_384737.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2020-03-11T07:32:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29312",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20coercion",
    "label_color": "0000ff",
    "label_name": "component: coercion",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29312#event-384737"
}
```



---

archive/issue_events_384738.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2020-03-11T07:32:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29312",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29312#event-384738"
}
```



---

archive/issue_events_384739.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2020-03-11T07:32:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29312",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29312#event-384739"
}
```



---

archive/issue_events_384740.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2020-03-11T08:40:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29312",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29312#event-384740"
}
```



---

archive/issue_comments_462660.json:
```json
{
    "body": "Branch: **[u/pbruin/29312-from_base_ring](https://github.com/sagemath/sagetrac-mirror/tree/u/pbruin/29312-from_base_ring)**",
    "created_at": "2020-03-11T08:40:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29312#issuecomment-462660",
    "user": "https://github.com/pjbruin"
}
```

Branch: **[u/pbruin/29312-from_base_ring](https://github.com/sagemath/sagetrac-mirror/tree/u/pbruin/29312-from_base_ring)**



---

archive/issue_comments_462661.json:
```json
{
    "body": "Commit: **[c4631b6](https://github.com/sagemath/sagetrac-mirror/commit/c4631b6b2ff70da6dc329785a521c9a29dc1eade)**",
    "created_at": "2020-03-11T08:40:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29312#issuecomment-462661",
    "user": "https://github.com/pjbruin"
}
```

Commit: **[c4631b6](https://github.com/sagemath/sagetrac-mirror/commit/c4631b6b2ff70da6dc329785a521c9a29dc1eade)**



---

archive/issue_comments_462662.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nThis looks good to me. Thanks.",
    "created_at": "2020-03-11T18:46:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29312#issuecomment-462662",
    "user": "https://github.com/mwageringel"
}
```

<div id="comment:2" align="right">Comment 2</div>

This looks good to me. Thanks.



---

archive/issue_comments_462663.json:
```json
{
    "body": "Reviewer: **Markus Wageringel**",
    "created_at": "2020-03-11T18:46:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29312#issuecomment-462663",
    "user": "https://github.com/mwageringel"
}
```

Reviewer: **Markus Wageringel**



---

archive/issue_events_384741.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-03-11T18:46:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29312",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29312#event-384741"
}
```



---

archive/issue_events_384742.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-03-11T18:46:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29312",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29312#event-384742"
}
```



---

archive/issue_events_384743.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-03-12T22:48:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29312",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29312#event-384743"
}
```



---

archive/issue_events_384744.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "33ec6fea83ff32afdd9441dbb74bd83e6f63ca4b",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-03-12T22:48:12Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29312",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29312#event-384744"
}
```



---

archive/issue_comments_462664.json:
```json
{
    "body": "Changed branch from **[u/pbruin/29312-from_base_ring](https://github.com/sagemath/sagetrac-mirror/tree/u/pbruin/29312-from_base_ring)** to **[c4631b6](https://github.com/sagemath/sagetrac-mirror/commit/c4631b6b2ff70da6dc329785a521c9a29dc1eade)**",
    "created_at": "2020-03-12T22:48:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29312#issuecomment-462664",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/pbruin/29312-from_base_ring](https://github.com/sagemath/sagetrac-mirror/tree/u/pbruin/29312-from_base_ring)** to **[c4631b6](https://github.com/sagemath/sagetrac-mirror/commit/c4631b6b2ff70da6dc329785a521c9a29dc1eade)**
