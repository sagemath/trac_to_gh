# Issue 29416: tox.ini: Add local-copy

archive/issues_029179.json:
```json
{
    "assignees": [],
    "body": "This would create a copy of the worktree in the toxenvdir. Using `git worktree add --detached` + copying over `git ls-files --exclude-standard -m -o ` or similar.\n\nThis would be an alternative to #21469 (Enable VPATH builds (several independent build trees connected to one source tree))\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29416_\n\n",
    "created_at": "2020-03-27T16:24:15Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "tox.ini: Add local-copy",
    "type": "issue",
    "updated_at": "2023-01-07T21:39:40Z",
    "url": "https://github.com/sagemath/sage/issues/29416",
    "user": "https://github.com/mkoeppe"
}
```
This would create a copy of the worktree in the toxenvdir. Using `git worktree add --detached` + copying over `git ls-files --exclude-standard -m -o ` or similar.

This would be an alternative to #21469 (Enable VPATH builds (several independent build trees connected to one source tree))

_Issue created by migration from https://trac.sagemath.org/ticket/29416_





---

archive/issue_events_386283.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-27T16:24:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29416",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29416#event-386283"
}
```



---

archive/issue_events_386284.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-27T16:24:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29416",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting",
    "label_color": "000080",
    "label_name": "component: porting",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29416#event-386284"
}
```



---

archive/issue_events_386285.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-27T16:24:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29416",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29416#event-386285"
}
```



---

archive/issue_events_386286.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-27T16:24:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29416",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29416#event-386286"
}
```



---

archive/issue_comments_465047.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n This would create a copy of the worktree in the toxenvdir. Using `git worktree add --detached` + copying over `git ls-files --exclude-standard -m -o ` or similar.\n \n-\n+This would be an alternative to #21469 (Enable VPATH builds (several independent build trees connected to one source tree))\n``````\n",
    "created_at": "2020-03-27T16:59:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29416",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29416#issuecomment-465047",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 This would create a copy of the worktree in the toxenvdir. Using `git worktree add --detached` + copying over `git ls-files --exclude-standard -m -o ` or similar.
 
-
+This would be an alternative to #21469 (Enable VPATH builds (several independent build trees connected to one source tree))
``````




---

archive/issue_comments_465048.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\npushing these forward to 9.2",
    "created_at": "2020-04-14T06:38:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29416",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29416#issuecomment-465048",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

pushing these forward to 9.2



---

archive/issue_events_386287.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T06:38:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29416",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29416#event-386287"
}
```



---

archive/issue_events_386288.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T06:38:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29416",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29416#event-386288"
}
```



---

archive/issue_events_386289.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-13T17:11:30Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29416",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29416#event-386289"
}
```



---

archive/issue_events_386290.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-13T17:11:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29416",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29416#event-386290"
}
```



---

archive/issue_events_386291.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29416",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29416#event-386291"
}
```



---

archive/issue_events_386292.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29416",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29416#event-386292"
}
```



---

archive/issue_comments_465049.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nSetting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29416",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29416#issuecomment-465049",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:4" align="right">Comment 4</div>

Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_386293.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29416",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29416#event-386293"
}
```



---

archive/issue_events_386294.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29416",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29416#event-386294"
}
```



---

archive/issue_events_386295.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29416",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29416#event-386295"
}
```



---

archive/issue_events_386296.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29416",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29416#event-386296"
}
```



---

archive/issue_events_386297.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T00:06:20Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29416",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29416#event-386297"
}
```



---

archive/issue_events_386298.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T00:06:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29416",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29416#event-386298"
}
```



---

archive/issue_events_386299.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29416",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29416#event-386299"
}
```



---

archive/issue_events_386300.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29416",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29416#event-386300"
}
```



---

archive/issue_events_386301.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T21:39:40Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29416",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29416#event-386301"
}
```



---

archive/issue_events_386302.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T21:39:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29416",
    "milestone_number": null,
    "milestone_title": "sage-9.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29416#event-386302"
}
```
