# Issue 29920: Remove deprecated scripts sage-list-optional, sage-list-experimental, sage-list-standard

archive/issues_029683.json:
```json
{
    "assignees": [],
    "body": "These scripts have issued a deprecation warning since 2014, directing users to use `sage-list-packages` instead.  They are not used by another script.\n\nWe finally remove them.\n\n\nCC:  @videlec @jhpalmieri\n\nBranch/Commit: **[9c7116b](https://github.com/sagemath/sagetrac-mirror/commit/9c7116b5cb5bb36cfa4834719d7cd4220fae0d91)**\n\nReviewer: **John Palmieri**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29920_\n\n",
    "closed_at": "2020-07-08T19:33:41Z",
    "created_at": "2020-06-20T20:29:43Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20scripts",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove deprecated scripts sage-list-optional, sage-list-experimental, sage-list-standard",
    "type": "issue",
    "updated_at": "2020-07-08T19:33:41Z",
    "url": "https://github.com/sagemath/sage/issues/29920",
    "user": "https://github.com/mkoeppe"
}
```
These scripts have issued a deprecation warning since 2014, directing users to use `sage-list-packages` instead.  They are not used by another script.

We finally remove them.


CC:  @videlec @jhpalmieri

Branch/Commit: **[9c7116b](https://github.com/sagemath/sagetrac-mirror/commit/9c7116b5cb5bb36cfa4834719d7cd4220fae0d91)**

Reviewer: **John Palmieri**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/29920_





---

archive/issue_events_393565.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-20T20:29:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29920",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29920#event-393565"
}
```



---

archive/issue_events_393566.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-20T20:29:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29920",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20scripts",
    "label_color": "000080",
    "label_name": "component: scripts",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29920#event-393566"
}
```



---

archive/issue_events_393567.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-20T20:29:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29920",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29920#event-393567"
}
```



---

archive/issue_events_393568.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-20T20:29:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29920",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29920#event-393568"
}
```



---

archive/issue_comments_475869.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-These scripts have issued a deprecation warning since 2014, directing users to use `sage-list-packages` instead.\n+These scripts have issued a deprecation warning since 2014, directing users to use `sage-list-packages` instead.  They are not used by another script.\n \n We finally remove them.\n \n``````\n",
    "created_at": "2020-06-20T20:33:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29920",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29920#issuecomment-475869",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-These scripts have issued a deprecation warning since 2014, directing users to use `sage-list-packages` instead.
+These scripts have issued a deprecation warning since 2014, directing users to use `sage-list-packages` instead.  They are not used by another script.
 
 We finally remove them.
 
``````




---

archive/issue_comments_475870.json:
```json
{
    "body": "Branch: **[u/mkoeppe/remove_deprecated_scripts_sage_list_optional__sage_list_experimental__sage_list_standard](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/remove_deprecated_scripts_sage_list_optional__sage_list_experimental__sage_list_standard)**",
    "created_at": "2020-06-20T20:34:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29920",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29920#issuecomment-475870",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/remove_deprecated_scripts_sage_list_optional__sage_list_experimental__sage_list_standard](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/remove_deprecated_scripts_sage_list_optional__sage_list_experimental__sage_list_standard)**



---

archive/issue_events_393569.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-20T20:35:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29920",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29920#event-393569"
}
```



---

archive/issue_comments_475871.json:
```json
{
    "body": "Commit: **[9c7116b](https://github.com/sagemath/sagetrac-mirror/commit/9c7116b5cb5bb36cfa4834719d7cd4220fae0d91)**",
    "created_at": "2020-06-20T20:35:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29920",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29920#issuecomment-475871",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[9c7116b](https://github.com/sagemath/sagetrac-mirror/commit/9c7116b5cb5bb36cfa4834719d7cd4220fae0d91)**



---

archive/issue_comments_475872.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9c7116b5cb5bb36cfa4834719d7cd4220fae0d91\">9c7116b</a></td><td><code>src/bin/sage-list-optional, sage-list-experimental, sage-list-standard: Remove deprecated scripts</code></td></tr></table>\n",
    "created_at": "2020-06-20T20:35:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29920",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29920#issuecomment-475872",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:3" align="right">Comment 3</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9c7116b5cb5bb36cfa4834719d7cd4220fae0d91">9c7116b</a></td><td><code>src/bin/sage-list-optional, sage-list-experimental, sage-list-standard: Remove deprecated scripts</code></td></tr></table>




---

archive/issue_comments_475873.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2020-06-20T20:35:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29920",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29920#issuecomment-475873",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_events_393570.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2020-06-20T21:12:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29920",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29920#event-393570"
}
```



---

archive/issue_events_393571.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2020-06-20T21:12:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29920",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29920#event-393571"
}
```



---

archive/issue_comments_475874.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nYes, good idea. Can we also remove `sage-cython` (on an other ticket)?",
    "created_at": "2020-06-20T21:12:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29920",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29920#issuecomment-475874",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:4" align="right">Comment 4</div>

Yes, good idea. Can we also remove `sage-cython` (on an other ticket)?



---

archive/issue_comments_475875.json:
```json
{
    "body": "Reviewer: **John Palmieri**",
    "created_at": "2020-06-20T21:12:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29920",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29920#issuecomment-475875",
    "user": "https://github.com/jhpalmieri"
}
```

Reviewer: **John Palmieri**



---

archive/issue_comments_475876.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nSee #29923 for `sage-cython`.",
    "created_at": "2020-06-20T22:32:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29920",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29920#issuecomment-475876",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:5" align="right">Comment 5</div>

See #29923 for `sage-cython`.



---

archive/issue_comments_475877.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nThanks!",
    "created_at": "2020-06-20T22:50:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29920",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29920#issuecomment-475877",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:6" align="right">Comment 6</div>

Thanks!



---

archive/issue_comments_475878.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/remove_deprecated_scripts_sage_list_optional__sage_list_experimental__sage_list_standard](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/remove_deprecated_scripts_sage_list_optional__sage_list_experimental__sage_list_standard)** to **[9c7116b](https://github.com/sagemath/sagetrac-mirror/commit/9c7116b5cb5bb36cfa4834719d7cd4220fae0d91)**",
    "created_at": "2020-07-08T19:33:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29920",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29920#issuecomment-475878",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/remove_deprecated_scripts_sage_list_optional__sage_list_experimental__sage_list_standard](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/remove_deprecated_scripts_sage_list_optional__sage_list_experimental__sage_list_standard)** to **[9c7116b](https://github.com/sagemath/sagetrac-mirror/commit/9c7116b5cb5bb36cfa4834719d7cd4220fae0d91)**



---

archive/issue_events_393572.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-07-08T19:33:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29920",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29920#event-393572"
}
```



---

archive/issue_events_393573.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "2de06fc4298dfcd6c28a7b96bda41b101666dec2",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-07-08T19:33:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29920",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29920#event-393573"
}
```
