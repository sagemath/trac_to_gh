# Issue 29969: Make misc doctests ready for random seeds

archive/issues_029732.json:
```json
{
    "assignees": [],
    "body": "This ticket makes\n\n```\nsage -t --long --random-seed=n src/sage/misc/\n```\npass for different values `n` than just `0`.\n\nCC:  @DaveWitteMorris\n\nBranch/Commit: **[bcd0654](https://github.com/sagemath/sagetrac-mirror/commit/bcd0654754507febba1956e54e230f18bb7a2959)**\n\nReviewer: **Marc Mezzarobba**\n\nAuthor: **Jonathan Kliem**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29969_\n\n",
    "closed_at": "2021-02-20T10:46:23Z",
    "created_at": "2020-06-24T20:33:15Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20doctest%20framework",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make misc doctests ready for random seeds",
    "type": "issue",
    "updated_at": "2021-02-20T10:46:23Z",
    "url": "https://github.com/sagemath/sage/issues/29969",
    "user": "https://github.com/kliem"
}
```
This ticket makes

```
sage -t --long --random-seed=n src/sage/misc/
```
pass for different values `n` than just `0`.

CC:  @DaveWitteMorris

Branch/Commit: **[bcd0654](https://github.com/sagemath/sagetrac-mirror/commit/bcd0654754507febba1956e54e230f18bb7a2959)**

Reviewer: **Marc Mezzarobba**

Author: **Jonathan Kliem**

_Issue created by migration from https://trac.sagemath.org/ticket/29969_





---

archive/issue_events_394352.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-06-24T20:33:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29969",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29969#event-394352"
}
```



---

archive/issue_events_394353.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-06-24T20:33:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29969",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20doctest%20framework",
    "label_color": "000080",
    "label_name": "component: doctest framework",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29969#event-394353"
}
```



---

archive/issue_events_394354.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-06-24T20:33:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29969",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29969#event-394354"
}
```



---

archive/issue_events_394355.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-06-24T20:33:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29969",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29969#event-394355"
}
```



---

archive/issue_comments_477150.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nAt least the following need fixes\n\n```\nsage -t --long --random-seed=151058820726654196682836430928254760259 src/sage/misc/misc.py  # 4 doctests failed\nsage -t --long --random-seed=151058820726654196682836430928254760259 src/sage/misc/prandom.py  # 35 doctests failed\nsage -t --long --random-seed=151058820726654196682836430928254760259 src/sage/misc/randstate.pyx  # 2 doctests failed\nsage -t --long --random-seed=151058820726654196682836430928254760259 src/sage/misc/sage_unittest.py  # 2 doctests failed\n```",
    "created_at": "2020-06-24T20:34:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29969",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29969#issuecomment-477150",
    "user": "https://github.com/kliem"
}
```

<div id="comment:1" align="right">Comment 1</div>

At least the following need fixes

```
sage -t --long --random-seed=151058820726654196682836430928254760259 src/sage/misc/misc.py  # 4 doctests failed
sage -t --long --random-seed=151058820726654196682836430928254760259 src/sage/misc/prandom.py  # 35 doctests failed
sage -t --long --random-seed=151058820726654196682836430928254760259 src/sage/misc/randstate.pyx  # 2 doctests failed
sage -t --long --random-seed=151058820726654196682836430928254760259 src/sage/misc/sage_unittest.py  # 2 doctests failed
```



---

archive/issue_events_394356.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-05T19:21:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29969",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29969#event-394356"
}
```



---

archive/issue_events_394357.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-05T19:21:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29969",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29969#event-394357"
}
```



---

archive/issue_comments_477151.json:
```json
{
    "body": "Commit: **[bcd0654](https://github.com/sagemath/sagetrac-mirror/commit/bcd0654754507febba1956e54e230f18bb7a2959)**",
    "created_at": "2021-01-25T10:00:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29969",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29969#issuecomment-477151",
    "user": "https://github.com/kliem"
}
```

Commit: **[bcd0654](https://github.com/sagemath/sagetrac-mirror/commit/bcd0654754507febba1956e54e230f18bb7a2959)**



---

archive/issue_comments_477152.json:
```json
{
    "body": "Author: **Jonathan Kliem**",
    "created_at": "2021-01-25T10:00:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29969",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29969#issuecomment-477152",
    "user": "https://github.com/kliem"
}
```

Author: **Jonathan Kliem**



---

archive/issue_comments_477153.json:
```json
{
    "body": "Branch: **[public/29969](https://github.com/sagemath/sagetrac-mirror/tree/public/29969)**",
    "created_at": "2021-01-25T10:00:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29969",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29969#issuecomment-477153",
    "user": "https://github.com/kliem"
}
```

Branch: **[public/29969](https://github.com/sagemath/sagetrac-mirror/tree/public/29969)**



---

archive/issue_events_394358.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2021-01-25T10:00:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29969",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29969#event-394358"
}
```



---

archive/issue_comments_477154.json:
```json
{
    "body": "Changed dependencies from **#29962** to none",
    "created_at": "2021-01-25T10:00:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29969",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29969#issuecomment-477154",
    "user": "https://github.com/kliem"
}
```

Changed dependencies from **#29962** to none



---

archive/issue_comments_477155.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bcd0654754507febba1956e54e230f18bb7a2959\">bcd0654</a></td><td><code>make sage/misc ready for implicit fuzzuing of doctests</code></td></tr></table>\n",
    "created_at": "2021-01-25T10:00:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29969",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29969#issuecomment-477155",
    "user": "https://github.com/kliem"
}
```

<div id="comment:3" align="right">Comment 3</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bcd0654754507febba1956e54e230f18bb7a2959">bcd0654</a></td><td><code>make sage/misc ready for implicit fuzzuing of doctests</code></td></tr></table>




---

archive/issue_events_394359.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2021-01-26T09:19:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29969",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29969#event-394359"
}
```



---

archive/issue_events_394360.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2021-01-26T09:19:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29969",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29969#event-394360"
}
```



---

archive/issue_comments_477156.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nLooks good (and very thorough) to me, thank you!",
    "created_at": "2021-01-26T09:19:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29969",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29969#issuecomment-477156",
    "user": "https://github.com/mezzarobba"
}
```

<div id="comment:4" align="right">Comment 4</div>

Looks good (and very thorough) to me, thank you!



---

archive/issue_comments_477157.json:
```json
{
    "body": "Reviewer: **Marc Mezzarobba**",
    "created_at": "2021-01-26T09:19:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29969",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29969#issuecomment-477157",
    "user": "https://github.com/mezzarobba"
}
```

Reviewer: **Marc Mezzarobba**



---

archive/issue_comments_477158.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThank you for reviewing.",
    "created_at": "2021-01-26T09:20:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29969",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29969#issuecomment-477158",
    "user": "https://github.com/kliem"
}
```

<div id="comment:5" align="right">Comment 5</div>

Thank you for reviewing.



---

archive/issue_events_394361.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-02-20T10:46:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29969",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29969#event-394361"
}
```



---

archive/issue_events_394362.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "46f654efa9bd0248f2e299f447ec851272537836",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-02-20T10:46:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29969",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29969#event-394362"
}
```



---

archive/issue_comments_477159.json:
```json
{
    "body": "Changed branch from **[public/29969](https://github.com/sagemath/sagetrac-mirror/tree/public/29969)** to **[bcd0654](https://github.com/sagemath/sagetrac-mirror/commit/bcd0654754507febba1956e54e230f18bb7a2959)**",
    "created_at": "2021-02-20T10:46:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29969",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29969#issuecomment-477159",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/29969](https://github.com/sagemath/sagetrac-mirror/tree/public/29969)** to **[bcd0654](https://github.com/sagemath/sagetrac-mirror/commit/bcd0654754507febba1956e54e230f18bb7a2959)**
