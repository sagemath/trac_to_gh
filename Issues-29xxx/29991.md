# Issue 29991: Category of ordered sets

archive/issues_029754.json:
```json
{
    "assignees": [],
    "body": "We introduce the category of (totally) ordered sets.\n\n\n\nSee also:\n- #19269: add category `Posets` to `ZZ` and `QQ`\n- #18222: provide <=, <, >=, > for poset elements by the category - commented out method `Posets.ElementMethods.__le__` (\"TODO: implement x<y, x<=y, x>y, x>=y appropriately once #10130 is resolved\")\n- https://docs.python.org/3/library/functools.html#functools.total_ordering\n- `@`richcmp_method\n- #17601 Meta ticket: Asymptotic Expansions in [SageMath](../wiki/SageMath)\n- https://github.com/leanprover-community/mathlib/tree/master/src/order\n\nCC:  @tscrim @nthiery @videlec @jm58660 @dkrenn @dimpase\n\nBranch/Commit: **[u/mkoeppe/ordered_sets](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/ordered_sets) @ [343f321](https://github.com/sagemath/sagetrac-mirror/commit/343f321a63b7213defcf96da1570bf5a22c799da)**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29991_\n\n",
    "created_at": "2020-06-26T01:49:04Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20categories",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Category of ordered sets",
    "type": "issue",
    "updated_at": "2023-01-07T21:39:40Z",
    "url": "https://github.com/sagemath/sage/issues/29991",
    "user": "https://github.com/mkoeppe"
}
```
We introduce the category of (totally) ordered sets.



See also:
- #19269: add category `Posets` to `ZZ` and `QQ`
- #18222: provide <=, <, >=, > for poset elements by the category - commented out method `Posets.ElementMethods.__le__` ("TODO: implement x<y, x<=y, x>y, x>=y appropriately once #10130 is resolved")
- https://docs.python.org/3/library/functools.html#functools.total_ordering
- `@`richcmp_method
- #17601 Meta ticket: Asymptotic Expansions in [SageMath](../wiki/SageMath)
- https://github.com/leanprover-community/mathlib/tree/master/src/order

CC:  @tscrim @nthiery @videlec @jm58660 @dkrenn @dimpase

Branch/Commit: **[u/mkoeppe/ordered_sets](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/ordered_sets) @ [343f321](https://github.com/sagemath/sagetrac-mirror/commit/343f321a63b7213defcf96da1570bf5a22c799da)**

_Issue created by migration from https://trac.sagemath.org/ticket/29991_





---

archive/issue_events_394725.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-26T01:49:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29991#event-394725"
}
```



---

archive/issue_events_394726.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-26T01:49:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20categories",
    "label_color": "0000ff",
    "label_name": "component: categories",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29991#event-394726"
}
```



---

archive/issue_events_394727.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-26T01:49:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29991#event-394727"
}
```



---

archive/issue_events_394728.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-26T01:49:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29991#event-394728"
}
```



---

archive/issue_comments_477565.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,2 +1,9 @@\n We introduce the category of (totally) ordered sets.\n \n+\n+\n+See also:\n+- https://docs.python.org/3/library/functools.html#functools.total_ordering\n+- `@`richcmp_method\n+\n+\n``````\n",
    "created_at": "2020-06-26T03:39:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29991#issuecomment-477565",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,2 +1,9 @@
 We introduce the category of (totally) ordered sets.
 
+
+
+See also:
+- https://docs.python.org/3/library/functools.html#functools.total_ordering
+- `@`richcmp_method
+
+
``````




---

archive/issue_comments_477566.json:
```json
{
    "body": "Branch: **[u/mkoeppe/ordered_sets](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/ordered_sets)**",
    "created_at": "2020-06-26T03:42:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29991#issuecomment-477566",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/ordered_sets](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/ordered_sets)**



---

archive/issue_comments_477567.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -3,7 +3,8 @@\n \n \n See also:\n+- #19269: add category `Posets` to `ZZ` and `QQ`\n+- commented out method `Posets.ElementMethods.__le__` (\"TODO: implement x<y, x<=y, x>y, x>=y appropriately once #10130 is resolved\")\n - https://docs.python.org/3/library/functools.html#functools.total_ordering\n - `@`richcmp_method\n \n-\n``````\n",
    "created_at": "2020-06-26T03:46:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29991#issuecomment-477567",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -3,7 +3,8 @@
 
 
 See also:
+- #19269: add category `Posets` to `ZZ` and `QQ`
+- commented out method `Posets.ElementMethods.__le__` ("TODO: implement x<y, x<=y, x>y, x>=y appropriately once #10130 is resolved")
 - https://docs.python.org/3/library/functools.html#functools.total_ordering
 - `@`richcmp_method
 
-
``````




---

archive/issue_comments_477568.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/207768403083c9cdd16a41ab03a79d30a24356a1\">2077684</a></td><td><code>sage.categories.ordered_sets: New</code></td></tr></table>\n",
    "created_at": "2020-06-26T03:46:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29991#issuecomment-477568",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:3" align="right">Comment 3</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/207768403083c9cdd16a41ab03a79d30a24356a1">2077684</a></td><td><code>sage.categories.ordered_sets: New</code></td></tr></table>




---

archive/issue_comments_477569.json:
```json
{
    "body": "Commit: **[2077684](https://github.com/sagemath/sagetrac-mirror/commit/207768403083c9cdd16a41ab03a79d30a24356a1)**",
    "created_at": "2020-06-26T03:46:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29991#issuecomment-477569",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[2077684](https://github.com/sagemath/sagetrac-mirror/commit/207768403083c9cdd16a41ab03a79d30a24356a1)**



---

archive/issue_comments_477570.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -4,7 +4,8 @@\n \n See also:\n - #19269: add category `Posets` to `ZZ` and `QQ`\n-- commented out method `Posets.ElementMethods.__le__` (\"TODO: implement x<y, x<=y, x>y, x>=y appropriately once #10130 is resolved\")\n+- #18222: provide <=, <, >=, > for poset elements by the category - commented out method `Posets.ElementMethods.__le__` (\"TODO: implement x<y, x<=y, x>y, x>=y appropriately once #10130 is resolved\")\n - https://docs.python.org/3/library/functools.html#functools.total_ordering\n - `@`richcmp_method\n+- #17601 Meta ticket: Asymptotic Expansions in [SageMath](../wiki/SageMath)\n \n``````\n",
    "created_at": "2020-06-26T03:57:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29991#issuecomment-477570",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -4,7 +4,8 @@
 
 See also:
 - #19269: add category `Posets` to `ZZ` and `QQ`
-- commented out method `Posets.ElementMethods.__le__` ("TODO: implement x<y, x<=y, x>y, x>=y appropriately once #10130 is resolved")
+- #18222: provide <=, <, >=, > for poset elements by the category - commented out method `Posets.ElementMethods.__le__` ("TODO: implement x<y, x<=y, x>y, x>=y appropriately once #10130 is resolved")
 - https://docs.python.org/3/library/functools.html#functools.total_ordering
 - `@`richcmp_method
+- #17601 Meta ticket: Asymptotic Expansions in [SageMath](../wiki/SageMath)
 
``````




---

archive/issue_events_394729.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-26T17:25:37Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "title_is": "Category of ordered sets",
    "title_was": "Ordered Sets",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29991#event-394729"
}
```



---

archive/issue_comments_477571.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -8,4 +8,4 @@\n - https://docs.python.org/3/library/functools.html#functools.total_ordering\n - `@`richcmp_method\n - #17601 Meta ticket: Asymptotic Expansions in [SageMath](../wiki/SageMath)\n-\n+- https://github.com/leanprover-community/mathlib/tree/master/src/order\n``````\n",
    "created_at": "2020-06-26T17:25:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29991#issuecomment-477571",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -8,4 +8,4 @@
 - https://docs.python.org/3/library/functools.html#functools.total_ordering
 - `@`richcmp_method
 - #17601 Meta ticket: Asymptotic Expansions in [SageMath](../wiki/SageMath)
-
+- https://github.com/leanprover-community/mathlib/tree/master/src/order
``````




---

archive/issue_comments_477572.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0b2dada19583fd09fea35de2e894025370aa847a\">0b2dada</a></td><td><code>sage.categories.ordered_sets: New</code></td></tr></table>\n",
    "created_at": "2020-07-19T04:59:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29991#issuecomment-477572",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0b2dada19583fd09fea35de2e894025370aa847a">0b2dada</a></td><td><code>sage.categories.ordered_sets: New</code></td></tr></table>




---

archive/issue_comments_477573.json:
```json
{
    "body": "Changed commit from **[2077684](https://github.com/sagemath/sagetrac-mirror/commit/207768403083c9cdd16a41ab03a79d30a24356a1)** to **[0b2dada](https://github.com/sagemath/sagetrac-mirror/commit/0b2dada19583fd09fea35de2e894025370aa847a)**",
    "created_at": "2020-07-19T04:59:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29991#issuecomment-477573",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[2077684](https://github.com/sagemath/sagetrac-mirror/commit/207768403083c9cdd16a41ab03a79d30a24356a1)** to **[0b2dada](https://github.com/sagemath/sagetrac-mirror/commit/0b2dada19583fd09fea35de2e894025370aa847a)**



---

archive/issue_comments_477574.json:
```json
{
    "body": "Changed commit from **[0b2dada](https://github.com/sagemath/sagetrac-mirror/commit/0b2dada19583fd09fea35de2e894025370aa847a)** to **[343f321](https://github.com/sagemath/sagetrac-mirror/commit/343f321a63b7213defcf96da1570bf5a22c799da)**",
    "created_at": "2020-07-19T05:27:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29991#issuecomment-477574",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[0b2dada](https://github.com/sagemath/sagetrac-mirror/commit/0b2dada19583fd09fea35de2e894025370aa847a)** to **[343f321](https://github.com/sagemath/sagetrac-mirror/commit/343f321a63b7213defcf96da1570bf5a22c799da)**



---

archive/issue_comments_477575.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/343f321a63b7213defcf96da1570bf5a22c799da\">343f321</a></td><td><code>sage/categories/ordered_sets.py: Implement meet, join</code></td></tr></table>\n",
    "created_at": "2020-07-19T05:27:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29991#issuecomment-477575",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:7" align="right">Comment 7</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/343f321a63b7213defcf96da1570bf5a22c799da">343f321</a></td><td><code>sage/categories/ordered_sets.py: Implement meet, join</code></td></tr></table>




---

archive/issue_events_394730.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-13T17:11:30Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29991#event-394730"
}
```



---

archive/issue_events_394731.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-13T17:11:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29991#event-394731"
}
```



---

archive/issue_comments_477576.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nSetting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29991#issuecomment-477576",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:9" align="right">Comment 9</div>

Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_394732.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29991#event-394732"
}
```



---

archive/issue_events_394733.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29991#event-394733"
}
```



---

archive/issue_events_394734.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29991#event-394734"
}
```



---

archive/issue_events_394735.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29991#event-394735"
}
```



---

archive/issue_events_394736.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29991#event-394736"
}
```



---

archive/issue_events_394737.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29991#event-394737"
}
```



---

archive/issue_events_394738.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T00:06:20Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29991#event-394738"
}
```



---

archive/issue_events_394739.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T00:06:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29991#event-394739"
}
```



---

archive/issue_events_394740.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29991#event-394740"
}
```



---

archive/issue_events_394741.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29991#event-394741"
}
```



---

archive/issue_events_394742.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T21:39:40Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29991#event-394742"
}
```



---

archive/issue_events_394743.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T21:39:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29991",
    "milestone_number": null,
    "milestone_title": "sage-9.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29991#event-394743"
}
```
