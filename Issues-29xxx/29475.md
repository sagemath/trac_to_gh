# Issue 29475: Weil polynomials iterator is not handling coefficient constraints correctly

archive/issues_029238.json:
```json
{
    "assignees": [],
    "body": "Currently, when iterating over Weil polynomals, initial coefficient constraints are not being handled correctly in some cases:\n\n```\nsage: from sage.rings.polynomial.weil.weil_polynomials import WeilPolynomials as WP\nsage: P.<x> = QQ[]\nsage: u = x^6 + x^5 + 6*x^4 - 2*x^3 + 66*x^2 + 121*x + 1331\nsage: u.is_weil_polynomial()\nTrue\nsage: l = list(WP(6, 11, 1, [1,1])); u in l # This is good\nTrue\nsage: l = list(WP(6, 11, 1, [1,1,6])); u in l # This is bad\nFalse\n```\n\nCC:  @roed314\n\nKeywords: **Weil polynomials**\n\nBranch/Commit: **[a751aad](https://github.com/sagemath/sagetrac-mirror/commit/a751aada700aa3b9fe8aa7a31c32513e1d35b128)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Kiran Kedlaya**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29475_\n\n",
    "closed_at": "2020-04-12T15:34:18Z",
    "created_at": "2020-04-07T17:05:16Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Weil polynomials iterator is not handling coefficient constraints correctly",
    "type": "issue",
    "updated_at": "2020-04-12T15:34:18Z",
    "url": "https://github.com/sagemath/sage/issues/29475",
    "user": "https://github.com/kedlaya"
}
```
Currently, when iterating over Weil polynomals, initial coefficient constraints are not being handled correctly in some cases:

```
sage: from sage.rings.polynomial.weil.weil_polynomials import WeilPolynomials as WP
sage: P.<x> = QQ[]
sage: u = x^6 + x^5 + 6*x^4 - 2*x^3 + 66*x^2 + 121*x + 1331
sage: u.is_weil_polynomial()
True
sage: l = list(WP(6, 11, 1, [1,1])); u in l # This is good
True
sage: l = list(WP(6, 11, 1, [1,1,6])); u in l # This is bad
False
```

CC:  @roed314

Keywords: **Weil polynomials**

Branch/Commit: **[a751aad](https://github.com/sagemath/sagetrac-mirror/commit/a751aada700aa3b9fe8aa7a31c32513e1d35b128)**

Reviewer: **Frédéric Chapoton**

Author: **Kiran Kedlaya**

_Issue created by migration from https://trac.sagemath.org/ticket/29475_





---

archive/issue_events_387132.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2020-04-07T17:05:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29475",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29475#event-387132"
}
```



---

archive/issue_events_387133.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2020-04-07T17:05:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29475",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29475#event-387133"
}
```



---

archive/issue_events_387134.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2020-04-07T17:05:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29475",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29475#event-387134"
}
```



---

archive/issue_events_387135.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2020-04-07T17:05:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29475",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29475#event-387135"
}
```



---

archive/issue_comments_466211.json:
```json
{
    "body": "Branch: **[u/kedlaya/fix_constraints_for_Weil_polys](https://github.com/sagemath/sagetrac-mirror/tree/u/kedlaya/fix_constraints_for_Weil_polys)**",
    "created_at": "2020-04-07T17:05:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29475",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29475#issuecomment-466211",
    "user": "https://github.com/kedlaya"
}
```

Branch: **[u/kedlaya/fix_constraints_for_Weil_polys](https://github.com/sagemath/sagetrac-mirror/tree/u/kedlaya/fix_constraints_for_Weil_polys)**



---

archive/issue_comments_466212.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nThis turns out to be a trivial bug: when converting from symmetric to asymmetric polynomials in the wrapper, I was missing a power of `q`. This wasn't caught by the previous doctests because the ones with coefficient constraints had `q == 1`. \n\nBesides the one-line fix, I've added a doctest to certify it. I also patched up a couple of formatting issues in the docstring for `WeilPolynomials`.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e8fe54f9e8978a1b3ff27d01db0527e0359f75d2\">e8fe54f</a></td><td><code>Add missing power of q when asymmetrizing Weil polynomials for search</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a751aada700aa3b9fe8aa7a31c32513e1d35b128\">a751aad</a></td><td><code>Fix ticket number</code></td></tr></table>\n",
    "created_at": "2020-04-07T17:08:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29475",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29475#issuecomment-466212",
    "user": "https://github.com/kedlaya"
}
```

<div id="comment:2" align="right">Comment 2</div>

This turns out to be a trivial bug: when converting from symmetric to asymmetric polynomials in the wrapper, I was missing a power of `q`. This wasn't caught by the previous doctests because the ones with coefficient constraints had `q == 1`. 

Besides the one-line fix, I've added a doctest to certify it. I also patched up a couple of formatting issues in the docstring for `WeilPolynomials`.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e8fe54f9e8978a1b3ff27d01db0527e0359f75d2">e8fe54f</a></td><td><code>Add missing power of q when asymmetrizing Weil polynomials for search</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a751aada700aa3b9fe8aa7a31c32513e1d35b128">a751aad</a></td><td><code>Fix ticket number</code></td></tr></table>




---

archive/issue_comments_466213.json:
```json
{
    "body": "Author: **Kiran Kedlaya**",
    "created_at": "2020-04-07T17:08:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29475",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29475#issuecomment-466213",
    "user": "https://github.com/kedlaya"
}
```

Author: **Kiran Kedlaya**



---

archive/issue_comments_466214.json:
```json
{
    "body": "Commit: **[a751aad](https://github.com/sagemath/sagetrac-mirror/commit/a751aada700aa3b9fe8aa7a31c32513e1d35b128)**",
    "created_at": "2020-04-07T17:08:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29475",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29475#issuecomment-466214",
    "user": "https://github.com/kedlaya"
}
```

Commit: **[a751aad](https://github.com/sagemath/sagetrac-mirror/commit/a751aada700aa3b9fe8aa7a31c32513e1d35b128)**



---

archive/issue_events_387136.json:
```json
{
    "actor": "https://github.com/kedlaya",
    "created_at": "2020-04-07T17:08:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29475",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29475#event-387136"
}
```



---

archive/issue_events_387137.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-04-07T18:45:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29475",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29475#event-387137"
}
```



---

archive/issue_events_387138.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-04-07T18:45:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29475",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29475#event-387138"
}
```



---

archive/issue_comments_466215.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2020-04-07T18:45:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29475",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29475#issuecomment-466215",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_466216.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nok, thanks. Did you have a quick glance at #22713 ?",
    "created_at": "2020-04-07T18:45:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29475",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29475#issuecomment-466216",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">Comment 3</div>

ok, thanks. Did you have a quick glance at #22713 ?



---

archive/issue_events_387139.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-04-12T15:34:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29475",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29475#event-387139"
}
```



---

archive/issue_events_387140.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "b351e65d3d5129e3abfa6020afa1f6150bd1feef",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-04-12T15:34:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29475",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29475#event-387140"
}
```



---

archive/issue_comments_466217.json:
```json
{
    "body": "Changed branch from **[u/kedlaya/fix_constraints_for_Weil_polys](https://github.com/sagemath/sagetrac-mirror/tree/u/kedlaya/fix_constraints_for_Weil_polys)** to **[a751aad](https://github.com/sagemath/sagetrac-mirror/commit/a751aada700aa3b9fe8aa7a31c32513e1d35b128)**",
    "created_at": "2020-04-12T15:34:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29475",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29475#issuecomment-466217",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/kedlaya/fix_constraints_for_Weil_polys](https://github.com/sagemath/sagetrac-mirror/tree/u/kedlaya/fix_constraints_for_Weil_polys)** to **[a751aad](https://github.com/sagemath/sagetrac-mirror/commit/a751aada700aa3b9fe8aa7a31c32513e1d35b128)**
