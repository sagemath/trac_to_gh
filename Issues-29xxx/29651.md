...
<a id='comment:1'></a>This fixes the problem for me. The only part I'm not happy about is silencing the warning in sphinxbuild.py, because that message could indicate a problem in some circumstances:
...
<a id='comment:2'></a>Thanks! This does make (1) and (2) work for me (and (3) was already working). Do you want to make the patches into a branch (or should I)? I don't understand the docbuild system, so I don't have a suggestion for an alternative to silencing the warning (and I can't review this fix), but maybe I will before 9.2 comes out.
...
```json
{
    "body": "<a id='comment:3'></a>Let me think more about the change to `sphinxbuild.py`. There should be another way to do this.",
...
<a id='comment:3'></a>Let me think more about the change to `sphinxbuild.py`. There should be another way to do this.
...
<a id='comment:5'></a>This branch partially works for me:
...
```json
{
    "body": "<a id='comment:7'></a>I'm marking this as \"needs review\", but it could perhaps use improvements, as indicated in[comment:5](#comment%3A5).\n\n---\nNew commits:",
    "created_at": "2020-08-29T21:06:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29651",
    "type": "issue_comment",
...
<a id='comment:7'></a>I'm marking this as "needs review", but it could perhaps use improvements, as indicated in[comment:5](#comment%3A5).
...
```json
{
    "body": "<a id='comment:9'></a>Note \"attempted relative import with no known parent package\"",
...
<a id='comment:9'></a>Note "attempted relative import with no known parent package"
...
```json
{
    "body": "<a id='comment:10'></a>I can confirm that `./sage --docbuild file=src/sage/plot/text.py html` works",
...
<a id='comment:10'></a>I can confirm that `./sage --docbuild file=src/sage/plot/text.py html` works
...
```json
{
    "body": "<a id='comment:13'></a>Maybe the module name needs to be set more intelligently if the path starts with `SAGE_SRC`, or something like that.",
...
<a id='comment:13'></a>Maybe the module name needs to be set more intelligently if the path starts with `SAGE_SRC`, or something like that.
...
```json
{
    "body": "<a id='comment:14'></a>Looks like the approach of `SingleFileBuilder`, creating a symlink to the source file, is not compatible with `from .module import ...`",
...
<a id='comment:14'></a>Looks like the approach of `SingleFileBuilder`, creating a symlink to the source file, is not compatible with `from .module import ...`
...
```json
{
    "body": "<a id='comment:17'></a>In any case, this is already an improvement. We can improve it further on a follow-up ticket.",
    "created_at": "2020-08-30T02:44:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29651",
    "type": "issue_comment",
...
<a id='comment:17'></a>In any case, this is already an improvement. We can improve it further on a follow-up ticket.
