...
```json
{
    "body": "<a id='comment:7'></a>\nI looked at one of the build logs on [GitHub](../wikis/GitHub) and it does look like `SAGE_FAT_BINARY=yes` is being passed down, so other than that I'm not aware of any issue that would cause this unless there's a new issue with NTL I haven't seen before.",
