# Issue 29188: Move vertex facet graph to combinatorial polyhedron

archive/issues_028951.json:
```json
{
    "assignees": [],
    "body": "We move the method `vertex_facet_graph` from `Polyhedron_base` to `CombinatorialPolyhedron`.\n\nAlong the way we fix an bug, namely that `vertex_facet_graph(labels=False)` has the edges the wrong way::\n\n```\nsage: P = polytopes.cube()\nsage: P.vertex_facet_graph().is_isomorphic(P.vertex_facet_graph(False))\nFalse\nsage: P.vertex_facet_graph().is_isomorphic(P.vertex_facet_graph(False).reverse())\nTrue\n```\n\nCC:  @jplab @LaisRast\n\nKeywords: **polyhedra, vertex facet graph, combinatorial polyhedron**\n\nBranch/Commit: **[a36a211](https://github.com/sagemath/sagetrac-mirror/commit/a36a21179ccb0e23792f312d75283db5697f2aed)**\n\nReviewer: **Jean-Philippe Labb\u00e9**\n\nAuthor: **Jonathan Kliem**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29188_\n\n",
    "closed_at": "2020-04-23T22:32:54Z",
    "created_at": "2020-02-12T12:20:33Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Move vertex facet graph to combinatorial polyhedron",
    "type": "issue",
    "updated_at": "2020-05-02T02:15:55Z",
    "url": "https://github.com/sagemath/sage/issues/29188",
    "user": "https://github.com/kliem"
}
```
We move the method `vertex_facet_graph` from `Polyhedron_base` to `CombinatorialPolyhedron`.

Along the way we fix an bug, namely that `vertex_facet_graph(labels=False)` has the edges the wrong way::

```
sage: P = polytopes.cube()
sage: P.vertex_facet_graph().is_isomorphic(P.vertex_facet_graph(False))
False
sage: P.vertex_facet_graph().is_isomorphic(P.vertex_facet_graph(False).reverse())
True
```

CC:  @jplab @LaisRast

Keywords: **polyhedra, vertex facet graph, combinatorial polyhedron**

Branch/Commit: **[a36a211](https://github.com/sagemath/sagetrac-mirror/commit/a36a21179ccb0e23792f312d75283db5697f2aed)**

Reviewer: **Jean-Philippe Labbé**

Author: **Jonathan Kliem**

_Issue created by migration from https://trac.sagemath.org/ticket/29188_





---

archive/issue_events_383034.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-02-12T12:20:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29188#event-383034"
}
```



---

archive/issue_events_383035.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-02-12T12:20:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "component: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29188#event-383035"
}
```



---

archive/issue_events_383036.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-02-12T12:20:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29188#event-383036"
}
```



---

archive/issue_events_383037.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-02-12T12:20:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29188#event-383037"
}
```



---

archive/issue_comments_460606.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/630b8a8568d2ed6126f81c2a7f0452afcb690ce4\">630b8a8</a></td><td><code>add vertex_facet_graph to combinatorial polyhedron</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6dbf267cb51e5cc05354ce330427aedcdb43eb30\">6dbf267</a></td><td><code>use combinatorial polyhedron to obtain the vertex facet graph</code></td></tr></table>\n",
    "created_at": "2020-02-12T12:30:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29188#issuecomment-460606",
    "user": "https://github.com/kliem"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/630b8a8568d2ed6126f81c2a7f0452afcb690ce4">630b8a8</a></td><td><code>add vertex_facet_graph to combinatorial polyhedron</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6dbf267cb51e5cc05354ce330427aedcdb43eb30">6dbf267</a></td><td><code>use combinatorial polyhedron to obtain the vertex facet graph</code></td></tr></table>




---

archive/issue_comments_460607.json:
```json
{
    "body": "Branch: **[public/29188](https://github.com/sagemath/sagetrac-mirror/tree/public/29188)**",
    "created_at": "2020-02-12T12:30:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29188#issuecomment-460607",
    "user": "https://github.com/kliem"
}
```

Branch: **[public/29188](https://github.com/sagemath/sagetrac-mirror/tree/public/29188)**



---

archive/issue_events_383038.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-02-12T12:30:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29188#event-383038"
}
```



---

archive/issue_comments_460608.json:
```json
{
    "body": "Commit: **[6dbf267](https://github.com/sagemath/sagetrac-mirror/commit/6dbf267cb51e5cc05354ce330427aedcdb43eb30)**",
    "created_at": "2020-02-12T12:30:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29188#issuecomment-460608",
    "user": "https://github.com/kliem"
}
```

Commit: **[6dbf267](https://github.com/sagemath/sagetrac-mirror/commit/6dbf267cb51e5cc05354ce330427aedcdb43eb30)**



---

archive/issue_comments_460609.json:
```json
{
    "body": "Reviewer: **Jean-Philippe Labb\u00e9**",
    "created_at": "2020-02-14T10:11:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29188#issuecomment-460609",
    "user": "https://github.com/jplab"
}
```

Reviewer: **Jean-Philippe Labbé**



---

archive/issue_comments_460610.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThere is a space missing in the inputs.\n\nThen, I would just swap the two examples to show the default behavior first. \n\nCould you make this sentence more clear:\n\n```\nIf ``names`` is ``True`` but no names are provided,\n```\n\nI would suggest to put `If names is True (the default) but no names...`.",
    "created_at": "2020-02-14T10:11:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29188#issuecomment-460610",
    "user": "https://github.com/jplab"
}
```

<a id='comment:2'>Comment 2:</a>
There is a space missing in the inputs.

Then, I would just swap the two examples to show the default behavior first. 

Could you make this sentence more clear:

```
If ``names`` is ``True`` but no names are provided,
```

I would suggest to put `If names is True (the default) but no names...`.



---

archive/issue_events_383039.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-02-14T10:11:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29188#event-383039"
}
```



---

archive/issue_events_383040.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-02-14T10:11:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29188#event-383040"
}
```



---

archive/issue_comments_460611.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9a62a5261a141febdc3c1a39003d0ff2d48ec2fc\">9a62a52</a></td><td><code>add vertex_facet_graph to combinatorial polyhedron</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1e10ebd76f91afaf0d703f2b800ffddbff34ac24\">1e10ebd</a></td><td><code>use combinatorial polyhedron to obtain the vertex facet graph</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7bae9dc943a7bd12f7e6a03993ea5d319345db2a\">7bae9dc</a></td><td><code>improved doc</code></td></tr></table>\n",
    "created_at": "2020-02-14T11:12:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29188#issuecomment-460611",
    "user": "https://github.com/kliem"
}
```

<a id='comment:3'>Comment 3:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9a62a5261a141febdc3c1a39003d0ff2d48ec2fc">9a62a52</a></td><td><code>add vertex_facet_graph to combinatorial polyhedron</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1e10ebd76f91afaf0d703f2b800ffddbff34ac24">1e10ebd</a></td><td><code>use combinatorial polyhedron to obtain the vertex facet graph</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7bae9dc943a7bd12f7e6a03993ea5d319345db2a">7bae9dc</a></td><td><code>improved doc</code></td></tr></table>




---

archive/issue_events_383041.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-02-14T11:12:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29188#event-383041"
}
```



---

archive/issue_events_383042.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-02-14T11:12:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29188#event-383042"
}
```



---

archive/issue_comments_460612.json:
```json
{
    "body": "Changed branch from **[public/29188](https://github.com/sagemath/sagetrac-mirror/tree/public/29188)** to **[public/29188-reb](https://github.com/sagemath/sagetrac-mirror/tree/public/29188-reb)**",
    "created_at": "2020-02-14T11:12:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29188#issuecomment-460612",
    "user": "https://github.com/kliem"
}
```

Changed branch from **[public/29188](https://github.com/sagemath/sagetrac-mirror/tree/public/29188)** to **[public/29188-reb](https://github.com/sagemath/sagetrac-mirror/tree/public/29188-reb)**



---

archive/issue_comments_460613.json:
```json
{
    "body": "Changed commit from **[6dbf267](https://github.com/sagemath/sagetrac-mirror/commit/6dbf267cb51e5cc05354ce330427aedcdb43eb30)** to **[7bae9dc](https://github.com/sagemath/sagetrac-mirror/commit/7bae9dc943a7bd12f7e6a03993ea5d319345db2a)**",
    "created_at": "2020-02-14T11:12:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29188#issuecomment-460613",
    "user": "https://github.com/kliem"
}
```

Changed commit from **[6dbf267](https://github.com/sagemath/sagetrac-mirror/commit/6dbf267cb51e5cc05354ce330427aedcdb43eb30)** to **[7bae9dc](https://github.com/sagemath/sagetrac-mirror/commit/7bae9dc943a7bd12f7e6a03993ea5d319345db2a)**



---

archive/issue_comments_460614.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d4679c7c6ae42d3aa7e133f029622434bec511fb\">d4679c7</a></td><td><code>removed unused import</code></td></tr></table>\n",
    "created_at": "2020-02-19T08:35:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29188#issuecomment-460614",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d4679c7c6ae42d3aa7e133f029622434bec511fb">d4679c7</a></td><td><code>removed unused import</code></td></tr></table>




---

archive/issue_comments_460615.json:
```json
{
    "body": "Changed commit from **[7bae9dc](https://github.com/sagemath/sagetrac-mirror/commit/7bae9dc943a7bd12f7e6a03993ea5d319345db2a)** to **[d4679c7](https://github.com/sagemath/sagetrac-mirror/commit/d4679c7c6ae42d3aa7e133f029622434bec511fb)**",
    "created_at": "2020-02-19T08:35:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29188#issuecomment-460615",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[7bae9dc](https://github.com/sagemath/sagetrac-mirror/commit/7bae9dc943a7bd12f7e6a03993ea5d319345db2a)** to **[d4679c7](https://github.com/sagemath/sagetrac-mirror/commit/d4679c7c6ae42d3aa7e133f029622434bec511fb)**



---

archive/issue_events_383043.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-02-20T06:51:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29188#event-383043"
}
```



---

archive/issue_events_383044.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-02-20T06:51:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29188#event-383044"
}
```



---

archive/issue_comments_460616.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nI'll just put it back to needs work until this is done.\n\nReplying to [@jplab](#comment%3A2):\n> There is a space missing in the inputs.\n> \n> Then, I would just swap the two examples to show the default behavior first. \n> \n> Could you make this sentence more clear:\n> \n> ```\n> If ``names`` is ``True`` but no names are provided,\n> ```\n> \n> I would suggest to put `If names is True (the default) but no names...`.",
    "created_at": "2020-02-20T06:51:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29188#issuecomment-460616",
    "user": "https://github.com/kliem"
}
```

<a id='comment:5'>Comment 5:</a>
I'll just put it back to needs work until this is done.

Replying to [@jplab](#comment%3A2):
> There is a space missing in the inputs.
> 
> Then, I would just swap the two examples to show the default behavior first. 
> 
> Could you make this sentence more clear:
> 
> ```
> If ``names`` is ``True`` but no names are provided,
> ```
> 
> I would suggest to put `If names is True (the default) but no names...`.



---

archive/issue_comments_460617.json:
```json
{
    "body": "Changed commit from **[d4679c7](https://github.com/sagemath/sagetrac-mirror/commit/d4679c7c6ae42d3aa7e133f029622434bec511fb)** to **[a36a211](https://github.com/sagemath/sagetrac-mirror/commit/a36a21179ccb0e23792f312d75283db5697f2aed)**",
    "created_at": "2020-02-20T08:55:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29188#issuecomment-460617",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[d4679c7](https://github.com/sagemath/sagetrac-mirror/commit/d4679c7c6ae42d3aa7e133f029622434bec511fb)** to **[a36a211](https://github.com/sagemath/sagetrac-mirror/commit/a36a21179ccb0e23792f312d75283db5697f2aed)**



---

archive/issue_comments_460618.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a36a21179ccb0e23792f312d75283db5697f2aed\">a36a211</a></td><td><code>swapped examples</code></td></tr></table>\n",
    "created_at": "2020-02-20T08:55:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29188#issuecomment-460618",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:6'>Comment 6:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a36a21179ccb0e23792f312d75283db5697f2aed">a36a211</a></td><td><code>swapped examples</code></td></tr></table>




---

archive/issue_events_383045.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-02-20T08:55:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29188#event-383045"
}
```



---

archive/issue_events_383046.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-02-20T08:55:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29188#event-383046"
}
```



---

archive/issue_comments_460619.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nBatch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.",
    "created_at": "2020-04-14T19:41:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29188#issuecomment-460619",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:8'>Comment 8:</a>
Batch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.



---

archive/issue_events_383047.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29188#event-383047"
}
```



---

archive/issue_events_383048.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29188#event-383048"
}
```



---

archive/issue_events_383049.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-04-20T09:53:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29188#event-383049"
}
```



---

archive/issue_events_383050.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-04-20T09:53:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29188#event-383050"
}
```



---

archive/issue_comments_460620.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nLGTM.",
    "created_at": "2020-04-20T09:53:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29188#issuecomment-460620",
    "user": "https://github.com/jplab"
}
```

<a id='comment:9'>Comment 9:</a>
LGTM.



---

archive/issue_events_383051.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-04-23T22:32:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29188#event-383051"
}
```



---

archive/issue_events_383052.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "1470ad613938f0c4028d951c8022755a4e5af5a8",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-04-23T22:32:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29188#event-383052"
}
```



---

archive/issue_comments_460621.json:
```json
{
    "body": "Changed branch from **[public/29188-reb](https://github.com/sagemath/sagetrac-mirror/tree/public/29188-reb)** to **[a36a211](https://github.com/sagemath/sagetrac-mirror/commit/a36a21179ccb0e23792f312d75283db5697f2aed)**",
    "created_at": "2020-04-23T22:32:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29188#issuecomment-460621",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/29188-reb](https://github.com/sagemath/sagetrac-mirror/tree/public/29188-reb)** to **[a36a211](https://github.com/sagemath/sagetrac-mirror/commit/a36a21179ccb0e23792f312d75283db5697f2aed)**



---

archive/issue_events_383053.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-05-02T02:15:55Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29188#event-383053"
}
```



---

archive/issue_events_383054.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-05-02T02:15:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29188",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29188#event-383054"
}
```
