# Issue 29128: pass the installation prefix of libffi  to ECL

archive/issues_028891.json:
```json
{
    "body": "at present ECL installation takes the headers of libffi from whereever it can, and first of all from \"standard\" locations. Thus an imcompatible libffi headers (and version 3.3 introduced an apparent incompatibility)  installed system-wide break building of ECL even though `--without-system-ecl` is given.\n\nThis was reported to happen on Debian testing.\n(And an earlier related ticket is #21811). The initial debugging started on this [thread](https://groups.google.com/d/msg/sage-release/eVvjvJ6Iii8/_AMWpclTFAAJ) of sage-devel. \n\nECL's configure has an option `--with-libffi-prefix=path` which hopefully does the job (untested); if so, this would at least allow\nthe build to succeed in such a configuration.\n\nAnother task is to test for the incompatible libffi (well, we can just test the version not to be above 3.2.x for some x).\n\n\n\nCC:  @mkoeppe @isuruf @embray @kiwifb @EmmanuelCharpentier @mezzarobba\n\nReviewer: Travis Scrimshaw\n\nResolution: duplicate\n\nIssue created by migration from https://trac.sagemath.org/ticket/29128\n\n",
    "closed_at": "2020-02-09T19:15:23Z",
    "created_at": "2020-01-31T22:42:24Z",
    "labels": [
        "component: build",
        "critical",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "pass the installation prefix of libffi  to ECL",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/29128",
    "user": "https://github.com/dimpase"
}
```
at present ECL installation takes the headers of libffi from whereever it can, and first of all from "standard" locations. Thus an imcompatible libffi headers (and version 3.3 introduced an apparent incompatibility)  installed system-wide break building of ECL even though `--without-system-ecl` is given.

This was reported to happen on Debian testing.
(And an earlier related ticket is #21811). The initial debugging started on this [thread](https://groups.google.com/d/msg/sage-release/eVvjvJ6Iii8/_AMWpclTFAAJ) of sage-devel. 

ECL's configure has an option `--with-libffi-prefix=path` which hopefully does the job (untested); if so, this would at least allow
the build to succeed in such a configuration.

Another task is to test for the incompatible libffi (well, we can just test the version not to be above 3.2.x for some x).



CC:  @mkoeppe @isuruf @embray @kiwifb @EmmanuelCharpentier @mezzarobba

Reviewer: Travis Scrimshaw

Resolution: duplicate

Issue created by migration from https://trac.sagemath.org/ticket/29128





---

archive/issue_comments_567824.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,12 +1,15 @@\n at present ECL installation takes the headers of libffi from whereever it can, and first of all from \"standard\" locations. Thus an imcompatible libffi headers (and version 3.3 introduced an apparent incompatibility)  installed system-wide break building of ECL even though `--without-system-ecl` is given.\n \n This was reported to happen on Debian testing.\n+(And an earlier related ticket is #21811) \n \n ECL's configure has an option `--with-libffi-prefix=path` which hopefully does the job (untested); if so, this would at least allow\n the build to succeed in such a configuration.\n \n Another task is to test for the incompatible libffi (well, we can just test the version not to be above 3.2.x for some x).\n \n+\n+\n Comment: 1\n \n Issue created by migration from https://trac.sagemath.org/ticket/29128\n``````\n",
    "created_at": "2020-01-31T22:55:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29128#issuecomment-567824",
    "user": "https://github.com/dimpase"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,12 +1,15 @@
 at present ECL installation takes the headers of libffi from whereever it can, and first of all from "standard" locations. Thus an imcompatible libffi headers (and version 3.3 introduced an apparent incompatibility)  installed system-wide break building of ECL even though `--without-system-ecl` is given.
 
 This was reported to happen on Debian testing.
+(And an earlier related ticket is #21811) 
 
 ECL's configure has an option `--with-libffi-prefix=path` which hopefully does the job (untested); if so, this would at least allow
 the build to succeed in such a configuration.
 
 Another task is to test for the incompatible libffi (well, we can just test the version not to be above 3.2.x for some x).
 
+
+
 Comment: 1
 
 Issue created by migration from https://trac.sagemath.org/ticket/29128
``````




---

archive/issue_comments_567825.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,13 @@\n+at present ECL installation takes the headers of libffi from whereever it can, and first of all from \"standard\" locations. Thus an imcompatible libffi headers (and version 3.3 introduced an apparent incompatibility)  installed system-wide break building of ECL even though `--without-system-ecl` is given.\n+\n+This was reported to happen on Debian testing.\n+(And an earlier related ticket is #21811). The initial debugging started on this [thread](https://groups.google.com/d/msg/sage-release/eVvjvJ6Iii8/_AMWpclTFAAJ) of sage-devel. \n+\n+ECL's configure has an option `--with-libffi-prefix=path` which hopefully does the job (untested); if so, this would at least allow\n+the build to succeed in such a configuration.\n+\n+Another task is to test for the incompatible libffi (well, we can just test the version not to be above 3.2.x for some x).\n+\n \n \n Comment: 1\n``````\n",
    "created_at": "2020-01-31T23:46:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29128#issuecomment-567825",
    "user": "https://github.com/EmmanuelCharpentier"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,13 @@
+at present ECL installation takes the headers of libffi from whereever it can, and first of all from "standard" locations. Thus an imcompatible libffi headers (and version 3.3 introduced an apparent incompatibility)  installed system-wide break building of ECL even though `--without-system-ecl` is given.
+
+This was reported to happen on Debian testing.
+(And an earlier related ticket is #21811). The initial debugging started on this [thread](https://groups.google.com/d/msg/sage-release/eVvjvJ6Iii8/_AMWpclTFAAJ) of sage-devel. 
+
+ECL's configure has an option `--with-libffi-prefix=path` which hopefully does the job (untested); if so, this would at least allow
+the build to succeed in such a configuration.
+
+Another task is to test for the incompatible libffi (well, we can just test the version not to be above 3.2.x for some x).
+
 
 
 Comment: 1
``````




---

archive/issue_comments_567826.json:
```json
{
    "body": "<a id='comment:4'></a>In upstream ECL it's addressed in https://gitlab.com/embeddable-common-lisp/ecl/issues/302\n\nSo we hopefully can backport this to out 16.1.2, and support all versions of libffi, old and current.",
    "created_at": "2020-02-03T17:12:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29128#issuecomment-567826",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:4'></a>In upstream ECL it's addressed in https://gitlab.com/embeddable-common-lisp/ecl/issues/302

So we hopefully can backport this to out 16.1.2, and support all versions of libffi, old and current.



---

archive/issue_comments_567827.json:
```json
{
    "body": "<a id='comment:5'></a>the ECL's option appears not be working, anyway, on #21811 there is a better solution.\nWe'll fix the issue on #21811",
    "created_at": "2020-02-03T18:26:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29128#issuecomment-567827",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:5'></a>the ECL's option appears not be working, anyway, on #21811 there is a better solution.
We'll fix the issue on #21811



---

archive/issue_events_073878.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-02-03T18:26:18Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/29128",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29128#event-73878"
}
```



---

archive/issue_comments_567828.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2020-02-03T18:26:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29128#issuecomment-567828",
    "user": "https://github.com/dimpase"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_567829.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2020-02-08T21:48:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29128#issuecomment-567829",
    "user": "https://github.com/tscrim"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_567830.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Travis Scrimshaw\"",
    "created_at": "2020-02-08T21:48:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29128#issuecomment-567830",
    "user": "https://github.com/tscrim"
}
```

Changing reviewer from "" to "Travis Scrimshaw"



---

archive/issue_comments_567831.json:
```json
{
    "body": "<a id='comment:6'></a>Since #21811 is a better solution and positively reviewed, I agree with closing this.",
    "created_at": "2020-02-08T21:48:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29128#issuecomment-567831",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:6'></a>Since #21811 is a better solution and positively reviewed, I agree with closing this.



---

archive/issue_comments_567832.json:
```json
{
    "body": "Resolution: duplicate",
    "created_at": "2020-02-09T19:15:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29128#issuecomment-567832",
    "user": "https://github.com/fchapoton"
}
```

Resolution: duplicate



---

archive/issue_events_073879.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-02-09T19:15:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/29128",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29128#event-73879"
}
```
