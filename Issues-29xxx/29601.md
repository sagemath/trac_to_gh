# Issue 29601: Fix singular configure patch

archive/issues_029364.json:
```json
{
    "assignees": [],
    "body": "Follow-up from #29335:\n\nThe patch was incomplete and this ticket fixes it\n\nCC:  @mkoeppe\n\nBranch/Commit: **[840b1d3](https://github.com/sagemath/sagetrac-mirror/commit/840b1d320db1bb59a2c363884206d098c1aee870)**\n\nReviewer: **Matthias Koeppe**\n\nAuthor: **Isuru Fernando**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29601_\n\n",
    "closed_at": "2020-05-04T06:41:14Z",
    "created_at": "2020-04-27T06:32:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix singular configure patch",
    "type": "issue",
    "updated_at": "2020-05-04T06:41:14Z",
    "url": "https://github.com/sagemath/sage/issues/29601",
    "user": "https://github.com/isuruf"
}
```
Follow-up from #29335:

The patch was incomplete and this ticket fixes it

CC:  @mkoeppe

Branch/Commit: **[840b1d3](https://github.com/sagemath/sagetrac-mirror/commit/840b1d320db1bb59a2c363884206d098c1aee870)**

Reviewer: **Matthias Koeppe**

Author: **Isuru Fernando**

_Issue created by migration from https://trac.sagemath.org/ticket/29601_





---

archive/issue_events_388941.json:
```json
{
    "actor": "https://github.com/isuruf",
    "created_at": "2020-04-27T06:32:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29601",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29601#event-388941"
}
```



---

archive/issue_events_388942.json:
```json
{
    "actor": "https://github.com/isuruf",
    "created_at": "2020-04-27T06:32:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29601",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29601#event-388942"
}
```



---

archive/issue_events_388943.json:
```json
{
    "actor": "https://github.com/isuruf",
    "created_at": "2020-04-27T06:32:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29601",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29601#event-388943"
}
```



---

archive/issue_events_388944.json:
```json
{
    "actor": "https://github.com/isuruf",
    "created_at": "2020-04-27T06:32:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29601",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29601#event-388944"
}
```



---

archive/issue_events_388945.json:
```json
{
    "actor": "https://github.com/isuruf",
    "created_at": "2020-04-27T06:32:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29601",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29601#event-388945"
}
```



---

archive/issue_comments_468819.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2020-04-27T14:57:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29601#issuecomment-468819",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_comments_468820.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n+Follow-up from #29335:\n+\n The patch was incomplete and this ticket fixes it\n``````\n",
    "created_at": "2020-04-27T14:57:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29601#issuecomment-468820",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
+Follow-up from #29335:
+
 The patch was incomplete and this ticket fixes it
``````




---

archive/issue_comments_468821.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nI have some trouble with singular on my test branch that includes this ticket -  on homebrew \u200bhttps://github.com/mkoeppe/sage/runs/624792671",
    "created_at": "2020-04-28T17:31:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29601#issuecomment-468821",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:3" align="right">Comment 3</div>

I have some trouble with singular on my test branch that includes this ticket -  on homebrew ​https://github.com/mkoeppe/sage/runs/624792671



---

archive/issue_events_388946.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-29T04:21:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29601",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29601#event-388946"
}
```



---

archive/issue_events_388947.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-29T04:21:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29601",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29601#event-388947"
}
```



---

archive/issue_events_388948.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-05-01T05:51:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29601",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29601#event-388948"
}
```



---

archive/issue_events_388949.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-05-01T05:51:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29601",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29601#event-388949"
}
```



---

archive/issue_comments_468822.json:
```json
{
    "body": "Changed branch from **[u/isuruf/singular_configure](https://github.com/sagemath/sagetrac-mirror/tree/u/isuruf/singular_configure)** to **[840b1d3](https://github.com/sagemath/sagetrac-mirror/commit/840b1d320db1bb59a2c363884206d098c1aee870)**",
    "created_at": "2020-05-04T06:41:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29601#issuecomment-468822",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/isuruf/singular_configure](https://github.com/sagemath/sagetrac-mirror/tree/u/isuruf/singular_configure)** to **[840b1d3](https://github.com/sagemath/sagetrac-mirror/commit/840b1d320db1bb59a2c363884206d098c1aee870)**



---

archive/issue_events_388950.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-05-04T06:41:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29601",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29601#event-388950"
}
```



---

archive/issue_events_388951.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "d92d65d89f2982f9159513041ee856c50aed5ed4",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-05-04T06:41:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29601",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29601#event-388951"
}
```
