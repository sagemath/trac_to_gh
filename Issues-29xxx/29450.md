...
```json
{
    "body": "I finally remembered that I needed to do this. The last month has been somewhat of a tough ride. I decided against testing against `.nodoctest` as well as `nodoctest` and `nodoctest.py` in control.py. Since, hopefully we'll do everything on the same release and `.nodoctest` is not installed, there shouldn't be any issues.\n\n---\nNew commits:",
    "created_at": "2020-04-02T02:10:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29450",
    "type": "issue_comment",
...
```json
{
    "body": "Which glob formula are you referring to?",
...
```json
{
    "body": "The one for `package_data` in `setup.py`. The alternative is to try to glob dotfiles as well. I don't think that's worth it.",
