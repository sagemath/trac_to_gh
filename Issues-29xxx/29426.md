# Issue 29426: Make dependencies of optional packages more detailed, no longer depend on all standard packages

archive/issues_029189.json:
```json
{
    "assignees": [],
    "body": "#28095 (Add `--enable-OPTIONALSPKG` options to `configure`) made it possible to build Sage and a number of optional packages in one pass.\n\nTo improve opportunities for build parallelization, we should make dependencies of optional packages more detailed and no longer depend on all standard packages.\n\nThis will also make it faster to test builds from scratch to a specific optional package.\n\n\nCC:  @embray @dimpase @jhpalmieri\n\nBranch: **[a5818c0](https://github.com/sagemath/sagetrac-mirror/commit/a5818c08ff57d35be19ed2429321bd90a965f1c6)**\n\nReviewer: **Jonathan Kliem**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29426_\n\n",
    "closed_at": "2020-04-23T22:32:45Z",
    "created_at": "2020-03-29T17:33:24Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make dependencies of optional packages more detailed, no longer depend on all standard packages",
    "type": "issue",
    "updated_at": "2020-08-31T16:12:33Z",
    "url": "https://github.com/sagemath/sage/issues/29426",
    "user": "https://github.com/mkoeppe"
}
```
#28095 (Add `--enable-OPTIONALSPKG` options to `configure`) made it possible to build Sage and a number of optional packages in one pass.

To improve opportunities for build parallelization, we should make dependencies of optional packages more detailed and no longer depend on all standard packages.

This will also make it faster to test builds from scratch to a specific optional package.


CC:  @embray @dimpase @jhpalmieri

Branch: **[a5818c0](https://github.com/sagemath/sagetrac-mirror/commit/a5818c08ff57d35be19ed2429321bd90a965f1c6)**

Reviewer: **Jonathan Kliem**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/29426_





---

archive/issue_events_386457.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-29T17:33:24Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29426#event-386457"
}
```



---

archive/issue_events_386458.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-29T17:33:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29426#event-386458"
}
```



---

archive/issue_events_386459.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-29T17:33:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29426#event-386459"
}
```



---

archive/issue_events_386460.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-29T17:33:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29426#event-386460"
}
```



---

archive/issue_comments_465352.json:
```json
{
    "body": "Branch: **[u/mkoeppe/make_dependencies_of_optional_packages_more_detailed__no_longer_depend_on_all_standard_packages](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/make_dependencies_of_optional_packages_more_detailed__no_longer_depend_on_all_standard_packages)**",
    "created_at": "2020-04-08T22:54:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29426#issuecomment-465352",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/make_dependencies_of_optional_packages_more_detailed__no_longer_depend_on_all_standard_packages](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/make_dependencies_of_optional_packages_more_detailed__no_longer_depend_on_all_standard_packages)**



---

archive/issue_comments_465353.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\npushing these forward to 9.2",
    "created_at": "2020-04-14T06:33:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29426#issuecomment-465353",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

pushing these forward to 9.2



---

archive/issue_events_386461.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T06:33:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29426#event-386461"
}
```



---

archive/issue_events_386462.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T06:33:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29426#event-386462"
}
```



---

archive/issue_comments_465354.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nTests ran at https://github.com/mkoeppe/sage/actions/runs/77739151\n\nSeveral optional packages that run their tests with the Sage doctester depend on `ipywidgets` when `SAGE_CHECK=yes`.\nSee for example https://github.com/mkoeppe/sage/runs/584777416?check_suite_focus=true: \n`sage_numerical_backends_coin`, `p_group_cohomology`\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ebb07d1a8b08051bfcecacfec5731a97f5178ace\">ebb07d1</a></td><td><code>Optional/experimental packages: Remove order-only dependencies on all standard packages</code></td></tr></table>\n",
    "created_at": "2020-04-15T05:34:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29426#issuecomment-465354",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:3" align="right">Comment 3</div>

Tests ran at https://github.com/mkoeppe/sage/actions/runs/77739151

Several optional packages that run their tests with the Sage doctester depend on `ipywidgets` when `SAGE_CHECK=yes`.
See for example https://github.com/mkoeppe/sage/runs/584777416?check_suite_focus=true: 
`sage_numerical_backends_coin`, `p_group_cohomology`

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ebb07d1a8b08051bfcecacfec5731a97f5178ace">ebb07d1</a></td><td><code>Optional/experimental packages: Remove order-only dependencies on all standard packages</code></td></tr></table>




---

archive/issue_comments_465355.json:
```json
{
    "body": "Commit: **[ebb07d1](https://github.com/sagemath/sagetrac-mirror/commit/ebb07d1a8b08051bfcecacfec5731a97f5178ace)**",
    "created_at": "2020-04-15T05:34:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29426#issuecomment-465355",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[ebb07d1](https://github.com/sagemath/sagetrac-mirror/commit/ebb07d1a8b08051bfcecacfec5731a97f5178ace)**



---

archive/issue_comments_465356.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d4773b899080fff9b5ab601f7ee26ce60f681541\">d4773b8</a></td><td><code>build/pkgs: Add ipywidgets as a dependency to packages using the sage doctester</code></td></tr></table>\n",
    "created_at": "2020-04-17T07:29:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29426#issuecomment-465356",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d4773b899080fff9b5ab601f7ee26ce60f681541">d4773b8</a></td><td><code>build/pkgs: Add ipywidgets as a dependency to packages using the sage doctester</code></td></tr></table>




---

archive/issue_comments_465357.json:
```json
{
    "body": "Changed commit from **[ebb07d1](https://github.com/sagemath/sagetrac-mirror/commit/ebb07d1a8b08051bfcecacfec5731a97f5178ace)** to **[d4773b8](https://github.com/sagemath/sagetrac-mirror/commit/d4773b899080fff9b5ab601f7ee26ce60f681541)**",
    "created_at": "2020-04-17T07:29:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29426#issuecomment-465357",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[ebb07d1](https://github.com/sagemath/sagetrac-mirror/commit/ebb07d1a8b08051bfcecacfec5731a97f5178ace)** to **[d4773b8](https://github.com/sagemath/sagetrac-mirror/commit/d4773b899080fff9b5ab601f7ee26ce60f681541)**



---

archive/issue_comments_465358.json:
```json
{
    "body": "Changed commit from **[d4773b8](https://github.com/sagemath/sagetrac-mirror/commit/d4773b899080fff9b5ab601f7ee26ce60f681541)** to **[a5818c0](https://github.com/sagemath/sagetrac-mirror/commit/a5818c08ff57d35be19ed2429321bd90a965f1c6)**",
    "created_at": "2020-04-17T20:11:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29426#issuecomment-465358",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[d4773b8](https://github.com/sagemath/sagetrac-mirror/commit/d4773b899080fff9b5ab601f7ee26ce60f681541)** to **[a5818c0](https://github.com/sagemath/sagetrac-mirror/commit/a5818c08ff57d35be19ed2429321bd90a965f1c6)**



---

archive/issue_comments_465359.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6b20b84fa42303d8d96373f6e8f75546b97a079c\">6b20b84</a></td><td><code>trac 26018: embray's work rebased to 9.1.rc0</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8e6f6945c019137eaec2809f32b5105aa86d4483\">8e6f694</a></td><td><code>Optional/experimental packages: Remove order-only dependencies on all standard packages</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a5818c08ff57d35be19ed2429321bd90a965f1c6\">a5818c0</a></td><td><code>build/pkgs: Add ipywidgets as a dependency to packages using the sage doctester</code></td></tr></table>\n",
    "created_at": "2020-04-17T20:11:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29426#issuecomment-465359",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:5" align="right">Comment 5</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6b20b84fa42303d8d96373f6e8f75546b97a079c">6b20b84</a></td><td><code>trac 26018: embray's work rebased to 9.1.rc0</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8e6f6945c019137eaec2809f32b5105aa86d4483">8e6f694</a></td><td><code>Optional/experimental packages: Remove order-only dependencies on all standard packages</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a5818c08ff57d35be19ed2429321bd90a965f1c6">a5818c0</a></td><td><code>build/pkgs: Add ipywidgets as a dependency to packages using the sage doctester</code></td></tr></table>




---

archive/issue_comments_465360.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nRebased on top of #26018",
    "created_at": "2020-04-17T20:12:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29426#issuecomment-465360",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:6" align="right">Comment 6</div>

Rebased on top of #26018



---

archive/issue_comments_465361.json:
```json
{
    "body": "Dependencies: **#26018**",
    "created_at": "2020-04-17T20:12:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29426#issuecomment-465361",
    "user": "https://github.com/mkoeppe"
}
```

Dependencies: **#26018**



---

archive/issue_events_386463.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-18T19:00:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29426#event-386463"
}
```



---

archive/issue_events_386464.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-18T19:00:27Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29426#event-386464"
}
```



---

archive/issue_events_386465.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-18T19:00:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29426#event-386465"
}
```



---

archive/issue_comments_465362.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2020-04-18T19:00:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29426#issuecomment-465362",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_comments_465363.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nNew tests are running at https://github.com/mkoeppe/sage/actions/runs/81669565 (with #29341)",
    "created_at": "2020-04-18T19:21:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29426#issuecomment-465363",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:8" align="right">Comment 8</div>

New tests are running at https://github.com/mkoeppe/sage/actions/runs/81669565 (with #29341)



---

archive/issue_comments_465364.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nThis seems to work well. Needs review",
    "created_at": "2020-04-19T19:24:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29426#issuecomment-465364",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:9" align="right">Comment 9</div>

This seems to work well. Needs review



---

archive/issue_comments_465365.json:
```json
{
    "body": "Reviewer: **Jonathan Kliem**",
    "created_at": "2020-04-21T07:48:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29426#issuecomment-465365",
    "user": "https://github.com/kliem"
}
```

Reviewer: **Jonathan Kliem**



---

archive/issue_comments_465366.json:
```json
{
    "body": "Changed dependencies from **#26018** to none",
    "created_at": "2020-04-21T07:48:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29426#issuecomment-465366",
    "user": "https://github.com/kliem"
}
```

Changed dependencies from **#26018** to none



---

archive/issue_events_386466.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-04-21T07:48:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29426#event-386466"
}
```



---

archive/issue_events_386467.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-04-21T07:48:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29426#event-386467"
}
```



---

archive/issue_comments_465367.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nLGTM.",
    "created_at": "2020-04-21T07:48:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29426#issuecomment-465367",
    "user": "https://github.com/kliem"
}
```

<div id="comment:10" align="right">Comment 10</div>

LGTM.



---

archive/issue_comments_465368.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nThanks!",
    "created_at": "2020-04-21T16:11:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29426#issuecomment-465368",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:11" align="right">Comment 11</div>

Thanks!



---

archive/issue_events_386468.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-04-23T22:32:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29426#event-386468"
}
```



---

archive/issue_events_386469.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "29a41a6aa84817ac5587e04923da10745e6e8809",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-04-23T22:32:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29426#event-386469"
}
```



---

archive/issue_comments_465369.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/make_dependencies_of_optional_packages_more_detailed__no_longer_depend_on_all_standard_packages](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/make_dependencies_of_optional_packages_more_detailed__no_longer_depend_on_all_standard_packages)** to **[a5818c0](https://github.com/sagemath/sagetrac-mirror/commit/a5818c08ff57d35be19ed2429321bd90a965f1c6)**",
    "created_at": "2020-04-23T22:32:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29426#issuecomment-465369",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/make_dependencies_of_optional_packages_more_detailed__no_longer_depend_on_all_standard_packages](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/make_dependencies_of_optional_packages_more_detailed__no_longer_depend_on_all_standard_packages)** to **[a5818c0](https://github.com/sagemath/sagetrac-mirror/commit/a5818c08ff57d35be19ed2429321bd90a965f1c6)**



---

archive/issue_comments_465370.json:
```json
{
    "body": "Changed commit from **[a5818c0](https://github.com/sagemath/sagetrac-mirror/commit/a5818c08ff57d35be19ed2429321bd90a965f1c6)** to none",
    "created_at": "2020-08-10T02:41:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29426#issuecomment-465370",
    "user": "https://github.com/mkoeppe"
}
```

Changed commit from **[a5818c0](https://github.com/sagemath/sagetrac-mirror/commit/a5818c08ff57d35be19ed2429321bd90a965f1c6)** to none



---

archive/issue_comments_465371.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nFollow-up: #30324",
    "created_at": "2020-08-10T02:41:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29426#issuecomment-465371",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:13" align="right">Comment 13</div>

Follow-up: #30324



---

archive/issue_comments_465372.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\n+1 thanks",
    "created_at": "2020-08-31T16:12:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29426",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29426#issuecomment-465372",
    "user": "https://github.com/embray"
}
```

<div id="comment:14" align="right">Comment 14</div>

+1 thanks
