# Issue 29176: Bug in Voronoi Diagram

archive/issues_028939.json:
```json
{
    "assignees": [],
    "body": "We fix a bug that was exposed computing an inexact Voronoi Diagram (see below).\n\n`cdd` already computes the incidence matrix, we expose it in this ticket.\n\nWhen deciding whether or not a vertex lies on a hyperplane for inexact polyhedra, we allow numerical noise up to absolute value of `1e-6`. This might be a bad choice depending on the size of the polyhedron as witnessed in the following example:\n\n```\n\nsage: e = [[11582947.657000002, 5374.38, 4177.06, 1.0], [11562795.9322, 5373.62, 4168.38, 1.0]]\nsage: p = Polyhedron(ieqs=e); p\nA 3-dimensional polyhedron in RDF^3 defined as the convex hull of 1 vertex, 2 rays, 1 line\nsage: p.incidence_matrix()\n[0 0]\n[0 0]\n[0 1]\n[0 0]\n```\n\nSetting the cache of incidence matrix to the matrix that `cdd` already computed for us produces the correct output:\n\n```\nsage: p.incidence_matrix()\n[1 1]\n[1 0]\n[0 1]\n[1 1]\n```\n\nThis fixes the error reported here:\n\nhttps://ask.sagemath.org/question/49749/voronoidiagram-returns-empty-regions/\n\nThe following should give two non-empty regions\n\n```\nsage: P = [[-2687.19, -2088.53], [-2686.81, -2084.19]]\n....: V = VoronoiDiagram(P)\n....: R = V.regions()\n....: \nsage: R\n{P(-2687.19000000000, -2088.53000000000): The empty polyhedron in RDF^0,\n P(-2686.81000000000, -2084.19000000000): A 1-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex and 1 ray\n```\n\nWith this ticket we obtain:\n\n```\nsage: R\n{P(-2687.19000000000, -2088.53000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line,\n P(-2686.81000000000, -2084.19000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line}\n```\n\nCC:  @jplab @LaisRast\n\nKeywords: **cdd, incidence matrix**\n\nBranch/Commit: **[fb2d66a](https://github.com/sagemath/sagetrac-mirror/commit/fb2d66a0e71f164e9bef37922c8d544b9b270c9c)**\n\nReviewer: **Jean-Philippe Labb\u00e9**\n\nAuthor: **Jonathan Kliem**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29176_\n\n",
    "closed_at": "2020-05-02T21:58:26Z",
    "created_at": "2020-02-10T12:53:15Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Bug in Voronoi Diagram",
    "type": "issue",
    "updated_at": "2020-05-02T21:58:26Z",
    "url": "https://github.com/sagemath/sage/issues/29176",
    "user": "https://github.com/jplab"
}
```
We fix a bug that was exposed computing an inexact Voronoi Diagram (see below).

`cdd` already computes the incidence matrix, we expose it in this ticket.

When deciding whether or not a vertex lies on a hyperplane for inexact polyhedra, we allow numerical noise up to absolute value of `1e-6`. This might be a bad choice depending on the size of the polyhedron as witnessed in the following example:

```

sage: e = [[11582947.657000002, 5374.38, 4177.06, 1.0], [11562795.9322, 5373.62, 4168.38, 1.0]]
sage: p = Polyhedron(ieqs=e); p
A 3-dimensional polyhedron in RDF^3 defined as the convex hull of 1 vertex, 2 rays, 1 line
sage: p.incidence_matrix()
[0 0]
[0 0]
[0 1]
[0 0]
```

Setting the cache of incidence matrix to the matrix that `cdd` already computed for us produces the correct output:

```
sage: p.incidence_matrix()
[1 1]
[1 0]
[0 1]
[1 1]
```

This fixes the error reported here:

https://ask.sagemath.org/question/49749/voronoidiagram-returns-empty-regions/

The following should give two non-empty regions

```
sage: P = [[-2687.19, -2088.53], [-2686.81, -2084.19]]
....: V = VoronoiDiagram(P)
....: R = V.regions()
....: 
sage: R
{P(-2687.19000000000, -2088.53000000000): The empty polyhedron in RDF^0,
 P(-2686.81000000000, -2084.19000000000): A 1-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex and 1 ray
```

With this ticket we obtain:

```
sage: R
{P(-2687.19000000000, -2088.53000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line,
 P(-2686.81000000000, -2084.19000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line}
```

CC:  @jplab @LaisRast

Keywords: **cdd, incidence matrix**

Branch/Commit: **[fb2d66a](https://github.com/sagemath/sagetrac-mirror/commit/fb2d66a0e71f164e9bef37922c8d544b9b270c9c)**

Reviewer: **Jean-Philippe Labbé**

Author: **Jonathan Kliem**

_Issue created by migration from https://trac.sagemath.org/ticket/29176_





---

archive/issue_events_369552.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-02-10T12:53:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-369552"
}
```



---

archive/issue_events_369553.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-02-10T12:53:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "component: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-369553"
}
```



---

archive/issue_events_369554.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-02-10T12:53:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-369554"
}
```



---

archive/issue_events_369555.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-02-10T12:53:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-369555"
}
```



---

archive/issue_comments_460479.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nThis can and should be solved by fetching the incidences from cdd.\n\nI will try to do so. This might get rid of a lot of numerical inconsistencies.",
    "created_at": "2020-03-19T11:12:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460479",
    "user": "https://github.com/kliem"
}
```

<a id='comment:1'>Comment 1:</a>
This can and should be solved by fetching the incidences from cdd.

I will try to do so. This might get rid of a lot of numerical inconsistencies.



---

archive/issue_comments_460480.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c03aee4b4445c7954c667adcf233fa8ba1612d62\">c03aee4</a></td><td><code>expose incidence matrix computed by backend cdd</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/33b59112324e37dcbdab7706047e47cf9dbf5568\">33b5911</a></td><td><code>add doctests that 29176 is fixed</code></td></tr></table>\n",
    "created_at": "2020-03-19T14:55:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460480",
    "user": "https://github.com/kliem"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c03aee4b4445c7954c667adcf233fa8ba1612d62">c03aee4</a></td><td><code>expose incidence matrix computed by backend cdd</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/33b59112324e37dcbdab7706047e47cf9dbf5568">33b5911</a></td><td><code>add doctests that 29176 is fixed</code></td></tr></table>




---

archive/issue_comments_460481.json:
```json
{
    "body": "Branch: **[public/29176](https://github.com/sagemath/sagetrac-mirror/tree/public/29176)**",
    "created_at": "2020-03-19T14:55:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460481",
    "user": "https://github.com/kliem"
}
```

Branch: **[public/29176](https://github.com/sagemath/sagetrac-mirror/tree/public/29176)**



---

archive/issue_comments_460482.json:
```json
{
    "body": "Changed keywords from **** to **cdd, incidence matrix**",
    "created_at": "2020-03-19T14:55:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460482",
    "user": "https://github.com/kliem"
}
```

Changed keywords from **** to **cdd, incidence matrix**



---

archive/issue_comments_460483.json:
```json
{
    "body": "Commit: **[33b5911](https://github.com/sagemath/sagetrac-mirror/commit/33b59112324e37dcbdab7706047e47cf9dbf5568)**",
    "created_at": "2020-03-19T14:55:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460483",
    "user": "https://github.com/kliem"
}
```

Commit: **[33b5911](https://github.com/sagemath/sagetrac-mirror/commit/33b59112324e37dcbdab7706047e47cf9dbf5568)**



---

archive/issue_comments_460484.json:
```json
{
    "body": "Author: **Jonathan Kliem**",
    "created_at": "2020-03-19T14:55:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460484",
    "user": "https://github.com/kliem"
}
```

Author: **Jonathan Kliem**



---

archive/issue_events_369556.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-03-19T14:55:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-369556"
}
```



---

archive/issue_comments_460485.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,36 @@\n-The following should give two non-empty regions:\n+We fix a bug that was exposed computing an inexact Voronoi Diagram (see below).\n+\n+`cdd` already computes the incidence matrix, we expose it in this ticket.\n+\n+When deciding whether or not a vertex lies on a hyperplane for inexact polyhedra, we allow numerical noise up to absolute value of `1e-6`. This might be a bad choice depending on the size of the polyhedron as witnessed in the following example:\n+\n+```\n+\n+sage: e = [[11582947.657000002, 5374.38, 4177.06, 1.0], [11562795.9322, 5373.62, 4168.38, 1.0]]\n+sage: p = Polyhedron(ieqs=e); p\n+A 3-dimensional polyhedron in RDF^3 defined as the convex hull of 1 vertex, 2 rays, 1 line\n+sage: p.incidence_matrix()\n+[0 0]\n+[0 0]\n+[0 1]\n+[0 0]\n+```\n+\n+Setting the cache of incidence matrix to the matrix that `cdd` already computed for us produces the correct output:\n+\n+```\n+sage: p.incidence_matrix()\n+[1 1]\n+[1 0]\n+[0 1]\n+[1 1]\n+```\n+\n+This fixes the error reported here:\n+\n+https://ask.sagemath.org/question/49749/voronoidiagram-returns-empty-regions/\n+\n+The following should give two non-empty regions\n \n ```\n sage: P = [[-2687.19, -2088.53], [-2686.81, -2084.19]]\n@@ -10,6 +42,10 @@\n  P(-2686.81000000000, -2084.19000000000): A 1-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex and 1 ray\n ```\n \n-This way reported here:\n+With this ticket we obtain:\n \n-https://ask.sagemath.org/question/49749/voronoidiagram-returns-empty-regions/\n+```\n+sage: R\n+{P(-2687.19000000000, -2088.53000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line,\n+ P(-2686.81000000000, -2084.19000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line}\n+```\n``````\n",
    "created_at": "2020-03-19T14:55:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460485",
    "user": "https://github.com/kliem"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,36 @@
-The following should give two non-empty regions:
+We fix a bug that was exposed computing an inexact Voronoi Diagram (see below).
+
+`cdd` already computes the incidence matrix, we expose it in this ticket.
+
+When deciding whether or not a vertex lies on a hyperplane for inexact polyhedra, we allow numerical noise up to absolute value of `1e-6`. This might be a bad choice depending on the size of the polyhedron as witnessed in the following example:
+
+```
+
+sage: e = [[11582947.657000002, 5374.38, 4177.06, 1.0], [11562795.9322, 5373.62, 4168.38, 1.0]]
+sage: p = Polyhedron(ieqs=e); p
+A 3-dimensional polyhedron in RDF^3 defined as the convex hull of 1 vertex, 2 rays, 1 line
+sage: p.incidence_matrix()
+[0 0]
+[0 0]
+[0 1]
+[0 0]
+```
+
+Setting the cache of incidence matrix to the matrix that `cdd` already computed for us produces the correct output:
+
+```
+sage: p.incidence_matrix()
+[1 1]
+[1 0]
+[0 1]
+[1 1]
+```
+
+This fixes the error reported here:
+
+https://ask.sagemath.org/question/49749/voronoidiagram-returns-empty-regions/
+
+The following should give two non-empty regions
 
 ```
 sage: P = [[-2687.19, -2088.53], [-2686.81, -2084.19]]
@@ -10,6 +42,10 @@
  P(-2686.81000000000, -2084.19000000000): A 1-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex and 1 ray
 ```
 
-This way reported here:
+With this ticket we obtain:
 
-https://ask.sagemath.org/question/49749/voronoidiagram-returns-empty-regions/
+```
+sage: R
+{P(-2687.19000000000, -2088.53000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line,
+ P(-2686.81000000000, -2084.19000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line}
+```
``````




---

archive/issue_comments_460486.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThanks! Looks good to me.",
    "created_at": "2020-04-19T13:33:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460486",
    "user": "https://github.com/jplab"
}
```

<a id='comment:3'>Comment 3:</a>
Thanks! Looks good to me.



---

archive/issue_events_369557.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-04-19T13:33:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-369557"
}
```



---

archive/issue_events_369558.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-04-19T13:33:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-369558"
}
```



---

archive/issue_comments_460487.json:
```json
{
    "body": "Reviewer: **Jean-Philippe Labb\u00e9**",
    "created_at": "2020-04-19T13:33:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460487",
    "user": "https://github.com/jplab"
}
```

Reviewer: **Jean-Philippe Labbé**



---

archive/issue_comments_460488.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nThank you.",
    "created_at": "2020-04-19T13:34:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460488",
    "user": "https://github.com/kliem"
}
```

<a id='comment:4'>Comment 4:</a>
Thank you.



---

archive/issue_events_369559.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-04-21T22:42:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-369559"
}
```



---

archive/issue_events_369560.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-04-21T22:42:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-369560"
}
```



---

archive/issue_comments_460489.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nOn Python 2 Debian 9 x86_64:\n\n```\n**********************************************************************\nFile \"src/sage/geometry/polyhedron/backend_cdd.py\", line 231, in sage.geometry.polyhedron.backend_cdd.Polyhedron_cdd._init_from_cdd_output\nFailed example:\n    R\nExpected:\n    {P(-2686.81000000000, -2084.19000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line,\n     P(-2687.19000000000, -2088.53000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line}\nGot:\n    {P(-2687.19000000000, -2088.53000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line,\n     P(-2686.81000000000, -2084.19000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line}\n**********************************************************************\n1 item had failures:\n```",
    "created_at": "2020-04-21T22:42:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460489",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:5'>Comment 5:</a>
On Python 2 Debian 9 x86_64:

```
**********************************************************************
File "src/sage/geometry/polyhedron/backend_cdd.py", line 231, in sage.geometry.polyhedron.backend_cdd.Polyhedron_cdd._init_from_cdd_output
Failed example:
    R
Expected:
    {P(-2686.81000000000, -2084.19000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line,
     P(-2687.19000000000, -2088.53000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line}
Got:
    {P(-2687.19000000000, -2088.53000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line,
     P(-2686.81000000000, -2084.19000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line}
**********************************************************************
1 item had failures:
```



---

archive/issue_comments_460490.json:
```json
{
    "body": "Changed commit from **[33b5911](https://github.com/sagemath/sagetrac-mirror/commit/33b59112324e37dcbdab7706047e47cf9dbf5568)** to **[9352525](https://github.com/sagemath/sagetrac-mirror/commit/9352525b50851921f000335615e75e0f1bdd5f2e)**",
    "created_at": "2020-04-22T05:40:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460490",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[33b5911](https://github.com/sagemath/sagetrac-mirror/commit/33b59112324e37dcbdab7706047e47cf9dbf5568)** to **[9352525](https://github.com/sagemath/sagetrac-mirror/commit/9352525b50851921f000335615e75e0f1bdd5f2e)**



---

archive/issue_comments_460491.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9352525b50851921f000335615e75e0f1bdd5f2e\">9352525</a></td><td><code>fixed dictionary in python2</code></td></tr></table>\n",
    "created_at": "2020-04-22T05:40:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460491",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:6'>Comment 6:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9352525b50851921f000335615e75e0f1bdd5f2e">9352525</a></td><td><code>fixed dictionary in python2</code></td></tr></table>




---

archive/issue_events_369561.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-04-22T05:41:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-369561"
}
```



---

archive/issue_events_369562.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-04-22T05:41:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-369562"
}
```



---

archive/issue_comments_460492.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nI tested this in python2:\n\n```\n>>> {(-2686.81000000000, -2084.19000000000): 0, (-2687.19000000000, -2088.53000000000): 0}\n{(-2687.19, -2088.53): 0, (-2686.81, -2084.19): 0}\n```\n\nSo the fix should work fine.",
    "created_at": "2020-04-22T05:41:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460492",
    "user": "https://github.com/kliem"
}
```

<a id='comment:7'>Comment 7:</a>
I tested this in python2:

```
>>> {(-2686.81000000000, -2084.19000000000): 0, (-2687.19000000000, -2088.53000000000): 0}
{(-2687.19, -2088.53): 0, (-2686.81, -2084.19): 0}
```

So the fix should work fine.



---

archive/issue_events_369563.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-04-22T12:34:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-369563"
}
```



---

archive/issue_events_369564.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-04-22T12:34:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-369564"
}
```



---

archive/issue_comments_460493.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nThanks for the quick fix.",
    "created_at": "2020-04-22T12:34:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460493",
    "user": "https://github.com/jplab"
}
```

<a id='comment:8'>Comment 8:</a>
Thanks for the quick fix.



---

archive/issue_comments_460494.json:
```json
{
    "body": "Changed branch from **[public/29176](https://github.com/sagemath/sagetrac-mirror/tree/public/29176)** to **[9352525](https://github.com/sagemath/sagetrac-mirror/commit/9352525b50851921f000335615e75e0f1bdd5f2e)**",
    "created_at": "2020-04-23T22:32:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460494",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/29176](https://github.com/sagemath/sagetrac-mirror/tree/public/29176)** to **[9352525](https://github.com/sagemath/sagetrac-mirror/commit/9352525b50851921f000335615e75e0f1bdd5f2e)**



---

archive/issue_events_369565.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-04-23T22:32:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-369565"
}
```



---

archive/issue_events_369566.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-04-23T22:32:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-369566"
}
```



---

archive/issue_comments_460495.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\n\n```\n**********************************************************************\nFile \"src/sage/geometry/polyhedron/backend_cdd.py\", line 234, in sage.geometry.polyhedron.backend_cdd.Polyhedron_cdd._init_from_cdd_output\nFailed example:\n    R  # py2\nExpected:\n    {P(-2687.19000000000, -2088.53000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line,\n     P(-2686.81000000000, -2084.19000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line}\nGot:\n    {P(-2686.81000000000, -2084.19000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line,\n     P(-2687.19000000000, -2088.53000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line}\n**********************************************************************\n1 item had failures:\n   1 of  10 in sage.geometry.polyhedron.backend_cdd.Polyhedron_cdd._init_from_cdd_output\n    [46 tests, 1 failure, 0.59 s]\n**********************************************************************\n```",
    "created_at": "2020-04-25T08:52:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460495",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:10'>Comment 10:</a>

```
**********************************************************************
File "src/sage/geometry/polyhedron/backend_cdd.py", line 234, in sage.geometry.polyhedron.backend_cdd.Polyhedron_cdd._init_from_cdd_output
Failed example:
    R  # py2
Expected:
    {P(-2687.19000000000, -2088.53000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line,
     P(-2686.81000000000, -2084.19000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line}
Got:
    {P(-2686.81000000000, -2084.19000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line,
     P(-2687.19000000000, -2088.53000000000): A 2-dimensional polyhedron in RDF^2 defined as the convex hull of 1 vertex, 1 ray, 1 line}
**********************************************************************
1 item had failures:
   1 of  10 in sage.geometry.polyhedron.backend_cdd.Polyhedron_cdd._init_from_cdd_output
    [46 tests, 1 failure, 0.59 s]
**********************************************************************
```



---

archive/issue_comments_460496.json:
```json
{
    "body": "Changed commit from **[9352525](https://github.com/sagemath/sagetrac-mirror/commit/9352525b50851921f000335615e75e0f1bdd5f2e)** to none",
    "created_at": "2020-04-25T08:52:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460496",
    "user": "https://github.com/vbraun"
}
```

Changed commit from **[9352525](https://github.com/sagemath/sagetrac-mirror/commit/9352525b50851921f000335615e75e0f1bdd5f2e)** to none



---

archive/issue_comments_460497.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nGot rid of the dictionary printing in the tests.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2860b021050540ec678324ada4393e9ea78f603c\">2860b02</a></td><td><code>expose incidence matrix computed by backend cdd</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b13fb31d2cdb711bc234868a260d37fcf7d1e3f2\">b13fb31</a></td><td><code>add doctests that 29176 is fixed</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/581f5547b85ee1971c24f3c8580c2f04950b6b32\">581f554</a></td><td><code>fixed dictionary in python2</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fb2d66a0e71f164e9bef37922c8d544b9b270c9c\">fb2d66a</a></td><td><code>fix test by avoiding printing the dictionary at all</code></td></tr></table>\n",
    "created_at": "2020-04-25T22:12:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460497",
    "user": "https://github.com/kliem"
}
```

<a id='comment:11'>Comment 11:</a>
Got rid of the dictionary printing in the tests.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2860b021050540ec678324ada4393e9ea78f603c">2860b02</a></td><td><code>expose incidence matrix computed by backend cdd</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b13fb31d2cdb711bc234868a260d37fcf7d1e3f2">b13fb31</a></td><td><code>add doctests that 29176 is fixed</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/581f5547b85ee1971c24f3c8580c2f04950b6b32">581f554</a></td><td><code>fixed dictionary in python2</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fb2d66a0e71f164e9bef37922c8d544b9b270c9c">fb2d66a</a></td><td><code>fix test by avoiding printing the dictionary at all</code></td></tr></table>




---

archive/issue_comments_460498.json:
```json
{
    "body": "Commit: **[fb2d66a](https://github.com/sagemath/sagetrac-mirror/commit/fb2d66a0e71f164e9bef37922c8d544b9b270c9c)**",
    "created_at": "2020-04-25T22:12:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460498",
    "user": "https://github.com/kliem"
}
```

Commit: **[fb2d66a](https://github.com/sagemath/sagetrac-mirror/commit/fb2d66a0e71f164e9bef37922c8d544b9b270c9c)**



---

archive/issue_events_369567.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-04-25T22:12:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-369567"
}
```



---

archive/issue_comments_460499.json:
```json
{
    "body": "Changed branch from **[9352525](https://github.com/sagemath/sagetrac-mirror/commit/9352525b50851921f000335615e75e0f1bdd5f2e)** to **[public/29176-reb](https://github.com/sagemath/sagetrac-mirror/tree/public/29176-reb)**",
    "created_at": "2020-04-25T22:12:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460499",
    "user": "https://github.com/kliem"
}
```

Changed branch from **[9352525](https://github.com/sagemath/sagetrac-mirror/commit/9352525b50851921f000335615e75e0f1bdd5f2e)** to **[public/29176-reb](https://github.com/sagemath/sagetrac-mirror/tree/public/29176-reb)**



---

archive/issue_events_369568.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-04-27T07:24:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-369568"
}
```



---

archive/issue_events_369569.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2020-04-27T07:24:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-369569"
}
```



---

archive/issue_comments_460500.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nThank you.",
    "created_at": "2020-04-27T07:24:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460500",
    "user": "https://github.com/kliem"
}
```

<a id='comment:13'>Comment 13:</a>
Thank you.



---

archive/issue_comments_460501.json:
```json
{
    "body": "Changed branch from **[public/29176-reb](https://github.com/sagemath/sagetrac-mirror/tree/public/29176-reb)** to **[fb2d66a](https://github.com/sagemath/sagetrac-mirror/commit/fb2d66a0e71f164e9bef37922c8d544b9b270c9c)**",
    "created_at": "2020-05-02T21:58:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29176#issuecomment-460501",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/29176-reb](https://github.com/sagemath/sagetrac-mirror/tree/public/29176-reb)** to **[fb2d66a](https://github.com/sagemath/sagetrac-mirror/commit/fb2d66a0e71f164e9bef37922c8d544b9b270c9c)**



---

archive/issue_events_369570.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-05-02T21:58:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-369570"
}
```



---

archive/issue_events_369571.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "cbcab1ad339232b14004cb08956f6e9c8b19ee49",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-05-02T21:58:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29176",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29176#event-369571"
}
```
