# Issue 29055: Installing any optional package on the binary distribution triggers full recompile of sagelib

archive/issues_028818.json:
```json
{
    "assignees": [],
    "body": "https://github.com/mkoeppe/sage-numerical-backends-coin/blob/master/tox.ini\nwith `tox -e docker-sage_binary-cbc_spkg`.\n\nhttps://github.com/mkoeppe/sage-numerical-backends-coin/blob/master/Dockerfile-sage_binary\nhttps://github.com/mkoeppe/sage-numerical-backends-coin/blob/master/Dockerfile-cbc_spkg\n\n\nCC:  @vbraun @dimpase @jdemeyer @embray\n\nReviewer: **Dima Pasechnik**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29055_\n\n",
    "closed_at": "2020-03-30T06:24:09Z",
    "created_at": "2020-01-20T15:28:48Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Installing any optional package on the binary distribution triggers full recompile of sagelib",
    "type": "issue",
    "updated_at": "2020-03-30T06:24:09Z",
    "url": "https://github.com/sagemath/sage/issues/29055",
    "user": "https://github.com/mkoeppe"
}
```
https://github.com/mkoeppe/sage-numerical-backends-coin/blob/master/tox.ini
with `tox -e docker-sage_binary-cbc_spkg`.

https://github.com/mkoeppe/sage-numerical-backends-coin/blob/master/Dockerfile-sage_binary
https://github.com/mkoeppe/sage-numerical-backends-coin/blob/master/Dockerfile-cbc_spkg


CC:  @vbraun @dimpase @jdemeyer @embray

Reviewer: **Dima Pasechnik**

_Issue created by migration from https://trac.sagemath.org/ticket/29055_





---

archive/issue_events_381056.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-01-20T15:28:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29055",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29055#event-381056"
}
```



---

archive/issue_events_381057.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-01-20T15:28:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29055",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29055#event-381057"
}
```



---

archive/issue_events_381058.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-01-20T15:28:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29055",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29055#event-381058"
}
```



---

archive/issue_events_381059.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-01-20T15:28:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29055",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29055#event-381059"
}
```



---

archive/issue_events_381060.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-02-17T01:14:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29055",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29055#event-381060"
}
```



---

archive/issue_events_381061.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-02-17T01:14:09Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29055",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29055#event-381061"
}
```



---

archive/issue_events_381062.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-02-17T01:14:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29055",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29055#event-381062"
}
```



---

archive/issue_events_381063.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-02-17T01:14:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29055",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29055#event-381063"
}
```



---

archive/issue_comments_457584.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\n#29158 addresses the same problem.",
    "created_at": "2020-02-17T01:14:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29055",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29055#issuecomment-457584",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:1" align="right">Comment 1</div>

#29158 addresses the same problem.



---

archive/issue_events_381064.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-03-29T15:23:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29055",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29055#event-381064"
}
```



---

archive/issue_events_381065.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-03-29T15:23:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29055",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29055#event-381065"
}
```



---

archive/issue_comments_457585.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2020-03-29T15:23:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29055",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29055#issuecomment-457585",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_events_381066.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-03-30T06:24:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29055",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29055#event-381066"
}
```



---

archive/issue_events_381067.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-03-30T06:24:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29055",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29055#event-381067"
}
```



---

archive/issue_events_381068.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-03-30T06:24:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29055",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29055#event-381068"
}
```
