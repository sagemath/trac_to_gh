...
```json
{
    "body": "<a id='comment:18'></a>The composite map is probably faster because `QQ` has specialized routines for converting `int` because of `mpq` and then creating the polynomial with elements already in `QQ` is likely very optimized as well. So I would go with Peter's solution in comment:16.",
...
<a id='comment:18'></a>The composite map is probably faster because `QQ` has specialized routines for converting `int` because of `mpq` and then creating the polynomial with elements already in `QQ` is likely very optimized as well. So I would go with Peter's solution in comment:16.
...
```json
{
    "body": "<a id='comment:19'></a>See #29311 and #29312 (the latter with a better solution than the one I suggested in comment:17).",
...
<a id='comment:19'></a>See #29311 and #29312 (the latter with a better solution than the one I suggested in comment:17).
