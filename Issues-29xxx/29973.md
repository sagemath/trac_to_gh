# Issue 29973: Make sets doctests ready for random seeds

archive/issues_029736.json:
```json
{
    "assignees": [],
    "body": "This ticket makes\n\n```\nsage -t --long --random-seed=n src/sage/sets/\n```\npass for different values `n` than just `0`.\n\nDepends on #29962\n\nBranch/Commit: **[aae7813](https://github.com/sagemath/sagetrac-mirror/commit/aae78139c7a229087872b014875ce9380f34bacf)**\n\nReviewer: **Jonathan Kliem**\n\nAuthor: **Dave Morris**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29973_\n\n",
    "closed_at": "2020-12-27T00:23:23Z",
    "created_at": "2020-06-24T20:51:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20doctest%20framework",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make sets doctests ready for random seeds",
    "type": "issue",
    "updated_at": "2020-12-27T00:23:23Z",
    "url": "https://github.com/sagemath/sage/issues/29973",
    "user": "https://github.com/kliem"
}
```
This ticket makes

```
sage -t --long --random-seed=n src/sage/sets/
```
pass for different values `n` than just `0`.

Depends on #29962

Branch/Commit: **[aae7813](https://github.com/sagemath/sagetrac-mirror/commit/aae78139c7a229087872b014875ce9380f34bacf)**

Reviewer: **Jonathan Kliem**

Author: **Dave Morris**

_Issue created by migration from https://trac.sagemath.org/ticket/29973_





---

archive/issue_events_394412.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-06-24T20:51:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29973",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29973#event-394412"
}
```



---

archive/issue_events_394413.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-06-24T20:51:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29973",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20doctest%20framework",
    "label_color": "000080",
    "label_name": "component: doctest framework",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29973#event-394413"
}
```



---

archive/issue_events_394414.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-06-24T20:51:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29973",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29973#event-394414"
}
```



---

archive/issue_events_394415.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-06-24T20:51:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29973",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29973#event-394415"
}
```



---

archive/issue_comments_477236.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAt least the following needs to be fixed:\n\n```\nsage -t --long --random_seed=151058820726654196682836430928254760259 src/sage/sets/finite_enumerated_set.py  # 1 doctest failed\n```",
    "created_at": "2020-06-24T20:52:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29973#issuecomment-477236",
    "user": "https://github.com/kliem"
}
```

<a id='comment:1'>Comment 1:</a>
At least the following needs to be fixed:

```
sage -t --long --random_seed=151058820726654196682836430928254760259 src/sage/sets/finite_enumerated_set.py  # 1 doctest failed
```



---

archive/issue_comments_477237.json:
```json
{
    "body": "Dependencies: **#29962**",
    "created_at": "2020-06-24T20:52:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29973#issuecomment-477237",
    "user": "https://github.com/kliem"
}
```

Dependencies: **#29962**



---

archive/issue_events_394416.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-05T19:21:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/29973",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29973#event-394416"
}
```



---

archive/issue_events_394417.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-05T19:21:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29973",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29973#event-394417"
}
```



---

archive/issue_comments_477238.json:
```json
{
    "body": "Branch: **[public/29973](https://github.com/sagemath/sagetrac-mirror/tree/public/29973)**",
    "created_at": "2020-12-17T04:27:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29973#issuecomment-477238",
    "user": "https://github.com/DaveWitteMorris"
}
```

Branch: **[public/29973](https://github.com/sagemath/sagetrac-mirror/tree/public/29973)**



---

archive/issue_comments_477239.json:
```json
{
    "body": "Author: **Dave Morris**",
    "created_at": "2020-12-17T04:36:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29973#issuecomment-477239",
    "user": "https://github.com/DaveWitteMorris"
}
```

Author: **Dave Morris**



---

archive/issue_events_394418.json:
```json
{
    "actor": "https://github.com/DaveWitteMorris",
    "created_at": "2020-12-17T04:36:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29973",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29973#event-394418"
}
```



---

archive/issue_comments_477240.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nI added a `# random` tag to the example that failed in [comment:1](#comment%3A1) (and also added a basic doctest `S.random_element() in S` to the `random_element()` method of `FiniteEnumeratedSet` ). I tried a few various seeds and have not seen any other doctest failures.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/aae78139c7a229087872b014875ce9380f34bacf\">aae7813</a></td><td><code>trac 29973 random enumerated sets</code></td></tr></table>\n",
    "created_at": "2020-12-17T04:36:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29973#issuecomment-477240",
    "user": "https://github.com/DaveWitteMorris"
}
```

<a id='comment:4'>Comment 4:</a>
I added a `# random` tag to the example that failed in [comment:1](#comment%3A1) (and also added a basic doctest `S.random_element() in S` to the `random_element()` method of `FiniteEnumeratedSet` ). I tried a few various seeds and have not seen any other doctest failures.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/aae78139c7a229087872b014875ce9380f34bacf">aae7813</a></td><td><code>trac 29973 random enumerated sets</code></td></tr></table>




---

archive/issue_comments_477241.json:
```json
{
    "body": "Commit: **[aae7813](https://github.com/sagemath/sagetrac-mirror/commit/aae78139c7a229087872b014875ce9380f34bacf)**",
    "created_at": "2020-12-17T04:36:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29973#issuecomment-477241",
    "user": "https://github.com/DaveWitteMorris"
}
```

Commit: **[aae7813](https://github.com/sagemath/sagetrac-mirror/commit/aae78139c7a229087872b014875ce9380f34bacf)**



---

archive/issue_comments_477242.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nAfter submitting the PR, I noticed that #29935 suggests `S.random_element().parent() is S` as a doctest, but I don't think that will work here:\n\n```\nsage: S = FiniteEnumeratedSet([1,2,3])                                                   \nsage: a = S.random_element()                                                             \nsage: a                                                                                  \n2\nsage: a.parent()                                                                         \nInteger Ring\nsage: S.random_element().parent() is S                                                   \nFalse\n```",
    "created_at": "2020-12-17T04:53:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29973#issuecomment-477242",
    "user": "https://github.com/DaveWitteMorris"
}
```

<a id='comment:5'>Comment 5:</a>
After submitting the PR, I noticed that #29935 suggests `S.random_element().parent() is S` as a doctest, but I don't think that will work here:

```
sage: S = FiniteEnumeratedSet([1,2,3])                                                   
sage: a = S.random_element()                                                             
sage: a                                                                                  
2
sage: a.parent()                                                                         
Integer Ring
sage: S.random_element().parent() is S                                                   
False
```



---

archive/issue_comments_477243.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nThank you. I think it is fine.",
    "created_at": "2020-12-17T19:42:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29973#issuecomment-477243",
    "user": "https://github.com/kliem"
}
```

<a id='comment:6'>Comment 6:</a>
Thank you. I think it is fine.



---

archive/issue_events_394419.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-12-17T19:42:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29973",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29973#event-394419"
}
```



---

archive/issue_events_394420.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-12-17T19:42:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29973",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29973#event-394420"
}
```



---

archive/issue_comments_477244.json:
```json
{
    "body": "Reviewer: **Jonathan Kliem**",
    "created_at": "2020-12-17T19:42:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29973#issuecomment-477244",
    "user": "https://github.com/kliem"
}
```

Reviewer: **Jonathan Kliem**



---

archive/issue_comments_477245.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nI think it is important to implement fuzzing of the doctests, so I will try to look at some of the other tickets in #29935 too. Thanks very much for your extensive work on this project!! (and thank you for the review of this ticket)",
    "created_at": "2020-12-17T20:20:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29973#issuecomment-477245",
    "user": "https://github.com/DaveWitteMorris"
}
```

<a id='comment:7'>Comment 7:</a>
I think it is important to implement fuzzing of the doctests, so I will try to look at some of the other tickets in #29935 too. Thanks very much for your extensive work on this project!! (and thank you for the review of this ticket)



---

archive/issue_comments_477246.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nThanks.\n\nWell there is #29954, which is pretty annoying and made me slow down a bit. This needs to be taken care of.",
    "created_at": "2020-12-17T22:40:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29973#issuecomment-477246",
    "user": "https://github.com/kliem"
}
```

<a id='comment:8'>Comment 8:</a>
Thanks.

Well there is #29954, which is pretty annoying and made me slow down a bit. This needs to be taken care of.



---

archive/issue_comments_477247.json:
```json
{
    "body": "Changed branch from **[public/29973](https://github.com/sagemath/sagetrac-mirror/tree/public/29973)** to **[aae7813](https://github.com/sagemath/sagetrac-mirror/commit/aae78139c7a229087872b014875ce9380f34bacf)**",
    "created_at": "2020-12-27T00:23:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29973",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29973#issuecomment-477247",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/29973](https://github.com/sagemath/sagetrac-mirror/tree/public/29973)** to **[aae7813](https://github.com/sagemath/sagetrac-mirror/commit/aae78139c7a229087872b014875ce9380f34bacf)**



---

archive/issue_events_394421.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-12-27T00:23:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29973",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29973#event-394421"
}
```



---

archive/issue_events_394422.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "c073c753ba1fe108b920a91aac411cfac0af800e",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-12-27T00:23:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29973",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29973#event-394422"
}
```
