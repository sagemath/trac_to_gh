# Issue 29110: Make `dim` and alias for `dimension` in combinatorial polyhedron

archive/issues_028873.json:
```json
{
    "body": "This ticket adds the alias `dim` for `dimension` of both `CombinatorialPolyhedron` and `CombinatorialFace`.\n\n**CC:**  @jplab @laisrast\n\n**Keywords:** combinatorial polyhedron, dimension\n\n**Branch/Commit:** [bb92657665de6ca7505d89003e8b4688cd51f490](https://github.com/sagemath/sagetrac-mirror/commit/bb92657665de6ca7505d89003e8b4688cd51f490)\n\n**Reviewer:** Laith Rastanawi\n\n**Author:** Jonathan Kliem\n\n**Resolution:** fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/29110\n\n",
    "closed_at": "2020-02-11T21:05:55Z",
    "created_at": "2020-01-30T15:48:47Z",
    "labels": [
        "component: geometry",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.1",
    "title": "Make `dim` and alias for `dimension` in combinatorial polyhedron",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/29110",
    "user": "https://github.com/kliem"
}
```
This ticket adds the alias `dim` for `dimension` of both `CombinatorialPolyhedron` and `CombinatorialFace`.

**CC:**  @jplab @laisrast

**Keywords:** combinatorial polyhedron, dimension

**Branch/Commit:** [bb92657665de6ca7505d89003e8b4688cd51f490](https://github.com/sagemath/sagetrac-mirror/commit/bb92657665de6ca7505d89003e8b4688cd51f490)

**Reviewer:** Laith Rastanawi

**Author:** Jonathan Kliem

**Resolution:** fixed

Issue created by migration from https://trac.sagemath.org/ticket/29110





---

archive/issue_comments_568252.json:
```json
{
    "body": "<a id='comment:1'></a>\nNew commits:\n|                                                                                                                                          |                                                                           |\n|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------|\n|[bb92657](https://github.com/sagemath/sagetrac-mirror/commit/bb92657665de6ca7505d89003e8b4688cd51f490)|<code>make ``dim`` and alias of dimension in CombinatorialPolyhedron</code>|",
    "created_at": "2020-01-30T15:49:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29110",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29110#issuecomment-568252",
    "user": "https://github.com/kliem"
}
```


<a id='comment:1'></a>
New commits:
|                                                                                                                                          |                                                                           |
|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------|
|[bb92657](https://github.com/sagemath/sagetrac-mirror/commit/bb92657665de6ca7505d89003e8b4688cd51f490)|<code>make ``dim`` and alias of dimension in CombinatorialPolyhedron</code>|



---

archive/issue_comments_568253.json:
```json
{
    "body": "**Changing status** from new to needs_review.",
    "created_at": "2020-01-30T15:49:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29110",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29110#issuecomment-568253",
    "user": "https://github.com/kliem"
}
```


**Changing status** from new to needs_review.



---

archive/issue_comments_568254.json:
```json
{
    "body": "**Branch:** [public/29110](https://github.com/sagemath/sagetrac-mirror/tree/public/29110)",
    "created_at": "2020-01-30T15:49:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29110",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29110#issuecomment-568254",
    "user": "https://github.com/kliem"
}
```


**Branch:** [public/29110](https://github.com/sagemath/sagetrac-mirror/tree/public/29110)



---

archive/issue_comments_568255.json:
```json
{
    "body": "**Commit:** [bb92657665de6ca7505d89003e8b4688cd51f490](https://github.com/sagemath/sagetrac-mirror/commit/bb92657665de6ca7505d89003e8b4688cd51f490)",
    "created_at": "2020-01-30T15:49:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29110",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29110#issuecomment-568255",
    "user": "https://github.com/kliem"
}
```


**Commit:** [bb92657665de6ca7505d89003e8b4688cd51f490](https://github.com/sagemath/sagetrac-mirror/commit/bb92657665de6ca7505d89003e8b4688cd51f490)



---

archive/issue_comments_568256.json:
```json
{
    "body": "**Reviewer:** Laith Rastanawi",
    "created_at": "2020-01-31T09:30:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29110",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29110#issuecomment-568256",
    "user": "https://github.com/LaisRast"
}
```


**Reviewer:** Laith Rastanawi



---

archive/issue_comments_568257.json:
```json
{
    "body": "**Changing status** from needs_review to positive_review.",
    "created_at": "2020-01-31T09:30:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29110",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29110#issuecomment-568257",
    "user": "https://github.com/LaisRast"
}
```


**Changing status** from needs_review to positive_review.



---

archive/issue_comments_568258.json:
```json
{
    "body": "**Resolution:** fixed",
    "created_at": "2020-02-11T21:05:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29110",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29110#issuecomment-568258",
    "user": "https://github.com/vbraun"
}
```


**Resolution:** fixed



---

archive/issue_events_083355.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-02-11T21:05:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/29110",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29110#event-83355"
}
```




---

archive/issue_comments_568259.json:
```json
{
    "body": "**Changing branch** from \"[public/29110](https://github.com/sagemath/sagetrac-mirror/tree/public/29110)\" to \"[bb92657665de6ca7505d89003e8b4688cd51f490](https://github.com/sagemath/sagetrac-mirror/commit/bb92657665de6ca7505d89003e8b4688cd51f490)\".",
    "created_at": "2020-02-11T21:05:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29110",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29110#issuecomment-568259",
    "user": "https://github.com/vbraun"
}
```


**Changing branch** from "[public/29110](https://github.com/sagemath/sagetrac-mirror/tree/public/29110)" to "[bb92657665de6ca7505d89003e8b4688cd51f490](https://github.com/sagemath/sagetrac-mirror/commit/bb92657665de6ca7505d89003e8b4688cd51f490)".
