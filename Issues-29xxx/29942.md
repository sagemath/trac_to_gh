# Issue 29942: More efficient rank and unrank for Permutations_mset

archive/issues_029705.json:
```json
{
    "assignees": [],
    "body": "The [Permutations_mset](https://doc.sagemath.org/html/en/reference/combinat/sage/combinat/permutation.html#sage.combinat.permutation.Permutations_mset)\n`rank` and `unrank` methods inherit a generic implementation from `EnumeratedSets`\n[rank](https://doc.sagemath.org/html/en/reference/categories/sage/categories/enumerated_sets.html#sage.categories.enumerated_sets.EnumeratedSets.ParentMethods.rank) and\n[unrank](https://doc.sagemath.org/html/en/reference/categories/sage/categories/enumerated_sets.html#sage.categories.enumerated_sets.EnumeratedSets.ParentMethods.unrank),\nwhich have running time proportional to the rank:\n\n```\nsage: mset = list(range(10)) * 3\nsage: p = Permutations(mset)\nsage: for e in range(8): timeit('p.unrank({})'.format(10^e), number=1, repeat=5)\n1 loop, best of 5: 24.2 \u03bcs per loop\n1 loop, best of 5: 39.1 \u03bcs per loop\n1 loop, best of 5: 191 \u03bcs per loop\n1 loop, best of 5: 1.28 ms per loop\n1 loop, best of 5: 12.7 ms per loop\n1 loop, best of 5: 126 ms per loop\n1 loop, best of 5: 1.27 s per loop\n1 loop, best of 5: 13 s per loop\nsage: p.rank(list(reversed(sorted(mset))))\n... never finishes ...\n```\n\nThis branch makes the running time roughly proportional to the size of the multiset:\n\n```\nsage: mset = list(range(10)) * 3\nsage: p = Permutations(mset)\nsage: for e in range(8): timeit('p.unrank({})'.format(10^e), number=1, repeat=5)\n1 loop, best of 5: 1.35 ms per loop\n1 loop, best of 5: 1.43 ms per loop\n1 loop, best of 5: 1.37 ms per loop\n1 loop, best of 5: 1.37 ms per loop\n1 loop, best of 5: 2.23 ms per loop\n1 loop, best of 5: 2.12 ms per loop\n1 loop, best of 5: 1.46 ms per loop\n1 loop, best of 5: 1.44 ms per loop\nsage: timeit('p.rank(list(reversed(sorted(mset))))', number=1, repeat=5)\n1 loop, best of 5: 1.51 ms per loop\n```\n\n\n## Algorithm\n\nThe `rank` algorithm is a straightforward application of the solution to exercise 7.2.1.2\u20134 in\n*The Art of Computer Programming*, Volume 4A.\nI will quote the exercise:\n\n> Q. Generalizing exercise 3, explain how to compute the rank of\n>    *a*<sub>1</sub>\u2026*a*<sub>*n*</sub>\n>    with respect to Algorithm L [lexicographic permutation generation]\n>    when {*a*<sub>1</sub>, \u2026, *a*<sub>*n*</sub>} is the multiset {*n*<sub>1</sub>\u00b7*x*<sub>1</sub>, \u2026, *n*<sub>*t*</sub>\u00b7*x*<sub>*t*</sub>};\n>    here *n*<sub>1</sub> + \u2026 + *n*<sub>*t*</sub> = *n* and\n>    *x*<sub>1</sub> < \u2026 < *x*<sub>*t*</sub>. (The total number of permutations is, of course, the multinomial coefficient\n>        multinomial(*n*<sub>1</sub>, \u2026, *n*<sub>*t*</sub>) = *n*! / (*n*<sub>1</sub>! \u2026 *n*<sub>*t*</sub>!);\n>    see Eq. 5.1.2\u2013(3).) What is the rank of 314159265?\n\n> A. Use the recurrence rank(*a*<sub>1</sub> \u2026 *a*<sub>*n*</sub>) =\n>    1/*n* \u03a3<sub>j=1\u2026*t*</sub> *n*<sub>*j*</sub> [*x*<sub>*j*</sub> < *a*<sub>1</sub>] multinomial(*n*<sub>1</sub>, \u2026, *n*<sub>*t*</sub>) + rank(*a*<sub>2</sub> \u2026 *a*<sub>*n*</sub>).\n>    For example, rank(314159265) is\n>        3/9 multinomial(2, 1, 1, 1, 2, 1, 1)\n>        + 0\n>        + 2/7 multinomial(1, 1, 1, 2, 1, 1)\n>        + 0\n>        + 1/5 multinomial(1, 2, 1, 1)\n>        + 3/4 multinomial(1, 1, 1, 1)\n>        + 0\n>        + 1/2 multinomial(1, 1) = 30991.\n\nThe `unrank` algorithm is also based on this recurrence;\nhowever the application is not so straightforward.\nI did not find an explicit statement of an unranking algorithm elsewhere,\nand the algorithm I came up with isn't particularly elegant.\nI'd appreciate having another opinion on it or a pointer to some other reference.\nThe main idea is that given a rank *r* and a multiset\n*m* = {*n*<sub>1</sub>\u00b7*x*<sub>1</sub>, \u2026, *n*<sub>*t*</sub>\u00b7*x*<sub>*t*</sub>},\nwe know that\n*r* = *c*/*n* multinomial(*n*<sub>1</sub>, \u2026, *n*<sub>*t*</sub>) + rank(*m*\\*c*),\nwhere *c* is the number of elements in *m* that are strictly less than\nthe first element of the output sequence, and *m*\\*c* stands for\n*m* with one instance of the *c*th smallest element removed.\n\n\n## Notes/caveats\n\nThe `rank` method is actually independent of the `Permutations_mset` object\nit is attached to.\nOther than the `self(p).check()` line,\nit never refers to `self`.\nIt could be defined in terms of a standalone function,\nas [StandardPermutations_n.unrank](https://doc.sagemath.org/html/en/reference/combinat/sage/combinat/permutation.html#sage.combinat.permutation.StandardPermutations_n.unrank)\nis defined in terms of [from_rank](https://doc.sagemath.org/html/en/reference/combinat/sage/combinat/permutation.html#sage.combinat.permutation.from_rank).\n\nI am aware of two ways in which this branch differs in behavior from what is there now.\nThey both seem to me to be because of bugs in the `EnumeratedSets` fallback implementations,\nbut I'll document them here in case complete compatibility is needed.\n\nThe first is that the existing\n[rank](https://github.com/sagemath/sagetrac-mirror/blob/develop/src/sage/categories/enumerated_sets.py?h=9.1#n668)\nmethod returns `None` when passed a list or a tuple,\nbecause it uses `==` comparison with an `element_class` type\n(I'm not sure how that works exactly, but at any rate it doesn't compare equal.)\nTo get `rank` to return something other than `None`, you have to pass in\na permutation produced by the `Permutations_mset` object itself.\n\n```\nsage: mset = [1, 1, 2, 3, 4, 5, 5, 6, 9]\nsage: p = Permutations(mset)\nsage: p.rank(mset)\nsage: p.rank([1, 1, 2, 3, 4, 5, 5, 6, 9])\nsage: p.unrank(3)\n[1, 1, 2, 3, 4, 5, 6, 9, 5]\nsage: type(p.unrank(3))\n<class 'sage.combinat.permutation.Permutations_mset_with_category.element_class'>\nsage: p.rank(p.unrank(3))\n3\n```\n\nIn this branch, the value passed to `rank` doesn't have to be an `element_class`.\n\n```\nsage: mset = [1, 1, 2, 3, 4, 5, 5, 6, 9]\nsage: p = Permutations(mset)\nsage: p.rank(mset)\n0\nsage: p.rank([1, 1, 2, 3, 4, 5, 5, 6, 9])\n0\nsage: p.unrank(3)\n[1, 1, 2, 3, 4, 5, 6, 9, 5]\nsage: type(p.unrank(3))\n<class 'list'>\nsage: p.rank(p.unrank(3))\n3\n```\n\nThe other way in which this branch changes behavior\nis that the current code does not canonicalize the multiset\nused to create the `Permutations_mset` object.\n`Permutations([1, 1, 2, 3, 4])` and `Permutations([3, 2, 1, 4, 1])`\nvisit the same permutations but in a different order;\nthe latter acts like an already partially enumerated version of the former:\n\n```\nsage: Permutations([1, 1, 2, 3, 4]).unrank(0)\n[1, 1, 2, 3, 4]\nsage: Permutations([3, 2, 1, 4, 1]).unrank(0)\n[3, 2, 1, 1, 4]\n```\n\nIn this branch, the order of the input multiset does not make a difference:\n\n```\nsage: Permutations([1, 1, 2, 3, 4]).unrank(0)\n[1, 1, 2, 3, 4]\nsage: Permutations([3, 2, 1, 4, 1]).unrank(0)\n[1, 1, 2, 3, 4]\n```\n\nIt would be possible to work around this canonicalization, if necessary,\nby computing the rank of the input multiset on creation,\nand using it as an offset for future calls to `unrank`.\n\nI was working on a project that needed fast unranking of a multiset,\nwhen I found that Sage could not handle a multiset of the size I needed.\nI came up with [an algorithm](https://www.bamsoftware.com/comicrando/seed.html#algorithm)\nthat worked well enough for my purposes, however its ranking order is not lexicographic.\nThe algorithms in this branch are the result of a little more research\nto find a lexicographic algorithm.\n\n\nDepends on #30005\n\nCC:  @slel\n\nBranch/Commit: **[141f9a9](https://github.com/sagemath/sagetrac-mirror/commit/141f9a97c16facf1780bf14402915d68d814f354)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton, Travis Scrimshaw**\n\nAuthor: **David Fifield**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29942_\n\n",
    "closed_at": "2020-07-10T19:33:49Z",
    "created_at": "2020-06-23T06:43:12Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "More efficient rank and unrank for Permutations_mset",
    "type": "issue",
    "updated_at": "2020-07-10T19:33:49Z",
    "url": "https://github.com/sagemath/sage/issues/29942",
    "user": "https://github.com/sagetrac-dcfifield"
}
```
The [Permutations_mset](https://doc.sagemath.org/html/en/reference/combinat/sage/combinat/permutation.html#sage.combinat.permutation.Permutations_mset)
`rank` and `unrank` methods inherit a generic implementation from `EnumeratedSets`
[rank](https://doc.sagemath.org/html/en/reference/categories/sage/categories/enumerated_sets.html#sage.categories.enumerated_sets.EnumeratedSets.ParentMethods.rank) and
[unrank](https://doc.sagemath.org/html/en/reference/categories/sage/categories/enumerated_sets.html#sage.categories.enumerated_sets.EnumeratedSets.ParentMethods.unrank),
which have running time proportional to the rank:

```
sage: mset = list(range(10)) * 3
sage: p = Permutations(mset)
sage: for e in range(8): timeit('p.unrank({})'.format(10^e), number=1, repeat=5)
1 loop, best of 5: 24.2 μs per loop
1 loop, best of 5: 39.1 μs per loop
1 loop, best of 5: 191 μs per loop
1 loop, best of 5: 1.28 ms per loop
1 loop, best of 5: 12.7 ms per loop
1 loop, best of 5: 126 ms per loop
1 loop, best of 5: 1.27 s per loop
1 loop, best of 5: 13 s per loop
sage: p.rank(list(reversed(sorted(mset))))
... never finishes ...
```

This branch makes the running time roughly proportional to the size of the multiset:

```
sage: mset = list(range(10)) * 3
sage: p = Permutations(mset)
sage: for e in range(8): timeit('p.unrank({})'.format(10^e), number=1, repeat=5)
1 loop, best of 5: 1.35 ms per loop
1 loop, best of 5: 1.43 ms per loop
1 loop, best of 5: 1.37 ms per loop
1 loop, best of 5: 1.37 ms per loop
1 loop, best of 5: 2.23 ms per loop
1 loop, best of 5: 2.12 ms per loop
1 loop, best of 5: 1.46 ms per loop
1 loop, best of 5: 1.44 ms per loop
sage: timeit('p.rank(list(reversed(sorted(mset))))', number=1, repeat=5)
1 loop, best of 5: 1.51 ms per loop
```


## Algorithm

The `rank` algorithm is a straightforward application of the solution to exercise 7.2.1.2–4 in
*The Art of Computer Programming*, Volume 4A.
I will quote the exercise:

> Q. Generalizing exercise 3, explain how to compute the rank of
>    *a*<sub>1</sub>…*a*<sub>*n*</sub>
>    with respect to Algorithm L [lexicographic permutation generation]
>    when {*a*<sub>1</sub>, …, *a*<sub>*n*</sub>} is the multiset {*n*<sub>1</sub>·*x*<sub>1</sub>, …, *n*<sub>*t*</sub>·*x*<sub>*t*</sub>};
>    here *n*<sub>1</sub> + … + *n*<sub>*t*</sub> = *n* and
>    *x*<sub>1</sub> < … < *x*<sub>*t*</sub>. (The total number of permutations is, of course, the multinomial coefficient
>        multinomial(*n*<sub>1</sub>, …, *n*<sub>*t*</sub>) = *n*! / (*n*<sub>1</sub>! … *n*<sub>*t*</sub>!);
>    see Eq. 5.1.2–(3).) What is the rank of 314159265?

> A. Use the recurrence rank(*a*<sub>1</sub> … *a*<sub>*n*</sub>) =
>    1/*n* Σ<sub>j=1…*t*</sub> *n*<sub>*j*</sub> [*x*<sub>*j*</sub> < *a*<sub>1</sub>] multinomial(*n*<sub>1</sub>, …, *n*<sub>*t*</sub>) + rank(*a*<sub>2</sub> … *a*<sub>*n*</sub>).
>    For example, rank(314159265) is
>        3/9 multinomial(2, 1, 1, 1, 2, 1, 1)
>        + 0
>        + 2/7 multinomial(1, 1, 1, 2, 1, 1)
>        + 0
>        + 1/5 multinomial(1, 2, 1, 1)
>        + 3/4 multinomial(1, 1, 1, 1)
>        + 0
>        + 1/2 multinomial(1, 1) = 30991.

The `unrank` algorithm is also based on this recurrence;
however the application is not so straightforward.
I did not find an explicit statement of an unranking algorithm elsewhere,
and the algorithm I came up with isn't particularly elegant.
I'd appreciate having another opinion on it or a pointer to some other reference.
The main idea is that given a rank *r* and a multiset
*m* = {*n*<sub>1</sub>·*x*<sub>1</sub>, …, *n*<sub>*t*</sub>·*x*<sub>*t*</sub>},
we know that
*r* = *c*/*n* multinomial(*n*<sub>1</sub>, …, *n*<sub>*t*</sub>) + rank(*m*\*c*),
where *c* is the number of elements in *m* that are strictly less than
the first element of the output sequence, and *m*\*c* stands for
*m* with one instance of the *c*th smallest element removed.


## Notes/caveats

The `rank` method is actually independent of the `Permutations_mset` object
it is attached to.
Other than the `self(p).check()` line,
it never refers to `self`.
It could be defined in terms of a standalone function,
as [StandardPermutations_n.unrank](https://doc.sagemath.org/html/en/reference/combinat/sage/combinat/permutation.html#sage.combinat.permutation.StandardPermutations_n.unrank)
is defined in terms of [from_rank](https://doc.sagemath.org/html/en/reference/combinat/sage/combinat/permutation.html#sage.combinat.permutation.from_rank).

I am aware of two ways in which this branch differs in behavior from what is there now.
They both seem to me to be because of bugs in the `EnumeratedSets` fallback implementations,
but I'll document them here in case complete compatibility is needed.

The first is that the existing
[rank](https://github.com/sagemath/sagetrac-mirror/blob/develop/src/sage/categories/enumerated_sets.py?h=9.1#n668)
method returns `None` when passed a list or a tuple,
because it uses `==` comparison with an `element_class` type
(I'm not sure how that works exactly, but at any rate it doesn't compare equal.)
To get `rank` to return something other than `None`, you have to pass in
a permutation produced by the `Permutations_mset` object itself.

```
sage: mset = [1, 1, 2, 3, 4, 5, 5, 6, 9]
sage: p = Permutations(mset)
sage: p.rank(mset)
sage: p.rank([1, 1, 2, 3, 4, 5, 5, 6, 9])
sage: p.unrank(3)
[1, 1, 2, 3, 4, 5, 6, 9, 5]
sage: type(p.unrank(3))
<class 'sage.combinat.permutation.Permutations_mset_with_category.element_class'>
sage: p.rank(p.unrank(3))
3
```

In this branch, the value passed to `rank` doesn't have to be an `element_class`.

```
sage: mset = [1, 1, 2, 3, 4, 5, 5, 6, 9]
sage: p = Permutations(mset)
sage: p.rank(mset)
0
sage: p.rank([1, 1, 2, 3, 4, 5, 5, 6, 9])
0
sage: p.unrank(3)
[1, 1, 2, 3, 4, 5, 6, 9, 5]
sage: type(p.unrank(3))
<class 'list'>
sage: p.rank(p.unrank(3))
3
```

The other way in which this branch changes behavior
is that the current code does not canonicalize the multiset
used to create the `Permutations_mset` object.
`Permutations([1, 1, 2, 3, 4])` and `Permutations([3, 2, 1, 4, 1])`
visit the same permutations but in a different order;
the latter acts like an already partially enumerated version of the former:

```
sage: Permutations([1, 1, 2, 3, 4]).unrank(0)
[1, 1, 2, 3, 4]
sage: Permutations([3, 2, 1, 4, 1]).unrank(0)
[3, 2, 1, 1, 4]
```

In this branch, the order of the input multiset does not make a difference:

```
sage: Permutations([1, 1, 2, 3, 4]).unrank(0)
[1, 1, 2, 3, 4]
sage: Permutations([3, 2, 1, 4, 1]).unrank(0)
[1, 1, 2, 3, 4]
```

It would be possible to work around this canonicalization, if necessary,
by computing the rank of the input multiset on creation,
and using it as an offset for future calls to `unrank`.

I was working on a project that needed fast unranking of a multiset,
when I found that Sage could not handle a multiset of the size I needed.
I came up with [an algorithm](https://www.bamsoftware.com/comicrando/seed.html#algorithm)
that worked well enough for my purposes, however its ranking order is not lexicographic.
The algorithms in this branch are the result of a little more research
to find a lexicographic algorithm.


Depends on #30005

CC:  @slel

Branch/Commit: **[141f9a9](https://github.com/sagemath/sagetrac-mirror/commit/141f9a97c16facf1780bf14402915d68d814f354)**

Reviewer: **Frédéric Chapoton, Travis Scrimshaw**

Author: **David Fifield**

_Issue created by migration from https://trac.sagemath.org/ticket/29942_





---

archive/issue_events_393937.json:
```json
{
    "actor": "https://github.com/sagetrac-dcfifield",
    "created_at": "2020-06-23T06:43:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29942#event-393937"
}
```



---

archive/issue_events_393938.json:
```json
{
    "actor": "https://github.com/sagetrac-dcfifield",
    "created_at": "2020-06-23T06:43:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29942#event-393938"
}
```



---

archive/issue_events_393939.json:
```json
{
    "actor": "https://github.com/sagetrac-dcfifield",
    "created_at": "2020-06-23T06:43:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29942#event-393939"
}
```



---

archive/issue_events_393940.json:
```json
{
    "actor": "https://github.com/sagetrac-dcfifield",
    "created_at": "2020-06-23T06:43:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29942#event-393940"
}
```



---

archive/issue_events_393941.json:
```json
{
    "actor": "https://github.com/sagetrac-dcfifield",
    "created_at": "2020-06-23T06:44:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29942#event-393941"
}
```



---

archive/issue_comments_476613.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThe documentation is not formatted correctly. Each time you insert a sentence or TEST between two blocks of tests, it should end with `::` unless there is another sentence just after.\n\nsee https://doc.sagemath.org/html/en/developer/coding_basics.html#template",
    "created_at": "2020-06-23T07:11:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29942#issuecomment-476613",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:2'>Comment 2:</a>
The documentation is not formatted correctly. Each time you insert a sentence or TEST between two blocks of tests, it should end with `::` unless there is another sentence just after.

see https://doc.sagemath.org/html/en/developer/coding_basics.html#template



---

archive/issue_comments_476614.json:
```json
{
    "body": "Changed commit from **[a59f514](https://github.com/sagemath/sagetrac-mirror/commit/a59f51403d4d2e35533dfb5c017861b8e0ff8827)** to **[9b9d936](https://github.com/sagemath/sagetrac-mirror/commit/9b9d936081bc4b04d5476ce4c4e54af0997a9295)**",
    "created_at": "2020-06-23T14:22:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29942#issuecomment-476614",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[a59f514](https://github.com/sagemath/sagetrac-mirror/commit/a59f51403d4d2e35533dfb5c017861b8e0ff8827)** to **[9b9d936](https://github.com/sagemath/sagetrac-mirror/commit/9b9d936081bc4b04d5476ce4c4e54af0997a9295)**



---

archive/issue_comments_476615.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9b9d936081bc4b04d5476ce4c4e54af0997a9295\">9b9d936</a></td><td><code>Fix doc formatting of Permutations_mset rank and unrank.</code></td></tr></table>\n",
    "created_at": "2020-06-23T14:22:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29942#issuecomment-476615",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:3'>Comment 3:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9b9d936081bc4b04d5476ce4c4e54af0997a9295">9b9d936</a></td><td><code>Fix doc formatting of Permutations_mset rank and unrank.</code></td></tr></table>




---

archive/issue_comments_476616.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nReplying to [@fchapoton](#comment%3A2):\n> The documentation is not formatted correctly. Each time you insert a sentence or TEST between two blocks of tests, it should end with `::` unless there is another sentence just after.\n\nThanks. I fixed the formatting in [9b9d936](https://github.com/sagemath/sagetrac-mirror/commit/9b9d936081bc4b04d5476ce4c4e54af0997a9295). I can see the difference with `sage --docbuild --include-tests-blocks`.",
    "created_at": "2020-06-23T14:26:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29942#issuecomment-476616",
    "user": "https://github.com/sagetrac-dcfifield"
}
```

<a id='comment:4'>Comment 4:</a>
Replying to [@fchapoton](#comment%3A2):
> The documentation is not formatted correctly. Each time you insert a sentence or TEST between two blocks of tests, it should end with `::` unless there is another sentence just after.

Thanks. I fixed the formatting in [9b9d936](https://github.com/sagemath/sagetrac-mirror/commit/9b9d936081bc4b04d5476ce4c4e54af0997a9295). I can see the difference with `sage --docbuild --include-tests-blocks`.



---

archive/issue_comments_476617.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -20,7 +20,7 @@\n ... never finishes ...\n ```\n \n-This branch makes the running time roughtly proportional to the size of the multiset:\n+This branch makes the running time roughly proportional to the size of the multiset:\n \n ```\n sage: mset = list(range(10)) * 3\n``````\n",
    "created_at": "2020-06-25T01:54:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29942#issuecomment-476617",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -20,7 +20,7 @@
 ... never finishes ...
 ```
 
-This branch makes the running time roughtly proportional to the size of the multiset:
+This branch makes the running time roughly proportional to the size of the multiset:
 
 ```
 sage: mset = list(range(10)) * 3
``````




---

archive/issue_comments_476618.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nA quick comment for testing coverage. I would add an exhaustive test on something `X` of reasonable size such that\n\n```python\nfor i, p in enumerate(X):\n    assert X.rank(p) == i\n    assert X.unrank(i) == p\n```",
    "created_at": "2020-06-25T23:55:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29942#issuecomment-476618",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:6'>Comment 6:</a>
A quick comment for testing coverage. I would add an exhaustive test on something `X` of reasonable size such that

```python
for i, p in enumerate(X):
    assert X.rank(p) == i
    assert X.unrank(i) == p
```



---

archive/issue_comments_476619.json:
```json
{
    "body": "Changed commit from **[9b9d936](https://github.com/sagemath/sagetrac-mirror/commit/9b9d936081bc4b04d5476ce4c4e54af0997a9295)** to **[a4adb8a](https://github.com/sagemath/sagetrac-mirror/commit/a4adb8a15db91dfdec4d9fe43e3826397c47e607)**",
    "created_at": "2020-06-27T19:41:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29942#issuecomment-476619",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[9b9d936](https://github.com/sagemath/sagetrac-mirror/commit/9b9d936081bc4b04d5476ce4c4e54af0997a9295)** to **[a4adb8a](https://github.com/sagemath/sagetrac-mirror/commit/a4adb8a15db91dfdec4d9fe43e3826397c47e607)**



---

archive/issue_comments_476620.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a4adb8a15db91dfdec4d9fe43e3826397c47e607\">a4adb8a</a></td><td><code>Do an exhaustive rank/unrank roundtrip test.</code></td></tr></table>\n",
    "created_at": "2020-06-27T19:41:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29942#issuecomment-476620",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:7'>Comment 7:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a4adb8a15db91dfdec4d9fe43e3826397c47e607">a4adb8a</a></td><td><code>Do an exhaustive rank/unrank roundtrip test.</code></td></tr></table>




---

archive/issue_comments_476621.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nReplying to [@tscrim](#comment%3A6):\n> A quick comment for testing coverage. I would add an exhaustive test on something `X` of reasonable size\n\nI did so in [4adb8a](https://github.com/sagemath/sagetrac-mirror/commit/a4adb8a15db91dfdec4d9fe43e3826397c47e607) over the multiset `[2, 2, 3, 3, 3, 5, 5, 5, 5, 5]`, which has 2520 permutations. (I wasn't sure what a reasonable size for a test case would be.) In the same loop, I added a check for lexicographic ordering. I had to convert the permutations from a `list` to a `tuple` to facilitate comparison.",
    "created_at": "2020-06-27T19:48:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29942#issuecomment-476621",
    "user": "https://github.com/sagetrac-dcfifield"
}
```

<a id='comment:8'>Comment 8:</a>
Replying to [@tscrim](#comment%3A6):
> A quick comment for testing coverage. I would add an exhaustive test on something `X` of reasonable size

I did so in [4adb8a](https://github.com/sagemath/sagetrac-mirror/commit/a4adb8a15db91dfdec4d9fe43e3826397c47e607) over the multiset `[2, 2, 3, 3, 3, 5, 5, 5, 5, 5]`, which has 2520 permutations. (I wasn't sure what a reasonable size for a test case would be.) In the same loop, I added a check for lexicographic ordering. I had to convert the permutations from a `list` to a `tuple` to facilitate comparison.



---

archive/issue_comments_476622.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nI think that is a reasonable size; thank you.\n\nI just have three more minor changes:\n\n```diff\n-        - ``p`` -- a permutation of `M`.\n+        - ``p`` -- a permutation of `M`\n```\n\n```diff\n         - ``r`` -- an integer between ``0`` and ``self.cardinality()-1``\n-          inclusive.\n+          inclusive\n```\nUpdate the Knuth reference based off #30005.\n\nSomething that is stylistic, so it is up to you if you want to change it, but I would change the\n\n```\n-Some sentence explanation. ::\n+Some sentence explanation::\n```\nto have it end each block with a colon as the code is better associated with the sentence. However, that is something you can ignore.\n\nFr\u00e9d\u00e9ric, do you have any other comments?",
    "created_at": "2020-07-01T08:00:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29942#issuecomment-476622",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:9'>Comment 9:</a>
I think that is a reasonable size; thank you.

I just have three more minor changes:

```diff
-        - ``p`` -- a permutation of `M`.
+        - ``p`` -- a permutation of `M`
```

```diff
         - ``r`` -- an integer between ``0`` and ``self.cardinality()-1``
-          inclusive.
+          inclusive
```
Update the Knuth reference based off #30005.

Something that is stylistic, so it is up to you if you want to change it, but I would change the

```
-Some sentence explanation. ::
+Some sentence explanation::
```
to have it end each block with a colon as the code is better associated with the sentence. However, that is something you can ignore.

Frédéric, do you have any other comments?



---

archive/issue_comments_476623.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton, Travis Scrimshaw**",
    "created_at": "2020-07-01T08:00:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29942#issuecomment-476623",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Frédéric Chapoton, Travis Scrimshaw**



---

archive/issue_comments_476624.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nok, looks good to me too. The patchbot warning are false-positive. Once Travis suggestions are fixed, this can be set to positive and will be a very nice improvement.",
    "created_at": "2020-07-01T20:00:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29942#issuecomment-476624",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:10'>Comment 10:</a>
ok, looks good to me too. The patchbot warning are false-positive. Once Travis suggestions are fixed, this can be set to positive and will be a very nice improvement.



---

archive/issue_comments_476625.json:
```json
{
    "body": "Changed commit from **[a4adb8a](https://github.com/sagemath/sagetrac-mirror/commit/a4adb8a15db91dfdec4d9fe43e3826397c47e607)** to **[a694a35](https://github.com/sagemath/sagetrac-mirror/commit/a694a352f26f0c6236334f485a6c0e7237eecabc)**",
    "created_at": "2020-07-04T17:33:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29942#issuecomment-476625",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[a4adb8a](https://github.com/sagemath/sagetrac-mirror/commit/a4adb8a15db91dfdec4d9fe43e3826397c47e607)** to **[a694a35](https://github.com/sagemath/sagetrac-mirror/commit/a694a352f26f0c6236334f485a6c0e7237eecabc)**



---

archive/issue_comments_476626.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/35b6e192112c01cd50fdd357efd6c1e0d4fb78ec\">35b6e19</a></td><td><code>Update a variable reference in a comment.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a694a352f26f0c6236334f485a6c0e7237eecabc\">a694a35</a></td><td><code>Doc improvements in Permutations_mset rank and unrank.</code></td></tr></table>\n",
    "created_at": "2020-07-04T17:33:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29942#issuecomment-476626",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:11'>Comment 11:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/35b6e192112c01cd50fdd357efd6c1e0d4fb78ec">35b6e19</a></td><td><code>Update a variable reference in a comment.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a694a352f26f0c6236334f485a6c0e7237eecabc">a694a35</a></td><td><code>Doc improvements in Permutations_mset rank and unrank.</code></td></tr></table>




---

archive/issue_comments_476627.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nReplying to [@tscrim](#comment%3A9):\n> \n> ```diff\n> -        - ``p`` -- a permutation of `M`.\n> +        - ``p`` -- a permutation of `M`\n> ```\n> \n> ```diff\n>          - ``r`` -- an integer between ``0`` and ``self.cardinality()-1``\n> -          inclusive.\n> +          inclusive\n> ```\n> \n> ```\n> -Some sentence explanation. ::\n> +Some sentence explanation::\n> ```\n\nI made these changes in [\u200ba694a35](https://github.com/sagemath/sagetrac-mirror/commit/a694a352f26f0c6236334f485a6c0e7237eecabc).\n\n> Update the Knuth reference based off #30005.\n\nIt seems the right way to handle this is to remove the [REFERENCES](https://github.com/sagemath/sagetrac-mirror/blob/develop/src/sage/combinat/gray_codes.py?h=b45d5fb82ba163f5df09223cfc27327dd0b58f44#n4) section from gray_codes.py, containing the [Knuth-TAOCP4A] reference used in that file only, and update [Knuth-TAOCP4A] references to point to the new [https://github.com/sagemath/sagetrac-mirror/blob/develop/src/doc/en/reference/references/index.rst?id=a694a352f26f0c6236334f485a6c0e7237eecabc#n3216 \"[Knu2011]\"] in references/index.rst. But #30005 is not merged yet and [Knu2011] does not exist in its branch; while modifying gray_codes.py in this branch may prevent #30005 from merging cleanly. How should I handle this?",
    "created_at": "2020-07-04T17:46:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29942#issuecomment-476627",
    "user": "https://github.com/sagetrac-dcfifield"
}
```

<a id='comment:12'>Comment 12:</a>
Replying to [@tscrim](#comment%3A9):
> 
> ```diff
> -        - ``p`` -- a permutation of `M`.
> +        - ``p`` -- a permutation of `M`
> ```
> 
> ```diff
>          - ``r`` -- an integer between ``0`` and ``self.cardinality()-1``
> -          inclusive.
> +          inclusive
> ```
> 
> ```
> -Some sentence explanation. ::
> +Some sentence explanation::
> ```

I made these changes in [​a694a35](https://github.com/sagemath/sagetrac-mirror/commit/a694a352f26f0c6236334f485a6c0e7237eecabc).

> Update the Knuth reference based off #30005.

It seems the right way to handle this is to remove the [REFERENCES](https://github.com/sagemath/sagetrac-mirror/blob/develop/src/sage/combinat/gray_codes.py?h=b45d5fb82ba163f5df09223cfc27327dd0b58f44#n4) section from gray_codes.py, containing the [Knuth-TAOCP4A] reference used in that file only, and update [Knuth-TAOCP4A] references to point to the new [https://github.com/sagemath/sagetrac-mirror/blob/develop/src/doc/en/reference/references/index.rst?id=a694a352f26f0c6236334f485a6c0e7237eecabc#n3216 "[Knu2011]"] in references/index.rst. But #30005 is not merged yet and [Knu2011] does not exist in its branch; while modifying gray_codes.py in this branch may prevent #30005 from merging cleanly. How should I handle this?



---

archive/issue_comments_476628.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nReplying to [dcfifield](#comment%3A12):\n> Replying to [@tscrim](#comment%3A9):\n> > Update the Knuth reference based off #30005.\n\n> \n> It seems the right way to handle this is to remove the [REFERENCES](https://github.com/sagemath/sagetrac-mirror/blob/develop/src/sage/combinat/gray_codes.py?h=b45d5fb82ba163f5df09223cfc27327dd0b58f44#n4) section from gray_codes.py, containing the [Knuth-TAOCP4A] reference used in that file only, and update [Knuth-TAOCP4A] references to point to the new [https://github.com/sagemath/sagetrac-mirror/blob/develop/src/doc/en/reference/references/index.rst?id=a694a352f26f0c6236334f485a6c0e7237eecabc#n3216 \"[Knu2011]\"] in references/index.rst. But #30005 is not merged yet and [Knu2011] does not exist in its branch; while modifying gray_codes.py in this branch may prevent #30005 from merging cleanly. How should I handle this?\n\nYou merge in the branch from #30005 into this one and set it as a dependency in the ticket. Actually, you probably should remove that reference on this ticket (again, after merging in #30005) and use the one in the master reference file.",
    "created_at": "2020-07-04T21:53:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29942#issuecomment-476628",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:13'>Comment 13:</a>
Replying to [dcfifield](#comment%3A12):
> Replying to [@tscrim](#comment%3A9):
> > Update the Knuth reference based off #30005.

> 
> It seems the right way to handle this is to remove the [REFERENCES](https://github.com/sagemath/sagetrac-mirror/blob/develop/src/sage/combinat/gray_codes.py?h=b45d5fb82ba163f5df09223cfc27327dd0b58f44#n4) section from gray_codes.py, containing the [Knuth-TAOCP4A] reference used in that file only, and update [Knuth-TAOCP4A] references to point to the new [https://github.com/sagemath/sagetrac-mirror/blob/develop/src/doc/en/reference/references/index.rst?id=a694a352f26f0c6236334f485a6c0e7237eecabc#n3216 "[Knu2011]"] in references/index.rst. But #30005 is not merged yet and [Knu2011] does not exist in its branch; while modifying gray_codes.py in this branch may prevent #30005 from merging cleanly. How should I handle this?

You merge in the branch from #30005 into this one and set it as a dependency in the ticket. Actually, you probably should remove that reference on this ticket (again, after merging in #30005) and use the one in the master reference file.



---

archive/issue_comments_476629.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/91990d0e1a3b6d934725cb9dfd4500aefebf7da9\">91990d0</a></td><td><code>Fix variable name in StandardPermutations_n.rank exception message</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f424a248c31e96a2dfcb9f8694cb7d9f5f1d3613\">f424a24</a></td><td><code>Typo fixes in src/sage/combinat/gray_codes.py.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b45d5fb82ba163f5df09223cfc27327dd0b58f44\">b45d5fb</a></td><td><code>Update references in src/sage/combinat/gray_codes.py.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9577bdc112afa8da9309cd075a6bbc434aba84a6\">9577bdc</a></td><td><code>Merge branch 'combinat_doc_fixes' into permutations_mset_rank_unrank</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/141f9a97c16facf1780bf14402915d68d814f354\">141f9a9</a></td><td><code>Point references in combinat/gray_codes.py to the master bibliography file.</code></td></tr></table>\n",
    "created_at": "2020-07-04T23:22:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29942#issuecomment-476629",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:14'>Comment 14:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/91990d0e1a3b6d934725cb9dfd4500aefebf7da9">91990d0</a></td><td><code>Fix variable name in StandardPermutations_n.rank exception message</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f424a248c31e96a2dfcb9f8694cb7d9f5f1d3613">f424a24</a></td><td><code>Typo fixes in src/sage/combinat/gray_codes.py.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b45d5fb82ba163f5df09223cfc27327dd0b58f44">b45d5fb</a></td><td><code>Update references in src/sage/combinat/gray_codes.py.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9577bdc112afa8da9309cd075a6bbc434aba84a6">9577bdc</a></td><td><code>Merge branch 'combinat_doc_fixes' into permutations_mset_rank_unrank</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/141f9a97c16facf1780bf14402915d68d814f354">141f9a9</a></td><td><code>Point references in combinat/gray_codes.py to the master bibliography file.</code></td></tr></table>




---

archive/issue_comments_476630.json:
```json
{
    "body": "Changed commit from **[a694a35](https://github.com/sagemath/sagetrac-mirror/commit/a694a352f26f0c6236334f485a6c0e7237eecabc)** to **[141f9a9](https://github.com/sagemath/sagetrac-mirror/commit/141f9a97c16facf1780bf14402915d68d814f354)**",
    "created_at": "2020-07-04T23:22:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29942#issuecomment-476630",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[a694a35](https://github.com/sagemath/sagetrac-mirror/commit/a694a352f26f0c6236334f485a6c0e7237eecabc)** to **[141f9a9](https://github.com/sagemath/sagetrac-mirror/commit/141f9a97c16facf1780bf14402915d68d814f354)**



---

archive/issue_comments_476631.json:
```json
{
    "body": "Dependencies: **#30005**",
    "created_at": "2020-07-04T23:28:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29942#issuecomment-476631",
    "user": "https://github.com/sagetrac-dcfifield"
}
```

Dependencies: **#30005**



---

archive/issue_comments_476632.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nReplying to [@tscrim](#comment%3A13):\n> You merge in the branch from #30005 into this one and set it as a dependency in the ticket.\n\nThanks for your guidance. I've merged #30005 into this branch and made it a dependency.\n\n> Actually, you probably should remove that reference on this ticket (again, after merging in #30005) and use the one in the master reference file.\n\nIt was actually the other way around. It is this ticket (#29942) that adds a reference to the master reference file, and #30005 that used a REFERENCES section local to gray_codes.py. In [141f9a9](https://github.com/sagemath/sagetrac-mirror/commit/141f9a97c16facf1780bf14402915d68d814f354) I edited gray_codes.py to point to the master reference file.",
    "created_at": "2020-07-04T23:28:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29942#issuecomment-476632",
    "user": "https://github.com/sagetrac-dcfifield"
}
```

<a id='comment:15'>Comment 15:</a>
Replying to [@tscrim](#comment%3A13):
> You merge in the branch from #30005 into this one and set it as a dependency in the ticket.

Thanks for your guidance. I've merged #30005 into this branch and made it a dependency.

> Actually, you probably should remove that reference on this ticket (again, after merging in #30005) and use the one in the master reference file.

It was actually the other way around. It is this ticket (#29942) that adds a reference to the master reference file, and #30005 that used a REFERENCES section local to gray_codes.py. In [141f9a9](https://github.com/sagemath/sagetrac-mirror/commit/141f9a97c16facf1780bf14402915d68d814f354) I edited gray_codes.py to point to the master reference file.



---

archive/issue_comments_476633.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nReplying to [dcfifield](#comment%3A15):\n> Replying to [@tscrim](#comment%3A13):\n> > Actually, you probably should remove that reference on this ticket (again, after merging in #30005) and use the one in the master reference file.\n\n> \n> It was actually the other way around. It is this ticket (#29942) that adds a reference to the master reference file, and #30005 that used a REFERENCES section local to gray_codes.py. In [141f9a9](https://github.com/sagemath/sagetrac-mirror/commit/141f9a97c16facf1780bf14402915d68d814f354) I edited gray_codes.py to point to the master reference file.\n\nThe reason I was saying that was because #30005 was positively reviewed (and not clear there should be a dependency relation). However, I am happy with the changes here. I will just wait for the patchbot one more time.",
    "created_at": "2020-07-04T23:39:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29942#issuecomment-476633",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:16'>Comment 16:</a>
Replying to [dcfifield](#comment%3A15):
> Replying to [@tscrim](#comment%3A13):
> > Actually, you probably should remove that reference on this ticket (again, after merging in #30005) and use the one in the master reference file.

> 
> It was actually the other way around. It is this ticket (#29942) that adds a reference to the master reference file, and #30005 that used a REFERENCES section local to gray_codes.py. In [141f9a9](https://github.com/sagemath/sagetrac-mirror/commit/141f9a97c16facf1780bf14402915d68d814f354) I edited gray_codes.py to point to the master reference file.

The reason I was saying that was because #30005 was positively reviewed (and not clear there should be a dependency relation). However, I am happy with the changes here. I will just wait for the patchbot one more time.



---

archive/issue_comments_476634.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nFinished testing locally. LGTM.",
    "created_at": "2020-07-06T00:13:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29942#issuecomment-476634",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:17'>Comment 17:</a>
Finished testing locally. LGTM.



---

archive/issue_events_393942.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-07-06T00:13:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29942#event-393942"
}
```



---

archive/issue_events_393943.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-07-06T00:13:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29942#event-393943"
}
```



---

archive/issue_events_393944.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-07-10T19:33:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29942#event-393944"
}
```



---

archive/issue_events_393945.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "a0d17af00271be73aa7dd23b38153923592009af",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-07-10T19:33:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29942#event-393945"
}
```



---

archive/issue_comments_476635.json:
```json
{
    "body": "Changed branch from **[u/dcfifield/permutations_mset_rank_unrank](https://github.com/sagemath/sagetrac-mirror/tree/u/dcfifield/permutations_mset_rank_unrank)** to **[141f9a9](https://github.com/sagemath/sagetrac-mirror/commit/141f9a97c16facf1780bf14402915d68d814f354)**",
    "created_at": "2020-07-10T19:33:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29942",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29942#issuecomment-476635",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/dcfifield/permutations_mset_rank_unrank](https://github.com/sagemath/sagetrac-mirror/tree/u/dcfifield/permutations_mset_rank_unrank)** to **[141f9a9](https://github.com/sagemath/sagetrac-mirror/commit/141f9a97c16facf1780bf14402915d68d814f354)**
