# Issue 29533: Remove defunct SAGE_PORT mechanism

archive/issues_029296.json:
```json
{
    "assignees": [],
    "body": "We remove the last traces of the `SAGE_PORT` mechanism from the source and from the developer guide.\n\n`SAGE_PORT` stopped working when the build process started to depend on `configure` output files. \n\n(from #29316)\n\n\nCC:  @dimpase @jhpalmieri\n\nBranch/Commit: **[c9758e9](https://github.com/sagemath/sagetrac-mirror/commit/c9758e913c383301c6937dc98d3566a8784dd133)**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29533_\n\n",
    "closed_at": "2020-04-23T22:32:38Z",
    "created_at": "2020-04-19T18:33:46Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build%3A%20configure",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove defunct SAGE_PORT mechanism",
    "type": "issue",
    "updated_at": "2020-04-23T22:32:38Z",
    "url": "https://github.com/sagemath/sage/issues/29533",
    "user": "https://github.com/mkoeppe"
}
```
We remove the last traces of the `SAGE_PORT` mechanism from the source and from the developer guide.

`SAGE_PORT` stopped working when the build process started to depend on `configure` output files. 

(from #29316)


CC:  @dimpase @jhpalmieri

Branch/Commit: **[c9758e9](https://github.com/sagemath/sagetrac-mirror/commit/c9758e913c383301c6937dc98d3566a8784dd133)**

Reviewer: **Dima Pasechnik**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/29533_





---

archive/issue_events_387943.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-19T18:33:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29533",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29533#event-387943"
}
```



---

archive/issue_events_387944.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-19T18:33:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29533",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build%3A%20configure",
    "label_color": "000080",
    "label_name": "component: build: configure",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29533#event-387944"
}
```



---

archive/issue_events_387945.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-19T18:33:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29533",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29533#event-387945"
}
```



---

archive/issue_events_387946.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-19T18:33:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29533",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29533#event-387946"
}
```



---

archive/issue_comments_467336.json:
```json
{
    "body": "Branch: **[u/mkoeppe/remove_defunct_sage_port_mechanism](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/remove_defunct_sage_port_mechanism)**",
    "created_at": "2020-04-19T18:43:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29533",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29533#issuecomment-467336",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/remove_defunct_sage_port_mechanism](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/remove_defunct_sage_port_mechanism)**



---

archive/issue_events_387947.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-19T18:44:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29533",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29533#event-387947"
}
```



---

archive/issue_comments_467337.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c9758e913c383301c6937dc98d3566a8784dd133\">c9758e9</a></td><td><code>Makefile, src/doc/en/installation/source.rst: Remove defunct SAGE_PORT mechanism</code></td></tr></table>\n",
    "created_at": "2020-04-19T18:44:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29533",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29533#issuecomment-467337",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c9758e913c383301c6937dc98d3566a8784dd133">c9758e9</a></td><td><code>Makefile, src/doc/en/installation/source.rst: Remove defunct SAGE_PORT mechanism</code></td></tr></table>




---

archive/issue_comments_467338.json:
```json
{
    "body": "Commit: **[c9758e9](https://github.com/sagemath/sagetrac-mirror/commit/c9758e913c383301c6937dc98d3566a8784dd133)**",
    "created_at": "2020-04-19T18:44:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29533",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29533#issuecomment-467338",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[c9758e9](https://github.com/sagemath/sagetrac-mirror/commit/c9758e913c383301c6937dc98d3566a8784dd133)**



---

archive/issue_comments_467339.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2020-04-19T18:44:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29533",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29533#issuecomment-467339",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_events_387948.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-04-20T03:52:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29533",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29533#event-387948"
}
```



---

archive/issue_events_387949.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-04-20T03:52:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29533",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29533#event-387949"
}
```



---

archive/issue_comments_467340.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2020-04-20T03:52:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29533",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29533#issuecomment-467340",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_comments_467341.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nlgtm, thanks!",
    "created_at": "2020-04-20T03:52:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29533",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29533#issuecomment-467341",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:4" align="right">Comment 4</div>

lgtm, thanks!



---

archive/issue_comments_467342.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThanks",
    "created_at": "2020-04-20T06:21:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29533",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29533#issuecomment-467342",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:5" align="right">Comment 5</div>

Thanks



---

archive/issue_comments_467343.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/remove_defunct_sage_port_mechanism](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/remove_defunct_sage_port_mechanism)** to **[c9758e9](https://github.com/sagemath/sagetrac-mirror/commit/c9758e913c383301c6937dc98d3566a8784dd133)**",
    "created_at": "2020-04-23T22:32:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29533",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29533#issuecomment-467343",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/remove_defunct_sage_port_mechanism](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/remove_defunct_sage_port_mechanism)** to **[c9758e9](https://github.com/sagemath/sagetrac-mirror/commit/c9758e913c383301c6937dc98d3566a8784dd133)**



---

archive/issue_events_387950.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-04-23T22:32:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29533",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29533#event-387950"
}
```



---

archive/issue_events_387951.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "e394130700bf6754f693b29745304bf3dbfb574f",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-04-23T22:32:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29533",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29533#event-387951"
}
```
