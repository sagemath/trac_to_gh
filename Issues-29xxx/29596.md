# Issue 29596: Add rpath-link on Linux

archive/issues_029359.json:
```json
{
    "assignees": [],
    "body": "This should fix some of the errors in #29327\n\nThis is needed because on conda, the flag `-Wl,--as-needed` is used by default.\n\nCC:  @mkoeppe @dimpase\n\nBranch/Commit: **[87231f8](https://github.com/sagemath/sagetrac-mirror/commit/87231f811388c55cc4f499d86b482d5922e1a8ba)**\n\nReviewer: **Matthias Koeppe**\n\nAuthor: **Isuru Fernando**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29596_\n\n",
    "closed_at": "2020-05-04T06:41:23Z",
    "created_at": "2020-04-27T03:23:58Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add rpath-link on Linux",
    "type": "issue",
    "updated_at": "2020-05-04T06:41:23Z",
    "url": "https://github.com/sagemath/sage/issues/29596",
    "user": "https://github.com/isuruf"
}
```
This should fix some of the errors in #29327

This is needed because on conda, the flag `-Wl,--as-needed` is used by default.

CC:  @mkoeppe @dimpase

Branch/Commit: **[87231f8](https://github.com/sagemath/sagetrac-mirror/commit/87231f811388c55cc4f499d86b482d5922e1a8ba)**

Reviewer: **Matthias Koeppe**

Author: **Isuru Fernando**

_Issue created by migration from https://trac.sagemath.org/ticket/29596_





---

archive/issue_events_388872.json:
```json
{
    "actor": "https://github.com/isuruf",
    "created_at": "2020-04-27T03:23:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29596",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29596#event-388872"
}
```



---

archive/issue_events_388873.json:
```json
{
    "actor": "https://github.com/isuruf",
    "created_at": "2020-04-27T03:23:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29596",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29596#event-388873"
}
```



---

archive/issue_events_388874.json:
```json
{
    "actor": "https://github.com/isuruf",
    "created_at": "2020-04-27T03:23:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29596",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29596#event-388874"
}
```



---

archive/issue_events_388875.json:
```json
{
    "actor": "https://github.com/isuruf",
    "created_at": "2020-04-27T03:23:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29596",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29596#event-388875"
}
```



---

archive/issue_events_388876.json:
```json
{
    "actor": "https://github.com/isuruf",
    "created_at": "2020-04-27T03:24:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29596",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29596#event-388876"
}
```



---

archive/issue_comments_468759.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThanks a lot! I'll test this",
    "created_at": "2020-04-27T03:35:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29596#issuecomment-468759",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:2'>Comment 2:</a>
Thanks a lot! I'll test this



---

archive/issue_comments_468760.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2020-04-27T03:50:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29596#issuecomment-468760",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_comments_468761.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nReplying to [@mkoeppe](#comment%3A2):\n> Thanks a lot! I'll test this\n\nTests run at https://github.com/mkoeppe/sage/actions/runs/88744943",
    "created_at": "2020-04-27T03:52:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29596#issuecomment-468761",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:4'>Comment 4:</a>
Replying to [@mkoeppe](#comment%3A2):
> Thanks a lot! I'll test this

Tests run at https://github.com/mkoeppe/sage/actions/runs/88744943



---

archive/issue_events_388877.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-29T04:24:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29596",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29596#event-388877"
}
```



---

archive/issue_events_388878.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-29T04:24:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29596",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29596#event-388878"
}
```



---

archive/issue_events_388879.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-05-01T05:51:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29596",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29596#event-388879"
}
```



---

archive/issue_events_388880.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-05-01T05:51:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29596",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29596#event-388880"
}
```



---

archive/issue_events_388881.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-05-04T06:41:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29596",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29596#event-388881"
}
```



---

archive/issue_events_388882.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-05-04T06:41:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29596",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29596#event-388882"
}
```



---

archive/issue_comments_468762.json:
```json
{
    "body": "Changed branch from **[u/isuruf/rpath_link](https://github.com/sagemath/sagetrac-mirror/tree/u/isuruf/rpath_link)** to **[87231f8](https://github.com/sagemath/sagetrac-mirror/commit/87231f811388c55cc4f499d86b482d5922e1a8ba)**",
    "created_at": "2020-05-04T06:41:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29596#issuecomment-468762",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/isuruf/rpath_link](https://github.com/sagemath/sagetrac-mirror/tree/u/isuruf/rpath_link)** to **[87231f8](https://github.com/sagemath/sagetrac-mirror/commit/87231f811388c55cc4f499d86b482d5922e1a8ba)**
