# Issue 29843: Set up linear transformation with precomputed data if injective

archive/issues_029606.json:
```json
{
    "assignees": [],
    "body": "We set up linear transformation with double description, if it preserves the polytope.\n\nAlso we slightly modify the affine hull projection to make use of this.\n\nBefore this ticket:\n\n(Run tests twice to obtain the real timinigs. The first time it computes twice to populate the coercion model, see https://groups.google.com/d/msg/sage-devel/UAvbXKSN_JU/vcVN2AVQBQAJ)\n\n```\nsage: P = polytopes.permutahedron(6)\nsage: %time Q = P.affine_hull_projection()\nCPU times: user 94.1 ms, sys: 2 \u00b5s, total: 94.1 ms\nWall time: 93.5 ms\n\nsage: P = polytopes.permutahedron(7)\nsage: %time Q = P.affine_hull_projection()\nCPU times: user 2.21 s, sys: 1 \u00b5s, total: 2.21 s\nWall time: 2.21 s\n\nsage: P = polytopes.permutahedron(4)\nsage: %time Q = P.affine_hull_projection(orthonormal=True, extend=True)\nCPU times: user 1.36 s, sys: 2 \u00b5s, total: 1.36 s\nWall time: 1.36 s\n\nsage: P = polytopes.hypercube(13)*Polyhedron(vertices=[[0]])\nsage: %time Q = P.affine_hull_projection()\nCPU times: user 1.84 s, sys: 5 \u00b5s, total: 1.84 s\nWall time: 1.84 s\n```\n\nWith this ticket on top of #29841 (this tickets new implementation uses `CombinatorialPolyhedron` which is much faster obtained with #29841):\n\n```\nsage: P = polytopes.permutahedron(6)\nsage: %time Q = P.affine_hull_projection()\nCPU times: user 78.9 ms, sys: 0 ns, total: 78.9 ms\nWall time: 77.7 ms\nsage: P = polytopes.permutahedron(7)\nsage: %time Q = P.affine_hull_projection()\nCPU times: user 1.23 s, sys: 35 \u00b5s, total: 1.23 s\nWall time: 1.23 s\n\nsage: P = polytopes.permutahedron(7)\nsage: P = P.base_extend(P.base_ring(), backend='field')\nsage: %time Q = P.affine_hull_projection()\nCPU times: user 366 ms, sys: 3.98 ms, total: 370 ms\nWall time: 369 ms\n\nsage: P = polytopes.permutahedron(4) \nsage: %time Q = P.affine_hull_projection(orthonormal=True, extend=True)\nCPU times: user 187 ms, sys: 10.8 ms, total: 198 ms\nWall time: 196 ms\n\nsage: P = polytopes.hypercube(13)*Polyhedron(vertices=[[0]])\nsage: %time Q = P.affine_hull_projection()\nCPU times: user 847 ms, sys: 36 \u00b5s, total: 848 ms\nWall time: 846 ms\n```\n\nCC:  @jplab @LaisRast\n\nBranch/Commit: **[28819ea](https://github.com/sagemath/sagetrac-mirror/commit/28819ea12c90d3a9c268f0913ef0bc21ec3c0ff3)**\n\nReviewer: **Matthias Koeppe**\n\nAuthor: **Jonathan Kliem**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29843_\n\n",
    "closed_at": "2020-07-10T19:34:00Z",
    "created_at": "2020-06-10T19:42:35Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Set up linear transformation with precomputed data if injective",
    "type": "issue",
    "updated_at": "2020-07-10T19:34:00Z",
    "url": "https://github.com/sagemath/sage/issues/29843",
    "user": "https://github.com/kliem"
}
```
We set up linear transformation with double description, if it preserves the polytope.

Also we slightly modify the affine hull projection to make use of this.

Before this ticket:

(Run tests twice to obtain the real timinigs. The first time it computes twice to populate the coercion model, see https://groups.google.com/d/msg/sage-devel/UAvbXKSN_JU/vcVN2AVQBQAJ)

```
sage: P = polytopes.permutahedron(6)
sage: %time Q = P.affine_hull_projection()
CPU times: user 94.1 ms, sys: 2 µs, total: 94.1 ms
Wall time: 93.5 ms

sage: P = polytopes.permutahedron(7)
sage: %time Q = P.affine_hull_projection()
CPU times: user 2.21 s, sys: 1 µs, total: 2.21 s
Wall time: 2.21 s

sage: P = polytopes.permutahedron(4)
sage: %time Q = P.affine_hull_projection(orthonormal=True, extend=True)
CPU times: user 1.36 s, sys: 2 µs, total: 1.36 s
Wall time: 1.36 s

sage: P = polytopes.hypercube(13)*Polyhedron(vertices=[[0]])
sage: %time Q = P.affine_hull_projection()
CPU times: user 1.84 s, sys: 5 µs, total: 1.84 s
Wall time: 1.84 s
```

With this ticket on top of #29841 (this tickets new implementation uses `CombinatorialPolyhedron` which is much faster obtained with #29841):

```
sage: P = polytopes.permutahedron(6)
sage: %time Q = P.affine_hull_projection()
CPU times: user 78.9 ms, sys: 0 ns, total: 78.9 ms
Wall time: 77.7 ms
sage: P = polytopes.permutahedron(7)
sage: %time Q = P.affine_hull_projection()
CPU times: user 1.23 s, sys: 35 µs, total: 1.23 s
Wall time: 1.23 s

sage: P = polytopes.permutahedron(7)
sage: P = P.base_extend(P.base_ring(), backend='field')
sage: %time Q = P.affine_hull_projection()
CPU times: user 366 ms, sys: 3.98 ms, total: 370 ms
Wall time: 369 ms

sage: P = polytopes.permutahedron(4) 
sage: %time Q = P.affine_hull_projection(orthonormal=True, extend=True)
CPU times: user 187 ms, sys: 10.8 ms, total: 198 ms
Wall time: 196 ms

sage: P = polytopes.hypercube(13)*Polyhedron(vertices=[[0]])
sage: %time Q = P.affine_hull_projection()
CPU times: user 847 ms, sys: 36 µs, total: 848 ms
Wall time: 846 ms
```

CC:  @jplab @LaisRast

Branch/Commit: **[28819ea](https://github.com/sagemath/sagetrac-mirror/commit/28819ea12c90d3a9c268f0913ef0bc21ec3c0ff3)**

Reviewer: **Matthias Koeppe**

Author: **Jonathan Kliem**

_Issue created by migration from https://trac.sagemath.org/ticket/29843_





---

archive/issue_events_392447.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-06-10T19:42:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29843",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29843#event-392447"
}
```



---

archive/issue_events_392448.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-06-10T19:42:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29843",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "component: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29843#event-392448"
}
```



---

archive/issue_events_392449.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-06-10T19:42:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29843",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29843#event-392449"
}
```



---

archive/issue_events_392450.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-06-10T19:42:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29843",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29843#event-392450"
}
```



---

archive/issue_comments_474153.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/28819ea12c90d3a9c268f0913ef0bc21ec3c0ff3\">28819ea</a></td><td><code>set up linear transformation with both Vrep and Hrep, if possible</code></td></tr></table>\n",
    "created_at": "2020-06-10T19:44:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29843#issuecomment-474153",
    "user": "https://github.com/kliem"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/28819ea12c90d3a9c268f0913ef0bc21ec3c0ff3">28819ea</a></td><td><code>set up linear transformation with both Vrep and Hrep, if possible</code></td></tr></table>




---

archive/issue_events_392451.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-06-10T19:44:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29843",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29843#event-392451"
}
```



---

archive/issue_comments_474154.json:
```json
{
    "body": "Commit: **[28819ea](https://github.com/sagemath/sagetrac-mirror/commit/28819ea12c90d3a9c268f0913ef0bc21ec3c0ff3)**",
    "created_at": "2020-06-10T19:44:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29843#issuecomment-474154",
    "user": "https://github.com/kliem"
}
```

Commit: **[28819ea](https://github.com/sagemath/sagetrac-mirror/commit/28819ea12c90d3a9c268f0913ef0bc21ec3c0ff3)**



---

archive/issue_comments_474155.json:
```json
{
    "body": "Branch: **[public/29843](https://github.com/sagemath/sagetrac-mirror/tree/public/29843)**",
    "created_at": "2020-06-10T19:44:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29843#issuecomment-474155",
    "user": "https://github.com/kliem"
}
```

Branch: **[public/29843](https://github.com/sagemath/sagetrac-mirror/tree/public/29843)**



---

archive/issue_comments_474156.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2020-07-04T20:19:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29843#issuecomment-474156",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_events_392452.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-07-04T20:19:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29843",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29843#event-392452"
}
```



---

archive/issue_events_392453.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-07-04T20:19:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29843",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29843#event-392453"
}
```



---

archive/issue_comments_474157.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThank you.",
    "created_at": "2020-07-05T06:03:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29843#issuecomment-474157",
    "user": "https://github.com/kliem"
}
```

<a id='comment:3'>Comment 3:</a>
Thank you.



---

archive/issue_comments_474158.json:
```json
{
    "body": "Changed branch from **[public/29843](https://github.com/sagemath/sagetrac-mirror/tree/public/29843)** to **[28819ea](https://github.com/sagemath/sagetrac-mirror/commit/28819ea12c90d3a9c268f0913ef0bc21ec3c0ff3)**",
    "created_at": "2020-07-10T19:34:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29843#issuecomment-474158",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/29843](https://github.com/sagemath/sagetrac-mirror/tree/public/29843)** to **[28819ea](https://github.com/sagemath/sagetrac-mirror/commit/28819ea12c90d3a9c268f0913ef0bc21ec3c0ff3)**



---

archive/issue_events_392454.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-07-10T19:34:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29843",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29843#event-392454"
}
```



---

archive/issue_events_392455.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "66a033851d86a853b93592cef0439252c071932c",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-07-10T19:34:00Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29843",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29843#event-392455"
}
```
