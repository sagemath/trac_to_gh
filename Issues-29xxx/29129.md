# Issue 29129: Add more system packages to build/pkgs/SPKG/distros/

archive/issues_028892.json:
```json
{
    "assignees": [],
    "body": "As a follow-up to #29053, more packages should be added:\n\nR - see https://github.com/sagemath/sagetrac-mirror/commit/0bad8b4e936ddffb430a71ce27b24677af9c4087\n\nMany Fedora packages missing.\n- `diffutils` needed for `pcre` configure (https://github.com/mkoeppe/sage/runs/430594157?check_suite_focus=true)\n\n\nCC:  @dimpase @jhpalmieri @orlitzky @kiwifb\n\nBranch/Commit: **[c49acdc](https://github.com/sagemath/sagetrac-mirror/commit/c49acdce35a9d834e8525f5612c6f778f01a12a0)**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29129_\n\n",
    "closed_at": "2020-03-11T23:46:22Z",
    "created_at": "2020-01-31T22:51:43Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add more system packages to build/pkgs/SPKG/distros/",
    "type": "issue",
    "updated_at": "2020-03-11T23:46:22Z",
    "url": "https://github.com/sagemath/sage/issues/29129",
    "user": "https://github.com/mkoeppe"
}
```
As a follow-up to #29053, more packages should be added:

R - see https://github.com/sagemath/sagetrac-mirror/commit/0bad8b4e936ddffb430a71ce27b24677af9c4087

Many Fedora packages missing.
- `diffutils` needed for `pcre` configure (https://github.com/mkoeppe/sage/runs/430594157?check_suite_focus=true)


CC:  @dimpase @jhpalmieri @orlitzky @kiwifb

Branch/Commit: **[c49acdc](https://github.com/sagemath/sagetrac-mirror/commit/c49acdce35a9d834e8525f5612c6f778f01a12a0)**

Reviewer: **Dima Pasechnik**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/29129_





---

archive/issue_events_382148.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-01-31T22:51:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29129#event-382148"
}
```



---

archive/issue_events_382149.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-01-31T22:51:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29129#event-382149"
}
```



---

archive/issue_events_382150.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-01-31T22:51:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29129#event-382150"
}
```



---

archive/issue_events_382151.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-01-31T22:51:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29129#event-382151"
}
```



---

archive/issue_comments_459551.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -3,5 +3,5 @@\n R - see https://github.com/sagemath/sagetrac-mirror/commit/0bad8b4e936ddffb430a71ce27b24677af9c4087\n \n Many Fedora packages missing.\n+- `diffutils` needed for `pcre` configure (https://github.com/mkoeppe/sage/runs/430594157?check_suite_focus=true)\n \n-\n``````\n",
    "created_at": "2020-02-06T21:41:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29129#issuecomment-459551",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -3,5 +3,5 @@
 R - see https://github.com/sagemath/sagetrac-mirror/commit/0bad8b4e936ddffb430a71ce27b24677af9c4087
 
 Many Fedora packages missing.
+- `diffutils` needed for `pcre` configure (https://github.com/mkoeppe/sage/runs/430594157?check_suite_focus=true)
 
-
``````




---

archive/issue_comments_459552.json:
```json
{
    "body": "Branch: **[u/mkoeppe/add_more_system_packages_to_build_pkgs_spkg_distros_](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/add_more_system_packages_to_build_pkgs_spkg_distros_)**",
    "created_at": "2020-02-07T20:51:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29129#issuecomment-459552",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/add_more_system_packages_to_build_pkgs_spkg_distros_](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/add_more_system_packages_to_build_pkgs_spkg_distros_)**



---

archive/issue_comments_459553.json:
```json
{
    "body": "Commit: **[f7785ce](https://github.com/sagemath/sagetrac-mirror/commit/f7785ce08e9335db8ecb817396097362032e65be)**",
    "created_at": "2020-02-07T20:58:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29129#issuecomment-459553",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[f7785ce](https://github.com/sagemath/sagetrac-mirror/commit/f7785ce08e9335db8ecb817396097362032e65be)**



---

archive/issue_comments_459554.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f7785ce08e9335db8ecb817396097362032e65be\">f7785ce</a></td><td><code>build/pkgs/fedora.txt: Add diffutils</code></td></tr></table>\n",
    "created_at": "2020-02-07T20:58:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29129#issuecomment-459554",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:3'>Comment 3:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f7785ce08e9335db8ecb817396097362032e65be">f7785ce</a></td><td><code>build/pkgs/fedora.txt: Add diffutils</code></td></tr></table>




---

archive/issue_comments_459555.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6d2bbd8fb8e36270f16844f5ff9c2071c75f0ff3\">6d2bbd8</a></td><td><code>build/pkgs/arch.txt: Add 'which'</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0aa90c12bb74cfc0bd045f6f189ddc4dbc2732da\">0aa90c1</a></td><td><code>build/pkgs/debian.txt: Add ca-certificates</code></td></tr></table>\n",
    "created_at": "2020-02-12T02:43:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29129#issuecomment-459555",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6d2bbd8fb8e36270f16844f5ff9c2071c75f0ff3">6d2bbd8</a></td><td><code>build/pkgs/arch.txt: Add 'which'</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0aa90c12bb74cfc0bd045f6f189ddc4dbc2732da">0aa90c1</a></td><td><code>build/pkgs/debian.txt: Add ca-certificates</code></td></tr></table>




---

archive/issue_comments_459556.json:
```json
{
    "body": "Changed commit from **[f7785ce](https://github.com/sagemath/sagetrac-mirror/commit/f7785ce08e9335db8ecb817396097362032e65be)** to **[0aa90c1](https://github.com/sagemath/sagetrac-mirror/commit/0aa90c12bb74cfc0bd045f6f189ddc4dbc2732da)**",
    "created_at": "2020-02-12T02:43:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29129#issuecomment-459556",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[f7785ce](https://github.com/sagemath/sagetrac-mirror/commit/f7785ce08e9335db8ecb817396097362032e65be)** to **[0aa90c1](https://github.com/sagemath/sagetrac-mirror/commit/0aa90c12bb74cfc0bd045f6f189ddc4dbc2732da)**



---

archive/issue_comments_459557.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2020-02-22T23:57:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29129#issuecomment-459557",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_events_382152.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-02-22T23:57:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29129#event-382152"
}
```



---

archive/issue_comments_459558.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nGentoo looks OK. Are you sure you don't need libm4ri-devel and gd-devel in some places? For example, I convinced myself that packages with those names exist on openSUSE: https://download.opensuse.org/tumbleweed/repo/oss/x86_64/",
    "created_at": "2020-02-24T22:08:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29129#issuecomment-459558",
    "user": "https://github.com/orlitzky"
}
```

<a id='comment:8'>Comment 8:</a>
Gentoo looks OK. Are you sure you don't need libm4ri-devel and gd-devel in some places? For example, I convinced myself that packages with those names exist on openSUSE: https://download.opensuse.org/tumbleweed/repo/oss/x86_64/



---

archive/issue_comments_459559.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nThanks for checking. \nThe lists in this ticket are certainly incomplete. Any contributions are welcome",
    "created_at": "2020-02-24T22:10:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29129#issuecomment-459559",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:9'>Comment 9:</a>
Thanks for checking. 
The lists in this ticket are certainly incomplete. Any contributions are welcome



---

archive/issue_comments_459560.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nGentoo doesn't do \"dev/devel\" packages - occasionally doc may be a separate package in some circumstances. Sorry, I am flat out this month and won't be able to do any proper testing before March.",
    "created_at": "2020-02-26T04:38:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29129#issuecomment-459560",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:10'>Comment 10:</a>
Gentoo doesn't do "dev/devel" packages - occasionally doc may be a separate package in some circumstances. Sorry, I am flat out this month and won't be able to do any proper testing before March.



---

archive/issue_comments_459561.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nR packages are dealt with on #29273",
    "created_at": "2020-03-03T12:38:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29129#issuecomment-459561",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:11'>Comment 11:</a>
R packages are dealt with on #29273



---

archive/issue_events_382153.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-03-03T23:58:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29129#event-382153"
}
```



---

archive/issue_events_382154.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2020-03-03T23:58:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29129#event-382154"
}
```



---

archive/issue_comments_459562.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nI added few missing here gentoo things on #29273",
    "created_at": "2020-03-03T23:58:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29129#issuecomment-459562",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:12'>Comment 12:</a>
I added few missing here gentoo things on #29273



---

archive/issue_comments_459563.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2020-03-03T23:58:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29129#issuecomment-459563",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_comments_459564.json:
```json
{
    "body": "Changed commit from **[0aa90c1](https://github.com/sagemath/sagetrac-mirror/commit/0aa90c12bb74cfc0bd045f6f189ddc4dbc2732da)** to **[c49acdc](https://github.com/sagemath/sagetrac-mirror/commit/c49acdce35a9d834e8525f5612c6f778f01a12a0)**",
    "created_at": "2020-03-09T00:47:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29129#issuecomment-459564",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[0aa90c1](https://github.com/sagemath/sagetrac-mirror/commit/0aa90c12bb74cfc0bd045f6f189ddc4dbc2732da)** to **[c49acdc](https://github.com/sagemath/sagetrac-mirror/commit/c49acdce35a9d834e8525f5612c6f778f01a12a0)**



---

archive/issue_events_382155.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2020-03-09T00:47:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29129#event-382155"
}
```



---

archive/issue_events_382156.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2020-03-09T00:47:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29129#event-382156"
}
```



---

archive/issue_comments_459565.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nBranch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c49acdce35a9d834e8525f5612c6f778f01a12a0\">c49acdc</a></td><td><code>Merge tag '9.1.beta7' into t/29129/add_more_system_packages_to_build_pkgs_spkg_distros_</code></td></tr></table>\n",
    "created_at": "2020-03-09T00:47:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29129#issuecomment-459565",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:13'>Comment 13:</a>
Branch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c49acdce35a9d834e8525f5612c6f778f01a12a0">c49acdc</a></td><td><code>Merge tag '9.1.beta7' into t/29129/add_more_system_packages_to_build_pkgs_spkg_distros_</code></td></tr></table>




---

archive/issue_events_382157.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-09T00:47:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29129#event-382157"
}
```



---

archive/issue_events_382158.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-03-09T00:47:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29129#event-382158"
}
```



---

archive/issue_events_382159.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-03-11T23:46:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29129#event-382159"
}
```



---

archive/issue_events_382160.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "7873429005c8734f958c52c1bf30264f67156686",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-03-11T23:46:22Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29129#event-382160"
}
```



---

archive/issue_comments_459566.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/add_more_system_packages_to_build_pkgs_spkg_distros_](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/add_more_system_packages_to_build_pkgs_spkg_distros_)** to **[c49acdc](https://github.com/sagemath/sagetrac-mirror/commit/c49acdce35a9d834e8525f5612c6f778f01a12a0)**",
    "created_at": "2020-03-11T23:46:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29129#issuecomment-459566",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/add_more_system_packages_to_build_pkgs_spkg_distros_](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/add_more_system_packages_to_build_pkgs_spkg_distros_)** to **[c49acdc](https://github.com/sagemath/sagetrac-mirror/commit/c49acdce35a9d834e8525f5612c6f778f01a12a0)**
