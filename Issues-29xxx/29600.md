...
```json
{
    "body": "This might be related to: https://github.com/microsoft/WSL/issues/830 (long double floating point calculations give inexact results)",
...
```json
{
    "body": "If everyone agree, I can do the removal (let me CC people involved in #24667).",
...
```json
{
    "body": "The bug is clearly in the ad-hoc float precision code in that file! It has ifdefs for the behavior of `long double`.",
...
```json
{
    "body": "I won't be surprised if the bug in question also shows up on non-x86(_64) hardware. e.g. on ARM (which lacks that 80-bit extended precision).",
...
```json
{
    "body": "+1 for removal of outdated code, which is better served by a maintained external library.",
...
```json
{
    "body": "I don't think it is fair to call it outdated code. Since Pari's implementation is at least as fast as this (so we can use that to test against each other) and everyone seems intent on just sweeping the problem away, then go ahead and remove it.",
...
Replying to [comment:16 fredrik.johansson]:
> I don't see the point of maintaining obsolete code in Sage when there are multiple superior implementations in its standard libraries.
...
```json
{
    "body": "Nobody has worked on fixing this code, so let's please remove it for 9.2. Any takers?",
...
```json
{
    "body": "If the patchbot light shows green, you can set this to positive review on my behalf",
...
```json
{
    "body": "The patchbot doctest failure seems unrelated.",
