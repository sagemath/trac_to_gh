# Issue 29848: MR43: Force misc-clean to run after sagelib-clean

archive/issues_029611.json:
```json
{
    "assignees": [],
    "body": "Julian R\u00fcth ([@saraedum](https://gitlab.com/saraedum)) opened a merge request at https://gitlab.com/sagemath/sage/-/merge_requests/43:\n\n---\n\n```markdown\nthese two must not run in parallel or in reversed order since\nsagelib-clean needs files that misc-clean deletes\n```\n\n\n\n\n\n\nBranch/Commit: **[7db58ae](https://github.com/sagemath/sagetrac-mirror/commit/7db58ae715cb76ba7d1818361f2102a3ae233e6e)**\n\nReviewer: **Matthias Koeppe**\n\nAuthor: **Julian R\u00fcth**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29848_\n\n",
    "closed_at": "2020-06-21T22:37:27Z",
    "created_at": "2020-06-11T17:21:24Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20docker",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "MR43: Force misc-clean to run after sagelib-clean",
    "type": "issue",
    "updated_at": "2020-06-21T22:37:27Z",
    "url": "https://github.com/sagemath/sage/issues/29848",
    "user": "https://github.com/sagetrac-gitlab-bot"
}
```
Julian Rüth ([@saraedum](https://gitlab.com/saraedum)) opened a merge request at https://gitlab.com/sagemath/sage/-/merge_requests/43:

---

```markdown
these two must not run in parallel or in reversed order since
sagelib-clean needs files that misc-clean deletes
```






Branch/Commit: **[7db58ae](https://github.com/sagemath/sagetrac-mirror/commit/7db58ae715cb76ba7d1818361f2102a3ae233e6e)**

Reviewer: **Matthias Koeppe**

Author: **Julian Rüth**

_Issue created by migration from https://trac.sagemath.org/ticket/29848_





---

archive/issue_events_392577.json:
```json
{
    "actor": "https://github.com/sagetrac-gitlab-bot",
    "created_at": "2020-06-11T17:21:24Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29848",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29848#event-392577"
}
```



---

archive/issue_events_392578.json:
```json
{
    "actor": "https://github.com/sagetrac-gitlab-bot",
    "created_at": "2020-06-11T17:21:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29848",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29848#event-392578"
}
```



---

archive/issue_events_392579.json:
```json
{
    "actor": "https://github.com/sagetrac-gitlab-bot",
    "created_at": "2020-06-11T17:21:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29848",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29848#event-392579"
}
```



---

archive/issue_events_392580.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2020-06-11T17:22:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29848",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20docker",
    "label_color": "000080",
    "label_name": "component: docker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29848#event-392580"
}
```



---

archive/issue_events_392581.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2020-06-11T17:22:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29848",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29848#event-392581"
}
```



---

archive/issue_events_392582.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2020-06-11T17:22:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29848",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29848#event-392582"
}
```



---

archive/issue_comments_474398.json:
```json
{
    "body": "Description changed:\n``````diff\n\n``````\n",
    "created_at": "2020-06-11T17:22:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29848#issuecomment-474398",
    "user": "https://github.com/saraedum"
}
```

Description changed:
``````diff

``````




---

archive/issue_events_392583.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2020-06-11T17:22:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29848",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29848#event-392583"
}
```



---

archive/issue_comments_474399.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nLGTM",
    "created_at": "2020-06-11T18:37:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29848#issuecomment-474399",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

LGTM



---

archive/issue_comments_474400.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2020-06-11T18:37:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29848#issuecomment-474400",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_events_392584.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-11T18:37:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29848",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29848#event-392584"
}
```



---

archive/issue_events_392585.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-11T18:37:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29848",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29848#event-392585"
}
```



---

archive/issue_events_392586.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-06-21T22:37:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29848",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29848#event-392586"
}
```



---

archive/issue_events_392587.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-06-21T22:37:27Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29848",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29848#event-392587"
}
```



---

archive/issue_comments_474401.json:
```json
{
    "body": "Changed branch from **[u/galois/mrs/43/docker-build-order](https://github.com/sagemath/sagetrac-mirror/tree/u/galois/mrs/43/docker-build-order)** to **[7db58ae](https://github.com/sagemath/sagetrac-mirror/commit/7db58ae715cb76ba7d1818361f2102a3ae233e6e)**",
    "created_at": "2020-06-21T22:37:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29848#issuecomment-474401",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/galois/mrs/43/docker-build-order](https://github.com/sagemath/sagetrac-mirror/tree/u/galois/mrs/43/docker-build-order)** to **[7db58ae](https://github.com/sagemath/sagetrac-mirror/commit/7db58ae715cb76ba7d1818361f2102a3ae233e6e)**
