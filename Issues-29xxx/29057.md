# Issue 29057: make stack method of Polyhedron use fraction_field

archive/issues_028820.json:
```json
{
    "assignees": [],
    "body": "`stack` method of `Polyhedron` has the following behavior:\n\n```\nsage: P = polytopes.cross_polytope(4)\nsage: P.stack(P.faces(3)[0])\n---------------------------------------------------------------------------\nTypeError                                 Traceback (most recent call last)\n...\nTypeError: no conversion of this rational to integer\n```\nWe fix this by extending the `base_ring` of `locus_polyhedron` to its `fraction_field`.\n\nCC:  @jplab @kliem\n\nKeywords: **polytopes, stack**\n\nBranch/Commit: **[ac7882e](https://github.com/sagemath/sagetrac-mirror/commit/ac7882ea3ece571411885cc7582990ec16c1db16)**\n\nReviewer: **Jonathan Kliem**\n\nAuthor: **Laith Rastanawi**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29057_\n\n",
    "closed_at": "2020-01-25T17:27:36Z",
    "created_at": "2020-01-20T21:01:25Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "make stack method of Polyhedron use fraction_field",
    "type": "issue",
    "updated_at": "2020-01-25T17:27:36Z",
    "url": "https://github.com/sagemath/sage/issues/29057",
    "user": "https://github.com/LaisRast"
}
```
`stack` method of `Polyhedron` has the following behavior:

```
sage: P = polytopes.cross_polytope(4)
sage: P.stack(P.faces(3)[0])
---------------------------------------------------------------------------
TypeError                                 Traceback (most recent call last)
...
TypeError: no conversion of this rational to integer
```
We fix this by extending the `base_ring` of `locus_polyhedron` to its `fraction_field`.

CC:  @jplab @kliem

Keywords: **polytopes, stack**

Branch/Commit: **[ac7882e](https://github.com/sagemath/sagetrac-mirror/commit/ac7882ea3ece571411885cc7582990ec16c1db16)**

Reviewer: **Jonathan Kliem**

Author: **Laith Rastanawi**

_Issue created by migration from https://trac.sagemath.org/ticket/29057_





---

archive/issue_events_381080.json:
```json
{
    "actor": "https://github.com/LaisRast",
    "created_at": "2020-01-20T21:01:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29057",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29057#event-381080"
}
```



---

archive/issue_events_381081.json:
```json
{
    "actor": "https://github.com/LaisRast",
    "created_at": "2020-01-20T21:01:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29057",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "component: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29057#event-381081"
}
```



---

archive/issue_events_381082.json:
```json
{
    "actor": "https://github.com/LaisRast",
    "created_at": "2020-01-20T21:01:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29057",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29057#event-381082"
}
```



---

archive/issue_events_381083.json:
```json
{
    "actor": "https://github.com/LaisRast",
    "created_at": "2020-01-20T21:01:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29057",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29057#event-381083"
}
```



---

archive/issue_comments_457609.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b7173576997263730cdcc0b918222492a077b1f8\">b717357</a></td><td><code> make stack method of Polyhedron use fraction_field</code></td></tr></table>\n",
    "created_at": "2020-01-20T21:04:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29057#issuecomment-457609",
    "user": "https://github.com/LaisRast"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b7173576997263730cdcc0b918222492a077b1f8">b717357</a></td><td><code> make stack method of Polyhedron use fraction_field</code></td></tr></table>




---

archive/issue_events_381084.json:
```json
{
    "actor": "https://github.com/LaisRast",
    "created_at": "2020-01-20T21:04:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29057",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29057#event-381084"
}
```



---

archive/issue_comments_457610.json:
```json
{
    "body": "Branch: **[public/29057](https://github.com/sagemath/sagetrac-mirror/tree/public/29057)**",
    "created_at": "2020-01-20T21:04:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29057#issuecomment-457610",
    "user": "https://github.com/LaisRast"
}
```

Branch: **[public/29057](https://github.com/sagemath/sagetrac-mirror/tree/public/29057)**



---

archive/issue_comments_457611.json:
```json
{
    "body": "Author: **Laith Rastanawi**",
    "created_at": "2020-01-20T21:04:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29057#issuecomment-457611",
    "user": "https://github.com/LaisRast"
}
```

Author: **Laith Rastanawi**



---

archive/issue_comments_457612.json:
```json
{
    "body": "Commit: **[b717357](https://github.com/sagemath/sagetrac-mirror/commit/b7173576997263730cdcc0b918222492a077b1f8)**",
    "created_at": "2020-01-20T21:04:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29057#issuecomment-457612",
    "user": "https://github.com/LaisRast"
}
```

Commit: **[b717357](https://github.com/sagemath/sagetrac-mirror/commit/b7173576997263730cdcc0b918222492a077b1f8)**



---

archive/issue_comments_457613.json:
```json
{
    "body": "Changed commit from **[b717357](https://github.com/sagemath/sagetrac-mirror/commit/b7173576997263730cdcc0b918222492a077b1f8)** to **[add3c82](https://github.com/sagemath/sagetrac-mirror/commit/add3c827441724ad9d8d05bbd8e98da60fc6ce62)**",
    "created_at": "2020-01-20T21:08:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29057#issuecomment-457613",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[b717357](https://github.com/sagemath/sagetrac-mirror/commit/b7173576997263730cdcc0b918222492a077b1f8)** to **[add3c82](https://github.com/sagemath/sagetrac-mirror/commit/add3c827441724ad9d8d05bbd8e98da60fc6ce62)**



---

archive/issue_comments_457614.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/adc4ca4c27fa9d1d7633838822619fd4476d6d26\">adc4ca4</a></td><td><code>fix testing ticket number</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/add3c827441724ad9d8d05bbd8e98da60fc6ce62\">add3c82</a></td><td><code>old parent changes to an appropriate base_ring</code></td></tr></table>\n",
    "created_at": "2020-01-20T21:08:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29057#issuecomment-457614",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/adc4ca4c27fa9d1d7633838822619fd4476d6d26">adc4ca4</a></td><td><code>fix testing ticket number</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/add3c827441724ad9d8d05bbd8e98da60fc6ce62">add3c82</a></td><td><code>old parent changes to an appropriate base_ring</code></td></tr></table>




---

archive/issue_comments_457615.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -8,4 +8,4 @@\n ...\n TypeError: no conversion of this rational to integer\n ```\n-We fix this by extending the `base_ring` to its `fraction_field`.\n+We fix this by extending the `base_ring` if `locus_polyhedron` to its `fraction_field`.\n``````\n",
    "created_at": "2020-01-20T21:10:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29057#issuecomment-457615",
    "user": "https://github.com/LaisRast"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -8,4 +8,4 @@
 ...
 TypeError: no conversion of this rational to integer
 ```
-We fix this by extending the `base_ring` to its `fraction_field`.
+We fix this by extending the `base_ring` if `locus_polyhedron` to its `fraction_field`.
``````




---

archive/issue_comments_457616.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -8,4 +8,4 @@\n ...\n TypeError: no conversion of this rational to integer\n ```\n-We fix this by extending the `base_ring` if `locus_polyhedron` to its `fraction_field`.\n+We fix this by extending the `base_ring` of `locus_polyhedron` to its `fraction_field`.\n``````\n",
    "created_at": "2020-01-20T21:11:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29057#issuecomment-457616",
    "user": "https://github.com/LaisRast"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -8,4 +8,4 @@
 ...
 TypeError: no conversion of this rational to integer
 ```
-We fix this by extending the `base_ring` if `locus_polyhedron` to its `fraction_field`.
+We fix this by extending the `base_ring` of `locus_polyhedron` to its `fraction_field`.
``````




---

archive/issue_comments_457617.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nLooks fine. Maybe you can add `backend=self.backend()` for locus polyhedron.\n\nAs the inequalities and equations are taken from our original polyhedron, the backend can always handle locus polyhedron.",
    "created_at": "2020-01-21T06:44:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29057#issuecomment-457617",
    "user": "https://github.com/kliem"
}
```

<div id="comment:5" align="right">Comment 5</div>

Looks fine. Maybe you can add `backend=self.backend()` for locus polyhedron.

As the inequalities and equations are taken from our original polyhedron, the backend can always handle locus polyhedron.



---

archive/issue_comments_457618.json:
```json
{
    "body": "Changed commit from **[add3c82](https://github.com/sagemath/sagetrac-mirror/commit/add3c827441724ad9d8d05bbd8e98da60fc6ce62)** to **[ac7882e](https://github.com/sagemath/sagetrac-mirror/commit/ac7882ea3ece571411885cc7582990ec16c1db16)**",
    "created_at": "2020-01-21T09:20:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29057#issuecomment-457618",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[add3c82](https://github.com/sagemath/sagetrac-mirror/commit/add3c827441724ad9d8d05bbd8e98da60fc6ce62)** to **[ac7882e](https://github.com/sagemath/sagetrac-mirror/commit/ac7882ea3ece571411885cc7582990ec16c1db16)**



---

archive/issue_comments_457619.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ac7882ea3ece571411885cc7582990ec16c1db16\">ac7882e</a></td><td><code>use self.backend() in locus_polyhedron</code></td></tr></table>\n",
    "created_at": "2020-01-21T09:20:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29057#issuecomment-457619",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ac7882ea3ece571411885cc7582990ec16c1db16">ac7882e</a></td><td><code>use self.backend() in locus_polyhedron</code></td></tr></table>




---

archive/issue_comments_457620.json:
```json
{
    "body": "Reviewer: **Jonathan Kliem**",
    "created_at": "2020-01-21T09:24:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29057#issuecomment-457620",
    "user": "https://github.com/kliem"
}
```

Reviewer: **Jonathan Kliem**



---

archive/issue_comments_457621.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nLGTM.",
    "created_at": "2020-01-21T09:24:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29057#issuecomment-457621",
    "user": "https://github.com/kliem"
}
```

<div id="comment:7" align="right">Comment 7</div>

LGTM.



---

archive/issue_events_381085.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-01-21T09:24:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29057",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29057#event-381085"
}
```



---

archive/issue_events_381086.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2020-01-21T09:24:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29057",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29057#event-381086"
}
```



---

archive/issue_events_381087.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-01-25T17:27:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29057",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29057#event-381087"
}
```



---

archive/issue_events_381088.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "229d4365d8b9145f38e316857fa1460f79f467c7",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-01-25T17:27:36Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29057",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29057#event-381088"
}
```



---

archive/issue_comments_457622.json:
```json
{
    "body": "Changed branch from **[public/29057](https://github.com/sagemath/sagetrac-mirror/tree/public/29057)** to **[ac7882e](https://github.com/sagemath/sagetrac-mirror/commit/ac7882ea3ece571411885cc7582990ec16c1db16)**",
    "created_at": "2020-01-25T17:27:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29057#issuecomment-457622",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/29057](https://github.com/sagemath/sagetrac-mirror/tree/public/29057)** to **[ac7882e](https://github.com/sagemath/sagetrac-mirror/commit/ac7882ea3ece571411885cc7582990ec16c1db16)**
