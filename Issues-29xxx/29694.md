# Issue 29694: conflict between lazy attribute and abstract method tester

archive/issues_029457.json:
```json
{
    "assignees": [],
    "body": "This ticket fixes a test suite failure that occurs when a lazy attribute raises a `NotImplementedError`.\n\n```\nsage: class NotAbstract(SageObject):\n....:     @lazy_attribute\n....:     def bla(self):\n....:         raise NotImplementedError(\"not implemented\")\nsage: NotAbstract()._test_not_implemented_methods()\n...\nAssertionError: Not implemented method: bla\n```\n\nThe method `_test_not_implemented_methods` should only fail when an `AbstractMethod` is not implemented.\n\n\nCC:  @kwankyu\n\nBranch/Commit: **[68cee52](https://github.com/sagemath/sagetrac-mirror/commit/68cee5252b8401d8143df121bdc2348d832539ee)**\n\nReviewer: **Kwankyu Lee**\n\nAuthor: **Markus Wageringel**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29694_\n\n",
    "closed_at": "2020-07-28T22:31:51Z",
    "created_at": "2020-05-16T19:32:46Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "conflict between lazy attribute and abstract method tester",
    "type": "issue",
    "updated_at": "2020-07-28T22:31:51Z",
    "url": "https://github.com/sagemath/sage/issues/29694",
    "user": "https://github.com/mwageringel"
}
```
This ticket fixes a test suite failure that occurs when a lazy attribute raises a `NotImplementedError`.

```
sage: class NotAbstract(SageObject):
....:     @lazy_attribute
....:     def bla(self):
....:         raise NotImplementedError("not implemented")
sage: NotAbstract()._test_not_implemented_methods()
...
AssertionError: Not implemented method: bla
```

The method `_test_not_implemented_methods` should only fail when an `AbstractMethod` is not implemented.


CC:  @kwankyu

Branch/Commit: **[68cee52](https://github.com/sagemath/sagetrac-mirror/commit/68cee5252b8401d8143df121bdc2348d832539ee)**

Reviewer: **Kwankyu Lee**

Author: **Markus Wageringel**

_Issue created by migration from https://trac.sagemath.org/ticket/29694_





---

archive/issue_events_390324.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-05-16T19:32:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29694",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29694#event-390324"
}
```



---

archive/issue_events_390325.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-05-16T19:32:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29694",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29694#event-390325"
}
```



---

archive/issue_events_390326.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-05-16T19:32:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29694",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29694#event-390326"
}
```



---

archive/issue_events_390327.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-05-16T19:39:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29694",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29694#event-390327"
}
```



---

archive/issue_comments_470881.json:
```json
{
    "body": "Commit: **[68cee52](https://github.com/sagemath/sagetrac-mirror/commit/68cee5252b8401d8143df121bdc2348d832539ee)**",
    "created_at": "2020-05-16T19:39:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29694",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29694#issuecomment-470881",
    "user": "https://github.com/mwageringel"
}
```

Commit: **[68cee52](https://github.com/sagemath/sagetrac-mirror/commit/68cee5252b8401d8143df121bdc2348d832539ee)**



---

archive/issue_comments_470882.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/68cee5252b8401d8143df121bdc2348d832539ee\">68cee52</a></td><td><code>29694: fix conflict between lazy attribute/abstract method</code></td></tr></table>\n",
    "created_at": "2020-05-16T19:39:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29694",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29694#issuecomment-470882",
    "user": "https://github.com/mwageringel"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/68cee5252b8401d8143df121bdc2348d832539ee">68cee52</a></td><td><code>29694: fix conflict between lazy attribute/abstract method</code></td></tr></table>




---

archive/issue_comments_470883.json:
```json
{
    "body": "Author: **Markus Wageringel**",
    "created_at": "2020-05-16T19:39:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29694",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29694#issuecomment-470883",
    "user": "https://github.com/mwageringel"
}
```

Author: **Markus Wageringel**



---

archive/issue_comments_470884.json:
```json
{
    "body": "Branch: **[u/gh-mwageringel/29694](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/29694)**",
    "created_at": "2020-05-16T19:39:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29694",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29694#issuecomment-470884",
    "user": "https://github.com/mwageringel"
}
```

Branch: **[u/gh-mwageringel/29694](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/29694)**



---

archive/issue_events_390328.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2020-07-16T06:30:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29694",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29694#event-390328"
}
```



---

archive/issue_events_390329.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2020-07-16T06:30:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29694",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29694#event-390329"
}
```



---

archive/issue_comments_470885.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLGTM.",
    "created_at": "2020-07-16T06:30:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29694",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29694#issuecomment-470885",
    "user": "https://github.com/kwankyu"
}
```

<div id="comment:3" align="right">Comment 3</div>

LGTM.



---

archive/issue_comments_470886.json:
```json
{
    "body": "Reviewer: **Kwankyu Lee**",
    "created_at": "2020-07-16T06:30:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29694",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29694#issuecomment-470886",
    "user": "https://github.com/kwankyu"
}
```

Reviewer: **Kwankyu Lee**



---

archive/issue_comments_470887.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThank you.",
    "created_at": "2020-07-16T06:39:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29694",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29694#issuecomment-470887",
    "user": "https://github.com/mwageringel"
}
```

<div id="comment:5" align="right">Comment 5</div>

Thank you.



---

archive/issue_events_390330.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-07-28T22:31:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29694",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29694#event-390330"
}
```



---

archive/issue_events_390331.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "a1c3b4863f0c203f5a8e1764d5e3d87b53d1154f",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-07-28T22:31:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29694",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29694#event-390331"
}
```



---

archive/issue_comments_470888.json:
```json
{
    "body": "Changed branch from **[u/gh-mwageringel/29694](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/29694)** to **[68cee52](https://github.com/sagemath/sagetrac-mirror/commit/68cee5252b8401d8143df121bdc2348d832539ee)**",
    "created_at": "2020-07-28T22:31:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29694",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29694#issuecomment-470888",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/gh-mwageringel/29694](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/29694)** to **[68cee52](https://github.com/sagemath/sagetrac-mirror/commit/68cee5252b8401d8143df121bdc2348d832539ee)**
