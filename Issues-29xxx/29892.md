# Issue 29892: Move sage.misc.misc.coeff_repr, repr_lincomb to new module sage.misc.repr

archive/issues_029655.json:
```json
{
    "assignees": [],
    "body": "In fact, we have 2 copies of `coeff_repr` and `repr_lincomb`: one in `sage.misc.misc`, one in `sage.misc.latex`.  Perhaps they can be merged in a follow up ticket as well; and perhaps even #14509 be fixed...\n\n`sage.structure.formal_sum.FormalSum._latex_` has a comment (from nthiery 2012):\n\n```\n        return sage.misc.latex.repr_lincomb(symbols, coeffs)\n        # TODO: finish merging sage.misc.latex.repr_lincomb and\n        # sage.misc.misc.repr_lincomb and use instead:\n        # return sage.misc.misc.repr_lincomb([[t,c] for c,t in self], is_latex=True)\n```\n\nAlso `sage.modular.modsym.element`:\n\n```\n        # TODO: use repr_lincomb with is_latex=True\n        return latex.repr_lincomb(v, c)\n```\n\n---\n\nThis ticket is part of an effort to reduce `sage.misc.misc`, motivated by #29865 (Modularization of sagelib: Break out a separate package `sage-objects`)\n\nDepends on #29869\n\nCC:  @fchapoton @kcrisman @tscrim @slel @nthiery @DaveWitteMorris\n\nBranch/Commit: **[b5f63a5](https://github.com/sagemath/sagetrac-mirror/commit/b5f63a5ff65e154be5b2bc1cc3bdebeba7722825)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29892_\n\n",
    "closed_at": "2020-07-08T19:34:07Z",
    "created_at": "2020-06-18T01:47:48Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/refactoring",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Move sage.misc.misc.coeff_repr, repr_lincomb to new module sage.misc.repr",
    "type": "issue",
    "updated_at": "2020-07-08T19:34:07Z",
    "url": "https://github.com/sagemath/sage/issues/29892",
    "user": "https://github.com/mkoeppe"
}
```
In fact, we have 2 copies of `coeff_repr` and `repr_lincomb`: one in `sage.misc.misc`, one in `sage.misc.latex`.  Perhaps they can be merged in a follow up ticket as well; and perhaps even #14509 be fixed...

`sage.structure.formal_sum.FormalSum._latex_` has a comment (from nthiery 2012):

```
        return sage.misc.latex.repr_lincomb(symbols, coeffs)
        # TODO: finish merging sage.misc.latex.repr_lincomb and
        # sage.misc.misc.repr_lincomb and use instead:
        # return sage.misc.misc.repr_lincomb([[t,c] for c,t in self], is_latex=True)
```

Also `sage.modular.modsym.element`:

```
        # TODO: use repr_lincomb with is_latex=True
        return latex.repr_lincomb(v, c)
```

---

This ticket is part of an effort to reduce `sage.misc.misc`, motivated by #29865 (Modularization of sagelib: Break out a separate package `sage-objects`)

Depends on #29869

CC:  @fchapoton @kcrisman @tscrim @slel @nthiery @DaveWitteMorris

Branch/Commit: **[b5f63a5](https://github.com/sagemath/sagetrac-mirror/commit/b5f63a5ff65e154be5b2bc1cc3bdebeba7722825)**

Reviewer: **Travis Scrimshaw**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/29892_





---

archive/issue_events_393206.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-18T01:47:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29892#event-393206"
}
```



---

archive/issue_events_393207.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-18T01:47:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29892#event-393207"
}
```



---

archive/issue_events_393208.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-18T01:47:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29892#event-393208"
}
```



---

archive/issue_events_393209.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-18T01:51:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "label": "https://github.com/sagemath/sage/labels/refactoring",
    "label_color": "696969",
    "label_name": "refactoring",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29892#event-393209"
}
```



---

archive/issue_comments_475352.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,11 @@\n In fact, we have 2 copies of `coeff_repr` and `repr_lincomb`: one in `sage.misc.misc`, one in `sage.misc.latex`.\n \n+`sage.structure.formal_sum.FormalSum._latex_` has a comment (from nthiery 2012):\n \n+```\n+        return sage.misc.latex.repr_lincomb(symbols, coeffs)\n+        # TODO: finish merging sage.misc.latex.repr_lincomb and\n+        # sage.misc.misc.repr_lincomb and use instead:\n+        # return sage.misc.misc.repr_lincomb([[t,c] for c,t in self], is_latex=True)\n+```\n+\n``````\n",
    "created_at": "2020-06-18T01:51:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475352",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,11 @@
 In fact, we have 2 copies of `coeff_repr` and `repr_lincomb`: one in `sage.misc.misc`, one in `sage.misc.latex`.
 
+`sage.structure.formal_sum.FormalSum._latex_` has a comment (from nthiery 2012):
 
+```
+        return sage.misc.latex.repr_lincomb(symbols, coeffs)
+        # TODO: finish merging sage.misc.latex.repr_lincomb and
+        # sage.misc.misc.repr_lincomb and use instead:
+        # return sage.misc.misc.repr_lincomb([[t,c] for c,t in self], is_latex=True)
+```
+
``````




---

archive/issue_comments_475353.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -9,3 +9,6 @@\n         # return sage.misc.misc.repr_lincomb([[t,c] for c,t in self], is_latex=True)\n ```\n \n+\n+\n+This ticket is part of an effort to reduce `sage.misc.misc`, motivated by #29865 (Modularization of sagelib: Break out a separate package `sage-objects`)\n``````\n",
    "created_at": "2020-06-18T01:53:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475353",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -9,3 +9,6 @@
         # return sage.misc.misc.repr_lincomb([[t,c] for c,t in self], is_latex=True)
 ```
 
+
+
+This ticket is part of an effort to reduce `sage.misc.misc`, motivated by #29865 (Modularization of sagelib: Break out a separate package `sage-objects`)
``````




---

archive/issue_comments_475354.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-In fact, we have 2 copies of `coeff_repr` and `repr_lincomb`: one in `sage.misc.misc`, one in `sage.misc.latex`.\n+In fact, we have 2 copies of `coeff_repr` and `repr_lincomb`: one in `sage.misc.misc`, one in `sage.misc.latex`.  Perhaps they can be merged in this ticket as well; and perhaps even #14509 be fixed.\n \n `sage.structure.formal_sum.FormalSum._latex_` has a comment (from nthiery 2012):\n \n``````\n",
    "created_at": "2020-06-18T01:56:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475354",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-In fact, we have 2 copies of `coeff_repr` and `repr_lincomb`: one in `sage.misc.misc`, one in `sage.misc.latex`.
+In fact, we have 2 copies of `coeff_repr` and `repr_lincomb`: one in `sage.misc.misc`, one in `sage.misc.latex`.  Perhaps they can be merged in this ticket as well; and perhaps even #14509 be fixed.
 
 `sage.structure.formal_sum.FormalSum._latex_` has a comment (from nthiery 2012):
 
``````




---

archive/issue_comments_475355.json:
```json
{
    "body": "Dependencies: **#29869**",
    "created_at": "2020-06-18T02:16:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475355",
    "user": "https://github.com/mkoeppe"
}
```

Dependencies: **#29869**



---

archive/issue_comments_475356.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-In fact, we have 2 copies of `coeff_repr` and `repr_lincomb`: one in `sage.misc.misc`, one in `sage.misc.latex`.  Perhaps they can be merged in this ticket as well; and perhaps even #14509 be fixed.\n+In fact, we have 2 copies of `coeff_repr` and `repr_lincomb`: one in `sage.misc.misc`, one in `sage.misc.latex`.  Perhaps they can be merged in a follow up ticket as well; and perhaps even #14509 be fixed...\n \n `sage.structure.formal_sum.FormalSum._latex_` has a comment (from nthiery 2012):\n \n@@ -9,6 +9,13 @@\n         # return sage.misc.misc.repr_lincomb([[t,c] for c,t in self], is_latex=True)\n ```\n \n+Also `sage.modular.modsym.element`:\n \n+```\n+        # TODO: use repr_lincomb with is_latex=True\n+        return latex.repr_lincomb(v, c)\n+```\n+\n+---\n \n This ticket is part of an effort to reduce `sage.misc.misc`, motivated by #29865 (Modularization of sagelib: Break out a separate package `sage-objects`)\n``````\n",
    "created_at": "2020-06-18T02:36:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475356",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-In fact, we have 2 copies of `coeff_repr` and `repr_lincomb`: one in `sage.misc.misc`, one in `sage.misc.latex`.  Perhaps they can be merged in this ticket as well; and perhaps even #14509 be fixed.
+In fact, we have 2 copies of `coeff_repr` and `repr_lincomb`: one in `sage.misc.misc`, one in `sage.misc.latex`.  Perhaps they can be merged in a follow up ticket as well; and perhaps even #14509 be fixed...
 
 `sage.structure.formal_sum.FormalSum._latex_` has a comment (from nthiery 2012):
 
@@ -9,6 +9,13 @@
         # return sage.misc.misc.repr_lincomb([[t,c] for c,t in self], is_latex=True)
 ```
 
+Also `sage.modular.modsym.element`:
 
+```
+        # TODO: use repr_lincomb with is_latex=True
+        return latex.repr_lincomb(v, c)
+```
+
+---
 
 This ticket is part of an effort to reduce `sage.misc.misc`, motivated by #29865 (Modularization of sagelib: Break out a separate package `sage-objects`)
``````




---

archive/issue_comments_475357.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2020-06-18T02:36:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475357",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_comments_475358.json:
```json
{
    "body": "Branch: **[u/mkoeppe/move_sage_misc_misc_coeff_repr__repr_lincomb_to_new_module_sage_misc_repr](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/move_sage_misc_misc_coeff_repr__repr_lincomb_to_new_module_sage_misc_repr)**",
    "created_at": "2020-06-18T03:06:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475358",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/move_sage_misc_misc_coeff_repr__repr_lincomb_to_new_module_sage_misc_repr](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/move_sage_misc_misc_coeff_repr__repr_lincomb_to_new_module_sage_misc_repr)**



---

archive/issue_comments_475359.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/907feeb085ce0c64e27192920df1ba8f0f4f175e\">907feeb</a></td><td><code>Move attrcall and friends from sage.misc.misc to new module sage.misc.call</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a5453bfeca6a2c0fb29ccdcc598a8015b3714514\">a5453bf</a></td><td><code>Fixup: Add src/sage/misc/call.py</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/64c570195c7947420ea8cffb15d00e9a02dc7881\">64c5701</a></td><td><code>lazy_import from sage.misc.call with deprecation</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/65414f7bd923a5bb7d904f8a4feb9533499431c5\">65414f7</a></td><td><code>Fix imports and one deprecation warning</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b9314d4da2107ff79d1b4d62d2837c3a747484bb\">b9314d4</a></td><td><code>sage.misc.call: Add standard header information, add to reference manual</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6024ffd4f207db7b5d419696b3ce1b6711a81056\">6024ffd</a></td><td><code>src/sage/misc/call.py: register_unpickle_override for call_method</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ab4e0fdb530592b85d044a619c0b75a0da0b8f29\">ab4e0fd</a></td><td><code>Move sage.misc.misc.coeff_repr, repr_lincomb to new module sage.misc.repr</code></td></tr></table>\n",
    "created_at": "2020-06-18T03:07:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475359",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:7'>Comment 7:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/907feeb085ce0c64e27192920df1ba8f0f4f175e">907feeb</a></td><td><code>Move attrcall and friends from sage.misc.misc to new module sage.misc.call</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a5453bfeca6a2c0fb29ccdcc598a8015b3714514">a5453bf</a></td><td><code>Fixup: Add src/sage/misc/call.py</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/64c570195c7947420ea8cffb15d00e9a02dc7881">64c5701</a></td><td><code>lazy_import from sage.misc.call with deprecation</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/65414f7bd923a5bb7d904f8a4feb9533499431c5">65414f7</a></td><td><code>Fix imports and one deprecation warning</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b9314d4da2107ff79d1b4d62d2837c3a747484bb">b9314d4</a></td><td><code>sage.misc.call: Add standard header information, add to reference manual</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6024ffd4f207db7b5d419696b3ce1b6711a81056">6024ffd</a></td><td><code>src/sage/misc/call.py: register_unpickle_override for call_method</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ab4e0fdb530592b85d044a619c0b75a0da0b8f29">ab4e0fd</a></td><td><code>Move sage.misc.misc.coeff_repr, repr_lincomb to new module sage.misc.repr</code></td></tr></table>




---

archive/issue_events_393210.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-18T03:07:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29892#event-393210"
}
```



---

archive/issue_comments_475360.json:
```json
{
    "body": "Commit: **[ab4e0fd](https://github.com/sagemath/sagetrac-mirror/commit/ab4e0fdb530592b85d044a619c0b75a0da0b8f29)**",
    "created_at": "2020-06-18T03:07:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475360",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[ab4e0fd](https://github.com/sagemath/sagetrac-mirror/commit/ab4e0fdb530592b85d044a619c0b75a0da0b8f29)**



---

archive/issue_events_393211.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-18T06:09:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29892#event-393211"
}
```



---

archive/issue_events_393212.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-18T06:09:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29892#event-393212"
}
```



---

archive/issue_comments_475361.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3f5b6eacdfabf83db6e23e49aacbf8300bf7ff01\">3f5b6ea</a></td><td><code>Move sage.misc.misc.coeff_repr, repr_lincomb to new module sage.misc.repr</code></td></tr></table>\n",
    "created_at": "2020-06-18T06:10:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475361",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:10'>Comment 10:</a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3f5b6eacdfabf83db6e23e49aacbf8300bf7ff01">3f5b6ea</a></td><td><code>Move sage.misc.misc.coeff_repr, repr_lincomb to new module sage.misc.repr</code></td></tr></table>




---

archive/issue_comments_475362.json:
```json
{
    "body": "Changed commit from **[ab4e0fd](https://github.com/sagemath/sagetrac-mirror/commit/ab4e0fdb530592b85d044a619c0b75a0da0b8f29)** to **[3f5b6ea](https://github.com/sagemath/sagetrac-mirror/commit/3f5b6eacdfabf83db6e23e49aacbf8300bf7ff01)**",
    "created_at": "2020-06-18T06:10:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475362",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[ab4e0fd](https://github.com/sagemath/sagetrac-mirror/commit/ab4e0fdb530592b85d044a619c0b75a0da0b8f29)** to **[3f5b6ea](https://github.com/sagemath/sagetrac-mirror/commit/3f5b6eacdfabf83db6e23e49aacbf8300bf7ff01)**



---

archive/issue_events_393213.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-18T06:11:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29892#event-393213"
}
```



---

archive/issue_events_393214.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-18T06:11:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29892#event-393214"
}
```



---

archive/issue_comments_475363.json:
```json
{
    "body": "Changed commit from **[3f5b6ea](https://github.com/sagemath/sagetrac-mirror/commit/3f5b6eacdfabf83db6e23e49aacbf8300bf7ff01)** to **[08fedfa](https://github.com/sagemath/sagetrac-mirror/commit/08fedfa9b22214c9b3e8c4640c7c10fbd66f78ee)**",
    "created_at": "2020-06-18T17:36:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475363",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[3f5b6ea](https://github.com/sagemath/sagetrac-mirror/commit/3f5b6eacdfabf83db6e23e49aacbf8300bf7ff01)** to **[08fedfa](https://github.com/sagemath/sagetrac-mirror/commit/08fedfa9b22214c9b3e8c4640c7c10fbd66f78ee)**



---

archive/issue_comments_475364.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/08fedfa9b22214c9b3e8c4640c7c10fbd66f78ee\">08fedfa</a></td><td><code>src/sage/combinat/root_system/type_dual.py: Remove unused variable to fix pyflakes warning</code></td></tr></table>\n",
    "created_at": "2020-06-18T17:36:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475364",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:12'>Comment 12:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/08fedfa9b22214c9b3e8c4640c7c10fbd66f78ee">08fedfa</a></td><td><code>src/sage/combinat/root_system/type_dual.py: Remove unused variable to fix pyflakes warning</code></td></tr></table>




---

archive/issue_comments_475365.json:
```json
{
    "body": "Changed commit from **[08fedfa](https://github.com/sagemath/sagetrac-mirror/commit/08fedfa9b22214c9b3e8c4640c7c10fbd66f78ee)** to **[05efc11](https://github.com/sagemath/sagetrac-mirror/commit/05efc119c9341ac32e19acf969438c91c5008918)**",
    "created_at": "2020-06-18T17:42:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475365",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[08fedfa](https://github.com/sagemath/sagetrac-mirror/commit/08fedfa9b22214c9b3e8c4640c7c10fbd66f78ee)** to **[05efc11](https://github.com/sagemath/sagetrac-mirror/commit/05efc119c9341ac32e19acf969438c91c5008918)**



---

archive/issue_comments_475366.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/05efc119c9341ac32e19acf969438c91c5008918\">05efc11</a></td><td><code>sage.misc.repr.coeff_repr: Add doctest, adapted from sage.misc.latex.coeff_repr</code></td></tr></table>\n",
    "created_at": "2020-06-18T17:42:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475366",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:13'>Comment 13:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/05efc119c9341ac32e19acf969438c91c5008918">05efc11</a></td><td><code>sage.misc.repr.coeff_repr: Add doctest, adapted from sage.misc.latex.coeff_repr</code></td></tr></table>




---

archive/issue_comments_475367.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/538323b4ed2dc5a2593acceaa5794fb4dd502f8d\">538323b</a></td><td><code>src/sage/misc/call.py: Fix block syntax in docstring</code></td></tr></table>\n",
    "created_at": "2020-06-18T22:15:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475367",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:14'>Comment 14:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/538323b4ed2dc5a2593acceaa5794fb4dd502f8d">538323b</a></td><td><code>src/sage/misc/call.py: Fix block syntax in docstring</code></td></tr></table>




---

archive/issue_comments_475368.json:
```json
{
    "body": "Changed commit from **[05efc11](https://github.com/sagemath/sagetrac-mirror/commit/05efc119c9341ac32e19acf969438c91c5008918)** to **[538323b](https://github.com/sagemath/sagetrac-mirror/commit/538323b4ed2dc5a2593acceaa5794fb4dd502f8d)**",
    "created_at": "2020-06-18T22:15:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475368",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[05efc11](https://github.com/sagemath/sagetrac-mirror/commit/05efc119c9341ac32e19acf969438c91c5008918)** to **[538323b](https://github.com/sagemath/sagetrac-mirror/commit/538323b4ed2dc5a2593acceaa5794fb4dd502f8d)**



---

archive/issue_comments_475369.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nNot sure what's up with the \"block syntax\" warning",
    "created_at": "2020-06-22T05:37:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475369",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:15'>Comment 15:</a>
Not sure what's up with the "block syntax" warning



---

archive/issue_comments_475370.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nReturns should be Return",
    "created_at": "2020-06-22T06:00:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475370",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:16'>Comment 16:</a>
Returns should be Return



---

archive/issue_comments_475371.json:
```json
{
    "body": "Changed commit from **[538323b](https://github.com/sagemath/sagetrac-mirror/commit/538323b4ed2dc5a2593acceaa5794fb4dd502f8d)** to **[b5f63a5](https://github.com/sagemath/sagetrac-mirror/commit/b5f63a5ff65e154be5b2bc1cc3bdebeba7722825)**",
    "created_at": "2020-06-22T06:06:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475371",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[538323b](https://github.com/sagemath/sagetrac-mirror/commit/538323b4ed2dc5a2593acceaa5794fb4dd502f8d)** to **[b5f63a5](https://github.com/sagemath/sagetrac-mirror/commit/b5f63a5ff65e154be5b2bc1cc3bdebeba7722825)**



---

archive/issue_comments_475372.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b5f63a5ff65e154be5b2bc1cc3bdebeba7722825\">b5f63a5</a></td><td><code>src/sage/misc/call.py: Returns should be Return</code></td></tr></table>\n",
    "created_at": "2020-06-22T06:06:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475372",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:17'>Comment 17:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b5f63a5ff65e154be5b2bc1cc3bdebeba7722825">b5f63a5</a></td><td><code>src/sage/misc/call.py: Returns should be Return</code></td></tr></table>




---

archive/issue_comments_475373.json:
```json
{
    "body": "<a id='comment:18'>Comment 18:</a>\nThanks!",
    "created_at": "2020-06-22T06:06:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475373",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:18'>Comment 18:</a>
Thanks!



---

archive/issue_comments_475374.json:
```json
{
    "body": "<a id='comment:19'>Comment 19:</a>\nThe remaining plugin warnings are old news. Needs review",
    "created_at": "2020-06-22T18:00:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475374",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:19'>Comment 19:</a>
The remaining plugin warnings are old news. Needs review



---

archive/issue_comments_475375.json:
```json
{
    "body": "<a id='comment:20'>Comment 20:</a>\nI agree with every change except for 08fedfa. This is used:\n\n```\nsage: CartanType.options.notation='Kac'\nsage: CartanType(['BC', 4, 2]).dual()\n['A', 8, 2]^+\nsage: CartanType(['BC', 4, 2]).dual()._repr_(compact=True)\n'A8^2+'\n\nsage: CartanType.options.notation='Stembridge'\nsage: CartanType(['BC', 4, 2]).dual()\n['BC', 4, 2]^*\nsage: CartanType(['BC', 4, 2]).dual()._repr_(compact=True)\n'BC4~*'\n```\nIt is suppose to represent the notation for affine Cartan type introduced by Kac of A<sub>2n</sub><sup>(2)\\dagger</sup>. There probably should be a doctest showing this though...",
    "created_at": "2020-06-30T04:35:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475375",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:20'>Comment 20:</a>
I agree with every change except for 08fedfa. This is used:

```
sage: CartanType.options.notation='Kac'
sage: CartanType(['BC', 4, 2]).dual()
['A', 8, 2]^+
sage: CartanType(['BC', 4, 2]).dual()._repr_(compact=True)
'A8^2+'

sage: CartanType.options.notation='Stembridge'
sage: CartanType(['BC', 4, 2]).dual()
['BC', 4, 2]^*
sage: CartanType(['BC', 4, 2]).dual()._repr_(compact=True)
'BC4~*'
```
It is suppose to represent the notation for affine Cartan type introduced by Kac of A<sub>2n</sub><sup>(2)\dagger</sup>. There probably should be a doctest showing this though...



---

archive/issue_comments_475376.json:
```json
{
    "body": "<a id='comment:21'>Comment 21:</a>\nThe function delegates to `CartanType._repr_` in this case, which computes `dual_str` again and uses it",
    "created_at": "2020-06-30T04:55:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475376",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:21'>Comment 21:</a>
The function delegates to `CartanType._repr_` in this case, which computes `dual_str` again and uses it



---

archive/issue_comments_475377.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2020-06-30T05:01:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475377",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_393215.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-06-30T05:01:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29892#event-393215"
}
```



---

archive/issue_events_393216.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-06-30T05:01:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29892#event-393216"
}
```



---

archive/issue_comments_475378.json:
```json
{
    "body": "<a id='comment:22'>Comment 22:</a>\nAh, I see. I was looking at the wrong `_repr_`. There seems to be some redundant code that probably should be cleaned up. A good starter ticket for a new developer.\n\nLGTM. Thanks.",
    "created_at": "2020-06-30T05:01:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475378",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:22'>Comment 22:</a>
Ah, I see. I was looking at the wrong `_repr_`. There seems to be some redundant code that probably should be cleaned up. A good starter ticket for a new developer.

LGTM. Thanks.



---

archive/issue_comments_475379.json:
```json
{
    "body": "<a id='comment:23'>Comment 23:</a>\nThanks!",
    "created_at": "2020-06-30T05:02:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475379",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:23'>Comment 23:</a>
Thanks!



---

archive/issue_events_393217.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-07-08T19:34:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29892#event-393217"
}
```



---

archive/issue_events_393218.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "62b405c4f3bf442d95b7b3f1b3e87624e9c7df3c",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-07-08T19:34:07Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29892#event-393218"
}
```



---

archive/issue_comments_475380.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/move_sage_misc_misc_coeff_repr__repr_lincomb_to_new_module_sage_misc_repr](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/move_sage_misc_misc_coeff_repr__repr_lincomb_to_new_module_sage_misc_repr)** to **[b5f63a5](https://github.com/sagemath/sagetrac-mirror/commit/b5f63a5ff65e154be5b2bc1cc3bdebeba7722825)**",
    "created_at": "2020-07-08T19:34:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29892",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29892#issuecomment-475380",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/move_sage_misc_misc_coeff_repr__repr_lincomb_to_new_module_sage_misc_repr](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/move_sage_misc_misc_coeff_repr__repr_lincomb_to_new_module_sage_misc_repr)** to **[b5f63a5](https://github.com/sagemath/sagetrac-mirror/commit/b5f63a5ff65e154be5b2bc1cc3bdebeba7722825)**
