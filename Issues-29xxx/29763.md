# Issue 29763: Direct convertion from Univariate Polynomial Ring over Rational function field to Symbolic Ring doesn't work

archive/issues_029526.json:
```json
{
    "body": "Lets consider following code\n\n```\nF = FunctionField(QQ, 't')\nR.<u> = F[]\npoly = u^2 + (t^2 + 1)*u - t^2 - t\n\nSR(poly)\n# TypeError: no common canonical parent for objects with parents: 'Rational function field in t over Rational Field' and 'Symbolic Ring'\n```\n\nwhereas conversion through Multivariate Polynomial Ring works as expected:\n\n```\n\nP.<t,u> = QQ[]\nSR(P(poly))\n# t^2*u - t^2 + u^2 - t + u\n```\n\n\n**Keywords:** symbolic ring, conversion\n\n**Status:** new\n\nIssue created by migration from https://trac.sagemath.org/ticket/29763\n\n",
    "created_at": "2020-05-30T13:25:43Z",
    "labels": [
        "component: symbolics",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.8",
    "title": "Direct convertion from Univariate Polynomial Ring over Rational function field to Symbolic Ring doesn't work",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/29763",
    "user": "https://github.com/petRUShka"
}
```
Lets consider following code

```
F = FunctionField(QQ, 't')
R.<u> = F[]
poly = u^2 + (t^2 + 1)*u - t^2 - t

SR(poly)
# TypeError: no common canonical parent for objects with parents: 'Rational function field in t over Rational Field' and 'Symbolic Ring'
```

whereas conversion through Multivariate Polynomial Ring works as expected:

```

P.<t,u> = QQ[]
SR(P(poly))
# t^2*u - t^2 + u^2 - t + u
```


**Keywords:** symbolic ring, conversion

**Status:** new

Issue created by migration from https://trac.sagemath.org/ticket/29763





---

archive/issue_comments_556824.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -9,7 +9,7 @@\n # TypeError: no common canonical parent for objects with parents: 'Rational function field in t over Rational Field' and 'Symbolic Ring'\n ```\n \n-whereas convrsion through Multivariate Polynomial Ring works as expected:\n+whereas conversion through Multivariate Polynomial Ring works as expected:\n \n ```\n \n``````\n",
    "created_at": "2020-05-30T13:26:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29763#issuecomment-556824",
    "user": "https://github.com/petRUShka"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -9,7 +9,7 @@
 # TypeError: no common canonical parent for objects with parents: 'Rational function field in t over Rational Field' and 'Symbolic Ring'
 ```
 
-whereas convrsion through Multivariate Polynomial Ring works as expected:
+whereas conversion through Multivariate Polynomial Ring works as expected:
 
 ```
 
``````




---

archive/issue_comments_556825.json:
```json
{
    "body": "<a id='comment:3'></a>\nUsual caveat:\n\n```\nF=FunctionField(QQ,'t')['t']\n```\nHowever confusing it is, we cannot forbid name collisions as such above because of automatic ring constructions:\n\n```\nMatrix(FunctionField(QQ,'x'),2,2,[1,0,0,x]).charpoly()\n```\nNeedless to say, conversion of these objects to SR would give rather surprising results.",
    "created_at": "2020-05-30T18:06:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29763#issuecomment-556825",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:3'></a>
Usual caveat:

```
F=FunctionField(QQ,'t')['t']
```
However confusing it is, we cannot forbid name collisions as such above because of automatic ring constructions:

```
Matrix(FunctionField(QQ,'x'),2,2,[1,0,0,x]).charpoly()
```
Needless to say, conversion of these objects to SR would give rather surprising results.



---

archive/issue_comments_556826.json:
```json
{
    "body": "<a id='comment:4'></a>\nYou are right, but it is possible to notify user that there is dangerous place in his code through warning or with some other instrument.\n\nIt is very convenient to have that conversion at least in predictable cases like I've mentioned in the issue.",
    "created_at": "2020-05-31T13:22:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29763#issuecomment-556826",
    "user": "https://github.com/petRUShka"
}
```

<a id='comment:4'></a>
You are right, but it is possible to notify user that there is dangerous place in his code through warning or with some other instrument.

It is very convenient to have that conversion at least in predictable cases like I've mentioned in the issue.



---

archive/issue_events_086278.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-21T03:12:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/29763",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29763#event-86278"
}
```



---

archive/issue_events_086279.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/29763",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29763#event-86279"
}
```



---

archive/issue_events_086280.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/29763",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29763#event-86280"
}
```



---

archive/issue_comments_556827.json:
```json
{
    "body": "<a id='comment:7'></a>\nSage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-03-24T02:04:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/29763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/29763#issuecomment-556827",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:7'></a>
Sage development has entered the release candidate phase for 9.3. Setting a new milestone for this ticket based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_086281.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-24T02:04:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/29763",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29763#event-86281"
}
```



---

archive/issue_events_086282.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-24T02:04:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/29763",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29763#event-86282"
}
```



---

archive/issue_events_086283.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-09T21:26:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/29763",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29763#event-86283"
}
```



---

archive/issue_events_086284.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-09T21:26:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/29763",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29763#event-86284"
}
```



---

archive/issue_events_086285.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/29763",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29763#event-86285"
}
```



---

archive/issue_events_086286.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/29763",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29763#event-86286"
}
```



---

archive/issue_events_086287.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/29763",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29763#event-86287"
}
```



---

archive/issue_events_086288.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/29763",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29763#event-86288"
}
```



---

archive/issue_events_086289.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/29763",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29763#event-86289"
}
```



---

archive/issue_events_086290.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/29763",
    "milestone": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/29763#event-86290"
}
```
