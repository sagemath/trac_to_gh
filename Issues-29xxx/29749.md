# Issue 29749: incorrect package name for python37-urllib3 on Cygwin in installation guide

archive/issues_029512.json:
```json
{
    "assignees": [],
    "body": "`./build/pkgs/cygwin.txt` lists the package name `python37-urllib` but the correct name of the package is `python37-urllib3`:\n\nhttps://cygwin.com/cgi-bin2/package-cat.cgi?file=x86_64%2Fpython37-urllib3%2Fpython37-urllib3-1.24.1-1&grep=urllib\n\nKeywords: **sd109**\n\nBranch/Commit: **[cd4c063](https://github.com/sagemath/sagetrac-mirror/commit/cd4c06307cce805351e7c75037e4bd7c2ebe61c3)**\n\nReviewer: **Julian R\u00fcth**\n\nAuthor: **Erik Bray**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/29749_\n\n",
    "closed_at": "2020-06-03T22:30:55Z",
    "created_at": "2020-05-28T12:06:21Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "incorrect package name for python37-urllib3 on Cygwin in installation guide",
    "type": "issue",
    "updated_at": "2020-06-03T22:30:55Z",
    "url": "https://github.com/sagemath/sage/issues/29749",
    "user": "https://github.com/embray"
}
```
`./build/pkgs/cygwin.txt` lists the package name `python37-urllib` but the correct name of the package is `python37-urllib3`:

https://cygwin.com/cgi-bin2/package-cat.cgi?file=x86_64%2Fpython37-urllib3%2Fpython37-urllib3-1.24.1-1&grep=urllib

Keywords: **sd109**

Branch/Commit: **[cd4c063](https://github.com/sagemath/sagetrac-mirror/commit/cd4c06307cce805351e7c75037e4bd7c2ebe61c3)**

Reviewer: **Julian Rüth**

Author: **Erik Bray**

_Issue created by migration from https://trac.sagemath.org/ticket/29749_





---

archive/issue_events_391104.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2020-05-28T12:06:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/29749",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29749#event-391104"
}
```



---

archive/issue_events_391105.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2020-05-28T12:06:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29749",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29749#event-391105"
}
```



---

archive/issue_events_391106.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2020-05-28T12:06:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29749",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29749#event-391106"
}
```



---

archive/issue_events_391107.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2020-05-28T12:06:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29749",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29749#event-391107"
}
```



---

archive/issue_comments_472400.json:
```json
{
    "body": "Branch: **[u/embray/issue-29749](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/issue-29749)**",
    "created_at": "2020-05-28T12:38:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29749",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29749#issuecomment-472400",
    "user": "https://github.com/embray"
}
```

Branch: **[u/embray/issue-29749](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/issue-29749)**



---

archive/issue_events_391108.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2020-05-28T12:38:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29749",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29749#event-391108"
}
```



---

archive/issue_comments_472401.json:
```json
{
    "body": "Author: **Erik Bray**",
    "created_at": "2020-05-28T12:38:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29749",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29749#issuecomment-472401",
    "user": "https://github.com/embray"
}
```

Author: **Erik Bray**



---

archive/issue_comments_472402.json:
```json
{
    "body": "Commit: **[cd4c063](https://github.com/sagemath/sagetrac-mirror/commit/cd4c06307cce805351e7c75037e4bd7c2ebe61c3)**",
    "created_at": "2020-05-28T12:38:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29749",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29749#issuecomment-472402",
    "user": "https://github.com/embray"
}
```

Commit: **[cd4c063](https://github.com/sagemath/sagetrac-mirror/commit/cd4c06307cce805351e7c75037e4bd7c2ebe61c3)**



---

archive/issue_comments_472403.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cd4c06307cce805351e7c75037e4bd7c2ebe61c3\">cd4c063</a></td><td><code>Trac #29749: correct package name for python37-urllib3 on Cygwin</code></td></tr></table>\n",
    "created_at": "2020-05-28T12:38:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29749",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29749#issuecomment-472403",
    "user": "https://github.com/embray"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cd4c06307cce805351e7c75037e4bd7c2ebe61c3">cd4c063</a></td><td><code>Trac #29749: correct package name for python37-urllib3 on Cygwin</code></td></tr></table>




---

archive/issue_events_391109.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2020-05-28T12:40:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29749",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29749#event-391109"
}
```



---

archive/issue_events_391110.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2020-05-28T12:40:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/29749",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29749#event-391110"
}
```



---

archive/issue_comments_472404.json:
```json
{
    "body": "Reviewer: **Julian R\u00fcth**",
    "created_at": "2020-05-28T12:40:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29749",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29749#issuecomment-472404",
    "user": "https://github.com/saraedum"
}
```

Reviewer: **Julian Rüth**



---

archive/issue_comments_472405.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThanks for catching this. Just a quick note -- it's not just \"documentation\", also the new [GitHub](../wiki/GitHub) CI workflow uses these package lists, see https://github.com/sagemath/sage/actions?query=workflow%3A%22CI+cygwin-standard%22",
    "created_at": "2020-05-28T18:17:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29749",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29749#issuecomment-472405",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:3'>Comment 3:</a>
Thanks for catching this. Just a quick note -- it's not just "documentation", also the new [GitHub](../wiki/GitHub) CI workflow uses these package lists, see https://github.com/sagemath/sage/actions?query=workflow%3A%22CI+cygwin-standard%22



---

archive/issue_comments_472406.json:
```json
{
    "body": "Changed keywords from **** to **sd109**",
    "created_at": "2020-05-28T18:17:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29749",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29749#issuecomment-472406",
    "user": "https://github.com/mkoeppe"
}
```

Changed keywords from **** to **sd109**



---

archive/issue_events_391111.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-06-03T22:30:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/29749",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29749#event-391111"
}
```



---

archive/issue_events_391112.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "6932618e024842b9258271c5e3d96eb09a27195c",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-06-03T22:30:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/29749",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/29749#event-391112"
}
```



---

archive/issue_comments_472407.json:
```json
{
    "body": "Changed branch from **[u/embray/issue-29749](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/issue-29749)** to **[cd4c063](https://github.com/sagemath/sagetrac-mirror/commit/cd4c06307cce805351e7c75037e4bd7c2ebe61c3)**",
    "created_at": "2020-06-03T22:30:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/29749",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/29749#issuecomment-472407",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/embray/issue-29749](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/issue-29749)** to **[cd4c063](https://github.com/sagemath/sagetrac-mirror/commit/cd4c06307cce805351e7c75037e4bd7c2ebe61c3)**
