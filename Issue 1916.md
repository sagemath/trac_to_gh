# Issue 1916: notebook -- implement a way to turn off word wrap globally in the notebook

archive/issues_001916.json:
```json
{
    "body": "Assignee: boothby\n\nI'm not sure what interface to use; comments welcome. Implementing this on the server side would likely be very very easy.  The hard part is really the design. \n\nIssue created by migration from https://trac.sagemath.org/ticket/1916\n\n",
    "created_at": "2008-01-24T20:09:30Z",
    "labels": [
        "component: notebook"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "notebook -- implement a way to turn off word wrap globally in the notebook",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/1916",
    "user": "https://github.com/williamstein"
}
```
Assignee: boothby

I'm not sure what interface to use; comments welcome. Implementing this on the server side would likely be very very easy.  The hard part is really the design. 

Issue created by migration from https://trac.sagemath.org/ticket/1916





---

archive/issue_comments_012106.json:
```json
{
    "body": "This is a duplicate of #1468. Please close this if the other is also closed.",
    "created_at": "2009-11-15T15:54:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1916",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1916#issuecomment-12106",
    "user": "https://github.com/TimDumol"
}
```

This is a duplicate of #1468. Please close this if the other is also closed.



---

archive/issue_comments_012107.json:
```json
{
    "body": "Resolution: duplicate",
    "created_at": "2009-11-19T21:40:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1916",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1916#issuecomment-12107",
    "user": "https://github.com/williamstein"
}
```

Resolution: duplicate



---

archive/issue_events_002072.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-11-19T21:40:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/1916",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1916#event-2072"
}
```



---

archive/issue_comments_012108.json:
```json
{
    "body": "This is a dup of #1468.",
    "created_at": "2009-11-19T21:40:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1916",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1916#issuecomment-12108",
    "user": "https://github.com/williamstein"
}
```

This is a dup of #1468.
