...
sage: assume(x>2)
sage: solve(x<3,x)
[This is the Trac macro *x < 3* that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#x < 3-macro)
sage: solve((x>2,x<3),x)
[This is the Trac macro *2 < x, x < 3* that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#2 < x, x < 3-macro)
```
...
```
sage: solve((x^2<1),x)
[This is the Trac macro *x > -1, x < 1* that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#x > -1, x < 1-macro)
sage: solve((x^2>1),x)
[[x < -1], [x > 1]]
```
