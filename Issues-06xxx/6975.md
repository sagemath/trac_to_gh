# Issue 6975: flint package fixed to work in cygwin

archive/issues_006975.json:
```json
{
    "body": "fixed package is here:\n\nhttp://wstein.org/home/ondrej/patches/flint-1.3.0.p3.spkg\n\nIssue created by migration from https://trac.sagemath.org/ticket/6975\n\n",
    "closed_at": "2009-09-28T04:39:50Z",
    "created_at": "2009-09-21T05:19:03Z",
    "labels": [
        "component: algebra",
        "bug",
        "duplicate"
    ],
    "title": "flint package fixed to work in cygwin",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/6975",
    "user": "https://github.com/certik"
}
```
fixed package is here:

http://wstein.org/home/ondrej/patches/flint-1.3.0.p3.spkg

Issue created by migration from https://trac.sagemath.org/ticket/6975





---

archive/issue_events_047473.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-21T05:20:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/6975",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6975#event-47473"
}
```



---

archive/issue_events_047474.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-21T05:41:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/6975",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6975#event-47474"
}
```



---

archive/issue_events_047475.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-21T05:41:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/6975",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6975#event-47475"
}
```



---

archive/issue_comments_052726.json:
```json
{
    "body": "<a id='comment:3'></a>\nSee #6919 for a FLINT 1.5.0 spkg which is based off the spkg here.",
    "created_at": "2009-09-25T05:17:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6975",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6975#issuecomment-52726",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:3'></a>
See #6919 for a FLINT 1.5.0 spkg which is based off the spkg here.



---

archive/issue_events_047476.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-09-28T04:39:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/6975",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6975#event-47476"
}
```



---

archive/issue_events_047477.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-09-28T04:39:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/6975",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6975#event-47477"
}
```



---

archive/issue_events_047478.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-09-28T04:39:50Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/6975",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6975#event-47478"
}
```



---

archive/issue_events_047479.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-09-28T04:39:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/6975",
    "label": "duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6975#event-47479"
}
```



---

archive/issue_comments_052727.json:
```json
{
    "body": "<a id='comment:4'></a>\nClosing this ticket as a duplicate of #6919.",
    "created_at": "2009-09-28T04:39:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6975",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6975#issuecomment-52727",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:4'></a>
Closing this ticket as a duplicate of #6919.
