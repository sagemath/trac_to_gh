# Issue 6800: formal/lazy/infinite powerseries

archive/issues_006800.json:
```json
{
    "body": "New code that implements lazy power and Laurent series.\n\n**Assignee:** @burcin\n\n**CC:**  sage-combinat @mantepse\n\n**Keywords:** LazyPowerSeries\n\n**Reviewer:** Martin Rubey\n\n**Author:** Henryk Trappmann\n\nIssue created by migration from https://trac.sagemath.org/ticket/6800\n\n",
    "closed_at": "2022-09-01T02:30:35Z",
    "created_at": "2009-08-22T07:32:14Z",
    "labels": [
        "component: algebra",
        "enhancement",
        "invalid"
    ],
    "title": "formal/lazy/infinite powerseries",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/6800",
    "user": "https://trac.sagemath.org/admin/accounts/users/Henryk.Trappmann"
}
```
New code that implements lazy power and Laurent series.

**Assignee:** @burcin

**CC:**  sage-combinat @mantepse

**Keywords:** LazyPowerSeries

**Reviewer:** Martin Rubey

**Author:** Henryk Trappmann

Issue created by migration from https://trac.sagemath.org/ticket/6800





---

archive/attachments_008136.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "12846.patch",
    "asset_url": "tarball://root/attachments/ticket6800/12846.patch",
    "created_at": "2009-08-22T07:33:49Z",
    "issue": "https://github.com/sagemath/sage/issues/6800",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket6800/12846.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/Henryk.Trappmann"
}
```



---

archive/issue_comments_049544.json:
```json
{
    "body": "Attachment [12846.patch](https://github.com/sagemath/sage/files/ticket6800/12846.patch) by Henryk.Trappmann created at 2009-08-22 07:33:49\n\npatch adds the file formal_powerseries.py",
    "created_at": "2009-08-22T07:33:49Z",
    "issue": "https://github.com/sagemath/sage/issues/6800",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6800#issuecomment-49544",
    "user": "https://trac.sagemath.org/admin/accounts/users/Henryk.Trappmann"
}
```

Attachment [12846.patch](https://github.com/sagemath/sage/files/ticket6800/12846.patch) by Henryk.Trappmann created at 2009-08-22 07:33:49

patch adds the file formal_powerseries.py



---

archive/issue_events_045105.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-12-08T10:03:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6800",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6800#event-45105"
}
```



---

archive/issue_events_045106.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-12-08T10:03:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6800",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6800#event-45106"
}
```



---

archive/issue_comments_049545.json:
```json
{
    "body": "<a id='comment:1'></a>\nThe documentation needs some serious reformatting to adhere to the ReST format.  I am cc-ing sage-combinat because a lot of people there would be interested in formal power and Laurent series.",
    "created_at": "2009-12-08T10:03:09Z",
    "issue": "https://github.com/sagemath/sage/issues/6800",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6800#issuecomment-49545",
    "user": "https://github.com/aghitza"
}
```

<a id='comment:1'></a>
The documentation needs some serious reformatting to adhere to the ReST format.  I am cc-ing sage-combinat because a lot of people there would be interested in formal power and Laurent series.



---

archive/issue_comments_049546.json:
```json
{
    "body": "**Author:** Henryk Trappmann",
    "created_at": "2009-12-08T10:03:09Z",
    "issue": "https://github.com/sagemath/sage/issues/6800",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6800#issuecomment-49546",
    "user": "https://github.com/aghitza"
}
```

**Author:** Henryk Trappmann



---

archive/issue_events_045107.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-12-08T10:03:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6800",
    "label": "component: calculus",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6800#event-45107"
}
```



---

archive/issue_events_045108.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-12-08T10:03:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6800",
    "label": "component: algebra",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6800#event-45108"
}
```



---

archive/issue_comments_049547.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-New code that implements lazy power and Laurant series.\n+New code that implements lazy power and Laurent series.\n``````\n",
    "created_at": "2009-12-08T10:03:09Z",
    "issue": "https://github.com/sagemath/sage/issues/6800",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6800#issuecomment-49547",
    "user": "https://github.com/aghitza"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1 +1 @@
-New code that implements lazy power and Laurant series.
+New code that implements lazy power and Laurent series.
``````




---

archive/issue_events_045109.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6800",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6800#event-45109"
}
```



---

archive/issue_events_045110.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6800",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6800#event-45110"
}
```



---

archive/issue_comments_049548.json:
```json
{
    "body": "**Changing keywords** from \"\" to \"LazyPowerSeries\".",
    "created_at": "2014-01-10T20:48:44Z",
    "issue": "https://github.com/sagemath/sage/issues/6800",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6800#issuecomment-49548",
    "user": "https://github.com/mantepse"
}
```

**Changing keywords** from "" to "LazyPowerSeries".



---

archive/issue_events_045111.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6800",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6800#event-45111"
}
```



---

archive/issue_events_045112.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6800",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6800#event-45112"
}
```



---

archive/issue_comments_049549.json:
```json
{
    "body": "<a id='comment:5'></a>\nI think although there is `combinat.species.series.LazyPowerSeries` this implementation would still be good to have, as the implementation in combinat misses many features included here. It is also needed for P-finite sequences. \n\nHowever, I don't think it's right to define all special functions anew: the ring or a static function should be able to create a series from a symbolic expression (interpreted as e.g.f.), and , in case of a rational polynomial, delegate to CFiniteSequence (#15714).\n\nThere were a few failures:\n\n```\n   1 of  39 in sage.rings.formal_powerseries.FormalPowerSeries\n   1 of   6 in sage.rings.formal_powerseries.FormalPowerSeries.nipow\n   1 of   6 in sage.rings.formal_powerseries.FormalPowerSeries.pow\n   3 of   8 in sage.rings.formal_powerseries.FormalPowerSeries0.abel\n   1 of   4 in sage.rings.formal_powerseries.decidable0\n```",
    "created_at": "2014-03-15T17:01:31Z",
    "issue": "https://github.com/sagemath/sage/issues/6800",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6800#issuecomment-49549",
    "user": "https://github.com/rwst"
}
```

<a id='comment:5'></a>
I think although there is `combinat.species.series.LazyPowerSeries` this implementation would still be good to have, as the implementation in combinat misses many features included here. It is also needed for P-finite sequences. 

However, I don't think it's right to define all special functions anew: the ring or a static function should be able to create a series from a symbolic expression (interpreted as e.g.f.), and , in case of a rational polynomial, delegate to CFiniteSequence (#15714).

There were a few failures:

```
   1 of  39 in sage.rings.formal_powerseries.FormalPowerSeries
   1 of   6 in sage.rings.formal_powerseries.FormalPowerSeries.nipow
   1 of   6 in sage.rings.formal_powerseries.FormalPowerSeries.pow
   3 of   8 in sage.rings.formal_powerseries.FormalPowerSeries0.abel
   1 of   4 in sage.rings.formal_powerseries.decidable0
```



---

archive/issue_events_045113.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6800",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6800#event-45113"
}
```



---

archive/issue_events_045114.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6800",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6800#event-45114"
}
```



---

archive/issue_events_045115.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6800",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6800#event-45115"
}
```



---

archive/issue_events_045116.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6800",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6800#event-45116"
}
```



---

archive/issue_comments_049550.json:
```json
{
    "body": "<a id='comment:8'></a>\nThis is subsumed by #32324.",
    "created_at": "2022-08-11T17:48:51Z",
    "issue": "https://github.com/sagemath/sage/issues/6800",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6800#issuecomment-49550",
    "user": "https://github.com/mantepse"
}
```

<a id='comment:8'></a>
This is subsumed by #32324.



---

archive/issue_events_045117.json:
```json
{
    "actor": "https://github.com/mantepse",
    "created_at": "2022-08-11T17:49:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6800",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6800#event-45117"
}
```



---

archive/issue_events_045118.json:
```json
{
    "actor": "https://github.com/mantepse",
    "created_at": "2022-08-11T17:49:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6800",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6800#event-45118"
}
```



---

archive/issue_events_045119.json:
```json
{
    "actor": "https://github.com/mantepse",
    "created_at": "2022-08-11T17:49:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6800",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6800#event-45119"
}
```



---

archive/issue_events_045120.json:
```json
{
    "actor": "https://github.com/mantepse",
    "created_at": "2022-08-11T17:49:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6800",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6800#event-45120"
}
```



---

archive/issue_events_045121.json:
```json
{
    "actor": "https://github.com/mantepse",
    "created_at": "2022-08-11T17:49:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6800",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6800#event-45121"
}
```



---

archive/issue_comments_049551.json:
```json
{
    "body": "**Reviewer:** Martin Rubey",
    "created_at": "2022-08-11T17:49:46Z",
    "issue": "https://github.com/sagemath/sage/issues/6800",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6800#issuecomment-49551",
    "user": "https://github.com/mantepse"
}
```

**Reviewer:** Martin Rubey



---

archive/issue_events_045122.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-01T02:30:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6800",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6800#event-45122"
}
```



---

archive/issue_events_045123.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-01T02:30:35Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6800",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6800#event-45123"
}
```
