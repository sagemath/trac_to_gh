# Issue 6798: fix doctest timeout in modules/vector_double_dense.pyx

archive/issues_006798.json:
```json
{
    "body": "On Solaris 10 update 7 (SPARC), the following tests timed out. Both ECL and Maxima were updated - ECL version 9.8.4 (see trac #6564); Maxima version 5.19.1 (see trac #6699). Updated spkgs can be found here. I'm not sure if this is new or not since updating Maxima + ECL, so it may or may not be related to that. \n\nhttp://sage.math.washington.edu/home/kirkby/Solaris-fixes/ecl-9.8.4/\n\nhttp://sage.math.washington.edu/home/kirkby/Solaris-fixes/maxima-5.19.1/\n\n```\n\n----------------------------------------------------------------------\n| Sage Version 4.1.1, Release Date: 2009-08-14                       |\n| Type notebook() for the GUI, and license() for information.        |\n----------------------------------------------------------------------\nThu Aug 20 20:02:37 BST 2009\ndsage-trial tmp directory doesn't exist - creating ...\nThis script will run the unit tests for DSage\n```\n\n<SNIP>\n\n```\nsage -t  \"devel/sage/sage/modules/vector_double_dense.pyx\"\n         [71.4 s]\n\nsage -t  \"devel/sage/sage/lfunctions/sympow.py\"\n*** *** Error: TIMED OUT! PROCESS KILLED! *** ***\n*** *** Error: TIMED OUT! *** ***\n*** *** Error: TIMED OUT! *** ***\n         [361.1 s]\nsage -t  \"devel/sage/sage/lfunctions/all.py\"\n         [1.2 s]\n```\n\n\n**Reviewer:** Jeroen Demeyer\n\nIssue created by migration from https://trac.sagemath.org/ticket/6798\n\n",
    "closed_at": "2013-12-05T08:07:53Z",
    "created_at": "2009-08-21T08:16:12Z",
    "labels": [
        "component: porting: solaris",
        "bug",
        "invalid"
    ],
    "title": "fix doctest timeout in modules/vector_double_dense.pyx",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/6798",
    "user": "https://trac.sagemath.org/admin/accounts/users/drkirkby"
}
```
On Solaris 10 update 7 (SPARC), the following tests timed out. Both ECL and Maxima were updated - ECL version 9.8.4 (see trac #6564); Maxima version 5.19.1 (see trac #6699). Updated spkgs can be found here. I'm not sure if this is new or not since updating Maxima + ECL, so it may or may not be related to that. 

http://sage.math.washington.edu/home/kirkby/Solaris-fixes/ecl-9.8.4/

http://sage.math.washington.edu/home/kirkby/Solaris-fixes/maxima-5.19.1/

```

----------------------------------------------------------------------
| Sage Version 4.1.1, Release Date: 2009-08-14                       |
| Type notebook() for the GUI, and license() for information.        |
----------------------------------------------------------------------
Thu Aug 20 20:02:37 BST 2009
dsage-trial tmp directory doesn't exist - creating ...
This script will run the unit tests for DSage
```

<SNIP>

```
sage -t  "devel/sage/sage/modules/vector_double_dense.pyx"
         [71.4 s]

sage -t  "devel/sage/sage/lfunctions/sympow.py"
*** *** Error: TIMED OUT! PROCESS KILLED! *** ***
*** *** Error: TIMED OUT! *** ***
*** *** Error: TIMED OUT! *** ***
         [361.1 s]
sage -t  "devel/sage/sage/lfunctions/all.py"
         [1.2 s]
```


**Reviewer:** Jeroen Demeyer

Issue created by migration from https://trac.sagemath.org/ticket/6798





---

archive/issue_events_046090.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/drkirkby",
    "created_at": "2009-11-09T14:05:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/6798",
    "label": "component: algebra",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6798#event-46090"
}
```



---

archive/issue_events_046091.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/drkirkby",
    "created_at": "2009-11-09T14:05:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/6798",
    "label": "component: porting: solaris",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6798#event-46091"
}
```



---

archive/issue_events_046092.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6798",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6798#event-46092"
}
```



---

archive/issue_events_046093.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6798",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6798#event-46093"
}
```



---

archive/issue_events_046094.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-12-02T20:52:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/6798",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6798#event-46094"
}
```



---

archive/issue_comments_050752.json:
```json
{
    "body": "**Upstream:** N/A",
    "created_at": "2013-12-02T20:52:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6798",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6798#issuecomment-50752",
    "user": "https://github.com/jdemeyer"
}
```

**Upstream:** N/A



---

archive/issue_events_046095.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-12-02T20:52:09Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6798",
    "milestone": "sage-5.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6798#event-46095"
}
```



---

archive/issue_events_046096.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-12-02T20:52:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/6798",
    "label": "duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6798#event-46096"
}
```



---

archive/issue_comments_050753.json:
```json
{
    "body": "**Reviewer:** Jeroen Demeyer",
    "created_at": "2013-12-02T20:52:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6798",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6798#issuecomment-50753",
    "user": "https://github.com/jdemeyer"
}
```

**Reviewer:** Jeroen Demeyer



---

archive/issue_events_046097.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-12-02T20:52:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/6798",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6798#event-46097"
}
```



---

archive/issue_events_046098.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-12-02T20:52:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/6798",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6798#event-46098"
}
```



---

archive/issue_events_046099.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-12-05T08:07:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/6798",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6798#event-46099"
}
```



---

archive/issue_events_046100.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-12-05T08:07:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/6798",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6798#event-46100"
}
```
