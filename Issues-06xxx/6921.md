# Issue 6921: MATLAB crashes on Snow Leopard due to library conflicts

archive/issues_006921.json:
```json
{
    "assignees": [],
    "body": "When using the MATLAB interface in Sage MATLAB crashes on startup. This is due to library conflicts with Sage.\n\nI have created a (minor) patch to use sage-native-execute when starting MATLAB. This fixes the problems on my machine (OS X 10.6) and the interface now passes all doctests.\n\nAssignee: **@williamstein**\n\nAuthor: **Jonathan J. Hunt**\n\nReviewer: **William Stein**\n\nMerged: **Sage 4.1.2.alpha4**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6921_\n\n",
    "closed_at": "2009-09-23T06:36:18Z",
    "created_at": "2009-09-11T04:28:16Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "MATLAB crashes on Snow Leopard due to library conflicts",
    "type": "issue",
    "updated_at": "2009-09-27T09:49:12Z",
    "url": "https://github.com/sagemath/sage/issues/6921",
    "user": "https://github.com/sagetrac-jjh"
}
```
When using the MATLAB interface in Sage MATLAB crashes on startup. This is due to library conflicts with Sage.

I have created a (minor) patch to use sage-native-execute when starting MATLAB. This fixes the problems on my machine (OS X 10.6) and the interface now passes all doctests.

Assignee: **@williamstein**

Author: **Jonathan J. Hunt**

Reviewer: **William Stein**

Merged: **Sage 4.1.2.alpha4**

_Issue created by migration from https://trac.sagemath.org/ticket/6921_





---

archive/issue_events_073873.json:
```json
{
    "actor": "https://github.com/sagetrac-jjh",
    "created_at": "2009-09-11T04:28:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6921",
    "milestone_number": null,
    "milestone_title": "sage-4.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6921#event-73873"
}
```



---

archive/issue_events_073874.json:
```json
{
    "actor": "https://github.com/sagetrac-jjh",
    "created_at": "2009-09-11T04:28:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6921",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "component: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6921#event-73874"
}
```



---

archive/issue_events_073875.json:
```json
{
    "actor": "https://github.com/sagetrac-jjh",
    "created_at": "2009-09-11T04:28:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6921",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6921#event-73875"
}
```



---

archive/issue_events_073876.json:
```json
{
    "actor": "https://github.com/sagetrac-jjh",
    "created_at": "2009-09-11T04:28:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6921",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6921#event-73876"
}
```



---

archive/issue_comments_050836.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[matlab-native-execute.patch.gz](https://github.com/sagemath/sage/files/ticket6921/matlab-native-execute.patch.gz)**",
    "created_at": "2009-09-11T04:33:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6921",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6921#issuecomment-50836",
    "user": "https://github.com/sagetrac-jjh"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[matlab-native-execute.patch.gz](https://github.com/sagemath/sage/files/ticket6921/matlab-native-execute.patch.gz)**



---

archive/issue_events_073877.json:
```json
{
    "actor": "https://github.com/sagetrac-jjh",
    "created_at": "2009-09-11T04:33:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6921",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6921#event-73877"
}
```



---

archive/issue_comments_050837.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nI can't test the exact setup, but in all cases using sage-native-execute is a good idea, and this does work on Linux, so positive review.",
    "created_at": "2009-09-22T14:49:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6921",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6921#issuecomment-50837",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:2'>Comment 2:</a>
I can't test the exact setup, but in all cases using sage-native-execute is a good idea, and this does work on Linux, so positive review.



---

archive/issue_events_073878.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-22T14:49:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6921",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6921#event-73878"
}
```



---

archive/issue_events_073879.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-22T14:49:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6921",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6921#event-73879"
}
```



---

archive/issue_comments_050838.json:
```json
{
    "body": "Author: **Jonathan J. Hunt**",
    "created_at": "2009-09-23T06:36:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6921",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6921#issuecomment-50838",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Author: **Jonathan J. Hunt**



---

archive/issue_events_073880.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-23T06:36:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6921",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6921#event-73880"
}
```



---

archive/issue_events_073881.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-23T06:36:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6921",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6921#event-73881"
}
```



---

archive/issue_comments_050839.json:
```json
{
    "body": "Merged: **Sage 4.1.2.alpha3**",
    "created_at": "2009-09-23T06:36:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6921",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6921#issuecomment-50839",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **Sage 4.1.2.alpha3**



---

archive/issue_comments_050840.json:
```json
{
    "body": "Reviewer: **William Stein**",
    "created_at": "2009-09-23T06:36:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6921",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6921#issuecomment-50840",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Reviewer: **William Stein**



---

archive/issue_events_073882.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-23T06:36:18Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6921",
    "milestone_number": null,
    "milestone_title": "sage-4.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6921#event-73882"
}
```



---

archive/issue_comments_050841.json:
```json
{
    "body": "Changed merged from **Sage 4.1.2.alpha3** to **Sage 4.1.2.alpha4**",
    "created_at": "2009-09-27T09:49:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6921",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6921#issuecomment-50841",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Changed merged from **Sage 4.1.2.alpha3** to **Sage 4.1.2.alpha4**



---

archive/issue_comments_050842.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nThere is no 4.1.2.alpha3. Sage 4.1.2.alpha3 was William Stein's release for working on making the notebook a standalone package.",
    "created_at": "2009-09-27T09:49:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6921",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6921#issuecomment-50842",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<a id='comment:4'>Comment 4:</a>
There is no 4.1.2.alpha3. Sage 4.1.2.alpha3 was William Stein's release for working on making the notebook a standalone package.
