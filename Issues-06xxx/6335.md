# Issue 6335: optional doctest failure -- kash interface (trivial to fix)

archive/issues_006335.json:
```json
{
    "body": "\n```\nsage -t -long --optional devel/sage/sage/interfaces/kash.py\n**********************************************************************\nFile \"/scratch/wstein/build/sage-4.0.2.alpha3/devel/sage-main/sage/interfaces/kash.py\", line 68:\n    sage: a = kash('(9 - 7) * (5 + 6)'); a              # optional -- kash\nExpected nothing\nGot:\n    22\n**********************************************************************\n1 items had failures:\n   1 of 103 in __main__.example_0\n***Test Failed*** 1 failures.\n\n```\n\nAssignee: tbd\n\nStatus: needs_work\n\nIssue created by migration from https://trac.sagemath.org/ticket/6335\n\n",
    "created_at": "2009-06-16T15:18:16Z",
    "labels": [
        "component: packages: optional",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.4",
    "title": "optional doctest failure -- kash interface (trivial to fix)",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/6335",
    "user": "https://github.com/williamstein"
}
```

```
sage -t -long --optional devel/sage/sage/interfaces/kash.py
**********************************************************************
File "/scratch/wstein/build/sage-4.0.2.alpha3/devel/sage-main/sage/interfaces/kash.py", line 68:
    sage: a = kash('(9 - 7) * (5 + 6)'); a              # optional -- kash
Expected nothing
Got:
    22
**********************************************************************
1 items had failures:
   1 of 103 in __main__.example_0
***Test Failed*** 1 failures.

```

Assignee: tbd

Status: needs_work

Issue created by migration from https://trac.sagemath.org/ticket/6335





---

archive/issue_comments_055296.json:
```json
{
    "body": "Changing upstream from \"\" to \"N/A\"",
    "created_at": "2013-02-18T21:43:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6335#issuecomment-55296",
    "user": "https://github.com/KPanComputes"
}
```

Changing upstream from "" to "N/A"



---

archive/issue_comments_055297.json:
```json
{
    "body": "<a id='comment:1'></a>\nOn my Sage 5.7.beta4, all tests pass. \n\n```\nsage -t  \"devel/sage-main/sage/interfaces/kash.py\"          \n\t [17.2 s]\n \n----------------------------------------------------------------------\nAll tests passed!\nTotal time for all tests: 17.4 seconds\n```\n\nSo, I am marking this invalid.",
    "created_at": "2013-02-18T21:43:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6335#issuecomment-55297",
    "user": "https://github.com/KPanComputes"
}
```

<a id='comment:1'></a>
On my Sage 5.7.beta4, all tests pass. 

```
sage -t  "devel/sage-main/sage/interfaces/kash.py"          
	 [17.2 s]
 
----------------------------------------------------------------------
All tests passed!
Total time for all tests: 17.4 seconds
```

So, I am marking this invalid.



---

archive/issue_comments_055298.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2013-02-18T21:43:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6335#issuecomment-55298",
    "user": "https://github.com/KPanComputes"
}
```

Changing status from new to needs_review.



---

archive/issue_events_025426.json:
```json
{
    "actor": "https://github.com/KPanComputes",
    "created_at": "2013-02-18T21:43:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6335",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6335#event-25426"
}
```



---

archive/issue_comments_055299.json:
```json
{
    "body": "<a id='comment:2'></a>\nOops!! There are two optional tests failing now!! So, not invalid.",
    "created_at": "2013-02-18T21:45:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6335#issuecomment-55299",
    "user": "https://github.com/KPanComputes"
}
```

<a id='comment:2'></a>
Oops!! There are two optional tests failing now!! So, not invalid.



---

archive/issue_events_025427.json:
```json
{
    "actor": "https://github.com/KPanComputes",
    "created_at": "2013-02-18T21:45:35Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6335",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6335#event-25427"
}
```



---

archive/issue_events_025428.json:
```json
{
    "actor": "https://github.com/KPanComputes",
    "created_at": "2013-02-18T21:45:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6335",
    "milestone": "sage-5.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6335#event-25428"
}
```



---

archive/issue_comments_055300.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2013-02-18T21:45:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6335",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6335#issuecomment-55300",
    "user": "https://github.com/KPanComputes"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_events_025429.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6335",
    "milestone": "sage-5.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6335#event-25429"
}
```



---

archive/issue_events_025430.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6335",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6335#event-25430"
}
```



---

archive/issue_events_025431.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6335",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6335#event-25431"
}
```



---

archive/issue_events_025432.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6335",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6335#event-25432"
}
```



---

archive/issue_events_025433.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6335",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6335#event-25433"
}
```



---

archive/issue_events_025434.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6335",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6335#event-25434"
}
```



---

archive/issue_events_025435.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6335",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6335#event-25435"
}
```



---

archive/issue_events_025436.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6335",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6335#event-25436"
}
```
