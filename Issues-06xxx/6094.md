...
```json
{
    "body": "<a id='comment:1'></a>\"method\" is not only easier to say, but it's easier to spell, and probably is more memorable to people...",
...
<a id='comment:1'></a>"method" is not only easier to say, but it's easier to spell, and probably is more memorable to people...
...
```json
{
    "body": "<a id='comment:3'></a>okay, I didn't realize there was a standard.  The weight of history changes the situation a bit.",
...
<a id='comment:3'></a>okay, I didn't realize there was a standard.  The weight of history changes the situation a bit.
...
```json
{
    "body": "<a id='comment:5'></a>Thanks. I'll get to these when #5701 is applied unless someone says I should just create a patch based on the patches there.",
...
<a id='comment:5'></a>Thanks. I'll get to these when #5701 is applied unless someone says I should just create a patch based on the patches there.
...
```json
{
    "body": "<a id='comment:6'></a>Attachment [trac_6094-method-vs-algorithm.patch](tarball://root/attachments/some-uuid/ticket6094/trac_6094-method-vs-algorithm.patch) by @wdjoyner created at 2009-05-27 10:16:12\n\nThe current patch passes sage -testall with guava removed and all patched on #5701 applied.",
...
<a id='comment:6'></a>Attachment [trac_6094-method-vs-algorithm.patch](tarball://root/attachments/some-uuid/ticket6094/trac_6094-method-vs-algorithm.patch) by @wdjoyner created at 2009-05-27 10:16:12
...
<a id='comment:8'></a>Fails to apply cleanly to Sage 4.0.1.
...
```json
{
    "body": "<a id='comment:9'></a>I don't know how to rebase. Can someone point me to a reference? I might be able to do it this weekend.",
...
<a id='comment:9'></a>I don't know how to rebase. Can someone point me to a reference? I might be able to do it this weekend.
...
```json
{
    "body": "<a id='comment:11'></a>Passes all tests.  There are more instances of the use of `method=` in coding/code_bounds.py which will have a patch at #7971.  Positive review.",
    "created_at": "2010-01-18T05:14:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6094",
    "type": "issue_comment",
...
<a id='comment:11'></a>Passes all tests.  There are more instances of the use of `method=` in coding/code_bounds.py which will have a patch at #7971.  Positive review.
...
```json
{
    "body": "<a id='comment:13'></a>It was pointed out by people in our status reports session that this patch violates our deprecation policy.  Yuck.  I.e., technically we should do\n\n```\ndef f(algorithm='default', method=None):\n    if method is not None:\n        deprecation('...')\n        algorithm = method\n```\nbasically *everywhere*, then remove them all in a year.  Hmmmm....",
...
<a id='comment:13'></a>It was pointed out by people in our status reports session that this patch violates our deprecation policy.  Yuck.  I.e., technically we should do
...
```json
{
    "body": "<a id='comment:14'></a>I've sent this back to \"needs work\" to await deprecation warnings.  With more to do, include the changes in ccoding/code_bounds.py here as part of this ticket (see #7971).",
    "created_at": "2010-01-18T21:19:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6094",
    "type": "issue_comment",
...
<a id='comment:14'></a>I've sent this back to "needs work" to await deprecation warnings.  With more to do, include the changes in ccoding/code_bounds.py here as part of this ticket (see #7971).
...
```json
{
    "body": "<a id='comment:15'></a>I wonder if it could be done with a decorator?\n\n```\n@deprecate_method\ndef f(..., method=\"foo\"):\n    ...\n\n```",
...
<a id='comment:15'></a>I wonder if it could be done with a decorator?
...
<a id='comment:17'></a>I wrote a new patch which uses the decorator mentioned above. **This patch assumes applying the patch for Trac #9907!**
...
<a id='comment:19'></a>Decorator complaint is bigger than these tickets.  Its on sage-devel.
...
<a id='comment:21'></a>Replying to [comment:20 jsrn]:
> Great, thanks for looking into this. Sorry about the rudimentary stuff on the patch; I'm still learning the ropes.
...
<a id='comment:23'></a>Replying to [comment:22 jsrn]:
> I'm also interested in cleaning up and improving Sage ;-) But you're right. This opened up a can of worms, I didn't expect. 
...
<a id='comment:24'></a>Ok, now it gets hairy (and annoying). In plot.misc, several functions from sage.ext.fast_eval are imported; these in turn imports all sorts of stuff (e.g. rings), which means that there will be circular references from e.g. polynomial_ring when applying this patch. This just underlines the importance of moving the decorators to a general place in Trac #9907. It also means that unless e.g. this patch the one for Trac #9919 is expanded to (illogically) move the sage.ext.fast_eval import statements down into the functions in which they are used, this patch will have to depend on #9907 (which should now probably depend on #9919). Ye gods  -- isolating problems are nice in theory, but...
...
```json
{
    "body": "<a id='comment:25'></a>Ok, so now I've posted fixes for Trac #9919 and #9907. I decided to assume both of those, as it seemed hard to get around either. So, assuming both of those, the just uploaded patch gives (me) no errors. The just uploaded is like yours, Rob, but with the frobenius() doctest fixed.\n\nAs I posted on sage.devel, I also wrote a patch for the decorator-complaints in Trac #9976.\n\nCheers, Johan",
    "created_at": "2010-09-23T14:08:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6094",
    "type": "issue_comment",
...
<a id='comment:25'></a>Ok, so now I've posted fixes for Trac #9919 and #9907. I decided to assume both of those, as it seemed hard to get around either. So, assuming both of those, the just uploaded patch gives (me) no errors. The just uploaded is like yours, Rob, but with the frobenius() doctest fixed.
...
```json
{
    "body": "<a id='comment:26'></a>By the way, this patch contains all changes which were at one point suggested to be contained in Trac #7971. That trac can therefore be closed/invalidated.",
...
<a id='comment:26'></a>By the way, this patch contains all changes which were at one point suggested to be contained in Trac #7971. That trac can therefore be closed/invalidated.
...
```json
{
    "body": "<a id='comment:27'></a>If I apply #6094, #9907, #9919, and #10107 together on top of sage-4.6, all long tests pass. The code looks good.\n\nI have also closed #7971.",
    "created_at": "2010-11-09T20:22:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6094",
    "type": "issue_comment",
...
<a id='comment:27'></a>If I apply #6094, #9907, #9919, and #10107 together on top of sage-4.6, all long tests pass. The code looks good.
...
<a id='comment:28'></a>The patch needs to be rebased to sage-4.6.1.alpha0:
...
```json
{
    "body": "<a id='comment:29'></a>Attachment [trac_6094-4.6.1.alpha0.with_deprecation.patch](tarball://root/attachments/some-uuid/ticket6094/trac_6094-4.6.1.alpha0.with_deprecation.patch) by @johanrosenkilde created at 2010-11-16 10:00:02",
...
<a id='comment:29'></a>Attachment [trac_6094-4.6.1.alpha0.with_deprecation.patch](tarball://root/attachments/some-uuid/ticket6094/trac_6094-4.6.1.alpha0.with_deprecation.patch) by @johanrosenkilde created at 2010-11-16 10:00:02
...
```json
{
    "body": "<a id='comment:30'></a>Robert Miller already gave the green light to the old version, so the review is only for the minor change in sage.matrix.matrix_double_dense made for rebasing.",
...
<a id='comment:30'></a>Robert Miller already gave the green light to the old version, so the review is only for the minor change in sage.matrix.matrix_double_dense made for rebasing.
