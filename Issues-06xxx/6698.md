# Issue 6698: [with patch, positive review] spell-check all modules under sage/games

archive/issues_006698.json:
```json
{
    "body": "As the subject says.\n\nAssignee: tba\n\nResolution: fixed\n\nAuthor: Minh Van Nguyen\n\nReviewer: William Stein\n\nMerged: Sage 4.1.1\n\nIssue created by migration from https://trac.sagemath.org/ticket/6698\n\n",
    "closed_at": "2009-08-12T05:24:56Z",
    "created_at": "2009-08-09T07:38:01Z",
    "labels": [
        "component: documentation",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.1.1",
    "title": "[with patch, positive review] spell-check all modules under sage/games",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/6698",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```
As the subject says.

Assignee: tba

Resolution: fixed

Author: Minh Van Nguyen

Reviewer: William Stein

Merged: Sage 4.1.1

Issue created by migration from https://trac.sagemath.org/ticket/6698





---

archive/issue_comments_061076.json:
```json
{
    "body": "Attachment [trac_6698-spell-check-games.patch](tarball://root/attachments/some-uuid/ticket6698/trac_6698-spell-check-games.patch) by mvngu created at 2009-08-09 07:39:57\n\nbased on Sage 4.1.1.rc2",
    "created_at": "2009-08-09T07:39:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6698",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6698#issuecomment-61076",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Attachment [trac_6698-spell-check-games.patch](tarball://root/attachments/some-uuid/ticket6698/trac_6698-spell-check-games.patch) by mvngu created at 2009-08-09 07:39:57

based on Sage 4.1.1.rc2



---

archive/issue_comments_061077.json:
```json
{
    "body": "Changing author from \"\" to \"Minh Van Nguyen\"",
    "created_at": "2009-08-09T07:40:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6698",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6698#issuecomment-61077",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Changing author from "" to "Minh Van Nguyen"



---

archive/issue_events_027086.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/6698",
    "rename": {
        "from": "spell-check all modules under sage/games",
        "to": "[with patch, needs review] spell-check all modules under sage/games"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6698#event-27086"
}
```



---

archive/issue_events_027087.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/6698",
    "rename": {
        "from": "[with patch, needs review] spell-check all modules under sage/games",
        "to": "[with patch, positive review] spell-check all modules under sage/games"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6698#event-27087"
}
```



---

archive/issue_events_027088.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-08-12T05:24:56Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/6698",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6698#event-27088"
}
```



---

archive/issue_comments_061078.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-08-12T05:24:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6698",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6698#issuecomment-61078",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Resolution: fixed



---

archive/issue_comments_061079.json:
```json
{
    "body": "Changing merged from \"\" to \"Sage 4.1.1\"",
    "created_at": "2009-08-12T05:24:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6698",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6698#issuecomment-61079",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Changing merged from "" to "Sage 4.1.1"



---

archive/issue_comments_061080.json:
```json
{
    "body": "Changing reviewer from \"\" to \"William Stein\"",
    "created_at": "2009-08-12T05:24:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6698",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6698#issuecomment-61080",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Changing reviewer from "" to "William Stein"
