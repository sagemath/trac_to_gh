# Issue 6006: Bring plot/point.py to 100% coverage

archive/issues_006006.json:
```json
{
    "body": "Bring plot/point.py to 100% coverage.\n\n**Assignee:** @kcrisman\n\n**Resolution:** fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/6006\n\n",
    "closed_at": "2009-05-15T07:54:47Z",
    "created_at": "2009-05-08T05:09:27Z",
    "labels": [
        "component: documentation",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.0",
    "title": "Bring plot/point.py to 100% coverage",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/6006",
    "user": "https://github.com/kcrisman"
}
```
Bring plot/point.py to 100% coverage.

**Assignee:** @kcrisman

**Resolution:** fixed

Issue created by migration from https://trac.sagemath.org/ticket/6006





---

archive/issue_comments_058010.json:
```json
{
    "body": "**Changing status** from new to needs_review.",
    "created_at": "2009-05-08T05:13:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6006#issuecomment-58010",
    "user": "https://github.com/kcrisman"
}
```

**Changing status** from new to needs_review.



---

archive/issue_comments_058011.json:
```json
{
    "body": "**Assignee:** @kcrisman",
    "created_at": "2009-05-08T05:13:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6006#issuecomment-58011",
    "user": "https://github.com/kcrisman"
}
```

**Assignee:** @kcrisman



---

archive/issue_comments_058012.json:
```json
{
    "body": "**Changing status** from new to assigned.",
    "created_at": "2009-05-08T05:13:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6006#issuecomment-58012",
    "user": "https://github.com/kcrisman"
}
```

**Changing status** from new to assigned.



---

archive/attachments_007003.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_6006.patch",
    "asset_url": "tarball://root/attachments/some-uuid/ticket6006/trac_6006.patch",
    "created_at": "2009-05-08T05:13:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6006",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.patch",
    "user": "https://github.com/kcrisman"
}
```



---

archive/issue_comments_058013.json:
```json
{
    "body": "<a id='comment:1'></a>\nThis patch also actually implements the height option for the plot3d method of the Point class that the old doctest was actually using through a 3D graphics object.",
    "created_at": "2009-05-08T05:13:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6006#issuecomment-58013",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:1'></a>
This patch also actually implements the height option for the plot3d method of the Point class that the old doctest was actually using through a 3D graphics object.



---

archive/issue_comments_058014.json:
```json
{
    "body": "<a id='comment:2'></a>\nI should also point out that I could not find a way to test for `_allowed_`options for the keywords in Point.plot3d, so I put something about that in the TODO but also pointed it out in the documentation, lest one confuse pointsize and size (et al.).",
    "created_at": "2009-05-08T05:15:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6006#issuecomment-58014",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:2'></a>
I should also point out that I could not find a way to test for `_allowed_`options for the keywords in Point.plot3d, so I put something about that in the TODO but also pointed it out in the documentation, lest one confuse pointsize and size (et al.).



---

archive/issue_comments_058015.json:
```json
{
    "body": "<a id='comment:3'></a>\nSee [http://groups.google.com/group/sage-devel/browse_thread/thread/1adac4035031b140/c36f1f8a7c8a9b43#c36f1f8a7c8a9b43](http://groups.google.com/group/sage-devel/browse_thread/thread/1adac4035031b140/c36f1f8a7c8a9b43#c36f1f8a7c8a9b43) for explanation of why there is no loads(dumps()) test.",
    "created_at": "2009-05-08T16:46:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6006#issuecomment-58015",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:3'></a>
See [http://groups.google.com/group/sage-devel/browse_thread/thread/1adac4035031b140/c36f1f8a7c8a9b43#c36f1f8a7c8a9b43](http://groups.google.com/group/sage-devel/browse_thread/thread/1adac4035031b140/c36f1f8a7c8a9b43#c36f1f8a7c8a9b43) for explanation of why there is no loads(dumps()) test.



---

archive/issue_comments_058016.json:
```json
{
    "body": "**Changing status** from needs_review to needs_work.",
    "created_at": "2009-05-13T04:36:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6006#issuecomment-58016",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Changing status** from needs_review to needs_work.



---

archive/issue_comments_058017.json:
```json
{
    "body": "<a id='comment:4'></a>\nOn line 43, the constructor for `Point(GraphicPrimitive_xydata)` is\n\n```\n43\t    def `__init__`(self, xdata, ydata, options)\n```\nCurrently with Sphinx, any docstring of `__init__` will not be shown in the documentation for `Point(GraphicPrimitive_xydata)`. The situation is the same for every class. So one issue to fix is to copy the examples in `__init__(self, xdata, ydata, options)` and paste them within the docstring for `Point(GraphicPrimitive_xydata)` after line 32. Another issue is, the constructor arguments `xdata, ydata, options` on line 43 are not documented in the docstring for `Point(GraphicPrimitive_xydata)`. Constructor arguments must be documented to explain what they are, and how to properly use them.",
    "created_at": "2009-05-13T04:36:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6006#issuecomment-58017",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:4'></a>
On line 43, the constructor for `Point(GraphicPrimitive_xydata)` is

```
43	    def `__init__`(self, xdata, ydata, options)
```
Currently with Sphinx, any docstring of `__init__` will not be shown in the documentation for `Point(GraphicPrimitive_xydata)`. The situation is the same for every class. So one issue to fix is to copy the examples in `__init__(self, xdata, ydata, options)` and paste them within the docstring for `Point(GraphicPrimitive_xydata)` after line 32. Another issue is, the constructor arguments `xdata, ydata, options` on line 43 are not documented in the docstring for `Point(GraphicPrimitive_xydata)`. Constructor arguments must be documented to explain what they are, and how to properly use them.



---

archive/issue_comments_058018.json:
```json
{
    "body": "<a id='comment:5'></a>\nI was not aware of this.  I would be happy to improve this, and copying examples is easy.  However, could you point me to a file with a correctly documented set of constructor arguments?  I am uncertain how to proceed with that, particularly because I modeled my own documentation after other examples in plot/; the xdata and ydata, for instance, come from the function getxydata or something like that, which is called when a point (as opposed to Point) is made.   Also, what *should* be in the docstring for `__init__`, if not what is currently there?  Thanks for any ideas or model files to look at!",
    "created_at": "2009-05-13T13:35:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6006#issuecomment-58018",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:5'></a>
I was not aware of this.  I would be happy to improve this, and copying examples is easy.  However, could you point me to a file with a correctly documented set of constructor arguments?  I am uncertain how to proceed with that, particularly because I modeled my own documentation after other examples in plot/; the xdata and ydata, for instance, come from the function getxydata or something like that, which is called when a point (as opposed to Point) is made.   Also, what *should* be in the docstring for `__init__`, if not what is currently there?  Thanks for any ideas or model files to look at!



---

archive/issue_comments_058019.json:
```json
{
    "body": "<a id='comment:6'></a>\nReplying to [mvngu](#comment%3A4):\n}\n> Currently with Sphinx, any docstring of `__init__` will not be shown in the documentation for `Point(GraphicPrimitive_xydata)`. The situation is the same for every class. So one issue to fix is to copy the examples in `__init__(self, xdata, ydata, options)` and paste them within the docstring for `Point(GraphicPrimitive_xydata)` after line 32.\n\n\nNo, do not do this. It will be fixed in the future. This is still \"needs work\" due to the other issue.\n\nCheers,\n\nMichael",
    "created_at": "2009-05-13T17:18:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6006#issuecomment-58019",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:6'></a>
Replying to [mvngu](#comment%3A4):
}
> Currently with Sphinx, any docstring of `__init__` will not be shown in the documentation for `Point(GraphicPrimitive_xydata)`. The situation is the same for every class. So one issue to fix is to copy the examples in `__init__(self, xdata, ydata, options)` and paste them within the docstring for `Point(GraphicPrimitive_xydata)` after line 32.


No, do not do this. It will be fixed in the future. This is still "needs work" due to the other issue.

Cheers,

Michael



---

archive/attachments_007004.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_6006-fix.patch",
    "asset_url": "tarball://root/attachments/some-uuid/ticket6006/trac_6006-fix.patch",
    "created_at": "2009-05-14T16:14:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6006",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.patch",
    "user": "https://github.com/kcrisman"
}
```



---

archive/issue_comments_058020.json:
```json
{
    "body": "<a id='comment:7'></a>\n",
    "created_at": "2009-05-14T16:14:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6006#issuecomment-58020",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:7'></a>




---

archive/issue_comments_058021.json:
```json
{
    "body": "**Changing status** from needs_work to needs_review.",
    "created_at": "2009-05-14T16:14:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6006#issuecomment-58021",
    "user": "https://github.com/kcrisman"
}
```

**Changing status** from needs_work to needs_review.



---

archive/issue_comments_058022.json:
```json
{
    "body": "reviewer patch",
    "created_at": "2009-05-15T07:20:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6006#issuecomment-58022",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

reviewer patch



---

archive/issue_events_018226.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-05-15T07:22:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6006",
    "milestone": "sage-4.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6006#event-18226"
}
```



---

archive/attachments_007005.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_6006-reviewer.patch",
    "asset_url": "tarball://root/attachments/some-uuid/ticket6006/trac_6006-reviewer.patch",
    "created_at": "2009-05-15T07:22:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6006",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```



---

archive/issue_comments_058023.json:
```json
{
    "body": "<a id='comment:8'></a>\nPositive review! Apply patches in the following order:\n1. `trac_6006.patch`\n2. `trac_6006-fix.patch`\n3. `trac_6006-reviewer.patch` -- this fixes two trivial typos introduced in `trac_6006.patch`",
    "created_at": "2009-05-15T07:22:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6006#issuecomment-58023",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:8'></a>
Positive review! Apply patches in the following order:
1. `trac_6006.patch`
2. `trac_6006-fix.patch`
3. `trac_6006-reviewer.patch` -- this fixes two trivial typos introduced in `trac_6006.patch`



---

archive/issue_comments_058024.json:
```json
{
    "body": "**Changing status** from needs_review to positive_review.",
    "created_at": "2009-05-15T07:22:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6006#issuecomment-58024",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Changing status** from needs_review to positive_review.



---

archive/issue_comments_058025.json:
```json
{
    "body": "<a id='comment:9'></a>\nMerged all three patches in Sage 4.0.alpha0.\n\nCheers,\n\nMichael",
    "created_at": "2009-05-15T07:54:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6006#issuecomment-58025",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:9'></a>
Merged all three patches in Sage 4.0.alpha0.

Cheers,

Michael



---

archive/issue_events_018227.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-05-15T07:54:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/6006",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6006#event-18227"
}
```



---

archive/issue_comments_058026.json:
```json
{
    "body": "**Resolution:** fixed",
    "created_at": "2009-05-15T07:54:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6006#issuecomment-58026",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

**Resolution:** fixed
