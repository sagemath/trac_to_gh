...
```json
{
    "body": "<a id='comment:1'></a>Minor, but I just fixed two of these elsewhere: it's spelled \"consistent\"",
...
<a id='comment:1'></a>Minor, but I just fixed two of these elsewhere: it's spelled "consistent"
...
<a id='comment:3'></a>The patch applies with some fuzz to 4.0.rc2, but I'm seeing a bunch of doctest failures in sage/rings:
...
<a id='comment:4'></a>In Python 3, "greater than" etc. comparisons between objects where no natural ordering exist are supposed to raise a "TypeError"
...
```json
{
    "body": "<a id='comment:6'></a>Note that to follow this convention, we have to use __richcmp__ as == and != should still work for unordered elements.",
...
<a id='comment:6'></a>Note that to follow this convention, we have to use __richcmp__ as == and != should still work for unordered elements.
...
```json
{
    "body": "<a id='comment:7'></a>Apparently related to #7160 and #10064, see [this sage-devel discussion](http://groups.google.com/group/sage-support/browse_thread/thread/28bbd04a78dadb57/01168722573ff736).",
...
<a id='comment:7'></a>Apparently related to #7160 and #10064, see [this sage-devel discussion](http://groups.google.com/group/sage-support/browse_thread/thread/28bbd04a78dadb57/01168722573ff736).
...
```json
{
    "body": "<a id='comment:8'></a>See also #9572.",
...
<a id='comment:8'></a>See also #9572.
...
```json
{
    "body": "<a id='comment:10'></a>See #7160 for a related ticket/discussion.",
...
<a id='comment:10'></a>See #7160 for a related ticket/discussion.
...
```json
{
    "body": "<a id='comment:11'></a>I think we can close this as a duplicate of those other tickets now that they are merged.",
...
<a id='comment:11'></a>I think we can close this as a duplicate of those other tickets now that they are merged.
