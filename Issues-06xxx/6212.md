# Issue 6212: make sage -upgrade check that there are no applied hg queue patches

archive/issues_006212.json:
```json
{
    "assignees": [],
    "body": "It would be nice if sage -upgrade aborted if are applied hg queue patches, in the same way it aborts if there are local changes.\n\n**CC:**  @jasongrout jthurber\n\n**Keywords:** sage upgrade applied hg queue\n\nIssue created by migration from https://trac.sagemath.org/ticket/6212\n\n",
    "closed_at": "2014-03-19T04:35:53Z",
    "created_at": "2009-06-04T20:40:01Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20distribution",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "make sage -upgrade check that there are no applied hg queue patches",
    "type": "issue",
    "updated_at": "2014-03-19T04:35:53Z",
    "url": "https://github.com/sagemath/sage/issues/6212",
    "user": "https://github.com/ncalexan"
}
```
It would be nice if sage -upgrade aborted if are applied hg queue patches, in the same way it aborts if there are local changes.

**CC:**  @jasongrout jthurber

**Keywords:** sage upgrade applied hg queue

Issue created by migration from https://trac.sagemath.org/ticket/6212





---

archive/issue_events_061719.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-06-04T20:40:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20distribution",
    "label_color": "08517b",
    "label_name": "component: distribution",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-61719"
}
```



---

archive/issue_events_061720.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-06-04T20:40:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "008080",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-61720"
}
```



---

archive/issue_events_061721.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-06-04T20:40:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "008080",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-61721"
}
```



---

archive/issue_comments_042615.json:
```json
{
    "body": "<a id='comment:1'></a>\nEven better:\n\n```\n[1:38pm] jason-: you have patches that are applied using mercurial queues.  Do you want to pop all of the patches? [y/n]\n[1:38pm] jason-: yes would pop and continue the upgrae\n[1:39pm] jason-: no would quit the upgrade\n```",
    "created_at": "2009-06-04T20:40:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6212#issuecomment-42615",
    "user": "https://github.com/ncalexan"
}
```

<a id='comment:1'></a>
Even better:

```
[1:38pm] jason-: you have patches that are applied using mercurial queues.  Do you want to pop all of the patches? [y/n]
[1:38pm] jason-: yes would pop and continue the upgrae
[1:39pm] jason-: no would quit the upgrade
```



---

archive/issue_events_061722.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-61722"
}
```



---

archive/issue_events_061723.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-61723"
}
```



---

archive/issue_events_061724.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-61724"
}
```



---

archive/issue_events_061725.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-61725"
}
```



---

archive/issue_events_061726.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-03-14T16:44:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "008080",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-61726"
}
```



---

archive/issue_comments_042616.json:
```json
{
    "body": "<a id='comment:5'></a>\nwe have switched to git, this is obsolete",
    "created_at": "2014-03-14T16:44:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6212#issuecomment-42616",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:5'></a>
we have switched to git, this is obsolete



---

archive/issue_events_061727.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-03-14T16:44:26Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-61727"
}
```



---

archive/issue_events_061728.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2014-03-15T13:33:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "008080",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-61728"
}
```



---

archive/issue_events_061729.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2014-03-15T13:33:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "008080",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-61729"
}
```



---

archive/issue_events_061730.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-03-19T04:35:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "008080",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-61730"
}
```



---

archive/issue_events_061731.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-03-19T04:35:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-61731"
}
```



---

archive/issue_events_061732.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-03-19T04:35:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6212",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "008080",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6212#event-61732"
}
```
