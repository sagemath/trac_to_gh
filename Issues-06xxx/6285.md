# Issue 6285: Wrong description of arcsin function

archive/issues_006285.json:
```json
{
    "assignees": [],
    "body": "(1) Description for \"arcsin\" wrongly says it is \"The inverse of the hyperbolic sine function\" !!\n\n```\narcsin?\n\nFile:        /home/golam/foo/sage-4.0.1/local/lib/python2.5/site-packages/sage/functions/trig.py\nType:        <class 'sage.functions.trig.Function_arcsin'>\nDefinition:  arcsin(x, hold='False')\nDocstring: \n\n        The inverse of the hyperbolic sine function.\n\n        EXAMPLES::\n\n            sage: arcsinh(0.5)\n            0.481211825059603\n            sage: arcsinh(1/2)\n            arcsinh(1/2)\n            sage: arcsinh(1 + 1.0*I)\n\n```\n\n(2) Is it also missing \" ginac='acos' \" in conversion dict?\n\nBoth \"arccos\" and \"arctan\" have that conversion defined.\n\n\nAssignee: **@burcin**\n\nKeywords: **arcsin, wrong description**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6285_\n\n",
    "closed_at": "2009-06-14T22:20:26Z",
    "created_at": "2009-06-14T11:43:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.0.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Wrong description of arcsin function",
    "type": "issue",
    "updated_at": "2009-06-14T22:20:26Z",
    "url": "https://github.com/sagemath/sage/issues/6285",
    "user": "https://github.com/golam-m-hossain"
}
```
(1) Description for "arcsin" wrongly says it is "The inverse of the hyperbolic sine function" !!

```
arcsin?

File:        /home/golam/foo/sage-4.0.1/local/lib/python2.5/site-packages/sage/functions/trig.py
Type:        <class 'sage.functions.trig.Function_arcsin'>
Definition:  arcsin(x, hold='False')
Docstring: 

        The inverse of the hyperbolic sine function.

        EXAMPLES::

            sage: arcsinh(0.5)
            0.481211825059603
            sage: arcsinh(1/2)
            arcsinh(1/2)
            sage: arcsinh(1 + 1.0*I)

```

(2) Is it also missing " ginac='acos' " in conversion dict?

Both "arccos" and "arctan" have that conversion defined.


Assignee: **@burcin**

Keywords: **arcsin, wrong description**

_Issue created by migration from https://trac.sagemath.org/ticket/6285_





---

archive/issue_events_066054.json:
```json
{
    "actor": "https://github.com/golam-m-hossain",
    "created_at": "2009-06-14T11:43:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6285",
    "milestone_number": null,
    "milestone_title": "sage-4.0.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6285#event-66054"
}
```



---

archive/issue_events_066055.json:
```json
{
    "actor": "https://github.com/golam-m-hossain",
    "created_at": "2009-06-14T11:43:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6285",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6285#event-66055"
}
```



---

archive/issue_events_066056.json:
```json
{
    "actor": "https://github.com/golam-m-hossain",
    "created_at": "2009-06-14T11:43:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6285",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6285#event-66056"
}
```



---

archive/issue_events_066057.json:
```json
{
    "actor": "https://github.com/golam-m-hossain",
    "created_at": "2009-06-14T11:43:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6285",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6285#event-66057"
}
```



---

archive/issue_comments_043231.json:
```json
{
    "body": "Changed keywords from **** to **arcsin, wrong description**",
    "created_at": "2009-06-14T12:47:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6285",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6285#issuecomment-43231",
    "user": "https://github.com/golam-m-hossain"
}
```

Changed keywords from **** to **arcsin, wrong description**



---

archive/issue_comments_043232.json:
```json
{
    "body": "Changed reviewer from **arcsin, wrong description** to none",
    "created_at": "2009-06-14T12:47:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6285",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6285#issuecomment-43232",
    "user": "https://github.com/golam-m-hossain"
}
```

Changed reviewer from **arcsin, wrong description** to none



---

archive/issue_comments_043233.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-Description for \"arcsin\" wrongly says it is \"The inverse of the hyperbolic sine function\" !!\n+(1) Description for \"arcsin\" wrongly says it is \"The inverse of the hyperbolic sine function\" !!\n \n ```\n arcsin?\n@@ -20,3 +20,7 @@\n \n ```\n \n+(2) Is it also missing \" ginac='acos' \" in conversion dict?\n+\n+Both \"arccos\" and \"arctan\" have that conversion defined.\n+\n``````\n",
    "created_at": "2009-06-14T12:47:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6285",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6285#issuecomment-43233",
    "user": "https://github.com/golam-m-hossain"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-Description for "arcsin" wrongly says it is "The inverse of the hyperbolic sine function" !!
+(1) Description for "arcsin" wrongly says it is "The inverse of the hyperbolic sine function" !!
 
 ```
 arcsin?
@@ -20,3 +20,7 @@
 
 ```
 
+(2) Is it also missing " ginac='acos' " in conversion dict?
+
+Both "arccos" and "arctan" have that conversion defined.
+
``````




---

archive/issue_comments_043234.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nI fixed these as a part of #6244. This bug can be closed once #6244 is merged.",
    "created_at": "2009-06-14T13:02:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6285",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6285#issuecomment-43234",
    "user": "https://github.com/burcin"
}
```

<a id='comment:2'>Comment 2:</a>
I fixed these as a part of #6244. This bug can be closed once #6244 is merged.



---

archive/issue_comments_043235.json:
```json
{
    "body": "Assignee: **@burcin**",
    "created_at": "2009-06-14T13:02:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6285",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6285#issuecomment-43235",
    "user": "https://github.com/burcin"
}
```

Assignee: **@burcin**



---

archive/issue_events_066058.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-06-14T13:02:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6285",
    "milestone_number": null,
    "milestone_title": "sage-4.0.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6285#event-66058"
}
```



---

archive/issue_events_066059.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-06-14T22:20:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6285",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6285#event-66059"
}
```
