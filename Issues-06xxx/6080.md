# Issue 6080: Add utility methods to Integer (needed for mpmath)

archive/issues_006080.json:
```json
{
    "assignees": [],
    "body": "Patch adds sqrtrem (I only found an existing isqrt) and a method to count trailing zero bits, both of which are needed to make mpmath on top of sage.Integer reasonably fast.\n\nThe value of (0).trailing_zero_bits() (as well as the name of the method) could be adjusted.\n\nComponent: **basic arithmetic**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6080_\n\n",
    "closed_at": "2009-05-19T20:37:01Z",
    "created_at": "2009-05-19T03:35:31Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add utility methods to Integer (needed for mpmath)",
    "type": "issue",
    "updated_at": "2009-05-19T20:37:01Z",
    "url": "https://github.com/sagemath/sage/issues/6080",
    "user": "https://github.com/fredrik-johansson"
}
```
Patch adds sqrtrem (I only found an existing isqrt) and a method to count trailing zero bits, both of which are needed to make mpmath on top of sage.Integer reasonably fast.

The value of (0).trailing_zero_bits() (as well as the name of the method) could be adjusted.

Component: **basic arithmetic**

_Issue created by migration from https://trac.sagemath.org/ticket/6080_





---

archive/issue_events_072918.json:
```json
{
    "actor": "https://github.com/fredrik-johansson",
    "created_at": "2009-05-19T03:35:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6080",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "c: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6080#event-72918"
}
```



---

archive/issue_events_072919.json:
```json
{
    "actor": "https://github.com/fredrik-johansson",
    "created_at": "2009-05-19T03:35:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6080",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6080#event-72919"
}
```



---

archive/issue_events_072920.json:
```json
{
    "actor": "https://github.com/fredrik-johansson",
    "created_at": "2009-05-19T03:35:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6080",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6080#event-72920"
}
```



---

archive/issue_comments_039867.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[intutils.patch.gz](https://github.com/sagemath/sage/files/ticket6080/intutils.patch.gz)**",
    "created_at": "2009-05-19T03:56:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6080",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6080#issuecomment-39867",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[intutils.patch.gz](https://github.com/sagemath/sage/files/ticket6080/intutils.patch.gz)**



---

archive/issue_events_072921.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-05-19T03:56:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6080",
    "milestone_number": null,
    "milestone_title": "sage-4.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6080#event-72921"
}
```



---

archive/issue_events_072922.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-05-19T03:56:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6080",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6080#event-72922"
}
```



---

archive/issue_events_072923.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-19T04:33:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6080",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6080#event-72923"
}
```



---

archive/issue_events_072924.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-19T04:33:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6080",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6080#event-72924"
}
```



---

archive/issue_comments_039868.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nLooks good to me.",
    "created_at": "2009-05-19T04:33:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6080",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6080#issuecomment-39868",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:2" align="right">comment:2</div>

Looks good to me.



---

archive/issue_events_072925.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-19T20:37:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6080",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6080#event-72925"
}
```



---

archive/issue_events_072926.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-19T20:37:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6080",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6080#event-72926"
}
```



---

archive/issue_events_072927.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-19T20:37:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6080",
    "milestone_number": null,
    "milestone_title": "sage-4.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6080#event-72927"
}
```



---

archive/issue_events_072928.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-19T20:37:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6080",
    "milestone_number": null,
    "milestone_title": "sage-4.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6080#event-72928"
}
```



---

archive/issue_comments_039869.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in Sage 4.0.rc0.\n\nCheers,\n\nMichael",
    "created_at": "2009-05-19T20:37:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6080",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6080#issuecomment-39869",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in Sage 4.0.rc0.

Cheers,

Michael
