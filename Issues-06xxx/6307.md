# Issue 6307: Move javascript out of python-land

archive/issues_006307.json:
```json
{
    "assignees": [
        "https://github.com/boothby"
    ],
    "body": "As Mike Hansen noted in #5564, the javascript shouldn't be in a python file in triple-quoted strings.  Unfortunately, the patch he submitted to do this bitrotted. \n\nComponent: **notebook**\n\nAuthor: **Mike Hansen, Tom Boothby, Robert Miller**\n\nReviewer: **Tom Boothby, Minh Van Nguyen, John Palmieri**\n\nMerged: **sage-4.1.1.alpha1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6307_\n\n",
    "closed_at": "2009-07-23T09:16:17Z",
    "created_at": "2009-06-16T05:42:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Move javascript out of python-land",
    "type": "issue",
    "updated_at": "2009-07-23T09:16:17Z",
    "url": "https://github.com/sagemath/sage/issues/6307",
    "user": "https://github.com/boothby"
}
```
As Mike Hansen noted in #5564, the javascript shouldn't be in a python file in triple-quoted strings.  Unfortunately, the patch he submitted to do this bitrotted. 

Component: **notebook**

Author: **Mike Hansen, Tom Boothby, Robert Miller**

Reviewer: **Tom Boothby, Minh Van Nguyen, John Palmieri**

Merged: **sage-4.1.1.alpha1**

_Issue created by migration from https://trac.sagemath.org/ticket/6307_





---

archive/issue_events_075650.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-06-16T05:42:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "milestone_number": null,
    "milestone_title": "sage-4.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6307#event-75650"
}
```



---

archive/issue_events_075651.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-06-16T05:42:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "c: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6307#event-75651"
}
```



---

archive/issue_events_075652.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-06-16T05:42:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6307#event-75652"
}
```



---

archive/issue_events_075653.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-06-16T05:42:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6307#event-75653"
}
```



---

archive/issue_events_075654.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-06-16T05:46:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6307#event-75654"
}
```



---

archive/issue_events_075655.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-06-16T05:47:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "c: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6307#event-75655"
}
```



---

archive/issue_events_075656.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-06-16T05:47:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6307#event-75656"
}
```



---

archive/issue_events_075657.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-06-16T05:47:03Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "subject": "https://github.com/boothby",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6307#event-75657"
}
```



---

archive/issue_events_075658.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-06-16T22:18:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6307#event-75658"
}
```



---

archive/issue_events_075659.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-06-16T22:18:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6307#event-75659"
}
```



---

archive/issue_comments_041968.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nIn #5564, mhansen mentioned that this patch breaks history; if anyone really cares about preserving history, it's easy enough with `hg cp`. I made a version of this patch which does exactly the same thing with respect to file contents, but also tells Mercurial about the history: http://sage.math.washington.edu/home/drake/6307bis.patch\n\n(It's rather bigger than the patch on this ticket because it's diffing the new .js files against js.py, instead of /dev/null.)",
    "created_at": "2009-06-18T00:09:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6307#issuecomment-41968",
    "user": "https://github.com/dandrake"
}
```

<div id="comment:4" align="right">comment:4</div>

In #5564, mhansen mentioned that this patch breaks history; if anyone really cares about preserving history, it's easy enough with `hg cp`. I made a version of this patch which does exactly the same thing with respect to file contents, but also tells Mercurial about the history: http://sage.math.washington.edu/home/drake/6307bis.patch

(It's rather bigger than the patch on this ticket because it's diffing the new .js files against js.py, instead of /dev/null.)



---

archive/issue_comments_041969.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nAttachment: **[6307bis.patch.gz](https://github.com/sagemath/sage/files/ticket6307/6307bis.patch.gz)**\n\nVery nice, I didn't know that functionality existed.",
    "created_at": "2009-06-18T02:33:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6307#issuecomment-41969",
    "user": "https://github.com/boothby"
}
```

<div id="comment:5" align="right">comment:5</div>

Attachment: **[6307bis.patch.gz](https://github.com/sagemath/sage/files/ticket6307/6307bis.patch.gz)**

Very nice, I didn't know that functionality existed.



---

archive/issue_events_075660.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-06-20T18:42:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6307#event-75660"
}
```



---

archive/issue_events_075661.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-06-20T18:42:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6307#event-75661"
}
```



---

archive/issue_comments_041970.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\ndoctest failure in psage.py",
    "created_at": "2009-06-20T18:42:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6307#issuecomment-41970",
    "user": "https://github.com/boothby"
}
```

<div id="comment:6" align="right">comment:6</div>

doctest failure in psage.py



---

archive/issue_events_075662.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-09T18:36:14Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "milestone_number": null,
    "milestone_title": "sage-4.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6307#event-75662"
}
```



---

archive/issue_events_075663.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-09T18:36:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "milestone_number": null,
    "milestone_title": "sage-4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6307#event-75663"
}
```



---

archive/issue_events_075664.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-09T18:53:38Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "milestone_number": null,
    "milestone_title": "sage-4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6307#event-75664"
}
```



---

archive/issue_events_075665.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-09T18:53:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "milestone_number": null,
    "milestone_title": "sage-4.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6307#event-75665"
}
```



---

archive/issue_comments_041971.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nI can't reproduce the error in psage.py... I'm re-marking this as a positive review.",
    "created_at": "2009-07-09T19:18:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6307#issuecomment-41971",
    "user": "https://github.com/boothby"
}
```

<div id="comment:9" align="right">comment:9</div>

I can't reproduce the error in psage.py... I'm re-marking this as a positive review.



---

archive/issue_events_075666.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-07-09T19:18:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6307#event-75666"
}
```



---

archive/issue_events_075667.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-07-09T19:18:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6307#event-75667"
}
```



---

archive/issue_comments_041972.json:
```json
{
    "body": "reviewer patch; fixes typos",
    "created_at": "2009-07-16T11:25:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6307#issuecomment-41972",
    "user": "https://github.com/sagetrac-mvngu"
}
```

reviewer patch; fixes typos



---

archive/issue_comments_041973.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nAttachment: **[trac_6307-reviewer.patch.gz](https://github.com/sagemath/sage/files/ticket6307/trac_6307-reviewer.patch.gz)**",
    "created_at": "2009-07-16T11:32:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6307#issuecomment-41973",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:10" align="right">comment:10</div>

Attachment: **[trac_6307-reviewer.patch.gz](https://github.com/sagemath/sage/files/ticket6307/trac_6307-reviewer.patch.gz)**



---

archive/issue_comments_041974.json:
```json
{
    "body": "Changed author from **boothby, mhansen** to **Mike Hansen**",
    "created_at": "2009-07-16T11:32:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6307#issuecomment-41974",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Changed author from **boothby, mhansen** to **Mike Hansen**



---

archive/issue_comments_041975.json:
```json
{
    "body": "Reviewer: **Tom Boothby, Minh Van Nguyen**",
    "created_at": "2009-07-16T11:32:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6307#issuecomment-41975",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Reviewer: **Tom Boothby, Minh Van Nguyen**



---

archive/issue_comments_041976.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">comment:11</div>\n\nOnce this ticket is closed, ticket #5564 should also be closed as a consequence of the patches on this ticket. Just to let people know, this has been merged in sage-4.1.1-alpha0. I can't close this ticket because I don't have the privilege to do so. Sorry, folks :-(",
    "created_at": "2009-07-16T11:53:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6307#issuecomment-41976",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:11" align="right">comment:11</div>

Once this ticket is closed, ticket #5564 should also be closed as a consequence of the patches on this ticket. Just to let people know, this has been merged in sage-4.1.1-alpha0. I can't close this ticket because I don't have the privilege to do so. Sorry, folks :-(



---

archive/issue_events_075668.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-16T21:22:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6307#event-75668"
}
```



---

archive/issue_events_075669.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-16T21:22:24Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6307#event-75669"
}
```



---

archive/issue_comments_041977.json:
```json
{
    "body": "Merged: **sage-4.1.1.alpha0**",
    "created_at": "2009-07-16T21:22:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6307#issuecomment-41977",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **sage-4.1.1.alpha0**



---

archive/issue_comments_041978.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">comment:13</div>\n\nThis ticket results in a corrupt repository. After merging this ticket in Sage 4.1.1.alpha0, I created a source distribution with `sage -sdist 4.1.1.alpha0`. Now compile that source distribution, then cd to `SAGE_ROOT/devel/sage-main` and do:\n\n```\n[mvngu@sage sage-main]$ hg st\n! sage/server/notebook/templates/async_lib.js\n! sage/server/notebook/templates/jmol_lib.js\n! sage/server/notebook/templates/notebook_lib.js\n```\nI'm marking this ticket as \"needs work\" and reverting it in my merge tree.",
    "created_at": "2009-07-22T17:16:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6307#issuecomment-41978",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:13" align="right">comment:13</div>

This ticket results in a corrupt repository. After merging this ticket in Sage 4.1.1.alpha0, I created a source distribution with `sage -sdist 4.1.1.alpha0`. Now compile that source distribution, then cd to `SAGE_ROOT/devel/sage-main` and do:

```
[mvngu@sage sage-main]$ hg st
! sage/server/notebook/templates/async_lib.js
! sage/server/notebook/templates/jmol_lib.js
! sage/server/notebook/templates/notebook_lib.js
```
I'm marking this ticket as "needs work" and reverting it in my merge tree.



---

archive/issue_events_075670.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-22T17:16:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6307#event-75670"
}
```



---

archive/issue_events_075671.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-22T17:16:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6307#event-75671"
}
```



---

archive/issue_comments_041979.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">comment:15</div>\n\nWorking on a fix for this right now...",
    "created_at": "2009-07-22T17:24:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6307#issuecomment-41979",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:15" align="right">comment:15</div>

Working on a fix for this right now...



---

archive/issue_comments_041980.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">comment:16</div>\n\nRather, I have a fix, but the sdist command is taking a long time. Once that is done I can confirm that it worked.",
    "created_at": "2009-07-22T17:41:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6307#issuecomment-41980",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:16" align="right">comment:16</div>

Rather, I have a fix, but the sdist command is taking a long time. Once that is done I can confirm that it worked.



---

archive/issue_events_075672.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-22T18:08:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6307#event-75672"
}
```



---

archive/issue_comments_041981.json:
```json
{
    "body": "<div id=\"comment:17\" align=\"right\">comment:17</div>\n\nAttachment: **[trac_6307-manifest.in.patch.gz](https://github.com/sagemath/sage/files/ticket6307/trac_6307-manifest.in.patch.gz)**\n\nIt works!\n\n```\n[rlm-book templates]$ pwd\n/Users/rlmill/sage-4.1.1.alpha0.6307/dist/sage-4.1.1.alpha0.fix.test/spkg/standard/sage-4.1.1.alpha0.fix.test/sage/server/notebook/templates\n[rlm-book templates]$ ls\ntotal 208K\n-rw-r--r-- 1 rlmill  786 2009-07-22 10:23 account_recovery.html\n-rw-r--r-- 1 rlmill 1.5K 2009-07-22 10:23 account_settings.html\n-rw-r--r-- 1 rlmill  729 2009-07-22 10:23 async_lib.js\n-rw-r--r-- 1 rlmill  448 2009-07-22 10:23 banner.html\n-rw-r--r-- 1 rlmill  467 2009-07-22 10:23 base.html\n-rw-r--r-- 1 rlmill  440 2009-07-22 10:23 base_authenticated.html\n-rw-r--r-- 1 rlmill 2.8K 2009-07-22 10:23 docs.html\n-rw-r--r-- 1 rlmill  324 2009-07-22 10:23 error_message.html\n-rw-r--r-- 1 rlmill  534 2009-07-22 10:23 history.html\n-rw-r--r-- 1 rlmill 1.2K 2009-07-22 10:23 jmol_lib.js\n-rw-r--r-- 1 rlmill  385 2009-07-22 10:23 list_top.html\n-rw-r--r-- 1 rlmill 2.8K 2009-07-22 10:23 login.html\n-rw-r--r-- 1 rlmill 117K 2009-07-22 10:23 notebook_lib.js\n-rw-r--r-- 1 rlmill 2.3K 2009-07-22 10:23 registration.html\n-rw-r--r-- 1 rlmill  284 2009-07-22 10:23 search.html\n-rw-r--r-- 1 rlmill  780 2009-07-22 10:23 source_code.html\n-rw-r--r-- 1 rlmill  220 2009-07-22 10:23 template_error.html\n-rw-r--r-- 1 rlmill 1.3K 2009-07-22 10:23 top_bar.html\n-rw-r--r-- 1 rlmill 1.2K 2009-07-22 10:23 upload.html\n-rw-r--r-- 1 rlmill  324 2009-07-22 10:23 user_management.html\n-rw-r--r-- 1 rlmill 6.7K 2009-07-22 10:23 worksheet_listing.html\n-rw-r--r-- 1 rlmill  280 2009-07-22 10:23 yes_no.html\n```",
    "created_at": "2009-07-22T18:08:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6307#issuecomment-41981",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:17" align="right">comment:17</div>

Attachment: **[trac_6307-manifest.in.patch.gz](https://github.com/sagemath/sage/files/ticket6307/trac_6307-manifest.in.patch.gz)**

It works!

```
[rlm-book templates]$ pwd
/Users/rlmill/sage-4.1.1.alpha0.6307/dist/sage-4.1.1.alpha0.fix.test/spkg/standard/sage-4.1.1.alpha0.fix.test/sage/server/notebook/templates
[rlm-book templates]$ ls
total 208K
-rw-r--r-- 1 rlmill  786 2009-07-22 10:23 account_recovery.html
-rw-r--r-- 1 rlmill 1.5K 2009-07-22 10:23 account_settings.html
-rw-r--r-- 1 rlmill  729 2009-07-22 10:23 async_lib.js
-rw-r--r-- 1 rlmill  448 2009-07-22 10:23 banner.html
-rw-r--r-- 1 rlmill  467 2009-07-22 10:23 base.html
-rw-r--r-- 1 rlmill  440 2009-07-22 10:23 base_authenticated.html
-rw-r--r-- 1 rlmill 2.8K 2009-07-22 10:23 docs.html
-rw-r--r-- 1 rlmill  324 2009-07-22 10:23 error_message.html
-rw-r--r-- 1 rlmill  534 2009-07-22 10:23 history.html
-rw-r--r-- 1 rlmill 1.2K 2009-07-22 10:23 jmol_lib.js
-rw-r--r-- 1 rlmill  385 2009-07-22 10:23 list_top.html
-rw-r--r-- 1 rlmill 2.8K 2009-07-22 10:23 login.html
-rw-r--r-- 1 rlmill 117K 2009-07-22 10:23 notebook_lib.js
-rw-r--r-- 1 rlmill 2.3K 2009-07-22 10:23 registration.html
-rw-r--r-- 1 rlmill  284 2009-07-22 10:23 search.html
-rw-r--r-- 1 rlmill  780 2009-07-22 10:23 source_code.html
-rw-r--r-- 1 rlmill  220 2009-07-22 10:23 template_error.html
-rw-r--r-- 1 rlmill 1.3K 2009-07-22 10:23 top_bar.html
-rw-r--r-- 1 rlmill 1.2K 2009-07-22 10:23 upload.html
-rw-r--r-- 1 rlmill  324 2009-07-22 10:23 user_management.html
-rw-r--r-- 1 rlmill 6.7K 2009-07-22 10:23 worksheet_listing.html
-rw-r--r-- 1 rlmill  280 2009-07-22 10:23 yes_no.html
```



---

archive/issue_comments_041982.json:
```json
{
    "body": "Changed reviewer from **Tom Boothby, Minh Van Nguyen** to **Tom Boothby, Minh Van Nguyen, John Palmieri**",
    "created_at": "2009-07-22T18:52:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6307#issuecomment-41982",
    "user": "https://github.com/jhpalmieri"
}
```

Changed reviewer from **Tom Boothby, Minh Van Nguyen** to **Tom Boothby, Minh Van Nguyen, John Palmieri**



---

archive/issue_comments_041983.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">comment:18</div>\n\nThe new patch should fix the repository issues.",
    "created_at": "2009-07-22T18:52:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6307#issuecomment-41983",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:18" align="right">comment:18</div>

The new patch should fix the repository issues.



---

archive/issue_comments_041984.json:
```json
{
    "body": "Changed author from **Mike Hansen** to **Mike Hansen, Tom Boothby, Robert Miller**",
    "created_at": "2009-07-22T18:52:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6307#issuecomment-41984",
    "user": "https://github.com/jhpalmieri"
}
```

Changed author from **Mike Hansen** to **Mike Hansen, Tom Boothby, Robert Miller**



---

archive/issue_events_075673.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-07-22T18:52:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6307#event-75673"
}
```



---

archive/issue_events_075674.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-07-22T18:52:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6307#event-75674"
}
```



---

archive/issue_comments_041985.json:
```json
{
    "body": "Changed merged from **sage-4.1.1.alpha0** to **sage-4.1.1.alpha1**",
    "created_at": "2009-07-23T09:16:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6307#issuecomment-41985",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Changed merged from **sage-4.1.1.alpha0** to **sage-4.1.1.alpha1**



---

archive/issue_events_075675.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-23T09:16:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6307#event-75675"
}
```



---

archive/issue_events_075676.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-23T09:16:17Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6307",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6307#event-75676"
}
```
