# Issue 6819: multinomial to accept lists as argument too

archive/issues_006819.json:
```json
{
    "body": "I have modified multinomial to accept lists as argument too. It makes programming with it much easier\n\n**Keywords:** arithmetic\n\n**Author:** Rishikesh\n\n**Reviewer:** Nathann Cohen\n\n**Merged:** Sage 4.1.2.alpha0\n\nIssue created by migration from https://trac.sagemath.org/ticket/6819\n\n",
    "closed_at": "2009-08-30T08:42:46Z",
    "created_at": "2009-08-24T18:23:05Z",
    "labels": [
        "component: algebra",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1.2",
    "title": "multinomial to accept lists as argument too",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/6819",
    "user": "https://github.com/rishikesha"
}
```
I have modified multinomial to accept lists as argument too. It makes programming with it much easier

**Keywords:** arithmetic

**Author:** Rishikesh

**Reviewer:** Nathann Cohen

**Merged:** Sage 4.1.2.alpha0

Issue created by migration from https://trac.sagemath.org/ticket/6819





---

archive/attachments_008162.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "12846.patch",
    "asset_url": "tarball://root/attachments/ticket6819/12846.patch",
    "created_at": "2009-08-24T18:32:29Z",
    "issue": "https://github.com/sagemath/sage/issues/6819",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket6819/12846.patch",
    "user": "https://github.com/rishikesha"
}
```



---

archive/issue_comments_049783.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [12846.patch](https://github.com/sagemath/sage/files/ticket6819/12846.patch) by @rishikesha created at 2009-08-24 18:32:29",
    "created_at": "2009-08-24T18:32:29Z",
    "issue": "https://github.com/sagemath/sage/issues/6819",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6819#issuecomment-49783",
    "user": "https://github.com/rishikesha"
}
```

<a id='comment:1'></a>
Attachment [12846.patch](https://github.com/sagemath/sage/files/ticket6819/12846.patch) by @rishikesha created at 2009-08-24 18:32:29



---

archive/issue_events_045288.json:
```json
{
    "actor": "https://github.com/rishikesha",
    "created_at": "2009-08-24T18:32:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6819",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6819#event-45288"
}
```



---

archive/issue_events_045289.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-08-25T07:26:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6819",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6819#event-45289"
}
```



---

archive/issue_events_045290.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2009-08-25T07:26:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6819",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6819#event-45290"
}
```



---

archive/issue_comments_049784.json:
```json
{
    "body": "<a id='comment:2'></a>\nSeems ok to me ! I Applied it, used it, and did not understand why it was not possible already !\n\nBy the way, I added some docstrings to the function, so if you think it is ok.. ;-)",
    "created_at": "2009-08-25T07:26:39Z",
    "issue": "https://github.com/sagemath/sage/issues/6819",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6819#issuecomment-49784",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:2'></a>
Seems ok to me ! I Applied it, used it, and did not understand why it was not possible already !

By the way, I added some docstrings to the function, so if you think it is ok.. ;-)



---

archive/attachments_008163.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "multinomial_list.patch",
    "asset_url": "tarball://root/attachments/ticket6819/multinomial_list.patch",
    "created_at": "2009-08-25T07:27:04Z",
    "issue": "https://github.com/sagemath/sage/issues/6819",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket6819/multinomial_list.patch",
    "user": "https://github.com/nathanncohen"
}
```



---

archive/issue_comments_049785.json:
```json
{
    "body": "Attachment [multinomial_list.patch](https://github.com/sagemath/sage/files/ticket6819/multinomial_list.patch) by @nathanncohen created at 2009-08-25 07:27:04\n\npatch reviewed + some docstrings",
    "created_at": "2009-08-25T07:27:04Z",
    "issue": "https://github.com/sagemath/sage/issues/6819",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6819#issuecomment-49785",
    "user": "https://github.com/nathanncohen"
}
```

Attachment [multinomial_list.patch](https://github.com/sagemath/sage/files/ticket6819/multinomial_list.patch) by @nathanncohen created at 2009-08-25 07:27:04

patch reviewed + some docstrings



---

archive/issue_comments_049786.json:
```json
{
    "body": "<a id='comment:3'></a>\nReplying to [ncohen](#comment%3A2):\n> Seems ok to me ! I Applied it, used it, and did not understand why it was not possible already !\n> \n> By the way, I added some docstrings to the function, so if you think it is ok.. ;-)\n\n\nThanks for the docstrings.",
    "created_at": "2009-08-25T13:59:22Z",
    "issue": "https://github.com/sagemath/sage/issues/6819",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6819#issuecomment-49786",
    "user": "https://github.com/rishikesha"
}
```

<a id='comment:3'></a>
Replying to [ncohen](#comment%3A2):
> Seems ok to me ! I Applied it, used it, and did not understand why it was not possible already !
> 
> By the way, I added some docstrings to the function, so if you think it is ok.. ;-)


Thanks for the docstrings.



---

archive/attachments_008164.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_6819-reviewer.patch",
    "asset_url": "tarball://root/attachments/ticket6819/trac_6819-reviewer.patch",
    "created_at": "2009-08-30T07:51:19Z",
    "issue": "https://github.com/sagemath/sage/issues/6819",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket6819/trac_6819-reviewer.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```



---

archive/issue_comments_049787.json:
```json
{
    "body": "Attachment [trac_6819-reviewer.patch](https://github.com/sagemath/sage/files/ticket6819/trac_6819-reviewer.patch) by mvngu created at 2009-08-30 07:51:19\n\nncohen's reviewer patch",
    "created_at": "2009-08-30T07:51:19Z",
    "issue": "https://github.com/sagemath/sage/issues/6819",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6819#issuecomment-49787",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Attachment [trac_6819-reviewer.patch](https://github.com/sagemath/sage/files/ticket6819/trac_6819-reviewer.patch) by mvngu created at 2009-08-30 07:51:19

ncohen's reviewer patch



---

archive/issue_comments_049788.json:
```json
{
    "body": "**Merged:** Sage 4.1.2.alpha0",
    "created_at": "2009-08-30T08:42:46Z",
    "issue": "https://github.com/sagemath/sage/issues/6819",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6819#issuecomment-49788",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Merged:** Sage 4.1.2.alpha0



---

archive/issue_events_045291.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-08-30T08:42:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6819",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6819#event-45291"
}
```



---

archive/issue_events_045292.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-08-30T08:42:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6819",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6819#event-45292"
}
```



---

archive/issue_comments_049789.json:
```json
{
    "body": "**Reviewer:** Nathann Cohen",
    "created_at": "2009-08-30T08:42:46Z",
    "issue": "https://github.com/sagemath/sage/issues/6819",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6819#issuecomment-49789",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Reviewer:** Nathann Cohen



---

archive/issue_comments_049790.json:
```json
{
    "body": "**Author:** Rishikesh",
    "created_at": "2009-08-30T08:42:46Z",
    "issue": "https://github.com/sagemath/sage/issues/6819",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6819#issuecomment-49790",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Author:** Rishikesh



---

archive/issue_comments_049791.json:
```json
{
    "body": "<a id='comment:4'></a>\nThe patch `multinomial_list.patch` contains some badly formatted docstrings. But those shouldn't prevent the patch from being merged. Better to fix those formatting issues in a separate ticket. See #6845 for a follow up to fix this docstring issue.\n\n\n\nncohen -- Your username should be in your patches; it makes it easier to credit you for your contributions. Please also remember to put in a sensible commit message for your patches.\n\n\n\nWhile merging and testing these patches:\n\n1. `12846.patch` -- rishi's contribution\n2. `trac_6819-reviewer.patch` -- ncohen's contribution\n \nI ran into a doctest failure:\n\n```\nsage -t -long devel/sage-main/sage/misc/getusage.py\n**********************************************************************\nFile \"/scratch/mvngu/release/sage-4.1.1/devel/sage-main/sage/misc/getusage.py\", line 69:\n    sage: get_memory_usage(t)          # amount of memory more than when we defined t.\nExpected:\n    0.0\nGot:\n    0.34375\n**********************************************************************\n1 items had failures:\n   1 of   4 in __main__.example_2\n***Test Failed*** 1 failures.\nFor whitespace errors, see the file /scratch/mvngu/release/sage-4.1.1/tmp/.doctest_getusage.py\n\t [2.6 s]\n\n```\nThis has nothing to do with the above patches. Strangely, it crops up when I run the test on sage.math. But the test passes on mod.math and geom.math. Merged patches in this order:\n\n1. `12846.patch`\n2. `trac_6819-reviewer.patch`",
    "created_at": "2009-08-30T08:42:46Z",
    "issue": "https://github.com/sagemath/sage/issues/6819",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6819#issuecomment-49791",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:4'></a>
The patch `multinomial_list.patch` contains some badly formatted docstrings. But those shouldn't prevent the patch from being merged. Better to fix those formatting issues in a separate ticket. See #6845 for a follow up to fix this docstring issue.



ncohen -- Your username should be in your patches; it makes it easier to credit you for your contributions. Please also remember to put in a sensible commit message for your patches.



While merging and testing these patches:

1. `12846.patch` -- rishi's contribution
2. `trac_6819-reviewer.patch` -- ncohen's contribution
 
I ran into a doctest failure:

```
sage -t -long devel/sage-main/sage/misc/getusage.py
**********************************************************************
File "/scratch/mvngu/release/sage-4.1.1/devel/sage-main/sage/misc/getusage.py", line 69:
    sage: get_memory_usage(t)          # amount of memory more than when we defined t.
Expected:
    0.0
Got:
    0.34375
**********************************************************************
1 items had failures:
   1 of   4 in __main__.example_2
***Test Failed*** 1 failures.
For whitespace errors, see the file /scratch/mvngu/release/sage-4.1.1/tmp/.doctest_getusage.py
	 [2.6 s]

```
This has nothing to do with the above patches. Strangely, it crops up when I run the test on sage.math. But the test passes on mod.math and geom.math. Merged patches in this order:

1. `12846.patch`
2. `trac_6819-reviewer.patch`



---

archive/issue_events_045293.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-08-30T08:42:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6819",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6819#event-45293"
}
```
