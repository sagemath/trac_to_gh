# Issue 6129: Ammend docstring in ode.pyx

archive/issues_006129.json:
```json
{
    "body": "\n```\n# HG changeset patch\n# User Anthony David <adavid@adavid.com.au>\n# Date 1243305447 -36000\n# Node ID 894f488ddccd3411fdd0736b455f27e2d8272099\n# Parent  958178a11b9e809788f1eda0cc29107c456a1bbe\nammend EXAMPLE comment in sage/gsl/ode.pyx to match doctest function g_1\n\ndiff -r 958178a11b9e -r 894f488ddccd sage/gsl/ode.pyx\n--- a/sage/gsl/ode.pyx Mon May 25 00:46:38 2009 +1000\n+++ b/sage/gsl/ode.pyx Tue May 26 12:37:27 2009 +1000\n@@ -213,7 +213,7 @@\n \n          Lets try a system\n \n-         y_0'=y_2*y_3\n+         y_0'=y_1*y_2\n          y_1'=-y_0*y_2\n          y_2'=-.51*y_0*y_1\n```\n\n**Keywords:** ode docstring\n\nIssue created by migration from https://trac.sagemath.org/ticket/6129\n\n",
    "closed_at": "2009-05-28T07:31:58Z",
    "created_at": "2009-05-26T02:15:06Z",
    "labels": [
        "component: documentation",
        "trivial",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.0",
    "title": "Ammend docstring in ode.pyx",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/6129",
    "user": "https://trac.sagemath.org/admin/accounts/users/adavid"
}
```

```
# HG changeset patch
# User Anthony David <adavid@adavid.com.au>
# Date 1243305447 -36000
# Node ID 894f488ddccd3411fdd0736b455f27e2d8272099
# Parent  958178a11b9e809788f1eda0cc29107c456a1bbe
ammend EXAMPLE comment in sage/gsl/ode.pyx to match doctest function g_1

diff -r 958178a11b9e -r 894f488ddccd sage/gsl/ode.pyx
--- a/sage/gsl/ode.pyx Mon May 25 00:46:38 2009 +1000
+++ b/sage/gsl/ode.pyx Tue May 26 12:37:27 2009 +1000
@@ -213,7 +213,7 @@
 
          Lets try a system
 
-         y_0'=y_2*y_3
+         y_0'=y_1*y_2
          y_1'=-y_0*y_2
          y_2'=-.51*y_0*y_1
```

**Keywords:** ode docstring

Issue created by migration from https://trac.sagemath.org/ticket/6129





---

archive/issue_comments_041855.json:
```json
{
    "body": "<a id='comment:1'></a>\nLooks good to me.",
    "created_at": "2009-05-28T07:22:01Z",
    "issue": "https://github.com/sagemath/sage/issues/6129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6129#issuecomment-41855",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'></a>
Looks good to me.



---

archive/issue_events_039793.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-28T07:22:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6129",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6129#event-39793"
}
```



---

archive/attachments_007218.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_6129.patch",
    "asset_url": "tarball://root/attachments/ticket6129/trac_6129.patch",
    "created_at": "2009-05-28T07:31:30Z",
    "issue": "https://github.com/sagemath/sage/issues/6129",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket6129/trac_6129.patch",
    "user": "https://github.com/mwhansen"
}
```



---

archive/issue_comments_041856.json:
```json
{
    "body": "Attachment [trac_6129.patch](https://github.com/sagemath/sage/files/ticket6129/trac_6129.patch) by @mwhansen created at 2009-05-28 07:31:30",
    "created_at": "2009-05-28T07:31:30Z",
    "issue": "https://github.com/sagemath/sage/issues/6129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6129#issuecomment-41856",
    "user": "https://github.com/mwhansen"
}
```

Attachment [trac_6129.patch](https://github.com/sagemath/sage/files/ticket6129/trac_6129.patch) by @mwhansen created at 2009-05-28 07:31:30



---

archive/issue_events_039794.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-28T07:31:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6129",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6129#event-39794"
}
```



---

archive/issue_events_039795.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-28T07:31:58Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6129",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6129#event-39795"
}
```



---

archive/issue_comments_041857.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged trac_6129.patch in 4.0.rc1.",
    "created_at": "2009-05-28T07:31:58Z",
    "issue": "https://github.com/sagemath/sage/issues/6129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6129#issuecomment-41857",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:2'></a>
Merged trac_6129.patch in 4.0.rc1.



---

archive/issue_comments_041858.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -8,8 +8,8 @@\n ammend EXAMPLE comment in sage/gsl/ode.pyx to match doctest function g_1\n \n diff -r 958178a11b9e -r 894f488ddccd sage/gsl/ode.pyx\n---- a/sage/gsl/ode.pyx\tMon May 25 00:46:38 2009 +1000\n-+++ b/sage/gsl/ode.pyx\tTue May 26 12:37:27 2009 +1000\n+--- a/sage/gsl/ode.pyx Mon May 25 00:46:38 2009 +1000\n++++ b/sage/gsl/ode.pyx Tue May 26 12:37:27 2009 +1000\n @@ -213,7 +213,7 @@\n  \n           Lets try a system\n``````\n",
    "created_at": "2015-09-06T16:37:25Z",
    "issue": "https://github.com/sagemath/sage/issues/6129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6129#issuecomment-41858",
    "user": "https://github.com/fchapoton"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -8,8 +8,8 @@
 ammend EXAMPLE comment in sage/gsl/ode.pyx to match doctest function g_1
 
 diff -r 958178a11b9e -r 894f488ddccd sage/gsl/ode.pyx
---- a/sage/gsl/ode.pyx	Mon May 25 00:46:38 2009 +1000
-+++ b/sage/gsl/ode.pyx	Tue May 26 12:37:27 2009 +1000
+--- a/sage/gsl/ode.pyx Mon May 25 00:46:38 2009 +1000
++++ b/sage/gsl/ode.pyx Tue May 26 12:37:27 2009 +1000
 @@ -213,7 +213,7 @@
  
           Lets try a system
``````

