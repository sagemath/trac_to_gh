# Issue 6473: ideal.interreduced_basis hangs forever

archive/issues_006473.json:
```json
{
    "body": "This completes in a fraction of a second:\n\n```\nsage: R.<a,b,c,d>=PolynomialRing(QQ,order=\"lex\")\nsage: ideal(a^16,a-b^16,b-c^16,c-d^15).interreduced_basis()\n[d^61440, c - d^15, b - d^240, a - d^3840]\n```\n\nI stopped the following after more than an hour, which leads me to believe that Sage is stuck in an infinite loop:\n\n```\nsage: R.<a,b,c,d>=PolynomialRing(QQ,order=\"lex\")\nsage: ideal(a^16,a-b^16,b-c^16,c-d^16).interreduced_basis()\n```\n\nThe only difference between the two is that the power of d in the input binomial involving c as the initial term is increased from 15 to 16. This difference has no mathematical significance and should have no impact on the computation time.\n\nI suspect that the reason is that by default Sage uses Singular to implement interreduced_basis, and Singular has limitations on the size of exponents. See http://www.singular.uni-kl.de/Manual/latest/sing_343.htm#SEC384 which in particular says\n\n```\nthe maximal allowed exponent of a ring variable depends on the ordering of the ring and is at least 32767.\n```\n\nIn this case increasing the exponent from 15 to 16 makes the output have an exponent of 16<sup>4</sup>=2<sup>16</sup>=65536, while leaving it at 15 puts it just a bit below that, allowing to contain it within a 16 bit integer.\n\n\nAssignee: tbd\n\nKeywords: groebner\n\nStatus: new\n\nIssue created by migration from https://trac.sagemath.org/ticket/6473\n\n",
    "created_at": "2009-07-07T05:00:28Z",
    "labels": [
        "component: commutative algebra",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.4",
    "title": "ideal.interreduced_basis hangs forever",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/6473",
    "user": "https://trac.sagemath.org/admin/accounts/users/broune"
}
```
This completes in a fraction of a second:

```
sage: R.<a,b,c,d>=PolynomialRing(QQ,order="lex")
sage: ideal(a^16,a-b^16,b-c^16,c-d^15).interreduced_basis()
[d^61440, c - d^15, b - d^240, a - d^3840]
```

I stopped the following after more than an hour, which leads me to believe that Sage is stuck in an infinite loop:

```
sage: R.<a,b,c,d>=PolynomialRing(QQ,order="lex")
sage: ideal(a^16,a-b^16,b-c^16,c-d^16).interreduced_basis()
```

The only difference between the two is that the power of d in the input binomial involving c as the initial term is increased from 15 to 16. This difference has no mathematical significance and should have no impact on the computation time.

I suspect that the reason is that by default Sage uses Singular to implement interreduced_basis, and Singular has limitations on the size of exponents. See http://www.singular.uni-kl.de/Manual/latest/sing_343.htm#SEC384 which in particular says

```
the maximal allowed exponent of a ring variable depends on the ordering of the ring and is at least 32767.
```

In this case increasing the exponent from 15 to 16 makes the output have an exponent of 16<sup>4</sup>=2<sup>16</sup>=65536, while leaving it at 15 puts it just a bit below that, allowing to contain it within a 16 bit integer.


Assignee: tbd

Keywords: groebner

Status: new

Issue created by migration from https://trac.sagemath.org/ticket/6473





---

archive/issue_events_026116.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-07-07T18:59:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6473",
    "milestone": "sage-4.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6473#event-26116"
}
```



---

archive/issue_events_026117.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6473",
    "milestone": "sage-4.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6473#event-26117"
}
```



---

archive/issue_events_026118.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6473",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6473#event-26118"
}
```



---

archive/issue_events_026119.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6473",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6473#event-26119"
}
```



---

archive/issue_events_026120.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6473",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6473#event-26120"
}
```



---

archive/issue_events_026121.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6473",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6473#event-26121"
}
```



---

archive/issue_events_026122.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6473",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6473#event-26122"
}
```



---

archive/issue_events_026123.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6473",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6473#event-26123"
}
```



---

archive/issue_events_026124.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6473",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6473#event-26124"
}
```



---

archive/issue_comments_057538.json:
```json
{
    "body": "Changing upstream from \"\" to \"N/A\"",
    "created_at": "2020-10-13T11:18:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6473",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6473#issuecomment-57538",
    "user": "https://github.com/fchapoton"
}
```

Changing upstream from "" to "N/A"



---

archive/issue_comments_057539.json:
```json
{
    "body": "Changing keywords from \"\" to \"groebner\".",
    "created_at": "2020-10-13T11:18:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6473",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6473#issuecomment-57539",
    "user": "https://github.com/fchapoton"
}
```

Changing keywords from "" to "groebner".
