# Issue 6350: [with spkg, with positive review] update M4RI to newest upstream release (20090617)

archive/issues_006350.json:
```json
{
    "body": "While fixing a few issues for OSX PPC and IA64 (Iras) I fixed L1 cache size detection for OSX x86 too. Thus, now M4RI uses the correct L1 cache size on OSX x86 machines.\n\nThe SPKG is at:\n\n  http://sage.math.washington.edu/home/malb/spkgs/libm4ri-20090617.spkg\n\nAssignee: @malb\n\nCC:  @craigcitro\n\nKeywords: m4ri\n\nResolution: fixed\n\nAuthor: Martin Albrecht\n\nReviewer: Craig Citro\n\nMerged: 4.0.2.rc3\n\nIssue created by migration from https://trac.sagemath.org/ticket/6350\n\n",
    "closed_at": "2009-06-17T23:56:37Z",
    "created_at": "2009-06-17T22:28:03Z",
    "labels": [
        "component: packages: standard"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.0.2",
    "title": "[with spkg, with positive review] update M4RI to newest upstream release (20090617)",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/6350",
    "user": "https://github.com/malb"
}
```
While fixing a few issues for OSX PPC and IA64 (Iras) I fixed L1 cache size detection for OSX x86 too. Thus, now M4RI uses the correct L1 cache size on OSX x86 machines.

The SPKG is at:

  http://sage.math.washington.edu/home/malb/spkgs/libm4ri-20090617.spkg

Assignee: @malb

CC:  @craigcitro

Keywords: m4ri

Resolution: fixed

Author: Martin Albrecht

Reviewer: Craig Citro

Merged: 4.0.2.rc3

Issue created by migration from https://trac.sagemath.org/ticket/6350





---

archive/issue_comments_056790.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Craig Citro\"",
    "created_at": "2009-06-17T23:56:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6350#issuecomment-56790",
    "user": "https://github.com/craigcitro"
}
```

Changing reviewer from "" to "Craig Citro"



---

archive/issue_comments_056791.json:
```json
{
    "body": "Changing merged from \"\" to \"4.0.2.rc3\"",
    "created_at": "2009-06-17T23:56:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6350#issuecomment-56791",
    "user": "https://github.com/craigcitro"
}
```

Changing merged from "" to "4.0.2.rc3"



---

archive/issue_comments_056792.json:
```json
{
    "body": "<a id='comment:1'></a>Looks good, works on several platforms (including the two that had issues before).",
    "created_at": "2009-06-17T23:56:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6350#issuecomment-56792",
    "user": "https://github.com/craigcitro"
}
```

<a id='comment:1'></a>Looks good, works on several platforms (including the two that had issues before).



---

archive/issue_comments_056793.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-06-17T23:56:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6350#issuecomment-56793",
    "user": "https://github.com/craigcitro"
}
```

Resolution: fixed



---

archive/issue_events_014941.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2009-06-17T23:56:37Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/6350",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6350#event-14941"
}
```
