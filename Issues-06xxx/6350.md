# Issue 6350: update M4RI to newest upstream release (20090617)

archive/issues_006350.json:
```json
{
    "assignees": [],
    "body": "While fixing a few issues for OSX PPC and IA64 (Iras) I fixed L1 cache size detection for OSX x86 too. Thus, now M4RI uses the correct L1 cache size on OSX x86 machines.\n\nThe SPKG is at:\n\n  http://sage.math.washington.edu/home/malb/spkgs/libm4ri-20090617.spkg\n\nAssignee: **@malb**\n\nCC:  @craigcitro\n\nKeywords: **m4ri**\n\nAuthor: **Martin Albrecht**\n\nReviewer: **Craig Citro**\n\nMerged: **4.0.2.rc3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6350_\n\n",
    "closed_at": "2009-06-17T23:56:37Z",
    "created_at": "2009-06-17T22:28:03Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.0.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "update M4RI to newest upstream release (20090617)",
    "type": "issue",
    "updated_at": "2009-06-17T23:56:37Z",
    "url": "https://github.com/sagemath/sage/issues/6350",
    "user": "https://github.com/malb"
}
```
While fixing a few issues for OSX PPC and IA64 (Iras) I fixed L1 cache size detection for OSX x86 too. Thus, now M4RI uses the correct L1 cache size on OSX x86 machines.

The SPKG is at:

  http://sage.math.washington.edu/home/malb/spkgs/libm4ri-20090617.spkg

Assignee: **@malb**

CC:  @craigcitro

Keywords: **m4ri**

Author: **Martin Albrecht**

Reviewer: **Craig Citro**

Merged: **4.0.2.rc3**

_Issue created by migration from https://trac.sagemath.org/ticket/6350_





---

archive/issue_events_066976.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-06-17T22:28:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6350",
    "milestone_number": null,
    "milestone_title": "sage-4.0.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6350#event-66976"
}
```



---

archive/issue_events_066977.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-06-17T22:28:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6350",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6350#event-66977"
}
```



---

archive/issue_events_066978.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-06-17T22:28:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6350",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6350#event-66978"
}
```



---

archive/issue_events_066979.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-06-17T22:28:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6350",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6350#event-66979"
}
```



---

archive/issue_events_066980.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-06-17T22:28:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6350",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6350#event-66980"
}
```



---

archive/issue_comments_043900.json:
```json
{
    "body": "Reviewer: **Craig Citro**",
    "created_at": "2009-06-17T23:56:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6350#issuecomment-43900",
    "user": "https://github.com/craigcitro"
}
```

Reviewer: **Craig Citro**



---

archive/issue_comments_043901.json:
```json
{
    "body": "Merged: **4.0.2.rc3**",
    "created_at": "2009-06-17T23:56:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6350#issuecomment-43901",
    "user": "https://github.com/craigcitro"
}
```

Merged: **4.0.2.rc3**



---

archive/issue_events_066981.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2009-06-17T23:56:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6350",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6350#event-66981"
}
```



---

archive/issue_comments_043902.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nLooks good, works on several platforms (including the two that had issues before).",
    "created_at": "2009-06-17T23:56:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6350#issuecomment-43902",
    "user": "https://github.com/craigcitro"
}
```

<a id='comment:1'>Comment 1:</a>
Looks good, works on several platforms (including the two that had issues before).



---

archive/issue_events_066982.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2009-06-17T23:56:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6350",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6350#event-66982"
}
```



---

archive/issue_events_066983.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2009-06-17T23:56:37Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6350",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6350#event-66983"
}
```
