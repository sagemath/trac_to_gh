# Issue 6130: README about documentation is outdated.

archive/issues_006130.json:
```json
{
    "body": "\n```\n8. OPTIONAL: Documentation: If you want to (try to) build the\n   documentation, change into SAGE_ROOT/devel/doc and type \"make\n   html\" or \"make pdf\".  This requires having latex and latex2html\n   installed, and there are some issues with the \\url macro.  Note\n   that the latex\n```\n\n**Assignee:** @mwhansen\n\nIssue created by migration from https://trac.sagemath.org/ticket/6130\n\n",
    "closed_at": "2009-05-28T07:44:14Z",
    "created_at": "2009-05-26T14:32:56Z",
    "labels": [
        "component: documentation",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.0",
    "title": "README about documentation is outdated.",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/6130",
    "user": "https://github.com/malb"
}
```

```
8. OPTIONAL: Documentation: If you want to (try to) build the
   documentation, change into SAGE_ROOT/devel/doc and type "make
   html" or "make pdf".  This requires having latex and latex2html
   installed, and there are some issues with the \url macro.  Note
   that the latex
```

**Assignee:** @mwhansen

Issue created by migration from https://trac.sagemath.org/ticket/6130





---

archive/issue_events_039796.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-28T07:37:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6130",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6130#event-39796"
}
```



---

archive/issue_comments_041859.json:
```json
{
    "body": "**Assignee:** @mwhansen",
    "created_at": "2009-05-28T07:37:02Z",
    "issue": "https://github.com/sagemath/sage/issues/6130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6130#issuecomment-41859",
    "user": "https://github.com/mwhansen"
}
```

**Assignee:** @mwhansen



---

archive/issue_comments_041860.json:
```json
{
    "body": "<a id='comment:1'></a>\nI've changed the text to:\n\n```\n    8. OPTIONAL: Documentation: If you want to (try to) build the\n       documentation, run \"sage -docbuild help\" for instructions.\n       This requires having latex installed (if you want to build PDFs\n       or HTML with PNG images for the math).  Note that the latex\n       docs come *pre-built* with Sage, and are in\n       SAGE_ROOT/devel/sage/doc/output/html.\n```",
    "created_at": "2009-05-28T07:37:02Z",
    "issue": "https://github.com/sagemath/sage/issues/6130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6130#issuecomment-41860",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'></a>
I've changed the text to:

```
    8. OPTIONAL: Documentation: If you want to (try to) build the
       documentation, run "sage -docbuild help" for instructions.
       This requires having latex installed (if you want to build PDFs
       or HTML with PNG images for the math).  Note that the latex
       docs come *pre-built* with Sage, and are in
       SAGE_ROOT/devel/sage/doc/output/html.
```



---

archive/issue_events_039797.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-28T07:37:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6130",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6130#event-39797"
}
```



---

archive/issue_events_039798.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-05-28T07:43:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6130",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6130#event-39798"
}
```



---

archive/issue_events_039799.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-28T07:44:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6130",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6130#event-39799"
}
```



---

archive/issue_events_039800.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-28T07:44:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6130",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6130#event-39800"
}
```



---

archive/issue_comments_041861.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in 4.0.rc1.",
    "created_at": "2009-05-28T07:44:14Z",
    "issue": "https://github.com/sagemath/sage/issues/6130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6130#issuecomment-41861",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:3'></a>
Merged in 4.0.rc1.
