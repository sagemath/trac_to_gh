# Issue 6689: [with patch, positive review] spell-check all modules under sage/categories

archive/issues_006689.json:
```json
{
    "body": "As the subject says.\n\nAssignee: tba\n\nCC:  sage-combinat\n\nResolution: fixed\n\nAuthor: Minh Van Nguyen\n\nReviewer: William Stein\n\nMerged: Sage 4.1.1\n\nIssue created by migration from https://trac.sagemath.org/ticket/6689\n\n",
    "closed_at": "2009-08-12T04:21:36Z",
    "created_at": "2009-08-08T17:18:48Z",
    "labels": [
        "component: documentation",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.1.1",
    "title": "[with patch, positive review] spell-check all modules under sage/categories",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/6689",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```
As the subject says.

Assignee: tba

CC:  sage-combinat

Resolution: fixed

Author: Minh Van Nguyen

Reviewer: William Stein

Merged: Sage 4.1.1

Issue created by migration from https://trac.sagemath.org/ticket/6689





---

archive/issue_comments_061011.json:
```json
{
    "body": "based on Sage 4.1.1.rc2",
    "created_at": "2009-08-08T17:20:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6689#issuecomment-61011",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

based on Sage 4.1.1.rc2



---

archive/issue_comments_061012.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [trac_6689-spell-check-categories.patch](tarball://root/attachments/some-uuid/ticket6689/trac_6689-spell-check-categories.patch) by mvngu created at 2009-08-08 17:21:10",
    "created_at": "2009-08-08T17:21:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6689#issuecomment-61012",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:1'></a>
Attachment [trac_6689-spell-check-categories.patch](tarball://root/attachments/some-uuid/ticket6689/trac_6689-spell-check-categories.patch) by mvngu created at 2009-08-08 17:21:10



---

archive/issue_comments_061013.json:
```json
{
    "body": "Changing author from \"\" to \"Minh Van Nguyen\"",
    "created_at": "2009-08-08T17:21:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6689#issuecomment-61013",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Changing author from "" to "Minh Van Nguyen"



---

archive/issue_events_027057.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/6689",
    "rename": {
        "from": "spell-check all modules under sage/categories",
        "to": "[with patch, needs review] spell-check all modules under sage/categories"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6689#event-27057"
}
```



---

archive/issue_events_027058.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/6689",
    "rename": {
        "from": "[with patch, needs review] spell-check all modules under sage/categories",
        "to": "[with patch, positive review] spell-check all modules under sage/categories"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6689#event-27058"
}
```



---

archive/issue_comments_061014.json:
```json
{
    "body": "Changing reviewer from \"\" to \"William Stein\"",
    "created_at": "2009-08-12T04:20:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6689#issuecomment-61014",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Changing reviewer from "" to "William Stein"



---

archive/issue_comments_061015.json:
```json
{
    "body": "Changing merged from \"\" to \"Sage 4.1.1\"",
    "created_at": "2009-08-12T04:20:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6689#issuecomment-61015",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Changing merged from "" to "Sage 4.1.1"



---

archive/issue_events_027059.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-08-12T04:21:36Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/6689",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6689#event-27059"
}
```



---

archive/issue_comments_061016.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-08-12T04:21:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6689#issuecomment-61016",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Resolution: fixed
