# Issue 6397: implement general Newton's method root finding for power series rings

archive/issues_006397.json:
```json
{
    "assignees": [
        "https://github.com/malb"
    ],
    "body": "Extracting a square root of a power series is implemented in `power_series_ring_element.pyx`.  Could we have the more general \"improving a root of a polynomial\" Newton's method?\n\nMy use case is calculating Puiseaux expansions around points of algebraic curves.\n\nCC:  @robertwb @williamstein\n\nKeywords: **power series root newton method**\n\nComponent: **commutative algebra**\n\nAuthor: **Nick Alexander**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6397_\n\n",
    "created_at": "2009-06-24T18:13:21Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20commutative%20algebra",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/feature",
        "https://github.com/sagemath/sage/labels/needs%20work"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "implement general Newton's method root finding for power series rings",
    "type": "issue",
    "updated_at": "2009-06-24T20:06:16Z",
    "url": "https://github.com/sagemath/sage/issues/6397",
    "user": "https://github.com/ncalexan"
}
```
Extracting a square root of a power series is implemented in `power_series_ring_element.pyx`.  Could we have the more general "improving a root of a polynomial" Newton's method?

My use case is calculating Puiseaux expansions around points of algebraic curves.

CC:  @robertwb @williamstein

Keywords: **power series root newton method**

Component: **commutative algebra**

Author: **Nick Alexander**

_Issue created by migration from https://trac.sagemath.org/ticket/6397_





---

archive/issue_events_077096.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-06-24T18:13:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6397",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20commutative%20algebra",
    "label_color": "0000ff",
    "label_name": "c: commutative algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6397#event-77096"
}
```



---

archive/issue_events_077097.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-06-24T18:13:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6397",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6397#event-77097"
}
```



---

archive/issue_events_077098.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-06-24T18:13:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6397",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6397#event-77098"
}
```



---

archive/issue_events_077099.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-06-24T18:13:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6397",
    "label": "https://github.com/sagemath/sage/labels/feature",
    "label_color": "008080",
    "label_name": "feature",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6397#event-77099"
}
```



---

archive/issue_events_077100.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-06-24T18:13:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6397",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6397#event-77100"
}
```



---

archive/issue_events_077101.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2009-06-24T18:13:21Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6397",
    "subject": "https://github.com/ncalexan",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6397#event-77101"
}
```



---

archive/issue_comments_043120.json:
```json
{
    "body": "Author: **Nick Alexander**",
    "created_at": "2009-06-24T20:06:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6397",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6397#issuecomment-43120",
    "user": "https://github.com/ncalexan"
}
```

Author: **Nick Alexander**



---

archive/issue_comments_043121.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nHere's a stand-alone implementation that needs to be plugged into the hell that is `polynomial.roots()`.",
    "created_at": "2009-06-24T20:06:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6397",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6397#issuecomment-43121",
    "user": "https://github.com/ncalexan"
}
```

<div id="comment:1" align="right">comment:1</div>

Here's a stand-alone implementation that needs to be plugged into the hell that is `polynomial.roots()`.



---

archive/issue_comments_043122.json:
```json
{
    "body": "Attachment: **[trac_6397-power-series-newton-ncalexan-1.pyx.gz](https://github.com/sagemath/sage/files/ticket6397/trac_6397-power-series-newton-ncalexan-1.pyx.gz)**",
    "created_at": "2009-06-24T20:07:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6397",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6397#issuecomment-43122",
    "user": "https://github.com/ncalexan"
}
```

Attachment: **[trac_6397-power-series-newton-ncalexan-1.pyx.gz](https://github.com/sagemath/sage/files/ticket6397/trac_6397-power-series-newton-ncalexan-1.pyx.gz)**
