# Issue 6356: Implement sage -t notebook.sws

archive/issues_006356.json:
```json
{
    "assignees": [],
    "body": "The title says it all.\nSee http://groups.google.com/group/sage-devel/msg/636fda60ff2f4d1c\n\nAlso, it would be nice to have a button when using notebook that would\nlaunch sage -t on it and display the result.\n\n**Keywords:** doctest, notebook\n\nIssue created by migration from https://trac.sagemath.org/ticket/6356\n\n",
    "closed_at": "2009-06-18T19:05:19Z",
    "created_at": "2009-06-18T16:55:04Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20doctest%20coverage",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Implement sage -t notebook.sws",
    "type": "issue",
    "updated_at": "2009-06-18T19:05:19Z",
    "url": "https://github.com/sagemath/sage/issues/6356",
    "user": "https://github.com/nthiery"
}
```
The title says it all.
See http://groups.google.com/group/sage-devel/msg/636fda60ff2f4d1c

Also, it would be nice to have a button when using notebook that would
launch sage -t on it and display the result.

**Keywords:** doctest, notebook

Issue created by migration from https://trac.sagemath.org/ticket/6356





---

archive/issue_events_063381.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2009-06-18T16:55:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6356",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20doctest%20coverage",
    "label_color": "08517b",
    "label_name": "component: doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6356#event-63381"
}
```



---

archive/issue_events_063382.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2009-06-18T16:55:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6356",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "008080",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6356#event-63382"
}
```



---

archive/issue_comments_043950.json:
```json
{
    "body": "<a id='comment:1'>**Comment 1:**</a>\n#6357 duplicates this.  Since I commented on that one a lot before seeing this one, I've closed this.",
    "created_at": "2009-06-18T19:05:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6356",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6356#issuecomment-43950",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'>**Comment 1:**</a>
#6357 duplicates this.  Since I commented on that one a lot before seeing this one, I've closed this.



---

archive/issue_events_063383.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-06-18T19:05:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6356",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "008080",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6356#event-63383"
}
```



---

archive/issue_events_063384.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-06-18T19:05:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6356",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6356#event-63384"
}
```
