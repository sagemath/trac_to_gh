# Issue 6156: [with patch; positive review] gap packages don't load anymore on itanium, even with gap-4.4.10.p11 (!)

archive/issues_006156.json:
```json
{
    "body": "\n\nAssignee: tbd\n\nResolution: fixed\n\nAuthor: William Stein\n\nReviewer: Mike Hansen\n\nMerged: 4.0.1.rc0\n\nIssue created by migration from https://trac.sagemath.org/ticket/6156\n\n",
    "closed_at": "2009-06-04T05:55:48Z",
    "created_at": "2009-05-30T04:06:32Z",
    "labels": [
        "component: build",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.0.1",
    "title": "[with patch; positive review] gap packages don't load anymore on itanium, even with gap-4.4.10.p11 (!)",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/6156",
    "user": "https://github.com/williamstein"
}
```


Assignee: tbd

Resolution: fixed

Author: William Stein

Reviewer: Mike Hansen

Merged: 4.0.1.rc0

Issue created by migration from https://trac.sagemath.org/ticket/6156





---

archive/issue_comments_054458.json:
```json
{
    "body": "<a id='comment:1'></a>mabshoff: \"gcc-4.4.0 is the reason GAP doesn't work on itanium skynet.\"\n\nmabshoff: \"the correct thing to do is to not compile gcc with arch or march settings. I have fixed 95% of #2999-3001 in all spkgs which would all to inject flags like march into each spkg.\"",
    "created_at": "2009-05-30T23:48:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6156",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6156#issuecomment-54458",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'></a>mabshoff: "gcc-4.4.0 is the reason GAP doesn't work on itanium skynet."

mabshoff: "the correct thing to do is to not compile gcc with arch or march settings. I have fixed 95% of #2999-3001 in all spkgs which would all to inject flags like march into each spkg."



---

archive/issue_comments_054459.json:
```json
{
    "body": "<a id='comment:2'></a>A fix for this problem is to build GAP with -O0. That's what we'll do.  \n\nI do wonder if gap-4.4.12 would work fine with -O0?\n\nAn spkg is up here, ready for review:   \n\n       http://sage.math.washington.edu/home/wstein/patches/gap-4.4.10.p12.spkg",
    "created_at": "2009-06-03T06:19:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6156",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6156#issuecomment-54459",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:2'></a>A fix for this problem is to build GAP with -O0. That's what we'll do.  

I do wonder if gap-4.4.12 would work fine with -O0?

An spkg is up here, ready for review:   

       http://sage.math.washington.edu/home/wstein/patches/gap-4.4.10.p12.spkg



---

archive/issue_comments_054460.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-06-04T05:55:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6156",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6156#issuecomment-54460",
    "user": "https://github.com/mwhansen"
}
```

Resolution: fixed



---

archive/issue_events_014479.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-06-04T05:55:48Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/6156",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6156#event-14479"
}
```



---

archive/issue_comments_054461.json:
```json
{
    "body": "<a id='comment:3'></a>Looks good to me.\n\nMerged in 4.0.1.rc0.",
    "created_at": "2009-06-04T05:55:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6156",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6156#issuecomment-54461",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:3'></a>Looks good to me.

Merged in 4.0.1.rc0.



---

archive/issue_comments_054462.json:
```json
{
    "body": "Changing author from \"\" to \"William Stein\"",
    "created_at": "2009-06-06T22:40:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6156",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6156#issuecomment-54462",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Changing author from "" to "William Stein"



---

archive/issue_comments_054463.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Mike Hansen\"",
    "created_at": "2009-06-06T22:40:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6156",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6156#issuecomment-54463",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Changing reviewer from "" to "Mike Hansen"



---

archive/issue_comments_054464.json:
```json
{
    "body": "Changing merged from \"\" to \"4.0.1.rc0\"",
    "created_at": "2009-06-06T22:40:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6156",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6156#issuecomment-54464",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Changing merged from "" to "4.0.1.rc0"
