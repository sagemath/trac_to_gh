# Issue 6557: fix bug in number field caching

archive/issues_006557.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "\n```\nHere are two independent Sage 4.1 sessions which demonstrate that the\nconstruction of NumberField's is context dependent:\n\n       sage: K.<x> = CyclotomicField(5)[]\n       sage: W.<a> = NumberField(x^2 + 1)\n       sage: W\n       Number Field in a with defining polynomial x^2 + 1 over its base field\n\n       sage: W1 = NumberField(x^2+1,'a')\n       sage: K.<x> = CyclotomicField(5)[]\n       sage: W.<a> = NumberField(x^2 + 1)\n       sage: W\n       Number Field in a with defining polynomial x^2 + 1\n\nIn fact:\n\n       sage: W1 is W0\n       True\n```\n\nKeywords: **Number fields**\n\nComponent: **number theory**\n\nAuthor: **William Stein**\n\nReviewer: **John Cremona**\n\nMerged: **sage-4.1.1.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6557_\n\n",
    "closed_at": "2009-07-19T14:19:47Z",
    "created_at": "2009-07-18T22:07:13Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20number%20theory",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix bug in number field caching",
    "type": "issue",
    "updated_at": "2009-07-19T14:19:47Z",
    "url": "https://github.com/sagemath/sage/issues/6557",
    "user": "https://github.com/williamstein"
}
```

```
Here are two independent Sage 4.1 sessions which demonstrate that the
construction of NumberField's is context dependent:

       sage: K.<x> = CyclotomicField(5)[]
       sage: W.<a> = NumberField(x^2 + 1)
       sage: W
       Number Field in a with defining polynomial x^2 + 1 over its base field

       sage: W1 = NumberField(x^2+1,'a')
       sage: K.<x> = CyclotomicField(5)[]
       sage: W.<a> = NumberField(x^2 + 1)
       sage: W
       Number Field in a with defining polynomial x^2 + 1

In fact:

       sage: W1 is W0
       True
```

Keywords: **Number fields**

Component: **number theory**

Author: **William Stein**

Reviewer: **John Cremona**

Merged: **sage-4.1.1.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/6557_





---

archive/issue_events_079294.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-07-18T22:07:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6557",
    "milestone_number": null,
    "milestone_title": "sage-4.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6557#event-79294"
}
```



---

archive/issue_events_079295.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-07-18T22:07:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6557",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20number%20theory",
    "label_color": "0000ff",
    "label_name": "c: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6557#event-79295"
}
```



---

archive/issue_events_079296.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-07-18T22:07:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6557",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6557#event-79296"
}
```



---

archive/issue_events_079297.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-07-18T22:07:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6557",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6557#event-79297"
}
```



---

archive/issue_events_079298.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-07-18T22:07:13Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6557",
    "subject": "https://github.com/williamstein",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6557#event-79298"
}
```



---

archive/issue_events_079299.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-07-18T22:18:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6557",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6557#event-79299"
}
```



---

archive/issue_comments_045298.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_6557.patch.gz](https://github.com/sagemath/sage/files/ticket6557/trac_6557.patch.gz)**",
    "created_at": "2009-07-18T22:18:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6557#issuecomment-45298",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_6557.patch.gz](https://github.com/sagemath/sage/files/ticket6557/trac_6557.patch.gz)**



---

archive/issue_comments_045299.json:
```json
{
    "body": "Reviewer: **John Cremona**",
    "created_at": "2009-07-19T12:19:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6557#issuecomment-45299",
    "user": "https://github.com/JohnCremona"
}
```

Reviewer: **John Cremona**



---

archive/issue_comments_045300.json:
```json
{
    "body": "Author: **William Stein**",
    "created_at": "2009-07-19T12:19:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6557#issuecomment-45300",
    "user": "https://github.com/JohnCremona"
}
```

Author: **William Stein**



---

archive/issue_comments_045301.json:
```json
{
    "body": "Changed keywords from none to **Number fields**",
    "created_at": "2009-07-19T12:19:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6557#issuecomment-45301",
    "user": "https://github.com/JohnCremona"
}
```

Changed keywords from none to **Number fields**



---

archive/issue_comments_045302.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nPositive review.  Applies to 4.1 and all tests in sage/rings/number_fields pass.",
    "created_at": "2009-07-19T12:19:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6557#issuecomment-45302",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:2" align="right">comment:2</div>

Positive review.  Applies to 4.1 and all tests in sage/rings/number_fields pass.



---

archive/issue_events_079300.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-07-19T12:19:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6557",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6557#event-79300"
}
```



---

archive/issue_events_079301.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-07-19T12:19:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6557",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6557#event-79301"
}
```



---

archive/issue_events_079302.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-19T14:19:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6557",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6557#event-79302"
}
```



---

archive/issue_events_079303.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-19T14:19:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6557",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6557#event-79303"
}
```



---

archive/issue_comments_045303.json:
```json
{
    "body": "Merged: **sage-4.1.1.alpha0**",
    "created_at": "2009-07-19T14:19:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6557",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6557#issuecomment-45303",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **sage-4.1.1.alpha0**



---

archive/issue_events_079304.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-19T14:19:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6557",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6557#event-79304"
}
```
