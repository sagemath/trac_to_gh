# Issue 6254: include mpmath 0.12

archive/issues_006254.json:
```json
{
    "body": "\n\n**Assignee:** mabshoff\n\n**CC:**  fredrik @jasongrout\n\n**Keywords:** mpmath\n\nIssue created by migration from https://trac.sagemath.org/ticket/6254\n\n",
    "closed_at": "2009-08-13T19:21:10Z",
    "created_at": "2009-06-09T22:28:27Z",
    "labels": [
        "component: packages: standard",
        "duplicate"
    ],
    "title": "include mpmath 0.12",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/6254",
    "user": "https://github.com/mwhansen"
}
```


**Assignee:** mabshoff

**CC:**  fredrik @jasongrout

**Keywords:** mpmath

Issue created by migration from https://trac.sagemath.org/ticket/6254





---

archive/issue_comments_043969.json:
```json
{
    "body": "<a id='comment:2'></a>\nWhat needs to be done?\n\nSee also #6196.",
    "created_at": "2009-06-11T11:08:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6254",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6254#issuecomment-43969",
    "user": "https://github.com/fredrik-johansson"
}
```

<a id='comment:2'></a>
What needs to be done?

See also #6196.



---

archive/issue_comments_043970.json:
```json
{
    "body": "<a id='comment:3'></a>\nThis is a duplicate of #6196.",
    "created_at": "2009-08-13T19:21:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6254",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6254#issuecomment-43970",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:3'></a>
This is a duplicate of #6196.



---

archive/issue_events_041443.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-08-13T19:21:10Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/6254",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6254#event-41443"
}
```



---

archive/issue_events_041444.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-08-13T19:21:10Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6254",
    "milestone": "sage-4.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6254#event-41444"
}
```



---

archive/issue_events_041445.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-08-13T19:21:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/6254",
    "label": "duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6254#event-41445"
}
```



---

archive/issue_events_041446.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-08-13T19:21:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/6254",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6254#event-41446"
}
```
