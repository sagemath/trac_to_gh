# Issue 6190: Fix documentation in sage/combinat/backtrack.py

archive/issues_006190.json:
```json
{
    "assignees": [
        "https://github.com/rbeezer"
    ],
    "body": "Four doctests introduced in #6000 need proper indentations, plus some lists, and some spelling.\n\nCC:  @loefflerd\n\nComponent: **combinatorics**\n\nAuthor: **David Loeffler**\n\nReviewer: **Rob Beezer**\n\nMerged: **4.0.1.rc0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6190_\n\n",
    "closed_at": "2009-06-03T20:25:30Z",
    "created_at": "2009-06-02T20:04:48Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.0.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix documentation in sage/combinat/backtrack.py",
    "type": "issue",
    "updated_at": "2009-06-06T22:43:08Z",
    "url": "https://github.com/sagemath/sage/issues/6190",
    "user": "https://github.com/rbeezer"
}
```
Four doctests introduced in #6000 need proper indentations, plus some lists, and some spelling.

CC:  @loefflerd

Component: **combinatorics**

Author: **David Loeffler**

Reviewer: **Rob Beezer**

Merged: **4.0.1.rc0**

_Issue created by migration from https://trac.sagemath.org/ticket/6190_





---

archive/issue_events_074293.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2009-06-02T20:04:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6190",
    "milestone_number": null,
    "milestone_title": "sage-4.0.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6190#event-74293"
}
```



---

archive/issue_events_074294.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2009-06-02T20:04:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6190",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "c: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6190#event-74294"
}
```



---

archive/issue_events_074295.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2009-06-02T20:04:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6190",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6190#event-74295"
}
```



---

archive/issue_events_074296.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2009-06-02T20:04:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6190",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6190#event-74296"
}
```



---

archive/issue_events_074297.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2009-06-02T20:04:48Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6190",
    "subject": "https://github.com/rbeezer",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6190#event-74297"
}
```



---

archive/issue_events_074298.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-06-02T22:17:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6190",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6190#event-74298"
}
```



---

archive/issue_comments_041002.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_6190.patch.gz](https://github.com/sagemath/sage/files/ticket6190/trac_6190.patch.gz)**\n\nHere's a patch.",
    "created_at": "2009-06-02T22:17:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6190",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6190#issuecomment-41002",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_6190.patch.gz](https://github.com/sagemath/sage/files/ticket6190/trac_6190.patch.gz)**

Here's a patch.



---

archive/issue_events_074299.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2009-06-03T00:13:35Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6190",
    "milestone_number": null,
    "milestone_title": "sage-4.0.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6190#event-74299"
}
```



---

archive/issue_events_074300.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2009-06-03T00:13:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6190",
    "milestone_number": null,
    "milestone_title": "sage-4.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6190#event-74300"
}
```



---

archive/issue_events_074301.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2009-06-03T00:13:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6190",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6190#event-74301"
}
```



---

archive/issue_events_074302.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2009-06-03T00:13:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6190",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6190#event-74302"
}
```



---

archive/issue_comments_041003.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nHi David,\n\nThanks for the changes.  I'd intended to do them, since I understand ReST better now than I did a few weeks ago, but you've done a nice job with it.\n\nPasses doctests on this one file, which it should since all of the changes are to documentation (not code, or tests).  HTML version builds without errors and looks very nice.\n\nPositive review.\n\nRob",
    "created_at": "2009-06-03T00:13:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6190",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6190#issuecomment-41003",
    "user": "https://github.com/rbeezer"
}
```

<div id="comment:2" align="right">comment:2</div>

Hi David,

Thanks for the changes.  I'd intended to do them, since I understand ReST better now than I did a few weeks ago, but you've done a nice job with it.

Passes doctests on this one file, which it should since all of the changes are to documentation (not code, or tests).  HTML version builds without errors and looks very nice.

Positive review.

Rob



---

archive/issue_comments_041004.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in 4.0.1.rc0.",
    "created_at": "2009-06-03T20:25:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6190",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6190#issuecomment-41004",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in 4.0.1.rc0.



---

archive/issue_events_074303.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-06-03T20:25:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6190",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6190#event-74303"
}
```



---

archive/issue_events_074304.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-06-03T20:25:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6190",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6190#event-74304"
}
```



---

archive/issue_comments_041005.json:
```json
{
    "body": "Reviewer: **Rob Beezer**",
    "created_at": "2009-06-06T22:43:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6190",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6190#issuecomment-41005",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Reviewer: **Rob Beezer**



---

archive/issue_comments_041006.json:
```json
{
    "body": "Author: **David Loeffler**",
    "created_at": "2009-06-06T22:43:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6190",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6190#issuecomment-41006",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Author: **David Loeffler**



---

archive/issue_comments_041007.json:
```json
{
    "body": "Merged: **4.0.1.rc0**",
    "created_at": "2009-06-06T22:43:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6190",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6190#issuecomment-41007",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **4.0.1.rc0**
