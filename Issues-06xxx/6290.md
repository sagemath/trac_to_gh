# Issue 6290: [with patch, positive review] Allow keywords such as latex_name=LaTeX while defining symbolic function

archive/issues_006290.json:
```json
{
    "body": "In new symbolics, underlying \"sage.symbolic.function.SFunction\" class allows one to pass the keywords such as \"latex_name=LaTeX\". It would be really good if we expose this feature at the user interface level. This will directly allow LaTeX customization by users according to their need.\n\nCurrently, Sage (4.0.1) raises error if one tries to do so.\n\n\nCouple of example usage:\n\n**(1)** Define symbolic function with its latex representaion\n\n```\nriemann(x) = function('riemann', x, latex_name=\"\\\\mathcal{R}\")\nlatex( riemann(x) )\n\\mathcal{R}\\left(x\\right)\n```\n\n\n**(2)**   Printing arguments of a symbolic functions as its indices.\n\n```\nmu,nu = var('mu,nu')\ndef my_latex_print(*args): \n    return \"\\psi_{%s}\"%(', '.join(map(latex, args)))\n\npsi(mu,nu) = function('psi', mu,nu, print_latex_func=my_latex_print)\n\nlatex(psi(mu,nu))\n\\psi_{\\mu, \\nu}\n```\n\n\nAttached patch allows keyword arguments while defining symbolic functions.\n\n\n\nCC:  @williamstein\n\nKeywords: latex_name, symbolic function\n\nResolution: fixed\n\nAuthor: Golam Mortuza Hossain\n\nReviewer: Nick Alexander\n\nMerged: 4.1.alpha0\n\nIssue created by migration from https://trac.sagemath.org/ticket/6290\n\n",
    "closed_at": "2009-06-24T09:44:35Z",
    "created_at": "2009-06-14T23:29:44Z",
    "labels": [
        "component: symbolics"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.1",
    "title": "[with patch, positive review] Allow keywords such as latex_name=LaTeX while defining symbolic function",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/6290",
    "user": "https://github.com/golam-m-hossain"
}
```
In new symbolics, underlying "sage.symbolic.function.SFunction" class allows one to pass the keywords such as "latex_name=LaTeX". It would be really good if we expose this feature at the user interface level. This will directly allow LaTeX customization by users according to their need.

Currently, Sage (4.0.1) raises error if one tries to do so.


Couple of example usage:

**(1)** Define symbolic function with its latex representaion

```
riemann(x) = function('riemann', x, latex_name="\\mathcal{R}")
latex( riemann(x) )
\mathcal{R}\left(x\right)
```


**(2)**   Printing arguments of a symbolic functions as its indices.

```
mu,nu = var('mu,nu')
def my_latex_print(*args): 
    return "\psi_{%s}"%(', '.join(map(latex, args)))

psi(mu,nu) = function('psi', mu,nu, print_latex_func=my_latex_print)

latex(psi(mu,nu))
\psi_{\mu, \nu}
```


Attached patch allows keyword arguments while defining symbolic functions.



CC:  @williamstein

Keywords: latex_name, symbolic function

Resolution: fixed

Author: Golam Mortuza Hossain

Reviewer: Nick Alexander

Merged: 4.1.alpha0

Issue created by migration from https://trac.sagemath.org/ticket/6290





---

archive/issue_comments_055988.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -3,11 +3,11 @@\n Sage (4.0.1) raises error if one tries to do so.\n \n \n-Example uses:\n+Example usage:\n \n ```\n riemann(x) = function('riemann', x, latex_name=\"\\\\mathcal{R}\")\n latex( riemann(x) )\n-\\mathcal{R}\n+\\mathcal{R}\\left(x\\right)\n ```\n \n``````\n",
    "created_at": "2009-06-14T23:35:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6290",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6290#issuecomment-55988",
    "user": "https://github.com/golam-m-hossain"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -3,11 +3,11 @@
 Sage (4.0.1) raises error if one tries to do so.
 
 
-Example uses:
+Example usage:
 
 ```
 riemann(x) = function('riemann', x, latex_name="\\mathcal{R}")
 latex( riemann(x) )
-\mathcal{R}
+\mathcal{R}\left(x\right)
 ```
 
``````




---

archive/issue_comments_055989.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,9 +1,11 @@\n-In new symbolics, underlying \"sage.symbolic.SFunction\" class allows\n-one to pass the keyword \"latex_name=LaTeX\". It would be really good if we expose this feature at the user interface level. Currently,\n-Sage (4.0.1) raises error if one tries to do so.\n+In new symbolics, underlying \"sage.symbolic.function.SFunction\" class allows one to pass the keywords such as \"latex_name=LaTeX\". It would be really good if we expose this feature at the user interface level. This will directly allow LaTeX customization by users according to their need.\n+\n+Currently, Sage (4.0.1) raises error if one tries to do so.\n \n \n-Example usage:\n+Couple of example usage:\n+\n+**(1)** Define symbolic function with its latex representaion\n \n ```\n riemann(x) = function('riemann', x, latex_name=\"\\\\mathcal{R}\")\n@@ -11,3 +13,21 @@\n \\mathcal{R}\\left(x\\right)\n ```\n \n+\n+**(2)**   Printing arguments of a symbolic functions as its indices.\n+\n+```\n+mu,nu = var('mu,nu')\n+def my_latex_print(*args): \n+    return \"\\psi_{%s}\"%(', '.join(map(latex, args)))\n+\n+psi(mu,nu) = function('psi', mu,nu, print_latex_func=my_latex_print)\n+\n+latex(psi(mu,nu))\n+\\psi_{\\mu, \\nu}\n+```\n+\n+\n+Attached patch allows keyword arguments while defining symbolic functions.\n+\n+\n``````\n",
    "created_at": "2009-06-15T17:09:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6290",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6290#issuecomment-55989",
    "user": "https://github.com/golam-m-hossain"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,9 +1,11 @@
-In new symbolics, underlying "sage.symbolic.SFunction" class allows
-one to pass the keyword "latex_name=LaTeX". It would be really good if we expose this feature at the user interface level. Currently,
-Sage (4.0.1) raises error if one tries to do so.
+In new symbolics, underlying "sage.symbolic.function.SFunction" class allows one to pass the keywords such as "latex_name=LaTeX". It would be really good if we expose this feature at the user interface level. This will directly allow LaTeX customization by users according to their need.
+
+Currently, Sage (4.0.1) raises error if one tries to do so.
 
 
-Example usage:
+Couple of example usage:
+
+**(1)** Define symbolic function with its latex representaion
 
 ```
 riemann(x) = function('riemann', x, latex_name="\\mathcal{R}")
@@ -11,3 +13,21 @@
 \mathcal{R}\left(x\right)
 ```
 
+
+**(2)**   Printing arguments of a symbolic functions as its indices.
+
+```
+mu,nu = var('mu,nu')
+def my_latex_print(*args): 
+    return "\psi_{%s}"%(', '.join(map(latex, args)))
+
+psi(mu,nu) = function('psi', mu,nu, print_latex_func=my_latex_print)
+
+latex(psi(mu,nu))
+\psi_{\mu, \nu}
+```
+
+
+Attached patch allows keyword arguments while defining symbolic functions.
+
+
``````




---

archive/issue_comments_055990.json:
```json
{
    "body": "<a id='comment:3'></a>Nice.  Could you add more to the docstring for function (at least the one that I get by default when I do function? from the sage prompt) that explains what happens to all kwds arguments?  You do give a nice example (with riemann), but a sentence or two explaining what is going on would be very nice.",
    "created_at": "2009-06-15T17:44:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6290",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6290#issuecomment-55990",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:3'></a>Nice.  Could you add more to the docstring for function (at least the one that I get by default when I do function? from the sage prompt) that explains what happens to all kwds arguments?  You do give a nice example (with riemann), but a sentence or two explaining what is going on would be very nice.



---

archive/issue_comments_055991.json:
```json
{
    "body": "Attachment [allow-keyword-arguments-such-as-latex_name-in-symbolic-functions.patch](tarball://root/attachments/some-uuid/ticket6290/allow-keyword-arguments-such-as-latex_name-in-symbolic-functions.patch) by @golam-m-hossain created at 2009-06-15 20:07:59",
    "created_at": "2009-06-15T20:07:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6290",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6290#issuecomment-55991",
    "user": "https://github.com/golam-m-hossain"
}
```

Attachment [allow-keyword-arguments-such-as-latex_name-in-symbolic-functions.patch](tarball://root/attachments/some-uuid/ticket6290/allow-keyword-arguments-such-as-latex_name-in-symbolic-functions.patch) by @golam-m-hossain created at 2009-06-15 20:07:59



---

archive/issue_comments_055992.json:
```json
{
    "body": "<a id='comment:4'></a>Replying to [was](#comment%3A3):\n> Nice.  Could you add more to the docstring for function (at least the one that I get by default when I do function? from the sage prompt) that explains what happens to all kwds arguments?  \n\n\nThanks for your comments! As you suggested, I have updated the patch with expanded docstrings.",
    "created_at": "2009-06-15T20:12:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6290",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6290#issuecomment-55992",
    "user": "https://github.com/golam-m-hossain"
}
```

<a id='comment:4'></a>Replying to [was](#comment%3A3):
> Nice.  Could you add more to the docstring for function (at least the one that I get by default when I do function? from the sage prompt) that explains what happens to all kwds arguments?  


Thanks for your comments! As you suggested, I have updated the patch with expanded docstrings.



---

archive/issue_comments_055993.json:
```json
{
    "body": "<a id='comment:5'></a>Looks good to me.",
    "created_at": "2009-06-15T20:36:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6290",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6290#issuecomment-55993",
    "user": "https://github.com/ncalexan"
}
```

<a id='comment:5'></a>Looks good to me.



---

archive/issue_events_014704.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-06-24T09:44:35Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/6290",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6290#event-14704"
}
```



---

archive/issue_comments_055994.json:
```json
{
    "body": "<a id='comment:6'></a>Merged in sage-4.1.alpha0.",
    "created_at": "2009-06-24T09:44:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6290",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6290#issuecomment-55994",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:6'></a>Merged in sage-4.1.alpha0.



---

archive/issue_comments_055995.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-06-24T09:44:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6290",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6290#issuecomment-55995",
    "user": "https://github.com/rlmill"
}
```

Resolution: fixed



---

archive/issue_events_014705.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-06-24T09:44:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6290",
    "milestone": "sage-4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6290#event-14705"
}
```



---

archive/issue_comments_055996.json:
```json
{
    "body": "Changing author from \"\" to \"Golam Mortuza Hossain\"",
    "created_at": "2009-06-25T00:33:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6290",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6290#issuecomment-55996",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Changing author from "" to "Golam Mortuza Hossain"



---

archive/issue_comments_055997.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Nick Alexander\"",
    "created_at": "2009-06-25T00:33:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6290",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6290#issuecomment-55997",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Changing reviewer from "" to "Nick Alexander"



---

archive/issue_comments_055998.json:
```json
{
    "body": "Changing merged from \"\" to \"4.1.alpha0\"",
    "created_at": "2009-06-25T00:33:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6290",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6290#issuecomment-55998",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Changing merged from "" to "4.1.alpha0"



---

archive/issue_comments_055999.json:
```json
{
    "body": "<a id='comment:8'></a>Would something similar be possible whenever a variable is defined?\nE.g. if I want to use multiple subscripts, I would like to define\nvar('A_gs', latex_name=\"A_{gs}\"). \n\nThat would make it really flexible.",
    "created_at": "2009-06-25T10:15:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6290",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6290#issuecomment-55999",
    "user": "https://trac.sagemath.org/admin/accounts/users/schymans"
}
```

<a id='comment:8'></a>Would something similar be possible whenever a variable is defined?
E.g. if I want to use multiple subscripts, I would like to define
var('A_gs', latex_name="A_{gs}"). 

That would make it really flexible.



---

archive/issue_comments_056000.json:
```json
{
    "body": "<a id='comment:9'></a>Replying to [schymans](#comment%3A8):\n> Would something similar be possible whenever a variable is defined?\n> E.g. if I want to use multiple subscripts, I would like to define\n> var('A_gs', latex_name=\"A_{gs}\"). \n\n\nHmmm, thats a great suggestion. Could you please post this to sage-devel?",
    "created_at": "2009-06-25T10:44:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6290",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6290#issuecomment-56000",
    "user": "https://github.com/golam-m-hossain"
}
```

<a id='comment:9'></a>Replying to [schymans](#comment%3A8):
> Would something similar be possible whenever a variable is defined?
> E.g. if I want to use multiple subscripts, I would like to define
> var('A_gs', latex_name="A_{gs}"). 


Hmmm, thats a great suggestion. Could you please post this to sage-devel?
