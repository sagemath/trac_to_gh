# Issue 6274: fix formatting of files under sage/plot

archive/issues_006274.json:
```json
{
    "assignees": [],
    "body": "This is a follow-up to #6270. Here, we expose more 2-D plotting functions in the reference manual. Patches should be applied in this order:\n1. the patch at #6257\n2. the patch at #6270\n3. and finally the patch on this ticket.\n\nCC:  @kcrisman @jhpalmieri\n\nKeywords: **docstring, doctest, plot**\n\nAuthor: **Minh Van Nguyen**\n\nReviewer: **John Palmieri**\n\nMerged: **sage-4.1.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6274_\n\n",
    "closed_at": "2009-06-24T10:11:04Z",
    "created_at": "2009-06-13T12:23:07Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix formatting of files under sage/plot",
    "type": "issue",
    "updated_at": "2009-06-24T10:11:04Z",
    "url": "https://github.com/sagemath/sage/issues/6274",
    "user": "https://github.com/sagetrac-mvngu"
}
```
This is a follow-up to #6270. Here, we expose more 2-D plotting functions in the reference manual. Patches should be applied in this order:
1. the patch at #6257
2. the patch at #6270
3. and finally the patch on this ticket.

CC:  @kcrisman @jhpalmieri

Keywords: **docstring, doctest, plot**

Author: **Minh Van Nguyen**

Reviewer: **John Palmieri**

Merged: **sage-4.1.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/6274_





---

archive/issue_events_065963.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-06-13T12:23:07Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "milestone_number": null,
    "milestone_title": "sage-4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6274#event-65963"
}
```



---

archive/issue_events_065964.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-06-13T12:23:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6274#event-65964"
}
```



---

archive/issue_events_065965.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-06-13T12:23:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6274#event-65965"
}
```



---

archive/issue_events_065966.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-06-13T12:23:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6274#event-65966"
}
```



---

archive/issue_comments_043162.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[trac_6274.patch.gz](https://github.com/sagemath/sage/files/ticket6274/trac_6274.patch.gz)**",
    "created_at": "2009-06-13T15:27:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6274#issuecomment-43162",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[trac_6274.patch.gz](https://github.com/sagemath/sage/files/ticket6274/trac_6274.patch.gz)**



---

archive/issue_comments_043163.json:
```json
{
    "body": "Changed keywords from **** to **docstring, doctest, plot**",
    "created_at": "2009-06-13T15:27:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6274#issuecomment-43163",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Changed keywords from **** to **docstring, doctest, plot**



---

archive/issue_events_065967.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-06-13T15:27:39Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "milestone_number": null,
    "milestone_title": "sage-4.0.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6274#event-65967"
}
```



---

archive/issue_comments_043164.json:
```json
{
    "body": "Author: **Minh Van Nguyen**",
    "created_at": "2009-06-13T15:27:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6274#issuecomment-43164",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Author: **Minh Van Nguyen**



---

archive/issue_events_065968.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-06-13T15:27:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6274#event-65968"
}
```



---

archive/issue_comments_043165.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,4 @@\n-This is a follow-up to #6270.\n+This is a follow-up to #6270. Here, we expose more 2-D plotting functions in the reference manual. Patches should be applied in this order:\n+1. the patch at #6257\n+2. the patch at #6270\n+3. and finally the patch on this ticket.\n``````\n",
    "created_at": "2009-06-13T15:27:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6274#issuecomment-43165",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,4 @@
-This is a follow-up to #6270.
+This is a follow-up to #6270. Here, we expose more 2-D plotting functions in the reference manual. Patches should be applied in this order:
+1. the patch at #6257
+2. the patch at #6270
+3. and finally the patch on this ticket.
``````




---

archive/issue_events_065969.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-06-15T23:42:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6274#event-65969"
}
```



---

archive/issue_events_065970.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-06-15T23:42:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6274#event-65970"
}
```



---

archive/issue_comments_043166.json:
```json
{
    "body": "Reviewer: **John Palmieri**",
    "created_at": "2009-06-15T23:42:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6274#issuecomment-43166",
    "user": "https://github.com/jhpalmieri"
}
```

Reviewer: **John Palmieri**



---

archive/issue_comments_043167.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nLooks good.  I'm attaching a referee's patch with two trivial fixes.",
    "created_at": "2009-06-15T23:42:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6274#issuecomment-43167",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:2'>Comment 2:</a>
Looks good.  I'm attaching a referee's patch with two trivial fixes.



---

archive/issue_comments_043168.json:
```json
{
    "body": "Attachment: **[ref_6274.patch.gz](https://github.com/sagemath/sage/files/ticket6274/ref_6274.patch.gz)**",
    "created_at": "2009-06-15T23:42:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6274#issuecomment-43168",
    "user": "https://github.com/jhpalmieri"
}
```

Attachment: **[ref_6274.patch.gz](https://github.com/sagemath/sage/files/ticket6274/ref_6274.patch.gz)**



---

archive/issue_comments_043169.json:
```json
{
    "body": "Merged: **sage-4.1.alpha0**",
    "created_at": "2009-06-24T10:11:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6274#issuecomment-43169",
    "user": "https://github.com/rlmill"
}
```

Merged: **sage-4.1.alpha0**



---

archive/issue_events_065971.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-06-24T10:11:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6274#event-65971"
}
```



---

archive/issue_events_065972.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-06-24T10:11:04Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6274",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6274#event-65972"
}
```
