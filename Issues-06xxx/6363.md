# Issue 6363: [with patch, with positive review] Display Sage version on notebook login page

archive/issues_006363.json:
```json
{
    "body": "Cf. [sage-devel](http://groups.google.com/group/sage-devel/browse_thread/thread/3e8484254e3a1cfb).\n\nAssignee: boothby\n\nResolution: fixed\n\nAuthor: Mitesh Patel\n\nReviewer: John Cremona\n\nMerged: sage-4.1.alpha0\n\nIssue created by migration from https://trac.sagemath.org/ticket/6363\n\n",
    "closed_at": "2009-06-24T09:46:01Z",
    "created_at": "2009-06-20T12:21:47Z",
    "labels": [
        "component: notebook",
        "trivial"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.1",
    "title": "[with patch, with positive review] Display Sage version on notebook login page",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/6363",
    "user": "https://github.com/qed777"
}
```
Cf. [sage-devel](http://groups.google.com/group/sage-devel/browse_thread/thread/3e8484254e3a1cfb).

Assignee: boothby

Resolution: fixed

Author: Mitesh Patel

Reviewer: John Cremona

Merged: sage-4.1.alpha0

Issue created by migration from https://trac.sagemath.org/ticket/6363





---

archive/issue_comments_056966.json:
```json
{
    "body": "Attachment [trac_6363_notebook_version.patch](tarball://root/attachments/some-uuid/ticket6363/trac_6363_notebook_version.patch) by @qed777 created at 2009-06-20 12:26:40",
    "created_at": "2009-06-20T12:26:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6363",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6363#issuecomment-56966",
    "user": "https://github.com/qed777"
}
```

Attachment [trac_6363_notebook_version.patch](tarball://root/attachments/some-uuid/ticket6363/trac_6363_notebook_version.patch) by @qed777 created at 2009-06-20 12:26:40



---

archive/issue_comments_056967.json:
```json
{
    "body": "<a id='comment:1'></a>The patch displays the Sage version on the notebook login page, e.g.,\n\n```\nSign into the Sage Notebook v4.0.2\n```\nin place of\n\n```\nSign into the Sage Notebook\n```",
    "created_at": "2009-06-20T12:29:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6363",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6363#issuecomment-56967",
    "user": "https://github.com/qed777"
}
```

<a id='comment:1'></a>The patch displays the Sage version on the notebook login page, e.g.,

```
Sign into the Sage Notebook v4.0.2
```
in place of

```
Sign into the Sage Notebook
```



---

archive/issue_comments_056968.json:
```json
{
    "body": "<a id='comment:2'></a>Review: patch applies fine to 4.0.2 and does what it says.  I tried both from the command line (sage -notebook) and also using notebook() from Sage prompt.",
    "created_at": "2009-06-20T13:31:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6363",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6363#issuecomment-56968",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:2'></a>Review: patch applies fine to 4.0.2 and does what it says.  I tried both from the command line (sage -notebook) and also using notebook() from Sage prompt.



---

archive/issue_comments_056969.json:
```json
{
    "body": "Changing author from \"mpatel\" to \"Mitesh Patel\"",
    "created_at": "2009-06-24T09:46:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6363",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6363#issuecomment-56969",
    "user": "https://github.com/rlmill"
}
```

Changing author from "mpatel" to "Mitesh Patel"



---

archive/issue_comments_056970.json:
```json
{
    "body": "Changing reviewer from \"\" to \"John Cremona\"",
    "created_at": "2009-06-24T09:46:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6363",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6363#issuecomment-56970",
    "user": "https://github.com/rlmill"
}
```

Changing reviewer from "" to "John Cremona"



---

archive/issue_events_014973.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-06-24T09:46:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6363",
    "milestone": "sage-4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6363#event-14973"
}
```



---

archive/issue_comments_056971.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-4.1.alpha0\"",
    "created_at": "2009-06-24T09:46:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6363",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6363#issuecomment-56971",
    "user": "https://github.com/rlmill"
}
```

Changing merged from "" to "sage-4.1.alpha0"



---

archive/issue_comments_056972.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-06-24T09:46:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6363",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6363#issuecomment-56972",
    "user": "https://github.com/rlmill"
}
```

Resolution: fixed



---

archive/issue_events_014974.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-06-24T09:46:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/6363",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6363#event-14974"
}
```
