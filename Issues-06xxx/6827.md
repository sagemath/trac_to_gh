# Issue 6827: probability distributions doctests + general discrete distribution

archive/issues_006827.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mhampton"
    ],
    "body": "This patch attends to sage/gsl/probability_distribution.pyx:\n\n- 100% doctest coverage (previously this file had nodoctest)\n\n- fixed formatting (all indents now 4 spaces).\n\n- ReST docstring formatting.\n\n- One extra class: general discrete distributions (I think that #6662 should be marked as invalid since the code there fits better here).\n\n- valgrind ok\n\n- probability_distribution.pyx added to the reference manual under Probability.\n\n\nComponent: **statistics**\n\nAuthor: **Carlo Hamalainen**\n\nReviewer: **Mike Hansen**\n\nMerged: **Sage 4.1.2.alpha2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6827_\n\n",
    "closed_at": "2009-09-09T10:39:00Z",
    "created_at": "2009-08-26T08:28:09Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20statistics",
        "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "probability distributions doctests + general discrete distribution",
    "type": "issue",
    "updated_at": "2009-09-09T10:39:00Z",
    "url": "https://github.com/sagemath/sage/issues/6827",
    "user": "https://github.com/sagetrac-carlohamalainen"
}
```
This patch attends to sage/gsl/probability_distribution.pyx:

- 100% doctest coverage (previously this file had nodoctest)

- fixed formatting (all indents now 4 spaces).

- ReST docstring formatting.

- One extra class: general discrete distributions (I think that #6662 should be marked as invalid since the code there fits better here).

- valgrind ok

- probability_distribution.pyx added to the reference manual under Probability.


Component: **statistics**

Author: **Carlo Hamalainen**

Reviewer: **Mike Hansen**

Merged: **Sage 4.1.2.alpha2**

_Issue created by migration from https://trac.sagemath.org/ticket/6827_





---

archive/issue_events_082885.json:
```json
{
    "actor": "https://github.com/sagetrac-carlohamalainen",
    "created_at": "2009-08-26T08:28:09Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6827",
    "milestone_number": null,
    "milestone_title": "sage-4.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6827#event-82885"
}
```



---

archive/issue_events_082886.json:
```json
{
    "actor": "https://github.com/sagetrac-carlohamalainen",
    "created_at": "2009-08-26T08:28:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6827",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20statistics",
    "label_color": "0000ff",
    "label_name": "c: statistics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6827#event-82886"
}
```



---

archive/issue_events_082887.json:
```json
{
    "actor": "https://github.com/sagetrac-carlohamalainen",
    "created_at": "2009-08-26T08:28:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6827",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6827#event-82887"
}
```



---

archive/issue_events_082888.json:
```json
{
    "actor": "https://github.com/sagetrac-carlohamalainen",
    "created_at": "2009-08-26T08:28:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6827",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6827#event-82888"
}
```



---

archive/issue_events_082889.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-08-26T08:28:09Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6827",
    "subject": "https://github.com/sagetrac-carlohamalainen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6827#event-82889"
}
```



---

archive/issue_events_082890.json:
```json
{
    "actor": "https://github.com/sagetrac-carlohamalainen",
    "created_at": "2009-08-26T08:28:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6827",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6827#event-82890"
}
```



---

archive/issue_comments_048368.json:
```json
{
    "body": "Attachment: **[probability_distribution.patch.gz](https://github.com/sagemath/sage/files/ticket6827/probability_distribution.patch.gz)**",
    "created_at": "2009-08-26T08:28:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6827",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6827#issuecomment-48368",
    "user": "https://github.com/sagetrac-carlohamalainen"
}
```

Attachment: **[probability_distribution.patch.gz](https://github.com/sagemath/sage/files/ticket6827/probability_distribution.patch.gz)**



---

archive/issue_comments_048369.json:
```json
{
    "body": "Author: **Carlo Hamalainen**",
    "created_at": "2009-09-07T21:22:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6827",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6827#issuecomment-48369",
    "user": "https://github.com/mwhansen"
}
```

Author: **Carlo Hamalainen**



---

archive/issue_comments_048370.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_6827_review.patch.gz](https://github.com/sagemath/sage/files/ticket6827/trac_6827_review.patch.gz)**\n\nLooks good to me.  I added a one character change to get the docs to build without warning.  Both patches should be applied.\n\nIt would probably be good to factor each of the RealDistributions out into their own classes so we don't have to have the massive if/elif/else statements.  It might be a good easy project if someone is working on stats stuff this fall.",
    "created_at": "2009-09-07T21:22:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6827",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6827#issuecomment-48370",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_6827_review.patch.gz](https://github.com/sagemath/sage/files/ticket6827/trac_6827_review.patch.gz)**

Looks good to me.  I added a one character change to get the docs to build without warning.  Both patches should be applied.

It would probably be good to factor each of the RealDistributions out into their own classes so we don't have to have the massive if/elif/else statements.  It might be a good easy project if someone is working on stats stuff this fall.



---

archive/issue_events_082891.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-09-07T21:22:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6827",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6827#event-82891"
}
```



---

archive/issue_comments_048371.json:
```json
{
    "body": "Reviewer: **Mike Hansen**",
    "created_at": "2009-09-07T21:22:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6827",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6827#issuecomment-48371",
    "user": "https://github.com/mwhansen"
}
```

Reviewer: **Mike Hansen**



---

archive/issue_events_082892.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-09T10:39:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6827",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6827#event-82892"
}
```



---

archive/issue_events_082893.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-09T10:39:00Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6827",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6827#event-82893"
}
```



---

archive/issue_comments_048372.json:
```json
{
    "body": "Merged: **Sage 4.1.2.alpha2**",
    "created_at": "2009-09-09T10:39:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6827",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6827#issuecomment-48372",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **Sage 4.1.2.alpha2**



---

archive/issue_comments_048373.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nMerged patches in this order:\n\n1. `probability_distribution.patch`\n2. `trac_6827_review.patch`",
    "created_at": "2009-09-09T10:39:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6827",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6827#issuecomment-48373",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:2" align="right">comment:2</div>

Merged patches in this order:

1. `probability_distribution.patch`
2. `trac_6827_review.patch`
