# Issue 6917: Minkowsky sum should work with polyhedra and not only with polytopes

archive/issues_006917.json:
```json
{
    "body": "The attached patch makes Minkowsky sum handle unbounded polyhedra.\n\nHowever, as a side effect, it makes bug #6915 blatant, thus doctests don't pass anymore.\n\nAssignee: mhampton\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/6917\n\n",
    "closed_at": "2010-04-03T14:43:09Z",
    "created_at": "2009-09-10T12:48:39Z",
    "labels": [
        "component: geometry"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.4",
    "title": "Minkowsky sum should work with polyhedra and not only with polytopes",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/6917",
    "user": "https://trac.sagemath.org/admin/accounts/users/sbarthelemy"
}
```
The attached patch makes Minkowsky sum handle unbounded polyhedra.

However, as a side effect, it makes bug #6915 blatant, thus doctests don't pass anymore.

Assignee: mhampton

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/6917





---

archive/issue_comments_063878.json:
```json
{
    "body": "Attachment [mink_sum.patch](tarball://root/attachments/some-uuid/ticket6917/mink_sum.patch) by sbarthelemy created at 2009-09-10 12:49:17",
    "created_at": "2009-09-10T12:49:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6917",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6917#issuecomment-63878",
    "user": "https://trac.sagemath.org/admin/accounts/users/sbarthelemy"
}
```

Attachment [mink_sum.patch](tarball://root/attachments/some-uuid/ticket6917/mink_sum.patch) by sbarthelemy created at 2009-09-10 12:49:17



---

archive/issue_comments_063879.json:
```json
{
    "body": "<a id='comment:1'></a>\nIs this ready for review? If so, please change the summary to \"[with patch, needs review]\".",
    "created_at": "2009-09-10T18:06:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6917",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6917#issuecomment-63879",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:1'></a>
Is this ready for review? If so, please change the summary to "[with patch, needs review]".



---

archive/issue_comments_063880.json:
```json
{
    "body": "<a id='comment:2'></a>\nI believe this patch is unnecessary because of #7109, which fixed a lot of related problems.",
    "created_at": "2010-04-03T14:43:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6917",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6917#issuecomment-63880",
    "user": "https://trac.sagemath.org/admin/accounts/users/mhampton"
}
```

<a id='comment:2'></a>
I believe this patch is unnecessary because of #7109, which fixed a lot of related problems.



---

archive/issue_events_027966.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/6917",
    "rename": {
        "from": "[with patch] Minkowsky sum sould work with polyhedra and not only with polytopes",
        "to": "[with patch] Minkowsky sum should work with polyhedra and not only with polytopes"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6917#event-27966"
}
```



---

archive/issue_comments_063881.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2010-04-03T14:43:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6917",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6917#issuecomment-63881",
    "user": "https://trac.sagemath.org/admin/accounts/users/mhampton"
}
```

Resolution: fixed



---

archive/issue_comments_063882.json:
```json
{
    "body": "Changing upstream from \"\" to \"N/A\"",
    "created_at": "2010-04-03T14:43:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6917",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6917#issuecomment-63882",
    "user": "https://trac.sagemath.org/admin/accounts/users/mhampton"
}
```

Changing upstream from "" to "N/A"



---

archive/issue_events_027967.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mhampton",
    "created_at": "2010-04-03T14:43:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/6917",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6917#event-27967"
}
```



---

archive/issue_events_027968.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/6917",
    "rename": {
        "from": "[with patch] Minkowsky sum should work with polyhedra and not only with polytopes",
        "to": "Minkowsky sum should work with polyhedra and not only with polytopes"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6917#event-27968"
}
```



---

archive/issue_events_027969.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-04-05T18:29:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6917",
    "milestone": "sage-4.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6917#event-27969"
}
```
