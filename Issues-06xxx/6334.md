# Issue 6334: optional doctest failure -- broken finance doctest failures

archive/issues_006334.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage -t -long --optional devel/sage/sage/finance/stock.py\n**********************************************************************\nFile \"/scratch/wstein/build/sage-4.0.2.alpha3/devel/sage-main/sage/finance/stock.py\", line 180:\n    sage: finance.Stock('vmw').google()[:5]   # optional -- internet\nExpected:\n    [\n     28-Nov-07 80.57 88.49 80.57 87.69    7496000,\n     29-Nov-07 90.91 93.20 89.50 90.85    5497600,\n     30-Nov-07 95.39 95.60 89.85 91.37    4750200,\n      3-Dec-07 89.87 96.00 88.70 94.97    4401100,\n      4-Dec-07 92.26 97.10 92.05 95.08    2896600\n    ]\nGot:\n    [\n     16-Jun-08 66.00 67.50 65.60 67.47    1742000,\n     17-Jun-08 67.84 67.84 66.03 67.00    1196900,\n     18-Jun-08 66.50 66.56 64.76 66.19    1186400,\n     19-Jun-08 65.92 66.50 64.69 65.72     549200,\n     20-Jun-08 65.72 65.72 63.12 63.86    1242300\n    ]\n**********************************************************************\n1 items had failures:\n   1 of   7 in __main__.example_9\n***Test Failed*** 1 failures.\n```\n\nAssignee: **@cswiercz**\n\nCC:  @cswiercz\n\nKeywords: **finance, stock**\n\nReviewer: **Burcin Erocal**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6334_\n\n",
    "closed_at": "2014-01-04T04:23:19Z",
    "created_at": "2009-06-16T15:16:52Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "optional doctest failure -- broken finance doctest failures",
    "type": "issue",
    "updated_at": "2014-01-04T04:23:19Z",
    "url": "https://github.com/sagemath/sage/issues/6334",
    "user": "https://github.com/williamstein"
}
```

```
sage -t -long --optional devel/sage/sage/finance/stock.py
**********************************************************************
File "/scratch/wstein/build/sage-4.0.2.alpha3/devel/sage-main/sage/finance/stock.py", line 180:
    sage: finance.Stock('vmw').google()[:5]   # optional -- internet
Expected:
    [
     28-Nov-07 80.57 88.49 80.57 87.69    7496000,
     29-Nov-07 90.91 93.20 89.50 90.85    5497600,
     30-Nov-07 95.39 95.60 89.85 91.37    4750200,
      3-Dec-07 89.87 96.00 88.70 94.97    4401100,
      4-Dec-07 92.26 97.10 92.05 95.08    2896600
    ]
Got:
    [
     16-Jun-08 66.00 67.50 65.60 67.47    1742000,
     17-Jun-08 67.84 67.84 66.03 67.00    1196900,
     18-Jun-08 66.50 66.56 64.76 66.19    1186400,
     19-Jun-08 65.92 66.50 64.69 65.72     549200,
     20-Jun-08 65.72 65.72 63.12 63.86    1242300
    ]
**********************************************************************
1 items had failures:
   1 of   7 in __main__.example_9
***Test Failed*** 1 failures.
```

Assignee: **@cswiercz**

CC:  @cswiercz

Keywords: **finance, stock**

Reviewer: **Burcin Erocal**

_Issue created by migration from https://trac.sagemath.org/ticket/6334_





---

archive/issue_events_068525.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-06-16T15:16:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "milestone_number": null,
    "milestone_title": "sage-5.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6334#event-68525"
}
```



---

archive/issue_events_068526.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-06-16T15:16:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
    "label_color": "000080",
    "label_name": "component: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6334#event-68526"
}
```



---

archive/issue_events_068527.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-06-16T15:16:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6334#event-68527"
}
```



---

archive/issue_events_068528.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-06-16T15:16:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6334#event-68528"
}
```



---

archive/issue_events_068529.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-06-16T15:16:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6334#event-68529"
}
```



---

archive/issue_comments_043652.json:
```json
{
    "body": "Assignee: **@cswiercz**",
    "created_at": "2009-06-16T15:44:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6334#issuecomment-43652",
    "user": "https://github.com/cswiercz"
}
```

Assignee: **@cswiercz**



---

archive/issue_comments_043653.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nAttachment: **[sage-6334.patch.gz](https://github.com/sagemath/sage/files/ticket6334/sage-6334.patch.gz)**\n\nI also updated stock.py's documentation for Sphinx formatting.",
    "created_at": "2009-06-16T22:18:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6334#issuecomment-43653",
    "user": "https://github.com/cswiercz"
}
```

<a id='comment:2'>Comment 2:</a>
Attachment: **[sage-6334.patch.gz](https://github.com/sagemath/sage/files/ticket6334/sage-6334.patch.gz)**

I also updated stock.py's documentation for Sphinx formatting.



---

archive/issue_comments_043654.json:
```json
{
    "body": "Changed keywords from **** to **finance, stock**",
    "created_at": "2009-06-16T22:18:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6334#issuecomment-43654",
    "user": "https://github.com/cswiercz"
}
```

Changed keywords from **** to **finance, stock**



---

archive/issue_events_068530.json:
```json
{
    "actor": "https://github.com/cswiercz",
    "created_at": "2009-06-16T22:18:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6334#event-68530"
}
```



---

archive/issue_events_068531.json:
```json
{
    "actor": "https://github.com/cswiercz",
    "created_at": "2009-06-16T22:18:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6334#event-68531"
}
```



---

archive/issue_events_068532.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2009-07-17T04:00:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6334#event-68532"
}
```



---

archive/issue_events_068533.json:
```json
{
    "actor": "https://github.com/rbeezer",
    "created_at": "2009-07-17T04:00:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6334#event-68533"
}
```



---

archive/issue_comments_043655.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\n1.  I still get 12 optional doctest failures, it looks like the requested dates are not being honored, but I haven't deduced a pattern to it.  Full output in attached text file.\n\n2.  Documentation can be built with a lot fewer blank lines, especially inbetween list elements.  Reviewer patch illustrates this and corrects one misspelled word.\n\n3.  I couldn't test the documentation since it doesn't seem to get pulled into the reference manual.  Should it be included?  If not, how should it be tested?",
    "created_at": "2009-07-17T04:00:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6334#issuecomment-43655",
    "user": "https://github.com/rbeezer"
}
```

<a id='comment:3'>Comment 3:</a>
1.  I still get 12 optional doctest failures, it looks like the requested dates are not being honored, but I haven't deduced a pattern to it.  Full output in attached text file.

2.  Documentation can be built with a lot fewer blank lines, especially inbetween list elements.  Reviewer patch illustrates this and corrects one misspelled word.

3.  I couldn't test the documentation since it doesn't seem to get pulled into the reference manual.  Should it be included?  If not, how should it be tested?



---

archive/issue_comments_043656.json:
```json
{
    "body": "sage -t -optional   output on  stock.py",
    "created_at": "2009-07-17T04:01:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6334#issuecomment-43656",
    "user": "https://github.com/rbeezer"
}
```

sage -t -optional   output on  stock.py



---

archive/issue_comments_043657.json:
```json
{
    "body": "Attachment: **[stock-tests.txt](https://github.com/sagemath/sage/files/ticket6334/stock-tests.txt)**",
    "created_at": "2009-07-17T04:02:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6334#issuecomment-43657",
    "user": "https://github.com/rbeezer"
}
```

Attachment: **[stock-tests.txt](https://github.com/sagemath/sage/files/ticket6334/stock-tests.txt)**



---

archive/issue_events_068534.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2013-01-11T11:05:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "milestone_number": null,
    "milestone_title": "sage-5.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6334#event-68534"
}
```



---

archive/issue_events_068535.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2013-01-11T11:05:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6334#event-68535"
}
```



---

archive/issue_events_068536.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2013-01-11T11:05:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6334#event-68536"
}
```



---

archive/issue_events_068537.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2013-01-11T11:05:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6334#event-68537"
}
```



---

archive/issue_events_068538.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2013-01-11T11:05:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6334#event-68538"
}
```



---

archive/issue_comments_043658.json:
```json
{
    "body": "Reviewer: **Burcin Erocal**",
    "created_at": "2013-01-11T11:05:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6334#issuecomment-43658",
    "user": "https://github.com/burcin"
}
```

Reviewer: **Burcin Erocal**



---

archive/issue_comments_043659.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nAttachment: **[trac_6334_reviewer_edits.patch.gz](https://github.com/sagemath/sage/files/ticket6334/trac_6334_reviewer_edits.patch.gz)**\n\nIt looks like Minh fixed the documentation in `sage/finance` with [attachment: trac_9218-reviewer.patch:ticket:9218.gz](https://github.com/sagemath/sage/files/ticket6334/1601d95241f0887001f45926f86ba742.gz).\n\nThere is also #13884 to fix the optional doctests, where Karl-Dieter posted a patch.\n\nShall we close this ticket as duplicate?",
    "created_at": "2013-01-11T11:05:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6334#issuecomment-43659",
    "user": "https://github.com/burcin"
}
```

<a id='comment:4'>Comment 4:</a>
Attachment: **[trac_6334_reviewer_edits.patch.gz](https://github.com/sagemath/sage/files/ticket6334/trac_6334_reviewer_edits.patch.gz)**

It looks like Minh fixed the documentation in `sage/finance` with [attachment: trac_9218-reviewer.patch:ticket:9218.gz](https://github.com/sagemath/sage/files/ticket6334/1601d95241f0887001f45926f86ba742.gz).

There is also #13884 to fix the optional doctests, where Karl-Dieter posted a patch.

Shall we close this ticket as duplicate?



---

archive/issue_events_068539.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-12-25T13:29:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6334#event-68539"
}
```



---

archive/issue_events_068540.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-12-25T13:29:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6334#event-68540"
}
```



---

archive/issue_events_068541.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-12-25T13:29:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6334#event-68541"
}
```



---

archive/issue_events_068542.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-12-25T13:29:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6334#event-68542"
}
```



---

archive/issue_events_068543.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-01-04T04:23:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6334#event-68543"
}
```



---

archive/issue_events_068544.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-01-04T04:23:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6334#event-68544"
}
```



---

archive/issue_events_068545.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-01-04T04:23:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6334",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6334#event-68545"
}
```
