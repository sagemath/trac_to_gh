# Issue 6497: [with patch, positive review] symbolic functions should understand numpy arrays

archive/issues_006497.json:
```json
{
    "body": "We want\n\n```\nsage: import numpy\nsage: sin(numpy.arange(5))\narray([ 0.        ,  0.84147098,  0.90929743,  0.14112001, -0.7568025 ])\n```\n\nAssignee: @burcin\n\nResolution: fixed\n\nAuthor: Robert Bradshaw\n\nReviewer: Burcin Erocal\n\nMerged: Sage 4.1.1.rc0\n\nIssue created by migration from https://trac.sagemath.org/ticket/6497\n\n",
    "closed_at": "2009-09-10T09:41:40Z",
    "created_at": "2009-07-09T10:14:40Z",
    "labels": [
        "component: calculus",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.1.2",
    "title": "[with patch, positive review] symbolic functions should understand numpy arrays",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/6497",
    "user": "https://github.com/robertwb"
}
```
We want

```
sage: import numpy
sage: sin(numpy.arange(5))
array([ 0.        ,  0.84147098,  0.90929743,  0.14112001, -0.7568025 ])
```

Assignee: @burcin

Resolution: fixed

Author: Robert Bradshaw

Reviewer: Burcin Erocal

Merged: Sage 4.1.1.rc0

Issue created by migration from https://trac.sagemath.org/ticket/6497





---

archive/issue_comments_058196.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [6497-numpy-sin.patch](tarball://root/attachments/some-uuid/ticket6497/6497-numpy-sin.patch) by @burcin created at 2009-07-12 10:33:03\n\nThis depends on #5081.",
    "created_at": "2009-07-12T10:33:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6497#issuecomment-58196",
    "user": "https://github.com/burcin"
}
```

<a id='comment:1'></a>
Attachment [6497-numpy-sin.patch](tarball://root/attachments/some-uuid/ticket6497/6497-numpy-sin.patch) by @burcin created at 2009-07-12 10:33:03

This depends on #5081.



---

archive/issue_events_026217.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/6497",
    "rename": {
        "from": "[with patch, needs review] symbolic functions should understand numpy arrays",
        "to": "[with patch, positive review] symbolic functions should understand numpy arrays"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6497#event-26217"
}
```



---

archive/issue_comments_058197.json:
```json
{
    "body": "Changing author from \"\" to \"Robert Bradshaw\"",
    "created_at": "2009-07-12T10:33:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6497#issuecomment-58197",
    "user": "https://github.com/burcin"
}
```

Changing author from "" to "Robert Bradshaw"



---

archive/issue_comments_058198.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Burcin Erocal\"",
    "created_at": "2009-07-12T10:33:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6497#issuecomment-58198",
    "user": "https://github.com/burcin"
}
```

Changing reviewer from "" to "Burcin Erocal"



---

archive/issue_comments_058199.json:
```json
{
    "body": "<a id='comment:2'></a>\nWith the patches at #5081 and #6506, and `6497-numpy-sin.patch` on this ticket, I got one doctest failure:\n\n```\nsage -t -long devel/sage-main/sage/modules/vector_double_dense.pyx\n**********************************************************************\nFile \"/scratch/mvngu/release/sage-4.1.1.alpha1/devel/sage-main/sage/modules/vector_double_dense.pyx\", line 663:\n    sage: v.stats_kurtosis()\nExpected:\n    -1.23\nGot:\n    doctest:106: SyntaxWarning: assertion is always true, perhaps remove parentheses?\n    -1.23\n**********************************************************************\n1 items had failures:\n   1 of   6 in __main__.example_29\n***Test Failed*** 1 failures.\nFor whitespace errors, see the file /scratch/mvngu/release/sage-4.1.1.alpha1/tmp/.doctest_vector_double_dense.py\n\t [2.5 s]\n```",
    "created_at": "2009-07-30T01:42:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6497#issuecomment-58199",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:2'></a>
With the patches at #5081 and #6506, and `6497-numpy-sin.patch` on this ticket, I got one doctest failure:

```
sage -t -long devel/sage-main/sage/modules/vector_double_dense.pyx
**********************************************************************
File "/scratch/mvngu/release/sage-4.1.1.alpha1/devel/sage-main/sage/modules/vector_double_dense.pyx", line 663:
    sage: v.stats_kurtosis()
Expected:
    -1.23
Got:
    doctest:106: SyntaxWarning: assertion is always true, perhaps remove parentheses?
    -1.23
**********************************************************************
1 items had failures:
   1 of   6 in __main__.example_29
***Test Failed*** 1 failures.
For whitespace errors, see the file /scratch/mvngu/release/sage-4.1.1.alpha1/tmp/.doctest_vector_double_dense.py
	 [2.5 s]
```



---

archive/issue_events_026218.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/6497",
    "rename": {
        "from": "[with patch, positive review] symbolic functions should understand numpy arrays",
        "to": "[with patch, needs work] symbolic functions should understand numpy arrays"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6497#event-26218"
}
```



---

archive/issue_comments_058200.json:
```json
{
    "body": "<a id='comment:3'></a>\nI am unable to reproduce the error above on 4.1.1, so I am re-instating the positive review. (Also, the error itself seems totally unrelated to this patch.)",
    "created_at": "2009-09-10T09:28:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6497#issuecomment-58200",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:3'></a>
I am unable to reproduce the error above on 4.1.1, so I am re-instating the positive review. (Also, the error itself seems totally unrelated to this patch.)



---

archive/issue_events_026219.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/6497",
    "rename": {
        "from": "[with patch, needs work] symbolic functions should understand numpy arrays",
        "to": "[with patch, positive review] symbolic functions should understand numpy arrays"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6497#event-26219"
}
```



---

archive/issue_comments_058201.json:
```json
{
    "body": "<a id='comment:4'></a>\nReplying to [robertwb](#comment%3A3):\n> I am unable to reproduce the error above on 4.1.1, so I am re-instating the positive review. (Also, the error itself seems totally unrelated to this patch.)\n\nThe above error is not due to the patch. Ticket #6825 contains steps to reproduce the errors.",
    "created_at": "2009-09-10T09:31:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6497#issuecomment-58201",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:4'></a>
Replying to [robertwb](#comment%3A3):
> I am unable to reproduce the error above on 4.1.1, so I am re-instating the positive review. (Also, the error itself seems totally unrelated to this patch.)

The above error is not due to the patch. Ticket #6825 contains steps to reproduce the errors.



---

archive/issue_comments_058202.json:
```json
{
    "body": "Changing merged from \"\" to \"Sage 4.1.1.rc0\"",
    "created_at": "2009-09-10T09:41:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6497#issuecomment-58202",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Changing merged from "" to "Sage 4.1.1.rc0"



---

archive/issue_events_026220.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-09-10T09:41:40Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/6497",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6497#event-26220"
}
```



---

archive/issue_comments_058203.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-09-10T09:41:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6497#issuecomment-58203",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Resolution: fixed
