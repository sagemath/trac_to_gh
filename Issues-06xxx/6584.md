# Issue 6584: Use pari to do ideal intersections

archive/issues_006584.json:
```json
{
    "assignees": [
        "https://github.com/loefflerd"
    ],
    "body": "As an addendum to #6457, I propose reworking the code to use the pari function `idealintersect`.   The patch does this.  The result is a significantly faster function. \n\nCC:  @loefflerd @ncalexan\n\nKeywords: **ideal, intersecton, pari**\n\nComponent: **number fields**\n\nAuthor: **Francis Clarke**\n\nReviewer: **John Cremona**\n\nMerged: **sage-4.3.1.rc1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6584_\n\n",
    "closed_at": "2010-01-19T00:56:17Z",
    "created_at": "2009-07-21T21:33:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20number%20fields",
        "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Use pari to do ideal intersections",
    "type": "issue",
    "updated_at": "2010-01-19T00:56:17Z",
    "url": "https://github.com/sagemath/sage/issues/6584",
    "user": "https://github.com/sagetrac-fwclarke"
}
```
As an addendum to #6457, I propose reworking the code to use the pari function `idealintersect`.   The patch does this.  The result is a significantly faster function. 

CC:  @loefflerd @ncalexan

Keywords: **ideal, intersecton, pari**

Component: **number fields**

Author: **Francis Clarke**

Reviewer: **John Cremona**

Merged: **sage-4.3.1.rc1**

_Issue created by migration from https://trac.sagemath.org/ticket/6584_





---

archive/issue_events_079694.json:
```json
{
    "actor": "https://github.com/sagetrac-fwclarke",
    "created_at": "2009-07-21T21:33:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6584",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20number%20fields",
    "label_color": "0000ff",
    "label_name": "c: number fields",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6584#event-79694"
}
```



---

archive/issue_events_079695.json:
```json
{
    "actor": "https://github.com/sagetrac-fwclarke",
    "created_at": "2009-07-21T21:33:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6584",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6584#event-79695"
}
```



---

archive/issue_events_079696.json:
```json
{
    "actor": "https://github.com/sagetrac-fwclarke",
    "created_at": "2009-07-21T21:33:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6584",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6584#event-79696"
}
```



---

archive/issue_events_079697.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-21T21:33:28Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6584",
    "subject": "https://github.com/sagetrac-fwclarke",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6584#event-79697"
}
```



---

archive/issue_comments_045625.json:
```json
{
    "body": "patch against 4.1.1.alpha0",
    "created_at": "2009-07-21T21:35:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6584",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6584#issuecomment-45625",
    "user": "https://github.com/sagetrac-fwclarke"
}
```

patch against 4.1.1.alpha0



---

archive/issue_comments_045626.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_6584.patch.gz](https://github.com/sagemath/sage/files/ticket6584/trac_6584.patch.gz)**\n\nThe patch still applies to 4.3.1.rc0 (miracle!) and all tests in rings/number_field and libs/pari pass.",
    "created_at": "2010-01-18T20:34:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6584",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6584#issuecomment-45626",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_6584.patch.gz](https://github.com/sagemath/sage/files/ticket6584/trac_6584.patch.gz)**

The patch still applies to 4.3.1.rc0 (miracle!) and all tests in rings/number_field and libs/pari pass.



---

archive/issue_events_079698.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2010-01-18T20:34:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6584",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6584#event-79698"
}
```



---

archive/issue_events_079699.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2010-01-18T20:34:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6584",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6584#event-79699"
}
```



---

archive/issue_events_079700.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2010-01-18T20:34:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6584",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6584#event-79700"
}
```



---

archive/issue_events_079701.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-01-19T00:56:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6584",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6584#event-79701"
}
```



---

archive/issue_events_079702.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2010-01-19T00:56:17Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6584",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6584#event-79702"
}
```



---

archive/issue_comments_045627.json:
```json
{
    "body": "Reviewer: **John Cremona**",
    "created_at": "2010-01-19T00:56:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6584",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6584#issuecomment-45627",
    "user": "https://github.com/rlmill"
}
```

Reviewer: **John Cremona**



---

archive/issue_comments_045628.json:
```json
{
    "body": "Merged: **sage-4.3.1.rc1**",
    "created_at": "2010-01-19T00:56:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6584",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6584#issuecomment-45628",
    "user": "https://github.com/rlmill"
}
```

Merged: **sage-4.3.1.rc1**
