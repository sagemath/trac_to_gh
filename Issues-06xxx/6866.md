# Issue 6866: Latex typos in orthogonal_polys.py

archive/issues_006866.json:
```json
{
    "assignees": [],
    "body": "There are some LaTeX typos in the reference manual on orthogonal\npolynomials.\n\n\nKeywords: **orthogonal polynomials**\n\nComponent: **documentation**\n\nAuthor: **H\u00e5kan Granath**\n\nReviewer: **John Palmieri**\n\nMerged: **Sage 4.1.2.alpha0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6866_\n\n",
    "closed_at": "2009-09-03T08:15:55Z",
    "created_at": "2009-09-02T15:54:21Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Latex typos in orthogonal_polys.py",
    "type": "issue",
    "updated_at": "2009-09-03T08:15:55Z",
    "url": "https://github.com/sagemath/sage/issues/6866",
    "user": "https://github.com/sagetrac-hgranath"
}
```
There are some LaTeX typos in the reference manual on orthogonal
polynomials.


Keywords: **orthogonal polynomials**

Component: **documentation**

Author: **Håkan Granath**

Reviewer: **John Palmieri**

Merged: **Sage 4.1.2.alpha0**

_Issue created by migration from https://trac.sagemath.org/ticket/6866_





---

archive/issue_events_083364.json:
```json
{
    "actor": "https://github.com/sagetrac-hgranath",
    "created_at": "2009-09-02T15:54:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6866",
    "milestone_number": null,
    "milestone_title": "sage-4.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6866#event-83364"
}
```



---

archive/issue_events_083365.json:
```json
{
    "actor": "https://github.com/sagetrac-hgranath",
    "created_at": "2009-09-02T15:54:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6866",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6866#event-83365"
}
```



---

archive/issue_events_083366.json:
```json
{
    "actor": "https://github.com/sagetrac-hgranath",
    "created_at": "2009-09-02T15:54:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6866",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6866#event-83366"
}
```



---

archive/issue_events_083367.json:
```json
{
    "actor": "https://github.com/sagetrac-hgranath",
    "created_at": "2009-09-02T15:54:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6866",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6866#event-83367"
}
```



---

archive/issue_comments_048782.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[typos_orth_poly.patch.gz](https://github.com/sagemath/sage/files/ticket6866/typos_orth_poly.patch.gz)**",
    "created_at": "2009-09-02T15:57:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6866",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6866#issuecomment-48782",
    "user": "https://github.com/sagetrac-hgranath"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[typos_orth_poly.patch.gz](https://github.com/sagemath/sage/files/ticket6866/typos_orth_poly.patch.gz)**



---

archive/issue_events_083368.json:
```json
{
    "actor": "https://github.com/sagetrac-hgranath",
    "created_at": "2009-09-02T15:57:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6866",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6866#event-83368"
}
```



---

archive/issue_comments_048783.json:
```json
{
    "body": "Author: **H\u00e5kan Granath**",
    "created_at": "2009-09-02T15:57:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6866",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6866#issuecomment-48783",
    "user": "https://github.com/sagetrac-hgranath"
}
```

Author: **Håkan Granath**



---

archive/issue_comments_048784.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nLooks good to me.",
    "created_at": "2009-09-03T04:54:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6866",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6866#issuecomment-48784",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:2" align="right">comment:2</div>

Looks good to me.



---

archive/issue_comments_048785.json:
```json
{
    "body": "Reviewer: **John Palmieri**",
    "created_at": "2009-09-03T04:54:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6866",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6866#issuecomment-48785",
    "user": "https://github.com/jhpalmieri"
}
```

Reviewer: **John Palmieri**



---

archive/issue_events_083369.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-09-03T04:54:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6866",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6866#event-83369"
}
```



---

archive/issue_events_083370.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-09-03T04:54:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6866",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6866#event-83370"
}
```



---

archive/issue_comments_048786.json:
```json
{
    "body": "Merged: **Sage 4.1.2.alpha0**",
    "created_at": "2009-09-03T08:15:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6866",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6866#issuecomment-48786",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **Sage 4.1.2.alpha0**



---

archive/issue_events_083371.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-03T08:15:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6866",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6866#event-83371"
}
```



---

archive/issue_events_083372.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-03T08:15:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6866",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6866#event-83372"
}
```
