# Issue 6866: [with patch, positive review] Latex typos in orthogonal_polys.py

archive/issues_006866.json:
```json
{
    "body": "There are some LaTeX typos in the reference manual on orthogonal\npolynomials.\n\n\nAssignee: tba\n\nKeywords: orthogonal polynomials\n\nResolution: fixed\n\nAuthor: H\u00e5kan Granath\n\nReviewer: John Palmieri\n\nMerged: Sage 4.1.2.alpha0\n\nIssue created by migration from https://trac.sagemath.org/ticket/6866\n\n",
    "closed_at": "2009-09-03T08:15:55Z",
    "created_at": "2009-09-02T15:54:21Z",
    "labels": [
        "component: documentation",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.1.2",
    "title": "[with patch, positive review] Latex typos in orthogonal_polys.py",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/6866",
    "user": "https://trac.sagemath.org/admin/accounts/users/hgranath"
}
```
There are some LaTeX typos in the reference manual on orthogonal
polynomials.


Assignee: tba

Keywords: orthogonal polynomials

Resolution: fixed

Author: Håkan Granath

Reviewer: John Palmieri

Merged: Sage 4.1.2.alpha0

Issue created by migration from https://trac.sagemath.org/ticket/6866





---

archive/issue_comments_063273.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [typos_orth_poly.patch](tarball://root/attachments/some-uuid/ticket6866/typos_orth_poly.patch) by hgranath created at 2009-09-02 15:57:08",
    "created_at": "2009-09-02T15:57:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6866",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6866#issuecomment-63273",
    "user": "https://trac.sagemath.org/admin/accounts/users/hgranath"
}
```

<a id='comment:1'></a>
Attachment [typos_orth_poly.patch](tarball://root/attachments/some-uuid/ticket6866/typos_orth_poly.patch) by hgranath created at 2009-09-02 15:57:08



---

archive/issue_events_027789.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/6866",
    "rename": {
        "from": "Latex typos in orthogonal_polys.py",
        "to": "[with patch, needs review] Latex typos in orthogonal_polys.py"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6866#event-27789"
}
```



---

archive/issue_comments_063274.json:
```json
{
    "body": "Changing author from \"\" to \"H\u00e5kan Granath\"",
    "created_at": "2009-09-02T15:57:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6866",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6866#issuecomment-63274",
    "user": "https://trac.sagemath.org/admin/accounts/users/hgranath"
}
```

Changing author from "" to "Håkan Granath"



---

archive/issue_comments_063275.json:
```json
{
    "body": "<a id='comment:2'></a>\nLooks good to me.",
    "created_at": "2009-09-03T04:54:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6866",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6866#issuecomment-63275",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:2'></a>
Looks good to me.



---

archive/issue_comments_063276.json:
```json
{
    "body": "Changing reviewer from \"\" to \"John Palmieri\"",
    "created_at": "2009-09-03T04:54:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6866",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6866#issuecomment-63276",
    "user": "https://github.com/jhpalmieri"
}
```

Changing reviewer from "" to "John Palmieri"



---

archive/issue_events_027790.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/6866",
    "rename": {
        "from": "[with patch, needs review] Latex typos in orthogonal_polys.py",
        "to": "[with patch, positive review] Latex typos in orthogonal_polys.py"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6866#event-27790"
}
```



---

archive/issue_comments_063277.json:
```json
{
    "body": "Changing merged from \"\" to \"Sage 4.1.2.alpha0\"",
    "created_at": "2009-09-03T08:15:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6866",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6866#issuecomment-63277",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Changing merged from "" to "Sage 4.1.2.alpha0"



---

archive/issue_events_027791.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-09-03T08:15:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/6866",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6866#event-27791"
}
```



---

archive/issue_comments_063278.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-09-03T08:15:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6866",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6866#issuecomment-63278",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Resolution: fixed
