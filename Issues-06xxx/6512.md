...
```json
{
    "body": "<a id='comment:1'></a>The patch removes a redundant link to jsMath.  Sphinx should now include references to jsMath in its output only if `--jsmath` is part of the command-line.  For example,\n\n`sage -docbuild reference html --jsmath`",
...
<a id='comment:1'></a>The patch removes a redundant link to jsMath.  Sphinx should now include references to jsMath in its output only if `--jsmath` is part of the command-line.  For example,
...
```json
{
    "body": "<a id='comment:4'></a>This works for me: with the patch installed, it loads jsMath when it's needed, and doesn't load it when it isn't. Positive review.",
...
<a id='comment:4'></a>This works for me: with the patch installed, it loads jsMath when it's needed, and doesn't load it when it isn't. Positive review.
...
```json
{
    "body": "<a id='comment:6'></a>Mitesh, thank you very much for fixing this.",
    "created_at": "2009-07-17T09:21:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6512",
    "type": "issue_comment",
...
<a id='comment:6'></a>Mitesh, thank you very much for fixing this.
...
<a id='comment:7'></a>I agree that the change above looks like the correct one.  Another option was to have jsmath automatically typeset the stuff in the alt parameter of the img files.  Davide Cervone sent me a message with a way to do that:
