# Issue 6916: follow-up to #6749: fix warnings when building reference manual

archive/issues_006916.json:
```json
{
    "assignees": [],
    "body": "As the subject says. This is a follow-up to ticket #6749.\n\nCC:  @nathanncohen\n\nComponent: **documentation**\n\nAuthor: **Minh Van Nguyen**\n\nReviewer: **William Stein, John Palmieri**\n\nMerged: **Sage 4.1.2.rc0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6916_\n\n",
    "closed_at": "2009-09-30T03:53:59Z",
    "created_at": "2009-09-10T11:59:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "follow-up to #6749: fix warnings when building reference manual",
    "type": "issue",
    "updated_at": "2009-09-30T03:53:59Z",
    "url": "https://github.com/sagemath/sage/issues/6916",
    "user": "https://github.com/sagetrac-mvngu"
}
```
As the subject says. This is a follow-up to ticket #6749.

CC:  @nathanncohen

Component: **documentation**

Author: **Minh Van Nguyen**

Reviewer: **William Stein, John Palmieri**

Merged: **Sage 4.1.2.rc0**

_Issue created by migration from https://trac.sagemath.org/ticket/6916_





---

archive/issue_events_083978.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-10T11:59:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6916",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "c: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6916#event-83978"
}
```



---

archive/issue_events_083979.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-10T11:59:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6916",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6916#event-83979"
}
```



---

archive/issue_events_083980.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-10T11:59:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6916",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6916#event-83980"
}
```



---

archive/issue_events_083981.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-10T17:44:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6916",
    "milestone_number": null,
    "milestone_title": "sage-4.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6916#event-83981"
}
```



---

archive/issue_events_083982.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-10T17:44:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6916",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "c: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6916#event-83982"
}
```



---

archive/issue_events_083983.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-10T17:44:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6916",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6916#event-83983"
}
```



---

archive/issue_comments_049255.json:
```json
{
    "body": "Author: **Minh Van Nguyen**",
    "created_at": "2009-09-10T17:44:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6916",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6916#issuecomment-49255",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Author: **Minh Van Nguyen**



---

archive/issue_events_083984.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-10T17:44:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6916",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6916#event-83984"
}
```



---

archive/issue_events_083985.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-29T19:10:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6916",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6916#event-83985"
}
```



---

archive/issue_events_083986.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-09-29T19:10:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6916",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6916#event-83986"
}
```



---

archive/issue_comments_049256.json:
```json
{
    "body": "Reviewer: **William Stein, John Palmieri**",
    "created_at": "2009-09-29T21:55:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6916",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6916#issuecomment-49256",
    "user": "https://github.com/jhpalmieri"
}
```

Reviewer: **William Stein, John Palmieri**



---

archive/issue_comments_049257.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nI give it a positive review, too, but if you want to change the lines\n\n```\n    - ``seq`` -- Two different possible types: \n      - A sequence of pairs (weight, value). \n      - A sequence of reals (a value of 1 is assumed). \n```\nby adding a blank line after the first one, it looks a little better (to me) when typeset.  I'll give that a positive review in advance, so if you want to make the change, no need for more review.",
    "created_at": "2009-09-29T21:55:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6916",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6916#issuecomment-49257",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:3" align="right">comment:3</div>

I give it a positive review, too, but if you want to change the lines

```
    - ``seq`` -- Two different possible types: 
      - A sequence of pairs (weight, value). 
      - A sequence of reals (a value of 1 is assumed). 
```
by adding a blank line after the first one, it looks a little better (to me) when typeset.  I'll give that a positive review in advance, so if you want to make the change, no need for more review.



---

archive/issue_comments_049258.json:
```json
{
    "body": "Attachment: **[trac_6916-typos.patch.gz](https://github.com/sagemath/sage/files/ticket6916/trac_6916-typos.patch.gz)**\n\nbased on Sage 4.1.2.alpha4",
    "created_at": "2009-09-30T02:54:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6916",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6916#issuecomment-49258",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Attachment: **[trac_6916-typos.patch.gz](https://github.com/sagemath/sage/files/ticket6916/trac_6916-typos.patch.gz)**

based on Sage 4.1.2.alpha4



---

archive/issue_comments_049259.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nReplying to [@jhpalmieri](#comment:3):\n> I give it a positive review, too, but if you want to change the lines\n\n\n```\n    - ``seq`` -- Two different possible types: \n      - A sequence of pairs (weight, value). \n      - A sequence of reals (a value of 1 is assumed). \n```\n> by adding a blank line after the first one, it looks a little better (to me) when typeset.\n\nDone.",
    "created_at": "2009-09-30T02:55:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6916",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6916#issuecomment-49259",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:4" align="right">comment:4</div>

Replying to [@jhpalmieri](#comment:3):
> I give it a positive review, too, but if you want to change the lines


```
    - ``seq`` -- Two different possible types: 
      - A sequence of pairs (weight, value). 
      - A sequence of reals (a value of 1 is assumed). 
```
> by adding a blank line after the first one, it looks a little better (to me) when typeset.

Done.



---

archive/issue_comments_049260.json:
```json
{
    "body": "Merged: **Sage 4.1.2.rc0**",
    "created_at": "2009-09-30T03:53:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6916",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6916#issuecomment-49260",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **Sage 4.1.2.rc0**



---

archive/issue_events_083987.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-30T03:53:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6916",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6916#event-83987"
}
```



---

archive/issue_events_083988.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-30T03:53:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6916",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6916#event-83988"
}
```
