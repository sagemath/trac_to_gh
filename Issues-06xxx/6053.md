# Issue 6053: Cython numpy broken in notebook

archive/issues_006053.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-jkantor"
    ],
    "body": "in a notebook cell with\n\n\n```\n%cython\ncimport numpy as np\n```\n\nan error is thrown because numpy/arrayobject.h isn't found by gcc.  No directive for the numpy include directories is part of the compiler invocation.\n\nthe header path is:\n\n$SAGE_LOCAL/lib/python2.5/site-packages/numpy/core/include/numpy/arrayobject.h\n\n\n\nKeywords: **numpy, cython, notebook**\n\nComponent: **numerical**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6053_\n\n",
    "closed_at": "2009-05-28T07:17:21Z",
    "created_at": "2009-05-17T04:44:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20numerical",
        "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Cython numpy broken in notebook",
    "type": "issue",
    "updated_at": "2009-05-28T07:17:21Z",
    "url": "https://github.com/sagemath/sage/issues/6053",
    "user": "https://github.com/sagetrac-ghtdak"
}
```
in a notebook cell with


```
%cython
cimport numpy as np
```

an error is thrown because numpy/arrayobject.h isn't found by gcc.  No directive for the numpy include directories is part of the compiler invocation.

the header path is:

$SAGE_LOCAL/lib/python2.5/site-packages/numpy/core/include/numpy/arrayobject.h



Keywords: **numpy, cython, notebook**

Component: **numerical**

_Issue created by migration from https://trac.sagemath.org/ticket/6053_





---

archive/issue_events_072592.json:
```json
{
    "actor": "https://github.com/sagetrac-ghtdak",
    "created_at": "2009-05-17T04:44:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6053",
    "milestone_number": null,
    "milestone_title": "sage-4.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6053#event-72592"
}
```



---

archive/issue_events_072593.json:
```json
{
    "actor": "https://github.com/sagetrac-ghtdak",
    "created_at": "2009-05-17T04:44:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6053",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20numerical",
    "label_color": "0000ff",
    "label_name": "c: numerical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6053#event-72593"
}
```



---

archive/issue_events_072594.json:
```json
{
    "actor": "https://github.com/sagetrac-ghtdak",
    "created_at": "2009-05-17T04:44:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6053",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6053#event-72594"
}
```



---

archive/issue_events_072595.json:
```json
{
    "actor": "https://github.com/sagetrac-ghtdak",
    "created_at": "2009-05-17T04:44:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6053",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6053#event-72595"
}
```



---

archive/issue_events_072596.json:
```json
{
    "actor": "https://github.com/sagetrac-jkantor",
    "created_at": "2009-05-17T04:44:56Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6053",
    "subject": "https://github.com/sagetrac-ghtdak",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6053#event-72596"
}
```



---

archive/issue_comments_039693.json:
```json
{
    "body": "Attachment: **[trac-6053-cython-numpy-header.patch.gz](https://github.com/sagemath/sage/files/ticket6053/trac-6053-cython-numpy-header.patch.gz)**",
    "created_at": "2009-05-28T07:09:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6053",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6053#issuecomment-39693",
    "user": "https://github.com/jasongrout"
}
```

Attachment: **[trac-6053-cython-numpy-header.patch.gz](https://github.com/sagemath/sage/files/ticket6053/trac-6053-cython-numpy-header.patch.gz)**



---

archive/issue_events_072597.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-05-28T07:10:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6053",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6053#event-72597"
}
```



---

archive/issue_comments_039694.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nLooks good to me.\n\nMerged in 4.0.rc1.",
    "created_at": "2009-05-28T07:17:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6053",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6053#issuecomment-39694",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:2" align="right">comment:2</div>

Looks good to me.

Merged in 4.0.rc1.



---

archive/issue_events_072598.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-28T07:17:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6053",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6053#event-72598"
}
```



---

archive/issue_events_072599.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-28T07:17:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6053",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6053#event-72599"
}
```



---

archive/issue_events_072600.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-28T07:17:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6053",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6053#event-72600"
}
```
