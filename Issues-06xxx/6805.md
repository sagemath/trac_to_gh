# Issue 6805: Integrality check in LatticePolytope

archive/issues_006805.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mhampton"
    ],
    "body": "It is possible now to create a lattice polytope with rational vertices, which allows things to work, but causes wrong answers:\n\n```\nsage: m = matrix([1/2, 3/2])\nsage: m\n[1/2 3/2]\nsage: LatticePolytope(m).points()\n[0 1]\n```\nThis patch adds an extra check/conversion to the constructor to prevent this:\n\n```\nsage: m = matrix([1/2, 3/2])\nsage: m\n[1/2 3/2]\nsage: LatticePolytope(m).points()\nTraceback (most recent call last):\n...\nValueError: Points must be integral!\nGiven:\n[1/2 3/2]\n```\n\nComponent: **geometry**\n\nAuthor: **Andrey Novoseltsev**\n\nReviewer: **Marshall Hampton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6805_\n\n",
    "closed_at": "2009-11-02T04:36:31Z",
    "created_at": "2009-08-22T20:25:59Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.2.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Integrality check in LatticePolytope",
    "type": "issue",
    "updated_at": "2009-11-02T04:36:31Z",
    "url": "https://github.com/sagemath/sage/issues/6805",
    "user": "https://github.com/novoselt"
}
```
It is possible now to create a lattice polytope with rational vertices, which allows things to work, but causes wrong answers:

```
sage: m = matrix([1/2, 3/2])
sage: m
[1/2 3/2]
sage: LatticePolytope(m).points()
[0 1]
```
This patch adds an extra check/conversion to the constructor to prevent this:

```
sage: m = matrix([1/2, 3/2])
sage: m
[1/2 3/2]
sage: LatticePolytope(m).points()
Traceback (most recent call last):
...
ValueError: Points must be integral!
Given:
[1/2 3/2]
```

Component: **geometry**

Author: **Andrey Novoseltsev**

Reviewer: **Marshall Hampton**

_Issue created by migration from https://trac.sagemath.org/ticket/6805_





---

archive/issue_events_082575.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2009-08-22T20:25:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6805",
    "milestone_number": null,
    "milestone_title": "sage-4.2.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6805#event-82575"
}
```



---

archive/issue_events_082576.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2009-08-22T20:25:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6805",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "c: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6805#event-82576"
}
```



---

archive/issue_events_082577.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2009-08-22T20:25:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6805",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6805#event-82577"
}
```



---

archive/issue_events_082578.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2009-08-22T20:25:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6805",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6805#event-82578"
}
```



---

archive/issue_events_082579.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2009-08-22T20:25:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6805",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6805#event-82579"
}
```



---

archive/issue_events_082580.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-08-22T20:25:59Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6805",
    "subject": "https://github.com/novoselt",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6805#event-82580"
}
```



---

archive/issue_comments_048070.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_6805_integrality_check_in_lattice_polytope.patch.gz](https://github.com/sagemath/sage/files/ticket6805/trac_6805_integrality_check_in_lattice_polytope.patch.gz)**\n\nSeems like a reasonable change, and tests out OK, so positive review.",
    "created_at": "2009-10-29T18:42:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6805",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6805#issuecomment-48070",
    "user": "https://github.com/sagetrac-mhampton"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_6805_integrality_check_in_lattice_polytope.patch.gz](https://github.com/sagemath/sage/files/ticket6805/trac_6805_integrality_check_in_lattice_polytope.patch.gz)**

Seems like a reasonable change, and tests out OK, so positive review.



---

archive/issue_events_082581.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-10-29T18:42:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6805",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6805#event-82581"
}
```



---

archive/issue_events_082582.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-10-29T18:42:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6805",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6805#event-82582"
}
```



---

archive/issue_comments_048071.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThis patch is included as a part of a rebased patch for #6831.",
    "created_at": "2009-10-30T05:35:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6805",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6805#issuecomment-48071",
    "user": "https://github.com/novoselt"
}
```

<div id="comment:2" align="right">comment:2</div>

This patch is included as a part of a rebased patch for #6831.



---

archive/issue_comments_048072.json:
```json
{
    "body": "Author: **Andrey Novoseltsev**",
    "created_at": "2009-11-02T04:36:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6805",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6805#issuecomment-48072",
    "user": "https://github.com/mwhansen"
}
```

Author: **Andrey Novoseltsev**



---

archive/issue_comments_048073.json:
```json
{
    "body": "Reviewer: **Marshall Hampton**",
    "created_at": "2009-11-02T04:36:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6805",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6805#issuecomment-48073",
    "user": "https://github.com/mwhansen"
}
```

Reviewer: **Marshall Hampton**



---

archive/issue_events_082583.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-02T04:36:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6805",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6805#event-82583"
}
```



---

archive/issue_events_082584.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-11-02T04:36:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6805",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6805#event-82584"
}
```



---

archive/issue_comments_048074.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nFixed in #6831.",
    "created_at": "2009-11-02T04:36:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6805",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6805#issuecomment-48074",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:3" align="right">comment:3</div>

Fixed in #6831.
