# Issue 6548: Improve tasteful ticks documentation

archive/issues_006548.json:
```json
{
    "assignees": [],
    "body": "Improve tasteful ticks documentation in plot/axes.py.  See #5649.\n\nCC:  wcauchois mvngu\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6548_\n\n",
    "closed_at": "2009-09-29T16:04:41Z",
    "created_at": "2009-07-17T12:58:24Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Improve tasteful ticks documentation",
    "type": "issue",
    "updated_at": "2009-09-29T16:04:41Z",
    "url": "https://github.com/sagemath/sage/issues/6548",
    "user": "https://github.com/kcrisman"
}
```
Improve tasteful ticks documentation in plot/axes.py.  See #5649.

CC:  wcauchois mvngu

_Issue created by migration from https://trac.sagemath.org/ticket/6548_





---

archive/issue_events_069505.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-07-17T12:58:24Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6548",
    "milestone_number": null,
    "milestone_title": "sage-4.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6548#event-69505"
}
```



---

archive/issue_events_069506.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-07-17T12:58:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6548",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6548#event-69506"
}
```



---

archive/issue_events_069507.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-07-17T12:58:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6548",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6548#event-69507"
}
```



---

archive/issue_events_069508.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-07-17T12:58:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6548",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6548#event-69508"
}
```



---

archive/issue_comments_046709.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-Improve tasteful ticks documentation in plot/axes.py.  See #5469.\n+Improve tasteful ticks documentation in plot/axes.py.  See #5649.\n``````\n",
    "created_at": "2009-07-22T02:53:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6548",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6548#issuecomment-46709",
    "user": "https://github.com/jhpalmieri"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-Improve tasteful ticks documentation in plot/axes.py.  See #5469.
+Improve tasteful ticks documentation in plot/axes.py.  See #5649.
``````




---

archive/issue_comments_046710.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nTo release manager: this should be closed now that #5448 is merged and tasteful and tasteless ticks are deprecated.",
    "created_at": "2009-09-15T17:38:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6548",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6548#issuecomment-46710",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:2'>Comment 2:</a>
To release manager: this should be closed now that #5448 is merged and tasteful and tasteless ticks are deprecated.



---

archive/issue_comments_046711.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThis should be closed due to #5448.",
    "created_at": "2009-09-29T15:58:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6548",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6548#issuecomment-46711",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:3'>Comment 3:</a>
This should be closed due to #5448.



---

archive/issue_comments_046712.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nThe module `sage/plot/axes.py` is being deprecated and its functionality has been implemented in matplotlib. Closing this ticket as fixed by #5448.",
    "created_at": "2009-09-29T16:04:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6548",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6548#issuecomment-46712",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<a id='comment:4'>Comment 4:</a>
The module `sage/plot/axes.py` is being deprecated and its functionality has been implemented in matplotlib. Closing this ticket as fixed by #5448.



---

archive/issue_events_069509.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-29T16:04:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6548",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6548#event-69509"
}
```
