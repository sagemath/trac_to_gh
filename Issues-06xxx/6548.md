# Issue 6548: Improve tasteful ticks documentation

archive/issues_006548.json:
```json
{
    "assignees": [],
    "body": "Improve tasteful ticks documentation in plot/axes.py.  See #5649.\n\nCC:  @sagetrac-wcauchois @sagetrac-mvngu\n\nComponent: **documentation**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6548_\n\n",
    "closed_at": "2009-09-29T16:04:41Z",
    "created_at": "2009-07-17T12:58:24Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Improve tasteful ticks documentation",
    "type": "issue",
    "updated_at": "2009-09-29T16:04:41Z",
    "url": "https://github.com/sagemath/sage/issues/6548",
    "user": "https://github.com/kcrisman"
}
```
Improve tasteful ticks documentation in plot/axes.py.  See #5649.

CC:  @sagetrac-wcauchois @sagetrac-mvngu

Component: **documentation**

_Issue created by migration from https://trac.sagemath.org/ticket/6548_





---

archive/issue_events_079487.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-07-17T12:58:24Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6548",
    "milestone_number": null,
    "milestone_title": "sage-4.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6548#event-79487"
}
```



---

archive/issue_events_079488.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-07-17T12:58:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6548",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6548#event-79488"
}
```



---

archive/issue_events_079489.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-07-17T12:58:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6548",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6548#event-79489"
}
```



---

archive/issue_events_079490.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-07-17T12:58:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6548",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6548#event-79490"
}
```



---

archive/issue_comments_045193.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-Improve tasteful ticks documentation in plot/axes.py.  See #5469.\n+Improve tasteful ticks documentation in plot/axes.py.  See #5649.\n``````\n",
    "created_at": "2009-07-22T02:53:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6548",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6548#issuecomment-45193",
    "user": "https://github.com/jhpalmieri"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-Improve tasteful ticks documentation in plot/axes.py.  See #5469.
+Improve tasteful ticks documentation in plot/axes.py.  See #5649.
``````




---

archive/issue_comments_045194.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nTo release manager: this should be closed now that #5448 is merged and tasteful and tasteless ticks are deprecated.",
    "created_at": "2009-09-15T17:38:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6548",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6548#issuecomment-45194",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:2" align="right">comment:2</div>

To release manager: this should be closed now that #5448 is merged and tasteful and tasteless ticks are deprecated.



---

archive/issue_comments_045195.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nThis should be closed due to #5448.",
    "created_at": "2009-09-29T15:58:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6548",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6548#issuecomment-45195",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:3" align="right">comment:3</div>

This should be closed due to #5448.



---

archive/issue_comments_045196.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nThe module `sage/plot/axes.py` is being deprecated and its functionality has been implemented in matplotlib. Closing this ticket as fixed by #5448.",
    "created_at": "2009-09-29T16:04:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6548",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6548#issuecomment-45196",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:4" align="right">comment:4</div>

The module `sage/plot/axes.py` is being deprecated and its functionality has been implemented in matplotlib. Closing this ticket as fixed by #5448.



---

archive/issue_events_079491.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-29T16:04:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6548",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6548#event-79491"
}
```
