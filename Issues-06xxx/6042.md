...
<a id='comment:1'></a>Attachment [trac_6042.patch](tarball://root/attachments/some-uuid/ticket6042/trac_6042.patch) by @JohnCremona created at 2009-05-16 15:39:20
...
```json
{
    "body": "<a id='comment:4'></a>I have started but haven't gotten very far yet, so I won't be upset if you want to do it.  Most of the stuff is fairly straightforward, but there are some functions where you introduced documentation and examples, and then John's patch is doing things all over again.  So that would best be done by one of you.",
...
<a id='comment:4'></a>I have started but haven't gotten very far yet, so I won't be upset if you want to do it.  Most of the stuff is fairly straightforward, but there are some functions where you introduced documentation and examples, and then John's patch is doing things all over again.  So that would best be done by one of you.
...
```json
{
    "body": "<a id='comment:5'></a>I've rebased the patch to 4.0.alpha0 and checked that it passes doctests, and had a look through the code, and it all looks fine -- let's get this in without further delay!",
...
<a id='comment:5'></a>I've rebased the patch to 4.0.alpha0 and checked that it passes doctests, and had a look through the code, and it all looks fine -- let's get this in without further delay!
...
```json
{
    "body": "<a id='comment:6'></a>Thanks David (and Alex)!",
...
<a id='comment:6'></a>Thanks David (and Alex)!
...
```json
{
    "body": "<a id='comment:7'></a>BTW, I see that \"rebasing\" has included \"reattributing credit in the patch header\"! :)",
...
<a id='comment:7'></a>BTW, I see that "rebasing" has included "reattributing credit in the patch header"! :)
...
```json
{
    "body": "<a id='comment:10'></a>I just found out about \"qrefresh -u\", so I can masquerade as anybody I like. I'll now attribute any patches that don't work to some unsuspecting victim :-) I've uploaded a new version with credit correctly attributed to John.",
...
<a id='comment:10'></a>I just found out about "qrefresh -u", so I can masquerade as anybody I like. I'll now attribute any patches that don't work to some unsuspecting victim :-) I've uploaded a new version with credit correctly attributed to John.
...
<a id='comment:12'></a>The rebased patch on top of two other trivial tickets in my 4.0.rc0 merge tree:
...
```json
{
    "body": "<a id='comment:13'></a>That makes it all worth while!",
...
<a id='comment:13'></a>That makes it all worth while!
