# Issue 6446: memory issue from #6394

archive/issues_006446.json:
```json
{
    "assignees": [],
    "body": "\n\nComponent: **memleak**\n\nAuthor: **Robert Miller**\n\nReviewer: **John Cremona**\n\nMerged: **sage-4.1.alpha3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6446_\n\n",
    "closed_at": "2009-06-29T20:43:05Z",
    "created_at": "2009-06-29T17:29:46Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/memleak",
        "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "memory issue from #6394",
    "type": "issue",
    "updated_at": "2009-06-29T20:43:05Z",
    "url": "https://github.com/sagemath/sage/issues/6446",
    "user": "https://github.com/rlmill"
}
```


Component: **memleak**

Author: **Robert Miller**

Reviewer: **John Cremona**

Merged: **sage-4.1.alpha3**

_Issue created by migration from https://trac.sagemath.org/ticket/6446_





---

archive/issue_events_077703.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-06-29T17:29:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6446",
    "milestone_number": null,
    "milestone_title": "sage-4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6446#event-77703"
}
```



---

archive/issue_events_077704.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-06-29T17:29:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6446",
    "label": "https://github.com/sagemath/sage/labels/memleak",
    "label_color": "696969",
    "label_name": "memleak",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6446#event-77704"
}
```



---

archive/issue_events_077705.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-06-29T17:29:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6446",
    "label": "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p: 1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6446#event-77705"
}
```



---

archive/issue_events_077706.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-06-29T17:29:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6446",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6446#event-77706"
}
```



---

archive/issue_comments_043517.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_6446-memleak_coarsest_eq_refn.patch.gz](https://github.com/sagemath/sage/files/ticket6446/trac_6446-memleak_coarsest_eq_refn.patch.gz)**",
    "created_at": "2009-06-29T17:38:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6446",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6446#issuecomment-43517",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_6446-memleak_coarsest_eq_refn.patch.gz](https://github.com/sagemath/sage/files/ticket6446/trac_6446-memleak_coarsest_eq_refn.patch.gz)**



---

archive/issue_events_077707.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-06-29T17:38:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6446",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6446#event-77707"
}
```



---

archive/issue_comments_043518.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nI'm testing this now....",
    "created_at": "2009-06-29T18:16:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6446",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6446#issuecomment-43518",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:2" align="right">comment:2</div>

I'm testing this now....



---

archive/issue_events_077708.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-06-29T18:23:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6446",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6446#event-77708"
}
```



---

archive/issue_events_077709.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2009-06-29T18:23:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6446",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6446#event-77709"
}
```



---

archive/issue_comments_043519.json:
```json
{
    "body": "Reviewer: **John Cremona**",
    "created_at": "2009-06-29T18:23:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6446",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6446#issuecomment-43519",
    "user": "https://github.com/JohnCremona"
}
```

Reviewer: **John Cremona**



---

archive/issue_comments_043520.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nTesting on the same machine & build where I reported the problem (ububtu 32-bit):  I applied the patch and now there is no problem with testing graphs/graph.py, or sage/groups/perm_gps/partn_ref/refinement_graphs.pyx (the file patched).\n\nSo: positive reivew!",
    "created_at": "2009-06-29T18:23:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6446",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6446#issuecomment-43520",
    "user": "https://github.com/JohnCremona"
}
```

<div id="comment:3" align="right">comment:3</div>

Testing on the same machine & build where I reported the problem (ububtu 32-bit):  I applied the patch and now there is no problem with testing graphs/graph.py, or sage/groups/perm_gps/partn_ref/refinement_graphs.pyx (the file patched).

So: positive reivew!



---

archive/issue_events_077710.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-06-29T20:43:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6446",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6446#event-77710"
}
```



---

archive/issue_events_077711.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2009-06-29T20:43:05Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6446",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6446#event-77711"
}
```



---

archive/issue_comments_043521.json:
```json
{
    "body": "Merged: **sage-4.1.alpha3**",
    "created_at": "2009-06-29T20:43:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6446",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6446#issuecomment-43521",
    "user": "https://github.com/rlmill"
}
```

Merged: **sage-4.1.alpha3**
