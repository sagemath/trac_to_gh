# Issue 6067: Published worksheet URL stability

archive/issues_006067.json:
```json
{
    "assignees": [],
    "body": "If a the publication status of a worksheet is selected from published to unpublished to published, the URL changes (the number is incremented to the next available). This causes grief if the URL has been given to anyone. It would be much friendlier if the URL remained stable once it had be defined, regardless of any changes in publication status.\n\n**CC:**  @embray @jdemeyer @kiwifb\n\n**Reviewer:** Fran\u00e7ois Bissey\n\nIssue created by migration from https://trac.sagemath.org/ticket/6067\n\n",
    "closed_at": "2019-06-15T12:40:53Z",
    "created_at": "2009-05-18T13:07:14Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20notebook",
        "https://github.com/sagemath/sage/labels/minor",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Published worksheet URL stability",
    "type": "issue",
    "updated_at": "2019-06-15T12:40:53Z",
    "url": "https://github.com/sagemath/sage/issues/6067",
    "user": "https://github.com/sagetrac-khorton"
}
```
If a the publication status of a worksheet is selected from published to unpublished to published, the URL changes (the number is incremented to the next available). This causes grief if the URL has been given to anyone. It would be much friendlier if the URL remained stable once it had be defined, regardless of any changes in publication status.

**CC:**  @embray @jdemeyer @kiwifb

**Reviewer:** François Bissey

Issue created by migration from https://trac.sagemath.org/ticket/6067





---

archive/issue_comments_041131.json:
```json
{
    "body": "<a id='comment:1'></a>\nlegacy sagenb is deprecated, so this can be closed",
    "created_at": "2019-06-15T07:20:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6067",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6067#issuecomment-41131",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:1'></a>
legacy sagenb is deprecated, so this can be closed



---

archive/issue_events_039259.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-06-15T07:20:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6067",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "008080",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6067#event-39259"
}
```



---

archive/issue_comments_041132.json:
```json
{
    "body": "<a id='comment:2'></a>\nSure.",
    "created_at": "2019-06-15T07:42:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6067",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6067#issuecomment-41132",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:2'></a>
Sure.



---

archive/issue_comments_041133.json:
```json
{
    "body": "**Reviewer:** Fran\u00e7ois Bissey",
    "created_at": "2019-06-15T07:42:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6067",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6067#issuecomment-41133",
    "user": "https://github.com/kiwifb"
}
```

**Reviewer:** François Bissey



---

archive/issue_events_039260.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2019-06-15T07:42:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6067",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "008080",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6067#event-39260"
}
```



---

archive/issue_events_039261.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2019-06-15T07:42:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6067",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "008080",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6067#event-39261"
}
```



---

archive/issue_comments_041134.json:
```json
{
    "body": "<a id='comment:3'></a>\nclose 3 old invalid tickets",
    "created_at": "2019-06-15T12:40:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6067",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6067#issuecomment-41134",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:3'></a>
close 3 old invalid tickets



---

archive/issue_events_039262.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-06-15T12:40:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6067",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "008080",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6067#event-39262"
}
```



---

archive/issue_events_039263.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-06-15T12:40:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6067",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6067#event-39263"
}
```
