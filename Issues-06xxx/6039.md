# Issue 6039: Change name of pari's sum function when imported

archive/issues_006039.json:
```json
{
    "assignees": [
        "https://github.com/craigcitro"
    ],
    "body": "When we include Pari's `sum` function via `libs/pari/decl.pxi`, it takes precedence over the default Python one. This causes some rather confusing compiler errors -- see, e.g., this thread:\n\nhttp://groups.google.com/group/sage-devel/browse_thread/thread/68a7bd7e99ef22e6#\n\nThe attached patch uses string replace magic to rename it to `pari_sum`. \n\nComponent: **interfaces**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6039_\n\n",
    "closed_at": "2009-05-14T17:33:03Z",
    "created_at": "2009-05-14T16:49:37Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20interfaces",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Change name of pari's sum function when imported",
    "type": "issue",
    "updated_at": "2009-05-14T17:33:03Z",
    "url": "https://github.com/sagemath/sage/issues/6039",
    "user": "https://github.com/craigcitro"
}
```
When we include Pari's `sum` function via `libs/pari/decl.pxi`, it takes precedence over the default Python one. This causes some rather confusing compiler errors -- see, e.g., this thread:

http://groups.google.com/group/sage-devel/browse_thread/thread/68a7bd7e99ef22e6#

The attached patch uses string replace magic to rename it to `pari_sum`. 

Component: **interfaces**

_Issue created by migration from https://trac.sagemath.org/ticket/6039_





---

archive/issue_events_072418.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2009-05-14T16:49:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6039",
    "milestone_number": null,
    "milestone_title": "sage-4.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6039#event-72418"
}
```



---

archive/issue_events_072419.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2009-05-14T16:49:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6039",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "c: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6039#event-72419"
}
```



---

archive/issue_events_072420.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2009-05-14T16:49:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6039",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6039#event-72420"
}
```



---

archive/issue_events_072421.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2009-05-14T16:49:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6039",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6039#event-72421"
}
```



---

archive/issue_events_072422.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2009-05-14T16:49:37Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6039",
    "subject": "https://github.com/craigcitro",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6039#event-72422"
}
```



---

archive/issue_events_072423.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2009-05-14T16:49:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6039",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6039#event-72423"
}
```



---

archive/issue_comments_039544.json:
```json
{
    "body": "Attachment: **[trac-6039.patch.gz](https://github.com/sagemath/sage/files/ticket6039/trac-6039.patch.gz)**",
    "created_at": "2009-05-14T16:50:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6039",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6039#issuecomment-39544",
    "user": "https://github.com/craigcitro"
}
```

Attachment: **[trac-6039.patch.gz](https://github.com/sagemath/sage/files/ticket6039/trac-6039.patch.gz)**



---

archive/issue_comments_039545.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nIt is never a good idea to touch sage/libs/pari/decl.pxi :). Positive review.\n\nCheers,\n\nMichael",
    "created_at": "2009-05-14T17:15:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6039",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6039#issuecomment-39545",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">comment:1</div>

It is never a good idea to touch sage/libs/pari/decl.pxi :). Positive review.

Cheers,

Michael



---

archive/issue_events_072424.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-14T17:15:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6039",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6039#event-72424"
}
```



---

archive/issue_comments_039546.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nMerged in Sage 4.0.alpha0.\n\nCheers,\n\nMichael",
    "created_at": "2009-05-14T17:33:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6039",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6039#issuecomment-39546",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Merged in Sage 4.0.alpha0.

Cheers,

Michael



---

archive/issue_events_072425.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-14T17:33:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6039",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6039#event-72425"
}
```



---

archive/issue_events_072426.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-05-14T17:33:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6039",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6039#event-72426"
}
```
