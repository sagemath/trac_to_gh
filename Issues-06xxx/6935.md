# Issue 6935: Improve documentation for plots with new axis code

archive/issues_006935.json:
```json
{
    "assignees": [],
    "body": "There are a few issues that are left over from #5448, which at the very least should be documented.  None of them are major.\n\nContour plot - if fill=False and contours are grayscale, the axes could be misinterpreted\n\nContour plot - show(axes=False) and show(axes=True) seem to be identical on the last example\n\nPlotting - how well documented is the new axis behavior, where it does NOT intersect? This should be clear, e.g. the Riemann zeta example in plot.py looks funny, until you realize it's from 1 to 27. It still seems weird to me when it's that close, but I suppose it's okay as long as it is very very clear in documentation.\n\nAxis labels - should point out difference between ['x','y'] and ['$x$','$y$']. Some people might not like the LaTeXed? version\n\nWhen scientific notation comes into play is not always clear, and should be in the documentation - compare plot(x**2, 490,500) and plot(x**2,-490,500), which have the same \"height\" but only one gets e, presumably since it covers a larger range\n\nAssignee: **@williamstein**\n\nCC:  @jasongrout\n\nAuthor: **Karl-Dieter Crisman**\n\nReviewer: **Jason Grout**\n\nMerged: **sage-4.2.alpha1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6935_\n\n",
    "closed_at": "2009-10-19T05:56:45Z",
    "created_at": "2009-09-15T17:39:04Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graphics",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Improve documentation for plots with new axis code",
    "type": "issue",
    "updated_at": "2009-10-19T05:56:45Z",
    "url": "https://github.com/sagemath/sage/issues/6935",
    "user": "https://github.com/kcrisman"
}
```
There are a few issues that are left over from #5448, which at the very least should be documented.  None of them are major.

Contour plot - if fill=False and contours are grayscale, the axes could be misinterpreted

Contour plot - show(axes=False) and show(axes=True) seem to be identical on the last example

Plotting - how well documented is the new axis behavior, where it does NOT intersect? This should be clear, e.g. the Riemann zeta example in plot.py looks funny, until you realize it's from 1 to 27. It still seems weird to me when it's that close, but I suppose it's okay as long as it is very very clear in documentation.

Axis labels - should point out difference between ['x','y'] and ['$x$','$y$']. Some people might not like the LaTeXed? version

When scientific notation comes into play is not always clear, and should be in the documentation - compare plot(x**2, 490,500) and plot(x**2,-490,500), which have the same "height" but only one gets e, presumably since it covers a larger range

Assignee: **@williamstein**

CC:  @jasongrout

Author: **Karl-Dieter Crisman**

Reviewer: **Jason Grout**

Merged: **sage-4.2.alpha1**

_Issue created by migration from https://trac.sagemath.org/ticket/6935_





---

archive/issue_events_074087.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-09-15T17:39:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6935",
    "milestone_number": null,
    "milestone_title": "sage-4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6935#event-74087"
}
```



---

archive/issue_events_074088.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-09-15T17:39:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6935",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graphics",
    "label_color": "0000ff",
    "label_name": "component: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6935#event-74088"
}
```



---

archive/issue_events_074089.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-09-15T17:39:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6935",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6935#event-74089"
}
```



---

archive/issue_events_074090.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-09-15T17:39:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6935",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6935#event-74090"
}
```



---

archive/issue_events_074091.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-09-15T17:39:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6935",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6935#event-74091"
}
```



---

archive/issue_comments_051037.json:
```json
{
    "body": "Author: **Karl-Dieter Crisman**",
    "created_at": "2009-10-05T18:05:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6935#issuecomment-51037",
    "user": "https://github.com/kcrisman"
}
```

Author: **Karl-Dieter Crisman**



---

archive/issue_comments_051038.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nThe first issue is dealt with in #6996, but needed a bit more documentation.  The second issue turned out not to be an issue.\n\nOtherwise documentation has been updated for all these things as appropriate.",
    "created_at": "2009-10-05T18:05:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6935#issuecomment-51038",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:1'>Comment 1:</a>
The first issue is dealt with in #6996, but needed a bit more documentation.  The second issue turned out not to be an issue.

Otherwise documentation has been updated for all these things as appropriate.



---

archive/issue_comments_051039.json:
```json
{
    "body": "Attachment: **[trac_6935-mpl-upgrade-documentation.patch.gz](https://github.com/sagemath/sage/files/ticket6935/trac_6935-mpl-upgrade-documentation.patch.gz)**\n\nBased on 4.1.2.alpha4",
    "created_at": "2009-10-05T18:05:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6935#issuecomment-51039",
    "user": "https://github.com/kcrisman"
}
```

Attachment: **[trac_6935-mpl-upgrade-documentation.patch.gz](https://github.com/sagemath/sage/files/ticket6935/trac_6935-mpl-upgrade-documentation.patch.gz)**

Based on 4.1.2.alpha4



---

archive/issue_events_074092.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-10-06T20:46:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6935",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6935#event-74092"
}
```



---

archive/issue_events_074093.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-10-06T20:46:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6935",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6935#event-74093"
}
```



---

archive/issue_comments_051040.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nLooks great!",
    "created_at": "2009-10-06T20:46:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6935#issuecomment-51040",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:2'>Comment 2:</a>
Looks great!



---

archive/issue_events_074094.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-10-06T20:49:55Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/6935",
    "milestone_number": null,
    "milestone_title": "sage-4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6935#event-74094"
}
```



---

archive/issue_events_074095.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-10-06T20:49:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6935",
    "milestone_number": null,
    "milestone_title": "sage-4.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6935#event-74095"
}
```



---

archive/issue_comments_051041.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nWhen we merged #6996, it was understood that this documentation should make it into the same release so that people wouldn't be really confused by the new axes behavior.",
    "created_at": "2009-10-06T20:49:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6935#issuecomment-51041",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:3'>Comment 3:</a>
When we merged #6996, it was understood that this documentation should make it into the same release so that people wouldn't be really confused by the new axes behavior.



---

archive/issue_comments_051042.json:
```json
{
    "body": "Merged: **sage-4.2.alpha1**",
    "created_at": "2009-10-19T05:56:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6935#issuecomment-51042",
    "user": "https://github.com/mwhansen"
}
```

Merged: **sage-4.2.alpha1**



---

archive/issue_comments_051043.json:
```json
{
    "body": "Reviewer: **Jason Grout**",
    "created_at": "2009-10-19T05:56:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6935#issuecomment-51043",
    "user": "https://github.com/mwhansen"
}
```

Reviewer: **Jason Grout**



---

archive/issue_events_074096.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-19T05:56:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6935",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6935#event-74096"
}
```



---

archive/issue_events_074097.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-19T05:56:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6935",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6935#event-74097"
}
```
