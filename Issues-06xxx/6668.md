# Issue 6668: [with patch, no longer relevant] Allow doctesting of .tex, .rst. .sage files outside the devel tree

archive/issues_006668.json:
```json
{
    "body": "Since 3.4.1 it has not been possible to doctest .tex, .rst or .sage files which were not in the SAGE_ROOT/devel tree.  The patch (for the scripts repository) fixes this, and a few other things are tidied up.\n\n\nAssignee: tbd\n\nKeywords: sage-doctest\n\nResolution: fixed\n\nAuthor: Francis Clarke\n\nIssue created by migration from https://trac.sagemath.org/ticket/6668\n\n",
    "closed_at": "2009-10-23T16:05:59Z",
    "created_at": "2009-08-03T19:06:43Z",
    "labels": [
        "component: doctest coverage",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.1.2",
    "title": "[with patch, no longer relevant] Allow doctesting of .tex, .rst. .sage files outside the devel tree",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/6668",
    "user": "https://trac.sagemath.org/admin/accounts/users/fwclarke"
}
```
Since 3.4.1 it has not been possible to doctest .tex, .rst or .sage files which were not in the SAGE_ROOT/devel tree.  The patch (for the scripts repository) fixes this, and a few other things are tidied up.


Assignee: tbd

Keywords: sage-doctest

Resolution: fixed

Author: Francis Clarke

Issue created by migration from https://trac.sagemath.org/ticket/6668





---

archive/issue_comments_060696.json:
```json
{
    "body": "apply to scripts repository in sage-4.1.1.rc0",
    "created_at": "2009-08-03T19:08:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6668",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6668#issuecomment-60696",
    "user": "https://trac.sagemath.org/admin/accounts/users/fwclarke"
}
```

apply to scripts repository in sage-4.1.1.rc0



---

archive/issue_comments_060697.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [trac_6668_scripts_doctest.patch](tarball://root/attachments/some-uuid/ticket6668/trac_6668_scripts_doctest.patch) by @aghitza created at 2009-08-03 23:32:17",
    "created_at": "2009-08-03T23:32:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6668",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6668#issuecomment-60697",
    "user": "https://github.com/aghitza"
}
```

<a id='comment:1'></a>
Attachment [trac_6668_scripts_doctest.patch](tarball://root/attachments/some-uuid/ticket6668/trac_6668_scripts_doctest.patch) by @aghitza created at 2009-08-03 23:32:17



---

archive/issue_events_026971.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/6668",
    "rename": {
        "from": "Allow doctesting of .tex, .rst. .sage files outside the devel tree",
        "to": "[with patch, needs review] Allow doctesting of .tex, .rst. .sage files outside the devel tree"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6668#event-26971"
}
```



---

archive/issue_comments_060698.json:
```json
{
    "body": "<a id='comment:2'></a>\nA new patch to has been attached to #6861 which includes the changes proposed here as well as others addressing related issues.  If that is postively reviewed, then this ticket can be closed.",
    "created_at": "2009-09-05T22:50:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6668",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6668#issuecomment-60698",
    "user": "https://trac.sagemath.org/admin/accounts/users/fwclarke"
}
```

<a id='comment:2'></a>
A new patch to has been attached to #6861 which includes the changes proposed here as well as others addressing related issues.  If that is postively reviewed, then this ticket can be closed.



---

archive/issue_comments_060699.json:
```json
{
    "body": "<a id='comment:3'></a>\nThis ticket could now be closed, since the proposed changes have been incorporated in #6861.",
    "created_at": "2009-09-24T19:59:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6668",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6668#issuecomment-60699",
    "user": "https://trac.sagemath.org/admin/accounts/users/fwclarke"
}
```

<a id='comment:3'></a>
This ticket could now be closed, since the proposed changes have been incorporated in #6861.



---

archive/issue_events_026972.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/6668",
    "rename": {
        "from": "[with patch, needs review] Allow doctesting of .tex, .rst. .sage files outside the devel tree",
        "to": "[with patch, no longer relevant] Allow doctesting of .tex, .rst. .sage files outside the devel tree"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6668#event-26972"
}
```



---

archive/issue_comments_060700.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-10-23T16:05:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6668",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6668#issuecomment-60700",
    "user": "https://github.com/mwhansen"
}
```

Resolution: fixed



---

archive/issue_events_026973.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-23T16:05:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/6668",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6668#event-26973"
}
```



---

archive/issue_events_026974.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-23T16:05:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6668",
    "milestone": "sage-4.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6668#event-26974"
}
```
