# Issue 6668: [with patch, no longer relevant] Allow doctesting of .tex, .rst. .sage files outside the devel tree

archive/issues_006668.json:
```json
{
    "body": "Assignee: tbd\n\nKeywords: sage-doctest\n\nSince 3.4.1 it has not been possible to doctest .tex, .rst or .sage files which were not in the SAGE_ROOT/devel tree.  The patch (for the scripts repository) fixes this, and a few other things are tidied up.\n\n\nIssue created by migration from https://trac.sagemath.org/ticket/6668\n\n",
    "closed_at": "2009-10-23T16:05:59Z",
    "created_at": "2009-08-03T19:06:43Z",
    "labels": [
        "component: doctest coverage",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.1.2",
    "title": "[with patch, no longer relevant] Allow doctesting of .tex, .rst. .sage files outside the devel tree",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/6668",
    "user": "https://trac.sagemath.org/admin/accounts/users/fwclarke"
}
```
Assignee: tbd

Keywords: sage-doctest

Since 3.4.1 it has not been possible to doctest .tex, .rst or .sage files which were not in the SAGE_ROOT/devel tree.  The patch (for the scripts repository) fixes this, and a few other things are tidied up.


Issue created by migration from https://trac.sagemath.org/ticket/6668





---

archive/issue_comments_054648.json:
```json
{
    "body": "apply to scripts repository in sage-4.1.1.rc0",
    "created_at": "2009-08-03T19:08:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6668",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6668#issuecomment-54648",
    "user": "https://trac.sagemath.org/admin/accounts/users/fwclarke"
}
```

apply to scripts repository in sage-4.1.1.rc0



---

archive/issue_comments_054649.json:
```json
{
    "body": "Attachment [trac_6668_scripts_doctest.patch](tarball://root/attachments/some-uuid/ticket6668/trac_6668_scripts_doctest.patch) by @aghitza created at 2009-08-03 23:32:17",
    "created_at": "2009-08-03T23:32:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6668",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6668#issuecomment-54649",
    "user": "https://github.com/aghitza"
}
```

Attachment [trac_6668_scripts_doctest.patch](tarball://root/attachments/some-uuid/ticket6668/trac_6668_scripts_doctest.patch) by @aghitza created at 2009-08-03 23:32:17



---

archive/issue_comments_054650.json:
```json
{
    "body": "A new patch to has been attached to #6861 which includes the changes proposed here as well as others addressing related issues.  If that is postively reviewed, then this ticket can be closed.",
    "created_at": "2009-09-05T22:50:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6668",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6668#issuecomment-54650",
    "user": "https://trac.sagemath.org/admin/accounts/users/fwclarke"
}
```

A new patch to has been attached to #6861 which includes the changes proposed here as well as others addressing related issues.  If that is postively reviewed, then this ticket can be closed.



---

archive/issue_comments_054651.json:
```json
{
    "body": "This ticket could now be closed, since the proposed changes have been incorporated in #6861.",
    "created_at": "2009-09-24T19:59:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6668",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6668#issuecomment-54651",
    "user": "https://trac.sagemath.org/admin/accounts/users/fwclarke"
}
```

This ticket could now be closed, since the proposed changes have been incorporated in #6861.



---

archive/issue_comments_054652.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-10-23T16:05:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6668",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6668#issuecomment-54652",
    "user": "https://github.com/mwhansen"
}
```

Resolution: fixed



---

archive/issue_events_015737.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-23T16:05:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/6668",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6668#event-15737"
}
```



---

archive/issue_events_015738.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-10-23T16:05:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6668",
    "milestone": "sage-4.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6668#event-15738"
}
```
