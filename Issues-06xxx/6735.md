# Issue 6735: [with patch, positive review] spell-check all modules under sage/structure

archive/issues_006735.json:
```json
{
    "body": "As the subject says.\n\nAssignee: tba\n\nResolution: fixed\n\nAuthor: Minh Van Nguyen\n\nReviewer: Marshall Hampton\n\nMerged: Sage 4.1.1\n\nIssue created by migration from https://trac.sagemath.org/ticket/6735\n\n",
    "closed_at": "2009-08-12T06:26:22Z",
    "created_at": "2009-08-11T10:08:04Z",
    "labels": [
        "component: documentation",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.1.1",
    "title": "[with patch, positive review] spell-check all modules under sage/structure",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/6735",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```
As the subject says.

Assignee: tba

Resolution: fixed

Author: Minh Van Nguyen

Reviewer: Marshall Hampton

Merged: Sage 4.1.1

Issue created by migration from https://trac.sagemath.org/ticket/6735





---

archive/issue_comments_061346.json:
```json
{
    "body": "Changing author from \"\" to \"Minh Van Nguyen\"",
    "created_at": "2009-08-11T10:10:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6735",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6735#issuecomment-61346",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Changing author from "" to "Minh Van Nguyen"



---

archive/issue_events_027246.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/6735",
    "rename": {
        "from": "spell-check all modules under sage/structure",
        "to": "[with patch, needs review] spell-check all modules under sage/structure"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6735#event-27246"
}
```



---

archive/issue_comments_061347.json:
```json
{
    "body": "based on Sage 4.1.1.rc2",
    "created_at": "2009-08-11T17:33:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6735",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6735#issuecomment-61347",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

based on Sage 4.1.1.rc2



---

archive/issue_comments_061348.json:
```json
{
    "body": "<a id='comment:2'></a>\nAttachment [trac_6735-spell-check-structure.patch](tarball://root/attachments/some-uuid/ticket6735/trac_6735-spell-check-structure.patch) by mhampton created at 2009-08-11 20:21:44\n\nAll these changes look good; as discussed on sage-devel, \"Pyrex\" should be changed to \"Cython\" but that is done on another ticket.  I tested the patch just to be safe, but as expected these changes don't cause any doctesting problems.",
    "created_at": "2009-08-11T20:21:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6735",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6735#issuecomment-61348",
    "user": "https://trac.sagemath.org/admin/accounts/users/mhampton"
}
```

<a id='comment:2'></a>
Attachment [trac_6735-spell-check-structure.patch](tarball://root/attachments/some-uuid/ticket6735/trac_6735-spell-check-structure.patch) by mhampton created at 2009-08-11 20:21:44

All these changes look good; as discussed on sage-devel, "Pyrex" should be changed to "Cython" but that is done on another ticket.  I tested the patch just to be safe, but as expected these changes don't cause any doctesting problems.



---

archive/issue_events_027247.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/6735",
    "rename": {
        "from": "[with patch, needs review] spell-check all modules under sage/structure",
        "to": "[with patch, positive review] spell-check all modules under sage/structure"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6735#event-27247"
}
```



---

archive/issue_comments_061349.json:
```json
{
    "body": "Changing merged from \"\" to \"Sage 4.1.1\"",
    "created_at": "2009-08-12T06:26:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6735",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6735#issuecomment-61349",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Changing merged from "" to "Sage 4.1.1"



---

archive/issue_comments_061350.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-08-12T06:26:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6735",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6735#issuecomment-61350",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Resolution: fixed



---

archive/issue_comments_061351.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Marshall Hampton\"",
    "created_at": "2009-08-12T06:26:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6735",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6735#issuecomment-61351",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

Changing reviewer from "" to "Marshall Hampton"



---

archive/issue_events_027248.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-08-12T06:26:22Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/6735",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6735#event-27248"
}
```
