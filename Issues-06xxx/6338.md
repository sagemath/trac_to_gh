# Issue 6338: Support sage --sdist sage-VERSION

archive/issues_006338.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage --sdist sage-VERSION\n```\ninstead of\n\n```\nsage --sdist VERSION\n```\ncauses problems.\n\n**Apply** [attachment: 6338_sagedist.patch.gz](https://github.com/sagemath/sage/files/ticket6338/6338_sagedist.patch.gz) to the SCRIPTS repository.\n\nAssignee: **@nexttime**\n\nReviewer: **Volker Braun**\n\nAuthor: **Jeroen Demeyer**\n\nMerged: **sage-5.1.beta1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6338_\n\n",
    "closed_at": "2012-05-23T21:30:58Z",
    "created_at": "2009-06-16T19:00:05Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20scripts",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Support sage --sdist sage-VERSION",
    "type": "issue",
    "updated_at": "2012-05-23T21:30:58Z",
    "url": "https://github.com/sagemath/sage/issues/6338",
    "user": "https://github.com/ncalexan"
}
```

```
sage --sdist sage-VERSION
```
instead of

```
sage --sdist VERSION
```
causes problems.

**Apply** [attachment: 6338_sagedist.patch.gz](https://github.com/sagemath/sage/files/ticket6338/6338_sagedist.patch.gz) to the SCRIPTS repository.

Assignee: **@nexttime**

Reviewer: **Volker Braun**

Author: **Jeroen Demeyer**

Merged: **sage-5.1.beta1**

_Issue created by migration from https://trac.sagemath.org/ticket/6338_





---

archive/issue_events_068606.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-06-16T19:00:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6338",
    "milestone_number": null,
    "milestone_title": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6338#event-68606"
}
```



---

archive/issue_events_068607.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-06-16T19:00:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6338",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20distribution",
    "label_color": "000080",
    "label_name": "component: distribution",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6338#event-68607"
}
```



---

archive/issue_events_068608.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-06-16T19:00:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6338",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6338#event-68608"
}
```



---

archive/issue_events_068609.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2009-06-16T19:00:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6338",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6338#event-68609"
}
```



---

archive/issue_events_068610.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-18T13:48:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6338",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20distribution",
    "label_color": "000080",
    "label_name": "component: distribution",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6338#event-68610"
}
```



---

archive/issue_events_068611.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-18T13:48:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6338",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20scripts",
    "label_color": "000080",
    "label_name": "component: scripts",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6338#event-68611"
}
```



---

archive/issue_comments_043686.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,10 @@\n-It's annoying that one has to hg tag --remove VERSION by hand in a bunch of repos if one wants to rebundle a release.\n \n-Also, sage -sdist sage-VERSION instead of sage -sdist VERSION causes problems.  Check for this and raise an error.\n+```\n+sage --sdist sage-VERSION\n+```\n+instead of\n+\n+```\n+sage --sdist VERSION\n+```\n+causes problems.\n``````\n",
    "created_at": "2012-05-18T13:48:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6338",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6338#issuecomment-43686",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,10 @@
-It's annoying that one has to hg tag --remove VERSION by hand in a bunch of repos if one wants to rebundle a release.
 
-Also, sage -sdist sage-VERSION instead of sage -sdist VERSION causes problems.  Check for this and raise an error.
+```
+sage --sdist sage-VERSION
+```
+instead of
+
+```
+sage --sdist VERSION
+```
+causes problems.
``````




---

archive/issue_events_068612.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-18T13:48:24Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/6338",
    "title_is": "Support sage --sdist sage-VERSION",
    "title_was": "make sage -sdist and -bdist take an existing tag name and verify that the tag name is valid",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6338#event-68612"
}
```



---

archive/issue_comments_043687.json:
```json
{
    "body": "Assignee: **@nexttime**",
    "created_at": "2012-05-18T13:48:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6338",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6338#issuecomment-43687",
    "user": "https://github.com/jdemeyer"
}
```

Assignee: **@nexttime**



---

archive/issue_comments_043688.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -8,3 +8,5 @@\n sage --sdist VERSION\n ```\n causes problems.\n+\n+**Apply** [attachment: 6338_sagedist.patch.gz](https://github.com/sagemath/sage/files/ticket6338/6338_sagedist.patch.gz) to the SCRIPTS repository.\n``````\n",
    "created_at": "2012-05-18T13:55:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6338",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6338#issuecomment-43688",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -8,3 +8,5 @@
 sage --sdist VERSION
 ```
 causes problems.
+
+**Apply** [attachment: 6338_sagedist.patch.gz](https://github.com/sagemath/sage/files/ticket6338/6338_sagedist.patch.gz) to the SCRIPTS repository.
``````




---

archive/issue_events_068613.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-18T13:55:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6338",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6338#event-68613"
}
```



---

archive/issue_events_068614.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-18T13:55:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6338",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6338#event-68614"
}
```



---

archive/issue_comments_043689.json:
```json
{
    "body": "Author: **Jeroen Demeyer**",
    "created_at": "2012-05-18T13:55:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6338",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6338#issuecomment-43689",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Jeroen Demeyer**



---

archive/issue_events_068615.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-18T13:55:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6338",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6338#event-68615"
}
```



---

archive/issue_comments_043690.json:
```json
{
    "body": "Attachment: **[6338_sagedist.patch.gz](https://github.com/sagemath/sage/files/ticket6338/6338_sagedist.patch.gz)**",
    "created_at": "2012-05-19T08:30:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6338",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6338#issuecomment-43690",
    "user": "https://github.com/jdemeyer"
}
```

Attachment: **[6338_sagedist.patch.gz](https://github.com/sagemath/sage/files/ticket6338/6338_sagedist.patch.gz)**



---

archive/issue_events_068616.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-05-23T16:03:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6338",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6338#event-68616"
}
```



---

archive/issue_events_068617.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-05-23T16:03:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6338",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6338#event-68617"
}
```



---

archive/issue_comments_043691.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nLooks good. I tripped over this more than once, thanks for fixing!",
    "created_at": "2012-05-23T16:03:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6338",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6338#issuecomment-43691",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:3'>Comment 3:</a>
Looks good. I tripped over this more than once, thanks for fixing!



---

archive/issue_comments_043692.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2012-05-23T21:30:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6338",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6338#issuecomment-43692",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Volker Braun**



---

archive/issue_events_068618.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-23T21:30:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6338",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6338#event-68618"
}
```



---

archive/issue_events_068619.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-05-23T21:30:58Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6338",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6338#event-68619"
}
```



---

archive/issue_comments_043693.json:
```json
{
    "body": "Merged: **sage-5.1.beta1**",
    "created_at": "2012-05-23T21:30:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6338",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6338#issuecomment-43693",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.1.beta1**
