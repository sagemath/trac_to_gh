# Issue 6114: symbolics -- fix removal of constants.so

archive/issues_006114.json:
```json
{
    "assignees": [
        "https://github.com/mwhansen"
    ],
    "body": "\n```\nUpgrading from alpha0 on Fedora 10 failed somehow. I had to remove constants.so\nby hand.\n\n$ rm devel/sage/build/sage/symbolic/constants.so\n\nNow testing.\n\nOn Fedora 9 I'll do a fresh install.\n\n```\n\nI (=william) did remove enough constants.* files, which resulted in the above.  I will attach a patch to fix this.\n\nComponent: **calculus**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6114_\n\n",
    "closed_at": "2009-05-28T08:04:21Z",
    "created_at": "2009-05-21T18:38:31Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20calculus",
        "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "symbolics -- fix removal of constants.so",
    "type": "issue",
    "updated_at": "2009-05-28T08:04:21Z",
    "url": "https://github.com/sagemath/sage/issues/6114",
    "user": "https://github.com/williamstein"
}
```

```
Upgrading from alpha0 on Fedora 10 failed somehow. I had to remove constants.so
by hand.

$ rm devel/sage/build/sage/symbolic/constants.so

Now testing.

On Fedora 9 I'll do a fresh install.

```

I (=william) did remove enough constants.* files, which resulted in the above.  I will attach a patch to fix this.

Component: **calculus**

_Issue created by migration from https://trac.sagemath.org/ticket/6114_





---

archive/issue_events_073432.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-05-21T18:38:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6114",
    "milestone_number": null,
    "milestone_title": "sage-4.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6114#event-73432"
}
```



---

archive/issue_events_073433.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-05-21T18:38:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6114",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20calculus",
    "label_color": "0000ff",
    "label_name": "c: calculus",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6114#event-73433"
}
```



---

archive/issue_events_073434.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-05-21T18:38:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6114",
    "label": "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p: 1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6114#event-73434"
}
```



---

archive/issue_events_073435.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-05-21T18:38:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6114",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6114#event-73435"
}
```



---

archive/issue_events_073436.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-05-21T18:38:31Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6114",
    "subject": "https://github.com/williamstein",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6114#event-73436"
}
```



---

archive/issue_comments_040372.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nCraig's sync-build at #5977 gives a cleaner way to do this.",
    "created_at": "2009-05-28T02:59:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6114#issuecomment-40372",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:1" align="right">comment:1</div>

Craig's sync-build at #5977 gives a cleaner way to do this.



---

archive/issue_events_073437.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-28T02:59:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6114",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6114#event-73437"
}
```



---

archive/issue_events_073438.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-05-28T02:59:37Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/6114",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6114#event-73438"
}
```



---

archive/issue_events_073439.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-28T02:59:37Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6114",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6114#event-73439"
}
```



---

archive/issue_comments_040373.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nActually, this won't work if there is a stale constants.cpp file sitting around in the directory.",
    "created_at": "2009-05-28T06:11:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6114#issuecomment-40373",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:2" align="right">comment:2</div>

Actually, this won't work if there is a stale constants.cpp file sitting around in the directory.



---

archive/issue_events_073440.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-05-28T08:01:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6114",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6114#event-73440"
}
```



---

archive/issue_events_073441.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-05-28T08:01:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6114",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6114#event-73441"
}
```



---

archive/issue_comments_040374.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nAttachment: **[trac_6114.patch.gz](https://github.com/sagemath/sage/files/ticket6114/trac_6114.patch.gz)**",
    "created_at": "2009-05-28T08:01:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6114#issuecomment-40374",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:3" align="right">comment:3</div>

Attachment: **[trac_6114.patch.gz](https://github.com/sagemath/sage/files/ticket6114/trac_6114.patch.gz)**



---

archive/issue_events_073442.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-28T08:04:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6114",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6114#event-73442"
}
```



---

archive/issue_events_073443.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-28T08:04:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6114",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6114#event-73443"
}
```



---

archive/issue_comments_040375.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nMerged in 4.0.rc1.",
    "created_at": "2009-05-28T08:04:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6114",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6114#issuecomment-40375",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:4" align="right">comment:4</div>

Merged in 4.0.rc1.
