# Issue 6589: bring doctest coverage for ring/laurent_series_ring.py to 100%

archive/issues_006589.json:
```json
{
    "body": "I (Marshall Hampton) am hoping to work on this in late July or August 2009.  If it hasn't been done by September, assume that I didn't get around to it.\n\nAssignee: tbd\n\nKeywords: Laurent series, doctest, coverage\n\nReviewer: David Roe\n\nResolution: worksforme\n\nIssue created by migration from https://trac.sagemath.org/ticket/6589\n\n",
    "closed_at": "2013-02-08T13:24:35Z",
    "created_at": "2009-07-22T13:07:51Z",
    "labels": [
        "component: algebra",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "bring doctest coverage for ring/laurent_series_ring.py to 100%",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/6589",
    "user": "https://trac.sagemath.org/admin/accounts/users/mhampton"
}
```
I (Marshall Hampton) am hoping to work on this in late July or August 2009.  If it hasn't been done by September, assume that I didn't get around to it.

Assignee: tbd

Keywords: Laurent series, doctest, coverage

Reviewer: David Roe

Resolution: worksforme

Issue created by migration from https://trac.sagemath.org/ticket/6589





---

archive/issue_comments_060835.json:
```json
{
    "body": "<a id='comment:1'></a>As of Sage 5.7.beta2, the file in question does have 100% coverage. So, I think this has been done. See the related #12259 (which itself is a part of the bigger meta ticket: #12024). So, I'll set this to sage-invalid...",
    "created_at": "2013-02-03T19:49:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6589",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6589#issuecomment-60835",
    "user": "https://github.com/KPanComputes"
}
```

<a id='comment:1'></a>As of Sage 5.7.beta2, the file in question does have 100% coverage. So, I think this has been done. See the related #12259 (which itself is a part of the bigger meta ticket: #12024). So, I'll set this to sage-invalid...



---

archive/issue_comments_060836.json:
```json
{
    "body": "Changing upstream from \"\" to \"N/A\"",
    "created_at": "2013-02-03T19:49:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6589",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6589#issuecomment-60836",
    "user": "https://github.com/KPanComputes"
}
```

Changing upstream from "" to "N/A"



---

archive/issue_comments_060837.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2013-02-03T19:50:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6589",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6589#issuecomment-60837",
    "user": "https://github.com/KPanComputes"
}
```

Changing status from new to needs_review.



---

archive/issue_events_015531.json:
```json
{
    "actor": "https://github.com/KPanComputes",
    "created_at": "2013-02-03T19:50:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6589",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6589#event-15531"
}
```



---

archive/issue_comments_060838.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2013-02-05T18:02:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6589",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6589#issuecomment-60838",
    "user": "https://github.com/tscrim"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_060839.json:
```json
{
    "body": "<a id='comment:3'></a>Also in `5.5.rc0`:\n\n```\ntravis@travis-virtualbox:~/sage-5.5.rc0/devel/sage/sage$ sage -coverage rings/laurent_series_ring.py \n----------------------------------------------------------------------\nrings/laurent_series_ring.py\nSCORE rings/laurent_series_ring.py: 100% (25 of 25)\n----------------------------------------------------------------------\n```",
    "created_at": "2013-02-05T18:02:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6589",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6589#issuecomment-60839",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:3'></a>Also in `5.5.rc0`:

```
travis@travis-virtualbox:~/sage-5.5.rc0/devel/sage/sage$ sage -coverage rings/laurent_series_ring.py 
----------------------------------------------------------------------
rings/laurent_series_ring.py
SCORE rings/laurent_series_ring.py: 100% (25 of 25)
----------------------------------------------------------------------
```



---

archive/issue_events_015532.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-08T13:24:35Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/6589",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6589#event-15532"
}
```



---

archive/issue_comments_060840.json:
```json
{
    "body": "Changing reviewer from \"roed\" to \"David Roe\"",
    "created_at": "2013-02-08T13:24:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6589",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6589#issuecomment-60840",
    "user": "https://github.com/jdemeyer"
}
```

Changing reviewer from "roed" to "David Roe"



---

archive/issue_comments_060841.json:
```json
{
    "body": "Resolution: worksforme",
    "created_at": "2013-02-08T13:24:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6589",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6589#issuecomment-60841",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: worksforme
