# Issue 6663: add new 4ti2 experimental package

archive/issues_006663.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "The 4ti2 and glpk packages in experimental are very out of date.  Since there seems to be some growing interest in having them available in sage they should be updated.  If they can be made to work on more platforms they should be moved to optional.\n\nKeywords: **4ti2, glpk, sandpile**\n\nComponent: **packages: experimental**\n\nAuthor: **Marshall Hampton**\n\nReviewer: **David Joyner, Alex Ghitza, Bjarke Hammersholt Roune, Mike Hansen, David Perkinson**\n\nMerged: **Sage 4.1.2.alpha2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/6663_\n\n",
    "closed_at": "2009-09-11T18:11:36Z",
    "created_at": "2009-07-31T20:14:51Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20experimental",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "add new 4ti2 experimental package",
    "type": "issue",
    "updated_at": "2009-09-20T22:35:51Z",
    "url": "https://github.com/sagemath/sage/issues/6663",
    "user": "https://github.com/sagetrac-mhampton"
}
```
The 4ti2 and glpk packages in experimental are very out of date.  Since there seems to be some growing interest in having them available in sage they should be updated.  If they can be made to work on more platforms they should be moved to optional.

Keywords: **4ti2, glpk, sandpile**

Component: **packages: experimental**

Author: **Marshall Hampton**

Reviewer: **David Joyner, Alex Ghitza, Bjarke Hammersholt Roune, Mike Hansen, David Perkinson**

Merged: **Sage 4.1.2.alpha2**

_Issue created by migration from https://trac.sagemath.org/ticket/6663_





---

archive/issue_events_080775.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-07-31T20:14:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/6663",
    "milestone_number": null,
    "milestone_title": "sage-4.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6663#event-80775"
}
```



---

archive/issue_events_080776.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-07-31T20:14:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6663",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "c: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6663#event-80776"
}
```



---

archive/issue_events_080777.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-07-31T20:14:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6663",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6663#event-80777"
}
```



---

archive/issue_events_080778.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-07-31T20:14:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6663",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6663#event-80778"
}
```



---

archive/issue_events_080779.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-07-31T20:19:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6663",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6663#event-80779"
}
```



---

archive/issue_comments_046610.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nMy current package attempts are at:\n\nhttp://www.d.umn.edu/~mhampton/4ti2.p0.spkg\nhttp://www.d.umn.edu/~mhampton/glpk.p0.spkg \n\n-Marshall",
    "created_at": "2009-07-31T20:19:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6663",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6663#issuecomment-46610",
    "user": "https://github.com/sagetrac-mhampton"
}
```

<div id="comment:1" align="right">comment:1</div>

My current package attempts are at:

http://www.d.umn.edu/~mhampton/4ti2.p0.spkg
http://www.d.umn.edu/~mhampton/glpk.p0.spkg 

-Marshall



---

archive/issue_comments_046611.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nI was unaware of the glpk package at:\nhttps://github.com/sagemath/sage/issues/6602\n\nwhich has a more recent version of glpk.  That spkg should be used instead of mine.",
    "created_at": "2009-07-31T23:06:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6663",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6663#issuecomment-46611",
    "user": "https://github.com/sagetrac-mhampton"
}
```

<div id="comment:2" align="right">comment:2</div>

I was unaware of the glpk package at:
https://github.com/sagemath/sage/issues/6602

which has a more recent version of glpk.  That spkg should be used instead of mine.



---

archive/issue_events_080780.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2009-07-31T23:06:41Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/6663",
    "title_is": "add new 4ti2 experimental package",
    "title_was": "add new 4ti2 and glpk experimental packages",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6663#event-80780"
}
```



---

archive/issue_comments_046612.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nThis 4ti2 spkg installed fine with N Cohen's glpk (https://github.com/sagemath/sage/issues/6602) on an amd64 ubuntu 9.04 machine.",
    "created_at": "2009-08-01T04:08:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6663",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6663#issuecomment-46612",
    "user": "https://github.com/wdjoyner"
}
```

<div id="comment:3" align="right">comment:3</div>

This 4ti2 spkg installed fine with N Cohen's glpk (https://github.com/sagemath/sage/issues/6602) on an amd64 ubuntu 9.04 machine.



---

archive/issue_comments_046613.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nThis installed fine and passed sage -testall (except for the known failures) on an intel macbook with 4.1.1.a0 running OS 10.4.11.\n\nThis gets a positive review from me. Are there more tests which should be run before \"needs review\" gets changed to \"positive review\"?",
    "created_at": "2009-08-01T20:11:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6663",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6663#issuecomment-46613",
    "user": "https://github.com/wdjoyner"
}
```

<div id="comment:4" align="right">comment:4</div>

This installed fine and passed sage -testall (except for the known failures) on an intel macbook with 4.1.1.a0 running OS 10.4.11.

This gets a positive review from me. Are there more tests which should be run before "needs review" gets changed to "positive review"?



---

archive/issue_events_080781.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-08-16T05:52:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6663",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "c: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6663#event-80781"
}
```



---

archive/issue_events_080782.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-08-16T05:52:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6663",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6663#event-80782"
}
```



---

archive/issue_events_080783.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-08-16T05:52:54Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/6663",
    "subject": "https://github.com/aghitza",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6663#event-80783"
}
```



---

archive/issue_events_080784.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-08-16T14:11:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6663",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6663#event-80784"
}
```



---

archive/issue_events_080785.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-08-16T14:11:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6663",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6663#event-80785"
}
```



---

archive/issue_comments_046614.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nI have downloaded the spkg to my home directory:\n\n\n\nhttp://sage.math.washington.edu/home/mvngu/patch/4ti2.p0.spkg\n\n\n\nIs there any special reason why the package is not under revision control?\n\n```\n[mvngu@mod mvngu]$ tar -jxf 4ti2.p0.spkg \n[mvngu@mod mvngu]$ cd 4ti2.p0/\n[mvngu@mod 4ti2.p0]$ hg st\nabort: There is no Mercurial repository here (.hg not found)!\n```",
    "created_at": "2009-09-02T08:11:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6663",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6663#issuecomment-46614",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:7" align="right">comment:7</div>

I have downloaded the spkg to my home directory:



http://sage.math.washington.edu/home/mvngu/patch/4ti2.p0.spkg



Is there any special reason why the package is not under revision control?

```
[mvngu@mod mvngu]$ tar -jxf 4ti2.p0.spkg 
[mvngu@mod mvngu]$ cd 4ti2.p0/
[mvngu@mod 4ti2.p0]$ hg st
abort: There is no Mercurial repository here (.hg not found)!
```



---

archive/issue_events_080786.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-11T18:11:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6663",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6663#event-80786"
}
```



---

archive/issue_events_080787.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-11T18:11:36Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6663",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6663#event-80787"
}
```



---

archive/issue_comments_046615.json:
```json
{
    "body": "Author: **Marshall Hampton**",
    "created_at": "2009-09-11T18:11:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6663",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6663#issuecomment-46615",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Author: **Marshall Hampton**



---

archive/issue_comments_046616.json:
```json
{
    "body": "Changed reviewer from **broune, mhansen, dperkinson** to **David Joyner, Alex Ghitza, Bjarke Hammersholt Roune, Mike Hansen, David Perkinson**",
    "created_at": "2009-09-11T18:11:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6663",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6663#issuecomment-46616",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Changed reviewer from **broune, mhansen, dperkinson** to **David Joyner, Alex Ghitza, Bjarke Hammersholt Roune, Mike Hansen, David Perkinson**



---

archive/issue_comments_046617.json:
```json
{
    "body": "Merged: **Sage 4.1.2.alpha2**",
    "created_at": "2009-09-11T18:11:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6663",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6663#issuecomment-46617",
    "user": "https://github.com/sagetrac-mvngu"
}
```

Merged: **Sage 4.1.2.alpha2**



---

archive/issue_comments_046618.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nMerged in the experimental packages repository at\n\nhttp://www.sagemath.org/packages/experimental/\n\nThe updated spkg is available at\n\nhttp://www.sagemath.org/packages/experimental/4ti2.p0.spkg",
    "created_at": "2009-09-11T18:11:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/6663",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6663#issuecomment-46618",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:8" align="right">comment:8</div>

Merged in the experimental packages repository at

http://www.sagemath.org/packages/experimental/

The updated spkg is available at

http://www.sagemath.org/packages/experimental/4ti2.p0.spkg



---

archive/issue_events_080788.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-20T22:35:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6663",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6663#event-80788"
}
```



---

archive/issue_events_080789.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-09-20T22:35:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6663",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20experimental",
    "label_color": "000080",
    "label_name": "c: packages: experimental",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6663#event-80789"
}
```
