# Issue 6127: [with patch, positive review] fix trivial typo in documentation

archive/issues_006127.json:
```json
{
    "body": "This was pointed out by Ron Evans (of UCSD).\n\nAssignee: tba\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/6127\n\n",
    "closed_at": "2009-05-28T00:32:32Z",
    "created_at": "2009-05-25T01:13:55Z",
    "labels": [
        "component: documentation",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.0",
    "title": "[with patch, positive review] fix trivial typo in documentation",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/6127",
    "user": "https://github.com/williamstein"
}
```
This was pointed out by Ron Evans (of UCSD).

Assignee: tba

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/6127





---

archive/issue_comments_053083.json:
```json
{
    "body": "Attachment [trac_6127.patch](tarball://root/attachments/some-uuid/ticket6127/trac_6127.patch) by @williamstein created at 2009-05-25 01:14:34",
    "created_at": "2009-05-25T01:14:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6127",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6127#issuecomment-53083",
    "user": "https://github.com/williamstein"
}
```

Attachment [trac_6127.patch](tarball://root/attachments/some-uuid/ticket6127/trac_6127.patch) by @williamstein created at 2009-05-25 01:14:34



---

archive/issue_events_024620.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/6127",
    "rename": {
        "from": "[with patch; needs review] fix trivial typo in documentation",
        "to": "[with patch, positive review] fix trivial typo in documentation"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6127#event-24620"
}
```



---

archive/issue_comments_053084.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-05-28T00:32:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6127",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6127#issuecomment-53084",
    "user": "https://github.com/mwhansen"
}
```

Resolution: fixed



---

archive/issue_events_024621.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-28T00:32:32Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/6127",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6127#event-24621"
}
```



---

archive/issue_comments_053085.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in 4.0.rc1.",
    "created_at": "2009-05-28T00:32:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6127",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6127#issuecomment-53085",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:2'></a>
Merged in 4.0.rc1.



---

archive/issue_events_024622.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-28T00:32:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6127",
    "milestone": "sage-4.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6127#event-24622"
}
```
