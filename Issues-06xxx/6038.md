# Issue 6038: [with patch, positive review] add implicit_plot3d to the reference manual (and maybe an example in the tutorial)

archive/issues_006038.json:
```json
{
    "body": "Please add the wonderful implicit_plot3d to the reference manual, and possibly even add an example to the tutorial.\n\nAssignee: tba\n\nCC:  wcauchois\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/6038\n\n",
    "closed_at": "2009-05-26T16:49:29Z",
    "created_at": "2009-05-14T15:26:53Z",
    "labels": [
        "component: documentation",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.0",
    "title": "[with patch, positive review] add implicit_plot3d to the reference manual (and maybe an example in the tutorial)",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/6038",
    "user": "https://github.com/williamstein"
}
```
Please add the wonderful implicit_plot3d to the reference manual, and possibly even add an example to the tutorial.

Assignee: tba

CC:  wcauchois

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/6038





---

archive/issue_events_024209.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/6038",
    "rename": {
        "from": "add implicit_plot3d to the reference manual (and maybe an example in the tutorial)",
        "to": "[with patch, needs review] add implicit_plot3d to the reference manual (and maybe an example in the tutorial)"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6038#event-24209"
}
```



---

archive/issue_comments_051964.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [trac6038.patch](tarball://root/attachments/some-uuid/ticket6038/trac6038.patch) by wcauchois created at 2009-05-25 05:37:25\n\n(This patch is based on Sage 4.0.rc0.)\n\nIn order to make the automatically generated reference documentation clearer, I moved the implicit_plot3d() function into a separate file called \"implicit_plot3d.py\". I see this has been done for some of the other plotting functions as well. I also took the liberty of adding a little bit to the meager section on \"Three-Dimensional Plots\" in the tour of Sage.",
    "created_at": "2009-05-25T05:37:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6038",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6038#issuecomment-51964",
    "user": "https://trac.sagemath.org/admin/accounts/users/wcauchois"
}
```

<a id='comment:1'></a>
Attachment [trac6038.patch](tarball://root/attachments/some-uuid/ticket6038/trac6038.patch) by wcauchois created at 2009-05-25 05:37:25

(This patch is based on Sage 4.0.rc0.)

In order to make the automatically generated reference documentation clearer, I moved the implicit_plot3d() function into a separate file called "implicit_plot3d.py". I see this has been done for some of the other plotting functions as well. I also took the liberty of adding a little bit to the meager section on "Three-Dimensional Plots" in the tour of Sage.



---

archive/issue_comments_051965.json:
```json
{
    "body": "<a id='comment:3'></a>\nLooks good to me.\n\nMerged in Sage 4.0.rc1.",
    "created_at": "2009-05-26T16:49:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6038",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6038#issuecomment-51965",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:3'></a>
Looks good to me.

Merged in Sage 4.0.rc1.



---

archive/issue_events_024210.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-05-26T16:49:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/6038",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6038#event-24210"
}
```



---

archive/issue_events_024211.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/6038",
    "rename": {
        "from": "[with patch, needs review] add implicit_plot3d to the reference manual (and maybe an example in the tutorial)",
        "to": "[with patch, positive review] add implicit_plot3d to the reference manual (and maybe an example in the tutorial)"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6038#event-24211"
}
```



---

archive/issue_comments_051966.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-05-26T16:49:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6038",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6038#issuecomment-51966",
    "user": "https://github.com/mwhansen"
}
```

Resolution: fixed
