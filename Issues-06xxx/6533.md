# Issue 6533: sage >= 4.1.rc1 doesn't build on cleo (ia64-Linux-rhel5)

archive/issues_006533.json:
```json
{
    "body": "The Sage library fails to build (when linking symmetrica) on  ia64-Linux-rhel5.  I am investigating this. Note that sage-4.1.rc0 built fine, and there are minimal changes between rc0 and rc1. \n\nIssue created by migration from https://trac.sagemath.org/ticket/6533\n\n",
    "closed_at": "2009-11-11T17:43:16Z",
    "created_at": "2009-07-14T18:39:01Z",
    "labels": [
        "component: build",
        "blocker",
        "bug",
        "invalid"
    ],
    "title": "sage >= 4.1.rc1 doesn't build on cleo (ia64-Linux-rhel5)",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/6533",
    "user": "https://github.com/williamstein"
}
```
The Sage library fails to build (when linking symmetrica) on  ia64-Linux-rhel5.  I am investigating this. Note that sage-4.1.rc0 built fine, and there are minimal changes between rc0 and rc1. 

Issue created by migration from https://trac.sagemath.org/ticket/6533





---

archive/issue_events_043012.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-10-02T16:19:09Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6533",
    "milestone": "sage-4.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6533#event-43012"
}
```



---

archive/issue_events_043013.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-10-02T16:19:09Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6533",
    "milestone": "sage-4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6533#event-43013"
}
```



---

archive/issue_events_043014.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-10-02T17:44:41Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6533",
    "milestone": "sage-4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6533#event-43014"
}
```



---

archive/issue_events_043015.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-10-02T17:44:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6533",
    "milestone": "sage-4.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6533#event-43015"
}
```



---

archive/issue_events_043016.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-11-11T17:43:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/6533",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6533#event-43016"
}
```



---

archive/issue_comments_046548.json:
```json
{
    "body": "<a id='comment:3'></a>\nAccording to the sysadmin, this appears to be a problem with the binutils and compiler setup on that computer.  So I'm closing this as invalid for now.",
    "created_at": "2009-11-11T17:43:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6533",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6533#issuecomment-46548",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:3'></a>
According to the sysadmin, this appears to be a problem with the binutils and compiler setup on that computer.  So I'm closing this as invalid for now.



---

archive/issue_events_043017.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2010-01-05T20:45:10Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/6533",
    "milestone": "sage-4.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6533#event-43017"
}
```
