# Issue 6965: preventing repository corruption with MANIFEST.in

archive/issues_006965.json:
```json
{
    "body": "From [sage-devel](http://groups.google.com/group/sage-devel/browse_thread/thread/61612a7bcca169e7/17f2ab8eb4f63b7f):\n\n```\nI should have posted this question here instead of at #6865, as the\nanswer is probably interesting to many people here.  I'd heard things\nabout MANIFEST.in, but for some reason didn't have a clear idea of what\nit was or what I should do about it.  It would be nice if something was\nadded to the developer's guide... \n```\n\n**CC:**  @jasongrout\n\n**Author:** John Palmieri\n\n**Reviewer:** Minh Van Nguyen\n\n**Merged:** sage-4.3.1.alpha0\n\nIssue created by migration from https://trac.sagemath.org/ticket/6965\n\n",
    "closed_at": "2010-01-03T22:04:51Z",
    "created_at": "2009-09-20T02:56:51Z",
    "labels": [
        "component: documentation",
        "minor",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.1",
    "title": "preventing repository corruption with MANIFEST.in",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/6965",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```
From [sage-devel](http://groups.google.com/group/sage-devel/browse_thread/thread/61612a7bcca169e7/17f2ab8eb4f63b7f):

```
I should have posted this question here instead of at #6865, as the
answer is probably interesting to many people here.  I'd heard things
about MANIFEST.in, but for some reason didn't have a clear idea of what
it was or what I should do about it.  It would be nice if something was
added to the developer's guide... 
```

**CC:**  @jasongrout

**Author:** John Palmieri

**Reviewer:** Minh Van Nguyen

**Merged:** sage-4.3.1.alpha0

Issue created by migration from https://trac.sagemath.org/ticket/6965





---

archive/issue_comments_051351.json:
```json
{
    "body": "<a id='comment:1'></a>\nFor future reference: see [http://www.python.org/doc/2.6.2/distutils/sourcedist.html#specifying-the-files-to-distribute](http://www.python.org/doc/2.6.2/distutils/sourcedist.html#specifying-the-files-to-distribute) for the relevant part of the Python distribution.",
    "created_at": "2009-09-21T23:19:13Z",
    "issue": "https://github.com/sagemath/sage/issues/6965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6965#issuecomment-51351",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:1'></a>
For future reference: see [http://www.python.org/doc/2.6.2/distutils/sourcedist.html#specifying-the-files-to-distribute](http://www.python.org/doc/2.6.2/distutils/sourcedist.html#specifying-the-files-to-distribute) for the relevant part of the Python distribution.



---

archive/issue_comments_051352.json:
```json
{
    "body": "<a id='comment:2'></a>\n> For future reference: see http://www.python.org/doc/2.6.2/distutils/sourcedist.html#specifying-the-files-to-distribute for the relevant part of the Python distribution.\n\n\n(change \"distribution\" to \"documentation\")",
    "created_at": "2009-09-21T23:20:24Z",
    "issue": "https://github.com/sagemath/sage/issues/6965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6965#issuecomment-51352",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:2'></a>
> For future reference: see http://www.python.org/doc/2.6.2/distutils/sourcedist.html#specifying-the-files-to-distribute for the relevant part of the Python distribution.


(change "distribution" to "documentation")



---

archive/issue_comments_051353.json:
```json
{
    "body": "<a id='comment:3'></a>\nHere's a patch.",
    "created_at": "2009-12-22T00:43:45Z",
    "issue": "https://github.com/sagemath/sage/issues/6965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6965#issuecomment-51353",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:3'></a>
Here's a patch.



---

archive/issue_events_046373.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-12-22T00:43:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6965",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6965#event-46373"
}
```



---

archive/issue_events_046374.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-12-22T00:43:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6965",
    "label": "minor",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6965#event-46374"
}
```



---

archive/issue_comments_051354.json:
```json
{
    "body": "**Author:** John Palmieri",
    "created_at": "2009-12-22T00:43:45Z",
    "issue": "https://github.com/sagemath/sage/issues/6965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6965#issuecomment-51354",
    "user": "https://github.com/jhpalmieri"
}
```

**Author:** John Palmieri



---

archive/issue_events_046375.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-12-22T08:33:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6965",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6965#event-46375"
}
```



---

archive/issue_events_046376.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-12-22T08:33:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6965",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6965#event-46376"
}
```



---

archive/issue_comments_051355.json:
```json
{
    "body": "<a id='comment:4'></a>\nLooks good. Applied OK against Sage 4.3.rc0. As a side note, the URL for \"Specifying the files to distribute\" could also be \n\nhttp://docs.python.org/distutils/sourcedist.html#specifying-the-files-to-distribute\n\nwhich is usually more up-to-date than the given URL. The given URL is more specific to Python 2.6.2.",
    "created_at": "2009-12-22T08:33:13Z",
    "issue": "https://github.com/sagemath/sage/issues/6965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6965#issuecomment-51355",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:4'></a>
Looks good. Applied OK against Sage 4.3.rc0. As a side note, the URL for "Specifying the files to distribute" could also be 

http://docs.python.org/distutils/sourcedist.html#specifying-the-files-to-distribute

which is usually more up-to-date than the given URL. The given URL is more specific to Python 2.6.2.



---

archive/issue_comments_051356.json:
```json
{
    "body": "**Reviewer:** Minh Van Nguyen",
    "created_at": "2009-12-22T08:33:13Z",
    "issue": "https://github.com/sagemath/sage/issues/6965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6965#issuecomment-51356",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Reviewer:** Minh Van Nguyen



---

archive/attachments_008372.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_6965-manifest.patch",
    "asset_url": "tarball://root/attachments/ticket6965/trac_6965-manifest.patch",
    "created_at": "2009-12-22T16:02:30Z",
    "issue": "https://github.com/sagemath/sage/issues/6965",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket6965/trac_6965-manifest.patch",
    "user": "https://github.com/jhpalmieri"
}
```



---

archive/issue_comments_051357.json:
```json
{
    "body": "<a id='comment:5'></a>\nAttachment [trac_6965-manifest.patch](https://github.com/sagemath/sage/files/ticket6965/trac_6965-manifest.patch) by @jhpalmieri created at 2009-12-22 16:02:30\n\nGood point about the URL.  I've replaced the patch with a new one, changing just the URL to the more permanent one.",
    "created_at": "2009-12-22T16:02:30Z",
    "issue": "https://github.com/sagemath/sage/issues/6965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6965#issuecomment-51357",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:5'></a>
Attachment [trac_6965-manifest.patch](https://github.com/sagemath/sage/files/ticket6965/trac_6965-manifest.patch) by @jhpalmieri created at 2009-12-22 16:02:30

Good point about the URL.  I've replaced the patch with a new one, changing just the URL to the more permanent one.



---

archive/issue_comments_051358.json:
```json
{
    "body": "**Merged:** sage-4.3.1.alpha0",
    "created_at": "2010-01-03T22:04:51Z",
    "issue": "https://github.com/sagemath/sage/issues/6965",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6965#issuecomment-51358",
    "user": "https://github.com/mwhansen"
}
```

**Merged:** sage-4.3.1.alpha0



---

archive/issue_events_046377.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-01-03T22:04:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6965",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6965#event-46377"
}
```



---

archive/issue_events_046378.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2010-01-03T22:04:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6965",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6965#event-46378"
}
```
