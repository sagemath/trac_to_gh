# Issue 6902: log(x) is typeset as \ln x

archive/issues_006902.json:
```json
{
    "body": "\n```\nsage: log(x)\nlog(x)\nsage: latex(log(x))\n\\ln\\left(x\\right)\n```\n\nWe should switch back to `\\log`. See this thread:\n\nhttp://groups.google.com/group/sage-devel/browse_thread/thread/dc6530a2071bd6db\n\n**Assignee:** @burcin\n\n**Author:** Burcin Erocal\n\n**Reviewer:** Karl-Dieter Crisman\n\n**Merged:** Sage 4.1.2.alpha4\n\nIssue created by migration from https://trac.sagemath.org/ticket/6902\n\n",
    "closed_at": "2009-09-25T22:44:59Z",
    "created_at": "2009-09-07T19:05:26Z",
    "labels": [
        "component: symbolics",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.1.2",
    "title": "log(x) is typeset as \\ln x",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/6902",
    "user": "https://github.com/burcin"
}
```

```
sage: log(x)
log(x)
sage: latex(log(x))
\ln\left(x\right)
```

We should switch back to `\log`. See this thread:

http://groups.google.com/group/sage-devel/browse_thread/thread/dc6530a2071bd6db

**Assignee:** @burcin

**Author:** Burcin Erocal

**Reviewer:** Karl-Dieter Crisman

**Merged:** Sage 4.1.2.alpha4

Issue created by migration from https://trac.sagemath.org/ticket/6902





---

archive/issue_comments_050686.json:
```json
{
    "body": "**Assignee:** @burcin",
    "created_at": "2009-09-19T15:11:41Z",
    "issue": "https://github.com/sagemath/sage/issues/6902",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6902#issuecomment-50686",
    "user": "https://github.com/burcin"
}
```

**Assignee:** @burcin



---

archive/issue_comments_050687.json:
```json
{
    "body": "**Author:** Burcin Erocal",
    "created_at": "2009-09-19T15:11:41Z",
    "issue": "https://github.com/sagemath/sage/issues/6902",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6902#issuecomment-50687",
    "user": "https://github.com/burcin"
}
```

**Author:** Burcin Erocal



---

archive/issue_events_045873.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-09-19T15:11:41Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/6902",
    "rename": {
        "from": "log(x) is typeset as \\ln x",
        "to": "[waiting for new pynac] log(x) is typeset as \\ln x"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6902#event-45873"
}
```



---

archive/attachments_008282.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_6902-log_latex.patch",
    "asset_url": "tarball://root/attachments/ticket6902/trac_6902-log_latex.patch",
    "created_at": "2009-09-19T15:11:41Z",
    "issue": "https://github.com/sagemath/sage/issues/6902",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket6902/trac_6902-log_latex.patch",
    "user": "https://github.com/burcin"
}
```



---

archive/issue_comments_050688.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [trac_6902-log_latex.patch](https://github.com/sagemath/sage/files/ticket6902/trac_6902-log_latex.patch) by @burcin created at 2009-09-19 15:11:41\n\nThis is fixed in my pynac tree. Doctest for Sage is in attachment:trac_6902-log_latex.patch.\n\nI will post a new pynac spkg and review instructions soon.",
    "created_at": "2009-09-19T15:11:41Z",
    "issue": "https://github.com/sagemath/sage/issues/6902",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6902#issuecomment-50688",
    "user": "https://github.com/burcin"
}
```

<a id='comment:1'></a>
Attachment [trac_6902-log_latex.patch](https://github.com/sagemath/sage/files/ticket6902/trac_6902-log_latex.patch) by @burcin created at 2009-09-19 15:11:41

This is fixed in my pynac tree. Doctest for Sage is in attachment:trac_6902-log_latex.patch.

I will post a new pynac spkg and review instructions soon.



---

archive/issue_events_045874.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-09-22T19:28:15Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/6902",
    "rename": {
        "from": "[waiting for new pynac] log(x) is typeset as \\ln x",
        "to": "log(x) is typeset as \\ln x"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6902#event-45874"
}
```



---

archive/issue_events_045875.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2009-09-22T19:28:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6902",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6902#event-45875"
}
```



---

archive/issue_comments_050689.json:
```json
{
    "body": "<a id='comment:2'></a>\nNew pynac package available at #6993.",
    "created_at": "2009-09-22T19:28:15Z",
    "issue": "https://github.com/sagemath/sage/issues/6902",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6902#issuecomment-50689",
    "user": "https://github.com/burcin"
}
```

<a id='comment:2'></a>
New pynac package available at #6993.



---

archive/issue_events_045876.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-09-23T02:35:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6902",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6902#event-45876"
}
```



---

archive/issue_events_045877.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2009-09-23T02:35:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/6902",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6902#event-45877"
}
```



---

archive/issue_comments_050690.json:
```json
{
    "body": "**Reviewer:** Karl-Dieter Crisman",
    "created_at": "2009-09-23T02:35:33Z",
    "issue": "https://github.com/sagemath/sage/issues/6902",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6902#issuecomment-50690",
    "user": "https://github.com/kcrisman"
}
```

**Reviewer:** Karl-Dieter Crisman



---

archive/issue_comments_050691.json:
```json
{
    "body": "<a id='comment:3'></a>\nThis works.  Pending of course review of new Pynac as a whole.",
    "created_at": "2009-09-23T02:35:33Z",
    "issue": "https://github.com/sagemath/sage/issues/6902",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6902#issuecomment-50691",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:3'></a>
This works.  Pending of course review of new Pynac as a whole.



---

archive/issue_comments_050692.json:
```json
{
    "body": "**Merged:** Sage 4.1.2.alpha3",
    "created_at": "2009-09-25T22:44:59Z",
    "issue": "https://github.com/sagemath/sage/issues/6902",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6902#issuecomment-50692",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Merged:** Sage 4.1.2.alpha3



---

archive/issue_events_045878.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-09-25T22:44:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/6902",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6902#event-45878"
}
```



---

archive/issue_events_045879.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mvngu",
    "created_at": "2009-09-25T22:44:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/6902",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/6902#event-45879"
}
```



---

archive/issue_comments_050693.json:
```json
{
    "body": "**Changing merged** from \"Sage 4.1.2.alpha3\" to \"Sage 4.1.2.alpha4\".",
    "created_at": "2009-09-27T10:39:40Z",
    "issue": "https://github.com/sagemath/sage/issues/6902",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6902#issuecomment-50693",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

**Changing merged** from "Sage 4.1.2.alpha3" to "Sage 4.1.2.alpha4".



---

archive/issue_comments_050694.json:
```json
{
    "body": "<a id='comment:5'></a>\nThere is no 4.1.2.alpha3. Sage 4.1.2.alpha3 was William Stein's release for working on making the notebook a standalone package.",
    "created_at": "2009-09-27T10:39:40Z",
    "issue": "https://github.com/sagemath/sage/issues/6902",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/6902#issuecomment-50694",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:5'></a>
There is no 4.1.2.alpha3. Sage 4.1.2.alpha3 was William Stein's release for working on making the notebook a standalone package.
