# Issue 6346: [with patch; with positive review] sage-4.0.2.rc2 build test problem on ppc os x 10.5

archive/issues_006346.json:
```json
{
    "body": "this is the only problem:\n\n```\nsage -t -long \"devel/sage/sage/rings/number_field/number_field_rel.py\"\n**********************************************************************\nFile \"/Users/wstein/build/sage-4.0.2.rc2/devel/sage/sage/rings/number_field/number_field_rel.py\", line 1767:\n    sage: L.places()\nExpected:\n    [Relative number field morphism:\n    From: Number Field in b with defining polynomial x^2 - 5 over its base field\n    To:   Real Field with 106 bits of precision\n    Defn: b |--> -2.236067977499789696409173668937\n    c |--> -1.213411662762229634132131377426,\n    Relative number field morphism:\n    From: Number Field in b with defining polynomial x^2 - 5 over its base field\n    To:   Real Field with 106 bits of precision\n    Defn: b |--> 2.236067977499789696411548005367\n    c |--> -1.213411662762229634130492421800,\n    Relative number field morphism:\n    From: Number Field in b with defining polynomial x^2 - 5 over its base field\n    To:   Complex Field with 53 bits of precision\n    Defn: b |--> -2.23606797749979 - 2.22044604925031e-16*I\n    c |--> 0.606705831381115 - 1.45061224918844*I,\n    Relative number field morphism:\n    From: Number Field in b with defining polynomial x^2 - 5 over its base field\n    To:   Complex Field with 53 bits of precision \n    Defn: b |--> 2.23606797749979 - 4.44089209850063e-16*I\n    c |--> 0.606705831381115 - 1.45061224918844*I]\nGot:\n    [Relative number field morphism:\n      From: Number Field in b with defining polynomial x^2 - 5 over its base field\n      To:   Real Field with 106 bits of precision \n      Defn: b |--> -2.236067977499789696409173668937\n            c |--> -1.213411662762229634132131377426, Relative number field morphism:\n      From: Number Field in b with defining polynomial x^2 - 5 over its base field\n      To:   Real Field with 106 bits of precision \n      Defn: b |--> 2.236067977499789696411548005367\n            c |--> -1.213411662762229634130492421800, Relative number field morphism:\n      From: Number Field in b with defining polynomial x^2 - 5 over its base field\n      To:   Complex Field with 53 bits of precision\n      Defn: b |--> -2.23606797749979 - 1.11022302462516e-15*I\n            c |--> 0.606705831381116 - 1.45061224918844*I, Relative number field morphism:\n      From: Number Field in b with defining polynomial x^2 - 5 over its base field\n      To:   Complex Field with 53 bits of precision\n      Defn: b |--> 2.23606797749979 - 4.44089209850063e-16*I\n            c |--> 0.606705831381115 - 1.45061224918844*I]\n**********************************************************************\n1 items had failures:\n   1 of   4 in __main__.example_54\n***Test Failed*** 1 failures.\nFor whitespace errors, see the file /Users/wstein/build/sage-4.0.2.rc2/tmp/.doctest_number_field_rel.py\n         [46.5 s]\n```\n\nIt's just numerical noise I think.  Replace some e- term by ... to fix.\n\nAssignee: tbd\n\nResolution: fixed\n\nAuthor: William Stein\n\nReviewer: Craig Citro\n\nMerged: 4.0.2.rc3\n\nIssue created by migration from https://trac.sagemath.org/ticket/6346\n\n",
    "closed_at": "2009-06-17T23:58:11Z",
    "created_at": "2009-06-17T08:40:58Z",
    "labels": [
        "component: doctest coverage",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.0.2",
    "title": "[with patch; with positive review] sage-4.0.2.rc2 build test problem on ppc os x 10.5",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/6346",
    "user": "https://github.com/williamstein"
}
```
this is the only problem:

```
sage -t -long "devel/sage/sage/rings/number_field/number_field_rel.py"
**********************************************************************
File "/Users/wstein/build/sage-4.0.2.rc2/devel/sage/sage/rings/number_field/number_field_rel.py", line 1767:
    sage: L.places()
Expected:
    [Relative number field morphism:
    From: Number Field in b with defining polynomial x^2 - 5 over its base field
    To:   Real Field with 106 bits of precision
    Defn: b |--> -2.236067977499789696409173668937
    c |--> -1.213411662762229634132131377426,
    Relative number field morphism:
    From: Number Field in b with defining polynomial x^2 - 5 over its base field
    To:   Real Field with 106 bits of precision
    Defn: b |--> 2.236067977499789696411548005367
    c |--> -1.213411662762229634130492421800,
    Relative number field morphism:
    From: Number Field in b with defining polynomial x^2 - 5 over its base field
    To:   Complex Field with 53 bits of precision
    Defn: b |--> -2.23606797749979 - 2.22044604925031e-16*I
    c |--> 0.606705831381115 - 1.45061224918844*I,
    Relative number field morphism:
    From: Number Field in b with defining polynomial x^2 - 5 over its base field
    To:   Complex Field with 53 bits of precision 
    Defn: b |--> 2.23606797749979 - 4.44089209850063e-16*I
    c |--> 0.606705831381115 - 1.45061224918844*I]
Got:
    [Relative number field morphism:
      From: Number Field in b with defining polynomial x^2 - 5 over its base field
      To:   Real Field with 106 bits of precision 
      Defn: b |--> -2.236067977499789696409173668937
            c |--> -1.213411662762229634132131377426, Relative number field morphism:
      From: Number Field in b with defining polynomial x^2 - 5 over its base field
      To:   Real Field with 106 bits of precision 
      Defn: b |--> 2.236067977499789696411548005367
            c |--> -1.213411662762229634130492421800, Relative number field morphism:
      From: Number Field in b with defining polynomial x^2 - 5 over its base field
      To:   Complex Field with 53 bits of precision
      Defn: b |--> -2.23606797749979 - 1.11022302462516e-15*I
            c |--> 0.606705831381116 - 1.45061224918844*I, Relative number field morphism:
      From: Number Field in b with defining polynomial x^2 - 5 over its base field
      To:   Complex Field with 53 bits of precision
      Defn: b |--> 2.23606797749979 - 4.44089209850063e-16*I
            c |--> 0.606705831381115 - 1.45061224918844*I]
**********************************************************************
1 items had failures:
   1 of   4 in __main__.example_54
***Test Failed*** 1 failures.
For whitespace errors, see the file /Users/wstein/build/sage-4.0.2.rc2/tmp/.doctest_number_field_rel.py
         [46.5 s]
```

It's just numerical noise I think.  Replace some e- term by ... to fix.

Assignee: tbd

Resolution: fixed

Author: William Stein

Reviewer: Craig Citro

Merged: 4.0.2.rc3

Issue created by migration from https://trac.sagemath.org/ticket/6346





---

archive/issue_events_025507.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/6346",
    "rename": {
        "from": "sage-4.0.2.rc2 build test problem on ppc os x 10.5",
        "to": "[with patch; needs review] sage-4.0.2.rc2 build test problem on ppc os x 10.5"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6346#event-25507"
}
```



---

archive/issue_comments_055532.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [trac_6346.patch](tarball://root/attachments/some-uuid/ticket6346/trac_6346.patch) by @williamstein created at 2009-06-17 09:05:29",
    "created_at": "2009-06-17T09:05:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6346",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6346#issuecomment-55532",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'></a>
Attachment [trac_6346.patch](tarball://root/attachments/some-uuid/ticket6346/trac_6346.patch) by @williamstein created at 2009-06-17 09:05:29



---

archive/issue_events_025508.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/6346",
    "rename": {
        "from": "[with patch; needs review] sage-4.0.2.rc2 build test problem on ppc os x 10.5",
        "to": "[with patch; with positive review] sage-4.0.2.rc2 build test problem on ppc os x 10.5"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6346#event-25508"
}
```



---

archive/issue_comments_055533.json:
```json
{
    "body": "<a id='comment:2'></a>\nLooks good, and it fixes the issue.",
    "created_at": "2009-06-17T09:10:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6346",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6346#issuecomment-55533",
    "user": "https://github.com/craigcitro"
}
```

<a id='comment:2'></a>
Looks good, and it fixes the issue.



---

archive/issue_comments_055534.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-06-17T23:58:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6346",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6346#issuecomment-55534",
    "user": "https://github.com/craigcitro"
}
```

Resolution: fixed



---

archive/issue_comments_055535.json:
```json
{
    "body": "Changing merged from \"\" to \"4.0.2.rc3\"",
    "created_at": "2009-06-17T23:58:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6346",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6346#issuecomment-55535",
    "user": "https://github.com/craigcitro"
}
```

Changing merged from "" to "4.0.2.rc3"



---

archive/issue_comments_055536.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Craig Citro\"",
    "created_at": "2009-06-17T23:58:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6346",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6346#issuecomment-55536",
    "user": "https://github.com/craigcitro"
}
```

Changing reviewer from "" to "Craig Citro"



---

archive/issue_comments_055537.json:
```json
{
    "body": "Changing author from \"\" to \"William Stein\"",
    "created_at": "2009-06-17T23:58:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6346",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6346#issuecomment-55537",
    "user": "https://github.com/craigcitro"
}
```

Changing author from "" to "William Stein"



---

archive/issue_events_025509.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2009-06-17T23:58:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/6346",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/6346#event-25509"
}
```
