# Issue 13920: Non-ASCII character in smallgraphs.py

archive/issues_013716.json:
```json
{
    "assignees": [],
    "body": "\n```\nSyntaxError: Non-ASCII character '\\xe2' in file /home/ncohen/.Sage/local/lib/python/site-packages/sage/graphs/generators/smallgraphs.py on line 281\n```\n\nDepends on #13862\n\nAssignee: **@jasongrout**\n\nReviewer: **Nathann Cohen**\n\nAuthor: **Jeroen Demeyer**\n\nMerged: **sage-5.6.beta3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13920_\n\n",
    "closed_at": "2013-01-07T20:59:10Z",
    "created_at": "2013-01-07T09:54:50Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Non-ASCII character in smallgraphs.py",
    "type": "issue",
    "updated_at": "2013-01-07T20:59:10Z",
    "url": "https://github.com/sagemath/sage/issues/13920",
    "user": "https://github.com/jdemeyer"
}
```

```
SyntaxError: Non-ASCII character '\xe2' in file /home/ncohen/.Sage/local/lib/python/site-packages/sage/graphs/generators/smallgraphs.py on line 281
```

Depends on #13862

Assignee: **@jasongrout**

Reviewer: **Nathann Cohen**

Author: **Jeroen Demeyer**

Merged: **sage-5.6.beta3**

_Issue created by migration from https://trac.sagemath.org/ticket/13920_





---

archive/issue_events_179284.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-07T09:54:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13920",
    "milestone_number": null,
    "milestone_title": "sage-5.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13920#event-179284"
}
```



---

archive/issue_events_179285.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-07T09:54:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13920",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13920#event-179285"
}
```



---

archive/issue_events_179286.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-07T09:54:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13920",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13920#event-179286"
}
```



---

archive/issue_comments_167078.json:
```json
{
    "body": "Dependencies: **#13862**",
    "created_at": "2013-01-07T09:56:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13920",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13920#issuecomment-167078",
    "user": "https://github.com/jdemeyer"
}
```

Dependencies: **#13862**



---

archive/issue_comments_167079.json:
```json
{
    "body": "Attachment: **[13920.patch.gz](https://github.com/sagemath/sage/files/ticket13920/13920.patch.gz)**",
    "created_at": "2013-01-07T09:58:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13920",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13920#issuecomment-167079",
    "user": "https://github.com/jdemeyer"
}
```

Attachment: **[13920.patch.gz](https://github.com/sagemath/sage/files/ticket13920/13920.patch.gz)**



---

archive/issue_events_179287.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-07T09:59:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13920",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13920#event-179287"
}
```



---

archive/issue_comments_167080.json:
```json
{
    "body": "Reviewer: **Nathann Cohen**",
    "created_at": "2013-01-07T10:54:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13920",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13920#issuecomment-167080",
    "user": "https://github.com/nathanncohen"
}
```

Reviewer: **Nathann Cohen**



---

archive/issue_events_179288.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-01-07T10:54:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13920",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13920#event-179288"
}
```



---

archive/issue_events_179289.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-01-07T10:54:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13920",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13920#event-179289"
}
```



---

archive/issue_comments_167081.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nGood to go ! Sorry for the delay, I had to compile beta2 again `:-)`\n\nNathann",
    "created_at": "2013-01-07T10:54:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13920",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13920#issuecomment-167081",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:3" align="right">Comment 3</div>

Good to go ! Sorry for the delay, I had to compile beta2 again `:-)`

Nathann



---

archive/issue_events_179290.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-07T20:59:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13920",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13920#event-179290"
}
```



---

archive/issue_events_179291.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-07T20:59:10Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13920",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13920#event-179291"
}
```



---

archive/issue_comments_167082.json:
```json
{
    "body": "Merged: **sage-5.6.beta3**",
    "created_at": "2013-01-07T20:59:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13920",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13920#issuecomment-167082",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.6.beta3**
