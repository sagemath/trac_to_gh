# Issue 13885: Fix optional magma tests

archive/issues_013681.json:
```json
{
    "body": "There are, once again, a huge number of failures.\n\nAssignee: @williamstein\n\nCC:  @videlec @slel\n\nKeywords: interface, pexpect, doctesting, magma\n\nStatus: needs_info\n\nIssue created by migration from https://trac.sagemath.org/ticket/13885\n\n",
    "created_at": "2012-12-29T23:32:47Z",
    "labels": [
        "component: interfaces: optional",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-7.4",
    "title": "Fix optional magma tests",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/13885",
    "user": "https://github.com/robertwb"
}
```
There are, once again, a huge number of failures.

Assignee: @williamstein

CC:  @videlec @slel

Keywords: interface, pexpect, doctesting, magma

Status: needs_info

Issue created by migration from https://trac.sagemath.org/ticket/13885





---

archive/issue_comments_211338.json:
```json
{
    "body": "<a id='comment:1'></a>So this doesn't happen again, re-enable these via #13540.\n\n```\negrep -l -r 'optional.*magma' devel/sage/sage | egrep '\\.pyx?' | xargs ./sage -tp 10 --only-optional=magma\n[...]\n\tsage -t --only-optional=magma devel/sage/sage/rings/number_field/number_field_element.pyx # 2 doctests failed\n\tsage -t --only-optional=magma devel/sage/sage/rings/polynomial/multi_polynomial_sequence.py # 1 doctests failed\n\tsage -t --only-optional=magma devel/sage/sage/rings/polynomial/multi_polynomial_ideal.py # 1 doctests failed\n\tsage -t --only-optional=magma devel/sage/sage/rings/number_field/number_field.py # 4 doctests failed\n\tsage -t --only-optional=magma devel/sage/sage/coding/linear_code.py # 1 doctests failed\n\tsage -t --only-optional=magma devel/sage/sage/rings/polynomial/pbori.pyx # 3 doctests failed\n\tsage -t --only-optional=magma devel/sage/sage/crypto/mq/mpolynomialsystem.py # 1 doctests failed\n\tsage -t --only-optional=magma devel/sage/sage/algebras/quatalg/quaternion_algebra.py # 2 doctests failed\n\tsage -t --only-optional=magma devel/sage/sage/rings/polynomial/term_order.py # 1 doctests failed\n\tsage -t --only-optional=magma devel/sage/sage/rings/polynomial/multi_polynomial_ring_generic.pyx # 5 doctests failed\n\tsage -t --only-optional=magma devel/sage/sage/schemes/plane_conics/con_field.py # 14 doctests failed\n\tsage -t --only-optional=magma devel/sage/sage/rings/integer_ring.pyx # 1 doctests failed\n\tsage -t --only-optional=magma devel/sage/sage/interfaces/magma.py # 20 doctests failed\n\tsage -t --only-optional=magma devel/sage/sage/schemes/plane_conics/con_number_field.py # 1 doctests failed\n```",
    "created_at": "2012-12-30T00:57:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13885#issuecomment-211338",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:1'></a>So this doesn't happen again, re-enable these via #13540.

```
egrep -l -r 'optional.*magma' devel/sage/sage | egrep '\.pyx?' | xargs ./sage -tp 10 --only-optional=magma
[...]
	sage -t --only-optional=magma devel/sage/sage/rings/number_field/number_field_element.pyx # 2 doctests failed
	sage -t --only-optional=magma devel/sage/sage/rings/polynomial/multi_polynomial_sequence.py # 1 doctests failed
	sage -t --only-optional=magma devel/sage/sage/rings/polynomial/multi_polynomial_ideal.py # 1 doctests failed
	sage -t --only-optional=magma devel/sage/sage/rings/number_field/number_field.py # 4 doctests failed
	sage -t --only-optional=magma devel/sage/sage/coding/linear_code.py # 1 doctests failed
	sage -t --only-optional=magma devel/sage/sage/rings/polynomial/pbori.pyx # 3 doctests failed
	sage -t --only-optional=magma devel/sage/sage/crypto/mq/mpolynomialsystem.py # 1 doctests failed
	sage -t --only-optional=magma devel/sage/sage/algebras/quatalg/quaternion_algebra.py # 2 doctests failed
	sage -t --only-optional=magma devel/sage/sage/rings/polynomial/term_order.py # 1 doctests failed
	sage -t --only-optional=magma devel/sage/sage/rings/polynomial/multi_polynomial_ring_generic.pyx # 5 doctests failed
	sage -t --only-optional=magma devel/sage/sage/schemes/plane_conics/con_field.py # 14 doctests failed
	sage -t --only-optional=magma devel/sage/sage/rings/integer_ring.pyx # 1 doctests failed
	sage -t --only-optional=magma devel/sage/sage/interfaces/magma.py # 20 doctests failed
	sage -t --only-optional=magma devel/sage/sage/schemes/plane_conics/con_number_field.py # 1 doctests failed
```



---

archive/issue_events_038955.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13885",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13885#event-38955"
}
```



---

archive/issue_events_038956.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13885",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13885#event-38956"
}
```



---

archive/issue_events_038957.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13885",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13885#event-38957"
}
```



---

archive/issue_events_038958.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13885",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13885#event-38958"
}
```



---

archive/issue_events_038959.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13885",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13885#event-38959"
}
```



---

archive/issue_events_038960.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13885",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13885#event-38960"
}
```



---

archive/issue_events_038961.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13885",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13885#event-38961"
}
```



---

archive/issue_comments_211339.json:
```json
{
    "body": "Changing component from interfaces to interfaces: optional.",
    "created_at": "2015-06-23T13:49:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13885#issuecomment-211339",
    "user": "https://github.com/jdemeyer"
}
```

Changing component from interfaces to interfaces: optional.



---

archive/issue_events_038962.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2016-04-13T02:15:19Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13885",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13885#event-38962"
}
```



---

archive/issue_events_038963.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2016-04-13T02:15:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13885",
    "milestone": "sage-7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13885#event-38963"
}
```



---

archive/issue_comments_211340.json:
```json
{
    "body": "<a id='comment:8'></a>I think this is more like a problem with the testing framework. Here is a sample block that fails the test:\n\n```\nsage: C = codes.HammingCode(3,GF(2))\nsage: Cm = magma(C)                 # optional - magma, indirect doctest\nsage: Cm.MinimumWeight()            # optional - magma\n3\n```\n\nThe problem with this (and I suspect most of the others) is that when the ``--only-optional=magma`` flag prevents the first line in the block above to get executed, and thus C remains undefined and causes a failure in the second line.\n\nI think that having to write the ``# optional - magma`` thing in each line of a block is quite awful. It would be better that the ``--only-optional=magma`` flag ran (in full) those blocks containing at least one ``# optional - magma`` line.\n\nI don't know how to do this myself, though...",
    "created_at": "2016-04-15T08:32:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13885#issuecomment-211340",
    "user": "https://github.com/mmasdeu"
}
```

<a id='comment:8'></a>I think this is more like a problem with the testing framework. Here is a sample block that fails the test:

```
sage: C = codes.HammingCode(3,GF(2))
sage: Cm = magma(C)                 # optional - magma, indirect doctest
sage: Cm.MinimumWeight()            # optional - magma
3
```

The problem with this (and I suspect most of the others) is that when the ``--only-optional=magma`` flag prevents the first line in the block above to get executed, and thus C remains undefined and causes a failure in the second line.

I think that having to write the ``# optional - magma`` thing in each line of a block is quite awful. It would be better that the ``--only-optional=magma`` flag ran (in full) those blocks containing at least one ``# optional - magma`` line.

I don't know how to do this myself, though...



---

archive/issue_comments_211341.json:
```json
{
    "body": "<a id='comment:9'></a>The proper way to run magma doctest is with `sage -t -optional=magma,sage`. The optional `sage` applies to line without `# optional`.",
    "created_at": "2016-04-15T12:56:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13885#issuecomment-211341",
    "user": "https://github.com/videlec"
}
```

<a id='comment:9'></a>The proper way to run magma doctest is with `sage -t -optional=magma,sage`. The optional `sage` applies to line without `# optional`.



---

archive/issue_events_038964.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2016-08-29T16:21:19Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13885",
    "milestone": "sage-7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13885#event-38964"
}
```



---

archive/issue_events_038965.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2016-08-29T16:21:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13885",
    "milestone": "sage-7.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13885#event-38965"
}
```



---

archive/issue_comments_211342.json:
```json
{
    "body": "Changing status from new to needs_info.",
    "created_at": "2016-08-29T16:21:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13885#issuecomment-211342",
    "user": "https://github.com/nexttime"
}
```

Changing status from new to needs_info.



---

archive/issue_comments_211343.json:
```json
{
    "body": "<a id='comment:10'></a>Replying to [comment:9 vdelecroix]:\n> The proper way to run magma doctest is with `sage -t -optional=magma,sage`. The optional `sage` applies to line without `# optional`.\n\n\nIs this documented well?\n\n---\n\nIs there still a problem, or can we close this ticket?  (I think I've seen some more recent one regarding the Magma interface, but I'm not sure.)",
    "created_at": "2016-08-29T16:21:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13885#issuecomment-211343",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:10'></a>Replying to [comment:9 vdelecroix]:
> The proper way to run magma doctest is with `sage -t -optional=magma,sage`. The optional `sage` applies to line without `# optional`.


Is this documented well?

---

Is there still a problem, or can we close this ticket?  (I think I've seen some more recent one regarding the Magma interface, but I'm not sure.)



---

archive/issue_comments_211344.json:
```json
{
    "body": "Changing keywords from \"\" to \"interface pexpect doctesting\".",
    "created_at": "2016-08-29T16:21:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13885#issuecomment-211344",
    "user": "https://github.com/nexttime"
}
```

Changing keywords from "" to "interface pexpect doctesting".



---

archive/issue_comments_211345.json:
```json
{
    "body": "<a id='comment:11'></a>Such more recent tickets might include\n\n- #16322 Magma doctest failures\n- #17235 magma failing tests\n- #18558 Test all installed optional packages by default\n- #20182 Automatic doctest for external softwares\n- #20388 Fix the Magma interface to work with remote installations\n- #20816 pep8 in magma interface\n\nSee also trac queries\n- [keywords contains magma](https://trac.sagemath.org/query?order=id&desc=1&keywords=~magma)\n- [summary contains magma](https://trac.sagemath.org/query?order=id&desc=1&summary=~magma)\n\nI can't answer `@`leif's questions, but someone should.",
    "created_at": "2016-08-30T07:47:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13885#issuecomment-211345",
    "user": "https://github.com/slel"
}
```

<a id='comment:11'></a>Such more recent tickets might include

- #16322 Magma doctest failures
- #17235 magma failing tests
- #18558 Test all installed optional packages by default
- #20182 Automatic doctest for external softwares
- #20388 Fix the Magma interface to work with remote installations
- #20816 pep8 in magma interface

See also trac queries
- [keywords contains magma](https://trac.sagemath.org/query?order=id&desc=1&keywords=~magma)
- [summary contains magma](https://trac.sagemath.org/query?order=id&desc=1&summary=~magma)

I can't answer `@`leif's questions, but someone should.



---

archive/issue_comments_211346.json:
```json
{
    "body": "Changing keywords from \"interface pexpect doctesting\" to \"interface, pexpect, doctesting, magma\".",
    "created_at": "2016-08-30T07:47:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13885",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13885#issuecomment-211346",
    "user": "https://github.com/slel"
}
```

Changing keywords from "interface pexpect doctesting" to "interface, pexpect, doctesting, magma".
