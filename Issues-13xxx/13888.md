# Issue 13888: Barnette Sphere

archive/issues_013684.json:
```json
{
    "assignees": [],
    "body": "The Barnette Sphere is a triangulation of the 3-sphere. This patch provides this as a simplicial complex\n\napply trac-13888-barnette-sphere-fc.patch \n\nDepends on #13943\n\nAssignee: **sage-combinat**\n\nKeywords: **simplicial sphere**\n\nReviewer: **Nathann Cohen**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\nMerged: **sage-5.7.beta2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13888_\n\n",
    "closed_at": "2013-01-30T07:34:40Z",
    "created_at": "2012-12-30T13:12:11Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Barnette Sphere",
    "type": "issue",
    "updated_at": "2013-01-30T07:34:40Z",
    "url": "https://github.com/sagemath/sage/issues/13888",
    "user": "https://github.com/fchapoton"
}
```
The Barnette Sphere is a triangulation of the 3-sphere. This patch provides this as a simplicial complex

apply trac-13888-barnette-sphere-fc.patch 

Depends on #13943

Assignee: **sage-combinat**

Keywords: **simplicial sphere**

Reviewer: **Nathann Cohen**

Author: **Frédéric Chapoton**

Merged: **sage-5.7.beta2**

_Issue created by migration from https://trac.sagemath.org/ticket/13888_





---

archive/issue_events_172499.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2012-12-30T13:12:11Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "milestone_number": null,
    "milestone_title": "sage-5.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13888#event-172499"
}
```



---

archive/issue_events_172500.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2012-12-30T13:12:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13888#event-172500"
}
```



---

archive/issue_events_172501.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2012-12-30T13:12:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13888#event-172501"
}
```



---

archive/issue_events_172502.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2012-12-30T13:12:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13888#event-172502"
}
```



---

archive/issue_comments_166456.json:
```json
{
    "body": "Changed keywords from **simplicial** to **simplicial sphere**",
    "created_at": "2012-12-30T13:25:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13888#issuecomment-166456",
    "user": "https://github.com/fchapoton"
}
```

Changed keywords from **simplicial** to **simplicial sphere**



---

archive/issue_events_172503.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2012-12-30T13:25:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13888#event-172503"
}
```



---

archive/issue_comments_166457.json:
```json
{
    "body": "Author: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2012-12-30T13:25:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13888#issuecomment-166457",
    "user": "https://github.com/fchapoton"
}
```

Author: **Frédéric Chapoton**



---

archive/issue_comments_166458.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nHey Frederic,\n\nJust to let you know, right now we are having a discussion over implementation of examples of simplicial complexes in #13566 that might have an impact on this patch.\n\nBest,\n\nTravis",
    "created_at": "2013-01-02T15:33:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13888#issuecomment-166458",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:2'>Comment 2:</a>
Hey Frederic,

Just to let you know, right now we are having a discussion over implementation of examples of simplicial complexes in #13566 that might have an impact on this patch.

Best,

Travis



---

archive/issue_events_172504.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-01-22T16:19:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13888#event-172504"
}
```



---

archive/issue_events_172505.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-01-22T16:19:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13888#event-172505"
}
```



---

archive/issue_comments_166459.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nHellooooooooooooo !!\n\nWell, I was just able to check the definition of Barnette sphere as given in the third of your references, and those coordinates are not the ones you use... So I added a new doctest that cheks that your definition is equivalent to this one. This is a simplicial complex isomorphism test, so if you end up implemnting it somewhere you can 1) use this code 2) update the doctest.\n\nAs everything else is fine, you can set this ticket to `positive_review` if you agree with those changes.\n\nHave fuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuunnn !!\n\nNathann",
    "created_at": "2013-01-22T16:19:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13888#issuecomment-166459",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:3'>Comment 3:</a>
Hellooooooooooooo !!

Well, I was just able to check the definition of Barnette sphere as given in the third of your references, and those coordinates are not the ones you use... So I added a new doctest that cheks that your definition is equivalent to this one. This is a simplicial complex isomorphism test, so if you end up implemnting it somewhere you can 1) use this code 2) update the doctest.

As everything else is fine, you can set this ticket to `positive_review` if you agree with those changes.

Have fuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuunnn !!

Nathann



---

archive/issue_comments_166460.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\n(oops `^^;`)",
    "created_at": "2013-01-22T16:20:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13888#issuecomment-166460",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:4'>Comment 4:</a>
(oops `^^;`)



---

archive/issue_events_172506.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-01-22T16:20:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13888#event-172506"
}
```



---

archive/issue_events_172507.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-01-22T16:20:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13888#event-172507"
}
```



---

archive/issue_events_172508.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-01-22T16:20:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13888#event-172508"
}
```



---

archive/issue_events_172509.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-01-22T16:20:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13888#event-172509"
}
```



---

archive/issue_comments_166461.json:
```json
{
    "body": "Dependencies: **#13943**",
    "created_at": "2013-01-22T18:04:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13888#issuecomment-166461",
    "user": "https://github.com/nathanncohen"
}
```

Dependencies: **#13943**



---

archive/issue_comments_166462.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nAttachment: **[trac-13888-barnette-sphere-fc.patch.gz](https://github.com/sagemath/sage/files/ticket13888/trac-13888-barnette-sphere-fc.patch.gz)**\n\napply trac-13888-barnette-sphere-fc.patch",
    "created_at": "2013-01-23T10:55:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13888#issuecomment-166462",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:7'>Comment 7:</a>
Attachment: **[trac-13888-barnette-sphere-fc.patch.gz](https://github.com/sagemath/sage/files/ticket13888/trac-13888-barnette-sphere-fc.patch.gz)**

apply trac-13888-barnette-sphere-fc.patch



---

archive/issue_comments_166463.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n The Barnette Sphere is a triangulation of the 3-sphere. This patch provides this as a simplicial complex\n+\n+apply trac-13888-barnette-sphere-fc.patch \n``````\n",
    "created_at": "2013-01-23T10:55:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13888#issuecomment-166463",
    "user": "https://github.com/fchapoton"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
 The Barnette Sphere is a triangulation of the 3-sphere. This patch provides this as a simplicial complex
+
+apply trac-13888-barnette-sphere-fc.patch 
``````




---

archive/issue_comments_166464.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nMuuuuuuuuuuuuch better than previously, thanks to `is_isomorphic`. And it passes all long (and veeeeery long doctests). Good to go !\n\nNathann",
    "created_at": "2013-01-23T11:14:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13888#issuecomment-166464",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:8'>Comment 8:</a>
Muuuuuuuuuuuuch better than previously, thanks to `is_isomorphic`. And it passes all long (and veeeeery long doctests). Good to go !

Nathann



---

archive/issue_events_172510.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-01-23T11:14:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13888#event-172510"
}
```



---

archive/issue_events_172511.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-01-23T11:14:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13888#event-172511"
}
```



---

archive/issue_events_172512.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-23T13:14:42Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "milestone_number": null,
    "milestone_title": "sage-5.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13888#event-172512"
}
```



---

archive/issue_events_172513.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-23T13:14:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "milestone_number": null,
    "milestone_title": "sage-5.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13888#event-172513"
}
```



---

archive/issue_comments_166465.json:
```json
{
    "body": "Reviewer: **Nathann Cohen**",
    "created_at": "2013-01-23T13:14:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13888#issuecomment-166465",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Nathann Cohen**



---

archive/issue_events_172514.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-25T09:41:02Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "milestone_number": null,
    "milestone_title": "sage-5.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13888#event-172514"
}
```



---

archive/issue_events_172515.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-25T09:41:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "label": "https://github.com/sagemath/sage/labels/pending",
    "label_color": "008080",
    "label_name": "pending",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13888#event-172515"
}
```



---

archive/issue_events_172516.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-26T10:52:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "milestone_number": null,
    "milestone_title": "sage-5.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13888#event-172516"
}
```



---

archive/issue_events_172517.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-26T10:52:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "label": "https://github.com/sagemath/sage/labels/pending",
    "label_color": "008080",
    "label_name": "pending",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13888#event-172517"
}
```



---

archive/issue_events_172518.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-30T07:34:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13888#event-172518"
}
```



---

archive/issue_events_172519.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-30T07:34:40Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13888#event-172519"
}
```



---

archive/issue_comments_166466.json:
```json
{
    "body": "Merged: **sage-5.7.beta2**",
    "created_at": "2013-01-30T07:34:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13888",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13888#issuecomment-166466",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.7.beta2**
