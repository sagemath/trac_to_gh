# Issue 13449: Problem with testjava.sh in jmol package finding bash

archive/issues_013277.json:
```json
{
    "assignees": [],
    "body": "The package jmol contains testjava.sh.  The top of the file is\n\n#!/bin/bash\n\nUnfortunately in FreeBSD, this has to be one of:\n\n#!/usr/local/bin/bash\nor\n#!/bin/sh\n\n**spkg**: [http://www.uwosh.edu/faculty_staff/gutow/jmol-12.3.27.p3.spkg](http://www.uwosh.edu/faculty_staff/gutow/jmol-12.3.27.p3.spkg)\n\nAssignee: **@gutow**\n\nCC:  @nexttime\n\nKeywords: **FreeBSD jmol testjava.sh**\n\nReviewer: **Dmitrii Pasechnik**\n\nAuthor: **Jonathan Gutow**\n\nMerged: **sage-5.4.beta2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13449_\n\n",
    "closed_at": "2012-09-18T06:39:38Z",
    "created_at": "2012-09-12T03:35:30Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20scripts",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Problem with testjava.sh in jmol package finding bash",
    "type": "issue",
    "updated_at": "2012-09-18T06:39:38Z",
    "url": "https://github.com/sagemath/sage/issues/13449",
    "user": "https://github.com/sagetrac-stephen"
}
```
The package jmol contains testjava.sh.  The top of the file is

#!/bin/bash

Unfortunately in FreeBSD, this has to be one of:

#!/usr/local/bin/bash
or
#!/bin/sh

**spkg**: [http://www.uwosh.edu/faculty_staff/gutow/jmol-12.3.27.p3.spkg](http://www.uwosh.edu/faculty_staff/gutow/jmol-12.3.27.p3.spkg)

Assignee: **@gutow**

CC:  @nexttime

Keywords: **FreeBSD jmol testjava.sh**

Reviewer: **Dmitrii Pasechnik**

Author: **Jonathan Gutow**

Merged: **sage-5.4.beta2**

_Issue created by migration from https://trac.sagemath.org/ticket/13449_





---

archive/issue_events_172345.json:
```json
{
    "actor": "https://github.com/sagetrac-stephen",
    "created_at": "2012-09-12T03:35:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13449",
    "milestone_number": null,
    "milestone_title": "sage-5.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13449#event-172345"
}
```



---

archive/issue_events_172346.json:
```json
{
    "actor": "https://github.com/sagetrac-stephen",
    "created_at": "2012-09-12T03:35:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13449",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20scripts",
    "label_color": "000080",
    "label_name": "component: scripts",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13449#event-172346"
}
```



---

archive/issue_events_172347.json:
```json
{
    "actor": "https://github.com/sagetrac-stephen",
    "created_at": "2012-09-12T03:35:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13449",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13449#event-172347"
}
```



---

archive/issue_events_172348.json:
```json
{
    "actor": "https://github.com/sagetrac-stephen",
    "created_at": "2012-09-12T03:35:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13449",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13449#event-172348"
}
```



---

archive/issue_comments_159545.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nHow about `#!/usr/bin/env bash`? That is used at the top of many Sage scripts.",
    "created_at": "2012-09-12T03:46:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13449",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13449#issuecomment-159545",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:1'>Comment 1:</a>
How about `#!/usr/bin/env bash`? That is used at the top of many Sage scripts.



---

archive/issue_comments_159546.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nYes,  #!/usr/bin/env bash works.",
    "created_at": "2012-09-12T09:38:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13449",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13449#issuecomment-159546",
    "user": "https://github.com/sagetrac-stephen"
}
```

<a id='comment:2'>Comment 2:</a>
Yes,  #!/usr/bin/env bash works.



---

archive/issue_comments_159547.json:
```json
{
    "body": "Changed assignee from **@nexttime** to **@gutow**",
    "created_at": "2012-09-13T14:15:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13449",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13449#issuecomment-159547",
    "user": "https://github.com/gutow"
}
```

Changed assignee from **@nexttime** to **@gutow**



---

archive/issue_comments_159548.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nI've created a new .spkg that uses \n\n```\n#!/usr/bin/env bash\n```\nIt can be downloaded [here](http://www.uwosh.edu/faculty_staff/gutow/jmol-12.3.27.p3.spkg)\n\nTo install\n\n```\n./sage -f http://www.uwosh.edu/faculty_staff/gutow/jmol-12.3.27.p3.spkg\n```\n\nPlease review.",
    "created_at": "2012-09-15T18:40:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13449",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13449#issuecomment-159548",
    "user": "https://github.com/gutow"
}
```

<a id='comment:4'>Comment 4:</a>
I've created a new .spkg that uses 

```
#!/usr/bin/env bash
```
It can be downloaded [here](http://www.uwosh.edu/faculty_staff/gutow/jmol-12.3.27.p3.spkg)

To install

```
./sage -f http://www.uwosh.edu/faculty_staff/gutow/jmol-12.3.27.p3.spkg
```

Please review.



---

archive/issue_events_172349.json:
```json
{
    "actor": "https://github.com/gutow",
    "created_at": "2012-09-15T18:41:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13449",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13449#event-172349"
}
```



---

archive/issue_comments_159549.json:
```json
{
    "body": "patch - for reviewing purposes",
    "created_at": "2012-09-16T12:15:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13449",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13449#issuecomment-159549",
    "user": "https://github.com/dimpase"
}
```

patch - for reviewing purposes



---

archive/issue_comments_159550.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nAttachment: **[13449.patch.gz](https://github.com/sagemath/sage/files/ticket13449/13449.patch.gz)**\n\nReplying to [@gutow](#comment%3A5):\n\nlooks good, positive review.",
    "created_at": "2012-09-16T12:17:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13449",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13449#issuecomment-159550",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:6'>Comment 6:</a>
Attachment: **[13449.patch.gz](https://github.com/sagemath/sage/files/ticket13449/13449.patch.gz)**

Replying to [@gutow](#comment%3A5):

looks good, positive review.



---

archive/issue_events_172350.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2012-09-16T12:17:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13449",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13449#event-172350"
}
```



---

archive/issue_events_172351.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2012-09-16T12:17:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13449",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13449#event-172351"
}
```



---

archive/issue_comments_159551.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -8,3 +8,4 @@\n or\n #!/bin/sh\n \n+**spkg**: [http://www.uwosh.edu/faculty_staff/gutow/jmol-12.3.27.p3.spkg](http://www.uwosh.edu/faculty_staff/gutow/jmol-12.3.27.p3.spkg)\n``````\n",
    "created_at": "2012-09-16T19:49:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13449",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13449#issuecomment-159551",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -8,3 +8,4 @@
 or
 #!/bin/sh
 
+**spkg**: [http://www.uwosh.edu/faculty_staff/gutow/jmol-12.3.27.p3.spkg](http://www.uwosh.edu/faculty_staff/gutow/jmol-12.3.27.p3.spkg)
``````




---

archive/issue_comments_159552.json:
```json
{
    "body": "Author: **Jonathan Gutow**",
    "created_at": "2012-09-16T19:49:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13449",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13449#issuecomment-159552",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Jonathan Gutow**



---

archive/issue_comments_159553.json:
```json
{
    "body": "Reviewer: **Dmitrii Pasechnik**",
    "created_at": "2012-09-16T19:49:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13449",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13449#issuecomment-159553",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Dmitrii Pasechnik**



---

archive/issue_comments_159554.json:
```json
{
    "body": "Merged: **sage-5.4.beta2**",
    "created_at": "2012-09-18T06:39:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13449",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13449#issuecomment-159554",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.4.beta2**



---

archive/issue_events_172352.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-09-18T06:39:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13449",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13449#event-172352"
}
```



---

archive/issue_events_172353.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-09-18T06:39:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13449",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13449#event-172353"
}
```
