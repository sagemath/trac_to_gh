# Issue 13824: ZeroDivisionError in finite field __contains__

archive/issues_013620.json:
```json
{
    "body": "The `__contains__` method should return ``False`` if the element does not make sense in the finite field, but instead it blows up with a zero division error if the denominator is zero in the finite field:\n\n```\nsage: 4/3 in GF(3)\nERROR: An unexpected error occurred while tokenizing input\nThe following traceback may be corrupted or invalid\nThe error message is: ('EOF in multi-line statement', (1931, 0))\n\nERROR: An unexpected error occurred while tokenizing input\nThe following traceback may be corrupted or invalid\nThe error message is: ('EOF in multi-line statement', (1936, 0))\n\n---------------------------------------------------------------------------\nZeroDivisionError                         Traceback (most recent call last)\n\n/home/vbraun/Documents/Ftheory/ToricFibrationsP2/TateShafarevich/<ipython console> in <module>()\n\n/home/vbraun/opt/sage-5.5.rc0/local/lib/python2.7/site-packages/sage/structure/parent.so in sage.structure.parent.Parent.__contains__ (sage/structure/parent.c:7845)()\n\n/home/vbraun/opt/sage-5.5.rc0/local/lib/python2.7/site-packages/sage/structure/parent.so in sage.structure.parent.Parent.__call__ (sage/structure/parent.c:7305)()\n\n/home/vbraun/opt/sage-5.5.rc0/local/lib/python2.7/site-packages/sage/structure/coerce_maps.so in sage.structure.coerce_maps.DefaultConvertMap_unique._call_ (sage/structure/coerce_maps.c:3583)()\n\n/home/vbraun/opt/sage-5.5.rc0/local/lib/python2.7/site-packages/sage/structure/coerce_maps.so in sage.structure.coerce_maps.DefaultConvertMap_unique._call_ (sage/structure/coerce_maps.c:3485)()\n\n/home/vbraun/opt/sage-5.5.rc0/local/lib/python2.7/site-packages/sage/rings/finite_rings/integer_mod_ring.pyc in _element_constructor_(self, x)\n    928         \"\"\"\n    929         try:\n--> 930             return integer_mod.IntegerMod(self, x)\n    931         except (NotImplementedError, PariError):\n    932             raise TypeError, \"error coercing to finite field\"\n\n/home/vbraun/opt/sage-5.5.rc0/local/lib/python2.7/site-packages/sage/rings/finite_rings/integer_mod.so in sage.rings.finite_rings.integer_mod.IntegerMod (sage/rings/finite_rings/integer_mod.c:3501)()\n\n/home/vbraun/opt/sage-5.5.rc0/local/lib/python2.7/site-packages/sage/rings/finite_rings/integer_mod.so in sage.rings.finite_rings.integer_mod.IntegerMod_int.__init__ (sage/rings/finite_rings/integer_mod.c:19191)()\n\n/home/vbraun/opt/sage-5.5.rc0/local/lib/python2.7/site-packages/sage/rings/rational.so in sage.rings.rational.Rational.__mod__ (sage/rings/rational.c:20034)()\n\n/home/vbraun/opt/sage-5.5.rc0/local/lib/python2.7/site-packages/sage/rings/integer.so in sage.rings.integer.Integer.inverse_mod (sage/rings/integer.c:32846)()\n\nZeroDivisionError: Inverse does not exist.\n```\n\nAssignee: @aghitza\n\nCC:  sage-combinat\n\nKeywords: beginner\n\nReviewer: Volker Braun, S\u00e9bastien Labb\u00e9\n\nAuthor: Travis Scrimshaw\n\nMerged: sage-5.7.beta1\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/13824\n\n",
    "closed_at": "2013-01-26T09:54:18Z",
    "created_at": "2012-12-12T10:41:55Z",
    "labels": [
        "component: basic arithmetic",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-5.7",
    "title": "ZeroDivisionError in finite field __contains__",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/13824",
    "user": "https://github.com/vbraun"
}
```
The `__contains__` method should return ``False`` if the element does not make sense in the finite field, but instead it blows up with a zero division error if the denominator is zero in the finite field:

```
sage: 4/3 in GF(3)
ERROR: An unexpected error occurred while tokenizing input
The following traceback may be corrupted or invalid
The error message is: ('EOF in multi-line statement', (1931, 0))

ERROR: An unexpected error occurred while tokenizing input
The following traceback may be corrupted or invalid
The error message is: ('EOF in multi-line statement', (1936, 0))

---------------------------------------------------------------------------
ZeroDivisionError                         Traceback (most recent call last)

/home/vbraun/Documents/Ftheory/ToricFibrationsP2/TateShafarevich/<ipython console> in <module>()

/home/vbraun/opt/sage-5.5.rc0/local/lib/python2.7/site-packages/sage/structure/parent.so in sage.structure.parent.Parent.__contains__ (sage/structure/parent.c:7845)()

/home/vbraun/opt/sage-5.5.rc0/local/lib/python2.7/site-packages/sage/structure/parent.so in sage.structure.parent.Parent.__call__ (sage/structure/parent.c:7305)()

/home/vbraun/opt/sage-5.5.rc0/local/lib/python2.7/site-packages/sage/structure/coerce_maps.so in sage.structure.coerce_maps.DefaultConvertMap_unique._call_ (sage/structure/coerce_maps.c:3583)()

/home/vbraun/opt/sage-5.5.rc0/local/lib/python2.7/site-packages/sage/structure/coerce_maps.so in sage.structure.coerce_maps.DefaultConvertMap_unique._call_ (sage/structure/coerce_maps.c:3485)()

/home/vbraun/opt/sage-5.5.rc0/local/lib/python2.7/site-packages/sage/rings/finite_rings/integer_mod_ring.pyc in _element_constructor_(self, x)
    928         """
    929         try:
--> 930             return integer_mod.IntegerMod(self, x)
    931         except (NotImplementedError, PariError):
    932             raise TypeError, "error coercing to finite field"

/home/vbraun/opt/sage-5.5.rc0/local/lib/python2.7/site-packages/sage/rings/finite_rings/integer_mod.so in sage.rings.finite_rings.integer_mod.IntegerMod (sage/rings/finite_rings/integer_mod.c:3501)()

/home/vbraun/opt/sage-5.5.rc0/local/lib/python2.7/site-packages/sage/rings/finite_rings/integer_mod.so in sage.rings.finite_rings.integer_mod.IntegerMod_int.__init__ (sage/rings/finite_rings/integer_mod.c:19191)()

/home/vbraun/opt/sage-5.5.rc0/local/lib/python2.7/site-packages/sage/rings/rational.so in sage.rings.rational.Rational.__mod__ (sage/rings/rational.c:20034)()

/home/vbraun/opt/sage-5.5.rc0/local/lib/python2.7/site-packages/sage/rings/integer.so in sage.rings.integer.Integer.inverse_mod (sage/rings/integer.c:32846)()

ZeroDivisionError: Inverse does not exist.
```

Assignee: @aghitza

CC:  sage-combinat

Keywords: beginner

Reviewer: Volker Braun, Sébastien Labbé

Author: Travis Scrimshaw

Merged: sage-5.7.beta1

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/13824





---

archive/issue_comments_209973.json:
```json
{
    "body": "Changing assignee from tbd to @aghitza.",
    "created_at": "2012-12-12T10:42:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13824",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13824#issuecomment-209973",
    "user": "https://github.com/vbraun"
}
```

Changing assignee from tbd to @aghitza.



---

archive/issue_comments_209974.json:
```json
{
    "body": "Changing component from PLEASE CHANGE to basic arithmetic.",
    "created_at": "2012-12-12T10:42:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13824",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13824#issuecomment-209974",
    "user": "https://github.com/vbraun"
}
```

Changing component from PLEASE CHANGE to basic arithmetic.



---

archive/issue_comments_209975.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-The `__contains__` method should return ``False`` if the element is not cyclotomic, but instead it blows up with a zero division error if the denominator is zero in the cyclotomic field:\n+The `__contains__` method should return ``False`` if the element does not make sense in the finite field, but instead it blows up with a zero division error if the denominator is zero in the finite field:\n \n ```\n sage: 4/3 in GF(3)\n``````\n",
    "created_at": "2012-12-12T10:44:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13824",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13824#issuecomment-209975",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-The `__contains__` method should return ``False`` if the element is not cyclotomic, but instead it blows up with a zero division error if the denominator is zero in the cyclotomic field:
+The `__contains__` method should return ``False`` if the element does not make sense in the finite field, but instead it blows up with a zero division error if the denominator is zero in the finite field:
 
 ```
 sage: 4/3 in GF(3)
``````




---

archive/issue_comments_209976.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2012-12-17T03:07:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13824",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13824#issuecomment-209976",
    "user": "https://github.com/tscrim"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_209977.json:
```json
{
    "body": "Changing author from \"\" to \"Travis Scrimshaw\"",
    "created_at": "2012-12-17T03:07:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13824",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13824#issuecomment-209977",
    "user": "https://github.com/tscrim"
}
```

Changing author from "" to "Travis Scrimshaw"



---

archive/issue_comments_209978.json:
```json
{
    "body": "<a id='comment:3'></a>Fixed by writing a `__contains__()` which catches the zero division exception and returns false if caught. Also fixed for `IntergerModRing`.",
    "created_at": "2012-12-17T03:07:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13824",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13824#issuecomment-209978",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:3'></a>Fixed by writing a `__contains__()` which catches the zero division exception and returns false if caught. Also fixed for `IntergerModRing`.



---

archive/issue_comments_209979.json:
```json
{
    "body": "<a id='comment:4'></a>In `Parent.__contains__` there is already a try/except block with\n\n```\n    except (TypeError, ValueError):\n        return False\n```\nI would prefer it if we would just add `ZeroDivisionError` there once and for all instead of overriding it in special parents.",
    "created_at": "2012-12-17T11:09:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13824",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13824#issuecomment-209979",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:4'></a>In `Parent.__contains__` there is already a try/except block with

```
    except (TypeError, ValueError):
        return False
```
I would prefer it if we would just add `ZeroDivisionError` there once and for all instead of overriding it in special parents.



---

archive/issue_comments_209980.json:
```json
{
    "body": "<a id='comment:5'></a>Attachment [trac_13824-gf_zero_div_error-ts.patch](tarball://root/attachments/some-uuid/ticket13824/trac_13824-gf_zero_div_error-ts.patch) by @tscrim created at 2012-12-30 02:57:19\n\nNew patch with `ZeroDivisionError` only added to `Parent.__contains__()`.",
    "created_at": "2012-12-30T02:57:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13824",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13824#issuecomment-209980",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:5'></a>Attachment [trac_13824-gf_zero_div_error-ts.patch](tarball://root/attachments/some-uuid/ticket13824/trac_13824-gf_zero_div_error-ts.patch) by @tscrim created at 2012-12-30 02:57:19

New patch with `ZeroDivisionError` only added to `Parent.__contains__()`.



---

archive/issue_comments_209981.json:
```json
{
    "body": "<a id='comment:6'></a>I started the review of this ticket. All test passed (tested on sage-5.6.rc0). Documentation builds fine without warning. As I understand it, the following command returns an error and it is fine:\n\n```\nsage: Integers(6) (15/36)\n...\nZeroDivisionError: Inverse does not exist.\n```\n\nBut we still want the following to return False even if the coercion raises the above error:\n\n```\nsage: 15/36 in Integers(6)   # with the patch applied\nFalse\n```",
    "created_at": "2013-01-24T15:10:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13824",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13824#issuecomment-209981",
    "user": "https://github.com/seblabbe"
}
```

<a id='comment:6'></a>I started the review of this ticket. All test passed (tested on sage-5.6.rc0). Documentation builds fine without warning. As I understand it, the following command returns an error and it is fine:

```
sage: Integers(6) (15/36)
...
ZeroDivisionError: Inverse does not exist.
```

But we still want the following to return False even if the coercion raises the above error:

```
sage: 15/36 in Integers(6)   # with the patch applied
False
```



---

archive/issue_comments_209982.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Volker Braun, S\u00e9bastien Labb\u00e9\"",
    "created_at": "2013-01-24T15:10:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13824",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13824#issuecomment-209982",
    "user": "https://github.com/seblabbe"
}
```

Changing reviewer from "" to "Volker Braun, Sébastien Labbé"



---

archive/issue_comments_209983.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2013-01-24T15:10:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13824",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13824#issuecomment-209983",
    "user": "https://github.com/seblabbe"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_209984.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-5.7.beta1\"",
    "created_at": "2013-01-26T09:54:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13824",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13824#issuecomment-209984",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-5.7.beta1"



---

archive/issue_events_038762.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-26T09:54:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/13824",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13824#event-38762"
}
```



---

archive/issue_comments_209985.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2013-01-26T09:54:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13824",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13824#issuecomment-209985",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed
