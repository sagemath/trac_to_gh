# Issue 13141: implement constructing the dual of a linear program

archive/issues_012969.json:
```json
{
    "body": "There is currently no support for constructing the classic LP dual of a linear program (LP).\nIt would be very useful for various reasons, last but not the least constructing certificates of\noptimality and of infeasibility of an LP.\n\n\nAssignee: @nathanncohen\n\nCC:  @nathanncohen @ppurka\n\nReviewer: Matthias Koeppe\n\nAuthor: Dima Pasechnik\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/13141\n\n",
    "closed_at": "2016-06-12T12:02:30Z",
    "created_at": "2012-06-20T09:08:48Z",
    "labels": [
        "component: linear programming"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "implement constructing the dual of a linear program",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/13141",
    "user": "https://github.com/dimpase"
}
```
There is currently no support for constructing the classic LP dual of a linear program (LP).
It would be very useful for various reasons, last but not the least constructing certificates of
optimality and of infeasibility of an LP.


Assignee: @nathanncohen

CC:  @nathanncohen @ppurka

Reviewer: Matthias Koeppe

Author: Dima Pasechnik

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/13141





---

archive/issue_comments_194461.json:
```json
{
    "body": "<a id='comment:1'></a>I've actually have a module that does construct duals, but it is a \"from scratch\" implementation of linear programs and simplex method steps for educational reasons. It is not yet ready for inclusion in Sage, but I am likely to teach linear optimization this fall and plan to post it for review by the end of the term or earlier.",
    "created_at": "2012-06-21T08:51:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13141#issuecomment-194461",
    "user": "https://github.com/novoselt"
}
```

<a id='comment:1'></a>I've actually have a module that does construct duals, but it is a "from scratch" implementation of linear programs and simplex method steps for educational reasons. It is not yet ready for inclusion in Sage, but I am likely to teach linear optimization this fall and plan to post it for review by the end of the term or earlier.



---

archive/issue_comments_194462.json:
```json
{
    "body": "<a id='comment:2'></a>Replying to [comment:1 novoselt]:\n> I've actually have a module that does construct duals, but it is a \"from scratch\" implementation of linear programs and simplex method steps for educational reasons. It is not yet ready for inclusion in Sage, but I am likely to teach linear optimization this fall and plan to post it for review by the end of the term or earlier.\n\n\nI hope it will be a solver backend rather than feature a yet another way to enter linear equations and inequalities into Sage. \n\nIMHO we need to initiate a major cleanup of these, as PPL backend, MILP backends, CVXOPT, and perhaps other places I am not aware of or don't recall each have its own way of accomplishing this task. And this certainly sucks from the userland point of view.",
    "created_at": "2012-06-21T09:01:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13141#issuecomment-194462",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:2'></a>Replying to [comment:1 novoselt]:
> I've actually have a module that does construct duals, but it is a "from scratch" implementation of linear programs and simplex method steps for educational reasons. It is not yet ready for inclusion in Sage, but I am likely to teach linear optimization this fall and plan to post it for review by the end of the term or earlier.


I hope it will be a solver backend rather than feature a yet another way to enter linear equations and inequalities into Sage. 

IMHO we need to initiate a major cleanup of these, as PPL backend, MILP backends, CVXOPT, and perhaps other places I am not aware of or don't recall each have its own way of accomplishing this task. And this certainly sucks from the userland point of view.



---

archive/issue_comments_194463.json:
```json
{
    "body": "<a id='comment:3'></a>It cannot be a solver back-end: the point is not to have another algorithm of solving LPs, but to allow students using simplex method step-by-step without worrying about arithmetic. I also made the output match the lecture notes which we were using, and input is done by matrices/vectors. The standard way of constructing linear programs in Sage is, IMHO, extremely confusing for beginners.\n\nOne of the first versions of the module was published here:\nhttp://www.sagenb.org/home/pub/3318/\nmost of the math is now screwed up, which is quite unfortunate and annoying, but it is still more or less clear what it does. Later versions also support \"revised dictionaries\" and plots for graphical solving in 2D, e.g. this exam solutions were generated by it via SageTeX:\nhttp://www.math.ualberta.ca/~novoseltsev/2011Fall373A1/final_solutions.pdf",
    "created_at": "2012-06-21T11:45:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13141#issuecomment-194463",
    "user": "https://github.com/novoselt"
}
```

<a id='comment:3'></a>It cannot be a solver back-end: the point is not to have another algorithm of solving LPs, but to allow students using simplex method step-by-step without worrying about arithmetic. I also made the output match the lecture notes which we were using, and input is done by matrices/vectors. The standard way of constructing linear programs in Sage is, IMHO, extremely confusing for beginners.

One of the first versions of the module was published here:
http://www.sagenb.org/home/pub/3318/
most of the math is now screwed up, which is quite unfortunate and annoying, but it is still more or less clear what it does. Later versions also support "revised dictionaries" and plots for graphical solving in 2D, e.g. this exam solutions were generated by it via SageTeX:
http://www.math.ualberta.ca/~novoseltsev/2011Fall373A1/final_solutions.pdf



---

archive/issue_events_036191.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13141",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13141#event-36191"
}
```



---

archive/issue_events_036192.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13141",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13141#event-36192"
}
```



---

archive/issue_events_036193.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13141",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13141#event-36193"
}
```



---

archive/issue_events_036194.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13141",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13141#event-36194"
}
```



---

archive/issue_events_036195.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13141",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13141#event-36195"
}
```



---

archive/issue_events_036196.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13141",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13141#event-36196"
}
```



---

archive/issue_events_036197.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13141",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13141#event-36197"
}
```



---

archive/issue_events_036198.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2016-04-03T20:00:27Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13141",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13141#event-36198"
}
```



---

archive/issue_events_036199.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2016-04-03T20:00:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13141",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13141#event-36199"
}
```



---

archive/issue_comments_194464.json:
```json
{
    "body": "<a id='comment:8'></a>I think this is a dup of #7290. And I think explicitly constructing dual LPs from a given LP is something that one wouldn't do with a solver. Rather set solver parameters that explicit request the primal or dual simplex method, when available. \n#7290, #18733, #18804 give access to dual information in various form.\nExplicitly dualizing is already available for the textbook implementation of the simplex method, `InteractiveLPProblem`. \n\nThat's why I'm marking this as \"duplicate\"",
    "created_at": "2016-04-03T20:00:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13141#issuecomment-194464",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:8'></a>I think this is a dup of #7290. And I think explicitly constructing dual LPs from a given LP is something that one wouldn't do with a solver. Rather set solver parameters that explicit request the primal or dual simplex method, when available. 
#7290, #18733, #18804 give access to dual information in various form.
Explicitly dualizing is already available for the textbook implementation of the simplex method, `InteractiveLPProblem`. 

That's why I'm marking this as "duplicate"



---

archive/issue_comments_194465.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2016-04-03T20:00:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13141#issuecomment-194465",
    "user": "https://github.com/mkoeppe"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_194466.json:
```json
{
    "body": "<a id='comment:9'></a>when I opened this I needed to construct Farkas certs of infeasibility, and that was (still is?) something that Sage could not do. \n\n(I have had a collection of small LP's that only PPL could solve properly, without scaling problems).",
    "created_at": "2016-04-03T22:21:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13141#issuecomment-194466",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:9'></a>when I opened this I needed to construct Farkas certs of infeasibility, and that was (still is?) something that Sage could not do. 

(I have had a collection of small LP's that only PPL could solve properly, without scaling problems).



---

archive/issue_comments_194467.json:
```json
{
    "body": "<a id='comment:10'></a>Please add your name to the author field (otherwise the patchbot will not check this ticket)",
    "created_at": "2016-04-07T18:56:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13141#issuecomment-194467",
    "user": "https://github.com/videlec"
}
```

<a id='comment:10'></a>Please add your name to the author field (otherwise the patchbot will not check this ticket)



---

archive/issue_comments_194468.json:
```json
{
    "body": "<a id='comment:11'></a>Replying to [comment:10 vdelecroix]:\n> Please add your name to the author field (otherwise the patchbot will not check this ticket)\n\n\nI better not, otherwise the patchbot might get confused by the absence of a branch on the ticket :-)",
    "created_at": "2016-04-07T20:21:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13141#issuecomment-194468",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:11'></a>Replying to [comment:10 vdelecroix]:
> Please add your name to the author field (otherwise the patchbot will not check this ticket)


I better not, otherwise the patchbot might get confused by the absence of a branch on the ticket :-)



---

archive/issue_comments_194469.json:
```json
{
    "body": "Changing author from \"\" to \"Dima Pasechnik\"",
    "created_at": "2016-05-12T08:05:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13141#issuecomment-194469",
    "user": "https://github.com/dimpase"
}
```

Changing author from "" to "Dima Pasechnik"



---

archive/issue_comments_194470.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Matthias Koeppe\"",
    "created_at": "2016-05-12T08:05:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13141#issuecomment-194470",
    "user": "https://github.com/dimpase"
}
```

Changing reviewer from "" to "Matthias Koeppe"



---

archive/issue_comments_194471.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2016-05-12T08:05:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13141#issuecomment-194471",
    "user": "https://github.com/dimpase"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_194472.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2016-06-12T12:02:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13141",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13141#issuecomment-194472",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_events_036200.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-06-12T12:02:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/13141",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13141#event-36200"
}
```
