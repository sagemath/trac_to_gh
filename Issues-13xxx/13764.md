# Issue 13764: CartesianProduct with generators -> silent wrong answer

archive/issues_013560.json:
```json
{
    "body": "This is actually documented in the docstring:\n\n```\nsage: def a(n): yield 1*n; yield 2*n\nsage: def b(): yield 'a'; yield 'b'\nsage: CartesianProduct(a(3), b()).list()\n[[3, 'a'], [3, 'b']]\n```\nBut by the principle of minimal surprise this should yield an error about generators and not silently return the wrong answer. Just to illustrate how dangerous this behavior is note that `sage/combinat/integer_vector.py` falls precisely into this trap.\n\n\n\nAssignee: sage-combinat\n\nReviewer: Nathann Cohen\n\nAuthor: Volker Braun\n\nMerged: sage-5.7.beta3\n\nDependencies: #13763\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/13764\n\n",
    "closed_at": "2013-02-05T08:19:02Z",
    "created_at": "2012-11-27T18:01:06Z",
    "labels": [
        "component: combinatorics",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-5.7",
    "title": "CartesianProduct with generators -> silent wrong answer",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/13764",
    "user": "https://github.com/vbraun"
}
```
This is actually documented in the docstring:

```
sage: def a(n): yield 1*n; yield 2*n
sage: def b(): yield 'a'; yield 'b'
sage: CartesianProduct(a(3), b()).list()
[[3, 'a'], [3, 'b']]
```
But by the principle of minimal surprise this should yield an error about generators and not silently return the wrong answer. Just to illustrate how dangerous this behavior is note that `sage/combinat/integer_vector.py` falls precisely into this trap.



Assignee: sage-combinat

Reviewer: Nathann Cohen

Author: Volker Braun

Merged: sage-5.7.beta3

Dependencies: #13763

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/13764





---

archive/issue_comments_208832.json:
```json
{
    "body": "<a id='comment:1'></a>Hellooooooo !!\n\nWell, same question as the one asked on sage-devel : why shouldn't we build the list of each iterator ? If the list is infinite then the code will not terminate, but of course the user asked for it, and if we do make lists from generators we store a quantity of information which is (expectedly) small compared to the output ?\n\nErr... Actually, if the function does not return a list but only \"yields\" the elements one at a time, what we store could actually be much, much more than what we should.\n\nHence, what about doing this :\n* If we are given two lists, just do the job without problems\n* If we are given iterators, convert them to lists first then do the job ?\n\nWith a `.. NOTE::` in the method's documentation, saying exactly that. This way we never use more than what we actually need to use.\n\nNathann",
    "created_at": "2012-11-28T14:14:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208832",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:1'></a>Hellooooooo !!

Well, same question as the one asked on sage-devel : why shouldn't we build the list of each iterator ? If the list is infinite then the code will not terminate, but of course the user asked for it, and if we do make lists from generators we store a quantity of information which is (expectedly) small compared to the output ?

Err... Actually, if the function does not return a list but only "yields" the elements one at a time, what we store could actually be much, much more than what we should.

Hence, what about doing this :
* If we are given two lists, just do the job without problems
* If we are given iterators, convert them to lists first then do the job ?

With a `.. NOTE::` in the method's documentation, saying exactly that. This way we never use more than what we actually need to use.

Nathann



---

archive/issue_comments_208833.json:
```json
{
    "body": "Changing status from new to needs_info.",
    "created_at": "2012-11-28T14:14:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208833",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from new to needs_info.



---

archive/issue_comments_208834.json:
```json
{
    "body": "<a id='comment:2'></a>Nobody forces you to traverse the whole `CartesianProduct`, you might just want 10 different elements of a product of potentially infinite iterators.",
    "created_at": "2012-11-28T14:17:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208834",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:2'></a>Nobody forces you to traverse the whole `CartesianProduct`, you might just want 10 different elements of a product of potentially infinite iterators.



---

archive/issue_comments_208835.json:
```json
{
    "body": "Changing status from needs_info to needs_review.",
    "created_at": "2012-11-28T14:17:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208835",
    "user": "https://github.com/vbraun"
}
```

Changing status from needs_info to needs_review.



---

archive/issue_comments_208836.json:
```json
{
    "body": "<a id='comment:4'></a>Oh right. Then what about, in this case build partial (local) lists of what the iterators contain, as they are explored ?\n\nNathann",
    "created_at": "2012-11-28T14:18:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208836",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:4'></a>Oh right. Then what about, in this case build partial (local) lists of what the iterators contain, as they are explored ?

Nathann



---

archive/issue_comments_208837.json:
```json
{
    "body": "<a id='comment:5'></a>There is no way to find out if a generator is infinite or not.",
    "created_at": "2012-11-28T14:27:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208837",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:5'></a>There is no way to find out if a generator is infinite or not.



---

archive/issue_comments_208838.json:
```json
{
    "body": "<a id='comment:6'></a>NOnonononoo I know ! But I was thinking of something like that :\n\nYou can easily build an iterator over the elements of `N^15` : you first need to enumerate the elements of `[0] x ... x [0]`. Then, once you are done, you can enumerate the elements of `[0,1] x ... x [0,1]` that you have not enumerate before. Then, once you are done, you can enumerate the elements of `[0,1,2] x .... x [0,1,2]` that you have not enumerated before, etc, etc. You see what I mean ? What I mean is of course a bit more complicated than that, because in this situation you do not know the cardinality of your iterators, you do not even know (and cannot, as you say) decide whether they are finite, etc... But if you \"ask a new element from an iterator\" only when you need it, you can always remember a finite partial list from the elements of each iterator, and still iterate over ALL elements !\n\nNathann",
    "created_at": "2012-11-28T14:31:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208838",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:6'></a>NOnonononoo I know ! But I was thinking of something like that :

You can easily build an iterator over the elements of `N^15` : you first need to enumerate the elements of `[0] x ... x [0]`. Then, once you are done, you can enumerate the elements of `[0,1] x ... x [0,1]` that you have not enumerate before. Then, once you are done, you can enumerate the elements of `[0,1,2] x .... x [0,1,2]` that you have not enumerated before, etc, etc. You see what I mean ? What I mean is of course a bit more complicated than that, because in this situation you do not know the cardinality of your iterators, you do not even know (and cannot, as you say) decide whether they are finite, etc... But if you "ask a new element from an iterator" only when you need it, you can always remember a finite partial list from the elements of each iterator, and still iterate over ALL elements !

Nathann



---

archive/issue_comments_208839.json:
```json
{
    "body": "<a id='comment:7'></a>(and of course there is a way to do this without remembering \"the list of things you have already returned\")\n\nOr else, a bit easier, is to return the elements in lexicographical order. It's less pretty, but easier to implement. And in this situation, you only remember from each iterator the list of elements you have already used. And you can \"ask for another element\" when needed.\n\nActually, this order of enumeration is more realistic.\nI prefered the previous one, but it is trickier to enumerate :-P\n\nNathann",
    "created_at": "2012-11-28T14:34:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208839",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:7'></a>(and of course there is a way to do this without remembering "the list of things you have already returned")

Or else, a bit easier, is to return the elements in lexicographical order. It's less pretty, but easier to implement. And in this situation, you only remember from each iterator the list of elements you have already used. And you can "ask for another element" when needed.

Actually, this order of enumeration is more realistic.
I prefered the previous one, but it is trickier to enumerate :-P

Nathann



---

archive/issue_comments_208840.json:
```json
{
    "body": "<a id='comment:8'></a>Isn't all the funky stuff part of the `*EnumeratedSet` framework that the combinat people are working on? I don't want to replace the `CartesianProduct` iterator with something that incurs significant overhead.",
    "created_at": "2012-11-28T14:40:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208840",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:8'></a>Isn't all the funky stuff part of the `*EnumeratedSet` framework that the combinat people are working on? I don't want to replace the `CartesianProduct` iterator with something that incurs significant overhead.



---

archive/issue_comments_208841.json:
```json
{
    "body": "<a id='comment:9'></a>> Isn't all the funky stuff part of the `*EnumeratedSet` framework that the combinat people are working on? I don't want to replace the `CartesianProduct` iterator with something that incurs significant overhead.\n\n\nYeah, when we need significant overhead we know that we can always delegate to combinat stuff `:-P`\n\nOf course, we could *only* do that with the elements of the list which are generators, and use sensible code otherwise. With warnings everywhere.\n\nActually, perhaps the best would be that :\n\n   There is an optional flag called cache_iterator or something, which is set to False by default. When it is set to False, the function behaves exactly like the one you wrote, that is REFUSES INPUT if there is an iterator among them. The message could even say \"if your iterators are all finite, wrap them with a `list()` to make them finite. \n\nThen, we could say (in the exception message, or in the doc, or at both places) that if you want the code to work with infinite stuff, or when you cannot ensure that all iterators are finite, and hence cannot wrap them with `list()` casts, then you can set `cache_iterator` to `True` which would do all the caching (or call the combinat stuff and enjoy their overhead if they already wrote it somewhere).\n\nHence :\n* No speed loss for smart cases\n* The speed will not be destroyed because the user forgot to wrap with `list()` calls, or just did not know it could make a difference\n* Crazy guys iterating on infinite things can still use the function, be it finally implemented in the method itself or delegated to combinat stuff.\n\nWell.. Is there any other corner case ? `:-P`\n\nNathann",
    "created_at": "2012-11-28T14:46:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208841",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:9'></a>> Isn't all the funky stuff part of the `*EnumeratedSet` framework that the combinat people are working on? I don't want to replace the `CartesianProduct` iterator with something that incurs significant overhead.


Yeah, when we need significant overhead we know that we can always delegate to combinat stuff `:-P`

Of course, we could *only* do that with the elements of the list which are generators, and use sensible code otherwise. With warnings everywhere.

Actually, perhaps the best would be that :

   There is an optional flag called cache_iterator or something, which is set to False by default. When it is set to False, the function behaves exactly like the one you wrote, that is REFUSES INPUT if there is an iterator among them. The message could even say "if your iterators are all finite, wrap them with a `list()` to make them finite. 

Then, we could say (in the exception message, or in the doc, or at both places) that if you want the code to work with infinite stuff, or when you cannot ensure that all iterators are finite, and hence cannot wrap them with `list()` casts, then you can set `cache_iterator` to `True` which would do all the caching (or call the combinat stuff and enjoy their overhead if they already wrote it somewhere).

Hence :
* No speed loss for smart cases
* The speed will not be destroyed because the user forgot to wrap with `list()` calls, or just did not know it could make a difference
* Crazy guys iterating on infinite things can still use the function, be it finally implemented in the method itself or delegated to combinat stuff.

Well.. Is there any other corner case ? `:-P`

Nathann



---

archive/issue_comments_208842.json:
```json
{
    "body": "<a id='comment:10'></a>Iterators (objects implementing `__iter__`) are fine , only generators (the `yield` keyword) don't work.\n\nI don't see the point in an extra flag to install different workarounds to generators, if you want to convert the input to a list then just do it yourself.",
    "created_at": "2012-11-28T14:54:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208842",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:10'></a>Iterators (objects implementing `__iter__`) are fine , only generators (the `yield` keyword) don't work.

I don't see the point in an extra flag to install different workarounds to generators, if you want to convert the input to a list then just do it yourself.



---

archive/issue_comments_208843.json:
```json
{
    "body": "<a id='comment:11'></a>> Iterators (objects implementing `__iter__`) are fine , only generators (the `yield` keyword) don't work.\n\n\nOops, I see. I thought that the problem came from infinite sets, sorry.. But right now iterators are copied, and so the elements they iterate over are generated several times... Scary `O_o`\n\n> I don't see the point in an extra flag to install different workarounds to generators, if you want to convert the input to a list then just do it yourself.\n\n\nWell, you cannot convert them to lists when they are infinite, and you do not want to cache stuff when you already stored it once.\n\nAnyway, sorry for the misunderstanding. I'll look at the code and start making sense, if possible.\n\nNathann",
    "created_at": "2012-11-28T14:59:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208843",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:11'></a>> Iterators (objects implementing `__iter__`) are fine , only generators (the `yield` keyword) don't work.


Oops, I see. I thought that the problem came from infinite sets, sorry.. But right now iterators are copied, and so the elements they iterate over are generated several times... Scary `O_o`

> I don't see the point in an extra flag to install different workarounds to generators, if you want to convert the input to a list then just do it yourself.


Well, you cannot convert them to lists when they are infinite, and you do not want to cache stuff when you already stored it once.

Anyway, sorry for the misunderstanding. I'll look at the code and start making sense, if possible.

Nathann



---

archive/issue_comments_208844.json:
```json
{
    "body": "<a id='comment:12'></a>Replying to [ncohen](#comment%3A11):\n> Oops, I see. I thought that the problem came from infinite sets, sorry.. But right now iterators are copied, and so the elements they iterate over are generated several times...\n\n\nThats how the cartesian product works. Shouldn't come as a surprise.\n\n> Well, you cannot convert them to lists when they are infinite, and you do not want to cache stuff when you already stored it once.\n\n\nBut you can wrap your generator into an `IterableFunctionCall` if it is potentially infinite.",
    "created_at": "2012-11-28T15:05:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208844",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:12'></a>Replying to [ncohen](#comment%3A11):
> Oops, I see. I thought that the problem came from infinite sets, sorry.. But right now iterators are copied, and so the elements they iterate over are generated several times...


Thats how the cartesian product works. Shouldn't come as a surprise.

> Well, you cannot convert them to lists when they are infinite, and you do not want to cache stuff when you already stored it once.


But you can wrap your generator into an `IterableFunctionCall` if it is potentially infinite.



---

archive/issue_comments_208845.json:
```json
{
    "body": "<a id='comment:13'></a>> Thats how the cartesian product works. Shouldn't come as a surprise.\n\n\nYeah, but you could want to cache them sometimes.. Do not generate them over and over if it takes time to generate them... Of course the opposite may also be true : it is very cheap to generate them, even over and over, but you do not want to store them.\n\n> But you can wrap your generator into an `IterableFunctionCall` if it is potentially infinite.\n\n\nYepyep. Right.\n\nNathann",
    "created_at": "2012-11-28T15:07:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208845",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:13'></a>> Thats how the cartesian product works. Shouldn't come as a surprise.


Yeah, but you could want to cache them sometimes.. Do not generate them over and over if it takes time to generate them... Of course the opposite may also be true : it is very cheap to generate them, even over and over, but you do not want to store them.

> But you can wrap your generator into an `IterableFunctionCall` if it is potentially infinite.


Yepyep. Right.

Nathann



---

archive/issue_comments_208846.json:
```json
{
    "body": "<a id='comment:14'></a>Ahahahah. Well, very easy patch in the end... Looks all good, but could you modify the exception to something like that : \"Generators are not allowed : read the method's documentation for a workaround\" ?\n\nYou just told me about `IterableFunctionCall` and I had never heard of it before `:-)`\n\nNathann",
    "created_at": "2012-11-28T15:09:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208846",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:14'></a>Ahahahah. Well, very easy patch in the end... Looks all good, but could you modify the exception to something like that : "Generators are not allowed : read the method's documentation for a workaround" ?

You just told me about `IterableFunctionCall` and I had never heard of it before `:-)`

Nathann



---

archive/issue_comments_208847.json:
```json
{
    "body": "<a id='comment:15'></a>Of and also could you please make `IterableFunctionCall` a html link, so that it is easier to find its documentation when reading the doc ?\n\n(sorryyyyyyyyyy)\n\nNathann",
    "created_at": "2012-11-28T15:10:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208847",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:15'></a>Of and also could you please make `IterableFunctionCall` a html link, so that it is easier to find its documentation when reading the doc ?

(sorryyyyyyyyyy)

Nathann



---

archive/issue_comments_208848.json:
```json
{
    "body": "Updated patch",
    "created_at": "2012-11-28T15:24:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208848",
    "user": "https://github.com/vbraun"
}
```

Updated patch



---

archive/issue_comments_208849.json:
```json
{
    "body": "<a id='comment:16'></a>Attachment [trac_13764_CartesianProduct_check.patch](tarball://root/attachments/some-uuid/ticket13764/trac_13764_CartesianProduct_check.patch) by @vbraun created at 2012-11-28 15:25:12\n\nDone",
    "created_at": "2012-11-28T15:25:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208849",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:16'></a>Attachment [trac_13764_CartesianProduct_check.patch](tarball://root/attachments/some-uuid/ticket13764/trac_13764_CartesianProduct_check.patch) by @vbraun created at 2012-11-28 15:25:12

Done



---

archive/issue_comments_208850.json:
```json
{
    "body": "<a id='comment:17'></a>Well.. Looks good, passes all tests... Sorry for the first misunderstanding `:-)`\n\nNathann",
    "created_at": "2012-11-28T16:23:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208850",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:17'></a>Well.. Looks good, passes all tests... Sorry for the first misunderstanding `:-)`

Nathann



---

archive/issue_comments_208851.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2012-11-28T16:23:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208851",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_208852.json:
```json
{
    "body": "<a id='comment:18'></a>Arggggggggg....\n\nAhem... \n\n```\nsage -t -long \"devel/sage-2/sage/combinat/root_system/associahedron.py\"\n**********************************************************************\nFile \"/home/ncohen/.Sage/devel/sage-2/sage/combinat/root_system/associahedron.py\", line 83:\n    sage: TestSuite(Asso).run()\nExpected nothing\nGot:\n    Failure in _test_pickling:\n    Traceback (most recent call last):\n      File \"/home/ncohen/.Sage/local/lib/python/site-packages/sage/misc/sage_unittest.py\", line 275, in run\n        test_method(tester = tester)\n      File \"sage_object.pyx\", line 396, in sage.structure.sage_object.SageObject._test_pickling (sage/structure/sage_object.c:3669)\n      File \"/home/ncohen/.Sage/local/lib/python2.7/unittest/case.py\", line 511, in assertEqual\n        assertion_func(first, second, msg=msg)\n      File \"/home/ncohen/.Sage/local/lib/python2.7/unittest/case.py\", line 501, in _baseAssertEqual\n        if not first == second:\n      File \"/home/ncohen/.Sage/local/lib/python/site-packages/sage/geometry/polyhedron/base.py\", line 382, in __eq__\n        return self._is_subpolyhedron(other) and other._is_subpolyhedron(self)\n      File \"/home/ncohen/.Sage/local/lib/python/site-packages/sage/geometry/polyhedron/base.py\", line 490, in _is_subpolyhedron\n        CartesianProduct(other.Hrep_generator(), self.Vrep_generator()) )\n      File \"/home/ncohen/.Sage/local/lib/python/site-packages/sage/combinat/cartesian_product.py\", line 64, in CartesianProduct\n        '(type \"CartesianProduct?\") for a workaround')\n    ValueError: generators are not allowed, see the documentation (type \"CartesianProduct?\") for a workaround\n    ------------------------------------------------------------\n    The following tests failed: _test_pickling\n**********************************************************************\n```",
    "created_at": "2012-11-28T16:33:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208852",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:18'></a>Arggggggggg....

Ahem... 

```
sage -t -long "devel/sage-2/sage/combinat/root_system/associahedron.py"
**********************************************************************
File "/home/ncohen/.Sage/devel/sage-2/sage/combinat/root_system/associahedron.py", line 83:
    sage: TestSuite(Asso).run()
Expected nothing
Got:
    Failure in _test_pickling:
    Traceback (most recent call last):
      File "/home/ncohen/.Sage/local/lib/python/site-packages/sage/misc/sage_unittest.py", line 275, in run
        test_method(tester = tester)
      File "sage_object.pyx", line 396, in sage.structure.sage_object.SageObject._test_pickling (sage/structure/sage_object.c:3669)
      File "/home/ncohen/.Sage/local/lib/python2.7/unittest/case.py", line 511, in assertEqual
        assertion_func(first, second, msg=msg)
      File "/home/ncohen/.Sage/local/lib/python2.7/unittest/case.py", line 501, in _baseAssertEqual
        if not first == second:
      File "/home/ncohen/.Sage/local/lib/python/site-packages/sage/geometry/polyhedron/base.py", line 382, in __eq__
        return self._is_subpolyhedron(other) and other._is_subpolyhedron(self)
      File "/home/ncohen/.Sage/local/lib/python/site-packages/sage/geometry/polyhedron/base.py", line 490, in _is_subpolyhedron
        CartesianProduct(other.Hrep_generator(), self.Vrep_generator()) )
      File "/home/ncohen/.Sage/local/lib/python/site-packages/sage/combinat/cartesian_product.py", line 64, in CartesianProduct
        '(type "CartesianProduct?") for a workaround')
    ValueError: generators are not allowed, see the documentation (type "CartesianProduct?") for a workaround
    ------------------------------------------------------------
    The following tests failed: _test_pickling
**********************************************************************
```



---

archive/issue_comments_208853.json:
```json
{
    "body": "Changing status from positive_review to needs_work.",
    "created_at": "2012-11-28T16:33:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208853",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from positive_review to needs_work.



---

archive/issue_comments_208854.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Nathann Cohen\"",
    "created_at": "2012-11-28T16:43:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208854",
    "user": "https://github.com/vbraun"
}
```

Changing reviewer from "" to "Nathann Cohen"



---

archive/issue_comments_208855.json:
```json
{
    "body": "Changing status from needs_work to positive_review.",
    "created_at": "2012-11-28T16:43:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208855",
    "user": "https://github.com/vbraun"
}
```

Changing status from needs_work to positive_review.



---

archive/issue_comments_208856.json:
```json
{
    "body": "<a id='comment:19'></a>The failure in polyhedra is fixed in #13763. This was where I noticed the breakage in `CartesianProduct` and came to open this ticket. I'm switching this ticket back to positive review since there is nothing to change here.",
    "created_at": "2012-11-28T16:43:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208856",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:19'></a>The failure in polyhedra is fixed in #13763. This was where I noticed the breakage in `CartesianProduct` and came to open this ticket. I'm switching this ticket back to positive review since there is nothing to change here.



---

archive/issue_comments_208857.json:
```json
{
    "body": "Changing dependencies from \"\" to \"#13763\"",
    "created_at": "2012-11-28T16:43:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208857",
    "user": "https://github.com/vbraun"
}
```

Changing dependencies from "" to "#13763"



---

archive/issue_comments_208858.json:
```json
{
    "body": "<a id='comment:20'></a>Well, then... `:-)`\n\nNathann",
    "created_at": "2012-11-28T17:01:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208858",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:20'></a>Well, then... `:-)`

Nathann



---

archive/issue_events_038579.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-12-16T14:09:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "milestone": "sage-pending",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13764#event-38579"
}
```



---

archive/issue_events_038580.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-30T16:04:05Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "milestone": "sage-pending",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13764#event-38580"
}
```



---

archive/issue_events_038581.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-30T16:04:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "milestone": "sage-5.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13764#event-38581"
}
```



---

archive/issue_comments_208859.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2013-02-05T08:19:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208859",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed



---

archive/issue_events_038582.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-05T08:19:02Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13764#event-38582"
}
```



---

archive/issue_comments_208860.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-5.7.beta3\"",
    "created_at": "2013-02-05T08:19:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13764#issuecomment-208860",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-5.7.beta3"
