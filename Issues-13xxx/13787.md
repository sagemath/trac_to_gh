# Issue 13787: Bug in GenericGraph.line_graph() when the vertices cannot be compared

archive/issues_013583.json:
```json
{
    "assignees": [],
    "body": "Reported by Georgi Guninski :\n\n```\nsage: g=graphs.KneserGraph(7,1)\nsage: C = graphs.CompleteGraph(7)\nsage: C.is_isomorphic(g)\nTrue\nsage: C.line_graph().is_isomorphic(g.line_graph())\nFalse\n```\n\nAssignee: **@jasongrout, @nathanncohen, @rlmill**\n\nCC:  @fchapoton\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Nathann Cohen**\n\nMerged: **sage-5.8.beta0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13787_\n\n",
    "closed_at": "2013-02-17T22:43:17Z",
    "created_at": "2012-12-02T09:58:15Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Bug in GenericGraph.line_graph() when the vertices cannot be compared",
    "type": "issue",
    "updated_at": "2013-02-17T22:43:17Z",
    "url": "https://github.com/sagemath/sage/issues/13787",
    "user": "https://github.com/nathanncohen"
}
```
Reported by Georgi Guninski :

```
sage: g=graphs.KneserGraph(7,1)
sage: C = graphs.CompleteGraph(7)
sage: C.is_isomorphic(g)
True
sage: C.line_graph().is_isomorphic(g.line_graph())
False
```

Assignee: **@jasongrout, @nathanncohen, @rlmill**

CC:  @fchapoton

Reviewer: **Frédéric Chapoton**

Author: **Nathann Cohen**

Merged: **sage-5.8.beta0**

_Issue created by migration from https://trac.sagemath.org/ticket/13787_





---

archive/issue_events_177335.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2012-12-02T09:58:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13787",
    "milestone_number": null,
    "milestone_title": "sage-5.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13787#event-177335"
}
```



---

archive/issue_events_177336.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2012-12-02T09:58:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13787",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13787#event-177336"
}
```



---

archive/issue_events_177337.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2012-12-02T09:58:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13787",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13787#event-177337"
}
```



---

archive/issue_events_177338.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2012-12-02T09:58:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13787",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13787#event-177338"
}
```



---

archive/issue_events_177339.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2012-12-02T10:51:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13787",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13787#event-177339"
}
```



---

archive/issue_comments_164804.json:
```json
{
    "body": "Attachment: **[trac_13787.patch.gz](https://github.com/sagemath/sage/files/ticket13787/trac_13787.patch.gz)**",
    "created_at": "2013-01-23T18:17:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13787",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13787#issuecomment-164804",
    "user": "https://github.com/nathanncohen"
}
```

Attachment: **[trac_13787.patch.gz](https://github.com/sagemath/sage/files/ticket13787/trac_13787.patch.gz)**



---

archive/issue_comments_164805.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nImplementing Frederic s remark. Much better this way `:-)`\n\nNathann",
    "created_at": "2013-01-23T18:17:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13787",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13787#issuecomment-164805",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:2'>Comment 2:</a>
Implementing Frederic s remark. Much better this way `:-)`

Nathann



---

archive/issue_comments_164806.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThis patch is ok for me. Nathann, if you are happy with the cosmetic changes in the review patch, you can set a positive review when the bot gives a green light.",
    "created_at": "2013-02-14T10:23:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13787",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13787#issuecomment-164806",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:3'>Comment 3:</a>
This patch is ok for me. Nathann, if you are happy with the cosmetic changes in the review patch, you can set a positive review when the bot gives a green light.



---

archive/issue_events_177340.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-02-14T10:31:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13787",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13787#event-177340"
}
```



---

archive/issue_events_177341.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-02-14T10:31:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13787",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13787#event-177341"
}
```



---

archive/issue_comments_164807.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2013-02-14T10:31:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13787",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13787#issuecomment-164807",
    "user": "https://github.com/nathanncohen"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_164808.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nYeahhhhhhhhhhh !! Thank you very much Frederic `:-)`\n\nMy patch got a green light by itself, and your changes (thanks for them too!) are not likely to produce bugs outside of `generic_graph.py`. I just applied both on beta4 without any problems and all tests pass on that file, so I set it to `positive_review` before I forget.\n\nNathann",
    "created_at": "2013-02-14T10:31:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13787",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13787#issuecomment-164808",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:4'>Comment 4:</a>
Yeahhhhhhhhhhh !! Thank you very much Frederic `:-)`

My patch got a green light by itself, and your changes (thanks for them too!) are not likely to produce bugs outside of `generic_graph.py`. I just applied both on beta4 without any problems and all tests pass on that file, so I set it to `positive_review` before I forget.

Nathann



---

archive/issue_events_177342.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-15T08:01:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13787",
    "milestone_number": null,
    "milestone_title": "sage-5.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13787#event-177342"
}
```



---

archive/issue_events_177343.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-15T08:01:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13787",
    "milestone_number": null,
    "milestone_title": "sage-5.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13787#event-177343"
}
```



---

archive/issue_events_177344.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-15T08:20:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13787",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13787#event-177344"
}
```



---

archive/issue_events_177345.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-15T08:20:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13787",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13787#event-177345"
}
```



---

archive/issue_comments_164809.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\n[attachment: trac-13787-review-fc.patch.gz](https://github.com/sagemath/sage/files/ticket13787/trac-13787-review-fc.patch.gz) needs a proper commit message.",
    "created_at": "2013-02-15T08:20:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13787",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13787#issuecomment-164809",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:6'>Comment 6:</a>
[attachment: trac-13787-review-fc.patch.gz](https://github.com/sagemath/sage/files/ticket13787/trac-13787-review-fc.patch.gz) needs a proper commit message.



---

archive/issue_comments_164810.json:
```json
{
    "body": "Attachment: **[trac-13787-review-fc.patch.gz](https://github.com/sagemath/sage/files/ticket13787/trac-13787-review-fc.patch.gz)**",
    "created_at": "2013-02-15T09:05:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13787",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13787#issuecomment-164810",
    "user": "https://github.com/fchapoton"
}
```

Attachment: **[trac-13787-review-fc.patch.gz](https://github.com/sagemath/sage/files/ticket13787/trac-13787-review-fc.patch.gz)**



---

archive/issue_events_177346.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-02-15T09:06:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13787",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13787#event-177346"
}
```



---

archive/issue_events_177347.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-02-15T09:06:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13787",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13787#event-177347"
}
```



---

archive/issue_comments_164811.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nadded commit message, back to positive review",
    "created_at": "2013-02-15T09:06:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13787",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13787#issuecomment-164811",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:7'>Comment 7:</a>
added commit message, back to positive review



---

archive/issue_events_177348.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-17T22:43:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13787",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13787#event-177348"
}
```



---

archive/issue_events_177349.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-17T22:43:17Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13787",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13787#event-177349"
}
```



---

archive/issue_comments_164812.json:
```json
{
    "body": "Merged: **sage-5.8.beta0**",
    "created_at": "2013-02-17T22:43:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13787",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13787#issuecomment-164812",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.8.beta0**
