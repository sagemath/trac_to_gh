# Issue 13817: Add documentation about how to use Type-1 fonts in graphics

archive/issues_013613.json:
```json
{
    "body": "See this [ask.sagemath thread](http://ask.sagemath.org/question/2066/fonts-in-graph-graph-theory-pictures-are-type-3).\n\nThe default matplotlib behavior is to use Type-3 fonts in graphics. However, some (many?) publications have a strict stipulation of using Type-1 fonts only in the document. The method to force matplotlib to have this behavior is to add a matplotlibrc file in the working directory with the following contents:\n\n```\nrcParams['ps.useafm'] = True\nrcParams['pdf.use14corefonts'] = True\nrcParams['text.usetex'] = True\n```\n\nThis tidbit should be added to the main graphics documentation in some nice way. A `..note` maybe?\n\n**Assignee:** mvngu\n\n**Reviewer:** Punarbasu Purkayastha\n\nIssue created by migration from https://trac.sagemath.org/ticket/13817\n\n",
    "closed_at": "2013-10-30T14:18:42Z",
    "created_at": "2012-12-10T19:31:43Z",
    "labels": [
        "component: documentation",
        "enhancement",
        "duplicate"
    ],
    "title": "Add documentation about how to use Type-1 fonts in graphics",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/13817",
    "user": "https://github.com/ppurka"
}
```
See this [ask.sagemath thread](http://ask.sagemath.org/question/2066/fonts-in-graph-graph-theory-pictures-are-type-3).

The default matplotlib behavior is to use Type-3 fonts in graphics. However, some (many?) publications have a strict stipulation of using Type-1 fonts only in the document. The method to force matplotlib to have this behavior is to add a matplotlibrc file in the working directory with the following contents:

```
rcParams['ps.useafm'] = True
rcParams['pdf.use14corefonts'] = True
rcParams['text.usetex'] = True
```

This tidbit should be added to the main graphics documentation in some nice way. A `..note` maybe?

**Assignee:** mvngu

**Reviewer:** Punarbasu Purkayastha

Issue created by migration from https://trac.sagemath.org/ticket/13817





---

archive/issue_events_121108.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13817",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13817#event-121108"
}
```



---

archive/issue_events_121109.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13817",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13817#event-121109"
}
```



---

archive/issue_comments_166783.json:
```json
{
    "body": "<a id='comment:2'></a>\nFixed by #14664",
    "created_at": "2013-10-12T08:25:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13817",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13817#issuecomment-166783",
    "user": "https://github.com/ppurka"
}
```

<a id='comment:2'></a>
Fixed by #14664



---

archive/issue_events_121110.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2013-10-12T08:25:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/13817",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13817#event-121110"
}
```



---

archive/issue_events_121111.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2013-10-12T08:25:22Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13817",
    "milestone": "sage-5.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13817#event-121111"
}
```



---

archive/issue_events_121112.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2013-10-12T08:25:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/13817",
    "label": "duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13817#event-121112"
}
```



---

archive/issue_events_121113.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2013-10-12T08:27:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/13817",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13817#event-121113"
}
```



---

archive/issue_events_121114.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2013-10-12T08:27:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/13817",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13817#event-121114"
}
```



---

archive/issue_comments_166784.json:
```json
{
    "body": "**Reviewer:** Punarbasu Purkayastha",
    "created_at": "2013-10-12T08:27:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13817",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13817#issuecomment-166784",
    "user": "https://github.com/ppurka"
}
```

**Reviewer:** Punarbasu Purkayastha



---

archive/issue_events_121115.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-30T14:18:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/13817",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13817#event-121115"
}
```



---

archive/issue_events_121116.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-30T14:18:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/13817",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13817#event-121116"
}
```



---

archive/issue_events_121117.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-30T14:18:42Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/13817",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13817#event-121117"
}
```
