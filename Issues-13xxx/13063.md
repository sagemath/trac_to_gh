# Issue 13063: Doctesting rst files

archive/issues_012891.json:
```json
{
    "assignees": [],
    "body": "Currently we parse rst files in the documentation in exactly the same way as we parse python files: search for triple quoted strings and then search for sage: within them.  This is the wrong approach within an rst file, since doctests can occur within normal text.  Instead, we should use `docutils` to parse the rst files and look for verbatim or doctest blocks, and then search within those.  \n\nDepends on #12415\n\nAssignee: **@roed314**\n\nCC:  @kini\n\nReviewer: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13063_\n\n",
    "closed_at": "2014-10-13T15:49:14Z",
    "created_at": "2012-05-29T22:17:12Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20doctest%20framework",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Doctesting rst files",
    "type": "issue",
    "updated_at": "2014-10-13T15:49:14Z",
    "url": "https://github.com/sagemath/sage/issues/13063",
    "user": "https://github.com/roed314"
}
```
Currently we parse rst files in the documentation in exactly the same way as we parse python files: search for triple quoted strings and then search for sage: within them.  This is the wrong approach within an rst file, since doctests can occur within normal text.  Instead, we should use `docutils` to parse the rst files and look for verbatim or doctest blocks, and then search within those.  

Depends on #12415

Assignee: **@roed314**

CC:  @kini

Reviewer: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/13063_





---

archive/issue_events_165826.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-05-29T22:17:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13063",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13063#event-165826"
}
```



---

archive/issue_events_165827.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-05-29T22:17:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13063",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13063#event-165827"
}
```



---

archive/issue_events_165828.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-05-29T22:17:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13063",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13063#event-165828"
}
```



---

archive/issue_events_165829.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-05-29T22:17:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13063",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13063#event-165829"
}
```



---

archive/issue_events_165830.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2013-03-28T22:43:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13063",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13063#event-165830"
}
```



---

archive/issue_events_165831.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2013-03-28T22:43:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13063",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20doctest%20framework",
    "label_color": "000080",
    "label_name": "component: doctest framework",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13063#event-165831"
}
```



---

archive/issue_comments_151172.json:
```json
{
    "body": "Changed assignee from **mvngu** to **@roed314**",
    "created_at": "2013-03-28T22:43:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13063#issuecomment-151172",
    "user": "https://github.com/roed314"
}
```

Changed assignee from **mvngu** to **@roed314**



---

archive/issue_events_165832.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13063",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13063#event-165832"
}
```



---

archive/issue_events_165833.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13063",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13063#event-165833"
}
```



---

archive/issue_events_165834.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13063",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13063#event-165834"
}
```



---

archive/issue_events_165835.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13063",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13063#event-165835"
}
```



---

archive/issue_events_165836.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13063",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13063#event-165836"
}
```



---

archive/issue_events_165837.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13063",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13063#event-165837"
}
```



---

archive/issue_events_165838.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13063",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13063#event-165838"
}
```



---

archive/issue_events_165839.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13063",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13063#event-165839"
}
```



---

archive/issue_events_165840.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-01T13:05:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13063",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13063#event-165840"
}
```



---

archive/issue_comments_151173.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2014-10-01T13:05:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13063#issuecomment-151173",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_165841.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-01T13:05:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13063",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13063#event-165841"
}
```



---

archive/issue_events_165842.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-01T13:05:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13063",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13063#event-165842"
}
```



---

archive/issue_events_165843.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-01T13:05:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13063",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13063#event-165843"
}
```



---

archive/issue_comments_151174.json:
```json
{
    "body": "Replying to [ticket:13063 roed]:\n> Currently we parse rst files in the documentation in exactly the same way as we parse python files: search for triple quoted strings and then search for sage: within them.\n\nThis is simply not true anymore, all files in `src/doc/[a-z][a-z]` are tested.",
    "created_at": "2014-10-01T13:05:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13063#issuecomment-151174",
    "user": "https://github.com/jdemeyer"
}
```

Replying to [ticket:13063 roed]:
> Currently we parse rst files in the documentation in exactly the same way as we parse python files: search for triple quoted strings and then search for sage: within them.

This is simply not true anymore, all files in `src/doc/[a-z][a-z]` are tested.



---

archive/issue_events_165844.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-01T13:05:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13063",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13063#event-165844"
}
```



---

archive/issue_events_165845.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-01T13:05:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13063",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13063#event-165845"
}
```



---

archive/issue_events_165846.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-13T15:49:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13063",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13063#event-165846"
}
```



---

archive/issue_events_165847.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-13T15:49:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13063",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13063#event-165847"
}
```
