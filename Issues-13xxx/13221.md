...
```json
{
    "body": "I don't see this behaviour anymore. Please confirm and close.",
...
```json
{
    "body": "And why should it be `(x,y)` instead of `(y,x)`? What if one depends on `(a, b)` and the other on `(alpha, beta)`, what should the result be? I think that in such cases the result should be a symbolic expression (which is unambiguous), but without any default order of variables. This is why polynomials in different variables cannot be added - ordering is non-canonical.",
