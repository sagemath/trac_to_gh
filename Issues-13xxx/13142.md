# Issue 13142: show() of a (MI)LP should not hardwire double for entries

archive/issues_012970.json:
```json
{
    "assignees": [
        "https://github.com/nathanncohen"
    ],
    "body": "Currently, show() method of an (MI)LP hardwires the coefficients of inequalities and equations to (Cython) double. \nThis is asking for trouble when an arbitrary precision (MI)LP solver, such as one provided by PPL (see #12533) is used.\n\nCC:  @nathanncohen @ppurka @ptrrsn\n\nReviewer: **Jeroen Demeyer**\n\nComponent: **linear programming**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13142_\n\n",
    "closed_at": "2014-08-20T20:36:11Z",
    "created_at": "2012-06-20T12:54:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20linear%20programming",
        "https://github.com/sagemath/sage/labels/worksforme"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "show() of a (MI)LP should not hardwire double for entries",
    "type": "issue",
    "updated_at": "2014-08-20T20:36:11Z",
    "url": "https://github.com/sagemath/sage/issues/13142",
    "user": "https://github.com/dimpase"
}
```
Currently, show() method of an (MI)LP hardwires the coefficients of inequalities and equations to (Cython) double. 
This is asking for trouble when an arbitrary precision (MI)LP solver, such as one provided by PPL (see #12533) is used.

CC:  @nathanncohen @ppurka @ptrrsn

Reviewer: **Jeroen Demeyer**

Component: **linear programming**

_Issue created by migration from https://trac.sagemath.org/ticket/13142_





---

archive/issue_events_182578.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2012-06-20T12:54:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13142",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13142#event-182578"
}
```



---

archive/issue_events_182579.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2012-06-20T12:54:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13142",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20linear%20programming",
    "label_color": "0000ff",
    "label_name": "c: linear programming",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13142#event-182579"
}
```



---

archive/issue_events_182580.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2012-06-20T12:54:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13142",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13142#event-182580"
}
```



---

archive/issue_events_182581.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2012-06-20T12:54:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13142",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13142#event-182581"
}
```



---

archive/issue_events_182582.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2012-06-20T12:54:29Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/13142",
    "subject": "https://github.com/dimpase",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13142#event-182582"
}
```



---

archive/issue_events_182583.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13142",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13142#event-182583"
}
```



---

archive/issue_events_182584.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13142",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13142#event-182584"
}
```



---

archive/issue_events_182585.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13142",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13142#event-182585"
}
```



---

archive/issue_events_182586.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13142",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13142#event-182586"
}
```



---

archive/issue_events_182587.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13142",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13142#event-182587"
}
```



---

archive/issue_events_182588.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13142",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13142#event-182588"
}
```



---

archive/issue_comments_150537.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2014-08-05T06:15:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13142",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13142#issuecomment-150537",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_comments_150538.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nWorksforme:\n\n```\nsage: p = MixedIntegerLinearProgram(solver=\"PPL\")\nsage: x = p.new_variable(nonnegative=True)\nsage: p.add_constraint(4/7*x[0] >= 1/2)\nsage: p.show()\nMaximization:\n \nConstraints:\n  constraint_0: -4/7 x_0 <= -1/2\nVariables:\n  x_0 is a continuous variable (min=0, max=+oo)\n```",
    "created_at": "2014-08-05T06:15:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13142",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13142#issuecomment-150538",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:4" align="right">comment:4</div>

Worksforme:

```
sage: p = MixedIntegerLinearProgram(solver="PPL")
sage: x = p.new_variable(nonnegative=True)
sage: p.add_constraint(4/7*x[0] >= 1/2)
sage: p.show()
Maximization:
 
Constraints:
  constraint_0: -4/7 x_0 <= -1/2
Variables:
  x_0 is a continuous variable (min=0, max=+oo)
```



---

archive/issue_events_182589.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-08-05T06:15:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13142",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13142#event-182589"
}
```



---

archive/issue_events_182590.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-08-05T06:15:32Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13142",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13142#event-182590"
}
```



---

archive/issue_events_182591.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-08-05T06:15:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13142",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13142#event-182591"
}
```



---

archive/issue_events_182592.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-08-05T06:15:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13142",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13142#event-182592"
}
```



---

archive/issue_events_182593.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-08-05T06:15:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13142",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13142#event-182593"
}
```



---

archive/issue_events_182594.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-08-05T06:15:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13142",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13142#event-182594"
}
```



---

archive/issue_events_182595.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-08-20T20:36:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13142",
    "label": "https://github.com/sagemath/sage/labels/worksforme",
    "label_color": "a6a6a6",
    "label_name": "worksforme",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13142#event-182595"
}
```



---

archive/issue_events_182596.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-08-20T20:36:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13142",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13142#event-182596"
}
```



---

archive/issue_events_182597.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-08-20T20:36:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13142",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13142#event-182597"
}
```
