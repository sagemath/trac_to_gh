# Issue 13017: create a large linear algebra thematic tutorial that will be part of the official Sage documentation

archive/issues_012845.json:
```json
{
    "assignees": [],
    "body": "Jason Grout, Rob Beezer, Robert Bradshaw, and William Stein have mostly written a rather extensive thematic tutorial on linear algebra for Sage (for inclusion in the HLA = Handbook of Linear Algebra, 2nd edition).  It has a creative commons license.  This ticket is for getting the tutorial into Sage itself.  \n\nThe PDF version of this chapter is here: http://sage.math.washington.edu/home/jason/sage-HLA2.pdf\n\nA git branch with the necessary changes is at https://github.com/jasongrout/sage/tree/hla (changes are listed at https://github.com/jasongrout/sage/compare/hla)\n\n\nAssignee: **@jasongrout, @williamstein**\n\nCC:  @jasongrout @novoselt @williamstein @rbeezer @robertwb @jhpalmieri @kcrisman\n\nKeywords: **sd40.5**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13017_\n\n",
    "created_at": "2012-05-25T23:31:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/needs%20work"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "create a large linear algebra thematic tutorial that will be part of the official Sage documentation",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/13017",
    "user": "https://github.com/williamstein"
}
```
Jason Grout, Rob Beezer, Robert Bradshaw, and William Stein have mostly written a rather extensive thematic tutorial on linear algebra for Sage (for inclusion in the HLA = Handbook of Linear Algebra, 2nd edition).  It has a creative commons license.  This ticket is for getting the tutorial into Sage itself.  

The PDF version of this chapter is here: http://sage.math.washington.edu/home/jason/sage-HLA2.pdf

A git branch with the necessary changes is at https://github.com/jasongrout/sage/tree/hla (changes are listed at https://github.com/jasongrout/sage/compare/hla)


Assignee: **@jasongrout, @williamstein**

CC:  @jasongrout @novoselt @williamstein @rbeezer @robertwb @jhpalmieri @kcrisman

Keywords: **sd40.5**

_Issue created by migration from https://trac.sagemath.org/ticket/13017_





---

archive/issue_events_165058.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-05-25T23:31:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13017#event-165058"
}
```



---

archive/issue_events_165059.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-05-25T23:31:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13017#event-165059"
}
```



---

archive/issue_events_165060.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-05-25T23:31:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13017#event-165060"
}
```



---

archive/issue_events_165061.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-05-25T23:31:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13017#event-165061"
}
```



---

archive/issue_comments_150376.json:
```json
{
    "body": "Changed keywords from none to **sd40.5**",
    "created_at": "2012-05-25T23:32:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13017#issuecomment-150376",
    "user": "https://github.com/williamstein"
}
```

Changed keywords from none to **sd40.5**



---

archive/issue_comments_150377.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n Jason Grout, Rob Beezer, Robert Bradshaw, and William Stein have mostly written a rather extensive thematic tutorial on linear algebra for Sage (for inclusion in the HLA = Handbook of Linear Algebra, 2nd edition).  It has a creative commons license.  This ticket is for getting the tutorial into Sage itself.  \n \n-\n+The PDF version of this chapter is here: http://sage.math.washington.edu/home/jason/sage-HLA2.pdf \n``````\n",
    "created_at": "2013-03-15T15:30:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13017#issuecomment-150377",
    "user": "https://github.com/jasongrout"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 Jason Grout, Rob Beezer, Robert Bradshaw, and William Stein have mostly written a rather extensive thematic tutorial on linear algebra for Sage (for inclusion in the HLA = Handbook of Linear Algebra, 2nd edition).  It has a creative commons license.  This ticket is for getting the tutorial into Sage itself.  
 
-
+The PDF version of this chapter is here: http://sage.math.washington.edu/home/jason/sage-HLA2.pdf 
``````




---

archive/issue_comments_150378.json:
```json
{
    "body": "Attachment: **[sphinx.patch.gz](https://github.com/sagemath/sage/files/ticket13017/sphinx.patch.gz)**",
    "created_at": "2013-03-15T23:34:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13017#issuecomment-150378",
    "user": "https://github.com/jasongrout"
}
```

Attachment: **[sphinx.patch.gz](https://github.com/sagemath/sage/files/ticket13017/sphinx.patch.gz)**



---

archive/issue_comments_150379.json:
```json
{
    "body": "Attachment: **[trac-13017-linear-algebra-tutorial.patch.gz](https://github.com/sagemath/sage/files/ticket13017/trac-13017-linear-algebra-tutorial.patch.gz)**",
    "created_at": "2013-03-15T23:40:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13017#issuecomment-150379",
    "user": "https://github.com/jasongrout"
}
```

Attachment: **[trac-13017-linear-algebra-tutorial.patch.gz](https://github.com/sagemath/sage/files/ticket13017/trac-13017-linear-algebra-tutorial.patch.gz)**



---

archive/issue_comments_150380.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nI added the main patch, as well as a second patch that I used to patch Sage's local copy of sphinx to generate \"nicer\" tex output.  I think you can probably safely ignore the sphinx patch.",
    "created_at": "2013-03-15T23:42:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13017#issuecomment-150380",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:7" align="right">Comment 7</div>

I added the main patch, as well as a second patch that I used to patch Sage's local copy of sphinx to generate "nicer" tex output.  I think you can probably safely ignore the sphinx patch.



---

archive/issue_events_165062.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2013-03-15T23:43:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13017#event-165062"
}
```



---

archive/issue_events_165063.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2013-03-15T23:43:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13017#event-165063"
}
```



---

archive/issue_events_165064.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2013-03-15T23:43:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13017#event-165064"
}
```



---

archive/issue_comments_150381.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n Jason Grout, Rob Beezer, Robert Bradshaw, and William Stein have mostly written a rather extensive thematic tutorial on linear algebra for Sage (for inclusion in the HLA = Handbook of Linear Algebra, 2nd edition).  It has a creative commons license.  This ticket is for getting the tutorial into Sage itself.  \n \n-The PDF version of this chapter is here: http://sage.math.washington.edu/home/jason/sage-HLA2.pdf \n+The PDF version of this chapter is here: http://sage.math.washington.edu/home/jason/sage-HLA2.pdf\n``````\n",
    "created_at": "2013-03-15T23:48:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13017#issuecomment-150381",
    "user": "https://github.com/jasongrout"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 Jason Grout, Rob Beezer, Robert Bradshaw, and William Stein have mostly written a rather extensive thematic tutorial on linear algebra for Sage (for inclusion in the HLA = Handbook of Linear Algebra, 2nd edition).  It has a creative commons license.  This ticket is for getting the tutorial into Sage itself.  
 
-The PDF version of this chapter is here: http://sage.math.washington.edu/home/jason/sage-HLA2.pdf 
+The PDF version of this chapter is here: http://sage.math.washington.edu/home/jason/sage-HLA2.pdf
``````




---

archive/issue_comments_150382.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nAt this point, I think someone just needs to scan through to see if there are big problems in any of the formats.  Long-term things would be expanding it by uncommenting a lot of the examples we had to cut due to page limits, or rearranging things so that the examples are inline instead of grouped at the end of each section.",
    "created_at": "2013-03-15T23:48:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13017#issuecomment-150382",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:10" align="right">Comment 10</div>

At this point, I think someone just needs to scan through to see if there are big problems in any of the formats.  Long-term things would be expanding it by uncommenting a lot of the examples we had to cut due to page limits, or rearranging things so that the examples are inline instead of grouped at the end of each section.



---

archive/issue_comments_150383.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nOh, and making the examples live in the html docs, and making function names link to the function documentation.",
    "created_at": "2013-03-15T23:49:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13017#issuecomment-150383",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:11" align="right">Comment 11</div>

Oh, and making the examples live in the html docs, and making function names link to the function documentation.



---

archive/issue_comments_150384.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,5 @@\n Jason Grout, Rob Beezer, Robert Bradshaw, and William Stein have mostly written a rather extensive thematic tutorial on linear algebra for Sage (for inclusion in the HLA = Handbook of Linear Algebra, 2nd edition).  It has a creative commons license.  This ticket is for getting the tutorial into Sage itself.  \n \n The PDF version of this chapter is here: http://sage.math.washington.edu/home/jason/sage-HLA2.pdf\n+\n+*apply*: [attachment: trac-13017-linear-algebra-tutorial.patch](https://github.com/sagemath/sage/files/ticket13017/trac-13017-linear-algebra-tutorial.patch.gz)\n``````\n",
    "created_at": "2013-03-15T23:49:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13017#issuecomment-150384",
    "user": "https://github.com/jasongrout"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,5 @@
 Jason Grout, Rob Beezer, Robert Bradshaw, and William Stein have mostly written a rather extensive thematic tutorial on linear algebra for Sage (for inclusion in the HLA = Handbook of Linear Algebra, 2nd edition).  It has a creative commons license.  This ticket is for getting the tutorial into Sage itself.  
 
 The PDF version of this chapter is here: http://sage.math.washington.edu/home/jason/sage-HLA2.pdf
+
+*apply*: [attachment: trac-13017-linear-algebra-tutorial.patch](https://github.com/sagemath/sage/files/ticket13017/trac-13017-linear-algebra-tutorial.patch.gz)
``````




---

archive/issue_comments_150385.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nIndex file has changed dramatically, so needed a rebase.  I'll post it in a minute.  Possibly still not right, I believe, since there is a warning about a label.  Maybe a second pass would remedy that.  So table of contents for thematic tutorials is not right for me at the moment.  (On 5.10.rc0.)\n\nSeveral references to Chapters of HLA.  These should be removed or made clear.  First paragraph is from the chapter saying there is a Sage tutorial.  This **is** the tutorial.  So...this needs a sweep for references or comments that only make sense for the HLA chapter and not for the thematic tutorial.\n\nRob",
    "created_at": "2013-06-05T22:16:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13017#issuecomment-150385",
    "user": "https://github.com/rbeezer"
}
```

<div id="comment:13" align="right">Comment 13</div>

Index file has changed dramatically, so needed a rebase.  I'll post it in a minute.  Possibly still not right, I believe, since there is a warning about a label.  Maybe a second pass would remedy that.  So table of contents for thematic tutorials is not right for me at the moment.  (On 5.10.rc0.)

Several references to Chapters of HLA.  These should be removed or made clear.  First paragraph is from the chapter saying there is a Sage tutorial.  This **is** the tutorial.  So...this needs a sweep for references or comments that only make sense for the HLA chapter and not for the thematic tutorial.

Rob



---

archive/issue_comments_150386.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,4 +2,4 @@\n \n The PDF version of this chapter is here: http://sage.math.washington.edu/home/jason/sage-HLA2.pdf\n \n-*apply*: [attachment: trac-13017-linear-algebra-tutorial.patch](https://github.com/sagemath/sage/files/ticket13017/trac-13017-linear-algebra-tutorial.patch.gz)\n+*apply*: [attachment: trac-13017-linear-algebra-tutorial-rebase.patch](https://github.com/sagemath/sage/files/ticket13017/trac-13017-linear-algebra-tutorial-rebase.patch.gz)\n``````\n",
    "created_at": "2013-06-05T22:25:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13017#issuecomment-150386",
    "user": "https://github.com/rbeezer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,4 +2,4 @@
 
 The PDF version of this chapter is here: http://sage.math.washington.edu/home/jason/sage-HLA2.pdf
 
-*apply*: [attachment: trac-13017-linear-algebra-tutorial.patch](https://github.com/sagemath/sage/files/ticket13017/trac-13017-linear-algebra-tutorial.patch.gz)
+*apply*: [attachment: trac-13017-linear-algebra-tutorial-rebase.patch](https://github.com/sagemath/sage/files/ticket13017/trac-13017-linear-algebra-tutorial-rebase.patch.gz)
``````




---

archive/issue_comments_150387.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nAttachment: **[trac-13017-linear-algebra-tutorial-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket13017/trac-13017-linear-algebra-tutorial-rebase.patch.gz)**",
    "created_at": "2013-06-05T22:25:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13017#issuecomment-150387",
    "user": "https://github.com/rbeezer"
}
```

<div id="comment:14" align="right">Comment 14</div>

Attachment: **[trac-13017-linear-algebra-tutorial-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket13017/trac-13017-linear-algebra-tutorial-rebase.patch.gz)**



---

archive/issue_events_165065.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13017#event-165065"
}
```



---

archive/issue_events_165066.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13017#event-165066"
}
```



---

archive/issue_comments_150388.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,4 +2,5 @@\n \n The PDF version of this chapter is here: http://sage.math.washington.edu/home/jason/sage-HLA2.pdf\n \n-*apply*: [attachment: trac-13017-linear-algebra-tutorial-rebase.patch](https://github.com/sagemath/sage/files/ticket13017/trac-13017-linear-algebra-tutorial-rebase.patch.gz)\n+A git branch with the necessary changes is at https://github.com/jasongrout/sage/tree/hla (changes are listed at https://github.com/jasongrout/sage/compare/hla)\n+\n``````\n",
    "created_at": "2013-10-19T19:45:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13017#issuecomment-150388",
    "user": "https://github.com/jasongrout"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,4 +2,5 @@
 
 The PDF version of this chapter is here: http://sage.math.washington.edu/home/jason/sage-HLA2.pdf
 
-*apply*: [attachment: trac-13017-linear-algebra-tutorial-rebase.patch](https://github.com/sagemath/sage/files/ticket13017/trac-13017-linear-algebra-tutorial-rebase.patch.gz)
+A git branch with the necessary changes is at https://github.com/jasongrout/sage/tree/hla (changes are listed at https://github.com/jasongrout/sage/compare/hla)
+
``````




---

archive/issue_comments_150389.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\nI updated a version in my github repository.",
    "created_at": "2013-10-19T19:45:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13017#issuecomment-150389",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:16" align="right">Comment 16</div>

I updated a version in my github repository.



---

archive/issue_events_165067.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13017#event-165067"
}
```



---

archive/issue_events_165068.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13017#event-165068"
}
```



---

archive/issue_events_165069.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13017#event-165069"
}
```



---

archive/issue_events_165070.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13017#event-165070"
}
```



---

archive/issue_events_165071.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13017#event-165071"
}
```



---

archive/issue_events_165072.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13017#event-165072"
}
```



---

archive/issue_events_165073.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13017",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13017#event-165073"
}
```
