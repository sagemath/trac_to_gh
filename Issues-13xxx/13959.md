...
```json
{
    "body": "I think that what's happening is that the `free_ram` is somehow getting the active and not the free line.  I'm not sure what is going on that that is happening, since it does pass the check for parsing `PhysMem`.",
...
```json
{
    "body": "Its `['top', '-l', '1']` (top dash-ell one).",
...
```json
{
    "body": "Erk.  That's what happens when you just type instead of cut-and-paste - sorry, I have three computers within a foot of each other on the desk, and only one ethernet cable (and don't want to use wifi for BLODA reasons on the Cygwin...).\n\nWith `sage -ipython`:\n\n```\nIn [9]: for line in meminfo.splitlines():\n    if line.startswith('PhysMem'):\n        line = line.split()\n   ....:         break\n   ....:         \n   ....:         \n\nIn [12]: line\nOut[12]: \n['PhysMem:',\n '65.2M',\n 'wired,',\n '89.6M',\n 'active,',\n '138M',\n 'inactive,',\n '293M',\n 'used,',\n '218M',\n 'free']\n```\nSeems okay here, I'm not sure why it is doing this.",
...
```json
{
    "body": "Fixed. The problem is when the free memory drops below 100M, then you'll get a decimal point.",
...
```json
{
    "body": "I've added the 32-bit branch for the doctest.",
...
```json
{
    "body": "I'll try to check this tomorrow but looks good.  Well, that computer only has 500 MB of RAM, if I'm not mistaken, so it was inevitable :-) but I wouldn't be surprised if some OLPC or something had that too...",
...
```json
{
    "body": "Also, I assume this will not conflict with the long-awaited #13588?  I would hate for that to happen!  Though I suppose this is a blocker since it's a failing doctest and Jeroen asked me to open this asap.",
