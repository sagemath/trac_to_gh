# Issue 13565: Add get_bandwidth to matrix/matrix2.pyx

archive/issues_013361.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-r-gaia-cs"
    ],
    "body": "It will be great one method to return the bandwidth of a matrix.\n\n---\nApply [attachment: trac_13565-bandwidth.patch](https://github.com/sagemath/sage/files/ticket13565/trac_13565-bandwidth.patch.gz)\n\nDepends on #13564\n\nKeywords: **matrix, bandwidth**\n\nAuthor: **r.gaia.cs**\n\nComponent: **linear algebra**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13565_\n\n",
    "created_at": "2012-10-03T23:55:01Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/needs%20work"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add get_bandwidth to matrix/matrix2.pyx",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/13565",
    "user": "https://github.com/sagetrac-r-gaia-cs"
}
```
It will be great one method to return the bandwidth of a matrix.

---
Apply [attachment: trac_13565-bandwidth.patch](https://github.com/sagemath/sage/files/ticket13565/trac_13565-bandwidth.patch.gz)

Depends on #13564

Keywords: **matrix, bandwidth**

Author: **r.gaia.cs**

Component: **linear algebra**

_Issue created by migration from https://trac.sagemath.org/ticket/13565_





---

archive/issue_events_189443.json:
```json
{
    "actor": "https://github.com/sagetrac-r-gaia-cs",
    "created_at": "2012-10-03T23:55:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13565",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13565#event-189443"
}
```



---

archive/issue_events_189444.json:
```json
{
    "actor": "https://github.com/sagetrac-r-gaia-cs",
    "created_at": "2012-10-03T23:55:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13565",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "c: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13565#event-189444"
}
```



---

archive/issue_events_189445.json:
```json
{
    "actor": "https://github.com/sagetrac-r-gaia-cs",
    "created_at": "2012-10-03T23:55:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13565",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13565#event-189445"
}
```



---

archive/issue_events_189446.json:
```json
{
    "actor": "https://github.com/sagetrac-r-gaia-cs",
    "created_at": "2012-10-03T23:55:01Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/13565",
    "subject": "https://github.com/sagetrac-r-gaia-cs",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13565#event-189446"
}
```



---

archive/issue_comments_158427.json:
```json
{
    "body": "Attachment: **[bandwidth.patch.gz](https://github.com/sagemath/sage/files/ticket13565/bandwidth.patch.gz)**",
    "created_at": "2012-10-03T23:55:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13565#issuecomment-158427",
    "user": "https://github.com/sagetrac-r-gaia-cs"
}
```

Attachment: **[bandwidth.patch.gz](https://github.com/sagemath/sage/files/ticket13565/bandwidth.patch.gz)**



---

archive/issue_events_189447.json:
```json
{
    "actor": "https://github.com/sagetrac-r-gaia-cs",
    "created_at": "2012-10-06T12:20:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13565",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13565#event-189447"
}
```



---

archive/issue_events_189448.json:
```json
{
    "actor": "https://github.com/sagetrac-r-gaia-cs",
    "created_at": "2012-10-06T12:20:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13565",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13565#event-189448"
}
```



---

archive/issue_comments_158428.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nHello,\n1. you need to provide a good commit message. You can do that by using hg. This will then appear in your patch instead of the `[mq]: bandwidth` at the top of the patch.\n\n```\nsage -hg qref -m \"a good commit message\"\n```\n2. Also, name your patch as per this ticket number. Such as `trac_13565-bandwidth.patch`, instead of simply `bandwidth.patch`.\n\nBoth these modifications help the release manager when the patch is merged into Sage proper. Same comments apply to your dependency ticket #13564.",
    "created_at": "2012-10-08T09:14:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13565#issuecomment-158428",
    "user": "https://github.com/ppurka"
}
```

<div id="comment:2" align="right">comment:2</div>

Hello,
1. you need to provide a good commit message. You can do that by using hg. This will then appear in your patch instead of the `[mq]: bandwidth` at the top of the patch.

```
sage -hg qref -m "a good commit message"
```
2. Also, name your patch as per this ticket number. Such as `trac_13565-bandwidth.patch`, instead of simply `bandwidth.patch`.

Both these modifications help the release manager when the patch is merged into Sage proper. Same comments apply to your dependency ticket #13564.



---

archive/issue_comments_158429.json:
```json
{
    "body": "Attachment: **[trac_13565-bandwidth.patch.gz](https://github.com/sagemath/sage/files/ticket13565/trac_13565-bandwidth.patch.gz)**\n\nMake correction sugested by ppurka.",
    "created_at": "2012-10-08T14:55:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13565#issuecomment-158429",
    "user": "https://github.com/sagetrac-r-gaia-cs"
}
```

Attachment: **[trac_13565-bandwidth.patch.gz](https://github.com/sagemath/sage/files/ticket13565/trac_13565-bandwidth.patch.gz)**

Make correction sugested by ppurka.



---

archive/issue_comments_158430.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,4 @@\n It will be great one method to return the bandwidth of a matrix.\n+\n+---\n+Apply [attachment: trac_13565-bandwidth.patch](https://github.com/sagemath/sage/files/ticket13565/trac_13565-bandwidth.patch.gz)\n``````\n",
    "created_at": "2012-10-10T13:19:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13565#issuecomment-158430",
    "user": "https://github.com/ppurka"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,4 @@
 It will be great one method to return the bandwidth of a matrix.
+
+---
+Apply [attachment: trac_13565-bandwidth.patch](https://github.com/sagemath/sage/files/ticket13565/trac_13565-bandwidth.patch.gz)
``````




---

archive/issue_comments_158431.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nForce patchbot.\n\nPatchbot apply trac_13565-bandwidth.patch",
    "created_at": "2012-10-12T08:01:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13565#issuecomment-158431",
    "user": "https://github.com/ppurka"
}
```

<div id="comment:4" align="right">comment:4</div>

Force patchbot.

Patchbot apply trac_13565-bandwidth.patch



---

archive/issue_events_189449.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13565",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13565#event-189449"
}
```



---

archive/issue_events_189450.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13565",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13565#event-189450"
}
```



---

archive/issue_events_189451.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13565",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13565#event-189451"
}
```



---

archive/issue_events_189452.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13565",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13565#event-189452"
}
```



---

archive/issue_events_189453.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13565",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13565#event-189453"
}
```



---

archive/issue_events_189454.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13565",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13565#event-189454"
}
```



---

archive/issue_comments_158432.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nneeds a git branch",
    "created_at": "2014-07-27T09:56:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13565",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13565#issuecomment-158432",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:8" align="right">comment:8</div>

needs a git branch



---

archive/issue_events_189455.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-07-27T09:56:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13565",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13565#event-189455"
}
```



---

archive/issue_events_189456.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2014-07-27T09:56:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13565",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13565#event-189456"
}
```



---

archive/issue_events_189457.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13565",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13565#event-189457"
}
```



---

archive/issue_events_189458.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13565",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13565#event-189458"
}
```



---

archive/issue_events_189459.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13565",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13565#event-189459"
}
```
