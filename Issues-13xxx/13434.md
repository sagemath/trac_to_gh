# Issue 13434: Listing elements in Cartesian Products for Sets

archive/issues_013262.json:
```json
{
    "assignees": [],
    "body": "Right now, CartesianProduct has no attribute 'list'. One would like to be able to index the elements of a Cartesian product. For instance:\n\n```\nS = Set([1,2])\nC = S.cartesian_product(S)\nC.list()\n```\n\nKeywords: **Cartesian Product**\n\nAuthor: **Chris Berg**\n\nComponent: **basic arithmetic**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13434_\n\n",
    "created_at": "2012-09-06T16:13:41Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/wishlist%20item",
        "https://github.com/sagemath/sage/labels/needs%20work"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Listing elements in Cartesian Products for Sets",
    "type": "issue",
    "updated_at": "2013-04-21T23:00:48Z",
    "url": "https://github.com/sagemath/sage/issues/13434",
    "user": "https://github.com/sagetrac-chrisjamesberg"
}
```
Right now, CartesianProduct has no attribute 'list'. One would like to be able to index the elements of a Cartesian product. For instance:

```
S = Set([1,2])
C = S.cartesian_product(S)
C.list()
```

Keywords: **Cartesian Product**

Author: **Chris Berg**

Component: **basic arithmetic**

_Issue created by migration from https://trac.sagemath.org/ticket/13434_





---

archive/issue_events_187741.json:
```json
{
    "actor": "https://github.com/sagetrac-chrisjamesberg",
    "created_at": "2012-09-06T16:13:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13434",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "c: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13434#event-187741"
}
```



---

archive/issue_events_187742.json:
```json
{
    "actor": "https://github.com/sagetrac-chrisjamesberg",
    "created_at": "2012-09-06T16:13:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13434",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13434#event-187742"
}
```



---

archive/issue_events_187743.json:
```json
{
    "actor": "https://github.com/sagetrac-chrisjamesberg",
    "created_at": "2012-09-06T16:13:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13434",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13434#event-187743"
}
```



---

archive/issue_events_187744.json:
```json
{
    "actor": "https://github.com/sagetrac-chrisjamesberg",
    "created_at": "2012-09-06T16:13:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13434",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13434#event-187744"
}
```



---

archive/issue_events_187745.json:
```json
{
    "actor": "https://github.com/sagetrac-LostPw",
    "created_at": "2013-04-19T08:32:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13434",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13434#event-187745"
}
```



---

archive/issue_comments_156793.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\n> \n> ```\n> S = Set([1,2])\n> C = CarstesianProduct(S,S)\n> C.list()\n> ```\n\nworks.",
    "created_at": "2013-04-19T08:32:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13434",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13434#issuecomment-156793",
    "user": "https://github.com/sagetrac-LostPw"
}
```

<div id="comment:1" align="right">comment:1</div>

> 
> ```
> S = Set([1,2])
> C = CarstesianProduct(S,S)
> C.list()
> ```

works.



---

archive/issue_comments_156794.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThose are different objects, and yes, it is somewhat confusion (I believe there's a ticket already about/related to this). So there is something to be fixed here.",
    "created_at": "2013-04-21T23:00:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13434",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13434#issuecomment-156794",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:2" align="right">comment:2</div>

Those are different objects, and yes, it is somewhat confusion (I believe there's a ticket already about/related to this). So there is something to be fixed here.



---

archive/issue_events_187746.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-04-21T23:00:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13434",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13434#event-187746"
}
```



---

archive/issue_events_187747.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-04-21T23:00:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13434",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13434#event-187747"
}
```
