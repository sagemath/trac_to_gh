...
```json
{
    "body": "<a id='comment:19'></a>cvxopt is now a pseudo-optional package, but we should still be able to use the approach in comment:14. We can `super()` if cvxopt isn't available.",
...
<a id='comment:19'></a>cvxopt is now a pseudo-optional package, but we should still be able to use the approach in comment:14. We can `super()` if cvxopt isn't available.
...
<a id='comment:26'></a>Replying to [comment:25 gh-collares]:
> Opened #33023 for the test failure, thanks for letting me know about the correct procedure. I didn't know the "Commit" field would be cleared when I posted my first comment, sorry about that.
