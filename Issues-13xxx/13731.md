...
```json
{
    "body": "No, libdir is used in spkg-install. But I wonder: The original Singular spkg contains three Makefiles: in dyn_modules/python, in calldummy/ and in omalloc/Misc/dlmalloc. But the malloc Singular package from here contains 14 Makefiles. Could it be that some of them should better be created by `./configure`?",
...
```json
{
    "body": "The spkg also contains several files created by configure scripts. Is that really intended?",
...
```json
{
    "body": "By the way, what is the ultimate aim of this ticket? Replacing Singular's memory management by malloc makes Singular very slow, if I understand correctly. So, the aim is to provide a tool to investigate flaws in Sage-Singular interrelation, right?\n\nSo, could one say that the aim is to provide an optional package?",
...
```json
{
    "body": "News become worse: The crash happens in `pp_Mult_qq`, so that I start to believe it might be an upstream problem.",
...
```json
{
    "body": "I should have read your backtrace more carefully. It mentions `gnc_mm_Mult_nn`, and that's what I am analysing now. A shame that one needs to recompile so many things for testing it...",
...
```json
{
    "body": "For the record: The problem is reported [upstream](http://www.singular.uni-kl.de:8002/trac/ticket/463), and Hannes seems to acknowledge that there is a problem.",
...
```json
{
    "body": "Aha! You also changed spkg-install, so that the patches are actually *not* applied! That is of course the reason why it didn't work.",
...
```json
{
    "body": "PS: Actually it seems to me that the changes in `src/Singular/configure.in` and `src/Singular/configure` are due to the patch `patches/singular_trac_443.patch` that is to be applied anyway.",
...
```json
{
    "body": "`src/omalloc/omTables.inc` doesn't state that it is auto-generated, but it certainly looks like.\n\nSo, third question: Did you write `src/omalloc/omTables.inc` yourself? Or can I safely delete it?",
...
Attachment [sage_trac_13731.patch](tarball://root/attachments/some-uuid/ticket13731/sage_trac_13731.patch) by @simon-king-jena created at 2012-11-25 08:39:36
...
```json
{
    "body": "bsd.math is OS X, isn't it? I think I had trouble with that recently because malloc.h is in a subdirectory - not at the top level. You would need an extra -I I guess. I'll see if I can find it.",
...
```json
{
    "body": "It is in /usr/include/malloc/",
...
```json
{
    "body": "Rool eyes.... but not completely surprised.",
...
```json
{
    "body": "I am not sure what you would get for a non-mac darwin target, otherwise the easiest thing is probably \ngcc -dumpmachine, On my 10.5.8 machine:\n\n```\nBlueFerniMac1:~ frb15$ gcc -dumpmachine\ni686-apple-darwin9\n```",
...
```json
{
    "body": "After more thinking I think you may want to use [AC_CANONICAL_TARGET](http://www.delorie.com/gnu/docs/autoconf/autoconf_127.html) it is usually used for cross compilation but it should work in this case. It is much better than testing gcc since we may want to use clang.",
...
```json
{
    "body": "I think AC_DEFINE puts stuff in config.h and you would have to include config.h before this piece of code. Having a code file changed from configure is quite some work. I would have to look it up.",
...
```json
{
    "body": "I wouldn't add AC_CONFIG_HEADERS if it isn't already there. Are you working on the top level or at the Singular folder level? configure.in in Singular does call AC_CONFIG_HEADERS and puts definitions in mod2.h.",
...
```json
{
    "body": "I am slightly shocked that omalloc/configure.in uses a hardcoded path to malloc.h:\n\n```\nAC_CHECK_HEADERS(unistd.h sys/mman.h fcntl.h /usr/include/malloc.h)\n```\n\nIsn't that bad?",
...
```json
{
    "body": "Simon, `configure` is generated from `configure.in` using `autoconf`. However, Singular requires a special autoconf version (I think it is a particularly old one) which makes fixing things in its configure script tricky. I think this should go on [libsingular-devel]. Hans can help, I think.",
...
```json
{
    "body": "Sorry, when posting the previous comment, I forgot to delete two paragraphs before submitting...",
...
```json
{
    "body": "On [libsingular-devel](https://groups.google.com/forum/#!topic/libsingular-devel/AZI-6eZAgus), Hans suggests to use xalloc, not malloc to detect memory-related problems.\n\nNils, do you know how to do those things?",
...
```json
{
    "body": "PPS: I am afraid the bug fixes and the xalloc patch do not commute. Hence, when including the fixes, I need to update the xalloc patch as well.",
...
```json
{
    "body": "Hooray, it does build on OSX (bsd.math)!!",
...
```json
{
    "body": "No. It doesn't work in the way I just tried. Now Sage won't even start.",
...
I have updated the [new spkg](http://sage.math.washington.edu/home/SimonKing/SAGE/spkgs/singular-3-1-5.p2.spkg) and posted a new version of [attachment:singular_xalloc.patch].
...
```json
{
    "body": "PS: Normally, mylimits.h just includes \"limits.h\" - but in IRIX-6 machines that wouldn't work, and so on these machines the header omlimits.h generated by the configure script would be included.\n\nHence, I will soon create a new spkg that installs mylimits.h including limits.h - but the xalloc-version of the spkg will not work on IRIX-6 machines.",
...
```json
{
    "body": "Attachment [singular_xalloc.patch](tarball://root/attachments/some-uuid/ticket13731/singular_xalloc.patch) by @simon-king-jena created at 2012-11-30 14:26:19\n\nPPS: The spkg installing mylimits.h is already posted.",
...
Attachment [singular_xalloc.patch](tarball://root/attachments/some-uuid/ticket13731/singular_xalloc.patch) by @simon-king-jena created at 2012-11-30 14:26:19
...
```json
{
    "body": "PS: The graph_generators test pass fine if `MALLOC_CHECK_` is not defined. Hence, we see a new bug here. I'll report on sage-devel.",
...
```json
{
    "body": "PPS: The graph_generators test also crashes with the default omalloc Singular. Hence, it definitely is unrelated.",
...
```json
{
    "body": "How can I increase the timeout for doctests? According to what google gave me, I tried `export TIMEOUT=1800`, but that didn't work.",
...
```json
{
    "body": "Aha! It seems to require `SAGE_TIMEOUT`!",
...
```json
{
    "body": "Even a timeout of 1800 seconds is not enough for any test. I tried sage/schemes (because memory problems often show up there), but every single test times out.",
...
```json
{
    "body": "I have produced a new patchlevel for the singular spkg. The optional patch is now smaller, since I simply erase the content of the omalloc/ folder, so that the removed files are not part of the patch.\n\nNeeds review!",
    "created_at": "2012-12-03T13:04:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13731",
    "type": "issue_comment",
...
```json
{
    "body": "doctests check out as far as I can test. Business rules for spkg have been followed, so I think this can get a positive review.\n\nThe new spkg doesn't actually contain code written by me, so I think I'm eligible to give a positive review.",
    "created_at": "2012-12-07T21:29:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13731",
    "type": "issue_comment",
...
```json
{
    "body": "Where did the Makefile for xalloc come from? It's written in a non-autotools way and should be fixed. It's probably easy to fix, but it should be reported \"upstream\", whoever that is.",
...
```json
{
    "body": "I'll fix the Makefile, hang on...",
...
```json
{
    "body": "Attachment [xalloc-makefile.diff](tarball://root/attachments/some-uuid/ticket13731/xalloc-makefile.diff) by @jdemeyer created at 2012-12-21 11:01:30\n\nI changed the spkg, the changes are here: [attachment:xalloc-makefile.diff]. Needs review.",
...
Attachment [xalloc-makefile.diff](tarball://root/attachments/some-uuid/ticket13731/xalloc-makefile.diff) by @jdemeyer created at 2012-12-21 11:01:30
...
```json
{
    "body": "I'm trying to run Sage under the electric fence memory debugger and also ran into the `prime_phi` issue. It seems you managed to discuss it in detail but then failed to open an actual ticket for it. This is now #13861",
...
```json
{
    "body": "Jeroen's changes look good to me.",
...
```json
{
    "body": "Sorry for asking so late (after resolving the ticket as fixed). But perhaps it has been a bad idea to use a new environment variable `SINGULAR_XALLOC` to trigger the use of xalloc. What do you think: Would it be a better idea to build singular with xalloc if `SAGE_DEBUG` is set to \"yes\", as in #13864?",
...
```json
{
    "body": "Switching `SINGULAR_XALLOC` to `SAGE_DEBUG` will be done in #13876.",
