# Issue 13728: Implements some standard methods for fields

archive/issues_013524.json:
```json
{
    "assignees": [],
    "body": "Implements\n\n- `is_integral_domain`\n- `is_field`\n- `fraction_field`\n- `__pow__`\n\nfor fields, and\n\n- `is_unit`\n\nfor field elements.\n\nAssignee: **@aghitza**\n\nCC:  @fchapoton\n\nKeywords: **field, ring, is_unit**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Christian Stump**\n\nMerged: **sage-5.6.beta0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13728_\n\n",
    "closed_at": "2012-12-18T11:15:03Z",
    "created_at": "2012-11-20T10:13:14Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Implements some standard methods for fields",
    "type": "issue",
    "updated_at": "2012-12-18T11:15:03Z",
    "url": "https://github.com/sagemath/sage/issues/13728",
    "user": "https://github.com/sagetrac-stumpc5"
}
```
Implements

- `is_integral_domain`
- `is_field`
- `fraction_field`
- `__pow__`

for fields, and

- `is_unit`

for field elements.

Assignee: **@aghitza**

CC:  @fchapoton

Keywords: **field, ring, is_unit**

Reviewer: **Frédéric Chapoton**

Author: **Christian Stump**

Merged: **sage-5.6.beta0**

_Issue created by migration from https://trac.sagemath.org/ticket/13728_





---

archive/issue_events_176426.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2012-11-20T10:13:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13728",
    "milestone_number": null,
    "milestone_title": "sage-5.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13728#event-176426"
}
```



---

archive/issue_events_176427.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2012-11-20T10:13:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13728",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13728#event-176427"
}
```



---

archive/issue_events_176428.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2012-11-20T10:13:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13728",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13728#event-176428"
}
```



---

archive/issue_events_176429.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2012-11-20T10:13:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13728",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13728#event-176429"
}
```



---

archive/issue_comments_163766.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,12 +1,12 @@\n Implements\n \n-- is_integral_domain\n-- is_field\n-- fraction_field\n-- !__pow__\n+- `is_integral_domain`\n+- `is_field`\n+- `fraction_field`\n+- `__pow__`\n \n for fields, and\n \n-- is_unit\n+- `is_unit`\n \n for field elements.\n``````\n",
    "created_at": "2012-11-20T10:17:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13728",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13728#issuecomment-163766",
    "user": "https://github.com/sagetrac-stumpc5"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,12 +1,12 @@
 Implements
 
-- is_integral_domain
-- is_field
-- fraction_field
-- !__pow__
+- `is_integral_domain`
+- `is_field`
+- `fraction_field`
+- `__pow__`
 
 for fields, and
 
-- is_unit
+- `is_unit`
 
 for field elements.
``````




---

archive/issue_events_176430.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2012-11-20T10:17:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13728",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13728#event-176430"
}
```



---

archive/issue_comments_163767.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2012-11-20T11:18:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13728",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13728#issuecomment-163767",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_163768.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nAttachment: **[trac_13728-some_standard_methods_for_fields-cs.patch.gz](https://github.com/sagemath/sage/files/ticket13728/trac_13728-some_standard_methods_for_fields-cs.patch.gz)**\n\nok, looks good. Positive review",
    "created_at": "2012-11-20T11:18:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13728",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13728#issuecomment-163768",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:2'>Comment 2:</a>
Attachment: **[trac_13728-some_standard_methods_for_fields-cs.patch.gz](https://github.com/sagemath/sage/files/ticket13728/trac_13728-some_standard_methods_for_fields-cs.patch.gz)**

ok, looks good. Positive review



---

archive/issue_events_176431.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2012-11-20T11:18:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13728",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13728#event-176431"
}
```



---

archive/issue_events_176432.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2012-11-20T11:18:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13728",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13728#event-176432"
}
```



---

archive/issue_events_176433.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-12-18T11:15:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13728",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13728#event-176433"
}
```



---

archive/issue_events_176434.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-12-18T11:15:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13728",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13728#event-176434"
}
```



---

archive/issue_comments_163769.json:
```json
{
    "body": "Merged: **sage-5.6.beta0**",
    "created_at": "2012-12-18T11:15:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13728",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13728#issuecomment-163769",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.6.beta0**
