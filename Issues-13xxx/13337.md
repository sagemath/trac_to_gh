# Issue 13337: expression.pyx fails to build on Cygwin

archive/issues_013165.json:
```json
{
    "assignees": [],
    "body": "expression.pyx fails to build on Cygwin because of template problems with the infinity class from pyanc.\nMaybe its confused with sage.rings.infinity[.infinity]?\nReplacing it with GiNaC::infinity pleases g++.\n\nApply [attachment: trac_13337-template.patch.gz](https://github.com/sagemath/sage/files/ticket13337/trac_13337-template.patch.gz).\n\nKeywords: **cygwin pynac**\n\nReviewer: **Dmitrii Pasechnik**\n\nAuthor: **Jean-Pierre Flori**\n\nMerged: **sage-5.3.beta1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13337_\n\n",
    "closed_at": "2012-08-12T19:02:52Z",
    "created_at": "2012-08-04T20:08:48Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "expression.pyx fails to build on Cygwin",
    "type": "issue",
    "updated_at": "2012-08-12T19:02:52Z",
    "url": "https://github.com/sagemath/sage/issues/13337",
    "user": "https://github.com/sagetrac-jpflori"
}
```
expression.pyx fails to build on Cygwin because of template problems with the infinity class from pyanc.
Maybe its confused with sage.rings.infinity[.infinity]?
Replacing it with GiNaC::infinity pleases g++.

Apply [attachment: trac_13337-template.patch.gz](https://github.com/sagemath/sage/files/ticket13337/trac_13337-template.patch.gz).

Keywords: **cygwin pynac**

Reviewer: **Dmitrii Pasechnik**

Author: **Jean-Pierre Flori**

Merged: **sage-5.3.beta1**

_Issue created by migration from https://trac.sagemath.org/ticket/13337_





---

archive/issue_events_170562.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2012-08-04T20:08:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13337",
    "milestone_number": null,
    "milestone_title": "sage-5.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13337#event-170562"
}
```



---

archive/issue_events_170563.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2012-08-04T20:08:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13337",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13337#event-170563"
}
```



---

archive/issue_comments_157023.json:
```json
{
    "body": "Attachment: **[trac_13337-template.patch.gz](https://github.com/sagemath/sage/files/ticket13337/trac_13337-template.patch.gz)**\n\nMake ginac.pxk more explicit for infinity class.",
    "created_at": "2012-08-04T20:12:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13337",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13337#issuecomment-157023",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Attachment: **[trac_13337-template.patch.gz](https://github.com/sagemath/sage/files/ticket13337/trac_13337-template.patch.gz)**

Make ginac.pxk more explicit for infinity class.



---

archive/issue_comments_157024.json:
```json
{
    "body": "Changed keywords from **** to **cygwin pynac**",
    "created_at": "2012-08-04T20:13:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13337",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13337#issuecomment-157024",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Changed keywords from **** to **cygwin pynac**



---

archive/issue_events_170564.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2012-08-04T20:13:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13337",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13337#event-170564"
}
```



---

archive/issue_events_170565.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2012-08-04T20:13:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13337",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
    "label_color": "000080",
    "label_name": "component: porting: cygwin",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13337#event-170565"
}
```



---

archive/issue_comments_157025.json:
```json
{
    "body": "Author: **Jean-Pierre Flori**",
    "created_at": "2012-08-04T20:13:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13337",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13337#issuecomment-157025",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Author: **Jean-Pierre Flori**



---

archive/issue_comments_157026.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,5 @@\n expression.pyx fails to build on Cygwin because of template problems with the infinity class from pyanc.\n Maybe its confused with sage.rings.infinity[.infinity]?\n-Replacing it with GiNaC::infinity please g++.\n+Replacing it with GiNaC::infinity pleases g++.\n+\n+Apply [attachment: trac_13337-template.patch.gz](https://github.com/sagemath/sage/files/ticket13337/trac_13337-template.patch.gz).\n``````\n",
    "created_at": "2012-08-04T20:13:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13337",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13337#issuecomment-157026",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,5 @@
 expression.pyx fails to build on Cygwin because of template problems with the infinity class from pyanc.
 Maybe its confused with sage.rings.infinity[.infinity]?
-Replacing it with GiNaC::infinity please g++.
+Replacing it with GiNaC::infinity pleases g++.
+
+Apply [attachment: trac_13337-template.patch.gz](https://github.com/sagemath/sage/files/ticket13337/trac_13337-template.patch.gz).
``````




---

archive/issue_events_170566.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2012-08-04T20:13:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13337",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13337#event-170566"
}
```



---

archive/issue_comments_157027.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThis fix is needed and works on my Cygwin, too.",
    "created_at": "2012-08-05T04:49:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13337",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13337#issuecomment-157027",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:2'>Comment 2:</a>
This fix is needed and works on my Cygwin, too.



---

archive/issue_events_170567.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2012-08-08T04:21:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13337",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13337#event-170567"
}
```



---

archive/issue_events_170568.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2012-08-08T04:21:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13337",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13337#event-170568"
}
```



---

archive/issue_comments_157028.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nlooks good.",
    "created_at": "2012-08-08T04:21:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13337",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13337#issuecomment-157028",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:3'>Comment 3:</a>
looks good.



---

archive/issue_comments_157029.json:
```json
{
    "body": "Reviewer: **Dmitrii Pasechnik**",
    "created_at": "2012-08-08T12:35:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13337",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13337#issuecomment-157029",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Dmitrii Pasechnik**



---

archive/issue_events_170569.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-08-12T19:02:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13337",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13337#event-170569"
}
```



---

archive/issue_events_170570.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-08-12T19:02:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13337",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13337#event-170570"
}
```



---

archive/issue_comments_157030.json:
```json
{
    "body": "Merged: **sage-5.3.beta1**",
    "created_at": "2012-08-12T19:02:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13337",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13337#issuecomment-157030",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.3.beta1**
