# Issue 13790: Allow SR.symbol() to return a tuple.

archive/issues_013586.json:
```json
{
    "assignees": [],
    "body": "The standard `var` function can return a tuple of symbols:\n\n```\nsage: a1,a2,a3 = var('a1,a2,a3')\n```\n\nBut `SR.symbol()`, which is recommended for use in library code, cannot:\n\n```\nsage: a1,a2,a3 = SR.symbol('a1,a2,a3')\n---------------------------------------------------------------------------\nTypeError                                 Traceback (most recent call last)\n\n/home/mjo/<ipython console> in <module>()\n\nTypeError: 'sage.symbolic.expression.Expression' object is not iterable\n```\n\nFor convenience and consistently, they should behave the same. There is a slight possibility for backwards-incompatibility here, since `SR.symbol()` will create a symbol with a comma in the name:\n\n```\nsage: foo = SR.symbol('a1,a2,a3')     \nsage: foo\na1,a2,a3\n```\n\nbut hopefully, that's not actually used anywhere.\n\nAssignee: **@burcin**\n\nReviewer: **Nils Bruin**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13790_\n\n",
    "closed_at": "2012-12-21T22:46:38Z",
    "created_at": "2012-12-02T20:57:37Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Allow SR.symbol() to return a tuple.",
    "type": "issue",
    "updated_at": "2012-12-21T22:46:38Z",
    "url": "https://github.com/sagemath/sage/issues/13790",
    "user": "https://github.com/orlitzky"
}
```
The standard `var` function can return a tuple of symbols:

```
sage: a1,a2,a3 = var('a1,a2,a3')
```

But `SR.symbol()`, which is recommended for use in library code, cannot:

```
sage: a1,a2,a3 = SR.symbol('a1,a2,a3')
---------------------------------------------------------------------------
TypeError                                 Traceback (most recent call last)

/home/mjo/<ipython console> in <module>()

TypeError: 'sage.symbolic.expression.Expression' object is not iterable
```

For convenience and consistently, they should behave the same. There is a slight possibility for backwards-incompatibility here, since `SR.symbol()` will create a symbol with a comma in the name:

```
sage: foo = SR.symbol('a1,a2,a3')     
sage: foo
a1,a2,a3
```

but hopefully, that's not actually used anywhere.

Assignee: **@burcin**

Reviewer: **Nils Bruin**

_Issue created by migration from https://trac.sagemath.org/ticket/13790_





---

archive/issue_events_177372.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2012-12-02T20:57:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13790",
    "milestone_number": null,
    "milestone_title": "sage-5.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13790#event-177372"
}
```



---

archive/issue_events_177373.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2012-12-02T20:57:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13790",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13790#event-177373"
}
```



---

archive/issue_events_177374.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2012-12-02T20:57:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13790",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13790#event-177374"
}
```



---

archive/issue_events_177375.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2012-12-02T20:57:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13790",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13790#event-177375"
}
```



---

archive/issue_comments_164821.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\n-1: we already have `SR.var` for that.\n\nI'm pretty sure that `SR.symbol` is recommended in library code specifically *because* its return type does not depend on the presence of `,` in the input string. That makes it much easier to deal with its results in a programmatic manner.\n\nIn general, you want a primitive an absolutely basic access function to infrastructure (`SR.symbol`) that does minimal processing on input and is completely regular in its output. If it turns out certain usage patterns arise again and again, one can introduce convenience wrappers (which are easy to write *beause* the primitive is so regular):\n\n```\nmap(SR.symbol,('a','b','c'))\n```\nwhich got wrapped as\n\n```\nmap(SR.symbol,'a,b,c'.split(','))\n```\nbecause people were probably hesitant about making `var` variadic and/or didn't know that could be done while keeping keyword-only parameters as well and/or didn't like typing the extra quotes involved.",
    "created_at": "2012-12-02T21:17:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13790",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13790#issuecomment-164821",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:1'>Comment 1:</a>
-1: we already have `SR.var` for that.

I'm pretty sure that `SR.symbol` is recommended in library code specifically *because* its return type does not depend on the presence of `,` in the input string. That makes it much easier to deal with its results in a programmatic manner.

In general, you want a primitive an absolutely basic access function to infrastructure (`SR.symbol`) that does minimal processing on input and is completely regular in its output. If it turns out certain usage patterns arise again and again, one can introduce convenience wrappers (which are easy to write *beause* the primitive is so regular):

```
map(SR.symbol,('a','b','c'))
```
which got wrapped as

```
map(SR.symbol,'a,b,c'.split(','))
```
because people were probably hesitant about making `var` variadic and/or didn't know that could be done while keeping keyword-only parameters as well and/or didn't like typing the extra quotes involved.



---

archive/issue_events_177376.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2012-12-02T21:23:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13790",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13790#event-177376"
}
```



---

archive/issue_comments_164822.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nI see. I didn't know `SR.var` and `var` behaved differently. Sorry for the noise.",
    "created_at": "2012-12-02T21:23:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13790",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13790#issuecomment-164822",
    "user": "https://github.com/orlitzky"
}
```

<a id='comment:2'>Comment 2:</a>
I see. I didn't know `SR.var` and `var` behaved differently. Sorry for the noise.



---

archive/issue_events_177377.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2012-12-02T21:24:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13790",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13790#event-177377"
}
```



---

archive/issue_events_177378.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2012-12-02T21:24:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13790",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13790#event-177378"
}
```



---

archive/issue_events_177379.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2012-12-02T21:24:22Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13790",
    "milestone_number": null,
    "milestone_title": "sage-5.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13790#event-177379"
}
```



---

archive/issue_events_177380.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2012-12-02T21:24:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13790",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13790#event-177380"
}
```



---

archive/issue_events_177381.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2012-12-02T21:24:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13790",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13790#event-177381"
}
```



---

archive/issue_events_177382.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-12-21T22:46:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13790",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13790#event-177382"
}
```



---

archive/issue_events_177383.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-12-21T22:46:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13790",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13790#event-177383"
}
```



---

archive/issue_comments_164823.json:
```json
{
    "body": "Reviewer: **Nils Bruin**",
    "created_at": "2012-12-21T22:46:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13790",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13790#issuecomment-164823",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Nils Bruin**
