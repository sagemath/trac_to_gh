# Issue 13293: Change add_big_oh to add_bigoh in padic factorization code

archive/issues_013121.json:
```json
{
    "assignees": [],
    "body": "The following code produces an error:\n\n```\nsage: R.<T>=Qp(3)[]\nsage: f=1926*T^2 + 312*T + 387\nsage: f.factor()\nAttributeError: 'sage.rings.padics.padic_capped_absolute_element.pAdicCappedAbsoluteElement' object has no attribute 'add_big_oh'\n```\n\nThis appears to be a typo. Changing it to `add_bigoh` resolves the problem.\n\nAssignee: **@roed314**\n\nReviewer: **David Roe**\n\nAuthor: **Julian Rueth**\n\nMerged: **sage-5.3.beta1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13293_\n\n",
    "closed_at": "2012-08-12T19:01:25Z",
    "created_at": "2012-07-26T14:00:47Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20padics",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/good%20first%20issue"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Change add_big_oh to add_bigoh in padic factorization code",
    "type": "issue",
    "updated_at": "2012-08-12T19:01:25Z",
    "url": "https://github.com/sagemath/sage/issues/13293",
    "user": "https://github.com/saraedum"
}
```
The following code produces an error:

```
sage: R.<T>=Qp(3)[]
sage: f=1926*T^2 + 312*T + 387
sage: f.factor()
AttributeError: 'sage.rings.padics.padic_capped_absolute_element.pAdicCappedAbsoluteElement' object has no attribute 'add_big_oh'
```

This appears to be a typo. Changing it to `add_bigoh` resolves the problem.

Assignee: **@roed314**

Reviewer: **David Roe**

Author: **Julian Rueth**

Merged: **sage-5.3.beta1**

_Issue created by migration from https://trac.sagemath.org/ticket/13293_





---

archive/issue_events_169830.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2012-07-26T14:00:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13293",
    "milestone_number": null,
    "milestone_title": "sage-5.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13293#event-169830"
}
```



---

archive/issue_events_169831.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2012-07-26T14:00:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13293",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20padics",
    "label_color": "0000ff",
    "label_name": "component: padics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13293#event-169831"
}
```



---

archive/issue_events_169832.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2012-07-26T14:00:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13293",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13293#event-169832"
}
```



---

archive/issue_events_169833.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2012-07-26T14:00:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13293",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13293#event-169833"
}
```



---

archive/issue_events_169834.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2012-07-26T14:49:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13293",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13293#event-169834"
}
```



---

archive/issue_events_169835.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2012-07-26T14:49:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13293",
    "label": "https://github.com/sagemath/sage/labels/good%20first%20issue",
    "label_color": "7057ff",
    "label_name": "good first issue",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13293#event-169835"
}
```



---

archive/issue_comments_155949.json:
```json
{
    "body": "Author: **Julian Rueth**",
    "created_at": "2012-07-26T14:49:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13293",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13293#issuecomment-155949",
    "user": "https://github.com/saraedum"
}
```

Author: **Julian Rueth**



---

archive/issue_comments_155950.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nA typo: \"Check that :trac:`13293` if fixed\".\n\nOther than that, I'm happy to give this a positive review once patchbot tests it.",
    "created_at": "2012-07-26T20:00:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13293",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13293#issuecomment-155950",
    "user": "https://github.com/roed314"
}
```

<a id='comment:2'>Comment 2:</a>
A typo: "Check that :trac:`13293` if fixed".

Other than that, I'm happy to give this a positive review once patchbot tests it.



---

archive/issue_comments_155951.json:
```json
{
    "body": "Attachment: **[trac_13293.patch.gz](https://github.com/sagemath/sage/files/ticket13293/trac_13293.patch.gz)**",
    "created_at": "2012-07-26T20:53:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13293",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13293#issuecomment-155951",
    "user": "https://github.com/saraedum"
}
```

Attachment: **[trac_13293.patch.gz](https://github.com/sagemath/sage/files/ticket13293/trac_13293.patch.gz)**



---

archive/issue_comments_155952.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThanks for reviewing. I fixed the typo.",
    "created_at": "2012-07-26T20:54:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13293",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13293#issuecomment-155952",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:3'>Comment 3:</a>
Thanks for reviewing. I fixed the typo.



---

archive/issue_comments_155953.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nLooks good.  Sorry for the delay: I've been traveling.",
    "created_at": "2012-08-03T00:59:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13293",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13293#issuecomment-155953",
    "user": "https://github.com/roed314"
}
```

<a id='comment:4'>Comment 4:</a>
Looks good.  Sorry for the delay: I've been traveling.



---

archive/issue_events_169836.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-08-03T00:59:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13293",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13293#event-169836"
}
```



---

archive/issue_events_169837.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2012-08-03T00:59:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13293",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13293#event-169837"
}
```



---

archive/issue_comments_155954.json:
```json
{
    "body": "Reviewer: **David Roe**",
    "created_at": "2012-08-04T14:48:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13293",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13293#issuecomment-155954",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **David Roe**



---

archive/issue_events_169838.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-08-12T19:01:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13293",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13293#event-169838"
}
```



---

archive/issue_events_169839.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-08-12T19:01:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13293",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13293#event-169839"
}
```



---

archive/issue_comments_155955.json:
```json
{
    "body": "Merged: **sage-5.3.beta1**",
    "created_at": "2012-08-12T19:01:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13293",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13293#issuecomment-155955",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.3.beta1**
