...
<a id='comment:1'></a>There's the more severe problem that during GC cleanup, a weakref callback could lead to the deletion of a non-weakreffable key, which could retrigger the dealloc of the object that has just generated the weakref. That would lead to a double dealloc.
...
```json
{
    "body": "<a id='comment:2'></a>I don't think we need this ticket for the `__delitem__` optimization. That can be wrapped into #13387.",
...
<a id='comment:2'></a>I don't think we need this ticket for the `__delitem__` optimization. That can be wrapped into #13387.
...
<a id='comment:3'></a>Nils, indeed it might be an idea to use the ideas from #13387. However, the ticket here is not about optimization.
...
```json
{
    "body": "<a id='comment:5'></a>Oops, I just notice that the patch attempts to fix `TripleDictEraser.__call__`, hence, the callback, but not `TripleDict.__delitem__`. So, I *did* have a look into the callback function...",
...
<a id='comment:5'></a>Oops, I just notice that the patch attempts to fix `TripleDictEraser.__call__`, hence, the callback, but not `TripleDict.__delitem__`. So, I *did* have a look into the callback function...
...
<a id='comment:6'></a>Replying to [SimonKing](#comment%3A4):
> Ah, you say that the problem actually isn't so much the `__delitem__` but the callback? OK, I'll have a look into that. And of course we should try to rebase #13387.
...
<a id='comment:7'></a>Replying to [nbruin](#comment%3A1):
> There's the more severe problem that during GC cleanup, a weakref callback could lead to the deletion of a non-weakreffable key, which could retrigger the dealloc of the object that has just generated the weakref. That would lead to a double dealloc.
...
<a id='comment:9'></a>Replying to [nbruin](#comment%3A1):
> There's the more severe problem that during GC cleanup, a weakref callback could lead to the deletion of a non-weakreffable key, which could retrigger the dealloc of the object that has just generated the weakref. That would lead to a double dealloc.
...
```json
{
    "body": "<a id='comment:12'></a>By the way, my first attachment is wrong anyway, because I mispelled `gc.isenabled()`.",
...
<a id='comment:12'></a>By the way, my first attachment is wrong anyway, because I mispelled `gc.isenabled()`.
...
<a id='comment:13'></a>Attachment [trac_13904_debug_only.patch](tarball://root/attachments/some-uuid/ticket13904/trac_13904_debug_only.patch) by @simon-king-jena created at 2013-01-03 21:26:56
...
<a id='comment:15'></a>Attachment [test.sage](tarball://root/attachments/some-uuid/ticket13904/test.sage) by @nbruin created at 2013-01-03 21:47:44
...
<a id='comment:20'></a>It seems that indeed #13896 fixes the problem. Hooray `\<sup>.</sup>/`
...
<a id='comment:22'></a>Can we get the change from __delitem__ to del quickly in?
It is trivial, produce better C code, and potentially avoid horribly complicated gc problems?
...
<a id='comment:23'></a>Oh in fact, I've read a little less quickly and the problem Nils thought about does not occur?
So let's just get the trivial improvement here.
I agree it is not a fix, but an imporvement, so I've changed the ticket title to reflect that as well.
...
```json
{
    "body": "<a id='comment:25'></a>Note that #12313 comprises a patch that makes the callback of a `TripleDictEraser` (hence, the deletion of dead items of a `TripleDict`) more secure. But I forgot to include the little improvement from here. Hence, I still think this ticket here is valid.",
...
<a id='comment:25'></a>Note that #12313 comprises a patch that makes the callback of a `TripleDictEraser` (hence, the deletion of dead items of a `TripleDict`) more secure. But I forgot to include the little improvement from here. Hence, I still think this ticket here is valid.
...
<a id='comment:26'></a>Attachment [trac13904_use_del.patch](tarball://root/attachments/some-uuid/ticket13904/trac13904_use_del.patch) by @simon-king-jena created at 2013-01-21 10:10:44
...
```json
{
    "body": "<a id='comment:31'></a>Unmerging because of trouble with #12313.",
    "created_at": "2013-02-20T16:04:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13904",
    "type": "issue_comment",
...
<a id='comment:31'></a>Unmerging because of trouble with #12313.
...
```json
{
    "body": "<a id='comment:32'></a>Perhaps try #13387 instead? With the solution there, this ticket is superseded there. It improves the performance of these dictionaries in some other ways as well.",
...
<a id='comment:32'></a>Perhaps try #13387 instead? With the solution there, this ticket is superseded there. It improves the performance of these dictionaries in some other ways as well.
...
```json
{
    "body": "<a id='comment:33'></a>So this one should get closed as #12313 and #13387 will get merged together?",
...
<a id='comment:33'></a>So this one should get closed as #12313 and #13387 will get merged together?
