# Issue 13171: limit of expression that tends to zeta(1/2)

archive/issues_012999.json:
```json
{
    "assignees": [],
    "body": "The following cannot be evaluated in Sage\n\n```\nsage: var('k N')\n(k, N)\nsage: (sum(1/sqrt(k),k,1,N) - 2*sqrt(N)).limit(N=oo)\nlimit(-2*sqrt(N) + sum(1/sqrt(k), k, 1, N), N, +Infinity)\n```\nBut Mathematica can\n\n```\nIn[1]:= Limit[Sum[1/Sqrt[k], {k, 1, N}] - 2 Sqrt[N], N -> \\[Infinity]]\n\n             1\nOut[1]= Zeta[-]\n             2\n```\n\nAssignee: **@burcin**\n\nKeywords: **limit expression sum zeta mathematica**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13171_\n\n",
    "created_at": "2012-06-27T16:42:38Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/wishlist%20item"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "limit of expression that tends to zeta(1/2)",
    "type": "issue",
    "updated_at": "2015-01-31T09:52:43Z",
    "url": "https://github.com/sagemath/sage/issues/13171",
    "user": "https://github.com/dkrenn"
}
```
The following cannot be evaluated in Sage

```
sage: var('k N')
(k, N)
sage: (sum(1/sqrt(k),k,1,N) - 2*sqrt(N)).limit(N=oo)
limit(-2*sqrt(N) + sum(1/sqrt(k), k, 1, N), N, +Infinity)
```
But Mathematica can

```
In[1]:= Limit[Sum[1/Sqrt[k], {k, 1, N}] - 2 Sqrt[N], N -> \[Infinity]]

             1
Out[1]= Zeta[-]
             2
```

Assignee: **@burcin**

Keywords: **limit expression sum zeta mathematica**

_Issue created by migration from https://trac.sagemath.org/ticket/13171_





---

archive/issue_events_167711.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2012-06-27T16:42:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13171",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13171#event-167711"
}
```



---

archive/issue_events_167712.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2012-06-27T16:42:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13171",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13171#event-167712"
}
```



---

archive/issue_events_167713.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2012-06-27T16:42:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13171",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13171#event-167713"
}
```



---

archive/issue_events_167714.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2012-06-27T16:42:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13171",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13171#event-167714"
}
```



---

archive/issue_comments_153329.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nIs this only a problem in the way we convert `sum()` to maxima? Otherwise, this ticket should be put in the `sage-wishlist` milestone.",
    "created_at": "2012-06-28T09:08:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13171",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13171#issuecomment-153329",
    "user": "https://github.com/burcin"
}
```

<div id="comment:1" align="right">Comment 1</div>

Is this only a problem in the way we convert `sum()` to maxima? Otherwise, this ticket should be put in the `sage-wishlist` milestone.



---

archive/issue_events_167715.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13171",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13171#event-167715"
}
```



---

archive/issue_events_167716.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13171",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13171#event-167716"
}
```



---

archive/issue_events_167717.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13171",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13171#event-167717"
}
```



---

archive/issue_events_167718.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13171",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13171#event-167718"
}
```



---

archive/issue_events_167719.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13171",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13171#event-167719"
}
```



---

archive/issue_events_167720.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13171",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13171#event-167720"
}
```



---

archive/issue_events_167721.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13171",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13171#event-167721"
}
```



---

archive/issue_events_167722.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13171",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13171#event-167722"
}
```



---

archive/issue_events_167723.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-01-31T09:52:43Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13171",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13171#event-167723"
}
```



---

archive/issue_events_167724.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-01-31T09:52:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13171",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13171#event-167724"
}
```
