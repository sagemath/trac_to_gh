# Issue 13748: Fix sig_block() doctest

archive/issues_013544.json:
```json
{
    "body": "The following problem arose while working on #12415.  Put the following code in your `~/.sage/init.sage`:\n\n```\nimport multiprocessing\nclass Task(multiprocessing.Process):\n    def run(self):\n        sage_namespace = dict(sage.all_cmdline.__dict__)\n        exec compile(\"from sage.tests.interrupt import *\",\"\",\"single\",0,1) in sage_namespace\n        exec compile(\"test_sig_block()\",\"\",\"single\",0,1) in sage_namespace\n        exec compile(\"sig_on_count()\",\"\",\"single\",0,1) in sage_namespace\nTask().start()\n```\n\nIt will result in the following being printed (though your Sage process survives since the segfault occurs in a chile process):\n\n```\n42\n0\n\n------------------------------------------------------------------------\nUnhandled SIGSEGV: A segmentation fault occurred in Sage.\nThis probably occurred because a *compiled* component of Sage has a bug\nin it and is not properly wrapped with sig_on(), sig_off(). You might\nwant to run Sage under gdb with 'sage -gdb' to debug this.\nSage will now terminate.\n------------------------------------------------------------------------\n```\n\nTyping it in to a running Sage session does not cause the same failure.\n\nAssignee: @jdemeyer\n\nCC:  @kini @ohanar @jhpalmieri @jdemeyer\n\nReviewer: Volker Braun\n\nAuthor: Jeroen Demeyer\n\nMerged: sage-5.6.beta3\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/13748\n\n",
    "closed_at": "2013-01-09T09:00:02Z",
    "created_at": "2012-11-24T13:38:46Z",
    "labels": [
        "component: c_lib",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-5.6",
    "title": "Fix sig_block() doctest",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/13748",
    "user": "https://github.com/roed314"
}
```
The following problem arose while working on #12415.  Put the following code in your `~/.sage/init.sage`:

```
import multiprocessing
class Task(multiprocessing.Process):
    def run(self):
        sage_namespace = dict(sage.all_cmdline.__dict__)
        exec compile("from sage.tests.interrupt import *","","single",0,1) in sage_namespace
        exec compile("test_sig_block()","","single",0,1) in sage_namespace
        exec compile("sig_on_count()","","single",0,1) in sage_namespace
Task().start()
```

It will result in the following being printed (though your Sage process survives since the segfault occurs in a chile process):

```
42
0

------------------------------------------------------------------------
Unhandled SIGSEGV: A segmentation fault occurred in Sage.
This probably occurred because a *compiled* component of Sage has a bug
in it and is not properly wrapped with sig_on(), sig_off(). You might
want to run Sage under gdb with 'sage -gdb' to debug this.
Sage will now terminate.
------------------------------------------------------------------------
```

Typing it in to a running Sage session does not cause the same failure.

Assignee: @jdemeyer

CC:  @kini @ohanar @jhpalmieri @jdemeyer

Reviewer: Volker Braun

Author: Jeroen Demeyer

Merged: sage-5.6.beta3

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/13748





---

archive/issue_comments_208630.json:
```json
{
    "body": "<a id='comment:1'></a>I found the culprit. Putting all the code inside `try:/except:` instead of just `sig_on()` fixes the problem.",
    "created_at": "2013-01-03T17:25:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13748",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13748#issuecomment-208630",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:1'></a>I found the culprit. Putting all the code inside `try:/except:` instead of just `sig_on()` fixes the problem.



---

archive/issue_comments_208631.json:
```json
{
    "body": "<a id='comment:2'></a>Attachment [13748_fix_sig_block_test.patch](tarball://root/attachments/some-uuid/ticket13748/13748_fix_sig_block_test.patch) by @jdemeyer created at 2013-01-03 18:38:57\n\nApparently, it is illegal to exit a `try` block while `sig_on()` is in effect.\n\nPatch (which also adds some debug code for `sig_block()`/`sig_unblock()`) needs review.",
    "created_at": "2013-01-03T18:38:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13748",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13748#issuecomment-208631",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'></a>Attachment [13748_fix_sig_block_test.patch](tarball://root/attachments/some-uuid/ticket13748/13748_fix_sig_block_test.patch) by @jdemeyer created at 2013-01-03 18:38:57

Apparently, it is illegal to exit a `try` block while `sig_on()` is in effect.

Patch (which also adds some debug code for `sig_block()`/`sig_unblock()`) needs review.



---

archive/issue_comments_208632.json:
```json
{
    "body": "Changing author from \"\" to \"Jeroen Demeyer\"",
    "created_at": "2013-01-03T18:39:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13748",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13748#issuecomment-208632",
    "user": "https://github.com/jdemeyer"
}
```

Changing author from "" to "Jeroen Demeyer"



---

archive/issue_comments_208633.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2013-01-03T18:39:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13748",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13748#issuecomment-208633",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_208634.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2013-01-08T11:41:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13748",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13748#issuecomment-208634",
    "user": "https://github.com/vbraun"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_208635.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Volker Braun\"",
    "created_at": "2013-01-08T11:41:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13748",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13748#issuecomment-208635",
    "user": "https://github.com/vbraun"
}
```

Changing reviewer from "" to "Volker Braun"



---

archive/issue_comments_208636.json:
```json
{
    "body": "<a id='comment:5'></a>I suppose that it does confuse something on the Python heap when you suddenly jump back into the try/except block? \n\nPatch looks good to me.",
    "created_at": "2013-01-08T11:41:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13748",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13748#issuecomment-208636",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:5'></a>I suppose that it does confuse something on the Python heap when you suddenly jump back into the try/except block? 

Patch looks good to me.



---

archive/issue_comments_208637.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-5.6.beta3\"",
    "created_at": "2013-01-09T09:00:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13748",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13748#issuecomment-208637",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-5.6.beta3"



---

archive/issue_events_038516.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-09T09:00:02Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/13748",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13748#event-38516"
}
```



---

archive/issue_comments_208638.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2013-01-09T09:00:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13748",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13748#issuecomment-208638",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed
