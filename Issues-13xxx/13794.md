# Issue 13794: sage -n is broken in 5.5rc0

archive/issues_013590.json:
```json
{
    "assignees": [
        "https://github.com/jasongrout",
        "https://github.com/qed777",
        "https://github.com/williamstein"
    ],
    "body": "<div id=\"comment:0\"></div>\n\nSee this [sage-devel thread](https://groups.google.com/d/topic/sage-devel/dGtkGWYlI-k/discussion).\nI should have opened the bug report here. This is a clear blocker for 5.5.\n\n---\n\nApply [attachment: trac_13794-scripts.patch](https://github.com/sagemath/sage/files/ticket13794/trac_13794-scripts.patch.gz) to the scripts repo.\n\n\nComponent: **notebook**\n\nAuthor: **Jeroen Demeyer, John Palmieri**\n\nReviewer: **Punarbasu Purkayastha**\n\nMerged: **sage-5.5.rc1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13794_\n\n",
    "closed_at": "2012-12-17T09:40:57Z",
    "created_at": "2012-12-04T10:31:09Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage -n is broken in 5.5rc0",
    "type": "issue",
    "updated_at": "2012-12-17T09:40:57Z",
    "url": "https://github.com/sagemath/sage/issues/13794",
    "user": "https://github.com/ppurka"
}
```
<div id="comment:0"></div>

See this [sage-devel thread](https://groups.google.com/d/topic/sage-devel/dGtkGWYlI-k/discussion).
I should have opened the bug report here. This is a clear blocker for 5.5.

---

Apply [attachment: trac_13794-scripts.patch](https://github.com/sagemath/sage/files/ticket13794/trac_13794-scripts.patch.gz) to the scripts repo.


Component: **notebook**

Author: **Jeroen Demeyer, John Palmieri**

Reviewer: **Punarbasu Purkayastha**

Merged: **sage-5.5.rc1**

_Issue created by migration from https://trac.sagemath.org/ticket/13794_





---

archive/issue_events_191642.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2012-12-04T10:31:09Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13794",
    "milestone_number": null,
    "milestone_title": "sage-5.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13794#event-191642"
}
```



---

archive/issue_events_191643.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2012-12-04T10:31:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13794",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "0000b0",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13794#event-191643"
}
```



---

archive/issue_events_191644.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2012-12-04T10:31:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13794",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20blocker%20/%201",
    "label_color": "ff0000",
    "label_name": "p: blocker / 1",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13794#event-191644"
}
```



---

archive/issue_events_191645.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2012-12-04T10:31:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13794",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13794#event-191645"
}
```



---

archive/issue_events_191646.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2012-12-04T10:31:09Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/13794",
    "subject": "https://github.com/ppurka",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13794#event-191646"
}
```



---

archive/issue_events_191647.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2012-12-04T10:31:09Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/13794",
    "subject": "https://github.com/ppurka",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13794#event-191647"
}
```



---

archive/issue_events_191648.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2012-12-04T10:31:09Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/13794",
    "subject": "https://github.com/ppurka",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13794#event-191648"
}
```



---

archive/issue_comments_162480.json:
```json
{
    "body": "Replying to [@ppurka](https://github.com/sagemath/sage/issues/13794#comment:0):\n> See this [sage-devel thread](https://groups.google.com/d/topic/sage-devel/dGtkGWYlI-k/discussion).\n> I should have opened the bug report here. This is a clear blocker for 5.5.\n\n... or posted to sage-**release** in the first place ... ;-)",
    "created_at": "2012-12-04T11:21:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13794#issuecomment-162480",
    "user": "https://github.com/nexttime"
}
```

Replying to [@ppurka](https://github.com/sagemath/sage/issues/13794#comment:0):
> See this [sage-devel thread](https://groups.google.com/d/topic/sage-devel/dGtkGWYlI-k/discussion).
> I should have opened the bug report here. This is a clear blocker for 5.5.

... or posted to sage-**release** in the first place ... ;-)



---

archive/issue_comments_162481.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nSee also #11409, in particular the [scripts patch](https://github.com/sagemath/sage/files/ticket11409/11409_scripts.patch.gz).",
    "created_at": "2012-12-04T17:06:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13794#issuecomment-162481",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:2" align="right">comment:2</div>

See also #11409, in particular the [scripts patch](https://github.com/sagemath/sage/files/ticket11409/11409_scripts.patch.gz).



---

archive/issue_comments_162482.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nReplying to [@jhpalmieri](#comment%3A2):\n> See also #11409, in particular the [scripts patch](https://github.com/sagemath/sage/files/ticket11409/11409_scripts.patch.gz).\n\nI remember having tried only the scripts patch. It didn't work unfortunately. Only the error message changed.",
    "created_at": "2012-12-04T23:34:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13794#issuecomment-162482",
    "user": "https://github.com/ppurka"
}
```

<div id="comment:3" align="right">comment:3</div>

Replying to [@jhpalmieri](#comment%3A2):
> See also #11409, in particular the [scripts patch](https://github.com/sagemath/sage/files/ticket11409/11409_scripts.patch.gz).

I remember having tried only the scripts patch. It didn't work unfortunately. Only the error message changed.



---

archive/issue_comments_162483.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nMaybe we also need this change to `sage-notebook` and `sage-notebook-insecure`:\n\n```diff\ndiff --git a/sage-notebook b/sage-notebook\n--- a/sage-notebook\n+++ b/sage-notebook\n@@ -6,6 +6,7 @@ print open(os.environ['SAGE_ROOT'] + '/l\n \n print \"Please wait while the Sage Notebook server starts...\"\n \n+from sage.all import *\n from sagenb.notebook.all import notebook\n \n print \"...\"\n```",
    "created_at": "2012-12-05T00:39:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13794#issuecomment-162483",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:4" align="right">comment:4</div>

Maybe we also need this change to `sage-notebook` and `sage-notebook-insecure`:

```diff
diff --git a/sage-notebook b/sage-notebook
--- a/sage-notebook
+++ b/sage-notebook
@@ -6,6 +6,7 @@ print open(os.environ['SAGE_ROOT'] + '/l
 
 print "Please wait while the Sage Notebook server starts..."
 
+from sage.all import *
 from sagenb.notebook.all import notebook
 
 print "..."
```



---

archive/issue_comments_162484.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nPlease try the scripts patch from #11409 again. I'm attaching it here and making this ticket a prerequisite for #11409, since it is hard to predict when a new sagenb spkg might be ready, so it's hard to predict when #11409 might get merged.",
    "created_at": "2012-12-08T05:44:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13794#issuecomment-162484",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:5" align="right">comment:5</div>

Please try the scripts patch from #11409 again. I'm attaching it here and making this ticket a prerequisite for #11409, since it is hard to predict when a new sagenb spkg might be ready, so it's hard to predict when #11409 might get merged.



---

archive/issue_comments_162485.json:
```json
{
    "body": "Author: **Jeroen Demeyer, John Palmieri**",
    "created_at": "2012-12-08T05:44:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13794#issuecomment-162485",
    "user": "https://github.com/jhpalmieri"
}
```

Author: **Jeroen Demeyer, John Palmieri**



---

archive/issue_events_191649.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-12-08T05:44:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13794",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13794#event-191649"
}
```



---

archive/issue_comments_162486.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nOk. This patch works by itself with the nb in 5.5rc0. Thanks for the fix.\n\nCan you do the following:\n1. change the name of the patch - it has the wrong ticket number.\n2. change the commit message.\n\nI would give it a positive review otherwise.",
    "created_at": "2012-12-08T06:06:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13794#issuecomment-162486",
    "user": "https://github.com/ppurka"
}
```

<div id="comment:6" align="right">comment:6</div>

Ok. This patch works by itself with the nb in 5.5rc0. Thanks for the fix.

Can you do the following:
1. change the name of the patch - it has the wrong ticket number.
2. change the commit message.

I would give it a positive review otherwise.



---

archive/issue_comments_162487.json:
```json
{
    "body": "Attachment: **[trac_13794-scripts.patch.gz](https://github.com/sagemath/sage/files/ticket13794/trac_13794-scripts.patch.gz)**\n\nscripts repo",
    "created_at": "2012-12-08T16:06:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13794#issuecomment-162487",
    "user": "https://github.com/jhpalmieri"
}
```

Attachment: **[trac_13794-scripts.patch.gz](https://github.com/sagemath/sage/files/ticket13794/trac_13794-scripts.patch.gz)**

scripts repo



---

archive/issue_comments_162488.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nHere's a revised patch.",
    "created_at": "2012-12-08T16:07:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13794#issuecomment-162488",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:7" align="right">comment:7</div>

Here's a revised patch.



---

archive/issue_comments_162489.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,2 +1,7 @@\n See this [sage-devel thread](https://groups.google.com/d/topic/sage-devel/dGtkGWYlI-k/discussion).\n I should have opened the bug report here. This is a clear blocker for 5.5.\n+\n+---\n+\n+Apply [attachment: trac_13794-scripts.patch](https://github.com/sagemath/sage/files/ticket13794/trac_13794-scripts.patch.gz) to the scripts repo.\n+\n``````\n",
    "created_at": "2012-12-08T16:07:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13794#issuecomment-162489",
    "user": "https://github.com/jhpalmieri"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,2 +1,7 @@
 See this [sage-devel thread](https://groups.google.com/d/topic/sage-devel/dGtkGWYlI-k/discussion).
 I should have opened the bug report here. This is a clear blocker for 5.5.
+
+---
+
+Apply [attachment: trac_13794-scripts.patch](https://github.com/sagemath/sage/files/ticket13794/trac_13794-scripts.patch.gz) to the scripts repo.
+
``````




---

archive/issue_comments_162490.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nThanks.",
    "created_at": "2012-12-09T07:13:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13794#issuecomment-162490",
    "user": "https://github.com/ppurka"
}
```

<div id="comment:8" align="right">comment:8</div>

Thanks.



---

archive/issue_comments_162491.json:
```json
{
    "body": "Reviewer: **Punarbasu Purkayastha**",
    "created_at": "2012-12-09T07:13:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13794#issuecomment-162491",
    "user": "https://github.com/ppurka"
}
```

Reviewer: **Punarbasu Purkayastha**



---

archive/issue_events_191650.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2012-12-09T07:13:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13794",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13794#event-191650"
}
```



---

archive/issue_events_191651.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2012-12-09T07:13:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13794",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13794#event-191651"
}
```



---

archive/issue_events_191652.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-12-17T09:40:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13794",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13794#event-191652"
}
```



---

archive/issue_events_191653.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-12-17T09:40:57Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13794",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13794#event-191653"
}
```



---

archive/issue_comments_162492.json:
```json
{
    "body": "Merged: **sage-5.5.rc1**",
    "created_at": "2012-12-17T09:40:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13794",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13794#issuecomment-162492",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.5.rc1**
