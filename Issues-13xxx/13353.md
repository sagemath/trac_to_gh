# Issue 13353: In sage-spkg, export SAGE_KEEP_BUILT_SPKGS

archive/issues_013181.json:
```json
{
    "assignees": [],
    "body": "Analogously to `SAGE_CHECK`, some `spkg-install` scripts might want to use `SAGE_KEEP_BUILT_SPKGS` to determine whether to delete a temporary directory (e.g. to save space during the build).\n\nSo we should export that variable and set it to \"yes\" if the `-s` option to `sage-spkg` is given.\n\nAssignee: **@sagetrac-GeorgSWeber**\n\nCC:  @jhpalmieri\n\nReviewer: **John Palmieri**\n\nAuthor: **Jeroen Demeyer**\n\nMerged: **sage-5.3.beta2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13353_\n\n",
    "closed_at": "2012-08-14T07:06:57Z",
    "created_at": "2012-08-09T08:13:32Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "In sage-spkg, export SAGE_KEEP_BUILT_SPKGS",
    "type": "issue",
    "updated_at": "2012-08-14T07:06:57Z",
    "url": "https://github.com/sagemath/sage/issues/13353",
    "user": "https://github.com/jdemeyer"
}
```
Analogously to `SAGE_CHECK`, some `spkg-install` scripts might want to use `SAGE_KEEP_BUILT_SPKGS` to determine whether to delete a temporary directory (e.g. to save space during the build).

So we should export that variable and set it to "yes" if the `-s` option to `sage-spkg` is given.

Assignee: **@sagetrac-GeorgSWeber**

CC:  @jhpalmieri

Reviewer: **John Palmieri**

Author: **Jeroen Demeyer**

Merged: **sage-5.3.beta2**

_Issue created by migration from https://trac.sagemath.org/ticket/13353_





---

archive/issue_events_170782.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-08-09T08:13:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13353",
    "milestone_number": null,
    "milestone_title": "sage-5.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13353#event-170782"
}
```



---

archive/issue_events_170783.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-08-09T08:13:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13353",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13353#event-170783"
}
```



---

archive/issue_events_170784.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-08-09T08:13:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13353",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13353#event-170784"
}
```



---

archive/issue_events_170785.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-08-09T08:13:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13353",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13353#event-170785"
}
```



---

archive/issue_events_170786.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-08-09T19:32:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13353",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13353#event-170786"
}
```



---

archive/issue_comments_157266.json:
```json
{
    "body": "Author: **Jeroen Demeyer**",
    "created_at": "2012-08-09T19:32:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13353",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13353#issuecomment-157266",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Jeroen Demeyer**



---

archive/issue_comments_157267.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nAttachment: **[13353_SAGE_KEEP_BUILT_SPKGS.patch.gz](https://github.com/sagemath/sage/files/ticket13353/13353_SAGE_KEEP_BUILT_SPKGS.patch.gz)**",
    "created_at": "2012-08-09T19:32:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13353",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13353#issuecomment-157267",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:1" align="right">Comment 1</div>

Attachment: **[13353_SAGE_KEEP_BUILT_SPKGS.patch.gz](https://github.com/sagemath/sage/files/ticket13353/13353_SAGE_KEEP_BUILT_SPKGS.patch.gz)**



---

archive/issue_comments_157268.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nLooks okay to me.",
    "created_at": "2012-08-09T22:39:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13353",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13353#issuecomment-157268",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:2" align="right">Comment 2</div>

Looks okay to me.



---

archive/issue_comments_157269.json:
```json
{
    "body": "Reviewer: **John Palmieri**",
    "created_at": "2012-08-09T22:39:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13353",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13353#issuecomment-157269",
    "user": "https://github.com/jhpalmieri"
}
```

Reviewer: **John Palmieri**



---

archive/issue_events_170787.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-08-09T22:39:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13353",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13353#event-170787"
}
```



---

archive/issue_events_170788.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-08-09T22:39:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13353",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13353#event-170788"
}
```



---

archive/issue_comments_157270.json:
```json
{
    "body": "Merged: **sage-5.3.beta2**",
    "created_at": "2012-08-14T07:05:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13353",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13353#issuecomment-157270",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.3.beta2**



---

archive/issue_events_170789.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-08-14T07:06:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13353",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13353#event-170789"
}
```



---

archive/issue_events_170790.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-08-14T07:06:57Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13353",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13353#event-170790"
}
```
