# Issue 13597: tutorial: fix hash-bang in section on programming

archive/issues_013393.json:
```json
{
    "assignees": [],
    "body": "The [section in the tutorial on standalone scripts](http://www.sagemath.org/doc/tutorial/programming.html#standalone-python-sage-scripts) needs some fixing. It suggests writing a script starting with\n\n```\n#!/usr/bin/env sage -python\n\nimport sys\nfrom sage.all import *\n...\n```\nBut on sage.math.washington.edu, and probably on other linux systems, `/usr/bin/env` doesn't handle multiple arguments very well. I think that replacing the first line with `#!/usr/bin/env sage` should work.\n\nAssignee: **mvngu**\n\nCC:  @kcrisman\n\nBranch/Commit: **[99c96af](https://github.com/sagemath/sagetrac-mirror/commit/99c96af9aeab5460ec0a75462786cfbb94f69c86)**\n\nReviewer: **John Palmieri**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13597_\n\n",
    "closed_at": "2015-09-11T16:47:08Z",
    "created_at": "2012-10-13T02:21:01Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "tutorial: fix hash-bang in section on programming",
    "type": "issue",
    "updated_at": "2015-09-11T16:47:08Z",
    "url": "https://github.com/sagemath/sage/issues/13597",
    "user": "https://github.com/jhpalmieri"
}
```
The [section in the tutorial on standalone scripts](http://www.sagemath.org/doc/tutorial/programming.html#standalone-python-sage-scripts) needs some fixing. It suggests writing a script starting with

```
#!/usr/bin/env sage -python

import sys
from sage.all import *
...
```
But on sage.math.washington.edu, and probably on other linux systems, `/usr/bin/env` doesn't handle multiple arguments very well. I think that replacing the first line with `#!/usr/bin/env sage` should work.

Assignee: **mvngu**

CC:  @kcrisman

Branch/Commit: **[99c96af](https://github.com/sagemath/sagetrac-mirror/commit/99c96af9aeab5460ec0a75462786cfbb94f69c86)**

Reviewer: **John Palmieri**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/13597_





---

archive/issue_events_174240.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-10-13T02:21:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13597#event-174240"
}
```



---

archive/issue_events_174241.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-10-13T02:21:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13597#event-174241"
}
```



---

archive/issue_events_174242.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-10-13T02:21:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13597#event-174242"
}
```



---

archive/issue_events_174243.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-10-13T02:21:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13597#event-174243"
}
```



---

archive/issue_events_174244.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13597#event-174244"
}
```



---

archive/issue_events_174245.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13597#event-174245"
}
```



---

archive/issue_events_174246.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13597#event-174246"
}
```



---

archive/issue_events_174247.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13597#event-174247"
}
```



---

archive/issue_events_174248.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13597#event-174248"
}
```



---

archive/issue_events_174249.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13597#event-174249"
}
```



---

archive/issue_events_174250.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13597#event-174250"
}
```



---

archive/issue_events_174251.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13597#event-174251"
}
```



---

archive/issue_comments_161581.json:
```json
{
    "body": "Branch: **[u/jdemeyer/tutorial__fix_hash_bang_in_section_on_programming](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/tutorial__fix_hash_bang_in_section_on_programming)**",
    "created_at": "2015-09-10T13:33:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13597#issuecomment-161581",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/tutorial__fix_hash_bang_in_section_on_programming](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/tutorial__fix_hash_bang_in_section_on_programming)**



---

archive/issue_comments_161582.json:
```json
{
    "body": "Commit: **[70f7b1c](https://github.com/sagemath/sagetrac-mirror/commit/70f7b1c5d206e8627f3d124a28b7083e3a82313a)**",
    "created_at": "2015-09-10T13:33:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13597#issuecomment-161582",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[70f7b1c](https://github.com/sagemath/sagetrac-mirror/commit/70f7b1c5d206e8627f3d124a28b7083e3a82313a)**



---

archive/issue_events_174252.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-09-10T13:33:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13597#event-174252"
}
```



---

archive/issue_comments_161583.json:
```json
{
    "body": "Author: **Jeroen Demeyer**",
    "created_at": "2015-09-10T13:33:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13597#issuecomment-161583",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Jeroen Demeyer**



---

archive/issue_comments_161584.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/99c96af9aeab5460ec0a75462786cfbb94f69c86\">99c96af</a></td><td><code>tutorial: fix hash-bang in section on programming</code></td></tr></table>\n",
    "created_at": "2015-09-10T13:34:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13597#issuecomment-161584",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:8'>Comment 8:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/99c96af9aeab5460ec0a75462786cfbb94f69c86">99c96af</a></td><td><code>tutorial: fix hash-bang in section on programming</code></td></tr></table>




---

archive/issue_comments_161585.json:
```json
{
    "body": "Changed commit from **[70f7b1c](https://github.com/sagemath/sagetrac-mirror/commit/70f7b1c5d206e8627f3d124a28b7083e3a82313a)** to **[99c96af](https://github.com/sagemath/sagetrac-mirror/commit/99c96af9aeab5460ec0a75462786cfbb94f69c86)**",
    "created_at": "2015-09-10T13:34:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13597#issuecomment-161585",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[70f7b1c](https://github.com/sagemath/sagetrac-mirror/commit/70f7b1c5d206e8627f3d124a28b7083e3a82313a)** to **[99c96af](https://github.com/sagemath/sagetrac-mirror/commit/99c96af9aeab5460ec0a75462786cfbb94f69c86)**



---

archive/issue_comments_161586.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nThis doesn't fix what is in the description, which is fixed already, I guess.  What I don't understand is why you remove the `-python`, since otherwise there is no need for\n\n```\nfrom sage.all import *\n```\nis there?  Maybe I'm missing something.",
    "created_at": "2015-09-10T13:41:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13597#issuecomment-161586",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:9'>Comment 9:</a>
This doesn't fix what is in the description, which is fixed already, I guess.  What I don't understand is why you remove the `-python`, since otherwise there is no need for

```
from sage.all import *
```
is there?  Maybe I'm missing something.



---

archive/issue_comments_161587.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nI think this is the right thing to do. First, we should certainly not advocate `#!/usr/bin/env` followed by multiple arguments. Second, I just tried the script from the tutorial but with just `#!/usr/bin/env sage` at the top. It worked fine, but it didn't work if I removed `from sage.all import *`:\n\n```\nTraceback (most recent call last):\n  File \"./my_script\", line 10, in <module>\n    print factor(sage_eval(sys.argv[1]))\nNameError: name 'factor' is not defined\n```",
    "created_at": "2015-09-10T16:48:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13597#issuecomment-161587",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:10'>Comment 10:</a>
I think this is the right thing to do. First, we should certainly not advocate `#!/usr/bin/env` followed by multiple arguments. Second, I just tried the script from the tutorial but with just `#!/usr/bin/env sage` at the top. It worked fine, but it didn't work if I removed `from sage.all import *`:

```
Traceback (most recent call last):
  File "./my_script", line 10, in <module>
    print factor(sage_eval(sys.argv[1]))
NameError: name 'factor' is not defined
```



---

archive/issue_comments_161588.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nIn particular, I'm happy to give this a positive review. Karl-Dieter, any objections?",
    "created_at": "2015-09-10T16:52:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13597#issuecomment-161588",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:11'>Comment 11:</a>
In particular, I'm happy to give this a positive review. Karl-Dieter, any objections?



---

archive/issue_comments_161589.json:
```json
{
    "body": "Reviewer: **John Palmieri, Karl-Dieter Crisman**",
    "created_at": "2015-09-10T16:52:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13597#issuecomment-161589",
    "user": "https://github.com/jhpalmieri"
}
```

Reviewer: **John Palmieri, Karl-Dieter Crisman**



---

archive/issue_comments_161590.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nNo objections if it works this way and doesn't otherwise!  I didn't do anything useful here so I'm taking my name off, though.",
    "created_at": "2015-09-10T21:44:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13597#issuecomment-161590",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:12'>Comment 12:</a>
No objections if it works this way and doesn't otherwise!  I didn't do anything useful here so I'm taking my name off, though.



---

archive/issue_comments_161591.json:
```json
{
    "body": "Changed reviewer from **John Palmieri, Karl-Dieter Crisman** to **John Palmieri**",
    "created_at": "2015-09-10T21:44:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13597#issuecomment-161591",
    "user": "https://github.com/kcrisman"
}
```

Changed reviewer from **John Palmieri, Karl-Dieter Crisman** to **John Palmieri**



---

archive/issue_events_174253.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2015-09-10T21:44:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13597#event-174253"
}
```



---

archive/issue_events_174254.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2015-09-10T21:44:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13597#event-174254"
}
```



---

archive/issue_events_174255.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-09-11T16:47:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13597#event-174255"
}
```



---

archive/issue_events_174256.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "fe78a702fc9b8da8e60bcc5a5475f2d6f51d3317",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-09-11T16:47:08Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13597#event-174256"
}
```



---

archive/issue_comments_161592.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/tutorial__fix_hash_bang_in_section_on_programming](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/tutorial__fix_hash_bang_in_section_on_programming)** to **[99c96af](https://github.com/sagemath/sagetrac-mirror/commit/99c96af9aeab5460ec0a75462786cfbb94f69c86)**",
    "created_at": "2015-09-11T16:47:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13597",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13597#issuecomment-161592",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/tutorial__fix_hash_bang_in_section_on_programming](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/tutorial__fix_hash_bang_in_section_on_programming)** to **[99c96af](https://github.com/sagemath/sagetrac-mirror/commit/99c96af9aeab5460ec0a75462786cfbb94f69c86)**
