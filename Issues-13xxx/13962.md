# Issue 13962: Small improvements to Boolean polynomial docs

archive/issues_013758.json:
```json
{
    "assignees": [],
    "body": "At\u00a0#13849\u00a0Michael suggested to encourage Sage users more to look at the\u00a0PolyBoRi\u00a0(lowlevel) docs.\n\nAssignee: **mvngu**\n\nCC:  PolyBoRi @alexanderdreyer\n\nReviewer: **knsam**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13962_\n\n",
    "created_at": "2013-01-17T11:09:47Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/needs%20work"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Small improvements to Boolean polynomial docs",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/13962",
    "user": "https://github.com/malb"
}
```
At #13849 Michael suggested to encourage Sage users more to look at the PolyBoRi (lowlevel) docs.

Assignee: **mvngu**

CC:  PolyBoRi @alexanderdreyer

Reviewer: **knsam**

_Issue created by migration from https://trac.sagemath.org/ticket/13962_





---

archive/issue_events_179819.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2013-01-17T11:09:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13962",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13962#event-179819"
}
```



---

archive/issue_events_179820.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2013-01-17T11:09:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13962",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13962#event-179820"
}
```



---

archive/issue_events_179821.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2013-01-17T11:09:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13962",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13962#event-179821"
}
```



---

archive/issue_events_179822.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2013-01-17T11:09:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13962",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13962#event-179822"
}
```



---

archive/issue_events_179823.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2013-01-17T11:18:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13962",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13962#event-179823"
}
```



---

archive/issue_comments_167658.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nSee\u00a0http://sage.math.washington.edu/home/malb/scratch/sage-5.5/devel/sage/doc/output/html/en/reference/sage/rings/polynomial/pbori.html\u00a0for typeset documentation.",
    "created_at": "2013-01-17T11:18:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13962",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13962#issuecomment-167658",
    "user": "https://github.com/malb"
}
```

<a id='comment:1'>Comment 1:</a>
See http://sage.math.washington.edu/home/malb/scratch/sage-5.5/devel/sage/doc/output/html/en/reference/sage/rings/polynomial/pbori.html for typeset documentation.



---

archive/issue_comments_167659.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nHm,  fine with that. But shouldn't you update your email adress?",
    "created_at": "2013-01-17T14:48:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13962",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13962#issuecomment-167659",
    "user": "https://github.com/alexanderdreyer"
}
```

<a id='comment:2'>Comment 2:</a>
Hm,  fine with that. But shouldn't you update your email adress?



---

archive/issue_comments_167660.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nAttachment: **[trac_13962_polybori_docs.patch.gz](https://github.com/sagemath/sage/files/ticket13962/trac_13962_polybori_docs.patch.gz)**\n\nOkay, I've dropped it completely.",
    "created_at": "2013-01-17T15:26:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13962",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13962#issuecomment-167660",
    "user": "https://github.com/malb"
}
```

<a id='comment:3'>Comment 3:</a>
Attachment: **[trac_13962_polybori_docs.patch.gz](https://github.com/sagemath/sage/files/ticket13962/trac_13962_polybori_docs.patch.gz)**

Okay, I've dropped it completely.



---

archive/issue_comments_167661.json:
```json
{
    "body": "Reviewer: **knsam**",
    "created_at": "2013-01-23T17:22:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13962",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13962#issuecomment-167661",
    "user": "https://github.com/KPanComputes"
}
```

Reviewer: **knsam**



---

archive/issue_comments_167662.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nReplying to [@malb](#comment%3A1):\n> See\u00a0http://sage.math.washington.edu/home/malb/scratch/sage-5.5/devel/sage/doc/output/html/en/reference/sage/rings/polynomial/pbori.html\u00a0for typeset documentation.\n\nThat this is about looking at the Documentation for PolyBoRi, I'd like to suggest the following for a start(based on the typeset documentation you linked to): \n\nGeneral Observation: Please use `\\langle` and `\\rangle` to typeset < and > respectively. We might also want care for the plurality whenever relevant: like EXAMPLES. \n\nSpecifics (I am commenting on the present state, and I would like some docstrings to be more descriptive): \n\n1. In the introduction, the {0,1} is being codified while it should be really typeset.\nExample  \n2. \"that is the ideals `J` satisfying `I \\subset J \\subset P`\" -- `P` is an undefined piece of notation; Also, we really want `\\subseteq`. \nBooleConstant \n3. The function `has_constant_part()` has the word \"for\" repeated twice. And, the BooleConstant(1) bit needs to be codified. \n4. `is_constant` docstring sounds slightly awkard grammatically. \n5. `variables()`: \"and\" is a typo, I guess. \nBooleSet\n6. `diff`: Use `\\setminus` may be? \n7. `intersect`: \"union\" is really intersection. \nBooleanSetIterator \n8. `iterindex()`: indices is misspelt. \nBooleanMonomialIterator\n9. codify StopIteration. \n10. `gen`: codify self \nBooleanMonomialVariableIterator\n11. codify StopIteration\nBooleanPolynomial \n12. `is_one`: codify self. \n23. `is_univariate`: codify self. \n34. `lex_lead()`: The term \"lexicographical term ordering\" sounds unfamiliar to me. Is \"term\" a part of the common usage? \nBooleanPolynomialRing\n15. The class description relies on the INPUT section, perhaps not a good idea. The default value for `order` (lex) needs to be codified. \n26. `clone`: Indentation issue and also codify ring.clone... \n\n37. etc... [What I mean is, I'd get back with more comments, perhaps at a level not as superficial as the ones here. I know these are minor, but for a start...] \n\nThere are some part where we could illustrate the necessary conditions in the Examples. I'll  go through more carefully to point out such cases.",
    "created_at": "2013-01-23T20:58:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13962",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13962#issuecomment-167662",
    "user": "https://github.com/KPanComputes"
}
```

<a id='comment:5'>Comment 5:</a>
Replying to [@malb](#comment%3A1):
> See http://sage.math.washington.edu/home/malb/scratch/sage-5.5/devel/sage/doc/output/html/en/reference/sage/rings/polynomial/pbori.html for typeset documentation.

That this is about looking at the Documentation for PolyBoRi, I'd like to suggest the following for a start(based on the typeset documentation you linked to): 

General Observation: Please use `\langle` and `\rangle` to typeset < and > respectively. We might also want care for the plurality whenever relevant: like EXAMPLES. 

Specifics (I am commenting on the present state, and I would like some docstrings to be more descriptive): 

1. In the introduction, the {0,1} is being codified while it should be really typeset.
Example  
2. "that is the ideals `J` satisfying `I \subset J \subset P`" -- `P` is an undefined piece of notation; Also, we really want `\subseteq`. 
BooleConstant 
3. The function `has_constant_part()` has the word "for" repeated twice. And, the BooleConstant(1) bit needs to be codified. 
4. `is_constant` docstring sounds slightly awkard grammatically. 
5. `variables()`: "and" is a typo, I guess. 
BooleSet
6. `diff`: Use `\setminus` may be? 
7. `intersect`: "union" is really intersection. 
BooleanSetIterator 
8. `iterindex()`: indices is misspelt. 
BooleanMonomialIterator
9. codify StopIteration. 
10. `gen`: codify self 
BooleanMonomialVariableIterator
11. codify StopIteration
BooleanPolynomial 
12. `is_one`: codify self. 
23. `is_univariate`: codify self. 
34. `lex_lead()`: The term "lexicographical term ordering" sounds unfamiliar to me. Is "term" a part of the common usage? 
BooleanPolynomialRing
15. The class description relies on the INPUT section, perhaps not a good idea. The default value for `order` (lex) needs to be codified. 
26. `clone`: Indentation issue and also codify ring.clone... 

37. etc... [What I mean is, I'd get back with more comments, perhaps at a level not as superficial as the ones here. I know these are minor, but for a start...] 

There are some part where we could illustrate the necessary conditions in the Examples. I'll  go through more carefully to point out such cases.



---

archive/issue_events_179824.json:
```json
{
    "actor": "https://github.com/KPanComputes",
    "created_at": "2013-01-23T20:59:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13962",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13962#event-179824"
}
```



---

archive/issue_events_179825.json:
```json
{
    "actor": "https://github.com/KPanComputes",
    "created_at": "2013-01-23T20:59:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13962",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13962#event-179825"
}
```



---

archive/issue_comments_167663.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nI won't be getting around to fixing these in the next few days, so if you provide a patch I can look at it once I have a bit of free time.",
    "created_at": "2013-01-24T00:02:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13962",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13962#issuecomment-167663",
    "user": "https://github.com/malb"
}
```

<a id='comment:7'>Comment 7:</a>
I won't be getting around to fixing these in the next few days, so if you provide a patch I can look at it once I have a bit of free time.



---

archive/issue_events_179826.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13962",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13962#event-179826"
}
```



---

archive/issue_events_179827.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13962",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13962#event-179827"
}
```



---

archive/issue_events_179828.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13962",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13962#event-179828"
}
```



---

archive/issue_events_179829.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13962",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13962#event-179829"
}
```



---

archive/issue_events_179830.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13962",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13962#event-179830"
}
```



---

archive/issue_events_179831.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13962",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13962#event-179831"
}
```



---

archive/issue_events_179832.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13962",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13962#event-179832"
}
```



---

archive/issue_events_179833.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13962",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13962#event-179833"
}
```



---

archive/issue_events_179834.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13962",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13962#event-179834"
}
```
