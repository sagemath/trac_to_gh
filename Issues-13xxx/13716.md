# Issue 13716: (very) Minor improvements to the multivariate polynomials documentation

archive/issues_013512.json:
```json
{
    "body": "Mostly typo fixing, adding references to trac tickets, adding quotes, fixing a few spelling mistakes, ...\n\nAssignee: @malb\n\nAuthor: Charles Bouillaguet\n\nIssue created by migration from https://trac.sagemath.org/ticket/13716\n\n",
    "created_at": "2012-11-15T23:30:39Z",
    "labels": [
        "component: commutative algebra",
        "trivial"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.4",
    "title": "(very) Minor improvements to the multivariate polynomials documentation",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/13716",
    "user": "https://trac.sagemath.org/admin/accounts/users/Bouillaguet"
}
```
Mostly typo fixing, adding references to trac tickets, adding quotes, fixing a few spelling mistakes, ...

Assignee: @malb

Author: Charles Bouillaguet

Issue created by migration from https://trac.sagemath.org/ticket/13716





---

archive/issue_comments_166523.json:
```json
{
    "body": "Attachment [13716_improve_multipolynomial_doc.patch](tarball://root/attachments/some-uuid/ticket13716/13716_improve_multipolynomial_doc.patch) by Bouillaguet created at 2012-11-15 23:31:58",
    "created_at": "2012-11-15T23:31:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13716",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13716#issuecomment-166523",
    "user": "https://trac.sagemath.org/admin/accounts/users/Bouillaguet"
}
```

Attachment [13716_improve_multipolynomial_doc.patch](tarball://root/attachments/some-uuid/ticket13716/13716_improve_multipolynomial_doc.patch) by Bouillaguet created at 2012-11-15 23:31:58



---

archive/issue_comments_166524.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2012-11-15T23:34:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13716",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13716#issuecomment-166524",
    "user": "https://trac.sagemath.org/admin/accounts/users/Bouillaguet"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_166525.json:
```json
{
    "body": "<a id='comment:2'></a>Some comments:\n1. Line 1308 is missing a backtick on closing.\n\n```\nReturns the leading term of ``self`` i.e., ``self.lc()*self.lm()`. The\n```\n2. Lines 1577 and 1578 should have the latex variables enclosed in single backticks not double (like ``I = (f_1,...,f_r)``:\n\n```\n       given an ideal ``I = (f_1,...,f_r)`` and some ``g (== self)`` in ``I``, find \n        ``s_1,...,s_r`` such that ``g = s_1*f_1 + ... + s_r*f_r``. \n```\n3. Shouldn't line 1975 also have `\\ne` in the latter expression?\n\n```\n        - `LT(g_i) \\neq LT(g_j)` for all `i != j` \n```\n4. Should we refer to the `derivative()` function in line 342 instead as `:meth:`sage.calculus.functional.derivative`` ? This will create a hyperlink to the derivative documentation in the notebook help.\n5. I am not very happy to see lots of \"self\" throughout the docs, but converting all of them would be a lot of work. So, I guess we can just let it pass and hope it gets fixed little by little over time.\n6. Most importantly, this patch touches a huge number of lines in several files. I am ok with this change (in fact, ok with any change which improves docs), but ultimately its up to Jeroen whether this gets merged, since he doesn't usually like massive minor changes. :)",
    "created_at": "2012-11-16T09:04:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13716",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13716#issuecomment-166525",
    "user": "https://github.com/ppurka"
}
```

<a id='comment:2'></a>Some comments:
1. Line 1308 is missing a backtick on closing.

```
Returns the leading term of ``self`` i.e., ``self.lc()*self.lm()`. The
```
2. Lines 1577 and 1578 should have the latex variables enclosed in single backticks not double (like ``I = (f_1,...,f_r)``:

```
       given an ideal ``I = (f_1,...,f_r)`` and some ``g (== self)`` in ``I``, find 
        ``s_1,...,s_r`` such that ``g = s_1*f_1 + ... + s_r*f_r``. 
```
3. Shouldn't line 1975 also have `\ne` in the latter expression?

```
        - `LT(g_i) \neq LT(g_j)` for all `i != j` 
```
4. Should we refer to the `derivative()` function in line 342 instead as `:meth:`sage.calculus.functional.derivative`` ? This will create a hyperlink to the derivative documentation in the notebook help.
5. I am not very happy to see lots of "self" throughout the docs, but converting all of them would be a lot of work. So, I guess we can just let it pass and hope it gets fixed little by little over time.
6. Most importantly, this patch touches a huge number of lines in several files. I am ok with this change (in fact, ok with any change which improves docs), but ultimately its up to Jeroen whether this gets merged, since he doesn't usually like massive minor changes. :)



---

archive/issue_comments_166526.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2012-12-19T14:04:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13716",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13716#issuecomment-166526",
    "user": "https://github.com/malb"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_events_038406.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13716",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13716#event-38406"
}
```



---

archive/issue_events_038407.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13716",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13716#event-38407"
}
```



---

archive/issue_events_038408.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13716",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13716#event-38408"
}
```



---

archive/issue_events_038409.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13716",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13716#event-38409"
}
```



---

archive/issue_events_038410.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13716",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13716#event-38410"
}
```



---

archive/issue_events_038411.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13716",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13716#event-38411"
}
```



---

archive/issue_events_038412.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13716",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13716#event-38412"
}
```



---

archive/issue_comments_166527.json:
```json
{
    "body": "Changing keywords from \"beginner\" to \"\".",
    "created_at": "2020-05-27T03:01:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13716",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13716#issuecomment-166527",
    "user": "https://github.com/DaveWitteMorris"
}
```

Changing keywords from "beginner" to "".



---

archive/issue_comments_166528.json:
```json
{
    "body": "<a id='comment:8'></a>Removing the \"beginner\" tag from old tickets. Some could be returned to beginner-friendly status by adding a comment about what needs to be done. Some others might be easy for an experienced developer to finish.",
    "created_at": "2020-05-27T03:01:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13716",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13716#issuecomment-166528",
    "user": "https://github.com/DaveWitteMorris"
}
```

<a id='comment:8'></a>Removing the "beginner" tag from old tickets. Some could be returned to beginner-friendly status by adding a comment about what needs to be done. Some others might be easy for an experienced developer to finish.
