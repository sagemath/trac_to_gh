# Issue 13601: Module to load matrix from file

archive/issues_013397.json:
```json
{
    "body": "Numpy and Scipy has functions to load matrix save in text files (Numpy: http://docs.scipy.org/doc/numpy/reference/routines.io.html and Scipy: http://docs.scipy.org/doc/scipy/reference/io.html).\n\nWill be nice to have a Sage function to call some of this function as backend and to handle with more file formats.\n\nIn attachment trac_13601-matrix_io.patch was suggest the follow formats:\n* A single matrix in a text file without header or any other information.\n* Coordinate Text File.\n* Octave's binary data format.\n* Octave's binary data format.\n* hdf5 format.\n* The Matrix Market (MM) exchange formats.\n* The Harwell-Boeing format.\n* Matlab format.\n* IDL save file.\n* Mathematical Programming System (mps).\n\nLoad Matlab and IDL format was implemented as a backend of Scipy and MPS was implemented from scratch.\n\nLoad others suggest formats wasn't implemented. Neither the functions to write.\n\n**To improve this module to handle with any of the not implemented formats or any new format, please open other ticket.**\n\nWill be nice to implemented the suggest of [ticket:13601#comment:4 vbraun].\n\n\n\n\n\nAssignee: jason, was\n\nCC:  bouillaguet\n\nStatus: needs_work\n\nIssue created by migration from https://trac.sagemath.org/ticket/13601\n\n",
    "created_at": "2012-10-15T13:37:50Z",
    "labels": [
        "component: linear algebra"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.4",
    "title": "Module to load matrix from file",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/13601",
    "user": "https://trac.sagemath.org/admin/accounts/users/r.gaia.cs"
}
```
Numpy and Scipy has functions to load matrix save in text files (Numpy: http://docs.scipy.org/doc/numpy/reference/routines.io.html and Scipy: http://docs.scipy.org/doc/scipy/reference/io.html).

Will be nice to have a Sage function to call some of this function as backend and to handle with more file formats.

In attachment trac_13601-matrix_io.patch was suggest the follow formats:
* A single matrix in a text file without header or any other information.
* Coordinate Text File.
* Octave's binary data format.
* Octave's binary data format.
* hdf5 format.
* The Matrix Market (MM) exchange formats.
* The Harwell-Boeing format.
* Matlab format.
* IDL save file.
* Mathematical Programming System (mps).

Load Matlab and IDL format was implemented as a backend of Scipy and MPS was implemented from scratch.

Load others suggest formats wasn't implemented. Neither the functions to write.

**To improve this module to handle with any of the not implemented formats or any new format, please open other ticket.**

Will be nice to implemented the suggest of [ticket:13601#comment:4 vbraun].





Assignee: jason, was

CC:  bouillaguet

Status: needs_work

Issue created by migration from https://trac.sagemath.org/ticket/13601





---

archive/issue_comments_205252.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2012-10-15T16:38:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13601#issuecomment-205252",
    "user": "https://trac.sagemath.org/admin/accounts/users/r.gaia.cs"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_205253.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,23 @@\n Numpy and Scipy has functions to load matrix save in text files (Numpy: http://docs.scipy.org/doc/numpy/reference/routines.io.html and Scipy: http://docs.scipy.org/doc/scipy/reference/io.html).\n \n Will be nice to have a Sage function to call some of this function as backend and to handle with more file formats.\n+\n+In attachment trac_13601-matrix_io.patch was suggest the follow formats:\n+* A single matrix in a text file without header or any other information.\n+* Coordinate Text File.\n+* Octave's binary data format.\n+* Octave's binary data format.\n+* hdf5 format.\n+* The Matrix Market (MM) exchange formats.\n+* The Harwell-Boeing format.\n+* Matlab format.\n+* IDL save file.\n+* Mathematical Programming System (mps).\n+\n+Load Matlab and IDL format was implemented as a backend of Scipy and MPS was implemented from scratch.\n+\n+Load others suggest formats wasn't implemented. Neither the functions to write.\n+\n+**To improve this module, please open other ticket.**\n+\n+\n``````\n",
    "created_at": "2012-10-15T22:07:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13601#issuecomment-205253",
    "user": "https://trac.sagemath.org/admin/accounts/users/r.gaia.cs"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,23 @@
 Numpy and Scipy has functions to load matrix save in text files (Numpy: http://docs.scipy.org/doc/numpy/reference/routines.io.html and Scipy: http://docs.scipy.org/doc/scipy/reference/io.html).
 
 Will be nice to have a Sage function to call some of this function as backend and to handle with more file formats.
+
+In attachment trac_13601-matrix_io.patch was suggest the follow formats:
+* A single matrix in a text file without header or any other information.
+* Coordinate Text File.
+* Octave's binary data format.
+* Octave's binary data format.
+* hdf5 format.
+* The Matrix Market (MM) exchange formats.
+* The Harwell-Boeing format.
+* Matlab format.
+* IDL save file.
+* Mathematical Programming System (mps).
+
+Load Matlab and IDL format was implemented as a backend of Scipy and MPS was implemented from scratch.
+
+Load others suggest formats wasn't implemented. Neither the functions to write.
+
+**To improve this module, please open other ticket.**
+
+
``````




---

archive/issue_comments_205254.json:
```json
{
    "body": "<a id='comment:3'></a>In the version 0.2 of the attachment:trac_13601-matrix_io.patch I make some improves in the definition of the functions and the documentation and write tests for all the functions.\n\nIMHO, will be better to freeze this ticket and open others when write the not implemented functions.",
    "created_at": "2012-10-30T23:10:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13601#issuecomment-205254",
    "user": "https://trac.sagemath.org/admin/accounts/users/r.gaia.cs"
}
```

<a id='comment:3'></a>In the version 0.2 of the attachment:trac_13601-matrix_io.patch I make some improves in the definition of the functions and the documentation and write tests for all the functions.

IMHO, will be better to freeze this ticket and open others when write the not implemented functions.



---

archive/issue_comments_205255.json:
```json
{
    "body": "<a id='comment:4'></a>Instead of adding new global symbols `load_matrix` / `write_matrix`, how about making them accessible through the matrix constructor? That is, if the first argument to `matrix()` is a string then call `load_matrix(filename, **kwds)`. The `write_matrix` functionality should be wrapped into the `save()` method of matrices, this can then be discovered by the user with tab-completion.\n\nA minor English nitpick, its either load/save or read/write.",
    "created_at": "2012-10-31T11:15:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13601#issuecomment-205255",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:4'></a>Instead of adding new global symbols `load_matrix` / `write_matrix`, how about making them accessible through the matrix constructor? That is, if the first argument to `matrix()` is a string then call `load_matrix(filename, **kwds)`. The `write_matrix` functionality should be wrapped into the `save()` method of matrices, this can then be discovered by the user with tab-completion.

A minor English nitpick, its either load/save or read/write.



---

archive/issue_comments_205256.json:
```json
{
    "body": "<a id='comment:5'></a>Replying to [vbraun](#comment%3A4):\n> Instead of adding new global symbols `load_matrix` / `write_matrix`, how about making them accessible through the matrix constructor? That is, if the first argument to `matrix()` is a string then call `load_matrix(filename, **kwds)`. The `write_matrix` functionality should be wrapped into the `save()` method of matrices, this can then be discovered by the user with tab-completion.\n\n\nI like the suggestion.",
    "created_at": "2012-10-31T11:27:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13601#issuecomment-205256",
    "user": "https://trac.sagemath.org/admin/accounts/users/r.gaia.cs"
}
```

<a id='comment:5'></a>Replying to [vbraun](#comment%3A4):
> Instead of adding new global symbols `load_matrix` / `write_matrix`, how about making them accessible through the matrix constructor? That is, if the first argument to `matrix()` is a string then call `load_matrix(filename, **kwds)`. The `write_matrix` functionality should be wrapped into the `save()` method of matrices, this can then be discovered by the user with tab-completion.


I like the suggestion.



---

archive/issue_comments_205257.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -18,6 +18,10 @@\n \n Load others suggest formats wasn't implemented. Neither the functions to write.\n \n-**To improve this module, please open other ticket.**\n+**To improve this module to handle with any of the not implemented formats or any new format, please open other ticket.**\n+\n+Will be nice to implemented the suggest of [ticket:13601#comment:4 vbraun].\n \n \n+\n+\n``````\n",
    "created_at": "2012-10-31T11:35:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13601#issuecomment-205257",
    "user": "https://trac.sagemath.org/admin/accounts/users/r.gaia.cs"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -18,6 +18,10 @@
 
 Load others suggest formats wasn't implemented. Neither the functions to write.
 
-**To improve this module, please open other ticket.**
+**To improve this module to handle with any of the not implemented formats or any new format, please open other ticket.**
+
+Will be nice to implemented the suggest of [ticket:13601#comment:4 vbraun].
 
 
+
+
``````




---

archive/issue_comments_205258.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2012-10-31T11:35:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13601#issuecomment-205258",
    "user": "https://trac.sagemath.org/admin/accounts/users/r.gaia.cs"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_205259.json:
```json
{
    "body": "Attachment [trac_13601-matrix_io.patch](tarball://root/attachments/some-uuid/ticket13601/trac_13601-matrix_io.patch) by r.gaia.cs created at 2012-10-31 11:41:16\n\nVersion 0.2",
    "created_at": "2012-10-31T11:41:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13601#issuecomment-205259",
    "user": "https://trac.sagemath.org/admin/accounts/users/r.gaia.cs"
}
```

Attachment [trac_13601-matrix_io.patch](tarball://root/attachments/some-uuid/ticket13601/trac_13601-matrix_io.patch) by r.gaia.cs created at 2012-10-31 11:41:16

Version 0.2



---

archive/issue_comments_205260.json:
```json
{
    "body": "<a id='comment:7'></a>I would rather see matrix(str, ...) try to parse the string, so one could do\n\n```\nmatrix(\"\"\"\n[1, 2]\n[3, 4]\n\"\"\")\n```\n\nAnd use matrix(open(\"/path/to/file\", ...) for loading from a file.",
    "created_at": "2012-10-31T17:25:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13601#issuecomment-205260",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:7'></a>I would rather see matrix(str, ...) try to parse the string, so one could do

```
matrix("""
[1, 2]
[3, 4]
""")
```

And use matrix(open("/path/to/file", ...) for loading from a file.



---

archive/issue_comments_205261.json:
```json
{
    "body": "<a id='comment:8'></a>Another possibility would be to turn `matrix` into a class with `matrix.__call__` doing what we are doing now. Then one can add methods `matrix.load`, `matrix.identity`, ...",
    "created_at": "2012-10-31T17:47:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13601#issuecomment-205261",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:8'></a>Another possibility would be to turn `matrix` into a class with `matrix.__call__` doing what we are doing now. Then one can add methods `matrix.load`, `matrix.identity`, ...



---

archive/issue_comments_205262.json:
```json
{
    "body": "<a id='comment:9'></a>Replying to [robertwb](#comment%3A7):\n\nI like the idea of see `matrix(str, ...)` try to parse the string. But I prefer to use `matrix(\"/path/to/file\", ...)` rather than `matrix(open(\"/path/to/file\", ...), ...)`.",
    "created_at": "2012-10-31T18:28:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13601#issuecomment-205262",
    "user": "https://trac.sagemath.org/admin/accounts/users/r.gaia.cs"
}
```

<a id='comment:9'></a>Replying to [robertwb](#comment%3A7):

I like the idea of see `matrix(str, ...)` try to parse the string. But I prefer to use `matrix("/path/to/file", ...)` rather than `matrix(open("/path/to/file", ...), ...)`.



---

archive/issue_comments_205263.json:
```json
{
    "body": "<a id='comment:10'></a>Replying to [r.gaia.cs](#comment%3A9):\n> Replying to [robertwb](#comment%3A7):\n> \n> I like the idea of see `matrix(str, ...)` try to parse the string. But I prefer to use `matrix(\"/path/to/file\", ...)` rather than `matrix(open(\"/path/to/file\", ...), ...)`.\n\n\nBut how would you disambiguate the two?  It does seem odd to not be able to construct matrices from strings, as one can do `ZZ(\"123\")` and `SR(\"b^2 - 4a c\")` and `ZZ['x'](\"x^3 - 3x + 1\")`.\n\nA huge +1 to Volker's idea. #13678",
    "created_at": "2012-10-31T21:50:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13601#issuecomment-205263",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:10'></a>Replying to [r.gaia.cs](#comment%3A9):
> Replying to [robertwb](#comment%3A7):
> 
> I like the idea of see `matrix(str, ...)` try to parse the string. But I prefer to use `matrix("/path/to/file", ...)` rather than `matrix(open("/path/to/file", ...), ...)`.


But how would you disambiguate the two?  It does seem odd to not be able to construct matrices from strings, as one can do `ZZ("123")` and `SR("b^2 - 4a c")` and `ZZ['x']("x^3 - 3x + 1")`.

A huge +1 to Volker's idea. #13678



---

archive/issue_comments_205264.json:
```json
{
    "body": "<a id='comment:11'></a>Replying to [robertwb](#comment%3A10):\n> Replying to [r.gaia.cs](#comment%3A9):\n> > Replying to [robertwb](#comment%3A7):\n> > \n> > I like the idea of see `matrix(str, ...)` try to parse the string. But I prefer to use `matrix(\"/path/to/file\", ...)` rather than `matrix(open(\"/path/to/file\", ...), ...)`.\n\n> \n> But how would you disambiguate the two?\n\n\nIMHO, for the string to be parse it must be equal to the nice string representation of:\n\n* a flat list of elements: `'[1, 2, 3, 4]'`;\n* a list of lists (i.e., a list of rows): `[[1, 2], [3, 4]]`;\n* a list of Sage vectors: `'[(1, 2), (3, 4)]'`\n* a dictionary having positions as keys and matrix entries as values: `'{(0, 1): 2, (1, 0): 3, (0, 0): 1, (1, 1): 4}'`\n\nOnce in all the four cases above the 0-position of the string is `[` or `{` it can be used to disambiguate between the two.\n\nWill be good too that one could do\n\n```\nmatrix(\"\"\"\n1, 2\n3, 4\n\"\"\")\n```\n\nFor this special case we can use the new line character to disambiguate.\n\n> A huge +1 to Volker's idea. #13678\n\nA huge +2 to Volker's idea. #13678",
    "created_at": "2012-10-31T23:40:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13601#issuecomment-205264",
    "user": "https://trac.sagemath.org/admin/accounts/users/r.gaia.cs"
}
```

<a id='comment:11'></a>Replying to [robertwb](#comment%3A10):
> Replying to [r.gaia.cs](#comment%3A9):
> > Replying to [robertwb](#comment%3A7):
> > 
> > I like the idea of see `matrix(str, ...)` try to parse the string. But I prefer to use `matrix("/path/to/file", ...)` rather than `matrix(open("/path/to/file", ...), ...)`.

> 
> But how would you disambiguate the two?


IMHO, for the string to be parse it must be equal to the nice string representation of:

* a flat list of elements: `'[1, 2, 3, 4]'`;
* a list of lists (i.e., a list of rows): `[[1, 2], [3, 4]]`;
* a list of Sage vectors: `'[(1, 2), (3, 4)]'`
* a dictionary having positions as keys and matrix entries as values: `'{(0, 1): 2, (1, 0): 3, (0, 0): 1, (1, 1): 4}'`

Once in all the four cases above the 0-position of the string is `[` or `{` it can be used to disambiguate between the two.

Will be good too that one could do

```
matrix("""
1, 2
3, 4
""")
```

For this special case we can use the new line character to disambiguate.

> A huge +1 to Volker's idea. #13678

A huge +2 to Volker's idea. #13678



---

archive/issue_comments_205265.json:
```json
{
    "body": "<a id='comment:12'></a>I think it would be odd if `ZZ(s)` tried to open the file `s` if it didin't represent a valid integer (and/or refused to if it did). Same issue applies to matrices. IMHO, better to raise an error than have very different semantics here.\n\nYou could rebase this atop the #13678 (using matrix.load) to sidestep the whole issue.",
    "created_at": "2012-11-01T06:09:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13601#issuecomment-205265",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:12'></a>I think it would be odd if `ZZ(s)` tried to open the file `s` if it didin't represent a valid integer (and/or refused to if it did). Same issue applies to matrices. IMHO, better to raise an error than have very different semantics here.

You could rebase this atop the #13678 (using matrix.load) to sidestep the whole issue.



---

archive/issue_comments_205266.json:
```json
{
    "body": "<a id='comment:13'></a>To parse a string-matrix I think we should just match numbers and strip everything else out, as in\n\n```\nsage: number = re.compile('[-+]?[0-9]*\\.?[0-9]+([eE][-+]?[0-9]+)?')\nsage: [ match.group() for match in number.finditer('[ 1e3, 0 | 3 6, 1.3 .123)') ]\n['1e3', '0', '3', '6', '1.3', '.123']\n```\nThen put the numbers from each line into the rows of a matrix. This should grok all ascii-art matrix representations.\n\nIf the input string is already valid Python, say, then the user can just eval it directly. So we shouldn't worry too much about that case.\n\n+1 to use `matrix.load()` to load the matrix.",
    "created_at": "2012-11-01T09:40:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13601#issuecomment-205266",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:13'></a>To parse a string-matrix I think we should just match numbers and strip everything else out, as in

```
sage: number = re.compile('[-+]?[0-9]*\.?[0-9]+([eE][-+]?[0-9]+)?')
sage: [ match.group() for match in number.finditer('[ 1e3, 0 | 3 6, 1.3 .123)') ]
['1e3', '0', '3', '6', '1.3', '.123']
```
Then put the numbers from each line into the rows of a matrix. This should grok all ascii-art matrix representations.

If the input string is already valid Python, say, then the user can just eval it directly. So we shouldn't worry too much about that case.

+1 to use `matrix.load()` to load the matrix.



---

archive/issue_comments_205267.json:
```json
{
    "body": "<a id='comment:14'></a>+1 to use matrix.load() to load the matrix !\n\nThis ticket should really be rebased on top of #13678 !\n\nMinor problems I spotted : there are a lot of english mistakes in the patch, copyright notice should not say 2005 W.stein, but 2012 R.Silva.\n\nLastly, I find it weird to write a whole lot of boilerplate, especially in the (user-visible) documentation, about things that are not implemented. I think it could confuse users, and I personally think that it would be better if we took the pain of actually doing the work before merging the ticket. Or at least it could be rephrased \"TODO : other formats that we could possibly handle in the future include....\"\n\nAbout the MatrixMarket format : as Raniere mentions, SciPy has functions to read/write this format. These functions could probably be used as-is for floating-point matrices, but as far as I understand what they do, they do not support integer matrices. And even if they would, they would be limited to 32/64 bits integers because they rely on NumPy. Therefore I tend to believe that an integer version of the MatrixMarket code would have to be re-implemented. We cannot patch SciPy to work with arbitrarily large integers... So I think that we should take SciPy's MatrixMarket code and improve it. I am willing to do this, but I'd rather be sure that we agree before doing it.\n\nOh, a last detail : the MatrixMarket format comes in a dense and in a sparse version, real, complex, integer or binary, thus it would seem natural that the loader itself sets the sparsity flag and the ring. In the patch they are given to the loader.",
    "created_at": "2012-12-06T06:51:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13601",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13601#issuecomment-205267",
    "user": "https://trac.sagemath.org/admin/accounts/users/Bouillaguet"
}
```

<a id='comment:14'></a>+1 to use matrix.load() to load the matrix !

This ticket should really be rebased on top of #13678 !

Minor problems I spotted : there are a lot of english mistakes in the patch, copyright notice should not say 2005 W.stein, but 2012 R.Silva.

Lastly, I find it weird to write a whole lot of boilerplate, especially in the (user-visible) documentation, about things that are not implemented. I think it could confuse users, and I personally think that it would be better if we took the pain of actually doing the work before merging the ticket. Or at least it could be rephrased "TODO : other formats that we could possibly handle in the future include...."

About the MatrixMarket format : as Raniere mentions, SciPy has functions to read/write this format. These functions could probably be used as-is for floating-point matrices, but as far as I understand what they do, they do not support integer matrices. And even if they would, they would be limited to 32/64 bits integers because they rely on NumPy. Therefore I tend to believe that an integer version of the MatrixMarket code would have to be re-implemented. We cannot patch SciPy to work with arbitrarily large integers... So I think that we should take SciPy's MatrixMarket code and improve it. I am willing to do this, but I'd rather be sure that we agree before doing it.

Oh, a last detail : the MatrixMarket format comes in a dense and in a sparse version, real, complex, integer or binary, thus it would seem natural that the loader itself sets the sparsity flag and the ring. In the patch they are given to the loader.



---

archive/issue_events_037908.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13601",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13601#event-37908"
}
```



---

archive/issue_events_037909.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13601",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13601#event-37909"
}
```



---

archive/issue_events_037910.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13601",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13601#event-37910"
}
```



---

archive/issue_events_037911.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13601",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13601#event-37911"
}
```



---

archive/issue_events_037912.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13601",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13601#event-37912"
}
```



---

archive/issue_events_037913.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13601",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13601#event-37913"
}
```



---

archive/issue_events_037914.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13601",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13601#event-37914"
}
```
