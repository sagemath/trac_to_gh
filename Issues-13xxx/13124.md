# Issue 13124: Updating PolyBoRi to release 0.8.2

archive/issues_012952.json:
```json
{
    "assignees": [],
    "body": "PolyBoRi's next minor release will be out soon.\nThere were no changes of the interface between PolyBoRi 0.8.2 and 0.8.1, so we just have to update the sources from here:\n\nhttps://sourceforge.net/projects/polybori/files/polybori/0.8.2rc\n\nPolyBoRi 0.8.2 is a bugfix release incorporating all previous patches from Sage.\n\n### Current spkg\n* **Install** http://boxen.math.washington.edu/home/dreyer/spkg/polybori-0.8.2.spkg\n\n\n\nAssignee: **@alexanderdreyer**\n\nCC:  PolyBoRi @malb @burcin\n\nUpstream: **None of the above - read trac for reasoning.**\n\nReviewer: **Martin Albrecht**\n\nAuthor: **Alexander Dreyer**\n\nMerged: **sage-5.2.beta1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13124_\n\n",
    "closed_at": "2012-07-07T22:31:05Z",
    "created_at": "2012-06-16T20:50:58Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Updating PolyBoRi to release 0.8.2",
    "type": "issue",
    "updated_at": "2012-07-09T13:06:36Z",
    "url": "https://github.com/sagemath/sage/issues/13124",
    "user": "https://github.com/alexanderdreyer"
}
```
PolyBoRi's next minor release will be out soon.
There were no changes of the interface between PolyBoRi 0.8.2 and 0.8.1, so we just have to update the sources from here:

https://sourceforge.net/projects/polybori/files/polybori/0.8.2rc

PolyBoRi 0.8.2 is a bugfix release incorporating all previous patches from Sage.

### Current spkg
* **Install** http://boxen.math.washington.edu/home/dreyer/spkg/polybori-0.8.2.spkg



Assignee: **@alexanderdreyer**

CC:  PolyBoRi @malb @burcin

Upstream: **None of the above - read trac for reasoning.**

Reviewer: **Martin Albrecht**

Author: **Alexander Dreyer**

Merged: **sage-5.2.beta1**

_Issue created by migration from https://trac.sagemath.org/ticket/13124_





---

archive/issue_events_161100.json:
```json
{
    "actor": "https://github.com/alexanderdreyer",
    "created_at": "2012-06-16T20:50:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "milestone_number": null,
    "milestone_title": "sage-5.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13124#event-161100"
}
```



---

archive/issue_events_161101.json:
```json
{
    "actor": "https://github.com/alexanderdreyer",
    "created_at": "2012-06-16T20:50:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13124#event-161101"
}
```



---

archive/issue_events_161102.json:
```json
{
    "actor": "https://github.com/alexanderdreyer",
    "created_at": "2012-06-16T20:50:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13124#event-161102"
}
```



---

archive/issue_events_161103.json:
```json
{
    "actor": "https://github.com/alexanderdreyer",
    "created_at": "2012-06-16T20:50:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13124#event-161103"
}
```



---

archive/issue_comments_152355.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nThe spkg is in place now.",
    "created_at": "2012-06-16T20:57:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152355",
    "user": "https://github.com/alexanderdreyer"
}
```

<a id='comment:1'>Comment 1:</a>
The spkg is in place now.



---

archive/issue_comments_152356.json:
```json
{
    "body": "Author: **AlexanderDreyer**",
    "created_at": "2012-06-16T20:57:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152356",
    "user": "https://github.com/alexanderdreyer"
}
```

Author: **AlexanderDreyer**



---

archive/issue_events_161104.json:
```json
{
    "actor": "https://github.com/alexanderdreyer",
    "created_at": "2012-06-16T20:57:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13124#event-161104"
}
```



---

archive/issue_comments_152357.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\n* installs fine\n  * doctests pass\n  * package look okay\n\nOnly issue: I think it's our policy not to have .p0 for fresh packages, but just the name. Can you drop the .p0?",
    "created_at": "2012-06-17T13:45:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152357",
    "user": "https://github.com/malb"
}
```

<a id='comment:2'>Comment 2:</a>
* installs fine
  * doctests pass
  * package look okay

Only issue: I think it's our policy not to have .p0 for fresh packages, but just the name. Can you drop the .p0?



---

archive/issue_comments_152358.json:
```json
{
    "body": "Reviewer: **Martin Albrecht**",
    "created_at": "2012-06-17T13:45:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152358",
    "user": "https://github.com/malb"
}
```

Reviewer: **Martin Albrecht**



---

archive/issue_comments_152359.json:
```json
{
    "body": "Changed author from **AlexanderDreyer** to **Alexander Dreyer**",
    "created_at": "2012-06-17T13:45:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152359",
    "user": "https://github.com/malb"
}
```

Changed author from **AlexanderDreyer** to **Alexander Dreyer**



---

archive/issue_comments_152360.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nReplying to [@malb](#comment%3A2):\n> Only issue: I think it's our policy not to have .p0 for fresh packages, but just the name. Can you drop the .p0?\n\nNo problem, the updated spkg (renamed and changed tag line in `SPKG.txt`) is in place, see above.",
    "created_at": "2012-06-17T19:50:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152360",
    "user": "https://github.com/alexanderdreyer"
}
```

<a id='comment:3'>Comment 3:</a>
Replying to [@malb](#comment%3A2):
> Only issue: I think it's our policy not to have .p0 for fresh packages, but just the name. Can you drop the .p0?

No problem, the updated spkg (renamed and changed tag line in `SPKG.txt`) is in place, see above.



---

archive/issue_comments_152361.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -6,6 +6,6 @@\n PolyBoRi 0.8.2 is a bugfix release incorporating all previous patches from Sage.\n \n ### Current spkg\n-* **Install** http://boxen.math.washington.edu/home/dreyer/spkg/polybori-0.8.2.p0.spkg\n+* **Install** http://boxen.math.washington.edu/home/dreyer/spkg/polybori-0.8.2.spkg\n \n \n``````\n",
    "created_at": "2012-06-17T19:52:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152361",
    "user": "https://github.com/alexanderdreyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -6,6 +6,6 @@
 PolyBoRi 0.8.2 is a bugfix release incorporating all previous patches from Sage.
 
 ### Current spkg
-* **Install** http://boxen.math.washington.edu/home/dreyer/spkg/polybori-0.8.2.p0.spkg
+* **Install** http://boxen.math.washington.edu/home/dreyer/spkg/polybori-0.8.2.spkg
 
 
``````




---

archive/issue_events_161105.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2012-06-18T08:35:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13124#event-161105"
}
```



---

archive/issue_events_161106.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2012-06-18T08:35:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13124#event-161106"
}
```



---

archive/issue_comments_152362.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nAll done then. :)",
    "created_at": "2012-06-18T08:35:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152362",
    "user": "https://github.com/malb"
}
```

<a id='comment:5'>Comment 5:</a>
All done then. :)



---

archive/issue_events_161107.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-18T13:33:16Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "milestone_number": null,
    "milestone_title": "sage-5.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13124#event-161107"
}
```



---

archive/issue_events_161108.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-18T13:33:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "milestone_number": null,
    "milestone_title": "sage-5.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13124#event-161108"
}
```



---

archive/issue_comments_152363.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nI just updated the spkg to include the following patch in the upstream sources:\nhttps://bitbucket.org/brickenstein/polybori/changeset/b5c2173f7a43\nThe issue was discovered in #13133. I don't add `.p0`, since the patch will be in PolyBoRi 0.8.2's next rc.",
    "created_at": "2012-06-19T08:44:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152363",
    "user": "https://github.com/alexanderdreyer"
}
```

<a id='comment:7'>Comment 7:</a>
I just updated the spkg to include the following patch in the upstream sources:
https://bitbucket.org/brickenstein/polybori/changeset/b5c2173f7a43
The issue was discovered in #13133. I don't add `.p0`, since the patch will be in PolyBoRi 0.8.2's next rc.



---

archive/issue_comments_152364.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\n@malb Staying positive reviewed?",
    "created_at": "2012-06-19T08:46:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152364",
    "user": "https://github.com/alexanderdreyer"
}
```

<a id='comment:8'>Comment 8:</a>
@malb Staying positive reviewed?



---

archive/issue_comments_152365.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nPlease don't name the spkg version \"0.8.2\" if polybori-0.8.2 hasn't actually been released!  If the upstream version is called \"0.8.2rc1\", then so should the Sage version be (including in `SPKG.txt`).",
    "created_at": "2012-06-19T08:49:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152365",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:9'>Comment 9:</a>
Please don't name the spkg version "0.8.2" if polybori-0.8.2 hasn't actually been released!  If the upstream version is called "0.8.2rc1", then so should the Sage version be (including in `SPKG.txt`).



---

archive/issue_events_161109.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-19T08:49:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13124#event-161109"
}
```



---

archive/issue_events_161110.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-19T08:49:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13124#event-161110"
}
```



---

archive/issue_comments_152366.json:
```json
{
    "body": "Work Issues: **version number**",
    "created_at": "2012-06-19T08:49:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152366",
    "user": "https://github.com/jdemeyer"
}
```

Work Issues: **version number**



---

archive/issue_comments_152367.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nWe are currently co-developing. PolyBoRi 0.8.2 will be whatever the result of this ticket is.",
    "created_at": "2012-06-19T08:53:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152367",
    "user": "https://github.com/alexanderdreyer"
}
```

<a id='comment:11'>Comment 11:</a>
We are currently co-developing. PolyBoRi 0.8.2 will be whatever the result of this ticket is.



---

archive/issue_comments_152368.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nThat makes sense, you you should really make sure that the upstream polybori-0.8.2 is **identical** to the Sage polybori-0.8.2.\n\nBelieve me, I have seen this kind of problems in the past with other packages where the upstream version was close to, but not quite equal to the Sage version.",
    "created_at": "2012-06-19T09:00:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152368",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:12'>Comment 12:</a>
That makes sense, you you should really make sure that the upstream polybori-0.8.2 is **identical** to the Sage polybori-0.8.2.

Believe me, I have seen this kind of problems in the past with other packages where the upstream version was close to, but not quite equal to the Sage version.



---

archive/issue_comments_152369.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nReplying to [@jdemeyer](#comment%3A9):\n> Please don't name the spkg version \"0.8.2\" if polybori-0.8.2 hasn't actually been released!  If the upstream version is called \"0.8.2rc1\", then so should the Sage version be (including in `SPKG.txt`).\n\nBecause packages like PolyBoRi (and e.g., M4RI) wait for Sage's tests to pass before releasing, this strategy would create unnecessary bureaucracy. We'd do a rcX, Sage includes the package, we another package which is the final.",
    "created_at": "2012-06-19T09:02:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152369",
    "user": "https://github.com/malb"
}
```

<a id='comment:13'>Comment 13:</a>
Replying to [@jdemeyer](#comment%3A9):
> Please don't name the spkg version "0.8.2" if polybori-0.8.2 hasn't actually been released!  If the upstream version is called "0.8.2rc1", then so should the Sage version be (including in `SPKG.txt`).

Because packages like PolyBoRi (and e.g., M4RI) wait for Sage's tests to pass before releasing, this strategy would create unnecessary bureaucracy. We'd do a rcX, Sage includes the package, we another package which is the final.



---

archive/issue_comments_152370.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nReplying to [@alexanderdreyer](#comment%3A8):\n> @malb Staying positive reviewed?\n\nYep.",
    "created_at": "2012-06-19T09:03:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152370",
    "user": "https://github.com/malb"
}
```

<a id='comment:14'>Comment 14:</a>
Replying to [@alexanderdreyer](#comment%3A8):
> @malb Staying positive reviewed?

Yep.



---

archive/issue_events_161111.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-19T09:15:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13124#event-161111"
}
```



---

archive/issue_events_161112.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-19T09:15:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13124#event-161112"
}
```



---

archive/issue_comments_152371.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nPositive review then.",
    "created_at": "2012-06-19T09:15:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152371",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:15'>Comment 15:</a>
Positive review then.



---

archive/issue_comments_152372.json:
```json
{
    "body": "Changed work issues from **version number** to none",
    "created_at": "2012-06-19T09:15:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152372",
    "user": "https://github.com/jdemeyer"
}
```

Changed work issues from **version number** to none



---

archive/issue_comments_152373.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nUnderstanding that spkgs could be messed up, I document the source matching here:\n\n```\n> md5sum *\n7e077b1838ec7a80adf250d64ed17e38  polybori-0.8.2rc3.tar.gz\n872cd541b398de44a602a058286d6a17  polybori-0.8.2.spkg\n> mkdir orig; tar -C orig -xzf polybori-0.8.2rc3.tar.gz\n> mkdir spkg; tar -C spkg -xjf polybori-0.8.2.spkg\n> diff --recursive -u spkg/polybori-0.8.2/src/ orig/polybori-0.8.2/\nOnly in orig/polybori-0.8.2/: M4RI\n```",
    "created_at": "2012-06-19T09:42:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152373",
    "user": "https://github.com/alexanderdreyer"
}
```

<a id='comment:16'>Comment 16:</a>
Understanding that spkgs could be messed up, I document the source matching here:

```
> md5sum *
7e077b1838ec7a80adf250d64ed17e38  polybori-0.8.2rc3.tar.gz
872cd541b398de44a602a058286d6a17  polybori-0.8.2.spkg
> mkdir orig; tar -C orig -xzf polybori-0.8.2rc3.tar.gz
> mkdir spkg; tar -C spkg -xjf polybori-0.8.2.spkg
> diff --recursive -u spkg/polybori-0.8.2/src/ orig/polybori-0.8.2/
Only in orig/polybori-0.8.2/: M4RI
```



---

archive/issue_comments_152374.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nI just noticed that polybori was the culprit for the `mult_table0.png` .. files, which is fixed in this version. So please update ;-)",
    "created_at": "2012-06-21T13:31:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152374",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:17'>Comment 17:</a>
I just noticed that polybori was the culprit for the `mult_table0.png` .. files, which is fixed in this version. So please update ;-)



---

archive/issue_comments_152375.json:
```json
{
    "body": "<a id='comment:18'>Comment 18:</a>\nFails to build on `hawk` (OpenSolaris 06.2009-32):\n\n```\npolybori-0.8.2\n====================================================\nExtracting package /export/home/buildbot/build/sage/hawk-1/hawk_full/build/sage-5.2.beta0/spkg/standard/polybori-0.8.2.spkg\n-rw-r--r--   1 buildbot other    13486080 Jun 29 22:30 /export/home/buildbot/build/sage/hawk-1/hawk_full/build/sage-5.2.beta0/spkg/standard/polybori-0.8.2.spkg\nFinished extraction\n****************************************************\nHost system:\nSunOS hawk 5.11 snv_134 i86pc i386 i86pc\n****************************************************\nC compiler: gcc\nC compiler version:\nUsing built-in specs.\nTarget: i386-pc-solaris2.11\nConfigured with: ../gcc-4.4-20100112/configure --with-as=/usr/local/binutils-2.20/bin/as --with-ld=/usr/ccs/bin/ld --with-gmp=/usr/local --with-mpfr=/usr/local\nThread model: posix\ngcc version 4.4.3 20100112 (prerelease) (GCC) \n****************************************************\nStarting build...\nRemoving old PolyBoRi install...\nDone removing old PolyBoRi install.\nRunning build_polybori...\nscons: Reading SConscript files ...\nPlatform:  sunos5\nPlatform:  sunos5\nPlatform:  sunos5\nValueError: too many values to unpack:\n  File \"/export/home/buildbot/build/sage/hawk-1/hawk_full/build/sage-5.2.beta0/spkg/build/polybori-0.8.2/src/SConstruct\", line 483:\n    (env, opts) = setup_env(defaultenv)\nError building PolyBoRi.\n```",
    "created_at": "2012-06-30T07:51:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152375",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:18'>Comment 18:</a>
Fails to build on `hawk` (OpenSolaris 06.2009-32):

```
polybori-0.8.2
====================================================
Extracting package /export/home/buildbot/build/sage/hawk-1/hawk_full/build/sage-5.2.beta0/spkg/standard/polybori-0.8.2.spkg
-rw-r--r--   1 buildbot other    13486080 Jun 29 22:30 /export/home/buildbot/build/sage/hawk-1/hawk_full/build/sage-5.2.beta0/spkg/standard/polybori-0.8.2.spkg
Finished extraction
****************************************************
Host system:
SunOS hawk 5.11 snv_134 i86pc i386 i86pc
****************************************************
C compiler: gcc
C compiler version:
Using built-in specs.
Target: i386-pc-solaris2.11
Configured with: ../gcc-4.4-20100112/configure --with-as=/usr/local/binutils-2.20/bin/as --with-ld=/usr/ccs/bin/ld --with-gmp=/usr/local --with-mpfr=/usr/local
Thread model: posix
gcc version 4.4.3 20100112 (prerelease) (GCC) 
****************************************************
Starting build...
Removing old PolyBoRi install...
Done removing old PolyBoRi install.
Running build_polybori...
scons: Reading SConscript files ...
Platform:  sunos5
Platform:  sunos5
Platform:  sunos5
ValueError: too many values to unpack:
  File "/export/home/buildbot/build/sage/hawk-1/hawk_full/build/sage-5.2.beta0/spkg/build/polybori-0.8.2/src/SConstruct", line 483:
    (env, opts) = setup_env(defaultenv)
Error building PolyBoRi.
```



---

archive/issue_events_161113.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-30T07:51:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13124#event-161113"
}
```



---

archive/issue_events_161114.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-06-30T07:51:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13124#event-161114"
}
```



---

archive/issue_comments_152376.json:
```json
{
    "body": "<a id='comment:19'>Comment 19:</a>\nI fixed that issue and added an updated spkg at the same place.\nhttp://boxen.math.washington.edu/home/dreyer/spkg/polybori-0.8.2.spkg\n(The previous one archived to polybori-0.8.2.spkg.1)",
    "created_at": "2012-06-30T14:16:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152376",
    "user": "https://github.com/alexanderdreyer"
}
```

<a id='comment:19'>Comment 19:</a>
I fixed that issue and added an updated spkg at the same place.
http://boxen.math.washington.edu/home/dreyer/spkg/polybori-0.8.2.spkg
(The previous one archived to polybori-0.8.2.spkg.1)



---

archive/issue_events_161115.json:
```json
{
    "actor": "https://github.com/alexanderdreyer",
    "created_at": "2012-06-30T14:16:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13124#event-161115"
}
```



---

archive/issue_events_161116.json:
```json
{
    "actor": "https://github.com/alexanderdreyer",
    "created_at": "2012-06-30T14:16:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13124#event-161116"
}
```



---

archive/issue_comments_152377.json:
```json
{
    "body": "<a id='comment:20'>Comment 20:</a>\nThe download link gives a 404 Not Found error.",
    "created_at": "2012-07-02T15:43:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152377",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:20'>Comment 20:</a>
The download link gives a 404 Not Found error.



---

archive/issue_events_161117.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-02T15:43:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13124#event-161117"
}
```



---

archive/issue_events_161118.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-02T15:43:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13124#event-161118"
}
```



---

archive/issue_comments_152378.json:
```json
{
    "body": "Work Issues: **HTTP 404**",
    "created_at": "2012-07-02T15:43:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152378",
    "user": "https://github.com/jdemeyer"
}
```

Work Issues: **HTTP 404**



---

archive/issue_comments_152379.json:
```json
{
    "body": "Changed work issues from **HTTP 404** to none",
    "created_at": "2012-07-02T18:42:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152379",
    "user": "https://github.com/alexanderdreyer"
}
```

Changed work issues from **HTTP 404** to none



---

archive/issue_events_161119.json:
```json
{
    "actor": "https://github.com/alexanderdreyer",
    "created_at": "2012-07-02T18:42:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13124#event-161119"
}
```



---

archive/issue_events_161120.json:
```json
{
    "actor": "https://github.com/alexanderdreyer",
    "created_at": "2012-07-02T18:42:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13124#event-161120"
}
```



---

archive/issue_comments_152380.json:
```json
{
    "body": "<a id='comment:21'>Comment 21:</a>\nSorry, I'd accidentally put it to the wrong dir. Now it's at the correct url.",
    "created_at": "2012-07-02T18:42:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152380",
    "user": "https://github.com/alexanderdreyer"
}
```

<a id='comment:21'>Comment 21:</a>
Sorry, I'd accidentally put it to the wrong dir. Now it's at the correct url.



---

archive/issue_events_161121.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-06T09:25:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13124#event-161121"
}
```



---

archive/issue_events_161122.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-06T09:25:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13124#event-161122"
}
```



---

archive/issue_comments_152381.json:
```json
{
    "body": "<a id='comment:22'>Comment 22:</a>\nCompiles fine on `hawk`.",
    "created_at": "2012-07-06T09:25:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152381",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:22'>Comment 22:</a>
Compiles fine on `hawk`.



---

archive/issue_events_161123.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-07T22:31:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13124#event-161123"
}
```



---

archive/issue_events_161124.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-07T22:31:05Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13124#event-161124"
}
```



---

archive/issue_comments_152382.json:
```json
{
    "body": "Merged: **sage-5.2.beta1**",
    "created_at": "2012-07-07T22:31:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152382",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.2.beta1**



---

archive/issue_comments_152383.json:
```json
{
    "body": "<a id='comment:24'>Comment 24:</a>\nJust for documentation: the sources of `polybori-0.8.2.spkg` from sage-5.2.rc0 became release 0.8.2 at https://sourceforge.net/projects/polybori/files/polybori/0.8.2/:\n\n```\n> md5sum polybori-0.8.2.tar.gz polybori-0.8.2.spkg\n82649420bae3275138469db6f19aac8e  polybori-0.8.2.tar.gz\n5d3fb11901c5ab23e34ca3eefbf3028c  polybori-0.8.2.spkg\n> mkdir orig; tar -C orig -xzf polybori-0.8.2.tar.gz\n> mkdir spkg; tar -C spkg -xjf polybori-0.8.2.spkg\n> diff --recursive -u spkg/polybori-0.8.2/src/ orig/polybori-0.8.2/\nOnly in orig/polybori-0.8.2/: M4RI\n```",
    "created_at": "2012-07-09T13:06:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13124#issuecomment-152383",
    "user": "https://github.com/alexanderdreyer"
}
```

<a id='comment:24'>Comment 24:</a>
Just for documentation: the sources of `polybori-0.8.2.spkg` from sage-5.2.rc0 became release 0.8.2 at https://sourceforge.net/projects/polybori/files/polybori/0.8.2/:

```
> md5sum polybori-0.8.2.tar.gz polybori-0.8.2.spkg
82649420bae3275138469db6f19aac8e  polybori-0.8.2.tar.gz
5d3fb11901c5ab23e34ca3eefbf3028c  polybori-0.8.2.spkg
> mkdir orig; tar -C orig -xzf polybori-0.8.2.tar.gz
> mkdir spkg; tar -C spkg -xjf polybori-0.8.2.spkg
> diff --recursive -u spkg/polybori-0.8.2/src/ orig/polybori-0.8.2/
Only in orig/polybori-0.8.2/: M4RI
```
