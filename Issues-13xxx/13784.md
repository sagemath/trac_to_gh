# Issue 13784: Move methods from GenericGraph to Graph

archive/issues_013580.json:
```json
{
    "body": "From Sage's reference manual :\n\n\"Graph stuff that should not be in this file\"\n\nThis patch solves the problem :\n\nApply:\n* [attachment:trac_13784.patch]\n* [attachment:trac_13784-bugfix.patch]\n\nAssignee: jason, ncohen, rlm\n\nReviewer: David Coudert\n\nAuthor: Nathann Cohen\n\nMerged: sage-5.6.beta0\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/13784\n\n",
    "closed_at": "2012-12-18T11:18:01Z",
    "created_at": "2012-12-01T11:43:58Z",
    "labels": [
        "component: graph theory",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-5.6",
    "title": "Move methods from GenericGraph to Graph",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/13784",
    "user": "https://github.com/nathanncohen"
}
```
From Sage's reference manual :

"Graph stuff that should not be in this file"

This patch solves the problem :

Apply:
* [attachment:trac_13784.patch]
* [attachment:trac_13784-bugfix.patch]

Assignee: jason, ncohen, rlm

Reviewer: David Coudert

Author: Nathann Cohen

Merged: sage-5.6.beta0

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/13784





---

archive/issue_comments_209262.json:
```json
{
    "body": "Attachment [trac_13784.patch](tarball://root/attachments/some-uuid/ticket13784/trac_13784.patch) by @nathanncohen created at 2012-12-01 11:44:24",
    "created_at": "2012-12-01T11:44:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13784",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13784#issuecomment-209262",
    "user": "https://github.com/nathanncohen"
}
```

Attachment [trac_13784.patch](tarball://root/attachments/some-uuid/ticket13784/trac_13784.patch) by @nathanncohen created at 2012-12-01 11:44:24



---

archive/issue_comments_209263.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2012-12-01T11:45:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13784",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13784#issuecomment-209263",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_209264.json:
```json
{
    "body": "<a id='comment:2'></a>Some updates are needed.\n\n```\nsage -t  \"devel/sage-myclone/sage/graphs/graph.py\"          \nIBM ILOG License Manager: \"IBM ILOG Optimization Suite for Academic Initiative\" is accessing CPLEX 12 with option(s): \"e m b q \".\n**********************************************************************\nFile \"/path-to-sage/sage-5.5.rc0/devel/sage-myclone/sage/graphs/graph.py\", line 4904:\n    sage: cores=b.cores(with_labels=True); cores\nException raised:\n    Traceback (most recent call last):\n      File \"/path-to-sage/sage-5.5.rc0/local/bin/ncadoctest.py\", line 1231, in run_one_test\n        self.run_one_example(test, example, filename, compileflags)\n      File \"/path-to-sage/sage-5.5.rc0/local/bin/sagedoctest.py\", line 38, in run_one_example\n        OrigDocTestRunner.run_one_example(self, test, example, filename, compileflags)\n      File \"/path-to-sage/sage-5.5.rc0/local/bin/ncadoctest.py\", line 1172, in run_one_example\n        compileflags, 1) in test.globs\n      File \"<doctest __main__.example_46[7]>\", line 1, in <module>\n        cores=b.cores(with_labels=True); cores###line 4904:\n    sage: cores=b.cores(with_labels=True); cores\n    AttributeError: 'DiGraph' object has no attribute 'cores'\n**********************************************************************\nFile \"/path-to-sage/sage-5.5.rc0/devel/sage-myclone/sage/graphs/graph.py\", line 4906:\n    sage: [v for v,c in cores.items() if c>=2] # the vertices in the 2-core\nException raised:\n    Traceback (most recent call last):\n      File \"/path-to-sage/sage-5.5.rc0/local/bin/ncadoctest.py\", line 1231, in run_one_test\n        self.run_one_example(test, example, filename, compileflags)\n      File \"/path-to-sage/sage-5.5.rc0/local/bin/sagedoctest.py\", line 38, in run_one_example\n        OrigDocTestRunner.run_one_example(self, test, example, filename, compileflags)\n      File \"/path-to-sage/sage-5.5.rc0/local/bin/ncadoctest.py\", line 1172, in run_one_example\n        compileflags, 1) in test.globs\n      File \"<doctest __main__.example_46[8]>\", line 1, in <module>\n        [v for v,c in cores.items() if c>=Integer(2)] # the vertices in the 2-core###line 4906:\n    sage: [v for v,c in cores.items() if c>=2] # the vertices in the 2-core\n    NameError: name 'cores' is not defined\n**********************************************************************\n1 items had failures:\n   2 of  13 in __main__.example_46\n***Test Failed*** 2 failures.\nFor whitespace errors, see the file /home/dcoudert/.sage//tmp/graph_29310.py\n         [17.6 s]\n \n----------------------------------------------------------------------\nThe following tests failed:\n\n\n        sage -t  \"devel/sage-myclone/sage/graphs/graph.py\"\nTotal time for all tests: 17.6 seconds\n```",
    "created_at": "2012-12-01T14:56:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13784",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13784#issuecomment-209264",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:2'></a>Some updates are needed.

```
sage -t  "devel/sage-myclone/sage/graphs/graph.py"          
IBM ILOG License Manager: "IBM ILOG Optimization Suite for Academic Initiative" is accessing CPLEX 12 with option(s): "e m b q ".
**********************************************************************
File "/path-to-sage/sage-5.5.rc0/devel/sage-myclone/sage/graphs/graph.py", line 4904:
    sage: cores=b.cores(with_labels=True); cores
Exception raised:
    Traceback (most recent call last):
      File "/path-to-sage/sage-5.5.rc0/local/bin/ncadoctest.py", line 1231, in run_one_test
        self.run_one_example(test, example, filename, compileflags)
      File "/path-to-sage/sage-5.5.rc0/local/bin/sagedoctest.py", line 38, in run_one_example
        OrigDocTestRunner.run_one_example(self, test, example, filename, compileflags)
      File "/path-to-sage/sage-5.5.rc0/local/bin/ncadoctest.py", line 1172, in run_one_example
        compileflags, 1) in test.globs
      File "<doctest __main__.example_46[7]>", line 1, in <module>
        cores=b.cores(with_labels=True); cores###line 4904:
    sage: cores=b.cores(with_labels=True); cores
    AttributeError: 'DiGraph' object has no attribute 'cores'
**********************************************************************
File "/path-to-sage/sage-5.5.rc0/devel/sage-myclone/sage/graphs/graph.py", line 4906:
    sage: [v for v,c in cores.items() if c>=2] # the vertices in the 2-core
Exception raised:
    Traceback (most recent call last):
      File "/path-to-sage/sage-5.5.rc0/local/bin/ncadoctest.py", line 1231, in run_one_test
        self.run_one_example(test, example, filename, compileflags)
      File "/path-to-sage/sage-5.5.rc0/local/bin/sagedoctest.py", line 38, in run_one_example
        OrigDocTestRunner.run_one_example(self, test, example, filename, compileflags)
      File "/path-to-sage/sage-5.5.rc0/local/bin/ncadoctest.py", line 1172, in run_one_example
        compileflags, 1) in test.globs
      File "<doctest __main__.example_46[8]>", line 1, in <module>
        [v for v,c in cores.items() if c>=Integer(2)] # the vertices in the 2-core###line 4906:
    sage: [v for v,c in cores.items() if c>=2] # the vertices in the 2-core
    NameError: name 'cores' is not defined
**********************************************************************
1 items had failures:
   2 of  13 in __main__.example_46
***Test Failed*** 2 failures.
For whitespace errors, see the file /home/dcoudert/.sage//tmp/graph_29310.py
         [17.6 s]
 
----------------------------------------------------------------------
The following tests failed:


        sage -t  "devel/sage-myclone/sage/graphs/graph.py"
Total time for all tests: 17.6 seconds
```



---

archive/issue_comments_209265.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2012-12-01T14:56:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13784",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13784#issuecomment-209265",
    "user": "https://github.com/dcoudert"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_209266.json:
```json
{
    "body": "Changing reviewer from \"\" to \"David Coudert\"",
    "created_at": "2012-12-01T14:56:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13784",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13784#issuecomment-209266",
    "user": "https://github.com/dcoudert"
}
```

Changing reviewer from "" to "David Coudert"



---

archive/issue_comments_209267.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2012-12-02T09:28:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13784",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13784#issuecomment-209267",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_209268.json:
```json
{
    "body": "<a id='comment:3'></a>Weird `O_o`\n\nI read the source code again, but the conclusion is the same : the cores method is not meant to be used on DiGraph... I mean : in this situation, it returns the same result as G.cores() on the underlying graph.\n\nI just added another patch that changes this `DiGraph` to a `Graph`, and tests pass afterwards. I do this in another patch so that the first one only moves methods around without modifying them. Thanks for noticing this !!!! \n\nNathann",
    "created_at": "2012-12-02T09:28:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13784",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13784#issuecomment-209268",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:3'></a>Weird `O_o`

I read the source code again, but the conclusion is the same : the cores method is not meant to be used on DiGraph... I mean : in this situation, it returns the same result as G.cores() on the underlying graph.

I just added another patch that changes this `DiGraph` to a `Graph`, and tests pass afterwards. I do this in another patch so that the first one only moves methods around without modifying them. Thanks for noticing this !!!! 

Nathann



---

archive/issue_comments_209269.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,4 +2,8 @@\n \n \"Graph stuff that should not be in this file\"\n \n-This patch solves the problem\n+This patch solves the problem :\n+\n+Apply:\n+* [attachment:trac_13784.patch]\n+* [attachment:trac_13784-bugfix.patch]\n``````\n",
    "created_at": "2012-12-02T09:30:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13784",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13784#issuecomment-209269",
    "user": "https://github.com/nathanncohen"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,4 +2,8 @@
 
 "Graph stuff that should not be in this file"
 
-This patch solves the problem
+This patch solves the problem :
+
+Apply:
+* [attachment:trac_13784.patch]
+* [attachment:trac_13784-bugfix.patch]
``````




---

archive/issue_comments_209270.json:
```json
{
    "body": "<a id='comment:5'></a>The patch is OK, and it passes successfully tests on the entire graph directory.\n\nHowever, when looking at the doc, I saw a very small bug that is not due to this patch:\n\n```\n    :meth=`~GenericGraph.is_cut_edge` | Returns True if the input edge is a cut-edge or a bridge. \n```\nCould you fix this missing \":\" after \":meth\" in this patch, or is it better to have a dedicated patch?",
    "created_at": "2012-12-02T10:58:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13784",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13784#issuecomment-209270",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:5'></a>The patch is OK, and it passes successfully tests on the entire graph directory.

However, when looking at the doc, I saw a very small bug that is not due to this patch:

```
    :meth=`~GenericGraph.is_cut_edge` | Returns True if the input edge is a cut-edge or a bridge. 
```
Could you fix this missing ":" after ":meth" in this patch, or is it better to have a dedicated patch?



---

archive/issue_comments_209271.json:
```json
{
    "body": "<a id='comment:6'></a>No you are right, I already forgot many times to fix it somewhere `^^;`\n\nGive me ten seconds `:-P`\n\nNathann",
    "created_at": "2012-12-02T11:00:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13784",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13784#issuecomment-209271",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:6'></a>No you are right, I already forgot many times to fix it somewhere `^^;`

Give me ten seconds `:-P`

Nathann



---

archive/issue_comments_209272.json:
```json
{
    "body": "<a id='comment:7'></a>Here it is !\n\nNathann",
    "created_at": "2012-12-02T11:01:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13784",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13784#issuecomment-209272",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:7'></a>Here it is !

Nathann



---

archive/issue_comments_209273.json:
```json
{
    "body": "Attachment [trac_13784-bugfix.patch](tarball://root/attachments/some-uuid/ticket13784/trac_13784-bugfix.patch) by @nathanncohen created at 2012-12-02 11:02:01",
    "created_at": "2012-12-02T11:02:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13784",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13784#issuecomment-209273",
    "user": "https://github.com/nathanncohen"
}
```

Attachment [trac_13784-bugfix.patch](tarball://root/attachments/some-uuid/ticket13784/trac_13784-bugfix.patch) by @nathanncohen created at 2012-12-02 11:02:01



---

archive/issue_comments_209274.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2012-12-02T11:07:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13784",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13784#issuecomment-209274",
    "user": "https://github.com/dcoudert"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_209275.json:
```json
{
    "body": "Changing author from \"\" to \"Nathann Cohen\"",
    "created_at": "2012-12-02T11:07:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13784",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13784#issuecomment-209275",
    "user": "https://github.com/dcoudert"
}
```

Changing author from "" to "Nathann Cohen"



---

archive/issue_comments_209276.json:
```json
{
    "body": "<a id='comment:8'></a>Thanks.",
    "created_at": "2012-12-02T11:07:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13784",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13784#issuecomment-209276",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:8'></a>Thanks.



---

archive/issue_comments_209277.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-5.6.beta0\"",
    "created_at": "2012-12-18T11:18:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13784",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13784#issuecomment-209277",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-5.6.beta0"



---

archive/issue_events_038650.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-12-18T11:18:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/13784",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13784#event-38650"
}
```



---

archive/issue_comments_209278.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2012-12-18T11:18:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13784",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13784#issuecomment-209278",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed
