# Issue 13681: fix issues with SAGE_TMP

archive/issues_013477.json:
```json
{
    "assignees": [],
    "body": "#13579 changed `SAGE_TMP`. There are some places with less-than-ideal handling of `SAGE_TMP` that are broken in the process.\n\n* `sage-cleaner` hardcodes '.../temp/...' but thats not correct any more.\n* Many doctests use SAGE_TMP+'foo.bar'. The new SAGE_TMP doesn't end in a backslash any more. Using `os.path.join` would have saved the day.\n\nFollow-up: #13147\n\nApply \n* [attachment: 13681_scripts.patch.gz](https://github.com/sagemath/sage/files/ticket13681/13681_scripts.patch.gz) to the `SAGE_ROOT/local/bin` repo\n* [attachment: 13681_sagelib.patch.gz](https://github.com/sagemath/sage/files/ticket13681/13681_sagelib.patch.gz) to the Sage library\n\n\nDepends on #13631\n\nAssignee: **@nexttime**\n\nCC:  @jdemeyer\n\nReviewer: **Volker Braun**\n\nAuthor: **Jeroen Demeyer**\n\nMerged: **sage-5.4.rc4**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13681_\n\n",
    "closed_at": "2012-11-05T20:45:05Z",
    "created_at": "2012-11-03T16:11:11Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20scripts",
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix issues with SAGE_TMP",
    "type": "issue",
    "updated_at": "2012-11-06T19:45:28Z",
    "url": "https://github.com/sagemath/sage/issues/13681",
    "user": "https://github.com/vbraun"
}
```
#13579 changed `SAGE_TMP`. There are some places with less-than-ideal handling of `SAGE_TMP` that are broken in the process.

* `sage-cleaner` hardcodes '.../temp/...' but thats not correct any more.
* Many doctests use SAGE_TMP+'foo.bar'. The new SAGE_TMP doesn't end in a backslash any more. Using `os.path.join` would have saved the day.

Follow-up: #13147

Apply 
* [attachment: 13681_scripts.patch.gz](https://github.com/sagemath/sage/files/ticket13681/13681_scripts.patch.gz) to the `SAGE_ROOT/local/bin` repo
* [attachment: 13681_sagelib.patch.gz](https://github.com/sagemath/sage/files/ticket13681/13681_sagelib.patch.gz) to the Sage library


Depends on #13631

Assignee: **@nexttime**

CC:  @jdemeyer

Reviewer: **Volker Braun**

Author: **Jeroen Demeyer**

Merged: **sage-5.4.rc4**

_Issue created by migration from https://trac.sagemath.org/ticket/13681_





---

archive/issue_events_175612.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-11-03T16:11:11Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "milestone_number": null,
    "milestone_title": "sage-5.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13681#event-175612"
}
```



---

archive/issue_events_175613.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-11-03T16:11:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20scripts",
    "label_color": "000080",
    "label_name": "component: scripts",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13681#event-175613"
}
```



---

archive/issue_events_175614.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-11-03T16:11:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13681#event-175614"
}
```



---

archive/issue_events_175615.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-11-03T16:11:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13681#event-175615"
}
```



---

archive/issue_events_175616.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-11-03T18:10:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "milestone_number": null,
    "milestone_title": "sage-5.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13681#event-175616"
}
```



---

archive/issue_events_175617.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-11-03T18:10:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "milestone_number": null,
    "milestone_title": "sage-5.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13681#event-175617"
}
```



---

archive/issue_comments_162947.json:
```json
{
    "body": "Author: **Jeroen Demeyer**",
    "created_at": "2012-11-03T18:10:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13681#issuecomment-162947",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Jeroen Demeyer**



---

archive/issue_events_175618.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-11-03T18:10:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13681#event-175618"
}
```



---

archive/issue_events_175619.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-11-03T18:10:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13681#event-175619"
}
```



---

archive/issue_comments_162948.json:
```json
{
    "body": "Attachment: **[13681_scripts.patch.gz](https://github.com/sagemath/sage/files/ticket13681/13681_scripts.patch.gz)**",
    "created_at": "2012-11-03T18:10:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13681#issuecomment-162948",
    "user": "https://github.com/jdemeyer"
}
```

Attachment: **[13681_scripts.patch.gz](https://github.com/sagemath/sage/files/ticket13681/13681_scripts.patch.gz)**



---

archive/issue_comments_162949.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nWould you consider merging #13147 into 5.4, and then rebasing this on top of that?",
    "created_at": "2012-11-03T19:32:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13681#issuecomment-162949",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:2'>Comment 2:</a>
Would you consider merging #13147 into 5.4, and then rebasing this on top of that?



---

archive/issue_comments_162950.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nReplying to [@jhpalmieri](#comment%3A2):\n> Would you consider merging #13147 into 5.4, and then rebasing this on top of that?\n\nWhy?  I don't see the advantage.",
    "created_at": "2012-11-03T19:40:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13681#issuecomment-162950",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:3'>Comment 3:</a>
Replying to [@jhpalmieri](#comment%3A2):
> Would you consider merging #13147 into 5.4, and then rebasing this on top of that?

Why?  I don't see the advantage.



---

archive/issue_comments_162951.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nI could incorporate all the changes from #13147 *except* for the \"lazy string\" part.",
    "created_at": "2012-11-03T19:45:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13681#issuecomment-162951",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:4'>Comment 4:</a>
I could incorporate all the changes from #13147 *except* for the "lazy string" part.



---

archive/issue_comments_162952.json:
```json
{
    "body": "Attachment: **[13681_sagelib.patch.gz](https://github.com/sagemath/sage/files/ticket13681/13681_sagelib.patch.gz)**",
    "created_at": "2012-11-03T19:47:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13681#issuecomment-162952",
    "user": "https://github.com/jdemeyer"
}
```

Attachment: **[13681_sagelib.patch.gz](https://github.com/sagemath/sage/files/ticket13681/13681_sagelib.patch.gz)**



---

archive/issue_comments_162953.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,3 +2,5 @@\n \n * `sage-cleaner` hardcodes '.../temp/...' but thats not correct any more.\n * Many doctests use SAGE_TMP+'foo.bar'. The new SAGE_TMP doesn't end in a backslash any more. Using `os.path.join` would have saved the day.\n+\n+Follow-up: #13147\n``````\n",
    "created_at": "2012-11-03T19:47:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13681#issuecomment-162953",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,3 +2,5 @@
 
 * `sage-cleaner` hardcodes '.../temp/...' but thats not correct any more.
 * Many doctests use SAGE_TMP+'foo.bar'. The new SAGE_TMP doesn't end in a backslash any more. Using `os.path.join` would have saved the day.
+
+Follow-up: #13147
``````




---

archive/issue_events_175620.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-11-03T21:00:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13681#event-175620"
}
```



---

archive/issue_comments_162954.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nReplying to [@jdemeyer](#comment%3A4):\n> I could incorporate all the changes from #13147 *except* for the \"lazy string\" part.\n\nDone.",
    "created_at": "2012-11-03T21:00:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13681#issuecomment-162954",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:7'>Comment 7:</a>
Replying to [@jdemeyer](#comment%3A4):
> I could incorporate all the changes from #13147 *except* for the "lazy string" part.

Done.



---

archive/issue_comments_162955.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nReplying to [@jdemeyer](#comment%3A3):\n> Replying to [@jhpalmieri](#comment%3A2):\n> > Would you consider merging #13147 into 5.4, and then rebasing this on top of that?\n\n> Why?  I don't see the advantage.\n\nOne advantage would have been so #13147 wouldn't need rebasing, and it also solved some of the same issues. But now I guess it will need rebasing anyway.",
    "created_at": "2012-11-03T21:21:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13681#issuecomment-162955",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:8'>Comment 8:</a>
Replying to [@jdemeyer](#comment%3A3):
> Replying to [@jhpalmieri](#comment%3A2):
> > Would you consider merging #13147 into 5.4, and then rebasing this on top of that?

> Why?  I don't see the advantage.

One advantage would have been so #13147 wouldn't need rebasing, and it also solved some of the same issues. But now I guess it will need rebasing anyway.



---

archive/issue_comments_162956.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nDoctesting still litters files around than don't get deleted because `SAGE_TESTDIR` is not a the expected `DOT_SAGE/tmp/hostname/pid/...`. I'll attach a patch to fix this.\n\nPositive review to all the other patches.",
    "created_at": "2012-11-04T00:54:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13681#issuecomment-162956",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:9'>Comment 9:</a>
Doctesting still litters files around than don't get deleted because `SAGE_TESTDIR` is not a the expected `DOT_SAGE/tmp/hostname/pid/...`. I'll attach a patch to fix this.

Positive review to all the other patches.



---

archive/issue_comments_162957.json:
```json
{
    "body": "Initial patch",
    "created_at": "2012-11-04T00:54:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13681#issuecomment-162957",
    "user": "https://github.com/vbraun"
}
```

Initial patch



---

archive/issue_comments_162958.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nAttachment: **[trac_13681_root.patch.gz](https://github.com/sagemath/sage/files/ticket13681/trac_13681_root.patch.gz)**",
    "created_at": "2012-11-04T00:56:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13681#issuecomment-162958",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:10'>Comment 10:</a>
Attachment: **[trac_13681_root.patch.gz](https://github.com/sagemath/sage/files/ticket13681/trac_13681_root.patch.gz)**



---

archive/issue_comments_162959.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2012-11-04T00:56:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13681#issuecomment-162959",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_comments_162960.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -4,3 +4,9 @@\n * Many doctests use SAGE_TMP+'foo.bar'. The new SAGE_TMP doesn't end in a backslash any more. Using `os.path.join` would have saved the day.\n \n Follow-up: #13147\n+\n+Apply \n+* [attachment: 13681_scripts.patch.gz](https://github.com/sagemath/sage/files/ticket13681/13681_scripts.patch.gz) to the `SAGE_ROOT/local/bin` repo\n+* [attachment: 13681_sagelib.patch.gz](https://github.com/sagemath/sage/files/ticket13681/13681_sagelib.patch.gz) to the Sage library\n+* [attachment: trac_13681_root.patch.gz](https://github.com/sagemath/sage/files/ticket13681/trac_13681_root.patch.gz) to the root repo\n+\n``````\n",
    "created_at": "2012-11-04T00:56:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13681#issuecomment-162960",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -4,3 +4,9 @@
 * Many doctests use SAGE_TMP+'foo.bar'. The new SAGE_TMP doesn't end in a backslash any more. Using `os.path.join` would have saved the day.
 
 Follow-up: #13147
+
+Apply 
+* [attachment: 13681_scripts.patch.gz](https://github.com/sagemath/sage/files/ticket13681/13681_scripts.patch.gz) to the `SAGE_ROOT/local/bin` repo
+* [attachment: 13681_sagelib.patch.gz](https://github.com/sagemath/sage/files/ticket13681/13681_sagelib.patch.gz) to the Sage library
+* [attachment: trac_13681_root.patch.gz](https://github.com/sagemath/sage/files/ticket13681/trac_13681_root.patch.gz) to the root repo
+
``````




---

archive/issue_comments_162961.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nReplying to [@vbraun](#comment%3A9):\n> Doctesting still litters files around than don't get deleted because `SAGE_TESTDIR` is not a the expected `DOT_SAGE/tmp/hostname/pid/...`.\n\nIs this a regression due to #13579?  If it's not, I prefer to leave this change for a different ticket.",
    "created_at": "2012-11-04T08:43:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13681#issuecomment-162961",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:11'>Comment 11:</a>
Replying to [@vbraun](#comment%3A9):
> Doctesting still litters files around than don't get deleted because `SAGE_TESTDIR` is not a the expected `DOT_SAGE/tmp/hostname/pid/...`.

Is this a regression due to #13579?  If it's not, I prefer to leave this change for a different ticket.



---

archive/issue_comments_162962.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nVolker, could you agree with not merging your patch here?  I feel that the chance of stuff breaking is too big to risk merging it.",
    "created_at": "2012-11-05T07:48:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13681#issuecomment-162962",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:12'>Comment 12:</a>
Volker, could you agree with not merging your patch here?  I feel that the chance of stuff breaking is too big to risk merging it.



---

archive/issue_events_175621.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-11-05T20:06:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13681#event-175621"
}
```



---

archive/issue_events_175622.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-11-05T20:06:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13681#event-175622"
}
```



---

archive/issue_comments_162963.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nI'll attach the `SAGE_TESTDIR` stuff to the followup ticket, then.",
    "created_at": "2012-11-05T20:06:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13681#issuecomment-162963",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:13'>Comment 13:</a>
I'll attach the `SAGE_TESTDIR` stuff to the followup ticket, then.



---

archive/issue_comments_162964.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -8,5 +8,4 @@\n Apply \n * [attachment: 13681_scripts.patch.gz](https://github.com/sagemath/sage/files/ticket13681/13681_scripts.patch.gz) to the `SAGE_ROOT/local/bin` repo\n * [attachment: 13681_sagelib.patch.gz](https://github.com/sagemath/sage/files/ticket13681/13681_sagelib.patch.gz) to the Sage library\n-* [attachment: trac_13681_root.patch.gz](https://github.com/sagemath/sage/files/ticket13681/trac_13681_root.patch.gz) to the root repo\n \n``````\n",
    "created_at": "2012-11-05T20:09:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13681#issuecomment-162964",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -8,5 +8,4 @@
 Apply 
 * [attachment: 13681_scripts.patch.gz](https://github.com/sagemath/sage/files/ticket13681/13681_scripts.patch.gz) to the `SAGE_ROOT/local/bin` repo
 * [attachment: 13681_sagelib.patch.gz](https://github.com/sagemath/sage/files/ticket13681/13681_sagelib.patch.gz) to the Sage library
-* [attachment: trac_13681_root.patch.gz](https://github.com/sagemath/sage/files/ticket13681/trac_13681_root.patch.gz) to the root repo
 
``````




---

archive/issue_comments_162965.json:
```json
{
    "body": "Merged: **sage-5.4.rc4**",
    "created_at": "2012-11-05T20:45:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13681#issuecomment-162965",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.4.rc4**



---

archive/issue_events_175623.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-11-05T20:45:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13681#event-175623"
}
```



---

archive/issue_events_175624.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-11-05T20:45:05Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13681#event-175624"
}
```



---

archive/issue_comments_162966.json:
```json
{
    "body": "Dependencies: **#13631**",
    "created_at": "2012-11-06T19:45:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13681",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13681#issuecomment-162966",
    "user": "https://github.com/jdemeyer"
}
```

Dependencies: **#13631**
