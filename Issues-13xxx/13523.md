# Issue 13523: Implement sage -O which passes the -O option to python to disable assertions

archive/issues_013319.json:
```json
{
    "assignees": [],
    "body": "In the meantime, one can export `PYTHONOPTIMIZE=yes` to get the same effect.\n\nAssignee: **@nexttime**\n\nKeywords: **days85**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13523_\n\n",
    "created_at": "2012-09-24T08:40:43Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20scripts",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/needs%20info"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Implement sage -O which passes the -O option to python to disable assertions",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/13523",
    "user": "https://github.com/nthiery"
}
```
In the meantime, one can export `PYTHONOPTIMIZE=yes` to get the same effect.

Assignee: **@nexttime**

Keywords: **days85**

_Issue created by migration from https://trac.sagemath.org/ticket/13523_





---

archive/issue_events_172982.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2012-09-24T08:40:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13523#event-172982"
}
```



---

archive/issue_events_172983.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2012-09-24T08:40:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20scripts",
    "label_color": "000080",
    "label_name": "component: scripts",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13523#event-172983"
}
```



---

archive/issue_events_172984.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2012-09-24T08:40:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13523#event-172984"
}
```



---

archive/issue_events_172985.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2012-09-24T08:40:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13523#event-172985"
}
```



---

archive/issue_events_172986.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13523#event-172986"
}
```



---

archive/issue_events_172987.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13523#event-172987"
}
```



---

archive/issue_events_172988.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13523#event-172988"
}
```



---

archive/issue_events_172989.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13523#event-172989"
}
```



---

archive/issue_events_172990.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13523#event-172990"
}
```



---

archive/issue_events_172991.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13523#event-172991"
}
```



---

archive/issue_events_172992.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13523#event-172992"
}
```



---

archive/issue_events_172993.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13523#event-172993"
}
```



---

archive/issue_comments_160172.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-\n+In the meantime, one can export `PYTHONOPTIMIZE=yes` to get the same effect.\n``````\n",
    "created_at": "2016-12-07T18:27:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13523#issuecomment-160172",
    "user": "https://github.com/saraedum"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-
+In the meantime, one can export `PYTHONOPTIMIZE=yes` to get the same effect.
``````




---

archive/issue_comments_160173.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nSince the command line interface is hacky enough already maybe it would be better to document the `PYTHONOPTIMIZE=yes` somewhere instead?",
    "created_at": "2017-03-14T12:23:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13523#issuecomment-160173",
    "user": "https://github.com/saraedum"
}
```

<div id="comment:6" align="right">Comment 6</div>

Since the command line interface is hacky enough already maybe it would be better to document the `PYTHONOPTIMIZE=yes` somewhere instead?



---

archive/issue_events_172994.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-03-14T12:23:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13523#event-172994"
}
```



---

archive/issue_comments_160174.json:
```json
{
    "body": "Changed keywords from **** to **days85**",
    "created_at": "2017-03-14T16:12:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13523#issuecomment-160174",
    "user": "https://github.com/saraedum"
}
```

Changed keywords from **** to **days85**



---

archive/issue_events_172995.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13523",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13523#event-172995"
}
```
