# Issue 13815: Fix type + code cleanup in numerical.optimize

archive/issues_013611.json:
```json
{
    "assignees": [],
    "body": "It's *Broyden-Fletcher-Goldfarb-Shanno* not *Broyden-Fletcher-Goldfarb-Shanno**n***.\n\nThe attached patch also uses a python3 compatible syntax to raise errors.\n\nAssignee: **@jasongrout, jkantor**\n\nReviewer: **Volker Braun**\n\nAuthor: **Andr\u00e9 Apitzsch**\n\nMerged: **sage-5.6.beta2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13815_\n\n",
    "closed_at": "2012-12-27T10:24:29Z",
    "created_at": "2012-12-09T21:07:45Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20numerical",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix type + code cleanup in numerical.optimize",
    "type": "issue",
    "updated_at": "2012-12-27T10:24:29Z",
    "url": "https://github.com/sagemath/sage/issues/13815",
    "user": "https://github.com/a-andre"
}
```
It's *Broyden-Fletcher-Goldfarb-Shanno* not *Broyden-Fletcher-Goldfarb-Shanno**n***.

The attached patch also uses a python3 compatible syntax to raise errors.

Assignee: **@jasongrout, jkantor**

Reviewer: **Volker Braun**

Author: **André Apitzsch**

Merged: **sage-5.6.beta2**

_Issue created by migration from https://trac.sagemath.org/ticket/13815_





---

archive/issue_events_177727.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2012-12-09T21:07:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13815",
    "milestone_number": null,
    "milestone_title": "sage-5.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13815#event-177727"
}
```



---

archive/issue_events_177728.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2012-12-09T21:07:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13815",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20numerical",
    "label_color": "0000ff",
    "label_name": "component: numerical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13815#event-177728"
}
```



---

archive/issue_events_177729.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2012-12-09T21:07:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13815",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13815#event-177729"
}
```



---

archive/issue_events_177730.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2012-12-09T21:07:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13815",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13815#event-177730"
}
```



---

archive/issue_comments_165224.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nPlease no unnecessary whitespace changes. This only serves to break other people's patches.",
    "created_at": "2012-12-10T10:37:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13815",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13815#issuecomment-165224",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:1'>Comment 1:</a>
Please no unnecessary whitespace changes. This only serves to break other people's patches.



---

archive/issue_comments_165225.json:
```json
{
    "body": "Attachment: **[trac_13815.patch.gz](https://github.com/sagemath/sage/files/ticket13815/trac_13815.patch.gz)**",
    "created_at": "2012-12-10T17:34:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13815",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13815#issuecomment-165225",
    "user": "https://github.com/a-andre"
}
```

Attachment: **[trac_13815.patch.gz](https://github.com/sagemath/sage/files/ticket13815/trac_13815.patch.gz)**



---

archive/issue_comments_165226.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nI removed the whitespace changes. Let's hope we get rid of the trailing whitespaces when moving to git.",
    "created_at": "2012-12-10T17:41:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13815",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13815#issuecomment-165226",
    "user": "https://github.com/a-andre"
}
```

<a id='comment:2'>Comment 2:</a>
I removed the whitespace changes. Let's hope we get rid of the trailing whitespaces when moving to git.



---

archive/issue_events_177731.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2012-12-10T17:41:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13815",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13815#event-177731"
}
```



---

archive/issue_comments_165227.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n It's *Broyden-Fletcher-Goldfarb-Shanno* not *Broyden-Fletcher-Goldfarb-Shanno**n***.\n \n-The attached patch also removes trailing white spaces and uses a python3 compatible syntax to raise errors.\n+The attached patch also uses a python3 compatible syntax to raise errors.\n``````\n",
    "created_at": "2012-12-10T17:41:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13815",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13815#issuecomment-165227",
    "user": "https://github.com/a-andre"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 It's *Broyden-Fletcher-Goldfarb-Shanno* not *Broyden-Fletcher-Goldfarb-Shanno**n***.
 
-The attached patch also removes trailing white spaces and uses a python3 compatible syntax to raise errors.
+The attached patch also uses a python3 compatible syntax to raise errors.
``````




---

archive/issue_events_177732.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-12-10T18:19:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13815",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13815#event-177732"
}
```



---

archive/issue_events_177733.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-12-10T18:19:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13815",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13815#event-177733"
}
```



---

archive/issue_comments_165228.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThe `%`-operator is deprecated in py3, so I guess we are not fully compliant. But its better than before ;-)",
    "created_at": "2012-12-10T18:19:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13815",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13815#issuecomment-165228",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:3'>Comment 3:</a>
The `%`-operator is deprecated in py3, so I guess we are not fully compliant. But its better than before ;-)



---

archive/issue_comments_165229.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2012-12-10T18:19:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13815",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13815#issuecomment-165229",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_comments_165230.json:
```json
{
    "body": "Merged: **sage-5.6.beta2**",
    "created_at": "2012-12-27T10:24:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13815",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13815#issuecomment-165230",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.6.beta2**



---

archive/issue_events_177734.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-12-27T10:24:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13815",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13815#event-177734"
}
```



---

archive/issue_events_177735.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-12-27T10:24:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13815",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13815#event-177735"
}
```
