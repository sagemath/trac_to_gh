# Issue 13740: Upgrade Cython to 0.17.2

archive/issues_013536.json:
```json
{
    "assignees": [],
    "body": "New Cython version is out! Get it while its hot :-)\n\nhttp://www.stp.dias.ie/~vbraun/Sage/spkg/cython-0.17.2.spkg\n\n**Apply**: [attachment: trac_13740_final_fixes.patch.gz](https://github.com/sagemath/sage/files/ticket13740/trac_13740_final_fixes.patch.gz)\n\nReviewer: **Robert Bradshaw**\n\nAuthor: **Volker Braun**\n\nMerged: **sage-5.6.beta0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13740_\n\n",
    "closed_at": "2012-12-18T11:14:40Z",
    "created_at": "2012-11-21T22:42:14Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Upgrade Cython to 0.17.2",
    "type": "issue",
    "updated_at": "2012-12-18T11:14:40Z",
    "url": "https://github.com/sagemath/sage/issues/13740",
    "user": "https://github.com/vbraun"
}
```
New Cython version is out! Get it while its hot :-)

http://www.stp.dias.ie/~vbraun/Sage/spkg/cython-0.17.2.spkg

**Apply**: [attachment: trac_13740_final_fixes.patch.gz](https://github.com/sagemath/sage/files/ticket13740/trac_13740_final_fixes.patch.gz)

Reviewer: **Robert Bradshaw**

Author: **Volker Braun**

Merged: **sage-5.6.beta0**

_Issue created by migration from https://trac.sagemath.org/ticket/13740_





---

archive/issue_events_176617.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-11-21T22:42:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13740",
    "milestone_number": null,
    "milestone_title": "sage-5.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13740#event-176617"
}
```



---

archive/issue_events_176618.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-11-21T22:42:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13740",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13740#event-176618"
}
```



---

archive/issue_events_176619.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-11-21T22:42:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13740",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13740#event-176619"
}
```



---

archive/issue_events_176620.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-11-21T22:42:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13740",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13740#event-176620"
}
```



---

archive/issue_comments_164116.json:
```json
{
    "body": "Initial patch",
    "created_at": "2012-11-22T21:08:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13740",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13740#issuecomment-164116",
    "user": "https://github.com/vbraun"
}
```

Initial patch



---

archive/issue_comments_164117.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[trac_13740_final_fixes.patch.gz](https://github.com/sagemath/sage/files/ticket13740/trac_13740_final_fixes.patch.gz)**\n\nThe patch removes **inline** statements that are either nonsensical (where another module tries to import an inline'd function) or currently unsupported by Cython, see\n\nhttps://groups.google.com/d/topic/cython-users/UuQvpvdNhjc/discussion",
    "created_at": "2012-11-22T21:11:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13740",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13740#issuecomment-164117",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[trac_13740_final_fixes.patch.gz](https://github.com/sagemath/sage/files/ticket13740/trac_13740_final_fixes.patch.gz)**

The patch removes **inline** statements that are either nonsensical (where another module tries to import an inline'd function) or currently unsupported by Cython, see

https://groups.google.com/d/topic/cython-users/UuQvpvdNhjc/discussion



---

archive/issue_events_176621.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2012-11-24T09:00:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13740",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13740#event-176621"
}
```



---

archive/issue_events_176622.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2012-11-24T09:04:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13740",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13740#event-176622"
}
```



---

archive/issue_events_176623.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2012-11-24T09:04:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13740",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13740#event-176623"
}
```



---

archive/issue_comments_164118.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nBoth the patch and the spkg look good to me.",
    "created_at": "2012-11-24T09:04:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13740",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13740#issuecomment-164118",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:3'>Comment 3:</a>
Both the patch and the spkg look good to me.



---

archive/issue_comments_164119.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nCython 0.17.3 is at #13832",
    "created_at": "2012-12-14T15:05:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13740",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13740#issuecomment-164119",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:4'>Comment 4:</a>
Cython 0.17.3 is at #13832



---

archive/issue_comments_164120.json:
```json
{
    "body": "Reviewer: **Robert Bradshaw**",
    "created_at": "2012-12-14T15:05:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13740",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13740#issuecomment-164120",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Robert Bradshaw**



---

archive/issue_comments_164121.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,3 +2,4 @@\n \n http://www.stp.dias.ie/~vbraun/Sage/spkg/cython-0.17.2.spkg\n \n+**Apply**: [attachment: trac_13740_final_fixes.patch.gz](https://github.com/sagemath/sage/files/ticket13740/trac_13740_final_fixes.patch.gz)\n``````\n",
    "created_at": "2012-12-16T14:34:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13740",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13740#issuecomment-164121",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,3 +2,4 @@
 
 http://www.stp.dias.ie/~vbraun/Sage/spkg/cython-0.17.2.spkg
 
+**Apply**: [attachment: trac_13740_final_fixes.patch.gz](https://github.com/sagemath/sage/files/ticket13740/trac_13740_final_fixes.patch.gz)
``````




---

archive/issue_comments_164122.json:
```json
{
    "body": "Merged: **sage-5.6.beta0**",
    "created_at": "2012-12-18T11:14:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13740",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13740#issuecomment-164122",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.6.beta0**



---

archive/issue_events_176624.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-12-18T11:14:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13740",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13740#event-176624"
}
```



---

archive/issue_events_176625.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-12-18T11:14:40Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13740",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13740#event-176625"
}
```
