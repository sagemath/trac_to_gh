# Issue 13363: Move sage/graphs/planarity/ to sage/graphs/planarity_c/

archive/issues_013191.json:
```json
{
    "assignees": [],
    "body": "When running doctests with #12659 applied, an \"ImportWarning: Not importing directory\" is raised due to there being a sage/graphs/planarity.so and sage/graphs/planarity directory.  This moves the C source code into a planarity_c directory.\n\nAssignee: **@jasongrout, @nathanncohen, @rlmill**\n\nReviewer: **John Palmieri**\n\nAuthor: **Mike Hansen**\n\nMerged: **sage-5.3.beta2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13363_\n\n",
    "closed_at": "2012-08-14T07:07:21Z",
    "created_at": "2012-08-13T21:21:33Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Move sage/graphs/planarity/ to sage/graphs/planarity_c/",
    "type": "issue",
    "updated_at": "2012-08-14T07:07:21Z",
    "url": "https://github.com/sagemath/sage/issues/13363",
    "user": "https://github.com/mwhansen"
}
```
When running doctests with #12659 applied, an "ImportWarning: Not importing directory" is raised due to there being a sage/graphs/planarity.so and sage/graphs/planarity directory.  This moves the C source code into a planarity_c directory.

Assignee: **@jasongrout, @nathanncohen, @rlmill**

Reviewer: **John Palmieri**

Author: **Mike Hansen**

Merged: **sage-5.3.beta2**

_Issue created by migration from https://trac.sagemath.org/ticket/13363_





---

archive/issue_events_170939.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2012-08-13T21:21:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13363",
    "milestone_number": null,
    "milestone_title": "sage-5.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13363#event-170939"
}
```



---

archive/issue_events_170940.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2012-08-13T21:21:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13363",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13363#event-170940"
}
```



---

archive/issue_events_170941.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2012-08-13T21:21:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13363",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13363#event-170941"
}
```



---

archive/issue_events_170942.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2012-08-13T21:21:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13363",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13363#event-170942"
}
```



---

archive/issue_comments_157450.json:
```json
{
    "body": "Attachment: **[trac_13363.patch.gz](https://github.com/sagemath/sage/files/ticket13363/trac_13363.patch.gz)**",
    "created_at": "2012-08-13T21:23:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13363",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13363#issuecomment-157450",
    "user": "https://github.com/mwhansen"
}
```

Attachment: **[trac_13363.patch.gz](https://github.com/sagemath/sage/files/ticket13363/trac_13363.patch.gz)**



---

archive/issue_events_170943.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2012-08-13T21:23:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13363",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13363#event-170943"
}
```



---

archive/issue_comments_157451.json:
```json
{
    "body": "Reviewer: **John Palmieri**",
    "created_at": "2012-08-14T03:30:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13363",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13363#issuecomment-157451",
    "user": "https://github.com/jhpalmieri"
}
```

Reviewer: **John Palmieri**



---

archive/issue_comments_157452.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nLooks good to me.",
    "created_at": "2012-08-14T03:30:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13363",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13363#issuecomment-157452",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:2" align="right">Comment 2</div>

Looks good to me.



---

archive/issue_events_170944.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-08-14T03:30:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13363",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13363#event-170944"
}
```



---

archive/issue_events_170945.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2012-08-14T03:30:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13363",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13363#event-170945"
}
```



---

archive/issue_comments_157453.json:
```json
{
    "body": "Merged: **sage-5.3.beta2**",
    "created_at": "2012-08-14T07:06:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13363",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13363#issuecomment-157453",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.3.beta2**



---

archive/issue_events_170946.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-08-14T07:07:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13363",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13363#event-170946"
}
```



---

archive/issue_events_170947.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-08-14T07:07:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13363",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13363#event-170947"
}
```
