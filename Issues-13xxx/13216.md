# Issue 13216: one test in lattice polytope should be tagged # random

archive/issues_013044.json:
```json
{
    "assignees": [],
    "body": "in the file sage/geometry/lattice_polytope.py\n\ninside the procedure\n\n```\ndef _read_nef_x_partitions(data):\n```\n\nthe line\n\n```\n        sage: print s\n```\n\nshould be replaced  by \n\n```\n        sage: print s  # random\n```\n\nbecause the result contains a timing result, which does not have a fix value. Inside the same file, three other similar tests do already have this tag.\n\nAssignee: **mhampton**\n\nKeywords: **random test, polytope**\n\nReviewer: **Volker Braun**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\nMerged: **sage-5.2.beta1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13216_\n\n",
    "closed_at": "2012-07-13T11:44:35Z",
    "created_at": "2012-07-09T09:10:10Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "one test in lattice polytope should be tagged # random",
    "type": "issue",
    "updated_at": "2012-07-13T11:44:35Z",
    "url": "https://github.com/sagemath/sage/issues/13216",
    "user": "https://github.com/fchapoton"
}
```
in the file sage/geometry/lattice_polytope.py

inside the procedure

```
def _read_nef_x_partitions(data):
```

the line

```
        sage: print s
```

should be replaced  by 

```
        sage: print s  # random
```

because the result contains a timing result, which does not have a fix value. Inside the same file, three other similar tests do already have this tag.

Assignee: **mhampton**

Keywords: **random test, polytope**

Reviewer: **Volker Braun**

Author: **Frédéric Chapoton**

Merged: **sage-5.2.beta1**

_Issue created by migration from https://trac.sagemath.org/ticket/13216_





---

archive/issue_events_168553.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2012-07-09T09:10:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13216",
    "milestone_number": null,
    "milestone_title": "sage-5.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13216#event-168553"
}
```



---

archive/issue_events_168554.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2012-07-09T09:10:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13216",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "component: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13216#event-168554"
}
```



---

archive/issue_events_168555.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2012-07-09T09:10:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13216",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13216#event-168555"
}
```



---

archive/issue_events_168556.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2012-07-09T09:10:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13216",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13216#event-168556"
}
```



---

archive/issue_comments_154618.json:
```json
{
    "body": "Changed work issues from **add the random keyword** to none",
    "created_at": "2012-07-09T09:18:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13216",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13216#issuecomment-154618",
    "user": "https://github.com/fchapoton"
}
```

Changed work issues from **add the random keyword** to none



---

archive/issue_comments_154619.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[trac_13216_random_test-fc.patch.gz](https://github.com/sagemath/sage/files/ticket13216/trac_13216_random_test-fc.patch.gz)**",
    "created_at": "2012-07-09T09:18:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13216",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13216#issuecomment-154619",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[trac_13216_random_test-fc.patch.gz](https://github.com/sagemath/sage/files/ticket13216/trac_13216_random_test-fc.patch.gz)**



---

archive/issue_events_168557.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2012-07-09T09:18:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13216",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13216#event-168557"
}
```



---

archive/issue_comments_154620.json:
```json
{
    "body": "Author: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2012-07-09T09:18:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13216",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13216#issuecomment-154620",
    "user": "https://github.com/fchapoton"
}
```

Author: **Frédéric Chapoton**



---

archive/issue_comments_154621.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nHow slow is your computer that this test took a measurable (rounded) number of seconds? ;-)\n\nPatch looks good to me!",
    "created_at": "2012-07-09T09:26:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13216",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13216#issuecomment-154621",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:2'>Comment 2:</a>
How slow is your computer that this test took a measurable (rounded) number of seconds? ;-)

Patch looks good to me!



---

archive/issue_comments_154622.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2012-07-09T09:26:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13216",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13216#issuecomment-154622",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_events_168558.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-07-09T09:26:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13216",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13216#event-168558"
}
```



---

archive/issue_events_168559.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-07-09T09:26:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13216",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13216#event-168559"
}
```



---

archive/issue_comments_154623.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThanks a lot. I had to do that because the patchbot protested on my own ticket #13077, having nothing to do with this one (see the failing test on 5.1.rc1 in http://patchbot.sagemath.org/ticket/13077/)",
    "created_at": "2012-07-09T09:43:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13216",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13216#issuecomment-154623",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:3'>Comment 3:</a>
Thanks a lot. I had to do that because the patchbot protested on my own ticket #13077, having nothing to do with this one (see the failing test on 5.1.rc1 in http://patchbot.sagemath.org/ticket/13077/)



---

archive/issue_events_168560.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-09T20:37:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13216",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13216#event-168560"
}
```



---

archive/issue_events_168561.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-09T20:37:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13216",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13216#event-168561"
}
```



---

archive/issue_comments_154624.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nLong overdue.",
    "created_at": "2012-07-09T20:37:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13216",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13216#issuecomment-154624",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:4'>Comment 4:</a>
Long overdue.



---

archive/issue_events_168562.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-13T11:44:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13216",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13216#event-168562"
}
```



---

archive/issue_events_168563.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-07-13T11:44:35Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13216",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13216#event-168563"
}
```



---

archive/issue_comments_154625.json:
```json
{
    "body": "Merged: **sage-5.2.beta1**",
    "created_at": "2012-07-13T11:44:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13216",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13216#issuecomment-154625",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.2.beta1**
