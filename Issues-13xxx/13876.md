# Issue 13876: Singular and SAGE_DEBUG fixes

archive/issues_013672.json:
```json
{
    "assignees": [],
    "body": "Use `SAGE_DEBUG` instead of the new and undocumented environment variable `SINGULAR_XALLOC`.\n\nAlso, building the Sage library with `SAGE_DEBUG=yes` is broken. This is because distutils then does not define `NDEBUG` which turns on broken Singular debugging stuff.\n\nUpdated spkg is at [http://boxen.math.washington.edu/home/jdemeyer/spkg/singular-3-1-5.p3.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/singular-3-1-5.p3.spkg)\n\nReviewer: **Simon King**\n\nAuthor: **Volker Braun**\n\nMerged: **sage-5.7.beta0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13876_\n\n",
    "closed_at": "2013-01-23T07:47:47Z",
    "created_at": "2012-12-28T11:36:46Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Singular and SAGE_DEBUG fixes",
    "type": "issue",
    "updated_at": "2013-01-30T07:40:53Z",
    "url": "https://github.com/sagemath/sage/issues/13876",
    "user": "https://github.com/vbraun"
}
```
Use `SAGE_DEBUG` instead of the new and undocumented environment variable `SINGULAR_XALLOC`.

Also, building the Sage library with `SAGE_DEBUG=yes` is broken. This is because distutils then does not define `NDEBUG` which turns on broken Singular debugging stuff.

Updated spkg is at [http://boxen.math.washington.edu/home/jdemeyer/spkg/singular-3-1-5.p3.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/singular-3-1-5.p3.spkg)

Reviewer: **Simon King**

Author: **Volker Braun**

Merged: **sage-5.7.beta0**

_Issue created by migration from https://trac.sagemath.org/ticket/13876_





---

archive/issue_events_172347.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-12-28T11:36:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13876",
    "milestone_number": null,
    "milestone_title": "sage-5.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13876#event-172347"
}
```



---

archive/issue_events_172348.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-12-28T11:36:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13876",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13876#event-172348"
}
```



---

archive/issue_events_172349.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-12-28T11:36:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13876",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13876#event-172349"
}
```



---

archive/issue_events_172350.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-12-28T11:36:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13876",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13876#event-172350"
}
```



---

archive/issue_comments_166272.json:
```json
{
    "body": "diff for review only",
    "created_at": "2012-12-28T12:38:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13876",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13876#issuecomment-166272",
    "user": "https://github.com/vbraun"
}
```

diff for review only



---

archive/issue_comments_166273.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,5 @@\n Use `SAGE_DEBUG` instead of the new and undocumented environment variable `SINGULAR_XALLOC`.\n \n Also, building the Sage library with `SAGE_DEBUG=yes` is broken. This is because distutils then does not define `NDEBUG` which turns on broken Singular debugging stuff.\n+\n+Updated spkg is at http://www.stp.dias.ie/~vbraun/Sage/spkg/singular-3-1-5.p3.spkg\n``````\n",
    "created_at": "2012-12-28T12:38:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13876",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13876#issuecomment-166273",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,5 @@
 Use `SAGE_DEBUG` instead of the new and undocumented environment variable `SINGULAR_XALLOC`.
 
 Also, building the Sage library with `SAGE_DEBUG=yes` is broken. This is because distutils then does not define `NDEBUG` which turns on broken Singular debugging stuff.
+
+Updated spkg is at http://www.stp.dias.ie/~vbraun/Sage/spkg/singular-3-1-5.p3.spkg
``````




---

archive/issue_comments_166274.json:
```json
{
    "body": "Author: **Volker Braun**",
    "created_at": "2012-12-28T12:38:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13876",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13876#issuecomment-166274",
    "user": "https://github.com/vbraun"
}
```

Author: **Volker Braun**



---

archive/issue_events_172351.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2012-12-28T12:38:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13876",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13876#event-172351"
}
```



---

archive/issue_comments_166275.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[singular-p3.diff.gz](https://github.com/sagemath/sage/files/ticket13876/singular-p3.diff.gz)**",
    "created_at": "2012-12-28T12:38:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13876",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13876#issuecomment-166275",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[singular-p3.diff.gz](https://github.com/sagemath/sage/files/ticket13876/singular-p3.diff.gz)**



---

archive/issue_comments_166276.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nIn `SPKG.txt`, `SINGULAR_DEBUG` should be `SAGE_DEBUG`.",
    "created_at": "2012-12-30T08:45:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13876",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13876#issuecomment-166276",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'>Comment 2:</a>
In `SPKG.txt`, `SINGULAR_DEBUG` should be `SAGE_DEBUG`.



---

archive/issue_comments_166277.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nFixed, spkg at same location.",
    "created_at": "2012-12-30T10:23:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13876",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13876#issuecomment-166277",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:3'>Comment 3:</a>
Fixed, spkg at same location.



---

archive/issue_comments_166278.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nSorry, I forgot to review it.\n\nAll tests pass, both with and without SAGE_DEBUG=yes, the spkg's repository is clean, the changes are documented in SPKG.txt. Hence, I can give it a positive review.",
    "created_at": "2013-01-21T11:21:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13876",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13876#issuecomment-166278",
    "user": "https://github.com/simon-king-jena"
}
```

<a id='comment:4'>Comment 4:</a>
Sorry, I forgot to review it.

All tests pass, both with and without SAGE_DEBUG=yes, the spkg's repository is clean, the changes are documented in SPKG.txt. Hence, I can give it a positive review.



---

archive/issue_comments_166279.json:
```json
{
    "body": "Reviewer: **Simon King**",
    "created_at": "2013-01-21T11:21:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13876",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13876#issuecomment-166279",
    "user": "https://github.com/simon-king-jena"
}
```

Reviewer: **Simon King**



---

archive/issue_events_172352.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2013-01-21T11:21:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13876",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13876#event-172352"
}
```



---

archive/issue_events_172353.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2013-01-21T11:21:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13876",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13876#event-172353"
}
```



---

archive/issue_events_172354.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-21T14:51:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13876",
    "milestone_number": null,
    "milestone_title": "sage-5.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13876#event-172354"
}
```



---

archive/issue_events_172355.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-21T14:51:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13876",
    "milestone_number": null,
    "milestone_title": "sage-5.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13876#event-172355"
}
```



---

archive/issue_events_172356.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-23T07:47:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13876",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13876#event-172356"
}
```



---

archive/issue_events_172357.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-23T07:47:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13876",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13876#event-172357"
}
```



---

archive/issue_comments_166280.json:
```json
{
    "body": "Merged: **sage-5.7.beta0**",
    "created_at": "2013-01-23T07:47:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13876",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13876#issuecomment-166280",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.7.beta0**



---

archive/issue_comments_166281.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,4 +2,4 @@\n \n Also, building the Sage library with `SAGE_DEBUG=yes` is broken. This is because distutils then does not define `NDEBUG` which turns on broken Singular debugging stuff.\n \n-Updated spkg is at http://www.stp.dias.ie/~vbraun/Sage/spkg/singular-3-1-5.p3.spkg\n+Updated spkg is at [http://boxen.math.washington.edu/home/jdemeyer/spkg/singular-3-1-5.p3.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/singular-3-1-5.p3.spkg)\n``````\n",
    "created_at": "2013-01-30T07:40:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13876",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13876#issuecomment-166281",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,4 +2,4 @@
 
 Also, building the Sage library with `SAGE_DEBUG=yes` is broken. This is because distutils then does not define `NDEBUG` which turns on broken Singular debugging stuff.
 
-Updated spkg is at http://www.stp.dias.ie/~vbraun/Sage/spkg/singular-3-1-5.p3.spkg
+Updated spkg is at [http://boxen.math.washington.edu/home/jdemeyer/spkg/singular-3-1-5.p3.spkg](http://boxen.math.washington.edu/home/jdemeyer/spkg/singular-3-1-5.p3.spkg)
``````

