# Issue 13462: quaternion algebra discriminant generalized to number fields

archive/issues_013290.json:
```json
{
    "assignees": [],
    "body": "I've implemented the hilbert_conductor for elements of number fields and the discriminant for quaternion algebras over number fields.\n\nAssignee: **@williamstein**\n\nCC:  @sagetrac-schisholm\n\nKeywords: **quaternion, discriminant, hilbert conductor**\n\nReviewer: **Sarah Chisholm**\n\nAuthor: **Aly Deines**\n\nMerged: **sage-5.4.beta2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13462_\n\n",
    "closed_at": "2012-09-26T07:47:02Z",
    "created_at": "2012-09-13T22:50:54Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20number%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "quaternion algebra discriminant generalized to number fields",
    "type": "issue",
    "updated_at": "2012-09-26T07:47:02Z",
    "url": "https://github.com/sagemath/sage/issues/13462",
    "user": "https://github.com/adeines"
}
```
I've implemented the hilbert_conductor for elements of number fields and the discriminant for quaternion algebras over number fields.

Assignee: **@williamstein**

CC:  @sagetrac-schisholm

Keywords: **quaternion, discriminant, hilbert conductor**

Reviewer: **Sarah Chisholm**

Author: **Aly Deines**

Merged: **sage-5.4.beta2**

_Issue created by migration from https://trac.sagemath.org/ticket/13462_





---

archive/issue_events_172511.json:
```json
{
    "actor": "https://github.com/adeines",
    "created_at": "2012-09-13T22:50:54Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13462",
    "milestone_number": null,
    "milestone_title": "sage-5.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13462#event-172511"
}
```



---

archive/issue_events_172512.json:
```json
{
    "actor": "https://github.com/adeines",
    "created_at": "2012-09-13T22:50:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13462",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20theory",
    "label_color": "0000ff",
    "label_name": "component: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13462#event-172512"
}
```



---

archive/issue_events_172513.json:
```json
{
    "actor": "https://github.com/adeines",
    "created_at": "2012-09-13T22:50:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13462",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13462#event-172513"
}
```



---

archive/issue_events_172514.json:
```json
{
    "actor": "https://github.com/adeines",
    "created_at": "2012-09-13T22:50:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13462",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13462#event-172514"
}
```



---

archive/issue_comments_159697.json:
```json
{
    "body": "Attachment: **[Trac_13463_discriminant.patch.gz](https://github.com/sagemath/sage/files/ticket13462/Trac_13463_discriminant.patch.gz)**",
    "created_at": "2012-09-13T22:56:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13462",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13462#issuecomment-159697",
    "user": "https://github.com/adeines"
}
```

Attachment: **[Trac_13463_discriminant.patch.gz](https://github.com/sagemath/sage/files/ticket13462/Trac_13463_discriminant.patch.gz)**



---

archive/issue_events_172515.json:
```json
{
    "actor": "https://github.com/adeines",
    "created_at": "2012-09-20T16:50:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13462",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13462#event-172515"
}
```



---

archive/issue_comments_159698.json:
```json
{
    "body": "Author: **Aly Deines**",
    "created_at": "2012-09-20T16:50:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13462",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13462#issuecomment-159698",
    "user": "https://github.com/adeines"
}
```

Author: **Aly Deines**



---

archive/issue_events_172516.json:
```json
{
    "actor": "https://github.com/sagetrac-schisholm",
    "created_at": "2012-09-21T18:16:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13462",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13462#event-172516"
}
```



---

archive/issue_events_172517.json:
```json
{
    "actor": "https://github.com/sagetrac-schisholm",
    "created_at": "2012-09-21T18:16:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13462",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13462#event-172517"
}
```



---

archive/issue_comments_159699.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nLooks great!",
    "created_at": "2012-09-21T18:16:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13462",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13462#issuecomment-159699",
    "user": "https://github.com/sagetrac-schisholm"
}
```

<div id="comment:2" align="right">Comment 2</div>

Looks great!



---

archive/issue_comments_159700.json:
```json
{
    "body": "Reviewer: **Sarah Chisholm**",
    "created_at": "2012-09-21T18:16:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13462",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13462#issuecomment-159700",
    "user": "https://github.com/sagetrac-schisholm"
}
```

Reviewer: **Sarah Chisholm**



---

archive/issue_comments_159701.json:
```json
{
    "body": "Merged: **sage-5.4.beta2**",
    "created_at": "2012-09-26T07:47:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13462",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13462#issuecomment-159701",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.4.beta2**



---

archive/issue_events_172518.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-09-26T07:47:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13462",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13462#event-172518"
}
```



---

archive/issue_events_172519.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-09-26T07:47:02Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13462",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13462#event-172519"
}
```
