# Issue 13747: Change default behaviour of Poset to facade = True

archive/issues_013543.json:
```json
{
    "body": "As mentionned in the following discussions :\n* http://groups.google.com/forum/#!msg/sage-devel/qQzzZEh-JRo/WxU_qyxH2igJ\n* http://groups.google.com/d/topic/sage-combinat-devel/bfWbG54sedM/discussion (sage-combinat fork of the discussion)\n* https://groups.google.com/d/topic/sage-devel/xQxQLkm-RUU/discussion\n\nThis patch changes the default behaviour of the Poset constructor to facade = True, so that the elements that you define the Poset with are the elements it contains.\n\nTwo comments about this ticket :\n* While the documentation of the poset/ files say that `facade` is a boolean, it is not. Its former default value is actually `None`, and the code itself tests several times `if facade is None`. No idea why. All I know is that I first tried to make it really boolean, and to replace those `None` tests by `if not facade`, but I was not able to make all tests pass without any idea why. I added a warning about this in the file's header, and the doctests based on the former versions of Posets are now defined with a `facade = None` flag. Even though it should be a boolean. Well, if this wasn't a problem before, no reason why it should become one now. This clearly has to be fixed too.\n* There is a `Poset.linear_extensions` method whose default behaviour is also `facade = None`. Despite taking an optional flag, this method *DOES NOT WORK* as it is clearly indicated in its docstring. One can easily wonder what the hell this function is doing in Sage's code. Hence this function's behaviour is left untouched. It will use `facade=None` by default until it is rewritten correctly, or removed.\n\nNathann\n\nApply:\n* [attachment:trac_13747.patch]\n* [attachment:trac_13747-doctests.patch]\n* [attachment:trac_13747_reviewer.patch]\n* [attachment:trac_13747-rebase-11763.patch]\n* [attachment:trac_13747-rebase-12930.patch]\n\nAssignee: sage-combinat\n\nCC:  @nthiery fhivert @dimpase\n\nReviewer: Christian Kuper\n\nAuthor: Nathann Cohen\n\nMerged: sage-5.6.beta2\n\nDependencies: #11763, #12930\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/13747\n\n",
    "closed_at": "2012-12-29T19:31:29Z",
    "created_at": "2012-11-24T02:21:51Z",
    "labels": [
        "component: combinatorics",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-5.6",
    "title": "Change default behaviour of Poset to facade = True",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/13747",
    "user": "https://github.com/nathanncohen"
}
```
As mentionned in the following discussions :
* http://groups.google.com/forum/#!msg/sage-devel/qQzzZEh-JRo/WxU_qyxH2igJ
* http://groups.google.com/d/topic/sage-combinat-devel/bfWbG54sedM/discussion (sage-combinat fork of the discussion)
* https://groups.google.com/d/topic/sage-devel/xQxQLkm-RUU/discussion

This patch changes the default behaviour of the Poset constructor to facade = True, so that the elements that you define the Poset with are the elements it contains.

Two comments about this ticket :
* While the documentation of the poset/ files say that `facade` is a boolean, it is not. Its former default value is actually `None`, and the code itself tests several times `if facade is None`. No idea why. All I know is that I first tried to make it really boolean, and to replace those `None` tests by `if not facade`, but I was not able to make all tests pass without any idea why. I added a warning about this in the file's header, and the doctests based on the former versions of Posets are now defined with a `facade = None` flag. Even though it should be a boolean. Well, if this wasn't a problem before, no reason why it should become one now. This clearly has to be fixed too.
* There is a `Poset.linear_extensions` method whose default behaviour is also `facade = None`. Despite taking an optional flag, this method *DOES NOT WORK* as it is clearly indicated in its docstring. One can easily wonder what the hell this function is doing in Sage's code. Hence this function's behaviour is left untouched. It will use `facade=None` by default until it is rewritten correctly, or removed.

Nathann

Apply:
* [attachment:trac_13747.patch]
* [attachment:trac_13747-doctests.patch]
* [attachment:trac_13747_reviewer.patch]
* [attachment:trac_13747-rebase-11763.patch]
* [attachment:trac_13747-rebase-12930.patch]

Assignee: sage-combinat

CC:  @nthiery fhivert @dimpase

Reviewer: Christian Kuper

Author: Nathann Cohen

Merged: sage-5.6.beta2

Dependencies: #11763, #12930

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/13747





---

archive/issue_comments_208577.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2012-11-24T02:32:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208577",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_208578.json:
```json
{
    "body": "<a id='comment:2'></a>facade=None just means that the facade option is *not* specified at this stage. The default value will be decided at a lower level, or from the context.\n\nPlease edit the description to something more constructive (that you don't know the rationale for something doesn't mean that there is no rationale in the first place and that you can insult at will those who wrote it). Once done, you can erase this comment too.",
    "created_at": "2012-11-24T10:11:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208578",
    "user": "https://github.com/nthiery"
}
```

<a id='comment:2'></a>facade=None just means that the facade option is *not* specified at this stage. The default value will be decided at a lower level, or from the context.

Please edit the description to something more constructive (that you don't know the rationale for something doesn't mean that there is no rationale in the first place and that you can insult at will those who wrote it). Once done, you can erase this comment too.



---

archive/issue_comments_208579.json:
```json
{
    "body": "<a id='comment:3'></a>Replying to [nthiery](#comment%3A2):\n> Please edit the description to something more constructive (that you don't know the rationale for something doesn't mean that there is no rationale in the first place and that you can insult at will those who wrote it). Once done, you can erase this comment too.\n\n\nHey Nicolas, when you DO WRITE in the documentation that a thing is binary, make it binary. Else, you HAVE TO explain what None means. What you have in your head when you look at code stays in your head. What am I guilty of in this case ? Not guessing what you intended ? There are PROBLEMS with the combinat documentation, and each time a stupid guy like me is wasting his week-end and going to sleep at 4am in order to fix your bugs and write your documentation (and bear in mind that it is precisely what is happening right now), this mess is improving a bit. But there is no reason why I should be expected to know what should have been written in the file all along.\n\nNathann",
    "created_at": "2012-11-24T10:19:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208579",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:3'></a>Replying to [nthiery](#comment%3A2):
> Please edit the description to something more constructive (that you don't know the rationale for something doesn't mean that there is no rationale in the first place and that you can insult at will those who wrote it). Once done, you can erase this comment too.


Hey Nicolas, when you DO WRITE in the documentation that a thing is binary, make it binary. Else, you HAVE TO explain what None means. What you have in your head when you look at code stays in your head. What am I guilty of in this case ? Not guessing what you intended ? There are PROBLEMS with the combinat documentation, and each time a stupid guy like me is wasting his week-end and going to sleep at 4am in order to fix your bugs and write your documentation (and bear in mind that it is precisely what is happening right now), this mess is improving a bit. But there is no reason why I should be expected to know what should have been written in the file all along.

Nathann



---

archive/issue_comments_208580.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2012-11-24T10:32:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208580",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_208581.json:
```json
{
    "body": "<a id='comment:5'></a>Still editing the doc. I am about to try to replace some `facade = None` by `facade = False` which I tried earlier and failed to do, but I was wondering if you thought having `facade = None` in this file still made sense ? I mean, having undefined parameters like that is something you do when it is the default, but it feels weird to see a user explicitely define `facade` as `None`. Tell me what you think ! I'll continue editing the file, add some doc, stuff like that.\n\nNathann",
    "created_at": "2012-11-24T10:52:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208581",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:5'></a>Still editing the doc. I am about to try to replace some `facade = None` by `facade = False` which I tried earlier and failed to do, but I was wondering if you thought having `facade = None` in this file still made sense ? I mean, having undefined parameters like that is something you do when it is the default, but it feels weird to see a user explicitely define `facade` as `None`. Tell me what you think ! I'll continue editing the file, add some doc, stuff like that.

Nathann



---

archive/issue_comments_208582.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2012-11-24T11:48:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208582",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_208583.json:
```json
{
    "body": "<a id='comment:6'></a>Patch updated ! With a brand new doc for posets. I fixed the doc (the functions that have an optional `facade` argument should at the very least say which values it can take, especially if the examples use it). I removed the warning at the top because you know what you are doing, ad in the end all went well with `facade = False` in the examples. In my former attempts I had modified stuff in the constructors (replacing `facade is None` by  `not facade`) when I thought it would make no difference, but that was a mistake).\n\nI hope that this class is better now than it was before.\n\nCheers !\n\nNathann",
    "created_at": "2012-11-24T11:48:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208583",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:6'></a>Patch updated ! With a brand new doc for posets. I fixed the doc (the functions that have an optional `facade` argument should at the very least say which values it can take, especially if the examples use it). I removed the warning at the top because you know what you are doing, ad in the end all went well with `facade = False` in the examples. In my former attempts I had modified stuff in the constructors (replacing `facade is None` by  `not facade`) when I thought it would make no difference, but that was a mistake).

I hope that this class is better now than it was before.

Cheers !

Nathann



---

archive/issue_comments_208584.json:
```json
{
    "body": "<a id='comment:7'></a>By the way, I just took another look at the documentation of Posets, and I wondered whether something may be added in the file's header to say that `facade = None` is not the default anymore... What do you think ? I removed the warning that I added before, so there's an empty space for a warning to be filled `:-P`\n\nNathann",
    "created_at": "2012-11-24T12:14:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208584",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:7'></a>By the way, I just took another look at the documentation of Posets, and I wondered whether something may be added in the file's header to say that `facade = None` is not the default anymore... What do you think ? I removed the warning that I added before, so there's an empty space for a warning to be filled `:-P`

Nathann



---

archive/issue_comments_208585.json:
```json
{
    "body": "<a id='comment:9'></a>Hello Nathann,\n\na few remarks from my side:\n\n* Yes, the doc is better :-)\n\n* I still don't feel very comfortable with the following facade doc: \" the `facade` variable will be set to `True` or `False` by a lower-level call, or depending on the context.\" Depending on what context? I believe the doc should clearly state what influence a certain parameter has.\n\n* In the examples you sometimes changed the former default facade value (i.e. facade = None) to facade = False, in other examples you left the coding as it was so the examples work with the new defualt value facade = True. (See 1st and 2nd example of Posets(...)). Not sure whether this was done on purpose of forgotten\n\n* I think there is one \"`\" to much in the Output doc of the constructor (class FinitePoset)\n\nCheers\n\nChristian",
    "created_at": "2012-11-28T05:02:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208585",
    "user": "https://trac.sagemath.org/admin/accounts/users/christiankuper"
}
```

<a id='comment:9'></a>Hello Nathann,

a few remarks from my side:

* Yes, the doc is better :-)

* I still don't feel very comfortable with the following facade doc: " the `facade` variable will be set to `True` or `False` by a lower-level call, or depending on the context." Depending on what context? I believe the doc should clearly state what influence a certain parameter has.

* In the examples you sometimes changed the former default facade value (i.e. facade = None) to facade = False, in other examples you left the coding as it was so the examples work with the new defualt value facade = True. (See 1st and 2nd example of Posets(...)). Not sure whether this was done on purpose of forgotten

* I think there is one "`" to much in the Output doc of the constructor (class FinitePoset)

Cheers

Christian



---

archive/issue_comments_208586.json:
```json
{
    "body": "<a id='comment:10'></a>Hellooooooo Christian !!!\n\n> a few remarks from my side:\n> \n> * Yes, the doc is better :-)\n\n\nCool `:-D`\n\n>  * I still don't feel very comfortable with the following facade doc: \" the `facade` variable will be set to `True` or `False` by a lower-level call, or depending on the context.\" Depending on what context? I believe the doc should clearly state what influence a certain parameter has.\n\n\nAhem. I believe so, too... But from the comments on this ticket you can see that I just said there what Nicolas gave me to chew. What this ticket initially contained is a warning like \"There are three values for a boolean instead of three, the third is not well defined and should not be trusted -- it should be removed eventually or documented\". But well, Nicolasgave me that explanation and honestly I do not mind much leaving it like that for the moment -- it is still much better than before and it will take manyyyyyyyyyyyyyyyy patches before this file is cleaned. Besides, that's not a \"local\" problem. Facades are used in many places and I do not expect their documentation to be very different.\n\n>  * In the examples you sometimes changed the former default facade value (i.e. facade = None) to facade = False, in other examples you left the coding as it was so the examples work with the new defualt value facade = True. (See 1st and 2nd example of Posets(...)). Not sure whether this \n \nwas done on purpose of forgotten\n\nOh. Yeah, easy. Many explanations usually : as the new default is `facade = True`, I felt like this should be heavily tested in the docstrings. So when a docstring did not fail after that change, I left it like that. If it worked by changing it to `facade = False`, I was happy. If that did not work either I changed it to `facade = None` which was the former default.\n\nNot that on some other occasions the documentation says \"let us us `facade = True` in the following example\", and in those cases (because is was explicitely said) I let `facade = True` stay, even though it is now the default too.\n\n>  * I think there is one \"`\" to much in the Output doc of the constructor (class FinitePoset)\n\n\nOh ? Noooooooooo ! Actually, what I want to write is \"whether the Poset's elements\", but I wrapped Poset inside of a `:meth:` tag... But it appears in the documentation as \"whether the Poset's elements\" as it should, and Sphinx compiles without warning `:-)`\n\nNathann",
    "created_at": "2012-11-28T10:37:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208586",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:10'></a>Hellooooooo Christian !!!

> a few remarks from my side:
> 
> * Yes, the doc is better :-)


Cool `:-D`

>  * I still don't feel very comfortable with the following facade doc: " the `facade` variable will be set to `True` or `False` by a lower-level call, or depending on the context." Depending on what context? I believe the doc should clearly state what influence a certain parameter has.


Ahem. I believe so, too... But from the comments on this ticket you can see that I just said there what Nicolas gave me to chew. What this ticket initially contained is a warning like "There are three values for a boolean instead of three, the third is not well defined and should not be trusted -- it should be removed eventually or documented". But well, Nicolasgave me that explanation and honestly I do not mind much leaving it like that for the moment -- it is still much better than before and it will take manyyyyyyyyyyyyyyyy patches before this file is cleaned. Besides, that's not a "local" problem. Facades are used in many places and I do not expect their documentation to be very different.

>  * In the examples you sometimes changed the former default facade value (i.e. facade = None) to facade = False, in other examples you left the coding as it was so the examples work with the new defualt value facade = True. (See 1st and 2nd example of Posets(...)). Not sure whether this 
 
was done on purpose of forgotten

Oh. Yeah, easy. Many explanations usually : as the new default is `facade = True`, I felt like this should be heavily tested in the docstrings. So when a docstring did not fail after that change, I left it like that. If it worked by changing it to `facade = False`, I was happy. If that did not work either I changed it to `facade = None` which was the former default.

Not that on some other occasions the documentation says "let us us `facade = True` in the following example", and in those cases (because is was explicitely said) I let `facade = True` stay, even though it is now the default too.

>  * I think there is one "`" to much in the Output doc of the constructor (class FinitePoset)


Oh ? Noooooooooo ! Actually, what I want to write is "whether the Poset's elements", but I wrapped Poset inside of a `:meth:` tag... But it appears in the documentation as "whether the Poset's elements" as it should, and Sphinx compiles without warning `:-)`

Nathann



---

archive/issue_comments_208587.json:
```json
{
    "body": "<a id='comment:11'></a>(just had a chat with Nicolas Thiery, who told me that `None` should behave like the default input, and so that the default input should stay `None`, and so that the meaning of `None` should be `facade = True` when there is a doubt.)\n\nPatch updated. Still waiting for a review !\n\nNathann",
    "created_at": "2012-11-29T13:06:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208587",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:11'></a>(just had a chat with Nicolas Thiery, who told me that `None` should behave like the default input, and so that the default input should stay `None`, and so that the meaning of `None` should be `facade = True` when there is a doubt.)

Patch updated. Still waiting for a review !

Nathann



---

archive/issue_comments_208588.json:
```json
{
    "body": "<a id='comment:12'></a>Hello Nathann,\n\nI only had the time to run a few tests up to now:\n\n* Applying the patch: No issues\n* \"Short\" doctest: All passed\n* \"Long\" doctest: Produces a number of failures. At least at a first glance some of them look related to the module you modified.\n\nThese are the failures:\n\n* sage -t -long devel/sage-test/sage/categories/finite_lattice_posets.py # 1 doctests failed\n* sage -t -long devel/sage-test/sage/categories/posets.py # 1 doctests failed\n* sage -t -long devel/sage-test/sage/categories/finite_posets.py # 9 doctests failed\n* sage -t -long devel/sage-test/sage/categories/coxeter_groups.py # 1 doctests failed\n* sage -t -long devel/sage-test/sage/schemes/generic/algebraic_scheme.py # 69 doctests failed\n* sage -t -long devel/sage-test/sage/schemes/toric/homset.py # 32 doctests failed\n* sage -t -long devel/sage-test/sage/schemes/toric/divisor_class.pyx # 50 doctests failed\n* sage -t -long devel/sage-test/sage/schemes/toric/morphism.py # 40 doctests failed\n* sage -t -long devel/sage-test/sage/schemes/toric/library.py # 78 doctests failed\n* sage -t -long devel/sage-test/sage/schemes/toric/chow_group.py # 195 doctests failed\n* sage -t -long devel/sage-test/sage/schemes/toric/variety.py # 199 doctests failed\n* sage -t -long devel/sage-test/sage/schemes/toric/divisor.py # 260 doctests failed\n* sage -t -long devel/sage-test/sage/schemes/toric/fano_variety.py # 9 doctests failed\n* sage -t -long devel/sage-test/sage/geometry/fan_isomorphism.py # 14 doctests failed\n* sage -t -long devel/sage-test/sage/geometry/fan_morphism.py # 136 doctests failed\n* sage -t -long devel/sage-test/sage/geometry/toric_plotter.py # 17 doctests failed\n* sage -t -long devel/sage-test/sage/geometry/fan.py # 130 doctests failed\n* sage -t -long devel/sage-test/sage/geometry/triangulation/element.py # 1 doctests failed\n* sage -t -long devel/sage-test/sage/geometry/polyhedron/representation.py # 6 doctests failed\n* sage -t -long devel/sage-test/sage/geometry/polyhedron/base.py # 26 doctests failed\n* sage -t -long devel/sage-test/sage/geometry/cone.py # 93 doctests fail\n\nChristian",
    "created_at": "2012-12-02T09:51:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208588",
    "user": "https://trac.sagemath.org/admin/accounts/users/christiankuper"
}
```

<a id='comment:12'></a>Hello Nathann,

I only had the time to run a few tests up to now:

* Applying the patch: No issues
* "Short" doctest: All passed
* "Long" doctest: Produces a number of failures. At least at a first glance some of them look related to the module you modified.

These are the failures:

* sage -t -long devel/sage-test/sage/categories/finite_lattice_posets.py # 1 doctests failed
* sage -t -long devel/sage-test/sage/categories/posets.py # 1 doctests failed
* sage -t -long devel/sage-test/sage/categories/finite_posets.py # 9 doctests failed
* sage -t -long devel/sage-test/sage/categories/coxeter_groups.py # 1 doctests failed
* sage -t -long devel/sage-test/sage/schemes/generic/algebraic_scheme.py # 69 doctests failed
* sage -t -long devel/sage-test/sage/schemes/toric/homset.py # 32 doctests failed
* sage -t -long devel/sage-test/sage/schemes/toric/divisor_class.pyx # 50 doctests failed
* sage -t -long devel/sage-test/sage/schemes/toric/morphism.py # 40 doctests failed
* sage -t -long devel/sage-test/sage/schemes/toric/library.py # 78 doctests failed
* sage -t -long devel/sage-test/sage/schemes/toric/chow_group.py # 195 doctests failed
* sage -t -long devel/sage-test/sage/schemes/toric/variety.py # 199 doctests failed
* sage -t -long devel/sage-test/sage/schemes/toric/divisor.py # 260 doctests failed
* sage -t -long devel/sage-test/sage/schemes/toric/fano_variety.py # 9 doctests failed
* sage -t -long devel/sage-test/sage/geometry/fan_isomorphism.py # 14 doctests failed
* sage -t -long devel/sage-test/sage/geometry/fan_morphism.py # 136 doctests failed
* sage -t -long devel/sage-test/sage/geometry/toric_plotter.py # 17 doctests failed
* sage -t -long devel/sage-test/sage/geometry/fan.py # 130 doctests failed
* sage -t -long devel/sage-test/sage/geometry/triangulation/element.py # 1 doctests failed
* sage -t -long devel/sage-test/sage/geometry/polyhedron/representation.py # 6 doctests failed
* sage -t -long devel/sage-test/sage/geometry/polyhedron/base.py # 26 doctests failed
* sage -t -long devel/sage-test/sage/geometry/cone.py # 93 doctests fail

Christian



---

archive/issue_comments_208589.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Christian Kuper\"",
    "created_at": "2012-12-02T09:51:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208589",
    "user": "https://trac.sagemath.org/admin/accounts/users/christiankuper"
}
```

Changing reviewer from "" to "Christian Kuper"



---

archive/issue_comments_208590.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2012-12-02T09:51:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208590",
    "user": "https://trac.sagemath.org/admin/accounts/users/christiankuper"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_208591.json:
```json
{
    "body": "<a id='comment:13'></a>`>_<`\n\nThank you so much for telling me... God. I did not think tht it would be that bad `:-/`\n\nYou will get it today.\n\nThanks again.\n\nNathann",
    "created_at": "2012-12-02T09:54:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208591",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:13'></a>`>_<`

Thank you so much for telling me... God. I did not think tht it would be that bad `:-/`

You will get it today.

Thanks again.

Nathann



---

archive/issue_comments_208592.json:
```json
{
    "body": "Changing status from needs_work to needs_info.",
    "created_at": "2012-12-02T12:56:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208592",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from needs_work to needs_info.



---

archive/issue_comments_208593.json:
```json
{
    "body": "<a id='comment:14'></a>Looks like I finally found my way through this chaos ! `:-D`\n\nThe only problems actually came from a couple of files, and everything was solved once this was fixed. I still have a problem with a couple of doctests in geometry/cone.py, and ask for some help on sage-devel.\n\nhttps://groups.google.com/d/topic/sage-devel/Bbs_65wxKY8/discussion\n\nIf it turns out to be harmless, I will just change the doctests and this patch will be ready for a review `:-)`\n\nNathann",
    "created_at": "2012-12-02T12:56:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208593",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:14'></a>Looks like I finally found my way through this chaos ! `:-D`

The only problems actually came from a couple of files, and everything was solved once this was fixed. I still have a problem with a couple of doctests in geometry/cone.py, and ask for some help on sage-devel.

https://groups.google.com/d/topic/sage-devel/Bbs_65wxKY8/discussion

If it turns out to be harmless, I will just change the doctests and this patch will be ready for a review `:-)`

Nathann



---

archive/issue_comments_208594.json:
```json
{
    "body": "<a id='comment:15'></a>Apply trac_13747.patch, trac_13747-doctests.patch",
    "created_at": "2012-12-02T12:58:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208594",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:15'></a>Apply trac_13747.patch, trac_13747-doctests.patch



---

archive/issue_comments_208595.json:
```json
{
    "body": "<a id='comment:16'></a>Solved ! And easily ! Thanks to Volker ! `:-)`\n\nNathann",
    "created_at": "2012-12-02T13:47:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208595",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:16'></a>Solved ! And easily ! Thanks to Volker ! `:-)`

Nathann



---

archive/issue_comments_208596.json:
```json
{
    "body": "Changing status from needs_info to needs_review.",
    "created_at": "2012-12-02T13:47:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208596",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from needs_info to needs_review.



---

archive/issue_comments_208597.json:
```json
{
    "body": "<a id='comment:17'></a>Hello Nathann,\n\nas far as I can judge there are no more issues with this patch:\n\n* All test passed successfully\n* The doc look ok\n* The methods are behaving as promised in the doc (default behavior with facade = True etc)\n\nBTW, the explanation for the facade = True / False / None bevavior is much clearer than in your first version ;-)\n\nJust be clear about the extension of my review: I focused ONLY on the changes you made. IMHO there is still room for improvment in this module, as you stated earlier. ;-)\n\nGood work!\n\nChristian",
    "created_at": "2012-12-02T18:26:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208597",
    "user": "https://trac.sagemath.org/admin/accounts/users/christiankuper"
}
```

<a id='comment:17'></a>Hello Nathann,

as far as I can judge there are no more issues with this patch:

* All test passed successfully
* The doc look ok
* The methods are behaving as promised in the doc (default behavior with facade = True etc)

BTW, the explanation for the facade = True / False / None bevavior is much clearer than in your first version ;-)

Just be clear about the extension of my review: I focused ONLY on the changes you made. IMHO there is still room for improvment in this module, as you stated earlier. ;-)

Good work!

Christian



---

archive/issue_comments_208598.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2012-12-03T05:10:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208598",
    "user": "https://github.com/novoselt"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_208599.json:
```json
{
    "body": "<a id='comment:18'></a>The changes in `sage/geometry/fan.py` do not look OK to me:\n\n```\nf = f \n```\nSeriously???\n\n```\n'agree'\n```\ninstead of \n\n```\n\"agree\"\n```\njust a change for the sake of change?\n\n```\n(e for e in level)\n```\nNot much better than the first one and it is repeated in `sage/geometry/cone.py`. If fixing doctests was done by some script/RE, it would be nice to read the result, especially if the code got changed, not just doctests as the patch name suggests.",
    "created_at": "2012-12-03T05:10:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208599",
    "user": "https://github.com/novoselt"
}
```

<a id='comment:18'></a>The changes in `sage/geometry/fan.py` do not look OK to me:

```
f = f 
```
Seriously???

```
'agree'
```
instead of 

```
"agree"
```
just a change for the sake of change?

```
(e for e in level)
```
Not much better than the first one and it is repeated in `sage/geometry/cone.py`. If fixing doctests was done by some script/RE, it would be nice to read the result, especially if the code got changed, not just doctests as the patch name suggests.



---

archive/issue_comments_208600.json:
```json
{
    "body": "<a id='comment:19'></a>Hellooooooooooooooooo !!!\n\n> The changes in `sage/geometry/fan.py` do not look OK to me:\n\n\nAnd indeedthey aren't `:-P`\n\n> {{{\n> f = f \n> }}}\n> Seriously???\n\n\nYou're not in the right frame of mind to appreciate such a change. Here is a quote from the Wikipedia Page on monochrome paintings (it actually talks about things like that : http://en.wikipedia.org/wiki/File:Black_square.jpg)\n\n---\nSuprematism and Constructivism\n\nMonochrome painting as it is usually understood today began in Moscow, with Suprematist Composition: White on White [5] of 1918 by Suprematist artist Kazimir Malevich. This was a variation on or sequel to his 1915 work \u201cBlack Square on a White Field\u201d, a very important work in its own right to 20th century geometric abstraction.\nIn 1921, Constructivist artist Alexandr Rodchenko exhibited three paintings together, each a monochrome of one of the three primary colours. He intended this work to represent The Death of Painting.[6]\n\nWhile Rodchenko intended his monochrome to be a dismantling of the typical assumptions of painting, Malevich saw his work as a concentration on them, a kind of meditation on art\u2019s essence (\u201cpure feeling\u201d).\n\nThese two approaches articulated very early on in its history this kind of work\u2019s almost paradoxical dynamic: that one can read a monochrome either as a flat surface (material entity or \u201cpainting as object\u201d) which represents nothing but itself, and therefore representing an ending in the evolution of illusionism in painting (i.e. Rodchenko); or as a depiction of multidimensional (infinite) space, a fulfillment of illusionistic painting, representing a new evolution\u2014a new beginning\u2014in Western painting\u2019s history (Malevich). Additionally, many have pointed out that it may be difficult to deduce the artist\u2019s intentions from the painting itself, without referring to the artist\u2019s comment.\n\n---\n\nSo of course, f=f could seem a bit trivial and useless to you. You may even think that you could have done it by yourself. But the truth is that there is a depth behind that you just cannot appreciate.\n\nI removed it from the new version of my patch, because I don't get it either.\n\n> \n> \n> ```\n> 'agree'\n> ```\n> instead of \n> \n> ```\n> \"agree\"\n> ```\n> just a change for the sake of change?\n\n\nTotally. I love change. I first intended to replace \"agree\" by $#$aaaagrRRrrrrrrrrreeeeeee$#$ but hesitated when I considered the extravagant cost of the characters.\nThis, and because emacs seems to have trouble with the \" in the docstring, and for this reason believed that in the rest of the file comments were code, and code comments...\nDo you mind ? `^^;`\n\n> {{{\n> (e for e in level)\n> }}}\n> Not much better than the first one and it is repeated in `sage/geometry/cone.py`. If fixing doctests was done by some script/RE, it would be nice to read the result, especially if the code got changed, not just doctests as the patch name suggests.\n\n\nOf course here I have to make the same remark as previously. There is depth in `(e for e in level)`, a depth that is not reached by the expression `iter(level)`. I changed it anyway, there and at all other places where it was needed.\n\nSorry for all that. It's just that I changed these files sooooooooooooo many times that I was a bit too excited when it passed all doctests. Thank you for looking at it.\n\nHave fuuuuuuuuuuuuuuuun !\n\nNathann\n\nP.S. : By the way, if only to relieve you of a legitimate fear if you thought that all this was done without me looking : I indeed did most of these changes with emacs macros, but each time something was about to be changed in the code emacs asked me whether I agreed with the change (he's that kind). I only changed code when I thought the change made sense and would not change the code's meaning. As I said, I changed this code in so many ways, hoping to finally get the doctests to pass, that I just wanted to see whether the method worked. And if not, just delete the patch and try another way `:-)`",
    "created_at": "2012-12-03T09:14:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208600",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:19'></a>Hellooooooooooooooooo !!!

> The changes in `sage/geometry/fan.py` do not look OK to me:


And indeedthey aren't `:-P`

> {{{
> f = f 
> }}}
> Seriously???


You're not in the right frame of mind to appreciate such a change. Here is a quote from the Wikipedia Page on monochrome paintings (it actually talks about things like that : http://en.wikipedia.org/wiki/File:Black_square.jpg)

---
Suprematism and Constructivism

Monochrome painting as it is usually understood today began in Moscow, with Suprematist Composition: White on White [5] of 1918 by Suprematist artist Kazimir Malevich. This was a variation on or sequel to his 1915 work “Black Square on a White Field”, a very important work in its own right to 20th century geometric abstraction.
In 1921, Constructivist artist Alexandr Rodchenko exhibited three paintings together, each a monochrome of one of the three primary colours. He intended this work to represent The Death of Painting.[6]

While Rodchenko intended his monochrome to be a dismantling of the typical assumptions of painting, Malevich saw his work as a concentration on them, a kind of meditation on art’s essence (“pure feeling”).

These two approaches articulated very early on in its history this kind of work’s almost paradoxical dynamic: that one can read a monochrome either as a flat surface (material entity or “painting as object”) which represents nothing but itself, and therefore representing an ending in the evolution of illusionism in painting (i.e. Rodchenko); or as a depiction of multidimensional (infinite) space, a fulfillment of illusionistic painting, representing a new evolution—a new beginning—in Western painting’s history (Malevich). Additionally, many have pointed out that it may be difficult to deduce the artist’s intentions from the painting itself, without referring to the artist’s comment.

---

So of course, f=f could seem a bit trivial and useless to you. You may even think that you could have done it by yourself. But the truth is that there is a depth behind that you just cannot appreciate.

I removed it from the new version of my patch, because I don't get it either.

> 
> 
> ```
> 'agree'
> ```
> instead of 
> 
> ```
> "agree"
> ```
> just a change for the sake of change?


Totally. I love change. I first intended to replace "agree" by $#$aaaagrRRrrrrrrrrreeeeeee$#$ but hesitated when I considered the extravagant cost of the characters.
This, and because emacs seems to have trouble with the " in the docstring, and for this reason believed that in the rest of the file comments were code, and code comments...
Do you mind ? `^^;`

> {{{
> (e for e in level)
> }}}
> Not much better than the first one and it is repeated in `sage/geometry/cone.py`. If fixing doctests was done by some script/RE, it would be nice to read the result, especially if the code got changed, not just doctests as the patch name suggests.


Of course here I have to make the same remark as previously. There is depth in `(e for e in level)`, a depth that is not reached by the expression `iter(level)`. I changed it anyway, there and at all other places where it was needed.

Sorry for all that. It's just that I changed these files sooooooooooooo many times that I was a bit too excited when it passed all doctests. Thank you for looking at it.

Have fuuuuuuuuuuuuuuuun !

Nathann

P.S. : By the way, if only to relieve you of a legitimate fear if you thought that all this was done without me looking : I indeed did most of these changes with emacs macros, but each time something was about to be changed in the code emacs asked me whether I agreed with the change (he's that kind). I only changed code when I thought the change made sense and would not change the code's meaning. As I said, I changed this code in so many ways, hoping to finally get the doctests to pass, that I just wanted to see whether the method worked. And if not, just delete the patch and try another way `:-)`



---

archive/issue_comments_208601.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2012-12-03T09:14:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208601",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_208602.json:
```json
{
    "body": "Attachment [trac_13747_reviewer.patch](tarball://root/attachments/some-uuid/ticket13747/trac_13747_reviewer.patch) by @novoselt created at 2012-12-03 18:53:52",
    "created_at": "2012-12-03T18:53:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208602",
    "user": "https://github.com/novoselt"
}
```

Attachment [trac_13747_reviewer.patch](tarball://root/attachments/some-uuid/ticket13747/trac_13747_reviewer.patch) by @novoselt created at 2012-12-03 18:53:52



---

archive/issue_comments_208603.json:
```json
{
    "body": "<a id='comment:20'></a>I do mind change of quotes for no reason, just as fiddling with whitespace. I had an impression that emacs was the most configurable and smart editor ever, so certainly it can understand what is comment and what is not. Anyway, the patch on top of others removes unnecessary iterators and adjusts the flow of documentation comments that dealt with the old behaviour.",
    "created_at": "2012-12-03T19:00:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208603",
    "user": "https://github.com/novoselt"
}
```

<a id='comment:20'></a>I do mind change of quotes for no reason, just as fiddling with whitespace. I had an impression that emacs was the most configurable and smart editor ever, so certainly it can understand what is comment and what is not. Anyway, the patch on top of others removes unnecessary iterators and adjusts the flow of documentation comments that dealt with the old behaviour.



---

archive/issue_comments_208604.json:
```json
{
    "body": "<a id='comment:21'></a>OKayyyyyyyyyyyy... Well, what do we do with this ticket now ? I've got no problem at all with your changes.. `:-)`\n\nNathann",
    "created_at": "2012-12-04T10:00:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208604",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:21'></a>OKayyyyyyyyyyyy... Well, what do we do with this ticket now ? I've got no problem at all with your changes.. `:-)`

Nathann



---

archive/issue_comments_208605.json:
```json
{
    "body": "<a id='comment:22'></a>Well, I have no more objections to the ticket and I think that new default is more intuitive, but I didn't look over the main patch, so I can't set it to positive review.\n\nChristian - are you OK with the ticket overall?",
    "created_at": "2012-12-04T15:43:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208605",
    "user": "https://github.com/novoselt"
}
```

<a id='comment:22'></a>Well, I have no more objections to the ticket and I think that new default is more intuitive, but I didn't look over the main patch, so I can't set it to positive review.

Christian - are you OK with the ticket overall?



---

archive/issue_comments_208606.json:
```json
{
    "body": "<a id='comment:23'></a>Ok, I feel comfortable with the patch. As there seem to be no more open issues I will set it to positive review.",
    "created_at": "2012-12-05T13:34:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208606",
    "user": "https://trac.sagemath.org/admin/accounts/users/christiankuper"
}
```

<a id='comment:23'></a>Ok, I feel comfortable with the patch. As there seem to be no more open issues I will set it to positive review.



---

archive/issue_comments_208607.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2012-12-05T13:34:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208607",
    "user": "https://trac.sagemath.org/admin/accounts/users/christiankuper"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_208608.json:
```json
{
    "body": "<a id='comment:24'></a>Thaaaaaaaaanks ! `:-)`\n\nNathann",
    "created_at": "2012-12-05T13:35:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208608",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:24'></a>Thaaaaaaaaanks ! `:-)`

Nathann



---

archive/issue_comments_208609.json:
```json
{
    "body": "<a id='comment:25'></a>Thanks for the patch and the review. Now the next step is to send a big fat warning on sage-combinat-devel and sage-devel about this backward incompatible change!",
    "created_at": "2012-12-05T21:52:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208609",
    "user": "https://github.com/nthiery"
}
```

<a id='comment:25'></a>Thanks for the patch and the review. Now the next step is to send a big fat warning on sage-combinat-devel and sage-devel about this backward incompatible change!



---

archive/issue_comments_208610.json:
```json
{
    "body": "Changing dependencies from \"\" to \"#11763\"",
    "created_at": "2012-12-18T13:04:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208610",
    "user": "https://github.com/jdemeyer"
}
```

Changing dependencies from "" to "#11763"



---

archive/issue_comments_208611.json:
```json
{
    "body": "<a id='comment:26'></a>This patch conflicts with #11763. I propose that this patch gets rebased to #11763.",
    "created_at": "2012-12-18T13:04:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208611",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:26'></a>This patch conflicts with #11763. I propose that this patch gets rebased to #11763.



---

archive/issue_comments_208612.json:
```json
{
    "body": "Changing status from positive_review to needs_work.",
    "created_at": "2012-12-18T13:04:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208612",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from positive_review to needs_work.



---

archive/issue_comments_208613.json:
```json
{
    "body": "<a id='comment:27'></a>Replying to [jdemeyer](#comment%3A26):\n> This patch conflicts with #11763. I propose that this patch gets rebased to #11763.\n\nyeah, my bad (as the reviewer of #11763). I should have proposed it as a dependence here.",
    "created_at": "2012-12-18T13:11:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208613",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:27'></a>Replying to [jdemeyer](#comment%3A26):
> This patch conflicts with #11763. I propose that this patch gets rebased to #11763.

yeah, my bad (as the reviewer of #11763). I should have proposed it as a dependence here.



---

archive/issue_comments_208614.json:
```json
{
    "body": "Attachment [trac_13747.patch](tarball://root/attachments/some-uuid/ticket13747/trac_13747.patch) by @nathanncohen created at 2012-12-18 13:13:02",
    "created_at": "2012-12-18T13:13:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208614",
    "user": "https://github.com/nathanncohen"
}
```

Attachment [trac_13747.patch](tarball://root/attachments/some-uuid/ticket13747/trac_13747.patch) by @nathanncohen created at 2012-12-18 13:13:02



---

archive/issue_comments_208615.json:
```json
{
    "body": "<a id='comment:28'></a>Attachment [trac_13747-doctests.patch](tarball://root/attachments/some-uuid/ticket13747/trac_13747-doctests.patch) by @nathanncohen created at 2012-12-18 13:16:28\n\nPatches updates, I'm running tests right now.\n\nNathann",
    "created_at": "2012-12-18T13:16:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208615",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:28'></a>Attachment [trac_13747-doctests.patch](tarball://root/attachments/some-uuid/ticket13747/trac_13747-doctests.patch) by @nathanncohen created at 2012-12-18 13:16:28

Patches updates, I'm running tests right now.

Nathann



---

archive/issue_comments_208616.json:
```json
{
    "body": "<a id='comment:29'></a>Apply trac_13747.patch, trac_13747-doctests.patch, trac_13747_reviewer.patch",
    "created_at": "2012-12-18T13:16:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208616",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:29'></a>Apply trac_13747.patch, trac_13747-doctests.patch, trac_13747_reviewer.patch



---

archive/issue_comments_208617.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -10,3 +10,10 @@\n * There is a `Poset.linear_extensions` method whose default behaviour is also `facade = None`. Despite taking an optional flag, this method *DOES NOT WORK* as it is clearly indicated in its docstring. One can easily wonder what the hell this function is doing in Sage's code. Hence this function's behaviour is left untouched. It will use `facade=None` by default until it is rewritten correctly, or removed.\n \n Nathann\n+\n+Apply:\n+* [attachment:trac_13747.patch]\n+* [attachment:trac_13747-doctests.patch]\n+* [attachment:trac_13747_reviewer.patch]\n+* [attachment:trac_13747-rebase-11763.patch]\n+\n``````\n",
    "created_at": "2012-12-18T13:47:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208617",
    "user": "https://github.com/nathanncohen"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -10,3 +10,10 @@
 * There is a `Poset.linear_extensions` method whose default behaviour is also `facade = None`. Despite taking an optional flag, this method *DOES NOT WORK* as it is clearly indicated in its docstring. One can easily wonder what the hell this function is doing in Sage's code. Hence this function's behaviour is left untouched. It will use `facade=None` by default until it is rewritten correctly, or removed.
 
 Nathann
+
+Apply:
+* [attachment:trac_13747.patch]
+* [attachment:trac_13747-doctests.patch]
+* [attachment:trac_13747_reviewer.patch]
+* [attachment:trac_13747-rebase-11763.patch]
+
``````




---

archive/issue_comments_208618.json:
```json
{
    "body": "<a id='comment:30'></a>Because of new occurrences of \".element\" in 11763, this ticket needs another patch to pass doctests... It's just about removing those occurrences, nothing smart involved.\n\nApply trac_13747.patch, trac_13747-doctests.patch, trac_13747_reviewer.patch, trac_13747-rebase-11763.patch",
    "created_at": "2012-12-18T13:47:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208618",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:30'></a>Because of new occurrences of ".element" in 11763, this ticket needs another patch to pass doctests... It's just about removing those occurrences, nothing smart involved.

Apply trac_13747.patch, trac_13747-doctests.patch, trac_13747_reviewer.patch, trac_13747-rebase-11763.patch



---

archive/issue_comments_208619.json:
```json
{
    "body": "Changing status from needs_work to positive_review.",
    "created_at": "2012-12-18T13:47:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208619",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from needs_work to positive_review.



---

archive/issue_comments_208620.json:
```json
{
    "body": "<a id='comment:31'></a>Attachment [trac_13747-rebase-11763.patch](tarball://root/attachments/some-uuid/ticket13747/trac_13747-rebase-11763.patch) by @jdemeyer created at 2012-12-23 21:07:24\n\n\n```\nsage -t  -force_lib devel/sage/sage/combinat/alternating_sign_matrix.py\n**********************************************************************\nFile \"/release/merger/sage-5.6.beta2/devel/sage-main/sage/combinat/alternating_sign_matrix.py\", line 113:\n    sage: L.category()\nExpected:\n    Category of finite lattice posets\nGot:\n    Category of facade finite lattice posets\n**********************************************************************\n```",
    "created_at": "2012-12-23T21:07:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208620",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:31'></a>Attachment [trac_13747-rebase-11763.patch](tarball://root/attachments/some-uuid/ticket13747/trac_13747-rebase-11763.patch) by @jdemeyer created at 2012-12-23 21:07:24


```
sage -t  -force_lib devel/sage/sage/combinat/alternating_sign_matrix.py
**********************************************************************
File "/release/merger/sage-5.6.beta2/devel/sage-main/sage/combinat/alternating_sign_matrix.py", line 113:
    sage: L.category()
Expected:
    Category of finite lattice posets
Got:
    Category of facade finite lattice posets
**********************************************************************
```



---

archive/issue_comments_208621.json:
```json
{
    "body": "Changing status from positive_review to needs_work.",
    "created_at": "2012-12-23T21:07:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208621",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from positive_review to needs_work.



---

archive/issue_comments_208622.json:
```json
{
    "body": "<a id='comment:32'></a>Oh. This is because of #12930 that got merged in the meantime. \n\nNathann",
    "created_at": "2012-12-24T13:39:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208622",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:32'></a>Oh. This is because of #12930 that got merged in the meantime. 

Nathann



---

archive/issue_comments_208623.json:
```json
{
    "body": "Changing dependencies from \"#11763\" to \"#11763, #12930\"",
    "created_at": "2012-12-24T13:49:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208623",
    "user": "https://github.com/nathanncohen"
}
```

Changing dependencies from "#11763" to "#11763, #12930"



---

archive/issue_comments_208624.json:
```json
{
    "body": "<a id='comment:33'></a>Updated ! The most sensible was to change the doctest's output.\n\nNathann",
    "created_at": "2012-12-24T13:49:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208624",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:33'></a>Updated ! The most sensible was to change the doctest's output.

Nathann



---

archive/issue_comments_208625.json:
```json
{
    "body": "Changing status from needs_work to positive_review.",
    "created_at": "2012-12-24T13:49:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208625",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from needs_work to positive_review.



---

archive/issue_comments_208626.json:
```json
{
    "body": "Attachment [trac_13747-rebase-12930.patch](tarball://root/attachments/some-uuid/ticket13747/trac_13747-rebase-12930.patch) by @nathanncohen created at 2012-12-24 13:49:29",
    "created_at": "2012-12-24T13:49:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208626",
    "user": "https://github.com/nathanncohen"
}
```

Attachment [trac_13747-rebase-12930.patch](tarball://root/attachments/some-uuid/ticket13747/trac_13747-rebase-12930.patch) by @nathanncohen created at 2012-12-24 13:49:29



---

archive/issue_comments_208627.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -16,4 +16,4 @@\n * [attachment:trac_13747-doctests.patch]\n * [attachment:trac_13747_reviewer.patch]\n * [attachment:trac_13747-rebase-11763.patch]\n-\n+* [attachment:trac_13747-rebase-12930.patch]\n``````\n",
    "created_at": "2012-12-24T13:49:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208627",
    "user": "https://github.com/nathanncohen"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -16,4 +16,4 @@
 * [attachment:trac_13747-doctests.patch]
 * [attachment:trac_13747_reviewer.patch]
 * [attachment:trac_13747-rebase-11763.patch]
-
+* [attachment:trac_13747-rebase-12930.patch]
``````




---

archive/issue_events_038515.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-12-29T19:31:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13747#event-38515"
}
```



---

archive/issue_comments_208628.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2012-12-29T19:31:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208628",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed



---

archive/issue_comments_208629.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-5.6.beta2\"",
    "created_at": "2012-12-29T19:31:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13747",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13747#issuecomment-208629",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-5.6.beta2"
