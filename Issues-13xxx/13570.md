# Issue 13570: Matrix Entries Can Be Callable Objects not tested

archive/issues_013366.json:
```json
{
    "assignees": [],
    "body": "In the documentation, http://www.sagemath.org/doc/reference/sage/matrix/constructor.html,\n\nit states that matrices can pass in callable objects, though there is no example or test of this here.  Is there documentation at least showing a test of this functionality?  If not, it probably should be added, if not for the end user, at least to ensure it doesn't break.\n\nIn the meanwhile, any tip on getting a matrix like:\n\n['apple', 'frog']\n[[3,2]   , 3      ]\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13570_\n\n",
    "closed_at": "2014-03-11T14:05:51Z",
    "created_at": "2012-10-05T02:53:18Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Matrix Entries Can Be Callable Objects not tested",
    "type": "issue",
    "updated_at": "2014-03-11T14:05:51Z",
    "url": "https://github.com/sagemath/sage/issues/13570",
    "user": "https://github.com/sagetrac-startakovsky"
}
```
In the documentation, http://www.sagemath.org/doc/reference/sage/matrix/constructor.html,

it states that matrices can pass in callable objects, though there is no example or test of this here.  Is there documentation at least showing a test of this functionality?  If not, it probably should be added, if not for the end user, at least to ensure it doesn't break.

In the meanwhile, any tip on getting a matrix like:

['apple', 'frog']
[[3,2]   , 3      ]

_Issue created by migration from https://trac.sagemath.org/ticket/13570_





---

archive/issue_events_173744.json:
```json
{
    "actor": "https://github.com/sagetrac-startakovsky",
    "created_at": "2012-10-05T02:53:18Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13570",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13570#event-173744"
}
```



---

archive/issue_events_173745.json:
```json
{
    "actor": "https://github.com/sagetrac-startakovsky",
    "created_at": "2012-10-05T02:53:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13570",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13570#event-173745"
}
```



---

archive/issue_events_173746.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13570",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13570#event-173746"
}
```



---

archive/issue_events_173747.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13570",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13570#event-173747"
}
```



---

archive/issue_events_173748.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13570",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13570#event-173748"
}
```



---

archive/issue_events_173749.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13570",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13570#event-173749"
}
```



---

archive/issue_comments_160860.json:
```json
{
    "body": "Replying to [ticket:13570 startakovsky]:\n> In the documentation, http://www.sagemath.org/doc/reference/sage/matrix/constructor.html,\n> it states that matrices can pass in callable objects, though there is no example or test of this here.  Is there documentation at least showing a test of this functionality?\n\nYes, there is:\n\n```\nsage: m = matrix(QQ, 3, 3, lambda i, j: i+j); m\n[0 1 2]\n[1 2 3]\n[2 3 4]\n```\n\n> In the meanwhile, any tip on getting a matrix like:\n> \n> ['apple', 'frog']\n> [[3,2]   , 3      ]\n\nI don't think there is special support for matrices *of* callable objects on which multiplication would act as function application, if that is what you are looking for.",
    "created_at": "2014-02-24T13:50:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13570#issuecomment-160860",
    "user": "https://github.com/mezzarobba"
}
```

Replying to [ticket:13570 startakovsky]:
> In the documentation, http://www.sagemath.org/doc/reference/sage/matrix/constructor.html,
> it states that matrices can pass in callable objects, though there is no example or test of this here.  Is there documentation at least showing a test of this functionality?

Yes, there is:

```
sage: m = matrix(QQ, 3, 3, lambda i, j: i+j); m
[0 1 2]
[1 2 3]
[2 3 4]
```

> In the meanwhile, any tip on getting a matrix like:
> 
> ['apple', 'frog']
> [[3,2]   , 3      ]

I don't think there is special support for matrices *of* callable objects on which multiplication would act as function application, if that is what you are looking for.



---

archive/issue_events_173750.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2014-02-24T13:50:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13570",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13570#event-173750"
}
```



---

archive/issue_events_173751.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2014-02-24T13:50:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13570",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13570#event-173751"
}
```



---

archive/issue_events_173752.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2014-02-24T13:50:11Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13570",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13570#event-173752"
}
```



---

archive/issue_events_173753.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2014-02-24T13:50:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13570",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13570#event-173753"
}
```



---

archive/issue_events_173754.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2014-02-24T13:50:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13570",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13570#event-173754"
}
```



---

archive/issue_comments_160861.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nThanks.  I ended up solving what I needed to solve and ultimately defined class on which there exists multiplication and addition, and that multiplication is cartesian product and addition is disjoint unions, and it works well.",
    "created_at": "2014-02-25T00:56:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13570#issuecomment-160861",
    "user": "https://github.com/sagetrac-startakovsky"
}
```

<a id='comment:4'>Comment 4:</a>
Thanks.  I ended up solving what I needed to solve and ultimately defined class on which there exists multiplication and addition, and that multiplication is cartesian product and addition is disjoint unions, and it works well.



---

archive/issue_events_173755.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2014-03-09T22:14:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13570",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13570#event-173755"
}
```



---

archive/issue_events_173756.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2014-03-09T22:14:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13570",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13570#event-173756"
}
```



---

archive/issue_events_173757.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-03-11T14:05:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13570",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13570#event-173757"
}
```



---

archive/issue_events_173758.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-03-11T14:05:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13570",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13570#event-173758"
}
```
