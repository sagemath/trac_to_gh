# Issue 13381: Add quickstarts from PREP workshops to standard documentation

archive/issues_013209.json:
```json
{
    "assignees": [],
    "body": "This ticket is to add the quickstart tutorials, originally at #13260.  It was split off from that ticket for convenience.\n\nApply [[attachment: trac_13381-ref.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-ref.patch.gz), and [attachment: trac_13381-fixtests.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-fixtests.patch.gz)(https://github.com/sagemath/sage/files/ticket13381/7f3287c0a3d0f3480e1174e414c98a90.gz).\n\nDepends on #5457\nDepends on #9005\nDepends on #14014\n\nAssignee: **mvngu**\n\nCC:  @jasongrout @rbeezer @dandrake @wdjoyner mhampton @benjaminfjones @wypong DanaErnst travis @novoselt @jhpalmieri\n\nReviewer: **John Palmieri, Jason Grout, Jeroen Demeyer**\n\nAuthor: **Karl-Dieter Crisman**\n\nMerged: **sage-5.11.rc0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13381_\n\n",
    "closed_at": "2013-07-31T12:53:41Z",
    "created_at": "2012-08-19T03:45:55Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.11",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add quickstarts from PREP workshops to standard documentation",
    "type": "issue",
    "updated_at": "2013-07-31T12:53:41Z",
    "url": "https://github.com/sagemath/sage/issues/13381",
    "user": "https://github.com/kcrisman"
}
```
This ticket is to add the quickstart tutorials, originally at #13260.  It was split off from that ticket for convenience.

Apply [[attachment: trac_13381-ref.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-ref.patch.gz), and [attachment: trac_13381-fixtests.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-fixtests.patch.gz)(https://github.com/sagemath/sage/files/ticket13381/7f3287c0a3d0f3480e1174e414c98a90.gz).

Depends on #5457
Depends on #9005
Depends on #14014

Assignee: **mvngu**

CC:  @jasongrout @rbeezer @dandrake @wdjoyner mhampton @benjaminfjones @wypong DanaErnst travis @novoselt @jhpalmieri

Reviewer: **John Palmieri, Jason Grout, Jeroen Demeyer**

Author: **Karl-Dieter Crisman**

Merged: **sage-5.11.rc0**

_Issue created by migration from https://trac.sagemath.org/ticket/13381_





---

archive/issue_events_165263.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-08-19T03:45:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13381#event-165263"
}
```



---

archive/issue_events_165264.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-08-19T03:45:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13381#event-165264"
}
```



---

archive/issue_events_165265.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-08-19T03:45:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13381#event-165265"
}
```



---

archive/issue_events_165266.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-08-19T03:45:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13381#event-165266"
}
```



---

archive/issue_comments_157933.json:
```json
{
    "body": "Dependencies: **#13260**",
    "created_at": "2012-08-19T03:49:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157933",
    "user": "https://github.com/kcrisman"
}
```

Dependencies: **#13260**



---

archive/issue_comments_157934.json:
```json
{
    "body": "Based on 5.3.beta1",
    "created_at": "2012-08-19T03:51:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157934",
    "user": "https://github.com/kcrisman"
}
```

Based on 5.3.beta1



---

archive/issue_events_165267.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-08-19T03:52:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13381#event-165267"
}
```



---

archive/issue_comments_157935.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n This ticket is to add the quickstart tutorials, originally at #13260.  It was split off from that ticket for convenience.\n+\n+Apply [attachment: trac_13381-quickstarts.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-quickstarts.patch.gz).\n``````\n",
    "created_at": "2012-08-19T03:52:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157935",
    "user": "https://github.com/kcrisman"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
 This ticket is to add the quickstart tutorials, originally at #13260.  It was split off from that ticket for convenience.
+
+Apply [attachment: trac_13381-quickstarts.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-quickstarts.patch.gz).
``````




---

archive/issue_comments_157936.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nAttachment: **[trac_13381-quickstarts.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-quickstarts.patch.gz)**\n\nReady for review.\n\nPatchbot, apply trac_13381-quickstarts.patch.",
    "created_at": "2012-08-19T03:52:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157936",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:2'>Comment 2:</a>
Attachment: **[trac_13381-quickstarts.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-quickstarts.patch.gz)**

Ready for review.

Patchbot, apply trac_13381-quickstarts.patch.



---

archive/issue_comments_157937.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n This ticket is to add the quickstart tutorials, originally at #13260.  It was split off from that ticket for convenience.\n \n-Apply [attachment: trac_13381-quickstarts.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-quickstarts.patch.gz).\n+Apply [and [attachment: trac_13381-ref.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-ref.patch.gz)(https://github.com/sagemath/sage/files/ticket13381/2228d54876801ab8df34ab7f46aab09e.gz).\n``````\n",
    "created_at": "2012-09-07T21:09:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157937",
    "user": "https://github.com/jhpalmieri"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 This ticket is to add the quickstart tutorials, originally at #13260.  It was split off from that ticket for convenience.
 
-Apply [attachment: trac_13381-quickstarts.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-quickstarts.patch.gz).
+Apply [and [attachment: trac_13381-ref.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-ref.patch.gz)(https://github.com/sagemath/sage/files/ticket13381/2228d54876801ab8df34ab7f46aab09e.gz).
``````




---

archive/issue_comments_157938.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nHere is a referee patch. If it's okay with you, then one item remains to be addressed: in `NumAnalysis.rst`:\n\n- \"The first step is to normalize\" - The first step in what? It's not clear what's being done here. It looks like you want to convert x to floating point binary, but it also looks like that was done a few lines earlier in `x.str(base=2)`. So a little more explanation would be helpful.\n\nOther comments (I think most of my changes are self-explanatory): \n\n- in `Abstract-Algebra.rst` and elsewhere, I've changed `\\ZZ_n` to `\\ZZ/n\\ZZ`. The subscript notation is ambiguous \u2014 many people will read `\\ZZ_p` as denoting the p-adics, not the integers mod p, and so in my opinion this notation should never be used to denote quotient rings of the integers.\n\n- in `Linear-Algebra.rst`, I've added a definition of \"left kernel\": personally, I can tell from the name \"left kernel\" that either the matrix or the vector is supposed to be on the left, but I can never remember which.\n\nThe use of upper case in the file names here (and in the parent directory) doesn't follow the same pattern as elsewhere in the Sage library. This is certainly not a big deal, but it looks a little strange to me.",
    "created_at": "2012-09-07T21:09:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157938",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:3'>Comment 3:</a>
Here is a referee patch. If it's okay with you, then one item remains to be addressed: in `NumAnalysis.rst`:

- "The first step is to normalize" - The first step in what? It's not clear what's being done here. It looks like you want to convert x to floating point binary, but it also looks like that was done a few lines earlier in `x.str(base=2)`. So a little more explanation would be helpful.

Other comments (I think most of my changes are self-explanatory): 

- in `Abstract-Algebra.rst` and elsewhere, I've changed `\ZZ_n` to `\ZZ/n\ZZ`. The subscript notation is ambiguous — many people will read `\ZZ_p` as denoting the p-adics, not the integers mod p, and so in my opinion this notation should never be used to denote quotient rings of the integers.

- in `Linear-Algebra.rst`, I've added a definition of "left kernel": personally, I can tell from the name "left kernel" that either the matrix or the vector is supposed to be on the left, but I can never remember which.

The use of upper case in the file names here (and in the parent directory) doesn't follow the same pattern as elsewhere in the Sage library. This is certainly not a big deal, but it looks a little strange to me.



---

archive/issue_comments_157939.json:
```json
{
    "body": "Attachment: **[trac_13381-ref.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-ref.patch.gz)**\n\nreferee patch; apply on top of other one",
    "created_at": "2012-09-07T21:11:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157939",
    "user": "https://github.com/jhpalmieri"
}
```

Attachment: **[trac_13381-ref.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-ref.patch.gz)**

referee patch; apply on top of other one



---

archive/issue_comments_157940.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nregarding left vs. right: I guess I always think that the way I do it naturally is the \"right\" (correct) way to do it :).  That helps me remember that the normal linear algebra way (column vectors, A*x) is the \"right\" way.\n\nNot that I'm biased or anything :).",
    "created_at": "2012-09-07T22:47:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157940",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:4'>Comment 4:</a>
regarding left vs. right: I guess I always think that the way I do it naturally is the "right" (correct) way to do it :).  That helps me remember that the normal linear algebra way (column vectors, A*x) is the "right" way.

Not that I'm biased or anything :).



---

archive/issue_comments_157941.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nJohn, did you review this positively, or is there still more to do?",
    "created_at": "2012-10-02T16:19:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157941",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:5'>Comment 5:</a>
John, did you review this positively, or is there still more to do?



---

archive/issue_comments_157942.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nIn [my comment above](#comment%3A3), I said that \"one item remains to be addressed\". This is still true.",
    "created_at": "2012-10-02T16:24:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157942",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:6'>Comment 6:</a>
In [my comment above](#comment%3A3), I said that "one item remains to be addressed". This is still true.



---

archive/issue_comments_157943.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nI think he was at least thinking the `NumAnalysis.rst` thing about normalization needed to be fixed (which I think I agree about, but it was a while ago that I read this so I forget).   Also, his patch is long enough that is probably could use a quick once-over to make sure he didn't miss any obvious formatting things, though this **IS** jhpalmieri, the king of Sphinx, we're talking about here :)",
    "created_at": "2012-10-02T16:26:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157943",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:7'>Comment 7:</a>
I think he was at least thinking the `NumAnalysis.rst` thing about normalization needed to be fixed (which I think I agree about, but it was a while ago that I read this so I forget).   Also, his patch is long enough that is probably could use a quick once-over to make sure he didn't miss any obvious formatting things, though this **IS** jhpalmieri, the king of Sphinx, we're talking about here :)



---

archive/issue_comments_157944.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nThe heading could be changed to \"Converting to IEEE floating point binary format\"---does that make things clearer?  The rest of the paragraph in question explains that the first step in IEEE conversion is normalizing the number so that it is a number between 1 and 2 (multiplied by an appropriate power of 2).",
    "created_at": "2012-10-02T16:59:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157944",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:8'>Comment 8:</a>
The heading could be changed to "Converting to IEEE floating point binary format"---does that make things clearer?  The rest of the paragraph in question explains that the first step in IEEE conversion is normalizing the number so that it is a number between 1 and 2 (multiplied by an appropriate power of 2).



---

archive/issue_comments_157945.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nOr how about this rewording of the paragraph:\n\n```\nNow let's convert ``x`` to the IEEE 754 binary format that is commonly used in computers.  For `IEEE 754\n<http://grouper.ieee.org/groups/754/>`_, the first step in getting the binary format is to normalize the \nnumber, or express the number as a number between 1 and 2 multiplied by a power of 2.  For our ``x`` above, \nwe multiply by `2^{-3}` to get a number between 1 and 2.\n```\n\nIs that better?",
    "created_at": "2012-10-02T17:05:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157945",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:9'>Comment 9:</a>
Or how about this rewording of the paragraph:

```
Now let's convert ``x`` to the IEEE 754 binary format that is commonly used in computers.  For `IEEE 754
<http://grouper.ieee.org/groups/754/>`_, the first step in getting the binary format is to normalize the 
number, or express the number as a number between 1 and 2 multiplied by a power of 2.  For our ``x`` above, 
we multiply by `2^{-3}` to get a number between 1 and 2.
```

Is that better?



---

archive/issue_comments_157946.json:
```json
{
    "body": "Reviewer: **John Palmieri, Jason Grout**",
    "created_at": "2012-10-02T17:27:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157946",
    "user": "https://github.com/kcrisman"
}
```

Reviewer: **John Palmieri, Jason Grout**



---

archive/issue_comments_157947.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nThat seems reasonable.  Actually, making the section heading \"Converting to floating point binary (IEEE format)\" would be the best compromise on the heading title as well.",
    "created_at": "2012-10-02T17:27:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157947",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:10'>Comment 10:</a>
That seems reasonable.  Actually, making the section heading "Converting to floating point binary (IEEE format)" would be the best compromise on the heading title as well.



---

archive/issue_comments_157948.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nThese all look like good suggestions to me.",
    "created_at": "2012-10-02T17:31:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157948",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:11'>Comment 11:</a>
These all look like good suggestions to me.



---

archive/issue_comments_157949.json:
```json
{
    "body": "Apply on top of others",
    "created_at": "2013-06-20T00:29:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157949",
    "user": "https://github.com/kcrisman"
}
```

Apply on top of others



---

archive/issue_comments_157950.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n This ticket is to add the quickstart tutorials, originally at #13260.  It was split off from that ticket for convenience.\n \n-Apply [and [attachment: trac_13381-ref.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-ref.patch.gz)(https://github.com/sagemath/sage/files/ticket13381/2228d54876801ab8df34ab7f46aab09e.gz).\n+Apply [[attachment: trac_13381-ref.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-ref.patch.gz), and [attachment: trac_13381-ieee.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-ieee.patch.gz)(https://github.com/sagemath/sage/files/ticket13381/7f3287c0a3d0f3480e1174e414c98a90.gz).\n``````\n",
    "created_at": "2013-06-20T00:30:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157950",
    "user": "https://github.com/kcrisman"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 This ticket is to add the quickstart tutorials, originally at #13260.  It was split off from that ticket for convenience.
 
-Apply [and [attachment: trac_13381-ref.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-ref.patch.gz)(https://github.com/sagemath/sage/files/ticket13381/2228d54876801ab8df34ab7f46aab09e.gz).
+Apply [[attachment: trac_13381-ref.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-ref.patch.gz), and [attachment: trac_13381-ieee.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-ieee.patch.gz)(https://github.com/sagemath/sage/files/ticket13381/7f3287c0a3d0f3480e1174e414c98a90.gz).
``````




---

archive/issue_comments_157951.json:
```json
{
    "body": "Changed dependencies from **#13260** to none",
    "created_at": "2013-06-20T00:30:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157951",
    "user": "https://github.com/kcrisman"
}
```

Changed dependencies from **#13260** to none



---

archive/issue_comments_157952.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nAttachment: **[trac_13381-ieee.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-ieee.patch.gz)**\n\nOkay, I've finished this off by just implementing this.  John was okay with this, and all I did was put what Jason suggested.  Yay!",
    "created_at": "2013-06-20T00:30:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157952",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:12'>Comment 12:</a>
Attachment: **[trac_13381-ieee.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-ieee.patch.gz)**

Okay, I've finished this off by just implementing this.  John was okay with this, and all I did was put what Jason suggested.  Yay!



---

archive/issue_events_165268.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-06-20T00:30:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13381#event-165268"
}
```



---

archive/issue_events_165269.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-06-20T00:30:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13381#event-165269"
}
```



---

archive/issue_comments_157953.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nThere are [obvious doctest failures](http://patchbot.sagemath.org/log/13381/Fedora/18/x86_64/3.9.5-201.fc18.x86_64/desktop/2013-06-19%2012:38:41%20+0100?short), why did this ever get positive review?\n\nWhile you're at it, you should also use the new-style doctest continuations\n\n```\nsage: for K in D.conjugacy_classes_subgroups(): \n....:     print K \n```\ninstead of\n\n```\nsage: for K in D.conjugacy_classes_subgroups(): \n...       print K \n```",
    "created_at": "2013-06-20T11:50:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157953",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:13'>Comment 13:</a>
There are [obvious doctest failures](http://patchbot.sagemath.org/log/13381/Fedora/18/x86_64/3.9.5-201.fc18.x86_64/desktop/2013-06-19%2012:38:41%20+0100?short), why did this ever get positive review?

While you're at it, you should also use the new-style doctest continuations

```
sage: for K in D.conjugacy_classes_subgroups(): 
....:     print K 
```
instead of

```
sage: for K in D.conjugacy_classes_subgroups(): 
...       print K 
```



---

archive/issue_events_165270.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-20T11:50:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13381#event-165270"
}
```



---

archive/issue_events_165271.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-20T11:50:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13381#event-165271"
}
```



---

archive/issue_comments_157954.json:
```json
{
    "body": "Dependencies: **#5457, #9005, #14014**",
    "created_at": "2013-06-20T11:50:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157954",
    "user": "https://github.com/jdemeyer"
}
```

Dependencies: **#5457, #9005, #14014**



---

archive/issue_comments_157955.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\n> There are  obvious doctest failures, why did this ever get positive review?\n\nBecause the only previous complaints were about non-doctest issues.  Patchbot is very flaky, in my opinion, and often fails for trivial reasons.\n\nInterestingly, at least one of these failures indicates a regression in Sage.  The rest are pretty easy to fix, I'll do this later today along with the new-style continuations.",
    "created_at": "2013-06-20T15:51:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157955",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:14'>Comment 14:</a>
> There are  obvious doctest failures, why did this ever get positive review?

Because the only previous complaints were about non-doctest issues.  Patchbot is very flaky, in my opinion, and often fails for trivial reasons.

Interestingly, at least one of these failures indicates a regression in Sage.  The rest are pretty easy to fix, I'll do this later today along with the new-style continuations.



---

archive/issue_comments_157956.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nI also changed to new continuations in the main prep stuff.  Very annoying.  Some of the changes are pretty recent as well.\n\nThere are two remaining issues, I think.\n\n```\nsage: import scipy.stats\nsage: my_data=[lognormvariate(2,3) for i in range(10)]\nsage: scipy.stats.scoreatpercentile(my_data, int(50))\n1.2439846750720158\nsage: scipy.stats.scoreatpercentile(my_data, 50)\n---------------------------------------------------------------------------\n\nTypeError: 'sage.rings.integer.Integer' object is not iterable\n```\nThis is because scipy now assumes a Sage Integer is not a scalar.  I'm not sure whether this is an upstream bug or not.\n\nThen there is this strange regression in plotting power series.\n\n```\nsage: R.<t> = PowerSeriesRing(QQ)\nsage: c = t + 1 + O(t^2)\nsage: plot(c,(t,0,1))\nAttributeError: 'float' object has no attribute 'parent'\n```\nIf I just do the polynomial, it's fine, so I can fix the test.  Interestingly, I get a different error with the standard syntax:\n\n```\nplot(c.polynomial(),(t,0,1))\nError...\n```",
    "created_at": "2013-06-20T17:44:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157956",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:15'>Comment 15:</a>
I also changed to new continuations in the main prep stuff.  Very annoying.  Some of the changes are pretty recent as well.

There are two remaining issues, I think.

```
sage: import scipy.stats
sage: my_data=[lognormvariate(2,3) for i in range(10)]
sage: scipy.stats.scoreatpercentile(my_data, int(50))
1.2439846750720158
sage: scipy.stats.scoreatpercentile(my_data, 50)
---------------------------------------------------------------------------

TypeError: 'sage.rings.integer.Integer' object is not iterable
```
This is because scipy now assumes a Sage Integer is not a scalar.  I'm not sure whether this is an upstream bug or not.

Then there is this strange regression in plotting power series.

```
sage: R.<t> = PowerSeriesRing(QQ)
sage: c = t + 1 + O(t^2)
sage: plot(c,(t,0,1))
AttributeError: 'float' object has no attribute 'parent'
```
If I just do the polynomial, it's fine, so I can fix the test.  Interestingly, I get a different error with the standard syntax:

```
plot(c.polynomial(),(t,0,1))
Error...
```



---

archive/issue_comments_157957.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nI think I have discovered a really nasty bug with new-style line continuations.  I cannot get live documentation to work with it.  So I am *not* going to do this.  For some reason the new such doctests don't seem to be in the live documentation (probably in underscore methods, and not a lot of them), so that's why it hasn't been noticed.  This is as of 5.11.beta1.",
    "created_at": "2013-06-20T18:02:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157957",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:16'>Comment 16:</a>
I think I have discovered a really nasty bug with new-style line continuations.  I cannot get live documentation to work with it.  So I am *not* going to do this.  For some reason the new such doctests don't seem to be in the live documentation (probably in underscore methods, and not a lot of them), so that's why it hasn't been noticed.  This is as of 5.11.beta1.



---

archive/issue_events_165272.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-06-20T18:54:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13381#event-165272"
}
```



---

archive/issue_events_165273.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-06-20T18:54:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13381#event-165273"
}
```



---

archive/issue_comments_157958.json:
```json
{
    "body": "Changed reviewer from **John Palmieri, Jason Grout** to **John Palmieri, Jason Grout, Jeroen Demeyer**",
    "created_at": "2013-06-20T18:54:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157958",
    "user": "https://github.com/kcrisman"
}
```

Changed reviewer from **John Palmieri, Jason Grout** to **John Palmieri, Jason Grout, Jeroen Demeyer**



---

archive/issue_comments_157959.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n This ticket is to add the quickstart tutorials, originally at #13260.  It was split off from that ticket for convenience.\n \n-Apply [[attachment: trac_13381-ref.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-ref.patch.gz), and [attachment: trac_13381-ieee.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-ieee.patch.gz)(https://github.com/sagemath/sage/files/ticket13381/7f3287c0a3d0f3480e1174e414c98a90.gz).\n+Apply [[attachment: trac_13381-ref.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-ref.patch.gz), and [attachment: trac_13381-fixtests.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-fixtests.patch.gz)(https://github.com/sagemath/sage/files/ticket13381/7f3287c0a3d0f3480e1174e414c98a90.gz).\n``````\n",
    "created_at": "2013-06-20T18:54:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157959",
    "user": "https://github.com/kcrisman"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 This ticket is to add the quickstart tutorials, originally at #13260.  It was split off from that ticket for convenience.
 
-Apply [[attachment: trac_13381-ref.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-ref.patch.gz), and [attachment: trac_13381-ieee.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-ieee.patch.gz)(https://github.com/sagemath/sage/files/ticket13381/7f3287c0a3d0f3480e1174e414c98a90.gz).
+Apply [[attachment: trac_13381-ref.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-ref.patch.gz), and [attachment: trac_13381-fixtests.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-fixtests.patch.gz)(https://github.com/sagemath/sage/files/ticket13381/7f3287c0a3d0f3480e1174e414c98a90.gz).
``````




---

archive/issue_comments_157960.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nPatchbot, apply trac_13381-quickstarts.patch, trac_13381-ref.patch, and trac_13381-fixtests.patch\n\nDoctests are now fixed, I fixed some broken links (no thanks to the reorganization of the reference manual!), and of course the IEEE business.  All tests pass.  There is nothing substantive changed, though, and I think jeroen's comments suffice.",
    "created_at": "2013-06-20T18:54:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157960",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:17'>Comment 17:</a>
Patchbot, apply trac_13381-quickstarts.patch, trac_13381-ref.patch, and trac_13381-fixtests.patch

Doctests are now fixed, I fixed some broken links (no thanks to the reorganization of the reference manual!), and of course the IEEE business.  All tests pass.  There is nothing substantive changed, though, and I think jeroen's comments suffice.



---

archive/issue_comments_157961.json:
```json
{
    "body": "<a id='comment:18'>Comment 18:</a>\nStill some issues in live documentation, one moment...",
    "created_at": "2013-06-20T18:57:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157961",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:18'>Comment 18:</a>
Still some issues in live documentation, one moment...



---

archive/issue_events_165274.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-06-20T18:57:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13381#event-165274"
}
```



---

archive/issue_events_165275.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-06-20T18:57:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13381#event-165275"
}
```



---

archive/issue_comments_157962.json:
```json
{
    "body": "Apply last",
    "created_at": "2013-06-20T19:17:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157962",
    "user": "https://github.com/kcrisman"
}
```

Apply last



---

archive/issue_comments_157963.json:
```json
{
    "body": "<a id='comment:19'>Comment 19:</a>\nAttachment: **[trac_13381-fixtests.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-fixtests.patch.gz)**\n\nAll set!!\n\nPatchbot, apply trac_13381-quickstarts.patch, trac_13381-ref.patch, and trac_13381-fixtests.patch",
    "created_at": "2013-06-20T19:17:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157963",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:19'>Comment 19:</a>
Attachment: **[trac_13381-fixtests.patch.gz](https://github.com/sagemath/sage/files/ticket13381/trac_13381-fixtests.patch.gz)**

All set!!

Patchbot, apply trac_13381-quickstarts.patch, trac_13381-ref.patch, and trac_13381-fixtests.patch



---

archive/issue_events_165276.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-06-20T19:17:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13381#event-165276"
}
```



---

archive/issue_events_165277.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-06-20T19:17:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13381#event-165277"
}
```



---

archive/issue_events_165278.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-07-31T12:53:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13381#event-165278"
}
```



---

archive/issue_events_165279.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-07-31T12:53:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13381#event-165279"
}
```



---

archive/issue_comments_157964.json:
```json
{
    "body": "Merged: **sage-5.11.rc0**",
    "created_at": "2013-07-31T12:53:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13381#issuecomment-157964",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.11.rc0**
