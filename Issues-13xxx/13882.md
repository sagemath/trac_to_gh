...
<a id='comment:1'></a>Smaller example:
...
```json
{
    "body": "<a id='comment:2'></a>The problem occurs in the add_rows method, if no rows are given: we would then have an uninitialised vector_vector_int that is used in some c-level function - and this is asking for trouble.\n\nMy suggestion: If the input is empty, then return before using the uninitialized value.",
    "created_at": "2012-12-29T22:03:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13882",
    "type": "issue_comment",
...
<a id='comment:2'></a>The problem occurs in the add_rows method, if no rows are given: we would then have an uninitialised vector_vector_int that is used in some c-level function - and this is asking for trouble.
...
```json
{
    "body": "<a id='comment:3'></a>Nice we had that crash previously in sage-on-gentoo good to have a fix. I remember that in the original tiling.py the test would fail in the command after the call to the solver and for that reason that command wasn't tested. So I think that particular test should also be re-enabled in tiling.py.",
...
<a id='comment:4'></a>With your change the following in combinat/tiling.py will have to be changed
...
```json
{
    "body": "<a id='comment:6'></a>Apologies! When I saw your patch I became convinced it would do something for this. To the point of obsession only once, I posted (and gone to bed) did it occur to me that  it wouldn't. I'll do one or two extra tests and give this ticket a positive review later today.",
...
<a id='comment:6'></a>Apologies! When I saw your patch I became convinced it would do something for this. To the point of obsession only once, I posted (and gone to bed) did it occur to me that  it wouldn't. I'll do one or two extra tests and give this ticket a positive review later today.
...
```json
{
    "body": "<a id='comment:7'></a>Pass my tests. Looks good to me. Positive review as is.",
    "created_at": "2012-12-30T22:05:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13882",
    "type": "issue_comment",
...
<a id='comment:7'></a>Pass my tests. Looks good to me. Positive review as is.
