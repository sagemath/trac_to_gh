...
```json
{
    "body": "<a id='comment:4'></a>There is indeed another maximum, and the function will find it if the range is set from 7 to 8, rather than 0 to 8.\n\n```\nsage: find_local_maximum(fast_float(8*e^ (-x)*sin(x) - 1, x), 0, 8)\n(1.5791755355586758, 0.78539816111340355)\nsage: find_local_maximum(fast_float(8*e^ (-x)*sin(x) - 1, x), 7, 8)\n(-0.9951835373923219, 7.0685835822289791)\n```\n\nSo perhaps the test does need two changes.",
...
<a id='comment:4'></a>There is indeed another maximum, and the function will find it if the range is set from 7 to 8, rather than 0 to 8.
