# Issue 13647: padic elements can not be initialized from polynomials

archive/issues_013443.json:
```json
{
    "assignees": [],
    "body": "Currently, elements of extensions of Qp/Zp can not be initialized from polynomials in some cases:\n\n```\nsage: K = Zp(3)\nsage: R.<u> = K[]\nsage: L.<u> = K.extension(u^2 + 1)\nsage: L(R.gen())\nu + O(3^20)\n\nsage: K = ZpCA(3)\nsage: R.<u> = K[]\nsage: L.<u> = K.extension(u^2 + 1)\nsage: L(R.gen())\nNotImplementedError\n\nsage: K = ZpFM(3)\nsage: R.<u> = K[]\nsage: L.<u> = K.extension(u^2 + 1)\nsage: L(R.gen())\nNotImplementedError\n```\n\nThe attached patch fixes this.\n\nAssignee: **@roed314**\n\nKeywords: **sd59**\n\nBranch/Commit: **[5181719](https://github.com/sagemath/sagetrac-mirror/commit/5181719ee4f44a95401d32f3baee0fad8f1433ee)**\n\nReviewer: **Martin Albrecht**\n\nAuthor: **Julian Rueth**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13647_\n\n",
    "closed_at": "2014-07-01T12:54:24Z",
    "created_at": "2012-10-23T17:16:02Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20padics",
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "padic elements can not be initialized from polynomials",
    "type": "issue",
    "updated_at": "2014-07-01T12:54:24Z",
    "url": "https://github.com/sagemath/sage/issues/13647",
    "user": "https://github.com/saraedum"
}
```
Currently, elements of extensions of Qp/Zp can not be initialized from polynomials in some cases:

```
sage: K = Zp(3)
sage: R.<u> = K[]
sage: L.<u> = K.extension(u^2 + 1)
sage: L(R.gen())
u + O(3^20)

sage: K = ZpCA(3)
sage: R.<u> = K[]
sage: L.<u> = K.extension(u^2 + 1)
sage: L(R.gen())
NotImplementedError

sage: K = ZpFM(3)
sage: R.<u> = K[]
sage: L.<u> = K.extension(u^2 + 1)
sage: L(R.gen())
NotImplementedError
```

The attached patch fixes this.

Assignee: **@roed314**

Keywords: **sd59**

Branch/Commit: **[5181719](https://github.com/sagemath/sagetrac-mirror/commit/5181719ee4f44a95401d32f3baee0fad8f1433ee)**

Reviewer: **Martin Albrecht**

Author: **Julian Rueth**

_Issue created by migration from https://trac.sagemath.org/ticket/13647_





---

archive/issue_events_175067.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2012-10-23T17:16:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13647",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13647#event-175067"
}
```



---

archive/issue_events_175068.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2012-10-23T17:16:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13647",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20padics",
    "label_color": "0000ff",
    "label_name": "component: padics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13647#event-175068"
}
```



---

archive/issue_events_175069.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2012-10-23T17:16:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13647",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13647#event-175069"
}
```



---

archive/issue_events_175070.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2012-10-23T17:16:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13647",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13647#event-175070"
}
```



---

archive/issue_comments_162515.json:
```json
{
    "body": "Attachment: **[trac_13647.patch.gz](https://github.com/sagemath/sage/files/ticket13647/trac_13647.patch.gz)**",
    "created_at": "2012-10-23T17:18:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13647",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13647#issuecomment-162515",
    "user": "https://github.com/saraedum"
}
```

Attachment: **[trac_13647.patch.gz](https://github.com/sagemath/sage/files/ticket13647/trac_13647.patch.gz)**



---

archive/issue_events_175071.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13647",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13647#event-175071"
}
```



---

archive/issue_events_175072.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13647",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13647#event-175072"
}
```



---

archive/issue_events_175073.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13647",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13647#event-175073"
}
```



---

archive/issue_events_175074.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13647",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13647#event-175074"
}
```



---

archive/issue_events_175075.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13647",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13647#event-175075"
}
```



---

archive/issue_events_175076.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13647",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13647#event-175076"
}
```



---

archive/issue_comments_162516.json:
```json
{
    "body": "Branch: **[u/saraedum/ticket/13647](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/13647)**",
    "created_at": "2014-06-25T20:24:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13647",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13647#issuecomment-162516",
    "user": "https://github.com/saraedum"
}
```

Branch: **[u/saraedum/ticket/13647](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/13647)**



---

archive/issue_events_175077.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-06-25T20:24:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13647",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13647#event-175077"
}
```



---

archive/issue_comments_162517.json:
```json
{
    "body": "Changed keywords from **** to **sd59**",
    "created_at": "2014-06-25T20:24:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13647",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13647#issuecomment-162517",
    "user": "https://github.com/saraedum"
}
```

Changed keywords from **** to **sd59**



---

archive/issue_comments_162518.json:
```json
{
    "body": "Commit: **[5181719](https://github.com/sagemath/sagetrac-mirror/commit/5181719ee4f44a95401d32f3baee0fad8f1433ee)**",
    "created_at": "2014-06-30T18:06:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13647",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13647#issuecomment-162518",
    "user": "https://github.com/malb"
}
```

Commit: **[5181719](https://github.com/sagemath/sagetrac-mirror/commit/5181719ee4f44a95401d32f3baee0fad8f1433ee)**



---

archive/issue_events_175078.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2014-06-30T18:06:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13647",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13647#event-175078"
}
```



---

archive/issue_events_175079.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2014-06-30T18:06:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13647",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13647#event-175079"
}
```



---

archive/issue_comments_162519.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nLooks okay\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5181719ee4f44a95401d32f3baee0fad8f1433ee\">5181719</a></td><td><code>Trac #13647: fix initialization of padics from polynomials</code></td></tr></table>\n",
    "created_at": "2014-06-30T18:06:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13647",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13647#issuecomment-162519",
    "user": "https://github.com/malb"
}
```

<a id='comment:6'>Comment 6:</a>
Looks okay

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5181719ee4f44a95401d32f3baee0fad8f1433ee">5181719</a></td><td><code>Trac #13647: fix initialization of padics from polynomials</code></td></tr></table>




---

archive/issue_comments_162520.json:
```json
{
    "body": "Reviewer: **Martin Albrecht**",
    "created_at": "2014-06-30T18:06:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13647",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13647#issuecomment-162520",
    "user": "https://github.com/malb"
}
```

Reviewer: **Martin Albrecht**



---

archive/issue_comments_162521.json:
```json
{
    "body": "Changed branch from **[u/saraedum/ticket/13647](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/13647)** to **[5181719](https://github.com/sagemath/sagetrac-mirror/commit/5181719ee4f44a95401d32f3baee0fad8f1433ee)**",
    "created_at": "2014-07-01T12:54:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13647",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13647#issuecomment-162521",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/saraedum/ticket/13647](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/13647)** to **[5181719](https://github.com/sagemath/sagetrac-mirror/commit/5181719ee4f44a95401d32f3baee0fad8f1433ee)**



---

archive/issue_events_175080.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-07-01T12:54:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13647",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13647#event-175080"
}
```



---

archive/issue_events_175081.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "c0280a09516062a5356f80ea92e063d9b05c1e36",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-07-01T12:54:24Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13647",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13647#event-175081"
}
```
