...
```json
{
    "body": "<a id='comment:1'></a>Attachment [13933-slow-call.patch](tarball://root/attachments/some-uuid/ticket13933/13933-slow-call.patch) by @robertwb created at 2013-01-09 10:35:58",
    "created_at": "2013-01-09T10:35:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13933",
    "type": "issue_comment",
...
<a id='comment:1'></a>Attachment [13933-slow-call.patch](tarball://root/attachments/some-uuid/ticket13933/13933-slow-call.patch) by @robertwb created at 2013-01-09 10:35:58
...
<a id='comment:2'></a>In principle it is a good idea. I did something similar in attachment:trac_1173-move_call.patch:ticket:1173.
...
<a id='comment:3'></a>Looks like our patches crossed mid-air... I just posted a patch that moves the `__call__` up.
...
<a id='comment:4'></a>Yep, you posted your patch while I was writing this comment.
...
```json
{
    "body": "<a id='comment:5'></a>Attachment [13933-doctests.patch](tarball://root/attachments/some-uuid/ticket13933/13933-doctests.patch) by @robertwb created at 2013-01-22 05:12:26\n\nAdded attachment that fixes some doctests, in each case the returned result has been changed to be of the same parent as the input.",
...
<a id='comment:5'></a>Attachment [13933-doctests.patch](tarball://root/attachments/some-uuid/ticket13933/13933-doctests.patch) by @robertwb created at 2013-01-22 05:12:26
...
```json
{
    "body": "<a id='comment:6'></a>> The _alt_name code path is still taking up valuable time. It would be better to settle what name we use in the constructor and only try one option. If alt_name is set, we use that, otherwise, we use name.\nSo... is this needs work? Just trying to clarify what the situation is here.",
...
<a id='comment:6'></a>> The _alt_name code path is still taking up valuable time. It would be better to settle what name we use in the constructor and only try one option. If alt_name is set, we use that, otherwise, we use name.
So... is this needs work? Just trying to clarify what the situation is here.
...
<a id='comment:7'></a>Attachment [13933-more-doctests.patch](tarball://root/attachments/some-uuid/ticket13933/13933-more-doctests.patch) by @robertwb created at 2013-06-13 06:39:52
...
```json
{
    "body": "<a id='comment:8'></a>I would be likely to agree with this sentiment.  Burcin, are you okay with Robert's main patch in that case, as it seems?  Then we can just look at the doctests and make sure that was all of the fixes.\n\nI'm not clear on what would be backwards-incompatible, though?",
...
<a id='comment:8'></a>I would be likely to agree with this sentiment.  Burcin, are you okay with Robert's main patch in that case, as it seems?  Then we can just look at the doctests and make sure that was all of the fixes.
...
```json
{
    "body": "<a id='comment:9'></a>It's backwards incompatible because if some objects (defined within or outside of Sage) define sgn() and others define sign(), removing this aliasing would cause one or the other break. \n\nIf we decide to go this route, I think it should be a separate ticket than this performance enhancement.",
...
<a id='comment:9'></a>It's backwards incompatible because if some objects (defined within or outside of Sage) define sgn() and others define sign(), removing this aliasing would cause one or the other break. 
...
```json
{
    "body": "<a id='comment:10'></a>Okay, you're not saying that anything in *this* ticket is breaking anything.  I didn't think so...",
...
<a id='comment:10'></a>Okay, you're not saying that anything in *this* ticket is breaking anything.  I didn't think so...
...
```json
{
    "body": "<a id='comment:12'></a>Patchbot, apply 13933-slow-call.v2.patch 13933-doctests.patch 13933-more-doctests.patch",
...
<a id='comment:12'></a>Patchbot, apply 13933-slow-call.v2.patch 13933-doctests.patch 13933-more-doctests.patch
...
```json
{
    "body": "<a id='comment:13'></a>All tests pass.",
...
<a id='comment:13'></a>All tests pass.
...
<a id='comment:15'></a>Hi guys!  This seems to have either
* exposed inadequate case-checking in trig.py, or
* contained a bug for functions which are `BuiltinFunction`s but used to be directly evaluated using `Function`'s `__call__` method, as opposed to the one now in `BuiltinFunction` (which used to only be used for `GinacFunction`).
Can one of you take a look at[comment:31](#comment%3A31):ticket:13246 to help us ascertain which one it is?   Thanks!
