...
```json
{
    ...
    "asset_url": "tarball://root/attachments/some-uuid/ticket13245/trac_13245-clone-docbuild.patch",
...
```json
{
    ...
    "asset_url": "tarball://root/attachments/some-uuid/ticket13245/trac_13245-clone-nodocbuild.patch",
...
```json
{
    ...
    "asset_url": "tarball://root/attachments/some-uuid/ticket13245/trac_13245-clone-cython.patch",
...
```json
{
    "body": "<a id='comment:19'></a>\nReplying to [leif](#comment%3A4):\n> Oh, forgot:  Cython-generated header files (`*.h`) don't contain a special comment on that on the first line (which is probably an upstream Cython bug), so the following currently doesn't work as expected either:\n> \n> ```python\n>             if ext in ['.h', '.c', '.cpp']:\n>                 if 'Cython' in open(src + '/' + F).readline():\n>                     os.link(src + '/' + F, dest + '/' +F)\n>                     os.utime(dest + '/' +F, None)\n> ```\n> \n> (For the moment, we could look for `^#ifndef `__PYX_HAVE__sage__`` in those, probably even without the `sage__`.)\n\n\n\n\n \n\nI'm having another header file in the original but not the clone:\n\n`sage/rings/complex_double_api.h`, which starts with\n\n```c\n#ifndef `__PYX_HAVE_API__sage__rings__complex_`double\n#define `__PYX_HAVE_API__sage__rings__complex_`double\n#include \"Python.h\"\n#include \"complex_double.h\"\n\nstatic struct `__pyx_obj_`4sage_5rings_14complex_double_ComplexDoubleElement *(*__pyx_f_4sage_5rings_14complex_double_new_ComplexDoubleElement)(void) = 0;\n#define new_ComplexDoubleElement `__pyx_f_`4sage_5rings_14complex_double_new_ComplexDoubleElement\n\n#ifndef `__PYX_HAVE_RT_`ImportModule\n#define `__PYX_HAVE_RT_`ImportModule\n```\n\n(and is explicitly listed in `.hgignore`).\n\nI couldn't find any places where this is actually used though, nor do I know where it originates from.",
...
```json
{
    ...
    "asset_url": "tarball://root/attachments/some-uuid/ticket13245/build_log.txt",
