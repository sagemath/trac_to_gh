# Issue 13899: Don't use TAB characters for indentation

archive/issues_013695.json:
```json
{
    "assignees": [],
    "body": "* TABs in Makefiles are needed, so keep them.\n* Some TABs not used for indentation are also kept.\n* TABs in the obsolete directory `devel/sage/sage/server/notebook` (#11409) are not fixed.\n* The directory `sage/graphs/planarity_c` uses a very inconsistent indentation scheme (mixing spaces and TABs and having varying amounts of indentation) is hopeless to easily fix automatically.\n* Also, make indentation consistent and/or remove trailing spaces in some places.\n\nThe release manager script has been updated to check all new patches for added TAB indentation.\n\nApply:\n* [attachment: 13899_TAB_sage_root.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_TAB_sage_root.patch.gz) to the root repository\n* [and [attachment: 13899_doctest.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_doctest.patch.gz)(https://github.com/sagemath/sage/files/ticket13899/bae42736953abaadde5a29d5dec4a42d.gz) to the Sage library\n* [attachment: 13899_TAB_scripts.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_TAB_scripts.patch.gz) to the scripts repository\n\nAssignee: **@jasongrout**\n\nReviewer: **Leif Leonhardy, Karl-Dieter Crisman**\n\nAuthor: **Jeroen Demeyer**\n\nMerged: **sage-5.6.beta3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13899_\n\n",
    "closed_at": "2013-01-07T20:58:33Z",
    "created_at": "2013-01-02T10:56:07Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Don't use TAB characters for indentation",
    "type": "issue",
    "updated_at": "2013-01-15T17:19:10Z",
    "url": "https://github.com/sagemath/sage/issues/13899",
    "user": "https://github.com/jdemeyer"
}
```
* TABs in Makefiles are needed, so keep them.
* Some TABs not used for indentation are also kept.
* TABs in the obsolete directory `devel/sage/sage/server/notebook` (#11409) are not fixed.
* The directory `sage/graphs/planarity_c` uses a very inconsistent indentation scheme (mixing spaces and TABs and having varying amounts of indentation) is hopeless to easily fix automatically.
* Also, make indentation consistent and/or remove trailing spaces in some places.

The release manager script has been updated to check all new patches for added TAB indentation.

Apply:
* [attachment: 13899_TAB_sage_root.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_TAB_sage_root.patch.gz) to the root repository
* [and [attachment: 13899_doctest.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_doctest.patch.gz)(https://github.com/sagemath/sage/files/ticket13899/bae42736953abaadde5a29d5dec4a42d.gz) to the Sage library
* [attachment: 13899_TAB_scripts.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_TAB_scripts.patch.gz) to the scripts repository

Assignee: **@jasongrout**

Reviewer: **Leif Leonhardy, Karl-Dieter Crisman**

Author: **Jeroen Demeyer**

Merged: **sage-5.6.beta3**

_Issue created by migration from https://trac.sagemath.org/ticket/13899_





---

archive/issue_events_178958.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-02T10:56:07Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "milestone_number": null,
    "milestone_title": "sage-5.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13899#event-178958"
}
```



---

archive/issue_events_178959.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-02T10:56:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13899#event-178959"
}
```



---

archive/issue_events_178960.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-02T10:56:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13899#event-178960"
}
```



---

archive/issue_comments_166708.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n+TABs in Makefiles are needed, so keep them.\n \n+TABs not used for indentation are also kept.\n``````\n",
    "created_at": "2013-01-02T10:57:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166708",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
+TABs in Makefiles are needed, so keep them.
 
+TABs not used for indentation are also kept.
``````




---

archive/issue_comments_166709.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,5 @@\n-TABs in Makefiles are needed, so keep them.\n-\n-TABs not used for indentation are also kept.\n+* TABs in Makefiles are needed, so keep them.\n+* Some TABs not used for indentation are also kept.\n+* TABs in the obsolete directory `devel/sage/sage/server/notebook` (#11409) are not fixed.\n+* The directory `sage/graphs/planarity_c` uses a very inconsistent indentation scheme (mixing spaces and TABs and having varying amounts of indentation) is hopeless to easily fix automatically.\n+* Also, make indentation consistent and/or remove trailing spaces in some places.\n``````\n",
    "created_at": "2013-01-02T11:35:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166709",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,5 @@
-TABs in Makefiles are needed, so keep them.
-
-TABs not used for indentation are also kept.
+* TABs in Makefiles are needed, so keep them.
+* Some TABs not used for indentation are also kept.
+* TABs in the obsolete directory `devel/sage/sage/server/notebook` (#11409) are not fixed.
+* The directory `sage/graphs/planarity_c` uses a very inconsistent indentation scheme (mixing spaces and TABs and having varying amounts of indentation) is hopeless to easily fix automatically.
+* Also, make indentation consistent and/or remove trailing spaces in some places.
``````




---

archive/issue_comments_166710.json:
```json
{
    "body": "Attachment: **[13899_TAB_sage_root.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_TAB_sage_root.patch.gz)**",
    "created_at": "2013-01-02T11:43:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166710",
    "user": "https://github.com/jdemeyer"
}
```

Attachment: **[13899_TAB_sage_root.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_TAB_sage_root.patch.gz)**



---

archive/issue_comments_166711.json:
```json
{
    "body": "Attachment: **[13899_TAB_sagelib.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_TAB_sagelib.patch.gz)**",
    "created_at": "2013-01-02T11:43:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166711",
    "user": "https://github.com/jdemeyer"
}
```

Attachment: **[13899_TAB_sagelib.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_TAB_sagelib.patch.gz)**



---

archive/issue_comments_166712.json:
```json
{
    "body": "Attachment: **[13899_TAB_scripts.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_TAB_scripts.patch.gz)**",
    "created_at": "2013-01-02T11:43:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166712",
    "user": "https://github.com/jdemeyer"
}
```

Attachment: **[13899_TAB_scripts.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_TAB_scripts.patch.gz)**



---

archive/issue_comments_166713.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -3,3 +3,5 @@\n * TABs in the obsolete directory `devel/sage/sage/server/notebook` (#11409) are not fixed.\n * The directory `sage/graphs/planarity_c` uses a very inconsistent indentation scheme (mixing spaces and TABs and having varying amounts of indentation) is hopeless to easily fix automatically.\n * Also, make indentation consistent and/or remove trailing spaces in some places.\n+\n+The release manager script has been updated to check all new patches for added TAB indentation.\n``````\n",
    "created_at": "2013-01-02T11:44:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166713",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -3,3 +3,5 @@
 * TABs in the obsolete directory `devel/sage/sage/server/notebook` (#11409) are not fixed.
 * The directory `sage/graphs/planarity_c` uses a very inconsistent indentation scheme (mixing spaces and TABs and having varying amounts of indentation) is hopeless to easily fix automatically.
 * Also, make indentation consistent and/or remove trailing spaces in some places.
+
+The release manager script has been updated to check all new patches for added TAB indentation.
``````




---

archive/issue_comments_166714.json:
```json
{
    "body": "Author: **Jeroen Demeyer**",
    "created_at": "2013-01-02T11:44:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166714",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Jeroen Demeyer**



---

archive/issue_comments_166715.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nPatches look sane (yes, I've read the whole Sage library patch!), haven't tried to apply them yet though.\n\nOther than removing trailing whitespace (and removing useless backslashes at the end of a few lines), I don't see much value in touching the C/C++ and HTML files.  In particular, removing tabs that introduce end-of-line comments (in the C/C++ files) is rather counter-productive.  [Instead, it would have been worth to translate the French ones, and especially French error messages... ;-) ]",
    "created_at": "2013-01-02T18:01:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166715",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:4'>Comment 4:</a>
Patches look sane (yes, I've read the whole Sage library patch!), haven't tried to apply them yet though.

Other than removing trailing whitespace (and removing useless backslashes at the end of a few lines), I don't see much value in touching the C/C++ and HTML files.  In particular, removing tabs that introduce end-of-line comments (in the C/C++ files) is rather counter-productive.  [Instead, it would have been worth to translate the French ones, and especially French error messages... ;-) ]



---

archive/issue_events_178961.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2013-01-03T01:48:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13899#event-178961"
}
```



---

archive/issue_comments_166716.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nNot sure whether you planned to add further patches here; just noticed it was still \"new\".\n\nPatches apply to Sage 5.6.beta1, doc builds...  (The Sage library patch btw. triggers the rebuild of half of the Sage library.)\n\nHaven't reviewed the merger script though.",
    "created_at": "2013-01-03T01:48:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166716",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:5'>Comment 5:</a>
Not sure whether you planned to add further patches here; just noticed it was still "new".

Patches apply to Sage 5.6.beta1, doc builds...  (The Sage library patch btw. triggers the rebuild of half of the Sage library.)

Haven't reviewed the merger script though.



---

archive/issue_comments_166717.json:
```json
{
    "body": "Reviewer: **Leif Leonhardy**",
    "created_at": "2013-01-03T01:48:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166717",
    "user": "https://github.com/nexttime"
}
```

Reviewer: **Leif Leonhardy**



---

archive/issue_events_178962.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2013-01-03T01:48:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13899#event-178962"
}
```



---

archive/issue_events_178963.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2013-01-03T01:48:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13899#event-178963"
}
```



---

archive/issue_comments_166718.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nThere are actually doctest failures in `conventions.rst`, I guess because of a bug in the doctest script which causes the tests not to be run.",
    "created_at": "2013-01-03T07:14:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166718",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:7'>Comment 7:</a>
There are actually doctest failures in `conventions.rst`, I guess because of a bug in the doctest script which causes the tests not to be run.



---

archive/issue_events_178964.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-03T07:14:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13899#event-178964"
}
```



---

archive/issue_events_178965.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-03T07:14:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13899#event-178965"
}
```



---

archive/issue_events_178966.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-03T13:20:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13899#event-178966"
}
```



---

archive/issue_events_178967.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-03T13:20:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13899#event-178967"
}
```



---

archive/issue_comments_166719.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nThe Sage library patch [attachment: 13899_doctest.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_doctest.patch.gz) needs review.",
    "created_at": "2013-01-03T13:20:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166719",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:8'>Comment 8:</a>
The Sage library patch [attachment: 13899_doctest.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_doctest.patch.gz) needs review.



---

archive/issue_comments_166720.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\n> The Sage library patch [attachment: 13899_doctest.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_doctest.patch.gz) needs review.\n\nThis patch needs some work.   At least you should remove the remaining reference(s?) to `cdd_convert`.  Also, I'm a little surprised that you indented the last two examples by one more - shouldn't they be exactly four spaces from the start of the colon (I mean that column)?  I'm not sure why some of the doc has three spaces to the colon and then four more, but I've seen it before.  I assume the links for `tmp_dir` and `tmp_filename` work fine?",
    "created_at": "2013-01-03T14:55:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166720",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:9'>Comment 9:</a>
> The Sage library patch [attachment: 13899_doctest.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_doctest.patch.gz) needs review.

This patch needs some work.   At least you should remove the remaining reference(s?) to `cdd_convert`.  Also, I'm a little surprised that you indented the last two examples by one more - shouldn't they be exactly four spaces from the start of the colon (I mean that column)?  I'm not sure why some of the doc has three spaces to the colon and then four more, but I've seen it before.  I assume the links for `tmp_dir` and `tmp_filename` work fine?



---

archive/issue_comments_166721.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nReplying to [@kcrisman](#comment%3A9):\n> I assume the links for `tmp_dir` and `tmp_filename` work fine?\n\nNo they don't, but I think links between two different manuals (in this case, from the developer manual to the reference manual) are not supported.",
    "created_at": "2013-01-03T15:02:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166721",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:10'>Comment 10:</a>
Replying to [@kcrisman](#comment%3A9):
> I assume the links for `tmp_dir` and `tmp_filename` work fine?

No they don't, but I think links between two different manuals (in this case, from the developer manual to the reference manual) are not supported.



---

archive/issue_comments_166722.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\n> > I assume the links for `tmp_dir` and `tmp_filename` work fine?\n\n> No they don't, but I think links between two different manuals (in this case, from the developer manual to the reference manual) are not supported.\n\nGood point.  But in that case should they just be\n\n```\n`tmp_dir`\n```\ninstead of\n\n```\n:func:`tmp_dir`\n```\nor does it just ignore this and not make a link?  I'm surprised it doesn't raise an error in building the doc (or does it?).",
    "created_at": "2013-01-03T15:11:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166722",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:11'>Comment 11:</a>
> > I assume the links for `tmp_dir` and `tmp_filename` work fine?

> No they don't, but I think links between two different manuals (in this case, from the developer manual to the reference manual) are not supported.

Good point.  But in that case should they just be

```
`tmp_dir`
```
instead of

```
:func:`tmp_dir`
```
or does it just ignore this and not make a link?  I'm surprised it doesn't raise an error in building the doc (or does it?).



---

archive/issue_comments_166723.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nReplying to [@kcrisman](#comment%3A11):\n> or does it just ignore this and not make a link?\n\nThat's exactly what happens. If we ever allow inter-document links (does #6495 solve this?) the links should magically start working.",
    "created_at": "2013-01-03T15:17:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166723",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:12'>Comment 12:</a>
Replying to [@kcrisman](#comment%3A11):
> or does it just ignore this and not make a link?

That's exactly what happens. If we ever allow inter-document links (does #6495 solve this?) the links should magically start working.



---

archive/issue_comments_166724.json:
```json
{
    "body": "Attachment: **[13899_doctest.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_doctest.patch.gz)**",
    "created_at": "2013-01-03T15:22:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166724",
    "user": "https://github.com/jdemeyer"
}
```

Attachment: **[13899_doctest.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_doctest.patch.gz)**



---

archive/issue_comments_166725.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nReplying to [@kcrisman](#comment%3A9):\n> > The Sage library patch [attachment: 13899_doctest.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_doctest.patch.gz) needs review.\n\n> This patch needs some work.   At least you should remove the remaining reference(s?) to `cdd_convert`.  Also, I'm a little surprised that you indented the last two examples by one more - shouldn't they be exactly four spaces from the start of the colon (I mean that column)?  I'm not sure why some of the doc has three spaces to the colon and then four more, but I've seen it before.\n\nOkay, addressed your comments. New patch ready for review.",
    "created_at": "2013-01-03T15:26:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166725",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:13'>Comment 13:</a>
Replying to [@kcrisman](#comment%3A9):
> > The Sage library patch [attachment: 13899_doctest.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_doctest.patch.gz) needs review.

> This patch needs some work.   At least you should remove the remaining reference(s?) to `cdd_convert`.  Also, I'm a little surprised that you indented the last two examples by one more - shouldn't they be exactly four spaces from the start of the colon (I mean that column)?  I'm not sure why some of the doc has three spaces to the colon and then four more, but I've seen it before.

Okay, addressed your comments. New patch ready for review.



---

archive/issue_comments_166726.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nThe example now using `tmp_filename()` (instead of `SAGE_TMP`) could probably be improved, as the temporary file's name immediately gets discarded.  (I think one would typically reuse it, e.g. to load a saved file later in an example / doctest.)",
    "created_at": "2013-01-03T23:53:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166726",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:14'>Comment 14:</a>
The example now using `tmp_filename()` (instead of `SAGE_TMP`) could probably be improved, as the temporary file's name immediately gets discarded.  (I think one would typically reuse it, e.g. to load a saved file later in an example / doctest.)



---

archive/issue_comments_166727.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nOtherwise Developer's Guide builds and all of its doctests pass now (still with beta1 though), so positive review again if Karl-Dieter's ok with it, too.",
    "created_at": "2013-01-04T00:10:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166727",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:15'>Comment 15:</a>
Otherwise Developer's Guide builds and all of its doctests pass now (still with beta1 though), so positive review again if Karl-Dieter's ok with it, too.



---

archive/issue_comments_166728.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\n> Otherwise Developer's Guide builds and all of its doctests pass now (still with beta1 though), so positive review again if Karl-Dieter's ok with it, too.\n\nBelieve it or not, this does not apply to beta2.  The first hunk is now\n\n```\n        def __init__(self, S, m):\n            \"\"\"\n            See ``HillCryptosystem`` for full documentation.\n\n            EXAMPLES::\n            ...\n            \"\"\"\n```\nwhich is close enough to this patch not to warrant extra intrusion.",
    "created_at": "2013-01-04T01:42:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166728",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:16'>Comment 16:</a>
> Otherwise Developer's Guide builds and all of its doctests pass now (still with beta1 though), so positive review again if Karl-Dieter's ok with it, too.

Believe it or not, this does not apply to beta2.  The first hunk is now

```
        def __init__(self, S, m):
            """
            See ``HillCryptosystem`` for full documentation.

            EXAMPLES::
            ...
            """
```
which is close enough to this patch not to warrant extra intrusion.



---

archive/issue_comments_166729.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -5,3 +5,8 @@\n * Also, make indentation consistent and/or remove trailing spaces in some places.\n \n The release manager script has been updated to check all new patches for added TAB indentation.\n+\n+Apply:\n+* [attachment: 13899_TAB_sage_root.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_TAB_sage_root.patch.gz) to the root repository\n+* [and [attachment: trac_13899-doctest-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket13899/trac_13899-doctest-rebase.patch.gz)(https://github.com/sagemath/sage/files/ticket13899/bae42736953abaadde5a29d5dec4a42d.gz) to the Sage library\n+* [attachment: 13899_TAB_scripts.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_TAB_scripts.patch.gz) to the scripts repository\n``````\n",
    "created_at": "2013-01-04T01:45:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166729",
    "user": "https://github.com/kcrisman"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -5,3 +5,8 @@
 * Also, make indentation consistent and/or remove trailing spaces in some places.
 
 The release manager script has been updated to check all new patches for added TAB indentation.
+
+Apply:
+* [attachment: 13899_TAB_sage_root.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_TAB_sage_root.patch.gz) to the root repository
+* [and [attachment: trac_13899-doctest-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket13899/trac_13899-doctest-rebase.patch.gz)(https://github.com/sagemath/sage/files/ticket13899/bae42736953abaadde5a29d5dec4a42d.gz) to the Sage library
+* [attachment: 13899_TAB_scripts.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_TAB_scripts.patch.gz) to the scripts repository
``````




---

archive/issue_comments_166730.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nAttachment: **[trac_13899-doctest-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket13899/trac_13899-doctest-rebase.patch.gz)**\n\nPatchbot, apply 13899_TAB_sage_root.patch to root, 13899_TAB_sagelib.patch and trac_13899-doctest-rebase.patch to the Sage library, and 13899_TAB_scripts.patch to the scripts repository.\n\nI have NOT looked at the other patches, only the doctest fix.  I do give positive review to the doctest fix (with my trivial rebase).",
    "created_at": "2013-01-04T01:45:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166730",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:17'>Comment 17:</a>
Attachment: **[trac_13899-doctest-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket13899/trac_13899-doctest-rebase.patch.gz)**

Patchbot, apply 13899_TAB_sage_root.patch to root, 13899_TAB_sagelib.patch and trac_13899-doctest-rebase.patch to the Sage library, and 13899_TAB_scripts.patch to the scripts repository.

I have NOT looked at the other patches, only the doctest fix.  I do give positive review to the doctest fix (with my trivial rebase).



---

archive/issue_comments_166731.json:
```json
{
    "body": "<a id='comment:18'>Comment 18:</a>\nReplying to [@kcrisman](#comment%3A17):\n> Patchbot, apply 13899_TAB_sage_root.patch to root, 13899_TAB_sagelib.patch and trac_13899-doctest-rebase.patch to the Sage library, and 13899_TAB_scripts.patch to the scripts repository.\n> \n> I have NOT looked at the other patches, only the doctest fix.  I do give positive review to the doctest fix (with my trivial rebase).\n\nScratch that - I see what happened, Jeroen introduced the problem in his previous patch and then fixed it.  Now I have to wait for all those Cython files to rebuild to check how nice the developer guide looks... one moment.",
    "created_at": "2013-01-04T01:55:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166731",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:18'>Comment 18:</a>
Replying to [@kcrisman](#comment%3A17):
> Patchbot, apply 13899_TAB_sage_root.patch to root, 13899_TAB_sagelib.patch and trac_13899-doctest-rebase.patch to the Sage library, and 13899_TAB_scripts.patch to the scripts repository.
> 
> I have NOT looked at the other patches, only the doctest fix.  I do give positive review to the doctest fix (with my trivial rebase).

Scratch that - I see what happened, Jeroen introduced the problem in his previous patch and then fixed it.  Now I have to wait for all those Cython files to rebuild to check how nice the developer guide looks... one moment.



---

archive/issue_comments_166732.json:
```json
{
    "body": "<a id='comment:19'>Comment 19:</a>\nPatchbot, apply 13899_TAB_sage_root.patch to the root repository, 13899_TAB_sagelib.patch and 13899_doctest.patch to the Sage library, and 13899_TAB_scripts.patch to the scripts repository.\n\nOkay, Jeroen's doctest patch is fine.  Sorry for the confusion.",
    "created_at": "2013-01-04T01:56:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166732",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:19'>Comment 19:</a>
Patchbot, apply 13899_TAB_sage_root.patch to the root repository, 13899_TAB_sagelib.patch and 13899_doctest.patch to the Sage library, and 13899_TAB_scripts.patch to the scripts repository.

Okay, Jeroen's doctest patch is fine.  Sorry for the confusion.



---

archive/issue_comments_166733.json:
```json
{
    "body": "Changed reviewer from **Leif Leonhardy** to **Leif Leonhardy, Karl-Dieter Crisman**",
    "created_at": "2013-01-04T01:56:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166733",
    "user": "https://github.com/kcrisman"
}
```

Changed reviewer from **Leif Leonhardy** to **Leif Leonhardy, Karl-Dieter Crisman**



---

archive/issue_comments_166734.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -8,5 +8,5 @@\n \n Apply:\n * [attachment: 13899_TAB_sage_root.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_TAB_sage_root.patch.gz) to the root repository\n-* [and [attachment: trac_13899-doctest-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket13899/trac_13899-doctest-rebase.patch.gz)(https://github.com/sagemath/sage/files/ticket13899/bae42736953abaadde5a29d5dec4a42d.gz) to the Sage library\n+* [and [attachment: 13899_doctest.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_doctest.patch.gz)(https://github.com/sagemath/sage/files/ticket13899/bae42736953abaadde5a29d5dec4a42d.gz) to the Sage library\n * [attachment: 13899_TAB_scripts.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_TAB_scripts.patch.gz) to the scripts repository\n``````\n",
    "created_at": "2013-01-04T01:56:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166734",
    "user": "https://github.com/kcrisman"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -8,5 +8,5 @@
 
 Apply:
 * [attachment: 13899_TAB_sage_root.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_TAB_sage_root.patch.gz) to the root repository
-* [and [attachment: trac_13899-doctest-rebase.patch.gz](https://github.com/sagemath/sage/files/ticket13899/trac_13899-doctest-rebase.patch.gz)(https://github.com/sagemath/sage/files/ticket13899/bae42736953abaadde5a29d5dec4a42d.gz) to the Sage library
+* [and [attachment: 13899_doctest.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_doctest.patch.gz)(https://github.com/sagemath/sage/files/ticket13899/bae42736953abaadde5a29d5dec4a42d.gz) to the Sage library
 * [attachment: 13899_TAB_scripts.patch.gz](https://github.com/sagemath/sage/files/ticket13899/13899_TAB_scripts.patch.gz) to the scripts repository
``````




---

archive/issue_comments_166735.json:
```json
{
    "body": "<a id='comment:20'>Comment 20:</a>\nReplying to [@kcrisman](#comment%3A19):\n> Okay, Jeroen's doctest patch is fine.  Sorry for the confusion.\n\nSince leif already reviewed the other patches, I guess this means positive review then.",
    "created_at": "2013-01-04T09:14:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166735",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:20'>Comment 20:</a>
Replying to [@kcrisman](#comment%3A19):
> Okay, Jeroen's doctest patch is fine.  Sorry for the confusion.

Since leif already reviewed the other patches, I guess this means positive review then.



---

archive/issue_events_178968.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-04T09:14:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13899#event-178968"
}
```



---

archive/issue_events_178969.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-04T09:14:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13899#event-178969"
}
```



---

archive/issue_comments_166736.json:
```json
{
    "body": "Merged: **sage-5.6.beta3**",
    "created_at": "2013-01-07T20:58:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166736",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.6.beta3**



---

archive/issue_events_178970.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-07T20:58:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13899#event-178970"
}
```



---

archive/issue_events_178971.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-07T20:58:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13899#event-178971"
}
```



---

archive/issue_comments_166737.json:
```json
{
    "body": "<a id='comment:22'>Comment 22:</a>\nThis breaks the collection of 25 patches for IPython 0.13. Hopefully beautifying whitespace at least satisfies your OCD.",
    "created_at": "2013-01-13T14:36:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166737",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:22'>Comment 22:</a>
This breaks the collection of 25 patches for IPython 0.13. Hopefully beautifying whitespace at least satisfies your OCD.



---

archive/issue_comments_166738.json:
```json
{
    "body": "<a id='comment:23'>Comment 23:</a>\nReplying to [@vbraun](#comment%3A22):\n> Hopefully beautifying whitespace at least satisfies your OCD.\n\nAnd Guido van Rossums: [http://www.python.org/dev/peps/pep-0008/#indentation](http://www.python.org/dev/peps/pep-0008/#indentation)",
    "created_at": "2013-01-13T19:20:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166738",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:23'>Comment 23:</a>
Replying to [@vbraun](#comment%3A22):
> Hopefully beautifying whitespace at least satisfies your OCD.

And Guido van Rossums: [http://www.python.org/dev/peps/pep-0008/#indentation](http://www.python.org/dev/peps/pep-0008/#indentation)



---

archive/issue_comments_166739.json:
```json
{
    "body": "<a id='comment:24'>Comment 24:</a>\nWhew; -1 on massive whitespace changes like this unless they are actual errors because it breaks patches, which can be a lot of work to rebase.  I thought we had a whitespace beautifying discussion on sage-devel a while ago and we decided on the strategy of correcting whitespace as it was naturally encountered in code changes.  At least, that's what I thought.",
    "created_at": "2013-01-15T00:32:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166739",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:24'>Comment 24:</a>
Whew; -1 on massive whitespace changes like this unless they are actual errors because it breaks patches, which can be a lot of work to rebase.  I thought we had a whitespace beautifying discussion on sage-devel a while ago and we decided on the strategy of correcting whitespace as it was naturally encountered in code changes.  At least, that's what I thought.



---

archive/issue_comments_166740.json:
```json
{
    "body": "<a id='comment:25'>Comment 25:</a>\nI guess it's this that I have the most objection to: \"Also, make indentation consistent and/or remove trailing spaces in some places.\n\"",
    "created_at": "2013-01-15T00:33:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166740",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:25'>Comment 25:</a>
I guess it's this that I have the most objection to: "Also, make indentation consistent and/or remove trailing spaces in some places.
"



---

archive/issue_comments_166741.json:
```json
{
    "body": "<a id='comment:26'>Comment 26:</a>\nReplying to [@jasongrout](#comment%3A24):\n> Whew; -1 on massive whitespace changes like this unless they are actual errors because it breaks patches, which can be a lot of work to rebase.  I thought we had a whitespace beautifying discussion on sage-devel a while ago and we decided on the strategy of correcting whitespace as it was naturally encountered in code changes.  At least, that's what I thought.\n\nSorry, guys, I guess I should have caught this because I recall that discussion too.  (And implemented it or something similar in some cases - see my comment at #13255.)  I was just trying to help out with the doctest patch, but somehow didn't really think of the rest as it had been \"taken care of\", but that's no excuse.",
    "created_at": "2013-01-15T02:27:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166741",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:26'>Comment 26:</a>
Replying to [@jasongrout](#comment%3A24):
> Whew; -1 on massive whitespace changes like this unless they are actual errors because it breaks patches, which can be a lot of work to rebase.  I thought we had a whitespace beautifying discussion on sage-devel a while ago and we decided on the strategy of correcting whitespace as it was naturally encountered in code changes.  At least, that's what I thought.

Sorry, guys, I guess I should have caught this because I recall that discussion too.  (And implemented it or something similar in some cases - see my comment at #13255.)  I was just trying to help out with the doctest patch, but somehow didn't really think of the rest as it had been "taken care of", but that's no excuse.



---

archive/issue_comments_166742.json:
```json
{
    "body": "<a id='comment:27'>Comment 27:</a>\nI apologize for the trouble.  It's true that I'm usually against there changes, but I somehow thought (apparently a mistake) that this was mostly code which isn't often changed.\n\nJason: which is the patch you're having trouble with rebasing?",
    "created_at": "2013-01-15T07:27:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166742",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:27'>Comment 27:</a>
I apologize for the trouble.  It's true that I'm usually against there changes, but I somehow thought (apparently a mistake) that this was mostly code which isn't often changed.

Jason: which is the patch you're having trouble with rebasing?



---

archive/issue_comments_166743.json:
```json
{
    "body": "<a id='comment:28'>Comment 28:</a>\nReplying to [@jdemeyer](#comment%3A27):\n> I apologize for the trouble.  It's true that I'm usually against there changes, but I somehow thought (apparently a mistake) that this was mostly code which isn't often changed.\n\nTBH, I was pretty aware of the discussions in the past when reviewing this, but regarding the files touched, didn't expect major breakage -- just like Jereon.  So to avoid reopening the lengthy debates of the past, I tried to quickly review it... ;-)\n\n[And as mentioned, changing C/C++ and HTML files doesn't make that much sense to me, although those changes probably don't cause the trouble you may have now.]",
    "created_at": "2013-01-15T15:20:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166743",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:28'>Comment 28:</a>
Replying to [@jdemeyer](#comment%3A27):
> I apologize for the trouble.  It's true that I'm usually against there changes, but I somehow thought (apparently a mistake) that this was mostly code which isn't often changed.

TBH, I was pretty aware of the discussions in the past when reviewing this, but regarding the files touched, didn't expect major breakage -- just like Jereon.  So to avoid reopening the lengthy debates of the past, I tried to quickly review it... ;-)

[And as mentioned, changing C/C++ and HTML files doesn't make that much sense to me, although those changes probably don't cause the trouble you may have now.]



---

archive/issue_comments_166744.json:
```json
{
    "body": "<a id='comment:29'>Comment 29:</a>\nThe big set of patches that caught my attention and made me aware of this ticket was the IPython ticket, #12719, that Volker already rebased (see the comment above by him, which I think shows his frustration with having to rebase things).  I wouldn't be surprised if there were lots of other patches affected by some of the whitespace changes.",
    "created_at": "2013-01-15T16:11:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166744",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:29'>Comment 29:</a>
The big set of patches that caught my attention and made me aware of this ticket was the IPython ticket, #12719, that Volker already rebased (see the comment above by him, which I think shows his frustration with having to rebase things).  I wouldn't be surprised if there were lots of other patches affected by some of the whitespace changes.



---

archive/issue_comments_166745.json:
```json
{
    "body": "<a id='comment:30'>Comment 30:</a>\nFor the record, I would be +1 on reverting the parts of this patch that are touching lots of files for beauty's sake only (like removing trailing whitespace).  Large rebasing of tickets like #12719 could easily introduce subtle code errors, and not forcing those changes should take priority over end-of-line spacing.",
    "created_at": "2013-01-15T16:14:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166745",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:30'>Comment 30:</a>
For the record, I would be +1 on reverting the parts of this patch that are touching lots of files for beauty's sake only (like removing trailing whitespace).  Large rebasing of tickets like #12719 could easily introduce subtle code errors, and not forcing those changes should take priority over end-of-line spacing.



---

archive/issue_comments_166746.json:
```json
{
    "body": "<a id='comment:31'>Comment 31:</a>\nReplying to [@jasongrout](#comment%3A30):\n> For the record, I would be +1 on reverting the parts of this patch that are touching lots of files for beauty's sake only (like removing trailing whitespace)\n\nApart from the changes to `conventions.rst`, this patch is only about touching files for beauty's sake.  Given that #12719 is already rebased to this ticket, unmerging would be unwise. It seems that #12719 (which is a **huge** patch) conflicts with this in only one hunk, for a file which is removed in its totality.\n\nSo it's not a total disaster and it's not contradictory to leif's and my idea that these patches only touch rarely-edited files (except `conventions.rst` but that file really needed to be fixed).",
    "created_at": "2013-01-15T16:33:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166746",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:31'>Comment 31:</a>
Replying to [@jasongrout](#comment%3A30):
> For the record, I would be +1 on reverting the parts of this patch that are touching lots of files for beauty's sake only (like removing trailing whitespace)

Apart from the changes to `conventions.rst`, this patch is only about touching files for beauty's sake.  Given that #12719 is already rebased to this ticket, unmerging would be unwise. It seems that #12719 (which is a **huge** patch) conflicts with this in only one hunk, for a file which is removed in its totality.

So it's not a total disaster and it's not contradictory to leif's and my idea that these patches only touch rarely-edited files (except `conventions.rst` but that file really needed to be fixed).



---

archive/issue_comments_166747.json:
```json
{
    "body": "<a id='comment:32'>Comment 32:</a>\nReplying to [@jasongrout](#comment%3A30):\n> Large rebasing of tickets like #12719 could easily introduce subtle code errors, and not forcing those changes should take priority over end-of-line spacing.\n\nJust checked, and as far as I can see, the only offending patch here was removing an empty line from a file that #12719 removes in its entirety.  So I wouldn't say we broke much here, at least until now...\n\nIn general, patches IMHO shouldn't touch parts they don't have to (i.e., only doing some reformatting there), which we agreed on IIRC.  But in this case, the *purpose of the ticket* is to remove tabs (and \"normalize\" code -- once and for all -- hopefully), so it does *not* touch code it doesn't have to...",
    "created_at": "2013-01-15T16:46:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166747",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:32'>Comment 32:</a>
Replying to [@jasongrout](#comment%3A30):
> Large rebasing of tickets like #12719 could easily introduce subtle code errors, and not forcing those changes should take priority over end-of-line spacing.

Just checked, and as far as I can see, the only offending patch here was removing an empty line from a file that #12719 removes in its entirety.  So I wouldn't say we broke much here, at least until now...

In general, patches IMHO shouldn't touch parts they don't have to (i.e., only doing some reformatting there), which we agreed on IIRC.  But in this case, the *purpose of the ticket* is to remove tabs (and "normalize" code -- once and for all -- hopefully), so it does *not* touch code it doesn't have to...



---

archive/issue_comments_166748.json:
```json
{
    "body": "<a id='comment:33'>Comment 33:</a>\nAutomatically enforcing code style is nice but should only be done though local commit hooks. Having a check in Jeroen's release script is BS, that just adds a completely avoidable multi-week ping-pong game when trying to merge a ticket.\n\nIn any case this ticket is done so lets leave it at that. But please post to sage-devel next time so we can talk you out of it ;-)",
    "created_at": "2013-01-15T17:19:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13899",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13899#issuecomment-166748",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:33'>Comment 33:</a>
Automatically enforcing code style is nice but should only be done though local commit hooks. Having a check in Jeroen's release script is BS, that just adds a completely avoidable multi-week ping-pong game when trying to merge a ticket.

In any case this ticket is done so lets leave it at that. But please post to sage-devel next time so we can talk you out of it ;-)
