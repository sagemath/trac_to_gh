# Issue 13285: add mpmath library to sage.misc.citations module

archive/issues_013113.json:
```json
{
    "body": "The mpmath library which is used now in many cases for the numerical evaluation of special functions in Sage, is not listed in the `sage.misc.citations` module. This should be fixed.\n\nAssignee: @burcin\n\nKeywords: mpmath citation\n\nAuthor: Benjamin Jones\n\nIssue created by migration from https://trac.sagemath.org/ticket/13285\n\n",
    "created_at": "2012-07-23T18:16:10Z",
    "labels": [
        "component: misc",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.4",
    "title": "add mpmath library to sage.misc.citations module",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/13285",
    "user": "https://github.com/benjaminfjones"
}
```
The mpmath library which is used now in many cases for the numerical evaluation of special functions in Sage, is not listed in the `sage.misc.citations` module. This should be fixed.

Assignee: @burcin

Keywords: mpmath citation

Author: Benjamin Jones

Issue created by migration from https://trac.sagemath.org/ticket/13285





---

archive/issue_comments_159315.json:
```json
{
    "body": "add mpmath library to sage.misc.citation",
    "created_at": "2012-07-23T18:18:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13285",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13285#issuecomment-159315",
    "user": "https://github.com/benjaminfjones"
}
```

add mpmath library to sage.misc.citation



---

archive/issue_comments_159316.json:
```json
{
    "body": "<a id='comment:1'></a>Attachment [trac_13285_mpmath_citation.patch](tarball://root/attachments/some-uuid/ticket13285/trac_13285_mpmath_citation.patch) by @kcrisman created at 2012-07-25 03:36:02\n\nHmm, good idea.  How would I test this?  Do `get_systems` on evaluating something that does it with mpmath?  Should we doctest this, even?",
    "created_at": "2012-07-25T03:36:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13285",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13285#issuecomment-159316",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:1'></a>Attachment [trac_13285_mpmath_citation.patch](tarball://root/attachments/some-uuid/ticket13285/trac_13285_mpmath_citation.patch) by @kcrisman created at 2012-07-25 03:36:02

Hmm, good idea.  How would I test this?  Do `get_systems` on evaluating something that does it with mpmath?  Should we doctest this, even?



---

archive/issue_comments_159317.json:
```json
{
    "body": "<a id='comment:2'></a>Yes, I should add a doctest. With the patch applied, I tested:\n\n```\nsage: from sage.misc.citation import get_systems\nsage: get_systems('lambert_w(1).n()')\n['mpmath', 'ginac', 'GMP']\n```",
    "created_at": "2012-07-25T04:46:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13285",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13285#issuecomment-159317",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:2'></a>Yes, I should add a doctest. With the patch applied, I tested:

```
sage: from sage.misc.citation import get_systems
sage: get_systems('lambert_w(1).n()')
['mpmath', 'ginac', 'GMP']
```



---

archive/issue_comments_159318.json:
```json
{
    "body": "Attachment [trac_13285_mpmath_citation.2.patch](tarball://root/attachments/some-uuid/ticket13285/trac_13285_mpmath_citation.2.patch) by @benjaminfjones created at 2012-07-25 04:47:02\n\nadded a doctest",
    "created_at": "2012-07-25T04:47:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13285",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13285#issuecomment-159318",
    "user": "https://github.com/benjaminfjones"
}
```

Attachment [trac_13285_mpmath_citation.2.patch](tarball://root/attachments/some-uuid/ticket13285/trac_13285_mpmath_citation.2.patch) by @benjaminfjones created at 2012-07-25 04:47:02

added a doctest



---

archive/issue_events_036748.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13285",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13285#event-36748"
}
```



---

archive/issue_events_036749.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13285",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13285#event-36749"
}
```



---

archive/issue_events_036750.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13285",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13285#event-36750"
}
```



---

archive/issue_events_036751.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13285",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13285#event-36751"
}
```



---

archive/issue_events_036752.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13285",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13285#event-36752"
}
```



---

archive/issue_events_036753.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13285",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13285#event-36753"
}
```



---

archive/issue_events_036754.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13285",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13285#event-36754"
}
```



---

archive/issue_comments_159319.json:
```json
{
    "body": "Changing component from symbolics to misc.",
    "created_at": "2015-02-01T07:04:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13285",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13285#issuecomment-159319",
    "user": "https://github.com/rwst"
}
```

Changing component from symbolics to misc.



---

archive/issue_comments_159320.json:
```json
{
    "body": "Changing assignee from @burcin to @jasongrout.",
    "created_at": "2015-02-01T07:04:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13285",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13285#issuecomment-159320",
    "user": "https://github.com/rwst"
}
```

Changing assignee from @burcin to @jasongrout.



---

archive/issue_comments_159321.json:
```json
{
    "body": "Changing assignee from @jasongrout to @burcin.",
    "created_at": "2015-02-01T07:05:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13285",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13285#issuecomment-159321",
    "user": "https://github.com/rwst"
}
```

Changing assignee from @jasongrout to @burcin.
