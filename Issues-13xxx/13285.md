# Issue 13285: add mpmath library to sage.misc.citations module

archive/issues_013113.json:
```json
{
    "assignees": [],
    "body": "The mpmath library which is used now in many cases for the numerical evaluation of special functions in Sage, is not listed in the `sage.misc.citations` module. This should be fixed.\n\nAssignee: **@burcin**\n\nKeywords: **mpmath citation**\n\nAuthor: **Benjamin Jones**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13285_\n\n",
    "created_at": "2012-07-23T18:16:10Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "add mpmath library to sage.misc.citations module",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/13285",
    "user": "https://github.com/benjaminfjones"
}
```
The mpmath library which is used now in many cases for the numerical evaluation of special functions in Sage, is not listed in the `sage.misc.citations` module. This should be fixed.

Assignee: **@burcin**

Keywords: **mpmath citation**

Author: **Benjamin Jones**

_Issue created by migration from https://trac.sagemath.org/ticket/13285_





---

archive/issue_events_169708.json:
```json
{
    "actor": "https://github.com/benjaminfjones",
    "created_at": "2012-07-23T18:16:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13285",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13285#event-169708"
}
```



---

archive/issue_events_169709.json:
```json
{
    "actor": "https://github.com/benjaminfjones",
    "created_at": "2012-07-23T18:16:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13285",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13285#event-169709"
}
```



---

archive/issue_events_169710.json:
```json
{
    "actor": "https://github.com/benjaminfjones",
    "created_at": "2012-07-23T18:16:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13285",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13285#event-169710"
}
```



---

archive/issue_events_169711.json:
```json
{
    "actor": "https://github.com/benjaminfjones",
    "created_at": "2012-07-23T18:16:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13285",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13285#event-169711"
}
```



---

archive/issue_comments_155818.json:
```json
{
    "body": "add mpmath library to sage.misc.citation",
    "created_at": "2012-07-23T18:18:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13285",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13285#issuecomment-155818",
    "user": "https://github.com/benjaminfjones"
}
```

add mpmath library to sage.misc.citation



---

archive/issue_comments_155819.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[trac_13285_mpmath_citation.patch.gz](https://github.com/sagemath/sage/files/ticket13285/trac_13285_mpmath_citation.patch.gz)**\n\nHmm, good idea.  How would I test this?  Do `get_systems` on evaluating something that does it with mpmath?  Should we doctest this, even?",
    "created_at": "2012-07-25T03:36:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13285",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13285#issuecomment-155819",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[trac_13285_mpmath_citation.patch.gz](https://github.com/sagemath/sage/files/ticket13285/trac_13285_mpmath_citation.patch.gz)**

Hmm, good idea.  How would I test this?  Do `get_systems` on evaluating something that does it with mpmath?  Should we doctest this, even?



---

archive/issue_comments_155820.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nYes, I should add a doctest. With the patch applied, I tested:\n\n```\nsage: from sage.misc.citation import get_systems\nsage: get_systems('lambert_w(1).n()')\n['mpmath', 'ginac', 'GMP']\n```",
    "created_at": "2012-07-25T04:46:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13285",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13285#issuecomment-155820",
    "user": "https://github.com/benjaminfjones"
}
```

<a id='comment:2'>Comment 2:</a>
Yes, I should add a doctest. With the patch applied, I tested:

```
sage: from sage.misc.citation import get_systems
sage: get_systems('lambert_w(1).n()')
['mpmath', 'ginac', 'GMP']
```



---

archive/issue_comments_155821.json:
```json
{
    "body": "Attachment: **[trac_13285_mpmath_citation.2.patch.gz](https://github.com/sagemath/sage/files/ticket13285/trac_13285_mpmath_citation.2.patch.gz)**\n\nadded a doctest",
    "created_at": "2012-07-25T04:47:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13285",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13285#issuecomment-155821",
    "user": "https://github.com/benjaminfjones"
}
```

Attachment: **[trac_13285_mpmath_citation.2.patch.gz](https://github.com/sagemath/sage/files/ticket13285/trac_13285_mpmath_citation.2.patch.gz)**

added a doctest



---

archive/issue_events_169712.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13285",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13285#event-169712"
}
```



---

archive/issue_events_169713.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13285",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13285#event-169713"
}
```



---

archive/issue_events_169714.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13285",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13285#event-169714"
}
```



---

archive/issue_events_169715.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13285",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13285#event-169715"
}
```



---

archive/issue_events_169716.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13285",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13285#event-169716"
}
```



---

archive/issue_events_169717.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13285",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13285#event-169717"
}
```



---

archive/issue_events_169718.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13285",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13285#event-169718"
}
```



---

archive/issue_events_169719.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13285",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13285#event-169719"
}
```



---

archive/issue_events_169720.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2015-02-01T07:04:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13285",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13285#event-169720"
}
```



---

archive/issue_comments_155822.json:
```json
{
    "body": "Changed assignee from **@burcin** to **@jasongrout**",
    "created_at": "2015-02-01T07:04:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13285",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13285#issuecomment-155822",
    "user": "https://github.com/rwst"
}
```

Changed assignee from **@burcin** to **@jasongrout**



---

archive/issue_comments_155823.json:
```json
{
    "body": "Changed assignee from **@jasongrout** to **@burcin**",
    "created_at": "2015-02-01T07:05:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13285",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13285#issuecomment-155823",
    "user": "https://github.com/rwst"
}
```

Changed assignee from **@jasongrout** to **@burcin**



---

archive/issue_events_169721.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/13285",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13285#event-169721"
}
```
