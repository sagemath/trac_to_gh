# Issue 13803: Python binding to limits.h

archive/issues_013599.json:
```json
{
    "body": "Prior to #12224 we need to know what C types limits. The patch provides a direct Python access to limits.h.\n\nAssignee: @jasongrout\n\nCC:  @staroste\n\nKeywords: limits, C datatype\n\nReviewer: Leif Leonhardy, Vincent Delecroix\n\nResolution: wontfix\n\nIssue created by migration from https://trac.sagemath.org/ticket/13803\n\n",
    "closed_at": "2013-06-10T08:47:37Z",
    "created_at": "2012-12-06T09:45:12Z",
    "labels": [
        "component: misc"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "Python binding to limits.h",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/13803",
    "user": "https://github.com/videlec"
}
```
Prior to #12224 we need to know what C types limits. The patch provides a direct Python access to limits.h.

Assignee: @jasongrout

CC:  @staroste

Keywords: limits, C datatype

Reviewer: Leif Leonhardy, Vincent Delecroix

Resolution: wontfix

Issue created by migration from https://trac.sagemath.org/ticket/13803





---

archive/issue_comments_209542.json:
```json
{
    "body": "<a id='comment:1'></a>Attachment [trac_13803-limits.patch](tarball://root/attachments/some-uuid/ticket13803/trac_13803-limits.patch) by @videlec created at 2012-12-06 09:47:52",
    "created_at": "2012-12-06T09:47:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13803#issuecomment-209542",
    "user": "https://github.com/videlec"
}
```

<a id='comment:1'></a>Attachment [trac_13803-limits.patch](tarball://root/attachments/some-uuid/ticket13803/trac_13803-limits.patch) by @videlec created at 2012-12-06 09:47:52



---

archive/issue_comments_209543.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2012-12-06T09:47:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13803#issuecomment-209543",
    "user": "https://github.com/videlec"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_209544.json:
```json
{
    "body": "<a id='comment:3'></a>`s/Mimimum/Minimum/` (once)\n\n`s/a C long long max/a C long long int/`\n\nNot sure whether the doctests make sense when they're all tagged `# random`...\n\nWe also have things like `sys.maxint`, a couple of (size-related) definitions from `distutils.sysconfig.get_config_var[s]()`, and `os.sysconf()`.\n\nThe values don't vary much on the platforms *supported by Sage*; AFAIK there are currently only differences depending on whether we're on a 32- or 64-bit operating system, or, more precisely, whether Sage *was built* 32-bit or 64-bit (as one could in principle run a 32-bit binary dist on a 64-bit OS, or even build and run a 32-bit version on a 64-bit OS, e.g. Solaris).  It would IMHO make sense to get a doctest error in case some results are unexpected.",
    "created_at": "2012-12-06T11:13:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13803#issuecomment-209544",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:3'></a>`s/Mimimum/Minimum/` (once)

`s/a C long long max/a C long long int/`

Not sure whether the doctests make sense when they're all tagged `# random`...

We also have things like `sys.maxint`, a couple of (size-related) definitions from `distutils.sysconfig.get_config_var[s]()`, and `os.sysconf()`.

The values don't vary much on the platforms *supported by Sage*; AFAIK there are currently only differences depending on whether we're on a 32- or 64-bit operating system, or, more precisely, whether Sage *was built* 32-bit or 64-bit (as one could in principle run a 32-bit binary dist on a 64-bit OS, or even build and run a 32-bit version on a 64-bit OS, e.g. Solaris).  It would IMHO make sense to get a doctest error in case some results are unexpected.



---

archive/issue_comments_209545.json:
```json
{
    "body": "<a id='comment:4'></a>P.S.: In the docstrings, I'd typeset the C types monospaced, e.g. ```unsigned long int```.",
    "created_at": "2012-12-06T11:24:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13803#issuecomment-209545",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:4'></a>P.S.: In the docstrings, I'd typeset the C types monospaced, e.g. ```unsigned long int```.



---

archive/issue_comments_209546.json:
```json
{
    "body": "<a id='comment:5'></a>P.P.S.:\n\n`LLONG_*` and `ULLONG_*` of course require C99, so you have to add `extra_compile_args = ['-std=c99']` to the `module_list.py` entry.  (There are systems on which GCC does *not* default to `-std=gnu99`.)",
    "created_at": "2012-12-06T11:40:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13803#issuecomment-209546",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:5'></a>P.P.S.:

`LLONG_*` and `ULLONG_*` of course require C99, so you have to add `extra_compile_args = ['-std=c99']` to the `module_list.py` entry.  (There are systems on which GCC does *not* default to `-std=gnu99`.)



---

archive/issue_comments_209547.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Leif Leonhardy\"",
    "created_at": "2012-12-06T11:40:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13803#issuecomment-209547",
    "user": "https://github.com/nexttime"
}
```

Changing reviewer from "" to "Leif Leonhardy"



---

archive/issue_comments_209548.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2012-12-06T11:40:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13803#issuecomment-209548",
    "user": "https://github.com/nexttime"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_209549.json:
```json
{
    "body": "<a id='comment:6'></a>Thanks very much (I am not at all a specialist but I need to call char_max() in some constructor).\n\nIs the following a safe test ?\n\n```\nsage: from sage.misc.limits import int_min\nsage: int_min() # random (architecture and platform dependence)\n-2147483648\nsage: import sysconfig\nsage: s = sysconfig.get_config_var('SIZEOF_INT')\nsage: int_min() == - 2**(s*8-1)\nTrue\n```\n\nBest,\nVincent",
    "created_at": "2012-12-06T12:38:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13803#issuecomment-209549",
    "user": "https://github.com/videlec"
}
```

<a id='comment:6'></a>Thanks very much (I am not at all a specialist but I need to call char_max() in some constructor).

Is the following a safe test ?

```
sage: from sage.misc.limits import int_min
sage: int_min() # random (architecture and platform dependence)
-2147483648
sage: import sysconfig
sage: s = sysconfig.get_config_var('SIZEOF_INT')
sage: int_min() == - 2**(s*8-1)
True
```

Best,
Vincent



---

archive/issue_comments_209550.json:
```json
{
    "body": "<a id='comment:7'></a>It seems that this would be a valuable upstream Cython contribution as well.\n\nIn fact, isn't this it?  https://github.com/cython/cython/blob/master/Cython/Includes/libc/limits.pxd\n\nMaybe this patch would be cleaner if you did:\n\n```\nfrom libc cimport limits\n```\nat the top and then used limits.<whatever>?  Or maybe not.  It's a suggestion.",
    "created_at": "2012-12-06T12:43:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13803#issuecomment-209550",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:7'></a>It seems that this would be a valuable upstream Cython contribution as well.

In fact, isn't this it?  https://github.com/cython/cython/blob/master/Cython/Includes/libc/limits.pxd

Maybe this patch would be cleaner if you did:

```
from libc cimport limits
```
at the top and then used limits.<whatever>?  Or maybe not.  It's a suggestion.



---

archive/issue_comments_209551.json:
```json
{
    "body": "<a id='comment:8'></a>It seems that #12224 doesn't require this, nor does it seem needed since it's in Cython as Jason mentioned.",
    "created_at": "2013-06-07T13:05:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13803#issuecomment-209551",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:8'></a>It seems that #12224 doesn't require this, nor does it seem needed since it's in Cython as Jason mentioned.



---

archive/issue_comments_209552.json:
```json
{
    "body": "<a id='comment:9'></a>Replying to [comment:8 jdemeyer]:\n> It seems that #12224 doesn't require this, nor does it seem needed since it's in Cython as Jason mentioned.\n\n\nYou are right. Not anymore.",
    "created_at": "2013-06-07T13:09:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13803#issuecomment-209552",
    "user": "https://github.com/videlec"
}
```

<a id='comment:9'></a>Replying to [comment:8 jdemeyer]:
> It seems that #12224 doesn't require this, nor does it seem needed since it's in Cython as Jason mentioned.


You are right. Not anymore.



---

archive/issue_events_038687.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2013-06-07T13:09:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/13803",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13803#event-38687"
}
```



---

archive/issue_comments_209553.json:
```json
{
    "body": "Changing status from needs_work to positive_review.",
    "created_at": "2013-06-07T14:15:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13803#issuecomment-209553",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_work to positive_review.



---

archive/issue_comments_209554.json:
```json
{
    "body": "Changing author from \"Vincent Delecroix\" to \"\"",
    "created_at": "2013-06-07T14:15:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13803#issuecomment-209554",
    "user": "https://github.com/jdemeyer"
}
```

Changing author from "Vincent Delecroix" to ""



---

archive/issue_comments_209555.json:
```json
{
    "body": "Changing reviewer from \"Leif Leonhardy\" to \"Leif Leonhardy, Vincent Delecroix\"",
    "created_at": "2013-06-07T14:15:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13803#issuecomment-209555",
    "user": "https://github.com/jdemeyer"
}
```

Changing reviewer from "Leif Leonhardy" to "Leif Leonhardy, Vincent Delecroix"



---

archive/issue_events_038688.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-10T08:47:37Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/13803",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/13803#event-38688"
}
```



---

archive/issue_comments_209556.json:
```json
{
    "body": "Resolution: wontfix",
    "created_at": "2013-06-10T08:47:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/13803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/13803#issuecomment-209556",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: wontfix
