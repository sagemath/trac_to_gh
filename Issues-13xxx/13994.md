# Issue 13994: Implementation of EL labellings of posets

archive/issues_013790.json:
```json
{
    "assignees": [],
    "body": "This patch implements a method to check if a given function on cover relations of a poset is an EL labelling.\n\nAssignee: **sage-combinat**\n\nCC:  @fchapoton\n\nKeywords: **poset, EL labelling**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Christian Stump**\n\nMerged: **sage-5.8.beta3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/13994_\n\n",
    "closed_at": "2013-03-06T10:26:31Z",
    "created_at": "2013-01-23T12:30:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Implementation of EL labellings of posets",
    "type": "issue",
    "updated_at": "2013-03-06T10:26:31Z",
    "url": "https://github.com/sagemath/sage/issues/13994",
    "user": "https://github.com/sagetrac-stumpc5"
}
```
This patch implements a method to check if a given function on cover relations of a poset is an EL labelling.

Assignee: **sage-combinat**

CC:  @fchapoton

Keywords: **poset, EL labelling**

Reviewer: **Frédéric Chapoton**

Author: **Christian Stump**

Merged: **sage-5.8.beta3**

_Issue created by migration from https://trac.sagemath.org/ticket/13994_





---

archive/issue_events_180330.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2013-01-23T12:30:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/13994",
    "milestone_number": null,
    "milestone_title": "sage-5.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13994#event-180330"
}
```



---

archive/issue_events_180331.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2013-01-23T12:30:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13994",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13994#event-180331"
}
```



---

archive/issue_events_180332.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2013-01-23T12:30:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13994",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13994#event-180332"
}
```



---

archive/issue_events_180333.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2013-01-23T12:30:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13994",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13994#event-180333"
}
```



---

archive/issue_events_180334.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2013-01-23T12:32:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13994",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13994#event-180334"
}
```



---

archive/issue_comments_168245.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nHello,\n\nmaybe you could \n\n* add a reference for the definition\n* explain the meaning of the word EL",
    "created_at": "2013-03-03T11:58:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13994",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13994#issuecomment-168245",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:3'>Comment 3:</a>
Hello,

maybe you could 

* add a reference for the definition
* explain the meaning of the word EL



---

archive/issue_comments_168246.json:
```json
{
    "body": "Attachment: **[trac_13994-is_el_shelling-cs.patch.gz](https://github.com/sagemath/sage/files/ticket13994/trac_13994-is_el_shelling-cs.patch.gz)**",
    "created_at": "2013-03-04T13:55:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13994",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13994#issuecomment-168246",
    "user": "https://github.com/sagetrac-stumpc5"
}
```

Attachment: **[trac_13994-is_el_shelling-cs.patch.gz](https://github.com/sagemath/sage/files/ticket13994/trac_13994-is_el_shelling-cs.patch.gz)**



---

archive/issue_comments_168247.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nReplying to [@fchapoton](#comment%3A3):\n> * add a reference for the definition\n> * explain the meaning of the word EL\n\nDone!",
    "created_at": "2013-03-04T16:31:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13994",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13994#issuecomment-168247",
    "user": "https://github.com/sagetrac-stumpc5"
}
```

<a id='comment:4'>Comment 4:</a>
Replying to [@fchapoton](#comment%3A3):
> * add a reference for the definition
> * explain the meaning of the word EL

Done!



---

archive/issue_comments_168248.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nHello Christian,\n\nI have made some minor changes in the doc in my review patch. If you are happy with my review patch, you can set this ticket to positive review.",
    "created_at": "2013-03-04T21:26:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13994",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13994#issuecomment-168248",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:5'>Comment 5:</a>
Hello Christian,

I have made some minor changes in the doc in my review patch. If you are happy with my review patch, you can set this ticket to positive review.



---

archive/issue_comments_168249.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nReplying to [@fchapoton](#comment%3A5):\n> Hello Christian,\n> \n> I have made some minor changes in the doc in my review patch. If you are happy with my review patch, you can set this ticket to positive review.\n\nI am fine with your changes (though I do never understand why some editors introduce line breaks after 80 cols, while I would prefer that anyone can decide him-/herself where to break the lines in his/her editor).\n\nThanks for the review!",
    "created_at": "2013-03-05T07:52:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13994",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13994#issuecomment-168249",
    "user": "https://github.com/sagetrac-stumpc5"
}
```

<a id='comment:6'>Comment 6:</a>
Replying to [@fchapoton](#comment%3A5):
> Hello Christian,
> 
> I have made some minor changes in the doc in my review patch. If you are happy with my review patch, you can set this ticket to positive review.

I am fine with your changes (though I do never understand why some editors introduce line breaks after 80 cols, while I would prefer that anyone can decide him-/herself where to break the lines in his/her editor).

Thanks for the review!



---

archive/issue_comments_168250.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2013-03-05T07:52:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13994",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13994#issuecomment-168250",
    "user": "https://github.com/sagetrac-stumpc5"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_events_180335.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2013-03-05T07:52:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13994",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13994#event-180335"
}
```



---

archive/issue_events_180336.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2013-03-05T07:52:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/13994",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13994#event-180336"
}
```



---

archive/issue_comments_168251.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nThe second patch needs a proper commit message.",
    "created_at": "2013-03-05T10:41:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13994",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13994#issuecomment-168251",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:7'>Comment 7:</a>
The second patch needs a proper commit message.



---

archive/issue_comments_168252.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nAttachment: **[trac_13994-review-fc.patch.gz](https://github.com/sagemath/sage/files/ticket13994/trac_13994-review-fc.patch.gz)**\n\nI have added the commit message.",
    "created_at": "2013-03-05T18:49:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13994",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13994#issuecomment-168252",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:8'>Comment 8:</a>
Attachment: **[trac_13994-review-fc.patch.gz](https://github.com/sagemath/sage/files/ticket13994/trac_13994-review-fc.patch.gz)**

I have added the commit message.



---

archive/issue_comments_168253.json:
```json
{
    "body": "Merged: **sage-5.8.beta3**",
    "created_at": "2013-03-06T10:26:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/13994",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/13994#issuecomment-168253",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.8.beta3**



---

archive/issue_events_180337.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-06T10:26:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/13994",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13994#event-180337"
}
```



---

archive/issue_events_180338.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-06T10:26:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/13994",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/13994#event-180338"
}
```
