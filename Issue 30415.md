# Issue 30415: Use Arb to evaluate polynomials with integer/rational coefficients at balls

archive/issues_030415.json:
```json
{
    "body": "Interface the arb functions `arb_fmpz_poly_evaluate_*()` for evaluating p at x when p \u2208 \u2124[x] and x \u2208 RBF or CBF.\n\nIssue created by migration from https://trac.sagemath.org/ticket/30652\n\n",
    "closed_at": "2020-10-31T18:07:28Z",
    "created_at": "2020-09-24T12:49:44Z",
    "labels": [
        "component: numerical"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.3",
    "title": "Use Arb to evaluate polynomials with integer/rational coefficients at balls",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/30415",
    "user": "https://github.com/mezzarobba"
}
```
Interface the arb functions `arb_fmpz_poly_evaluate_*()` for evaluating p at x when p ∈ ℤ[x] and x ∈ RBF or CBF.

Issue created by migration from https://trac.sagemath.org/ticket/30652





---

archive/issue_comments_433405.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2020-09-24T13:22:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30415#issuecomment-433405",
    "user": "https://github.com/mezzarobba"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_433406.json:
```json
{
    "body": "(At first I wanted to wrap `arb_fmpz_poly_complex_roots` as well, but that requires more refactoring of the generic `roots()` method that I have the bandwidth for right now.)\n\n---\nNew commits:",
    "created_at": "2020-09-24T13:22:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30415#issuecomment-433406",
    "user": "https://github.com/mezzarobba"
}
```

(At first I wanted to wrap `arb_fmpz_poly_complex_roots` as well, but that requires more refactoring of the generic `roots()` method that I have the bandwidth for right now.)

---
New commits:



---

archive/issue_comments_433407.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2020-09-24T13:48:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30415#issuecomment-433407",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_433408.json:
```json
{
    "body": "good to go",
    "created_at": "2020-10-10T21:19:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30415#issuecomment-433408",
    "user": "https://github.com/videlec"
}
```

good to go



---

archive/issue_events_079735.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2020-10-10T21:19:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/30415",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30415#event-79735"
}
```



---

archive/issue_comments_433409.json:
```json
{
    "body": "thnak you!",
    "created_at": "2020-10-11T07:28:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30415#issuecomment-433409",
    "user": "https://github.com/mezzarobba"
}
```

thnak you!



---

archive/issue_comments_433410.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2020-10-12T20:47:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30415#issuecomment-433410",
    "user": "https://github.com/videlec"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_433411.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2020-10-31T18:07:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/30415",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/30415#issuecomment-433411",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_events_079736.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-10-31T18:07:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/30415",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/30415#event-79736"
}
```
