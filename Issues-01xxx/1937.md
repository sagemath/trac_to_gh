# Issue 1937: [with doc patch, with positive review] fix LLL documentation

archive/issues_001937.json:
```json
{
    "body": "\n```\n[10:42] <malb> sage: A.LLL?\n...\n[10:44] <craigcitro> what's mu_{i+1,i} ?\n```\n\nAssignee: tba\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/1937\n\n",
    "closed_at": "2008-01-26T11:19:15Z",
    "created_at": "2008-01-26T10:48:58Z",
    "labels": [
        "component: documentation",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.10.1",
    "title": "[with doc patch, with positive review] fix LLL documentation",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/1937",
    "user": "https://github.com/malb"
}
```

```
[10:42] <malb> sage: A.LLL?
...
[10:44] <craigcitro> what's mu_{i+1,i} ?
```

Assignee: tba

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/1937





---

archive/issue_comments_012792.json:
```json
{
    "body": "Attachment [trac1937_LLL_doc.patch](tarball://root/attachments/some-uuid/ticket1937/trac1937_LLL_doc.patch) by @malb created at 2008-01-26 11:01:57",
    "created_at": "2008-01-26T11:01:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1937",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1937#issuecomment-12792",
    "user": "https://github.com/malb"
}
```

Attachment [trac1937_LLL_doc.patch](tarball://root/attachments/some-uuid/ticket1937/trac1937_LLL_doc.patch) by @malb created at 2008-01-26 11:01:57



---

archive/issue_events_006576.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1937",
    "rename": {
        "from": "fix LLL documentation",
        "to": "[with doc patch, needs review] fix LLL documentation"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1937#event-6576"
}
```



---

archive/issue_events_006577.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1937",
    "rename": {
        "from": "[with doc patch, needs review] fix LLL documentation",
        "to": "[with doc patch, with positive review] fix LLL documentation"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1937#event-6577"
}
```



---

archive/issue_comments_012793.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 2.10.1.rc0",
    "created_at": "2008-01-26T11:19:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1937",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1937#issuecomment-12793",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 2.10.1.rc0



---

archive/issue_events_006578.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-01-26T11:19:15Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/1937",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1937#event-6578"
}
```



---

archive/issue_comments_012794.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-01-26T11:19:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1937",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1937#issuecomment-12794",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
