# Issue 1061: fix python compile on OSX 10.5

archive/issues_001061.json:
```json
{
    "assignees": [],
    "body": "I've posted a patched python spkg here:\n\nhttp://sage.math.washington.edu/tmp/python-2.5.1.p8.spkg\n\n--William\n\nAssignee: **mabshoff**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1061_\n\n",
    "closed_at": "2007-11-02T01:48:31Z",
    "created_at": "2007-11-02T01:30:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.11",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix python compile on OSX 10.5",
    "type": "issue",
    "updated_at": "2007-11-02T01:48:31Z",
    "url": "https://github.com/sagemath/sage/issues/1061",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
I've posted a patched python spkg here:

http://sage.math.washington.edu/tmp/python-2.5.1.p8.spkg

--William

Assignee: **mabshoff**

_Issue created by migration from https://trac.sagemath.org/ticket/1061_





---

archive/issue_events_008942.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-02T01:30:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1061",
    "milestone_number": null,
    "milestone_title": "sage-2.8.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1061#event-8942"
}
```



---

archive/issue_events_008943.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-02T01:30:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1061",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting",
    "label_color": "000080",
    "label_name": "component: porting",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1061#event-8943"
}
```



---

archive/issue_events_008944.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-02T01:30:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1061",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1061#event-8944"
}
```



---

archive/issue_events_008945.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-02T01:30:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1061",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1061#event-8945"
}
```



---

archive/issue_comments_004895.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\napplied to 2.8.11.rc1 - via with new python-2.5.1.p8.spkg",
    "created_at": "2007-11-02T01:48:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1061",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1061#issuecomment-4895",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:1'>Comment 1:</a>
applied to 2.8.11.rc1 - via with new python-2.5.1.p8.spkg



---

archive/issue_events_008946.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-02T01:48:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1061",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1061#event-8946"
}
```
