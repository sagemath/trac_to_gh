# Issue 1182: update freetype.spkg to 2.3.5

archive/issues_001182.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "William did mention it somewhere on sage-devel and I cannot find the reference, but the freetype spkg we ship is very out of date. So update to the current (2.3.5 at the moment of this bug report) release.\n\nCheers,\n\nMichael\n\nComponent: **packages: standard**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1182_\n\n",
    "closed_at": "2007-12-09T10:06:57Z",
    "created_at": "2007-11-16T02:22:09Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "update freetype.spkg to 2.3.5",
    "type": "issue",
    "updated_at": "2007-12-09T10:06:57Z",
    "url": "https://github.com/sagemath/sage/issues/1182",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
William did mention it somewhere on sage-devel and I cannot find the reference, but the freetype spkg we ship is very out of date. So update to the current (2.3.5 at the moment of this bug report) release.

Cheers,

Michael

Component: **packages: standard**

_Issue created by migration from https://trac.sagemath.org/ticket/1182_





---

archive/issue_events_011411.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-16T02:22:09Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1182",
    "milestone_number": null,
    "milestone_title": "sage-2.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1182#event-11411"
}
```



---

archive/issue_events_011412.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-16T02:22:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1182",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1182#event-11412"
}
```



---

archive/issue_events_011413.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-16T02:22:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1182",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1182#event-11413"
}
```



---

archive/issue_events_011414.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-16T02:22:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1182",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1182#event-11414"
}
```



---

archive/issue_events_011415.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-16T02:22:09Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1182",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1182#event-11415"
}
```



---

archive/issue_events_011416.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-04T14:36:21Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/1182",
    "milestone_number": null,
    "milestone_title": "sage-2.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1182#event-11416"
}
```



---

archive/issue_events_011417.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-04T14:36:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1182",
    "milestone_number": null,
    "milestone_title": "sage-2.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1182#event-11417"
}
```



---

archive/issue_comments_005264.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nI've put an spkg at http://sage.math.washington.edu/home/mhansen/freetype-2.3.5.spkg .  It installs fine on 64-bit Linux.",
    "created_at": "2007-12-06T05:09:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1182",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1182#issuecomment-5264",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:3" align="right">comment:3</div>

I've put an spkg at http://sage.math.washington.edu/home/mhansen/freetype-2.3.5.spkg .  It installs fine on 64-bit Linux.



---

archive/issue_comments_005265.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nBuilds fine on Solaris, OSX 10.4, OSX 10.5. \n\nCheers,\n\nMichael",
    "created_at": "2007-12-09T10:05:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1182",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1182#issuecomment-5265",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Builds fine on Solaris, OSX 10.4, OSX 10.5. 

Cheers,

Michael



---

archive/issue_comments_005266.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nMerged in 2.9.alpha2.",
    "created_at": "2007-12-09T10:06:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1182",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1182#issuecomment-5266",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">comment:5</div>

Merged in 2.9.alpha2.



---

archive/issue_events_011418.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-09T10:06:57Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1182",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1182#event-11418"
}
```
