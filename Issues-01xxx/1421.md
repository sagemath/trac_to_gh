# Issue 1421: finer control in ECM interface

archive/issues_001421.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "I have added a new method \"one_curve\" (maybe a better name can be found) to run exactly one\ncurve (either ECM or P-1 or P+1). Also added examples for find_factor() method.\n\nComponent: **number theory**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1421_\n\n",
    "closed_at": "2007-12-18T02:12:38Z",
    "created_at": "2007-12-07T19:34:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20number%20theory",
        "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.9.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "finer control in ECM interface",
    "type": "issue",
    "updated_at": "2007-12-18T02:12:38Z",
    "url": "https://github.com/sagemath/sage/issues/1421",
    "user": "https://github.com/zimmermann6"
}
```
I have added a new method "one_curve" (maybe a better name can be found) to run exactly one
curve (either ECM or P-1 or P+1). Also added examples for find_factor() method.

Component: **number theory**

_Issue created by migration from https://trac.sagemath.org/ticket/1421_





---

archive/issue_events_014078.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2007-12-07T19:34:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1421",
    "milestone_number": null,
    "milestone_title": "sage-2.9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1421#event-14078"
}
```



---

archive/issue_events_014079.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2007-12-07T19:34:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1421",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20number%20theory",
    "label_color": "0000ff",
    "label_name": "c: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1421#event-14079"
}
```



---

archive/issue_events_014080.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2007-12-07T19:34:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1421",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1421#event-14080"
}
```



---

archive/issue_events_014081.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2007-12-07T19:34:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1421",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1421#event-14081"
}
```



---

archive/issue_events_014082.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-12-07T19:34:29Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1421",
    "subject": "https://github.com/zimmermann6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1421#event-14082"
}
```



---

archive/issue_comments_006657.json:
```json
{
    "body": "Attachment: **[7544.patch.gz](https://github.com/sagemath/sage/files/ticket1421/7544.patch.gz)**",
    "created_at": "2007-12-07T19:34:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1421",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1421#issuecomment-6657",
    "user": "https://github.com/zimmermann6"
}
```

Attachment: **[7544.patch.gz](https://github.com/sagemath/sage/files/ticket1421/7544.patch.gz)**



---

archive/issue_comments_006658.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\ncode looks good -- if it passes tests, put it in.",
    "created_at": "2007-12-15T10:18:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1421",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1421#issuecomment-6658",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:1" align="right">comment:1</div>

code looks good -- if it passes tests, put it in.



---

archive/issue_events_014083.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2007-12-18T01:45:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1421",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1421#event-14083"
}
```



---

archive/issue_comments_006659.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nFixed one line in the original patch -- it creates a string to return as the error message, and then accidentally returns something other than that string (which I'm assuming wasn't the intended behavior).",
    "created_at": "2007-12-18T01:45:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1421",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1421#issuecomment-6659",
    "user": "https://github.com/craigcitro"
}
```

<div id="comment:2" align="right">comment:2</div>

Fixed one line in the original patch -- it creates a string to return as the error message, and then accidentally returns something other than that string (which I'm assuming wasn't the intended behavior).



---

archive/issue_comments_006660.json:
```json
{
    "body": "Attachment: **[1421-pt2.patch.gz](https://github.com/sagemath/sage/files/ticket1421/1421-pt2.patch.gz)**",
    "created_at": "2007-12-18T01:45:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1421",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1421#issuecomment-6660",
    "user": "https://github.com/craigcitro"
}
```

Attachment: **[1421-pt2.patch.gz](https://github.com/sagemath/sage/files/ticket1421/1421-pt2.patch.gz)**



---

archive/issue_events_014084.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2007-12-18T02:12:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1421",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1421#event-14084"
}
```



---

archive/issue_events_014085.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2007-12-18T02:12:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1421",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1421#event-14085"
}
```



---

archive/issue_comments_006661.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in 2.9.1.alpha1",
    "created_at": "2007-12-18T02:12:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1421",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1421#issuecomment-6661",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in 2.9.1.alpha1
