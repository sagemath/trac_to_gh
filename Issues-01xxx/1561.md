# Issue 1561: [with patch, positive review] add ._matrix_() and .transpose() to vector/FreeModuleElement.

archive/issues_001561.json:
```json
{
    "body": "This makes `matrix(vector(2, 3, 4))` and `transpose(vector(2, 3, 4))` work -- very handy when translating PARI code.\n\nAssignee: @malb\n\nKeywords: matrix vector transpose FreeModuleElement\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/1561\n\n",
    "closed_at": "2007-12-21T22:37:48Z",
    "created_at": "2007-12-18T19:36:47Z",
    "labels": [
        "component: commutative algebra",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.9.1",
    "title": "[with patch, positive review] add ._matrix_() and .transpose() to vector/FreeModuleElement.",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/1561",
    "user": "https://github.com/ncalexan"
}
```
This makes `matrix(vector(2, 3, 4))` and `transpose(vector(2, 3, 4))` work -- very handy when translating PARI code.

Assignee: @malb

Keywords: matrix vector transpose FreeModuleElement

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/1561





---

archive/issue_comments_010380.json:
```json
{
    "body": "Attachment [ncalexan-vector-matrix.hg](tarball://root/attachments/some-uuid/ticket1561/ncalexan-vector-matrix.hg) by @ncalexan created at 2007-12-18 19:37:27",
    "created_at": "2007-12-18T19:37:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1561#issuecomment-10380",
    "user": "https://github.com/ncalexan"
}
```

Attachment [ncalexan-vector-matrix.hg](tarball://root/attachments/some-uuid/ticket1561/ncalexan-vector-matrix.hg) by @ncalexan created at 2007-12-18 19:37:27



---

archive/issue_events_005291.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1561",
    "rename": {
        "from": "[with patch] add ._matrix_() and .transpose() to vector/FreeModuleElement.",
        "to": "[with patch, positive review] add ._matrix_() and .transpose() to vector/FreeModuleElement."
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1561#event-5291"
}
```



---

archive/issue_comments_010381.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2007-12-21T22:37:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1561#issuecomment-10381",
    "user": "https://github.com/rlmill"
}
```

Resolution: fixed



---

archive/issue_events_005292.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2007-12-21T22:37:48Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/1561",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1561#event-5292"
}
```



---

archive/issue_comments_010382.json:
```json
{
    "body": "<a id='comment:1'></a>\nmerged in 2.9.1 alpha3",
    "created_at": "2007-12-21T22:37:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1561",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1561#issuecomment-10382",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:1'></a>
merged in 2.9.1 alpha3
