# Issue 1701: attempt to clean up currRing if deallocated

archive/issues_001701.json:
```json
{
    "assignees": [],
    "body": "This patch used to be attached to #1541 but logically doesn't belong there.\n\nAssignee: **@malb**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1701_\n\n",
    "closed_at": "2008-01-07T17:28:14Z",
    "created_at": "2008-01-06T13:23:05Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.10",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "attempt to clean up currRing if deallocated",
    "type": "issue",
    "updated_at": "2008-01-07T17:28:14Z",
    "url": "https://github.com/sagemath/sage/issues/1701",
    "user": "https://github.com/malb"
}
```
This patch used to be attached to #1541 but logically doesn't belong there.

Assignee: **@malb**

_Issue created by migration from https://trac.sagemath.org/ticket/1701_





---

archive/issue_events_014639.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2008-01-06T13:23:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1701",
    "milestone_number": null,
    "milestone_title": "sage-2.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1701#event-14639"
}
```



---

archive/issue_events_014640.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2008-01-06T13:23:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1701",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
    "label_color": "0000ff",
    "label_name": "component: commutative algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1701#event-14640"
}
```



---

archive/issue_events_014641.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2008-01-06T13:23:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1701",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1701#event-14641"
}
```



---

archive/issue_events_014642.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2008-01-06T13:23:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1701",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1701#event-14642"
}
```



---

archive/issue_comments_008296.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[currRing.patch.gz](https://github.com/sagemath/sage/files/ticket1701/currRing.patch.gz)**\n\nI am under the impression that this patch already got merged, but I will investigate.\n\nCheers,\n\nMichael",
    "created_at": "2008-01-06T16:10:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1701",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1701#issuecomment-8296",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[currRing.patch.gz](https://github.com/sagemath/sage/files/ticket1701/currRing.patch.gz)**

I am under the impression that this patch already got merged, but I will investigate.

Cheers,

Michael



---

archive/issue_comments_008297.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nI just checked, it is not merged yet:\n\nmulti_polynomial_libsingular.pyx in Sage 2.9.2:\n\n```\n    def __dealloc__(self):\n        \"\"\"\n        \"\"\"\n        cdef ring *oldRing = NULL\n        if currRing != self._ring:\n            oldRing = currRing\n            rChangeCurrRing(self._ring)\n        rDelete(self._ring)\n        if oldRing != NULL:\n            rChangeCurrRing(oldRing)\n```",
    "created_at": "2008-01-06T16:17:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1701",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1701#issuecomment-8297",
    "user": "https://github.com/malb"
}
```

<a id='comment:2'>Comment 2:</a>
I just checked, it is not merged yet:

multi_polynomial_libsingular.pyx in Sage 2.9.2:

```
    def __dealloc__(self):
        """
        """
        cdef ring *oldRing = NULL
        if currRing != self._ring:
            oldRing = currRing
            rChangeCurrRing(self._ring)
        rDelete(self._ring)
        if oldRing != NULL:
            rChangeCurrRing(oldRing)
```



---

archive/issue_events_014643.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-01-07T17:28:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1701",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1701#event-14643"
}
```



---

archive/issue_comments_008298.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nLooks good to me, Merged in Sage 2.10.alpha0.\n\nCheers,\n\nMichael",
    "created_at": "2008-01-07T17:28:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1701",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1701#issuecomment-8298",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:3'>Comment 3:</a>
Looks good to me, Merged in Sage 2.10.alpha0.

Cheers,

Michael
