# Issue 1356: [with patch, with positive review] fix bug when taking abs() of exactly known QQbar

archive/issues_001356.json:
```json
{
    "body": "The following test fails in 2.8.15.alpha1:\n\n```\n            sage: v = QQbar.zeta(3) + 1\n            sage: v.exactify()\n            sage: v.abs().minpoly()\n```\nbut the attached patch fixes it.\n\nAssignee: somebody\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/1356\n\n",
    "closed_at": "2007-12-02T20:12:08Z",
    "created_at": "2007-12-02T01:25:31Z",
    "labels": [
        "component: basic arithmetic",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.8.15",
    "title": "[with patch, with positive review] fix bug when taking abs() of exactly known QQbar",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/1356",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```
The following test fails in 2.8.15.alpha1:

```
            sage: v = QQbar.zeta(3) + 1
            sage: v.exactify()
            sage: v.abs().minpoly()
```
but the attached patch fixes it.

Assignee: somebody

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/1356





---

archive/issue_comments_009054.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [1356.patch](tarball://root/attachments/some-uuid/ticket1356/1356.patch) by @rlmill created at 2007-12-02 19:34:21\n\nNow:\n\n```\nsage: v = QQbar.zeta(3) + 1\nsage: v.exactify()\nsage: v.abs().minpoly()\nx - 1\n```",
    "created_at": "2007-12-02T19:34:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1356",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1356#issuecomment-9054",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:1'></a>
Attachment [1356.patch](tarball://root/attachments/some-uuid/ticket1356/1356.patch) by @rlmill created at 2007-12-02 19:34:21

Now:

```
sage: v = QQbar.zeta(3) + 1
sage: v.exactify()
sage: v.abs().minpoly()
x - 1
```



---

archive/issue_events_004603.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1356",
    "rename": {
        "from": "[with patch] fix bug when taking abs() of exactly known QQbar",
        "to": "[with patch, tested] fix bug when taking abs() of exactly known QQbar"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1356#event-4603"
}
```



---

archive/issue_events_004604.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1356",
    "rename": {
        "from": "[with patch, tested] fix bug when taking abs() of exactly known QQbar",
        "to": "[with patch, with positive review] fix bug when taking abs() of exactly known QQbar"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1356#event-4604"
}
```



---

archive/issue_comments_009055.json:
```json
{
    "body": "<a id='comment:3'></a>\nlooks good to me.",
    "created_at": "2007-12-02T19:38:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1356",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1356#issuecomment-9055",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:3'></a>
looks good to me.



---

archive/issue_comments_009056.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in 2.8.15.rc0.",
    "created_at": "2007-12-02T20:12:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1356",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1356#issuecomment-9056",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in 2.8.15.rc0.



---

archive/issue_comments_009057.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2007-12-02T20:12:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1356",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1356#issuecomment-9057",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_004605.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-12-02T20:12:08Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/1356",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1356#event-4605"
}
```
