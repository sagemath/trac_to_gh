# Issue 1644: Enable email notification of trac tickets

archive/issues_001644.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "I think it would a real improvement to enable email notifications of trac tickets you are watching ala bugzilla style.  This makes it much easier to keep track of tickets you are interested in and prevents you from forgetting about them.  Also, it will allow us to set up a mailing list which keeps track of all changes to all trac tickets.  For more information on how to set this up, consult \n\nhttp://trac.edgewall.org/wiki/TracNotification\n\nCC:  @sagetrac-mabshoff\n\nKeywords: **trac**\n\nComponent: **website/wiki**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1644_\n\n",
    "closed_at": "2007-12-31T09:39:42Z",
    "created_at": "2007-12-31T08:01:20Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20website/wiki",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.9.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Enable email notification of trac tickets",
    "type": "issue",
    "updated_at": "2023-01-14T14:00:40Z",
    "url": "https://github.com/sagemath/sage/issues/1644",
    "user": "https://github.com/yqiang"
}
```
I think it would a real improvement to enable email notifications of trac tickets you are watching ala bugzilla style.  This makes it much easier to keep track of tickets you are interested in and prevents you from forgetting about them.  Also, it will allow us to set up a mailing list which keeps track of all changes to all trac tickets.  For more information on how to set this up, consult 

http://trac.edgewall.org/wiki/TracNotification

CC:  @sagetrac-mabshoff

Keywords: **trac**

Component: **website/wiki**

_Issue created by migration from https://trac.sagemath.org/ticket/1644_





---

archive/issue_events_016323.json:
```json
{
    "actor": "https://github.com/yqiang",
    "created_at": "2007-12-31T08:01:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20website/wiki",
    "label_color": "0075ca",
    "label_name": "c: website/wiki",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1644#event-16323"
}
```



---

archive/issue_events_016324.json:
```json
{
    "actor": "https://github.com/yqiang",
    "created_at": "2007-12-31T08:01:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1644#event-16324"
}
```



---

archive/issue_events_016325.json:
```json
{
    "actor": "https://github.com/yqiang",
    "created_at": "2007-12-31T08:01:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1644#event-16325"
}
```



---

archive/issue_events_016326.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-12-31T08:01:20Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "subject": "https://github.com/yqiang",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1644#event-16326"
}
```



---

archive/issue_comments_007551.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nThis has been much requested feature. I just checked trac.ini and I hopefully fixed the issue.\n\nCheers,\n\nMichael",
    "created_at": "2007-12-31T08:19:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1644#issuecomment-7551",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">comment:1</div>

This has been much requested feature. I just checked trac.ini and I hopefully fixed the issue.

Cheers,

Michael



---

archive/issue_events_016327.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-31T08:30:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "milestone_number": null,
    "milestone_title": "sage-2.9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1644#event-16327"
}
```



---

archive/issue_comments_007552.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nLet's see if those changes work this time.\n\nCheers,\n\nMichael",
    "created_at": "2007-12-31T08:31:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1644#issuecomment-7552",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Let's see if those changes work this time.

Cheers,

Michael



---

archive/issue_comments_007553.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nNow that I am editing the right file hopefully this might make a difference.\n\nCheers,\n\nMichael",
    "created_at": "2007-12-31T08:41:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1644#issuecomment-7553",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Now that I am editing the right file hopefully this might make a difference.

Cheers,

Michael



---

archive/issue_comments_007554.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nOk, now we are getting somewhere. Hopefully this time the config works.\n\nCheers,\n\nMichael",
    "created_at": "2007-12-31T08:42:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1644#issuecomment-7554",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">comment:5</div>

Ok, now we are getting somewhere. Hopefully this time the config works.

Cheers,

Michael



---

archive/issue_comments_007555.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nOk, now hopefully the CC to sage-trac will work.\n\nCheers,\n\nMichael",
    "created_at": "2007-12-31T09:25:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1644#issuecomment-7555",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:6" align="right">comment:6</div>

Ok, now hopefully the CC to sage-trac will work.

Cheers,

Michael



---

archive/issue_comments_007556.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nOk, that wasn't a good idea. Let's try it again.\n\nCheers,\n\nMichael",
    "created_at": "2007-12-31T09:28:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1644#issuecomment-7556",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:7" align="right">comment:7</div>

Ok, that wasn't a good idea. Let's try it again.

Cheers,

Michael



---

archive/issue_comments_007557.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nOk, one more test to make sure the message shows up in sage-trac as is. I set the \"from\" field to Williams's gmail address.\n\nCheers,\n\nMichael",
    "created_at": "2007-12-31T09:35:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1644#issuecomment-7557",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:8" align="right">comment:8</div>

Ok, one more test to make sure the message shows up in sage-trac as is. I set the "from" field to Williams's gmail address.

Cheers,

Michael



---

archive/issue_events_016328.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-31T09:39:42Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1644#event-16328"
}
```



---

archive/issue_comments_007558.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nok, now everything works as expected.\n\nCheers,\n\nMichael",
    "created_at": "2007-12-31T09:39:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1644#issuecomment-7558",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:9" align="right">comment:9</div>

ok, now everything works as expected.

Cheers,

Michael



---

archive/issue_comments_007559.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nHopefully the footer of the email notification is now fixed.\n\nCheers,\n\nMichael",
    "created_at": "2007-12-31T09:48:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1644#issuecomment-7559",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:10" align="right">comment:10</div>

Hopefully the footer of the email notification is now fixed.

Cheers,

Michael



---

archive/issue_comments_007560.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">comment:11</div>\n\nOops, something is escaping the '\"'. Fixed.\n\nCheers,\n\nMichael",
    "created_at": "2007-12-31T09:51:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1644#issuecomment-7560",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:11" align="right">comment:11</div>

Oops, something is escaping the '"'. Fixed.

Cheers,

Michael
