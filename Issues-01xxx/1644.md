# Issue 1644: Enable email notification of trac tickets

archive/issues_001644.json:
```json
{
    "assignees": [],
    "body": "I think it would a real improvement to enable email notifications of trac tickets you are watching ala bugzilla style.  This makes it much easier to keep track of tickets you are interested in and prevents you from forgetting about them.  Also, it will allow us to set up a mailing list which keeps track of all changes to all trac tickets.  For more information on how to set this up, consult \n\nhttp://trac.edgewall.org/wiki/TracNotification\n\n**Assignee:** @williamstein\n\n**CC:**  michael.abshoff@googlemail.com\n\n**Keywords:** trac\n\nIssue created by migration from https://trac.sagemath.org/ticket/1644\n\n",
    "closed_at": "2007-12-31T09:39:42Z",
    "created_at": "2007-12-31T08:01:20Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20website/wiki",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.9.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Enable email notification of trac tickets",
    "type": "issue",
    "updated_at": "2007-12-31T09:51:23Z",
    "url": "https://github.com/sagemath/sage/issues/1644",
    "user": "https://github.com/yqiang"
}
```
I think it would a real improvement to enable email notifications of trac tickets you are watching ala bugzilla style.  This makes it much easier to keep track of tickets you are interested in and prevents you from forgetting about them.  Also, it will allow us to set up a mailing list which keeps track of all changes to all trac tickets.  For more information on how to set this up, consult 

http://trac.edgewall.org/wiki/TracNotification

**Assignee:** @williamstein

**CC:**  michael.abshoff@googlemail.com

**Keywords:** trac

Issue created by migration from https://trac.sagemath.org/ticket/1644





---

archive/issue_events_012864.json:
```json
{
    "actor": "https://github.com/yqiang",
    "created_at": "2007-12-31T08:01:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "milestone_number": null,
    "milestone_title": "sage-2.9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1644#event-12864"
}
```



---

archive/issue_events_012865.json:
```json
{
    "actor": "https://github.com/yqiang",
    "created_at": "2007-12-31T08:01:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20website/wiki",
    "label_color": "08517b",
    "label_name": "component: website/wiki",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1644#event-12865"
}
```



---

archive/issue_events_012866.json:
```json
{
    "actor": "https://github.com/yqiang",
    "created_at": "2007-12-31T08:01:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "008080",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1644#event-12866"
}
```



---

archive/issue_comments_008041.json:
```json
{
    "body": "<a id='comment:1'>**Comment 1:**</a>\nThis has been much requested feature. I just checked trac.ini and I hopefully fixed the issue.\n\nCheers,\n\nMichael",
    "created_at": "2007-12-31T08:19:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1644#issuecomment-8041",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:1'>**Comment 1:**</a>
This has been much requested feature. I just checked trac.ini and I hopefully fixed the issue.

Cheers,

Michael



---

archive/issue_events_012867.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-31T08:30:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "milestone_number": null,
    "milestone_title": "sage-2.9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1644#event-12867"
}
```



---

archive/issue_comments_008042.json:
```json
{
    "body": "<a id='comment:3'>**Comment 3:**</a>\nLet's see if those changes work this time.\n\nCheers,\n\nMichael",
    "created_at": "2007-12-31T08:31:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1644#issuecomment-8042",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:3'>**Comment 3:**</a>
Let's see if those changes work this time.

Cheers,

Michael



---

archive/issue_comments_008043.json:
```json
{
    "body": "<a id='comment:4'>**Comment 4:**</a>\nNow that I am editing the right file hopefully this might make a difference.\n\nCheers,\n\nMichael",
    "created_at": "2007-12-31T08:41:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1644#issuecomment-8043",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:4'>**Comment 4:**</a>
Now that I am editing the right file hopefully this might make a difference.

Cheers,

Michael



---

archive/issue_comments_008044.json:
```json
{
    "body": "<a id='comment:5'>**Comment 5:**</a>\nOk, now we are getting somewhere. Hopefully this time the config works.\n\nCheers,\n\nMichael",
    "created_at": "2007-12-31T08:42:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1644#issuecomment-8044",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:5'>**Comment 5:**</a>
Ok, now we are getting somewhere. Hopefully this time the config works.

Cheers,

Michael



---

archive/issue_comments_008045.json:
```json
{
    "body": "<a id='comment:6'>**Comment 6:**</a>\nOk, now hopefully the CC to sage-trac will work.\n\nCheers,\n\nMichael",
    "created_at": "2007-12-31T09:25:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1644#issuecomment-8045",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:6'>**Comment 6:**</a>
Ok, now hopefully the CC to sage-trac will work.

Cheers,

Michael



---

archive/issue_comments_008046.json:
```json
{
    "body": "<a id='comment:7'>**Comment 7:**</a>\nOk, that wasn't a good idea. Let's try it again.\n\nCheers,\n\nMichael",
    "created_at": "2007-12-31T09:28:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1644#issuecomment-8046",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:7'>**Comment 7:**</a>
Ok, that wasn't a good idea. Let's try it again.

Cheers,

Michael



---

archive/issue_comments_008047.json:
```json
{
    "body": "<a id='comment:8'>**Comment 8:**</a>\nOk, one more test to make sure the message shows up in sage-trac as is. I set the \"from\" field to Williams's gmail address.\n\nCheers,\n\nMichael",
    "created_at": "2007-12-31T09:35:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1644#issuecomment-8047",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:8'>**Comment 8:**</a>
Ok, one more test to make sure the message shows up in sage-trac as is. I set the "from" field to Williams's gmail address.

Cheers,

Michael



---

archive/issue_events_012868.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-31T09:39:42Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1644#event-12868"
}
```



---

archive/issue_comments_008048.json:
```json
{
    "body": "<a id='comment:9'>**Comment 9:**</a>\nok, now everything works as expected.\n\nCheers,\n\nMichael",
    "created_at": "2007-12-31T09:39:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1644#issuecomment-8048",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:9'>**Comment 9:**</a>
ok, now everything works as expected.

Cheers,

Michael



---

archive/issue_comments_008049.json:
```json
{
    "body": "<a id='comment:10'>**Comment 10:**</a>\nHopefully the footer of the email notification is now fixed.\n\nCheers,\n\nMichael",
    "created_at": "2007-12-31T09:48:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1644#issuecomment-8049",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:10'>**Comment 10:**</a>
Hopefully the footer of the email notification is now fixed.

Cheers,

Michael



---

archive/issue_comments_008050.json:
```json
{
    "body": "<a id='comment:11'>**Comment 11:**</a>\nOops, something is escaping the '\"'. Fixed.\n\nCheers,\n\nMichael",
    "created_at": "2007-12-31T09:51:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1644#issuecomment-8050",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:11'>**Comment 11:**</a>
Oops, something is escaping the '"'. Fixed.

Cheers,

Michael
