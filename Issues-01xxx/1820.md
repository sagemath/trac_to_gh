# Issue 1820: [with patch, with positive review] update documentation for combinat/skew_partition.py

archive/issues_001820.json:
```json
{
    "body": "\n\nAssignee: @mwhansen\n\nCC:  sage-combinat\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/1820\n\n",
    "closed_at": "2008-01-21T06:05:40Z",
    "created_at": "2008-01-18T00:55:20Z",
    "labels": [
        "component: combinatorics",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.10.1",
    "title": "[with patch, with positive review] update documentation for combinat/skew_partition.py",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/1820",
    "user": "https://github.com/mwhansen"
}
```


Assignee: @mwhansen

CC:  sage-combinat

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/1820





---

archive/issue_comments_011998.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [1820.patch](tarball://root/attachments/some-uuid/ticket1820/1820.patch) by @mwhansen created at 2008-01-18 00:57:52",
    "created_at": "2008-01-18T00:57:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1820#issuecomment-11998",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'></a>
Attachment [1820.patch](tarball://root/attachments/some-uuid/ticket1820/1820.patch) by @mwhansen created at 2008-01-18 00:57:52



---

archive/issue_comments_011999.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-01-18T00:57:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1820#issuecomment-11999",
    "user": "https://github.com/mwhansen"
}
```

Changing status from new to assigned.



---

archive/issue_events_006153.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1820",
    "rename": {
        "from": "update documentation for combinat/skew_partition.py",
        "to": "[with patch] update documentation for combinat/skew_partition.py"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1820#event-6153"
}
```



---

archive/issue_comments_012000.json:
```json
{
    "body": "<a id='comment:2'></a>\nDocumentation looks excellent!  Apply.",
    "created_at": "2008-01-20T19:31:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1820#issuecomment-12000",
    "user": "https://github.com/ncalexan"
}
```

<a id='comment:2'></a>
Documentation looks excellent!  Apply.



---

archive/issue_events_006154.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1820",
    "rename": {
        "from": "[with patch] update documentation for combinat/skew_partition.py",
        "to": "[with patch, with positive review] update documentation for combinat/skew_partition.py"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1820#event-6154"
}
```



---

archive/issue_comments_012001.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-01-21T06:05:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1820#issuecomment-12001",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_012002.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 2.10.1.alpha1",
    "created_at": "2008-01-21T06:05:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1820#issuecomment-12002",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 2.10.1.alpha1



---

archive/issue_events_006155.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-01-21T06:05:40Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/1820",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1820#event-6155"
}
```
