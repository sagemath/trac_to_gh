# Issue 1095: silly annoyance in sage -coverage

archive/issues_001095.json:
```json
{
    "assignees": [
        "https://github.com/craigcitro"
    ],
    "body": "I made a silly mistake when I was editing sage-coverage -- it will always tell you that a function is \"possibly wrong\" (i.e. function name doesn't occur in doctest) when there is no doctest; this is pretty obvious. This patch fixes it, so that now things only appear in the \"possibly wrong\" list if they *don't* appear in either of the other lists (missing documentation or missing doctests).\n\nComponent: **doctest coverage**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1095_\n\n",
    "closed_at": "2007-11-06T22:33:52Z",
    "created_at": "2007-11-04T01:15:24Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/p%3A%205%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.12",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "silly annoyance in sage -coverage",
    "type": "issue",
    "updated_at": "2007-11-06T22:33:52Z",
    "url": "https://github.com/sagemath/sage/issues/1095",
    "user": "https://github.com/craigcitro"
}
```
I made a silly mistake when I was editing sage-coverage -- it will always tell you that a function is "possibly wrong" (i.e. function name doesn't occur in doctest) when there is no doctest; this is pretty obvious. This patch fixes it, so that now things only appear in the "possibly wrong" list if they *don't* appear in either of the other lists (missing documentation or missing doctests).

Component: **doctest coverage**

_Issue created by migration from https://trac.sagemath.org/ticket/1095_





---

archive/issue_events_010513.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2007-11-04T01:15:24Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1095",
    "milestone_number": null,
    "milestone_title": "sage-2.8.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1095#event-10513"
}
```



---

archive/issue_events_010514.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2007-11-04T01:15:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1095",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1095#event-10514"
}
```



---

archive/issue_events_010515.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2007-11-04T01:15:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1095",
    "label": "https://github.com/sagemath/sage/labels/p%3A%205%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p: 5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1095#event-10515"
}
```



---

archive/issue_events_010516.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2007-11-04T01:15:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1095",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1095#event-10516"
}
```



---

archive/issue_events_010517.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2007-11-04T01:15:24Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1095",
    "subject": "https://github.com/craigcitro",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1095#event-10517"
}
```



---

archive/issue_comments_004737.json:
```json
{
    "body": "Attachment: **[trac_1095.hg.gz](https://github.com/sagemath/sage/files/ticket1095/trac_1095.hg.gz)**\n\nbundle for $SAGE_ROOT/local/bin",
    "created_at": "2007-11-04T01:16:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1095",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1095#issuecomment-4737",
    "user": "https://github.com/craigcitro"
}
```

Attachment: **[trac_1095.hg.gz](https://github.com/sagemath/sage/files/ticket1095/trac_1095.hg.gz)**

bundle for $SAGE_ROOT/local/bin



---

archive/issue_comments_004738.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\napplied to 2.8.12.rc0",
    "created_at": "2007-11-06T22:33:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1095",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1095#issuecomment-4738",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">comment:1</div>

applied to 2.8.12.rc0



---

archive/issue_events_010518.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-06T22:33:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1095",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1095#event-10518"
}
```
