# Issue 1722: Symbolic Matrices should be callable.

archive/issues_001722.json:
```json
{
    "assignees": [
        "https://github.com/mwhansen"
    ],
    "body": "Matrices of symbolic objects should either be callable, or support substitution, per user request at conference.\n\nComponent: **linear algebra**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1722_\n\n",
    "closed_at": "2008-02-28T00:43:14Z",
    "created_at": "2008-01-08T19:11:43Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.10.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Symbolic Matrices should be callable.",
    "type": "issue",
    "updated_at": "2008-02-28T00:43:14Z",
    "url": "https://github.com/sagemath/sage/issues/1722",
    "user": "https://github.com/boothby"
}
```
Matrices of symbolic objects should either be callable, or support substitution, per user request at conference.

Component: **linear algebra**

_Issue created by migration from https://trac.sagemath.org/ticket/1722_





---

archive/issue_events_016969.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-01-08T19:11:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1722",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "c: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1722#event-16969"
}
```



---

archive/issue_events_016970.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-01-08T19:11:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1722",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1722#event-16970"
}
```



---

archive/issue_events_016971.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-01-08T19:11:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1722",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1722#event-16971"
}
```



---

archive/issue_events_016972.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-01-08T19:11:43Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1722",
    "subject": "https://github.com/boothby",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1722#event-16972"
}
```



---

archive/issue_events_016973.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-01-13T18:58:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1722",
    "milestone_number": null,
    "milestone_title": "sage-2.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1722#event-16973"
}
```



---

archive/issue_events_016974.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-01-27T01:17:49Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/1722",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1722#event-16974"
}
```



---

archive/issue_events_016975.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-01-27T01:17:49Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1722",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1722#event-16975"
}
```



---

archive/issue_events_016976.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-01-27T22:32:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1722",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1722#event-16976"
}
```



---

archive/issue_comments_007914.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nGreat doctests, good comments, looks good to me.  I say apply.",
    "created_at": "2008-02-15T05:17:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1722",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1722#issuecomment-7914",
    "user": "https://github.com/ncalexan"
}
```

<div id="comment:4" align="right">comment:4</div>

Great doctests, good comments, looks good to me.  I say apply.



---

archive/issue_events_016977.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2008-02-15T05:17:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1722",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1722#event-16977"
}
```



---

archive/issue_events_016978.json:
```json
{
    "actor": "https://github.com/ncalexan",
    "created_at": "2008-02-15T05:17:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1722",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1722#event-16978"
}
```



---

archive/issue_comments_007915.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nAgainst my 2.10.2.alpha0 I get a reject:\n\n```\npatch -p1 --dry-run < trac_1722.patch\npatching file sage/matrix/matrix_symbolic_dense.pxd\npatching file sage/matrix/matrix_symbolic_dense.pyx\nHunk #1 succeeded at 15 with fuzz 2 (offset 9 lines).\nHunk #2 succeeded at 48 with fuzz 2 (offset 8 lines).\nHunk #3 succeeded at 536 (offset 248 lines).\nHunk #4 FAILED at 617.\nHunk #5 FAILED at 681.\n2 out of 5 hunks FAILED -- saving rejects to file sage/matrix/matrix_symbolic_dense.pyx.rej\n```\nThe patch should be rebased against 2.10.2.alpha0 once it is out.\n\nCheers,\n\nMichael",
    "created_at": "2008-02-15T05:20:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1722",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1722#issuecomment-7915",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">comment:5</div>

Against my 2.10.2.alpha0 I get a reject:

```
patch -p1 --dry-run < trac_1722.patch
patching file sage/matrix/matrix_symbolic_dense.pxd
patching file sage/matrix/matrix_symbolic_dense.pyx
Hunk #1 succeeded at 15 with fuzz 2 (offset 9 lines).
Hunk #2 succeeded at 48 with fuzz 2 (offset 8 lines).
Hunk #3 succeeded at 536 (offset 248 lines).
Hunk #4 FAILED at 617.
Hunk #5 FAILED at 681.
2 out of 5 hunks FAILED -- saving rejects to file sage/matrix/matrix_symbolic_dense.pyx.rej
```
The patch should be rebased against 2.10.2.alpha0 once it is out.

Cheers,

Michael



---

archive/issue_events_016979.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-02-18T14:30:48Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/1722",
    "title_is": "[needs rebase] Symbolic Matrices should be callable.",
    "title_was": "Symbolic Matrices should be callable.",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1722#event-16979"
}
```



---

archive/issue_comments_007916.json:
```json
{
    "body": "Attachment: **[1722.patch.gz](https://github.com/sagemath/sage/files/ticket1722/1722.patch.gz)**",
    "created_at": "2008-02-27T20:16:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1722",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1722#issuecomment-7916",
    "user": "https://github.com/mwhansen"
}
```

Attachment: **[1722.patch.gz](https://github.com/sagemath/sage/files/ticket1722/1722.patch.gz)**



---

archive/issue_events_016980.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-02-27T20:16:46Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/1722",
    "title_is": "Symbolic Matrices should be callable.",
    "title_was": "[needs rebase] Symbolic Matrices should be callable.",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1722#event-16980"
}
```



---

archive/issue_comments_007917.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nNew rebased patch attached.",
    "created_at": "2008-02-27T20:16:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1722",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1722#issuecomment-7917",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:7" align="right">comment:7</div>

New rebased patch attached.



---

archive/issue_comments_007918.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nMerged in Sage 2.10.3.rc0",
    "created_at": "2008-02-28T00:43:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1722",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1722#issuecomment-7918",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:8" align="right">comment:8</div>

Merged in Sage 2.10.3.rc0



---

archive/issue_events_016981.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-02-28T00:43:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1722",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1722#event-16981"
}
```



---

archive/issue_events_016982.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-02-28T00:43:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1722",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1722#event-16982"
}
```
