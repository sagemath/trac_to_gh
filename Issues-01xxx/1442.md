# Issue 1442: wrong statement in calculus.py

archive/issues_001442.json:
```json
{
    "assignees": [
        "https://github.com/mwhansen"
    ],
    "body": "I'm reading calculus.py (this is probably the best way to learn SAGE).\nThis statement seems wrong to me:\n\n```\n\\sage predefines upper and lowercase letters as global\n    indeterminates.\n```\nIndeed, I believe only 'x' is predefined.\n\nComponent: **documentation**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1442_\n\n",
    "closed_at": "2007-12-15T11:59:45Z",
    "created_at": "2007-12-09T21:36:34Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "wrong statement in calculus.py",
    "type": "issue",
    "updated_at": "2007-12-15T11:59:45Z",
    "url": "https://github.com/sagemath/sage/issues/1442",
    "user": "https://github.com/zimmermann6"
}
```
I'm reading calculus.py (this is probably the best way to learn SAGE).
This statement seems wrong to me:

```
\sage predefines upper and lowercase letters as global
    indeterminates.
```
Indeed, I believe only 'x' is predefined.

Component: **documentation**

_Issue created by migration from https://trac.sagemath.org/ticket/1442_





---

archive/issue_events_014209.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2007-12-09T21:36:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1442",
    "milestone_number": null,
    "milestone_title": "sage-2.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1442#event-14209"
}
```



---

archive/issue_events_014210.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2007-12-09T21:36:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1442",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1442#event-14210"
}
```



---

archive/issue_events_014211.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2007-12-09T21:36:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1442",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1442#event-14211"
}
```



---

archive/issue_events_014212.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2007-12-09T21:36:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1442",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1442#event-14212"
}
```



---

archive/issue_comments_006789.json:
```json
{
    "body": "Attachment: **[1442.patch.gz](https://github.com/sagemath/sage/files/ticket1442/1442.patch.gz)**",
    "created_at": "2007-12-14T07:19:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1442#issuecomment-6789",
    "user": "https://github.com/mwhansen"
}
```

Attachment: **[1442.patch.gz](https://github.com/sagemath/sage/files/ticket1442/1442.patch.gz)**



---

archive/issue_events_014213.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2007-12-14T07:19:26Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1442",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1442#event-14213"
}
```



---

archive/issue_comments_006790.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThis used to be true, but it caused way too much confusion, so we changed it.  But forgot to change the docs.",
    "created_at": "2007-12-15T10:38:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1442#issuecomment-6790",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:2" align="right">comment:2</div>

This used to be true, but it caused way too much confusion, so we changed it.  But forgot to change the docs.



---

archive/issue_events_014214.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-15T11:58:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1442",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1442#event-14214"
}
```



---

archive/issue_comments_006791.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nLooks good to me.\n\nCheers,\n\nMichael",
    "created_at": "2007-12-15T11:58:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1442#issuecomment-6791",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Looks good to me.

Cheers,

Michael



---

archive/issue_comments_006792.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nMerged in 2.9.rc0.",
    "created_at": "2007-12-15T11:59:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1442",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1442#issuecomment-6792",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Merged in 2.9.rc0.



---

archive/issue_events_014215.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-15T11:59:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1442",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1442#event-14215"
}
```



---

archive/issue_events_014216.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-15T11:59:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1442",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1442#event-14216"
}
```
