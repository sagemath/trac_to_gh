# Issue 1558: more NTL wrapping, coefficient access and factoring

archive/issues_001558.json:
```json
{
    "assignees": [],
    "body": "The attached patch provides some more lines in ntl/decl.pxi to give direct access to some more of NTL.  The are two main functionality improvements:\n\n1) !__getitem!__ now has one less copy of the coefficient.  This results in a noticable speed improvement for coefficients (and !__hash!__)\n\n2) New _factor_ntl and _factor_pari in sage/rings/polynomial/polynomial_integer_dense_ntl.pyx enable us to have more fun with head-to-head comparisons of their factoring routines.  It really seems a mixed bag about who is faster.  I have a basic check in place that keeps the factoring all in NTL for small coefficients, but this really isn't the end of the story as far as the benchmarking goes.\n\n```\n# original\nsage: R.<x>=ZZ[]\nsage: f=x^2-1\nsage: timeit f.factor()\n1000 loops, best of 3: 784 \u00b5s per loop\n```\n\n```\n# patched\nsage: R.<x>=ZZ[]\nsage: f=x^2-1\nsage: timeit f.factor()\n10000 loops, best of 3: 153 \u00b5s per loop\n```\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1558_\n\n",
    "closed_at": "2007-12-23T07:08:24Z",
    "created_at": "2007-12-18T16:26:15Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.9.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "more NTL wrapping, coefficient access and factoring",
    "type": "issue",
    "updated_at": "2007-12-23T07:08:24Z",
    "url": "https://github.com/sagemath/sage/issues/1558",
    "user": "https://github.com/sagetrac-jbmohler"
}
```
The attached patch provides some more lines in ntl/decl.pxi to give direct access to some more of NTL.  The are two main functionality improvements:

1) !__getitem!__ now has one less copy of the coefficient.  This results in a noticable speed improvement for coefficients (and !__hash!__)

2) New _factor_ntl and _factor_pari in sage/rings/polynomial/polynomial_integer_dense_ntl.pyx enable us to have more fun with head-to-head comparisons of their factoring routines.  It really seems a mixed bag about who is faster.  I have a basic check in place that keeps the factoring all in NTL for small coefficients, but this really isn't the end of the story as far as the benchmarking goes.

```
# original
sage: R.<x>=ZZ[]
sage: f=x^2-1
sage: timeit f.factor()
1000 loops, best of 3: 784 µs per loop
```

```
# patched
sage: R.<x>=ZZ[]
sage: f=x^2-1
sage: timeit f.factor()
10000 loops, best of 3: 153 µs per loop
```

_Issue created by migration from https://trac.sagemath.org/ticket/1558_





---

archive/issue_events_013425.json:
```json
{
    "actor": "https://github.com/sagetrac-jbmohler",
    "created_at": "2007-12-18T16:26:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1558",
    "milestone_number": null,
    "milestone_title": "sage-2.9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1558#event-13425"
}
```



---

archive/issue_events_013426.json:
```json
{
    "actor": "https://github.com/sagetrac-jbmohler",
    "created_at": "2007-12-18T16:26:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1558",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "component: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1558#event-13426"
}
```



---

archive/issue_events_013427.json:
```json
{
    "actor": "https://github.com/sagetrac-jbmohler",
    "created_at": "2007-12-18T16:26:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1558",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1558#event-13427"
}
```



---

archive/issue_events_013428.json:
```json
{
    "actor": "https://github.com/sagetrac-jbmohler",
    "created_at": "2007-12-18T16:26:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1558",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1558#event-13428"
}
```



---

archive/issue_comments_007663.json:
```json
{
    "body": "Attachment: **[uni-factoring-ntl.patch.gz](https://github.com/sagemath/sage/files/ticket1558/uni-factoring-ntl.patch.gz)**\n\na patch with better decision process for pari or ntl",
    "created_at": "2007-12-19T19:51:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1558",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1558#issuecomment-7663",
    "user": "https://github.com/sagetrac-jbmohler"
}
```

Attachment: **[uni-factoring-ntl.patch.gz](https://github.com/sagemath/sage/files/ticket1558/uni-factoring-ntl.patch.gz)**

a patch with better decision process for pari or ntl



---

archive/issue_events_013429.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2007-12-22T01:14:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1558",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1558#event-13429"
}
```



---

archive/issue_comments_007664.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThe patch applies, builds, and passes tests for me.",
    "created_at": "2007-12-22T23:11:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1558",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1558#issuecomment-7664",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:2'>Comment 2:</a>
The patch applies, builds, and passes tests for me.



---

archive/issue_events_013430.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2007-12-22T23:11:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1558",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1558#event-13430"
}
```



---

archive/issue_events_013431.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2007-12-22T23:11:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1558",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1558#event-13431"
}
```



---

archive/issue_events_013432.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2007-12-23T07:08:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1558",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1558#event-13432"
}
```



---

archive/issue_events_013433.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2007-12-23T07:08:24Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1558",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1558#event-13433"
}
```



---

archive/issue_comments_007665.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nmerged in 2.9.1 rc2",
    "created_at": "2007-12-23T07:08:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1558",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1558#issuecomment-7665",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:3'>Comment 3:</a>
merged in 2.9.1 rc2
