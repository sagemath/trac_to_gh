# Issue 1987: [with positive review] "forall" and "exists" need pointers in docstring to "all" and "any

archive/issues_001987.json:
```json
{
    "body": "The names of \"forall\" and \"exists\" suggest that these are predicates that can be used where a conditional is needed. In fact, doing so does not result in an error but in unintended results:\n`len([ i for i in [1..300] if forall(prime_divisors(i),lambda i: i<10)])`\nreturns\n`300`\nwhich does not reflect the apparent meaning of the expression. The proper result is returned by inserting an index:\n`len([ i for i in [1..300] if forall(prime_divisors(i),lambda i: i<10)[0]])`\nwhich returns\n`82`\n\nI would suggest an optional parameter to \"forall\" and \"exists\", say, witness=True to return a second return value giving the index. The default should be index=False in my opinion.\n\n**Assignee:** cwitty\n\n**Resolution:** fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/1987\n\n",
    "closed_at": "2008-02-16T20:45:58Z",
    "created_at": "2008-01-30T22:28:41Z",
    "labels": [
        "component: misc",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.10.2",
    "title": "[with positive review] \"forall\" and \"exists\" need pointers in docstring to \"all\" and \"any",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/1987",
    "user": "https://github.com/nbruin"
}
```
The names of "forall" and "exists" suggest that these are predicates that can be used where a conditional is needed. In fact, doing so does not result in an error but in unintended results:
`len([ i for i in [1..300] if forall(prime_divisors(i),lambda i: i<10)])`
returns
`300`
which does not reflect the apparent meaning of the expression. The proper result is returned by inserting an index:
`len([ i for i in [1..300] if forall(prime_divisors(i),lambda i: i<10)[0]])`
which returns
`82`

I would suggest an optional parameter to "forall" and "exists", say, witness=True to return a second return value giving the index. The default should be index=False in my opinion.

**Assignee:** cwitty

**Resolution:** fixed

Issue created by migration from https://trac.sagemath.org/ticket/1987





---

archive/issue_comments_014025.json:
```json
{
    "body": "<a id='comment:1'></a>\nAs Mike Hansen points out, use python natives \"all\" and \"any\" instead. Patch should probably not be merged.",
    "created_at": "2008-01-30T23:14:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1987",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1987#issuecomment-14025",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:1'></a>
As Mike Hansen points out, use python natives "all" and "any" instead. Patch should probably not be merged.



---

archive/attachments_001644.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "witness.patch",
    "asset_url": "tarball://root/attachments/some-uuid/ticket1987/witness.patch",
    "created_at": "2008-02-01T02:00:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1987",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.patch",
    "user": "https://github.com/nbruin"
}
```



---

archive/issue_comments_014026.json:
```json
{
    "body": "Patch to fix docstrings of forall and exists to refer to Python natives.",
    "created_at": "2008-02-01T02:00:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1987",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1987#issuecomment-14026",
    "user": "https://github.com/nbruin"
}
```

Patch to fix docstrings of forall and exists to refer to Python natives.



---

archive/issue_events_005842.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1987",
    "rename": {
        "from": "unintuitive return values for \"forall\" and \"exists\"",
        "to": "\"forall\" and \"exists\" need pointers in docstring to \"all\" and \"any"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1987#event-5842"
}
```



---

archive/issue_comments_014027.json:
```json
{
    "body": "<a id='comment:2'></a>\nPatch now changed to fix docstrings of \"forall\" and \"exists\". Please do consider this change for inclusion, so that other people have an easier time finding the more appropriate python natives.",
    "created_at": "2008-02-01T02:02:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1987",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1987#issuecomment-14027",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:2'></a>
Patch now changed to fix docstrings of "forall" and "exists". Please do consider this change for inclusion, so that other people have an easier time finding the more appropriate python natives.



---

archive/issue_comments_014028.json:
```json
{
    "body": "<a id='comment:3'></a>\nI made some comments on sage-devel.  This patch should definitely be included\nthough, even if none of my comments are addressed.",
    "created_at": "2008-02-01T03:02:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1987",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1987#issuecomment-14028",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:3'></a>
I made some comments on sage-devel.  This patch should definitely be included
though, even if none of my comments are addressed.



---

archive/issue_comments_014029.json:
```json
{
    "body": "<a id='comment:4'></a>\nThe patch has sat in trac for about two weeks now. William suggested to merge, but maybe Nils has some more input?\n\nCheers,\n\nMichael",
    "created_at": "2008-02-15T23:51:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1987",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1987#issuecomment-14029",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
The patch has sat in trac for about two weeks now. William suggested to merge, but maybe Nils has some more input?

Cheers,

Michael



---

archive/issue_comments_014030.json:
```json
{
    "body": "**Changing status** from new to needs_review.",
    "created_at": "2008-02-15T23:51:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1987",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1987#issuecomment-14030",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

**Changing status** from new to needs_review.



---

archive/issue_comments_014031.json:
```json
{
    "body": "<a id='comment:5'></a>\nThis patch can `_certainly_` be applied as is, since it just adds some useful info to docstrings.\n\nThe only remaining issues from the sage-devel discussion that I can see are some odd timing issues.  But that does not seem a reason to hold back on this.",
    "created_at": "2008-02-16T20:37:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1987",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1987#issuecomment-14031",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:5'></a>
This patch can `_certainly_` be applied as is, since it just adds some useful info to docstrings.

The only remaining issues from the sage-devel discussion that I can see are some odd timing issues.  But that does not seem a reason to hold back on this.



---

archive/issue_events_005843.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1987",
    "rename": {
        "from": "\"forall\" and \"exists\" need pointers in docstring to \"all\" and \"any",
        "to": "[with positive review] \"forall\" and \"exists\" need pointers in docstring to \"all\" and \"any"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1987#event-5843"
}
```



---

archive/issue_comments_014032.json:
```json
{
    "body": "<a id='comment:6'></a>\nOk, I consider John's review to be a positive one. Changing the subject accordingly. The patch applies cleanly against my current 2.10.2.alpha1 buil.d",
    "created_at": "2008-02-16T20:44:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1987",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1987#issuecomment-14032",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:6'></a>
Ok, I consider John's review to be a positive one. Changing the subject accordingly. The patch applies cleanly against my current 2.10.2.alpha1 buil.d



---

archive/issue_events_005844.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-02-16T20:45:58Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/1987",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1987#event-5844"
}
```



---

archive/issue_comments_014033.json:
```json
{
    "body": "**Resolution:** fixed",
    "created_at": "2008-02-16T20:45:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1987",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1987#issuecomment-14033",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

**Resolution:** fixed



---

archive/issue_comments_014034.json:
```json
{
    "body": "<a id='comment:7'></a>\nMerged in Sage 2.10.2.alpha1",
    "created_at": "2008-02-16T20:45:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1987",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1987#issuecomment-14034",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:7'></a>
Merged in Sage 2.10.2.alpha1
