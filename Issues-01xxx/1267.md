# Issue 1267: documentation for piecewise does not show up in notebook

archive/issues_001267.json:
```json
{
    "assignees": [
        "https://github.com/mwhansen"
    ],
    "body": "In the public notebook (on www.sagenb.org), when I evaluate a cell with `piecewise?`, I get this:\n\n```\nFile:        /usr/local/sage-2.6/local/lib/python2.5/site-packages/sage/functions/piecewise.py\nType:        <type 'classobj'>\nDefinition:  piecewise(x0)\nDocstring:\n```\nwith no actual docstring.  (Doing the same thing from the command line does give a useful docstring.)\n\n\nComponent: **notebook**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1267_\n\n",
    "closed_at": "2007-12-09T13:13:17Z",
    "created_at": "2007-11-25T15:09:41Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "documentation for piecewise does not show up in notebook",
    "type": "issue",
    "updated_at": "2007-12-09T13:13:17Z",
    "url": "https://github.com/sagemath/sage/issues/1267",
    "user": "https://github.com/sagetrac-cwitty"
}
```
In the public notebook (on www.sagenb.org), when I evaluate a cell with `piecewise?`, I get this:

```
File:        /usr/local/sage-2.6/local/lib/python2.5/site-packages/sage/functions/piecewise.py
Type:        <type 'classobj'>
Definition:  piecewise(x0)
Docstring:
```
with no actual docstring.  (Doing the same thing from the command line does give a useful docstring.)


Component: **notebook**

_Issue created by migration from https://trac.sagemath.org/ticket/1267_





---

archive/issue_events_012272.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2007-11-25T15:09:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1267",
    "milestone_number": null,
    "milestone_title": "sage-2.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1267#event-12272"
}
```



---

archive/issue_events_012273.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2007-11-25T15:09:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1267",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1267#event-12273"
}
```



---

archive/issue_events_012274.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2007-11-25T15:09:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1267",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1267#event-12274"
}
```



---

archive/issue_events_012275.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2007-11-25T15:09:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1267",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1267#event-12275"
}
```



---

archive/issue_events_012276.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2007-11-25T15:09:41Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1267",
    "subject": "https://github.com/sagetrac-cwitty",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1267#event-12276"
}
```



---

archive/issue_events_012277.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-25T17:56:11Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/1267",
    "milestone_number": null,
    "milestone_title": "sage-2.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1267#event-12277"
}
```



---

archive/issue_events_012278.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-25T17:56:11Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1267",
    "milestone_number": null,
    "milestone_title": "sage-2.8.15",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1267#event-12278"
}
```



---

archive/issue_events_012279.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-25T17:56:11Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/1267",
    "title_is": "documentation for piecewise does not work",
    "title_was": "documentation for piecewise does not show up in notebook",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1267#event-12279"
}
```



---

archive/issue_comments_005738.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nAfter some testing it turns out that cwitty is right and it does work in console mode with 2.8.14 So was is wrong and change it back.",
    "created_at": "2007-11-25T17:59:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1267#issuecomment-5738",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

After some testing it turns out that cwitty is right and it does work in console mode with 2.8.14 So was is wrong and change it back.



---

archive/issue_events_012280.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-25T17:59:42Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/1267",
    "title_is": "documentation for piecewise does not show up in notebook",
    "title_was": "documentation for piecewise does not work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1267#event-12280"
}
```



---

archive/issue_comments_005739.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nI can confirm and replicate this problem:\n\n```\n[10:09am] was_: It does fail on my ocally-running notebook.\n[10:09am] was_: Ipython does the help on the command line.\n[10:09am] was_: *I* wrote what does the help in the notebook.\n[10:09am] DenisG_: (my locally running notebook)\n[10:09am] was_: It's separate code; I think it is is sage/server/support.py or something like that\n[10:10am] was_: And there is definitely a bug.\n```",
    "created_at": "2007-11-25T18:35:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1267#issuecomment-5739",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:3" align="right">comment:3</div>

I can confirm and replicate this problem:

```
[10:09am] was_: It does fail on my ocally-running notebook.
[10:09am] was_: Ipython does the help on the command line.
[10:09am] was_: *I* wrote what does the help in the notebook.
[10:09am] DenisG_: (my locally running notebook)
[10:09am] was_: It's separate code; I think it is is sage/server/support.py or something like that
[10:10am] was_: And there is definitely a bug.
```



---

archive/issue_events_012281.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2007-12-06T00:22:44Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/1267",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1267#event-12281"
}
```



---

archive/issue_events_012282.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2007-12-06T00:22:44Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1267",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1267#event-12282"
}
```



---

archive/issue_comments_005740.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nActually, I don't think anything wrong with the ? or ?? notation.  What was happening was roughly the following:\n\n```\nclass PiecewisePolynomial:\n    def __init__(self, list_of_pairs):\n        \"\"\"docstring\"\"\"\n\n    ...\n\npiecewise = PiecewisePolynomial\n```\n\nThe result of piecewise? was correct since PiecewisePolynomial didn't have a class docstring.",
    "created_at": "2007-12-06T00:22:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1267#issuecomment-5740",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:4" align="right">comment:4</div>

Actually, I don't think anything wrong with the ? or ?? notation.  What was happening was roughly the following:

```
class PiecewisePolynomial:
    def __init__(self, list_of_pairs):
        """docstring"""

    ...

piecewise = PiecewisePolynomial
```

The result of piecewise? was correct since PiecewisePolynomial didn't have a class docstring.



---

archive/issue_comments_005741.json:
```json
{
    "body": "Attachment: **[1267.patch.gz](https://github.com/sagemath/sage/files/ticket1267/1267.patch.gz)**",
    "created_at": "2007-12-06T00:28:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1267#issuecomment-5741",
    "user": "https://github.com/mwhansen"
}
```

Attachment: **[1267.patch.gz](https://github.com/sagemath/sage/files/ticket1267/1267.patch.gz)**



---

archive/issue_comments_005742.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nLooks good to me.\n\nCheers,\n\nMichael",
    "created_at": "2007-12-09T12:34:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1267#issuecomment-5742",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">comment:5</div>

Looks good to me.

Cheers,

Michael



---

archive/issue_comments_005743.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nMerged in 2.9.alpha2.",
    "created_at": "2007-12-09T13:13:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1267#issuecomment-5743",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:6" align="right">comment:6</div>

Merged in 2.9.alpha2.



---

archive/issue_events_012283.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-09T13:13:17Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1267",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1267#event-12283"
}
```
