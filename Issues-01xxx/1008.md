# Issue 1008: dumb mistake in setup.py -- touching has adverse consequences

archive/issues_001008.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-cwitty"
    ],
    "body": "I made a stupid change to setup.py that solved one rare problem, while introducing a very annoying one.  This patch fixes that. \n\nComponent: **misc**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1008_\n\n",
    "closed_at": "2007-10-27T04:55:23Z",
    "created_at": "2007-10-27T03:28:04Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.10",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "dumb mistake in setup.py -- touching has adverse consequences",
    "type": "issue",
    "updated_at": "2007-10-27T04:55:23Z",
    "url": "https://github.com/sagemath/sage/issues/1008",
    "user": "https://github.com/williamstein"
}
```
I made a stupid change to setup.py that solved one rare problem, while introducing a very annoying one.  This patch fixes that. 

Component: **misc**

_Issue created by migration from https://trac.sagemath.org/ticket/1008_





---

archive/issue_events_009717.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-27T03:28:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1008",
    "milestone_number": null,
    "milestone_title": "sage-2.8.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1008#event-9717"
}
```



---

archive/issue_events_009718.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-27T03:28:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1008",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1008#event-9718"
}
```



---

archive/issue_events_009719.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-10-27T03:28:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1008",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1008#event-9719"
}
```



---

archive/issue_events_009720.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2007-10-27T03:28:04Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1008",
    "subject": "https://github.com/williamstein",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1008#event-9720"
}
```



---

archive/issue_comments_004421.json:
```json
{
    "body": "Attachment: **[7147.patch.gz](https://github.com/sagemath/sage/files/ticket1008/7147.patch.gz)**",
    "created_at": "2007-10-27T03:28:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1008",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1008#issuecomment-4421",
    "user": "https://github.com/williamstein"
}
```

Attachment: **[7147.patch.gz](https://github.com/sagemath/sage/files/ticket1008/7147.patch.gz)**



---

archive/issue_events_009721.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2007-10-27T04:55:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1008",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1008#event-9721"
}
```
