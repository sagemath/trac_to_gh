# Issue 1642: Fortran.spkg: search for common Fortran compiles if no binary is present

archive/issues_001642.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "We do not have binaries on less common platforms. If SAGE_FORTRAN isn't set we just fail, but we should check for common Fortran compilers and set them then instead of failing. Preference should be:\n* gfotran\n* g95\n* g77\n* f77 - on BSD the g77 is commonly called f77\n\nCheers,\n\nMichael\n\nComponent: **packages: standard**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1642_\n\n",
    "closed_at": "2008-04-20T06:31:21Z",
    "created_at": "2007-12-30T18:50:22Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p%3A%202%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fortran.spkg: search for common Fortran compiles if no binary is present",
    "type": "issue",
    "updated_at": "2008-04-20T06:31:21Z",
    "url": "https://github.com/sagemath/sage/issues/1642",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
We do not have binaries on less common platforms. If SAGE_FORTRAN isn't set we just fail, but we should check for common Fortran compilers and set them then instead of failing. Preference should be:
* gfotran
* g95
* g77
* f77 - on BSD the g77 is commonly called f77

Cheers,

Michael

Component: **packages: standard**

_Issue created by migration from https://trac.sagemath.org/ticket/1642_





---

archive/issue_events_016230.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-30T18:50:22Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1642",
    "milestone_number": null,
    "milestone_title": "sage-3.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1642#event-16230"
}
```



---

archive/issue_events_016231.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-30T18:50:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1642",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1642#event-16231"
}
```



---

archive/issue_events_016232.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-30T18:50:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1642",
    "label": "https://github.com/sagemath/sage/labels/p%3A%202%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p: 2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1642#event-16232"
}
```



---

archive/issue_events_016233.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-30T18:50:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1642",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1642#event-16233"
}
```



---

archive/issue_events_016234.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-30T18:50:22Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1642",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1642#event-16234"
}
```



---

archive/issue_events_016235.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-20T06:31:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1642",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1642#event-16235"
}
```



---

archive/issue_events_016236.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-20T06:31:21Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/1642",
    "milestone_number": null,
    "milestone_title": "sage-3.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1642#event-16236"
}
```



---

archive/issue_events_016237.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-04-20T06:31:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1642",
    "milestone_number": null,
    "milestone_title": "sage-3.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1642#event-16237"
}
```



---

archive/issue_comments_007571.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThis has been fixed by William Stein a while ago.\n\nCheers,\n\nMichael",
    "created_at": "2008-04-20T06:31:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1642#issuecomment-7571",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

This has been fixed by William Stein a while ago.

Cheers,

Michael
