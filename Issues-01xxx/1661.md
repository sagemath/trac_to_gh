# Issue 1661: [with bundle, needs review] click-to-evaluate button

archive/issues_001661.json:
```json
{
    "body": "Add a button that users can click to evaluate a cell.\n\nAssignee: boothby\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/1661\n\n",
    "closed_at": "2008-01-05T02:14:34Z",
    "created_at": "2008-01-02T22:47:18Z",
    "labels": [
        "component: notebook"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.9.2",
    "title": "[with bundle, needs review] click-to-evaluate button",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/1661",
    "user": "https://trac.sagemath.org/admin/accounts/users/boothby"
}
```
Add a button that users can click to evaluate a cell.

Assignee: boothby

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/1661





---

archive/issue_comments_011370.json:
```json
{
    "body": "Attachment [1661.hg](tarball://root/attachments/some-uuid/ticket1661/1661.hg) by boothby created at 2008-01-02 22:49:05",
    "created_at": "2008-01-02T22:49:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1661",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1661#issuecomment-11370",
    "user": "https://trac.sagemath.org/admin/accounts/users/boothby"
}
```

Attachment [1661.hg](tarball://root/attachments/some-uuid/ticket1661/1661.hg) by boothby created at 2008-01-02 22:49:05



---

archive/issue_comments_011371.json:
```json
{
    "body": "<a id='comment:1'></a>... both files are identical, I accidentally double-clicked the submit button.",
    "created_at": "2008-01-02T22:51:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1661",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1661#issuecomment-11371",
    "user": "https://trac.sagemath.org/admin/accounts/users/boothby"
}
```

<a id='comment:1'></a>... both files are identical, I accidentally double-clicked the submit button.



---

archive/issue_events_004101.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-01-05T02:14:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/1661",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1661#event-4101"
}
```



---

archive/issue_comments_011372.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-01-05T02:14:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1661",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1661#issuecomment-11372",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_011373.json:
```json
{
    "body": "<a id='comment:3'></a>Merged in 2.9.2.rc1.",
    "created_at": "2008-01-05T02:14:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1661",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1661#issuecomment-11373",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>Merged in 2.9.2.rc1.
