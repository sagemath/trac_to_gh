# Issue 1156: Functions lost in massive combinatorics reorganization?

archive/issues_001156.json:
```json
{
    "assignees": [],
    "body": "See changeset \"Initial commit of combinat/ files and bug fixes.\" from Mike Hansen. \n\nSome functions lost. bernoulli_polynomial among them. \n\nAssignee: **@mwhansen**\n\nCC:  sage-combinat\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1156_\n\n",
    "closed_at": "2007-11-19T21:16:32Z",
    "created_at": "2007-11-12T20:41:15Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.13",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Functions lost in massive combinatorics reorganization?",
    "type": "issue",
    "updated_at": "2009-04-14T02:23:42Z",
    "url": "https://github.com/sagemath/sage/issues/1156",
    "user": "https://github.com/robertwb"
}
```
See changeset "Initial commit of combinat/ files and bug fixes." from Mike Hansen. 

Some functions lost. bernoulli_polynomial among them. 

Assignee: **@mwhansen**

CC:  sage-combinat

_Issue created by migration from https://trac.sagemath.org/ticket/1156_





---

archive/issue_events_009809.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2007-11-12T20:41:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1156",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1156#event-9809"
}
```



---

archive/issue_events_009810.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2007-11-12T20:41:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1156",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1156#event-9810"
}
```



---

archive/issue_events_009811.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2007-11-12T20:41:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1156",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1156#event-9811"
}
```



---

archive/issue_events_009812.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-15T02:23:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1156",
    "milestone_number": null,
    "milestone_title": "sage-2.8.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1156#event-9812"
}
```



---

archive/issue_comments_005363.json:
```json
{
    "body": "Attachment: **[1156.patch.gz](https://github.com/sagemath/sage/files/ticket1156/1156.patch.gz)**",
    "created_at": "2007-11-17T20:51:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1156",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1156#issuecomment-5363",
    "user": "https://github.com/mwhansen"
}
```

Attachment: **[1156.patch.gz](https://github.com/sagemath/sage/files/ticket1156/1156.patch.gz)**



---

archive/issue_comments_005364.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\n\n```\n[21:57] <mabshoff> Is there any more to #1156 or is that patch all that is missing?\n[21:59] <mhansen> I went through a diff with the old version and those three things \nwere the only ones I found missing.\n```",
    "created_at": "2007-11-19T21:07:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1156",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1156#issuecomment-5364",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:3'>Comment 3:</a>

```
[21:57] <mabshoff> Is there any more to #1156 or is that patch all that is missing?
[21:59] <mhansen> I went through a diff with the old version and those three things 
were the only ones I found missing.
```



---

archive/issue_comments_005365.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nMerged in 2.8.13.alpha1.\n\nAfter feedback from Mike Hansen closed.\n\nCheers,\n\nMichael",
    "created_at": "2007-11-19T21:16:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1156",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1156#issuecomment-5365",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:4'>Comment 4:</a>
Merged in 2.8.13.alpha1.

After feedback from Mike Hansen closed.

Cheers,

Michael



---

archive/issue_events_009813.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-19T21:16:32Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1156",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1156#event-9813"
}
```
