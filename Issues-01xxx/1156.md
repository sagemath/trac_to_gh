# Issue 1156: Functions lost in massive combinatorics reorganization?

archive/issues_001156.json:
```json
{
    "assignees": [
        "https://github.com/mwhansen"
    ],
    "body": "See changeset \"Initial commit of combinat/ files and bug fixes.\" from Mike Hansen. \n\nSome functions lost. bernoulli_polynomial among them. \n\nCC:  @sagetrac-sage-combinat\n\nComponent: **combinatorics**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1156_\n\n",
    "closed_at": "2007-11-19T21:16:32Z",
    "created_at": "2007-11-12T20:41:15Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.13",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Functions lost in massive combinatorics reorganization?",
    "type": "issue",
    "updated_at": "2009-04-14T02:23:42Z",
    "url": "https://github.com/sagemath/sage/issues/1156",
    "user": "https://github.com/robertwb"
}
```
See changeset "Initial commit of combinat/ files and bug fixes." from Mike Hansen. 

Some functions lost. bernoulli_polynomial among them. 

CC:  @sagetrac-sage-combinat

Component: **combinatorics**

_Issue created by migration from https://trac.sagemath.org/ticket/1156_





---

archive/issue_events_011149.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2007-11-12T20:41:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1156",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "c: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1156#event-11149"
}
```



---

archive/issue_events_011150.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2007-11-12T20:41:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1156",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1156#event-11150"
}
```



---

archive/issue_events_011151.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2007-11-12T20:41:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1156",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1156#event-11151"
}
```



---

archive/issue_events_011152.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2007-11-12T20:41:15Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1156",
    "subject": "https://github.com/robertwb",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1156#event-11152"
}
```



---

archive/issue_events_011153.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-15T02:23:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1156",
    "milestone_number": null,
    "milestone_title": "sage-2.8.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1156#event-11153"
}
```



---

archive/issue_comments_005057.json:
```json
{
    "body": "Attachment: **[1156.patch.gz](https://github.com/sagemath/sage/files/ticket1156/1156.patch.gz)**",
    "created_at": "2007-11-17T20:51:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1156",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1156#issuecomment-5057",
    "user": "https://github.com/mwhansen"
}
```

Attachment: **[1156.patch.gz](https://github.com/sagemath/sage/files/ticket1156/1156.patch.gz)**



---

archive/issue_comments_005058.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\n\n```\n[21:57] <mabshoff> Is there any more to #1156 or is that patch all that is missing?\n[21:59] <mhansen> I went through a diff with the old version and those three things \nwere the only ones I found missing.\n```",
    "created_at": "2007-11-19T21:07:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1156",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1156#issuecomment-5058",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>


```
[21:57] <mabshoff> Is there any more to #1156 or is that patch all that is missing?
[21:59] <mhansen> I went through a diff with the old version and those three things 
were the only ones I found missing.
```



---

archive/issue_comments_005059.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nMerged in 2.8.13.alpha1.\n\nAfter feedback from Mike Hansen closed.\n\nCheers,\n\nMichael",
    "created_at": "2007-11-19T21:16:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1156",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1156#issuecomment-5059",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Merged in 2.8.13.alpha1.

After feedback from Mike Hansen closed.

Cheers,

Michael



---

archive/issue_events_011154.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-19T21:16:32Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1156",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1156#event-11154"
}
```
