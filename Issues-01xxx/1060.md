# Issue 1060: fix flintqs compile on OSX 10.5

archive/issues_001060.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "The fix for flintqs is the same as for givaro, basically. To the file\n\n```\n    src/lanczos.c\n```\nadd the following as the first line:\n\n```\n#include \"sys/types.h\"\n```\nThen it builds fine. \n\n-- William\n\nComponent: **porting**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1060_\n\n",
    "closed_at": "2007-11-02T01:08:01Z",
    "created_at": "2007-11-02T00:25:17Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20porting",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.11",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix flintqs compile on OSX 10.5",
    "type": "issue",
    "updated_at": "2007-11-02T01:08:01Z",
    "url": "https://github.com/sagemath/sage/issues/1060",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
The fix for flintqs is the same as for givaro, basically. To the file

```
    src/lanczos.c
```
add the following as the first line:

```
#include "sys/types.h"
```
Then it builds fine. 

-- William

Component: **porting**

_Issue created by migration from https://trac.sagemath.org/ticket/1060_





---

archive/issue_events_010202.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-02T00:25:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1060",
    "milestone_number": null,
    "milestone_title": "sage-2.8.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1060#event-10202"
}
```



---

archive/issue_events_010203.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-02T00:25:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1060",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20porting",
    "label_color": "000080",
    "label_name": "c: porting",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1060#event-10203"
}
```



---

archive/issue_events_010204.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-02T00:25:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1060",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1060#event-10204"
}
```



---

archive/issue_events_010205.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-02T00:25:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1060",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1060#event-10205"
}
```



---

archive/issue_events_010206.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-02T00:25:17Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1060",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1060#event-10206"
}
```



---

archive/issue_events_010207.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-02T01:08:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1060",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1060#event-10207"
}
```



---

archive/issue_comments_004609.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\napplied to 2.8.11.rc1 - via new flintqs-20070817.p0",
    "created_at": "2007-11-02T01:08:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1060",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1060#issuecomment-4609",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">comment:1</div>

applied to 2.8.11.rc1 - via new flintqs-20070817.p0
