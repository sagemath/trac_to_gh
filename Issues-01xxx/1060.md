# Issue 1060: fix flintqs compile on OSX 10.5

archive/issues_001060.json:
```json
{
    "assignees": [],
    "body": "The fix for flintqs is the same as for givaro, basically. To the file\n\n```\n    src/lanczos.c\n```\nadd the following as the first line:\n\n```\n#include \"sys/types.h\"\n```\nThen it builds fine. \n\n-- William\n\nAssignee: **mabshoff**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1060_\n\n",
    "closed_at": "2007-11-02T01:08:01Z",
    "created_at": "2007-11-02T00:25:17Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.11",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix flintqs compile on OSX 10.5",
    "type": "issue",
    "updated_at": "2007-11-02T01:08:01Z",
    "url": "https://github.com/sagemath/sage/issues/1060",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
The fix for flintqs is the same as for givaro, basically. To the file

```
    src/lanczos.c
```
add the following as the first line:

```
#include "sys/types.h"
```
Then it builds fine. 

-- William

Assignee: **mabshoff**

_Issue created by migration from https://trac.sagemath.org/ticket/1060_





---

archive/issue_events_008965.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-02T00:25:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1060",
    "milestone_number": null,
    "milestone_title": "sage-2.8.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1060#event-8965"
}
```



---

archive/issue_events_008966.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-02T00:25:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1060",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting",
    "label_color": "000080",
    "label_name": "component: porting",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1060#event-8966"
}
```



---

archive/issue_events_008967.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-02T00:25:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1060",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1060#event-8967"
}
```



---

archive/issue_events_008968.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-02T00:25:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1060",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1060#event-8968"
}
```



---

archive/issue_events_008969.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-02T01:08:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1060",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1060#event-8969"
}
```



---

archive/issue_comments_004894.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\napplied to 2.8.11.rc1 - via new flintqs-20070817.p0",
    "created_at": "2007-11-02T01:08:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1060",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1060#issuecomment-4894",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:1'>Comment 1:</a>
applied to 2.8.11.rc1 - via new flintqs-20070817.p0
