# Issue 1164: Error in tutorial documentation about %hist

archive/issues_001164.json:
```json
{
    "assignees": [
        "https://github.com/mwhansen"
    ],
    "body": "In the interactive tutorial for SAGE, page \"3.1 Your SAGE session\", the documentation should make it clear that the %hist command does not work in the SAGE notebook, and that one cannot type in %hist and assume that it will work. It should also be noted that __ and _oh do not work in the notebook (although _ does).\n\nAlso, in the line \"The PATHhastheSAGEbindirectoryatthefront,soifyourungp,gap,singular,maxima,etc.,yougettheversionsincludedwithSAGE. \n\", it looks like an end-maths-environment symbol has been omitted.\n\nComponent: **documentation**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1164_\n\n",
    "closed_at": "2007-12-09T11:58:45Z",
    "created_at": "2007-11-13T22:12:25Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Error in tutorial documentation about %hist",
    "type": "issue",
    "updated_at": "2007-12-09T11:58:45Z",
    "url": "https://github.com/sagemath/sage/issues/1164",
    "user": "https://github.com/sagetrac-ljpk"
}
```
In the interactive tutorial for SAGE, page "3.1 Your SAGE session", the documentation should make it clear that the %hist command does not work in the SAGE notebook, and that one cannot type in %hist and assume that it will work. It should also be noted that __ and _oh do not work in the notebook (although _ does).

Also, in the line "The PATHhastheSAGEbindirectoryatthefront,soifyourungp,gap,singular,maxima,etc.,yougettheversionsincludedwithSAGE. 
", it looks like an end-maths-environment symbol has been omitted.

Component: **documentation**

_Issue created by migration from https://trac.sagemath.org/ticket/1164_





---

archive/issue_events_011160.json:
```json
{
    "actor": "https://github.com/sagetrac-ljpk",
    "created_at": "2007-11-13T22:12:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1164",
    "milestone_number": null,
    "milestone_title": "sage-2.9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1164#event-11160"
}
```



---

archive/issue_events_011161.json:
```json
{
    "actor": "https://github.com/sagetrac-ljpk",
    "created_at": "2007-11-13T22:12:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1164",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1164#event-11161"
}
```



---

archive/issue_events_011162.json:
```json
{
    "actor": "https://github.com/sagetrac-ljpk",
    "created_at": "2007-11-13T22:12:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1164",
    "label": "https://github.com/sagemath/sage/labels/p%3A%20minor%20/%204",
    "label_color": "ffe799",
    "label_name": "p: minor / 4",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1164#event-11162"
}
```



---

archive/issue_events_011163.json:
```json
{
    "actor": "https://github.com/sagetrac-ljpk",
    "created_at": "2007-11-13T22:12:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1164",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1164#event-11163"
}
```



---

archive/issue_events_011164.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-15T09:13:15Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/1164",
    "milestone_number": null,
    "milestone_title": "sage-2.9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1164#event-11164"
}
```



---

archive/issue_events_011165.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-15T09:13:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1164",
    "milestone_number": null,
    "milestone_title": "sage-2.8.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1164#event-11165"
}
```



---

archive/issue_events_011166.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2007-12-06T21:17:38Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1164",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1164#event-11166"
}
```



---

archive/issue_comments_005120.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nApply #1348 first.",
    "created_at": "2007-12-07T21:58:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1164",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1164#issuecomment-5120",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:3" align="right">comment:3</div>

Apply #1348 first.



---

archive/issue_comments_005121.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nAttachment: **[1164-doc.patch.gz](https://github.com/sagemath/sage/files/ticket1164/1164-doc.patch.gz)**\n\nLooks good to me.\n\nCheers,\n\nMichael",
    "created_at": "2007-12-09T11:57:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1164",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1164#issuecomment-5121",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Attachment: **[1164-doc.patch.gz](https://github.com/sagemath/sage/files/ticket1164/1164-doc.patch.gz)**

Looks good to me.

Cheers,

Michael



---

archive/issue_comments_005122.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nMerged in 2.9.alpha2.",
    "created_at": "2007-12-09T11:58:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1164",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1164#issuecomment-5122",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">comment:5</div>

Merged in 2.9.alpha2.



---

archive/issue_events_011167.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-09T11:58:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1164",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1164#event-11167"
}
```



---

archive/issue_events_011168.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-09T11:58:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1164",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1164#event-11168"
}
```



---

archive/issue_events_011169.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-09T11:58:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1164",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1164#event-11169"
}
```
