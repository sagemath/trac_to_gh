# Issue 1647: rewrite spkg chapter in the developer handbook

archive/issues_001647.json:
```json
{
    "assignees": [],
    "body": "We should do the following:\n\n* add a trivial example spkg \n* add a yet to be determined description of SPKG.txt and its content\n* mandate the use of a hg repo for the top level files\n\nCheers,\n\nMichael\n\nAssignee: **mabshoff**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1647_\n\n",
    "closed_at": "2008-08-31T07:57:08Z",
    "created_at": "2007-12-31T18:12:35Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "rewrite spkg chapter in the developer handbook",
    "type": "issue",
    "updated_at": "2008-08-31T07:57:08Z",
    "url": "https://github.com/sagemath/sage/issues/1647",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
We should do the following:

* add a trivial example spkg 
* add a yet to be determined description of SPKG.txt and its content
* mandate the use of a hg repo for the top level files

Cheers,

Michael

Assignee: **mabshoff**

_Issue created by migration from https://trac.sagemath.org/ticket/1647_





---

archive/issue_events_014226.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-31T18:12:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1647",
    "milestone_number": null,
    "milestone_title": "sage-3.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1647#event-14226"
}
```



---

archive/issue_events_014227.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-31T18:12:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1647",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1647#event-14227"
}
```



---

archive/issue_events_014228.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-31T18:12:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1647",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1647#event-14228"
}
```



---

archive/issue_events_014229.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-31T18:12:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1647",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1647#event-14229"
}
```



---

archive/issue_comments_008061.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThis ticket can be closed once #3905 has been merged.\n\nCheers,\n\nMichael",
    "created_at": "2008-08-25T21:31:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1647",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1647#issuecomment-8061",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:2'>Comment 2:</a>
This ticket can be closed once #3905 has been merged.

Cheers,

Michael



---

archive/issue_comments_008062.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nSince #3905 has been merged this can be closed. In case you want improvements please open a specific followup ticket.\n\nCheers,\n\nMichael",
    "created_at": "2008-08-31T07:57:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1647",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1647#issuecomment-8062",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:3'>Comment 3:</a>
Since #3905 has been merged this can be closed. In case you want improvements please open a specific followup ticket.

Cheers,

Michael



---

archive/issue_events_014230.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-08-31T07:57:08Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1647",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1647#event-14230"
}
```
