# Issue 1627: update libgcrypt to 1.4

archive/issues_001627.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "\n\nComponent: **packages: standard**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1627_\n\n",
    "closed_at": "2008-01-19T23:34:15Z",
    "created_at": "2007-12-29T04:40:24Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.10.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "update libgcrypt to 1.4",
    "type": "issue",
    "updated_at": "2008-01-19T23:34:15Z",
    "url": "https://github.com/sagemath/sage/issues/1627",
    "user": "https://github.com/sagetrac-mabshoff"
}
```


Component: **packages: standard**

_Issue created by migration from https://trac.sagemath.org/ticket/1627_





---

archive/issue_events_016079.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-29T04:40:24Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1627",
    "milestone_number": null,
    "milestone_title": "sage-2.10.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1627#event-16079"
}
```



---

archive/issue_events_016080.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-29T04:40:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1627",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1627#event-16080"
}
```



---

archive/issue_events_016081.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-29T04:40:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1627",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1627#event-16081"
}
```



---

archive/issue_events_016082.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-29T04:40:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1627",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1627#event-16082"
}
```



---

archive/issue_events_016083.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-29T04:40:24Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1627",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1627#event-16083"
}
```



---

archive/issue_comments_007525.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nAn updated spkg with OSX 64 bit build support can be found at\n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-2.10.1/alpha0/libgcrypt-1.4.0.spkg\n\nCheers,\n\nMichael",
    "created_at": "2008-01-19T23:33:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1627",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1627#issuecomment-7525",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

An updated spkg with OSX 64 bit build support can be found at

http://sage.math.washington.edu/home/mabshoff/release-cycles-2.10.1/alpha0/libgcrypt-1.4.0.spkg

Cheers,

Michael



---

archive/issue_events_016084.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-01-19T23:34:15Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1627",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1627#event-16084"
}
```
