# Issue 1354: Solaris modp 64 bit fix

archive/issues_001354.json:
```json
{
    "assignees": [],
    "body": "We are currently only compiling Solaris in 32 bit mode. Then a 64 int has to be a long long. Fix that so that all the modp doctests pass.\n\nCheers,\n\nMichael\n\nAssignee: **mabshoff**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1354_\n\n",
    "closed_at": "2007-12-01T21:25:49Z",
    "created_at": "2007-12-01T21:22:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.15",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Solaris modp 64 bit fix",
    "type": "issue",
    "updated_at": "2007-12-01T21:25:49Z",
    "url": "https://github.com/sagemath/sage/issues/1354",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
We are currently only compiling Solaris in 32 bit mode. Then a 64 int has to be a long long. Fix that so that all the modp doctests pass.

Cheers,

Michael

Assignee: **mabshoff**

_Issue created by migration from https://trac.sagemath.org/ticket/1354_





---

archive/issue_events_011535.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-01T21:22:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1354",
    "milestone_number": null,
    "milestone_title": "sage-2.8.15",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1354#event-11535"
}
```



---

archive/issue_events_011536.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-01T21:22:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1354",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1354#event-11536"
}
```



---

archive/issue_events_011537.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-01T21:22:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1354",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1354#event-11537"
}
```



---

archive/issue_events_011538.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-01T21:22:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1354",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1354#event-11538"
}
```



---

archive/issue_events_011539.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-01T21:25:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1354",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1354#event-11539"
}
```



---

archive/issue_comments_006682.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[Sage-2.8.15.alpha1-fix-int64-in-Solaris-32bit-mode.patch.gz](https://github.com/sagemath/sage/files/ticket1354/Sage-2.8.15.alpha1-fix-int64-in-Solaris-32bit-mode.patch.gz)**\n\nMerged in 2.8.15.alpha2.",
    "created_at": "2007-12-01T21:25:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1354",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1354#issuecomment-6682",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[Sage-2.8.15.alpha1-fix-int64-in-Solaris-32bit-mode.patch.gz](https://github.com/sagemath/sage/files/ticket1354/Sage-2.8.15.alpha1-fix-int64-in-Solaris-32bit-mode.patch.gz)**

Merged in 2.8.15.alpha2.
