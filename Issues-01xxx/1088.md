# Issue 1088: graphs: minimum spanning tree function

archive/issues_001088.json:
```json
{
    "assignees": [
        "https://github.com/mwhansen"
    ],
    "body": "Here's an implementation of a minimum spanning tree function.  Well, actually 3 implementations, defaulting to Kruskal's algorithm.\n\nComponent: **combinatorics**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1088_\n\n",
    "closed_at": "2007-11-06T23:54:16Z",
    "created_at": "2007-11-03T20:45:05Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.12",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "graphs: minimum spanning tree function",
    "type": "issue",
    "updated_at": "2007-11-06T23:54:16Z",
    "url": "https://github.com/sagemath/sage/issues/1088",
    "user": "https://github.com/jasongrout"
}
```
Here's an implementation of a minimum spanning tree function.  Well, actually 3 implementations, defaulting to Kruskal's algorithm.

Component: **combinatorics**

_Issue created by migration from https://trac.sagemath.org/ticket/1088_





---

archive/issue_events_010489.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2007-11-03T20:45:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1088",
    "milestone_number": null,
    "milestone_title": "sage-2.8.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1088#event-10489"
}
```



---

archive/issue_events_010490.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2007-11-03T20:45:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1088",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "c: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1088#event-10490"
}
```



---

archive/issue_events_010491.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2007-11-03T20:45:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1088",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1088#event-10491"
}
```



---

archive/issue_events_010492.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2007-11-03T20:45:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1088",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1088#event-10492"
}
```



---

archive/issue_events_010493.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2007-11-03T20:45:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1088",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
    "label_color": "eeeeee",
    "label_name": "c: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1088#event-10493"
}
```



---

archive/issue_events_010494.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2007-11-03T20:45:05Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1088",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1088#event-10494"
}
```



---

archive/issue_comments_004698.json:
```json
{
    "body": "Attachment: **[minspantree.patch.gz](https://github.com/sagemath/sage/files/ticket1088/minspantree.patch.gz)**",
    "created_at": "2007-11-03T20:45:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1088",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1088#issuecomment-4698",
    "user": "https://github.com/jasongrout"
}
```

Attachment: **[minspantree.patch.gz](https://github.com/sagemath/sage/files/ticket1088/minspantree.patch.gz)**



---

archive/issue_events_010495.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2007-11-03T23:20:23Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/1088",
    "title_is": "[tested] graphs: minimum spanning tree function",
    "title_was": "graphs: minimum spanning tree function",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1088#event-10495"
}
```



---

archive/issue_comments_004699.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[doc_1088.patch.gz](https://github.com/sagemath/sage/files/ticket1088/doc_1088.patch.gz)**",
    "created_at": "2007-11-03T23:20:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1088",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1088#issuecomment-4699",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[doc_1088.patch.gz](https://github.com/sagemath/sage/files/ticket1088/doc_1088.patch.gz)**



---

archive/issue_events_010496.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-11-03T23:38:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1088",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1088#event-10496"
}
```



---

archive/issue_comments_004700.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nI believe there is a bug in the code change of the doc_1088 patch above.  The third patch (1088-correct-bugs-improve-doctests.patch) reverses this change, reimplements the doctests, and makes the code faster by sorting using key instead of cmp.\n\nThe third doctest should be applied after the first two doctests.",
    "created_at": "2007-11-06T22:36:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1088",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1088#issuecomment-4700",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:3" align="right">comment:3</div>

I believe there is a bug in the code change of the doc_1088 patch above.  The third patch (1088-correct-bugs-improve-doctests.patch) reverses this change, reimplements the doctests, and makes the code faster by sorting using key instead of cmp.

The third doctest should be applied after the first two doctests.



---

archive/issue_comments_004701.json:
```json
{
    "body": "Attachment: **[1088-correct-bugs-improve-doctests.patch.gz](https://github.com/sagemath/sage/files/ticket1088/1088-correct-bugs-improve-doctests.patch.gz)**",
    "created_at": "2007-11-06T22:37:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1088",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1088#issuecomment-4701",
    "user": "https://github.com/jasongrout"
}
```

Attachment: **[1088-correct-bugs-improve-doctests.patch.gz](https://github.com/sagemath/sage/files/ticket1088/1088-correct-bugs-improve-doctests.patch.gz)**



---

archive/issue_comments_004702.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nThe third patch above also corrects a bug in the original code.",
    "created_at": "2007-11-06T22:38:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1088",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1088#issuecomment-4702",
    "user": "https://github.com/jasongrout"
}
```

<div id="comment:4" align="right">comment:4</div>

The third patch above also corrects a bug in the original code.



---

archive/issue_events_010497.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2007-11-06T22:38:20Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/1088",
    "title_is": "graphs: minimum spanning tree function",
    "title_was": "[tested] graphs: minimum spanning tree function",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1088#event-10497"
}
```



---

archive/issue_events_010498.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-06T23:54:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1088",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1088#event-10498"
}
```
