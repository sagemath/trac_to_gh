# Issue 1872: [with patch; with positive review] serious bug in pickling elliptic curves

archive/issues_001872.json:
```json
{
    "body": "\n```\nsage: E = EllipticCurve('185c1')\nsage: E.gens()\n[(-5/4 : 3/8 : 1)]\nsage: loads(dumps(E))\n---------------------------------------------------------------------------\n<type 'exceptions.AttributeError'>        Traceback (most recent call last)\n\n/Users/was/papers/bsdalg/newdata/heegner_index_rank1/data/<ipython console> in <module>()\n\n/Users/was/papers/bsdalg/newdata/heegner_index_rank1/data/sage_object.pyx in sage.structure.sage_object.loads()\n\n/Users/was/papers/bsdalg/newdata/heegner_index_rank1/data/sage_object.pyx in sage.structure.sage_object.loads()\n\n/Users/was/s/local/lib/python2.5/site-packages/sage/schemes/elliptic_curves/ell_point.py in make_point(X, v)\n    648     # It looks like there's generic code to do this, but it's been commented out. \n    649     #\n    650     # Here we create a new (equivalent) parent manually. \n--> 651     del X._Scheme__ring_point_homset\n    652     return EllipticCurvePoint_field(X, v)\n\n<type 'str'>: (<type 'exceptions.AttributeError'>, AttributeError(\"'EllipticCurve_rational_field' object has no attribute '_EllipticCurve_generic__ainvs'\",))\n\n```\n\nAssignee: @williamstein\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/1872\n\n",
    "closed_at": "2008-01-21T09:37:59Z",
    "created_at": "2008-01-20T22:22:44Z",
    "labels": [
        "component: number theory",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.10.1",
    "title": "[with patch; with positive review] serious bug in pickling elliptic curves",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/1872",
    "user": "https://github.com/williamstein"
}
```

```
sage: E = EllipticCurve('185c1')
sage: E.gens()
[(-5/4 : 3/8 : 1)]
sage: loads(dumps(E))
---------------------------------------------------------------------------
<type 'exceptions.AttributeError'>        Traceback (most recent call last)

/Users/was/papers/bsdalg/newdata/heegner_index_rank1/data/<ipython console> in <module>()

/Users/was/papers/bsdalg/newdata/heegner_index_rank1/data/sage_object.pyx in sage.structure.sage_object.loads()

/Users/was/papers/bsdalg/newdata/heegner_index_rank1/data/sage_object.pyx in sage.structure.sage_object.loads()

/Users/was/s/local/lib/python2.5/site-packages/sage/schemes/elliptic_curves/ell_point.py in make_point(X, v)
    648     # It looks like there's generic code to do this, but it's been commented out. 
    649     #
    650     # Here we create a new (equivalent) parent manually. 
--> 651     del X._Scheme__ring_point_homset
    652     return EllipticCurvePoint_field(X, v)

<type 'str'>: (<type 'exceptions.AttributeError'>, AttributeError("'EllipticCurve_rational_field' object has no attribute '_EllipticCurve_generic__ainvs'",))

```

Assignee: @williamstein

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/1872





---

archive/issue_comments_012349.json:
```json
{
    "body": "Attachment [trac-1872.patch](tarball://root/attachments/some-uuid/ticket1872/trac-1872.patch) by @williamstein created at 2008-01-21 06:33:37\n\nthis fixes the problem",
    "created_at": "2008-01-21T06:33:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1872",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1872#issuecomment-12349",
    "user": "https://github.com/williamstein"
}
```

Attachment [trac-1872.patch](tarball://root/attachments/some-uuid/ticket1872/trac-1872.patch) by @williamstein created at 2008-01-21 06:33:37

this fixes the problem



---

archive/issue_events_006339.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1872",
    "rename": {
        "from": "serious bug in pickling elliptic curves",
        "to": "[with patch; needs review] serious bug in pickling elliptic curves"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1872#event-6339"
}
```



---

archive/issue_comments_012350.json:
```json
{
    "body": "<a id='comment:1'></a>\nThe patch simply deletes some functions, deletes some commented out code, and names a variable something meaningful instead of meaningless.",
    "created_at": "2008-01-21T06:35:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1872",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1872#issuecomment-12350",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'></a>
The patch simply deletes some functions, deletes some commented out code, and names a variable something meaningful instead of meaningless.



---

archive/issue_events_006340.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1872",
    "rename": {
        "from": "[with patch; needs review] serious bug in pickling elliptic curves",
        "to": "[with patch; with positive review] serious bug in pickling elliptic curves"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1872#event-6340"
}
```



---

archive/issue_comments_012351.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-01-21T09:37:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1872",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1872#issuecomment-12351",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_012352.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 2.10.1.alpha0",
    "created_at": "2008-01-21T09:37:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1872",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1872#issuecomment-12352",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 2.10.1.alpha0



---

archive/issue_events_006341.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-01-21T09:37:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/1872",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1872#event-6341"
}
```
