# Issue 1754: gnutls self signed certificates do not work with firefox 3 (pre-beta 3)

archive/issues_001754.json:
```json
{
    "body": "Using the latest beta version of firefox 3, if one tries to connect to the notebook via ssl, the following happens as outlined in the screenshots. \n\n\nAssignee: @yqiang\n\nKeywords: gnutls https ssl\n\nResolution: wontfix\n\nIssue created by migration from https://trac.sagemath.org/ticket/1754\n\n",
    "closed_at": "2008-10-26T16:03:37Z",
    "created_at": "2008-01-11T00:44:18Z",
    "labels": [
        "component: documentation"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "gnutls self signed certificates do not work with firefox 3 (pre-beta 3)",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/1754",
    "user": "https://github.com/yqiang"
}
```
Using the latest beta version of firefox 3, if one tries to connect to the notebook via ssl, the following happens as outlined in the screenshots. 


Assignee: @yqiang

Keywords: gnutls https ssl

Resolution: wontfix

Issue created by migration from https://trac.sagemath.org/ticket/1754





---

archive/issue_comments_011894.json:
```json
{
    "body": "What happens when you connect to the notebook server.",
    "created_at": "2008-01-11T00:44:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1754#issuecomment-11894",
    "user": "https://github.com/yqiang"
}
```

What happens when you connect to the notebook server.



---

archive/issue_comments_011895.json:
```json
{
    "body": "Attachment [firefox_gnutls_1.png](tarball://root/attachments/some-uuid/ticket1754/firefox_gnutls_1.png) by @yqiang created at 2008-01-11 00:45:18\n\nSecurity dialogue box",
    "created_at": "2008-01-11T00:45:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1754#issuecomment-11895",
    "user": "https://github.com/yqiang"
}
```

Attachment [firefox_gnutls_1.png](tarball://root/attachments/some-uuid/ticket1754/firefox_gnutls_1.png) by @yqiang created at 2008-01-11 00:45:18

Security dialogue box



---

archive/issue_comments_011896.json:
```json
{
    "body": "Attachment [firefox_gnutls_3.png](tarball://root/attachments/some-uuid/ticket1754/firefox_gnutls_3.png) by @yqiang created at 2008-01-11 00:45:45\n\nError message",
    "created_at": "2008-01-11T00:45:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1754#issuecomment-11896",
    "user": "https://github.com/yqiang"
}
```

Attachment [firefox_gnutls_3.png](tarball://root/attachments/some-uuid/ticket1754/firefox_gnutls_3.png) by @yqiang created at 2008-01-11 00:45:45

Error message



---

archive/issue_comments_011897.json:
```json
{
    "body": "<a id='comment:1'></a>Confirmed FF3B1 OSX 10.5.1\n\nSimilar problems elsewhere:\n* https://bugs.launchpad.net/ubuntu/+source/cupsys/+bug/187341\n* http://forums.mozillazine.org/viewtopic.php?p=3214810&sid=83ec36b154173a769dcf000463a6b153\n* http://groups.google.com/group/mozilla.feedback.firefox.prerelease/browse_thread/thread/885b8914a0cc9e80",
    "created_at": "2008-03-04T00:11:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1754#issuecomment-11897",
    "user": "https://trac.sagemath.org/admin/accounts/users/edrex"
}
```

<a id='comment:1'></a>Confirmed FF3B1 OSX 10.5.1

Similar problems elsewhere:
* https://bugs.launchpad.net/ubuntu/+source/cupsys/+bug/187341
* http://forums.mozillazine.org/viewtopic.php?p=3214810&sid=83ec36b154173a769dcf000463a6b153
* http://groups.google.com/group/mozilla.feedback.firefox.prerelease/browse_thread/thread/885b8914a0cc9e80



---

archive/issue_comments_011898.json:
```json
{
    "body": "<a id='comment:2'></a>Any self signed certificate causes issues with FF 3B4. So I don't think this has anything to do with Sage or GNUTLS. The solution is to add an exception rule so that firefox will access the site.\n\nI don't consider this a bug since it is a Firefox issue. The workaround needs to be documented and then this issue will be closed.\n\nCheers,\n\nMichael",
    "created_at": "2008-03-23T19:46:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1754#issuecomment-11898",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>Any self signed certificate causes issues with FF 3B4. So I don't think this has anything to do with Sage or GNUTLS. The solution is to add an exception rule so that firefox will access the site.

I don't consider this a bug since it is a Firefox issue. The workaround needs to be documented and then this issue will be closed.

Cheers,

Michael



---

archive/issue_comments_011899.json:
```json
{
    "body": "<a id='comment:3'></a>In general, self-signed certs can be in FF3 used by adding an exception. Certain certs (such as those generated by gnutls (openssl also?) for sage) result in the error above when attempting to add an exception. Either this is a Firefox 3 bug or it is a legitimate (but undocumented) tightening of standards for certs. The FF team needs to decide which is the case and either fix the bug or document the changes in criteria for certs.\n\nRight now, the workaround is to use \"sage -inotebook\".",
    "created_at": "2008-03-23T22:22:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1754#issuecomment-11899",
    "user": "https://trac.sagemath.org/admin/accounts/users/edrex"
}
```

<a id='comment:3'></a>In general, self-signed certs can be in FF3 used by adding an exception. Certain certs (such as those generated by gnutls (openssl also?) for sage) result in the error above when attempting to add an exception. Either this is a Firefox 3 bug or it is a legitimate (but undocumented) tightening of standards for certs. The FF team needs to decide which is the case and either fix the bug or document the changes in criteria for certs.

Right now, the workaround is to use "sage -inotebook".



---

archive/issue_comments_011900.json:
```json
{
    "body": "<a id='comment:4'></a>I just tried adding a security exception in Firefox 3 Beta 5 and it seems to work correctly (i.e., the firefox team fixed adding security exceptions of self-signed certificates).",
    "created_at": "2008-04-02T20:52:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1754#issuecomment-11900",
    "user": "https://github.com/yqiang"
}
```

<a id='comment:4'></a>I just tried adding a security exception in Firefox 3 Beta 5 and it seems to work correctly (i.e., the firefox team fixed adding security exceptions of self-signed certificates).



---

archive/issue_comments_011901.json:
```json
{
    "body": "<a id='comment:5'></a>I'm also able to add an exception with FF3B5. The procedure for adding an exception to FF3 is non-obvious so perhaps it should be in the documentation (Wiki FAQs?). Changing to a documentation issue.\n\nQ: When I try to access the notebook using Firefox 3 I receive an error message:\n\n```\nlocalhost:8000 uses an invalid security certificate.\n\nThe certificate is not trusted because the issuer certificate is unknown.\nThe certificate is not valid for any server names.\n\n(Error code: sec_error_unknown_issuer)}}}\n\nA: Add a certificate exception:\n # Open the Firefox preferences\n # Go to the _Advanced/Encryption_ tab.\n # Click _View Certificates_\n # In the _Servers_ tab, click _Add Exception..._\n # Paste the URL for your notebook , eg https://localhost:8000/",
    "created_at": "2008-04-02T22:41:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1754#issuecomment-11901",
    "user": "https://trac.sagemath.org/admin/accounts/users/edrex"
}
```

<a id='comment:5'></a>I'm also able to add an exception with FF3B5. The procedure for adding an exception to FF3 is non-obvious so perhaps it should be in the documentation (Wiki FAQs?). Changing to a documentation issue.

Q: When I try to access the notebook using Firefox 3 I receive an error message:

```
localhost:8000 uses an invalid security certificate.

The certificate is not trusted because the issuer certificate is unknown.
The certificate is not valid for any server names.

(Error code: sec_error_unknown_issuer)}}}

A: Add a certificate exception:
 # Open the Firefox preferences
 # Go to the _Advanced/Encryption_ tab.
 # Click _View Certificates_
 # In the _Servers_ tab, click _Add Exception..._
 # Paste the URL for your notebook , eg https://localhost:8000/



---

archive/issue_comments_011902.json:
```json
{
    "body": "Changing type from defect to task.",
    "created_at": "2008-04-02T22:41:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1754#issuecomment-11902",
    "user": "https://trac.sagemath.org/admin/accounts/users/edrex"
}
```

Changing type from defect to task.



---

archive/issue_comments_011903.json:
```json
{
    "body": "Changing component from notebook to documentation.",
    "created_at": "2008-04-02T22:41:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1754#issuecomment-11903",
    "user": "https://trac.sagemath.org/admin/accounts/users/edrex"
}
```

Changing component from notebook to documentation.



---

archive/issue_comments_011904.json:
```json
{
    "body": "<a id='comment:6'></a>Can someone close this now? You can add exceptions with firefox 3 (officially released today) now.",
    "created_at": "2008-06-17T21:56:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1754#issuecomment-11904",
    "user": "https://github.com/yqiang"
}
```

<a id='comment:6'></a>Can someone close this now? You can add exceptions with firefox 3 (officially released today) now.



---

archive/issue_comments_011905.json:
```json
{
    "body": "<a id='comment:7'></a>Yep, this is closed as wontfix.\n\nCheers,\n\nMichael",
    "created_at": "2008-10-26T16:03:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1754#issuecomment-11905",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:7'></a>Yep, this is closed as wontfix.

Cheers,

Michael



---

archive/issue_comments_011906.json:
```json
{
    "body": "Resolution: wontfix",
    "created_at": "2008-10-26T16:03:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1754#issuecomment-11906",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: wontfix



---

archive/issue_events_004247.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-10-26T16:03:37Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/1754",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1754#event-4247"
}
```
