# Issue 1768: get emails from any trac change on a certain component

archive/issues_001768.json:
```json
{
    "assignees": [],
    "body": "For example, I don't own a certain component, but I am still passionate enough about it to want to know whenever anything happens in it. Right now, \"Settings\" is only name and e-mail...\n\n**Assignee:** @williamstein\n\n**CC:**  cc @kcrisman @embray @slel\n\n**Keywords:** Trac\n\nIssue created by migration from https://trac.sagemath.org/ticket/1768\n\n",
    "closed_at": "2018-08-03T17:23:51Z",
    "created_at": "2008-01-14T00:44:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20website/wiki",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "get emails from any trac change on a certain component",
    "type": "issue",
    "updated_at": "2018-08-03T17:23:51Z",
    "url": "https://github.com/sagemath/sage/issues/1768",
    "user": "https://github.com/rlmill"
}
```
For example, I don't own a certain component, but I am still passionate enough about it to want to know whenever anything happens in it. Right now, "Settings" is only name and e-mail...

**Assignee:** @williamstein

**CC:**  cc @kcrisman @embray @slel

**Keywords:** Trac

Issue created by migration from https://trac.sagemath.org/ticket/1768





---

archive/issue_events_013812.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-01-14T00:44:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1768",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20website/wiki",
    "label_color": "08517b",
    "label_name": "component: website/wiki",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1768#event-13812"
}
```



---

archive/issue_events_013813.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-01-14T00:44:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1768",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "008080",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1768#event-13813"
}
```



---

archive/issue_events_013814.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-01-14T00:44:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1768",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "008080",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1768#event-13814"
}
```



---

archive/issue_comments_008604.json:
```json
{
    "body": "<a id='comment:1'></a>\nThe [AnnouncerPlugin](http://trac-hacks.org/wiki/AnnouncerPlugin) seems promising.",
    "created_at": "2010-01-16T20:13:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1768",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1768#issuecomment-8604",
    "user": "https://github.com/qed777"
}
```

<a id='comment:1'></a>
The [AnnouncerPlugin](http://trac-hacks.org/wiki/AnnouncerPlugin) seems promising.



---

archive/issue_events_013815.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:34:36Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/1768",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1768#event-13815"
}
```



---

archive/issue_events_013816.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:34:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1768",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1768#event-13816"
}
```



---

archive/issue_events_013817.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/1768",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1768#event-13817"
}
```



---

archive/issue_events_013818.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1768",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1768#event-13818"
}
```



---

archive/issue_events_013819.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:19:32Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/1768",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1768#event-13819"
}
```



---

archive/issue_events_013820.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:19:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1768",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1768#event-13820"
}
```



---

archive/issue_events_013821.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/1768",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1768#event-13821"
}
```



---

archive/issue_events_013822.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1768",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1768#event-13822"
}
```



---

archive/issue_comments_008605.json:
```json
{
    "body": "**Changing keywords** from \"\" to \"Trac\".",
    "created_at": "2018-08-01T16:52:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1768",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1768#issuecomment-8605",
    "user": "https://github.com/slel"
}
```

**Changing keywords** from "" to "Trac".



---

archive/issue_events_013823.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-08-03T17:23:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1768",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "008080",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1768#event-13823"
}
```



---

archive/issue_events_013824.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-08-03T17:23:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1768",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1768#event-13824"
}
```



---

archive/issue_events_013825.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-08-03T17:23:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/1768",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1768#event-13825"
}
```



---

archive/issue_comments_008606.json:
```json
{
    "body": "<a id='comment:8'></a>\n1. I don't think Trac-related issues should have tickets here, ironically.  Most stuff related to the Sage Trac plugin is tracked at https://github.com/sagemath/sage_trac_plugin\n2. I don't think there's a way out of-the-box to \"subscribe\" to specific components like this via e-mail; it would probably require a small plugin and I'm not sufficiently motivated to work on Trac plugins that aren't directly related to improving the overall workflow and/or migrating/integrating with GitLab/GitHub.\n3. As it is you can create a custom query for whatever criteria you want and subscribe to it as an RSS feed: https://trac.sagemath.org/query?status=new&component=algebra&format=rss&col=id&col=summary&col=component&col=type&col=status&col=priority&col=milestone&order=priority",
    "created_at": "2018-08-03T17:23:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1768",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1768#issuecomment-8606",
    "user": "https://github.com/embray"
}
```

<a id='comment:8'></a>
1. I don't think Trac-related issues should have tickets here, ironically.  Most stuff related to the Sage Trac plugin is tracked at https://github.com/sagemath/sage_trac_plugin
2. I don't think there's a way out of-the-box to "subscribe" to specific components like this via e-mail; it would probably require a small plugin and I'm not sufficiently motivated to work on Trac plugins that aren't directly related to improving the overall workflow and/or migrating/integrating with GitLab/GitHub.
3. As it is you can create a custom query for whatever criteria you want and subscribe to it as an RSS feed: https://trac.sagemath.org/query?status=new&component=algebra&format=rss&col=id&col=summary&col=component&col=type&col=status&col=priority&col=milestone&order=priority
