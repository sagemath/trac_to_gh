# Issue 1972: [with patch; with positive review] programming guide -- fix a confusing typo

archive/issues_001972.json:
```json
{
    "body": "\n\nAssignee: tba\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/1972\n\n",
    "closed_at": "2008-01-29T13:17:17Z",
    "created_at": "2008-01-29T12:58:36Z",
    "labels": [
        "component: documentation",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.10.1",
    "title": "[with patch; with positive review] programming guide -- fix a confusing typo",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/1972",
    "user": "https://github.com/williamstein"
}
```


Assignee: tba

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/1972





---

archive/issue_events_006758.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1972",
    "rename": {
        "from": "programming guide -- fix a confusing typo",
        "to": "[with patch; needs trivial review] programming guide -- fix a confusing typo"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1972#event-6758"
}
```



---

archive/issue_comments_013314.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [doc-1972.patch](tarball://root/attachments/some-uuid/ticket1972/doc-1972.patch) by @williamstein created at 2008-01-29 12:59:48",
    "created_at": "2008-01-29T12:59:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1972",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1972#issuecomment-13314",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'></a>
Attachment [doc-1972.patch](tarball://root/attachments/some-uuid/ticket1972/doc-1972.patch) by @williamstein created at 2008-01-29 12:59:48



---

archive/issue_comments_013315.json:
```json
{
    "body": "<a id='comment:2'></a>\nLooks good to me.",
    "created_at": "2008-01-29T13:15:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1972",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1972#issuecomment-13315",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Looks good to me.



---

archive/issue_events_006759.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1972",
    "rename": {
        "from": "[with patch; needs trivial review] programming guide -- fix a confusing typo",
        "to": "[with patch; with positive review] programming guide -- fix a confusing typo"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1972#event-6759"
}
```



---

archive/issue_comments_013316.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-01-29T13:17:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1972",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1972#issuecomment-13316",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_013317.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 2.10.1.rc3",
    "created_at": "2008-01-29T13:17:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1972",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1972#issuecomment-13317",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 2.10.1.rc3



---

archive/issue_events_006760.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-01-29T13:17:17Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/1972",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1972#event-6760"
}
```
