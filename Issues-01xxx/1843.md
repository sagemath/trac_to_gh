# Issue 1843: [with patch; positive review] cputime for gp interface

archive/issues_001843.json:
```json
{
    "body": "\n\nAssignee: @williamstein\n\nCC:  @malb\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/1843\n\n",
    "closed_at": "2008-01-21T05:57:44Z",
    "created_at": "2008-01-19T01:14:11Z",
    "labels": [
        "component: interfaces"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.10.1",
    "title": "[with patch; positive review] cputime for gp interface",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/1843",
    "user": "https://github.com/williamstein"
}
```


Assignee: @williamstein

CC:  @malb

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/1843





---

archive/issue_comments_012146.json:
```json
{
    "body": "Attachment [trac-1843.patch](tarball://root/attachments/some-uuid/ticket1843/trac-1843.patch) by @williamstein created at 2008-01-19 01:14:26",
    "created_at": "2008-01-19T01:14:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1843#issuecomment-12146",
    "user": "https://github.com/williamstein"
}
```

Attachment [trac-1843.patch](tarball://root/attachments/some-uuid/ticket1843/trac-1843.patch) by @williamstein created at 2008-01-19 01:14:26



---

archive/issue_comments_012147.json:
```json
{
    "body": "<a id='comment:1'></a>\nThe actual code looks good, but note that \n* no doctests were added\n* this patch will conflict with #1714 (but that is my fault because I didn't appropriately describe the attached patch in the ticket description)",
    "created_at": "2008-01-20T14:27:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1843#issuecomment-12147",
    "user": "https://github.com/malb"
}
```

<a id='comment:1'></a>
The actual code looks good, but note that 
* no doctests were added
* this patch will conflict with #1714 (but that is my fault because I didn't appropriately describe the attached patch in the ticket description)



---

archive/issue_events_006231.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1843",
    "rename": {
        "from": "[with patch; needs review] cputime for gp interface",
        "to": "[with patch; mostly positive review] cputime for gp interface"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1843#event-6231"
}
```



---

archive/issue_comments_012148.json:
```json
{
    "body": "this adds doctests  (apply both patches)",
    "created_at": "2008-01-21T05:50:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1843#issuecomment-12148",
    "user": "https://github.com/williamstein"
}
```

this adds doctests  (apply both patches)



---

archive/issue_comments_012149.json:
```json
{
    "body": "<a id='comment:3'></a>\nAttachment [trac-1843-part2.patch](tarball://root/attachments/some-uuid/ticket1843/trac-1843-part2.patch) by @mwhansen created at 2008-01-21 05:52:45\n\nThey both apply and work for me.",
    "created_at": "2008-01-21T05:52:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1843#issuecomment-12149",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:3'></a>
Attachment [trac-1843-part2.patch](tarball://root/attachments/some-uuid/ticket1843/trac-1843-part2.patch) by @mwhansen created at 2008-01-21 05:52:45

They both apply and work for me.



---

archive/issue_events_006232.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1843",
    "rename": {
        "from": "[with patch; mostly positive review] cputime for gp interface",
        "to": "[with patch; positive review] cputime for gp interface"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1843#event-6232"
}
```



---

archive/issue_events_006233.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-01-21T05:57:44Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/1843",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1843#event-6233"
}
```



---

archive/issue_comments_012150.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-01-21T05:57:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1843#issuecomment-12150",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_012151.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 2.10.1.alpha1",
    "created_at": "2008-01-21T05:57:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1843#issuecomment-12151",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 2.10.1.alpha1
