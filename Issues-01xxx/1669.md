# Issue 1669: remove bogus recommendation to set SAGE_ATLAS when numpy fails

archive/issues_001669.json:
```json
{
    "assignees": [],
    "body": "When numpy fails to build it prints the following error message which is no longer valid:\n\n```\n Error building numpy.\nTry setting SAGE_ATLAS to the directory that contains lib/libatlas.a ?\n```\n\nAssignee: **jkantor**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1669_\n\n",
    "closed_at": "2008-01-09T01:56:05Z",
    "created_at": "2008-01-03T15:51:27Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.10",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "remove bogus recommendation to set SAGE_ATLAS when numpy fails",
    "type": "issue",
    "updated_at": "2008-01-09T01:56:05Z",
    "url": "https://github.com/sagemath/sage/issues/1669",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
When numpy fails to build it prints the following error message which is no longer valid:

```
 Error building numpy.
Try setting SAGE_ATLAS to the directory that contains lib/libatlas.a ?
```

Assignee: **jkantor**

_Issue created by migration from https://trac.sagemath.org/ticket/1669_





---

archive/issue_events_014211.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-01-03T15:51:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1669",
    "milestone_number": null,
    "milestone_title": "sage-2.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1669#event-14211"
}
```



---

archive/issue_events_014212.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-01-03T15:51:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1669",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1669#event-14212"
}
```



---

archive/issue_events_014213.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-01-03T15:51:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1669",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1669#event-14213"
}
```



---

archive/issue_events_014214.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-01-03T15:51:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1669",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1669#event-14214"
}
```



---

archive/issue_comments_008157.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nThis will be fixed via the new numpy.spkg linked from #1720.\n\nCheers,\n\nMichael",
    "created_at": "2008-01-09T00:01:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1669",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1669#issuecomment-8157",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:1'>Comment 1:</a>
This will be fixed via the new numpy.spkg linked from #1720.

Cheers,

Michael



---

archive/issue_events_014215.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-01-09T01:56:05Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1669",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1669#event-14215"
}
```



---

archive/issue_comments_008158.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nFixed in Sage 2.10.alpah1.",
    "created_at": "2008-01-09T01:56:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1669",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1669#issuecomment-8158",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:2'>Comment 2:</a>
Fixed in Sage 2.10.alpah1.
