# Issue 1687: makefile program -- "make distclean" doesn't delete test.log

archive/issues_001687.json:
```json
{
    "assignees": [],
    "body": "\n\nAssignee: **cwitty**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1687_\n\n",
    "closed_at": "2008-01-05T00:28:23Z",
    "created_at": "2008-01-05T00:25:19Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.9.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "makefile program -- \"make distclean\" doesn't delete test.log",
    "type": "issue",
    "updated_at": "2008-01-05T00:28:23Z",
    "url": "https://github.com/sagemath/sage/issues/1687",
    "user": "https://github.com/williamstein"
}
```


Assignee: **cwitty**

_Issue created by migration from https://trac.sagemath.org/ticket/1687_





---

archive/issue_events_014340.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-01-05T00:25:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1687",
    "milestone_number": null,
    "milestone_title": "sage-2.9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1687#event-14340"
}
```



---

archive/issue_events_014341.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-01-05T00:25:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1687",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1687#event-14341"
}
```



---

archive/issue_events_014342.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-01-05T00:25:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1687",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1687#event-14342"
}
```



---

archive/issue_comments_008257.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nsee http://sage.math.washington.edu/home/was/tmp/makefile",
    "created_at": "2008-01-05T00:26:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1687",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1687#issuecomment-8257",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'>Comment 1:</a>
see http://sage.math.washington.edu/home/was/tmp/makefile



---

archive/issue_events_014343.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-01-05T00:28:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1687",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1687#event-14343"
}
```



---

archive/issue_events_014344.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-01-05T00:28:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1687",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1687#event-14344"
}
```



---

archive/issue_comments_008258.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nLooks good to me. Merged in 2.9.2.rc1.\n\nCheers,\n\nMichael",
    "created_at": "2008-01-05T00:28:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1687",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1687#issuecomment-8258",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:2'>Comment 2:</a>
Looks good to me. Merged in 2.9.2.rc1.

Cheers,

Michael
