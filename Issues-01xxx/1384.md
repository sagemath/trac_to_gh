# Issue 1384: 2.8.15.rc0: fix numerical doctest fallout on PCC

archive/issues_001384.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "There are various doctest failures related to numerical noise and different order of result on PPC for\n\n```\ndevel/sage-main/sage/rings/polynomial/complex_roots.py\ndevel/sage-main/sage/rings/polynomial/polynomial_element.pyx\ndevel/sage-main/sage/rings/qqbar.py\n```\nPatch coming shortly.\n\nCheers,\n\nMichael\n\nComponent: **numerical**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1384_\n\n",
    "closed_at": "2007-12-03T19:28:51Z",
    "created_at": "2007-12-03T19:10:19Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20numerical",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.15",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "2.8.15.rc0: fix numerical doctest fallout on PCC",
    "type": "issue",
    "updated_at": "2007-12-03T19:28:51Z",
    "url": "https://github.com/sagemath/sage/issues/1384",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
There are various doctest failures related to numerical noise and different order of result on PPC for

```
devel/sage-main/sage/rings/polynomial/complex_roots.py
devel/sage-main/sage/rings/polynomial/polynomial_element.pyx
devel/sage-main/sage/rings/qqbar.py
```
Patch coming shortly.

Cheers,

Michael

Component: **numerical**

_Issue created by migration from https://trac.sagemath.org/ticket/1384_





---

archive/issue_events_013599.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-03T19:10:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1384",
    "milestone_number": null,
    "milestone_title": "sage-2.8.15",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1384#event-13599"
}
```



---

archive/issue_events_013600.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-03T19:10:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1384",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20numerical",
    "label_color": "0000ff",
    "label_name": "c: numerical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1384#event-13600"
}
```



---

archive/issue_events_013601.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-03T19:10:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1384",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1384#event-13601"
}
```



---

archive/issue_events_013602.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-03T19:10:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1384",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1384#event-13602"
}
```



---

archive/issue_events_013603.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-03T19:10:19Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1384",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1384#event-13603"
}
```



---

archive/issue_comments_006462.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[Sage-2.8.15.rc1-fix-numerical-noise-OSX-PPC.patch.gz](https://github.com/sagemath/sage/files/ticket1384/Sage-2.8.15.rc1-fix-numerical-noise-OSX-PPC.patch.gz)**",
    "created_at": "2007-12-03T19:20:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1384",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1384#issuecomment-6462",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[Sage-2.8.15.rc1-fix-numerical-noise-OSX-PPC.patch.gz](https://github.com/sagemath/sage/files/ticket1384/Sage-2.8.15.rc1-fix-numerical-noise-OSX-PPC.patch.gz)**



---

archive/issue_comments_006463.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,9 +1,10 @@\n There are various doctest failures related to numerical noise and different order of result on PPC for\n \n+```\n devel/sage-main/sage/rings/polynomial/complex_roots.py\n devel/sage-main/sage/rings/polynomial/polynomial_element.pyx\n devel/sage-main/sage/rings/qqbar.py\n-\n+```\n Patch coming shortly.\n \n Cheers,\n``````\n",
    "created_at": "2007-12-03T19:22:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1384",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1384#issuecomment-6463",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,9 +1,10 @@
 There are various doctest failures related to numerical noise and different order of result on PPC for
 
+```
 devel/sage-main/sage/rings/polynomial/complex_roots.py
 devel/sage-main/sage/rings/polynomial/polynomial_element.pyx
 devel/sage-main/sage/rings/qqbar.py
-
+```
 Patch coming shortly.
 
 Cheers,
``````




---

archive/issue_comments_006464.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nI needed another minimal fix for x86-64 Linux, but this is now in.\n\nCheers,\n\nMichael",
    "created_at": "2007-12-03T19:28:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1384",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1384#issuecomment-6464",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

I needed another minimal fix for x86-64 Linux, but this is now in.

Cheers,

Michael



---

archive/issue_comments_006465.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nMerged in 2.8.15.rc1.",
    "created_at": "2007-12-03T19:28:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1384",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1384#issuecomment-6465",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Merged in 2.8.15.rc1.



---

archive/issue_events_013604.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-03T19:28:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1384",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1384#event-13604"
}
```
