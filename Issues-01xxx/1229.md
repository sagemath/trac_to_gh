# Issue 1229: 2.8.13.rc1: sage/calculus/calculus.py doctest failure

archive/issues_001229.json:
```json
{
    "assignees": [],
    "body": "\n```\nFile \"calculus.py\", line 209:\n     sage: expand((x+y)^3)\nExpected:\n     y^3 + 3*x*y^2 + 3*x^2*y + x^3\nGot:\n     x^6 + 3*x^5 + 3*x^4 + x^3\n```\nThis is likely fallout from #1215.\n\n\nCheers,\n\nMichael\n\nAssignee: **mabshoff**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1229_\n\n",
    "closed_at": "2007-11-21T13:01:01Z",
    "created_at": "2007-11-20T23:06:20Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.13",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "2.8.13.rc1: sage/calculus/calculus.py doctest failure",
    "type": "issue",
    "updated_at": "2007-11-21T13:01:01Z",
    "url": "https://github.com/sagemath/sage/issues/1229",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

```
File "calculus.py", line 209:
     sage: expand((x+y)^3)
Expected:
     y^3 + 3*x*y^2 + 3*x^2*y + x^3
Got:
     x^6 + 3*x^5 + 3*x^4 + x^3
```
This is likely fallout from #1215.


Cheers,

Michael

Assignee: **mabshoff**

_Issue created by migration from https://trac.sagemath.org/ticket/1229_





---

archive/issue_events_010422.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-20T23:06:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1229",
    "milestone_number": null,
    "milestone_title": "sage-2.8.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1229#event-10422"
}
```



---

archive/issue_events_010423.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-20T23:06:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1229",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1229#event-10423"
}
```



---

archive/issue_events_010424.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-20T23:06:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1229",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1229#event-10424"
}
```



---

archive/issue_events_010425.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-20T23:06:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1229",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1229#event-10425"
}
```



---

archive/issue_comments_005854.json:
```json
{
    "body": "Attachment: **[trac1229.patch.gz](https://github.com/sagemath/sage/files/ticket1229/trac1229.patch.gz)**",
    "created_at": "2007-11-21T12:49:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1229#issuecomment-5854",
    "user": "https://github.com/williamstein"
}
```

Attachment: **[trac1229.patch.gz](https://github.com/sagemath/sage/files/ticket1229/trac1229.patch.gz)**



---

archive/issue_comments_005855.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nMerged in 2.8.13.rc2.",
    "created_at": "2007-11-21T13:00:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1229#issuecomment-5855",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:1'>Comment 1:</a>
Merged in 2.8.13.rc2.



---

archive/issue_comments_005856.json:
```json
{
    "body": "Changed assignee from **@williamstein** to **mabshoff**",
    "created_at": "2007-11-21T13:00:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1229#issuecomment-5856",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

Changed assignee from **@williamstein** to **mabshoff**



---

archive/issue_events_010426.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-21T13:01:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1229",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1229#event-10426"
}
```



---

archive/issue_comments_005857.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nMerged in 2.8.13.rc2.",
    "created_at": "2007-11-21T13:01:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1229#issuecomment-5857",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:2'>Comment 2:</a>
Merged in 2.8.13.rc2.
