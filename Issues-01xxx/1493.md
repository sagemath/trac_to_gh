# Issue 1493: polybori doesn't free m4ri data on exit

archive/issues_001493.json:
```json
{
    "assignees": [
        "https://github.com/burcin"
    ],
    "body": "The polybori wrapper initializes m4ri by building some tables, but this is not freed on exit. As polybori is now imported from top level, this shows up as still reachable memory on valgrind for every sage session.\n\nThe bundle attached fixes this, along with a minor modification to remove a workaround for a polybori bug which is now fixed.\n\nComponent: **memleak**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1493_\n\n",
    "closed_at": "2007-12-14T02:28:18Z",
    "created_at": "2007-12-13T20:39:42Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/memleak",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "polybori doesn't free m4ri data on exit",
    "type": "issue",
    "updated_at": "2007-12-14T02:28:18Z",
    "url": "https://github.com/sagemath/sage/issues/1493",
    "user": "https://github.com/burcin"
}
```
The polybori wrapper initializes m4ri by building some tables, but this is not freed on exit. As polybori is now imported from top level, this shows up as still reachable memory on valgrind for every sage session.

The bundle attached fixes this, along with a minor modification to remove a workaround for a polybori bug which is now fixed.

Component: **memleak**

_Issue created by migration from https://trac.sagemath.org/ticket/1493_





---

archive/issue_events_014746.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2007-12-13T20:39:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1493",
    "milestone_number": null,
    "milestone_title": "sage-2.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1493#event-14746"
}
```



---

archive/issue_events_014747.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2007-12-13T20:39:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1493",
    "label": "https://github.com/sagemath/sage/labels/memleak",
    "label_color": "696969",
    "label_name": "memleak",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1493#event-14747"
}
```



---

archive/issue_events_014748.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2007-12-13T20:39:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1493",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1493#event-14748"
}
```



---

archive/issue_events_014749.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2007-12-13T20:39:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1493",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1493#event-14749"
}
```



---

archive/issue_events_014750.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2007-12-13T20:39:42Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1493",
    "subject": "https://github.com/burcin",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1493#event-14750"
}
```



---

archive/issue_events_014751.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-14T02:28:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1493",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1493#event-14751"
}
```



---

archive/issue_comments_006994.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[polybori_m4ri_free.hg.gz](https://github.com/sagemath/sage/files/ticket1493/polybori_m4ri_free.hg.gz)**\n\nMerged in 2.9.alpha7.",
    "created_at": "2007-12-14T02:28:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1493",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1493#issuecomment-6994",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[polybori_m4ri_free.hg.gz](https://github.com/sagemath/sage/files/ticket1493/polybori_m4ri_free.hg.gz)**

Merged in 2.9.alpha7.
