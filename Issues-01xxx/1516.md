# Issue 1516: jmol spkg, useable from command line

archive/issues_001516.json:
```json
{
    "body": "\n\n**Assignee:** @robertwb\n\nIssue created by migration from https://trac.sagemath.org/ticket/1516\n\n",
    "closed_at": "2008-01-05T14:29:46Z",
    "created_at": "2007-12-15T02:18:58Z",
    "labels": [
        "component: graphics",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.9.2",
    "title": "jmol spkg, useable from command line",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/1516",
    "user": "https://github.com/robertwb"
}
```


**Assignee:** @robertwb

Issue created by migration from https://trac.sagemath.org/ticket/1516





---

archive/issue_comments_007663.json:
```json
{
    "body": "**Changing assignee** from @williamstein to @robertwb.",
    "created_at": "2007-12-15T02:19:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1516",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1516#issuecomment-7663",
    "user": "https://github.com/robertwb"
}
```

**Changing assignee** from @williamstein to @robertwb.



---

archive/attachments_001187.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "jmol-cmdline.hg",
    "asset_url": "tarball://root/attachments/some-uuid/ticket1516/jmol-cmdline.hg",
    "created_at": "2007-12-16T10:08:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1516",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.hg",
    "user": "https://github.com/robertwb"
}
```



---

archive/issue_comments_007664.json:
```json
{
    "body": "<a id='comment:2'></a>\nSee spkg at http://sage.math.washington.edu/home/robertwb/3d/jmol-11.2.14.spkg\n\nNote, this is 14MB, but it is just the full Jmol bundle which has about 4MB docs, 6MB source, and 3M redundant applet binaries, not all of which we may want to include. \n\nI realize the bundle has other patches from other tickets (e.g. #1533), but these are needed.",
    "created_at": "2007-12-16T10:08:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1516",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1516#issuecomment-7664",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:2'></a>
See spkg at http://sage.math.washington.edu/home/robertwb/3d/jmol-11.2.14.spkg

Note, this is 14MB, but it is just the full Jmol bundle which has about 4MB docs, 6MB source, and 3M redundant applet binaries, not all of which we may want to include. 

I realize the bundle has other patches from other tickets (e.g. #1533), but these are needed.



---

archive/issue_events_007242.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2007-12-16T10:08:10Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1516",
    "rename": {
        "from": "jmol spkg, use from notebook and command line",
        "to": "jmol spkg, useable from command line"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1516#event-7242"
}
```



---

archive/issue_comments_007665.json:
```json
{
    "body": "<a id='comment:3'></a>\nExamples\n\n```\nsage: from sage.plot.plot3d.all import *\nsage: S = plot3d(lambda x, y: abs(zeta(x+y*i)), (-3,3), (-3,3), ['red','blue'])\nsage: S.show(viewer='jmol')\nsage: (S + axes3d(5)).show(viewer='jmol')\n```",
    "created_at": "2007-12-16T10:31:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1516",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1516#issuecomment-7665",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:3'></a>
Examples

```
sage: from sage.plot.plot3d.all import *
sage: S = plot3d(lambda x, y: abs(zeta(x+y*i)), (-3,3), (-3,3), ['red','blue'])
sage: S.show(viewer='jmol')
sage: (S + axes3d(5)).show(viewer='jmol')
```



---

archive/issue_events_007243.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-12-19T10:26:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/1516",
    "milestone": "sage-2.9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1516#event-7243"
}
```



---

archive/issue_events_007244.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2007-12-22T01:13:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/1516",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1516#event-7244"
}
```



---

archive/issue_events_007245.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-01-05T14:29:46Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/1516",
    "milestone": "sage-2.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1516#event-7245"
}
```



---

archive/issue_events_007246.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-01-05T14:29:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/1516",
    "milestone": "sage-2.9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1516#event-7246"
}
```



---

archive/issue_comments_007666.json:
```json
{
    "body": "<a id='comment:6'></a>\nThis was merged some time in 2.9.2.X.\n\nCheers,\n\nMichael",
    "created_at": "2008-01-05T14:29:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1516",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1516#issuecomment-7666",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:6'></a>
This was merged some time in 2.9.2.X.

Cheers,

Michael



---

archive/issue_events_007247.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-01-05T14:29:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/1516",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1516#event-7247"
}
```



---

archive/issue_events_007248.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-01-05T14:29:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/1516",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1516#event-7248"
}
```
