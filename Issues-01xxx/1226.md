# Issue 1226: cremona/homspace doctest prints "transposing ..."

archive/issues_001226.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "Subject says it all, ought to be easily fixed by grepping through the homspace source code.\n\nCheers,\n\nMichael\n\nComponent: **doctest coverage**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1226_\n\n",
    "closed_at": "2007-11-21T14:02:56Z",
    "created_at": "2007-11-20T22:59:40Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.13",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "cremona/homspace doctest prints \"transposing ...\"",
    "type": "issue",
    "updated_at": "2007-11-21T14:02:56Z",
    "url": "https://github.com/sagemath/sage/issues/1226",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
Subject says it all, ought to be easily fixed by grepping through the homspace source code.

Cheers,

Michael

Component: **doctest coverage**

_Issue created by migration from https://trac.sagemath.org/ticket/1226_





---

archive/issue_events_011860.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-20T22:59:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1226",
    "milestone_number": null,
    "milestone_title": "sage-2.8.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1226#event-11860"
}
```



---

archive/issue_events_011861.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-20T22:59:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1226",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1226#event-11861"
}
```



---

archive/issue_events_011862.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-20T22:59:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1226",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1226#event-11862"
}
```



---

archive/issue_events_011863.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-20T22:59:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1226",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1226#event-11863"
}
```



---

archive/issue_events_011864.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-20T22:59:40Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1226",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1226#event-11864"
}
```



---

archive/issue_comments_005518.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\n\n```\n[01:01] <mabshoff> I just fixed the annoying \"transposing...\" print out in cremona.spkg\n[01:03] <was__> cremona sent me a bundle or something that has a fix for that and more.\n```\nSo, I will wait for the update from John.\n\nCheers,\n\nMichael",
    "created_at": "2007-11-21T00:06:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1226",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1226#issuecomment-5518",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>


```
[01:01] <mabshoff> I just fixed the annoying "transposing..." print out in cremona.spkg
[01:03] <was__> cremona sent me a bundle or something that has a fix for that and more.
```
So, I will wait for the update from John.

Cheers,

Michael



---

archive/issue_comments_005519.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nWell, William was wrong, there isn't any update bundle. But the spkg at \n\nhttp://sage.math.washington.edu/home/mabshoff/cremona-20071116.p0.spkg\n\ncontains at least a temporary fix.\n\nCheers,\n\nMichael",
    "created_at": "2007-11-21T13:56:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1226",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1226#issuecomment-5519",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Well, William was wrong, there isn't any update bundle. But the spkg at 

http://sage.math.washington.edu/home/mabshoff/cremona-20071116.p0.spkg

contains at least a temporary fix.

Cheers,

Michael



---

archive/issue_events_011865.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-21T14:02:56Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1226",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1226#event-11865"
}
```
