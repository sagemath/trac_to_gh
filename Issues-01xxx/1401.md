# Issue 1401: [with patch, positive review that is a patch itself] deprecate A[n] for n a matrix (easy to implement usability improvement)

archive/issues_001401.json:
```json
{
    "body": "Sage currently works this way:\n\n```\nsage: a = matrix(ZZ, 2, [1..4])\nsage: a[1]\n(3, 4)\nsage: a.row(1)\n(3, 4)\nsage: a[1][0] = 5\nsage: a\n[1 2]\n[3 4]\n```\n\nInstead Sage should do this:\n\n```\nsage: a = matrix(ZZ, 2, [1..4])\nsage: a[1]\nboom!\nsage: a.row(1)\n(3, 4)\nsage: a[1][0] = 5\nboom!\n```\n\nWhere boom explains that one should use a.row(...) to get a row, or a[i,j] to get/set the ij entry.\n\nThis confuses the heck out of TONS of people!!!  (Not me, but others.)\n\nAssignee: @mwhansen\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/1401\n\n",
    "closed_at": "2007-12-15T13:32:06Z",
    "created_at": "2007-12-04T23:57:36Z",
    "labels": [
        "component: linear algebra"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.9",
    "title": "[with patch, positive review that is a patch itself] deprecate A[n] for n a matrix (easy to implement usability improvement)",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/1401",
    "user": "https://github.com/williamstein"
}
```
Sage currently works this way:

```
sage: a = matrix(ZZ, 2, [1..4])
sage: a[1]
(3, 4)
sage: a.row(1)
(3, 4)
sage: a[1][0] = 5
sage: a
[1 2]
[3 4]
```

Instead Sage should do this:

```
sage: a = matrix(ZZ, 2, [1..4])
sage: a[1]
boom!
sage: a.row(1)
(3, 4)
sage: a[1][0] = 5
boom!
```

Where boom explains that one should use a.row(...) to get a row, or a[i,j] to get/set the ij entry.

This confuses the heck out of TONS of people!!!  (Not me, but others.)

Assignee: @mwhansen

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/1401





---

archive/issue_comments_009432.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2007-12-06T21:17:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1401",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1401#issuecomment-9432",
    "user": "https://github.com/mwhansen"
}
```

Changing status from new to assigned.



---

archive/issue_comments_009433.json:
```json
{
    "body": "Changing assignee from @williamstein to @mwhansen.",
    "created_at": "2007-12-06T21:17:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1401",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1401#issuecomment-9433",
    "user": "https://github.com/mwhansen"
}
```

Changing assignee from @williamstein to @mwhansen.



---

archive/issue_comments_009434.json:
```json
{
    "body": "Attachment [1401.patch](tarball://root/attachments/some-uuid/ticket1401/1401.patch) by @mwhansen created at 2007-12-06 23:15:40",
    "created_at": "2007-12-06T23:15:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1401",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1401#issuecomment-9434",
    "user": "https://github.com/mwhansen"
}
```

Attachment [1401.patch](tarball://root/attachments/some-uuid/ticket1401/1401.patch) by @mwhansen created at 2007-12-06 23:15:40



---

archive/issue_comments_009435.json:
```json
{
    "body": "<a id='comment:2'></a>\nPatch attached.",
    "created_at": "2007-12-06T23:15:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1401",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1401#issuecomment-9435",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:2'></a>
Patch attached.



---

archive/issue_events_004798.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1401",
    "rename": {
        "from": "deprecate A[n] for n a matrix (easy to implement usability improvement)",
        "to": "[with patch] deprecate A[n] for n a matrix (easy to implement usability improvement)"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1401#event-4798"
}
```



---

archive/issue_comments_009436.json:
```json
{
    "body": "<a id='comment:3'></a>\nI just realized that a much better solution is to finish implementing immutable vectors and make the return of A[n] be an immutable row.  It accomplishes the same thing and is more usable.  So I did this.\n\nThat said, the above patch is fine -- using .row(...) all over in code is fine and faster.",
    "created_at": "2007-12-15T12:57:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1401",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1401#issuecomment-9436",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:3'></a>
I just realized that a much better solution is to finish implementing immutable vectors and make the return of A[n] be an immutable row.  It accomplishes the same thing and is more usable.  So I did this.

That said, the above patch is fine -- using .row(...) all over in code is fine and faster.



---

archive/issue_comments_009437.json:
```json
{
    "body": "Attachment [trac-1401-part2.patch](tarball://root/attachments/some-uuid/ticket1401/trac-1401-part2.patch) by @williamstein created at 2007-12-15 12:57:27",
    "created_at": "2007-12-15T12:57:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1401",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1401#issuecomment-9437",
    "user": "https://github.com/williamstein"
}
```

Attachment [trac-1401-part2.patch](tarball://root/attachments/some-uuid/ticket1401/trac-1401-part2.patch) by @williamstein created at 2007-12-15 12:57:27



---

archive/issue_events_004799.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1401",
    "rename": {
        "from": "[with patch] deprecate A[n] for n a matrix (easy to implement usability improvement)",
        "to": "[with patch, positive review that is a patch itself] deprecate A[n] for n a matrix (easy to implement usability improvement)"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1401#event-4799"
}
```



---

archive/issue_comments_009438.json:
```json
{
    "body": "<a id='comment:5'></a>\nMerged in 2.9.rc0.",
    "created_at": "2007-12-15T13:32:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1401",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1401#issuecomment-9438",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:5'></a>
Merged in 2.9.rc0.



---

archive/issue_events_004800.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-12-15T13:32:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/1401",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1401#event-4800"
}
```



---

archive/issue_comments_009439.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2007-12-15T13:32:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1401",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1401#issuecomment-9439",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
