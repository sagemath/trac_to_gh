...
```json
{
    "body": "<a id='comment:2'></a>this is *only* a documentation addition -- no code, so should be easy to referee.",
...
<a id='comment:2'></a>this is *only* a documentation addition -- no code, so should be easy to referee.
...
```json
{
    "body": "<a id='comment:3'></a>If the second hunk fails, no problem.  That's not relevant.  Only the first big\nhunk is.",
...
<a id='comment:3'></a>If the second hunk fails, no problem.  That's not relevant.  Only the first big
hunk is.
