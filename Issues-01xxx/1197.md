# Issue 1197: add cremona's modular symbols program g0n to sage and add wrapper supprot

archive/issues_001197.json:
```json
{
    "assignees": [],
    "body": "A first solid version of this is now done, due to work of Ralf PW, John Cremona, and myself. \n\nA patch bundle that applies cleanly and the corresponding spkg are both available here:\n\n    http://sage.math.washington.edu/home/was/cremona/\n\nThe wrapper code now has 100% doctest coverage, is structured correctly, and provides some useful functionality.  \n\nAssignee: **@williamstein**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1197_\n\n",
    "closed_at": "2007-11-18T08:25:34Z",
    "created_at": "2007-11-18T06:28:08Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20modular%20forms",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.13",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "add cremona's modular symbols program g0n to sage and add wrapper supprot",
    "type": "issue",
    "updated_at": "2007-11-18T08:25:34Z",
    "url": "https://github.com/sagemath/sage/issues/1197",
    "user": "https://github.com/williamstein"
}
```
A first solid version of this is now done, due to work of Ralf PW, John Cremona, and myself. 

A patch bundle that applies cleanly and the corresponding spkg are both available here:

    http://sage.math.washington.edu/home/was/cremona/

The wrapper code now has 100% doctest coverage, is structured correctly, and provides some useful functionality.  

Assignee: **@williamstein**

_Issue created by migration from https://trac.sagemath.org/ticket/1197_





---

archive/issue_events_010164.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-11-18T06:28:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1197",
    "milestone_number": null,
    "milestone_title": "sage-2.8.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1197#event-10164"
}
```



---

archive/issue_events_010165.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-11-18T06:28:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1197",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20modular%20forms",
    "label_color": "0000ff",
    "label_name": "component: modular forms",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1197#event-10165"
}
```



---

archive/issue_events_010166.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-11-18T06:28:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1197",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1197#event-10166"
}
```



---

archive/issue_events_010167.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-11-18T06:28:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1197",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1197#event-10167"
}
```



---

archive/issue_comments_005687.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,6 +1,7 @@\n A first solid version of this is now done, due to work of Ralf PW, John Cremona, and myself. \n \n-A patch bundle that applies cleanly and the corresponding spkg are both\n-available here:\n+A patch bundle that applies cleanly and the corresponding spkg are both available here:\n \n     http://sage.math.washington.edu/home/was/cremona/\n+\n+The wrapper code now has 100% doctest coverage, is structured correctly, and provides some useful functionality.  \n``````\n",
    "created_at": "2007-11-18T06:28:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1197",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1197#issuecomment-5687",
    "user": "https://github.com/williamstein"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,6 +1,7 @@
 A first solid version of this is now done, due to work of Ralf PW, John Cremona, and myself. 
 
-A patch bundle that applies cleanly and the corresponding spkg are both
-available here:
+A patch bundle that applies cleanly and the corresponding spkg are both available here:
 
     http://sage.math.washington.edu/home/was/cremona/
+
+The wrapper code now has 100% doctest coverage, is structured correctly, and provides some useful functionality.  
``````




---

archive/issue_comments_005688.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nMerged in 2.8.13.alpha0.",
    "created_at": "2007-11-18T08:25:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1197",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1197#issuecomment-5688",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:2'>Comment 2:</a>
Merged in 2.8.13.alpha0.



---

archive/issue_events_010168.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-18T08:25:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1197",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1197#event-10168"
}
```
