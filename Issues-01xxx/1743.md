# Issue 1743: implausibly old time stamp in optional package gap_packages-4.4.10_2.spkg

archive/issues_001743.json:
```json
{
    "assignees": [],
    "body": "Subject says it all, but see \n\nhttps://groups.google.com/group/sage-devel/t/c047ab8aee519bf\n\nfor the discussion. Reported initially by Kate.\n\nAssignee: **@sagetrac-mabshoff**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1743_\n\n",
    "closed_at": "2008-01-11T21:03:34Z",
    "created_at": "2008-01-10T06:48:17Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.10",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "implausibly old time stamp in optional package gap_packages-4.4.10_2.spkg",
    "type": "issue",
    "updated_at": "2008-01-11T21:03:34Z",
    "url": "https://github.com/sagemath/sage/issues/1743",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
Subject says it all, but see 

https://groups.google.com/group/sage-devel/t/c047ab8aee519bf

for the discussion. Reported initially by Kate.

Assignee: **@sagetrac-mabshoff**

_Issue created by migration from https://trac.sagemath.org/ticket/1743_





---

archive/issue_events_014979.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-01-10T06:48:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1743",
    "milestone_number": null,
    "milestone_title": "sage-2.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1743#event-14979"
}
```



---

archive/issue_events_014980.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-01-10T06:48:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1743",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1743#event-14980"
}
```



---

archive/issue_events_014981.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-01-10T06:48:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1743",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1743#event-14981"
}
```



---

archive/issue_events_014982.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-01-10T06:48:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1743",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1743#event-14982"
}
```



---

archive/issue_comments_008473.json:
```json
{
    "body": "Changed assignee from **@sagetrac-wdjoyner** to **@sagetrac-mabshoff**",
    "created_at": "2008-01-11T21:02:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1743",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1743#issuecomment-8473",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

Changed assignee from **@sagetrac-wdjoyner** to **@sagetrac-mabshoff**



---

archive/issue_comments_008474.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nFix in gap_packages-4.4.10_3.spkg - already in the optional repo:\n\n```\nsage@modular:~/www/packages/optional$ tar xf gap_packages-4.4.10_3.spkg\nsage@modular:~/www/packages/optional$ \n```\n\nCheers,\n\nMichael",
    "created_at": "2008-01-11T21:02:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1743",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1743#issuecomment-8474",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">Comment 1</div>

Fix in gap_packages-4.4.10_3.spkg - already in the optional repo:

```
sage@modular:~/www/packages/optional$ tar xf gap_packages-4.4.10_3.spkg
sage@modular:~/www/packages/optional$ 
```

Cheers,

Michael



---

archive/issue_events_014983.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-01-11T21:03:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1743",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1743#event-14983"
}
```



---

archive/issue_comments_008475.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nMerged into the optional package repo and mirrored out.",
    "created_at": "2008-01-11T21:03:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1743",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1743#issuecomment-8475",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">Comment 2</div>

Merged into the optional package repo and mirrored out.
