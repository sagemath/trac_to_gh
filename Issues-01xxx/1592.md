# Issue 1592: jmol -- misc small changes; added minimal 3d parametric plotting

archive/issues_001592.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "\n\nComponent: **graphics**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1592_\n\n",
    "closed_at": "2008-01-05T03:05:52Z",
    "created_at": "2007-12-24T07:38:40Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20graphics",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.9.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "jmol -- misc small changes; added minimal 3d parametric plotting",
    "type": "issue",
    "updated_at": "2008-01-05T03:05:52Z",
    "url": "https://github.com/sagemath/sage/issues/1592",
    "user": "https://github.com/williamstein"
}
```


Component: **graphics**

_Issue created by migration from https://trac.sagemath.org/ticket/1592_





---

archive/issue_events_015676.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-12-24T07:38:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1592",
    "milestone_number": null,
    "milestone_title": "sage-2.9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1592#event-15676"
}
```



---

archive/issue_events_015677.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-12-24T07:38:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1592",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graphics",
    "label_color": "0000ff",
    "label_name": "c: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1592#event-15677"
}
```



---

archive/issue_events_015678.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-12-24T07:38:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1592",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1592#event-15678"
}
```



---

archive/issue_events_015679.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-12-24T07:38:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1592",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1592#event-15679"
}
```



---

archive/issue_events_015680.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-12-24T07:38:40Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1592",
    "subject": "https://github.com/williamstein",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1592#event-15680"
}
```



---

archive/issue_comments_007359.json:
```json
{
    "body": "Attachment: **[3d-3.hg.gz](https://github.com/sagemath/sage/files/ticket1592/3d-3.hg.gz)**",
    "created_at": "2007-12-24T07:39:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1592",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1592#issuecomment-7359",
    "user": "https://github.com/williamstein"
}
```

Attachment: **[3d-3.hg.gz](https://github.com/sagemath/sage/files/ticket1592/3d-3.hg.gz)**



---

archive/issue_events_015681.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-29T17:29:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1592",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1592#event-15681"
}
```



---

archive/issue_comments_007360.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in 2.9.2.rc1 via some of the other jmol/3D plotting bundles.",
    "created_at": "2008-01-05T03:05:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1592",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1592#issuecomment-7360",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in 2.9.2.rc1 via some of the other jmol/3D plotting bundles.



---

archive/issue_events_015682.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-01-05T03:05:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1592",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1592#event-15682"
}
```



---

archive/issue_events_015683.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-01-05T03:05:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1592",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1592#event-15683"
}
```
