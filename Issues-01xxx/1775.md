# Issue 1775: clicking on 'search worksheets' can log you out.

archive/issues_001775.json:
```json
{
    "assignees": [
        "https://github.com/boothby"
    ],
    "body": "I have just succeeded in creating a personal id on my sage server (as opposed to admin).\nWhen I logged in I wondered what 'search worksheets' might do so I clicked on it\nand found I had been logged out! An error message which left me logged in would be\nmuch better.\n\nComponent: **notebook**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1775_\n\n",
    "closed_at": "2009-01-23T12:43:32Z",
    "created_at": "2008-01-14T10:44:00Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "clicking on 'search worksheets' can log you out.",
    "type": "issue",
    "updated_at": "2009-01-23T12:43:32Z",
    "url": "https://github.com/sagemath/sage/issues/1775",
    "user": "https://github.com/sagetrac-bill-p"
}
```
I have just succeeded in creating a personal id on my sage server (as opposed to admin).
When I logged in I wondered what 'search worksheets' might do so I clicked on it
and found I had been logged out! An error message which left me logged in would be
much better.

Component: **notebook**

_Issue created by migration from https://trac.sagemath.org/ticket/1775_





---

archive/issue_events_017574.json:
```json
{
    "actor": "https://github.com/sagetrac-bill-p",
    "created_at": "2008-01-14T10:44:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1775",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1775#event-17574"
}
```



---

archive/issue_events_017575.json:
```json
{
    "actor": "https://github.com/sagetrac-bill-p",
    "created_at": "2008-01-14T10:44:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1775",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1775#event-17575"
}
```



---

archive/issue_events_017576.json:
```json
{
    "actor": "https://github.com/sagetrac-bill-p",
    "created_at": "2008-01-14T10:44:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1775",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1775#event-17576"
}
```



---

archive/issue_events_017577.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-01-14T10:44:00Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1775",
    "subject": "https://github.com/sagetrac-bill-p",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1775#event-17577"
}
```



---

archive/issue_events_017578.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-01-14T12:48:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1775",
    "milestone_number": null,
    "milestone_title": "sage-2.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1775#event-17578"
}
```



---

archive/issue_comments_008152.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nI can not replicate this.  You might have been confused about whether or not you were actually logged in or something?  Please give clear step-by-step directions that allow you to replicate tis problem every time.",
    "created_at": "2008-01-14T14:51:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1775",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1775#issuecomment-8152",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:2" align="right">comment:2</div>

I can not replicate this.  You might have been confused about whether or not you were actually logged in or something?  Please give clear step-by-step directions that allow you to replicate tis problem every time.



---

archive/issue_comments_008153.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nWilliam, I have just created a new user (charlie) via the 'Create new user' on\nthe login page method. Logged in as Charlie, then clicked on 'Search worksheets'\nand it logged me out:\n\n---\nLogin failure\nYou have entered an invalid username. Please try again.\n\nValid login names:\nadmin,\ncharlie,\nbill \n\n---",
    "created_at": "2008-01-14T15:17:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1775",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1775#issuecomment-8153",
    "user": "https://github.com/sagetrac-bill-p"
}
```

<div id="comment:3" align="right">comment:3</div>

William, I have just created a new user (charlie) via the 'Create new user' on
the login page method. Logged in as Charlie, then clicked on 'Search worksheets'
and it logged me out:

---
Login failure
You have entered an invalid username. Please try again.

Valid login names:
admin,
charlie,
bill 

---



---

archive/issue_comments_008154.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nOK, I can definitely replicate this bug now, so we should easily be able to fix it.",
    "created_at": "2008-01-14T15:46:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1775",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1775#issuecomment-8154",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:4" align="right">comment:4</div>

OK, I can definitely replicate this bug now, so we should easily be able to fix it.



---

archive/issue_comments_008155.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nI can't reproduce this at https://sage.math.washington.edu:8102",
    "created_at": "2008-08-27T16:55:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1775",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1775#issuecomment-8155",
    "user": "https://github.com/TimothyClemans"
}
```

<div id="comment:5" align="right">comment:5</div>

I can't reproduce this at https://sage.math.washington.edu:8102



---

archive/issue_comments_008156.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nI cannot duplicate this in Sage 3.2.3 either.  I'm going to mark it as invalid at this point.",
    "created_at": "2009-01-23T12:43:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1775",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1775#issuecomment-8156",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:6" align="right">comment:6</div>

I cannot duplicate this in Sage 3.2.3 either.  I'm going to mark it as invalid at this point.



---

archive/issue_events_017579.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-23T12:43:32Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/1775",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1775#event-17579"
}
```



---

archive/issue_events_017580.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-23T12:43:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1775",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1775#event-17580"
}
```



---

archive/issue_events_017581.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-23T12:43:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1775",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1775#event-17581"
}
```



---

archive/issue_events_017582.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-23T12:43:32Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1775",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1775#event-17582"
}
```
