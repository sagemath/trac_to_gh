# Issue 1514: fix breakage and lameness in foo? and foo?? especially in the notebook.

archive/issues_001514.json:
```json
{
    "assignees": [
        "https://github.com/boothby"
    ],
    "body": "I'm sick of foo? or foo?? failing with tracebacks.  They should never do that.\n\nComponent: **notebook**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1514_\n\n",
    "closed_at": "2007-12-15T07:21:37Z",
    "created_at": "2007-12-15T00:17:52Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix breakage and lameness in foo? and foo?? especially in the notebook.",
    "type": "issue",
    "updated_at": "2007-12-15T07:21:37Z",
    "url": "https://github.com/sagemath/sage/issues/1514",
    "user": "https://github.com/williamstein"
}
```
I'm sick of foo? or foo?? failing with tracebacks.  They should never do that.

Component: **notebook**

_Issue created by migration from https://trac.sagemath.org/ticket/1514_





---

archive/issue_events_014934.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-12-15T00:17:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1514",
    "milestone_number": null,
    "milestone_title": "sage-2.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1514#event-14934"
}
```



---

archive/issue_events_014935.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-12-15T00:17:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1514",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1514#event-14935"
}
```



---

archive/issue_events_014936.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-12-15T00:17:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1514",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1514#event-14936"
}
```



---

archive/issue_events_014937.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-12-15T00:17:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1514",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1514#event-14937"
}
```



---

archive/issue_events_014938.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2007-12-15T00:17:52Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1514",
    "subject": "https://github.com/williamstein",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1514#event-14938"
}
```



---

archive/issue_comments_007073.json:
```json
{
    "body": "Attachment: **[trac-1514.patch.gz](https://github.com/sagemath/sage/files/ticket1514/trac-1514.patch.gz)**",
    "created_at": "2007-12-15T01:45:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1514",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1514#issuecomment-7073",
    "user": "https://github.com/williamstein"
}
```

Attachment: **[trac-1514.patch.gz](https://github.com/sagemath/sage/files/ticket1514/trac-1514.patch.gz)**



---

archive/issue_events_014939.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-12-15T05:28:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1514",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1514#event-14939"
}
```



---

archive/issue_events_014940.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-12-15T05:28:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1514",
    "label": "https://github.com/sagemath/sage/labels/p%3A%201%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p: 1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1514#event-14940"
}
```



---

archive/issue_comments_007074.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\n\n```\n[9:43pm] cwitty-rvw-1514: wstein-rvw-1119, it looks like #1514 does not have any doctests for whatever bugs you are fixing?\n[9:43pm] craigcitro: gmp comes before pari in the build order for libcsage\n[9:44pm] wstein-rvw-1119: cwitty-1514 -- the buginess is that nothing works at all.\n[9:44pm] wstein-rvw-1119: it's hard to have a doctest for that.\n[9:44pm] wstein-rvw-1119: However, notice the first line of the patch, which turns *on* doctesting for the sageinspect.py file\n[9:44pm] wstein-rvw-1119: So there are many new doctests as a result.\n[9:45pm] wstein-rvw-1119: It's really a design change anyway -- to use the files in SAGE_ROOT/devel/sage/sage instead of SAGE_ROOT/local/lib/python/site-packages/sage/,\n[9:45pm] wstein-rvw-1119: since for some reason often some .pyx files or other files that are relevant don't get copied over there.\n[9:45pm] wstein-rvw-1119: But SAGE_ROOT/devel/sage/sage does.\n```",
    "created_at": "2007-12-15T05:47:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1514",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1514#issuecomment-7074",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:2" align="right">comment:2</div>


```
[9:43pm] cwitty-rvw-1514: wstein-rvw-1119, it looks like #1514 does not have any doctests for whatever bugs you are fixing?
[9:43pm] craigcitro: gmp comes before pari in the build order for libcsage
[9:44pm] wstein-rvw-1119: cwitty-1514 -- the buginess is that nothing works at all.
[9:44pm] wstein-rvw-1119: it's hard to have a doctest for that.
[9:44pm] wstein-rvw-1119: However, notice the first line of the patch, which turns *on* doctesting for the sageinspect.py file
[9:44pm] wstein-rvw-1119: So there are many new doctests as a result.
[9:45pm] wstein-rvw-1119: It's really a design change anyway -- to use the files in SAGE_ROOT/devel/sage/sage instead of SAGE_ROOT/local/lib/python/site-packages/sage/,
[9:45pm] wstein-rvw-1119: since for some reason often some .pyx files or other files that are relevant don't get copied over there.
[9:45pm] wstein-rvw-1119: But SAGE_ROOT/devel/sage/sage does.
```



---

archive/issue_comments_007075.json:
```json
{
    "body": "Attachment: **[trac-1514-fixdoctest.patch.gz](https://github.com/sagemath/sage/files/ticket1514/trac-1514-fixdoctest.patch.gz)**",
    "created_at": "2007-12-15T06:07:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1514",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1514#issuecomment-7075",
    "user": "https://github.com/sagetrac-cwitty"
}
```

Attachment: **[trac-1514-fixdoctest.patch.gz](https://github.com/sagemath/sage/files/ticket1514/trac-1514-fixdoctest.patch.gz)**



---

archive/issue_events_014941.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2007-12-15T06:08:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1514",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1514#event-14941"
}
```



---

archive/issue_comments_007076.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nLooks good to me; fixes doctests.\n\nApply both patches.",
    "created_at": "2007-12-15T06:08:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1514",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1514#issuecomment-7076",
    "user": "https://github.com/sagetrac-cwitty"
}
```

<div id="comment:3" align="right">comment:3</div>

Looks good to me; fixes doctests.

Apply both patches.



---

archive/issue_events_014942.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-15T07:21:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1514",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1514#event-14942"
}
```



---

archive/issue_events_014943.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-15T07:21:37Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1514",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1514#event-14943"
}
```



---

archive/issue_comments_007077.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nMerged in 2.9.rc0.",
    "created_at": "2007-12-15T07:21:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1514",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1514#issuecomment-7077",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Merged in 2.9.rc0.
