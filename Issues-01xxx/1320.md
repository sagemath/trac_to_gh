# Issue 1320: planarity testing

archive/issues_001320.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-ekirkman"
    ],
    "body": "From Chris Godsil's wishlist.\n\n```\n>>> Someone is eventually going to ask for a routine to test for planarity. I\n>>> believe that there are good ones in existence, but it's going to be\n>>> hard to get\n>>> a good one with an open source licence.\n>> The nauty README has this to say about the new planarity testing feature:\n>> \"New program planarg to test for planarity and find planar embeddings:\n>> planarg -help for details. The planarity code was written by Paulette\n>> Lieby for the Magma project and used with permission.\"\n>>\n>> Does anyone know Paulette Lieby? Can we ask about releasing the code\n>> under GPL? It looks like the source has now been released as a part of\n>> nauty.\n> Emily Kirkman understands a linear time algorithm for testing for\n> planarity. There is one in BOOST, which is GPL, and has been nominated\n> for inclusion in Sage several times.\n```\n\n\nCC:  @sagetrac-bober\n\nComponent: **graph theory**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1320_\n\n",
    "closed_at": "2008-02-28T06:08:32Z",
    "created_at": "2007-11-28T20:08:11Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.10.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "planarity testing",
    "type": "issue",
    "updated_at": "2008-02-28T06:08:32Z",
    "url": "https://github.com/sagemath/sage/issues/1320",
    "user": "https://github.com/jasongrout"
}
```
From Chris Godsil's wishlist.

```
>>> Someone is eventually going to ask for a routine to test for planarity. I
>>> believe that there are good ones in existence, but it's going to be
>>> hard to get
>>> a good one with an open source licence.
>> The nauty README has this to say about the new planarity testing feature:
>> "New program planarg to test for planarity and find planar embeddings:
>> planarg -help for details. The planarity code was written by Paulette
>> Lieby for the Magma project and used with permission."
>>
>> Does anyone know Paulette Lieby? Can we ask about releasing the code
>> under GPL? It looks like the source has now been released as a part of
>> nauty.
> Emily Kirkman understands a linear time algorithm for testing for
> planarity. There is one in BOOST, which is GPL, and has been nominated
> for inclusion in Sage several times.
```


CC:  @sagetrac-bober

Component: **graph theory**

_Issue created by migration from https://trac.sagemath.org/ticket/1320_





---

archive/issue_events_012906.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2007-11-28T20:08:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1320",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "c: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1320#event-12906"
}
```



---

archive/issue_events_012907.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2007-11-28T20:08:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1320",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1320#event-12907"
}
```



---

archive/issue_events_012908.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2007-11-28T20:08:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1320",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1320#event-12908"
}
```



---

archive/issue_events_012909.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2007-11-28T20:08:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1320",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1320#event-12909"
}
```



---

archive/issue_events_012910.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2007-11-28T20:08:11Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1320",
    "subject": "https://github.com/jasongrout",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1320#event-12910"
}
```



---

archive/issue_events_012911.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2007-11-29T16:25:48Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/1320",
    "title_is": "graphs: planarity testing",
    "title_was": "[graphs] planarity testing",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1320#event-12911"
}
```



---

archive/issue_comments_006095.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nI plan to begin implementing the Boyer-Myrvold linear time planar test/embedding algorithm right after autumn quarter finals.  (Dec 13th).  It should be available in early January.",
    "created_at": "2007-11-30T03:56:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1320#issuecomment-6095",
    "user": "https://github.com/sagetrac-ekirkman"
}
```

<div id="comment:2" align="right">comment:2</div>

I plan to begin implementing the Boyer-Myrvold linear time planar test/embedding algorithm right after autumn quarter finals.  (Dec 13th).  It should be available in early January.



---

archive/issue_events_012912.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2007-11-30T03:56:03Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/1320",
    "subject": "https://github.com/sagetrac-ekirkman",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1320#event-12912"
}
```



---

archive/issue_events_012913.json:
```json
{
    "actor": "https://github.com/sagetrac-ekirkman",
    "created_at": "2007-11-30T03:56:03Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1320",
    "subject": "https://github.com/sagetrac-ekirkman",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1320#event-12913"
}
```



---

archive/issue_comments_006096.json:
```json
{
    "body": "Changed keywords from **graphs** to none",
    "created_at": "2007-12-17T15:22:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1320#issuecomment-6096",
    "user": "https://github.com/rlmill"
}
```

Changed keywords from **graphs** to none



---

archive/issue_events_012914.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2007-12-17T15:22:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1320",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "c: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1320#event-12914"
}
```



---

archive/issue_events_012915.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2007-12-17T15:22:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1320",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "c: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1320#event-12915"
}
```



---

archive/issue_comments_006097.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nAttachment: **[planarity.hg.gz](https://github.com/sagemath/sage/files/ticket1320/planarity.hg.gz)**",
    "created_at": "2008-02-28T02:27:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1320#issuecomment-6097",
    "user": "https://github.com/rlmill"
}
```

<div id="comment:5" align="right">comment:5</div>

Attachment: **[planarity.hg.gz](https://github.com/sagemath/sage/files/ticket1320/planarity.hg.gz)**



---

archive/issue_events_012916.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-02-28T02:27:22Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/1320",
    "title_is": "planarity testing",
    "title_was": "graphs: planarity testing",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1320#event-12916"
}
```



---

archive/issue_events_012917.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-02-28T02:27:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1320",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1320#event-12917"
}
```



---

archive/issue_events_012918.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-02-28T02:27:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1320",
    "milestone_number": null,
    "milestone_title": "sage-2.10.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1320#event-12918"
}
```



---

archive/issue_events_012919.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-02-28T02:27:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1320",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1320#event-12919"
}
```



---

archive/issue_comments_006098.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nHi, I had a single, easy to fix merge conflict:\n\n```\n<<<<<<< /scratch/mabshoff/release-cycle/sage-2.10.3.rc0/devel/sage-main/sage/graphs/graph.py.orig.1734827483\nfrom sage.graphs.graph_coloring import chromatic_number, chromatic_polynomial\n||||||| /tmp/graph.py~base.vsk2R5\n=======\nfrom sage.rings.rational import Rational\n```\nThe above was caused by the work on the chromatic number and chromatic polynomial by Tom.\n\nCheers,\n\nMichael",
    "created_at": "2008-02-28T06:03:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1320#issuecomment-6098",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:7" align="right">comment:7</div>

Hi, I had a single, easy to fix merge conflict:

```
<<<<<<< /scratch/mabshoff/release-cycle/sage-2.10.3.rc0/devel/sage-main/sage/graphs/graph.py.orig.1734827483
from sage.graphs.graph_coloring import chromatic_number, chromatic_polynomial
||||||| /tmp/graph.py~base.vsk2R5
=======
from sage.rings.rational import Rational
```
The above was caused by the work on the chromatic number and chromatic polynomial by Tom.

Cheers,

Michael



---

archive/issue_comments_006099.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nMerged in Sage 2.10.3.rc0",
    "created_at": "2008-02-28T06:08:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1320#issuecomment-6099",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:8" align="right">comment:8</div>

Merged in Sage 2.10.3.rc0



---

archive/issue_events_012920.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-02-28T06:08:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1320",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1320#event-12920"
}
```



---

archive/issue_events_012921.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-02-28T06:08:32Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1320",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1320#event-12921"
}
```
