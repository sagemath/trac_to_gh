# Issue 1716: add function log1p in mpfr interface

archive/issues_001716.json:
```json
{
    "assignees": [],
    "body": "I guess other functions implemented in mpfr-2.3.0 are not available from SAGE.\nIf this patch is accepted, I will propose a more complete one with all missing functions.\n\n\nComponent: **basic arithmetic**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1716_\n\n",
    "closed_at": "2008-01-15T01:32:10Z",
    "created_at": "2008-01-07T21:26:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.10",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "add function log1p in mpfr interface",
    "type": "issue",
    "updated_at": "2008-01-15T01:32:10Z",
    "url": "https://github.com/sagemath/sage/issues/1716",
    "user": "https://github.com/zimmermann6"
}
```
I guess other functions implemented in mpfr-2.3.0 are not available from SAGE.
If this patch is accepted, I will propose a more complete one with all missing functions.


Component: **basic arithmetic**

_Issue created by migration from https://trac.sagemath.org/ticket/1716_





---

archive/issue_events_016920.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2008-01-07T21:26:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1716",
    "milestone_number": null,
    "milestone_title": "sage-2.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1716#event-16920"
}
```



---

archive/issue_events_016921.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2008-01-07T21:26:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1716",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "c: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1716#event-16921"
}
```



---

archive/issue_events_016922.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2008-01-07T21:26:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1716",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1716#event-16922"
}
```



---

archive/issue_events_016923.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2008-01-07T21:26:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1716",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1716#event-16923"
}
```



---

archive/issue_events_016924.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-01-15T00:43:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1716",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1716#event-16924"
}
```



---

archive/issue_comments_007888.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[7881.patch.gz](https://github.com/sagemath/sage/files/ticket1716/7881.patch.gz)**\n\nWorks for me.  I think it would be good to add the other functions as well.",
    "created_at": "2008-01-15T00:43:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1716",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1716#issuecomment-7888",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[7881.patch.gz](https://github.com/sagemath/sage/files/ticket1716/7881.patch.gz)**

Works for me.  I think it would be good to add the other functions as well.



---

archive/issue_comments_007889.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nMerged in Sage 2.10.alpha3.",
    "created_at": "2008-01-15T01:32:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1716",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1716#issuecomment-7889",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Merged in Sage 2.10.alpha3.



---

archive/issue_events_016925.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-01-15T01:32:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1716",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1716#event-16925"
}
```



---

archive/issue_events_016926.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-01-15T01:32:10Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1716",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1716#event-16926"
}
```
