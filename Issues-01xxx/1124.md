# Issue 1124: ModularSymbol.complement crashes on full subspaces

archive/issues_001124.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "Here is a bug in modular symbols code:\n\n``` \nsage: M=ModularSymbols(11,2,1)\nsage: M.complement()\n---------------------------------------------------------------------------\n<type 'exceptions.AttributeError'>        Traceback (most recent call last)\n\n/net/mathserv/1/home/syazdani/research/programs/<ipython console> in <module>()\n\n/home/syazdani/sage/local/lib/python2.5/site-packages/sage/modular/hecke/ambient_module.py in complement(self)\n     96         Return the largest Hecke-stable complement of this space.\n     97         \"\"\"\n---> 98         return self.zero_subspace()\n     99\n    100     def decomposition_matrix(self):\n\n<type 'exceptions.AttributeError'>: 'ModularSymbolsAmbient_wt2_g0' object has no attribute 'zero_subspace'\n```\n\nThe problem is that zero_subspace is not implemented. Although zero_submodule is.\nOne possible fix is to change self.zero_subspace to self.zero_submodule(). That's the included patch.\n\nKeywords: **zero_subspace**\n\nComponent: **modular forms**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1124_\n\n",
    "closed_at": "2007-11-18T14:22:25Z",
    "created_at": "2007-11-07T18:07:46Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20modular%20forms",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.13",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "ModularSymbol.complement crashes on full subspaces",
    "type": "issue",
    "updated_at": "2007-11-18T14:22:25Z",
    "url": "https://github.com/sagemath/sage/issues/1124",
    "user": "https://github.com/syazdani77"
}
```
Here is a bug in modular symbols code:

``` 
sage: M=ModularSymbols(11,2,1)
sage: M.complement()
---------------------------------------------------------------------------
<type 'exceptions.AttributeError'>        Traceback (most recent call last)

/net/mathserv/1/home/syazdani/research/programs/<ipython console> in <module>()

/home/syazdani/sage/local/lib/python2.5/site-packages/sage/modular/hecke/ambient_module.py in complement(self)
     96         Return the largest Hecke-stable complement of this space.
     97         """
---> 98         return self.zero_subspace()
     99
    100     def decomposition_matrix(self):

<type 'exceptions.AttributeError'>: 'ModularSymbolsAmbient_wt2_g0' object has no attribute 'zero_subspace'
```

The problem is that zero_subspace is not implemented. Although zero_submodule is.
One possible fix is to change self.zero_subspace to self.zero_submodule(). That's the included patch.

Keywords: **zero_subspace**

Component: **modular forms**

_Issue created by migration from https://trac.sagemath.org/ticket/1124_





---

archive/issue_events_010779.json:
```json
{
    "actor": "https://github.com/syazdani77",
    "created_at": "2007-11-07T18:07:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1124",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20modular%20forms",
    "label_color": "0000ff",
    "label_name": "c: modular forms",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1124#event-10779"
}
```



---

archive/issue_events_010780.json:
```json
{
    "actor": "https://github.com/syazdani77",
    "created_at": "2007-11-07T18:07:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1124",
    "label": "https://github.com/sagemath/sage/labels/p%3A%205%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p: 5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1124#event-10780"
}
```



---

archive/issue_events_010781.json:
```json
{
    "actor": "https://github.com/syazdani77",
    "created_at": "2007-11-07T18:07:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1124",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1124#event-10781"
}
```



---

archive/issue_events_010782.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-11-07T18:07:46Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1124",
    "subject": "https://github.com/syazdani77",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1124#event-10782"
}
```



---

archive/issue_comments_004852.json:
```json
{
    "body": "trivial patch",
    "created_at": "2007-11-07T18:08:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1124#issuecomment-4852",
    "user": "https://github.com/syazdani77"
}
```

trivial patch



---

archive/issue_comments_004853.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[zero_submodule.gz](https://github.com/sagemath/sage/files/ticket1124/zero_submodule.gz)**\n\n[with patch]",
    "created_at": "2007-11-07T18:15:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1124#issuecomment-4853",
    "user": "https://github.com/syazdani77"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[zero_submodule.gz](https://github.com/sagemath/sage/files/ticket1124/zero_submodule.gz)**

[with patch]



---

archive/issue_events_010783.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-07T18:18:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1124",
    "milestone_number": null,
    "milestone_title": "sage-2.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1124#event-10783"
}
```



---

archive/issue_events_010784.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-07T18:19:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1124",
    "label": "https://github.com/sagemath/sage/labels/p%3A%205%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p: 5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1124#event-10784"
}
```



---

archive/issue_events_010785.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-07T18:19:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1124",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1124#event-10785"
}
```



---

archive/issue_comments_004854.json:
```json
{
    "body": "Added doctest for complement. Both patches should be applied.",
    "created_at": "2007-11-07T18:25:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1124#issuecomment-4854",
    "user": "https://github.com/syazdani77"
}
```

Added doctest for complement. Both patches should be applied.



---

archive/issue_comments_004855.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nAttachment: **[doctest.gz](https://github.com/sagemath/sage/files/ticket1124/doctest.gz)**",
    "created_at": "2007-11-16T11:56:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1124#issuecomment-4855",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Attachment: **[doctest.gz](https://github.com/sagemath/sage/files/ticket1124/doctest.gz)**



---

archive/issue_events_010786.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-16T11:56:26Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/1124",
    "milestone_number": null,
    "milestone_title": "sage-2.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1124#event-10786"
}
```



---

archive/issue_events_010787.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-16T11:56:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1124",
    "milestone_number": null,
    "milestone_title": "sage-2.8.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1124#event-10787"
}
```



---

archive/issue_comments_004856.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\n** \"I have reviewed this patch and it agree with it.\"",
    "created_at": "2007-11-18T03:55:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1124#issuecomment-4856",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:5" align="right">comment:5</div>

** "I have reviewed this patch and it agree with it."



---

archive/issue_events_010788.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-18T14:22:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1124",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1124#event-10788"
}
```



---

archive/issue_comments_004857.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nMerged in 2.8.13.alpha0.",
    "created_at": "2007-11-18T14:22:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1124",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1124#issuecomment-4857",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:6" align="right">comment:6</div>

Merged in 2.8.13.alpha0.
