# Issue 1079: DSage improper get_worker_count

archive/issues_001079.json:
```json
{
    "assignees": [
        "https://github.com/yqiang"
    ],
    "body": "When I do D.get_worker_count(), it always tells me that I have 2 workers--even though I have 30 machines connected each with 2 workers so the answer should be 30*2 = 60.  It works OK if I have only one DSage login with, say, 12 workers.\n\n\nComponent: **packages: standard**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1079_\n\n",
    "closed_at": "2007-11-06T21:59:37Z",
    "created_at": "2007-11-03T17:11:50Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.12",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "DSage improper get_worker_count",
    "type": "issue",
    "updated_at": "2007-11-06T21:59:37Z",
    "url": "https://github.com/sagemath/sage/issues/1079",
    "user": "https://github.com/jvoight"
}
```
When I do D.get_worker_count(), it always tells me that I have 2 workers--even though I have 30 machines connected each with 2 workers so the answer should be 30*2 = 60.  It works OK if I have only one DSage login with, say, 12 workers.


Component: **packages: standard**

_Issue created by migration from https://trac.sagemath.org/ticket/1079_





---

archive/issue_events_010365.json:
```json
{
    "actor": "https://github.com/jvoight",
    "created_at": "2007-11-03T17:11:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1079",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1079#event-10365"
}
```



---

archive/issue_events_010366.json:
```json
{
    "actor": "https://github.com/jvoight",
    "created_at": "2007-11-03T17:11:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1079",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1079#event-10366"
}
```



---

archive/issue_events_010367.json:
```json
{
    "actor": "https://github.com/jvoight",
    "created_at": "2007-11-03T17:11:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1079",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1079#event-10367"
}
```



---

archive/issue_events_010368.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-11-03T17:11:50Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1079",
    "subject": "https://github.com/jvoight",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1079#event-10368"
}
```



---

archive/issue_events_010369.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-11-03T17:48:53Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/1079",
    "subject": "https://github.com/yqiang",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1079#event-10369"
}
```



---

archive/issue_events_010370.json:
```json
{
    "actor": "https://github.com/yqiang",
    "created_at": "2007-11-03T17:48:53Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1079",
    "subject": "https://github.com/yqiang",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1079#event-10370"
}
```



---

archive/issue_events_010371.json:
```json
{
    "actor": "https://github.com/yqiang",
    "created_at": "2007-11-03T20:23:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1079",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1079#event-10371"
}
```



---

archive/issue_comments_004664.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nFixed.  Get bundle here:\nhttp://sage.math.washington.edu/home/yqiang/dsage.hg",
    "created_at": "2007-11-03T20:23:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1079",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1079#issuecomment-4664",
    "user": "https://github.com/yqiang"
}
```

<div id="comment:2" align="right">comment:2</div>

Fixed.  Get bundle here:
http://sage.math.washington.edu/home/yqiang/dsage.hg



---

archive/issue_comments_004665.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nReopening",
    "created_at": "2007-11-03T20:28:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1079",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1079#issuecomment-4665",
    "user": "https://github.com/yqiang"
}
```

<div id="comment:3" align="right">comment:3</div>

Reopening



---

archive/issue_events_010372.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-03T23:49:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1079",
    "milestone_number": null,
    "milestone_title": "sage-2.8.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1079#event-10372"
}
```



---

archive/issue_comments_004666.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\napplied to 2.8.12.rc0",
    "created_at": "2007-11-06T21:59:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1079",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1079#issuecomment-4666",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">comment:5</div>

applied to 2.8.12.rc0



---

archive/issue_events_010373.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-06T21:59:37Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1079",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1079#event-10373"
}
```
