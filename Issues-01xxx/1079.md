# Issue 1079: DSage improper get_worker_count

archive/issues_001079.json:
```json
{
    "assignees": [],
    "body": "When I do D.get_worker_count(), it always tells me that I have 2 workers--even though I have 30 machines connected each with 2 workers so the answer should be 30*2 = 60.  It works OK if I have only one DSage login with, say, 12 workers.\n\n\nAssignee: **@yqiang**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1079_\n\n",
    "closed_at": "2007-11-06T21:59:37Z",
    "created_at": "2007-11-03T17:11:50Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.12",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "DSage improper get_worker_count",
    "type": "issue",
    "updated_at": "2007-11-06T21:59:37Z",
    "url": "https://github.com/sagemath/sage/issues/1079",
    "user": "https://github.com/jvoight"
}
```
When I do D.get_worker_count(), it always tells me that I have 2 workers--even though I have 30 machines connected each with 2 workers so the answer should be 30*2 = 60.  It works OK if I have only one DSage login with, say, 12 workers.


Assignee: **@yqiang**

_Issue created by migration from https://trac.sagemath.org/ticket/1079_





---

archive/issue_events_009128.json:
```json
{
    "actor": "https://github.com/jvoight",
    "created_at": "2007-11-03T17:11:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1079",
    "milestone_number": null,
    "milestone_title": "sage-2.8.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1079#event-9128"
}
```



---

archive/issue_events_009129.json:
```json
{
    "actor": "https://github.com/jvoight",
    "created_at": "2007-11-03T17:11:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1079",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1079#event-9129"
}
```



---

archive/issue_events_009130.json:
```json
{
    "actor": "https://github.com/jvoight",
    "created_at": "2007-11-03T17:11:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1079",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1079#event-9130"
}
```



---

archive/issue_events_009131.json:
```json
{
    "actor": "https://github.com/jvoight",
    "created_at": "2007-11-03T17:11:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1079",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1079#event-9131"
}
```



---

archive/issue_comments_004949.json:
```json
{
    "body": "Changed assignee from **@williamstein** to **@yqiang**",
    "created_at": "2007-11-03T17:48:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1079",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1079#issuecomment-4949",
    "user": "https://github.com/yqiang"
}
```

Changed assignee from **@williamstein** to **@yqiang**



---

archive/issue_events_009132.json:
```json
{
    "actor": "https://github.com/yqiang",
    "created_at": "2007-11-03T20:23:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1079",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1079#event-9132"
}
```



---

archive/issue_comments_004950.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nFixed.  Get bundle here:\nhttp://sage.math.washington.edu/home/yqiang/dsage.hg",
    "created_at": "2007-11-03T20:23:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1079",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1079#issuecomment-4950",
    "user": "https://github.com/yqiang"
}
```

<a id='comment:2'>Comment 2:</a>
Fixed.  Get bundle here:
http://sage.math.washington.edu/home/yqiang/dsage.hg



---

archive/issue_comments_004951.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nReopening",
    "created_at": "2007-11-03T20:28:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1079",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1079#issuecomment-4951",
    "user": "https://github.com/yqiang"
}
```

<a id='comment:3'>Comment 3:</a>
Reopening



---

archive/issue_events_009133.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-03T23:49:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1079",
    "milestone_number": null,
    "milestone_title": "sage-2.8.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1079#event-9133"
}
```



---

archive/issue_comments_004952.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\napplied to 2.8.12.rc0",
    "created_at": "2007-11-06T21:59:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1079",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1079#issuecomment-4952",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:5'>Comment 5:</a>
applied to 2.8.12.rc0



---

archive/issue_events_009134.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-06T21:59:37Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1079",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1079#event-9134"
}
```
