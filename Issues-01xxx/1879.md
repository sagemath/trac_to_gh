# Issue 1879: [with patch, positive review] notebook -- registering redirects to annoying url

archive/issues_001879.json:
```json
{
    "body": "\n```\nOn Jan 21, 2008 7:24 AM, Martin Albrecht <> wrote:\n>\n> > I also made it so the notebook doesn't require a funny port, so it should\n> > work fine if you're behind some sort of firewall  that doesn't allow\n> > connections to ports.   Finally, I  reduced the number of security\n> > warnings.\n>\n> I am behind such a funny firewall and it doesn't work for me. I don't have an\n> account on this particular NB server yet and registering times out because it\n> redirects to http:sage.math.washington.edu:8101/register. This is where the\n> firewall won't let me connect.\n\nThat's annoying.  I wonder why that happens.  In any case, if you register you\nonly get sent to 8101 *after* you register -- your registration should still go\nthrough fine.  You can then login by manually going to sagenb.org (or going\nback with the browser back button).\n\n```\n\nAssignee: boothby\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/1879\n\n",
    "closed_at": "2008-03-17T04:35:14Z",
    "created_at": "2008-01-21T18:36:29Z",
    "labels": [
        "component: notebook",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.10.4",
    "title": "[with patch, positive review] notebook -- registering redirects to annoying url",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/1879",
    "user": "https://github.com/williamstein"
}
```

```
On Jan 21, 2008 7:24 AM, Martin Albrecht <> wrote:
>
> > I also made it so the notebook doesn't require a funny port, so it should
> > work fine if you're behind some sort of firewall  that doesn't allow
> > connections to ports.   Finally, I  reduced the number of security
> > warnings.
>
> I am behind such a funny firewall and it doesn't work for me. I don't have an
> account on this particular NB server yet and registering times out because it
> redirects to http:sage.math.washington.edu:8101/register. This is where the
> firewall won't let me connect.

That's annoying.  I wonder why that happens.  In any case, if you register you
only get sent to 8101 *after* you register -- your registration should still go
through fine.  You can then login by manually going to sagenb.org (or going
back with the browser back button).

```

Assignee: boothby

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/1879





---

archive/issue_events_006359.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/boothby",
    "created_at": "2008-03-16T19:29:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/1879",
    "milestone": "sage-2.10.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1879#event-6359"
}
```



---

archive/issue_events_006360.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1879",
    "rename": {
        "from": "notebook -- registering redirects to annoying url",
        "to": "[with patch, needs review] notebook -- registering redirects to annoying url"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1879#event-6360"
}
```



---

archive/issue_comments_012384.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [1879-relativelinks.patch](tarball://root/attachments/some-uuid/ticket1879/1879-relativelinks.patch) by boothby created at 2008-03-16 19:29:30",
    "created_at": "2008-03-16T19:29:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1879#issuecomment-12384",
    "user": "https://trac.sagemath.org/admin/accounts/users/boothby"
}
```

<a id='comment:1'></a>
Attachment [1879-relativelinks.patch](tarball://root/attachments/some-uuid/ticket1879/1879-relativelinks.patch) by boothby created at 2008-03-16 19:29:30



---

archive/issue_events_006361.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1879",
    "rename": {
        "from": "[with patch, needs review] notebook -- registering redirects to annoying url",
        "to": "[with patch, positive review] notebook -- registering redirects to annoying url"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1879#event-6361"
}
```



---

archive/issue_comments_012385.json:
```json
{
    "body": "<a id='comment:2'></a>\nExcellent!\n\nWorks perfectly for me.",
    "created_at": "2008-03-17T04:30:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1879#issuecomment-12385",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:2'></a>
Excellent!

Works perfectly for me.



---

archive/issue_comments_012386.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 2.10.4.final",
    "created_at": "2008-03-17T04:35:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1879#issuecomment-12386",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 2.10.4.final



---

archive/issue_comments_012387.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-03-17T04:35:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1879#issuecomment-12387",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_006362.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-03-17T04:35:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/1879",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1879#event-6362"
}
```
