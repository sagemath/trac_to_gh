# Issue 1538: upgrades of sage_scripts can confuse bash

archive/issues_001538.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "Doing an upgrade of sage_scripts may modify files in local/bin while bash is executing them.  This means that bash may attempt to execute some mixture of the old and new versions (which will usually lead to an error, because bash will begin executing the new version in the middle of a line).\n\nCurrently Sage includes an effective workaround for this problem... the upgrade is automatically retried if it fails.  However, we can properly fix this by using \"exec\" such that the scripts `spkg/bin/sage` and `local/bin/sage-upgrade` are no longer running.\n\nReviewer: **Jeroen Demeyer**\n\nComponent: **build**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1538_\n\n",
    "closed_at": "2013-12-20T15:59:23Z",
    "created_at": "2007-12-16T16:01:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20build",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "upgrades of sage_scripts can confuse bash",
    "type": "issue",
    "updated_at": "2013-12-20T15:59:23Z",
    "url": "https://github.com/sagemath/sage/issues/1538",
    "user": "https://github.com/sagetrac-cwitty"
}
```
Doing an upgrade of sage_scripts may modify files in local/bin while bash is executing them.  This means that bash may attempt to execute some mixture of the old and new versions (which will usually lead to an error, because bash will begin executing the new version in the middle of a line).

Currently Sage includes an effective workaround for this problem... the upgrade is automatically retried if it fails.  However, we can properly fix this by using "exec" such that the scripts `spkg/bin/sage` and `local/bin/sage-upgrade` are no longer running.

Reviewer: **Jeroen Demeyer**

Component: **build**

_Issue created by migration from https://trac.sagemath.org/ticket/1538_





---

archive/issue_events_015176.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2007-12-16T16:01:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1538",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1538#event-15176"
}
```



---

archive/issue_events_015177.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2007-12-16T16:01:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1538",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebraic%20geometry",
    "label_color": "0000ff",
    "label_name": "c: algebraic geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1538#event-15177"
}
```



---

archive/issue_events_015178.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2007-12-16T16:01:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1538",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1538#event-15178"
}
```



---

archive/issue_events_015179.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2007-12-16T16:01:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1538",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1538#event-15179"
}
```



---

archive/issue_events_015180.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-12-16T16:01:56Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1538",
    "subject": "https://github.com/sagetrac-cwitty",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1538#event-15180"
}
```



---

archive/issue_events_015181.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-12-16T16:06:19Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/1538",
    "subject": "https://github.com/sagetrac-cwitty",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1538#event-15181"
}
```



---

archive/issue_events_015182.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-16T16:06:19Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1538",
    "subject": "https://github.com/sagetrac-cwitty",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1538#event-15182"
}
```



---

archive/issue_events_015183.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2007-12-16T16:06:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1538",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebraic%20geometry",
    "label_color": "0000ff",
    "label_name": "c: algebraic geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1538#event-15183"
}
```



---

archive/issue_events_015184.json:
```json
{
    "actor": "https://github.com/sagetrac-cwitty",
    "created_at": "2007-12-16T16:06:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1538",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20distribution",
    "label_color": "000080",
    "label_name": "c: distribution",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1538#event-15184"
}
```



---

archive/issue_comments_007135.json:
```json
{
    "body": "Attachment: **[1538_upgrade_root.patch.gz](https://github.com/sagemath/sage/files/ticket1538/1538_upgrade_root.patch.gz)**",
    "created_at": "2012-10-05T13:09:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1538#issuecomment-7135",
    "user": "https://github.com/jdemeyer"
}
```

Attachment: **[1538_upgrade_root.patch.gz](https://github.com/sagemath/sage/files/ticket1538/1538_upgrade_root.patch.gz)**



---

archive/issue_comments_007136.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n Doing an upgrade of sage_scripts may modify files in local/bin while bash is executing them.  This means that bash may attempt to execute some mixture of the old and new versions (which will usually lead to an error, because bash will begin executing the new version in the middle of a line).\n \n-Currently Sage includes an effective workaround for this problem... the upgrade is automatically retried if it fails.  However, we should put in a real fix.\n+Currently Sage includes an effective workaround for this problem... the upgrade is automatically retried if it fails.  However, we can properly fix this by using \"exec\" such that the scripts `spkg/bin/sage` and `local/bin/sage-upgrade` are no longer running.\n``````\n",
    "created_at": "2012-10-05T13:18:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1538#issuecomment-7136",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 Doing an upgrade of sage_scripts may modify files in local/bin while bash is executing them.  This means that bash may attempt to execute some mixture of the old and new versions (which will usually lead to an error, because bash will begin executing the new version in the middle of a line).
 
-Currently Sage includes an effective workaround for this problem... the upgrade is automatically retried if it fails.  However, we should put in a real fix.
+Currently Sage includes an effective workaround for this problem... the upgrade is automatically retried if it fails.  However, we can properly fix this by using "exec" such that the scripts `spkg/bin/sage` and `local/bin/sage-upgrade` are no longer running.
``````




---

archive/issue_comments_007137.json:
```json
{
    "body": "Author: **Jeroen Demeyer**",
    "created_at": "2012-10-05T13:18:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1538#issuecomment-7137",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Jeroen Demeyer**



---

archive/issue_events_015185.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-10-05T13:18:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1538",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20distribution",
    "label_color": "000080",
    "label_name": "c: distribution",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1538#event-15185"
}
```



---

archive/issue_events_015186.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-10-05T13:18:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1538",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20build",
    "label_color": "000080",
    "label_name": "c: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1538#event-15186"
}
```



---

archive/issue_events_015187.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:34:36Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/1538",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1538#event-15187"
}
```



---

archive/issue_events_015188.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:34:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1538",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1538#event-15188"
}
```



---

archive/issue_comments_007138.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nAttachment: **[1538_upgrade_scripts.patch.gz](https://github.com/sagemath/sage/files/ticket1538/1538_upgrade_scripts.patch.gz)**",
    "created_at": "2013-08-13T15:34:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1538#issuecomment-7138",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:4" align="right">comment:4</div>

Attachment: **[1538_upgrade_scripts.patch.gz](https://github.com/sagemath/sage/files/ticket1538/1538_upgrade_scripts.patch.gz)**



---

archive/issue_comments_007139.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nThis is solved by the git workflow because everything is now updated **before** the build, not **during** the build.",
    "created_at": "2013-12-19T12:16:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1538#issuecomment-7139",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:5" align="right">comment:5</div>

This is solved by the git workflow because everything is now updated **before** the build, not **during** the build.



---

archive/issue_comments_007140.json:
```json
{
    "body": "Changed author from **Jeroen Demeyer** to none",
    "created_at": "2013-12-19T12:17:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1538#issuecomment-7140",
    "user": "https://github.com/jdemeyer"
}
```

Changed author from **Jeroen Demeyer** to none



---

archive/issue_comments_007141.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2013-12-19T12:17:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1538",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1538#issuecomment-7141",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_015189.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-12-19T12:17:12Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/1538",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1538#event-15189"
}
```



---

archive/issue_events_015190.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-12-19T12:17:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1538",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1538#event-15190"
}
```



---

archive/issue_events_015191.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-12-19T12:17:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1538",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1538#event-15191"
}
```



---

archive/issue_events_015192.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-12-19T12:17:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1538",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1538#event-15192"
}
```



---

archive/issue_events_015193.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-12-19T12:17:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1538",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1538#event-15193"
}
```



---

archive/issue_events_015194.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-12-19T12:17:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1538",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1538#event-15194"
}
```



---

archive/issue_events_015195.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-12-20T15:59:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/1538",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1538#event-15195"
}
```



---

archive/issue_events_015196.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-12-20T15:59:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1538",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1538#event-15196"
}
```
