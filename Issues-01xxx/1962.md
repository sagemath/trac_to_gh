# Issue 1962: [with positive review] set_edge_label creates edges when multiple edges are allowed

archive/issues_001962.json:
```json
{
    "body": "\n```\nsage: g = Graph({0: [0,1,1,2]}, loops=True, multiedges=True)\nsage: g.set_edge_label(0,0,'test')\nsage: g.edges()\n\n[(0, 0, 'e'),\n (0, 0, 's'),\n (0, 0, 't'),\n (0, 0, 't'),\n (0, 1, None),\n (0, 1, None),\n (0, 2, None)]\n```\n\nI suggest that set_edge_labels should *never* create an edge or the function name should be changed.\n\n**Assignee:** @rlmill\n\n**Resolution:** fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/1962\n\n",
    "closed_at": "2008-02-26T01:08:26Z",
    "created_at": "2008-01-28T19:49:16Z",
    "labels": [
        "component: graph theory",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.10.3",
    "title": "[with positive review] set_edge_label creates edges when multiple edges are allowed",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/1962",
    "user": "https://github.com/jasongrout"
}
```

```
sage: g = Graph({0: [0,1,1,2]}, loops=True, multiedges=True)
sage: g.set_edge_label(0,0,'test')
sage: g.edges()

[(0, 0, 'e'),
 (0, 0, 's'),
 (0, 0, 't'),
 (0, 0, 't'),
 (0, 1, None),
 (0, 1, None),
 (0, 2, None)]
```

I suggest that set_edge_labels should *never* create an edge or the function name should be changed.

**Assignee:** @rlmill

**Resolution:** fixed

Issue created by migration from https://trac.sagemath.org/ticket/1962





---

archive/issue_comments_013800.json:
```json
{
    "body": "<a id='comment:1'></a>\nThis is not a bug in `set_edge_label` itself:\n\n```\nsage: g._nxg.adj\n{0: {0: 'test', 1: [None, None], 2: [None]},\n 1: {0: [None, None]},\n 2: {0: [None]}}\n```\nIt is in fact a bug in NetworkX's `edges` function:\n\n```\nsage: g._nxg.edges()\n[(0, 0, 't'),\n (0, 0, 'e'),\n (0, 0, 's'),\n (0, 0, 't'),\n (0, 1, None),\n (0, 1, None),\n (0, 2, None)]\n```",
    "created_at": "2008-02-17T00:07:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1962",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1962#issuecomment-13800",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:1'></a>
This is not a bug in `set_edge_label` itself:

```
sage: g._nxg.adj
{0: {0: 'test', 1: [None, None], 2: [None]},
 1: {0: [None, None]},
 2: {0: [None]}}
```
It is in fact a bug in NetworkX's `edges` function:

```
sage: g._nxg.edges()
[(0, 0, 't'),
 (0, 0, 'e'),
 (0, 0, 's'),
 (0, 0, 't'),
 (0, 1, None),
 (0, 1, None),
 (0, 2, None)]
```



---

archive/issue_comments_013801.json:
```json
{
    "body": "<a id='comment:2'></a>\nMy mistake: when multiple edges is True, the representation is slightly different:\n\n```\nsage: G = Graph({0:[1]})\nsage: G._nxg.adj\n{0: {1: None}, 1: {0: None}}\nsage: G = Graph({0:[1]}, multiedges=True)\nsage: G._nxg.adj\n{0: {1: [None]}, 1: {0: [None]}}\n```",
    "created_at": "2008-02-17T00:09:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1962",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1962#issuecomment-13801",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:2'></a>
My mistake: when multiple edges is True, the representation is slightly different:

```
sage: G = Graph({0:[1]})
sage: G._nxg.adj
{0: {1: None}, 1: {0: None}}
sage: G = Graph({0:[1]}, multiedges=True)
sage: G._nxg.adj
{0: {1: [None]}, 1: {0: [None]}}
```



---

archive/issue_comments_013802.json:
```json
{
    "body": "<a id='comment:3'></a>\nIn fact, `set_edge_label` only makes sense when there is only one possible edge whose label is to be set: if there is more than one, which label to set?",
    "created_at": "2008-02-17T00:11:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1962",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1962#issuecomment-13802",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:3'></a>
In fact, `set_edge_label` only makes sense when there is only one possible edge whose label is to be set: if there is more than one, which label to set?



---

archive/issue_comments_013803.json:
```json
{
    "body": "**Changing status** from new to needs_review.",
    "created_at": "2008-02-17T00:24:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1962",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1962#issuecomment-13803",
    "user": "https://github.com/rlmill"
}
```

**Changing status** from new to needs_review.



---

archive/attachments_001601.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "1962.patch",
    "asset_url": "tarball://root/attachments/some-uuid/ticket1962/1962.patch",
    "created_at": "2008-02-17T00:24:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1962",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.patch",
    "user": "https://github.com/rlmill"
}
```



---

archive/issue_comments_013804.json:
```json
{
    "body": "<a id='comment:4'></a>\n",
    "created_at": "2008-02-17T00:24:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1962",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1962#issuecomment-13804",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:4'></a>




---

archive/issue_events_005778.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1962",
    "rename": {
        "from": "set_edge_label creates edges when multiple edges are allowed",
        "to": "[with positive review] set_edge_label creates edges when multiple edges are allowed"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1962#event-5778"
}
```



---

archive/attachments_001602.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "1962-ref.patch",
    "asset_url": "tarball://root/attachments/some-uuid/ticket1962/1962-ref.patch",
    "created_at": "2008-02-26T01:06:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1962",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.patch",
    "user": "https://github.com/ncalexan"
}
```



---

archive/issue_comments_013805.json:
```json
{
    "body": "<a id='comment:5'></a>\nAfter discussion on IRC, this looks good to me.  Apply.  Needs both patches, -ref second.",
    "created_at": "2008-02-26T01:06:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1962",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1962#issuecomment-13805",
    "user": "https://github.com/ncalexan"
}
```

<a id='comment:5'></a>
After discussion on IRC, this looks good to me.  Apply.  Needs both patches, -ref second.



---

archive/issue_comments_013806.json:
```json
{
    "body": "<a id='comment:6'></a>\nMerged both patches in Sage 2.10.3.alpha0",
    "created_at": "2008-02-26T01:08:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1962",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1962#issuecomment-13806",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:6'></a>
Merged both patches in Sage 2.10.3.alpha0



---

archive/issue_comments_013807.json:
```json
{
    "body": "**Resolution:** fixed",
    "created_at": "2008-02-26T01:08:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1962",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1962#issuecomment-13807",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

**Resolution:** fixed



---

archive/issue_events_005779.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-02-26T01:08:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/1962",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1962#event-5779"
}
```
