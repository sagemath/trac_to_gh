# Issue 1199: [with spkg] cvxopt update to cvxopt-0.9.p1.spkg

archive/issues_001199.json:
```json
{
    "body": "Minor addition to cvxopt package so that when building with gfortran, sage/local/lib is in the library path. Also stripped out documentation to save some space. \n\ncvxopt-0.9.p1.spkg\n\nhttp://sage.math.washington.edu/home/jkantor/spkgs\n\nThis supercedes the cvxopt versions in 1125 and 1161\n\nAssignee: @williamstein\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/1199\n\n",
    "closed_at": "2007-11-18T08:24:53Z",
    "created_at": "2007-11-18T08:03:11Z",
    "labels": [
        "component: packages: standard"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.8.13",
    "title": "[with spkg] cvxopt update to cvxopt-0.9.p1.spkg",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/1199",
    "user": "https://trac.sagemath.org/admin/accounts/users/jkantor"
}
```
Minor addition to cvxopt package so that when building with gfortran, sage/local/lib is in the library path. Also stripped out documentation to save some space. 

cvxopt-0.9.p1.spkg

http://sage.math.washington.edu/home/jkantor/spkgs

This supercedes the cvxopt versions in 1125 and 1161

Assignee: @williamstein

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/1199





---

archive/issue_comments_007786.json:
```json
{
    "body": "Changing assignee from @williamstein to jkantor.",
    "created_at": "2007-11-18T08:03:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1199",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1199#issuecomment-7786",
    "user": "https://trac.sagemath.org/admin/accounts/users/jkantor"
}
```

Changing assignee from @williamstein to jkantor.



---

archive/issue_events_004082.json:
```json
{
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/1199",
    "label": "component: numerical",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1199#event-4082"
}
```



---

archive/issue_events_004083.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1199",
    "rename": {
        "from": "cvxopt fix [with spkg]",
        "to": "[with spkg] cvxopt update to cvxopt-0.9.p1.spkg"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1199#event-4083"
}
```



---

archive/issue_events_004084.json:
```json
{
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/1199",
    "label": "component: numerical",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1199#event-4084"
}
```



---

archive/issue_events_004085.json:
```json
{
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/1199",
    "label": "component: packages",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1199#event-4085"
}
```



---

archive/issue_comments_007787.json:
```json
{
    "body": "Changing assignee from jkantor to @williamstein.",
    "created_at": "2007-11-18T08:16:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1199",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1199#issuecomment-7787",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing assignee from jkantor to @williamstein.



---

archive/issue_comments_007788.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2007-11-18T08:24:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1199",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1199#issuecomment-7788",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_007789.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in 2.8.13.alpha0.",
    "created_at": "2007-11-18T08:24:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1199",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1199#issuecomment-7789",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in 2.8.13.alpha0.



---

archive/issue_events_004086.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-11-18T08:24:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/1199",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1199#event-4086"
}
```
