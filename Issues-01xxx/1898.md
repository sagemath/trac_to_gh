# Issue 1898: [with patch, with positive review] Sage 2.10: numerical doctest failure for polynomial_element.pyx on Linux/Itanium

archive/issues_001898.json:
```json
{
    "body": "\n```\nsage -t  devel/sage-main/sage/rings/polynomial/\npolynomial_element.pyx**********************************************************************\nFile \"polynomial_element.pyx\", line 2669:\n    sage: p.roots(ring=CIF)\nExpected:\n    [([-1.4142135623730952 .. -1.4142135623730949], 1),\n([1.4142135623730949 ..\n1.4142135623730952], 1), ([-1.2146389322441827 .. -1.2146389322441821]\n- [0.1414250525823937... .. 0.1414250525823939...]*I, 2),\n([-0.141425052582393... .. -0.1414250525823937...] +\n[1.2146389322441821 .. 1.2146389322441827]*I, 2),\n([0.141425052582393... .. 0.141425052582393...] -\n[1.2146389322441821 .. 1.2146389322441827]*I, 2),\n([1.2146389322441821 .. 1.2146389322441827] + [0.14142505258239376 ..\n0.14142505258239399]*I, 2)]\nGot:\n    [([-1.4142135623730952 .. -1.4142135623730949], 1),\n([1.4142135623730949 ..\n1.4142135623730952], 1), ([-1.2146389322441827 .. -1.2146389322441821]\n- [0.14142505258239371 .. 0.14142505258239397]*I, 2),\n([-0.14142505258239397 .. -0.14142505258239376] +\n[1.2146389322441821 .. 1.2146389322441829]*I, 2),\n([0.14142505258239373 .. 0.14142505258239394] - [1.2146389322441821 ..\n1.2146389322441829]*I, 2), ([1.2146389322441821 .. 1.2146389322441827]\n+ [0.14142505258239376 .. 0.14142505258239399]*I, 2)]\n**********************************************************************\n```\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/1898\n\n",
    "closed_at": "2008-01-24T20:48:19Z",
    "created_at": "2008-01-23T21:15:57Z",
    "labels": [
        "component: doctest coverage",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.10.1",
    "title": "[with patch, with positive review] Sage 2.10: numerical doctest failure for polynomial_element.pyx on Linux/Itanium",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/1898",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

```
sage -t  devel/sage-main/sage/rings/polynomial/
polynomial_element.pyx**********************************************************************
File "polynomial_element.pyx", line 2669:
    sage: p.roots(ring=CIF)
Expected:
    [([-1.4142135623730952 .. -1.4142135623730949], 1),
([1.4142135623730949 ..
1.4142135623730952], 1), ([-1.2146389322441827 .. -1.2146389322441821]
- [0.1414250525823937... .. 0.1414250525823939...]*I, 2),
([-0.141425052582393... .. -0.1414250525823937...] +
[1.2146389322441821 .. 1.2146389322441827]*I, 2),
([0.141425052582393... .. 0.141425052582393...] -
[1.2146389322441821 .. 1.2146389322441827]*I, 2),
([1.2146389322441821 .. 1.2146389322441827] + [0.14142505258239376 ..
0.14142505258239399]*I, 2)]
Got:
    [([-1.4142135623730952 .. -1.4142135623730949], 1),
([1.4142135623730949 ..
1.4142135623730952], 1), ([-1.2146389322441827 .. -1.2146389322441821]
- [0.14142505258239371 .. 0.14142505258239397]*I, 2),
([-0.14142505258239397 .. -0.14142505258239376] +
[1.2146389322441821 .. 1.2146389322441829]*I, 2),
([0.14142505258239373 .. 0.14142505258239394] - [1.2146389322441821 ..
1.2146389322441829]*I, 2), ([1.2146389322441821 .. 1.2146389322441827]
+ [0.14142505258239376 .. 0.14142505258239399]*I, 2)]
**********************************************************************
```

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/1898





---

archive/issue_events_006439.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1898",
    "rename": {
        "from": "Sage 2.10: numerical doctest failure for polynomial_element.pyx on Linux/Itanium",
        "to": "[with patch, needs review] Sage 2.10: numerical doctest failure for polynomial_element.pyx on Linux/Itanium"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1898#event-6439"
}
```



---

archive/issue_comments_012506.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [Sage-2.10.1.alpha2-fix-numerical-doctests-failure-trac_1898.patch](tarball://root/attachments/some-uuid/ticket1898/Sage-2.10.1.alpha2-fix-numerical-doctests-failure-trac_1898.patch) by mabshoff created at 2008-01-24 00:29:37",
    "created_at": "2008-01-24T00:29:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1898",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1898#issuecomment-12506",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
Attachment [Sage-2.10.1.alpha2-fix-numerical-doctests-failure-trac_1898.patch](tarball://root/attachments/some-uuid/ticket1898/Sage-2.10.1.alpha2-fix-numerical-doctests-failure-trac_1898.patch) by mabshoff created at 2008-01-24 00:29:37



---

archive/issue_comments_012507.json:
```json
{
    "body": "Attachment [trac-1898-part2-fix_for_lower_order_doctest_bits.patch](tarball://root/attachments/some-uuid/ticket1898/trac-1898-part2-fix_for_lower_order_doctest_bits.patch) by @williamstein created at 2008-01-24 19:48:53\n\napply the other patch then this one.",
    "created_at": "2008-01-24T19:48:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1898",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1898#issuecomment-12507",
    "user": "https://github.com/williamstein"
}
```

Attachment [trac-1898-part2-fix_for_lower_order_doctest_bits.patch](tarball://root/attachments/some-uuid/ticket1898/trac-1898-part2-fix_for_lower_order_doctest_bits.patch) by @williamstein created at 2008-01-24 19:48:53

apply the other patch then this one.



---

archive/issue_events_006440.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1898",
    "rename": {
        "from": "[with patch, needs review] Sage 2.10: numerical doctest failure for polynomial_element.pyx on Linux/Itanium",
        "to": "[with patch, with positive review] Sage 2.10: numerical doctest failure for polynomial_element.pyx on Linux/Itanium"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1898#event-6440"
}
```



---

archive/issue_comments_012508.json:
```json
{
    "body": "<a id='comment:2'></a>\nKate confirmed in https://groups.google.com/group/sage-support/t/d489d89ec68b6706 that William's patch now fixes it.\n\nCheers,\n\nMichael",
    "created_at": "2008-01-24T20:48:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1898",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1898#issuecomment-12508",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Kate confirmed in https://groups.google.com/group/sage-support/t/d489d89ec68b6706 that William's patch now fixes it.

Cheers,

Michael



---

archive/issue_comments_012509.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-01-24T20:48:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1898",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1898#issuecomment-12509",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_012510.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 2.10.1.alpha2",
    "created_at": "2008-01-24T20:48:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1898",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1898#issuecomment-12510",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 2.10.1.alpha2



---

archive/issue_events_006441.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-01-24T20:48:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/1898",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1898#event-6441"
}
```
