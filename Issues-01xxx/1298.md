# Issue 1298: To build atlas on osx, we need to actually build lapack on osx.

archive/issues_001298.json:
```json
{
    "assignees": [],
    "body": "The current lapack.spkg doesn't build on osx (because osx intel has one in /usr/lib)\nSince we will buid atlas on all platforms, we need to build lapack on osx. \n\nhttp://sage.math.washington.edu/home/jkantor/spkgs/lapack-20071123.spkg \n\n\n\n\n               \n\nAssignee: **@sagetrac-jkantor**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1298_\n\n",
    "closed_at": "2007-12-01T23:25:04Z",
    "created_at": "2007-11-28T09:47:57Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20numerical",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.15",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "To build atlas on osx, we need to actually build lapack on osx.",
    "type": "issue",
    "updated_at": "2007-12-01T23:25:04Z",
    "url": "https://github.com/sagemath/sage/issues/1298",
    "user": "https://github.com/sagetrac-jkantor"
}
```
The current lapack.spkg doesn't build on osx (because osx intel has one in /usr/lib)
Since we will buid atlas on all platforms, we need to build lapack on osx. 

http://sage.math.washington.edu/home/jkantor/spkgs/lapack-20071123.spkg 




               

Assignee: **@sagetrac-jkantor**

_Issue created by migration from https://trac.sagemath.org/ticket/1298_





---

archive/issue_events_011046.json:
```json
{
    "actor": "https://github.com/sagetrac-jkantor",
    "created_at": "2007-11-28T09:47:57Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1298",
    "milestone_number": null,
    "milestone_title": "sage-2.8.15",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1298#event-11046"
}
```



---

archive/issue_events_011047.json:
```json
{
    "actor": "https://github.com/sagetrac-jkantor",
    "created_at": "2007-11-28T09:47:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1298",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20numerical",
    "label_color": "0000ff",
    "label_name": "component: numerical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1298#event-11047"
}
```



---

archive/issue_events_011048.json:
```json
{
    "actor": "https://github.com/sagetrac-jkantor",
    "created_at": "2007-11-28T09:47:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1298",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1298#event-11048"
}
```



---

archive/issue_events_011049.json:
```json
{
    "actor": "https://github.com/sagetrac-jkantor",
    "created_at": "2007-11-28T09:47:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1298",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1298#event-11049"
}
```



---

archive/issue_events_011050.json:
```json
{
    "actor": "https://github.com/sagetrac-jkantor",
    "created_at": "2007-11-28T09:51:40Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/1298",
    "title_is": "To build atlas on osx, we need to actually build lapack on osx.",
    "title_was": "To build atlas on osx, we need to actual build lapack on osx.",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1298#event-11050"
}
```



---

archive/issue_comments_006251.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nLooks good to me.\n\nCheers,\n\nMichael",
    "created_at": "2007-12-01T23:21:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1298",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1298#issuecomment-6251",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">Comment 2</div>

Looks good to me.

Cheers,

Michael



---

archive/issue_events_011051.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-01T23:25:04Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1298",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1298#event-11051"
}
```



---

archive/issue_comments_006252.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nMerged in 2.8.15.alpha2.",
    "created_at": "2007-12-01T23:25:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1298",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1298#issuecomment-6252",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">Comment 3</div>

Merged in 2.8.15.alpha2.
