# Issue 1269: [with patch, with positive review] sage -t --omega does not work

archive/issues_001269.json:
```json
{
    "body": "After this patch, sage -t --omega should work.\n\nAssignee: @williamstein\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/1269\n\n",
    "closed_at": "2007-12-01T21:28:47Z",
    "created_at": "2007-11-25T15:44:13Z",
    "labels": [
        "component: user interface",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.8.15",
    "title": "[with patch, with positive review] sage -t --omega does not work",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/1269",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```
After this patch, sage -t --omega should work.

Assignee: @williamstein

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/1269





---

archive/issue_comments_008318.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [1269.patch](tarball://root/attachments/some-uuid/ticket1269/1269.patch) by mabshoff created at 2007-11-25 16:47:52\n\nLooks good to me.\n\nCheers,\n\nMichael",
    "created_at": "2007-11-25T16:47:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1269",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1269#issuecomment-8318",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
Attachment [1269.patch](tarball://root/attachments/some-uuid/ticket1269/1269.patch) by mabshoff created at 2007-11-25 16:47:52

Looks good to me.

Cheers,

Michael



---

archive/issue_events_004316.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1269",
    "rename": {
        "from": "[with patch] sage -t --omega does not work",
        "to": "[with patch, with positive review] sage -t --omega does not work"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1269#event-4316"
}
```



---

archive/issue_events_004317.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-12-01T21:28:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/1269",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1269#event-4317"
}
```



---

archive/issue_comments_008319.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in 2.8.15.alpha2.",
    "created_at": "2007-12-01T21:28:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1269",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1269#issuecomment-8319",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in 2.8.15.alpha2.



---

archive/issue_comments_008320.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2007-12-01T21:28:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1269",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1269#issuecomment-8320",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
