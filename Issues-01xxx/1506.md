# Issue 1506: [with spkg] ntl spkg -- dumb intentional error during the build

archive/issues_001506.json:
```json
{
    "body": "This is dumb:\n\n```\ndmharvey: i'm building sage-2.9alpha7, and there's a problem in the build log with the NTL build, but the rest of it seems to be going okay (for the moment)\n[06:49am] dmharvey: i686-apple-darwin8-g++-4.0.1: unrecognized option '-shared'\n[06:49am] dmharvey: _main\n[06:49am] dmharvey: ___gmpn_add_n\n[06:49am] dmharvey: ___gmpn_addmul_1\n[06:49am] dmharvey: ___gmpn_divrem_1\n[06:49am] dmharvey: ___gmpn_gcd\n```\n\nAssignee: @williamstein\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/1506\n\n",
    "closed_at": "2007-12-14T22:28:06Z",
    "created_at": "2007-12-14T17:11:11Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.9",
    "title": "[with spkg] ntl spkg -- dumb intentional error during the build",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/1506",
    "user": "https://github.com/williamstein"
}
```
This is dumb:

```
dmharvey: i'm building sage-2.9alpha7, and there's a problem in the build log with the NTL build, but the rest of it seems to be going okay (for the moment)
[06:49am] dmharvey: i686-apple-darwin8-g++-4.0.1: unrecognized option '-shared'
[06:49am] dmharvey: _main
[06:49am] dmharvey: ___gmpn_add_n
[06:49am] dmharvey: ___gmpn_addmul_1
[06:49am] dmharvey: ___gmpn_divrem_1
[06:49am] dmharvey: ___gmpn_gcd
```

Assignee: @williamstein

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/1506





---

archive/issue_events_005134.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1506",
    "rename": {
        "from": "ntl spkg -- dumb intentional error during the build",
        "to": "[with patch] ntl spkg -- dumb intentional error during the build"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1506#event-5134"
}
```



---

archive/issue_comments_010081.json:
```json
{
    "body": "<a id='comment:2'></a>\nNew spkg at \n\n http://sage.math.washington.edu/home/was/tmp/ntl-5.4.1.p8.spkg",
    "created_at": "2007-12-14T17:35:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1506",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1506#issuecomment-10081",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:2'></a>
New spkg at 

 http://sage.math.washington.edu/home/was/tmp/ntl-5.4.1.p8.spkg



---

archive/issue_events_005135.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1506",
    "rename": {
        "from": "[with patch] ntl spkg -- dumb intentional error during the build",
        "to": "[with spkg] ntl spkg -- dumb intentional error during the build"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1506#event-5135"
}
```



---

archive/issue_comments_010082.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2007-12-14T22:28:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1506",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1506#issuecomment-10082",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_010083.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in 2.9.rc0.",
    "created_at": "2007-12-14T22:28:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1506",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1506#issuecomment-10083",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in 2.9.rc0.



---

archive/issue_events_005136.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-12-14T22:28:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/1506",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1506#event-5136"
}
```
