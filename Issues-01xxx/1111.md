# Issue 1111: [with patch+review] Symbolic equation expand left and right

archive/issues_001111.json:
```json
{
    "body": "\n```\nsage: eqn.expand() # does it to both sides\nsage: eqn.expand('right') # does it to the right\nsage: eqn.expand('left') # does it to the right\n```\n\nSee the sage-devel thread Enhancing the SymbolicEquation class\n\nAssignee: @mwhansen\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/1111\n\n",
    "closed_at": "2007-12-01T11:31:59Z",
    "created_at": "2007-11-06T05:25:35Z",
    "labels": [
        "component: calculus"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.8.15",
    "title": "[with patch+review] Symbolic equation expand left and right",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/1111",
    "user": "https://github.com/robertwb"
}
```

```
sage: eqn.expand() # does it to both sides
sage: eqn.expand('right') # does it to the right
sage: eqn.expand('left') # does it to the right
```

See the sage-devel thread Enhancing the SymbolicEquation class

Assignee: @mwhansen

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/1111





---

archive/issue_events_003738.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-11-07T08:40:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/1111",
    "milestone": "sage-2.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1111#event-3738"
}
```



---

archive/issue_events_003739.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1111",
    "rename": {
        "from": "Symbolic equation expand left and right",
        "to": "[with patch] Symbolic equation expand left and right"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1111#event-3739"
}
```



---

archive/issue_comments_007021.json:
```json
{
    "body": "<a id='comment:2'></a>\nAttachment [1111.patch](tarball://root/attachments/some-uuid/ticket1111/1111.patch) by @mwhansen created at 2007-11-26 22:01:19",
    "created_at": "2007-11-26T22:01:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1111",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1111#issuecomment-7021",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:2'></a>
Attachment [1111.patch](tarball://root/attachments/some-uuid/ticket1111/1111.patch) by @mwhansen created at 2007-11-26 22:01:19



---

archive/issue_comments_007022.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2007-11-26T22:01:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1111",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1111#issuecomment-7022",
    "user": "https://github.com/mwhansen"
}
```

Changing status from new to assigned.



---

archive/issue_comments_007023.json:
```json
{
    "body": "Changing assignee from @williamstein to @mwhansen.",
    "created_at": "2007-11-26T22:01:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1111",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1111#issuecomment-7023",
    "user": "https://github.com/mwhansen"
}
```

Changing assignee from @williamstein to @mwhansen.



---

archive/issue_events_003740.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2007-11-26T22:01:19Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/1111",
    "milestone": "sage-2.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1111#event-3740"
}
```



---

archive/issue_events_003741.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2007-11-26T22:01:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/1111",
    "milestone": "sage-2.8.15",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1111#event-3741"
}
```



---

archive/issue_events_003742.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/1111",
    "rename": {
        "from": "[with patch] Symbolic equation expand left and right",
        "to": "[with patch+review] Symbolic equation expand left and right"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1111#event-3742"
}
```



---

archive/issue_comments_007024.json:
```json
{
    "body": "<a id='comment:3'></a>\nWorks great for me. Are there any other symbolic-expression functions that one would want to add to symbolic equations in a like manner? Could this be automated?",
    "created_at": "2007-12-01T00:08:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1111",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1111#issuecomment-7024",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:3'></a>
Works great for me. Are there any other symbolic-expression functions that one would want to add to symbolic equations in a like manner? Could this be automated?



---

archive/issue_comments_007025.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2007-12-01T11:31:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1111",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1111#issuecomment-7025",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_007026.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in 2.8.15.alpha0.",
    "created_at": "2007-12-01T11:31:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/1111",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/1111#issuecomment-7026",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in 2.8.15.alpha0.



---

archive/issue_events_003743.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2007-12-01T11:31:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/1111",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/1111#event-3743"
}
```
