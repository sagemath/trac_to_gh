# Issue 1111: [with patch+review] Symbolic equation expand left and right

archive/issues_001111.json:
```json
{
    "assignees": [
        "https://github.com/mwhansen"
    ],
    "body": "\n```\nsage: eqn.expand() # does it to both sides\nsage: eqn.expand('right') # does it to the right\nsage: eqn.expand('left') # does it to the right\n```\n\nSee the sage-devel thread Enhancing the SymbolicEquation class\n\nComponent: **calculus**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/1111_\n\n",
    "closed_at": "2007-12-01T11:31:59Z",
    "created_at": "2007-11-06T05:25:35Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20calculus",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-2.8.15",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "[with patch+review] Symbolic equation expand left and right",
    "type": "issue",
    "updated_at": "2007-12-01T11:31:59Z",
    "url": "https://github.com/sagemath/sage/issues/1111",
    "user": "https://github.com/robertwb"
}
```

```
sage: eqn.expand() # does it to both sides
sage: eqn.expand('right') # does it to the right
sage: eqn.expand('left') # does it to the right
```

See the sage-devel thread Enhancing the SymbolicEquation class

Component: **calculus**

_Issue created by migration from https://trac.sagemath.org/ticket/1111_





---

archive/issue_events_010659.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2007-11-06T05:25:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1111",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20calculus",
    "label_color": "0000ff",
    "label_name": "c: calculus",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1111#event-10659"
}
```



---

archive/issue_events_010660.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2007-11-06T05:25:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1111",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1111#event-10660"
}
```



---

archive/issue_events_010661.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2007-11-06T05:25:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1111",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1111#event-10661"
}
```



---

archive/issue_events_010662.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-11-06T05:25:35Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1111",
    "subject": "https://github.com/robertwb",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1111#event-10662"
}
```



---

archive/issue_events_010663.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-11-07T08:40:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1111",
    "milestone_number": null,
    "milestone_title": "sage-2.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1111#event-10663"
}
```



---

archive/issue_comments_004801.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nAttachment: **[1111.patch.gz](https://github.com/sagemath/sage/files/ticket1111/1111.patch.gz)**",
    "created_at": "2007-11-26T22:01:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1111",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1111#issuecomment-4801",
    "user": "https://github.com/mwhansen"
}
```

<div id="comment:2" align="right">comment:2</div>

Attachment: **[1111.patch.gz](https://github.com/sagemath/sage/files/ticket1111/1111.patch.gz)**



---

archive/issue_events_010664.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2007-11-26T22:01:19Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/1111",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1111#event-10664"
}
```



---

archive/issue_events_010665.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2007-11-26T22:01:19Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/1111",
    "subject": "https://github.com/mwhansen",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1111#event-10665"
}
```



---

archive/issue_events_010666.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2007-11-26T22:01:19Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/1111",
    "milestone_number": null,
    "milestone_title": "sage-2.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1111#event-10666"
}
```



---

archive/issue_events_010667.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2007-11-26T22:01:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1111",
    "milestone_number": null,
    "milestone_title": "sage-2.8.15",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1111#event-10667"
}
```



---

archive/issue_events_010668.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2007-12-01T00:08:40Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/1111",
    "title_is": "[with patch+review] Symbolic equation expand left and right",
    "title_was": "Symbolic equation expand left and right",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1111#event-10668"
}
```



---

archive/issue_comments_004802.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nWorks great for me. Are there any other symbolic-expression functions that one would want to add to symbolic equations in a like manner? Could this be automated?",
    "created_at": "2007-12-01T00:08:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1111",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1111#issuecomment-4802",
    "user": "https://github.com/robertwb"
}
```

<div id="comment:3" align="right">comment:3</div>

Works great for me. Are there any other symbolic-expression functions that one would want to add to symbolic equations in a like manner? Could this be automated?



---

archive/issue_comments_004803.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nMerged in 2.8.15.alpha0.",
    "created_at": "2007-12-01T11:31:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1111",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1111#issuecomment-4803",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Merged in 2.8.15.alpha0.



---

archive/issue_events_010669.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2007-12-01T11:31:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1111",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1111#event-10669"
}
```
