# Issue 1950: When one clicks the print button in the notebook it should render the html with jsmath but with no warning at all.

archive/issues_001950.json:
```json
{
    "assignees": [],
    "body": "Suggested by Bill Purvis.\n\n**Assignee:** boothby\n\nIssue created by migration from https://trac.sagemath.org/ticket/1950\n\n",
    "closed_at": "2008-04-21T14:23:41Z",
    "created_at": "2008-01-27T20:27:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20notebook",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "When one clicks the print button in the notebook it should render the html with jsmath but with no warning at all.",
    "type": "issue",
    "updated_at": "2008-04-21T14:23:41Z",
    "url": "https://github.com/sagemath/sage/issues/1950",
    "user": "https://github.com/williamstein"
}
```
Suggested by Bill Purvis.

**Assignee:** boothby

Issue created by migration from https://trac.sagemath.org/ticket/1950





---

archive/issue_events_015492.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-01-27T20:27:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1950",
    "milestone_number": null,
    "milestone_title": "sage-3.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1950#event-15492"
}
```



---

archive/issue_events_015493.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-01-27T20:27:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1950",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20notebook",
    "label_color": "08517b",
    "label_name": "component: notebook",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1950#event-15493"
}
```



---

archive/issue_events_015494.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-01-27T20:27:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/1950",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "008080",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1950#event-15494"
}
```



---

archive/issue_events_015495.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-04-21T14:23:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/1950",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1950#event-15495"
}
```



---

archive/issue_events_015496.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-04-21T14:23:41Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/1950",
    "milestone_number": null,
    "milestone_title": "sage-3.0.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1950#event-15496"
}
```



---

archive/issue_events_015497.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-04-21T14:23:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/1950",
    "milestone_number": null,
    "milestone_title": "sage-3.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/1950#event-15497"
}
```



---

archive/issue_comments_009594.json:
```json
{
    "body": "<a id='comment:1'>**Comment 1:**</a>\nThis is fixed, since now we never show a jsmath warning.",
    "created_at": "2008-04-21T14:23:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/1950",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/1950#issuecomment-9594",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'>**Comment 1:**</a>
This is fixed, since now we never show a jsmath warning.
