...
```json
{
    ...
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8968#issuecomment-82653",
    "user": "was"
}
```
...
```json
{
    "body": "You have to do `R.<x> = QQ[]` first, which is the analogue of Magma's `R<x> := PolynomialRing(RationalField());` .  Otherwise, x is merely a formal symbolic variable (in the sense of Sage's maple-like symbolic calculus package), and since Magma has no functionality for working with symbolic expressions, it just turns symbolic expressions into strings in Magma.  So here's the way to do the computation you want:\n\n\n```\n\nsage: R.<x> = QQ[]\nsage: poly0 = x^2 + x + 1\nsage: poly1 = x^3 + x + 1\nsage: p0 = magma(poly0)\nsage: p1 = magma(poly1)\nsage:  magma.Resultant(p0,p1)\n3\n```\n",
    "created_at": "2010-06-04T05:43:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8968",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8968#issuecomment-82654",
    "user": "was"
}
```
