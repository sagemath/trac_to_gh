# Issue 2379: Merge sage-ptest into sage-test

archive/issues_002379.json:
```json
{
    "body": "Assignee: @garyfurnish\n\nWhen stable, merge sage-ptest into sage-test. This will also give us tex file failures in the list of failures when testing.  \n\nIssue created by migration from https://trac.sagemath.org/ticket/2379\n\n",
    "created_at": "2008-03-03T22:42:17Z",
    "labels": [
        "build",
        "major",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.0",
    "title": "Merge sage-ptest into sage-test",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2379",
    "user": "@garyfurnish"
}
```
Assignee: @garyfurnish

When stable, merge sage-ptest into sage-test. This will also give us tex file failures in the list of failures when testing.  

Issue created by migration from https://trac.sagemath.org/ticket/2379





---

archive/issue_comments_016053.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-03-03T22:42:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2379",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2379#issuecomment-16053",
    "user": "@garyfurnish"
}
```

Changing status from new to assigned.



---

archive/issue_comments_016054.json:
```json
{
    "body": "Required: Fix hang on test segfault",
    "created_at": "2008-03-20T11:26:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2379",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2379#issuecomment-16054",
    "user": "@garyfurnish"
}
```

Required: Fix hang on test segfault



---

archive/issue_comments_016055.json:
```json
{
    "body": "Hang on segfault appears to be fixed",
    "created_at": "2008-03-21T14:24:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2379",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2379#issuecomment-16055",
    "user": "@garyfurnish"
}
```

Hang on segfault appears to be fixed



---

archive/issue_comments_016056.json:
```json
{
    "body": "Attachment [trac_2739_local_1.patch](tarball://root/attachments/some-uuid/ticket2379/trac_2739_local_1.patch) by @garyfurnish created at 2008-04-08 10:49:28",
    "created_at": "2008-04-08T10:49:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2379",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2379#issuecomment-16056",
    "user": "@garyfurnish"
}
```

Attachment [trac_2739_local_1.patch](tarball://root/attachments/some-uuid/ticket2379/trac_2739_local_1.patch) by @garyfurnish created at 2008-04-08 10:49:28



---

archive/issue_comments_016057.json:
```json
{
    "body": "Attachment [trac_2739_local_2.patch](tarball://root/attachments/some-uuid/ticket2379/trac_2739_local_2.patch) by mabshoff created at 2008-04-08 11:02:23\n\nPatches look good to me. Let's merge this now.\n\nCheers,\n\nMichael",
    "created_at": "2008-04-08T11:02:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2379",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2379#issuecomment-16057",
    "user": "mabshoff"
}
```

Attachment [trac_2739_local_2.patch](tarball://root/attachments/some-uuid/ticket2379/trac_2739_local_2.patch) by mabshoff created at 2008-04-08 11:02:23

Patches look good to me. Let's merge this now.

Cheers,

Michael



---

archive/issue_comments_016058.json:
```json
{
    "body": "Merged in Sage 3.0.alpha3",
    "created_at": "2008-04-08T11:02:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2379",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2379#issuecomment-16058",
    "user": "mabshoff"
}
```

Merged in Sage 3.0.alpha3



---

archive/issue_comments_016059.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-04-08T11:02:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2379",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2379#issuecomment-16059",
    "user": "mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_016060.json:
```json
{
    "body": "Attachment [trac_2379-fix-makefile.patch](tarball://root/attachments/some-uuid/ticket2379/trac_2379-fix-makefile.patch) by mabshoff created at 2008-04-08 13:45:31\n\nthis fixes the makefile so that \"make check\" and friends work",
    "created_at": "2008-04-08T13:45:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2379",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2379#issuecomment-16060",
    "user": "mabshoff"
}
```

Attachment [trac_2379-fix-makefile.patch](tarball://root/attachments/some-uuid/ticket2379/trac_2379-fix-makefile.patch) by mabshoff created at 2008-04-08 13:45:31

this fixes the makefile so that "make check" and friends work
