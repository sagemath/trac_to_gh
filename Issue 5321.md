# Issue 5321: [with patch, needs review] follow up to #5287

archive/issues_005321.json:
```json
{
    "body": "Assignee: malb\n\nCC:  was\n\nThe attached patch addresses some outstanding referee remarks from #5287.\n\nIssue created by migration from https://trac.sagemath.org/ticket/5321\n\n",
    "created_at": "2009-02-20T11:51:37Z",
    "labels": [
        "commutative algebra",
        "major",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.0.2",
    "title": "[with patch, needs review] follow up to #5287",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/5321",
    "user": "malb"
}
```
Assignee: malb

CC:  was

The attached patch addresses some outstanding referee remarks from #5287.

Issue created by migration from https://trac.sagemath.org/ticket/5321





---

archive/issue_comments_040972.json:
```json
{
    "body": "This seems to conflict with either #5493 or #5527. Please rebase post 3.4.1.alpha0.\n\nCheers,\n\nMichael",
    "created_at": "2009-03-20T20:56:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5321#issuecomment-40972",
    "user": "mabshoff"
}
```

This seems to conflict with either #5493 or #5527. Please rebase post 3.4.1.alpha0.

Cheers,

Michael



---

archive/issue_comments_040973.json:
```json
{
    "body": "Rebased to 3.4.2 + #5586",
    "created_at": "2009-05-12T02:00:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5321#issuecomment-40973",
    "user": "malb"
}
```

Rebased to 3.4.2 + #5586



---

archive/issue_comments_040974.json:
```json
{
    "body": "AFAICS, this already had a positive review from was and only needed to be rebased. In any case, the patch looks good to me and it applies cleanly to 4.0. Positive review once again.",
    "created_at": "2009-06-04T19:49:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5321#issuecomment-40974",
    "user": "burcin"
}
```

AFAICS, this already had a positive review from was and only needed to be rebased. In any case, the patch looks good to me and it applies cleanly to 4.0. Positive review once again.



---

archive/issue_comments_040975.json:
```json
{
    "body": "Attachment [sr_doctests.patch](tarball://root/attachments/some-uuid/ticket5321/sr_doctests.patch) by malb created at 2009-06-08 10:58:57\n\nThe updated patch _only_ changes ` \\var{foo} ` to ` ``foo`` `.",
    "created_at": "2009-06-08T10:58:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5321#issuecomment-40975",
    "user": "malb"
}
```

Attachment [sr_doctests.patch](tarball://root/attachments/some-uuid/ticket5321/sr_doctests.patch) by malb created at 2009-06-08 10:58:57

The updated patch _only_ changes ` \var{foo} ` to ` ``foo`` `.



---

archive/issue_comments_040976.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-06-13T19:35:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/5321",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/5321#issuecomment-40976",
    "user": "ncalexan"
}
```

Resolution: fixed
