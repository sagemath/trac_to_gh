# Issue 14460: Enable type1 fonts in matplotlib

archive/issues_014460.json:
```json
{
    "body": "Assignee: jason, was\n\nEssentially what the ticket title says. Some publications require all the fonts to be embedded in the document and require the use of type-1 fonts. It is not very hard to enable these fonts.\n\nIssue created by migration from https://trac.sagemath.org/ticket/14664\n\n",
    "created_at": "2013-05-30T12:41:41Z",
    "labels": [
        "graphics",
        "major",
        "enhancement"
    ],
    "title": "Enable type1 fonts in matplotlib",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/14460",
    "user": "ppurka"
}
```
Assignee: jason, was

Essentially what the ticket title says. Some publications require all the fonts to be embedded in the document and require the use of type-1 fonts. It is not very hard to enable these fonts.

Issue created by migration from https://trac.sagemath.org/ticket/14664





---

archive/issue_comments_182947.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2013-05-30T13:13:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14460#issuecomment-182947",
    "user": "ppurka"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_182948.json:
```json
{
    "body": "This isn't really about type 1 fonts, right? You are switching TeXing labels on/off. How about calling it `plot(latex=bool)`. \n\nIt would be nice if we could autodetect the prerequisites and default to the right thing.",
    "created_at": "2013-05-30T13:19:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14460#issuecomment-182948",
    "user": "vbraun"
}
```

This isn't really about type 1 fonts, right? You are switching TeXing labels on/off. How about calling it `plot(latex=bool)`. 

It would be nice if we could autodetect the prerequisites and default to the right thing.



---

archive/issue_comments_182949.json:
```json
{
    "body": "It *is* about type 1 fonts, as far as I can tell. The first thing publications tell is that type 3 fonts should not be present in the document. Some of them silently fail when you try to upload your document - the upload website just says that the upload didn't succeed.\n\nAs for references, I found these links earlier:\n* http://nerdjusttyped.blogspot.sg/2010/07/type-1-fonts-and-matplotlib-figures.html\n* http://matplotlib.1069221.n5.nabble.com/Type-1-font-in-figures-needed-td10294.html\n* http://brainlogging.wordpress.com/2011/11/16/type-1-vs-3-fonts/\n\n*About using `latex=bool`*:\nUnless someone knows that matplotlib has its own math renderer and doesn't use latex by default for rendering math, it will not be clear that something like `latex=True` will switch fonts to type 1 fonts.",
    "created_at": "2013-05-30T13:32:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14460#issuecomment-182949",
    "user": "ppurka"
}
```

It *is* about type 1 fonts, as far as I can tell. The first thing publications tell is that type 3 fonts should not be present in the document. Some of them silently fail when you try to upload your document - the upload website just says that the upload didn't succeed.

As for references, I found these links earlier:
* http://nerdjusttyped.blogspot.sg/2010/07/type-1-fonts-and-matplotlib-figures.html
* http://matplotlib.1069221.n5.nabble.com/Type-1-font-in-figures-needed-td10294.html
* http://brainlogging.wordpress.com/2011/11/16/type-1-vs-3-fonts/

*About using `latex=bool`*:
Unless someone knows that matplotlib has its own math renderer and doesn't use latex by default for rendering math, it will not be clear that something like `latex=True` will switch fonts to type 1 fonts.



---

archive/issue_comments_182950.json:
```json
{
    "body": "`rcParams['text.usetex'] = False` means that you disable TeX entirely, right? Maybe we want separate `plot(usetex=<bool>, type1_fonts=<bool>)`? Or `plot(tex=[None(default)|True|False|\"type1\"])`?",
    "created_at": "2013-05-30T13:35:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14460#issuecomment-182950",
    "user": "vbraun"
}
```

`rcParams['text.usetex'] = False` means that you disable TeX entirely, right? Maybe we want separate `plot(usetex=<bool>, type1_fonts=<bool>)`? Or `plot(tex=[None(default)|True|False|"type1"])`?



---

archive/issue_comments_182951.json:
```json
{
    "body": "`False` is the default. You can try the following in your sage installation\n\n```\nsage: from matplotlib import rcParams\nsage: rcParams['text.usetex']\nFalse\n```\n\nmatplotlib [does not require latex](http://matplotlib.org/users/mathtext.html#mathtext-tutorial) to render math fonts.",
    "created_at": "2013-05-30T13:40:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14460#issuecomment-182951",
    "user": "ppurka"
}
```

`False` is the default. You can try the following in your sage installation

```
sage: from matplotlib import rcParams
sage: rcParams['text.usetex']
False
```

matplotlib [does not require latex](http://matplotlib.org/users/mathtext.html#mathtext-tutorial) to render math fonts.



---

archive/issue_comments_182952.json:
```json
{
    "body": "What I'm saying is: Whether or not to use LaTeX is completely orthogonal to which fonts to use. But your patch links `type1_fonts` to both. You should be a able to use LaTeX without Type1 and the built-in renderer with Type1 fonts.",
    "created_at": "2013-05-30T13:53:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14460#issuecomment-182952",
    "user": "vbraun"
}
```

What I'm saying is: Whether or not to use LaTeX is completely orthogonal to which fonts to use. But your patch links `type1_fonts` to both. You should be a able to use LaTeX without Type1 and the built-in renderer with Type1 fonts.



---

archive/issue_comments_182953.json:
```json
{
    "body": "The following setting might be relevant:\n\n```\nsage: matplotlib.rcParams['pdf.fonttype']\n3\n```\n",
    "created_at": "2013-05-30T14:58:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14460#issuecomment-182953",
    "user": "vbraun"
}
```

The following setting might be relevant:

```
sage: matplotlib.rcParams['pdf.fonttype']
3
```




---

archive/issue_comments_182954.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2013-05-30T17:00:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14460#issuecomment-182954",
    "user": "ppurka"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_182955.json:
```json
{
    "body": "Replying to [comment:6 vbraun]:\n> What I'm saying is: Whether or not to use LaTeX is completely orthogonal to which fonts to use. But your patch links `type1_fonts` to both. You should be a able to use LaTeX without Type1 and the built-in renderer with Type1 fonts.\nI am not sure if the latter is possible.\n\nBut you are right about decoupling latex from type1. I will make that change.",
    "created_at": "2013-05-30T17:00:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14460#issuecomment-182955",
    "user": "ppurka"
}
```

Replying to [comment:6 vbraun]:
> What I'm saying is: Whether or not to use LaTeX is completely orthogonal to which fonts to use. But your patch links `type1_fonts` to both. You should be a able to use LaTeX without Type1 and the built-in renderer with Type1 fonts.
I am not sure if the latter is possible.

But you are right about decoupling latex from type1. I will make that change.



---

archive/issue_comments_182956.json:
```json
{
    "body": "Replying to [comment:7 vbraun]:\n> The following setting might be relevant:\n> {{{\n> sage: matplotlib.rcParams['pdf.fonttype']\n> 3\n> }}}\nFrom an initial reading, I believe this is something completely different and is only used with the internal matplotlib tex engine.",
    "created_at": "2013-05-30T17:05:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14460#issuecomment-182956",
    "user": "ppurka"
}
```

Replying to [comment:7 vbraun]:
> The following setting might be relevant:
> {{{
> sage: matplotlib.rcParams['pdf.fonttype']
> 3
> }}}
From an initial reading, I believe this is something completely different and is only used with the internal matplotlib tex engine.



---

archive/issue_comments_182957.json:
```json
{
    "body": "I changed the option to `font_style`. It take three strings now, \"type1\", \"latex\" and \"default\". In my limited testing just turning on \"latex\" by itself gives me embedded type 1 fonts, but maybe there are cases where this is not true.",
    "created_at": "2013-05-30T23:35:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14460#issuecomment-182957",
    "user": "ppurka"
}
```

I changed the option to `font_style`. It take three strings now, "type1", "latex" and "default". In my limited testing just turning on "latex" by itself gives me embedded type 1 fonts, but maybe there are cases where this is not true.



---

archive/issue_comments_182958.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2013-05-30T23:35:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14460#issuecomment-182958",
    "user": "ppurka"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_182959.json:
```json
{
    "body": "Font style means serif/sans serif. How about `typeset=\"latex\"`?",
    "created_at": "2013-05-31T09:33:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14460#issuecomment-182959",
    "user": "vbraun"
}
```

Font style means serif/sans serif. How about `typeset="latex"`?



---

archive/issue_comments_182960.json:
```json
{
    "body": "Thanks. I have changed it to `typeset`.",
    "created_at": "2013-05-31T10:39:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14460#issuecomment-182960",
    "user": "ppurka"
}
```

Thanks. I have changed it to `typeset`.



---

archive/issue_comments_182961.json:
```json
{
    "body": "I get fuzz 2 with sage-5.10.rc0",
    "created_at": "2013-05-31T12:18:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14460#issuecomment-182961",
    "user": "vbraun"
}
```

I get fuzz 2 with sage-5.10.rc0



---

archive/issue_comments_182962.json:
```json
{
    "body": "* can we get a ValueError if `typeset` is not one of the allowed values?\n* If you change matplotlib rcParams then you should switch it back when you are finished",
    "created_at": "2013-05-31T12:23:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14460#issuecomment-182962",
    "user": "vbraun"
}
```

* can we get a ValueError if `typeset` is not one of the allowed values?
* If you change matplotlib rcParams then you should switch it back when you are finished



---

archive/issue_comments_182963.json:
```json
{
    "body": "Replying to [comment:14 vbraun]:\n> * can we get a ValueError if `typeset` is not one of the allowed values?\n\nI will do that in a day or two. I need to compile 5.10rc0 first.\n\n> * If you change matplotlib rcParams then you should switch it back when you are finished\n\nNot resetting the rcParams actually affects only those plots which are a mixture of Sage and (direct) matplotlib calls. Currently, there is nothing in Sage that actually does this. I will make the change nevertheless because there might be someone doing this manually.",
    "created_at": "2013-05-31T17:46:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14460#issuecomment-182963",
    "user": "ppurka"
}
```

Replying to [comment:14 vbraun]:
> * can we get a ValueError if `typeset` is not one of the allowed values?

I will do that in a day or two. I need to compile 5.10rc0 first.

> * If you change matplotlib rcParams then you should switch it back when you are finished

Not resetting the rcParams actually affects only those plots which are a mixture of Sage and (direct) matplotlib calls. Currently, there is nothing in Sage that actually does this. I will make the change nevertheless because there might be someone doing this manually.



---

archive/issue_comments_182964.json:
```json
{
    "body": "Updated the patch. The \"default\" does not mess with the settings at all now.",
    "created_at": "2013-06-02T08:26:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14460#issuecomment-182964",
    "user": "ppurka"
}
```

Updated the patch. The "default" does not mess with the settings at all now.



---

archive/issue_comments_182965.json:
```json
{
    "body": "Looks good but unfortunately conflicts with #14640. The latter has already positive review, so you'll have to rebase your patch.",
    "created_at": "2013-06-02T12:39:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14460#issuecomment-182965",
    "user": "vbraun"
}
```

Looks good but unfortunately conflicts with #14640. The latter has already positive review, so you'll have to rebase your patch.



---

archive/issue_comments_182966.json:
```json
{
    "body": "Done. Thanks.",
    "created_at": "2013-06-02T13:02:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14460#issuecomment-182966",
    "user": "ppurka"
}
```

Done. Thanks.



---

archive/issue_comments_182967.json:
```json
{
    "body": "You are restoring `rc_backup` before saving it; The fuzz 2 ended up inserting a hunk at the wrong place.",
    "created_at": "2013-06-02T13:14:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14460#issuecomment-182967",
    "user": "vbraun"
}
```

You are restoring `rc_backup` before saving it; The fuzz 2 ended up inserting a hunk at the wrong place.



---

archive/issue_comments_182968.json:
```json
{
    "body": "So sorry. I should have checked the git diff. Thanks for catching this.",
    "created_at": "2013-06-02T13:18:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14460#issuecomment-182968",
    "user": "ppurka"
}
```

So sorry. I should have checked the git diff. Thanks for catching this.



---

archive/issue_comments_182969.json:
```json
{
    "body": "Apply to devel/sage",
    "created_at": "2013-06-02T13:26:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14460#issuecomment-182969",
    "user": "ppurka"
}
```

Apply to devel/sage



---

archive/issue_comments_182970.json:
```json
{
    "body": "Attachment [trac_14664-type1-fonts.patch](tarball://root/attachments/some-uuid/ticket14664/trac_14664-type1-fonts.patch) by ppurka created at 2013-06-02 13:26:49\n\nok. Now it should be done.",
    "created_at": "2013-06-02T13:26:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14460#issuecomment-182970",
    "user": "ppurka"
}
```

Attachment [trac_14664-type1-fonts.patch](tarball://root/attachments/some-uuid/ticket14664/trac_14664-type1-fonts.patch) by ppurka created at 2013-06-02 13:26:49

ok. Now it should be done.



---

archive/issue_comments_182971.json:
```json
{
    "body": "Looks good to me",
    "created_at": "2013-06-02T13:31:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14460#issuecomment-182971",
    "user": "vbraun"
}
```

Looks good to me



---

archive/issue_comments_182972.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2013-06-02T13:31:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14460#issuecomment-182972",
    "user": "vbraun"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_182973.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2013-06-06T12:34:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14460",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14460#issuecomment-182973",
    "user": "jdemeyer"
}
```

Resolution: fixed
