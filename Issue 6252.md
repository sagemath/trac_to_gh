# Issue 6252: Add 11 people to the development map

archive/issues_006252.json:
```json
{
    "body": "Assignee: tba\n\nCC:  schilly craigcitro ncalexan\n\nKeywords: development map, contributors.xml\n\nAdd the following people to the development map:\n1. David M\u00f8ller Hansen\n2. Fredrik Johansson\n3. Glenn Tarbox\n4. Lloyd Kilford\n5. Luis Berlioz\n6. Rob Beezer\n7. Paul Butler\n8. Ron Evans\n9. Ryan Dingman\n10. S\u00e9bastien Barth\u00e9lemy\n11. Tom Draper\n\nIssue created by migration from https://trac.sagemath.org/ticket/6252\n\n",
    "created_at": "2009-06-09T05:53:55Z",
    "labels": [
        "documentation",
        "major",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.0.2",
    "title": "Add 11 people to the development map",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/6252",
    "user": "mvngu"
}
```
Assignee: tba

CC:  schilly craigcitro ncalexan

Keywords: development map, contributors.xml

Add the following people to the development map:
1. David Møller Hansen
2. Fredrik Johansson
3. Glenn Tarbox
4. Lloyd Kilford
5. Luis Berlioz
6. Rob Beezer
7. Paul Butler
8. Ron Evans
9. Ryan Dingman
10. Sébastien Barthélemy
11. Tom Draper

Issue created by migration from https://trac.sagemath.org/ticket/6252





---

archive/issue_comments_049936.json:
```json
{
    "body": "Patch is against the current contributors list at\n\n\nhttp://www.sagemath.org/res/contributors.xml",
    "created_at": "2009-06-09T05:57:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6252#issuecomment-49936",
    "user": "mvngu"
}
```

Patch is against the current contributors list at


http://www.sagemath.org/res/contributors.xml



---

archive/issue_comments_049937.json:
```json
{
    "body": "Changing component from documentation to website/wiki.",
    "created_at": "2009-06-09T18:20:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6252#issuecomment-49937",
    "user": "mvngu"
}
```

Changing component from documentation to website/wiki.



---

archive/issue_comments_049938.json:
```json
{
    "body": "Changing assignee from tba to schilly.",
    "created_at": "2009-06-09T18:20:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6252#issuecomment-49938",
    "user": "mvngu"
}
```

Changing assignee from tba to schilly.



---

archive/issue_comments_049939.json:
```json
{
    "body": "Attachment [contributors.xml](tarball://root/attachments/some-uuid/ticket6252/contributors.xml) by mvngu created at 2009-06-09 20:07:04",
    "created_at": "2009-06-09T20:07:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6252#issuecomment-49939",
    "user": "mvngu"
}
```

Attachment [contributors.xml](tarball://root/attachments/some-uuid/ticket6252/contributors.xml) by mvngu created at 2009-06-09 20:07:04



---

archive/issue_comments_049940.json:
```json
{
    "body": "Looks good to me.",
    "created_at": "2009-06-09T20:38:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6252#issuecomment-49940",
    "user": "mhansen"
}
```

Looks good to me.



---

archive/issue_comments_049941.json:
```json
{
    "body": "I think this can be closed now. There's no need to apply the patch to the Sage source distro, nor to incorporate the XML file. This is for the Sage website, so only apply the patch to sagemath on boxen. I've applied the patch and updated the website. The updated page should be at\n\n\n\nhttp://www.sagemath.org/development-map.html\n\n\n\nwhere one can search for the names of the above new contributors.",
    "created_at": "2009-06-09T22:41:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6252#issuecomment-49941",
    "user": "mvngu"
}
```

I think this can be closed now. There's no need to apply the patch to the Sage source distro, nor to incorporate the XML file. This is for the Sage website, so only apply the patch to sagemath on boxen. I've applied the patch and updated the website. The updated page should be at



http://www.sagemath.org/development-map.html



where one can search for the names of the above new contributors.



---

archive/issue_comments_049942.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-06-14T22:27:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6252#issuecomment-49942",
    "user": "craigcitro"
}
```

Resolution: fixed
