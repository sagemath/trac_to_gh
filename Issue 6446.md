# Issue 6446: memory issue from #6394

archive/issues_006446.json:
```json
{
    "body": "Assignee: tbd\n\n\n\nIssue created by migration from https://trac.sagemath.org/ticket/6446\n\n",
    "created_at": "2009-06-29T17:29:46Z",
    "labels": [
        "component: memleak",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.1",
    "title": "memory issue from #6394",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/6446",
    "user": "https://github.com/rlmill"
}
```
Assignee: tbd



Issue created by migration from https://trac.sagemath.org/ticket/6446





---

archive/issue_comments_051699.json:
```json
{
    "body": "Attachment [trac_6446-memleak_coarsest_eq_refn.patch](tarball://root/attachments/some-uuid/ticket6446/trac_6446-memleak_coarsest_eq_refn.patch) by @rlmill created at 2009-06-29 17:38:40",
    "created_at": "2009-06-29T17:38:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6446",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6446#issuecomment-51699",
    "user": "https://github.com/rlmill"
}
```

Attachment [trac_6446-memleak_coarsest_eq_refn.patch](tarball://root/attachments/some-uuid/ticket6446/trac_6446-memleak_coarsest_eq_refn.patch) by @rlmill created at 2009-06-29 17:38:40



---

archive/issue_comments_051700.json:
```json
{
    "body": "I'm testing this now....",
    "created_at": "2009-06-29T18:16:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6446",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6446#issuecomment-51700",
    "user": "https://github.com/JohnCremona"
}
```

I'm testing this now....



---

archive/issue_comments_051701.json:
```json
{
    "body": "Testing on the same machine & build where I reported the problem (ububtu 32-bit):  I applied the patch and now there is no problem with testing graphs/graph.py, or sage/groups/perm_gps/partn_ref/refinement_graphs.pyx (the file patched).\n\nSo: positive reivew!",
    "created_at": "2009-06-29T18:23:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6446",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6446#issuecomment-51701",
    "user": "https://github.com/JohnCremona"
}
```

Testing on the same machine & build where I reported the problem (ububtu 32-bit):  I applied the patch and now there is no problem with testing graphs/graph.py, or sage/groups/perm_gps/partn_ref/refinement_graphs.pyx (the file patched).

So: positive reivew!



---

archive/issue_comments_051702.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-06-29T20:43:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/6446",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/6446#issuecomment-51702",
    "user": "https://github.com/rlmill"
}
```

Resolution: fixed
