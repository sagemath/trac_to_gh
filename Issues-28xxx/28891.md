# Issue 28891: character art and sympy-conversion of symbolic series

archive/issues_028654.json:
```json
{
    "assignees": [],
    "body": "This ticket fixes the broken ascii and unicode art of symbolic series by implementing the appropriate conversions to Sympy (which are called by the generic character art methods of symbolic expressions).\n\nBefore:\n\n```\nsage: unicode_art(exp(x).series(x, 4))\n(1, +, 1*x, +, 1/2*x\u00b2, +, 1/6*x\u00b3, +, Order(x\u2074\u207e)\nsage: unicode_art(exp(x).series(x==1, 3))\n((e), +, (e)*(x, -, 1), +, (1/2*e)*(x, -, 1)\u00b2, +, Order((x, -, 1)\u00b3\u207e)\n```\n\nAfter:\n\n```\nsage: unicode_art(exp(x).series(x, 4))\n         2    3\n        x    x     \u239b 4\u239e\n1 + x + \u2500\u2500 + \u2500\u2500 + O\u239dx \u23a0\n        2    6\nsage: unicode_art(exp(x).series(x==1, 3))\n                         2\n                \u212f\u22c5(x - 1)     \u239b       3       \u239e\n\u212f + \u212f\u22c5(x - 1) + \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500 + O\u239d(x - 1) ; x \u2192 1\u23a0\n                    2\n```\n\nExtra care is taken to allow series expansions at non-zero positions as above, as Sympy's function `O` takes slightly different arguments than Sage's `Order`.\n\n\nKeywords: **sympy**\n\nBranch/Commit: **[8bdd72d](https://github.com/sagemath/sagetrac-mirror/commit/8bdd72df1d6e8e0ce807da2deca8239e657bc559)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Markus Wageringel**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28891_\n\n",
    "closed_at": "2019-12-17T22:46:17Z",
    "created_at": "2019-12-16T17:42:04Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "character art and sympy-conversion of symbolic series",
    "type": "issue",
    "updated_at": "2019-12-17T22:46:17Z",
    "url": "https://github.com/sagemath/sage/issues/28891",
    "user": "https://github.com/mwageringel"
}
```
This ticket fixes the broken ascii and unicode art of symbolic series by implementing the appropriate conversions to Sympy (which are called by the generic character art methods of symbolic expressions).

Before:

```
sage: unicode_art(exp(x).series(x, 4))
(1, +, 1*x, +, 1/2*x², +, 1/6*x³, +, Order(x⁴⁾)
sage: unicode_art(exp(x).series(x==1, 3))
((e), +, (e)*(x, -, 1), +, (1/2*e)*(x, -, 1)², +, Order((x, -, 1)³⁾)
```

After:

```
sage: unicode_art(exp(x).series(x, 4))
         2    3
        x    x     ⎛ 4⎞
1 + x + ── + ── + O⎝x ⎠
        2    6
sage: unicode_art(exp(x).series(x==1, 3))
                         2
                ℯ⋅(x - 1)     ⎛       3       ⎞
ℯ + ℯ⋅(x - 1) + ────────── + O⎝(x - 1) ; x → 1⎠
                    2
```

Extra care is taken to allow series expansions at non-zero positions as above, as Sympy's function `O` takes slightly different arguments than Sage's `Order`.


Keywords: **sympy**

Branch/Commit: **[8bdd72d](https://github.com/sagemath/sagetrac-mirror/commit/8bdd72df1d6e8e0ce807da2deca8239e657bc559)**

Reviewer: **Frédéric Chapoton**

Author: **Markus Wageringel**

_Issue created by migration from https://trac.sagemath.org/ticket/28891_





---

archive/issue_events_365523.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2019-12-16T17:42:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28891",
    "milestone_number": null,
    "milestone_title": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28891#event-365523"
}
```



---

archive/issue_events_365524.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2019-12-16T17:42:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28891",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28891#event-365524"
}
```



---

archive/issue_events_365525.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2019-12-16T17:42:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28891",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28891#event-365525"
}
```



---

archive/issue_events_365526.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2019-12-16T17:42:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28891",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28891#event-365526"
}
```



---

archive/issue_comments_453948.json:
```json
{
    "body": "Commit: **[7818ba9](https://github.com/sagemath/sagetrac-mirror/commit/7818ba9b652570eff8b4254a6b36460f3efbc8c7)**",
    "created_at": "2019-12-16T17:57:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28891",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28891#issuecomment-453948",
    "user": "https://github.com/mwageringel"
}
```

Commit: **[7818ba9](https://github.com/sagemath/sagetrac-mirror/commit/7818ba9b652570eff8b4254a6b36460f3efbc8c7)**



---

archive/issue_comments_453949.json:
```json
{
    "body": "Author: **Markus Wageringel**",
    "created_at": "2019-12-16T17:57:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28891",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28891#issuecomment-453949",
    "user": "https://github.com/mwageringel"
}
```

Author: **Markus Wageringel**



---

archive/issue_events_365527.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2019-12-16T17:57:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28891",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28891#event-365527"
}
```



---

archive/issue_comments_453950.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7818ba9b652570eff8b4254a6b36460f3efbc8c7\">7818ba9</a></td><td><code>28891: character art/sympy-conversion of symbolic series</code></td></tr></table>\n",
    "created_at": "2019-12-16T17:57:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28891",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28891#issuecomment-453950",
    "user": "https://github.com/mwageringel"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7818ba9b652570eff8b4254a6b36460f3efbc8c7">7818ba9</a></td><td><code>28891: character art/sympy-conversion of symbolic series</code></td></tr></table>




---

archive/issue_comments_453951.json:
```json
{
    "body": "Branch: **[u/gh-mwageringel/28891](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/28891)**",
    "created_at": "2019-12-16T17:57:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28891",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28891#issuecomment-453951",
    "user": "https://github.com/mwageringel"
}
```

Branch: **[u/gh-mwageringel/28891](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/28891)**



---

archive/issue_comments_453952.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/90a595f8f3dc17f0e89dfd084755010112ea4043\">90a595f</a></td><td><code>28891: character art/sympy-conversion of symbolic series</code></td></tr></table>\n",
    "created_at": "2019-12-16T19:35:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28891",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28891#issuecomment-453952",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/90a595f8f3dc17f0e89dfd084755010112ea4043">90a595f</a></td><td><code>28891: character art/sympy-conversion of symbolic series</code></td></tr></table>




---

archive/issue_comments_453953.json:
```json
{
    "body": "Changed commit from **[7818ba9](https://github.com/sagemath/sagetrac-mirror/commit/7818ba9b652570eff8b4254a6b36460f3efbc8c7)** to **[90a595f](https://github.com/sagemath/sagetrac-mirror/commit/90a595f8f3dc17f0e89dfd084755010112ea4043)**",
    "created_at": "2019-12-16T19:35:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28891",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28891#issuecomment-453953",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[7818ba9](https://github.com/sagemath/sagetrac-mirror/commit/7818ba9b652570eff8b4254a6b36460f3efbc8c7)** to **[90a595f](https://github.com/sagemath/sagetrac-mirror/commit/90a595f8f3dc17f0e89dfd084755010112ea4043)**



---

archive/issue_comments_453954.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8bdd72df1d6e8e0ce807da2deca8239e657bc559\">8bdd72d</a></td><td><code>28891: character art/sympy-conversion of symbolic series</code></td></tr></table>\n",
    "created_at": "2019-12-16T19:40:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28891",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28891#issuecomment-453954",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:3'>Comment 3:</a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8bdd72df1d6e8e0ce807da2deca8239e657bc559">8bdd72d</a></td><td><code>28891: character art/sympy-conversion of symbolic series</code></td></tr></table>




---

archive/issue_comments_453955.json:
```json
{
    "body": "Changed commit from **[90a595f](https://github.com/sagemath/sagetrac-mirror/commit/90a595f8f3dc17f0e89dfd084755010112ea4043)** to **[8bdd72d](https://github.com/sagemath/sagetrac-mirror/commit/8bdd72df1d6e8e0ce807da2deca8239e657bc559)**",
    "created_at": "2019-12-16T19:40:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28891",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28891#issuecomment-453955",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[90a595f](https://github.com/sagemath/sagetrac-mirror/commit/90a595f8f3dc17f0e89dfd084755010112ea4043)** to **[8bdd72d](https://github.com/sagemath/sagetrac-mirror/commit/8bdd72df1d6e8e0ce807da2deca8239e657bc559)**



---

archive/issue_events_365528.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-12-16T21:21:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28891",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28891#event-365528"
}
```



---

archive/issue_events_365529.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-12-16T21:21:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28891",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28891#event-365529"
}
```



---

archive/issue_comments_453956.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2019-12-16T21:21:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28891",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28891#issuecomment-453956",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_453957.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nok, thx",
    "created_at": "2019-12-16T21:21:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28891",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28891#issuecomment-453957",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:4'>Comment 4:</a>
ok, thx



---

archive/issue_comments_453958.json:
```json
{
    "body": "Changed branch from **[u/gh-mwageringel/28891](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/28891)** to **[8bdd72d](https://github.com/sagemath/sagetrac-mirror/commit/8bdd72df1d6e8e0ce807da2deca8239e657bc559)**",
    "created_at": "2019-12-17T22:46:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28891",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28891#issuecomment-453958",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/gh-mwageringel/28891](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/28891)** to **[8bdd72d](https://github.com/sagemath/sagetrac-mirror/commit/8bdd72df1d6e8e0ce807da2deca8239e657bc559)**



---

archive/issue_events_365530.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-12-17T22:46:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28891",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28891#event-365530"
}
```



---

archive/issue_events_365531.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "c391f3c70953ec296a665b25520e084f342c740a",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-12-17T22:46:17Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28891",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28891#event-365531"
}
```
