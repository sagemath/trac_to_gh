# Issue 28932: fix QuadraticField constructor if embedding=False

archive/issues_028695.json:
```json
{
    "assignees": [],
    "body": "The `QuadraticField` constructor takes a boolean argument `embedding`, according to its documentation:\n\n```\n    -  ``embedding`` - bool or square root of D in an\n       ambient field (default: True)\n```\n\nbut `embedding=False` raises an error, which is fixed by this ticket:\n\n```\nsage: QuadraticField(3, embedding=False)\n...\nAttributeError: 'bool' object has no attribute 'parent'\n```\n\n\nBranch/Commit: **[b250485](https://github.com/sagemath/sagetrac-mirror/commit/b2504856da3fc526892a2231c22304c7cdfd008f)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Markus Wageringel**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28932_\n\n",
    "closed_at": "2020-01-11T17:44:57Z",
    "created_at": "2020-01-01T11:46:05Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix QuadraticField constructor if embedding=False",
    "type": "issue",
    "updated_at": "2020-01-11T17:44:57Z",
    "url": "https://github.com/sagemath/sage/issues/28932",
    "user": "https://github.com/mwageringel"
}
```
The `QuadraticField` constructor takes a boolean argument `embedding`, according to its documentation:

```
    -  ``embedding`` - bool or square root of D in an
       ambient field (default: True)
```

but `embedding=False` raises an error, which is fixed by this ticket:

```
sage: QuadraticField(3, embedding=False)
...
AttributeError: 'bool' object has no attribute 'parent'
```


Branch/Commit: **[b250485](https://github.com/sagemath/sagetrac-mirror/commit/b2504856da3fc526892a2231c22304c7cdfd008f)**

Reviewer: **Travis Scrimshaw**

Author: **Markus Wageringel**

_Issue created by migration from https://trac.sagemath.org/ticket/28932_





---

archive/issue_events_379286.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-01-01T11:46:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28932",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28932#event-379286"
}
```



---

archive/issue_events_379287.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-01-01T11:46:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28932",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
    "label_color": "0000ff",
    "label_name": "component: number fields",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28932#event-379287"
}
```



---

archive/issue_events_379288.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-01-01T11:46:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28932",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28932#event-379288"
}
```



---

archive/issue_events_379289.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-01-01T11:46:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28932",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28932#event-379289"
}
```



---

archive/issue_comments_454845.json:
```json
{
    "body": "Commit: **[b250485](https://github.com/sagemath/sagetrac-mirror/commit/b2504856da3fc526892a2231c22304c7cdfd008f)**",
    "created_at": "2020-01-01T11:50:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28932#issuecomment-454845",
    "user": "https://github.com/mwageringel"
}
```

Commit: **[b250485](https://github.com/sagemath/sagetrac-mirror/commit/b2504856da3fc526892a2231c22304c7cdfd008f)**



---

archive/issue_comments_454846.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b2504856da3fc526892a2231c22304c7cdfd008f\">b250485</a></td><td><code>28932: fix QuadraticField when embedding=False</code></td></tr></table>\n",
    "created_at": "2020-01-01T11:50:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28932#issuecomment-454846",
    "user": "https://github.com/mwageringel"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b2504856da3fc526892a2231c22304c7cdfd008f">b250485</a></td><td><code>28932: fix QuadraticField when embedding=False</code></td></tr></table>




---

archive/issue_comments_454847.json:
```json
{
    "body": "Author: **Markus Wageringel**",
    "created_at": "2020-01-01T11:50:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28932#issuecomment-454847",
    "user": "https://github.com/mwageringel"
}
```

Author: **Markus Wageringel**



---

archive/issue_events_379290.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-01-01T11:50:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28932",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28932#event-379290"
}
```



---

archive/issue_comments_454848.json:
```json
{
    "body": "Branch: **[u/gh-mwageringel/28932](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/28932)**",
    "created_at": "2020-01-01T11:50:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28932#issuecomment-454848",
    "user": "https://github.com/mwageringel"
}
```

Branch: **[u/gh-mwageringel/28932](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/28932)**



---

archive/issue_comments_454849.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2020-01-04T21:10:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28932#issuecomment-454849",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_454850.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nLGTM.",
    "created_at": "2020-01-04T21:10:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28932#issuecomment-454850",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:2" align="right">Comment 2</div>

LGTM.



---

archive/issue_events_379291.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-01-04T21:10:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28932",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28932#event-379291"
}
```



---

archive/issue_events_379292.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-01-04T21:10:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28932",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28932#event-379292"
}
```



---

archive/issue_events_379293.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-01-11T17:44:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28932",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28932#event-379293"
}
```



---

archive/issue_events_379294.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "582712b85de7329dce47770eda0a6043cd8ee2dc",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-01-11T17:44:57Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28932",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28932#event-379294"
}
```



---

archive/issue_comments_454851.json:
```json
{
    "body": "Changed branch from **[u/gh-mwageringel/28932](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/28932)** to **[b250485](https://github.com/sagemath/sagetrac-mirror/commit/b2504856da3fc526892a2231c22304c7cdfd008f)**",
    "created_at": "2020-01-11T17:44:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28932#issuecomment-454851",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/gh-mwageringel/28932](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/28932)** to **[b250485](https://github.com/sagemath/sagetrac-mirror/commit/b2504856da3fc526892a2231c22304c7cdfd008f)**
