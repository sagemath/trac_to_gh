# Issue 28210: py3 perfect_matching

archive/issues_027973.json:
```json
{
    "assignees": [],
    "body": "Fix the py3 doctest failures in `combinat/perfect_matching.py`.\n\n\nBranch/Commit: **[363729d](https://github.com/sagemath/sagetrac-mirror/commit/363729d10da5225f0f580afcf49e26fb84bf80b7)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **John Palmieri**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28210_\n\n",
    "closed_at": "2019-07-23T21:03:59Z",
    "created_at": "2019-07-17T20:20:30Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20python3",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "py3 perfect_matching",
    "type": "issue",
    "updated_at": "2019-07-23T21:03:59Z",
    "url": "https://github.com/sagemath/sage/issues/28210",
    "user": "https://github.com/jhpalmieri"
}
```
Fix the py3 doctest failures in `combinat/perfect_matching.py`.


Branch/Commit: **[363729d](https://github.com/sagemath/sagetrac-mirror/commit/363729d10da5225f0f580afcf49e26fb84bf80b7)**

Reviewer: **Frédéric Chapoton**

Author: **John Palmieri**

_Issue created by migration from https://trac.sagemath.org/ticket/28210_





---

archive/issue_events_368688.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2019-07-17T20:20:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28210",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28210#event-368688"
}
```



---

archive/issue_events_368689.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2019-07-17T20:20:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28210",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20python3",
    "label_color": "000080",
    "label_name": "component: python3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28210#event-368689"
}
```



---

archive/issue_events_368690.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2019-07-17T20:20:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28210",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28210#event-368690"
}
```



---

archive/issue_events_368691.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2019-07-17T20:20:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28210",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28210#event-368691"
}
```



---

archive/issue_comments_442290.json:
```json
{
    "body": "Branch: **[u/jhpalmieri/perfect_matching](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/perfect_matching)**",
    "created_at": "2019-07-17T20:21:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28210",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28210#issuecomment-442290",
    "user": "https://github.com/jhpalmieri"
}
```

Branch: **[u/jhpalmieri/perfect_matching](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/perfect_matching)**



---

archive/issue_comments_442291.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nI marked some doctests `random`, and then I added extra tests to make sure the output was still mathematically correct. This may be a bit of overkill, but at least the newly added stuff has all gone into `TESTS` blocks, so it should be hidden from casual users.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/363729d10da5225f0f580afcf49e26fb84bf80b7\">363729d</a></td><td><code>trac 28210: fix the three py3 doctest failures in combinat/perfect_matching.py</code></td></tr></table>\n",
    "created_at": "2019-07-17T20:23:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28210",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28210#issuecomment-442291",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:2" align="right">Comment 2</div>

I marked some doctests `random`, and then I added extra tests to make sure the output was still mathematically correct. This may be a bit of overkill, but at least the newly added stuff has all gone into `TESTS` blocks, so it should be hidden from casual users.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/363729d10da5225f0f580afcf49e26fb84bf80b7">363729d</a></td><td><code>trac 28210: fix the three py3 doctest failures in combinat/perfect_matching.py</code></td></tr></table>




---

archive/issue_events_368692.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2019-07-17T20:23:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28210",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28210#event-368692"
}
```



---

archive/issue_comments_442292.json:
```json
{
    "body": "Commit: **[363729d](https://github.com/sagemath/sagetrac-mirror/commit/363729d10da5225f0f580afcf49e26fb84bf80b7)**",
    "created_at": "2019-07-17T20:23:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28210",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28210#issuecomment-442292",
    "user": "https://github.com/jhpalmieri"
}
```

Commit: **[363729d](https://github.com/sagemath/sagetrac-mirror/commit/363729d10da5225f0f580afcf49e26fb84bf80b7)**



---

archive/issue_events_368693.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-07-18T07:15:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28210",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28210#event-368693"
}
```



---

archive/issue_events_368694.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-07-18T07:15:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28210",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28210#event-368694"
}
```



---

archive/issue_comments_442293.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2019-07-18T07:15:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28210",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28210#issuecomment-442293",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_442294.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nok, thx",
    "created_at": "2019-07-18T07:15:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28210",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28210#issuecomment-442294",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">Comment 3</div>

ok, thx



---

archive/issue_comments_442295.json:
```json
{
    "body": "Changed branch from **[u/jhpalmieri/perfect_matching](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/perfect_matching)** to **[363729d](https://github.com/sagemath/sagetrac-mirror/commit/363729d10da5225f0f580afcf49e26fb84bf80b7)**",
    "created_at": "2019-07-23T21:03:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28210",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28210#issuecomment-442295",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jhpalmieri/perfect_matching](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/perfect_matching)** to **[363729d](https://github.com/sagemath/sagetrac-mirror/commit/363729d10da5225f0f580afcf49e26fb84bf80b7)**



---

archive/issue_events_368695.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-07-23T21:03:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28210",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28210#event-368695"
}
```



---

archive/issue_events_368696.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "ed5f50991ec5738f8b6f99fb6ff9c20b81507cdf",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-07-23T21:03:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28210",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28210#event-368696"
}
```
