# Issue 28596: Fix jmol permissions

archive/issues_028359.json:
```json
{
    "body": "This is a followup to #25026: the permissions of `SAGE_ROOT/local/share/jmol/` are wrong:\n\n```\n  drwxr-xr-x 24 palmieri staff  768 Sep 12 00:11 info\n  drwx------ 21 palmieri staff  672 Oct 12 10:28 jmol\n  drwxr-xr-x 80 palmieri staff 2560 Oct 12 10:28 jsmol\n  drwxr-xr-x  4 palmieri staff  128 Sep 12 00:11 jupyter\n```\n\n\nCC:  @jdemeyer @embray @novoselt\n\nBranch/Commit: 87c694f059e839a4bb12db80037f3c4398b56c2b\n\nReviewer: John Palmieri\n\nAuthor: Erik Bray\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/28596\n\n",
    "closed_at": "2019-12-20T22:46:04Z",
    "created_at": "2019-10-12T17:13:45Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.0",
    "title": "Fix jmol permissions",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/28596",
    "user": "https://github.com/jhpalmieri"
}
```
This is a followup to #25026: the permissions of `SAGE_ROOT/local/share/jmol/` are wrong:

```
  drwxr-xr-x 24 palmieri staff  768 Sep 12 00:11 info
  drwx------ 21 palmieri staff  672 Oct 12 10:28 jmol
  drwxr-xr-x 80 palmieri staff 2560 Oct 12 10:28 jsmol
  drwxr-xr-x  4 palmieri staff  128 Sep 12 00:11 jupyter
```


CC:  @jdemeyer @embray @novoselt

Branch/Commit: 87c694f059e839a4bb12db80037f3c4398b56c2b

Reviewer: John Palmieri

Author: Erik Bray

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/28596





---

archive/issue_comments_554923.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,9 @@\n-This is a followup to #25026: the permissions of `SAGE_ROOT/local/share/jmol/` are wrong.\n+This is a followup to #25026: the permissions of `SAGE_ROOT/local/share/jmol/` are wrong:\n+\n+```\n+  drwxr-xr-x 24 palmieri staff  768 Sep 12 00:11 info\n+  drwx------ 21 palmieri staff  672 Oct 12 10:28 jmol\n+  drwxr-xr-x 80 palmieri staff 2560 Oct 12 10:28 jsmol\n+  drwxr-xr-x  4 palmieri staff  128 Sep 12 00:11 jupyter\n+```\n+\n``````\n",
    "created_at": "2019-10-12T17:29:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28596#issuecomment-554923",
    "user": "https://github.com/jhpalmieri"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,9 @@
-This is a followup to #25026: the permissions of `SAGE_ROOT/local/share/jmol/` are wrong.
+This is a followup to #25026: the permissions of `SAGE_ROOT/local/share/jmol/` are wrong:
+
+```
+  drwxr-xr-x 24 palmieri staff  768 Sep 12 00:11 info
+  drwx------ 21 palmieri staff  672 Oct 12 10:28 jmol
+  drwxr-xr-x 80 palmieri staff 2560 Oct 12 10:28 jsmol
+  drwxr-xr-x  4 palmieri staff  128 Sep 12 00:11 jupyter
+```
+
``````




---

archive/issue_comments_554924.json:
```json
{
    "body": "<a id='comment:2'></a>\nI can see two solutions: change jmol's spkg-install script so it sets the right permissions, or set the permissions differently for all packages. Here is a branch which does the second of these: after unpacking the tarball into `src`, run `chmod +rx src`. Is this a bad idea for some reason?",
    "created_at": "2019-10-12T17:31:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28596#issuecomment-554924",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:2'></a>
I can see two solutions: change jmol's spkg-install script so it sets the right permissions, or set the permissions differently for all packages. Here is a branch which does the second of these: after unpacking the tarball into `src`, run `chmod +rx src`. Is this a bad idea for some reason?



---

archive/issue_comments_554925.json:
```json
{
    "body": "Changing branch from \"\" to \"u/jhpalmieri/jmol-permissions\"",
    "created_at": "2019-10-12T17:32:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28596#issuecomment-554925",
    "user": "https://github.com/jhpalmieri"
}
```

Changing branch from "" to "u/jhpalmieri/jmol-permissions"



---

archive/issue_comments_554926.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2019-10-12T17:32:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28596#issuecomment-554926",
    "user": "https://github.com/jhpalmieri"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_554927.json:
```json
{
    "body": "Changing commit from \"\" to \"82e736b7c1f5f93042d61d34149150ab7d9cec2b\"",
    "created_at": "2019-10-12T17:32:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28596#issuecomment-554927",
    "user": "https://github.com/jhpalmieri"
}
```

Changing commit from "" to "82e736b7c1f5f93042d61d34149150ab7d9cec2b"



---

archive/issue_comments_554928.json:
```json
{
    "body": "<a id='comment:4'></a>\nNew commits:\n|                                                                                                                                          |                                                                               |\n|------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------|\n|[82e736b](https://git.sagemath.org/sage.git/commit?id=82e736b7c1f5f93042d61d34149150ab7d9cec2b)|`trac 28596: change permissions on src after unpacking each package's tarball.`|",
    "created_at": "2019-10-12T17:32:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28596#issuecomment-554928",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:4'></a>
New commits:
|                                                                                                                                          |                                                                               |
|------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------|
|[82e736b](https://git.sagemath.org/sage.git/commit?id=82e736b7c1f5f93042d61d34149150ab7d9cec2b)|`trac 28596: change permissions on src after unpacking each package's tarball.`|



---

archive/issue_comments_554929.json:
```json
{
    "body": "<a id='comment:5'></a>\nI want to do this for all packages to avoid similar situations in the future.",
    "created_at": "2019-10-12T17:34:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28596#issuecomment-554929",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:5'></a>
I want to do this for all packages to avoid similar situations in the future.



---

archive/issue_comments_554930.json:
```json
{
    "body": "<a id='comment:6'></a>\nThis does not seem to fix the doc building issue.",
    "created_at": "2019-10-12T18:59:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28596#issuecomment-554930",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:6'></a>
This does not seem to fix the doc building issue.



---

archive/issue_comments_554931.json:
```json
{
    "body": "<a id='comment:7'></a>\nReplying to [chapoton](#comment%3A6):\n> This does not seem to fix the doc building issue.\n\n\nI don't know what doc building issue you're referencing.",
    "created_at": "2019-12-16T11:16:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28596#issuecomment-554931",
    "user": "https://github.com/embray"
}
```

<a id='comment:7'></a>
Replying to [chapoton](#comment%3A6):
> This does not seem to fix the doc building issue.


I don't know what doc building issue you're referencing.



---

archive/issue_comments_554932.json:
```json
{
    "body": "<a id='comment:8'></a>\nHere is the docbuild log where the problem appears:\n\nhttps://patchbot.sagemath.org/log/0/Ubuntu/16.04/x86_64/4.4.0-166-generic/atlas/2019-11-27%2010:02:21?plugin=docbuild\n\nThis prevents me from using this machine as a patchbot.",
    "created_at": "2019-12-16T13:31:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28596#issuecomment-554932",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:8'></a>
Here is the docbuild log where the problem appears:

https://patchbot.sagemath.org/log/0/Ubuntu/16.04/x86_64/4.4.0-166-generic/atlas/2019-11-27%2010:02:21?plugin=docbuild

This prevents me from using this machine as a patchbot.



---

archive/issue_comments_554933.json:
```json
{
    "body": "<a id='comment:9'></a>\nThis behavior originates from #24567, and would be different if we'd gone with Jeroen's suggestion of a less restricted mask on the files than I used.\n\nI wrote at the time:\n\n> this should have no effect on the permissions applied when the files are installed, but I guess it depends in part on the installation mechanism\n\n\nWhich, incidentally was true at the time.  But the part of \"depends on the installation mechanism\" was also true, and the installation mechanism *did* change after that, in such a way that this did have an effect on the installed files.\n\nSo I think the better option here might be to try Jeroen's original suggestion of a less restrictive permission mask on the source tarball's extraction location.",
    "created_at": "2019-12-16T15:31:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28596#issuecomment-554933",
    "user": "https://github.com/embray"
}
```

<a id='comment:9'></a>
This behavior originates from #24567, and would be different if we'd gone with Jeroen's suggestion of a less restricted mask on the files than I used.

I wrote at the time:

> this should have no effect on the permissions applied when the files are installed, but I guess it depends in part on the installation mechanism


Which, incidentally was true at the time.  But the part of "depends on the installation mechanism" was also true, and the installation mechanism *did* change after that, in such a way that this did have an effect on the installed files.

So I think the better option here might be to try Jeroen's original suggestion of a less restrictive permission mask on the source tarball's extraction location.



---

archive/issue_comments_554934.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2019-12-16T15:31:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28596#issuecomment-554934",
    "user": "https://github.com/embray"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_554935.json:
```json
{
    "body": "Changing author from \"\" to \"Erik Bray\"",
    "created_at": "2019-12-16T16:04:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28596#issuecomment-554935",
    "user": "https://github.com/embray"
}
```

Changing author from "" to "Erik Bray"



---

archive/issue_comments_554936.json:
```json
{
    "body": "<a id='comment:10'></a>\nThis approach fixes the issue closer to the source.  However, it would be good to test a full rebuild.  I don't know for sure if there aren't some pathological packages that can break because of this (and if so those packages should probably be fixed...).\n\n---\nNew commits:\n|                                                                                                                                          |                                                                        |\n|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------|\n|[df58d4b](https://git.sagemath.org/sage.git/commit?id=df58d4ba3705aedf929c355afe7347714afff89b)|`Trac #28596: Apply less restricted permission mask to extracted source`|\n|[87c694f](https://git.sagemath.org/sage.git/commit?id=87c694f059e839a4bb12db80037f3c4398b56c2b)|`Add an spkg-legacy-uninstall for jmol.`|",
    "created_at": "2019-12-16T16:04:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28596#issuecomment-554936",
    "user": "https://github.com/embray"
}
```

<a id='comment:10'></a>
This approach fixes the issue closer to the source.  However, it would be good to test a full rebuild.  I don't know for sure if there aren't some pathological packages that can break because of this (and if so those packages should probably be fixed...).

---
New commits:
|                                                                                                                                          |                                                                        |
|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------|
|[df58d4b](https://git.sagemath.org/sage.git/commit?id=df58d4ba3705aedf929c355afe7347714afff89b)|`Trac #28596: Apply less restricted permission mask to extracted source`|
|[87c694f](https://git.sagemath.org/sage.git/commit?id=87c694f059e839a4bb12db80037f3c4398b56c2b)|`Add an spkg-legacy-uninstall for jmol.`|



---

archive/issue_comments_554937.json:
```json
{
    "body": "Changing commit from \"82e736b7c1f5f93042d61d34149150ab7d9cec2b\" to \"87c694f059e839a4bb12db80037f3c4398b56c2b\"",
    "created_at": "2019-12-16T16:04:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28596#issuecomment-554937",
    "user": "https://github.com/embray"
}
```

Changing commit from "82e736b7c1f5f93042d61d34149150ab7d9cec2b" to "87c694f059e839a4bb12db80037f3c4398b56c2b"



---

archive/issue_comments_554938.json:
```json
{
    "body": "Changing branch from \"u/jhpalmieri/jmol-permissions\" to \"u/embray/ticket-28596\"",
    "created_at": "2019-12-16T16:04:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28596#issuecomment-554938",
    "user": "https://github.com/embray"
}
```

Changing branch from "u/jhpalmieri/jmol-permissions" to "u/embray/ticket-28596"



---

archive/issue_comments_554939.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2019-12-16T16:05:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28596#issuecomment-554939",
    "user": "https://github.com/embray"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_554940.json:
```json
{
    "body": "<a id='comment:12'></a>\nAll tests pass for me on OS X, with a build from scratch, and the permissions look okay.",
    "created_at": "2019-12-16T22:24:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28596#issuecomment-554940",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:12'></a>
All tests pass for me on OS X, with a build from scratch, and the permissions look okay.



---

archive/issue_comments_554941.json:
```json
{
    "body": "<a id='comment:13'></a>\nthen it should be ok, no ?",
    "created_at": "2019-12-17T19:16:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28596#issuecomment-554941",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:13'></a>
then it should be ok, no ?



---

archive/issue_comments_554942.json:
```json
{
    "body": "Changing reviewer from \"\" to \"John Palmieri\"",
    "created_at": "2019-12-17T19:17:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28596#issuecomment-554942",
    "user": "https://github.com/jhpalmieri"
}
```

Changing reviewer from "" to "John Palmieri"



---

archive/issue_comments_554943.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2019-12-17T19:17:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28596#issuecomment-554943",
    "user": "https://github.com/jhpalmieri"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_554944.json:
```json
{
    "body": "<a id='comment:14'></a>\nOkay with me.",
    "created_at": "2019-12-17T19:17:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28596#issuecomment-554944",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:14'></a>
Okay with me.



---

archive/issue_comments_554945.json:
```json
{
    "body": "<a id='comment:15'></a>\nI'm sure, if he were paying attention, Jeroen would be pleased to be vindicated :)",
    "created_at": "2019-12-18T10:22:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28596#issuecomment-554945",
    "user": "https://github.com/embray"
}
```

<a id='comment:15'></a>
I'm sure, if he were paying attention, Jeroen would be pleased to be vindicated :)



---

archive/issue_comments_554946.json:
```json
{
    "body": "Changing branch from \"u/embray/ticket-28596\" to \"87c694f059e839a4bb12db80037f3c4398b56c2b\"",
    "created_at": "2019-12-20T22:46:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28596#issuecomment-554946",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/embray/ticket-28596" to "87c694f059e839a4bb12db80037f3c4398b56c2b"



---

archive/issue_events_071879.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-12-20T22:46:04Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/28596",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28596#event-71879"
}
```



---

archive/issue_comments_554947.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2019-12-20T22:46:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28596",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28596#issuecomment-554947",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
