# Issue 28509: Polyhedron: bounding box does not catch empty polyhedron

archive/issues_028272.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: Polyhedron().bounding_box()\n```\n\ndoes not throw an error, which was clearly not the intention of the code. This ticket fixes this by adding the missing parenthesis.\n\n\n\nCC:  @jplab @LaisRast\n\nKeywords: **polytopes**\n\nBranch/Commit: **[d332d96](https://github.com/sagemath/sagetrac-mirror/commit/d332d96a9cbcabd9505cc8e770b433a08cf648a1)**\n\nReviewer: **Laith Rastanawi**\n\nAuthor: **Jonathan Kliem**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28509_\n\n",
    "closed_at": "2019-10-06T23:06:39Z",
    "created_at": "2019-09-16T18:49:54Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Polyhedron: bounding box does not catch empty polyhedron",
    "type": "issue",
    "updated_at": "2019-10-06T23:06:39Z",
    "url": "https://github.com/sagemath/sage/issues/28509",
    "user": "https://github.com/kliem"
}
```

```
sage: Polyhedron().bounding_box()
```

does not throw an error, which was clearly not the intention of the code. This ticket fixes this by adding the missing parenthesis.



CC:  @jplab @LaisRast

Keywords: **polytopes**

Branch/Commit: **[d332d96](https://github.com/sagemath/sagetrac-mirror/commit/d332d96a9cbcabd9505cc8e770b433a08cf648a1)**

Reviewer: **Laith Rastanawi**

Author: **Jonathan Kliem**

_Issue created by migration from https://trac.sagemath.org/ticket/28509_





---

archive/issue_events_373120.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2019-09-16T18:49:54Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28509",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28509#event-373120"
}
```



---

archive/issue_events_373121.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2019-09-16T18:49:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28509",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "component: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28509#event-373121"
}
```



---

archive/issue_events_373122.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2019-09-16T18:49:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28509",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28509#event-373122"
}
```



---

archive/issue_events_373123.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2019-09-16T18:49:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28509",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28509#event-373123"
}
```



---

archive/issue_comments_447378.json:
```json
{
    "body": "Branch: **[public/28509](https://github.com/sagemath/sagetrac-mirror/tree/public/28509)**",
    "created_at": "2019-09-16T18:50:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28509",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28509#issuecomment-447378",
    "user": "https://github.com/kliem"
}
```

Branch: **[public/28509](https://github.com/sagemath/sagetrac-mirror/tree/public/28509)**



---

archive/issue_events_373124.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2019-09-16T18:50:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28509",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28509#event-373124"
}
```



---

archive/issue_comments_447379.json:
```json
{
    "body": "Commit: **[e25b4ff](https://github.com/sagemath/sagetrac-mirror/commit/e25b4ff8de19d1a7df386703c217f7cfe4b8f144)**",
    "created_at": "2019-09-16T18:50:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28509",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28509#issuecomment-447379",
    "user": "https://github.com/kliem"
}
```

Commit: **[e25b4ff](https://github.com/sagemath/sagetrac-mirror/commit/e25b4ff8de19d1a7df386703c217f7cfe4b8f144)**



---

archive/issue_comments_447380.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e25b4ff8de19d1a7df386703c217f7cfe4b8f144\">e25b4ff</a></td><td><code>fixed non-caught error for bounding_box</code></td></tr></table>\n",
    "created_at": "2019-09-16T18:50:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28509",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28509#issuecomment-447380",
    "user": "https://github.com/kliem"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e25b4ff8de19d1a7df386703c217f7cfe4b8f144">e25b4ff</a></td><td><code>fixed non-caught error for bounding_box</code></td></tr></table>




---

archive/issue_comments_447381.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d332d96a9cbcabd9505cc8e770b433a08cf648a1\">d332d96</a></td><td><code>correct error message for other polyhedra with no vertices</code></td></tr></table>\n",
    "created_at": "2019-09-16T19:05:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28509",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28509#issuecomment-447381",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d332d96a9cbcabd9505cc8e770b433a08cf648a1">d332d96</a></td><td><code>correct error message for other polyhedra with no vertices</code></td></tr></table>




---

archive/issue_comments_447382.json:
```json
{
    "body": "Changed commit from **[e25b4ff](https://github.com/sagemath/sagetrac-mirror/commit/e25b4ff8de19d1a7df386703c217f7cfe4b8f144)** to **[d332d96](https://github.com/sagemath/sagetrac-mirror/commit/d332d96a9cbcabd9505cc8e770b433a08cf648a1)**",
    "created_at": "2019-09-16T19:05:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28509",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28509#issuecomment-447382",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[e25b4ff](https://github.com/sagemath/sagetrac-mirror/commit/e25b4ff8de19d1a7df386703c217f7cfe4b8f144)** to **[d332d96](https://github.com/sagemath/sagetrac-mirror/commit/d332d96a9cbcabd9505cc8e770b433a08cf648a1)**



---

archive/issue_events_373125.json:
```json
{
    "actor": "https://github.com/LaisRast",
    "created_at": "2019-09-17T08:13:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28509",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28509#event-373125"
}
```



---

archive/issue_events_373126.json:
```json
{
    "actor": "https://github.com/LaisRast",
    "created_at": "2019-09-17T08:13:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28509",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28509#event-373126"
}
```



---

archive/issue_comments_447383.json:
```json
{
    "body": "Reviewer: **Laith Rastanawi**",
    "created_at": "2019-09-17T08:13:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28509",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28509#issuecomment-447383",
    "user": "https://github.com/LaisRast"
}
```

Reviewer: **Laith Rastanawi**



---

archive/issue_comments_447384.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nThe fix looks good to me, thus I will put it on \"positive review\".",
    "created_at": "2019-09-17T08:13:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28509",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28509#issuecomment-447384",
    "user": "https://github.com/LaisRast"
}
```

<div id="comment:3" align="right">Comment 3</div>

The fix looks good to me, thus I will put it on "positive review".



---

archive/issue_comments_447385.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -3,6 +3,6 @@\n sage: Polyhedron().bounding_box()\n ```\n \n-does not throw an error, which was clearly not the intention of the code. This ticket fixes this.\n+does not throw an error, which was clearly not the intention of the code. This ticket fixes this by adding the missing parenthesis.\n \n \n``````\n",
    "created_at": "2019-09-20T08:47:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28509",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28509#issuecomment-447385",
    "user": "https://github.com/jplab"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -3,6 +3,6 @@
 sage: Polyhedron().bounding_box()
 ```
 
-does not throw an error, which was clearly not the intention of the code. This ticket fixes this.
+does not throw an error, which was clearly not the intention of the code. This ticket fixes this by adding the missing parenthesis.
 
 
``````




---

archive/issue_comments_447386.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nmoving milestone to 9.0 (after release of 8.9)",
    "created_at": "2019-09-30T08:12:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28509",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28509#issuecomment-447386",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:5" align="right">Comment 5</div>

moving milestone to 9.0 (after release of 8.9)



---

archive/issue_events_373127.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-09-30T08:12:27Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28509",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28509#event-373127"
}
```



---

archive/issue_events_373128.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-09-30T08:12:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28509",
    "milestone_number": null,
    "milestone_title": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28509#event-373128"
}
```



---

archive/issue_comments_447387.json:
```json
{
    "body": "Changed branch from **[public/28509](https://github.com/sagemath/sagetrac-mirror/tree/public/28509)** to **[d332d96](https://github.com/sagemath/sagetrac-mirror/commit/d332d96a9cbcabd9505cc8e770b433a08cf648a1)**",
    "created_at": "2019-10-06T23:06:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28509",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28509#issuecomment-447387",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/28509](https://github.com/sagemath/sagetrac-mirror/tree/public/28509)** to **[d332d96](https://github.com/sagemath/sagetrac-mirror/commit/d332d96a9cbcabd9505cc8e770b433a08cf648a1)**



---

archive/issue_events_373129.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-10-06T23:06:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28509",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28509#event-373129"
}
```



---

archive/issue_events_373130.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "934ecf55e7f0123cfff45de7ba9bf526d12241f9",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-10-06T23:06:39Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28509",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28509#event-373130"
}
```
