# Issue 28187: Make _derivative consistent

archive/issues_027950.json:
```json
{
    "assignees": [],
    "body": "Derivation with respect to `var` has different behaviors for polynomials. Basically, the first test that is performed is whether `var` is the generator of the polynomial ring. But the test is performed in two different ways depending on the implementations:\n\n* Either `var is self._parent.gen()` (or equivalent);\n* Or `var == self._parent.gen()`.\n\nThe consequence is as follows:\n\n```python\nsage: R = PolynomialRing(ZZ, 'x')\nsage: R.gen().derivative(x) # implemented with ==\n1 \nsage: R = PolynomialRing(ZZ, 'x', sparse=True)\nsage: R.gen().derivative(x) # implemented with is\nTraceback (most recent call last):\n...\nAttributeError: 'sage.rings.integer.Integer' object has no attribute '_derivative'\n```\n\nMy proposal is to replace all the `is`'s by `==`'s. It also solves problems with derivatives of rational functions, see #26844. \n\nAltogether, the behavior for univariate polynomials is the following after my changes, consistently:\n\n* `p.derivative(x)` returns the derivative of `p` if `bool(x == p.parent().gen())` returns `True` (it includes some cases where `x` is not the generator itself);\n* `p.derivative(t)` (with `t != p.parent().gen()`) returns the polynomial obtained after derivation of each coefficient of `p` with respect to `t`;\n* if `t` does not appear to be a variable of `p` (even recursively), an exception is raised\u00b9.\n\n\n\u00b9 Initially, I wanted to return `0` when differentiating with respect to a variable not in `p`, but this causes quite a lot of non trivial code since one should also be able to raise an Exception if one tries to differentiate with respect to something that does not appear to be a variable (w.r.t `2*x`, `x+y` or `12` for instance). \n\nDepends on #28147\n\nKeywords: **polynomial derivative**\n\nBranch/Commit: **[b7ab5aa](https://github.com/sagemath/sagetrac-mirror/commit/b7ab5aad7f9a14a6d5d67ed3d138424a001da5dd)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Bruno Grenet**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28187_\n\n",
    "closed_at": "2019-10-06T23:06:54Z",
    "created_at": "2019-07-13T08:48:53Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make _derivative consistent",
    "type": "issue",
    "updated_at": "2019-10-06T23:06:54Z",
    "url": "https://github.com/sagemath/sage/issues/28187",
    "user": "https://github.com/bgrenet"
}
```
Derivation with respect to `var` has different behaviors for polynomials. Basically, the first test that is performed is whether `var` is the generator of the polynomial ring. But the test is performed in two different ways depending on the implementations:

* Either `var is self._parent.gen()` (or equivalent);
* Or `var == self._parent.gen()`.

The consequence is as follows:

```python
sage: R = PolynomialRing(ZZ, 'x')
sage: R.gen().derivative(x) # implemented with ==
1 
sage: R = PolynomialRing(ZZ, 'x', sparse=True)
sage: R.gen().derivative(x) # implemented with is
Traceback (most recent call last):
...
AttributeError: 'sage.rings.integer.Integer' object has no attribute '_derivative'
```

My proposal is to replace all the `is`'s by `==`'s. It also solves problems with derivatives of rational functions, see #26844. 

Altogether, the behavior for univariate polynomials is the following after my changes, consistently:

* `p.derivative(x)` returns the derivative of `p` if `bool(x == p.parent().gen())` returns `True` (it includes some cases where `x` is not the generator itself);
* `p.derivative(t)` (with `t != p.parent().gen()`) returns the polynomial obtained after derivation of each coefficient of `p` with respect to `t`;
* if `t` does not appear to be a variable of `p` (even recursively), an exception is raised¹.


¹ Initially, I wanted to return `0` when differentiating with respect to a variable not in `p`, but this causes quite a lot of non trivial code since one should also be able to raise an Exception if one tries to differentiate with respect to something that does not appear to be a variable (w.r.t `2*x`, `x+y` or `12` for instance). 

Depends on #28147

Keywords: **polynomial derivative**

Branch/Commit: **[b7ab5aa](https://github.com/sagemath/sagetrac-mirror/commit/b7ab5aad7f9a14a6d5d67ed3d138424a001da5dd)**

Reviewer: **Travis Scrimshaw**

Author: **Bruno Grenet**

_Issue created by migration from https://trac.sagemath.org/ticket/28187_





---

archive/issue_events_368371.json:
```json
{
    "actor": "https://github.com/bgrenet",
    "created_at": "2019-07-13T08:48:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28187#event-368371"
}
```



---

archive/issue_events_368372.json:
```json
{
    "actor": "https://github.com/bgrenet",
    "created_at": "2019-07-13T08:48:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28187#event-368372"
}
```



---

archive/issue_events_368373.json:
```json
{
    "actor": "https://github.com/bgrenet",
    "created_at": "2019-07-13T08:48:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28187#event-368373"
}
```



---

archive/issue_events_368374.json:
```json
{
    "actor": "https://github.com/bgrenet",
    "created_at": "2019-07-13T08:48:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28187#event-368374"
}
```



---

archive/issue_comments_441740.json:
```json
{
    "body": "Changed dependencies from **28147** to **#28147**",
    "created_at": "2019-07-13T09:55:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28187#issuecomment-441740",
    "user": "https://github.com/fchapoton"
}
```

Changed dependencies from **28147** to **#28147**



---

archive/issue_comments_441741.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6384e2ee1e9bfd719d86fda731ee68f090220a20\">6384e2e</a></td><td><code>28147: Remove _derivative</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/dcba4094c2910c1956413b1e310ca8465d2c7790\">dcba409</a></td><td><code>28147: Doctests</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1168449bd95676a20dd42731351e900eb91e1950\">1168449</a></td><td><code>28187: Consistent behavior of _derivative</code></td></tr></table>\n",
    "created_at": "2019-07-13T15:22:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28187#issuecomment-441741",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6384e2ee1e9bfd719d86fda731ee68f090220a20">6384e2e</a></td><td><code>28147: Remove _derivative</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/dcba4094c2910c1956413b1e310ca8465d2c7790">dcba409</a></td><td><code>28147: Doctests</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1168449bd95676a20dd42731351e900eb91e1950">1168449</a></td><td><code>28187: Consistent behavior of _derivative</code></td></tr></table>




---

archive/issue_comments_441742.json:
```json
{
    "body": "Commit: **[1168449](https://github.com/sagemath/sagetrac-mirror/commit/1168449bd95676a20dd42731351e900eb91e1950)**",
    "created_at": "2019-07-13T15:22:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28187#issuecomment-441742",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[1168449](https://github.com/sagemath/sagetrac-mirror/commit/1168449bd95676a20dd42731351e900eb91e1950)**



---

archive/issue_comments_441743.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -18,9 +18,11 @@\n \n My proposal is to replace all the `is`'s by `==`'s. It also solves problems with derivatives of rational functions, see #26844. \n \n-In the same ticket, I'd like to make the derivative with respect to a non existing variable return `0` rather than raising an exception.\n-\n Altogether, the behavior for univariate polynomials is the following after my changes, consistently:\n \n-* `p.derivative(x)` returns the derivative of `p` if `bool(x == p.parent().gen())` returns `True` (it includes the case where `x` is not the generator itself, but the string used for the variable of the polynomial ring is `'x'` for instance);\n-* `p.derivative(t)` (with `t != p.parent().gen()`) returns the polynomial obtained after derivation of each coefficient of `p` with respect to `t` (extra care must be taken since one cannot always use a derivative on the coefficients, so exceptions must be caught). \n+* `p.derivative(x)` returns the derivative of `p` if `bool(x == p.parent().gen())` returns `True` (it includes some cases where `x` is not the generator itself);\n+* `p.derivative(t)` (with `t != p.parent().gen()`) returns the polynomial obtained after derivation of each coefficient of `p` with respect to `t`;\n+* if `t` does not appear to be a variable of `p` (even recursively), an exception is raised\u00b9.\n+\n+\n+\u00b9 Initially, I wanted to return `0` when differentiating with respect to a variable not in `p`, but this causes quite a lot of non trivial code since one should also be able to raise an Exception if one tries to differentiate with respect to something that does not appear to be a variable (w.r.t `2*x`, `x+y` or `12` for instance). \n``````\n",
    "created_at": "2019-07-13T15:27:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28187#issuecomment-441743",
    "user": "https://github.com/bgrenet"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -18,9 +18,11 @@
 
 My proposal is to replace all the `is`'s by `==`'s. It also solves problems with derivatives of rational functions, see #26844. 
 
-In the same ticket, I'd like to make the derivative with respect to a non existing variable return `0` rather than raising an exception.
-
 Altogether, the behavior for univariate polynomials is the following after my changes, consistently:
 
-* `p.derivative(x)` returns the derivative of `p` if `bool(x == p.parent().gen())` returns `True` (it includes the case where `x` is not the generator itself, but the string used for the variable of the polynomial ring is `'x'` for instance);
-* `p.derivative(t)` (with `t != p.parent().gen()`) returns the polynomial obtained after derivation of each coefficient of `p` with respect to `t` (extra care must be taken since one cannot always use a derivative on the coefficients, so exceptions must be caught). 
+* `p.derivative(x)` returns the derivative of `p` if `bool(x == p.parent().gen())` returns `True` (it includes some cases where `x` is not the generator itself);
+* `p.derivative(t)` (with `t != p.parent().gen()`) returns the polynomial obtained after derivation of each coefficient of `p` with respect to `t`;
+* if `t` does not appear to be a variable of `p` (even recursively), an exception is raised¹.
+
+
+¹ Initially, I wanted to return `0` when differentiating with respect to a variable not in `p`, but this causes quite a lot of non trivial code since one should also be able to raise an Exception if one tries to differentiate with respect to something that does not appear to be a variable (w.r.t `2*x`, `x+y` or `12` for instance). 
``````




---

archive/issue_events_368375.json:
```json
{
    "actor": "https://github.com/bgrenet",
    "created_at": "2019-07-13T15:27:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28187#event-368375"
}
```



---

archive/issue_comments_441744.json:
```json
{
    "body": "Changed commit from **[1168449](https://github.com/sagemath/sagetrac-mirror/commit/1168449bd95676a20dd42731351e900eb91e1950)** to **[9b2e010](https://github.com/sagemath/sagetrac-mirror/commit/9b2e01010057914d472ef0193a7f4952698c9363)**",
    "created_at": "2019-07-13T15:48:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28187#issuecomment-441744",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[1168449](https://github.com/sagemath/sagetrac-mirror/commit/1168449bd95676a20dd42731351e900eb91e1950)** to **[9b2e010](https://github.com/sagemath/sagetrac-mirror/commit/9b2e01010057914d472ef0193a7f4952698c9363)**



---

archive/issue_comments_441745.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9b2e01010057914d472ef0193a7f4952698c9363\">9b2e010</a></td><td><code>28187: extend consistency to power series</code></td></tr></table>\n",
    "created_at": "2019-07-13T15:48:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28187#issuecomment-441745",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9b2e01010057914d472ef0193a7f4952698c9363">9b2e010</a></td><td><code>28187: extend consistency to power series</code></td></tr></table>




---

archive/issue_events_368376.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-08-29T13:29:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28187#event-368376"
}
```



---

archive/issue_events_368377.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-08-29T13:29:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28187#event-368377"
}
```



---

archive/issue_comments_441746.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nred branch, needs rebase",
    "created_at": "2019-08-29T13:29:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28187#issuecomment-441746",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:5'>Comment 5:</a>
red branch, needs rebase



---

archive/issue_comments_441747.json:
```json
{
    "body": "Changed commit from **[9b2e010](https://github.com/sagemath/sagetrac-mirror/commit/9b2e01010057914d472ef0193a7f4952698c9363)** to **[9063475](https://github.com/sagemath/sagetrac-mirror/commit/90634759f59710f50e85b610966b9e1e7abfccbd)**",
    "created_at": "2019-09-12T13:29:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28187#issuecomment-441747",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[9b2e010](https://github.com/sagemath/sagetrac-mirror/commit/9b2e01010057914d472ef0193a7f4952698c9363)** to **[9063475](https://github.com/sagemath/sagetrac-mirror/commit/90634759f59710f50e85b610966b9e1e7abfccbd)**



---

archive/issue_comments_441748.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bc82deb35ac1e30b959465a06778f73f2e204adf\">bc82deb</a></td><td><code>28187: Consistent behavior of _derivative</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/90634759f59710f50e85b610966b9e1e7abfccbd\">9063475</a></td><td><code>28187: extend consistency to power series</code></td></tr></table>\n",
    "created_at": "2019-09-12T13:29:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28187#issuecomment-441748",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:6'>Comment 6:</a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bc82deb35ac1e30b959465a06778f73f2e204adf">bc82deb</a></td><td><code>28187: Consistent behavior of _derivative</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/90634759f59710f50e85b610966b9e1e7abfccbd">9063475</a></td><td><code>28187: extend consistency to power series</code></td></tr></table>




---

archive/issue_events_368378.json:
```json
{
    "actor": "https://github.com/bgrenet",
    "created_at": "2019-09-12T13:30:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28187#event-368378"
}
```



---

archive/issue_events_368379.json:
```json
{
    "actor": "https://github.com/bgrenet",
    "created_at": "2019-09-12T13:30:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28187#event-368379"
}
```



---

archive/issue_comments_441749.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nRebased!",
    "created_at": "2019-09-12T13:30:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28187#issuecomment-441749",
    "user": "https://github.com/bgrenet"
}
```

<a id='comment:7'>Comment 7:</a>
Rebased!



---

archive/issue_comments_441750.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2019-09-13T01:12:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28187#issuecomment-441750",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_441751.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nIt definitely should not be using `is` for comparison as\n\n```\nsage: R.<x> = ZZ[]\nsage: 1*x is x\nFalse\nsage: 1*x == x\nTrue\n```\nSo I think it is too easy to pass something that `==` to a generator but not `is` a generator.\n\nTwo little things that once done you can set a positive review on my behalf. This needs a blank line:\n\n```diff\n         Check that :trac:`28187` is fixed::\n+\n             sage: x = var(\"x\")\n             sage: f._derivative(x)\n             4*x^3 - 1\n```\nThis is just some PEP8 cleanup to make the code look nicer (while we are at it):\n\n```diff\n-                return P({n:self.__coeffs[n]._derivative(var) \\\n-                            for n in self.__coeffs})\n+                return P({n:self.__coeffs[n]._derivative(var)\n+                           for n in self.__coeffs})\n```",
    "created_at": "2019-09-13T01:12:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28187#issuecomment-441751",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:8'>Comment 8:</a>
It definitely should not be using `is` for comparison as

```
sage: R.<x> = ZZ[]
sage: 1*x is x
False
sage: 1*x == x
True
```
So I think it is too easy to pass something that `==` to a generator but not `is` a generator.

Two little things that once done you can set a positive review on my behalf. This needs a blank line:

```diff
         Check that :trac:`28187` is fixed::
+
             sage: x = var("x")
             sage: f._derivative(x)
             4*x^3 - 1
```
This is just some PEP8 cleanup to make the code look nicer (while we are at it):

```diff
-                return P({n:self.__coeffs[n]._derivative(var) \
-                            for n in self.__coeffs})
+                return P({n:self.__coeffs[n]._derivative(var)
+                           for n in self.__coeffs})
```



---

archive/issue_comments_441752.json:
```json
{
    "body": "Changed commit from **[9063475](https://github.com/sagemath/sagetrac-mirror/commit/90634759f59710f50e85b610966b9e1e7abfccbd)** to **[b7ab5aa](https://github.com/sagemath/sagetrac-mirror/commit/b7ab5aad7f9a14a6d5d67ed3d138424a001da5dd)**",
    "created_at": "2019-09-13T12:44:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28187#issuecomment-441752",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[9063475](https://github.com/sagemath/sagetrac-mirror/commit/90634759f59710f50e85b610966b9e1e7abfccbd)** to **[b7ab5aa](https://github.com/sagemath/sagetrac-mirror/commit/b7ab5aad7f9a14a6d5d67ed3d138424a001da5dd)**



---

archive/issue_comments_441753.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b7ab5aad7f9a14a6d5d67ed3d138424a001da5dd\">b7ab5aa</a></td><td><code>28187: cosmetic</code></td></tr></table>\n",
    "created_at": "2019-09-13T12:44:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28187#issuecomment-441753",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:9'>Comment 9:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b7ab5aad7f9a14a6d5d67ed3d138424a001da5dd">b7ab5aa</a></td><td><code>28187: cosmetic</code></td></tr></table>




---

archive/issue_events_368380.json:
```json
{
    "actor": "https://github.com/bgrenet",
    "created_at": "2019-09-13T12:45:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28187#event-368380"
}
```



---

archive/issue_events_368381.json:
```json
{
    "actor": "https://github.com/bgrenet",
    "created_at": "2019-09-13T12:45:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28187#event-368381"
}
```



---

archive/issue_comments_441754.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nThank you!",
    "created_at": "2019-09-13T12:45:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28187#issuecomment-441754",
    "user": "https://github.com/bgrenet"
}
```

<a id='comment:10'>Comment 10:</a>
Thank you!



---

archive/issue_comments_441755.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nmoving milestone to 9.0 (after release of 8.9)",
    "created_at": "2019-09-30T08:12:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28187#issuecomment-441755",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:11'>Comment 11:</a>
moving milestone to 9.0 (after release of 8.9)



---

archive/issue_events_368382.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-09-30T08:12:27Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28187#event-368382"
}
```



---

archive/issue_events_368383.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-09-30T08:12:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "milestone_number": null,
    "milestone_title": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28187#event-368383"
}
```



---

archive/issue_comments_441756.json:
```json
{
    "body": "Changed branch from **[u/bruno/derivative_consistency](https://github.com/sagemath/sagetrac-mirror/tree/u/bruno/derivative_consistency)** to **[b7ab5aa](https://github.com/sagemath/sagetrac-mirror/commit/b7ab5aad7f9a14a6d5d67ed3d138424a001da5dd)**",
    "created_at": "2019-10-06T23:06:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28187#issuecomment-441756",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/bruno/derivative_consistency](https://github.com/sagemath/sagetrac-mirror/tree/u/bruno/derivative_consistency)** to **[b7ab5aa](https://github.com/sagemath/sagetrac-mirror/commit/b7ab5aad7f9a14a6d5d67ed3d138424a001da5dd)**



---

archive/issue_events_368384.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-10-06T23:06:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28187#event-368384"
}
```



---

archive/issue_events_368385.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "05d62701df6852fb833261049072302cc5432287",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-10-06T23:06:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28187",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28187#event-368385"
}
```
