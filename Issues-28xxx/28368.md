# Issue 28368: A multivariable zeta function for a singularity via CDG and Némethi

archive/issues_028131.json:
```json
{
    "assignees": [],
    "body": "This is an addition to the PlumbingGraph() class which calculates the zeta function introduced by Campillo, Delgado and Gusein-Zade, and by N\u00e9methi. It assumes that the plumbing graph is the link of a singularity (i.e. edges are positive, genera are positive, the intersection matrix is negative definite) and that the link is a rational homology sphere (i.e. genera are zero and the graph is a tree). In this case, the CDGN zeta function, a topological invariant, coincides with a Poincar\u00e9 series in certain cases, although this does not hold in general.\n\nDepends on #26945\n\nKeywords: **plumbing graph, zeta function, rational homology three sphere**\n\nBranch/Commit: **[u/gh-baldursigurds/a_multivariable_zeta_function_for_a_singularity_via_cdg_and_n_methi](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-baldursigurds/a_multivariable_zeta_function_for_a_singularity_via_cdg_and_n_methi) @ [73114ce](https://github.com/sagemath/sagetrac-mirror/commit/73114cefab3de6461bf69ef8064e76a8291fa7df)**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28368_\n\n",
    "created_at": "2019-08-20T03:41:21Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "A multivariable zeta function for a singularity via CDG and N\u00e9methi",
    "type": "issue",
    "updated_at": "2022-08-31T05:25:02Z",
    "url": "https://github.com/sagemath/sage/issues/28368",
    "user": "https://github.com/baldursigurds"
}
```
This is an addition to the PlumbingGraph() class which calculates the zeta function introduced by Campillo, Delgado and Gusein-Zade, and by Némethi. It assumes that the plumbing graph is the link of a singularity (i.e. edges are positive, genera are positive, the intersection matrix is negative definite) and that the link is a rational homology sphere (i.e. genera are zero and the graph is a tree). In this case, the CDGN zeta function, a topological invariant, coincides with a Poincaré series in certain cases, although this does not hold in general.

Depends on #26945

Keywords: **plumbing graph, zeta function, rational homology three sphere**

Branch/Commit: **[u/gh-baldursigurds/a_multivariable_zeta_function_for_a_singularity_via_cdg_and_n_methi](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-baldursigurds/a_multivariable_zeta_function_for_a_singularity_via_cdg_and_n_methi) @ [73114ce](https://github.com/sagemath/sagetrac-mirror/commit/73114cefab3de6461bf69ef8064e76a8291fa7df)**

_Issue created by migration from https://trac.sagemath.org/ticket/28368_





---

archive/issue_events_371085.json:
```json
{
    "actor": "https://github.com/baldursigurds",
    "created_at": "2019-08-20T03:41:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28368",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28368#event-371085"
}
```



---

archive/issue_events_371086.json:
```json
{
    "actor": "https://github.com/baldursigurds",
    "created_at": "2019-08-20T03:41:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28368",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "component: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28368#event-371086"
}
```



---

archive/issue_events_371087.json:
```json
{
    "actor": "https://github.com/baldursigurds",
    "created_at": "2019-08-20T03:41:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28368",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28368#event-371087"
}
```



---

archive/issue_events_371088.json:
```json
{
    "actor": "https://github.com/baldursigurds",
    "created_at": "2019-08-20T03:41:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28368",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28368#event-371088"
}
```



---

archive/issue_comments_444929.json:
```json
{
    "body": "Branch: **[u/gh-baldursigurds/a_multivariable_zeta_function_for_a_singularity_via_cdg_and_n_methi](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-baldursigurds/a_multivariable_zeta_function_for_a_singularity_via_cdg_and_n_methi)**",
    "created_at": "2019-08-24T23:41:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28368",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28368#issuecomment-444929",
    "user": "https://github.com/baldursigurds"
}
```

Branch: **[u/gh-baldursigurds/a_multivariable_zeta_function_for_a_singularity_via_cdg_and_n_methi](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-baldursigurds/a_multivariable_zeta_function_for_a_singularity_via_cdg_and_n_methi)**



---

archive/issue_comments_444930.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nI pushed some code here, only so anyone can look at it if they want to. There still seem to be some small issues, so it's not ready yet. But it can still compute some stuff.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c6d741f5db8ce986ef5729c3a689ae3028c81de3\">c6d741f</a></td><td><code>initial commit for plumbing_graph</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f5d5cc4734982fcc051711040b6b38409045872f\">f5d5cc4</a></td><td><code>removed some unnecessary lines</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/96b5a63e42f3f41ba1b9b2836b1ad952bea7a6f7\">96b5a63</a></td><td><code>Added the function step_3() to the class PlumbingGraph, and an example.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ab62c43d53a2005e1a2412787071aa72450046f7\">ab62c43</a></td><td><code>fixed a simple typo in the example for step_3()</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6e79dc16264227a1b6299caab167a6f297372d6d\">6e79dc1</a></td><td><code>at this point, all the functions for the algorithm for the minimal</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/90a32900ac63b2c398911f50532477252888bb95\">90a3290</a></td><td><code>Merge branch 'u/gh-baldursigurds/plumbing_graph' into t/28368/a_multivariable_zeta_function_for_a_singularity_via_cdg_and_n_methi</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/73114cefab3de6461bf69ef8064e76a8291fa7df\">73114ce</a></td><td><code>first commit, there are some issues still that I'm working on</code></td></tr></table>\n",
    "created_at": "2019-08-24T23:44:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28368",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28368#issuecomment-444930",
    "user": "https://github.com/baldursigurds"
}
```

<div id="comment:2" align="right">Comment 2</div>

I pushed some code here, only so anyone can look at it if they want to. There still seem to be some small issues, so it's not ready yet. But it can still compute some stuff.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c6d741f5db8ce986ef5729c3a689ae3028c81de3">c6d741f</a></td><td><code>initial commit for plumbing_graph</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f5d5cc4734982fcc051711040b6b38409045872f">f5d5cc4</a></td><td><code>removed some unnecessary lines</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/96b5a63e42f3f41ba1b9b2836b1ad952bea7a6f7">96b5a63</a></td><td><code>Added the function step_3() to the class PlumbingGraph, and an example.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ab62c43d53a2005e1a2412787071aa72450046f7">ab62c43</a></td><td><code>fixed a simple typo in the example for step_3()</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6e79dc16264227a1b6299caab167a6f297372d6d">6e79dc1</a></td><td><code>at this point, all the functions for the algorithm for the minimal</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/90a32900ac63b2c398911f50532477252888bb95">90a3290</a></td><td><code>Merge branch 'u/gh-baldursigurds/plumbing_graph' into t/28368/a_multivariable_zeta_function_for_a_singularity_via_cdg_and_n_methi</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/73114cefab3de6461bf69ef8064e76a8291fa7df">73114ce</a></td><td><code>first commit, there are some issues still that I'm working on</code></td></tr></table>




---

archive/issue_comments_444931.json:
```json
{
    "body": "Commit: **[73114ce](https://github.com/sagemath/sagetrac-mirror/commit/73114cefab3de6461bf69ef8064e76a8291fa7df)**",
    "created_at": "2019-08-24T23:44:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28368",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28368#issuecomment-444931",
    "user": "https://github.com/baldursigurds"
}
```

Commit: **[73114ce](https://github.com/sagemath/sagetrac-mirror/commit/73114cefab3de6461bf69ef8064e76a8291fa7df)**



---

archive/issue_comments_444932.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nTicket retargeted after milestone closed",
    "created_at": "2019-12-30T14:48:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28368",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28368#issuecomment-444932",
    "user": "https://github.com/embray"
}
```

<div id="comment:3" align="right">Comment 3</div>

Ticket retargeted after milestone closed



---

archive/issue_events_371089.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28368",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28368#event-371089"
}
```



---

archive/issue_events_371090.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28368",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28368#event-371090"
}
```



---

archive/issue_comments_444933.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBatch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.",
    "created_at": "2020-04-14T19:41:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28368",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28368#issuecomment-444933",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:4" align="right">Comment 4</div>

Batch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.



---

archive/issue_events_371091.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28368",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28368#event-371091"
}
```



---

archive/issue_events_371092.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28368",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28368#event-371092"
}
```



---

archive/issue_events_371093.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T16:37:33Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28368",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28368#event-371093"
}
```



---

archive/issue_events_371094.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T16:37:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28368",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28368#event-371094"
}
```



---

archive/issue_comments_444934.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nSetting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28368",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28368#issuecomment-444934",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:6" align="right">Comment 6</div>

Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_371095.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28368",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28368#event-371095"
}
```



---

archive/issue_events_371096.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28368",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28368#event-371096"
}
```



---

archive/issue_events_371097.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28368",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28368#event-371097"
}
```



---

archive/issue_events_371098.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28368",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28368#event-371098"
}
```



---

archive/issue_events_371099.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28368",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28368#event-371099"
}
```



---

archive/issue_events_371100.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28368",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28368#event-371100"
}
```



---

archive/issue_events_371101.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-02T04:32:35Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28368",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28368#event-371101"
}
```



---

archive/issue_events_371102.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-02T04:32:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28368",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28368#event-371102"
}
```



---

archive/issue_events_371103.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T05:25:02Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28368",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28368#event-371103"
}
```



---

archive/issue_events_371104.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T05:25:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28368",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28368#event-371104"
}
```
