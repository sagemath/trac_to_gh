# Issue 28947: multiplicative Abelian groups are not GAP friendly

archive/issues_028710.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: M = groups.misc.MultiplicativeAbelian([12])\nsage: Mgap = libgap(M)     # fine\nsage: libgap(M.gens()[0])  # fails\n```\n\nBranch/Commit: **[75c73de](https://github.com/sagemath/sagetrac-mirror/commit/75c73de334368df4de7eae9baf589010a3c458a5)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Vincent Delecroix**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28947_\n\n",
    "closed_at": "2020-01-09T23:44:10Z",
    "created_at": "2020-01-03T08:03:35Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20group%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "multiplicative Abelian groups are not GAP friendly",
    "type": "issue",
    "updated_at": "2020-01-09T23:44:10Z",
    "url": "https://github.com/sagemath/sage/issues/28947",
    "user": "https://github.com/videlec"
}
```

```
sage: M = groups.misc.MultiplicativeAbelian([12])
sage: Mgap = libgap(M)     # fine
sage: libgap(M.gens()[0])  # fails
```

Branch/Commit: **[75c73de](https://github.com/sagemath/sagetrac-mirror/commit/75c73de334368df4de7eae9baf589010a3c458a5)**

Reviewer: **Travis Scrimshaw**

Author: **Vincent Delecroix**

_Issue created by migration from https://trac.sagemath.org/ticket/28947_





---

archive/issue_events_379506.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2020-01-03T08:03:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28947",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28947#event-379506"
}
```



---

archive/issue_events_379507.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2020-01-03T08:03:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28947",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20group%20theory",
    "label_color": "0000ff",
    "label_name": "component: group theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28947#event-379507"
}
```



---

archive/issue_events_379508.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2020-01-03T08:03:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28947",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28947#event-379508"
}
```



---

archive/issue_events_379509.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2020-01-03T08:03:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28947",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28947#event-379509"
}
```



---

archive/issue_comments_455059.json:
```json
{
    "body": "Author: **u/vdelecroix/28947**",
    "created_at": "2020-01-03T08:28:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28947",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28947#issuecomment-455059",
    "user": "https://github.com/videlec"
}
```

Author: **u/vdelecroix/28947**



---

archive/issue_events_379510.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2020-01-03T08:28:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28947",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28947#event-379510"
}
```



---

archive/issue_comments_455060.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/75c73de334368df4de7eae9baf589010a3c458a5\">75c73de</a></td><td><code>Make multiplicative groups GAP friendly</code></td></tr></table>\n",
    "created_at": "2020-01-03T08:28:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28947",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28947#issuecomment-455060",
    "user": "https://github.com/videlec"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/75c73de334368df4de7eae9baf589010a3c458a5">75c73de</a></td><td><code>Make multiplicative groups GAP friendly</code></td></tr></table>




---

archive/issue_comments_455061.json:
```json
{
    "body": "Changed author from **u/vdelecroix/28947** to **Vincent Delecroix**",
    "created_at": "2020-01-03T08:28:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28947",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28947#issuecomment-455061",
    "user": "https://github.com/videlec"
}
```

Changed author from **u/vdelecroix/28947** to **Vincent Delecroix**



---

archive/issue_comments_455062.json:
```json
{
    "body": "Commit: **[75c73de](https://github.com/sagemath/sagetrac-mirror/commit/75c73de334368df4de7eae9baf589010a3c458a5)**",
    "created_at": "2020-01-03T08:28:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28947",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28947#issuecomment-455062",
    "user": "https://github.com/videlec"
}
```

Commit: **[75c73de](https://github.com/sagemath/sagetrac-mirror/commit/75c73de334368df4de7eae9baf589010a3c458a5)**



---

archive/issue_comments_455063.json:
```json
{
    "body": "Branch: **[u/vdelecroix/28947](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/28947)**",
    "created_at": "2020-01-03T08:28:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28947",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28947#issuecomment-455063",
    "user": "https://github.com/videlec"
}
```

Branch: **[u/vdelecroix/28947](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/28947)**



---

archive/issue_events_379511.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-01-04T21:09:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28947",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28947#event-379511"
}
```



---

archive/issue_events_379512.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-01-04T21:09:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28947",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28947#event-379512"
}
```



---

archive/issue_comments_455064.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2020-01-04T21:09:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28947",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28947#issuecomment-455064",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_455065.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nLGTM.",
    "created_at": "2020-01-04T21:09:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28947",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28947#issuecomment-455065",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:3'>Comment 3:</a>
LGTM.



---

archive/issue_comments_455066.json:
```json
{
    "body": "Changed branch from **[u/vdelecroix/28947](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/28947)** to **[75c73de](https://github.com/sagemath/sagetrac-mirror/commit/75c73de334368df4de7eae9baf589010a3c458a5)**",
    "created_at": "2020-01-09T23:44:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28947",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28947#issuecomment-455066",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/vdelecroix/28947](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/28947)** to **[75c73de](https://github.com/sagemath/sagetrac-mirror/commit/75c73de334368df4de7eae9baf589010a3c458a5)**



---

archive/issue_events_379513.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-01-09T23:44:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28947",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28947#event-379513"
}
```



---

archive/issue_events_379514.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "d6b83409ebd3b8d471ab01ad2639486d0598bf82",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-01-09T23:44:10Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28947",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28947#event-379514"
}
```
