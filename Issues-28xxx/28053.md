# Issue 28053: Add length of a propositional formula function in the Logic Module

archive/issues_027816.json:
```json
{
    "assignees": [],
    "body": "This function returns the length of a formula (parentheses and spaces are ignored). \nUseful for proofs by induction on the length of a propositional formula.\n\n```\nsage: f = propcalc.formula(\"a\")\nsage: len(f)\n1\nsage: g = propcalc.formula(\"a|b\")\nsage: len(g)\n3\n```\n\n\nKeywords: **logic, symbolics, propositional formula**\n\nBranch: **[8e331fa](https://github.com/sagemath/sagetrac-mirror/commit/8e331fad4af24b83b13b3f7fb6711a615ec25e63)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Medha Sharma, Dave Morris**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28053_\n\n",
    "closed_at": "2020-03-17T23:35:44Z",
    "created_at": "2019-06-25T12:07:14Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add length of a propositional formula function in the Logic Module",
    "type": "issue",
    "updated_at": "2021-07-07T01:47:16Z",
    "url": "https://github.com/sagemath/sage/issues/28053",
    "user": "https://github.com/Medha210699"
}
```
This function returns the length of a formula (parentheses and spaces are ignored). 
Useful for proofs by induction on the length of a propositional formula.

```
sage: f = propcalc.formula("a")
sage: len(f)
1
sage: g = propcalc.formula("a|b")
sage: len(g)
3
```


Keywords: **logic, symbolics, propositional formula**

Branch: **[8e331fa](https://github.com/sagemath/sagetrac-mirror/commit/8e331fad4af24b83b13b3f7fb6711a615ec25e63)**

Reviewer: **Travis Scrimshaw**

Author: **Medha Sharma, Dave Morris**

_Issue created by migration from https://trac.sagemath.org/ticket/28053_





---

archive/issue_events_366460.json:
```json
{
    "actor": "https://github.com/Medha210699",
    "created_at": "2019-06-25T12:07:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28053#event-366460"
}
```



---

archive/issue_events_366461.json:
```json
{
    "actor": "https://github.com/Medha210699",
    "created_at": "2019-06-25T12:07:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28053#event-366461"
}
```



---

archive/issue_events_366462.json:
```json
{
    "actor": "https://github.com/Medha210699",
    "created_at": "2019-06-25T12:07:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28053#event-366462"
}
```



---

archive/issue_events_366463.json:
```json
{
    "actor": "https://github.com/Medha210699",
    "created_at": "2019-06-25T12:07:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28053#event-366463"
}
```



---

archive/issue_events_366464.json:
```json
{
    "actor": "https://github.com/Medha210699",
    "created_at": "2019-06-25T12:07:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28053#event-366464"
}
```



---

archive/issue_events_366465.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-06-25T19:14:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28053#event-366465"
}
```



---

archive/issue_events_366466.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-06-25T19:14:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28053#event-366466"
}
```



---

archive/issue_comments_439363.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nAs [#28052 comment:3](https://github.com/sagemath/sage/issues/28052#comment:3).",
    "created_at": "2019-06-25T19:14:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28053#issuecomment-439363",
    "user": "https://github.com/videlec"
}
```

<div id="comment:2" align="right">Comment 2</div>

As [#28052 comment:3](https://github.com/sagemath/sage/issues/28052#comment:3).



---

archive/issue_comments_439364.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nTicket retargeted after milestone closed",
    "created_at": "2019-12-30T14:48:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28053#issuecomment-439364",
    "user": "https://github.com/embray"
}
```

<div id="comment:3" align="right">Comment 3</div>

Ticket retargeted after milestone closed



---

archive/issue_events_366467.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28053#event-366467"
}
```



---

archive/issue_events_366468.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28053#event-366468"
}
```



---

archive/issue_comments_439365.json:
```json
{
    "body": "Branch: **[public/28053](https://github.com/sagemath/sagetrac-mirror/tree/public/28053)**",
    "created_at": "2020-03-11T19:06:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28053#issuecomment-439365",
    "user": "https://github.com/DaveWitteMorris"
}
```

Branch: **[public/28053](https://github.com/sagemath/sagetrac-mirror/tree/public/28053)**



---

archive/issue_events_366469.json:
```json
{
    "actor": "https://github.com/DaveWitteMorris",
    "created_at": "2020-03-11T19:18:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28053#event-366469"
}
```



---

archive/issue_events_366470.json:
```json
{
    "actor": "https://github.com/DaveWitteMorris",
    "created_at": "2020-03-11T19:18:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28053#event-366470"
}
```



---

archive/issue_comments_439366.json:
```json
{
    "body": "Commit: **[8e331fa](https://github.com/sagemath/sagetrac-mirror/commit/8e331fad4af24b83b13b3f7fb6711a615ec25e63)**",
    "created_at": "2020-03-11T19:18:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28053#issuecomment-439366",
    "user": "https://github.com/DaveWitteMorris"
}
```

Commit: **[8e331fa](https://github.com/sagemath/sagetrac-mirror/commit/8e331fad4af24b83b13b3f7fb6711a615ec25e63)**



---

archive/issue_comments_439367.json:
```json
{
    "body": "Changed author from **Medha Sharma** to **Medha Sharma, Dave Morris**",
    "created_at": "2020-03-11T19:18:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28053#issuecomment-439367",
    "user": "https://github.com/DaveWitteMorris"
}
```

Changed author from **Medha Sharma** to **Medha Sharma, Dave Morris**



---

archive/issue_events_366471.json:
```json
{
    "actor": "https://github.com/DaveWitteMorris",
    "created_at": "2020-03-11T19:18:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28053#event-366471"
}
```



---

archive/issue_events_366472.json:
```json
{
    "actor": "https://github.com/DaveWitteMorris",
    "created_at": "2020-03-11T19:18:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28053#event-366472"
}
```



---

archive/issue_comments_439368.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThis patch defines the `__len__()` attribute of a Boolean formula to be the length of the formula. So `len(f)` is the length of the formula `f`. This is the number of operators plus the number of variables (counting multiplicity). Parentheses and spaces in the formula are ignored.\n\n```\nsage: f = propcalc.formula(\"a\")\nsage: len(f)\n1\nsage: g = propcalc.formula(\"a|b\")\nsage: len(g)\n3\n```\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8e331fad4af24b83b13b3f7fb6711a615ec25e63\">8e331fa</a></td><td><code>define __len__() atrribute</code></td></tr></table>\n",
    "created_at": "2020-03-11T19:18:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28053#issuecomment-439368",
    "user": "https://github.com/DaveWitteMorris"
}
```

<div id="comment:5" align="right">Comment 5</div>

This patch defines the `__len__()` attribute of a Boolean formula to be the length of the formula. So `len(f)` is the length of the formula `f`. This is the number of operators plus the number of variables (counting multiplicity). Parentheses and spaces in the formula are ignored.

```
sage: f = propcalc.formula("a")
sage: len(f)
1
sage: g = propcalc.formula("a|b")
sage: len(g)
3
```

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8e331fad4af24b83b13b3f7fb6711a615ec25e63">8e331fa</a></td><td><code>define __len__() atrribute</code></td></tr></table>




---

archive/issue_comments_439369.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2020-03-13T00:16:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28053#issuecomment-439369",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_439370.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,13 +1,12 @@\n-This function first properly parenthesizes the given formula, then returns its length. \n+This function returns the length of a formula (parentheses and spaces are ignored). \n Useful for proofs by induction on the length of a propositional formula.\n \n ```\n sage: f = propcalc.formula(\"a\")\n sage: f.length()\n-3\n+1\n sage: g = propcalc.formula(\"a|b\")\n sage: g.length()\n-5\n+3\n ```\n \n-Note: This function will first parenthesize f to (a) and g to (a|b)\n``````\n",
    "created_at": "2020-03-13T00:16:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28053#issuecomment-439370",
    "user": "https://github.com/tscrim"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,13 +1,12 @@
-This function first properly parenthesizes the given formula, then returns its length. 
+This function returns the length of a formula (parentheses and spaces are ignored). 
 Useful for proofs by induction on the length of a propositional formula.
 
 ```
 sage: f = propcalc.formula("a")
 sage: f.length()
-3
+1
 sage: g = propcalc.formula("a|b")
 sage: g.length()
-5
+3
 ```
 
-Note: This function will first parenthesize f to (a) and g to (a|b)
``````




---

archive/issue_comments_439371.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nLGTM. I updated to description to match the implementation.",
    "created_at": "2020-03-13T00:16:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28053#issuecomment-439371",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:6" align="right">Comment 6</div>

LGTM. I updated to description to match the implementation.



---

archive/issue_events_366473.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-03-13T00:16:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28053#event-366473"
}
```



---

archive/issue_events_366474.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-03-13T00:16:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28053#event-366474"
}
```



---

archive/issue_comments_439372.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -3,10 +3,10 @@\n \n ```\n sage: f = propcalc.formula(\"a\")\n-sage: f.length()\n+sage: len(f)\n 1\n sage: g = propcalc.formula(\"a|b\")\n-sage: g.length()\n+sage: len(g)\n 3\n ```\n \n``````\n",
    "created_at": "2020-03-13T01:02:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28053#issuecomment-439372",
    "user": "https://github.com/DaveWitteMorris"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -3,10 +3,10 @@
 
 ```
 sage: f = propcalc.formula("a")
-sage: f.length()
+sage: len(f)
 1
 sage: g = propcalc.formula("a|b")
-sage: g.length()
+sage: len(g)
 3
 ```
 
``````




---

archive/issue_comments_439373.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nThanks for the review and for correcting the description. However, I made a further correction to the description because `f.length()` does not work -- it needs to be `len(f)`.",
    "created_at": "2020-03-13T01:02:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28053#issuecomment-439373",
    "user": "https://github.com/DaveWitteMorris"
}
```

<div id="comment:7" align="right">Comment 7</div>

Thanks for the review and for correcting the description. However, I made a further correction to the description because `f.length()` does not work -- it needs to be `len(f)`.



---

archive/issue_comments_439374.json:
```json
{
    "body": "Changed branch from **[public/28053](https://github.com/sagemath/sagetrac-mirror/tree/public/28053)** to **[8e331fa](https://github.com/sagemath/sagetrac-mirror/commit/8e331fad4af24b83b13b3f7fb6711a615ec25e63)**",
    "created_at": "2020-03-17T23:35:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28053#issuecomment-439374",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/28053](https://github.com/sagemath/sagetrac-mirror/tree/public/28053)** to **[8e331fa](https://github.com/sagemath/sagetrac-mirror/commit/8e331fad4af24b83b13b3f7fb6711a615ec25e63)**



---

archive/issue_events_366475.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-03-17T23:35:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28053#event-366475"
}
```



---

archive/issue_events_366476.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "fec8cfb8d3a18853f8c253956cc6270d95fe794c",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-03-17T23:35:44Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28053#event-366476"
}
```



---

archive/issue_comments_439375.json:
```json
{
    "body": "Changed commit from **[8e331fa](https://github.com/sagemath/sagetrac-mirror/commit/8e331fad4af24b83b13b3f7fb6711a615ec25e63)** to none",
    "created_at": "2021-07-06T09:53:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28053#issuecomment-439375",
    "user": "https://github.com/slel"
}
```

Changed commit from **[8e331fa](https://github.com/sagemath/sagetrac-mirror/commit/8e331fad4af24b83b13b3f7fb6711a615ec25e63)** to none



---

archive/issue_events_366477.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-07-06T09:53:58Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "title_is": "Add length of a propositional formula function in the Logic Module",
    "title_was": "Adding length of a propositonal formula function in the Logic Module",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28053#event-366477"
}
```



---

archive/issue_comments_439376.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nRelated:\n\n- #32144: Add tree_size method to symbolic expressions\n- #29738: Perhaps symbolic expressions should not have a length",
    "created_at": "2021-07-06T09:53:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28053#issuecomment-439376",
    "user": "https://github.com/slel"
}
```

<div id="comment:9" align="right">Comment 9</div>

Related:

- #32144: Add tree_size method to symbolic expressions
- #29738: Perhaps symbolic expressions should not have a length



---

archive/issue_comments_439377.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nFollowup ticket: I opened #32148 to change `len(f)` to `f.length()`.",
    "created_at": "2021-07-07T01:47:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28053",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28053#issuecomment-439377",
    "user": "https://github.com/DaveWitteMorris"
}
```

<div id="comment:10" align="right">Comment 10</div>

Followup ticket: I opened #32148 to change `len(f)` to `f.length()`.
