# Issue 28654: A Bug in the backend `field`

archive/issues_028417.json:
```json
{
    "assignees": [],
    "body": "The backend `field` have the following bug:\n\n```\nsage: Polyhedron(lines=[[1]], backend='field')\nThe empty polyhedron in QQ^1\n```\n\nThis is fixed by fixing #17339. We add a doctest verifying the fix.\n\nDepends on #17339\n\nCC:  @jplab @kliem @videlec\n\nKeywords: **polytopes, fields, empty, lines**\n\nBranch/Commit: **[b8543fe](https://github.com/sagemath/sagetrac-mirror/commit/b8543feda019c2b350da32fa811e86c2c17b1352)**\n\nReviewer: **Laith Rastanawi**\n\nAuthor: **Jonathan Kliem**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28654_\n\n",
    "closed_at": "2019-12-25T19:09:30Z",
    "created_at": "2019-10-25T08:09:06Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "A Bug in the backend `field`",
    "type": "issue",
    "updated_at": "2019-12-25T19:09:30Z",
    "url": "https://github.com/sagemath/sage/issues/28654",
    "user": "https://github.com/LaisRast"
}
```
The backend `field` have the following bug:

```
sage: Polyhedron(lines=[[1]], backend='field')
The empty polyhedron in QQ^1
```

This is fixed by fixing #17339. We add a doctest verifying the fix.

Depends on #17339

CC:  @jplab @kliem @videlec

Keywords: **polytopes, fields, empty, lines**

Branch/Commit: **[b8543fe](https://github.com/sagemath/sagetrac-mirror/commit/b8543feda019c2b350da32fa811e86c2c17b1352)**

Reviewer: **Laith Rastanawi**

Author: **Jonathan Kliem**

_Issue created by migration from https://trac.sagemath.org/ticket/28654_





---

archive/issue_events_375385.json:
```json
{
    "actor": "https://github.com/LaisRast",
    "created_at": "2019-10-25T08:09:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28654",
    "milestone_number": null,
    "milestone_title": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28654#event-375385"
}
```



---

archive/issue_events_375386.json:
```json
{
    "actor": "https://github.com/LaisRast",
    "created_at": "2019-10-25T08:09:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28654",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "component: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28654#event-375386"
}
```



---

archive/issue_events_375387.json:
```json
{
    "actor": "https://github.com/LaisRast",
    "created_at": "2019-10-25T08:09:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28654",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28654#event-375387"
}
```



---

archive/issue_events_375388.json:
```json
{
    "actor": "https://github.com/LaisRast",
    "created_at": "2019-10-25T08:09:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28654",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28654#event-375388"
}
```



---

archive/issue_comments_449945.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nIt seems to be related to #17339.\n\nWhen one tries to fix this, the verification that checks\n\n```\nPolyhedron(ieqs=[], ambient_dim=1) == Polyhedron(lines=[[1]])\n```\nin the class `Hrep2Vrep` in `double_description_inhomogeneous.py` will fail.",
    "created_at": "2019-10-25T13:46:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28654",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28654#issuecomment-449945",
    "user": "https://github.com/kliem"
}
```

<a id='comment:1'>Comment 1:</a>
It seems to be related to #17339.

When one tries to fix this, the verification that checks

```
Polyhedron(ieqs=[], ambient_dim=1) == Polyhedron(lines=[[1]])
```
in the class `Hrep2Vrep` in `double_description_inhomogeneous.py` will fail.



---

archive/issue_comments_449946.json:
```json
{
    "body": "Dependencies: **#17339**",
    "created_at": "2019-12-11T15:07:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28654",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28654#issuecomment-449946",
    "user": "https://github.com/kliem"
}
```

Dependencies: **#17339**



---

archive/issue_comments_449947.json:
```json
{
    "body": "Author: **Jonathan Kliem**",
    "created_at": "2019-12-11T15:07:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28654",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28654#issuecomment-449947",
    "user": "https://github.com/kliem"
}
```

Author: **Jonathan Kliem**



---

archive/issue_events_375389.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2019-12-11T15:07:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28654",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28654#event-375389"
}
```



---

archive/issue_comments_449948.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -4,3 +4,5 @@\n sage: Polyhedron(lines=[[1]], backend='field')\n The empty polyhedron in QQ^1\n ```\n+\n+This is fixed by fixing #17339. We add a doctest verifying the fix.\n``````\n",
    "created_at": "2019-12-11T15:07:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28654",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28654#issuecomment-449948",
    "user": "https://github.com/kliem"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -4,3 +4,5 @@
 sage: Polyhedron(lines=[[1]], backend='field')
 The empty polyhedron in QQ^1
 ```
+
+This is fixed by fixing #17339. We add a doctest verifying the fix.
``````




---

archive/issue_comments_449949.json:
```json
{
    "body": "Branch: **[public/28654](https://github.com/sagemath/sagetrac-mirror/tree/public/28654)**",
    "created_at": "2019-12-11T15:07:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28654",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28654#issuecomment-449949",
    "user": "https://github.com/kliem"
}
```

Branch: **[public/28654](https://github.com/sagemath/sagetrac-mirror/tree/public/28654)**



---

archive/issue_comments_449950.json:
```json
{
    "body": "Commit: **[b8543fe](https://github.com/sagemath/sagetrac-mirror/commit/b8543feda019c2b350da32fa811e86c2c17b1352)**",
    "created_at": "2019-12-11T15:07:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28654",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28654#issuecomment-449950",
    "user": "https://github.com/kliem"
}
```

Commit: **[b8543fe](https://github.com/sagemath/sagetrac-mirror/commit/b8543feda019c2b350da32fa811e86c2c17b1352)**



---

archive/issue_comments_449951.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b8543feda019c2b350da32fa811e86c2c17b1352\">b8543fe</a></td><td><code>backend field treats polyhedra that are universe correctly</code></td></tr></table>\n",
    "created_at": "2019-12-11T15:07:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28654",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28654#issuecomment-449951",
    "user": "https://github.com/kliem"
}
```

<a id='comment:3'>Comment 3:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b8543feda019c2b350da32fa811e86c2c17b1352">b8543fe</a></td><td><code>backend field treats polyhedra that are universe correctly</code></td></tr></table>




---

archive/issue_events_375390.json:
```json
{
    "actor": "https://github.com/LaisRast",
    "created_at": "2019-12-17T11:29:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28654",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28654#event-375390"
}
```



---

archive/issue_events_375391.json:
```json
{
    "actor": "https://github.com/LaisRast",
    "created_at": "2019-12-17T11:29:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28654",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28654#event-375391"
}
```



---

archive/issue_comments_449952.json:
```json
{
    "body": "Reviewer: **Laith Rastanawi**",
    "created_at": "2019-12-17T11:29:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28654",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28654#issuecomment-449952",
    "user": "https://github.com/LaisRast"
}
```

Reviewer: **Laith Rastanawi**



---

archive/issue_comments_449953.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\n#17339 fixed the problem.\nSince #17339 is on positive review, this also should be on positive review.",
    "created_at": "2019-12-17T11:29:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28654",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28654#issuecomment-449953",
    "user": "https://github.com/LaisRast"
}
```

<a id='comment:4'>Comment 4:</a>
#17339 fixed the problem.
Since #17339 is on positive review, this also should be on positive review.



---

archive/issue_comments_449954.json:
```json
{
    "body": "Changed branch from **[public/28654](https://github.com/sagemath/sagetrac-mirror/tree/public/28654)** to **[b8543fe](https://github.com/sagemath/sagetrac-mirror/commit/b8543feda019c2b350da32fa811e86c2c17b1352)**",
    "created_at": "2019-12-25T19:09:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28654",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28654#issuecomment-449954",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/28654](https://github.com/sagemath/sagetrac-mirror/tree/public/28654)** to **[b8543fe](https://github.com/sagemath/sagetrac-mirror/commit/b8543feda019c2b350da32fa811e86c2c17b1352)**



---

archive/issue_events_375392.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-12-25T19:09:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28654",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28654#event-375392"
}
```



---

archive/issue_events_375393.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "8da01cc05d2a8fa29a90d58b8510b52d9c6b9926",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-12-25T19:09:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28654",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28654#event-375393"
}
```
