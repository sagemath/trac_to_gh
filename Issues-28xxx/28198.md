...
```json
{
    "body": "<a id='comment:1'></a>If you have in mind a smarter way to handle initialization, please share it. The branch is in public, so you can implement it directly.\n\n---\nNew commits:",
    "created_at": "2019-07-14T19:58:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28198",
    "type": "issue_comment",
...
<a id='comment:1'></a>If you have in mind a smarter way to handle initialization, please share it. The branch is in public, so you can implement it directly.
...
<a id='comment:2'></a>Insteed of having a weird and mostly useless special case
...
```json
{
    "body": "<a id='comment:5'></a>I agree with your improvement. I did a small correction to avoid this issue\n\n```\nsage: a = b = set()\nsage: a.add(1)\nsage: b\n{1}\n```\n\nFor me this ticket is good to go. I let you conclude. Thank you.",
...
<a id='comment:5'></a>I agree with your improvement. I did a small correction to avoid this issue
...
<a id='comment:6'></a>- I don't like so much the duplication in the `BipartiteGraph.__init__`.
- The `sets_from_colors` would be more natural if it would return `ans` instead of `ans.values()`. That is it would transform a dictionary `k -> v` in `v -> {keys with that values}`.
...
```json
{
    "body": "<a id='comment:7'></a>Sure, the branch is in public so that we can interact (and I'm not the author of the commit \"proposition of simplification for 28198\")",
...
<a id='comment:7'></a>Sure, the branch is in public so that we can interact (and I'm not the author of the commit "proposition of simplification for 28198")
...
```json
{
    "body": "<a id='comment:9'></a>I like it better now :-)",
...
<a id='comment:9'></a>I like it better now :-)
...
```json
{
    "body": "<a id='comment:12'></a>we can simplify more putting the content of `sets_from_colors` inside `_upgrade_from_graph`, no ?",
...
<a id='comment:12'></a>we can simplify more putting the content of `sets_from_colors` inside `_upgrade_from_graph`, no ?
...
```json
{
    "body": "<a id='comment:13'></a>make sense",
...
```json
{
    "body": "<a id='comment:15'></a>Should be good now.",
...
<a id='comment:15'></a>Should be good now.
...
```json
{
    "body": "<a id='comment:16'></a>good for me at least",
    "created_at": "2019-07-17T17:03:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28198",
    "type": "issue_comment",
...
<a id='comment:16'></a>good for me at least
