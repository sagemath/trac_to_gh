...
<a id='comment:3'></a>Actually, I've trouble with my proposed solution. I actually tried two possibilities, but with difficulties in both cases:
...
```json
{
    "body": "<a id='comment:4'></a>Would it be hard to check if the term order is weighted, and return a multivariate ring in a single variable or a univariate ring depending on that?",
...
<a id='comment:4'></a>Would it be hard to check if the term order is weighted, and return a multivariate ring in a single variable or a univariate ring depending on that?
