# Issue 28146: The Input Kernel for the Isogeny of Elliptic Curves does not accept points in the Torsion Subgroup

archive/issues_027909.json:
```json
{
    "assignees": [],
    "body": "The ``EllipticCurveIsogeny`` class only accepts kernels formed by points in the elliptic curve, but not points in the Torsion subgroup, even though they are the same points. \n\nFor example one should be able to create an elliptic curve isogeny where the kernel is taken directly from the torsion subgroup rather than the abelian group of the elliptic curve:\n\n\n```     \nsage: E = EllipticCurve('11a')\nsage: P = E.torsion_subgroup().gens()[0]\nsage: EllipticCurveIsogeny(E, kernel=P)\nIsogeny of degree 5 from Elliptic Curve defined by y^2 + y = x^3 - x^2 - 10*x - 20 over Rational Field to Elliptic Curve defined by y^2 + y = x^3 - x^2 - 7820*x - 263580 over Rational Field\nsage: EllipticCurveIsogeny(E, kernel=E(P)) == EllipticCurveIsogeny(E, kernel=P)\nTrue\n```\n\n\nKeywords: **Elliptic Curves, Isogenies, Kernel**\n\nBranch/Commit: **[u/menosgeze/the_input_kernel_for_the_isogeny_of_elliptic_curves_does_not_accept_points_in_the_torsion_subgroup](https://github.com/sagemath/sagetrac-mirror/tree/u/menosgeze/the_input_kernel_for_the_isogeny_of_elliptic_curves_does_not_accept_points_in_the_torsion_subgroup) @ [09c2462](https://github.com/sagemath/sagetrac-mirror/commit/09c24625e2cf31e02d35f7ad7425b458ecad3af7)**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28146_\n\n",
    "created_at": "2019-07-09T17:37:30Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20elliptic%20curves",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "The Input Kernel for the Isogeny of Elliptic Curves does not accept points in the Torsion Subgroup",
    "type": "issue",
    "updated_at": "2022-09-19T18:58:47Z",
    "url": "https://github.com/sagemath/sage/issues/28146",
    "user": "https://github.com/sagetrac-menosgeze"
}
```
The ``EllipticCurveIsogeny`` class only accepts kernels formed by points in the elliptic curve, but not points in the Torsion subgroup, even though they are the same points. 

For example one should be able to create an elliptic curve isogeny where the kernel is taken directly from the torsion subgroup rather than the abelian group of the elliptic curve:


```     
sage: E = EllipticCurve('11a')
sage: P = E.torsion_subgroup().gens()[0]
sage: EllipticCurveIsogeny(E, kernel=P)
Isogeny of degree 5 from Elliptic Curve defined by y^2 + y = x^3 - x^2 - 10*x - 20 over Rational Field to Elliptic Curve defined by y^2 + y = x^3 - x^2 - 7820*x - 263580 over Rational Field
sage: EllipticCurveIsogeny(E, kernel=E(P)) == EllipticCurveIsogeny(E, kernel=P)
True
```


Keywords: **Elliptic Curves, Isogenies, Kernel**

Branch/Commit: **[u/menosgeze/the_input_kernel_for_the_isogeny_of_elliptic_curves_does_not_accept_points_in_the_torsion_subgroup](https://github.com/sagemath/sagetrac-mirror/tree/u/menosgeze/the_input_kernel_for_the_isogeny_of_elliptic_curves_does_not_accept_points_in_the_torsion_subgroup) @ [09c2462](https://github.com/sagemath/sagetrac-mirror/commit/09c24625e2cf31e02d35f7ad7425b458ecad3af7)**

_Issue created by migration from https://trac.sagemath.org/ticket/28146_





---

archive/issue_events_367759.json:
```json
{
    "actor": "https://github.com/sagetrac-menosgeze",
    "created_at": "2019-07-09T17:37:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28146#event-367759"
}
```



---

archive/issue_events_367760.json:
```json
{
    "actor": "https://github.com/sagetrac-menosgeze",
    "created_at": "2019-07-09T17:37:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20elliptic%20curves",
    "label_color": "0000ff",
    "label_name": "component: elliptic curves",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28146#event-367760"
}
```



---

archive/issue_events_367761.json:
```json
{
    "actor": "https://github.com/sagetrac-menosgeze",
    "created_at": "2019-07-09T17:37:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28146#event-367761"
}
```



---

archive/issue_events_367762.json:
```json
{
    "actor": "https://github.com/sagetrac-menosgeze",
    "created_at": "2019-07-09T17:37:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28146#event-367762"
}
```



---

archive/issue_comments_440888.json:
```json
{
    "body": "Branch: **[u/menosgeze/the_input_kernel_for_the_isogeny_of_elliptic_curves_does_not_accept_points_in_the_torsion_subgroup](https://github.com/sagemath/sagetrac-mirror/tree/u/menosgeze/the_input_kernel_for_the_isogeny_of_elliptic_curves_does_not_accept_points_in_the_torsion_subgroup)**",
    "created_at": "2019-07-09T18:14:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28146#issuecomment-440888",
    "user": "https://github.com/sagetrac-menosgeze"
}
```

Branch: **[u/menosgeze/the_input_kernel_for_the_isogeny_of_elliptic_curves_does_not_accept_points_in_the_torsion_subgroup](https://github.com/sagemath/sagetrac-mirror/tree/u/menosgeze/the_input_kernel_for_the_isogeny_of_elliptic_curves_does_not_accept_points_in_the_torsion_subgroup)**



---

archive/issue_comments_440889.json:
```json
{
    "body": "Commit: **[e327bbd](https://github.com/sagemath/sagetrac-mirror/commit/e327bbd29df8531fea4f31c2b61549a2ae445bf9)**",
    "created_at": "2019-07-09T18:48:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28146#issuecomment-440889",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[e327bbd](https://github.com/sagemath/sagetrac-mirror/commit/e327bbd29df8531fea4f31c2b61549a2ae445bf9)**



---

archive/issue_comments_440890.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e327bbd29df8531fea4f31c2b61549a2ae445bf9\">e327bbd</a></td><td><code>corrected a silly extra parenthesis that I left behind</code></td></tr></table>\n",
    "created_at": "2019-07-09T18:48:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28146#issuecomment-440890",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e327bbd29df8531fea4f31c2b61549a2ae445bf9">e327bbd</a></td><td><code>corrected a silly extra parenthesis that I left behind</code></td></tr></table>




---

archive/issue_comments_440891.json:
```json
{
    "body": "Changed commit from **[e327bbd](https://github.com/sagemath/sagetrac-mirror/commit/e327bbd29df8531fea4f31c2b61549a2ae445bf9)** to **[09c2462](https://github.com/sagemath/sagetrac-mirror/commit/09c24625e2cf31e02d35f7ad7425b458ecad3af7)**",
    "created_at": "2019-07-09T18:59:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28146#issuecomment-440891",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[e327bbd](https://github.com/sagemath/sagetrac-mirror/commit/e327bbd29df8531fea4f31c2b61549a2ae445bf9)** to **[09c2462](https://github.com/sagemath/sagetrac-mirror/commit/09c24625e2cf31e02d35f7ad7425b458ecad3af7)**



---

archive/issue_comments_440892.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/09c24625e2cf31e02d35f7ad7425b458ecad3af7\">09c2462</a></td><td><code>added example to the documentation</code></td></tr></table>\n",
    "created_at": "2019-07-09T18:59:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28146#issuecomment-440892",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/09c24625e2cf31e02d35f7ad7425b458ecad3af7">09c2462</a></td><td><code>added example to the documentation</code></td></tr></table>




---

archive/issue_comments_440893.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,2 +1,10 @@\n The ``EllipticCurveIsogeny`` class only accepts kernels formed by points in the elliptic curve, but not points in the Torsion subgroup, even though they are the same points. \n \n+For example one should be able to create an elliptic curve isogeny where the kernel is taken directly from the torsion subgroup rather than the abelian group of the elliptic curve (see :trac:'28146')::\n+\n+        sage: E = EllipticCurve('11a')\n+        sage: P = E.torsion_subgroup().gens()[0]\n+        sage: EllipticCurveIsogeny(E, kernel=P)\n+        Isogeny of degree 5 from Elliptic Curve defined by y^2 + y = x^3 - x^2 - 10*x - 20 over Rational Field to Elliptic Curve defined by y^2 + y = x^3 - x^2 - 7820*x - 263580 over Rational Field\n+        sage: EllipticCurveIsogeny(E, kernel=E(P)) == EllipticCurveIsogeny(E, kernel=P)\n+        True\n``````\n",
    "created_at": "2019-07-09T19:01:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28146#issuecomment-440893",
    "user": "https://github.com/sagetrac-menosgeze"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,2 +1,10 @@
 The ``EllipticCurveIsogeny`` class only accepts kernels formed by points in the elliptic curve, but not points in the Torsion subgroup, even though they are the same points. 
 
+For example one should be able to create an elliptic curve isogeny where the kernel is taken directly from the torsion subgroup rather than the abelian group of the elliptic curve (see :trac:'28146')::
+
+        sage: E = EllipticCurve('11a')
+        sage: P = E.torsion_subgroup().gens()[0]
+        sage: EllipticCurveIsogeny(E, kernel=P)
+        Isogeny of degree 5 from Elliptic Curve defined by y^2 + y = x^3 - x^2 - 10*x - 20 over Rational Field to Elliptic Curve defined by y^2 + y = x^3 - x^2 - 7820*x - 263580 over Rational Field
+        sage: EllipticCurveIsogeny(E, kernel=E(P)) == EllipticCurveIsogeny(E, kernel=P)
+        True
``````




---

archive/issue_comments_440894.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,9 +2,12 @@\n \n For example one should be able to create an elliptic curve isogeny where the kernel is taken directly from the torsion subgroup rather than the abelian group of the elliptic curve (see :trac:'28146')::\n \n-        sage: E = EllipticCurve('11a')\n+\n+{{{     sage: E = EllipticCurve('11a')\n         sage: P = E.torsion_subgroup().gens()[0]\n         sage: EllipticCurveIsogeny(E, kernel=P)\n         Isogeny of degree 5 from Elliptic Curve defined by y^2 + y = x^3 - x^2 - 10*x - 20 over Rational Field to Elliptic Curve defined by y^2 + y = x^3 - x^2 - 7820*x - 263580 over Rational Field\n         sage: EllipticCurveIsogeny(E, kernel=E(P)) == EllipticCurveIsogeny(E, kernel=P)\n         True\n+}}}\n+\n``````\n",
    "created_at": "2019-07-09T19:02:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28146#issuecomment-440894",
    "user": "https://github.com/sagetrac-menosgeze"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,9 +2,12 @@
 
 For example one should be able to create an elliptic curve isogeny where the kernel is taken directly from the torsion subgroup rather than the abelian group of the elliptic curve (see :trac:'28146')::
 
-        sage: E = EllipticCurve('11a')
+
+{{{     sage: E = EllipticCurve('11a')
         sage: P = E.torsion_subgroup().gens()[0]
         sage: EllipticCurveIsogeny(E, kernel=P)
         Isogeny of degree 5 from Elliptic Curve defined by y^2 + y = x^3 - x^2 - 10*x - 20 over Rational Field to Elliptic Curve defined by y^2 + y = x^3 - x^2 - 7820*x - 263580 over Rational Field
         sage: EllipticCurveIsogeny(E, kernel=E(P)) == EllipticCurveIsogeny(E, kernel=P)
         True
+}}}
+
``````




---

archive/issue_comments_440895.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -3,11 +3,12 @@\n For example one should be able to create an elliptic curve isogeny where the kernel is taken directly from the torsion subgroup rather than the abelian group of the elliptic curve (see :trac:'28146')::\n \n \n-{{{     sage: E = EllipticCurve('11a')\n-        sage: P = E.torsion_subgroup().gens()[0]\n-        sage: EllipticCurveIsogeny(E, kernel=P)\n-        Isogeny of degree 5 from Elliptic Curve defined by y^2 + y = x^3 - x^2 - 10*x - 20 over Rational Field to Elliptic Curve defined by y^2 + y = x^3 - x^2 - 7820*x - 263580 over Rational Field\n-        sage: EllipticCurveIsogeny(E, kernel=E(P)) == EllipticCurveIsogeny(E, kernel=P)\n-        True\n-}}}\n+```     \n+sage: E = EllipticCurve('11a')\n+sage: P = E.torsion_subgroup().gens()[0]\n+sage: EllipticCurveIsogeny(E, kernel=P)\n+Isogeny of degree 5 from Elliptic Curve defined by y^2 + y = x^3 - x^2 - 10*x - 20 over Rational Field to Elliptic Curve defined by y^2 + y = x^3 - x^2 - 7820*x - 263580 over Rational Field\n+sage: EllipticCurveIsogeny(E, kernel=E(P)) == EllipticCurveIsogeny(E, kernel=P)\n+True\n+```\n \n``````\n",
    "created_at": "2019-07-09T19:04:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28146#issuecomment-440895",
    "user": "https://github.com/sagetrac-menosgeze"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -3,11 +3,12 @@
 For example one should be able to create an elliptic curve isogeny where the kernel is taken directly from the torsion subgroup rather than the abelian group of the elliptic curve (see :trac:'28146')::
 
 
-{{{     sage: E = EllipticCurve('11a')
-        sage: P = E.torsion_subgroup().gens()[0]
-        sage: EllipticCurveIsogeny(E, kernel=P)
-        Isogeny of degree 5 from Elliptic Curve defined by y^2 + y = x^3 - x^2 - 10*x - 20 over Rational Field to Elliptic Curve defined by y^2 + y = x^3 - x^2 - 7820*x - 263580 over Rational Field
-        sage: EllipticCurveIsogeny(E, kernel=E(P)) == EllipticCurveIsogeny(E, kernel=P)
-        True
-}}}
+```     
+sage: E = EllipticCurve('11a')
+sage: P = E.torsion_subgroup().gens()[0]
+sage: EllipticCurveIsogeny(E, kernel=P)
+Isogeny of degree 5 from Elliptic Curve defined by y^2 + y = x^3 - x^2 - 10*x - 20 over Rational Field to Elliptic Curve defined by y^2 + y = x^3 - x^2 - 7820*x - 263580 over Rational Field
+sage: EllipticCurveIsogeny(E, kernel=E(P)) == EllipticCurveIsogeny(E, kernel=P)
+True
+```
 
``````




---

archive/issue_comments_440896.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,6 +1,6 @@\n The ``EllipticCurveIsogeny`` class only accepts kernels formed by points in the elliptic curve, but not points in the Torsion subgroup, even though they are the same points. \n \n-For example one should be able to create an elliptic curve isogeny where the kernel is taken directly from the torsion subgroup rather than the abelian group of the elliptic curve (see :trac:'28146')::\n+For example one should be able to create an elliptic curve isogeny where the kernel is taken directly from the torsion subgroup rather than the abelian group of the elliptic curve:\n \n \n ```     \n``````\n",
    "created_at": "2019-07-09T19:05:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28146#issuecomment-440896",
    "user": "https://github.com/sagetrac-menosgeze"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,6 +1,6 @@
 The ``EllipticCurveIsogeny`` class only accepts kernels formed by points in the elliptic curve, but not points in the Torsion subgroup, even though they are the same points. 
 
-For example one should be able to create an elliptic curve isogeny where the kernel is taken directly from the torsion subgroup rather than the abelian group of the elliptic curve (see :trac:'28146')::
+For example one should be able to create an elliptic curve isogeny where the kernel is taken directly from the torsion subgroup rather than the abelian group of the elliptic curve:
 
 
 ```     
``````




---

archive/issue_comments_440897.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nHi. Have you run the doctests? I don't think they would pass: `.torsion_subgroup()` is only defined for curves over the rationals, but the isogeny object is used for curves over any field.",
    "created_at": "2019-07-15T15:58:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28146#issuecomment-440897",
    "user": "https://github.com/defeo"
}
```

<div id="comment:8" align="right">Comment 8</div>

Hi. Have you run the doctests? I don't think they would pass: `.torsion_subgroup()` is only defined for curves over the rationals, but the isogeny object is used for curves over any field.



---

archive/issue_events_367763.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28146#event-367763"
}
```



---

archive/issue_events_367764.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28146#event-367764"
}
```



---

archive/issue_comments_440898.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nTicket retargeted after milestone closed",
    "created_at": "2019-12-30T14:48:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28146#issuecomment-440898",
    "user": "https://github.com/embray"
}
```

<div id="comment:9" align="right">Comment 9</div>

Ticket retargeted after milestone closed



---

archive/issue_comments_440899.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nMoving tickets to milestone sage-9.2 based on a review of last modification date, branch status, and severity.",
    "created_at": "2020-05-01T04:28:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28146#issuecomment-440899",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:10" align="right">Comment 10</div>

Moving tickets to milestone sage-9.2 based on a review of last modification date, branch status, and severity.



---

archive/issue_events_367765.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-05-01T04:28:42Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28146#event-367765"
}
```



---

archive/issue_events_367766.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-05-01T04:28:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28146#event-367766"
}
```



---

archive/issue_events_367767.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28146#event-367767"
}
```



---

archive/issue_events_367768.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28146#event-367768"
}
```



---

archive/issue_events_367769.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28146#event-367769"
}
```



---

archive/issue_events_367770.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28146#event-367770"
}
```



---

archive/issue_comments_440900.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nSetting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28146#issuecomment-440900",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:12" align="right">Comment 12</div>

Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_367771.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28146#event-367771"
}
```



---

archive/issue_events_367772.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28146#event-367772"
}
```



---

archive/issue_events_367773.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28146#event-367773"
}
```



---

archive/issue_events_367774.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28146#event-367774"
}
```



---

archive/issue_events_367775.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28146#event-367775"
}
```



---

archive/issue_events_367776.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28146#event-367776"
}
```



---

archive/issue_events_367777.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28146#event-367777"
}
```



---

archive/issue_events_367778.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28146",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28146#event-367778"
}
```
