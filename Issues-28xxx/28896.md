# Issue 28896: Graphs: Move common methods of backends to CGraph

archive/issues_028659.json:
```json
{
    "assignees": [],
    "body": "There is a few methods that are implemented exactly the same way in dense and sparse graph:\n\n- add_arc\n- has_arc\n- del_all_arcs\n- out_neighbors\n- in_neighbors\n\nAll they do is wrap the corresponding unsafe methods. So it makes sense to move that code all to `CGraph`.\n\nKeywords: **graphs, backends**\n\nBranch/Commit: **[28c028d](https://github.com/sagemath/sagetrac-mirror/commit/28c028d378b75e21badc1e8cdafbe7f0be5248ea)**\n\nReviewer: **David Coudert**\n\nAuthor: **Jonathan Kliem**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28896_\n\n",
    "closed_at": "2019-12-25T19:09:24Z",
    "created_at": "2019-12-18T20:18:03Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Graphs: Move common methods of backends to CGraph",
    "type": "issue",
    "updated_at": "2019-12-25T19:09:24Z",
    "url": "https://github.com/sagemath/sage/issues/28896",
    "user": "https://github.com/kliem"
}
```
There is a few methods that are implemented exactly the same way in dense and sparse graph:

- add_arc
- has_arc
- del_all_arcs
- out_neighbors
- in_neighbors

All they do is wrap the corresponding unsafe methods. So it makes sense to move that code all to `CGraph`.

Keywords: **graphs, backends**

Branch/Commit: **[28c028d](https://github.com/sagemath/sagetrac-mirror/commit/28c028d378b75e21badc1e8cdafbe7f0be5248ea)**

Reviewer: **David Coudert**

Author: **Jonathan Kliem**

_Issue created by migration from https://trac.sagemath.org/ticket/28896_





---

archive/issue_events_378787.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2019-12-18T20:18:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28896",
    "milestone_number": null,
    "milestone_title": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28896#event-378787"
}
```



---

archive/issue_events_378788.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2019-12-18T20:18:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28896",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28896#event-378788"
}
```



---

archive/issue_events_378789.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2019-12-18T20:18:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28896",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28896#event-378789"
}
```



---

archive/issue_events_378790.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2019-12-18T20:18:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28896",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28896#event-378790"
}
```



---

archive/issue_comments_454036.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/28c028d378b75e21badc1e8cdafbe7f0be5248ea\">28c028d</a></td><td><code>move common methods of graph backends to CGraph</code></td></tr></table>\n",
    "created_at": "2019-12-18T20:27:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28896",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28896#issuecomment-454036",
    "user": "https://github.com/kliem"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/28c028d378b75e21badc1e8cdafbe7f0be5248ea">28c028d</a></td><td><code>move common methods of graph backends to CGraph</code></td></tr></table>




---

archive/issue_comments_454037.json:
```json
{
    "body": "Branch: **[public/28896](https://github.com/sagemath/sagetrac-mirror/tree/public/28896)**",
    "created_at": "2019-12-18T20:27:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28896",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28896#issuecomment-454037",
    "user": "https://github.com/kliem"
}
```

Branch: **[public/28896](https://github.com/sagemath/sagetrac-mirror/tree/public/28896)**



---

archive/issue_comments_454038.json:
```json
{
    "body": "Commit: **[28c028d](https://github.com/sagemath/sagetrac-mirror/commit/28c028d378b75e21badc1e8cdafbe7f0be5248ea)**",
    "created_at": "2019-12-18T20:27:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28896",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28896#issuecomment-454038",
    "user": "https://github.com/kliem"
}
```

Commit: **[28c028d](https://github.com/sagemath/sagetrac-mirror/commit/28c028d378b75e21badc1e8cdafbe7f0be5248ea)**



---

archive/issue_events_378791.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2019-12-18T20:27:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28896",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28896#event-378791"
}
```



---

archive/issue_comments_454039.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLGTM.",
    "created_at": "2019-12-20T11:44:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28896",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28896#issuecomment-454039",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:3" align="right">Comment 3</div>

LGTM.



---

archive/issue_comments_454040.json:
```json
{
    "body": "Reviewer: **David Coudert**",
    "created_at": "2019-12-20T11:44:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28896",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28896#issuecomment-454040",
    "user": "https://github.com/dcoudert"
}
```

Reviewer: **David Coudert**



---

archive/issue_events_378792.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-12-20T11:44:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28896",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28896#event-378792"
}
```



---

archive/issue_events_378793.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-12-20T11:44:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28896",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28896#event-378793"
}
```



---

archive/issue_comments_454041.json:
```json
{
    "body": "Changed branch from **[public/28896](https://github.com/sagemath/sagetrac-mirror/tree/public/28896)** to **[28c028d](https://github.com/sagemath/sagetrac-mirror/commit/28c028d378b75e21badc1e8cdafbe7f0be5248ea)**",
    "created_at": "2019-12-25T19:09:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28896",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28896#issuecomment-454041",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/28896](https://github.com/sagemath/sagetrac-mirror/tree/public/28896)** to **[28c028d](https://github.com/sagemath/sagetrac-mirror/commit/28c028d378b75e21badc1e8cdafbe7f0be5248ea)**



---

archive/issue_events_378794.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-12-25T19:09:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28896",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28896#event-378794"
}
```



---

archive/issue_events_378795.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "d08e04849b820bf6a16f0d7767df57e0186dd44a",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-12-25T19:09:24Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28896",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28896#event-378795"
}
```
