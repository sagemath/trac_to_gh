# Issue 28676: Pass --with-python=sage-python23 to Singular configure

archive/issues_028439.json:
```json
{
    "body": "CC:  @isuruf @slel @dimpase\n\nKeywords: Singular\n\nEither singular should depend on the python package\nor `--without-python` should be passed to `configure`. Otherwise the configure script may find some unexpected python version.\n\nSee sage-release discussion:\n\n- https://groups.google.com/d/msg/sage-release/wL-vwH-5GIk/3YBJMuSKAgAJ\n\nIssue created by migration from https://trac.sagemath.org/ticket/28676\n\n",
    "closed_at": "2019-12-28T22:53:21Z",
    "created_at": "2019-10-30T00:47:24Z",
    "labels": [
        "component: build",
        "critical",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.0",
    "title": "Pass --with-python=sage-python23 to Singular configure",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/28676",
    "user": "https://github.com/slel"
}
```
CC:  @isuruf @slel @dimpase

Keywords: Singular

Either singular should depend on the python package
or `--without-python` should be passed to `configure`. Otherwise the configure script may find some unexpected python version.

See sage-release discussion:

- https://groups.google.com/d/msg/sage-release/wL-vwH-5GIk/3YBJMuSKAgAJ

Issue created by migration from https://trac.sagemath.org/ticket/28676





---

archive/issue_comments_401273.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2019-12-28T03:02:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28676",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28676#issuecomment-401273",
    "user": "https://github.com/mkoeppe"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_401274.json:
```json
{
    "body": "This is a problem for anyone who has a `python2.7` binary somewhere in `PATH`, for example in `/usr/local/bin`, coming from homebrew. \n\nWith the simple patch on this ticket, in a Python 3 build of Sage, this now gives:\n\n```\n[singular-4.1.1p2.p0] checking for --with-python... sage-python23\n[singular-4.1.1p2.p0] checking whether sage-python23 version >= 2.4... yes\n[singular-4.1.1p2.p0] checking whether sage-python23 version >= 3... too recent, skipping python interface!\n```\n\n---\nNew commits:",
    "created_at": "2019-12-28T03:02:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28676",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28676#issuecomment-401274",
    "user": "https://github.com/mkoeppe"
}
```

This is a problem for anyone who has a `python2.7` binary somewhere in `PATH`, for example in `/usr/local/bin`, coming from homebrew. 

With the simple patch on this ticket, in a Python 3 build of Sage, this now gives:

```
[singular-4.1.1p2.p0] checking for --with-python... sage-python23
[singular-4.1.1p2.p0] checking whether sage-python23 version >= 2.4... yes
[singular-4.1.1p2.p0] checking whether sage-python23 version >= 3... too recent, skipping python interface!
```

---
New commits:



---

archive/issue_comments_401275.json:
```json
{
    "body": "Changing priority from major to critical.",
    "created_at": "2019-12-28T03:02:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28676",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28676#issuecomment-401275",
    "user": "https://github.com/mkoeppe"
}
```

Changing priority from major to critical.



---

archive/issue_comments_401276.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2019-12-28T07:34:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28676",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28676#issuecomment-401276",
    "user": "https://github.com/dimpase"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_401277.json:
```json
{
    "body": "Looks good to me. As far as I understand, it has no effect on sagelib, only on possible use of Singular's python interface outside of sagelib.",
    "created_at": "2019-12-28T07:34:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28676",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28676#issuecomment-401277",
    "user": "https://github.com/dimpase"
}
```

Looks good to me. As far as I understand, it has no effect on sagelib, only on possible use of Singular's python interface outside of sagelib.



---

archive/issue_comments_401278.json:
```json
{
    "body": "Yes, there are no effects on sagelib. And as I remember py3 is not supported, so this interface will probably just die since upstream doesn't seem to be interested in porting it to a more recent python. Not that we should care.",
    "created_at": "2019-12-28T20:37:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28676",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28676#issuecomment-401278",
    "user": "https://github.com/kiwifb"
}
```

Yes, there are no effects on sagelib. And as I remember py3 is not supported, so this interface will probably just die since upstream doesn't seem to be interested in porting it to a more recent python. Not that we should care.



---

archive/issue_comments_401279.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2019-12-28T22:53:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28676",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28676#issuecomment-401279",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_events_072218.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-12-28T22:53:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/28676",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28676#event-72218"
}
```



---

archive/issue_comments_401280.json:
```json
{
    "body": "Follow up in #25993 - where we change it to `--without-python`",
    "created_at": "2020-07-06T18:08:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28676",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28676#issuecomment-401280",
    "user": "https://github.com/mkoeppe"
}
```

Follow up in #25993 - where we change it to `--without-python`
