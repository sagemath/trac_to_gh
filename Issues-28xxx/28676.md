# Issue 28676: Pass --with-python=sage-python23 to Singular configure

archive/issues_028439.json:
```json
{
    "assignees": [],
    "body": "Either singular should depend on the python package\nor `--without-python` should be passed to `configure`. Otherwise the configure script may find some unexpected python version.\n\nSee sage-release discussion:\n\n- https://groups.google.com/d/msg/sage-release/wL-vwH-5GIk/3YBJMuSKAgAJ\n\nCC:  @isuruf @slel @dimpase\n\nKeywords: **Singular**\n\nBranch: **[68327e9](https://github.com/sagemath/sagetrac-mirror/commit/68327e976c9bc65f60debd1a7022ccca9f4fb2a1)**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28676_\n\n",
    "closed_at": "2019-12-28T22:53:21Z",
    "created_at": "2019-10-30T00:47:24Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Pass --with-python=sage-python23 to Singular configure",
    "type": "issue",
    "updated_at": "2020-07-06T18:08:22Z",
    "url": "https://github.com/sagemath/sage/issues/28676",
    "user": "https://github.com/slel"
}
```
Either singular should depend on the python package
or `--without-python` should be passed to `configure`. Otherwise the configure script may find some unexpected python version.

See sage-release discussion:

- https://groups.google.com/d/msg/sage-release/wL-vwH-5GIk/3YBJMuSKAgAJ

CC:  @isuruf @slel @dimpase

Keywords: **Singular**

Branch: **[68327e9](https://github.com/sagemath/sagetrac-mirror/commit/68327e976c9bc65f60debd1a7022ccca9f4fb2a1)**

Reviewer: **Dima Pasechnik**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/28676_





---

archive/issue_events_375636.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-10-30T00:47:24Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28676",
    "milestone_number": null,
    "milestone_title": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28676#event-375636"
}
```



---

archive/issue_events_375637.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-10-30T00:47:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28676",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28676#event-375637"
}
```



---

archive/issue_events_375638.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-10-30T00:47:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28676",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28676#event-375638"
}
```



---

archive/issue_events_375639.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-10-30T00:47:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28676",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28676#event-375639"
}
```



---

archive/issue_comments_450294.json:
```json
{
    "body": "Branch: **[u/mkoeppe/singular-configure-python](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/singular-configure-python)**",
    "created_at": "2019-12-28T02:59:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28676",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28676#issuecomment-450294",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/singular-configure-python](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/singular-configure-python)**



---

archive/issue_events_375640.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-12-28T03:02:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28676",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28676#event-375640"
}
```



---

archive/issue_comments_450295.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nThis is a problem for anyone who has a `python2.7` binary somewhere in `PATH`, for example in `/usr/local/bin`, coming from homebrew. \n\nWith the simple patch on this ticket, in a Python 3 build of Sage, this now gives:\n\n```\n[singular-4.1.1p2.p0] checking for --with-python... sage-python23\n[singular-4.1.1p2.p0] checking whether sage-python23 version >= 2.4... yes\n[singular-4.1.1p2.p0] checking whether sage-python23 version >= 3... too recent, skipping python interface!\n```\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/68327e976c9bc65f60debd1a7022ccca9f4fb2a1\">68327e9</a></td><td><code>28676: Pass --with-python=sage-python23 to Singular configure</code></td></tr></table>\n",
    "created_at": "2019-12-28T03:02:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28676",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28676#issuecomment-450295",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

This is a problem for anyone who has a `python2.7` binary somewhere in `PATH`, for example in `/usr/local/bin`, coming from homebrew. 

With the simple patch on this ticket, in a Python 3 build of Sage, this now gives:

```
[singular-4.1.1p2.p0] checking for --with-python... sage-python23
[singular-4.1.1p2.p0] checking whether sage-python23 version >= 2.4... yes
[singular-4.1.1p2.p0] checking whether sage-python23 version >= 3... too recent, skipping python interface!
```

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/68327e976c9bc65f60debd1a7022ccca9f4fb2a1">68327e9</a></td><td><code>28676: Pass --with-python=sage-python23 to Singular configure</code></td></tr></table>




---

archive/issue_comments_450296.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,5 +1,5 @@\n Either singular should depend on the python package\n-or `--without-python` should be passed to `configure`.\n+or `--without-python` should be passed to `configure`. Otherwise the configure script may find some unexpected python version.\n \n See sage-release discussion:\n \n``````\n",
    "created_at": "2019-12-28T03:02:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28676",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28676#issuecomment-450296",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,5 +1,5 @@
 Either singular should depend on the python package
-or `--without-python` should be passed to `configure`.
+or `--without-python` should be passed to `configure`. Otherwise the configure script may find some unexpected python version.
 
 See sage-release discussion:
 
``````




---

archive/issue_events_375641.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-12-28T03:02:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28676",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28676#event-375641"
}
```



---

archive/issue_events_375642.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-12-28T03:02:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28676",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28676#event-375642"
}
```



---

archive/issue_comments_450297.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2019-12-28T03:02:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28676",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28676#issuecomment-450297",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_events_375643.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-12-28T03:02:24Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/28676",
    "title_is": "Pass --with-python=sage-python23 to Singular configure",
    "title_was": "Clarify Singular dependence on Python",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28676#event-375643"
}
```



---

archive/issue_comments_450298.json:
```json
{
    "body": "Commit: **[68327e9](https://github.com/sagemath/sagetrac-mirror/commit/68327e976c9bc65f60debd1a7022ccca9f4fb2a1)**",
    "created_at": "2019-12-28T03:02:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28676",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28676#issuecomment-450298",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[68327e9](https://github.com/sagemath/sagetrac-mirror/commit/68327e976c9bc65f60debd1a7022ccca9f4fb2a1)**



---

archive/issue_events_375644.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2019-12-28T07:34:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28676",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28676#event-375644"
}
```



---

archive/issue_events_375645.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2019-12-28T07:34:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28676",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28676#event-375645"
}
```



---

archive/issue_comments_450299.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLooks good to me. As far as I understand, it has no effect on sagelib, only on possible use of Singular's python interface outside of sagelib.",
    "created_at": "2019-12-28T07:34:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28676",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28676#issuecomment-450299",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:3" align="right">Comment 3</div>

Looks good to me. As far as I understand, it has no effect on sagelib, only on possible use of Singular's python interface outside of sagelib.



---

archive/issue_comments_450300.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2019-12-28T07:34:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28676",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28676#issuecomment-450300",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_comments_450301.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nYes, there are no effects on sagelib. And as I remember py3 is not supported, so this interface will probably just die since upstream doesn't seem to be interested in porting it to a more recent python. Not that we should care.",
    "created_at": "2019-12-28T20:37:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28676",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28676#issuecomment-450301",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:4" align="right">Comment 4</div>

Yes, there are no effects on sagelib. And as I remember py3 is not supported, so this interface will probably just die since upstream doesn't seem to be interested in porting it to a more recent python. Not that we should care.



---

archive/issue_events_375646.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-12-28T22:53:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28676",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28676#event-375646"
}
```



---

archive/issue_events_375647.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "115dcc63aab21333836c5d3d6247920a3b36ef27",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-12-28T22:53:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28676",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28676#event-375647"
}
```



---

archive/issue_comments_450302.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/singular-configure-python](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/singular-configure-python)** to **[68327e9](https://github.com/sagemath/sagetrac-mirror/commit/68327e976c9bc65f60debd1a7022ccca9f4fb2a1)**",
    "created_at": "2019-12-28T22:53:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28676",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28676#issuecomment-450302",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/singular-configure-python](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/singular-configure-python)** to **[68327e9](https://github.com/sagemath/sagetrac-mirror/commit/68327e976c9bc65f60debd1a7022ccca9f4fb2a1)**



---

archive/issue_comments_450303.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nFollow up in #25993 - where we change it to `--without-python`",
    "created_at": "2020-07-06T18:08:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28676",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28676#issuecomment-450303",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:6" align="right">Comment 6</div>

Follow up in #25993 - where we change it to `--without-python`



---

archive/issue_comments_450304.json:
```json
{
    "body": "Changed commit from **[68327e9](https://github.com/sagemath/sagetrac-mirror/commit/68327e976c9bc65f60debd1a7022ccca9f4fb2a1)** to none",
    "created_at": "2020-07-06T18:08:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28676",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28676#issuecomment-450304",
    "user": "https://github.com/mkoeppe"
}
```

Changed commit from **[68327e9](https://github.com/sagemath/sagetrac-mirror/commit/68327e976c9bc65f60debd1a7022ccca9f4fb2a1)** to none
