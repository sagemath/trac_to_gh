# Issue 28452: Add weighted degrees to InfinitePolynomialRing

archive/issues_028215.json:
```json
{
    "assignees": [],
    "body": "For an infinite polynomial ring of the form k[x_0,x_1,...] it is useful to have degree(x_n) = n (or n+1 to have finite dimensional graded components). This is quite common in the study of arc spaces for example. A quick look at sage/rings/polynomial/infinite_polynomial.py and it seems that this should be easily implemented. I am forcing this degree scheme with the attached patch. It would be useful to have an implementation where one passes a weight (say k) for each variable (say x) and then one has degree(x_n) = k+n.\n\nCC:  potuz@potuz.net\n\nKeywords: **InfinitePolynomialRing TermOrder**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28452_\n\n",
    "created_at": "2019-09-03T13:02:19Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add weighted degrees to InfinitePolynomialRing",
    "type": "issue",
    "updated_at": "2022-09-19T18:58:47Z",
    "url": "https://github.com/sagemath/sage/issues/28452",
    "user": "https://github.com/potuz"
}
```
For an infinite polynomial ring of the form k[x_0,x_1,...] it is useful to have degree(x_n) = n (or n+1 to have finite dimensional graded components). This is quite common in the study of arc spaces for example. A quick look at sage/rings/polynomial/infinite_polynomial.py and it seems that this should be easily implemented. I am forcing this degree scheme with the attached patch. It would be useful to have an implementation where one passes a weight (say k) for each variable (say x) and then one has degree(x_n) = k+n.

CC:  potuz@potuz.net

Keywords: **InfinitePolynomialRing TermOrder**

_Issue created by migration from https://trac.sagemath.org/ticket/28452_





---

archive/issue_events_359206.json:
```json
{
    "actor": "https://github.com/potuz",
    "created_at": "2019-09-03T13:02:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28452",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28452#event-359206"
}
```



---

archive/issue_events_359207.json:
```json
{
    "actor": "https://github.com/potuz",
    "created_at": "2019-09-03T13:02:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28452",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28452#event-359207"
}
```



---

archive/issue_events_359208.json:
```json
{
    "actor": "https://github.com/potuz",
    "created_at": "2019-09-03T13:02:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28452",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28452#event-359208"
}
```



---

archive/issue_events_359209.json:
```json
{
    "actor": "https://github.com/potuz",
    "created_at": "2019-09-03T13:02:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28452",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28452#event-359209"
}
```



---

archive/issue_comments_446210.json:
```json
{
    "body": "Attachment: **[infinite_polynomial_ring.patch.gz](https://github.com/sagemath/sage/files/ticket28452/infinite_polynomial_ring.patch.gz)**",
    "created_at": "2019-09-03T13:02:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28452",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28452#issuecomment-446210",
    "user": "https://github.com/potuz"
}
```

Attachment: **[infinite_polynomial_ring.patch.gz](https://github.com/sagemath/sage/files/ticket28452/infinite_polynomial_ring.patch.gz)**



---

archive/issue_comments_446211.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nTicket retargeted after milestone closed",
    "created_at": "2019-12-30T14:48:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28452",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28452#issuecomment-446211",
    "user": "https://github.com/embray"
}
```

<a id='comment:1'>Comment 1:</a>
Ticket retargeted after milestone closed



---

archive/issue_events_359210.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28452",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28452#event-359210"
}
```



---

archive/issue_events_359211.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28452",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28452#event-359211"
}
```



---

archive/issue_events_359212.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28452",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28452#event-359212"
}
```



---

archive/issue_events_359213.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28452",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28452#event-359213"
}
```



---

archive/issue_comments_446212.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBatch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.",
    "created_at": "2020-04-14T19:41:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28452",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28452#issuecomment-446212",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:2'>Comment 2:</a>
Batch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.



---

archive/issue_events_359214.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T16:37:33Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28452",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28452#event-359214"
}
```



---

archive/issue_events_359215.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T16:37:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28452",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28452#event-359215"
}
```



---

archive/issue_events_359216.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28452",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28452#event-359216"
}
```



---

archive/issue_events_359217.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28452",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28452#event-359217"
}
```



---

archive/issue_comments_446213.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nSetting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28452",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28452#issuecomment-446213",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:4'>Comment 4:</a>
Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_359218.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28452",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28452#event-359218"
}
```



---

archive/issue_events_359219.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28452",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28452#event-359219"
}
```



---

archive/issue_events_359220.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28452",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28452#event-359220"
}
```



---

archive/issue_events_359221.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28452",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28452#event-359221"
}
```



---

archive/issue_events_359222.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28452",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28452#event-359222"
}
```



---

archive/issue_events_359223.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28452",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28452#event-359223"
}
```



---

archive/issue_events_359224.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28452",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28452#event-359224"
}
```



---

archive/issue_events_359225.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28452",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28452#event-359225"
}
```
