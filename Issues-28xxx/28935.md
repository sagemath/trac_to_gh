# Issue 28935: infinite recursion in exponential of sparse matrices

archive/issues_028698.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: matrix([[0]], sparse=True).exp()\n...\nRecursionError: maximum recursion depth exceeded while calling a Python object\n```\n\nas reported at [https://ask.sagemath.org/question/49190/exponential-function-does-not-work-for-diagonal-matrices](https://ask.sagemath.org/question/49190/exponential-function-does-not-work-for-diagonal-matrices).\n\n\nBranch: **[1ec3605](https://github.com/sagemath/sagetrac-mirror/commit/1ec360547144f4a5dbcaf0dce960307dd9854dcc)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Markus Wageringel**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28935_\n\n",
    "closed_at": "2020-01-09T23:44:16Z",
    "created_at": "2020-01-01T18:41:13Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "infinite recursion in exponential of sparse matrices",
    "type": "issue",
    "updated_at": "2020-01-24T20:38:07Z",
    "url": "https://github.com/sagemath/sage/issues/28935",
    "user": "https://github.com/mwageringel"
}
```

```
sage: matrix([[0]], sparse=True).exp()
...
RecursionError: maximum recursion depth exceeded while calling a Python object
```

as reported at [https://ask.sagemath.org/question/49190/exponential-function-does-not-work-for-diagonal-matrices](https://ask.sagemath.org/question/49190/exponential-function-does-not-work-for-diagonal-matrices).


Branch: **[1ec3605](https://github.com/sagemath/sagetrac-mirror/commit/1ec360547144f4a5dbcaf0dce960307dd9854dcc)**

Reviewer: **Frédéric Chapoton**

Author: **Markus Wageringel**

_Issue created by migration from https://trac.sagemath.org/ticket/28935_





---

archive/issue_events_379319.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-01-01T18:41:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28935",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28935#event-379319"
}
```



---

archive/issue_events_379320.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-01-01T18:41:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28935",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28935#event-379320"
}
```



---

archive/issue_events_379321.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-01-01T18:41:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28935",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28935#event-379321"
}
```



---

archive/issue_events_379322.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-01-01T18:41:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28935",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28935#event-379322"
}
```



---

archive/issue_comments_454871.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1ec360547144f4a5dbcaf0dce960307dd9854dcc\">1ec3605</a></td><td><code>28935: fix exponential of sparse matrix</code></td></tr></table>\n",
    "created_at": "2020-01-01T20:06:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28935#issuecomment-454871",
    "user": "https://github.com/mwageringel"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1ec360547144f4a5dbcaf0dce960307dd9854dcc">1ec3605</a></td><td><code>28935: fix exponential of sparse matrix</code></td></tr></table>




---

archive/issue_comments_454872.json:
```json
{
    "body": "Commit: **[1ec3605](https://github.com/sagemath/sagetrac-mirror/commit/1ec360547144f4a5dbcaf0dce960307dd9854dcc)**",
    "created_at": "2020-01-01T20:06:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28935#issuecomment-454872",
    "user": "https://github.com/mwageringel"
}
```

Commit: **[1ec3605](https://github.com/sagemath/sagetrac-mirror/commit/1ec360547144f4a5dbcaf0dce960307dd9854dcc)**



---

archive/issue_events_379323.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2020-01-01T20:06:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28935",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28935#event-379323"
}
```



---

archive/issue_comments_454873.json:
```json
{
    "body": "Branch: **[u/gh-mwageringel/28935](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/28935)**",
    "created_at": "2020-01-01T20:06:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28935#issuecomment-454873",
    "user": "https://github.com/mwageringel"
}
```

Branch: **[u/gh-mwageringel/28935](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/28935)**



---

archive/issue_comments_454874.json:
```json
{
    "body": "Author: **Markus Wageringel**",
    "created_at": "2020-01-01T20:06:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28935#issuecomment-454874",
    "user": "https://github.com/mwageringel"
}
```

Author: **Markus Wageringel**



---

archive/issue_events_379324.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-01-02T20:17:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28935",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28935#event-379324"
}
```



---

archive/issue_events_379325.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-01-02T20:17:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28935",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28935#event-379325"
}
```



---

archive/issue_comments_454875.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2020-01-02T20:17:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28935#issuecomment-454875",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_454876.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nok, as a bandaid",
    "created_at": "2020-01-02T20:17:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28935#issuecomment-454876",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

ok, as a bandaid



---

archive/issue_comments_454877.json:
```json
{
    "body": "Changed branch from **[u/gh-mwageringel/28935](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/28935)** to **[1ec3605](https://github.com/sagemath/sagetrac-mirror/commit/1ec360547144f4a5dbcaf0dce960307dd9854dcc)**",
    "created_at": "2020-01-09T23:44:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28935#issuecomment-454877",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/gh-mwageringel/28935](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/28935)** to **[1ec3605](https://github.com/sagemath/sagetrac-mirror/commit/1ec360547144f4a5dbcaf0dce960307dd9854dcc)**



---

archive/issue_events_379326.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-01-09T23:44:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28935",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28935#event-379326"
}
```



---

archive/issue_events_379327.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "ba2f9499614dc37ea32b879228cea17f5e239028",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-01-09T23:44:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28935",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28935#event-379327"
}
```



---

archive/issue_comments_454878.json:
```json
{
    "body": "Changed commit from **[1ec3605](https://github.com/sagemath/sagetrac-mirror/commit/1ec360547144f4a5dbcaf0dce960307dd9854dcc)** to none",
    "created_at": "2020-01-24T20:38:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28935#issuecomment-454878",
    "user": "https://github.com/kcrisman"
}
```

Changed commit from **[1ec3605](https://github.com/sagemath/sagetrac-mirror/commit/1ec360547144f4a5dbcaf0dce960307dd9854dcc)** to none



---

archive/issue_comments_454879.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nSee also #12101 and some of the comments there - bandaid indeed, due to heavy computation and perhaps people don't want denseness.",
    "created_at": "2020-01-24T20:38:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28935",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28935#issuecomment-454879",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:4" align="right">Comment 4</div>

See also #12101 and some of the comments there - bandaid indeed, due to heavy computation and perhaps people don't want denseness.
