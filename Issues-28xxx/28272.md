# Issue 28272: Can't scale vector space morphisms on the left

archive/issues_028035.json:
```json
{
    "assignees": [],
    "body": "Presumably this should work, since somebody implemented it on the right,\n\n```\nsage: from sage.modules.vector_space_morphism import VectorSpaceMorphism\nsage: V = VectorSpace(QQ,2)\nsage: f = VectorSpaceMorphism(V.Hom(V), identity_matrix(QQ,2))\nsage: f*(1/2)\nVector space morphism represented by the matrix:\n[1/2   0]\n[  0 1/2]\nDomain: Vector space of dimension 2 over Rational Field\nCodomain: Vector space of dimension 2 over Rational Field\n```\n\njust not on the left:\n\n```\nsage: (1/2)*f\n---------------------------------------------------------------------------\nTypeError                                 Traceback (most recent call last)\n...\nTypeError: unsupported operand parent(s) for *: 'Rational Field' and 'Set of Morphisms (Linear Transformations) from Vector space of dimension 2 over Rational Field to Vector space of dimension 2 over Rational Field'\n```\n\nCC:  @tscrim\n\nBranch/Commit: **[public/linear_algebra/vector_space_morphism_action-28272](https://github.com/sagemath/sagetrac-mirror/tree/public/linear_algebra/vector_space_morphism_action-28272) @ [4e1f821](https://github.com/sagemath/sagetrac-mirror/commit/4e1f82179e1729f2b3b4d3263fdc77ce432e9978)**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28272_\n\n",
    "created_at": "2019-07-27T14:44:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/needs%20work"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Can't scale vector space morphisms on the left",
    "type": "issue",
    "updated_at": "2022-09-19T18:58:47Z",
    "url": "https://github.com/sagemath/sage/issues/28272",
    "user": "https://github.com/orlitzky"
}
```
Presumably this should work, since somebody implemented it on the right,

```
sage: from sage.modules.vector_space_morphism import VectorSpaceMorphism
sage: V = VectorSpace(QQ,2)
sage: f = VectorSpaceMorphism(V.Hom(V), identity_matrix(QQ,2))
sage: f*(1/2)
Vector space morphism represented by the matrix:
[1/2   0]
[  0 1/2]
Domain: Vector space of dimension 2 over Rational Field
Codomain: Vector space of dimension 2 over Rational Field
```

just not on the left:

```
sage: (1/2)*f
---------------------------------------------------------------------------
TypeError                                 Traceback (most recent call last)
...
TypeError: unsupported operand parent(s) for *: 'Rational Field' and 'Set of Morphisms (Linear Transformations) from Vector space of dimension 2 over Rational Field to Vector space of dimension 2 over Rational Field'
```

CC:  @tscrim

Branch/Commit: **[public/linear_algebra/vector_space_morphism_action-28272](https://github.com/sagemath/sagetrac-mirror/tree/public/linear_algebra/vector_space_morphism_action-28272) @ [4e1f821](https://github.com/sagemath/sagetrac-mirror/commit/4e1f82179e1729f2b3b4d3263fdc77ce432e9978)**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/28272_





---

archive/issue_events_369786.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2019-07-27T14:44:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28272#event-369786"
}
```



---

archive/issue_events_369787.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2019-07-27T14:44:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28272#event-369787"
}
```



---

archive/issue_events_369788.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2019-07-27T14:44:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28272#event-369788"
}
```



---

archive/issue_events_369789.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2019-07-27T14:44:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28272#event-369789"
}
```



---

archive/issue_events_369790.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28272#event-369790"
}
```



---

archive/issue_events_369791.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28272#event-369791"
}
```



---

archive/issue_comments_443662.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nTicket retargeted after milestone closed",
    "created_at": "2019-12-30T14:48:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28272#issuecomment-443662",
    "user": "https://github.com/embray"
}
```

<a id='comment:1'>Comment 1:</a>
Ticket retargeted after milestone closed



---

archive/issue_comments_443663.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBatch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.",
    "created_at": "2020-04-14T19:41:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28272#issuecomment-443663",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:2'>Comment 2:</a>
Batch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.



---

archive/issue_events_369792.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28272#event-369792"
}
```



---

archive/issue_events_369793.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28272#event-369793"
}
```



---

archive/issue_events_369794.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T16:37:33Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28272#event-369794"
}
```



---

archive/issue_events_369795.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T16:37:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28272#event-369795"
}
```



---

archive/issue_events_369796.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-08-30T04:15:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28272#event-369796"
}
```



---

archive/issue_comments_443664.json:
```json
{
    "body": "Author: **Travis Scrimshaw**",
    "created_at": "2020-08-30T04:15:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28272#issuecomment-443664",
    "user": "https://github.com/tscrim"
}
```

Author: **Travis Scrimshaw**



---

archive/issue_comments_443665.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nI implemented a `_rmul_` and `_lmul_`. While I was at it, I also changed the `_add_` and `_sub_` to use the coercion model.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/328125f6292de28326dda0e2497c2474aa54c7b7\">328125f</a></td><td><code>Moving vector space morphisms to the coercion model.</code></td></tr></table>\n",
    "created_at": "2020-08-30T04:15:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28272#issuecomment-443665",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:5'>Comment 5:</a>
I implemented a `_rmul_` and `_lmul_`. While I was at it, I also changed the `_add_` and `_sub_` to use the coercion model.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/328125f6292de28326dda0e2497c2474aa54c7b7">328125f</a></td><td><code>Moving vector space morphisms to the coercion model.</code></td></tr></table>




---

archive/issue_comments_443666.json:
```json
{
    "body": "Commit: **[328125f](https://github.com/sagemath/sagetrac-mirror/commit/328125f6292de28326dda0e2497c2474aa54c7b7)**",
    "created_at": "2020-08-30T04:15:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28272#issuecomment-443666",
    "user": "https://github.com/tscrim"
}
```

Commit: **[328125f](https://github.com/sagemath/sagetrac-mirror/commit/328125f6292de28326dda0e2497c2474aa54c7b7)**



---

archive/issue_comments_443667.json:
```json
{
    "body": "Branch: **[public/linear_algebra/vector_space_morphism_action-28272](https://github.com/sagemath/sagetrac-mirror/tree/public/linear_algebra/vector_space_morphism_action-28272)**",
    "created_at": "2020-08-30T04:15:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28272#issuecomment-443667",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/linear_algebra/vector_space_morphism_action-28272](https://github.com/sagemath/sagetrac-mirror/tree/public/linear_algebra/vector_space_morphism_action-28272)**



---

archive/issue_comments_443668.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nthis triggers failing doctests, and worse, segmentation faults",
    "created_at": "2020-08-30T06:52:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28272#issuecomment-443668",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:6'>Comment 6:</a>
this triggers failing doctests, and worse, segmentation faults



---

archive/issue_comments_443669.json:
```json
{
    "body": "Changed commit from **[328125f](https://github.com/sagemath/sagetrac-mirror/commit/328125f6292de28326dda0e2497c2474aa54c7b7)** to **[4e1f821](https://github.com/sagemath/sagetrac-mirror/commit/4e1f82179e1729f2b3b4d3263fdc77ce432e9978)**",
    "created_at": "2020-08-30T07:09:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28272#issuecomment-443669",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[328125f](https://github.com/sagemath/sagetrac-mirror/commit/328125f6292de28326dda0e2497c2474aa54c7b7)** to **[4e1f821](https://github.com/sagemath/sagetrac-mirror/commit/4e1f82179e1729f2b3b4d3263fdc77ce432e9978)**



---

archive/issue_comments_443670.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4e1f82179e1729f2b3b4d3263fdc77ce432e9978\">4e1f821</a></td><td><code>Only doing the minimal change for the left/right action.</code></td></tr></table>\n",
    "created_at": "2020-08-30T07:09:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28272#issuecomment-443670",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:7'>Comment 7:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4e1f82179e1729f2b3b4d3263fdc77ce432e9978">4e1f821</a></td><td><code>Only doing the minimal change for the left/right action.</code></td></tr></table>




---

archive/issue_comments_443671.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nIndeed, debugging will be needed for moving `__add__` and `__sub__` to the coercion model. So here is a version with the minimal change for the left/right action of the base ring. Let's see if that works better.",
    "created_at": "2020-08-30T07:10:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28272#issuecomment-443671",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:8'>Comment 8:</a>
Indeed, debugging will be needed for moving `__add__` and `__sub__` to the coercion model. So here is a version with the minimal change for the left/right action of the base ring. Let's see if that works better.



---

archive/issue_events_369797.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-08-30T07:10:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28272#event-369797"
}
```



---

archive/issue_events_369798.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-08-30T07:10:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28272#event-369798"
}
```



---

archive/issue_events_369799.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-08-30T07:10:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28272#event-369799"
}
```



---

archive/issue_events_369800.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-08-30T07:10:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28272#event-369800"
}
```



---

archive/issue_events_369801.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-30T18:31:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28272#event-369801"
}
```



---

archive/issue_events_369802.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-30T18:31:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28272#event-369802"
}
```



---

archive/issue_comments_443672.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nstill has segfaults.",
    "created_at": "2020-08-30T18:31:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28272#issuecomment-443672",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:10'>Comment 10:</a>
still has segfaults.



---

archive/issue_comments_443673.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nHmm...I don't know. It will take some deeper debugging to figure it out. Unfortunately I don't have time right now to do so. `:/` If someone else can take over, please do so.",
    "created_at": "2020-08-31T05:37:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28272#issuecomment-443673",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:11'>Comment 11:</a>
Hmm...I don't know. It will take some deeper debugging to figure it out. Unfortunately I don't have time right now to do so. `:/` If someone else can take over, please do so.



---

archive/issue_comments_443674.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nSetting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28272#issuecomment-443674",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:12'>Comment 12:</a>
Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_369803.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28272#event-369803"
}
```



---

archive/issue_events_369804.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28272#event-369804"
}
```



---

archive/issue_events_369805.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-22T20:35:10Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28272#event-369805"
}
```



---

archive/issue_events_369806.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-22T20:35:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28272#event-369806"
}
```



---

archive/issue_events_369807.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:24:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28272#event-369807"
}
```



---

archive/issue_events_369808.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:24:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28272#event-369808"
}
```



---

archive/issue_events_369809.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-02T20:27:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28272#event-369809"
}
```



---

archive/issue_events_369810.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-02T20:27:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28272#event-369810"
}
```



---

archive/issue_events_369811.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28272#event-369811"
}
```



---

archive/issue_events_369812.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28272",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28272#event-369812"
}
```
