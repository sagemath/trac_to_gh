# Issue 28560: Ignore newlines in three.js version search

archive/issues_028323.json:
```json
{
    "assignees": [],
    "body": "The regexp-based version number search for three.js introduced in #28086 doesn't work for three.js version r109. The reason is that in the compiled three.min.js there is a line break right in the middle of the regexp match. \n\nThis patch strips newline characters from the file before extracting the version number\n\n\n\nCC:  @isuruf @paulmasson\n\nBranch/Commit: **[6d8e131](https://github.com/sagemath/sagetrac-mirror/commit/6d8e1311f2b5dd6d477fecbf493bbd06e8c1348b)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Antonio Rojas**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28560_\n\n",
    "closed_at": "2019-10-07T22:51:08Z",
    "created_at": "2019-10-05T21:31:15Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Ignore newlines in three.js version search",
    "type": "issue",
    "updated_at": "2019-10-07T22:51:08Z",
    "url": "https://github.com/sagemath/sage/issues/28560",
    "user": "https://github.com/antonio-rojas"
}
```
The regexp-based version number search for three.js introduced in #28086 doesn't work for three.js version r109. The reason is that in the compiled three.min.js there is a line break right in the middle of the regexp match. 

This patch strips newline characters from the file before extracting the version number



CC:  @isuruf @paulmasson

Branch/Commit: **[6d8e131](https://github.com/sagemath/sagetrac-mirror/commit/6d8e1311f2b5dd6d477fecbf493bbd06e8c1348b)**

Reviewer: **Frédéric Chapoton**

Author: **Antonio Rojas**

_Issue created by migration from https://trac.sagemath.org/ticket/28560_





---

archive/issue_events_373831.json:
```json
{
    "actor": "https://github.com/antonio-rojas",
    "created_at": "2019-10-05T21:31:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28560",
    "milestone_number": null,
    "milestone_title": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28560#event-373831"
}
```



---

archive/issue_events_373832.json:
```json
{
    "actor": "https://github.com/antonio-rojas",
    "created_at": "2019-10-05T21:31:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28560",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28560#event-373832"
}
```



---

archive/issue_comments_448195.json:
```json
{
    "body": "Branch: **[u/arojas/ignore_newlines_in_three_js_version_search](https://github.com/sagemath/sagetrac-mirror/tree/u/arojas/ignore_newlines_in_three_js_version_search)**",
    "created_at": "2019-10-05T21:32:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28560",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28560#issuecomment-448195",
    "user": "https://github.com/antonio-rojas"
}
```

Branch: **[u/arojas/ignore_newlines_in_three_js_version_search](https://github.com/sagemath/sagetrac-mirror/tree/u/arojas/ignore_newlines_in_three_js_version_search)**



---

archive/issue_events_373833.json:
```json
{
    "actor": "https://github.com/antonio-rojas",
    "created_at": "2019-10-05T21:36:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28560",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28560#event-373833"
}
```



---

archive/issue_comments_448196.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,5 @@\n+The regexp-based version number search for three.js introduced in #28086 doesn't work for three.js version r109. The reason is that in the compiled three.min.js there is a line break right in the middle of the regexp match. \n \n+This patch strips newline characters from the file before extracting the version number\n+\n+\n``````\n",
    "created_at": "2019-10-05T21:36:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28560",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28560#issuecomment-448196",
    "user": "https://github.com/antonio-rojas"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,5 @@
+The regexp-based version number search for three.js introduced in #28086 doesn't work for three.js version r109. The reason is that in the compiled three.min.js there is a line break right in the middle of the regexp match. 
 
+This patch strips newline characters from the file before extracting the version number
+
+
``````




---

archive/issue_events_373834.json:
```json
{
    "actor": "https://github.com/antonio-rojas",
    "created_at": "2019-10-05T21:36:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28560",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28560#event-373834"
}
```



---

archive/issue_comments_448197.json:
```json
{
    "body": "Author: **Antonio Rojas**",
    "created_at": "2019-10-05T21:36:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28560",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28560#issuecomment-448197",
    "user": "https://github.com/antonio-rojas"
}
```

Author: **Antonio Rojas**



---

archive/issue_comments_448198.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6d8e1311f2b5dd6d477fecbf493bbd06e8c1348b\">6d8e131</a></td><td><code>Strip newlines from three.min.js before extracting the version number</code></td></tr></table>\n",
    "created_at": "2019-10-05T21:36:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28560",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28560#issuecomment-448198",
    "user": "https://github.com/antonio-rojas"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6d8e1311f2b5dd6d477fecbf493bbd06e8c1348b">6d8e131</a></td><td><code>Strip newlines from three.min.js before extracting the version number</code></td></tr></table>




---

archive/issue_comments_448199.json:
```json
{
    "body": "Commit: **[6d8e131](https://github.com/sagemath/sagetrac-mirror/commit/6d8e1311f2b5dd6d477fecbf493bbd06e8c1348b)**",
    "created_at": "2019-10-05T21:36:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28560",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28560#issuecomment-448199",
    "user": "https://github.com/antonio-rojas"
}
```

Commit: **[6d8e131](https://github.com/sagemath/sagetrac-mirror/commit/6d8e1311f2b5dd6d477fecbf493bbd06e8c1348b)**



---

archive/issue_events_373835.json:
```json
{
    "actor": "https://github.com/antonio-rojas",
    "created_at": "2019-10-05T21:36:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28560",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28560#event-373835"
}
```



---

archive/issue_events_373836.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-10-06T07:11:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28560",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28560#event-373836"
}
```



---

archive/issue_events_373837.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-10-06T07:11:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28560",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28560#event-373837"
}
```



---

archive/issue_comments_448200.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nok",
    "created_at": "2019-10-06T07:11:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28560",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28560#issuecomment-448200",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">Comment 3</div>

ok



---

archive/issue_comments_448201.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2019-10-06T07:11:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28560",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28560#issuecomment-448201",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_events_373838.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-10-07T22:51:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28560",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28560#event-373838"
}
```



---

archive/issue_events_373839.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "fcc11d6effa39f375bc5f4ea5831fb7a2f2767da",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-10-07T22:51:08Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28560",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28560#event-373839"
}
```



---

archive/issue_comments_448202.json:
```json
{
    "body": "Changed branch from **[u/arojas/ignore_newlines_in_three_js_version_search](https://github.com/sagemath/sagetrac-mirror/tree/u/arojas/ignore_newlines_in_three_js_version_search)** to **[6d8e131](https://github.com/sagemath/sagetrac-mirror/commit/6d8e1311f2b5dd6d477fecbf493bbd06e8c1348b)**",
    "created_at": "2019-10-07T22:51:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28560",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28560#issuecomment-448202",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/arojas/ignore_newlines_in_three_js_version_search](https://github.com/sagemath/sagetrac-mirror/tree/u/arojas/ignore_newlines_in_three_js_version_search)** to **[6d8e131](https://github.com/sagemath/sagetrac-mirror/commit/6d8e1311f2b5dd6d477fecbf493bbd06e8c1348b)**
