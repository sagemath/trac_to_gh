# Issue 28246: Use SageMathCell to embed interacts examples in the docs

archive/issues_028009.json:
```json
{
    "assignees": [],
    "body": "The official [Interact Quickstart](https://doc.sagemath.org/html/en/prep/Quickstarts/Interact.html) in the documentation lacks example output, making it a bit unimpressive to readers, who might not otherwise be sufficiently motivated to try it themselves.\n\nThe page *does* link to http://wiki.sagemath.org/interact, which shows an interactive example in [SageMathCell](../wiki/SageMathCell), but it would be better if the actual examples in the docs worked interactively as well.\n\nI think it would be straightforward to add a special Sphinx directive for embedding an interactive cell.  Obviously, it would only work when viewing the docs with an internet connection.  For when it doesn't work, it would also be nice if the directive could be supplied a fallback screenshot (ideally, and interactive GIF).  These fallback screenshots would have to be supplied manually, unfortunately.\n\nAdding a special directive would be ideal since it would make it easier to provide interact examples in other parts of the docs as well.\n\nAn even more sophisticated possibility would be a button to turn any examples in the docs into interactive cells.  I feel like this has been tried before, but I don't remember where I've seen it...\n\nCC:  @kwankyu\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28246_\n\n",
    "created_at": "2019-07-24T13:27:05Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Use SageMathCell to embed interacts examples in the docs",
    "type": "issue",
    "updated_at": "2022-08-31T02:51:13Z",
    "url": "https://github.com/sagemath/sage/issues/28246",
    "user": "https://github.com/embray"
}
```
The official [Interact Quickstart](https://doc.sagemath.org/html/en/prep/Quickstarts/Interact.html) in the documentation lacks example output, making it a bit unimpressive to readers, who might not otherwise be sufficiently motivated to try it themselves.

The page *does* link to http://wiki.sagemath.org/interact, which shows an interactive example in [SageMathCell](../wiki/SageMathCell), but it would be better if the actual examples in the docs worked interactively as well.

I think it would be straightforward to add a special Sphinx directive for embedding an interactive cell.  Obviously, it would only work when viewing the docs with an internet connection.  For when it doesn't work, it would also be nice if the directive could be supplied a fallback screenshot (ideally, and interactive GIF).  These fallback screenshots would have to be supplied manually, unfortunately.

Adding a special directive would be ideal since it would make it easier to provide interact examples in other parts of the docs as well.

An even more sophisticated possibility would be a button to turn any examples in the docs into interactive cells.  I feel like this has been tried before, but I don't remember where I've seen it...

CC:  @kwankyu

_Issue created by migration from https://trac.sagemath.org/ticket/28246_





---

archive/issue_events_369301.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-07-24T13:27:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28246",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28246#event-369301"
}
```



---

archive/issue_events_369302.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-07-24T13:27:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28246",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28246#event-369302"
}
```



---

archive/issue_events_369303.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-07-24T13:27:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28246",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28246#event-369303"
}
```



---

archive/issue_events_369304.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-07-24T13:27:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28246",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28246#event-369304"
}
```



---

archive/issue_comments_443185.json:
```json
{
    "body": "Replying to [ticket:28246 embray]:\n> An even more sophisticated possibility would be a button to turn any examples in the docs into interactive cells.  I feel like this has been tried before, but I don't remember where I've seen it...\n\nMaybe this was here: [https://sage-package.readthedocs.io/en/latest/sage_package/thebe.html](https://sage-package.readthedocs.io/en/latest/sage_package/thebe.html). I can definitely see this being helpful.",
    "created_at": "2019-07-25T05:29:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28246#issuecomment-443185",
    "user": "https://github.com/mwageringel"
}
```

Replying to [ticket:28246 embray]:
> An even more sophisticated possibility would be a button to turn any examples in the docs into interactive cells.  I feel like this has been tried before, but I don't remember where I've seen it...

Maybe this was here: [https://sage-package.readthedocs.io/en/latest/sage_package/thebe.html](https://sage-package.readthedocs.io/en/latest/sage_package/thebe.html). I can definitely see this being helpful.



---

archive/issue_events_369305.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28246",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28246#event-369305"
}
```



---

archive/issue_events_369306.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28246",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28246#event-369306"
}
```



---

archive/issue_comments_443186.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nTicket retargeted after milestone closed",
    "created_at": "2019-12-30T14:48:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28246#issuecomment-443186",
    "user": "https://github.com/embray"
}
```

<a id='comment:2'>Comment 2:</a>
Ticket retargeted after milestone closed



---

archive/issue_events_369307.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28246",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28246#event-369307"
}
```



---

archive/issue_events_369308.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28246",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28246#event-369308"
}
```



---

archive/issue_comments_443187.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nBatch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.",
    "created_at": "2020-04-14T19:41:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28246#issuecomment-443187",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:3'>Comment 3:</a>
Batch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.



---

archive/issue_events_369309.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T16:37:33Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28246",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28246#event-369309"
}
```



---

archive/issue_events_369310.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T16:37:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28246",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28246#event-369310"
}
```



---

archive/issue_events_369311.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28246",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28246#event-369311"
}
```



---

archive/issue_events_369312.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28246",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28246#event-369312"
}
```



---

archive/issue_comments_443188.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nSetting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28246#issuecomment-443188",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:5'>Comment 5:</a>
Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_369313.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28246",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28246#event-369313"
}
```



---

archive/issue_events_369314.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28246",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28246#event-369314"
}
```



---

archive/issue_events_369315.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:11:26Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28246",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28246#event-369315"
}
```



---

archive/issue_events_369316.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:11:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28246",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28246#event-369316"
}
```



---

archive/issue_comments_443189.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\n#33320 now fulfills the wishes of this ticket albeit with jupyter-sphinx rather than sagemath-cell.",
    "created_at": "2022-04-03T13:29:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28246#issuecomment-443189",
    "user": "https://github.com/kwankyu"
}
```

<a id='comment:9'>Comment 9:</a>
#33320 now fulfills the wishes of this ticket albeit with jupyter-sphinx rather than sagemath-cell.



---

archive/issue_events_369317.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28246",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28246#event-369317"
}
```



---

archive/issue_events_369318.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-05-03T15:17:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28246",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28246#event-369318"
}
```



---

archive/issue_events_369319.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28246",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28246#event-369319"
}
```



---

archive/issue_events_369320.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28246",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28246#event-369320"
}
```
