# Issue 28993: Adding keyword `prec` to the element constructor of Laurent series

archive/issues_028756.json:
```json
{
    "assignees": [],
    "body": "Currently, we have a keyword `prec` for power series, but not for Laurent series. A consequence of this is that the function `qexp_eta` cannot be applied to Laurent series, as observed in this [blog post](https://oscar.computeralgebra.de/blogs/2018/08/03/PuiseuxSeries/), for example.\n\n```\nsage: L.<q> = LaurentSeriesRing(ZZ)\nsage: qexp_eta(L, prec=30)\nTraceback (most recent call last):\n...\nTypeError: _element_constructor_() got an unexpected keyword argument 'prec'\n```\n\nThis ticket adds the keyword to Laurent series, as well.\n\n\nCC:  @tscrim\n\nKeywords: **Laurent series, Puiseux series**\n\nBranch/Commit: **[87cd023](https://github.com/sagemath/sagetrac-mirror/commit/87cd023005a814a14e5fb1785bdd5cc6c267dfda)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Sebastian Oehms**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28993_\n\n",
    "closed_at": "2020-01-20T21:18:13Z",
    "created_at": "2020-01-12T08:24:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Adding keyword `prec` to the element constructor of Laurent series",
    "type": "issue",
    "updated_at": "2020-01-20T21:18:13Z",
    "url": "https://github.com/sagemath/sage/issues/28993",
    "user": "https://github.com/soehms"
}
```
Currently, we have a keyword `prec` for power series, but not for Laurent series. A consequence of this is that the function `qexp_eta` cannot be applied to Laurent series, as observed in this [blog post](https://oscar.computeralgebra.de/blogs/2018/08/03/PuiseuxSeries/), for example.

```
sage: L.<q> = LaurentSeriesRing(ZZ)
sage: qexp_eta(L, prec=30)
Traceback (most recent call last):
...
TypeError: _element_constructor_() got an unexpected keyword argument 'prec'
```

This ticket adds the keyword to Laurent series, as well.


CC:  @tscrim

Keywords: **Laurent series, Puiseux series**

Branch/Commit: **[87cd023](https://github.com/sagemath/sagetrac-mirror/commit/87cd023005a814a14e5fb1785bdd5cc6c267dfda)**

Reviewer: **Travis Scrimshaw**

Author: **Sebastian Oehms**

_Issue created by migration from https://trac.sagemath.org/ticket/28993_





---

archive/issue_events_366875.json:
```json
{
    "actor": "https://github.com/soehms",
    "created_at": "2020-01-12T08:24:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28993",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28993#event-366875"
}
```



---

archive/issue_events_366876.json:
```json
{
    "actor": "https://github.com/soehms",
    "created_at": "2020-01-12T08:24:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28993",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
    "label_color": "0000ff",
    "label_name": "component: commutative algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28993#event-366876"
}
```



---

archive/issue_events_366877.json:
```json
{
    "actor": "https://github.com/soehms",
    "created_at": "2020-01-12T08:24:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28993",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28993#event-366877"
}
```



---

archive/issue_events_366878.json:
```json
{
    "actor": "https://github.com/soehms",
    "created_at": "2020-01-12T08:24:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28993",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28993#event-366878"
}
```



---

archive/issue_comments_455767.json:
```json
{
    "body": "Branch: **[u/soehms/prec_laurent_elem_constr_28993](https://github.com/sagemath/sagetrac-mirror/tree/u/soehms/prec_laurent_elem_constr_28993)**",
    "created_at": "2020-01-12T10:18:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28993",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28993#issuecomment-455767",
    "user": "https://github.com/soehms"
}
```

Branch: **[u/soehms/prec_laurent_elem_constr_28993](https://github.com/sagemath/sagetrac-mirror/tree/u/soehms/prec_laurent_elem_constr_28993)**



---

archive/issue_comments_455768.json:
```json
{
    "body": "Commit: **[87cd023](https://github.com/sagemath/sagetrac-mirror/commit/87cd023005a814a14e5fb1785bdd5cc6c267dfda)**",
    "created_at": "2020-01-12T10:20:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28993",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28993#issuecomment-455768",
    "user": "https://github.com/soehms"
}
```

Commit: **[87cd023](https://github.com/sagemath/sagetrac-mirror/commit/87cd023005a814a14e5fb1785bdd5cc6c267dfda)**



---

archive/issue_comments_455769.json:
```json
{
    "body": "Author: **Sebastian Oehms**",
    "created_at": "2020-01-12T10:20:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28993",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28993#issuecomment-455769",
    "user": "https://github.com/soehms"
}
```

Author: **Sebastian Oehms**



---

archive/issue_events_366879.json:
```json
{
    "actor": "https://github.com/soehms",
    "created_at": "2020-01-12T10:20:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28993",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28993#event-366879"
}
```



---

archive/issue_comments_455770.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nI realize the application of that keyword appending a call of the `add_bigoh` method in order to share the same code for the same functionality.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/87cd023005a814a14e5fb1785bdd5cc6c267dfda\">87cd023</a></td><td><code>28993: initial version</code></td></tr></table>\n",
    "created_at": "2020-01-12T10:20:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28993",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28993#issuecomment-455770",
    "user": "https://github.com/soehms"
}
```

<a id='comment:2'>Comment 2:</a>
I realize the application of that keyword appending a call of the `add_bigoh` method in order to share the same code for the same functionality.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/87cd023005a814a14e5fb1785bdd5cc6c267dfda">87cd023</a></td><td><code>28993: initial version</code></td></tr></table>




---

archive/issue_comments_455771.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2020-01-12T17:40:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28993",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28993#issuecomment-455771",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_366880.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-01-12T17:40:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28993",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28993#event-366880"
}
```



---

archive/issue_events_366881.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-01-12T17:40:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28993",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28993#event-366881"
}
```



---

archive/issue_comments_455772.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nAll of the logic for handling the precision is done in the `add_bigoh`, so this is a good solution IMO.",
    "created_at": "2020-01-12T17:40:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28993",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28993#issuecomment-455772",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:3'>Comment 3:</a>
All of the logic for handling the precision is done in the `add_bigoh`, so this is a good solution IMO.



---

archive/issue_events_366882.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-01-20T21:18:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28993",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28993#event-366882"
}
```



---

archive/issue_events_366883.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "8966c95bd53221aee94f3e430dea9ec0ef4bdbe0",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-01-20T21:18:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28993",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28993#event-366883"
}
```



---

archive/issue_comments_455773.json:
```json
{
    "body": "Changed branch from **[u/soehms/prec_laurent_elem_constr_28993](https://github.com/sagemath/sagetrac-mirror/tree/u/soehms/prec_laurent_elem_constr_28993)** to **[87cd023](https://github.com/sagemath/sagetrac-mirror/commit/87cd023005a814a14e5fb1785bdd5cc6c267dfda)**",
    "created_at": "2020-01-20T21:18:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28993",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28993#issuecomment-455773",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/soehms/prec_laurent_elem_constr_28993](https://github.com/sagemath/sagetrac-mirror/tree/u/soehms/prec_laurent_elem_constr_28993)** to **[87cd023](https://github.com/sagemath/sagetrac-mirror/commit/87cd023005a814a14e5fb1785bdd5cc6c267dfda)**
