# Issue 28385: tides does not pass self-tests on 32bit system

archive/issues_028148.json:
```json
{
    "assignees": [],
    "body": "tides does not pass self-tests on 32bit system, `sage -f -c tides` produces the error:\n\n```\n[tides-2.0.p0] cc1: error: CPU you selected does not support x86-64 instruction set\n```\n\nThe fix is similar to #26021.\n\n\nCC:  @miguelmarco @jdemeyer @saraedum\n\nKeywords: **sdl**\n\nBranch/Commit: **[6c3a484](https://github.com/sagemath/sagetrac-mirror/commit/6c3a4840fb910e149c93d2ea4b714869f8078c48)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Thierry Monteil**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28385_\n\n",
    "closed_at": "2020-01-22T20:44:02Z",
    "created_at": "2019-08-22T18:14:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "tides does not pass self-tests on 32bit system",
    "type": "issue",
    "updated_at": "2020-01-22T20:44:02Z",
    "url": "https://github.com/sagemath/sage/issues/28385",
    "user": "https://github.com/sagetrac-tmonteil"
}
```
tides does not pass self-tests on 32bit system, `sage -f -c tides` produces the error:

```
[tides-2.0.p0] cc1: error: CPU you selected does not support x86-64 instruction set
```

The fix is similar to #26021.


CC:  @miguelmarco @jdemeyer @saraedum

Keywords: **sdl**

Branch/Commit: **[6c3a484](https://github.com/sagemath/sagetrac-mirror/commit/6c3a4840fb910e149c93d2ea4b714869f8078c48)**

Reviewer: **Frédéric Chapoton**

Author: **Thierry Monteil**

_Issue created by migration from https://trac.sagemath.org/ticket/28385_





---

archive/issue_events_371329.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2019-08-22T18:14:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28385",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28385#event-371329"
}
```



---

archive/issue_events_371330.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2019-08-22T18:14:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28385",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
    "label_color": "000080",
    "label_name": "component: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28385#event-371330"
}
```



---

archive/issue_events_371331.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2019-08-22T18:14:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28385",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28385#event-371331"
}
```



---

archive/issue_events_371332.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2019-08-22T18:14:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28385",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28385#event-371332"
}
```



---

archive/issue_comments_445120.json:
```json
{
    "body": "Branch: **[u/tmonteil/tides_does_not_pass_self_tests_on_32bit_system](https://github.com/sagemath/sagetrac-mirror/tree/u/tmonteil/tides_does_not_pass_self_tests_on_32bit_system)**",
    "created_at": "2019-08-22T18:28:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28385#issuecomment-445120",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Branch: **[u/tmonteil/tides_does_not_pass_self_tests_on_32bit_system](https://github.com/sagemath/sagetrac-mirror/tree/u/tmonteil/tides_does_not_pass_self_tests_on_32bit_system)**



---

archive/issue_comments_445121.json:
```json
{
    "body": "Commit: **[6c3a484](https://github.com/sagemath/sagetrac-mirror/commit/6c3a4840fb910e149c93d2ea4b714869f8078c48)**",
    "created_at": "2019-08-22T18:30:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28385#issuecomment-445121",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Commit: **[6c3a484](https://github.com/sagemath/sagetrac-mirror/commit/6c3a4840fb910e149c93d2ea4b714869f8078c48)**



---

archive/issue_events_371333.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2019-08-22T18:30:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28385",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28385#event-371333"
}
```



---

archive/issue_comments_445122.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6c3a4840fb910e149c93d2ea4b714869f8078c48\">6c3a484</a></td><td><code>#28385 : let tides self-tests pass on 32bit systems, by clearing -m64 flags.</code></td></tr></table>\n",
    "created_at": "2019-08-22T18:30:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28385#issuecomment-445122",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6c3a4840fb910e149c93d2ea4b714869f8078c48">6c3a484</a></td><td><code>#28385 : let tides self-tests pass on 32bit systems, by clearing -m64 flags.</code></td></tr></table>




---

archive/issue_events_371334.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28385",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28385#event-371334"
}
```



---

archive/issue_events_371335.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28385",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28385#event-371335"
}
```



---

archive/issue_comments_445123.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nTicket retargeted after milestone closed",
    "created_at": "2019-12-30T14:48:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28385#issuecomment-445123",
    "user": "https://github.com/embray"
}
```

<a id='comment:3'>Comment 3:</a>
Ticket retargeted after milestone closed



---

archive/issue_events_371336.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-01-21T20:02:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28385",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28385#event-371336"
}
```



---

archive/issue_events_371337.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-01-21T20:02:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28385",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28385#event-371337"
}
```



---

archive/issue_comments_445124.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nok",
    "created_at": "2020-01-21T20:02:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28385#issuecomment-445124",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:4'>Comment 4:</a>
ok



---

archive/issue_comments_445125.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2020-01-21T20:02:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28385#issuecomment-445125",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_445126.json:
```json
{
    "body": "Changed branch from **[u/tmonteil/tides_does_not_pass_self_tests_on_32bit_system](https://github.com/sagemath/sagetrac-mirror/tree/u/tmonteil/tides_does_not_pass_self_tests_on_32bit_system)** to **[6c3a484](https://github.com/sagemath/sagetrac-mirror/commit/6c3a4840fb910e149c93d2ea4b714869f8078c48)**",
    "created_at": "2020-01-22T20:44:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28385",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28385#issuecomment-445126",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/tmonteil/tides_does_not_pass_self_tests_on_32bit_system](https://github.com/sagemath/sagetrac-mirror/tree/u/tmonteil/tides_does_not_pass_self_tests_on_32bit_system)** to **[6c3a484](https://github.com/sagemath/sagetrac-mirror/commit/6c3a4840fb910e149c93d2ea4b714869f8078c48)**



---

archive/issue_events_371338.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-01-22T20:44:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28385",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28385#event-371338"
}
```



---

archive/issue_events_371339.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "3deaa0a61a4108c3dc01a75e852142546d3ab3c8",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-01-22T20:44:02Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28385",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28385#event-371339"
}
```
