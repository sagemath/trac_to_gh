# Issue 28178: get rid of ETupleIter

archive/issues_027941.json:
```json
{
    "assignees": [],
    "body": "`ETuple` used in exponents of generic multivariate polynomials implement their own iterator `ETupleIter` which is broken with respect to\n\n```\nsage: from array import array\nsage: R.<x,y,z> = QQ[]\nsage: e = (x*y*z).exponents()[0]\nsage: array('l', e)\nTraceback (most recent call last):\n...\nTypeError: 'sage.rings.polynomial.polydict.ETupleIter' object is not iterable\n```\nNow that Cython is more modern than at the time this was implemented we get rid of this custom iterator.\n\nBranch/Commit: **[273cf12](https://github.com/sagemath/sagetrac-mirror/commit/273cf129af58929570cf20d641611c3a4d85892d)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Vincent Delecroix**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28178_\n\n",
    "closed_at": "2019-07-14T23:14:02Z",
    "created_at": "2019-07-12T05:21:48Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "get rid of ETupleIter",
    "type": "issue",
    "updated_at": "2019-07-14T23:14:02Z",
    "url": "https://github.com/sagemath/sage/issues/28178",
    "user": "https://github.com/videlec"
}
```
`ETuple` used in exponents of generic multivariate polynomials implement their own iterator `ETupleIter` which is broken with respect to

```
sage: from array import array
sage: R.<x,y,z> = QQ[]
sage: e = (x*y*z).exponents()[0]
sage: array('l', e)
Traceback (most recent call last):
...
TypeError: 'sage.rings.polynomial.polydict.ETupleIter' object is not iterable
```
Now that Cython is more modern than at the time this was implemented we get rid of this custom iterator.

Branch/Commit: **[273cf12](https://github.com/sagemath/sagetrac-mirror/commit/273cf129af58929570cf20d641611c3a4d85892d)**

Reviewer: **Frédéric Chapoton**

Author: **Vincent Delecroix**

_Issue created by migration from https://trac.sagemath.org/ticket/28178_





---

archive/issue_events_368222.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-07-12T05:21:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28178",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28178#event-368222"
}
```



---

archive/issue_events_368223.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-07-12T05:21:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28178",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28178#event-368223"
}
```



---

archive/issue_events_368224.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-07-12T05:21:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28178",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28178#event-368224"
}
```



---

archive/issue_events_368225.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-07-12T05:21:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28178",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28178#event-368225"
}
```



---

archive/issue_comments_441604.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -9,20 +9,4 @@\n ...\n TypeError: 'sage.rings.polynomial.polydict.ETupleIter' object is not iterable\n ```\n-The reason is that `ETupleIter` does not implement `__iter__` while it is customary to return `self`. Here is the example of lists\n-\n-```\n-sage: it = iter([1,2,3])\n-sage: iter(it) is it\n-True\n-```\n-And the broken version with `ETuple`\n-\n-```\n-sage: it = iter(e)\n-sage: iter(e)\n-Traceback (most recent call last):\n-...\n-TypeError: 'sage.rings.polynomial.polydict.ETupleIter' object is not iterable\n-```\n-\n+Now that Cython is more modern than at the time this was implemented we get rid of this custom iterator.\n``````\n",
    "created_at": "2019-07-12T05:37:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28178",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28178#issuecomment-441604",
    "user": "https://github.com/videlec"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -9,20 +9,4 @@
 ...
 TypeError: 'sage.rings.polynomial.polydict.ETupleIter' object is not iterable
 ```
-The reason is that `ETupleIter` does not implement `__iter__` while it is customary to return `self`. Here is the example of lists
-
-```
-sage: it = iter([1,2,3])
-sage: iter(it) is it
-True
-```
-And the broken version with `ETuple`
-
-```
-sage: it = iter(e)
-sage: iter(e)
-Traceback (most recent call last):
-...
-TypeError: 'sage.rings.polynomial.polydict.ETupleIter' object is not iterable
-```
-
+Now that Cython is more modern than at the time this was implemented we get rid of this custom iterator.
``````




---

archive/issue_comments_441605.json:
```json
{
    "body": "Author: **Vincent Delecroix**",
    "created_at": "2019-07-12T05:37:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28178",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28178#issuecomment-441605",
    "user": "https://github.com/videlec"
}
```

Author: **Vincent Delecroix**



---

archive/issue_events_368226.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-07-12T05:37:20Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/28178",
    "title_is": "get rid of ETupleIter",
    "title_was": "implement __iter__ for ETupleIter",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28178#event-368226"
}
```



---

archive/issue_events_368227.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-07-12T05:37:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28178",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28178#event-368227"
}
```



---

archive/issue_comments_441606.json:
```json
{
    "body": "Branch: **[u/vdelecroix/28178](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/28178)**",
    "created_at": "2019-07-12T05:37:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28178",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28178#issuecomment-441606",
    "user": "https://github.com/videlec"
}
```

Branch: **[u/vdelecroix/28178](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/28178)**



---

archive/issue_comments_441607.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d1e48b3bb0ad6682e5afc59e109b2e205c2f5b45\">d1e48b3</a></td><td><code>28178: get rid of ETupleIter</code></td></tr></table>\n",
    "created_at": "2019-07-12T05:38:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28178",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28178#issuecomment-441607",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d1e48b3bb0ad6682e5afc59e109b2e205c2f5b45">d1e48b3</a></td><td><code>28178: get rid of ETupleIter</code></td></tr></table>




---

archive/issue_comments_441608.json:
```json
{
    "body": "Commit: **[d1e48b3](https://github.com/sagemath/sagetrac-mirror/commit/d1e48b3bb0ad6682e5afc59e109b2e205c2f5b45)**",
    "created_at": "2019-07-12T05:38:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28178",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28178#issuecomment-441608",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[d1e48b3](https://github.com/sagemath/sagetrac-mirror/commit/d1e48b3bb0ad6682e5afc59e109b2e205c2f5b45)**



---

archive/issue_events_368228.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-07-12T11:45:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28178",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28178#event-368228"
}
```



---

archive/issue_events_368229.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-07-12T11:45:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28178",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28178#event-368229"
}
```



---

archive/issue_comments_441609.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\n2 failing doctests, see patchbot report",
    "created_at": "2019-07-12T11:45:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28178",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28178#issuecomment-441609",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">Comment 3</div>

2 failing doctests, see patchbot report



---

archive/issue_comments_441610.json:
```json
{
    "body": "Changed commit from **[d1e48b3](https://github.com/sagemath/sagetrac-mirror/commit/d1e48b3bb0ad6682e5afc59e109b2e205c2f5b45)** to **[273cf12](https://github.com/sagemath/sagetrac-mirror/commit/273cf129af58929570cf20d641611c3a4d85892d)**",
    "created_at": "2019-07-12T11:52:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28178",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28178#issuecomment-441610",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[d1e48b3](https://github.com/sagemath/sagetrac-mirror/commit/d1e48b3bb0ad6682e5afc59e109b2e205c2f5b45)** to **[273cf12](https://github.com/sagemath/sagetrac-mirror/commit/273cf129af58929570cf20d641611c3a4d85892d)**



---

archive/issue_comments_441611.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/273cf129af58929570cf20d641611c3a4d85892d\">273cf12</a></td><td><code>28178: fix length 1 ETuple string</code></td></tr></table>\n",
    "created_at": "2019-07-12T11:52:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28178",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28178#issuecomment-441611",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/273cf129af58929570cf20d641611c3a4d85892d">273cf12</a></td><td><code>28178: fix length 1 ETuple string</code></td></tr></table>




---

archive/issue_comments_441612.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nIndeed.",
    "created_at": "2019-07-12T11:52:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28178",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28178#issuecomment-441612",
    "user": "https://github.com/videlec"
}
```

<div id="comment:5" align="right">Comment 5</div>

Indeed.



---

archive/issue_events_368230.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-07-12T11:52:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28178",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28178#event-368230"
}
```



---

archive/issue_events_368231.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-07-12T11:52:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28178",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28178#event-368231"
}
```



---

archive/issue_comments_441613.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nok, lgtm",
    "created_at": "2019-07-12T18:17:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28178",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28178#issuecomment-441613",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:6" align="right">Comment 6</div>

ok, lgtm



---

archive/issue_events_368232.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-07-12T18:17:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28178",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28178#event-368232"
}
```



---

archive/issue_events_368233.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-07-12T18:17:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28178",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28178#event-368233"
}
```



---

archive/issue_comments_441614.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2019-07-12T18:17:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28178",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28178#issuecomment-441614",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_events_368234.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-07-14T23:14:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28178",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28178#event-368234"
}
```



---

archive/issue_events_368235.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "be1db3ee614870da36be7c3ac6e1e9d6b136b366",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-07-14T23:14:02Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28178",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28178#event-368235"
}
```



---

archive/issue_comments_441615.json:
```json
{
    "body": "Changed branch from **[u/vdelecroix/28178](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/28178)** to **[273cf12](https://github.com/sagemath/sagetrac-mirror/commit/273cf129af58929570cf20d641611c3a4d85892d)**",
    "created_at": "2019-07-14T23:14:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28178",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28178#issuecomment-441615",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/vdelecroix/28178](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/28178)** to **[273cf12](https://github.com/sagemath/sagetrac-mirror/commit/273cf129af58929570cf20d641611c3a4d85892d)**
