# Issue 28380: bump up the version of givaro in spkg-configure.m4

archive/issues_028143.json:
```json
{
    "assignees": [],
    "body": "something went a bit wrong with testing of #28342, and in fact\n#26932 breaks the use of givaro 4.0.4. One needs to apply\n\n```diff\n--- a/build/pkgs/givaro/spkg-configure.m4\n+++ b/build/pkgs/givaro/spkg-configure.m4\n@@ -1,5 +1,5 @@\n SAGE_SPKG_CONFIGURE([givaro], [\n-    m4_pushdef([SAGE_GIVARO_MINVER],[\"40004\"])\n+    m4_pushdef([SAGE_GIVARO_MINVER],[\"40101\"])\n     SAGE_SPKG_DEPCHECK([gmp mpir], [\n         AC_PATH_PROG([GIVAROCONFIG], [givaro-config])\n         AS_IF([test x$GIVAROCONFIG = x], [\n```\n\nDepends on #28342\n\nCC:  @embray @kiwifb @isuruf\n\nKeywords: **spkg-configure**\n\nBranch/Commit: **[2a2fa57](https://github.com/sagemath/sagetrac-mirror/commit/2a2fa57e9082596c3eade7a3f864fa2ee35eb7f2)**\n\nReviewer: **Isuru Fernando**\n\nAuthor: **Dima Pasechnik**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28380_\n\n",
    "closed_at": "2019-08-28T19:55:16Z",
    "created_at": "2019-08-21T20:16:16Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build%3A%20configure",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "bump up the version of givaro in spkg-configure.m4",
    "type": "issue",
    "updated_at": "2019-08-28T19:55:16Z",
    "url": "https://github.com/sagemath/sage/issues/28380",
    "user": "https://github.com/dimpase"
}
```
something went a bit wrong with testing of #28342, and in fact
#26932 breaks the use of givaro 4.0.4. One needs to apply

```diff
--- a/build/pkgs/givaro/spkg-configure.m4
+++ b/build/pkgs/givaro/spkg-configure.m4
@@ -1,5 +1,5 @@
 SAGE_SPKG_CONFIGURE([givaro], [
-    m4_pushdef([SAGE_GIVARO_MINVER],["40004"])
+    m4_pushdef([SAGE_GIVARO_MINVER],["40101"])
     SAGE_SPKG_DEPCHECK([gmp mpir], [
         AC_PATH_PROG([GIVAROCONFIG], [givaro-config])
         AS_IF([test x$GIVAROCONFIG = x], [
```

Depends on #28342

CC:  @embray @kiwifb @isuruf

Keywords: **spkg-configure**

Branch/Commit: **[2a2fa57](https://github.com/sagemath/sagetrac-mirror/commit/2a2fa57e9082596c3eade7a3f864fa2ee35eb7f2)**

Reviewer: **Isuru Fernando**

Author: **Dima Pasechnik**

_Issue created by migration from https://trac.sagemath.org/ticket/28380_





---

archive/issue_events_371257.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2019-08-21T20:16:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28380",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28380#event-371257"
}
```



---

archive/issue_events_371258.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2019-08-21T20:16:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28380",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28380#event-371258"
}
```



---

archive/issue_events_371259.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2019-08-21T20:16:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28380",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28380#event-371259"
}
```



---

archive/issue_comments_445061.json:
```json
{
    "body": "Author: **Dima Pasechnik**",
    "created_at": "2019-08-21T22:12:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28380",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28380#issuecomment-445061",
    "user": "https://github.com/dimpase"
}
```

Author: **Dima Pasechnik**



---

archive/issue_comments_445062.json:
```json
{
    "body": "Branch: **[u/dimpase/packages/givaro411](https://github.com/sagemath/sagetrac-mirror/tree/u/dimpase/packages/givaro411)**",
    "created_at": "2019-08-21T22:12:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28380",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28380#issuecomment-445062",
    "user": "https://github.com/dimpase"
}
```

Branch: **[u/dimpase/packages/givaro411](https://github.com/sagemath/sagetrac-mirror/tree/u/dimpase/packages/givaro411)**



---

archive/issue_events_371260.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2019-08-21T22:12:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28380",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build%3A%20configure",
    "label_color": "000080",
    "label_name": "component: build: configure",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28380#event-371260"
}
```



---

archive/issue_events_371261.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2019-08-21T22:12:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28380",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28380#event-371261"
}
```



---

archive/issue_comments_445063.json:
```json
{
    "body": "Changed keywords from **** to **spkg-configure**",
    "created_at": "2019-08-21T22:12:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28380",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28380#issuecomment-445063",
    "user": "https://github.com/dimpase"
}
```

Changed keywords from **** to **spkg-configure**



---

archive/issue_comments_445064.json:
```json
{
    "body": "Commit: **[5e47125](https://github.com/sagemath/sagetrac-mirror/commit/5e4712548d128a997e72ff31a0a00f8257a6bd81)**",
    "created_at": "2019-08-21T22:12:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28380",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28380#issuecomment-445064",
    "user": "https://github.com/dimpase"
}
```

Commit: **[5e47125](https://github.com/sagemath/sagetrac-mirror/commit/5e4712548d128a997e72ff31a0a00f8257a6bd81)**



---

archive/issue_comments_445065.json:
```json
{
    "body": "Dependencies: **#28342**",
    "created_at": "2019-08-21T22:12:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28380",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28380#issuecomment-445065",
    "user": "https://github.com/dimpase"
}
```

Dependencies: **#28342**



---

archive/issue_comments_445066.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f0bf6de98449777df900c50d9520687e92d8ce3b\">f0bf6de</a></td><td><code>SAGE_SPKG_DEPCHECK, spkg-configure for m4ri(e), givaro</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7eca6415c82debc8735b4d51ddeebbb95f548efa\">7eca641</a></td><td><code>remove spurious '$'</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/50a3256632a3b9fc037e70cbc87b5ebb075685d8\">50a3256</a></td><td><code>update lists of packages which can be useful for installation</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2b348c59645e46f603246eec83cad2fe6a85ab38\">2b348c5</a></td><td><code>remove unneeded parameter, use SPKG_NAME instead</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/313ddc397bce6e97fd3010db2c1de563d46659e1\">313ddc3</a></td><td><code>mention lcalc system packages (prepare for #28224)</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5e4712548d128a997e72ff31a0a00f8257a6bd81\">5e47125</a></td><td><code>bump up givaro version due to #26932</code></td></tr></table>\n",
    "created_at": "2019-08-21T22:12:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28380",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28380#issuecomment-445066",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f0bf6de98449777df900c50d9520687e92d8ce3b">f0bf6de</a></td><td><code>SAGE_SPKG_DEPCHECK, spkg-configure for m4ri(e), givaro</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7eca6415c82debc8735b4d51ddeebbb95f548efa">7eca641</a></td><td><code>remove spurious '$'</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/50a3256632a3b9fc037e70cbc87b5ebb075685d8">50a3256</a></td><td><code>update lists of packages which can be useful for installation</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2b348c59645e46f603246eec83cad2fe6a85ab38">2b348c5</a></td><td><code>remove unneeded parameter, use SPKG_NAME instead</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/313ddc397bce6e97fd3010db2c1de563d46659e1">313ddc3</a></td><td><code>mention lcalc system packages (prepare for #28224)</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5e4712548d128a997e72ff31a0a00f8257a6bd81">5e47125</a></td><td><code>bump up givaro version due to #26932</code></td></tr></table>




---

archive/issue_comments_445067.json:
```json
{
    "body": "Reviewer: **Isuru Fernando**",
    "created_at": "2019-08-26T06:33:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28380",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28380#issuecomment-445067",
    "user": "https://github.com/isuruf"
}
```

Reviewer: **Isuru Fernando**



---

archive/issue_events_371262.json:
```json
{
    "actor": "https://github.com/isuruf",
    "created_at": "2019-08-26T06:33:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28380",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28380#event-371262"
}
```



---

archive/issue_events_371263.json:
```json
{
    "actor": "https://github.com/isuruf",
    "created_at": "2019-08-26T06:33:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28380",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28380#event-371263"
}
```



---

archive/issue_comments_445068.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nMerge conflict",
    "created_at": "2019-08-26T22:23:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28380",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28380#issuecomment-445068",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:3" align="right">Comment 3</div>

Merge conflict



---

archive/issue_events_371264.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-08-26T22:23:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28380",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28380#event-371264"
}
```



---

archive/issue_events_371265.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-08-26T22:23:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28380",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28380#event-371265"
}
```



---

archive/issue_comments_445069.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2a2fa57e9082596c3eade7a3f864fa2ee35eb7f2\">2a2fa57</a></td><td><code>bump up givaro version due to #26932</code></td></tr></table>\n",
    "created_at": "2019-08-26T22:32:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28380",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28380#issuecomment-445069",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2a2fa57e9082596c3eade7a3f864fa2ee35eb7f2">2a2fa57</a></td><td><code>bump up givaro version due to #26932</code></td></tr></table>




---

archive/issue_comments_445070.json:
```json
{
    "body": "Changed commit from **[5e47125](https://github.com/sagemath/sagetrac-mirror/commit/5e4712548d128a997e72ff31a0a00f8257a6bd81)** to **[2a2fa57](https://github.com/sagemath/sagetrac-mirror/commit/2a2fa57e9082596c3eade7a3f864fa2ee35eb7f2)**",
    "created_at": "2019-08-26T22:32:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28380",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28380#issuecomment-445070",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[5e47125](https://github.com/sagemath/sagetrac-mirror/commit/5e4712548d128a997e72ff31a0a00f8257a6bd81)** to **[2a2fa57](https://github.com/sagemath/sagetrac-mirror/commit/2a2fa57e9082596c3eade7a3f864fa2ee35eb7f2)**



---

archive/issue_events_371266.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2019-08-26T22:33:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28380",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28380#event-371266"
}
```



---

archive/issue_events_371267.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2019-08-26T22:33:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28380",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28380#event-371267"
}
```



---

archive/issue_comments_445071.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nrebased",
    "created_at": "2019-08-26T22:33:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28380",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28380#issuecomment-445071",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:5" align="right">Comment 5</div>

rebased



---

archive/issue_events_371268.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-08-28T19:55:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28380",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28380#event-371268"
}
```



---

archive/issue_events_371269.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "addde7bcc7af710c1ef57ae6f28f30c0a9b47e41",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-08-28T19:55:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28380",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28380#event-371269"
}
```



---

archive/issue_comments_445072.json:
```json
{
    "body": "Changed branch from **[u/dimpase/packages/givaro411](https://github.com/sagemath/sagetrac-mirror/tree/u/dimpase/packages/givaro411)** to **[2a2fa57](https://github.com/sagemath/sagetrac-mirror/commit/2a2fa57e9082596c3eade7a3f864fa2ee35eb7f2)**",
    "created_at": "2019-08-28T19:55:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28380",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28380#issuecomment-445072",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/dimpase/packages/givaro411](https://github.com/sagemath/sagetrac-mirror/tree/u/dimpase/packages/givaro411)** to **[2a2fa57](https://github.com/sagemath/sagetrac-mirror/commit/2a2fa57e9082596c3eade7a3f864fa2ee35eb7f2)**
