# Issue 28861: change some returns to return

archive/issues_028624.json:
```json
{
    "assignees": [],
    "body": "\n\nBranch/Commit: **[04f106a](https://github.com/sagemath/sagetrac-mirror/commit/04f106a68212e8c5f0e87bf907d196561d974ab5)**\n\nReviewer: **Andr\u00e9 Apitzsch**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28861_\n\n",
    "closed_at": "2019-12-16T23:14:58Z",
    "created_at": "2019-12-09T17:31:39Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "change some returns to return",
    "type": "issue",
    "updated_at": "2019-12-16T23:14:58Z",
    "url": "https://github.com/sagemath/sage/issues/28861",
    "user": "https://github.com/fchapoton"
}
```


Branch/Commit: **[04f106a](https://github.com/sagemath/sagetrac-mirror/commit/04f106a68212e8c5f0e87bf907d196561d974ab5)**

Reviewer: **André Apitzsch**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/28861_





---

archive/issue_events_365031.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-12-09T17:31:39Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28861",
    "milestone_number": null,
    "milestone_title": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28861#event-365031"
}
```



---

archive/issue_events_365032.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-12-09T17:31:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28861",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28861#event-365032"
}
```



---

archive/issue_events_365033.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-12-09T17:31:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28861",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28861#event-365033"
}
```



---

archive/issue_events_365034.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-12-09T17:31:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28861",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28861#event-365034"
}
```



---

archive/issue_events_365035.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-12-09T17:31:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28861",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28861#event-365035"
}
```



---

archive/issue_comments_453173.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\n\n```patch\n@@ -1397,7 +1397,7 @@ class OEISSequence(SageObject, UniqueRepresentation):\n \n     def __iter__(self):\n         r\"\"\"\n-        Iterates over the first terms of ``self``, and raises an error if\n+        Iterate over the first terms of ``self``, and raises an error if\n         those first terms are exhausted and the real associated sequence\n         still have terms to produce.\n```\n\n`raises` -> `raise`",
    "created_at": "2019-12-09T22:00:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28861#issuecomment-453173",
    "user": "https://github.com/a-andre"
}
```

<a id='comment:2'>Comment 2:</a>

```patch
@@ -1397,7 +1397,7 @@ class OEISSequence(SageObject, UniqueRepresentation):
 
     def __iter__(self):
         r"""
-        Iterates over the first terms of ``self``, and raises an error if
+        Iterate over the first terms of ``self``, and raises an error if
         those first terms are exhausted and the real associated sequence
         still have terms to produce.
```

`raises` -> `raise`



---

archive/issue_comments_453174.json:
```json
{
    "body": "Changed commit from **[1da26c0](https://github.com/sagemath/sagetrac-mirror/commit/1da26c0a63f73ce9002c4f5f0d8e2430248a5988)** to **[04f106a](https://github.com/sagemath/sagetrac-mirror/commit/04f106a68212e8c5f0e87bf907d196561d974ab5)**",
    "created_at": "2019-12-10T18:12:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28861#issuecomment-453174",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[1da26c0](https://github.com/sagemath/sagetrac-mirror/commit/1da26c0a63f73ce9002c4f5f0d8e2430248a5988)** to **[04f106a](https://github.com/sagemath/sagetrac-mirror/commit/04f106a68212e8c5f0e87bf907d196561d974ab5)**



---

archive/issue_comments_453175.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/04f106a68212e8c5f0e87bf907d196561d974ab5\">04f106a</a></td><td><code>trac 28861 fix one raises to raise</code></td></tr></table>\n",
    "created_at": "2019-12-10T18:12:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28861#issuecomment-453175",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:3'>Comment 3:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/04f106a68212e8c5f0e87bf907d196561d974ab5">04f106a</a></td><td><code>trac 28861 fix one raises to raise</code></td></tr></table>




---

archive/issue_comments_453176.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nthx,done",
    "created_at": "2019-12-10T18:12:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28861#issuecomment-453176",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:4'>Comment 4:</a>
thx,done



---

archive/issue_events_365036.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2019-12-10T22:39:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28861",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28861#event-365036"
}
```



---

archive/issue_events_365037.json:
```json
{
    "actor": "https://github.com/a-andre",
    "created_at": "2019-12-10T22:39:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28861",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28861#event-365037"
}
```



---

archive/issue_comments_453177.json:
```json
{
    "body": "Reviewer: **Andr\u00e9 Apitzsch**",
    "created_at": "2019-12-10T22:39:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28861#issuecomment-453177",
    "user": "https://github.com/a-andre"
}
```

Reviewer: **André Apitzsch**



---

archive/issue_comments_453178.json:
```json
{
    "body": "Changed branch from **[u/chapoton/Returns_tgv](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/Returns_tgv)** to **[04f106a](https://github.com/sagemath/sagetrac-mirror/commit/04f106a68212e8c5f0e87bf907d196561d974ab5)**",
    "created_at": "2019-12-16T23:14:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28861",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28861#issuecomment-453178",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/Returns_tgv](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/Returns_tgv)** to **[04f106a](https://github.com/sagemath/sagetrac-mirror/commit/04f106a68212e8c5f0e87bf907d196561d974ab5)**



---

archive/issue_events_365038.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-12-16T23:14:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28861",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28861#event-365038"
}
```



---

archive/issue_events_365039.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "c22568d92c9cb8b6d28d7d82d8a3c8f4e1863e1e",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-12-16T23:14:58Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28861",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28861#event-365039"
}
```
