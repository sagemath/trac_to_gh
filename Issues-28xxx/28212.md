# Issue 28212: Optimizing all_periodic_points() for small periods

archive/issues_027975.json:
```json
{
    "assignees": [],
    "body": "Currently, the all_periodic_points() method uses an algorithm that reduces the function modulo p and checks for possible cycle lengths and lifts. This method may be slow in certain cases, such as x<sup>2</sup> - 3/4, where we resort to calling the periodic_points() method which solves for the dynatomic polynomial. \n\nKeywords: **SI2019, sd104**\n\nBranch/Commit: **[u/gh-Zopherus/28212](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-Zopherus/28212) @ [6b8b99c](https://github.com/sagemath/sagetrac-mirror/commit/6b8b99c5ac78dacede53dffbc65a7340d70eb2f1)**\n\nReviewer: **Ben Hutz**\n\nAuthor: **Eric Zhu, Alex Galarraga, Bianca Thompson**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28212_\n\n",
    "closed_at": "2019-11-18T09:16:43Z",
    "created_at": "2019-07-18T13:55:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20dynamics",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Optimizing all_periodic_points() for small periods",
    "type": "issue",
    "updated_at": "2019-11-18T09:16:43Z",
    "url": "https://github.com/sagemath/sage/issues/28212",
    "user": "https://github.com/Zopherus"
}
```
Currently, the all_periodic_points() method uses an algorithm that reduces the function modulo p and checks for possible cycle lengths and lifts. This method may be slow in certain cases, such as x<sup>2</sup> - 3/4, where we resort to calling the periodic_points() method which solves for the dynatomic polynomial. 

Keywords: **SI2019, sd104**

Branch/Commit: **[u/gh-Zopherus/28212](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-Zopherus/28212) @ [6b8b99c](https://github.com/sagemath/sagetrac-mirror/commit/6b8b99c5ac78dacede53dffbc65a7340d70eb2f1)**

Reviewer: **Ben Hutz**

Author: **Eric Zhu, Alex Galarraga, Bianca Thompson**

_Issue created by migration from https://trac.sagemath.org/ticket/28212_





---

archive/issue_events_368713.json:
```json
{
    "actor": "https://github.com/Zopherus",
    "created_at": "2019-07-18T13:55:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28212",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28212#event-368713"
}
```



---

archive/issue_events_368714.json:
```json
{
    "actor": "https://github.com/Zopherus",
    "created_at": "2019-07-18T13:55:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28212",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20dynamics",
    "label_color": "0000ff",
    "label_name": "component: dynamics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28212#event-368714"
}
```



---

archive/issue_events_368715.json:
```json
{
    "actor": "https://github.com/Zopherus",
    "created_at": "2019-07-18T13:55:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28212",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28212#event-368715"
}
```



---

archive/issue_events_368716.json:
```json
{
    "actor": "https://github.com/Zopherus",
    "created_at": "2019-07-18T13:55:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28212",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28212#event-368716"
}
```



---

archive/issue_comments_442299.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-Currently, the all_periodic_points() method uses an algorithm that reduces the function modulo p and checks for possible cycle lengths and lifts. This method may be slow in certain cases, such as x^2 - 3/4, where we resort to calling the periodic_points() method which solves for the dynatomic polynomial. \n+Currently, the all_periodic_points() method uses an algorithm that reduces the function modulo p and checks for possible cycle lengths and lifts. This method may be slow in certain cases, such as x\\^2 - 3/4, where we resort to calling the periodic_points() method which solves for the dynatomic polynomial. \n``````\n",
    "created_at": "2019-07-18T13:55:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28212#issuecomment-442299",
    "user": "https://github.com/Zopherus"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-Currently, the all_periodic_points() method uses an algorithm that reduces the function modulo p and checks for possible cycle lengths and lifts. This method may be slow in certain cases, such as x^2 - 3/4, where we resort to calling the periodic_points() method which solves for the dynatomic polynomial. 
+Currently, the all_periodic_points() method uses an algorithm that reduces the function modulo p and checks for possible cycle lengths and lifts. This method may be slow in certain cases, such as x\^2 - 3/4, where we resort to calling the periodic_points() method which solves for the dynatomic polynomial. 
``````




---

archive/issue_comments_442300.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-Currently, the all_periodic_points() method uses an algorithm that reduces the function modulo p and checks for possible cycle lengths and lifts. This method may be slow in certain cases, such as x\\^2 - 3/4, where we resort to calling the periodic_points() method which solves for the dynatomic polynomial. \n+Currently, the all_periodic_points() method uses an algorithm that reduces the function modulo p and checks for possible cycle lengths and lifts. This method may be slow in certain cases, such as x<sup>2</sup> - 3/4, where we resort to calling the periodic_points() method which solves for the dynatomic polynomial. \n``````\n",
    "created_at": "2019-07-18T13:56:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28212#issuecomment-442300",
    "user": "https://github.com/Zopherus"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-Currently, the all_periodic_points() method uses an algorithm that reduces the function modulo p and checks for possible cycle lengths and lifts. This method may be slow in certain cases, such as x\^2 - 3/4, where we resort to calling the periodic_points() method which solves for the dynatomic polynomial. 
+Currently, the all_periodic_points() method uses an algorithm that reduces the function modulo p and checks for possible cycle lengths and lifts. This method may be slow in certain cases, such as x<sup>2</sup> - 3/4, where we resort to calling the periodic_points() method which solves for the dynatomic polynomial. 
``````




---

archive/issue_comments_442301.json:
```json
{
    "body": "Branch: **[u/gh-Zopherus/28212](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-Zopherus/28212)**",
    "created_at": "2019-07-18T15:15:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28212#issuecomment-442301",
    "user": "https://github.com/Zopherus"
}
```

Branch: **[u/gh-Zopherus/28212](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-Zopherus/28212)**



---

archive/issue_comments_442302.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nLast 10 new commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8a46a0fe35b857f656ee2413de88e119bf3782a4\">8a46a0f</a></td><td><code>Merge branch 'u/gh-Zopherus/28109' of git://trac.sagemath.org/sage into 28109</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/025c74aec957d58adc63e20d80a9a51c297040a4\">025c74a</a></td><td><code>Added method to cython helper</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f20eadff5e8ee55d5ce192ca8fe3b0fd6e8ba6c7\">f20eadf</a></td><td><code>Merge branch 'u/gh-Zopherus/28109' of git://trac.sagemath.org/sage into 28109</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0f27e692154f62d9fd2dd9cd2ab1a4988284d38a\">0f27e69</a></td><td><code>added C code to all_periodic_points()</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0682214dafba1a13f4d8fb8be8d7051e39d5d096\">0682214</a></td><td><code>Merge branch 'u/gh-Zopherus/28109' of git://trac.sagemath.org/sage into 28109</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/db9b2336fbf065a63f3d178dfb0c55e70bb26bbc\">db9b233</a></td><td><code>added documentation and deprecated rational_periodic_points</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7b48f15f300f2e3ac9a88406fb0433fe2609b9d5\">7b48f15</a></td><td><code>Finished depreciation</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/81df7105e3957cf0bcf3375213031e0da696820f\">81df710</a></td><td><code>Added optional parameter for base ring</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/686251446eff9d325ec3985ceef73546842a8a7e\">6862514</a></td><td><code>delete print statement</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6b8b99c5ac78dacede53dffbc65a7340d70eb2f1\">6b8b99c</a></td><td><code>Added check to do dynatomic method</code></td></tr></table>\n",
    "created_at": "2019-07-25T13:40:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28212#issuecomment-442302",
    "user": "https://github.com/Zopherus"
}
```

<div id="comment:4" align="right">Comment 4</div>

Last 10 new commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8a46a0fe35b857f656ee2413de88e119bf3782a4">8a46a0f</a></td><td><code>Merge branch 'u/gh-Zopherus/28109' of git://trac.sagemath.org/sage into 28109</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/025c74aec957d58adc63e20d80a9a51c297040a4">025c74a</a></td><td><code>Added method to cython helper</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f20eadff5e8ee55d5ce192ca8fe3b0fd6e8ba6c7">f20eadf</a></td><td><code>Merge branch 'u/gh-Zopherus/28109' of git://trac.sagemath.org/sage into 28109</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0f27e692154f62d9fd2dd9cd2ab1a4988284d38a">0f27e69</a></td><td><code>added C code to all_periodic_points()</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0682214dafba1a13f4d8fb8be8d7051e39d5d096">0682214</a></td><td><code>Merge branch 'u/gh-Zopherus/28109' of git://trac.sagemath.org/sage into 28109</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/db9b2336fbf065a63f3d178dfb0c55e70bb26bbc">db9b233</a></td><td><code>added documentation and deprecated rational_periodic_points</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7b48f15f300f2e3ac9a88406fb0433fe2609b9d5">7b48f15</a></td><td><code>Finished depreciation</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/81df7105e3957cf0bcf3375213031e0da696820f">81df710</a></td><td><code>Added optional parameter for base ring</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/686251446eff9d325ec3985ceef73546842a8a7e">6862514</a></td><td><code>delete print statement</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6b8b99c5ac78dacede53dffbc65a7340d70eb2f1">6b8b99c</a></td><td><code>Added check to do dynatomic method</code></td></tr></table>




---

archive/issue_comments_442303.json:
```json
{
    "body": "Commit: **[6b8b99c](https://github.com/sagemath/sagetrac-mirror/commit/6b8b99c5ac78dacede53dffbc65a7340d70eb2f1)**",
    "created_at": "2019-07-25T13:40:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28212#issuecomment-442303",
    "user": "https://github.com/Zopherus"
}
```

Commit: **[6b8b99c](https://github.com/sagemath/sagetrac-mirror/commit/6b8b99c5ac78dacede53dffbc65a7340d70eb2f1)**



---

archive/issue_events_368717.json:
```json
{
    "actor": "https://github.com/Zopherus",
    "created_at": "2019-07-25T13:40:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28212",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28212#event-368717"
}
```



---

archive/issue_comments_442304.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThis ticket depends on ticket #28109 so it needs to be added to dependencies.",
    "created_at": "2019-07-25T14:42:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28212#issuecomment-442304",
    "user": "https://github.com/jasminecamero"
}
```

<div id="comment:5" align="right">Comment 5</div>

This ticket depends on ticket #28109 so it needs to be added to dependencies.



---

archive/issue_comments_442305.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nThis optimization is already done in #28109 as part of the major overhaul of the periodic point functionality.",
    "created_at": "2019-07-30T15:33:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28212#issuecomment-442305",
    "user": "https://github.com/bhutz"
}
```

<div id="comment:6" align="right">Comment 6</div>

This optimization is already done in #28109 as part of the major overhaul of the periodic point functionality.



---

archive/issue_comments_442306.json:
```json
{
    "body": "Reviewer: **Ben Hutz**",
    "created_at": "2019-07-30T15:33:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28212#issuecomment-442306",
    "user": "https://github.com/bhutz"
}
```

Reviewer: **Ben Hutz**



---

archive/issue_events_368718.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2019-07-30T15:33:45Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28212",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28212#event-368718"
}
```



---

archive/issue_events_368719.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2019-07-30T15:33:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28212",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28212#event-368719"
}
```



---

archive/issue_events_368720.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2019-07-30T15:33:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28212",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28212#event-368720"
}
```



---

archive/issue_comments_442307.json:
```json
{
    "body": "Changed keywords from **SI2019** to **SI2019, sd104**",
    "created_at": "2019-11-17T20:21:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28212",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28212#issuecomment-442307",
    "user": "https://github.com/bhutz"
}
```

Changed keywords from **SI2019** to **SI2019, sd104**



---

archive/issue_events_368721.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2019-11-17T20:21:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28212",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28212#event-368721"
}
```



---

archive/issue_events_368722.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2019-11-17T20:21:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28212",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28212#event-368722"
}
```



---

archive/issue_events_368723.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-11-18T09:16:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28212",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28212#event-368723"
}
```



---

archive/issue_events_368724.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-11-18T09:16:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28212",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28212#event-368724"
}
```



---

archive/issue_events_368725.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-11-18T09:16:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28212",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28212#event-368725"
}
```
