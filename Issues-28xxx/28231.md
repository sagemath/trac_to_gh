# Issue 28231: spkg-configure.m4 for lrcalc

archive/issues_027994.json:
```json
{
    "assignees": [],
    "body": "lrcalc is available in Debian (`lrcalc` and `liblrcalc-dev`) and in Fedora (`lrcalc-devel`)\n\nOn Fedora one also needs to run \"module load lrcalc-x86_64\" to add the location of `rlcalc` binaries to PATH, although it appears to be not important, as only the dylib of lrcalc is used in Sage.\n\nCC:  @embray @kiwifb @isuruf\n\nKeywords: **spkg-configure**\n\nBranch/Commit: **[6afb95a](https://github.com/sagemath/sagetrac-mirror/commit/6afb95a36278f6f9dc71447b5ce6c2078967d84b)**\n\nReviewer: **Isuru Fernando**\n\nAuthor: **Dima Pasechnik**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28231_\n\n",
    "closed_at": "2019-08-05T20:02:46Z",
    "created_at": "2019-07-22T18:12:44Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build%3A%20configure",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "spkg-configure.m4 for lrcalc",
    "type": "issue",
    "updated_at": "2019-08-05T20:02:46Z",
    "url": "https://github.com/sagemath/sage/issues/28231",
    "user": "https://github.com/dimpase"
}
```
lrcalc is available in Debian (`lrcalc` and `liblrcalc-dev`) and in Fedora (`lrcalc-devel`)

On Fedora one also needs to run "module load lrcalc-x86_64" to add the location of `rlcalc` binaries to PATH, although it appears to be not important, as only the dylib of lrcalc is used in Sage.

CC:  @embray @kiwifb @isuruf

Keywords: **spkg-configure**

Branch/Commit: **[6afb95a](https://github.com/sagemath/sagetrac-mirror/commit/6afb95a36278f6f9dc71447b5ce6c2078967d84b)**

Reviewer: **Isuru Fernando**

Author: **Dima Pasechnik**

_Issue created by migration from https://trac.sagemath.org/ticket/28231_





---

archive/issue_events_369045.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2019-07-22T18:12:44Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28231",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28231#event-369045"
}
```



---

archive/issue_events_369046.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2019-07-22T18:12:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28231",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build%3A%20configure",
    "label_color": "000080",
    "label_name": "component: build: configure",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28231#event-369046"
}
```



---

archive/issue_events_369047.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2019-07-22T18:12:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28231",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28231#event-369047"
}
```



---

archive/issue_events_369048.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2019-07-22T18:12:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28231",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28231#event-369048"
}
```



---

archive/issue_comments_442742.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n lrcalc is available in Debian (`lrcalc` and `liblrcalc-dev`) and in Fedora (`lrcalc-devel`)\n+\n+On Fedora one also needs to run \"module load lrcalc-x86_64\" to add the location of `rlcalc` binaries to PATH, although it appears to be not important, as only the dylib of lrcalc is used in Sage.\n``````\n",
    "created_at": "2019-07-22T18:18:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28231",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28231#issuecomment-442742",
    "user": "https://github.com/dimpase"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
 lrcalc is available in Debian (`lrcalc` and `liblrcalc-dev`) and in Fedora (`lrcalc-devel`)
+
+On Fedora one also needs to run "module load lrcalc-x86_64" to add the location of `rlcalc` binaries to PATH, although it appears to be not important, as only the dylib of lrcalc is used in Sage.
``````




---

archive/issue_comments_442743.json:
```json
{
    "body": "Author: **Dima Pasechnik**",
    "created_at": "2019-07-23T00:38:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28231",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28231#issuecomment-442743",
    "user": "https://github.com/dimpase"
}
```

Author: **Dima Pasechnik**



---

archive/issue_comments_442744.json:
```json
{
    "body": "Branch: **[u/dimpase/packages/lrcalcconfig](https://github.com/sagemath/sagetrac-mirror/tree/u/dimpase/packages/lrcalcconfig)**",
    "created_at": "2019-07-23T00:38:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28231",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28231#issuecomment-442744",
    "user": "https://github.com/dimpase"
}
```

Branch: **[u/dimpase/packages/lrcalcconfig](https://github.com/sagemath/sagetrac-mirror/tree/u/dimpase/packages/lrcalcconfig)**



---

archive/issue_comments_442745.json:
```json
{
    "body": "Commit: **[6afb95a](https://github.com/sagemath/sagetrac-mirror/commit/6afb95a36278f6f9dc71447b5ce6c2078967d84b)**",
    "created_at": "2019-07-23T00:38:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28231",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28231#issuecomment-442745",
    "user": "https://github.com/dimpase"
}
```

Commit: **[6afb95a](https://github.com/sagemath/sagetrac-mirror/commit/6afb95a36278f6f9dc71447b5ce6c2078967d84b)**



---

archive/issue_comments_442746.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6afb95a36278f6f9dc71447b5ce6c2078967d84b\">6afb95a</a></td><td><code>spkg-configure for lrcalc</code></td></tr></table>\n",
    "created_at": "2019-07-23T00:38:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28231",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28231#issuecomment-442746",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6afb95a36278f6f9dc71447b5ce6c2078967d84b">6afb95a</a></td><td><code>spkg-configure for lrcalc</code></td></tr></table>




---

archive/issue_comments_442747.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nI'll change the installation documentation (per distro) on lrcalc on #28224, to minimise hard to merge small changes to the same file...\n\nBy the way, is lrcalc available on Conda?",
    "created_at": "2019-07-23T09:48:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28231",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28231#issuecomment-442747",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:3'>Comment 3:</a>
I'll change the installation documentation (per distro) on lrcalc on #28224, to minimise hard to merge small changes to the same file...

By the way, is lrcalc available on Conda?



---

archive/issue_events_369049.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2019-07-23T09:48:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28231",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28231#event-369049"
}
```



---

archive/issue_comments_442748.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\n> By the way, is lrcalc available on Conda? \n\nYes, all standard packages are.",
    "created_at": "2019-07-24T03:26:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28231",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28231#issuecomment-442748",
    "user": "https://github.com/isuruf"
}
```

<a id='comment:4'>Comment 4:</a>
> By the way, is lrcalc available on Conda? 

Yes, all standard packages are.



---

archive/issue_events_369050.json:
```json
{
    "actor": "https://github.com/isuruf",
    "created_at": "2019-08-02T22:13:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28231",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28231#event-369050"
}
```



---

archive/issue_events_369051.json:
```json
{
    "actor": "https://github.com/isuruf",
    "created_at": "2019-08-02T22:13:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28231",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28231#event-369051"
}
```



---

archive/issue_comments_442749.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nWorks for me with the conda package.",
    "created_at": "2019-08-02T22:13:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28231",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28231#issuecomment-442749",
    "user": "https://github.com/isuruf"
}
```

<a id='comment:5'>Comment 5:</a>
Works for me with the conda package.



---

archive/issue_events_369052.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-08-03T12:20:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28231",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28231#event-369052"
}
```



---

archive/issue_events_369053.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-08-03T12:20:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28231",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28231#event-369053"
}
```



---

archive/issue_comments_442750.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nReviewer name is missing",
    "created_at": "2019-08-03T12:20:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28231",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28231#issuecomment-442750",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:6'>Comment 6:</a>
Reviewer name is missing



---

archive/issue_comments_442751.json:
```json
{
    "body": "Reviewer: **Isuru Fernando**",
    "created_at": "2019-08-03T16:53:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28231",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28231#issuecomment-442751",
    "user": "https://github.com/isuruf"
}
```

Reviewer: **Isuru Fernando**



---

archive/issue_events_369054.json:
```json
{
    "actor": "https://github.com/isuruf",
    "created_at": "2019-08-03T16:53:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28231",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28231#event-369054"
}
```



---

archive/issue_events_369055.json:
```json
{
    "actor": "https://github.com/isuruf",
    "created_at": "2019-08-03T16:53:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28231",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28231#event-369055"
}
```



---

archive/issue_comments_442752.json:
```json
{
    "body": "Changed branch from **[u/dimpase/packages/lrcalcconfig](https://github.com/sagemath/sagetrac-mirror/tree/u/dimpase/packages/lrcalcconfig)** to **[6afb95a](https://github.com/sagemath/sagetrac-mirror/commit/6afb95a36278f6f9dc71447b5ce6c2078967d84b)**",
    "created_at": "2019-08-05T20:02:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28231",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28231#issuecomment-442752",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/dimpase/packages/lrcalcconfig](https://github.com/sagemath/sagetrac-mirror/tree/u/dimpase/packages/lrcalcconfig)** to **[6afb95a](https://github.com/sagemath/sagetrac-mirror/commit/6afb95a36278f6f9dc71447b5ce6c2078967d84b)**



---

archive/issue_events_369056.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-08-05T20:02:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28231",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28231#event-369056"
}
```



---

archive/issue_events_369057.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "189a96bb1dbf927de204644527b42a66576b4a2b",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-08-05T20:02:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28231",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28231#event-369057"
}
```
