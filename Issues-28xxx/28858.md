# Issue 28858: Fix frobby interface with python3

archive/issues_028621.json:
```json
{
    "assignees": [],
    "body": "Frobby interface got broken when we moved to python3, since it uses strings, and gets bytes objects instead.\n\nThis patch converts back and forth between strings and bytes\n\nCC:  @tscrim @jdemeyer\n\nKeywords: **Frobby**\n\nBranch/Commit: **[8035353](https://github.com/sagemath/sagetrac-mirror/commit/8035353f21fd01af81d90e6fa7e229ed81245ccc)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Miguel Marco**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28858_\n\n",
    "closed_at": "2019-12-11T21:46:24Z",
    "created_at": "2019-12-09T10:51:46Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix frobby interface with python3",
    "type": "issue",
    "updated_at": "2019-12-11T21:46:24Z",
    "url": "https://github.com/sagemath/sage/issues/28858",
    "user": "https://github.com/miguelmarco"
}
```
Frobby interface got broken when we moved to python3, since it uses strings, and gets bytes objects instead.

This patch converts back and forth between strings and bytes

CC:  @tscrim @jdemeyer

Keywords: **Frobby**

Branch/Commit: **[8035353](https://github.com/sagemath/sagetrac-mirror/commit/8035353f21fd01af81d90e6fa7e229ed81245ccc)**

Reviewer: **Frédéric Chapoton**

Author: **Miguel Marco**

_Issue created by migration from https://trac.sagemath.org/ticket/28858_





---

archive/issue_events_365004.json:
```json
{
    "actor": "https://github.com/miguelmarco",
    "created_at": "2019-12-09T10:51:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28858",
    "milestone_number": null,
    "milestone_title": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28858#event-365004"
}
```



---

archive/issue_events_365005.json:
```json
{
    "actor": "https://github.com/miguelmarco",
    "created_at": "2019-12-09T10:51:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28858",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28858#event-365005"
}
```



---

archive/issue_comments_453150.json:
```json
{
    "body": "Branch: **[u/mmarco/fix_frobby](https://github.com/sagemath/sagetrac-mirror/tree/u/mmarco/fix_frobby)**",
    "created_at": "2019-12-09T10:52:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28858#issuecomment-453150",
    "user": "https://github.com/miguelmarco"
}
```

Branch: **[u/mmarco/fix_frobby](https://github.com/sagemath/sagetrac-mirror/tree/u/mmarco/fix_frobby)**



---

archive/issue_comments_453151.json:
```json
{
    "body": "Changed keywords from **** to **Frobby**",
    "created_at": "2019-12-09T10:55:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28858#issuecomment-453151",
    "user": "https://github.com/miguelmarco"
}
```

Changed keywords from **** to **Frobby**



---

archive/issue_comments_453152.json:
```json
{
    "body": "Commit: **[e1c4aab](https://github.com/sagemath/sagetrac-mirror/commit/e1c4aabe7fb6d15092c5458cf00459d069e54eb4)**",
    "created_at": "2019-12-09T10:55:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28858#issuecomment-453152",
    "user": "https://github.com/miguelmarco"
}
```

Commit: **[e1c4aab](https://github.com/sagemath/sagetrac-mirror/commit/e1c4aabe7fb6d15092c5458cf00459d069e54eb4)**



---

archive/issue_events_365006.json:
```json
{
    "actor": "https://github.com/miguelmarco",
    "created_at": "2019-12-09T10:55:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28858",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "component: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28858#event-365006"
}
```



---

archive/issue_events_365007.json:
```json
{
    "actor": "https://github.com/miguelmarco",
    "created_at": "2019-12-09T10:55:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28858",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28858#event-365007"
}
```



---

archive/issue_comments_453153.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/20e5444714dc867c4a3b5080cf2c89bbdba0bc22\">20e5444</a></td><td><code>convert between bytes and strings</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e1c4aabe7fb6d15092c5458cf00459d069e54eb4\">e1c4aab</a></td><td><code>Merge branch 'fix_frobby' into t/28858/fix_frobby</code></td></tr></table>\n",
    "created_at": "2019-12-09T10:55:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28858#issuecomment-453153",
    "user": "https://github.com/miguelmarco"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/20e5444714dc867c4a3b5080cf2c89bbdba0bc22">20e5444</a></td><td><code>convert between bytes and strings</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e1c4aabe7fb6d15092c5458cf00459d069e54eb4">e1c4aab</a></td><td><code>Merge branch 'fix_frobby' into t/28858/fix_frobby</code></td></tr></table>




---

archive/issue_comments_453154.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n+Frobby interface got broken when we moved to python3, since it uses strings, and gets bytes objects instead.\n \n+This patch converts back and forth between strings and bytes\n``````\n",
    "created_at": "2019-12-09T10:55:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28858#issuecomment-453154",
    "user": "https://github.com/miguelmarco"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
+Frobby interface got broken when we moved to python3, since it uses strings, and gets bytes objects instead.
 
+This patch converts back and forth between strings and bytes
``````




---

archive/issue_comments_453155.json:
```json
{
    "body": "Author: **Miguel Marco**",
    "created_at": "2019-12-09T10:55:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28858#issuecomment-453155",
    "user": "https://github.com/miguelmarco"
}
```

Author: **Miguel Marco**



---

archive/issue_comments_453156.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nplease rather use bytes_to_str or str_to_bytes, from sage.cpython.string",
    "created_at": "2019-12-09T14:13:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28858#issuecomment-453156",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:3'>Comment 3:</a>
please rather use bytes_to_str or str_to_bytes, from sage.cpython.string



---

archive/issue_comments_453157.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8035353f21fd01af81d90e6fa7e229ed81245ccc\">8035353</a></td><td><code>Use str_to_bytes and bytes_to_str</code></td></tr></table>\n",
    "created_at": "2019-12-09T14:40:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28858#issuecomment-453157",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8035353f21fd01af81d90e6fa7e229ed81245ccc">8035353</a></td><td><code>Use str_to_bytes and bytes_to_str</code></td></tr></table>




---

archive/issue_comments_453158.json:
```json
{
    "body": "Changed commit from **[e1c4aab](https://github.com/sagemath/sagetrac-mirror/commit/e1c4aabe7fb6d15092c5458cf00459d069e54eb4)** to **[8035353](https://github.com/sagemath/sagetrac-mirror/commit/8035353f21fd01af81d90e6fa7e229ed81245ccc)**",
    "created_at": "2019-12-09T14:40:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28858#issuecomment-453158",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[e1c4aab](https://github.com/sagemath/sagetrac-mirror/commit/e1c4aabe7fb6d15092c5458cf00459d069e54eb4)** to **[8035353](https://github.com/sagemath/sagetrac-mirror/commit/8035353f21fd01af81d90e6fa7e229ed81245ccc)**



---

archive/issue_comments_453159.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nThanks for the hint.",
    "created_at": "2019-12-10T06:42:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28858#issuecomment-453159",
    "user": "https://github.com/miguelmarco"
}
```

<a id='comment:5'>Comment 5:</a>
Thanks for the hint.



---

archive/issue_events_365008.json:
```json
{
    "actor": "https://github.com/miguelmarco",
    "created_at": "2019-12-10T06:42:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28858",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28858#event-365008"
}
```



---

archive/issue_comments_453160.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nok, assuming that you tested.",
    "created_at": "2019-12-10T18:15:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28858#issuecomment-453160",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:7'>Comment 7:</a>
ok, assuming that you tested.



---

archive/issue_events_365009.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-12-10T18:15:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28858",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28858#event-365009"
}
```



---

archive/issue_events_365010.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-12-10T18:15:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28858",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28858#event-365010"
}
```



---

archive/issue_comments_453161.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2019-12-10T18:15:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28858#issuecomment-453161",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_453162.json:
```json
{
    "body": "Changed branch from **[u/mmarco/fix_frobby](https://github.com/sagemath/sagetrac-mirror/tree/u/mmarco/fix_frobby)** to **[8035353](https://github.com/sagemath/sagetrac-mirror/commit/8035353f21fd01af81d90e6fa7e229ed81245ccc)**",
    "created_at": "2019-12-11T21:46:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28858#issuecomment-453162",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mmarco/fix_frobby](https://github.com/sagemath/sagetrac-mirror/tree/u/mmarco/fix_frobby)** to **[8035353](https://github.com/sagemath/sagetrac-mirror/commit/8035353f21fd01af81d90e6fa7e229ed81245ccc)**



---

archive/issue_events_365011.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-12-11T21:46:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28858",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28858#event-365011"
}
```



---

archive/issue_events_365012.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "28a281c1a20b5e6a8e72189ecd7b3fc31240c877",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-12-11T21:46:24Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28858",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28858#event-365012"
}
```
