# Issue 28793: 2 more random doctest failures

archive/issues_028556.json:
```json
{
    "assignees": [],
    "body": "The following doctest fails occasionally when, due to collisions, the generated matrix has fewer than 99 elements:\n\n```\n            sage: M = MatrixSpace(ZZ, 1000, 1000, sparse=True).an_element()\n            sage: M.density()\n            99/1000000\n```\n\nMoreover, this probabilistic `giacpy_sage` test in multi_polynomial_ideal.py sometimes fails due to the output containing additional lines:\n\n```\n            sage: I9.groebner_basis(\"giac\",proba_epsilon=1e-7) # optional - giacpy_sage, long time (3s)\n            Running a probabilistic check for the reconstructed Groebner basis...\n            Polynomial Sequence with 143 Polynomials in 9 Variables\n```\n\nThese can be seen in the patchbot results [here](https://patchbot.sagemath.org/log/28756/CentOS%20Linux/7.7.1908/x86_64/3.10.0-1062.4.3.el7.x86_64/r640/2019-11-19%2001:00:01) and [here](https://patchbot.sagemath.org/log/27154/CentOS%20Linux/7.7.1908/x86_64/3.10.0-1062.4.3.el7.x86_64/r640/2019-11-19%2021:13:12), for example.\n\n\nKeywords: **giacpy_sage**\n\nBranch/Commit: **[24fe9f4](https://github.com/sagemath/sagetrac-mirror/commit/24fe9f40933492970258ec3afc821e55a81b930c)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Markus Wageringel**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28793_\n\n",
    "closed_at": "2019-11-29T23:57:47Z",
    "created_at": "2019-11-23T18:27:14Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "2 more random doctest failures",
    "type": "issue",
    "updated_at": "2019-11-29T23:57:47Z",
    "url": "https://github.com/sagemath/sage/issues/28793",
    "user": "https://github.com/mwageringel"
}
```
The following doctest fails occasionally when, due to collisions, the generated matrix has fewer than 99 elements:

```
            sage: M = MatrixSpace(ZZ, 1000, 1000, sparse=True).an_element()
            sage: M.density()
            99/1000000
```

Moreover, this probabilistic `giacpy_sage` test in multi_polynomial_ideal.py sometimes fails due to the output containing additional lines:

```
            sage: I9.groebner_basis("giac",proba_epsilon=1e-7) # optional - giacpy_sage, long time (3s)
            Running a probabilistic check for the reconstructed Groebner basis...
            Polynomial Sequence with 143 Polynomials in 9 Variables
```

These can be seen in the patchbot results [here](https://patchbot.sagemath.org/log/28756/CentOS%20Linux/7.7.1908/x86_64/3.10.0-1062.4.3.el7.x86_64/r640/2019-11-19%2001:00:01) and [here](https://patchbot.sagemath.org/log/27154/CentOS%20Linux/7.7.1908/x86_64/3.10.0-1062.4.3.el7.x86_64/r640/2019-11-19%2021:13:12), for example.


Keywords: **giacpy_sage**

Branch/Commit: **[24fe9f4](https://github.com/sagemath/sagetrac-mirror/commit/24fe9f40933492970258ec3afc821e55a81b930c)**

Reviewer: **Frédéric Chapoton**

Author: **Markus Wageringel**

_Issue created by migration from https://trac.sagemath.org/ticket/28793_





---

archive/issue_events_377294.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2019-11-23T18:27:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28793",
    "milestone_number": null,
    "milestone_title": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28793#event-377294"
}
```



---

archive/issue_events_377295.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2019-11-23T18:27:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28793",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
    "label_color": "000080",
    "label_name": "component: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28793#event-377295"
}
```



---

archive/issue_events_377296.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2019-11-23T18:27:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28793",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28793#event-377296"
}
```



---

archive/issue_events_377297.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2019-11-23T18:27:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28793",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28793#event-377297"
}
```



---

archive/issue_comments_452161.json:
```json
{
    "body": "Branch: **[u/gh-mwageringel/28793](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/28793)**",
    "created_at": "2019-11-23T18:34:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28793",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28793#issuecomment-452161",
    "user": "https://github.com/mwageringel"
}
```

Branch: **[u/gh-mwageringel/28793](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/28793)**



---

archive/issue_events_377298.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2019-11-23T18:34:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28793",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28793#event-377298"
}
```



---

archive/issue_comments_452162.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a1f5bde4f8f6e0cd29e7e8f91c0ae70a8bf70b6d\">a1f5bde</a></td><td><code>28793: fix sporadic doctest failure for matrix space</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/24fe9f40933492970258ec3afc821e55a81b930c\">24fe9f4</a></td><td><code>28793: fix random output in another giacpy_sage doctest</code></td></tr></table>\n",
    "created_at": "2019-11-23T18:34:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28793",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28793#issuecomment-452162",
    "user": "https://github.com/mwageringel"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a1f5bde4f8f6e0cd29e7e8f91c0ae70a8bf70b6d">a1f5bde</a></td><td><code>28793: fix sporadic doctest failure for matrix space</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/24fe9f40933492970258ec3afc821e55a81b930c">24fe9f4</a></td><td><code>28793: fix random output in another giacpy_sage doctest</code></td></tr></table>




---

archive/issue_comments_452163.json:
```json
{
    "body": "Changed keywords from **** to **giacpy_sage**",
    "created_at": "2019-11-23T18:34:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28793",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28793#issuecomment-452163",
    "user": "https://github.com/mwageringel"
}
```

Changed keywords from **** to **giacpy_sage**



---

archive/issue_comments_452164.json:
```json
{
    "body": "Commit: **[24fe9f4](https://github.com/sagemath/sagetrac-mirror/commit/24fe9f40933492970258ec3afc821e55a81b930c)**",
    "created_at": "2019-11-23T18:34:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28793",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28793#issuecomment-452164",
    "user": "https://github.com/mwageringel"
}
```

Commit: **[24fe9f4](https://github.com/sagemath/sagetrac-mirror/commit/24fe9f40933492970258ec3afc821e55a81b930c)**



---

archive/issue_events_377299.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-11-23T19:13:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28793",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28793#event-377299"
}
```



---

archive/issue_events_377300.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-11-23T19:13:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28793",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28793#event-377300"
}
```



---

archive/issue_comments_452165.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nok, thx",
    "created_at": "2019-11-23T19:13:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28793",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28793#issuecomment-452165",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:2'>Comment 2:</a>
ok, thx



---

archive/issue_comments_452166.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2019-11-23T19:13:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28793",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28793#issuecomment-452166",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_452167.json:
```json
{
    "body": "Changed branch from **[u/gh-mwageringel/28793](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/28793)** to **[24fe9f4](https://github.com/sagemath/sagetrac-mirror/commit/24fe9f40933492970258ec3afc821e55a81b930c)**",
    "created_at": "2019-11-29T23:57:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28793",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28793#issuecomment-452167",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/gh-mwageringel/28793](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/28793)** to **[24fe9f4](https://github.com/sagemath/sagetrac-mirror/commit/24fe9f40933492970258ec3afc821e55a81b930c)**



---

archive/issue_events_377301.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-11-29T23:57:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28793",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28793#event-377301"
}
```



---

archive/issue_events_377302.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "737eadb5aecb4822b1cd1efb49f14045197561a5",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-11-29T23:57:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28793",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28793#event-377302"
}
```
