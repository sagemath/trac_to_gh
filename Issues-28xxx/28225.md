# Issue 28225: Allow sage to run in the absence of sage-env

archive/issues_027988.json:
```json
{
    "body": "The sage-env script has never been particularly useful in distributions, and has actually been more of a nuisance: it needs to be heavily patched or even replaced with a custom one. \n\nAfter #25786 (which removes SAGE_DOC_SRC usage at runtime) the only use for it that I can think of is setting DOT_SAGE. This patch allows to completely do away with this script, by setting a fallback for DOT_SAGE in the sage executable.\n\nCC:  @saraedum @isuruf @kiwifb @timokau\n\nReviewer: Fran\u00e7ois Bissey\n\nAuthor: Antonio Rojas\n\nBranch: u/arojas/allow_sage_to_run_in_the_absence_of_sage_env\n\nDependencies: #25786\n\nCommit: 83a6cec200e7179a632daee8658ac95b561d92ac\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/28225\n\n",
    "closed_at": "2019-08-03T23:51:18Z",
    "created_at": "2019-07-21T10:20:40Z",
    "labels": [
        "component: distribution"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.9",
    "title": "Allow sage to run in the absence of sage-env",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/28225",
    "user": "https://github.com/antonio-rojas"
}
```
The sage-env script has never been particularly useful in distributions, and has actually been more of a nuisance: it needs to be heavily patched or even replaced with a custom one. 

After #25786 (which removes SAGE_DOC_SRC usage at runtime) the only use for it that I can think of is setting DOT_SAGE. This patch allows to completely do away with this script, by setting a fallback for DOT_SAGE in the sage executable.

CC:  @saraedum @isuruf @kiwifb @timokau

Reviewer: François Bissey

Author: Antonio Rojas

Branch: u/arojas/allow_sage_to_run_in_the_absence_of_sage_env

Dependencies: #25786

Commit: 83a6cec200e7179a632daee8658ac95b561d92ac

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/28225





---

archive/issue_comments_547357.json:
```json
{
    "body": "Changing branch from \"\" to \"u/arojas/allow_sage_to_run_in_the_absence_of_sage_env\"",
    "created_at": "2019-07-21T10:21:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547357",
    "user": "https://github.com/antonio-rojas"
}
```

Changing branch from "" to "u/arojas/allow_sage_to_run_in_the_absence_of_sage_env"



---

archive/issue_comments_547358.json:
```json
{
    "body": "<a id='comment:2'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                              |\n|-------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------|\n|[dda6986](https://git.sagemath.org/sage.git/commit/?id=dda6986e1a3b4c3d06150475d75a207c1e133394)|`Allow sage to run if sage-env is not present`|",
    "created_at": "2019-07-21T10:23:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547358",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:2'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                              |
|-------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------|
|[dda6986](https://git.sagemath.org/sage.git/commit/?id=dda6986e1a3b4c3d06150475d75a207c1e133394)|`Allow sage to run if sage-env is not present`|



---

archive/issue_comments_547359.json:
```json
{
    "body": "Changing commit from \"\" to \"dda6986e1a3b4c3d06150475d75a207c1e133394\"",
    "created_at": "2019-07-21T10:23:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547359",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "" to "dda6986e1a3b4c3d06150475d75a207c1e133394"



---

archive/issue_comments_547360.json:
```json
{
    "body": "Changing component from PLEASE CHANGE to distribution.",
    "created_at": "2019-07-21T10:30:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547360",
    "user": "https://github.com/antonio-rojas"
}
```

Changing component from PLEASE CHANGE to distribution.



---

archive/issue_comments_547361.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n+The sage-env script has never been particularly useful in distributions, and has actually been more of a nuisance: it needs to be heavily patched or even replaced with a custom one. \n \n+After #25786 (which removes SAGE_DOC_SRC usage at runtime) the only use for it that I can think of is setting DOT_SAGE. This patch allows to completely do away with this script, by setting a fallback for DOT_SAGE in the sage executable.\n``````\n",
    "created_at": "2019-07-21T10:30:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547361",
    "user": "https://github.com/antonio-rojas"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
+The sage-env script has never been particularly useful in distributions, and has actually been more of a nuisance: it needs to be heavily patched or even replaced with a custom one. 
 
+After #25786 (which removes SAGE_DOC_SRC usage at runtime) the only use for it that I can think of is setting DOT_SAGE. This patch allows to completely do away with this script, by setting a fallback for DOT_SAGE in the sage executable.
``````




---

archive/issue_comments_547362.json:
```json
{
    "body": "Changing dependencies from \"\" to \"#25786\"",
    "created_at": "2019-07-21T10:30:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547362",
    "user": "https://github.com/antonio-rojas"
}
```

Changing dependencies from "" to "#25786"



---

archive/issue_comments_547363.json:
```json
{
    "body": "Changing author from \"\" to \"Antonio Rojas\"",
    "created_at": "2019-07-21T10:30:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547363",
    "user": "https://github.com/antonio-rojas"
}
```

Changing author from "" to "Antonio Rojas"



---

archive/issue_comments_547364.json:
```json
{
    "body": "Changing type from PLEASE CHANGE to enhancement.",
    "created_at": "2019-07-21T10:33:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547364",
    "user": "https://github.com/antonio-rojas"
}
```

Changing type from PLEASE CHANGE to enhancement.



---

archive/issue_comments_547365.json:
```json
{
    "body": "<a id='comment:5'></a>\nI have been `sage-env` free for a few releases now in sage-on-gentoo. I ship my own sage script now, but the change I see in the commit are good. I think you should also fix `sage-cython` https://github.com/cschwan/sage-on-gentoo/blob/master/sci-mathematics/sage/files/sage-8.1-sage-cython.patch which won't work with stuff from the environment. I also have this patch for various scripts https://github.com/cschwan/sage-on-gentoo/blob/master/sci-mathematics/sage/files/sage-bin-8.7.patch\n\nI also have \n\n```\n# Replace SAGE_EXTCODE with the installation location\n\tsed -i \"s:\\$SAGE_EXTCODE:${EPREFIX}/usr/share/sage/ext:g\" \\\n\t\tbin/sage-ipynb2rst \\\n\t\tbin/sage-valgrind\n```\nwhich may be harder to replace more generally.\n\nI haven't made any fix to runtests, but `DOT_SAGE` should definitely be set - for some exotic usage with valgrind and other tools.",
    "created_at": "2019-07-21T11:07:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547365",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:5'></a>
I have been `sage-env` free for a few releases now in sage-on-gentoo. I ship my own sage script now, but the change I see in the commit are good. I think you should also fix `sage-cython` https://github.com/cschwan/sage-on-gentoo/blob/master/sci-mathematics/sage/files/sage-8.1-sage-cython.patch which won't work with stuff from the environment. I also have this patch for various scripts https://github.com/cschwan/sage-on-gentoo/blob/master/sci-mathematics/sage/files/sage-bin-8.7.patch

I also have 

```
# Replace SAGE_EXTCODE with the installation location
	sed -i "s:\$SAGE_EXTCODE:${EPREFIX}/usr/share/sage/ext:g" \
		bin/sage-ipynb2rst \
		bin/sage-valgrind
```
which may be harder to replace more generally.

I haven't made any fix to runtests, but `DOT_SAGE` should definitely be set - for some exotic usage with valgrind and other tools.



---

archive/issue_comments_547366.json:
```json
{
    "body": "<a id='comment:6'></a>\nThanks. Most of these work fine here just because /bin symlinked to /usr/bin, but that certainly shouldn't be assumed",
    "created_at": "2019-07-21T11:16:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547366",
    "user": "https://github.com/antonio-rojas"
}
```

<a id='comment:6'></a>
Thanks. Most of these work fine here just because /bin symlinked to /usr/bin, but that certainly shouldn't be assumed



---

archive/issue_comments_547367.json:
```json
{
    "body": "<a id='comment:7'></a>\nYes, I hadn't thought of that scenario at all, where the variable being undefined is actually helpful. But it will break on other system. But that doesn't help with `sage-cython`.",
    "created_at": "2019-07-21T11:20:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547367",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:7'></a>
Yes, I hadn't thought of that scenario at all, where the variable being undefined is actually helpful. But it will break on other system. But that doesn't help with `sage-cython`.



---

archive/issue_comments_547368.json:
```json
{
    "body": "Changing commit from \"dda6986e1a3b4c3d06150475d75a207c1e133394\" to \"6d049b14a3a1b77033fa09687a3942fe66f1384e\"",
    "created_at": "2019-07-21T11:43:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547368",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "dda6986e1a3b4c3d06150475d75a207c1e133394" to "6d049b14a3a1b77033fa09687a3942fe66f1384e"



---

archive/issue_comments_547369.json:
```json
{
    "body": "<a id='comment:8'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                                |\n|-------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------|\n|[e59156b](https://git.sagemath.org/sage.git/commit/?id=e59156b09f7b7c1c44e6726fe3d24ffad224a187)|`Fix sage-cython when sage-env is not available`|\n|[a52972e](https://git.sagemath.org/sage.git/commit/?id=a52972e293045854ac9996eb261f23470675cf21)|`export DOT_SAGE so it can be used in spawned scripts`|\n|[6d049b1](https://git.sagemath.org/sage.git/commit/?id=6d049b14a3a1b77033fa09687a3942fe66f1384e)|`Fix several scripts to work without sage-env`|",
    "created_at": "2019-07-21T11:43:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547369",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:8'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                                |
|-------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------|
|[e59156b](https://git.sagemath.org/sage.git/commit/?id=e59156b09f7b7c1c44e6726fe3d24ffad224a187)|`Fix sage-cython when sage-env is not available`|
|[a52972e](https://git.sagemath.org/sage.git/commit/?id=a52972e293045854ac9996eb261f23470675cf21)|`export DOT_SAGE so it can be used in spawned scripts`|
|[6d049b1](https://git.sagemath.org/sage.git/commit/?id=6d049b14a3a1b77033fa09687a3942fe66f1384e)|`Fix several scripts to work without sage-env`|



---

archive/issue_comments_547370.json:
```json
{
    "body": "<a id='comment:9'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                                                             |\n|-------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------|\n|[2dacae2](https://git.sagemath.org/sage.git/commit/?id=2dacae22b2e2da5bbfd89a3ecaec08c2f17f31f9)|`Fix sage-ipynb2rst and sage-valgrind scripts when sage-env is not available`|",
    "created_at": "2019-07-21T12:03:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547370",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:9'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                                                             |
|-------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------|
|[2dacae2](https://git.sagemath.org/sage.git/commit/?id=2dacae22b2e2da5bbfd89a3ecaec08c2f17f31f9)|`Fix sage-ipynb2rst and sage-valgrind scripts when sage-env is not available`|



---

archive/issue_comments_547371.json:
```json
{
    "body": "Changing commit from \"6d049b14a3a1b77033fa09687a3942fe66f1384e\" to \"2dacae22b2e2da5bbfd89a3ecaec08c2f17f31f9\"",
    "created_at": "2019-07-21T12:03:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547371",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "6d049b14a3a1b77033fa09687a3942fe66f1384e" to "2dacae22b2e2da5bbfd89a3ecaec08c2f17f31f9"



---

archive/issue_comments_547372.json:
```json
{
    "body": "<a id='comment:10'></a>\nReplying to [fbissey](#comment%3A5):\n> I also have \n> \n> ```\n> # Replace SAGE_EXTCODE with the installation location\n> \tsed -i \"s:\\$SAGE_EXTCODE:${EPREFIX}/usr/share/sage/ext:g\" \\\n> \t\tbin/sage-ipynb2rst \\\n> \t\tbin/sage-valgrind\n> ```\n> which may be harder to replace more generally.\n\n\nFixed in the latest commit (in a somewhat hackish way, would be nice if someone could come up with a nicer way to do it)",
    "created_at": "2019-07-21T12:05:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547372",
    "user": "https://github.com/antonio-rojas"
}
```

<a id='comment:10'></a>
Replying to [fbissey](#comment%3A5):
> I also have 
> 
> ```
> # Replace SAGE_EXTCODE with the installation location
> 	sed -i "s:\$SAGE_EXTCODE:${EPREFIX}/usr/share/sage/ext:g" \
> 		bin/sage-ipynb2rst \
> 		bin/sage-valgrind
> ```
> which may be harder to replace more generally.


Fixed in the latest commit (in a somewhat hackish way, would be nice if someone could come up with a nicer way to do it)



---

archive/issue_comments_547373.json:
```json
{
    "body": "<a id='comment:11'></a>\nReplying to [arojas](#comment%3A10):\n> Replying to [fbissey](#comment%3A5):\n> > I also have \n> > \n> > ```\n> > # Replace SAGE_EXTCODE with the installation location\n> > \tsed -i \"s:\\$SAGE_EXTCODE:${EPREFIX}/usr/share/sage/ext:g\" \\\n> > \t\tbin/sage-ipynb2rst \\\n> > \t\tbin/sage-valgrind\n> > ```\n> > which may be harder to replace more generally.\n\n> \n> Fixed in the latest commit (in a somewhat hackish way, would be nice if someone could come up with a nicer way to do it)\n\n\nCertainly does look hackish. I would have never thought of using `print` that way. Does this really work?",
    "created_at": "2019-07-21T20:57:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547373",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:11'></a>
Replying to [arojas](#comment%3A10):
> Replying to [fbissey](#comment%3A5):
> > I also have 
> > 
> > ```
> > # Replace SAGE_EXTCODE with the installation location
> > 	sed -i "s:\$SAGE_EXTCODE:${EPREFIX}/usr/share/sage/ext:g" \
> > 		bin/sage-ipynb2rst \
> > 		bin/sage-valgrind
> > ```
> > which may be harder to replace more generally.

> 
> Fixed in the latest commit (in a somewhat hackish way, would be nice if someone could come up with a nicer way to do it)


Certainly does look hackish. I would have never thought of using `print` that way. Does this really work?



---

archive/issue_comments_547374.json:
```json
{
    "body": "<a id='comment:12'></a>\n> Certainly does look hackish. I would have never thought of using `print` that way. Does this really work?\n\n\nSure it does - we use this kind of thing frequently in Arch build scripts to get the python lib dir independently of the version.",
    "created_at": "2019-07-21T21:03:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547374",
    "user": "https://github.com/antonio-rojas"
}
```

<a id='comment:12'></a>
> Certainly does look hackish. I would have never thought of using `print` that way. Does this really work?


Sure it does - we use this kind of thing frequently in Arch build scripts to get the python lib dir independently of the version.



---

archive/issue_comments_547375.json:
```json
{
    "body": "<a id='comment:13'></a>\nOK, I am reviewing all my patches and stuff for interesting bits. In `sage/doctest/control.py` There are several commands build with `$SAGE_LOCAL` in a way that requires it to be picked up from the environment (lines 1028, 1078 in code and lines 1026, 1062, 1069 for stuff that could be doctests). We probably want to import `SAGE_LOCAL` from `env.py` instead and insert that value.",
    "created_at": "2019-07-21T21:17:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547375",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:13'></a>
OK, I am reviewing all my patches and stuff for interesting bits. In `sage/doctest/control.py` There are several commands build with `$SAGE_LOCAL` in a way that requires it to be picked up from the environment (lines 1028, 1078 in code and lines 1026, 1062, 1069 for stuff that could be doctests). We probably want to import `SAGE_LOCAL` from `env.py` instead and insert that value.



---

archive/issue_comments_547376.json:
```json
{
    "body": "<a id='comment:14'></a>\nHow do you deal with `SAGE_LOCAL` in the `sage` script? Because of arch having `/usr/bin` and `/bin` symlinked you may not catch problems in line 7 and 8. There are a number of calls to `SAGE_LOCAL` in that file you are probably missing altogether.",
    "created_at": "2019-07-21T21:24:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547376",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:14'></a>
How do you deal with `SAGE_LOCAL` in the `sage` script? Because of arch having `/usr/bin` and `/bin` symlinked you may not catch problems in line 7 and 8. There are a number of calls to `SAGE_LOCAL` in that file you are probably missing altogether.



---

archive/issue_comments_547377.json:
```json
{
    "body": "<a id='comment:15'></a>\nReplying to [fbissey](#comment%3A13):\n> OK, I am reviewing all my patches and stuff for interesting bits. In `sage/doctest/control.py` There are several commands build with `$SAGE_LOCAL` in a way that requires it to be picked up from the environment (lines 1028, 1078 in code and lines 1026, 1062, 1069 for stuff that could be doctests). We probably want to import `SAGE_LOCAL` from `env.py` instead and insert that value.\n\n\nAfter line 1078 we also have `$SAGE_EXTCODE` in lines 1099 to 1101.",
    "created_at": "2019-07-21T22:06:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547377",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:15'></a>
Replying to [fbissey](#comment%3A13):
> OK, I am reviewing all my patches and stuff for interesting bits. In `sage/doctest/control.py` There are several commands build with `$SAGE_LOCAL` in a way that requires it to be picked up from the environment (lines 1028, 1078 in code and lines 1026, 1062, 1069 for stuff that could be doctests). We probably want to import `SAGE_LOCAL` from `env.py` instead and insert that value.


After line 1078 we also have `$SAGE_EXTCODE` in lines 1099 to 1101.



---

archive/issue_comments_547378.json:
```json
{
    "body": "Changing branch from \"u/arojas/allow_sage_to_run_in_the_absence_of_sage_env\" to \"u/fbissey/allow_sage_to_run_in_the_absence_of_sage_env\"",
    "created_at": "2019-07-21T22:23:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547378",
    "user": "https://github.com/kiwifb"
}
```

Changing branch from "u/arojas/allow_sage_to_run_in_the_absence_of_sage_env" to "u/fbissey/allow_sage_to_run_in_the_absence_of_sage_env"



---

archive/issue_comments_547379.json:
```json
{
    "body": "Changing commit from \"2dacae22b2e2da5bbfd89a3ecaec08c2f17f31f9\" to \"bbaf22f491ab6d334885ba4762256278d18a1bff\"",
    "created_at": "2019-07-21T22:23:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547379",
    "user": "https://github.com/kiwifb"
}
```

Changing commit from "2dacae22b2e2da5bbfd89a3ecaec08c2f17f31f9" to "bbaf22f491ab6d334885ba4762256278d18a1bff"



---

archive/issue_comments_547380.json:
```json
{
    "body": "<a id='comment:16'></a>\nAdding my ideas for `sage/doctests/control.py`. Feel free to regain control of the branch.\n\n---\nNew commits:\n|                                                                                                                                          |                                                              |\n|------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------|\n|[bbaf22f](https://git.sagemath.org/sage.git/commit?id=bbaf22f491ab6d334885ba4762256278d18a1bff)|`Fix control.py so it doesn't depend on values from sage-env.`|",
    "created_at": "2019-07-21T22:23:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547380",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:16'></a>
Adding my ideas for `sage/doctests/control.py`. Feel free to regain control of the branch.

---
New commits:
|                                                                                                                                          |                                                              |
|------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------|
|[bbaf22f](https://git.sagemath.org/sage.git/commit?id=bbaf22f491ab6d334885ba4762256278d18a1bff)|`Fix control.py so it doesn't depend on values from sage-env.`|



---

archive/issue_comments_547381.json:
```json
{
    "body": "<a id='comment:17'></a>\nIt may be impossible to completely fix the `sage` script at this stage. Best efforts at improvement would be acceptable to me.",
    "created_at": "2019-07-21T22:26:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547381",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:17'></a>
It may be impossible to completely fix the `sage` script at this stage. Best efforts at improvement would be acceptable to me.



---

archive/issue_comments_547382.json:
```json
{
    "body": "Changing branch from \"u/fbissey/allow_sage_to_run_in_the_absence_of_sage_env\" to \"u/arojas/allow_sage_to_run_in_the_absence_of_sage_env\"",
    "created_at": "2019-07-21T22:57:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547382",
    "user": "https://github.com/antonio-rojas"
}
```

Changing branch from "u/fbissey/allow_sage_to_run_in_the_absence_of_sage_env" to "u/arojas/allow_sage_to_run_in_the_absence_of_sage_env"



---

archive/issue_comments_547383.json:
```json
{
    "body": "Changing commit from \"bbaf22f491ab6d334885ba4762256278d18a1bff\" to \"25c686bc4c5696eb63a5ba1fd44f2529749cae8d\"",
    "created_at": "2019-07-21T22:59:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547383",
    "user": "https://github.com/antonio-rojas"
}
```

Changing commit from "bbaf22f491ab6d334885ba4762256278d18a1bff" to "25c686bc4c5696eb63a5ba1fd44f2529749cae8d"



---

archive/issue_comments_547384.json:
```json
{
    "body": "<a id='comment:19'></a>\nI think all SAGE_LOCAL usage in the sage executable part that is relevant for !sage-the-distro is gone now\n\n---\nNew commits:\n|                                                                                                                                          |                                             |\n|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------|\n|[2c31b26](https://git.sagemath.org/sage.git/commit?id=2c31b262effd66f591286ec6629dda65ea2d6991)|`Replace SAGE_LOCAL usage in sage executable`|\n|[25c686b](https://git.sagemath.org/sage.git/commit?id=25c686bc4c5696eb63a5ba1fd44f2529749cae8d)|`Skip uncompiled tarball check if sage-env is not present`|",
    "created_at": "2019-07-21T22:59:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547384",
    "user": "https://github.com/antonio-rojas"
}
```

<a id='comment:19'></a>
I think all SAGE_LOCAL usage in the sage executable part that is relevant for !sage-the-distro is gone now

---
New commits:
|                                                                                                                                          |                                             |
|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------|
|[2c31b26](https://git.sagemath.org/sage.git/commit?id=2c31b262effd66f591286ec6629dda65ea2d6991)|`Replace SAGE_LOCAL usage in sage executable`|
|[25c686b](https://git.sagemath.org/sage.git/commit?id=25c686bc4c5696eb63a5ba1fd44f2529749cae8d)|`Skip uncompiled tarball check if sage-env is not present`|



---

archive/issue_comments_547385.json:
```json
{
    "body": "<a id='comment:20'></a>\nI don't think there is anything to add. Time for review?",
    "created_at": "2019-07-21T23:01:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547385",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:20'></a>
I don't think there is anything to add. Time for review?



---

archive/issue_comments_547386.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2019-07-22T02:07:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547386",
    "user": "https://github.com/kiwifb"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_547387.json:
```json
{
    "body": "<a id='comment:21'></a>\nI am putting in \"needs review\" to start the patchbots (I hope).",
    "created_at": "2019-07-22T02:07:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547387",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:21'></a>
I am putting in "needs review" to start the patchbots (I hope).



---

archive/issue_comments_547388.json:
```json
{
    "body": "<a id='comment:22'></a>\nSome tests are failing locally without sage-env\n\n```\nsage -t --long /usr/lib/python2.7/site-packages/sage/misc/sagedoc.py  # 1 doctest failed\nsage -t --long /usr/lib/python2.7/site-packages/sage/misc/sphinxify.py  # 4 doctests failed\nsage -t --long /usr/lib/python2.7/site-packages/sage/libs/singular/function.pyx  # 1 doctest failed\nsage -t --long /usr/lib/python2.7/site-packages/sage/interfaces/singular.py  # 5 doctests failed\n```",
    "created_at": "2019-07-22T06:56:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547388",
    "user": "https://github.com/antonio-rojas"
}
```

<a id='comment:22'></a>
Some tests are failing locally without sage-env

```
sage -t --long /usr/lib/python2.7/site-packages/sage/misc/sagedoc.py  # 1 doctest failed
sage -t --long /usr/lib/python2.7/site-packages/sage/misc/sphinxify.py  # 4 doctests failed
sage -t --long /usr/lib/python2.7/site-packages/sage/libs/singular/function.pyx  # 1 doctest failed
sage -t --long /usr/lib/python2.7/site-packages/sage/interfaces/singular.py  # 5 doctests failed
```



---

archive/issue_comments_547389.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2019-07-22T06:56:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547389",
    "user": "https://github.com/antonio-rojas"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_547390.json:
```json
{
    "body": "Changing commit from \"25c686bc4c5696eb63a5ba1fd44f2529749cae8d\" to \"045e01c75afdc50562bf3335c9c9977defec0d0d\"",
    "created_at": "2019-07-22T07:23:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547390",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "25c686bc4c5696eb63a5ba1fd44f2529749cae8d" to "045e01c75afdc50562bf3335c9c9977defec0d0d"



---

archive/issue_comments_547391.json:
```json
{
    "body": "<a id='comment:23'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                                                                                         |\n|-------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------|\n|[c86bd9d](https://git.sagemath.org/sage.git/commit/?id=c86bd9d765fc39e3b8b7fea3a434c7bcf71102bb)|`Search for custom html themes also in SAGE_DOC, so they are found at runtime if SAGE_DOC_SRC is not set`|\n|[045e01c](https://git.sagemath.org/sage.git/commit/?id=045e01c75afdc50562bf3335c9c9977defec0d0d)|`Define SINGULARPATH in env.py so it is available at runtime when sage-env is not imported`|",
    "created_at": "2019-07-22T07:23:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547391",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:23'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                                                                                         |
|-------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------|
|[c86bd9d](https://git.sagemath.org/sage.git/commit/?id=c86bd9d765fc39e3b8b7fea3a434c7bcf71102bb)|`Search for custom html themes also in SAGE_DOC, so they are found at runtime if SAGE_DOC_SRC is not set`|
|[045e01c](https://git.sagemath.org/sage.git/commit/?id=045e01c75afdc50562bf3335c9c9977defec0d0d)|`Define SINGULARPATH in env.py so it is available at runtime when sage-env is not imported`|



---

archive/issue_comments_547392.json:
```json
{
    "body": "<a id='comment:24'></a>\nAll tests pass now. This branch conflicts with #25786 so it will need rebasing once that one goes in.",
    "created_at": "2019-07-22T07:24:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547392",
    "user": "https://github.com/antonio-rojas"
}
```

<a id='comment:24'></a>
All tests pass now. This branch conflicts with #25786 so it will need rebasing once that one goes in.



---

archive/issue_comments_547393.json:
```json
{
    "body": "<a id='comment:25'></a>\nI missed the singularpath one because I deal with it in the same patch as I do adjustments to `env.py`. I neglected that. I don't have a problem with `SAGE_DOC_SRC` because I have it default to `SAGE_DOC`  not `SAGE_SRC/doc` in sage-on-gentoo. I am surprised that's the only place you had to fix. I am expecting anything calling `SAGE_DOC_SRC` at runtime to fail if left at that value.\n\nI know there is no doctests in `sage/misc/copying.py` (https://github.com/sagemath/sage/blob/master/src/sage/misc/copying.py) but I am wondering if you do anything about it.",
    "created_at": "2019-07-22T07:40:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547393",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:25'></a>
I missed the singularpath one because I deal with it in the same patch as I do adjustments to `env.py`. I neglected that. I don't have a problem with `SAGE_DOC_SRC` because I have it default to `SAGE_DOC`  not `SAGE_SRC/doc` in sage-on-gentoo. I am surprised that's the only place you had to fix. I am expecting anything calling `SAGE_DOC_SRC` at runtime to fail if left at that value.

I know there is no doctests in `sage/misc/copying.py` (https://github.com/sagemath/sage/blob/master/src/sage/misc/copying.py) but I am wondering if you do anything about it.



---

archive/issue_comments_547394.json:
```json
{
    "body": "<a id='comment:26'></a>\nReplying to [fbissey](#comment%3A25):\n> I am surprised that's the only place you had to fix. I am expecting anything calling `SAGE_DOC_SRC` at runtime to fail if left at that value.\n\n\nThe only remaining usages of SAGE_DOC_SRC in sagelib are in sage/doctest/control.py (which adds it to the list of tested dirs when calling sage -t -a) and sage/docs/conf.py (where AFAICS all usages besides the one fixed here are for doc build), so I don't expect more failures.\n\n> I know there is no doctests in `sage/misc/copying.py` (https://github.com/sagemath/sage/blob/master/src/sage/misc/copying.py) but I am wondering if you do anything about it.\n\n\nNo. COPYING.txt should probably be installed somewhere in SAGE_SHARE. But given that this is currently broken for distros anyway, I'd say it's material for a different ticket.",
    "created_at": "2019-07-22T08:23:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547394",
    "user": "https://github.com/antonio-rojas"
}
```

<a id='comment:26'></a>
Replying to [fbissey](#comment%3A25):
> I am surprised that's the only place you had to fix. I am expecting anything calling `SAGE_DOC_SRC` at runtime to fail if left at that value.


The only remaining usages of SAGE_DOC_SRC in sagelib are in sage/doctest/control.py (which adds it to the list of tested dirs when calling sage -t -a) and sage/docs/conf.py (where AFAICS all usages besides the one fixed here are for doc build), so I don't expect more failures.

> I know there is no doctests in `sage/misc/copying.py` (https://github.com/sagemath/sage/blob/master/src/sage/misc/copying.py) but I am wondering if you do anything about it.


No. COPYING.txt should probably be installed somewhere in SAGE_SHARE. But given that this is currently broken for distros anyway, I'd say it's material for a different ticket.



---

archive/issue_comments_547395.json:
```json
{
    "body": "<a id='comment:27'></a>\nReplying to [arojas](#comment%3A26):\n> Replying to [fbissey](#comment%3A25):\n> > I am surprised that's the only place you had to fix. I am expecting anything calling `SAGE_DOC_SRC` at runtime to fail if left at that value.\n\n> \n> The only remaining usages of SAGE_DOC_SRC in sagelib are in sage/doctest/control.py (which adds it to the list of tested dirs when calling sage -t -a) and sage/docs/conf.py (where AFAICS all usages besides the one fixed here are for doc build), so I don't expect more failures.\n\n\nCool, that's a clean up that's almost done then.\n\n> \n> > I know there is no doctests in `sage/misc/copying.py` (https://github.com/sagemath/sage/blob/master/src/sage/misc/copying.py) but I am wondering if you do anything about it.\n\n> \n> No. COPYING.txt should probably be installed somewhere in SAGE_SHARE. But given that this is currently broken for distros anyway, I'd say it's material for a different ticket.\n\n\nAgreed. What to do with that file may be dependent on distro policies as well. It belongs to the location I set `SAGE_DOC` to, by policy in sage-on-gentoo.\n\nThis ticket is dealing with a good number of loose ends, it is quite nice.",
    "created_at": "2019-07-22T08:31:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547395",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:27'></a>
Replying to [arojas](#comment%3A26):
> Replying to [fbissey](#comment%3A25):
> > I am surprised that's the only place you had to fix. I am expecting anything calling `SAGE_DOC_SRC` at runtime to fail if left at that value.

> 
> The only remaining usages of SAGE_DOC_SRC in sagelib are in sage/doctest/control.py (which adds it to the list of tested dirs when calling sage -t -a) and sage/docs/conf.py (where AFAICS all usages besides the one fixed here are for doc build), so I don't expect more failures.


Cool, that's a clean up that's almost done then.

> 
> > I know there is no doctests in `sage/misc/copying.py` (https://github.com/sagemath/sage/blob/master/src/sage/misc/copying.py) but I am wondering if you do anything about it.

> 
> No. COPYING.txt should probably be installed somewhere in SAGE_SHARE. But given that this is currently broken for distros anyway, I'd say it's material for a different ticket.


Agreed. What to do with that file may be dependent on distro policies as well. It belongs to the location I set `SAGE_DOC` to, by policy in sage-on-gentoo.

This ticket is dealing with a good number of loose ends, it is quite nice.



---

archive/issue_comments_547396.json:
```json
{
    "body": "<a id='comment:28'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                                                                                                  |\n|-------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------|\n|[c960520](https://git.sagemath.org/sage.git/commit/?id=c9605200e2b5a1cadefb868fd28ef13ef2bef8c3)|`Merge branch 'develop' of git://trac.sagemath.org/sage into t/28225/allow_sage_to_run_in_the_absence_of_sage_env`|",
    "created_at": "2019-07-29T17:34:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547396",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:28'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                                                                                                  |
|-------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------|
|[c960520](https://git.sagemath.org/sage.git/commit/?id=c9605200e2b5a1cadefb868fd28ef13ef2bef8c3)|`Merge branch 'develop' of git://trac.sagemath.org/sage into t/28225/allow_sage_to_run_in_the_absence_of_sage_env`|



---

archive/issue_comments_547397.json:
```json
{
    "body": "Changing commit from \"045e01c75afdc50562bf3335c9c9977defec0d0d\" to \"c9605200e2b5a1cadefb868fd28ef13ef2bef8c3\"",
    "created_at": "2019-07-29T17:34:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547397",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "045e01c75afdc50562bf3335c9c9977defec0d0d" to "c9605200e2b5a1cadefb868fd28ef13ef2bef8c3"



---

archive/issue_comments_547398.json:
```json
{
    "body": "<a id='comment:29'></a>\nRebased on top of beta 4",
    "created_at": "2019-07-29T17:36:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547398",
    "user": "https://github.com/antonio-rojas"
}
```

<a id='comment:29'></a>
Rebased on top of beta 4



---

archive/issue_comments_547399.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2019-07-29T17:36:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547399",
    "user": "https://github.com/antonio-rojas"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_547400.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2019-07-29T20:44:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547400",
    "user": "https://github.com/kiwifb"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_547401.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Fran\u00e7ois Bissey\"",
    "created_at": "2019-07-29T20:44:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547401",
    "user": "https://github.com/kiwifb"
}
```

Changing reviewer from "" to "François Bissey"



---

archive/issue_comments_547402.json:
```json
{
    "body": "<a id='comment:30'></a>\nWell, it all looks good to me. Anyone knows why the patchbot doesn't seem to want to pick it? Is it too early after 8.9.beta4?",
    "created_at": "2019-07-29T20:44:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547402",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:30'></a>
Well, it all looks good to me. Anyone knows why the patchbot doesn't seem to want to pick it? Is it too early after 8.9.beta4?



---

archive/issue_comments_547403.json:
```json
{
    "body": "<a id='comment:31'></a>\nThis is being merge-tested right now. Not runtime but build time, I wish I had spotted some instances of `os.environ` in `sage_setup/docbuild/__init__.py`. \n\nI am hoping the ticket will be merged in the current state and then we can address those little left overs in a follow up ticket.",
    "created_at": "2019-08-02T01:36:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547403",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:31'></a>
This is being merge-tested right now. Not runtime but build time, I wish I had spotted some instances of `os.environ` in `sage_setup/docbuild/__init__.py`. 

I am hoping the ticket will be merged in the current state and then we can address those little left overs in a follow up ticket.



---

archive/issue_comments_547404.json:
```json
{
    "body": "<a id='comment:32'></a>\nAnother kink I missed before because of my setting of `SAGE_DOC_SRC`. sage doctest the documentation source not the installed one. I guess it also test the source code by looking at SAGE_SRC which default to SAGE_LIB when SAGE_ROOT is undefined. May be we should have gone for a similar model for SAGE_DOC_SRC - instead of the fix currently in `sage/docs/conf.py`?",
    "created_at": "2019-08-02T09:43:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547404",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:32'></a>
Another kink I missed before because of my setting of `SAGE_DOC_SRC`. sage doctest the documentation source not the installed one. I guess it also test the source code by looking at SAGE_SRC which default to SAGE_LIB when SAGE_ROOT is undefined. May be we should have gone for a similar model for SAGE_DOC_SRC - instead of the fix currently in `sage/docs/conf.py`?



---

archive/issue_comments_547405.json:
```json
{
    "body": "<a id='comment:33'></a>\nBranch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:\n|                                                                                                                                           |                                                                                                                    |\n|-------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------|\n|[83a6cec](https://git.sagemath.org/sage.git/commit/?id=83a6cec200e7179a632daee8658ac95b561d92ac)|`Set SAGE_DOC as fallback for SAGE_DOC_SRC in env.py, and make sure that the fallback is used if SAGE_ROOT is unset`|",
    "created_at": "2019-08-02T20:39:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547405",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:33'></a>
Branch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:
|                                                                                                                                           |                                                                                                                    |
|-------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------|
|[83a6cec](https://git.sagemath.org/sage.git/commit/?id=83a6cec200e7179a632daee8658ac95b561d92ac)|`Set SAGE_DOC as fallback for SAGE_DOC_SRC in env.py, and make sure that the fallback is used if SAGE_ROOT is unset`|



---

archive/issue_comments_547406.json:
```json
{
    "body": "Changing status from positive_review to needs_review.",
    "created_at": "2019-08-02T20:39:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547406",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing status from positive_review to needs_review.



---

archive/issue_comments_547407.json:
```json
{
    "body": "Changing commit from \"c9605200e2b5a1cadefb868fd28ef13ef2bef8c3\" to \"83a6cec200e7179a632daee8658ac95b561d92ac\"",
    "created_at": "2019-08-02T20:39:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547407",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "c9605200e2b5a1cadefb868fd28ef13ef2bef8c3" to "83a6cec200e7179a632daee8658ac95b561d92ac"



---

archive/issue_comments_547408.json:
```json
{
    "body": "<a id='comment:34'></a>\nI should have said to wait for a follow up ticket. Nevermind, let's see how it plays out.",
    "created_at": "2019-08-02T20:42:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547408",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:34'></a>
I should have said to wait for a follow up ticket. Nevermind, let's see how it plays out.



---

archive/issue_comments_547409.json:
```json
{
    "body": "<a id='comment:35'></a>\nSome version of this ticket was merged into 8.9.beta5, and I'm guessing the last commit didn't make it. In any case, the changes that were merged cause an error with Python 3:\n\n```\nsage -t --warn-long 78.7 src/sage/tests/cmdline.py\n**********************************************************************\nFile \"src/sage/tests/cmdline.py\", line 503, in sage.tests.cmdline.test_executable\nFailed example:\n    print(err)\nExpected:\n    Cython (http://cython.org) is a compiler for code written in the\n    Cython language.  Cython is based on Pyrex by Greg Ewing.\n    ...\nGot:\n    Traceback (most recent call last):\n      File \"/Users/palmieri/Desktop/Sage_stuff/sage_builds/PYTHON3/sage-8.9.beta5/src/bin/sage-cython\", line 12, in <module>\n        from sage.env import SAGE_SRC\n    ImportError: No module named sage.env\n    <BLANKLINE>\n```\nThe reason is that the `sage-cython` script begins\n\n```/usr/bin/env python\n```\nThat means that it uses Python 2 by default. With a Python 3 build, Python 2 doesn't know about the `sage` module.\n\nI'm not sure what you should do instead. Maybe\n\n```\ntry:\n    from sage.env import SAGE_SRC\nexcept ImportError:\n    SAGE_SRC = (something involving SAGE_ROOT or SAGE_LIB?)\n```\nWhat variables are guaranteed to be set when `sage-cython` is run?",
    "created_at": "2019-08-03T22:44:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547409",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:35'></a>
Some version of this ticket was merged into 8.9.beta5, and I'm guessing the last commit didn't make it. In any case, the changes that were merged cause an error with Python 3:

```
sage -t --warn-long 78.7 src/sage/tests/cmdline.py
**********************************************************************
File "src/sage/tests/cmdline.py", line 503, in sage.tests.cmdline.test_executable
Failed example:
    print(err)
Expected:
    Cython (http://cython.org) is a compiler for code written in the
    Cython language.  Cython is based on Pyrex by Greg Ewing.
    ...
Got:
    Traceback (most recent call last):
      File "/Users/palmieri/Desktop/Sage_stuff/sage_builds/PYTHON3/sage-8.9.beta5/src/bin/sage-cython", line 12, in <module>
        from sage.env import SAGE_SRC
    ImportError: No module named sage.env
    <BLANKLINE>
```
The reason is that the `sage-cython` script begins

```/usr/bin/env python
```
That means that it uses Python 2 by default. With a Python 3 build, Python 2 doesn't know about the `sage` module.

I'm not sure what you should do instead. Maybe

```
try:
    from sage.env import SAGE_SRC
except ImportError:
    SAGE_SRC = (something involving SAGE_ROOT or SAGE_LIB?)
```
What variables are guaranteed to be set when `sage-cython` is run?



---

archive/issue_comments_547410.json:
```json
{
    "body": "<a id='comment:36'></a>\nI think `sage-cython` should use the python sage uses otherwise, what's the point? While I don't like it, I am guessing it should use `sage-python23` instead of plain `python`. I am open to contrary opinions.\n\nWe need a follow up ticket for that last commit that didn't make and a couple of things I spotted in `sage_setup` we can throw that in as well.",
    "created_at": "2019-08-03T23:24:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547410",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:36'></a>
I think `sage-cython` should use the python sage uses otherwise, what's the point? While I don't like it, I am guessing it should use `sage-python23` instead of plain `python`. I am open to contrary opinions.

We need a follow up ticket for that last commit that didn't make and a couple of things I spotted in `sage_setup` we can throw that in as well.



---

archive/issue_comments_547411.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2019-08-03T23:51:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547411",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_events_070175.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-08-03T23:51:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28225#event-70175"
}
```



---

archive/issue_comments_547412.json:
```json
{
    "body": "<a id='comment:37'></a>\nc9605200e2b5a1cadefb868fd28ef13ef2bef8c3 was merged and is in 8.9.beta5, go and make a new ticket...",
    "created_at": "2019-08-03T23:51:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547412",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:37'></a>
c9605200e2b5a1cadefb868fd28ef13ef2bef8c3 was merged and is in 8.9.beta5, go and make a new ticket...



---

archive/issue_comments_547413.json:
```json
{
    "body": "<a id='comment:38'></a>\nReplying to [fbissey](#comment%3A36):\n> I think `sage-cython` should use the python sage uses otherwise, what's the point? While I don't like it, I am guessing it should use `sage-python23` instead of plain `python`. I am open to contrary opinions.\n\n\nThe only thing that script does is to check arguments and run `cython`, so it should run fine with the system Python. For distro use, do you even care about how SAGE_SRC is used here? Maybe you could have\n\n```\ntry:\n    SAGE_SRC = os.environ[\"SAGE_SRC\"]\nexcept ImportError:\n    SAGE_SRC = \"/does/not/exist\"\n```\nplus some explanatory comments. The whole script is deprecated, so maybe it doesn't matter much what happens to it, but you might cc jdemeyer on the followup ticket.\n \n> We need a follow up ticket for that last commit that didn't make and a couple of things I spotted in `sage_setup` we can throw that in as well.\n\n\nEither one or two: one for your followup issues, maybe a separate one to fix the doctest?",
    "created_at": "2019-08-04T00:22:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547413",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:38'></a>
Replying to [fbissey](#comment%3A36):
> I think `sage-cython` should use the python sage uses otherwise, what's the point? While I don't like it, I am guessing it should use `sage-python23` instead of plain `python`. I am open to contrary opinions.


The only thing that script does is to check arguments and run `cython`, so it should run fine with the system Python. For distro use, do you even care about how SAGE_SRC is used here? Maybe you could have

```
try:
    SAGE_SRC = os.environ["SAGE_SRC"]
except ImportError:
    SAGE_SRC = "/does/not/exist"
```
plus some explanatory comments. The whole script is deprecated, so maybe it doesn't matter much what happens to it, but you might cc jdemeyer on the followup ticket.
 
> We need a follow up ticket for that last commit that didn't make and a couple of things I spotted in `sage_setup` we can throw that in as well.


Either one or two: one for your followup issues, maybe a separate one to fix the doctest?



---

archive/issue_comments_547414.json:
```json
{
    "body": "<a id='comment:39'></a>\nFiled #28320",
    "created_at": "2019-08-04T09:02:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547414",
    "user": "https://github.com/antonio-rojas"
}
```

<a id='comment:39'></a>
Filed #28320



---

archive/issue_comments_547415.json:
```json
{
    "body": "<a id='comment:40'></a>\nFollow up: #29022",
    "created_at": "2020-01-17T19:49:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547415",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:40'></a>
Follow up: #29022



---

archive/issue_comments_547416.json:
```json
{
    "body": "<a id='comment:41'></a>\nand #25486",
    "created_at": "2020-01-17T20:28:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28225",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28225#issuecomment-547416",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:41'></a>
and #25486
