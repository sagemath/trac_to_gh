# Issue 28567: allow user to specify strategy for Groebner basis computations with Macaulay2

archive/issues_028330.json:
```json
{
    "assignees": [],
    "body": "The `groebner_basis` method for an ideal in a polynomial ring can be computed using the default Groebner basis command in Macaulay2. Here we extend the functionality by allowing the user to specify the strategy to be used by Macaulay2. \n\nCC:  @dimpase @mwageringel @antonleykin\n\nKeywords: **IMA Coding Sprint, macaulay2, interfaces**\n\nBranch/Commit: **[a22d20c](https://github.com/sagemath/sagetrac-mirror/commit/a22d20cea775dd533a51a739c8ce449b26d5aeed)**\n\nReviewer: **Markus Wageringel, Dima Pasechnik**\n\nAuthor: **Dima Pasechnik, Franco Saliola**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28567_\n\n",
    "closed_at": "2019-10-20T22:19:01Z",
    "created_at": "2019-10-06T17:42:24Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20interfaces%3A%20optional",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "allow user to specify strategy for Groebner basis computations with Macaulay2",
    "type": "issue",
    "updated_at": "2019-10-20T22:19:01Z",
    "url": "https://github.com/sagemath/sage/issues/28567",
    "user": "https://github.com/saliola"
}
```
The `groebner_basis` method for an ideal in a polynomial ring can be computed using the default Groebner basis command in Macaulay2. Here we extend the functionality by allowing the user to specify the strategy to be used by Macaulay2. 

CC:  @dimpase @mwageringel @antonleykin

Keywords: **IMA Coding Sprint, macaulay2, interfaces**

Branch/Commit: **[a22d20c](https://github.com/sagemath/sagetrac-mirror/commit/a22d20cea775dd533a51a739c8ce449b26d5aeed)**

Reviewer: **Markus Wageringel, Dima Pasechnik**

Author: **Dima Pasechnik, Franco Saliola**

_Issue created by migration from https://trac.sagemath.org/ticket/28567_





---

archive/issue_events_373941.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2019-10-06T17:42:24Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "milestone_number": null,
    "milestone_title": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28567#event-373941"
}
```



---

archive/issue_events_373942.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2019-10-06T17:42:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28567#event-373942"
}
```



---

archive/issue_comments_448359.json:
```json
{
    "body": "Commit: **[59b3215](https://github.com/sagemath/sagetrac-mirror/commit/59b3215321353ed165a573fc16462dd055fd1bb2)**",
    "created_at": "2019-10-06T17:42:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28567#issuecomment-448359",
    "user": "https://github.com/saliola"
}
```

Commit: **[59b3215](https://github.com/sagemath/sagetrac-mirror/commit/59b3215321353ed165a573fc16462dd055fd1bb2)**



---

archive/issue_comments_448360.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0d908887b2b61f69d86d31a8c348b33c0b822ee9\">0d90888</a></td><td><code>f4 and mgb opts for M2 gb computations</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/59b3215321353ed165a573fc16462dd055fd1bb2\">59b3215</a></td><td><code>add documentation and minor fixes</code></td></tr></table>\n",
    "created_at": "2019-10-06T17:42:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28567#issuecomment-448360",
    "user": "https://github.com/saliola"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0d908887b2b61f69d86d31a8c348b33c0b822ee9">0d90888</a></td><td><code>f4 and mgb opts for M2 gb computations</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/59b3215321353ed165a573fc16462dd055fd1bb2">59b3215</a></td><td><code>add documentation and minor fixes</code></td></tr></table>




---

archive/issue_comments_448361.json:
```json
{
    "body": "Branch: **[public/interfaces/m2/f4etc](https://github.com/sagemath/sagetrac-mirror/tree/public/interfaces/m2/f4etc)**",
    "created_at": "2019-10-06T17:42:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28567#issuecomment-448361",
    "user": "https://github.com/saliola"
}
```

Branch: **[public/interfaces/m2/f4etc](https://github.com/sagemath/sagetrac-mirror/tree/public/interfaces/m2/f4etc)**



---

archive/issue_events_373943.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2019-10-06T17:43:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28567#event-373943"
}
```



---

archive/issue_events_373944.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2019-10-06T17:43:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces%3A%20optional",
    "label_color": "0000ff",
    "label_name": "component: interfaces: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28567#event-373944"
}
```



---

archive/issue_events_373945.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2019-10-06T17:43:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28567#event-373945"
}
```



---

archive/issue_comments_448362.json:
```json
{
    "body": "Changed keywords from **macaulay2, interfaces, groebner basis** to **IMA Coding Sprint, macaulay2, interfaces**",
    "created_at": "2019-10-06T19:53:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28567#issuecomment-448362",
    "user": "https://github.com/saliola"
}
```

Changed keywords from **macaulay2, interfaces, groebner basis** to **IMA Coding Sprint, macaulay2, interfaces**



---

archive/issue_comments_448363.json:
```json
{
    "body": "Changed commit from **[59b3215](https://github.com/sagemath/sagetrac-mirror/commit/59b3215321353ed165a573fc16462dd055fd1bb2)** to **[1ad24c3](https://github.com/sagemath/sagetrac-mirror/commit/1ad24c3f6850e4f739a68d63a1341a11916a0127)**",
    "created_at": "2019-10-06T21:19:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28567#issuecomment-448363",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[59b3215](https://github.com/sagemath/sagetrac-mirror/commit/59b3215321353ed165a573fc16462dd055fd1bb2)** to **[1ad24c3](https://github.com/sagemath/sagetrac-mirror/commit/1ad24c3f6850e4f739a68d63a1341a11916a0127)**



---

archive/issue_comments_448364.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1ad24c3f6850e4f739a68d63a1341a11916a0127\">1ad24c3</a></td><td><code>pass F4 and MGB to M2 as strings</code></td></tr></table>\n",
    "created_at": "2019-10-06T21:19:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28567#issuecomment-448364",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1ad24c3f6850e4f739a68d63a1341a11916a0127">1ad24c3</a></td><td><code>pass F4 and MGB to M2 as strings</code></td></tr></table>




---

archive/issue_comments_448365.json:
```json
{
    "body": "Changed commit from **[1ad24c3](https://github.com/sagemath/sagetrac-mirror/commit/1ad24c3f6850e4f739a68d63a1341a11916a0127)** to **[7038ba2](https://github.com/sagemath/sagetrac-mirror/commit/7038ba2e92d2dd64fbc601677730baa0b1cf2bcd)**",
    "created_at": "2019-10-06T22:12:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28567#issuecomment-448365",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[1ad24c3](https://github.com/sagemath/sagetrac-mirror/commit/1ad24c3f6850e4f739a68d63a1341a11916a0127)** to **[7038ba2](https://github.com/sagemath/sagetrac-mirror/commit/7038ba2e92d2dd64fbc601677730baa0b1cf2bcd)**



---

archive/issue_comments_448366.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7038ba2e92d2dd64fbc601677730baa0b1cf2bcd\">7038ba2</a></td><td><code>28567: run Macaulay2 f4 and mgb doctests over a finite field</code></td></tr></table>\n",
    "created_at": "2019-10-06T22:12:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28567#issuecomment-448366",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:7" align="right">Comment 7</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7038ba2e92d2dd64fbc601677730baa0b1cf2bcd">7038ba2</a></td><td><code>28567: run Macaulay2 f4 and mgb doctests over a finite field</code></td></tr></table>




---

archive/issue_comments_448367.json:
```json
{
    "body": "Reviewer: **Markus Wageringel**",
    "created_at": "2019-10-06T22:15:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28567#issuecomment-448367",
    "user": "https://github.com/mwageringel"
}
```

Reviewer: **Markus Wageringel**



---

archive/issue_comments_448368.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nI have changed the examples in the doctests, so that the Macaulay2 `F4` and `MGB` algorithms are executed over a finite field, and also changed a small sphinx formatting problem. If you agree with my changes, you can set this to positive.",
    "created_at": "2019-10-06T22:15:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28567#issuecomment-448368",
    "user": "https://github.com/mwageringel"
}
```

<div id="comment:8" align="right">Comment 8</div>

I have changed the examples in the doctests, so that the Macaulay2 `F4` and `MGB` algorithms are executed over a finite field, and also changed a small sphinx formatting problem. If you agree with my changes, you can set this to positive.



---

archive/issue_comments_448369.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/00c028f94219dd8f5fbc68f566027ae1e80f2051\">00c028f</a></td><td><code>28567: add missing optional doctest flag</code></td></tr></table>\n",
    "created_at": "2019-10-08T15:25:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28567#issuecomment-448369",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:9" align="right">Comment 9</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/00c028f94219dd8f5fbc68f566027ae1e80f2051">00c028f</a></td><td><code>28567: add missing optional doctest flag</code></td></tr></table>




---

archive/issue_comments_448370.json:
```json
{
    "body": "Changed commit from **[7038ba2](https://github.com/sagemath/sagetrac-mirror/commit/7038ba2e92d2dd64fbc601677730baa0b1cf2bcd)** to **[00c028f](https://github.com/sagemath/sagetrac-mirror/commit/00c028f94219dd8f5fbc68f566027ae1e80f2051)**",
    "created_at": "2019-10-08T15:25:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28567#issuecomment-448370",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[7038ba2](https://github.com/sagemath/sagetrac-mirror/commit/7038ba2e92d2dd64fbc601677730baa0b1cf2bcd)** to **[00c028f](https://github.com/sagemath/sagetrac-mirror/commit/00c028f94219dd8f5fbc68f566027ae1e80f2051)**



---

archive/issue_events_373946.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2019-10-08T16:56:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28567#event-373946"
}
```



---

archive/issue_events_373947.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2019-10-08T16:56:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28567#event-373947"
}
```



---

archive/issue_comments_448371.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nThanks for the review!",
    "created_at": "2019-10-08T16:56:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28567#issuecomment-448371",
    "user": "https://github.com/saliola"
}
```

<div id="comment:11" align="right">Comment 11</div>

Thanks for the review!



---

archive/issue_comments_448372.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\n\n```\n**********************************************************************\nFile \"src/sage/misc/cachefunc.pyx\", line 929, in sage.misc.cachefunc.CachedFunction._sage_src_lines_\nFailed example:\n    l in sage_getsourcelines(I.groebner_basis)[0] # indirect doctest\nExpected:\n    True\nGot:\n    False\n**********************************************************************\n1 item had failures:\n   1 of   6 in sage.misc.cachefunc.CachedFunction._sage_src_lines_\n    [825 tests, 1 failure, 10.34 s]\n----------------------------------------------------------------------\nsage -t --warn-long 45.7 src/sage/misc/cachefunc.pyx  # 1 doctest failed\n----------------------------------------------------------------------\n```",
    "created_at": "2019-10-11T22:36:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28567#issuecomment-448372",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:12" align="right">Comment 12</div>


```
**********************************************************************
File "src/sage/misc/cachefunc.pyx", line 929, in sage.misc.cachefunc.CachedFunction._sage_src_lines_
Failed example:
    l in sage_getsourcelines(I.groebner_basis)[0] # indirect doctest
Expected:
    True
Got:
    False
**********************************************************************
1 item had failures:
   1 of   6 in sage.misc.cachefunc.CachedFunction._sage_src_lines_
    [825 tests, 1 failure, 10.34 s]
----------------------------------------------------------------------
sage -t --warn-long 45.7 src/sage/misc/cachefunc.pyx  # 1 doctest failed
----------------------------------------------------------------------
```



---

archive/issue_events_373948.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-10-11T22:36:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28567#event-373948"
}
```



---

archive/issue_events_373949.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-10-11T22:36:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28567#event-373949"
}
```



---

archive/issue_comments_448373.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nOK, I'll fix this, it's easy.",
    "created_at": "2019-10-14T10:06:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28567#issuecomment-448373",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:13" align="right">Comment 13</div>

OK, I'll fix this, it's easy.



---

archive/issue_comments_448374.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2605f9f3acb8a9bb2c524c09e6ae2c371a05673b\">2605f9f</a></td><td><code>f4 and mgb opts for M2 gb computations</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/41ce073ca2a222d50b92d62f84a4ecf276784bd4\">41ce073</a></td><td><code>add documentation and minor fixes</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4a3a7c0e373fdbde6e060fb27b20b1428f5cb749\">4a3a7c0</a></td><td><code>pass F4 and MGB to M2 as strings</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/48e1c87a4e9149de09680a5c9d4c11d9c00277bc\">48e1c87</a></td><td><code>28567: run Macaulay2 f4 and mgb doctests over a finite field</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7058795f794396b578f29ae9713855aafdc43789\">7058795</a></td><td><code>28567: add missing optional doctest flag</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a22d20cea775dd533a51a739c8ce449b26d5aeed\">a22d20c</a></td><td><code>changed, as the source of groebner_basis changed</code></td></tr></table>\n",
    "created_at": "2019-10-14T10:40:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28567#issuecomment-448374",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:14" align="right">Comment 14</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2605f9f3acb8a9bb2c524c09e6ae2c371a05673b">2605f9f</a></td><td><code>f4 and mgb opts for M2 gb computations</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/41ce073ca2a222d50b92d62f84a4ecf276784bd4">41ce073</a></td><td><code>add documentation and minor fixes</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4a3a7c0e373fdbde6e060fb27b20b1428f5cb749">4a3a7c0</a></td><td><code>pass F4 and MGB to M2 as strings</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/48e1c87a4e9149de09680a5c9d4c11d9c00277bc">48e1c87</a></td><td><code>28567: run Macaulay2 f4 and mgb doctests over a finite field</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7058795f794396b578f29ae9713855aafdc43789">7058795</a></td><td><code>28567: add missing optional doctest flag</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a22d20cea775dd533a51a739c8ce449b26d5aeed">a22d20c</a></td><td><code>changed, as the source of groebner_basis changed</code></td></tr></table>




---

archive/issue_comments_448375.json:
```json
{
    "body": "Changed commit from **[00c028f](https://github.com/sagemath/sagetrac-mirror/commit/00c028f94219dd8f5fbc68f566027ae1e80f2051)** to **[a22d20c](https://github.com/sagemath/sagetrac-mirror/commit/a22d20cea775dd533a51a739c8ce449b26d5aeed)**",
    "created_at": "2019-10-14T10:40:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28567#issuecomment-448375",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[00c028f](https://github.com/sagemath/sagetrac-mirror/commit/00c028f94219dd8f5fbc68f566027ae1e80f2051)** to **[a22d20c](https://github.com/sagemath/sagetrac-mirror/commit/a22d20cea775dd533a51a739c8ce449b26d5aeed)**



---

archive/issue_comments_448376.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">Comment 15</div>\n\nmade adjustment for change in source, rebase",
    "created_at": "2019-10-14T10:44:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28567#issuecomment-448376",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:15" align="right">Comment 15</div>

made adjustment for change in source, rebase



---

archive/issue_comments_448377.json:
```json
{
    "body": "Changed reviewer from **Markus Wageringel** to **Markus Wageringel, Dima Pasechnik**",
    "created_at": "2019-10-14T10:44:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28567#issuecomment-448377",
    "user": "https://github.com/dimpase"
}
```

Changed reviewer from **Markus Wageringel** to **Markus Wageringel, Dima Pasechnik**



---

archive/issue_events_373950.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2019-10-14T10:44:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28567#event-373950"
}
```



---

archive/issue_events_373951.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2019-10-14T10:44:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28567#event-373951"
}
```



---

archive/issue_comments_448378.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\nSorry about that. Sometimes it is hard to see the relevant failures in the patchbot results.",
    "created_at": "2019-10-15T01:26:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28567#issuecomment-448378",
    "user": "https://github.com/mwageringel"
}
```

<div id="comment:16" align="right">Comment 16</div>

Sorry about that. Sometimes it is hard to see the relevant failures in the patchbot results.



---

archive/issue_events_373952.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-10-20T22:19:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28567#event-373952"
}
```



---

archive/issue_events_373953.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "56920d53f862291149309aa730ac4b3c5edcbd0d",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-10-20T22:19:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28567#event-373953"
}
```



---

archive/issue_comments_448379.json:
```json
{
    "body": "Changed branch from **[public/interfaces/m2/f4etc](https://github.com/sagemath/sagetrac-mirror/tree/public/interfaces/m2/f4etc)** to **[a22d20c](https://github.com/sagemath/sagetrac-mirror/commit/a22d20cea775dd533a51a739c8ce449b26d5aeed)**",
    "created_at": "2019-10-20T22:19:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28567",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28567#issuecomment-448379",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/interfaces/m2/f4etc](https://github.com/sagemath/sagetrac-mirror/tree/public/interfaces/m2/f4etc)** to **[a22d20c](https://github.com/sagemath/sagetrac-mirror/commit/a22d20cea775dd533a51a739c8ce449b26d5aeed)**
