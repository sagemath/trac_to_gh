# Issue 28961: Better class hierarchy for classical matrix Lie algebras

archive/issues_028724.json:
```json
{
    "assignees": [],
    "body": "Right now, `ClassicalMatrixLieAlgebra` only inherits from `LieAlgebraFromAssociative`, but instead it should inherit from `MatrixLieAlgebraFromAssociative`. This way, all of its elements will be immutable after creation.\n\nWe also lift the `matrix` element method up to a more natural place from this.\n\nCC:  @slel @darijgr @fchapoton\n\nKeywords: **lie algebras**\n\nBranch/Commit: **[83b09a7](https://github.com/sagemath/sagetrac-mirror/commit/83b09a76fa78fac7d2f35c4fcd7405e6ad1ecc50)**\n\nReviewer: **Darij Grinberg**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28961_\n\n",
    "closed_at": "2020-01-20T21:18:25Z",
    "created_at": "2020-01-06T17:59:54Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Better class hierarchy for classical matrix Lie algebras",
    "type": "issue",
    "updated_at": "2020-01-20T21:18:25Z",
    "url": "https://github.com/sagemath/sage/issues/28961",
    "user": "https://github.com/tscrim"
}
```
Right now, `ClassicalMatrixLieAlgebra` only inherits from `LieAlgebraFromAssociative`, but instead it should inherit from `MatrixLieAlgebraFromAssociative`. This way, all of its elements will be immutable after creation.

We also lift the `matrix` element method up to a more natural place from this.

CC:  @slel @darijgr @fchapoton

Keywords: **lie algebras**

Branch/Commit: **[83b09a7](https://github.com/sagemath/sagetrac-mirror/commit/83b09a76fa78fac7d2f35c4fcd7405e6ad1ecc50)**

Reviewer: **Darij Grinberg**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/28961_





---

archive/issue_events_379658.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-01-06T17:59:54Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28961",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28961#event-379658"
}
```



---

archive/issue_events_379659.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-01-06T17:59:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28961",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28961#event-379659"
}
```



---

archive/issue_events_379660.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-01-06T17:59:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28961",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28961#event-379660"
}
```



---

archive/issue_events_379661.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-01-06T17:59:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28961",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28961#event-379661"
}
```



---

archive/issue_comments_455247.json:
```json
{
    "body": "Commit: **[e2e07c4](https://github.com/sagemath/sagetrac-mirror/commit/e2e07c4aab94ebb160ed1fd8d3159395ccc51e81)**",
    "created_at": "2020-01-06T18:00:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28961#issuecomment-455247",
    "user": "https://github.com/tscrim"
}
```

Commit: **[e2e07c4](https://github.com/sagemath/sagetrac-mirror/commit/e2e07c4aab94ebb160ed1fd8d3159395ccc51e81)**



---

archive/issue_comments_455248.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e2e07c4aab94ebb160ed1fd8d3159395ccc51e81\">e2e07c4</a></td><td><code>Better class hierarchy for classical matrix Lie algebras.</code></td></tr></table>\n",
    "created_at": "2020-01-06T18:00:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28961#issuecomment-455248",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e2e07c4aab94ebb160ed1fd8d3159395ccc51e81">e2e07c4</a></td><td><code>Better class hierarchy for classical matrix Lie algebras.</code></td></tr></table>




---

archive/issue_events_379662.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-01-06T18:00:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28961",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28961#event-379662"
}
```



---

archive/issue_comments_455249.json:
```json
{
    "body": "Branch: **[public/lie_algebras/classical_matrix_hierarchy-28961](https://github.com/sagemath/sagetrac-mirror/tree/public/lie_algebras/classical_matrix_hierarchy-28961)**",
    "created_at": "2020-01-06T18:00:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28961#issuecomment-455249",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/lie_algebras/classical_matrix_hierarchy-28961](https://github.com/sagemath/sagetrac-mirror/tree/public/lie_algebras/classical_matrix_hierarchy-28961)**



---

archive/issue_comments_455250.json:
```json
{
    "body": "Changed commit from **[e2e07c4](https://github.com/sagemath/sagetrac-mirror/commit/e2e07c4aab94ebb160ed1fd8d3159395ccc51e81)** to **[83b09a7](https://github.com/sagemath/sagetrac-mirror/commit/83b09a76fa78fac7d2f35c4fcd7405e6ad1ecc50)**",
    "created_at": "2020-01-13T19:07:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28961#issuecomment-455250",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[e2e07c4](https://github.com/sagemath/sagetrac-mirror/commit/e2e07c4aab94ebb160ed1fd8d3159395ccc51e81)** to **[83b09a7](https://github.com/sagemath/sagetrac-mirror/commit/83b09a76fa78fac7d2f35c4fcd7405e6ad1ecc50)**



---

archive/issue_comments_455251.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3f0d79982af217d6909ee25851ca49722cbda66e\">3f0d799</a></td><td><code>Merge branch 'public/lie_algebras/classical_matrix_hierarchy-28961' of git://trac.sagemath.org/sage into 28961</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/83b09a76fa78fac7d2f35c4fcd7405e6ad1ecc50\">83b09a7</a></td><td><code>move Element class of gln to a more appropriate superclass</code></td></tr></table>\n",
    "created_at": "2020-01-13T19:07:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28961#issuecomment-455251",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:3'>Comment 3:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3f0d79982af217d6909ee25851ca49722cbda66e">3f0d799</a></td><td><code>Merge branch 'public/lie_algebras/classical_matrix_hierarchy-28961' of git://trac.sagemath.org/sage into 28961</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/83b09a76fa78fac7d2f35c4fcd7405e6ad1ecc50">83b09a7</a></td><td><code>move Element class of gln to a more appropriate superclass</code></td></tr></table>




---

archive/issue_comments_455252.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nI've added one more change that clears some technical debt (the Element class of gl should not be a ClassicalLieAlgebra element class), and improved the doc a bit. Tests in sage/algebra pass. LGTM!",
    "created_at": "2020-01-13T19:18:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28961#issuecomment-455252",
    "user": "https://github.com/darijgr"
}
```

<a id='comment:4'>Comment 4:</a>
I've added one more change that clears some technical debt (the Element class of gl should not be a ClassicalLieAlgebra element class), and improved the doc a bit. Tests in sage/algebra pass. LGTM!



---

archive/issue_events_379663.json:
```json
{
    "actor": "https://github.com/darijgr",
    "created_at": "2020-01-13T19:18:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28961",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28961#event-379663"
}
```



---

archive/issue_events_379664.json:
```json
{
    "actor": "https://github.com/darijgr",
    "created_at": "2020-01-13T19:18:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28961",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28961#event-379664"
}
```



---

archive/issue_comments_455253.json:
```json
{
    "body": "Reviewer: **Darij Grinberg**",
    "created_at": "2020-01-13T19:18:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28961#issuecomment-455253",
    "user": "https://github.com/darijgr"
}
```

Reviewer: **Darij Grinberg**



---

archive/issue_events_379665.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-01-20T21:18:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28961",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28961#event-379665"
}
```



---

archive/issue_events_379666.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "25b20e31be3f1d68e146be59039a12ec9d68cf43",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-01-20T21:18:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28961",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28961#event-379666"
}
```



---

archive/issue_comments_455254.json:
```json
{
    "body": "Changed branch from **[public/lie_algebras/classical_matrix_hierarchy-28961](https://github.com/sagemath/sagetrac-mirror/tree/public/lie_algebras/classical_matrix_hierarchy-28961)** to **[83b09a7](https://github.com/sagemath/sagetrac-mirror/commit/83b09a76fa78fac7d2f35c4fcd7405e6ad1ecc50)**",
    "created_at": "2020-01-20T21:18:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28961",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28961#issuecomment-455254",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/lie_algebras/classical_matrix_hierarchy-28961](https://github.com/sagemath/sagetrac-mirror/tree/public/lie_algebras/classical_matrix_hierarchy-28961)** to **[83b09a7](https://github.com/sagemath/sagetrac-mirror/commit/83b09a76fa78fac7d2f35c4fcd7405e6ad1ecc50)**
