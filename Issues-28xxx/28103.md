# Issue 28103: flint-related failure in 8.9.beta0

archive/issues_027866.json:
```json
{
    "body": "in python3 only\n\nI get (with python3) this new failure in 8.9.b0:\n\n```\nFile \"src/sage/rings/polynomial/polynomial_rational_flint.pyx\", line 2055, in sage.rings.polynomial.polynomial_rational_flint.Polynomial_rational_flint.galois_group\nFailed example:\n    G = f.galois_group(); G\nExpected:\n    Transitive group number 5 of degree 4\nGot:\n    Exception (FLINT memory_manager). Unable to allocate memory.\n    Transitive group number 5 of degree 4\n```\nCould that be caused by ticket #27721 ?\n\n**CC:**  @embray\n\n**Keywords:** flint\n\nIssue created by migration from https://trac.sagemath.org/ticket/28103\n\n",
    "closed_at": "2019-10-01T19:32:52Z",
    "created_at": "2019-07-03T11:57:43Z",
    "labels": [
        "component: python3",
        "bug",
        "duplicate"
    ],
    "title": "flint-related failure in 8.9.beta0",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/28103",
    "user": "https://github.com/fchapoton"
}
```
in python3 only

I get (with python3) this new failure in 8.9.b0:

```
File "src/sage/rings/polynomial/polynomial_rational_flint.pyx", line 2055, in sage.rings.polynomial.polynomial_rational_flint.Polynomial_rational_flint.galois_group
Failed example:
    G = f.galois_group(); G
Expected:
    Transitive group number 5 of degree 4
Got:
    Exception (FLINT memory_manager). Unable to allocate memory.
    Transitive group number 5 of degree 4
```
Could that be caused by ticket #27721 ?

**CC:**  @embray

**Keywords:** flint

Issue created by migration from https://trac.sagemath.org/ticket/28103





---

archive/issue_comments_441591.json:
```json
{
    "body": "<a id='comment:2'></a>\nReported again as #28334. Since work has started there,\nlet us mark this one as duplicate.",
    "created_at": "2019-10-01T07:06:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28103",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28103#issuecomment-441591",
    "user": "https://github.com/slel"
}
```

<a id='comment:2'></a>
Reported again as #28334. Since work has started there,
let us mark this one as duplicate.



---

archive/issue_events_252498.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-10-01T07:06:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/28103",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28103#event-252498"
}
```



---

archive/issue_events_252499.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-10-01T07:06:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/28103",
    "label": "component: python3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28103#event-252499"
}
```



---

archive/issue_events_252500.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-10-01T07:06:12Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28103",
    "milestone": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28103#event-252500"
}
```



---

archive/issue_events_252501.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-10-01T07:06:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/28103",
    "label": "duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28103#event-252501"
}
```



---

archive/issue_comments_441592.json:
```json
{
    "body": "**Changing keywords** from \"\" to \"flint\".",
    "created_at": "2019-10-01T07:06:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28103",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28103#issuecomment-441592",
    "user": "https://github.com/slel"
}
```

**Changing keywords** from "" to "flint".



---

archive/issue_events_252502.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-10-01T19:32:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/28103",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28103#event-252502"
}
```



---

archive/issue_events_252503.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-10-01T19:32:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/28103",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28103#event-252503"
}
```



---

archive/issue_events_252504.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-10-01T19:32:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/28103",
    "label": "duplicate",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28103#event-252504"
}
```
