# Issue 28359: new graphics array bug for legend

archive/issues_028122.json:
```json
{
    "body": "Thank you to everyone for #27865!\n\nHowever, I think there are unintended consequences when it comes to certain types of overlapping options.  Can we set the options individually?\n\n```\nP1 = plot(prime_pi,3,30,color='black',legend_label='1')\n#P1 += plot(Li,3,30, color='green',legend_label='Li1')\nP2 = plot(prime_pi,40,70,color='blue',legend_label='2')\n#P2 += plot(Li,40,70, color='red',legend_label='Li2')\ngraphics_array([P1,P2])\n```\n\nThis doubles the labels in the second graphic.  If you uncomment the other lines more things happen, though it might all be just one bug.  Trying `graphics_array([P1,P2,P2])` only doubles the last one. \n\nI *believe* this is new since the new graphics array stuff, though I can't be sure. Thanks for looking into it!\n\nCC:  @egourgoulhon\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/28359\n\n",
    "closed_at": "2019-08-19T16:59:23Z",
    "created_at": "2019-08-15T15:33:08Z",
    "labels": [
        "component: graphics",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "new graphics array bug for legend",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/28359",
    "user": "https://github.com/kcrisman"
}
```
Thank you to everyone for #27865!

However, I think there are unintended consequences when it comes to certain types of overlapping options.  Can we set the options individually?

```
P1 = plot(prime_pi,3,30,color='black',legend_label='1')
#P1 += plot(Li,3,30, color='green',legend_label='Li1')
P2 = plot(prime_pi,40,70,color='blue',legend_label='2')
#P2 += plot(Li,40,70, color='red',legend_label='Li2')
graphics_array([P1,P2])
```

This doubles the labels in the second graphic.  If you uncomment the other lines more things happen, though it might all be just one bug.  Trying `graphics_array([P1,P2,P2])` only doubles the last one. 

I *believe* this is new since the new graphics array stuff, though I can't be sure. Thanks for looking into it!

CC:  @egourgoulhon

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/28359





---

archive/issue_comments_422913.json:
```json
{
    "body": "<a id='comment:1'></a>Are you using a version of Sage recent enough to include #27865 (i.e. >= 8.9.beta0)? I've tried with Sage 8.9.beta6 and everything is fine: the legend labels are not doubled. Uncommenting the other lines leads to a correct result as well.",
    "created_at": "2019-08-15T16:18:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28359#issuecomment-422913",
    "user": "https://github.com/egourgoulhon"
}
```

<a id='comment:1'></a>Are you using a version of Sage recent enough to include #27865 (i.e. >= 8.9.beta0)? I've tried with Sage 8.9.beta6 and everything is fine: the legend labels are not doubled. Uncommenting the other lines leads to a correct result as well.



---

archive/issue_comments_422914.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2019-08-15T18:46:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28359#issuecomment-422914",
    "user": "https://github.com/kcrisman"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_422915.json:
```json
{
    "body": "<a id='comment:2'></a>Oops!  I have a *local* install that is updated, but I found this using the Sage cell server, which is of course still not-necessarily-updated - but which I was using to test some auto-generated images before actually generating them.  Sorry!",
    "created_at": "2019-08-15T18:46:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28359#issuecomment-422915",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:2'></a>Oops!  I have a *local* install that is updated, but I found this using the Sage cell server, which is of course still not-necessarily-updated - but which I was using to test some auto-generated images before actually generating them.  Sorry!



---

archive/issue_events_070832.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2019-08-15T18:46:39Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28359",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28359#event-70832"
}
```



---

archive/issue_comments_422916.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2019-08-15T18:46:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28359#issuecomment-422916",
    "user": "https://github.com/kcrisman"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_422917.json:
```json
{
    "body": "<a id='comment:4'></a>Replying to [comment:2 kcrisman]:\n> Oops!  I have a *local* install that is updated, but I found this using the Sage cell server, which is of course still not-necessarily-updated - but which I was using to test some auto-generated images before actually generating them.  Sorry!\n\n\nNo problem (it is always nice to have extra checks). Indeed, the Sage Cell server is running Sage 8.8, while #27865 was included only in 8.9.beta0.",
    "created_at": "2019-08-15T20:44:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28359#issuecomment-422917",
    "user": "https://github.com/egourgoulhon"
}
```

<a id='comment:4'></a>Replying to [comment:2 kcrisman]:
> Oops!  I have a *local* install that is updated, but I found this using the Sage cell server, which is of course still not-necessarily-updated - but which I was using to test some auto-generated images before actually generating them.  Sorry!


No problem (it is always nice to have extra checks). Indeed, the Sage Cell server is running Sage 8.8, while #27865 was included only in 8.9.beta0.



---

archive/issue_comments_422918.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2019-08-19T16:59:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28359",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28359#issuecomment-422918",
    "user": "https://github.com/fchapoton"
}
```

Resolution: fixed



---

archive/issue_events_070833.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-08-19T16:59:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/28359",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28359#event-70833"
}
```
