# Issue 28478: Include traceback when Sage crashes on startup

archive/issues_028241.json:
```json
{
    "assignees": [],
    "body": "In #12719 (2012) we upgraded to iPython 0.13 and added a custom crash handler, which doesn't display a traceback when Sage crashes at startup.  At the time I was frustrated with the change: it's annoying to have to open up the crash log when you introduce a syntax or name error, or import something from the wrong file.  I didn't realize until this afternoon that it's trivial to include a traceback by default.\n\nI think that it's not a problem to include the traceback even for non-developers, since the last message seen will be the current error report.\n\nBranch/Commit: **[2d339e6](https://github.com/sagemath/sagetrac-mirror/commit/2d339e6617f04f7fa7ac2928125dd70f8c526c2f)**\n\nReviewer: **Julian R\u00fcth**\n\nAuthor: **David Roe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28478_\n\n",
    "closed_at": "2019-10-03T17:57:33Z",
    "created_at": "2019-09-12T13:24:51Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Include traceback when Sage crashes on startup",
    "type": "issue",
    "updated_at": "2019-10-03T17:57:33Z",
    "url": "https://github.com/sagemath/sage/issues/28478",
    "user": "https://github.com/roed314"
}
```
In #12719 (2012) we upgraded to iPython 0.13 and added a custom crash handler, which doesn't display a traceback when Sage crashes at startup.  At the time I was frustrated with the change: it's annoying to have to open up the crash log when you introduce a syntax or name error, or import something from the wrong file.  I didn't realize until this afternoon that it's trivial to include a traceback by default.

I think that it's not a problem to include the traceback even for non-developers, since the last message seen will be the current error report.

Branch/Commit: **[2d339e6](https://github.com/sagemath/sagetrac-mirror/commit/2d339e6617f04f7fa7ac2928125dd70f8c526c2f)**

Reviewer: **Julian Rüth**

Author: **David Roe**

_Issue created by migration from https://trac.sagemath.org/ticket/28478_





---

archive/issue_events_372665.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2019-09-12T13:24:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28478",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28478#event-372665"
}
```



---

archive/issue_events_372666.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2019-09-12T13:24:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28478",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "component: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28478#event-372666"
}
```



---

archive/issue_events_372667.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2019-09-12T13:24:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28478",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28478#event-372667"
}
```



---

archive/issue_events_372668.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2019-09-12T13:24:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28478",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28478#event-372668"
}
```



---

archive/issue_comments_446769.json:
```json
{
    "body": "Branch: **[u/roed/28478_crash_traceback](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/28478_crash_traceback)**",
    "created_at": "2019-09-12T13:32:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28478",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28478#issuecomment-446769",
    "user": "https://github.com/roed314"
}
```

Branch: **[u/roed/28478_crash_traceback](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/28478_crash_traceback)**



---

archive/issue_events_372669.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2019-09-12T13:33:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28478",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28478#event-372669"
}
```



---

archive/issue_comments_446770.json:
```json
{
    "body": "Commit: **[2d339e6](https://github.com/sagemath/sagetrac-mirror/commit/2d339e6617f04f7fa7ac2928125dd70f8c526c2f)**",
    "created_at": "2019-09-12T13:33:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28478",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28478#issuecomment-446770",
    "user": "https://github.com/roed314"
}
```

Commit: **[2d339e6](https://github.com/sagemath/sagetrac-mirror/commit/2d339e6617f04f7fa7ac2928125dd70f8c526c2f)**



---

archive/issue_comments_446771.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2d339e6617f04f7fa7ac2928125dd70f8c526c2f\">2d339e6</a></td><td><code>Include crash traceback</code></td></tr></table>\n",
    "created_at": "2019-09-12T13:33:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28478",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28478#issuecomment-446771",
    "user": "https://github.com/roed314"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2d339e6617f04f7fa7ac2928125dd70f8c526c2f">2d339e6</a></td><td><code>Include crash traceback</code></td></tr></table>




---

archive/issue_comments_446772.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nFeel free to set to positive review once the patchbot is happy.",
    "created_at": "2019-09-12T13:34:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28478",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28478#issuecomment-446772",
    "user": "https://github.com/saraedum"
}
```

<div id="comment:3" align="right">Comment 3</div>

Feel free to set to positive review once the patchbot is happy.



---

archive/issue_comments_446773.json:
```json
{
    "body": "Reviewer: **Julian R\u00fcth**",
    "created_at": "2019-09-12T13:34:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28478",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28478#issuecomment-446773",
    "user": "https://github.com/saraedum"
}
```

Reviewer: **Julian Rüth**



---

archive/issue_events_372670.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2019-09-12T23:42:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28478",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28478#event-372670"
}
```



---

archive/issue_events_372671.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2019-09-12T23:42:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28478",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28478#event-372671"
}
```



---

archive/issue_comments_446774.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThe pyflakes warning is spurious: we're importing `line_profiler` so that it's available in the shell.",
    "created_at": "2019-09-12T23:42:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28478",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28478#issuecomment-446774",
    "user": "https://github.com/roed314"
}
```

<div id="comment:4" align="right">Comment 4</div>

The pyflakes warning is spurious: we're importing `line_profiler` so that it's available in the shell.



---

archive/issue_events_372672.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-09-30T08:12:27Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28478",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28478#event-372672"
}
```



---

archive/issue_events_372673.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-09-30T08:12:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28478",
    "milestone_number": null,
    "milestone_title": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28478#event-372673"
}
```



---

archive/issue_comments_446775.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nmoving milestone to 9.0 (after release of 8.9)",
    "created_at": "2019-09-30T08:12:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28478",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28478#issuecomment-446775",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:5" align="right">Comment 5</div>

moving milestone to 9.0 (after release of 8.9)



---

archive/issue_events_372674.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-10-03T17:57:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28478",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28478#event-372674"
}
```



---

archive/issue_events_372675.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "cced5c278dbff888077046fbeec82cc127151d2a",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-10-03T17:57:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28478",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28478#event-372675"
}
```



---

archive/issue_comments_446776.json:
```json
{
    "body": "Changed branch from **[u/roed/28478_crash_traceback](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/28478_crash_traceback)** to **[2d339e6](https://github.com/sagemath/sagetrac-mirror/commit/2d339e6617f04f7fa7ac2928125dd70f8c526c2f)**",
    "created_at": "2019-10-03T17:57:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28478",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28478#issuecomment-446776",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/roed/28478_crash_traceback](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/28478_crash_traceback)** to **[2d339e6](https://github.com/sagemath/sagetrac-mirror/commit/2d339e6617f04f7fa7ac2928125dd70f8c526c2f)**
