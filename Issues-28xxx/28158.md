# Issue 28158: PLEASE DELETE!

archive/issues_027921.json:
```json
{
    "assignees": [],
    "body": "PLEASE DELETE!\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28158_\n\n",
    "closed_at": "2019-07-10T16:45:07Z",
    "created_at": "2019-07-10T15:46:14Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "PLEASE DELETE!",
    "type": "issue",
    "updated_at": "2019-07-10T16:45:07Z",
    "url": "https://github.com/sagemath/sage/issues/28158",
    "user": "https://github.com/DeRhamSource"
}
```
PLEASE DELETE!

_Issue created by migration from https://trac.sagemath.org/ticket/28158_





---

archive/issue_events_367928.json:
```json
{
    "actor": "https://github.com/DeRhamSource",
    "created_at": "2019-07-10T15:46:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28158",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28158#event-367928"
}
```



---

archive/issue_events_367929.json:
```json
{
    "actor": "https://github.com/DeRhamSource",
    "created_at": "2019-07-10T15:46:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28158",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28158#event-367929"
}
```



---

archive/issue_comments_441059.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-\n+PLEASE DELETE!\n``````\n",
    "created_at": "2019-07-10T15:51:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28158",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28158#issuecomment-441059",
    "user": "https://github.com/DeRhamSource"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-
+PLEASE DELETE!
``````




---

archive/issue_events_367930.json:
```json
{
    "actor": "https://github.com/DeRhamSource",
    "created_at": "2019-07-10T16:18:08Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/28158",
    "title_is": "PLEASE DELETE!",
    "title_was": "Last Twin Prime",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28158#event-367930"
}
```



---

archive/issue_events_367931.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-07-10T16:45:07Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28158",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28158#event-367931"
}
```



---

archive/issue_events_367932.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-07-10T16:45:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28158",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28158#event-367932"
}
```



---

archive/issue_events_367933.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-07-10T16:45:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28158",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28158#event-367933"
}
```



---

archive/issue_events_367934.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-07-10T16:45:07Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28158",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28158#event-367934"
}
```
