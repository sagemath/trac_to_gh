# Issue 28050: improve performance of asymptotic_expansions.ImplicitExpansion

archive/issues_027813.json:
```json
{
    "assignees": [],
    "body": "Within the `ImplicitExpansion` method of the asymptotic expansion generators, very large (symbolic) equations are solved.\n\nSimplifying these equations before attempting to solve them drastically improves performance.\n\nCC:  @dkrenn @cheuberg\n\nBranch/Commit: **[cafad61](https://github.com/sagemath/sagetrac-mirror/commit/cafad61cb3e87a915e71cc04051fd90123895469)**\n\nReviewer: **Clemens Heuberger**\n\nAuthor: **Benjamin Hackl**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28050_\n\n",
    "closed_at": "2019-07-05T12:02:48Z",
    "created_at": "2019-06-24T14:42:20Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20asymptotic%20expansions",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "improve performance of asymptotic_expansions.ImplicitExpansion",
    "type": "issue",
    "updated_at": "2019-07-05T12:02:48Z",
    "url": "https://github.com/sagemath/sage/issues/28050",
    "user": "https://github.com/behackl"
}
```
Within the `ImplicitExpansion` method of the asymptotic expansion generators, very large (symbolic) equations are solved.

Simplifying these equations before attempting to solve them drastically improves performance.

CC:  @dkrenn @cheuberg

Branch/Commit: **[cafad61](https://github.com/sagemath/sagetrac-mirror/commit/cafad61cb3e87a915e71cc04051fd90123895469)**

Reviewer: **Clemens Heuberger**

Author: **Benjamin Hackl**

_Issue created by migration from https://trac.sagemath.org/ticket/28050_





---

archive/issue_events_366413.json:
```json
{
    "actor": "https://github.com/behackl",
    "created_at": "2019-06-24T14:42:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28050",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28050#event-366413"
}
```



---

archive/issue_events_366414.json:
```json
{
    "actor": "https://github.com/behackl",
    "created_at": "2019-06-24T14:42:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28050",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20asymptotic%20expansions",
    "label_color": "0000ff",
    "label_name": "component: asymptotic expansions",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28050#event-366414"
}
```



---

archive/issue_events_366415.json:
```json
{
    "actor": "https://github.com/behackl",
    "created_at": "2019-06-24T14:42:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28050",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28050#event-366415"
}
```



---

archive/issue_events_366416.json:
```json
{
    "actor": "https://github.com/behackl",
    "created_at": "2019-06-24T14:42:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28050",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28050#event-366416"
}
```



---

archive/issue_comments_439320.json:
```json
{
    "body": "Commit: **[cafad61](https://github.com/sagemath/sagetrac-mirror/commit/cafad61cb3e87a915e71cc04051fd90123895469)**",
    "created_at": "2019-06-24T14:45:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28050#issuecomment-439320",
    "user": "https://github.com/behackl"
}
```

Commit: **[cafad61](https://github.com/sagemath/sagetrac-mirror/commit/cafad61cb3e87a915e71cc04051fd90123895469)**



---

archive/issue_comments_439321.json:
```json
{
    "body": "Branch: **[u/behackl/asy/implicitexpansion-performance](https://github.com/sagemath/sagetrac-mirror/tree/u/behackl/asy/implicitexpansion-performance)**",
    "created_at": "2019-06-24T14:45:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28050#issuecomment-439321",
    "user": "https://github.com/behackl"
}
```

Branch: **[u/behackl/asy/implicitexpansion-performance](https://github.com/sagemath/sagetrac-mirror/tree/u/behackl/asy/implicitexpansion-performance)**



---

archive/issue_comments_439322.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cafad61cb3e87a915e71cc04051fd90123895469\">cafad61</a></td><td><code>call simplify_rational before solving a (large) equation</code></td></tr></table>\n",
    "created_at": "2019-06-24T14:45:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28050#issuecomment-439322",
    "user": "https://github.com/behackl"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cafad61cb3e87a915e71cc04051fd90123895469">cafad61</a></td><td><code>call simplify_rational before solving a (large) equation</code></td></tr></table>




---

archive/issue_events_366417.json:
```json
{
    "actor": "https://github.com/behackl",
    "created_at": "2019-06-24T14:45:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28050",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28050#event-366417"
}
```



---

archive/issue_comments_439323.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nCode LGTM, waiting for a patchbot to pick this up. Feel free to set to positive review once patchbot is happy.",
    "created_at": "2019-06-26T07:48:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28050#issuecomment-439323",
    "user": "https://github.com/cheuberg"
}
```

<div id="comment:2" align="right">Comment 2</div>

Code LGTM, waiting for a patchbot to pick this up. Feel free to set to positive review once patchbot is happy.



---

archive/issue_comments_439324.json:
```json
{
    "body": "Reviewer: **Clemens Heuberger**",
    "created_at": "2019-06-29T14:51:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28050#issuecomment-439324",
    "user": "https://github.com/cheuberg"
}
```

Reviewer: **Clemens Heuberger**



---

archive/issue_comments_439325.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nI guess that the patchbots ignored the ticket because there was no author listed.",
    "created_at": "2019-06-29T14:51:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28050#issuecomment-439325",
    "user": "https://github.com/cheuberg"
}
```

<div id="comment:3" align="right">Comment 3</div>

I guess that the patchbots ignored the ticket because there was no author listed.



---

archive/issue_comments_439326.json:
```json
{
    "body": "Author: **Benjamin Hackl**",
    "created_at": "2019-06-29T14:51:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28050#issuecomment-439326",
    "user": "https://github.com/cheuberg"
}
```

Author: **Benjamin Hackl**



---

archive/issue_events_366418.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2019-06-30T12:14:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28050",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28050#event-366418"
}
```



---

archive/issue_events_366419.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2019-06-30T12:14:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28050",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28050#event-366419"
}
```



---

archive/issue_comments_439327.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nOne patchbot is happy, another one had a spurious error which cannot be related to this ticket.",
    "created_at": "2019-06-30T12:14:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28050#issuecomment-439327",
    "user": "https://github.com/cheuberg"
}
```

<div id="comment:4" align="right">Comment 4</div>

One patchbot is happy, another one had a spurious error which cannot be related to this ticket.



---

archive/issue_comments_439328.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nReplying to [@cheuberg](#comment%3A3):\n> I guess that the patchbots ignored the ticket because there was no author listed.\n\nThanks for fixing this -- and also, thanks for the review.",
    "created_at": "2019-07-01T09:53:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28050#issuecomment-439328",
    "user": "https://github.com/behackl"
}
```

<div id="comment:5" align="right">Comment 5</div>

Replying to [@cheuberg](#comment%3A3):
> I guess that the patchbots ignored the ticket because there was no author listed.

Thanks for fixing this -- and also, thanks for the review.



---

archive/issue_events_366420.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-07-05T12:02:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28050",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28050#event-366420"
}
```



---

archive/issue_events_366421.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "ffa9691f98662e86fb64ad534a5b34da8b29fab0",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-07-05T12:02:48Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28050",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28050#event-366421"
}
```



---

archive/issue_comments_439329.json:
```json
{
    "body": "Changed branch from **[u/behackl/asy/implicitexpansion-performance](https://github.com/sagemath/sagetrac-mirror/tree/u/behackl/asy/implicitexpansion-performance)** to **[cafad61](https://github.com/sagemath/sagetrac-mirror/commit/cafad61cb3e87a915e71cc04051fd90123895469)**",
    "created_at": "2019-07-05T12:02:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28050",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28050#issuecomment-439329",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/behackl/asy/implicitexpansion-performance](https://github.com/sagemath/sagetrac-mirror/tree/u/behackl/asy/implicitexpansion-performance)** to **[cafad61](https://github.com/sagemath/sagetrac-mirror/commit/cafad61cb3e87a915e71cc04051fd90123895469)**
