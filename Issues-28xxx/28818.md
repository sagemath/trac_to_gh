...
```json
{
    "body": "<a id='comment:1'></a>this should make this a bit more robust and quick",
...
<a id='comment:1'></a>this should make this a bit more robust and quick
...
```json
{
    "body": "<a id='comment:4'></a>a bit of cleanup in the updated commit",
...
<a id='comment:4'></a>a bit of cleanup in the updated commit
...
<a id='comment:8'></a>after `git trac try 28818` and `sage -b`, I did:
...
```json
{
    "body": "<a id='comment:10'></a>Right. Let me know if you have a fix, or if we let these doctests for another ticket.",
...
<a id='comment:10'></a>Right. Let me know if you have a fix, or if we let these doctests for another ticket.
...
```json
{
    "body": "<a id='comment:12'></a>OK, there was a failure in Graph's understanding that edges are edges.",
...
<a id='comment:12'></a>OK, there was a failure in Graph's understanding that edges are edges.
...
<a id='comment:13'></a>In `strongly_regular_db.pyx`, you changed
...
```json
{
    "body": "<a id='comment:14'></a>libgap is a standard package since I don't recall when. We're converting the whole sagelib to use libgap instead of gap.",
...
<a id='comment:14'></a>libgap is a standard package since I don't recall when. We're converting the whole sagelib to use libgap instead of gap.
...
```json
{
    "body": "<a id='comment:15'></a>didn't know that. thanks.\n\nLGTM",
...
<a id='comment:15'></a>didn't know that. thanks.
...
<a id='comment:16'></a>I'm getting timeouts at
...
```json
{
    "body": "<a id='comment:17'></a>these tests need internet connection to cache data via GAP's AtlasRep package.\nOne can tag them `# optional - internet`",
...
<a id='comment:17'></a>these tests need internet connection to cache data via GAP's AtlasRep package.
One can tag them `# optional - internet`
...
```json
{
    "body": "<a id='comment:18'></a>please do!",
...
```json
{
    "body": "<a id='comment:20'></a>rebased over latest beta, added commit \u200b6489a18\t(with `# optional - internet` tags)\n\noff to bots again.",
...
<a id='comment:20'></a>rebased over latest beta, added commit ​6489a18	(with `# optional - internet` tags)
...
```json
{
    "body": "<a id='comment:21'></a>I have the same errors when I try `./sage -tp --long --optional=sage,optional,external src/sage/graphs/` but not when I try `./sage -tp --long src/sage/graphs/`. So added flags are working well.\n\n\nLGTM.",
...
<a id='comment:21'></a>I have the same errors when I try `./sage -tp --long --optional=sage,optional,external src/sage/graphs/` but not when I try `./sage -tp --long src/sage/graphs/`. So added flags are working well.
