# Issue 28662: Missing edges when visualizing Polyhedra with threejs

archive/issues_028425.json:
```json
{
    "assignees": [],
    "body": "When visualizing polyhedra with `threejs`, the edges are missing:\n\n```\nsage: p = polytopes.icosahedron()\nsage: p.plot()\nLaunched html viewer for Graphics3d Object \n```\n\nbut\n\n```\nsage: icosahedron()\n```\n\nseems to have the edges.\n\nCC:  @dimpase @paulmasson @egourgoulhon\n\nKeywords: **threejs, polyhedron, polytope**\n\nBranch/Commit: **[ca23e0f](https://github.com/sagemath/sagetrac-mirror/commit/ca23e0f8fa73968407de31e7c102b34dfbb043e6)**\n\nReviewer: **Eric Gourgoulhon, Jean-Philippe Labb\u00e9**\n\nAuthor: **Paul Masson**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28662_\n\n",
    "closed_at": "2019-10-29T23:41:55Z",
    "created_at": "2019-10-28T08:26:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graphics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Missing edges when visualizing Polyhedra with threejs",
    "type": "issue",
    "updated_at": "2019-10-29T23:41:55Z",
    "url": "https://github.com/sagemath/sage/issues/28662",
    "user": "https://github.com/jplab"
}
```
When visualizing polyhedra with `threejs`, the edges are missing:

```
sage: p = polytopes.icosahedron()
sage: p.plot()
Launched html viewer for Graphics3d Object 
```

but

```
sage: icosahedron()
```

seems to have the edges.

CC:  @dimpase @paulmasson @egourgoulhon

Keywords: **threejs, polyhedron, polytope**

Branch/Commit: **[ca23e0f](https://github.com/sagemath/sagetrac-mirror/commit/ca23e0f8fa73968407de31e7c102b34dfbb043e6)**

Reviewer: **Eric Gourgoulhon, Jean-Philippe Labbé**

Author: **Paul Masson**

_Issue created by migration from https://trac.sagemath.org/ticket/28662_





---

archive/issue_events_375476.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2019-10-28T08:26:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28662",
    "milestone_number": null,
    "milestone_title": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28662#event-375476"
}
```



---

archive/issue_events_375477.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2019-10-28T08:26:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28662",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graphics",
    "label_color": "0000ff",
    "label_name": "component: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28662#event-375477"
}
```



---

archive/issue_events_375478.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2019-10-28T08:26:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28662",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28662#event-375478"
}
```



---

archive/issue_events_375479.json:
```json
{
    "actor": "https://github.com/jplab",
    "created_at": "2019-10-28T08:26:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28662",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28662#event-375479"
}
```



---

archive/issue_comments_450124.json:
```json
{
    "body": "Branch: **[u/paulmasson/28662](https://github.com/sagemath/sagetrac-mirror/tree/u/paulmasson/28662)**",
    "created_at": "2019-10-28T19:34:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28662#issuecomment-450124",
    "user": "https://github.com/paulmasson"
}
```

Branch: **[u/paulmasson/28662](https://github.com/sagemath/sagetrac-mirror/tree/u/paulmasson/28662)**



---

archive/issue_comments_450125.json:
```json
{
    "body": "Commit: **[ca23e0f](https://github.com/sagemath/sagetrac-mirror/commit/ca23e0f8fa73968407de31e7c102b34dfbb043e6)**",
    "created_at": "2019-10-28T19:38:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28662#issuecomment-450125",
    "user": "https://github.com/paulmasson"
}
```

Commit: **[ca23e0f](https://github.com/sagemath/sagetrac-mirror/commit/ca23e0f8fa73968407de31e7c102b34dfbb043e6)**



---

archive/issue_comments_450126.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nDuring the cleanup while adding [mesh plots](https://trac.sagemath.org/ticket/28343) I overlooked this index that needed to be changed. Oops!\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ca23e0f8fa73968407de31e7c102b34dfbb043e6\">ca23e0f</a></td><td><code>Fix addLine()</code></td></tr></table>\n",
    "created_at": "2019-10-28T19:38:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28662#issuecomment-450126",
    "user": "https://github.com/paulmasson"
}
```

<div id="comment:2" align="right">Comment 2</div>

During the cleanup while adding [mesh plots](https://trac.sagemath.org/ticket/28343) I overlooked this index that needed to be changed. Oops!

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ca23e0f8fa73968407de31e7c102b34dfbb043e6">ca23e0f</a></td><td><code>Fix addLine()</code></td></tr></table>




---

archive/issue_events_375480.json:
```json
{
    "actor": "https://github.com/paulmasson",
    "created_at": "2019-10-28T19:38:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28662",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28662#event-375480"
}
```



---

archive/issue_comments_450127.json:
```json
{
    "body": "Author: **Paul Masson**",
    "created_at": "2019-10-28T19:38:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28662#issuecomment-450127",
    "user": "https://github.com/paulmasson"
}
```

Author: **Paul Masson**



---

archive/issue_comments_450128.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nThanks for the fix! LGTM.",
    "created_at": "2019-10-28T21:57:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28662#issuecomment-450128",
    "user": "https://github.com/egourgoulhon"
}
```

<div id="comment:3" align="right">Comment 3</div>

Thanks for the fix! LGTM.



---

archive/issue_comments_450129.json:
```json
{
    "body": "Reviewer: **Eric Gourgoulhon**",
    "created_at": "2019-10-28T21:57:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28662#issuecomment-450129",
    "user": "https://github.com/egourgoulhon"
}
```

Reviewer: **Eric Gourgoulhon**



---

archive/issue_events_375481.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2019-10-28T21:57:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28662",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28662#event-375481"
}
```



---

archive/issue_events_375482.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2019-10-28T21:57:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28662",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28662#event-375482"
}
```



---

archive/issue_comments_450130.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nReplying to [@egourgoulhon](#comment%3A3):\n> Thanks for the fix! LGTM.\n\n+1\n\nThanks for the quick fix!",
    "created_at": "2019-10-29T09:13:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28662#issuecomment-450130",
    "user": "https://github.com/jplab"
}
```

<div id="comment:4" align="right">Comment 4</div>

Replying to [@egourgoulhon](#comment%3A3):
> Thanks for the fix! LGTM.

+1

Thanks for the quick fix!



---

archive/issue_comments_450131.json:
```json
{
    "body": "Changed reviewer from **Eric Gourgoulhon** to **Eric Gourgoulhon, Jean-Philippe Labb\u00e9**",
    "created_at": "2019-10-29T10:05:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28662#issuecomment-450131",
    "user": "https://github.com/egourgoulhon"
}
```

Changed reviewer from **Eric Gourgoulhon** to **Eric Gourgoulhon, Jean-Philippe Labbé**



---

archive/issue_comments_450132.json:
```json
{
    "body": "Changed branch from **[u/paulmasson/28662](https://github.com/sagemath/sagetrac-mirror/tree/u/paulmasson/28662)** to **[ca23e0f](https://github.com/sagemath/sagetrac-mirror/commit/ca23e0f8fa73968407de31e7c102b34dfbb043e6)**",
    "created_at": "2019-10-29T23:41:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28662#issuecomment-450132",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/paulmasson/28662](https://github.com/sagemath/sagetrac-mirror/tree/u/paulmasson/28662)** to **[ca23e0f](https://github.com/sagemath/sagetrac-mirror/commit/ca23e0f8fa73968407de31e7c102b34dfbb043e6)**



---

archive/issue_events_375483.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-10-29T23:41:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28662",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28662#event-375483"
}
```



---

archive/issue_events_375484.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "2fe777d78459d5edb1242b1c65d3a6976852b4ac",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-10-29T23:41:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28662",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28662#event-375484"
}
```
