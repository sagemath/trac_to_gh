# Issue 28542: change behaviour of denominator for p-adics

archive/issues_028305.json:
```json
{
    "assignees": [],
    "body": "Currently, if `x` is a p-adic number indistingushable from 0, the call `x.denominator()` raises an error. IMNO, it is not justified (and not convenient at all).\n\nInstead, I propose to let it return `p^n` where `n` is the smallest integer for which we have the guarantee that `p^n*x` is integral, i.e. is `O(p^N)` with `N` nonnegative.\n\n\nCC:  @roed314\n\nBranch/Commit: **[38bf7df](https://github.com/sagemath/sagetrac-mirror/commit/38bf7df0fe4f03665a12f293301d3ddc21d79d02)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Xavier Caruso**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28542_\n\n",
    "closed_at": "2019-10-06T23:07:14Z",
    "created_at": "2019-09-27T11:54:19Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20padics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "change behaviour of denominator for p-adics",
    "type": "issue",
    "updated_at": "2019-10-06T23:07:14Z",
    "url": "https://github.com/sagemath/sage/issues/28542",
    "user": "https://github.com/xcaruso"
}
```
Currently, if `x` is a p-adic number indistingushable from 0, the call `x.denominator()` raises an error. IMNO, it is not justified (and not convenient at all).

Instead, I propose to let it return `p^n` where `n` is the smallest integer for which we have the guarantee that `p^n*x` is integral, i.e. is `O(p^N)` with `N` nonnegative.


CC:  @roed314

Branch/Commit: **[38bf7df](https://github.com/sagemath/sagetrac-mirror/commit/38bf7df0fe4f03665a12f293301d3ddc21d79d02)**

Reviewer: **Frédéric Chapoton**

Author: **Xavier Caruso**

_Issue created by migration from https://trac.sagemath.org/ticket/28542_





---

archive/issue_events_373554.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2019-09-27T11:54:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28542",
    "milestone_number": null,
    "milestone_title": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28542#event-373554"
}
```



---

archive/issue_events_373555.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2019-09-27T11:54:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28542",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20padics",
    "label_color": "0000ff",
    "label_name": "component: padics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28542#event-373555"
}
```



---

archive/issue_events_373556.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2019-09-27T11:54:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28542",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28542#event-373556"
}
```



---

archive/issue_events_373557.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2019-09-27T11:54:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28542",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28542#event-373557"
}
```



---

archive/issue_comments_447956.json:
```json
{
    "body": "Branch: **[u/caruso/denominator](https://github.com/sagemath/sagetrac-mirror/tree/u/caruso/denominator)**",
    "created_at": "2019-09-27T11:54:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28542#issuecomment-447956",
    "user": "https://github.com/xcaruso"
}
```

Branch: **[u/caruso/denominator](https://github.com/sagemath/sagetrac-mirror/tree/u/caruso/denominator)**



---

archive/issue_comments_447957.json:
```json
{
    "body": "Commit: **[9ae3168](https://github.com/sagemath/sagetrac-mirror/commit/9ae31686f20a3f1c989f1873fc83c61178326104)**",
    "created_at": "2019-09-27T11:56:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28542#issuecomment-447957",
    "user": "https://github.com/xcaruso"
}
```

Commit: **[9ae3168](https://github.com/sagemath/sagetrac-mirror/commit/9ae31686f20a3f1c989f1873fc83c61178326104)**



---

archive/issue_events_373558.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2019-09-27T11:56:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28542",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28542#event-373558"
}
```



---

archive/issue_comments_447958.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nHere is a fix. I've also implemented a method `numerator()`.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9ae31686f20a3f1c989f1873fc83c61178326104\">9ae3168</a></td><td><code>denominator() and numerator() for p-adics</code></td></tr></table>\n",
    "created_at": "2019-09-27T11:56:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28542#issuecomment-447958",
    "user": "https://github.com/xcaruso"
}
```

<a id='comment:2'>Comment 2:</a>
Here is a fix. I've also implemented a method `numerator()`.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9ae31686f20a3f1c989f1873fc83c61178326104">9ae3168</a></td><td><code>denominator() and numerator() for p-adics</code></td></tr></table>




---

archive/issue_comments_447959.json:
```json
{
    "body": "Author: **Xavier Caruso**",
    "created_at": "2019-09-27T11:56:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28542#issuecomment-447959",
    "user": "https://github.com/xcaruso"
}
```

Author: **Xavier Caruso**



---

archive/issue_comments_447960.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\ntypos (twice both):\n\n```\nis `n` is nonnegative\n```\nand\n\n```\nnormalized is such\n```",
    "created_at": "2019-09-28T06:30:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28542#issuecomment-447960",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:3'>Comment 3:</a>
typos (twice both):

```
is `n` is nonnegative
```
and

```
normalized is such
```



---

archive/issue_comments_447961.json:
```json
{
    "body": "Changed commit from **[9ae3168](https://github.com/sagemath/sagetrac-mirror/commit/9ae31686f20a3f1c989f1873fc83c61178326104)** to **[c59a361](https://github.com/sagemath/sagetrac-mirror/commit/c59a361af6102fe87eb062370926f50619aa9755)**",
    "created_at": "2019-09-28T07:15:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28542#issuecomment-447961",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[9ae3168](https://github.com/sagemath/sagetrac-mirror/commit/9ae31686f20a3f1c989f1873fc83c61178326104)** to **[c59a361](https://github.com/sagemath/sagetrac-mirror/commit/c59a361af6102fe87eb062370926f50619aa9755)**



---

archive/issue_comments_447962.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c59a361af6102fe87eb062370926f50619aa9755\">c59a361</a></td><td><code>typo</code></td></tr></table>\n",
    "created_at": "2019-09-28T07:15:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28542#issuecomment-447962",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c59a361af6102fe87eb062370926f50619aa9755">c59a361</a></td><td><code>typo</code></td></tr></table>




---

archive/issue_comments_447963.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/caa6fe2a8503c4ed2f7d901fed8d76f19c5a91db\">caa6fe2</a></td><td><code>fix typo (bis)</code></td></tr></table>\n",
    "created_at": "2019-09-28T07:16:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28542#issuecomment-447963",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:5'>Comment 5:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/caa6fe2a8503c4ed2f7d901fed8d76f19c5a91db">caa6fe2</a></td><td><code>fix typo (bis)</code></td></tr></table>




---

archive/issue_comments_447964.json:
```json
{
    "body": "Changed commit from **[c59a361](https://github.com/sagemath/sagetrac-mirror/commit/c59a361af6102fe87eb062370926f50619aa9755)** to **[caa6fe2](https://github.com/sagemath/sagetrac-mirror/commit/caa6fe2a8503c4ed2f7d901fed8d76f19c5a91db)**",
    "created_at": "2019-09-28T07:16:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28542#issuecomment-447964",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[c59a361](https://github.com/sagemath/sagetrac-mirror/commit/c59a361af6102fe87eb062370926f50619aa9755)** to **[caa6fe2](https://github.com/sagemath/sagetrac-mirror/commit/caa6fe2a8503c4ed2f7d901fed8d76f19c5a91db)**



---

archive/issue_comments_447965.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nmissing :: after  otherwise.",
    "created_at": "2019-09-28T19:24:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28542#issuecomment-447965",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:6'>Comment 6:</a>
missing :: after  otherwise.



---

archive/issue_comments_447966.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/38bf7df0fe4f03665a12f293301d3ddc21d79d02\">38bf7df</a></td><td><code>add missing ::</code></td></tr></table>\n",
    "created_at": "2019-09-29T04:44:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28542#issuecomment-447966",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:7'>Comment 7:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/38bf7df0fe4f03665a12f293301d3ddc21d79d02">38bf7df</a></td><td><code>add missing ::</code></td></tr></table>




---

archive/issue_comments_447967.json:
```json
{
    "body": "Changed commit from **[caa6fe2](https://github.com/sagemath/sagetrac-mirror/commit/caa6fe2a8503c4ed2f7d901fed8d76f19c5a91db)** to **[38bf7df](https://github.com/sagemath/sagetrac-mirror/commit/38bf7df0fe4f03665a12f293301d3ddc21d79d02)**",
    "created_at": "2019-09-29T04:44:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28542#issuecomment-447967",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[caa6fe2](https://github.com/sagemath/sagetrac-mirror/commit/caa6fe2a8503c4ed2f7d901fed8d76f19c5a91db)** to **[38bf7df](https://github.com/sagemath/sagetrac-mirror/commit/38bf7df0fe4f03665a12f293301d3ddc21d79d02)**



---

archive/issue_comments_447968.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2019-09-29T08:57:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28542#issuecomment-447968",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_events_373559.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-09-29T08:57:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28542",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28542#event-373559"
}
```



---

archive/issue_events_373560.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-09-29T08:57:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28542",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28542#event-373560"
}
```



---

archive/issue_comments_447969.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nok",
    "created_at": "2019-09-29T08:57:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28542#issuecomment-447969",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:8'>Comment 8:</a>
ok



---

archive/issue_events_373561.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-10-06T23:07:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28542",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28542#event-373561"
}
```



---

archive/issue_events_373562.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "6c55e5486378c1d17ce1ee20a9716aff75a62001",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-10-06T23:07:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28542",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28542#event-373562"
}
```



---

archive/issue_comments_447970.json:
```json
{
    "body": "Changed branch from **[u/caruso/denominator](https://github.com/sagemath/sagetrac-mirror/tree/u/caruso/denominator)** to **[38bf7df](https://github.com/sagemath/sagetrac-mirror/commit/38bf7df0fe4f03665a12f293301d3ddc21d79d02)**",
    "created_at": "2019-10-06T23:07:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28542",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28542#issuecomment-447970",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/caruso/denominator](https://github.com/sagemath/sagetrac-mirror/tree/u/caruso/denominator)** to **[38bf7df](https://github.com/sagemath/sagetrac-mirror/commit/38bf7df0fe4f03665a12f293301d3ddc21d79d02)**
