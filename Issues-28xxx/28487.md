...
```json
{
    "body": "Couldn't we allow the construction `K.hom([x,y])` as a shortcut of `K.hom([x], base_map=K.base_ring().hom([y]))` for a two step extension?",
...
```json
{
    "body": "Sorry, I pushed on the wrong ticket.",
