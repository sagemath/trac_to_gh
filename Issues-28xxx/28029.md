# Issue 28029: Increase number of tries in unpack_archive

archive/issues_027792.json:
```json
{
    "assignees": [],
    "body": "In Cygwin under Windows, unpacking archives sometimes leads\nto a permission problem.\n\nThis is due to Windows locking files when they are opened\nby another application, and might mean an antivirus inspecting\na file prevents you from writing to it.\n\nBuilding Sage in Cygwin in Windows 7 on some computer reliably\nfailed before this ticket, and reliably works with it.\n\nCC:  @embray @slel\n\nKeywords: **cygwin, days101**\n\nBranch/Commit: **[53becef](https://github.com/sagemath/sagetrac-mirror/commit/53becef88b6ad3d1471aa0e66f0f9b4927b42e2e)**\n\nReviewer: **Samuel Leli\u00e8vre**\n\nAuthor: **Erik Bray**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28029_\n\n",
    "closed_at": "2019-06-28T04:29:35Z",
    "created_at": "2019-06-20T15:57:49Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Increase number of tries in unpack_archive",
    "type": "issue",
    "updated_at": "2019-06-28T04:29:35Z",
    "url": "https://github.com/sagemath/sage/issues/28029",
    "user": "https://github.com/slel"
}
```
In Cygwin under Windows, unpacking archives sometimes leads
to a permission problem.

This is due to Windows locking files when they are opened
by another application, and might mean an antivirus inspecting
a file prevents you from writing to it.

Building Sage in Cygwin in Windows 7 on some computer reliably
failed before this ticket, and reliably works with it.

CC:  @embray @slel

Keywords: **cygwin, days101**

Branch/Commit: **[53becef](https://github.com/sagemath/sagetrac-mirror/commit/53becef88b6ad3d1471aa0e66f0f9b4927b42e2e)**

Reviewer: **Samuel Lelièvre**

Author: **Erik Bray**

_Issue created by migration from https://trac.sagemath.org/ticket/28029_





---

archive/issue_events_366168.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-06-20T15:57:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28029",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28029#event-366168"
}
```



---

archive/issue_events_366169.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-06-20T15:57:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28029",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
    "label_color": "000080",
    "label_name": "component: porting: cygwin",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28029#event-366169"
}
```



---

archive/issue_events_366170.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-06-20T15:57:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28029",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28029#event-366170"
}
```



---

archive/issue_events_366171.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-06-20T15:57:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28029",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28029#event-366171"
}
```



---

archive/issue_comments_439034.json:
```json
{
    "body": "Changed keywords from **cygwin** to **cygwin, days101**",
    "created_at": "2019-06-20T15:58:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28029#issuecomment-439034",
    "user": "https://github.com/slel"
}
```

Changed keywords from **cygwin** to **cygwin, days101**



---

archive/issue_comments_439035.json:
```json
{
    "body": "Branch: **[u/slelievre/cygwin-retry-harder](https://github.com/sagemath/sagetrac-mirror/tree/u/slelievre/cygwin-retry-harder)**",
    "created_at": "2019-06-20T15:58:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28029#issuecomment-439035",
    "user": "https://github.com/slel"
}
```

Branch: **[u/slelievre/cygwin-retry-harder](https://github.com/sagemath/sagetrac-mirror/tree/u/slelievre/cygwin-retry-harder)**



---

archive/issue_comments_439036.json:
```json
{
    "body": "Changed branch from **[u/slelievre/cygwin-retry-harder](https://github.com/sagemath/sagetrac-mirror/tree/u/slelievre/cygwin-retry-harder)** to none",
    "created_at": "2019-06-20T15:59:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28029#issuecomment-439036",
    "user": "https://github.com/slel"
}
```

Changed branch from **[u/slelievre/cygwin-retry-harder](https://github.com/sagemath/sagetrac-mirror/tree/u/slelievre/cygwin-retry-harder)** to none



---

archive/issue_events_366172.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-06-20T15:59:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28029",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28029#event-366172"
}
```



---

archive/issue_comments_439037.json:
```json
{
    "body": "Commit: **[53becef](https://github.com/sagemath/sagetrac-mirror/commit/53becef88b6ad3d1471aa0e66f0f9b4927b42e2e)**",
    "created_at": "2019-06-20T16:00:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28029#issuecomment-439037",
    "user": "https://github.com/slel"
}
```

Commit: **[53becef](https://github.com/sagemath/sagetrac-mirror/commit/53becef88b6ad3d1471aa0e66f0f9b4927b42e2e)**



---

archive/issue_events_366173.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-06-20T16:00:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28029",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28029#event-366173"
}
```



---

archive/issue_events_366174.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-06-20T16:00:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28029",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28029#event-366174"
}
```



---

archive/issue_comments_439038.json:
```json
{
    "body": "Branch: **[u/slelievre/cygwin-retry-harder](https://github.com/sagemath/sagetrac-mirror/tree/u/slelievre/cygwin-retry-harder)**",
    "created_at": "2019-06-20T16:00:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28029#issuecomment-439038",
    "user": "https://github.com/slel"
}
```

Branch: **[u/slelievre/cygwin-retry-harder](https://github.com/sagemath/sagetrac-mirror/tree/u/slelievre/cygwin-retry-harder)**



---

archive/issue_comments_439039.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/53becef88b6ad3d1471aa0e66f0f9b4927b42e2e\">53becef</a></td><td><code>cygwin: retry harder</code></td></tr></table>\n",
    "created_at": "2019-06-20T16:00:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28029#issuecomment-439039",
    "user": "https://github.com/slel"
}
```

<a id='comment:4'>Comment 4:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/53becef88b6ad3d1471aa0e66f0f9b4927b42e2e">53becef</a></td><td><code>cygwin: retry harder</code></td></tr></table>




---

archive/issue_comments_439040.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nFWIW the `retry=len(archive.names)` is pretty arbitrary here.  It's just setting the number of retries needed to a factor of the number of files extracted (that might be getting checked one by one by a virus scanner or whatever).  In practice it should need way fewer retries than this since the default retry is every 1s.",
    "created_at": "2019-06-20T17:45:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28029#issuecomment-439040",
    "user": "https://github.com/embray"
}
```

<a id='comment:5'>Comment 5:</a>
FWIW the `retry=len(archive.names)` is pretty arbitrary here.  It's just setting the number of retries needed to a factor of the number of files extracted (that might be getting checked one by one by a virus scanner or whatever).  In practice it should need way fewer retries than this since the default retry is every 1s.



---

archive/issue_comments_439041.json:
```json
{
    "body": "Changed branch from **[u/slelievre/cygwin-retry-harder](https://github.com/sagemath/sagetrac-mirror/tree/u/slelievre/cygwin-retry-harder)** to **[53becef](https://github.com/sagemath/sagetrac-mirror/commit/53becef88b6ad3d1471aa0e66f0f9b4927b42e2e)**",
    "created_at": "2019-06-28T04:29:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28029#issuecomment-439041",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/slelievre/cygwin-retry-harder](https://github.com/sagemath/sagetrac-mirror/tree/u/slelievre/cygwin-retry-harder)** to **[53becef](https://github.com/sagemath/sagetrac-mirror/commit/53becef88b6ad3d1471aa0e66f0f9b4927b42e2e)**



---

archive/issue_events_366175.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-06-28T04:29:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28029",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28029#event-366175"
}
```



---

archive/issue_events_366176.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "75adc6a1236a21fd7e7278847cc0cc99f5a7319e",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-06-28T04:29:35Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28029",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28029#event-366176"
}
```
