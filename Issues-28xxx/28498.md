# Issue 28498: Memory leak in BasisExchangeMatroid.components

archive/issues_028261.json:
```json
{
    "body": "As reported in [this ask question](https://ask.sagemath.org/question/47839), there is a memory leak in `BasisExchangeMatroid.components`, see:\n\n```\nsage: G = [g for g in graphs.nauty_geng(\"12 16:16 -Ct\")]\nsage: for _ in range(20):\n....:     for g in G:\n....:         A = g.incidence_matrix()\n....:         M = Matroid(A, ring = GF(2))\n....:         if M.is_connected():\n....:             pass\n....:     print(get_memory_usage())\n....:     \n8539.23046875\n8543.7421875\n8547.73828125\n8551.734375\n8555.73046875\n8559.59765625\n8563.59375\n8566.9453125\n8570.94140625\n8574.9375\n8578.2890625\n8582.28515625\n8586.15234375\n8589.6328125\n8593.62890625\n8597.49609375\n8600.9765625\n8604.84375\n8608.83984375\n8612.3203125\n```\n\nThe memory leak is because `sig_free` is missing.\n\n\nCC:  rudi @jdemeyer\n\nKeywords: memleak\n\nBranch/Commit: e9ed458aa343140e07f03dbd760d61e5bfe73b28\n\nReviewer: Vincent Delecroix\n\nAuthor: Thierry Monteil\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/28498\n\n",
    "closed_at": "2019-10-05T07:57:54Z",
    "created_at": "2019-09-15T10:43:14Z",
    "labels": [
        "component: memleak",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.0",
    "title": "Memory leak in BasisExchangeMatroid.components",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/28498",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```
As reported in [this ask question](https://ask.sagemath.org/question/47839), there is a memory leak in `BasisExchangeMatroid.components`, see:

```
sage: G = [g for g in graphs.nauty_geng("12 16:16 -Ct")]
sage: for _ in range(20):
....:     for g in G:
....:         A = g.incidence_matrix()
....:         M = Matroid(A, ring = GF(2))
....:         if M.is_connected():
....:             pass
....:     print(get_memory_usage())
....:     
8539.23046875
8543.7421875
8547.73828125
8551.734375
8555.73046875
8559.59765625
8563.59375
8566.9453125
8570.94140625
8574.9375
8578.2890625
8582.28515625
8586.15234375
8589.6328125
8593.62890625
8597.49609375
8600.9765625
8604.84375
8608.83984375
8612.3203125
```

The memory leak is because `sig_free` is missing.


CC:  rudi @jdemeyer

Keywords: memleak

Branch/Commit: e9ed458aa343140e07f03dbd760d61e5bfe73b28

Reviewer: Vincent Delecroix

Author: Thierry Monteil

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/28498





---

archive/issue_comments_552893.json:
```json
{
    "body": "Changing branch from \"\" to \"u/tmonteil/memory_leak_in_basisexchangematroid_components\"",
    "created_at": "2019-09-15T11:03:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28498#issuecomment-552893",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```

Changing branch from "" to "u/tmonteil/memory_leak_in_basisexchangematroid_components"



---

archive/issue_comments_552894.json:
```json
{
    "body": "<a id='comment:2'></a>\nAfter the fix:\n\n```\nsage: G = [g for g in graphs.nauty_geng(\"12 16:16 -Ct\")]\nsage: for _ in range(20):\n....:     for g in G:\n....:         A = g.incidence_matrix()\n....:         M = Matroid(A, ring = GF(2))\n....:         if M.is_connected():\n....:             pass\n....:     print(get_memory_usage())\n....:     \n8462.484375\n8462.484375\n8462.484375\n8462.484375\n8462.484375\n8462.484375\n8462.484375\n8462.484375\n8462.484375\n8462.484375\n8462.484375\n8462.484375\n8462.484375\n8462.484375\n8462.484375\n8462.484375\n8462.484375\n8462.484375\n8462.484375\n8462.484375\n```\n\nI am not sure whether this should be doctested.\n\n---\nNew commits:\n|                                                                                                                                          |                                 |\n|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------|\n|[db6182b](https://git.sagemath.org/sage.git/commit?id=db6182b5cbe5fe899762b98aaff96b6a90fc8527)|`#28498 : remove trailing spaces`|\n|[e9ed458](https://git.sagemath.org/sage.git/commit?id=e9ed458aa343140e07f03dbd760d61e5bfe73b28)|`#28498 : fix memleak`|",
    "created_at": "2019-09-15T11:06:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28498#issuecomment-552894",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```

<a id='comment:2'></a>
After the fix:

```
sage: G = [g for g in graphs.nauty_geng("12 16:16 -Ct")]
sage: for _ in range(20):
....:     for g in G:
....:         A = g.incidence_matrix()
....:         M = Matroid(A, ring = GF(2))
....:         if M.is_connected():
....:             pass
....:     print(get_memory_usage())
....:     
8462.484375
8462.484375
8462.484375
8462.484375
8462.484375
8462.484375
8462.484375
8462.484375
8462.484375
8462.484375
8462.484375
8462.484375
8462.484375
8462.484375
8462.484375
8462.484375
8462.484375
8462.484375
8462.484375
8462.484375
```

I am not sure whether this should be doctested.

---
New commits:
|                                                                                                                                          |                                 |
|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------|
|[db6182b](https://git.sagemath.org/sage.git/commit?id=db6182b5cbe5fe899762b98aaff96b6a90fc8527)|`#28498 : remove trailing spaces`|
|[e9ed458](https://git.sagemath.org/sage.git/commit?id=e9ed458aa343140e07f03dbd760d61e5bfe73b28)|`#28498 : fix memleak`|



---

archive/issue_comments_552895.json:
```json
{
    "body": "Changing commit from \"\" to \"e9ed458aa343140e07f03dbd760d61e5bfe73b28\"",
    "created_at": "2019-09-15T11:06:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28498#issuecomment-552895",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```

Changing commit from "" to "e9ed458aa343140e07f03dbd760d61e5bfe73b28"



---

archive/issue_comments_552896.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2019-09-15T12:34:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28498#issuecomment-552896",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_552897.json:
```json
{
    "body": "<a id='comment:4'></a>\nNice catch! Such memory leaks should be doctested for each code snippet (ie running the same code 100 times should not increase memory usage). I am not sure it makes sense at the level of this single function.",
    "created_at": "2019-09-15T14:17:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28498#issuecomment-552897",
    "user": "https://github.com/videlec"
}
```

<a id='comment:4'></a>
Nice catch! Such memory leaks should be doctested for each code snippet (ie running the same code 100 times should not increase memory usage). I am not sure it makes sense at the level of this single function.



---

archive/issue_comments_552898.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Vincent Delecroix\"",
    "created_at": "2019-09-15T14:17:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28498#issuecomment-552898",
    "user": "https://github.com/videlec"
}
```

Changing reviewer from "" to "Vincent Delecroix"



---

archive/issue_comments_552899.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2019-09-15T14:19:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28498#issuecomment-552899",
    "user": "https://github.com/videlec"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_552900.json:
```json
{
    "body": "<a id='comment:6'></a>\nSorry for making this mistake in the first place, and thanks for fixing!",
    "created_at": "2019-09-15T20:49:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28498#issuecomment-552900",
    "user": "https://trac.sagemath.org/admin/accounts/users/Rudi"
}
```

<a id='comment:6'></a>
Sorry for making this mistake in the first place, and thanks for fixing!



---

archive/issue_comments_552901.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -32,5 +32,5 @@\n 8612.3203125\n ```\n \n-Siegfried is missing.\n+The memory leak is because `sig_free` is missing.\n \n``````\n",
    "created_at": "2019-09-16T03:15:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28498#issuecomment-552901",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -32,5 +32,5 @@
 8612.3203125
 ```
 
-Siegfried is missing.
+The memory leak is because `sig_free` is missing.
 
``````




---

archive/issue_comments_552902.json:
```json
{
    "body": "Changing keywords from \"\" to \"memleak\".",
    "created_at": "2019-09-16T03:15:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28498#issuecomment-552902",
    "user": "https://github.com/slel"
}
```

Changing keywords from "" to "memleak".



---

archive/issue_events_071464.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-09-30T08:12:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28498",
    "milestone": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28498#event-71464"
}
```



---

archive/issue_comments_552903.json:
```json
{
    "body": "<a id='comment:8'></a>\nmoving milestone to 9.0 (after release of 8.9)",
    "created_at": "2019-09-30T08:12:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28498#issuecomment-552903",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:8'></a>
moving milestone to 9.0 (after release of 8.9)



---

archive/issue_comments_552904.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2019-10-05T07:57:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28498#issuecomment-552904",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_comments_552905.json:
```json
{
    "body": "Changing branch from \"u/tmonteil/memory_leak_in_basisexchangematroid_components\" to \"e9ed458aa343140e07f03dbd760d61e5bfe73b28\"",
    "created_at": "2019-10-05T07:57:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28498#issuecomment-552905",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/tmonteil/memory_leak_in_basisexchangematroid_components" to "e9ed458aa343140e07f03dbd760d61e5bfe73b28"



---

archive/issue_events_071465.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-10-05T07:57:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/28498",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28498#event-71465"
}
```
