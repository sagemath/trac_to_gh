# Issue 28791: Implement Feature without using sage.misc.cachefunc, sage.structure.unique_representation

archive/issues_028554.json:
```json
{
    "assignees": [],
    "body": "This allows `Feature`s to be used for build management - for #25828.\n\n\nCC:  @timokau @tscrim @kiwifb @jdemeyer @dimpase\n\nBranch/Commit: **[24921a0](https://github.com/sagemath/sagetrac-mirror/commit/24921a0d0f40730dbb68edee1fd5550cedd0745e)**\n\nReviewer: **Fran\u00e7ois Bissey**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28791_\n\n",
    "closed_at": "2019-11-30T13:36:08Z",
    "created_at": "2019-11-23T16:59:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Implement Feature without using sage.misc.cachefunc, sage.structure.unique_representation",
    "type": "issue",
    "updated_at": "2019-11-30T13:36:08Z",
    "url": "https://github.com/sagemath/sage/issues/28791",
    "user": "https://github.com/mkoeppe"
}
```
This allows `Feature`s to be used for build management - for #25828.


CC:  @timokau @tscrim @kiwifb @jdemeyer @dimpase

Branch/Commit: **[24921a0](https://github.com/sagemath/sagetrac-mirror/commit/24921a0d0f40730dbb68edee1fd5550cedd0745e)**

Reviewer: **François Bissey**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/28791_





---

archive/issue_events_377279.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-11-23T16:59:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28791",
    "milestone_number": null,
    "milestone_title": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28791#event-377279"
}
```



---

archive/issue_events_377280.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-11-23T16:59:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28791",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28791#event-377280"
}
```



---

archive/issue_events_377281.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-11-23T16:59:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28791",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28791#event-377281"
}
```



---

archive/issue_events_377282.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-11-23T16:59:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28791",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28791#event-377282"
}
```



---

archive/issue_comments_452125.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2019-11-23T19:03:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28791",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28791#issuecomment-452125",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_comments_452126.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nI can't seem to push to trac at the moment, so I'm attaching the patch.",
    "created_at": "2019-11-23T19:37:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28791",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28791#issuecomment-452126",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

I can't seem to push to trac at the moment, so I'm attaching the patch.



---

archive/issue_events_377283.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-11-23T19:37:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28791",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28791#event-377283"
}
```



---

archive/issue_comments_452127.json:
```json
{
    "body": "Branch: **[u/mkoeppe/feature_without_using_sage_modules](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/feature_without_using_sage_modules)**",
    "created_at": "2019-11-24T14:59:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28791",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28791#issuecomment-452127",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/feature_without_using_sage_modules](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/feature_without_using_sage_modules)**



---

archive/issue_comments_452128.json:
```json
{
    "body": "Commit: **[24921a0](https://github.com/sagemath/sagetrac-mirror/commit/24921a0d0f40730dbb68edee1fd5550cedd0745e)**",
    "created_at": "2019-11-24T15:01:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28791",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28791#issuecomment-452128",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[24921a0](https://github.com/sagemath/sagetrac-mirror/commit/24921a0d0f40730dbb68edee1fd5550cedd0745e)**



---

archive/issue_comments_452129.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/24921a0d0f40730dbb68edee1fd5550cedd0745e\">24921a0</a></td><td><code>Implement Feature without using sage.misc.cachefunc, sage.structure.unique_representation</code></td></tr></table>\n",
    "created_at": "2019-11-24T15:01:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28791",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28791#issuecomment-452129",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:4" align="right">Comment 4</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/24921a0d0f40730dbb68edee1fd5550cedd0745e">24921a0</a></td><td><code>Implement Feature without using sage.misc.cachefunc, sage.structure.unique_representation</code></td></tr></table>




---

archive/issue_comments_452130.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,2 +1,2 @@\n-This is for #25828.\n+This allows `Feature`s to be used for build management - for #25828.\n \n``````\n",
    "created_at": "2019-11-24T15:01:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28791",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28791#issuecomment-452130",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,2 +1,2 @@
-This is for #25828.
+This allows `Feature`s to be used for build management - for #25828.
 
``````




---

archive/issue_comments_452131.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nI am not opposed to this. Is there any arguments against this change? Is caching that important here?",
    "created_at": "2019-11-25T21:19:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28791",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28791#issuecomment-452131",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:5" align="right">Comment 5</div>

I am not opposed to this. Is there any arguments against this change? Is caching that important here?



---

archive/issue_comments_452132.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nReplying to [@kiwifb](#comment%3A5):\n>  Is caching that important here?\n\nNote that it's still caching (which I think is important), it just does not use `sage.misc.cachefunc`.",
    "created_at": "2019-11-26T03:01:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28791",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28791#issuecomment-452132",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:6" align="right">Comment 6</div>

Replying to [@kiwifb](#comment%3A5):
>  Is caching that important here?

Note that it's still caching (which I think is important), it just does not use `sage.misc.cachefunc`.



---

archive/issue_events_377284.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2019-11-26T03:07:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28791",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28791#event-377284"
}
```



---

archive/issue_events_377285.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2019-11-26T03:07:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28791",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28791#event-377285"
}
```



---

archive/issue_comments_452133.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nI asked about caching because it was mentioned in #25828. If this does caching too I don't see any other obstacles. We may as well move on with it.",
    "created_at": "2019-11-26T03:07:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28791",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28791#issuecomment-452133",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:7" align="right">Comment 7</div>

I asked about caching because it was mentioned in #25828. If this does caching too I don't see any other obstacles. We may as well move on with it.



---

archive/issue_comments_452134.json:
```json
{
    "body": "Reviewer: **Fran\u00e7ois Bissey**",
    "created_at": "2019-11-26T03:07:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28791",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28791#issuecomment-452134",
    "user": "https://github.com/kiwifb"
}
```

Reviewer: **François Bissey**



---

archive/issue_comments_452135.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nThank you!",
    "created_at": "2019-11-26T03:27:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28791",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28791#issuecomment-452135",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:8" align="right">Comment 8</div>

Thank you!



---

archive/issue_comments_452136.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/feature_without_using_sage_modules](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/feature_without_using_sage_modules)** to **[24921a0](https://github.com/sagemath/sagetrac-mirror/commit/24921a0d0f40730dbb68edee1fd5550cedd0745e)**",
    "created_at": "2019-11-30T13:36:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28791",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28791#issuecomment-452136",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/feature_without_using_sage_modules](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/feature_without_using_sage_modules)** to **[24921a0](https://github.com/sagemath/sagetrac-mirror/commit/24921a0d0f40730dbb68edee1fd5550cedd0745e)**



---

archive/issue_events_377286.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-11-30T13:36:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28791",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28791#event-377286"
}
```



---

archive/issue_events_377287.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "f7906eb4eae8a2438e8a49377e8d27b3a7c0cabd",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-11-30T13:36:08Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28791",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28791#event-377287"
}
```
