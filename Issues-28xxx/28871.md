# Issue 28871: Fix warnings when compiling the meataxe wrappers

archive/issues_028634.json:
```json
{
    "body": "The following warnings pop up when doing \"sage -br\" after \"make meataxe\":\n\n```\nbuild/cythonized/sage/matrix/matrix_gfpn_dense.c: In function \u2018__pyx_pf_4sage_6matrix_17matrix_gfpn_dense_17Matrix_gfpn_dense_8__reduce__\u2019:\nbuild/cythonized/sage/matrix/matrix_gfpn_dense.c:6086:35: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]\n     for (__pyx_t_6 = 0; __pyx_t_6 < __pyx_t_5; __pyx_t_6+=1) {\n                                   ^\nbuild/cythonized/sage/matrix/matrix_gfpn_dense.c: In function \u2018__pyx_pf_4sage_6matrix_17matrix_gfpn_dense_17Matrix_gfpn_dense_12randomize\u2019:\nbuild/cythonized/sage/matrix/matrix_gfpn_dense.c:7491:43: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]\n           for (__pyx_t_12 = 0; __pyx_t_12 < __pyx_t_11; __pyx_t_12+=1) {\n                                           ^\nbuild/cythonized/sage/matrix/matrix_gfpn_dense.c:7595:43: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]\n           for (__pyx_t_12 = 0; __pyx_t_12 < __pyx_t_11; __pyx_t_12+=1) {\n                                           ^\nbuild/cythonized/sage/matrix/matrix_gfpn_dense.c: In function \u2018__pyx_pf_4sage_6matrix_17matrix_gfpn_dense_17Matrix_gfpn_dense_48_echelon_in_place_classical\u2019:\nbuild/cythonized/sage/matrix/matrix_gfpn_dense.c:15134:39: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]\n       for (__pyx_t_14 = 0; __pyx_t_14 < __pyx_t_4; __pyx_t_14+=1) {\n                                       ^\nbuild/cythonized/sage/matrix/matrix_gfpn_dense.c:15176:39: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]\n       for (__pyx_t_14 = 0; __pyx_t_14 < __pyx_t_4; __pyx_t_14+=1) {\n                                       ^\nbuild/cythonized/sage/matrix/matrix_gfpn_dense.c:15383:37: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]\n     for (__pyx_t_14 = 0; __pyx_t_14 < __pyx_t_4; __pyx_t_14+=1) {\n                                     ^\nbuild/cythonized/sage/matrix/matrix_gfpn_dense.c: In function \u2018__pyx_pf_4sage_6matrix_17matrix_gfpn_dense_mtx_unpickle\u2019:\nbuild/cythonized/sage/matrix/matrix_gfpn_dense.c:16411:43: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]\n     __pyx_t_4 = ((__pyx_v_pickled_rowsize == FfCurrentRowSizeIo) != 0);\n                                           ^\nbuild/cythonized/sage/matrix/matrix_gfpn_dense.c:16433:39: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]\n       for (__pyx_t_24 = 0; __pyx_t_24 < __pyx_t_9; __pyx_t_24+=1) {\n                                       ^\nbuild/cythonized/sage/matrix/matrix_gfpn_dense.c:16490:43: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]\n     __pyx_t_4 = ((__pyx_v_pickled_rowsize >= FfCurrentRowSizeIo) != 0);\n                                           ^\nbuild/cythonized/sage/matrix/matrix_gfpn_dense.c:16556:41: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]\n         for (__pyx_t_24 = 0; __pyx_t_24 < __pyx_t_9; __pyx_t_24+=1) {\n                                         ^\n```\nI guess these warnings should be addressed.\n\nKeywords: warning meataxe\n\nStatus: new\n\nIssue created by migration from https://trac.sagemath.org/ticket/28871\n\n",
    "created_at": "2019-12-11T14:41:03Z",
    "labels": [
        "component: packages: optional",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.8",
    "title": "Fix warnings when compiling the meataxe wrappers",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/28871",
    "user": "https://github.com/simon-king-jena"
}
```
The following warnings pop up when doing "sage -br" after "make meataxe":

```
build/cythonized/sage/matrix/matrix_gfpn_dense.c: In function ‘__pyx_pf_4sage_6matrix_17matrix_gfpn_dense_17Matrix_gfpn_dense_8__reduce__’:
build/cythonized/sage/matrix/matrix_gfpn_dense.c:6086:35: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]
     for (__pyx_t_6 = 0; __pyx_t_6 < __pyx_t_5; __pyx_t_6+=1) {
                                   ^
build/cythonized/sage/matrix/matrix_gfpn_dense.c: In function ‘__pyx_pf_4sage_6matrix_17matrix_gfpn_dense_17Matrix_gfpn_dense_12randomize’:
build/cythonized/sage/matrix/matrix_gfpn_dense.c:7491:43: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]
           for (__pyx_t_12 = 0; __pyx_t_12 < __pyx_t_11; __pyx_t_12+=1) {
                                           ^
build/cythonized/sage/matrix/matrix_gfpn_dense.c:7595:43: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]
           for (__pyx_t_12 = 0; __pyx_t_12 < __pyx_t_11; __pyx_t_12+=1) {
                                           ^
build/cythonized/sage/matrix/matrix_gfpn_dense.c: In function ‘__pyx_pf_4sage_6matrix_17matrix_gfpn_dense_17Matrix_gfpn_dense_48_echelon_in_place_classical’:
build/cythonized/sage/matrix/matrix_gfpn_dense.c:15134:39: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]
       for (__pyx_t_14 = 0; __pyx_t_14 < __pyx_t_4; __pyx_t_14+=1) {
                                       ^
build/cythonized/sage/matrix/matrix_gfpn_dense.c:15176:39: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]
       for (__pyx_t_14 = 0; __pyx_t_14 < __pyx_t_4; __pyx_t_14+=1) {
                                       ^
build/cythonized/sage/matrix/matrix_gfpn_dense.c:15383:37: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]
     for (__pyx_t_14 = 0; __pyx_t_14 < __pyx_t_4; __pyx_t_14+=1) {
                                     ^
build/cythonized/sage/matrix/matrix_gfpn_dense.c: In function ‘__pyx_pf_4sage_6matrix_17matrix_gfpn_dense_mtx_unpickle’:
build/cythonized/sage/matrix/matrix_gfpn_dense.c:16411:43: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]
     __pyx_t_4 = ((__pyx_v_pickled_rowsize == FfCurrentRowSizeIo) != 0);
                                           ^
build/cythonized/sage/matrix/matrix_gfpn_dense.c:16433:39: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]
       for (__pyx_t_24 = 0; __pyx_t_24 < __pyx_t_9; __pyx_t_24+=1) {
                                       ^
build/cythonized/sage/matrix/matrix_gfpn_dense.c:16490:43: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]
     __pyx_t_4 = ((__pyx_v_pickled_rowsize >= FfCurrentRowSizeIo) != 0);
                                           ^
build/cythonized/sage/matrix/matrix_gfpn_dense.c:16556:41: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]
         for (__pyx_t_24 = 0; __pyx_t_24 < __pyx_t_9; __pyx_t_24+=1) {
                                         ^
```
I guess these warnings should be addressed.

Keywords: warning meataxe

Status: new

Issue created by migration from https://trac.sagemath.org/ticket/28871





---

archive/issue_comments_560652.json:
```json
{
    "body": "<a id='comment:2'></a>I'm puzzled. In some instances of the compiler warnings, an int is compared with a size_t. So, I changed declarations so that only int with int or size_t with size_t are compared.\n\nHowever, it didn't fix the problem. Also, in some of the warnings, the comparison was between size_t and size_t.\n\nSo, can please someone explain to me what causes the compiler warning?",
    "created_at": "2019-12-12T15:42:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28871",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28871#issuecomment-560652",
    "user": "https://github.com/simon-king-jena"
}
```

<a id='comment:2'></a>I'm puzzled. In some instances of the compiler warnings, an int is compared with a size_t. So, I changed declarations so that only int with int or size_t with size_t are compared.

However, it didn't fix the problem. Also, in some of the warnings, the comparison was between size_t and size_t.

So, can please someone explain to me what causes the compiler warning?



---

archive/issue_comments_560653.json:
```json
{
    "body": "<a id='comment:3'></a>Ticket retargeted after milestone closed",
    "created_at": "2020-01-06T14:10:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28871",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28871#issuecomment-560653",
    "user": "https://github.com/embray"
}
```

<a id='comment:3'></a>Ticket retargeted after milestone closed



---

archive/issue_events_073026.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2020-01-06T14:10:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28871",
    "milestone": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28871#event-73026"
}
```



---

archive/issue_events_073027.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28871",
    "milestone": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28871#event-73027"
}
```



---

archive/issue_events_073028.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28871",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28871#event-73028"
}
```



---

archive/issue_comments_560654.json:
```json
{
    "body": "<a id='comment:4'></a>Batch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.",
    "created_at": "2020-04-14T19:41:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28871",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28871#issuecomment-560654",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:4'></a>Batch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.



---

archive/issue_events_073029.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T16:37:33Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28871",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28871#event-73029"
}
```



---

archive/issue_events_073030.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T16:37:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28871",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28871#event-73030"
}
```



---

archive/issue_comments_560655.json:
```json
{
    "body": "<a id='comment:6'></a>Setting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28871",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28871#issuecomment-560655",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:6'></a>Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_073031.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28871",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28871#event-73031"
}
```



---

archive/issue_events_073032.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28871",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28871#event-73032"
}
```



---

archive/issue_events_073033.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28871",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28871#event-73033"
}
```



---

archive/issue_events_073034.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28871",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28871#event-73034"
}
```



---

archive/issue_events_073035.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28871",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28871#event-73035"
}
```



---

archive/issue_events_073036.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28871",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28871#event-73036"
}
```



---

archive/issue_events_073037.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-01T21:16:35Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28871",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28871#event-73037"
}
```



---

archive/issue_events_073038.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-01T21:16:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28871",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28871#event-73038"
}
```



---

archive/issue_events_073039.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T05:25:02Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28871",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28871#event-73039"
}
```



---

archive/issue_events_073040.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T05:25:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28871",
    "milestone": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28871#event-73040"
}
```
