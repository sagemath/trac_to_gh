# Issue 28871: Fix warnings when compiling the meataxe wrappers

archive/issues_028634.json:
```json
{
    "assignees": [],
    "body": "The following warnings pop up when doing \"sage -br\" after \"make meataxe\":\n\n```\nbuild/cythonized/sage/matrix/matrix_gfpn_dense.c: In function \u2018__pyx_pf_4sage_6matrix_17matrix_gfpn_dense_17Matrix_gfpn_dense_8__reduce__\u2019:\nbuild/cythonized/sage/matrix/matrix_gfpn_dense.c:6086:35: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]\n     for (__pyx_t_6 = 0; __pyx_t_6 < __pyx_t_5; __pyx_t_6+=1) {\n                                   ^\nbuild/cythonized/sage/matrix/matrix_gfpn_dense.c: In function \u2018__pyx_pf_4sage_6matrix_17matrix_gfpn_dense_17Matrix_gfpn_dense_12randomize\u2019:\nbuild/cythonized/sage/matrix/matrix_gfpn_dense.c:7491:43: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]\n           for (__pyx_t_12 = 0; __pyx_t_12 < __pyx_t_11; __pyx_t_12+=1) {\n                                           ^\nbuild/cythonized/sage/matrix/matrix_gfpn_dense.c:7595:43: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]\n           for (__pyx_t_12 = 0; __pyx_t_12 < __pyx_t_11; __pyx_t_12+=1) {\n                                           ^\nbuild/cythonized/sage/matrix/matrix_gfpn_dense.c: In function \u2018__pyx_pf_4sage_6matrix_17matrix_gfpn_dense_17Matrix_gfpn_dense_48_echelon_in_place_classical\u2019:\nbuild/cythonized/sage/matrix/matrix_gfpn_dense.c:15134:39: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]\n       for (__pyx_t_14 = 0; __pyx_t_14 < __pyx_t_4; __pyx_t_14+=1) {\n                                       ^\nbuild/cythonized/sage/matrix/matrix_gfpn_dense.c:15176:39: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]\n       for (__pyx_t_14 = 0; __pyx_t_14 < __pyx_t_4; __pyx_t_14+=1) {\n                                       ^\nbuild/cythonized/sage/matrix/matrix_gfpn_dense.c:15383:37: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]\n     for (__pyx_t_14 = 0; __pyx_t_14 < __pyx_t_4; __pyx_t_14+=1) {\n                                     ^\nbuild/cythonized/sage/matrix/matrix_gfpn_dense.c: In function \u2018__pyx_pf_4sage_6matrix_17matrix_gfpn_dense_mtx_unpickle\u2019:\nbuild/cythonized/sage/matrix/matrix_gfpn_dense.c:16411:43: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]\n     __pyx_t_4 = ((__pyx_v_pickled_rowsize == FfCurrentRowSizeIo) != 0);\n                                           ^\nbuild/cythonized/sage/matrix/matrix_gfpn_dense.c:16433:39: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]\n       for (__pyx_t_24 = 0; __pyx_t_24 < __pyx_t_9; __pyx_t_24+=1) {\n                                       ^\nbuild/cythonized/sage/matrix/matrix_gfpn_dense.c:16490:43: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]\n     __pyx_t_4 = ((__pyx_v_pickled_rowsize >= FfCurrentRowSizeIo) != 0);\n                                           ^\nbuild/cythonized/sage/matrix/matrix_gfpn_dense.c:16556:41: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]\n         for (__pyx_t_24 = 0; __pyx_t_24 < __pyx_t_9; __pyx_t_24+=1) {\n                                         ^\n```\nI guess these warnings should be addressed.\n\nKeywords: **warning meataxe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28871_\n\n",
    "created_at": "2019-12-11T14:41:03Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix warnings when compiling the meataxe wrappers",
    "type": "issue",
    "updated_at": "2022-08-31T05:25:02Z",
    "url": "https://github.com/sagemath/sage/issues/28871",
    "user": "https://github.com/simon-king-jena"
}
```
The following warnings pop up when doing "sage -br" after "make meataxe":

```
build/cythonized/sage/matrix/matrix_gfpn_dense.c: In function ‘__pyx_pf_4sage_6matrix_17matrix_gfpn_dense_17Matrix_gfpn_dense_8__reduce__’:
build/cythonized/sage/matrix/matrix_gfpn_dense.c:6086:35: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]
     for (__pyx_t_6 = 0; __pyx_t_6 < __pyx_t_5; __pyx_t_6+=1) {
                                   ^
build/cythonized/sage/matrix/matrix_gfpn_dense.c: In function ‘__pyx_pf_4sage_6matrix_17matrix_gfpn_dense_17Matrix_gfpn_dense_12randomize’:
build/cythonized/sage/matrix/matrix_gfpn_dense.c:7491:43: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]
           for (__pyx_t_12 = 0; __pyx_t_12 < __pyx_t_11; __pyx_t_12+=1) {
                                           ^
build/cythonized/sage/matrix/matrix_gfpn_dense.c:7595:43: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]
           for (__pyx_t_12 = 0; __pyx_t_12 < __pyx_t_11; __pyx_t_12+=1) {
                                           ^
build/cythonized/sage/matrix/matrix_gfpn_dense.c: In function ‘__pyx_pf_4sage_6matrix_17matrix_gfpn_dense_17Matrix_gfpn_dense_48_echelon_in_place_classical’:
build/cythonized/sage/matrix/matrix_gfpn_dense.c:15134:39: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]
       for (__pyx_t_14 = 0; __pyx_t_14 < __pyx_t_4; __pyx_t_14+=1) {
                                       ^
build/cythonized/sage/matrix/matrix_gfpn_dense.c:15176:39: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]
       for (__pyx_t_14 = 0; __pyx_t_14 < __pyx_t_4; __pyx_t_14+=1) {
                                       ^
build/cythonized/sage/matrix/matrix_gfpn_dense.c:15383:37: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]
     for (__pyx_t_14 = 0; __pyx_t_14 < __pyx_t_4; __pyx_t_14+=1) {
                                     ^
build/cythonized/sage/matrix/matrix_gfpn_dense.c: In function ‘__pyx_pf_4sage_6matrix_17matrix_gfpn_dense_mtx_unpickle’:
build/cythonized/sage/matrix/matrix_gfpn_dense.c:16411:43: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]
     __pyx_t_4 = ((__pyx_v_pickled_rowsize == FfCurrentRowSizeIo) != 0);
                                           ^
build/cythonized/sage/matrix/matrix_gfpn_dense.c:16433:39: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]
       for (__pyx_t_24 = 0; __pyx_t_24 < __pyx_t_9; __pyx_t_24+=1) {
                                       ^
build/cythonized/sage/matrix/matrix_gfpn_dense.c:16490:43: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]
     __pyx_t_4 = ((__pyx_v_pickled_rowsize >= FfCurrentRowSizeIo) != 0);
                                           ^
build/cythonized/sage/matrix/matrix_gfpn_dense.c:16556:41: warning: comparison between signed and unsigned integer expressions [-Wsign-compare]
         for (__pyx_t_24 = 0; __pyx_t_24 < __pyx_t_9; __pyx_t_24+=1) {
                                         ^
```
I guess these warnings should be addressed.

Keywords: **warning meataxe**

_Issue created by migration from https://trac.sagemath.org/ticket/28871_





---

archive/issue_events_378363.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2019-12-11T14:41:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28871",
    "milestone_number": null,
    "milestone_title": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28871#event-378363"
}
```



---

archive/issue_events_378364.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2019-12-11T14:41:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28871",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
    "label_color": "000080",
    "label_name": "component: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28871#event-378364"
}
```



---

archive/issue_events_378365.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2019-12-11T14:41:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28871",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28871#event-378365"
}
```



---

archive/issue_events_378366.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2019-12-11T14:41:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28871",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28871#event-378366"
}
```



---

archive/issue_events_378367.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2019-12-11T14:41:27Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/28871",
    "title_is": "Fix warnings when compiling the meataxe wrappers",
    "title_was": "Fix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28871#event-378367"
}
```



---

archive/issue_comments_453485.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nI'm puzzled. In some instances of the compiler warnings, an int is compared with a size_t. So, I changed declarations so that only int with int or size_t with size_t are compared.\n\nHowever, it didn't fix the problem. Also, in some of the warnings, the comparison was between size_t and size_t.\n\nSo, can please someone explain to me what causes the compiler warning?",
    "created_at": "2019-12-12T15:42:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28871",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28871#issuecomment-453485",
    "user": "https://github.com/simon-king-jena"
}
```

<div id="comment:2" align="right">Comment 2</div>

I'm puzzled. In some instances of the compiler warnings, an int is compared with a size_t. So, I changed declarations so that only int with int or size_t with size_t are compared.

However, it didn't fix the problem. Also, in some of the warnings, the comparison was between size_t and size_t.

So, can please someone explain to me what causes the compiler warning?



---

archive/issue_comments_453486.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nTicket retargeted after milestone closed",
    "created_at": "2020-01-06T14:10:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28871",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28871#issuecomment-453486",
    "user": "https://github.com/embray"
}
```

<div id="comment:3" align="right">Comment 3</div>

Ticket retargeted after milestone closed



---

archive/issue_events_378368.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2020-01-06T14:10:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28871",
    "milestone_number": null,
    "milestone_title": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28871#event-378368"
}
```



---

archive/issue_events_378369.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2020-01-06T14:10:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28871",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28871#event-378369"
}
```



---

archive/issue_events_378370.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28871",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28871#event-378370"
}
```



---

archive/issue_events_378371.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28871",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28871#event-378371"
}
```



---

archive/issue_comments_453487.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBatch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.",
    "created_at": "2020-04-14T19:41:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28871",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28871#issuecomment-453487",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:4" align="right">Comment 4</div>

Batch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.



---

archive/issue_events_378372.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T16:37:33Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28871",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28871#event-378372"
}
```



---

archive/issue_events_378373.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T16:37:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28871",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28871#event-378373"
}
```



---

archive/issue_comments_453488.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nSetting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28871",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28871#issuecomment-453488",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:6" align="right">Comment 6</div>

Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_378374.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28871",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28871#event-378374"
}
```



---

archive/issue_events_378375.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28871",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28871#event-378375"
}
```



---

archive/issue_events_378376.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28871",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28871#event-378376"
}
```



---

archive/issue_events_378377.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28871",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28871#event-378377"
}
```



---

archive/issue_events_378378.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28871",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28871#event-378378"
}
```



---

archive/issue_events_378379.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28871",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28871#event-378379"
}
```



---

archive/issue_events_378380.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-01T21:16:35Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28871",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28871#event-378380"
}
```



---

archive/issue_events_378381.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-01T21:16:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28871",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28871#event-378381"
}
```



---

archive/issue_events_378382.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T05:25:02Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28871",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28871#event-378382"
}
```



---

archive/issue_events_378383.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T05:25:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28871",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28871#event-378383"
}
```
