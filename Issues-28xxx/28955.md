# Issue 28955: Bug in quadratic_form_from_invariants

archive/issues_028718.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: quadratic_form_from_invariants(QQ,3,2,[2],2)\n---------------------------------------------------------------------\nValueError                          Traceback (most recent call last)\n....\nValueError: list should be of even cardinality\n\n```\n\nCC:  @tscrim\n\nBranch/Commit: **[d0356c4](https://github.com/sagemath/sagetrac-mirror/commit/d0356c4023aa22d380df624efc5388fbaeea017c)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Simon Brandhorst**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28955_\n\n",
    "closed_at": "2020-01-11T17:45:14Z",
    "created_at": "2020-01-05T19:16:05Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20quadratic%20forms",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Bug in quadratic_form_from_invariants",
    "type": "issue",
    "updated_at": "2020-01-11T17:45:14Z",
    "url": "https://github.com/sagemath/sage/issues/28955",
    "user": "https://github.com/simonbrandhorst"
}
```

```
sage: quadratic_form_from_invariants(QQ,3,2,[2],2)
---------------------------------------------------------------------
ValueError                          Traceback (most recent call last)
....
ValueError: list should be of even cardinality

```

CC:  @tscrim

Branch/Commit: **[d0356c4](https://github.com/sagemath/sagetrac-mirror/commit/d0356c4023aa22d380df624efc5388fbaeea017c)**

Reviewer: **Travis Scrimshaw**

Author: **Simon Brandhorst**

_Issue created by migration from https://trac.sagemath.org/ticket/28955_





---

archive/issue_events_379593.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2020-01-05T19:16:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28955",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28955#event-379593"
}
```



---

archive/issue_events_379594.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2020-01-05T19:16:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28955",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20quadratic%20forms",
    "label_color": "0000ff",
    "label_name": "component: quadratic forms",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28955#event-379594"
}
```



---

archive/issue_events_379595.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2020-01-05T19:16:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28955",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28955#event-379595"
}
```



---

archive/issue_events_379596.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2020-01-05T19:16:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28955",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28955#event-379596"
}
```



---

archive/issue_comments_455160.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nThe reason was a change of formatting in my code which\nin fact changed its logic.",
    "created_at": "2020-01-05T19:21:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28955",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28955#issuecomment-455160",
    "user": "https://github.com/simonbrandhorst"
}
```

<div id="comment:1" align="right">Comment 1</div>

The reason was a change of formatting in my code which
in fact changed its logic.



---

archive/issue_comments_455161.json:
```json
{
    "body": "Branch: **[u/sbrandhorst/bug_in_quadratic_form_from_invariants](https://github.com/sagemath/sagetrac-mirror/tree/u/sbrandhorst/bug_in_quadratic_form_from_invariants)**",
    "created_at": "2020-01-05T19:22:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28955",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28955#issuecomment-455161",
    "user": "https://github.com/simonbrandhorst"
}
```

Branch: **[u/sbrandhorst/bug_in_quadratic_form_from_invariants](https://github.com/sagemath/sagetrac-mirror/tree/u/sbrandhorst/bug_in_quadratic_form_from_invariants)**



---

archive/issue_comments_455162.json:
```json
{
    "body": "Commit: **[6861bce](https://github.com/sagemath/sagetrac-mirror/commit/6861bce2b5cc1ef32d6a99b8742e02fce165b8a5)**",
    "created_at": "2020-01-05T19:23:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28955",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28955#issuecomment-455162",
    "user": "https://github.com/simonbrandhorst"
}
```

Commit: **[6861bce](https://github.com/sagemath/sagetrac-mirror/commit/6861bce2b5cc1ef32d6a99b8742e02fce165b8a5)**



---

archive/issue_events_379597.json:
```json
{
    "actor": "https://github.com/simonbrandhorst",
    "created_at": "2020-01-05T19:23:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28955",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28955#event-379597"
}
```



---

archive/issue_comments_455163.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6861bce2b5cc1ef32d6a99b8742e02fce165b8a5\">6861bce</a></td><td><code>fixed 28955 quadratic_form_from_invariants</code></td></tr></table>\n",
    "created_at": "2020-01-05T19:23:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28955",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28955#issuecomment-455163",
    "user": "https://github.com/simonbrandhorst"
}
```

<div id="comment:3" align="right">Comment 3</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6861bce2b5cc1ef32d6a99b8742e02fce165b8a5">6861bce</a></td><td><code>fixed 28955 quadratic_form_from_invariants</code></td></tr></table>




---

archive/issue_comments_455164.json:
```json
{
    "body": "Changed commit from **[6861bce](https://github.com/sagemath/sagetrac-mirror/commit/6861bce2b5cc1ef32d6a99b8742e02fce165b8a5)** to **[d0356c4](https://github.com/sagemath/sagetrac-mirror/commit/d0356c4023aa22d380df624efc5388fbaeea017c)**",
    "created_at": "2020-01-05T19:30:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28955",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28955#issuecomment-455164",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[6861bce](https://github.com/sagemath/sagetrac-mirror/commit/6861bce2b5cc1ef32d6a99b8742e02fce165b8a5)** to **[d0356c4](https://github.com/sagemath/sagetrac-mirror/commit/d0356c4023aa22d380df624efc5388fbaeea017c)**



---

archive/issue_comments_455165.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d0356c4023aa22d380df624efc5388fbaeea017c\">d0356c4</a></td><td><code>raise the correct error</code></td></tr></table>\n",
    "created_at": "2020-01-05T19:30:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28955",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28955#issuecomment-455165",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d0356c4023aa22d380df624efc5388fbaeea017c">d0356c4</a></td><td><code>raise the correct error</code></td></tr></table>




---

archive/issue_comments_455166.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -7,4 +7,3 @@\n ValueError: list should be of even cardinality\n \n ```\n-\n``````\n",
    "created_at": "2020-01-05T19:30:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28955",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28955#issuecomment-455166",
    "user": "https://github.com/simonbrandhorst"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -7,4 +7,3 @@
 ValueError: list should be of even cardinality
 
 ```
-
``````




---

archive/issue_comments_455167.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2020-01-06T16:46:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28955",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28955#issuecomment-455167",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_455168.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nLGTM.",
    "created_at": "2020-01-06T16:46:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28955",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28955#issuecomment-455168",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:6" align="right">Comment 6</div>

LGTM.



---

archive/issue_events_379598.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-01-06T16:46:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28955",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28955#event-379598"
}
```



---

archive/issue_events_379599.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-01-06T16:46:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28955",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28955#event-379599"
}
```



---

archive/issue_comments_455169.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nThank you.",
    "created_at": "2020-01-06T16:47:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28955",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28955#issuecomment-455169",
    "user": "https://github.com/simonbrandhorst"
}
```

<div id="comment:7" align="right">Comment 7</div>

Thank you.



---

archive/issue_comments_455170.json:
```json
{
    "body": "Changed branch from **[u/sbrandhorst/bug_in_quadratic_form_from_invariants](https://github.com/sagemath/sagetrac-mirror/tree/u/sbrandhorst/bug_in_quadratic_form_from_invariants)** to **[d0356c4](https://github.com/sagemath/sagetrac-mirror/commit/d0356c4023aa22d380df624efc5388fbaeea017c)**",
    "created_at": "2020-01-11T17:45:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28955",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28955#issuecomment-455170",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/sbrandhorst/bug_in_quadratic_form_from_invariants](https://github.com/sagemath/sagetrac-mirror/tree/u/sbrandhorst/bug_in_quadratic_form_from_invariants)** to **[d0356c4](https://github.com/sagemath/sagetrac-mirror/commit/d0356c4023aa22d380df624efc5388fbaeea017c)**



---

archive/issue_events_379600.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-01-11T17:45:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28955",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28955#event-379600"
}
```



---

archive/issue_events_379601.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "85c95a69593e087a01c185c39b84f099b190cd7b",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-01-11T17:45:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28955",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28955#event-379601"
}
```
