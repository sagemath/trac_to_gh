# Issue 28583: unicode_art of Matrix_mod2_dense fails

archive/issues_028346.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: m = matrix(GF(2), [[1,0],[0,1]])\nsage: unicode_art(m)\n...\nTypeError: str() got an unexpected keyword argument 'unicode'\n```\n\nThis ticket adjusts the signature of `Matrix_mod2_dense.str` to be the same as the one for `Matrix.str`. For complicated arguments, the former redirects to the latter, so now the same is done for `unicode` as well.\n\nBranch/Commit: **[3b8a69c](https://github.com/sagemath/sagetrac-mirror/commit/3b8a69cf8b533ab373fe2af2612d4cbab345ed8e)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Markus Wageringel**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28583_\n\n",
    "closed_at": "2019-12-01T00:38:50Z",
    "created_at": "2019-10-09T19:32:10Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "unicode_art of Matrix_mod2_dense fails",
    "type": "issue",
    "updated_at": "2019-12-01T00:38:50Z",
    "url": "https://github.com/sagemath/sage/issues/28583",
    "user": "https://github.com/mwageringel"
}
```

```
sage: m = matrix(GF(2), [[1,0],[0,1]])
sage: unicode_art(m)
...
TypeError: str() got an unexpected keyword argument 'unicode'
```

This ticket adjusts the signature of `Matrix_mod2_dense.str` to be the same as the one for `Matrix.str`. For complicated arguments, the former redirects to the latter, so now the same is done for `unicode` as well.

Branch/Commit: **[3b8a69c](https://github.com/sagemath/sagetrac-mirror/commit/3b8a69cf8b533ab373fe2af2612d4cbab345ed8e)**

Reviewer: **Frédéric Chapoton**

Author: **Markus Wageringel**

_Issue created by migration from https://trac.sagemath.org/ticket/28583_





---

archive/issue_events_374187.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2019-10-09T19:32:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28583",
    "milestone_number": null,
    "milestone_title": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28583#event-374187"
}
```



---

archive/issue_events_374188.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2019-10-09T19:32:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28583",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28583#event-374188"
}
```



---

archive/issue_events_374189.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2019-10-09T19:32:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28583",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28583#event-374189"
}
```



---

archive/issue_comments_448596.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3b8a69cf8b533ab373fe2af2612d4cbab345ed8e\">3b8a69c</a></td><td><code>28583: fix unicode_art for mod2 matrices</code></td></tr></table>\n",
    "created_at": "2019-11-17T17:55:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28583#issuecomment-448596",
    "user": "https://github.com/mwageringel"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3b8a69cf8b533ab373fe2af2612d4cbab345ed8e">3b8a69c</a></td><td><code>28583: fix unicode_art for mod2 matrices</code></td></tr></table>




---

archive/issue_comments_448597.json:
```json
{
    "body": "Author: **Markus Wageringel**",
    "created_at": "2019-11-17T17:55:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28583#issuecomment-448597",
    "user": "https://github.com/mwageringel"
}
```

Author: **Markus Wageringel**



---

archive/issue_comments_448598.json:
```json
{
    "body": "Branch: **[u/gh-mwageringel/28583](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/28583)**",
    "created_at": "2019-11-17T17:55:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28583#issuecomment-448598",
    "user": "https://github.com/mwageringel"
}
```

Branch: **[u/gh-mwageringel/28583](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/28583)**



---

archive/issue_events_374190.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2019-11-17T17:55:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28583",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28583#event-374190"
}
```



---

archive/issue_comments_448599.json:
```json
{
    "body": "Commit: **[3b8a69c](https://github.com/sagemath/sagetrac-mirror/commit/3b8a69cf8b533ab373fe2af2612d4cbab345ed8e)**",
    "created_at": "2019-11-17T17:55:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28583#issuecomment-448599",
    "user": "https://github.com/mwageringel"
}
```

Commit: **[3b8a69c](https://github.com/sagemath/sagetrac-mirror/commit/3b8a69cf8b533ab373fe2af2612d4cbab345ed8e)**



---

archive/issue_comments_448600.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -6,3 +6,4 @@\n TypeError: str() got an unexpected keyword argument 'unicode'\n ```\n \n+This ticket adjusts the signature of `Matrix_mod2_dense.str` to be the same as the one for `Matrix.str`. For complicated arguments, the former redirects to the latter, so now the same is done for `unicode` as well.\n``````\n",
    "created_at": "2019-11-17T17:55:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28583#issuecomment-448600",
    "user": "https://github.com/mwageringel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -6,3 +6,4 @@
 TypeError: str() got an unexpected keyword argument 'unicode'
 ```
 
+This ticket adjusts the signature of `Matrix_mod2_dense.str` to be the same as the one for `Matrix.str`. For complicated arguments, the former redirects to the latter, so now the same is done for `unicode` as well.
``````




---

archive/issue_events_374191.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-11-17T19:03:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28583",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28583#event-374191"
}
```



---

archive/issue_events_374192.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-11-17T19:03:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28583",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28583#event-374192"
}
```



---

archive/issue_comments_448601.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nok, thx",
    "created_at": "2019-11-17T19:03:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28583#issuecomment-448601",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

ok, thx



---

archive/issue_comments_448602.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2019-11-17T19:03:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28583#issuecomment-448602",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_events_374193.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-12-01T00:38:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28583",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28583#event-374193"
}
```



---

archive/issue_events_374194.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "5a1cacc884e35d5721b4d0143707e4ae0cb87238",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-12-01T00:38:50Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28583",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28583#event-374194"
}
```



---

archive/issue_comments_448603.json:
```json
{
    "body": "Changed branch from **[u/gh-mwageringel/28583](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/28583)** to **[3b8a69c](https://github.com/sagemath/sagetrac-mirror/commit/3b8a69cf8b533ab373fe2af2612d4cbab345ed8e)**",
    "created_at": "2019-12-01T00:38:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28583",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28583#issuecomment-448603",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/gh-mwageringel/28583](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/28583)** to **[3b8a69c](https://github.com/sagemath/sagetrac-mirror/commit/3b8a69cf8b533ab373fe2af2612d4cbab345ed8e)**
