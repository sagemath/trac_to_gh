# Issue 28823: str_to_bytes in math-readline script for Mathematica console

archive/issues_028586.json:
```json
{
    "assignees": [],
    "body": "This fixes an error that occurs when trying to evaluate anything after starting the Mathematica console using Python 3:\n\n```\nsage: mathematica.console(readline=True)\n```\n\n\nKeywords: **mathematica**\n\nBranch/Commit: **[b1d654c](https://github.com/sagemath/sagetrac-mirror/commit/b1d654c0b589f3d26e97ef3520378e2343b5a5c2)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Markus Wageringel**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28823_\n\n",
    "closed_at": "2019-12-01T00:38:28Z",
    "created_at": "2019-11-30T10:40:58Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20interfaces%3A%20optional",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "str_to_bytes in math-readline script for Mathematica console",
    "type": "issue",
    "updated_at": "2019-12-01T00:38:28Z",
    "url": "https://github.com/sagemath/sage/issues/28823",
    "user": "https://github.com/mwageringel"
}
```
This fixes an error that occurs when trying to evaluate anything after starting the Mathematica console using Python 3:

```
sage: mathematica.console(readline=True)
```


Keywords: **mathematica**

Branch/Commit: **[b1d654c](https://github.com/sagemath/sagetrac-mirror/commit/b1d654c0b589f3d26e97ef3520378e2343b5a5c2)**

Reviewer: **Frédéric Chapoton**

Author: **Markus Wageringel**

_Issue created by migration from https://trac.sagemath.org/ticket/28823_





---

archive/issue_events_377720.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2019-11-30T10:40:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28823",
    "milestone_number": null,
    "milestone_title": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28823#event-377720"
}
```



---

archive/issue_events_377721.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2019-11-30T10:40:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28823",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces%3A%20optional",
    "label_color": "0000ff",
    "label_name": "component: interfaces: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28823#event-377721"
}
```



---

archive/issue_events_377722.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2019-11-30T10:40:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28823",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28823#event-377722"
}
```



---

archive/issue_events_377723.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2019-11-30T10:40:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28823",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28823#event-377723"
}
```



---

archive/issue_comments_452511.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b1d654c0b589f3d26e97ef3520378e2343b5a5c2\">b1d654c</a></td><td><code>28823: fix bytes-vs-str in math-readline for mathematica_console</code></td></tr></table>\n",
    "created_at": "2019-11-30T10:43:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28823#issuecomment-452511",
    "user": "https://github.com/mwageringel"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b1d654c0b589f3d26e97ef3520378e2343b5a5c2">b1d654c</a></td><td><code>28823: fix bytes-vs-str in math-readline for mathematica_console</code></td></tr></table>




---

archive/issue_comments_452512.json:
```json
{
    "body": "Author: **Markus Wageringel**",
    "created_at": "2019-11-30T10:43:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28823#issuecomment-452512",
    "user": "https://github.com/mwageringel"
}
```

Author: **Markus Wageringel**



---

archive/issue_events_377724.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2019-11-30T10:43:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28823",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28823#event-377724"
}
```



---

archive/issue_comments_452513.json:
```json
{
    "body": "Commit: **[b1d654c](https://github.com/sagemath/sagetrac-mirror/commit/b1d654c0b589f3d26e97ef3520378e2343b5a5c2)**",
    "created_at": "2019-11-30T10:43:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28823#issuecomment-452513",
    "user": "https://github.com/mwageringel"
}
```

Commit: **[b1d654c](https://github.com/sagemath/sagetrac-mirror/commit/b1d654c0b589f3d26e97ef3520378e2343b5a5c2)**



---

archive/issue_comments_452514.json:
```json
{
    "body": "Changed keywords from **** to **mathematica**",
    "created_at": "2019-11-30T10:43:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28823#issuecomment-452514",
    "user": "https://github.com/mwageringel"
}
```

Changed keywords from **** to **mathematica**



---

archive/issue_comments_452515.json:
```json
{
    "body": "Branch: **[u/gh-mwageringel/28823](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/28823)**",
    "created_at": "2019-11-30T10:43:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28823#issuecomment-452515",
    "user": "https://github.com/mwageringel"
}
```

Branch: **[u/gh-mwageringel/28823](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/28823)**



---

archive/issue_comments_452516.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2019-11-30T11:06:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28823#issuecomment-452516",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_452517.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nI will assume that you tested it.",
    "created_at": "2019-11-30T11:06:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28823#issuecomment-452517",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

I will assume that you tested it.



---

archive/issue_events_377725.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-11-30T11:06:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28823",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28823#event-377725"
}
```



---

archive/issue_events_377726.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-11-30T11:06:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28823",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28823#event-377726"
}
```



---

archive/issue_comments_452518.json:
```json
{
    "body": "Changed branch from **[u/gh-mwageringel/28823](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/28823)** to **[b1d654c](https://github.com/sagemath/sagetrac-mirror/commit/b1d654c0b589f3d26e97ef3520378e2343b5a5c2)**",
    "created_at": "2019-12-01T00:38:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28823",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28823#issuecomment-452518",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/gh-mwageringel/28823](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/28823)** to **[b1d654c](https://github.com/sagemath/sagetrac-mirror/commit/b1d654c0b589f3d26e97ef3520378e2343b5a5c2)**



---

archive/issue_events_377727.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-12-01T00:38:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28823",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28823#event-377727"
}
```



---

archive/issue_events_377728.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "a6d82755a8f49cab3758b89519b048b7c898156c",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-12-01T00:38:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28823",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28823#event-377728"
}
```
