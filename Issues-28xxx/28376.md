# Issue 28376: Shrink Documentation

archive/issues_028139.json:
```json
{
    "body": "as a followup to #28374 of sorts: Why are we shipping 900+ MB of documentation (600MB zipped)?\n\nCC:  @nthiery @slel @haraldschilly @tobiasdiez\n\nStatus: new\n\nIssue created by migration from https://trac.sagemath.org/ticket/28376\n\n",
    "created_at": "2019-08-21T09:13:28Z",
    "labels": [
        "component: documentation"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.8",
    "title": "Shrink Documentation",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/28376",
    "user": "https://github.com/saraedum"
}
```
as a followup to #28374 of sorts: Why are we shipping 900+ MB of documentation (600MB zipped)?

CC:  @nthiery @slel @haraldschilly @tobiasdiez

Status: new

Issue created by migration from https://trac.sagemath.org/ticket/28376





---

archive/issue_comments_396982.json:
```json
{
    "body": "<a id='comment:1'></a>Some experiments:\n\n* html-minifier shrinks HTML content to 80%-90%\n* converting SVG that contain base64 PNGs to PNG and using pngquant shrinks 3D plots to 25%\n* zipping regular SVGs shrinks them to about 30% but browsers mostly refuse to display svgz.\n* imagemin with mozjpeg cuts most JPEG sizes in half.\n\nNow, I don't want to turn all of these tools into SPKGs but they could be installed with yarn easily.",
    "created_at": "2019-08-21T09:27:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28376",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28376#issuecomment-396982",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:1'></a>Some experiments:

* html-minifier shrinks HTML content to 80%-90%
* converting SVG that contain base64 PNGs to PNG and using pngquant shrinks 3D plots to 25%
* zipping regular SVGs shrinks them to about 30% but browsers mostly refuse to display svgz.
* imagemin with mozjpeg cuts most JPEG sizes in half.

Now, I don't want to turn all of these tools into SPKGs but they could be installed with yarn easily.



---

archive/issue_comments_396983.json:
```json
{
    "body": "<a id='comment:3'></a>Note that my numbers above (900MB/600MB) come out of the CI. Locally, things might look a bit different.",
    "created_at": "2019-08-21T09:45:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28376",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28376#issuecomment-396983",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:3'></a>Note that my numbers above (900MB/600MB) come out of the CI. Locally, things might look a bit different.



---

archive/issue_comments_396984.json:
```json
{
    "body": "<a id='comment:4'></a>Ticket retargeted after milestone closed",
    "created_at": "2019-12-30T14:48:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28376",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28376#issuecomment-396984",
    "user": "https://github.com/embray"
}
```

<a id='comment:4'></a>Ticket retargeted after milestone closed



---

archive/issue_events_070890.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28376",
    "milestone": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28376#event-70890"
}
```



---

archive/issue_comments_396985.json:
```json
{
    "body": "<a id='comment:5'></a>Batch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.",
    "created_at": "2020-04-14T19:41:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28376",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28376#issuecomment-396985",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:5'></a>Batch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.



---

archive/issue_events_070891.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28376",
    "milestone": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28376#event-70891"
}
```



---

archive/issue_events_070892.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28376",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28376#event-70892"
}
```



---

archive/issue_events_070893.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T16:37:33Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28376",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28376#event-70893"
}
```



---

archive/issue_events_070894.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T16:37:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28376",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28376#event-70894"
}
```



---

archive/issue_comments_396986.json:
```json
{
    "body": "<a id='comment:7'></a>Setting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28376",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28376#issuecomment-396986",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:7'></a>Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_070895.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28376",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28376#event-70895"
}
```



---

archive/issue_events_070896.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28376",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28376#event-70896"
}
```



---

archive/issue_events_070897.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28376",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28376#event-70897"
}
```



---

archive/issue_events_070898.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:43:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28376",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28376#event-70898"
}
```



---

archive/issue_events_070899.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28376",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28376#event-70899"
}
```



---

archive/issue_events_070900.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28376",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28376#event-70900"
}
```



---

archive/issue_events_070901.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-07T00:07:21Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28376",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28376#event-70901"
}
```



---

archive/issue_events_070902.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-07T00:07:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28376",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28376#event-70902"
}
```



---

archive/issue_events_070903.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28376",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28376#event-70903"
}
```



---

archive/issue_events_070904.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28376",
    "milestone": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28376#event-70904"
}
```
