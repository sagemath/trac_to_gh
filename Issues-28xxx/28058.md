# Issue 28058: Reimplement dualRSK algorithm

archive/issues_027821.json:
```json
{
    "assignees": [],
    "body": "To move the implementation of dualRSK from #25070 following the new RSK structure introduced in #27852 thereby finishing #25070.\n\nCC:  @tscrim @darijgr\n\nKeywords: **gsocRSK**\n\nBranch/Commit: **[u/gh-ChamanAgrawal/28058_dualRSK](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-ChamanAgrawal/28058_dualRSK) @ [1bc9fb9](https://github.com/sagemath/sagetrac-mirror/commit/1bc9fb9c07d2eaf62e2b58f2c893b464fff9e938)**\n\nReviewer: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28058_\n\n",
    "closed_at": "2019-08-20T06:23:14Z",
    "created_at": "2019-06-26T01:38:47Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Reimplement dualRSK algorithm",
    "type": "issue",
    "updated_at": "2019-08-20T06:23:14Z",
    "url": "https://github.com/sagemath/sage/issues/28058",
    "user": "https://github.com/ChamanAgrawal"
}
```
To move the implementation of dualRSK from #25070 following the new RSK structure introduced in #27852 thereby finishing #25070.

CC:  @tscrim @darijgr

Keywords: **gsocRSK**

Branch/Commit: **[u/gh-ChamanAgrawal/28058_dualRSK](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-ChamanAgrawal/28058_dualRSK) @ [1bc9fb9](https://github.com/sagemath/sagetrac-mirror/commit/1bc9fb9c07d2eaf62e2b58f2c893b464fff9e938)**

Reviewer: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/28058_





---

archive/issue_events_366522.json:
```json
{
    "actor": "https://github.com/ChamanAgrawal",
    "created_at": "2019-06-26T01:38:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28058",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28058#event-366522"
}
```



---

archive/issue_events_366523.json:
```json
{
    "actor": "https://github.com/ChamanAgrawal",
    "created_at": "2019-06-26T01:38:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28058",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28058#event-366523"
}
```



---

archive/issue_events_366524.json:
```json
{
    "actor": "https://github.com/ChamanAgrawal",
    "created_at": "2019-06-26T01:38:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28058",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28058#event-366524"
}
```



---

archive/issue_events_366525.json:
```json
{
    "actor": "https://github.com/ChamanAgrawal",
    "created_at": "2019-06-26T01:38:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28058",
    "label": "https://github.com/sagemath/sage/labels/feature",
    "label_color": "008080",
    "label_name": "feature",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28058#event-366525"
}
```



---

archive/issue_events_366526.json:
```json
{
    "actor": "https://github.com/ChamanAgrawal",
    "created_at": "2019-06-26T01:41:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28058",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28058#event-366526"
}
```



---

archive/issue_comments_439444.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nLast 10 new commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/45869d911ca94ac2baaebd6c73986e0d6ef4d498\">45869d9</a></td><td><code>Minor documentation changes</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cabc1ec163c1167c5b26c8ca1c125117eb52d782\">cabc1ec</a></td><td><code>Add one-line doc and examples to functions</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/53ee04a7882afa4c63462516bd226cab1e143f35\">53ee04a</a></td><td><code>Add examples to functions</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/37d09de5740b1eed47d3fd149ebef2c9b34cc4bb\">37d09de</a></td><td><code>Added simpler examples in RuleEG rev_insertion</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8d339e0bd1689fb7ad884d97978ea041914d8151\">8d339e0</a></td><td><code>Merge branch 'u/gh-ChamanAgrawal/27852_refactorRSK' into start_dualRSK</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6ced9c8483d196eaa4e785a0e73e9aee258c5f08\">6ced9c8</a></td><td><code>Added documentation and tests for DualRSK</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5313c0e8b5bb1bc6649f9defa1e6d3c39745da6f\">5313c0e</a></td><td><code>Changed (rev)insertion() to single row operation</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/734ae02079472057d8bd394b03c5e2edd0726d00\">734ae02</a></td><td><code>Moved rules' documentation into their class</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a21d65f875c76954a8b169c4efdb0d0d964d4535\">a21d65f</a></td><td><code>Change insertion function</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b467c1f40e56aa6f6216c6d322455eaeacace253\">b467c1f</a></td><td><code>Added documentation to dual RSK</code></td></tr></table>\n",
    "created_at": "2019-06-26T01:41:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28058",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28058#issuecomment-439444",
    "user": "https://github.com/ChamanAgrawal"
}
```

<a id='comment:1'>Comment 1:</a>
Last 10 new commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/45869d911ca94ac2baaebd6c73986e0d6ef4d498">45869d9</a></td><td><code>Minor documentation changes</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cabc1ec163c1167c5b26c8ca1c125117eb52d782">cabc1ec</a></td><td><code>Add one-line doc and examples to functions</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/53ee04a7882afa4c63462516bd226cab1e143f35">53ee04a</a></td><td><code>Add examples to functions</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/37d09de5740b1eed47d3fd149ebef2c9b34cc4bb">37d09de</a></td><td><code>Added simpler examples in RuleEG rev_insertion</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8d339e0bd1689fb7ad884d97978ea041914d8151">8d339e0</a></td><td><code>Merge branch 'u/gh-ChamanAgrawal/27852_refactorRSK' into start_dualRSK</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6ced9c8483d196eaa4e785a0e73e9aee258c5f08">6ced9c8</a></td><td><code>Added documentation and tests for DualRSK</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5313c0e8b5bb1bc6649f9defa1e6d3c39745da6f">5313c0e</a></td><td><code>Changed (rev)insertion() to single row operation</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/734ae02079472057d8bd394b03c5e2edd0726d00">734ae02</a></td><td><code>Moved rules' documentation into their class</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a21d65f875c76954a8b169c4efdb0d0d964d4535">a21d65f</a></td><td><code>Change insertion function</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b467c1f40e56aa6f6216c6d322455eaeacace253">b467c1f</a></td><td><code>Added documentation to dual RSK</code></td></tr></table>




---

archive/issue_comments_439445.json:
```json
{
    "body": "Commit: **[b467c1f](https://github.com/sagemath/sagetrac-mirror/commit/b467c1f40e56aa6f6216c6d322455eaeacace253)**",
    "created_at": "2019-06-26T01:41:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28058",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28058#issuecomment-439445",
    "user": "https://github.com/ChamanAgrawal"
}
```

Commit: **[b467c1f](https://github.com/sagemath/sagetrac-mirror/commit/b467c1f40e56aa6f6216c6d322455eaeacace253)**



---

archive/issue_comments_439446.json:
```json
{
    "body": "Branch: **[u/gh-ChamanAgrawal/28058_dualRSK](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-ChamanAgrawal/28058_dualRSK)**",
    "created_at": "2019-06-26T01:41:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28058",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28058#issuecomment-439446",
    "user": "https://github.com/ChamanAgrawal"
}
```

Branch: **[u/gh-ChamanAgrawal/28058_dualRSK](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-ChamanAgrawal/28058_dualRSK)**



---

archive/issue_comments_439447.json:
```json
{
    "body": "Changed commit from **[b467c1f](https://github.com/sagemath/sagetrac-mirror/commit/b467c1f40e56aa6f6216c6d322455eaeacace253)** to **[7246802](https://github.com/sagemath/sagetrac-mirror/commit/7246802eef84b5ec61046003d5b7e217d1f644ca)**",
    "created_at": "2019-06-26T20:47:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28058",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28058#issuecomment-439447",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[b467c1f](https://github.com/sagemath/sagetrac-mirror/commit/b467c1f40e56aa6f6216c6d322455eaeacace253)** to **[7246802](https://github.com/sagemath/sagetrac-mirror/commit/7246802eef84b5ec61046003d5b7e217d1f644ca)**



---

archive/issue_comments_439448.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d5bf7debd2d938a0d8e141f3bcab32145d3d4f73\">d5bf7de</a></td><td><code>Change insertion() to reduce passing parameters</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9f208c9ddb5ffe18fcd2b9c3eb0b61c775a80e2d\">9f208c9</a></td><td><code>Merge branch 'u/gh-ChamanAgrawal/27852_refactorRSK' into u/gh-ChamanAgrawal/28058_dualRSK</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7246802eef84b5ec61046003d5b7e217d1f644ca\">7246802</a></td><td><code>Removed extra parameter from insertion()</code></td></tr></table>\n",
    "created_at": "2019-06-26T20:47:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28058",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28058#issuecomment-439448",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d5bf7debd2d938a0d8e141f3bcab32145d3d4f73">d5bf7de</a></td><td><code>Change insertion() to reduce passing parameters</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9f208c9ddb5ffe18fcd2b9c3eb0b61c775a80e2d">9f208c9</a></td><td><code>Merge branch 'u/gh-ChamanAgrawal/27852_refactorRSK' into u/gh-ChamanAgrawal/28058_dualRSK</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7246802eef84b5ec61046003d5b7e217d1f644ca">7246802</a></td><td><code>Removed extra parameter from insertion()</code></td></tr></table>




---

archive/issue_comments_439449.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5cc0008640ffd3ded68d9c508d07f776d470d937\">5cc0008</a></td><td><code>Minor changes in documentation</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4ee1e3550add78cc8cee49ce0d6049b90d609c87\">4ee1e35</a></td><td><code>Remove insertion_complete for insertion()</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/02db2163641dc4f5d00c1c7b13414597dfd9bf6c\">02db216</a></td><td><code>Merge branch 'u/gh-ChamanAgrawal/27852_refactorRSK' into u/gh-ChamanAgrawal/28058_dualRSK</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/89a749c4a3863a4ed2dac1bed88202f359e34a02\">89a749c</a></td><td><code>Minor change in docs and insertion()</code></td></tr></table>\n",
    "created_at": "2019-06-29T17:50:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28058",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28058#issuecomment-439449",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:3'>Comment 3:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5cc0008640ffd3ded68d9c508d07f776d470d937">5cc0008</a></td><td><code>Minor changes in documentation</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4ee1e3550add78cc8cee49ce0d6049b90d609c87">4ee1e35</a></td><td><code>Remove insertion_complete for insertion()</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/02db2163641dc4f5d00c1c7b13414597dfd9bf6c">02db216</a></td><td><code>Merge branch 'u/gh-ChamanAgrawal/27852_refactorRSK' into u/gh-ChamanAgrawal/28058_dualRSK</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/89a749c4a3863a4ed2dac1bed88202f359e34a02">89a749c</a></td><td><code>Minor change in docs and insertion()</code></td></tr></table>




---

archive/issue_comments_439450.json:
```json
{
    "body": "Changed commit from **[7246802](https://github.com/sagemath/sagetrac-mirror/commit/7246802eef84b5ec61046003d5b7e217d1f644ca)** to **[89a749c](https://github.com/sagemath/sagetrac-mirror/commit/89a749c4a3863a4ed2dac1bed88202f359e34a02)**",
    "created_at": "2019-06-29T17:50:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28058",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28058#issuecomment-439450",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[7246802](https://github.com/sagemath/sagetrac-mirror/commit/7246802eef84b5ec61046003d5b7e217d1f644ca)** to **[89a749c](https://github.com/sagemath/sagetrac-mirror/commit/89a749c4a3863a4ed2dac1bed88202f359e34a02)**



---

archive/issue_comments_439451.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f946493ef296141eff97b13d186dcb23da1de4e4\">f946493</a></td><td><code>Add forgotten docs change</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e4554eebab17cb27c02e151c1e25566202e74d21\">e4554ee</a></td><td><code>Docs corrected</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e03a2c9a3f5a0caf1532c3e2259c12b65e3e87ec\">e03a2c9</a></td><td><code>Merge branch 'u/gh-ChamanAgrawal/27852_refactorRSK' into u/gh-ChamanAgrawal/28058_dualRSK</code></td></tr></table>\n",
    "created_at": "2019-07-02T18:19:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28058",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28058#issuecomment-439451",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f946493ef296141eff97b13d186dcb23da1de4e4">f946493</a></td><td><code>Add forgotten docs change</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e4554eebab17cb27c02e151c1e25566202e74d21">e4554ee</a></td><td><code>Docs corrected</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e03a2c9a3f5a0caf1532c3e2259c12b65e3e87ec">e03a2c9</a></td><td><code>Merge branch 'u/gh-ChamanAgrawal/27852_refactorRSK' into u/gh-ChamanAgrawal/28058_dualRSK</code></td></tr></table>




---

archive/issue_comments_439452.json:
```json
{
    "body": "Changed commit from **[89a749c](https://github.com/sagemath/sagetrac-mirror/commit/89a749c4a3863a4ed2dac1bed88202f359e34a02)** to **[e03a2c9](https://github.com/sagemath/sagetrac-mirror/commit/e03a2c9a3f5a0caf1532c3e2259c12b65e3e87ec)**",
    "created_at": "2019-07-02T18:19:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28058",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28058#issuecomment-439452",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[89a749c](https://github.com/sagemath/sagetrac-mirror/commit/89a749c4a3863a4ed2dac1bed88202f359e34a02)** to **[e03a2c9](https://github.com/sagemath/sagetrac-mirror/commit/e03a2c9a3f5a0caf1532c3e2259c12b65e3e87ec)**



---

archive/issue_comments_439453.json:
```json
{
    "body": "Changed commit from **[e03a2c9](https://github.com/sagemath/sagetrac-mirror/commit/e03a2c9a3f5a0caf1532c3e2259c12b65e3e87ec)** to **[1bc9fb9](https://github.com/sagemath/sagetrac-mirror/commit/1bc9fb9c07d2eaf62e2b58f2c893b464fff9e938)**",
    "created_at": "2019-07-02T18:39:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28058",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28058#issuecomment-439453",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[e03a2c9](https://github.com/sagemath/sagetrac-mirror/commit/e03a2c9a3f5a0caf1532c3e2259c12b65e3e87ec)** to **[1bc9fb9](https://github.com/sagemath/sagetrac-mirror/commit/1bc9fb9c07d2eaf62e2b58f2c893b464fff9e938)**



---

archive/issue_comments_439454.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1bc9fb9c07d2eaf62e2b58f2c893b464fff9e938\">1bc9fb9</a></td><td><code>Resolved merge conflicts</code></td></tr></table>\n",
    "created_at": "2019-07-02T18:39:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28058",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28058#issuecomment-439454",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:5'>Comment 5:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1bc9fb9c07d2eaf62e2b58f2c893b464fff9e938">1bc9fb9</a></td><td><code>Resolved merge conflicts</code></td></tr></table>




---

archive/issue_comments_439455.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2019-07-25T07:33:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28058",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28058#issuecomment-439455",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_439456.json:
```json
{
    "body": "Changed author from **Chaman Agrawal** to none",
    "created_at": "2019-07-25T07:33:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28058",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28058#issuecomment-439456",
    "user": "https://github.com/tscrim"
}
```

Changed author from **Chaman Agrawal** to none



---

archive/issue_events_366527.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-07-25T07:33:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28058",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28058#event-366527"
}
```



---

archive/issue_events_366528.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-07-25T07:33:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28058",
    "label": "https://github.com/sagemath/sage/labels/feature",
    "label_color": "008080",
    "label_name": "feature",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28058#event-366528"
}
```



---

archive/issue_events_366529.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-07-25T07:33:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28058",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28058#event-366529"
}
```



---

archive/issue_comments_439457.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nThis was reviewed in #25070.",
    "created_at": "2019-07-25T07:33:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28058",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28058#issuecomment-439457",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:6'>Comment 6:</a>
This was reviewed in #25070.



---

archive/issue_events_366530.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-08-18T03:33:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28058",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28058#event-366530"
}
```



---

archive/issue_events_366531.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2019-08-18T03:33:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28058",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28058#event-366531"
}
```



---

archive/issue_events_366532.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-08-20T06:23:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28058",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28058#event-366532"
}
```



---

archive/issue_events_366533.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-08-20T06:23:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28058",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28058#event-366533"
}
```



---

archive/issue_events_366534.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-08-20T06:23:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28058",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28058#event-366534"
}
```
