# Issue 28345: Bug with NumberField.abs_val at zero

archive/issues_028108.json:
```json
{
    "assignees": [],
    "body": "The following code fails:\n\n```\nsage: x = polygen(ZZ)\nsage: K = NumberField(x^3 - 3, 'a')\nsage: v = tuple(K.primes_above(3))[0]\nsage: K.abs_val(v, K.zero())\nTraceback (most recent call last)\n...\nTypeError: unsupported operand parent(s) for ^:\n'The Infinity Ring' and 'The Infinity Ring'\n```\n\nIt should return 0. We fix the bug and add a doctest.\n\nRelated:\n\n- #28241: Fix number field abs_val at non-real places\n\nCC:  @slel @JohnCremona @roed314 @jhpalmieri\n\nBranch/Commit: **[1d7248c](https://github.com/sagemath/sagetrac-mirror/commit/1d7248caaf3854bbbb64725b2eadc73986568b2d)**\n\nReviewer: **Samuel Leli\u00e8vre**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28345_\n\n",
    "closed_at": "2021-08-29T09:37:55Z",
    "created_at": "2019-08-12T13:09:37Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Bug with NumberField.abs_val at zero",
    "type": "issue",
    "updated_at": "2021-08-29T09:37:55Z",
    "url": "https://github.com/sagemath/sage/issues/28345",
    "user": "https://github.com/sagetrac-mercatp"
}
```
The following code fails:

```
sage: x = polygen(ZZ)
sage: K = NumberField(x^3 - 3, 'a')
sage: v = tuple(K.primes_above(3))[0]
sage: K.abs_val(v, K.zero())
Traceback (most recent call last)
...
TypeError: unsupported operand parent(s) for ^:
'The Infinity Ring' and 'The Infinity Ring'
```

It should return 0. We fix the bug and add a doctest.

Related:

- #28241: Fix number field abs_val at non-real places

CC:  @slel @JohnCremona @roed314 @jhpalmieri

Branch/Commit: **[1d7248c](https://github.com/sagemath/sagetrac-mirror/commit/1d7248caaf3854bbbb64725b2eadc73986568b2d)**

Reviewer: **Samuel Lelièvre**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/28345_





---

archive/issue_events_370763.json:
```json
{
    "actor": "https://github.com/sagetrac-mercatp",
    "created_at": "2019-08-12T13:09:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28345#event-370763"
}
```



---

archive/issue_events_370764.json:
```json
{
    "actor": "https://github.com/sagetrac-mercatp",
    "created_at": "2019-08-12T13:09:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
    "label_color": "0000ff",
    "label_name": "component: number fields",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28345#event-370764"
}
```



---

archive/issue_events_370765.json:
```json
{
    "actor": "https://github.com/sagetrac-mercatp",
    "created_at": "2019-08-12T13:09:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28345#event-370765"
}
```



---

archive/issue_events_370766.json:
```json
{
    "actor": "https://github.com/sagetrac-mercatp",
    "created_at": "2019-08-12T13:09:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28345#event-370766"
}
```



---

archive/issue_events_370767.json:
```json
{
    "actor": "https://github.com/sagetrac-mercatp",
    "created_at": "2019-08-12T13:38:51Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "title_is": "Bug with NumberField.abs_val at zero",
    "title_was": "Bug with NumberField.abs_val",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28345#event-370767"
}
```



---

archive/issue_comments_444651.json:
```json
{
    "body": "Branch: **[u/mercatp/bug_with_numberfield_abs_val_at_zero](https://github.com/sagemath/sagetrac-mirror/tree/u/mercatp/bug_with_numberfield_abs_val_at_zero)**",
    "created_at": "2019-08-12T13:41:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28345#issuecomment-444651",
    "user": "https://github.com/sagetrac-mercatp"
}
```

Branch: **[u/mercatp/bug_with_numberfield_abs_val_at_zero](https://github.com/sagemath/sagetrac-mirror/tree/u/mercatp/bug_with_numberfield_abs_val_at_zero)**



---

archive/issue_comments_444652.json:
```json
{
    "body": "Commit: **[9261c0b](https://github.com/sagemath/sagetrac-mirror/commit/9261c0ba965cb5c5dc8e5ab1090cb073d3da52b7)**",
    "created_at": "2019-08-12T13:42:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28345#issuecomment-444652",
    "user": "https://github.com/sagetrac-mercatp"
}
```

Commit: **[9261c0b](https://github.com/sagemath/sagetrac-mirror/commit/9261c0ba965cb5c5dc8e5ab1090cb073d3da52b7)**



---

archive/issue_comments_444653.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -8,3 +8,5 @@\n \n It should return 0.\n See also ticket #28241\n+\n+The bug is corrected and a doctest has been added.\n``````\n",
    "created_at": "2019-08-12T13:42:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28345#issuecomment-444653",
    "user": "https://github.com/sagetrac-mercatp"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -8,3 +8,5 @@
 
 It should return 0.
 See also ticket #28241
+
+The bug is corrected and a doctest has been added.
``````




---

archive/issue_comments_444654.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9261c0ba965cb5c5dc8e5ab1090cb073d3da52b7\">9261c0b</a></td><td><code>Correct a bug with NumberField.abs_val and add a doctest to check that it's corected</code></td></tr></table>\n",
    "created_at": "2019-08-12T13:42:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28345#issuecomment-444654",
    "user": "https://github.com/sagetrac-mercatp"
}
```

<div id="comment:3" align="right">Comment 3</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9261c0ba965cb5c5dc8e5ab1090cb073d3da52b7">9261c0b</a></td><td><code>Correct a bug with NumberField.abs_val and add a doctest to check that it's corected</code></td></tr></table>




---

archive/issue_events_370768.json:
```json
{
    "actor": "https://github.com/alexjbest",
    "created_at": "2019-09-05T12:07:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28345#event-370768"
}
```



---

archive/issue_comments_444655.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nI set this to needs review as it seems it is done, so the patchbots can check it.\n\nThe other way to do this would be to ensure that `p^(-Infinity)` returns 0 in some way, but I don't suppose that really matters.",
    "created_at": "2019-09-05T12:07:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28345#issuecomment-444655",
    "user": "https://github.com/alexjbest"
}
```

<div id="comment:4" align="right">Comment 4</div>

I set this to needs review as it seems it is done, so the patchbots can check it.

The other way to do this would be to ensure that `p^(-Infinity)` returns 0 in some way, but I don't suppose that really matters.



---

archive/issue_comments_444656.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nReturning `0` gives an `int` which is of the wrong type, so please change it to `return R.zero()` instead. Moreover, a blank line is needed after `TESTS::` and please remove the trailing whitespace in the line above that. Also enter your full name in the authors field.\n\nI am not too familiar with the theory, so it would be good if someone can confirm this is mathematically correct.",
    "created_at": "2019-12-01T19:03:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28345#issuecomment-444656",
    "user": "https://github.com/mwageringel"
}
```

<div id="comment:5" align="right">Comment 5</div>

Returning `0` gives an `int` which is of the wrong type, so please change it to `return R.zero()` instead. Moreover, a blank line is needed after `TESTS::` and please remove the trailing whitespace in the line above that. Also enter your full name in the authors field.

I am not too familiar with the theory, so it would be good if someone can confirm this is mathematically correct.



---

archive/issue_events_370769.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2019-12-01T19:04:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28345#event-370769"
}
```



---

archive/issue_events_370770.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2019-12-01T19:04:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28345#event-370770"
}
```



---

archive/issue_events_370771.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2019-12-01T19:04:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28345#event-370771"
}
```



---

archive/issue_events_370772.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2019-12-01T19:04:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "milestone_number": null,
    "milestone_title": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28345#event-370772"
}
```



---

archive/issue_comments_444657.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nTicket retargeted after milestone closed",
    "created_at": "2020-01-06T14:10:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28345#issuecomment-444657",
    "user": "https://github.com/embray"
}
```

<div id="comment:7" align="right">Comment 7</div>

Ticket retargeted after milestone closed



---

archive/issue_events_370773.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2020-01-06T14:10:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "milestone_number": null,
    "milestone_title": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28345#event-370773"
}
```



---

archive/issue_events_370774.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2020-01-06T14:10:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28345#event-370774"
}
```



---

archive/issue_comments_444658.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nMoving tickets to milestone sage-9.2 based on a review of last modification date, branch status, and severity.",
    "created_at": "2020-05-01T04:28:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28345#issuecomment-444658",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:8" align="right">Comment 8</div>

Moving tickets to milestone sage-9.2 based on a review of last modification date, branch status, and severity.



---

archive/issue_events_370775.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-05-01T04:28:42Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28345#event-370775"
}
```



---

archive/issue_events_370776.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-05-01T04:28:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28345#event-370776"
}
```



---

archive/issue_events_370777.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28345#event-370777"
}
```



---

archive/issue_events_370778.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28345#event-370778"
}
```



---

archive/issue_events_370779.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28345#event-370779"
}
```



---

archive/issue_events_370780.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28345#event-370780"
}
```



---

archive/issue_comments_444659.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nSetting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28345#issuecomment-444659",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:10" align="right">Comment 10</div>

Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_comments_444660.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,12 +1,18 @@\n-The following code fail:\n+The following code fails:\n \n ```\n-sage: K.<xi> = NumberField(x^3-3)\n-sage: v_fin = tuple(K.primes_above(3))[0]\n-sage: abs_val(K, v_fin, K(0))\n+sage: x = polygen(ZZ)\n+sage: K = NumberField(x^3 - 3, 'a')\n+sage: v = tuple(K.primes_above(3))[0]\n+sage: K.abs_val(v, K.zero())\n+Traceback (most recent call last)\n+...\n+TypeError: unsupported operand parent(s) for ^:\n+'The Infinity Ring' and 'The Infinity Ring'\n ```\n \n-It should return 0.\n-See also ticket #28241\n+It should return 0. We fix the bug and add a doctest.\n \n-The bug is corrected and a doctest has been added.\n+Related:\n+\n+- #28241: Fix number field abs_val at non-real places\n``````\n",
    "created_at": "2021-07-21T13:46:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28345#issuecomment-444660",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,12 +1,18 @@
-The following code fail:
+The following code fails:
 
 ```
-sage: K.<xi> = NumberField(x^3-3)
-sage: v_fin = tuple(K.primes_above(3))[0]
-sage: abs_val(K, v_fin, K(0))
+sage: x = polygen(ZZ)
+sage: K = NumberField(x^3 - 3, 'a')
+sage: v = tuple(K.primes_above(3))[0]
+sage: K.abs_val(v, K.zero())
+Traceback (most recent call last)
+...
+TypeError: unsupported operand parent(s) for ^:
+'The Infinity Ring' and 'The Infinity Ring'
 ```
 
-It should return 0.
-See also ticket #28241
+It should return 0. We fix the bug and add a doctest.
 
-The bug is corrected and a doctest has been added.
+Related:
+
+- #28241: Fix number field abs_val at non-real places
``````




---

archive/issue_events_370781.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-08-01T07:41:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28345#event-370781"
}
```



---

archive/issue_events_370782.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-08-01T07:41:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28345#event-370782"
}
```



---

archive/issue_comments_444661.json:
```json
{
    "body": "Changed commit from **[9261c0b](https://github.com/sagemath/sagetrac-mirror/commit/9261c0ba965cb5c5dc8e5ab1090cb073d3da52b7)** to **[1d7248c](https://github.com/sagemath/sagetrac-mirror/commit/1d7248caaf3854bbbb64725b2eadc73986568b2d)**",
    "created_at": "2021-08-01T07:41:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28345#issuecomment-444661",
    "user": "https://github.com/fchapoton"
}
```

Changed commit from **[9261c0b](https://github.com/sagemath/sagetrac-mirror/commit/9261c0ba965cb5c5dc8e5ab1090cb073d3da52b7)** to **[1d7248c](https://github.com/sagemath/sagetrac-mirror/commit/1d7248caaf3854bbbb64725b2eadc73986568b2d)**



---

archive/issue_comments_444662.json:
```json
{
    "body": "Changed branch from **[u/mercatp/bug_with_numberfield_abs_val_at_zero](https://github.com/sagemath/sagetrac-mirror/tree/u/mercatp/bug_with_numberfield_abs_val_at_zero)** to **[u/chapoton/28345](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/28345)**",
    "created_at": "2021-08-01T07:41:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28345#issuecomment-444662",
    "user": "https://github.com/fchapoton"
}
```

Changed branch from **[u/mercatp/bug_with_numberfield_abs_val_at_zero](https://github.com/sagemath/sagetrac-mirror/tree/u/mercatp/bug_with_numberfield_abs_val_at_zero)** to **[u/chapoton/28345](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/28345)**



---

archive/issue_comments_444663.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nhere is a brand new branch, please review\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1d7248caaf3854bbbb64725b2eadc73986568b2d\">1d7248c</a></td><td><code>fix absolute value of zero in number fields</code></td></tr></table>\n",
    "created_at": "2021-08-01T07:41:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28345#issuecomment-444663",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:12" align="right">Comment 12</div>

here is a brand new branch, please review

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1d7248caaf3854bbbb64725b2eadc73986568b2d">1d7248c</a></td><td><code>fix absolute value of zero in number fields</code></td></tr></table>




---

archive/issue_comments_444664.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\ngreen bot, please review this very simple ticket",
    "created_at": "2021-08-01T12:44:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28345#issuecomment-444664",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:14" align="right">Comment 14</div>

green bot, please review this very simple ticket



---

archive/issue_events_370783.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-08-01T13:17:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28345#event-370783"
}
```



---

archive/issue_events_370784.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-08-01T13:17:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28345#event-370784"
}
```



---

archive/issue_comments_444665.json:
```json
{
    "body": "Author: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2021-08-01T13:17:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28345#issuecomment-444665",
    "user": "https://github.com/slel"
}
```

Author: **Frédéric Chapoton**



---

archive/issue_comments_444666.json:
```json
{
    "body": "Reviewer: **Samuel Leli\u00e8vre**",
    "created_at": "2021-08-01T13:17:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28345#issuecomment-444666",
    "user": "https://github.com/slel"
}
```

Reviewer: **Samuel Lelièvre**



---

archive/issue_events_370785.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-22T20:35:10Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28345#event-370785"
}
```



---

archive/issue_events_370786.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-22T20:35:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28345#event-370786"
}
```



---

archive/issue_comments_444667.json:
```json
{
    "body": "Changed branch from **[u/chapoton/28345](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/28345)** to **[1d7248c](https://github.com/sagemath/sagetrac-mirror/commit/1d7248caaf3854bbbb64725b2eadc73986568b2d)**",
    "created_at": "2021-08-29T09:37:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28345#issuecomment-444667",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/28345](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/28345)** to **[1d7248c](https://github.com/sagemath/sagetrac-mirror/commit/1d7248caaf3854bbbb64725b2eadc73986568b2d)**



---

archive/issue_events_370787.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-08-29T09:37:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28345#event-370787"
}
```



---

archive/issue_events_370788.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "9ec8c5c036bf98e001589c10ae952d3d170d3882",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-08-29T09:37:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28345",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28345#event-370788"
}
```
