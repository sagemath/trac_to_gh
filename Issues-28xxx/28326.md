# Issue 28326: Restore default term state after printing environment variables

archive/issues_028089.json:
```json
{
    "assignees": [],
    "body": "Terminal may be pollute color sequences in environment variables during build.\nBasically, my build log is full green.\n\nThis can be avoided by restoring default color after the dump of variables.\n\nCC:  @embray\n\nBranch/Commit: **[671efa7](https://github.com/sagemath/sagetrac-mirror/commit/671efa739944f9188f5187f14ce5e1656994c765)**\n\nReviewer: **Erik Bray**\n\nAuthor: **Henri Derycke**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28326_\n\n",
    "closed_at": "2019-12-20T22:45:53Z",
    "created_at": "2019-08-06T12:13:25Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Restore default term state after printing environment variables",
    "type": "issue",
    "updated_at": "2019-12-20T22:45:53Z",
    "url": "https://github.com/sagemath/sage/issues/28326",
    "user": "https://github.com/nheir"
}
```
Terminal may be pollute color sequences in environment variables during build.
Basically, my build log is full green.

This can be avoided by restoring default color after the dump of variables.

CC:  @embray

Branch/Commit: **[671efa7](https://github.com/sagemath/sagetrac-mirror/commit/671efa739944f9188f5187f14ce5e1656994c765)**

Reviewer: **Erik Bray**

Author: **Henri Derycke**

_Issue created by migration from https://trac.sagemath.org/ticket/28326_





---

archive/issue_events_370514.json:
```json
{
    "actor": "https://github.com/nheir",
    "created_at": "2019-08-06T12:13:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28326",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28326#event-370514"
}
```



---

archive/issue_events_370515.json:
```json
{
    "actor": "https://github.com/nheir",
    "created_at": "2019-08-06T12:13:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28326",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28326#event-370515"
}
```



---

archive/issue_events_370516.json:
```json
{
    "actor": "https://github.com/nheir",
    "created_at": "2019-08-06T12:13:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28326",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28326#event-370516"
}
```



---

archive/issue_events_370517.json:
```json
{
    "actor": "https://github.com/nheir",
    "created_at": "2019-08-06T12:13:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28326",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28326#event-370517"
}
```



---

archive/issue_comments_444345.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/671efa739944f9188f5187f14ce5e1656994c765\">671efa7</a></td><td><code>restore terminal default colors</code></td></tr></table>\n",
    "created_at": "2019-08-06T12:16:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28326",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28326#issuecomment-444345",
    "user": "https://github.com/nheir"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/671efa739944f9188f5187f14ce5e1656994c765">671efa7</a></td><td><code>restore terminal default colors</code></td></tr></table>




---

archive/issue_comments_444346.json:
```json
{
    "body": "Commit: **[671efa7](https://github.com/sagemath/sagetrac-mirror/commit/671efa739944f9188f5187f14ce5e1656994c765)**",
    "created_at": "2019-08-06T12:16:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28326",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28326#issuecomment-444346",
    "user": "https://github.com/nheir"
}
```

Commit: **[671efa7](https://github.com/sagemath/sagetrac-mirror/commit/671efa739944f9188f5187f14ce5e1656994c765)**



---

archive/issue_comments_444347.json:
```json
{
    "body": "Branch: **[u/hderycke/28326_restore_term](https://github.com/sagemath/sagetrac-mirror/tree/u/hderycke/28326_restore_term)**",
    "created_at": "2019-08-06T12:16:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28326",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28326#issuecomment-444347",
    "user": "https://github.com/nheir"
}
```

Branch: **[u/hderycke/28326_restore_term](https://github.com/sagemath/sagetrac-mirror/tree/u/hderycke/28326_restore_term)**



---

archive/issue_comments_444348.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nwhile this work with `bash` and `zsh` on rxvt-unicode/xterm I expect it to work on any terminal on linux.\nI cannot test it on other OS.",
    "created_at": "2019-08-06T12:22:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28326",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28326#issuecomment-444348",
    "user": "https://github.com/nheir"
}
```

<div id="comment:2" align="right">Comment 2</div>

while this work with `bash` and `zsh` on rxvt-unicode/xterm I expect it to work on any terminal on linux.
I cannot test it on other OS.



---

archive/issue_events_370518.json:
```json
{
    "actor": "https://github.com/nheir",
    "created_at": "2019-08-06T12:23:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28326",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28326#event-370518"
}
```



---

archive/issue_comments_444349.json:
```json
{
    "body": "Reviewer: **Erik Bray**",
    "created_at": "2019-12-16T10:24:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28326",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28326#issuecomment-444349",
    "user": "https://github.com/embray"
}
```

Reviewer: **Erik Bray**



---

archive/issue_events_370519.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-16T10:24:21Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28326",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28326#event-370519"
}
```



---

archive/issue_events_370520.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-16T10:24:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28326",
    "milestone_number": null,
    "milestone_title": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28326#event-370520"
}
```



---

archive/issue_comments_444350.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nMakes sense, thanks.",
    "created_at": "2019-12-16T10:24:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28326",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28326#issuecomment-444350",
    "user": "https://github.com/embray"
}
```

<div id="comment:5" align="right">Comment 5</div>

Makes sense, thanks.



---

archive/issue_events_370521.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-16T10:24:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28326",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28326#event-370521"
}
```



---

archive/issue_events_370522.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-16T10:24:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28326",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28326#event-370522"
}
```



---

archive/issue_events_370523.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-12-16T23:27:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28326",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28326#event-370523"
}
```



---

archive/issue_events_370524.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-12-16T23:27:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28326",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28326#event-370524"
}
```



---

archive/issue_comments_444351.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nAuthor name is missing",
    "created_at": "2019-12-16T23:27:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28326",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28326#issuecomment-444351",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:6" align="right">Comment 6</div>

Author name is missing



---

archive/issue_comments_444352.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nhderycke, is this your correct full name ?",
    "created_at": "2019-12-17T10:00:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28326",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28326#issuecomment-444352",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:7" align="right">Comment 7</div>

hderycke, is this your correct full name ?



---

archive/issue_comments_444353.json:
```json
{
    "body": "Author: **Henri Derycke**",
    "created_at": "2019-12-17T10:00:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28326",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28326#issuecomment-444353",
    "user": "https://github.com/fchapoton"
}
```

Author: **Henri Derycke**



---

archive/issue_events_370525.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-17T12:51:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28326",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28326#event-370525"
}
```



---

archive/issue_events_370526.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-17T12:51:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28326",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28326#event-370526"
}
```



---

archive/issue_events_370527.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-12-20T22:45:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28326",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28326#event-370527"
}
```



---

archive/issue_events_370528.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "b299043fd2bdc8626bb6f89c367255e4af4915c6",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-12-20T22:45:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28326",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28326#event-370528"
}
```



---

archive/issue_comments_444354.json:
```json
{
    "body": "Changed branch from **[u/hderycke/28326_restore_term](https://github.com/sagemath/sagetrac-mirror/tree/u/hderycke/28326_restore_term)** to **[671efa7](https://github.com/sagemath/sagetrac-mirror/commit/671efa739944f9188f5187f14ce5e1656994c765)**",
    "created_at": "2019-12-20T22:45:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28326",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28326#issuecomment-444354",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/hderycke/28326_restore_term](https://github.com/sagemath/sagetrac-mirror/tree/u/hderycke/28326_restore_term)** to **[671efa7](https://github.com/sagemath/sagetrac-mirror/commit/671efa739944f9188f5187f14ce5e1656994c765)**
