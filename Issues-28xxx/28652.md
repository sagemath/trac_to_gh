...
```json
{
    "body": "Apparemment, ca compile pas. Voir les patchbots.",
...
```json
{
    "body": "J'ai du oubli\u00e9 le fichier `sage/groups/perm_gps/constructor.py`.",
...
```json
{
    "body": "failing doctests with py3 and pyflakes warnings, see the latest patchbot report",
...
```json
{
    "body": "D\u00e9sol\u00e9. J'ai fait n'importe quoi avec la branche.",
    "created_at": "2019-10-28T12:51:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28652",
    "type": "issue_comment",
...
```json
{
    "body": "please undo your last change, and add a note besides saying DO NOT REMOVE",
...
```json
{
    "body": "Why? The function call has a side effect. Not the affectation. Doesn't it?",
...
```json
{
    "body": "Well, it seems to have, if I remember correctly.Maybe some kind of reference-keeping, or whatever. This is not the first removal tentative, nad we should have added the warning before.",
...
```json
{
    "body": "Cette fois le patchbot est content!",
...
```json
{
    "body": "`@`Fr\u00e9d\u00e9ric: any further comment?",
...
```json
{
    "body": "An error is not a deprecation message, and this way has the same effect with being more Pythonic. Really that information should be contained in the documentation anyways. Plus, if someone is now getting an error, it means they are explicitly importing it, which is only likely happen in library code.",
...
```json
{
    "body": "I have much cleaner code that get rid of `_set_other`... will come in a minute.",
...
```json
{
    "body": "I agree that initializing from (lib)gap is a bit hacky anyway. I added an example to the documentation of `__init__`.",
...
```json
{
    "body": "LGTM. If the patchbot comes up green, then it is a positive review (including if you get rid of the local imports in `standardize_generator`).",
    "created_at": "2019-11-11T06:16:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28652",
    "type": "issue_comment",
...
```json
{
    "body": "Indeed, I am trying to remove the local imports in `constructor.py`.",
...
```json
{
    "body": "Thank you. Looks good and a very nice additional improvement. Just waiting on a green patchbot before setting to positive.",
...
```json
{
    "body": "failing doctests, see patchbot (the failures related to doc are not relevant)",
...
```json
{
    "body": "The first failure in `cubegroup.py` is serious, the other is trivial. Although the changed error message `invalid permutation to initialize a permutation` in the failing book test is strange; the original message is better IMO.",
...
```json
{
    "body": "Thank you for keeping us informed of the patchbot status Fr\u00e9d\u00e9ric (although I am not sure if you consider yourself as a reviewer; please add yourself if you do).",
    "created_at": "2019-11-11T21:19:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28652",
    "type": "issue_comment",
...
```json
{
    "body": "I am glad that we get this done! Thanks.",
