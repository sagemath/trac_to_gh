# Issue 28243: Accelerate `is_cactus()`

archive/issues_028006.json:
```json
{
    "assignees": [],
    "body": "[This proof](https://www.quora.com/How-do-I-prove-that-for-the-cactus-graph-number-of-edges-is-at-most-floor-1-5*-n-1-n-number-of-vertices-floor-is-rounding-down) could be used to accelerate `is_cactus()` by providing a quick O(1) check that can detect a large class of graphs which are not cactuses. This, in turn, would be helpful for situations where `is_cactus()` is applied to large graphs.\n\nKeywords: **cactus**\n\nBranch/Commit: **[ac1cf88](https://github.com/sagemath/sagetrac-mirror/commit/ac1cf884ade760e719e48a5c44461e7065370c9a)**\n\nReviewer: **David Coudert**\n\nAuthor: **Jo\u00e3o Tavares**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28243_\n\n",
    "closed_at": "2020-02-11T21:06:03Z",
    "created_at": "2019-07-23T21:04:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Accelerate `is_cactus()`",
    "type": "issue",
    "updated_at": "2020-02-11T21:06:03Z",
    "url": "https://github.com/sagemath/sage/issues/28243",
    "user": "https://github.com/r-barnes"
}
```
[This proof](https://www.quora.com/How-do-I-prove-that-for-the-cactus-graph-number-of-edges-is-at-most-floor-1-5*-n-1-n-number-of-vertices-floor-is-rounding-down) could be used to accelerate `is_cactus()` by providing a quick O(1) check that can detect a large class of graphs which are not cactuses. This, in turn, would be helpful for situations where `is_cactus()` is applied to large graphs.

Keywords: **cactus**

Branch/Commit: **[ac1cf88](https://github.com/sagemath/sagetrac-mirror/commit/ac1cf884ade760e719e48a5c44461e7065370c9a)**

Reviewer: **David Coudert**

Author: **João Tavares**

_Issue created by migration from https://trac.sagemath.org/ticket/28243_





---

archive/issue_events_369242.json:
```json
{
    "actor": "https://github.com/r-barnes",
    "created_at": "2019-07-23T21:04:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28243",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28243#event-369242"
}
```



---

archive/issue_events_369243.json:
```json
{
    "actor": "https://github.com/r-barnes",
    "created_at": "2019-07-23T21:04:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28243",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20group%20theory",
    "label_color": "0000ff",
    "label_name": "component: group theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28243#event-369243"
}
```



---

archive/issue_events_369244.json:
```json
{
    "actor": "https://github.com/r-barnes",
    "created_at": "2019-07-23T21:04:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28243",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28243#event-369244"
}
```



---

archive/issue_events_369245.json:
```json
{
    "actor": "https://github.com/r-barnes",
    "created_at": "2019-07-23T21:04:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28243",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28243#event-369245"
}
```



---

archive/issue_comments_443044.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\n(I have not yet found an academic citation for the theorem.)",
    "created_at": "2019-07-23T21:04:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28243",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28243#issuecomment-443044",
    "user": "https://github.com/r-barnes"
}
```

<div id="comment:1" align="right">Comment 1</div>

(I have not yet found an academic citation for the theorem.)



---

archive/issue_events_369246.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-07-24T00:50:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28243",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20group%20theory",
    "label_color": "0000ff",
    "label_name": "component: group theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28243#event-369246"
}
```



---

archive/issue_events_369247.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2019-07-24T00:50:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28243",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28243#event-369247"
}
```



---

archive/issue_comments_443045.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nIf you are not able to double check the proof, it's better to find a reference (article, school book, etc.). This looks like a class exercise...",
    "created_at": "2019-07-24T00:50:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28243",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28243#issuecomment-443045",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:2" align="right">Comment 2</div>

If you are not able to double check the proof, it's better to find a reference (article, school book, etc.). This looks like a class exercise...



---

archive/issue_comments_443046.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nActually, fill free to add the test without reference. This result is well known and often used as an exercise. Furthermore, this is a tight bound and we have graphs reaching it.\n\n```\nsage: for n in range(1, 10):\n....:     G = graphs.FriendshipGraph(n)\n....:     G.size() == 3 * (G.order() - 1) / 2\n....:     \nTrue\nTrue\nTrue\nTrue\nTrue\nTrue\nTrue\nTrue\nTrue\nsage: for _ in range(10):\n....:     G = graphs.RandomBlockGraph(10, 3)\n....:     G.size() == 3 * (G.order() - 1) / 2\n....:     \nTrue\nTrue\nTrue\nTrue\nTrue\nTrue\nTrue\nTrue\nTrue\nTrue\n```",
    "created_at": "2019-07-30T16:23:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28243",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28243#issuecomment-443046",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:3" align="right">Comment 3</div>

Actually, fill free to add the test without reference. This result is well known and often used as an exercise. Furthermore, this is a tight bound and we have graphs reaching it.

```
sage: for n in range(1, 10):
....:     G = graphs.FriendshipGraph(n)
....:     G.size() == 3 * (G.order() - 1) / 2
....:     
True
True
True
True
True
True
True
True
True
sage: for _ in range(10):
....:     G = graphs.RandomBlockGraph(10, 3)
....:     G.size() == 3 * (G.order() - 1) / 2
....:     
True
True
True
True
True
True
True
True
True
True
```



---

archive/issue_events_369248.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28243",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28243#event-369248"
}
```



---

archive/issue_events_369249.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28243",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28243#event-369249"
}
```



---

archive/issue_comments_443047.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nTicket retargeted after milestone closed",
    "created_at": "2019-12-30T14:48:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28243",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28243#issuecomment-443047",
    "user": "https://github.com/embray"
}
```

<div id="comment:4" align="right">Comment 4</div>

Ticket retargeted after milestone closed



---

archive/issue_comments_443048.json:
```json
{
    "body": "Branch: **[u/gh-tabus/accelerate__is_cactus___](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-tabus/accelerate__is_cactus___)**",
    "created_at": "2020-02-07T18:21:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28243",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28243#issuecomment-443048",
    "user": "https://github.com/tabus"
}
```

Branch: **[u/gh-tabus/accelerate__is_cactus___](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-tabus/accelerate__is_cactus___)**



---

archive/issue_events_369250.json:
```json
{
    "actor": "https://github.com/tabus",
    "created_at": "2020-02-07T18:24:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28243",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28243#event-369250"
}
```



---

archive/issue_comments_443049.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ac1cf884ade760e719e48a5c44461e7065370c9a\">ac1cf88</a></td><td><code>Added the necessary condition check</code></td></tr></table>\n",
    "created_at": "2020-02-07T18:24:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28243",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28243#issuecomment-443049",
    "user": "https://github.com/tabus"
}
```

<div id="comment:6" align="right">Comment 6</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ac1cf884ade760e719e48a5c44461e7065370c9a">ac1cf88</a></td><td><code>Added the necessary condition check</code></td></tr></table>




---

archive/issue_comments_443050.json:
```json
{
    "body": "Commit: **[ac1cf88](https://github.com/sagemath/sagetrac-mirror/commit/ac1cf884ade760e719e48a5c44461e7065370c9a)**",
    "created_at": "2020-02-07T18:24:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28243",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28243#issuecomment-443050",
    "user": "https://github.com/tabus"
}
```

Commit: **[ac1cf88](https://github.com/sagemath/sagetrac-mirror/commit/ac1cf884ade760e719e48a5c44461e7065370c9a)**



---

archive/issue_comments_443051.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nPlease add your full name in the field `Authors` of this page.",
    "created_at": "2020-02-08T11:25:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28243",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28243#issuecomment-443051",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:7" align="right">Comment 7</div>

Please add your full name in the field `Authors` of this page.



---

archive/issue_comments_443052.json:
```json
{
    "body": "Reviewer: **David Coudert**",
    "created_at": "2020-02-08T11:25:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28243",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28243#issuecomment-443052",
    "user": "https://github.com/dcoudert"
}
```

Reviewer: **David Coudert**



---

archive/issue_comments_443053.json:
```json
{
    "body": "Author: **Jo\u00e3o Tavares**",
    "created_at": "2020-02-08T23:05:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28243",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28243#issuecomment-443053",
    "user": "https://github.com/tabus"
}
```

Author: **João Tavares**



---

archive/issue_comments_443054.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nReplying to [@tabus](#comment%3A8):\nHere it is. Is there anything else that should be done?",
    "created_at": "2020-02-08T23:11:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28243",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28243#issuecomment-443054",
    "user": "https://github.com/tabus"
}
```

<div id="comment:9" align="right">Comment 9</div>

Replying to [@tabus](#comment%3A8):
Here it is. Is there anything else that should be done?



---

archive/issue_events_369251.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2020-02-09T10:20:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28243",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28243#event-369251"
}
```



---

archive/issue_events_369252.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2020-02-09T10:20:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28243",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28243#event-369252"
}
```



---

archive/issue_comments_443055.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nFor this simple test, this is enough.",
    "created_at": "2020-02-09T10:20:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28243",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28243#issuecomment-443055",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:10" align="right">Comment 10</div>

For this simple test, this is enough.



---

archive/issue_events_369253.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-02-11T21:06:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28243",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28243#event-369253"
}
```



---

archive/issue_events_369254.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "5aac4d5055d5e7959da316a44c59535685f0011b",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-02-11T21:06:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28243",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28243#event-369254"
}
```



---

archive/issue_comments_443056.json:
```json
{
    "body": "Changed branch from **[u/gh-tabus/accelerate__is_cactus___](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-tabus/accelerate__is_cactus___)** to **[ac1cf88](https://github.com/sagemath/sagetrac-mirror/commit/ac1cf884ade760e719e48a5c44461e7065370c9a)**",
    "created_at": "2020-02-11T21:06:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28243",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28243#issuecomment-443056",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/gh-tabus/accelerate__is_cactus___](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-tabus/accelerate__is_cactus___)** to **[ac1cf88](https://github.com/sagemath/sagetrac-mirror/commit/ac1cf884ade760e719e48a5c44461e7065370c9a)**
