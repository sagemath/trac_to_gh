# Issue 28100: Implements function that returns the sum of a number's digits

archive/issues_027863.json:
```json
{
    "assignees": [],
    "body": "This ticket adds a function to return, given a number and base, the sum of digits of that number in that base. For example, this enables:\n\n```\nsage: digit_sum(4412, b=10)\n11\n```\n\n```\nsage: digit_sum(4412, b=6)\n12\n```\n\n\n**Branch:** [u/gh-sheareralexj/implements_function_that_returns_the_sum_of_a_number_s_digits](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-sheareralexj/implements_function_that_returns_the_sum_of_a_number_s_digits)\n\n**Commit:** [a8e7f14fb87ea4ace4105455c01c8d24f3f6790b](https://github.com/sagemath/sagetrac-mirror/commit/a8e7f14fb87ea4ace4105455c01c8d24f3f6790b)\n\n**Author:** Alex Shearer\n\nIssue created by migration from https://trac.sagemath.org/ticket/28100\n\n",
    "created_at": "2019-07-02T20:50:39Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20number%20theory",
        "https://github.com/sagemath/sage/labels/minor",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/needs%20info"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Implements function that returns the sum of a number's digits",
    "type": "issue",
    "updated_at": "2022-09-19T18:58:47Z",
    "url": "https://github.com/sagemath/sage/issues/28100",
    "user": "https://github.com/sheareralexj"
}
```
This ticket adds a function to return, given a number and base, the sum of digits of that number in that base. For example, this enables:

```
sage: digit_sum(4412, b=10)
11
```

```
sage: digit_sum(4412, b=6)
12
```


**Branch:** [u/gh-sheareralexj/implements_function_that_returns_the_sum_of_a_number_s_digits](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-sheareralexj/implements_function_that_returns_the_sum_of_a_number_s_digits)

**Commit:** [a8e7f14fb87ea4ace4105455c01c8d24f3f6790b](https://github.com/sagemath/sagetrac-mirror/commit/a8e7f14fb87ea4ace4105455c01c8d24f3f6790b)

**Author:** Alex Shearer

Issue created by migration from https://trac.sagemath.org/ticket/28100





---

archive/issue_events_337171.json:
```json
{
    "actor": "https://github.com/sheareralexj",
    "created_at": "2019-07-02T20:50:39Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28100#event-337171"
}
```



---

archive/issue_events_337172.json:
```json
{
    "actor": "https://github.com/sheareralexj",
    "created_at": "2019-07-02T20:50:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20theory",
    "label_color": "08517b",
    "label_name": "component: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28100#event-337172"
}
```



---

archive/issue_events_337173.json:
```json
{
    "actor": "https://github.com/sheareralexj",
    "created_at": "2019-07-02T20:50:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "label": "https://github.com/sagemath/sage/labels/minor",
    "label_color": "ff0000",
    "label_name": "minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28100#event-337173"
}
```



---

archive/issue_events_337174.json:
```json
{
    "actor": "https://github.com/sheareralexj",
    "created_at": "2019-07-02T20:50:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "008080",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28100#event-337174"
}
```



---

archive/issue_comments_439950.json:
```json
{
    "body": "**Branch:** [u/gh-sheareralexj/implements_function_that_returns_the_sum_of_a_number_s_digits](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-sheareralexj/implements_function_that_returns_the_sum_of_a_number_s_digits)",
    "created_at": "2019-07-02T22:36:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28100#issuecomment-439950",
    "user": "https://github.com/sheareralexj"
}
```

**Branch:** [u/gh-sheareralexj/implements_function_that_returns_the_sum_of_a_number_s_digits](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-sheareralexj/implements_function_that_returns_the_sum_of_a_number_s_digits)



---

archive/issue_comments_439951.json:
```json
{
    "body": "<a id='comment:2'></a>\n**New commits:**\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a8e7f14fb87ea4ace4105455c01c8d24f3f6790b\">a8e7f14</a></td><td><code>28100: Implements function that returns sum of digits of `n`</code></td></tr></table>\n",
    "created_at": "2019-07-02T22:38:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28100#issuecomment-439951",
    "user": "https://github.com/sheareralexj"
}
```

<a id='comment:2'></a>
**New commits:**
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a8e7f14fb87ea4ace4105455c01c8d24f3f6790b">a8e7f14</a></td><td><code>28100: Implements function that returns sum of digits of `n`</code></td></tr></table>




---

archive/issue_comments_439952.json:
```json
{
    "body": "**Author:** Alex Shearer",
    "created_at": "2019-07-02T22:38:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28100#issuecomment-439952",
    "user": "https://github.com/sheareralexj"
}
```

**Author:** Alex Shearer



---

archive/issue_events_337175.json:
```json
{
    "actor": "https://github.com/sheareralexj",
    "created_at": "2019-07-02T22:38:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "008080",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28100#event-337175"
}
```



---

archive/issue_comments_439953.json:
```json
{
    "body": "**Commit:** [a8e7f14fb87ea4ace4105455c01c8d24f3f6790b](https://github.com/sagemath/sagetrac-mirror/commit/a8e7f14fb87ea4ace4105455c01c8d24f3f6790b)",
    "created_at": "2019-07-02T22:38:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28100#issuecomment-439953",
    "user": "https://github.com/sheareralexj"
}
```

**Commit:** [a8e7f14fb87ea4ace4105455c01c8d24f3f6790b](https://github.com/sagemath/sagetrac-mirror/commit/a8e7f14fb87ea4ace4105455c01c8d24f3f6790b)



---

archive/issue_comments_439954.json:
```json
{
    "body": "<a id='comment:3'></a>\nWhat is the advantage of this function compared to\n\n```\nsage: sum(n.digits(b))\n```\n?",
    "created_at": "2019-07-03T20:40:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28100#issuecomment-439954",
    "user": "https://github.com/videlec"
}
```

<a id='comment:3'></a>
What is the advantage of this function compared to

```
sage: sum(n.digits(b))
```
?



---

archive/issue_comments_439955.json:
```json
{
    "body": "<a id='comment:4'></a>\nReplying to [@videlec](#comment%3A3):\n> What is the advantage of this function compared to\n> \n> ```\n> sage: sum(n.digits(b))\n> ```\n> ?\n\nThere is no real advantage, although we thought it makes sense to have a dedicated sum of digits function implemented.",
    "created_at": "2019-07-03T21:15:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28100#issuecomment-439955",
    "user": "https://github.com/sheareralexj"
}
```

<a id='comment:4'></a>
Replying to [@videlec](#comment%3A3):
> What is the advantage of this function compared to
> 
> ```
> sage: sum(n.digits(b))
> ```
> ?

There is no real advantage, although we thought it makes sense to have a dedicated sum of digits function implemented.



---

archive/issue_comments_439956.json:
```json
{
    "body": "<a id='comment:5'></a>\nWhile we are at it, since `digits` is a method of Sage integers rather than a top-level function, maybe this should be also?\n\nAs an aside, it would have been more Pythonic if once upon a time, `digits` had been written to return an iterator rather than a list. But that would be a painful change to make now.",
    "created_at": "2019-12-19T17:59:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28100#issuecomment-439956",
    "user": "https://github.com/kedlaya"
}
```

<a id='comment:5'></a>
While we are at it, since `digits` is a method of Sage integers rather than a top-level function, maybe this should be also?

As an aside, it would have been more Pythonic if once upon a time, `digits` had been written to return an iterator rather than a list. But that would be a painful change to make now.



---

archive/issue_comments_439957.json:
```json
{
    "body": "<a id='comment:6'></a>\nReplying to [@videlec](#comment%3A3):\n> What is the advantage of this function compared to\n> \n> ```\n> sage: sum(n.digits(b))\n> ```\n> ?\n\nA related question: what are the disadvantages to the proposed function `digit_sum`? I don't know if this function is of enough general interest to warrant being defined at the top level, rather than being a method for integers and/or a function which has to be imported if you want to use it. There have been some efforts to remove underused items from the top-level name space (#25383), and I think there should be a good reason for adding a new one.\n\ntl;dr: why not instead implement `4412.digit_sum(b=10)`?",
    "created_at": "2019-12-20T00:38:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28100#issuecomment-439957",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:6'></a>
Replying to [@videlec](#comment%3A3):
> What is the advantage of this function compared to
> 
> ```
> sage: sum(n.digits(b))
> ```
> ?

A related question: what are the disadvantages to the proposed function `digit_sum`? I don't know if this function is of enough general interest to warrant being defined at the top level, rather than being a method for integers and/or a function which has to be imported if you want to use it. There have been some efforts to remove underused items from the top-level name space (#25383), and I think there should be a good reason for adding a new one.

tl;dr: why not instead implement `4412.digit_sum(b=10)`?



---

archive/issue_events_337176.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28100#event-337176"
}
```



---

archive/issue_events_337177.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28100#event-337177"
}
```



---

archive/issue_comments_439958.json:
```json
{
    "body": "<a id='comment:7'></a>\nTicket retargeted after milestone closed",
    "created_at": "2019-12-30T14:48:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28100#issuecomment-439958",
    "user": "https://github.com/embray"
}
```

<a id='comment:7'></a>
Ticket retargeted after milestone closed



---

archive/issue_comments_439959.json:
```json
{
    "body": "<a id='comment:8'></a>\nReplying to [@jhpalmieri](#comment%3A6):\n> Replying to [@videlec](#comment%3A3):\n> > What is the advantage of this function compared to\n> > \n> > ```\n> > sage: sum(n.digits(b))\n> > ```\n> > ?\n\n> \n> A related question: what are the disadvantages to the proposed function `digit_sum`? I don't know if this function is of enough general interest to warrant being defined at the top level, rather than being a method for integers and/or a function which has to be imported if you want to use it. There have been some efforts to remove underused items from the top-level name space (#25383), and I think there should be a good reason for adding a new one.\n> \n> tl;dr: why not instead implement `4412.digit_sum(b=10)`?\n\n+1",
    "created_at": "2020-02-14T21:33:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28100#issuecomment-439959",
    "user": "https://github.com/jplab"
}
```

<a id='comment:8'></a>
Replying to [@jhpalmieri](#comment%3A6):
> Replying to [@videlec](#comment%3A3):
> > What is the advantage of this function compared to
> > 
> > ```
> > sage: sum(n.digits(b))
> > ```
> > ?

> 
> A related question: what are the disadvantages to the proposed function `digit_sum`? I don't know if this function is of enough general interest to warrant being defined at the top level, rather than being a method for integers and/or a function which has to be imported if you want to use it. There have been some efforts to remove underused items from the top-level name space (#25383), and I think there should be a good reason for adding a new one.
> 
> tl;dr: why not instead implement `4412.digit_sum(b=10)`?

+1



---

archive/issue_comments_439960.json:
```json
{
    "body": "<a id='comment:9'></a>\nUnless the `digit_sum` provides a much faster way to perform the computation than `sum(n.digits(b=10))`, I don't see why we should add such method to integers. I don't think a method is more convenient. `sum` and `prod` are very much standard. If some people think they are not, then one can add an extra example to the documentation of `Integer.digits`.\n\nUsers would better learn a bit of Python rather being lost in an endless Sage namespace (that applies to the main namespace but also to objects).",
    "created_at": "2020-02-16T09:58:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28100#issuecomment-439960",
    "user": "https://github.com/videlec"
}
```

<a id='comment:9'></a>
Unless the `digit_sum` provides a much faster way to perform the computation than `sum(n.digits(b=10))`, I don't see why we should add such method to integers. I don't think a method is more convenient. `sum` and `prod` are very much standard. If some people think they are not, then one can add an extra example to the documentation of `Integer.digits`.

Users would better learn a bit of Python rather being lost in an endless Sage namespace (that applies to the main namespace but also to objects).



---

archive/issue_events_337178.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2020-02-16T09:58:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "008080",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28100#event-337178"
}
```



---

archive/issue_events_337179.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2020-02-16T09:58:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "008080",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28100#event-337179"
}
```



---

archive/issue_comments_439961.json:
```json
{
    "body": "<a id='comment:10'></a>\nAs I commented earlier, if `n.digits` had been written in the first place to return an iterable rather than a list, then there would be no speed differential.\n\nBut maybe a better solution would to be modify `n.digits` to give it an optional argument to return that iterable. Then `sum(n.digits(b=10,iterable=True))` would be as fast as anything else.\n\nAnd honestly, I think the use case for which this makes a difference is quite narrow.",
    "created_at": "2020-02-16T14:56:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28100#issuecomment-439961",
    "user": "https://github.com/kedlaya"
}
```

<a id='comment:10'></a>
As I commented earlier, if `n.digits` had been written in the first place to return an iterable rather than a list, then there would be no speed differential.

But maybe a better solution would to be modify `n.digits` to give it an optional argument to return that iterable. Then `sum(n.digits(b=10,iterable=True))` would be as fast as anything else.

And honestly, I think the use case for which this makes a difference is quite narrow.



---

archive/issue_events_337180.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "milestone_number": null,
    "milestone_title": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28100#event-337180"
}
```



---

archive/issue_events_337181.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28100#event-337181"
}
```



---

archive/issue_comments_439962.json:
```json
{
    "body": "<a id='comment:11'></a>\nBatch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.",
    "created_at": "2020-04-14T19:41:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28100#issuecomment-439962",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:11'></a>
Batch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.



---

archive/issue_events_337182.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-05T19:21:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28100#event-337182"
}
```



---

archive/issue_events_337183.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-05T19:21:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28100#event-337183"
}
```



---

archive/issue_events_337184.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-15T22:07:04Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28100#event-337184"
}
```



---

archive/issue_events_337185.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-03-15T22:07:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28100#event-337185"
}
```



---

archive/issue_comments_439963.json:
```json
{
    "body": "<a id='comment:13'></a>\nSetting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-03-15T22:07:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28100#issuecomment-439963",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:13'></a>
Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_comments_439964.json:
```json
{
    "body": "<a id='comment:14'></a>\nSetting a new milestone for this ticket based on a cursory review.",
    "created_at": "2021-07-19T00:44:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28100#issuecomment-439964",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:14'></a>
Setting a new milestone for this ticket based on a cursory review.



---

archive/issue_events_337186.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T00:44:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28100#event-337186"
}
```



---

archive/issue_events_337187.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T00:44:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28100#event-337187"
}
```



---

archive/issue_comments_439965.json:
```json
{
    "body": "<a id='comment:15'></a>\nStalled in `needs_review` or `needs_info`; likely won't make it into Sage 9.5.",
    "created_at": "2021-12-18T19:53:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28100#issuecomment-439965",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:15'></a>
Stalled in `needs_review` or `needs_info`; likely won't make it into Sage 9.5.



---

archive/issue_events_337188.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:53:12Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28100#event-337188"
}
```



---

archive/issue_events_337189.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:53:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28100#event-337189"
}
```



---

archive/issue_events_337190.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-02T17:54:54Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28100#event-337190"
}
```



---

archive/issue_events_337191.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-02T17:54:54Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28100#event-337191"
}
```



---

archive/issue_events_337192.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28100#event-337192"
}
```



---

archive/issue_events_337193.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28100",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28100#event-337193"
}
```
