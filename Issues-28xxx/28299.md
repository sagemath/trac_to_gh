# Issue 28299: Change default values of parameters loops and multiedges

archive/issues_028062.json:
```json
{
    "body": "After the removal of deprecation warning #15706 in #28290, it is time to change the default values of parameters `loops` and `multi edges` in the (Di)Graph constructors.\n\nAuthor: David Coudert\n\nResolution: invalid\n\nIssue created by migration from https://trac.sagemath.org/ticket/28299\n\n",
    "closed_at": "2020-04-15T19:32:50Z",
    "created_at": "2019-07-31T08:05:08Z",
    "labels": [
        "component: graph theory"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "Change default values of parameters loops and multiedges",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/28299",
    "user": "https://github.com/dcoudert"
}
```
After the removal of deprecation warning #15706 in #28290, it is time to change the default values of parameters `loops` and `multi edges` in the (Di)Graph constructors.

Author: David Coudert

Resolution: invalid

Issue created by migration from https://trac.sagemath.org/ticket/28299





---

archive/issue_comments_396177.json:
```json
{
    "body": "<a id='comment:1'></a>Ticket retargeted after milestone closed",
    "created_at": "2019-12-30T14:48:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28299",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28299#issuecomment-396177",
    "user": "https://github.com/embray"
}
```

<a id='comment:1'></a>Ticket retargeted after milestone closed



---

archive/issue_events_070641.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28299",
    "milestone": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28299#event-70641"
}
```



---

archive/issue_events_070642.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28299",
    "milestone": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28299#event-70642"
}
```



---

archive/issue_events_070643.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-04-14T19:41:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28299",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28299#event-70643"
}
```



---

archive/issue_comments_396178.json:
```json
{
    "body": "<a id='comment:2'></a>Batch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.",
    "created_at": "2020-04-14T19:41:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28299",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28299#issuecomment-396178",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:2'></a>Batch modifying tickets that will likely not be ready for 9.1, based on a review of the ticket title, branch/review status, and last modification date.



---

archive/issue_comments_396179.json:
```json
{
    "body": "<a id='comment:3'></a>Hello,\n\nAfter looking at this ticket in detail I am unsure of the desired changes.\nWhen the parameters `loops` and `multiedges` have their default value (`None`), graphs built from an integer or a list of edges are already loopless and simple. From `graphs.py`:\n\n```python\n        elif format == 'int':\n            self.allow_loops(bool(loops), check=False)\n            self.allow_multiple_edges(bool(multiedges), check=False)\n            ...\n```\nand\n\n```python\n        elif format == 'list_of_edges':\n            self.allow_multiple_edges(bool(multiedges), check=False)\n            self.allow_loops(bool(loops), check=False)\n            ...\n```\n\n(Similar for digraphs.) It was changed in #15706 when removing the deprecation warning.\n\nHowever having `loops=False` and `multiedges=False` by default seems dangerous because it will break the detection of the parameters when the graph is given, for instance, as an adjacency matrix:\n\n```\nsage: Graph(matrix([[0,2],[2,0]]))\nMulti-graph on 2 vertices\nsage: Graph(matrix([[1,2],[2,1]]))\nLooped multi-graph on 2 vertices\nsage: Graph(matrix([[1,0],[0,1]]))\nLooped graph on 2 vertices\n```\nBut\n\n```\nsage: Graph(matrix([[1,0],[0,1]]), loops=False)\nTraceback (most recent call last):\nValueError: the adjacency matrix of a loopless graph must have zeroes on the diagonal\n```\nSimilarily, when ones creates a graph from a sparse6 string, they have no obvious way to guess whether the graph has mutliedges/loops or not. (Currently loops and multiedges are allowed by default in this case.)",
    "created_at": "2020-04-15T12:23:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28299",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28299#issuecomment-396179",
    "user": "https://github.com/jfraymond"
}
```

<a id='comment:3'></a>Hello,

After looking at this ticket in detail I am unsure of the desired changes.
When the parameters `loops` and `multiedges` have their default value (`None`), graphs built from an integer or a list of edges are already loopless and simple. From `graphs.py`:

```python
        elif format == 'int':
            self.allow_loops(bool(loops), check=False)
            self.allow_multiple_edges(bool(multiedges), check=False)
            ...
```
and

```python
        elif format == 'list_of_edges':
            self.allow_multiple_edges(bool(multiedges), check=False)
            self.allow_loops(bool(loops), check=False)
            ...
```

(Similar for digraphs.) It was changed in #15706 when removing the deprecation warning.

However having `loops=False` and `multiedges=False` by default seems dangerous because it will break the detection of the parameters when the graph is given, for instance, as an adjacency matrix:

```
sage: Graph(matrix([[0,2],[2,0]]))
Multi-graph on 2 vertices
sage: Graph(matrix([[1,2],[2,1]]))
Looped multi-graph on 2 vertices
sage: Graph(matrix([[1,0],[0,1]]))
Looped graph on 2 vertices
```
But

```
sage: Graph(matrix([[1,0],[0,1]]), loops=False)
Traceback (most recent call last):
ValueError: the adjacency matrix of a loopless graph must have zeroes on the diagonal
```
Similarily, when ones creates a graph from a sparse6 string, they have no obvious way to guess whether the graph has mutliedges/loops or not. (Currently loops and multiedges are allowed by default in this case.)



---

archive/issue_comments_396180.json:
```json
{
    "body": "<a id='comment:4'></a>You are perfectly right, I did not though about these cases. Thanks.\n\nSo may be we can set this ticket to invalid.",
    "created_at": "2020-04-15T16:49:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28299",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28299#issuecomment-396180",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:4'></a>You are perfectly right, I did not though about these cases. Thanks.

So may be we can set this ticket to invalid.



---

archive/issue_events_070644.json:
```json
{
    "actor": "https://github.com/jfraymond",
    "created_at": "2020-04-15T19:30:24Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28299",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28299#event-70644"
}
```



---

archive/issue_events_070645.json:
```json
{
    "actor": "https://github.com/jfraymond",
    "created_at": "2020-04-15T19:30:24Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/28299",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28299#event-70645"
}
```



---

archive/issue_events_070646.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-04-15T19:32:50Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/28299",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28299#event-70646"
}
```



---

archive/issue_comments_396181.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2020-04-15T19:32:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28299",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28299#issuecomment-396181",
    "user": "https://github.com/fchapoton"
}
```

Resolution: invalid
