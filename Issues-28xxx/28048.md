# Issue 28048: fix gap workspace name on Python3

archive/issues_027811.json:
```json
{
    "assignees": [],
    "body": "hash of strings on Python3 depends on the session. As a consequence, when used with Python3 a gap workspace is created at each session and populates `DOT_SAGE/.gap` with a new workspace at each session. The ticket fixes the name by using `hashlib` instead.\n\nCC:  @fchapoton @isuruf\n\nKeywords: **python3 gap**\n\nBranch/Commit: **[fb5d367](https://github.com/sagemath/sagetrac-mirror/commit/fb5d367cc9acdb867af0b7657e50f2030f7cf1d3)**\n\nReviewer: **Vincent Klein**\n\nAuthor: **Vincent Delecroix**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28048_\n\n",
    "closed_at": "2019-07-04T11:09:35Z",
    "created_at": "2019-06-23T18:06:49Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix gap workspace name on Python3",
    "type": "issue",
    "updated_at": "2019-07-04T11:09:35Z",
    "url": "https://github.com/sagemath/sage/issues/28048",
    "user": "https://github.com/videlec"
}
```
hash of strings on Python3 depends on the session. As a consequence, when used with Python3 a gap workspace is created at each session and populates `DOT_SAGE/.gap` with a new workspace at each session. The ticket fixes the name by using `hashlib` instead.

CC:  @fchapoton @isuruf

Keywords: **python3 gap**

Branch/Commit: **[fb5d367](https://github.com/sagemath/sagetrac-mirror/commit/fb5d367cc9acdb867af0b7657e50f2030f7cf1d3)**

Reviewer: **Vincent Klein**

Author: **Vincent Delecroix**

_Issue created by migration from https://trac.sagemath.org/ticket/28048_





---

archive/issue_events_366393.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-06-23T18:06:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28048",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28048#event-366393"
}
```



---

archive/issue_events_366394.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-06-23T18:06:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28048",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "component: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28048#event-366394"
}
```



---

archive/issue_events_366395.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-06-23T18:06:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28048",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28048#event-366395"
}
```



---

archive/issue_events_366396.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-06-23T18:06:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28048",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28048#event-366396"
}
```



---

archive/issue_events_366397.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-06-23T18:15:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28048",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28048#event-366397"
}
```



---

archive/issue_comments_439308.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fb5d367cc9acdb867af0b7657e50f2030f7cf1d3\">fb5d367</a></td><td><code>28048: fix gap workspace name</code></td></tr></table>\n",
    "created_at": "2019-06-23T18:15:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28048",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28048#issuecomment-439308",
    "user": "https://github.com/videlec"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fb5d367cc9acdb867af0b7657e50f2030f7cf1d3">fb5d367</a></td><td><code>28048: fix gap workspace name</code></td></tr></table>




---

archive/issue_comments_439309.json:
```json
{
    "body": "Commit: **[fb5d367](https://github.com/sagemath/sagetrac-mirror/commit/fb5d367cc9acdb867af0b7657e50f2030f7cf1d3)**",
    "created_at": "2019-06-23T18:15:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28048",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28048#issuecomment-439309",
    "user": "https://github.com/videlec"
}
```

Commit: **[fb5d367](https://github.com/sagemath/sagetrac-mirror/commit/fb5d367cc9acdb867af0b7657e50f2030f7cf1d3)**



---

archive/issue_comments_439310.json:
```json
{
    "body": "Branch: **[u/vdelecroix/28048](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/28048)**",
    "created_at": "2019-06-23T18:15:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28048",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28048#issuecomment-439310",
    "user": "https://github.com/videlec"
}
```

Branch: **[u/vdelecroix/28048](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/28048)**



---

archive/issue_events_366398.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-06-23T18:17:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28048",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28048#event-366398"
}
```



---

archive/issue_events_366399.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2019-06-23T18:17:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28048",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28048#event-366399"
}
```



---

archive/issue_comments_439311.json:
```json
{
    "body": "Reviewer: **Vincent Klein**",
    "created_at": "2019-07-03T08:44:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28048",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28048#issuecomment-439311",
    "user": "https://github.com/vinklein"
}
```

Reviewer: **Vincent Klein**



---

archive/issue_events_366400.json:
```json
{
    "actor": "https://github.com/vinklein",
    "created_at": "2019-07-03T08:44:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28048",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28048#event-366400"
}
```



---

archive/issue_events_366401.json:
```json
{
    "actor": "https://github.com/vinklein",
    "created_at": "2019-07-03T08:44:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28048",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28048#event-366401"
}
```



---

archive/issue_comments_439312.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLooks good to me !",
    "created_at": "2019-07-03T08:44:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28048",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28048#issuecomment-439312",
    "user": "https://github.com/vinklein"
}
```

<div id="comment:3" align="right">Comment 3</div>

Looks good to me !



---

archive/issue_events_366402.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-07-04T11:09:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28048",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28048#event-366402"
}
```



---

archive/issue_events_366403.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "574c235be04df6f1d1891a58687a3fe93dd49827",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-07-04T11:09:35Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28048",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28048#event-366403"
}
```



---

archive/issue_comments_439313.json:
```json
{
    "body": "Changed branch from **[u/vdelecroix/28048](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/28048)** to **[fb5d367](https://github.com/sagemath/sagetrac-mirror/commit/fb5d367cc9acdb867af0b7657e50f2030f7cf1d3)**",
    "created_at": "2019-07-04T11:09:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28048",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28048#issuecomment-439313",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/vdelecroix/28048](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/28048)** to **[fb5d367](https://github.com/sagemath/sagetrac-mirror/commit/fb5d367cc9acdb867af0b7657e50f2030f7cf1d3)**
