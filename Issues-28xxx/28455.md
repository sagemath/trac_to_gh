# Issue 28455: 1 doctest failing in src/sage/databases/oeis.py with tag internet

archive/issues_028218.json:
```json
{
    "body": "With sage-8.9.beta9, the command \n\n```\nsage -t --optional=sage,internet src/sage/databases/oeis.py\n```\n\ngives\n\n```\nsage -t --long src/sage/databases/oeis.py\n**********************************************************************\nFile \"src/sage/databases/oeis.py\", line 90, in sage.databases.oeis\nFailed example:\n    p.cross_references(fetch=True)                # optional -- internet\nExpected:\n    0: A000798: Number of different quasi-orders (or topologies, or transitive digraphs) with n labeled elements.\n    1: A001035: Number of partially ordered sets (\"posets\") with n labeled elements (or labeled acyclic transitive digraphs).\n    2: A001930: Number of topologies, or transitive digraphs with n unlabeled nodes.\n    3: A006057: Number of topologies on n labeled points satisfying axioms T_0-T_4.\n    4: A079263: Number of constrained mixed models with n factors.\n    5: A079265: Number of antisymmetric transitive binary relations on n unlabeled points.\n    6: A263859: Triangle read by rows: T(n,k) (n>=1, k>=0) is the number of posets with n elements and rank k (or depth k+1).\nGot:\n     0: A000798: Number of different quasi-orders (or topologies, or transitive digraphs) with n labeled elements.\n     1: A001035: Number of partially ordered sets (\"posets\") with n labeled elements (or labeled acyclic transitive digraphs).\n     2: A001930: Number of topologies, or transitive digraphs with n unlabeled nodes.\n     3: A006057: Number of topologies on n labeled points satisfying axioms T_0-T_4.\n     4: A079263: Number of constrained mixed models with n factors.\n     5: A079265: Number of antisymmetric transitive binary relations on n unlabeled points.\n     6: A263859: Triangle read by rows: T(n,k) (n>=1, k>=0) is the number of posets with n elements and rank k (or depth k+1).\n     7: A316978: Number of factorizations of n into factors > 1 with no equivalent primes.\n     8: A319559: Number of non-isomorphic T_0 set systems of weight n.\n     9: A326939: Number of T_0 sets of subsets of {1..n} that cover all n vertices.\n    10: A326943: Number of T_0 sets of subsets of {1..n} that cover all n vertices and are closed under intersection.\n    11: A326944: Number of T_0 sets of subsets of {1..n} that cover all n vertices, contain {}, and are closed under intersection.\n    12: A326947: BII-numbers of T_0 set-systems.\n\n**********************************************************************\n1 item had failures:\n   1 of  26 in sage.databases.oeis\n    5 webbrowser tests not run\n    0 tests not run because we ran out of time\n    [266 tests, 1 failure, 62.87 s]\n----------------------------------------------------------------------\nsage -t src/sage/databases/oeis.py  # 1 doctest failed\n----------------------------------------------------------------------\n```\n\nBranch/Commit: 37b97ce9b9122a72a397f548404d40967b0bd27f\n\nReviewer: S\u00e9bastien Labb\u00e9\n\nAuthor: Thierry Monteil\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/28455\n\n",
    "closed_at": "2019-09-09T21:28:47Z",
    "created_at": "2019-09-05T08:21:42Z",
    "labels": [
        "component: doctest coverage",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.9",
    "title": "1 doctest failing in src/sage/databases/oeis.py with tag internet",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/28455",
    "user": "https://github.com/seblabbe"
}
```
With sage-8.9.beta9, the command 

```
sage -t --optional=sage,internet src/sage/databases/oeis.py
```

gives

```
sage -t --long src/sage/databases/oeis.py
**********************************************************************
File "src/sage/databases/oeis.py", line 90, in sage.databases.oeis
Failed example:
    p.cross_references(fetch=True)                # optional -- internet
Expected:
    0: A000798: Number of different quasi-orders (or topologies, or transitive digraphs) with n labeled elements.
    1: A001035: Number of partially ordered sets ("posets") with n labeled elements (or labeled acyclic transitive digraphs).
    2: A001930: Number of topologies, or transitive digraphs with n unlabeled nodes.
    3: A006057: Number of topologies on n labeled points satisfying axioms T_0-T_4.
    4: A079263: Number of constrained mixed models with n factors.
    5: A079265: Number of antisymmetric transitive binary relations on n unlabeled points.
    6: A263859: Triangle read by rows: T(n,k) (n>=1, k>=0) is the number of posets with n elements and rank k (or depth k+1).
Got:
     0: A000798: Number of different quasi-orders (or topologies, or transitive digraphs) with n labeled elements.
     1: A001035: Number of partially ordered sets ("posets") with n labeled elements (or labeled acyclic transitive digraphs).
     2: A001930: Number of topologies, or transitive digraphs with n unlabeled nodes.
     3: A006057: Number of topologies on n labeled points satisfying axioms T_0-T_4.
     4: A079263: Number of constrained mixed models with n factors.
     5: A079265: Number of antisymmetric transitive binary relations on n unlabeled points.
     6: A263859: Triangle read by rows: T(n,k) (n>=1, k>=0) is the number of posets with n elements and rank k (or depth k+1).
     7: A316978: Number of factorizations of n into factors > 1 with no equivalent primes.
     8: A319559: Number of non-isomorphic T_0 set systems of weight n.
     9: A326939: Number of T_0 sets of subsets of {1..n} that cover all n vertices.
    10: A326943: Number of T_0 sets of subsets of {1..n} that cover all n vertices and are closed under intersection.
    11: A326944: Number of T_0 sets of subsets of {1..n} that cover all n vertices, contain {}, and are closed under intersection.
    12: A326947: BII-numbers of T_0 set-systems.

**********************************************************************
1 item had failures:
   1 of  26 in sage.databases.oeis
    5 webbrowser tests not run
    0 tests not run because we ran out of time
    [266 tests, 1 failure, 62.87 s]
----------------------------------------------------------------------
sage -t src/sage/databases/oeis.py  # 1 doctest failed
----------------------------------------------------------------------
```

Branch/Commit: 37b97ce9b9122a72a397f548404d40967b0bd27f

Reviewer: Sébastien Labbé

Author: Thierry Monteil

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/28455





---

archive/issue_comments_551821.json:
```json
{
    "body": "Changing branch from \"\" to \"u/tmonteil/1_doctest_failing_in_src_sage_databases_oeis_py_with_tag_internet\"",
    "created_at": "2019-09-05T09:17:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28455#issuecomment-551821",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```

Changing branch from "" to "u/tmonteil/1_doctest_failing_in_src_sage_databases_oeis_py_with_tag_internet"



---

archive/issue_comments_551822.json:
```json
{
    "body": "<a id='comment:2'></a>Since the number of cross-references will keep increasing (which is good), i added some dots to avoid useless updates. With 11 references, there is more than enough to show the feature.\n\n---\nNew commits:",
    "created_at": "2019-09-05T09:20:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28455#issuecomment-551822",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```

<a id='comment:2'></a>Since the number of cross-references will keep increasing (which is good), i added some dots to avoid useless updates. With 11 references, there is more than enough to show the feature.

---
New commits:



---

archive/issue_comments_551823.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2019-09-05T09:20:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28455#issuecomment-551823",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_551824.json:
```json
{
    "body": "Changing author from \"\" to \"Thierry Monteil\"",
    "created_at": "2019-09-05T09:20:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28455#issuecomment-551824",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```

Changing author from "" to "Thierry Monteil"



---

archive/issue_comments_551825.json:
```json
{
    "body": "Changing commit from \"\" to \"37b97ce9b9122a72a397f548404d40967b0bd27f\"",
    "created_at": "2019-09-05T09:20:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28455#issuecomment-551825",
    "user": "https://trac.sagemath.org/admin/accounts/users/tmonteil"
}
```

Changing commit from "" to "37b97ce9b9122a72a397f548404d40967b0bd27f"



---

archive/issue_comments_551826.json:
```json
{
    "body": "<a id='comment:3'></a>keeping from 0 to 6 would have been enough I think, but let's go with 11.",
    "created_at": "2019-09-06T07:46:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28455#issuecomment-551826",
    "user": "https://github.com/seblabbe"
}
```

<a id='comment:3'></a>keeping from 0 to 6 would have been enough I think, but let's go with 11.



---

archive/issue_comments_551827.json:
```json
{
    "body": "Changing reviewer from \"\" to \"S\u00e9bastien Labb\u00e9\"",
    "created_at": "2019-09-06T07:46:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28455#issuecomment-551827",
    "user": "https://github.com/seblabbe"
}
```

Changing reviewer from "" to "Sébastien Labbé"



---

archive/issue_comments_551828.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2019-09-06T07:46:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28455#issuecomment-551828",
    "user": "https://github.com/seblabbe"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_551829.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2019-09-09T21:28:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28455#issuecomment-551829",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_events_071283.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-09-09T21:28:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/28455",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/28455#event-71283"
}
```



---

archive/issue_comments_551830.json:
```json
{
    "body": "Changing branch from \"u/tmonteil/1_doctest_failing_in_src_sage_databases_oeis_py_with_tag_internet\" to \"37b97ce9b9122a72a397f548404d40967b0bd27f\"",
    "created_at": "2019-09-09T21:28:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/28455",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/28455#issuecomment-551830",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/tmonteil/1_doctest_failing_in_src_sage_databases_oeis_py_with_tag_internet" to "37b97ce9b9122a72a397f548404d40967b0bd27f"
