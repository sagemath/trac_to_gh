# Issue 28233: fix doctest in gap_packages when ~/.gap is not empty

archive/issues_027996.json:
```json
{
    "assignees": [],
    "body": "One of the doctest in the file `sage/tests/gap_packages.py` has a result that can depend on the content of the testing user's home directory. Namely if `~/.gap` exist and it is not empty\n\n```\nsage -t --long /usr/lib64/python2.7/site-packages/sage/tests/gap_packages.py\n**********************************************************************\nFile \"/usr/lib64/python2.7/site-packages/sage/tests/gap_packages.py\", line 137, in sage.tests.gap_packages.all_installed_packages\nFailed example:\n    all_installed_packages() == all_installed_packages(gap=gap)\nExpected:\n    True\nGot:\n    False\n```\nThis can be fixed by telling the test to ignore the content of `~/.gap`.\n\n\nCC:  @strogdon\n\nBranch/Commit: **[25d55f9](https://github.com/sagemath/sagetrac-mirror/commit/25d55f928fb7b4ddf1ba9c7ad4fd88dbfc97457a)**\n\nReviewer: **Steven Trogdon**\n\nAuthor: **Fran\u00e7ois Bissey**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28233_\n\n",
    "closed_at": "2019-07-29T21:54:45Z",
    "created_at": "2019-07-23T02:10:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20distribution",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix doctest in gap_packages when ~/.gap is not empty",
    "type": "issue",
    "updated_at": "2019-07-29T21:54:45Z",
    "url": "https://github.com/sagemath/sage/issues/28233",
    "user": "https://github.com/kiwifb"
}
```
One of the doctest in the file `sage/tests/gap_packages.py` has a result that can depend on the content of the testing user's home directory. Namely if `~/.gap` exist and it is not empty

```
sage -t --long /usr/lib64/python2.7/site-packages/sage/tests/gap_packages.py
**********************************************************************
File "/usr/lib64/python2.7/site-packages/sage/tests/gap_packages.py", line 137, in sage.tests.gap_packages.all_installed_packages
Failed example:
    all_installed_packages() == all_installed_packages(gap=gap)
Expected:
    True
Got:
    False
```
This can be fixed by telling the test to ignore the content of `~/.gap`.


CC:  @strogdon

Branch/Commit: **[25d55f9](https://github.com/sagemath/sagetrac-mirror/commit/25d55f928fb7b4ddf1ba9c7ad4fd88dbfc97457a)**

Reviewer: **Steven Trogdon**

Author: **François Bissey**

_Issue created by migration from https://trac.sagemath.org/ticket/28233_





---

archive/issue_events_369071.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2019-07-23T02:10:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28233",
    "milestone_number": null,
    "milestone_title": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28233#event-369071"
}
```



---

archive/issue_events_369072.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2019-07-23T02:10:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28233",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20distribution",
    "label_color": "000080",
    "label_name": "component: distribution",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28233#event-369072"
}
```



---

archive/issue_events_369073.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2019-07-23T02:10:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28233",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28233#event-369073"
}
```



---

archive/issue_events_369074.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2019-07-23T02:10:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28233",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28233#event-369074"
}
```



---

archive/issue_comments_442758.json:
```json
{
    "body": "Author: **Fran\u00e7ois Bissey**",
    "created_at": "2019-07-23T02:19:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28233",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28233#issuecomment-442758",
    "user": "https://github.com/kiwifb"
}
```

Author: **François Bissey**



---

archive/issue_events_369075.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2019-07-23T02:19:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28233",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28233#event-369075"
}
```



---

archive/issue_comments_442759.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nReminder: this happens because by default `libgap` always look at the content of `~/.gap` but the pexpect interface to `gap` doesn't. This ticket explicitly discards the content of `~/.gap` in all cases.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/25d55f928fb7b4ddf1ba9c7ad4fd88dbfc97457a\">25d55f9</a></td><td><code>Use \"ignore_dot_gap\" in the all_installed_packages doctest for consistency.</code></td></tr></table>\n",
    "created_at": "2019-07-23T02:19:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28233",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28233#issuecomment-442759",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:1" align="right">Comment 1</div>

Reminder: this happens because by default `libgap` always look at the content of `~/.gap` but the pexpect interface to `gap` doesn't. This ticket explicitly discards the content of `~/.gap` in all cases.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/25d55f928fb7b4ddf1ba9c7ad4fd88dbfc97457a">25d55f9</a></td><td><code>Use "ignore_dot_gap" in the all_installed_packages doctest for consistency.</code></td></tr></table>




---

archive/issue_comments_442760.json:
```json
{
    "body": "Branch: **[u/fbissey/gap_packages_doctest](https://github.com/sagemath/sagetrac-mirror/tree/u/fbissey/gap_packages_doctest)**",
    "created_at": "2019-07-23T02:19:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28233",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28233#issuecomment-442760",
    "user": "https://github.com/kiwifb"
}
```

Branch: **[u/fbissey/gap_packages_doctest](https://github.com/sagemath/sagetrac-mirror/tree/u/fbissey/gap_packages_doctest)**



---

archive/issue_comments_442761.json:
```json
{
    "body": "Commit: **[25d55f9](https://github.com/sagemath/sagetrac-mirror/commit/25d55f928fb7b4ddf1ba9c7ad4fd88dbfc97457a)**",
    "created_at": "2019-07-23T02:19:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28233",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28233#issuecomment-442761",
    "user": "https://github.com/kiwifb"
}
```

Commit: **[25d55f9](https://github.com/sagemath/sagetrac-mirror/commit/25d55f928fb7b4ddf1ba9c7ad4fd88dbfc97457a)**



---

archive/issue_events_369076.json:
```json
{
    "actor": "https://github.com/strogdon",
    "created_at": "2019-07-23T02:54:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28233",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28233#event-369076"
}
```



---

archive/issue_events_369077.json:
```json
{
    "actor": "https://github.com/strogdon",
    "created_at": "2019-07-23T02:54:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28233",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28233#event-369077"
}
```



---

archive/issue_comments_442762.json:
```json
{
    "body": "Reviewer: **Steven Trogdon**",
    "created_at": "2019-07-23T02:54:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28233",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28233#issuecomment-442762",
    "user": "https://github.com/strogdon"
}
```

Reviewer: **Steven Trogdon**



---

archive/issue_comments_442763.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nI think given the status of `gap -r` that this is the best thing to do. It, of course, does work. Should the description doctest be relative to vanilla Sage? If so, I can change this if you do not have access to vanilla.",
    "created_at": "2019-07-23T02:54:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28233",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28233#issuecomment-442763",
    "user": "https://github.com/strogdon"
}
```

<div id="comment:2" align="right">Comment 2</div>

I think given the status of `gap -r` that this is the best thing to do. It, of course, does work. Should the description doctest be relative to vanilla Sage? If so, I can change this if you do not have access to vanilla.



---

archive/issue_comments_442764.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNo it doesn't matter. Leave it be.",
    "created_at": "2019-07-23T04:43:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28233",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28233#issuecomment-442764",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:3" align="right">Comment 3</div>

No it doesn't matter. Leave it be.



---

archive/issue_comments_442765.json:
```json
{
    "body": "Changed branch from **[u/fbissey/gap_packages_doctest](https://github.com/sagemath/sagetrac-mirror/tree/u/fbissey/gap_packages_doctest)** to **[25d55f9](https://github.com/sagemath/sagetrac-mirror/commit/25d55f928fb7b4ddf1ba9c7ad4fd88dbfc97457a)**",
    "created_at": "2019-07-29T21:54:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28233",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28233#issuecomment-442765",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/fbissey/gap_packages_doctest](https://github.com/sagemath/sagetrac-mirror/tree/u/fbissey/gap_packages_doctest)** to **[25d55f9](https://github.com/sagemath/sagetrac-mirror/commit/25d55f928fb7b4ddf1ba9c7ad4fd88dbfc97457a)**



---

archive/issue_events_369078.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-07-29T21:54:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28233",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28233#event-369078"
}
```



---

archive/issue_events_369079.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "504977080bc70d4deb857efc65f40778d3dd2a94",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-07-29T21:54:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28233",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28233#event-369079"
}
```
