# Issue 28736: fix Texture for negative inputs

archive/issues_028499.json:
```json
{
    "assignees": [],
    "body": "problem visible here:\n\n```\nsage: v = [(sin(i),cos(i),i) for i in [-4,-3.5,..4]]\nsage: S = sum(sphere(v[i], size=1/2, color=((i-4)/8, 1/2,(4-i)/8)) for i in range(len(v)))\nsage: S.show(aspect_ratio=[1,1,1], viewer='threejs')\n```\nwhere most sphere are white because of bad Textures.\n\nCC:  @tscrim @kcrisman @vinklein @jm58660\n\nBranch/Commit: **[446d90e](https://github.com/sagemath/sagetrac-mirror/commit/446d90ed9f527e100995330cf473c7735633ad0d)**\n\nReviewer: **Jori M\u00e4ntysalo**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/28736_\n\n",
    "closed_at": "2019-11-24T16:58:47Z",
    "created_at": "2019-11-14T13:12:40Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graphics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix Texture for negative inputs",
    "type": "issue",
    "updated_at": "2019-11-24T16:58:47Z",
    "url": "https://github.com/sagemath/sage/issues/28736",
    "user": "https://github.com/fchapoton"
}
```
problem visible here:

```
sage: v = [(sin(i),cos(i),i) for i in [-4,-3.5,..4]]
sage: S = sum(sphere(v[i], size=1/2, color=((i-4)/8, 1/2,(4-i)/8)) for i in range(len(v)))
sage: S.show(aspect_ratio=[1,1,1], viewer='threejs')
```
where most sphere are white because of bad Textures.

CC:  @tscrim @kcrisman @vinklein @jm58660

Branch/Commit: **[446d90e](https://github.com/sagemath/sagetrac-mirror/commit/446d90ed9f527e100995330cf473c7735633ad0d)**

Reviewer: **Jori Mäntysalo**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/28736_





---

archive/issue_events_376458.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-11-14T13:12:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/28736",
    "milestone_number": null,
    "milestone_title": "sage-9.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28736#event-376458"
}
```



---

archive/issue_events_376459.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-11-14T13:12:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28736",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graphics",
    "label_color": "0000ff",
    "label_name": "component: graphics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28736#event-376459"
}
```



---

archive/issue_events_376460.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-11-14T13:12:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28736",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28736#event-376460"
}
```



---

archive/issue_events_376461.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-11-14T13:12:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28736",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28736#event-376461"
}
```



---

archive/issue_events_376462.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-11-14T13:14:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28736",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28736#event-376462"
}
```



---

archive/issue_comments_451169.json:
```json
{
    "body": "Branch: **[u/chapoton/28736](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/28736)**",
    "created_at": "2019-11-14T13:14:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28736#issuecomment-451169",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/28736](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/28736)**



---

archive/issue_comments_451170.json:
```json
{
    "body": "Commit: **[0c2b9c2](https://github.com/sagemath/sagetrac-mirror/commit/0c2b9c231d2208bc1a3ed6099268f77ebe671dc2)**",
    "created_at": "2019-11-14T13:14:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28736#issuecomment-451170",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[0c2b9c2](https://github.com/sagemath/sagetrac-mirror/commit/0c2b9c231d2208bc1a3ed6099268f77ebe671dc2)**



---

archive/issue_comments_451171.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0c2b9c231d2208bc1a3ed6099268f77ebe671dc2\">0c2b9c2</a></td><td><code>fix Texture for negative inputs</code></td></tr></table>\n",
    "created_at": "2019-11-14T13:14:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28736#issuecomment-451171",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0c2b9c231d2208bc1a3ed6099268f77ebe671dc2">0c2b9c2</a></td><td><code>fix Texture for negative inputs</code></td></tr></table>




---

archive/issue_events_376463.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-11-14T13:15:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28736",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28736#event-376463"
}
```



---

archive/issue_events_376464.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-11-14T13:15:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28736",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28736#event-376464"
}
```



---

archive/issue_comments_451172.json:
```json
{
    "body": "Changed commit from **[0c2b9c2](https://github.com/sagemath/sagetrac-mirror/commit/0c2b9c231d2208bc1a3ed6099268f77ebe671dc2)** to **[446d90e](https://github.com/sagemath/sagetrac-mirror/commit/446d90ed9f527e100995330cf473c7735633ad0d)**",
    "created_at": "2019-11-14T13:21:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28736#issuecomment-451172",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[0c2b9c2](https://github.com/sagemath/sagetrac-mirror/commit/0c2b9c231d2208bc1a3ed6099268f77ebe671dc2)** to **[446d90e](https://github.com/sagemath/sagetrac-mirror/commit/446d90ed9f527e100995330cf473c7735633ad0d)**



---

archive/issue_comments_451173.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/446d90ed9f527e100995330cf473c7735633ad0d\">446d90e</a></td><td><code>trac 28736 fix the Texture in a better way</code></td></tr></table>\n",
    "created_at": "2019-11-14T13:21:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28736#issuecomment-451173",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/446d90ed9f527e100995330cf473c7735633ad0d">446d90e</a></td><td><code>trac 28736 fix the Texture in a better way</code></td></tr></table>




---

archive/issue_events_376465.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-11-14T13:21:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28736",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28736#event-376465"
}
```



---

archive/issue_events_376466.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-11-14T13:21:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28736",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28736#event-376466"
}
```



---

archive/issue_comments_451174.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nbot is morally green, please review",
    "created_at": "2019-11-14T18:01:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28736#issuecomment-451174",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:5" align="right">Comment 5</div>

bot is morally green, please review



---

archive/issue_comments_451175.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nreview, please, someone ?",
    "created_at": "2019-11-17T19:10:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28736#issuecomment-451175",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:6" align="right">Comment 6</div>

review, please, someone ?



---

archive/issue_comments_451176.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nI can take this.",
    "created_at": "2019-11-18T05:57:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28736#issuecomment-451176",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:7" align="right">Comment 7</div>

I can take this.



---

archive/issue_comments_451177.json:
```json
{
    "body": "Reviewer: **Jori M\u00e4ntysalo**",
    "created_at": "2019-11-18T05:57:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28736#issuecomment-451177",
    "user": "https://github.com/jm58660"
}
```

Reviewer: **Jori Mäntysalo**



---

archive/issue_events_376467.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2019-11-18T08:37:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28736",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28736#event-376467"
}
```



---

archive/issue_events_376468.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2019-11-18T08:37:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/28736",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28736#event-376468"
}
```



---

archive/issue_comments_451178.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nThis is OK.",
    "created_at": "2019-11-18T08:37:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28736#issuecomment-451178",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:8" align="right">Comment 8</div>

This is OK.



---

archive/issue_comments_451179.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nA question: is this the same bug that is seen in https://sage.sis.uta.fi/home/pub/198/ ? Accidentally I was just asked about that.",
    "created_at": "2019-11-19T11:08:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28736#issuecomment-451179",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:9" align="right">Comment 9</div>

A question: is this the same bug that is seen in https://sage.sis.uta.fi/home/pub/198/ ? Accidentally I was just asked about that.



---

archive/issue_comments_451180.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nReplying to [@jm58660](#comment%3A9):\n> A question: is this the same bug that is seen in https://sage.sis.uta.fi/home/pub/198/ ? Accidentally I was just asked about that.\n\nprobably not",
    "created_at": "2019-11-19T12:20:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28736#issuecomment-451180",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:10" align="right">Comment 10</div>

Replying to [@jm58660](#comment%3A9):
> A question: is this the same bug that is seen in https://sage.sis.uta.fi/home/pub/198/ ? Accidentally I was just asked about that.

probably not



---

archive/issue_comments_451181.json:
```json
{
    "body": "Changed branch from **[u/chapoton/28736](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/28736)** to **[446d90e](https://github.com/sagemath/sagetrac-mirror/commit/446d90ed9f527e100995330cf473c7735633ad0d)**",
    "created_at": "2019-11-24T16:58:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/28736",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/28736#issuecomment-451181",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/28736](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/28736)** to **[446d90e](https://github.com/sagemath/sagetrac-mirror/commit/446d90ed9f527e100995330cf473c7735633ad0d)**



---

archive/issue_events_376469.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-11-24T16:58:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/28736",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28736#event-376469"
}
```



---

archive/issue_events_376470.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "a5c1185e7ca9623f81a93c3355d0f9f6aaa96ad0",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-11-24T16:58:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/28736",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/28736#event-376470"
}
```
