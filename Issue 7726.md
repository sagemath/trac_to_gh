# Issue 7726: Upgrade biopython package to 1.53

archive/issues_007726.json:
```json
{
    "body": "Assignee: tbd\n\nCC:  @maxthemouse\n\nKeywords: biopython\n\nBiopython was released December 15th, 2009.  No major changes were made, so the new package just updates the source and SPKG.txt.\n\nIssue created by migration from https://trac.sagemath.org/ticket/7726\n\n",
    "created_at": "2009-12-17T19:10:15Z",
    "labels": [
        "packages: optional",
        "minor",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-4.3",
    "title": "Upgrade biopython package to 1.53",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/7726",
    "user": "mhampton"
}
```
Assignee: tbd

CC:  @maxthemouse

Keywords: biopython

Biopython was released December 15th, 2009.  No major changes were made, so the new package just updates the source and SPKG.txt.

Issue created by migration from https://trac.sagemath.org/ticket/7726





---

archive/issue_comments_066382.json:
```json
{
    "body": "New package available at:\n[http://sage.math.washington.edu/home/mhampton/biopython-1.53.p0.spkg](http://sage.math.washington.edu/home/mhampton/biopython-1.53.p0.spkg)",
    "created_at": "2009-12-17T19:11:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7726",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7726#issuecomment-66382",
    "user": "mhampton"
}
```

New package available at:
[http://sage.math.washington.edu/home/mhampton/biopython-1.53.p0.spkg](http://sage.math.washington.edu/home/mhampton/biopython-1.53.p0.spkg)



---

archive/issue_comments_066383.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2009-12-17T19:11:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7726",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7726#issuecomment-66383",
    "user": "mhampton"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_066384.json:
```json
{
    "body": "The package installs and the check runs fine. I got a couple of warnings from the BioSQL test but that seems to be expected if I understand the code. \n\nOne small thing: The changes haven't been committed to the hg depository in the package. Otherwise, it looks good to me.\n\nAdam",
    "created_at": "2009-12-17T21:24:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7726",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7726#issuecomment-66384",
    "user": "@maxthemouse"
}
```

The package installs and the check runs fine. I got a couple of warnings from the BioSQL test but that seems to be expected if I understand the code. 

One small thing: The changes haven't been committed to the hg depository in the package. Otherwise, it looks good to me.

Adam



---

archive/issue_comments_066385.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2009-12-17T21:24:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7726",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7726#issuecomment-66385",
    "user": "@maxthemouse"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_066386.json:
```json
{
    "body": "Thanks for the quick review.  I commited the changes in hg, and removed a couple of .DS_Store files that my mac had added.  This new package is up at the same link above.",
    "created_at": "2009-12-17T22:52:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7726",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7726#issuecomment-66386",
    "user": "mhampton"
}
```

Thanks for the quick review.  I commited the changes in hg, and removed a couple of .DS_Store files that my mac had added.  This new package is up at the same link above.



---

archive/issue_comments_066387.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-12-20T07:29:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7726",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7726#issuecomment-66387",
    "user": "@mwhansen"
}
```

Resolution: fixed



---

archive/issue_comments_066388.json:
```json
{
    "body": "wrong name",
    "created_at": "2017-07-19T08:44:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7726",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7726#issuecomment-66388",
    "user": "@fchapoton"
}
```

wrong name
