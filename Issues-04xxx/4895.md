# Issue 4895: [with patch; positive review] bug in pattern avoiding permutations

archive/issues_004895.json:
```json
{
    "body": "The following behavior is clearly bad:\n\n```\nsage: [p for p in Permutations(4,avoiding=[2,3,1]) if p.has_pattern([2,3,1])]               \n[[2, 3, 1, 4], [4, 2, 3, 1]]\n```\n\nSimilar behavior occurs when avoiding [1,3,2], [2,1,3], and [3,1,2].\n\nAssignee: @jbandlow\n\nCC:  @jbandlow sage-combinat\n\nKeywords: pattern avoiding permutations\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4895\n\n",
    "closed_at": "2009-01-25T02:20:55Z",
    "created_at": "2008-12-31T01:54:19Z",
    "labels": [
        "component: combinatorics",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.3",
    "title": "[with patch; positive review] bug in pattern avoiding permutations",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4895",
    "user": "https://github.com/jbandlow"
}
```
The following behavior is clearly bad:

```
sage: [p for p in Permutations(4,avoiding=[2,3,1]) if p.has_pattern([2,3,1])]               
[[2, 3, 1, 4], [4, 2, 3, 1]]
```

Similar behavior occurs when avoiding [1,3,2], [2,1,3], and [3,1,2].

Assignee: @jbandlow

CC:  @jbandlow sage-combinat

Keywords: pattern avoiding permutations

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4895





---

archive/issue_events_018887.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-01-09T23:27:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4895",
    "milestone": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4895#event-18887"
}
```



---

archive/issue_events_018888.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4895",
    "rename": {
        "from": "bug in pattern avoiding permutations",
        "to": "[with patch; needs review] bug in pattern avoiding permutations"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4895#event-18888"
}
```



---

archive/issue_comments_039374.json:
```json
{
    "body": "<a id='comment:2'></a>\nAttachment [4895.patch](tarball://root/attachments/some-uuid/ticket4895/4895.patch) by @jbandlow created at 2009-01-15 07:55:44\n\nThe problem was a simple typo in an initial condition (combined with a bunch of doctests that tested for incorrect behavior).  Warning: I've been travelling and not had an opportunity to upgrade sage recently, so this patch is based off 3.2.1.  I don't expect this to be a problem, but if the patch does not apply, let me know and I will fix it as soon as possible.",
    "created_at": "2009-01-15T07:55:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4895",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4895#issuecomment-39374",
    "user": "https://github.com/jbandlow"
}
```

<a id='comment:2'></a>
Attachment [4895.patch](tarball://root/attachments/some-uuid/ticket4895/4895.patch) by @jbandlow created at 2009-01-15 07:55:44

The problem was a simple typo in an initial condition (combined with a bunch of doctests that tested for incorrect behavior).  Warning: I've been travelling and not had an opportunity to upgrade sage recently, so this patch is based off 3.2.1.  I don't expect this to be a problem, but if the patch does not apply, let me know and I will fix it as soon as possible.



---

archive/issue_comments_039375.json:
```json
{
    "body": "Changing assignee from @mwhansen to @jbandlow.",
    "created_at": "2009-01-15T07:55:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4895",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4895#issuecomment-39375",
    "user": "https://github.com/jbandlow"
}
```

Changing assignee from @mwhansen to @jbandlow.



---

archive/issue_events_018889.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4895",
    "rename": {
        "from": "[with patch; needs review] bug in pattern avoiding permutations",
        "to": "[with patch; positive review] bug in pattern avoiding permutations"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4895#event-18889"
}
```



---

archive/issue_comments_039376.json:
```json
{
    "body": "<a id='comment:3'></a>\nNice work Jason.",
    "created_at": "2009-01-24T03:02:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4895",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4895#issuecomment-39376",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:3'></a>
Nice work Jason.



---

archive/issue_comments_039377.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-01-25T02:20:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4895",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4895#issuecomment-39377",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_018890.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-01-25T02:20:55Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4895",
    "milestone": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4895#event-18890"
}
```



---

archive/issue_events_018891.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-01-25T02:20:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4895",
    "milestone": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4895#event-18891"
}
```



---

archive/issue_comments_039378.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.3.alpha2.\n\nCheers,\n\nMichael",
    "created_at": "2009-01-25T02:20:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4895",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4895#issuecomment-39378",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.3.alpha2.

Cheers,

Michael



---

archive/issue_events_018892.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-01-25T02:20:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4895",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4895#event-18892"
}
```
