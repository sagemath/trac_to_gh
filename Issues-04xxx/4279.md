# Issue 4279: [with patch, positive review] Sage 3.1.3.rc0: numerical noise in rings/real_lazy.pyx

archive/issues_004279.json:
```json
{
    "body": "\n```\nsage -t  devel/sage/sage/rings/real_lazy.pyx \n********************************************************************** \nFile \"/Users/mh/Desktop/sage-3.1.3.rc0/tmp/real_lazy.py\", line 549: \n    sage: complex(CLF(-1)^(1/4)) \nExpected: \n    (0.70710678118654757+0.70710678118654746j) \nGot: \n    (0.70710678118654746+0.70710678118654757j) \n*********************************************************************\n```\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4279\n\n",
    "closed_at": "2008-10-14T12:25:43Z",
    "created_at": "2008-10-14T09:42:43Z",
    "labels": [
        "component: doctest coverage",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1.3",
    "title": "[with patch, positive review] Sage 3.1.3.rc0: numerical noise in rings/real_lazy.pyx",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4279",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

```
sage -t  devel/sage/sage/rings/real_lazy.pyx 
********************************************************************** 
File "/Users/mh/Desktop/sage-3.1.3.rc0/tmp/real_lazy.py", line 549: 
    sage: complex(CLF(-1)^(1/4)) 
Expected: 
    (0.70710678118654757+0.70710678118654746j) 
Got: 
    (0.70710678118654746+0.70710678118654757j) 
*********************************************************************
```

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4279





---

archive/issue_comments_033031.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-10-14T09:42:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4279",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4279#issuecomment-33031",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing status from new to assigned.



---

archive/issue_comments_033032.json:
```json
{
    "body": "<a id='comment:2'></a>\nOn a G5 with 10.4 I am seeing the following issue:\n\n```\nsage -t -long devel/sage/sage/rings/real_lazy.pyx           \n**********************************************************************\nFile \"/Users/mabshoff/sage-3.1.3.rc0/tmp/real_lazy.py\", line 549:\n    sage: complex(CLF(-1)^(1/4))\nExpected:\n    (0.70710678118654757+0.70710678118654746j)\nGot:\n    (0.70710678118654746+0.70710678118654757j)\n**********************************************************************\n```",
    "created_at": "2008-10-14T11:18:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4279",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4279#issuecomment-33032",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
On a G5 with 10.4 I am seeing the following issue:

```
sage -t -long devel/sage/sage/rings/real_lazy.pyx           
**********************************************************************
File "/Users/mabshoff/sage-3.1.3.rc0/tmp/real_lazy.py", line 549:
    sage: complex(CLF(-1)^(1/4))
Expected:
    (0.70710678118654757+0.70710678118654746j)
Got:
    (0.70710678118654746+0.70710678118654757j)
**********************************************************************
```



---

archive/issue_events_015987.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4279",
    "rename": {
        "from": "Sage 3.1.3.rc0: numerical noise in rings/real_lazy.pyx",
        "to": "[with patch, needs review] Sage 3.1.3.rc0: numerical noise in rings/real_lazy.pyx"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4279#event-15987"
}
```



---

archive/issue_comments_033033.json:
```json
{
    "body": "<a id='comment:3'></a>\nAttachment [trac_4279.patch](tarball://root/attachments/some-uuid/ticket4279/trac_4279.patch) by mabshoff created at 2008-10-14 11:57:19",
    "created_at": "2008-10-14T11:57:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4279",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4279#issuecomment-33033",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Attachment [trac_4279.patch](tarball://root/attachments/some-uuid/ticket4279/trac_4279.patch) by mabshoff created at 2008-10-14 11:57:19



---

archive/issue_comments_033034.json:
```json
{
    "body": "<a id='comment:4'></a>\nLooks good to me.",
    "created_at": "2008-10-14T11:58:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4279",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4279#issuecomment-33034",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:4'></a>
Looks good to me.



---

archive/issue_events_015988.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4279",
    "rename": {
        "from": "[with patch, needs review] Sage 3.1.3.rc0: numerical noise in rings/real_lazy.pyx",
        "to": "[with patch, positive review] Sage 3.1.3.rc0: numerical noise in rings/real_lazy.pyx"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4279#event-15988"
}
```



---

archive/issue_comments_033035.json:
```json
{
    "body": "<a id='comment:5'></a>\nMerged in 3.1.3.final",
    "created_at": "2008-10-14T12:25:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4279",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4279#issuecomment-33035",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:5'></a>
Merged in 3.1.3.final



---

archive/issue_events_015989.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-10-14T12:25:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4279",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4279#event-15989"
}
```



---

archive/issue_comments_033036.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-10-14T12:25:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4279",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4279#issuecomment-33036",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
