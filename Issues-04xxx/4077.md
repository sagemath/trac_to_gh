# Issue 4077: [with patch, positive review] notebook - ReactorNotRunning error consistently seen in sage-3.1.2.rc0

archive/issues_004077.json:
```json
{
    "body": "Traceback when I hit CLTRL-C to stop the notebook. \n\nThis is happening in sage-3.1.2.rc0 with and without the spkg at #4074.\n\n```\n2008-09-08 05:45:38-0700 [-] Log opened.\n2008-09-08 05:45:38-0700 [-] twistd 8.1.0 (/home/tclemans/sage-3.1.2.rc0/local/bin/python 2.5.2) starting up\n2008-09-08 05:45:38-0700 [-] reactor class: <class 'twisted.internet.selectreactor.SelectReactor'>\n2008-09-08 05:45:38-0700 [-] twisted.web2.channel.http.HTTPFactory starting on 8999\n2008-09-08 05:45:38-0700 [-] Starting factory <twisted.web2.channel.http.HTTPFactory instance at 0x2b779155d9e0>\n2008-09-08 05:45:49-0700 [-] Saving notebook...\n2008-09-08 05:45:49-0700 [-] Notebook cleanly saved.\n2008-09-08 05:45:49-0700 [-] Saving notebook...\n2008-09-08 05:45:49-0700 [-] Unhandled Error\n        Traceback (most recent call last):\n          File \"/home/tclemans/sage-3.1.2.rc0/local/lib/python2.5/site-packages/Twisted-8.1.0-py2.5-linux-x86_64.egg/twisted/internet/base.py\", line 1048, in run\n            self.mainLoop()\n          File \"/home/tclemans/sage-3.1.2.rc0/local/lib/python2.5/site-packages/Twisted-8.1.0-py2.5-linux-x86_64.egg/twisted/internet/base.py\", line 1057, in mainLoop\n            self.runUntilCurrent()\n          File \"/home/tclemans/sage-3.1.2.rc0/local/lib/python2.5/site-packages/Twisted-8.1.0-py2.5-linux-x86_64.egg/twisted/internet/base.py\", line 705, in runUntilCurrent\n            call.func(*call.args, **call.kw)\n          File \"/home/tclemans/sage-3.1.2.rc0/local/lib/python2.5/site-packages/Twisted-8.1.0-py2.5-linux-x86_64.egg/twisted/internet/base.py\", line 545, in fireSystemEvent\n            event.fireEvent()\n        --- <exception caught here> ---\n          File \"/home/tclemans/sage-3.1.2.rc0/local/lib/python2.5/site-packages/Twisted-8.1.0-py2.5-linux-x86_64.egg/twisted/internet/base.py\", line 368, in fireEvent\n            result = callable(*args, **kwargs)\n          File \"notebooktesting/twistedconf.tac\", line 25, in save_notebook\n            reactor.stop()\n          File \"/home/tclemans/sage-3.1.2.rc0/local/lib/python2.5/site-packages/Twisted-8.1.0-py2.5-linux-x86_64.egg/twisted/internet/base.py\", line 495, in stop\n            \"Can't stop reactor that isn't running.\")\n        twisted.internet.error.ReactorNotRunning: Can't stop reactor that isn't running.\n\n2008-09-08 05:45:49-0700 [-] (Port 8999 Closed)\n2008-09-08 05:45:49-0700 [-] Stopping factory <twisted.web2.channel.http.HTTPFactory instance at 0x2b779155d9e0>\n2008-09-08 05:45:49-0700 [-] Main loop terminated.\n2008-09-08 05:45:49-0700 [-] Server Shut Down.\nTrue\n```\n\nAssignee: @mwhansen\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4077\n\n",
    "closed_at": "2008-09-09T04:59:59Z",
    "created_at": "2008-09-08T12:52:39Z",
    "labels": [
        "component: notebook",
        "critical",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1.2",
    "title": "[with patch, positive review] notebook - ReactorNotRunning error consistently seen in sage-3.1.2.rc0",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4077",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```
Traceback when I hit CLTRL-C to stop the notebook. 

This is happening in sage-3.1.2.rc0 with and without the spkg at #4074.

```
2008-09-08 05:45:38-0700 [-] Log opened.
2008-09-08 05:45:38-0700 [-] twistd 8.1.0 (/home/tclemans/sage-3.1.2.rc0/local/bin/python 2.5.2) starting up
2008-09-08 05:45:38-0700 [-] reactor class: <class 'twisted.internet.selectreactor.SelectReactor'>
2008-09-08 05:45:38-0700 [-] twisted.web2.channel.http.HTTPFactory starting on 8999
2008-09-08 05:45:38-0700 [-] Starting factory <twisted.web2.channel.http.HTTPFactory instance at 0x2b779155d9e0>
2008-09-08 05:45:49-0700 [-] Saving notebook...
2008-09-08 05:45:49-0700 [-] Notebook cleanly saved.
2008-09-08 05:45:49-0700 [-] Saving notebook...
2008-09-08 05:45:49-0700 [-] Unhandled Error
        Traceback (most recent call last):
          File "/home/tclemans/sage-3.1.2.rc0/local/lib/python2.5/site-packages/Twisted-8.1.0-py2.5-linux-x86_64.egg/twisted/internet/base.py", line 1048, in run
            self.mainLoop()
          File "/home/tclemans/sage-3.1.2.rc0/local/lib/python2.5/site-packages/Twisted-8.1.0-py2.5-linux-x86_64.egg/twisted/internet/base.py", line 1057, in mainLoop
            self.runUntilCurrent()
          File "/home/tclemans/sage-3.1.2.rc0/local/lib/python2.5/site-packages/Twisted-8.1.0-py2.5-linux-x86_64.egg/twisted/internet/base.py", line 705, in runUntilCurrent
            call.func(*call.args, **call.kw)
          File "/home/tclemans/sage-3.1.2.rc0/local/lib/python2.5/site-packages/Twisted-8.1.0-py2.5-linux-x86_64.egg/twisted/internet/base.py", line 545, in fireSystemEvent
            event.fireEvent()
        --- <exception caught here> ---
          File "/home/tclemans/sage-3.1.2.rc0/local/lib/python2.5/site-packages/Twisted-8.1.0-py2.5-linux-x86_64.egg/twisted/internet/base.py", line 368, in fireEvent
            result = callable(*args, **kwargs)
          File "notebooktesting/twistedconf.tac", line 25, in save_notebook
            reactor.stop()
          File "/home/tclemans/sage-3.1.2.rc0/local/lib/python2.5/site-packages/Twisted-8.1.0-py2.5-linux-x86_64.egg/twisted/internet/base.py", line 495, in stop
            "Can't stop reactor that isn't running.")
        twisted.internet.error.ReactorNotRunning: Can't stop reactor that isn't running.

2008-09-08 05:45:49-0700 [-] (Port 8999 Closed)
2008-09-08 05:45:49-0700 [-] Stopping factory <twisted.web2.channel.http.HTTPFactory instance at 0x2b779155d9e0>
2008-09-08 05:45:49-0700 [-] Main loop terminated.
2008-09-08 05:45:49-0700 [-] Server Shut Down.
True
```

Assignee: @mwhansen

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4077





---

archive/issue_comments_031034.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [trac_4077.patch](tarball://root/attachments/some-uuid/ticket4077/trac_4077.patch) by @mwhansen created at 2008-09-08 23:20:30",
    "created_at": "2008-09-08T23:20:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4077",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4077#issuecomment-31034",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'></a>
Attachment [trac_4077.patch](tarball://root/attachments/some-uuid/ticket4077/trac_4077.patch) by @mwhansen created at 2008-09-08 23:20:30



---

archive/issue_comments_031035.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-09-08T23:20:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4077",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4077#issuecomment-31035",
    "user": "https://github.com/mwhansen"
}
```

Changing status from new to assigned.



---

archive/issue_events_015207.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4077",
    "rename": {
        "from": "notebook - ReactorNotRunning error consistently seen in sage-3.1.2.rc0",
        "to": "[with patch, needs review] notebook - ReactorNotRunning error consistently seen in sage-3.1.2.rc0"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4077#event-15207"
}
```



---

archive/issue_comments_031036.json:
```json
{
    "body": "Changing assignee from boothby to @mwhansen.",
    "created_at": "2008-09-08T23:20:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4077",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4077#issuecomment-31036",
    "user": "https://github.com/mwhansen"
}
```

Changing assignee from boothby to @mwhansen.



---

archive/issue_comments_031037.json:
```json
{
    "body": "<a id='comment:2'></a>\nPatch looks good to me.\n\nCheers,\n\nMichael",
    "created_at": "2008-09-09T04:58:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4077",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4077#issuecomment-31037",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Patch looks good to me.

Cheers,

Michael



---

archive/issue_events_015208.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4077",
    "rename": {
        "from": "[with patch, needs review] notebook - ReactorNotRunning error consistently seen in sage-3.1.2.rc0",
        "to": "[with patch, positive review] notebook - ReactorNotRunning error consistently seen in sage-3.1.2.rc0"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4077#event-15208"
}
```



---

archive/issue_events_015209.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-09T04:59:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4077",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4077#event-15209"
}
```



---

archive/issue_comments_031038.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.1.2.rc1",
    "created_at": "2008-09-09T04:59:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4077",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4077#issuecomment-31038",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.1.2.rc1



---

archive/issue_comments_031039.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-09-09T04:59:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4077",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4077#issuecomment-31039",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
