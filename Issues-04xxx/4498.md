# Issue 4498: Implement a symbolic version of the arg function

archive/issues_004498.json:
```json
{
    "assignees": [],
    "body": "It would be nice if there were a symbolic arg function, just like the symbolic sin, cos, etc., functions.  Then the following would happen:\n\n```\nsage: f = arg(x); f\narg(x)\nsage: f.subs(x=1+I)\narg(1+I)\n```\n\nNow we have\n\n```\nsage: arg(1+I)\n0.785398163397\nsage: type(arg(1+I))\n<type 'sage.rings.real_double.RealDoubleElement'>\n```\n\nI.e., the arg in Sage is currently the numerical person's arg, not the symbolic person's.  It just casts to CDF.\n\n\n\"The function should return the argument of a complex function.\" - Ronan Paix\u00e3o\n\nApply\n* [attachment: trac_4498-symbolic_arg.cleanup.patch.gz](https://github.com/sagemath/sage/files/ticket4498/trac_4498-symbolic_arg.cleanup.patch.gz)\n* [attachment: trac_4498-arg_evalf.patch.gz](https://github.com/sagemath/sage/files/ticket4498/trac_4498-arg_evalf.patch.gz)\n\nCC:  @kcrisman ktkohl\n\nKeywords: **sd35.5**\n\nReviewer: **Karl-Dieter Crisman, Burcin Erocal**\n\nAuthor: **Karen Kohl, Burcin Erocal**\n\nMerged: **sage-5.0.beta4**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4498_\n\n",
    "closed_at": "2012-02-14T14:20:11Z",
    "created_at": "2008-11-11T23:21:48Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/good%20first%20issue"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Implement a symbolic version of the arg function",
    "type": "issue",
    "updated_at": "2017-07-19T08:21:56Z",
    "url": "https://github.com/sagemath/sage/issues/4498",
    "user": "https://github.com/sagetrac-TimothyClemans"
}
```
It would be nice if there were a symbolic arg function, just like the symbolic sin, cos, etc., functions.  Then the following would happen:

```
sage: f = arg(x); f
arg(x)
sage: f.subs(x=1+I)
arg(1+I)
```

Now we have

```
sage: arg(1+I)
0.785398163397
sage: type(arg(1+I))
<type 'sage.rings.real_double.RealDoubleElement'>
```

I.e., the arg in Sage is currently the numerical person's arg, not the symbolic person's.  It just casts to CDF.


"The function should return the argument of a complex function." - Ronan Paixão

Apply
* [attachment: trac_4498-symbolic_arg.cleanup.patch.gz](https://github.com/sagemath/sage/files/ticket4498/trac_4498-symbolic_arg.cleanup.patch.gz)
* [attachment: trac_4498-arg_evalf.patch.gz](https://github.com/sagemath/sage/files/ticket4498/trac_4498-arg_evalf.patch.gz)

CC:  @kcrisman ktkohl

Keywords: **sd35.5**

Reviewer: **Karl-Dieter Crisman, Burcin Erocal**

Author: **Karen Kohl, Burcin Erocal**

Merged: **sage-5.0.beta4**

_Issue created by migration from https://trac.sagemath.org/ticket/4498_





---

archive/issue_events_044494.json:
```json
{
    "actor": "https://github.com/sagetrac-TimothyClemans",
    "created_at": "2008-11-11T23:21:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44494"
}
```



---

archive/issue_events_044495.json:
```json
{
    "actor": "https://github.com/sagetrac-TimothyClemans",
    "created_at": "2008-11-11T23:21:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "component: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44495"
}
```



---

archive/issue_events_044496.json:
```json
{
    "actor": "https://github.com/sagetrac-TimothyClemans",
    "created_at": "2008-11-11T23:21:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44496"
}
```



---

archive/issue_events_044497.json:
```json
{
    "actor": "https://github.com/sagetrac-TimothyClemans",
    "created_at": "2008-11-11T23:21:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44497"
}
```



---

archive/issue_comments_027269.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\n\n```\narg(x)\n///\n\nTraceback (most recent call last):\n File \"<stdin>\", line 1, in <module>\n File \"/home/ronan/.sage/sage_notebook/worksheets/admin/0/code/127.py\",\nline 6, in <module>\n   exec compile(ur'arg(x)' + '\\n', '', 'single')\n File\n\"/home/ronan/progs/sage/local/lib/python2.5/site-packages/ZODB3-3.7.0-py2.5-linux-i686.egg/\", line 1, in <module>\n\n File\n\"/home/ronan/progs/sage/local/lib/python2.5/site-packages/sage/misc/functional.py\", line 67, in arg\n   except AttributeError: return CDF(x).arg()\n File \"complex_double.pyx\", line 286, in\nsage.rings.complex_double.ComplexDoubleField_class.__call__\n(sage/rings/complex_double.c:3324)\n File\n\"/home/ronan/progs/sage/local/lib/python2.5/site-packages/sage/calculus/calculus.py\", line 1454, in _complex_double_\n   raise TypeError\nTypeError\n```",
    "created_at": "2008-11-11T23:24:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27269",
    "user": "https://github.com/sagetrac-TimothyClemans"
}
```

<a id='comment:1'>Comment 1:</a>

```
arg(x)
///

Traceback (most recent call last):
 File "<stdin>", line 1, in <module>
 File "/home/ronan/.sage/sage_notebook/worksheets/admin/0/code/127.py",
line 6, in <module>
   exec compile(ur'arg(x)' + '\n', '', 'single')
 File
"/home/ronan/progs/sage/local/lib/python2.5/site-packages/ZODB3-3.7.0-py2.5-linux-i686.egg/", line 1, in <module>

 File
"/home/ronan/progs/sage/local/lib/python2.5/site-packages/sage/misc/functional.py", line 67, in arg
   except AttributeError: return CDF(x).arg()
 File "complex_double.pyx", line 286, in
sage.rings.complex_double.ComplexDoubleField_class.__call__
(sage/rings/complex_double.c:3324)
 File
"/home/ronan/progs/sage/local/lib/python2.5/site-packages/sage/calculus/calculus.py", line 1454, in _complex_double_
   raise TypeError
TypeError
```



---

archive/issue_comments_027270.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nPlease post a complete session. As is the above is not very clear.\n\nCheers,\n\nMichael",
    "created_at": "2008-11-12T14:16:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27270",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:2'>Comment 2:</a>
Please post a complete session. As is the above is not very clear.

Cheers,

Michael



---

archive/issue_events_044498.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-12T17:13:16Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "milestone_number": null,
    "milestone_title": "sage-3.2.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44498"
}
```



---

archive/issue_events_044499.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-12T17:13:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44499"
}
```



---

archive/issue_events_044500.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-12T17:13:16Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "title_is": "Implement a symbolic version of the arg function; the current one is only for actual numbers",
    "title_was": "The argument function does not work with variables.",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44500"
}
```



---

archive/issue_events_044501.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-12T17:13:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44501"
}
```



---

archive/issue_events_044502.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-12T17:13:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44502"
}
```



---

archive/issue_comments_027271.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,22 @@\n+It would be nice if there were a symbolic arg function, just like the symbolic sin, cos, etc., functions.  Then the following would happen:\n+\n+```\n+sage: f = arg(x); f\n+arg(x)\n+sage: f.subs(x=1+I)\n+arg(1+I)\n+```\n+\n+Now we have\n+\n+```\n+sage: arg(1+I)\n+0.785398163397\n+sage: type(arg(1+I))\n+<type 'sage.rings.real_double.RealDoubleElement'>\n+```\n+\n+I.e., the arg in Sage is currently the numerical person's arg, not the symbolic person's.  It just casts to CDF.\n+\n+\n \"The function should return the argument of a complex function.\" - Ronan Paix\u00e3o\n``````\n",
    "created_at": "2008-11-12T17:13:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27271",
    "user": "https://github.com/williamstein"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,22 @@
+It would be nice if there were a symbolic arg function, just like the symbolic sin, cos, etc., functions.  Then the following would happen:
+
+```
+sage: f = arg(x); f
+arg(x)
+sage: f.subs(x=1+I)
+arg(1+I)
+```
+
+Now we have
+
+```
+sage: arg(1+I)
+0.785398163397
+sage: type(arg(1+I))
+<type 'sage.rings.real_double.RealDoubleElement'>
+```
+
+I.e., the arg in Sage is currently the numerical person's arg, not the symbolic person's.  It just casts to CDF.
+
+
 "The function should return the argument of a complex function." - Ronan Paixão
``````




---

archive/issue_comments_027272.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nIs this really an enchancement? As it is, just using arg(x) already raises an error and everything that needs arg() must be implemented numerically.",
    "created_at": "2008-11-15T17:17:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27272",
    "user": "https://github.com/sagetrac-ronanpaixao"
}
```

<a id='comment:4'>Comment 4:</a>
Is this really an enchancement? As it is, just using arg(x) already raises an error and everything that needs arg() must be implemented numerically.



---

archive/issue_comments_027273.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\n> Is this really an enchancement? As it is, just using arg(x) already \n> raises an error and everything that needs arg() must be implemented numerically. \n\nYes, this is an enhancement since it is implementing new functionality. \nIt would be a bug fix if there were a bug in the existing arg function, where it produced invalid results on supported input.\n\n -- William",
    "created_at": "2008-11-16T19:42:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27273",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:5'>Comment 5:</a>
> Is this really an enchancement? As it is, just using arg(x) already 
> raises an error and everything that needs arg() must be implemented numerically. 

Yes, this is an enhancement since it is implementing new functionality. 
It would be a bug fix if there were a bug in the existing arg function, where it produced invalid results on supported input.

 -- William



---

archive/issue_events_044503.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2010-05-26T21:00:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "component: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44503"
}
```



---

archive/issue_events_044504.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2010-05-26T21:00:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44504"
}
```



---

archive/issue_events_044505.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2010-05-27T10:25:40Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "title_is": "Implement a symbolic version of the arg function",
    "title_was": "Implement a symbolic version of the arg function; the current one is only for actual numbers",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44505"
}
```



---

archive/issue_comments_027274.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nThis was also reported in #6220. I will close that as duplicate.",
    "created_at": "2010-05-27T10:25:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27274",
    "user": "https://github.com/burcin"
}
```

<a id='comment:7'>Comment 7:</a>
This was also reported in #6220. I will close that as duplicate.



---

archive/issue_events_044506.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2010-08-28T16:14:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "label": "https://github.com/sagemath/sage/labels/good%20first%20issue",
    "label_color": "7057ff",
    "label_name": "good first issue",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44506"
}
```



---

archive/issue_comments_027275.json:
```json
{
    "body": "Changed keywords from **** to **sd35.5**",
    "created_at": "2012-01-11T15:39:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27275",
    "user": "https://github.com/sagetrac-ktkohl"
}
```

Changed keywords from **** to **sd35.5**



---

archive/issue_comments_027276.json:
```json
{
    "body": "symbolic arg function",
    "created_at": "2012-01-12T04:54:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27276",
    "user": "https://github.com/sagetrac-ktkohl"
}
```

symbolic arg function



---

archive/issue_comments_027277.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nAttachment: **[trac_4498_arg.1.patch.gz](https://github.com/sagemath/sage/files/ticket4498/trac_4498_arg.1.patch.gz)**",
    "created_at": "2012-01-12T04:54:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27277",
    "user": "https://github.com/sagetrac-ktkohl"
}
```

<a id='comment:10'>Comment 10:</a>
Attachment: **[trac_4498_arg.1.patch.gz](https://github.com/sagemath/sage/files/ticket4498/trac_4498_arg.1.patch.gz)**



---

archive/issue_comments_027278.json:
```json
{
    "body": "Author: **Karen T. Kohl**",
    "created_at": "2012-01-12T04:54:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27278",
    "user": "https://github.com/sagetrac-ktkohl"
}
```

Author: **Karen T. Kohl**



---

archive/issue_events_044507.json:
```json
{
    "actor": "https://github.com/sagetrac-ktkohl",
    "created_at": "2012-01-12T04:54:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44507"
}
```



---

archive/issue_comments_027279.json:
```json
{
    "body": "Reviewer: **Karl-Dieter Crisman**",
    "created_at": "2012-01-12T15:06:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27279",
    "user": "https://github.com/kcrisman"
}
```

Reviewer: **Karl-Dieter Crisman**



---

archive/issue_events_044508.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-01-12T15:06:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44508"
}
```



---

archive/issue_events_044509.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-01-12T15:06:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44509"
}
```



---

archive/issue_comments_027280.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nThere are a few small formatting issues, and it would be good to add an example showing that `arg(sqrt(2)+i)` remaining symbolic (as opposed to `arctan(1/sqrt(2))`) still evaluates correctly numerically.  Otherwise looks fine.  Currently running tests, as `arg` is likely used in a *lot* of places in Sage...",
    "created_at": "2012-01-12T15:06:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27280",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:11'>Comment 11:</a>
There are a few small formatting issues, and it would be good to add an example showing that `arg(sqrt(2)+i)` remaining symbolic (as opposed to `arctan(1/sqrt(2))`) still evaluates correctly numerically.  Otherwise looks fine.  Currently running tests, as `arg` is likely used in a *lot* of places in Sage...



---

archive/issue_comments_027281.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\n\n```\n\nFile \"/Users/karl-dietercrisman/Downloads/sage-4.8.alpha5/devel/sage-main/sage/symbolic/random_tests.py\", line 16:\n    sage: [f for (one,f,arity) in _mk_full_functions()]\nExpected:\n    [Ei, abs, arccos, arccosh, arccot, arccoth, arccsc, arccsch,\n    arcsec, arcsech, arcsin, arcsinh, arctan, arctan2, arctanh,\n    binomial, ceil, conjugate, cos, cosh, cot, coth, csc, csch,\n    dickman_rho, dilog, dirac_delta, elliptic_e, elliptic_ec,\n    elliptic_eu, elliptic_f, elliptic_kc, elliptic_pi, erf, exp,\n    factorial, floor, heaviside, imag_part, integrate,\n    kronecker_delta, log, polylog, real_part, sec, sech, sgn, sin,\n    sinh, tan, tanh, unit_step, zeta, zetaderiv]\nGot:\n    [Ei, abs, arccos, arccosh, arccot, arccoth, arccsc, arccsch, arcsec, arcsech, arcsin, arcsinh, arctan, arctan2, arctanh, arg, binomial, ceil, conjugate, cos, cosh, cot, coth, csc, csch, dickman_rho, dilog, dirac_delta, elliptic_e, elliptic_ec, elliptic_eu, elliptic_f, elliptic_kc, elliptic_pi, erf, exp, factorial, floor, heaviside, imag_part, integrate, kronecker_delta, log, polylog, real_part, sec, sech, sgn, sin, sinh, tan, tanh, unit_step, zeta, zetaderiv]\n**********************************************************************\nFile \"/Users/karl-dietercrisman/Downloads/sage-4.8.alpha5/devel/sage-main/sage/symbolic/random_tests.py\", line 238:\n    sage: random_expr(5, verbose=True)\nExpected:\n    About to apply dirac_delta to [1]\n    About to apply arccsch to [0]\n    About to apply <built-in function add> to [0, arccsch(0)]\n    arccsch(0)\nGot:\n    About to apply dirac_delta to [1]\n    About to apply arcsec to [0]\n    About to apply <built-in function add> to [0, arcsec(0)]\n    arcsec(0)\n```",
    "created_at": "2012-01-12T15:08:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27281",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:12'>Comment 12:</a>

```

File "/Users/karl-dietercrisman/Downloads/sage-4.8.alpha5/devel/sage-main/sage/symbolic/random_tests.py", line 16:
    sage: [f for (one,f,arity) in _mk_full_functions()]
Expected:
    [Ei, abs, arccos, arccosh, arccot, arccoth, arccsc, arccsch,
    arcsec, arcsech, arcsin, arcsinh, arctan, arctan2, arctanh,
    binomial, ceil, conjugate, cos, cosh, cot, coth, csc, csch,
    dickman_rho, dilog, dirac_delta, elliptic_e, elliptic_ec,
    elliptic_eu, elliptic_f, elliptic_kc, elliptic_pi, erf, exp,
    factorial, floor, heaviside, imag_part, integrate,
    kronecker_delta, log, polylog, real_part, sec, sech, sgn, sin,
    sinh, tan, tanh, unit_step, zeta, zetaderiv]
Got:
    [Ei, abs, arccos, arccosh, arccot, arccoth, arccsc, arccsch, arcsec, arcsech, arcsin, arcsinh, arctan, arctan2, arctanh, arg, binomial, ceil, conjugate, cos, cosh, cot, coth, csc, csch, dickman_rho, dilog, dirac_delta, elliptic_e, elliptic_ec, elliptic_eu, elliptic_f, elliptic_kc, elliptic_pi, erf, exp, factorial, floor, heaviside, imag_part, integrate, kronecker_delta, log, polylog, real_part, sec, sech, sgn, sin, sinh, tan, tanh, unit_step, zeta, zetaderiv]
**********************************************************************
File "/Users/karl-dietercrisman/Downloads/sage-4.8.alpha5/devel/sage-main/sage/symbolic/random_tests.py", line 238:
    sage: random_expr(5, verbose=True)
Expected:
    About to apply dirac_delta to [1]
    About to apply arccsch to [0]
    About to apply <built-in function add> to [0, arccsch(0)]
    arccsch(0)
Got:
    About to apply dirac_delta to [1]
    About to apply arcsec to [0]
    About to apply <built-in function add> to [0, arcsec(0)]
    arcsec(0)
```



---

archive/issue_comments_027282.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nI think the Maxima translation may not be correct.\n\n```\n\n -- Function: carg (<z>)\n     Returns the complex argument of <z>.  The complex argument is an\n     angle `theta' in `(-%pi, %pi]' such that `r exp (theta %i) = <z>'\n     where `r' is the magnitude of <z>.\n\n     `carg' is a computational function, not a simplifying function.\n\n     See also `abs' (complex magnitude), `polarform', `rectform',\n     `realpart', and `imagpart'.\n\n     Examples:\n\n          (%i1) carg (1);\n          (%o1)                           0\n          (%i2) carg (1 + %i);\n                                         %pi\n          (%o2)                          ---\n                                          4\n          (%i3) carg (exp (%i));\n          (%o3)                           1\n          (%i4) carg (exp (%pi * %i));\n          (%o4)                          %pi\n          (%i5) carg (exp (3/2 * %pi * %i));\n                                          %pi\n          (%o5)                         - ---\n                                           2\n          (%i6) carg (17 * exp (2 * %i));\n          (%o6)                           2\n\n\n(%o3)                                true\n```\nSee also Barton Willis' [parg](http://maxima.sourceforge.net/docs/manual/en/maxima_80.html#Item_003a-parg), though that only works if having loaded `to_poly_solve`.",
    "created_at": "2012-01-12T16:25:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27282",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:13'>Comment 13:</a>
I think the Maxima translation may not be correct.

```

 -- Function: carg (<z>)
     Returns the complex argument of <z>.  The complex argument is an
     angle `theta' in `(-%pi, %pi]' such that `r exp (theta %i) = <z>'
     where `r' is the magnitude of <z>.

     `carg' is a computational function, not a simplifying function.

     See also `abs' (complex magnitude), `polarform', `rectform',
     `realpart', and `imagpart'.

     Examples:

          (%i1) carg (1);
          (%o1)                           0
          (%i2) carg (1 + %i);
                                         %pi
          (%o2)                          ---
                                          4
          (%i3) carg (exp (%i));
          (%o3)                           1
          (%i4) carg (exp (%pi * %i));
          (%o4)                          %pi
          (%i5) carg (exp (3/2 * %pi * %i));
                                          %pi
          (%o5)                         - ---
                                           2
          (%i6) carg (17 * exp (2 * %i));
          (%o6)                           2


(%o3)                                true
```
See also Barton Willis' [parg](http://maxima.sourceforge.net/docs/manual/en/maxima_80.html#Item_003a-parg), though that only works if having loaded `to_poly_solve`.



---

archive/issue_comments_027283.json:
```json
{
    "body": "Work Issues: **random tests, Maxima**",
    "created_at": "2012-01-12T16:31:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27283",
    "user": "https://github.com/kcrisman"
}
```

Work Issues: **random tests, Maxima**



---

archive/issue_comments_027284.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nOtherwise, all tests pass!",
    "created_at": "2012-01-12T16:31:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27284",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:14'>Comment 14:</a>
Otherwise, all tests pass!



---

archive/issue_comments_027285.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nDo this to check the fix works.\n\n```\nsage: maxima(arg(x))\natan2(0,x)\nsage: maxima(arg(2+i))\natan(1/2)\nsage: maxima(arg(sqrt(2)+i))\natan(1/sqrt(2))\nsage: arg(2+i)\narctan(1/2)\nsage: arg(sqrt(2)+i)\narg(sqrt(2) + I)\n```\nIt also seems to help with the sqrt(2) issue, in a manner of speaking.  One could tell someone to do \n\n```\nsage: arg(sqrt(2)+i).simplify()\narctan(1/2*sqrt(2))\n```",
    "created_at": "2012-01-12T16:37:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27285",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:15'>Comment 15:</a>
Do this to check the fix works.

```
sage: maxima(arg(x))
atan2(0,x)
sage: maxima(arg(2+i))
atan(1/2)
sage: maxima(arg(sqrt(2)+i))
atan(1/sqrt(2))
sage: arg(2+i)
arctan(1/2)
sage: arg(sqrt(2)+i)
arg(sqrt(2) + I)
```
It also seems to help with the sqrt(2) issue, in a manner of speaking.  One could tell someone to do 

```
sage: arg(sqrt(2)+i).simplify()
arctan(1/2*sqrt(2))
```



---

archive/issue_comments_027286.json:
```json
{
    "body": "revision of arg function--apply instead of the previous patch",
    "created_at": "2012-01-12T17:06:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27286",
    "user": "https://github.com/sagetrac-ktkohl"
}
```

revision of arg function--apply instead of the previous patch



---

archive/issue_comments_027287.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nAttachment: **[trac_4498_arg.2.patch.gz](https://github.com/sagemath/sage/files/ticket4498/trac_4498_arg.2.patch.gz)**",
    "created_at": "2012-01-12T17:06:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27287",
    "user": "https://github.com/sagetrac-ktkohl"
}
```

<a id='comment:16'>Comment 16:</a>
Attachment: **[trac_4498_arg.2.patch.gz](https://github.com/sagemath/sage/files/ticket4498/trac_4498_arg.2.patch.gz)**



---

archive/issue_events_044510.json:
```json
{
    "actor": "https://github.com/sagetrac-ktkohl",
    "created_at": "2012-01-12T17:06:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44510"
}
```



---

archive/issue_events_044511.json:
```json
{
    "actor": "https://github.com/sagetrac-ktkohl",
    "created_at": "2012-01-12T17:06:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44511"
}
```



---

archive/issue_comments_027288.json:
```json
{
    "body": "Attachment: **[trac_4498_arg.3.patch.gz](https://github.com/sagemath/sage/files/ticket4498/trac_4498_arg.3.patch.gz)**\n\nsymbolic arg--fixed whitespace issues--use this one instead of others",
    "created_at": "2012-01-12T18:41:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27288",
    "user": "https://github.com/sagetrac-ktkohl"
}
```

Attachment: **[trac_4498_arg.3.patch.gz](https://github.com/sagemath/sage/files/ticket4498/trac_4498_arg.3.patch.gz)**

symbolic arg--fixed whitespace issues--use this one instead of others



---

archive/issue_comments_027289.json:
```json
{
    "body": "Attachment: **[trac_4498-symbolic_arg.cleanup.patch.gz](https://github.com/sagemath/sage/files/ticket4498/trac_4498-symbolic_arg.cleanup.patch.gz)**",
    "created_at": "2012-02-07T10:11:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27289",
    "user": "https://github.com/burcin"
}
```

Attachment: **[trac_4498-symbolic_arg.cleanup.patch.gz](https://github.com/sagemath/sage/files/ticket4498/trac_4498-symbolic_arg.cleanup.patch.gz)**



---

archive/issue_comments_027290.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nI uploaded a new patch with minor modifications to Karen's. In particular, it\n\n* removes the duplicate commands that appear in the `EXAMPLES` and `TESTS` blocks for `Function_arg`.\n* import `CC` directly instead of calling `ComplexField` in `_evalf_()`.\n\nI give a positive review to Karen's patch. If someone can take a quick look at my changes to check if I didn't mess anything up, this can be merged.",
    "created_at": "2012-02-07T10:30:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27290",
    "user": "https://github.com/burcin"
}
```

<a id='comment:17'>Comment 17:</a>
I uploaded a new patch with minor modifications to Karen's. In particular, it

* removes the duplicate commands that appear in the `EXAMPLES` and `TESTS` blocks for `Function_arg`.
* import `CC` directly instead of calling `ComplexField` in `_evalf_()`.

I give a positive review to Karen's patch. If someone can take a quick look at my changes to check if I didn't mess anything up, this can be merged.



---

archive/issue_comments_027291.json:
```json
{
    "body": "Changed reviewer from **Karl-Dieter Crisman** to **Karl-Dieter Crisman, Burcin Erocal**",
    "created_at": "2012-02-07T10:30:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27291",
    "user": "https://github.com/burcin"
}
```

Changed reviewer from **Karl-Dieter Crisman** to **Karl-Dieter Crisman, Burcin Erocal**



---

archive/issue_comments_027292.json:
```json
{
    "body": "<a id='comment:18'>Comment 18:</a>\nIt doesn't appear that you messed anything up, except ...\n\n```\nsage: arg(3.0)\n---------------------------------------------------------------------------\n<snip>\n   1426             return x.arg()\n   1427         except AttributeError:\n-> 1428             from sage.rings.complex_field import CC\n   1429             x = CC(x)\n   1430             return x.arg()\n\nImportError: cannot import name CC\n```\nSo apparently that won't work.  Otherwise the changes are fine.",
    "created_at": "2012-02-07T14:14:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27292",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:18'>Comment 18:</a>
It doesn't appear that you messed anything up, except ...

```
sage: arg(3.0)
---------------------------------------------------------------------------
<snip>
   1426             return x.arg()
   1427         except AttributeError:
-> 1428             from sage.rings.complex_field import CC
   1429             x = CC(x)
   1430             return x.arg()

ImportError: cannot import name CC
```
So apparently that won't work.  Otherwise the changes are fine.



---

archive/issue_events_044512.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-02-07T14:14:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44512"
}
```



---

archive/issue_events_044513.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-02-07T14:14:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44513"
}
```



---

archive/issue_comments_027293.json:
```json
{
    "body": "Attachment: **[trac_4498-arg_evalf.patch.gz](https://github.com/sagemath/sage/files/ticket4498/trac_4498-arg_evalf.patch.gz)**",
    "created_at": "2012-02-07T15:51:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27293",
    "user": "https://github.com/burcin"
}
```

Attachment: **[trac_4498-arg_evalf.patch.gz](https://github.com/sagemath/sage/files/ticket4498/trac_4498-arg_evalf.patch.gz)**



---

archive/issue_events_044514.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2012-02-07T15:54:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44514"
}
```



---

archive/issue_events_044515.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2012-02-07T15:54:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44515"
}
```



---

archive/issue_comments_027294.json:
```json
{
    "body": "<a id='comment:19'>Comment 19:</a>\nApparently I didn't run tests, latex output was also broken.\n\n[attachment: trac_4498-arg_evalf.patch,.gz](https://github.com/sagemath/sage/files/ticket4498/71eff9484b402d2cc3d109efd1bc232d.gz) to be applied after [attachment: trac_4498-symbolic_arg.cleanup.patch,.gz](https://github.com/sagemath/sage/files/ticket4498/2edfecd0e76379c21d1f88e59dd03d9d.gz) implements a new `_evalf_()` function which keeps the precision of the input. This one needs a real review. :)",
    "created_at": "2012-02-07T15:54:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27294",
    "user": "https://github.com/burcin"
}
```

<a id='comment:19'>Comment 19:</a>
Apparently I didn't run tests, latex output was also broken.

[attachment: trac_4498-arg_evalf.patch,.gz](https://github.com/sagemath/sage/files/ticket4498/71eff9484b402d2cc3d109efd1bc232d.gz) to be applied after [attachment: trac_4498-symbolic_arg.cleanup.patch,.gz](https://github.com/sagemath/sage/files/ticket4498/2edfecd0e76379c21d1f88e59dd03d9d.gz) implements a new `_evalf_()` function which keeps the precision of the input. This one needs a real review. :)



---

archive/issue_comments_027295.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -20,3 +20,7 @@\n \n \n \"The function should return the argument of a complex function.\" - Ronan Paix\u00e3o\n+\n+Apply\n+* [attachment: trac_4498-symbolic_arg.cleanup.patch.gz](https://github.com/sagemath/sage/files/ticket4498/trac_4498-symbolic_arg.cleanup.patch.gz)\n+* [attachment: trac_4498-arg_evalf.patch.gz](https://github.com/sagemath/sage/files/ticket4498/trac_4498-arg_evalf.patch.gz)\n``````\n",
    "created_at": "2012-02-07T15:54:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27295",
    "user": "https://github.com/burcin"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -20,3 +20,7 @@
 
 
 "The function should return the argument of a complex function." - Ronan Paixão
+
+Apply
+* [attachment: trac_4498-symbolic_arg.cleanup.patch.gz](https://github.com/sagemath/sage/files/ticket4498/trac_4498-symbolic_arg.cleanup.patch.gz)
+* [attachment: trac_4498-arg_evalf.patch.gz](https://github.com/sagemath/sage/files/ticket4498/trac_4498-arg_evalf.patch.gz)
``````




---

archive/issue_comments_027296.json:
```json
{
    "body": "Changed author from **Karen T. Kohl** to **Karen T. Kohl, Burcin Erocal**",
    "created_at": "2012-02-07T15:54:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27296",
    "user": "https://github.com/burcin"
}
```

Changed author from **Karen T. Kohl** to **Karen T. Kohl, Burcin Erocal**



---

archive/issue_comments_027297.json:
```json
{
    "body": "<a id='comment:20'>Comment 20:</a>\nThis makes a lot more sense.  Running tests...\n\nWhy `parent_d` and not `parent`?  Just wondering in case there is a convention I should be aware of.",
    "created_at": "2012-02-07T16:08:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27297",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:20'>Comment 20:</a>
This makes a lot more sense.  Running tests...

Why `parent_d` and not `parent`?  Just wondering in case there is a convention I should be aware of.



---

archive/issue_comments_027298.json:
```json
{
    "body": "<a id='comment:21'>Comment 21:</a>\nUsing `parent` as the name of the keyword argument masks the imported `parent()` function, which I used in the function body.",
    "created_at": "2012-02-07T16:13:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27298",
    "user": "https://github.com/burcin"
}
```

<a id='comment:21'>Comment 21:</a>
Using `parent` as the name of the keyword argument masks the imported `parent()` function, which I used in the function body.



---

archive/issue_events_044516.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2012-02-07T16:13:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "milestone_number": null,
    "milestone_title": "sage-5.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44516"
}
```



---

archive/issue_events_044517.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2012-02-07T16:13:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44517"
}
```



---

archive/issue_comments_027299.json:
```json
{
    "body": "<a id='comment:22'>Comment 22:</a>\nI wondered; that makes sense.\n\nAll looks well.  Just a question - do you want to include any of the following as tests?\n\n```\nsage: arg(long(1000))\n0\nsage: arg(1j)\n1.57079632679490\nsage: arg(1J)\n1.57079632679490\nsage: arg(complex(0,1))\n1.57079632679490\nsage: arg(complex(1,0))\n0.000000000000000\nsage: arg(int(10))\n0\n```\nIt's not a big deal to me either way, I just wanted to test them.",
    "created_at": "2012-02-07T16:39:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27299",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:22'>Comment 22:</a>
I wondered; that makes sense.

All looks well.  Just a question - do you want to include any of the following as tests?

```
sage: arg(long(1000))
0
sage: arg(1j)
1.57079632679490
sage: arg(1J)
1.57079632679490
sage: arg(complex(0,1))
1.57079632679490
sage: arg(complex(1,0))
0.000000000000000
sage: arg(int(10))
0
```
It's not a big deal to me either way, I just wanted to test them.



---

archive/issue_events_044518.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-02-07T16:39:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44518"
}
```



---

archive/issue_events_044519.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2012-02-07T16:39:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44519"
}
```



---

archive/issue_events_044520.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-12T12:47:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44520"
}
```



---

archive/issue_events_044521.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-12T12:47:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44521"
}
```



---

archive/issue_comments_027300.json:
```json
{
    "body": "<a id='comment:23'>Comment 23:</a>\nThis patch conflicts with #9130.  Either this one or #9130 should be rebased.",
    "created_at": "2012-02-12T12:47:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27300",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:23'>Comment 23:</a>
This patch conflicts with #9130.  Either this one or #9130 should be rebased.



---

archive/issue_events_044522.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-13T09:35:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44522"
}
```



---

archive/issue_events_044523.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-13T09:35:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44523"
}
```



---

archive/issue_comments_027301.json:
```json
{
    "body": "Changed work issues from **random tests, Maxima** to none",
    "created_at": "2012-02-13T09:35:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27301",
    "user": "https://github.com/jdemeyer"
}
```

Changed work issues from **random tests, Maxima** to none



---

archive/issue_comments_027302.json:
```json
{
    "body": "Merged: **sage-5.0.beta4**",
    "created_at": "2012-02-14T14:20:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27302",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.0.beta4**



---

archive/issue_events_044524.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-14T14:20:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44524"
}
```



---

archive/issue_events_044525.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-02-14T14:20:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4498#event-44525"
}
```



---

archive/issue_comments_027303.json:
```json
{
    "body": "Changed author from **Karen T. Kohl, Burcin Erocal** to **Karen Kohl, Burcin Erocal**",
    "created_at": "2017-07-19T08:21:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27303",
    "user": "https://github.com/fchapoton"
}
```

Changed author from **Karen T. Kohl, Burcin Erocal** to **Karen Kohl, Burcin Erocal**



---

archive/issue_comments_027304.json:
```json
{
    "body": "<a id='comment:26'>Comment 26:</a>\nstandard form for name",
    "created_at": "2017-07-19T08:21:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4498",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4498#issuecomment-27304",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:26'>Comment 26:</a>
standard form for name
