# Issue 4558: update to sympy-0.6.3.spkg (latest upstream)

archive/issues_004558.json:
```json
{
    "assignees": [],
    "body": "The spkg is here:\n\nhttp://sage.math.washington.edu/home/ondrej/spkg/sympy-0.6.3.spkg\n\nand also apply the attached patch, that fixes one failing test in test_sympy.py.\n\nThe Sage tests are still running, I'll report if all pass.\n\nAssignee: **@burcin**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4558_\n\n",
    "closed_at": "2008-11-23T00:58:22Z",
    "created_at": "2008-11-20T00:21:33Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20calculus",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.2.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "update to sympy-0.6.3.spkg (latest upstream)",
    "type": "issue",
    "updated_at": "2008-11-23T00:58:22Z",
    "url": "https://github.com/sagemath/sage/issues/4558",
    "user": "https://github.com/certik"
}
```
The spkg is here:

http://sage.math.washington.edu/home/ondrej/spkg/sympy-0.6.3.spkg

and also apply the attached patch, that fixes one failing test in test_sympy.py.

The Sage tests are still running, I'll report if all pass.

Assignee: **@burcin**

_Issue created by migration from https://trac.sagemath.org/ticket/4558_





---

archive/issue_events_046492.json:
```json
{
    "actor": "https://github.com/certik",
    "created_at": "2008-11-20T00:21:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4558",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20calculus",
    "label_color": "0000ff",
    "label_name": "component: calculus",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4558#event-46492"
}
```



---

archive/issue_events_046493.json:
```json
{
    "actor": "https://github.com/certik",
    "created_at": "2008-11-20T00:21:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4558",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4558#event-46493"
}
```



---

archive/issue_events_046494.json:
```json
{
    "actor": "https://github.com/certik",
    "created_at": "2008-11-20T00:21:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4558",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4558#event-46494"
}
```



---

archive/issue_comments_028124.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nAttachment: **[sympy-0.6.3.patch.gz](https://github.com/sagemath/sage/files/ticket4558/sympy-0.6.3.patch.gz)**\n\nOk, all tests pass:\n\nhttp://sage.math.washington.edu/home/ondrej/sympy-0.6.3-sage-tests.log",
    "created_at": "2008-11-20T01:06:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4558",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4558#issuecomment-28124",
    "user": "https://github.com/certik"
}
```

<div id="comment:1" align="right">Comment 1</div>

Attachment: **[sympy-0.6.3.patch.gz](https://github.com/sagemath/sage/files/ticket4558/sympy-0.6.3.patch.gz)**

Ok, all tests pass:

http://sage.math.washington.edu/home/ondrej/sympy-0.6.3-sage-tests.log



---

archive/issue_events_046495.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-20T01:07:28Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4558",
    "title_is": "update to sympy-0.6.3.spkg (latest upstream)",
    "title_was": "please update to sympy-0.6.3.spkg",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4558#event-46495"
}
```



---

archive/issue_events_046496.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-20T01:07:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4558",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4558#event-46496"
}
```



---

archive/issue_events_046497.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-20T01:07:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4558",
    "milestone_number": null,
    "milestone_title": "sage-3.2.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4558#event-46497"
}
```



---

archive/issue_comments_028125.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nOndrej,\n\nin the future make sure to use the sympy.spkg from the tree, not the one you build last time since the one in tree contains various cleanups from the review. Next time you do not use the latest upstream the review on my end will be an automatic \"needs work\" since I am tired of forward porting fixes I have done over and over again.\n\nCheers,\n\nMichael",
    "created_at": "2008-11-23T00:49:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4558",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4558#issuecomment-28125",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">Comment 3</div>

Ondrej,

in the future make sure to use the sympy.spkg from the tree, not the one you build last time since the one in tree contains various cleanups from the review. Next time you do not use the latest upstream the review on my end will be an automatic "needs work" since I am tired of forward porting fixes I have done over and over again.

Cheers,

Michael



---

archive/issue_events_046498.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-23T00:57:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4558",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4558#event-46498"
}
```



---

archive/issue_events_046499.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-23T00:57:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4558",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4558#event-46499"
}
```



---

archive/issue_comments_028126.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nPositive review on the spkg and the patch - I have some cleanups in \n\n http://sage.math.washington.edu/home/mabshoff/release-cycles-3.2.1/alpha0/sympy-0.6.3.p0.spkg\n\nCheers,\n\nMichael",
    "created_at": "2008-11-23T00:57:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4558",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4558#issuecomment-28126",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">Comment 4</div>

Positive review on the spkg and the patch - I have some cleanups in 

 http://sage.math.washington.edu/home/mabshoff/release-cycles-3.2.1/alpha0/sympy-0.6.3.p0.spkg

Cheers,

Michael



---

archive/issue_events_046500.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-23T00:58:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4558",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4558#event-46500"
}
```



---

archive/issue_events_046501.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-23T00:58:22Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4558",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4558#event-46501"
}
```



---

archive/issue_comments_028127.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nMerged in Sage 3.2.1.alpha0",
    "created_at": "2008-11-23T00:58:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4558",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4558#issuecomment-28127",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">Comment 5</div>

Merged in Sage 3.2.1.alpha0
