# Issue 4977: [with patch, positive review] vector(RR vector) doesn't create a new vector

archive/issues_004977.json:
```json
{
    "body": "\n```\nHi,\n\nIs there a reason why, in sage 3.2.2, the following works :\n\nsage: vector(vector((1, 6)))\n(1, 6)\n\nbut the following doesn't :\n\nsage: vector(vector((1, 6.8)))\nTraceback (most recent call last):\n...\nTypeError: _vector_() takes exactly one argument (0 given)\n\n???\n\nThank you,\n\nS\u00e9bastien Labb\u00e9\nUQAM\n\n\n```\n\n\nAssignee: @williamstein\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4977\n\n",
    "closed_at": "2009-01-23T08:03:28Z",
    "created_at": "2009-01-14T21:44:34Z",
    "labels": [
        "component: linear algebra",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.3",
    "title": "[with patch, positive review] vector(RR vector) doesn't create a new vector",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4977",
    "user": "https://github.com/jasongrout"
}
```

```
Hi,

Is there a reason why, in sage 3.2.2, the following works :

sage: vector(vector((1, 6)))
(1, 6)

but the following doesn't :

sage: vector(vector((1, 6.8)))
Traceback (most recent call last):
...
TypeError: _vector_() takes exactly one argument (0 given)

???

Thank you,

Sébastien Labbé
UQAM


```


Assignee: @williamstein

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4977





---

archive/issue_comments_040278.json:
```json
{
    "body": "I want also to mention that the same problem was occuring when using the Symbolic Ring :\n\n```\nsage: vector(vector(SR, (1, sqrt(2)) ) )\n```",
    "created_at": "2009-01-15T15:20:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4977",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4977#issuecomment-40278",
    "user": "https://github.com/seblabbe"
}
```

I want also to mention that the same problem was occuring when using the Symbolic Ring :

```
sage: vector(vector(SR, (1, sqrt(2)) ) )
```



---

archive/issue_comments_040279.json:
```json
{
    "body": "<a id='comment:2'></a>\nAttachment [4977.patch](tarball://root/attachments/some-uuid/ticket4977/4977.patch) by @ncalexan created at 2009-01-21 22:46:24",
    "created_at": "2009-01-21T22:46:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4977",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4977#issuecomment-40279",
    "user": "https://github.com/ncalexan"
}
```

<a id='comment:2'></a>
Attachment [4977.patch](tarball://root/attachments/some-uuid/ticket4977/4977.patch) by @ncalexan created at 2009-01-21 22:46:24



---

archive/issue_events_019282.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4977",
    "rename": {
        "from": "vector(RR vector) doesn't create a new vector",
        "to": "[with patch, needs review] vector(RR vector) doesn't create a new vector"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4977#event-19282"
}
```



---

archive/issue_events_019283.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4977",
    "rename": {
        "from": "[with patch, needs review] vector(RR vector) doesn't create a new vector",
        "to": "[with patch, positive review] vector(RR vector) doesn't create a new vector"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4977#event-19283"
}
```



---

archive/issue_comments_040280.json:
```json
{
    "body": "<a id='comment:3'></a>\nLooks good.",
    "created_at": "2009-01-22T00:12:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4977",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4977#issuecomment-40280",
    "user": "https://github.com/aghitza"
}
```

<a id='comment:3'></a>
Looks good.



---

archive/issue_comments_040281.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-01-23T08:03:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4977",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4977#issuecomment-40281",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_040282.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.3.alpha1",
    "created_at": "2009-01-23T08:03:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4977",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4977#issuecomment-40282",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.3.alpha1



---

archive/issue_events_019284.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-01-23T08:03:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4977",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4977#event-19284"
}
```
