# Issue 4985: [with patch, positive review] Expand documentation for list_plot to point out the utility of zip

archive/issues_004985.json:
```json
{
    "body": "We should expand the documentation for list_plot so that questions like on the latter half of this thread don't happen:\n\nhttp://groups.google.com/group/sage-support/browse_thread/thread/e523b8ade175746c\n\nBasically, we should explain how to use zip like thus:\n\nlist_plot(zip(list of x-coords, list of y-coords))\n\nso that people from Matlab don't get confused by the very unhelpful error message.  Maybe the error message ought to be changed too.\n\nAssignee: @williamstein\n\nCC:  @jhpalmieri\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4985\n\n",
    "closed_at": "2009-03-25T06:08:13Z",
    "created_at": "2009-01-16T02:50:47Z",
    "labels": [
        "component: graphics",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.4.1",
    "title": "[with patch, positive review] Expand documentation for list_plot to point out the utility of zip",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4985",
    "user": "https://github.com/jasongrout"
}
```
We should expand the documentation for list_plot so that questions like on the latter half of this thread don't happen:

http://groups.google.com/group/sage-support/browse_thread/thread/e523b8ade175746c

Basically, we should explain how to use zip like thus:

list_plot(zip(list of x-coords, list of y-coords))

so that people from Matlab don't get confused by the very unhelpful error message.  Maybe the error message ought to be changed too.

Assignee: @williamstein

CC:  @jhpalmieri

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4985





---

archive/issue_comments_040359.json:
```json
{
    "body": "<a id='comment:1'></a>\nGood idea, IMHO. \nI wonder if it also a good idea to add plot_list as an alias to the namespace so people who hunt for plot commands (and the documentation for them) using tab completion can find this more easily?",
    "created_at": "2009-01-16T02:55:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4985#issuecomment-40359",
    "user": "https://github.com/wdjoyner"
}
```

<a id='comment:1'></a>
Good idea, IMHO. 
I wonder if it also a good idea to add plot_list as an alias to the namespace so people who hunt for plot commands (and the documentation for them) using tab completion can find this more easily?



---

archive/issue_events_019318.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4985",
    "rename": {
        "from": "Expand documentation for list_plot to point out the utility of zip",
        "to": "[with patch, needs review] Expand documentation for list_plot to point out the utility of zip"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4985#event-19318"
}
```



---

archive/issue_comments_040360.json:
```json
{
    "body": "<a id='comment:2'></a>\nHere's a patch, adding to the list_plot docstring.  I feel neutral, or maybe a little negative, about the suggestion to add plot_list as an alias.  Would we then need to do the same for list_plot3d?  Anyway, that can be dealt with as a separate ticket.\n\nBy the way, I wasn't sure what \"the very unhelpful error message\" is. We might consider testing whether the second argument to list_plot (which is \"plotjoined\", should be boolean) is a list or tuple, and then print a warning, because perhaps someone ran \"list_plot([list1], [list2])\" without meaning to.  Opinions?",
    "created_at": "2009-03-22T23:51:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4985#issuecomment-40360",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:2'></a>
Here's a patch, adding to the list_plot docstring.  I feel neutral, or maybe a little negative, about the suggestion to add plot_list as an alias.  Would we then need to do the same for list_plot3d?  Anyway, that can be dealt with as a separate ticket.

By the way, I wasn't sure what "the very unhelpful error message" is. We might consider testing whether the second argument to list_plot (which is "plotjoined", should be boolean) is a list or tuple, and then print a warning, because perhaps someone ran "list_plot([list1], [list2])" without meaning to.  Opinions?



---

archive/issue_comments_040361.json:
```json
{
    "body": "<a id='comment:3'></a>\nAttachment [4985.patch](tarball://root/attachments/some-uuid/ticket4985/4985.patch) by @jasongrout created at 2009-03-23 21:16:08\n\nNice!  Positive review.",
    "created_at": "2009-03-23T21:16:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4985#issuecomment-40361",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:3'></a>
Attachment [4985.patch](tarball://root/attachments/some-uuid/ticket4985/4985.patch) by @jasongrout created at 2009-03-23 21:16:08

Nice!  Positive review.



---

archive/issue_comments_040362.json:
```json
{
    "body": "<a id='comment:4'></a>\n(applies cleanly to 3.4, doctests in plot.py pass, and the plot looks right).",
    "created_at": "2009-03-23T21:20:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4985#issuecomment-40362",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:4'></a>
(applies cleanly to 3.4, doctests in plot.py pass, and the plot looks right).



---

archive/issue_events_019319.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4985",
    "rename": {
        "from": "[with patch, needs review] Expand documentation for list_plot to point out the utility of zip",
        "to": "[with patch, positive review] Expand documentation for list_plot to point out the utility of zip"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4985#event-19319"
}
```



---

archive/issue_comments_040363.json:
```json
{
    "body": "<a id='comment:5'></a>\n#5594 is for the suggestion by jhpalmieri above.",
    "created_at": "2009-03-23T21:23:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4985#issuecomment-40363",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:5'></a>
#5594 is for the suggestion by jhpalmieri above.



---

archive/issue_events_019320.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-03-25T06:08:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4985",
    "milestone": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4985#event-19320"
}
```



---

archive/issue_comments_040364.json:
```json
{
    "body": "<a id='comment:6'></a>\nMerged in Sage 3.4.1.alpha0.\n\nCheers,\n\nMichael",
    "created_at": "2009-03-25T06:08:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4985#issuecomment-40364",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:6'></a>
Merged in Sage 3.4.1.alpha0.

Cheers,

Michael



---

archive/issue_comments_040365.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-03-25T06:08:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4985",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4985#issuecomment-40365",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_019321.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-03-25T06:08:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4985",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4985#event-19321"
}
```
