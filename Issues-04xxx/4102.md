...
<a id='comment:9'></a>Replying to [comment:8 benjaminfjones]:
> What new names do people like in the interim? 
> 
> * `bessel_J_symbolic`
> * `bessel_J_sym`
...
<a id='comment:11'></a>Replying to [comment:8 benjaminfjones]:
> What new names do people like in the interim? 
...
<a id='comment:16'></a>Replying to [comment:14 burcin]:
...
<a id='comment:25'></a>Replying to [comment:22 kcrisman]:
...
```json
{
    "body": "<a id='comment:46'></a>Replying to [comment:45 benjaminfjones]:\n> So it looks like there is some hidden state affecting the doctesting in scenario (2). \n\nThe doctests which are run *before* that test are extra \"state\".",
...
<a id='comment:46'></a>Replying to [comment:45 benjaminfjones]:
> So it looks like there is some hidden state affecting the doctesting in scenario (2). 
...
<a id='comment:48'></a>I still have not been able to track down the doctesting issue (described starting at comment:41). I've tried bisecting the doctest state (delta debugging) to no avail. The problem seems to be intimately linked to the doctesting environment. I'm unable to reproduce the doctest failure when running __all__ the prior doctests which come before in `bessel.py`, in the right order, by hand, and then running the offending one.
...
<a id='comment:50'></a>Replying to [comment:49 benjaminfjones]:
> Tests are passing on 5.10.beta4. The startup time plugin is failing, however. Should we lazy import here? Also how about lazy import for all the special functions (the idea being that relatively few users will need any given special function)?
...
<a id='comment:54'></a>Replying to [comment:53 benjaminfjones]:
> `@`vbraun: Thanks! That's good detective work. How did you figure that out
...
<a id='comment:84'></a>Replying to [comment:83 eviatarbach]:
> New patch fixes this issue. Can this be merged in 5.11?
...
<a id='comment:86'></a>Replying to [comment:83 eviatarbach]:
> New patch fixes this issue. Can this be merged in 5.11?
