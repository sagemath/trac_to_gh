# Issue 4060: [with patch, positive review] Polyhedra don't handle real coordinates properly

archive/issues_004060.json:
```json
{
    "body": "Dear All,\n\nI have a question related to the polyhedra module.\nWhen I define a polyhedron using float rather than integer\ncoordinates, I get a weird behavior from the vert_to_ieq function.\nFor example, if I type something like this:\n\np = [[1.1, 2.2], [3.3, 4.4]]\nvert_to_ieq(p, cdd_type=\"real\") \n\nAssignee: somebody\n\nKeywords: polyhedra\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4060\n\n",
    "closed_at": "2008-09-19T03:20:08Z",
    "created_at": "2008-09-04T13:44:12Z",
    "labels": [
        "component: geometry",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1.3",
    "title": "[with patch, positive review] Polyhedra don't handle real coordinates properly",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4060",
    "user": "https://trac.sagemath.org/admin/accounts/users/mhampton"
}
```
Dear All,

I have a question related to the polyhedra module.
When I define a polyhedron using float rather than integer
coordinates, I get a weird behavior from the vert_to_ieq function.
For example, if I type something like this:

p = [[1.1, 2.2], [3.3, 4.4]]
vert_to_ieq(p, cdd_type="real") 

Assignee: somebody

Keywords: polyhedra

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4060





---

archive/issue_comments_030889.json:
```json
{
    "body": "Attachment [trac-4060-patch1.patch](tarball://root/attachments/some-uuid/ticket4060/trac-4060-patch1.patch) by mhampton created at 2008-09-04 20:36:38",
    "created_at": "2008-09-04T20:36:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4060",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4060#issuecomment-30889",
    "user": "https://trac.sagemath.org/admin/accounts/users/mhampton"
}
```

Attachment [trac-4060-patch1.patch](tarball://root/attachments/some-uuid/ticket4060/trac-4060-patch1.patch) by mhampton created at 2008-09-04 20:36:38



---

archive/issue_comments_030890.json:
```json
{
    "body": "Changing assignee from mhampton to somebody.",
    "created_at": "2008-09-04T20:38:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4060",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4060#issuecomment-30890",
    "user": "https://trac.sagemath.org/admin/accounts/users/mhampton"
}
```

Changing assignee from mhampton to somebody.



---

archive/issue_comments_030891.json:
```json
{
    "body": "<a id='comment:1'></a>\nOK, I think I have resolved this problem.  I added a doctest to the Polyhedron class to test this.",
    "created_at": "2008-09-04T20:38:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4060",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4060#issuecomment-30891",
    "user": "https://trac.sagemath.org/admin/accounts/users/mhampton"
}
```

<a id='comment:1'></a>
OK, I think I have resolved this problem.  I added a doctest to the Polyhedron class to test this.



---

archive/issue_events_015139.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4060",
    "rename": {
        "from": "Polyhedra don't handle real coordinates properly",
        "to": "[with patch, needs review] Polyhedra don't handle real coordinates properly"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4060#event-15139"
}
```



---

archive/issue_comments_030892.json:
```json
{
    "body": "<a id='comment:2'></a>\nThis looks good to me.",
    "created_at": "2008-09-19T00:43:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4060",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4060#issuecomment-30892",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:2'></a>
This looks good to me.



---

archive/issue_events_015140.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4060",
    "rename": {
        "from": "[with patch, needs review] Polyhedra don't handle real coordinates properly",
        "to": "[with patch, positive review] Polyhedra don't handle real coordinates properly"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4060#event-15140"
}
```



---

archive/issue_comments_030893.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-09-19T03:20:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4060",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4060#issuecomment-30893",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_015141.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-19T03:20:08Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4060",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4060#event-15141"
}
```



---

archive/issue_comments_030894.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.1.3.alpha0",
    "created_at": "2008-09-19T03:20:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4060",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4060#issuecomment-30894",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.1.3.alpha0
