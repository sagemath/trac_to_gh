# Issue 4562: [with patch, positive review] clean up a bunch of imports in matrix CDF/RDF code

archive/issues_004562.json:
```json
{
    "body": "This patch cleans up a bunch of unneeded imports.\n\nAssignee: @williamstein\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4562\n\n",
    "closed_at": "2008-11-23T01:59:50Z",
    "created_at": "2008-11-20T06:23:21Z",
    "labels": [
        "component: linear algebra",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.2.1",
    "title": "[with patch, positive review] clean up a bunch of imports in matrix CDF/RDF code",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4562",
    "user": "https://github.com/jasongrout"
}
```
This patch cleans up a bunch of unneeded imports.

Assignee: @williamstein

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4562





---

archive/issue_comments_036187.json:
```json
{
    "body": "Attachment [matrix-CDF-RDF-cleanup-imports.patch](tarball://root/attachments/some-uuid/ticket4562/matrix-CDF-RDF-cleanup-imports.patch) by @jasongrout created at 2008-11-20 06:23:36",
    "created_at": "2008-11-20T06:23:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4562",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4562#issuecomment-36187",
    "user": "https://github.com/jasongrout"
}
```

Attachment [matrix-CDF-RDF-cleanup-imports.patch](tarball://root/attachments/some-uuid/ticket4562/matrix-CDF-RDF-cleanup-imports.patch) by @jasongrout created at 2008-11-20 06:23:36



---

archive/issue_comments_036188.json:
```json
{
    "body": "<a id='comment:1'></a>\nThis is based on sage 3.2.rc2",
    "created_at": "2008-11-20T06:24:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4562",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4562#issuecomment-36188",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:1'></a>
This is based on sage 3.2.rc2



---

archive/issue_events_017303.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4562",
    "rename": {
        "from": "clean up a bunch of imports in matrix CDF/RDF code",
        "to": "[with patch, needs review] clean up a bunch of imports in matrix CDF/RDF code"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4562#event-17303"
}
```



---

archive/issue_events_017304.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4562",
    "rename": {
        "from": "[with patch, needs review] clean up a bunch of imports in matrix CDF/RDF code",
        "to": "[with patch, positive review] clean up a bunch of imports in matrix CDF/RDF code"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4562#event-17304"
}
```



---

archive/issue_comments_036189.json:
```json
{
    "body": "<a id='comment:3'></a>\nPatch looks reasonable, compiles, and all doctests in sage/matrix continue to pass after the patch.\n\nPositive review.",
    "created_at": "2008-11-23T01:21:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4562",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4562#issuecomment-36189",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```

<a id='comment:3'></a>
Patch looks reasonable, compiles, and all doctests in sage/matrix continue to pass after the patch.

Positive review.



---

archive/issue_events_017305.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-23T01:59:50Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4562",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4562#event-17305"
}
```



---

archive/issue_comments_036190.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-11-23T01:59:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4562",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4562#issuecomment-36190",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_036191.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.2.1.alpha0",
    "created_at": "2008-11-23T01:59:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4562",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4562#issuecomment-36191",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.2.1.alpha0
