# Issue 4497: Implement the function log10

archive/issues_004497.json:
```json
{
    "body": "\"very useful for those who use a lot logarithmic scale\" - Ronan Paix\u00e3o\n\nAssignee: somebody\n\nResolution: invalid\n\nIssue created by migration from https://trac.sagemath.org/ticket/4497\n\n",
    "closed_at": "2008-11-12T17:09:38Z",
    "created_at": "2008-11-11T23:19:57Z",
    "labels": [
        "component: basic arithmetic",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "Implement the function log10",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4497",
    "user": "https://trac.sagemath.org/admin/accounts/users/TimothyClemans"
}
```
"very useful for those who use a lot logarithmic scale" - Ronan Paixão

Assignee: somebody

Resolution: invalid

Issue created by migration from https://trac.sagemath.org/ticket/4497





---

archive/issue_comments_034045.json:
```json
{
    "body": "<a id='comment:1'></a>Why this ticket? We already have\n\n```\nsage: log_b?\nType:\t\tfunction\nBase Class:\t<type 'function'>\nString Form:\t<function log at 0x29b33f0>\nNamespace:\tInteractive\nFile:\t\t/Users/michaelabshoff/Desktop/sage-3.1.3.rc0/local/lib/python2.5/site-packages/sage/misc/functional.py\nDefinition:\tlog_b(x, b=None)\nDocstring:\n    \n        Return the log of x to the base b.  The default base is e.\n    \n        INPUT:\n            x -- number\n            b -- base (default: None, which means natural log)\n            \n        OUTPUT:\n            number\n    \n        NOTE: In Magma, the order of arguments is reversed from in\n        SAGE, i.e., the base is given first.  We use the opposite\n        ordering, so the base can be viewed as an optional second\n        argument.\n```\nOne could obviously implement log10, but why pollute the global namespace even more?\n\nCheers,\n\nMichael",
    "created_at": "2008-11-12T14:17:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4497#issuecomment-34045",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>Why this ticket? We already have

```
sage: log_b?
Type:		function
Base Class:	<type 'function'>
String Form:	<function log at 0x29b33f0>
Namespace:	Interactive
File:		/Users/michaelabshoff/Desktop/sage-3.1.3.rc0/local/lib/python2.5/site-packages/sage/misc/functional.py
Definition:	log_b(x, b=None)
Docstring:
    
        Return the log of x to the base b.  The default base is e.
    
        INPUT:
            x -- number
            b -- base (default: None, which means natural log)
            
        OUTPUT:
            number
    
        NOTE: In Magma, the order of arguments is reversed from in
        SAGE, i.e., the base is given first.  We use the opposite
        ordering, so the base can be viewed as an optional second
        argument.
```
One could obviously implement log10, but why pollute the global namespace even more?

Cheers,

Michael



---

archive/issue_events_010184.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-12T17:09:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4497",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4497#event-10184"
}
```



---

archive/issue_comments_034046.json:
```json
{
    "body": "<a id='comment:2'></a>Pari, Maple, and Mathematica all don't do this.   We should not do this in Sage either. It is trivial to implement in terms of existing functions.",
    "created_at": "2008-11-12T17:09:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4497#issuecomment-34046",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:2'></a>Pari, Maple, and Mathematica all don't do this.   We should not do this in Sage either. It is trivial to implement in terms of existing functions.



---

archive/issue_comments_034047.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2008-11-12T17:09:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4497",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4497#issuecomment-34047",
    "user": "https://github.com/williamstein"
}
```

Resolution: invalid



---

archive/issue_events_010185.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-14T06:02:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4497",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4497#event-10185"
}
```
