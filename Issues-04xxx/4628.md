# Issue 4628: [with patch; with positive review] sage-3.2.1.alpha1 -- setup.py build system is foobar'd

archive/issues_004628.json:
```json
{
    "body": "\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4628\n\n",
    "closed_at": "2008-11-27T02:02:47Z",
    "created_at": "2008-11-26T22:33:45Z",
    "labels": [
        "component: build",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.2.1",
    "title": "[with patch; with positive review] sage-3.2.1.alpha1 -- setup.py build system is foobar'd",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4628",
    "user": "https://github.com/williamstein"
}
```


Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4628





---

archive/issue_comments_036898.json:
```json
{
    "body": "Attachment [sage-4628.patch](tarball://root/attachments/some-uuid/ticket4628/sage-4628.patch) by @williamstein created at 2008-11-26 22:35:02",
    "created_at": "2008-11-26T22:35:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4628",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4628#issuecomment-36898",
    "user": "https://github.com/williamstein"
}
```

Attachment [sage-4628.patch](tarball://root/attachments/some-uuid/ticket4628/sage-4628.patch) by @williamstein created at 2008-11-26 22:35:02



---

archive/issue_events_017588.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4628",
    "rename": {
        "from": "[with patch; needs review] sage-3.2.1.alpha1 -- setup.py build system is foobar'd",
        "to": "[with patch; with positive review] sage-3.2.1.alpha1 -- setup.py build system is foobar'd"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4628#event-17588"
}
```



---

archive/issue_comments_036899.json:
```json
{
    "body": "<a id='comment:1'></a>\nYep, this is exactly the right fix. As one can see from the comments in the original code, I had intended to pass the module along -- I apparently just forgot to finish. Oops.",
    "created_at": "2008-11-26T23:13:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4628",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4628#issuecomment-36899",
    "user": "https://github.com/craigcitro"
}
```

<a id='comment:1'></a>
Yep, this is exactly the right fix. As one can see from the comments in the original code, I had intended to pass the module along -- I apparently just forgot to finish. Oops.



---

archive/issue_events_017589.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-27T02:02:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4628",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4628#event-17589"
}
```



---

archive/issue_comments_036900.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in Sage 3.2.1.alpha2",
    "created_at": "2008-11-27T02:02:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4628",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4628#issuecomment-36900",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Merged in Sage 3.2.1.alpha2



---

archive/issue_comments_036901.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-11-27T02:02:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4628",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4628#issuecomment-36901",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
