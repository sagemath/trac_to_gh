# Issue 4331: sage-vmware image - add keyboard reconfigure option

archive/issues_004331.json:
```json
{
    "body": "Change the sage vmware image so there is another login option:\n\n```\nlogin: keyboard\n```\n\nThis would run `dpkg-reconfigure console-setup`\nas suggested by Martin Rubey in email:\n\n```\n>\n> Sorry, I meant, of all the things you tried was\n> just typing\n>\n>    dpkg-reconfigure console-setup\n>\n> the one and only thing you did, and that it worked?\n\nNearly: you will then be asked some simple questions which are obvious to\nanswer if you do not have an english keyboard.\n\n-- Martin Rubey\n```\n\n**Assignee:** mabshoff\n\n**CC:**  @slel\n\n**Reviewer:** Matthias Koeppe, Samuel Leli\u00e8vre\n\nIssue created by migration from https://trac.sagemath.org/ticket/4331\n\n",
    "closed_at": "2021-09-01T07:08:45Z",
    "created_at": "2008-10-20T16:26:21Z",
    "labels": [
        "component: distribution",
        "bug",
        "invalid"
    ],
    "title": "sage-vmware image - add keyboard reconfigure option",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4331",
    "user": "https://github.com/williamstein"
}
```
Change the sage vmware image so there is another login option:

```
login: keyboard
```

This would run `dpkg-reconfigure console-setup`
as suggested by Martin Rubey in email:

```
>
> Sorry, I meant, of all the things you tried was
> just typing
>
>    dpkg-reconfigure console-setup
>
> the one and only thing you did, and that it worked?

Nearly: you will then be asked some simple questions which are obvious to
answer if you do not have an english keyboard.

-- Martin Rubey
```

**Assignee:** mabshoff

**CC:**  @slel

**Reviewer:** Matthias Koeppe, Samuel Lelièvre

Issue created by migration from https://trac.sagemath.org/ticket/4331





---

archive/issue_events_025164.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4331",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4331#event-25164"
}
```



---

archive/issue_events_025165.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4331",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4331#event-25165"
}
```



---

archive/issue_events_025166.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4331",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4331#event-25166"
}
```



---

archive/issue_events_025167.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4331",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4331#event-25167"
}
```



---

archive/issue_events_025168.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4331",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4331#event-25168"
}
```



---

archive/issue_events_025169.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4331",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4331#event-25169"
}
```



---

archive/issue_events_025170.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4331",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4331#event-25170"
}
```



---

archive/issue_events_025171.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4331",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4331#event-25171"
}
```



---

archive/issue_events_025172.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-26T02:16:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/4331",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4331#event-25172"
}
```



---

archive/issue_events_025173.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-26T02:16:22Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4331",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4331#event-25173"
}
```



---

archive/issue_comments_025983.json:
```json
{
    "body": "<a id='comment:5'></a>\noutdated, should close",
    "created_at": "2021-08-26T02:16:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4331",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4331#issuecomment-25983",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:5'></a>
outdated, should close



---

archive/issue_comments_025984.json:
```json
{
    "body": "**Reviewer:** Matthias Koeppe, Samuel Leli\u00e8vre",
    "created_at": "2021-08-26T22:22:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4331",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4331#issuecomment-25984",
    "user": "https://github.com/slel"
}
```

**Reviewer:** Matthias Koeppe, Samuel Lelièvre



---

archive/issue_events_025174.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-08-26T22:22:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/4331",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4331#event-25174"
}
```



---

archive/issue_events_025175.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-08-26T22:22:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/4331",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4331#event-25175"
}
```



---

archive/issue_comments_025985.json:
```json
{
    "body": "<a id='comment:6'></a>\nThere seems to be no plan to resume producing\nthe vmware image at each Sage release.\n\nIts main use was arguably to work around\nthe lack of a version of Sage for Windows.\n\nThat use case largely fell with Erik Bray's\nCygwin-based Sage installer for 64-bit Windows.\n\nThe Sage Debian Live distribution also\nprovides a versatile portable solution.\n\nIn view of that, ok to close this.",
    "created_at": "2021-08-26T22:22:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4331",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4331#issuecomment-25985",
    "user": "https://github.com/slel"
}
```

<a id='comment:6'></a>
There seems to be no plan to resume producing
the vmware image at each Sage release.

Its main use was arguably to work around
the lack of a version of Sage for Windows.

That use case largely fell with Erik Bray's
Cygwin-based Sage installer for 64-bit Windows.

The Sage Debian Live distribution also
provides a versatile portable solution.

In view of that, ok to close this.



---

archive/issue_events_025176.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-09-01T07:08:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/4331",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4331#event-25176"
}
```



---

archive/issue_events_025177.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-09-01T07:08:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4331",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4331#event-25177"
}
```
