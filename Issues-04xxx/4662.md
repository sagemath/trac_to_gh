# Issue 4662: [with patch, with positive review] Sage 3.2.1.rc1: Fix documentation build issues

archive/issues_004662.json:
```json
{
    "body": "Various LaTeX related build failures have to be fixed before 3.2.1.rc1 should be released. I am on this.\n\nCheers,\n\nMichael\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4662\n\n",
    "closed_at": "2008-11-30T10:43:22Z",
    "created_at": "2008-11-30T09:21:10Z",
    "labels": [
        "component: documentation",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.2.1",
    "title": "[with patch, with positive review] Sage 3.2.1.rc1: Fix documentation build issues",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4662",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
Various LaTeX related build failures have to be fixed before 3.2.1.rc1 should be released. I am on this.

Cheers,

Michael

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4662





---

archive/issue_comments_037219.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-11-30T09:21:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4662#issuecomment-37219",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing status from new to assigned.



---

archive/issue_comments_037220.json:
```json
{
    "body": "Changing assignee from tba to mabshoff.",
    "created_at": "2008-11-30T09:21:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4662#issuecomment-37220",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing assignee from tba to mabshoff.



---

archive/issue_events_017745.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4662",
    "rename": {
        "from": "Sage 3.2.1.rc1: Fix documentation build issues",
        "to": "[with patch, needs review] Sage 3.2.1.rc1: Fix documentation build issues"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4662#event-17745"
}
```



---

archive/issue_comments_037221.json:
```json
{
    "body": "<a id='comment:2'></a>\nAttachment [trac_4662.patch](tarball://root/attachments/some-uuid/ticket4662/trac_4662.patch) by mabshoff created at 2008-11-30 10:40:58\n\nFixes two LaTeX problems and a spelling error as an added bonus.\n\nCheers,\n\nMichael",
    "created_at": "2008-11-30T10:40:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4662#issuecomment-37221",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Attachment [trac_4662.patch](tarball://root/attachments/some-uuid/ticket4662/trac_4662.patch) by mabshoff created at 2008-11-30 10:40:58

Fixes two LaTeX problems and a spelling error as an added bonus.

Cheers,

Michael



---

archive/issue_events_017746.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4662",
    "rename": {
        "from": "[with patch, needs review] Sage 3.2.1.rc1: Fix documentation build issues",
        "to": "[with patch, with positive review] Sage 3.2.1.rc1: Fix documentation build issues"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4662#event-17746"
}
```



---

archive/issue_comments_037222.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.2.1.rc1",
    "created_at": "2008-11-30T10:43:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4662#issuecomment-37222",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.2.1.rc1



---

archive/issue_comments_037223.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-11-30T10:43:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4662#issuecomment-37223",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_017747.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-30T10:43:22Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4662",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4662#event-17747"
}
```
