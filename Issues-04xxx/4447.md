# Issue 4447: [with patch, positive review] graph attributes (_boundary, _pos, etc.) are not updated properly

archive/issues_004447.json:
```json
{
    "body": "The attached patch fixes a number of small issues with updating graph attributes.\n\nAssignee: @rlmill\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4447\n\n",
    "closed_at": "2008-11-13T18:42:46Z",
    "created_at": "2008-11-05T19:09:05Z",
    "labels": [
        "component: graph theory",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.2",
    "title": "[with patch, positive review] graph attributes (_boundary, _pos, etc.) are not updated properly",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4447",
    "user": "https://github.com/jasongrout"
}
```
The attached patch fixes a number of small issues with updating graph attributes.

Assignee: @rlmill

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4447





---

archive/issue_comments_034642.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [graph-update-attributes.patch](tarball://root/attachments/some-uuid/ticket4447/graph-update-attributes.patch) by @jasongrout created at 2008-11-05 19:10:24\n\nall tests in graphs/*.py and graphs/*.pyx pass.",
    "created_at": "2008-11-05T19:10:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4447",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4447#issuecomment-34642",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:1'></a>
Attachment [graph-update-attributes.patch](tarball://root/attachments/some-uuid/ticket4447/graph-update-attributes.patch) by @jasongrout created at 2008-11-05 19:10:24

all tests in graphs/*.py and graphs/*.pyx pass.



---

archive/issue_events_016722.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4447",
    "rename": {
        "from": "graph attributes (_boundary, _pos, etc.) are not updated properly",
        "to": "[with patch, needs review] graph attributes (_boundary, _pos, etc.) are not updated properly"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4447#event-16722"
}
```



---

archive/issue_comments_034643.json:
```json
{
    "body": "<a id='comment:2'></a>\nThis is good code. I don't have time to run tests, but as long as they pass this should definitely be merged.",
    "created_at": "2008-11-13T16:01:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4447",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4447#issuecomment-34643",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:2'></a>
This is good code. I don't have time to run tests, but as long as they pass this should definitely be merged.



---

archive/issue_events_016723.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4447",
    "rename": {
        "from": "[with patch, needs review] graph attributes (_boundary, _pos, etc.) are not updated properly",
        "to": "[with patch, positive review] graph attributes (_boundary, _pos, etc.) are not updated properly"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4447#event-16723"
}
```



---

archive/issue_comments_034644.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-11-13T18:42:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4447",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4447#issuecomment-34644",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_016724.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-13T18:42:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4447",
    "milestone": "sage-3.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4447#event-16724"
}
```



---

archive/issue_events_016725.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-13T18:42:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4447",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4447#event-16725"
}
```



---

archive/issue_comments_034645.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.2.rc1 - doctests do pass.\n\nCheers,\n\nMichael",
    "created_at": "2008-11-13T18:42:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4447",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4447#issuecomment-34645",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.2.rc1 - doctests do pass.

Cheers,

Michael
