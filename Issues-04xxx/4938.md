# Issue 4938: Words.count() returns a Python int instead of a Sage Integer (fixed by #5308)

archive/issues_004938.json:
```json
{
    "assignees": [],
    "body": "\n```\nHi guys,\n\nI sat around and was looking at the new ReST Words documentation and\nnoticed the following:\n\n[5:40pm] mabs: WTF:\n[5:40pm] mabs: sage: Words(7,13).count()\n[5:40pm] mabs: 96889010407L              # 32-bit\n[5:40pm] mabs: 96889010407                # 64-bit\n[5:40pm] mabs: So Words().count() returns a Python int?\n[5:40pm] wstein: ick\n[5:40pm] wstein: That stupid L at theend was suck an annoying decision by Guido.\n\nI would expect that count() returns a Sage Integer since that seems to\nbe the expected result in general. If you agree please open a ticket,\nbut this is not a high priority issue for me.\n\nCheers,\n\nMichael\n```\n\nAssignee: **@saliola**\n\nCC:  sage-combinat\n\nKeywords: **sage-words**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4938_\n\n",
    "closed_at": "2009-04-05T23:54:40Z",
    "created_at": "2009-01-04T19:55:25Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.4.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Words.count() returns a Python int instead of a Sage Integer (fixed by #5308)",
    "type": "issue",
    "updated_at": "2009-04-14T04:00:42Z",
    "url": "https://github.com/sagemath/sage/issues/4938",
    "user": "https://github.com/saliola"
}
```

```
Hi guys,

I sat around and was looking at the new ReST Words documentation and
noticed the following:

[5:40pm] mabs: WTF:
[5:40pm] mabs: sage: Words(7,13).count()
[5:40pm] mabs: 96889010407L              # 32-bit
[5:40pm] mabs: 96889010407                # 64-bit
[5:40pm] mabs: So Words().count() returns a Python int?
[5:40pm] wstein: ick
[5:40pm] wstein: That stupid L at theend was suck an annoying decision by Guido.

I would expect that count() returns a Sage Integer since that seems to
be the expected result in general. If you agree please open a ticket,
but this is not a high priority issue for me.

Cheers,

Michael
```

Assignee: **@saliola**

CC:  sage-combinat

Keywords: **sage-words**

_Issue created by migration from https://trac.sagemath.org/ticket/4938_





---

archive/issue_events_051177.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2009-01-04T19:55:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4938",
    "milestone_number": null,
    "milestone_title": "sage-3.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4938#event-51177"
}
```



---

archive/issue_events_051178.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2009-01-04T19:55:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4938",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4938#event-51178"
}
```



---

archive/issue_events_051179.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2009-01-04T19:55:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4938",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4938#event-51179"
}
```



---

archive/issue_events_051180.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2009-01-04T19:55:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4938",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4938#event-51180"
}
```



---

archive/issue_comments_031027.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nThe problem should be fixed with #5308. So you can close it as a duplicate.\n\nCheers,\n\nFlorent",
    "created_at": "2009-04-02T09:14:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4938",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4938#issuecomment-31027",
    "user": "https://github.com/hivert"
}
```

<a id='comment:1'>Comment 1:</a>
The problem should be fixed with #5308. So you can close it as a duplicate.

Cheers,

Florent



---

archive/issue_comments_031028.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThis ticket can be closed once #5308 is merged.\n\nCheers,\n\nMichael",
    "created_at": "2009-04-05T00:07:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4938",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4938#issuecomment-31028",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:2'>Comment 2:</a>
This ticket can be closed once #5308 is merged.

Cheers,

Michael



---

archive/issue_events_051181.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-05T00:07:16Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4938",
    "title_is": "Words.count() returns a Python int instead of a Sage Integer (fixed by #5308)",
    "title_was": "Words.count() returns a Python int instead of a Sage Integer",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4938#event-51181"
}
```



---

archive/issue_events_051182.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-05T23:54:40Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4938",
    "milestone_number": null,
    "milestone_title": "sage-3.4.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4938#event-51182"
}
```



---

archive/issue_events_051183.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-05T23:54:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4938",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4938#event-51183"
}
```



---

archive/issue_events_051184.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-04-05T23:54:40Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4938",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4938#event-51184"
}
```



---

archive/issue_comments_031029.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nFixed in Sage 3.4.1.rc1 via #5308.\n\nCheers,\n\nMichael",
    "created_at": "2009-04-05T23:54:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4938",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4938#issuecomment-31029",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:3'>Comment 3:</a>
Fixed in Sage 3.4.1.rc1 via #5308.

Cheers,

Michael
