# Issue 4707: [with patch; positive review] magma/sage interface -- another trivial easy-to-fix failure hopefully

archive/issues_004707.json:
```json
{
    "body": "\n```\nsage: E = EllipticCurve(GF(25,'a'), [0,0,1,4,0])\nsage: magma(E)\nboom\n```\n\nAssignee: @williamstein\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4707\n\n",
    "closed_at": "2008-12-11T11:09:55Z",
    "created_at": "2008-12-05T02:28:26Z",
    "labels": [
        "component: interfaces",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.2.2",
    "title": "[with patch; positive review] magma/sage interface -- another trivial easy-to-fix failure hopefully",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4707",
    "user": "https://github.com/williamstein"
}
```

```
sage: E = EllipticCurve(GF(25,'a'), [0,0,1,4,0])
sage: magma(E)
boom
```

Assignee: @williamstein

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4707





---

archive/issue_comments_037642.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [trac_4707.patch](tarball://root/attachments/some-uuid/ticket4707/trac_4707.patch) by @williamstein created at 2008-12-05 02:55:03",
    "created_at": "2008-12-05T02:55:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4707",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4707#issuecomment-37642",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'></a>
Attachment [trac_4707.patch](tarball://root/attachments/some-uuid/ticket4707/trac_4707.patch) by @williamstein created at 2008-12-05 02:55:03



---

archive/issue_events_017972.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4707",
    "rename": {
        "from": "magma/sage interface -- another trivial easy-to-fix failure hopefully",
        "to": "[with patch; needs review (EASY!)] magma/sage interface -- another trivial easy-to-fix failure hopefully"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4707#event-17972"
}
```



---

archive/issue_events_017973.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4707",
    "rename": {
        "from": "[with patch; needs review (EASY!)] magma/sage interface -- another trivial easy-to-fix failure hopefully",
        "to": "[with patch, with postitive review] magma/sage interface -- another trivial easy-to-fix failure hopefully"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4707#event-17973"
}
```



---

archive/issue_comments_037643.json:
```json
{
    "body": "<a id='comment:2'></a>\nAttachment [trac_4707_tiny_followup.patch](tarball://root/attachments/some-uuid/ticket4707/trac_4707_tiny_followup.patch) by @ncalexan created at 2008-12-06 23:49:58\n\nWorks for me.  Apply both patches.  We will open a separate ticket for the corresponding magma -> sage conversion.",
    "created_at": "2008-12-06T23:49:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4707",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4707#issuecomment-37643",
    "user": "https://github.com/ncalexan"
}
```

<a id='comment:2'></a>
Attachment [trac_4707_tiny_followup.patch](tarball://root/attachments/some-uuid/ticket4707/trac_4707_tiny_followup.patch) by @ncalexan created at 2008-12-06 23:49:58

Works for me.  Apply both patches.  We will open a separate ticket for the corresponding magma -> sage conversion.



---

archive/issue_comments_037644.json:
```json
{
    "body": "<a id='comment:3'></a>\nfix typo in title",
    "created_at": "2008-12-11T04:58:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4707",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4707#issuecomment-37644",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:3'></a>
fix typo in title



---

archive/issue_events_017974.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4707",
    "rename": {
        "from": "[with patch, with postitive review] magma/sage interface -- another trivial easy-to-fix failure hopefully",
        "to": "[with patch, with positive review] magma/sage interface -- another trivial easy-to-fix failure hopefully"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4707#event-17974"
}
```



---

archive/issue_events_017975.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4707",
    "rename": {
        "from": "[with patch, with positive review] magma/sage interface -- another trivial easy-to-fix failure hopefully",
        "to": "[with patch; positive review] magma/sage interface -- another trivial easy-to-fix failure hopefully"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4707#event-17975"
}
```



---

archive/issue_comments_037645.json:
```json
{
    "body": "<a id='comment:5'></a>\nMerged both patches in Sage 3.2.2.alpha2",
    "created_at": "2008-12-11T11:09:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4707",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4707#issuecomment-37645",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:5'></a>
Merged both patches in Sage 3.2.2.alpha2



---

archive/issue_events_017976.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-12-11T11:09:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4707",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4707#event-17976"
}
```



---

archive/issue_comments_037646.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-12-11T11:09:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4707",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4707#issuecomment-37646",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
