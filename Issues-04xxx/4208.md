# Issue 4208: [with spkg, positive review] Update mpfr to 2.3.2 release

archive/issues_004208.json:
```json
{
    "body": "There is a new upstream release of mpfr. The spkg at \n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.1.3/alpha2/mpfr-2.3.2.spkg\n\nupdates to that release.\n\nCheers,\n\nMichael\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4208\n\n",
    "closed_at": "2008-09-27T21:40:19Z",
    "created_at": "2008-09-27T20:10:12Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1.3",
    "title": "[with spkg, positive review] Update mpfr to 2.3.2 release",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4208",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
There is a new upstream release of mpfr. The spkg at 

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.1.3/alpha2/mpfr-2.3.2.spkg

updates to that release.

Cheers,

Michael

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4208





---

archive/issue_comments_030523.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-09-27T20:10:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4208",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4208#issuecomment-30523",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing status from new to assigned.



---

archive/issue_comments_030524.json:
```json
{
    "body": "<a id='comment:2'></a>Builds for me, mabshoff claims all tests pass on sage.math.",
    "created_at": "2008-09-27T21:00:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4208",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4208#issuecomment-30524",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:2'></a>Builds for me, mabshoff claims all tests pass on sage.math.



---

archive/issue_events_009539.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-27T21:40:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4208",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4208#event-9539"
}
```



---

archive/issue_comments_030525.json:
```json
{
    "body": "<a id='comment:3'></a>Merged in Sage 3.1.3.alpha2",
    "created_at": "2008-09-27T21:40:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4208",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4208#issuecomment-30525",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>Merged in Sage 3.1.3.alpha2



---

archive/issue_comments_030526.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-09-27T21:40:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4208",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4208#issuecomment-30526",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
