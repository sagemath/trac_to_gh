# Issue 4263: [with patch; positive review] elliptic curves -- point height serious stupid bug in raising error

archive/issues_004263.json:
```json
{
    "body": "This is dumb (first thing I tried broke):\n\n```\nsage: E = EllipticCurve('5077a1')\nsage: F = E.change_ring(QuadraticField(-3,'a'))\nsage: P = F([-2,3,1])\nsage: s = P.height(); s\nsage: type(s)\n<type 'NoneType'>\n```\n\n\n\nAssignee: @williamstein\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4263\n\n",
    "closed_at": "2008-10-11T12:13:11Z",
    "created_at": "2008-10-11T08:46:08Z",
    "labels": [
        "component: number theory",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1.3",
    "title": "[with patch; positive review] elliptic curves -- point height serious stupid bug in raising error",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4263",
    "user": "https://github.com/williamstein"
}
```
This is dumb (first thing I tried broke):

```
sage: E = EllipticCurve('5077a1')
sage: F = E.change_ring(QuadraticField(-3,'a'))
sage: P = F([-2,3,1])
sage: s = P.height(); s
sage: type(s)
<type 'NoneType'>
```



Assignee: @williamstein

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4263





---

archive/issue_comments_032812.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [sage-4263.patch](tarball://root/attachments/some-uuid/ticket4263/sage-4263.patch) by @williamstein created at 2008-10-11 08:51:49",
    "created_at": "2008-10-11T08:51:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4263#issuecomment-32812",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'></a>
Attachment [sage-4263.patch](tarball://root/attachments/some-uuid/ticket4263/sage-4263.patch) by @williamstein created at 2008-10-11 08:51:49



---

archive/issue_events_015914.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4263",
    "rename": {
        "from": "elliptic curves -- point height serious stupid bug in raising error",
        "to": "[with patch; needs review] elliptic curves -- point height serious stupid bug in raising error"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4263#event-15914"
}
```



---

archive/issue_events_015915.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4263",
    "rename": {
        "from": "[with patch; needs review] elliptic curves -- point height serious stupid bug in raising error",
        "to": "[with patch; positive review] elliptic curves -- point height serious stupid bug in raising error"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4263#event-15915"
}
```



---

archive/issue_comments_032813.json:
```json
{
    "body": "<a id='comment:2'></a>\nlooks good to me.",
    "created_at": "2008-10-11T10:01:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4263#issuecomment-32813",
    "user": "https://github.com/malb"
}
```

<a id='comment:2'></a>
looks good to me.



---

archive/issue_comments_032814.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-10-11T12:13:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4263#issuecomment-32814",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_015916.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-10-11T12:13:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4263",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4263#event-15916"
}
```



---

archive/issue_comments_032815.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.1.3.rc0",
    "created_at": "2008-10-11T12:13:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4263",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4263#issuecomment-32815",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.1.3.rc0
