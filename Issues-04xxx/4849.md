# Issue 4849: add functionality for computing Heegner points to Sage

archive/issues_004849.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "See #4848 for a motivation. Attached is some Magma code, but note that according to William\n\n```\nThen the file heegner.py should be attached to that ticket, since to do that \nticket, one might want to port some of what's in there to Sage. That said, it's \nnot so simple, since better algorithms for computing Heegner points were found \nafter that code was written.\n```\n\nComponent: **number theory**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4849_\n\n",
    "closed_at": "2009-07-20T15:52:03Z",
    "created_at": "2008-12-21T22:08:10Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20number%20theory",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "add functionality for computing Heegner points to Sage",
    "type": "issue",
    "updated_at": "2009-07-20T15:52:03Z",
    "url": "https://github.com/sagemath/sage/issues/4849",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
See #4848 for a motivation. Attached is some Magma code, but note that according to William

```
Then the file heegner.py should be attached to that ticket, since to do that 
ticket, one might want to port some of what's in there to Sage. That said, it's 
not so simple, since better algorithms for computing Heegner points were found 
after that code was written.
```

Component: **number theory**

_Issue created by migration from https://trac.sagemath.org/ticket/4849_





---

archive/issue_events_056010.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-21T22:08:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4849",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20number%20theory",
    "label_color": "0000ff",
    "label_name": "c: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4849#event-56010"
}
```



---

archive/issue_events_056011.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-21T22:08:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4849",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4849#event-56011"
}
```



---

archive/issue_events_056012.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-21T22:08:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4849",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4849#event-56012"
}
```



---

archive/issue_events_056013.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-21T22:08:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4849",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4849#event-56013"
}
```



---

archive/issue_events_056014.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-12-21T22:08:10Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4849",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4849#event-56014"
}
```



---

archive/issue_comments_029283.json:
```json
{
    "body": "Attachment: **[heegner.py.gz](https://github.com/sagemath/sage/files/ticket4849/heegner.py.gz)**\n\nThis is mostly Magma code",
    "created_at": "2008-12-21T22:10:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4849",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4849#issuecomment-29283",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

Attachment: **[heegner.py.gz](https://github.com/sagemath/sage/files/ticket4849/heegner.py.gz)**

This is mostly Magma code



---

archive/issue_events_056015.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-21T22:11:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4849",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4849#event-56015"
}
```



---

archive/issue_events_056016.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-21T22:11:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4849",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4849#event-56016"
}
```



---

archive/issue_events_056017.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-15T20:39:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4849",
    "milestone_number": null,
    "milestone_title": "sage-4.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4849#event-56017"
}
```



---

archive/issue_events_056018.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-15T20:39:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4849",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4849#event-56018"
}
```



---

archive/issue_events_056019.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-15T20:39:14Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4849",
    "title_is": "[fixed by #6045, positive review] add functionality for computing Heegner points to Sage",
    "title_was": "add functionality for computing Heegner points to Sage",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4849#event-56019"
}
```



---

archive/issue_comments_029284.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThis is fixed by #6045. I'm flagging this to \"positive review\" so it will come to the attention of mvngu, who has the authority to close tickets as 4.1.1 release manager.",
    "created_at": "2009-07-15T20:39:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4849",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4849#issuecomment-29284",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:2" align="right">comment:2</div>

This is fixed by #6045. I'm flagging this to "positive review" so it will come to the attention of mvngu, who has the authority to close tickets as 4.1.1 release manager.



---

archive/issue_comments_029285.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nThe patches on #6045 result in some numerical noise. Once that is fixed, this ticket can get positive review. At the moment, I don't have the privilege to close tickets. But I've merged some tickets in my tree. If you're interested, refer to the file\n\nhttp://sage.math.washington.edu/home/mvngu/release/merged-tkt.txt",
    "created_at": "2009-07-16T10:14:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4849",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4849#issuecomment-29285",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:3" align="right">comment:3</div>

The patches on #6045 result in some numerical noise. Once that is fixed, this ticket can get positive review. At the moment, I don't have the privilege to close tickets. But I've merged some tickets in my tree. If you're interested, refer to the file

http://sage.math.washington.edu/home/mvngu/release/merged-tkt.txt



---

archive/issue_events_056020.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-16T10:14:22Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4849",
    "title_is": "[see #6045, needs work] add functionality for computing Heegner points to Sage",
    "title_was": "[fixed by #6045, positive review] add functionality for computing Heegner points to Sage",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4849#event-56020"
}
```



---

archive/issue_events_056021.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-20T15:52:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4849",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4849#event-56021"
}
```



---

archive/issue_events_056022.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-20T15:52:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4849",
    "milestone_number": null,
    "milestone_title": "sage-4.1.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4849#event-56022"
}
```



---

archive/issue_events_056023.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-20T15:52:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4849",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4849#event-56023"
}
```



---

archive/issue_events_056024.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-20T15:52:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4849",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4849#event-56024"
}
```



---

archive/issue_events_056025.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-20T15:52:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4849",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4849#event-56025"
}
```



---

archive/issue_events_056026.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2009-07-20T15:52:03Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4849",
    "title_is": "add functionality for computing Heegner points to Sage",
    "title_was": "[see #6045, needs work] add functionality for computing Heegner points to Sage",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4849#event-56026"
}
```



---

archive/issue_comments_029286.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nI'm closing this as a duplicate of #6045.",
    "created_at": "2009-07-20T15:52:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4849",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4849#issuecomment-29286",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:4" align="right">comment:4</div>

I'm closing this as a duplicate of #6045.
