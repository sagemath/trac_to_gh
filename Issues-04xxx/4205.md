...
```json
{
    "body": "<a id='comment:1'></a>This patch depends on the patches at #3498.  Both patches touch the numpy.pxd file, so this needs to be applied after the patches at #3498.",
...
<a id='comment:1'></a>This patch depends on the patches at #3498.  Both patches touch the numpy.pxd file, so this needs to be applied after the patches at #3498.
...
```json
{
    "body": "<a id='comment:2'></a>sage -t sage/modules/*.pyx passes with the patch applied.",
...
<a id='comment:2'></a>sage -t sage/modules/*.pyx passes with the patch applied.
...
<a id='comment:3'></a>With #3498 and numpy-deprecations.patch applied we get some segfaults:
...
<a id='comment:4'></a>One more fix not yet in the patch:
...
<a id='comment:5'></a>Attachment [numpy-deprecations.patch](tarball://root/attachments/some-uuid/ticket4205/numpy-deprecations.patch) by mabshoff created at 2008-09-27 10:48:23
...
<a id='comment:6'></a>Hmm... what's up with
...
<a id='comment:8'></a>Sorry; as is obvious from the discussion and iterative approach, this ticket is indeed "needs work"; I thought that was how it was marked.  I think in the initial iteration, we thought we had something good, so it started out as "needs review" and then we forgot to change it to "needs work" when we started playing whack-a-mole.
...
```json
{
    "body": "<a id='comment:9'></a>And mabshoff is right, it's on hold until scipy 0.7 is release soon.",
...
<a id='comment:9'></a>And mabshoff is right, it's on hold until scipy 0.7 is release soon.
...
```json
{
    "body": "<a id='comment:10'></a>In fact, this ticket may not be needed anymore if the patch from #4206 is applied.",
...
<a id='comment:10'></a>In fact, this ticket may not be needed anymore if the patch from #4206 is applied.
...
```json
{
    "body": "<a id='comment:11'></a>Some bits of this patch fix scipy import issues since things have moved around. Other than that I agree with Jason that we likely will end up canning the rest of this patch.\n\nCheers,\n\nMichael",
...
<a id='comment:11'></a>Some bits of this patch fix scipy import issues since things have moved around. Other than that I agree with Jason that we likely will end up canning the rest of this patch.
...
```json
{
    "body": "<a id='comment:12'></a>This ticket can probably be closed once #6140 and #3391 are closed.",
...
<a id='comment:12'></a>This ticket can probably be closed once #6140 and #3391 are closed.
...
```json
{
    "body": "<a id='comment:13'></a>Closing this as `wontfix`, because it's superseded by #6140.",
    "created_at": "2009-06-12T06:57:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4205",
    "type": "issue_comment",
...
<a id='comment:13'></a>Closing this as `wontfix`, because it's superseded by #6140.
