# Issue 4402: Sage 3.1.4: magma related optional doctest failure in tut.tex

archive/issues_004402.json:
```json
{
    "assignees": [
        "https://github.com/williamstein"
    ],
    "body": "\n```\nsage -t -long -optional devel/doc/prog/prog.tex\n**********************************************************************\nFile \"/scratch/mabshoff/release-cycle/sage-3.2.alpha1/tmp/prog.py\", line 520:\n    sage: E._magma_init_() # optional -- requires Magma\nExpected:\n    'EllipticCurve([_sage_[1]|GF(41)!0,GF(41)!0,GF(41)!0,GF(41)!2,GF(41)!5])'\nGot:\n    'EllipticCurve([GF(41)|GF(41)!0,GF(41)!0,GF(41)!0,GF(41)!2,GF(41)!5])'\n**********************************************************************\n```\n\nTrivial to fix since this is just a printing issue.\n\nCheers,\n\nMichael\n\nComponent: **doctest coverage**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4402_\n\n",
    "closed_at": "2008-11-30T08:24:17Z",
    "created_at": "2008-10-30T17:34:25Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.2.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Sage 3.1.4: magma related optional doctest failure in tut.tex",
    "type": "issue",
    "updated_at": "2008-11-30T08:24:17Z",
    "url": "https://github.com/sagemath/sage/issues/4402",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

```
sage -t -long -optional devel/doc/prog/prog.tex
**********************************************************************
File "/scratch/mabshoff/release-cycle/sage-3.2.alpha1/tmp/prog.py", line 520:
    sage: E._magma_init_() # optional -- requires Magma
Expected:
    'EllipticCurve([_sage_[1]|GF(41)!0,GF(41)!0,GF(41)!0,GF(41)!2,GF(41)!5])'
Got:
    'EllipticCurve([GF(41)|GF(41)!0,GF(41)!0,GF(41)!0,GF(41)!2,GF(41)!5])'
**********************************************************************
```

Trivial to fix since this is just a printing issue.

Cheers,

Michael

Component: **doctest coverage**

_Issue created by migration from https://trac.sagemath.org/ticket/4402_





---

archive/issue_events_049723.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-10-30T17:34:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4402",
    "milestone_number": null,
    "milestone_title": "sage-3.2.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4402#event-49723"
}
```



---

archive/issue_events_049724.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-10-30T17:34:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4402",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4402#event-49724"
}
```



---

archive/issue_events_049725.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-10-30T17:34:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4402",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4402#event-49725"
}
```



---

archive/issue_events_049726.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-10-30T17:34:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4402",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4402#event-49726"
}
```



---

archive/issue_events_049727.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-10-30T17:34:25Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4402",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4402#event-49727"
}
```



---

archive/issue_comments_025517.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nWilliam,\n\nthis one is still there since the TeX documentation isn't run per default. Unless you beat me to it I will fix this tomorrow.\n\nCheers,\n\nMichael",
    "created_at": "2008-11-28T09:34:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4402#issuecomment-25517",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">comment:1</div>

William,

this one is still there since the TeX documentation isn't run per default. Unless you beat me to it I will fix this tomorrow.

Cheers,

Michael



---

archive/issue_events_049728.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-30T07:46:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4402",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4402#event-49728"
}
```



---

archive/issue_events_049729.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-30T07:46:55Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4402",
    "milestone_number": null,
    "milestone_title": "sage-3.2.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4402#event-49729"
}
```



---

archive/issue_events_049730.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-30T07:46:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4402",
    "milestone_number": null,
    "milestone_title": "sage-3.2.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4402#event-49730"
}
```



---

archive/issue_comments_025518.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nAttachment: **[trac_4402_doc.patch.gz](https://github.com/sagemath/sage/files/ticket4402/trac_4402_doc.patch.gz)**",
    "created_at": "2008-11-30T07:46:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4402#issuecomment-25518",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:2" align="right">comment:2</div>

Attachment: **[trac_4402_doc.patch.gz](https://github.com/sagemath/sage/files/ticket4402/trac_4402_doc.patch.gz)**



---

archive/issue_events_049731.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-30T07:48:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4402",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4402#event-49731"
}
```



---

archive/issue_events_049732.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-30T07:48:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4402",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4402#event-49732"
}
```



---

archive/issue_comments_025519.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nPositive review.\n\nCheers,\n\nMichael",
    "created_at": "2008-11-30T07:48:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4402#issuecomment-25519",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Positive review.

Cheers,

Michael



---

archive/issue_events_049733.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-30T08:24:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4402",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4402#event-49733"
}
```



---

archive/issue_events_049734.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-30T08:24:17Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4402",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4402#event-49734"
}
```



---

archive/issue_comments_025520.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nMerged in Sage 3.2.1.rc1",
    "created_at": "2008-11-30T08:24:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4402",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4402#issuecomment-25520",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Merged in Sage 3.2.1.rc1
