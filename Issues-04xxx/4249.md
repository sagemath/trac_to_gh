...
```json
{
    "body": "<a id='comment:2'></a>I strongly agree with John Cremona's suggested fix.",
...
<a id='comment:2'></a>I strongly agree with John Cremona's suggested fix.
...
```json
{
    "body": "<a id='comment:4'></a>I can't test this yet because I can't apply the patch to 3.1.2, and the upgrade is apparently not yet working.  I'm worried about all of the changes to reduced_basis which resulted from the change, but will have a look ASAP.  JV",
...
<a id='comment:4'></a>I can't test this yet because I can't apply the patch to 3.1.2, and the upgrade is apparently not yet working.  I'm worried about all of the changes to reduced_basis which resulted from the change, but will have a look ASAP.  JV
...
```json
{
    "body": "<a id='comment:5'></a>Thanks John.  I put \"not ready for review\" since we don't yet know exactly what is going on.  John",
...
<a id='comment:5'></a>Thanks John.  I put "not ready for review" since we don't yet know exactly what is going on.  John
...
<a id='comment:7'></a>When I apply my patch to 3.1.3 I get this (testing all files in number_field):
...
<a id='comment:8'></a>Huh.  Applying the patch on sage.math (on 3.1.3) gives:
   {{{
   sage: F.<t> = NumberField(x^2-5)
   sage: enumerate_totallyreal_fields_rel?
   sage: ls = enumerate_totallyreal_fields_rel(F, 2, 10^4)
...
```json
{
    "body": "<a id='comment:9'></a>I agree, it is likely to be a precision issue.  My laptop is 32-bit while sage.math is 64.",
...
<a id='comment:9'></a>I agree, it is likely to be a precision issue.  My laptop is 32-bit while sage.math is 64.
...
```json
{
    "body": "<a id='comment:10'></a>I downloaded a clean 3.1.3 and installed it on a 32-bit Ubuntu and it again worked fine.  I'm not sure what's going on.  JV",
...
<a id='comment:10'></a>I downloaded a clean 3.1.3 and installed it on a 32-bit Ubuntu and it again worked fine.  I'm not sure what's going on.  JV
...
<a id='comment:11'></a>Replying to [comment:10 jvoight]:
> I downloaded a clean 3.1.3 and installed it on a 32-bit Ubuntu and it again worked fine.  I'm not sure what's going on.  JV
...
<a id='comment:13'></a>Replying to [comment:12 jvoight]:
> Hi John,
> 
> I'm hoping to tackle this at Sage Days 11, or at least find someone willing to help.  What system are you using?  Maybe I can find someone with the same system so as to reproduce the bug...
> 
...
<a id='comment:14'></a>I applied my patch to 3.2.alpha3 on two machines, a 32-bit Suse linux and a 64-bit ubuntu.
On both machines two files failed doctests:
...
<a id='comment:15'></a>Argh!  Tried the patch on sage.math on 3.2.alpha3, still can't reproduce the error!  
...
```json
{
    "body": "<a id='comment:16'></a>Attached patch fixes the issue. It turns out that John Cremona's fix actually exposed a small bug in `integral_elements_with_trace`, which has been fixed and renamed. \n\nThis was written with John Voight.",
...
<a id='comment:16'></a>Attached patch fixes the issue. It turns out that John Cremona's fix actually exposed a small bug in `integral_elements_with_trace`, which has been fixed and renamed. 
...
```json
{
    "body": "<a id='comment:17'></a>Whoops. Forgot that we had to look at John's original patch, too. :)",
...
<a id='comment:17'></a>Whoops. Forgot that we had to look at John's original patch, too. :)
...
<a id='comment:18'></a>Attachment [trac-4249-pt2a.patch](tarball://root/attachments/some-uuid/ticket4249/trac-4249-pt2a.patch) by @craigcitro created at 2008-11-14 09:22:08
...
```json
{
    "body": "<a id='comment:19'></a>I strongly approve of the two new patches, which fix the bug which caused the totally_real_rel failure and also improve on my code design.  I tested them against 3.2.alpha3 and all was well (testing all in number_field/).\n\nSo I hope there's enough mutual positive reviewing here for these to be merged.\n\nIt would be even better if John V agreed!",
...
<a id='comment:19'></a>I strongly approve of the two new patches, which fix the bug which caused the totally_real_rel failure and also improve on my code design.  I tested them against 3.2.alpha3 and all was well (testing all in number_field/).
...
```json
{
    "body": "<a id='comment:20'></a>Yes, definitely I agree.  Positive review.  JV",
...
<a id='comment:20'></a>Yes, definitely I agree.  Positive review.  JV
...
<a id='comment:21'></a>There are some slight doctest failures to fix:
...
<a id='comment:22'></a>Strange about that first one -- it was one where I had had different output on 32- and 64-bit machines, but now they are the same.  Fixed in trac-4249-3.patch.
...
```json
{
    "body": "<a id='comment:23'></a>Yep, last patches look good.",
...
<a id='comment:23'></a>Yep, last patches look good.
...
```json
{
    "body": "<a id='comment:24'></a>What is the credit situation, i.e. authorship here: John Cremona, John Voight? Review Craig Citro?\n\nWhat about Aranes Maite?\n\nCheers,\n\nMichael",
...
<a id='comment:24'></a>What is the credit situation, i.e. authorship here: John Cremona, John Voight? Review Craig Citro?
...
```json
{
    "body": "<a id='comment:25'></a>I think John C wrote sage-4249.patch, I wrote trac-4249-1a.patch as part of the review, John Voight and I each wrote half of trac-4249-pt2a.patch and reviewed the other, and John Cremona also reviewed this patch. Then John Cremona wrote pt3 and the doc patch.\n\nEither that, or it was Col. Mustard in the study, with the candlestick. (I would be completely in favor of that actually appearing in the release notes.)\n\nI'm not sure if the original patch was written with Aranes Maite.",
...
<a id='comment:25'></a>I think John C wrote sage-4249.patch, I wrote trac-4249-1a.patch as part of the review, John Voight and I each wrote half of trac-4249-pt2a.patch and reviewed the other, and John Cremona also reviewed this patch. Then John Cremona wrote pt3 and the doc patch.
...
<a id='comment:26'></a>Replying to [comment:25 craigcitro]:
> I think John C wrote sage-4249.patch, I wrote trac-4249-1a.patch as part of the review, John Voight and I each wrote half of trac-4249-pt2a.patch and reviewed the other, and John Cremona also reviewed this patch. Then John Cremona wrote pt3 and the doc patch.
> 
...
```json
{
    "body": "<a id='comment:27'></a>Merged all five patches in Sage 3.2.rc1",
    "created_at": "2008-11-15T10:27:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4249",
    "type": "issue_comment",
...
<a id='comment:27'></a>Merged all five patches in Sage 3.2.rc1
