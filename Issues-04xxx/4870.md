# Issue 4870: Sage 3.2.3: turn off FLINT test suite in the final build

archive/issues_004870.json:
```json
{
    "body": "This is a followup to #4861 so we don't forget to turn off the FLINT test suite we run automatically.\n\nCheers,\n\nMichael\n\n**Assignee:** mabshoff\n\nIssue created by migration from https://trac.sagemath.org/ticket/4870\n\n",
    "closed_at": "2009-01-03T06:36:47Z",
    "created_at": "2008-12-24T14:00:46Z",
    "labels": [
        "component: packages: standard",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.2.3",
    "title": "Sage 3.2.3: turn off FLINT test suite in the final build",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4870",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
This is a followup to #4861 so we don't forget to turn off the FLINT test suite we run automatically.

Cheers,

Michael

**Assignee:** mabshoff

Issue created by migration from https://trac.sagemath.org/ticket/4870





---

archive/issue_events_030257.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-01-03T06:33:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/4870",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4870#event-30257"
}
```



---

archive/issue_comments_031144.json:
```json
{
    "body": "<a id='comment:1'></a>\nThe spkg at\n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.2.3/final/flint-1.0.21.p0.spkg\n\nturns the test suite off.\n\nCheers,\n\nMichael",
    "created_at": "2009-01-03T06:33:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4870",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4870#issuecomment-31144",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
The spkg at

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.2.3/final/flint-1.0.21.p0.spkg

turns the test suite off.

Cheers,

Michael



---

archive/issue_events_030258.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-01-03T06:35:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/4870",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4870#event-30258"
}
```



---

archive/issue_events_030259.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-01-03T06:35:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/4870",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4870#event-30259"
}
```



---

archive/issue_events_030260.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-01-03T06:36:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/4870",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4870#event-30260"
}
```



---

archive/issue_events_030261.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-01-03T06:36:47Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4870",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4870#event-30261"
}
```



---

archive/issue_comments_031145.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.2.3.final",
    "created_at": "2009-01-03T06:36:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4870",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4870#issuecomment-31145",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.2.3.final
