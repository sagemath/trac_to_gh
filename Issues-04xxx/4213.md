# Issue 4213: [with patch, positive review] Bug in Permutations(n, k)

archive/issues_004213.json:
```json
{
    "body": "\n```\nsage: list(Permutations([1,2,3,4,5], 4))\n\n[ ...\n [2, 3, 1, 5],\n [2, 3, 3, 1],\n [2, 3, 3, 4],\n [2, 3, 3, 5],\n [2, 3, 4, 1],\n ...\n [3, 2, 1, 5],\n [3, 2, 2, 1],\n [3, 2, 2, 4],\n [3, 2, 2, 5],\n [3, 2, 4, 1],\n ...\n [4, 2, 1, 5],\n [4, 2, 2, 1],\n [4, 2, 2, 3],\n [4, 2, 2, 5],\n [4, 2, 3, 1],\n ...\n [5, 2, 1, 4],\n [5, 2, 2, 1],\n [5, 2, 2, 3],\n [5, 2, 2, 4],\n [5, 2, 3, 1],\n```\n\nOnly the buggy parts are shown.\n\nThis does not occur for lists smaller that 5 or when len(n) == k.\n\nAssignee: @mwhansen\n\nCC:  sage-combinat\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4213\n\n",
    "closed_at": "2008-09-29T02:14:23Z",
    "created_at": "2008-09-28T21:26:17Z",
    "labels": [
        "component: combinatorics",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1.3",
    "title": "[with patch, positive review] Bug in Permutations(n, k)",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4213",
    "user": "https://trac.sagemath.org/admin/accounts/users/anakha"
}
```

```
sage: list(Permutations([1,2,3,4,5], 4))

[ ...
 [2, 3, 1, 5],
 [2, 3, 3, 1],
 [2, 3, 3, 4],
 [2, 3, 3, 5],
 [2, 3, 4, 1],
 ...
 [3, 2, 1, 5],
 [3, 2, 2, 1],
 [3, 2, 2, 4],
 [3, 2, 2, 5],
 [3, 2, 4, 1],
 ...
 [4, 2, 1, 5],
 [4, 2, 2, 1],
 [4, 2, 2, 3],
 [4, 2, 2, 5],
 [4, 2, 3, 1],
 ...
 [5, 2, 1, 4],
 [5, 2, 2, 1],
 [5, 2, 2, 3],
 [5, 2, 2, 4],
 [5, 2, 3, 1],
```

Only the buggy parts are shown.

This does not occur for lists smaller that 5 or when len(n) == k.

Assignee: @mwhansen

CC:  sage-combinat

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4213





---

archive/issue_comments_032292.json:
```json
{
    "body": "Attachment [trac_4213.patch](tarball://root/attachments/some-uuid/ticket4213/trac_4213.patch) by @mwhansen created at 2008-09-28 21:51:14",
    "created_at": "2008-09-28T21:51:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4213",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4213#issuecomment-32292",
    "user": "https://github.com/mwhansen"
}
```

Attachment [trac_4213.patch](tarball://root/attachments/some-uuid/ticket4213/trac_4213.patch) by @mwhansen created at 2008-09-28 21:51:14



---

archive/issue_events_015711.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4213",
    "rename": {
        "from": "Bug in Permutations(n, k)",
        "to": "[with patch, needs review] Bug in Permutations(n, k)"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4213#event-15711"
}
```



---

archive/issue_comments_032293.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-09-28T21:51:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4213",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4213#issuecomment-32293",
    "user": "https://github.com/mwhansen"
}
```

Changing status from new to assigned.



---

archive/issue_events_015712.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4213",
    "rename": {
        "from": "[with patch, needs review] Bug in Permutations(n, k)",
        "to": "[with patch, positive review] Bug in Permutations(n, k)"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4213#event-15712"
}
```



---

archive/issue_comments_032294.json:
```json
{
    "body": "<a id='comment:2'></a>\nPasses tests and fixes the problem on my end.",
    "created_at": "2008-09-28T22:13:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4213",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4213#issuecomment-32294",
    "user": "https://trac.sagemath.org/admin/accounts/users/anakha"
}
```

<a id='comment:2'></a>
Passes tests and fixes the problem on my end.



---

archive/issue_comments_032295.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.1.3.alpha2",
    "created_at": "2008-09-29T02:14:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4213",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4213#issuecomment-32295",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.1.3.alpha2



---

archive/issue_events_015713.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-29T02:14:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4213",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4213#event-15713"
}
```



---

archive/issue_comments_032296.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-09-29T02:14:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4213",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4213#issuecomment-32296",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
