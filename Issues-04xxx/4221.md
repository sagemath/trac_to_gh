# Issue 4221: [with package, positive review] add initial pynac-0.1.p0 package

archive/issues_004221.json:
```json
{
    "body": "To split the task of testing the new pynac package and the interface code from #3872, the new version of the pynac package is moved here.\n\nLatest version of the package can be downloaded from:\n\nhttp://www.risc.jku.at/people/berocal/sage/pynac-0.1.p0.spkg\n\nAssignee: @burcin\n\nKeywords: symbolics, pynac\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4221\n\n",
    "closed_at": "2008-09-30T12:55:08Z",
    "created_at": "2008-09-30T12:34:07Z",
    "labels": [
        "component: packages: standard"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1.3",
    "title": "[with package, positive review] add initial pynac-0.1.p0 package",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4221",
    "user": "https://github.com/burcin"
}
```
To split the task of testing the new pynac package and the interface code from #3872, the new version of the pynac package is moved here.

Latest version of the package can be downloaded from:

http://www.risc.jku.at/people/berocal/sage/pynac-0.1.p0.spkg

Assignee: @burcin

Keywords: symbolics, pynac

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4221





---

archive/issue_events_015742.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4221",
    "rename": {
        "from": "[with package, needs review] pynac package",
        "to": "[with package, positive review] add initial pynac-0.1.p0 package"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4221#event-15742"
}
```



---

archive/issue_comments_032355.json:
```json
{
    "body": "<a id='comment:1'></a>\nBuild tested on \n\n* OSX 10.4, 10.5\n* Linux x86, x86-64, Itanium \n* Solaris x86\n\nIn total I ran 10 builds including all SkyNet machines. That obviously does not mean it actually works as intended, but the spkg passes the required build tests. Positive review.\n\nBurcin's spkg with some small SPKG.txt cleanups can be found at\n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.1.3/alpha2/pynac-0.1.p0.spkg\n\n\nCheers,\n\nMichael",
    "created_at": "2008-09-30T12:55:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4221",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4221#issuecomment-32355",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
Build tested on 

* OSX 10.4, 10.5
* Linux x86, x86-64, Itanium 
* Solaris x86

In total I ran 10 builds including all SkyNet machines. That obviously does not mean it actually works as intended, but the spkg passes the required build tests. Positive review.

Burcin's spkg with some small SPKG.txt cleanups can be found at

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.1.3/alpha2/pynac-0.1.p0.spkg


Cheers,

Michael



---

archive/issue_comments_032356.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-09-30T12:55:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4221",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4221#issuecomment-32356",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_015743.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-30T12:55:08Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4221",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4221#event-15743"
}
```



---

archive/issue_comments_032357.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in Sage 3.1.3.alpha2",
    "created_at": "2008-09-30T12:56:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4221",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4221#issuecomment-32357",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Merged in Sage 3.1.3.alpha2
