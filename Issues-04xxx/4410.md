# Issue 4410: Map.__pow__ should return identity for power 0

archive/issues_004410.json:
```json
{
    "assignees": [
        "https://github.com/burcin"
    ],
    "body": "`sage.categories.map.Map.__pow__` calls `generic_power`, which messes up power 0. There is this todo note there:\n\n```\n        # todo -- what about the case n=0 -- need to specify the identity map somehow.\n```\n\nAttached patch returns the identity map for power 0.\n\nCC:  @robertwb\n\nComponent: **basic arithmetic**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4410_\n\n",
    "closed_at": "2008-10-31T15:36:46Z",
    "created_at": "2008-10-31T09:05:10Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Map.__pow__ should return identity for power 0",
    "type": "issue",
    "updated_at": "2008-10-31T15:36:46Z",
    "url": "https://github.com/sagemath/sage/issues/4410",
    "user": "https://github.com/burcin"
}
```
`sage.categories.map.Map.__pow__` calls `generic_power`, which messes up power 0. There is this todo note there:

```
        # todo -- what about the case n=0 -- need to specify the identity map somehow.
```

Attached patch returns the identity map for power 0.

CC:  @robertwb

Component: **basic arithmetic**

_Issue created by migration from https://trac.sagemath.org/ticket/4410_





---

archive/issue_events_049822.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2008-10-31T09:05:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4410",
    "milestone_number": null,
    "milestone_title": "sage-3.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4410#event-49822"
}
```



---

archive/issue_events_049823.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2008-10-31T09:05:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4410",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "c: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4410#event-49823"
}
```



---

archive/issue_events_049824.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2008-10-31T09:05:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4410",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4410#event-49824"
}
```



---

archive/issue_events_049825.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2008-10-31T09:05:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4410",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4410#event-49825"
}
```



---

archive/issue_events_049826.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2008-10-31T09:05:10Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4410",
    "subject": "https://github.com/burcin",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4410#event-49826"
}
```



---

archive/issue_events_049827.json:
```json
{
    "actor": "https://github.com/burcin",
    "created_at": "2008-10-31T09:05:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4410",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4410#event-49827"
}
```



---

archive/issue_comments_025556.json:
```json
{
    "body": "make Map.__pow__ return identity for power 0",
    "created_at": "2008-10-31T09:06:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4410",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4410#issuecomment-25556",
    "user": "https://github.com/burcin"
}
```

make Map.__pow__ return identity for power 0



---

archive/issue_events_049828.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-10-31T15:35:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4410",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4410#event-49828"
}
```



---

archive/issue_comments_025557.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_4410_map_pow.patch.gz](https://github.com/sagemath/sage/files/ticket4410/trac_4410_map_pow.patch.gz)**\n\nThis is the suggested fix discussed by RobertWB and Burcin in IRC last night. The code looks correct and passes doctests, positive review.\n\nCheers,\n\nMichael",
    "created_at": "2008-10-31T15:35:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4410",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4410#issuecomment-25557",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_4410_map_pow.patch.gz](https://github.com/sagemath/sage/files/ticket4410/trac_4410_map_pow.patch.gz)**

This is the suggested fix discussed by RobertWB and Burcin in IRC last night. The code looks correct and passes doctests, positive review.

Cheers,

Michael



---

archive/issue_comments_025558.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nMerged in Sage 3.2.alpha2",
    "created_at": "2008-10-31T15:36:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4410",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4410#issuecomment-25558",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Merged in Sage 3.2.alpha2



---

archive/issue_events_049829.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-10-31T15:36:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4410",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4410#event-49829"
}
```



---

archive/issue_events_049830.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-10-31T15:36:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4410",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4410#event-49830"
}
```
