# Issue 4178: dist_functions.py doctest timeout is too small

archive/issues_004178.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-GeorgSWeber"
    ],
    "body": "On my PPC Power Book 550 MHz, 768 MB RAM, Mac OS X 10.4.11, Xcode 2.5, the \"long\" doctest fails because the timeout is too small.\n\nAs soon as I have checked how big it should be (on that box) to make it run through, I'll add the info here. \n\nComponent: **doctest coverage**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4178_\n\n",
    "closed_at": "2008-09-23T22:04:21Z",
    "created_at": "2008-09-23T21:58:17Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "dist_functions.py doctest timeout is too small",
    "type": "issue",
    "updated_at": "2008-09-23T22:04:21Z",
    "url": "https://github.com/sagemath/sage/issues/4178",
    "user": "https://github.com/sagetrac-GeorgSWeber"
}
```
On my PPC Power Book 550 MHz, 768 MB RAM, Mac OS X 10.4.11, Xcode 2.5, the "long" doctest fails because the timeout is too small.

As soon as I have checked how big it should be (on that box) to make it run through, I'll add the info here. 

Component: **doctest coverage**

_Issue created by migration from https://trac.sagemath.org/ticket/4178_





---

archive/issue_events_046790.json:
```json
{
    "actor": "https://github.com/sagetrac-GeorgSWeber",
    "created_at": "2008-09-23T21:58:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4178",
    "milestone_number": null,
    "milestone_title": "sage-3.1.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4178#event-46790"
}
```



---

archive/issue_events_046791.json:
```json
{
    "actor": "https://github.com/sagetrac-GeorgSWeber",
    "created_at": "2008-09-23T21:58:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4178",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4178#event-46791"
}
```



---

archive/issue_events_046792.json:
```json
{
    "actor": "https://github.com/sagetrac-GeorgSWeber",
    "created_at": "2008-09-23T21:58:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4178",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4178#event-46792"
}
```



---

archive/issue_events_046793.json:
```json
{
    "actor": "https://github.com/sagetrac-GeorgSWeber",
    "created_at": "2008-09-23T21:58:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4178",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4178#event-46793"
}
```



---

archive/issue_events_046794.json:
```json
{
    "actor": "https://github.com/sagetrac-GeorgSWeber",
    "created_at": "2008-09-23T21:58:17Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4178",
    "subject": "https://github.com/sagetrac-GeorgSWeber",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4178#event-46794"
}
```



---

archive/issue_events_046795.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-09-23T22:04:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4178",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4178#event-46795"
}
```



---

archive/issue_events_046796.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-09-23T22:04:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4178",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4178#event-46796"
}
```



---

archive/issue_events_046797.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-09-23T22:04:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4178",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4178#event-46797"
}
```



---

archive/issue_events_046798.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-09-23T22:04:21Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4178",
    "milestone_number": null,
    "milestone_title": "sage-3.1.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4178#event-46798"
}
```



---

archive/issue_comments_023783.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nNope, this is a won't fix. The long timeout is *huge*, i.e. 30 minutes. The problem is another one, i.e. ecm not returning and hence the doctest never finishes. This issue is already covered by another DSage ticket.\n\nCheers,\n\nMichael",
    "created_at": "2008-09-23T22:04:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4178",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4178#issuecomment-23783",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Nope, this is a won't fix. The long timeout is *huge*, i.e. 30 minutes. The problem is another one, i.e. ecm not returning and hence the doctest never finishes. This issue is already covered by another DSage ticket.

Cheers,

Michael
