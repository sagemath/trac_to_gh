# Issue 4453: elliptic curves -- heegner_index command gives nonsense when rank > 1

archive/issues_004453.json:
```json
{
    "assignees": [],
    "body": "For any elliptic curve over QQ of rank >= 2 the heegner_index command must always give +Infinity as output.   So the following 1 at the end is just wrong.\n\n```\nsage: E = EllipticCurve('389a')\nsage: D = E.heegner_discriminants_list(1)[0]\nsage: D\n-7\nsage: E.heegner_index(D)\n1\n```\n\nApply trac_4453-rebased.patch and 4453-doctest-fix.patch\n\n**Reviewer:** John Cremona, Robert Bradshaw\n\n**Author:** William Stein\n\n**Merged:** sage-5.5.beta1\n\nIssue created by migration from https://trac.sagemath.org/ticket/4453\n\n",
    "closed_at": "2012-11-01T12:01:54Z",
    "created_at": "2008-11-06T14:47:03Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20elliptic%20curves",
        "https://github.com/sagemath/sage/labels/minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "elliptic curves -- heegner_index command gives nonsense when rank > 1",
    "type": "issue",
    "updated_at": "2012-11-01T12:01:54Z",
    "url": "https://github.com/sagemath/sage/issues/4453",
    "user": "https://github.com/williamstein"
}
```
For any elliptic curve over QQ of rank >= 2 the heegner_index command must always give +Infinity as output.   So the following 1 at the end is just wrong.

```
sage: E = EllipticCurve('389a')
sage: D = E.heegner_discriminants_list(1)[0]
sage: D
-7
sage: E.heegner_index(D)
1
```

Apply trac_4453-rebased.patch and 4453-doctest-fix.patch

**Reviewer:** John Cremona, Robert Bradshaw

**Author:** William Stein

**Merged:** sage-5.5.beta1

Issue created by migration from https://trac.sagemath.org/ticket/4453





---

archive/issue_events_041180.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-06T14:47:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "milestone_number": null,
    "milestone_title": "sage-5.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41180"
}
```



---

archive/issue_events_041181.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-06T14:47:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20theory",
    "label_color": "08517b",
    "label_name": "component: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41181"
}
```



---

archive/issue_events_041182.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-06T14:47:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/minor",
    "label_color": "ff0000",
    "label_name": "minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41182"
}
```



---

archive/issue_events_041183.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-06T14:47:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "008080",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41183"
}
```



---

archive/issue_events_041184.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-20T19:49:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20theory",
    "label_color": "08517b",
    "label_name": "component: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41184"
}
```



---

archive/issue_events_041185.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2009-07-20T19:49:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20elliptic%20curves",
    "label_color": "08517b",
    "label_name": "component: elliptic curves",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41185"
}
```



---

archive/issue_comments_026900.json:
```json
{
    "body": "**Changing assignee** from @williamstein to @loefflerd.",
    "created_at": "2009-07-20T19:49:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26900",
    "user": "https://github.com/loefflerd"
}
```

**Changing assignee** from @williamstein to @loefflerd.



---

archive/issue_comments_026901.json:
```json
{
    "body": "**Remove assignee** @loefflerd.",
    "created_at": "2009-10-09T09:14:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26901",
    "user": "https://github.com/loefflerd"
}
```

**Remove assignee** @loefflerd.



---

archive/issue_events_041186.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2010-01-18T04:35:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "008080",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41186"
}
```



---

archive/issue_comments_026902.json:
```json
{
    "body": "<a id='comment:4'>**Comment 4:**</a>\nDepends on #6616?",
    "created_at": "2010-01-20T10:18:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26902",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:4'>**Comment 4:**</a>
Depends on #6616?



---

archive/issue_comments_026903.json:
```json
{
    "body": "**Author:** was",
    "created_at": "2010-01-31T17:31:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26903",
    "user": "https://github.com/JohnCremona"
}
```

**Author:** was



---

archive/issue_events_041187.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2010-01-31T17:31:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "008080",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41187"
}
```



---

archive/issue_events_041188.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2010-01-31T17:31:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "008080",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41188"
}
```



---

archive/issue_comments_026904.json:
```json
{
    "body": "**Reviewer:** cremona",
    "created_at": "2010-01-31T17:31:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26904",
    "user": "https://github.com/JohnCremona"
}
```

**Reviewer:** cremona



---

archive/issue_comments_026905.json:
```json
{
    "body": "<a id='comment:5'>**Comment 5:**</a>\nPositive review: looks good (apart from a docstring typo \"We higher rank examples\") and applies fine to 4.3.1, tests pass and docs build & look good.\n\nI added a new patch which changes the above to \"Some higher rank examples\" with no further changes.",
    "created_at": "2010-01-31T17:31:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26905",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:5'>**Comment 5:**</a>
Positive review: looks good (apart from a docstring typo "We higher rank examples") and applies fine to 4.3.1, tests pass and docs build & look good.

I added a new patch which changes the above to "Some higher rank examples" with no further changes.



---

archive/issue_comments_026906.json:
```json
{
    "body": "<a id='comment:6'>**Comment 6:**</a>\nI get two hunk failures when applying [trac_4453.2.patch](https://github.com/sagemath/sage/files/ticket4453/trac_4453.2.patch.gz) on top of Sage 4.3.3.alpha0:\n\n```\n[mvngu@sage sage-main]$ pwd\n/dev/shm/mvngu/sage-4.3.3.alpha0/devel/sage-main\n[mvngu@sage sage-main]$ hg qimport https://github.com/sagemath/sage/files/ticket4453/trac_4453.2.patch.gz && hg qpush\nadding trac_4453.2.patch to series file\napplying trac_4453.2.patch\npatching file sage/schemes/elliptic_curves/heegner.py\nHunk #3 FAILED at 6350\nHunk #4 succeeded at 6370 with fuzz 2 (offset -12 lines).\nHunk #5 FAILED at 6430\n2 out of 5 hunks FAILED -- saving rejects to file sage/schemes/elliptic_curves/heegner.py.rej\npatch failed, unable to continue (try -v)\npatch failed, rejects left in working dir\nerrors during apply, please fix and refresh trac_4453.2.patch\n```\nThe attachment [trac_4453.2.patch](https://github.com/sagemath/sage/files/ticket4453/trac_4453.2.patch.gz) needs rebase against Sage 4.3.3.alpha0.",
    "created_at": "2010-02-13T05:59:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26906",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<a id='comment:6'>**Comment 6:**</a>
I get two hunk failures when applying [trac_4453.2.patch](https://github.com/sagemath/sage/files/ticket4453/trac_4453.2.patch.gz) on top of Sage 4.3.3.alpha0:

```
[mvngu@sage sage-main]$ pwd
/dev/shm/mvngu/sage-4.3.3.alpha0/devel/sage-main
[mvngu@sage sage-main]$ hg qimport https://github.com/sagemath/sage/files/ticket4453/trac_4453.2.patch.gz && hg qpush
adding trac_4453.2.patch to series file
applying trac_4453.2.patch
patching file sage/schemes/elliptic_curves/heegner.py
Hunk #3 FAILED at 6350
Hunk #4 succeeded at 6370 with fuzz 2 (offset -12 lines).
Hunk #5 FAILED at 6430
2 out of 5 hunks FAILED -- saving rejects to file sage/schemes/elliptic_curves/heegner.py.rej
patch failed, unable to continue (try -v)
patch failed, rejects left in working dir
errors during apply, please fix and refresh trac_4453.2.patch
```
The attachment [trac_4453.2.patch](https://github.com/sagemath/sage/files/ticket4453/trac_4453.2.patch.gz) needs rebase against Sage 4.3.3.alpha0.



---

archive/issue_events_041189.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-02-13T05:59:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "008080",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41189"
}
```



---

archive/issue_events_041190.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-02-13T05:59:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "008080",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41190"
}
```



---

archive/issue_comments_026907.json:
```json
{
    "body": "**Work Issues:** rebase against Sage 4.3.3.alpha0",
    "created_at": "2010-02-13T05:59:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26907",
    "user": "https://github.com/sagetrac-mvngu"
}
```

**Work Issues:** rebase against Sage 4.3.3.alpha0



---

archive/issue_comments_026908.json:
```json
{
    "body": "**Attachment:** [trac_4453-rebased.patch.gz](https://github.com/sagemath/sage/files/ticket4453/trac_4453-rebased.patch.gz)",
    "created_at": "2010-07-29T06:02:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26908",
    "user": "https://github.com/robertwb"
}
```

**Attachment:** [trac_4453-rebased.patch.gz](https://github.com/sagemath/sage/files/ticket4453/trac_4453-rebased.patch.gz)



---

archive/issue_comments_026909.json:
```json
{
    "body": "<a id='comment:7'>**Comment 7:**</a>\nRebased, I also fixed a small error when check_rank=False.",
    "created_at": "2010-07-29T06:03:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26909",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:7'>**Comment 7:**</a>
Rebased, I also fixed a small error when check_rank=False.



---

archive/issue_events_041191.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-07-29T06:03:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "008080",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41191"
}
```



---

archive/issue_events_041192.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-07-29T06:03:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "008080",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41192"
}
```



---

archive/issue_comments_026910.json:
```json
{
    "body": "**Changing reviewer** from \"cremona\" to \"John Cremona\".",
    "created_at": "2010-08-07T09:58:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26910",
    "user": "https://github.com/qed777"
}
```

**Changing reviewer** from "cremona" to "John Cremona".



---

archive/issue_comments_026911.json:
```json
{
    "body": "**Changing author** from \"was\" to \"William Stein\".",
    "created_at": "2010-08-07T09:58:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26911",
    "user": "https://github.com/qed777"
}
```

**Changing author** from "was" to "William Stein".



---

archive/issue_comments_026912.json:
```json
{
    "body": "**Changing work issues** from \"rebase against Sage 4.3.3.alpha0\" to \"\".",
    "created_at": "2010-08-07T09:58:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26912",
    "user": "https://github.com/qed777"
}
```

**Changing work issues** from "rebase against Sage 4.3.3.alpha0" to "".



---

archive/issue_comments_026913.json:
```json
{
    "body": "<a id='comment:8'>**Comment 8:**</a>\nWith the forthcoming 4.5.2 (4.5.2.rc0 + #9226) and the rebased patch, I get a doctest failure:\n\n```python\nsage -t -long \"devel/sage/sage/schemes/elliptic_curves/heegner.py\"\n**********************************************************************\nFile \"/mnt/usb1/scratch/mpatel/apps/sage-4.5.2/devel/sage/sage/schemes/elliptic_curves/heegner.py\", line 6465:\n    sage: E.heegner_index(-8, descent_second_limit=16)\nException raised:\n    Traceback (most recent call last):\n      File \"/mnt/usb1/scratch/mpatel/apps/sage-4.5.2/local/bin/ncadoctest.py\", line 1231, in run_one_test\n        self.run_one_example(test, example, filename, compileflags)\n      File \"/mnt/usb1/scratch/mpatel/apps/sage-4.5.2/local/bin/sagedoctest.py\", line 38, in run_one_example\n        OrigDocTestRunner.run_one_example(self, test, example, filename, compileflags)\n      File \"/mnt/usb1/scratch/mpatel/apps/sage-4.5.2/local/bin/ncadoctest.py\", line 1172, in run_one_example\n        compileflags, 1) in test.globs\n      File \"<doctest __main__.example_229[14]>\", line 1, in <module>\n        E.heegner_index(-Integer(8), descent_second_limit=Integer(16))###line 6465: \n    sage: E.heegner_index(-8, descent_second_limit=16)\n      File \"/mnt/usb1/scratch/mpatel/apps/sage-4.5.2/local/lib/python/site-packages/sage/schemes/elliptic_curves/heegner.py\", line 6485, in heegner_index\n        if check_rank and self.rank() >= 2:\n      File \"/mnt/usb1/scratch/mpatel/apps/sage-4.5.2/local/lib/python/site-packages/sage/schemes/elliptic_curves/ell_rational_field.py\", line 1741, in rank\n        raise RuntimeError, 'Rank not provably correct.'\n    RuntimeError: Rank not provably correct.\n**********************************************************************\n1 items had failures:\n   1 of  21 in __main__.example_229\n***Test Failed*** 1 failures.\n```",
    "created_at": "2010-08-07T09:58:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26913",
    "user": "https://github.com/qed777"
}
```

<a id='comment:8'>**Comment 8:**</a>
With the forthcoming 4.5.2 (4.5.2.rc0 + #9226) and the rebased patch, I get a doctest failure:

```python
sage -t -long "devel/sage/sage/schemes/elliptic_curves/heegner.py"
**********************************************************************
File "/mnt/usb1/scratch/mpatel/apps/sage-4.5.2/devel/sage/sage/schemes/elliptic_curves/heegner.py", line 6465:
    sage: E.heegner_index(-8, descent_second_limit=16)
Exception raised:
    Traceback (most recent call last):
      File "/mnt/usb1/scratch/mpatel/apps/sage-4.5.2/local/bin/ncadoctest.py", line 1231, in run_one_test
        self.run_one_example(test, example, filename, compileflags)
      File "/mnt/usb1/scratch/mpatel/apps/sage-4.5.2/local/bin/sagedoctest.py", line 38, in run_one_example
        OrigDocTestRunner.run_one_example(self, test, example, filename, compileflags)
      File "/mnt/usb1/scratch/mpatel/apps/sage-4.5.2/local/bin/ncadoctest.py", line 1172, in run_one_example
        compileflags, 1) in test.globs
      File "<doctest __main__.example_229[14]>", line 1, in <module>
        E.heegner_index(-Integer(8), descent_second_limit=Integer(16))###line 6465: 
    sage: E.heegner_index(-8, descent_second_limit=16)
      File "/mnt/usb1/scratch/mpatel/apps/sage-4.5.2/local/lib/python/site-packages/sage/schemes/elliptic_curves/heegner.py", line 6485, in heegner_index
        if check_rank and self.rank() >= 2:
      File "/mnt/usb1/scratch/mpatel/apps/sage-4.5.2/local/lib/python/site-packages/sage/schemes/elliptic_curves/ell_rational_field.py", line 1741, in rank
        raise RuntimeError, 'Rank not provably correct.'
    RuntimeError: Rank not provably correct.
**********************************************************************
1 items had failures:
   1 of  21 in __main__.example_229
***Test Failed*** 1 failures.
```



---

archive/issue_events_041193.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-08-07T09:58:31Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "milestone_number": null,
    "milestone_title": "sage-4.5.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41193"
}
```



---

archive/issue_events_041194.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-08-07T09:58:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "milestone_number": null,
    "milestone_title": "sage-4.5.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41194"
}
```



---

archive/issue_events_041195.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-08-07T09:58:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "008080",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41195"
}
```



---

archive/issue_events_041196.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-08-07T09:58:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "008080",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41196"
}
```



---

archive/issue_comments_026914.json:
```json
{
    "body": "<a id='comment:9'>**Comment 9:**</a>\nReplying to [@qed777](#comment%3A8):\n> With the forthcoming 4.5.2 (4.5.2.rc0 + #9226) and the rebased patch, I get a doctest failure:\n\nThat should be \"4.5.2 (4.5.2.rc1 + #9226)\".",
    "created_at": "2010-08-07T10:00:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26914",
    "user": "https://github.com/qed777"
}
```

<a id='comment:9'>**Comment 9:**</a>
Replying to [@qed777](#comment%3A8):
> With the forthcoming 4.5.2 (4.5.2.rc0 + #9226) and the rebased patch, I get a doctest failure:

That should be "4.5.2 (4.5.2.rc1 + #9226)".



---

archive/issue_comments_026915.json:
```json
{
    "body": "<a id='comment:10'>**Comment 10:**</a>\nthis patch still applies fine to 4.7.1, and the new examples in the patch seem to work.\nHowever the description says that the following should give 0 and not `+Infinity`:\n\n```\nsage: E = EllipticCurve('389a')\nsage: E.heegner_index(-7)\n+Infinity\n```\nIs the description wrong?\n\nPaul",
    "created_at": "2011-09-15T16:06:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26915",
    "user": "https://github.com/zimmermann6"
}
```

<a id='comment:10'>**Comment 10:**</a>
this patch still applies fine to 4.7.1, and the new examples in the patch seem to work.
However the description says that the following should give 0 and not `+Infinity`:

```
sage: E = EllipticCurve('389a')
sage: E.heegner_index(-7)
+Infinity
```
Is the description wrong?

Paul



---

archive/issue_events_041197.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2011-09-15T16:06:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "008080",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41197"
}
```



---

archive/issue_events_041198.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2011-09-15T16:06:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "008080",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41198"
}
```



---

archive/issue_comments_026916.json:
```json
{
    "body": "<a id='comment:11'>**Comment 11:**</a>\nThe description is wrong, the Heegner point is torsion for rank >= 2 curves, hence its index in the full group is infinite.",
    "created_at": "2011-09-15T23:25:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26916",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:11'>**Comment 11:**</a>
The description is wrong, the Heegner point is torsion for rank >= 2 curves, hence its index in the full group is infinite.



---

archive/issue_comments_026917.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-For any elliptic curve over QQ of rank >= 2 the heegner_index command must always give 0 as output.   So the following 1 at the end is just wrong.\n+For any elliptic curve over QQ of rank >= 2 the heegner_index command must always give +Infinity as output.   So the following 1 at the end is just wrong.\n \n ```\n sage: E = EllipticCurve('389a')\n``````\n",
    "created_at": "2011-09-16T06:26:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26917",
    "user": "https://github.com/zimmermann6"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-For any elliptic curve over QQ of rank >= 2 the heegner_index command must always give 0 as output.   So the following 1 at the end is just wrong.
+For any elliptic curve over QQ of rank >= 2 the heegner_index command must always give +Infinity as output.   So the following 1 at the end is just wrong.
 
 ```
 sage: E = EllipticCurve('389a')
``````




---

archive/issue_comments_026918.json:
```json
{
    "body": "<a id='comment:12'>**Comment 12:**</a>\nthanks Robert. Is the new description ok? I put back as \"needs review\" to check if the doctests\npass.\n\nPaul",
    "created_at": "2011-09-16T06:26:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26918",
    "user": "https://github.com/zimmermann6"
}
```

<a id='comment:12'>**Comment 12:**</a>
thanks Robert. Is the new description ok? I put back as "needs review" to check if the doctests
pass.

Paul



---

archive/issue_events_041199.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2011-09-16T06:26:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "008080",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41199"
}
```



---

archive/issue_events_041200.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2011-09-16T06:26:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "008080",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41200"
}
```



---

archive/issue_comments_026919.json:
```json
{
    "body": "**Work Issues:** one doctest fails",
    "created_at": "2011-09-16T09:06:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26919",
    "user": "https://github.com/zimmermann6"
}
```

**Work Issues:** one doctest fails



---

archive/issue_events_041201.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2011-09-16T09:06:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "008080",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41201"
}
```



---

archive/issue_events_041202.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2011-09-16T09:06:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "008080",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41202"
}
```



---

archive/issue_comments_026920.json:
```json
{
    "body": "<a id='comment:13'>**Comment 13:**</a>\nthe failure mentioned in comment [comment:8] is still there:\n\n```\nsage: E = EllipticCurve([0, 0, 1, -34874, -2506691])\nsage: E.heegner_index(-8, descent_second_limit=16)\n...\nRuntimeError: Rank not provably correct.\n```\nthus this ticket needs work.\n\nPaul",
    "created_at": "2011-09-16T09:06:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26920",
    "user": "https://github.com/zimmermann6"
}
```

<a id='comment:13'>**Comment 13:**</a>
the failure mentioned in comment [comment:8] is still there:

```
sage: E = EllipticCurve([0, 0, 1, -34874, -2506691])
sage: E.heegner_index(-8, descent_second_limit=16)
...
RuntimeError: Rank not provably correct.
```
thus this ticket needs work.

Paul



---

archive/issue_comments_026921.json:
```json
{
    "body": "<a id='comment:14'>**Comment 14:**</a>\n**Attachment:** [4453-doctest-fix.patch.gz](https://github.com/sagemath/sage/files/ticket4453/4453-doctest-fix.patch.gz)\n\nApply trac_4453-rebased.patch, 4453-doctest-fix.patch",
    "created_at": "2012-10-24T07:22:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26921",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:14'>**Comment 14:**</a>
**Attachment:** [4453-doctest-fix.patch.gz](https://github.com/sagemath/sage/files/ticket4453/4453-doctest-fix.patch.gz)

Apply trac_4453-rebased.patch, 4453-doctest-fix.patch



---

archive/issue_events_041203.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2012-10-24T07:22:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "008080",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41203"
}
```



---

archive/issue_events_041204.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2012-10-24T07:22:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "008080",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41204"
}
```



---

archive/issue_comments_026922.json:
```json
{
    "body": "**Changing reviewer** from \"John Cremona\" to \"John Cremona, Robert Bradshaw\".",
    "created_at": "2012-10-24T07:23:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26922",
    "user": "https://github.com/robertwb"
}
```

**Changing reviewer** from "John Cremona" to "John Cremona, Robert Bradshaw".



---

archive/issue_comments_026923.json:
```json
{
    "body": "**Changing work issues** from \"one doctest fails\" to \"\".",
    "created_at": "2012-10-24T07:23:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26923",
    "user": "https://github.com/robertwb"
}
```

**Changing work issues** from "one doctest fails" to "".



---

archive/issue_events_041205.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2012-10-24T07:23:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "008080",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41205"
}
```



---

archive/issue_events_041206.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2012-10-24T07:23:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "008080",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41206"
}
```



---

archive/issue_comments_026924.json:
```json
{
    "body": "<a id='comment:16'>**Comment 16:**</a>\nRobert, I find it strange that you give a positive review to your own patch...\n\nPaul",
    "created_at": "2012-10-24T07:27:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26924",
    "user": "https://github.com/zimmermann6"
}
```

<a id='comment:16'>**Comment 16:**</a>
Robert, I find it strange that you give a positive review to your own patch...

Paul



---

archive/issue_comments_026925.json:
```json
{
    "body": "<a id='comment:17'>**Comment 17:**</a>\nI was giving a positive review to the original patch; I suppose someone should review my 1-line doctest fix as well.",
    "created_at": "2012-10-24T07:30:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26925",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:17'>**Comment 17:**</a>
I was giving a positive review to the original patch; I suppose someone should review my 1-line doctest fix as well.



---

archive/issue_events_041207.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2012-10-24T07:30:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "008080",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41207"
}
```



---

archive/issue_events_041208.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2012-10-24T07:30:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "008080",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41208"
}
```



---

archive/issue_events_041209.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2012-10-24T07:31:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "008080",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41209"
}
```



---

archive/issue_events_041210.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2012-10-24T07:31:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "008080",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41210"
}
```



---

archive/issue_comments_026926.json:
```json
{
    "body": "<a id='comment:19'>**Comment 19:**</a>\nI'll check Robert's one-liner but not this week.",
    "created_at": "2012-10-24T08:31:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26926",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:19'>**Comment 19:**</a>
I'll check Robert's one-liner but not this week.



---

archive/issue_events_041211.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-10-24T09:12:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "008080",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41211"
}
```



---

archive/issue_events_041212.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2012-10-24T09:12:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "008080",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41212"
}
```



---

archive/issue_comments_026927.json:
```json
{
    "body": "<a id='comment:20'>**Comment 20:**</a>\nRob's patch looks fine to me, and patchbot's happy with it, so let's get this in.",
    "created_at": "2012-10-24T09:12:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26927",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:20'>**Comment 20:**</a>
Rob's patch looks fine to me, and patchbot's happy with it, so let's get this in.



---

archive/issue_events_041213.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-10-29T21:29:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "milestone_number": null,
    "milestone_title": "sage-5.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41213"
}
```



---

archive/issue_events_041214.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-10-29T21:29:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "milestone_number": null,
    "milestone_title": "sage-5.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41214"
}
```



---

archive/issue_comments_026928.json:
```json
{
    "body": "<a id='comment:22'>**Comment 22:**</a>\nPlease specify which patch(es) to apply.",
    "created_at": "2012-10-30T16:10:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26928",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:22'>**Comment 22:**</a>
Please specify which patch(es) to apply.



---

archive/issue_events_041215.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-10-30T16:10:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "008080",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41215"
}
```



---

archive/issue_events_041216.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-10-30T16:10:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "008080",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41216"
}
```



---

archive/issue_comments_026929.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -8,3 +8,5 @@\n sage: E.heegner_index(D)\n 1\n ```\n+\n+Apply trac_4453-rebased.patch and 4453-doctest-fix.patch\n``````\n",
    "created_at": "2012-10-30T17:27:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26929",
    "user": "https://github.com/robertwb"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -8,3 +8,5 @@
 sage: E.heegner_index(D)
 1
 ```
+
+Apply trac_4453-rebased.patch and 4453-doctest-fix.patch
``````




---

archive/issue_events_041217.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2012-10-30T17:27:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "008080",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41217"
}
```



---

archive/issue_events_041218.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2012-10-30T17:27:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "008080",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41218"
}
```



---

archive/issue_events_041219.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2012-10-30T17:28:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "008080",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41219"
}
```



---

archive/issue_events_041220.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2012-10-30T17:28:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "008080",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41220"
}
```



---

archive/issue_comments_026930.json:
```json
{
    "body": "**Merged:** sage-5.5.beta1",
    "created_at": "2012-11-01T12:01:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4453#issuecomment-26930",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-5.5.beta1



---

archive/issue_events_041221.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-11-01T12:01:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "008080",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41221"
}
```



---

archive/issue_events_041222.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2012-11-01T12:01:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4453",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4453#event-41222"
}
```
