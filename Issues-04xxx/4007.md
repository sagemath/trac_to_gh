# Issue 4007: [with spkg, positive review] OSX 10.4/5: build libpng.dylib again

archive/issues_004007.json:
```json
{
    "body": "On OSX we delete libpng.dylib due to missing symbols when building R and python. But this is causing trouble for code at #3324 for example. The spkg at \n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.1.2/alpha3/libpng-1.2.22.p8.spkg\n\ndoes no longer delete the old dynamic lib. But this ticket needs to be merged with the following two tickets updating R and python.\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4007\n\n",
    "closed_at": "2008-08-30T23:51:57Z",
    "created_at": "2008-08-30T23:39:07Z",
    "labels": [
        "component: build",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1.2",
    "title": "[with spkg, positive review] OSX 10.4/5: build libpng.dylib again",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4007",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
On OSX we delete libpng.dylib due to missing symbols when building R and python. But this is causing trouble for code at #3324 for example. The spkg at 

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.1.2/alpha3/libpng-1.2.22.p8.spkg

does no longer delete the old dynamic lib. But this ticket needs to be merged with the following two tickets updating R and python.

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4007





---

archive/issue_comments_030517.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-08-30T23:43:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4007",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4007#issuecomment-30517",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing status from new to assigned.



---

archive/issue_events_014949.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4007",
    "rename": {
        "from": "OSX 10.4/5: build libpng.dylib again",
        "to": "[with spkg, needs review] OSX 10.4/5: build libpng.dylib again"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4007#event-14949"
}
```



---

archive/issue_events_014950.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4007",
    "rename": {
        "from": "[with spkg, needs review] OSX 10.4/5: build libpng.dylib again",
        "to": "[with spkg, positive review] OSX 10.4/5: build libpng.dylib again"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4007#event-14950"
}
```



---

archive/issue_events_014951.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-08-30T23:51:57Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4007",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4007#event-14951"
}
```



---

archive/issue_comments_030518.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-08-30T23:51:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4007",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4007#issuecomment-30518",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_030519.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.1.2.alpha3",
    "created_at": "2008-08-30T23:51:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4007",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4007#issuecomment-30519",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.1.2.alpha3
