# Issue 4229: special functions should use mpfr when available

archive/issues_004229.json:
```json
{
    "body": "MPFR has fast implementations for restricted types of arguments in some special functions, e.g. Bessel J and Y with integer order and positive real argument.  We should be using these instead of Pari or Maxima or Scipy whenever that is feasible.\n\nExample:\n\n```\nsage: a = RR(2)\nsage: timeit(\"bessel_J(1, a)\")\n625 loops, best of 3: 370 \u00b5s per loop\nsage: timeit(\"a.j1()\")\n625 loops, best of 3: 13.9 \u00b5s per loop\n```\n\nThat's 26 times faster than Pari.\n\nAssignee: @burcin\n\nCC:  @jdemeyer\n\nKeywords: sd31\n\nResolution: duplicate\n\nReviewer: Burcin Erocal, Karl-Dieter Crisman\n\nIssue created by migration from https://trac.sagemath.org/ticket/4229\n\n",
    "closed_at": "2011-06-14T20:39:08Z",
    "created_at": "2008-10-01T09:55:56Z",
    "labels": [
        "component: calculus",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "special functions should use mpfr when available",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4229",
    "user": "https://github.com/aghitza"
}
```
MPFR has fast implementations for restricted types of arguments in some special functions, e.g. Bessel J and Y with integer order and positive real argument.  We should be using these instead of Pari or Maxima or Scipy whenever that is feasible.

Example:

```
sage: a = RR(2)
sage: timeit("bessel_J(1, a)")
625 loops, best of 3: 370 µs per loop
sage: timeit("a.j1()")
625 loops, best of 3: 13.9 µs per loop
```

That's 26 times faster than Pari.

Assignee: @burcin

CC:  @jdemeyer

Keywords: sd31

Resolution: duplicate

Reviewer: Burcin Erocal, Karl-Dieter Crisman

Issue created by migration from https://trac.sagemath.org/ticket/4229





---

archive/issue_comments_033235.json:
```json
{
    "body": "<a id='comment:1'></a>Or maybe we should use mpmath - there are a number of tickets about that.  Such as Alex's own comment in #3426 :)",
    "created_at": "2011-03-16T15:38:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4229#issuecomment-33235",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:1'></a>Or maybe we should use mpmath - there are a number of tickets about that.  Such as Alex's own comment in #3426 :)



---

archive/issue_comments_033236.json:
```json
{
    "body": "Changing upstream from \"\" to \"N/A\"",
    "created_at": "2011-03-16T15:38:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4229#issuecomment-33236",
    "user": "https://github.com/kcrisman"
}
```

Changing upstream from "" to "N/A"



---

archive/issue_comments_033237.json:
```json
{
    "body": "Changing priority from major to minor.",
    "created_at": "2011-03-16T15:38:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4229#issuecomment-33237",
    "user": "https://github.com/kcrisman"
}
```

Changing priority from major to minor.



---

archive/issue_comments_033238.json:
```json
{
    "body": "<a id='comment:2'></a>This ticket is too broad, I suggest we close it as invalid.\n\nAt the time it was created, there was no general framework to handle these functions. The (not so) new pynac-based symbolics provide this framework. It is true that a lot of work is still needed to sort these numerical evaluation issues out, but we need a separate specific ticket for each issue.\n\nSee [symbolics/functions](symbolics-functions) for an overview on the progress of symbolic functions.",
    "created_at": "2011-06-14T18:07:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4229#issuecomment-33238",
    "user": "https://github.com/burcin"
}
```

<a id='comment:2'></a>This ticket is too broad, I suggest we close it as invalid.

At the time it was created, there was no general framework to handle these functions. The (not so) new pynac-based symbolics provide this framework. It is true that a lot of work is still needed to sort these numerical evaluation issues out, but we need a separate specific ticket for each issue.

See [symbolics/functions](symbolics-functions) for an overview on the progress of symbolic functions.



---

archive/issue_comments_033239.json:
```json
{
    "body": "Changing keywords from \"\" to \"sd31\".",
    "created_at": "2011-06-14T18:07:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4229#issuecomment-33239",
    "user": "https://github.com/burcin"
}
```

Changing keywords from "" to "sd31".



---

archive/issue_comments_033240.json:
```json
{
    "body": "Changing author from \"\" to \"Burcin Erocal\"",
    "created_at": "2011-06-14T18:07:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4229#issuecomment-33240",
    "user": "https://github.com/burcin"
}
```

Changing author from "" to "Burcin Erocal"



---

archive/issue_comments_033241.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2011-06-14T18:07:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4229#issuecomment-33241",
    "user": "https://github.com/burcin"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_033242.json:
```json
{
    "body": "<a id='comment:3'></a>Agreed.  This wiki page solves the problem.",
    "created_at": "2011-06-14T18:09:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4229#issuecomment-33242",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:3'></a>Agreed.  This wiki page solves the problem.



---

archive/issue_comments_033243.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2011-06-14T18:09:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4229#issuecomment-33243",
    "user": "https://github.com/kcrisman"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_033244.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Karl-Dieter Crisman\"",
    "created_at": "2011-06-14T18:09:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4229#issuecomment-33244",
    "user": "https://github.com/kcrisman"
}
```

Changing reviewer from "" to "Karl-Dieter Crisman"



---

archive/issue_events_009569.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-14T19:11:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4229",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4229#event-9569"
}
```



---

archive/issue_comments_033245.json:
```json
{
    "body": "Resolution: duplicate",
    "created_at": "2011-06-14T20:39:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4229#issuecomment-33245",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: duplicate



---

archive/issue_comments_033246.json:
```json
{
    "body": "Changing author from \"Burcin Erocal\" to \"\"",
    "created_at": "2011-06-14T20:39:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4229#issuecomment-33246",
    "user": "https://github.com/jdemeyer"
}
```

Changing author from "Burcin Erocal" to ""



---

archive/issue_comments_033247.json:
```json
{
    "body": "Changing reviewer from \"Karl-Dieter Crisman\" to \"Burcin Erocal, Karl-Dieter Crisman\"",
    "created_at": "2011-06-14T20:39:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4229",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4229#issuecomment-33247",
    "user": "https://github.com/jdemeyer"
}
```

Changing reviewer from "Karl-Dieter Crisman" to "Burcin Erocal, Karl-Dieter Crisman"



---

archive/issue_events_009570.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2011-06-14T20:39:08Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4229",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4229#event-9570"
}
```
