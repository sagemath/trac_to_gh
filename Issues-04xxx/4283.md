# Issue 4283: [with proto-patch] A Speed-up Patch for NTL's ZZXFactoring.c

archive/issues_004283.json:
```json
{
    "body": "The goal of this patch is to speed-up NTL's factoring algorithm for polynomials in Z[X].  The speed-up comes from using fpLLL rather than NTL's native LLL algorithm.  We do this by converting a ZZ_mat of ZZ's (NTL's multi-precision integers) and passing them into a mat_ZZ<mpz_t> matrix of mpz_t's (fpLLL's native format).  Then run fpLLL on the new matrix and pass the entries back to NTL.  I don't replace NTL's LLL just pass what should be an already reduced basis to NTL's LLL.  (NTL computes extra information that would require a hack into fpLLL to get and might not be worth it.)  This patch allows NTL to beat MAGMA on many examples (it still is a little slower than MAGMA (but faster than SAGE) on irreducible polynomials).  I think that the cross over between Pari's factoring and NTL's factoring should be re-evaluated (currently Pari is used for polynomials of degree 30 through 300) if not just use NTL for all polynomials now.  \n\n**Assignee:** anovocin\n\n**Keywords:** NTL, LLL, Univariate\n\n**Status:** needs_work\n\nIssue created by migration from https://trac.sagemath.org/ticket/4283\n\n",
    "created_at": "2008-10-14T14:26:40Z",
    "labels": [
        "component: factorization",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.4",
    "title": "[with proto-patch] A Speed-up Patch for NTL's ZZXFactoring.c",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4283",
    "user": "https://trac.sagemath.org/admin/accounts/users/anovocin"
}
```
The goal of this patch is to speed-up NTL's factoring algorithm for polynomials in Z[X].  The speed-up comes from using fpLLL rather than NTL's native LLL algorithm.  We do this by converting a ZZ_mat of ZZ's (NTL's multi-precision integers) and passing them into a mat_ZZ<mpz_t> matrix of mpz_t's (fpLLL's native format).  Then run fpLLL on the new matrix and pass the entries back to NTL.  I don't replace NTL's LLL just pass what should be an already reduced basis to NTL's LLL.  (NTL computes extra information that would require a hack into fpLLL to get and might not be worth it.)  This patch allows NTL to beat MAGMA on many examples (it still is a little slower than MAGMA (but faster than SAGE) on irreducible polynomials).  I think that the cross over between Pari's factoring and NTL's factoring should be re-evaluated (currently Pari is used for polynomials of degree 30 through 300) if not just use NTL for all polynomials now.  

**Assignee:** anovocin

**Keywords:** NTL, LLL, Univariate

**Status:** needs_work

Issue created by migration from https://trac.sagemath.org/ticket/4283





---

archive/issue_comments_032085.json:
```json
{
    "body": "**Changing status** from new to assigned.",
    "created_at": "2008-10-14T14:27:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4283",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4283#issuecomment-32085",
    "user": "https://trac.sagemath.org/admin/accounts/users/anovocin"
}
```

**Changing status** from new to assigned.



---

archive/issue_comments_032086.json:
```json
{
    "body": "**Assignee:** anovocin",
    "created_at": "2008-10-14T14:27:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4283",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4283#issuecomment-32086",
    "user": "https://trac.sagemath.org/admin/accounts/users/anovocin"
}
```

**Assignee:** anovocin



---

archive/issue_events_012737.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4283",
    "rename": {
        "from": "[with proto-patch]A Speed-up Patch for NTL's ZZXFactoring.c",
        "to": "[with proto-patch] A Speed-up Patch for NTL's ZZXFactoring.c"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4283#event-12737"
}
```



---

archive/attachments_004574.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "ntlfactor.patch",
    "asset_url": "tarball://root/attachments/some-uuid/ticket4283/ntlfactor.patch",
    "created_at": "2008-10-14T15:21:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4283",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/anovocin"
}
```



---

archive/issue_comments_032087.json:
```json
{
    "body": "",
    "created_at": "2008-10-14T15:21:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4283",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4283#issuecomment-32087",
    "user": "https://trac.sagemath.org/admin/accounts/users/anovocin"
}
```





---

archive/issue_events_012738.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4283",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4283#event-12738"
}
```



---

archive/issue_events_012739.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4283",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4283#event-12739"
}
```



---

archive/issue_events_012740.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4283",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4283#event-12740"
}
```



---

archive/issue_events_012741.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4283",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4283#event-12741"
}
```



---

archive/issue_events_012742.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4283",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4283#event-12742"
}
```



---

archive/issue_events_012743.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4283",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4283#event-12743"
}
```



---

archive/issue_events_012744.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4283",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4283#event-12744"
}
```
