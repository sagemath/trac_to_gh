# Issue 4708: [with patch, with positive review] sage -tp doesn't test absolute file names nor does it ignore non-existent files

archive/issues_004708.json:
```json
{
    "body": "\n```\n-*- mode: sage-test; default-directory: \"~/sage-3.2.1.alpha1/devel/sage-nca/sage/modules/\" -*-\nsage-test started at Thu Dec  4 20:43:38\n\n/Users/ncalexan/bin/sage -b >/dev/null && /Users/ncalexan/bin/sage -tp 2 /Users/ncalexan/sage-3.2.1.alpha1/devel/sage-nca/sage/modules/free_module.py\n\nreal 0m1.413s\nuser 0m0.875s\nsys 0m0.426s\nGlobal iterations: 1\nFile iterations: 1\nTeX files: 0\n \n----------------------------------------------------------------------\nAll tests passed!\nTotal time for all tests: 0.0 seconds\n\nsage-test finished (all test passed) at Thu Dec  4 20:43:40\n```\n\nAlso:\n\n```\n-*- mode: sage-test; default-directory: \"~/sage-3.2.1.alpha1/devel/sage-nca/sage/modules/\" -*-\nsage-test started at Thu Dec  4 20:44:39\n\n/Users/ncalexan/bin/sage -b >/dev/null && /Users/ncalexan/bin/sage -t /Users/ncalexan/sage-3.2.1.alpha1/devel/sage-nca/sage/modules/free_module.py2\n\nreal 0m1.311s\nuser 0m0.872s\nsys 0m0.416s\nERROR: File .//Users/ncalexan/sage-3.2.1.alpha1/devel/sage-nca/sage/modules/free_module.py2 is missing\nexit code: 1\n \n----------------------------------------------------------------------\nThe following tests failed:\n\n\n.//Users/ncalexan/sage-3.2.1.alpha1/devel/sage-nca/sage/modules/free_module.py2\nTotal time for all tests: 0.0 seconds\n\nsage-test finished (all test passed) at Thu Dec  4 20:44:40\n```\n\nAssignee: @garyfurnish\n\nKeywords: ptest parallel test absolute filename\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4708\n\n",
    "closed_at": "2008-12-05T06:38:32Z",
    "created_at": "2008-12-05T04:47:56Z",
    "labels": [
        "component: doctest coverage",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.2.2",
    "title": "[with patch, with positive review] sage -tp doesn't test absolute file names nor does it ignore non-existent files",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4708",
    "user": "https://github.com/ncalexan"
}
```

```
-*- mode: sage-test; default-directory: "~/sage-3.2.1.alpha1/devel/sage-nca/sage/modules/" -*-
sage-test started at Thu Dec  4 20:43:38

/Users/ncalexan/bin/sage -b >/dev/null && /Users/ncalexan/bin/sage -tp 2 /Users/ncalexan/sage-3.2.1.alpha1/devel/sage-nca/sage/modules/free_module.py

real 0m1.413s
user 0m0.875s
sys 0m0.426s
Global iterations: 1
File iterations: 1
TeX files: 0
 
----------------------------------------------------------------------
All tests passed!
Total time for all tests: 0.0 seconds

sage-test finished (all test passed) at Thu Dec  4 20:43:40
```

Also:

```
-*- mode: sage-test; default-directory: "~/sage-3.2.1.alpha1/devel/sage-nca/sage/modules/" -*-
sage-test started at Thu Dec  4 20:44:39

/Users/ncalexan/bin/sage -b >/dev/null && /Users/ncalexan/bin/sage -t /Users/ncalexan/sage-3.2.1.alpha1/devel/sage-nca/sage/modules/free_module.py2

real 0m1.311s
user 0m0.872s
sys 0m0.416s
ERROR: File .//Users/ncalexan/sage-3.2.1.alpha1/devel/sage-nca/sage/modules/free_module.py2 is missing
exit code: 1
 
----------------------------------------------------------------------
The following tests failed:


.//Users/ncalexan/sage-3.2.1.alpha1/devel/sage-nca/sage/modules/free_module.py2
Total time for all tests: 0.0 seconds

sage-test finished (all test passed) at Thu Dec  4 20:44:40
```

Assignee: @garyfurnish

Keywords: ptest parallel test absolute filename

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4708





---

archive/issue_comments_037647.json:
```json
{
    "body": "Attachment [trac_4708_bin.patch](tarball://root/attachments/some-uuid/ticket4708/trac_4708_bin.patch) by @garyfurnish created at 2008-12-05 04:55:48",
    "created_at": "2008-12-05T04:55:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4708",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4708#issuecomment-37647",
    "user": "https://github.com/garyfurnish"
}
```

Attachment [trac_4708_bin.patch](tarball://root/attachments/some-uuid/ticket4708/trac_4708_bin.patch) by @garyfurnish created at 2008-12-05 04:55:48



---

archive/issue_comments_037648.json:
```json
{
    "body": "<a id='comment:1'></a>\nTrivial fix attached.",
    "created_at": "2008-12-05T04:56:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4708",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4708#issuecomment-37648",
    "user": "https://github.com/garyfurnish"
}
```

<a id='comment:1'></a>
Trivial fix attached.



---

archive/issue_comments_037649.json:
```json
{
    "body": "Changing assignee from mabshoff to @garyfurnish.",
    "created_at": "2008-12-05T04:56:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4708",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4708#issuecomment-37649",
    "user": "https://github.com/garyfurnish"
}
```

Changing assignee from mabshoff to @garyfurnish.



---

archive/issue_events_017977.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4708",
    "rename": {
        "from": "sage -tp doesn't test absolute file names nor does it ignore non-existent files",
        "to": "[with patch, needs review] sage -tp doesn't test absolute file names nor does it ignore non-existent files"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4708#event-17977"
}
```



---

archive/issue_comments_037650.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-12-05T04:56:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4708",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4708#issuecomment-37650",
    "user": "https://github.com/garyfurnish"
}
```

Changing status from new to assigned.



---

archive/issue_events_017978.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4708",
    "rename": {
        "from": "[with patch, needs review] sage -tp doesn't test absolute file names nor does it ignore non-existent files",
        "to": "[with patch, with positive review] sage -tp doesn't test absolute file names nor does it ignore non-existent files"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4708#event-17978"
}
```



---

archive/issue_comments_037651.json:
```json
{
    "body": "<a id='comment:2'></a>\nAttachment [4708_bin-2.patch](tarball://root/attachments/some-uuid/ticket4708/4708_bin-2.patch) by @ncalexan created at 2008-12-05 05:44:12\n\nApply only 4708_bin-2.patch, all credit to gfurnish.",
    "created_at": "2008-12-05T05:44:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4708",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4708#issuecomment-37651",
    "user": "https://github.com/ncalexan"
}
```

<a id='comment:2'></a>
Attachment [4708_bin-2.patch](tarball://root/attachments/some-uuid/ticket4708/4708_bin-2.patch) by @ncalexan created at 2008-12-05 05:44:12

Apply only 4708_bin-2.patch, all credit to gfurnish.



---

archive/issue_comments_037652.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged 4708_bin-2.patch in Sage 3.2.2.alpha0",
    "created_at": "2008-12-05T06:38:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4708",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4708#issuecomment-37652",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged 4708_bin-2.patch in Sage 3.2.2.alpha0



---

archive/issue_events_017979.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-12-05T06:38:32Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4708",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4708#event-17979"
}
```



---

archive/issue_comments_037653.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-12-05T06:38:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4708",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4708#issuecomment-37653",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_037654.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -5,9 +5,9 @@\n \n /Users/ncalexan/bin/sage -b >/dev/null && /Users/ncalexan/bin/sage -tp 2 /Users/ncalexan/sage-3.2.1.alpha1/devel/sage-nca/sage/modules/free_module.py\n \n-real\t0m1.413s\n-user\t0m0.875s\n-sys\t0m0.426s\n+real 0m1.413s\n+user 0m0.875s\n+sys 0m0.426s\n Global iterations: 1\n File iterations: 1\n TeX files: 0\n@@ -27,9 +27,9 @@\n \n /Users/ncalexan/bin/sage -b >/dev/null && /Users/ncalexan/bin/sage -t /Users/ncalexan/sage-3.2.1.alpha1/devel/sage-nca/sage/modules/free_module.py2\n \n-real\t0m1.311s\n-user\t0m0.872s\n-sys\t0m0.416s\n+real 0m1.311s\n+user 0m0.872s\n+sys 0m0.416s\n ERROR: File .//Users/ncalexan/sage-3.2.1.alpha1/devel/sage-nca/sage/modules/free_module.py2 is missing\n exit code: 1\n  \n@@ -37,7 +37,7 @@\n The following tests failed:\n \n \n-\t.//Users/ncalexan/sage-3.2.1.alpha1/devel/sage-nca/sage/modules/free_module.py2\n+.//Users/ncalexan/sage-3.2.1.alpha1/devel/sage-nca/sage/modules/free_module.py2\n Total time for all tests: 0.0 seconds\n \n sage-test finished (all test passed) at Thu Dec  4 20:44:40\n``````\n",
    "created_at": "2015-09-06T17:39:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4708",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4708#issuecomment-37654",
    "user": "https://github.com/fchapoton"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -5,9 +5,9 @@
 
 /Users/ncalexan/bin/sage -b >/dev/null && /Users/ncalexan/bin/sage -tp 2 /Users/ncalexan/sage-3.2.1.alpha1/devel/sage-nca/sage/modules/free_module.py
 
-real	0m1.413s
-user	0m0.875s
-sys	0m0.426s
+real 0m1.413s
+user 0m0.875s
+sys 0m0.426s
 Global iterations: 1
 File iterations: 1
 TeX files: 0
@@ -27,9 +27,9 @@
 
 /Users/ncalexan/bin/sage -b >/dev/null && /Users/ncalexan/bin/sage -t /Users/ncalexan/sage-3.2.1.alpha1/devel/sage-nca/sage/modules/free_module.py2
 
-real	0m1.311s
-user	0m0.872s
-sys	0m0.416s
+real 0m1.311s
+user 0m0.872s
+sys 0m0.416s
 ERROR: File .//Users/ncalexan/sage-3.2.1.alpha1/devel/sage-nca/sage/modules/free_module.py2 is missing
 exit code: 1
  
@@ -37,7 +37,7 @@
 The following tests failed:
 
 
-	.//Users/ncalexan/sage-3.2.1.alpha1/devel/sage-nca/sage/modules/free_module.py2
+.//Users/ncalexan/sage-3.2.1.alpha1/devel/sage-nca/sage/modules/free_module.py2
 Total time for all tests: 0.0 seconds
 
 sage-test finished (all test passed) at Thu Dec  4 20:44:40
``````




---

archive/issue_comments_037655.json:
```json
{
    "body": "Changing upstream from \"\" to \"N/A\"",
    "created_at": "2015-09-06T17:39:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4708",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4708#issuecomment-37655",
    "user": "https://github.com/fchapoton"
}
```

Changing upstream from "" to "N/A"
