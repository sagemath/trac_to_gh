# Issue 4458: tutorial and reference manual should explain NameError

archive/issues_004458.json:
```json
{
    "body": "Perhaps even a blurb about variables being references (if it's not to complicated). However, at the very least there should be something in the calculus section(s) to the effect of \"If you get a NameError, check to see if you either misspelled something or forgot to define a variable with var(...)\"\n\n\n**Assignee:** afleckenstein\n\n**CC:**  @kcrisman\n\n**Branch/Commit:** [5d5ac7f03dd61566a973c0de610b224e4406e3e5](https://github.com/sagemath/sagetrac-mirror/commit/5d5ac7f03dd61566a973c0de610b224e4406e3e5)\n\n**Reviewer:** Karl-Dieter Crisman, Nathann Cohen\n\n**Author:** Andrew Fleckenstein\n\nIssue created by migration from https://trac.sagemath.org/ticket/4458\n\n",
    "closed_at": "2015-01-02T15:46:32Z",
    "created_at": "2008-11-07T01:39:27Z",
    "labels": [
        "component: documentation",
        "bug",
        "good first issue"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "title": "tutorial and reference manual should explain NameError",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/4458",
    "user": "https://github.com/robertwb"
}
```
Perhaps even a blurb about variables being references (if it's not to complicated). However, at the very least there should be something in the calculus section(s) to the effect of "If you get a NameError, check to see if you either misspelled something or forgot to define a variable with var(...)"


**Assignee:** afleckenstein

**CC:**  @kcrisman

**Branch/Commit:** [5d5ac7f03dd61566a973c0de610b224e4406e3e5](https://github.com/sagemath/sagetrac-mirror/commit/5d5ac7f03dd61566a973c0de610b224e4406e3e5)

**Reviewer:** Karl-Dieter Crisman, Nathann Cohen

**Author:** Andrew Fleckenstein

Issue created by migration from https://trac.sagemath.org/ticket/4458





---

archive/issue_events_026111.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2010-05-26T15:17:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "label": "good first issue",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4458#event-26111"
}
```



---

archive/issue_comments_026937.json:
```json
{
    "body": "<a id='comment:3'></a>\nI think that the section [Some Common Issues With Functions](http://www.sagemath.org/doc/tutorial/tour_functions.html) in the tutorial explains it quite well. The patch I added specifies an error that is mentioned as a NameError, not just \"an error,\" and is also more specific about how to fix it.",
    "created_at": "2012-12-21T14:11:45Z",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4458#issuecomment-26937",
    "user": "https://trac.sagemath.org/admin/accounts/users/afleckenstein"
}
```

<a id='comment:3'></a>
I think that the section [Some Common Issues With Functions](http://www.sagemath.org/doc/tutorial/tour_functions.html) in the tutorial explains it quite well. The patch I added specifies an error that is mentioned as a NameError, not just "an error," and is also more specific about how to fix it.



---

archive/issue_events_026112.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/afleckenstein",
    "created_at": "2012-12-21T14:14:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4458#event-26112"
}
```



---

archive/issue_comments_026938.json:
```json
{
    "body": "<a id='comment:5'></a>\nHello! \n\nI have two comments about the patch: there is a grammatical error: \"an NameError\" and I think \"NameError\" should be codified -- by enclosing it with two backticks. \n\nSo, needs_work. :)",
    "created_at": "2013-01-31T03:30:46Z",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4458#issuecomment-26938",
    "user": "https://github.com/KPanComputes"
}
```

<a id='comment:5'></a>
Hello! 

I have two comments about the patch: there is a grammatical error: "an NameError" and I think "NameError" should be codified -- by enclosing it with two backticks. 

So, needs_work. :)



---

archive/issue_events_026113.json:
```json
{
    "actor": "https://github.com/KPanComputes",
    "created_at": "2013-01-31T03:31:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4458#event-26113"
}
```



---

archive/issue_events_026114.json:
```json
{
    "actor": "https://github.com/KPanComputes",
    "created_at": "2013-01-31T03:31:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4458#event-26114"
}
```



---

archive/attachments_004814.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_4458_name_error_explanation.patch",
    "asset_url": "tarball://root/attachments/ticket4458/trac_4458_name_error_explanation.patch",
    "created_at": "2013-02-01T14:01:20Z",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket4458/trac_4458_name_error_explanation.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/afleckenstein"
}
```



---

archive/issue_comments_026939.json:
```json
{
    "body": "<a id='comment:7'></a>\nAttachment [trac_4458_name_error_explanation.patch](https://github.com/sagemath/sage/files/ticket4458/trac_4458_name_error_explanation.patch) by afleckenstein created at 2013-02-01 14:01:20\n\nReplying to [knsam](#comment%3A5):\n> Hello! \n> \n> I have two comments about the patch: there is a grammatical error: \"an NameError\" and I think \"NameError\" should be codified -- by enclosing it with two backticks. \n> \n> So, needs_work. :) \n\n\nThanks for the input!",
    "created_at": "2013-02-01T14:01:20Z",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4458#issuecomment-26939",
    "user": "https://trac.sagemath.org/admin/accounts/users/afleckenstein"
}
```

<a id='comment:7'></a>
Attachment [trac_4458_name_error_explanation.patch](https://github.com/sagemath/sage/files/ticket4458/trac_4458_name_error_explanation.patch) by afleckenstein created at 2013-02-01 14:01:20

Replying to [knsam](#comment%3A5):
> Hello! 
> 
> I have two comments about the patch: there is a grammatical error: "an NameError" and I think "NameError" should be codified -- by enclosing it with two backticks. 
> 
> So, needs_work. :) 


Thanks for the input!



---

archive/issue_events_026115.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/afleckenstein",
    "created_at": "2013-02-01T14:01:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4458#event-26115"
}
```



---

archive/issue_events_026116.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/afleckenstein",
    "created_at": "2013-02-01T14:01:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4458#event-26116"
}
```



---

archive/issue_comments_026940.json:
```json
{
    "body": "**Assignee:** afleckenstein",
    "created_at": "2013-02-01T14:01:39Z",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4458#issuecomment-26940",
    "user": "https://trac.sagemath.org/admin/accounts/users/afleckenstein"
}
```

**Assignee:** afleckenstein



---

archive/issue_comments_026941.json:
```json
{
    "body": "<a id='comment:9'></a>\nHello! \n\nSorry for not being complete last time. I have one last comment, I am really sorry for doing this:\n\nThe description in the patch says: 'at the very least there should be something in the calculus section(s) to the effect of \"If you get a NameError?, check to see if you either misspelled something or forgot to define a variable with var(...)\"'. \n\nI think this could be done, by adding a note. Can you please do this, so that there is an explicit place where this is explained? I am also thinking if it might be possible to implement the first suggestion in the description. Hope you'd find an appropriate place to do so. \n\nRegards, Kannappan.",
    "created_at": "2013-02-01T16:00:48Z",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4458#issuecomment-26941",
    "user": "https://github.com/KPanComputes"
}
```

<a id='comment:9'></a>
Hello! 

Sorry for not being complete last time. I have one last comment, I am really sorry for doing this:

The description in the patch says: 'at the very least there should be something in the calculus section(s) to the effect of "If you get a NameError?, check to see if you either misspelled something or forgot to define a variable with var(...)"'. 

I think this could be done, by adding a note. Can you please do this, so that there is an explicit place where this is explained? I am also thinking if it might be possible to implement the first suggestion in the description. Hope you'd find an appropriate place to do so. 

Regards, Kannappan.



---

archive/issue_events_026117.json:
```json
{
    "actor": "https://github.com/KPanComputes",
    "created_at": "2013-02-02T15:51:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4458#event-26117"
}
```



---

archive/issue_events_026118.json:
```json
{
    "actor": "https://github.com/KPanComputes",
    "created_at": "2013-02-02T15:51:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "label": "needs info",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4458#event-26118"
}
```



---

archive/attachments_004815.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_4458_name_error_calculus.patch",
    "asset_url": "tarball://root/attachments/ticket4458/trac_4458_name_error_calculus.patch",
    "created_at": "2013-02-07T14:15:52Z",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket4458/trac_4458_name_error_calculus.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/afleckenstein"
}
```



---

archive/issue_comments_026942.json:
```json
{
    "body": "<a id='comment:11'></a>\nAttachment [trac_4458_name_error_calculus.patch](https://github.com/sagemath/sage/files/ticket4458/trac_4458_name_error_calculus.patch) by afleckenstein created at 2013-02-07 14:15:52\n\nReplying to [knsam](#comment%3A9):\n> The description in the patch says: 'at the very least there should be something in the calculus section(s) to the effect of \"If you get a NameError?, check to see if you either misspelled something or forgot to define a variable with var(...)\"'. \n> \n> I think this could be done, by adding a note. Can you please do this, so that there is an explicit place where this is explained? I am also thinking if it might be possible to implement the first suggestion in the description. Hope you'd find an appropriate place to do so. \n\n\nDone! I added it to a separate patch, so you can pick either one, or both :-)",
    "created_at": "2013-02-07T14:15:52Z",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4458#issuecomment-26942",
    "user": "https://trac.sagemath.org/admin/accounts/users/afleckenstein"
}
```

<a id='comment:11'></a>
Attachment [trac_4458_name_error_calculus.patch](https://github.com/sagemath/sage/files/ticket4458/trac_4458_name_error_calculus.patch) by afleckenstein created at 2013-02-07 14:15:52

Replying to [knsam](#comment%3A9):
> The description in the patch says: 'at the very least there should be something in the calculus section(s) to the effect of "If you get a NameError?, check to see if you either misspelled something or forgot to define a variable with var(...)"'. 
> 
> I think this could be done, by adding a note. Can you please do this, so that there is an explicit place where this is explained? I am also thinking if it might be possible to implement the first suggestion in the description. Hope you'd find an appropriate place to do so. 


Done! I added it to a separate patch, so you can pick either one, or both :-)



---

archive/issue_events_026119.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/afleckenstein",
    "created_at": "2013-02-07T14:15:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "label": "needs info",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4458#event-26119"
}
```



---

archive/issue_events_026120.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/afleckenstein",
    "created_at": "2013-02-07T14:15:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4458#event-26120"
}
```



---

archive/issue_events_026121.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-02-12T16:04:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4458#event-26121"
}
```



---

archive/issue_events_026122.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-02-12T16:04:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4458#event-26122"
}
```



---

archive/issue_comments_026943.json:
```json
{
    "body": "<a id='comment:12'></a>\nI feel like these are ... a little dangerous.\n\nFirst, \n\n```\nare defined to be ``var(...)``\n```\nIs there a noun missing here?  Or \"via\", or \"exist using\"?\n\n```\nsage: diff(sin(var('u')), var('u'))\n```\nI'm a little leery of suggesting people do this.  `var('u')` injects things into the global namespace as well, so this is redundant.  Better is\n\n```\nsage: var('u')\ndiff(...)\n```\n\nAlso, I suspect you'll need an extra line before the new section\n\n```\n.. _section-systems: \n```\nto avoid some documentation build error.",
    "created_at": "2013-02-12T16:04:42Z",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4458#issuecomment-26943",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:12'></a>
I feel like these are ... a little dangerous.

First, 

```
are defined to be ``var(...)``
```
Is there a noun missing here?  Or "via", or "exist using"?

```
sage: diff(sin(var('u')), var('u'))
```
I'm a little leery of suggesting people do this.  `var('u')` injects things into the global namespace as well, so this is redundant.  Better is

```
sage: var('u')
diff(...)
```

Also, I suspect you'll need an extra line before the new section

```
.. _section-systems: 
```
to avoid some documentation build error.



---

archive/issue_comments_026944.json:
```json
{
    "body": "<a id='comment:13'></a>\nReplying to [kcrisman](#comment%3A12):\n\n>`var('u')` injects things into the global namespace as well, so this is redundant.\n\n\nI didn't know this happened! That's pretty neat. Why do we have\n\n```\nu = var('u')\n```\nin the tutorial then? Is \n\n```\nvar('u')\n```\nshorthand for u = var('u'), or does it do something a little different?\n\nAndrew",
    "created_at": "2013-02-13T14:09:27Z",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4458#issuecomment-26944",
    "user": "https://trac.sagemath.org/admin/accounts/users/afleckenstein"
}
```

<a id='comment:13'></a>
Replying to [kcrisman](#comment%3A12):

>`var('u')` injects things into the global namespace as well, so this is redundant.


I didn't know this happened! That's pretty neat. Why do we have

```
u = var('u')
```
in the tutorial then? Is 

```
var('u')
```
shorthand for u = var('u'), or does it do something a little different?

Andrew



---

archive/issue_comments_026945.json:
```json
{
    "body": "<a id='comment:14'></a>\nCompare\n\n```\nsage: u = var('u')\nsage:\n```\nwith \n\n```\nsage: var('u')\nu\nsage:\n```\nThat's my reason, anyway.  There may be some very subtle other difference as well.",
    "created_at": "2013-02-13T15:49:14Z",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4458#issuecomment-26945",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:14'></a>
Compare

```
sage: u = var('u')
sage:
```
with 

```
sage: var('u')
u
sage:
```
That's my reason, anyway.  There may be some very subtle other difference as well.



---

archive/issue_comments_026946.json:
```json
{
    "body": "<a id='comment:15'></a>\nYou can just get this behavior:\n\n```\nsage: v = var('u')\nsage: u\nu\nsage: v\nu\n```",
    "created_at": "2013-03-29T22:53:49Z",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4458#issuecomment-26946",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:15'></a>
You can just get this behavior:

```
sage: v = var('u')
sage: u
u
sage: v
u
```



---

archive/issue_events_026123.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4458#event-26123"
}
```



---

archive/issue_events_026124.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4458#event-26124"
}
```



---

archive/issue_events_026125.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4458#event-26125"
}
```



---

archive/issue_events_026126.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4458#event-26126"
}
```



---

archive/issue_events_026127.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4458#event-26127"
}
```



---

archive/issue_events_026128.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4458#event-26128"
}
```



---

archive/issue_events_026129.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4458#event-26129"
}
```



---

archive/issue_events_026130.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4458#event-26130"
}
```



---

archive/issue_comments_026947.json:
```json
{
    "body": "**Reviewer:** Karl-Dieter Crisman",
    "created_at": "2014-11-20T18:16:16Z",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4458#issuecomment-26947",
    "user": "https://github.com/kcrisman"
}
```

**Reviewer:** Karl-Dieter Crisman



---

archive/issue_comments_026948.json:
```json
{
    "body": "<a id='comment:20'></a>\nI'm happy, having done the branch, but I'd appreciate a separate reviewer.\n\n---\n**New commits:**\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5d5ac7f03dd61566a973c0de610b224e4406e3e5\">5d5ac7f</a></td><td><code>Trac 4485 - explain NameError in tutorial</code></td></tr></table>\n",
    "created_at": "2014-11-20T18:16:16Z",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4458#issuecomment-26948",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:20'></a>
I'm happy, having done the branch, but I'd appreciate a separate reviewer.

---
**New commits:**
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5d5ac7f03dd61566a973c0de610b224e4406e3e5">5d5ac7f</a></td><td><code>Trac 4485 - explain NameError in tutorial</code></td></tr></table>




---

archive/issue_comments_026949.json:
```json
{
    "body": "**Author:** Andrew Fleckenstein",
    "created_at": "2014-11-20T18:16:16Z",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4458#issuecomment-26949",
    "user": "https://github.com/kcrisman"
}
```

**Author:** Andrew Fleckenstein



---

archive/issue_comments_026950.json:
```json
{
    "body": "**Branch:** [u/kcrisman/ticket/4458](https://github.com/sagemath/sagetrac-mirror/tree/u/kcrisman/ticket/4458)",
    "created_at": "2014-11-20T18:16:16Z",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4458#issuecomment-26950",
    "user": "https://github.com/kcrisman"
}
```

**Branch:** [u/kcrisman/ticket/4458](https://github.com/sagemath/sagetrac-mirror/tree/u/kcrisman/ticket/4458)



---

archive/issue_comments_026951.json:
```json
{
    "body": "**Commit:** [5d5ac7f03dd61566a973c0de610b224e4406e3e5](https://github.com/sagemath/sagetrac-mirror/commit/5d5ac7f03dd61566a973c0de610b224e4406e3e5)",
    "created_at": "2014-11-20T18:16:16Z",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4458#issuecomment-26951",
    "user": "https://github.com/kcrisman"
}
```

**Commit:** [5d5ac7f03dd61566a973c0de610b224e4406e3e5](https://github.com/sagemath/sagetrac-mirror/commit/5d5ac7f03dd61566a973c0de610b224e4406e3e5)



---

archive/issue_events_026131.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-11-20T18:16:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "label": "needs work",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4458#event-26131"
}
```



---

archive/issue_events_026132.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-11-20T18:16:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4458#event-26132"
}
```



---

archive/issue_comments_026952.json:
```json
{
    "body": "<a id='comment:21'></a>\nWorks for me !\n\nNathann",
    "created_at": "2014-12-31T10:50:03Z",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4458#issuecomment-26952",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:21'></a>
Works for me !

Nathann



---

archive/issue_events_026133.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-12-31T10:50:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4458#event-26133"
}
```



---

archive/issue_events_026134.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-12-31T10:50:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4458#event-26134"
}
```



---

archive/issue_comments_026953.json:
```json
{
    "body": "**Changing reviewer** from \"Karl-Dieter Crisman\" to \"Karl-Dieter Crisman, Nathann Cohen\".",
    "created_at": "2014-12-31T10:50:03Z",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4458#issuecomment-26953",
    "user": "https://github.com/nathanncohen"
}
```

**Changing reviewer** from "Karl-Dieter Crisman" to "Karl-Dieter Crisman, Nathann Cohen".



---

archive/issue_events_026135.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-01-02T15:46:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4458#event-26135"
}
```



---

archive/issue_events_026136.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-01-02T15:46:32Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4458#event-26136"
}
```



---

archive/issue_comments_026954.json:
```json
{
    "body": "**Changing branch** from \"[u/kcrisman/ticket/4458](https://github.com/sagemath/sagetrac-mirror/tree/u/kcrisman/ticket/4458)\" to \"[5d5ac7f03dd61566a973c0de610b224e4406e3e5](https://github.com/sagemath/sagetrac-mirror/commit/5d5ac7f03dd61566a973c0de610b224e4406e3e5)\".",
    "created_at": "2015-01-02T15:46:32Z",
    "issue": "https://github.com/sagemath/sage/issues/4458",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4458#issuecomment-26954",
    "user": "https://github.com/vbraun"
}
```

**Changing branch** from "[u/kcrisman/ticket/4458](https://github.com/sagemath/sagetrac-mirror/tree/u/kcrisman/ticket/4458)" to "[5d5ac7f03dd61566a973c0de610b224e4406e3e5](https://github.com/sagemath/sagetrac-mirror/commit/5d5ac7f03dd61566a973c0de610b224e4406e3e5)".
