# Issue 4679: Spelling errors in documentation for sage.rings.polynomial.polynomial_element

archive/issues_004679.json:
```json
{
    "assignees": [],
    "body": "There are a number of small spelling errors in the docstrings for the class sage.rings.polynomial.polynomial_element. \n\nA patch will be forthcoming.\n\nKeywords: **polynomial, documentation**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4679_\n\n",
    "closed_at": "2008-12-04T13:36:25Z",
    "created_at": "2008-12-02T18:17:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.2.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Spelling errors in documentation for sage.rings.polynomial.polynomial_element",
    "type": "issue",
    "updated_at": "2008-12-04T13:36:25Z",
    "url": "https://github.com/sagemath/sage/issues/4679",
    "user": "https://github.com/tjl"
}
```
There are a number of small spelling errors in the docstrings for the class sage.rings.polynomial.polynomial_element. 

A patch will be forthcoming.

Keywords: **polynomial, documentation**

_Issue created by migration from https://trac.sagemath.org/ticket/4679_





---

archive/issue_events_046813.json:
```json
{
    "actor": "https://github.com/tjl",
    "created_at": "2008-12-02T18:17:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "milestone_number": null,
    "milestone_title": "sage-3.2.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4679#event-46813"
}
```



---

archive/issue_events_046814.json:
```json
{
    "actor": "https://github.com/tjl",
    "created_at": "2008-12-02T18:17:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4679#event-46814"
}
```



---

archive/issue_events_046815.json:
```json
{
    "actor": "https://github.com/tjl",
    "created_at": "2008-12-02T18:17:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4679#event-46815"
}
```



---

archive/issue_events_046816.json:
```json
{
    "actor": "https://github.com/tjl",
    "created_at": "2008-12-02T18:17:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4679#event-46816"
}
```



---

archive/issue_comments_029117.json:
```json
{
    "body": "Spelling errors patch for Trac 4679.",
    "created_at": "2008-12-02T18:19:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4679#issuecomment-29117",
    "user": "https://github.com/tjl"
}
```

Spelling errors patch for Trac 4679.



---

archive/issue_comments_029118.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[trac_4679.patch.gz](https://github.com/sagemath/sage/files/ticket4679/trac_4679.patch.gz)**\n\nAdded patch to fix spelling errors.",
    "created_at": "2008-12-02T18:20:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4679#issuecomment-29118",
    "user": "https://github.com/tjl"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[trac_4679.patch.gz](https://github.com/sagemath/sage/files/ticket4679/trac_4679.patch.gz)**

Added patch to fix spelling errors.



---

archive/issue_events_046817.json:
```json
{
    "actor": "https://github.com/tjl",
    "created_at": "2008-12-02T18:20:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4679#event-46817"
}
```



---

archive/issue_events_046818.json:
```json
{
    "actor": "https://github.com/sagetrac-TimothyClemans",
    "created_at": "2008-12-03T00:35:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4679#event-46818"
}
```



---

archive/issue_events_046819.json:
```json
{
    "actor": "https://github.com/sagetrac-TimothyClemans",
    "created_at": "2008-12-03T00:35:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4679#event-46819"
}
```



---

archive/issue_comments_029119.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nTim,\n\nplease make sure to assign a milestone when opening a ticket. This is usually the current merge milestone, i.e. 3.2.2 at the moment.\n\nCheers,\n\nMichael",
    "created_at": "2008-12-03T00:41:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4679#issuecomment-29119",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:3'>Comment 3:</a>
Tim,

please make sure to assign a milestone when opening a ticket. This is usually the current merge milestone, i.e. 3.2.2 at the moment.

Cheers,

Michael



---

archive/issue_events_046820.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-03T00:41:57Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "milestone_number": null,
    "milestone_title": "sage-3.2.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4679#event-46820"
}
```



---

archive/issue_comments_029120.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nThanks. I wasn't sure what milestone to assign.",
    "created_at": "2008-12-03T00:52:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4679#issuecomment-29120",
    "user": "https://github.com/tjl"
}
```

<a id='comment:4'>Comment 4:</a>
Thanks. I wasn't sure what milestone to assign.



---

archive/issue_comments_029121.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nReplying to [@tjl](#comment%3A4):\n> Thanks. I wasn't sure what milestone to assign.\n\nNo problem. The new trac release 0.11.2 we will be upgrading to in about a week let's the admin assign default milestones from a panel, so in the future this will become a non-issue.\n\nCheers,\n\nMichael",
    "created_at": "2008-12-03T00:53:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4679#issuecomment-29121",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:5'>Comment 5:</a>
Replying to [@tjl](#comment%3A4):
> Thanks. I wasn't sure what milestone to assign.

No problem. The new trac release 0.11.2 we will be upgrading to in about a week let's the admin assign default milestones from a panel, so in the future this will become a non-issue.

Cheers,

Michael



---

archive/issue_events_046821.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-04T13:36:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4679#event-46821"
}
```



---

archive/issue_events_046822.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-04T13:36:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4679#event-46822"
}
```



---

archive/issue_comments_029122.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nMerged in Sage 3.2.2.alpha0",
    "created_at": "2008-12-04T13:36:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4679",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4679#issuecomment-29122",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:6'>Comment 6:</a>
Merged in Sage 3.2.2.alpha0
