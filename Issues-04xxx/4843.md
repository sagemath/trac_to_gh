# Issue 4843: Fix readline related build problems on OpenSUSE 11.1

archive/issues_004843.json:
```json
{
    "assignees": [],
    "body": "There are some small issues with Sage's readline and the one from OpenSUSE not playing well together. I have some temporary fix to get around this.\n\nCheers,\n\nMichael\n\n**Assignee:** mabshoff\n\nIssue created by migration from https://trac.sagemath.org/ticket/4843\n\n",
    "closed_at": "2009-01-06T06:39:29Z",
    "created_at": "2008-12-20T22:57:16Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.2.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix readline related build problems on OpenSUSE 11.1",
    "type": "issue",
    "updated_at": "2009-01-06T06:39:29Z",
    "url": "https://github.com/sagemath/sage/issues/4843",
    "user": "https://github.com/sagetrac-mabshoff"
}
```
There are some small issues with Sage's readline and the one from OpenSUSE not playing well together. I have some temporary fix to get around this.

Cheers,

Michael

**Assignee:** mabshoff

Issue created by migration from https://trac.sagemath.org/ticket/4843





---

archive/issue_events_045861.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-20T22:57:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4843",
    "milestone_number": null,
    "milestone_title": "sage-3.2.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4843#event-45861"
}
```



---

archive/issue_events_045862.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-20T22:57:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4843",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "08517b",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4843#event-45862"
}
```



---

archive/issue_events_045863.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-20T22:57:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4843",
    "label": "https://github.com/sagemath/sage/labels/critical",
    "label_color": "ff0000",
    "label_name": "critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4843#event-45863"
}
```



---

archive/issue_events_045864.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-20T22:57:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4843",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "008080",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4843#event-45864"
}
```



---

archive/issue_events_045865.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-02T23:17:30Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4843",
    "milestone_number": null,
    "milestone_title": "sage-3.2.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4843#event-45865"
}
```



---

archive/issue_events_045866.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-02T23:17:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4843",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4843#event-45866"
}
```



---

archive/issue_comments_030378.json:
```json
{
    "body": "<a id='comment:1'></a>\nUnfortunately this will be bumped to Sage 3.3.\n\nCheers,\n\nMichael",
    "created_at": "2009-01-02T23:17:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4843#issuecomment-30378",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:1'></a>
Unfortunately this will be bumped to Sage 3.3.

Cheers,

Michael



---

archive/issue_events_045867.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-06T04:38:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4843",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4843#event-45867"
}
```



---

archive/issue_events_045868.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-06T04:38:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4843",
    "milestone_number": null,
    "milestone_title": "sage-3.2.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4843#event-45868"
}
```



---

archive/issue_events_045869.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-06T04:38:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4843",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "008080",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4843#event-45869"
}
```



---

archive/issue_events_045870.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-06T04:38:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4843",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "008080",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4843#event-45870"
}
```



---

archive/issue_comments_030379.json:
```json
{
    "body": "<a id='comment:2'></a>\nThe spkg at\n\nhttp://sage.math.washington.edu/home/mabshoff/spkgs/readline-5.2.p5.spkg\n\nfixes the issue. It also cleans up SPKG.txt and brings it up to code.\n\nCheers,\n\nMichael",
    "created_at": "2009-01-06T04:38:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4843#issuecomment-30379",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:2'></a>
The spkg at

http://sage.math.washington.edu/home/mabshoff/spkgs/readline-5.2.p5.spkg

fixes the issue. It also cleans up SPKG.txt and brings it up to code.

Cheers,

Michael



---

archive/issue_events_045871.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2009-01-06T06:35:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4843",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "008080",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4843#event-45871"
}
```



---

archive/issue_events_045872.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-06T06:39:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4843",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "008080",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4843#event-45872"
}
```



---

archive/issue_events_045873.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-06T06:39:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4843",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4843#event-45873"
}
```



---

archive/issue_comments_030380.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.2.3\n\nCheers,\n\nMichael",
    "created_at": "2009-01-06T06:39:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4843#issuecomment-30380",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.2.3

Cheers,

Michael
