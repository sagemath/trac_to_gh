...
```json
{
    "body": "<a id='comment:2'></a>Code looks good. Only thing needed are some doctests on `__getitem__` in `mat.pyx` -- it would be nice to see a few doctests there, especially ones written to test weird corner cases.\n\n(Also edited a typo in the description for this ticket.)",
...
<a id='comment:2'></a>Code looks good. Only thing needed are some doctests on `__getitem__` in `mat.pyx` -- it would be nice to see a few doctests there, especially ones written to test weird corner cases.
...
```json
{
    "body": "<a id='comment:4'></a>I added a docstring for the getitem function with doctest, and also fixed one other doctest.  Now the coverage is 100% (though there is no loads(dumps) test).\n\nShould work on 4.0.",
...
<a id='comment:4'></a>I added a docstring for the getitem function with doctest, and also fixed one other doctest.  Now the coverage is 100% (though there is no loads(dumps) test).
