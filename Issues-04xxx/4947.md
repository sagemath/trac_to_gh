...
<a id='comment:1'></a>> Note: this is NOT a duplicate of #4363.
...
```json
{
    "body": "<a id='comment:2'></a>I just checked on the public sagenb.org, and *did* observe this behavior.  So it does happen systematically on some installs.  This is obviously a terrible bug.",
...
<a id='comment:2'></a>I just checked on the public sagenb.org, and *did* observe this behavior.  So it does happen systematically on some installs.  This is obviously a terrible bug.
...
```json
{
    "body": "<a id='comment:3'></a>This happened to me, but I cannot replicate it on 3.2.1 - only 3.2.2.  I hope that helps track it down.",
...
<a id='comment:3'></a>This happened to me, but I cannot replicate it on 3.2.1 - only 3.2.2.  I hope that helps track it down.
...
<a id='comment:5'></a>After a non-expert look at all changesets for 3.2.2, the only one that really stuck out was #3950 - not that there was anything immediately obvious in it that would cause this behavior, but it seemed to be the only nontrivial change to the notebook.  There were a few others that touched it, though all (including this one) seemed to be mostly on the listing of worksheets or html, not starting processes!
...
```json
{
    "body": "<a id='comment:6'></a>Is it possible that #3950 could have opened each worksheet, and when doing that, #4363 kicks in and starts running things?\n\nTimothyClemans, can you look at this and see if #3950 caused the problem?",
...
<a id='comment:6'></a>Is it possible that #3950 could have opened each worksheet, and when doing that, #4363 kicks in and starts running things?
...
```json
{
    "body": "<a id='comment:7'></a>I can verify this behavior on 3.2.3 (ubuntu 8.10 x86_64) and wanted to also verify that the worksheets are indeed running and have `@`interact cells",
...
<a id='comment:7'></a>I can verify this behavior on 3.2.3 (ubuntu 8.10 x86_64) and wanted to also verify that the worksheets are indeed running and have `@`interact cells
...
```json
{
    "body": "<a id='comment:8'></a>This is fixed by #4363.",
...
<a id='comment:8'></a>This is fixed by #4363.
...
```json
{
    "body": "<a id='comment:9'></a>Fixed in Sage 3.3.alpha0 by merging #4363.\n\nCheers,\n\nMichael",
...
<a id='comment:9'></a>Fixed in Sage 3.3.alpha0 by merging #4363.
