# Issue 4912: convert sage.geometry.* docstrings to Sphinx (open issues at #5375)

archive/issues_004912.json:
```json
{
    "assignees": [],
    "body": "\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4912_\n\n",
    "closed_at": "2009-02-24T18:52:49Z",
    "created_at": "2009-01-01T22:51:30Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "convert sage.geometry.* docstrings to Sphinx (open issues at #5375)",
    "type": "issue",
    "updated_at": "2009-02-26T16:17:53Z",
    "url": "https://github.com/sagemath/sage/issues/4912",
    "user": "https://github.com/mwhansen"
}
```


_Issue created by migration from https://trac.sagemath.org/ticket/4912_





---

archive/issue_events_049621.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-01T22:51:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4912",
    "milestone_number": null,
    "milestone_title": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4912#event-49621"
}
```



---

archive/issue_events_049622.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-01T22:51:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4912",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4912#event-49622"
}
```



---

archive/issue_events_049623.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-01T22:51:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4912",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4912#event-49623"
}
```



---

archive/issue_events_049624.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-01T22:51:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4912",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4912#event-49624"
}
```



---

archive/issue_comments_030850.json:
```json
{
    "body": "Attachment: **[trac_4912.patch.gz](https://github.com/sagemath/sage/files/ticket4912/trac_4912.patch.gz)**",
    "created_at": "2009-01-02T02:35:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4912#issuecomment-30850",
    "user": "https://github.com/mwhansen"
}
```

Attachment: **[trac_4912.patch.gz](https://github.com/sagemath/sage/files/ticket4912/trac_4912.patch.gz)**



---

archive/issue_events_049625.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-01-02T02:35:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4912",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4912#event-49625"
}
```



---

archive/issue_comments_030851.json:
```json
{
    "body": "Attachment: **[sage.geometry-final.patch.gz](https://github.com/sagemath/sage/files/ticket4912/sage.geometry-final.patch.gz)**",
    "created_at": "2009-02-21T19:18:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4912#issuecomment-30851",
    "user": "https://github.com/mwhansen"
}
```

Attachment: **[sage.geometry-final.patch.gz](https://github.com/sagemath/sage/files/ticket4912/sage.geometry-final.patch.gz)**



---

archive/issue_comments_030852.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nlattice_polytope.py, line 2677: \"codeLatticePolytope\" should be something like <code>\\`\\`LatticePolytope\\`\\`</code> (not your fault: the original said code{...} instead of \\code{...})\n\nsame three other places: lines 2715, 2769, 2801\n\nline 3182: change `mn` to something like <code>\\`m>n\\`</code>\n\nOtherwise, it looks good to me.",
    "created_at": "2009-02-22T00:09:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4912#issuecomment-30852",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:2'>Comment 2:</a>
lattice_polytope.py, line 2677: "codeLatticePolytope" should be something like <code>\`\`LatticePolytope\`\`</code> (not your fault: the original said code{...} instead of \code{...})

same three other places: lines 2715, 2769, 2801

line 3182: change `mn` to something like <code>\`m>n\`</code>

Otherwise, it looks good to me.



---

archive/issue_events_049626.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-02-22T00:09:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4912",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4912#event-49626"
}
```



---

archive/issue_events_049627.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2009-02-22T00:09:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4912",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4912#event-49627"
}
```



---

archive/issue_events_049628.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-24T18:52:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4912",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4912#event-49628"
}
```



---

archive/issue_events_049629.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-24T18:52:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4912",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4912#event-49629"
}
```



---

archive/issue_comments_030853.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nMerged in Sage 3.4.alpha0.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-24T18:52:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4912#issuecomment-30853",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:3'>Comment 3:</a>
Merged in Sage 3.4.alpha0.

Cheers,

Michael



---

archive/issue_comments_030854.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nSee #5375.",
    "created_at": "2009-02-26T15:23:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4912#issuecomment-30854",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:4'>Comment 4:</a>
See #5375.



---

archive/issue_comments_030855.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nThanks John.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-26T16:17:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4912",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4912#issuecomment-30855",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:5'>Comment 5:</a>
Thanks John.

Cheers,

Michael



---

archive/issue_events_049630.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-26T16:17:53Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4912",
    "title_is": "convert sage.geometry.* docstrings to Sphinx (open issues at #5375)",
    "title_was": "convert sage.geometry.* docstrings to Sphinx",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4912#event-49630"
}
```



---

archive/issue_events_049631.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-26T16:17:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4912",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4912#event-49631"
}
```
