# Issue 4224: small error in new question-mark interval printing

archive/issues_004224.json:
```json
{
    "body": "Some intervals were printing incorrectly in question-mark style (the printed result didn't always include the lower bound of the interval), as reported here: http://groups.google.com/group/sage-support/browse_thread/thread/844cde94499c42a1#\n\n(Thanks to Pong for reporting the problem, and Jason Grout for bringing it to my attention!)\n\nIt turns out that this is a single-character fix: I had RNDN (round-to-nearest) where I needed RNDD (round-down).  Unfortunately, lots of doctests recorded incorrect printing, so the actual patch is almost entirely doctest fixes.\n\nThis patch against 3.1.2 passes -testall.  I hand-checked several (but not all) of the doctest changes against .str(style='brackets').\n\n**Assignee:** cwitty\n\n**CC:**  @jasongrout\n\nIssue created by migration from https://trac.sagemath.org/ticket/4224\n\n",
    "closed_at": "2008-10-01T11:30:39Z",
    "created_at": "2008-09-30T19:09:41Z",
    "labels": [
        "component: basic arithmetic",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1.3",
    "title": "small error in new question-mark interval printing",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/4224",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```
Some intervals were printing incorrectly in question-mark style (the printed result didn't always include the lower bound of the interval), as reported here: http://groups.google.com/group/sage-support/browse_thread/thread/844cde94499c42a1#

(Thanks to Pong for reporting the problem, and Jason Grout for bringing it to my attention!)

It turns out that this is a single-character fix: I had RNDN (round-to-nearest) where I needed RNDD (round-down).  Unfortunately, lots of doctests recorded incorrect printing, so the actual patch is almost entirely doctest fixes.

This patch against 3.1.2 passes -testall.  I hand-checked several (but not all) of the doctest changes against .str(style='brackets').

**Assignee:** cwitty

**CC:**  @jasongrout

Issue created by migration from https://trac.sagemath.org/ticket/4224





---

archive/attachments_004476.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac4224-question-printing-lbound.patch",
    "asset_url": "tarball://root/attachments/ticket4224/trac4224-question-printing-lbound.patch",
    "created_at": "2008-09-30T19:10:42Z",
    "issue": "https://github.com/sagemath/sage/issues/4224",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket4224/trac4224-question-printing-lbound.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```



---

archive/issue_comments_025039.json:
```json
{
    "body": "Attachment [trac4224-question-printing-lbound.patch](https://github.com/sagemath/sage/files/ticket4224/trac4224-question-printing-lbound.patch) by cwitty created at 2008-09-30 19:10:42",
    "created_at": "2008-09-30T19:10:42Z",
    "issue": "https://github.com/sagemath/sage/issues/4224",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4224#issuecomment-25039",
    "user": "https://trac.sagemath.org/admin/accounts/users/cwitty"
}
```

Attachment [trac4224-question-printing-lbound.patch](https://github.com/sagemath/sage/files/ticket4224/trac4224-question-printing-lbound.patch) by cwitty created at 2008-09-30 19:10:42



---

archive/issue_events_024344.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/cwitty",
    "created_at": "2008-09-30T19:11:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4224",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4224#event-24344"
}
```



---

archive/issue_comments_025040.json:
```json
{
    "body": "<a id='comment:2'></a>\nCarl, \n\nI observed the following failure on a Core2 Quad running Solaris:\n\n```\nsage -t  devel/sage/sage/rings/qqbar.py                     \n**********************************************************************\nFile \"/home/mabshoff/build-3.1.3.alpha0/sage-3.1.3.alpha0-fulvia/tmp/qqbar.py\", line 3770:\n    sage: cp.complex_roots(30, 1)\nExpected:\n    [1.189207115002721?,\n     -1.189207115002721?,\n     1.189207115002721?*I,\n     -1.189207115002721?*I]\nGot:\n    [1.189207115002721?, -1.1892071150027211?, 1.189207115002721?*I, -1.189207115002721?*I]\n**********************************************************************\n```\nCould this be related to this issue? I am reviewing the patch and will see what is going on on Solaris once I build 3.1.3 on it.\n\nCheers,\n\nMichael",
    "created_at": "2008-10-01T10:38:40Z",
    "issue": "https://github.com/sagemath/sage/issues/4224",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4224#issuecomment-25040",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Carl, 

I observed the following failure on a Core2 Quad running Solaris:

```
sage -t  devel/sage/sage/rings/qqbar.py                     
**********************************************************************
File "/home/mabshoff/build-3.1.3.alpha0/sage-3.1.3.alpha0-fulvia/tmp/qqbar.py", line 3770:
    sage: cp.complex_roots(30, 1)
Expected:
    [1.189207115002721?,
     -1.189207115002721?,
     1.189207115002721?*I,
     -1.189207115002721?*I]
Got:
    [1.189207115002721?, -1.1892071150027211?, 1.189207115002721?*I, -1.189207115002721?*I]
**********************************************************************
```
Could this be related to this issue? I am reviewing the patch and will see what is going on on Solaris once I build 3.1.3 on it.

Cheers,

Michael



---

archive/issue_events_024345.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-10-01T11:30:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4224",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4224#event-24345"
}
```



---

archive/issue_events_024346.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-10-01T11:30:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4224",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4224#event-24346"
}
```



---

archive/issue_comments_025041.json:
```json
{
    "body": "<a id='comment:3'></a>\nPatch is good and passes doctests. Positive review.\n\nCheers,\n\nMichael",
    "created_at": "2008-10-01T11:30:26Z",
    "issue": "https://github.com/sagemath/sage/issues/4224",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4224#issuecomment-25041",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Patch is good and passes doctests. Positive review.

Cheers,

Michael



---

archive/issue_events_024347.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-10-01T11:30:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4224",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4224#event-24347"
}
```



---

archive/issue_events_024348.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-10-01T11:30:39Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4224",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4224#event-24348"
}
```



---

archive/issue_comments_025042.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.1.3.alpha3",
    "created_at": "2008-10-01T11:30:39Z",
    "issue": "https://github.com/sagemath/sage/issues/4224",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4224#issuecomment-25042",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.1.3.alpha3



---

archive/issue_comments_025043.json:
```json
{
    "body": "<a id='comment:5'></a>\nI also looked at this patch, for the record, and it looked reasonable.  I did not have time to apply it and check it, though.",
    "created_at": "2008-10-01T16:40:09Z",
    "issue": "https://github.com/sagemath/sage/issues/4224",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4224#issuecomment-25043",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:5'></a>
I also looked at this patch, for the record, and it looked reasonable.  I did not have time to apply it and check it, though.
