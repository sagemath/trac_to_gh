# Issue 4613: doctests for big-o

archive/issues_004613.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "Also now takes polynomial powers.\n\nComponent: **doctest coverage**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4613_\n\n",
    "closed_at": "2008-11-27T03:13:27Z",
    "created_at": "2008-11-25T07:33:52Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.2.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "doctests for big-o",
    "type": "issue",
    "updated_at": "2008-11-27T03:13:27Z",
    "url": "https://github.com/sagemath/sage/issues/4613",
    "user": "https://github.com/robertwb"
}
```
Also now takes polynomial powers.

Component: **doctest coverage**

_Issue created by migration from https://trac.sagemath.org/ticket/4613_





---

archive/issue_events_052694.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2008-11-25T07:33:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4613",
    "milestone_number": null,
    "milestone_title": "sage-3.2.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4613#event-52694"
}
```



---

archive/issue_events_052695.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2008-11-25T07:33:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4613",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4613#event-52695"
}
```



---

archive/issue_events_052696.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2008-11-25T07:33:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4613",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4613#event-52696"
}
```



---

archive/issue_events_052697.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2008-11-25T07:33:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4613",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4613#event-52697"
}
```



---

archive/issue_events_052698.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-25T07:33:52Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4613",
    "subject": "https://github.com/robertwb",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4613#event-52698"
}
```



---

archive/issue_events_052699.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2008-11-25T07:33:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4613",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4613#event-52699"
}
```



---

archive/issue_events_052700.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2008-11-27T00:36:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4613",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4613#event-52700"
}
```



---

archive/issue_comments_027497.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[4613-bigO-doctests.patch.gz](https://github.com/sagemath/sage/files/ticket4613/4613-bigO-doctests.patch.gz)**\n\nLooks good. I particularly like the ability to do \"O(x^10)\" and implicitly create the power series ring -- it feels really natural.",
    "created_at": "2008-11-27T00:36:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4613",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4613#issuecomment-27497",
    "user": "https://github.com/craigcitro"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[4613-bigO-doctests.patch.gz](https://github.com/sagemath/sage/files/ticket4613/4613-bigO-doctests.patch.gz)**

Looks good. I particularly like the ability to do "O(x^10)" and implicitly create the power series ring -- it feels really natural.



---

archive/issue_events_052701.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-27T03:13:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4613",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4613#event-52701"
}
```



---

archive/issue_events_052702.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-27T03:13:27Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4613",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4613#event-52702"
}
```



---

archive/issue_comments_027498.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nMerged in Sage 3.2.1.alpha2",
    "created_at": "2008-11-27T03:13:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4613",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4613#issuecomment-27498",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Merged in Sage 3.2.1.alpha2
