# Issue 4650: make density() in matrix_modn_sparse much faster

archive/issues_004650.json:
```json
{
    "assignees": [
        "https://github.com/craigcitro"
    ],
    "body": "matrix_modn_sparse needs some housecleaning.  One gem, in particular:\n\n```\n    cdef Py_ssize_t i, j, k\n    k = 0\n    for i from 0 <= i < self._nrows:\n        for j from 0 <= j < self.rows[i].num_nonzero:\n            k+=1\n    return QQ(k)/QQ(self.nrows()*self.ncols()) \n```\n\nalso, it could use some fast nonzero_positions, getitem, etc. methods\n\nKeywords: **sparse**\n\nComponent: **linear algebra**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4650_\n\n",
    "closed_at": "2009-01-18T04:50:41Z",
    "created_at": "2008-11-29T02:35:02Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "make density() in matrix_modn_sparse much faster",
    "type": "issue",
    "updated_at": "2009-01-18T04:50:41Z",
    "url": "https://github.com/sagemath/sage/issues/4650",
    "user": "https://github.com/boothby"
}
```
matrix_modn_sparse needs some housecleaning.  One gem, in particular:

```
    cdef Py_ssize_t i, j, k
    k = 0
    for i from 0 <= i < self._nrows:
        for j from 0 <= j < self.rows[i].num_nonzero:
            k+=1
    return QQ(k)/QQ(self.nrows()*self.ncols()) 
```

also, it could use some fast nonzero_positions, getitem, etc. methods

Keywords: **sparse**

Component: **linear algebra**

_Issue created by migration from https://trac.sagemath.org/ticket/4650_





---

archive/issue_events_053209.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-11-29T02:35:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4650",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4650#event-53209"
}
```



---

archive/issue_events_053210.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-11-29T02:35:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4650",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "c: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4650#event-53210"
}
```



---

archive/issue_events_053211.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-11-29T02:35:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4650",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4650#event-53211"
}
```



---

archive/issue_events_053212.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-11-29T02:35:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4650",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4650#event-53212"
}
```



---

archive/issue_events_053213.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-11-29T02:35:02Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4650",
    "subject": "https://github.com/boothby",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4650#event-53213"
}
```



---

archive/issue_events_053214.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2009-01-15T23:08:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4650",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4650#event-53214"
}
```



---

archive/issue_events_053215.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2009-01-15T23:08:14Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/4650",
    "subject": "https://github.com/craigcitro",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4650#event-53215"
}
```



---

archive/issue_events_053216.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2009-01-15T23:08:14Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4650",
    "subject": "https://github.com/craigcitro",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4650#event-53216"
}
```



---

archive/issue_comments_027824.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac-4650.patch.gz](https://github.com/sagemath/sage/files/ticket4650/trac-4650.patch.gz)**\n\nI agree, this file needs a lot of cleanup.\n\nI fixed the one example Tom mentioned above ... lots more needs to be done, but I thought it would be good to close this and open a new series of specific tickets (most of which will probably get handled at SD12 in San Diego).",
    "created_at": "2009-01-15T23:08:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4650",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4650#issuecomment-27824",
    "user": "https://github.com/craigcitro"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac-4650.patch.gz](https://github.com/sagemath/sage/files/ticket4650/trac-4650.patch.gz)**

I agree, this file needs a lot of cleanup.

I fixed the one example Tom mentioned above ... lots more needs to be done, but I thought it would be good to close this and open a new series of specific tickets (most of which will probably get handled at SD12 in San Diego).



---

archive/issue_events_053217.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2009-01-15T23:08:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4650",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4650#event-53217"
}
```



---

archive/issue_events_053218.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-18T04:50:29Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4650",
    "title_is": "make density() in matrix_modn_sparse much faster",
    "title_was": "matrix_modn_sparse needs cleanup",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4650#event-53218"
}
```



---

archive/issue_events_053219.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-18T04:50:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4650",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4650#event-53219"
}
```



---

archive/issue_comments_027825.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nPositive review. I change the summary to properly reflect what you fixed.\n\nAs you pointed out followup should happen via individual tickets.\n\nCheers,\n\nMichael",
    "created_at": "2009-01-18T04:50:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4650",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4650#issuecomment-27825",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Positive review. I change the summary to properly reflect what you fixed.

As you pointed out followup should happen via individual tickets.

Cheers,

Michael



---

archive/issue_events_053220.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-18T04:50:29Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4650",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4650#event-53220"
}
```



---

archive/issue_events_053221.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-18T04:50:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4650",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4650#event-53221"
}
```



---

archive/issue_comments_027826.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMerged in Sage 3.3.alpha0",
    "created_at": "2009-01-18T04:50:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4650",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4650#issuecomment-27826",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:3" align="right">comment:3</div>

Merged in Sage 3.3.alpha0



---

archive/issue_events_053222.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-18T04:50:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4650",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4650#event-53222"
}
```



---

archive/issue_events_053223.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-18T04:50:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4650",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4650#event-53223"
}
```
