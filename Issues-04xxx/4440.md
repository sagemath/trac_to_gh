# Issue 4440: [positive work] Automatic Indentation

archive/issues_004440.json:
```json
{
    "assignees": [],
    "body": "The patch provides automatic indentation for python after colons and same level identation.\nIt works up to four levels of identation.\n\nAssignee: **ahupfer**\n\nCC:  @jasongrout\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4440_\n\n",
    "closed_at": "2009-02-20T07:24:49Z",
    "created_at": "2008-11-04T19:26:07Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20notebook",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "[positive work] Automatic Indentation",
    "type": "issue",
    "updated_at": "2009-02-20T07:24:49Z",
    "url": "https://github.com/sagemath/sage/issues/4440",
    "user": "https://github.com/sagetrac-ahupfer"
}
```
The patch provides automatic indentation for python after colons and same level identation.
It works up to four levels of identation.

Assignee: **ahupfer**

CC:  @jasongrout

_Issue created by migration from https://trac.sagemath.org/ticket/4440_





---

archive/issue_events_043736.json:
```json
{
    "actor": "https://github.com/sagetrac-ahupfer",
    "created_at": "2008-11-04T19:26:07Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43736"
}
```



---

archive/issue_events_043737.json:
```json
{
    "actor": "https://github.com/sagetrac-ahupfer",
    "created_at": "2008-11-04T19:26:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20notebook",
    "label_color": "000080",
    "label_name": "component: notebook",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43737"
}
```



---

archive/issue_events_043738.json:
```json
{
    "actor": "https://github.com/sagetrac-ahupfer",
    "created_at": "2008-11-04T19:26:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43738"
}
```



---

archive/issue_events_043739.json:
```json
{
    "actor": "https://github.com/sagetrac-ahupfer",
    "created_at": "2008-11-04T19:26:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43739"
}
```



---

archive/issue_events_043740.json:
```json
{
    "actor": "https://github.com/sagetrac-ahupfer",
    "created_at": "2008-11-04T19:30:05Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "title_is": "[has patch] Automatic Identation",
    "title_was": "Automatic Identation",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43740"
}
```



---

archive/issue_events_043741.json:
```json
{
    "actor": "https://github.com/sagetrac-ahupfer",
    "created_at": "2008-11-04T19:30:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43741"
}
```



---

archive/issue_comments_026745.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThere is already an attempt to do this at #1684. \n\nCheers,\n\nMichael",
    "created_at": "2008-11-04T20:24:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4440#issuecomment-26745",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:2'>Comment 2:</a>
There is already an attempt to do this at #1684. 

Cheers,

Michael



---

archive/issue_events_043742.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-04T20:24:39Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "title_is": "Automatic Identation",
    "title_was": "[has patch] Automatic Identation",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43742"
}
```



---

archive/issue_events_043743.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2008-11-05T00:29:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43743"
}
```



---

archive/issue_events_043744.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2008-11-05T00:29:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43744"
}
```



---

archive/issue_comments_026746.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThe restriction to 4 indentation levels is decidedly strange, given that the patch itself has a 6-deep indentation.  This can be written more elegantly:\n\n```\nvar indenting, id, tab;\nindenting = RegExp(\"\\n( *)\",\"g\");\nwhile(indenting.test(text[0])) {\n  id   = indexing.lastIndex;\n  tab  = RegExp.lastMatch.substring(1);\n}\nif( id == second_last_break && second_last_break != -1) {\n    get_cell(id).value = text[0] + tab + text[1]; \n    set_cursor_position(cell, position + tab.length);\n}\n```\n\nThe above code hasn't been tested, but should be a good start.",
    "created_at": "2008-11-05T00:29:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4440#issuecomment-26746",
    "user": "https://github.com/sagetrac-boothby"
}
```

<a id='comment:3'>Comment 3:</a>
The restriction to 4 indentation levels is decidedly strange, given that the patch itself has a 6-deep indentation.  This can be written more elegantly:

```
var indenting, id, tab;
indenting = RegExp("\n( *)","g");
while(indenting.test(text[0])) {
  id   = indexing.lastIndex;
  tab  = RegExp.lastMatch.substring(1);
}
if( id == second_last_break && second_last_break != -1) {
    get_cell(id).value = text[0] + tab + text[1]; 
    set_cursor_position(cell, position + tab.length);
}
```

The above code hasn't been tested, but should be a good start.



---

archive/issue_comments_026747.json:
```json
{
    "body": "Attachment: **[identation.patch.gz](https://github.com/sagemath/sage/files/ticket4440/identation.patch.gz)**",
    "created_at": "2008-11-21T13:45:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4440#issuecomment-26747",
    "user": "https://github.com/sagetrac-ahupfer"
}
```

Attachment: **[identation.patch.gz](https://github.com/sagemath/sage/files/ticket4440/identation.patch.gz)**



---

archive/issue_comments_026748.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nImproved identation support with unlimited ident. Tested with Firefox, Safari, Chrome.\nWorks not, but doesn't break Internet Explorer and Opera.",
    "created_at": "2008-11-21T13:48:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4440#issuecomment-26748",
    "user": "https://github.com/sagetrac-ahupfer"
}
```

<a id='comment:4'>Comment 4:</a>
Improved identation support with unlimited ident. Tested with Firefox, Safari, Chrome.
Works not, but doesn't break Internet Explorer and Opera.



---

archive/issue_events_043745.json:
```json
{
    "actor": "https://github.com/sagetrac-ahupfer",
    "created_at": "2008-11-21T13:48:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43745"
}
```



---

archive/issue_events_043746.json:
```json
{
    "actor": "https://github.com/sagetrac-ahupfer",
    "created_at": "2008-11-21T13:48:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43746"
}
```



---

archive/issue_comments_026749.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nREVIEW\n\nThis is *REALLY* nice and *almost* works.  It has one bug, which is that if you enter a blank line, the auto-indent is off by one.  E.g., I got the following when I entered a blank line after the first print statement:\n\n```\nif True:\n    print \"hi\"\n    \n   print \"mom\"\n```\n\nI should have got either\n\n```\nif True:\n    print \"hi\"\n    \n    print \"mom\"\n```\nor \n\n```\nif True:\n    print \"hi\"\n\nprint \"mom\"\n```\n\nWhich is a design decision.  To be more like the Python/Ipython command line, one would get the latter.  To be more like an IDE, maybe the former.  I don't care which, just that the current behavior (which is neither) is buggy.\n\nThat said, if this bit rots forever that would be bad.  I would almost rather have the buggy patch than no patch at all, since even with the bug it's pretty nice functionality.\n\nIt would however by nice if there were a way to turn it off.  That will have to wait until Timothy Clemans user management features get in.",
    "created_at": "2008-11-27T17:33:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4440#issuecomment-26749",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:5'>Comment 5:</a>
REVIEW

This is *REALLY* nice and *almost* works.  It has one bug, which is that if you enter a blank line, the auto-indent is off by one.  E.g., I got the following when I entered a blank line after the first print statement:

```
if True:
    print "hi"
    
   print "mom"
```

I should have got either

```
if True:
    print "hi"
    
    print "mom"
```
or 

```
if True:
    print "hi"

print "mom"
```

Which is a design decision.  To be more like the Python/Ipython command line, one would get the latter.  To be more like an IDE, maybe the former.  I don't care which, just that the current behavior (which is neither) is buggy.

That said, if this bit rots forever that would be bad.  I would almost rather have the buggy patch than no patch at all, since even with the bug it's pretty nice functionality.

It would however by nice if there were a way to turn it off.  That will have to wait until Timothy Clemans user management features get in.



---

archive/issue_events_043747.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-27T17:33:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43747"
}
```



---

archive/issue_events_043748.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-27T17:33:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43748"
}
```



---

archive/issue_comments_026750.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nAttachment: **[4440.2.patch.gz](https://github.com/sagemath/sage/files/ticket4440/4440.2.patch.gz)**",
    "created_at": "2009-01-21T23:56:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4440#issuecomment-26750",
    "user": "https://github.com/sagetrac-boothby"
}
```

<a id='comment:6'>Comment 6:</a>
Attachment: **[4440.2.patch.gz](https://github.com/sagemath/sage/files/ticket4440/4440.2.patch.gz)**



---

archive/issue_events_043749.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2009-01-21T23:56:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43749"
}
```



---

archive/issue_events_043750.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2009-01-21T23:56:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43750"
}
```



---

archive/issue_comments_026751.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nI fixed the bug was reported.  I've tested this in firefox on windows only.  Also, I made the regexp significantly uglier to make it more robust (sadly), and so commented the snot out of it.",
    "created_at": "2009-01-21T23:58:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4440#issuecomment-26751",
    "user": "https://github.com/sagetrac-boothby"
}
```

<a id='comment:7'>Comment 7:</a>
I fixed the bug was reported.  I've tested this in firefox on windows only.  Also, I made the regexp significantly uglier to make it more robust (sadly), and so commented the snot out of it.



---

archive/issue_events_043751.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-01-22T00:27:18Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "title_is": "Automatic Indentation",
    "title_was": "Automatic Identation",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43751"
}
```



---

archive/issue_events_043752.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2009-01-22T17:21:21Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "milestone_number": null,
    "milestone_title": "sage-3.4.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43752"
}
```



---

archive/issue_events_043753.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2009-01-22T17:21:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43753"
}
```



---

archive/issue_comments_026752.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nNotes from Tom:\n* Fold the patches into one patch\n* Make the iphone check *after* the resize code",
    "created_at": "2009-01-22T19:26:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4440#issuecomment-26752",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:10'>Comment 10:</a>
Notes from Tom:
* Fold the patches into one patch
* Make the iphone check *after* the resize code



---

archive/issue_events_043754.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-01-22T19:26:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43754"
}
```



---

archive/issue_events_043755.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-01-22T19:26:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43755"
}
```



---

archive/issue_comments_026753.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nThis feature seems to work well.  I can see it being annoying if there happens to be a semi-colon at the end of the line for a language other than python.  For that reason, there should be a way to turn this off.  I think it's probably not going to be a huge problem, though, so I think this ought to go in (after the two fixes above are done), and a feature ticket created to make an option for the user to turn this off.",
    "created_at": "2009-01-22T19:33:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4440#issuecomment-26753",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:11'>Comment 11:</a>
This feature seems to work well.  I can see it being annoying if there happens to be a semi-colon at the end of the line for a language other than python.  For that reason, there should be a way to turn this off.  I think it's probably not going to be a huge problem, though, so I think this ought to go in (after the two fixes above are done), and a feature ticket created to make an option for the user to turn this off.



---

archive/issue_comments_026754.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nPositive review pending the above changes in \"Notes from Tom\"",
    "created_at": "2009-01-22T19:33:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4440#issuecomment-26754",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:12'>Comment 12:</a>
Positive review pending the above changes in "Notes from Tom"



---

archive/issue_comments_026755.json:
```json
{
    "body": "Replaces previous two patches",
    "created_at": "2009-01-23T09:28:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4440#issuecomment-26755",
    "user": "https://github.com/sagetrac-boothby"
}
```

Replaces previous two patches



---

archive/issue_events_043756.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2009-01-23T09:29:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43756"
}
```



---

archive/issue_events_043757.json:
```json
{
    "actor": "https://github.com/sagetrac-boothby",
    "created_at": "2009-01-23T09:29:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43757"
}
```



---

archive/issue_comments_026756.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nAttachment: **[4440-indentation.patch.gz](https://github.com/sagemath/sage/files/ticket4440/4440-indentation.patch.gz)**",
    "created_at": "2009-01-23T09:29:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4440#issuecomment-26756",
    "user": "https://github.com/sagetrac-boothby"
}
```

<a id='comment:13'>Comment 13:</a>
Attachment: **[4440-indentation.patch.gz](https://github.com/sagemath/sage/files/ticket4440/4440-indentation.patch.gz)**



---

archive/issue_comments_026757.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nVery nice.  This addresses wstein's comment above and now does the second option.\n\nPositive review.",
    "created_at": "2009-01-28T17:19:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4440#issuecomment-26757",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:14'>Comment 14:</a>
Very nice.  This addresses wstein's comment above and now does the second option.

Positive review.



---

archive/issue_events_043758.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-01-28T17:19:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43758"
}
```



---

archive/issue_events_043759.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-01-28T17:19:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43759"
}
```



---

archive/issue_comments_026758.json:
```json
{
    "body": "<a id='comment:15'>Comment 15:</a>\nMerged 4440-indentation.patch in Sage 3.3.alpha3.\n\nCheers,\n\nMichael",
    "created_at": "2009-01-28T18:04:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4440#issuecomment-26758",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:15'>Comment 15:</a>
Merged 4440-indentation.patch in Sage 3.3.alpha3.

Cheers,

Michael



---

archive/issue_events_043760.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-28T18:04:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43760"
}
```



---

archive/issue_events_043761.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-28T18:04:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43761"
}
```



---

archive/issue_comments_026759.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nThis causes a serious bug: #5293.  I vote we remove this patch from 3.3 and work on it.",
    "created_at": "2009-02-17T23:03:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4440#issuecomment-26759",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:16'>Comment 16:</a>
This causes a serious bug: #5293.  I vote we remove this patch from 3.3 and work on it.



---

archive/issue_events_043762.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2009-02-17T23:04:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43762"
}
```



---

archive/issue_comments_026760.json:
```json
{
    "body": "<a id='comment:18'>Comment 18:</a>\nmabshoff says that he will revert this patch for now.  I hope that it is fixed and gets back in soon!  It's a great piece of functionality!",
    "created_at": "2009-02-17T23:04:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4440#issuecomment-26760",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:18'>Comment 18:</a>
mabshoff says that he will revert this patch for now.  I hope that it is fixed and gets back in soon!  It's a great piece of functionality!



---

archive/issue_comments_026761.json:
```json
{
    "body": "<a id='comment:19'>Comment 19:</a>\n(to get a positive review, we need to fix the issue at #5293)",
    "created_at": "2009-02-17T23:05:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4440#issuecomment-26761",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:19'>Comment 19:</a>
(to get a positive review, we need to fix the issue at #5293)



---

archive/issue_comments_026762.json:
```json
{
    "body": "<a id='comment:20'>Comment 20:</a>\nReinstated due to positive review of the fix at #5293.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-20T06:47:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4440#issuecomment-26762",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:20'>Comment 20:</a>
Reinstated due to positive review of the fix at #5293.

Cheers,

Michael



---

archive/issue_events_043763.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-20T06:47:45Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "title_is": "[positive work] Automatic Indentation",
    "title_was": "Automatic Indentation",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43763"
}
```



---

archive/issue_comments_026763.json:
```json
{
    "body": "<a id='comment:21'>Comment 21:</a>\nMerged in Sage 3.3.rc3 again :)\n\nCheers,\n\nMichael",
    "created_at": "2009-02-20T07:24:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4440#issuecomment-26763",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:21'>Comment 21:</a>
Merged in Sage 3.3.rc3 again :)

Cheers,

Michael



---

archive/issue_events_043764.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-20T07:24:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43764"
}
```



---

archive/issue_events_043765.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-02-20T07:24:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4440",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4440#event-43765"
}
```
