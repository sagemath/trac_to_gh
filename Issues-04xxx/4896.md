# Issue 4896: fill in missing magma --> sage conversions

archive/issues_004896.json:
```json
{
    "assignees": [],
    "body": "Make it so all the following work:\n\n```\nsage: magma(QQ['x,y'].0).sage()\n```\n\nNote that a huge number of sage-->magma conversions for ring elements now work.  To find examples where the converse doesn't work, use this script:\n\n```\nsage: for R in sage.rings.tests.random_rings(): print R, magma(R.random_element()).sage()\n```\nafter applying #4779.\n\nWhen the above loop runs for \"a while\" without crashing (after applying #4779), then this ticket can be closed.\n\nAssignee: **@williamstein**\n\nKeywords: **magma**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4896_\n\n",
    "created_at": "2008-12-31T02:33:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20interfaces%3A%20optional",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fill in missing magma --> sage conversions",
    "type": "issue",
    "updated_at": "2022-12-29T01:42:26Z",
    "url": "https://github.com/sagemath/sage/issues/4896",
    "user": "https://github.com/williamstein"
}
```
Make it so all the following work:

```
sage: magma(QQ['x,y'].0).sage()
```

Note that a huge number of sage-->magma conversions for ring elements now work.  To find examples where the converse doesn't work, use this script:

```
sage: for R in sage.rings.tests.random_rings(): print R, magma(R.random_element()).sage()
```
after applying #4779.

When the above loop runs for "a while" without crashing (after applying #4779), then this ticket can be closed.

Assignee: **@williamstein**

Keywords: **magma**

_Issue created by migration from https://trac.sagemath.org/ticket/4896_





---

archive/issue_events_050711.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-12-31T02:33:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "milestone_number": null,
    "milestone_title": "sage-3.2.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4896#event-50711"
}
```



---

archive/issue_events_050712.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-12-31T02:33:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "component: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4896#event-50712"
}
```



---

archive/issue_events_050713.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-12-31T02:33:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4896#event-50713"
}
```



---

archive/issue_events_050714.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-12-31T02:33:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4896#event-50714"
}
```



---

archive/issue_comments_030704.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNo patch, i.e. better luck in 3.4 :)\n\nCheers,\n\nMichael",
    "created_at": "2009-01-02T07:07:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4896#issuecomment-30704",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:1'>Comment 1:</a>
No patch, i.e. better luck in 3.4 :)

Cheers,

Michael



---

archive/issue_events_050715.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-02T07:07:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "milestone_number": null,
    "milestone_title": "sage-3.2.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4896#event-50715"
}
```



---

archive/issue_events_050716.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-02T07:07:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "milestone_number": null,
    "milestone_title": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4896#event-50716"
}
```



---

archive/issue_events_050717.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-01-23T02:43:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4896#event-50717"
}
```



---

archive/issue_events_050718.json:
```json
{
    "actor": "https://github.com/aghitza",
    "created_at": "2009-01-23T02:43:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4896#event-50718"
}
```



---

archive/issue_events_050719.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4896#event-50719"
}
```



---

archive/issue_events_050720.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4896#event-50720"
}
```



---

archive/issue_events_050721.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4896#event-50721"
}
```



---

archive/issue_events_050722.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4896#event-50722"
}
```



---

archive/issue_events_050723.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4896#event-50723"
}
```



---

archive/issue_events_050724.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4896#event-50724"
}
```



---

archive/issue_events_050725.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4896#event-50725"
}
```



---

archive/issue_events_050726.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4896#event-50726"
}
```



---

archive/issue_events_050727.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-06-23T13:49:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "component: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4896#event-50727"
}
```



---

archive/issue_events_050728.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2015-06-23T13:49:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces%3A%20optional",
    "label_color": "0000ff",
    "label_name": "component: interfaces: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4896#event-50728"
}
```



---

archive/issue_comments_030705.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nGot\n\n```\nMultivariate Polynomial Ring in x0, x1, x2, x3, x4, x5, x6, x7 over Ring of integers modulo 30768  File \"<string>\", line 1\n    Residue class ring of integers modulo Integer(30768)['x0, x1, x2, x3, x4, x5, x6, x7'.replace('$.', 'x').replace('.', '')](dict([ ( ( Integer('0x0'), Integer('0x0'), Integer('0x0'), Integer('0x0'), Integer('0x2'), Integer('0x0'), Integer('0x0'), Integer('0x0') ), Integer(17090) ), ( ( Integer('0x0'), Integer('0x0'), Integer('0x0'), Integer('0x0'), Integer('0x0'), Integer('0x2'), Integer('0x0'), Integer('0x0') ), Integer(8615) ), ( ( Integer('0x0'), Integer('0x0'), Integer('0x0'), Integer('0x1'), Integer('0x0'), Integer('0x0'), Integer('0x1'), Integer('0x0') ), Integer(24187) ), ( ( Integer('0x0'), Integer('0x0'), Integer('0x0'), Integer('0x0'), Integer('0x1'), Integer('0x0'), Integer('0x0'), Integer('0x0') ), Integer(5374) ), ( ( Integer('0x0'), Integer('0x0'), Integer('0x0'), Integer('0x0'), Integer('0x0'), Integer('0x0'), Integer('0x0'), Integer('0x0') ), Integer(27378) ) ]))\n                ^\nSyntaxError: invalid syntax\n```\nand\n\n```\nUnivariate Polynomial Ring in x over Ring of integers modulo 11908  File \"<string>\", line 1\n    Residue class ring of integers modulo 11908['x'.replace('$.', 'x').replace('.', '')]([ 9823, 11770, 6616 ])\n                ^\nSyntaxError: invalid syntax\n```",
    "created_at": "2018-06-23T08:08:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4896#issuecomment-30705",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:8'>Comment 8:</a>
Got

```
Multivariate Polynomial Ring in x0, x1, x2, x3, x4, x5, x6, x7 over Ring of integers modulo 30768  File "<string>", line 1
    Residue class ring of integers modulo Integer(30768)['x0, x1, x2, x3, x4, x5, x6, x7'.replace('$.', 'x').replace('.', '')](dict([ ( ( Integer('0x0'), Integer('0x0'), Integer('0x0'), Integer('0x0'), Integer('0x2'), Integer('0x0'), Integer('0x0'), Integer('0x0') ), Integer(17090) ), ( ( Integer('0x0'), Integer('0x0'), Integer('0x0'), Integer('0x0'), Integer('0x0'), Integer('0x2'), Integer('0x0'), Integer('0x0') ), Integer(8615) ), ( ( Integer('0x0'), Integer('0x0'), Integer('0x0'), Integer('0x1'), Integer('0x0'), Integer('0x0'), Integer('0x1'), Integer('0x0') ), Integer(24187) ), ( ( Integer('0x0'), Integer('0x0'), Integer('0x0'), Integer('0x0'), Integer('0x1'), Integer('0x0'), Integer('0x0'), Integer('0x0') ), Integer(5374) ), ( ( Integer('0x0'), Integer('0x0'), Integer('0x0'), Integer('0x0'), Integer('0x0'), Integer('0x0'), Integer('0x0'), Integer('0x0') ), Integer(27378) ) ]))
                ^
SyntaxError: invalid syntax
```
and

```
Univariate Polynomial Ring in x over Ring of integers modulo 11908  File "<string>", line 1
    Residue class ring of integers modulo 11908['x'.replace('$.', 'x').replace('.', '')]([ 9823, 11770, 6616 ])
                ^
SyntaxError: invalid syntax
```



---

archive/issue_comments_030706.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nComes from\n\n```\nsage: R=Zmod(137)\nsage: magma(R)\nResidue class ring of integers modulo 137\nsage: magma(R).sage()\n  File \"<string>\", line 1\n    Residue class ring of integers modulo Integer(137)\n                ^\nSyntaxError: invalid syntax\n```",
    "created_at": "2018-06-23T12:19:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4896#issuecomment-30706",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:9'>Comment 9:</a>
Comes from

```
sage: R=Zmod(137)
sage: magma(R)
Residue class ring of integers modulo 137
sage: magma(R).sage()
  File "<string>", line 1
    Residue class ring of integers modulo Integer(137)
                ^
SyntaxError: invalid syntax
```



---

archive/issue_events_050729.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-06-23T12:20:34Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4896#event-50729"
}
```



---

archive/issue_events_050730.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-06-23T12:20:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4896#event-50730"
}
```



---

archive/issue_comments_030707.json:
```json
{
    "body": "Changed keywords from **** to **magma**",
    "created_at": "2018-06-23T12:20:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4896#issuecomment-30707",
    "user": "https://github.com/fchapoton"
}
```

Changed keywords from **** to **magma**



---

archive/issue_comments_030708.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ba30041901e5097f0fb106e9fd886ad6e9bd1d3d\">ba30041</a></td><td><code>conversion of Zmod(n) back from magma</code></td></tr></table>\n",
    "created_at": "2018-06-23T14:34:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4896#issuecomment-30708",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:11'>Comment 11:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ba30041901e5097f0fb106e9fd886ad6e9bd1d3d">ba30041</a></td><td><code>conversion of Zmod(n) back from magma</code></td></tr></table>




---

archive/issue_comments_030709.json:
```json
{
    "body": "Branch: **[u/chapoton/4896](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/4896)**",
    "created_at": "2018-06-23T14:34:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4896#issuecomment-30709",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/4896](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/4896)**



---

archive/issue_comments_030710.json:
```json
{
    "body": "Commit: **[ba30041](https://github.com/sagemath/sagetrac-mirror/commit/ba30041901e5097f0fb106e9fd886ad6e9bd1d3d)**",
    "created_at": "2018-06-23T14:34:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4896#issuecomment-30710",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[ba30041](https://github.com/sagemath/sagetrac-mirror/commit/ba30041901e5097f0fb106e9fd886ad6e9bd1d3d)**



---

archive/issue_comments_030711.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ab7d8a5c5057113b579bd774f7c7692e4f88367c\">ab7d8a5</a></td><td><code>adding a doctest for conversion from magma to sage of Zmod(n)</code></td></tr></table>\n",
    "created_at": "2018-06-23T14:38:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4896#issuecomment-30711",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:12'>Comment 12:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ab7d8a5c5057113b579bd774f7c7692e4f88367c">ab7d8a5</a></td><td><code>adding a doctest for conversion from magma to sage of Zmod(n)</code></td></tr></table>




---

archive/issue_comments_030712.json:
```json
{
    "body": "Changed commit from **[ba30041](https://github.com/sagemath/sagetrac-mirror/commit/ba30041901e5097f0fb106e9fd886ad6e9bd1d3d)** to **[ab7d8a5](https://github.com/sagemath/sagetrac-mirror/commit/ab7d8a5c5057113b579bd774f7c7692e4f88367c)**",
    "created_at": "2018-06-23T14:38:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4896#issuecomment-30712",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[ba30041](https://github.com/sagemath/sagetrac-mirror/commit/ba30041901e5097f0fb106e9fd886ad6e9bd1d3d)** to **[ab7d8a5](https://github.com/sagemath/sagetrac-mirror/commit/ab7d8a5c5057113b579bd774f7c7692e4f88367c)**



---

archive/issue_comments_030713.json:
```json
{
    "body": "Changed commit from **[ab7d8a5](https://github.com/sagemath/sagetrac-mirror/commit/ab7d8a5c5057113b579bd774f7c7692e4f88367c)** to **[895c9b3](https://github.com/sagemath/sagetrac-mirror/commit/895c9b3a71f179172388cc4173909bfbe656d03e)**",
    "created_at": "2018-06-23T15:26:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4896#issuecomment-30713",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[ab7d8a5](https://github.com/sagemath/sagetrac-mirror/commit/ab7d8a5c5057113b579bd774f7c7692e4f88367c)** to **[895c9b3](https://github.com/sagemath/sagetrac-mirror/commit/895c9b3a71f179172388cc4173909bfbe656d03e)**



---

archive/issue_comments_030714.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/895c9b3a71f179172388cc4173909bfbe656d03e\">895c9b3</a></td><td><code>correct position inside basic.m</code></td></tr></table>\n",
    "created_at": "2018-06-23T15:26:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4896#issuecomment-30714",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:13'>Comment 13:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/895c9b3a71f179172388cc4173909bfbe656d03e">895c9b3</a></td><td><code>correct position inside basic.m</code></td></tr></table>




---

archive/issue_comments_030715.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nbranch was moved to #25640",
    "created_at": "2018-06-24T11:25:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4896#issuecomment-30715",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:14'>Comment 14:</a>
branch was moved to #25640



---

archive/issue_comments_030716.json:
```json
{
    "body": "Changed branch from **[u/chapoton/4896](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/4896)** to none",
    "created_at": "2018-06-24T12:07:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4896#issuecomment-30716",
    "user": "https://github.com/fchapoton"
}
```

Changed branch from **[u/chapoton/4896](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/4896)** to none



---

archive/issue_comments_030717.json:
```json
{
    "body": "Changed commit from **[895c9b3](https://github.com/sagemath/sagetrac-mirror/commit/895c9b3a71f179172388cc4173909bfbe656d03e)** to none",
    "created_at": "2018-06-24T12:07:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4896#issuecomment-30717",
    "user": "https://github.com/fchapoton"
}
```

Changed commit from **[895c9b3](https://github.com/sagemath/sagetrac-mirror/commit/895c9b3a71f179172388cc4173909bfbe656d03e)** to none



---

archive/issue_comments_030718.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nupdate milestone 8.3 -> 8.4",
    "created_at": "2018-08-03T19:20:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4896#issuecomment-30718",
    "user": "https://github.com/videlec"
}
```

<a id='comment:16'>Comment 16:</a>
update milestone 8.3 -> 8.4



---

archive/issue_events_050731.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-08-03T19:20:18Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4896#event-50731"
}
```



---

archive/issue_events_050732.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-08-03T19:20:18Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4896#event-50732"
}
```



---

archive/issue_events_050733.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:42:26Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4896",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4896#event-50733"
}
```
