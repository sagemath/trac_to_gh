# Issue 4233: [with patch, positive review] fix doctest failure due to deprecation warning in calculus/all.py

archive/issues_004233.json:
```json
{
    "body": "\n\nAssignee: somebody\n\nKeywords: calculus, deprecation\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4233\n\n",
    "closed_at": "2008-10-02T01:42:19Z",
    "created_at": "2008-10-01T21:33:20Z",
    "labels": [
        "component: calculus",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1.3",
    "title": "[with patch, positive review] fix doctest failure due to deprecation warning in calculus/all.py",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4233",
    "user": "https://github.com/jhpalmieri"
}
```


Assignee: somebody

Keywords: calculus, deprecation

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4233





---

archive/issue_comments_032454.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [calc.patch](tarball://root/attachments/some-uuid/ticket4233/calc.patch) by @mwhansen created at 2008-10-02 00:33:21\n\nLooks good to me.",
    "created_at": "2008-10-02T00:33:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4233",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4233#issuecomment-32454",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'></a>
Attachment [calc.patch](tarball://root/attachments/some-uuid/ticket4233/calc.patch) by @mwhansen created at 2008-10-02 00:33:21

Looks good to me.



---

archive/issue_events_015791.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4233",
    "rename": {
        "from": "[with patch, needs review] fix doctest failure due to deprecation warning in calculus/all.py",
        "to": "[with patch, positive review] fix doctest failure due to deprecation warning in calculus/all.py"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4233#event-15791"
}
```



---

archive/issue_comments_032455.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-10-02T01:42:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4233",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4233#issuecomment-32455",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_032456.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in Sage 3.1.3.alpha3",
    "created_at": "2008-10-02T01:42:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4233",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4233#issuecomment-32456",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Merged in Sage 3.1.3.alpha3



---

archive/issue_events_015792.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-10-02T01:42:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4233",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4233#event-15792"
}
```
