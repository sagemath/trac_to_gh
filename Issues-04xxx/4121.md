# Issue 4121: [with patch, positive review] correct the translation for automorphism groups of graphs with edge labels

archive/issues_004121.json:
```json
{
    "body": "\n\nAssignee: @rlmill\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4121\n\n",
    "closed_at": "2008-09-15T01:54:26Z",
    "created_at": "2008-09-14T23:32:27Z",
    "labels": [
        "component: graph theory",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1.2",
    "title": "[with patch, positive review] correct the translation for automorphism groups of graphs with edge labels",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4121",
    "user": "https://github.com/rlmill"
}
```


Assignee: @rlmill

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4121





---

archive/issue_comments_031543.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [trac_4121-translation-edge-labels.patch](tarball://root/attachments/some-uuid/ticket4121/trac_4121-translation-edge-labels.patch) by @mwhansen created at 2008-09-15 00:00:12\n\nLooks good to me.  Good catch.",
    "created_at": "2008-09-15T00:00:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4121",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4121#issuecomment-31543",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'></a>
Attachment [trac_4121-translation-edge-labels.patch](tarball://root/attachments/some-uuid/ticket4121/trac_4121-translation-edge-labels.patch) by @mwhansen created at 2008-09-15 00:00:12

Looks good to me.  Good catch.



---

archive/issue_events_015379.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4121",
    "rename": {
        "from": "[with patch, needs review] correct the translation for automorphism groups of graphs with edge labels",
        "to": "[with patch, positive review] correct the translation for automorphism groups of graphs with edge labels"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4121#event-15379"
}
```



---

archive/issue_events_015380.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-15T01:54:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4121",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4121#event-15380"
}
```



---

archive/issue_comments_031544.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in Sage 3.1.2.rc4",
    "created_at": "2008-09-15T01:54:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4121",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4121#issuecomment-31544",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Merged in Sage 3.1.2.rc4



---

archive/issue_comments_031545.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-09-15T01:54:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4121",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4121#issuecomment-31545",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_015381.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-15T01:54:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4121",
    "milestone": "sage-3.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4121#event-15381"
}
```
