# Issue 4034: [with spkg, positive review] Fix rubiks.spkg build on Solaris

archive/issues_004034.json:
```json
{
    "body": "This is a similar problem like the singular.spkg: /usr/bin/install does not exist. The spkg at\n\nhttp://sage.math.washington.edu/home/mabshoff/release-cycles-3.1.2/alpha4/rubiks-20070912.p8.spkg\n\nworks around that.\n\nCheers,\n\nMichael\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4034\n\n",
    "closed_at": "2008-09-01T11:34:23Z",
    "created_at": "2008-09-01T09:50:03Z",
    "labels": [
        "component: porting: solaris",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1.2",
    "title": "[with spkg, positive review] Fix rubiks.spkg build on Solaris",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4034",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
This is a similar problem like the singular.spkg: /usr/bin/install does not exist. The spkg at

http://sage.math.washington.edu/home/mabshoff/release-cycles-3.1.2/alpha4/rubiks-20070912.p8.spkg

works around that.

Cheers,

Michael

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4034





---

archive/issue_comments_029800.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-09-01T09:50:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4034",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4034#issuecomment-29800",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing status from new to assigned.



---

archive/issue_comments_029801.json:
```json
{
    "body": "<a id='comment:2'></a>Installs fine on my 64-bit Debian/GNU Linux Core2Duo. `sage -t interfaces/rubik.py` works.",
    "created_at": "2008-09-01T10:35:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4034",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4034#issuecomment-29801",
    "user": "https://github.com/malb"
}
```

<a id='comment:2'></a>Installs fine on my 64-bit Debian/GNU Linux Core2Duo. `sage -t interfaces/rubik.py` works.



---

archive/issue_comments_029802.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-09-01T11:34:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4034",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4034#issuecomment-29802",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_009219.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-01T11:34:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4034",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4034#event-9219"
}
```



---

archive/issue_comments_029803.json:
```json
{
    "body": "<a id='comment:3'></a>Merged in Sage 3.1.2.alpha4",
    "created_at": "2008-09-01T11:34:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4034",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4034#issuecomment-29803",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>Merged in Sage 3.1.2.alpha4
