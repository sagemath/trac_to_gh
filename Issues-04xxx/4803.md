# Issue 4803: improvements to the polyhedra module documentation

archive/issues_004803.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-sbarthelemy"
    ],
    "body": "The attached patch corrects some imprecisions in the polyhedra module documentation.\n\nCC:  @sagetrac-mhampton\n\nComponent: **geometry**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4803_\n\n",
    "closed_at": "2008-12-17T14:37:46Z",
    "created_at": "2008-12-15T09:50:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.2.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "improvements to the polyhedra module documentation",
    "type": "issue",
    "updated_at": "2008-12-17T14:37:46Z",
    "url": "https://github.com/sagemath/sage/issues/4803",
    "user": "https://github.com/sagetrac-sbarthelemy"
}
```
The attached patch corrects some imprecisions in the polyhedra module documentation.

CC:  @sagetrac-mhampton

Component: **geometry**

_Issue created by migration from https://trac.sagemath.org/ticket/4803_





---

archive/issue_events_055352.json:
```json
{
    "actor": "https://github.com/sagetrac-sbarthelemy",
    "created_at": "2008-12-15T09:50:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "c: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-55352"
}
```



---

archive/issue_events_055353.json:
```json
{
    "actor": "https://github.com/sagetrac-sbarthelemy",
    "created_at": "2008-12-15T09:50:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-55353"
}
```



---

archive/issue_events_055354.json:
```json
{
    "actor": "https://github.com/sagetrac-sbarthelemy",
    "created_at": "2008-12-15T09:50:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-55354"
}
```



---

archive/issue_events_055355.json:
```json
{
    "actor": "https://github.com/sagetrac-sbarthelemy",
    "created_at": "2008-12-15T09:50:23Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "subject": "https://github.com/sagetrac-sbarthelemy",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-55355"
}
```



---

archive/issue_events_055356.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-15T09:52:22Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "milestone_number": null,
    "milestone_title": "sage-3.2.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-55356"
}
```



---

archive/issue_comments_029002.json:
```json
{
    "body": "Attachment: **[11113.patch.gz](https://github.com/sagemath/sage/files/ticket4803/11113.patch.gz)**",
    "created_at": "2008-12-15T09:57:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4803#issuecomment-29002",
    "user": "https://github.com/sagetrac-sbarthelemy"
}
```

Attachment: **[11113.patch.gz](https://github.com/sagemath/sage/files/ticket4803/11113.patch.gz)**



---

archive/issue_events_055357.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2008-12-15T10:15:00Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "title_is": "[with review] improvements to the polyhedra module documentation",
    "title_was": "improvements to the polyhedra module documentation",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-55357"
}
```



---

archive/issue_events_055358.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2008-12-15T10:15:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-55358"
}
```



---

archive/issue_comments_029003.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nThere are a couple typos in this proposed documentation change:\n\n\"minkowsky sum\" should be \"Minkowski sum\"\n\"positive (aka conic) combination\" should be \"positive (aka convex) combination\"\n\n(unless there is a meaning of conic combination that I don't know).\n\nThe Fukuda reference is good, it would be better to add a \"REFERENCES\" section since eventually such sections might be searched for and organized.\n\nThanks for working on this!\nMarshall Hampton",
    "created_at": "2008-12-15T10:15:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4803#issuecomment-29003",
    "user": "https://github.com/sagetrac-mhampton"
}
```

<div id="comment:2" align="right">comment:2</div>

There are a couple typos in this proposed documentation change:

"minkowsky sum" should be "Minkowski sum"
"positive (aka conic) combination" should be "positive (aka convex) combination"

(unless there is a meaning of conic combination that I don't know).

The Fukuda reference is good, it would be better to add a "REFERENCES" section since eventually such sections might be searched for and organized.

Thanks for working on this!
Marshall Hampton



---

archive/issue_events_055359.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2008-12-15T10:15:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "c: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-55359"
}
```



---

archive/issue_events_055360.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2008-12-15T10:15:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "c: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-55360"
}
```



---

archive/issue_comments_029004.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nHere is a new patch, against the previous\n\n> \"minkowsky sum\" should be \"Minkowski sum\"\n\nI fixed that thanks\n\n> \"positive (aka conic) combination\" should be \"positive (aka convex) combination\"\n> (unless there is a meaning of conic combination that I don't know).\n\nsee http://en.wikipedia.org/wiki/Conical_combination\n\nDattorro [1] uses \"conic hull\" instead of \"conical hull\". Fukuda uses noneg(ray1, ray2,...). I don't know which term is better. In the new patch I chose \"conic hull\", change it to what sounds best to you, english speakers.\n\n> The Fukuda reference is good, it would be better to add a \"REFERENCES\" section since eventually such sections might be searched for and organized.\n\nYou're right, fixed in the patch\n\n> Thanks for working on this!\n\nThanks for the feedback!\n\nS\u00e9bastien\n\n[1] http://meboo.convexoptimization.com/BOOK/convexgeometry.pdf",
    "created_at": "2008-12-15T11:04:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4803#issuecomment-29004",
    "user": "https://github.com/sagetrac-sbarthelemy"
}
```

<div id="comment:3" align="right">comment:3</div>

Here is a new patch, against the previous

> "minkowsky sum" should be "Minkowski sum"

I fixed that thanks

> "positive (aka conic) combination" should be "positive (aka convex) combination"
> (unless there is a meaning of conic combination that I don't know).

see http://en.wikipedia.org/wiki/Conical_combination

Dattorro [1] uses "conic hull" instead of "conical hull". Fukuda uses noneg(ray1, ray2,...). I don't know which term is better. In the new patch I chose "conic hull", change it to what sounds best to you, english speakers.

> The Fukuda reference is good, it would be better to add a "REFERENCES" section since eventually such sections might be searched for and organized.

You're right, fixed in the patch

> Thanks for working on this!

Thanks for the feedback!

Sébastien

[1] http://meboo.convexoptimization.com/BOOK/convexgeometry.pdf



---

archive/issue_comments_029005.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nAttachment: **[11114.patch.gz](https://github.com/sagemath/sage/files/ticket4803/11114.patch.gz)**",
    "created_at": "2008-12-15T11:08:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4803#issuecomment-29005",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Attachment: **[11114.patch.gz](https://github.com/sagemath/sage/files/ticket4803/11114.patch.gz)**



---

archive/issue_events_055361.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-15T11:08:01Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "title_is": "improvements to the polyhedra module documentation",
    "title_was": "[with review] improvements to the polyhedra module documentation",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-55361"
}
```



---

archive/issue_events_055362.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-15T11:08:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-55362"
}
```



---

archive/issue_events_055363.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-15T11:08:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-55363"
}
```



---

archive/issue_comments_029006.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nGreat, I think with those changes it should go in.  Its unclear to me what the best term is for the \"conic hull\" but that can be tweaked later.",
    "created_at": "2008-12-16T14:09:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4803#issuecomment-29006",
    "user": "https://github.com/sagetrac-mhampton"
}
```

<div id="comment:5" align="right">comment:5</div>

Great, I think with those changes it should go in.  Its unclear to me what the best term is for the "conic hull" but that can be tweaked later.



---

archive/issue_events_055364.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2008-12-16T14:09:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-55364"
}
```



---

archive/issue_events_055365.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2008-12-16T14:09:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-55365"
}
```



---

archive/issue_comments_029007.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nMerged in Sage 3.2.2.rc1",
    "created_at": "2008-12-17T14:37:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4803#issuecomment-29007",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:6" align="right">comment:6</div>

Merged in Sage 3.2.2.rc1



---

archive/issue_events_055366.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-17T14:37:46Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "milestone_number": null,
    "milestone_title": "sage-3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-55366"
}
```



---

archive/issue_events_055367.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-17T14:37:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "milestone_number": null,
    "milestone_title": "sage-3.2.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-55367"
}
```



---

archive/issue_events_055368.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-17T14:37:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-55368"
}
```



---

archive/issue_events_055369.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-12-17T14:37:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4803",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4803#event-55369"
}
```
