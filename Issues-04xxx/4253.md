# Issue 4253: polybori interface: equality operator for navigators

archive/issues_004253.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: r.<x,y>=BooleanPolynomialRing(2)\n\nsage: p=r(\"0\")\nsage: p.navigation()==p.navigation()\nFalse\n```\nShould be True, probably `__eq__` not implemented.\n\nMichael\n\nAssignee: **@malb**\n\nKeywords: **polybori**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4253_\n\n",
    "closed_at": "2008-10-11T06:40:49Z",
    "created_at": "2008-10-08T06:42:41Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.1.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "polybori interface: equality operator for navigators",
    "type": "issue",
    "updated_at": "2008-10-11T06:40:49Z",
    "url": "https://github.com/sagemath/sage/issues/4253",
    "user": "https://github.com/sagetrac-PolyBoRi"
}
```

```
sage: r.<x,y>=BooleanPolynomialRing(2)

sage: p=r("0")
sage: p.navigation()==p.navigation()
False
```
Should be True, probably `__eq__` not implemented.

Michael

Assignee: **@malb**

Keywords: **polybori**

_Issue created by migration from https://trac.sagemath.org/ticket/4253_





---

archive/issue_events_042567.json:
```json
{
    "actor": "https://github.com/sagetrac-PolyBoRi",
    "created_at": "2008-10-08T06:42:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4253",
    "milestone_number": null,
    "milestone_title": "sage-3.1.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4253#event-42567"
}
```



---

archive/issue_events_042568.json:
```json
{
    "actor": "https://github.com/sagetrac-PolyBoRi",
    "created_at": "2008-10-08T06:42:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4253",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20commutative%20algebra",
    "label_color": "0000ff",
    "label_name": "component: commutative algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4253#event-42568"
}
```



---

archive/issue_events_042569.json:
```json
{
    "actor": "https://github.com/sagetrac-PolyBoRi",
    "created_at": "2008-10-08T06:42:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4253",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4253#event-42569"
}
```



---

archive/issue_events_042570.json:
```json
{
    "actor": "https://github.com/sagetrac-PolyBoRi",
    "created_at": "2008-10-08T06:42:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4253",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4253#event-42570"
}
```



---

archive/issue_comments_025251.json:
```json
{
    "body": "Attachment: **[pbori_navigator_eq.patch.gz](https://github.com/sagemath/sage/files/ticket4253/pbori_navigator_eq.patch.gz)**",
    "created_at": "2008-10-08T14:17:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4253#issuecomment-25251",
    "user": "https://github.com/malb"
}
```

Attachment: **[pbori_navigator_eq.patch.gz](https://github.com/sagemath/sage/files/ticket4253/pbori_navigator_eq.patch.gz)**



---

archive/issue_events_042571.json:
```json
{
    "actor": "https://github.com/malb",
    "created_at": "2008-10-08T14:17:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4253",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4253#event-42571"
}
```



---

archive/issue_comments_025252.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nthanks, works :-)",
    "created_at": "2008-10-09T06:48:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4253#issuecomment-25252",
    "user": "https://github.com/sagetrac-PolyBoRi"
}
```

<a id='comment:2'>Comment 2:</a>
thanks, works :-)



---

archive/issue_events_042572.json:
```json
{
    "actor": "https://github.com/sagetrac-PolyBoRi",
    "created_at": "2008-10-09T06:48:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4253",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4253#event-42572"
}
```



---

archive/issue_events_042573.json:
```json
{
    "actor": "https://github.com/sagetrac-PolyBoRi",
    "created_at": "2008-10-09T06:48:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4253",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4253#event-42573"
}
```



---

archive/issue_comments_025253.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nMerged in Sage 3.1.3.rc0",
    "created_at": "2008-10-11T06:40:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4253#issuecomment-25253",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<a id='comment:3'>Comment 3:</a>
Merged in Sage 3.1.3.rc0



---

archive/issue_events_042574.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-10-11T06:40:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4253",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4253#event-42574"
}
```



---

archive/issue_events_042575.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-10-11T06:40:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4253",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4253#event-42575"
}
```
