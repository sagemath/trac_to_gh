# Issue 4437: [with patch, positive review] Sage 3.2.a2: numerical noise in sage/graphs/graph.py

archive/issues_004437.json:
```json
{
    "body": "On an x86:\n\n```\nsage -t  devel/sage/sage/graphs/graph.py                    **********************************************************************\nFile \"/home/mabshoff/build-3.2.a2/sage-3.2.alpha2-cicero/tmp/graph.py\", line 5802:\n    sage: P.spectrum(laplacian=True)\nExpected:\n    [...e-16, 2.0, 2.0, 2.0, 2.0, 2.0, 5.0, 5.0, 5.0, 5.0]\nGot:\n    [4.89153937105e-17, 2.0, 2.0, 2.0, 2.0, 2.0, 5.0, 5.0, 5.0, 5.0]\n**********************************************************************\n```\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4437\n\n",
    "closed_at": "2008-11-05T23:13:27Z",
    "created_at": "2008-11-04T13:53:58Z",
    "labels": [
        "component: doctest coverage",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.2",
    "title": "[with patch, positive review] Sage 3.2.a2: numerical noise in sage/graphs/graph.py",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4437",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
On an x86:

```
sage -t  devel/sage/sage/graphs/graph.py                    **********************************************************************
File "/home/mabshoff/build-3.2.a2/sage-3.2.alpha2-cicero/tmp/graph.py", line 5802:
    sage: P.spectrum(laplacian=True)
Expected:
    [...e-16, 2.0, 2.0, 2.0, 2.0, 2.0, 5.0, 5.0, 5.0, 5.0]
Got:
    [4.89153937105e-17, 2.0, 2.0, 2.0, 2.0, 2.0, 5.0, 5.0, 5.0, 5.0]
**********************************************************************
```

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4437





---

archive/issue_comments_034465.json:
```json
{
    "body": "Attachment [trac_4437.patch](tarball://root/attachments/some-uuid/ticket4437/trac_4437.patch) by mabshoff created at 2008-11-05 21:52:06",
    "created_at": "2008-11-05T21:52:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4437",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4437#issuecomment-34465",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Attachment [trac_4437.patch](tarball://root/attachments/some-uuid/ticket4437/trac_4437.patch) by mabshoff created at 2008-11-05 21:52:06



---

archive/issue_events_016671.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4437",
    "rename": {
        "from": "Sage 3.2.a2: numerical noise in sage/graphs/graph.py",
        "to": "[with patch, needs review] Sage 3.2.a2: numerical noise in sage/graphs/graph.py"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4437#event-16671"
}
```



---

archive/issue_events_016672.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4437",
    "rename": {
        "from": "[with patch, needs review] Sage 3.2.a2: numerical noise in sage/graphs/graph.py",
        "to": "[with patch, positive review] Sage 3.2.a2: numerical noise in sage/graphs/graph.py"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4437#event-16672"
}
```



---

archive/issue_comments_034466.json:
```json
{
    "body": "<a id='comment:2'></a>\nI'm okay with this.",
    "created_at": "2008-11-05T22:20:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4437",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4437#issuecomment-34466",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:2'></a>
I'm okay with this.



---

archive/issue_events_016673.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-05T23:13:27Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4437",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4437#event-16673"
}
```



---

archive/issue_comments_034467.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-11-05T23:13:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4437",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4437#issuecomment-34467",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_034468.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.2.alpha3",
    "created_at": "2008-11-05T23:13:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4437",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4437#issuecomment-34468",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.2.alpha3
