# Issue 4488: notebook -- login page not given when logged out and trying to view your worksheets

archive/issues_004488.json:
```json
{
    "assignees": [
        "https://github.com/TimothyClemans"
    ],
    "body": "Should redirect to login page instead of giving an error message.\n\nCC:  @mwhansen\n\nComponent: **notebook**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4488_\n\n",
    "closed_at": "2008-11-29T00:48:58Z",
    "created_at": "2008-11-10T03:28:03Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.2.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "notebook -- login page not given when logged out and trying to view your worksheets",
    "type": "issue",
    "updated_at": "2008-11-29T00:48:58Z",
    "url": "https://github.com/sagemath/sage/issues/4488",
    "user": "https://github.com/TimothyClemans"
}
```
Should redirect to login page instead of giving an error message.

CC:  @mwhansen

Component: **notebook**

_Issue created by migration from https://trac.sagemath.org/ticket/4488_





---

archive/issue_events_050910.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-11-10T03:28:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4488",
    "milestone_number": null,
    "milestone_title": "sage-3.2.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4488#event-50910"
}
```



---

archive/issue_events_050911.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-11-10T03:28:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4488",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4488#event-50911"
}
```



---

archive/issue_events_050912.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-11-10T03:28:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4488",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4488#event-50912"
}
```



---

archive/issue_events_050913.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-11-10T03:28:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4488",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4488#event-50913"
}
```



---

archive/issue_events_050914.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-11-10T03:28:03Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4488",
    "subject": "https://github.com/TimothyClemans",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4488#event-50914"
}
```



---

archive/issue_comments_026139.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[sage-4488.patch.gz](https://github.com/sagemath/sage/files/ticket4488/sage-4488.patch.gz)**",
    "created_at": "2008-11-10T03:32:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4488",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4488#issuecomment-26139",
    "user": "https://github.com/TimothyClemans"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[sage-4488.patch.gz](https://github.com/sagemath/sage/files/ticket4488/sage-4488.patch.gz)**



---

archive/issue_events_050915.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-11-10T03:32:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4488",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4488#event-50915"
}
```



---

archive/issue_events_050916.json:
```json
{
    "actor": "https://github.com/boothby",
    "created_at": "2008-11-10T04:27:31Z",
    "event": "unassigned",
    "issue": "https://github.com/sagemath/sage/issues/4488",
    "subject": "https://github.com/TimothyClemans",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4488#event-50916"
}
```



---

archive/issue_events_050917.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-11-10T04:27:31Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4488",
    "subject": "https://github.com/TimothyClemans",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4488#event-50917"
}
```



---

archive/issue_events_050918.json:
```json
{
    "actor": "https://github.com/TimothyClemans",
    "created_at": "2008-11-10T04:27:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4488",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4488#event-50918"
}
```



---

archive/issue_comments_026140.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nMike could you please review this? Thanks",
    "created_at": "2008-11-10T04:28:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4488",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4488#issuecomment-26140",
    "user": "https://github.com/TimothyClemans"
}
```

<div id="comment:3" align="right">comment:3</div>

Mike could you please review this? Thanks



---

archive/issue_comments_026141.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nLooks very good.  I like this and it moves things to be more the way standard web apps work.  Plus it's a simple 2-liner which is well done.",
    "created_at": "2008-11-28T23:55:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4488",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4488#issuecomment-26141",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:4" align="right">comment:4</div>

Looks very good.  I like this and it moves things to be more the way standard web apps work.  Plus it's a simple 2-liner which is well done.



---

archive/issue_events_050919.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-28T23:55:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4488",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4488#event-50919"
}
```



---

archive/issue_events_050920.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-28T23:56:05Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4488",
    "milestone_number": null,
    "milestone_title": "sage-3.2.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4488#event-50920"
}
```



---

archive/issue_events_050921.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-28T23:56:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4488",
    "milestone_number": null,
    "milestone_title": "sage-3.2.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4488#event-50921"
}
```



---

archive/issue_events_050922.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-29T00:48:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4488",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4488#event-50922"
}
```



---

archive/issue_events_050923.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-29T00:48:58Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4488",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4488#event-50923"
}
```



---

archive/issue_comments_026142.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nMerged in Sage 3.2.1.rc0",
    "created_at": "2008-11-29T00:48:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4488",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4488#issuecomment-26142",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:6" align="right">comment:6</div>

Merged in Sage 3.2.1.rc0
