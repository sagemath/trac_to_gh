# Issue 4434: hgmerge massively broken on os x

archive/issues_004434.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "When doing a graphical 3-way merge under OS X, hgmerge is missing so one gets infinite loops or errors.  This is a major problem.\n\nThe spkg here:\n\nhttp://sage.math.washington.edu/home/was/patches/mercurial-1.01.p2.spkg\n\ndoes the following:\n\n* Added custom hgmerge script for OS X. For some reason no script at all was \n  copied over, which lead to (1) hg merge silently failing painfully for most\n  users, and (2) for users that run install_scripts, they would get a fork\n  bomb, since hgmerge would call sage -hgmerge which would call hgmerge \n  ad infintum.\n\n\n\nComponent: **packages: standard**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4434_\n\n",
    "closed_at": "2008-11-04T13:34:54Z",
    "created_at": "2008-11-04T04:41:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "hgmerge massively broken on os x",
    "type": "issue",
    "updated_at": "2011-04-04T16:45:24Z",
    "url": "https://github.com/sagemath/sage/issues/4434",
    "user": "https://github.com/williamstein"
}
```
When doing a graphical 3-way merge under OS X, hgmerge is missing so one gets infinite loops or errors.  This is a major problem.

The spkg here:

http://sage.math.washington.edu/home/was/patches/mercurial-1.01.p2.spkg

does the following:

* Added custom hgmerge script for OS X. For some reason no script at all was 
  copied over, which lead to (1) hg merge silently failing painfully for most
  users, and (2) for users that run install_scripts, they would get a fork
  bomb, since hgmerge would call sage -hgmerge which would call hgmerge 
  ad infintum.



Component: **packages: standard**

_Issue created by migration from https://trac.sagemath.org/ticket/4434_





---

archive/issue_events_050130.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-04T04:41:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4434",
    "milestone_number": null,
    "milestone_title": "sage-3.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4434#event-50130"
}
```



---

archive/issue_events_050131.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-04T04:41:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4434",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4434#event-50131"
}
```



---

archive/issue_events_050132.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-04T04:41:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4434",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4434#event-50132"
}
```



---

archive/issue_events_050133.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-04T04:41:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4434",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4434#event-50133"
}
```



---

archive/issue_events_050134.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-04T04:41:23Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4434",
    "subject": "https://github.com/williamstein",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4434#event-50134"
}
```



---

archive/issue_events_050135.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-04T04:41:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4434",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4434#event-50135"
}
```



---

archive/issue_events_050136.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-04T13:34:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4434",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4434#event-50136"
}
```



---

archive/issue_comments_025697.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nSpkg looks good, but I checked in all outstanding changes.\n\nCheers,\n\nMichael",
    "created_at": "2008-11-04T13:34:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4434",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4434#issuecomment-25697",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:1" align="right">comment:1</div>

Spkg looks good, but I checked in all outstanding changes.

Cheers,

Michael



---

archive/issue_events_050137.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-04T13:34:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4434",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4434#event-50137"
}
```



---

archive/issue_events_050138.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-04T13:34:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4434",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4434#event-50138"
}
```



---

archive/issue_comments_025698.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nMerged in Sage 3.2.alpha3",
    "created_at": "2008-11-04T13:34:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4434",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4434#issuecomment-25698",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:2" align="right">comment:2</div>

Merged in Sage 3.2.alpha3



---

archive/issue_comments_025699.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\n#11120 reverses this patch. Your comments would be greatly appreciated!",
    "created_at": "2011-04-04T16:45:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4434",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4434#issuecomment-25699",
    "user": "https://github.com/kini"
}
```

<div id="comment:3" align="right">comment:3</div>

#11120 reverses this patch. Your comments would be greatly appreciated!
