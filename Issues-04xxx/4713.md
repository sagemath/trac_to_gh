# Issue 4713: [with patch, positive review] make an apply_map function for vectors

archive/issues_004713.json:
```json
{
    "body": "Matrices have the function; it would be handy for vectors to also have this utility function.\n\nAssignee: @jasongrout\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4713\n\n",
    "closed_at": "2008-12-07T09:06:44Z",
    "created_at": "2008-12-05T08:08:13Z",
    "labels": [
        "component: linear algebra",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.2.2",
    "title": "[with patch, positive review] make an apply_map function for vectors",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4713",
    "user": "https://github.com/jasongrout"
}
```
Matrices have the function; it would be handy for vectors to also have this utility function.

Assignee: @jasongrout

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4713





---

archive/issue_comments_037679.json:
```json
{
    "body": "Changing assignee from @williamstein to @jasongrout.",
    "created_at": "2008-12-05T08:32:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4713",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4713#issuecomment-37679",
    "user": "https://github.com/jasongrout"
}
```

Changing assignee from @williamstein to @jasongrout.



---

archive/issue_events_017990.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4713",
    "rename": {
        "from": "make an apply_map function for vectors",
        "to": "[with patch, needs review] make an apply_map function for vectors"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4713#event-17990"
}
```



---

archive/issue_comments_037680.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-12-05T08:32:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4713",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4713#issuecomment-37680",
    "user": "https://github.com/jasongrout"
}
```

Changing status from new to assigned.



---

archive/issue_comments_037681.json:
```json
{
    "body": "<a id='comment:2'></a>\nYou could also do this in the first example:\n\n```\nsage: m = vector(ZZ, 9, range(9)) \nsage: k.<a> = GF(9) \nsage: m.apply_map(k)\n(0, 1, 2, 0, 1, 2, 0, 1, 2)\n```\n\nI think it would be nice to have a really simple first example, that requires much less knowledge of \"abstract algebra\".  Maybe the first example could be for engineers or something?\n\n```\nsage: m = vector([1,x,sin(x+1)])\nsage: m.apply_map(x^2)\n(1, x^2, sin(x + 1)^2)\nsage: m.apply_map(sin)\n(sin(1), sin(x), sin(sin(x + 1)))\n```",
    "created_at": "2008-12-06T23:26:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4713",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4713#issuecomment-37681",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:2'></a>
You could also do this in the first example:

```
sage: m = vector(ZZ, 9, range(9)) 
sage: k.<a> = GF(9) 
sage: m.apply_map(k)
(0, 1, 2, 0, 1, 2, 0, 1, 2)
```

I think it would be nice to have a really simple first example, that requires much less knowledge of "abstract algebra".  Maybe the first example could be for engineers or something?

```
sage: m = vector([1,x,sin(x+1)])
sage: m.apply_map(x^2)
(1, x^2, sin(x + 1)^2)
sage: m.apply_map(sin)
(sin(1), sin(x), sin(sin(x + 1)))
```



---

archive/issue_events_017991.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4713",
    "rename": {
        "from": "[with patch, needs review] make an apply_map function for vectors",
        "to": "[with patch, needs work] make an apply_map function for vectors"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4713#event-17991"
}
```



---

archive/issue_comments_037682.json:
```json
{
    "body": "<a id='comment:3'></a>\nBy the way, definitely positive review pending adding the doctests suggested above.",
    "created_at": "2008-12-06T23:26:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4713",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4713#issuecomment-37682",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:3'></a>
By the way, definitely positive review pending adding the doctests suggested above.



---

archive/issue_events_017992.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4713",
    "rename": {
        "from": "[with patch, needs work] make an apply_map function for vectors",
        "to": "[with patch, positive review] make an apply_map function for vectors"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4713#event-17992"
}
```



---

archive/issue_comments_037683.json:
```json
{
    "body": "<a id='comment:4'></a>\nAttachment [vector_apply_map.patch](tarball://root/attachments/some-uuid/ticket4713/vector_apply_map.patch) by @jasongrout created at 2008-12-06 23:45:23\n\nupdated patch with the suggestions.  Accordingly, marking this positive review.",
    "created_at": "2008-12-06T23:45:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4713",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4713#issuecomment-37683",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:4'></a>
Attachment [vector_apply_map.patch](tarball://root/attachments/some-uuid/ticket4713/vector_apply_map.patch) by @jasongrout created at 2008-12-06 23:45:23

updated patch with the suggestions.  Accordingly, marking this positive review.



---

archive/issue_comments_037684.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-12-07T09:06:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4713",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4713#issuecomment-37684",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_017993.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-12-07T09:06:44Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4713",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4713#event-17993"
}
```



---

archive/issue_comments_037685.json:
```json
{
    "body": "<a id='comment:5'></a>\nMerged in Sage 3.2.2.alpha1",
    "created_at": "2008-12-07T09:06:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4713",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4713#issuecomment-37685",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:5'></a>
Merged in Sage 3.2.2.alpha1
