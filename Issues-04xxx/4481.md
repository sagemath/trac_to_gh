# Issue 4481: [with patch, positive review] Fix cached_function to accept functions without func_doc or func_name

archive/issues_004481.json:
```json
{
    "body": "Fix cached_function to accept functions without func_doc or func_name (with patch)\n\nAssignee: @nthiery\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4481\n\n",
    "closed_at": "2008-11-29T04:18:38Z",
    "created_at": "2008-11-09T14:13:04Z",
    "labels": [
        "component: misc",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.2.1",
    "title": "[with patch, positive review] Fix cached_function to accept functions without func_doc or func_name",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4481",
    "user": "https://github.com/nthiery"
}
```
Fix cached_function to accept functions without func_doc or func_name (with patch)

Assignee: @nthiery

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4481





---

archive/issue_events_016867.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4481",
    "rename": {
        "from": "Fix cached_function to accept functions without func_doc or func_name (with patch)",
        "to": "[with patch, needs review] Fix cached_function to accept functions without func_doc or func_name"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4481#event-16867"
}
```



---

archive/issue_comments_034987.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [cachefunc-fix-4481-nt.patch](tarball://root/attachments/some-uuid/ticket4481/cachefunc-fix-4481-nt.patch) by mabshoff created at 2008-11-09 16:22:44\n\nHi Nicolas,\n\nplease make sure to mark tickets with patches properly, i.e. prepend \"[with patch, needs review]\". That way the automated reports pick up this ticket.\n\nYou should also accept the ticket, so that the patch is properly credited to you in trac.\n\nCheers,\n\nMichael",
    "created_at": "2008-11-09T16:22:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4481#issuecomment-34987",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
Attachment [cachefunc-fix-4481-nt.patch](tarball://root/attachments/some-uuid/ticket4481/cachefunc-fix-4481-nt.patch) by mabshoff created at 2008-11-09 16:22:44

Hi Nicolas,

please make sure to mark tickets with patches properly, i.e. prepend "[with patch, needs review]". That way the automated reports pick up this ticket.

You should also accept the ticket, so that the patch is properly credited to you in trac.

Cheers,

Michael



---

archive/issue_comments_034988.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-11-09T21:01:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4481#issuecomment-34988",
    "user": "https://github.com/nthiery"
}
```

Changing status from new to assigned.



---

archive/issue_comments_034989.json:
```json
{
    "body": "Changing assignee from cwitty to @nthiery.",
    "created_at": "2008-11-09T21:01:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4481#issuecomment-34989",
    "user": "https://github.com/nthiery"
}
```

Changing assignee from cwitty to @nthiery.



---

archive/issue_comments_034990.json:
```json
{
    "body": "<a id='comment:2'></a>\nOops, done. Thanks Michael.",
    "created_at": "2008-11-09T21:01:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4481#issuecomment-34990",
    "user": "https://github.com/nthiery"
}
```

<a id='comment:2'></a>
Oops, done. Thanks Michael.



---

archive/issue_events_016868.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-11T12:35:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4481",
    "milestone": "sage-3.2.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4481#event-16868"
}
```



---

archive/issue_events_016869.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4481",
    "rename": {
        "from": "[with patch, needs review] Fix cached_function to accept functions without func_doc or func_name",
        "to": "[with patch, positive review] Fix cached_function to accept functions without func_doc or func_name"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4481#event-16869"
}
```



---

archive/issue_comments_034991.json:
```json
{
    "body": "<a id='comment:4'></a>\nLooks good.",
    "created_at": "2008-11-29T03:05:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4481#issuecomment-34991",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:4'></a>
Looks good.



---

archive/issue_comments_034992.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-11-29T04:18:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4481#issuecomment-34992",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_016870.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-29T04:18:38Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4481",
    "milestone": "sage-3.2.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4481#event-16870"
}
```



---

archive/issue_events_016871.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-29T04:18:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4481",
    "milestone": "sage-3.2.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4481#event-16871"
}
```



---

archive/issue_events_016872.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-29T04:18:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4481",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4481#event-16872"
}
```



---

archive/issue_comments_034993.json:
```json
{
    "body": "<a id='comment:5'></a>\nMerged in Sage 3.2.1.rc0.\n\nNicolas: Please make sure to post an hg patch instead of a diff next time. The patch has been checked in under your name, so credit is preserved.\n\nCheers,\n\nMichael",
    "created_at": "2008-11-29T04:18:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4481#issuecomment-34993",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:5'></a>
Merged in Sage 3.2.1.rc0.

Nicolas: Please make sure to post an hg patch instead of a diff next time. The patch has been checked in under your name, so credit is preserved.

Cheers,

Michael
