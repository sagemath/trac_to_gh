# Issue 4574: add notebook_object.py docs to the reference manual, and possibly a short survey about that to notebook help (top letter)

archive/issues_004574.json:
```json
{
    "assignees": [],
    "body": "\n```\n\n\nOn Thu, Nov 20, 2008 at 6:30 PM, seberino@spawar.navy.mil <seberino@spawar.navy.mil> wrote:\n>\n> On Nov 20, 1:05 pm, \"William Stein\" <wst...@gmail.com> wrote:\n>> Type notebook? and read the docs.  If that isn't enough please please\n>> do ask here again.\n>\n> The \"Help\" link inside the notebook gives a ton of great docs on how\n> to do great things with notebook.  I couldn't find the section on how\n> to add new notebook accounts in those docs.\n> Should I be looking somewhere else?\n\nIt \"should\" be in chapter 3 on the notebook.  Unfortunately, I just\nlooked and it isn't.    The problem is that the file notebook_object.py\nisn't included in the reference manual.  \n\nMaybe Mike Hansen can fix this when he Sphinxifies the documentation\nfor Sage, which he'll do very soon. \n```\n\nCC:  @sagetrac-mvngu\n\nComponent: **documentation**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4574_\n\n",
    "closed_at": "2010-02-27T23:42:23Z",
    "created_at": "2008-11-21T04:24:52Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-4.3.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "add notebook_object.py docs to the reference manual, and possibly a short survey about that to notebook help (top letter)",
    "type": "issue",
    "updated_at": "2010-02-27T23:42:23Z",
    "url": "https://github.com/sagemath/sage/issues/4574",
    "user": "https://github.com/williamstein"
}
```

```


On Thu, Nov 20, 2008 at 6:30 PM, seberino@spawar.navy.mil <seberino@spawar.navy.mil> wrote:
>
> On Nov 20, 1:05 pm, "William Stein" <wst...@gmail.com> wrote:
>> Type notebook? and read the docs.  If that isn't enough please please
>> do ask here again.
>
> The "Help" link inside the notebook gives a ton of great docs on how
> to do great things with notebook.  I couldn't find the section on how
> to add new notebook accounts in those docs.
> Should I be looking somewhere else?

It "should" be in chapter 3 on the notebook.  Unfortunately, I just
looked and it isn't.    The problem is that the file notebook_object.py
isn't included in the reference manual.  

Maybe Mike Hansen can fix this when he Sphinxifies the documentation
for Sage, which he'll do very soon. 
```

CC:  @sagetrac-mvngu

Component: **documentation**

_Issue created by migration from https://trac.sagemath.org/ticket/4574_





---

archive/issue_events_052212.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-21T04:24:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4574",
    "milestone_number": null,
    "milestone_title": "sage-4.3.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4574#event-52212"
}
```



---

archive/issue_events_052213.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-21T04:24:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4574",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4574#event-52213"
}
```



---

archive/issue_events_052214.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-21T04:24:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4574",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4574#event-52214"
}
```



---

archive/issue_events_052215.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-21T04:24:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4574",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4574#event-52215"
}
```



---

archive/issue_comments_027165.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nRelated: #7506",
    "created_at": "2010-01-20T11:37:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4574",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4574#issuecomment-27165",
    "user": "https://github.com/qed777"
}
```

<div id="comment:1" align="right">comment:1</div>

Related: #7506



---

archive/issue_events_052216.json:
```json
{
    "actor": "https://github.com/qed777",
    "created_at": "2010-02-27T22:51:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4574",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4574#event-52216"
}
```



---

archive/issue_comments_027166.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nShould we close this?",
    "created_at": "2010-02-27T22:51:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4574",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4574#issuecomment-27166",
    "user": "https://github.com/qed777"
}
```

<div id="comment:2" align="right">comment:2</div>

Should we close this?



---

archive/issue_comments_027167.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nClose as fixed by #7506.",
    "created_at": "2010-02-27T23:42:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4574",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4574#issuecomment-27167",
    "user": "https://github.com/sagetrac-mvngu"
}
```

<div id="comment:3" align="right">comment:3</div>

Close as fixed by #7506.



---

archive/issue_events_052217.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-02-27T23:42:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4574",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4574#event-52217"
}
```



---

archive/issue_events_052218.json:
```json
{
    "actor": "https://github.com/sagetrac-mvngu",
    "created_at": "2010-02-27T23:42:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4574",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4574#event-52218"
}
```
