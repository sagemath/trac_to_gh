# Issue 4751: if spkg/standard contains an extracted directory then "sage -upgrade" fails in multiple ways

archive/issues_004751.json:
```json
{
    "body": "\n```\n  File \"/home/was/build/sage-3.2.2.alpha0/local/bin/sage-update\", line 178, in do_update\n    if 'Placeholder spkg file' in open(F).readline():\nIOError: [Errno 21] Is a directory\nError getting new packages!\nwas@sage:~/build/sage-3.2.2.alpha0$ \n```\n\nAlso, later it would try to move the directory out of the way, which will fail.\n\n**Assignee:** mabshoff\n\nIssue created by migration from https://trac.sagemath.org/ticket/4751\n\n",
    "closed_at": "2008-12-11T11:15:16Z",
    "created_at": "2008-12-10T13:28:03Z",
    "labels": [
        "component: packages: standard",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.2.2",
    "title": "if spkg/standard contains an extracted directory then \"sage -upgrade\" fails in multiple ways",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4751",
    "user": "https://github.com/williamstein"
}
```

```
  File "/home/was/build/sage-3.2.2.alpha0/local/bin/sage-update", line 178, in do_update
    if 'Placeholder spkg file' in open(F).readline():
IOError: [Errno 21] Is a directory
Error getting new packages!
was@sage:~/build/sage-3.2.2.alpha0$ 
```

Also, later it would try to move the directory out of the way, which will fail.

**Assignee:** mabshoff

Issue created by migration from https://trac.sagemath.org/ticket/4751





---

archive/attachments_005303.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_4751.patch",
    "asset_url": "tarball://root/attachments/some-uuid/ticket4751/trac_4751.patch",
    "created_at": "2008-12-10T13:40:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4751",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.patch",
    "user": "https://github.com/williamstein"
}
```



---

archive/issue_comments_030344.json:
```json
{
    "body": "<a id='comment:1'></a>\n",
    "created_at": "2008-12-10T13:40:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4751",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4751#issuecomment-30344",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:1'></a>




---

archive/issue_events_029259.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-12-10T13:40:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/4751",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4751#event-29259"
}
```



---

archive/issue_events_029260.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-12-11T11:14:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/4751",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4751#event-29260"
}
```



---

archive/issue_events_029261.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-12-11T11:14:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/4751",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4751#event-29261"
}
```



---

archive/issue_comments_030345.json:
```json
{
    "body": "<a id='comment:2'></a>\nLooks good to me.\n\nCheers,\n\nMichael",
    "created_at": "2008-12-11T11:14:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4751",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4751#issuecomment-30345",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Looks good to me.

Cheers,

Michael



---

archive/issue_comments_030346.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.2.2.alpha2",
    "created_at": "2008-12-11T11:15:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4751",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4751#issuecomment-30346",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.2.2.alpha2



---

archive/issue_events_029262.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-12-11T11:15:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/4751",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4751#event-29262"
}
```



---

archive/issue_events_029263.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-12-11T11:15:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4751",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4751#event-29263"
}
```
