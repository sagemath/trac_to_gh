# Issue 4765: [with patch, positive review] Sage 3.2.2.alpha1: numerical noise in sage/rings/number_field/number_field_morphisms.pyx on OSX 10.4/G5

archive/issues_004765.json:
```json
{
    "body": "\n```\nsage -t -long \"devel/sage/sage/rings/number_field/number_field_morphisms.pyx\"\n**********************************************************************\nFile \"/Users/mabshoff/sage-3.2.2.alpha1/devel/sage/sage/rings/number_field/number_field_morphisms.pyx\", line 214, in __main__.example_10\nFailed example:\n    matching_root(x**Integer(3)-Integer(1), CDF.gen(0))###line 227:_sage_    >>> matching_root(x^3-1, CDF.0)\nExpected:\n    -0.500000000000000 + 0.86602540378443...*I\nGot:\n    -0.500000000000001 + 0.866025403784439*I\n**********************************************************************\n```\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4765\n\n",
    "closed_at": "2008-12-12T14:44:24Z",
    "created_at": "2008-12-12T03:42:00Z",
    "labels": [
        "component: doctest coverage",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.2.2",
    "title": "[with patch, positive review] Sage 3.2.2.alpha1: numerical noise in sage/rings/number_field/number_field_morphisms.pyx on OSX 10.4/G5",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4765",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

```
sage -t -long "devel/sage/sage/rings/number_field/number_field_morphisms.pyx"
**********************************************************************
File "/Users/mabshoff/sage-3.2.2.alpha1/devel/sage/sage/rings/number_field/number_field_morphisms.pyx", line 214, in __main__.example_10
Failed example:
    matching_root(x**Integer(3)-Integer(1), CDF.gen(0))###line 227:_sage_    >>> matching_root(x^3-1, CDF.0)
Expected:
    -0.500000000000000 + 0.86602540378443...*I
Got:
    -0.500000000000001 + 0.866025403784439*I
**********************************************************************
```

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4765





---

archive/issue_comments_038298.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-12-12T03:42:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4765",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4765#issuecomment-38298",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing status from new to assigned.



---

archive/issue_comments_038299.json:
```json
{
    "body": "Attachment [trac_4765.patch](tarball://root/attachments/some-uuid/ticket4765/trac_4765.patch) by mabshoff created at 2008-12-12 13:52:50",
    "created_at": "2008-12-12T13:52:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4765",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4765#issuecomment-38299",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Attachment [trac_4765.patch](tarball://root/attachments/some-uuid/ticket4765/trac_4765.patch) by mabshoff created at 2008-12-12 13:52:50



---

archive/issue_comments_038300.json:
```json
{
    "body": "<a id='comment:2'></a>\nLooks good to me.",
    "created_at": "2008-12-12T13:55:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4765",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4765#issuecomment-38300",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:2'></a>
Looks good to me.



---

archive/issue_events_018255.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4765",
    "rename": {
        "from": "Sage 3.2.2.alpha1: numerical noise in sage/rings/number_field/number_field_morphisms.pyx on OSX 10.4/G5",
        "to": "[with patch, positive review] Sage 3.2.2.alpha1: numerical noise in sage/rings/number_field/number_field_morphisms.pyx on OSX 10.4/G5"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4765#event-18255"
}
```



---

archive/issue_events_018256.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-12-12T14:44:24Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4765",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4765#event-18256"
}
```



---

archive/issue_comments_038301.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.2.2.alpha2",
    "created_at": "2008-12-12T14:44:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4765",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4765#issuecomment-38301",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.2.2.alpha2



---

archive/issue_comments_038302.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-12-12T14:44:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4765",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4765#issuecomment-38302",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
