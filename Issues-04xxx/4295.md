# Issue 4295: delete mercurial-howto.txt

archive/issues_004295.json:
```json
{
    "body": "\n\n**Assignee:** mabshoff\n\nIssue created by migration from https://trac.sagemath.org/ticket/4295\n\n",
    "closed_at": "2008-10-15T15:39:12Z",
    "created_at": "2008-10-15T12:57:25Z",
    "labels": [
        "component: misc",
        "critical",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1.4",
    "title": "delete mercurial-howto.txt",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4295",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```


**Assignee:** mabshoff

Issue created by migration from https://trac.sagemath.org/ticket/4295





---

archive/attachments_004588.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_4295.patch",
    "asset_url": "tarball://root/attachments/some-uuid/ticket4295/trac_4295.patch",
    "created_at": "2008-10-15T15:32:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4295",
    "type": "attachment",
    "url": "https://github.com/assets/some-id/some-uuid.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```



---

archive/issue_comments_025682.json:
```json
{
    "body": "",
    "created_at": "2008-10-15T15:32:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4295",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4295#issuecomment-25682",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```





---

archive/issue_events_024871.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-10-15T15:33:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/4295",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4295#event-24871"
}
```



---

archive/issue_events_024872.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-10-15T15:33:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/4295",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4295#event-24872"
}
```



---

archive/issue_events_024873.json:
```json
{
    "actor": "https://github.com/rlmill",
    "created_at": "2008-10-15T15:33:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sagetest/issues/4295",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4295#event-24873"
}
```



---

archive/issue_events_024874.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-10-15T15:39:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sagetest/issues/4295",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4295#event-24874"
}
```



---

archive/issue_events_024875.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-10-15T15:39:12Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4295",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4295#event-24875"
}
```



---

archive/issue_comments_025683.json:
```json
{
    "body": "<a id='comment:4'></a>\nMerged in Sage 3.1.4.\n\nCheers,\n\nMichael",
    "created_at": "2008-10-15T15:39:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4295",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4295#issuecomment-25683",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>
Merged in Sage 3.1.4.

Cheers,

Michael



---

archive/issue_events_024876.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-10-15T15:39:12Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4295",
    "milestone": "sage-3.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4295#event-24876"
}
```



---

archive/issue_events_024877.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-10-15T15:39:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4295",
    "milestone": "sage-3.1.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4295#event-24877"
}
```
