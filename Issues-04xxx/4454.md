# Issue 4454: bug in preparser

archive/issues_004454.json:
```json
{
    "body": "sage 3.1.4 on a 32-bit core duo\n\nconsider a file 'time.sage' with the following content:\n\n```\ndef func(time = 5):\n    time = RDF(time)\n    return time\n```\nloading this file then yields following error:\n\n```\nsage: load time.sage\n------------------------------------------------------------\n   File \"/home/georg/.sage/temp/HILBERT/4119/_home_georg_Daten_Sync_Software_Sage_Experimente_time_time_sage_0.py\", line 7\n     __time__=misc.cputime(); __wall__=misc.walltime();  = RDF(time); print     \"Time: CPU %.2f s, Wall: %.2f s\"%(misc.cputime(__time__), misc.walltime(__wall__))\n                                                         ^\nSyntaxError: invalid syntax\n\nWARNING: Failure executing file: </home/georg/.sage/temp/HILBERT/4119/_home_georg_Daten_Sync_Software_Sage_Experimente_time_time_sage_0.py>\n```\n\nIf one quits the space between 'time' and '=' in the second line of 'time.sage' it works as expected.\nDefining this function directly on the sage prompt also works as expected.\n\nAssignee: cwitty\n\nCC:  @robertwb boothby\n\nResolution: invalid\n\nIssue created by migration from https://trac.sagemath.org/ticket/4454\n\n",
    "closed_at": "2009-06-04T23:35:27Z",
    "created_at": "2008-11-06T15:25:38Z",
    "labels": [
        "component: misc",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "bug in preparser",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4454",
    "user": "https://trac.sagemath.org/admin/accounts/users/ggrafendorfer"
}
```
sage 3.1.4 on a 32-bit core duo

consider a file 'time.sage' with the following content:

```
def func(time = 5):
    time = RDF(time)
    return time
```
loading this file then yields following error:

```
sage: load time.sage
------------------------------------------------------------
   File "/home/georg/.sage/temp/HILBERT/4119/_home_georg_Daten_Sync_Software_Sage_Experimente_time_time_sage_0.py", line 7
     __time__=misc.cputime(); __wall__=misc.walltime();  = RDF(time); print     "Time: CPU %.2f s, Wall: %.2f s"%(misc.cputime(__time__), misc.walltime(__wall__))
                                                         ^
SyntaxError: invalid syntax

WARNING: Failure executing file: </home/georg/.sage/temp/HILBERT/4119/_home_georg_Daten_Sync_Software_Sage_Experimente_time_time_sage_0.py>
```

If one quits the space between 'time' and '=' in the second line of 'time.sage' it works as expected.
Defining this function directly on the sage prompt also works as expected.

Assignee: cwitty

CC:  @robertwb boothby

Resolution: invalid

Issue created by migration from https://trac.sagemath.org/ticket/4454





---

archive/issue_comments_035575.json:
```json
{
    "body": "<a id='comment:1'></a>I don't know why in the final view it looks like this, of course the 'return time' part of the second line of the function definition should be a separate line.",
    "created_at": "2008-11-06T15:29:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4454",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4454#issuecomment-35575",
    "user": "https://trac.sagemath.org/admin/accounts/users/ggrafendorfer"
}
```

<a id='comment:1'></a>I don't know why in the final view it looks like this, of course the 'return time' part of the second line of the function definition should be a separate line.



---

archive/issue_comments_035576.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,22 +2,23 @@\n \n consider a file 'time.sage' with the following content:\n \n+```\n def func(time = 5):\n     time = RDF(time)\n     return time\n-\n+```\n loading this file then yields following error:\n \n+```\n sage: load time.sage\n-\n----\n+------------------------------------------------------------\n    File \"/home/georg/.sage/temp/HILBERT/4119/_home_georg_Daten_Sync_Software_Sage_Experimente_time_time_sage_0.py\", line 7\n      __time__=misc.cputime(); __wall__=misc.walltime();  = RDF(time); print     \"Time: CPU %.2f s, Wall: %.2f s\"%(misc.cputime(__time__), misc.walltime(__wall__))\n                                                          ^\n SyntaxError: invalid syntax\n \n WARNING: Failure executing file: </home/georg/.sage/temp/HILBERT/4119/_home_georg_Daten_Sync_Software_Sage_Experimente_time_time_sage_0.py>\n-\n+```\n \n If one quits the space between 'time' and '=' in the second line of 'time.sage' it works as expected.\n Defining this function directly on the sage prompt also works as expected.\n``````\n",
    "created_at": "2008-11-06T22:04:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4454",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4454#issuecomment-35576",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,22 +2,23 @@
 
 consider a file 'time.sage' with the following content:
 
+```
 def func(time = 5):
     time = RDF(time)
     return time
-
+```
 loading this file then yields following error:
 
+```
 sage: load time.sage
-
----
+------------------------------------------------------------
    File "/home/georg/.sage/temp/HILBERT/4119/_home_georg_Daten_Sync_Software_Sage_Experimente_time_time_sage_0.py", line 7
      __time__=misc.cputime(); __wall__=misc.walltime();  = RDF(time); print     "Time: CPU %.2f s, Wall: %.2f s"%(misc.cputime(__time__), misc.walltime(__wall__))
                                                          ^
 SyntaxError: invalid syntax
 
 WARNING: Failure executing file: </home/georg/.sage/temp/HILBERT/4119/_home_georg_Daten_Sync_Software_Sage_Experimente_time_time_sage_0.py>
-
+```
 
 If one quits the space between 'time' and '=' in the second line of 'time.sage' it works as expected.
 Defining this function directly on the sage prompt also works as expected.
``````




---

archive/issue_comments_035577.json:
```json
{
    "body": "<a id='comment:3'></a>Hi,\n\nyou need to use \"`\" and \"`\" to get verbatim output - see also the main trac page. It would also be useful to have a more descriptive summary since as is \"bug in preparser\" is not very detailed :)\n\nCheers,\n\nMichael",
    "created_at": "2008-11-06T22:06:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4454",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4454#issuecomment-35577",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>Hi,

you need to use "`" and "`" to get verbatim output - see also the main trac page. It would also be useful to have a more descriptive summary since as is "bug in preparser" is not very detailed :)

Cheers,

Michael



---

archive/issue_comments_035578.json:
```json
{
    "body": "<a id='comment:4'></a>This is still a problem despite numerous preparser bugs being fixed during SD 12:\n\n```\nsage: load time.sage\n------------------------------------------------------------\n   File \"/scratch/mabshoff/dot_sage/temp/sage.math.washington.edu/4348/_scratch_mabshoff_sage_3_3_rc0_time_sage_1.py\", line 8\n     __time__=misc.cputime(); __wall__=misc.walltime();  = RDF(time); print         \"Time: CPU %.2f s, Wall: %.2f s\"%(misc.cputime(__time__), misc.walltime(__wall__))\n                                                         ^\nSyntaxError: invalid syntax\n\nWARNING: Failure executing file: </scratch/mabshoff/dot_sage/temp/sage.math.washington.edu/4348/_scratch_mabshoff_sage_3_3_rc0_time_sage_1.py>\n```",
    "created_at": "2009-02-11T07:53:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4454",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4454#issuecomment-35578",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:4'></a>This is still a problem despite numerous preparser bugs being fixed during SD 12:

```
sage: load time.sage
------------------------------------------------------------
   File "/scratch/mabshoff/dot_sage/temp/sage.math.washington.edu/4348/_scratch_mabshoff_sage_3_3_rc0_time_sage_1.py", line 8
     __time__=misc.cputime(); __wall__=misc.walltime();  = RDF(time); print         "Time: CPU %.2f s, Wall: %.2f s"%(misc.cputime(__time__), misc.walltime(__wall__))
                                                         ^
SyntaxError: invalid syntax

WARNING: Failure executing file: </scratch/mabshoff/dot_sage/temp/sage.math.washington.edu/4348/_scratch_mabshoff_sage_3_3_rc0_time_sage_1.py>
```



---

archive/issue_comments_035579.json:
```json
{
    "body": "<a id='comment:5'></a>Have you verified that this is not taken care of by #5106?",
    "created_at": "2009-02-11T07:57:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4454",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4454#issuecomment-35579",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:5'></a>Have you verified that this is not taken care of by #5106?



---

archive/issue_events_010054.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-06-04T23:35:27Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4454",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4454#event-10054"
}
```



---

archive/issue_comments_035580.json:
```json
{
    "body": "<a id='comment:6'></a>This has been fixed by #5106:\n\n```\n----------------------------------------------------------------------\n----------------------------------------------------------------------\nsage: load time.sage\nsage: func()\n5.0\nsage: func(10)\n10.0\n```",
    "created_at": "2009-06-04T23:35:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4454",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4454#issuecomment-35580",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:6'></a>This has been fixed by #5106:

```
----------------------------------------------------------------------
----------------------------------------------------------------------
sage: load time.sage
sage: func()
5.0
sage: func(10)
10.0
```



---

archive/issue_comments_035581.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2009-06-04T23:35:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4454",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4454#issuecomment-35581",
    "user": "https://github.com/mwhansen"
}
```

Resolution: invalid



---

archive/issue_events_010055.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2009-06-04T23:35:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4454",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4454#event-10055"
}
```
