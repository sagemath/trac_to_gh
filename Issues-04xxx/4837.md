...
<a id='comment:1'></a>OK, the attached patch implements the simple idea given above.  More precisely: let a be a primitive element of K, then every element of K can be written as a polynomial of degree (at most) [K:QQ]-1 in the variable a, with rational coefficients.  Therefore the function returns a random polynomial of this type.
...
```json
{
    "body": "<a id='comment:5'></a>I fully agree:  use the existing method for absolute fields (where there's essentially no need to find a primitive element), use the recursive method for relative extensions, and make a new ticket for random elements of orders (and fractional ideals while we're at it) which will use Z-bases.\n\nWill you be adapting the current patch accordingly?",
...
<a id='comment:5'></a>I fully agree:  use the existing method for absolute fields (where there's essentially no need to find a primitive element), use the recursive method for relative extensions, and make a new ticket for random elements of orders (and fractional ideals while we're at it) which will use Z-bases.
...
```json
{
    "body": "<a id='comment:6'></a>Good.  I will modify the patch to use this strategy and resubmit it.",
...
<a id='comment:6'></a>Good.  I will modify the patch to use this strategy and resubmit it.
...
<a id='comment:7'></a>Alex, I just noticed that you can do this in one line with 
...
<a id='comment:8'></a>Following the discussion at
http://groups.google.com/group/sage-devel/browse_thread/thread/67b40b64970bbf19
I implemented coercion of elements from the isomorphic polynomial quotient ring to the number field.  As a consequence of this and the code for random elements of quotient rings at #4218, random_element() for number fields became a fairly trivial one-liner.
...
<a id='comment:9'></a>The patch applies fine (over 3.2.2 + #4218).  Just one thing:  for an absolute extension we can create an element from a polynomial over the base field, but not for a relative extension:
...
```json
{
    "body": "<a id='comment:10'></a>I was hoping to be able to take care of this quickly, but I keep running into brick walls.  I agree that it should be fixed though, and I've opened a new ticket for it, see #4869.",
...
<a id='comment:10'></a>I was hoping to be able to take care of this quickly, but I keep running into brick walls.  I agree that it should be fixed though, and I've opened a new ticket for it, see #4869.
...
```json
{
    "body": "<a id='comment:11'></a>OK, given the new ticket I'm happy to give this the positive review it has been waiting for.  Sorry to be fussy, Alex!",
...
<a id='comment:11'></a>OK, given the new ticket I'm happy to give this the positive review it has been waiting for.  Sorry to be fussy, Alex!
...
```json
{
    "body": "<a id='comment:13'></a>Merged in Sage 3.3.alpha0",
...
<a id='comment:13'></a>Merged in Sage 3.3.alpha0
