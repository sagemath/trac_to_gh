# Issue 4054: [with patch, positive review] shorten doctesting in graph_generators.py

archive/issues_004054.json:
```json
{
    "body": "On my MacBook Pro, before this patch:\n\n```\nsage -t  devel/sage-main/sage/graphs/graph_generators.py    \n [117.4 s]\nsage -t -long devel/sage-main/sage/graphs/graph_generators.py\n [242.7 s]\n```\n\nAnd after this patch:\n\n```\nsage -t  devel/sage-main/sage/graphs/graph_generators.py    \n [20.7 s]\nsage -t -long devel/sage-main/sage/graphs/graph_generators.py\n [109.6 s]\n```\n\nAssignee: @rlmill\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4054\n\n",
    "closed_at": "2008-09-04T23:22:43Z",
    "created_at": "2008-09-03T22:53:32Z",
    "labels": [
        "component: algebra",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1.2",
    "title": "[with patch, positive review] shorten doctesting in graph_generators.py",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4054",
    "user": "https://github.com/rlmill"
}
```
On my MacBook Pro, before this patch:

```
sage -t  devel/sage-main/sage/graphs/graph_generators.py    
 [117.4 s]
sage -t -long devel/sage-main/sage/graphs/graph_generators.py
 [242.7 s]
```

And after this patch:

```
sage -t  devel/sage-main/sage/graphs/graph_generators.py    
 [20.7 s]
sage -t -long devel/sage-main/sage/graphs/graph_generators.py
 [109.6 s]
```

Assignee: @rlmill

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4054





---

archive/issue_comments_031616.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -16,8 +16,6 @@\n \t [86.9 s]\n ```\n \n-Assignee: tbd\n-\n Comment: 1\n \n Summary: [with patch, needs review] shorten doctesting in graph_generators.py\n``````\n",
    "created_at": "2008-09-03T22:56:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4054",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4054#issuecomment-31616",
    "user": "https://github.com/rlmill"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -16,8 +16,6 @@
 	 [86.9 s]
 ```
 
-Assignee: tbd
-
 Comment: 1
 
 Summary: [with patch, needs review] shorten doctesting in graph_generators.py
``````




---

archive/issue_comments_031617.json:
```json
{
    "body": "Changing assignee from tbd to @rlmill.",
    "created_at": "2008-09-03T23:25:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4054",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4054#issuecomment-31617",
    "user": "https://github.com/rlmill"
}
```

Changing assignee from tbd to @rlmill.



---

archive/issue_events_009257.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-04T00:12:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4054",
    "milestone": "sage-3.1.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4054#event-9257"
}
```



---

archive/issue_comments_031618.json:
```json
{
    "body": "<a id='comment:3'></a>Hi,\n\nI would make the \"not tested\" \"long\" since one day we will have a framework that compares expected with actual plotting output. Other than that positive review.\n\nCheers,\n\nMichael",
    "created_at": "2008-09-04T00:12:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4054",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4054#issuecomment-31618",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>Hi,

I would make the "not tested" "long" since one day we will have a framework that compares expected with actual plotting output. Other than that positive review.

Cheers,

Michael



---

archive/issue_comments_031619.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,20 @@\n+On my MacBook Pro, before this patch:\n \n+```\n+sage -t  devel/sage-main/sage/graphs/graph_generators.py    \n+\t [117.4 s]\n+sage -t -long devel/sage-main/sage/graphs/graph_generators.py\n+\t [242.7 s]\n+```\n+\n+And after this patch:\n+\n+```\n+sage -t  devel/sage-main/sage/graphs/graph_generators.py    \n+\t [20.7 s]\n+sage -t -long devel/sage-main/sage/graphs/graph_generators.py\n+\t [109.6 s]\n+```\n \n Comment: 1\n \n``````\n",
    "created_at": "2008-09-04T22:54:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4054",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4054#issuecomment-31619",
    "user": "https://github.com/rlmill"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,20 @@
+On my MacBook Pro, before this patch:
 
+```
+sage -t  devel/sage-main/sage/graphs/graph_generators.py    
+	 [117.4 s]
+sage -t -long devel/sage-main/sage/graphs/graph_generators.py
+	 [242.7 s]
+```
+
+And after this patch:
+
+```
+sage -t  devel/sage-main/sage/graphs/graph_generators.py    
+	 [20.7 s]
+sage -t -long devel/sage-main/sage/graphs/graph_generators.py
+	 [109.6 s]
+```
 
 Comment: 1
 
``````




---

archive/issue_comments_031620.json:
```json
{
    "body": "<a id='comment:4'></a>Attachment [trac_4054-long_docs.patch](tarball://root/attachments/some-uuid/ticket4054/trac_4054-long_docs.patch) by @rlmill created at 2008-09-04 22:54:41",
    "created_at": "2008-09-04T22:54:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4054",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4054#issuecomment-31620",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:4'></a>Attachment [trac_4054-long_docs.patch](tarball://root/attachments/some-uuid/ticket4054/trac_4054-long_docs.patch) by @rlmill created at 2008-09-04 22:54:41



---

archive/issue_comments_031621.json:
```json
{
    "body": "<a id='comment:5'></a>Positive review now that the \"not tested\" have been converted.\n\nCheers,\n\nMichael",
    "created_at": "2008-09-04T23:02:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4054",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4054#issuecomment-31621",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:5'></a>Positive review now that the "not tested" have been converted.

Cheers,

Michael



---

archive/issue_comments_031622.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-09-04T23:22:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4054",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4054#issuecomment-31622",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_031623.json:
```json
{
    "body": "<a id='comment:6'></a>Merged in Sage 3.1.2.rc0",
    "created_at": "2008-09-04T23:22:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4054",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4054#issuecomment-31623",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:6'></a>Merged in Sage 3.1.2.rc0



---

archive/issue_events_009258.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-09-04T23:22:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4054",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4054#event-9258"
}
```



---

archive/issue_comments_031624.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,20 @@\n+On my MacBook Pro, before this patch:\n \n+```\n+sage -t  devel/sage-main/sage/graphs/graph_generators.py    \n+ [117.4 s]\n+sage -t -long devel/sage-main/sage/graphs/graph_generators.py\n+ [242.7 s]\n+```\n+\n+And after this patch:\n+\n+```\n+sage -t  devel/sage-main/sage/graphs/graph_generators.py    \n+ [20.7 s]\n+sage -t -long devel/sage-main/sage/graphs/graph_generators.py\n+ [109.6 s]\n+```\n \n Comment: 1\n \n``````\n",
    "created_at": "2015-09-06T15:00:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4054",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4054#issuecomment-31624",
    "user": "https://github.com/fchapoton"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,20 @@
+On my MacBook Pro, before this patch:
 
+```
+sage -t  devel/sage-main/sage/graphs/graph_generators.py    
+ [117.4 s]
+sage -t -long devel/sage-main/sage/graphs/graph_generators.py
+ [242.7 s]
+```
+
+And after this patch:
+
+```
+sage -t  devel/sage-main/sage/graphs/graph_generators.py    
+ [20.7 s]
+sage -t -long devel/sage-main/sage/graphs/graph_generators.py
+ [109.6 s]
+```
 
 Comment: 1
 
``````




---

archive/issue_comments_031625.json:
```json
{
    "body": "Changing upstream from \"\" to \"N/A\"",
    "created_at": "2015-09-06T15:00:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4054",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4054#issuecomment-31625",
    "user": "https://github.com/fchapoton"
}
```

Changing upstream from "" to "N/A"
