# Issue 4660: [with patch; positive review] mark  inline fortran and inline cython examples optional

archive/issues_004660.json:
```json
{
    "body": "Since gcc and fortran compilers are both optional to run sage, mark doctests that use them \n\n```\n  # optional -- gcc\n```\nand\n\n```\n  # optional -- fortran\n```\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4660\n\n",
    "closed_at": "2008-11-30T08:24:01Z",
    "created_at": "2008-11-30T07:54:45Z",
    "labels": [
        "component: doctest coverage",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.2.1",
    "title": "[with patch; positive review] mark  inline fortran and inline cython examples optional",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4660",
    "user": "https://github.com/williamstein"
}
```
Since gcc and fortran compilers are both optional to run sage, mark doctests that use them 

```
  # optional -- gcc
```
and

```
  # optional -- fortran
```

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4660





---

archive/issue_comments_037212.json:
```json
{
    "body": "Attachment [trac_4660_sage.patch](tarball://root/attachments/some-uuid/ticket4660/trac_4660_sage.patch) by @williamstein created at 2008-11-30 08:01:34",
    "created_at": "2008-11-30T08:01:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4660",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4660#issuecomment-37212",
    "user": "https://github.com/williamstein"
}
```

Attachment [trac_4660_sage.patch](tarball://root/attachments/some-uuid/ticket4660/trac_4660_sage.patch) by @williamstein created at 2008-11-30 08:01:34



---

archive/issue_events_017740.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4660",
    "rename": {
        "from": "mark  inline fortran and inline cython examples optional",
        "to": "[with patch; needs review] mark  inline fortran and inline cython examples optional"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4660#event-17740"
}
```



---

archive/issue_comments_037213.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in Sage 3.2.1.rc1",
    "created_at": "2008-11-30T08:24:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4660",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4660#issuecomment-37213",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Merged in Sage 3.2.1.rc1



---

archive/issue_events_017741.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-30T08:24:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4660",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4660#event-17741"
}
```



---

archive/issue_comments_037214.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-11-30T08:24:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4660",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4660#issuecomment-37214",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_037215.json:
```json
{
    "body": "<a id='comment:3'></a>\nOops, forgot the positive review.\n\nCheers,\n\nMichael",
    "created_at": "2008-11-30T08:26:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4660",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4660#issuecomment-37215",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Oops, forgot the positive review.

Cheers,

Michael



---

archive/issue_events_017742.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4660",
    "rename": {
        "from": "[with patch; needs review] mark  inline fortran and inline cython examples optional",
        "to": "[with patch; positive review] mark  inline fortran and inline cython examples optional"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4660#event-17742"
}
```
