# Issue 4909: [with patch, needs little work] convert sage.dsage.* docstrings to Sphinx

archive/issues_004909.json:
```json
{
    "body": "\n\nAssignee: tba\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4909\n\n",
    "closed_at": "2009-02-24T18:33:39Z",
    "created_at": "2009-01-01T22:50:29Z",
    "labels": [
        "component: documentation",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.4",
    "title": "[with patch, needs little work] convert sage.dsage.* docstrings to Sphinx",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4909",
    "user": "https://github.com/mwhansen"
}
```


Assignee: tba

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4909





---

archive/issue_comments_039528.json:
```json
{
    "body": "Attachment [trac_4909.patch](tarball://root/attachments/some-uuid/ticket4909/trac_4909.patch) by @mwhansen created at 2009-01-02 02:25:34",
    "created_at": "2009-01-02T02:25:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4909#issuecomment-39528",
    "user": "https://github.com/mwhansen"
}
```

Attachment [trac_4909.patch](tarball://root/attachments/some-uuid/ticket4909/trac_4909.patch) by @mwhansen created at 2009-01-02 02:25:34



---

archive/issue_events_018957.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4909",
    "rename": {
        "from": "convert sage.dsage.* docstrings to Sphinx",
        "to": "[with patch, needs review] convert sage.dsage.* docstrings to Sphinx"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4909#event-18957"
}
```



---

archive/issue_comments_039529.json:
```json
{
    "body": "Attachment [sage.dsage-final.patch](tarball://root/attachments/some-uuid/ticket4909/sage.dsage-final.patch) by @mwhansen created at 2009-02-21 19:17:23",
    "created_at": "2009-02-21T19:17:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4909#issuecomment-39529",
    "user": "https://github.com/mwhansen"
}
```

Attachment [sage.dsage-final.patch](tarball://root/attachments/some-uuid/ticket4909/sage.dsage-final.patch) by @mwhansen created at 2009-02-21 19:17:23



---

archive/issue_events_018958.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4909",
    "rename": {
        "from": "[with patch, needs review] convert sage.dsage.* docstrings to Sphinx",
        "to": "[with patch, needs little work] convert sage.dsage.* docstrings to Sphinx"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4909#event-18958"
}
```



---

archive/issue_comments_039530.json:
```json
{
    "body": "<a id='comment:2'></a>\nA little problem:\n\n```\nNote that configuration files will be stored in the  \n    directory \\code{\\$DOT\\_SAGE/dsage}.\n```\nIs replaced now by\n\n```\nNote that configuration files will be stored in the directory \n``$DOT \nSage/dsage``. \n``` \nthe \"_\" must be kept. \n\nOtherwise this is correct. \n\nCheers,\n\nFlorent",
    "created_at": "2009-02-24T17:42:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4909#issuecomment-39530",
    "user": "https://github.com/hivert"
}
```

<a id='comment:2'></a>
A little problem:

```
Note that configuration files will be stored in the  
    directory \code{\$DOT\_SAGE/dsage}.
```
Is replaced now by

```
Note that configuration files will be stored in the directory 
``$DOT 
Sage/dsage``. 
``` 
the "_" must be kept. 

Otherwise this is correct. 

Cheers,

Florent



---

archive/issue_events_018959.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2009-02-24T18:33:39Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4909",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4909#event-18959"
}
```



---

archive/issue_comments_039531.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.4.alpha0.\n\nCheers,\n\nMichael",
    "created_at": "2009-02-24T18:33:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4909#issuecomment-39531",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.4.alpha0.

Cheers,

Michael



---

archive/issue_comments_039532.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2009-02-24T18:33:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4909#issuecomment-39532",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
