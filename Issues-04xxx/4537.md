# Issue 4537: [with patch, positive review] inverse_mod for number field ideals

archive/issues_004537.json:
```json
{
    "body": "The inverse_mod method currently isn't implemented for ideals in rings of integers of number fields. It should be, as it's not difficult.\n\nAssignee: @loefflerd\n\nResolution: fixed\n\nAuthor: David Loeffler\n\nReviewer: William Stein\n\nMerged: 3.2.1.rc0\n\nIssue created by migration from https://trac.sagemath.org/ticket/4537\n\n",
    "closed_at": "2008-11-28T08:36:03Z",
    "created_at": "2008-11-17T07:51:11Z",
    "labels": [
        "component: number theory"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.2.1",
    "title": "[with patch, positive review] inverse_mod for number field ideals",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4537",
    "user": "https://github.com/loefflerd"
}
```
The inverse_mod method currently isn't implemented for ideals in rings of integers of number fields. It should be, as it's not difficult.

Assignee: @loefflerd

Resolution: fixed

Author: David Loeffler

Reviewer: William Stein

Merged: 3.2.1.rc0

Issue created by migration from https://trac.sagemath.org/ticket/4537





---

archive/issue_comments_036651.json:
```json
{
    "body": "<a id='comment:1'></a>Attachment [4537-inversemod.patch](tarball://root/attachments/some-uuid/ticket4537/4537-inversemod.patch) by @loefflerd created at 2008-11-17 07:58:40\n\nI've uploaded a patch. Should work under 3.2.rc1 with the patch for #4536 installed (as it uses the coordinates() method of orders). \n\nI've implemented quadratic fields and general absolute fields; it would be trivial to implement for relative orders too, but there is so much general brokenness for relative orders that there doesn't seem much point (we don't even have is_integral() for relative orders yet).",
    "created_at": "2008-11-17T07:58:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4537",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4537#issuecomment-36651",
    "user": "https://github.com/loefflerd"
}
```

<a id='comment:1'></a>Attachment [4537-inversemod.patch](tarball://root/attachments/some-uuid/ticket4537/4537-inversemod.patch) by @loefflerd created at 2008-11-17 07:58:40

I've uploaded a patch. Should work under 3.2.rc1 with the patch for #4536 installed (as it uses the coordinates() method of orders). 

I've implemented quadratic fields and general absolute fields; it would be trivial to implement for relative orders too, but there is so much general brokenness for relative orders that there doesn't seem much point (we don't even have is_integral() for relative orders yet).



---

archive/issue_comments_036652.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-11-17T07:58:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4537",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4537#issuecomment-36652",
    "user": "https://github.com/loefflerd"
}
```

Changing status from new to assigned.



---

archive/issue_comments_036653.json:
```json
{
    "body": "<a id='comment:2'></a>I would find something slightly more general useful, and you could implement your functions via this one too:\n\nIf A and B are coprime integral ideals, express 1=a+b with a in A, b in B.  (cf Prop 1.3.1 of Cohen's GTM 193 which I know you are using already).  The more general version of Thm 1.3.3 (op cit) would also be useful.",
    "created_at": "2008-11-17T09:51:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4537",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4537#issuecomment-36653",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:2'></a>I would find something slightly more general useful, and you could implement your functions via this one too:

If A and B are coprime integral ideals, express 1=a+b with a in A, b in B.  (cf Prop 1.3.1 of Cohen's GTM 193 which I know you are using already).  The more general version of Thm 1.3.3 (op cit) would also be useful.



---

archive/issue_comments_036654.json:
```json
{
    "body": "<a id='comment:3'></a>Replying to [davidloeffler](#comment%3A1):\n> I've uploaded a patch. Should work under 3.2.rc1 with the patch for #4536 installed (as it uses the coordinates() method of orders). \n> \n> I've implemented quadratic fields and general absolute fields; it would be trivial to implement for relative orders too, but there is so much general brokenness for relative orders that there doesn't seem much point (we don't even have is_integral() for relative orders yet). \n\n\nNote that #4536 now has a second bug-fixing patch attached, though the coordinates() funciton is not affected.  John",
    "created_at": "2008-11-18T17:29:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4537",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4537#issuecomment-36654",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:3'></a>Replying to [davidloeffler](#comment%3A1):
> I've uploaded a patch. Should work under 3.2.rc1 with the patch for #4536 installed (as it uses the coordinates() method of orders). 
> 
> I've implemented quadratic fields and general absolute fields; it would be trivial to implement for relative orders too, but there is so much general brokenness for relative orders that there doesn't seem much point (we don't even have is_integral() for relative orders yet). 


Note that #4536 now has a second bug-fixing patch attached, though the coordinates() funciton is not affected.  John



---

archive/issue_comments_036655.json:
```json
{
    "body": "<a id='comment:4'></a>REFEREE REPORT:\n\nPositive review, since this is a very nice patch.  I agree with John that it would be nice to have something more general, but that can be for another patch later. \n\nThis is very nice.  Good to go.",
    "created_at": "2008-11-28T06:33:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4537",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4537#issuecomment-36655",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:4'></a>REFEREE REPORT:

Positive review, since this is a very nice patch.  I agree with John that it would be nice to have something more general, but that can be for another patch later. 

This is very nice.  Good to go.



---

archive/issue_comments_036656.json:
```json
{
    "body": "<a id='comment:5'></a>Merged in Sage 3.2.1.rc0",
    "created_at": "2008-11-28T08:36:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4537",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4537#issuecomment-36656",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:5'></a>Merged in Sage 3.2.1.rc0



---

archive/issue_comments_036657.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-11-28T08:36:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4537",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4537#issuecomment-36657",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_010323.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-28T08:36:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4537",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4537#event-10323"
}
```



---

archive/issue_comments_036658.json:
```json
{
    "body": "Changing merged from \"\" to \"3.2.1.rc0\"",
    "created_at": "2009-06-07T09:29:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4537",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4537#issuecomment-36658",
    "user": "https://github.com/loefflerd"
}
```

Changing merged from "" to "3.2.1.rc0"



---

archive/issue_comments_036659.json:
```json
{
    "body": "Changing author from \"\" to \"David Loeffler\"",
    "created_at": "2009-06-07T09:29:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4537",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4537#issuecomment-36659",
    "user": "https://github.com/loefflerd"
}
```

Changing author from "" to "David Loeffler"



---

archive/issue_comments_036660.json:
```json
{
    "body": "Changing reviewer from \"\" to \"William Stein\"",
    "created_at": "2009-06-07T09:29:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4537",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4537#issuecomment-36660",
    "user": "https://github.com/loefflerd"
}
```

Changing reviewer from "" to "William Stein"
