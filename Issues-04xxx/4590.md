...
<a id='comment:1'></a>Actually, the only files that get used in that function are these:
...
```json
{
    "body": "<a id='comment:2'></a>Either way is fine with me.  Looking at the code, it seems that mhansen is right.  I'd probably go with the patch so things are less confusing.",
...
<a id='comment:2'></a>Either way is fine with me.  Looking at the code, it seems that mhansen is right.  I'd probably go with the patch so things are less confusing.
