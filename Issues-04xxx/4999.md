# Issue 4999: Solaris 10/Sparc: numerical noise doctest failure in sage/gsl/integration.pyx

archive/issues_004999.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mabshoff"
    ],
    "body": "\n```\nsage -t  \"devel/sage/sage/gsl/integration.pyx\"              \n**********************************************************************\nFile \"/home/mabshoff/build-3.2.3/sage-3.2.3-mark/devel/sage/sage/gsl/integration.pyx\", line 163:\n    sage: numerical_integral(exp(-1/x), 1, 2)\nExpected:\n    (0.50479221787318407, 5.6043194293440744e-15)\nGot:\n    (0.50479221787318396, 5.6043194293440728e-15)\n**********************************************************************\n```\n\nComponent: **porting: Solaris**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4999_\n\n",
    "closed_at": "2009-01-18T14:06:54Z",
    "created_at": "2009-01-17T16:02:24Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20porting%3A%20solaris",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Solaris 10/Sparc: numerical noise doctest failure in sage/gsl/integration.pyx",
    "type": "issue",
    "updated_at": "2009-01-18T14:06:54Z",
    "url": "https://github.com/sagemath/sage/issues/4999",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

```
sage -t  "devel/sage/sage/gsl/integration.pyx"              
**********************************************************************
File "/home/mabshoff/build-3.2.3/sage-3.2.3-mark/devel/sage/sage/gsl/integration.pyx", line 163:
    sage: numerical_integral(exp(-1/x), 1, 2)
Expected:
    (0.50479221787318407, 5.6043194293440744e-15)
Got:
    (0.50479221787318396, 5.6043194293440728e-15)
**********************************************************************
```

Component: **porting: Solaris**

_Issue created by migration from https://trac.sagemath.org/ticket/4999_





---

archive/issue_events_058024.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-17T16:02:24Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4999",
    "milestone_number": null,
    "milestone_title": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4999#event-58024"
}
```



---

archive/issue_events_058025.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-17T16:02:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4999",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20porting%3A%20solaris",
    "label_color": "000080",
    "label_name": "c: porting: solaris",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4999#event-58025"
}
```



---

archive/issue_events_058026.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-17T16:02:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4999",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4999#event-58026"
}
```



---

archive/issue_events_058027.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-17T16:02:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4999",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4999#event-58027"
}
```



---

archive/issue_events_058028.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-17T16:02:24Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4999",
    "subject": "https://github.com/sagetrac-mabshoff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4999#event-58028"
}
```



---

archive/issue_comments_030444.json:
```json
{
    "body": "Attachment: **[trac_4999_integration.pyx-sparc.patch.gz](https://github.com/sagemath/sage/files/ticket4999/trac_4999_integration.pyx-sparc.patch.gz)**",
    "created_at": "2009-01-18T06:30:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4999",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4999#issuecomment-30444",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

Attachment: **[trac_4999_integration.pyx-sparc.patch.gz](https://github.com/sagemath/sage/files/ticket4999/trac_4999_integration.pyx-sparc.patch.gz)**



---

archive/issue_events_058029.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-18T06:30:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4999",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4999#event-58029"
}
```



---

archive/issue_events_058030.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2009-01-18T06:32:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4999",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4999#event-58030"
}
```



---

archive/issue_events_058031.json:
```json
{
    "actor": "https://github.com/craigcitro",
    "created_at": "2009-01-18T06:32:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4999",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4999#event-58031"
}
```



---

archive/issue_comments_030445.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nMerged in Sage 3.3.alpha0",
    "created_at": "2009-01-18T14:06:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4999",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4999#issuecomment-30445",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:4" align="right">comment:4</div>

Merged in Sage 3.3.alpha0



---

archive/issue_events_058032.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-18T14:06:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4999",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4999#event-58032"
}
```



---

archive/issue_events_058033.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2009-01-18T14:06:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4999",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4999#event-58033"
}
```
