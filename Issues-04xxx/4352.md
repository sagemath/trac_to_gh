# Issue 4352: add support for weight vectors to gran/groebner_fan

archive/issues_004352.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-mhampton"
    ],
    "body": "This should be pretty straightforward.\n\nsage-support request from Ursula Whitcher:\nI asked Anders Jensen, \"I would like to compute the weight vectors\ncorresponding to each reduced Groebner basis in gfan's output.  Is\nthere a way to tell gfan to do this?\"\n\nHe replied:\n\n\"There is a command \"weightvector\" that does exactly this. The command\nis hidden (does not show up in the manual or the file system). To run\nit type \"gfan _weightvector\" in your shell. MIND THE SPACE BEFORE '_'.\nAccording to the --help text the correct thing to do would be to run\n\"gfan _weightvector -m\" with the gfan output as input.\n\nFor example\ngfan | gfan _weightvector -m\nQ[x,y]\n{x-y}\n\nwill produce a list of two vectors.\nI hope this works out for you.\nBest regards,\nAnders\"\n\nIs there a way to access the weightvector command from the Sage\nimplementation of gfan?\n\nThanks!\nUrsula \n\nKeywords: **gfan, groebner_fan, weight vectors**\n\nComponent: **interfaces**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/4352_\n\n",
    "closed_at": "2008-11-30T23:11:55Z",
    "created_at": "2008-10-23T21:23:08Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20interfaces",
        "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.2.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "add support for weight vectors to gran/groebner_fan",
    "type": "issue",
    "updated_at": "2008-11-30T23:11:55Z",
    "url": "https://github.com/sagemath/sage/issues/4352",
    "user": "https://github.com/sagetrac-mhampton"
}
```
This should be pretty straightforward.

sage-support request from Ursula Whitcher:
I asked Anders Jensen, "I would like to compute the weight vectors
corresponding to each reduced Groebner basis in gfan's output.  Is
there a way to tell gfan to do this?"

He replied:

"There is a command "weightvector" that does exactly this. The command
is hidden (does not show up in the manual or the file system). To run
it type "gfan _weightvector" in your shell. MIND THE SPACE BEFORE '_'.
According to the --help text the correct thing to do would be to run
"gfan _weightvector -m" with the gfan output as input.

For example
gfan | gfan _weightvector -m
Q[x,y]
{x-y}

will produce a list of two vectors.
I hope this works out for you.
Best regards,
Anders"

Is there a way to access the weightvector command from the Sage
implementation of gfan?

Thanks!
Ursula 

Keywords: **gfan, groebner_fan, weight vectors**

Component: **interfaces**

_Issue created by migration from https://trac.sagemath.org/ticket/4352_





---

archive/issue_events_049075.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2008-10-23T21:23:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4352",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "c: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4352#event-49075"
}
```



---

archive/issue_events_049076.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2008-10-23T21:23:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4352",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4352#event-49076"
}
```



---

archive/issue_events_049077.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2008-10-23T21:23:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4352",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4352#event-49077"
}
```



---

archive/issue_events_049078.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2008-10-23T21:23:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4352",
    "label": "https://github.com/sagemath/sage/labels/feature",
    "label_color": "008080",
    "label_name": "feature",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4352#event-49078"
}
```



---

archive/issue_events_049079.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2008-10-23T21:23:08Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/4352",
    "subject": "https://github.com/sagetrac-mhampton",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4352#event-49079"
}
```



---

archive/issue_comments_025176.json:
```json
{
    "body": "Attachment: **[4352_1.patch.gz](https://github.com/sagemath/sage/files/ticket4352/4352_1.patch.gz)**\n\nbased on 3.1.4 but should be fine against 3.2-alpha0",
    "created_at": "2008-10-24T02:16:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4352#issuecomment-25176",
    "user": "https://github.com/sagetrac-mhampton"
}
```

Attachment: **[4352_1.patch.gz](https://github.com/sagemath/sage/files/ticket4352/4352_1.patch.gz)**

based on 3.1.4 but should be fine against 3.2-alpha0



---

archive/issue_events_049080.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2008-10-24T02:17:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4352",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4352#event-49080"
}
```



---

archive/issue_events_049081.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-29T02:49:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4352",
    "milestone_number": null,
    "milestone_title": "sage-3.2.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4352#event-49081"
}
```



---

archive/issue_events_049082.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-29T02:49:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4352",
    "label": "https://github.com/sagemath/sage/labels/feature",
    "label_color": "008080",
    "label_name": "feature",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4352#event-49082"
}
```



---

archive/issue_events_049083.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-29T02:49:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4352",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4352#event-49083"
}
```



---

archive/issue_events_049084.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-29T02:49:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4352",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4352#event-49084"
}
```



---

archive/issue_comments_025177.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nREVIEW:\n\npatch applies and passes test.  Code raises major red flag!!\n\n```\n\t        ans = eval(ans.replace('{','').replace('}','').replace('\\n','')) \n```\n\nIf the output -- which you make into vectors over QQ ever actually has any rational numbers, then eval will do very bad things to them, e.g., \n\n```\nsage: eval('2/3')\n0\n```\nOops!\n\nUse sage_eval instead:\n\n```\nsage: sage_eval('2/3')\n2/3\n```",
    "created_at": "2008-11-29T02:49:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4352#issuecomment-25177",
    "user": "https://github.com/williamstein"
}
```

<div id="comment:2" align="right">comment:2</div>

REVIEW:

patch applies and passes test.  Code raises major red flag!!

```
	        ans = eval(ans.replace('{','').replace('}','').replace('\n','')) 
```

If the output -- which you make into vectors over QQ ever actually has any rational numbers, then eval will do very bad things to them, e.g., 

```
sage: eval('2/3')
0
```
Oops!

Use sage_eval instead:

```
sage: sage_eval('2/3')
2/3
```



---

archive/issue_comments_025178.json:
```json
{
    "body": "Attachment: **[4352_2.patch.gz](https://github.com/sagemath/sage/files/ticket4352/4352_2.patch.gz)**\n\nsupercedes previous patch, addresses review",
    "created_at": "2008-11-29T14:00:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4352#issuecomment-25178",
    "user": "https://github.com/sagetrac-mhampton"
}
```

Attachment: **[4352_2.patch.gz](https://github.com/sagemath/sage/files/ticket4352/4352_2.patch.gz)**

supercedes previous patch, addresses review



---

archive/issue_events_049085.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2008-11-29T14:01:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4352",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4352#event-49085"
}
```



---

archive/issue_events_049086.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2008-11-29T14:01:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4352",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4352#event-49086"
}
```



---

archive/issue_events_049087.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-29T21:31:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4352",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4352#event-49087"
}
```



---

archive/issue_events_049088.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-29T21:31:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4352",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4352#event-49088"
}
```



---

archive/issue_comments_025179.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nThis patch no longer applies cleanly to my 3.2.1.rc1 merge tree:\n\n```\nsage-3.2.1.rc1/devel/sage$ patch -p1 --dry-run < trac_4352_2.patch \npatching file sage/rings/polynomial/groebner_fan.py\nHunk #2 FAILED at 76.\n1 out of 3 hunks FAILED -- saving rejects to file sage/rings/polynomial/groebner_fan.py.rej\n```\nI gather from the patch description that only trac_4352_2.patch should be applied. So unless I am mistaken please rebase this. If there are unknown dependencies for this ticket please list them.\n\nCheers,\n\nMichael",
    "created_at": "2008-11-30T06:41:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4352#issuecomment-25179",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:5" align="right">comment:5</div>

This patch no longer applies cleanly to my 3.2.1.rc1 merge tree:

```
sage-3.2.1.rc1/devel/sage$ patch -p1 --dry-run < trac_4352_2.patch 
patching file sage/rings/polynomial/groebner_fan.py
Hunk #2 FAILED at 76.
1 out of 3 hunks FAILED -- saving rejects to file sage/rings/polynomial/groebner_fan.py.rej
```
I gather from the patch description that only trac_4352_2.patch should be applied. So unless I am mistaken please rebase this. If there are unknown dependencies for this ticket please list them.

Cheers,

Michael



---

archive/issue_events_049089.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-30T06:41:32Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4352",
    "title_is": "[needs rebase] add support for weight vectors to gran/groebner_fan",
    "title_was": "add support for weight vectors to gran/groebner_fan",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4352#event-49089"
}
```



---

archive/issue_comments_025180.json:
```json
{
    "body": "Attachment: **[4352_3.patch.gz](https://github.com/sagemath/sage/files/ticket4352/4352_3.patch.gz)**\n\nrebased against 3.2.1.rc0",
    "created_at": "2008-11-30T14:05:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4352#issuecomment-25180",
    "user": "https://github.com/sagetrac-mhampton"
}
```

Attachment: **[4352_3.patch.gz](https://github.com/sagemath/sage/files/ticket4352/4352_3.patch.gz)**

rebased against 3.2.1.rc0



---

archive/issue_events_049090.json:
```json
{
    "actor": "https://github.com/sagetrac-mhampton",
    "created_at": "2008-11-30T14:13:12Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4352",
    "title_is": "[rebased] add support for weight vectors to gran/groebner_fan",
    "title_was": "[needs rebase] add support for weight vectors to gran/groebner_fan",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4352#event-49090"
}
```



---

archive/issue_comments_025181.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nMerged 4352_3.patch in Sage 3.2.1.rc1",
    "created_at": "2008-11-30T23:11:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/4352",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4352#issuecomment-25181",
    "user": "https://github.com/sagetrac-mabshoff"
}
```

<div id="comment:7" align="right">comment:7</div>

Merged 4352_3.patch in Sage 3.2.1.rc1



---

archive/issue_events_049091.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-30T23:11:55Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/4352",
    "title_is": "add support for weight vectors to gran/groebner_fan",
    "title_was": "[rebased] add support for weight vectors to gran/groebner_fan",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4352#event-49091"
}
```



---

archive/issue_events_049092.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-30T23:11:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4352",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4352#event-49092"
}
```



---

archive/issue_events_049093.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-30T23:11:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4352",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4352#event-49093"
}
```



---

archive/issue_events_049094.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-30T23:11:55Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4352",
    "milestone_number": null,
    "milestone_title": "sage-3.2.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4352#event-49094"
}
```



---

archive/issue_events_049095.json:
```json
{
    "actor": "https://github.com/sagetrac-mabshoff",
    "created_at": "2008-11-30T23:11:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4352",
    "milestone_number": null,
    "milestone_title": "sage-3.2.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4352#event-49095"
}
```
