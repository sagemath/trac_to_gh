# Issue 4624: Sage 3.2.1.a1: add ipy_profile_sage.py to list of files copied when sdisting

archive/issues_004624.json:
```json
{
    "body": "Sigh:\n\n```\nsage-3.2.1.alpha2/spkg/standard/sage_scripts-3.2.1.alpha1$ hg stat\n! ipy_profile_sage.py\n```\n\n**Assignee:** mabshoff\n\n**CC:**  @mwhansen\n\nIssue created by migration from https://trac.sagemath.org/ticket/4624\n\n",
    "closed_at": "2008-11-27T02:06:45Z",
    "created_at": "2008-11-26T14:56:24Z",
    "labels": [
        "component: distribution",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-3.2.1",
    "title": "Sage 3.2.1.a1: add ipy_profile_sage.py to list of files copied when sdisting",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/4624",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
Sigh:

```
sage-3.2.1.alpha2/spkg/standard/sage_scripts-3.2.1.alpha1$ hg stat
! ipy_profile_sage.py
```

**Assignee:** mabshoff

**CC:**  @mwhansen

Issue created by migration from https://trac.sagemath.org/ticket/4624





---

archive/issue_events_027573.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-26T15:02:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4624",
    "label": "blocker",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4624#event-27573"
}
```



---

archive/issue_events_027574.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-26T15:02:34Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/4624",
    "milestone": "sage-3.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4624#event-27574"
}
```



---

archive/issue_events_027575.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-26T15:02:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/4624",
    "milestone": "sage-3.2.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4624#event-27575"
}
```



---

archive/issue_comments_028691.json:
```json
{
    "body": "<a id='comment:1'></a>\nOne way to fix this is to rename the file sage-ipy_profile.py which is likely a lot less pain long term.\n\nMike: any thoughts?\n\nCheers,\n\nMichael",
    "created_at": "2008-11-26T15:02:34Z",
    "issue": "https://github.com/sagemath/sage/issues/4624",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4624#issuecomment-28691",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:1'></a>
One way to fix this is to rename the file sage-ipy_profile.py which is likely a lot less pain long term.

Mike: any thoughts?

Cheers,

Michael



---

archive/issue_comments_028692.json:
```json
{
    "body": "<a id='comment:2'></a>\nThis doesn't work because ipython wants it explictly named that way for looking up the profile.",
    "created_at": "2008-11-26T15:09:58Z",
    "issue": "https://github.com/sagemath/sage/issues/4624",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4624#issuecomment-28692",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:2'></a>
This doesn't work because ipython wants it explictly named that way for looking up the profile.



---

archive/issue_comments_028693.json:
```json
{
    "body": "<a id='comment:3'></a>\nReplying to [mhansen](#comment%3A2):\n> This doesn't work because ipython wants it explictly named that way for looking up the profile.\n\n\nYep, I looked at the file and I came to the same conclusion. I have \"fixed\" the issue by correcting the sage_scripts repo in the 3.2.1.a1 tarball manually for now, but will take care of this once I catch some sleep.\n\nCheers,\n\nMichael",
    "created_at": "2008-11-26T15:15:45Z",
    "issue": "https://github.com/sagemath/sage/issues/4624",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4624#issuecomment-28693",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Replying to [mhansen](#comment%3A2):
> This doesn't work because ipython wants it explictly named that way for looking up the profile.


Yep, I looked at the file and I came to the same conclusion. I have "fixed" the issue by correcting the sage_scripts repo in the 3.2.1.a1 tarball manually for now, but will take care of this once I catch some sleep.

Cheers,

Michael



---

archive/attachments_005119.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "scripts-4624.patch",
    "asset_url": "tarball://root/attachments/ticket4624/scripts-4624.patch",
    "created_at": "2008-11-27T01:35:10Z",
    "issue": "https://github.com/sagemath/sage/issues/4624",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket4624/scripts-4624.patch",
    "user": "https://github.com/williamstein"
}
```



---

archive/issue_comments_028694.json:
```json
{
    "body": "Attachment [scripts-4624.patch](https://github.com/sagemath/sage/files/ticket4624/scripts-4624.patch) by @williamstein created at 2008-11-27 01:35:10",
    "created_at": "2008-11-27T01:35:10Z",
    "issue": "https://github.com/sagemath/sage/issues/4624",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4624#issuecomment-28694",
    "user": "https://github.com/williamstein"
}
```

Attachment [scripts-4624.patch](https://github.com/sagemath/sage/files/ticket4624/scripts-4624.patch) by @williamstein created at 2008-11-27 01:35:10



---

archive/issue_events_027576.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2008-11-27T01:35:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4624",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4624#event-27576"
}
```



---

archive/issue_events_027577.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-27T01:41:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4624",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4624#event-27577"
}
```



---

archive/issue_events_027578.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-27T01:41:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/4624",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4624#event-27578"
}
```



---

archive/attachments_005120.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "scripts-4624-part2.patch",
    "asset_url": "tarball://root/attachments/ticket4624/scripts-4624-part2.patch",
    "created_at": "2008-11-27T01:41:24Z",
    "issue": "https://github.com/sagemath/sage/issues/4624",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket4624/scripts-4624-part2.patch",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```



---

archive/issue_comments_028695.json:
```json
{
    "body": "<a id='comment:5'></a>\nAttachment [scripts-4624-part2.patch](https://github.com/sagemath/sage/files/ticket4624/scripts-4624-part2.patch) by mabshoff created at 2008-11-27 01:41:24\n\nWith the second patch this is good to go.\n\nCheers,\n\nMichael",
    "created_at": "2008-11-27T01:41:24Z",
    "issue": "https://github.com/sagemath/sage/issues/4624",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4624#issuecomment-28695",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:5'></a>
Attachment [scripts-4624-part2.patch](https://github.com/sagemath/sage/files/ticket4624/scripts-4624-part2.patch) by mabshoff created at 2008-11-27 01:41:24

With the second patch this is good to go.

Cheers,

Michael



---

archive/issue_events_027579.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-27T02:06:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/4624",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4624#event-27579"
}
```



---

archive/issue_events_027580.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-27T02:06:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/4624",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/4624#event-27580"
}
```



---

archive/issue_comments_028696.json:
```json
{
    "body": "<a id='comment:6'></a>\nMerged both patches in Sage 3.2.1.alpha2",
    "created_at": "2008-11-27T02:06:45Z",
    "issue": "https://github.com/sagemath/sage/issues/4624",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/4624#issuecomment-28696",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:6'></a>
Merged both patches in Sage 3.2.1.alpha2
