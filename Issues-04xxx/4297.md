# Issue 4297: [with patch, positive review] make sage -cython work on .spyx and library files

archive/issues_004297.json:
```json
{
    "body": "This adds the path information, and is handy for invoking cython directly (e.g. to produce an annotation). \n\nAssignee: mabshoff\n\nCC:  @rlmill\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4297\n\n",
    "closed_at": "2008-10-18T20:31:28Z",
    "created_at": "2008-10-15T14:41:05Z",
    "labels": [
        "component: packages: standard"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.2",
    "title": "[with patch, positive review] make sage -cython work on .spyx and library files",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4297",
    "user": "https://github.com/robertwb"
}
```
This adds the path information, and is handy for invoking cython directly (e.g. to produce an annotation). 

Assignee: mabshoff

CC:  @rlmill

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4297





---

archive/issue_events_016068.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4297",
    "rename": {
        "from": "[with patch, needs review] make sage -cython work on .spyx and library files",
        "to": "[with patch, positive review] make sage -cython work on .spyx and library files"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4297#event-16068"
}
```



---

archive/issue_comments_033171.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [4297-sage-cython.patch](tarball://root/attachments/some-uuid/ticket4297/4297-sage-cython.patch) by @rlmill created at 2008-10-15 14:50:44\n\nMakes annotating the Sage library much easier. Good work!",
    "created_at": "2008-10-15T14:50:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4297",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4297#issuecomment-33171",
    "user": "https://github.com/rlmill"
}
```

<a id='comment:1'></a>
Attachment [4297-sage-cython.patch](tarball://root/attachments/some-uuid/ticket4297/4297-sage-cython.patch) by @rlmill created at 2008-10-15 14:50:44

Makes annotating the Sage library much easier. Good work!



---

archive/issue_comments_033172.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-10-18T20:31:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4297",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4297#issuecomment-33172",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_events_016069.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-10-18T20:31:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4297",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4297#event-16069"
}
```



---

archive/issue_comments_033173.json:
```json
{
    "body": "<a id='comment:2'></a>\nMerged in Sage 3.2.alpha0",
    "created_at": "2008-10-18T20:31:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4297",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4297#issuecomment-33173",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Merged in Sage 3.2.alpha0
