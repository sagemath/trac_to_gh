# Issue 4006: [with patch, positive review] Remove unused code in sage/libs/pari/functional.py

archive/issues_004006.json:
```json
{
    "body": "\n\nAssignee: @williamstein\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4006\n\n",
    "closed_at": "2008-08-30T22:46:20Z",
    "created_at": "2008-08-30T19:05:49Z",
    "labels": [
        "component: interfaces",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.1.2",
    "title": "[with patch, positive review] Remove unused code in sage/libs/pari/functional.py",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4006",
    "user": "https://github.com/mwhansen"
}
```


Assignee: @williamstein

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4006





---

archive/issue_comments_030513.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [trac_4006.patch](tarball://root/attachments/some-uuid/ticket4006/trac_4006.patch) by @mwhansen created at 2008-08-30 20:01:45\n\nThis code is totally broken due to the imports at the top.  Plus, it's old, untested, and a bit crufty.",
    "created_at": "2008-08-30T20:01:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4006#issuecomment-30513",
    "user": "https://github.com/mwhansen"
}
```

<a id='comment:1'></a>
Attachment [trac_4006.patch](tarball://root/attachments/some-uuid/ticket4006/trac_4006.patch) by @mwhansen created at 2008-08-30 20:01:45

This code is totally broken due to the imports at the top.  Plus, it's old, untested, and a bit crufty.



---

archive/issue_events_014947.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4006",
    "rename": {
        "from": "[with patch, needs review] Remove unused code in sage/libs/pari/functional.py",
        "to": "[with patch, positive review] Remove unused code in sage/libs/pari/functional.py"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4006#event-14947"
}
```



---

archive/issue_comments_030514.json:
```json
{
    "body": "<a id='comment:2'></a>\nReplying to [mhansen](#comment%3A1):\n> This code is totally broken due to the imports at the top.  Plus, it's old, untested, and a bit crufty.\n\n\n+1. Should anybody want this code he/she has to resubmit it. It has been broken since at least Sage 0.10.12, so good riddance. Positive review.\n\nCheers,\n\nMichael",
    "created_at": "2008-08-30T22:24:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4006#issuecomment-30514",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Replying to [mhansen](#comment%3A1):
> This code is totally broken due to the imports at the top.  Plus, it's old, untested, and a bit crufty.


+1. Should anybody want this code he/she has to resubmit it. It has been broken since at least Sage 0.10.12, so good riddance. Positive review.

Cheers,

Michael



---

archive/issue_events_014948.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-08-30T22:46:20Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4006",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4006#event-14948"
}
```



---

archive/issue_comments_030515.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-08-30T22:46:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4006#issuecomment-30515",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed



---

archive/issue_comments_030516.json:
```json
{
    "body": "<a id='comment:3'></a>\nMerged in Sage 3.1.2.alpha3",
    "created_at": "2008-08-30T22:46:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4006#issuecomment-30516",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:3'></a>
Merged in Sage 3.1.2.alpha3
