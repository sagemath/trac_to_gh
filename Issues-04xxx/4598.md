# Issue 4598: [with patch, positive review] add sage/libs/gmp/__init__.py to MANIFEST.in

archive/issues_004598.json:
```json
{
    "body": "This causes build failures of the Sage library in Sage 3.2.1.alpha0.\n\nCheers,\n\nMichael\n\nAssignee: mabshoff\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/4598\n\n",
    "closed_at": "2008-11-25T13:46:13Z",
    "created_at": "2008-11-23T21:20:17Z",
    "labels": [
        "component: build",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-3.2.1",
    "title": "[with patch, positive review] add sage/libs/gmp/__init__.py to MANIFEST.in",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4598",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```
This causes build failures of the Sage library in Sage 3.2.1.alpha0.

Cheers,

Michael

Assignee: mabshoff

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/4598





---

archive/issue_comments_036509.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-11-23T21:20:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4598",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4598#issuecomment-36509",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Changing status from new to assigned.



---

archive/issue_events_017439.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4598",
    "rename": {
        "from": "add sage/libs/gmp/__init__.py to MANIFEST.in",
        "to": "[with patch, needs review] add sage/libs/gmp/__init__.py to MANIFEST.in"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4598#event-17439"
}
```



---

archive/issue_comments_036510.json:
```json
{
    "body": "<a id='comment:2'></a>\nWithout the patch:\n\n```\nmabshoff@sage:/scratch/mabshoff/release-cycle/sage-3.2.1.alpha0/spkg/standard/sage-3.2.1.alpha0$ hg status\n! sage/libs/gmp/__init__.py\n```\nWith the patch applied:\n\n```\nmabshoff@sage:/scratch/mabshoff/release-cycle/sage-3.2.1.alpha0/spkg/standard/sage-3.2.1.alpha00$ hg stat\nmabshoff@sage:/scratch/mabshoff/release-cycle/sage-3.2.1.alpha0/spkg/standard/sage-3.2.1.alpha00$ \n```",
    "created_at": "2008-11-25T11:11:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4598",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4598#issuecomment-36510",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:2'></a>
Without the patch:

```
mabshoff@sage:/scratch/mabshoff/release-cycle/sage-3.2.1.alpha0/spkg/standard/sage-3.2.1.alpha0$ hg status
! sage/libs/gmp/__init__.py
```
With the patch applied:

```
mabshoff@sage:/scratch/mabshoff/release-cycle/sage-3.2.1.alpha0/spkg/standard/sage-3.2.1.alpha00$ hg stat
mabshoff@sage:/scratch/mabshoff/release-cycle/sage-3.2.1.alpha0/spkg/standard/sage-3.2.1.alpha00$ 
```



---

archive/issue_comments_036511.json:
```json
{
    "body": "Attachment [trac_4598.patch](tarball://root/attachments/some-uuid/ticket4598/trac_4598.patch) by mabshoff created at 2008-11-25 11:12:20",
    "created_at": "2008-11-25T11:12:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4598",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4598#issuecomment-36511",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Attachment [trac_4598.patch](tarball://root/attachments/some-uuid/ticket4598/trac_4598.patch) by mabshoff created at 2008-11-25 11:12:20



---

archive/issue_comments_036512.json:
```json
{
    "body": "<a id='comment:3'></a>\nLooks good to me. Thanks!",
    "created_at": "2008-11-25T13:44:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4598",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4598#issuecomment-36512",
    "user": "https://github.com/certik"
}
```

<a id='comment:3'></a>
Looks good to me. Thanks!



---

archive/issue_events_017440.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4598",
    "rename": {
        "from": "[with patch, needs review] add sage/libs/gmp/__init__.py to MANIFEST.in",
        "to": "[with patch, reviewed] add sage/libs/gmp/__init__.py to MANIFEST.in"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4598#event-17440"
}
```



---

archive/issue_events_017441.json:
```json
{
    "event": "renamed",
    "issue": "https://github.com/sagemath/sagetest/issues/4598",
    "rename": {
        "from": "[with patch, reviewed] add sage/libs/gmp/__init__.py to MANIFEST.in",
        "to": "[with patch, positive review] add sage/libs/gmp/__init__.py to MANIFEST.in"
    },
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4598#event-17441"
}
```



---

archive/issue_events_017442.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/mabshoff",
    "created_at": "2008-11-25T13:46:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/4598",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4598#event-17442"
}
```



---

archive/issue_comments_036513.json:
```json
{
    "body": "<a id='comment:5'></a>\nMerged in Sage 3.2.1.alpha1",
    "created_at": "2008-11-25T13:46:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4598",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4598#issuecomment-36513",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

<a id='comment:5'></a>
Merged in Sage 3.2.1.alpha1



---

archive/issue_comments_036514.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2008-11-25T13:46:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4598",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4598#issuecomment-36514",
    "user": "https://trac.sagemath.org/admin/accounts/users/mabshoff"
}
```

Resolution: fixed
