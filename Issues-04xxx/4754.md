# Issue 4754: Merge minimum rank code

archive/issues_004754.json:
```json
{
    "body": "Based on the discussion at http://groups.google.com/group/sage-support/browse_thread/thread/3ec4cc026e9c65bd, it would be great to merge the code found at http://arxiv.org/abs/0812.1616 into the Sage library.  Several functions should probably go into the main graph library (e.g., the edge clique cover function), while others probably ought to go into a minimum_rank.sage file.\n\nI am one of the developers and hereby give my permission to incorporate the code into Sage.  I will ask the other developers as well.\n\nAssignee: @jasongrout\n\nCC:  mvngu\n\nStatus: needs_work\n\nIssue created by migration from https://trac.sagemath.org/ticket/4754\n\n",
    "created_at": "2008-12-11T04:15:52Z",
    "labels": [
        "component: graph theory"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.4",
    "title": "Merge minimum rank code",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/4754",
    "user": "https://github.com/jasongrout"
}
```
Based on the discussion at http://groups.google.com/group/sage-support/browse_thread/thread/3ec4cc026e9c65bd, it would be great to merge the code found at http://arxiv.org/abs/0812.1616 into the Sage library.  Several functions should probably go into the main graph library (e.g., the edge clique cover function), while others probably ought to go into a minimum_rank.sage file.

I am one of the developers and hereby give my permission to incorporate the code into Sage.  I will ask the other developers as well.

Assignee: @jasongrout

CC:  mvngu

Status: needs_work

Issue created by migration from https://trac.sagemath.org/ticket/4754





---

archive/issue_comments_039092.json:
```json
{
    "body": "Changing status from new to assigned.",
    "created_at": "2008-12-11T04:15:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4754#issuecomment-39092",
    "user": "https://github.com/jasongrout"
}
```

Changing status from new to assigned.



---

archive/issue_comments_039093.json:
```json
{
    "body": "Changing assignee from @rlmill to @jasongrout.",
    "created_at": "2008-12-11T04:15:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4754#issuecomment-39093",
    "user": "https://github.com/jasongrout"
}
```

Changing assignee from @rlmill to @jasongrout.



---

archive/issue_comments_039094.json:
```json
{
    "body": "<a id='comment:2'></a>I also have a *much* faster Cython zero-forcing-set tester that I could donate to Sage.",
    "created_at": "2008-12-11T04:19:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4754#issuecomment-39094",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:2'></a>I also have a *much* faster Cython zero-forcing-set tester that I could donate to Sage.



---

archive/issue_comments_039095.json:
```json
{
    "body": "<a id='comment:3'></a>> while others probably ought to go into a minimum_rank.sage file. \n\n\nI don't think anything included in the core main sage library should go in a .sage file.  It should go in a .py file.\n\nWilliam",
    "created_at": "2008-12-11T04:55:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4754#issuecomment-39095",
    "user": "https://github.com/williamstein"
}
```

<a id='comment:3'></a>> while others probably ought to go into a minimum_rank.sage file. 


I don't think anything included in the core main sage library should go in a .sage file.  It should go in a .py file.

William



---

archive/issue_comments_039096.json:
```json
{
    "body": "<a id='comment:4'></a>From personal email from Geoff Tims, 13 Dec 2008:\n\n```\nHey Jason,\n \nI don't know exactly how I feel.  I'm probably fine with it.\n \nI agree to license the code available from http://arxiv.org/abs/0812.1616 as GPL version 2 or later\n \nGeoff\n```",
    "created_at": "2008-12-22T18:41:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4754#issuecomment-39096",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:4'></a>From personal email from Geoff Tims, 13 Dec 2008:

```
Hey Jason,
 
I don't know exactly how I feel.  I'm probably fine with it.
 
I agree to license the code available from http://arxiv.org/abs/0812.1616 as GPL version 2 or later
 
Geoff
```



---

archive/issue_comments_039097.json:
```json
{
    "body": "<a id='comment:5'></a>From personal email from Tracy McKay, 11 Dec 2008:\n\n```\nHi Jason,\n\nI agree to license the code available from http://arxiv.org/abs/0812.1616 as GPL version 2 or later. \n\nThanks,\n\nTracy McKay\n```",
    "created_at": "2008-12-22T18:42:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4754#issuecomment-39097",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:5'></a>From personal email from Tracy McKay, 11 Dec 2008:

```
Hi Jason,

I agree to license the code available from http://arxiv.org/abs/0812.1616 as GPL version 2 or later. 

Thanks,

Tracy McKay
```



---

archive/issue_comments_039098.json:
```json
{
    "body": "<a id='comment:6'></a>From personal email from Laura DeLoss, 11 Dec 2008:\n\n```\nI agree to license the code available from http://arxiv.org/abs/0812.1616 as GPL version 2 or later and for the EGR group (Jason, Jason, Geoff, Tracy and myself) to be listed as the Sage contributors. \n```",
    "created_at": "2008-12-22T18:43:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4754#issuecomment-39098",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:6'></a>From personal email from Laura DeLoss, 11 Dec 2008:

```
I agree to license the code available from http://arxiv.org/abs/0812.1616 as GPL version 2 or later and for the EGR group (Jason, Jason, Geoff, Tracy and myself) to be listed as the Sage contributors. 
```



---

archive/issue_comments_039099.json:
```json
{
    "body": "<a id='comment:7'></a>CC'ing myself",
    "created_at": "2009-06-27T00:49:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4754#issuecomment-39099",
    "user": "https://trac.sagemath.org/admin/accounts/users/mvngu"
}
```

<a id='comment:7'></a>CC'ing myself



---

archive/issue_comments_039100.json:
```json
{
    "body": "<a id='comment:8'></a>Jason Smith also verbally gave me permission to license the code GPLv2 or later.",
    "created_at": "2010-01-05T04:16:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4754#issuecomment-39100",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:8'></a>Jason Smith also verbally gave me permission to license the code GPLv2 or later.



---

archive/issue_comments_039101.json:
```json
{
    "body": "Changing upstream from \"\" to \"N/A\"",
    "created_at": "2010-01-05T04:16:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4754#issuecomment-39101",
    "user": "https://github.com/jasongrout"
}
```

Changing upstream from "" to "N/A"



---

archive/issue_comments_039102.json:
```json
{
    "body": "Attachment [trac-4754-minrank.patch](tarball://root/attachments/some-uuid/ticket4754/trac-4754-minrank.patch) by @jasongrout created at 2010-02-02 10:09:25\n\napply on top of previous patch",
    "created_at": "2010-02-02T10:09:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4754#issuecomment-39102",
    "user": "https://github.com/jasongrout"
}
```

Attachment [trac-4754-minrank.patch](tarball://root/attachments/some-uuid/ticket4754/trac-4754-minrank.patch) by @jasongrout created at 2010-02-02 10:09:25

apply on top of previous patch



---

archive/issue_comments_039103.json:
```json
{
    "body": "Attachment [trac-4754-minrank2.patch](tarball://root/attachments/some-uuid/ticket4754/trac-4754-minrank2.patch) by @jasongrout created at 2010-02-02 10:20:06\n\napply on top of previous patches",
    "created_at": "2010-02-02T10:20:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4754#issuecomment-39103",
    "user": "https://github.com/jasongrout"
}
```

Attachment [trac-4754-minrank2.patch](tarball://root/attachments/some-uuid/ticket4754/trac-4754-minrank2.patch) by @jasongrout created at 2010-02-02 10:20:06

apply on top of previous patches



---

archive/issue_comments_039104.json:
```json
{
    "body": "<a id='comment:9'></a>Attachment [trac-4754-cython-zeroforcing.patch](tarball://root/attachments/some-uuid/ticket4754/trac-4754-cython-zeroforcing.patch) by @jasongrout created at 2010-02-02 10:21:18\n\nAdded rough versions of patches.",
    "created_at": "2010-02-02T10:21:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4754#issuecomment-39104",
    "user": "https://github.com/jasongrout"
}
```

<a id='comment:9'></a>Attachment [trac-4754-cython-zeroforcing.patch](tarball://root/attachments/some-uuid/ticket4754/trac-4754-cython-zeroforcing.patch) by @jasongrout created at 2010-02-02 10:21:18

Added rough versions of patches.



---

archive/issue_comments_039105.json:
```json
{
    "body": "Changing status from new to needs_work.",
    "created_at": "2010-02-02T10:21:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4754#issuecomment-39105",
    "user": "https://github.com/jasongrout"
}
```

Changing status from new to needs_work.



---

archive/issue_events_010869.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4754",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4754#event-10869"
}
```



---

archive/issue_events_010870.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4754",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4754#event-10870"
}
```



---

archive/issue_events_010871.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4754",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4754#event-10871"
}
```



---

archive/issue_events_010872.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4754",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4754#event-10872"
}
```



---

archive/issue_events_010873.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4754",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4754#event-10873"
}
```



---

archive/issue_events_010874.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4754",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4754#event-10874"
}
```



---

archive/issue_events_010875.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/4754",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/4754#event-10875"
}
```



---

archive/issue_comments_039106.json:
```json
{
    "body": "<a id='comment:15'></a>Is it best to merge the code or to make https://github.com/jasongrout/minimum_rank an optional package ?",
    "created_at": "2021-10-02T14:31:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4754",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/4754#issuecomment-39106",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:15'></a>Is it best to merge the code or to make https://github.com/jasongrout/minimum_rank an optional package ?
