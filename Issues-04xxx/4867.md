...
```json
{
    "body": "<a id='comment:1'></a>The issue here is that seemingly the 32 bit userspace bits are missing.\n\nCheers,\n\nMichael",
...
<a id='comment:1'></a>The issue here is that seemingly the 32 bit userspace bits are missing.
...
```json
{
    "body": "<a id='comment:2'></a>The issue is not that bits are missing, the problem is plainly and simply that on Ubuntu multi lib in *any* upstream gcc is broken because the Ubuntu people chose to rename\n\n```\nlib64 -> lib\nlib -> lib32\n```\nOne can disable multilib support and get a 64 bit gcc on sage.math that way.\n\nCheers,\n\nMichael",
...
<a id='comment:2'></a>The issue is not that bits are missing, the problem is plainly and simply that on Ubuntu multi lib in *any* upstream gcc is broken because the Ubuntu people chose to rename
...
```json
{
    "body": "<a id='comment:4'></a>Invalid, even just because said package doesn't exist anymore.",
    "created_at": "2013-08-13T15:54:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/4867",
    "type": "issue_comment",
...
<a id='comment:4'></a>Invalid, even just because said package doesn't exist anymore.
