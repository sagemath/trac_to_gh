# Issue 34447: Make EdgesView accept two sets of vertices

archive/issues_034210.json:
```json
{
    "assignees": [],
    "body": "It would be useful to have `EdgesView(source_vertices, target_vertices)` be the list of edges with one end in `source_vertices` and one end in `target_vertices`.\n\nCC:  @dcoudert\n\nBranch/Commit: **[public/graphs/34447_view](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/34447_view) @ [e64a32c](https://github.com/sagemath/sagetrac-mirror/commit/e64a32cb5b044e7276c6c7e77fc6dff1a459fd06)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/34447_\n\n",
    "created_at": "2022-08-27T09:43:54Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/needs%20review"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make EdgesView accept two sets of vertices",
    "type": "issue",
    "updated_at": "2022-09-19T18:58:47Z",
    "url": "https://github.com/sagemath/sage/issues/34447",
    "user": "https://github.com/videlec"
}
```
It would be useful to have `EdgesView(source_vertices, target_vertices)` be the list of edges with one end in `source_vertices` and one end in `target_vertices`.

CC:  @dcoudert

Branch/Commit: **[public/graphs/34447_view](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/34447_view) @ [e64a32c](https://github.com/sagemath/sagetrac-mirror/commit/e64a32cb5b044e7276c6c7e77fc6dff1a459fd06)**

Reviewer: **Travis Scrimshaw**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/34447_





---

archive/issue_events_452178.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2022-08-27T09:43:54Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/34447",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34447#event-452178"
}
```



---

archive/issue_events_452179.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2022-08-27T09:43:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34447",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34447#event-452179"
}
```



---

archive/issue_events_452180.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2022-08-27T09:43:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34447",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34447#event-452180"
}
```



---

archive/issue_events_452181.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2022-08-27T09:44:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34447",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34447#event-452181"
}
```



---

archive/issue_comments_559520.json:
```json
{
    "body": "Commit: **[e64a32c](https://github.com/sagemath/sagetrac-mirror/commit/e64a32cb5b044e7276c6c7e77fc6dff1a459fd06)**",
    "created_at": "2022-08-28T17:15:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34447",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34447#issuecomment-559520",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[e64a32c](https://github.com/sagemath/sagetrac-mirror/commit/e64a32cb5b044e7276c6c7e77fc6dff1a459fd06)**



---

archive/issue_comments_559521.json:
```json
{
    "body": "Author: **David Coudert**",
    "created_at": "2022-08-28T17:15:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34447",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34447#issuecomment-559521",
    "user": "https://github.com/dcoudert"
}
```

Author: **David Coudert**



---

archive/issue_events_452182.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2022-08-28T17:15:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34447",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34447#event-452182"
}
```



---

archive/issue_comments_559522.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nHere is a proposal. Let me know if this is what you have in mind.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e64a32cb5b044e7276c6c7e77fc6dff1a459fd06\">e64a32c</a></td><td><code>trac #34447: extend EdgesView to support edge boundary</code></td></tr></table>\n",
    "created_at": "2022-08-28T17:15:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34447",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34447#issuecomment-559522",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:2" align="right">Comment 2</div>

Here is a proposal. Let me know if this is what you have in mind.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e64a32cb5b044e7276c6c7e77fc6dff1a459fd06">e64a32c</a></td><td><code>trac #34447: extend EdgesView to support edge boundary</code></td></tr></table>




---

archive/issue_comments_559523.json:
```json
{
    "body": "Branch: **[public/graphs/34447_view](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/34447_view)**",
    "created_at": "2022-08-28T17:15:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34447",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34447#issuecomment-559523",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[public/graphs/34447_view](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/34447_view)**



---

archive/issue_comments_559524.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2022-08-30T07:00:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34447",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34447#issuecomment-559524",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_559525.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBot is green modulo one test that I am all but certain is unrelated.\n\nVincent, any objections to me setting a positive review?",
    "created_at": "2022-08-30T07:00:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34447",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34447#issuecomment-559525",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:3" align="right">Comment 3</div>

Bot is green modulo one test that I am all but certain is unrelated.

Vincent, any objections to me setting a positive review?



---

archive/issue_comments_559526.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nNote that we are not yet used `EdgesView` in `edge_boundary`. One of the reason is that we have several open tickets for cleaning `generic_graph.py`. The main reason is that we need to first ensure that the changes done here are what we expect.",
    "created_at": "2022-08-30T07:10:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34447",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34447#issuecomment-559526",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:4" align="right">Comment 4</div>

Note that we are not yet used `EdgesView` in `edge_boundary`. One of the reason is that we have several open tickets for cleaning `generic_graph.py`. The main reason is that we need to first ensure that the changes done here are what we expect.



---

archive/issue_events_452183.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/34447",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34447#event-452183"
}
```



---

archive/issue_events_452184.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/34447",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34447#event-452184"
}
```
