# Issue 34240: With dot2tex installed, doctesting thematic_tutorials/algebraic_combinatorics/rsk.rst opens an image viewer

archive/issues_034003.json:
```json
{
    "assignees": [],
    "body": "At least on OS X, if `dot2tex` is installed, then doctesting the file `thematic_tutorials/algebraic_combinatorics/rsk.rst` opens up the Preview app to display an image, because of line 23:\n\n```\n    sage: view(H)  # optional - dot2tex graphviz\n```\n\n\nBranch/Commit: **[3d4407f](https://github.com/sagemath/sagetrac-mirror/commit/3d4407f8624d9da15d00bb93c34d6cf0ca548a2d)**\n\nReviewer: **Matthias Koeppe**\n\nAuthor: **John Palmieri**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/34240_\n\n",
    "closed_at": "2022-08-04T22:45:15Z",
    "created_at": "2022-07-29T05:37:12Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "With dot2tex installed, doctesting thematic_tutorials/algebraic_combinatorics/rsk.rst opens an image viewer",
    "type": "issue",
    "updated_at": "2022-08-04T22:45:15Z",
    "url": "https://github.com/sagemath/sage/issues/34240",
    "user": "https://github.com/jhpalmieri"
}
```
At least on OS X, if `dot2tex` is installed, then doctesting the file `thematic_tutorials/algebraic_combinatorics/rsk.rst` opens up the Preview app to display an image, because of line 23:

```
    sage: view(H)  # optional - dot2tex graphviz
```


Branch/Commit: **[3d4407f](https://github.com/sagemath/sagetrac-mirror/commit/3d4407f8624d9da15d00bb93c34d6cf0ca548a2d)**

Reviewer: **Matthias Koeppe**

Author: **John Palmieri**

_Issue created by migration from https://trac.sagemath.org/ticket/34240_





---

archive/issue_events_449887.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2022-07-29T05:37:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/34240",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34240#event-449887"
}
```



---

archive/issue_events_449888.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2022-07-29T05:37:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34240",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
    "label_color": "000080",
    "label_name": "component: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34240#event-449888"
}
```



---

archive/issue_events_449889.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2022-07-29T05:37:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34240",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34240#event-449889"
}
```



---

archive/issue_events_449890.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2022-07-29T05:37:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34240",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34240#event-449890"
}
```



---

archive/issue_comments_556049.json:
```json
{
    "body": "Branch: **[u/jhpalmieri/34240-dot2tex-doctest](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/34240-dot2tex-doctest)**",
    "created_at": "2022-07-29T05:40:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34240",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34240#issuecomment-556049",
    "user": "https://github.com/jhpalmieri"
}
```

Branch: **[u/jhpalmieri/34240-dot2tex-doctest](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/34240-dot2tex-doctest)**



---

archive/issue_comments_556050.json:
```json
{
    "body": "Author: **John Palmieri**",
    "created_at": "2022-07-29T05:40:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34240",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34240#issuecomment-556050",
    "user": "https://github.com/jhpalmieri"
}
```

Author: **John Palmieri**



---

archive/issue_comments_556051.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/18e602a9f8be54b33c0e17ee6f2f544e3162c223\">18e602a</a></td><td><code>trac 34240: dot2tex: don't open an image viewer app when doctesting</code></td></tr></table>\n",
    "created_at": "2022-07-29T05:40:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34240",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34240#issuecomment-556051",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/18e602a9f8be54b33c0e17ee6f2f544e3162c223">18e602a</a></td><td><code>trac 34240: dot2tex: don't open an image viewer app when doctesting</code></td></tr></table>




---

archive/issue_events_449891.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2022-07-29T05:40:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34240",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34240#event-449891"
}
```



---

archive/issue_comments_556052.json:
```json
{
    "body": "Commit: **[18e602a](https://github.com/sagemath/sagetrac-mirror/commit/18e602a9f8be54b33c0e17ee6f2f544e3162c223)**",
    "created_at": "2022-07-29T05:40:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34240",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34240#issuecomment-556052",
    "user": "https://github.com/jhpalmieri"
}
```

Commit: **[18e602a](https://github.com/sagemath/sagetrac-mirror/commit/18e602a9f8be54b33c0e17ee6f2f544e3162c223)**



---

archive/issue_comments_556053.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nI think it should be `# not tested`",
    "created_at": "2022-07-29T06:27:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34240",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34240#issuecomment-556053",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:3" align="right">Comment 3</div>

I think it should be `# not tested`



---

archive/issue_comments_556054.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3d4407f8624d9da15d00bb93c34d6cf0ca548a2d\">3d4407f</a></td><td><code>trac 34240: dot2tex: don't open an image viewer app when doctesting</code></td></tr></table>\n",
    "created_at": "2022-07-29T15:03:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34240",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34240#issuecomment-556054",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3d4407f8624d9da15d00bb93c34d6cf0ca548a2d">3d4407f</a></td><td><code>trac 34240: dot2tex: don't open an image viewer app when doctesting</code></td></tr></table>




---

archive/issue_comments_556055.json:
```json
{
    "body": "Changed commit from **[18e602a](https://github.com/sagemath/sagetrac-mirror/commit/18e602a9f8be54b33c0e17ee6f2f544e3162c223)** to **[3d4407f](https://github.com/sagemath/sagetrac-mirror/commit/3d4407f8624d9da15d00bb93c34d6cf0ca548a2d)**",
    "created_at": "2022-07-29T15:03:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34240",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34240#issuecomment-556055",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[18e602a](https://github.com/sagemath/sagetrac-mirror/commit/18e602a9f8be54b33c0e17ee6f2f544e3162c223)** to **[3d4407f](https://github.com/sagemath/sagetrac-mirror/commit/3d4407f8624d9da15d00bb93c34d6cf0ca548a2d)**



---

archive/issue_comments_556056.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nYes, you're right, thanks for the fix.",
    "created_at": "2022-07-29T16:58:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34240",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34240#issuecomment-556056",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:5" align="right">Comment 5</div>

Yes, you're right, thanks for the fix.



---

archive/issue_events_449892.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-07-29T17:40:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34240",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34240#event-449892"
}
```



---

archive/issue_events_449893.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-07-29T17:40:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34240",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34240#event-449893"
}
```



---

archive/issue_comments_556057.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2022-07-29T17:40:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34240",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34240#issuecomment-556057",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_comments_556058.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nThanks!",
    "created_at": "2022-07-29T17:40:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34240",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34240#issuecomment-556058",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:7" align="right">Comment 7</div>

Thanks!



---

archive/issue_events_449894.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-08-04T22:45:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34240",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34240#event-449894"
}
```



---

archive/issue_events_449895.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "ffe67f9e9ad799b3788d8b1af765f5cbb29aec70",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-08-04T22:45:15Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/34240",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34240#event-449895"
}
```



---

archive/issue_comments_556059.json:
```json
{
    "body": "Changed branch from **[u/jhpalmieri/34240-dot2tex-doctest](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/34240-dot2tex-doctest)** to **[3d4407f](https://github.com/sagemath/sagetrac-mirror/commit/3d4407f8624d9da15d00bb93c34d6cf0ca548a2d)**",
    "created_at": "2022-08-04T22:45:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34240",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34240#issuecomment-556059",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jhpalmieri/34240-dot2tex-doctest](https://github.com/sagemath/sagetrac-mirror/tree/u/jhpalmieri/34240-dot2tex-doctest)** to **[3d4407f](https://github.com/sagemath/sagetrac-mirror/commit/3d4407f8624d9da15d00bb93c34d6cf0ca548a2d)**
