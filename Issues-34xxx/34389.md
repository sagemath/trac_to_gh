# Issue 34389: Remove mention of multisets from EnumeratedSets

archive/issues_034152.json:
```json
{
    "assignees": [],
    "body": "... as it does not make sense as a subcategory of `Sets`\n\nInstead if multisets are needed anywhere, they should go through the `values()` or `items()` methods of `EnumeratedFamilies` (#34340).\n\nFor enumerated sets of non-hashable elements, we need to provide (slow) methods for cardinality, containment, etc. that use comparisons only (see #23324).\n\n\nDepends on #34340\n\nCC:  @tscrim\n\n_Issue created by migration from https://trac.sagemath.org/ticket/34389_\n\n",
    "created_at": "2022-08-18T17:00:16Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20categories",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove mention of multisets from EnumeratedSets",
    "type": "issue",
    "updated_at": "2023-01-07T21:39:40Z",
    "url": "https://github.com/sagemath/sage/issues/34389",
    "user": "https://github.com/mkoeppe"
}
```
... as it does not make sense as a subcategory of `Sets`

Instead if multisets are needed anywhere, they should go through the `values()` or `items()` methods of `EnumeratedFamilies` (#34340).

For enumerated sets of non-hashable elements, we need to provide (slow) methods for cardinality, containment, etc. that use comparisons only (see #23324).


Depends on #34340

CC:  @tscrim

_Issue created by migration from https://trac.sagemath.org/ticket/34389_





---

archive/issue_events_451553.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-18T17:00:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/34389",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34389#event-451553"
}
```



---

archive/issue_events_451554.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-18T17:00:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34389",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20categories",
    "label_color": "0000ff",
    "label_name": "component: categories",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34389#event-451554"
}
```



---

archive/issue_events_451555.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-18T17:00:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34389",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34389#event-451555"
}
```



---

archive/issue_events_451556.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-18T17:00:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34389",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34389#event-451556"
}
```



---

archive/issue_comments_558552.json:
```json
{
    "body": "Dependencies: **#34340**",
    "created_at": "2022-08-18T20:41:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34389",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34389#issuecomment-558552",
    "user": "https://github.com/mkoeppe"
}
```

Dependencies: **#34340**



---

archive/issue_comments_558553.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n ... as it does not make sense as a subcategory of `Sets`\n \n-\n+Instead if multisets are needed anywhere, they should go through the `values()` or `items()` methods of `EnumeratedFamilies` (#34340)\n``````\n",
    "created_at": "2022-08-18T20:41:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34389",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34389#issuecomment-558553",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 ... as it does not make sense as a subcategory of `Sets`
 
-
+Instead if multisets are needed anywhere, they should go through the `values()` or `items()` methods of `EnumeratedFamilies` (#34340)
``````




---

archive/issue_events_451557.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/34389",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34389#event-451557"
}
```



---

archive/issue_events_451558.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/34389",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34389#event-451558"
}
```



---

archive/issue_comments_558554.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,6 @@\n ... as it does not make sense as a subcategory of `Sets`\n \n-Instead if multisets are needed anywhere, they should go through the `values()` or `items()` methods of `EnumeratedFamilies` (#34340)\n+Instead if multisets are needed anywhere, they should go through the `values()` or `items()` methods of `EnumeratedFamilies` (#34340).\n+\n+For enumerated sets of non-hashable elements, we need to provide (slow) methods for cardinality, containment, etc. that use comparisons only (see #23324).\n+\n``````\n",
    "created_at": "2022-08-31T22:09:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34389",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34389#issuecomment-558554",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,6 @@
 ... as it does not make sense as a subcategory of `Sets`
 
-Instead if multisets are needed anywhere, they should go through the `values()` or `items()` methods of `EnumeratedFamilies` (#34340)
+Instead if multisets are needed anywhere, they should go through the `values()` or `items()` methods of `EnumeratedFamilies` (#34340).
+
+For enumerated sets of non-hashable elements, we need to provide (slow) methods for cardinality, containment, etc. that use comparisons only (see #23324).
+
``````




---

archive/issue_events_451559.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T21:39:40Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/34389",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34389#event-451559"
}
```



---

archive/issue_events_451560.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T21:39:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/34389",
    "milestone_number": null,
    "milestone_title": "sage-9.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34389#event-451560"
}
```
