# Issue 34389: Remove mention of multisets from EnumeratedSets

archive/issues_034152.json:
```json
{
    "body": "... as it does not make sense as a subcategory of `Sets`\n\nInstead if multisets are needed anywhere, they should go through the `values()` or `items()` methods of `EnumeratedFamilies` (#34340).\n\nFor enumerated sets of non-hashable elements, we need to provide (slow) methods for cardinality, containment, etc. that use comparisons only (see #23324).\n\n\n**CC:**  @tscrim\n\n**Dependencies:** #34340\n\nIssue created by migration from https://trac.sagemath.org/ticket/34389\n\n",
    "created_at": "2022-08-18T17:00:16Z",
    "labels": [
        "component: categories",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.8",
    "title": "Remove mention of multisets from EnumeratedSets",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/34389",
    "user": "https://github.com/mkoeppe"
}
```
... as it does not make sense as a subcategory of `Sets`

Instead if multisets are needed anywhere, they should go through the `values()` or `items()` methods of `EnumeratedFamilies` (#34340).

For enumerated sets of non-hashable elements, we need to provide (slow) methods for cardinality, containment, etc. that use comparisons only (see #23324).


**CC:**  @tscrim

**Dependencies:** #34340

Issue created by migration from https://trac.sagemath.org/ticket/34389





---

archive/issue_comments_559645.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n ... as it does not make sense as a subcategory of `Sets`\n \n-\n+Instead if multisets are needed anywhere, they should go through the `values()` or `items()` methods of `EnumeratedFamilies` (#34340)\n``````\n",
    "created_at": "2022-08-18T20:41:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34389",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/34389#issuecomment-559645",
    "user": "https://github.com/mkoeppe"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 ... as it does not make sense as a subcategory of `Sets`
 
-
+Instead if multisets are needed anywhere, they should go through the `values()` or `items()` methods of `EnumeratedFamilies` (#34340)
``````




---

archive/issue_comments_559646.json:
```json
{
    "body": "**Dependencies:** #34340",
    "created_at": "2022-08-18T20:41:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34389",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/34389#issuecomment-559646",
    "user": "https://github.com/mkoeppe"
}
```

**Dependencies:** #34340



---

archive/issue_events_310680.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/34389",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/34389#event-310680"
}
```



---

archive/issue_events_310681.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/34389",
    "milestone": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/34389#event-310681"
}
```



---

archive/issue_comments_559647.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,3 +1,6 @@\n ... as it does not make sense as a subcategory of `Sets`\n \n-Instead if multisets are needed anywhere, they should go through the `values()` or `items()` methods of `EnumeratedFamilies` (#34340)\n+Instead if multisets are needed anywhere, they should go through the `values()` or `items()` methods of `EnumeratedFamilies` (#34340).\n+\n+For enumerated sets of non-hashable elements, we need to provide (slow) methods for cardinality, containment, etc. that use comparisons only (see #23324).\n+\n``````\n",
    "created_at": "2022-08-31T22:09:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34389",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/34389#issuecomment-559647",
    "user": "https://github.com/mkoeppe"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,3 +1,6 @@
 ... as it does not make sense as a subcategory of `Sets`
 
-Instead if multisets are needed anywhere, they should go through the `values()` or `items()` methods of `EnumeratedFamilies` (#34340)
+Instead if multisets are needed anywhere, they should go through the `values()` or `items()` methods of `EnumeratedFamilies` (#34340).
+
+For enumerated sets of non-hashable elements, we need to provide (slow) methods for cardinality, containment, etc. that use comparisons only (see #23324).
+
``````

