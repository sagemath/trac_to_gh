# Issue 34094: Increasing Periodic/Preperiodic Functionality

archive/issues_033857.json:
```json
{
    "assignees": [],
    "body": "Currently, the following call returns an error:\n\n```\nsage: P.<x,y> = ProjectiveSpace(RR, 1)\nsage: f = DynamicalSystem_projective([x^2 - y^2, y^2])\nsage: f.periodic_points(3, minimal=False)\n```\n\nHowever, the error is entirely artificial. That is, the code in the method works, but an error is thrown if the base ring is not a number field or a finite field. The intent of such an error is to fail \"nicely\" in order to show the user a nice error message. The cost, however, is that many unnecessary errors are thrown.\n\nWe remove such checks from `periodic_points` and `preperoidic_points`, as the cost in functionality far outweighs the benefits of a \"nice\" error message.\n\nCC:  @bhutz\n\nBranch: **[u/gh-EnderWannabe/preperiodic_field](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-EnderWannabe/preperiodic_field)**\n\nAuthor: **Alexander Galarraga**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/34094_\n\n",
    "created_at": "2022-06-28T16:16:47Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20dynamics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Increasing Periodic/Preperiodic Functionality",
    "type": "issue",
    "updated_at": "2022-09-19T18:58:47Z",
    "url": "https://github.com/sagemath/sage/issues/34094",
    "user": "https://github.com/EnderWannabe"
}
```
Currently, the following call returns an error:

```
sage: P.<x,y> = ProjectiveSpace(RR, 1)
sage: f = DynamicalSystem_projective([x^2 - y^2, y^2])
sage: f.periodic_points(3, minimal=False)
```

However, the error is entirely artificial. That is, the code in the method works, but an error is thrown if the base ring is not a number field or a finite field. The intent of such an error is to fail "nicely" in order to show the user a nice error message. The cost, however, is that many unnecessary errors are thrown.

We remove such checks from `periodic_points` and `preperoidic_points`, as the cost in functionality far outweighs the benefits of a "nice" error message.

CC:  @bhutz

Branch: **[u/gh-EnderWannabe/preperiodic_field](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-EnderWannabe/preperiodic_field)**

Author: **Alexander Galarraga**

_Issue created by migration from https://trac.sagemath.org/ticket/34094_





---

archive/issue_events_448410.json:
```json
{
    "actor": "https://github.com/EnderWannabe",
    "created_at": "2022-06-28T16:16:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/34094",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34094#event-448410"
}
```



---

archive/issue_events_448411.json:
```json
{
    "actor": "https://github.com/EnderWannabe",
    "created_at": "2022-06-28T16:16:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34094",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20dynamics",
    "label_color": "0000ff",
    "label_name": "component: dynamics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34094#event-448411"
}
```



---

archive/issue_events_448412.json:
```json
{
    "actor": "https://github.com/EnderWannabe",
    "created_at": "2022-06-28T16:16:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34094",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34094#event-448412"
}
```



---

archive/issue_events_448413.json:
```json
{
    "actor": "https://github.com/EnderWannabe",
    "created_at": "2022-06-28T16:16:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34094",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34094#event-448413"
}
```



---

archive/issue_comments_553627.json:
```json
{
    "body": "Branch: **[u/gh-EnderWannabe/preperiodic_field](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-EnderWannabe/preperiodic_field)**",
    "created_at": "2022-06-28T17:02:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34094",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34094#issuecomment-553627",
    "user": "https://github.com/EnderWannabe"
}
```

Branch: **[u/gh-EnderWannabe/preperiodic_field](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-EnderWannabe/preperiodic_field)**



---

archive/issue_events_448414.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/34094",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34094#event-448414"
}
```



---

archive/issue_events_448415.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/34094",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34094#event-448415"
}
```
