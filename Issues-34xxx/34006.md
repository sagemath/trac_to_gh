# Issue 34006: Fix the Killing form and generators for a Lie subalgebra

archive/issues_033769.json:
```json
{
    "assignees": [],
    "body": "Currently the Killing form does not work for a Lie subalgebra since the `adjoint_matrix()` of an element is not returning coordinates in the basis for the subalgebra.\n\nIt was also noted that the `adjoint_matrix()` is returning the transpose of what it should be in order to define a representation. We fix this on this ticket since we also want to fix it for the subalgebras as well.\n\nWe also fix another issue that the generators of a Lie subalgebra are not elements of the subalgebra, but instead belong to the ambient algebra.\n\nCC:  @fchapoton @darijgr @heluani @ehaka @soehms\n\nKeywords: **lie algebra, subalgebra**\n\nBranch/Commit: **[c7fb79d](https://github.com/sagemath/sagetrac-mirror/commit/c7fb79dd150931f7b2df163a4ef0dfc9a1b70b3f)**\n\nReviewer: **Darij Grinberg**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/34006_\n\n",
    "closed_at": "2022-07-09T22:33:19Z",
    "created_at": "2022-06-17T02:26:15Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix the Killing form and generators for a Lie subalgebra",
    "type": "issue",
    "updated_at": "2022-07-09T22:33:19Z",
    "url": "https://github.com/sagemath/sage/issues/34006",
    "user": "https://github.com/tscrim"
}
```
Currently the Killing form does not work for a Lie subalgebra since the `adjoint_matrix()` of an element is not returning coordinates in the basis for the subalgebra.

It was also noted that the `adjoint_matrix()` is returning the transpose of what it should be in order to define a representation. We fix this on this ticket since we also want to fix it for the subalgebras as well.

We also fix another issue that the generators of a Lie subalgebra are not elements of the subalgebra, but instead belong to the ambient algebra.

CC:  @fchapoton @darijgr @heluani @ehaka @soehms

Keywords: **lie algebra, subalgebra**

Branch/Commit: **[c7fb79d](https://github.com/sagemath/sagetrac-mirror/commit/c7fb79dd150931f7b2df163a4ef0dfc9a1b70b3f)**

Reviewer: **Darij Grinberg**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/34006_





---

archive/issue_events_447537.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-06-17T02:26:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/34006",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34006#event-447537"
}
```



---

archive/issue_events_447538.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-06-17T02:26:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34006",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34006#event-447538"
}
```



---

archive/issue_events_447539.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-06-17T02:26:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34006",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34006#event-447539"
}
```



---

archive/issue_events_447540.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-06-17T02:26:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34006",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34006#event-447540"
}
```



---

archive/issue_events_447541.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-06-17T02:30:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34006",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34006#event-447541"
}
```



---

archive/issue_comments_552641.json:
```json
{
    "body": "Branch: **[public/lie_algebras/fix_subalgebra_vector-34006](https://github.com/sagemath/sagetrac-mirror/tree/public/lie_algebras/fix_subalgebra_vector-34006)**",
    "created_at": "2022-06-17T02:30:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34006#issuecomment-552641",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/lie_algebras/fix_subalgebra_vector-34006](https://github.com/sagemath/sagetrac-mirror/tree/public/lie_algebras/fix_subalgebra_vector-34006)**



---

archive/issue_comments_552642.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nHere is a fix that is fairly simple. Unfortunately I don't know of a more general solution then the whack-a-mole way of fixing such issues as we come across them.\n\nSee also #33836 for a similar issue.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/141bc3e873cce7ac63c4f5bea8ff91921d4e0237\">141bc3e</a></td><td><code>Using the coordinate_vector when working with subalgebra vectors.</code></td></tr></table>\n",
    "created_at": "2022-06-17T02:30:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34006#issuecomment-552642",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:1'>Comment 1:</a>
Here is a fix that is fairly simple. Unfortunately I don't know of a more general solution then the whack-a-mole way of fixing such issues as we come across them.

See also #33836 for a similar issue.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/141bc3e873cce7ac63c4f5bea8ff91921d4e0237">141bc3e</a></td><td><code>Using the coordinate_vector when working with subalgebra vectors.</code></td></tr></table>




---

archive/issue_comments_552643.json:
```json
{
    "body": "Commit: **[141bc3e](https://github.com/sagemath/sagetrac-mirror/commit/141bc3e873cce7ac63c4f5bea8ff91921d4e0237)**",
    "created_at": "2022-06-17T02:30:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34006#issuecomment-552643",
    "user": "https://github.com/tscrim"
}
```

Commit: **[141bc3e](https://github.com/sagemath/sagetrac-mirror/commit/141bc3e873cce7ac63c4f5bea8ff91921d4e0237)**



---

archive/issue_comments_552644.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c7fb79dd150931f7b2df163a4ef0dfc9a1b70b3f\">c7fb79d</a></td><td><code>Fixing the adjoint matrix being the transposed version.</code></td></tr></table>\n",
    "created_at": "2022-06-20T03:10:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34006#issuecomment-552644",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c7fb79dd150931f7b2df163a4ef0dfc9a1b70b3f">c7fb79d</a></td><td><code>Fixing the adjoint matrix being the transposed version.</code></td></tr></table>




---

archive/issue_comments_552645.json:
```json
{
    "body": "Changed commit from **[141bc3e](https://github.com/sagemath/sagetrac-mirror/commit/141bc3e873cce7ac63c4f5bea8ff91921d4e0237)** to **[c7fb79d](https://github.com/sagemath/sagetrac-mirror/commit/c7fb79dd150931f7b2df163a4ef0dfc9a1b70b3f)**",
    "created_at": "2022-06-20T03:10:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34006#issuecomment-552645",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[141bc3e](https://github.com/sagemath/sagetrac-mirror/commit/141bc3e873cce7ac63c4f5bea8ff91921d4e0237)** to **[c7fb79d](https://github.com/sagemath/sagetrac-mirror/commit/c7fb79dd150931f7b2df163a4ef0dfc9a1b70b3f)**



---

archive/issue_comments_552646.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,5 @@\n Currently the Killing form does not work for a Lie subalgebra since the `adjoint_matrix()` of an element is not returning coordinates in the basis for the subalgebra.\n \n+It was also noted that the `adjoint_matrix()` is returning the transpose of what it should be in order to define a representation. We fix this on this ticket since we also want to fix it for the subalgebras as well.\n+\n We also fix another issue that the generators of a Lie subalgebra are not elements of the subalgebra, but instead belong to the ambient algebra.\n``````\n",
    "created_at": "2022-06-20T03:11:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34006#issuecomment-552646",
    "user": "https://github.com/tscrim"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,5 @@
 Currently the Killing form does not work for a Lie subalgebra since the `adjoint_matrix()` of an element is not returning coordinates in the basis for the subalgebra.
 
+It was also noted that the `adjoint_matrix()` is returning the transpose of what it should be in order to define a representation. We fix this on this ticket since we also want to fix it for the subalgebras as well.
+
 We also fix another issue that the generators of a Lie subalgebra are not elements of the subalgebra, but instead belong to the ambient algebra.
``````




---

archive/issue_comments_552647.json:
```json
{
    "body": "Reviewer: **Darij Grinberg**",
    "created_at": "2022-06-20T03:11:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34006#issuecomment-552647",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Darij Grinberg**



---

archive/issue_comments_552648.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nThe changes look good to me. Thanks for doing this, Travis!",
    "created_at": "2022-06-20T10:32:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34006#issuecomment-552648",
    "user": "https://github.com/darijgr"
}
```

<a id='comment:4'>Comment 4:</a>
The changes look good to me. Thanks for doing this, Travis!



---

archive/issue_comments_552649.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nThanks. Positive review once the bot comes back (morally) green?",
    "created_at": "2022-06-20T11:06:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34006#issuecomment-552649",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:5'>Comment 5:</a>
Thanks. Positive review once the bot comes back (morally) green?



---

archive/issue_events_447542.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-06-22T01:47:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34006",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34006#event-447542"
}
```



---

archive/issue_events_447543.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-06-22T01:47:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34006",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34006#event-447543"
}
```



---

archive/issue_comments_552650.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nMorally green patchbot.",
    "created_at": "2022-06-22T01:47:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34006#issuecomment-552650",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:6'>Comment 6:</a>
Morally green patchbot.



---

archive/issue_comments_552651.json:
```json
{
    "body": "Changed branch from **[public/lie_algebras/fix_subalgebra_vector-34006](https://github.com/sagemath/sagetrac-mirror/tree/public/lie_algebras/fix_subalgebra_vector-34006)** to **[c7fb79d](https://github.com/sagemath/sagetrac-mirror/commit/c7fb79dd150931f7b2df163a4ef0dfc9a1b70b3f)**",
    "created_at": "2022-07-09T22:33:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34006",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34006#issuecomment-552651",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/lie_algebras/fix_subalgebra_vector-34006](https://github.com/sagemath/sagetrac-mirror/tree/public/lie_algebras/fix_subalgebra_vector-34006)** to **[c7fb79d](https://github.com/sagemath/sagetrac-mirror/commit/c7fb79dd150931f7b2df163a4ef0dfc9a1b70b3f)**



---

archive/issue_events_447544.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-07-09T22:33:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34006",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34006#event-447544"
}
```



---

archive/issue_events_447545.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "add158fc0185ba969a6999fcd63c14404bd98d7b",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-07-09T22:33:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/34006",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34006#event-447545"
}
```
