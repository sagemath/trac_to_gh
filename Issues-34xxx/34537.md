...
```json
{
    "body": "<a id='comment:1'></a>Attachment [giac-1.9.0.15p0.log](tarball://root/attachments/some-uuid/ticket34537/giac-1.9.0.15p0.log) by @videlec created at 2022-09-15 09:20:41",
    "created_at": "2022-09-15T09:20:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34537",
    "type": "issue_comment",
...
<a id='comment:1'></a>Attachment [giac-1.9.0.15p0.log](tarball://root/attachments/some-uuid/ticket34537/giac-1.9.0.15p0.log) by @videlec created at 2022-09-15 09:20:41
...
```json
{
    "body": "<a id='comment:2'></a>Indeed, according to Bill Allombert `ANYARG` is gone in PARI/GP 2.15.0.",
...
<a id='comment:2'></a>Indeed, according to Bill Allombert `ANYARG` is gone in PARI/GP 2.15.0.
...
```json
{
    "body": "<a id='comment:3'></a>a solution\n\n```\n#ifdef __cplusplus\n#  define ANYARG ...\n#else\n#  define ANYARG\n#endif\n```",
...
<a id='comment:5'></a>For the complete log, see [testall.log](https://trac.sagemath.org/raw-attachment/ticket/34537/testall.log)
...
<a id='comment:7'></a>Pushed fixes for test failures caused by different choice of generators and syntax changes so we have a clear picture of the actual issues. Remaining failures:
...
```json
{
    "body": "<a id='comment:11'></a>Upstream report for the failures in `schemes.curves.affine_curve` and `rings.number_field.number_field`:\n\nhttps://pari.math.u-bordeaux.fr/cgi-bin/bugreport.cgi?bug=2414",
    "created_at": "2022-09-21T08:04:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34537",
    "type": "issue_comment",
...
<a id='comment:11'></a>Upstream report for the failures in `schemes.curves.affine_curve` and `rings.number_field.number_field`:
...
<a id='comment:16'></a>Only two remaining issues that are going to need more help from experts:
...
<a id='comment:19'></a>Replying to [Antonio Rojas](#comment%3A11):
> Upstream report for the failures in `schemes.curves.affine_curve` and `rings.number_field.number_field`:
> 
> https://pari.math.u-bordeaux.fr/cgi-bin/bugreport.cgi?bug=2414
...
```json
{
    "body": "<a id='comment:21'></a>Is this ready to go, or should we put the giac build fix on a separate ticket?",
...
<a id='comment:21'></a>Is this ready to go, or should we put the giac build fix on a separate ticket?
...
```json
{
    "body": "<a id='comment:22'></a>It's not ready, see[comment:16](#comment%3A16)",
...
<a id='comment:22'></a>It's not ready, see[comment:16](#comment%3A16)
...
```json
{
    "body": "<a id='comment:24'></a>That's now #34586",
...
<a id='comment:24'></a>That's now #34586
...
```json
{
    "body": "<a id='comment:25'></a>Instead of using the `ellQ_ellrank` from Denis Simon script we should be using the `ellrank` from PARI/GP (available from version 2.15). Though, the latter only works over rational numbers.",
...
<a id='comment:25'></a>Instead of using the `ellQ_ellrank` from Denis Simon script we should be using the `ellrank` from PARI/GP (available from version 2.15). Though, the latter only works over rational numbers.
...
<a id='comment:26'></a>Replying to [Vincent Delecroix](#comment%3A25):
> Instead of using the `ellQ_ellrank` from Denis Simon script we should be using the `ellrank` from PARI/GP (available from version 2.15). Though, the latter only works over rational numbers.
...
```json
{
    "body": "<a id='comment:27'></a>Replying to [Antonio Rojas](#comment%3A26):\n> Replying to [Vincent Delecroix](#comment%3A25):\n> > Instead of using the `ellQ_ellrank` from Denis Simon script we should be using the `ellrank` from PARI/GP (available from version 2.15). Though, the latter only works over rational numbers.\n\n> \n> That should go in a follow-up ticket, I think. The branch is already big enough.\n\n\nSure: #34608",
...
<a id='comment:27'></a>Replying to [Antonio Rojas](#comment%3A26):
> Replying to [Vincent Delecroix](#comment%3A25):
> > Instead of using the `ellQ_ellrank` from Denis Simon script we should be using the `ellrank` from PARI/GP (available from version 2.15). Though, the latter only works over rational numbers.
...
```json
{
    "body": "<a id='comment:28'></a>I am with Bill. I will push a fix for the genus 2 reduction.",
...
<a id='comment:28'></a>I am with Bill. I will push a fix for the genus 2 reduction.
...
```json
{
    "body": "<a id='comment:30'></a>The modifications I have done are incompatible with PARI < 2.15. I think it would be simpler to enforce PARI >= 2.15 in the next sage version.",
...
<a id='comment:30'></a>The modifications I have done are incompatible with PARI < 2.15. I think it would be simpler to enforce PARI >= 2.15 in the next sage version.
...
<a id='comment:32'></a>Replying to [Vincent Delecroix](#comment%3A30):
> I think it would be simpler to enforce PARI >= 2.15 in the next sage version.
...
```json
{
    "body": "<a id='comment:33'></a>Meanwhile, please see #34496 - to forbid system Pari 2.15 and newer, until we're done here.",
...
<a id='comment:33'></a>Meanwhile, please see #34496 - to forbid system Pari 2.15 and newer, until we're done here.
...
```json
{
    "body": "<a id='comment:35'></a>This is almost ready, just needs someone to figure out the failure in `sage/groups/fqf_orthogonal.py`",
...
<a id='comment:35'></a>This is almost ready, just needs someone to figure out the failure in `sage/groups/fqf_orthogonal.py`
...
```json
{
    "body": "<a id='comment:36'></a>The test in `fqf_orthogonal.py` does not make much sense as `Oq` and `OL` are different groups.",
...
<a id='comment:36'></a>The test in `fqf_orthogonal.py` does not make much sense as `Oq` and `OL` are different groups.
...
<a id='comment:38'></a>Replying to [Antonio Rojas](#comment%3A35):
> This is almost ready, just needs someone to figure out the failure in `sage/groups/fqf_orthogonal.py`
...
```json
{
    "body": "<a id='comment:39'></a>I pushed a \"fix\" in 48ea610. I do not think there is anything deep about generators that is tested here. Otherwise this should have been documented!",
...
<a id='comment:39'></a>I pushed a "fix" in 48ea610. I do not think there is anything deep about generators that is tested here. Otherwise this should have been documented!
...
```json
{
    "body": "<a id='comment:41'></a>Antonio, if you don't like my last commit feel free to replace with \"known bug\".",
...
<a id='comment:41'></a>Antonio, if you don't like my last commit feel free to replace with "known bug".
...
```json
{
    "body": "<a id='comment:42'></a>Should we enforce pari 2.15 at configure step?",
...
<a id='comment:42'></a>Should we enforce pari 2.15 at configure step?
...
<a id='comment:44'></a>Replying to [Vincent Delecroix](#comment%3A42):
> Should we enforce pari 2.15 at configure step?
...
```json
{
    "body": "<a id='comment:45'></a>see the rationale for putting #34496 into dependencies in #34496",
...
<a id='comment:45'></a>see the rationale for putting #34496 into dependencies in #34496
...
```json
{
    "body": "<a id='comment:46'></a>Also `m4_pushdef([SAGE_PARI_MINVER],[\"134912\"])` is not right. This is pari 2.15.0, but without a patch it won't work. So this has to be a bigger value.",
...
<a id='comment:46'></a>Also `m4_pushdef([SAGE_PARI_MINVER],["134912"])` is not right. This is pari 2.15.0, but without a patch it won't work. So this has to be a bigger value.
...
```json
{
    "body": "<a id='comment:47'></a>Branch pushed to git repo; I updated commit sha1. This was a forced push. Last 10 new commits:",
    "created_at": "2022-09-30T15:23:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34537",
    "type": "issue_comment",
...
<a id='comment:47'></a>Branch pushed to git repo; I updated commit sha1. This was a forced push. Last 10 new commits:
...
```json
{
    "body": "<a id='comment:48'></a>I've rebased over the branch of #34496 and bumped SAGE_PARI_MIN/MAXVER accordingly.",
...
<a id='comment:48'></a>I've rebased over the branch of #34496 and bumped SAGE_PARI_MIN/MAXVER accordingly.
...
<a id='comment:49'></a>Replying to [Dima Pasechnik](#comment%3A46):
> Also `m4_pushdef([SAGE_PARI_MINVER],["134912"])` is not right. This is pari 2.15.0, but without a patch it won't work. So this has to be a bigger value.
...
```json
{
    "body": "<a id='comment:51'></a>OK, right, I didn't pay attention. It's giac's patch, not pari's. Fixed.",
...
<a id='comment:51'></a>OK, right, I didn't pay attention. It's giac's patch, not pari's. Fixed.
...
```json
{
    "body": "<a id='comment:52'></a>Looks like the force push reverted part of the 9.8.beta1 rebase. The changes in `sage/geometry/polyhedron/backend_normaliz.py` shouldn't be there, and the pycodestyle changes from #34522 in `src/sage/lfunctions/pari.py` are missing.",
    "created_at": "2022-09-30T16:01:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34537",
    "type": "issue_comment",
...
<a id='comment:52'></a>Looks like the force push reverted part of the 9.8.beta1 rebase. The changes in `sage/geometry/polyhedron/backend_normaliz.py` shouldn't be there, and the pycodestyle changes from #34522 in `src/sage/lfunctions/pari.py` are missing.
...
```json
{
    "body": "<a id='comment:55'></a>tests good on macOS with Homebrew's pari 2.15.0",
...
<a id='comment:55'></a>tests good on macOS with Homebrew's pari 2.15.0
...
```json
{
    "body": "<a id='comment:56'></a>I suppose this needs a round of testing on GH Actions.",
    "created_at": "2022-10-01T07:35:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34537",
    "type": "issue_comment",
...
<a id='comment:56'></a>I suppose this needs a round of testing on GH Actions.
...
```json
{
    "body": "<a id='comment:57'></a>I've started https://github.com/sagemath/sagetrac-mirror/actions/runs/3164081241\nI see the CI has been refactored, not sure how to run more tests.",
...
<a id='comment:57'></a>I've started https://github.com/sagemath/sagetrac-mirror/actions/runs/3164081241
I see the CI has been refactored, not sure how to run more tests.
...
<a id='comment:58'></a>Replying to [Dima Pasechnik](#comment%3A57):
> I've started https://github.com/sagemath/sagetrac-mirror/actions/runs/3164081241
> I see the CI has been refactored, not sure how to run more tests.
...
```json
{
    "body": "<a id='comment:60'></a>works on M1 mac with Homebrew, too.",
...
<a id='comment:60'></a>works on M1 mac with Homebrew, too.
...
```json
{
    "body": "<a id='comment:61'></a>there were errors with many CI runs on macOS, but I think it's not because of this. LGTM",
    "created_at": "2022-10-03T15:01:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34537",
    "type": "issue_comment",
...
<a id='comment:61'></a>there were errors with many CI runs on macOS, but I think it's not because of this. LGTM
...
<a id='comment:62'></a>I spoke too soon. While Sage tests all pass, some of giac's selftests fail (this is on macOS M1 machine)
...
<a id='comment:63'></a>I'm seeing just one failure, which is a segfault
...
```json
{
    "body": "<a id='comment:64'></a>I see this segfault too. Does setting `threadsizemax` in `~/.gprc` help?",
...
<a id='comment:64'></a>I see this segfault too. Does setting `threadsizemax` in `~/.gprc` help?
...
```json
{
    "body": "<a id='comment:65'></a>Dima, as noted at #32785#comment:7, it is not new that `giac` fails its test suite, at least on OS X machines. On Intel (old version of `giac` and `pari`):\n\n```\nPASS: chk_partfrac\nPASS: chk_fhan3\nPASS: chk_factor\nFAIL: chk_fhan2\nPASS: chk_geo\nPASS: chk_integrate\nPASS: chk_fhan0\nPASS: chk_cas\nPASS: chk_fhan5\nPASS: chk_fhan13\nPASS: chk_fhan12\nPASS: chk_fhan14\nPASS: chk_fhan15\nPASS: chk_fhan17\nPASS: chk_fhan6\nPASS: chk_fhan19\nPASS: chk_fhan16\nPASS: chk_fhan20\nPASS: chk_fhan21\nFAIL: chk_fhan9\nPASS: chk_fhan18\nPASS: chk_fhan7\nPASS: chk_xavier\nPASS: chk_morley_demo\nPASS: chk_limit\nPASS: chk_fhan11\nPASS: chk_fhan8\nPASS: chk_fhan4\nPASS: chk_fhan1\nPASS: chk_normalize\n```\nI'll try on an m2 machine later, to see what failures I get.",
...
<a id='comment:65'></a>Dima, as noted at #32785#comment:7, it is not new that `giac` fails its test suite, at least on OS X machines. On Intel (old version of `giac` and `pari`):
...
```json
{
    "body": "<a id='comment:66'></a>ok, thus it is not much of a regression.",
...
<a id='comment:66'></a>ok, thus it is not much of a regression.
...
```json
{
    "body": "<a id='comment:67'></a>By the way, should #34496 really be a dependency, or is the point of this ticket to be able to bypass the problem of building with the newest `pari`?",
...
<a id='comment:67'></a>By the way, should #34496 really be a dependency, or is the point of this ticket to be able to bypass the problem of building with the newest `pari`?
...
<a id='comment:68'></a>Replying to [John Palmieri](#comment%3A67):
> By the way, should #34496 really be a dependency, or is the point of this ticket to be able to bypass the problem of building with the newest `pari`?
...
```json
{
    "body": "<a id='comment:69'></a>I'm not going to be able to check test suite failures with the old `pari`/`giac` on Apple M2 until Friday.",
...
<a id='comment:69'></a>I'm not going to be able to check test suite failures with the old `pari`/`giac` on Apple M2 until Friday.
...
```json
{
    "body": "<a id='comment:71'></a>On Apple M2, the old pari and the old giac give these results in giac's test suite:\n\n```\nmake  check-TESTS\nPASS: chk_partfrac\nPASS: chk_factor\nPASS: chk_integrate\nPASS: chk_geo\nPASS: chk_cas\nPASS: chk_limit\nFAIL: chk_fhan2\nPASS: chk_xavier\nPASS: chk_morley_demo\nPASS: chk_fhan3\nPASS: chk_normalize\nFAIL: chk_fhan4\nFAIL: chk_fhan6\nFAIL: chk_fhan8\nPASS: chk_fhan5\nPASS: chk_fhan0\nFAIL: chk_fhan13\nFAIL: chk_fhan12\nPASS: chk_fhan1\nPASS: chk_fhan14\nPASS: chk_fhan15\nFAIL: chk_fhan11\nPASS: chk_fhan17\nPASS: chk_fhan16\nPASS: chk_fhan20\nPASS: chk_fhan19\nPASS: chk_fhan21\nFAIL: chk_fhan9\nPASS: chk_fhan18\nPASS: chk_fhan7\n```",
...
<a id='comment:71'></a>On Apple M2, the old pari and the old giac give these results in giac's test suite:
...
```json
{
    "body": "<a id='comment:72'></a>Looks like the same failures that Dima saw, so not a regression. I don't think it should be an obstacle to this ticket.",
...
<a id='comment:72'></a>Looks like the same failures that Dima saw, so not a regression. I don't think it should be an obstacle to this ticket.
...
```json
{
    "body": "<a id='comment:73'></a>OK, thanks. Back to positive.",
    "created_at": "2022-10-08T20:28:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34537",
    "type": "issue_comment",
...
<a id='comment:73'></a>OK, thanks. Back to positive.
...
<a id='comment:74'></a>The cypari testsuite fails on OSX (intel), worked before
...
<a id='comment:75'></a>Replying to [Volker Braun](#comment%3A74):
> The cypari testsuite fails on OSX (intel), worked before
> 
> ```
> ...
...
```json
{
    "body": "<a id='comment:76'></a>The (cy)pari testsuite is generally pretty solid so I'd hate to have to permanently disable it on the buildbot",
...
<a id='comment:76'></a>The (cy)pari testsuite is generally pretty solid so I'd hate to have to permanently disable it on the buildbot
...
```json
{
    "body": "<a id='comment:77'></a>Here is an explanation for the cypari error:\nhttps://bugs.debian.org/cgi-bin/bugreport.cgi?bug=1020456#10",
...
<a id='comment:77'></a>Here is an explanation for the cypari error:
https://bugs.debian.org/cgi-bin/bugreport.cgi?bug=1020456#10
...
<a id='comment:78'></a>Replying to [Tobias Hansen](#comment%3A77):
> Here is an explanation for the cypari error:
> https://bugs.debian.org/cgi-bin/bugreport.cgi?bug=1020456#10
...
<a id='comment:79'></a>I have cypari2 pass all tests on pari-2.15 using
- patch on pari: https://pari.math.u-bordeaux.fr/cgi-bin/gitweb.cgi?p=pari.git;a=commit;h=9c4bcd3f497f30f757c8eecadc9b45b33754a540 (just remove the first hunk patching the file `CHANGES` since it will not apply otherwise)
- patch on cypari2: https://github.com/sagemath/cypari2/pull/123
...
```json
{
    "body": "<a id='comment:80'></a>pari 2.15.1 is out, see https://pari.math.u-bordeaux.fr/archives/pari-announce-22/msg00002.html",
...
<a id='comment:80'></a>pari 2.15.1 is out, see https://pari.math.u-bordeaux.fr/archives/pari-announce-22/msg00002.html
...
<a id='comment:81'></a>Replying to [Dima Pasechnik](#comment%3A80):
> pari 2.15.1 is out, see https://pari.math.u-bordeaux.fr/archives/pari-announce-22/msg00002.html
...
```json
{
    "body": "<a id='comment:82'></a>Also, giac 1.9.0.29 builds fine against 2.15 without patching.",
...
<a id='comment:82'></a>Also, giac 1.9.0.29 builds fine against 2.15 without patching.
...
<a id='comment:83'></a>Replying to [Antonio Rojas](#comment%3A82):
> Also, giac 1.9.0.29 builds fine against 2.15 without patching.
...
```json
{
    "body": "<a id='comment:84'></a>cypari2 upgrade in #33878",
    "created_at": "2022-11-11T18:53:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34537",
    "type": "issue_comment",
...
<a id='comment:84'></a>cypari2 upgrade in #33878
...
<a id='comment:85'></a>Replying to [Dima Pasechnik](#comment%3A83):
> do you want giac updated here, or on another ticket?
...
```json
{
    "body": "<a id='comment:87'></a>The title of the ticket says that we're upgrading to 2.15.1, but the branch only goes to 2.15.0.",
...
<a id='comment:87'></a>The title of the ticket says that we're upgrading to 2.15.1, but the branch only goes to 2.15.0.
...
```json
{
    "body": "<a id='comment:88'></a>Oh, right, I tested with Homebrew's pari, and Gentoo's pari (they are both 2.15.1), and forgot to bump this branch.",
...
<a id='comment:88'></a>Oh, right, I tested with Homebrew's pari, and Gentoo's pari (they are both 2.15.1), and forgot to bump this branch.
...
```json
{
    "body": "<a id='comment:89'></a>OK, readly now\n\n---\nLast 10 new commits:",
    "created_at": "2022-11-29T20:34:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34537",
    "type": "issue_comment",
...
<a id='comment:89'></a>OK, readly now
...
```json
{
    "body": "<a id='comment:90'></a>rebased over 9.8.beta4",
...
<a id='comment:90'></a>rebased over 9.8.beta4
...
<a id='comment:91'></a>On OS X Intel, the basic functionality is there (i.e., Sage builds and `make ptestlong` works), but the test suite for pari hangs. I see this in the log file, with no changes in more than an hour:
...
```json
{
    "body": "<a id='comment:92'></a>I seem to get the same thing with the previous version of `pari`. I'm trying again and I will update later.",
...
<a id='comment:92'></a>I seem to get the same thing with the previous version of `pari`. I'm trying again and I will update later.
...
```json
{
    "body": "<a id='comment:93'></a>perhaps upstream does not test with macOS at all, or perhaps only with old versions, and not 13.0 ?",
...
<a id='comment:93'></a>perhaps upstream does not test with macOS at all, or perhaps only with old versions, and not 13.0 ?
...
```json
{
    "body": "<a id='comment:94'></a>Right, I get the same with the previous version of `pari`, so this failure should not hold up this ticket.",
...
<a id='comment:94'></a>Right, I get the same with the previous version of `pari`, so this failure should not hold up this ticket.
...
```json
{
    "body": "<a id='comment:95'></a>On Apple Silicon, both the old and new `pari` pass their test suites.",
...
<a id='comment:95'></a>On Apple Silicon, both the old and new `pari` pass their test suites.
...
```json
{
    "body": "<a id='comment:96'></a>Many of the changes in doctests look fine to me, but I can't vouch for all of them. Can other people check them? (One example is the change in sage/schemes/elliptic_curves/ell_field.py, although there are others. Looks plausible but I don't know the details.)",
...
<a id='comment:96'></a>Many of the changes in doctests look fine to me, but I can't vouch for all of them. Can other people check them? (One example is the change in sage/schemes/elliptic_curves/ell_field.py, although there are others. Looks plausible but I don't know the details.)
...
```json
{
    "body": "<a id='comment:97'></a>John C., can you have a look ?",
...
<a id='comment:97'></a>John C., can you have a look ?
...
<a id='comment:98'></a>Replying to [Dima Pasechnik](#comment%3A97):
> John C., can you have a look ?
...
<a id='comment:99'></a>genus2reduction: makes sense for our doctests to just have y^2 = ... since these reductions are not unique. The most important thing is for the minimal discriminant to be correct which we do check.
...
```json
{
    "body": "<a id='comment:0'></a>Thanks John, I take your long review as OK.",
...
<a id='comment:0'></a>Thanks John, I take your long review as OK.
...
```json
{
    "body": "<a id='comment:101'></a>Replying to [Dima Pasechnik](#comment%3A100):\n> Thanks John, I take your long review as OK.\n\n\nyes!",
...
<a id='comment:101'></a>Replying to [Dima Pasechnik](#comment%3A100):
> Thanks John, I take your long review as OK.
...
<a id='comment:2'></a>There are a bunch of numerical noise issues on 32-bit:
...
```json
{
    "body": "<a id='comment:3'></a>Many of these numerical noises do not involve PARI/GP at all!",
...
<a id='comment:3'></a>Many of these numerical noises do not involve PARI/GP at all!
...
```json
{
    "body": "<a id='comment:6'></a>BTW, I've just submitted a report upstream to the pari bug tracker, which I had neglected to do before.",
...
<a id='comment:6'></a>BTW, I've just submitted a report upstream to the pari bug tracker, which I had neglected to do before.
...
<a id='comment:107'></a>Replying to [Vincent Delecroix](#comment%3A103):
> Many of these numerical noises do not involve PARI/GP at all!
...
<a id='comment:108'></a>Replying to [Volker Braun](#comment%3A107):
> Replying to [Vincent Delecroix](#comment%3A103):
> > Many of these numerical noises do not involve PARI/GP at all!
...
<a id='comment:109'></a>Replying to [Dima Pasechnik](#comment%3A108):
> Replying to [Volker Braun](#comment%3A107):
> > Replying to [Vincent Delecroix](#comment%3A103):
> > > Many of these numerical noises do not involve PARI/GP at all!
...
```json
{
    "body": "<a id='comment:0'></a>https://pari.math.u-bordeaux.fr/cgi-bin/bugreport.cgi?bug=2435",
...
<a id='comment:0'></a>https://pari.math.u-bordeaux.fr/cgi-bin/bugreport.cgi?bug=2435
...
<a id='comment:1'></a>should we proceed with the Gonzalo's patch ? 
