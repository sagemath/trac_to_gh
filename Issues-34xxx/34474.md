# Issue 34474: Make FiniteRankFreeModule.tensor_module(0, 1) return the dual

archive/issues_034237.json:
```json
{
    "assignees": [],
    "body": "In Sage 9.7.rc0, we have\n\n```\nsage: M = FiniteRankFreeModule(ZZ, 2, name='M')\nsage: M.tensor_module(0, 1) is M.dual()\nFalse\n```\nThe identity of the two objects, which is mathematically exact, is implemented here. This also simplifies some coercion maps. \n\nCC:  @mkoeppe @tscrim @mjungmath\n\nKeywords: **tensor_module**\n\nBranch/Commit: **[803f7e4](https://github.com/sagemath/sagetrac-mirror/commit/803f7e4437d853189d9c21ea73345fa672a9df5b)**\n\nReviewer: **Matthias Koeppe**\n\nAuthor: **Eric Gourgoulhon**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/34474_\n\n",
    "closed_at": "2022-09-22T22:35:22Z",
    "created_at": "2022-09-01T13:13:00Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make FiniteRankFreeModule.tensor_module(0, 1) return the dual",
    "type": "issue",
    "updated_at": "2022-09-22T22:35:22Z",
    "url": "https://github.com/sagemath/sage/issues/34474",
    "user": "https://github.com/egourgoulhon"
}
```
In Sage 9.7.rc0, we have

```
sage: M = FiniteRankFreeModule(ZZ, 2, name='M')
sage: M.tensor_module(0, 1) is M.dual()
False
```
The identity of the two objects, which is mathematically exact, is implemented here. This also simplifies some coercion maps. 

CC:  @mkoeppe @tscrim @mjungmath

Keywords: **tensor_module**

Branch/Commit: **[803f7e4](https://github.com/sagemath/sagetrac-mirror/commit/803f7e4437d853189d9c21ea73345fa672a9df5b)**

Reviewer: **Matthias Koeppe**

Author: **Eric Gourgoulhon**

_Issue created by migration from https://trac.sagemath.org/ticket/34474_





---

archive/issue_events_452488.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2022-09-01T13:13:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/34474",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34474#event-452488"
}
```



---

archive/issue_events_452489.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2022-09-01T13:13:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34474",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34474#event-452489"
}
```



---

archive/issue_events_452490.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2022-09-01T13:13:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34474",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34474#event-452490"
}
```



---

archive/issue_events_452491.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2022-09-01T13:13:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34474",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34474#event-452491"
}
```



---

archive/issue_comments_559964.json:
```json
{
    "body": "Branch: **[public/manifolds/tensor_module_dual-34474](https://github.com/sagemath/sagetrac-mirror/tree/public/manifolds/tensor_module_dual-34474)**",
    "created_at": "2022-09-01T13:17:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34474",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34474#issuecomment-559964",
    "user": "https://github.com/egourgoulhon"
}
```

Branch: **[public/manifolds/tensor_module_dual-34474](https://github.com/sagemath/sagetrac-mirror/tree/public/manifolds/tensor_module_dual-34474)**



---

archive/issue_events_452492.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2022-09-01T13:17:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34474",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34474#event-452492"
}
```



---

archive/issue_comments_559965.json:
```json
{
    "body": "Commit: **[803f7e4](https://github.com/sagemath/sagetrac-mirror/commit/803f7e4437d853189d9c21ea73345fa672a9df5b)**",
    "created_at": "2022-09-01T13:17:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34474",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34474#issuecomment-559965",
    "user": "https://github.com/egourgoulhon"
}
```

Commit: **[803f7e4](https://github.com/sagemath/sagetrac-mirror/commit/803f7e4437d853189d9c21ea73345fa672a9df5b)**



---

archive/issue_comments_559966.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/803f7e4437d853189d9c21ea73345fa672a9df5b\">803f7e4</a></td><td><code>Make FiniteRankFreeModule.tensor_module(0, 1) return the dual (#34474)</code></td></tr></table>\n",
    "created_at": "2022-09-01T13:17:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34474",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34474#issuecomment-559966",
    "user": "https://github.com/egourgoulhon"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/803f7e4437d853189d9c21ea73345fa672a9df5b">803f7e4</a></td><td><code>Make FiniteRankFreeModule.tensor_module(0, 1) return the dual (#34474)</code></td></tr></table>




---

archive/issue_comments_559967.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2022-09-01T15:35:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34474",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34474#issuecomment-559967",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_comments_559968.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nLGTM and passes all tests.\n\nPossible follow-up - #30241",
    "created_at": "2022-09-01T15:35:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34474",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34474#issuecomment-559968",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

LGTM and passes all tests.

Possible follow-up - #30241



---

archive/issue_events_452493.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-01T15:35:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34474",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34474#event-452493"
}
```



---

archive/issue_events_452494.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-01T15:35:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34474",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34474#event-452494"
}
```



---

archive/issue_comments_559969.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nThanks for the review!",
    "created_at": "2022-09-01T16:49:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34474",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34474#issuecomment-559969",
    "user": "https://github.com/egourgoulhon"
}
```

<div id="comment:3" align="right">Comment 3</div>

Thanks for the review!



---

archive/issue_events_452495.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/34474",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34474#event-452495"
}
```



---

archive/issue_events_452496.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/34474",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34474#event-452496"
}
```



---

archive/issue_events_452497.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-09-22T22:35:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34474",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34474#event-452497"
}
```



---

archive/issue_events_452498.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "857f16ca4dfe4af3a19a1745cd8336782d2dfb37",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-09-22T22:35:22Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/34474",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34474#event-452498"
}
```



---

archive/issue_comments_559970.json:
```json
{
    "body": "Changed branch from **[public/manifolds/tensor_module_dual-34474](https://github.com/sagemath/sagetrac-mirror/tree/public/manifolds/tensor_module_dual-34474)** to **[803f7e4](https://github.com/sagemath/sagetrac-mirror/commit/803f7e4437d853189d9c21ea73345fa672a9df5b)**",
    "created_at": "2022-09-22T22:35:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34474",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34474#issuecomment-559970",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/manifolds/tensor_module_dual-34474](https://github.com/sagemath/sagetrac-mirror/tree/public/manifolds/tensor_module_dual-34474)** to **[803f7e4](https://github.com/sagemath/sagetrac-mirror/commit/803f7e4437d853189d9c21ea73345fa672a9df5b)**
