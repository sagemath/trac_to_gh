...
<a id='comment:10'></a>Replying to [Kwankyu Lee](#comment%3A9):
...
<a id='comment:12'></a>Replying to [Dima Pasechnik](#comment%3A4):
> I'm using an import statement 
> 
> ```
> from sage.interfaces.gap import is_GapElement
...
<a id='comment:13'></a>Replying to [Kwankyu Lee](#comment%3A12):
...
<a id='comment:17'></a>Replying to [Kwankyu Lee](#comment%3A15):
> Do you want to avoid importing from `sage.interfaces.gap`? Why? 
...
<a id='comment:18'></a>Replying to [Matthias Köppe](#comment%3A16):
> If this is for modularization purposes in which an import may fail: I am using this idiom for such isinstance tests:
...
<a id='comment:20'></a>Replying to [Dima Pasechnik](#comment%3A18):
> Replying to [Matthias Köppe](#comment%3A16):
> > If this is for modularization purposes in which an import may fail: I am using this idiom for such isinstance tests:
...
<a id='comment:21'></a>Replying to [Dima Pasechnik](#comment%3A17):
> Replying to [Kwankyu Lee](#comment%3A15):
> > Do you want to avoid importing from `sage.interfaces.gap`? Why? 
...
<a id='comment:23'></a>Replying to [Matthias Köppe](#comment%3A19):
> Another approach: #34804 Deprecate `sage.interfaces` `is_...Element` functions
...
<a id='comment:24'></a>Replying to [Dima Pasechnik](#comment%3A23):
> Replying to [Matthias Köppe](#comment%3A19):
> > Another approach: #34804 Deprecate `sage.interfaces` `is_...Element` functions
...
```json
{
    "body": "<a id='comment:31'></a>Replying to [Dima Pasechnik](#comment%3A30):\n> Why don't we instead go the way we went with `GapElement` here, by adding `sage.libs.abc`, etc?\n\n\nThat's also fine",
...
<a id='comment:31'></a>Replying to [Dima Pasechnik](#comment%3A30):
> Why don't we instead go the way we went with `GapElement` here, by adding `sage.libs.abc`, etc?
...
<a id='comment:32'></a>Replying to [Matthias Köppe](#comment%3A31):
> Replying to [Dima Pasechnik](#comment%3A30):
> > Why don't we instead go the way we went with `GapElement` here, by adding `sage.libs.abc`, etc?
...
```json
{
    "body": "<a id='comment:33'></a>It's kind of why I suggested to use `try except ... ()` in[comment:30](#comment%3A30).",
...
<a id='comment:33'></a>It's kind of why I suggested to use `try except ... ()` in[comment:30](#comment%3A30).
...
<a id='comment:34'></a>Replying to [Matthias Köppe](#comment%3A33):
> It's kind of why I suggested to use `try except ... ()` in[comment:30](#comment%3A30).
...
<a id='comment:36'></a>Replying to [Matthias Köppe](#comment%3A35):
> The ticket is introducing a modularization regression by inserting this new module-level import.
...
<a id='comment:38'></a>Replying to [Matthias Köppe](#comment%3A37):
> There are no unprotected module-level imports from `sage.libs.gap` in sage.rings. Please don't introduce one, that's all I'm asking.
