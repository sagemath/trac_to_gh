...
```json
{
    "body": "<a id='comment:6'></a>Forgot to mention in the description that I added a few docstrings and doctests",
...
<a id='comment:6'></a>Forgot to mention in the description that I added a few docstrings and doctests
...
```json
{
    "body": "<a id='comment:8'></a>Rather than `Compute` in a docstring use `Return`. The fact that a function computes is good to know but not very relevant for its specification.",
...
<a id='comment:8'></a>Rather than `Compute` in a docstring use `Return`. The fact that a function computes is good to know but not very relevant for its specification.
...
```json
{
    "body": "<a id='comment:9'></a>Why removing  `[DGH2020]` completely from the references?",
...
<a id='comment:9'></a>Why removing  `[DGH2020]` completely from the references?
...
<a id='comment:11'></a>Why is there a `*` introduced in front of "McCaffrey" here
