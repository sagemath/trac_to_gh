# Issue 34653: __floordiv__ for sparse polynomials

archive/issues_034416.json:
```json
{
    "assignees": [],
    "body": "Before this ticket, the following would raise an `ArithmeticError` in the sparse case:\n\n```\nsage: P.<x,y> = PolynomialRing(SR)\nsage: R.<z> = PolynomialRing(P, sparse=False); R\nUnivariate Polynomial Ring in z over Multivariate Polynomial Ring in x, y over Symbolic Ring\nsage: (x*z) // x\nz\nsage: R.<z> = PolynomialRing(P, sparse=True); R\nSparse Univariate Polynomial Ring in z over Multivariate Polynomial Ring in x, y over Symbolic Ring\nsage: (x*z) // x\nz\n```\n\nBranch/Commit: **[9577386](https://github.com/sagemath/sagetrac-mirror/commit/957738684943d639805ed2c727da7e3ccdc6d5f1)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Martin Rubey**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/34653_\n\n",
    "closed_at": "2022-10-17T22:43:44Z",
    "created_at": "2022-10-12T23:27:47Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "__floordiv__ for sparse polynomials",
    "type": "issue",
    "updated_at": "2022-10-17T22:43:44Z",
    "url": "https://github.com/sagemath/sage/issues/34653",
    "user": "https://github.com/mantepse"
}
```
Before this ticket, the following would raise an `ArithmeticError` in the sparse case:

```
sage: P.<x,y> = PolynomialRing(SR)
sage: R.<z> = PolynomialRing(P, sparse=False); R
Univariate Polynomial Ring in z over Multivariate Polynomial Ring in x, y over Symbolic Ring
sage: (x*z) // x
z
sage: R.<z> = PolynomialRing(P, sparse=True); R
Sparse Univariate Polynomial Ring in z over Multivariate Polynomial Ring in x, y over Symbolic Ring
sage: (x*z) // x
z
```

Branch/Commit: **[9577386](https://github.com/sagemath/sagetrac-mirror/commit/957738684943d639805ed2c727da7e3ccdc6d5f1)**

Reviewer: **Travis Scrimshaw**

Author: **Martin Rubey**

_Issue created by migration from https://trac.sagemath.org/ticket/34653_





---

archive/issue_events_454158.json:
```json
{
    "actor": "https://github.com/mantepse",
    "created_at": "2022-10-12T23:27:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/34653",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34653#event-454158"
}
```



---

archive/issue_events_454159.json:
```json
{
    "actor": "https://github.com/mantepse",
    "created_at": "2022-10-12T23:27:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34653",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34653#event-454159"
}
```



---

archive/issue_comments_562549.json:
```json
{
    "body": "Branch: **[u/mantepse/__floordiv___for_sparse_polynomials](https://github.com/sagemath/sagetrac-mirror/tree/u/mantepse/__floordiv___for_sparse_polynomials)**",
    "created_at": "2022-10-12T23:29:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34653#issuecomment-562549",
    "user": "https://github.com/mantepse"
}
```

Branch: **[u/mantepse/__floordiv___for_sparse_polynomials](https://github.com/sagemath/sagetrac-mirror/tree/u/mantepse/__floordiv___for_sparse_polynomials)**



---

archive/issue_events_454160.json:
```json
{
    "actor": "https://github.com/mantepse",
    "created_at": "2022-10-12T23:31:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34653",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34653#event-454160"
}
```



---

archive/issue_events_454161.json:
```json
{
    "actor": "https://github.com/mantepse",
    "created_at": "2022-10-12T23:31:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34653",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34653#event-454161"
}
```



---

archive/issue_events_454162.json:
```json
{
    "actor": "https://github.com/mantepse",
    "created_at": "2022-10-12T23:31:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34653",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34653#event-454162"
}
```



---

archive/issue_comments_562550.json:
```json
{
    "body": "Commit: **[9577386](https://github.com/sagemath/sagetrac-mirror/commit/957738684943d639805ed2c727da7e3ccdc6d5f1)**",
    "created_at": "2022-10-12T23:31:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34653#issuecomment-562550",
    "user": "https://github.com/mantepse"
}
```

Commit: **[9577386](https://github.com/sagemath/sagetrac-mirror/commit/957738684943d639805ed2c727da7e3ccdc6d5f1)**



---

archive/issue_comments_562551.json:
```json
{
    "body": "Author: **Martin Rubey**",
    "created_at": "2022-10-12T23:31:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34653#issuecomment-562551",
    "user": "https://github.com/mantepse"
}
```

Author: **Martin Rubey**



---

archive/issue_comments_562552.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/957738684943d639805ed2c727da7e3ccdc6d5f1\">9577386</a></td><td><code>implement Polynomial_generic_sparse.__floordiv__</code></td></tr></table>\n",
    "created_at": "2022-10-12T23:31:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34653#issuecomment-562552",
    "user": "https://github.com/mantepse"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/957738684943d639805ed2c727da7e3ccdc6d5f1">9577386</a></td><td><code>implement Polynomial_generic_sparse.__floordiv__</code></td></tr></table>




---

archive/issue_comments_562553.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,13 @@\n+Before this ticket, the following would raise an `ArithmeticError` in the sparse case:\n \n+```\n+sage: P.<x,y> = PolynomialRing(SR)\n+sage: R.<z> = PolynomialRing(P, sparse=False); R\n+Univariate Polynomial Ring in z over Multivariate Polynomial Ring in x, y over Symbolic Ring\n+sage: (x*z) // x\n+z\n+sage: R.<z> = PolynomialRing(P, sparse=True); R\n+Sparse Univariate Polynomial Ring in z over Multivariate Polynomial Ring in x, y over Symbolic Ring\n+sage: (x*z) // x\n+z\n+```\n``````\n",
    "created_at": "2022-10-12T23:31:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34653#issuecomment-562553",
    "user": "https://github.com/mantepse"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,13 @@
+Before this ticket, the following would raise an `ArithmeticError` in the sparse case:
 
+```
+sage: P.<x,y> = PolynomialRing(SR)
+sage: R.<z> = PolynomialRing(P, sparse=False); R
+Univariate Polynomial Ring in z over Multivariate Polynomial Ring in x, y over Symbolic Ring
+sage: (x*z) // x
+z
+sage: R.<z> = PolynomialRing(P, sparse=True); R
+Sparse Univariate Polynomial Ring in z over Multivariate Polynomial Ring in x, y over Symbolic Ring
+sage: (x*z) // x
+z
+```
``````




---

archive/issue_events_454163.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-10-13T04:33:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34653",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34653#event-454163"
}
```



---

archive/issue_events_454164.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-10-13T04:33:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34653",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34653#event-454164"
}
```



---

archive/issue_comments_562554.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nLGTM.",
    "created_at": "2022-10-13T04:33:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34653#issuecomment-562554",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:3'>Comment 3:</a>
LGTM.



---

archive/issue_comments_562555.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2022-10-13T04:33:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34653#issuecomment-562555",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_562556.json:
```json
{
    "body": "Changed branch from **[u/mantepse/__floordiv___for_sparse_polynomials](https://github.com/sagemath/sagetrac-mirror/tree/u/mantepse/__floordiv___for_sparse_polynomials)** to **[9577386](https://github.com/sagemath/sagetrac-mirror/commit/957738684943d639805ed2c727da7e3ccdc6d5f1)**",
    "created_at": "2022-10-17T22:43:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34653",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34653#issuecomment-562556",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mantepse/__floordiv___for_sparse_polynomials](https://github.com/sagemath/sagetrac-mirror/tree/u/mantepse/__floordiv___for_sparse_polynomials)** to **[9577386](https://github.com/sagemath/sagetrac-mirror/commit/957738684943d639805ed2c727da7e3ccdc6d5f1)**



---

archive/issue_events_454165.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-10-17T22:43:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34653",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34653#event-454165"
}
```



---

archive/issue_events_454166.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "9c071901f9868b565ea57d0bff825ecd07542fed",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-10-17T22:43:44Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/34653",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34653#event-454166"
}
```
