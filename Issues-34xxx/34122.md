# Issue 34122: Bug in is_planar() method for directed graphs

archive/issues_033885.json:
```json
{
    "assignees": [],
    "body": "There may be a bug in the `is_planar()` implementation for directed graphs.  We have a case which returns `False` instead of `True` for the below planarity test.\n\n```\n     # We include both [1,5] and [5,1]\n     G = [ [1,2], [1,4], [1,5], [2,3], [2,4], [2,5], [3,4], [3,5], [4,5], [5,1] ]\n     G1 = Graph(G)\n     G2 = DiGraph(G)\n     G1.is_planar() # True\n     G2.is_planar() # False => Should be true\n```\n\nNote that it does not seem to be a problem of cycle, since it works for the following case.\n\n```\n     # We include both [1,2] and [2,1]\n     G = [ [1,2] , [2,1] ]\n     G1 = Graph(G)\n     G2 = DiGraph(G)\n     G1.is_planar() # True\n     G2.is_planar() # True !\n```\n\nKeywords: **digraph, planar**\n\nBranch/Commit: **[70042b3](https://github.com/sagemath/sagetrac-mirror/commit/70042b344f706b4df710f8a35649dfd37ce6cd40)**\n\nReviewer: **David Coudert**\n\nAuthor: **Dave Morris**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/34122_\n\n",
    "closed_at": "2022-07-17T18:14:20Z",
    "created_at": "2022-07-06T14:06:34Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Bug in is_planar() method for directed graphs",
    "type": "issue",
    "updated_at": "2022-07-17T18:14:20Z",
    "url": "https://github.com/sagemath/sage/issues/34122",
    "user": "https://github.com/gdolle"
}
```
There may be a bug in the `is_planar()` implementation for directed graphs.  We have a case which returns `False` instead of `True` for the below planarity test.

```
     # We include both [1,5] and [5,1]
     G = [ [1,2], [1,4], [1,5], [2,3], [2,4], [2,5], [3,4], [3,5], [4,5], [5,1] ]
     G1 = Graph(G)
     G2 = DiGraph(G)
     G1.is_planar() # True
     G2.is_planar() # False => Should be true
```

Note that it does not seem to be a problem of cycle, since it works for the following case.

```
     # We include both [1,2] and [2,1]
     G = [ [1,2] , [2,1] ]
     G1 = Graph(G)
     G2 = DiGraph(G)
     G1.is_planar() # True
     G2.is_planar() # True !
```

Keywords: **digraph, planar**

Branch/Commit: **[70042b3](https://github.com/sagemath/sagetrac-mirror/commit/70042b344f706b4df710f8a35649dfd37ce6cd40)**

Reviewer: **David Coudert**

Author: **Dave Morris**

_Issue created by migration from https://trac.sagemath.org/ticket/34122_





---

archive/issue_events_448700.json:
```json
{
    "actor": "https://github.com/gdolle",
    "created_at": "2022-07-06T14:06:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/34122",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34122#event-448700"
}
```



---

archive/issue_events_448701.json:
```json
{
    "actor": "https://github.com/gdolle",
    "created_at": "2022-07-06T14:06:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34122",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34122#event-448701"
}
```



---

archive/issue_events_448702.json:
```json
{
    "actor": "https://github.com/gdolle",
    "created_at": "2022-07-06T14:06:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34122",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34122#event-448702"
}
```



---

archive/issue_events_448703.json:
```json
{
    "actor": "https://github.com/gdolle",
    "created_at": "2022-07-06T14:06:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34122",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34122#event-448703"
}
```



---

archive/issue_comments_554004.json:
```json
{
    "body": "Author: **Dave Morris**",
    "created_at": "2022-07-07T04:07:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34122",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34122#issuecomment-554004",
    "user": "https://github.com/DaveWitteMorris"
}
```

Author: **Dave Morris**



---

archive/issue_comments_554005.json:
```json
{
    "body": "Branch: **[public/34122](https://github.com/sagemath/sagetrac-mirror/tree/public/34122)**",
    "created_at": "2022-07-07T04:20:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34122",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34122#issuecomment-554005",
    "user": "https://github.com/DaveWitteMorris"
}
```

Branch: **[public/34122](https://github.com/sagemath/sagetrac-mirror/tree/public/34122)**



---

archive/issue_comments_554006.json:
```json
{
    "body": "Commit: **[70042b3](https://github.com/sagemath/sagetrac-mirror/commit/70042b344f706b4df710f8a35649dfd37ce6cd40)**",
    "created_at": "2022-07-07T04:35:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34122",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34122#issuecomment-554006",
    "user": "https://github.com/DaveWitteMorris"
}
```

Commit: **[70042b3](https://github.com/sagemath/sagetrac-mirror/commit/70042b344f706b4df710f8a35649dfd37ce6cd40)**



---

archive/issue_events_448704.json:
```json
{
    "actor": "https://github.com/DaveWitteMorris",
    "created_at": "2022-07-07T04:35:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34122",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34122#event-448704"
}
```



---

archive/issue_events_448705.json:
```json
{
    "actor": "https://github.com/DaveWitteMorris",
    "created_at": "2022-07-07T04:35:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34122",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34122#event-448705"
}
```



---

archive/issue_events_448706.json:
```json
{
    "actor": "https://github.com/DaveWitteMorris",
    "created_at": "2022-07-07T04:35:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34122",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34122#event-448706"
}
```



---

archive/issue_comments_554007.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nThanks for pointing out the bug. This is a fundamental error in basic functionality, so I increased the priority to \"critical\". \n\nThe code applied an inequality to digraphs that is only valid for graphs.  A digraph without loops or multiple edges can have twice as many edges as the corresponding graph (because each undirected edge in a graph can be replaced with two oppositely directed edges in a digraph), so it is not true that a digraph with `self.size() > 3 * self.order() - 6` cannot be planar. \n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/70042b344f706b4df710f8a35649dfd37ce6cd40\">70042b3</a></td><td><code>trac 34122 fix is_planar bug</code></td></tr></table>\n",
    "created_at": "2022-07-07T04:35:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34122",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34122#issuecomment-554007",
    "user": "https://github.com/DaveWitteMorris"
}
```

<div id="comment:3" align="right">Comment 3</div>

Thanks for pointing out the bug. This is a fundamental error in basic functionality, so I increased the priority to "critical". 

The code applied an inequality to digraphs that is only valid for graphs.  A digraph without loops or multiple edges can have twice as many edges as the corresponding graph (because each undirected edge in a graph can be replaced with two oppositely directed edges in a digraph), so it is not true that a digraph with `self.size() > 3 * self.order() - 6` cannot be planar. 

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/70042b344f706b4df710f8a35649dfd37ce6cd40">70042b3</a></td><td><code>trac 34122 fix is_planar bug</code></td></tr></table>




---

archive/issue_events_448707.json:
```json
{
    "actor": "https://github.com/DaveWitteMorris",
    "created_at": "2022-07-07T04:35:30Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/34122",
    "title_is": "Bug in is_planar() method for directed graphs",
    "title_was": "Bug is_planar() test for directed graph",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34122#event-448707"
}
```



---

archive/issue_comments_554008.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,27 +1,21 @@\n-There may be a bug in is_planar() implementation for directed graph, we have a case which return false instead of true for the below planar test,\n+There may be a bug in the `is_planar()` implementation for directed graphs.  We have a case which returns `False` instead of `True` for the below planarity test.\n \n-```div\n-  {{{#!python\n-     # Multiplicity [1,5]\n+```\n+     # We include both [1,5] and [5,1]\n      G = [ [1,2], [1,4], [1,5], [2,3], [2,4], [2,5], [3,4], [3,5], [4,5], [5,1] ]\n      G1 = Graph(G)\n      G2 = DiGraph(G)\n      G1.is_planar() # True\n      G2.is_planar() # False => Should be true\n-  }}}\n ```\n \n-Note that it does not seem to be a problem of cycle, since it works for the case\n+Note that it does not seem to be a problem of cycle, since it works for the following case.\n \n-```div\n-  {{{#!python\n-     # Multiplicity [1,5]\n+```\n+     # We include both [1,2] and [2,1]\n      G = [ [1,2] , [2,1] ]\n      G1 = Graph(G)\n      G2 = DiGraph(G)\n      G1.is_planar() # True\n      G2.is_planar() # True !\n-}}}\n-\n-\n-\n+```\n``````\n",
    "created_at": "2022-07-07T04:35:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34122",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34122#issuecomment-554008",
    "user": "https://github.com/DaveWitteMorris"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,27 +1,21 @@
-There may be a bug in is_planar() implementation for directed graph, we have a case which return false instead of true for the below planar test,
+There may be a bug in the `is_planar()` implementation for directed graphs.  We have a case which returns `False` instead of `True` for the below planarity test.
 
-```div
-  {{{#!python
-     # Multiplicity [1,5]
+```
+     # We include both [1,5] and [5,1]
      G = [ [1,2], [1,4], [1,5], [2,3], [2,4], [2,5], [3,4], [3,5], [4,5], [5,1] ]
      G1 = Graph(G)
      G2 = DiGraph(G)
      G1.is_planar() # True
      G2.is_planar() # False => Should be true
-  }}}
 ```
 
-Note that it does not seem to be a problem of cycle, since it works for the case
+Note that it does not seem to be a problem of cycle, since it works for the following case.
 
-```div
-  {{{#!python
-     # Multiplicity [1,5]
+```
+     # We include both [1,2] and [2,1]
      G = [ [1,2] , [2,1] ]
      G1 = Graph(G)
      G2 = DiGraph(G)
      G1.is_planar() # True
      G2.is_planar() # True !
-}}}
-
-
-
+```
``````




---

archive/issue_comments_554009.json:
```json
{
    "body": "Changed keywords from **** to **digraph, planar**",
    "created_at": "2022-07-07T04:35:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34122",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34122#issuecomment-554009",
    "user": "https://github.com/DaveWitteMorris"
}
```

Changed keywords from **** to **digraph, planar**



---

archive/issue_comments_554010.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nGood catch. Thank you both for reporting and fixing the issue.",
    "created_at": "2022-07-11T16:50:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34122",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34122#issuecomment-554010",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:4" align="right">Comment 4</div>

Good catch. Thank you both for reporting and fixing the issue.



---

archive/issue_comments_554011.json:
```json
{
    "body": "Reviewer: **David Coudert**",
    "created_at": "2022-07-11T16:50:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34122",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34122#issuecomment-554011",
    "user": "https://github.com/dcoudert"
}
```

Reviewer: **David Coudert**



---

archive/issue_events_448708.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2022-07-11T16:50:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34122",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34122#event-448708"
}
```



---

archive/issue_events_448709.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2022-07-11T16:50:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34122",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34122#event-448709"
}
```



---

archive/issue_comments_554012.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nThanks for the review!",
    "created_at": "2022-07-12T05:20:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34122",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34122#issuecomment-554012",
    "user": "https://github.com/DaveWitteMorris"
}
```

<div id="comment:6" align="right">Comment 6</div>

Thanks for the review!



---

archive/issue_events_448710.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-07-17T18:14:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34122",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34122#event-448710"
}
```



---

archive/issue_events_448711.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "4657052737942bfc038156a00d3eb6952fd462e6",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-07-17T18:14:20Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/34122",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34122#event-448711"
}
```



---

archive/issue_comments_554013.json:
```json
{
    "body": "Changed branch from **[public/34122](https://github.com/sagemath/sagetrac-mirror/tree/public/34122)** to **[70042b3](https://github.com/sagemath/sagetrac-mirror/commit/70042b344f706b4df710f8a35649dfd37ce6cd40)**",
    "created_at": "2022-07-17T18:14:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34122",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34122#issuecomment-554013",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/34122](https://github.com/sagemath/sagetrac-mirror/tree/public/34122)** to **[70042b3](https://github.com/sagemath/sagetrac-mirror/commit/70042b344f706b4df710f8a35649dfd37ce6cd40)**
