# Issue 34901: random doctest failure in src/doc/en/thematic_tutorials/group_theory.rst

archive/issues_034664.json:
```json
{
    "assignees": [],
    "body": "Part of #32544:\n\n```\nsage -t --long --random-seed=185470747385175316039405094141387664605 src/doc/en/thematic_tutorials/group_theory.rst\n**********************************************************************\nFile \"src/doc/en/thematic_tutorials/group_theory.rst\", line 207, in doc.en.thematic_tutorials.group_theory\nFailed example:\n    euler_phi(m*n) == euler_phi(m) * euler_phi(n)\nExpected:\n    True\nGot:\n    False\n**********************************************************************\n1 item had failures:\n   1 of 170 in doc.en.thematic_tutorials.group_theory\n    [127 tests, 1 failure, 4.40 s]\n```\n\n(From a patchbot run in #34880.)\n\nBranch/Commit: **[u/chapoton/34901](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/34901) @ [a8ee692](https://github.com/sagemath/sagetrac-mirror/commit/a8ee69222295df9eb3d17e2ec1804246824e4f55)**\n\nReviewer: **Lorenz Panny**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/34901_\n\n",
    "created_at": "2023-01-09T03:28:45Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/positive%20review"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "random doctest failure in src/doc/en/thematic_tutorials/group_theory.rst",
    "type": "issue",
    "updated_at": "2023-01-10T00:53:21Z",
    "url": "https://github.com/sagemath/sage/issues/34901",
    "user": "https://github.com/yyyyx4"
}
```
Part of #32544:

```
sage -t --long --random-seed=185470747385175316039405094141387664605 src/doc/en/thematic_tutorials/group_theory.rst
**********************************************************************
File "src/doc/en/thematic_tutorials/group_theory.rst", line 207, in doc.en.thematic_tutorials.group_theory
Failed example:
    euler_phi(m*n) == euler_phi(m) * euler_phi(n)
Expected:
    True
Got:
    False
**********************************************************************
1 item had failures:
   1 of 170 in doc.en.thematic_tutorials.group_theory
    [127 tests, 1 failure, 4.40 s]
```

(From a patchbot run in #34880.)

Branch/Commit: **[u/chapoton/34901](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/34901) @ [a8ee692](https://github.com/sagemath/sagetrac-mirror/commit/a8ee69222295df9eb3d17e2ec1804246824e4f55)**

Reviewer: **Lorenz Panny**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/34901_





---

archive/issue_events_456307.json:
```json
{
    "actor": "https://github.com/yyyyx4",
    "created_at": "2023-01-09T03:28:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/34901",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34901#event-456307"
}
```



---

archive/issue_events_456308.json:
```json
{
    "actor": "https://github.com/yyyyx4",
    "created_at": "2023-01-09T03:28:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34901",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34901#event-456308"
}
```



---

archive/issue_events_456309.json:
```json
{
    "actor": "https://github.com/yyyyx4",
    "created_at": "2023-01-09T03:28:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34901",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34901#event-456309"
}
```



---

archive/issue_events_456310.json:
```json
{
    "actor": "https://github.com/yyyyx4",
    "created_at": "2023-01-09T03:28:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34901",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34901#event-456310"
}
```



---

archive/issue_comments_565208.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nIn the test, `m` and `n` are random primes less than `10000`. The test fails if and only if `m == n`.",
    "created_at": "2023-01-09T03:32:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34901#issuecomment-565208",
    "user": "https://github.com/yyyyx4"
}
```

<div id="comment:1" align="right">Comment 1</div>

In the test, `m` and `n` are random primes less than `10000`. The test fails if and only if `m == n`.



---

archive/issue_events_456311.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2023-01-09T18:57:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34901",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34901#event-456311"
}
```



---

archive/issue_comments_565209.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a8ee69222295df9eb3d17e2ec1804246824e4f55\">a8ee692</a></td><td><code>fix a random doctest in tutorial</code></td></tr></table>\n",
    "created_at": "2023-01-09T18:57:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34901#issuecomment-565209",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a8ee69222295df9eb3d17e2ec1804246824e4f55">a8ee692</a></td><td><code>fix a random doctest in tutorial</code></td></tr></table>




---

archive/issue_comments_565210.json:
```json
{
    "body": "Branch: **[u/chapoton/34901](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/34901)**",
    "created_at": "2023-01-09T18:57:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34901#issuecomment-565210",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/34901](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/34901)**



---

archive/issue_comments_565211.json:
```json
{
    "body": "Commit: **[a8ee692](https://github.com/sagemath/sagetrac-mirror/commit/a8ee69222295df9eb3d17e2ec1804246824e4f55)**",
    "created_at": "2023-01-09T18:57:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34901#issuecomment-565211",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[a8ee692](https://github.com/sagemath/sagetrac-mirror/commit/a8ee69222295df9eb3d17e2ec1804246824e4f55)**



---

archive/issue_comments_565212.json:
```json
{
    "body": "Author: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2023-01-09T18:57:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34901#issuecomment-565212",
    "user": "https://github.com/fchapoton"
}
```

Author: **Frédéric Chapoton**



---

archive/issue_comments_565213.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nok like that ?",
    "created_at": "2023-01-09T18:58:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34901#issuecomment-565213",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">Comment 3</div>

ok like that ?



---

archive/issue_comments_565214.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThat should do. Thanks!",
    "created_at": "2023-01-10T00:53:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34901#issuecomment-565214",
    "user": "https://github.com/yyyyx4"
}
```

<div id="comment:4" align="right">Comment 4</div>

That should do. Thanks!



---

archive/issue_comments_565215.json:
```json
{
    "body": "Reviewer: **Lorenz Panny**",
    "created_at": "2023-01-10T00:53:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34901",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34901#issuecomment-565215",
    "user": "https://github.com/yyyyx4"
}
```

Reviewer: **Lorenz Panny**



---

archive/issue_events_456312.json:
```json
{
    "actor": "https://github.com/yyyyx4",
    "created_at": "2023-01-10T00:53:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34901",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34901#event-456312"
}
```



---

archive/issue_events_456313.json:
```json
{
    "actor": "https://github.com/yyyyx4",
    "created_at": "2023-01-10T00:53:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34901",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34901#event-456313"
}
```
