# Issue 34249: Fix documentation regarding number of edges of Turan graphs

archive/issues_034012.json:
```json
{
    "assignees": [],
    "body": "In `sage.graphs.graph_generators.GraphGenerators.TuranGraph`, the documentation of the function `graphs.TuranGraph(n, r)` states:\n\nThe Turan graph `T(n,r)` has `\\lfloor \\frac{(r-1)(n^2)}{2r} \\rfloor` edges.\n\nThis statement is not correct, as illustrated by the Turan graph with `n=12` vertices and `r=8` parts, which has 62 edges, whereas the expression evaluates to 63. The statement was widely thought to be true, but it has been debunked. See [math.stackexchange](https://math.stackexchange.com/questions/1213226/deriving-the-number-of-edges-in-a-tur%C3%A1n-graph) for more details.\n\nCC:  @sagetrac-mcognetta @dcoudert\n\nKeywords: **Turan graph, number of edges**\n\nBranch/Commit: **[1829a77](https://github.com/sagemath/sagetrac-mirror/commit/1829a77cd488aaf3b616a17eff29a3e9241d4dc1)**\n\nReviewer: **Serge Gaspers**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/34249_\n\n",
    "closed_at": "2022-08-04T22:45:07Z",
    "created_at": "2022-07-30T13:44:48Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix documentation regarding number of edges of Turan graphs",
    "type": "issue",
    "updated_at": "2022-08-04T22:45:07Z",
    "url": "https://github.com/sagemath/sage/issues/34249",
    "user": "https://github.com/serggasp"
}
```
In `sage.graphs.graph_generators.GraphGenerators.TuranGraph`, the documentation of the function `graphs.TuranGraph(n, r)` states:

The Turan graph `T(n,r)` has `\lfloor \frac{(r-1)(n^2)}{2r} \rfloor` edges.

This statement is not correct, as illustrated by the Turan graph with `n=12` vertices and `r=8` parts, which has 62 edges, whereas the expression evaluates to 63. The statement was widely thought to be true, but it has been debunked. See [math.stackexchange](https://math.stackexchange.com/questions/1213226/deriving-the-number-of-edges-in-a-tur%C3%A1n-graph) for more details.

CC:  @sagetrac-mcognetta @dcoudert

Keywords: **Turan graph, number of edges**

Branch/Commit: **[1829a77](https://github.com/sagemath/sagetrac-mirror/commit/1829a77cd488aaf3b616a17eff29a3e9241d4dc1)**

Reviewer: **Serge Gaspers**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/34249_





---

archive/issue_events_449971.json:
```json
{
    "actor": "https://github.com/serggasp",
    "created_at": "2022-07-30T13:44:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/34249",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34249#event-449971"
}
```



---

archive/issue_events_449972.json:
```json
{
    "actor": "https://github.com/serggasp",
    "created_at": "2022-07-30T13:44:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34249",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34249#event-449972"
}
```



---

archive/issue_events_449973.json:
```json
{
    "actor": "https://github.com/serggasp",
    "created_at": "2022-07-30T13:44:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34249",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34249#event-449973"
}
```



---

archive/issue_events_449974.json:
```json
{
    "actor": "https://github.com/serggasp",
    "created_at": "2022-07-30T13:44:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34249",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34249#event-449974"
}
```



---

archive/issue_comments_556124.json:
```json
{
    "body": "Commit: **[1829a77](https://github.com/sagemath/sagetrac-mirror/commit/1829a77cd488aaf3b616a17eff29a3e9241d4dc1)**",
    "created_at": "2022-07-30T14:38:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34249",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34249#issuecomment-556124",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[1829a77](https://github.com/sagemath/sagetrac-mirror/commit/1829a77cd488aaf3b616a17eff29a3e9241d4dc1)**



---

archive/issue_comments_556125.json:
```json
{
    "body": "Branch: **[public/graphs/34249_turan](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/34249_turan)**",
    "created_at": "2022-07-30T14:38:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34249",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34249#issuecomment-556125",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[public/graphs/34249_turan](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/34249_turan)**



---

archive/issue_events_449975.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2022-07-30T14:38:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34249",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34249#event-449975"
}
```



---

archive/issue_comments_556126.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nThank you for reporting this error in the documentation.\n\nI have corrected the documentation (and improved the coding style).\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1829a77cd488aaf3b616a17eff29a3e9241d4dc1\">1829a77</a></td><td><code>trac #34249: correct number of edges of the Turan graph</code></td></tr></table>\n",
    "created_at": "2022-07-30T14:38:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34249",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34249#issuecomment-556126",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:1" align="right">Comment 1</div>

Thank you for reporting this error in the documentation.

I have corrected the documentation (and improved the coding style).

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1829a77cd488aaf3b616a17eff29a3e9241d4dc1">1829a77</a></td><td><code>trac #34249: correct number of edges of the Turan graph</code></td></tr></table>




---

archive/issue_events_449976.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2022-07-30T22:18:30Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/34249",
    "title_is": "Fix documentation regarding number of edges of Turan graphs",
    "title_was": "Turan Graph generator documentation error relating to the number of edges of Turan graphs",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34249#event-449976"
}
```



---

archive/issue_comments_556127.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nThank you. The modification looks good to me.",
    "created_at": "2022-07-31T12:29:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34249",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34249#issuecomment-556127",
    "user": "https://github.com/serggasp"
}
```

<div id="comment:3" align="right">Comment 3</div>

Thank you. The modification looks good to me.



---

archive/issue_events_449977.json:
```json
{
    "actor": "https://github.com/serggasp",
    "created_at": "2022-07-31T12:29:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34249",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34249#event-449977"
}
```



---

archive/issue_events_449978.json:
```json
{
    "actor": "https://github.com/serggasp",
    "created_at": "2022-07-31T12:29:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34249",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34249#event-449978"
}
```



---

archive/issue_comments_556128.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nauthor/reviewer name",
    "created_at": "2022-07-31T16:27:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34249",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34249#issuecomment-556128",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:4" align="right">Comment 4</div>

author/reviewer name



---

archive/issue_comments_556129.json:
```json
{
    "body": "Changed author from **Serge Gaspers** to **David Coudert**",
    "created_at": "2022-07-31T16:30:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34249",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34249#issuecomment-556129",
    "user": "https://github.com/dcoudert"
}
```

Changed author from **Serge Gaspers** to **David Coudert**



---

archive/issue_comments_556130.json:
```json
{
    "body": "Reviewer: **Serge Gaspers**",
    "created_at": "2022-07-31T16:30:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34249",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34249#issuecomment-556130",
    "user": "https://github.com/dcoudert"
}
```

Reviewer: **Serge Gaspers**



---

archive/issue_events_449979.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-08-04T22:45:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34249",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34249#event-449979"
}
```



---

archive/issue_events_449980.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "74eaa6fff7f921c3cbd8c3b06f69381477df25a7",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-08-04T22:45:07Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/34249",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34249#event-449980"
}
```



---

archive/issue_comments_556131.json:
```json
{
    "body": "Changed branch from **[public/graphs/34249_turan](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/34249_turan)** to **[1829a77](https://github.com/sagemath/sagetrac-mirror/commit/1829a77cd488aaf3b616a17eff29a3e9241d4dc1)**",
    "created_at": "2022-08-04T22:45:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34249",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34249#issuecomment-556131",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/graphs/34249_turan](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/34249_turan)** to **[1829a77](https://github.com/sagemath/sagetrac-mirror/commit/1829a77cd488aaf3b616a17eff29a3e9241d4dc1)**
