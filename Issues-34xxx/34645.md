# Issue 34645: fix more W605 in rings

archive/issues_034408.json:
```json
{
    "assignees": [],
    "body": "after #34639 \n\nCC:  @tscrim @slel @kwankyu @kliem\n\nBranch/Commit: **[47a089d](https://github.com/sagemath/sagetrac-mirror/commit/47a089da18197117af8e946659facd5702f942f7)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/34645_\n\n",
    "closed_at": "2022-10-17T22:43:29Z",
    "created_at": "2022-10-10T10:26:52Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/refactoring",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix more W605 in rings",
    "type": "issue",
    "updated_at": "2022-10-17T22:43:29Z",
    "url": "https://github.com/sagemath/sage/issues/34645",
    "user": "https://github.com/fchapoton"
}
```
after #34639 

CC:  @tscrim @slel @kwankyu @kliem

Branch/Commit: **[47a089d](https://github.com/sagemath/sagetrac-mirror/commit/47a089da18197117af8e946659facd5702f942f7)**

Reviewer: **Travis Scrimshaw**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/34645_





---

archive/issue_events_454081.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-10-10T10:26:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/34645",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34645#event-454081"
}
```



---

archive/issue_events_454082.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-10-10T10:26:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34645",
    "label": "https://github.com/sagemath/sage/labels/refactoring",
    "label_color": "696969",
    "label_name": "refactoring",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34645#event-454082"
}
```



---

archive/issue_events_454083.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-10-10T10:26:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34645",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34645#event-454083"
}
```



---

archive/issue_events_454084.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-10-10T10:26:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34645",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34645#event-454084"
}
```



---

archive/issue_comments_562426.json:
```json
{
    "body": "Branch: **[u/chapoton/34645](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/34645)**",
    "created_at": "2022-10-10T10:27:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34645#issuecomment-562426",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/34645](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/34645)**



---

archive/issue_comments_562427.json:
```json
{
    "body": "Commit: **[47a089d](https://github.com/sagemath/sagetrac-mirror/commit/47a089da18197117af8e946659facd5702f942f7)**",
    "created_at": "2022-10-10T10:27:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34645#issuecomment-562427",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[47a089d](https://github.com/sagemath/sagetrac-mirror/commit/47a089da18197117af8e946659facd5702f942f7)**



---

archive/issue_events_454085.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-10-10T10:27:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34645",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34645#event-454085"
}
```



---

archive/issue_comments_562428.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/47a089da18197117af8e946659facd5702f942f7\">47a089d</a></td><td><code>fix more W605 in rings/</code></td></tr></table>\n",
    "created_at": "2022-10-10T10:27:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34645#issuecomment-562428",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/47a089da18197117af8e946659facd5702f942f7">47a089d</a></td><td><code>fix more W605 in rings/</code></td></tr></table>




---

archive/issue_comments_562429.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nbot and lights are morally green, so please review (should be easy)",
    "created_at": "2022-10-11T08:39:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34645#issuecomment-562429",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

bot and lights are morally green, so please review (should be easy)



---

archive/issue_events_454086.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-10-11T08:40:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34645",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34645#event-454086"
}
```



---

archive/issue_events_454087.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-10-11T08:40:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34645",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34645#event-454087"
}
```



---

archive/issue_comments_562430.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLGTM.",
    "created_at": "2022-10-11T08:40:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34645#issuecomment-562430",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:3" align="right">Comment 3</div>

LGTM.



---

archive/issue_events_454088.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-10-11T09:18:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34645",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34645#event-454088"
}
```



---

archive/issue_events_454089.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-10-11T09:18:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34645",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34645#event-454089"
}
```



---

archive/issue_comments_562431.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nMerge failure on top of:\n\nc2baae7e947 Trac #34640: fix some pep8 in words/\n\n4c44e082e69 Trac #34632: wrappers for acb_poly_[rl]gamma_series\n\nbbd3c50d9c4 Trac #34629: Use the kbd role in the doc\n\n561a8b88870 Trac #34625: Refresh the file free_quadratic_module\n\n6eb9a59c7bc Trac #34622: partial cleanup in partition.py\n\nb75e452282b Trac #34620: pep cleanup for the file weyl_group.py\n\nd22e4b2de0f Trac #34619: fix some details in doc of charpoly_frobenius\n\n1068d8fd597 Trac #34613: fix some typos\n\nbe07ee91bc3 Trac #34606: fix doc with respect to a change in behavior for tikz method of polyhedron (since 9.7 -> since 9.8)\n\n2d712830271 Trac #34602: add some missing EXAMPLES\n\n953c8949c60 Trac #34599: add helper function to compute all monomials of a given degree\n\nf6c53f3520c Trac #34594: Fix error in sage.misc.latex.view doctest\n\n15696c2bd7a Trac #34585: pep8 cleanup in algebras/letterplace pyx files\n\n015bd808c45 Trac #34463: make PolynomialQuotientRing_generic inherit from QuotientRing_generic\n\n098f47f3898 Trac #34394: pycodestyle cleanup in src/sage/graphs/generic_graph.py (part 6)\n\naa4dc01bf54 Trac #34123: Fast initialization for edge connectivity\n\n66a62faa88b Trac #34647: some details in permutation groups\n\n1a1d67ddeea Trac #34644: minor details in schemes\n\n3384309eae7 Trac #34639: fix some W605 in pyx files in rings\n\nc448d77ee0b Trac #34618: use libgap in dual abelian group elements\n\n33a0f7adac1 Trac #34617: fix and activate pycodestyle W391\n\n75636756b8f Trac #34615: Update to sphinx 5.2\n\n685a07c0fda Trac #34614: add missing EllipticCurveHom methods to \u221a\u00e9lu isogenies\n\na7dac85cb87 Trac #34612: 1 internet doctest failing in graphs/isgci.py\n\nd9c40919205 Trac #34579: Faster iterator for planar set partitions\n\n321f97084f9 Trac #34552: more testsuites for lazy series rings\n\n87626b0bcd0 Trac #34470: categories of lazy series\n\n77c6eed67ee Trac #34410: make new composite elliptic-curve isogenies the default\n\n2356379d701 Trac #33969: Implement missing KnotInfo wrappers for polynomial invariants\n\na9abd3b2da4 Trac #32874: New option \"./configure --enable-wheels\"\n\n84ecc3f3575 Trac #32686: points_of_bounded_height for projective space is incorrect\n\n364b605217c Trac #29413: Meta-ticket: Upgrade cddlib, fix cddlib header search for packages latte_int, gfan, topcom\n\nbe3b5abefd1 Trac #34609: 5 internet doctests failing in oeis\n\nf1d4c32fba7 Trac #34607: 2 doctests failures with external software octave\n\n792d877c428 Trac #34603: Fix singular spkg-configure\n\n12539de55cb Trac #34268: Reduce R to a dummy package, upgrade rpy2 to 3.4.5\n\nac6888e735a Trac #33680: moving _repr_fixups list from doctest/parsing.py header to method do_fixup\n\nb670d032bf5 Trac #33597: Wrong computation of Hilbert polynomials\n\n6b198bd73cd Trac #32367: Replace Lazy Power Series in species directory\n\n00ebfa91d14 Trac #32354: Fix integral(1/max_symbolic(x, 1)^2, x, 0, oo, algorithm='giac')\n\nad120da7d6a Trac #31588: Update Normaliz to 3.9.x, pynormaliz to 2.17, e-antic to 1.x, add optional package antic\n\nfd47b9927ea Trac #14222: Coercion/conversion from Cone and other objects to Polyhedron\n\nde296798b65 Trac #34623: Fix broken gitpod\n\ne91fd4a93da Trac #34604: latte_int: Fix up patch\n\n1758b6322dc Trac #34496: disallow PARI 2.15 from the system, until giac is fixed\n\n54cd6fe6de5 Updated [SageMath](../wiki/SageMath) version to 9.8.beta1\n\n\n\nreviewer '' does not look right",
    "created_at": "2022-10-11T09:18:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34645#issuecomment-562431",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:4" align="right">Comment 4</div>

Merge failure on top of:

c2baae7e947 Trac #34640: fix some pep8 in words/

4c44e082e69 Trac #34632: wrappers for acb_poly_[rl]gamma_series

bbd3c50d9c4 Trac #34629: Use the kbd role in the doc

561a8b88870 Trac #34625: Refresh the file free_quadratic_module

6eb9a59c7bc Trac #34622: partial cleanup in partition.py

b75e452282b Trac #34620: pep cleanup for the file weyl_group.py

d22e4b2de0f Trac #34619: fix some details in doc of charpoly_frobenius

1068d8fd597 Trac #34613: fix some typos

be07ee91bc3 Trac #34606: fix doc with respect to a change in behavior for tikz method of polyhedron (since 9.7 -> since 9.8)

2d712830271 Trac #34602: add some missing EXAMPLES

953c8949c60 Trac #34599: add helper function to compute all monomials of a given degree

f6c53f3520c Trac #34594: Fix error in sage.misc.latex.view doctest

15696c2bd7a Trac #34585: pep8 cleanup in algebras/letterplace pyx files

015bd808c45 Trac #34463: make PolynomialQuotientRing_generic inherit from QuotientRing_generic

098f47f3898 Trac #34394: pycodestyle cleanup in src/sage/graphs/generic_graph.py (part 6)

aa4dc01bf54 Trac #34123: Fast initialization for edge connectivity

66a62faa88b Trac #34647: some details in permutation groups

1a1d67ddeea Trac #34644: minor details in schemes

3384309eae7 Trac #34639: fix some W605 in pyx files in rings

c448d77ee0b Trac #34618: use libgap in dual abelian group elements

33a0f7adac1 Trac #34617: fix and activate pycodestyle W391

75636756b8f Trac #34615: Update to sphinx 5.2

685a07c0fda Trac #34614: add missing EllipticCurveHom methods to √élu isogenies

a7dac85cb87 Trac #34612: 1 internet doctest failing in graphs/isgci.py

d9c40919205 Trac #34579: Faster iterator for planar set partitions

321f97084f9 Trac #34552: more testsuites for lazy series rings

87626b0bcd0 Trac #34470: categories of lazy series

77c6eed67ee Trac #34410: make new composite elliptic-curve isogenies the default

2356379d701 Trac #33969: Implement missing KnotInfo wrappers for polynomial invariants

a9abd3b2da4 Trac #32874: New option "./configure --enable-wheels"

84ecc3f3575 Trac #32686: points_of_bounded_height for projective space is incorrect

364b605217c Trac #29413: Meta-ticket: Upgrade cddlib, fix cddlib header search for packages latte_int, gfan, topcom

be3b5abefd1 Trac #34609: 5 internet doctests failing in oeis

f1d4c32fba7 Trac #34607: 2 doctests failures with external software octave

792d877c428 Trac #34603: Fix singular spkg-configure

12539de55cb Trac #34268: Reduce R to a dummy package, upgrade rpy2 to 3.4.5

ac6888e735a Trac #33680: moving _repr_fixups list from doctest/parsing.py header to method do_fixup

b670d032bf5 Trac #33597: Wrong computation of Hilbert polynomials

6b198bd73cd Trac #32367: Replace Lazy Power Series in species directory

00ebfa91d14 Trac #32354: Fix integral(1/max_symbolic(x, 1)^2, x, 0, oo, algorithm='giac')

ad120da7d6a Trac #31588: Update Normaliz to 3.9.x, pynormaliz to 2.17, e-antic to 1.x, add optional package antic

fd47b9927ea Trac #14222: Coercion/conversion from Cone and other objects to Polyhedron

de296798b65 Trac #34623: Fix broken gitpod

e91fd4a93da Trac #34604: latte_int: Fix up patch

1758b6322dc Trac #34496: disallow PARI 2.15 from the system, until giac is fixed

54cd6fe6de5 Updated [SageMath](../wiki/SageMath) version to 9.8.beta1



reviewer '' does not look right



---

archive/issue_comments_562432.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2022-10-11T18:41:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34645#issuecomment-562432",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_454090.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-10-11T18:41:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34645",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34645#event-454090"
}
```



---

archive/issue_events_454091.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-10-11T18:41:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34645",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34645#event-454091"
}
```



---

archive/issue_comments_562433.json:
```json
{
    "body": "Changed branch from **[u/chapoton/34645](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/34645)** to **[47a089d](https://github.com/sagemath/sagetrac-mirror/commit/47a089da18197117af8e946659facd5702f942f7)**",
    "created_at": "2022-10-17T22:43:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34645",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34645#issuecomment-562433",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/34645](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/34645)** to **[47a089d](https://github.com/sagemath/sagetrac-mirror/commit/47a089da18197117af8e946659facd5702f942f7)**



---

archive/issue_events_454092.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-10-17T22:43:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34645",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34645#event-454092"
}
```



---

archive/issue_events_454093.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "eab83caf777858ec885f5ea9f1f08f8fcc40058b",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-10-17T22:43:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/34645",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34645#event-454093"
}
```
