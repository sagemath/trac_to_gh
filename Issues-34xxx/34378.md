...
```json
{
    "body": "That ticket has more issues than this one. And I imagine this one is easier to fix and agree upon \"the right way\".",
...
```json
{
    "body": "Since we now use rpy2 rather than the custom thing we had a decade ago, that might be a little tougher.  (But really just catching it there is probably the \"easy, if not \"right\", way.)  I remember `NULL` returns from various R commands for many years, so I don't think this is new.  Sorry I can't help.",
