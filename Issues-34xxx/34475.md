...
<a id='comment:16'></a>It is a generic implementation and it is the best attempt at doing something without making it an ``@`abstract_method(optional=True)`. The `SubmoduleWithBasis` should, in principle, work with anything in the category. Unfortunately I don’t think we are quite at that reality yet.
...
<a id='comment:24'></a>Replying to [Matthias Köppe](#comment%3A20):
> Is there a way to shorten "Category of subobjects of modules with basis over Integer Ring" to "Category of submodules with basis over Integer Ring"?
...
<a id='comment:25'></a>Replying to [Matthias Köppe](#comment%3A21):
> There a few failures of `_test_construction` tests.
> At least one of them is a case where the equality of a `UniqueRepresentation` is too strict:
...
<a id='comment:26'></a>Replying to [Travis Scrimshaw](#comment%3A25):
> it is not fair to call it too strict.
