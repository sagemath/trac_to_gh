...
```json
{
    "body": "<a id='comment:4'></a>I think the infinite iterators in `src/sage/modular/hecke/algebra.py` should be replaced by an `EnumeratedSetFromIterator`",
...
<a id='comment:4'></a>I think the infinite iterators in `src/sage/modular/hecke/algebra.py` should be replaced by an `EnumeratedSetFromIterator`
...
```json
{
    "body": "<a id='comment:5'></a>Also all `gens_dict` methods should probably be replaced by `gens` returning a `Family`",
...
<a id='comment:5'></a>Also all `gens_dict` methods should probably be replaced by `gens` returning a `Family`
...
```json
{
    "body": "<a id='comment:6'></a>And +1 on changing all lists to tuples.",
...
<a id='comment:6'></a>And +1 on changing all lists to tuples.
...
```json
{
    "body": "<a id='comment:7'></a>partly handled in #34500",
...
<a id='comment:7'></a>partly handled in #34500
...
```json
{
    "body": "<a id='comment:8'></a>here is some partial work on the case of rings/\n\n---\nNew commits:",
    "created_at": "2022-09-06T19:20:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34120",
    "type": "issue_comment",
...
<a id='comment:8'></a>here is some partial work on the case of rings/
