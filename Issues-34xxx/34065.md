# Issue 34065: pycodestyle cleanup in src/sage/graphs/bliss.pyx

archive/issues_033828.json:
```json
{
    "assignees": [],
    "body": "With this tickets, it remains\n\n```\n4       E211 whitespace before '('\n32      E225 missing whitespace around operator\n4       E227 missing whitespace around bitwise or shift operator\n```\ndue to the definition of cppclass and the manipulation of pointers.\n\nBranch/Commit: **[8f75741](https://github.com/sagemath/sagetrac-mirror/commit/8f7574169b6743890bc32b58d9d2afcb4a5d6375)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/34065_\n\n",
    "closed_at": "2022-07-11T21:55:53Z",
    "created_at": "2022-06-24T16:55:02Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "pycodestyle cleanup in src/sage/graphs/bliss.pyx",
    "type": "issue",
    "updated_at": "2022-07-11T21:55:53Z",
    "url": "https://github.com/sagemath/sage/issues/34065",
    "user": "https://github.com/dcoudert"
}
```
With this tickets, it remains

```
4       E211 whitespace before '('
32      E225 missing whitespace around operator
4       E227 missing whitespace around bitwise or shift operator
```
due to the definition of cppclass and the manipulation of pointers.

Branch/Commit: **[8f75741](https://github.com/sagemath/sagetrac-mirror/commit/8f7574169b6743890bc32b58d9d2afcb4a5d6375)**

Reviewer: **Frédéric Chapoton**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/34065_





---

archive/issue_events_448096.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2022-06-24T16:55:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/34065",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34065#event-448096"
}
```



---

archive/issue_events_448097.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2022-06-24T16:55:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34065",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34065#event-448097"
}
```



---

archive/issue_events_448098.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2022-06-24T16:55:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34065",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34065#event-448098"
}
```



---

archive/issue_events_448099.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2022-06-24T16:55:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34065",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34065#event-448099"
}
```



---

archive/issue_events_448100.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2022-06-24T16:56:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34065",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34065#event-448100"
}
```



---

archive/issue_comments_553232.json:
```json
{
    "body": "Commit: **[8f75741](https://github.com/sagemath/sagetrac-mirror/commit/8f7574169b6743890bc32b58d9d2afcb4a5d6375)**",
    "created_at": "2022-06-24T16:56:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34065",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34065#issuecomment-553232",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[8f75741](https://github.com/sagemath/sagetrac-mirror/commit/8f7574169b6743890bc32b58d9d2afcb4a5d6375)**



---

archive/issue_comments_553233.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8f7574169b6743890bc32b58d9d2afcb4a5d6375\">8f75741</a></td><td><code>trac #34065: clean bliss.pyx</code></td></tr></table>\n",
    "created_at": "2022-06-24T16:56:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34065",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34065#issuecomment-553233",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8f7574169b6743890bc32b58d9d2afcb4a5d6375">8f75741</a></td><td><code>trac #34065: clean bliss.pyx</code></td></tr></table>




---

archive/issue_comments_553234.json:
```json
{
    "body": "Branch: **[public/graphs/34065_bliss](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/34065_bliss)**",
    "created_at": "2022-06-24T16:56:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34065",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34065#issuecomment-553234",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[public/graphs/34065_bliss](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/34065_bliss)**



---

archive/issue_comments_553235.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2022-06-24T19:39:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34065",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34065#issuecomment-553235",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_553236.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\noui",
    "created_at": "2022-06-24T19:39:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34065",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34065#issuecomment-553236",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

oui



---

archive/issue_events_448101.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-06-24T19:39:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34065",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34065#event-448101"
}
```



---

archive/issue_events_448102.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-06-24T19:39:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34065",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34065#event-448102"
}
```



---

archive/issue_comments_553237.json:
```json
{
    "body": "Changed branch from **[public/graphs/34065_bliss](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/34065_bliss)** to **[8f75741](https://github.com/sagemath/sagetrac-mirror/commit/8f7574169b6743890bc32b58d9d2afcb4a5d6375)**",
    "created_at": "2022-07-11T21:55:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34065",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34065#issuecomment-553237",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/graphs/34065_bliss](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/34065_bliss)** to **[8f75741](https://github.com/sagemath/sagetrac-mirror/commit/8f7574169b6743890bc32b58d9d2afcb4a5d6375)**



---

archive/issue_events_448103.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-07-11T21:55:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34065",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34065#event-448103"
}
```



---

archive/issue_events_448104.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "798adaa5a3bffe5f4f38e444aca06cf79571326f",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-07-11T21:55:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/34065",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34065#event-448104"
}
```
