...
```json
{
    "body": "As a workaround, you can use `case 1r: ...`. I agree that the preparser should be fixed to handle this situation, though.",
...
```json
{
    "body": "I think this should be solved by defining `__match_args__` for various of our classes, see https://peps.python.org/pep-0634/#class-patterns",
...
```json
{
    "body": "How broadly could this be done? For `SageObject`? `Element`?",
...
```json
{
    "body": "In fact I get this behavior in plain Python. I guess this is binding the undefined symbol `c` to `x` and proceeding from there, so this is not a problem with the above patch. However, it still doesn't work:\n\n```\nsage: x = 3\nsage: match x:\n....:     case 1: print('hello')\n....:     case 3.2: print('world')\n....: \n---------------------------------------------------------------------------\nTypeError                                 Traceback (most recent call last)\nInput In [21], in <cell line: 1>()\n      1 match x:\n      2     case Integer(1): print('hello')\n----> 3     case RealNumber('3.2'): print('world')\n\nTypeError: called match pattern must be a type\n```\nIt accepts Sage `Integer`s but not `RealNumber`s.",
...
```json
{
    "body": "`c` becomes defined as a parameter in the match pattern.",
...
```json
{
    "body": "The difference is that `Integer` is a class, but `RealNumber` is only a function.",
...
```json
{
    "body": "I think we would be able to handle `RealNumber` by changing it to a class with a `__classcall__` method.",
...
```json
{
    "body": "That may be more of a job than I have time for right now.",
