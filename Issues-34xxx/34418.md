# Issue 34418: Fix coinv for nonattacking fillings

archive/issues_034181.json:
```json
{
    "assignees": [],
    "body": "Currently the following seems wrong: \n\n``` \nsage: nn = NonattackingFillings([0,0,1],PermutationGroupElement([2,1,3]))[0]; nn\n[[2], [1], [3, 1]]\nsage: nn.coinv()\n1\n```\nThe output should be 0\n\nIt seems that specifying a permutation was added later by sdenton but coinv was not adapted to that change.\n\n\n\n\nCC:  @hivert @saraedum @slel @amruthap1 @sdenton4\n\nBranch/Commit: **[public/34418](https://github.com/sagemath/sagetrac-mirror/tree/public/34418) @ [7e10a7e](https://github.com/sagemath/sagetrac-mirror/commit/7e10a7ee15d15fd2511ec88b2edb124462708e04)**\n\nAuthor: **Aritra Bhattacharya, ...**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/34418_\n\n",
    "created_at": "2022-08-24T06:38:16Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix coinv for nonattacking fillings",
    "type": "issue",
    "updated_at": "2023-01-08T13:48:02Z",
    "url": "https://github.com/sagemath/sage/issues/34418",
    "user": "https://github.com/aritra-bhattacharya-0"
}
```
Currently the following seems wrong: 

``` 
sage: nn = NonattackingFillings([0,0,1],PermutationGroupElement([2,1,3]))[0]; nn
[[2], [1], [3, 1]]
sage: nn.coinv()
1
```
The output should be 0

It seems that specifying a permutation was added later by sdenton but coinv was not adapted to that change.




CC:  @hivert @saraedum @slel @amruthap1 @sdenton4

Branch/Commit: **[public/34418](https://github.com/sagemath/sagetrac-mirror/tree/public/34418) @ [7e10a7e](https://github.com/sagemath/sagetrac-mirror/commit/7e10a7ee15d15fd2511ec88b2edb124462708e04)**

Author: **Aritra Bhattacharya, ...**

_Issue created by migration from https://trac.sagemath.org/ticket/34418_





---

archive/issue_events_451874.json:
```json
{
    "actor": "https://github.com/aritra-bhattacharya-0",
    "created_at": "2022-08-24T06:38:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/34418",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34418#event-451874"
}
```



---

archive/issue_events_451875.json:
```json
{
    "actor": "https://github.com/aritra-bhattacharya-0",
    "created_at": "2022-08-24T06:38:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34418",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34418#event-451875"
}
```



---

archive/issue_events_451876.json:
```json
{
    "actor": "https://github.com/aritra-bhattacharya-0",
    "created_at": "2022-08-24T06:38:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34418",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34418#event-451876"
}
```



---

archive/issue_events_451877.json:
```json
{
    "actor": "https://github.com/aritra-bhattacharya-0",
    "created_at": "2022-08-24T06:38:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34418",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34418#event-451877"
}
```



---

archive/issue_comments_559108.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -8,3 +8,7 @@\n ```\n The output should be 0\n \n+It seems that specifying a permutation was added later by sdenton but coinv was not adapted to that change.\n+\n+\n+\n``````\n",
    "created_at": "2022-08-24T07:16:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34418",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34418#issuecomment-559108",
    "user": "https://github.com/aritra-bhattacharya-0"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -8,3 +8,7 @@
 ```
 The output should be 0
 
+It seems that specifying a permutation was added later by sdenton but coinv was not adapted to that change.
+
+
+
``````




---

archive/issue_comments_559109.json:
```json
{
    "body": "Branch: **[public/34418](https://github.com/sagemath/sagetrac-mirror/tree/public/34418)**",
    "created_at": "2022-08-24T07:38:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34418",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34418#issuecomment-559109",
    "user": "https://github.com/aritra-bhattacharya-0"
}
```

Branch: **[public/34418](https://github.com/sagemath/sagetrac-mirror/tree/public/34418)**



---

archive/issue_comments_559110.json:
```json
{
    "body": "Commit: **[7e10a7e](https://github.com/sagemath/sagetrac-mirror/commit/7e10a7ee15d15fd2511ec88b2edb124462708e04)**",
    "created_at": "2022-08-24T07:38:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34418",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34418#issuecomment-559110",
    "user": "https://github.com/aritra-bhattacharya-0"
}
```

Commit: **[7e10a7e](https://github.com/sagemath/sagetrac-mirror/commit/7e10a7ee15d15fd2511ec88b2edb124462708e04)**



---

archive/issue_comments_559111.json:
```json
{
    "body": "Author: **Aritra Bhattacharya, ...**",
    "created_at": "2022-08-24T07:38:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34418",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34418#issuecomment-559111",
    "user": "https://github.com/aritra-bhattacharya-0"
}
```

Author: **Aritra Bhattacharya, ...**



---

archive/issue_comments_559112.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7e10a7ee15d15fd2511ec88b2edb124462708e04\">7e10a7e</a></td><td><code>Add coinv tests from literature</code></td></tr></table>\n",
    "created_at": "2022-08-24T07:38:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34418",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34418#issuecomment-559112",
    "user": "https://github.com/aritra-bhattacharya-0"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7e10a7ee15d15fd2511ec88b2edb124462708e04">7e10a7e</a></td><td><code>Add coinv tests from literature</code></td></tr></table>




---

archive/issue_comments_559113.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nAdded some tests from the literature that are all failing in the current implementation",
    "created_at": "2022-08-24T07:39:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34418",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34418#issuecomment-559113",
    "user": "https://github.com/aritra-bhattacharya-0"
}
```

<div id="comment:3" align="right">Comment 3</div>

Added some tests from the literature that are all failing in the current implementation



---

archive/issue_events_451878.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/34418",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34418#event-451878"
}
```



---

archive/issue_events_451879.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/34418",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34418#event-451879"
}
```



---

archive/issue_comments_559114.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nyou should set the status to \"needs_review\" if this is ready",
    "created_at": "2023-01-08T13:48:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34418",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34418#issuecomment-559114",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:5" align="right">Comment 5</div>

you should set the status to "needs_review" if this is ready
