# Issue 34811: rational maps for WeierstrassIsomorphism should not be polynomials

archive/issues_034574.json:
```json
{
    "assignees": [],
    "body": "The methods `.rational_maps()` and `.x_rational_map()` for `WeierstrassIsomorphism` return polynomials, but the same methods for other `EllipticCurveHom` return rational maps (i.e., elements of the fraction field).\n\nFor consistency, these methods should return the same types in all child classes of `EllipticCurveHom`, despite the rational maps actually having denominator `1` in this case.\n\nBranch/Commit: **[public/34811](https://github.com/sagemath/sagetrac-mirror/tree/public/34811) @ [551d007](https://github.com/sagemath/sagetrac-mirror/commit/551d0070ad4de4fc52649bdd62ffbe8706cc2373)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Lorenz Panny**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/34811_\n\n",
    "created_at": "2022-11-30T07:13:40Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20elliptic%20curves",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/positive%20review"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "rational maps for WeierstrassIsomorphism should not be polynomials",
    "type": "issue",
    "updated_at": "2022-12-14T05:07:23Z",
    "url": "https://github.com/sagemath/sage/issues/34811",
    "user": "https://github.com/yyyyx4"
}
```
The methods `.rational_maps()` and `.x_rational_map()` for `WeierstrassIsomorphism` return polynomials, but the same methods for other `EllipticCurveHom` return rational maps (i.e., elements of the fraction field).

For consistency, these methods should return the same types in all child classes of `EllipticCurveHom`, despite the rational maps actually having denominator `1` in this case.

Branch/Commit: **[public/34811](https://github.com/sagemath/sagetrac-mirror/tree/public/34811) @ [551d007](https://github.com/sagemath/sagetrac-mirror/commit/551d0070ad4de4fc52649bdd62ffbe8706cc2373)**

Reviewer: **Travis Scrimshaw**

Author: **Lorenz Panny**

_Issue created by migration from https://trac.sagemath.org/ticket/34811_





---

archive/issue_events_455632.json:
```json
{
    "actor": "https://github.com/yyyyx4",
    "created_at": "2022-11-30T07:13:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/34811",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34811#event-455632"
}
```



---

archive/issue_events_455633.json:
```json
{
    "actor": "https://github.com/yyyyx4",
    "created_at": "2022-11-30T07:13:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34811",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20elliptic%20curves",
    "label_color": "0000ff",
    "label_name": "component: elliptic curves",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34811#event-455633"
}
```



---

archive/issue_events_455634.json:
```json
{
    "actor": "https://github.com/yyyyx4",
    "created_at": "2022-11-30T07:13:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34811",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34811#event-455634"
}
```



---

archive/issue_events_455635.json:
```json
{
    "actor": "https://github.com/yyyyx4",
    "created_at": "2022-11-30T07:13:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34811",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34811#event-455635"
}
```



---

archive/issue_comments_564381.json:
```json
{
    "body": "Commit: **[551d007](https://github.com/sagemath/sagetrac-mirror/commit/551d0070ad4de4fc52649bdd62ffbe8706cc2373)**",
    "created_at": "2022-11-30T07:15:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34811",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34811#issuecomment-564381",
    "user": "https://github.com/yyyyx4"
}
```

Commit: **[551d007](https://github.com/sagemath/sagetrac-mirror/commit/551d0070ad4de4fc52649bdd62ffbe8706cc2373)**



---

archive/issue_comments_564382.json:
```json
{
    "body": "Branch: **[public/34811](https://github.com/sagemath/sagetrac-mirror/tree/public/34811)**",
    "created_at": "2022-11-30T07:15:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34811",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34811#issuecomment-564382",
    "user": "https://github.com/yyyyx4"
}
```

Branch: **[public/34811](https://github.com/sagemath/sagetrac-mirror/tree/public/34811)**



---

archive/issue_comments_564383.json:
```json
{
    "body": "Author: **Lorenz Panny**",
    "created_at": "2022-11-30T07:15:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34811",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34811#issuecomment-564383",
    "user": "https://github.com/yyyyx4"
}
```

Author: **Lorenz Panny**



---

archive/issue_events_455636.json:
```json
{
    "actor": "https://github.com/yyyyx4",
    "created_at": "2022-11-30T07:15:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34811",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34811#event-455636"
}
```



---

archive/issue_events_455637.json:
```json
{
    "actor": "https://github.com/yyyyx4",
    "created_at": "2022-11-30T08:24:13Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/34811",
    "title_is": "rational maps for WeierstrassIsomorphism should not be polynomials",
    "title_was": "rational maps for WeierstrassIsomorphism are polynomials",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34811#event-455637"
}
```



---

archive/issue_comments_564384.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLGTM.",
    "created_at": "2022-12-14T05:07:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34811",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34811#issuecomment-564384",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:3" align="right">Comment 3</div>

LGTM.



---

archive/issue_comments_564385.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2022-12-14T05:07:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34811",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34811#issuecomment-564385",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_455638.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-12-14T05:07:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34811",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34811#event-455638"
}
```



---

archive/issue_events_455639.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-12-14T05:07:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34811",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34811#event-455639"
}
```
