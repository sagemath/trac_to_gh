# Issue 34702: fix deprecated use of PyEval_Call*

archive/issues_034465.json:
```json
{
    "assignees": [],
    "body": "Deprecated since Python 3.9 - https://docs.python.org/3/whatsnew/3.9.html?highlight=pyeval_callobject#changes-in-the-c-api\n\nusing `PyObject_Call` instead\n\nhttps://docs.python.org/3/c-api/call.html#c.PyObject_Call\n\nCC:  @tscrim @kwankyu @kliem @slel\n\nBranch/Commit: **[285c58f](https://github.com/sagemath/sagetrac-mirror/commit/285c58f75903c835647cf1ecbaa5cc220890368d)**\n\nReviewer: **Matthias Koeppe**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/34702_\n\n",
    "closed_at": "2022-11-07T20:25:45Z",
    "created_at": "2022-10-30T12:49:40Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/refactoring",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix deprecated use of PyEval_Call*",
    "type": "issue",
    "updated_at": "2022-11-07T20:25:45Z",
    "url": "https://github.com/sagemath/sage/issues/34702",
    "user": "https://github.com/fchapoton"
}
```
Deprecated since Python 3.9 - https://docs.python.org/3/whatsnew/3.9.html?highlight=pyeval_callobject#changes-in-the-c-api

using `PyObject_Call` instead

https://docs.python.org/3/c-api/call.html#c.PyObject_Call

CC:  @tscrim @kwankyu @kliem @slel

Branch/Commit: **[285c58f](https://github.com/sagemath/sagetrac-mirror/commit/285c58f75903c835647cf1ecbaa5cc220890368d)**

Reviewer: **Matthias Koeppe**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/34702_





---

archive/issue_events_454609.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-10-30T12:49:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/34702",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34702#event-454609"
}
```



---

archive/issue_events_454610.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-10-30T12:49:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34702",
    "label": "https://github.com/sagemath/sage/labels/refactoring",
    "label_color": "696969",
    "label_name": "refactoring",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34702#event-454610"
}
```



---

archive/issue_events_454611.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-10-30T12:49:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34702",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34702#event-454611"
}
```



---

archive/issue_events_454612.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-10-30T12:49:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34702",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34702#event-454612"
}
```



---

archive/issue_comments_563204.json:
```json
{
    "body": "Commit: **[d9de90a](https://github.com/sagemath/sagetrac-mirror/commit/d9de90ace8082a5ac80421e2f57c7898bc4dd991)**",
    "created_at": "2022-10-30T12:50:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34702",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34702#issuecomment-563204",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[d9de90a](https://github.com/sagemath/sagetrac-mirror/commit/d9de90ace8082a5ac80421e2f57c7898bc4dd991)**



---

archive/issue_comments_563205.json:
```json
{
    "body": "Branch: **[u/chapoton/34702](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/34702)**",
    "created_at": "2022-10-30T12:50:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34702",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34702#issuecomment-563205",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/34702](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/34702)**



---

archive/issue_events_454613.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-10-30T12:50:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34702",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34702#event-454613"
}
```



---

archive/issue_comments_563206.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d9de90ace8082a5ac80421e2f57c7898bc4dd991\">d9de90a</a></td><td><code>fix deprecated use of PyEval_Call</code></td></tr></table>\n",
    "created_at": "2022-10-30T12:50:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34702",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34702#issuecomment-563206",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d9de90ace8082a5ac80421e2f57c7898bc4dd991">d9de90a</a></td><td><code>fix deprecated use of PyEval_Call</code></td></tr></table>




---

archive/issue_comments_563207.json:
```json
{
    "body": "Changed commit from **[d9de90a](https://github.com/sagemath/sagetrac-mirror/commit/d9de90ace8082a5ac80421e2f57c7898bc4dd991)** to **[285c58f](https://github.com/sagemath/sagetrac-mirror/commit/285c58f75903c835647cf1ecbaa5cc220890368d)**",
    "created_at": "2022-10-30T13:28:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34702",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34702#issuecomment-563207",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[d9de90a](https://github.com/sagemath/sagetrac-mirror/commit/d9de90ace8082a5ac80421e2f57c7898bc4dd991)** to **[285c58f](https://github.com/sagemath/sagetrac-mirror/commit/285c58f75903c835647cf1ecbaa5cc220890368d)**



---

archive/issue_comments_563208.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/285c58f75903c835647cf1ecbaa5cc220890368d\">285c58f</a></td><td><code>fix detail</code></td></tr></table>\n",
    "created_at": "2022-10-30T13:28:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34702",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34702#issuecomment-563208",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/285c58f75903c835647cf1ecbaa5cc220890368d">285c58f</a></td><td><code>fix detail</code></td></tr></table>




---

archive/issue_events_454614.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-10-30T15:01:12Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/34702",
    "title_is": "fix deprecated use of PyEval_Call*",
    "title_was": "fix deprecated use of PyEvall_Call*",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34702#event-454614"
}
```



---

archive/issue_comments_563209.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\ngreen lights, so please review",
    "created_at": "2022-10-30T19:19:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34702",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34702#issuecomment-563209",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:4" align="right">Comment 4</div>

green lights, so please review



---

archive/issue_comments_563210.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,5 @@\n+Deprecated since Python 3.9 - https://docs.python.org/3/whatsnew/3.9.html?highlight=pyeval_callobject#changes-in-the-c-api\n+\n using `PyObject_Call` instead\n \n https://docs.python.org/3/c-api/call.html#c.PyObject_Call\n``````\n",
    "created_at": "2022-10-30T22:19:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34702",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34702#issuecomment-563210",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,5 @@
+Deprecated since Python 3.9 - https://docs.python.org/3/whatsnew/3.9.html?highlight=pyeval_callobject#changes-in-the-c-api
+
 using `PyObject_Call` instead
 
 https://docs.python.org/3/c-api/call.html#c.PyObject_Call
``````




---

archive/issue_events_454615.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-10-30T22:22:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34702",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34702#event-454615"
}
```



---

archive/issue_events_454616.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-10-30T22:22:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34702",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34702#event-454616"
}
```



---

archive/issue_comments_563211.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2022-10-30T22:22:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34702",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34702#issuecomment-563211",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_events_454617.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-11-07T20:25:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34702",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34702#event-454617"
}
```



---

archive/issue_events_454618.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "dee975299c2417684a494f6d57824580bbfa213d",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-11-07T20:25:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/34702",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34702#event-454618"
}
```



---

archive/issue_comments_563212.json:
```json
{
    "body": "Changed branch from **[u/chapoton/34702](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/34702)** to **[285c58f](https://github.com/sagemath/sagetrac-mirror/commit/285c58f75903c835647cf1ecbaa5cc220890368d)**",
    "created_at": "2022-11-07T20:25:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34702",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34702#issuecomment-563212",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/34702](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/34702)** to **[285c58f](https://github.com/sagemath/sagetrac-mirror/commit/285c58f75903c835647cf1ecbaa5cc220890368d)**
