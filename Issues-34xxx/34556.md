# Issue 34556: improve approximate order on __getitem__ calls

archive/issues_034319.json:
```json
{
    "assignees": [],
    "body": "We improve the approximate order of a stream whenever `Stream_inexact.__getitem__` is called.\n\nThis may slow down `Stream_inexact.__getitem__` but should speed up other operations.\n\nIt must be checked carefully whether this is an overall improvement or not.\n\nDepends on #34552\n\nCC:  @tscrim\n\nKeywords: **LazyPowerSeries**\n\nBranch/Commit: **[u/mantepse/improve_approximate_order_on___getitem___calls](https://github.com/sagemath/sagetrac-mirror/tree/u/mantepse/improve_approximate_order_on___getitem___calls) @ [5a2cb8b](https://github.com/sagemath/sagetrac-mirror/commit/5a2cb8be80981af79580cb5359205a33ffabff02)**\n\nAuthor: **Martin Rubey**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/34556_\n\n",
    "created_at": "2022-09-19T11:48:43Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "improve approximate order on __getitem__ calls",
    "type": "issue",
    "updated_at": "2022-09-19T11:56:51Z",
    "url": "https://github.com/sagemath/sage/issues/34556",
    "user": "https://github.com/mantepse"
}
```
We improve the approximate order of a stream whenever `Stream_inexact.__getitem__` is called.

This may slow down `Stream_inexact.__getitem__` but should speed up other operations.

It must be checked carefully whether this is an overall improvement or not.

Depends on #34552

CC:  @tscrim

Keywords: **LazyPowerSeries**

Branch/Commit: **[u/mantepse/improve_approximate_order_on___getitem___calls](https://github.com/sagemath/sagetrac-mirror/tree/u/mantepse/improve_approximate_order_on___getitem___calls) @ [5a2cb8b](https://github.com/sagemath/sagetrac-mirror/commit/5a2cb8be80981af79580cb5359205a33ffabff02)**

Author: **Martin Rubey**

_Issue created by migration from https://trac.sagemath.org/ticket/34556_





---

archive/issue_events_438654.json:
```json
{
    "actor": "https://github.com/mantepse",
    "created_at": "2022-09-19T11:48:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/34556",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34556#event-438654"
}
```



---

archive/issue_events_438655.json:
```json
{
    "actor": "https://github.com/mantepse",
    "created_at": "2022-09-19T11:48:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34556",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34556#event-438655"
}
```



---

archive/issue_comments_561407.json:
```json
{
    "body": "Branch: **[u/mantepse/improve_approximate_order_on___getitem___calls](https://github.com/sagemath/sagetrac-mirror/tree/u/mantepse/improve_approximate_order_on___getitem___calls)**",
    "created_at": "2022-09-19T11:52:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34556#issuecomment-561407",
    "user": "https://github.com/mantepse"
}
```

Branch: **[u/mantepse/improve_approximate_order_on___getitem___calls](https://github.com/sagemath/sagetrac-mirror/tree/u/mantepse/improve_approximate_order_on___getitem___calls)**



---

archive/issue_comments_561408.json:
```json
{
    "body": "Commit: **[5a2cb8b](https://github.com/sagemath/sagetrac-mirror/commit/5a2cb8be80981af79580cb5359205a33ffabff02)**",
    "created_at": "2022-09-19T11:53:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34556#issuecomment-561408",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[5a2cb8b](https://github.com/sagemath/sagetrac-mirror/commit/5a2cb8be80981af79580cb5359205a33ffabff02)**



---

archive/issue_comments_561409.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5a2cb8be80981af79580cb5359205a33ffabff02\">5a2cb8b</a></td><td><code>improve approximate order when getitem is called</code></td></tr></table>\n",
    "created_at": "2022-09-19T11:53:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34556#issuecomment-561409",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5a2cb8be80981af79580cb5359205a33ffabff02">5a2cb8b</a></td><td><code>improve approximate order when getitem is called</code></td></tr></table>




---

archive/issue_comments_561410.json:
```json
{
    "body": "Dependencies: **#34552**",
    "created_at": "2022-09-19T11:56:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34556#issuecomment-561410",
    "user": "https://github.com/mantepse"
}
```

Dependencies: **#34552**



---

archive/issue_comments_561411.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,5 @@\n+We improve the approximate order of a stream whenever `Stream_inexact.__getitem__` is called.\n \n+This may slow down `Stream_inexact.__getitem__` but should speed up other operations.\n+\n+It must be checked carefully whether this is an overall improvement or not.\n``````\n",
    "created_at": "2022-09-19T11:56:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34556#issuecomment-561411",
    "user": "https://github.com/mantepse"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,5 @@
+We improve the approximate order of a stream whenever `Stream_inexact.__getitem__` is called.
 
+This may slow down `Stream_inexact.__getitem__` but should speed up other operations.
+
+It must be checked carefully whether this is an overall improvement or not.
``````




---

archive/issue_events_438656.json:
```json
{
    "actor": "https://github.com/mantepse",
    "created_at": "2022-09-19T11:56:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34556",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34556#event-438656"
}
```



---

archive/issue_events_438657.json:
```json
{
    "actor": "https://github.com/mantepse",
    "created_at": "2022-09-19T11:56:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34556",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34556#event-438657"
}
```



---

archive/issue_comments_561412.json:
```json
{
    "body": "Author: **Martin Rubey**",
    "created_at": "2022-09-19T11:56:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34556#issuecomment-561412",
    "user": "https://github.com/mantepse"
}
```

Author: **Martin Rubey**



---

archive/issue_comments_561413.json:
```json
{
    "body": "Changed keywords from **** to **LazyPowerSeries**",
    "created_at": "2022-09-19T11:56:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34556",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34556#issuecomment-561413",
    "user": "https://github.com/mantepse"
}
```

Changed keywords from **** to **LazyPowerSeries**
