# Issue 34270: .gitpod.yml: Do not hardcode the workspace name as sagetrac-mirror

archive/issues_034033.json:
```json
{
    "assignees": [],
    "body": "For example https://gitpod.io/#https://github.com/sagemath/sage, which appears as a link in our manuals, for instance,\n\n- find the link \"going to Gitpod\" in https://113a817a9b18c078a108e31522579eab1afac19c--sagemath-tobias.netlify.app/developer/index.html\n\ndoes not work because the workspace directory is `/workspace/sage`, not `/workspace/sagetrac-mirror`\n\n\nCC:  @tobiasdiez @dimpase\n\nBranch/Commit: **[eb839d4](https://github.com/sagemath/sagetrac-mirror/commit/eb839d4775e208ab0ee64f5380cb80ed68664a8c)**\n\nReviewer: **Kwankyu Lee**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/34270_\n\n",
    "closed_at": "2022-08-29T11:24:38Z",
    "created_at": "2022-08-03T21:18:27Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": ".gitpod.yml: Do not hardcode the workspace name as sagetrac-mirror",
    "type": "issue",
    "updated_at": "2022-08-29T11:24:38Z",
    "url": "https://github.com/sagemath/sage/issues/34270",
    "user": "https://github.com/mkoeppe"
}
```
For example https://gitpod.io/#https://github.com/sagemath/sage, which appears as a link in our manuals, for instance,

- find the link "going to Gitpod" in https://113a817a9b18c078a108e31522579eab1afac19c--sagemath-tobias.netlify.app/developer/index.html

does not work because the workspace directory is `/workspace/sage`, not `/workspace/sagetrac-mirror`


CC:  @tobiasdiez @dimpase

Branch/Commit: **[eb839d4](https://github.com/sagemath/sagetrac-mirror/commit/eb839d4775e208ab0ee64f5380cb80ed68664a8c)**

Reviewer: **Kwankyu Lee**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/34270_





---

archive/issue_events_450219.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-03T21:18:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/34270",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34270#event-450219"
}
```



---

archive/issue_events_450220.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-03T21:18:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34270",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "component: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34270#event-450220"
}
```



---

archive/issue_events_450221.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-03T21:18:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34270",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34270#event-450221"
}
```



---

archive/issue_events_450222.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-03T21:18:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34270",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34270#event-450222"
}
```



---

archive/issue_comments_556602.json:
```json
{
    "body": "Branch: **[u/mkoeppe/_gitpod_yml__do_not_hardcode_the_workspace_name_as_sagetrac_mirror](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/_gitpod_yml__do_not_hardcode_the_workspace_name_as_sagetrac_mirror)**",
    "created_at": "2022-08-03T21:22:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34270",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34270#issuecomment-556602",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/_gitpod_yml__do_not_hardcode_the_workspace_name_as_sagetrac_mirror](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/_gitpod_yml__do_not_hardcode_the_workspace_name_as_sagetrac_mirror)**



---

archive/issue_events_450223.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-03T22:09:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34270",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34270#event-450223"
}
```



---

archive/issue_comments_556603.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/eb839d4775e208ab0ee64f5380cb80ed68664a8c\">eb839d4</a></td><td><code>.gitpod.yml: Use $(pwd) to refer to the workspace dir</code></td></tr></table>\n",
    "created_at": "2022-08-03T22:09:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34270",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34270#issuecomment-556603",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/eb839d4775e208ab0ee64f5380cb80ed68664a8c">eb839d4</a></td><td><code>.gitpod.yml: Use $(pwd) to refer to the workspace dir</code></td></tr></table>




---

archive/issue_comments_556604.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2022-08-03T22:09:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34270",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34270#issuecomment-556604",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_comments_556605.json:
```json
{
    "body": "Commit: **[eb839d4](https://github.com/sagemath/sagetrac-mirror/commit/eb839d4775e208ab0ee64f5380cb80ed68664a8c)**",
    "created_at": "2022-08-03T22:09:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34270",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34270#issuecomment-556605",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[eb839d4](https://github.com/sagemath/sagetrac-mirror/commit/eb839d4775e208ab0ee64f5380cb80ed68664a8c)**



---

archive/issue_events_450224.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-07T23:54:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34270",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34270#event-450224"
}
```



---

archive/issue_events_450225.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-07T23:54:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34270",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34270#event-450225"
}
```



---

archive/issue_comments_556606.json:
```json
{
    "body": "Reviewer: **Kwankyu Lee**",
    "created_at": "2022-08-09T01:20:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34270",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34270#issuecomment-556606",
    "user": "https://github.com/kwankyu"
}
```

Reviewer: **Kwankyu Lee**



---

archive/issue_events_450226.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2022-08-09T01:20:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34270",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34270#event-450226"
}
```



---

archive/issue_events_450227.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2022-08-09T01:20:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34270",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34270#event-450227"
}
```



---

archive/issue_comments_556607.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nYes, it works.",
    "created_at": "2022-08-09T01:20:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34270",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34270#issuecomment-556607",
    "user": "https://github.com/kwankyu"
}
```

<div id="comment:4" align="right">Comment 4</div>

Yes, it works.



---

archive/issue_comments_556608.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,2 +1,6 @@\n-For example https://gitpod.io/#https://github.com/sagemath/sage (which appears as a link in our manuals) does not work because the workspace directory is `/workspace/sage`, not `/workspace/sagetrac-mirror`\n+For example https://gitpod.io/#https://github.com/sagemath/sage, which appears as a link in our manuals, for instance,\n \n+- find the link \"going to Gitpod\" in https://113a817a9b18c078a108e31522579eab1afac19c--sagemath-tobias.netlify.app/developer/index.html\n+\n+does not work because the workspace directory is `/workspace/sage`, not `/workspace/sagetrac-mirror`\n+\n``````\n",
    "created_at": "2022-08-09T01:31:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34270",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34270#issuecomment-556608",
    "user": "https://github.com/kwankyu"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,2 +1,6 @@
-For example https://gitpod.io/#https://github.com/sagemath/sage (which appears as a link in our manuals) does not work because the workspace directory is `/workspace/sage`, not `/workspace/sagetrac-mirror`
+For example https://gitpod.io/#https://github.com/sagemath/sage, which appears as a link in our manuals, for instance,
 
+- find the link "going to Gitpod" in https://113a817a9b18c078a108e31522579eab1afac19c--sagemath-tobias.netlify.app/developer/index.html
+
+does not work because the workspace directory is `/workspace/sage`, not `/workspace/sagetrac-mirror`
+
``````




---

archive/issue_comments_556609.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nThanks!",
    "created_at": "2022-08-09T01:37:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34270",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34270#issuecomment-556609",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:6" align="right">Comment 6</div>

Thanks!



---

archive/issue_comments_556610.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/_gitpod_yml__do_not_hardcode_the_workspace_name_as_sagetrac_mirror](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/_gitpod_yml__do_not_hardcode_the_workspace_name_as_sagetrac_mirror)** to **[eb839d4](https://github.com/sagemath/sagetrac-mirror/commit/eb839d4775e208ab0ee64f5380cb80ed68664a8c)**",
    "created_at": "2022-08-29T11:24:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34270",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34270#issuecomment-556610",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/_gitpod_yml__do_not_hardcode_the_workspace_name_as_sagetrac_mirror](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/_gitpod_yml__do_not_hardcode_the_workspace_name_as_sagetrac_mirror)** to **[eb839d4](https://github.com/sagemath/sagetrac-mirror/commit/eb839d4775e208ab0ee64f5380cb80ed68664a8c)**



---

archive/issue_events_450228.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-08-29T11:24:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34270",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34270#event-450228"
}
```



---

archive/issue_events_450229.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "696bf78075d16fdaf530b9012b634ad36a325805",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-08-29T11:24:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/34270",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34270#event-450229"
}
```
