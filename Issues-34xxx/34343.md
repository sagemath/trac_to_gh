# Issue 34343: Speed up computing outside corners of partition

archive/issues_034106.json:
```json
{
    "body": "Speed up the code for getting the corners of a partition.\n\nCC:  @tscrim\n\nKeywords: gsoc2022 partition optimization\n\nBranch/Commit: [bde6bb5e919836e7d7412288d51d78833f1976f6](https://github.com/sagemath/sagetrac-mirror/commit/bde6bb5e919836e7d7412288d51d78833f1976f6)\n\nReviewer: Travis Scrimshaw\n\nAuthor: Trevor K. Karn\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/34343\n\n",
    "closed_at": "2022-08-30T06:51:21Z",
    "created_at": "2022-08-11T19:10:37Z",
    "labels": [
        "component: combinatorics"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.7",
    "title": "Speed up computing outside corners of partition",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/34343",
    "user": "https://github.com/trevorkarn"
}
```
Speed up the code for getting the corners of a partition.

CC:  @tscrim

Keywords: gsoc2022 partition optimization

Branch/Commit: [bde6bb5e919836e7d7412288d51d78833f1976f6](https://github.com/sagemath/sagetrac-mirror/commit/bde6bb5e919836e7d7412288d51d78833f1976f6)

Reviewer: Travis Scrimshaw

Author: Trevor K. Karn

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/34343





---

archive/issue_comments_683517.json:
```json
{
    "body": "<a id='comment:1'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                        |\n|-------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------|\n|[434071a](https://github.com/sagemath/sagetrac-mirror/commit/434071a5f28bd8ec8b57dfb52a7d3302a598164f)|`Rewrite loop computing outside corners`|",
    "created_at": "2022-08-11T19:25:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/34343#issuecomment-683517",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:1'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                        |
|-------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------|
|[434071a](https://github.com/sagemath/sagetrac-mirror/commit/434071a5f28bd8ec8b57dfb52a7d3302a598164f)|`Rewrite loop computing outside corners`|



---

archive/issue_comments_683518.json:
```json
{
    "body": "Commit: [434071a5f28bd8ec8b57dfb52a7d3302a598164f](https://github.com/sagemath/sagetrac-mirror/commit/434071a5f28bd8ec8b57dfb52a7d3302a598164f)",
    "created_at": "2022-08-11T19:25:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/34343#issuecomment-683518",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Commit: [434071a5f28bd8ec8b57dfb52a7d3302a598164f](https://github.com/sagemath/sagetrac-mirror/commit/434071a5f28bd8ec8b57dfb52a7d3302a598164f)



---

archive/issue_comments_683519.json:
```json
{
    "body": "<a id='comment:2'></a>\nWith change:\n\n```\nsage: mu = Partition(range(100,0,-1))\nsage: %lprun -f mu.outside_corners mu.outside_corners()\nTimer unit: 1e-06 s\n\nTotal time: 0.000114 s\nFile: /Users/trevorkarn/Applications/sage/src/sage/combinat/partition.py\nFunction: outside_corners at line 4006\n\nLine #      Hits         Time  Per Hit   % Time  Line Contents\n==============================================================\n  4006                                               def outside_corners(self):\n  4007                                                   r\"\"\"\n  4008                                                   Return a list of the outside corners of the partition ``self``.\n  4009                                           \n  4010                                                   An outside corner (also called a cocorner) of a partition\n  4011                                                   `\\lambda` is a cell on `\\ZZ^2` which does not belong to\n  4012                                                   the Young diagram of `\\lambda` but can be added to this Young\n  4013                                                   diagram to still form a straight-shape Young diagram.\n  4014                                           \n  4015                                                   The entries of the list returned are pairs of the form `(i,j)`,\n  4016                                                   where `i` and `j` are the coordinates of the respective corner.\n  4017                                                   The coordinates are counted from `0`.\n  4018                                           \n  4019                                                   EXAMPLES::\n  4020                                           \n  4021                                                       sage: Partition([2,2,1]).outside_corners()\n  4022                                                       [(0, 2), (2, 1), (3, 0)]\n  4023                                                       sage: Partition([2,2]).outside_corners()\n  4024                                                       [(0, 2), (2, 0)]\n  4025                                                       sage: Partition([6,3,3,1,1,1]).outside_corners()\n  4026                                                       [(0, 6), (1, 3), (3, 1), (6, 0)]\n  4027                                                       sage: Partition([]).outside_corners()\n  4028                                                       [(0, 0)]\n  4029                                                   \"\"\"\n  4030         1          6.0      6.0      5.3          p = self\n  4031         1          8.0      8.0      7.0          if p.is_empty():\n  4032                                                       return [(0,0)]\n  4033         1          3.0      3.0      2.6          res = [(0, p[0])]\n  4034         1         93.0     93.0     81.6          res.extend((n, j) for n, (i, j) in enumerate(zip(p[:-1], p[1:]), start=1) if i != j)\n  4035         1          3.0      3.0      2.6          res.append((len(p), 0))\n  4036         1          1.0      1.0      0.9          return res\n```\n\nvs before:\n\n```\nTimer unit: 1e-06 s\n\nTotal time: 0.000405 s\nFile: /Users/trevorkarn/Applications/sage/src/sage/combinat/partition.py\nFunction: outside_corners at line 4006\n\nLine #      Hits         Time  Per Hit   % Time  Line Contents\n==============================================================\n  4006                                               def outside_corners(self):                                               \n  ....\n  4030         1          3.0      3.0      0.7          p = self\n  4031         1          9.0      9.0      2.2          if p.is_empty():\n  4032                                                       return [(0,0)]\n  4033         1          3.0      3.0      0.7          res = [ (0, p[0]) ]\n  4034       100         75.0      0.8     18.5          for i in range(1, len(p)):\n  4035        99        168.0      1.7     41.5              if p[i-1] != p[i]:\n  4036        99        145.0      1.5     35.8                  res.append((i,p[i]))\n  4037         1          2.0      2.0      0.5          res.append((len(p), 0))\n  4038                                           \n  4039         1          0.0      0.0      0.0          return res\n```\n\n---\nNew commits:\n|                                                                                                                                          |                                        |\n|------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------|\n|[434071a](https://github.com/sagemath/sagetrac-mirror/commit/434071a5f28bd8ec8b57dfb52a7d3302a598164f)|`Rewrite loop computing outside corners`|\n\n---\nNew commits:\n|                                                                                                                                          |                                        |\n|------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------|\n|[434071a](https://github.com/sagemath/sagetrac-mirror/commit/434071a5f28bd8ec8b57dfb52a7d3302a598164f)|`Rewrite loop computing outside corners`|",
    "created_at": "2022-08-11T19:25:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/34343#issuecomment-683519",
    "user": "https://github.com/trevorkarn"
}
```

<a id='comment:2'></a>
With change:

```
sage: mu = Partition(range(100,0,-1))
sage: %lprun -f mu.outside_corners mu.outside_corners()
Timer unit: 1e-06 s

Total time: 0.000114 s
File: /Users/trevorkarn/Applications/sage/src/sage/combinat/partition.py
Function: outside_corners at line 4006

Line #      Hits         Time  Per Hit   % Time  Line Contents
==============================================================
  4006                                               def outside_corners(self):
  4007                                                   r"""
  4008                                                   Return a list of the outside corners of the partition ``self``.
  4009                                           
  4010                                                   An outside corner (also called a cocorner) of a partition
  4011                                                   `\lambda` is a cell on `\ZZ^2` which does not belong to
  4012                                                   the Young diagram of `\lambda` but can be added to this Young
  4013                                                   diagram to still form a straight-shape Young diagram.
  4014                                           
  4015                                                   The entries of the list returned are pairs of the form `(i,j)`,
  4016                                                   where `i` and `j` are the coordinates of the respective corner.
  4017                                                   The coordinates are counted from `0`.
  4018                                           
  4019                                                   EXAMPLES::
  4020                                           
  4021                                                       sage: Partition([2,2,1]).outside_corners()
  4022                                                       [(0, 2), (2, 1), (3, 0)]
  4023                                                       sage: Partition([2,2]).outside_corners()
  4024                                                       [(0, 2), (2, 0)]
  4025                                                       sage: Partition([6,3,3,1,1,1]).outside_corners()
  4026                                                       [(0, 6), (1, 3), (3, 1), (6, 0)]
  4027                                                       sage: Partition([]).outside_corners()
  4028                                                       [(0, 0)]
  4029                                                   """
  4030         1          6.0      6.0      5.3          p = self
  4031         1          8.0      8.0      7.0          if p.is_empty():
  4032                                                       return [(0,0)]
  4033         1          3.0      3.0      2.6          res = [(0, p[0])]
  4034         1         93.0     93.0     81.6          res.extend((n, j) for n, (i, j) in enumerate(zip(p[:-1], p[1:]), start=1) if i != j)
  4035         1          3.0      3.0      2.6          res.append((len(p), 0))
  4036         1          1.0      1.0      0.9          return res
```

vs before:

```
Timer unit: 1e-06 s

Total time: 0.000405 s
File: /Users/trevorkarn/Applications/sage/src/sage/combinat/partition.py
Function: outside_corners at line 4006

Line #      Hits         Time  Per Hit   % Time  Line Contents
==============================================================
  4006                                               def outside_corners(self):                                               
  ....
  4030         1          3.0      3.0      0.7          p = self
  4031         1          9.0      9.0      2.2          if p.is_empty():
  4032                                                       return [(0,0)]
  4033         1          3.0      3.0      0.7          res = [ (0, p[0]) ]
  4034       100         75.0      0.8     18.5          for i in range(1, len(p)):
  4035        99        168.0      1.7     41.5              if p[i-1] != p[i]:
  4036        99        145.0      1.5     35.8                  res.append((i,p[i]))
  4037         1          2.0      2.0      0.5          res.append((len(p), 0))
  4038                                           
  4039         1          0.0      0.0      0.0          return res
```

---
New commits:
|                                                                                                                                          |                                        |
|------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------|
|[434071a](https://github.com/sagemath/sagetrac-mirror/commit/434071a5f28bd8ec8b57dfb52a7d3302a598164f)|`Rewrite loop computing outside corners`|

---
New commits:
|                                                                                                                                          |                                        |
|------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------|
|[434071a](https://github.com/sagemath/sagetrac-mirror/commit/434071a5f28bd8ec8b57dfb52a7d3302a598164f)|`Rewrite loop computing outside corners`|



---

archive/issue_comments_683520.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2022-08-11T19:26:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/34343#issuecomment-683520",
    "user": "https://github.com/trevorkarn"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_683521.json:
```json
{
    "body": "<a id='comment:4'></a>\n\n```\nsage: mu = Partition(range(100,0,-1))\nsage: %lprun -f mu.outside_corners mu.outside_corners()\n\n  4030         1         11.0     11.0      7.0          from itertools import islice\n  4031         1          5.0      5.0      3.2          p = self._list\n  4032         1          2.0      2.0      1.3          if not p:\n  4033                                                       return [(0,0)]\n  4034         1          1.0      1.0      0.6          res = [(0, p[0])]\n  4035         2        128.0     64.0     81.0          res.extend((n, j) for n, (i, j)\n  4036         1          8.0      8.0      5.1                     in enumerate(zip(islice(p, 0, len(p)-1), islice(p, 1, len(p))), start=1)\n  4037                                                              if i != j)\n  4038         1          2.0      2.0      1.3          res.append((len(p), 0))\n  4039         1          1.0      1.0      0.6          return res\n```",
    "created_at": "2022-08-12T02:57:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/34343#issuecomment-683521",
    "user": "https://github.com/trevorkarn"
}
```

<a id='comment:4'></a>

```
sage: mu = Partition(range(100,0,-1))
sage: %lprun -f mu.outside_corners mu.outside_corners()

  4030         1         11.0     11.0      7.0          from itertools import islice
  4031         1          5.0      5.0      3.2          p = self._list
  4032         1          2.0      2.0      1.3          if not p:
  4033                                                       return [(0,0)]
  4034         1          1.0      1.0      0.6          res = [(0, p[0])]
  4035         2        128.0     64.0     81.0          res.extend((n, j) for n, (i, j)
  4036         1          8.0      8.0      5.1                     in enumerate(zip(islice(p, 0, len(p)-1), islice(p, 1, len(p))), start=1)
  4037                                                              if i != j)
  4038         1          2.0      2.0      1.3          res.append((len(p), 0))
  4039         1          1.0      1.0      0.6          return res
```



---

archive/issue_comments_683522.json:
```json
{
    "body": "<a id='comment:5'></a>\n\n```\nsage: mu = Partition(range(100000,0,-1))\nsage: %lprun -f mu.outside_corners mu.outside_corners()\nTimer unit: 1e-06 s\n\nTotal time: 0.163733 s\nFile: /Users/trevorkarn/Applications/sage/src/sage/combinat/partition.py\nFunction: outside_corners at line 4006\n\nLine #      Hits         Time  Per Hit   % Time  Line Contents\n==============================================================\n  4030         1         10.0     10.0      0.0          p = self._list\n  4031         1          2.0      2.0      0.0          if not p:\n  4032                                                       return [(0,0)]\n  4033         1          2.0      2.0      0.0          res = [(0, p[0])]\n  4034         1     163711.0 163711.0    100.0          res.extend((n, j) for n, (i, j) in enumerate(zip(p[:-1], p[1:]), start=1) if i != j)\n  4035         1          8.0      8.0      0.0          res.append((len(p), 0))\n  4036         1          0.0      0.0      0.0          return res\n\n```\n\nIt seems like making `p = self._list` and then using normal slicing (rather than `islice`) is the fastest.\n\nCompare to\n\n```\nsage: mu = Partition(range(100000,0,-1))\nsage: %lprun -f mu.outside_corners mu.outside_corners()\nTimer unit: 1e-06 s\n\nTotal time: 0.191122 s\nFile: /Users/trevorkarn/Applications/sage/src/sage/combinat/partition.py\nFunction: outside_corners at line 4006\n\nLine #      Hits         Time  Per Hit   % Time  Line Contents\n==============================================================\n  4030         1          7.0      7.0      0.0          from itertools import islice\n  4031         1          2.0      2.0      0.0          p = self._list\n  4032         1          1.0      1.0      0.0          if not p:\n  4033                                                       return [(0,0)]\n  4034         1          1.0      1.0      0.0          res = [(0, p[0])]\n  4035         2     191099.0  95549.5    100.0          res.extend((n, j) for n, (i, j)\n  4036         1          5.0      5.0      0.0                     in enumerate(zip(islice(p, 0, len(p)-1), islice(p, 1, len(p))), start=1)\n  4037                                                              if i != j)\n  4038         1          7.0      7.0      0.0          res.append((len(p), 0))\n  4039         1          0.0      0.0      0.0          return res\n\n```\n\n```\n\nsage: mu = Partition(range(100000,0,-1))\nsage: %lprun -f mu.outside_corners mu.outside_corners()\nTimer unit: 1e-06 s\n\nTotal time: 0.174545 s\nFile: /Users/trevorkarn/Applications/sage/src/sage/combinat/partition.py\nFunction: outside_corners at line 4006\n\nLine #      Hits         Time  Per Hit   % Time  Line Contents\n==============================================================\n  4030         1          6.0      6.0      0.0          p = self\n  4031         1         11.0     11.0      0.0          if p.is_empty():\n  4032                                                       return [(0,0)]\n  4033         1          3.0      3.0      0.0          res = [(0, p[0])]\n  4034         1     174511.0 174511.0    100.0          res.extend((n, j) for n, (i, j) in enumerate(zip(p[:-1], p[1:]), start=1) if i != j)\n  4035         1         14.0     14.0      0.0          res.append((len(p), 0))\n  4036         1          0.0      0.0      0.0          return res\n\n```",
    "created_at": "2022-08-12T03:15:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/34343#issuecomment-683522",
    "user": "https://github.com/trevorkarn"
}
```

<a id='comment:5'></a>

```
sage: mu = Partition(range(100000,0,-1))
sage: %lprun -f mu.outside_corners mu.outside_corners()
Timer unit: 1e-06 s

Total time: 0.163733 s
File: /Users/trevorkarn/Applications/sage/src/sage/combinat/partition.py
Function: outside_corners at line 4006

Line #      Hits         Time  Per Hit   % Time  Line Contents
==============================================================
  4030         1         10.0     10.0      0.0          p = self._list
  4031         1          2.0      2.0      0.0          if not p:
  4032                                                       return [(0,0)]
  4033         1          2.0      2.0      0.0          res = [(0, p[0])]
  4034         1     163711.0 163711.0    100.0          res.extend((n, j) for n, (i, j) in enumerate(zip(p[:-1], p[1:]), start=1) if i != j)
  4035         1          8.0      8.0      0.0          res.append((len(p), 0))
  4036         1          0.0      0.0      0.0          return res

```

It seems like making `p = self._list` and then using normal slicing (rather than `islice`) is the fastest.

Compare to

```
sage: mu = Partition(range(100000,0,-1))
sage: %lprun -f mu.outside_corners mu.outside_corners()
Timer unit: 1e-06 s

Total time: 0.191122 s
File: /Users/trevorkarn/Applications/sage/src/sage/combinat/partition.py
Function: outside_corners at line 4006

Line #      Hits         Time  Per Hit   % Time  Line Contents
==============================================================
  4030         1          7.0      7.0      0.0          from itertools import islice
  4031         1          2.0      2.0      0.0          p = self._list
  4032         1          1.0      1.0      0.0          if not p:
  4033                                                       return [(0,0)]
  4034         1          1.0      1.0      0.0          res = [(0, p[0])]
  4035         2     191099.0  95549.5    100.0          res.extend((n, j) for n, (i, j)
  4036         1          5.0      5.0      0.0                     in enumerate(zip(islice(p, 0, len(p)-1), islice(p, 1, len(p))), start=1)
  4037                                                              if i != j)
  4038         1          7.0      7.0      0.0          res.append((len(p), 0))
  4039         1          0.0      0.0      0.0          return res

```

```

sage: mu = Partition(range(100000,0,-1))
sage: %lprun -f mu.outside_corners mu.outside_corners()
Timer unit: 1e-06 s

Total time: 0.174545 s
File: /Users/trevorkarn/Applications/sage/src/sage/combinat/partition.py
Function: outside_corners at line 4006

Line #      Hits         Time  Per Hit   % Time  Line Contents
==============================================================
  4030         1          6.0      6.0      0.0          p = self
  4031         1         11.0     11.0      0.0          if p.is_empty():
  4032                                                       return [(0,0)]
  4033         1          3.0      3.0      0.0          res = [(0, p[0])]
  4034         1     174511.0 174511.0    100.0          res.extend((n, j) for n, (i, j) in enumerate(zip(p[:-1], p[1:]), start=1) if i != j)
  4035         1         14.0     14.0      0.0          res.append((len(p), 0))
  4036         1          0.0      0.0      0.0          return res

```



---

archive/issue_comments_683523.json:
```json
{
    "body": "<a id='comment:6'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                |\n|-------------------------------------------------------------------------------------------------------------------------------------------|----------------|\n|[bde6bb5](https://github.com/sagemath/sagetrac-mirror/commit/bde6bb5e919836e7d7412288d51d78833f1976f6)|`Change to list`|",
    "created_at": "2022-08-12T17:34:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/34343#issuecomment-683523",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:6'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                |
|-------------------------------------------------------------------------------------------------------------------------------------------|----------------|
|[bde6bb5](https://github.com/sagemath/sagetrac-mirror/commit/bde6bb5e919836e7d7412288d51d78833f1976f6)|`Change to list`|



---

archive/issue_comments_683524.json:
```json
{
    "body": "Changing commit from \"[434071a5f28bd8ec8b57dfb52a7d3302a598164f](https://github.com/sagemath/sagetrac-mirror/commit/434071a5f28bd8ec8b57dfb52a7d3302a598164f)\" to \"[bde6bb5e919836e7d7412288d51d78833f1976f6](https://github.com/sagemath/sagetrac-mirror/commit/bde6bb5e919836e7d7412288d51d78833f1976f6)\"",
    "created_at": "2022-08-12T17:34:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/34343#issuecomment-683524",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "[434071a5f28bd8ec8b57dfb52a7d3302a598164f](https://github.com/sagemath/sagetrac-mirror/commit/434071a5f28bd8ec8b57dfb52a7d3302a598164f)" to "[bde6bb5e919836e7d7412288d51d78833f1976f6](https://github.com/sagemath/sagetrac-mirror/commit/bde6bb5e919836e7d7412288d51d78833f1976f6)"



---

archive/issue_comments_683525.json:
```json
{
    "body": "Reviewer: Travis Scrimshaw",
    "created_at": "2022-08-15T01:38:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/34343#issuecomment-683525",
    "user": "https://github.com/tscrim"
}
```

Reviewer: Travis Scrimshaw



---

archive/issue_comments_683526.json:
```json
{
    "body": "<a id='comment:7'></a>\nLet it be so.",
    "created_at": "2022-08-15T01:38:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/34343#issuecomment-683526",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:7'></a>
Let it be so.



---

archive/issue_comments_683527.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2022-08-15T01:38:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/34343#issuecomment-683527",
    "user": "https://github.com/tscrim"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_683528.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2022-08-30T06:51:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/34343#issuecomment-683528",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_comments_683529.json:
```json
{
    "body": "Changing branch from \"[u/tkarn/partition-corners-202208](https://github.com/sagemath/sagetrac-mirror/tree/u/tkarn/partition-corners-202208)\" to \"[bde6bb5e919836e7d7412288d51d78833f1976f6](https://github.com/sagemath/sagetrac-mirror/commit/bde6bb5e919836e7d7412288d51d78833f1976f6)\"",
    "created_at": "2022-08-30T06:51:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/34343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/34343#issuecomment-683529",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "[u/tkarn/partition-corners-202208](https://github.com/sagemath/sagetrac-mirror/tree/u/tkarn/partition-corners-202208)" to "[bde6bb5e919836e7d7412288d51d78833f1976f6](https://github.com/sagemath/sagetrac-mirror/commit/bde6bb5e919836e7d7412288d51d78833f1976f6)"



---

archive/issue_events_100681.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-08-30T06:51:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/34343",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/34343#event-100681"
}
```
