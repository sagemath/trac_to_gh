# Issue 34162: Fix docstring markup in sage/doctest

archive/issues_033925.json:
```json
{
    "assignees": [],
    "body": "Part of #34157:\n\n```\nsage/doctest/sources.py:193:1: RST215 Inline interpreted text or phrase reference start-string without end-string.\nsage/doctest/sources.py:239:1: RST215 Inline interpreted text or phrase reference start-string without end-string.\nsage/doctest/sources.py:429:1: RST201 Block quote ends without a blank line; unexpected unindent.\nsage/doctest/sources.py:431:1: RST201 Block quote ends without a blank line; unexpected unindent.\nsage/doctest/sources.py:765:1: RST214 Inline literal start-string without end-string.\nsage/doctest/sources.py:765:1: RST214 Inline literal start-string without end-string.\n```\n\nBranch/Commit: **[d7ab884](https://github.com/sagemath/sagetrac-mirror/commit/d7ab8840c35f3cc2e00e18a6c56c4c9b7d76a1a6)**\n\nReviewer: **Matthias Koeppe**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/34162_\n\n",
    "closed_at": "2022-08-01T20:20:08Z",
    "created_at": "2022-07-12T06:04:40Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix docstring markup in sage/doctest",
    "type": "issue",
    "updated_at": "2022-08-01T20:20:08Z",
    "url": "https://github.com/sagemath/sage/issues/34162",
    "user": "https://github.com/kwankyu"
}
```
Part of #34157:

```
sage/doctest/sources.py:193:1: RST215 Inline interpreted text or phrase reference start-string without end-string.
sage/doctest/sources.py:239:1: RST215 Inline interpreted text or phrase reference start-string without end-string.
sage/doctest/sources.py:429:1: RST201 Block quote ends without a blank line; unexpected unindent.
sage/doctest/sources.py:431:1: RST201 Block quote ends without a blank line; unexpected unindent.
sage/doctest/sources.py:765:1: RST214 Inline literal start-string without end-string.
sage/doctest/sources.py:765:1: RST214 Inline literal start-string without end-string.
```

Branch/Commit: **[d7ab884](https://github.com/sagemath/sagetrac-mirror/commit/d7ab8840c35f3cc2e00e18a6c56c4c9b7d76a1a6)**

Reviewer: **Matthias Koeppe**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/34162_





---

archive/issue_events_449110.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2022-07-12T06:04:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/34162",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34162#event-449110"
}
```



---

archive/issue_events_449111.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2022-07-12T06:04:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34162",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34162#event-449111"
}
```



---

archive/issue_events_449112.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2022-07-12T06:04:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34162",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34162#event-449112"
}
```



---

archive/issue_events_449113.json:
```json
{
    "actor": "https://github.com/kwankyu",
    "created_at": "2022-07-12T06:04:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34162",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34162#event-449113"
}
```



---

archive/issue_comments_554877.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,4 @@\n+Part of #34157:\n \n ```\n sage/doctest/sources.py:193:1: RST215 Inline interpreted text or phrase reference start-string without end-string.\n``````\n",
    "created_at": "2022-07-12T07:57:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34162",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34162#issuecomment-554877",
    "user": "https://github.com/kwankyu"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,4 @@
+Part of #34157:
 
 ```
 sage/doctest/sources.py:193:1: RST215 Inline interpreted text or phrase reference start-string without end-string.
``````




---

archive/issue_comments_554878.json:
```json
{
    "body": "Commit: **[d7ab884](https://github.com/sagemath/sagetrac-mirror/commit/d7ab8840c35f3cc2e00e18a6c56c4c9b7d76a1a6)**",
    "created_at": "2022-07-13T09:08:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34162",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34162#issuecomment-554878",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[d7ab884](https://github.com/sagemath/sagetrac-mirror/commit/d7ab8840c35f3cc2e00e18a6c56c4c9b7d76a1a6)**



---

archive/issue_comments_554879.json:
```json
{
    "body": "Author: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2022-07-13T09:08:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34162",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34162#issuecomment-554879",
    "user": "https://github.com/fchapoton"
}
```

Author: **Frédéric Chapoton**



---

archive/issue_events_449114.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2022-07-13T09:08:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34162",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34162#event-449114"
}
```



---

archive/issue_comments_554880.json:
```json
{
    "body": "Branch: **[u/chapoton/34162](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/34162)**",
    "created_at": "2022-07-13T09:08:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34162",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34162#issuecomment-554880",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/34162](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/34162)**



---

archive/issue_comments_554881.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\ndone and checked\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bacd72de27c1fa944388dd0c022269cc2cbb1525\">bacd72d</a></td><td><code>rst fixes in doctest</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d7ab8840c35f3cc2e00e18a6c56c4c9b7d76a1a6\">d7ab884</a></td><td><code>fix</code></td></tr></table>\n",
    "created_at": "2022-07-13T09:08:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34162",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34162#issuecomment-554881",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

done and checked

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bacd72de27c1fa944388dd0c022269cc2cbb1525">bacd72d</a></td><td><code>rst fixes in doctest</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d7ab8840c35f3cc2e00e18a6c56c4c9b7d76a1a6">d7ab884</a></td><td><code>fix</code></td></tr></table>




---

archive/issue_events_449115.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-07-14T16:17:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34162",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34162#event-449115"
}
```



---

archive/issue_events_449116.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-07-14T16:17:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34162",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34162#event-449116"
}
```



---

archive/issue_comments_554882.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2022-07-14T16:17:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34162",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34162#issuecomment-554882",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_events_449117.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-07-14T16:17:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34162",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34162#event-449117"
}
```



---

archive/issue_events_449118.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-07-14T16:17:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34162",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34162#event-449118"
}
```



---

archive/issue_events_449119.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-08-01T20:20:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34162",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34162#event-449119"
}
```



---

archive/issue_events_449120.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "a129ff5fa275ac1aa3adcc7cde81d8dbacd4c043",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-08-01T20:20:08Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/34162",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34162#event-449120"
}
```



---

archive/issue_comments_554883.json:
```json
{
    "body": "Changed branch from **[u/chapoton/34162](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/34162)** to **[d7ab884](https://github.com/sagemath/sagetrac-mirror/commit/d7ab8840c35f3cc2e00e18a6c56c4c9b7d76a1a6)**",
    "created_at": "2022-08-01T20:20:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34162",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34162#issuecomment-554883",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/34162](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/34162)** to **[d7ab884](https://github.com/sagemath/sagetrac-mirror/commit/d7ab8840c35f3cc2e00e18a6c56c4c9b7d76a1a6)**
