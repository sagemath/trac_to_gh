# Issue 34902: LatticePoset breadth, error in documentation

archive/issues_034665.json:
```json
{
    "assignees": [],
    "body": "I got a mail about my posting in year 2017: \"However, I read Stephan's paper differently: He states that breadth k is equivalent\u00a0 to containing a Boolean lattice with k atoms (but none with k+1 atoms) as a subORDER, and then states in his Lemma 1 that in the case k=3 this is the same as subLATTICE.\"\n\nHence documentation of `breadth()` in `LatticePoset` containing\n\nThis can be also characterized by sublattices: a lattice of breadth at least `$n$` contains a sublattice isomorphic to the Boolean lattice of `$2^n$` elements.\n\nmust be changed to\n\nThis can be also characterized by subposets: a lattice of breadth at least `$n$` contains a subposet isomorphic to the Boolean lattice of `$2^n$` elements.\n\n\nKeywords: **latticeposet**\n\nBranch/Commit: **[u/chapoton/34902](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/34902) @ [26d7ced](https://github.com/sagemath/sagetrac-mirror/commit/26d7ceda4f3d4bf30ed3f788083a0556d0e27c59)**\n\nReviewer: **Jori M\u00e4ntysalo**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/34902_\n\n",
    "created_at": "2023-01-09T07:48:01Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/positive%20review"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "LatticePoset breadth, error in documentation",
    "type": "issue",
    "updated_at": "2023-01-10T01:35:13Z",
    "url": "https://github.com/sagemath/sage/issues/34902",
    "user": "https://github.com/jm58660"
}
```
I got a mail about my posting in year 2017: "However, I read Stephan's paper differently: He states that breadth k is equivalent  to containing a Boolean lattice with k atoms (but none with k+1 atoms) as a subORDER, and then states in his Lemma 1 that in the case k=3 this is the same as subLATTICE."

Hence documentation of `breadth()` in `LatticePoset` containing

This can be also characterized by sublattices: a lattice of breadth at least `$n$` contains a sublattice isomorphic to the Boolean lattice of `$2^n$` elements.

must be changed to

This can be also characterized by subposets: a lattice of breadth at least `$n$` contains a subposet isomorphic to the Boolean lattice of `$2^n$` elements.


Keywords: **latticeposet**

Branch/Commit: **[u/chapoton/34902](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/34902) @ [26d7ced](https://github.com/sagemath/sagetrac-mirror/commit/26d7ceda4f3d4bf30ed3f788083a0556d0e27c59)**

Reviewer: **Jori Mäntysalo**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/34902_





---

archive/issue_events_456310.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2023-01-09T07:48:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/34902",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34902#event-456310"
}
```



---

archive/issue_events_456311.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2023-01-09T07:48:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34902",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34902#event-456311"
}
```



---

archive/issue_events_456312.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2023-01-09T07:48:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34902",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34902#event-456312"
}
```



---

archive/issue_events_456313.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2023-01-09T07:48:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34902",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34902#event-456313"
}
```



---

archive/issue_comments_565218.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nHello Jori, Happy New Year. Nice to see you back.\n\nI have made the change, is it ok ?\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/26d7ceda4f3d4bf30ed3f788083a0556d0e27c59\">26d7ced</a></td><td><code>fix doc in lattice</code></td></tr></table>\n",
    "created_at": "2023-01-09T18:54:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34902",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34902#issuecomment-565218",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:1'>Comment 1:</a>
Hello Jori, Happy New Year. Nice to see you back.

I have made the change, is it ok ?

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/26d7ceda4f3d4bf30ed3f788083a0556d0e27c59">26d7ced</a></td><td><code>fix doc in lattice</code></td></tr></table>




---

archive/issue_comments_565219.json:
```json
{
    "body": "Commit: **[26d7ced](https://github.com/sagemath/sagetrac-mirror/commit/26d7ceda4f3d4bf30ed3f788083a0556d0e27c59)**",
    "created_at": "2023-01-09T18:54:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34902",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34902#issuecomment-565219",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[26d7ced](https://github.com/sagemath/sagetrac-mirror/commit/26d7ceda4f3d4bf30ed3f788083a0556d0e27c59)**



---

archive/issue_events_456314.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2023-01-09T18:54:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34902",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34902#event-456314"
}
```



---

archive/issue_comments_565220.json:
```json
{
    "body": "Author: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2023-01-09T18:54:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34902",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34902#issuecomment-565220",
    "user": "https://github.com/fchapoton"
}
```

Author: **Frédéric Chapoton**



---

archive/issue_comments_565221.json:
```json
{
    "body": "Branch: **[u/chapoton/34902](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/34902)**",
    "created_at": "2023-01-09T18:54:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34902",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34902#issuecomment-565221",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/34902](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/34902)**



---

archive/issue_events_456315.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2023-01-09T19:47:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/34902",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34902#event-456315"
}
```



---

archive/issue_events_456316.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2023-01-09T19:47:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/34902",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/34902#event-456316"
}
```



---

archive/issue_comments_565222.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nWell, not really back; just a note of this, as this was my mistake years ago.\n\nAnd yes, the patch is good.",
    "created_at": "2023-01-09T19:47:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34902",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34902#issuecomment-565222",
    "user": "https://github.com/jm58660"
}
```

<a id='comment:2'>Comment 2:</a>
Well, not really back; just a note of this, as this was my mistake years ago.

And yes, the patch is good.



---

archive/issue_comments_565223.json:
```json
{
    "body": "Reviewer: **Jori M\u00e4ntysalo**",
    "created_at": "2023-01-10T01:35:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/34902",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/34902#issuecomment-565223",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Jori Mäntysalo**
