# Issue 7312: Add a function .st (meaning "such that" )which is an alias to .add_constraint

archive/issues_007312.json:
```json
{
    "body": "Assignee: jkantor\n\nAdd a function .st (meaning \"such that\" )which is an alias to .add_constraint. This would shorten the code, as the add_constraint option is the most used, and copy the syntax used in Pymprog\n\nIssue created by migration from https://trac.sagemath.org/ticket/7312\n\n",
    "closed_at": "2010-01-14T05:54:56Z",
    "created_at": "2009-10-26T13:35:41Z",
    "labels": [
        "component: numerical",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "Add a function .st (meaning \"such that\" )which is an alias to .add_constraint",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/7312",
    "user": "https://github.com/nathanncohen"
}
```
Assignee: jkantor

Add a function .st (meaning "such that" )which is an alias to .add_constraint. This would shorten the code, as the add_constraint option is the most used, and copy the syntax used in Pymprog

Issue created by migration from https://trac.sagemath.org/ticket/7312





---

archive/issue_comments_060986.json:
```json
{
    "body": "I'm not convinced that this is a good idea.  I think that the benefit of shorter code is outweighed by making the code more cryptic.",
    "created_at": "2009-10-26T17:18:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7312#issuecomment-60986",
    "user": "https://github.com/mwhansen"
}
```

I'm not convinced that this is a good idea.  I think that the benefit of shorter code is outweighed by making the code more cryptic.



---

archive/issue_comments_060987.json:
```json
{
    "body": "What would you think of a .such_that method then ?",
    "created_at": "2009-10-26T18:44:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7312#issuecomment-60987",
    "user": "https://github.com/nathanncohen"
}
```

What would you think of a .such_that method then ?



---

archive/issue_comments_060988.json:
```json
{
    "body": "Changing status from new to needs_info.",
    "created_at": "2009-10-26T18:44:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7312#issuecomment-60988",
    "user": "https://github.com/nathanncohen"
}
```

Changing status from new to needs_info.



---

archive/issue_comments_060989.json:
```json
{
    "body": "I don't see how that that is much better than add_constraint.",
    "created_at": "2009-11-02T04:55:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7312#issuecomment-60989",
    "user": "https://github.com/mwhansen"
}
```

I don't see how that that is much better than add_constraint.



---

archive/issue_comments_060990.json:
```json
{
    "body": "True enough....\n\nThen this ticket should be closed as \"stupid idea\" :-)\n\nNathann",
    "created_at": "2009-11-02T06:52:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7312#issuecomment-60990",
    "user": "https://github.com/nathanncohen"
}
```

True enough....

Then this ticket should be closed as "stupid idea" :-)

Nathann



---

archive/issue_comments_060991.json:
```json
{
    "body": "No need to keep the ticket open then...",
    "created_at": "2010-01-14T05:54:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7312#issuecomment-60991",
    "user": "https://github.com/robertwb"
}
```

No need to keep the ticket open then...



---

archive/issue_events_017326.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-01-14T05:54:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/7312",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7312#event-17326"
}
```



---

archive/issue_comments_060992.json:
```json
{
    "body": "Resolution: wontfix",
    "created_at": "2010-01-14T05:54:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/7312",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/7312#issuecomment-60992",
    "user": "https://github.com/robertwb"
}
```

Resolution: wontfix



---

archive/issue_events_017327.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2010-01-14T05:54:56Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/7312",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/7312#event-17327"
}
```
