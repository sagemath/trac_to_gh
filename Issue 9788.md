# Issue 9788: Add support for degree bounds of Groebner bases

archive/issues_009788.json:
```json
{
    "body": "Assignee: malb\n\nSee http://ask.sagemath.org/question/55/implementing-different-groebner_basis-algorithms\n\nIssue created by migration from https://trac.sagemath.org/ticket/9789\n\n",
    "created_at": "2010-08-23T19:57:10Z",
    "labels": [
        "commutative algebra",
        "major",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.4",
    "title": "Add support for degree bounds of Groebner bases",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/9788",
    "user": "mhansen"
}
```
Assignee: malb

See http://ask.sagemath.org/question/55/implementing-different-groebner_basis-algorithms

Issue created by migration from https://trac.sagemath.org/ticket/9789





---

archive/issue_comments_096109.json:
```json
{
    "body": "This ticket is a duplicate of #1396, shall I close this ticket then?",
    "created_at": "2010-08-23T21:04:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9788",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9788#issuecomment-96109",
    "user": "malb"
}
```

This ticket is a duplicate of #1396, shall I close this ticket then?



---

archive/issue_comments_096110.json:
```json
{
    "body": "The code from #1396 has not been merged into Sage despite the ticket being closed.  The new number for that ticket is #9599.\n\nI'll change this to implement degree bounds using Macaulay2.",
    "created_at": "2010-08-23T23:12:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/9788",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/9788#issuecomment-96110",
    "user": "mhansen"
}
```

The code from #1396 has not been merged into Sage despite the ticket being closed.  The new number for that ticket is #9599.

I'll change this to implement degree bounds using Macaulay2.
