# Issue 8256: The documentation of Misc is a mess

archive/issues_008256.json:
```json
{
    "body": "Assignee: mvngu\n\nKeywords: misc documentation\n\nThe documentation of the misc directory is a mess. We should sort and organize the file `misc.rst`.\n\nIssue created by migration from https://trac.sagemath.org/ticket/8256\n\n",
    "created_at": "2010-02-13T16:51:49Z",
    "labels": [
        "component: documentation",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "The documentation of Misc is a mess",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/8256",
    "user": "https://github.com/hivert"
}
```
Assignee: mvngu

Keywords: misc documentation

The documentation of the misc directory is a mess. We should sort and organize the file `misc.rst`.

Issue created by migration from https://trac.sagemath.org/ticket/8256





---

archive/issue_comments_072942.json:
```json
{
    "body": "This is ... really vague.  If someone doesn't put something a little more concrete here, maybe we should close it - not because the doc is better, of course.",
    "created_at": "2013-01-25T15:40:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8256#issuecomment-72942",
    "user": "https://github.com/kcrisman"
}
```

This is ... really vague.  If someone doesn't put something a little more concrete here, maybe we should close it - not because the doc is better, of course.



---

archive/issue_comments_072943.json:
```json
{
    "body": "Duplicate of #17932.",
    "created_at": "2015-07-07T14:18:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8256#issuecomment-72943",
    "user": "https://github.com/jdemeyer"
}
```

Duplicate of #17932.



---

archive/issue_comments_072944.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2015-07-07T14:18:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8256#issuecomment-72944",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_072945.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2015-07-07T14:19:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8256#issuecomment-72945",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_events_008455.json:
```json
{
    "actor": "@vbraun",
    "created_at": "2015-07-17T20:06:10Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/8256",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/8256#event-8455"
}
```



---

archive/issue_comments_072946.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2015-07-17T20:06:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/8256",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/8256#issuecomment-72946",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
