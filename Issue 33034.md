# Issue 33034: broken image EXAMPLEs in ​Density Plots

archive/issues_033034.json:
```json
{
    "body": "I tried to run some EXAMPLEs in [Density Plots](https://doc.sagemath.org/html/en/reference/plotting/sage/plot/density_plot.html)\n\n## environment\n\n```\nmacOS Monterey\nVersion 12.1\n```\n\n\n\n```\n\u250c\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2510\n\u2502 SageMath version 9.5.rc2, Release Date: 2022-01-16                 \u2502\n\u2502 Using Python 3.9.9. Type \"help()\" for help.                        \u2502\n\u2514\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2518\n\u250f\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2513\n\u2503 Warning: this is a prerelease version, and it may be unstable.     \u2503\n\u2517\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u251b\n```\n\n\n## EXAMPLE1\n\n\n```\nsage: density_plot(log(x) + log(y), (x,1,10), (y,1,10), dpi=20) \n....:\n```\n\n\n## output image1\n![](ticket::tmp_m0fft6r1.png)\n\n## expected image1\n![](https://doc.sagemath.org/html/en/reference/plotting/_images/density_plot-6.svg)\n\n## EXAMPLE2\n\n```\nsage: density_plot(log(x) + log(y), (x,1,10), (y,1,10)).show(dpi=20) # These are\n....:  equivalent \n```\n\n\n## output image2\n![](ticket::tmp_yoj2qy3g.png)\n\n## expected image2\n![](https://doc.sagemath.org/html/en/reference/plotting/_images/density_plot-7.svg)\n\nIssue created by migration from https://trac.sagemath.org/ticket/33271\n\n",
    "created_at": "2022-02-01T19:26:15Z",
    "labels": [
        "component: graphics"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.6",
    "title": "broken image EXAMPLEs in \u200bDensity Plots",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/33034",
    "user": "https://github.com/umedoblock"
}
```
I tried to run some EXAMPLEs in [Density Plots](https://doc.sagemath.org/html/en/reference/plotting/sage/plot/density_plot.html)

## environment

```
macOS Monterey
Version 12.1
```



```
┌────────────────────────────────────────────────────────────────────┐
│ SageMath version 9.5.rc2, Release Date: 2022-01-16                 │
│ Using Python 3.9.9. Type "help()" for help.                        │
└────────────────────────────────────────────────────────────────────┘
┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓
┃ Warning: this is a prerelease version, and it may be unstable.     ┃
┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛
```


## EXAMPLE1


```
sage: density_plot(log(x) + log(y), (x,1,10), (y,1,10), dpi=20) 
....:
```


## output image1
![](ticket::tmp_m0fft6r1.png)

## expected image1
![](https://doc.sagemath.org/html/en/reference/plotting/_images/density_plot-6.svg)

## EXAMPLE2

```
sage: density_plot(log(x) + log(y), (x,1,10), (y,1,10)).show(dpi=20) # These are
....:  equivalent 
```


## output image2
![](ticket::tmp_yoj2qy3g.png)

## expected image2
![](https://doc.sagemath.org/html/en/reference/plotting/_images/density_plot-7.svg)

Issue created by migration from https://trac.sagemath.org/ticket/33271





---

archive/issue_comments_470451.json:
```json
{
    "body": "output image1",
    "created_at": "2022-02-01T19:26:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33034",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33034#issuecomment-470451",
    "user": "https://github.com/umedoblock"
}
```

output image1



---

archive/issue_comments_470452.json:
```json
{
    "body": "Attachment [tmp_m0fft6r1.png](tarball://root/attachments/some-uuid/ticket33271/tmp_m0fft6r1.png) by @umedoblock created at 2022-02-01 19:27:22\n\noutput image2",
    "created_at": "2022-02-01T19:27:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33034",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33034#issuecomment-470452",
    "user": "https://github.com/umedoblock"
}
```

Attachment [tmp_m0fft6r1.png](tarball://root/attachments/some-uuid/ticket33271/tmp_m0fft6r1.png) by @umedoblock created at 2022-02-01 19:27:22

output image2



---

archive/issue_comments_470453.json:
```json
{
    "body": "Attachment [tmp_yoj2qy3g.png](tarball://root/attachments/some-uuid/ticket33271/tmp_yoj2qy3g.png) by @umedoblock created at 2022-02-01 19:33:30",
    "created_at": "2022-02-01T19:33:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33034",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33034#issuecomment-470453",
    "user": "https://github.com/umedoblock"
}
```

Attachment [tmp_yoj2qy3g.png](tarball://root/attachments/some-uuid/ticket33271/tmp_yoj2qy3g.png) by @umedoblock created at 2022-02-01 19:33:30



---

archive/issue_comments_470454.json:
```json
{
    "body": "All of the EXAMPLEs works in [Plotting fields](https://doc.sagemath.org/html/en/reference/plotting/sage/plot/plot_field.html).",
    "created_at": "2022-02-01T19:42:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33034",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33034#issuecomment-470454",
    "user": "https://github.com/umedoblock"
}
```

All of the EXAMPLEs works in [Plotting fields](https://doc.sagemath.org/html/en/reference/plotting/sage/plot/plot_field.html).



---

archive/issue_comments_470455.json:
```json
{
    "body": "The documentation only includes one image for these two lines of code because they are equivalent. Expected image 1 belongs to the previous example\n\n```\nsage: density_plot(y^2 + 1 - x^3 - x, (x,-pi,pi), (y,-pi,pi))\n```\n\nso that is not relevant here.\n\nThe small size is what I obtain as well, but 20 dpi is honestly not a very high resolution, so I do not see any problem here.",
    "created_at": "2022-02-01T19:45:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33034",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33034#issuecomment-470455",
    "user": "https://github.com/mwageringel"
}
```

The documentation only includes one image for these two lines of code because they are equivalent. Expected image 1 belongs to the previous example

```
sage: density_plot(y^2 + 1 - x^3 - x, (x,-pi,pi), (y,-pi,pi))
```

so that is not relevant here.

The small size is what I obtain as well, but 20 dpi is honestly not a very high resolution, so I do not see any problem here.



---

archive/issue_comments_470456.json:
```json
{
    "body": "low resolution image",
    "created_at": "2022-02-02T01:38:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33034",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33034#issuecomment-470456",
    "user": "https://github.com/umedoblock"
}
```

low resolution image



---

archive/issue_comments_470457.json:
```json
{
    "body": "Attachment [a.png](tarball://root/attachments/some-uuid/ticket33271/a.png) by @umedoblock created at 2022-02-02 01:44:19\n\nSo sorry.\n\nI gave you wrong report.\n\nexpected image1 is wrong.\n\nI mistake.\n\nSorry...\n\nAnd I'd like to talk about \"dpi=20\".\n\nFirst, I hope to similar resolution image with below.\n\n\n```\ndensity_plot(y^2 + 1 - x^3 - x, (y,-pi,pi), (x,-pi,pi))\ndensity_plot(y^2 + 1 - x^3 - x, (x,-pi,pi), (y,-pi,pi))\n```\n\n\nSo please show \"dpi=200\" for sagemath beginner.\n\n```\ndensity_plot(log(x) + log(y), (x,1,10), (y,1,10), dpi=200)\ndensity_plot(log(x) + log(y), (x,1,10), (y,1,10)).show(dpi=200)           \n```\n\n\nFirst I watched below image(dpi=20) and feel it broken.\n\nI cannot make shrink image.\n\nTOO BIG SORRY...\n\n![](ticket:33271:a.png)",
    "created_at": "2022-02-02T01:44:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33034",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33034#issuecomment-470457",
    "user": "https://github.com/umedoblock"
}
```

Attachment [a.png](tarball://root/attachments/some-uuid/ticket33271/a.png) by @umedoblock created at 2022-02-02 01:44:19

So sorry.

I gave you wrong report.

expected image1 is wrong.

I mistake.

Sorry...

And I'd like to talk about "dpi=20".

First, I hope to similar resolution image with below.


```
density_plot(y^2 + 1 - x^3 - x, (y,-pi,pi), (x,-pi,pi))
density_plot(y^2 + 1 - x^3 - x, (x,-pi,pi), (y,-pi,pi))
```


So please show "dpi=200" for sagemath beginner.

```
density_plot(log(x) + log(y), (x,1,10), (y,1,10), dpi=200)
density_plot(log(x) + log(y), (x,1,10), (y,1,10)).show(dpi=200)           
```


First I watched below image(dpi=20) and feel it broken.

I cannot make shrink image.

TOO BIG SORRY...

![](ticket:33271:a.png)



---

archive/issue_comments_470458.json:
```json
{
    "body": "Attachment [b.png](tarball://root/attachments/some-uuid/ticket33271/b.png) by @umedoblock created at 2022-02-02 01:45:37\n\nlow resolution image2",
    "created_at": "2022-02-02T01:45:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33034",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33034#issuecomment-470458",
    "user": "https://github.com/umedoblock"
}
```

Attachment [b.png](tarball://root/attachments/some-uuid/ticket33271/b.png) by @umedoblock created at 2022-02-02 01:45:37

low resolution image2



---

archive/issue_comments_470459.json:
```json
{
    "body": "Changing priority from major to minor.",
    "created_at": "2022-02-02T18:57:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33034",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33034#issuecomment-470459",
    "user": "https://github.com/mwageringel"
}
```

Changing priority from major to minor.



---

archive/issue_comments_470460.json:
```json
{
    "body": "Changing keywords from \"\" to \"beginner\".",
    "created_at": "2022-02-02T18:57:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33034",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33034#issuecomment-470460",
    "user": "https://github.com/mwageringel"
}
```

Changing keywords from "" to "beginner".



---

archive/issue_comments_470461.json:
```json
{
    "body": "Changing component from graphics to documentation.",
    "created_at": "2022-02-02T18:57:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33034",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33034#issuecomment-470461",
    "user": "https://github.com/mwageringel"
}
```

Changing component from graphics to documentation.



---

archive/issue_comments_470462.json:
```json
{
    "body": "Ok, so the source of the problem is that the function `sphinx_plot` in `src/sage/docs/conf.py`, which is used to produce the images in the documentation, explicitly discards the `dpi` keyword. I assume this makes sense so that all the images in the documentation have a consistent resolution.\n\nTo avoid confusion in this example, I would suggest to replace the `dpi=20` parameter by a different one such as `aspect_ratio=1` or `gridlines=True`. This would retain the original intent of the example by showing that the keywords get passed on to `show`.",
    "created_at": "2022-02-02T18:57:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33034",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33034#issuecomment-470462",
    "user": "https://github.com/mwageringel"
}
```

Ok, so the source of the problem is that the function `sphinx_plot` in `src/sage/docs/conf.py`, which is used to produce the images in the documentation, explicitly discards the `dpi` keyword. I assume this makes sense so that all the images in the documentation have a consistent resolution.

To avoid confusion in this example, I would suggest to replace the `dpi=20` parameter by a different one such as `aspect_ratio=1` or `gridlines=True`. This would retain the original intent of the example by showing that the keywords get passed on to `show`.



---

archive/issue_comments_470463.json:
```json
{
    "body": "Changing type from PLEASE CHANGE to enhancement.",
    "created_at": "2022-02-02T18:57:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33034",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33034#issuecomment-470463",
    "user": "https://github.com/mwageringel"
}
```

Changing type from PLEASE CHANGE to enhancement.



---

archive/issue_comments_470464.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2022-04-04T11:46:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33034",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33034#issuecomment-470464",
    "user": "https://github.com/scramblerdoodle"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_470465.json:
```json
{
    "body": "The \"Authors:\" field in the ticket box should be filled out",
    "created_at": "2022-04-04T18:32:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33034",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33034#issuecomment-470465",
    "user": "https://github.com/mkoeppe"
}
```

The "Authors:" field in the ticket box should be filled out



---

archive/issue_comments_470466.json:
```json
{
    "body": "Right, sorry!",
    "created_at": "2022-04-04T20:33:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33034",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33034#issuecomment-470466",
    "user": "https://github.com/scramblerdoodle"
}
```

Right, sorry!



---

archive/issue_comments_470467.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2022-04-08T08:47:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33034",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33034#issuecomment-470467",
    "user": "https://github.com/mwageringel"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_470468.json:
```json
{
    "body": "Thank you for the implementation. The change to the documentation can be seen [here](https://83ec6a249cd15c044d25bebdaff59fbd5fbdb819--sagemath-tobias.netlify.app/reference/plotting/sage/plot/density_plot.html#sage.plot.density_plot.density_plot) and looks good now.",
    "created_at": "2022-04-08T08:47:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33034",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33034#issuecomment-470468",
    "user": "https://github.com/mwageringel"
}
```

Thank you for the implementation. The change to the documentation can be seen [here](https://83ec6a249cd15c044d25bebdaff59fbd5fbdb819--sagemath-tobias.netlify.app/reference/plotting/sage/plot/density_plot.html#sage.plot.density_plot.density_plot) and looks good now.



---

archive/issue_events_087847.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-04-10T13:12:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/33034",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/33034#event-87847"
}
```



---

archive/issue_comments_470469.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2022-04-10T13:12:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/33034",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/33034#issuecomment-470469",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
