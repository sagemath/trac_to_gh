# Issue 14688: SystemError in cmp for matrix_integer_2x2

archive/issues_014484.json:
```json
{
    "assignees": [
        "https://github.com/jasongrout",
        "https://github.com/williamstein"
    ],
    "body": "The following throws a SystemError:\n\n```\nsage: from sage.matrix.matrix_integer_2x2 import MatrixSpace_ZZ_2x2\nsage: M2ZSpace = MatrixSpace_ZZ_2x2()\nsage: A = M2ZSpace([-5, -3, 7, 4])\nsage: B = M2ZSpace([1,0,-2,1])\nsage: A < B\n---------------------------------------------------------------------------\nSystemError                               Traceback (most recent call last)\n<ipython-input-60-e9066daab57d> in <module>()\n----> 1 A < B\n\nSystemError: error return without exception set\n```\n\n(I think this was probably found and fixed by David Roe, but he hasn't posted anything on trac, so I'm doing that).\n\nKeywords: **SystemError, matrix_integer_2x2, cmp**\n\nReviewer: **Jeroen Demeyer**\n\nAuthor: **Robert Harron**\n\nMerged: **sage-5.12.beta4**\n\nComponent: **linear algebra**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14688_\n\n",
    "closed_at": "2013-08-28T06:05:21Z",
    "created_at": "2013-06-05T02:03:59Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.12",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "SystemError in cmp for matrix_integer_2x2",
    "type": "issue",
    "updated_at": "2013-08-28T06:05:21Z",
    "url": "https://github.com/sagemath/sage/issues/14688",
    "user": "https://github.com/rharron"
}
```
The following throws a SystemError:

```
sage: from sage.matrix.matrix_integer_2x2 import MatrixSpace_ZZ_2x2
sage: M2ZSpace = MatrixSpace_ZZ_2x2()
sage: A = M2ZSpace([-5, -3, 7, 4])
sage: B = M2ZSpace([1,0,-2,1])
sage: A < B
---------------------------------------------------------------------------
SystemError                               Traceback (most recent call last)
<ipython-input-60-e9066daab57d> in <module>()
----> 1 A < B

SystemError: error return without exception set
```

(I think this was probably found and fixed by David Roe, but he hasn't posted anything on trac, so I'm doing that).

Keywords: **SystemError, matrix_integer_2x2, cmp**

Reviewer: **Jeroen Demeyer**

Author: **Robert Harron**

Merged: **sage-5.12.beta4**

Component: **linear algebra**

_Issue created by migration from https://trac.sagemath.org/ticket/14688_





---

archive/issue_events_208383.json:
```json
{
    "actor": "https://github.com/rharron",
    "created_at": "2013-06-05T02:03:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14688",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14688#event-208383"
}
```



---

archive/issue_events_208384.json:
```json
{
    "actor": "https://github.com/rharron",
    "created_at": "2013-06-05T02:03:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14688",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "c: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14688#event-208384"
}
```



---

archive/issue_events_208385.json:
```json
{
    "actor": "https://github.com/rharron",
    "created_at": "2013-06-05T02:03:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14688",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14688#event-208385"
}
```



---

archive/issue_events_208386.json:
```json
{
    "actor": "https://github.com/rharron",
    "created_at": "2013-06-05T02:03:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14688",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14688#event-208386"
}
```



---

archive/issue_events_208387.json:
```json
{
    "actor": "https://github.com/jasongrout",
    "created_at": "2013-06-05T02:03:59Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/14688",
    "subject": "https://github.com/rharron",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14688#event-208387"
}
```



---

archive/issue_events_208388.json:
```json
{
    "actor": "https://github.com/williamstein",
    "created_at": "2013-06-05T02:03:59Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/14688",
    "subject": "https://github.com/rharron",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14688#event-208388"
}
```



---

archive/issue_comments_178510.json:
```json
{
    "body": "Author: **Robert Harron**",
    "created_at": "2013-06-05T02:23:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14688#issuecomment-178510",
    "user": "https://github.com/rharron"
}
```

Author: **Robert Harron**



---

archive/issue_comments_178511.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_14688_fix_compare_in_matrix_integer_2x2.patch.gz](https://github.com/sagemath/sage/files/ticket14688/trac_14688_fix_compare_in_matrix_integer_2x2.patch.gz)**",
    "created_at": "2013-06-05T02:23:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14688#issuecomment-178511",
    "user": "https://github.com/rharron"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_14688_fix_compare_in_matrix_integer_2x2.patch.gz](https://github.com/sagemath/sage/files/ticket14688/trac_14688_fix_compare_in_matrix_integer_2x2.patch.gz)**



---

archive/issue_events_208389.json:
```json
{
    "actor": "https://github.com/rharron",
    "created_at": "2013-06-05T02:23:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14688",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14688#event-208389"
}
```



---

archive/issue_events_208390.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14688",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14688#event-208390"
}
```



---

archive/issue_events_208391.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14688",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14688#event-208391"
}
```



---

archive/issue_events_208392.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-28T06:05:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14688",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14688#event-208392"
}
```



---

archive/issue_events_208393.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-28T06:05:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14688",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14688#event-208393"
}
```



---

archive/issue_comments_178512.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2013-08-28T06:05:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14688#issuecomment-178512",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_comments_178513.json:
```json
{
    "body": "Merged: **sage-5.12.beta4**",
    "created_at": "2013-08-28T06:05:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14688",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14688#issuecomment-178513",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.12.beta4**
