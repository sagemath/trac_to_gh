# Issue 14494: HG ignore for eclipse project

archive/issues_014290.json:
```json
{
    "assignees": [],
    "body": "Hi,\n\nCould you put these following files in .hgignore\n\n.preferences.xml\n.project\n.pydevproject\nand .settings/org.eclipse.core.resources.prefs\n\nThese files are automatically created by pydev (Eclipse plugin for Python).\n\nI know HG will die but this is available with Git too...\n\nJean-Baptiste\n\nCC:  @nthiery @nathanncohen\n\nKeywords: **hgignore**\n\nReviewer: **Volker Braun**\n\nAuthor: **Jean-Baptiste Priez**\n\nMerged: **sage-5.10.beta2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14494_\n\n",
    "closed_at": "2013-05-07T09:06:48Z",
    "created_at": "2013-04-26T15:42:03Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20distribution",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.10",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "HG ignore for eclipse project",
    "type": "issue",
    "updated_at": "2023-01-08T15:27:55Z",
    "url": "https://github.com/sagemath/sage/issues/14494",
    "user": "https://github.com/sagetrac-elixyre"
}
```
Hi,

Could you put these following files in .hgignore

.preferences.xml
.project
.pydevproject
and .settings/org.eclipse.core.resources.prefs

These files are automatically created by pydev (Eclipse plugin for Python).

I know HG will die but this is available with Git too...

Jean-Baptiste

CC:  @nthiery @nathanncohen

Keywords: **hgignore**

Reviewer: **Volker Braun**

Author: **Jean-Baptiste Priez**

Merged: **sage-5.10.beta2**

_Issue created by migration from https://trac.sagemath.org/ticket/14494_





---

archive/issue_events_188175.json:
```json
{
    "actor": "https://github.com/sagetrac-elixyre",
    "created_at": "2013-04-26T15:42:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14494",
    "milestone_number": null,
    "milestone_title": "sage-5.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14494#event-188175"
}
```



---

archive/issue_events_188176.json:
```json
{
    "actor": "https://github.com/sagetrac-elixyre",
    "created_at": "2013-04-26T15:42:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14494",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20distribution",
    "label_color": "000080",
    "label_name": "component: distribution",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14494#event-188176"
}
```



---

archive/issue_events_188177.json:
```json
{
    "actor": "https://github.com/sagetrac-elixyre",
    "created_at": "2013-04-26T15:42:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14494",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14494#event-188177"
}
```



---

archive/issue_events_188178.json:
```json
{
    "actor": "https://github.com/sagetrac-elixyre",
    "created_at": "2013-04-26T15:42:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14494",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14494#event-188178"
}
```



---

archive/issue_events_188179.json:
```json
{
    "actor": "https://github.com/sagetrac-elixyre",
    "created_at": "2013-04-26T15:44:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14494",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14494#event-188179"
}
```



---

archive/issue_comments_177684.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThanks Nathann Cohen for your answer:\nhttp://stackoverflow.com/questions/6644174/mercurial-globally-ignore-files/6644205#6644205\n\nSo sorry for that post...\n\n++",
    "created_at": "2013-04-26T16:00:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14494#issuecomment-177684",
    "user": "https://github.com/sagetrac-elixyre"
}
```

<a id='comment:2'>Comment 2:</a>
Thanks Nathann Cohen for your answer:
http://stackoverflow.com/questions/6644174/mercurial-globally-ignore-files/6644205#6644205

So sorry for that post...

++



---

archive/issue_events_188180.json:
```json
{
    "actor": "https://github.com/sagetrac-elixyre",
    "created_at": "2013-04-26T16:06:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14494",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14494#event-188180"
}
```



---

archive/issue_events_188181.json:
```json
{
    "actor": "https://github.com/sagetrac-elixyre",
    "created_at": "2013-04-26T16:06:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14494",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14494#event-188181"
}
```



---

archive/issue_comments_177685.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nReplying to [elixyre](#comment%3A3):\n\nYou cannot review your own patch (which lacks author information btw.).",
    "created_at": "2013-04-26T16:32:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14494#issuecomment-177685",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:4'>Comment 4:</a>
Replying to [elixyre](#comment%3A3):

You cannot review your own patch (which lacks author information btw.).



---

archive/issue_comments_177686.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nOr should we move this ticket to the \"duplicate/invalid/wontfix\" milestone (as everybody can add those patterns to his/her `.hgrc`)?",
    "created_at": "2013-04-26T16:38:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14494#issuecomment-177686",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:5'>Comment 5:</a>
Or should we move this ticket to the "duplicate/invalid/wontfix" milestone (as everybody can add those patterns to his/her `.hgrc`)?



---

archive/issue_comments_177687.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nReplying to [@nexttime](#comment%3A5):\n> Or should we move this ticket to the \"duplicate/invalid/wontfix\" milestone (as everybody can add those patterns to his/her `.hgrc`)?\n\nAlternatively, repurpose this ticket to add some information to the Sage Developer's Guide?",
    "created_at": "2013-04-26T16:44:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14494#issuecomment-177687",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:6'>Comment 6:</a>
Replying to [@nexttime](#comment%3A5):
> Or should we move this ticket to the "duplicate/invalid/wontfix" milestone (as everybody can add those patterns to his/her `.hgrc`)?

Alternatively, repurpose this ticket to add some information to the Sage Developer's Guide?



---

archive/issue_events_188182.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2013-04-26T16:45:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14494",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14494#event-188182"
}
```



---

archive/issue_events_188183.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2013-04-26T16:45:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14494",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14494#event-188183"
}
```



---

archive/issue_comments_177688.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nI think it doesn't cost us anything to put them in the .hgignore, so we may just as well add it.\n\nThe patch needs author and a commit message. While you are at it, it would be nice to have a comment in the `.hgignore` that this is for eclipse-generated files.",
    "created_at": "2013-04-28T11:29:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14494#issuecomment-177688",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:8'>Comment 8:</a>
I think it doesn't cost us anything to put them in the .hgignore, so we may just as well add it.

The patch needs author and a commit message. While you are at it, it would be nice to have a comment in the `.hgignore` that this is for eclipse-generated files.



---

archive/issue_comments_177689.json:
```json
{
    "body": "new version with author and comment",
    "created_at": "2013-04-29T08:02:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14494#issuecomment-177689",
    "user": "https://github.com/sagetrac-elixyre"
}
```

new version with author and comment



---

archive/issue_comments_177690.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nAttachment: **[trac_14494_hgignore_EliX-jbp.2.patch.gz](https://github.com/sagemath/sage/files/ticket14494/trac_14494_hgignore_EliX-jbp.2.patch.gz)**\n\nI added author, comment and commit message. That's ok?\n\nIf you want to add some information to the Sage Dev's Guide, it's ok but I can't do it, my english is too bad...",
    "created_at": "2013-04-29T08:05:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14494#issuecomment-177690",
    "user": "https://github.com/sagetrac-elixyre"
}
```

<a id='comment:9'>Comment 9:</a>
Attachment: **[trac_14494_hgignore_EliX-jbp.2.patch.gz](https://github.com/sagemath/sage/files/ticket14494/trac_14494_hgignore_EliX-jbp.2.patch.gz)**

I added author, comment and commit message. That's ok?

If you want to add some information to the Sage Dev's Guide, it's ok but I can't do it, my english is too bad...



---

archive/issue_comments_177691.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nLooks good to me",
    "created_at": "2013-04-29T15:15:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14494#issuecomment-177691",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:10'>Comment 10:</a>
Looks good to me



---

archive/issue_comments_177692.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2013-04-29T15:15:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14494#issuecomment-177692",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_events_188184.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-04-29T15:15:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14494",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14494#event-188184"
}
```



---

archive/issue_events_188185.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-04-29T15:15:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14494",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14494#event-188185"
}
```



---

archive/issue_comments_177693.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nThe fastest way to improve your English is of course to write an essay on how to use Sage with Eclipse....",
    "created_at": "2013-04-29T15:16:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14494#issuecomment-177693",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:11'>Comment 11:</a>
The fastest way to improve your English is of course to write an essay on how to use Sage with Eclipse....



---

archive/issue_events_188186.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-04-29T15:16:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14494",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14494#event-188186"
}
```



---

archive/issue_events_188187.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-04-29T15:16:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14494",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14494#event-188187"
}
```



---

archive/issue_comments_177694.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nI can't swear that you will sell many copies, but I'm pretty sure people would find it useful and read it.\n\nRemember to put some ads on the webpage. You never know.. `:-D`\n\nNathann",
    "created_at": "2013-04-29T15:21:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14494#issuecomment-177694",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:12'>Comment 12:</a>
I can't swear that you will sell many copies, but I'm pretty sure people would find it useful and read it.

Remember to put some ads on the webpage. You never know.. `:-D`

Nathann



---

archive/issue_comments_177695.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nI can try to do that but eclipse is not really more efficient than emacs or whatever IDE... It will be short...",
    "created_at": "2013-04-29T17:18:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14494#issuecomment-177695",
    "user": "https://github.com/sagetrac-elixyre"
}
```

<a id='comment:13'>Comment 13:</a>
I can try to do that but eclipse is not really more efficient than emacs or whatever IDE... It will be short...



---

archive/issue_events_188188.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-07T09:06:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14494",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14494#event-188188"
}
```



---

archive/issue_events_188189.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-07T09:06:48Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14494",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14494#event-188189"
}
```



---

archive/issue_comments_177696.json:
```json
{
    "body": "Merged: **sage-5.10.beta2**",
    "created_at": "2013-05-07T09:06:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14494",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14494#issuecomment-177696",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.10.beta2**
