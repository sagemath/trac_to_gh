# Issue 14987: Dilation of empty polyhedra

archive/issues_014750.json:
```json
{
    "assignees": [],
    "body": "There is a bug in the dilation of polyhedra (compare the ambient dimensions):\n\n```\nsage: p = Polyhedron(ambient_dim=2); p\nThe empty polyhedron in ZZ^2\nsage: p.dilation(3)\nThe empty polyhedron in ZZ^0\n```\n\nThe attached patch fixes this.\n\nKeywords: **polyhedra**\n\nReviewer: **Nathann Cohen**\n\nAuthor: **Xavier Caruso**\n\nMerged: **sage-5.12.beta2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14987_\n\n",
    "closed_at": "2013-08-16T21:19:58Z",
    "created_at": "2013-07-31T08:11:32Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.12",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Dilation of empty polyhedra",
    "type": "issue",
    "updated_at": "2013-08-16T21:19:58Z",
    "url": "https://github.com/sagemath/sage/issues/14987",
    "user": "https://github.com/xcaruso"
}
```
There is a bug in the dilation of polyhedra (compare the ambient dimensions):

```
sage: p = Polyhedron(ambient_dim=2); p
The empty polyhedron in ZZ^2
sage: p.dilation(3)
The empty polyhedron in ZZ^0
```

The attached patch fixes this.

Keywords: **polyhedra**

Reviewer: **Nathann Cohen**

Author: **Xavier Caruso**

Merged: **sage-5.12.beta2**

_Issue created by migration from https://trac.sagemath.org/ticket/14987_





---

archive/issue_events_195352.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2013-07-31T08:11:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14987",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14987#event-195352"
}
```



---

archive/issue_events_195353.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2013-07-31T08:11:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14987",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "component: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14987#event-195353"
}
```



---

archive/issue_events_195354.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2013-07-31T08:11:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14987",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14987#event-195354"
}
```



---

archive/issue_events_195355.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2013-07-31T08:11:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14987",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14987#event-195355"
}
```



---

archive/issue_comments_186222.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nCould you replace the reference to the trac ticket by <code>:trac:\\`14987\\`</code> ? That's how we should write it now `:-)`\n\nOnce this is done, you can set this ticket to `positive_review` on my behalf.\n\nThaaaaaaaaaaaanks !\n\nNathann\n\nP.S. : Oh, and there is also a `::` immediately after TESTS which should be a single `:`.",
    "created_at": "2013-07-31T10:46:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14987",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14987#issuecomment-186222",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:1" align="right">Comment 1</div>

Could you replace the reference to the trac ticket by <code>:trac:\`14987\`</code> ? That's how we should write it now `:-)`

Once this is done, you can set this ticket to `positive_review` on my behalf.

Thaaaaaaaaaaaanks !

Nathann

P.S. : Oh, and there is also a `::` immediately after TESTS which should be a single `:`.



---

archive/issue_comments_186223.json:
```json
{
    "body": "Reviewer: **Nathann Cohen**",
    "created_at": "2013-07-31T10:46:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14987",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14987#issuecomment-186223",
    "user": "https://github.com/nathanncohen"
}
```

Reviewer: **Nathann Cohen**



---

archive/issue_events_195356.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-07-31T10:46:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14987",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14987#event-195356"
}
```



---

archive/issue_comments_186224.json:
```json
{
    "body": "Attachment: **[trac_14987_dilation_polyhedrons.patch.gz](https://github.com/sagemath/sage/files/ticket14987/trac_14987_dilation_polyhedrons.patch.gz)**",
    "created_at": "2013-07-31T11:50:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14987",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14987#issuecomment-186224",
    "user": "https://github.com/xcaruso"
}
```

Attachment: **[trac_14987_dilation_polyhedrons.patch.gz](https://github.com/sagemath/sage/files/ticket14987/trac_14987_dilation_polyhedrons.patch.gz)**



---

archive/issue_events_195357.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2013-07-31T11:51:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14987",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14987#event-195357"
}
```



---

archive/issue_events_195358.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2013-07-31T11:51:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14987",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14987#event-195358"
}
```



---

archive/issue_comments_186225.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nDone. Thanks for your review.",
    "created_at": "2013-07-31T11:51:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14987",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14987#issuecomment-186225",
    "user": "https://github.com/xcaruso"
}
```

<div id="comment:2" align="right">Comment 2</div>

Done. Thanks for your review.



---

archive/issue_events_195359.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-16T21:19:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14987",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14987#event-195359"
}
```



---

archive/issue_events_195360.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-16T21:19:58Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14987",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14987#event-195360"
}
```



---

archive/issue_comments_186226.json:
```json
{
    "body": "Merged: **sage-5.12.beta2**",
    "created_at": "2013-08-16T21:19:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14987",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14987#issuecomment-186226",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.12.beta2**
