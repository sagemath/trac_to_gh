# Issue 14842: Add generator for fusenes

archive/issues_014638.json:
```json
{
    "assignees": [],
    "body": "Another class of chemical graphs that are widely studied in chemical graph theory are fusenes and benzenoids. The benzene generator by Gunnar Brinkmann and Gilles Caporossi generates these at a ridiculously fast pace. This ticket adds a package benzene containing the program (the version released with Grinvin, since that version is GPL 3) and also adds some code to call the generator.\n\nNew spkg: http://users.ugent.be/~nvcleemp/benzene-20130630.spkg\n\nApply: \n* [attachment: trac_14842_benzene.2.patch](https://github.com/sagemath/sage/files/ticket14842/trac_14842_benzene.2.patch.gz)\n\nReviewer: **Nathann Cohen**\n\nAuthor: **Nico Van Cleemput**\n\nMerged: **sage-5.12.beta0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14842_\n\n",
    "closed_at": "2013-08-02T14:20:01Z",
    "created_at": "2013-06-30T14:01:14Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.12",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add generator for fusenes",
    "type": "issue",
    "updated_at": "2013-08-02T14:20:01Z",
    "url": "https://github.com/sagemath/sage/issues/14842",
    "user": "https://github.com/nvcleemp"
}
```
Another class of chemical graphs that are widely studied in chemical graph theory are fusenes and benzenoids. The benzene generator by Gunnar Brinkmann and Gilles Caporossi generates these at a ridiculously fast pace. This ticket adds a package benzene containing the program (the version released with Grinvin, since that version is GPL 3) and also adds some code to call the generator.

New spkg: http://users.ugent.be/~nvcleemp/benzene-20130630.spkg

Apply: 
* [attachment: trac_14842_benzene.2.patch](https://github.com/sagemath/sage/files/ticket14842/trac_14842_benzene.2.patch.gz)

Reviewer: **Nathann Cohen**

Author: **Nico Van Cleemput**

Merged: **sage-5.12.beta0**

_Issue created by migration from https://trac.sagemath.org/ticket/14842_





---

archive/issue_events_193707.json:
```json
{
    "actor": "https://github.com/nvcleemp",
    "created_at": "2013-06-30T14:01:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14842",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
    "label_color": "000080",
    "label_name": "component: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14842#event-193707"
}
```



---

archive/issue_events_193708.json:
```json
{
    "actor": "https://github.com/nvcleemp",
    "created_at": "2013-06-30T14:01:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14842",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14842#event-193708"
}
```



---

archive/issue_events_193709.json:
```json
{
    "actor": "https://github.com/nvcleemp",
    "created_at": "2013-06-30T14:01:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14842",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14842#event-193709"
}
```



---

archive/issue_events_193710.json:
```json
{
    "actor": "https://github.com/nvcleemp",
    "created_at": "2013-06-30T14:01:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14842",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14842#event-193710"
}
```



---

archive/issue_comments_184241.json:
```json
{
    "body": "Attachment: **[trac_14842_benzene.patch.gz](https://github.com/sagemath/sage/files/ticket14842/trac_14842_benzene.patch.gz)**",
    "created_at": "2013-06-30T14:07:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14842",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14842#issuecomment-184241",
    "user": "https://github.com/nvcleemp"
}
```

Attachment: **[trac_14842_benzene.patch.gz](https://github.com/sagemath/sage/files/ticket14842/trac_14842_benzene.patch.gz)**



---

archive/issue_comments_184242.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,6 @@\n Another class of chemical graphs that are widely studied in chemical graph theory are fusenes and benzenoids. The benzene generator by Gunnar Brinkmann and Gilles Caporossi generates these at a ridiculously fast pace. This ticket adds a package benzene containing the program (the version released with Grinvin, since that version is GPL 3) and also adds some code to call the generator.\n \n New spkg: http://users.ugent.be/~nvcleemp/benzene-20130630.spkg\n+\n+Apply: \n+* [attachment: trac_14842_benzene.patch](https://github.com/sagemath/sage/files/ticket14842/trac_14842_benzene.patch.gz)\n``````\n",
    "created_at": "2013-06-30T14:08:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14842",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14842#issuecomment-184242",
    "user": "https://github.com/nvcleemp"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,6 @@
 Another class of chemical graphs that are widely studied in chemical graph theory are fusenes and benzenoids. The benzene generator by Gunnar Brinkmann and Gilles Caporossi generates these at a ridiculously fast pace. This ticket adds a package benzene containing the program (the version released with Grinvin, since that version is GPL 3) and also adds some code to call the generator.
 
 New spkg: http://users.ugent.be/~nvcleemp/benzene-20130630.spkg
+
+Apply: 
+* [attachment: trac_14842_benzene.patch](https://github.com/sagemath/sage/files/ticket14842/trac_14842_benzene.patch.gz)
``````




---

archive/issue_events_193711.json:
```json
{
    "actor": "https://github.com/nvcleemp",
    "created_at": "2013-06-30T14:08:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14842",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14842#event-193711"
}
```



---

archive/issue_comments_184243.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nHellooooooooooooooooo !!!\n\nWow ! Very nice patch, very clean, nothing to say about that. Thouuuuuuuuuughh could you make your doctest a bit faster to check ? Right now listing all benzenoids with 12 hexagons eats all my memory and listing all those that have 10 vertices is already quite resource-consuming. Would you mind making this a 9 instead ? It's already around 6505 graphs, and takes 10 seconds on my computer `^^;`\n\nBy the way it is nice that the code is GPL, maybe this package will eventually make it to Sage's standards spkg `:-)`\n\nNathann",
    "created_at": "2013-06-30T18:55:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14842",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14842#issuecomment-184243",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:2" align="right">Comment 2</div>

Hellooooooooooooooooo !!!

Wow ! Very nice patch, very clean, nothing to say about that. Thouuuuuuuuuughh could you make your doctest a bit faster to check ? Right now listing all benzenoids with 12 hexagons eats all my memory and listing all those that have 10 vertices is already quite resource-consuming. Would you mind making this a 9 instead ? It's already around 6505 graphs, and takes 10 seconds on my computer `^^;`

By the way it is nice that the code is GPL, maybe this package will eventually make it to Sage's standards spkg `:-)`

Nathann



---

archive/issue_events_193712.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-06-30T18:55:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14842",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14842#event-193712"
}
```



---

archive/issue_events_193713.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-06-30T18:55:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14842",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14842#event-193713"
}
```



---

archive/issue_events_193714.json:
```json
{
    "actor": "https://github.com/nvcleemp",
    "created_at": "2013-06-30T20:00:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14842",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14842#event-193714"
}
```



---

archive/issue_events_193715.json:
```json
{
    "actor": "https://github.com/nvcleemp",
    "created_at": "2013-06-30T20:00:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14842",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14842#event-193715"
}
```



---

archive/issue_comments_184244.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nAttachment: **[trac_14842_benzene.2.patch.gz](https://github.com/sagemath/sage/files/ticket14842/trac_14842_benzene.2.patch.gz)**\n\nAdded a new version of the patch. I accidently forgot to overwrite the previous one, so only the second patch is needed.\n\nChanges the doctest is no problem. I more or less randomly choose the value, but indeed, it is quite high.\n\nCheers\nNico",
    "created_at": "2013-06-30T20:00:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14842",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14842#issuecomment-184244",
    "user": "https://github.com/nvcleemp"
}
```

<div id="comment:3" align="right">Comment 3</div>

Attachment: **[trac_14842_benzene.2.patch.gz](https://github.com/sagemath/sage/files/ticket14842/trac_14842_benzene.2.patch.gz)**

Added a new version of the patch. I accidently forgot to overwrite the previous one, so only the second patch is needed.

Changes the doctest is no problem. I more or less randomly choose the value, but indeed, it is quite high.

Cheers
Nico



---

archive/issue_comments_184245.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -3,4 +3,4 @@\n New spkg: http://users.ugent.be/~nvcleemp/benzene-20130630.spkg\n \n Apply: \n-* [attachment: trac_14842_benzene.patch](https://github.com/sagemath/sage/files/ticket14842/trac_14842_benzene.patch.gz)\n+* [attachment: trac_14842_benzene.2.patch](https://github.com/sagemath/sage/files/ticket14842/trac_14842_benzene.2.patch.gz)\n``````\n",
    "created_at": "2013-06-30T20:00:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14842",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14842#issuecomment-184245",
    "user": "https://github.com/nvcleemp"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -3,4 +3,4 @@
 New spkg: http://users.ugent.be/~nvcleemp/benzene-20130630.spkg
 
 Apply: 
-* [attachment: trac_14842_benzene.patch](https://github.com/sagemath/sage/files/ticket14842/trac_14842_benzene.patch.gz)
+* [attachment: trac_14842_benzene.2.patch](https://github.com/sagemath/sage/files/ticket14842/trac_14842_benzene.2.patch.gz)
``````




---

archive/issue_comments_184246.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nWell, then... `:-)`\n\nThanks !\n\nNathann",
    "created_at": "2013-06-30T20:01:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14842",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14842#issuecomment-184246",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:4" align="right">Comment 4</div>

Well, then... `:-)`

Thanks !

Nathann



---

archive/issue_comments_184247.json:
```json
{
    "body": "Reviewer: **Nathann Cohen**",
    "created_at": "2013-06-30T20:01:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14842",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14842#issuecomment-184247",
    "user": "https://github.com/nathanncohen"
}
```

Reviewer: **Nathann Cohen**



---

archive/issue_events_193716.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-06-30T20:01:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14842",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14842#event-193716"
}
```



---

archive/issue_events_193717.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-06-30T20:01:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14842",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14842#event-193717"
}
```



---

archive/issue_events_193718.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-07-04T21:50:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14842",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14842#event-193718"
}
```



---

archive/issue_events_193719.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-07-04T21:50:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14842",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14842#event-193719"
}
```



---

archive/issue_comments_184248.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nspkg landed on the servers \u2026",
    "created_at": "2013-07-30T10:36:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14842",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14842#issuecomment-184248",
    "user": "https://github.com/haraldschilly"
}
```

<div id="comment:6" align="right">Comment 6</div>

spkg landed on the servers …



---

archive/issue_comments_184249.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nCoooooooooool `:-)`\n\nNathann",
    "created_at": "2013-07-30T10:37:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14842",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14842#issuecomment-184249",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:7" align="right">Comment 7</div>

Coooooooooool `:-)`

Nathann



---

archive/issue_comments_184250.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nReplying to [@nathanncohen](#comment%3A7):\n> Coooooooooool `:-)`\n> \n> Nathann\n\n\nMy thoughts exactly! ;-)",
    "created_at": "2013-07-30T10:39:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14842",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14842#issuecomment-184250",
    "user": "https://github.com/nvcleemp"
}
```

<div id="comment:8" align="right">Comment 8</div>

Replying to [@nathanncohen](#comment%3A7):
> Coooooooooool `:-)`
> 
> Nathann


My thoughts exactly! ;-)



---

archive/issue_comments_184251.json:
```json
{
    "body": "Merged: **sage-5.12.beta0**",
    "created_at": "2013-08-02T14:20:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14842",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14842#issuecomment-184251",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.12.beta0**



---

archive/issue_events_193720.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-02T14:20:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14842",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14842#event-193720"
}
```



---

archive/issue_events_193721.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-02T14:20:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14842",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14842#event-193721"
}
```
