# Issue 14820: sage -f fails on a pristine source tree

archive/issues_014616.json:
```json
{
    "body": "Because `SAGE_LOCAL` does not yet exist you cannot install spkgs. Trivial fix is attached.\n\n---\n\n**Apply:**\n* [attachment:trac_14820_mkdir_SAGE_LOCAL.patch] to the Sage \"root\" repository\n\n**Assignee:** GeorgSWeber\n\n**Keywords:** SAGE_LOCAL write access\n\n**Reviewer:** Leif Leonhardy\n\n**Author:** Volker Braun\n\n**Merged:** sage-5.11.rc0\n\nIssue created by migration from https://trac.sagemath.org/ticket/14820\n\n",
    "closed_at": "2013-07-31T12:57:14Z",
    "created_at": "2013-06-26T04:31:13Z",
    "labels": [
        "component: build",
        "blocker",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.11",
    "title": "sage -f fails on a pristine source tree",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/14820",
    "user": "https://github.com/vbraun"
}
```
Because `SAGE_LOCAL` does not yet exist you cannot install spkgs. Trivial fix is attached.

---

**Apply:**
* [attachment:trac_14820_mkdir_SAGE_LOCAL.patch] to the Sage "root" repository

**Assignee:** GeorgSWeber

**Keywords:** SAGE_LOCAL write access

**Reviewer:** Leif Leonhardy

**Author:** Volker Braun

**Merged:** sage-5.11.rc0

Issue created by migration from https://trac.sagemath.org/ticket/14820





---

archive/issue_comments_183681.json:
```json
{
    "body": "Initial patch",
    "created_at": "2013-06-26T04:32:08Z",
    "issue": "https://github.com/sagemath/sage/issues/14820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14820#issuecomment-183681",
    "user": "https://github.com/vbraun"
}
```

Initial patch



---

archive/issue_comments_183682.json:
```json
{
    "body": "**Author:** Volker Braun",
    "created_at": "2013-06-26T04:35:01Z",
    "issue": "https://github.com/sagemath/sage/issues/14820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14820#issuecomment-183682",
    "user": "https://github.com/vbraun"
}
```

**Author:** Volker Braun



---

archive/issue_events_129790.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-06-26T04:35:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14820",
    "label": "blocker",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14820#event-129790"
}
```



---

archive/attachments_019950.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_14820_mkdir_SAGE_LOCAL.patch",
    "asset_url": "tarball://root/attachments/ticket14820/trac_14820_mkdir_SAGE_LOCAL.patch",
    "created_at": "2013-06-26T04:35:01Z",
    "issue": "https://github.com/sagemath/sage/issues/14820",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket14820/trac_14820_mkdir_SAGE_LOCAL.patch",
    "user": "https://github.com/vbraun"
}
```



---

archive/issue_comments_183683.json:
```json
{
    "body": "<a id='comment:1'></a>\nAttachment [trac_14820_mkdir_SAGE_LOCAL.patch](https://github.com/sagemath/sage/files/ticket14820/trac_14820_mkdir_SAGE_LOCAL.patch) by @vbraun created at 2013-06-26 04:35:01\n\nI'm making this a blocker since it is a potential PITA for support questions",
    "created_at": "2013-06-26T04:35:01Z",
    "issue": "https://github.com/sagemath/sage/issues/14820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14820#issuecomment-183683",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:1'></a>
Attachment [trac_14820_mkdir_SAGE_LOCAL.patch](https://github.com/sagemath/sage/files/ticket14820/trac_14820_mkdir_SAGE_LOCAL.patch) by @vbraun created at 2013-06-26 04:35:01

I'm making this a blocker since it is a potential PITA for support questions



---

archive/issue_events_129791.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-06-26T04:35:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14820",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14820#event-129791"
}
```



---

archive/issue_comments_183684.json:
```json
{
    "body": "**Changing keywords** from \"\" to \"SAGE_LOCAL write access\".",
    "created_at": "2013-06-29T10:25:13Z",
    "issue": "https://github.com/sagemath/sage/issues/14820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14820#issuecomment-183684",
    "user": "https://github.com/nexttime"
}
```

**Changing keywords** from "" to "SAGE_LOCAL write access".



---

archive/issue_comments_183685.json:
```json
{
    "body": "<a id='comment:2'></a>\nNot the most beautiful solution, but...",
    "created_at": "2013-06-29T10:25:13Z",
    "issue": "https://github.com/sagemath/sage/issues/14820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14820#issuecomment-183685",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:2'></a>
Not the most beautiful solution, but...



---

archive/issue_comments_183686.json:
```json
{
    "body": "**Description changed:**\n``````diff\n--- \n+++ \n@@ -1,3 +1,6 @@\n-Because `SAGE_LOCAL` does not yet exist you cannot download spkgs. Trivial fix is attached.\n+Because `SAGE_LOCAL` does not yet exist you cannot install spkgs. Trivial fix is attached.\n \n+---\n \n+**Apply:**\n+* [attachment:trac_14820_mkdir_SAGE_LOCAL.patch] to the Sage \"root\" repository\n``````\n",
    "created_at": "2013-06-29T10:25:13Z",
    "issue": "https://github.com/sagemath/sage/issues/14820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14820#issuecomment-183686",
    "user": "https://github.com/nexttime"
}
```

**Description changed:**
``````diff
--- 
+++ 
@@ -1,3 +1,6 @@
-Because `SAGE_LOCAL` does not yet exist you cannot download spkgs. Trivial fix is attached.
+Because `SAGE_LOCAL` does not yet exist you cannot install spkgs. Trivial fix is attached.
 
+---
 
+**Apply:**
+* [attachment:trac_14820_mkdir_SAGE_LOCAL.patch] to the Sage "root" repository
``````




---

archive/issue_events_129792.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2013-06-29T10:25:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14820",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14820#event-129792"
}
```



---

archive/issue_events_129793.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2013-06-29T10:25:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14820",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14820#event-129793"
}
```



---

archive/issue_comments_183687.json:
```json
{
    "body": "**Reviewer:** Leif Leonhardy",
    "created_at": "2013-06-29T10:25:13Z",
    "issue": "https://github.com/sagemath/sage/issues/14820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14820#issuecomment-183687",
    "user": "https://github.com/nexttime"
}
```

**Reviewer:** Leif Leonhardy



---

archive/issue_comments_183688.json:
```json
{
    "body": "<a id='comment:3'></a>\nP.S.: Btw, Jeroen's comments refer to the wrong ticket (#5852 instead of #5825)...",
    "created_at": "2013-06-29T10:38:13Z",
    "issue": "https://github.com/sagemath/sage/issues/14820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14820#issuecomment-183688",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:3'></a>
P.S.: Btw, Jeroen's comments refer to the wrong ticket (#5852 instead of #5825)...



---

archive/issue_events_129794.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-07-21T21:43:14Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14820",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14820#event-129794"
}
```



---

archive/issue_events_129795.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-07-21T21:43:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14820",
    "milestone": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14820#event-129795"
}
```



---

archive/issue_comments_183689.json:
```json
{
    "body": "**Merged:** sage-5.11.rc0",
    "created_at": "2013-07-31T12:57:14Z",
    "issue": "https://github.com/sagemath/sage/issues/14820",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14820#issuecomment-183689",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-5.11.rc0



---

archive/issue_events_129796.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-07-31T12:57:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14820",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14820#event-129796"
}
```



---

archive/issue_events_129797.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-07-31T12:57:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14820",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14820#event-129797"
}
```
