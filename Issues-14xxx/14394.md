# Issue 14394: FaceFan bug if origin is on boundary

archive/issues_014190.json:
```json
{
    "assignees": [],
    "body": "If the origin is on the boundary of the polytope then FaceFan fails to raise an appropriate error:\n\n```\nsage: FaceFan(Polyhedron([(-1,0), (1,0), (0,1)])).generating_cones()       \n(2-d cone of Rational polyhedral fan in 2-d lattice N,\n 1-d cone of Rational polyhedral fan in 2-d lattice N,\n 2-d cone of Rational polyhedral fan in 2-d lattice N)\n```\n\nDepends on #13194\n\nAssignee: **@sagetrac-mhampton**\n\nCC:  @novoselt\n\nKeywords: **toric**\n\nReviewer: **Andrey Novoseltsev**\n\nAuthor: **Volker Braun**\n\nMerged: **sage-5.10.beta0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14394_\n\n",
    "closed_at": "2013-04-13T13:52:53Z",
    "created_at": "2013-03-31T21:06:44Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.10",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "FaceFan bug if origin is on boundary",
    "type": "issue",
    "updated_at": "2013-04-13T13:52:53Z",
    "url": "https://github.com/sagemath/sage/issues/14394",
    "user": "https://github.com/vbraun"
}
```
If the origin is on the boundary of the polytope then FaceFan fails to raise an appropriate error:

```
sage: FaceFan(Polyhedron([(-1,0), (1,0), (0,1)])).generating_cones()       
(2-d cone of Rational polyhedral fan in 2-d lattice N,
 1-d cone of Rational polyhedral fan in 2-d lattice N,
 2-d cone of Rational polyhedral fan in 2-d lattice N)
```

Depends on #13194

Assignee: **@sagetrac-mhampton**

CC:  @novoselt

Keywords: **toric**

Reviewer: **Andrey Novoseltsev**

Author: **Volker Braun**

Merged: **sage-5.10.beta0**

_Issue created by migration from https://trac.sagemath.org/ticket/14394_





---

archive/issue_events_186642.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-03-31T21:06:44Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14394",
    "milestone_number": null,
    "milestone_title": "sage-5.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14394#event-186642"
}
```



---

archive/issue_events_186643.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-03-31T21:06:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14394",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "component: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14394#event-186643"
}
```



---

archive/issue_events_186644.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-03-31T21:06:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14394",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14394#event-186644"
}
```



---

archive/issue_events_186645.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-03-31T21:06:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14394",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14394#event-186645"
}
```



---

archive/issue_comments_175580.json:
```json
{
    "body": "Dependencies: **#13194**",
    "created_at": "2013-04-01T17:36:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14394",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14394#issuecomment-175580",
    "user": "https://github.com/vbraun"
}
```

Dependencies: **#13194**



---

archive/issue_comments_175581.json:
```json
{
    "body": "Author: **Volker Braun**",
    "created_at": "2013-04-01T17:36:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14394",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14394#issuecomment-175581",
    "user": "https://github.com/vbraun"
}
```

Author: **Volker Braun**



---

archive/issue_comments_175582.json:
```json
{
    "body": "Attachment: **[trac_14394_face_fan_bug.patch.gz](https://github.com/sagemath/sage/files/ticket14394/trac_14394_face_fan_bug.patch.gz)**\n\nInitial patch",
    "created_at": "2013-04-01T17:37:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14394",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14394#issuecomment-175582",
    "user": "https://github.com/vbraun"
}
```

Attachment: **[trac_14394_face_fan_bug.patch.gz](https://github.com/sagemath/sage/files/ticket14394/trac_14394_face_fan_bug.patch.gz)**

Initial patch



---

archive/issue_events_186646.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-04-01T17:37:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14394",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14394#event-186646"
}
```



---

archive/issue_comments_175583.json:
```json
{
    "body": "Changed keywords from none to **toric**",
    "created_at": "2013-04-02T06:05:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14394",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14394#issuecomment-175583",
    "user": "https://github.com/novoselt"
}
```

Changed keywords from none to **toric**



---

archive/issue_comments_175584.json:
```json
{
    "body": "Reviewer: **Andrey Novoseltsev**",
    "created_at": "2013-04-02T06:05:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14394",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14394#issuecomment-175584",
    "user": "https://github.com/novoselt"
}
```

Reviewer: **Andrey Novoseltsev**



---

archive/issue_comments_175585.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nAttachment: **[trac_14394_reviewer.patch.gz](https://github.com/sagemath/sage/files/ticket14394/trac_14394_reviewer.patch.gz)**\n\nSome tweaks to documentation and switch to `NotImplementedError` left from #13194, please switch to positive review if changes are OK!",
    "created_at": "2013-04-02T06:05:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14394",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14394#issuecomment-175585",
    "user": "https://github.com/novoselt"
}
```

<div id="comment:3" align="right">Comment 3</div>

Attachment: **[trac_14394_reviewer.patch.gz](https://github.com/sagemath/sage/files/ticket14394/trac_14394_reviewer.patch.gz)**

Some tweaks to documentation and switch to `NotImplementedError` left from #13194, please switch to positive review if changes are OK!



---

archive/issue_events_186647.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-04-02T09:01:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14394",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14394#event-186647"
}
```



---

archive/issue_events_186648.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-04-02T09:01:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14394",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14394#event-186648"
}
```



---

archive/issue_comments_175586.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThanks, looks good to me.",
    "created_at": "2013-04-02T09:01:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14394",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14394#issuecomment-175586",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:4" align="right">Comment 4</div>

Thanks, looks good to me.



---

archive/issue_events_186649.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-04-05T13:36:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14394",
    "milestone_number": null,
    "milestone_title": "sage-5.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14394#event-186649"
}
```



---

archive/issue_events_186650.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-04-05T13:36:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14394",
    "milestone_number": null,
    "milestone_title": "sage-5.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14394#event-186650"
}
```



---

archive/issue_events_186651.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-04-13T13:52:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14394",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14394#event-186651"
}
```



---

archive/issue_events_186652.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-04-13T13:52:53Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14394",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14394#event-186652"
}
```



---

archive/issue_comments_175587.json:
```json
{
    "body": "Merged: **sage-5.10.beta0**",
    "created_at": "2013-04-13T13:52:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14394",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14394#issuecomment-175587",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.10.beta0**
