# Issue 14673: turn crystalographic into crystallographic

archive/issues_014469.json:
```json
{
    "assignees": [],
    "body": "The word crystallographic should be spelled with two \"l\".\n\nThe spelling with one \"l\" is very rare.\n\nFor a comparison, try your favorite search engine.\n\nThis patch changes all the occurences of 'crystalographic\" into \"crystallographic\"\n\n\nAssignee: **@sagetrac-sage-combinat**\n\nCC:  @sagetrac-sage-combinat\n\nKeywords: **spelling**\n\nReviewer: **Travis Scrimshaw, Nicolas M. Thi\u00e9ry**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\nMerged: **sage-5.10.rc1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14673_\n\n",
    "closed_at": "2013-06-05T19:50:45Z",
    "created_at": "2013-05-31T19:34:30Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.10",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "turn crystalographic into crystallographic",
    "type": "issue",
    "updated_at": "2013-06-05T19:50:45Z",
    "url": "https://github.com/sagemath/sage/issues/14673",
    "user": "https://github.com/fchapoton"
}
```
The word crystallographic should be spelled with two "l".

The spelling with one "l" is very rare.

For a comparison, try your favorite search engine.

This patch changes all the occurences of 'crystalographic" into "crystallographic"


Assignee: **@sagetrac-sage-combinat**

CC:  @sagetrac-sage-combinat

Keywords: **spelling**

Reviewer: **Travis Scrimshaw, Nicolas M. Thiéry**

Author: **Frédéric Chapoton**

Merged: **sage-5.10.rc1**

_Issue created by migration from https://trac.sagemath.org/ticket/14673_





---

archive/issue_events_190942.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-31T19:34:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14673#event-190942"
}
```



---

archive/issue_events_190943.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-31T19:34:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14673#event-190943"
}
```



---

archive/issue_events_190944.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-31T19:34:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14673#event-190944"
}
```



---

archive/issue_events_190945.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-31T19:34:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14673#event-190945"
}
```



---

archive/issue_events_190946.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-31T19:38:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14673#event-190946"
}
```



---

archive/issue_comments_180703.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nHi Fr\u00e9d\u00e9ric,\n\nWhile I definitely see this as a worthwhile improvement, one will need to devise a plan for:\n\n- maintaining backward compatibility, with appropriate deprecation, for some time\n- making sure that this does not conflict with too many patches outthere\n\nCheers,\n                       Nicolas",
    "created_at": "2013-05-31T19:43:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14673#issuecomment-180703",
    "user": "https://github.com/nthiery"
}
```

<div id="comment:2" align="right">Comment 2</div>

Hi Frédéric,

While I definitely see this as a worthwhile improvement, one will need to devise a plan for:

- maintaining backward compatibility, with appropriate deprecation, for some time
- making sure that this does not conflict with too many patches outthere

Cheers,
                       Nicolas



---

archive/issue_comments_180704.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nI have added some aliases with deprecation warnings for backward compatibility.",
    "created_at": "2013-05-31T20:09:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14673#issuecomment-180704",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">Comment 3</div>

I have added some aliases with deprecation warnings for backward compatibility.



---

archive/issue_comments_180705.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nNicolas,\n\nI don't think this will break much, if anything, in the queue. I'll try this out later tonight and can do the review then too.\n\nBest,\n\nTravis",
    "created_at": "2013-05-31T22:25:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14673#issuecomment-180705",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:4" align="right">Comment 4</div>

Nicolas,

I don't think this will break much, if anything, in the queue. I'll try this out later tonight and can do the review then too.

Best,

Travis



---

archive/issue_events_190947.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-06-01T16:35:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14673#event-190947"
}
```



---

archive/issue_events_190948.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-06-01T16:35:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14673#event-190948"
}
```



---

archive/issue_comments_180706.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2013-06-01T16:35:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14673#issuecomment-180706",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_180707.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThis didn't really break anything (some trivial rebasing due to the extra import). There might be one or two additional rebased in the queue that are necessary, but I'm giving this a positive review since the aliases will keep it backwards compatible.\n\nBest,\n\nTravis",
    "created_at": "2013-06-01T16:35:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14673#issuecomment-180707",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:5" align="right">Comment 5</div>

This didn't really break anything (some trivial rebasing due to the extra import). There might be one or two additional rebased in the queue that are necessary, but I'm giving this a positive review since the aliases will keep it backwards compatible.

Best,

Travis



---

archive/issue_events_190949.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-03T11:37:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14673#event-190949"
}
```



---

archive/issue_events_190950.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-03T11:37:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "milestone_number": null,
    "milestone_title": "sage-5.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14673#event-190950"
}
```



---

archive/issue_events_190951.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2013-06-03T17:57:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14673#event-190951"
}
```



---

archive/issue_events_190952.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2013-06-03T17:57:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14673#event-190952"
}
```



---

archive/issue_comments_180708.json:
```json
{
    "body": "Changed reviewer from **Travis Scrimshaw** to **Travis Scrimshaw, Nicolas M. Thi\u00e9ry**",
    "created_at": "2013-06-03T17:57:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14673#issuecomment-180708",
    "user": "https://github.com/nthiery"
}
```

Changed reviewer from **Travis Scrimshaw** to **Travis Scrimshaw, Nicolas M. Thiéry**



---

archive/issue_comments_180709.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nHi Fr\u00e9d\u00e9ric!\n\nThanks for fixing this original typo of mine :-)\n\nI went through the patch and am happy with it up to two details:\n\n- Unless I missed it, there lacks a backward compatibility alias for CartanType.samples(crystalographic=True).\n\n- It would be good to add tests for the backward compatibility aliases.",
    "created_at": "2013-06-03T17:57:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14673#issuecomment-180709",
    "user": "https://github.com/nthiery"
}
```

<div id="comment:7" align="right">Comment 7</div>

Hi Frédéric!

Thanks for fixing this original typo of mine :-)

I went through the patch and am happy with it up to two details:

- Unless I missed it, there lacks a backward compatibility alias for CartanType.samples(crystalographic=True).

- It would be good to add tests for the backward compatibility aliases.



---

archive/issue_comments_180710.json:
```json
{
    "body": "Work Issues: **missing backward compatibility alias in CartanType.samples**",
    "created_at": "2013-06-03T17:57:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14673#issuecomment-180710",
    "user": "https://github.com/nthiery"
}
```

Work Issues: **missing backward compatibility alias in CartanType.samples**



---

archive/issue_events_190953.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-06-03T18:51:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14673#event-190953"
}
```



---

archive/issue_events_190954.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-06-03T18:51:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14673#event-190954"
}
```



---

archive/issue_comments_180711.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nAttachment: **[trac_14673_crystallo-fc.patch.gz](https://github.com/sagemath/sage/files/ticket14673/trac_14673_crystallo-fc.patch.gz)**\n\ndone",
    "created_at": "2013-06-03T18:51:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14673#issuecomment-180711",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:8" align="right">Comment 8</div>

Attachment: **[trac_14673_crystallo-fc.patch.gz](https://github.com/sagemath/sage/files/ticket14673/trac_14673_crystallo-fc.patch.gz)**

done



---

archive/issue_comments_180712.json:
```json
{
    "body": "Changed work issues from **missing backward compatibility alias in CartanType.samples** to none",
    "created_at": "2013-06-04T18:44:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14673#issuecomment-180712",
    "user": "https://github.com/tscrim"
}
```

Changed work issues from **missing backward compatibility alias in CartanType.samples** to none



---

archive/issue_comments_180713.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nLooks good to me. Nicolas?",
    "created_at": "2013-06-04T18:44:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14673#issuecomment-180713",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:9" align="right">Comment 9</div>

Looks good to me. Nicolas?



---

archive/issue_comments_180714.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\n+1!\n\nThanks Fr\u00e9d\u00e9ric!",
    "created_at": "2013-06-04T19:05:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14673#issuecomment-180714",
    "user": "https://github.com/nthiery"
}
```

<div id="comment:10" align="right">Comment 10</div>

+1!

Thanks Frédéric!



---

archive/issue_events_190955.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2013-06-04T19:05:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14673#event-190955"
}
```



---

archive/issue_events_190956.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2013-06-04T19:05:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14673#event-190956"
}
```



---

archive/issue_events_190957.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-05T19:50:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14673#event-190957"
}
```



---

archive/issue_events_190958.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-05T19:50:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14673#event-190958"
}
```



---

archive/issue_comments_180715.json:
```json
{
    "body": "Merged: **sage-5.10.rc1**",
    "created_at": "2013-06-05T19:50:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14673",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14673#issuecomment-180715",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.10.rc1**
