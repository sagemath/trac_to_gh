# Issue 14831: Doc typo in .is_similar() method

archive/issues_014627.json:
```json
{
    "assignees": [],
    "body": "In the doc of the method `.is_similar()` (located in in `sage/sage/matrix/matrix2.pyx`), we get this unreadable block:\n\n```\n      sage: A = matrix(QQ, [[ 19, -7, -29], ...\n      [-16, 11,  30], ...                   [ 15, -7, -25]]) sage: B =\n      matrix(QQ, [[-38, -63,  42], ...                   [ 14,  25,\n      -14], ...                   [-14, -21,  18]]) sage: A.charpoly()\n      == B.charpoly() True sage: A.jordan_form() [-3| 0  0] [--+-----]\n      [ 0| 4  1] [ 0| 0  4] sage: B.jordan_form() [-3| 0| 0]\n      [--+--+--] [ 0| 4| 0] [--+--+--] [ 0| 0| 4] sage:\n      A.is_similar(B) False\n```\n\nThis is due to a missing `::`.\n\nAssignee: **mvngu**\n\nReviewer: **Thierry Monteil**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\nMerged: **sage-5.12.beta1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14831_\n\n",
    "closed_at": "2013-08-16T21:12:50Z",
    "created_at": "2013-06-26T22:06:34Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/good%20first%20issue"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.12",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Doc typo in .is_similar() method",
    "type": "issue",
    "updated_at": "2013-08-16T21:12:50Z",
    "url": "https://github.com/sagemath/sage/issues/14831",
    "user": "https://github.com/sagetrac-tmonteil"
}
```
In the doc of the method `.is_similar()` (located in in `sage/sage/matrix/matrix2.pyx`), we get this unreadable block:

```
      sage: A = matrix(QQ, [[ 19, -7, -29], ...
      [-16, 11,  30], ...                   [ 15, -7, -25]]) sage: B =
      matrix(QQ, [[-38, -63,  42], ...                   [ 14,  25,
      -14], ...                   [-14, -21,  18]]) sage: A.charpoly()
      == B.charpoly() True sage: A.jordan_form() [-3| 0  0] [--+-----]
      [ 0| 4  1] [ 0| 0  4] sage: B.jordan_form() [-3| 0| 0]
      [--+--+--] [ 0| 4| 0] [--+--+--] [ 0| 0| 4] sage:
      A.is_similar(B) False
```

This is due to a missing `::`.

Assignee: **mvngu**

Reviewer: **Thierry Monteil**

Author: **Frédéric Chapoton**

Merged: **sage-5.12.beta1**

_Issue created by migration from https://trac.sagemath.org/ticket/14831_





---

archive/issue_events_193547.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2013-06-26T22:06:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14831",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14831#event-193547"
}
```



---

archive/issue_events_193548.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2013-06-26T22:06:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14831",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14831#event-193548"
}
```



---

archive/issue_events_193549.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2013-06-26T22:06:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14831",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14831#event-193549"
}
```



---

archive/issue_events_193550.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2013-06-26T22:06:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14831",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14831#event-193550"
}
```



---

archive/issue_events_193551.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2013-06-26T22:06:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14831",
    "label": "https://github.com/sagemath/sage/labels/good%20first%20issue",
    "label_color": "7057ff",
    "label_name": "good first issue",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14831#event-193551"
}
```



---

archive/issue_comments_184030.json:
```json
{
    "body": "Author: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2013-07-08T11:44:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14831",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14831#issuecomment-184030",
    "user": "https://github.com/fchapoton"
}
```

Author: **Frédéric Chapoton**



---

archive/issue_comments_184031.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[trac_14831.patch.gz](https://github.com/sagemath/sage/files/ticket14831/trac_14831.patch.gz)**\n\nhere is a patch, please review",
    "created_at": "2013-07-08T11:44:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14831",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14831#issuecomment-184031",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[trac_14831.patch.gz](https://github.com/sagemath/sage/files/ticket14831/trac_14831.patch.gz)**

here is a patch, please review



---

archive/issue_events_193552.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-07-08T11:44:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14831",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14831#event-193552"
}
```



---

archive/issue_comments_184032.json:
```json
{
    "body": "Reviewer: **Thierry Monteil**",
    "created_at": "2013-07-08T14:12:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14831",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14831#issuecomment-184032",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Reviewer: **Thierry Monteil**



---

archive/issue_events_193553.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2013-07-08T14:12:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14831",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14831#event-193553"
}
```



---

archive/issue_events_193554.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2013-07-08T14:12:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14831",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14831#event-193554"
}
```



---

archive/issue_comments_184033.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nLooks good to me, though i am not sure you should consider yourself as a beginner ;)",
    "created_at": "2013-07-08T14:12:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14831",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14831#issuecomment-184033",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

<a id='comment:2'>Comment 2:</a>
Looks good to me, though i am not sure you should consider yourself as a beginner ;)



---

archive/issue_events_193555.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-16T21:12:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14831",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14831#event-193555"
}
```



---

archive/issue_events_193556.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-16T21:12:50Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14831",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14831#event-193556"
}
```



---

archive/issue_comments_184034.json:
```json
{
    "body": "Merged: **sage-5.12.beta1**",
    "created_at": "2013-08-16T21:12:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14831",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14831#issuecomment-184034",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.12.beta1**
