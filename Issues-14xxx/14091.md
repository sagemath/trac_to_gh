# Issue 14091: Fix intersphinx links to the reference manual from other documents

archive/issues_013887.json:
```json
{
    "body": "#9128 enabled links to the reference manual from other documents like the thematic tutorials. \nThis import feature is currently broken. Until this is fixed, the updated thematic tutorial index from #14090 will look lame.\n\nThe issue to be due to a wrong initialization order for the sphinx app. At least the attached patch, which I am using for now for the Sage-Combinat tutorials, seems to work around it. Probably the sphinx internals were change in the mean time. #13057 may have had an influence too.\n\nNote: I am not suggesting to merge or review the currently attached patch. It's just there to point where the problem seem to be.\n\n\n\nAssignee: mvngu\n\nCC:  sage-combinat\n\nKeywords: intersphinx, thematic tutorials\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/14091\n\n",
    "closed_at": "2015-05-19T06:43:35Z",
    "created_at": "2013-02-09T23:33:13Z",
    "labels": [
        "component: documentation",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "Fix intersphinx links to the reference manual from other documents",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/14091",
    "user": "https://github.com/nthiery"
}
```
#9128 enabled links to the reference manual from other documents like the thematic tutorials. 
This import feature is currently broken. Until this is fixed, the updated thematic tutorial index from #14090 will look lame.

The issue to be due to a wrong initialization order for the sphinx app. At least the attached patch, which I am using for now for the Sage-Combinat tutorials, seems to work around it. Probably the sphinx internals were change in the mean time. #13057 may have had an influence too.

Note: I am not suggesting to merge or review the currently attached patch. It's just there to point where the problem seem to be.



Assignee: mvngu

CC:  sage-combinat

Keywords: intersphinx, thematic tutorials

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/14091





---

archive/issue_comments_184218.json:
```json
{
    "body": "Attachment [intersphinx-workaround-nt.patch](tarball://root/attachments/some-uuid/ticket14091/intersphinx-workaround-nt.patch) by @nthiery created at 2013-02-09 23:33:29",
    "created_at": "2013-02-09T23:33:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14091#issuecomment-184218",
    "user": "https://github.com/nthiery"
}
```

Attachment [intersphinx-workaround-nt.patch](tarball://root/attachments/some-uuid/ticket14091/intersphinx-workaround-nt.patch) by @nthiery created at 2013-02-09 23:33:29



---

archive/issue_events_039681.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14091",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14091#event-39681"
}
```



---

archive/issue_events_039682.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14091",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14091#event-39682"
}
```



---

archive/issue_events_039683.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14091",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14091#event-39683"
}
```



---

archive/issue_events_039684.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14091",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14091#event-39684"
}
```



---

archive/issue_events_039685.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14091",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14091#event-39685"
}
```



---

archive/issue_events_039686.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14091",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14091#event-39686"
}
```



---

archive/issue_events_039687.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14091",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14091#event-39687"
}
```



---

archive/issue_comments_184219.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2015-05-12T08:42:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14091#issuecomment-184219",
    "user": "https://github.com/nthiery"
}
```

Changing status from new to needs_review.



---

archive/issue_events_039688.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2015-05-12T08:42:34Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14091",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14091#event-39688"
}
```



---

archive/issue_events_039689.json:
```json
{
    "actor": "https://github.com/nthiery",
    "created_at": "2015-05-12T08:42:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14091",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14091#event-39689"
}
```



---

archive/issue_comments_184220.json:
```json
{
    "body": "<a id='comment:5'></a>This seems to be fixed now. See e.g. the crosslink \"Introduction to combinatorics in Sage\" which points to the reference manual in:\n\nhttp://www.sagemath.org/doc/thematic_tutorials/index.html\n\nI thus mark this ticket as duplicate. If anyone has an idea of when this was fixed, it would be good to add a ref.",
    "created_at": "2015-05-12T08:42:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14091#issuecomment-184220",
    "user": "https://github.com/nthiery"
}
```

<a id='comment:5'></a>This seems to be fixed now. See e.g. the crosslink "Introduction to combinatorics in Sage" which points to the reference manual in:

http://www.sagemath.org/doc/thematic_tutorials/index.html

I thus mark this ticket as duplicate. If anyone has an idea of when this was fixed, it would be good to add a ref.



---

archive/issue_comments_184221.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2015-05-12T08:42:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14091#issuecomment-184221",
    "user": "https://github.com/nthiery"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_184222.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2015-05-19T06:43:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14091",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14091#issuecomment-184222",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_events_039690.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-05-19T06:43:35Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/14091",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14091#event-39690"
}
```
