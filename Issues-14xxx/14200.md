# Issue 14200: Add shortcut to add multiple variables with different names

archive/issues_013996.json:
```json
{
    "assignees": [],
    "body": "People have been asking me for an easy/automated way to create polynomial rings in two sets of varibles such as `x0, x1, x2, x3, y0, y1, y2, y3`. This patch provides a way to do so:\n\n```\nsage: PolynomialRing(ZZ, 3, var_array=['x','y'])\nMultivariate Polynomial Ring in x0, y0, x1, y1, x2, y2 over Integer Ring\nsage: PolynomialRing(ZZ, 3, var_array='a,b')\nMultivariate Polynomial Ring in a0, b0, a1, b1, a2, b2 over Integer Ring\nsage: PolynomialRing(ZZ, 2, 3, var_array='m')\nMultivariate Polynomial Ring in m00, m01, m02, m10, m11, m12 over Integer Ring\nsage: PolynomialRing(ZZ, 2, var_array='m')\nMultivariate Polynomial Ring in m00, m01, m10, m11, over Integer Ring\n```\n\nAssignee: **@tscrim**\n\nCC:  billey\n\nKeywords: **polynomials, variables**\n\nReviewer: **Sara Billey**\n\nAuthor: **Travis Scrimshaw**\n\nMerged: **sage-5.8.beta3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14200_\n\n",
    "closed_at": "2013-03-04T07:39:06Z",
    "created_at": "2013-02-27T21:45:15Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add shortcut to add multiple variables with different names",
    "type": "issue",
    "updated_at": "2013-03-04T07:39:06Z",
    "url": "https://github.com/sagemath/sage/issues/14200",
    "user": "https://github.com/tscrim"
}
```
People have been asking me for an easy/automated way to create polynomial rings in two sets of varibles such as `x0, x1, x2, x3, y0, y1, y2, y3`. This patch provides a way to do so:

```
sage: PolynomialRing(ZZ, 3, var_array=['x','y'])
Multivariate Polynomial Ring in x0, y0, x1, y1, x2, y2 over Integer Ring
sage: PolynomialRing(ZZ, 3, var_array='a,b')
Multivariate Polynomial Ring in a0, b0, a1, b1, a2, b2 over Integer Ring
sage: PolynomialRing(ZZ, 2, 3, var_array='m')
Multivariate Polynomial Ring in m00, m01, m02, m10, m11, m12 over Integer Ring
sage: PolynomialRing(ZZ, 2, var_array='m')
Multivariate Polynomial Ring in m00, m01, m10, m11, over Integer Ring
```

Assignee: **@tscrim**

CC:  billey

Keywords: **polynomials, variables**

Reviewer: **Sara Billey**

Author: **Travis Scrimshaw**

Merged: **sage-5.8.beta3**

_Issue created by migration from https://trac.sagemath.org/ticket/14200_





---

archive/issue_events_183420.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-27T21:45:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14200",
    "milestone_number": null,
    "milestone_title": "sage-5.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14200#event-183420"
}
```



---

archive/issue_events_183421.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-27T21:45:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14200",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14200#event-183421"
}
```



---

archive/issue_events_183422.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-27T21:45:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14200",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14200#event-183422"
}
```



---

archive/issue_events_183423.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-27T21:45:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14200",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14200#event-183423"
}
```



---

archive/issue_comments_171855.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[trac_14200-poly_var_array-ts.patch.gz](https://github.com/sagemath/sage/files/ticket14200/trac_14200-poly_var_array-ts.patch.gz)**",
    "created_at": "2013-02-27T21:48:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14200",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14200#issuecomment-171855",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[trac_14200-poly_var_array-ts.patch.gz](https://github.com/sagemath/sage/files/ticket14200/trac_14200-poly_var_array-ts.patch.gz)**



---

archive/issue_events_183424.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-27T21:48:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14200",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14200#event-183424"
}
```



---

archive/issue_comments_171856.json:
```json
{
    "body": "Reviewer: **Sara Billey**",
    "created_at": "2013-03-01T02:58:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14200",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14200#issuecomment-171856",
    "user": "https://github.com/sagetrac-billey"
}
```

Reviewer: **Sara Billey**



---

archive/issue_events_183425.json:
```json
{
    "actor": "https://github.com/sagetrac-billey",
    "created_at": "2013-03-01T02:58:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14200",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14200#event-183425"
}
```



---

archive/issue_events_183426.json:
```json
{
    "actor": "https://github.com/sagetrac-billey",
    "created_at": "2013-03-01T02:58:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14200",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14200#event-183426"
}
```



---

archive/issue_events_183427.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-04T07:39:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14200",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14200#event-183427"
}
```



---

archive/issue_events_183428.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-04T07:39:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14200",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14200#event-183428"
}
```



---

archive/issue_comments_171857.json:
```json
{
    "body": "Merged: **sage-5.8.beta3**",
    "created_at": "2013-03-04T07:39:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14200",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14200#issuecomment-171857",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.8.beta3**
