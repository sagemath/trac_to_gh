# Issue 14193: Converting polynomial quotient ring elements to pari yields plain polynomials

archive/issues_013989.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: R.<x> = QQ[]\nsage: I = R.ideal(x^10)\nsage: S.<xb> = R.quo(I)\nsage: pari(xb)^10\nxb^10\n```\n\nAssignee: **@williamstein**\n\nReviewer: **Simon Spicer**\n\nAuthor: **David Roe**\n\nMerged: **sage-5.8.beta3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14193_\n\n",
    "closed_at": "2013-03-04T07:38:54Z",
    "created_at": "2013-02-27T03:20:18Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Converting polynomial quotient ring elements to pari yields plain polynomials",
    "type": "issue",
    "updated_at": "2013-03-04T07:38:54Z",
    "url": "https://github.com/sagemath/sage/issues/14193",
    "user": "https://github.com/roed314"
}
```

```
sage: R.<x> = QQ[]
sage: I = R.ideal(x^10)
sage: S.<xb> = R.quo(I)
sage: pari(xb)^10
xb^10
```

Assignee: **@williamstein**

Reviewer: **Simon Spicer**

Author: **David Roe**

Merged: **sage-5.8.beta3**

_Issue created by migration from https://trac.sagemath.org/ticket/14193_





---

archive/issue_events_183334.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2013-02-27T03:20:18Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14193",
    "milestone_number": null,
    "milestone_title": "sage-5.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14193#event-183334"
}
```



---

archive/issue_events_183335.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2013-02-27T03:20:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14193",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "component: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14193#event-183335"
}
```



---

archive/issue_events_183336.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2013-02-27T03:20:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14193",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14193#event-183336"
}
```



---

archive/issue_events_183337.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2013-02-27T03:20:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14193",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14193#event-183337"
}
```



---

archive/issue_comments_171785.json:
```json
{
    "body": "Attachment: **[14193.patch.gz](https://github.com/sagemath/sage/files/ticket14193/14193.patch.gz)**",
    "created_at": "2013-02-27T03:21:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14193",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14193#issuecomment-171785",
    "user": "https://github.com/roed314"
}
```

Attachment: **[14193.patch.gz](https://github.com/sagemath/sage/files/ticket14193/14193.patch.gz)**



---

archive/issue_events_183338.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2013-02-27T03:21:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14193",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14193#event-183338"
}
```



---

archive/issue_comments_171786.json:
```json
{
    "body": "Reviewer: **Simon Spicer**",
    "created_at": "2013-03-01T00:25:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14193",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14193#issuecomment-171786",
    "user": "https://github.com/haikona"
}
```

Reviewer: **Simon Spicer**



---

archive/issue_comments_171787.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nYep.",
    "created_at": "2013-03-01T00:25:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14193",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14193#issuecomment-171787",
    "user": "https://github.com/haikona"
}
```

<div id="comment:2" align="right">Comment 2</div>

Yep.



---

archive/issue_events_183339.json:
```json
{
    "actor": "https://github.com/haikona",
    "created_at": "2013-03-01T00:25:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14193",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14193#event-183339"
}
```



---

archive/issue_events_183340.json:
```json
{
    "actor": "https://github.com/haikona",
    "created_at": "2013-03-01T00:25:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14193",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14193#event-183340"
}
```



---

archive/issue_events_183341.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-04T07:38:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14193",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14193#event-183341"
}
```



---

archive/issue_events_183342.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-04T07:38:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14193",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14193#event-183342"
}
```



---

archive/issue_comments_171788.json:
```json
{
    "body": "Merged: **sage-5.8.beta3**",
    "created_at": "2013-03-04T07:38:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14193",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14193#issuecomment-171788",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.8.beta3**
