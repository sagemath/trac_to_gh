# Issue 14439: Fix small bug in blocks_and_cut_vertices

archive/issues_014235.json:
```json
{
    "assignees": [],
    "body": "When the graph has a single vertex u, the method should return `[This is the Trac macro *u* that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#u-macro), []`, but it currently returns `[u], []`. This is fixed in this patch.\n\nAssignee: **@jasongrout, @nathanncohen, @rlmill**\n\nCC:  @nathanncohen\n\nReviewer: **Nathann Cohen**\n\nAuthor: **David Coudert**\n\nMerged: **sage-5.10.beta0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14439_\n\n",
    "closed_at": "2013-04-23T09:43:37Z",
    "created_at": "2013-04-11T09:19:44Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.10",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix small bug in blocks_and_cut_vertices",
    "type": "issue",
    "updated_at": "2013-04-23T09:43:37Z",
    "url": "https://github.com/sagemath/sage/issues/14439",
    "user": "https://github.com/dcoudert"
}
```
When the graph has a single vertex u, the method should return `[This is the Trac macro *u* that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#u-macro), []`, but it currently returns `[u], []`. This is fixed in this patch.

Assignee: **@jasongrout, @nathanncohen, @rlmill**

CC:  @nathanncohen

Reviewer: **Nathann Cohen**

Author: **David Coudert**

Merged: **sage-5.10.beta0**

_Issue created by migration from https://trac.sagemath.org/ticket/14439_





---

archive/issue_events_187362.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2013-04-11T09:19:44Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14439",
    "milestone_number": null,
    "milestone_title": "sage-5.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14439#event-187362"
}
```



---

archive/issue_events_187363.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2013-04-11T09:19:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14439",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14439#event-187363"
}
```



---

archive/issue_events_187364.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2013-04-11T09:19:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14439",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14439#event-187364"
}
```



---

archive/issue_events_187365.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2013-04-11T09:19:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14439",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14439#event-187365"
}
```



---

archive/issue_events_187366.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2013-04-11T09:43:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14439",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14439#event-187366"
}
```



---

archive/issue_comments_176396.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-When the graph has a single vertex u, the method should return [This is the Trac macro *u* that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#u-macro), [], but it currently returns [u], []. This is fixed in this patch.\n+When the graph has a single vertex u, the method should return `[This is the Trac macro *u* that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#u-macro), []`, but it currently returns `[u], []`. This is fixed in this patch.\n``````\n",
    "created_at": "2013-04-11T09:43:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14439#issuecomment-176396",
    "user": "https://github.com/dcoudert"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-When the graph has a single vertex u, the method should return [This is the Trac macro *u* that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#u-macro), [], but it currently returns [u], []. This is fixed in this patch.
+When the graph has a single vertex u, the method should return `[This is the Trac macro *u* that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#u-macro), []`, but it currently returns `[u], []`. This is fixed in this patch.
``````




---

archive/issue_events_187367.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2013-04-11T09:43:34Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14439",
    "milestone_number": null,
    "milestone_title": "sage-5.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14439#event-187367"
}
```



---

archive/issue_events_187368.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2013-04-11T09:43:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14439",
    "milestone_number": null,
    "milestone_title": "sage-5.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14439#event-187368"
}
```



---

archive/issue_comments_176397.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nAttachment: **[trac_14439.patch.gz](https://github.com/sagemath/sage/files/ticket14439/trac_14439.patch.gz)**\n\nI have set the milestone to 5.9 in case we have time to include it.",
    "created_at": "2013-04-11T09:43:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14439#issuecomment-176397",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:1" align="right">Comment 1</div>

Attachment: **[trac_14439.patch.gz](https://github.com/sagemath/sage/files/ticket14439/trac_14439.patch.gz)**

I have set the milestone to 5.9 in case we have time to include it.



---

archive/issue_comments_176398.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nLooks like you are just updating the doctests...\n\nNathann",
    "created_at": "2013-04-11T22:13:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14439#issuecomment-176398",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:2" align="right">Comment 2</div>

Looks like you are just updating the doctests...

Nathann



---

archive/issue_comments_176399.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNo, I'm also changing the result when the graph has only one vertex.\nThis is to be consistent.",
    "created_at": "2013-04-11T22:19:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14439#issuecomment-176399",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:3" align="right">Comment 3</div>

No, I'm also changing the result when the graph has only one vertex.
This is to be consistent.



---

archive/issue_comments_176400.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nRight.\n\nNathann",
    "created_at": "2013-04-11T22:22:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14439#issuecomment-176400",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:4" align="right">Comment 4</div>

Right.

Nathann



---

archive/issue_events_187369.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-04-11T22:22:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14439",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14439#event-187369"
}
```



---

archive/issue_events_187370.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-04-11T22:22:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14439",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14439#event-187370"
}
```



---

archive/issue_events_187371.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-04-12T14:37:19Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14439",
    "milestone_number": null,
    "milestone_title": "sage-5.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14439#event-187371"
}
```



---

archive/issue_events_187372.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-04-12T14:37:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14439",
    "milestone_number": null,
    "milestone_title": "sage-5.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14439#event-187372"
}
```



---

archive/issue_comments_176401.json:
```json
{
    "body": "Reviewer: **Nathann Cohen**",
    "created_at": "2013-04-12T14:37:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14439#issuecomment-176401",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Nathann Cohen**



---

archive/issue_comments_176402.json:
```json
{
    "body": "Merged: **sage-5.10.beta0**",
    "created_at": "2013-04-23T09:43:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14439#issuecomment-176402",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.10.beta0**



---

archive/issue_events_187373.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-04-23T09:43:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14439",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14439#event-187373"
}
```



---

archive/issue_events_187374.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-04-23T09:43:37Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14439",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14439#event-187374"
}
```
