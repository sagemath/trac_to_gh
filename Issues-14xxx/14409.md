# Issue 14409: sage_input support for some geometry objects

archive/issues_014205.json:
```json
{
    "assignees": [],
    "body": "This is sometimes handy:\n\n```\n  sage: fan = Fan([Cone([(1,0), (1,1)]), Cone([(-1,-1)])])\n  sage: sage_input(fan)\n  Fan(cones=[[0, 1], [2]], rays=[(1, 0), (1, 1), (-1, -1)])\n```\n\nAssignee: **mhampton**\n\nReviewer: **Andrey Novoseltsev**\n\nAuthor: **Volker Braun**\n\nMerged: **sage-5.9.beta5**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14409_\n\n",
    "closed_at": "2013-04-11T07:34:49Z",
    "created_at": "2013-04-03T21:30:33Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage_input support for some geometry objects",
    "type": "issue",
    "updated_at": "2013-04-11T07:34:49Z",
    "url": "https://github.com/sagemath/sage/issues/14409",
    "user": "https://github.com/vbraun"
}
```
This is sometimes handy:

```
  sage: fan = Fan([Cone([(1,0), (1,1)]), Cone([(-1,-1)])])
  sage: sage_input(fan)
  Fan(cones=[[0, 1], [2]], rays=[(1, 0), (1, 1), (-1, -1)])
```

Assignee: **mhampton**

Reviewer: **Andrey Novoseltsev**

Author: **Volker Braun**

Merged: **sage-5.9.beta5**

_Issue created by migration from https://trac.sagemath.org/ticket/14409_





---

archive/issue_events_186871.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-04-03T21:30:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14409",
    "milestone_number": null,
    "milestone_title": "sage-5.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14409#event-186871"
}
```



---

archive/issue_events_186872.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-04-03T21:30:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14409",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "component: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14409#event-186872"
}
```



---

archive/issue_events_186873.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-04-03T21:30:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14409",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14409#event-186873"
}
```



---

archive/issue_events_186874.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-04-03T21:30:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14409",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14409#event-186874"
}
```



---

archive/issue_comments_175927.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,7 @@\n+This is sometimes handy:\n \n+```\n+  sage: fan = Fan([Cone([(1,0), (1,1)]), Cone([(-1,-1)])])\n+  sage: sage_input(fan)\n+  Fan(cones=[[0, 1], [2]], rays=[(1, 0), (1, 1), (-1, -1)])\n+```\n``````\n",
    "created_at": "2013-04-03T21:38:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14409",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14409#issuecomment-175927",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,7 @@
+This is sometimes handy:
 
+```
+  sage: fan = Fan([Cone([(1,0), (1,1)]), Cone([(-1,-1)])])
+  sage: sage_input(fan)
+  Fan(cones=[[0, 1], [2]], rays=[(1, 0), (1, 1), (-1, -1)])
+```
``````




---

archive/issue_comments_175928.json:
```json
{
    "body": "Attachment: **[trac_14409_geometry_sage_input.patch.gz](https://github.com/sagemath/sage/files/ticket14409/trac_14409_geometry_sage_input.patch.gz)**\n\nInitial patch",
    "created_at": "2013-04-03T21:41:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14409",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14409#issuecomment-175928",
    "user": "https://github.com/vbraun"
}
```

Attachment: **[trac_14409_geometry_sage_input.patch.gz](https://github.com/sagemath/sage/files/ticket14409/trac_14409_geometry_sage_input.patch.gz)**

Initial patch



---

archive/issue_events_186875.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-04-03T21:42:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14409",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14409#event-186875"
}
```



---

archive/issue_comments_175929.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nCool, I didn't know we have such things!",
    "created_at": "2013-04-04T01:22:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14409",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14409#issuecomment-175929",
    "user": "https://github.com/novoselt"
}
```

<a id='comment:3'>Comment 3:</a>
Cool, I didn't know we have such things!



---

archive/issue_events_186876.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2013-04-08T03:44:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14409",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14409#event-186876"
}
```



---

archive/issue_events_186877.json:
```json
{
    "actor": "https://github.com/novoselt",
    "created_at": "2013-04-08T03:44:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14409",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14409#event-186877"
}
```



---

archive/issue_comments_175930.json:
```json
{
    "body": "Reviewer: **Andrey Novoseltsev**",
    "created_at": "2013-04-08T03:44:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14409",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14409#issuecomment-175930",
    "user": "https://github.com/novoselt"
}
```

Reviewer: **Andrey Novoseltsev**



---

archive/issue_comments_175931.json:
```json
{
    "body": "Merged: **sage-5.9.beta5**",
    "created_at": "2013-04-11T07:34:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14409",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14409#issuecomment-175931",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.9.beta5**



---

archive/issue_events_186878.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-04-11T07:34:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14409",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14409#event-186878"
}
```



---

archive/issue_events_186879.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-04-11T07:34:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14409",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14409#event-186879"
}
```
