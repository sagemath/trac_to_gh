# Issue 14205: polybori doctests involve randomness

archive/issues_014001.json:
```json
{
    "body": "As discusses in #13767, there is some doctests for the `random_set` function in polybori that have inherent random results. Because of that, they might break if the implementation of random functions changes, such as what happened in that ticket.\n\nThis is a rare occasion, but still it might be worthwhile to change these kind of doctests to only check for documented behavior. \n\nFor example, the doctests could check that `result = random_set(monomial, n)` should obey `len(result) == n` and `result.diff(monomial.divisors()).empty()`. (suggested by Alexander Dreyer)\n\nOr we could test against theorems (such as: One has random data, but the test is using an identity that must hold for any data). (suggested by Simon King) \n\nAssignee: @aghitza\n\nCC:  alexanderdreyer\n\nUpstream: None of the above - read trac for reasoning.\n\nStatus: needs_info\n\nIssue created by migration from https://trac.sagemath.org/ticket/14205\n\n",
    "created_at": "2013-02-28T16:43:39Z",
    "labels": [
        "component: algebra",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-wishlist",
    "title": "polybori doctests involve randomness",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/14205",
    "user": "https://github.com/tkluck"
}
```
As discusses in #13767, there is some doctests for the `random_set` function in polybori that have inherent random results. Because of that, they might break if the implementation of random functions changes, such as what happened in that ticket.

This is a rare occasion, but still it might be worthwhile to change these kind of doctests to only check for documented behavior. 

For example, the doctests could check that `result = random_set(monomial, n)` should obey `len(result) == n` and `result.diff(monomial.divisors()).empty()`. (suggested by Alexander Dreyer)

Or we could test against theorems (such as: One has random data, but the test is using an identity that must hold for any data). (suggested by Simon King) 

Assignee: @aghitza

CC:  alexanderdreyer

Upstream: None of the above - read trac for reasoning.

Status: needs_info

Issue created by migration from https://trac.sagemath.org/ticket/14205





---

archive/issue_comments_218214.json:
```json
{
    "body": "<a id='comment:1'></a>I'd rather have the output of boost.Random doctested so we know when the implementation changes. We also test all other RNGs to verify that the sequence that they produce is actually the same, that is, we set the seed correctly.",
    "created_at": "2013-02-28T18:23:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14205",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14205#issuecomment-218214",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:1'></a>I'd rather have the output of boost.Random doctested so we know when the implementation changes. We also test all other RNGs to verify that the sequence that they produce is actually the same, that is, we set the seed correctly.



---

archive/issue_comments_218215.json:
```json
{
    "body": "<a id='comment:2'></a>Replying to [comment:1 vbraun]:\n> I'd rather have the output of boost.Random doctested so we know when the implementation changes. We also test all other RNGs to verify that the sequence that they produce is actually the same, that is, we set the seed correctly.\n\nWell, I understood from the discussion here, that the test should not depend on the random generator:\nhttps://groups.google.com/forum/?fromgroups=#!topic/sage-devel/uSEWZrE-T_8\n\nAlso, sage-on-gentoo and sage-on-debian may use different versions of boost than the original Sage.\n\nAn alternative could be that we **additionally** add (and document) the generic tests and keep the explicit ones. This would simplify the bug hunt next time.",
    "created_at": "2013-03-01T09:21:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14205",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14205#issuecomment-218215",
    "user": "https://github.com/alexanderdreyer"
}
```

<a id='comment:2'></a>Replying to [comment:1 vbraun]:
> I'd rather have the output of boost.Random doctested so we know when the implementation changes. We also test all other RNGs to verify that the sequence that they produce is actually the same, that is, we set the seed correctly.

Well, I understood from the discussion here, that the test should not depend on the random generator:
https://groups.google.com/forum/?fromgroups=#!topic/sage-devel/uSEWZrE-T_8

Also, sage-on-gentoo and sage-on-debian may use different versions of boost than the original Sage.

An alternative could be that we **additionally** add (and document) the generic tests and keep the explicit ones. This would simplify the bug hunt next time.



---

archive/issue_comments_218216.json:
```json
{
    "body": "Changing upstream from \"N/A\" to \"None of the above - read trac for reasoning.\"",
    "created_at": "2013-03-01T09:22:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14205",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14205#issuecomment-218216",
    "user": "https://github.com/alexanderdreyer"
}
```

Changing upstream from "N/A" to "None of the above - read trac for reasoning."



---

archive/issue_comments_218217.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2013-03-01T10:30:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14205",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14205#issuecomment-218217",
    "user": "https://github.com/vbraun"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_218218.json:
```json
{
    "body": "<a id='comment:4'></a>Doctesting the random sequence (see `sage.misc.randstate`) and doctesting the correctness of the output are two different things. And ideally we'd have both, though its better to have randstate-dependent tests than none at all.",
    "created_at": "2013-03-01T10:30:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14205",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14205#issuecomment-218218",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:4'></a>Doctesting the random sequence (see `sage.misc.randstate`) and doctesting the correctness of the output are two different things. And ideally we'd have both, though its better to have randstate-dependent tests than none at all.



---

archive/issue_events_040090.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-03-01T10:30:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14205",
    "milestone": "sage-wishlist",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14205#event-40090"
}
```



---

archive/issue_comments_218219.json:
```json
{
    "body": "Changing status from needs_review to needs_info.",
    "created_at": "2013-03-01T10:36:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14205",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14205#issuecomment-218219",
    "user": "https://github.com/alexanderdreyer"
}
```

Changing status from needs_review to needs_info.



---

archive/issue_comments_218220.json:
```json
{
    "body": "<a id='comment:6'></a>So you would just keep it, like it was in #13767?",
    "created_at": "2013-03-01T10:36:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14205",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14205#issuecomment-218220",
    "user": "https://github.com/alexanderdreyer"
}
```

<a id='comment:6'></a>So you would just keep it, like it was in #13767?



---

archive/issue_comments_218221.json:
```json
{
    "body": "<a id='comment:7'></a>Correct me if I'm wrong, but the failing doctests were pretty much directly tests of the random number generator. If thats the case then I'd keep them since they test the rng seeding.",
    "created_at": "2013-03-01T17:36:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14205",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14205#issuecomment-218221",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:7'></a>Correct me if I'm wrong, but the failing doctests were pretty much directly tests of the random number generator. If thats the case then I'd keep them since they test the rng seeding.
