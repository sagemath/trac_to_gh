# Issue 14569: Upgrade to Cython 0.19.1

archive/issues_014365.json:
```json
{
    "assignees": [],
    "body": "Cython 0.19.1 includes fix for #14544 and workaround for restrict bug uncovered at #14452.\n\n\nApply:\n* new SPKG: http://boxen.math.washington.edu/home/ohanar/spkgs/cython-0.19.1.spkg ([spkg diff](https://github.com/sagemath/sage/files/ticket14569/cython-0.19.1.diff.gz))\n* [attachment: trac14569.patch.gz](https://github.com/sagemath/sage/files/ticket14569/trac14569.patch.gz) to sage library\n\n\nAssignee: **@jdemeyer**\n\nCC:  @nexttime @robertwb @jdemeyer\n\nReviewer: **Jeroen Demeyer**\n\nAuthor: **R. Andrew Ohana**\n\nMerged: **sage-5.10.beta3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14569_\n\n",
    "closed_at": "2013-05-15T08:25:21Z",
    "created_at": "2013-05-11T23:12:35Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.10",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Upgrade to Cython 0.19.1",
    "type": "issue",
    "updated_at": "2013-05-15T08:25:21Z",
    "url": "https://github.com/sagemath/sage/issues/14569",
    "user": "https://github.com/ohanar"
}
```
Cython 0.19.1 includes fix for #14544 and workaround for restrict bug uncovered at #14452.


Apply:
* new SPKG: http://boxen.math.washington.edu/home/ohanar/spkgs/cython-0.19.1.spkg ([spkg diff](https://github.com/sagemath/sage/files/ticket14569/cython-0.19.1.diff.gz))
* [attachment: trac14569.patch.gz](https://github.com/sagemath/sage/files/ticket14569/trac14569.patch.gz) to sage library


Assignee: **@jdemeyer**

CC:  @nexttime @robertwb @jdemeyer

Reviewer: **Jeroen Demeyer**

Author: **R. Andrew Ohana**

Merged: **sage-5.10.beta3**

_Issue created by migration from https://trac.sagemath.org/ticket/14569_





---

archive/issue_events_189301.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2013-05-11T23:12:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14569",
    "milestone_number": null,
    "milestone_title": "sage-5.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14569#event-189301"
}
```



---

archive/issue_events_189302.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2013-05-11T23:12:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14569",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14569#event-189302"
}
```



---

archive/issue_events_189303.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2013-05-11T23:12:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14569",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14569#event-189303"
}
```



---

archive/issue_events_189304.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2013-05-11T23:12:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14569",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14569#event-189304"
}
```



---

archive/issue_comments_178992.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n Cython 0.19.1 includes fix for #14544 and workaround for restrict bug uncovered at #14452.\n+\n+New SPKG: http://boxen.math.washington.edu/home/ohanar/cython-0.19.1.spkg\n``````\n",
    "created_at": "2013-05-11T23:15:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14569",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14569#issuecomment-178992",
    "user": "https://github.com/ohanar"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
 Cython 0.19.1 includes fix for #14544 and workaround for restrict bug uncovered at #14452.
+
+New SPKG: http://boxen.math.washington.edu/home/ohanar/cython-0.19.1.spkg
``````




---

archive/issue_comments_178993.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n Cython 0.19.1 includes fix for #14544 and workaround for restrict bug uncovered at #14452.\n \n-New SPKG: http://boxen.math.washington.edu/home/ohanar/cython-0.19.1.spkg\n+New SPKG: http://boxen.math.washington.edu/home/ohanar/spkgs/cython-0.19.1.spkg\n``````\n",
    "created_at": "2013-05-11T23:16:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14569",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14569#issuecomment-178993",
    "user": "https://github.com/ohanar"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 Cython 0.19.1 includes fix for #14544 and workaround for restrict bug uncovered at #14452.
 
-New SPKG: http://boxen.math.washington.edu/home/ohanar/cython-0.19.1.spkg
+New SPKG: http://boxen.math.washington.edu/home/ohanar/spkgs/cython-0.19.1.spkg
``````




---

archive/issue_comments_178994.json:
```json
{
    "body": "spkg diff",
    "created_at": "2013-05-11T23:17:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14569",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14569#issuecomment-178994",
    "user": "https://github.com/ohanar"
}
```

spkg diff



---

archive/issue_comments_178995.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nAttachment: **[cython-0.19.1.diff.gz](https://github.com/sagemath/sage/files/ticket14569/cython-0.19.1.diff.gz)**",
    "created_at": "2013-05-11T23:33:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14569",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14569#issuecomment-178995",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:3'>Comment 3:</a>
Attachment: **[cython-0.19.1.diff.gz](https://github.com/sagemath/sage/files/ticket14569/cython-0.19.1.diff.gz)**



---

archive/issue_comments_178996.json:
```json
{
    "body": "apply to sage library",
    "created_at": "2013-05-11T23:42:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14569",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14569#issuecomment-178996",
    "user": "https://github.com/ohanar"
}
```

apply to sage library



---

archive/issue_comments_178997.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nAttachment: **[trac14569.patch.gz](https://github.com/sagemath/sage/files/ticket14569/trac14569.patch.gz)**",
    "created_at": "2013-05-11T23:44:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14569",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14569#issuecomment-178997",
    "user": "https://github.com/ohanar"
}
```

<a id='comment:4'>Comment 4:</a>
Attachment: **[trac14569.patch.gz](https://github.com/sagemath/sage/files/ticket14569/trac14569.patch.gz)**



---

archive/issue_comments_178998.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,7 @@\n Cython 0.19.1 includes fix for #14544 and workaround for restrict bug uncovered at #14452.\n \n-New SPKG: http://boxen.math.washington.edu/home/ohanar/spkgs/cython-0.19.1.spkg\n+\n+Apply:\n+* new SPKG: http://boxen.math.washington.edu/home/ohanar/spkgs/cython-0.19.1.spkg ([spkg diff](https://github.com/sagemath/sage/files/ticket14569/cython-0.19.1.diff.gz))\n+* [attachment: trac14569.patch.gz](https://github.com/sagemath/sage/files/ticket14569/trac14569.patch.gz) to sage library\n+\n``````\n",
    "created_at": "2013-05-11T23:44:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14569",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14569#issuecomment-178998",
    "user": "https://github.com/ohanar"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,7 @@
 Cython 0.19.1 includes fix for #14544 and workaround for restrict bug uncovered at #14452.
 
-New SPKG: http://boxen.math.washington.edu/home/ohanar/spkgs/cython-0.19.1.spkg
+
+Apply:
+* new SPKG: http://boxen.math.washington.edu/home/ohanar/spkgs/cython-0.19.1.spkg ([spkg diff](https://github.com/sagemath/sage/files/ticket14569/cython-0.19.1.diff.gz))
+* [attachment: trac14569.patch.gz](https://github.com/sagemath/sage/files/ticket14569/trac14569.patch.gz) to sage library
+
``````




---

archive/issue_events_189305.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2013-05-12T01:49:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14569",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14569#event-189305"
}
```



---

archive/issue_comments_178999.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nLooks good to me and passes all buildbot tests!",
    "created_at": "2013-05-15T08:25:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14569",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14569#issuecomment-178999",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:6'>Comment 6:</a>
Looks good to me and passes all buildbot tests!



---

archive/issue_events_189306.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-15T08:25:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14569",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14569#event-189306"
}
```



---

archive/issue_events_189307.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-15T08:25:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14569",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14569#event-189307"
}
```



---

archive/issue_comments_179000.json:
```json
{
    "body": "Merged: **sage-5.10.beta3**",
    "created_at": "2013-05-15T08:25:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14569",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14569#issuecomment-179000",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.10.beta3**



---

archive/issue_comments_179001.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2013-05-15T08:25:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14569",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14569#issuecomment-179001",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**
