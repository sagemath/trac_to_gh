...
```json
{
    "body": "Here's an attempt to fix the problem with a small change to the coercion discovery algorithm. It does break a few things, but I'd argue it only does so by exposing weaknesses in existing code. The other patches in the series fix (or, in one case, work around) these weaknesses.\n\nSee the commit messages for details.\n\nWhat do the experts say?",
    "created_at": "2013-08-27T09:16:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14982",
    "type": "issue_comment",
...
```json
{
    "body": "Is this branch compatible with #15303?",
...
```json
{
    "body": "(git-)Rebased on top of 6.1.rc0, removed dependency on #15303.\n\n---\nNew commits:",
...
```json
{
    "body": "Rebased. (Tests are still running.)\n  \n---\nNew commits:",
...
```json
{
    "body": "(I haven't had time to run all tests with the last commit yet, only those that failed before plus modules related to things that changed.)",
...
```json
{
    "body": "Vincent, do your comments count as a review? (If so, you may want to check that I didn't do anything wrong with my last rebase.) Or are there still things to be checked/addressed?",
