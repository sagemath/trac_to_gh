# Issue 14372: Document ccache in developer guide

archive/issues_014168.json:
```json
{
    "assignees": [],
    "body": "This is useful and should be more widely visible.  Some hints as to how to enable this when it's already present in a distro would be helpful.\n\nCC:  @roed314 @kini @jdemeyer @ppurka @ohanar @videlec\n\nReviewer: **Matthias Koeppe**\n\nComponent: **documentation**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14372_\n\n",
    "closed_at": "2017-01-21T18:03:11Z",
    "created_at": "2013-03-28T00:52:31Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20documentation",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Document ccache in developer guide",
    "type": "issue",
    "updated_at": "2017-01-21T18:03:11Z",
    "url": "https://github.com/sagemath/sage/issues/14372",
    "user": "https://github.com/robertwb"
}
```
This is useful and should be more widely visible.  Some hints as to how to enable this when it's already present in a distro would be helpful.

CC:  @roed314 @kini @jdemeyer @ppurka @ohanar @videlec

Reviewer: **Matthias Koeppe**

Component: **documentation**

_Issue created by migration from https://trac.sagemath.org/ticket/14372_





---

archive/issue_events_203915.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2013-03-28T00:52:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203915"
}
```



---

archive/issue_events_203916.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2013-03-28T00:52:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203916"
}
```



---

archive/issue_events_203917.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2013-03-28T00:52:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203917"
}
```



---

archive/issue_events_203918.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2013-03-28T00:52:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203918"
}
```



---

archive/issue_comments_172780.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nWhy is this especially handy when using git?\n\nI hope that ccache will not be enabled by default, because you don't want to force 4GB of additional data in the user's home directory. And if it's not enabled by default, why should the package be standard?\n\nNote that I'm not against it being standard, it could be like GCC: shipped but not built by default.",
    "created_at": "2013-03-28T01:14:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172780",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:1" align="right">comment:1</div>

Why is this especially handy when using git?

I hope that ccache will not be enabled by default, because you don't want to force 4GB of additional data in the user's home directory. And if it's not enabled by default, why should the package be standard?

Note that I'm not against it being standard, it could be like GCC: shipped but not built by default.



---

archive/issue_comments_172781.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nOne option I just discussed with Robert is to by default only turn on ccache for files in the Sage library.  This could drop the space requirement substantially.",
    "created_at": "2013-03-28T02:19:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172781",
    "user": "https://github.com/roed314"
}
```

<div id="comment:2" align="right">comment:2</div>

One option I just discussed with Robert is to by default only turn on ccache for files in the Sage library.  This could drop the space requirement substantially.



---

archive/issue_comments_172782.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nBeing optional for a while doesn't imply it should get standard;  there are many packages that are optional since quite a while, and you certainly won't want to include all of them.\n\nIMHO we already have too many \"standard\" packages, some are only built on a few platforms, or under certain circumstances, some only useful to (or necessary for) a few people.",
    "created_at": "2013-03-28T02:27:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172782",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:3" align="right">comment:3</div>

Being optional for a while doesn't imply it should get standard;  there are many packages that are optional since quite a while, and you certainly won't want to include all of them.

IMHO we already have too many "standard" packages, some are only built on a few platforms, or under certain circumstances, some only useful to (or necessary for) a few people.



---

archive/issue_comments_172783.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nReplying to [@nexttime](#comment:3):\n> Being optional for a while doesn't imply it should get standard;  there are many packages that are optional since quite a while, and you certainly won't want to include all of them.\n> \n> IMHO we already have too many \"standard\" packages, some are only built on a few platforms, or under certain circumstances, some only useful to (or necessary for) a few people.\n\nI don't think Robert was claiming that ccache should become standard *because* it has been optional for a while, but rather that it has been optional for long enough to satisfy that requirement for becoming a standard package.\n\nWe do have a bunch of standard packages, but I agree with Robert that ccache would be useful for almost every Sage developer (which is more than can be said of some of our current standard spkgs).",
    "created_at": "2013-03-28T02:35:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172783",
    "user": "https://github.com/roed314"
}
```

<div id="comment:4" align="right">comment:4</div>

Replying to [@nexttime](#comment:3):
> Being optional for a while doesn't imply it should get standard;  there are many packages that are optional since quite a while, and you certainly won't want to include all of them.
> 
> IMHO we already have too many "standard" packages, some are only built on a few platforms, or under certain circumstances, some only useful to (or necessary for) a few people.

I don't think Robert was claiming that ccache should become standard *because* it has been optional for a while, but rather that it has been optional for long enough to satisfy that requirement for becoming a standard package.

We do have a bunch of standard packages, but I agree with Robert that ccache would be useful for almost every Sage developer (which is more than can be said of some of our current standard spkgs).



---

archive/issue_comments_172784.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nWell, it perfectly fits an *optional* package -- it doesn't require any specific support by the Sage library, just like gdb, autotools, valgrind, also GCC, or whatever, which are mostly there for convenience, but not needed by most Sage users.\n\nShipping (and reinstalling) it with each and every Sage release doesn't make sense to me;  <flame> any serious developer would have it installed [system-wide] anyway if it's of use to him/her </flame>.\n\nDon't make the horse an elephant, otherwise it'll really lose its ability to fly.",
    "created_at": "2013-03-28T02:50:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172784",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:5" align="right">comment:5</div>

Well, it perfectly fits an *optional* package -- it doesn't require any specific support by the Sage library, just like gdb, autotools, valgrind, also GCC, or whatever, which are mostly there for convenience, but not needed by most Sage users.

Shipping (and reinstalling) it with each and every Sage release doesn't make sense to me;  <flame> any serious developer would have it installed [system-wide] anyway if it's of use to him/her </flame>.

Don't make the horse an elephant, otherwise it'll really lose its ability to fly.



---

archive/issue_comments_172785.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nI essentially agree with leif here: it's a package which is essentially only useful for Sage *developers*, so why should we always use it?",
    "created_at": "2013-03-28T07:01:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172785",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:6" align="right">comment:6</div>

I essentially agree with leif here: it's a package which is essentially only useful for Sage *developers*, so why should we always use it?



---

archive/issue_comments_172786.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nIf we switch to Portage Prefix as our build system, maybe we can have `sci-mathematics/sage-9999[dev]` (or whatever) depend on ccache and other versions of `sci-mathematics/sage` not need to. Developers would build a make target that unmasked `sci-mathematics/sage-9999` before building, say, whereas for users it would remain masked and stuff like the dev scripts and ccache would not be installed.\n\nBut of course that's not really relevant to this ticket.",
    "created_at": "2013-03-28T07:04:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172786",
    "user": "https://github.com/kini"
}
```

<div id="comment:7" align="right">comment:7</div>

If we switch to Portage Prefix as our build system, maybe we can have `sci-mathematics/sage-9999[dev]` (or whatever) depend on ccache and other versions of `sci-mathematics/sage` not need to. Developers would build a make target that unmasked `sci-mathematics/sage-9999` before building, say, whereas for users it would remain masked and stuff like the dev scripts and ccache would not be installed.

But of course that's not really relevant to this ticket.



---

archive/issue_comments_172787.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\nCCing a couple people I thought might be interested.",
    "created_at": "2013-03-28T07:22:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172787",
    "user": "https://github.com/kini"
}
```

<div id="comment:8" align="right">comment:8</div>

CCing a couple people I thought might be interested.



---

archive/issue_comments_172788.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nI agree with the opinion that ccache shouldn't be a standard package. It is required only for developers, and maybe some developers will *not* want this to be default because of the 4G of storage it demands by default.\n\nFor those who are interested, it can be installed quite easily by setting the environment variable SAGE_INSTALL_CCACHE='yes'. This variable can be added to the `<SHELL>rc` since it doesn't clash with anything else, so that in every new sage installation or (re-)compilation ccache is used.",
    "created_at": "2013-03-28T12:47:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172788",
    "user": "https://github.com/ppurka"
}
```

<div id="comment:9" align="right">comment:9</div>

I agree with the opinion that ccache shouldn't be a standard package. It is required only for developers, and maybe some developers will *not* want this to be default because of the 4G of storage it demands by default.

For those who are interested, it can be installed quite easily by setting the environment variable SAGE_INSTALL_CCACHE='yes'. This variable can be added to the `<SHELL>rc` since it doesn't clash with anything else, so that in every new sage installation or (re-)compilation ccache is used.



---

archive/issue_comments_172789.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\n> I agree with the opinion that ccache shouldn't be a standard package. It is required only for developers, and maybe some developers will *not* want this to be default because of the 4G of storage it demands by default.\n\nIt seems like this indeed falls in the same category as the packages Leif mentioned.  We include everything needed to build but no more on the devel side.  Maybe if we do move to a more \"modular\" packaging system we could in fact *remove* standard spkgs...\n\nThat said, I don't know anything about ccache, and perhaps neither does the sage-combinat crew (?) so apparently not *all* developers need it.  But by the same token, maybe there is some compelling reason, and it sounds like it doesn't *have* to triple the size of a Sage installation.    What about Jeroen's idea to not build it by default - 185 KB is a fairly large addition, but it's not like we haven't bloated things before :)",
    "created_at": "2013-03-28T13:57:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172789",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:10" align="right">comment:10</div>

> I agree with the opinion that ccache shouldn't be a standard package. It is required only for developers, and maybe some developers will *not* want this to be default because of the 4G of storage it demands by default.

It seems like this indeed falls in the same category as the packages Leif mentioned.  We include everything needed to build but no more on the devel side.  Maybe if we do move to a more "modular" packaging system we could in fact *remove* standard spkgs...

That said, I don't know anything about ccache, and perhaps neither does the sage-combinat crew (?) so apparently not *all* developers need it.  But by the same token, maybe there is some compelling reason, and it sounds like it doesn't *have* to triple the size of a Sage installation.    What about Jeroen's idea to not build it by default - 185 KB is a fairly large addition, but it's not like we haven't bloated things before :)



---

archive/issue_comments_172790.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">comment:11</div>\n\n-1 from me, too. If you are a serious developer then you hopefully use a distro that ships a working gcc+ccache by default.",
    "created_at": "2013-03-28T15:10:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172790",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:11" align="right">comment:11</div>

-1 from me, too. If you are a serious developer then you hopefully use a distro that ships a working gcc+ccache by default.



---

archive/issue_comments_172791.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">comment:12</div>\n\nReplying to [@vbraun](#comment:11):\n> -1 from me, too. If you are a serious developer then you hopefully use a distro that ships a working gcc+ccache by default. \n\nNot every serious developer uses linux (and Sage is incompatible with macports, etc on OSX).\n\nThat said, I don't have a strong opinion on the matter.",
    "created_at": "2013-03-28T15:34:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172791",
    "user": "https://github.com/ohanar"
}
```

<div id="comment:12" align="right">comment:12</div>

Replying to [@vbraun](#comment:11):
> -1 from me, too. If you are a serious developer then you hopefully use a distro that ships a working gcc+ccache by default. 

Not every serious developer uses linux (and Sage is incompatible with macports, etc on OSX).

That said, I don't have a strong opinion on the matter.



---

archive/issue_comments_172792.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">comment:13</div>\n\nReplying to [@ohanar](#comment:12):\n> Replying to [@vbraun](#comment:11):\n> > -1 from me, too. If you are a serious developer then you hopefully use a distro that ships a working gcc+ccache by default. \n\n> Not every serious developer uses linux (and Sage is incompatible with macports, etc on OSX).\n\nIs there a section in the Sage Developer's Guide on how to setup a good Sage development environment on $OS / recommended packages, short introduction on how to setup / use $TOOL?",
    "created_at": "2013-03-28T16:18:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172792",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:13" align="right">comment:13</div>

Replying to [@ohanar](#comment:12):
> Replying to [@vbraun](#comment:11):
> > -1 from me, too. If you are a serious developer then you hopefully use a distro that ships a working gcc+ccache by default. 

> Not every serious developer uses linux (and Sage is incompatible with macports, etc on OSX).

Is there a section in the Sage Developer's Guide on how to setup a good Sage development environment on $OS / recommended packages, short introduction on how to setup / use $TOOL?



---

archive/issue_comments_172793.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">comment:14</div>\n\nReplying to [@ohanar](#comment:12):\n> Replying to [@vbraun](#comment:11):\n> > -1 from me, too. If you are a serious developer then you hopefully use a distro that ships a working gcc+ccache by default. \n\n> Not every serious developer uses linux (and Sage is incompatible with macports, etc on OSX).\n\nSo... what is this ccache thing?  If it's automatic and makes `sage -b` faster, should I just use this optional spkg?  I guess I agree that one potential resolution to this ticket would be to tell people to use it.",
    "created_at": "2013-03-28T18:04:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172793",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:14" align="right">comment:14</div>

Replying to [@ohanar](#comment:12):
> Replying to [@vbraun](#comment:11):
> > -1 from me, too. If you are a serious developer then you hopefully use a distro that ships a working gcc+ccache by default. 

> Not every serious developer uses linux (and Sage is incompatible with macports, etc on OSX).

So... what is this ccache thing?  If it's automatic and makes `sage -b` faster, should I just use this optional spkg?  I guess I agree that one potential resolution to this ticket would be to tell people to use it.



---

archive/issue_comments_172794.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">comment:15</div>\n\nReplying to [@kcrisman](#comment:14):\n> So... what is this ccache thing?\n\nA compiler cache. It essentially caches the output of `gcc`.\n\n> If it's automatic and makes `sage -b` faster\n\nThat's what it does indeed (in fact, it makes building all C/C++ code in Sage faster, not just `./sage -b`)\n\n> should I just use this optional spkg?\n\nIf you don't mind losing 4GB of disk space, yes you should.",
    "created_at": "2013-03-28T18:16:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172794",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:15" align="right">comment:15</div>

Replying to [@kcrisman](#comment:14):
> So... what is this ccache thing?

A compiler cache. It essentially caches the output of `gcc`.

> If it's automatic and makes `sage -b` faster

That's what it does indeed (in fact, it makes building all C/C++ code in Sage faster, not just `./sage -b`)

> should I just use this optional spkg?

If you don't mind losing 4GB of disk space, yes you should.



---

archive/issue_comments_172795.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">comment:16</div>\n\nIt's automatic and makes `sage -b` faster, but it requires a lot of disk space (a few gigabytes), and the effect of making `sage -b` faster is most visible if you used ccache during your initial build of sage from the source tarball, I think.",
    "created_at": "2013-03-28T18:21:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172795",
    "user": "https://github.com/kini"
}
```

<div id="comment:16" align="right">comment:16</div>

It's automatic and makes `sage -b` faster, but it requires a lot of disk space (a few gigabytes), and the effect of making `sage -b` faster is most visible if you used ccache during your initial build of sage from the source tarball, I think.



---

archive/issue_comments_172796.json:
```json
{
    "body": "<div id=\"comment:17\" align=\"right\">comment:17</div>\n\nYeah, I would say that one mention in the midst of [the env vars list](http://www.sagemath.org/doc/installation/source.html#environment-variables) in the *installation* (not developer) guide wouldn't be sufficient information to alert people to it.  You shouldn't expect a new (mathematically-oriented) developer to be aware of this, per se.",
    "created_at": "2013-03-28T18:29:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172796",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:17" align="right">comment:17</div>

Yeah, I would say that one mention in the midst of [the env vars list](http://www.sagemath.org/doc/installation/source.html#environment-variables) in the *installation* (not developer) guide wouldn't be sufficient information to alert people to it.  You shouldn't expect a new (mathematically-oriented) developer to be aware of this, per se.



---

archive/issue_comments_172797.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">comment:18</div>\n\nReplying to [@kini](#comment:16):\n> the effect of making sage-b faster is most visible if you used ccache during your initial build of sage from the source tarball, I think.\n\nSure, since it's a cache, it can only speed up compilation starting from the *second* time you build the same file.",
    "created_at": "2013-03-28T19:05:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172797",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:18" align="right">comment:18</div>

Replying to [@kini](#comment:16):
> the effect of making sage-b faster is most visible if you used ccache during your initial build of sage from the source tarball, I think.

Sure, since it's a cache, it can only speed up compilation starting from the *second* time you build the same file.



---

archive/issue_events_203919.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203919"
}
```



---

archive/issue_events_203920.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203920"
}
```



---

archive/issue_events_203921.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203921"
}
```



---

archive/issue_events_203922.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203922"
}
```



---

archive/issue_comments_172798.json:
```json
{
    "body": "<div id=\"comment:21\" align=\"right\">comment:21</div>\n\nNow that we have moved to git, what is the opinion on this?",
    "created_at": "2014-04-15T10:49:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172798",
    "user": "https://github.com/ppurka"
}
```

<div id="comment:21" align="right">comment:21</div>

Now that we have moved to git, what is the opinion on this?



---

archive/issue_comments_172799.json:
```json
{
    "body": "<div id=\"comment:22\" align=\"right\">comment:22</div>\n\nWe do have the `SAGE_INSTALL_CCACHE` variable now and I personally think that is a perfect solution.",
    "created_at": "2014-04-15T11:42:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172799",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:22" align="right">comment:22</div>

We do have the `SAGE_INSTALL_CCACHE` variable now and I personally think that is a perfect solution.



---

archive/issue_events_203923.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2014-04-15T13:05:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203923"
}
```



---

archive/issue_events_203924.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2014-04-15T13:05:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203924"
}
```



---

archive/issue_events_203925.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2014-04-15T13:05:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203925"
}
```



---

archive/issue_events_203926.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2014-04-15T13:05:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "c6c6c6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203926"
}
```



---

archive/issue_comments_172800.json:
```json
{
    "body": "Reviewer: **Punarbasu Purkayastha**",
    "created_at": "2014-04-15T13:05:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172800",
    "user": "https://github.com/ppurka"
}
```

Reviewer: **Punarbasu Purkayastha**



---

archive/issue_events_203927.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2014-04-15T13:05:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203927"
}
```



---

archive/issue_events_203928.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2014-04-15T13:05:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203928"
}
```



---

archive/issue_comments_172801.json:
```json
{
    "body": "<div id=\"comment:25\" align=\"right\">comment:25</div>\n\nSo we have to open another ticket to prominently advertise it (in the Sage *Developer's* Guide)? ;-)",
    "created_at": "2014-04-15T13:44:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172801",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:25" align="right">comment:25</div>

So we have to open another ticket to prominently advertise it (in the Sage *Developer's* Guide)? ;-)



---

archive/issue_comments_172802.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-It's been optional long enough and will be especially handy when using git.\n+This is useful and should be more widely visible.  Some hints as to how to enable this when it's already present in a distro would be helpful.\n``````\n",
    "created_at": "2014-04-15T19:10:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172802",
    "user": "https://github.com/kcrisman"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-It's been optional long enough and will be especially handy when using git.
+This is useful and should be more widely visible.  Some hints as to how to enable this when it's already present in a distro would be helpful.
``````




---

archive/issue_events_203929.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-04-15T19:10:02Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "title_is": "Document ccache better in developer guide",
    "title_was": "Make ccache a standard spkg",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203929"
}
```



---

archive/issue_events_203930.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-04-15T19:10:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203930"
}
```



---

archive/issue_events_203931.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-04-15T19:10:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203931"
}
```



---

archive/issue_events_203932.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-04-15T19:10:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203932"
}
```



---

archive/issue_comments_172803.json:
```json
{
    "body": "<div id=\"comment:26\" align=\"right\">comment:26</div>\n\n> So we have to open another ticket to prominently advertise it (in the Sage *Developer's* Guide)? ;-)\n\nOr repurpose this one for that, which seems fine to me.  I do agree that 4 GB is nothing to sneeze at, and will reiterate [comment:17](#comment:17).",
    "created_at": "2014-04-15T19:10:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172803",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:26" align="right">comment:26</div>

> So we have to open another ticket to prominently advertise it (in the Sage *Developer's* Guide)? ;-)

Or repurpose this one for that, which seems fine to me.  I do agree that 4 GB is nothing to sneeze at, and will reiterate [comment:17](#comment:17).



---

archive/issue_comments_172804.json:
```json
{
    "body": "Changed reviewer from **Punarbasu Purkayastha** to none",
    "created_at": "2014-04-15T19:10:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172804",
    "user": "https://github.com/kcrisman"
}
```

Changed reviewer from **Punarbasu Purkayastha** to none



---

archive/issue_comments_172805.json:
```json
{
    "body": "<div id=\"comment:27\" align=\"right\">comment:27</div>\n\nReplying to [@kcrisman](#comment:26):\n> > So we have to open another ticket to prominently advertise it (in the Sage *Developer's* Guide)? ;-)\n\n> Or repurpose this one for that, which seems fine to me.  I do agree that 4 GB is nothing to sneeze at, and will reiterate [comment:17](#comment:17).\n\nI don't think the necessary additions to the Developer's Guide will get *that* large.",
    "created_at": "2014-04-15T19:17:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172805",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:27" align="right">comment:27</div>

Replying to [@kcrisman](#comment:26):
> > So we have to open another ticket to prominently advertise it (in the Sage *Developer's* Guide)? ;-)

> Or repurpose this one for that, which seems fine to me.  I do agree that 4 GB is nothing to sneeze at, and will reiterate [comment:17](#comment:17).

I don't think the necessary additions to the Developer's Guide will get *that* large.



---

archive/issue_comments_172806.json:
```json
{
    "body": "<div id=\"comment:28\" align=\"right\">comment:28</div>\n\n> > > So we have to open another ticket to prominently advertise it (in the Sage *Developer's* Guide)? ;-)\n\n> > Or repurpose this one for that, which seems fine to me.  I do agree that 4 GB is nothing to sneeze at, and will reiterate [comment:17](#comment:17).\n\n> \n> I don't think the necessary additions to the Developer's Guide will get *that* large.\n\nLOL",
    "created_at": "2014-04-15T19:43:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172806",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:28" align="right">comment:28</div>

> > > So we have to open another ticket to prominently advertise it (in the Sage *Developer's* Guide)? ;-)

> > Or repurpose this one for that, which seems fine to me.  I do agree that 4 GB is nothing to sneeze at, and will reiterate [comment:17](#comment:17).

> 
> I don't think the necessary additions to the Developer's Guide will get *that* large.

LOL



---

archive/issue_comments_172807.json:
```json
{
    "body": "<div id=\"comment:29\" align=\"right\">comment:29</div>\n\nHi,\n\n(ccing me)\n\nI am against shipping ccache in Sage (following [comment:11](#comment:11)). On the other hand, if some OS fails to have it then why not doing the same as gcc? I have no strong opinion about this as I think that everybody should use a Unix/GNU distribution (with gcc+ccache available).\n\nNevertheless, it would be fantastic to document the usage of ccache\n- for compiling **only** sage source code\n- for compiling sage source code and third-party softwares\nand I see two places where this might be important\n- the Developer guide\n- the README\n\nI started using ccache today (thanks to Robert Bradshaw). The only thing I did is\n\n```\n $ export PATH=PATH_TO_CCACHE:$PATH\n $ sage -b\n```\nIs that the right way to use it? (it seems so as my cache is much bigger now).\n\nTell me if I can help for something on that ticket.\n\nBest\nVincent",
    "created_at": "2014-04-24T15:47:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172807",
    "user": "https://github.com/videlec"
}
```

<div id="comment:29" align="right">comment:29</div>

Hi,

(ccing me)

I am against shipping ccache in Sage (following [comment:11](#comment:11)). On the other hand, if some OS fails to have it then why not doing the same as gcc? I have no strong opinion about this as I think that everybody should use a Unix/GNU distribution (with gcc+ccache available).

Nevertheless, it would be fantastic to document the usage of ccache
- for compiling **only** sage source code
- for compiling sage source code and third-party softwares
and I see two places where this might be important
- the Developer guide
- the README

I started using ccache today (thanks to Robert Bradshaw). The only thing I did is

```
 $ export PATH=PATH_TO_CCACHE:$PATH
 $ sage -b
```
Is that the right way to use it? (it seems so as my cache is much bigger now).

Tell me if I can help for something on that ticket.

Best
Vincent



---

archive/issue_comments_172808.json:
```json
{
    "body": "<div id=\"comment:30\" align=\"right\">comment:30</div>\n\nFollow up on [comment:29](#comment:29): is there a way of caching results of Cythonization?",
    "created_at": "2014-04-24T15:52:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172808",
    "user": "https://github.com/videlec"
}
```

<div id="comment:30" align="right">comment:30</div>

Follow up on [comment:29](#comment:29): is there a way of caching results of Cythonization?



---

archive/issue_comments_172809.json:
```json
{
    "body": "<div id=\"comment:31\" align=\"right\">comment:31</div>\n\nReplying to [@jdemeyer](#comment:18):\n> Replying to [@kini](#comment:16):\n> > the effect of making sage-b faster is most visible if you used ccache during your initial build of sage from the source tarball, I think.\n\n> Sure, since it's a cache, it can only speed up compilation starting from the *second* time you build the same file.\n\nI'm pretty sure even the first compilation is faster, because some files are used multiple times within an install.",
    "created_at": "2014-04-25T07:33:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172809",
    "user": "https://github.com/rwst"
}
```

<div id="comment:31" align="right">comment:31</div>

Replying to [@jdemeyer](#comment:18):
> Replying to [@kini](#comment:16):
> > the effect of making sage-b faster is most visible if you used ccache during your initial build of sage from the source tarball, I think.

> Sure, since it's a cache, it can only speed up compilation starting from the *second* time you build the same file.

I'm pretty sure even the first compilation is faster, because some files are used multiple times within an install.



---

archive/issue_comments_172810.json:
```json
{
    "body": "<div id=\"comment:32\" align=\"right\">comment:32</div>\n\nReplying to [@videlec](#comment:30):\n> Follow up on [comment:29](#comment:29): is there a way of caching results of Cythonization?\n\nYes, see #16148. I think every developer should use it (4GB of hard drive space really isn't that much these days) and I think every user should be considered at least a potential developer (at least the ones who compile from source).",
    "created_at": "2014-04-27T03:43:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172810",
    "user": "https://github.com/robertwb"
}
```

<div id="comment:32" align="right">comment:32</div>

Replying to [@videlec](#comment:30):
> Follow up on [comment:29](#comment:29): is there a way of caching results of Cythonization?

Yes, see #16148. I think every developer should use it (4GB of hard drive space really isn't that much these days) and I think every user should be considered at least a potential developer (at least the ones who compile from source).



---

archive/issue_comments_172811.json:
```json
{
    "body": "<div id=\"comment:33\" align=\"right\">comment:33</div>\n\n> > Follow up on [comment:29](#comment:29): is there a way of caching results of Cythonization?\n\n> \n> Yes, see #16148. I think every developer should use it (4GB of hard drive space really isn't that much these days) and I think every user should be considered at least a potential developer (at least the ones who compile from source). \n\nThough I don't think it should be in binaries anyway.  On a small USB key and some other situations 4 GB is quite a bit.\n\nThat said, I still don't know how to use this properly!  Taking a chance that just using the env var in my .profile was enough...",
    "created_at": "2014-05-29T17:37:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172811",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:33" align="right">comment:33</div>

> > Follow up on [comment:29](#comment:29): is there a way of caching results of Cythonization?

> 
> Yes, see #16148. I think every developer should use it (4GB of hard drive space really isn't that much these days) and I think every user should be considered at least a potential developer (at least the ones who compile from source). 

Though I don't think it should be in binaries anyway.  On a small USB key and some other situations 4 GB is quite a bit.

That said, I still don't know how to use this properly!  Taking a chance that just using the env var in my .profile was enough...



---

archive/issue_comments_172812.json:
```json
{
    "body": "<div id=\"comment:34\" align=\"right\">comment:34</div>\n\nReplying to [@kcrisman](#comment:33):\n> Though I don't think it should be in binaries anyway.  On a small USB key and some other situations 4 GB is quite a bit.\n\nThe 4GB is the default cache size. It would sit in the user homedir, and only gets filled if something is actually compiled (which would fail in a binary distribution, I think). So, shipping ccache with a binary would be useless, but it wouldn't particularly use 4GB anywhere either.",
    "created_at": "2014-05-29T19:21:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172812",
    "user": "https://github.com/nbruin"
}
```

<div id="comment:34" align="right">comment:34</div>

Replying to [@kcrisman](#comment:33):
> Though I don't think it should be in binaries anyway.  On a small USB key and some other situations 4 GB is quite a bit.

The 4GB is the default cache size. It would sit in the user homedir, and only gets filled if something is actually compiled (which would fail in a binary distribution, I think). So, shipping ccache with a binary would be useless, but it wouldn't particularly use 4GB anywhere either.



---

archive/issue_comments_172813.json:
```json
{
    "body": "<div id=\"comment:35\" align=\"right\">comment:35</div>\n\nReplying to [@nbruin](#comment:34):\n> Replying to [@kcrisman](#comment:33):\n> > Though I don't think it should be in binaries anyway.  On a small USB key and some other situations 4 GB is quite a bit.\n\n> The 4GB is the default cache size. It would sit in the user homedir\n\nWhich in many cases would be the wrong place.\n\n\n> and only gets filled if something is actually compiled (which would fail in a binary distribution, I think).\n\nWhile it only gets filled upon compilation, `sage -b` is sufficient, as is the installation of an optional spkg in many if not most cases.  The only difference of bdists is that they don't come with *upstream* source code (i.e., spkgs, or tarballs in `upstream/`.)\n\n\n> So, shipping ccache with a binary would be useless, but it wouldn't particularly use 4GB anywhere either.\n\nSee above.",
    "created_at": "2014-05-29T19:43:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172813",
    "user": "https://github.com/nexttime"
}
```

<div id="comment:35" align="right">comment:35</div>

Replying to [@nbruin](#comment:34):
> Replying to [@kcrisman](#comment:33):
> > Though I don't think it should be in binaries anyway.  On a small USB key and some other situations 4 GB is quite a bit.

> The 4GB is the default cache size. It would sit in the user homedir

Which in many cases would be the wrong place.


> and only gets filled if something is actually compiled (which would fail in a binary distribution, I think).

While it only gets filled upon compilation, `sage -b` is sufficient, as is the installation of an optional spkg in many if not most cases.  The only difference of bdists is that they don't come with *upstream* source code (i.e., spkgs, or tarballs in `upstream/`.)


> So, shipping ccache with a binary would be useless, but it wouldn't particularly use 4GB anywhere either.

See above.



---

archive/issue_events_203933.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203933"
}
```



---

archive/issue_events_203934.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203934"
}
```



---

archive/issue_events_203935.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-11-13T14:06:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203935"
}
```



---

archive/issue_events_203936.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-11-13T14:06:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "c: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203936"
}
```



---

archive/issue_events_203937.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2014-11-20T15:36:00Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "title_is": "Document ccache in developer guide",
    "title_was": "Document ccache better in developer guide",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203937"
}
```



---

archive/issue_comments_172814.json:
```json
{
    "body": "<div id=\"comment:38\" align=\"right\">comment:38</div>\n\nGetting back to the original *point* of this ticket... \n\nIs [the information here](http://www.sagemath.org/doc/installation/source.html#section-envvar) sufficient?  It's true that the *developer* guide does not have any information on it, which seems odd.",
    "created_at": "2014-11-20T15:36:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172814",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:38" align="right">comment:38</div>

Getting back to the original *point* of this ticket... 

Is [the information here](http://www.sagemath.org/doc/installation/source.html#section-envvar) sufficient?  It's true that the *developer* guide does not have any information on it, which seems odd.



---

archive/issue_comments_172815.json:
```json
{
    "body": "<div id=\"comment:39\" align=\"right\">comment:39</div>\n\nReplying to [@kcrisman](#comment:38):\n> Getting back to the original *point* of this ticket... \n> \n> Is [the information here](http://www.sagemath.org/doc/installation/source.html#section-envvar) sufficient?  It's true that the *developer* guide does not have any information on it, which seems odd.\n\nI added some information on `ccache` to the developer guide a couple months ago in #21500 without seeing this discussion. If that's the only point of this ticket, then it can be closed as duplicate.",
    "created_at": "2016-11-15T01:12:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172815",
    "user": "https://github.com/paulmasson"
}
```

<div id="comment:39" align="right">comment:39</div>

Replying to [@kcrisman](#comment:38):
> Getting back to the original *point* of this ticket... 
> 
> Is [the information here](http://www.sagemath.org/doc/installation/source.html#section-envvar) sufficient?  It's true that the *developer* guide does not have any information on it, which seems odd.

I added some information on `ccache` to the developer guide a couple months ago in #21500 without seeing this discussion. If that's the only point of this ticket, then it can be closed as duplicate.



---

archive/issue_events_203938.json:
```json
{
    "actor": "https://github.com/paulmasson",
    "created_at": "2016-11-15T01:12:44Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203938"
}
```



---

archive/issue_events_203939.json:
```json
{
    "actor": "https://github.com/paulmasson",
    "created_at": "2016-11-15T01:12:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203939"
}
```



---

archive/issue_events_203940.json:
```json
{
    "actor": "https://github.com/paulmasson",
    "created_at": "2016-11-15T01:12:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203940"
}
```



---

archive/issue_comments_172816.json:
```json
{
    "body": "<div id=\"comment:41\" align=\"right\">comment:41</div>\n\nI agree, it can be closed as a dup.",
    "created_at": "2016-11-15T01:53:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172816",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:41" align="right">comment:41</div>

I agree, it can be closed as a dup.



---

archive/issue_comments_172817.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2016-11-15T01:53:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14372#issuecomment-172817",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_events_203941.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2016-11-15T01:53:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203941"
}
```



---

archive/issue_events_203942.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2016-11-15T01:53:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203942"
}
```



---

archive/issue_events_203943.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-01-21T18:03:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203943"
}
```



---

archive/issue_events_203944.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-01-21T18:03:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14372",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14372#event-203944"
}
```
