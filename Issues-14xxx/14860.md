# Issue 14860: Bug on binary trees

archive/issues_014656.json:
```json
{
    "assignees": [],
    "body": "The left-border symmetry and left-right symmetry of a leaf retuens None instead of a leaf.\n\n\n```\nsage: b = BinaryTree()\nsage: b.left_border_symmetry()\nsage: b.left_right_symmetry() \nsage: b.left_border_symmetry() in BinaryTrees()\nFals\n```\n\n\nAssignee: **@sagetrac-sage-combinat**\n\nCC:  @sagetrac-stumpc5 @sagetrac-chrisjamesberg @tscrim\n\nKeywords: **BinaryTrees, FindStat**\n\nReviewer: **Christian Stump**\n\nAuthor: **Viviane Pons**\n\nMerged: **sage-5.12.beta1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14860_\n\n",
    "closed_at": "2013-08-16T21:14:09Z",
    "created_at": "2013-07-06T09:12:58Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.12",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Bug on binary trees",
    "type": "issue",
    "updated_at": "2013-08-16T21:14:09Z",
    "url": "https://github.com/sagemath/sage/issues/14860",
    "user": "https://github.com/VivianePons"
}
```
The left-border symmetry and left-right symmetry of a leaf retuens None instead of a leaf.


```
sage: b = BinaryTree()
sage: b.left_border_symmetry()
sage: b.left_right_symmetry() 
sage: b.left_border_symmetry() in BinaryTrees()
Fals
```


Assignee: **@sagetrac-sage-combinat**

CC:  @sagetrac-stumpc5 @sagetrac-chrisjamesberg @tscrim

Keywords: **BinaryTrees, FindStat**

Reviewer: **Christian Stump**

Author: **Viviane Pons**

Merged: **sage-5.12.beta1**

_Issue created by migration from https://trac.sagemath.org/ticket/14860_





---

archive/issue_events_193975.json:
```json
{
    "actor": "https://github.com/VivianePons",
    "created_at": "2013-07-06T09:12:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14860",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14860#event-193975"
}
```



---

archive/issue_events_193976.json:
```json
{
    "actor": "https://github.com/VivianePons",
    "created_at": "2013-07-06T09:12:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14860",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14860#event-193976"
}
```



---

archive/issue_events_193977.json:
```json
{
    "actor": "https://github.com/VivianePons",
    "created_at": "2013-07-06T09:12:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14860",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14860#event-193977"
}
```



---

archive/issue_events_193978.json:
```json
{
    "actor": "https://github.com/VivianePons",
    "created_at": "2013-07-06T09:12:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14860",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14860#event-193978"
}
```



---

archive/issue_comments_184511.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nAttachment: **[trac_14860-bug-binary-trees-vp.patch.gz](https://github.com/sagemath/sage/files/ticket14860/trac_14860-bug-binary-trees-vp.patch.gz)**\n\nlooks good to me!",
    "created_at": "2013-07-06T15:25:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14860",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14860#issuecomment-184511",
    "user": "https://github.com/sagetrac-stumpc5"
}
```

<div id="comment:1" align="right">Comment 1</div>

Attachment: **[trac_14860-bug-binary-trees-vp.patch.gz](https://github.com/sagemath/sage/files/ticket14860/trac_14860-bug-binary-trees-vp.patch.gz)**

looks good to me!



---

archive/issue_comments_184512.json:
```json
{
    "body": "Reviewer: **Christian Stump**",
    "created_at": "2013-07-06T15:25:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14860",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14860#issuecomment-184512",
    "user": "https://github.com/sagetrac-stumpc5"
}
```

Reviewer: **Christian Stump**



---

archive/issue_events_193979.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2013-07-06T15:25:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14860",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14860#event-193979"
}
```



---

archive/issue_events_193980.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2013-07-06T15:25:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14860",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14860#event-193980"
}
```



---

archive/issue_events_193981.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2013-07-06T15:25:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14860",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14860#event-193981"
}
```



---

archive/issue_comments_184513.json:
```json
{
    "body": "Changed keywords from **BinaryTrees** to **BinaryTrees, FindStat**",
    "created_at": "2013-07-06T15:27:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14860",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14860#issuecomment-184513",
    "user": "https://github.com/sagetrac-stumpc5"
}
```

Changed keywords from **BinaryTrees** to **BinaryTrees, FindStat**



---

archive/issue_comments_184514.json:
```json
{
    "body": "Merged: **sage-5.12.beta1**",
    "created_at": "2013-08-16T21:14:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14860",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14860#issuecomment-184514",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.12.beta1**



---

archive/issue_events_193982.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-16T21:14:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14860",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14860#event-193982"
}
```



---

archive/issue_events_193983.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-16T21:14:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14860",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14860#event-193983"
}
```
