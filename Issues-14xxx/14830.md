# Issue 14830: Convergent series on ultrametric discs

archive/issues_014626.json:
```json
{
    "assignees": [
        "https://github.com/roed314"
    ],
    "body": "This patch implements various rings of convergent series on discs over ultrametric fields (that is padics fields or Laurent series fields).\n\nThe documentation is not written yet (it's why I leave the status of this ticket as new); if somebody wants to collaborate and help me to write this documentation, he is of course welcome!\n\nAn inline demo of the functionalities provided by this patch is available here: https://cethop.math.cnrs.fr:8443/home/pub/6/\n\nDepends on #14823\nDepends on #14824\nDepends on #14826\nDepends on #14828\nDepends on #14829\n\nKeywords: **padics series days71**\n\nBranch/Commit: **[u/caruso/iwasawa](https://github.com/sagemath/sagetrac-mirror/tree/u/caruso/iwasawa) @ [`dac3cf3`](https://github.com/sagemath/sagetrac-mirror/commit/dac3cf361e09e4a28dfa0210b57188fe721b648c)**\n\nComponent: **padics**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14830_\n\n",
    "created_at": "2013-06-26T16:51:54Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20padics",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Convergent series on ultrametric discs",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/14830",
    "user": "https://github.com/xcaruso"
}
```
This patch implements various rings of convergent series on discs over ultrametric fields (that is padics fields or Laurent series fields).

The documentation is not written yet (it's why I leave the status of this ticket as new); if somebody wants to collaborate and help me to write this documentation, he is of course welcome!

An inline demo of the functionalities provided by this patch is available here: https://cethop.math.cnrs.fr:8443/home/pub/6/

Depends on #14823
Depends on #14824
Depends on #14826
Depends on #14828
Depends on #14829

Keywords: **padics series days71**

Branch/Commit: **[u/caruso/iwasawa](https://github.com/sagemath/sagetrac-mirror/tree/u/caruso/iwasawa) @ [`dac3cf3`](https://github.com/sagemath/sagetrac-mirror/commit/dac3cf361e09e4a28dfa0210b57188fe721b648c)**

Component: **padics**

_Issue created by migration from https://trac.sagemath.org/ticket/14830_





---

archive/issue_events_210851.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2013-06-26T16:51:54Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14830",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14830#event-210851"
}
```



---

archive/issue_events_210852.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2013-06-26T16:51:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14830",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20padics",
    "label_color": "0000ff",
    "label_name": "c: padics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14830#event-210852"
}
```



---

archive/issue_events_210853.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2013-06-26T16:51:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14830",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14830#event-210853"
}
```



---

archive/issue_events_210854.json:
```json
{
    "actor": "https://github.com/xcaruso",
    "created_at": "2013-06-26T16:51:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14830",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14830#event-210854"
}
```



---

archive/issue_events_210855.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2013-06-26T16:51:54Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/14830",
    "subject": "https://github.com/xcaruso",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14830#event-210855"
}
```



---

archive/issue_comments_181601.json:
```json
{
    "body": "Dependencies: **#14823, #14824, #14826, #14828, #14829**",
    "created_at": "2013-06-26T17:00:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14830",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14830#issuecomment-181601",
    "user": "https://github.com/xcaruso"
}
```

Dependencies: **#14823, #14824, #14826, #14828, #14829**



---

archive/issue_comments_181602.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,5 +1,5 @@\n This patch implements various rings of convergent series on discs over ultrametric fields (that is padics fields or Laurent series fields).\n \n-The documentation is not written yet (it's why I leave the status of this ticket as need_works); if somebody wants to collaborate and help me to write this documentation, he is of course welcome!\n+The documentation is not written yet (it's why I leave the status of this ticket as new); if somebody wants to collaborate and help me to write this documentation, he is of course welcome!\n \n An inline demo of the functionalities provided by this patch is available here: https://cethop.math.cnrs.fr:8443/home/pub/6/\n``````\n",
    "created_at": "2013-06-26T17:00:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14830",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14830#issuecomment-181602",
    "user": "https://github.com/xcaruso"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,5 +1,5 @@
 This patch implements various rings of convergent series on discs over ultrametric fields (that is padics fields or Laurent series fields).
 
-The documentation is not written yet (it's why I leave the status of this ticket as need_works); if somebody wants to collaborate and help me to write this documentation, he is of course welcome!
+The documentation is not written yet (it's why I leave the status of this ticket as new); if somebody wants to collaborate and help me to write this documentation, he is of course welcome!
 
 An inline demo of the functionalities provided by this patch is available here: https://cethop.math.cnrs.fr:8443/home/pub/6/
``````




---

archive/issue_comments_181603.json:
```json
{
    "body": "Attachment: **[trac_14830_bounded_series.patch.gz](https://github.com/sagemath/sage/files/ticket14830/trac_14830_bounded_series.patch.gz)**",
    "created_at": "2013-06-26T17:00:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14830",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14830#issuecomment-181603",
    "user": "https://github.com/xcaruso"
}
```

Attachment: **[trac_14830_bounded_series.patch.gz](https://github.com/sagemath/sage/files/ticket14830/trac_14830_bounded_series.patch.gz)**



---

archive/issue_events_210856.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14830",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14830#event-210856"
}
```



---

archive/issue_events_210857.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14830",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14830#event-210857"
}
```



---

archive/issue_events_210858.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14830",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14830#event-210858"
}
```



---

archive/issue_events_210859.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14830",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14830#event-210859"
}
```



---

archive/issue_events_210860.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14830",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14830#event-210860"
}
```



---

archive/issue_events_210861.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14830",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14830#event-210861"
}
```



---

archive/issue_comments_181604.json:
```json
{
    "body": "Branch: **[u/caruso/iwasawa](https://github.com/sagemath/sagetrac-mirror/tree/u/caruso/iwasawa)**",
    "created_at": "2016-03-22T23:22:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14830",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14830#issuecomment-181604",
    "user": "https://github.com/xcaruso"
}
```

Branch: **[u/caruso/iwasawa](https://github.com/sagemath/sagetrac-mirror/tree/u/caruso/iwasawa)**



---

archive/issue_comments_181605.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nI have added a tutorial (but the full documentation is not written yet).\n\n---\nLast 10 new commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2a9e90c6311c22e48adc51a86559e2ea2d78810d\">2a9e90c</a></td><td><code>Merge branch 'public/ticket/14828' into 6.4.b6</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5a1a485c5888b374f8380915eed4fdd3b732d3a5\">5a1a485</a></td><td><code>trac #14828 more doc, other minor changes</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d2f52cf0e8631aee2856bbe60dcbb6e0e1dc2920\">d2f52cf</a></td><td><code>Merge branch 'public/ticket/14828' into 6.5.rc0</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d59c712cbededad8c3442388005774aa2c86267c\">d59c712</a></td><td><code>Merge branch 'public/ticket/14828' into 6.9.b4</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bbe959d69bc3868cdc49a2f2cdc8e62d83bc52d5\">bbe959d</a></td><td><code>trac #14828 trying to make it build again</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/edbf797a6ea03e827122d00895576d945ef6ea8f\">edbf797</a></td><td><code>trac #14828 trying to make the doc build</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/309d3732b825eabf9c7d192c965a7b28fdb512ab\">309d373</a></td><td><code>Merge branch 'public/ticket/14828' into 6.10.beta3</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b8cfd47068dfd122946a57cde3767fceed515cc3\">b8cfd47</a></td><td><code>Merge branch 'develop' into t/14828/public/ticket/14828</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c7c02f5e40ef4ef6815034e66e6ff89b02ff3e25\">c7c02f5</a></td><td><code>Merge branch 't/14828/public/ticket/14828' into iwasawa</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8ccc2442ba2d6784dca7a063be332ff99c8b3c3a\">8ccc244</a></td><td><code>Bugs fixed and tutorial added</code></td></tr></table>\n",
    "created_at": "2016-03-22T23:24:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14830",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14830#issuecomment-181605",
    "user": "https://github.com/xcaruso"
}
```

<div id="comment:6" align="right">comment:6</div>

I have added a tutorial (but the full documentation is not written yet).

---
Last 10 new commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2a9e90c6311c22e48adc51a86559e2ea2d78810d">2a9e90c</a></td><td><code>Merge branch 'public/ticket/14828' into 6.4.b6</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5a1a485c5888b374f8380915eed4fdd3b732d3a5">5a1a485</a></td><td><code>trac #14828 more doc, other minor changes</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d2f52cf0e8631aee2856bbe60dcbb6e0e1dc2920">d2f52cf</a></td><td><code>Merge branch 'public/ticket/14828' into 6.5.rc0</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d59c712cbededad8c3442388005774aa2c86267c">d59c712</a></td><td><code>Merge branch 'public/ticket/14828' into 6.9.b4</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bbe959d69bc3868cdc49a2f2cdc8e62d83bc52d5">bbe959d</a></td><td><code>trac #14828 trying to make it build again</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/edbf797a6ea03e827122d00895576d945ef6ea8f">edbf797</a></td><td><code>trac #14828 trying to make the doc build</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/309d3732b825eabf9c7d192c965a7b28fdb512ab">309d373</a></td><td><code>Merge branch 'public/ticket/14828' into 6.10.beta3</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b8cfd47068dfd122946a57cde3767fceed515cc3">b8cfd47</a></td><td><code>Merge branch 'develop' into t/14828/public/ticket/14828</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c7c02f5e40ef4ef6815034e66e6ff89b02ff3e25">c7c02f5</a></td><td><code>Merge branch 't/14828/public/ticket/14828' into iwasawa</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8ccc2442ba2d6784dca7a063be332ff99c8b3c3a">8ccc244</a></td><td><code>Bugs fixed and tutorial added</code></td></tr></table>




---

archive/issue_comments_181606.json:
```json
{
    "body": "Commit: **[`8ccc244`](https://github.com/sagemath/sagetrac-mirror/commit/8ccc2442ba2d6784dca7a063be332ff99c8b3c3a)**",
    "created_at": "2016-03-22T23:24:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14830",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14830#issuecomment-181606",
    "user": "https://github.com/xcaruso"
}
```

Commit: **[`8ccc244`](https://github.com/sagemath/sagetrac-mirror/commit/8ccc2442ba2d6784dca7a063be332ff99c8b3c3a)**



---

archive/issue_comments_181607.json:
```json
{
    "body": "<div id=\"comment:7\"></div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/dac3cf361e09e4a28dfa0210b57188fe721b648c\">dac3cf3</a></td><td><code>File bounded_series.patch removed</code></td></tr></table>\n",
    "created_at": "2016-03-22T23:27:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14830",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14830#issuecomment-181607",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:7"></div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/dac3cf361e09e4a28dfa0210b57188fe721b648c">dac3cf3</a></td><td><code>File bounded_series.patch removed</code></td></tr></table>




---

archive/issue_comments_181608.json:
```json
{
    "body": "Changed commit from **[`8ccc244`](https://github.com/sagemath/sagetrac-mirror/commit/8ccc2442ba2d6784dca7a063be332ff99c8b3c3a)** to **[`dac3cf3`](https://github.com/sagemath/sagetrac-mirror/commit/dac3cf361e09e4a28dfa0210b57188fe721b648c)**",
    "created_at": "2016-03-22T23:27:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14830",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14830#issuecomment-181608",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[`8ccc244`](https://github.com/sagemath/sagetrac-mirror/commit/8ccc2442ba2d6784dca7a063be332ff99c8b3c3a)** to **[`dac3cf3`](https://github.com/sagemath/sagetrac-mirror/commit/dac3cf361e09e4a28dfa0210b57188fe721b648c)**



---

archive/issue_comments_181609.json:
```json
{
    "body": "Changed keywords from **padics series** to **padics series sd71**",
    "created_at": "2016-03-22T23:28:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14830",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14830#issuecomment-181609",
    "user": "https://github.com/xcaruso"
}
```

Changed keywords from **padics series** to **padics series sd71**



---

archive/issue_comments_181610.json:
```json
{
    "body": "Changed keywords from **padics series sd71** to **padics series days71**",
    "created_at": "2016-03-23T17:40:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14830",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14830#issuecomment-181610",
    "user": "https://github.com/jbalakrishnan"
}
```

Changed keywords from **padics series sd71** to **padics series days71**



---

archive/issue_events_210862.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14830",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14830#event-210862"
}
```
