# Issue 14813: Finitely generated abelian group presentations

archive/issues_014609.json:
```json
{
    "body": "Method to construct finitely generated abelian groups as quotients of free groups. User inputs a list of integers to define an abelian group, then the invariants of the input list are used to build the group as a finite presentation.\n\n```\nsage:groups.presentation.FGAbelian([2,2])\nFinitely presented group < a, b | a^2, b^2, a^-1*b^-1*a*b >\nsage: groups.presentation.FGAbelian([6,3,5,17])\nFinitely presented group < a, b | a^3, b^510, a^-1*b^-1*a*b >\n```\n\nDepends on #14791, which creates the \"named\" finitely presented groups infrastructure, and #14790 which\ngives the python generator needed for variable names.\n\n__Apply__\n\n1 [attachment:trac_14813_fgabelian_fpgroups.patch]\n\n\nAssignee: tbd\n\nCC:  @rbeezer @vbraun @miguelmarco\n\nKeywords: abelian group presentations\n\nReviewer: Travis Scrimshaw\n\nAuthor: Davis Shurbert\n\nMerged: sage-5.13.beta1\n\nDependencies: #14790, #14791\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/14813\n\n",
    "closed_at": "2013-10-12T09:45:40Z",
    "created_at": "2013-06-25T01:17:22Z",
    "labels": [
        "component: group theory",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-5.13",
    "title": "Finitely generated abelian group presentations",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/14813",
    "user": "https://trac.sagemath.org/admin/accounts/users/dshurbert"
}
```
Method to construct finitely generated abelian groups as quotients of free groups. User inputs a list of integers to define an abelian group, then the invariants of the input list are used to build the group as a finite presentation.

```
sage:groups.presentation.FGAbelian([2,2])
Finitely presented group < a, b | a^2, b^2, a^-1*b^-1*a*b >
sage: groups.presentation.FGAbelian([6,3,5,17])
Finitely presented group < a, b | a^3, b^510, a^-1*b^-1*a*b >
```

Depends on #14791, which creates the "named" finitely presented groups infrastructure, and #14790 which
gives the python generator needed for variable names.

__Apply__

1 [attachment:trac_14813_fgabelian_fpgroups.patch]


Assignee: tbd

CC:  @rbeezer @vbraun @miguelmarco

Keywords: abelian group presentations

Reviewer: Travis Scrimshaw

Author: Davis Shurbert

Merged: sage-5.13.beta1

Dependencies: #14790, #14791

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/14813





---

archive/issue_comments_232644.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -6,6 +6,11 @@\n sage: groups.presentation.FGAbelian([6,3,5,17])\n Finitely presented group < a, b | a^3, b^510, a^-1*b^-1*a*b >\n ```\n+\n+__Apply__\n+\n+1 [attachment:trac_14813_fgabelian_fpgroups.patch]\n+\n \n Author: Davis Shurbert\n \n``````\n",
    "created_at": "2013-06-25T01:20:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14813#issuecomment-232644",
    "user": "https://trac.sagemath.org/admin/accounts/users/dshurbert"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -6,6 +6,11 @@
 sage: groups.presentation.FGAbelian([6,3,5,17])
 Finitely presented group < a, b | a^3, b^510, a^-1*b^-1*a*b >
 ```
+
+__Apply__
+
+1 [attachment:trac_14813_fgabelian_fpgroups.patch]
+
 
 Author: Davis Shurbert
 
``````




---

archive/issue_comments_232645.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,17 @@\n+Method to construct finitely generated abelian groups as quotients of free groups. User inputs a list of integers to define an abelian group, then the invariants of the input list are used to build the group as a finite presentation.\n+\n+```\n+sage:groups.presentation.FGAbelian([2,2])\n+Finitely presented group < a, b | a^2, b^2, a^-1*b^-1*a*b >\n+sage: groups.presentation.FGAbelian([6,3,5,17])\n+Finitely presented group < a, b | a^3, b^510, a^-1*b^-1*a*b >\n+```\n+\n+Depends on #14791, which creates the \"named\" finitely presented groups infrastructure.\n+\n+__Apply__\n+\n+1 [attachment:trac_14813_fgabelian_fpgroups.patch]\n \n \n Author: Davis Shurbert\n``````\n",
    "created_at": "2013-06-25T01:23:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14813#issuecomment-232645",
    "user": "https://trac.sagemath.org/admin/accounts/users/dshurbert"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,17 @@
+Method to construct finitely generated abelian groups as quotients of free groups. User inputs a list of integers to define an abelian group, then the invariants of the input list are used to build the group as a finite presentation.
+
+```
+sage:groups.presentation.FGAbelian([2,2])
+Finitely presented group < a, b | a^2, b^2, a^-1*b^-1*a*b >
+sage: groups.presentation.FGAbelian([6,3,5,17])
+Finitely presented group < a, b | a^3, b^510, a^-1*b^-1*a*b >
+```
+
+Depends on #14791, which creates the "named" finitely presented groups infrastructure.
+
+__Apply__
+
+1 [attachment:trac_14813_fgabelian_fpgroups.patch]
 
 
 Author: Davis Shurbert
``````




---

archive/issue_comments_232646.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,18 @@\n+Method to construct finitely generated abelian groups as quotients of free groups. User inputs a list of integers to define an abelian group, then the invariants of the input list are used to build the group as a finite presentation.\n+\n+```\n+sage:groups.presentation.FGAbelian([2,2])\n+Finitely presented group < a, b | a^2, b^2, a^-1*b^-1*a*b >\n+sage: groups.presentation.FGAbelian([6,3,5,17])\n+Finitely presented group < a, b | a^3, b^510, a^-1*b^-1*a*b >\n+```\n+\n+Depends on #14791, which creates the \"named\" finitely presented groups infrastructure, and #14790 which\n+gives the python generator needed for variable names.\n+\n+__Apply__\n+\n+1 [attachment:trac_14813_fgabelian_fpgroups.patch]\n \n \n Author: Davis Shurbert\n``````\n",
    "created_at": "2013-06-25T01:25:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14813#issuecomment-232646",
    "user": "https://trac.sagemath.org/admin/accounts/users/dshurbert"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,18 @@
+Method to construct finitely generated abelian groups as quotients of free groups. User inputs a list of integers to define an abelian group, then the invariants of the input list are used to build the group as a finite presentation.
+
+```
+sage:groups.presentation.FGAbelian([2,2])
+Finitely presented group < a, b | a^2, b^2, a^-1*b^-1*a*b >
+sage: groups.presentation.FGAbelian([6,3,5,17])
+Finitely presented group < a, b | a^3, b^510, a^-1*b^-1*a*b >
+```
+
+Depends on #14791, which creates the "named" finitely presented groups infrastructure, and #14790 which
+gives the python generator needed for variable names.
+
+__Apply__
+
+1 [attachment:trac_14813_fgabelian_fpgroups.patch]
 
 
 Author: Davis Shurbert
``````




---

archive/issue_comments_232647.json:
```json
{
    "body": "Changing status from new to needs_info.",
    "created_at": "2013-06-25T01:25:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14813#issuecomment-232647",
    "user": "https://trac.sagemath.org/admin/accounts/users/dshurbert"
}
```

Changing status from new to needs_info.



---

archive/issue_comments_232648.json:
```json
{
    "body": "Changing component from PLEASE CHANGE to group theory.",
    "created_at": "2013-06-25T01:26:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14813#issuecomment-232648",
    "user": "https://trac.sagemath.org/admin/accounts/users/dshurbert"
}
```

Changing component from PLEASE CHANGE to group theory.



---

archive/issue_comments_232649.json:
```json
{
    "body": "Changing status from needs_info to needs_review.",
    "created_at": "2013-07-14T19:55:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14813#issuecomment-232649",
    "user": "https://trac.sagemath.org/admin/accounts/users/dshurbert"
}
```

Changing status from needs_info to needs_review.



---

archive/issue_comments_232650.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2013-09-04T08:23:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14813#issuecomment-232650",
    "user": "https://trac.sagemath.org/admin/accounts/users/dshurbert"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_232651.json:
```json
{
    "body": "<a id='comment:6'></a>Rebasing to review patch on #14791",
    "created_at": "2013-09-04T08:23:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14813#issuecomment-232651",
    "user": "https://trac.sagemath.org/admin/accounts/users/dshurbert"
}
```

<a id='comment:6'></a>Rebasing to review patch on #14791



---

archive/issue_comments_232652.json:
```json
{
    "body": "<a id='comment:7'></a>Rebased the patch on this ticket to the review patch on #14791",
    "created_at": "2013-09-05T16:53:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14813#issuecomment-232652",
    "user": "https://trac.sagemath.org/admin/accounts/users/dshurbert"
}
```

<a id='comment:7'></a>Rebased the patch on this ticket to the review patch on #14791



---

archive/issue_comments_232653.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2013-09-05T16:53:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14813#issuecomment-232653",
    "user": "https://trac.sagemath.org/admin/accounts/users/dshurbert"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_232654.json:
```json
{
    "body": "<a id='comment:8'></a>Hey,\n\nOne minor thing, instead of `\\mathbb{Z}`, could you use `\\ZZ` so Sage's documentation is consistent? Other than that, it looks good to me.\n\nBest,\n\nTravis",
    "created_at": "2013-09-07T00:53:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14813#issuecomment-232654",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:8'></a>Hey,

One minor thing, instead of `\mathbb{Z}`, could you use `\ZZ` so Sage's documentation is consistent? Other than that, it looks good to me.

Best,

Travis



---

archive/issue_comments_232655.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Travis Scrimshaw\"",
    "created_at": "2013-09-07T00:53:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14813#issuecomment-232655",
    "user": "https://github.com/tscrim"
}
```

Changing reviewer from "" to "Travis Scrimshaw"



---

archive/issue_comments_232656.json:
```json
{
    "body": "Replacement patch",
    "created_at": "2013-09-09T15:20:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14813#issuecomment-232656",
    "user": "https://trac.sagemath.org/admin/accounts/users/dshurbert"
}
```

Replacement patch



---

archive/issue_comments_232657.json:
```json
{
    "body": "<a id='comment:9'></a>Attachment [trac_14813_fgabelian_fpgroups.patch](tarball://root/attachments/some-uuid/ticket14813/trac_14813_fgabelian_fpgroups.patch) by dshurbert created at 2013-09-09 15:20:59\n\nFixed documentation inconsistencies, thanks for the tip.\n\nDavis",
    "created_at": "2013-09-09T15:20:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14813#issuecomment-232657",
    "user": "https://trac.sagemath.org/admin/accounts/users/dshurbert"
}
```

<a id='comment:9'></a>Attachment [trac_14813_fgabelian_fpgroups.patch](tarball://root/attachments/some-uuid/ticket14813/trac_14813_fgabelian_fpgroups.patch) by dshurbert created at 2013-09-09 15:20:59

Fixed documentation inconsistencies, thanks for the tip.

Davis



---

archive/issue_comments_232658.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2013-09-09T15:24:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14813#issuecomment-232658",
    "user": "https://github.com/tscrim"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_232659.json:
```json
{
    "body": "<a id='comment:10'></a>Thank you.",
    "created_at": "2013-09-09T15:24:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14813#issuecomment-232659",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:10'></a>Thank you.



---

archive/issue_events_042529.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-09-26T15:47:44Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14813",
    "milestone": "sage-5.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14813#event-42529"
}
```



---

archive/issue_comments_232660.json:
```json
{
    "body": "Changing merged from \"\" to \"sage-5.13.beta1\"",
    "created_at": "2013-10-12T09:45:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14813#issuecomment-232660",
    "user": "https://github.com/jdemeyer"
}
```

Changing merged from "" to "sage-5.13.beta1"



---

archive/issue_events_042530.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-12T09:45:40Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/14813",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14813#event-42530"
}
```



---

archive/issue_comments_232661.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2013-10-12T09:45:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14813",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14813#issuecomment-232661",
    "user": "https://github.com/jdemeyer"
}
```

Resolution: fixed
