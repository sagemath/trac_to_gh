...
```json
{
    "body": "Attachment [prereq-1.2.diff](tarball://root/attachments/some-uuid/ticket14406/prereq-1.2.diff) by @jdemeyer created at 2013-04-03 13:58:47",
...
```json
{
    "body": "And I don't know what the FreeBSD libm lacks that make Sage's compilation fail (except for the logl function for R of course).",
...
```json
{
    "body": "Attachment [14406_cygwin_prereq.patch](tarball://root/attachments/some-uuid/ticket14406/14406_cygwin_prereq.patch) by @jdemeyer created at 2013-04-03 14:58:17\n\nReplying to [comment:9 leif]:\n> Actually, we need both `sqrtl()` and `[lt]gammal()` unless we build Cephes\n\nIt seems not, Cygwin doesn't have `sqrtl()`, nor does it build Cephes. Despite this, Sage builds.",
...
```json
{
    "body": "OK, simplified the `tar`/`make` checks.",
...
```json
{
    "body": "Ok, I've slightly recomplexified it :)",
...
```json
{
    "body": "Oops, something is wrong, coming back with a proper patch.",
...
```json
{
    "body": "P.S.:  I don't have a strong opinion on the libm tests; building Sage requires a C99 environment (compiler + library) anyway.  We may add a comment on this to the *Sage Porting Guide<sup>TM</sup>*.",
...
```json
{
    "body": "I would be against more system-specific checks in `prereq`, I would like to move to a real `./configure` before we do that. Because otherwise, it will lead to inconsistent checks (inconsistency between prereq and cephes's `spkg-install` like we're seeing now).",
...
```json
{
    "body": "Reviewer patch is okay for me.",
...
```json
{
    "body": "I've opened #14410 for the ATLAS on Cygwin stuff.",
...
```json
{
    "body": "Okay, this is better than the alternatives.",
...
```json
{
    "body": "Groumpf, I just realized that the last elif clause filtering OS names should include Cygwin (and beware that uname does not just return CYGWIN but CYGWIN... where ... gives additional precision which we don't really care about at the moment).\nShould we fix that on another ticket?",
