# Issue 14045: Comparison of float('inf') and infinity

archive/issues_013841.json:
```json
{
    "assignees": [],
    "body": "Currently infinity and float do not compare properly:\n\n```\nsage: oo > float('inf')\nTrue\nsage: oo == float('+inf')\nFalse\n```\nbecause of\n\n```\nsage: InfinityRing(float('inf'))\nA positive finite number\n```\n\nSage-devel topic: http://groups.google.com/forum/?fromgroups=#!topic/sage-devel/GMv525C0YcY\n\nAssignee: **@tscrim**\n\nKeywords: **infinity, coercion**\n\nReviewer: **Robert Bradshaw**\n\nAuthor: **Travis Scrimshaw**\n\nMerged: **sage-5.7.beta3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14045_\n\n",
    "closed_at": "2013-02-05T08:21:41Z",
    "created_at": "2013-01-31T20:41:17Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20coercion",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Comparison of float('inf') and infinity",
    "type": "issue",
    "updated_at": "2013-02-05T08:21:41Z",
    "url": "https://github.com/sagemath/sage/issues/14045",
    "user": "https://github.com/tscrim"
}
```
Currently infinity and float do not compare properly:

```
sage: oo > float('inf')
True
sage: oo == float('+inf')
False
```
because of

```
sage: InfinityRing(float('inf'))
A positive finite number
```

Sage-devel topic: http://groups.google.com/forum/?fromgroups=#!topic/sage-devel/GMv525C0YcY

Assignee: **@tscrim**

Keywords: **infinity, coercion**

Reviewer: **Robert Bradshaw**

Author: **Travis Scrimshaw**

Merged: **sage-5.7.beta3**

_Issue created by migration from https://trac.sagemath.org/ticket/14045_





---

archive/issue_events_174689.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-01-31T20:41:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14045",
    "milestone_number": null,
    "milestone_title": "sage-5.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14045#event-174689"
}
```



---

archive/issue_events_174690.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-01-31T20:41:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14045",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20coercion",
    "label_color": "0000ff",
    "label_name": "component: coercion",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14045#event-174690"
}
```



---

archive/issue_events_174691.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-01-31T20:41:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14045",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14045#event-174691"
}
```



---

archive/issue_events_174692.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-01-31T20:41:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14045",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14045#event-174692"
}
```



---

archive/issue_events_174693.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-01-31T20:57:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14045",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14045#event-174693"
}
```



---

archive/issue_comments_169041.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -12,3 +12,5 @@\n sage: InfinityRing(float('inf'))\n A positive finite number\n ```\n+\n+Sage-devel topic: http://groups.google.com/forum/?fromgroups=#!topic/sage-devel/GMv525C0YcY\n``````\n",
    "created_at": "2013-02-02T15:32:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14045",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14045#issuecomment-169041",
    "user": "https://github.com/tscrim"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -12,3 +12,5 @@
 sage: InfinityRing(float('inf'))
 A positive finite number
 ```
+
+Sage-devel topic: http://groups.google.com/forum/?fromgroups=#!topic/sage-devel/GMv525C0YcY
``````




---

archive/issue_comments_169042.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nMakes a lot of sense to me, thanks.",
    "created_at": "2013-02-02T19:11:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14045",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14045#issuecomment-169042",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:3'>Comment 3:</a>
Makes a lot of sense to me, thanks.



---

archive/issue_events_174694.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2013-02-02T19:11:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14045",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14045#event-174694"
}
```



---

archive/issue_events_174695.json:
```json
{
    "actor": "https://github.com/robertwb",
    "created_at": "2013-02-02T19:11:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14045",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14045#event-174695"
}
```



---

archive/issue_comments_169043.json:
```json
{
    "body": "Reviewer: **Robert Bradshaw**",
    "created_at": "2013-02-04T09:56:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14045",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14045#issuecomment-169043",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Robert Bradshaw**



---

archive/issue_events_174696.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-04T09:56:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14045",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14045#event-174696"
}
```



---

archive/issue_events_174697.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-04T09:56:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14045",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14045#event-174697"
}
```



---

archive/issue_comments_169044.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nHow was this patch file created?  It is missing a \"User\" field, you should create a `$HOME/.hgrc` file as explained in [http://sagemath.org/doc/developer/producing_patches.html](http://sagemath.org/doc/developer/producing_patches.html) and export the patch using `hg export tip`.",
    "created_at": "2013-02-04T09:56:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14045",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14045#issuecomment-169044",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:4'>Comment 4:</a>
How was this patch file created?  It is missing a "User" field, you should create a `$HOME/.hgrc` file as explained in [http://sagemath.org/doc/developer/producing_patches.html](http://sagemath.org/doc/developer/producing_patches.html) and export the patch using `hg export tip`.



---

archive/issue_comments_169045.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nAttachment: **[trac_14045-float_infinity_coercion-ts.patch.gz](https://github.com/sagemath/sage/files/ticket14045/trac_14045-float_infinity_coercion-ts.patch.gz)**\n\nForgot to export. Sorry. Fixed.",
    "created_at": "2013-02-04T15:39:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14045",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14045#issuecomment-169045",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:5'>Comment 5:</a>
Attachment: **[trac_14045-float_infinity_coercion-ts.patch.gz](https://github.com/sagemath/sage/files/ticket14045/trac_14045-float_infinity_coercion-ts.patch.gz)**

Forgot to export. Sorry. Fixed.



---

archive/issue_events_174698.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-04T15:39:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14045",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14045#event-174698"
}
```



---

archive/issue_events_174699.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-04T15:39:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14045",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14045#event-174699"
}
```



---

archive/issue_comments_169046.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nSorry I missed that, I was too busy looking at the patch's content :-P.",
    "created_at": "2013-02-04T19:39:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14045",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14045#issuecomment-169046",
    "user": "https://github.com/robertwb"
}
```

<a id='comment:6'>Comment 6:</a>
Sorry I missed that, I was too busy looking at the patch's content :-P.



---

archive/issue_comments_169047.json:
```json
{
    "body": "Merged: **sage-5.7.beta3**",
    "created_at": "2013-02-05T08:21:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14045",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14045#issuecomment-169047",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.7.beta3**



---

archive/issue_events_174700.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-05T08:21:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14045",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14045#event-174700"
}
```



---

archive/issue_events_174701.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-05T08:21:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14045",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14045#event-174701"
}
```
