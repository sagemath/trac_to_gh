# Issue 14915: Add _sympy_ method to real_mpfr.pyx

archive/issues_014711.json:
```json
{
    "assignees": [],
    "body": "See [this ask.sagemath](http://ask.sagemath.org/question/2835/error-while-integrate-using-algorithmsympy-bug) question. The code for introducing it is already in that ticket. It only needs an additional doctest.\n\n---\nApply [attachment: trac_14915-add_sympy.patch](https://github.com/sagemath/sage/files/ticket14915/trac_14915-add_sympy.patch.gz) to devel/sage\n\nCC:  @kcrisman\n\nKeywords: **sympy mpfr**\n\nReviewer: **Jeroen Demeyer**\n\nAuthor: **Punarbasu Purkayastha**\n\nMerged: **sage-5.13.beta2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14915_\n\n",
    "closed_at": "2013-10-31T19:16:54Z",
    "created_at": "2013-07-21T16:36:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20calculus",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.13",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add _sympy_ method to real_mpfr.pyx",
    "type": "issue",
    "updated_at": "2013-10-31T19:16:54Z",
    "url": "https://github.com/sagemath/sage/issues/14915",
    "user": "https://github.com/ppurka"
}
```
See [this ask.sagemath](http://ask.sagemath.org/question/2835/error-while-integrate-using-algorithmsympy-bug) question. The code for introducing it is already in that ticket. It only needs an additional doctest.

---
Apply [attachment: trac_14915-add_sympy.patch](https://github.com/sagemath/sage/files/ticket14915/trac_14915-add_sympy.patch.gz) to devel/sage

CC:  @kcrisman

Keywords: **sympy mpfr**

Reviewer: **Jeroen Demeyer**

Author: **Punarbasu Purkayastha**

Merged: **sage-5.13.beta2**

_Issue created by migration from https://trac.sagemath.org/ticket/14915_





---

archive/issue_events_194799.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2013-07-21T16:36:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14915",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20calculus",
    "label_color": "0000ff",
    "label_name": "component: calculus",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14915#event-194799"
}
```



---

archive/issue_events_194800.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2013-07-21T16:36:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14915",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14915#event-194800"
}
```



---

archive/issue_events_194801.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2013-07-21T16:36:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14915",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14915#event-194801"
}
```



---

archive/issue_comments_185646.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nTangentially related: #15256.",
    "created_at": "2013-10-06T01:52:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14915",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14915#issuecomment-185646",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:2" align="right">Comment 2</div>

Tangentially related: #15256.



---

archive/issue_comments_185647.json:
```json
{
    "body": "Apply to devel/sage",
    "created_at": "2013-10-12T06:28:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14915",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14915#issuecomment-185647",
    "user": "https://github.com/ppurka"
}
```

Apply to devel/sage



---

archive/issue_events_194802.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2013-10-12T06:31:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14915",
    "milestone_number": null,
    "milestone_title": "sage-5.13",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14915#event-194802"
}
```



---

archive/issue_events_194803.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2013-10-12T06:31:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14915",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14915#event-194803"
}
```



---

archive/issue_comments_185648.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,4 @@\n See [this ask.sagemath](http://ask.sagemath.org/question/2835/error-while-integrate-using-algorithmsympy-bug) question. The code for introducing it is already in that ticket. It only needs an additional doctest.\n+\n+---\n+Apply [attachment: trac_14915-add_sympy.patch](https://github.com/sagemath/sage/files/ticket14915/trac_14915-add_sympy.patch.gz) to devel/sage\n``````\n",
    "created_at": "2013-10-12T06:31:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14915",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14915#issuecomment-185648",
    "user": "https://github.com/ppurka"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,4 @@
 See [this ask.sagemath](http://ask.sagemath.org/question/2835/error-while-integrate-using-algorithmsympy-bug) question. The code for introducing it is already in that ticket. It only needs an additional doctest.
+
+---
+Apply [attachment: trac_14915-add_sympy.patch](https://github.com/sagemath/sage/files/ticket14915/trac_14915-add_sympy.patch.gz) to devel/sage
``````




---

archive/issue_comments_185649.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nAttachment: **[trac_14915-add_sympy.patch.gz](https://github.com/sagemath/sage/files/ticket14915/trac_14915-add_sympy.patch.gz)**\n\nPatch to fix the bug is attached.",
    "created_at": "2013-10-12T06:31:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14915",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14915#issuecomment-185649",
    "user": "https://github.com/ppurka"
}
```

<div id="comment:3" align="right">Comment 3</div>

Attachment: **[trac_14915-add_sympy.patch.gz](https://github.com/sagemath/sage/files/ticket14915/trac_14915-add_sympy.patch.gz)**

Patch to fix the bug is attached.



---

archive/issue_comments_185650.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2013-10-29T07:54:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14915",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14915#issuecomment-185650",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_comments_185651.json:
```json
{
    "body": "Author: **Punarbasu Purkayastha**",
    "created_at": "2013-10-29T07:54:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14915",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14915#issuecomment-185651",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Punarbasu Purkayastha**



---

archive/issue_events_194804.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-29T07:54:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14915",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14915#event-194804"
}
```



---

archive/issue_events_194805.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-29T07:54:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14915",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14915#event-194805"
}
```



---

archive/issue_events_194806.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-31T19:16:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14915",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14915#event-194806"
}
```



---

archive/issue_events_194807.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-31T19:16:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14915",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14915#event-194807"
}
```



---

archive/issue_comments_185652.json:
```json
{
    "body": "Merged: **sage-5.13.beta2**",
    "created_at": "2013-10-31T19:16:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14915",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14915#issuecomment-185652",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.13.beta2**
