# Issue 14588: find_local_minimum calls f with an expression

archive/issues_014384.json:
```json
{
    "assignees": [],
    "body": "The call\n\n```\nfind_local_minimum(lambda x: deviation(x, some_const),\n                   -maxrange, maxrange)\n```\n\ncalls the lambda with an expression of the form\n`1234 == 4567` at least once. Complete code:\n\n- http://alpha.sagenb.org/home/pub/701/\n\nAssignee: **@jasongrout, @sagetrac-jkantor**\n\nCC:  @slel\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14588_\n\n",
    "closed_at": "2020-10-10T19:46:00Z",
    "created_at": "2013-05-15T10:05:43Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20numerical",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "find_local_minimum calls f with an expression",
    "type": "issue",
    "updated_at": "2020-10-10T19:46:00Z",
    "url": "https://github.com/sagemath/sage/issues/14588",
    "user": "https://github.com/sagetrac-ManDay"
}
```
The call

```
find_local_minimum(lambda x: deviation(x, some_const),
                   -maxrange, maxrange)
```

calls the lambda with an expression of the form
`1234 == 4567` at least once. Complete code:

- http://alpha.sagenb.org/home/pub/701/

Assignee: **@jasongrout, @sagetrac-jkantor**

CC:  @slel

_Issue created by migration from https://trac.sagemath.org/ticket/14588_





---

archive/issue_events_189567.json:
```json
{
    "actor": "https://github.com/sagetrac-ManDay",
    "created_at": "2013-05-15T10:05:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14588",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14588#event-189567"
}
```



---

archive/issue_events_189568.json:
```json
{
    "actor": "https://github.com/sagetrac-ManDay",
    "created_at": "2013-05-15T10:05:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14588",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20numerical",
    "label_color": "0000ff",
    "label_name": "component: numerical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14588#event-189568"
}
```



---

archive/issue_events_189569.json:
```json
{
    "actor": "https://github.com/sagetrac-ManDay",
    "created_at": "2013-05-15T10:05:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14588",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14588#event-189569"
}
```



---

archive/issue_events_189570.json:
```json
{
    "actor": "https://github.com/sagetrac-ManDay",
    "created_at": "2013-05-15T10:05:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14588",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14588#event-189570"
}
```



---

archive/issue_events_189571.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:34:36Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14588",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14588#event-189571"
}
```



---

archive/issue_events_189572.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:34:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14588",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14588#event-189572"
}
```



---

archive/issue_events_189573.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14588",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14588#event-189573"
}
```



---

archive/issue_events_189574.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14588",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14588#event-189574"
}
```



---

archive/issue_events_189575.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:19:32Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14588",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14588#event-189575"
}
```



---

archive/issue_events_189576.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:19:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14588",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14588#event-189576"
}
```



---

archive/issue_events_189577.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14588",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14588#event-189577"
}
```



---

archive/issue_events_189578.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14588",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14588#event-189578"
}
```



---

archive/issue_events_189579.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-10-08T21:30:57Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14588",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14588#event-189579"
}
```



---

archive/issue_events_189580.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-10-08T21:30:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14588",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14588#event-189580"
}
```



---

archive/issue_events_189581.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-10-08T21:30:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14588",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14588#event-189581"
}
```



---

archive/issue_comments_179284.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nProposing to close as invalid.\n\nThe description does not give a minimal reproducible example.\n\nIt links to a page with complete code, which is probably lost.",
    "created_at": "2020-10-08T21:30:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14588#issuecomment-179284",
    "user": "https://github.com/slel"
}
```

<div id="comment:5" align="right">Comment 5</div>

Proposing to close as invalid.

The description does not give a minimal reproducible example.

It links to a page with complete code, which is probably lost.



---

archive/issue_comments_179285.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,5 +1,11 @@\n-The call find_local_minimum( lambda x: deviation( x,someConst ),-maxrange,maxrange ) calls the lambda with an expression of the form\n+The call\n \n-1234 == 4567\n+```\n+find_local_minimum(lambda x: deviation(x, some_const),\n+                   -maxrange, maxrange)\n+```\n \n-at least once. Complete code: http://alpha.sagenb.org/home/pub/701/\n+calls the lambda with an expression of the form\n+`1234 == 4567` at least once. Complete code:\n+\n+- http://alpha.sagenb.org/home/pub/701/\n``````\n",
    "created_at": "2020-10-08T21:30:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14588#issuecomment-179285",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,5 +1,11 @@
-The call find_local_minimum( lambda x: deviation( x,someConst ),-maxrange,maxrange ) calls the lambda with an expression of the form
+The call
 
-1234 == 4567
+```
+find_local_minimum(lambda x: deviation(x, some_const),
+                   -maxrange, maxrange)
+```
 
-at least once. Complete code: http://alpha.sagenb.org/home/pub/701/
+calls the lambda with an expression of the form
+`1234 == 4567` at least once. Complete code:
+
+- http://alpha.sagenb.org/home/pub/701/
``````




---

archive/issue_events_189582.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-10-08T21:30:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14588",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14588#event-189582"
}
```



---

archive/issue_events_189583.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2020-10-10T19:45:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14588",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14588#event-189583"
}
```



---

archive/issue_events_189584.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2020-10-10T19:45:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14588",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14588#event-189584"
}
```



---

archive/issue_events_189585.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2020-10-10T19:46:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14588",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14588#event-189585"
}
```



---

archive/issue_events_189586.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2020-10-10T19:46:00Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14588",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14588#event-189586"
}
```
