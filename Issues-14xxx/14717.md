# Issue 14717: The hash for GT pattern is broken

archive/issues_014513.json:
```json
{
    "assignees": [],
    "body": "Currently:\n\n```\nsage: GT = GelfandTsetlinPattern([[2, 1, 0], [2, 0], [1]])\nsage: hash(GT)\n12013657310631052168L\n```\nand thus\n\n```\nsage: GT in {}  \n...\nOverflowError: Python int too large to convert to C long\n```\n\nAssignee: **@sagetrac-sage-combinat**\n\nCC:  @sagetrac-chrisjamesberg @VivianePons @tscrim\n\nKeywords: **Gelfand-Tsetlin pattern, FindStatDays01**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Christian Stump**\n\nMerged: **sage-5.11.beta1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14717_\n\n",
    "closed_at": "2013-06-11T15:03:40Z",
    "created_at": "2013-06-10T20:32:02Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.11",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "The hash for GT pattern is broken",
    "type": "issue",
    "updated_at": "2013-06-12T08:22:21Z",
    "url": "https://github.com/sagemath/sage/issues/14717",
    "user": "https://github.com/stumpc5"
}
```
Currently:

```
sage: GT = GelfandTsetlinPattern([[2, 1, 0], [2, 0], [1]])
sage: hash(GT)
12013657310631052168L
```
and thus

```
sage: GT in {}  
...
OverflowError: Python int too large to convert to C long
```

Assignee: **@sagetrac-sage-combinat**

CC:  @sagetrac-chrisjamesberg @VivianePons @tscrim

Keywords: **Gelfand-Tsetlin pattern, FindStatDays01**

Reviewer: **Travis Scrimshaw**

Author: **Christian Stump**

Merged: **sage-5.11.beta1**

_Issue created by migration from https://trac.sagemath.org/ticket/14717_





---

archive/issue_events_191779.json:
```json
{
    "actor": "https://github.com/stumpc5",
    "created_at": "2013-06-10T20:32:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14717",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14717#event-191779"
}
```



---

archive/issue_events_191780.json:
```json
{
    "actor": "https://github.com/stumpc5",
    "created_at": "2013-06-10T20:32:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14717",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14717#event-191780"
}
```



---

archive/issue_events_191781.json:
```json
{
    "actor": "https://github.com/stumpc5",
    "created_at": "2013-06-10T20:32:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14717",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14717#event-191781"
}
```



---

archive/issue_events_191782.json:
```json
{
    "actor": "https://github.com/stumpc5",
    "created_at": "2013-06-10T20:32:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14717",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14717#event-191782"
}
```



---

archive/issue_comments_181876.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nAttachment: **[trac_14717-GT_pattern_hash-cs.patch.gz](https://github.com/sagemath/sage/files/ticket14717/trac_14717-GT_pattern_hash-cs.patch.gz)**",
    "created_at": "2013-06-10T20:41:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14717",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14717#issuecomment-181876",
    "user": "https://github.com/stumpc5"
}
```

<div id="comment:1" align="right">Comment 1</div>

Attachment: **[trac_14717-GT_pattern_hash-cs.patch.gz](https://github.com/sagemath/sage/files/ticket14717/trac_14717-GT_pattern_hash-cs.patch.gz)**



---

archive/issue_events_191783.json:
```json
{
    "actor": "https://github.com/stumpc5",
    "created_at": "2013-06-10T20:41:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14717",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14717#event-191783"
}
```



---

archive/issue_events_191784.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-06-10T20:45:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14717",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14717#event-191784"
}
```



---

archive/issue_events_191785.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-06-10T20:45:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14717",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14717#event-191785"
}
```



---

archive/issue_comments_181877.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nLooks good to me.",
    "created_at": "2013-06-10T20:45:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14717",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14717#issuecomment-181877",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:2" align="right">Comment 2</div>

Looks good to me.



---

archive/issue_comments_181878.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2013-06-10T20:45:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14717",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14717#issuecomment-181878",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_191786.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-11T15:03:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14717",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14717#event-191786"
}
```



---

archive/issue_events_191787.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-11T15:03:40Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14717",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14717#event-191787"
}
```



---

archive/issue_comments_181879.json:
```json
{
    "body": "Merged: **sage-5.11.beta1**",
    "created_at": "2013-06-11T15:03:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14717",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14717#issuecomment-181879",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.11.beta1**



---

archive/issue_comments_181880.json:
```json
{
    "body": "Changed keywords from **Gelfand-Tsetlin pattern** to **Gelfand-Tsetlin pattern, FindStatDays01**",
    "created_at": "2013-06-12T08:22:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14717",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14717#issuecomment-181880",
    "user": "https://github.com/stumpc5"
}
```

Changed keywords from **Gelfand-Tsetlin pattern** to **Gelfand-Tsetlin pattern, FindStatDays01**
