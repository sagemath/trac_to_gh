# Issue 14182: Fix whitespace in coercion_and_categories.rst

archive/issues_013978.json:
```json
{
    "assignees": [],
    "body": "`devel/sage/doc/en/thematic_tutorials/coercion_and_categories.rst` mixes TABs and spaces for indentation, which is very bad style and might also lead to doctesting trouble.\n\nDepends on #11490\n\nAssignee: **@sagetrac-mvngu**\n\nCC:  @simon-king-jena\n\nReviewer: **Simon King**\n\nAuthor: **Jeroen Demeyer**\n\nMerged: **sage-5.8.beta2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14182_\n\n",
    "closed_at": "2013-02-28T10:35:25Z",
    "created_at": "2013-02-25T21:06:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix whitespace in coercion_and_categories.rst",
    "type": "issue",
    "updated_at": "2013-02-28T10:35:25Z",
    "url": "https://github.com/sagemath/sage/issues/14182",
    "user": "https://github.com/jdemeyer"
}
```
`devel/sage/doc/en/thematic_tutorials/coercion_and_categories.rst` mixes TABs and spaces for indentation, which is very bad style and might also lead to doctesting trouble.

Depends on #11490

Assignee: **@sagetrac-mvngu**

CC:  @simon-king-jena

Reviewer: **Simon King**

Author: **Jeroen Demeyer**

Merged: **sage-5.8.beta2**

_Issue created by migration from https://trac.sagemath.org/ticket/14182_





---

archive/issue_events_183156.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-25T21:06:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14182",
    "milestone_number": null,
    "milestone_title": "sage-5.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14182#event-183156"
}
```



---

archive/issue_events_183157.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-25T21:06:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14182",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14182#event-183157"
}
```



---

archive/issue_events_183158.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-25T21:06:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14182",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14182#event-183158"
}
```



---

archive/issue_events_183159.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-25T21:06:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14182",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14182#event-183159"
}
```



---

archive/issue_comments_171608.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nHow to replace tabs by four blank spaces (in linux or vim)? Perhaps you can answer faster than I can look up in google...",
    "created_at": "2013-02-25T21:13:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14182",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14182#issuecomment-171608",
    "user": "https://github.com/simon-king-jena"
}
```

<div id="comment:1" align="right">Comment 1</div>

How to replace tabs by four blank spaces (in linux or vim)? Perhaps you can answer faster than I can look up in google...



---

archive/issue_comments_171609.json:
```json
{
    "body": "Attachment: **[14182_whitespace.patch.gz](https://github.com/sagemath/sage/files/ticket14182/14182_whitespace.patch.gz)**",
    "created_at": "2013-02-25T21:15:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14182",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14182#issuecomment-171609",
    "user": "https://github.com/jdemeyer"
}
```

Attachment: **[14182_whitespace.patch.gz](https://github.com/sagemath/sage/files/ticket14182/14182_whitespace.patch.gz)**



---

archive/issue_events_183160.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-25T21:15:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14182",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14182#event-183160"
}
```



---

archive/issue_comments_171610.json:
```json
{
    "body": "Author: **Jeroen Demeyer**",
    "created_at": "2013-02-25T21:15:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14182",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14182#issuecomment-171610",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Jeroen Demeyer**



---

archive/issue_comments_171611.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nOK, you were faster. I found `sed 's/\\\\t/    /g' oldcode.py >newcode.py` in google...",
    "created_at": "2013-02-25T21:22:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14182",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14182#issuecomment-171611",
    "user": "https://github.com/simon-king-jena"
}
```

<div id="comment:3" align="right">Comment 3</div>

OK, you were faster. I found `sed 's/\\t/    /g' oldcode.py >newcode.py` in google...



---

archive/issue_comments_171612.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nGood. After applying the patch, I could not find any tab space in the code. Since the patch does not change the actual text, since the documentation built w/o trouble, and since the result looks fine in the browser, I give it a positive review.",
    "created_at": "2013-02-25T21:35:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14182",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14182#issuecomment-171612",
    "user": "https://github.com/simon-king-jena"
}
```

<div id="comment:4" align="right">Comment 4</div>

Good. After applying the patch, I could not find any tab space in the code. Since the patch does not change the actual text, since the documentation built w/o trouble, and since the result looks fine in the browser, I give it a positive review.



---

archive/issue_comments_171613.json:
```json
{
    "body": "Reviewer: **Simon King**",
    "created_at": "2013-02-25T21:35:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14182",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14182#issuecomment-171613",
    "user": "https://github.com/simon-king-jena"
}
```

Reviewer: **Simon King**



---

archive/issue_events_183161.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2013-02-25T21:35:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14182",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14182#event-183161"
}
```



---

archive/issue_events_183162.json:
```json
{
    "actor": "https://github.com/simon-king-jena",
    "created_at": "2013-02-25T21:35:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14182",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14182#event-183162"
}
```



---

archive/issue_comments_171614.json:
```json
{
    "body": "Dependencies: **#11490**",
    "created_at": "2013-02-27T22:19:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14182",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14182#issuecomment-171614",
    "user": "https://github.com/roed314"
}
```

Dependencies: **#11490**



---

archive/issue_comments_171615.json:
```json
{
    "body": "Merged: **sage-5.8.beta2**",
    "created_at": "2013-02-28T10:35:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14182",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14182#issuecomment-171615",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.8.beta2**



---

archive/issue_events_183163.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-28T10:35:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14182",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14182#event-183163"
}
```



---

archive/issue_events_183164.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-28T10:35:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14182",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14182#event-183164"
}
```
