# Issue 14674: cleanup of decorators.py

archive/issues_014470.json:
```json
{
    "assignees": [],
    "body": "cleans the code and doc in misc/decorators.py\n\n* remove unsused import found by pyflakes\n* add links to trac\n* make code closer to pep8 standard\n* remove trailing whitespaces\n\nDepends on #14524\n\nAssignee: **@jasongrout**\n\nKeywords: **documentation**\n\nReviewer: **Volker Braun**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\nMerged: **sage-5.11.beta1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14674_\n\n",
    "closed_at": "2013-06-10T08:42:30Z",
    "created_at": "2013-05-31T20:37:32Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.11",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "cleanup of decorators.py",
    "type": "issue",
    "updated_at": "2013-06-10T08:42:30Z",
    "url": "https://github.com/sagemath/sage/issues/14674",
    "user": "https://github.com/fchapoton"
}
```
cleans the code and doc in misc/decorators.py

* remove unsused import found by pyflakes
* add links to trac
* make code closer to pep8 standard
* remove trailing whitespaces

Depends on #14524

Assignee: **@jasongrout**

Keywords: **documentation**

Reviewer: **Volker Braun**

Author: **Frédéric Chapoton**

Merged: **sage-5.11.beta1**

_Issue created by migration from https://trac.sagemath.org/ticket/14674_





---

archive/issue_events_190955.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-31T20:37:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14674",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14674#event-190955"
}
```



---

archive/issue_events_190956.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-31T20:37:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14674",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14674#event-190956"
}
```



---

archive/issue_events_190957.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-31T20:37:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14674",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14674#event-190957"
}
```



---

archive/issue_events_190958.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-31T20:41:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14674",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14674#event-190958"
}
```



---

archive/issue_comments_180718.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nLooks good to me, but patch conflicts with #14524. Can you rebase?",
    "created_at": "2013-06-02T20:25:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14674",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14674#issuecomment-180718",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:2'>Comment 2:</a>
Looks good to me, but patch conflicts with #14524. Can you rebase?



---

archive/issue_comments_180719.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nAttachment: **[trac_14674_cleanup_decorator_fc.patch.gz](https://github.com/sagemath/sage/files/ticket14674/trac_14674_cleanup_decorator_fc.patch.gz)**\n\nrebased on top of #14524.",
    "created_at": "2013-06-03T18:37:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14674",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14674#issuecomment-180719",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:3'>Comment 3:</a>
Attachment: **[trac_14674_cleanup_decorator_fc.patch.gz](https://github.com/sagemath/sage/files/ticket14674/trac_14674_cleanup_decorator_fc.patch.gz)**

rebased on top of #14524.



---

archive/issue_comments_180720.json:
```json
{
    "body": "Dependencies: **#14524**",
    "created_at": "2013-06-03T18:37:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14674",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14674#issuecomment-180720",
    "user": "https://github.com/fchapoton"
}
```

Dependencies: **#14524**



---

archive/issue_comments_180721.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nThanks, looks good to me",
    "created_at": "2013-06-03T20:32:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14674",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14674#issuecomment-180721",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:4'>Comment 4:</a>
Thanks, looks good to me



---

archive/issue_comments_180722.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2013-06-03T20:32:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14674",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14674#issuecomment-180722",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_events_190959.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-06-03T20:32:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14674",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14674#event-190959"
}
```



---

archive/issue_events_190960.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-06-03T20:32:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14674",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14674#event-190960"
}
```



---

archive/issue_comments_180723.json:
```json
{
    "body": "Merged: **sage-5.11.beta1**",
    "created_at": "2013-06-10T08:42:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14674",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14674#issuecomment-180723",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.11.beta1**



---

archive/issue_events_190961.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-10T08:42:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14674",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14674#event-190961"
}
```



---

archive/issue_events_190962.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-10T08:42:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14674",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14674#event-190962"
}
```
