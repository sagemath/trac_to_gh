...
```json
{
    "body": "Attachment [trac_14640_refactor_plot_expose.patch](tarball://root/attachments/some-uuid/ticket14640/trac_14640_refactor_plot_expose.patch) by @vbraun created at 2013-05-25 11:26:28",
...
```json
{
    "body": "The patch looks good to me, but how about a more specific (descriptive :P) name such as `text_description()`?\n\nBest,\n\nTravis",
...
```json
{
    "body": "Active is better than the passive verb from. And of course its text, did you expect that it is going to read it out to your through the speaker? ;-)",
...
```json
{
    "body": "That would be awesome if it did. I'm just thinking when I see this on tab completion that it is somewhat vague (but that's what the doc is for). Anyhow, I can't think of a better name, so I'm setting this to positive review. Nicolas, if you have any issues with the patch, feel free to set this back.",
    "created_at": "2013-05-25T20:08:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14640",
    "type": "issue_comment",
...
```json
{
    "body": "And this method doesn't return anything, it just prints to the screen.",
...
```json
{
    "body": "Attachment [trac_14640-refactor_plot_expose-review-ts.patch](tarball://root/attachments/some-uuid/ticket14640/trac_14640-refactor_plot_expose-review-ts.patch) by @tscrim created at 2013-05-28 05:07:02\n\nI've uploaded a review patch which changes `describe()` which prints to `description()` which returns a string as per Nicolas' request. Needs a review.",
...
```json
{
    "body": "Shouldn't the `plot_expose` function have been deprecated along with this change?",
...
```json
{
    "body": "Was never imported into the global namespace, so it does not need deprecation.",
...
```json
{
    "body": "Oh. I missed that. Seems combinat specific, but it is good to see it in the plot code.",
...
```json
{
    "body": "Actually, this is quite a useful feature. Perhaps it should be documented somewhere in the plot code. Probably in the Graphics class of sage/plot/graphics.py. Need not be done in this ticket.",
