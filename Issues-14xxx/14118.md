# Issue 14118: Compiling universal_cyclotomic_field_c.pyx  fails on Cygwin

archive/issues_013914.json:
```json
{
    "assignees": [],
    "body": "We need to explicitely link to gmp in module_list.py\n\n__APPLY__\n* [attachment: ucf.patch](https://github.com/sagemath/sage/files/ticket14118/ucf.patch.gz)\n\nDepends on #8327\n\nCC:  @kcrisman @dimpase\n\nKeywords: **cygwin gmp**\n\nReviewer: **Dmitrii Pasechnik**\n\nAuthor: **Jean-Pierre Flori**\n\nMerged: **sage-5.8.beta1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14118_\n\n",
    "closed_at": "2013-02-22T19:13:15Z",
    "created_at": "2013-02-14T14:44:25Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Compiling universal_cyclotomic_field_c.pyx  fails on Cygwin",
    "type": "issue",
    "updated_at": "2013-02-22T19:13:15Z",
    "url": "https://github.com/sagemath/sage/issues/14118",
    "user": "https://github.com/sagetrac-jpflori"
}
```
We need to explicitely link to gmp in module_list.py

__APPLY__
* [attachment: ucf.patch](https://github.com/sagemath/sage/files/ticket14118/ucf.patch.gz)

Depends on #8327

CC:  @kcrisman @dimpase

Keywords: **cygwin gmp**

Reviewer: **Dmitrii Pasechnik**

Author: **Jean-Pierre Flori**

Merged: **sage-5.8.beta1**

_Issue created by migration from https://trac.sagemath.org/ticket/14118_





---

archive/issue_events_182200.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2013-02-14T14:44:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14118",
    "milestone_number": null,
    "milestone_title": "sage-5.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14118#event-182200"
}
```



---

archive/issue_events_182201.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2013-02-14T14:44:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14118",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
    "label_color": "000080",
    "label_name": "component: porting: cygwin",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14118#event-182201"
}
```



---

archive/issue_events_182202.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2013-02-14T14:44:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14118",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14118#event-182202"
}
```



---

archive/issue_events_182203.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2013-02-14T14:44:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14118",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14118#event-182203"
}
```



---

archive/issue_comments_170511.json:
```json
{
    "body": "Attachment: **[ucf.patch.gz](https://github.com/sagemath/sage/files/ticket14118/ucf.patch.gz)**",
    "created_at": "2013-02-14T14:45:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14118",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14118#issuecomment-170511",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Attachment: **[ucf.patch.gz](https://github.com/sagemath/sage/files/ticket14118/ucf.patch.gz)**



---

archive/issue_events_182204.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2013-02-14T14:46:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14118",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14118#event-182204"
}
```



---

archive/issue_comments_170512.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,4 @@\n We need to explicitely link to gmp in module_list.py\n+\n+__APPLY__\n+* [attachment: ucf.patch](https://github.com/sagemath/sage/files/ticket14118/ucf.patch.gz)\n``````\n",
    "created_at": "2013-02-14T14:46:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14118",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14118#issuecomment-170512",
    "user": "https://github.com/sagetrac-jpflori"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,4 @@
 We need to explicitely link to gmp in module_list.py
+
+__APPLY__
+* [attachment: ucf.patch](https://github.com/sagemath/sage/files/ticket14118/ucf.patch.gz)
``````




---

archive/issue_comments_170513.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nhmm, does it depend on some trac ticket? Is it something new?",
    "created_at": "2013-02-14T14:51:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14118",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14118#issuecomment-170513",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:2" align="right">Comment 2</div>

hmm, does it depend on some trac ticket? Is it something new?



---

archive/issue_comments_170514.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nYup, #8327 - 5.7.beta3.  It's always *something*!",
    "created_at": "2013-02-15T03:25:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14118",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14118#issuecomment-170514",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:3" align="right">Comment 3</div>

Yup, #8327 - 5.7.beta3.  It's always *something*!



---

archive/issue_comments_170515.json:
```json
{
    "body": "Dependencies: **#8327**",
    "created_at": "2013-02-15T13:59:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14118",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14118#issuecomment-170515",
    "user": "https://github.com/dimpase"
}
```

Dependencies: **#8327**



---

archive/issue_events_182205.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2013-02-15T14:44:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14118",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14118#event-182205"
}
```



---

archive/issue_events_182206.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2013-02-15T14:44:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14118",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14118#event-182206"
}
```



---

archive/issue_comments_170516.json:
```json
{
    "body": "Reviewer: **Dmitrii Pasechnik**",
    "created_at": "2013-02-19T21:52:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14118",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14118#issuecomment-170516",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Dmitrii Pasechnik**



---

archive/issue_events_182207.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-22T19:13:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14118",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14118#event-182207"
}
```



---

archive/issue_events_182208.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-22T19:13:15Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14118",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14118#event-182208"
}
```



---

archive/issue_comments_170517.json:
```json
{
    "body": "Merged: **sage-5.8.beta1**",
    "created_at": "2013-02-22T19:13:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14118",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14118#issuecomment-170517",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.8.beta1**
