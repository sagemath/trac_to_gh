# Issue 14697: System-wide GCC picks Sage's libraries on Cygwin

archive/issues_014493.json:
```json
{
    "assignees": [],
    "body": "On Cygwin, libraries are stored in \"bin\" together with executables.\nAs we add Sage's bin directory to PATH (to be able to use the executables we generated!), the system-wide GCC may pick up some Sage's libraries (e.g. gmp, mpfr or mpc) rather than the system-wide ones it is supposed to if versions are identical.\nUnfortunately, this may raise incompatibilities: I experienced a bunch of segfaults when building different spkgs.\n\nCC:  @nexttime @kcrisman @dimpase\n\nKeywords: **cygwin gcc path**\n\nReviewer: **Jean-Pierre Flori**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14697_\n\n",
    "closed_at": "2014-11-28T18:39:46Z",
    "created_at": "2013-06-06T13:44:21Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "System-wide GCC picks Sage's libraries on Cygwin",
    "type": "issue",
    "updated_at": "2014-11-28T18:39:46Z",
    "url": "https://github.com/sagemath/sage/issues/14697",
    "user": "https://github.com/jpflori"
}
```
On Cygwin, libraries are stored in "bin" together with executables.
As we add Sage's bin directory to PATH (to be able to use the executables we generated!), the system-wide GCC may pick up some Sage's libraries (e.g. gmp, mpfr or mpc) rather than the system-wide ones it is supposed to if versions are identical.
Unfortunately, this may raise incompatibilities: I experienced a bunch of segfaults when building different spkgs.

CC:  @nexttime @kcrisman @dimpase

Keywords: **cygwin gcc path**

Reviewer: **Jean-Pierre Flori**

_Issue created by migration from https://trac.sagemath.org/ticket/14697_





---

archive/issue_events_191384.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-06T13:44:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-191384"
}
```



---

archive/issue_events_191385.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-06T13:44:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting%3A%20cygwin",
    "label_color": "000080",
    "label_name": "component: porting: cygwin",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-191385"
}
```



---

archive/issue_events_191386.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-06T13:44:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-191386"
}
```



---

archive/issue_events_191387.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-06-06T13:44:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-191387"
}
```



---

archive/issue_events_191388.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-191388"
}
```



---

archive/issue_events_191389.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-191389"
}
```



---

archive/issue_events_191390.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-191390"
}
```



---

archive/issue_events_191391.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-191391"
}
```



---

archive/issue_events_191392.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-191392"
}
```



---

archive/issue_events_191393.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-191393"
}
```



---

archive/issue_events_191394.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-191394"
}
```



---

archive/issue_events_191395.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-191395"
}
```



---

archive/issue_events_191396.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2014-11-25T14:37:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-191396"
}
```



---

archive/issue_comments_181058.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nFixed by #10572.",
    "created_at": "2014-11-25T14:37:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14697#issuecomment-181058",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:5" align="right">Comment 5</div>

Fixed by #10572.



---

archive/issue_events_191397.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2014-11-25T14:37:00Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-191397"
}
```



---

archive/issue_events_191398.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2014-11-25T14:37:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-191398"
}
```



---

archive/issue_events_191399.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2014-11-25T14:37:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-191399"
}
```



---

archive/issue_events_191400.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2014-11-25T14:37:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-191400"
}
```



---

archive/issue_events_191401.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2014-11-25T14:37:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-191401"
}
```



---

archive/issue_comments_181059.json:
```json
{
    "body": "Reviewer: **Jean-Pierre Flori**",
    "created_at": "2014-11-25T14:38:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14697#issuecomment-181059",
    "user": "https://github.com/jpflori"
}
```

Reviewer: **Jean-Pierre Flori**



---

archive/issue_events_191402.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-11-28T18:39:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-191402"
}
```



---

archive/issue_events_191403.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-11-28T18:39:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14697",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14697#event-191403"
}
```
