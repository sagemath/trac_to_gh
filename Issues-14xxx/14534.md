# Issue 14534: Composition of ring homomorphisms

archive/issues_014330.json:
```json
{
    "assignees": [],
    "body": "When composing a `RingHomomorphism` and an instance of `RingHomomorphism_im_gens` then\nthe images of the generators are again checked for their validity.\n\nThis patch avoids this unnecessary test.\n\nWith the patch\n\n```\nsage: F.<a> = GF(3^100, 'a')\nsage: f= F.hom([a^(3^5)])\nsage: %timeit f^100\n1 loops, best of 3: 290 ms per loop\n```\n\nbefore:\n\n```\n1 loops, best of 3: 1.73 s per loop\n```\n\n\nReviewer: **Francis Clarke**\n\nAuthor: **Thomas Feulner**\n\nMerged: **sage-5.10.beta2**\n\nComponent: **performance**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14534_\n\n",
    "closed_at": "2013-05-08T07:45:04Z",
    "created_at": "2013-05-06T07:52:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/performance",
        "https://github.com/sagemath/sage/labels/p%3A%205%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.10",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Composition of ring homomorphisms",
    "type": "issue",
    "updated_at": "2013-05-08T07:45:04Z",
    "url": "https://github.com/sagemath/sage/issues/14534",
    "user": "https://github.com/sagetrac-tfeulner"
}
```
When composing a `RingHomomorphism` and an instance of `RingHomomorphism_im_gens` then
the images of the generators are again checked for their validity.

This patch avoids this unnecessary test.

With the patch

```
sage: F.<a> = GF(3^100, 'a')
sage: f= F.hom([a^(3^5)])
sage: %timeit f^100
1 loops, best of 3: 290 ms per loop
```

before:

```
1 loops, best of 3: 1.73 s per loop
```


Reviewer: **Francis Clarke**

Author: **Thomas Feulner**

Merged: **sage-5.10.beta2**

Component: **performance**

_Issue created by migration from https://trac.sagemath.org/ticket/14534_





---

archive/issue_events_205735.json:
```json
{
    "actor": "https://github.com/sagetrac-tfeulner",
    "created_at": "2013-05-06T07:52:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14534",
    "milestone_number": null,
    "milestone_title": "sage-5.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14534#event-205735"
}
```



---

archive/issue_events_205736.json:
```json
{
    "actor": "https://github.com/sagetrac-tfeulner",
    "created_at": "2013-05-06T07:52:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14534",
    "label": "https://github.com/sagemath/sage/labels/performance",
    "label_color": "696969",
    "label_name": "performance",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14534#event-205736"
}
```



---

archive/issue_events_205737.json:
```json
{
    "actor": "https://github.com/sagetrac-tfeulner",
    "created_at": "2013-05-06T07:52:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14534",
    "label": "https://github.com/sagemath/sage/labels/p%3A%205%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p: 5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14534#event-205737"
}
```



---

archive/issue_events_205738.json:
```json
{
    "actor": "https://github.com/sagetrac-tfeulner",
    "created_at": "2013-05-06T07:52:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14534",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14534#event-205738"
}
```



---

archive/issue_comments_176046.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nAttachment: **[trac_14534_ring_morphism_composition.patch.gz](https://github.com/sagemath/sage/files/ticket14534/trac_14534_ring_morphism_composition.patch.gz)**",
    "created_at": "2013-05-06T10:31:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14534#issuecomment-176046",
    "user": "https://github.com/sagetrac-fwclarke"
}
```

<div id="comment:1" align="right">comment:1</div>

Attachment: **[trac_14534_ring_morphism_composition.patch.gz](https://github.com/sagemath/sage/files/ticket14534/trac_14534_ring_morphism_composition.patch.gz)**



---

archive/issue_events_205739.json:
```json
{
    "actor": "https://github.com/sagetrac-fwclarke",
    "created_at": "2013-05-06T10:31:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14534",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14534#event-205739"
}
```



---

archive/issue_events_205740.json:
```json
{
    "actor": "https://github.com/sagetrac-fwclarke",
    "created_at": "2013-05-06T10:32:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14534",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14534#event-205740"
}
```



---

archive/issue_events_205741.json:
```json
{
    "actor": "https://github.com/sagetrac-fwclarke",
    "created_at": "2013-05-06T10:32:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14534",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14534#event-205741"
}
```



---

archive/issue_comments_176047.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nWorks perfectly.  Positive review.",
    "created_at": "2013-05-06T10:32:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14534#issuecomment-176047",
    "user": "https://github.com/sagetrac-fwclarke"
}
```

<div id="comment:2" align="right">comment:2</div>

Works perfectly.  Positive review.



---

archive/issue_comments_176048.json:
```json
{
    "body": "Reviewer: **Francis Clarke**",
    "created_at": "2013-05-06T10:32:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14534#issuecomment-176048",
    "user": "https://github.com/sagetrac-fwclarke"
}
```

Reviewer: **Francis Clarke**



---

archive/issue_events_205742.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-08T07:45:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14534",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14534#event-205742"
}
```



---

archive/issue_events_205743.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-08T07:45:04Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14534",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14534#event-205743"
}
```



---

archive/issue_comments_176049.json:
```json
{
    "body": "Merged: **sage-5.10.beta2**",
    "created_at": "2013-05-08T07:45:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14534",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14534#issuecomment-176049",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.10.beta2**
