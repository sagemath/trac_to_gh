# Issue 14213: Maxima error on improper integral

archive/issues_014009.json:
```json
{
    "assignees": [],
    "body": "See [Maxima bug 2556](https://sourceforge.net/p/maxima/bugs/2556/) and [this ask.sagemath.org question](http://ask.sagemath.org/question/2314/illegal-kernel-in-adjoin-pvar).\n\n```\nvar('A, x, delta, k, dx, t, v, xi')\ntemp = 1/2*A*e^(1/2*(xi - 1)*dx*k^2*t*v - 1/4*delta^2*k^2 - I*k*t*v + I*k*x)/(sqrt(pi)*sqrt(delta^(-2)))\nintegral(temp,(k,0,oo))\n```\nkills Maxima\n\n```\nCondition of type: SIMPLE-CONDITION\nIllegal kernel in `adjoin-pvar'\nAvailable restarts:\n\n1. (CONTINUE) Return from BREAK.\n\nTop level.\n>\n```\n\nAssignee: **@burcin**\n\nUpstream: **Reported upstream. No feedback yet.**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14213_\n\n",
    "created_at": "2013-03-02T02:44:16Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20calculus",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Maxima error on improper integral",
    "type": "issue",
    "updated_at": "2022-12-29T01:28:25Z",
    "url": "https://github.com/sagemath/sage/issues/14213",
    "user": "https://github.com/kcrisman"
}
```
See [Maxima bug 2556](https://sourceforge.net/p/maxima/bugs/2556/) and [this ask.sagemath.org question](http://ask.sagemath.org/question/2314/illegal-kernel-in-adjoin-pvar).

```
var('A, x, delta, k, dx, t, v, xi')
temp = 1/2*A*e^(1/2*(xi - 1)*dx*k^2*t*v - 1/4*delta^2*k^2 - I*k*t*v + I*k*x)/(sqrt(pi)*sqrt(delta^(-2)))
integral(temp,(k,0,oo))
```
kills Maxima

```
Condition of type: SIMPLE-CONDITION
Illegal kernel in `adjoin-pvar'
Available restarts:

1. (CONTINUE) Return from BREAK.

Top level.
>
```

Assignee: **@burcin**

Upstream: **Reported upstream. No feedback yet.**

_Issue created by migration from https://trac.sagemath.org/ticket/14213_





---

archive/issue_events_183636.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-03-02T02:44:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14213",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14213#event-183636"
}
```



---

archive/issue_events_183637.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-03-02T02:44:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14213",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20calculus",
    "label_color": "0000ff",
    "label_name": "component: calculus",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14213#event-183637"
}
```



---

archive/issue_events_183638.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-03-02T02:44:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14213",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14213#event-183638"
}
```



---

archive/issue_events_183639.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-03-02T02:44:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14213",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14213#event-183639"
}
```



---

archive/issue_comments_172053.json:
```json
{
    "body": "Upstream: **Reported upstream. No feedback yet.**",
    "created_at": "2013-03-15T18:54:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14213",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14213#issuecomment-172053",
    "user": "https://github.com/kcrisman"
}
```

Upstream: **Reported upstream. No feedback yet.**



---

archive/issue_events_183640.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14213",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14213#event-183640"
}
```



---

archive/issue_events_183641.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14213",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14213#event-183641"
}
```



---

archive/issue_events_183642.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14213",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14213#event-183642"
}
```



---

archive/issue_events_183643.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14213",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14213#event-183643"
}
```



---

archive/issue_events_183644.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14213",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14213#event-183644"
}
```



---

archive/issue_events_183645.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14213",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14213#event-183645"
}
```



---

archive/issue_events_183646.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14213",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14213#event-183646"
}
```



---

archive/issue_events_183647.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14213",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14213#event-183647"
}
```



---

archive/issue_events_183648.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:28:25Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14213",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14213#event-183648"
}
```
