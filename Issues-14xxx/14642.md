# Issue 14642: Remove Cremona functions from the global namespace

archive/issues_014438.json:
```json
{
    "assignees": [],
    "body": "`cremona_letter_code`, `parse_cremona_label`, `old_cremona_letter_code`, `is_optimal_id` have been deprecated for over two years by #10107. They should now be removed.\n\nAssignee: **@JohnCremona**\n\nReviewer: **John Cremona**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\nMerged: **sage-5.12.beta2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14642_\n\n",
    "closed_at": "2013-08-16T21:22:38Z",
    "created_at": "2013-05-25T21:37:24Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20elliptic%20curves",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.12",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove Cremona functions from the global namespace",
    "type": "issue",
    "updated_at": "2013-08-16T21:22:38Z",
    "url": "https://github.com/sagemath/sage/issues/14642",
    "user": "https://github.com/eviatarbach"
}
```
`cremona_letter_code`, `parse_cremona_label`, `old_cremona_letter_code`, `is_optimal_id` have been deprecated for over two years by #10107. They should now be removed.

Assignee: **@JohnCremona**

Reviewer: **John Cremona**

Author: **Frédéric Chapoton**

Merged: **sage-5.12.beta2**

_Issue created by migration from https://trac.sagemath.org/ticket/14642_





---

archive/issue_events_190407.json:
```json
{
    "actor": "https://github.com/eviatarbach",
    "created_at": "2013-05-25T21:37:24Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14642",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14642#event-190407"
}
```



---

archive/issue_events_190408.json:
```json
{
    "actor": "https://github.com/eviatarbach",
    "created_at": "2013-05-25T21:37:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14642",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20elliptic%20curves",
    "label_color": "0000ff",
    "label_name": "component: elliptic curves",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14642#event-190408"
}
```



---

archive/issue_events_190409.json:
```json
{
    "actor": "https://github.com/eviatarbach",
    "created_at": "2013-05-25T21:37:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14642",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14642#event-190409"
}
```



---

archive/issue_events_190410.json:
```json
{
    "actor": "https://github.com/eviatarbach",
    "created_at": "2013-05-25T21:37:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14642",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14642#event-190410"
}
```



---

archive/issue_comments_180179.json:
```json
{
    "body": "Attachment: **[trac_14642.patch.gz](https://github.com/sagemath/sage/files/ticket14642/trac_14642.patch.gz)**",
    "created_at": "2013-07-24T13:46:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14642#issuecomment-180179",
    "user": "https://github.com/fchapoton"
}
```

Attachment: **[trac_14642.patch.gz](https://github.com/sagemath/sage/files/ticket14642/trac_14642.patch.gz)**



---

archive/issue_comments_180180.json:
```json
{
    "body": "Author: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2013-07-24T13:46:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14642#issuecomment-180180",
    "user": "https://github.com/fchapoton"
}
```

Author: **Frédéric Chapoton**



---

archive/issue_comments_180181.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nhere is a patch, needs review",
    "created_at": "2013-07-24T13:46:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14642#issuecomment-180181",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:1'>Comment 1:</a>
here is a patch, needs review



---

archive/issue_events_190411.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-07-24T13:46:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14642",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14642#event-190411"
}
```



---

archive/issue_comments_180182.json:
```json
{
    "body": "Reviewer: **John Cremona**",
    "created_at": "2013-07-24T14:04:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14642#issuecomment-180182",
    "user": "https://github.com/JohnCremona"
}
```

Reviewer: **John Cremona**



---

archive/issue_events_190412.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2013-07-24T14:04:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14642",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14642#event-190412"
}
```



---

archive/issue_events_190413.json:
```json
{
    "actor": "https://github.com/JohnCremona",
    "created_at": "2013-07-24T14:04:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14642",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14642#event-190413"
}
```



---

archive/issue_comments_180183.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nLooks fine to me.",
    "created_at": "2013-07-24T14:04:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14642#issuecomment-180183",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:2'>Comment 2:</a>
Looks fine to me.



---

archive/issue_events_190414.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-07-25T06:50:07Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14642",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14642#event-190414"
}
```



---

archive/issue_events_190415.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-07-25T06:50:07Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14642",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14642#event-190415"
}
```



---

archive/issue_comments_180184.json:
```json
{
    "body": "Merged: **sage-5.12.beta2**",
    "created_at": "2013-08-16T21:22:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14642",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14642#issuecomment-180184",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.12.beta2**



---

archive/issue_events_190416.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-16T21:22:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14642",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14642#event-190416"
}
```



---

archive/issue_events_190417.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-16T21:22:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14642",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14642#event-190417"
}
```
