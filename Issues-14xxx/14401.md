# Issue 14401: Arithmetic functions should be methods of Sage integers

archive/issues_014197.json:
```json
{
    "body": "One common use model of Sage is to use tab completion to identify methods of a given object. For Sage integers, this will currently fail to reveal many standard functions such as the following.\n\n```\nnumber_of_divisors # d(n)\nmoebius            # mu(n)\neuler_phi          # phi(n)\npartitions         # tau(n)\n```\nI propose to add methods to the class of Sage integers. In the process, perhaps one can add a few standard functions which are missing even from the class of bare functions, like sum_of_divisors (\"sigma(n)\").\n\nAssignee: @williamstein\n\nCC:  @kcrisman\n\nKeywords: arithmetic functions\n\nIssue created by migration from https://trac.sagemath.org/ticket/14401\n\n",
    "created_at": "2013-04-02T21:09:19Z",
    "labels": [
        "component: number theory",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-6.4",
    "title": "Arithmetic functions should be methods of Sage integers",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/14401",
    "user": "https://github.com/kedlaya"
}
```
One common use model of Sage is to use tab completion to identify methods of a given object. For Sage integers, this will currently fail to reveal many standard functions such as the following.

```
number_of_divisors # d(n)
moebius            # mu(n)
euler_phi          # phi(n)
partitions         # tau(n)
```
I propose to add methods to the class of Sage integers. In the process, perhaps one can add a few standard functions which are missing even from the class of bare functions, like sum_of_divisors ("sigma(n)").

Assignee: @williamstein

CC:  @kcrisman

Keywords: arithmetic functions

Issue created by migration from https://trac.sagemath.org/ticket/14401





---

archive/issue_events_040885.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14401",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14401#event-40885"
}
```



---

archive/issue_comments_178171.json:
```json
{
    "body": "<a id='comment:3'></a>I doubt the general usefulness of such an undertaking. The notion that functions that take an object as parameter should be equivalent to member functions of that object is too broad.",
    "created_at": "2014-01-28T20:38:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14401",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14401#issuecomment-178171",
    "user": "https://github.com/rwst"
}
```

<a id='comment:3'></a>I doubt the general usefulness of such an undertaking. The notion that functions that take an object as parameter should be equivalent to member functions of that object is too broad.



---

archive/issue_events_040886.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14401",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14401#event-40886"
}
```



---

archive/issue_events_040887.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14401",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14401#event-40887"
}
```



---

archive/issue_events_040888.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14401",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14401#event-40888"
}
```



---

archive/issue_events_040889.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14401",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14401#event-40889"
}
```



---

archive/issue_events_040890.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14401",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14401#event-40890"
}
```



---

archive/issue_events_040891.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14401",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14401#event-40891"
}
```



---

archive/issue_comments_178172.json:
```json
{
    "body": "<a id='comment:7'></a>I agree that the notion is too broad, but not in the way you have in mind: in Python, one generally tries to avoid clogging up the global namespace with things that really should be member functions of a particular class. So if I were starting from scratch, I would strongly consider implementing these functions exclusively as member functions of Sage integers; but I don't think it's appropriate to move in that direction now.\n\nIn any case, this case is much more special than the general discussion, because arithmetic functions are distinguished by the property of multiplicativity: if `gcd(m,n) == 1` then `f(m*n) = f(m)*f(n)`. So in a sense, they are more deeply linked to the class of integers than, say, the binomial or factorial functions.",
    "created_at": "2016-03-21T20:57:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14401",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14401#issuecomment-178172",
    "user": "https://github.com/kedlaya"
}
```

<a id='comment:7'></a>I agree that the notion is too broad, but not in the way you have in mind: in Python, one generally tries to avoid clogging up the global namespace with things that really should be member functions of a particular class. So if I were starting from scratch, I would strongly consider implementing these functions exclusively as member functions of Sage integers; but I don't think it's appropriate to move in that direction now.

In any case, this case is much more special than the general discussion, because arithmetic functions are distinguished by the property of multiplicativity: if `gcd(m,n) == 1` then `f(m*n) = f(m)*f(n)`. So in a sense, they are more deeply linked to the class of integers than, say, the binomial or factorial functions.



---

archive/issue_comments_178173.json:
```json
{
    "body": "<a id='comment:8'></a>Given that this discussion has gone dormant, I propose to mark this ticket as wontfix and close it.",
    "created_at": "2020-08-16T03:19:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14401",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14401#issuecomment-178173",
    "user": "https://github.com/kedlaya"
}
```

<a id='comment:8'></a>Given that this discussion has gone dormant, I propose to mark this ticket as wontfix and close it.
