# Issue 14320: More doctests from the book "Calcul mathématique avec Sage"

archive/issues_014116.json:
```json
{
    "assignees": [],
    "body": "The attached file provides doctests for examples in the chapter \"float\" (floating point numbers). Tests pass with 5.11.\n\nApply only [attachment: trac_14320_doctests_from_french_book.patch\u200b](https://github.com/sagemath/sage/files/ticket14320/d8bbf892d87f41af5906814f40962c1c.gz).\n\nAssignee: **@sagetrac-mvngu**\n\nKeywords: **french book**\n\nUpstream: **Completely fixed; Fix reported upstream**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Thierry Dumont, Paul Zimmermann**\n\nMerged: **sage-5.13.beta3**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14320_\n\n",
    "closed_at": "2013-11-06T12:49:20Z",
    "created_at": "2013-03-20T16:33:00Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/doctest%20coverage",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.13",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "More doctests from the book \"Calcul math\u00e9matique avec Sage\"",
    "type": "issue",
    "updated_at": "2013-11-06T12:49:20Z",
    "url": "https://github.com/sagemath/sage/issues/14320",
    "user": "https://github.com/sagetrac-tdumont"
}
```
The attached file provides doctests for examples in the chapter "float" (floating point numbers). Tests pass with 5.11.

Apply only [attachment: trac_14320_doctests_from_french_book.patch​](https://github.com/sagemath/sage/files/ticket14320/d8bbf892d87f41af5906814f40962c1c.gz).

Assignee: **@sagetrac-mvngu**

Keywords: **french book**

Upstream: **Completely fixed; Fix reported upstream**

Reviewer: **Frédéric Chapoton**

Author: **Thierry Dumont, Paul Zimmermann**

Merged: **sage-5.13.beta3**

_Issue created by migration from https://trac.sagemath.org/ticket/14320_





---

archive/issue_events_185447.json:
```json
{
    "actor": "https://github.com/sagetrac-tdumont",
    "created_at": "2013-03-20T16:33:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14320#event-185447"
}
```



---

archive/issue_events_185448.json:
```json
{
    "actor": "https://github.com/sagetrac-tdumont",
    "created_at": "2013-03-20T16:33:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14320#event-185448"
}
```



---

archive/issue_events_185449.json:
```json
{
    "actor": "https://github.com/sagetrac-tdumont",
    "created_at": "2013-03-20T16:33:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14320#event-185449"
}
```



---

archive/issue_comments_174241.json:
```json
{
    "body": "Attachment: **[float_doctest.sage.gz](https://github.com/sagemath/sage/files/ticket14320/float_doctest.sage.gz)**\n\ndoctets auto generated.",
    "created_at": "2013-03-20T16:34:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14320#issuecomment-174241",
    "user": "https://github.com/sagetrac-tdumont"
}
```

Attachment: **[float_doctest.sage.gz](https://github.com/sagemath/sage/files/ticket14320/float_doctest.sage.gz)**

doctets auto generated.



---

archive/issue_events_185450.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14320#event-185450"
}
```



---

archive/issue_events_185451.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14320#event-185451"
}
```



---

archive/issue_events_185452.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2013-08-23T10:20:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14320#event-185452"
}
```



---

archive/issue_comments_174242.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n-The attached file provides doctests for examples in the chapter \"float\" (floating point numbers). Tests pass with 5.8.\n+The attached file provides doctests for examples in the chapter \"float\" (floating point numbers). Tests pass with 5.11.\n+\n+Apply only trac_14320_doctests_from_french_book.patch\u200b.\n``````\n",
    "created_at": "2013-08-23T10:20:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14320#issuecomment-174242",
    "user": "https://github.com/zimmermann6"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
-The attached file provides doctests for examples in the chapter "float" (floating point numbers). Tests pass with 5.8.
+The attached file provides doctests for examples in the chapter "float" (floating point numbers). Tests pass with 5.11.
+
+Apply only trac_14320_doctests_from_french_book.patch​.
``````




---

archive/issue_comments_174243.json:
```json
{
    "body": "Changed author from **Thierry Dumont** to **Thierry Dumont, Paul Zimmermann**",
    "created_at": "2013-08-23T10:20:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14320#issuecomment-174243",
    "user": "https://github.com/zimmermann6"
}
```

Changed author from **Thierry Dumont** to **Thierry Dumont, Paul Zimmermann**



---

archive/issue_events_185453.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2013-08-23T10:20:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14320#event-185453"
}
```



---

archive/issue_comments_174244.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nI've attached a well-formed patch. I had to modify two things with respect to the book (which was with Sage 5.9), due to some recent changes in RDF:\n\n* I added \"abs tol 0.05\" somewhere to avoid a failing test with 5.11\n\n* `exact-sommation compensee` now gives 0.0 with 5.11 (instead of -0.001953125 with 5.9).\n  I thus changed to 0.0 in the doctest. This will thus differ from the book, which should be\n  updated to 5.11 (adding \"abs tol\" is not enough).\n\nPaul",
    "created_at": "2013-08-23T10:20:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14320#issuecomment-174244",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:2" align="right">Comment 2</div>

I've attached a well-formed patch. I had to modify two things with respect to the book (which was with Sage 5.9), due to some recent changes in RDF:

* I added "abs tol 0.05" somewhere to avoid a failing test with 5.11

* `exact-sommation compensee` now gives 0.0 with 5.11 (instead of -0.001953125 with 5.9).
  I thus changed to 0.0 in the doctest. This will thus differ from the book, which should be
  updated to 5.11 (adding "abs tol" is not enough).

Paul



---

archive/issue_events_185454.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-09-23T16:00:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14320#event-185454"
}
```



---

archive/issue_events_185455.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-09-23T16:00:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14320#event-185455"
}
```



---

archive/issue_comments_174245.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nsame remark about indentation: should be 4 spaces rather than 2",
    "created_at": "2013-09-23T16:00:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14320#issuecomment-174245",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">Comment 3</div>

same remark about indentation: should be 4 spaces rather than 2



---

archive/issue_events_185456.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2013-09-24T12:19:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14320#event-185456"
}
```



---

archive/issue_events_185457.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2013-09-24T12:19:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14320#event-185457"
}
```



---

archive/issue_comments_174246.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\n> same remark about indentation: should be 4 spaces rather than 2 \n\nis it better with the new patch?\n\nPaul",
    "created_at": "2013-09-24T12:19:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14320#issuecomment-174246",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:4" align="right">Comment 4</div>

> same remark about indentation: should be 4 spaces rather than 2 

is it better with the new patch?

Paul



---

archive/issue_comments_174247.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nyes, it is much better that way.",
    "created_at": "2013-10-19T19:41:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14320#issuecomment-174247",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:5" align="right">Comment 5</div>

yes, it is much better that way.



---

archive/issue_comments_174248.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nLet me say that it is good to go.",
    "created_at": "2013-10-19T20:06:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14320#issuecomment-174248",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:6" align="right">Comment 6</div>

Let me say that it is good to go.



---

archive/issue_comments_174249.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2013-10-19T20:06:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14320#issuecomment-174249",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_events_185458.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-10-19T20:06:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14320#event-185458"
}
```



---

archive/issue_events_185459.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-10-19T20:06:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14320#event-185459"
}
```



---

archive/issue_comments_174250.json:
```json
{
    "body": "Changed keywords from **** to **french book**",
    "created_at": "2013-10-19T20:06:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14320#issuecomment-174250",
    "user": "https://github.com/fchapoton"
}
```

Changed keywords from **** to **french book**



---

archive/issue_events_185460.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-31T07:59:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14320#event-185460"
}
```



---

archive/issue_events_185461.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-31T07:59:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14320#event-185461"
}
```



---

archive/issue_comments_174251.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nOn `arando` (Linux 32-bit i686):\n\n```\nsage -t --long devel/sage/sage/tests/french_book/float_doctest.py\n**********************************************************************\nFile \"devel/sage/sage/tests/french_book/float_doctest.py\", line 206, in sage.tests.french_book.float_doctest\nFailed example:\n    print \"exact - sommation classique:\", s-ii # abs tol 0.05\nExpected:\n    exact - sommation classique: -45.5\nGot:\n    exact - sommation classique: -45.591796875\nTolerance exceeded: 9e-02 > 5e-02\n**********************************************************************\n```",
    "created_at": "2013-10-31T07:59:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14320#issuecomment-174251",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:7" align="right">Comment 7</div>

On `arando` (Linux 32-bit i686):

```
sage -t --long devel/sage/sage/tests/french_book/float_doctest.py
**********************************************************************
File "devel/sage/sage/tests/french_book/float_doctest.py", line 206, in sage.tests.french_book.float_doctest
Failed example:
    print "exact - sommation classique:", s-ii # abs tol 0.05
Expected:
    exact - sommation classique: -45.5
Got:
    exact - sommation classique: -45.591796875
Tolerance exceeded: 9e-02 > 5e-02
**********************************************************************
```



---

archive/issue_comments_174252.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nI will have a look next week, unless someone beats me. However I wonder why the patchbot did not find this before. Did it work before on arando?\n\nPaul",
    "created_at": "2013-10-31T08:59:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14320#issuecomment-174252",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:8" align="right">Comment 8</div>

I will have a look next week, unless someone beats me. However I wonder why the patchbot did not find this before. Did it work before on arando?

Paul



---

archive/issue_comments_174253.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nIt seems the patchbot is having trouble itself... Also, I don't know whether we have any patchbots running on 32-bit i686.",
    "created_at": "2013-10-31T09:29:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14320#issuecomment-174253",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:9" align="right">Comment 9</div>

It seems the patchbot is having trouble itself... Also, I don't know whether we have any patchbots running on 32-bit i686.



---

archive/issue_comments_174254.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nAttachment: **[trac_14320_doctests_from_french_book.patch.gz](https://github.com/sagemath/sage/files/ticket14320/trac_14320_doctests_from_french_book.patch.gz)**\n\nI increased the tolerance to 0.1, it should be ok now.\n\nPaul",
    "created_at": "2013-11-04T10:51:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14320#issuecomment-174254",
    "user": "https://github.com/zimmermann6"
}
```

<div id="comment:10" align="right">Comment 10</div>

Attachment: **[trac_14320_doctests_from_french_book.patch.gz](https://github.com/sagemath/sage/files/ticket14320/trac_14320_doctests_from_french_book.patch.gz)**

I increased the tolerance to 0.1, it should be ok now.

Paul



---

archive/issue_events_185462.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2013-11-04T10:51:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14320#event-185462"
}
```



---

archive/issue_events_185463.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2013-11-04T10:51:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14320#event-185463"
}
```



---

archive/issue_events_185464.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-11-04T10:55:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14320#event-185464"
}
```



---

archive/issue_events_185465.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-11-04T10:55:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14320#event-185465"
}
```



---

archive/issue_comments_174255.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nShould be good.",
    "created_at": "2013-11-04T10:55:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14320#issuecomment-174255",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:11" align="right">Comment 11</div>

Should be good.



---

archive/issue_comments_174256.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n The attached file provides doctests for examples in the chapter \"float\" (floating point numbers). Tests pass with 5.11.\n \n-Apply only trac_14320_doctests_from_french_book.patch\u200b.\n+Apply only [attachment: trac_14320_doctests_from_french_book.patch\u200b](https://github.com/sagemath/sage/files/ticket14320/d8bbf892d87f41af5906814f40962c1c.gz).\n``````\n",
    "created_at": "2013-11-04T10:55:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14320#issuecomment-174256",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 The attached file provides doctests for examples in the chapter "float" (floating point numbers). Tests pass with 5.11.
 
-Apply only trac_14320_doctests_from_french_book.patch​.
+Apply only [attachment: trac_14320_doctests_from_french_book.patch​](https://github.com/sagemath/sage/files/ticket14320/d8bbf892d87f41af5906814f40962c1c.gz).
``````




---

archive/issue_events_185466.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-11-06T12:49:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14320#event-185466"
}
```



---

archive/issue_events_185467.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-11-06T12:49:20Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14320#event-185467"
}
```



---

archive/issue_comments_174257.json:
```json
{
    "body": "Merged: **sage-5.13.beta3**",
    "created_at": "2013-11-06T12:49:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14320",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14320#issuecomment-174257",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.13.beta3**
