# Issue 14160: Fix bug with the antipode in NonCommutativeSymmetricFunctions

archive/issues_013956.json:
```json
{
    "assignees": [],
    "body": "Fixes a bug in the antipode for non-commutative symmetric functions and adds a few doc-tests to check that it is correct\n\nAssignee: **@sagetrac-sage-combinat**\n\nCC:  @sagetrac-chrisjamesberg @saliola\n\nReviewer: **Franco Saliola**\n\nAuthor: **Mike Zabrocki**\n\nMerged: **sage-5.8.beta1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14160_\n\n",
    "closed_at": "2013-02-24T13:21:04Z",
    "created_at": "2013-02-22T02:28:07Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix bug with the antipode in NonCommutativeSymmetricFunctions",
    "type": "issue",
    "updated_at": "2013-02-24T13:21:04Z",
    "url": "https://github.com/sagemath/sage/issues/14160",
    "user": "https://github.com/zabrocki"
}
```
Fixes a bug in the antipode for non-commutative symmetric functions and adds a few doc-tests to check that it is correct

Assignee: **@sagetrac-sage-combinat**

CC:  @sagetrac-chrisjamesberg @saliola

Reviewer: **Franco Saliola**

Author: **Mike Zabrocki**

Merged: **sage-5.8.beta1**

_Issue created by migration from https://trac.sagemath.org/ticket/14160_





---

archive/issue_events_182843.json:
```json
{
    "actor": "https://github.com/zabrocki",
    "created_at": "2013-02-22T02:28:07Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14160",
    "milestone_number": null,
    "milestone_title": "sage-5.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14160#event-182843"
}
```



---

archive/issue_events_182844.json:
```json
{
    "actor": "https://github.com/zabrocki",
    "created_at": "2013-02-22T02:28:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14160",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14160#event-182844"
}
```



---

archive/issue_events_182845.json:
```json
{
    "actor": "https://github.com/zabrocki",
    "created_at": "2013-02-22T02:28:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14160",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14160#event-182845"
}
```



---

archive/issue_events_182846.json:
```json
{
    "actor": "https://github.com/zabrocki",
    "created_at": "2013-02-22T02:28:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14160",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14160#event-182846"
}
```



---

archive/issue_events_182847.json:
```json
{
    "actor": "https://github.com/zabrocki",
    "created_at": "2013-02-22T02:32:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14160",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14160#event-182847"
}
```



---

archive/issue_comments_171282.json:
```json
{
    "body": "Changed author from **zabrocki** to **Mike Zabrocki**",
    "created_at": "2013-02-22T05:11:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14160",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14160#issuecomment-171282",
    "user": "https://github.com/saliola"
}
```

Changed author from **zabrocki** to **Mike Zabrocki**



---

archive/issue_comments_171283.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nLooks good to me.\n\nTicket #14161 requests that a `_test_antipode` method be implemented (which would have caught this bug!)",
    "created_at": "2013-02-22T05:11:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14160",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14160#issuecomment-171283",
    "user": "https://github.com/saliola"
}
```

<div id="comment:2" align="right">Comment 2</div>

Looks good to me.

Ticket #14161 requests that a `_test_antipode` method be implemented (which would have caught this bug!)



---

archive/issue_comments_171284.json:
```json
{
    "body": "Reviewer: **Franco Saliola**",
    "created_at": "2013-02-22T05:11:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14160",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14160#issuecomment-171284",
    "user": "https://github.com/saliola"
}
```

Reviewer: **Franco Saliola**



---

archive/issue_events_182848.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2013-02-22T05:11:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14160",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14160#event-182848"
}
```



---

archive/issue_events_182849.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2013-02-22T05:11:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14160",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14160#event-182849"
}
```



---

archive/issue_events_182850.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2013-02-22T05:14:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14160",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14160#event-182850"
}
```



---

archive/issue_events_182851.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2013-02-22T05:14:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14160",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14160#event-182851"
}
```



---

archive/issue_comments_171285.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nMike, one more thing : you need to add the ticket number to the patch description.",
    "created_at": "2013-02-22T05:14:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14160",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14160#issuecomment-171285",
    "user": "https://github.com/saliola"
}
```

<div id="comment:3" align="right">Comment 3</div>

Mike, one more thing : you need to add the ticket number to the patch description.



---

archive/issue_comments_171286.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nAttachment: **[trac_14160_fix_ncsf_antipode-mz.patch.gz](https://github.com/sagemath/sage/files/ticket14160/trac_14160_fix_ncsf_antipode-mz.patch.gz)**",
    "created_at": "2013-02-22T16:16:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14160",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14160#issuecomment-171286",
    "user": "https://github.com/zabrocki"
}
```

<div id="comment:4" align="right">Comment 4</div>

Attachment: **[trac_14160_fix_ncsf_antipode-mz.patch.gz](https://github.com/sagemath/sage/files/ticket14160/trac_14160_fix_ncsf_antipode-mz.patch.gz)**



---

archive/issue_events_182852.json:
```json
{
    "actor": "https://github.com/zabrocki",
    "created_at": "2013-02-22T16:16:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14160",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14160#event-182852"
}
```



---

archive/issue_events_182853.json:
```json
{
    "actor": "https://github.com/zabrocki",
    "created_at": "2013-02-22T16:16:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14160",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14160#event-182853"
}
```



---

archive/issue_events_182854.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2013-02-22T17:27:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14160",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14160#event-182854"
}
```



---

archive/issue_events_182855.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2013-02-22T17:27:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14160",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14160#event-182855"
}
```



---

archive/issue_events_182856.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-24T13:21:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14160",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14160#event-182856"
}
```



---

archive/issue_events_182857.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-24T13:21:04Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14160",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14160#event-182857"
}
```



---

archive/issue_comments_171287.json:
```json
{
    "body": "Merged: **sage-5.8.beta1**",
    "created_at": "2013-02-24T13:21:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14160",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14160#issuecomment-171287",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.8.beta1**
