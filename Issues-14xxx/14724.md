# Issue 14724: Tableau.to_permutation equals Tableau.reading_word_permutation and both are broken

archive/issues_014520.json:
```json
{
    "assignees": [],
    "body": "The following doesn't work:\n\n```\nsage: T = Tableau([1,1]) \n...\nValueError: The permutation has length 2 but its maximal element is 1. Some element may be repeated, or an element is missing, but there is something wrong with its length.\n```\n\nDepends on #8392\n\nAssignee: **sage-combinat**\n\nCC:  chrisjamesberg @VivianePons @tscrim\n\nKeywords: **permutation, reading word, FindStatDays01**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Christian Stump**\n\nMerged: **sage-5.11.beta1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14724_\n\n",
    "closed_at": "2013-06-15T12:21:51Z",
    "created_at": "2013-06-11T20:56:11Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.11",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Tableau.to_permutation equals Tableau.reading_word_permutation and both are broken",
    "type": "issue",
    "updated_at": "2013-06-15T12:21:51Z",
    "url": "https://github.com/sagemath/sage/issues/14724",
    "user": "https://github.com/sagetrac-stumpc5"
}
```
The following doesn't work:

```
sage: T = Tableau([1,1]) 
...
ValueError: The permutation has length 2 but its maximal element is 1. Some element may be repeated, or an element is missing, but there is something wrong with its length.
```

Depends on #8392

Assignee: **sage-combinat**

CC:  chrisjamesberg @VivianePons @tscrim

Keywords: **permutation, reading word, FindStatDays01**

Reviewer: **Travis Scrimshaw**

Author: **Christian Stump**

Merged: **sage-5.11.beta1**

_Issue created by migration from https://trac.sagemath.org/ticket/14724_





---

archive/issue_events_191896.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2013-06-11T20:56:11Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14724",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14724#event-191896"
}
```



---

archive/issue_events_191897.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2013-06-11T20:56:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14724",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14724#event-191897"
}
```



---

archive/issue_events_191898.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2013-06-11T20:56:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14724",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14724#event-191898"
}
```



---

archive/issue_events_191899.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2013-06-11T20:56:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14724",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14724#event-191899"
}
```



---

archive/issue_comments_182044.json:
```json
{
    "body": "Changed keywords from **permutation, reading word** to **permutation, reading word, FindStatDays**",
    "created_at": "2013-06-12T08:20:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14724",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14724#issuecomment-182044",
    "user": "https://github.com/sagetrac-stumpc5"
}
```

Changed keywords from **permutation, reading word** to **permutation, reading word, FindStatDays**



---

archive/issue_events_191900.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2013-06-12T08:20:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14724",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14724#event-191900"
}
```



---

archive/issue_comments_182045.json:
```json
{
    "body": "Changed keywords from **permutation, reading word, FindStatDays** to **permutation, reading word, FindStatDays01**",
    "created_at": "2013-06-12T08:23:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14724",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14724#issuecomment-182045",
    "user": "https://github.com/sagetrac-stumpc5"
}
```

Changed keywords from **permutation, reading word, FindStatDays** to **permutation, reading word, FindStatDays01**



---

archive/issue_comments_182046.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2013-06-12T09:28:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14724",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14724#issuecomment-182046",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_182047.json:
```json
{
    "body": "Dependencies: **#8392**",
    "created_at": "2013-06-12T09:28:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14724",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14724#issuecomment-182047",
    "user": "https://github.com/tscrim"
}
```

Dependencies: **#8392**



---

archive/issue_comments_182048.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nuploaded the rebased version. Ready for review again!",
    "created_at": "2013-06-12T10:05:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14724",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14724#issuecomment-182048",
    "user": "https://github.com/sagetrac-stumpc5"
}
```

<a id='comment:4'>Comment 4:</a>
uploaded the rebased version. Ready for review again!



---

archive/issue_events_191901.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-06-12T10:29:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14724",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14724#event-191901"
}
```



---

archive/issue_events_191902.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-06-12T10:29:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14724",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14724#event-191902"
}
```



---

archive/issue_comments_182049.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nLooks good to me.",
    "created_at": "2013-06-12T10:29:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14724",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14724#issuecomment-182049",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:5'>Comment 5:</a>
Looks good to me.



---

archive/issue_comments_182050.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nThe `deprecated_function_alias` does not work properly with the `combinatorial_map` decorator.",
    "created_at": "2013-06-12T19:54:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14724",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14724#issuecomment-182050",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:6'>Comment 6:</a>
The `deprecated_function_alias` does not work properly with the `combinatorial_map` decorator.



---

archive/issue_events_191903.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-06-12T19:54:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14724",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14724#event-191903"
}
```



---

archive/issue_events_191904.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-06-12T19:54:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14724",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14724#event-191904"
}
```



---

archive/issue_events_191905.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2013-06-12T20:23:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14724",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14724#event-191905"
}
```



---

archive/issue_events_191906.json:
```json
{
    "actor": "https://github.com/sagetrac-stumpc5",
    "created_at": "2013-06-12T20:23:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14724",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14724#event-191906"
}
```



---

archive/issue_comments_182051.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nAttachment: **[trac_14724-tableau_to_permutation_and_friend-cs.patch.gz](https://github.com/sagemath/sage/files/ticket14724/trac_14724-tableau_to_permutation_and_friend-cs.patch.gz)**\n\nFixed in the new version of the patch - Travis and me verified it...",
    "created_at": "2013-06-12T20:23:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14724",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14724#issuecomment-182051",
    "user": "https://github.com/sagetrac-stumpc5"
}
```

<a id='comment:7'>Comment 7:</a>
Attachment: **[trac_14724-tableau_to_permutation_and_friend-cs.patch.gz](https://github.com/sagemath/sage/files/ticket14724/trac_14724-tableau_to_permutation_and_friend-cs.patch.gz)**

Fixed in the new version of the patch - Travis and me verified it...



---

archive/issue_events_191907.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-15T12:21:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14724",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14724#event-191907"
}
```



---

archive/issue_events_191908.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-15T12:21:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14724",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14724#event-191908"
}
```



---

archive/issue_comments_182052.json:
```json
{
    "body": "Merged: **sage-5.11.beta1**",
    "created_at": "2013-06-15T12:21:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14724",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14724#issuecomment-182052",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.11.beta1**
