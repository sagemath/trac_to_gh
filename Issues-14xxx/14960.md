# Issue 14960: Polynomials over Z, Q, Z/nZ don't compose efficiently

archive/issues_014723.json:
```json
{
    "assignees": [],
    "body": "The !__call!__ methods on flint-based polynomial types currently simply fall back to the generic Polynomial.!__call!__ for any nonconstant input. This is a most silly thing to do since flint has very fast code for polynomial composition.\n\nSpecifically, the !__call!__ methods in  Polynomial_integer_dense_flint, Polynomial_rational_flint, Polynomial_zmod_flint should use fmpz_poly_compose, fmpq_poly_compose or nmod_poly_compose when the output is of the appropriate type.\n\nKeywords: **flint, polynomials**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14960_\n\n",
    "closed_at": "2017-07-13T07:54:31Z",
    "created_at": "2013-07-23T22:26:45Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/performance",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Polynomials over Z, Q, Z/nZ don't compose efficiently",
    "type": "issue",
    "updated_at": "2017-07-13T07:54:31Z",
    "url": "https://github.com/sagemath/sage/issues/14960",
    "user": "https://github.com/fredrik-johansson"
}
```
The !__call!__ methods on flint-based polynomial types currently simply fall back to the generic Polynomial.!__call!__ for any nonconstant input. This is a most silly thing to do since flint has very fast code for polynomial composition.

Specifically, the !__call!__ methods in  Polynomial_integer_dense_flint, Polynomial_rational_flint, Polynomial_zmod_flint should use fmpz_poly_compose, fmpq_poly_compose or nmod_poly_compose when the output is of the appropriate type.

Keywords: **flint, polynomials**

_Issue created by migration from https://trac.sagemath.org/ticket/14960_





---

archive/issue_events_194959.json:
```json
{
    "actor": "https://github.com/fredrik-johansson",
    "created_at": "2013-07-23T22:26:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14960",
    "label": "https://github.com/sagemath/sage/labels/performance",
    "label_color": "696969",
    "label_name": "performance",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14960#event-194959"
}
```



---

archive/issue_events_194960.json:
```json
{
    "actor": "https://github.com/fredrik-johansson",
    "created_at": "2013-07-23T22:26:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14960",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14960#event-194960"
}
```



---

archive/issue_events_194961.json:
```json
{
    "actor": "https://github.com/fredrik-johansson",
    "created_at": "2013-07-23T22:26:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14960",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14960#event-194961"
}
```



---

archive/issue_comments_185780.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nIt doesn't seem to be the case (nor to have been back in 2013).",
    "created_at": "2017-01-09T15:26:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14960#issuecomment-185780",
    "user": "https://github.com/mezzarobba"
}
```

<a id='comment:1'>Comment 1:</a>
It doesn't seem to be the case (nor to have been back in 2013).



---

archive/issue_events_194962.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2017-01-09T15:26:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14960",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14960#event-194962"
}
```



---

archive/issue_events_194963.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2017-01-09T15:26:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14960",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14960#event-194963"
}
```



---

archive/issue_events_194964.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2017-01-09T15:26:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14960",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14960#event-194964"
}
```



---

archive/issue_events_194965.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2017-01-31T10:37:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14960",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14960#event-194965"
}
```



---

archive/issue_events_194966.json:
```json
{
    "actor": "https://github.com/sagetrac-jpflori",
    "created_at": "2017-01-31T10:37:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14960",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14960#event-194966"
}
```



---

archive/issue_events_194967.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-07-13T07:54:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14960",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14960#event-194967"
}
```



---

archive/issue_events_194968.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-07-13T07:54:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14960",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14960#event-194968"
}
```



---

archive/issue_comments_185781.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nClosing tickets in the sage-duplicate/invalid/wontfix module with positive_review (i.e. someone has confirmed they should be closed).",
    "created_at": "2017-07-13T07:54:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14960",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14960#issuecomment-185781",
    "user": "https://github.com/embray"
}
```

<a id='comment:4'>Comment 4:</a>
Closing tickets in the sage-duplicate/invalid/wontfix module with positive_review (i.e. someone has confirmed they should be closed).



---

archive/issue_events_194969.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-07-13T07:54:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14960",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14960#event-194969"
}
```
