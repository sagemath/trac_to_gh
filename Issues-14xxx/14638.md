# Issue 14638: Fixes a bug in mutation_class (cluster seed)

archive/issues_014434.json:
```json
{
    "assignees": [
        "https://github.com/sagetrac-sage-combinat"
    ],
    "body": "Reported by Bruce Fontaine: The method mutation_class() was giving the wrong answer for ClusterSeed(['E',8]]).\n\nThe length was supposed to be 25080, but our code gives 25067 instead.\n\nThe issue was that we refer to index of an element of a list, but we then turn that list into a set.\n\nCC:  bfontain@math.cornell.edu @stumpc5\n\nKeywords: **cluster mutation class**\n\nReviewer: **Christian Stump**\n\nAuthor: **Gregg Musiker**\n\nMerged: **sage-5.10.rc1**\n\nComponent: **combinatorics**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14638_\n\n",
    "closed_at": "2013-06-05T19:51:07Z",
    "created_at": "2013-05-24T15:41:32Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.10",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fixes a bug in mutation_class (cluster seed)",
    "type": "issue",
    "updated_at": "2013-06-05T19:51:07Z",
    "url": "https://github.com/sagemath/sage/issues/14638",
    "user": "https://github.com/sagetrac-gmoose05"
}
```
Reported by Bruce Fontaine: The method mutation_class() was giving the wrong answer for ClusterSeed(['E',8]]).

The length was supposed to be 25080, but our code gives 25067 instead.

The issue was that we refer to index of an element of a list, but we then turn that list into a set.

CC:  bfontain@math.cornell.edu @stumpc5

Keywords: **cluster mutation class**

Reviewer: **Christian Stump**

Author: **Gregg Musiker**

Merged: **sage-5.10.rc1**

Component: **combinatorics**

_Issue created by migration from https://trac.sagemath.org/ticket/14638_





---

archive/issue_events_207436.json:
```json
{
    "actor": "https://github.com/sagetrac-gmoose05",
    "created_at": "2013-05-24T15:41:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14638",
    "milestone_number": null,
    "milestone_title": "sage-5.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14638#event-207436"
}
```



---

archive/issue_events_207437.json:
```json
{
    "actor": "https://github.com/sagetrac-gmoose05",
    "created_at": "2013-05-24T15:41:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14638",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "c: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14638#event-207437"
}
```



---

archive/issue_events_207438.json:
```json
{
    "actor": "https://github.com/sagetrac-gmoose05",
    "created_at": "2013-05-24T15:41:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14638",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14638#event-207438"
}
```



---

archive/issue_events_207439.json:
```json
{
    "actor": "https://github.com/sagetrac-gmoose05",
    "created_at": "2013-05-24T15:41:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14638",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14638#event-207439"
}
```



---

archive/issue_events_207440.json:
```json
{
    "actor": "https://github.com/sagetrac-sage-combinat",
    "created_at": "2013-05-24T15:41:32Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/14638",
    "subject": "https://github.com/sagetrac-gmoose05",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14638#event-207440"
}
```



---

archive/issue_comments_177747.json:
```json
{
    "body": "Attachment: **[trac_14638-mutation_class_fix.patch.gz](https://github.com/sagemath/sage/files/ticket14638/trac_14638-mutation_class_fix.patch.gz)**",
    "created_at": "2013-05-24T18:34:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14638",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14638#issuecomment-177747",
    "user": "https://github.com/sagetrac-gmoose05"
}
```

Attachment: **[trac_14638-mutation_class_fix.patch.gz](https://github.com/sagemath/sage/files/ticket14638/trac_14638-mutation_class_fix.patch.gz)**



---

archive/issue_events_207441.json:
```json
{
    "actor": "https://github.com/stumpc5",
    "created_at": "2013-05-24T18:39:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14638",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14638#event-207441"
}
```



---

archive/issue_comments_177748.json:
```json
{
    "body": "Changed keywords from none to **cluster mutation class**",
    "created_at": "2013-05-24T18:39:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14638",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14638#issuecomment-177748",
    "user": "https://github.com/stumpc5"
}
```

Changed keywords from none to **cluster mutation class**



---

archive/issue_events_207442.json:
```json
{
    "actor": "https://github.com/stumpc5",
    "created_at": "2013-06-02T21:07:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14638",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14638#event-207442"
}
```



---

archive/issue_events_207443.json:
```json
{
    "actor": "https://github.com/stumpc5",
    "created_at": "2013-06-02T21:07:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14638",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14638#event-207443"
}
```



---

archive/issue_events_207444.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-03T10:03:36Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14638",
    "milestone_number": null,
    "milestone_title": "sage-5.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14638#event-207444"
}
```



---

archive/issue_events_207445.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-03T10:03:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14638",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14638#event-207445"
}
```



---

archive/issue_comments_177749.json:
```json
{
    "body": "Author: **Gregg Musiker**",
    "created_at": "2013-06-03T10:03:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14638",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14638#issuecomment-177749",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Gregg Musiker**



---

archive/issue_events_207446.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-03T11:47:36Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14638",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14638#event-207446"
}
```



---

archive/issue_events_207447.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-03T11:47:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14638",
    "milestone_number": null,
    "milestone_title": "sage-5.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14638#event-207447"
}
```



---

archive/issue_comments_177750.json:
```json
{
    "body": "Merged: **sage-5.10.rc1**",
    "created_at": "2013-06-05T19:51:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14638",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14638#issuecomment-177750",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.10.rc1**



---

archive/issue_events_207448.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-05T19:51:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14638",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14638#event-207448"
}
```



---

archive/issue_events_207449.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-05T19:51:07Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14638",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14638#event-207449"
}
```
