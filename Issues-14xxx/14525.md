...
```json
{
    "body": "I actually copied the patch for #10756, which returns `g.vertices()` as a maximum clique when `g.order()` is 0. I'd be happy to change it to `[[]]`, but maybe then I should also change the other method to return `[]` just to be consistent for all three methods.",
...
```json
{
    "body": "If you can, it would be nice indeed `:-)`\n\nNathann",
...
```json
{
    "body": "Attachment [trac_14525-rev.patch](tarball://root/attachments/some-uuid/ticket14525/trac_14525-rev.patch) by @nvcleemp created at 2013-05-29 13:13:45\n\nOK, looks ok and works for me.",
