# Issue 14511: Typeset checkbox doesn't display square roots properly

archive/issues_014307.json:
```json
{
    "assignees": [],
    "body": "On some outputs, the top of the square root sign isn't visible with the \"Typeset\" checkbox, but does work with `show`. The attached screenshot shows the issue.\n\nThis is a problem because some expressions become ambiguous; in the screenshot `sqrt(x^2 + 3)` and `sqrt(x^2) + 3` cannot be distinguished.\n\nAssignee: **@jasongrout, @qed777, @williamstein, @kini**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14511_\n\n",
    "closed_at": "2018-05-18T17:16:26Z",
    "created_at": "2013-05-01T06:35:56Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20notebook",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Typeset checkbox doesn't display square roots properly",
    "type": "issue",
    "updated_at": "2018-05-18T17:16:26Z",
    "url": "https://github.com/sagemath/sage/issues/14511",
    "user": "https://github.com/eviatarbach"
}
```
On some outputs, the top of the square root sign isn't visible with the "Typeset" checkbox, but does work with `show`. The attached screenshot shows the issue.

This is a problem because some expressions become ambiguous; in the screenshot `sqrt(x^2 + 3)` and `sqrt(x^2) + 3` cannot be distinguished.

Assignee: **@jasongrout, @qed777, @williamstein, @kini**

_Issue created by migration from https://trac.sagemath.org/ticket/14511_





---

archive/issue_events_188445.json:
```json
{
    "actor": "https://github.com/eviatarbach",
    "created_at": "2013-05-01T06:35:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14511",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14511#event-188445"
}
```



---

archive/issue_events_188446.json:
```json
{
    "actor": "https://github.com/eviatarbach",
    "created_at": "2013-05-01T06:35:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14511",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20notebook",
    "label_color": "000080",
    "label_name": "component: notebook",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14511#event-188446"
}
```



---

archive/issue_events_188447.json:
```json
{
    "actor": "https://github.com/eviatarbach",
    "created_at": "2013-05-01T06:35:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14511",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14511#event-188447"
}
```



---

archive/issue_events_188448.json:
```json
{
    "actor": "https://github.com/eviatarbach",
    "created_at": "2013-05-01T06:35:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14511",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14511#event-188448"
}
```



---

archive/issue_comments_178053.json:
```json
{
    "body": "Attachment: **[typeset bug.png](https://github.com/sagemath/sage/files/ticket14511/de6eac2f9fa60c8a198b8d299deab1a6.png)**",
    "created_at": "2013-05-01T06:38:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14511",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14511#issuecomment-178053",
    "user": "https://github.com/eviatarbach"
}
```

Attachment: **[typeset bug.png](https://github.com/sagemath/sage/files/ticket14511/de6eac2f9fa60c8a198b8d299deab1a6.png)**



---

archive/issue_comments_178054.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n On some outputs, the top of the square root sign isn't visible with the \"Typeset\" checkbox, but does work with `show`. The attached screenshot shows the issue.\n+\n+This is a problem because some expressions become ambiguous; in the screenshot `sqrt(x^2 + 3)` and `sqrt(x^2) + 3` cannot be distinguished.\n``````\n",
    "created_at": "2013-05-01T06:39:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14511",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14511#issuecomment-178054",
    "user": "https://github.com/eviatarbach"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
 On some outputs, the top of the square root sign isn't visible with the "Typeset" checkbox, but does work with `show`. The attached screenshot shows the issue.
+
+This is a problem because some expressions become ambiguous; in the screenshot `sqrt(x^2 + 3)` and `sqrt(x^2) + 3` cannot be distinguished.
``````




---

archive/issue_events_188449.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14511",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14511#event-188449"
}
```



---

archive/issue_events_188450.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14511",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14511#event-188450"
}
```



---

archive/issue_events_188451.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14511",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14511#event-188451"
}
```



---

archive/issue_events_188452.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14511",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14511#event-188452"
}
```



---

archive/issue_events_188453.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14511",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14511#event-188453"
}
```



---

archive/issue_events_188454.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14511",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14511#event-188454"
}
```



---

archive/issue_events_188455.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14511",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14511#event-188455"
}
```



---

archive/issue_events_188456.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14511",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14511#event-188456"
}
```



---

archive/issue_events_188457.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-01-17T12:27:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14511",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14511#event-188457"
}
```



---

archive/issue_comments_178055.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nno problem now",
    "created_at": "2018-01-17T12:27:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14511",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14511#issuecomment-178055",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:6" align="right">Comment 6</div>

no problem now



---

archive/issue_events_188458.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-01-17T12:27:57Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14511",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14511#event-188458"
}
```



---

archive/issue_events_188459.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-01-17T12:27:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14511",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14511#event-188459"
}
```



---

archive/issue_events_188460.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-01-17T12:27:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14511",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14511#event-188460"
}
```



---

archive/issue_events_188461.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-01-18T15:11:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14511",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14511#event-188461"
}
```



---

archive/issue_events_188462.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-01-18T15:11:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14511",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14511#event-188462"
}
```



---

archive/issue_comments_178056.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nclosing positively reviewed duplicates",
    "created_at": "2018-05-18T17:16:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14511",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14511#issuecomment-178056",
    "user": "https://github.com/videlec"
}
```

<div id="comment:8" align="right">Comment 8</div>

closing positively reviewed duplicates



---

archive/issue_events_188463.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-18T17:16:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14511",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14511#event-188463"
}
```



---

archive/issue_events_188464.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-18T17:16:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14511",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14511#event-188464"
}
```



---

archive/issue_events_188465.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-18T17:16:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14511",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14511#event-188465"
}
```
