# Issue 14432: Hash broken for unions, intersections, differences, and symmetric differences of sets

archive/issues_014228.json:
```json
{
    "assignees": [],
    "body": "The following code raises an exception\n\n```\nsage: S=Set(ZZ).union(Set([infinity]))\nsage: hash(S)\nTraceback (most recent call last):\n...\nRuntimeError: maximum recursion depth exceeded\n```\n\nThe same should happen for intersections, differences, and symmetric differences.\n\nAssignee: **sage-combinat**\n\nKeywords: **hash, set, sd59**\n\nBranch/Commit: **[05ed037](https://github.com/sagemath/sagetrac-mirror/commit/05ed0370f6a08b0e181f1f520b7df7ec6312a15c)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Julian Rueth**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14432_\n\n",
    "closed_at": "2015-01-24T13:18:41Z",
    "created_at": "2013-04-09T17:48:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Hash broken for unions, intersections, differences, and symmetric differences of sets",
    "type": "issue",
    "updated_at": "2015-01-24T13:18:41Z",
    "url": "https://github.com/sagemath/sage/issues/14432",
    "user": "https://github.com/saraedum"
}
```
The following code raises an exception

```
sage: S=Set(ZZ).union(Set([infinity]))
sage: hash(S)
Traceback (most recent call last):
...
RuntimeError: maximum recursion depth exceeded
```

The same should happen for intersections, differences, and symmetric differences.

Assignee: **sage-combinat**

Keywords: **hash, set, sd59**

Branch/Commit: **[05ed037](https://github.com/sagemath/sagetrac-mirror/commit/05ed0370f6a08b0e181f1f520b7df7ec6312a15c)**

Reviewer: **Travis Scrimshaw**

Author: **Julian Rueth**

_Issue created by migration from https://trac.sagemath.org/ticket/14432_





---

archive/issue_events_187235.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2013-04-09T17:48:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187235"
}
```



---

archive/issue_events_187236.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2013-04-09T17:48:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187236"
}
```



---

archive/issue_events_187237.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2013-04-09T17:48:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187237"
}
```



---

archive/issue_events_187238.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2013-04-09T17:48:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187238"
}
```



---

archive/issue_comments_176288.json:
```json
{
    "body": "Attachment: **[trac_14432.patch.gz](https://github.com/sagemath/sage/files/ticket14432/trac_14432.patch.gz)**",
    "created_at": "2013-04-09T18:45:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14432#issuecomment-176288",
    "user": "https://github.com/saraedum"
}
```

Attachment: **[trac_14432.patch.gz](https://github.com/sagemath/sage/files/ticket14432/trac_14432.patch.gz)**



---

archive/issue_comments_176289.json:
```json
{
    "body": "Author: **Julian Rueth**",
    "created_at": "2013-04-09T18:45:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14432#issuecomment-176289",
    "user": "https://github.com/saraedum"
}
```

Author: **Julian Rueth**



---

archive/issue_events_187239.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2013-04-09T18:45:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187239"
}
```



---

archive/issue_comments_176290.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nI do not have access to a 32-bit machine right now. Could someone fill in the missing hash values? Is there a way to compute them from the 64-bit values btw?",
    "created_at": "2013-04-09T18:48:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14432#issuecomment-176290",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:2'>Comment 2:</a>
I do not have access to a 32-bit machine right now. Could someone fill in the missing hash values? Is there a way to compute them from the 64-bit values btw?



---

archive/issue_comments_176291.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nI decided to remove some duplicated code rather than adding a `__hash__` method to all the implementations - hope that is ok.",
    "created_at": "2013-04-09T18:51:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14432#issuecomment-176291",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:3'>Comment 3:</a>
I decided to remove some duplicated code rather than adding a `__hash__` method to all the implementations - hope that is ok.



---

archive/issue_comments_176292.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nTo test hashing, it is better to do tests like `hash(s) == hash(s)` that do not depend upon the specific value but instead show that the object is hashable and it's value does not change.",
    "created_at": "2013-04-09T19:11:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14432#issuecomment-176292",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:4'>Comment 4:</a>
To test hashing, it is better to do tests like `hash(s) == hash(s)` that do not depend upon the specific value but instead show that the object is hashable and it's value does not change.



---

archive/issue_comments_176293.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nThat's a good point. Thanks for the input. I will change my patch accordingly.",
    "created_at": "2013-04-09T19:21:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14432#issuecomment-176293",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:5'>Comment 5:</a>
That's a good point. Thanks for the input. I will change my patch accordingly.



---

archive/issue_events_187240.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2013-04-09T19:21:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187240"
}
```



---

archive/issue_events_187241.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2013-04-09T19:21:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187241"
}
```



---

archive/issue_events_187242.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187242"
}
```



---

archive/issue_events_187243.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187243"
}
```



---

archive/issue_comments_176294.json:
```json
{
    "body": "Branch: **[u/saraedum/ticket/14432](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/14432)**",
    "created_at": "2013-08-28T14:05:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14432#issuecomment-176294",
    "user": "https://github.com/saraedum"
}
```

Branch: **[u/saraedum/ticket/14432](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/14432)**



---

archive/issue_events_187244.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2013-08-28T14:08:22Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187244"
}
```



---

archive/issue_events_187245.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2013-08-28T14:08:22Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "milestone_number": null,
    "milestone_title": "sage-6.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187245"
}
```



---

archive/issue_comments_176295.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-The following code raises an exception:\n+The following code raises an exception\n \n ```\n sage: S=Set(ZZ).union(Set([infinity]))\n``````\n",
    "created_at": "2013-08-28T14:08:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14432#issuecomment-176295",
    "user": "https://github.com/saraedum"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-The following code raises an exception:
+The following code raises an exception
 
 ```
 sage: S=Set(ZZ).union(Set([infinity]))
``````




---

archive/issue_events_187246.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2013-08-28T14:08:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187246"
}
```



---

archive/issue_events_187247.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2013-08-28T14:08:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187247"
}
```



---

archive/issue_comments_176296.json:
```json
{
    "body": "Commit: **[e851c6f](https://github.com/sagemath/sagetrac-mirror/commit/e851c6f179661e67e79e8aac44aa82371e7ff963)**",
    "created_at": "2013-10-28T11:09:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14432#issuecomment-176296",
    "user": "https://github.com/nathanncohen"
}
```

Commit: **[e851c6f](https://github.com/sagemath/sagetrac-mirror/commit/e851c6f179661e67e79e8aac44aa82371e7ff963)**



---

archive/issue_comments_176297.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nThis patch does not apply cleanly on 5.13.beta0. And it's probably because it contains many dependencies #14482 `O_o`\n\n```\n~/sage$ git fetch trac u/saraedum/ticket/14432:hash\nremote: Counting objects: 17, done.\nremote: Compressing objects: 100% (12/12), done.\nremote: Total 12 (delta 10), reused 0 (delta 0)\nUnpacking objects: 100% (12/12), done.\nFrom ssh://trac.sagemath.org:2222/sage\n * [new branch]      u/saraedum/ticket/14432 -> hash\n~/sage$ git merge hash\nAuto-merging src/sage/sets/set.py\nCONFLICT (content): Merge conflict in src/sage/sets/set.py\nAuto-merging src/sage/doctest/forker.py\nCONFLICT (content): Merge conflict in src/sage/doctest/forker.py\nAuto-merging build/pkgs/sagenb/package-version.txt\nCONFLICT (content): Merge conflict in build/pkgs/sagenb/package-version.txt\nAuto-merging build/pkgs/sagenb/checksums.ini\nCONFLICT (content): Merge conflict in build/pkgs/sagenb/checksums.ini\nAuto-merging build/pkgs/sagenb/SPKG.txt\nCONFLICT (content): Merge conflict in build/pkgs/sagenb/SPKG.txt\nAutomatic merge failed; fix conflicts and then commit the result.\n```\n\n---\nLast 10 new commits:\n<table><tr><td>[changeset:e851c6f]</td><td><code>fixed doctest for the hash of a union/intersection/... of sets</code></td></tr><tr><td>[changeset:10a3d1b]</td><td><code>(HEAD, refs/heads/stefan) Trac #14432: Fix `__hash__` for unions of sets</code></td></tr><tr><td>[changeset:a10bc4f]</td><td><code>Merge branch 'ticket/14482'</code></td></tr><tr><td>[changeset:fda55a9]</td><td><code>Merge branch 'u/saraedum/ticket/14482' of ssh://trac.sagemath.org:2222/sage into HEAD</code></td></tr><tr><td>[changeset:61236da]</td><td><code>Merge remote-tracking branch 'trac/public/sage-git/ticket/14482'</code></td></tr><tr><td>[changeset:08d3e94]</td><td><code>Merge branch 'ticket/14482'</code></td></tr><tr><td>[changeset:50e1ece]</td><td><code>Merge branch 'ticket/14482'</code></td></tr><tr><td>[changeset:1ca56cf]</td><td><code>Merge branch 'ticket/14273' into tip</code></td></tr><tr><td>[changeset:aa60895]</td><td><code>remove workaround for sagenb pull request 84 (#14273)</code></td></tr><tr><td>[changeset:133785b]</td><td><code>Merge branch 'ticket/14482' into tip</code></td></tr></table>\n",
    "created_at": "2013-10-28T11:09:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14432#issuecomment-176297",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:9'>Comment 9:</a>
This patch does not apply cleanly on 5.13.beta0. And it's probably because it contains many dependencies #14482 `O_o`

```
~/sage$ git fetch trac u/saraedum/ticket/14432:hash
remote: Counting objects: 17, done.
remote: Compressing objects: 100% (12/12), done.
remote: Total 12 (delta 10), reused 0 (delta 0)
Unpacking objects: 100% (12/12), done.
From ssh://trac.sagemath.org:2222/sage
 * [new branch]      u/saraedum/ticket/14432 -> hash
~/sage$ git merge hash
Auto-merging src/sage/sets/set.py
CONFLICT (content): Merge conflict in src/sage/sets/set.py
Auto-merging src/sage/doctest/forker.py
CONFLICT (content): Merge conflict in src/sage/doctest/forker.py
Auto-merging build/pkgs/sagenb/package-version.txt
CONFLICT (content): Merge conflict in build/pkgs/sagenb/package-version.txt
Auto-merging build/pkgs/sagenb/checksums.ini
CONFLICT (content): Merge conflict in build/pkgs/sagenb/checksums.ini
Auto-merging build/pkgs/sagenb/SPKG.txt
CONFLICT (content): Merge conflict in build/pkgs/sagenb/SPKG.txt
Automatic merge failed; fix conflicts and then commit the result.
```

---
Last 10 new commits:
<table><tr><td>[changeset:e851c6f]</td><td><code>fixed doctest for the hash of a union/intersection/... of sets</code></td></tr><tr><td>[changeset:10a3d1b]</td><td><code>(HEAD, refs/heads/stefan) Trac #14432: Fix `__hash__` for unions of sets</code></td></tr><tr><td>[changeset:a10bc4f]</td><td><code>Merge branch 'ticket/14482'</code></td></tr><tr><td>[changeset:fda55a9]</td><td><code>Merge branch 'u/saraedum/ticket/14482' of ssh://trac.sagemath.org:2222/sage into HEAD</code></td></tr><tr><td>[changeset:61236da]</td><td><code>Merge remote-tracking branch 'trac/public/sage-git/ticket/14482'</code></td></tr><tr><td>[changeset:08d3e94]</td><td><code>Merge branch 'ticket/14482'</code></td></tr><tr><td>[changeset:50e1ece]</td><td><code>Merge branch 'ticket/14482'</code></td></tr><tr><td>[changeset:1ca56cf]</td><td><code>Merge branch 'ticket/14273' into tip</code></td></tr><tr><td>[changeset:aa60895]</td><td><code>remove workaround for sagenb pull request 84 (#14273)</code></td></tr><tr><td>[changeset:133785b]</td><td><code>Merge branch 'ticket/14482' into tip</code></td></tr></table>




---

archive/issue_events_187248.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-10-28T11:10:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187248"
}
```



---

archive/issue_events_187249.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-10-28T11:10:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187249"
}
```



---

archive/issue_events_187250.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2013-12-17T18:39:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "milestone_number": null,
    "milestone_title": "sage-6.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187250"
}
```



---

archive/issue_events_187251.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2013-12-17T18:39:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187251"
}
```



---

archive/issue_events_187252.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187252"
}
```



---

archive/issue_events_187253.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187253"
}
```



---

archive/issue_events_187254.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187254"
}
```



---

archive/issue_events_187255.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187255"
}
```



---

archive/issue_comments_176298.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/08f5ee0aa8c89827781849ef560bb228a770b8e7\">08f5ee0</a></td><td><code>Merge branch 'develop' into ticket/14432</code></td></tr></table>\n",
    "created_at": "2014-06-26T05:34:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14432#issuecomment-176298",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:14'>Comment 14:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/08f5ee0aa8c89827781849ef560bb228a770b8e7">08f5ee0</a></td><td><code>Merge branch 'develop' into ticket/14432</code></td></tr></table>




---

archive/issue_comments_176299.json:
```json
{
    "body": "Changed commit from **[e851c6f](https://github.com/sagemath/sagetrac-mirror/commit/e851c6f179661e67e79e8aac44aa82371e7ff963)** to **[08f5ee0](https://github.com/sagemath/sagetrac-mirror/commit/08f5ee0aa8c89827781849ef560bb228a770b8e7)**",
    "created_at": "2014-06-26T05:34:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14432#issuecomment-176299",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[e851c6f](https://github.com/sagemath/sagetrac-mirror/commit/e851c6f179661e67e79e8aac44aa82371e7ff963)** to **[08f5ee0](https://github.com/sagemath/sagetrac-mirror/commit/08f5ee0aa8c89827781849ef560bb228a770b8e7)**



---

archive/issue_events_187256.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-06-26T05:35:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187256"
}
```



---

archive/issue_events_187257.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2014-06-26T05:35:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187257"
}
```



---

archive/issue_comments_176300.json:
```json
{
    "body": "Changed keywords from **hash, set** to **hash, set, sd59**",
    "created_at": "2014-06-26T05:35:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14432#issuecomment-176300",
    "user": "https://github.com/saraedum"
}
```

Changed keywords from **hash, set** to **hash, set, sd59**



---

archive/issue_comments_176301.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2014-07-02T16:33:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14432#issuecomment-176301",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_176302.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nI've made some minor review tweaks. If you're happy with them, then positive review.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0190e4d1140c1f6c892b16f49e4670071a41a0b7\">0190e4d</a></td><td><code>Some review changes to set.py.</code></td></tr></table>\n",
    "created_at": "2014-07-02T16:33:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14432#issuecomment-176302",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:16'>Comment 16:</a>
I've made some minor review tweaks. If you're happy with them, then positive review.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0190e4d1140c1f6c892b16f49e4670071a41a0b7">0190e4d</a></td><td><code>Some review changes to set.py.</code></td></tr></table>




---

archive/issue_comments_176303.json:
```json
{
    "body": "Changed commit from **[08f5ee0](https://github.com/sagemath/sagetrac-mirror/commit/08f5ee0aa8c89827781849ef560bb228a770b8e7)** to **[0190e4d](https://github.com/sagemath/sagetrac-mirror/commit/0190e4d1140c1f6c892b16f49e4670071a41a0b7)**",
    "created_at": "2014-07-02T16:33:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14432#issuecomment-176303",
    "user": "https://github.com/tscrim"
}
```

Changed commit from **[08f5ee0](https://github.com/sagemath/sagetrac-mirror/commit/08f5ee0aa8c89827781849ef560bb228a770b8e7)** to **[0190e4d](https://github.com/sagemath/sagetrac-mirror/commit/0190e4d1140c1f6c892b16f49e4670071a41a0b7)**



---

archive/issue_comments_176304.json:
```json
{
    "body": "Changed branch from **[u/saraedum/ticket/14432](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/14432)** to **[public/ticket/14432](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/14432)**",
    "created_at": "2014-07-02T16:33:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14432#issuecomment-176304",
    "user": "https://github.com/tscrim"
}
```

Changed branch from **[u/saraedum/ticket/14432](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/ticket/14432)** to **[public/ticket/14432](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/14432)**



---

archive/issue_comments_176305.json:
```json
{
    "body": "Changed commit from **[0190e4d](https://github.com/sagemath/sagetrac-mirror/commit/0190e4d1140c1f6c892b16f49e4670071a41a0b7)** to **[05ed037](https://github.com/sagemath/sagetrac-mirror/commit/05ed0370f6a08b0e181f1f520b7df7ec6312a15c)**",
    "created_at": "2014-07-02T16:35:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14432#issuecomment-176305",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[0190e4d](https://github.com/sagemath/sagetrac-mirror/commit/0190e4d1140c1f6c892b16f49e4670071a41a0b7)** to **[05ed037](https://github.com/sagemath/sagetrac-mirror/commit/05ed0370f6a08b0e181f1f520b7df7ec6312a15c)**



---

archive/issue_comments_176306.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/05ed0370f6a08b0e181f1f520b7df7ec6312a15c\">05ed037</a></td><td><code>Fix typo.</code></td></tr></table>\n",
    "created_at": "2014-07-02T16:35:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14432#issuecomment-176306",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:17'>Comment 17:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/05ed0370f6a08b0e181f1f520b7df7ec6312a15c">05ed037</a></td><td><code>Fix typo.</code></td></tr></table>




---

archive/issue_events_187258.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187258"
}
```



---

archive/issue_events_187259.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187259"
}
```



---

archive/issue_events_187260.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2015-01-22T20:52:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187260"
}
```



---

archive/issue_events_187261.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2015-01-22T20:52:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187261"
}
```



---

archive/issue_comments_176307.json:
```json
{
    "body": "Changed branch from **[public/ticket/14432](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/14432)** to **[05ed037](https://github.com/sagemath/sagetrac-mirror/commit/05ed0370f6a08b0e181f1f520b7df7ec6312a15c)**",
    "created_at": "2015-01-24T13:18:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14432#issuecomment-176307",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/ticket/14432](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/14432)** to **[05ed037](https://github.com/sagemath/sagetrac-mirror/commit/05ed0370f6a08b0e181f1f520b7df7ec6312a15c)**



---

archive/issue_events_187262.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2015-01-24T13:18:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187262"
}
```



---

archive/issue_events_187263.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "cc5feb0908c2c889b10fef0e7055e44686df545c",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2015-01-24T13:18:41Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14432",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14432#event-187263"
}
```
