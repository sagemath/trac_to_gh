# Issue 14611: cleanup of tiling.py

archive/issues_014407.json:
```json
{
    "assignees": [],
    "body": "this ticket aims at cleaning the doc of combinat/tiling.py\n\n* remove one unused variable\n* remove trailing whitespaces\n* format the reference\n* put raise into python3 syntax\n* put code into pep8 standard\n* add role for arxiv link\n\n\n\nAssignee: **@sagetrac-sage-combinat**\n\nCC:  @seblabbe\n\nKeywords: **documentation, tiling**\n\nReviewer: **Nathann Cohen**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\nMerged: **sage-5.10.beta5**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14611_\n\n",
    "closed_at": "2013-05-22T08:20:33Z",
    "created_at": "2013-05-18T19:40:42Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.10",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "cleanup of tiling.py",
    "type": "issue",
    "updated_at": "2013-05-22T08:20:33Z",
    "url": "https://github.com/sagemath/sage/issues/14611",
    "user": "https://github.com/fchapoton"
}
```
this ticket aims at cleaning the doc of combinat/tiling.py

* remove one unused variable
* remove trailing whitespaces
* format the reference
* put raise into python3 syntax
* put code into pep8 standard
* add role for arxiv link



Assignee: **@sagetrac-sage-combinat**

CC:  @seblabbe

Keywords: **documentation, tiling**

Reviewer: **Nathann Cohen**

Author: **Frédéric Chapoton**

Merged: **sage-5.10.beta5**

_Issue created by migration from https://trac.sagemath.org/ticket/14611_





---

archive/issue_events_189925.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-18T19:40:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14611",
    "milestone_number": null,
    "milestone_title": "sage-5.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14611#event-189925"
}
```



---

archive/issue_events_189926.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-18T19:40:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14611",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14611#event-189926"
}
```



---

archive/issue_events_189927.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-18T19:40:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14611",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14611#event-189927"
}
```



---

archive/issue_events_189928.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-18T19:40:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14611",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14611#event-189928"
}
```



---

archive/issue_comments_179677.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nAttachment: **[trac_14611_clean_tilings-fc.patch.gz](https://github.com/sagemath/sage/files/ticket14611/trac_14611_clean_tilings-fc.patch.gz)**\n\nshould be good, pass all tests, needs review",
    "created_at": "2013-05-18T19:45:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14611",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14611#issuecomment-179677",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:1" align="right">Comment 1</div>

Attachment: **[trac_14611_clean_tilings-fc.patch.gz](https://github.com/sagemath/sage/files/ticket14611/trac_14611_clean_tilings-fc.patch.gz)**

should be good, pass all tests, needs review



---

archive/issue_events_189929.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-18T19:45:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14611",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14611#event-189929"
}
```



---

archive/issue_comments_179678.json:
```json
{
    "body": "Reviewer: **Nathann Cohen**",
    "created_at": "2013-05-20T10:59:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14611",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14611#issuecomment-179678",
    "user": "https://github.com/nathanncohen"
}
```

Reviewer: **Nathann Cohen**



---

archive/issue_events_189930.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-05-20T10:59:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14611",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14611#event-189930"
}
```



---

archive/issue_events_189931.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-05-20T10:59:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14611",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14611#event-189931"
}
```



---

archive/issue_comments_179679.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nGood to go ! I like to replace the `\"dict( (i,j) for i, j in whatever )\"` with `{i:j for i,j in whatever}` these days. Looks cooler `:-)`\n\nNathann",
    "created_at": "2013-05-20T10:59:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14611",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14611#issuecomment-179679",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:2" align="right">Comment 2</div>

Good to go ! I like to replace the `"dict( (i,j) for i, j in whatever )"` with `{i:j for i,j in whatever}` these days. Looks cooler `:-)`

Nathann



---

archive/issue_events_189932.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-22T08:20:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14611",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14611#event-189932"
}
```



---

archive/issue_events_189933.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-22T08:20:33Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14611",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14611#event-189933"
}
```



---

archive/issue_comments_179680.json:
```json
{
    "body": "Merged: **sage-5.10.beta5**",
    "created_at": "2013-05-22T08:20:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14611",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14611#issuecomment-179680",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.10.beta5**
