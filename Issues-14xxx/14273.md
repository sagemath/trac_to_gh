# Issue 14273: Remove workaround for sagenb pull request 84

archive/issues_014069.json:
```json
{
    "assignees": [],
    "body": "In #12415 we added\n\n```\n# Workarounds for https://github.com/sagemath/sagenb/pull/84\nimport sagenb.notebook.misc\nimport sagenb.notebook.sage_email\ndef fixed_default_email_address():\n    import socket\n    import getpass\n    return getpass.getuser() + \"@\" + socket.gethostname()\nsagenb.notebook.sage_email.default_email_address = fixed_default_email_address\n```\nto `sage.doctest.forker`.  These whould be removed once the newer version of sagenb is incorporated into Sage.\n\n\nDepends on #12415\nDepends on #14330\nDepends on #14266\n\nAssignee: **@roed314**\n\nCC:  @kini @jdemeyer\n\nBranch: **[u/tkluck/ticket/14273](https://github.com/sagemath/sagetrac-mirror/tree/u/tkluck/ticket/14273)**\n\nReviewer: **Jeroen Demeyer, Punarbasu Purkayastha**\n\nAuthor: **R. Andrew Ohana**\n\nMerged: **sage-5.12.beta0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14273_\n\n",
    "closed_at": "2013-08-02T14:13:45Z",
    "created_at": "2013-03-14T20:07:45Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20doctest%20framework",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.12",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove workaround for sagenb pull request 84",
    "type": "issue",
    "updated_at": "2013-08-02T14:13:45Z",
    "url": "https://github.com/sagemath/sage/issues/14273",
    "user": "https://github.com/roed314"
}
```
In #12415 we added

```
# Workarounds for https://github.com/sagemath/sagenb/pull/84
import sagenb.notebook.misc
import sagenb.notebook.sage_email
def fixed_default_email_address():
    import socket
    import getpass
    return getpass.getuser() + "@" + socket.gethostname()
sagenb.notebook.sage_email.default_email_address = fixed_default_email_address
```
to `sage.doctest.forker`.  These whould be removed once the newer version of sagenb is incorporated into Sage.


Depends on #12415
Depends on #14330
Depends on #14266

Assignee: **@roed314**

CC:  @kini @jdemeyer

Branch: **[u/tkluck/ticket/14273](https://github.com/sagemath/sagetrac-mirror/tree/u/tkluck/ticket/14273)**

Reviewer: **Jeroen Demeyer, Punarbasu Purkayastha**

Author: **R. Andrew Ohana**

Merged: **sage-5.12.beta0**

_Issue created by migration from https://trac.sagemath.org/ticket/14273_





---

archive/issue_events_184619.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2013-03-14T20:07:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "milestone_number": null,
    "milestone_title": "sage-5.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14273#event-184619"
}
```



---

archive/issue_events_184620.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2013-03-14T20:07:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14273#event-184620"
}
```



---

archive/issue_events_184621.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2013-03-14T20:07:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14273#event-184621"
}
```



---

archive/issue_comments_173369.json:
```json
{
    "body": "Changed dependencies from **#12415** to **#12415, #14330**",
    "created_at": "2013-03-22T08:07:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14273#issuecomment-173369",
    "user": "https://github.com/jdemeyer"
}
```

Changed dependencies from **#12415** to **#12415, #14330**



---

archive/issue_comments_173370.json:
```json
{
    "body": "Changed assignee from **@sagetrac-mvngu** to **@roed314**",
    "created_at": "2013-03-28T22:45:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14273#issuecomment-173370",
    "user": "https://github.com/roed314"
}
```

Changed assignee from **@sagetrac-mvngu** to **@roed314**



---

archive/issue_events_184622.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2013-03-28T22:45:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "label": "https://github.com/sagemath/sage/labels/doctest%20coverage",
    "label_color": "696969",
    "label_name": "doctest coverage",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14273#event-184622"
}
```



---

archive/issue_events_184623.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2013-03-28T22:45:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20doctest%20framework",
    "label_color": "000080",
    "label_name": "component: doctest framework",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14273#event-184623"
}
```



---

archive/issue_events_184624.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2013-04-10T06:13:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14273#event-184624"
}
```



---

archive/issue_comments_173371.json:
```json
{
    "body": "Author: **R. Andrew Ohana**",
    "created_at": "2013-04-10T06:13:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14273#issuecomment-173371",
    "user": "https://github.com/ohanar"
}
```

Author: **R. Andrew Ohana**



---

archive/issue_comments_173372.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2013-04-10T08:17:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14273#issuecomment-173372",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_comments_173373.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nAssuming that [https://github.com/sagemath/sagenb/pull/84](https://github.com/sagemath/sagenb/pull/84) is merged, this patch is obviously fine.",
    "created_at": "2013-04-10T08:17:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14273#issuecomment-173373",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:4" align="right">Comment 4</div>

Assuming that [https://github.com/sagemath/sagenb/pull/84](https://github.com/sagemath/sagenb/pull/84) is merged, this patch is obviously fine.



---

archive/issue_events_184625.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-04-10T08:17:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14273#event-184625"
}
```



---

archive/issue_events_184626.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-04-10T08:17:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14273#event-184626"
}
```



---

archive/issue_events_184627.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-04-10T08:17:54Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "milestone_number": null,
    "milestone_title": "sage-5.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14273#event-184627"
}
```



---

archive/issue_events_184628.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-04-10T08:17:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "label": "https://github.com/sagemath/sage/labels/pending",
    "label_color": "008080",
    "label_name": "pending",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14273#event-184628"
}
```



---

archive/issue_comments_173374.json:
```json
{
    "body": "apply to sage library",
    "created_at": "2013-06-08T11:35:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14273#issuecomment-173374",
    "user": "https://github.com/jdemeyer"
}
```

apply to sage library



---

archive/issue_comments_173375.json:
```json
{
    "body": "Changed dependencies from **#12415, #14330** to **#12415, #14330, #14266**",
    "created_at": "2013-06-08T11:36:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14273#issuecomment-173375",
    "user": "https://github.com/jdemeyer"
}
```

Changed dependencies from **#12415, #14330** to **#12415, #14330, #14266**



---

archive/issue_comments_173376.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nAttachment: **[trac14273.patch.gz](https://github.com/sagemath/sage/files/ticket14273/trac14273.patch.gz)**\n\nRebased to #14266 and removed an additional workaround in `sage/doctests/control.py`",
    "created_at": "2013-06-08T11:36:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14273#issuecomment-173376",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:5" align="right">Comment 5</div>

Attachment: **[trac14273.patch.gz](https://github.com/sagemath/sage/files/ticket14273/trac14273.patch.gz)**

Rebased to #14266 and removed an additional workaround in `sage/doctests/control.py`



---

archive/issue_events_184629.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-08T11:36:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14273#event-184629"
}
```



---

archive/issue_events_184630.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-08T11:36:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14273#event-184630"
}
```



---

archive/issue_events_184631.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-08T11:36:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14273#event-184631"
}
```



---

archive/issue_events_184632.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-08T11:36:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14273#event-184632"
}
```



---

archive/issue_events_184633.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-14T07:18:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14273#event-184633"
}
```



---

archive/issue_events_184634.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-14T07:18:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "label": "https://github.com/sagemath/sage/labels/pending",
    "label_color": "008080",
    "label_name": "pending",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14273#event-184634"
}
```



---

archive/issue_comments_173377.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nThese changes were apparently used as workarounds for the old sagenb in #12415. I ran the following\n\n```\n...allations/sage-5.11.beta3\u00bb ./sage -btp 2 --sagenb\n```\nand all the tests passed. So, removing this workaround looks ok to me now.",
    "created_at": "2013-06-30T12:57:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14273#issuecomment-173377",
    "user": "https://github.com/ppurka"
}
```

<div id="comment:8" align="right">Comment 8</div>

These changes were apparently used as workarounds for the old sagenb in #12415. I ran the following

```
...allations/sage-5.11.beta3» ./sage -btp 2 --sagenb
```
and all the tests passed. So, removing this workaround looks ok to me now.



---

archive/issue_comments_173378.json:
```json
{
    "body": "Changed reviewer from **Jeroen Demeyer** to **Jeroen Demeyer, Punarbasu Purkayastha**",
    "created_at": "2013-06-30T12:58:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14273#issuecomment-173378",
    "user": "https://github.com/ppurka"
}
```

Changed reviewer from **Jeroen Demeyer** to **Jeroen Demeyer, Punarbasu Purkayastha**



---

archive/issue_events_184635.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2013-06-30T12:58:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14273#event-184635"
}
```



---

archive/issue_events_184636.json:
```json
{
    "actor": "https://github.com/ppurka",
    "created_at": "2013-06-30T12:58:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14273#event-184636"
}
```



---

archive/issue_events_184637.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-07-21T21:40:24Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14273#event-184637"
}
```



---

archive/issue_events_184638.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-07-21T21:40:24Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14273#event-184638"
}
```



---

archive/issue_comments_173379.json:
```json
{
    "body": "Branch: **[u/tkluck/ticket/14273](https://github.com/sagemath/sagetrac-mirror/tree/u/tkluck/ticket/14273)**",
    "created_at": "2013-07-23T10:12:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14273#issuecomment-173379",
    "user": "https://github.com/tkluck"
}
```

Branch: **[u/tkluck/ticket/14273](https://github.com/sagemath/sagetrac-mirror/tree/u/tkluck/ticket/14273)**



---

archive/issue_events_184639.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-02T14:13:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14273#event-184639"
}
```



---

archive/issue_events_184640.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-02T14:13:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14273#event-184640"
}
```



---

archive/issue_comments_173380.json:
```json
{
    "body": "Merged: **sage-5.12.beta0**",
    "created_at": "2013-08-02T14:13:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14273",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14273#issuecomment-173380",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.12.beta0**
