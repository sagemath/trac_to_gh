# Issue 14735: Bug with Python int division in wigner.py

archive/issues_014531.json:
```json
{
    "assignees": [],
    "body": "From [this ask.sagemath question](http://ask.sagemath.org/question/2683/different-answers-with-gaunt):\n\n```\nsage: gaunt(1,1,1,0,1,-1)\n0\nsage: gaunt(int(1),int(1),int(1),int(0),int(1),int(-1))\n1/2*sqrt(3)/sqrt(pi)\n```\nThis can come up when iterating over `range` instead of `srange`, which should be supported behavior (in my opinion, anyway).  As far as I can tell, the only place this is an issue is\n\n```\n    bigL = (l_1 + l_2 + l_3) / 2\n```\nbut you never know.\n\nI'm putting this under \"calculus\" but that doesn't really feel right.\n\nAssignee: **@burcin**\n\nCC:  @paulmasson @fchapoton\n\nBranch: **[1a5a5ad](https://github.com/sagemath/sagetrac-mirror/commit/1a5a5ad174d07e3acc62625903741b656a333977)**\n\nReviewer: **Matthias Koeppe**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14735_\n\n",
    "closed_at": "2020-09-06T21:51:12Z",
    "created_at": "2013-06-12T15:50:53Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20calculus",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Bug with Python int division in wigner.py",
    "type": "issue",
    "updated_at": "2020-09-08T06:17:13Z",
    "url": "https://github.com/sagemath/sage/issues/14735",
    "user": "https://github.com/kcrisman"
}
```
From [this ask.sagemath question](http://ask.sagemath.org/question/2683/different-answers-with-gaunt):

```
sage: gaunt(1,1,1,0,1,-1)
0
sage: gaunt(int(1),int(1),int(1),int(0),int(1),int(-1))
1/2*sqrt(3)/sqrt(pi)
```
This can come up when iterating over `range` instead of `srange`, which should be supported behavior (in my opinion, anyway).  As far as I can tell, the only place this is an issue is

```
    bigL = (l_1 + l_2 + l_3) / 2
```
but you never know.

I'm putting this under "calculus" but that doesn't really feel right.

Assignee: **@burcin**

CC:  @paulmasson @fchapoton

Branch: **[1a5a5ad](https://github.com/sagemath/sagetrac-mirror/commit/1a5a5ad174d07e3acc62625903741b656a333977)**

Reviewer: **Matthias Koeppe**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/14735_





---

archive/issue_events_192042.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-06-12T15:50:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14735#event-192042"
}
```



---

archive/issue_events_192043.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-06-12T15:50:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20calculus",
    "label_color": "0000ff",
    "label_name": "component: calculus",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14735#event-192043"
}
```



---

archive/issue_events_192044.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-06-12T15:50:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14735#event-192044"
}
```



---

archive/issue_events_192045.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2013-06-12T15:50:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14735#event-192045"
}
```



---

archive/issue_comments_182187.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,4 @@\n+From [this ask.sagemath question](http://ask.sagemath.org/question/2683/different-answers-with-gaunt):\n \n ```\n sage: gaunt(1,1,1,0,1,-1)\n``````\n",
    "created_at": "2013-06-12T15:53:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14735#issuecomment-182187",
    "user": "https://github.com/kcrisman"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,4 @@
+From [this ask.sagemath question](http://ask.sagemath.org/question/2683/different-answers-with-gaunt):
 
 ```
 sage: gaunt(1,1,1,0,1,-1)
``````




---

archive/issue_events_192046.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14735#event-192046"
}
```



---

archive/issue_events_192047.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14735#event-192047"
}
```



---

archive/issue_events_192048.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14735#event-192048"
}
```



---

archive/issue_events_192049.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14735#event-192049"
}
```



---

archive/issue_events_192050.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14735#event-192050"
}
```



---

archive/issue_events_192051.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14735#event-192051"
}
```



---

archive/issue_events_192052.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14735#event-192052"
}
```



---

archive/issue_events_192053.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14735#event-192053"
}
```



---

archive/issue_events_192054.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T18:45:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14735#event-192054"
}
```



---

archive/issue_events_192055.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T18:45:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14735#event-192055"
}
```



---

archive/issue_events_192056.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T18:45:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14735#event-192056"
}
```



---

archive/issue_events_192057.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-08-29T18:45:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14735#event-192057"
}
```



---

archive/issue_comments_182188.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nThis seems to work in 9.2.beta10",
    "created_at": "2020-08-29T18:45:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14735#issuecomment-182188",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:7" align="right">Comment 7</div>

This seems to work in 9.2.beta10



---

archive/issue_comments_182189.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nDoctest.  Perhaps our coercion improved?  I don't know anything about `gaunt` other than that I apparently filed this ticket.",
    "created_at": "2020-08-29T20:39:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14735#issuecomment-182189",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:8" align="right">Comment 8</div>

Doctest.  Perhaps our coercion improved?  I don't know anything about `gaunt` other than that I apparently filed this ticket.



---

archive/issue_events_192058.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2020-08-29T20:40:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14735#event-192058"
}
```



---

archive/issue_events_192059.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2020-08-29T20:40:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14735#event-192059"
}
```



---

archive/issue_comments_182190.json:
```json
{
    "body": "Author: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2020-08-31T19:28:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14735#issuecomment-182190",
    "user": "https://github.com/fchapoton"
}
```

Author: **Frédéric Chapoton**



---

archive/issue_events_192060.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-08-31T19:28:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14735#event-192060"
}
```



---

archive/issue_events_192061.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-08-31T19:28:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14735#event-192061"
}
```



---

archive/issue_comments_182191.json:
```json
{
    "body": "Branch: **[u/chapoton/14735](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/14735)**",
    "created_at": "2020-08-31T19:28:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14735#issuecomment-182191",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/14735](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/14735)**



---

archive/issue_comments_182192.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1a5a5ad174d07e3acc62625903741b656a333977\">1a5a5ad</a></td><td><code>add one doctest about gaunt</code></td></tr></table>\n",
    "created_at": "2020-08-31T19:28:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14735#issuecomment-182192",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:10" align="right">Comment 10</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1a5a5ad174d07e3acc62625903741b656a333977">1a5a5ad</a></td><td><code>add one doctest about gaunt</code></td></tr></table>




---

archive/issue_comments_182193.json:
```json
{
    "body": "Commit: **[1a5a5ad](https://github.com/sagemath/sagetrac-mirror/commit/1a5a5ad174d07e3acc62625903741b656a333977)**",
    "created_at": "2020-08-31T19:28:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14735#issuecomment-182193",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[1a5a5ad](https://github.com/sagemath/sagetrac-mirror/commit/1a5a5ad174d07e3acc62625903741b656a333977)**



---

archive/issue_events_192062.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2020-08-31T19:28:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14735#event-192062"
}
```



---

archive/issue_comments_182194.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2020-09-01T22:16:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14735#issuecomment-182194",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_events_192063.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-01T22:16:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14735#event-192063"
}
```



---

archive/issue_events_192064.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-01T22:16:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14735#event-192064"
}
```



---

archive/issue_events_192065.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2020-09-06T21:51:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14735#event-192065"
}
```



---

archive/issue_events_192066.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "21f7af35b43704c610df9bd052d72490e6abb614",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2020-09-06T21:51:12Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14735#event-192066"
}
```



---

archive/issue_comments_182195.json:
```json
{
    "body": "Changed branch from **[u/chapoton/14735](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/14735)** to **[1a5a5ad](https://github.com/sagemath/sagetrac-mirror/commit/1a5a5ad174d07e3acc62625903741b656a333977)**",
    "created_at": "2020-09-06T21:51:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14735#issuecomment-182195",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/14735](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/14735)** to **[1a5a5ad](https://github.com/sagemath/sagetrac-mirror/commit/1a5a5ad174d07e3acc62625903741b656a333977)**



---

archive/issue_events_192067.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-08T06:17:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "milestone_number": null,
    "milestone_title": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14735#event-192067"
}
```



---

archive/issue_events_192068.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-09-08T06:17:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "milestone_number": null,
    "milestone_title": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14735#event-192068"
}
```



---

archive/issue_comments_182196.json:
```json
{
    "body": "Changed commit from **[1a5a5ad](https://github.com/sagemath/sagetrac-mirror/commit/1a5a5ad174d07e3acc62625903741b656a333977)** to none",
    "created_at": "2020-09-08T06:17:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14735",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14735#issuecomment-182196",
    "user": "https://github.com/mkoeppe"
}
```

Changed commit from **[1a5a5ad](https://github.com/sagemath/sagetrac-mirror/commit/1a5a5ad174d07e3acc62625903741b656a333977)** to none
