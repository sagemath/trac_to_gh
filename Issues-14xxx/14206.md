# Issue 14206: random_element() of number fields ignores exceptions

archive/issues_014002.json:
```json
{
    "assignees": [],
    "body": "The following code crashes Sage:\n\n```\nQQ[I].random_element(-1) + 1\n```\n\nAssignee: **@loefflerd**\n\nKeywords: **segfault, random_element**\n\nReviewer: **Luis Felipe Tabera Alonso**\n\nAuthor: **Jeroen Demeyer**\n\nMerged: **sage-5.9.beta2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14206_\n\n",
    "closed_at": "2013-03-28T17:59:29Z",
    "created_at": "2013-03-01T10:01:51Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "random_element() of number fields ignores exceptions",
    "type": "issue",
    "updated_at": "2013-03-28T17:59:29Z",
    "url": "https://github.com/sagemath/sage/issues/14206",
    "user": "https://github.com/lftabera"
}
```
The following code crashes Sage:

```
QQ[I].random_element(-1) + 1
```

Assignee: **@loefflerd**

Keywords: **segfault, random_element**

Reviewer: **Luis Felipe Tabera Alonso**

Author: **Jeroen Demeyer**

Merged: **sage-5.9.beta2**

_Issue created by migration from https://trac.sagemath.org/ticket/14206_





---

archive/issue_events_183491.json:
```json
{
    "actor": "https://github.com/lftabera",
    "created_at": "2013-03-01T10:01:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14206",
    "milestone_number": null,
    "milestone_title": "sage-5.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14206#event-183491"
}
```



---

archive/issue_events_183492.json:
```json
{
    "actor": "https://github.com/lftabera",
    "created_at": "2013-03-01T10:01:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14206",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
    "label_color": "0000ff",
    "label_name": "component: number fields",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14206#event-183492"
}
```



---

archive/issue_events_183493.json:
```json
{
    "actor": "https://github.com/lftabera",
    "created_at": "2013-03-01T10:01:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14206",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14206#event-183493"
}
```



---

archive/issue_events_183494.json:
```json
{
    "actor": "https://github.com/lftabera",
    "created_at": "2013-03-01T10:01:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14206",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14206#event-183494"
}
```



---

archive/issue_comments_171960.json:
```json
{
    "body": "Author: **Jeroen Demeyer**",
    "created_at": "2013-03-14T10:00:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14206",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14206#issuecomment-171960",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Jeroen Demeyer**



---

archive/issue_events_183495.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-14T10:00:12Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/14206",
    "title_is": "random_element() of number fields ignores exceptions",
    "title_was": "Sage segfault with crafted random elements of number field",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14206#event-183495"
}
```



---

archive/issue_comments_171961.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-The following code crashes Sage\n+The following code crashes Sage:\n \n ```\n QQ[I].random_element(-1) + 1\n``````\n",
    "created_at": "2013-03-14T10:00:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14206",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14206#issuecomment-171961",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-The following code crashes Sage
+The following code crashes Sage:
 
 ```
 QQ[I].random_element(-1) + 1
``````




---

archive/issue_events_183496.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-14T10:19:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14206",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14206#event-183496"
}
```



---

archive/issue_comments_171962.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nAttachment: **[14206_nfeltquad_randomize.patch.gz](https://github.com/sagemath/sage/files/ticket14206/14206_nfeltquad_randomize.patch.gz)**",
    "created_at": "2013-03-14T10:19:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14206",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14206#issuecomment-171962",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'>Comment 2:</a>
Attachment: **[14206_nfeltquad_randomize.patch.gz](https://github.com/sagemath/sage/files/ticket14206/14206_nfeltquad_randomize.patch.gz)**



---

archive/issue_events_183497.json:
```json
{
    "actor": "https://github.com/lftabera",
    "created_at": "2013-03-26T11:53:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14206",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14206#event-183497"
}
```



---

archive/issue_events_183498.json:
```json
{
    "actor": "https://github.com/lftabera",
    "created_at": "2013-03-26T11:53:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14206",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14206#event-183498"
}
```



---

archive/issue_comments_171963.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nPositive review, despite the output of the patchbot the patch applies to 5.8 and 5.9.beta1",
    "created_at": "2013-03-26T11:53:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14206",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14206#issuecomment-171963",
    "user": "https://github.com/lftabera"
}
```

<a id='comment:3'>Comment 3:</a>
Positive review, despite the output of the patchbot the patch applies to 5.8 and 5.9.beta1



---

archive/issue_comments_171964.json:
```json
{
    "body": "Reviewer: **Luis Felipe Tabera Alonso**",
    "created_at": "2013-03-26T11:53:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14206",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14206#issuecomment-171964",
    "user": "https://github.com/lftabera"
}
```

Reviewer: **Luis Felipe Tabera Alonso**



---

archive/issue_events_183499.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-28T17:59:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14206",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14206#event-183499"
}
```



---

archive/issue_events_183500.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-28T17:59:29Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14206",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14206#event-183500"
}
```



---

archive/issue_comments_171965.json:
```json
{
    "body": "Merged: **sage-5.9.beta2**",
    "created_at": "2013-03-28T17:59:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14206",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14206#issuecomment-171965",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.9.beta2**
