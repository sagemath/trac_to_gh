# Issue 14451: # tol disables the meaning of ellipsis in doctesting

archive/issues_014247.json:
```json
{
    "assignees": [
        "https://github.com/roed314"
    ],
    "body": "To reproduce the behavior take e.g. the doctest starting at sage/numerical/mip.pyx:2031 (`p.solve() # tol 0.00001`) and replace in the expected output the version string (`v4.44`) with the ellipsis (`...`). The test fails as long as it is marked with the `# tol` attribute. See also dicussion at https://groups.google.com/forum/#!topic/sage-devel/E2kwJLVaJx4.\n\n\nReviewer: **Jeroen Demeyer**\n\nComponent: **doctest framework**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14451_\n\n",
    "closed_at": "2014-10-13T15:50:54Z",
    "created_at": "2013-04-15T08:49:09Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20doctest%20framework",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "# tol disables the meaning of ellipsis in doctesting",
    "type": "issue",
    "updated_at": "2014-10-13T15:50:54Z",
    "url": "https://github.com/sagemath/sage/issues/14451",
    "user": "https://github.com/sagetrac-cschwan"
}
```
To reproduce the behavior take e.g. the doctest starting at sage/numerical/mip.pyx:2031 (`p.solve() # tol 0.00001`) and replace in the expected output the version string (`v4.44`) with the ellipsis (`...`). The test fails as long as it is marked with the `# tol` attribute. See also dicussion at https://groups.google.com/forum/#!topic/sage-devel/E2kwJLVaJx4.


Reviewer: **Jeroen Demeyer**

Component: **doctest framework**

_Issue created by migration from https://trac.sagemath.org/ticket/14451_





---

archive/issue_events_204376.json:
```json
{
    "actor": "https://github.com/sagetrac-cschwan",
    "created_at": "2013-04-15T08:49:09Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14451",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14451#event-204376"
}
```



---

archive/issue_events_204377.json:
```json
{
    "actor": "https://github.com/sagetrac-cschwan",
    "created_at": "2013-04-15T08:49:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14451",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20doctest%20framework",
    "label_color": "000080",
    "label_name": "c: doctest framework",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14451#event-204377"
}
```



---

archive/issue_events_204378.json:
```json
{
    "actor": "https://github.com/sagetrac-cschwan",
    "created_at": "2013-04-15T08:49:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14451",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14451#event-204378"
}
```



---

archive/issue_events_204379.json:
```json
{
    "actor": "https://github.com/sagetrac-cschwan",
    "created_at": "2013-04-15T08:49:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14451",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14451#event-204379"
}
```



---

archive/issue_events_204380.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2013-04-15T08:49:09Z",
    "event": "assigned",
    "issue": "https://github.com/sagemath/sage/issues/14451",
    "subject": "https://github.com/sagetrac-cschwan",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14451#event-204380"
}
```



---

archive/issue_events_204381.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14451",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14451#event-204381"
}
```



---

archive/issue_events_204382.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14451",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14451#event-204382"
}
```



---

archive/issue_events_204383.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14451",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14451#event-204383"
}
```



---

archive/issue_events_204384.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14451",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14451#event-204384"
}
```



---

archive/issue_events_204385.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14451",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14451#event-204385"
}
```



---

archive/issue_events_204386.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14451",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14451#event-204386"
}
```



---

archive/issue_events_204387.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14451",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14451#event-204387"
}
```



---

archive/issue_events_204388.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14451",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14451#event-204388"
}
```



---

archive/issue_comments_174158.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2014-10-01T13:03:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14451#issuecomment-174158",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_204389.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-01T13:03:09Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14451",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14451#event-204389"
}
```



---

archive/issue_events_204390.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-01T13:03:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14451",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14451#event-204390"
}
```



---

archive/issue_events_204391.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-01T13:03:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14451",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14451#event-204391"
}
```



---

archive/issue_comments_174159.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">comment:5</div>\n\nI agree this is a bug, however it looks quite tricky to fix. As long as the `...` doesn't match any floats, it should work fine. The problem is that the code which handles floats and the code which handles `...` is completely separate. I think this is too hard to get right, so I propose \"wontfix\".",
    "created_at": "2014-10-01T13:03:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14451#issuecomment-174159",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:5" align="right">comment:5</div>

I agree this is a bug, however it looks quite tricky to fix. As long as the `...` doesn't match any floats, it should work fine. The problem is that the code which handles floats and the code which handles `...` is completely separate. I think this is too hard to get right, so I propose "wontfix".



---

archive/issue_events_204392.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-01T13:03:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14451",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14451#event-204392"
}
```



---

archive/issue_events_204393.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-01T13:03:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14451",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14451#event-204393"
}
```



---

archive/issue_events_204394.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2014-10-01T13:03:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14451",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14451#event-204394"
}
```



---

archive/issue_events_204395.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-13T15:50:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14451",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14451#event-204395"
}
```



---

archive/issue_events_204396.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-13T15:50:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14451",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14451#event-204396"
}
```



---

archive/issue_events_204397.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-10-13T15:50:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14451",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14451#event-204397"
}
```
