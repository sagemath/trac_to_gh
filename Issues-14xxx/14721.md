# Issue 14721: Disable "cythonizing out-of-tree" until sage-sync-build and sage-clone can deal with that

archive/issues_014517.json:
```json
{
    "assignees": [],
    "body": "See #13245 and the sage-devel thread on useless rebuilding of the Sage library ...\n\nThe patch effectively reverts the change to `setup.py` made by [attachment: ticket:14570:trac14570.patch.gz](https://github.com/sagemath/sage/files/ticket14721/eda7b1d72a76e585dcb876a3e5981075.gz) (btw. the only functional change of that patch).\n   \n---\n\n**Apply:** [attachment: trac_14721-postpone_cythonizing_out_of_tree.sagelib.patch.gz](https://github.com/sagemath/sage/files/ticket14721/trac_14721-postpone_cythonizing_out_of_tree.sagelib.patch.gz)\n\n\nAssignee: **GeorgSWeber**\n\nKeywords: **useless rebuild library build_dir**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Leif Leonhardy**\n\nMerged: **sage-5.10.rc2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14721_\n\n",
    "closed_at": "2013-06-13T08:21:04Z",
    "created_at": "2013-06-11T16:47:25Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.10",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Disable \"cythonizing out-of-tree\" until sage-sync-build and sage-clone can deal with that",
    "type": "issue",
    "updated_at": "2013-06-13T08:21:04Z",
    "url": "https://github.com/sagemath/sage/issues/14721",
    "user": "https://github.com/nexttime"
}
```
See #13245 and the sage-devel thread on useless rebuilding of the Sage library ...

The patch effectively reverts the change to `setup.py` made by [attachment: ticket:14570:trac14570.patch.gz](https://github.com/sagemath/sage/files/ticket14721/eda7b1d72a76e585dcb876a3e5981075.gz) (btw. the only functional change of that patch).
   
---

**Apply:** [attachment: trac_14721-postpone_cythonizing_out_of_tree.sagelib.patch.gz](https://github.com/sagemath/sage/files/ticket14721/trac_14721-postpone_cythonizing_out_of_tree.sagelib.patch.gz)


Assignee: **GeorgSWeber**

Keywords: **useless rebuild library build_dir**

Reviewer: **Travis Scrimshaw**

Author: **Leif Leonhardy**

Merged: **sage-5.10.rc2**

_Issue created by migration from https://trac.sagemath.org/ticket/14721_





---

archive/issue_events_191820.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2013-06-11T16:47:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14721",
    "milestone_number": null,
    "milestone_title": "sage-5.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14721#event-191820"
}
```



---

archive/issue_events_191821.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2013-06-11T16:47:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14721",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14721#event-191821"
}
```



---

archive/issue_events_191822.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2013-06-11T16:47:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14721",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14721#event-191822"
}
```



---

archive/issue_events_191823.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2013-06-11T16:47:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14721",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14721#event-191823"
}
```



---

archive/issue_comments_181920.json:
```json
{
    "body": "Changed keywords from **** to **useless rebuild library build_dir**",
    "created_at": "2013-06-11T16:50:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14721#issuecomment-181920",
    "user": "https://github.com/nexttime"
}
```

Changed keywords from **** to **useless rebuild library build_dir**



---

archive/issue_comments_181921.json:
```json
{
    "body": "Attachment: **[trac_14721-postpone_cythonizing_out_of_tree.sagelib.patch.gz](https://github.com/sagemath/sage/files/ticket14721/trac_14721-postpone_cythonizing_out_of_tree.sagelib.patch.gz)**\n\nApply to the Sage library.  Based on Sage 5.10.rc1.",
    "created_at": "2013-06-11T16:55:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14721#issuecomment-181921",
    "user": "https://github.com/nexttime"
}
```

Attachment: **[trac_14721-postpone_cythonizing_out_of_tree.sagelib.patch.gz](https://github.com/sagemath/sage/files/ticket14721/trac_14721-postpone_cythonizing_out_of_tree.sagelib.patch.gz)**

Apply to the Sage library.  Based on Sage 5.10.rc1.



---

archive/issue_comments_181922.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,6 @@\n+See #13245 and ....\n \n+---\n+\n+**Apply:** [attachment: trac_14721-postpone_cythonizing_out_of_tree.sagelib.patch.gz](https://github.com/sagemath/sage/files/ticket14721/trac_14721-postpone_cythonizing_out_of_tree.sagelib.patch.gz)\n+\n``````\n",
    "created_at": "2013-06-11T16:57:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14721#issuecomment-181922",
    "user": "https://github.com/nexttime"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,6 @@
+See #13245 and ....
 
+---
+
+**Apply:** [attachment: trac_14721-postpone_cythonizing_out_of_tree.sagelib.patch.gz](https://github.com/sagemath/sage/files/ticket14721/trac_14721-postpone_cythonizing_out_of_tree.sagelib.patch.gz)
+
``````




---

archive/issue_events_191824.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2013-06-11T16:57:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14721",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14721#event-191824"
}
```



---

archive/issue_events_191825.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-06-11T17:15:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14721",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14721#event-191825"
}
```



---

archive/issue_events_191826.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-06-11T17:15:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14721",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14721#event-191826"
}
```



---

archive/issue_comments_181923.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2013-06-11T17:15:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14721#issuecomment-181923",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_181924.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThis worked for me (with possibly #13245).\n\nEdit wrt [comment:5](#comment%3A5) - Yes I did",
    "created_at": "2013-06-11T17:15:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14721#issuecomment-181924",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:3'>Comment 3:</a>
This worked for me (with possibly #13245).

Edit wrt [comment:5](#comment%3A5) - Yes I did



---

archive/issue_comments_181925.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nThanks for looking into this Leif.",
    "created_at": "2013-06-11T17:16:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14721#issuecomment-181925",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:4'>Comment 4:</a>
Thanks for looking into this Leif.



---

archive/issue_comments_181926.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nReplying to [@tscrim](#comment%3A3):\n> This worked for me (with possibly #13425).\n\nI guess you meant #13245.",
    "created_at": "2013-06-11T17:19:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14721#issuecomment-181926",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:5'>Comment 5:</a>
Replying to [@tscrim](#comment%3A3):
> This worked for me (with possibly #13425).

I guess you meant #13245.



---

archive/issue_comments_181927.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,5 +1,7 @@\n-See #13245 and ....\n+See #13245 and the sage-devel thread on useless rebuilding of the Sage library ...\n \n+The patch effectively reverts the change to `setup.py` made by [attachment: ticket:14570:trac14570.patch.gz](https://github.com/sagemath/sage/files/ticket14721/eda7b1d72a76e585dcb876a3e5981075.gz) (btw. the only functional change of that patch).\n+   \n ---\n \n **Apply:** [attachment: trac_14721-postpone_cythonizing_out_of_tree.sagelib.patch.gz](https://github.com/sagemath/sage/files/ticket14721/trac_14721-postpone_cythonizing_out_of_tree.sagelib.patch.gz)\n``````\n",
    "created_at": "2013-06-11T17:20:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14721#issuecomment-181927",
    "user": "https://github.com/nexttime"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,5 +1,7 @@
-See #13245 and ....
+See #13245 and the sage-devel thread on useless rebuilding of the Sage library ...
 
+The patch effectively reverts the change to `setup.py` made by [attachment: ticket:14570:trac14570.patch.gz](https://github.com/sagemath/sage/files/ticket14721/eda7b1d72a76e585dcb876a3e5981075.gz) (btw. the only functional change of that patch).
+   
 ---
 
 **Apply:** [attachment: trac_14721-postpone_cythonizing_out_of_tree.sagelib.patch.gz](https://github.com/sagemath/sage/files/ticket14721/trac_14721-postpone_cythonizing_out_of_tree.sagelib.patch.gz)
``````




---

archive/issue_events_191827.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-13T08:21:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14721",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14721#event-191827"
}
```



---

archive/issue_events_191828.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-13T08:21:04Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14721",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14721#event-191828"
}
```



---

archive/issue_comments_181928.json:
```json
{
    "body": "Merged: **sage-5.10.rc2**",
    "created_at": "2013-06-13T08:21:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14721",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14721#issuecomment-181928",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.10.rc2**
