# Issue 14275: Lazy imports with deprecation

archive/issues_014071.json:
```json
{
    "assignees": [],
    "body": "It's annoying when other people move something around in the Sage library and it's no longer available in the previous location to be imported.  Sage currently has some mechanisms for alleviating this problem (`sage.misc.superceded.deprecated_callable_import` and `sage.structure.sage_object.register_unpickle_override` for example).  This ticket adds another option: lazily import the old name so that a deprecation warning is issued whenever it is referred to.\n\nAssignee: **@jasongrout**\n\nBranch/Commit: **[a484809](https://github.com/sagemath/sagetrac-mirror/commit/a4848093884df99a12d031dc853ce9d97c0c3dc2)**\n\nReviewer: **Travis Scrimshaw, Nathann Cohen**\n\nAuthor: **David Roe, Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14275_\n\n",
    "closed_at": "2014-04-04T15:55:31Z",
    "created_at": "2013-03-14T21:30:00Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-6.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Lazy imports with deprecation",
    "type": "issue",
    "updated_at": "2014-04-04T15:55:31Z",
    "url": "https://github.com/sagemath/sage/issues/14275",
    "user": "https://github.com/roed314"
}
```
It's annoying when other people move something around in the Sage library and it's no longer available in the previous location to be imported.  Sage currently has some mechanisms for alleviating this problem (`sage.misc.superceded.deprecated_callable_import` and `sage.structure.sage_object.register_unpickle_override` for example).  This ticket adds another option: lazily import the old name so that a deprecation warning is issued whenever it is referred to.

Assignee: **@jasongrout**

Branch/Commit: **[a484809](https://github.com/sagemath/sagetrac-mirror/commit/a4848093884df99a12d031dc853ce9d97c0c3dc2)**

Reviewer: **Travis Scrimshaw, Nathann Cohen**

Author: **David Roe, Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/14275_





---

archive/issue_events_184654.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2013-03-14T21:30:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14275#event-184654"
}
```



---

archive/issue_events_184655.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2013-03-14T21:30:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14275#event-184655"
}
```



---

archive/issue_events_184656.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2013-03-14T21:30:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14275#event-184656"
}
```



---

archive/issue_comments_173388.json:
```json
{
    "body": "Attachment: **[14275.patch.gz](https://github.com/sagemath/sage/files/ticket14275/14275.patch.gz)**",
    "created_at": "2013-03-14T21:32:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14275#issuecomment-173388",
    "user": "https://github.com/roed314"
}
```

Attachment: **[14275.patch.gz](https://github.com/sagemath/sage/files/ticket14275/14275.patch.gz)**



---

archive/issue_events_184657.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2013-03-14T21:36:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14275#event-184657"
}
```



---

archive/issue_comments_173389.json:
```json
{
    "body": "Work Issues: **failing doctest**",
    "created_at": "2013-04-03T16:29:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14275#issuecomment-173389",
    "user": "https://github.com/tscrim"
}
```

Work Issues: **failing doctest**



---

archive/issue_events_184658.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-04-03T16:29:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14275#event-184658"
}
```



---

archive/issue_events_184659.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-04-03T16:29:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14275#event-184659"
}
```



---

archive/issue_comments_173390.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nHey David,\n\nThe doctest is failing because the mismatch of the trac numbers. Also could you add a doctest with a message as well?\n\nThanks,\n\nTravis",
    "created_at": "2013-04-03T16:29:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14275#issuecomment-173390",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:2" align="right">Comment 2</div>

Hey David,

The doctest is failing because the mismatch of the trac numbers. Also could you add a doctest with a message as well?

Thanks,

Travis



---

archive/issue_comments_173391.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2013-04-03T16:29:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14275#issuecomment-173391",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_173392.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nThe way I see it, deprecation only makes sense for stuff that is imported into the global namespace. Anything that is not directly accessible from the command line is, by definition, an implementation detail. And once you require backwards compatibility for implementation details it basically becomes impossible to change anything. So there is no point in deprecating import locations.",
    "created_at": "2013-04-22T13:55:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14275#issuecomment-173392",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:3" align="right">Comment 3</div>

The way I see it, deprecation only makes sense for stuff that is imported into the global namespace. Anything that is not directly accessible from the command line is, by definition, an implementation detail. And once you require backwards compatibility for implementation details it basically becomes impossible to change anything. So there is no point in deprecating import locations.



---

archive/issue_events_184660.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14275#event-184660"
}
```



---

archive/issue_events_184661.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14275#event-184661"
}
```



---

archive/issue_events_184662.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14275#event-184662"
}
```



---

archive/issue_events_184663.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14275#event-184663"
}
```



---

archive/issue_comments_173393.json:
```json
{
    "body": "Commit: **[108e019](https://github.com/sagemath/sagetrac-mirror/commit/108e019185bf79a5e66c93a478f05116789278ae)**",
    "created_at": "2014-03-09T18:53:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14275#issuecomment-173393",
    "user": "https://github.com/tscrim"
}
```

Commit: **[108e019](https://github.com/sagemath/sagetrac-mirror/commit/108e019185bf79a5e66c93a478f05116789278ae)**



---

archive/issue_events_184664.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-03-09T18:53:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14275#event-184664"
}
```



---

archive/issue_events_184665.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-03-09T18:53:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14275#event-184665"
}
```



---

archive/issue_comments_173394.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nThis could be used to remove things from the global namespace but the class itself doesn't change.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/108e019185bf79a5e66c93a478f05116789278ae\">108e019</a></td><td><code>Adds a deprecation option to lazy_import.</code></td></tr></table>\n",
    "created_at": "2014-03-09T18:53:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14275#issuecomment-173394",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:6" align="right">Comment 6</div>

This could be used to remove things from the global namespace but the class itself doesn't change.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/108e019185bf79a5e66c93a478f05116789278ae">108e019</a></td><td><code>Adds a deprecation option to lazy_import.</code></td></tr></table>




---

archive/issue_comments_173395.json:
```json
{
    "body": "Branch: **[u/tscrim/14275](https://github.com/sagemath/sagetrac-mirror/tree/u/tscrim/14275)**",
    "created_at": "2014-03-09T18:53:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14275#issuecomment-173395",
    "user": "https://github.com/tscrim"
}
```

Branch: **[u/tscrim/14275](https://github.com/sagemath/sagetrac-mirror/tree/u/tscrim/14275)**



---

archive/issue_comments_173396.json:
```json
{
    "body": "Changed work issues from **failing doctest** to none",
    "created_at": "2014-03-09T18:54:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14275#issuecomment-173396",
    "user": "https://github.com/tscrim"
}
```

Changed work issues from **failing doctest** to none



---

archive/issue_comments_173397.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a4848093884df99a12d031dc853ce9d97c0c3dc2\">a484809</a></td><td><code>Doctest fixes.</code></td></tr></table>\n",
    "created_at": "2014-03-09T18:59:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14275#issuecomment-173397",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:8" align="right">Comment 8</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a4848093884df99a12d031dc853ce9d97c0c3dc2">a484809</a></td><td><code>Doctest fixes.</code></td></tr></table>




---

archive/issue_comments_173398.json:
```json
{
    "body": "Changed commit from **[108e019](https://github.com/sagemath/sagetrac-mirror/commit/108e019185bf79a5e66c93a478f05116789278ae)** to **[a484809](https://github.com/sagemath/sagetrac-mirror/commit/a4848093884df99a12d031dc853ce9d97c0c3dc2)**",
    "created_at": "2014-03-09T18:59:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14275#issuecomment-173398",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[108e019](https://github.com/sagemath/sagetrac-mirror/commit/108e019185bf79a5e66c93a478f05116789278ae)** to **[a484809](https://github.com/sagemath/sagetrac-mirror/commit/a4848093884df99a12d031dc853ce9d97c0c3dc2)**



---

archive/issue_comments_173399.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nComment from the sage-combinat thread :\n\nthe way it is implemented you add a \"deprecation\" \nfeature to lazy import. But lazy imports can be used a lot, so \nwouldn't it be better to instead add a \"lazy\" feature to \n\"deprecation_function_alias\" ? \n\nNathann",
    "created_at": "2014-04-03T14:11:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14275#issuecomment-173399",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:9" align="right">Comment 9</div>

Comment from the sage-combinat thread :

the way it is implemented you add a "deprecation" 
feature to lazy import. But lazy imports can be used a lot, so 
wouldn't it be better to instead add a "lazy" feature to 
"deprecation_function_alias" ? 

Nathann



---

archive/issue_events_184666.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-04-03T14:11:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14275#event-184666"
}
```



---

archive/issue_events_184667.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-04-03T14:11:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14275#event-184667"
}
```



---

archive/issue_comments_173400.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nNo, because then we'd have to import `deprecated_function_alias` (which doesn't currently accept a message too) and import the original object just to do something like\n\n```\nx = deprecation_function_alias(14275, x)\n```\nOtherwise we completely change the syntax and functionality to include `lazy_import`. Both of which are asinine.",
    "created_at": "2014-04-03T17:05:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14275#issuecomment-173400",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:10" align="right">Comment 10</div>

No, because then we'd have to import `deprecated_function_alias` (which doesn't currently accept a message too) and import the original object just to do something like

```
x = deprecation_function_alias(14275, x)
```
Otherwise we completely change the syntax and functionality to include `lazy_import`. Both of which are asinine.



---

archive/issue_events_184668.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-04-03T17:05:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14275#event-184668"
}
```



---

archive/issue_events_184669.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2014-04-03T17:05:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14275#event-184669"
}
```



---

archive/issue_comments_173401.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nAhahaahha. Well, for as long as you know that the two reasons you gave me are bad reasons and can be easily avoided, all is fine. It is not so bad.\n\nGood to go !\n\nNathann",
    "created_at": "2014-04-03T20:37:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14275#issuecomment-173401",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:11" align="right">Comment 11</div>

Ahahaahha. Well, for as long as you know that the two reasons you gave me are bad reasons and can be easily avoided, all is fine. It is not so bad.

Good to go !

Nathann



---

archive/issue_events_184670.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-04-03T20:37:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14275#event-184670"
}
```



---

archive/issue_events_184671.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2014-04-03T20:37:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14275#event-184671"
}
```



---

archive/issue_comments_173402.json:
```json
{
    "body": "Changed reviewer from **Travis Scrimshaw** to **Travis Scrimshaw, Nathann Cohen**",
    "created_at": "2014-04-03T20:37:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14275#issuecomment-173402",
    "user": "https://github.com/nathanncohen"
}
```

Changed reviewer from **Travis Scrimshaw** to **Travis Scrimshaw, Nathann Cohen**



---

archive/issue_comments_173403.json:
```json
{
    "body": "Changed author from **David Roe** to **David Roe, Travis Scrimshaw**",
    "created_at": "2014-04-03T20:37:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14275#issuecomment-173403",
    "user": "https://github.com/nathanncohen"
}
```

Changed author from **David Roe** to **David Roe, Travis Scrimshaw**



---

archive/issue_comments_173404.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nThanks Nathann.",
    "created_at": "2014-04-03T21:34:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14275#issuecomment-173404",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:12" align="right">Comment 12</div>

Thanks Nathann.



---

archive/issue_comments_173405.json:
```json
{
    "body": "Changed branch from **[u/tscrim/14275](https://github.com/sagemath/sagetrac-mirror/tree/u/tscrim/14275)** to **[a484809](https://github.com/sagemath/sagetrac-mirror/commit/a4848093884df99a12d031dc853ce9d97c0c3dc2)**",
    "created_at": "2014-04-04T15:55:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14275#issuecomment-173405",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/tscrim/14275](https://github.com/sagemath/sagetrac-mirror/tree/u/tscrim/14275)** to **[a484809](https://github.com/sagemath/sagetrac-mirror/commit/a4848093884df99a12d031dc853ce9d97c0c3dc2)**



---

archive/issue_events_184672.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-04-04T15:55:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14275#event-184672"
}
```



---

archive/issue_events_184673.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "32bee38fa892bbfd383b54a50f762a4671be5df1",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2014-04-04T15:55:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14275",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14275#event-184673"
}
```
