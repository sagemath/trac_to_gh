# Issue 14595: cleanup of doc in heegner.py

archive/issues_014391.json:
```json
{
    "assignees": [],
    "body": "this patch proposes to clean the heegner.py file :\n\nSome things found using pyflakes:\n* removing one duplicate import\n* removing unused variables assignements\nplus\n* removing trailing whitespaces\n* minor doc formatting\n* convert raise statements to python3 syntax\n\nApply [attachment: trac_14595_clean_heegner-fc.patch](https://github.com/sagemath/sage/files/ticket14595/trac_14595_clean_heegner-fc.patch.gz)\n\nDepends on #13213\n\nAssignee: **@JohnCremona**\n\nKeywords: **documentation, heegner**\n\nReviewer: **Volker Braun**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\nMerged: **sage-5.11.beta0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14595_\n\n",
    "closed_at": "2013-06-06T12:33:25Z",
    "created_at": "2013-05-15T20:09:25Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20elliptic%20curves",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.11",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "cleanup of doc in heegner.py",
    "type": "issue",
    "updated_at": "2013-06-06T12:33:25Z",
    "url": "https://github.com/sagemath/sage/issues/14595",
    "user": "https://github.com/fchapoton"
}
```
this patch proposes to clean the heegner.py file :

Some things found using pyflakes:
* removing one duplicate import
* removing unused variables assignements
plus
* removing trailing whitespaces
* minor doc formatting
* convert raise statements to python3 syntax

Apply [attachment: trac_14595_clean_heegner-fc.patch](https://github.com/sagemath/sage/files/ticket14595/trac_14595_clean_heegner-fc.patch.gz)

Depends on #13213

Assignee: **@JohnCremona**

Keywords: **documentation, heegner**

Reviewer: **Volker Braun**

Author: **Frédéric Chapoton**

Merged: **sage-5.11.beta0**

_Issue created by migration from https://trac.sagemath.org/ticket/14595_





---

archive/issue_events_189668.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-15T20:09:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14595",
    "milestone_number": null,
    "milestone_title": "sage-5.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14595#event-189668"
}
```



---

archive/issue_events_189669.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-15T20:09:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14595",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20elliptic%20curves",
    "label_color": "0000ff",
    "label_name": "component: elliptic curves",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14595#event-189669"
}
```



---

archive/issue_events_189670.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-15T20:09:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14595",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14595#event-189670"
}
```



---

archive/issue_events_189671.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-15T20:09:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14595",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14595#event-189671"
}
```



---

archive/issue_comments_179413.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNeeds review?",
    "created_at": "2013-05-16T20:41:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14595",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14595#issuecomment-179413",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:1" align="right">Comment 1</div>

Needs review?



---

archive/issue_events_189672.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-05-17T07:44:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14595",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14595#event-189672"
}
```



---

archive/issue_comments_179414.json:
```json
{
    "body": "Author: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2013-05-17T07:44:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14595",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14595#issuecomment-179414",
    "user": "https://github.com/fchapoton"
}
```

Author: **Frédéric Chapoton**



---

archive/issue_comments_179415.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\noh, yes, indeed. I forgot to click on **needs review**.",
    "created_at": "2013-05-17T07:44:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14595",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14595#issuecomment-179415",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

oh, yes, indeed. I forgot to click on **needs review**.



---

archive/issue_comments_179416.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLooks good but needs to be rebased on top of #13213",
    "created_at": "2013-05-17T10:22:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14595",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14595#issuecomment-179416",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:3" align="right">Comment 3</div>

Looks good but needs to be rebased on top of #13213



---

archive/issue_comments_179417.json:
```json
{
    "body": "Dependencies: **13213**",
    "created_at": "2013-05-17T10:22:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14595",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14595#issuecomment-179417",
    "user": "https://github.com/vbraun"
}
```

Dependencies: **13213**



---

archive/issue_comments_179418.json:
```json
{
    "body": "Attachment: **[trac_14595_clean_heegner-fc.patch.gz](https://github.com/sagemath/sage/files/ticket14595/trac_14595_clean_heegner-fc.patch.gz)**",
    "created_at": "2013-05-17T18:12:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14595",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14595#issuecomment-179418",
    "user": "https://github.com/fchapoton"
}
```

Attachment: **[trac_14595_clean_heegner-fc.patch.gz](https://github.com/sagemath/sage/files/ticket14595/trac_14595_clean_heegner-fc.patch.gz)**



---

archive/issue_comments_179419.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nhere is a rebased patch",
    "created_at": "2013-05-17T18:13:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14595",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14595#issuecomment-179419",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:4" align="right">Comment 4</div>

here is a rebased patch



---

archive/issue_comments_179420.json:
```json
{
    "body": "Changed dependencies from **13213** to **#13213**",
    "created_at": "2013-05-17T18:13:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14595",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14595#issuecomment-179420",
    "user": "https://github.com/fchapoton"
}
```

Changed dependencies from **13213** to **#13213**



---

archive/issue_comments_179421.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nbot has given a green light, is there anybody here for a review ?",
    "created_at": "2013-05-29T09:45:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14595",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14595#issuecomment-179421",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:5" align="right">Comment 5</div>

bot has given a green light, is there anybody here for a review ?



---

archive/issue_comments_179422.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nlgtm",
    "created_at": "2013-05-29T09:50:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14595",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14595#issuecomment-179422",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:6" align="right">Comment 6</div>

lgtm



---

archive/issue_events_189673.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-05-29T09:50:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14595",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14595#event-189673"
}
```



---

archive/issue_events_189674.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-05-29T09:50:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14595",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14595#event-189674"
}
```



---

archive/issue_comments_179423.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -7,3 +7,5 @@\n * removing trailing whitespaces\n * minor doc formatting\n * convert raise statements to python3 syntax\n+\n+Apply [attachment: trac_14595_clean_heegner-fc.patch](https://github.com/sagemath/sage/files/ticket14595/trac_14595_clean_heegner-fc.patch.gz)\n``````\n",
    "created_at": "2013-05-29T09:50:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14595",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14595#issuecomment-179423",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -7,3 +7,5 @@
 * removing trailing whitespaces
 * minor doc formatting
 * convert raise statements to python3 syntax
+
+Apply [attachment: trac_14595_clean_heegner-fc.patch](https://github.com/sagemath/sage/files/ticket14595/trac_14595_clean_heegner-fc.patch.gz)
``````




---

archive/issue_comments_179424.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2013-05-29T09:50:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14595",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14595#issuecomment-179424",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_events_189675.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-29T12:29:05Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14595",
    "milestone_number": null,
    "milestone_title": "sage-5.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14595#event-189675"
}
```



---

archive/issue_events_189676.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-29T12:29:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14595",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14595#event-189676"
}
```



---

archive/issue_comments_179425.json:
```json
{
    "body": "Merged: **sage-5.11.beta0**",
    "created_at": "2013-06-06T12:33:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14595",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14595#issuecomment-179425",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.11.beta0**



---

archive/issue_events_189677.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-06T12:33:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14595",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14595#event-189677"
}
```



---

archive/issue_events_189678.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-06T12:33:25Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14595",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14595#event-189678"
}
```
