...
```json
{
    "body": "<a id='comment:5'></a>I guess the plan is to patch Sage again when GCC fixes the problem (hopefully in 4.8.1 and 4.9.?)?",
...
<a id='comment:5'></a>I guess the plan is to patch Sage again when GCC fixes the problem (hopefully in 4.8.1 and 4.9.?)?
...
<a id='comment:6'></a>Are we going to remember this, or will that just linger around forever until some setup-archaeologist finds it again? I would prefer just to shitlist a particular version, so if it still crashes the next time then we'll know that its not fixed ;-) 
...
```json
{
    "body": "<a id='comment:12'></a>Replying to [jpflori](#comment%3A8):\n> Maybe a direct indication of what the bug is could help, e.g. adding \"which let sig_on() miscompile\" after \"bug\".\n\n\nDone.",
...
<a id='comment:12'></a>Replying to [jpflori](#comment%3A8):
> Maybe a direct indication of what the bug is could help, e.g. adding "which let sig_on() miscompile" after "bug".
...
<a id='comment:14'></a>I've rebuilt Sage (5.9.beta5 + #10508) completely using Debian gcc/g++/gfortran 4.8.0 without any env variables except for MAKE, CC, GXX, FC and SAGE_ATLAS_ARCH, then applied #14453, rerun "./sage -b", but "./sage -t devel/sage/sage/libs/gap/element.pyx" still passes its doctests...
...
<a id='comment:15'></a>And when "-Og" is used, then the fix indeed fixes the problem.
...
```json
{
    "body": "<a id='comment:17'></a>Attachment [14460_gcc_bug.patch](tarball://root/attachments/some-uuid/ticket14460/14460_gcc_bug.patch) by @jdemeyer created at 2013-04-17 10:06:40",
    "created_at": "2013-04-17T10:06:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14460",
    "type": "issue_comment",
...
<a id='comment:17'></a>Attachment [14460_gcc_bug.patch](tarball://root/attachments/some-uuid/ticket14460/14460_gcc_bug.patch) by @jdemeyer created at 2013-04-17 10:06:40
...
```json
{
    "body": "<a id='comment:19'></a>By the way, there is lots of activity on the GCC ticket. It seems some variation of this bug is reproducible back to GCC 4.1.",
...
<a id='comment:19'></a>By the way, there is lots of activity on the GCC ticket. It seems some variation of this bug is reproducible back to GCC 4.1.
...
<a id='comment:21'></a>Replying to [leif](#comment%3A20):
> On the other hand, if the bug only shows up in conjunction with `-Og` (or maybe `-Os` as well), does it make sense to add `-fno-tree-dominator-opts` regardless of the setting of `CFLAGS` etc., and for every extension module?
...
<a id='comment:22'></a>Replying to [leif](#comment%3A20):
> Well, if that's true, are you sure same or similar can't happen within the Sage library with GCC versions != 4.![89]*?
...
```json
{
    "body": "<a id='comment:23'></a>So, what is your opinion on adding `-fno-dominator-opts` for all GCC versions >= 4.1?",
...
<a id='comment:23'></a>So, what is your opinion on adding `-fno-dominator-opts` for all GCC versions >= 4.1?
...
```json
{
    "body": "<a id='comment:26'></a>Bug still exists in GCC-4.8.1.",
...
<a id='comment:26'></a>Bug still exists in GCC-4.8.1.
...
```json
{
    "body": "<a id='comment:31'></a>Let me copy this sentence from the ticket description:\n\nlibGAP is one of the few places in Sage where non-trivial use of the sig_on() signal handling is actually tested, so there might be many more places where the problem could occur.",
...
<a id='comment:31'></a>Let me copy this sentence from the ticket description:
...
<a id='comment:32'></a>Yeah I agree, no harm meant at all.
