# Issue 14428: Tachyon objects should be able to accept 3d plots and fields from sage.

archive/issues_014224.json:
```json
{
    "body": "Currently the sage objects created by \n\n```\nsage: t = Tachyon() \n```\nonly accept certain drawn primitives that are predefined inside **sage/plot/plot3d/tachyon.py** after they are created, e.g. using sphere, plane, cylinder methods to insert those figures. General sage 3d plots can't be added to such scenes. A round about method using \n\n```\nsage: var('x y')\n(x, y)\nsage: a = plot3d(sin(x+y), (x, -4, 4), (y, -4, 4))\nsage: tachyon_rt(a.tachyon())\n```\n\nis possible, however most of the options are hard-coded into **sage/plot/plot3d/base.pyx**. Even worse the whole scene is treated as one large text file making any further manipulation difficult at best.\n\nAssignee: anton_t\n\nCC:  @nilesjohnson @paulmasson @slel\n\nKeywords: tachyon 3d plot raytrace\n\nAuthor: anton_t\n\nStatus: new\n\nIssue created by migration from https://trac.sagemath.org/ticket/14428\n\n",
    "created_at": "2013-04-09T03:35:28Z",
    "labels": [
        "component: graphics"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-wishlist",
    "title": "Tachyon objects should be able to accept 3d plots and fields from sage.",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/14428",
    "user": "https://trac.sagemath.org/admin/accounts/users/anton_t"
}
```
Currently the sage objects created by 

```
sage: t = Tachyon() 
```
only accept certain drawn primitives that are predefined inside **sage/plot/plot3d/tachyon.py** after they are created, e.g. using sphere, plane, cylinder methods to insert those figures. General sage 3d plots can't be added to such scenes. A round about method using 

```
sage: var('x y')
(x, y)
sage: a = plot3d(sin(x+y), (x, -4, 4), (y, -4, 4))
sage: tachyon_rt(a.tachyon())
```

is possible, however most of the options are hard-coded into **sage/plot/plot3d/base.pyx**. Even worse the whole scene is treated as one large text file making any further manipulation difficult at best.

Assignee: anton_t

CC:  @nilesjohnson @paulmasson @slel

Keywords: tachyon 3d plot raytrace

Author: anton_t

Status: new

Issue created by migration from https://trac.sagemath.org/ticket/14428





---

archive/issue_comments_190750.json:
```json
{
    "body": "Attachment [tachyon.patch](tarball://root/attachments/some-uuid/ticket14428/tachyon.patch) by anton_t created at 2013-04-09 03:37:29\n\npatch to tachyon.py to enable general 3d objects to be inserted into tachyon scenes.",
    "created_at": "2013-04-09T03:37:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14428",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14428#issuecomment-190750",
    "user": "https://trac.sagemath.org/admin/accounts/users/anton_t"
}
```

Attachment [tachyon.patch](tarball://root/attachments/some-uuid/ticket14428/tachyon.patch) by anton_t created at 2013-04-09 03:37:29

patch to tachyon.py to enable general 3d objects to be inserted into tachyon scenes.



---

archive/issue_events_040988.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/anton_t",
    "created_at": "2013-04-09T09:21:39Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14428",
    "milestone": "sage-5.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14428#event-40988"
}
```



---

archive/issue_events_040989.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14428",
    "milestone": "sage-5.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14428#event-40989"
}
```



---

archive/issue_events_040990.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14428",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14428#event-40990"
}
```



---

archive/issue_events_040991.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14428",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14428#event-40991"
}
```



---

archive/issue_events_040992.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14428",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14428#event-40992"
}
```



---

archive/issue_comments_190751.json:
```json
{
    "body": "<a id='comment:5'></a>This definitely needs to be done :)  I'm not sure of the best way to go about it -- I had a different approach in mind, and yet another way is outlined at [this AskSage answer](http://ask.sagemath.org/question/20/the-tachyon-object-used-for-rendering-plots?answer=4703#4703).",
    "created_at": "2014-04-24T13:07:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14428",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14428#issuecomment-190751",
    "user": "https://github.com/nilesjohnson"
}
```

<a id='comment:5'></a>This definitely needs to be done :)  I'm not sure of the best way to go about it -- I had a different approach in mind, and yet another way is outlined at [this AskSage answer](http://ask.sagemath.org/question/20/the-tachyon-object-used-for-rendering-plots?answer=4703#4703).



---

archive/issue_events_040993.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14428",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14428#event-40993"
}
```



---

archive/issue_events_040994.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14428",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14428#event-40994"
}
```



---

archive/issue_events_040995.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14428",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14428#event-40995"
}
```



---

archive/issue_events_040996.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14428",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14428#event-40996"
}
```



---

archive/issue_comments_190752.json:
```json
{
    "body": "helpers to convert graphics objects to Tachyon or join two Tachyon objects",
    "created_at": "2014-09-12T12:57:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14428",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14428#issuecomment-190752",
    "user": "https://github.com/nilesjohnson"
}
```

helpers to convert graphics objects to Tachyon or join two Tachyon objects



---

archive/issue_comments_190753.json:
```json
{
    "body": "<a id='comment:8'></a>Attachment [tachyon_converter.sage](tarball://root/attachments/some-uuid/ticket14428/tachyon_converter.sage) by @nilesjohnson created at 2014-09-12 13:01:30\n\nI've cleaned up and attached some function definitions that outline the approach I had in mind.  Part of it is similar to the approach of anton_t, but it also includes some code for extracting a tachyon scene string from a graphics object and some code for joining the objects from a scene string into another Tachyon object.\n\nIt's still in a very sketchy state, and I'm not yet sure this is the best approach, but it's what I have at the moment :)",
    "created_at": "2014-09-12T13:01:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14428",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14428#issuecomment-190753",
    "user": "https://github.com/nilesjohnson"
}
```

<a id='comment:8'></a>Attachment [tachyon_converter.sage](tarball://root/attachments/some-uuid/ticket14428/tachyon_converter.sage) by @nilesjohnson created at 2014-09-12 13:01:30

I've cleaned up and attached some function definitions that outline the approach I had in mind.  Part of it is similar to the approach of anton_t, but it also includes some code for extracting a tachyon scene string from a graphics object and some code for joining the objects from a scene string into another Tachyon object.

It's still in a very sketchy state, and I'm not yet sure this is the best approach, but it's what I have at the moment :)



---

archive/issue_events_040997.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-05-04T00:14:02Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14428",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14428#event-40997"
}
```



---

archive/issue_events_040998.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2019-05-04T00:14:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14428",
    "milestone": "sage-wishlist",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14428#event-40998"
}
```
