...
```json
{
    "body": "please use the python3 syntax for raise\n\n```\nraise ValueError(\"message here\")\n```",
...
```json
{
    "body": "That's what I get for copying and pasting the code from NumberField_generic. It would probably be better if I just call that code. Is the proper syntax for calling a super class's method NumberField_generic.is_isomorphic(self, other)?\n\nReplying to [comment:2 chapoton]:\n> please use the python3 syntax for raise\n> \n> ```\n> raise ValueError(\"message here\")\n> ```",
...
```json
{
    "body": "Attachment [trac_14300_fix_CyclotomicField_is_isomorphic.2.patch](tarball://root/attachments/some-uuid/ticket14300/trac_14300_fix_CyclotomicField_is_isomorphic.2.patch) by @rharron created at 2013-04-07 21:05:29\n\nI think that is the correct syntax, so I uploaded a new version of the patch.",
...
Attachment [trac_14300_fix_CyclotomicField_is_isomorphic.2.patch](tarball://root/attachments/some-uuid/ticket14300/trac_14300_fix_CyclotomicField_is_isomorphic.2.patch) by @rharron created at 2013-04-07 21:05:29
...
```json
{
    "body": "(Actually, I don't think it was that I copy-pasted, rather I simply didn't change that line of code, though that's not clear from the way trac shows the diff.)\n\nReplying to [comment:2 chapoton]:\n> please use the python3 syntax for raise\n> \n> ```\n> raise ValueError(\"message here\")\n> ```",
...
```json
{
    "body": "Apply trac_14300_fix_CyclotomicField_is_isomorphic.2.patch",
