# Issue 14032: determinant() of integer matrices of size in [51,63] broken

archive/issues_013828.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: random_matrix(ZZ,60).det()\n[...]\nRuntimeError: maximum recursion depth exceeded while calling a Python object\n```\n\nThe problem was an infinite recursion where we compute a determinant over ZZ by working mod p and we compute a determinant over GF(p) by lifting to ZZ...\n\nThis recursion got broken because the bound to use LinBox for matrices mod p changed in #12883.  This patch fixes this bound.\n\nIt also does various clean-up of doctests and removes `_det_4x4_unsafe()` which only added an extra level of indirection (`determinant() -> _det_4x4_unsafe() -> four_dim_det()`)\n\nAssignee: **@jasongrout, @williamstein**\n\nReviewer: **Volker Braun**\n\nAuthor: **Jeroen Demeyer**\n\nMerged: **sage-5.7.beta2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14032_\n\n",
    "closed_at": "2013-01-31T09:19:39Z",
    "created_at": "2013-01-29T14:43:18Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "determinant() of integer matrices of size in [51,63] broken",
    "type": "issue",
    "updated_at": "2013-01-31T09:19:39Z",
    "url": "https://github.com/sagemath/sage/issues/14032",
    "user": "https://github.com/jdemeyer"
}
```

```
sage: random_matrix(ZZ,60).det()
[...]
RuntimeError: maximum recursion depth exceeded while calling a Python object
```

The problem was an infinite recursion where we compute a determinant over ZZ by working mod p and we compute a determinant over GF(p) by lifting to ZZ...

This recursion got broken because the bound to use LinBox for matrices mod p changed in #12883.  This patch fixes this bound.

It also does various clean-up of doctests and removes `_det_4x4_unsafe()` which only added an extra level of indirection (`determinant() -> _det_4x4_unsafe() -> four_dim_det()`)

Assignee: **@jasongrout, @williamstein**

Reviewer: **Volker Braun**

Author: **Jeroen Demeyer**

Merged: **sage-5.7.beta2**

_Issue created by migration from https://trac.sagemath.org/ticket/14032_





---

archive/issue_events_180851.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-29T14:43:18Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14032",
    "milestone_number": null,
    "milestone_title": "sage-5.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14032#event-180851"
}
```



---

archive/issue_events_180852.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-29T14:43:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14032",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14032#event-180852"
}
```



---

archive/issue_events_180853.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-29T14:43:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14032",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14032#event-180853"
}
```



---

archive/issue_events_180854.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-29T14:43:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14032",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14032#event-180854"
}
```



---

archive/issue_comments_168876.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\n50 is the threshold for defaulting to `algorithm='padic'`. The following works fine: `random_matrix(ZZ,60).determinant(algorithm='pari')`",
    "created_at": "2013-01-29T15:02:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14032",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14032#issuecomment-168876",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:1'>Comment 1:</a>
50 is the threshold for defaulting to `algorithm='padic'`. The following works fine: `random_matrix(ZZ,60).determinant(algorithm='pari')`



---

archive/issue_comments_168877.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\n(never mind, forgot about caching)",
    "created_at": "2013-01-29T15:34:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14032",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14032#issuecomment-168877",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'>Comment 2:</a>
(never mind, forgot about caching)



---

archive/issue_events_180855.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-29T18:47:26Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/14032",
    "title_is": "determinant() of integer matrices of size in [51,63] broken",
    "title_was": "determinant() of large integer matrices broken",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14032#event-180855"
}
```



---

archive/issue_comments_168878.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -4,3 +4,5 @@\n [...]\n RuntimeError: maximum recursion depth exceeded while calling a Python object\n ```\n+\n+It seems that the \"padic\" algorithm for integer determinants uses one implementation for sizes <= 50 and another implementation for sizes >= 64 and is broken in between those sizes (the reason is suspect two implementations is that size 64 is usually a lot faster than size 50).\n``````\n",
    "created_at": "2013-01-29T18:47:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14032",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14032#issuecomment-168878",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -4,3 +4,5 @@
 [...]
 RuntimeError: maximum recursion depth exceeded while calling a Python object
 ```
+
+It seems that the "padic" algorithm for integer determinants uses one implementation for sizes <= 50 and another implementation for sizes >= 64 and is broken in between those sizes (the reason is suspect two implementations is that size 64 is usually a lot faster than size 50).
``````




---

archive/issue_comments_168879.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nThere is an infinite recursion going on where we compute a determinant over `ZZ` by working mod p and we compute a determinant over `GF(p)` by lifting to `ZZ`...",
    "created_at": "2013-01-29T19:48:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14032",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14032#issuecomment-168879",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:4'>Comment 4:</a>
There is an infinite recursion going on where we compute a determinant over `ZZ` by working mod p and we compute a determinant over `GF(p)` by lifting to `ZZ`...



---

archive/issue_comments_168880.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -5,4 +5,8 @@\n RuntimeError: maximum recursion depth exceeded while calling a Python object\n ```\n \n-It seems that the \"padic\" algorithm for integer determinants uses one implementation for sizes <= 50 and another implementation for sizes >= 64 and is broken in between those sizes (the reason is suspect two implementations is that size 64 is usually a lot faster than size 50).\n+The problem was an infinite recursion where we compute a determinant over ZZ by working mod p and we compute a determinant over GF(p) by lifting to ZZ...\n+\n+This recursion got broken because the bound to use LinBox for matrices mod p changed in #12883.  This patch fixes this bound.\n+\n+It also does various clean-up of doctests and removes `_det_4x4_unsafe()` which only added an extra level of indirection (`determinant() -> _det_4x4_unsafe() -> four_dim_det()`)\n``````\n",
    "created_at": "2013-01-29T20:13:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14032",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14032#issuecomment-168880",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -5,4 +5,8 @@
 RuntimeError: maximum recursion depth exceeded while calling a Python object
 ```
 
-It seems that the "padic" algorithm for integer determinants uses one implementation for sizes <= 50 and another implementation for sizes >= 64 and is broken in between those sizes (the reason is suspect two implementations is that size 64 is usually a lot faster than size 50).
+The problem was an infinite recursion where we compute a determinant over ZZ by working mod p and we compute a determinant over GF(p) by lifting to ZZ...
+
+This recursion got broken because the bound to use LinBox for matrices mod p changed in #12883.  This patch fixes this bound.
+
+It also does various clean-up of doctests and removes `_det_4x4_unsafe()` which only added an extra level of indirection (`determinant() -> _det_4x4_unsafe() -> four_dim_det()`)
``````




---

archive/issue_comments_168881.json:
```json
{
    "body": "Author: **Jeroen Demeyer**",
    "created_at": "2013-01-29T20:13:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14032",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14032#issuecomment-168881",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Jeroen Demeyer**



---

archive/issue_events_180856.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-29T20:20:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14032",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14032#event-180856"
}
```



---

archive/issue_comments_168882.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nAttachment: **[14032_multimodular_det.patch.gz](https://github.com/sagemath/sage/files/ticket14032/14032_multimodular_det.patch.gz)**",
    "created_at": "2013-01-29T20:20:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14032",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14032#issuecomment-168882",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:6'>Comment 6:</a>
Attachment: **[14032_multimodular_det.patch.gz](https://github.com/sagemath/sage/files/ticket14032/14032_multimodular_det.patch.gz)**



---

archive/issue_comments_168883.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nLooks great!",
    "created_at": "2013-01-30T11:26:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14032",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14032#issuecomment-168883",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:7'>Comment 7:</a>
Looks great!



---

archive/issue_comments_168884.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2013-01-30T11:26:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14032",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14032#issuecomment-168884",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_events_180857.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-01-30T11:26:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14032",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14032#event-180857"
}
```



---

archive/issue_events_180858.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-01-30T11:26:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14032",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14032#event-180858"
}
```



---

archive/issue_comments_168885.json:
```json
{
    "body": "Merged: **sage-5.7.beta2**",
    "created_at": "2013-01-31T09:19:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14032",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14032#issuecomment-168885",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.7.beta2**



---

archive/issue_events_180859.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-31T09:19:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14032",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14032#event-180859"
}
```



---

archive/issue_events_180860.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-01-31T09:19:39Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14032",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14032#event-180860"
}
```
