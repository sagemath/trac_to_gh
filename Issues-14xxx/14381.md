# Issue 14381: Split m4 spkg from (big) autotools one

archive/issues_014177.json:
```json
{
    "assignees": [],
    "body": "Even though we don't make m4 spkg standard yet, I think it's a good idea to split it from the big autotools one.\n\nAnd as it's a Sage prereq on all platform and it may happen that the usually installed system-wide one can be broken, it will ease things to have it as a separate spkg for people who want to install a recent m4 through m4, or for us if one day we decide to make it standard.\n\n\nCC:  @jdemeyer\n\nKeywords: **m4 spkg**\n\nReviewer: **Michael Orlitzky**\n\nComponent: **packages: optional**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14381_\n\n",
    "closed_at": "2021-10-04T23:44:13Z",
    "created_at": "2013-03-29T10:42:51Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Split m4 spkg from (big) autotools one",
    "type": "issue",
    "updated_at": "2023-01-08T09:54:58Z",
    "url": "https://github.com/sagemath/sage/issues/14381",
    "user": "https://github.com/jpflori"
}
```
Even though we don't make m4 spkg standard yet, I think it's a good idea to split it from the big autotools one.

And as it's a Sage prereq on all platform and it may happen that the usually installed system-wide one can be broken, it will ease things to have it as a separate spkg for people who want to install a recent m4 through m4, or for us if one day we decide to make it standard.


CC:  @jdemeyer

Keywords: **m4 spkg**

Reviewer: **Michael Orlitzky**

Component: **packages: optional**

_Issue created by migration from https://trac.sagemath.org/ticket/14381_





---

archive/issue_events_204060.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-03-29T10:42:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-204060"
}
```



---

archive/issue_events_204061.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-03-29T10:42:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20optional",
    "label_color": "000080",
    "label_name": "c: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-204061"
}
```



---

archive/issue_events_204062.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-03-29T10:42:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-204062"
}
```



---

archive/issue_events_204063.json:
```json
{
    "actor": "https://github.com/jpflori",
    "created_at": "2013-03-29T10:42:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-204063"
}
```



---

archive/issue_comments_172970.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nThis could be helpful on Cygwin?",
    "created_at": "2013-03-29T15:43:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14381#issuecomment-172970",
    "user": "https://github.com/kcrisman"
}
```

<div id="comment:1" align="right">comment:1</div>

This could be helpful on Cygwin?



---

archive/issue_comments_172971.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nReplying to [@kcrisman](#comment:1):\n> This could be helpful on Cygwin?\n\nI don't think so.\nNote that m4 is a prereq on every platform anyway.",
    "created_at": "2013-03-29T15:46:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14381#issuecomment-172971",
    "user": "https://github.com/jpflori"
}
```

<div id="comment:2" align="right">comment:2</div>

Replying to [@kcrisman](#comment:1):
> This could be helpful on Cygwin?

I don't think so.
Note that m4 is a prereq on every platform anyway.



---

archive/issue_events_204064.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-204064"
}
```



---

archive/issue_events_204065.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "milestone_number": null,
    "milestone_title": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-204065"
}
```



---

archive/issue_events_204066.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "milestone_number": null,
    "milestone_title": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-204066"
}
```



---

archive/issue_events_204067.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-204067"
}
```



---

archive/issue_events_204068.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "milestone_number": null,
    "milestone_title": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-204068"
}
```



---

archive/issue_events_204069.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-204069"
}
```



---

archive/issue_events_204070.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "milestone_number": null,
    "milestone_title": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-204070"
}
```



---

archive/issue_events_204071.json:
```json
{
    "actor": "https://github.com/sagetrac-vbraun-spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-204071"
}
```



---

archive/issue_comments_172972.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">comment:7</div>\n\nSetting spkg proposals that have not seen recent activity to \"sage-wishlist\".",
    "created_at": "2020-06-19T18:16:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14381#issuecomment-172972",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:7" align="right">comment:7</div>

Setting spkg proposals that have not seen recent activity to "sage-wishlist".



---

archive/issue_events_204072.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-19T18:16:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-204072"
}
```



---

archive/issue_events_204073.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-19T18:16:00Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "milestone_number": null,
    "milestone_title": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-204073"
}
```



---

archive/issue_events_204074.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-19T18:16:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-204074"
}
```



---

archive/issue_events_204075.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-26T04:05:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "label": "https://github.com/sagemath/sage/labels/p%3A%204%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p: 4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-204075"
}
```



---

archive/issue_events_204076.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-26T04:05:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "label": "https://github.com/sagemath/sage/labels/wishlist%20item",
    "label_color": "e81ff9",
    "label_name": "wishlist item",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-204076"
}
```



---

archive/issue_events_204077.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-26T04:05:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "c6c6c6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-204077"
}
```



---

archive/issue_comments_172973.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">comment:8</div>\n\noutdated, should close",
    "created_at": "2021-08-26T04:05:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14381#issuecomment-172973",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:8" align="right">comment:8</div>

outdated, should close



---

archive/issue_events_204078.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-26T04:05:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-204078"
}
```



---

archive/issue_events_204079.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-26T04:05:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-204079"
}
```



---

archive/issue_comments_172974.json:
```json
{
    "body": "Reviewer: **Michael Orlitzky**",
    "created_at": "2021-10-04T23:08:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14381#issuecomment-172974",
    "user": "https://github.com/orlitzky"
}
```

Reviewer: **Michael Orlitzky**



---

archive/issue_events_204080.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2021-10-04T23:08:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-204080"
}
```



---

archive/issue_events_204081.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2021-10-04T23:08:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-204081"
}
```



---

archive/issue_comments_172975.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">comment:9</div>\n\nThere is no more autotools SPKG; it's a `_prereq`, and there are better ways to install this stuff besides.",
    "created_at": "2021-10-04T23:08:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14381#issuecomment-172975",
    "user": "https://github.com/orlitzky"
}
```

<div id="comment:9" align="right">comment:9</div>

There is no more autotools SPKG; it's a `_prereq`, and there are better ways to install this stuff besides.



---

archive/issue_comments_172976.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">comment:10</div>\n\nActually it's a `_bootstrap`, but yes",
    "created_at": "2021-10-04T23:16:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14381#issuecomment-172976",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:10" align="right">comment:10</div>

Actually it's a `_bootstrap`, but yes



---

archive/issue_events_204082.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T23:44:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-204082"
}
```



---

archive/issue_events_204083.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T23:44:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14381",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14381#event-204083"
}
```
