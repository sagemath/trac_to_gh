# Issue 14908: Two small issues with make micro_release

archive/issues_014704.json:
```json
{
    "assignees": [],
    "body": "A trivial patch to experiment with the git workflow...\n\n- the micro_release rule of the Makefile used |&, which is a bash4-ism\n- the filtering of error messages did not work in non-English locales\n\n\nReviewer: **Marc Mezzarobba, Volker Braun**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14908_\n\n",
    "closed_at": "2013-10-05T09:46:09Z",
    "created_at": "2013-07-18T17:18:21Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Two small issues with make micro_release",
    "type": "issue",
    "updated_at": "2013-10-05T09:46:48Z",
    "url": "https://github.com/sagemath/sage/issues/14908",
    "user": "https://github.com/mezzarobba"
}
```
A trivial patch to experiment with the git workflow...

- the micro_release rule of the Makefile used |&, which is a bash4-ism
- the filtering of error messages did not work in non-English locales


Reviewer: **Marc Mezzarobba, Volker Braun**

_Issue created by migration from https://trac.sagemath.org/ticket/14908_





---

archive/issue_events_194682.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2013-07-18T17:18:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14908#event-194682"
}
```



---

archive/issue_events_194683.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2013-07-18T17:18:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14908#event-194683"
}
```



---

archive/issue_events_194684.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2013-07-18T17:18:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14908#event-194684"
}
```



---

archive/issue_events_194685.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2013-07-18T17:18:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14908#event-194685"
}
```



---

archive/issue_events_194686.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2013-07-18T17:18:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14908#event-194686"
}
```



---

archive/issue_events_194687.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2013-07-19T14:47:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14908#event-194687"
}
```



---

archive/issue_events_194688.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2013-07-19T14:47:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14908#event-194688"
}
```



---

archive/issue_comments_185472.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nThe `working` branch was a workaround until #14781 got resolved.\n\nPlease rebase against the `build_system` branch.",
    "created_at": "2013-07-19T14:47:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14908#issuecomment-185472",
    "user": "https://github.com/ohanar"
}
```

<div id="comment:2" align="right">Comment 2</div>

The `working` branch was a workaround until #14781 got resolved.

Please rebase against the `build_system` branch.



---

archive/issue_events_194689.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2013-07-20T16:42:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14908#event-194689"
}
```



---

archive/issue_events_194690.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2013-07-20T16:42:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14908#event-194690"
}
```



---

archive/issue_comments_185473.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nReplying to [@ohanar](#comment%3A2):\n> Please rebase against the `build_system` branch. \n\nDone.",
    "created_at": "2013-07-20T16:42:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14908#issuecomment-185473",
    "user": "https://github.com/mezzarobba"
}
```

<div id="comment:3" align="right">Comment 3</div>

Replying to [@ohanar](#comment%3A2):
> Please rebase against the `build_system` branch. 

Done.



---

archive/issue_events_194691.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-07-31T22:18:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14908#event-194691"
}
```



---

archive/issue_events_194692.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-07-31T22:18:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "milestone_number": null,
    "milestone_title": "sage-6.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14908#event-194692"
}
```



---

archive/issue_events_194693.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-09-14T19:54:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14908#event-194693"
}
```



---

archive/issue_events_194694.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2013-09-14T19:54:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14908#event-194694"
}
```



---

archive/issue_comments_185474.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2013-09-14T19:54:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14908#issuecomment-185474",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_comments_185475.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nLooks good to me",
    "created_at": "2013-09-14T19:54:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14908#issuecomment-185475",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:5" align="right">Comment 5</div>

Looks good to me



---

archive/issue_comments_185476.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nI've merged this into the build_system branch at #14480, so I'm marking this as a duplicate so that we can close this ticket now.",
    "created_at": "2013-09-16T22:57:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14908#issuecomment-185476",
    "user": "https://github.com/ohanar"
}
```

<div id="comment:6" align="right">Comment 6</div>

I've merged this into the build_system branch at #14480, so I'm marking this as a duplicate so that we can close this ticket now.



---

archive/issue_events_194695.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2013-09-16T22:57:22Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "milestone_number": null,
    "milestone_title": "sage-6.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14908#event-194695"
}
```



---

archive/issue_events_194696.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2013-09-16T22:57:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14908#event-194696"
}
```



---

archive/issue_events_194697.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2013-09-16T22:57:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14908#event-194697"
}
```



---

archive/issue_comments_185477.json:
```json
{
    "body": "Changed branch from **[u/mmezzarobba/fix_micro_release](https://github.com/sagemath/sagetrac-mirror/tree/u/mmezzarobba/fix_micro_release)** to none",
    "created_at": "2013-09-24T08:24:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14908#issuecomment-185477",
    "user": "https://github.com/mezzarobba"
}
```

Changed branch from **[u/mmezzarobba/fix_micro_release](https://github.com/sagemath/sagetrac-mirror/tree/u/mmezzarobba/fix_micro_release)** to none



---

archive/issue_comments_185478.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nReplying to [@ohanar](#comment%3A6):\n> I've merged this into the build_system branch at #14480, so I'm marking this as a duplicate so that we can close this ticket now.\n\nOk, so I've deleted the branch and filled in the Commit field instead.",
    "created_at": "2013-09-24T08:24:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14908#issuecomment-185478",
    "user": "https://github.com/mezzarobba"
}
```

<div id="comment:7" align="right">Comment 7</div>

Replying to [@ohanar](#comment%3A6):
> I've merged this into the build_system branch at #14480, so I'm marking this as a duplicate so that we can close this ticket now.

Ok, so I've deleted the branch and filled in the Commit field instead.



---

archive/issue_comments_185479.json:
```json
{
    "body": "Commit: **[0628ee3](https://github.com/sagemath/sagetrac-mirror/commit/0628ee3b65e6195df2195c98b1df4c3767fcaad9)**",
    "created_at": "2013-09-24T08:24:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14908#issuecomment-185479",
    "user": "https://github.com/mezzarobba"
}
```

Commit: **[0628ee3](https://github.com/sagemath/sagetrac-mirror/commit/0628ee3b65e6195df2195c98b1df4c3767fcaad9)**



---

archive/issue_events_194698.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-05T09:46:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14908#event-194698"
}
```



---

archive/issue_events_194699.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-05T09:46:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14908#event-194699"
}
```



---

archive/issue_events_194700.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-10-05T09:46:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14908#event-194700"
}
```



---

archive/issue_comments_185480.json:
```json
{
    "body": "Changed commit from **[0628ee3](https://github.com/sagemath/sagetrac-mirror/commit/0628ee3b65e6195df2195c98b1df4c3767fcaad9)** to none",
    "created_at": "2013-10-05T09:46:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14908#issuecomment-185480",
    "user": "https://github.com/jdemeyer"
}
```

Changed commit from **[0628ee3](https://github.com/sagemath/sagetrac-mirror/commit/0628ee3b65e6195df2195c98b1df4c3767fcaad9)** to none



---

archive/issue_comments_185481.json:
```json
{
    "body": "Changed reviewer from **Volker Braun** to **Marc Mezzarobba, Volker Braun**",
    "created_at": "2013-10-05T09:46:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14908#issuecomment-185481",
    "user": "https://github.com/jdemeyer"
}
```

Changed reviewer from **Volker Braun** to **Marc Mezzarobba, Volker Braun**



---

archive/issue_comments_185482.json:
```json
{
    "body": "Changed author from **Marc Mezzarobba** to none",
    "created_at": "2013-10-05T09:46:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14908",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14908#issuecomment-185482",
    "user": "https://github.com/jdemeyer"
}
```

Changed author from **Marc Mezzarobba** to none
