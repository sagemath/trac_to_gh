# Issue 14328: Simplify test* targets in top-level Makefile and move logs to logs/

archive/issues_014124.json:
```json
{
    "assignees": [],
    "body": "Replace\n\n```\n$(PIPE) \"./sage -t --sagenb $(TESTDIRS) 2>&1\" \"tee -a test.log\"\n```\nby\n\n```\n./sage -t --all --logfile=logs/test.log\n```\nand analogous for other test targets.\n\n**Apply** [to `SAGE_ROOT`, and apply [attachment: trac_14328-scripts.patch.gz](https://github.com/sagemath/sage/files/ticket14328/trac_14328-scripts.patch.gz)(https://github.com/sagemath/sage/files/ticket14328/7ef15b027d28220e8ac5cadd569c2cbf.gz) to `local/bin`.\n\nAssignee: **GeorgSWeber**\n\nCC:  @roed314 @nexttime\n\nReviewer: **R. Andrew Ohana**\n\nAuthor: **Jeroen Demeyer, John Palmieri**\n\nMerged: **sage-5.9.beta4**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14328_\n\n",
    "closed_at": "2013-04-01T10:39:12Z",
    "created_at": "2013-03-21T14:12:35Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Simplify test* targets in top-level Makefile and move logs to logs/",
    "type": "issue",
    "updated_at": "2013-04-03T09:03:30Z",
    "url": "https://github.com/sagemath/sage/issues/14328",
    "user": "https://github.com/jdemeyer"
}
```
Replace

```
$(PIPE) "./sage -t --sagenb $(TESTDIRS) 2>&1" "tee -a test.log"
```
by

```
./sage -t --all --logfile=logs/test.log
```
and analogous for other test targets.

**Apply** [to `SAGE_ROOT`, and apply [attachment: trac_14328-scripts.patch.gz](https://github.com/sagemath/sage/files/ticket14328/trac_14328-scripts.patch.gz)(https://github.com/sagemath/sage/files/ticket14328/7ef15b027d28220e8ac5cadd569c2cbf.gz) to `local/bin`.

Assignee: **GeorgSWeber**

CC:  @roed314 @nexttime

Reviewer: **R. Andrew Ohana**

Author: **Jeroen Demeyer, John Palmieri**

Merged: **sage-5.9.beta4**

_Issue created by migration from https://trac.sagemath.org/ticket/14328_





---

archive/issue_events_185568.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-21T14:12:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "milestone_number": null,
    "milestone_title": "sage-5.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14328#event-185568"
}
```



---

archive/issue_events_185569.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-21T14:12:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14328#event-185569"
}
```



---

archive/issue_events_185570.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-21T14:12:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14328#event-185570"
}
```



---

archive/issue_events_185571.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-21T14:13:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14328#event-185571"
}
```



---

archive/issue_events_185572.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-29T01:49:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14328#event-185572"
}
```



---

archive/issue_comments_174316.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -9,3 +9,5 @@\n ./sage -t --all --logfile=test.log\n ```\n and analogous for other test targets.\n+\n+**Apply** [attachment: 14328_makefile_doctest_opts.patch.gz](https://github.com/sagemath/sage/files/ticket14328/14328_makefile_doctest_opts.patch.gz) to `SAGE_ROOT`.\n``````\n",
    "created_at": "2013-03-29T01:49:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14328#issuecomment-174316",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -9,3 +9,5 @@
 ./sage -t --all --logfile=test.log
 ```
 and analogous for other test targets.
+
+**Apply** [attachment: 14328_makefile_doctest_opts.patch.gz](https://github.com/sagemath/sage/files/ticket14328/14328_makefile_doctest_opts.patch.gz) to `SAGE_ROOT`.
``````




---

archive/issue_comments_174317.json:
```json
{
    "body": "Author: **Jeroen Demeyer**",
    "created_at": "2013-03-29T01:49:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14328#issuecomment-174317",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Jeroen Demeyer**



---

archive/issue_comments_174318.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -6,7 +6,7 @@\n by\n \n ```\n-./sage -t --all --logfile=test.log\n+./sage -t --all --logfile=logs/test.log\n ```\n and analogous for other test targets.\n \n``````\n",
    "created_at": "2013-03-29T02:42:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14328#issuecomment-174318",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -6,7 +6,7 @@
 by
 
 ```
-./sage -t --all --logfile=test.log
+./sage -t --all --logfile=logs/test.log
 ```
 and analogous for other test targets.
 
``````




---

archive/issue_events_185573.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-29T02:42:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14328#event-185573"
}
```



---

archive/issue_events_185574.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-29T02:42:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14328#event-185574"
}
```



---

archive/issue_events_185575.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-29T02:52:06Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "title_is": "Simplify test* targets in top-level Makefile and move logs to logs/",
    "title_was": "Simplify test* targets in top-level Makefile",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14328#event-185575"
}
```



---

archive/issue_events_185576.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-29T02:52:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14328#event-185576"
}
```



---

archive/issue_events_185577.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-03-29T02:52:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14328#event-185577"
}
```



---

archive/issue_comments_174319.json:
```json
{
    "body": "Reviewer: **R. Andrew Ohana**",
    "created_at": "2013-03-29T04:43:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14328#issuecomment-174319",
    "user": "https://github.com/ohanar"
}
```

Reviewer: **R. Andrew Ohana**



---

archive/issue_events_185578.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2013-03-29T04:43:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14328#event-185578"
}
```



---

archive/issue_events_185579.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2013-03-29T04:43:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14328#event-185579"
}
```



---

archive/issue_comments_174320.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nok, looks good and is working well",
    "created_at": "2013-03-29T04:43:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14328#issuecomment-174320",
    "user": "https://github.com/ohanar"
}
```

<a id='comment:6'>Comment 6:</a>
ok, looks good and is working well



---

archive/issue_comments_174321.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nShould you document in README.txt \u2014 or in the developer guide or the installation guide \u2014 where the log file is? And/or print a message with its location if doctesting doesn't succeed? (\"For more details, see ...\")",
    "created_at": "2013-03-29T05:37:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14328#issuecomment-174321",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:7'>Comment 7:</a>
Should you document in README.txt — or in the developer guide or the installation guide — where the log file is? And/or print a message with its location if doctesting doesn't succeed? ("For more details, see ...")



---

archive/issue_comments_174322.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nReplying to [@jhpalmieri](#comment%3A7):\n> Should you document in README.txt \u2014 or in the developer guide or the installation guide \u2014 where the log file is? And/or print a message with its location if doctesting doesn't succeed?\n\nWe never did that, should we? Nobody ever complained about this.",
    "created_at": "2013-03-29T05:56:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14328#issuecomment-174322",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:8'>Comment 8:</a>
Replying to [@jhpalmieri](#comment%3A7):
> Should you document in README.txt — or in the developer guide or the installation guide — where the log file is? And/or print a message with its location if doctesting doesn't succeed?

We never did that, should we? Nobody ever complained about this.



---

archive/issue_comments_174323.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nThat's because the log file was in the same directory where you ran `make ptest`. Now it's hidden, at least a little.\n\nBy the way, `local/bin/sage-starts` should be patched too, to put `start.log` in `logs`.",
    "created_at": "2013-03-29T14:30:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14328#issuecomment-174323",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:9'>Comment 9:</a>
That's because the log file was in the same directory where you ran `make ptest`. Now it's hidden, at least a little.

By the way, `local/bin/sage-starts` should be patched too, to put `start.log` in `logs`.



---

archive/issue_events_185580.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2013-03-29T17:17:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14328#event-185580"
}
```



---

archive/issue_events_185581.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2013-03-29T17:17:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14328#event-185581"
}
```



---

archive/issue_events_185582.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2013-03-29T17:18:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14328#event-185582"
}
```



---

archive/issue_events_185583.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2013-03-29T17:18:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14328#event-185583"
}
```



---

archive/issue_comments_174324.json:
```json
{
    "body": "Changed author from **Jeroen Demeyer** to **Jeroen Demeyer, John Palmieri**",
    "created_at": "2013-03-29T17:18:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14328#issuecomment-174324",
    "user": "https://github.com/jhpalmieri"
}
```

Changed author from **Jeroen Demeyer** to **Jeroen Demeyer, John Palmieri**



---

archive/issue_comments_174325.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -10,4 +10,4 @@\n ```\n and analogous for other test targets.\n \n-**Apply** [attachment: 14328_makefile_doctest_opts.patch.gz](https://github.com/sagemath/sage/files/ticket14328/14328_makefile_doctest_opts.patch.gz) to `SAGE_ROOT`.\n+**Apply** [to `SAGE_ROOT`, and apply [attachment: trac_14328-scripts.patch.gz](https://github.com/sagemath/sage/files/ticket14328/trac_14328-scripts.patch.gz)(https://github.com/sagemath/sage/files/ticket14328/7ef15b027d28220e8ac5cadd569c2cbf.gz) to `local/bin`.\n``````\n",
    "created_at": "2013-03-29T17:18:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14328#issuecomment-174325",
    "user": "https://github.com/jhpalmieri"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -10,4 +10,4 @@
 ```
 and analogous for other test targets.
 
-**Apply** [attachment: 14328_makefile_doctest_opts.patch.gz](https://github.com/sagemath/sage/files/ticket14328/14328_makefile_doctest_opts.patch.gz) to `SAGE_ROOT`.
+**Apply** [to `SAGE_ROOT`, and apply [attachment: trac_14328-scripts.patch.gz](https://github.com/sagemath/sage/files/ticket14328/trac_14328-scripts.patch.gz)(https://github.com/sagemath/sage/files/ticket14328/7ef15b027d28220e8ac5cadd569c2cbf.gz) to `local/bin`.
``````




---

archive/issue_comments_174326.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nAttachment: **[trac_14328-scripts.patch.gz](https://github.com/sagemath/sage/files/ticket14328/trac_14328-scripts.patch.gz)**\n\n[attachment: trac_14328-scripts.patch.gz](https://github.com/sagemath/sage/files/ticket14328/trac_14328-scripts.patch.gz) needs review.",
    "created_at": "2013-03-29T17:18:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14328#issuecomment-174326",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:11'>Comment 11:</a>
Attachment: **[trac_14328-scripts.patch.gz](https://github.com/sagemath/sage/files/ticket14328/trac_14328-scripts.patch.gz)**

[attachment: trac_14328-scripts.patch.gz](https://github.com/sagemath/sage/files/ticket14328/trac_14328-scripts.patch.gz) needs review.



---

archive/issue_comments_174327.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\n(I don't care that much about the location of the log files. If you want, we can try it as is, and if we start seeing questions on sage-release or sage-devel, then we'll fix it.)",
    "created_at": "2013-03-29T17:29:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14328#issuecomment-174327",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:12'>Comment 12:</a>
(I don't care that much about the location of the log files. If you want, we can try it as is, and if we start seeing questions on sage-release or sage-devel, then we'll fix it.)



---

archive/issue_comments_174328.json:
```json
{
    "body": "apply to root repository",
    "created_at": "2013-03-29T17:46:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14328#issuecomment-174328",
    "user": "https://github.com/ohanar"
}
```

apply to root repository



---

archive/issue_comments_174329.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -10,4 +10,4 @@\n ```\n and analogous for other test targets.\n \n-**Apply** [to `SAGE_ROOT`, and apply [attachment: trac_14328-scripts.patch.gz](https://github.com/sagemath/sage/files/ticket14328/trac_14328-scripts.patch.gz)(https://github.com/sagemath/sage/files/ticket14328/7ef15b027d28220e8ac5cadd569c2cbf.gz) to `local/bin`.\n+**Apply** [and [attachment: trac14328_hgignore.patch.gz](https://github.com/sagemath/sage/files/ticket14328/trac14328_hgignore.patch.gz) to `SAGE_ROOT`, and apply [attachment: trac_14328-scripts.patch.gz](https://github.com/sagemath/sage/files/ticket14328/trac_14328-scripts.patch.gz)(https://github.com/sagemath/sage/files/ticket14328/7ef15b027d28220e8ac5cadd569c2cbf.gz) to `local/bin`.\n``````\n",
    "created_at": "2013-03-29T17:47:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14328#issuecomment-174329",
    "user": "https://github.com/ohanar"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -10,4 +10,4 @@
 ```
 and analogous for other test targets.
 
-**Apply** [to `SAGE_ROOT`, and apply [attachment: trac_14328-scripts.patch.gz](https://github.com/sagemath/sage/files/ticket14328/trac_14328-scripts.patch.gz)(https://github.com/sagemath/sage/files/ticket14328/7ef15b027d28220e8ac5cadd569c2cbf.gz) to `local/bin`.
+**Apply** [and [attachment: trac14328_hgignore.patch.gz](https://github.com/sagemath/sage/files/ticket14328/trac14328_hgignore.patch.gz) to `SAGE_ROOT`, and apply [attachment: trac_14328-scripts.patch.gz](https://github.com/sagemath/sage/files/ticket14328/trac_14328-scripts.patch.gz)(https://github.com/sagemath/sage/files/ticket14328/7ef15b027d28220e8ac5cadd569c2cbf.gz) to `local/bin`.
``````




---

archive/issue_comments_174330.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nAttachment: **[trac14328_hgignore.patch.gz](https://github.com/sagemath/sage/files/ticket14328/trac14328_hgignore.patch.gz)**\n\nWe should also update the hgignore to reflect the change in logs location.",
    "created_at": "2013-03-29T17:47:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14328#issuecomment-174330",
    "user": "https://github.com/ohanar"
}
```

<a id='comment:13'>Comment 13:</a>
Attachment: **[trac14328_hgignore.patch.gz](https://github.com/sagemath/sage/files/ticket14328/trac14328_hgignore.patch.gz)**

We should also update the hgignore to reflect the change in logs location.



---

archive/issue_comments_174331.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nFor `.hgignore`, it's better to leave the old patterns in for upgrades of Sage.",
    "created_at": "2013-03-29T17:56:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14328#issuecomment-174331",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:14'>Comment 14:</a>
For `.hgignore`, it's better to leave the old patterns in for upgrades of Sage.



---

archive/issue_comments_174332.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -10,4 +10,4 @@\n ```\n and analogous for other test targets.\n \n-**Apply** [and [attachment: trac14328_hgignore.patch.gz](https://github.com/sagemath/sage/files/ticket14328/trac14328_hgignore.patch.gz) to `SAGE_ROOT`, and apply [attachment: trac_14328-scripts.patch.gz](https://github.com/sagemath/sage/files/ticket14328/trac_14328-scripts.patch.gz)(https://github.com/sagemath/sage/files/ticket14328/7ef15b027d28220e8ac5cadd569c2cbf.gz) to `local/bin`.\n+**Apply** [to `SAGE_ROOT`, and apply [attachment: trac_14328-scripts.patch.gz](https://github.com/sagemath/sage/files/ticket14328/trac_14328-scripts.patch.gz)(https://github.com/sagemath/sage/files/ticket14328/7ef15b027d28220e8ac5cadd569c2cbf.gz) to `local/bin`.\n``````\n",
    "created_at": "2013-03-29T17:58:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14328#issuecomment-174332",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -10,4 +10,4 @@
 ```
 and analogous for other test targets.
 
-**Apply** [and [attachment: trac14328_hgignore.patch.gz](https://github.com/sagemath/sage/files/ticket14328/trac14328_hgignore.patch.gz) to `SAGE_ROOT`, and apply [attachment: trac_14328-scripts.patch.gz](https://github.com/sagemath/sage/files/ticket14328/trac_14328-scripts.patch.gz)(https://github.com/sagemath/sage/files/ticket14328/7ef15b027d28220e8ac5cadd569c2cbf.gz) to `local/bin`.
+**Apply** [to `SAGE_ROOT`, and apply [attachment: trac_14328-scripts.patch.gz](https://github.com/sagemath/sage/files/ticket14328/trac_14328-scripts.patch.gz)(https://github.com/sagemath/sage/files/ticket14328/7ef15b027d28220e8ac5cadd569c2cbf.gz) to `local/bin`.
``````




---

archive/issue_comments_174333.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nAttachment: **[14328_makefile_doctest_opts.patch.gz](https://github.com/sagemath/sage/files/ticket14328/14328_makefile_doctest_opts.patch.gz)**\n\nok, I think all is good now (other than maybe documentation)",
    "created_at": "2013-03-29T18:39:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14328#issuecomment-174333",
    "user": "https://github.com/ohanar"
}
```

<a id='comment:16'>Comment 16:</a>
Attachment: **[14328_makefile_doctest_opts.patch.gz](https://github.com/sagemath/sage/files/ticket14328/14328_makefile_doctest_opts.patch.gz)**

ok, I think all is good now (other than maybe documentation)



---

archive/issue_events_185584.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2013-03-29T18:39:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14328#event-185584"
}
```



---

archive/issue_events_185585.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2013-03-29T18:39:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14328#event-185585"
}
```



---

archive/issue_comments_174334.json:
```json
{
    "body": "Merged: **sage-5.9.beta3**",
    "created_at": "2013-04-01T10:39:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14328#issuecomment-174334",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.9.beta3**



---

archive/issue_events_185586.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-04-01T10:39:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14328#event-185586"
}
```



---

archive/issue_events_185587.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-04-01T10:39:12Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14328#event-185587"
}
```



---

archive/issue_comments_174335.json:
```json
{
    "body": "Changed merged from **sage-5.9.beta3** to **sage-5.9.beta4**",
    "created_at": "2013-04-03T09:03:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14328",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14328#issuecomment-174335",
    "user": "https://github.com/jdemeyer"
}
```

Changed merged from **sage-5.9.beta3** to **sage-5.9.beta4**
