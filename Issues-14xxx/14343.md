# Issue 14343: improve the installation guide for SageTeX

archive/issues_014139.json:
```json
{
    "body": "The installation guide for SageTeX needs to be rearranged to be more friendly to beginning users. See https://groups.google.com/d/topic/sage-edu/yhQoMuuLLv8/discussion  which links to [this review of Sage](http://cunymathblog.commons.gc.cuny.edu/2012/04/09/sage/). \n\nI will base any patch here on the results of #14246.\n\n\n\nAssignee: mvngu\n\nReviewer: Dima Pasechnik, Karl-Dieter Crisman\n\nBranch: u/ddrake/sagetex-install-guide-14343\n\nDependencies: #14246\n\nCommit: d6b008042a3af9bd03dab2288f9c5583b9875ae7\n\nResolution: invalid\n\nIssue created by migration from https://trac.sagemath.org/ticket/14343\n\n",
    "closed_at": "2019-02-26T13:58:00Z",
    "created_at": "2013-03-23T00:30:37Z",
    "labels": [
        "component: documentation",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "improve the installation guide for SageTeX",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/14343",
    "user": "https://github.com/dandrake"
}
```
The installation guide for SageTeX needs to be rearranged to be more friendly to beginning users. See https://groups.google.com/d/topic/sage-edu/yhQoMuuLLv8/discussion  which links to [this review of Sage](http://cunymathblog.commons.gc.cuny.edu/2012/04/09/sage/). 

I will base any patch here on the results of #14246.



Assignee: mvngu

Reviewer: Dima Pasechnik, Karl-Dieter Crisman

Branch: u/ddrake/sagetex-install-guide-14343

Dependencies: #14246

Commit: d6b008042a3af9bd03dab2288f9c5583b9875ae7

Resolution: invalid

Issue created by migration from https://trac.sagemath.org/ticket/14343





---

archive/issue_events_040647.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2013-12-17T18:39:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14343#event-40647"
}
```



---

archive/issue_events_040648.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "milestone": "sage-6.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14343#event-40648"
}
```



---

archive/issue_events_040649.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14343#event-40649"
}
```



---

archive/issue_events_040650.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14343#event-40650"
}
```



---

archive/issue_events_040651.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:20:58Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14343#event-40651"
}
```



---

archive/issue_events_040652.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14343#event-40652"
}
```



---

archive/issue_events_040653.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14343#event-40653"
}
```



---

archive/issue_comments_221607.json:
```json
{
    "body": "Changing commit from \"\" to \"d6b008042a3af9bd03dab2288f9c5583b9875ae7\"",
    "created_at": "2015-08-27T02:32:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14343#issuecomment-221607",
    "user": "https://github.com/dandrake"
}
```

Changing commit from "" to "d6b008042a3af9bd03dab2288f9c5583b9875ae7"



---

archive/issue_comments_221608.json:
```json
{
    "body": "Changing branch from \"\" to \"u/ddrake/sagetex-install-guide-14343\"",
    "created_at": "2015-08-27T02:32:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14343#issuecomment-221608",
    "user": "https://github.com/dandrake"
}
```

Changing branch from "" to "u/ddrake/sagetex-install-guide-14343"



---

archive/issue_comments_221609.json:
```json
{
    "body": "<a id='comment:5'></a>I've updated `doc/en/tutorial/sagetex.rst`. In particular:\n\n* I put the installation instructions above the example, since the example is useless until TeX can find `sagetex.sty`.\n* The review of Sage describes a spectacularly unpleasant experience getting SageTeX to work. Some of his problems, I am sure, were related to the errant `sagetex.sty` that was included in TeXLive 2009 and which caused much misery. But it seems like he spent a long time trying to define systemwide environment variables. Yikes. So I demoted the `TEXINPUTS` bit in the installation guide.\n* I included some info on symlinking, which is actually the very best way to let TeX find the `.sty` file.\n* I updated the location of the source code, which is now in `...texmf/tex/latex/sagetex` instead of `...texmf/tex/generic/sagetex`.\n* Some other cruft was deleted.\n \n---\nNew commits:",
    "created_at": "2015-08-27T02:32:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14343#issuecomment-221609",
    "user": "https://github.com/dandrake"
}
```

<a id='comment:5'></a>I've updated `doc/en/tutorial/sagetex.rst`. In particular:

* I put the installation instructions above the example, since the example is useless until TeX can find `sagetex.sty`.
* The review of Sage describes a spectacularly unpleasant experience getting SageTeX to work. Some of his problems, I am sure, were related to the errant `sagetex.sty` that was included in TeXLive 2009 and which caused much misery. But it seems like he spent a long time trying to define systemwide environment variables. Yikes. So I demoted the `TEXINPUTS` bit in the installation guide.
* I included some info on symlinking, which is actually the very best way to let TeX find the `.sty` file.
* I updated the location of the source code, which is now in `...texmf/tex/latex/sagetex` instead of `...texmf/tex/generic/sagetex`.
* Some other cruft was deleted.
 
---
New commits:



---

archive/issue_comments_221610.json:
```json
{
    "body": "Changing author from \"\" to \"Dan Drake\"",
    "created_at": "2015-08-27T02:32:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14343#issuecomment-221610",
    "user": "https://github.com/dandrake"
}
```

Changing author from "" to "Dan Drake"



---

archive/issue_comments_221611.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2015-08-27T04:44:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14343#issuecomment-221611",
    "user": "https://github.com/dandrake"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_221612.json:
```json
{
    "body": "<a id='comment:7'></a>I like this overall but feel like \n\n```\n+      mkdir -p TEXMFHOME/tex/latex\n+      ln -s SAGE_ROOT/local/share/texmf/tex/latex/sagetex/sagetex.sty TEXMFHOME/tex/latex/sagetex.sty \n```\nonly works if people are always installing Sage in the same place every time, which might not be the case, especially if they are downloading binaries that are named after both the Sage version and the system (in the Mac app case, at the bottom of a long path via the app bundle).  What do you think a good way to deal with that is?  I don't want to have a crash course in filesystems, but this could be a (small) piece of the confusion.\n\nAlso, the \"spectacularly unpleasant experience\" is very similar to #18951, or at least to the email I got which precipitated my opening that ticket... I wonder if sometimes following even one step wrong causes the other steps to just \"not work\"?",
    "created_at": "2015-08-27T13:07:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14343#issuecomment-221612",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:7'></a>I like this overall but feel like 

```
+      mkdir -p TEXMFHOME/tex/latex
+      ln -s SAGE_ROOT/local/share/texmf/tex/latex/sagetex/sagetex.sty TEXMFHOME/tex/latex/sagetex.sty 
```
only works if people are always installing Sage in the same place every time, which might not be the case, especially if they are downloading binaries that are named after both the Sage version and the system (in the Mac app case, at the bottom of a long path via the app bundle).  What do you think a good way to deal with that is?  I don't want to have a crash course in filesystems, but this could be a (small) piece of the confusion.

Also, the "spectacularly unpleasant experience" is very similar to #18951, or at least to the email I got which precipitated my opening that ticket... I wonder if sometimes following even one step wrong causes the other steps to just "not work"?



---

archive/issue_comments_221613.json:
```json
{
    "body": "<a id='comment:8'></a>Replying to [comment:7 kcrisman]:\n> I like this overall but feel like \n> \n> ```\n> +      mkdir -p TEXMFHOME/tex/latex\n> +      ln -s SAGE_ROOT/local/share/texmf/tex/latex/sagetex/sagetex.sty TEXMFHOME/tex/latex/sagetex.sty \n> ```\n> only works if people are always installing Sage in the same place every time,\n\n\nOur releases have version and system stuff in the name, so users will have to copy the TeX stuff over...or rename their download so the symlink works. They have to do something and I don't know a good way to automate it. I wanted to add the symlink idea because that's what the TeXShop people suggest now -- and it's what I do.\n\nAlso, how do people actually run Sage? Don't they need to rename the sage-x.y.z directory? In the SageTeX documentation we always assume that \"``sage``\" is in their path, so how are users accomplishing that?\n\nShould I just remove the symlink suggestion?\n\n> Also, the \"spectacularly unpleasant experience\" is very similar to #18951, or at least to the email I got which precipitated my opening that ticket... I wonder if sometimes following even one step wrong causes the other steps to just \"not work\"?\n\n\nFor a certain class of users, I'm sure it does. If you have a poor understanding of Unix-like filesystems, paths, etc, then I'm sure it's easy to take one misstep and find yourself hopelessly lost.",
    "created_at": "2015-08-27T15:56:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14343#issuecomment-221613",
    "user": "https://github.com/dandrake"
}
```

<a id='comment:8'></a>Replying to [comment:7 kcrisman]:
> I like this overall but feel like 
> 
> ```
> +      mkdir -p TEXMFHOME/tex/latex
> +      ln -s SAGE_ROOT/local/share/texmf/tex/latex/sagetex/sagetex.sty TEXMFHOME/tex/latex/sagetex.sty 
> ```
> only works if people are always installing Sage in the same place every time,


Our releases have version and system stuff in the name, so users will have to copy the TeX stuff over...or rename their download so the symlink works. They have to do something and I don't know a good way to automate it. I wanted to add the symlink idea because that's what the TeXShop people suggest now -- and it's what I do.

Also, how do people actually run Sage? Don't they need to rename the sage-x.y.z directory? In the SageTeX documentation we always assume that "``sage``" is in their path, so how are users accomplishing that?

Should I just remove the symlink suggestion?

> Also, the "spectacularly unpleasant experience" is very similar to #18951, or at least to the email I got which precipitated my opening that ticket... I wonder if sometimes following even one step wrong causes the other steps to just "not work"?


For a certain class of users, I'm sure it does. If you have a poor understanding of Unix-like filesystems, paths, etc, then I'm sure it's easy to take one misstep and find yourself hopelessly lost.



---

archive/issue_comments_221614.json:
```json
{
    "body": "<a id='comment:9'></a>> Our releases have version and system stuff in the name, so users will have to copy the TeX stuff over...or rename their download so the symlink works. They have to do something and I don't know a good way to automate it. I wanted to add the symlink idea because that's what the TeXShop people suggest now -- and it's what I do.\n> \n> Also, how do people actually run Sage? Don't they need to rename the sage-x.y.z directory? In the SageTeX documentation we always assume that \"``sage``\" is in their path, so how are users accomplishing that?\n> \n> Should I just remove the symlink suggestion?\n\n\nNO NO NO do not remove it!  My point is just that perhaps there needs to be a reminder \"there is this whole Unixy thing you need to do to make some of this work, and that includes having the `sage` in a specific spot as well as the folder itself with the same name each time.  (Because just having `sage` in your path isn't the same as keeping it the same all the time; I have to manually redo the symlink for my Sage install when I upgrade my \"main\" Sage (as opposed to my development copy).",
    "created_at": "2015-08-27T17:54:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14343#issuecomment-221614",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:9'></a>> Our releases have version and system stuff in the name, so users will have to copy the TeX stuff over...or rename their download so the symlink works. They have to do something and I don't know a good way to automate it. I wanted to add the symlink idea because that's what the TeXShop people suggest now -- and it's what I do.
> 
> Also, how do people actually run Sage? Don't they need to rename the sage-x.y.z directory? In the SageTeX documentation we always assume that "``sage``" is in their path, so how are users accomplishing that?
> 
> Should I just remove the symlink suggestion?


NO NO NO do not remove it!  My point is just that perhaps there needs to be a reminder "there is this whole Unixy thing you need to do to make some of this work, and that includes having the `sage` in a specific spot as well as the folder itself with the same name each time.  (Because just having `sage` in your path isn't the same as keeping it the same all the time; I have to manually redo the symlink for my Sage install when I upgrade my "main" Sage (as opposed to my development copy).



---

archive/issue_comments_221615.json:
```json
{
    "body": "<a id='comment:10'></a>Unfortunately, sage-6.9.beta7 still ships a version of sagetex which puts its files into `tex/generic`, cf. #15900. So the note on \"older versions of SageTeX\" is not quite correct.\nOTOH, this point will be moot once SageTeX 3.0 is merged (#19072).",
    "created_at": "2015-09-25T08:46:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14343#issuecomment-221615",
    "user": "https://github.com/cheuberg"
}
```

<a id='comment:10'></a>Unfortunately, sage-6.9.beta7 still ships a version of sagetex which puts its files into `tex/generic`, cf. #15900. So the note on "older versions of SageTeX" is not quite correct.
OTOH, this point will be moot once SageTeX 3.0 is merged (#19072).



---

archive/issue_events_040654.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2015-09-25T08:46:07Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14343#event-40654"
}
```



---

archive/issue_events_040655.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2015-09-25T08:46:07Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "milestone": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14343#event-40655"
}
```



---

archive/issue_comments_221616.json:
```json
{
    "body": "Changing status from needs_review to needs_info.",
    "created_at": "2017-03-02T18:26:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14343#issuecomment-221616",
    "user": "https://github.com/kcrisman"
}
```

Changing status from needs_review to needs_info.



---

archive/issue_comments_221617.json:
```json
{
    "body": "<a id='comment:11'></a>What is the status here?  Is it in fact obsolete?  Also, see #22499 which might make dealing with this kind of thing easier.",
    "created_at": "2017-03-02T18:26:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14343#issuecomment-221617",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:11'></a>What is the status here?  Is it in fact obsolete?  Also, see #22499 which might make dealing with this kind of thing easier.



---

archive/issue_comments_221618.json:
```json
{
    "body": "<a id='comment:12'></a>Apparently Sage still at least in some places refers to SageTeX being on bitbucket, so that should be fixed too.  Any file of the form `src/doc/**/tutorial/sagetex.rst` needs to be changed.",
    "created_at": "2017-06-07T12:58:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14343#issuecomment-221618",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:12'></a>Apparently Sage still at least in some places refers to SageTeX being on bitbucket, so that should be fixed too.  Any file of the form `src/doc/**/tutorial/sagetex.rst` needs to be changed.



---

archive/issue_comments_221619.json:
```json
{
    "body": "<a id='comment:13'></a>Given the move to sagemath/sagetex on GH, this ticket should now be only about retrieving any part of ddrake's branch that is not already in the user guide and still relevant.  We've already improved the user guide incrementally over the years.",
    "created_at": "2017-11-30T18:04:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14343#issuecomment-221619",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:13'></a>Given the move to sagemath/sagetex on GH, this ticket should now be only about retrieving any part of ddrake's branch that is not already in the user guide and still relevant.  We've already improved the user guide incrementally over the years.



---

archive/issue_comments_221620.json:
```json
{
    "body": "<a id='comment:14'></a>I think this can now be closed, as there have been enough changes that it would be very hard to figure out what should be done on this ticket (and you can't click on the branch in any case).  I do encourage others (including myself) to open a new ticket with any specific new suggestions for improving the install guide!",
    "created_at": "2019-01-07T21:56:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14343#issuecomment-221620",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:14'></a>I think this can now be closed, as there have been enough changes that it would be very hard to figure out what should be done on this ticket (and you can't click on the branch in any case).  I do encourage others (including myself) to open a new ticket with any specific new suggestions for improving the install guide!



---

archive/issue_comments_221621.json:
```json
{
    "body": "Changing status from needs_info to needs_review.",
    "created_at": "2019-01-07T21:56:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14343#issuecomment-221621",
    "user": "https://github.com/kcrisman"
}
```

Changing status from needs_info to needs_review.



---

archive/issue_comments_221622.json:
```json
{
    "body": "Changing author from \"Dan Drake\" to \"\"",
    "created_at": "2019-01-07T21:56:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14343#issuecomment-221622",
    "user": "https://github.com/kcrisman"
}
```

Changing author from "Dan Drake" to ""



---

archive/issue_events_040656.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2019-01-07T21:56:15Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "milestone": "sage-6.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14343#event-40656"
}
```



---

archive/issue_events_040657.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2019-01-07T21:56:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14343#event-40657"
}
```



---

archive/issue_comments_221623.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Dima Pasechnik\"",
    "created_at": "2019-01-10T06:22:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14343#issuecomment-221623",
    "user": "https://github.com/dimpase"
}
```

Changing reviewer from "" to "Dima Pasechnik"



---

archive/issue_comments_221624.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2019-01-10T06:22:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14343#issuecomment-221624",
    "user": "https://github.com/dimpase"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_221625.json:
```json
{
    "body": "Changing reviewer from \"Dima Pasechnik\" to \"Dima Pasechnik, Karl-Dieter Crisman\"",
    "created_at": "2019-01-10T13:34:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14343#issuecomment-221625",
    "user": "https://github.com/kcrisman"
}
```

Changing reviewer from "Dima Pasechnik" to "Dima Pasechnik, Karl-Dieter Crisman"



---

archive/issue_comments_221626.json:
```json
{
    "body": "<a id='comment:17'></a>Presuming these are all correctly reviewed as either duplicate, invalid, or wontfix.",
    "created_at": "2019-02-26T13:58:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14343#issuecomment-221626",
    "user": "https://github.com/embray"
}
```

<a id='comment:17'></a>Presuming these are all correctly reviewed as either duplicate, invalid, or wontfix.



---

archive/issue_events_040658.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-02-26T13:58:00Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14343#event-40658"
}
```



---

archive/issue_comments_221627.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2019-02-26T13:58:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14343",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14343#issuecomment-221627",
    "user": "https://github.com/embray"
}
```

Resolution: invalid
