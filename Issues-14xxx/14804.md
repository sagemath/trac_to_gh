# Issue 14804: transition to using install program

archive/issues_014600.json:
```json
{
    "body": "To keep track of installed files, it's probably most convenient to use a unified install program within all spkg-install scripts, like the one included within #14796.\n\nFor transitional purposes, the non-#14796 build-system should provide an install program within $PATH. I propose to place it as `sage-dist-install` into `$SAGE_SRC/bin`. It should just ignore -F <filelist> for now.\n\nFor python-distutils packages, a setup.py wrapper is included.\n\nAssignee: tbd\n\nCC:  @nexttime @jondo\n\nKeywords: spkg-install filelist destdir\n\nAuthor: Felix Salfelder\n\nBranch: u/felixs/sagedist\n\nCommit: 258434416b495ccf4e81d1c97b244ceec9a26526\n\nResolution: wontfix\n\nIssue created by migration from https://trac.sagemath.org/ticket/14804\n\n",
    "closed_at": "2018-04-04T08:58:34Z",
    "created_at": "2013-06-22T08:38:09Z",
    "labels": [
        "component: distribution",
        "critical"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "transition to using install program",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/14804",
    "user": "https://trac.sagemath.org/admin/accounts/users/felixs"
}
```
To keep track of installed files, it's probably most convenient to use a unified install program within all spkg-install scripts, like the one included within #14796.

For transitional purposes, the non-#14796 build-system should provide an install program within $PATH. I propose to place it as `sage-dist-install` into `$SAGE_SRC/bin`. It should just ignore -F <filelist> for now.

For python-distutils packages, a setup.py wrapper is included.

Assignee: tbd

CC:  @nexttime @jondo

Keywords: spkg-install filelist destdir

Author: Felix Salfelder

Branch: u/felixs/sagedist

Commit: 258434416b495ccf4e81d1c97b244ceec9a26526

Resolution: wontfix

Issue created by migration from https://trac.sagemath.org/ticket/14804





---

archive/issue_comments_232440.json:
```json
{
    "body": "Attachment [sage-dist-install](tarball://root/attachments/some-uuid/ticket14804/sage-dist-install) by felixs created at 2013-06-22 08:38:28",
    "created_at": "2013-06-22T08:38:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14804#issuecomment-232440",
    "user": "https://trac.sagemath.org/admin/accounts/users/felixs"
}
```

Attachment [sage-dist-install](tarball://root/attachments/some-uuid/ticket14804/sage-dist-install) by felixs created at 2013-06-22 08:38:28



---

archive/issue_comments_232441.json:
```json
{
    "body": "<a id='comment:2'></a>Python distutils at least encapsulates the file operations in `distutils.file_utils`, so it would be a finite task to hook into that as well.\n\nThe alternative is to serialize the install phase and diff the `SAGE_ROOT` tree before and after.\n\nYet another possibility is to use separate install trees for each spkg in \"sage-the-distribution\", as in Nix or Stow.",
    "created_at": "2013-06-23T15:44:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14804#issuecomment-232441",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:2'></a>Python distutils at least encapsulates the file operations in `distutils.file_utils`, so it would be a finite task to hook into that as well.

The alternative is to serialize the install phase and diff the `SAGE_ROOT` tree before and after.

Yet another possibility is to use separate install trees for each spkg in "sage-the-distribution", as in Nix or Stow.



---

archive/issue_comments_232442.json:
```json
{
    "body": "<a id='comment:3'></a>Replying to [vbraun](#comment%3A2):\n> Python distutils at least encapsulates the file operations in `distutils.file_utils`, so it would be a finite task to hook into that as well.\n\n\nFor example. but i have no idea how to \"hook into that\" yet without patching distutils. is there any python magic you have in mind?\n\n> The alternative is to serialize the install phase and diff the `SAGE_ROOT` tree before and after.\n\n\nWon't give me DESTDIR. serialization is one of the uglier hacks (no way). diff takes quadratic time (even less way).\n\n> Yet another possibility is to use separate install trees for each spkg in \"sage-the-distribution\", as in Nix or Stow.\n\n\nJust complicates the issue. better is supporting no \"install\" at all (like sage-doc currently does).",
    "created_at": "2013-06-23T16:06:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14804#issuecomment-232442",
    "user": "https://trac.sagemath.org/admin/accounts/users/felixs"
}
```

<a id='comment:3'></a>Replying to [vbraun](#comment%3A2):
> Python distutils at least encapsulates the file operations in `distutils.file_utils`, so it would be a finite task to hook into that as well.


For example. but i have no idea how to "hook into that" yet without patching distutils. is there any python magic you have in mind?

> The alternative is to serialize the install phase and diff the `SAGE_ROOT` tree before and after.


Won't give me DESTDIR. serialization is one of the uglier hacks (no way). diff takes quadratic time (even less way).

> Yet another possibility is to use separate install trees for each spkg in "sage-the-distribution", as in Nix or Stow.


Just complicates the issue. better is supporting no "install" at all (like sage-doc currently does).



---

archive/issue_comments_232443.json:
```json
{
    "body": "<a id='comment:4'></a>Replying to [felixs](#comment%3A3):\n> Replying to [vbraun](#comment%3A2):\n> > Python distutils at least encapsulates the file operations in `distutils.file_utils`, so it would be a finite task to hook into that as well.\n\n> \n> For example. but i have no idea how to \"hook into that\" yet without patching distutils. is there any python magic you have in mind?\n\n\nYou can patch it (Python / setuptools / whatever), but you don't necessarily have to, as most Python objects are mutable, including functions / methods, i.e., you can overwrite them... :-)\n\n```\nIn [1]: import distutils.file_util\n\nIn [2]: def foo(s,t): print \"Hello\"\n   ...: \n\nIn [3]: distutils.file_util.copy_file=foo\n\nIn [4]: distutils.file_util.copy_file(\"bar\",\"baz\")\nHello\n```\n\n\nAnd you could e.g. do so in an spkg's `setup.py`, to use some alternate implementation (provided elsewhere).",
    "created_at": "2013-06-23T16:51:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14804#issuecomment-232443",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:4'></a>Replying to [felixs](#comment%3A3):
> Replying to [vbraun](#comment%3A2):
> > Python distutils at least encapsulates the file operations in `distutils.file_utils`, so it would be a finite task to hook into that as well.

> 
> For example. but i have no idea how to "hook into that" yet without patching distutils. is there any python magic you have in mind?


You can patch it (Python / setuptools / whatever), but you don't necessarily have to, as most Python objects are mutable, including functions / methods, i.e., you can overwrite them... :-)

```
In [1]: import distutils.file_util

In [2]: def foo(s,t): print "Hello"
   ...: 

In [3]: distutils.file_util.copy_file=foo

In [4]: distutils.file_util.copy_file("bar","baz")
Hello
```


And you could e.g. do so in an spkg's `setup.py`, to use some alternate implementation (provided elsewhere).



---

archive/issue_comments_232444.json:
```json
{
    "body": "<a id='comment:5'></a>Replying to [leif](#comment%3A4):\n\n> You can patch it (Python / setuptools / whatever), but you don't necessarily have to,\n\n\nI dont *want* to. I'd just patch upstream if it's completely hosed.\n\n> as most Python objects are mutable, including functions / methods, i.e., you can overwrite them... :-)\n> \n\n>...\n> \n> \n> And you could e.g. do so in an spkg's `setup.py`, to use some alternate implementation (provided elsewhere). \n\n\nI don't quite understand. Are you proposing to patch the upstream package? (this would be worse, as it involves more than just distutils patching).\n\nI need something of the kind\n\n```\nsetup.py install USEMYOWNFILEUTIL\n```\nor\n\n```\nIMPORTPATH=myhackedfileutil_path setup.py install\n```\nwithin spkg-install. is this even possible?",
    "created_at": "2013-06-23T17:21:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14804#issuecomment-232444",
    "user": "https://trac.sagemath.org/admin/accounts/users/felixs"
}
```

<a id='comment:5'></a>Replying to [leif](#comment%3A4):

> You can patch it (Python / setuptools / whatever), but you don't necessarily have to,


I dont *want* to. I'd just patch upstream if it's completely hosed.

> as most Python objects are mutable, including functions / methods, i.e., you can overwrite them... :-)
> 

>...
> 
> 
> And you could e.g. do so in an spkg's `setup.py`, to use some alternate implementation (provided elsewhere). 


I don't quite understand. Are you proposing to patch the upstream package? (this would be worse, as it involves more than just distutils patching).

I need something of the kind

```
setup.py install USEMYOWNFILEUTIL
```
or

```
IMPORTPATH=myhackedfileutil_path setup.py install
```
within spkg-install. is this even possible?



---

archive/issue_comments_232445.json:
```json
{
    "body": "<a id='comment:6'></a>Replying to [felixs](#comment%3A5):\n> Replying to [leif](#comment%3A4):\n> > as most Python objects are mutable, including functions / methods, i.e., you can overwrite them... :-)\n> > And you could e.g. do so in an spkg's `setup.py`, to use some alternate implementation (provided elsewhere). \n\n> \n> I don't quite understand. Are you proposing to patch the upstream package? (this would be worse, as it involves more than just distutils patching).\n> \n> I need something of the kind\n> \n> ```\n> setup.py install USEMYOWNFILEUTIL\n> ```\n> or\n> \n> ```\n> IMPORTPATH=myhackedfileutil_path setup.py install\n> ```\n> within spkg-install. is this even possible?\n\n\nSure, I was just writing a post scriptum:\n\nAnd in the latter case, you wouldn't even have to patch / modify the spkg's / upstream's `setup.py`, but just `spkg-install` (or whatever that will become), as you can also do things like\n\n```sh\npython <<EOF\n# Do some stuff, e.g. modifying distutils.file_util.*\n# (preferably importing an alternate implementation located elsewhere)\nimport sys\nsys.argv = [\"setup.py\", \"install\"]\nexecfile(\"setup.py\")\nEOF\n```\n\nOr just create a (shell) script that does similar, and call that instead of doing `python setup.py install`.",
    "created_at": "2013-06-23T17:33:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14804#issuecomment-232445",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:6'></a>Replying to [felixs](#comment%3A5):
> Replying to [leif](#comment%3A4):
> > as most Python objects are mutable, including functions / methods, i.e., you can overwrite them... :-)
> > And you could e.g. do so in an spkg's `setup.py`, to use some alternate implementation (provided elsewhere). 

> 
> I don't quite understand. Are you proposing to patch the upstream package? (this would be worse, as it involves more than just distutils patching).
> 
> I need something of the kind
> 
> ```
> setup.py install USEMYOWNFILEUTIL
> ```
> or
> 
> ```
> IMPORTPATH=myhackedfileutil_path setup.py install
> ```
> within spkg-install. is this even possible?


Sure, I was just writing a post scriptum:

And in the latter case, you wouldn't even have to patch / modify the spkg's / upstream's `setup.py`, but just `spkg-install` (or whatever that will become), as you can also do things like

```sh
python <<EOF
# Do some stuff, e.g. modifying distutils.file_util.*
# (preferably importing an alternate implementation located elsewhere)
import sys
sys.argv = ["setup.py", "install"]
execfile("setup.py")
EOF
```

Or just create a (shell) script that does similar, and call that instead of doing `python setup.py install`.



---

archive/issue_comments_232446.json:
```json
{
    "body": "<a id='comment:7'></a>P.P.S.:\n\nOf course, as is that relies on upstream **only** using distutils (at least to copy / install files), just like your custom `[sage-dist-]install` mechanism relies on upstream respecting autotools' conventions (and *exclusively* using `install[-sh]`) for such purposes.",
    "created_at": "2013-06-23T17:45:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14804#issuecomment-232446",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:7'></a>P.P.S.:

Of course, as is that relies on upstream **only** using distutils (at least to copy / install files), just like your custom `[sage-dist-]install` mechanism relies on upstream respecting autotools' conventions (and *exclusively* using `install[-sh]`) for such purposes.



---

archive/issue_comments_232447.json:
```json
{
    "body": "<a id='comment:8'></a>... but you may have to wrap `python` for some packages that use *both* (e.g.) autotools *and* Python's distutils (some support setting `PYTHON`).\n\nStill, uninstalling an spkg by (just) removing the files it added (probably restoring those it deleted or modified, too) can easily break a Sage installation, but I guess you're aware of the pitfalls (that aren't specific to Sage)...",
    "created_at": "2013-06-23T18:22:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14804#issuecomment-232447",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:8'></a>... but you may have to wrap `python` for some packages that use *both* (e.g.) autotools *and* Python's distutils (some support setting `PYTHON`).

Still, uninstalling an spkg by (just) removing the files it added (probably restoring those it deleted or modified, too) can easily break a Sage installation, but I guess you're aware of the pitfalls (that aren't specific to Sage)...



---

archive/issue_comments_232448.json:
```json
{
    "body": "<a id='comment:9'></a>Attachment [sage-setup.py](tarball://root/attachments/some-uuid/ticket14804/sage-setup.py) by felixs created at 2013-06-23 19:01:08\n\nThanks Leif, I wasn't aware of that overriding possibility.\n\nReplying to [leif](#comment%3A7):\n> \n> Of course, as is that relies on upstream **only** using distutils (at least to copy / install files), just like your custom `[sage-dist-]install` mechanism relies on upstream respecting autotools' conventions (and *exclusively* using `install[-sh]`) for such purposes.\n\n\n.. and lets see what else `copy_file` is abused for. But I think, this approach will finally cut it.\n\nautotools uses $(INSTALL), which can be overriden with anything. also others, otherwise broken make based build systems use $(INSTALL) and $(INSTALLDIRS).",
    "created_at": "2013-06-23T19:01:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14804#issuecomment-232448",
    "user": "https://trac.sagemath.org/admin/accounts/users/felixs"
}
```

<a id='comment:9'></a>Attachment [sage-setup.py](tarball://root/attachments/some-uuid/ticket14804/sage-setup.py) by felixs created at 2013-06-23 19:01:08

Thanks Leif, I wasn't aware of that overriding possibility.

Replying to [leif](#comment%3A7):
> 
> Of course, as is that relies on upstream **only** using distutils (at least to copy / install files), just like your custom `[sage-dist-]install` mechanism relies on upstream respecting autotools' conventions (and *exclusively* using `install[-sh]`) for such purposes.


.. and lets see what else `copy_file` is abused for. But I think, this approach will finally cut it.

autotools uses $(INSTALL), which can be overriden with anything. also others, otherwise broken make based build systems use $(INSTALL) and $(INSTALLDIRS).



---

archive/issue_comments_232449.json:
```json
{
    "body": "<a id='comment:10'></a>Replying to [felixs](#comment%3A9):\n> .. and lets see what else `copy_file` is abused for. But I think, this approach will finally cut it.\n\n\n\nNote that your replacement (or wrapper) function will of course finally have to mimic the full behaviour of the original one, i.e., has to take the same optional arguments (with the same default values), and has to return a tuple.",
    "created_at": "2013-06-23T19:52:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14804#issuecomment-232449",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:10'></a>Replying to [felixs](#comment%3A9):
> .. and lets see what else `copy_file` is abused for. But I think, this approach will finally cut it.



Note that your replacement (or wrapper) function will of course finally have to mimic the full behaviour of the original one, i.e., has to take the same optional arguments (with the same default values), and has to return a tuple.



---

archive/issue_comments_232450.json:
```json
{
    "body": "<a id='comment:11'></a>Replying to [leif](#comment%3A10):\n> Note that your replacement (or wrapper) function will of course finally have to mimic the full behaviour of the original one, i.e., has to take the same optional arguments (with the same default values), and has to return a tuple.\n\n\nIt is supposed to call the original function after it's done with file listing.\nI'd expect that\n\n```\ndef B(*args, **kwds): return B(args, kwds)\n```\nworks, except for the default values...",
    "created_at": "2013-06-23T20:04:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14804#issuecomment-232450",
    "user": "https://trac.sagemath.org/admin/accounts/users/felixs"
}
```

<a id='comment:11'></a>Replying to [leif](#comment%3A10):
> Note that your replacement (or wrapper) function will of course finally have to mimic the full behaviour of the original one, i.e., has to take the same optional arguments (with the same default values), and has to return a tuple.


It is supposed to call the original function after it's done with file listing.
I'd expect that

```
def B(*args, **kwds): return B(args, kwds)
```
works, except for the default values...



---

archive/issue_comments_232451.json:
```json
{
    "body": "<a id='comment:12'></a>Replying to [felixs](#comment%3A11):\n> It is supposed to call the original function after it's done with file listing.\n> I'd expect that\n> \n> ```\n> def B(*args, **kwds): return B(args, kwds)\n> ```\n> works, except for the default values...\n\n\n... and infinite recursion. ;-)",
    "created_at": "2013-06-23T20:11:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14804#issuecomment-232451",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:12'></a>Replying to [felixs](#comment%3A11):
> It is supposed to call the original function after it's done with file listing.
> I'd expect that
> 
> ```
> def B(*args, **kwds): return B(args, kwds)
> ```
> works, except for the default values...


... and infinite recursion. ;-)



---

archive/issue_comments_232452.json:
```json
{
    "body": "<a id='comment:13'></a>Replying to [leif](#comment%3A12):\n> ... and infinite recursion. ;-)\n\n\nthat was typed to fast, also some asterisks are missing. as it seems it even does the right thing wrt default values...",
    "created_at": "2013-06-23T20:14:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14804#issuecomment-232452",
    "user": "https://trac.sagemath.org/admin/accounts/users/felixs"
}
```

<a id='comment:13'></a>Replying to [leif](#comment%3A12):
> ... and infinite recursion. ;-)


that was typed to fast, also some asterisks are missing. as it seems it even does the right thing wrt default values...



---

archive/issue_comments_232453.json:
```json
{
    "body": "<a id='comment:14'></a>And indeed, `setup.py install` takes `--record <filename>`. I should have tried to patch distutils right away.... :)",
    "created_at": "2013-06-23T20:52:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14804#issuecomment-232453",
    "user": "https://trac.sagemath.org/admin/accounts/users/felixs"
}
```

<a id='comment:14'></a>And indeed, `setup.py install` takes `--record <filename>`. I should have tried to patch distutils right away.... :)



---

archive/issue_comments_232454.json:
```json
{
    "body": "<a id='comment:15'></a>It *may* make sense to wrap the whole ` distutils.core.setup()` function [too]...\n\n(depending on what else you intend to do)",
    "created_at": "2013-06-23T21:35:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14804#issuecomment-232454",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:15'></a>It *may* make sense to wrap the whole ` distutils.core.setup()` function [too]...

(depending on what else you intend to do)



---

archive/issue_comments_232455.json:
```json
{
    "body": "<a id='comment:16'></a>There is now `sage-dist-install`, `sage-dist-make`, `spkg.mk` and `sage-setup.py` within my WIP branch on github.\n\nWith these, most spkg-install scripts can be augmented to support both the #14796 build system (together with #14792) as well as the current one.\n\nPlease have a look whether this could break anything.",
    "created_at": "2013-06-30T09:15:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14804#issuecomment-232455",
    "user": "https://trac.sagemath.org/admin/accounts/users/felixs"
}
```

<a id='comment:16'></a>There is now `sage-dist-install`, `sage-dist-make`, `spkg.mk` and `sage-setup.py` within my WIP branch on github.

With these, most spkg-install scripts can be augmented to support both the #14796 build system (together with #14792) as well as the current one.

Please have a look whether this could break anything.



---

archive/issue_comments_232456.json:
```json
{
    "body": "<a id='comment:17'></a>This is now in git. It works for all packages I have tried with.",
    "created_at": "2013-08-28T15:28:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14804#issuecomment-232456",
    "user": "https://trac.sagemath.org/admin/accounts/users/felixs"
}
```

<a id='comment:17'></a>This is now in git. It works for all packages I have tried with.



---

archive/issue_comments_232457.json:
```json
{
    "body": "Changing branch from \"\" to \"u/felixs/sagedist\"",
    "created_at": "2013-08-28T15:28:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14804#issuecomment-232457",
    "user": "https://trac.sagemath.org/admin/accounts/users/felixs"
}
```

Changing branch from "" to "u/felixs/sagedist"



---

archive/issue_comments_232458.json:
```json
{
    "body": "Changing commit from \"\" to \"fdeefe58eb8cb929e88666df03b5d5f48909b13e\"",
    "created_at": "2013-08-28T15:28:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14804#issuecomment-232458",
    "user": "https://trac.sagemath.org/admin/accounts/users/felixs"
}
```

Changing commit from "" to "fdeefe58eb8cb929e88666df03b5d5f48909b13e"



---

archive/issue_comments_232459.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,4 +2,4 @@\n \n For transitional purposes, the non-#14796 build-system should provide an install program within $PATH. I propose to place it as `sage-dist-install` into `$SAGE_SRC/bin`. It should just ignore -F <filelist> for now.\n \n-(NB: This won't work for python-distutils packages, as they are baking their own cakes again)\n+For python-distutils packages, a setup.py wrapper is included.\n``````\n",
    "created_at": "2013-09-03T08:17:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14804#issuecomment-232459",
    "user": "https://trac.sagemath.org/admin/accounts/users/felixs"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,4 +2,4 @@
 
 For transitional purposes, the non-#14796 build-system should provide an install program within $PATH. I propose to place it as `sage-dist-install` into `$SAGE_SRC/bin`. It should just ignore -F <filelist> for now.
 
-(NB: This won't work for python-distutils packages, as they are baking their own cakes again)
+For python-distutils packages, a setup.py wrapper is included.
``````




---

archive/issue_comments_232460.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2013-09-03T08:17:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14804#issuecomment-232460",
    "user": "https://trac.sagemath.org/admin/accounts/users/felixs"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_232461.json:
```json
{
    "body": "Changing commit from \"fdeefe58eb8cb929e88666df03b5d5f48909b13e\" to \"258434416b495ccf4e81d1c97b244ceec9a26526\"",
    "created_at": "2013-09-08T09:00:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14804#issuecomment-232461",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "fdeefe58eb8cb929e88666df03b5d5f48909b13e" to "258434416b495ccf4e81d1c97b244ceec9a26526"



---

archive/issue_comments_232462.json:
```json
{
    "body": "<a id='comment:20'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2013-09-08T09:00:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14804#issuecomment-232462",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:20'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_232463.json:
```json
{
    "body": "<a id='comment:21'></a>Needs to be rebased.",
    "created_at": "2013-12-19T12:27:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14804#issuecomment-232463",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:21'></a>Needs to be rebased.



---

archive/issue_comments_232464.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2013-12-19T12:27:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14804#issuecomment-232464",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_events_042500.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14804#event-42500"
}
```



---

archive/issue_events_042501.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:19:32Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14804#event-42501"
}
```



---

archive/issue_events_042502.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-05-06T15:19:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14804#event-42502"
}
```



---

archive/issue_events_042503.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "milestone": "sage-6.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14804#event-42503"
}
```



---

archive/issue_events_042504.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-08-10T16:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14804#event-42504"
}
```



---

archive/issue_events_042505.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-04T08:58:34Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "milestone": "sage-6.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14804#event-42505"
}
```



---

archive/issue_events_042506.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-04T08:58:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14804#event-42506"
}
```



---

archive/issue_comments_232465.json:
```json
{
    "body": "<a id='comment:25'></a>This was in a way trying to do what is now accomplished (in spirit--not so much in the details) by some combination of #23160 an #22509.  I don't think this particular approach really fits in with what we're doing now.  But thank you Felix for your early pioneering work on trying to fix Sage's build system...",
    "created_at": "2018-04-04T08:58:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14804#issuecomment-232465",
    "user": "https://github.com/embray"
}
```

<a id='comment:25'></a>This was in a way trying to do what is now accomplished (in spirit--not so much in the details) by some combination of #23160 an #22509.  I don't think this particular approach really fits in with what we're doing now.  But thank you Felix for your early pioneering work on trying to fix Sage's build system...



---

archive/issue_events_042507.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-04T08:58:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/14804#event-42507"
}
```



---

archive/issue_comments_232466.json:
```json
{
    "body": "Resolution: wontfix",
    "created_at": "2018-04-04T08:58:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/14804",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/14804#issuecomment-232466",
    "user": "https://github.com/embray"
}
```

Resolution: wontfix
