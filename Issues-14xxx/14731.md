# Issue 14731: Combinatorial map from binary trees to permutations

archive/issues_014527.json:
```json
{
    "assignees": [],
    "body": "adding combinatorial map from binary tree to 132 and 312 avoiding permutations\n\n---\n\nApply: [attachment: trac_14731-binary-trees-to-permutations.patch.gz](https://github.com/sagemath/sage/files/ticket14731/trac_14731-binary-trees-to-permutations.patch.gz)\n\nDepends on #14719\n\nAssignee: **sage-combinat**\n\nCC:  stumpc5 chrisjamesberg @tscrim\n\nKeywords: **BinaryTrees, Permutations, FindStatDays01**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Viviane Pons**\n\nMerged: **sage-5.11.beta2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14731_\n\n",
    "closed_at": "2013-06-19T12:26:26Z",
    "created_at": "2013-06-12T13:45:41Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.11",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Combinatorial map from binary trees to permutations",
    "type": "issue",
    "updated_at": "2013-06-19T12:26:26Z",
    "url": "https://github.com/sagemath/sage/issues/14731",
    "user": "https://github.com/VivianePons"
}
```
adding combinatorial map from binary tree to 132 and 312 avoiding permutations

---

Apply: [attachment: trac_14731-binary-trees-to-permutations.patch.gz](https://github.com/sagemath/sage/files/ticket14731/trac_14731-binary-trees-to-permutations.patch.gz)

Depends on #14719

Assignee: **sage-combinat**

CC:  stumpc5 chrisjamesberg @tscrim

Keywords: **BinaryTrees, Permutations, FindStatDays01**

Reviewer: **Travis Scrimshaw**

Author: **Viviane Pons**

Merged: **sage-5.11.beta2**

_Issue created by migration from https://trac.sagemath.org/ticket/14731_





---

archive/issue_events_191988.json:
```json
{
    "actor": "https://github.com/VivianePons",
    "created_at": "2013-06-12T13:45:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14731",
    "milestone_number": null,
    "milestone_title": "sage-5.11",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14731#event-191988"
}
```



---

archive/issue_events_191989.json:
```json
{
    "actor": "https://github.com/VivianePons",
    "created_at": "2013-06-12T13:45:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14731",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14731#event-191989"
}
```



---

archive/issue_events_191990.json:
```json
{
    "actor": "https://github.com/VivianePons",
    "created_at": "2013-06-12T13:45:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14731",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14731#event-191990"
}
```



---

archive/issue_events_191991.json:
```json
{
    "actor": "https://github.com/sagetrac-chrisjamesberg",
    "created_at": "2013-06-12T14:07:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14731",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14731#event-191991"
}
```



---

archive/issue_events_191992.json:
```json
{
    "actor": "https://github.com/VivianePons",
    "created_at": "2013-06-12T14:57:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14731",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14731#event-191992"
}
```



---

archive/issue_comments_182117.json:
```json
{
    "body": "Dependencies: **#14719**",
    "created_at": "2013-06-12T15:00:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14731",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14731#issuecomment-182117",
    "user": "https://github.com/sagetrac-chrisjamesberg"
}
```

Dependencies: **#14719**



---

archive/issue_comments_182118.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,5 @@\n adding combinatorial map from binary tree to 132 and 312 avoiding permutations\n+\n+---\n+\n+Apply: [attachment: trac_14731-binary-trees-to-permutations.patch.gz](https://github.com/sagemath/sage/files/ticket14731/trac_14731-binary-trees-to-permutations.patch.gz)\n``````\n",
    "created_at": "2013-06-12T16:04:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14731",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14731#issuecomment-182118",
    "user": "https://github.com/tscrim"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,5 @@
 adding combinatorial map from binary tree to 132 and 312 avoiding permutations
+
+---
+
+Apply: [attachment: trac_14731-binary-trees-to-permutations.patch.gz](https://github.com/sagemath/sage/files/ticket14731/trac_14731-binary-trees-to-permutations.patch.gz)
``````




---

archive/issue_comments_182119.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nAttachment: **[trac_14731-binary-trees-to-permutations.patch.gz](https://github.com/sagemath/sage/files/ticket14731/trac_14731-binary-trees-to-permutations.patch.gz)**\n\nLooks good. Thanks.\n\nApply: trac_14731-binary-trees-to-permutations.patch",
    "created_at": "2013-06-12T16:04:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14731",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14731#issuecomment-182119",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:4'>Comment 4:</a>
Attachment: **[trac_14731-binary-trees-to-permutations.patch.gz](https://github.com/sagemath/sage/files/ticket14731/trac_14731-binary-trees-to-permutations.patch.gz)**

Looks good. Thanks.

Apply: trac_14731-binary-trees-to-permutations.patch



---

archive/issue_comments_182120.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2013-06-12T16:04:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14731",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14731#issuecomment-182120",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_191993.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-06-12T16:04:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14731",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14731#event-191993"
}
```



---

archive/issue_events_191994.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-06-12T16:04:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14731",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14731#event-191994"
}
```



---

archive/issue_events_191995.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-19T12:26:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14731",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14731#event-191995"
}
```



---

archive/issue_events_191996.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-06-19T12:26:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14731",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14731#event-191996"
}
```



---

archive/issue_comments_182121.json:
```json
{
    "body": "Merged: **sage-5.11.beta2**",
    "created_at": "2013-06-19T12:26:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14731",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14731#issuecomment-182121",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.11.beta2**
