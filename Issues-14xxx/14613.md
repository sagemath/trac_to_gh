# Issue 14613: Improved is_triangle_free using bitfields?

archive/issues_014409.json:
```json
{
    "assignees": [],
    "body": "Duplicate of #14614.\n\n\n\n\n\n\nAssignee: **@jasongrout, @nathanncohen, @rlmill**\n\nCC:  @sagetrac-Slani @sagetrac-Stefan @sagetrac-azi @dimpase\n\nReviewer: **Jernej Azarija**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14613_\n\n",
    "closed_at": "2013-05-19T10:22:56Z",
    "created_at": "2013-05-19T08:31:44Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Improved is_triangle_free using bitfields?",
    "type": "issue",
    "updated_at": "2013-05-19T10:22:56Z",
    "url": "https://github.com/sagemath/sage/issues/14613",
    "user": "https://github.com/sagetrac-azi"
}
```
Duplicate of #14614.






Assignee: **@jasongrout, @nathanncohen, @rlmill**

CC:  @sagetrac-Slani @sagetrac-Stefan @sagetrac-azi @dimpase

Reviewer: **Jernej Azarija**

_Issue created by migration from https://trac.sagemath.org/ticket/14613_





---

archive/issue_events_189949.json:
```json
{
    "actor": "https://github.com/sagetrac-azi",
    "created_at": "2013-05-19T08:31:44Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14613",
    "milestone_number": null,
    "milestone_title": "sage-5.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14613#event-189949"
}
```



---

archive/issue_events_189950.json:
```json
{
    "actor": "https://github.com/sagetrac-azi",
    "created_at": "2013-05-19T08:31:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14613",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14613#event-189950"
}
```



---

archive/issue_events_189951.json:
```json
{
    "actor": "https://github.com/sagetrac-azi",
    "created_at": "2013-05-19T08:31:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14613",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14613#event-189951"
}
```



---

archive/issue_events_189952.json:
```json
{
    "actor": "https://github.com/sagetrac-azi",
    "created_at": "2013-05-19T08:31:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14613",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14613#event-189952"
}
```



---

archive/issue_events_189953.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2013-05-19T09:51:33Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14613",
    "milestone_number": null,
    "milestone_title": "sage-5.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14613#event-189953"
}
```



---

archive/issue_events_189954.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2013-05-19T09:51:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14613",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14613#event-189954"
}
```



---

archive/issue_events_189955.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2013-05-19T09:51:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14613",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14613#event-189955"
}
```



---

archive/issue_events_189956.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2013-05-19T09:51:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14613",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14613#event-189956"
}
```



---

archive/issue_comments_179690.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,5 +1,6 @@\n-Test (and potentially) implement a faster is_triangle_free method using the data structure from #14589\n+Duplicate of #14614.\n \n \n \n \n+\n``````\n",
    "created_at": "2013-05-19T09:51:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14613",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14613#issuecomment-179690",
    "user": "https://github.com/nexttime"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,5 +1,6 @@
-Test (and potentially) implement a faster is_triangle_free method using the data structure from #14589
+Duplicate of #14614.
 
 
 
 
+
``````




---

archive/issue_events_189957.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2013-05-19T09:53:36Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/14613",
    "title_is": "Improved is_triangle_free using bitfields?",
    "title_was": "Improved is_triangle_free using bitfileds?",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14613#event-189957"
}
```



---

archive/issue_events_189958.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2013-05-19T09:53:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14613",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14613#event-189958"
}
```



---

archive/issue_events_189959.json:
```json
{
    "actor": "https://github.com/nexttime",
    "created_at": "2013-05-19T09:53:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14613",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14613#event-189959"
}
```



---

archive/issue_comments_179691.json:
```json
{
    "body": "Reviewer: **Jernej Azarija**",
    "created_at": "2013-05-19T09:53:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14613",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14613#issuecomment-179691",
    "user": "https://github.com/nexttime"
}
```

Reviewer: **Jernej Azarija**



---

archive/issue_events_189960.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-19T10:22:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14613",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14613#event-189960"
}
```



---

archive/issue_events_189961.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-19T10:22:56Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14613",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14613#event-189961"
}
```



---

archive/issue_events_189962.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-05-19T10:22:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14613",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14613#event-189962"
}
```
