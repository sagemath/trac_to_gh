# Issue 14063: Remove CombinatorialClass from Compositions

archive/issues_013859.json:
```json
{
    "assignees": [],
    "body": "Part of #12913.\n\nDepends on #14065\n\nAssignee: **sage-combinat**\n\nCC:  sage-combinat @nthiery\n\nKeywords: **deprecation**\n\nReviewer: **Vincent Delecroix**\n\nAuthor: **Travis Scrimshaw**\n\nMerged: **sage-5.8.beta2**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14063_\n\n",
    "closed_at": "2013-02-28T10:34:16Z",
    "created_at": "2013-02-05T23:19:13Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove CombinatorialClass from Compositions",
    "type": "issue",
    "updated_at": "2013-02-28T10:34:16Z",
    "url": "https://github.com/sagemath/sage/issues/14063",
    "user": "https://github.com/tscrim"
}
```
Part of #12913.

Depends on #14065

Assignee: **sage-combinat**

CC:  sage-combinat @nthiery

Keywords: **deprecation**

Reviewer: **Vincent Delecroix**

Author: **Travis Scrimshaw**

Merged: **sage-5.8.beta2**

_Issue created by migration from https://trac.sagemath.org/ticket/14063_





---

archive/issue_events_181330.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-05T23:19:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "milestone_number": null,
    "milestone_title": "sage-5.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-181330"
}
```



---

archive/issue_events_181331.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-05T23:19:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-181331"
}
```



---

archive/issue_events_181332.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-05T23:19:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-181332"
}
```



---

archive/issue_events_181333.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-05T23:19:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-181333"
}
```



---

archive/issue_comments_169575.json:
```json
{
    "body": "Dependencies: **#14065**",
    "created_at": "2013-02-06T04:09:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-169575",
    "user": "https://github.com/tscrim"
}
```

Dependencies: **#14065**



---

archive/issue_comments_169576.json:
```json
{
    "body": "Author: **Travis Scrimshaw**",
    "created_at": "2013-02-06T04:09:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-169576",
    "user": "https://github.com/tscrim"
}
```

Author: **Travis Scrimshaw**



---

archive/issue_events_181334.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-06T04:09:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-181334"
}
```



---

archive/issue_comments_169577.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nHi,\n\n* composition_from_subset must be renamed to from_subset\n* You may remove the few trailing whitespaces that remain.\n\nThe rest looks good.\n\nVincent",
    "created_at": "2013-02-15T17:50:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-169577",
    "user": "https://github.com/videlec"
}
```

<a id='comment:2'>Comment 2:</a>
Hi,

* composition_from_subset must be renamed to from_subset
* You may remove the few trailing whitespaces that remain.

The rest looks good.

Vincent



---

archive/issue_comments_169578.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nComments on the doc in composition.py\n* line 786: list(Compositions(4)) may be replaced with Compositions(4).list() (idem lines 877, 882, 927)\n* line 844: there is a strange sharp at the end of the line\n* line 859: a space is missing",
    "created_at": "2013-02-15T20:04:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-169578",
    "user": "https://github.com/videlec"
}
```

<a id='comment:3'>Comment 3:</a>
Comments on the doc in composition.py
* line 786: list(Compositions(4)) may be replaced with Compositions(4).list() (idem lines 877, 882, 927)
* line 844: there is a strange sharp at the end of the line
* line 859: a space is missing



---

archive/issue_comments_169579.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nMade fixes based on your comments.\n\nThanks,\n\nTravis",
    "created_at": "2013-02-15T21:17:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-169579",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:4'>Comment 4:</a>
Made fixes based on your comments.

Thanks,

Travis



---

archive/issue_events_181335.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2013-02-15T21:49:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-181335"
}
```



---

archive/issue_events_181336.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2013-02-15T21:49:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-181336"
}
```



---

archive/issue_comments_169580.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nGreat job !\n\nVincent",
    "created_at": "2013-02-15T21:49:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-169580",
    "user": "https://github.com/videlec"
}
```

<a id='comment:5'>Comment 5:</a>
Great job !

Vincent



---

archive/issue_comments_169581.json:
```json
{
    "body": "Reviewer: **Vincent Delecroix**",
    "created_at": "2013-02-16T11:30:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-169581",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Vincent Delecroix**



---

archive/issue_events_181337.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-16T11:30:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "milestone_number": null,
    "milestone_title": "sage-5.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-181337"
}
```



---

archive/issue_events_181338.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-16T11:30:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "milestone_number": null,
    "milestone_title": "sage-5.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-181338"
}
```



---

archive/issue_comments_169582.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nHow was this patch file created, as some headers are missing? You should use `hg export [tip]`.",
    "created_at": "2013-02-19T21:52:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-169582",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:7'>Comment 7:</a>
How was this patch file created, as some headers are missing? You should use `hg export [tip]`.



---

archive/issue_comments_169583.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nForgot to export, sorry about that. Fixed.",
    "created_at": "2013-02-19T22:16:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-169583",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:8'>Comment 8:</a>
Forgot to export, sorry about that. Fixed.



---

archive/issue_comments_169584.json:
```json
{
    "body": "Merged: **sage-5.8.beta1**",
    "created_at": "2013-02-22T19:12:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-169584",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.8.beta1**



---

archive/issue_events_181339.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-22T19:12:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-181339"
}
```



---

archive/issue_events_181340.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-22T19:12:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-181340"
}
```



---

archive/issue_comments_169585.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nThe LaTeX documentation doesn't build because you use backslashes inside the\n\n```\ndef cardinality(self):\n\"\"\"\n\"\"\"\n```\ndocstring instead of\n\n```\ndef cardinality(self):\nr\"\"\"\n\"\"\"\n```",
    "created_at": "2013-02-23T00:11:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-169585",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:10'>Comment 10:</a>
The LaTeX documentation doesn't build because you use backslashes inside the

```
def cardinality(self):
"""
"""
```
docstring instead of

```
def cardinality(self):
r"""
"""
```



---

archive/issue_comments_169586.json:
```json
{
    "body": "Changed merged from **sage-5.8.beta1** to none",
    "created_at": "2013-02-23T00:11:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-169586",
    "user": "https://github.com/jdemeyer"
}
```

Changed merged from **sage-5.8.beta1** to none



---

archive/issue_events_181341.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-23T00:58:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-181341"
}
```



---

archive/issue_comments_169587.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nMade the fixes (I put it in a few other places as well). I'll double/triple-check over the compiled docbuild once it finishes the rebuild.",
    "created_at": "2013-02-23T00:58:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-169587",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:11'>Comment 11:</a>
Made the fixes (I put it in a few other places as well). I'll double/triple-check over the compiled docbuild once it finishes the rebuild.



---

archive/issue_comments_169588.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nI also fixed a broken link. Jeroen, would it be okay if I set this back to positive review?\n\nSorry about this,\n\nTravis",
    "created_at": "2013-02-23T01:19:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-169588",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:12'>Comment 12:</a>
I also fixed a broken link. Jeroen, would it be okay if I set this back to positive review?

Sorry about this,

Travis



---

archive/issue_comments_169589.json:
```json
{
    "body": "Fixed docstrings",
    "created_at": "2013-02-23T02:59:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-169589",
    "user": "https://github.com/tscrim"
}
```

Fixed docstrings



---

archive/issue_comments_169590.json:
```json
{
    "body": "<a id='comment:13'>Comment 13:</a>\nAttachment: **[trac_14063-remove_cc_compositions-ts.patch.gz](https://github.com/sagemath/sage/files/ticket14063/trac_14063-remove_cc_compositions-ts.patch.gz)**\n\nI'm setting this back to positive review...I hope that's okay...",
    "created_at": "2013-02-23T03:00:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-169590",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:13'>Comment 13:</a>
Attachment: **[trac_14063-remove_cc_compositions-ts.patch.gz](https://github.com/sagemath/sage/files/ticket14063/trac_14063-remove_cc_compositions-ts.patch.gz)**

I'm setting this back to positive review...I hope that's okay...



---

archive/issue_events_181342.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-23T03:00:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-181342"
}
```



---

archive/issue_events_181343.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2013-02-23T03:00:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-181343"
}
```



---

archive/issue_events_181344.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-28T10:34:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-181344"
}
```



---

archive/issue_events_181345.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-28T10:34:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14063#event-181345"
}
```



---

archive/issue_comments_169591.json:
```json
{
    "body": "Merged: **sage-5.8.beta2**",
    "created_at": "2013-02-28T10:34:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14063",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14063#issuecomment-169591",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.8.beta2**
