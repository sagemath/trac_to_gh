# Issue 14377: Move spkg logs to logs/pkgs

archive/issues_014173.json:
```json
{
    "body": "Save logs of package builds in `logs/pkgs` instead of `spkg/logs`.\n\nDepends on #14328\n\nDepends on #14226\n\n**Assignee:** GeorgSWeber\n\n**CC:**  @ohanar @ohanar\n\n**Reviewer:** R. Andrew Ohana, Jeroen Demeyer\n\n**Author:** John Palmieri\n\n**Merged:** sage-5.9.beta4\n\nIssue created by migration from https://trac.sagemath.org/ticket/14377\n\n",
    "closed_at": "2013-04-04T17:40:32Z",
    "created_at": "2013-03-29T03:49:39Z",
    "labels": [
        "component: build",
        "blocker",
        "enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.9",
    "title": "Move spkg logs to logs/pkgs",
    "type": "issue",
    "url": "https://github.com/sagemath/sage/issues/14377",
    "user": "https://github.com/jdemeyer"
}
```
Save logs of package builds in `logs/pkgs` instead of `spkg/logs`.

Depends on #14328

Depends on #14226

**Assignee:** GeorgSWeber

**CC:**  @ohanar @ohanar

**Reviewer:** R. Andrew Ohana, Jeroen Demeyer

**Author:** John Palmieri

**Merged:** sage-5.9.beta4

Issue created by migration from https://trac.sagemath.org/ticket/14377





---

archive/issue_comments_175293.json:
```json
{
    "body": "**Dependencies:** #14328",
    "created_at": "2013-03-29T03:50:37Z",
    "issue": "https://github.com/sagemath/sage/issues/14377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14377#issuecomment-175293",
    "user": "https://github.com/jdemeyer"
}
```

**Dependencies:** #14328



---

archive/issue_comments_175294.json:
```json
{
    "body": "**Author:** John Palmieri",
    "created_at": "2013-03-29T17:19:51Z",
    "issue": "https://github.com/sagemath/sage/issues/14377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14377#issuecomment-175294",
    "user": "https://github.com/jhpalmieri"
}
```

**Author:** John Palmieri



---

archive/issue_comments_175295.json:
```json
{
    "body": "<a id='comment:2'></a>\nHere is an untested patch to the root repo, along with a change to the documentation.",
    "created_at": "2013-03-29T17:19:51Z",
    "issue": "https://github.com/sagemath/sage/issues/14377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14377#issuecomment-175295",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:2'></a>
Here is an untested patch to the root repo, along with a change to the documentation.



---

archive/issue_events_124795.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2013-03-29T17:19:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14377",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14377#event-124795"
}
```



---

archive/issue_comments_175296.json:
```json
{
    "body": "**Reviewer:** R. Andrew Ohana",
    "created_at": "2013-03-30T23:27:02Z",
    "issue": "https://github.com/sagemath/sage/issues/14377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14377#issuecomment-175296",
    "user": "https://github.com/ohanar"
}
```

**Reviewer:** R. Andrew Ohana



---

archive/issue_comments_175297.json:
```json
{
    "body": "<a id='comment:3'></a>\nother than maybe removing the line in `make distclean`, this looks good to me.",
    "created_at": "2013-03-30T23:27:02Z",
    "issue": "https://github.com/sagemath/sage/issues/14377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14377#issuecomment-175297",
    "user": "https://github.com/ohanar"
}
```

<a id='comment:3'></a>
other than maybe removing the line in `make distclean`, this looks good to me.



---

archive/issue_events_124796.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2013-03-30T23:27:02Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14377",
    "label": "needs review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14377#event-124796"
}
```



---

archive/issue_events_124797.json:
```json
{
    "actor": "https://github.com/ohanar",
    "created_at": "2013-03-30T23:27:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14377",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14377#event-124797"
}
```



---

archive/issue_comments_175298.json:
```json
{
    "body": "<a id='comment:4'></a>\nI think for the sake of upgrades, we should leave `make distclean` as is.",
    "created_at": "2013-03-31T14:50:50Z",
    "issue": "https://github.com/sagemath/sage/issues/14377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14377#issuecomment-175298",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:4'></a>
I think for the sake of upgrades, we should leave `make distclean` as is.



---

archive/issue_comments_175299.json:
```json
{
    "body": "<a id='comment:5'></a>\nIs this going to break anything?  There are probably at least a few \"help\" questions where spgk/logs is listed as the place to look.  I assume that no one wants it to be in both, of course.\n\nAlso, does `make test` still work, though, and put a log somewhere?  Is that still documented anywhere, or was the README the last place?  \n\nJust checking in both cases, I presume by default these are silly questions.",
    "created_at": "2013-04-01T13:07:49Z",
    "issue": "https://github.com/sagemath/sage/issues/14377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14377#issuecomment-175299",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:5'></a>
Is this going to break anything?  There are probably at least a few "help" questions where spgk/logs is listed as the place to look.  I assume that no one wants it to be in both, of course.

Also, does `make test` still work, though, and put a log somewhere?  Is that still documented anywhere, or was the README the last place?  

Just checking in both cases, I presume by default these are silly questions.



---

archive/issue_comments_175300.json:
```json
{
    "body": "<a id='comment:6'></a>\nReplying to [@kcrisman](#comment%3A5):\n> Is this going to break anything?  There are probably at least a few \"help\" questions where spgk/logs is listed as the place to look.  I assume that no one wants it to be in both, of course.\n\nIf anything, `logs/pkgs` should be easier to find than `spkg/logs`. And also, when building fails, the error message is now a lot more clear.\n\n> Also, does `make test` still work, though, and put a log somewhere?\n\nYes, but see #14328 for this (it is unrelated to this ticket, which deals with logs of building packages).\n\n> Is that still documented anywhere\n\nNot that I know.",
    "created_at": "2013-04-01T13:12:21Z",
    "issue": "https://github.com/sagemath/sage/issues/14377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14377#issuecomment-175300",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:6'></a>
Replying to [@kcrisman](#comment%3A5):
> Is this going to break anything?  There are probably at least a few "help" questions where spgk/logs is listed as the place to look.  I assume that no one wants it to be in both, of course.

If anything, `logs/pkgs` should be easier to find than `spkg/logs`. And also, when building fails, the error message is now a lot more clear.

> Also, does `make test` still work, though, and put a log somewhere?

Yes, but see #14328 for this (it is unrelated to this ticket, which deals with logs of building packages).

> Is that still documented anywhere

Not that I know.



---

archive/attachments_020063.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_14377-root.patch",
    "asset_url": "tarball://root/attachments/ticket14377/trac_14377-root.patch",
    "created_at": "2013-04-01T15:46:01Z",
    "issue": "https://github.com/sagemath/sage/issues/14377",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket14377/trac_14377-root.patch",
    "user": "https://github.com/jdemeyer"
}
```



---

archive/issue_comments_175301.json:
```json
{
    "body": "**Attachment:** [trac_14377-root.patch](https://github.com/sagemath/sage/files/ticket14377/trac_14377-root.patch)\n\nroot repo",
    "created_at": "2013-04-01T15:46:01Z",
    "issue": "https://github.com/sagemath/sage/issues/14377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14377#issuecomment-175301",
    "user": "https://github.com/jdemeyer"
}
```

**Attachment:** [trac_14377-root.patch](https://github.com/sagemath/sage/files/ticket14377/trac_14377-root.patch)

root repo



---

archive/issue_comments_175302.json:
```json
{
    "body": "**Changing reviewer** from \"R. Andrew Ohana\" to \"R. Andrew Ohana, Jeroen Demeyer\".",
    "created_at": "2013-04-01T15:48:18Z",
    "issue": "https://github.com/sagemath/sage/issues/14377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14377#issuecomment-175302",
    "user": "https://github.com/jdemeyer"
}
```

**Changing reviewer** from "R. Andrew Ohana" to "R. Andrew Ohana, Jeroen Demeyer".



---

archive/issue_comments_175303.json:
```json
{
    "body": "<a id='comment:7'></a>\nRebased to #14226 and added:\n\n```diff\ndiff --git a/spkg/bin/sage-spkg b/spkg/bin/sage-spkg\n\nIndex: spkg/bin/sage-spkg\n===================================================================\n--- a/spkg/bin/sage-spkg\n+++ b/spkg/bin/sage-spkg\n@@ -64,13 +64,13 @@\n ************************************************************************\n Please email sage-devel (http://groups.google.com/group/sage-devel)\n explaining the problem and including the relevant part of the log file\n-  $SAGE_ROOT/spkg/logs/$PKG_NAME.log\n+  $SAGE_LOGS/$PKG_NAME.log\n Describe your computer, operating system, etc.\n If you want to try to fix the problem yourself, *don't* just cd to\n `pwd` and type '$2' or whatever is appropriate.\n Instead, the following commands setup all environment variables\n correctly and load a subshell for you to debug the error:\n-  (cd '`pwd`' && '$SAGE_ROOT/sage' -sh)\n+  (cd '`pwd`' && '$SAGE_ROOT/sage' --sh)\n When you are done debugging, you can type \"exit\" to leave the subshell.\n ************************************************************************\n MESSAGE\n```",
    "created_at": "2013-04-01T15:48:18Z",
    "issue": "https://github.com/sagemath/sage/issues/14377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14377#issuecomment-175303",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:7'></a>
Rebased to #14226 and added:

```diff
diff --git a/spkg/bin/sage-spkg b/spkg/bin/sage-spkg

Index: spkg/bin/sage-spkg
===================================================================
--- a/spkg/bin/sage-spkg
+++ b/spkg/bin/sage-spkg
@@ -64,13 +64,13 @@
 ************************************************************************
 Please email sage-devel (http://groups.google.com/group/sage-devel)
 explaining the problem and including the relevant part of the log file
-  $SAGE_ROOT/spkg/logs/$PKG_NAME.log
+  $SAGE_LOGS/$PKG_NAME.log
 Describe your computer, operating system, etc.
 If you want to try to fix the problem yourself, *don't* just cd to
 `pwd` and type '$2' or whatever is appropriate.
 Instead, the following commands setup all environment variables
 correctly and load a subshell for you to debug the error:
-  (cd '`pwd`' && '$SAGE_ROOT/sage' -sh)
+  (cd '`pwd`' && '$SAGE_ROOT/sage' --sh)
 When you are done debugging, you can type "exit" to leave the subshell.
 ************************************************************************
 MESSAGE
```



---

archive/issue_comments_175304.json:
```json
{
    "body": "**Changing dependencies** from \"#14328\" to \"#14328, #14226\".",
    "created_at": "2013-04-01T15:48:18Z",
    "issue": "https://github.com/sagemath/sage/issues/14377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14377#issuecomment-175304",
    "user": "https://github.com/jdemeyer"
}
```

**Changing dependencies** from "#14328" to "#14328, #14226".



---

archive/issue_comments_175305.json:
```json
{
    "body": "<a id='comment:8'></a>\nOh, and now I've added this to the Sage library patch:\n\n```diff\ndiff --git a/sage/env.py b/sage/env.py\n--- a/sage/env.py\n+++ b/sage/env.py\n@@ -41,7 +41,7 @@\n         'SAGE_DATA'        : '$SAGE_SHARE',\n         'SAGE_EXTCODE'     : opj('$SAGE_SHARE', 'sage', 'ext'),\n         'SAGE_PACKAGES'    : opj('$SAGE_ROOT', 'spkg'),\n-        'SAGE_LOGS'        : opj('$SAGE_ROOT', 'spkg', 'logs'),\n+        'SAGE_LOGS'        : opj('$SAGE_ROOT', 'logs', 'pkgs'),\n         'SAGE_SPKG_INST'   : opj('$SAGE_ROOT', 'spkg', 'installed'),\n         'SAGE_DOC'         : opj('$SAGE_ROOT', 'devel', 'sage', 'doc'),\n         'DOT_SAGE'         : opj(os.environ.get('HOME','$SAGE_ROOT'), '.sage'),\n```",
    "created_at": "2013-04-01T16:29:54Z",
    "issue": "https://github.com/sagemath/sage/issues/14377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14377#issuecomment-175305",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:8'></a>
Oh, and now I've added this to the Sage library patch:

```diff
diff --git a/sage/env.py b/sage/env.py
--- a/sage/env.py
+++ b/sage/env.py
@@ -41,7 +41,7 @@
         'SAGE_DATA'        : '$SAGE_SHARE',
         'SAGE_EXTCODE'     : opj('$SAGE_SHARE', 'sage', 'ext'),
         'SAGE_PACKAGES'    : opj('$SAGE_ROOT', 'spkg'),
-        'SAGE_LOGS'        : opj('$SAGE_ROOT', 'spkg', 'logs'),
+        'SAGE_LOGS'        : opj('$SAGE_ROOT', 'logs', 'pkgs'),
         'SAGE_SPKG_INST'   : opj('$SAGE_ROOT', 'spkg', 'installed'),
         'SAGE_DOC'         : opj('$SAGE_ROOT', 'devel', 'sage', 'doc'),
         'DOT_SAGE'         : opj(os.environ.get('HOME','$SAGE_ROOT'), '.sage'),
```



---

archive/issue_comments_175306.json:
```json
{
    "body": "documentation change (Sage library)",
    "created_at": "2013-04-01T16:31:38Z",
    "issue": "https://github.com/sagemath/sage/issues/14377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14377#issuecomment-175306",
    "user": "https://github.com/jhpalmieri"
}
```

documentation change (Sage library)



---

archive/issue_events_124798.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-04-03T09:00:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14377",
    "label": "blocker",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14377#event-124798"
}
```



---

archive/attachments_020064.json:
```json
{
    "asset_content_type": "application/octet-stream",
    "asset_name": "trac_14377-sagelib.patch",
    "asset_url": "tarball://root/attachments/ticket14377/trac_14377-sagelib.patch",
    "created_at": "2013-04-03T09:00:42Z",
    "issue": "https://github.com/sagemath/sage/issues/14377",
    "type": "attachment",
    "url": "https://github.com/sagemath/sage/files/ticket14377/trac_14377-sagelib.patch",
    "user": "https://github.com/jdemeyer"
}
```



---

archive/issue_comments_175307.json:
```json
{
    "body": "<a id='comment:9'></a>\n**Attachment:** [trac_14377-sagelib.patch](https://github.com/sagemath/sage/files/ticket14377/trac_14377-sagelib.patch)",
    "created_at": "2013-04-03T09:00:42Z",
    "issue": "https://github.com/sagemath/sage/issues/14377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14377#issuecomment-175307",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:9'></a>
**Attachment:** [trac_14377-sagelib.patch](https://github.com/sagemath/sage/files/ticket14377/trac_14377-sagelib.patch)



---

archive/issue_events_124799.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-04-04T17:40:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14377",
    "label": "positive review",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14377#event-124799"
}
```



---

archive/issue_events_124800.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-04-04T17:40:32Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14377",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14377#event-124800"
}
```



---

archive/issue_comments_175308.json:
```json
{
    "body": "**Merged:** sage-5.9.beta4",
    "created_at": "2013-04-04T17:40:32Z",
    "issue": "https://github.com/sagemath/sage/issues/14377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14377#issuecomment-175308",
    "user": "https://github.com/jdemeyer"
}
```

**Merged:** sage-5.9.beta4



---

archive/issue_comments_175309.json:
```json
{
    "body": "<a id='comment:11'></a>\nWhen upgrading, it would probably make sense to move old spkg build logs to the new folder, and afterwards create a symlink from `spkg/logs` to `logs/pkg/`.",
    "created_at": "2013-04-07T11:24:42Z",
    "issue": "https://github.com/sagemath/sage/issues/14377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14377#issuecomment-175309",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:11'></a>
When upgrading, it would probably make sense to move old spkg build logs to the new folder, and afterwards create a symlink from `spkg/logs` to `logs/pkg/`.



---

archive/issue_comments_175310.json:
```json
{
    "body": "<a id='comment:12'></a>\nThe odd thing is that this disturbs filename completion `$SAGE_ROOT/l...`; now one has to type three characters to disambiguate `local` from `logs`.\n\n---\n\nAlso, why `pkgs`, rather than `spkgs`?  Or `log/[s]pkg[s]/`, which would be consistent (with e.g. `/var/log/`)?\n\n[EDIT: Ooops.]",
    "created_at": "2013-05-12T15:45:01Z",
    "issue": "https://github.com/sagemath/sage/issues/14377",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14377#issuecomment-175310",
    "user": "https://github.com/nexttime"
}
```

<a id='comment:12'></a>
The odd thing is that this disturbs filename completion `$SAGE_ROOT/l...`; now one has to type three characters to disambiguate `local` from `logs`.

---

Also, why `pkgs`, rather than `spkgs`?  Or `log/[s]pkg[s]/`, which would be consistent (with e.g. `/var/log/`)?

[EDIT: Ooops.]
