# Issue 14128: Very bad mistakes in linear_arboricity

archive/issues_013924.json:
```json
{
    "assignees": [],
    "body": "`linear_arboricity` has a parameter `k` which appears to define the number of colors, but does not. This patch renames is to `plus_one`. Plus the function did not compute the decomposition with a minimum number of colors. \nPlus Andrew Juell reported \"a counter example to a very old conjecture\" which turns out to be a Sage bug ! Too bad `:-)`\n\nNathann\n\nAssignee: **@jasongrout, @nathanncohen, @rlmill**\n\nCC:  @dcoudert\n\nReviewer: **David Coudert**\n\nAuthor: **Nathann Cohen**\n\nMerged: **sage-5.8.beta1**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14128_\n\n",
    "closed_at": "2013-02-22T19:13:28Z",
    "created_at": "2013-02-15T00:36:11Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Very bad mistakes in linear_arboricity",
    "type": "issue",
    "updated_at": "2013-02-22T19:13:28Z",
    "url": "https://github.com/sagemath/sage/issues/14128",
    "user": "https://github.com/nathanncohen"
}
```
`linear_arboricity` has a parameter `k` which appears to define the number of colors, but does not. This patch renames is to `plus_one`. Plus the function did not compute the decomposition with a minimum number of colors. 
Plus Andrew Juell reported "a counter example to a very old conjecture" which turns out to be a Sage bug ! Too bad `:-)`

Nathann

Assignee: **@jasongrout, @nathanncohen, @rlmill**

CC:  @dcoudert

Reviewer: **David Coudert**

Author: **Nathann Cohen**

Merged: **sage-5.8.beta1**

_Issue created by migration from https://trac.sagemath.org/ticket/14128_





---

archive/issue_events_182343.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-02-15T00:36:11Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14128",
    "milestone_number": null,
    "milestone_title": "sage-5.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14128#event-182343"
}
```



---

archive/issue_events_182344.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-02-15T00:36:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14128",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14128#event-182344"
}
```



---

archive/issue_events_182345.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-02-15T00:36:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14128",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14128#event-182345"
}
```



---

archive/issue_events_182346.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-02-15T00:36:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14128",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14128#event-182346"
}
```



---

archive/issue_events_182347.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-02-15T00:41:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14128",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14128#event-182347"
}
```



---

archive/issue_comments_170612.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThe patch is working but you could change the following text\n* \"`plus_one`` (integer) -- the number of colors to use.\"",
    "created_at": "2013-02-15T09:50:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14128#issuecomment-170612",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:2'>Comment 2:</a>
The patch is working but you could change the following text
* "`plus_one`` (integer) -- the number of colors to use."



---

archive/issue_events_182348.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2013-02-15T09:50:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14128",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14128#event-182348"
}
```



---

archive/issue_events_182349.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2013-02-15T09:50:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14128",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14128#event-182349"
}
```



---

archive/issue_comments_170613.json:
```json
{
    "body": "Reviewer: **David Coudert**",
    "created_at": "2013-02-15T09:50:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14128#issuecomment-170613",
    "user": "https://github.com/dcoudert"
}
```

Reviewer: **David Coudert**



---

archive/issue_events_182350.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-02-15T10:02:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14128",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14128#event-182350"
}
```



---

archive/issue_events_182351.json:
```json
{
    "actor": "https://github.com/nathanncohen",
    "created_at": "2013-02-15T10:02:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14128",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14128#event-182351"
}
```



---

archive/issue_comments_170614.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nWhat about that ? `:-)`\n\nNathann",
    "created_at": "2013-02-15T10:02:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14128#issuecomment-170614",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:3'>Comment 3:</a>
What about that ? `:-)`

Nathann



---

archive/issue_comments_170615.json:
```json
{
    "body": "Attachment: **[trac_14128.patch.gz](https://github.com/sagemath/sage/files/ticket14128/trac_14128.patch.gz)**",
    "created_at": "2013-02-15T10:02:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14128#issuecomment-170615",
    "user": "https://github.com/nathanncohen"
}
```

Attachment: **[trac_14128.patch.gz](https://github.com/sagemath/sage/files/ticket14128/trac_14128.patch.gz)**



---

archive/issue_events_182352.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2013-02-15T10:06:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14128",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14128#event-182352"
}
```



---

archive/issue_events_182353.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2013-02-15T10:06:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14128",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14128#event-182353"
}
```



---

archive/issue_comments_170616.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nMuch better !",
    "created_at": "2013-02-15T10:06:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14128#issuecomment-170616",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:4'>Comment 4:</a>
Much better !



---

archive/issue_comments_170617.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nThaaaaaaaaaaaaaaanks !!\n\nNathann",
    "created_at": "2013-02-15T10:06:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14128#issuecomment-170617",
    "user": "https://github.com/nathanncohen"
}
```

<a id='comment:5'>Comment 5:</a>
Thaaaaaaaaaaaaaaanks !!

Nathann



---

archive/issue_events_182354.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-22T19:13:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14128",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14128#event-182354"
}
```



---

archive/issue_events_182355.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-02-22T19:13:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14128",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14128#event-182355"
}
```



---

archive/issue_comments_170618.json:
```json
{
    "body": "Merged: **sage-5.8.beta1**",
    "created_at": "2013-02-22T19:13:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14128",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14128#issuecomment-170618",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.8.beta1**
