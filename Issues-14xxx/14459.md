# Issue 14459: add missing link for doc of q_bernoulli

archive/issues_014255.json:
```json
{
    "assignees": [],
    "body": "The q_bernoulli function does not appear in the doc, this corrects that.\n\nAssignee: **mvngu**\n\nCC:  @seblabbe\n\nKeywords: **documentation**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\nMerged: **sage-5.10.beta0**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/14459_\n\n",
    "closed_at": "2013-04-23T09:44:03Z",
    "created_at": "2013-04-16T14:33:04Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-5.10",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "add missing link for doc of q_bernoulli",
    "type": "issue",
    "updated_at": "2013-04-23T09:44:03Z",
    "url": "https://github.com/sagemath/sage/issues/14459",
    "user": "https://github.com/fchapoton"
}
```
The q_bernoulli function does not appear in the doc, this corrects that.

Assignee: **mvngu**

CC:  @seblabbe

Keywords: **documentation**

Reviewer: **Travis Scrimshaw**

Author: **Frédéric Chapoton**

Merged: **sage-5.10.beta0**

_Issue created by migration from https://trac.sagemath.org/ticket/14459_





---

archive/issue_events_187636.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-04-16T14:33:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/14459",
    "milestone_number": null,
    "milestone_title": "sage-5.10",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14459#event-187636"
}
```



---

archive/issue_events_187637.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-04-16T14:33:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14459",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14459#event-187637"
}
```



---

archive/issue_events_187638.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-04-16T14:33:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14459",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14459#event-187638"
}
```



---

archive/issue_events_187639.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-04-16T14:33:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14459",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14459#event-187639"
}
```



---

archive/issue_events_187640.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-04-16T19:01:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14459",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14459#event-187640"
}
```



---

archive/issue_comments_176685.json:
```json
{
    "body": "Author: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2013-04-16T19:01:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14459",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14459#issuecomment-176685",
    "user": "https://github.com/fchapoton"
}
```

Author: **Frédéric Chapoton**



---

archive/issue_comments_176686.json:
```json
{
    "body": "Changed keywords from **** to **documentation**",
    "created_at": "2013-04-16T19:01:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14459",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14459#issuecomment-176686",
    "user": "https://github.com/fchapoton"
}
```

Changed keywords from **** to **documentation**



---

archive/issue_comments_176687.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nAttachment: **[trac_14459_doc_of_qbernoulli-fc.patch.gz](https://github.com/sagemath/sage/files/ticket14459/trac_14459_doc_of_qbernoulli-fc.patch.gz)**",
    "created_at": "2013-04-16T19:01:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14459",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14459#issuecomment-176687",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:1'>Comment 1:</a>
Attachment: **[trac_14459_doc_of_qbernoulli-fc.patch.gz](https://github.com/sagemath/sage/files/ticket14459/trac_14459_doc_of_qbernoulli-fc.patch.gz)**



---

archive/issue_comments_176688.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nAttachment: **[trac_14459-doc_qbernoulli-review-ts.patch.gz](https://github.com/sagemath/sage/files/ticket14459/trac_14459-doc_qbernoulli-review-ts.patch.gz)**\n\nHey Frederic,\n\nThankfully the doc built without errors, however the `q_bernoulli.pyx` didn't have a module level doc (hence no title). I've uploaded a review patch which adds that. If it looks good to you, feel free to set this to positive review.\n\nThanks,\n\nTravis",
    "created_at": "2013-04-16T23:05:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14459",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14459#issuecomment-176688",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:2'>Comment 2:</a>
Attachment: **[trac_14459-doc_qbernoulli-review-ts.patch.gz](https://github.com/sagemath/sage/files/ticket14459/trac_14459-doc_qbernoulli-review-ts.patch.gz)**

Hey Frederic,

Thankfully the doc built without errors, however the `q_bernoulli.pyx` didn't have a module level doc (hence no title). I've uploaded a review patch which adds that. If it looks good to you, feel free to set this to positive review.

Thanks,

Travis



---

archive/issue_comments_176689.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2013-04-16T23:05:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14459",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14459#issuecomment-176689",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_176690.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThanks Travis !",
    "created_at": "2013-04-17T08:00:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14459",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14459#issuecomment-176690",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:3'>Comment 3:</a>
Thanks Travis !



---

archive/issue_events_187641.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-04-17T08:00:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14459",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14459#event-187641"
}
```



---

archive/issue_events_187642.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2013-04-17T08:00:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/14459",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14459#event-187642"
}
```



---

archive/issue_comments_176691.json:
```json
{
    "body": "Merged: **sage-5.10.beta0**",
    "created_at": "2013-04-23T09:44:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/14459",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/14459#issuecomment-176691",
    "user": "https://github.com/jdemeyer"
}
```

Merged: **sage-5.10.beta0**



---

archive/issue_events_187643.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-04-23T09:44:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/14459",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14459#event-187643"
}
```



---

archive/issue_events_187644.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-04-23T09:44:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/14459",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/14459#event-187644"
}
```
