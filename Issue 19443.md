# Issue 19443: Add mock, pytest, tox, virtualenv optional packages

archive/issues_019443.json:
```json
{
    "body": "These are all packages which are used for testing Python packages.\n\n**Upstream tarballs**:\n* https://pypi.python.org/packages/source/m/mock/mock-1.3.0.tar.gz\n* https://pypi.python.org/packages/source/n/nose/nose-1.3.7.tar.gz (existing package, just an update)\n* https://pypi.python.org/packages/source/p/pbr/pbr-1.8.1.tar.gz\n* https://pypi.python.org/packages/source/p/pluggy/pluggy-0.3.1.tar.gz\n* https://pypi.python.org/packages/source/p/py/py-1.4.31.tar.gz\n* https://pypi.python.org/packages/source/p/pytest/pytest-2.8.4.tar.gz\n* https://pypi.python.org/packages/source/t/tox/tox-2.2.1.tar.gz\n* https://pypi.python.org/packages/source/v/virtualenv/virtualenv-13.1.2.tar.gz\n\nIssue created by migration from https://trac.sagemath.org/ticket/19680\n\n",
    "closed_at": "2021-11-20T23:57:15Z",
    "created_at": "2015-12-08T13:25:39Z",
    "labels": [
        "component: packages: optional"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "Add mock, pytest, tox, virtualenv optional packages",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/19443",
    "user": "https://github.com/jdemeyer"
}
```
These are all packages which are used for testing Python packages.

**Upstream tarballs**:
* https://pypi.python.org/packages/source/m/mock/mock-1.3.0.tar.gz
* https://pypi.python.org/packages/source/n/nose/nose-1.3.7.tar.gz (existing package, just an update)
* https://pypi.python.org/packages/source/p/pbr/pbr-1.8.1.tar.gz
* https://pypi.python.org/packages/source/p/pluggy/pluggy-0.3.1.tar.gz
* https://pypi.python.org/packages/source/p/py/py-1.4.31.tar.gz
* https://pypi.python.org/packages/source/p/pytest/pytest-2.8.4.tar.gz
* https://pypi.python.org/packages/source/t/tox/tox-2.2.1.tar.gz
* https://pypi.python.org/packages/source/v/virtualenv/virtualenv-13.1.2.tar.gz

Issue created by migration from https://trac.sagemath.org/ticket/19680





---

archive/issue_comments_267032.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2015-12-08T14:56:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19443",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19443#issuecomment-267032",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_267033.json:
```json
{
    "body": "New commits:",
    "created_at": "2015-12-08T14:56:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19443",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19443#issuecomment-267033",
    "user": "https://github.com/jdemeyer"
}
```

New commits:



---

archive/issue_comments_267034.json:
```json
{
    "body": "Can't these all be installed via pip? Why not make them pip-type packages?",
    "created_at": "2015-12-27T16:45:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19443",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19443#issuecomment-267034",
    "user": "https://github.com/vbraun"
}
```

Can't these all be installed via pip? Why not make them pip-type packages?



---

archive/issue_comments_267035.json:
```json
{
    "body": "Replying to [comment:11 vbraun]:\n> Can't these all be installed via pip? Why not make them pip-type packages?\n\n\nWell, I would like to eventually change the build system to make such packages automatic dependencies if `SAGE_CHECK=yes`. Many Python packages have a test suite, but we don't run them because they require these extra packages.\n\nSince `pip` packages bypass the usual `local/var/lib/sage/installed` system, they should not be used as dependencies of other packages.",
    "created_at": "2015-12-28T09:47:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19443",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19443#issuecomment-267035",
    "user": "https://github.com/jdemeyer"
}
```

Replying to [comment:11 vbraun]:
> Can't these all be installed via pip? Why not make them pip-type packages?


Well, I would like to eventually change the build system to make such packages automatic dependencies if `SAGE_CHECK=yes`. Many Python packages have a test suite, but we don't run them because they require these extra packages.

Since `pip` packages bypass the usual `local/var/lib/sage/installed` system, they should not be used as dependencies of other packages.



---

archive/issue_comments_267036.json:
```json
{
    "body": "A pip-type package could just update `local/var/lib/sage/installed` with the output of `pip list`\n\nIMHO we are going to fight an uphill battle if we keep repackaging more and more of pypi for stuff that has no direct mathematical use. It just wastes our own resources that could be spent elsewhere. We should be relying more on pip, not less.",
    "created_at": "2015-12-28T11:35:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19443",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19443#issuecomment-267036",
    "user": "https://github.com/vbraun"
}
```

A pip-type package could just update `local/var/lib/sage/installed` with the output of `pip list`

IMHO we are going to fight an uphill battle if we keep repackaging more and more of pypi for stuff that has no direct mathematical use. It just wastes our own resources that could be spent elsewhere. We should be relying more on pip, not less.



---

archive/issue_events_054030.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-05-20T07:46:59Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/19443",
    "milestone": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/19443#event-54030"
}
```



---

archive/issue_comments_267037.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2016-05-20T07:46:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19443",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19443#issuecomment-267037",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_267038.json:
```json
{
    "body": "needs_work because of the old-style dependency lists. Let me know if anybody wants to review this, then I'll make the required changes.",
    "created_at": "2016-05-20T07:46:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19443",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19443#issuecomment-267038",
    "user": "https://github.com/jdemeyer"
}
```

needs_work because of the old-style dependency lists. Let me know if anybody wants to review this, then I'll make the required changes.



---

archive/issue_comments_267039.json:
```json
{
    "body": "I'll look, we need those for testing.",
    "created_at": "2016-05-20T11:38:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19443",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19443#issuecomment-267039",
    "user": "https://github.com/kiwifb"
}
```

I'll look, we need those for testing.



---

archive/issue_comments_267040.json:
```json
{
    "body": "I think its a bit too much to make all testsuite dependencies de facto standard packages because they are dependencies of standard packages. \n\nHow about requiring internet access for building with SAGE_CHECK=yes? Then all those pip packages could just be installed by a script-type `check_dependencies` package that just contains\n\n```\nif SAGE_CHECK\n    pip install -r check-requirements.txt\n```",
    "created_at": "2016-05-29T14:23:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19443",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19443#issuecomment-267040",
    "user": "https://github.com/vbraun"
}
```

I think its a bit too much to make all testsuite dependencies de facto standard packages because they are dependencies of standard packages. 

How about requiring internet access for building with SAGE_CHECK=yes? Then all those pip packages could just be installed by a script-type `check_dependencies` package that just contains

```
if SAGE_CHECK
    pip install -r check-requirements.txt
```



---

archive/issue_comments_267041.json:
```json
{
    "body": "Replying to [comment:16 vbraun]:\n> How about requiring internet access for building with SAGE_CHECK=yes? Then all those pip packages could just be installed by a script-type `check_dependencies` package that just contains\n> \n> ```\n> if SAGE_CHECK\n>     pip install -r check-requirements.txt\n> ```\n\nI have a version of this in #28998.",
    "created_at": "2020-01-13T00:50:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19443",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19443#issuecomment-267041",
    "user": "https://github.com/mkoeppe"
}
```

Replying to [comment:16 vbraun]:
> How about requiring internet access for building with SAGE_CHECK=yes? Then all those pip packages could just be installed by a script-type `check_dependencies` package that just contains
> 
> ```
> if SAGE_CHECK
>     pip install -r check-requirements.txt
> ```

I have a version of this in #28998.



---

archive/issue_comments_267042.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2020-06-06T05:39:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19443",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19443#issuecomment-267042",
    "user": "https://github.com/mkoeppe"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_events_054031.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-06T05:39:36Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/19443",
    "milestone": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/19443#event-54031"
}
```



---

archive/issue_events_054032.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-06-06T05:39:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/19443",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/19443#event-54032"
}
```



---

archive/issue_comments_267043.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2021-11-20T22:31:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19443",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19443#issuecomment-267043",
    "user": "https://github.com/orlitzky"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_267044.json:
```json
{
    "body": "Our python situation is unrecognizable from five years ago; this will need to be started from scratch if there's anything to be salvaged.",
    "created_at": "2021-11-20T22:31:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19443",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19443#issuecomment-267044",
    "user": "https://github.com/orlitzky"
}
```

Our python situation is unrecognizable from five years ago; this will need to be started from scratch if there's anything to be salvaged.



---

archive/issue_comments_267045.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2021-11-20T23:57:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/19443",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/19443#issuecomment-267045",
    "user": "https://github.com/mkoeppe"
}
```

Resolution: invalid



---

archive/issue_events_054033.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-11-20T23:57:15Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/19443",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/19443#event-54033"
}
```
