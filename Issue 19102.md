...
```
sage: dg = DiGraph([This is the Trac macro *5,6* that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#5,6-macro))
...
```
sage: Q=ClusterQuiver(DiGraph([This is the Trac macro *0,5* that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#0,5-macro)))
sage: Q.b_matrix()
[ 0  1]
[-1  0]
sage: Q.mutate(0)
...
```
sage: S=ClusterSeed(DiGraph([This is the Trac macro *'a','b'* that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#'a','b'-macro)))
sage: S.mutate('a')
}}}  
but the following throws an error
{{{
sage: S=ClusterSeed(DiGraph([This is the Trac macro *5,'b'* that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#5,'b'-macro)))
sage: S.mutate(5)
