...
+        # elsewhere)
+        while [This is the Trac macro *"$filename" == /* * that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#"$filename" == /* -macro); do
+            filename="${filename#/}"
+        done
+
         if [ $FIRST -eq 1 ]; then
...
+    # any trailing slash is removed; #26013
+    PREFIX="${SAGE_DESTDIR_LOCAL%/}"
