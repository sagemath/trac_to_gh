# Issue 32619: lazy_import: Muffle diagnostic messages for "at_startup"

archive/issues_032382.json:
```json
{
    "assignees": [],
    "body": "When loading a modularized part of Sage instead of `sage.all`, we should suppress the second of these two messages:\n\n```\n        if startup_guard and not self._at_startup:\n            print(f\"Resolving lazy import {self._name} during startup\")\n        elif self._at_startup and not startup_guard:\n            print(f\"Option ``at_startup=True`` for lazy import {self._name} not needed anymore\")\n```\n\nAlso, instead of `print`, we should use `warnings.warn`.\n\nPrevious discussion:\n- #30748\n\n\nCC:  @tscrim @jhpalmieri @nthiery\n\nBranch/Commit: **[4a8f409](https://github.com/sagemath/sagetrac-mirror/commit/4a8f4096e3bdd9b0845a2fd17a19983b00caca3c)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32619_\n\n",
    "closed_at": "2021-10-13T22:50:03Z",
    "created_at": "2021-10-04T02:05:32Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/refactoring",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "lazy_import: Muffle diagnostic messages for \"at_startup\"",
    "type": "issue",
    "updated_at": "2021-10-13T22:50:03Z",
    "url": "https://github.com/sagemath/sage/issues/32619",
    "user": "https://github.com/mkoeppe"
}
```
When loading a modularized part of Sage instead of `sage.all`, we should suppress the second of these two messages:

```
        if startup_guard and not self._at_startup:
            print(f"Resolving lazy import {self._name} during startup")
        elif self._at_startup and not startup_guard:
            print(f"Option ``at_startup=True`` for lazy import {self._name} not needed anymore")
```

Also, instead of `print`, we should use `warnings.warn`.

Previous discussion:
- #30748


CC:  @tscrim @jhpalmieri @nthiery

Branch/Commit: **[4a8f409](https://github.com/sagemath/sagetrac-mirror/commit/4a8f4096e3bdd9b0845a2fd17a19983b00caca3c)**

Reviewer: **Travis Scrimshaw**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/32619_





---

archive/issue_events_431134.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T02:05:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32619#event-431134"
}
```



---

archive/issue_events_431135.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T02:05:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "label": "https://github.com/sagemath/sage/labels/refactoring",
    "label_color": "696969",
    "label_name": "refactoring",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32619#event-431135"
}
```



---

archive/issue_events_431136.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T02:05:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32619#event-431136"
}
```



---

archive/issue_events_431137.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T02:05:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32619#event-431137"
}
```



---

archive/issue_comments_529115.json:
```json
{
    "body": "Branch: **[u/mkoeppe/lazy_import__muffle_diagnostic_messages_for__at_startup_](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/lazy_import__muffle_diagnostic_messages_for__at_startup_)**",
    "created_at": "2021-10-04T02:37:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32619#issuecomment-529115",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/lazy_import__muffle_diagnostic_messages_for__at_startup_](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/lazy_import__muffle_diagnostic_messages_for__at_startup_)**



---

archive/issue_comments_529116.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7e7f5fe8ddc7c10308584579701b556f8a13ddac\">7e7f5fe</a></td><td><code>sage.doctest.forker: Use sage.misc.lazy_import.ensure_startup_finished</code></td></tr></table>\n",
    "created_at": "2021-10-04T02:37:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32619#issuecomment-529116",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7e7f5fe8ddc7c10308584579701b556f8a13ddac">7e7f5fe</a></td><td><code>sage.doctest.forker: Use sage.misc.lazy_import.ensure_startup_finished</code></td></tr></table>




---

archive/issue_comments_529117.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2021-10-04T02:37:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32619#issuecomment-529117",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_comments_529118.json:
```json
{
    "body": "Commit: **[7e7f5fe](https://github.com/sagemath/sagetrac-mirror/commit/7e7f5fe8ddc7c10308584579701b556f8a13ddac)**",
    "created_at": "2021-10-04T02:37:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32619#issuecomment-529118",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[7e7f5fe](https://github.com/sagemath/sagetrac-mirror/commit/7e7f5fe8ddc7c10308584579701b556f8a13ddac)**



---

archive/issue_comments_529119.json:
```json
{
    "body": "Changed commit from **[7e7f5fe](https://github.com/sagemath/sagetrac-mirror/commit/7e7f5fe8ddc7c10308584579701b556f8a13ddac)** to **[181e4a0](https://github.com/sagemath/sagetrac-mirror/commit/181e4a00604d7ec5d4653acc9783738779dc8105)**",
    "created_at": "2021-10-04T02:39:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32619#issuecomment-529119",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[7e7f5fe](https://github.com/sagemath/sagetrac-mirror/commit/7e7f5fe8ddc7c10308584579701b556f8a13ddac)** to **[181e4a0](https://github.com/sagemath/sagetrac-mirror/commit/181e4a00604d7ec5d4653acc9783738779dc8105)**



---

archive/issue_comments_529120.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/181e4a00604d7ec5d4653acc9783738779dc8105\">181e4a0</a></td><td><code>src/sage/misc/lazy_import.pyx: Use warnings.warn instead of print</code></td></tr></table>\n",
    "created_at": "2021-10-04T02:39:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32619#issuecomment-529120",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:3'>Comment 3:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/181e4a00604d7ec5d4653acc9783738779dc8105">181e4a0</a></td><td><code>src/sage/misc/lazy_import.pyx: Use warnings.warn instead of print</code></td></tr></table>




---

archive/issue_events_431138.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T02:39:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32619#event-431138"
}
```



---

archive/issue_comments_529121.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nGreen bot => positive review.",
    "created_at": "2021-10-04T02:54:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32619#issuecomment-529121",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:5'>Comment 5:</a>
Green bot => positive review.



---

archive/issue_comments_529122.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2021-10-04T02:54:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32619#issuecomment-529122",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_431139.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T07:02:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32619#event-431139"
}
```



---

archive/issue_events_431140.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T07:02:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32619#event-431140"
}
```



---

archive/issue_comments_529123.json:
```json
{
    "body": "Changed commit from **[181e4a0](https://github.com/sagemath/sagetrac-mirror/commit/181e4a00604d7ec5d4653acc9783738779dc8105)** to **[bc2b707](https://github.com/sagemath/sagetrac-mirror/commit/bc2b707315325ec40dc4e257ce8fa538cbd75b6c)**",
    "created_at": "2021-10-05T17:28:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32619#issuecomment-529123",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[181e4a0](https://github.com/sagemath/sagetrac-mirror/commit/181e4a00604d7ec5d4653acc9783738779dc8105)** to **[bc2b707](https://github.com/sagemath/sagetrac-mirror/commit/bc2b707315325ec40dc4e257ce8fa538cbd75b6c)**



---

archive/issue_comments_529124.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bc2b707315325ec40dc4e257ce8fa538cbd75b6c\">bc2b707</a></td><td><code>sage.misc.lazy_import.finish_startup: Fixup</code></td></tr></table>\n",
    "created_at": "2021-10-05T17:28:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32619#issuecomment-529124",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:7'>Comment 7:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bc2b707315325ec40dc4e257ce8fa538cbd75b6c">bc2b707</a></td><td><code>sage.misc.lazy_import.finish_startup: Fixup</code></td></tr></table>




---

archive/issue_events_431141.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-05T17:28:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32619#event-431141"
}
```



---

archive/issue_events_431142.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-05T17:28:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32619#event-431142"
}
```



---

archive/issue_comments_529125.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9830e4eb3a0d8dd387b89891c9d8af73cc668dd8\">9830e4e</a></td><td><code>src/sage/misc/lazy_import.pyx: Adjust doctest output to use  of warnings.warn</code></td></tr></table>\n",
    "created_at": "2021-10-05T23:39:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32619#issuecomment-529125",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:9'>Comment 9:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9830e4eb3a0d8dd387b89891c9d8af73cc668dd8">9830e4e</a></td><td><code>src/sage/misc/lazy_import.pyx: Adjust doctest output to use  of warnings.warn</code></td></tr></table>




---

archive/issue_comments_529126.json:
```json
{
    "body": "Changed commit from **[bc2b707](https://github.com/sagemath/sagetrac-mirror/commit/bc2b707315325ec40dc4e257ce8fa538cbd75b6c)** to **[9830e4e](https://github.com/sagemath/sagetrac-mirror/commit/9830e4eb3a0d8dd387b89891c9d8af73cc668dd8)**",
    "created_at": "2021-10-05T23:39:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32619#issuecomment-529126",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[bc2b707](https://github.com/sagemath/sagetrac-mirror/commit/bc2b707315325ec40dc4e257ce8fa538cbd75b6c)** to **[9830e4e](https://github.com/sagemath/sagetrac-mirror/commit/9830e4eb3a0d8dd387b89891c9d8af73cc668dd8)**



---

archive/issue_comments_529127.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/dadfa07f6bf9356db6c971b51ee320fbb04481f2\">dadfa07</a></td><td><code>src/sage/misc/lazy_import.pyx: Fix markup of doctest output</code></td></tr></table>\n",
    "created_at": "2021-10-06T02:02:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32619#issuecomment-529127",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:10'>Comment 10:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/dadfa07f6bf9356db6c971b51ee320fbb04481f2">dadfa07</a></td><td><code>src/sage/misc/lazy_import.pyx: Fix markup of doctest output</code></td></tr></table>




---

archive/issue_comments_529128.json:
```json
{
    "body": "Changed commit from **[9830e4e](https://github.com/sagemath/sagetrac-mirror/commit/9830e4eb3a0d8dd387b89891c9d8af73cc668dd8)** to **[dadfa07](https://github.com/sagemath/sagetrac-mirror/commit/dadfa07f6bf9356db6c971b51ee320fbb04481f2)**",
    "created_at": "2021-10-06T02:02:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32619#issuecomment-529128",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[9830e4e](https://github.com/sagemath/sagetrac-mirror/commit/9830e4eb3a0d8dd387b89891c9d8af73cc668dd8)** to **[dadfa07](https://github.com/sagemath/sagetrac-mirror/commit/dadfa07f6bf9356db6c971b51ee320fbb04481f2)**



---

archive/issue_comments_529129.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nThere still is one doctest failure in `misc/lazy_import.pyx`.",
    "created_at": "2021-10-06T05:25:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32619#issuecomment-529129",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:11'>Comment 11:</a>
There still is one doctest failure in `misc/lazy_import.pyx`.



---

archive/issue_comments_529130.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/98d2e9b0ebb6d56b18518ec2624592532fac034a\">98d2e9b</a></td><td><code>src/sage/misc/lazy_import.pyx: In test_fake_startup, also reset finish_startup_called</code></td></tr></table>\n",
    "created_at": "2021-10-06T05:43:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32619#issuecomment-529130",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:12'>Comment 12:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/98d2e9b0ebb6d56b18518ec2624592532fac034a">98d2e9b</a></td><td><code>src/sage/misc/lazy_import.pyx: In test_fake_startup, also reset finish_startup_called</code></td></tr></table>




---

archive/issue_comments_529131.json:
```json
{
    "body": "Changed commit from **[dadfa07](https://github.com/sagemath/sagetrac-mirror/commit/dadfa07f6bf9356db6c971b51ee320fbb04481f2)** to **[98d2e9b](https://github.com/sagemath/sagetrac-mirror/commit/98d2e9b0ebb6d56b18518ec2624592532fac034a)**",
    "created_at": "2021-10-06T05:43:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32619#issuecomment-529131",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[dadfa07](https://github.com/sagemath/sagetrac-mirror/commit/dadfa07f6bf9356db6c971b51ee320fbb04481f2)** to **[98d2e9b](https://github.com/sagemath/sagetrac-mirror/commit/98d2e9b0ebb6d56b18518ec2624592532fac034a)**



---

archive/issue_events_431143.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-06T16:36:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32619#event-431143"
}
```



---

archive/issue_events_431144.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-06T16:36:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32619#event-431144"
}
```



---

archive/issue_comments_529132.json:
```json
{
    "body": "<a id='comment:14'>Comment 14:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4a8f4096e3bdd9b0845a2fd17a19983b00caca3c\">4a8f409</a></td><td><code>src/sage/misc/lazy_import.pyx: Warnings suppress duplicates, so use a different import for the repeated test</code></td></tr></table>\n",
    "created_at": "2021-10-10T05:15:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32619#issuecomment-529132",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:14'>Comment 14:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4a8f4096e3bdd9b0845a2fd17a19983b00caca3c">4a8f409</a></td><td><code>src/sage/misc/lazy_import.pyx: Warnings suppress duplicates, so use a different import for the repeated test</code></td></tr></table>




---

archive/issue_comments_529133.json:
```json
{
    "body": "Changed commit from **[98d2e9b](https://github.com/sagemath/sagetrac-mirror/commit/98d2e9b0ebb6d56b18518ec2624592532fac034a)** to **[4a8f409](https://github.com/sagemath/sagetrac-mirror/commit/4a8f4096e3bdd9b0845a2fd17a19983b00caca3c)**",
    "created_at": "2021-10-10T05:15:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32619#issuecomment-529133",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[98d2e9b](https://github.com/sagemath/sagetrac-mirror/commit/98d2e9b0ebb6d56b18518ec2624592532fac034a)** to **[4a8f409](https://github.com/sagemath/sagetrac-mirror/commit/4a8f4096e3bdd9b0845a2fd17a19983b00caca3c)**



---

archive/issue_events_431145.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-10T05:15:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32619#event-431145"
}
```



---

archive/issue_events_431146.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-10T05:15:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32619#event-431146"
}
```



---

archive/issue_events_431147.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-10-10T12:55:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32619#event-431147"
}
```



---

archive/issue_events_431148.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-10-10T12:55:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32619#event-431148"
}
```



---

archive/issue_comments_529134.json:
```json
{
    "body": "<a id='comment:16'>Comment 16:</a>\nLGTM.",
    "created_at": "2021-10-10T12:55:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32619#issuecomment-529134",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:16'>Comment 16:</a>
LGTM.



---

archive/issue_comments_529135.json:
```json
{
    "body": "<a id='comment:17'>Comment 17:</a>\nThanks!",
    "created_at": "2021-10-10T16:53:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32619#issuecomment-529135",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:17'>Comment 17:</a>
Thanks!



---

archive/issue_events_431149.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-10-13T22:50:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32619#event-431149"
}
```



---

archive/issue_events_431150.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "e28338b1ae9dcce6aeaac6fc338de07f974061b8",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-10-13T22:50:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32619#event-431150"
}
```



---

archive/issue_comments_529136.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/lazy_import__muffle_diagnostic_messages_for__at_startup_](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/lazy_import__muffle_diagnostic_messages_for__at_startup_)** to **[4a8f409](https://github.com/sagemath/sagetrac-mirror/commit/4a8f4096e3bdd9b0845a2fd17a19983b00caca3c)**",
    "created_at": "2021-10-13T22:50:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32619",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32619#issuecomment-529136",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/lazy_import__muffle_diagnostic_messages_for__at_startup_](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/lazy_import__muffle_diagnostic_messages_for__at_startup_)** to **[4a8f409](https://github.com/sagemath/sagetrac-mirror/commit/4a8f4096e3bdd9b0845a2fd17a19983b00caca3c)**
