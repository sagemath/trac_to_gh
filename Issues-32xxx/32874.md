...
```json
{
    "body": "<a id='comment:38'></a>These files from comment:35 are definitely in the wheel.",
...
<a id='comment:38'></a>These files from comment:35 are definitely in the wheel.
...
<a id='comment:46'></a>Replying to [comment:27 Matthias Köppe]:
> It would be worth checking if we can use an on-disk cache to speed up dependency tracking and package/module discovery.
...
<a id='comment:64'></a>Replying to [comment:63 Kwankyu Lee]:
> `./configure --enable-wheels` doesn't seem to work for me, even after `./bootstrap` (is this necessary?)
...
<a id='comment:74'></a>Replying to [comment:71 Kwankyu Lee]:
> Questions on changes in `src/MANIFEST.in`:
> 
> 1. Is this line `include MANIFEST.in` necessary? Perhaps because `src/MANIFEST.in` is a symlink?
...
<a id='comment:76'></a>Replying to [comment:71 Kwankyu Lee]:
> 2. How was this list made ?
