...
```json
{
    "body": "I'd have tried updating zeromq to the latest version, 4.3.4.",
...
```json
{
    "body": "I think the attempted configuration with `setup.cfg` does not work at all. This was masked so far by SAGE_VENV and SAGE_LOCAL being the same, which has changed.\n\nThis is another case where the locations discovered by the configure tests in `spkg-configure.m4` are not passed on",
...
```json
{
    "body": "Sorry, but the error happens in the minimal build, spkg-configure.m4 is not involved, right?",
...
```json
{
    "body": "You are right, it isn't; I should have written \"this is *also* another case...\"",
...
```json
{
    "body": "Is there a temptative solution to test?\n\nReplying to [comment:6 mkoeppe]:\n> I think the attempted configuration with `setup.cfg` does not work at all. This was masked so far by SAGE_VENV and SAGE_LOCAL being the same, which has changed.\n> \n> This is another case where the locations discovered by the configure tests in `spkg-configure.m4` are not passed on",
...
```json
{
    "body": "It looks like `pyzmq` is incompatible with our use of `SETUPTOOLS_USE_DISTUTILS=local`.",
...
```json
{
    "body": "In my case it worked with one more change; in `spkg-install.in` for `pyzmq`, replace `$SAGE_LOCAL` by `$SAGE_VENV` (I use python3.10).",
...
Replying to [comment:24 enriqueartal]:
> In my case it worked with one more change; in `spkg-install.in` for `pyzmq`, replace `$SAGE_LOCAL` by `$SAGE_VENV` (I use python3.10).
...
```json
{
    "body": "I made a clean install (including cffi) and modifying cysignals, and it works the compilation, I am testing now. Sorry for the noise.",
...
```json
{
    "body": "Thanks for testing. So for this ticket - installation was successful. The test failures are surely unrelated to setuptools and pyzmq.\n\nLet's get this fix in then.",
...
```json
{
    "body": "https://github.com/pypa/distutils/pull/65 has been merged",
...
```json
{
    "body": "... and is included in https://setuptools.pypa.io/en/latest/history.html#v59-0-0 (not yet on PyPI)",
...
```json
{
    "body": "setuptools 59.0.1 is out, but it looks like we should wait for 59.1.0 so that we can avoid the `setup_requires` deprecation done in 58.3.0 and reverted in 59.1.0",
...
```json
{
    "body": "Works for me on void linux using musl libc which used to fail (I was carrying a different patch before; I've replaced that patch by the changes in this ticket and it works).",
...
```json
{
    "body": "Follow-ups in #32944, #33059",
