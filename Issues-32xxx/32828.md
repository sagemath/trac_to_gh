...
<a id='comment:25'></a>Replying to [comment:24 enriqueartal]:
> In my case it worked with one more change; in `spkg-install.in` for `pyzmq`, replace `$SAGE_LOCAL` by `$SAGE_VENV` (I use python3.10).
...
<a id='comment:26'></a>Do you mean `zeromq` or `pyzmq`. FOR `zeromq` it used the system package. If it helps as I said in comment:11 jupyter is located also in `$SAGE_VENV/share`.
...
<a id='comment:27'></a>Replying to [comment:26 enriqueartal]:
> Do you mean `zeromq` or `pyzmq`. FOR `zeromq` it used the system package.
