# Issue 32869: fix improper names of several special functions for CBF

archive/issues_032632.json:
```json
{
    "assignees": [],
    "body": "CBF, i.e. sage/rings/complex_arb.pyx, names functions such as `Ei` impoperly, as `ei`. This in particular makes it impossible to do e.g. `CBF(Ei(I))` (one gets infinite recursion error).\n\nThe fix is just to rename them. One can first set aliases `Ei = ei`, etc.,\nand deprecate `ei`, and then, after the deprecation period, do the renaming.\n\n\n\nBranch/Commit: **[d9ee935](https://github.com/sagemath/sagetrac-mirror/commit/d9ee935f95001716b8493be7689b4e3b52b1afc5)**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Marc Mezzarobba**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32869_\n\n",
    "closed_at": "2021-11-15T23:16:11Z",
    "created_at": "2021-11-13T23:42:03Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix improper names of several special functions for CBF",
    "type": "issue",
    "updated_at": "2021-11-15T23:16:11Z",
    "url": "https://github.com/sagemath/sage/issues/32869",
    "user": "https://github.com/dimpase"
}
```
CBF, i.e. sage/rings/complex_arb.pyx, names functions such as `Ei` impoperly, as `ei`. This in particular makes it impossible to do e.g. `CBF(Ei(I))` (one gets infinite recursion error).

The fix is just to rename them. One can first set aliases `Ei = ei`, etc.,
and deprecate `ei`, and then, after the deprecation period, do the renaming.



Branch/Commit: **[d9ee935](https://github.com/sagemath/sagetrac-mirror/commit/d9ee935f95001716b8493be7689b4e3b52b1afc5)**

Reviewer: **Dima Pasechnik**

Author: **Marc Mezzarobba**

_Issue created by migration from https://trac.sagemath.org/ticket/32869_





---

archive/issue_events_434169.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-11-13T23:42:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32869",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32869#event-434169"
}
```



---

archive/issue_events_434170.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-11-13T23:42:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32869",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32869#event-434170"
}
```



---

archive/issue_events_434171.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-11-13T23:42:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32869",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32869#event-434171"
}
```



---

archive/issue_events_434172.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-11-13T23:42:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32869",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32869#event-434172"
}
```



---

archive/issue_comments_533624.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nThe names of the symbolic functions are also inconsistent: `Ei` is `Ei`, but `Si`, etc. are actually aliases for `sin_integral`, etc.",
    "created_at": "2021-11-14T10:25:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32869#issuecomment-533624",
    "user": "https://github.com/mezzarobba"
}
```

<a id='comment:1'>Comment 1:</a>
The names of the symbolic functions are also inconsistent: `Ei` is `Ei`, but `Si`, etc. are actually aliases for `sin_integral`, etc.



---

archive/issue_events_434173.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2021-11-14T10:55:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32869",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32869#event-434173"
}
```



---

archive/issue_comments_533625.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nsee #32871 for the infinite recursion\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d9ee935f95001716b8493be7689b4e3b52b1afc5\">d9ee935</a></td><td><code>#32869 ComplexBall: rename + add aliases for some special functions</code></td></tr></table>\n",
    "created_at": "2021-11-14T10:55:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32869#issuecomment-533625",
    "user": "https://github.com/mezzarobba"
}
```

<a id='comment:2'>Comment 2:</a>
see #32871 for the infinite recursion

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d9ee935f95001716b8493be7689b4e3b52b1afc5">d9ee935</a></td><td><code>#32869 ComplexBall: rename + add aliases for some special functions</code></td></tr></table>




---

archive/issue_comments_533626.json:
```json
{
    "body": "Commit: **[d9ee935](https://github.com/sagemath/sagetrac-mirror/commit/d9ee935f95001716b8493be7689b4e3b52b1afc5)**",
    "created_at": "2021-11-14T10:55:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32869#issuecomment-533626",
    "user": "https://github.com/mezzarobba"
}
```

Commit: **[d9ee935](https://github.com/sagemath/sagetrac-mirror/commit/d9ee935f95001716b8493be7689b4e3b52b1afc5)**



---

archive/issue_comments_533627.json:
```json
{
    "body": "Branch: **[u/mmezzarobba/CBF_renamespecfuns](https://github.com/sagemath/sagetrac-mirror/tree/u/mmezzarobba/CBF_renamespecfuns)**",
    "created_at": "2021-11-14T10:55:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32869#issuecomment-533627",
    "user": "https://github.com/mezzarobba"
}
```

Branch: **[u/mmezzarobba/CBF_renamespecfuns](https://github.com/sagemath/sagetrac-mirror/tree/u/mmezzarobba/CBF_renamespecfuns)**



---

archive/issue_comments_533628.json:
```json
{
    "body": "Author: **Marc Mezzarobba**",
    "created_at": "2021-11-14T10:55:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32869#issuecomment-533628",
    "user": "https://github.com/mezzarobba"
}
```

Author: **Marc Mezzarobba**



---

archive/issue_comments_533629.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2021-11-14T12:04:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32869#issuecomment-533629",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_comments_533630.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nlgtm",
    "created_at": "2021-11-14T12:04:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32869#issuecomment-533630",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:3'>Comment 3:</a>
lgtm



---

archive/issue_events_434174.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-11-14T12:04:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32869",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32869#event-434174"
}
```



---

archive/issue_events_434175.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-11-14T12:04:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32869",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32869#event-434175"
}
```



---

archive/issue_comments_533631.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nthank you!",
    "created_at": "2021-11-14T12:55:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32869#issuecomment-533631",
    "user": "https://github.com/mezzarobba"
}
```

<a id='comment:4'>Comment 4:</a>
thank you!



---

archive/issue_events_434176.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-11-15T23:16:11Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32869",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32869#event-434176"
}
```



---

archive/issue_events_434177.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "311fd3481e51a4988af14770f88626b6e9d0d373",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-11-15T23:16:11Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32869",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32869#event-434177"
}
```



---

archive/issue_comments_533632.json:
```json
{
    "body": "Changed branch from **[u/mmezzarobba/CBF_renamespecfuns](https://github.com/sagemath/sagetrac-mirror/tree/u/mmezzarobba/CBF_renamespecfuns)** to **[d9ee935](https://github.com/sagemath/sagetrac-mirror/commit/d9ee935f95001716b8493be7689b4e3b52b1afc5)**",
    "created_at": "2021-11-15T23:16:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32869",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32869#issuecomment-533632",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mmezzarobba/CBF_renamespecfuns](https://github.com/sagemath/sagetrac-mirror/tree/u/mmezzarobba/CBF_renamespecfuns)** to **[d9ee935](https://github.com/sagemath/sagetrac-mirror/commit/d9ee935f95001716b8493be7689b4e3b52b1afc5)**
