# Issue 32636: sage.matrix: Remove hard dependencies on sage.schemes

archive/issues_032399.json:
```json
{
    "assignees": [],
    "body": "We remove another case of the obstacle to modularization described in #32414: Importing a class from a far away Python module just for the purpose of `isinstance` testing.\n\nBecause this use of `SchemeHomset_generic`, `SchemeHomset_points` only happens in two files, we don't bother with introducing some abc and just use `try...except` around the import.\n\n(cherry-picked from #32432)\n\n\nCC:  @tscrim @bhutz @kliem\n\nBranch/Commit: **[e94e96f](https://github.com/sagemath/sagetrac-mirror/commit/e94e96f00087822e829ae46fc70c0ab01a88ec58)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32636_\n\n",
    "closed_at": "2021-10-19T20:31:58Z",
    "created_at": "2021-10-05T17:01:21Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/refactoring",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage.matrix: Remove hard dependencies on sage.schemes",
    "type": "issue",
    "updated_at": "2021-10-19T20:31:58Z",
    "url": "https://github.com/sagemath/sage/issues/32636",
    "user": "https://github.com/mkoeppe"
}
```
We remove another case of the obstacle to modularization described in #32414: Importing a class from a far away Python module just for the purpose of `isinstance` testing.

Because this use of `SchemeHomset_generic`, `SchemeHomset_points` only happens in two files, we don't bother with introducing some abc and just use `try...except` around the import.

(cherry-picked from #32432)


CC:  @tscrim @bhutz @kliem

Branch/Commit: **[e94e96f](https://github.com/sagemath/sagetrac-mirror/commit/e94e96f00087822e829ae46fc70c0ab01a88ec58)**

Reviewer: **Travis Scrimshaw**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/32636_





---

archive/issue_events_431326.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-05T17:01:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32636",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32636#event-431326"
}
```



---

archive/issue_events_431327.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-05T17:01:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32636",
    "label": "https://github.com/sagemath/sage/labels/refactoring",
    "label_color": "696969",
    "label_name": "refactoring",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32636#event-431327"
}
```



---

archive/issue_events_431328.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-05T17:01:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32636",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32636#event-431328"
}
```



---

archive/issue_events_431329.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-05T17:01:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32636",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32636#event-431329"
}
```



---

archive/issue_comments_529336.json:
```json
{
    "body": "Branch: **[u/mkoeppe/sage_matrix__remove_hard_dependencies_on_sage_schemes](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/sage_matrix__remove_hard_dependencies_on_sage_schemes)**",
    "created_at": "2021-10-05T17:01:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32636",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32636#issuecomment-529336",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/sage_matrix__remove_hard_dependencies_on_sage_schemes](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/sage_matrix__remove_hard_dependencies_on_sage_schemes)**



---

archive/issue_comments_529337.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e94e96f00087822e829ae46fc70c0ab01a88ec58\">e94e96f</a></td><td><code>src/sage/matrix/{matrix_space.py,action.pyx}: Do not fail if sage.schemes is not present</code></td></tr></table>\n",
    "created_at": "2021-10-05T17:02:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32636",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32636#issuecomment-529337",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e94e96f00087822e829ae46fc70c0ab01a88ec58">e94e96f</a></td><td><code>src/sage/matrix/{matrix_space.py,action.pyx}: Do not fail if sage.schemes is not present</code></td></tr></table>




---

archive/issue_comments_529338.json:
```json
{
    "body": "Commit: **[e94e96f](https://github.com/sagemath/sagetrac-mirror/commit/e94e96f00087822e829ae46fc70c0ab01a88ec58)**",
    "created_at": "2021-10-05T17:02:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32636",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32636#issuecomment-529338",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[e94e96f](https://github.com/sagemath/sagetrac-mirror/commit/e94e96f00087822e829ae46fc70c0ab01a88ec58)**



---

archive/issue_events_431330.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-05T17:02:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32636",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32636#event-431330"
}
```



---

archive/issue_comments_529339.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLGTM.",
    "created_at": "2021-10-06T02:30:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32636",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32636#issuecomment-529339",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:3" align="right">Comment 3</div>

LGTM.



---

archive/issue_events_431331.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-10-06T02:30:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32636",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32636#event-431331"
}
```



---

archive/issue_events_431332.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-10-06T02:30:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32636",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32636#event-431332"
}
```



---

archive/issue_comments_529340.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2021-10-06T02:30:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32636",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32636#issuecomment-529340",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_529341.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThank you!",
    "created_at": "2021-10-06T02:33:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32636",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32636#issuecomment-529341",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:4" align="right">Comment 4</div>

Thank you!



---

archive/issue_comments_529342.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/sage_matrix__remove_hard_dependencies_on_sage_schemes](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/sage_matrix__remove_hard_dependencies_on_sage_schemes)** to **[e94e96f](https://github.com/sagemath/sagetrac-mirror/commit/e94e96f00087822e829ae46fc70c0ab01a88ec58)**",
    "created_at": "2021-10-19T20:31:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32636",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32636#issuecomment-529342",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/sage_matrix__remove_hard_dependencies_on_sage_schemes](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/sage_matrix__remove_hard_dependencies_on_sage_schemes)** to **[e94e96f](https://github.com/sagemath/sagetrac-mirror/commit/e94e96f00087822e829ae46fc70c0ab01a88ec58)**



---

archive/issue_events_431333.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-10-19T20:31:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32636",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32636#event-431333"
}
```



---

archive/issue_events_431334.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "4c3a338d15d8a0da4ce77d20240336f2792623d2",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-10-19T20:31:58Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32636",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32636#event-431334"
}
```
