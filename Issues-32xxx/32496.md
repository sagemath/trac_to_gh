# Issue 32496: trivial __copy__ and __deepcopy__ methods for element classes in sage.rings.finite_rings

archive/issues_032259.json:
```json
{
    "assignees": [],
    "body": "(follow-up on #32454, part of Meta-ticket #13811)\n\n\nCC:  @tscrim @kwankyu @roed314 @mjungmath\n\nBranch/Commit: **[d6dd967](https://github.com/sagemath/sagetrac-mirror/commit/d6dd9678c627759e3c75a6223de45fefe3ec1d95)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32496_\n\n",
    "closed_at": "2021-09-19T09:58:12Z",
    "created_at": "2021-09-10T05:13:21Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20finite%20rings",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "trivial __copy__ and __deepcopy__ methods for element classes in sage.rings.finite_rings",
    "type": "issue",
    "updated_at": "2021-09-19T09:58:12Z",
    "url": "https://github.com/sagemath/sage/issues/32496",
    "user": "https://github.com/mkoeppe"
}
```
(follow-up on #32454, part of Meta-ticket #13811)


CC:  @tscrim @kwankyu @roed314 @mjungmath

Branch/Commit: **[d6dd967](https://github.com/sagemath/sagetrac-mirror/commit/d6dd9678c627759e3c75a6223de45fefe3ec1d95)**

Reviewer: **Travis Scrimshaw**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/32496_





---

archive/issue_events_429745.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-10T05:13:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32496",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32496#event-429745"
}
```



---

archive/issue_events_429746.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-10T05:13:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32496",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20finite%20rings",
    "label_color": "0000ff",
    "label_name": "component: finite rings",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32496#event-429746"
}
```



---

archive/issue_events_429747.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-10T05:13:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32496",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32496#event-429747"
}
```



---

archive/issue_events_429748.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-10T05:13:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32496",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32496#event-429748"
}
```



---

archive/issue_comments_527138.json:
```json
{
    "body": "Branch: **[u/mkoeppe/trivial___copy___and___deepcopy___methods_for_sage_rings_finite_rings](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/trivial___copy___and___deepcopy___methods_for_sage_rings_finite_rings)**",
    "created_at": "2021-09-10T05:35:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32496",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32496#issuecomment-527138",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/trivial___copy___and___deepcopy___methods_for_sage_rings_finite_rings](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/trivial___copy___and___deepcopy___methods_for_sage_rings_finite_rings)**



---

archive/issue_comments_527139.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2021-09-10T05:36:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32496",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32496#issuecomment-527139",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_comments_527140.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/87aadd36caf13576158cf07b35ddecbecbd77af9\">87aadd3</a></td><td><code>FiniteField_givaroElement: Add trivial `__deepcopy__` method</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/72550fdf01acb24e4a044b727f47644606d594c5\">72550fd</a></td><td><code>FiniteField_ntl_gf2eElement: Add trivial `__deepcopy__` method</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e69e116759283e73239acdf547269260e7209057\">e69e116</a></td><td><code>FiniteFieldElement_pari_ffelt.__copy__: Immutable, just return self; add trivial `__deepcopy__` method</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d6dd9678c627759e3c75a6223de45fefe3ec1d95\">d6dd967</a></td><td><code>IntegerMod_*.__copy__: Immutable, just return self; add trivial `__deepcopy__` methods</code></td></tr></table>\n",
    "created_at": "2021-09-10T05:36:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32496",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32496#issuecomment-527140",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/87aadd36caf13576158cf07b35ddecbecbd77af9">87aadd3</a></td><td><code>FiniteField_givaroElement: Add trivial `__deepcopy__` method</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/72550fdf01acb24e4a044b727f47644606d594c5">72550fd</a></td><td><code>FiniteField_ntl_gf2eElement: Add trivial `__deepcopy__` method</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e69e116759283e73239acdf547269260e7209057">e69e116</a></td><td><code>FiniteFieldElement_pari_ffelt.__copy__: Immutable, just return self; add trivial `__deepcopy__` method</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d6dd9678c627759e3c75a6223de45fefe3ec1d95">d6dd967</a></td><td><code>IntegerMod_*.__copy__: Immutable, just return self; add trivial `__deepcopy__` methods</code></td></tr></table>




---

archive/issue_comments_527141.json:
```json
{
    "body": "Commit: **[d6dd967](https://github.com/sagemath/sagetrac-mirror/commit/d6dd9678c627759e3c75a6223de45fefe3ec1d95)**",
    "created_at": "2021-09-10T05:36:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32496",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32496#issuecomment-527141",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[d6dd967](https://github.com/sagemath/sagetrac-mirror/commit/d6dd9678c627759e3c75a6223de45fefe3ec1d95)**



---

archive/issue_events_429749.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-10T05:36:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32496",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32496#event-429749"
}
```



---

archive/issue_events_429750.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-10T05:36:33Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/32496",
    "title_is": "trivial __copy__ and __deepcopy__ methods for element classes in sage.rings.finite_rings",
    "title_was": "trivial __copy__ and __deepcopy__ methods for sage.rings.finite_rings",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32496#event-429750"
}
```



---

archive/issue_events_429751.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-09-11T04:59:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32496",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32496#event-429751"
}
```



---

archive/issue_events_429752.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-09-11T04:59:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32496",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32496#event-429752"
}
```



---

archive/issue_comments_527142.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2021-09-11T04:59:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32496",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32496#issuecomment-527142",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_527143.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nLGTM.\n\nSide Question: Do you think we should just add a `__deepcopy__` in `Element` that just calls `__copy__`? Very few things (if any) would probably implement a distinct `__deepcopy__`.",
    "created_at": "2021-09-11T04:59:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32496",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32496#issuecomment-527143",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:4" align="right">Comment 4</div>

LGTM.

Side Question: Do you think we should just add a `__deepcopy__` in `Element` that just calls `__copy__`? Very few things (if any) would probably implement a distinct `__deepcopy__`.



---

archive/issue_comments_527144.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThank you!",
    "created_at": "2021-09-11T17:36:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32496",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32496#issuecomment-527144",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:5" align="right">Comment 5</div>

Thank you!



---

archive/issue_comments_527145.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nReplying to [@tscrim](#comment%3A4):\n> Do you think we should just add a `__deepcopy__` in `Element` that just calls `__copy__`? Very few things (if any) would probably implement a distinct `__deepcopy__`.\n\nWe could try that, but `Element.__copy__` is explicitly a shallow copy (attribute by attribute), so it feels a bit undisciplined.\n\nI would still favor making a set of mix-in classes that specify the mutability semantics, something like `Mutable`, `Immutable`, `DeeplyImmutable`.\nI think as long as mix-in classes do not define new attributes or new methods that would need to go in the vtable, there's no problem with mix-ins for extension classes, but I might be remembering this wrong.",
    "created_at": "2021-09-11T17:46:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32496",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32496#issuecomment-527145",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:6" align="right">Comment 6</div>

Replying to [@tscrim](#comment%3A4):
> Do you think we should just add a `__deepcopy__` in `Element` that just calls `__copy__`? Very few things (if any) would probably implement a distinct `__deepcopy__`.

We could try that, but `Element.__copy__` is explicitly a shallow copy (attribute by attribute), so it feels a bit undisciplined.

I would still favor making a set of mix-in classes that specify the mutability semantics, something like `Mutable`, `Immutable`, `DeeplyImmutable`.
I think as long as mix-in classes do not define new attributes or new methods that would need to go in the vtable, there's no problem with mix-ins for extension classes, but I might be remembering this wrong.



---

archive/issue_comments_527146.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nThe problem is there is no multiple inheritance in extension classes. `:/`",
    "created_at": "2021-09-17T02:35:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32496",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32496#issuecomment-527146",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:7" align="right">Comment 7</div>

The problem is there is no multiple inheritance in extension classes. `:/`



---

archive/issue_comments_527147.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/trivial___copy___and___deepcopy___methods_for_sage_rings_finite_rings](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/trivial___copy___and___deepcopy___methods_for_sage_rings_finite_rings)** to **[d6dd967](https://github.com/sagemath/sagetrac-mirror/commit/d6dd9678c627759e3c75a6223de45fefe3ec1d95)**",
    "created_at": "2021-09-19T09:58:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32496",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32496#issuecomment-527147",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/trivial___copy___and___deepcopy___methods_for_sage_rings_finite_rings](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/trivial___copy___and___deepcopy___methods_for_sage_rings_finite_rings)** to **[d6dd967](https://github.com/sagemath/sagetrac-mirror/commit/d6dd9678c627759e3c75a6223de45fefe3ec1d95)**



---

archive/issue_events_429753.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-09-19T09:58:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32496",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32496#event-429753"
}
```



---

archive/issue_events_429754.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "a58e28d1a7ab46efd198ea573078c361e9a92188",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-09-19T09:58:12Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32496",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32496#event-429754"
}
```
