# Issue 32239: Fix use of MixedIntegerLinearProgram in sage.graphs.graph - part 3

archive/issues_032002.json:
```json
{
    "assignees": [],
    "body": "Part of #32191 (Meta-ticket: Audit/fix all uses of `MixedIntegerLinearProgram` in `sage.combinat`, `sage.graphs`).\n\nWe update methods\n- `has_homomorphism_to `\n- `independent_set_of_representatives `\n- `minor`\n- `topological_minor `\n\nDepends on #32197\n\nCC:  @mkoeppe @tscrim @dimpase\n\nBranch/Commit: **[093f72e](https://github.com/sagemath/sagetrac-mirror/commit/093f72e0a5bc7618ee17d40d8a4c0794d448ae3f)**\n\nReviewer: **Matthias Koeppe**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32239_\n\n",
    "closed_at": "2021-07-25T13:26:14Z",
    "created_at": "2021-07-19T15:30:24Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix use of MixedIntegerLinearProgram in sage.graphs.graph - part 3",
    "type": "issue",
    "updated_at": "2021-07-25T13:26:14Z",
    "url": "https://github.com/sagemath/sage/issues/32239",
    "user": "https://github.com/dcoudert"
}
```
Part of #32191 (Meta-ticket: Audit/fix all uses of `MixedIntegerLinearProgram` in `sage.combinat`, `sage.graphs`).

We update methods
- `has_homomorphism_to `
- `independent_set_of_representatives `
- `minor`
- `topological_minor `

Depends on #32197

CC:  @mkoeppe @tscrim @dimpase

Branch/Commit: **[093f72e](https://github.com/sagemath/sagetrac-mirror/commit/093f72e0a5bc7618ee17d40d8a4c0794d448ae3f)**

Reviewer: **Matthias Koeppe**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/32239_





---

archive/issue_events_426434.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2021-07-19T15:30:24Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32239",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32239#event-426434"
}
```



---

archive/issue_events_426435.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2021-07-19T15:30:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32239",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32239#event-426435"
}
```



---

archive/issue_events_426436.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2021-07-19T15:30:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32239",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32239#event-426436"
}
```



---

archive/issue_events_426437.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2021-07-19T15:30:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32239",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32239#event-426437"
}
```



---

archive/issue_events_426438.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2021-07-19T15:38:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32239",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32239#event-426438"
}
```



---

archive/issue_comments_522407.json:
```json
{
    "body": "Branch: **[public/graphs/32239_graph_3](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/32239_graph_3)**",
    "created_at": "2021-07-19T15:38:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32239",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32239#issuecomment-522407",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[public/graphs/32239_graph_3](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/32239_graph_3)**



---

archive/issue_comments_522408.json:
```json
{
    "body": "Commit: **[093f72e](https://github.com/sagemath/sagetrac-mirror/commit/093f72e0a5bc7618ee17d40d8a4c0794d448ae3f)**",
    "created_at": "2021-07-19T15:38:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32239",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32239#issuecomment-522408",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[093f72e](https://github.com/sagemath/sagetrac-mirror/commit/093f72e0a5bc7618ee17d40d8a4c0794d448ae3f)**



---

archive/issue_comments_522409.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nLast 10 new commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3dca4e940cfdbe84b9a301a71f263b36a01d441b\">3dca4e9</a></td><td><code>MixedIntegerLinearProgram.get_values: If convert=True, convert real variables from float to base_ring</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9ad3fa80afd83753fe5b6445eb8338f8f885ddcc\">9ad3fa8</a></td><td><code>MixedIntegerLinearProgram.get_converted_values; subsumed by get_values</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f62a7777b342bc46ecd5e4ccb1819d12bdb5db8a\">f62a777</a></td><td><code>src/sage/numerical/mip.pyx: Remove unused imports</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cfd8c7becb47cbe3ad9ad5cf9b6fb2b12122657e\">cfd8c7b</a></td><td><code>trac #32197: remove another useless import</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cf16293c1131db37f0ed1274eded6717c819989c\">cf16293</a></td><td><code>MixedIntegerLinearProgram.get_values: Fix up for tolerance=None</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5a74113233bba7b0ac689f576849c9c36dd6cb39\">5a74113</a></td><td><code>MixedIntegerLinearProgram._backend_variable_value: Add docstring</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/aa4eed5843bce5bbb022ef79577e266d2ef41bfc\">aa4eed5</a></td><td><code>MixedIntegerLinearProgram._backend_variable_value*: Add docstrings, examples</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/00b34aaedbefbdfa7d431868f20055fd6d3572b6\">00b34aa</a></td><td><code>trac #32197: review commit</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/db83853c6df0ed792b4b011b842cf5fd01444765\">db83853</a></td><td><code>trac #32197: another review commit</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/093f72e0a5bc7618ee17d40d8a4c0794d448ae3f\">093f72e</a></td><td><code>trac #32239: graph part 3</code></td></tr></table>\n",
    "created_at": "2021-07-19T15:38:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32239",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32239#issuecomment-522409",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:1'>Comment 1:</a>
Last 10 new commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3dca4e940cfdbe84b9a301a71f263b36a01d441b">3dca4e9</a></td><td><code>MixedIntegerLinearProgram.get_values: If convert=True, convert real variables from float to base_ring</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9ad3fa80afd83753fe5b6445eb8338f8f885ddcc">9ad3fa8</a></td><td><code>MixedIntegerLinearProgram.get_converted_values; subsumed by get_values</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f62a7777b342bc46ecd5e4ccb1819d12bdb5db8a">f62a777</a></td><td><code>src/sage/numerical/mip.pyx: Remove unused imports</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cfd8c7becb47cbe3ad9ad5cf9b6fb2b12122657e">cfd8c7b</a></td><td><code>trac #32197: remove another useless import</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cf16293c1131db37f0ed1274eded6717c819989c">cf16293</a></td><td><code>MixedIntegerLinearProgram.get_values: Fix up for tolerance=None</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5a74113233bba7b0ac689f576849c9c36dd6cb39">5a74113</a></td><td><code>MixedIntegerLinearProgram._backend_variable_value: Add docstring</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/aa4eed5843bce5bbb022ef79577e266d2ef41bfc">aa4eed5</a></td><td><code>MixedIntegerLinearProgram._backend_variable_value*: Add docstrings, examples</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/00b34aaedbefbdfa7d431868f20055fd6d3572b6">00b34aa</a></td><td><code>trac #32197: review commit</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/db83853c6df0ed792b4b011b842cf5fd01444765">db83853</a></td><td><code>trac #32197: another review commit</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/093f72e0a5bc7618ee17d40d8a4c0794d448ae3f">093f72e</a></td><td><code>trac #32239: graph part 3</code></td></tr></table>




---

archive/issue_comments_522410.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2021-07-19T20:25:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32239",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32239#issuecomment-522410",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_comments_522411.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nLGTM. Green patchbot => positive review",
    "created_at": "2021-07-19T20:25:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32239",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32239#issuecomment-522411",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:2'>Comment 2:</a>
LGTM. Green patchbot => positive review



---

archive/issue_events_426439.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-20T06:25:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32239",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32239#event-426439"
}
```



---

archive/issue_events_426440.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-20T06:25:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32239",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32239#event-426440"
}
```



---

archive/issue_comments_522412.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nI've tested it locally",
    "created_at": "2021-07-20T06:25:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32239",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32239#issuecomment-522412",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:3'>Comment 3:</a>
I've tested it locally



---

archive/issue_events_426441.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-07-25T13:26:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32239",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32239#event-426441"
}
```



---

archive/issue_events_426442.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "c09e72a683e9a2c81747930c5da0a429239a26b1",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-07-25T13:26:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32239",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32239#event-426442"
}
```



---

archive/issue_comments_522413.json:
```json
{
    "body": "Changed branch from **[public/graphs/32239_graph_3](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/32239_graph_3)** to **[093f72e](https://github.com/sagemath/sagetrac-mirror/commit/093f72e0a5bc7618ee17d40d8a4c0794d448ae3f)**",
    "created_at": "2021-07-25T13:26:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32239",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32239#issuecomment-522413",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/graphs/32239_graph_3](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/32239_graph_3)** to **[093f72e](https://github.com/sagemath/sagetrac-mirror/commit/093f72e0a5bc7618ee17d40d8a4c0794d448ae3f)**
