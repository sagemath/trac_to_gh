# Issue 32233: Remove DeprecatedSFunction (SFunction, PrimitiveFunction)

archive/issues_031996.json:
```json
{
    "assignees": [],
    "body": "Apparently deprecated since the beginning of time. \n(see #6465)\n\nWe also remove the unused `sage.symbolic.function_factory.deprecated_custom_evalf_wrapper`\n(from #7490)\n\n\nCC:  @DaveWitteMorris @kcrisman @orlitzky\n\nBranch/Commit: **[8f3c22c](https://github.com/sagemath/sagetrac-mirror/commit/8f3c22cc23afa22c15cdd16f31b822540407412f)**\n\nReviewer: **Michael Orlitzky**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32233_\n\n",
    "closed_at": "2021-07-25T13:26:22Z",
    "created_at": "2021-07-18T20:32:12Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove DeprecatedSFunction (SFunction, PrimitiveFunction)",
    "type": "issue",
    "updated_at": "2021-07-25T13:26:22Z",
    "url": "https://github.com/sagemath/sage/issues/32233",
    "user": "https://github.com/mkoeppe"
}
```
Apparently deprecated since the beginning of time. 
(see #6465)

We also remove the unused `sage.symbolic.function_factory.deprecated_custom_evalf_wrapper`
(from #7490)


CC:  @DaveWitteMorris @kcrisman @orlitzky

Branch/Commit: **[8f3c22c](https://github.com/sagemath/sagetrac-mirror/commit/8f3c22cc23afa22c15cdd16f31b822540407412f)**

Reviewer: **Michael Orlitzky**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/32233_





---

archive/issue_events_426378.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-18T20:32:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32233",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32233#event-426378"
}
```



---

archive/issue_events_426379.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-18T20:32:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32233",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32233#event-426379"
}
```



---

archive/issue_events_426380.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-18T20:32:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32233",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32233#event-426380"
}
```



---

archive/issue_events_426381.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-18T20:32:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32233",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32233#event-426381"
}
```



---

archive/issue_comments_522280.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,5 @@\n Apparently deprecated since the beginning of time. \n (see #6465)\n \n+We also remove the unused `sage.symbolic.function_factory.deprecated_custom_evalf_wrapper`\n+\n``````\n",
    "created_at": "2021-07-18T20:43:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32233",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32233#issuecomment-522280",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,5 @@
 Apparently deprecated since the beginning of time. 
 (see #6465)
 
+We also remove the unused `sage.symbolic.function_factory.deprecated_custom_evalf_wrapper`
+
``````




---

archive/issue_comments_522281.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2021-07-18T20:43:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32233",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32233#issuecomment-522281",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_comments_522282.json:
```json
{
    "body": "Branch: **[u/mkoeppe/remove_deprecatedsfunction__sfunction__primitivefunction_](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/remove_deprecatedsfunction__sfunction__primitivefunction_)**",
    "created_at": "2021-07-18T20:44:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32233",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32233#issuecomment-522282",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/remove_deprecatedsfunction__sfunction__primitivefunction_](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/remove_deprecatedsfunction__sfunction__primitivefunction_)**



---

archive/issue_events_426382.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-18T20:46:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32233",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32233#event-426382"
}
```



---

archive/issue_comments_522283.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/94d7ce4e558cc68f1d8bdcca6f5fb279533a90f7\">94d7ce4</a></td><td><code>PrimitiveFunction: Remove</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/945e8021984dfcfc71c2c043103bc2a7587589c8\">945e802</a></td><td><code>SFunction: Remove</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/905a2346d4525e43a4b8962b6e47233a5da02366\">905a234</a></td><td><code>DeprecatedSFunction: Remove</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/48dff5b913aa9b77d8a42197ca9e3f54646ebd23\">48dff5b</a></td><td><code>deprecated_custom_evalf_wrapper: Remove</code></td></tr></table>\n",
    "created_at": "2021-07-18T20:46:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32233",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32233#issuecomment-522283",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:3'>Comment 3:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/94d7ce4e558cc68f1d8bdcca6f5fb279533a90f7">94d7ce4</a></td><td><code>PrimitiveFunction: Remove</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/945e8021984dfcfc71c2c043103bc2a7587589c8">945e802</a></td><td><code>SFunction: Remove</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/905a2346d4525e43a4b8962b6e47233a5da02366">905a234</a></td><td><code>DeprecatedSFunction: Remove</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/48dff5b913aa9b77d8a42197ca9e3f54646ebd23">48dff5b</a></td><td><code>deprecated_custom_evalf_wrapper: Remove</code></td></tr></table>




---

archive/issue_comments_522284.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,4 +2,5 @@\n (see #6465)\n \n We also remove the unused `sage.symbolic.function_factory.deprecated_custom_evalf_wrapper`\n+(from #7490)\n \n``````\n",
    "created_at": "2021-07-18T20:46:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32233",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32233#issuecomment-522284",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,4 +2,5 @@
 (see #6465)
 
 We also remove the unused `sage.symbolic.function_factory.deprecated_custom_evalf_wrapper`
+(from #7490)
 
``````




---

archive/issue_comments_522285.json:
```json
{
    "body": "Commit: **[48dff5b](https://github.com/sagemath/sagetrac-mirror/commit/48dff5b913aa9b77d8a42197ca9e3f54646ebd23)**",
    "created_at": "2021-07-18T20:46:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32233",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32233#issuecomment-522285",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[48dff5b](https://github.com/sagemath/sagetrac-mirror/commit/48dff5b913aa9b77d8a42197ca9e3f54646ebd23)**



---

archive/issue_comments_522286.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\ngit grep turns up a few docstrings that incorrectly reference `SFunction`, other than that OK.",
    "created_at": "2021-07-22T12:03:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32233",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32233#issuecomment-522286",
    "user": "https://github.com/orlitzky"
}
```

<a id='comment:4'>Comment 4:</a>
git grep turns up a few docstrings that incorrectly reference `SFunction`, other than that OK.



---

archive/issue_comments_522287.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/174bee434f5bb7a04c6544c4bf562156eba860fe\">174bee4</a></td><td><code>src/sage/symbolic/function.pyx: Update docstrings, no longer refer to functions as SFunction</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8f3c22cc23afa22c15cdd16f31b822540407412f\">8f3c22c</a></td><td><code>src/sage/calculus/calculus.py, src/sage/symbolic/expression.pyx: Update references to SFunction</code></td></tr></table>\n",
    "created_at": "2021-07-22T15:21:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32233",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32233#issuecomment-522287",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:5'>Comment 5:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/174bee434f5bb7a04c6544c4bf562156eba860fe">174bee4</a></td><td><code>src/sage/symbolic/function.pyx: Update docstrings, no longer refer to functions as SFunction</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8f3c22cc23afa22c15cdd16f31b822540407412f">8f3c22c</a></td><td><code>src/sage/calculus/calculus.py, src/sage/symbolic/expression.pyx: Update references to SFunction</code></td></tr></table>




---

archive/issue_comments_522288.json:
```json
{
    "body": "Changed commit from **[48dff5b](https://github.com/sagemath/sagetrac-mirror/commit/48dff5b913aa9b77d8a42197ca9e3f54646ebd23)** to **[8f3c22c](https://github.com/sagemath/sagetrac-mirror/commit/8f3c22cc23afa22c15cdd16f31b822540407412f)**",
    "created_at": "2021-07-22T15:21:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32233",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32233#issuecomment-522288",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[48dff5b](https://github.com/sagemath/sagetrac-mirror/commit/48dff5b913aa9b77d8a42197ca9e3f54646ebd23)** to **[8f3c22c](https://github.com/sagemath/sagetrac-mirror/commit/8f3c22cc23afa22c15cdd16f31b822540407412f)**



---

archive/issue_comments_522289.json:
```json
{
    "body": "Reviewer: **Michael Orlitzky**",
    "created_at": "2021-07-22T20:44:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32233",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32233#issuecomment-522289",
    "user": "https://github.com/orlitzky"
}
```

Reviewer: **Michael Orlitzky**



---

archive/issue_events_426383.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2021-07-22T20:44:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32233",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32233#event-426383"
}
```



---

archive/issue_events_426384.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2021-07-22T20:44:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32233",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32233#event-426384"
}
```



---

archive/issue_comments_522290.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nThanks!",
    "created_at": "2021-07-22T22:17:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32233",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32233#issuecomment-522290",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:7'>Comment 7:</a>
Thanks!



---

archive/issue_comments_522291.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/remove_deprecatedsfunction__sfunction__primitivefunction_](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/remove_deprecatedsfunction__sfunction__primitivefunction_)** to **[8f3c22c](https://github.com/sagemath/sagetrac-mirror/commit/8f3c22cc23afa22c15cdd16f31b822540407412f)**",
    "created_at": "2021-07-25T13:26:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32233",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32233#issuecomment-522291",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/remove_deprecatedsfunction__sfunction__primitivefunction_](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/remove_deprecatedsfunction__sfunction__primitivefunction_)** to **[8f3c22c](https://github.com/sagemath/sagetrac-mirror/commit/8f3c22cc23afa22c15cdd16f31b822540407412f)**



---

archive/issue_events_426385.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-07-25T13:26:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32233",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32233#event-426385"
}
```



---

archive/issue_events_426386.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "59a8fa6555fa6d708d15aa2d5c01a1ee8f2703fa",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-07-25T13:26:22Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32233",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32233#event-426386"
}
```
