# Issue 32449: Repair doctests for system giac 1.7

archive/issues_032212.json:
```json
{
    "assignees": [],
    "body": "(from [#29966 comment:14](https://github.com/sagemath/sage/issues/29966#comment:14))\n\n\n\nCC:  @antonio-rojas @fchapoton @dimpase @frederichan-IMJPRG @sagetrac-parisse @seblabbe @slel\n\nBranch/Commit: **[7d6a7b8](https://github.com/sagemath/sagetrac-mirror/commit/7d6a7b82886827480d8ef4664c0e484cb669aa1d)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Antonio Rojas**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32449_\n\n",
    "closed_at": "2021-09-19T09:58:50Z",
    "created_at": "2021-09-01T21:40:46Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Repair doctests for system giac 1.7",
    "type": "issue",
    "updated_at": "2021-09-19T09:58:50Z",
    "url": "https://github.com/sagemath/sage/issues/32449",
    "user": "https://github.com/mkoeppe"
}
```
(from [#29966 comment:14](https://github.com/sagemath/sage/issues/29966#comment:14))



CC:  @antonio-rojas @fchapoton @dimpase @frederichan-IMJPRG @sagetrac-parisse @seblabbe @slel

Branch/Commit: **[7d6a7b8](https://github.com/sagemath/sagetrac-mirror/commit/7d6a7b82886827480d8ef4664c0e484cb669aa1d)**

Reviewer: **Frédéric Chapoton**

Author: **Antonio Rojas**

_Issue created by migration from https://trac.sagemath.org/ticket/32449_





---

archive/issue_events_429209.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-01T21:40:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32449",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32449#event-429209"
}
```



---

archive/issue_events_429210.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-01T21:40:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32449",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32449#event-429210"
}
```



---

archive/issue_events_429211.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-01T21:40:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32449",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32449#event-429211"
}
```



---

archive/issue_events_429212.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-01T21:40:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32449",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32449#event-429212"
}
```



---

archive/issue_comments_526604.json:
```json
{
    "body": "Branch: **[u/arojas/repair_doctests_for_system_giac_1_7](https://github.com/sagemath/sagetrac-mirror/tree/u/arojas/repair_doctests_for_system_giac_1_7)**",
    "created_at": "2021-09-02T18:52:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32449",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32449#issuecomment-526604",
    "user": "https://github.com/antonio-rojas"
}
```

Branch: **[u/arojas/repair_doctests_for_system_giac_1_7](https://github.com/sagemath/sagetrac-mirror/tree/u/arojas/repair_doctests_for_system_giac_1_7)**



---

archive/issue_comments_526605.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nUsing a wildcard since this is only testing that it doesn't throw an error.\n\nI don't know how to test that the answer is correct in a way that works with both giac versions: testing that the derivative equals `sqrt(x+sqrt(x))` fails with old giac (but works with 1.7)\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7d6a7b82886827480d8ef4664c0e484cb669aa1d\">7d6a7b8</a></td><td><code>Make test pass with giac 1.7</code></td></tr></table>\n",
    "created_at": "2021-09-02T18:56:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32449",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32449#issuecomment-526605",
    "user": "https://github.com/antonio-rojas"
}
```

<div id="comment:2" align="right">Comment 2</div>

Using a wildcard since this is only testing that it doesn't throw an error.

I don't know how to test that the answer is correct in a way that works with both giac versions: testing that the derivative equals `sqrt(x+sqrt(x))` fails with old giac (but works with 1.7)

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7d6a7b82886827480d8ef4664c0e484cb669aa1d">7d6a7b8</a></td><td><code>Make test pass with giac 1.7</code></td></tr></table>




---

archive/issue_comments_526606.json:
```json
{
    "body": "Author: **Antonio Rojas**",
    "created_at": "2021-09-02T18:56:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32449",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32449#issuecomment-526606",
    "user": "https://github.com/antonio-rojas"
}
```

Author: **Antonio Rojas**



---

archive/issue_comments_526607.json:
```json
{
    "body": "Commit: **[7d6a7b8](https://github.com/sagemath/sagetrac-mirror/commit/7d6a7b82886827480d8ef4664c0e484cb669aa1d)**",
    "created_at": "2021-09-02T18:56:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32449",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32449#issuecomment-526607",
    "user": "https://github.com/antonio-rojas"
}
```

Commit: **[7d6a7b8](https://github.com/sagemath/sagetrac-mirror/commit/7d6a7b82886827480d8ef4664c0e484cb669aa1d)**



---

archive/issue_events_429213.json:
```json
{
    "actor": "https://github.com/antonio-rojas",
    "created_at": "2021-09-02T18:56:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32449",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32449#event-429213"
}
```



---

archive/issue_comments_526608.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nok, good enough",
    "created_at": "2021-09-02T19:19:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32449",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32449#issuecomment-526608",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">Comment 3</div>

ok, good enough



---

archive/issue_events_429214.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-09-02T19:19:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32449",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32449#event-429214"
}
```



---

archive/issue_events_429215.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-09-02T19:19:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32449",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32449#event-429215"
}
```



---

archive/issue_comments_526609.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2021-09-02T19:19:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32449",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32449#issuecomment-526609",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_events_429216.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-09-19T09:58:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32449",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32449#event-429216"
}
```



---

archive/issue_events_429217.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "bbdcdb6d3f00b263eea6e2158944940e54d44e75",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-09-19T09:58:50Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32449",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32449#event-429217"
}
```



---

archive/issue_comments_526610.json:
```json
{
    "body": "Changed branch from **[u/arojas/repair_doctests_for_system_giac_1_7](https://github.com/sagemath/sagetrac-mirror/tree/u/arojas/repair_doctests_for_system_giac_1_7)** to **[7d6a7b8](https://github.com/sagemath/sagetrac-mirror/commit/7d6a7b82886827480d8ef4664c0e484cb669aa1d)**",
    "created_at": "2021-09-19T09:58:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32449",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32449#issuecomment-526610",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/arojas/repair_doctests_for_system_giac_1_7](https://github.com/sagemath/sagetrac-mirror/tree/u/arojas/repair_doctests_for_system_giac_1_7)** to **[7d6a7b8](https://github.com/sagemath/sagetrac-mirror/commit/7d6a7b82886827480d8ef4664c0e484cb669aa1d)**
