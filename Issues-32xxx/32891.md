# Issue 32891: Unstable doctest geometry/hyperbolic_space/hyperbolic_model.py

archive/issues_032654.json:
```json
{
    "assignees": [],
    "body": "Part of #32544:\n\n```\nsage -t --long --random-seed=257293673212457698646690051018428858654 src/sage/geometry/hyperbolic_space/hyperbolic_model.py\n**********************************************************************\nFile \"src/sage/geometry/hyperbolic_space/hyperbolic_model.py\", line 567, in sage.geometry.hyperbolic_space.hyperbolic_model.HyperbolicModel.random_geodesic\nFailed example:\n    bool((h.endpoints()[0].coordinates()).imag() >= 0)\nExpected:\n    True\nGot:\n    False\n**********************************************************************\n1 item had failures:\n   1 of   3 in sage.geometry.hyperbolic_space.hyperbolic_model.HyperbolicModel.random_geodesic\n    [234 tests, 1 failure, 0.77 s]\n------------------------------------\n```\n\nCC:  @orlitzky @tscrim @sagetrac-jhonrubia6\n\nBranch/Commit: **[eddded5](https://github.com/sagemath/sagetrac-mirror/commit/eddded5fcfa1e342cd904041c2cfd012a76a85ea)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Michael Orlitzky**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32891_\n\n",
    "closed_at": "2022-01-31T23:30:57Z",
    "created_at": "2021-11-17T20:06:49Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20doctest%20framework",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Unstable doctest geometry/hyperbolic_space/hyperbolic_model.py",
    "type": "issue",
    "updated_at": "2022-01-31T23:30:57Z",
    "url": "https://github.com/sagemath/sage/issues/32891",
    "user": "https://github.com/kliem"
}
```
Part of #32544:

```
sage -t --long --random-seed=257293673212457698646690051018428858654 src/sage/geometry/hyperbolic_space/hyperbolic_model.py
**********************************************************************
File "src/sage/geometry/hyperbolic_space/hyperbolic_model.py", line 567, in sage.geometry.hyperbolic_space.hyperbolic_model.HyperbolicModel.random_geodesic
Failed example:
    bool((h.endpoints()[0].coordinates()).imag() >= 0)
Expected:
    True
Got:
    False
**********************************************************************
1 item had failures:
   1 of   3 in sage.geometry.hyperbolic_space.hyperbolic_model.HyperbolicModel.random_geodesic
    [234 tests, 1 failure, 0.77 s]
------------------------------------
```

CC:  @orlitzky @tscrim @sagetrac-jhonrubia6

Branch/Commit: **[eddded5](https://github.com/sagemath/sagetrac-mirror/commit/eddded5fcfa1e342cd904041c2cfd012a76a85ea)**

Reviewer: **Travis Scrimshaw**

Author: **Michael Orlitzky**

_Issue created by migration from https://trac.sagemath.org/ticket/32891_





---

archive/issue_events_434453.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2021-11-17T20:06:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32891",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32891#event-434453"
}
```



---

archive/issue_events_434454.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2021-11-17T20:06:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32891",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20doctest%20framework",
    "label_color": "000080",
    "label_name": "component: doctest framework",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32891#event-434454"
}
```



---

archive/issue_events_434455.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2021-11-17T20:06:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32891",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32891#event-434455"
}
```



---

archive/issue_events_434456.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2021-11-17T20:06:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32891",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32891#event-434456"
}
```



---

archive/issue_events_434457.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T20:12:32Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/32891",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32891#event-434457"
}
```



---

archive/issue_events_434458.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T20:12:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32891",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32891#event-434458"
}
```



---

archive/issue_comments_534119.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nWild ass guess: this should be testing `HyperbolicPlane().UHP()` and not `HyperbolicPlane().PD()`. The former stands for Upper Half Plane, the latter for Poincare Disk.",
    "created_at": "2021-12-27T13:21:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32891",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32891#issuecomment-534119",
    "user": "https://github.com/orlitzky"
}
```

<div id="comment:2" align="right">Comment 2</div>

Wild ass guess: this should be testing `HyperbolicPlane().UHP()` and not `HyperbolicPlane().PD()`. The former stands for Upper Half Plane, the latter for Poincare Disk.



---

archive/issue_comments_534120.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nIndeed, that test is only valid for the UHP, but that method is already being tested. Here, we should test that the absolute value is at most 1 to test in the PD model.",
    "created_at": "2021-12-28T00:03:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32891",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32891#issuecomment-534120",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:3" align="right">Comment 3</div>

Indeed, that test is only valid for the UHP, but that method is already being tested. Here, we should test that the absolute value is at most 1 to test in the PD model.



---

archive/issue_events_434459.json:
```json
{
    "actor": "https://github.com/orlitzky",
    "created_at": "2021-12-29T11:48:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32891",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32891#event-434459"
}
```



---

archive/issue_comments_534121.json:
```json
{
    "body": "Branch: **[u/mjo/ticket/32891](https://github.com/sagemath/sagetrac-mirror/tree/u/mjo/ticket/32891)**",
    "created_at": "2021-12-29T11:48:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32891",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32891#issuecomment-534121",
    "user": "https://github.com/orlitzky"
}
```

Branch: **[u/mjo/ticket/32891](https://github.com/sagemath/sagetrac-mirror/tree/u/mjo/ticket/32891)**



---

archive/issue_comments_534122.json:
```json
{
    "body": "Author: **Michael Orlitzky**",
    "created_at": "2021-12-29T11:48:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32891",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32891#issuecomment-534122",
    "user": "https://github.com/orlitzky"
}
```

Author: **Michael Orlitzky**



---

archive/issue_comments_534123.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/eddded5fcfa1e342cd904041c2cfd012a76a85ea\">eddded5</a></td><td><code>Trac #32891: fix a failing hyperbolic model doctest.</code></td></tr></table>\n",
    "created_at": "2021-12-29T11:48:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32891",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32891#issuecomment-534123",
    "user": "https://github.com/orlitzky"
}
```

<div id="comment:4" align="right">Comment 4</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/eddded5fcfa1e342cd904041c2cfd012a76a85ea">eddded5</a></td><td><code>Trac #32891: fix a failing hyperbolic model doctest.</code></td></tr></table>




---

archive/issue_comments_534124.json:
```json
{
    "body": "Commit: **[eddded5](https://github.com/sagemath/sagetrac-mirror/commit/eddded5fcfa1e342cd904041c2cfd012a76a85ea)**",
    "created_at": "2021-12-29T11:48:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32891",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32891#issuecomment-534124",
    "user": "https://github.com/orlitzky"
}
```

Commit: **[eddded5](https://github.com/sagemath/sagetrac-mirror/commit/eddded5fcfa1e342cd904041c2cfd012a76a85ea)**



---

archive/issue_comments_534125.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThank you. LGTM.",
    "created_at": "2022-01-01T02:43:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32891",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32891#issuecomment-534125",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:5" align="right">Comment 5</div>

Thank you. LGTM.



---

archive/issue_comments_534126.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2022-01-01T02:43:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32891",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32891#issuecomment-534126",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_434460.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-01-01T02:43:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32891",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32891#event-434460"
}
```



---

archive/issue_events_434461.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2022-01-01T02:43:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32891",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32891#event-434461"
}
```



---

archive/issue_comments_534127.json:
```json
{
    "body": "Changed branch from **[u/mjo/ticket/32891](https://github.com/sagemath/sagetrac-mirror/tree/u/mjo/ticket/32891)** to **[eddded5](https://github.com/sagemath/sagetrac-mirror/commit/eddded5fcfa1e342cd904041c2cfd012a76a85ea)**",
    "created_at": "2022-01-31T23:30:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32891",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32891#issuecomment-534127",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mjo/ticket/32891](https://github.com/sagemath/sagetrac-mirror/tree/u/mjo/ticket/32891)** to **[eddded5](https://github.com/sagemath/sagetrac-mirror/commit/eddded5fcfa1e342cd904041c2cfd012a76a85ea)**



---

archive/issue_events_434462.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-01-31T23:30:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32891",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32891#event-434462"
}
```



---

archive/issue_events_434463.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "50d9be9690d0baccd378575c4f54e3e41b97cdd0",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-01-31T23:30:57Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32891",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32891#event-434463"
}
```
