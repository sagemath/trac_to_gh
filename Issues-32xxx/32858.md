# Issue 32858: tiny doctest formatting fix in integer_mod_ring.py

archive/issues_032621.json:
```json
{
    "assignees": [],
    "body": "The last example for `IntegerModRing_generic.unit_gens()` is not formatted properly:\n\nhttps://doc.sagemath.org/html/en/reference/finite_rings/sage/rings/finite_rings/integer_mod_ring.html#sage.rings.finite_rings.integer_mod_ring.IntegerModRing_generic.unit_gens\n\nLet's add `::` to fix this.\n\nBranch/Commit: **[d6a179f](https://github.com/sagemath/sagetrac-mirror/commit/d6a179f69c1cceaf1af2ecd6ea5fa4463d1edcbd)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Lorenz Panny**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32858_\n\n",
    "closed_at": "2021-11-18T23:37:50Z",
    "created_at": "2021-11-12T06:06:31Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "tiny doctest formatting fix in integer_mod_ring.py",
    "type": "issue",
    "updated_at": "2021-11-18T23:37:50Z",
    "url": "https://github.com/sagemath/sage/issues/32858",
    "user": "https://github.com/yyyyx4"
}
```
The last example for `IntegerModRing_generic.unit_gens()` is not formatted properly:

https://doc.sagemath.org/html/en/reference/finite_rings/sage/rings/finite_rings/integer_mod_ring.html#sage.rings.finite_rings.integer_mod_ring.IntegerModRing_generic.unit_gens

Let's add `::` to fix this.

Branch/Commit: **[d6a179f](https://github.com/sagemath/sagetrac-mirror/commit/d6a179f69c1cceaf1af2ecd6ea5fa4463d1edcbd)**

Reviewer: **Frédéric Chapoton**

Author: **Lorenz Panny**

_Issue created by migration from https://trac.sagemath.org/ticket/32858_





---

archive/issue_events_434054.json:
```json
{
    "actor": "https://github.com/yyyyx4",
    "created_at": "2021-11-12T06:06:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32858",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32858#event-434054"
}
```



---

archive/issue_events_434055.json:
```json
{
    "actor": "https://github.com/yyyyx4",
    "created_at": "2021-11-12T06:06:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32858",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32858#event-434055"
}
```



---

archive/issue_events_434056.json:
```json
{
    "actor": "https://github.com/yyyyx4",
    "created_at": "2021-11-12T06:06:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32858",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32858#event-434056"
}
```



---

archive/issue_events_434057.json:
```json
{
    "actor": "https://github.com/yyyyx4",
    "created_at": "2021-11-12T06:06:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32858",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32858#event-434057"
}
```



---

archive/issue_comments_533421.json:
```json
{
    "body": "Commit: **[d6a179f](https://github.com/sagemath/sagetrac-mirror/commit/d6a179f69c1cceaf1af2ecd6ea5fa4463d1edcbd)**",
    "created_at": "2021-11-12T06:08:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32858#issuecomment-533421",
    "user": "https://github.com/yyyyx4"
}
```

Commit: **[d6a179f](https://github.com/sagemath/sagetrac-mirror/commit/d6a179f69c1cceaf1af2ecd6ea5fa4463d1edcbd)**



---

archive/issue_comments_533422.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,3 +2,4 @@\n \n https://doc.sagemath.org/html/en/reference/finite_rings/sage/rings/finite_rings/integer_mod_ring.html#sage.rings.finite_rings.integer_mod_ring.IntegerModRing_generic.unit_gens\n \n+Let's add `::` to fix this.\n``````\n",
    "created_at": "2021-11-12T06:08:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32858#issuecomment-533422",
    "user": "https://github.com/yyyyx4"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,3 +2,4 @@
 
 https://doc.sagemath.org/html/en/reference/finite_rings/sage/rings/finite_rings/integer_mod_ring.html#sage.rings.finite_rings.integer_mod_ring.IntegerModRing_generic.unit_gens
 
+Let's add `::` to fix this.
``````




---

archive/issue_events_434058.json:
```json
{
    "actor": "https://github.com/yyyyx4",
    "created_at": "2021-11-12T06:08:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32858",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32858#event-434058"
}
```



---

archive/issue_comments_533423.json:
```json
{
    "body": "Branch: **[public/32858](https://github.com/sagemath/sagetrac-mirror/tree/public/32858)**",
    "created_at": "2021-11-12T06:08:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32858#issuecomment-533423",
    "user": "https://github.com/yyyyx4"
}
```

Branch: **[public/32858](https://github.com/sagemath/sagetrac-mirror/tree/public/32858)**



---

archive/issue_comments_533424.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d6a179f69c1cceaf1af2ecd6ea5fa4463d1edcbd\">d6a179f</a></td><td><code>fix doctest formatting</code></td></tr></table>\n",
    "created_at": "2021-11-12T06:08:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32858#issuecomment-533424",
    "user": "https://github.com/yyyyx4"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d6a179f69c1cceaf1af2ecd6ea5fa4463d1edcbd">d6a179f</a></td><td><code>fix doctest formatting</code></td></tr></table>




---

archive/issue_comments_533425.json:
```json
{
    "body": "Author: **Lorenz Panny**",
    "created_at": "2021-11-12T06:08:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32858#issuecomment-533425",
    "user": "https://github.com/yyyyx4"
}
```

Author: **Lorenz Panny**



---

archive/issue_events_434059.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-11-12T07:31:57Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32858",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32858#event-434059"
}
```



---

archive/issue_events_434060.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-11-12T07:31:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32858",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32858#event-434060"
}
```



---

archive/issue_comments_533426.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2021-11-12T07:31:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32858#issuecomment-533426",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_533427.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nok",
    "created_at": "2021-11-12T07:31:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32858#issuecomment-533427",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

ok



---

archive/issue_events_434061.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-11-18T23:37:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32858",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32858#event-434061"
}
```



---

archive/issue_events_434062.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "cdb3614b34926cee3c4cc918313678d3deea4e8d",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-11-18T23:37:50Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32858",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32858#event-434062"
}
```



---

archive/issue_comments_533428.json:
```json
{
    "body": "Changed branch from **[public/32858](https://github.com/sagemath/sagetrac-mirror/tree/public/32858)** to **[d6a179f](https://github.com/sagemath/sagetrac-mirror/commit/d6a179f69c1cceaf1af2ecd6ea5fa4463d1edcbd)**",
    "created_at": "2021-11-18T23:37:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32858",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32858#issuecomment-533428",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/32858](https://github.com/sagemath/sagetrac-mirror/tree/public/32858)** to **[d6a179f](https://github.com/sagemath/sagetrac-mirror/commit/d6a179f69c1cceaf1af2ecd6ea5fa4463d1edcbd)**
