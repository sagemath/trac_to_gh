# Issue 32507: Remove useless 'from . import all' from some __init__.py files

archive/issues_032270.json:
```json
{
    "assignees": [],
    "body": "... and similar imports of modules in the package, found via\n\n```\nfind src/sage -name '__init__.py' | xargs grep import\n```\n\nPossibly some of them are needed to break circular imports, but likely not all of them.\n\nPart of #32501.\n\n\nDepends on #32506\n\nCC:  @tscrim @fchapoton\n\nBranch/Commit: **[55ca4bc](https://github.com/sagemath/sagetrac-mirror/commit/55ca4bc9a3a3d3740ee44690640f67803ff5e85f)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32507_\n\n",
    "closed_at": "2021-09-19T09:58:01Z",
    "created_at": "2021-09-12T05:46:16Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/refactoring",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove useless 'from . import all' from some __init__.py files",
    "type": "issue",
    "updated_at": "2021-09-19T09:58:01Z",
    "url": "https://github.com/sagemath/sage/issues/32507",
    "user": "https://github.com/mkoeppe"
}
```
... and similar imports of modules in the package, found via

```
find src/sage -name '__init__.py' | xargs grep import
```

Possibly some of them are needed to break circular imports, but likely not all of them.

Part of #32501.


Depends on #32506

CC:  @tscrim @fchapoton

Branch/Commit: **[55ca4bc](https://github.com/sagemath/sagetrac-mirror/commit/55ca4bc9a3a3d3740ee44690640f67803ff5e85f)**

Reviewer: **Frédéric Chapoton**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/32507_





---

archive/issue_events_429865.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-12T05:46:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32507#event-429865"
}
```



---

archive/issue_events_429866.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-12T05:46:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "label": "https://github.com/sagemath/sage/labels/refactoring",
    "label_color": "696969",
    "label_name": "refactoring",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32507#event-429866"
}
```



---

archive/issue_events_429867.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-12T05:46:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32507#event-429867"
}
```



---

archive/issue_events_429868.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-12T05:46:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32507#event-429868"
}
```



---

archive/issue_comments_527383.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,5 @@\n+... and similar imports of modules in the package.\n+\n Possibly some of them are needed to break circular imports, but likely not all of them.\n \n Part of #32501.\n``````\n",
    "created_at": "2021-09-12T05:50:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32507#issuecomment-527383",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,5 @@
+... and similar imports of modules in the package.
+
 Possibly some of them are needed to break circular imports, but likely not all of them.
 
 Part of #32501.
``````




---

archive/issue_comments_527384.json:
```json
{
    "body": "Branch: **[u/mkoeppe/remove_useless__from___import_all__from_some___init___py_files](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/remove_useless__from___import_all__from_some___init___py_files)**",
    "created_at": "2021-09-12T05:54:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32507#issuecomment-527384",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/remove_useless__from___import_all__from_some___init___py_files](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/remove_useless__from___import_all__from_some___init___py_files)**



---

archive/issue_comments_527385.json:
```json
{
    "body": "Commit: **[8efece0](https://github.com/sagemath/sagetrac-mirror/commit/8efece0a0ed470f57905a57e138e036a99e7b4da)**",
    "created_at": "2021-09-12T05:59:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32507#issuecomment-527385",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[8efece0](https://github.com/sagemath/sagetrac-mirror/commit/8efece0a0ed470f57905a57e138e036a99e7b4da)**



---

archive/issue_comments_527386.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nIn `src/sage/combinat/__init__.py`, there is \n\n```\nfrom . import quickref\nfrom . import tutorial\n```\nThese are modules without code. Are these imports needed for anything?\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8efece0a0ed470f57905a57e138e036a99e7b4da\">8efece0</a></td><td><code>sage.graphs, sage.graphs.graph_decompositions: Remove unnecessary import of modules</code></td></tr></table>\n",
    "created_at": "2021-09-12T05:59:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32507#issuecomment-527386",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:3" align="right">Comment 3</div>

In `src/sage/combinat/__init__.py`, there is 

```
from . import quickref
from . import tutorial
```
These are modules without code. Are these imports needed for anything?

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8efece0a0ed470f57905a57e138e036a99e7b4da">8efece0</a></td><td><code>sage.graphs, sage.graphs.graph_decompositions: Remove unnecessary import of modules</code></td></tr></table>




---

archive/issue_comments_527387.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\n`from . import all` was previously discussed in [#20617 comment:16](https://github.com/sagemath/sage/issues/20617#comment:16)",
    "created_at": "2021-09-13T06:01:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32507#issuecomment-527387",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:4" align="right">Comment 4</div>

`from . import all` was previously discussed in [#20617 comment:16](https://github.com/sagemath/sage/issues/20617#comment:16)



---

archive/issue_comments_527388.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2021-09-13T06:13:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32507#issuecomment-527388",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_comments_527389.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/892f2e7cebace702186044fe4a4cca5c414346a4\">892f2e7</a></td><td><code>Remove all 'from . import all' from __init__.py files</code></td></tr></table>\n",
    "created_at": "2021-09-13T06:21:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32507#issuecomment-527389",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/892f2e7cebace702186044fe4a4cca5c414346a4">892f2e7</a></td><td><code>Remove all 'from . import all' from __init__.py files</code></td></tr></table>




---

archive/issue_comments_527390.json:
```json
{
    "body": "Changed commit from **[8efece0](https://github.com/sagemath/sagetrac-mirror/commit/8efece0a0ed470f57905a57e138e036a99e7b4da)** to **[892f2e7](https://github.com/sagemath/sagetrac-mirror/commit/892f2e7cebace702186044fe4a4cca5c414346a4)**",
    "created_at": "2021-09-13T06:21:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32507#issuecomment-527390",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[8efece0](https://github.com/sagemath/sagetrac-mirror/commit/8efece0a0ed470f57905a57e138e036a99e7b4da)** to **[892f2e7](https://github.com/sagemath/sagetrac-mirror/commit/892f2e7cebace702186044fe4a4cca5c414346a4)**



---

archive/issue_comments_527391.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,8 @@\n-... and similar imports of modules in the package.\n+... and similar imports of modules in the package, found via\n+\n+```\n+find src/sage -name '__init__.py' | xargs grep import\n+```\n \n Possibly some of them are needed to break circular imports, but likely not all of them.\n \n``````\n",
    "created_at": "2021-09-13T06:25:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32507#issuecomment-527391",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,8 @@
-... and similar imports of modules in the package.
+... and similar imports of modules in the package, found via
+
+```
+find src/sage -name '__init__.py' | xargs grep import
+```
 
 Possibly some of them are needed to break circular imports, but likely not all of them.
 
``````




---

archive/issue_comments_527392.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nReplying to [@mkoeppe](#comment%3A3):\n> In `src/sage/combinat/__init__.py`, there is \n> \n> ```\n> from . import quickref\n> from . import tutorial\n> ```\n> These are modules without code. Are these imports needed for anything?\n\nAlso:\n\n```\nsrc/sage/categories/__init__.py:from . import primer\n```",
    "created_at": "2021-09-13T06:34:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32507#issuecomment-527392",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:8" align="right">Comment 8</div>

Replying to [@mkoeppe](#comment%3A3):
> In `src/sage/combinat/__init__.py`, there is 
> 
> ```
> from . import quickref
> from . import tutorial
> ```
> These are modules without code. Are these imports needed for anything?

Also:

```
src/sage/categories/__init__.py:from . import primer
```



---

archive/issue_comments_527393.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nI am guessing these are some kind of workarounds for the sphinx docbuild?\nWe can take care of them in #32508.",
    "created_at": "2021-09-13T06:43:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32507#issuecomment-527393",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:9" align="right">Comment 9</div>

I am guessing these are some kind of workarounds for the sphinx docbuild?
We can take care of them in #32508.



---

archive/issue_events_429869.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-13T06:43:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32507#event-429869"
}
```



---

archive/issue_comments_527394.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nlooks good, but does it pass the doctests with `optional - dot2tex` ?",
    "created_at": "2021-09-13T09:27:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32507#issuecomment-527394",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:10" align="right">Comment 10</div>

looks good, but does it pass the doctests with `optional - dot2tex` ?



---

archive/issue_comments_527395.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nJust tested, yes it does",
    "created_at": "2021-09-13T15:15:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32507#issuecomment-527395",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:11" align="right">Comment 11</div>

Just tested, yes it does



---

archive/issue_comments_527396.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nok, thx. Then let's go",
    "created_at": "2021-09-13T16:13:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32507#issuecomment-527396",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:12" align="right">Comment 12</div>

ok, thx. Then let's go



---

archive/issue_comments_527397.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2021-09-13T16:13:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32507#issuecomment-527397",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_events_429870.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-09-13T16:13:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32507#event-429870"
}
```



---

archive/issue_events_429871.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-09-13T16:13:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32507#event-429871"
}
```



---

archive/issue_comments_527398.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nThank you!",
    "created_at": "2021-09-13T17:34:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32507#issuecomment-527398",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:13" align="right">Comment 13</div>

Thank you!



---

archive/issue_comments_527399.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nBranch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1c106cd3519cf69465e29ed28a9dad041c978c14\">1c106cd</a></td><td><code>Remove useless 'file is not empty' comments from __init__.py files</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/55ca4bc9a3a3d3740ee44690640f67803ff5e85f\">55ca4bc</a></td><td><code>Merge branch 't/32506/remove_useless__file_is_not_empty__comments_from___init___py_files' into t/32507/remove_useless__from___import_all__from_some___init___py_files</code></td></tr></table>\n",
    "created_at": "2021-09-13T17:36:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32507#issuecomment-527399",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:14" align="right">Comment 14</div>

Branch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1c106cd3519cf69465e29ed28a9dad041c978c14">1c106cd</a></td><td><code>Remove useless 'file is not empty' comments from __init__.py files</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/55ca4bc9a3a3d3740ee44690640f67803ff5e85f">55ca4bc</a></td><td><code>Merge branch 't/32506/remove_useless__file_is_not_empty__comments_from___init___py_files' into t/32507/remove_useless__from___import_all__from_some___init___py_files</code></td></tr></table>




---

archive/issue_comments_527400.json:
```json
{
    "body": "Changed commit from **[892f2e7](https://github.com/sagemath/sagetrac-mirror/commit/892f2e7cebace702186044fe4a4cca5c414346a4)** to **[55ca4bc](https://github.com/sagemath/sagetrac-mirror/commit/55ca4bc9a3a3d3740ee44690640f67803ff5e85f)**",
    "created_at": "2021-09-13T17:36:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32507#issuecomment-527400",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[892f2e7](https://github.com/sagemath/sagetrac-mirror/commit/892f2e7cebace702186044fe4a4cca5c414346a4)** to **[55ca4bc](https://github.com/sagemath/sagetrac-mirror/commit/55ca4bc9a3a3d3740ee44690640f67803ff5e85f)**



---

archive/issue_events_429872.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2021-09-13T17:36:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32507#event-429872"
}
```



---

archive/issue_events_429873.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2021-09-13T17:36:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32507#event-429873"
}
```



---

archive/issue_events_429874.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-13T17:37:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32507#event-429874"
}
```



---

archive/issue_events_429875.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-13T17:37:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32507#event-429875"
}
```



---

archive/issue_comments_527401.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">Comment 15</div>\n\nMerged #32506 to remove the trivial merge conflicts",
    "created_at": "2021-09-13T17:37:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32507#issuecomment-527401",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:15" align="right">Comment 15</div>

Merged #32506 to remove the trivial merge conflicts



---

archive/issue_comments_527402.json:
```json
{
    "body": "Dependencies: **#32506**",
    "created_at": "2021-09-13T17:37:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32507#issuecomment-527402",
    "user": "https://github.com/mkoeppe"
}
```

Dependencies: **#32506**



---

archive/issue_events_429876.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-09-19T09:58:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32507#event-429876"
}
```



---

archive/issue_events_429877.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "15d5971552a327af32c0222cab89195066542d7d",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-09-19T09:58:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32507#event-429877"
}
```



---

archive/issue_comments_527403.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/remove_useless__from___import_all__from_some___init___py_files](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/remove_useless__from___import_all__from_some___init___py_files)** to **[55ca4bc](https://github.com/sagemath/sagetrac-mirror/commit/55ca4bc9a3a3d3740ee44690640f67803ff5e85f)**",
    "created_at": "2021-09-19T09:58:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32507",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32507#issuecomment-527403",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/remove_useless__from___import_all__from_some___init___py_files](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/remove_useless__from___import_all__from_some___init___py_files)** to **[55ca4bc](https://github.com/sagemath/sagetrac-mirror/commit/55ca4bc9a3a3d3740ee44690640f67803ff5e85f)**
