# Issue 32044: build/pkgs/compilerwrapper: Remove

archive/issues_031807.json:
```json
{
    "assignees": [],
    "body": "Looking at https://bitbucket.org/vbraun/compilerwrapper/src/master/ (no changes since 2013), it is likely that our `compilerwrapper` package (marked \"experimental\") is not needed any more.\n\nThis ticket removes it.\n\n\nCC:  @vbraun @dimpase\n\nBranch/Commit: **[eb76215](https://github.com/sagemath/sagetrac-mirror/commit/eb7621560d25bf7c62c11e692f402c5966175d34)**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32044_\n\n",
    "closed_at": "2021-07-06T21:29:18Z",
    "created_at": "2021-06-23T17:33:38Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20experimental",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "build/pkgs/compilerwrapper: Remove",
    "type": "issue",
    "updated_at": "2021-07-06T21:29:18Z",
    "url": "https://github.com/sagemath/sage/issues/32044",
    "user": "https://github.com/mkoeppe"
}
```
Looking at https://bitbucket.org/vbraun/compilerwrapper/src/master/ (no changes since 2013), it is likely that our `compilerwrapper` package (marked "experimental") is not needed any more.

This ticket removes it.


CC:  @vbraun @dimpase

Branch/Commit: **[eb76215](https://github.com/sagemath/sagetrac-mirror/commit/eb7621560d25bf7c62c11e692f402c5966175d34)**

Reviewer: **Dima Pasechnik**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/32044_





---

archive/issue_events_423707.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-06-23T17:33:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32044",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32044#event-423707"
}
```



---

archive/issue_events_423708.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-06-23T17:33:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32044",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20experimental",
    "label_color": "000080",
    "label_name": "component: packages: experimental",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32044#event-423708"
}
```



---

archive/issue_events_423709.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-06-23T17:33:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32044",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32044#event-423709"
}
```



---

archive/issue_events_423710.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-06-23T17:33:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32044",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32044#event-423710"
}
```



---

archive/issue_comments_519042.json:
```json
{
    "body": "Branch: **[u/mkoeppe/build_pkgs_compilerwrapper__remove](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/build_pkgs_compilerwrapper__remove)**",
    "created_at": "2021-06-23T17:34:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32044",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32044#issuecomment-519042",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/build_pkgs_compilerwrapper__remove](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/build_pkgs_compilerwrapper__remove)**



---

archive/issue_events_423711.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-06-23T17:34:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32044",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32044#event-423711"
}
```



---

archive/issue_comments_519043.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/eb7621560d25bf7c62c11e692f402c5966175d34\">eb76215</a></td><td><code>build/pkgs/compilerwrapper: Remove</code></td></tr></table>\n",
    "created_at": "2021-06-23T17:34:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32044",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32044#issuecomment-519043",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/eb7621560d25bf7c62c11e692f402c5966175d34">eb76215</a></td><td><code>build/pkgs/compilerwrapper: Remove</code></td></tr></table>




---

archive/issue_comments_519044.json:
```json
{
    "body": "Commit: **[eb76215](https://github.com/sagemath/sagetrac-mirror/commit/eb7621560d25bf7c62c11e692f402c5966175d34)**",
    "created_at": "2021-06-23T17:34:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32044",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32044#issuecomment-519044",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[eb76215](https://github.com/sagemath/sagetrac-mirror/commit/eb7621560d25bf7c62c11e692f402c5966175d34)**



---

archive/issue_comments_519045.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nOK",
    "created_at": "2021-06-24T19:53:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32044",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32044#issuecomment-519045",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:3" align="right">Comment 3</div>

OK



---

archive/issue_events_423712.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-06-24T19:53:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32044",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32044#event-423712"
}
```



---

archive/issue_events_423713.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-06-24T19:53:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32044",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32044#event-423713"
}
```



---

archive/issue_comments_519046.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2021-06-24T19:53:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32044",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32044#issuecomment-519046",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_comments_519047.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThanks. Volker, any objections?",
    "created_at": "2021-06-24T20:06:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32044",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32044#issuecomment-519047",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:4" align="right">Comment 4</div>

Thanks. Volker, any objections?



---

archive/issue_events_423714.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-07-06T21:29:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32044",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32044#event-423714"
}
```



---

archive/issue_events_423715.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "1a06c436b58dd982ab41596b14b970a461519e55",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-07-06T21:29:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32044",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32044#event-423715"
}
```



---

archive/issue_comments_519048.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/build_pkgs_compilerwrapper__remove](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/build_pkgs_compilerwrapper__remove)** to **[eb76215](https://github.com/sagemath/sagetrac-mirror/commit/eb7621560d25bf7c62c11e692f402c5966175d34)**",
    "created_at": "2021-07-06T21:29:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32044",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32044#issuecomment-519048",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/build_pkgs_compilerwrapper__remove](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/build_pkgs_compilerwrapper__remove)** to **[eb76215](https://github.com/sagemath/sagetrac-mirror/commit/eb7621560d25bf7c62c11e692f402c5966175d34)**
