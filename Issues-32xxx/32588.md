# Issue 32588: giac 1.6.0.47p3 does not pass self-tests

archive/issues_032351.json:
```json
{
    "assignees": [],
    "body": "giac 1.6.0.47p3 does not pass self-tests `chk_fhan2` and `chk_fhan9` on both 32bit and 64bit VM, see the various logs in attachment.\n\n\nDepends on #31563\n\nCC:  @frederichan-IMJPRG @sagetrac-parisse\n\nKeywords: **sdl**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32588_\n\n",
    "created_at": "2021-09-29T22:43:34Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/invalid",
        "https://github.com/sagemath/sage/labels/needs%20review"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "giac 1.6.0.47p3 does not pass self-tests",
    "type": "issue",
    "updated_at": "2022-02-06T20:23:15Z",
    "url": "https://github.com/sagemath/sage/issues/32588",
    "user": "https://github.com/sagetrac-tmonteil"
}
```
giac 1.6.0.47p3 does not pass self-tests `chk_fhan2` and `chk_fhan9` on both 32bit and 64bit VM, see the various logs in attachment.


Depends on #31563

CC:  @frederichan-IMJPRG @sagetrac-parisse

Keywords: **sdl**

_Issue created by migration from https://trac.sagemath.org/ticket/32588_





---

archive/issue_events_430777.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2021-09-29T22:43:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32588#event-430777"
}
```



---

archive/issue_events_430778.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2021-09-29T22:43:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32588#event-430778"
}
```



---

archive/issue_events_430779.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2021-09-29T22:43:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32588#event-430779"
}
```



---

archive/issue_events_430780.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2021-09-29T22:43:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32588#event-430780"
}
```



---

archive/issue_comments_528468.json:
```json
{
    "body": "Attachment: **[giac-1.6.0.47p3.log](https://github.com/sagemath/sage/files/ticket32588/giac-1.6.0.47p3.log)**",
    "created_at": "2021-09-29T22:44:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32588#issuecomment-528468",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Attachment: **[giac-1.6.0.47p3.log](https://github.com/sagemath/sage/files/ticket32588/giac-1.6.0.47p3.log)**



---

archive/issue_comments_528469.json:
```json
{
    "body": "Attachment: **[chk_fhan2.log](https://github.com/sagemath/sage/files/ticket32588/chk_fhan2.log)**",
    "created_at": "2021-09-29T22:44:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32588#issuecomment-528469",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Attachment: **[chk_fhan2.log](https://github.com/sagemath/sage/files/ticket32588/chk_fhan2.log)**



---

archive/issue_comments_528470.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nAttachment: **[chk_fhan9.log](https://github.com/sagemath/sage/files/ticket32588/chk_fhan9.log)**",
    "created_at": "2021-09-29T22:46:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32588#issuecomment-528470",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

<div id="comment:1" align="right">Comment 1</div>

Attachment: **[chk_fhan9.log](https://github.com/sagemath/sage/files/ticket32588/chk_fhan9.log)**



---

archive/issue_comments_528471.json:
```json
{
    "body": "Changed keywords from none to **sdl**",
    "created_at": "2021-09-29T23:53:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32588#issuecomment-528471",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Changed keywords from none to **sdl**



---

archive/issue_events_430781.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T20:12:32Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32588#event-430781"
}
```



---

archive/issue_events_430782.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T20:12:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32588#event-430782"
}
```



---

archive/issue_comments_528472.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,2 +1,2 @@\n-giac 1.6.0.47p3 does not pass self-tests `chk_fhan2` and `chk_fhan9` on my 32bit VM, see the various logs in attachment.\n+giac 1.6.0.47p3 does not pass self-tests `chk_fhan2` and `chk_fhan9` on both 32bit and 64bit VM, see the various logs in attachment.\n \n``````\n",
    "created_at": "2022-01-02T09:56:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32588#issuecomment-528472",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,2 +1,2 @@
-giac 1.6.0.47p3 does not pass self-tests `chk_fhan2` and `chk_fhan9` on my 32bit VM, see the various logs in attachment.
+giac 1.6.0.47p3 does not pass self-tests `chk_fhan2` and `chk_fhan9` on both 32bit and 64bit VM, see the various logs in attachment.
 
``````




---

archive/issue_events_430783.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2022-01-02T09:56:18Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "title_is": "giac 1.6.0.47p3 does not pass self-tests",
    "title_was": "giac 1.6.0.47p3 does not pass self-tests on 32bit systems",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32588#event-430783"
}
```



---

archive/issue_comments_528473.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nHave you tried the pending upgrade to giac 1.7.x? #31563",
    "created_at": "2022-01-02T20:48:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32588#issuecomment-528473",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:5" align="right">Comment 5</div>

Have you tried the pending upgrade to giac 1.7.x? #31563



---

archive/issue_events_430784.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-02-06T20:23:15Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32588#event-430784"
}
```



---

archive/issue_events_430785.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-02-06T20:23:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32588#event-430785"
}
```



---

archive/issue_events_430786.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-02-06T20:23:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32588#event-430786"
}
```



---

archive/issue_comments_528474.json:
```json
{
    "body": "Dependencies: **#31563**",
    "created_at": "2022-02-06T20:23:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32588#issuecomment-528474",
    "user": "https://github.com/mkoeppe"
}
```

Dependencies: **#31563**



---

archive/issue_events_430787.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-02-06T20:23:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32588",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32588#event-430787"
}
```
