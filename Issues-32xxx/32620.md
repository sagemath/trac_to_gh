# Issue 32620: Replace imports of QQ, ZZ, GF, Integer from sage.rings.all by more specific imports

archive/issues_032383.json:
```json
{
    "assignees": [],
    "body": "(cherry-picked from #32432)\n\n\nBranch/Commit: **[d9dd2d0](https://github.com/sagemath/sagetrac-mirror/commit/d9dd2d008821e19886c20cd3e2270c5bff3c5473)**\n\nReviewer: **Jonathan Kliem**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32620_\n\n",
    "closed_at": "2021-10-13T22:50:01Z",
    "created_at": "2021-10-04T04:55:43Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/refactoring",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Replace imports of QQ, ZZ, GF, Integer from sage.rings.all by more specific imports",
    "type": "issue",
    "updated_at": "2021-10-13T22:50:01Z",
    "url": "https://github.com/sagemath/sage/issues/32620",
    "user": "https://github.com/mkoeppe"
}
```
(cherry-picked from #32432)


Branch/Commit: **[d9dd2d0](https://github.com/sagemath/sagetrac-mirror/commit/d9dd2d008821e19886c20cd3e2270c5bff3c5473)**

Reviewer: **Jonathan Kliem**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/32620_





---

archive/issue_events_431155.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T04:55:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32620",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32620#event-431155"
}
```



---

archive/issue_events_431156.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T04:55:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32620",
    "label": "https://github.com/sagemath/sage/labels/refactoring",
    "label_color": "696969",
    "label_name": "refactoring",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32620#event-431156"
}
```



---

archive/issue_events_431157.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T04:55:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32620",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32620#event-431157"
}
```



---

archive/issue_events_431158.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T04:55:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32620",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32620#event-431158"
}
```



---

archive/issue_events_431159.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T04:57:40Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/32620",
    "title_is": "Replace imports of QQ, ZZ, GF, Integer from sage.rings.all by more specific imports",
    "title_was": "Remove imports of QQ, ZZ, GF, Integer from sage.rings.alll",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32620#event-431159"
}
```



---

archive/issue_comments_529135.json:
```json
{
    "body": "Branch: **[u/mkoeppe/replace_imports_of_qq__zz__gf__integer_from_sage_rings_all_by_more_specific_imports](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/replace_imports_of_qq__zz__gf__integer_from_sage_rings_all_by_more_specific_imports)**",
    "created_at": "2021-10-04T05:00:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32620",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32620#issuecomment-529135",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/replace_imports_of_qq__zz__gf__integer_from_sage_rings_all_by_more_specific_imports](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/replace_imports_of_qq__zz__gf__integer_from_sage_rings_all_by_more_specific_imports)**



---

archive/issue_comments_529136.json:
```json
{
    "body": "Commit: **[d9dd2d0](https://github.com/sagemath/sagetrac-mirror/commit/d9dd2d008821e19886c20cd3e2270c5bff3c5473)**",
    "created_at": "2021-10-04T05:00:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32620",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32620#issuecomment-529136",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[d9dd2d0](https://github.com/sagemath/sagetrac-mirror/commit/d9dd2d008821e19886c20cd3e2270c5bff3c5473)**



---

archive/issue_comments_529137.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f7e07bf5cdebaef88504d15169c45d7b8a43c285\">f7e07bf</a></td><td><code>git grep -l 'all import ZZ' src/sage | xargs sed -E -i.bak $'s/^( *)from sage.*all import (ZZ, QQ|QQ, ZZ) *$/\\1from sage.rings.integer_ring import ZZ\\\\n\\1from sage.rings.rational_field import QQ/'</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/acf7ae99825035ec3b5f0262c8d1c5c5124fdde0\">acf7ae9</a></td><td><code>git grep -l 'all import ZZ' src/sage | xargs sed -E -i.bak $'s/^( *)from sage.*all import ZZ *$/\\1from sage.rings.integer_ring import ZZ/'</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/31e17a41bf21118dfba1ea3a67667ada48bd078d\">31e17a4</a></td><td><code>git grep -l 'all import .*ZZ' src/sage | xargs sed -E -i.bak $'s/^( *)from sage.*all import (ZZ, GF) *$/\\1from sage.rings.integer_ring import ZZ\\\\n\\1from sage.rings.finite_rings.finite_field_constructor import GF/'</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d9dd2d008821e19886c20cd3e2270c5bff3c5473\">d9dd2d0</a></td><td><code>git grep -l 'all import.*ZZ' src/sage | xargs sed -E -i.bak $'s/^( *)from sage.*all import (ZZ, QQ|QQ, ZZ) *$/\\1from sage.rings.integer_ring import ZZ\\\\n\\1from sage.rings.rational_field import QQ/'</code></td></tr></table>\n",
    "created_at": "2021-10-04T05:00:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32620",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32620#issuecomment-529137",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:3" align="right">Comment 3</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f7e07bf5cdebaef88504d15169c45d7b8a43c285">f7e07bf</a></td><td><code>git grep -l 'all import ZZ' src/sage | xargs sed -E -i.bak $'s/^( *)from sage.*all import (ZZ, QQ|QQ, ZZ) *$/\1from sage.rings.integer_ring import ZZ\\n\1from sage.rings.rational_field import QQ/'</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/acf7ae99825035ec3b5f0262c8d1c5c5124fdde0">acf7ae9</a></td><td><code>git grep -l 'all import ZZ' src/sage | xargs sed -E -i.bak $'s/^( *)from sage.*all import ZZ *$/\1from sage.rings.integer_ring import ZZ/'</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/31e17a41bf21118dfba1ea3a67667ada48bd078d">31e17a4</a></td><td><code>git grep -l 'all import .*ZZ' src/sage | xargs sed -E -i.bak $'s/^( *)from sage.*all import (ZZ, GF) *$/\1from sage.rings.integer_ring import ZZ\\n\1from sage.rings.finite_rings.finite_field_constructor import GF/'</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d9dd2d008821e19886c20cd3e2270c5bff3c5473">d9dd2d0</a></td><td><code>git grep -l 'all import.*ZZ' src/sage | xargs sed -E -i.bak $'s/^( *)from sage.*all import (ZZ, QQ|QQ, ZZ) *$/\1from sage.rings.integer_ring import ZZ\\n\1from sage.rings.rational_field import QQ/'</code></td></tr></table>




---

archive/issue_comments_529138.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2021-10-04T05:00:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32620",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32620#issuecomment-529138",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_comments_529139.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,2 @@\n+(cherry-picked from #32432)\n \n``````\n",
    "created_at": "2021-10-04T05:00:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32620",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32620#issuecomment-529139",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,2 @@
+(cherry-picked from #32432)
 
``````




---

archive/issue_events_431160.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T05:01:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32620",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32620#event-431160"
}
```



---

archive/issue_comments_529140.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nLGTM. Green bot -> Positive Review.",
    "created_at": "2021-10-04T06:27:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32620",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32620#issuecomment-529140",
    "user": "https://github.com/kliem"
}
```

<div id="comment:5" align="right">Comment 5</div>

LGTM. Green bot -> Positive Review.



---

archive/issue_comments_529141.json:
```json
{
    "body": "Reviewer: **Jonathan Kliem**",
    "created_at": "2021-10-04T06:27:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32620",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32620#issuecomment-529141",
    "user": "https://github.com/kliem"
}
```

Reviewer: **Jonathan Kliem**



---

archive/issue_events_431161.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T15:46:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32620",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32620#event-431161"
}
```



---

archive/issue_events_431162.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T15:46:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32620",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32620#event-431162"
}
```



---

archive/issue_comments_529142.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nThank you!",
    "created_at": "2021-10-04T15:46:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32620",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32620#issuecomment-529142",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:6" align="right">Comment 6</div>

Thank you!



---

archive/issue_events_431163.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-10-13T22:50:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32620",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32620#event-431163"
}
```



---

archive/issue_events_431164.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "e78d69d71e1502d8c28f5a06da4788d314bfb3fb",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-10-13T22:50:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32620",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32620#event-431164"
}
```



---

archive/issue_comments_529143.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/replace_imports_of_qq__zz__gf__integer_from_sage_rings_all_by_more_specific_imports](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/replace_imports_of_qq__zz__gf__integer_from_sage_rings_all_by_more_specific_imports)** to **[d9dd2d0](https://github.com/sagemath/sagetrac-mirror/commit/d9dd2d008821e19886c20cd3e2270c5bff3c5473)**",
    "created_at": "2021-10-13T22:50:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32620",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32620#issuecomment-529143",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/replace_imports_of_qq__zz__gf__integer_from_sage_rings_all_by_more_specific_imports](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/replace_imports_of_qq__zz__gf__integer_from_sage_rings_all_by_more_specific_imports)** to **[d9dd2d0](https://github.com/sagemath/sagetrac-mirror/commit/d9dd2d008821e19886c20cd3e2270c5bff3c5473)**
