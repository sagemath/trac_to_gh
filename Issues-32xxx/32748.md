# Issue 32748: clean the processing of networkx (Multi)(Di)Graph in the (Di)Graph constructor

archive/issues_032511.json:
```json
{
    "assignees": [],
    "body": "We have tests to distinguish between networkx `Graph` and `MultiGraph` which are silently failing since for the following reason:\n\n```\nsage: H = networkx.MultiGraph()\nsage: H.add_edge(0, 1)\n0\nsage: H.add_edge(0, 1)\n1\nsage: isinstance(H, networkx.MultiGraph)\nTrue\nsage: isinstance(H, networkx.Graph)\nTrue\n```\nand so some code that should lead to an error is unreachable. In particular, the `DiGraph` constructor expect to call `data.selfloops` when data is a networkx `MultiDiGraph`, but the test is `is instance(data, networkx.DiGraph)` which is always true. Observe that method `selfloops` is no longer in networkx.\n\nHere, we add a specific helper method to feed a (di)graph with data from a networkx (Multi)(Di)Graph. \n\nThis will be useful for #9731.\n\nCC:  @fchapoton @tscrim @sagetrac-tmonteil\n\nBranch/Commit: **[0146309](https://github.com/sagemath/sagetrac-mirror/commit/01463095386995d8bbe2c487b19f67a4b70d535d)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32748_\n\n",
    "closed_at": "2021-10-31T22:20:55Z",
    "created_at": "2021-10-23T16:59:39Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "clean the processing of networkx (Multi)(Di)Graph in the (Di)Graph constructor",
    "type": "issue",
    "updated_at": "2021-10-31T22:20:55Z",
    "url": "https://github.com/sagemath/sage/issues/32748",
    "user": "https://github.com/dcoudert"
}
```
We have tests to distinguish between networkx `Graph` and `MultiGraph` which are silently failing since for the following reason:

```
sage: H = networkx.MultiGraph()
sage: H.add_edge(0, 1)
0
sage: H.add_edge(0, 1)
1
sage: isinstance(H, networkx.MultiGraph)
True
sage: isinstance(H, networkx.Graph)
True
```
and so some code that should lead to an error is unreachable. In particular, the `DiGraph` constructor expect to call `data.selfloops` when data is a networkx `MultiDiGraph`, but the test is `is instance(data, networkx.DiGraph)` which is always true. Observe that method `selfloops` is no longer in networkx.

Here, we add a specific helper method to feed a (di)graph with data from a networkx (Multi)(Di)Graph. 

This will be useful for #9731.

CC:  @fchapoton @tscrim @sagetrac-tmonteil

Branch/Commit: **[0146309](https://github.com/sagemath/sagetrac-mirror/commit/01463095386995d8bbe2c487b19f67a4b70d535d)**

Reviewer: **Travis Scrimshaw**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/32748_





---

archive/issue_events_432721.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2021-10-23T16:59:39Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32748",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32748#event-432721"
}
```



---

archive/issue_events_432722.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2021-10-23T16:59:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32748",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20game%20theory",
    "label_color": "0000ff",
    "label_name": "component: game theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32748#event-432722"
}
```



---

archive/issue_events_432723.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2021-10-23T16:59:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32748",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32748#event-432723"
}
```



---

archive/issue_events_432724.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2021-10-23T16:59:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32748",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32748#event-432724"
}
```



---

archive/issue_comments_531381.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -14,3 +14,5 @@\n and so some code that should lead to an error is unreachable. In particular, the `DiGraph` constructor expect to call `data.selfloops` when data is a networkx `MultiDiGraph`, but the test is `is instance(data, networkx.DiGraph)` which is always true. Observe that method `selfloops` is no longer in networkx.\n \n Here, we add a specific helper method to feed a (di)graph with data from a networkx (Multi)(Di)Graph. \n+\n+This will be useful for #9731.\n``````\n",
    "created_at": "2021-10-23T17:01:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32748",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32748#issuecomment-531381",
    "user": "https://github.com/dcoudert"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -14,3 +14,5 @@
 and so some code that should lead to an error is unreachable. In particular, the `DiGraph` constructor expect to call `data.selfloops` when data is a networkx `MultiDiGraph`, but the test is `is instance(data, networkx.DiGraph)` which is always true. Observe that method `selfloops` is no longer in networkx.
 
 Here, we add a specific helper method to feed a (di)graph with data from a networkx (Multi)(Di)Graph. 
+
+This will be useful for #9731.
``````




---

archive/issue_comments_531382.json:
```json
{
    "body": "Branch: **[public/graphs/32748_networkx](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/32748_networkx)**",
    "created_at": "2021-10-23T17:01:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32748",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32748#issuecomment-531382",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[public/graphs/32748_networkx](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/32748_networkx)**



---

archive/issue_events_432725.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2021-10-23T17:01:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32748",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32748#event-432725"
}
```



---

archive/issue_comments_531383.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/271f765fc84fb1e61182d4bd0afbacd66a2d0084\">271f765</a></td><td><code>trac #32748: better handling of networkx graphs in (di)graph constructor</code></td></tr></table>\n",
    "created_at": "2021-10-23T17:01:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32748",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32748#issuecomment-531383",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/271f765fc84fb1e61182d4bd0afbacd66a2d0084">271f765</a></td><td><code>trac #32748: better handling of networkx graphs in (di)graph constructor</code></td></tr></table>




---

archive/issue_comments_531384.json:
```json
{
    "body": "Commit: **[271f765](https://github.com/sagemath/sagetrac-mirror/commit/271f765fc84fb1e61182d4bd0afbacd66a2d0084)**",
    "created_at": "2021-10-23T17:01:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32748",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32748#issuecomment-531384",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[271f765](https://github.com/sagemath/sagetrac-mirror/commit/271f765fc84fb1e61182d4bd0afbacd66a2d0084)**



---

archive/issue_events_432726.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2021-10-23T18:12:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32748",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20game%20theory",
    "label_color": "0000ff",
    "label_name": "component: game theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32748#event-432726"
}
```



---

archive/issue_events_432727.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2021-10-23T18:12:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32748",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32748#event-432727"
}
```



---

archive/issue_comments_531385.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2021-10-29T00:10:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32748",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32748#issuecomment-531385",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_531386.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLGTM modulo the trivial extra space on the second line of some of the `INPUT:` in `from_networkx_graph`.",
    "created_at": "2021-10-29T00:10:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32748",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32748#issuecomment-531386",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:3" align="right">Comment 3</div>

LGTM modulo the trivial extra space on the second line of some of the `INPUT:` in `from_networkx_graph`.



---

archive/issue_comments_531387.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/01463095386995d8bbe2c487b19f67a4b70d535d\">0146309</a></td><td><code>trac #32748: remove extra spaces</code></td></tr></table>\n",
    "created_at": "2021-10-29T06:38:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32748",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32748#issuecomment-531387",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/01463095386995d8bbe2c487b19f67a4b70d535d">0146309</a></td><td><code>trac #32748: remove extra spaces</code></td></tr></table>




---

archive/issue_comments_531388.json:
```json
{
    "body": "Changed commit from **[271f765](https://github.com/sagemath/sagetrac-mirror/commit/271f765fc84fb1e61182d4bd0afbacd66a2d0084)** to **[0146309](https://github.com/sagemath/sagetrac-mirror/commit/01463095386995d8bbe2c487b19f67a4b70d535d)**",
    "created_at": "2021-10-29T06:38:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32748",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32748#issuecomment-531388",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[271f765](https://github.com/sagemath/sagetrac-mirror/commit/271f765fc84fb1e61182d4bd0afbacd66a2d0084)** to **[0146309](https://github.com/sagemath/sagetrac-mirror/commit/01463095386995d8bbe2c487b19f67a4b70d535d)**



---

archive/issue_comments_531389.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nGood catch. Thanks.",
    "created_at": "2021-10-29T06:38:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32748",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32748#issuecomment-531389",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:5" align="right">Comment 5</div>

Good catch. Thanks.



---

archive/issue_comments_531390.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nThank you.",
    "created_at": "2021-10-29T06:51:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32748",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32748#issuecomment-531390",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:6" align="right">Comment 6</div>

Thank you.



---

archive/issue_events_432728.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-10-29T06:51:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32748",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32748#event-432728"
}
```



---

archive/issue_events_432729.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-10-29T06:51:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32748",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32748#event-432729"
}
```



---

archive/issue_comments_531391.json:
```json
{
    "body": "Changed branch from **[public/graphs/32748_networkx](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/32748_networkx)** to **[0146309](https://github.com/sagemath/sagetrac-mirror/commit/01463095386995d8bbe2c487b19f67a4b70d535d)**",
    "created_at": "2021-10-31T22:20:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32748",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32748#issuecomment-531391",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/graphs/32748_networkx](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/32748_networkx)** to **[0146309](https://github.com/sagemath/sagetrac-mirror/commit/01463095386995d8bbe2c487b19f67a4b70d535d)**



---

archive/issue_events_432730.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-10-31T22:20:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32748",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32748#event-432730"
}
```



---

archive/issue_events_432731.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "37d880bc20217ed48f4783555f46dff3d845fee8",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-10-31T22:20:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32748",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32748#event-432731"
}
```
