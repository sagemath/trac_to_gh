# Issue 32032: DefaultTermMonoidFactory should be used in doctests in term_monoid

archive/issues_031795.json:
```json
{
    "assignees": [],
    "body": "The setup for getting a `TermMonoidFactory` in doctests in `sage.rings.asymptotic.term_monoid` currently looks like\n\n```\nsage: from sage.rings.asymptotic.term_monoid import TermMonoidFactory\nsage: TermMonoid = TermMonoidFactory('__main__.TermMonoid')\n```\n\nwhen in fact the already available `DefaultTermMonoidFactory` could be used via, e.g.,\n\n```\nsage: from sage.rings.asymptotic.term_monoid import DefaultTermMonoidFactory as TermMonoid\n```\n\nOn top of that, the note on lines 3985--3987 in `term_monoid.py` should be adapted accordingly (currently it is not true).\n\nCC:  @dkrenn @cheuberg @thhagelmayer\n\nBranch/Commit: **[a3bad87](https://github.com/sagemath/sagetrac-mirror/commit/a3bad87abf0a70f85adbcba9b88244b0393a82ad)**\n\nReviewer: **Benjamin Hackl**\n\nAuthor: **Daniel Krenn**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32032_\n\n",
    "closed_at": "2021-07-25T13:25:30Z",
    "created_at": "2021-06-22T08:58:35Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20asymptotic%20expansions",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "DefaultTermMonoidFactory should be used in doctests in term_monoid",
    "type": "issue",
    "updated_at": "2021-07-25T15:23:36Z",
    "url": "https://github.com/sagemath/sage/issues/32032",
    "user": "https://github.com/behackl"
}
```
The setup for getting a `TermMonoidFactory` in doctests in `sage.rings.asymptotic.term_monoid` currently looks like

```
sage: from sage.rings.asymptotic.term_monoid import TermMonoidFactory
sage: TermMonoid = TermMonoidFactory('__main__.TermMonoid')
```

when in fact the already available `DefaultTermMonoidFactory` could be used via, e.g.,

```
sage: from sage.rings.asymptotic.term_monoid import DefaultTermMonoidFactory as TermMonoid
```

On top of that, the note on lines 3985--3987 in `term_monoid.py` should be adapted accordingly (currently it is not true).

CC:  @dkrenn @cheuberg @thhagelmayer

Branch/Commit: **[a3bad87](https://github.com/sagemath/sagetrac-mirror/commit/a3bad87abf0a70f85adbcba9b88244b0393a82ad)**

Reviewer: **Benjamin Hackl**

Author: **Daniel Krenn**

_Issue created by migration from https://trac.sagemath.org/ticket/32032_





---

archive/issue_events_423512.json:
```json
{
    "actor": "https://github.com/behackl",
    "created_at": "2021-06-22T08:58:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32032",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32032#event-423512"
}
```



---

archive/issue_events_423513.json:
```json
{
    "actor": "https://github.com/behackl",
    "created_at": "2021-06-22T08:58:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32032",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20asymptotic%20expansions",
    "label_color": "0000ff",
    "label_name": "component: asymptotic expansions",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32032#event-423513"
}
```



---

archive/issue_events_423514.json:
```json
{
    "actor": "https://github.com/behackl",
    "created_at": "2021-06-22T08:58:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32032",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32032#event-423514"
}
```



---

archive/issue_events_423515.json:
```json
{
    "actor": "https://github.com/behackl",
    "created_at": "2021-06-22T08:58:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32032",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32032#event-423515"
}
```



---

archive/issue_events_423516.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:16:42Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/32032",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32032#event-423516"
}
```



---

archive/issue_events_423517.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:16:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32032",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32032#event-423517"
}
```



---

archive/issue_comments_518675.json:
```json
{
    "body": "Branch: **[u/dkrenn/cleanup-default-termmonoid-factory](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/cleanup-default-termmonoid-factory)**",
    "created_at": "2021-07-19T08:41:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32032",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32032#issuecomment-518675",
    "user": "https://github.com/dkrenn"
}
```

Branch: **[u/dkrenn/cleanup-default-termmonoid-factory](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/cleanup-default-termmonoid-factory)**



---

archive/issue_comments_518676.json:
```json
{
    "body": "Commit: **[a3bad87](https://github.com/sagemath/sagetrac-mirror/commit/a3bad87abf0a70f85adbcba9b88244b0393a82ad)**",
    "created_at": "2021-07-19T08:41:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32032",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32032#issuecomment-518676",
    "user": "https://github.com/dkrenn"
}
```

Commit: **[a3bad87](https://github.com/sagemath/sagetrac-mirror/commit/a3bad87abf0a70f85adbcba9b88244b0393a82ad)**



---

archive/issue_comments_518677.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fdfd5bed435744e485491ab6f4af8c2ce239780f\">fdfd5be</a></td><td><code>Trac #32032: use DefaultTermMonoidFactory in doctests</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a3bad87abf0a70f85adbcba9b88244b0393a82ad\">a3bad87</a></td><td><code>Trac #32032: fix docs: instance available as \"DefaultTermMonoidFactory\"</code></td></tr></table>\n",
    "created_at": "2021-07-19T08:41:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32032",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32032#issuecomment-518677",
    "user": "https://github.com/dkrenn"
}
```

<div id="comment:3" align="right">Comment 3</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fdfd5bed435744e485491ab6f4af8c2ce239780f">fdfd5be</a></td><td><code>Trac #32032: use DefaultTermMonoidFactory in doctests</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a3bad87abf0a70f85adbcba9b88244b0393a82ad">a3bad87</a></td><td><code>Trac #32032: fix docs: instance available as "DefaultTermMonoidFactory"</code></td></tr></table>




---

archive/issue_comments_518678.json:
```json
{
    "body": "Author: **Daniel Krenn**",
    "created_at": "2021-07-19T08:41:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32032",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32032#issuecomment-518678",
    "user": "https://github.com/dkrenn"
}
```

Author: **Daniel Krenn**



---

archive/issue_events_423518.json:
```json
{
    "actor": "https://github.com/dkrenn",
    "created_at": "2021-07-19T08:41:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32032",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32032#event-423518"
}
```



---

archive/issue_comments_518679.json:
```json
{
    "body": "Reviewer: **Benjamin Hackl**",
    "created_at": "2021-07-22T11:23:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32032",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32032#issuecomment-518679",
    "user": "https://github.com/behackl"
}
```

Reviewer: **Benjamin Hackl**



---

archive/issue_events_423519.json:
```json
{
    "actor": "https://github.com/behackl",
    "created_at": "2021-07-22T11:23:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32032",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32032#event-423519"
}
```



---

archive/issue_events_423520.json:
```json
{
    "actor": "https://github.com/behackl",
    "created_at": "2021-07-22T11:23:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32032",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32032#event-423520"
}
```



---

archive/issue_comments_518680.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nLooks good to me, thanks for changing the doctests and the reference to `TermMonoid`!",
    "created_at": "2021-07-22T11:23:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32032",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32032#issuecomment-518680",
    "user": "https://github.com/behackl"
}
```

<div id="comment:4" align="right">Comment 4</div>

Looks good to me, thanks for changing the doctests and the reference to `TermMonoid`!



---

archive/issue_comments_518681.json:
```json
{
    "body": "Changed branch from **[u/dkrenn/cleanup-default-termmonoid-factory](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/cleanup-default-termmonoid-factory)** to **[a3bad87](https://github.com/sagemath/sagetrac-mirror/commit/a3bad87abf0a70f85adbcba9b88244b0393a82ad)**",
    "created_at": "2021-07-25T13:25:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32032",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32032#issuecomment-518681",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/dkrenn/cleanup-default-termmonoid-factory](https://github.com/sagemath/sagetrac-mirror/tree/u/dkrenn/cleanup-default-termmonoid-factory)** to **[a3bad87](https://github.com/sagemath/sagetrac-mirror/commit/a3bad87abf0a70f85adbcba9b88244b0393a82ad)**



---

archive/issue_events_423521.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-07-25T13:25:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32032",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32032#event-423521"
}
```



---

archive/issue_events_423522.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "d8f408c088fb54e97adf3cb0064f8bef6246c992",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-07-25T13:25:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32032",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32032#event-423522"
}
```



---

archive/issue_events_423523.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-25T15:23:36Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/32032",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32032#event-423523"
}
```



---

archive/issue_events_423524.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-25T15:23:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32032",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32032#event-423524"
}
```
