...
```json
{
    "body": "I've gone all the way to 10 minutes. With `make ptest` on my desktop (AMD Phenom II X4), tensorfield.py already approaches eight minutes (~460s). If I watch a movie or browse the web simultaneously it gets worse.\n\nI still plan to check the timings on my laptop, probably tonight, to make sure that 10 is enough.\n\n---\nNew commits:",
...
```json
{
    "body": "The laptop (Core II duo `@` 1.8ghz) actually fares better than my desktop, but with fewer threads (2 vs 4). So ten minutes looks like a good number.",
...
```json
{
    "body": "I've switched `--warn-long` to CPU time in #32981. That should nicely fix half of the mess, leaving only the question of how to use `second_on_modern_computer()` to adjust the timeout.",
