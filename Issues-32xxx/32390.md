# Issue 32390: Make "./configure --enable-download-from-upstream-url" the default

archive/issues_032153.json:
```json
{
    "body": "CC:  @kliem @jhpalmieri\n\nThis has been available since 9.3 and may be ready for general consumption\n\n\nSee also: #32406 Make `./configure --enable-editable` the default\n\nIssue created by migration from https://trac.sagemath.org/ticket/32390\n\n",
    "closed_at": "2022-12-14T22:12:34Z",
    "created_at": "2021-08-17T18:00:39Z",
    "labels": [
        "component: build"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.8",
    "title": "Make \"./configure --enable-download-from-upstream-url\" the default",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/32390",
    "user": "https://github.com/mkoeppe"
}
```
CC:  @kliem @jhpalmieri

This has been available since 9.3 and may be ready for general consumption


See also: #32406 Make `./configure --enable-editable` the default

Issue created by migration from https://trac.sagemath.org/ticket/32390





---

archive/issue_comments_458747.json:
```json
{
    "body": "I'd also make \"enable-dowload-from-upstream-url\" default",
    "created_at": "2021-08-17T20:20:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32390#issuecomment-458747",
    "user": "https://github.com/dimpase"
}
```

I'd also make "enable-dowload-from-upstream-url" default



---

archive/issue_comments_458748.json:
```json
{
    "body": "Replying to [comment:1 dimpase]:\n> I'd also make \"enable-dowload-from-upstream-url\" default\n\n\n-1\n\nJust downloading something from some url and installling it is something, I would want to be notified about. I think it is opt-in and not opt-out.\n\n(I opted in, but I still think this is a choice, a user has to make.)",
    "created_at": "2021-08-17T20:24:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32390#issuecomment-458748",
    "user": "https://github.com/kliem"
}
```

Replying to [comment:1 dimpase]:
> I'd also make "enable-dowload-from-upstream-url" default


-1

Just downloading something from some url and installling it is something, I would want to be notified about. I think it is opt-in and not opt-out.

(I opted in, but I still think this is a choice, a user has to make.)



---

archive/issue_comments_458749.json:
```json
{
    "body": "Replying to [comment:2 gh-kliem]:\n> Replying to [comment:1 dimpase]:\n> > I'd also make \"enable-dowload-from-upstream-url\" default\n\n> \n> -1\n> \n> Just downloading something from some url and installling it is something, I would want to be notified about. I think it is opt-in and not opt-out.\n\n\nit's checking the checksums.\nWhy would you trust Sage mirrors more - do you think that \nour checksums are not secure?\n\n\n> \n> (I opted in, but I still think this is a choice, a user has to make.)",
    "created_at": "2021-08-21T20:05:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32390#issuecomment-458749",
    "user": "https://github.com/dimpase"
}
```

Replying to [comment:2 gh-kliem]:
> Replying to [comment:1 dimpase]:
> > I'd also make "enable-dowload-from-upstream-url" default

> 
> -1
> 
> Just downloading something from some url and installling it is something, I would want to be notified about. I think it is opt-in and not opt-out.


it's checking the checksums.
Why would you trust Sage mirrors more - do you think that 
our checksums are not secure?


> 
> (I opted in, but I still think this is a choice, a user has to make.)



---

archive/issue_comments_458750.json:
```json
{
    "body": "I have rededicated this ticket to the topic of discussion taking place in the comments.",
    "created_at": "2021-08-22T00:31:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32390#issuecomment-458750",
    "user": "https://github.com/mkoeppe"
}
```

I have rededicated this ticket to the topic of discussion taking place in the comments.



---

archive/issue_events_085945.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:41:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/32390",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/32390#event-85945"
}
```



---

archive/issue_comments_458751.json:
```json
{
    "body": "As a compromise, we could also do a yes/no prompt when `--enable-dowload-from-upstream-url` has not been used; just like we do when experimental packages are to be installed",
    "created_at": "2022-01-23T02:52:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32390#issuecomment-458751",
    "user": "https://github.com/mkoeppe"
}
```

As a compromise, we could also do a yes/no prompt when `--enable-dowload-from-upstream-url` has not been used; just like we do when experimental packages are to be installed



---

archive/issue_events_085946.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T01:55:27Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/32390",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/32390#event-85946"
}
```



---

archive/issue_events_085947.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T01:55:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/32390",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/32390#event-85947"
}
```



---

archive/issue_comments_458752.json:
```json
{
    "body": "just make it default. The only security problem here would be a rogue branch with checksums verifying a rogue package. It's only very marginally less secure than packages from our mirrors.",
    "created_at": "2022-08-24T11:53:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32390#issuecomment-458752",
    "user": "https://github.com/dimpase"
}
```

just make it default. The only security problem here would be a rogue branch with checksums verifying a rogue package. It's only very marginally less secure than packages from our mirrors.



---

archive/issue_comments_458753.json:
```json
{
    "body": "I agree that it should be the default. As long as we maintain the practice of keeping our own copies of current versions of package tarballs on Sage mirrors (and I think we *should* maintain this), then this will only affect developers testing a new release of some package. The security problems are pretty small, I think.\n\nRegarding \"Just downloading something from some url and installing it is something, I would want to be notified about\", I think that occasionally we miss a package and so this happens anyway. Maybe we've patched something to prevent this? Do any of the github actions (or similar) test building Sage without an internet connection?",
    "created_at": "2022-08-30T18:55:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32390#issuecomment-458753",
    "user": "https://github.com/jhpalmieri"
}
```

I agree that it should be the default. As long as we maintain the practice of keeping our own copies of current versions of package tarballs on Sage mirrors (and I think we *should* maintain this), then this will only affect developers testing a new release of some package. The security problems are pretty small, I think.

Regarding "Just downloading something from some url and installing it is something, I would want to be notified about", I think that occasionally we miss a package and so this happens anyway. Maybe we've patched something to prevent this? Do any of the github actions (or similar) test building Sage without an internet connection?



---

archive/issue_events_085948.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/32390",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/32390#event-85948"
}
```



---

archive/issue_events_085949.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/32390",
    "milestone": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/32390#event-85949"
}
```



---

archive/issue_comments_458754.json:
```json
{
    "body": "Time to revisit this?",
    "created_at": "2022-11-23T06:27:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32390#issuecomment-458754",
    "user": "https://github.com/jhpalmieri"
}
```

Time to revisit this?



---

archive/issue_comments_458755.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2022-11-23T06:57:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32390#issuecomment-458755",
    "user": "https://github.com/mkoeppe"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_458756.json:
```json
{
    "body": "New commits:",
    "created_at": "2022-11-23T06:57:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32390#issuecomment-458756",
    "user": "https://github.com/mkoeppe"
}
```

New commits:



---

archive/issue_comments_458757.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2022-11-23T07:02:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32390#issuecomment-458757",
    "user": "https://github.com/mkoeppe"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_458758.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2022-11-23T07:05:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32390#issuecomment-458758",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_458759.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2022-11-23T07:05:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32390#issuecomment-458759",
    "user": "https://github.com/mkoeppe"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_458760.json:
```json
{
    "body": "Replying to [comment:12 John Palmieri]:\n> Regarding \"Just downloading something from some url and installing it is something, I would want to be notified about\", I think that occasionally we miss a package and so this happens anyway.\n\n\nYes, we have a GH Actions workflow (`release_dist`) that checks that at least all standard packages exist on the mirror (because `make dist` downloads them all and puts them in the sdist tarball). We do not have such a check for optional packages.",
    "created_at": "2022-11-23T07:08:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32390#issuecomment-458760",
    "user": "https://github.com/mkoeppe"
}
```

Replying to [comment:12 John Palmieri]:
> Regarding "Just downloading something from some url and installing it is something, I would want to be notified about", I think that occasionally we miss a package and so this happens anyway.


Yes, we have a GH Actions workflow (`release_dist`) that checks that at least all standard packages exist on the mirror (because `make dist` downloads them all and puts them in the sdist tarball). We do not have such a check for optional packages.



---

archive/issue_comments_458761.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2022-11-23T21:06:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32390#issuecomment-458761",
    "user": "https://github.com/dimpase"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_458762.json:
```json
{
    "body": "this works",
    "created_at": "2022-11-23T21:06:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32390#issuecomment-458762",
    "user": "https://github.com/dimpase"
}
```

this works



---

archive/issue_comments_458763.json:
```json
{
    "body": "Thank you!",
    "created_at": "2022-11-23T21:07:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32390#issuecomment-458763",
    "user": "https://github.com/mkoeppe"
}
```

Thank you!



---

archive/issue_comments_458764.json:
```json
{
    "body": "Changing status from positive_review to needs_work.",
    "created_at": "2022-12-03T00:46:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32390#issuecomment-458764",
    "user": "https://github.com/vbraun"
}
```

Changing status from positive_review to needs_work.



---

archive/issue_comments_458765.json:
```json
{
    "body": "Merge failure on top of:\n\n34266283ab5 Trac #29360: change_ring() should preserve sparsity of vectors and vector spaces\n\n1f56ce0e9d7 Trac #27652: parent of plethysm\n\n019537d9929 Trac #34693: Further support for matplotlib 3.6\n\n59e9f7b4f01 Trac #34658: Update numpy to 1.23.5, scipy 1.9.3, networkx 2.8.8, meson_python 0.11.0\n\n6d03a671290 Trac #34593: Document and manage temporary directories\n\n454290087ec Trac #33842: Upgrade python to 3.11\n\nf53f07a063f Trac #34766: GH Actions: Update actions\n\n795383fbdc9 Trac #34728: change sorting for WeierstrassIsomorphism\n\n2cec793d624 Trac #33562: Bad error message for weighted adjacency matrix\n\n3670306d20f Trac #34740: dead hyperlinks in developer manual\n\n9666ae7ced6 Trac #34722: some code cleanup in WeierstrassIsomorphism\n\nf41abf6243d Trac #34759: some details in filtered simplicial complexes\n\ndfc299ba564 Trac #34756: Documentation regarding setting up SageMath's Jupyter kernel in an existing installation points to wrong directory\n\n513a7bc6a9c Trac #34753: fix all W391 in pyx files\n\n7503e42cf2f Trac #34751: Update sage tutorial\n\n623ea7446d3 Trac #34745: modernize super in algebras/ again\n\nf2fa7597737 Trac #34741: OS X 13: filter out dylib warning\n\na4748c342ab Trac #34738: tiny details in symbolic min and max\n\nfb213dfda77 Trac #34769: use libgap in simplicial_complex\n\n01beb6a1069 Trac #34765: meson: Add spkg-configure.m4\n\nd94c7334140 Trac #34762: Fix random chain complex doctest\n\nb3398f0543d Trac #34761: Remove src/sage/libs/fes.pyx\n\n3c42a395c22 Trac #34754: Remove module-level imports from sage.plot\n\n0d120581338 Trac #34569: Fix some quasimodular forms rings methods for congruence subgroups\n\n84f02afa5c8 Updated [SageMath](SageMath) version to 9.8.beta4\n\n\n\nmerge was not clean: conflicts in .github/workflows/dist.yml",
    "created_at": "2022-12-03T00:46:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32390#issuecomment-458765",
    "user": "https://github.com/vbraun"
}
```

Merge failure on top of:

34266283ab5 Trac #29360: change_ring() should preserve sparsity of vectors and vector spaces

1f56ce0e9d7 Trac #27652: parent of plethysm

019537d9929 Trac #34693: Further support for matplotlib 3.6

59e9f7b4f01 Trac #34658: Update numpy to 1.23.5, scipy 1.9.3, networkx 2.8.8, meson_python 0.11.0

6d03a671290 Trac #34593: Document and manage temporary directories

454290087ec Trac #33842: Upgrade python to 3.11

f53f07a063f Trac #34766: GH Actions: Update actions

795383fbdc9 Trac #34728: change sorting for WeierstrassIsomorphism

2cec793d624 Trac #33562: Bad error message for weighted adjacency matrix

3670306d20f Trac #34740: dead hyperlinks in developer manual

9666ae7ced6 Trac #34722: some code cleanup in WeierstrassIsomorphism

f41abf6243d Trac #34759: some details in filtered simplicial complexes

dfc299ba564 Trac #34756: Documentation regarding setting up SageMath's Jupyter kernel in an existing installation points to wrong directory

513a7bc6a9c Trac #34753: fix all W391 in pyx files

7503e42cf2f Trac #34751: Update sage tutorial

623ea7446d3 Trac #34745: modernize super in algebras/ again

f2fa7597737 Trac #34741: OS X 13: filter out dylib warning

a4748c342ab Trac #34738: tiny details in symbolic min and max

fb213dfda77 Trac #34769: use libgap in simplicial_complex

01beb6a1069 Trac #34765: meson: Add spkg-configure.m4

d94c7334140 Trac #34762: Fix random chain complex doctest

b3398f0543d Trac #34761: Remove src/sage/libs/fes.pyx

3c42a395c22 Trac #34754: Remove module-level imports from sage.plot

0d120581338 Trac #34569: Fix some quasimodular forms rings methods for congruence subgroups

84f02afa5c8 Updated [SageMath](SageMath) version to 9.8.beta4



merge was not clean: conflicts in .github/workflows/dist.yml



---

archive/issue_comments_458766.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2022-12-03T03:32:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32390#issuecomment-458766",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_458767.json:
```json
{
    "body": "Changing status from needs_work to positive_review.",
    "created_at": "2022-12-03T03:32:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32390#issuecomment-458767",
    "user": "https://github.com/mkoeppe"
}
```

Changing status from needs_work to positive_review.



---

archive/issue_comments_458768.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2022-12-14T22:12:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32390",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32390#issuecomment-458768",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_events_085950.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-12-14T22:12:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/32390",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/32390#event-85950"
}
```
