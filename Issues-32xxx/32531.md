# Issue 32531: SAGE_ROOT/tox.ini: Add variants that provision a system python using pyenv

archive/issues_032294.json:
```json
{
    "assignees": [],
    "body": "It was suggested (https://groups.google.com/g/sage-devel/c/cSsAsPuVnxg/m/uWEySfvWBQAJ) that testing of new Python versions (such as the upcoming 3.10) could be done via `pyenv` instead of test tickets (such as #30766) that update our `python3` spkg.\n\nIn this ticket, we add supporting infrastructure for this in `tox.ini`.\n\n- https://github.com/pyenv/pyenv\n- https://tox.readthedocs.io/en/latest/plugins.html tox-pyenv\n\nUpstream issue:\n- https://github.com/pyenv/pyenv/issues/2204\n\n- `python-build` breaks when `MAKE` is set as we document it for Sage (`export MAKE=\"make -j15\"):\n\n  ```\n  /usr/local/Cellar/pyenv/2.3.0/plugins/python-build/bin/python-build: line 789: make -j15: command not found\n  ```\n\nDepends on #29285\n\nCC:  @dimpase @orlitzky\n\nUpstream: **Reported upstream. No feedback yet.**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32531_\n\n",
    "created_at": "2021-09-17T20:08:02Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.9",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "SAGE_ROOT/tox.ini: Add variants that provision a system python using pyenv",
    "type": "issue",
    "updated_at": "2023-01-07T21:39:40Z",
    "url": "https://github.com/sagemath/sage/issues/32531",
    "user": "https://github.com/mkoeppe"
}
```
It was suggested (https://groups.google.com/g/sage-devel/c/cSsAsPuVnxg/m/uWEySfvWBQAJ) that testing of new Python versions (such as the upcoming 3.10) could be done via `pyenv` instead of test tickets (such as #30766) that update our `python3` spkg.

In this ticket, we add supporting infrastructure for this in `tox.ini`.

- https://github.com/pyenv/pyenv
- https://tox.readthedocs.io/en/latest/plugins.html tox-pyenv

Upstream issue:
- https://github.com/pyenv/pyenv/issues/2204

- `python-build` breaks when `MAKE` is set as we document it for Sage (`export MAKE="make -j15"):

  ```
  /usr/local/Cellar/pyenv/2.3.0/plugins/python-build/bin/python-build: line 789: make -j15: command not found
  ```

Depends on #29285

CC:  @dimpase @orlitzky

Upstream: **Reported upstream. No feedback yet.**

_Issue created by migration from https://trac.sagemath.org/ticket/32531_





---

archive/issue_events_430126.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-17T20:08:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32531",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32531#event-430126"
}
```



---

archive/issue_events_430127.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-17T20:08:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32531",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting",
    "label_color": "000080",
    "label_name": "component: porting",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32531#event-430127"
}
```



---

archive/issue_events_430128.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-17T20:08:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32531",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32531#event-430128"
}
```



---

archive/issue_events_430129.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-17T20:08:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32531",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32531#event-430129"
}
```



---

archive/issue_events_430130.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:11:26Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/32531",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32531#event-430130"
}
```



---

archive/issue_events_430131.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:11:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32531",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32531#event-430131"
}
```



---

archive/issue_comments_527728.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nRemove dependency on #32442, see [#29039 comment:163](https://github.com/sagemath/sage/issues/29039#comment:163).",
    "created_at": "2021-12-29T06:19:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32531",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32531#issuecomment-527728",
    "user": "https://github.com/slel"
}
```

<a id='comment:2'>Comment 2:</a>
Remove dependency on #32442, see [#29039 comment:163](https://github.com/sagemath/sage/issues/29039#comment:163).



---

archive/issue_comments_527729.json:
```json
{
    "body": "Changed dependencies from **#32442** to none",
    "created_at": "2021-12-29T06:19:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32531",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32531#issuecomment-527729",
    "user": "https://github.com/slel"
}
```

Changed dependencies from **#32442** to none



---

archive/issue_comments_527730.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\n`pyenv` (or, more precisely, its vendored version of `python-build`, which provides the `pyenv install` command) builds misconfigured pythons with `-I` and `-L` options in the sysconfig variables (same problem as in #31132, #33078):\n\n```\n$ brew install pyenv\n$ pyenv install 3.7.8\n$ eval \"$(pyenv init --path)\"\n$ echo $PATH\n/Users/mkoeppe/.pyenv/shims:/Users/mkoeppe/miniconda3/condabin:/Users/mkoeppe/bin:/usr/local/bin:/Users/mkoeppe/google-cloud-sdk/bin:/Users/mkoeppe/perl5/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin:/Library/TeX/texbin:/opt/X11/bin\n$ pyenv global 3.7.8\n$ python3 -m sysconfig\nPlatform: \"macosx-11.6-x86_64\"\nPython version: \"3.7\"\nCurrent installation scheme: \"posix_prefix\"\n\nPaths: \n\tdata = \"/Users/mkoeppe/.pyenv/versions/3.7.8\"\n\tinclude = \"/Users/mkoeppe/.pyenv/versions/3.7.8/include/python3.7m\"\n\tplatinclude = \"/Users/mkoeppe/.pyenv/versions/3.7.8/include/python3.7m\"\n\tplatlib = \"/Users/mkoeppe/.pyenv/versions/3.7.8/lib/python3.7/site-packages\"\n\tplatstdlib = \"/Users/mkoeppe/.pyenv/versions/3.7.8/lib/python3.7\"\n\tpurelib = \"/Users/mkoeppe/.pyenv/versions/3.7.8/lib/python3.7/site-packages\"\n\tscripts = \"/Users/mkoeppe/.pyenv/versions/3.7.8/bin\"\n\tstdlib = \"/Users/mkoeppe/.pyenv/versions/3.7.8/lib/python3.7\"\n\nVariables: \n[...]\n\tCFLAGS = \"-Wno-unused-result -Wsign-compare -Wunreachable-code -DNDEBUG -g -fwrapv -O3 -Wall -I/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include -I/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include\"\n[...]\n\tCFLAGS_NODIST = \"\"\n[...]\n\tLDFLAGS = \"-L/usr/local/opt/readline/lib -L/usr/local/opt/readline/lib -L/Users/mkoeppe/.pyenv/versions/3.7.8/lib  -L/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/lib -L/usr/local/opt/readline/lib -L/usr/local/opt/readline/lib -L/Users/mkoeppe/.pyenv/versions/3.7.8/lib  -L/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/lib\"\n\tLDFLAGS_NODIST = \"\"\n```",
    "created_at": "2022-01-02T00:41:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32531",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32531#issuecomment-527730",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:3'>Comment 3:</a>
`pyenv` (or, more precisely, its vendored version of `python-build`, which provides the `pyenv install` command) builds misconfigured pythons with `-I` and `-L` options in the sysconfig variables (same problem as in #31132, #33078):

```
$ brew install pyenv
$ pyenv install 3.7.8
$ eval "$(pyenv init --path)"
$ echo $PATH
/Users/mkoeppe/.pyenv/shims:/Users/mkoeppe/miniconda3/condabin:/Users/mkoeppe/bin:/usr/local/bin:/Users/mkoeppe/google-cloud-sdk/bin:/Users/mkoeppe/perl5/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin:/Library/TeX/texbin:/opt/X11/bin
$ pyenv global 3.7.8
$ python3 -m sysconfig
Platform: "macosx-11.6-x86_64"
Python version: "3.7"
Current installation scheme: "posix_prefix"

Paths: 
	data = "/Users/mkoeppe/.pyenv/versions/3.7.8"
	include = "/Users/mkoeppe/.pyenv/versions/3.7.8/include/python3.7m"
	platinclude = "/Users/mkoeppe/.pyenv/versions/3.7.8/include/python3.7m"
	platlib = "/Users/mkoeppe/.pyenv/versions/3.7.8/lib/python3.7/site-packages"
	platstdlib = "/Users/mkoeppe/.pyenv/versions/3.7.8/lib/python3.7"
	purelib = "/Users/mkoeppe/.pyenv/versions/3.7.8/lib/python3.7/site-packages"
	scripts = "/Users/mkoeppe/.pyenv/versions/3.7.8/bin"
	stdlib = "/Users/mkoeppe/.pyenv/versions/3.7.8/lib/python3.7"

Variables: 
[...]
	CFLAGS = "-Wno-unused-result -Wsign-compare -Wunreachable-code -DNDEBUG -g -fwrapv -O3 -Wall -I/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include -I/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/include"
[...]
	CFLAGS_NODIST = ""
[...]
	LDFLAGS = "-L/usr/local/opt/readline/lib -L/usr/local/opt/readline/lib -L/Users/mkoeppe/.pyenv/versions/3.7.8/lib  -L/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/lib -L/usr/local/opt/readline/lib -L/usr/local/opt/readline/lib -L/Users/mkoeppe/.pyenv/versions/3.7.8/lib  -L/Library/Developer/CommandLineTools/SDKs/MacOSX.sdk/usr/lib"
	LDFLAGS_NODIST = ""
```



---

archive/issue_comments_527731.json:
```json
{
    "body": "Dependencies: **#29285**",
    "created_at": "2022-01-02T02:18:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32531",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32531#issuecomment-527731",
    "user": "https://github.com/mkoeppe"
}
```

Dependencies: **#29285**



---

archive/issue_comments_527732.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -5,3 +5,5 @@\n - https://github.com/pyenv/pyenv\n - https://tox.readthedocs.io/en/latest/plugins.html tox-pyenv\n \n+Upstream issue:\n+- https://github.com/pyenv/pyenv/issues/2204\n``````\n",
    "created_at": "2022-01-02T19:03:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32531",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32531#issuecomment-527732",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -5,3 +5,5 @@
 - https://github.com/pyenv/pyenv
 - https://tox.readthedocs.io/en/latest/plugins.html tox-pyenv
 
+Upstream issue:
+- https://github.com/pyenv/pyenv/issues/2204
``````




---

archive/issue_comments_527733.json:
```json
{
    "body": "Upstream: **Reported upstream. No feedback yet.**",
    "created_at": "2022-01-12T17:53:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32531",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32531#issuecomment-527733",
    "user": "https://github.com/mkoeppe"
}
```

Upstream: **Reported upstream. No feedback yet.**



---

archive/issue_events_430132.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T01:55:27Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/32531",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32531#event-430132"
}
```



---

archive/issue_events_430133.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T01:55:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32531",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32531#event-430133"
}
```



---

archive/issue_comments_527734.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -7,3 +7,9 @@\n \n Upstream issue:\n - https://github.com/pyenv/pyenv/issues/2204\n+\n+- `python-build` breaks when `MAKE` is set as we document it for Sage (`export MAKE=\"make -j15\"):\n+\n+  ```\n+  /usr/local/Cellar/pyenv/2.3.0/plugins/python-build/bin/python-build: line 789: make -j15: command not found\n+  ```\n``````\n",
    "created_at": "2022-05-12T02:43:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32531",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32531#issuecomment-527734",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -7,3 +7,9 @@
 
 Upstream issue:
 - https://github.com/pyenv/pyenv/issues/2204
+
+- `python-build` breaks when `MAKE` is set as we document it for Sage (`export MAKE="make -j15"):
+
+  ```
+  /usr/local/Cellar/pyenv/2.3.0/plugins/python-build/bin/python-build: line 789: make -j15: command not found
+  ```
``````




---

archive/issue_events_430134.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-01T20:41:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/32531",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32531#event-430134"
}
```



---

archive/issue_events_430135.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-01T20:41:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32531",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32531#event-430135"
}
```



---

archive/issue_events_430136.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T21:39:40Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/32531",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32531#event-430136"
}
```



---

archive/issue_events_430137.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T21:39:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32531",
    "milestone_number": null,
    "milestone_title": "sage-9.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32531#event-430137"
}
```
