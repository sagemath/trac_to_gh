# Issue 32057: sage_setup.setenv: Only set ...PATH variables and LDFLAGS if SAGE_LOCAL differs from SAGE_VENV

archive/issues_031820.json:
```json
{
    "assignees": [],
    "body": "Here's a different attempt to solve the issue at [#31338 comment:13](https://github.com/sagemath/sage/issues/31338#comment:13)\n\nThis will need to be tested with #29039, #31396, for which this mechanism was created.\n\n\nCC:  @kiwifb @antonio-rojas\n\nBranch/Commit: **[0867811](https://github.com/sagemath/sagetrac-mirror/commit/08678114db533b7f6da8529a81cae556ce60461a)**\n\nReviewer: **Fran\u00e7ois Bissey**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32057_\n\n",
    "closed_at": "2021-07-06T21:29:14Z",
    "created_at": "2021-06-24T22:27:37Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage_setup.setenv: Only set ...PATH variables and LDFLAGS if SAGE_LOCAL differs from SAGE_VENV",
    "type": "issue",
    "updated_at": "2021-07-06T21:29:14Z",
    "url": "https://github.com/sagemath/sage/issues/32057",
    "user": "https://github.com/mkoeppe"
}
```
Here's a different attempt to solve the issue at [#31338 comment:13](https://github.com/sagemath/sage/issues/31338#comment:13)

This will need to be tested with #29039, #31396, for which this mechanism was created.


CC:  @kiwifb @antonio-rojas

Branch/Commit: **[0867811](https://github.com/sagemath/sagetrac-mirror/commit/08678114db533b7f6da8529a81cae556ce60461a)**

Reviewer: **François Bissey**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/32057_





---

archive/issue_events_423867.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-06-24T22:27:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32057",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32057#event-423867"
}
```



---

archive/issue_events_423868.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-06-24T22:27:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32057",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32057#event-423868"
}
```



---

archive/issue_events_423869.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-06-24T22:27:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32057",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32057#event-423869"
}
```



---

archive/issue_events_423870.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-06-24T22:27:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32057",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32057#event-423870"
}
```



---

archive/issue_comments_519195.json:
```json
{
    "body": "Branch: **[u/mkoeppe/sage_setup_setenv__only_set____path_variables_and_ldflags_if_sage_local_differs_from_sage_venv](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/sage_setup_setenv__only_set____path_variables_and_ldflags_if_sage_local_differs_from_sage_venv)**",
    "created_at": "2021-06-24T22:58:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32057#issuecomment-519195",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/sage_setup_setenv__only_set____path_variables_and_ldflags_if_sage_local_differs_from_sage_venv](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/sage_setup_setenv__only_set____path_variables_and_ldflags_if_sage_local_differs_from_sage_venv)**



---

archive/issue_comments_519196.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2021-06-24T22:58:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32057#issuecomment-519196",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_comments_519197.json:
```json
{
    "body": "Commit: **[0867811](https://github.com/sagemath/sagetrac-mirror/commit/08678114db533b7f6da8529a81cae556ce60461a)**",
    "created_at": "2021-06-24T22:58:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32057#issuecomment-519197",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[0867811](https://github.com/sagemath/sagetrac-mirror/commit/08678114db533b7f6da8529a81cae556ce60461a)**



---

archive/issue_comments_519198.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/08678114db533b7f6da8529a81cae556ce60461a\">0867811</a></td><td><code>src/sage_setup/setenv.py: Only set ...PATH variables and LDFLAGS if SAGE_LOCAL differs from SAGE_VENV</code></td></tr></table>\n",
    "created_at": "2021-06-24T22:58:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32057#issuecomment-519198",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/08678114db533b7f6da8529a81cae556ce60461a">0867811</a></td><td><code>src/sage_setup/setenv.py: Only set ...PATH variables and LDFLAGS if SAGE_LOCAL differs from SAGE_VENV</code></td></tr></table>




---

archive/issue_comments_519199.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nDoes what it says on the tin for sage-on-gentoo. No more log pollution.",
    "created_at": "2021-06-24T23:10:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32057#issuecomment-519199",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:3'>Comment 3:</a>
Does what it says on the tin for sage-on-gentoo. No more log pollution.



---

archive/issue_comments_519200.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nThanks for testing!",
    "created_at": "2021-06-24T23:14:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32057#issuecomment-519200",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:4'>Comment 4:</a>
Thanks for testing!



---

archive/issue_comments_519201.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nIt is a positive for me but we have to check it ticks all the other boxes you mention in the description.",
    "created_at": "2021-06-24T23:16:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32057#issuecomment-519201",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:5'>Comment 5:</a>
It is a positive for me but we have to check it ticks all the other boxes you mention in the description.



---

archive/issue_comments_519202.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nready for review?",
    "created_at": "2021-06-27T10:19:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32057#issuecomment-519202",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:6'>Comment 6:</a>
ready for review?



---

archive/issue_events_423871.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-06-27T10:20:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32057",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32057#event-423871"
}
```



---

archive/issue_comments_519203.json:
```json
{
    "body": "<a id='comment:8'>Comment 8:</a>\nSure, let's get this in.",
    "created_at": "2021-06-27T15:29:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32057#issuecomment-519203",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:8'>Comment 8:</a>
Sure, let's get this in.



---

archive/issue_comments_519204.json:
```json
{
    "body": "Reviewer: **Fran\u00e7ois Bissey**",
    "created_at": "2021-06-27T20:57:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32057#issuecomment-519204",
    "user": "https://github.com/kiwifb"
}
```

Reviewer: **François Bissey**



---

archive/issue_comments_519205.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nWell I am happy to it a glowing review.",
    "created_at": "2021-06-27T20:57:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32057#issuecomment-519205",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:9'>Comment 9:</a>
Well I am happy to it a glowing review.



---

archive/issue_events_423872.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2021-06-27T20:57:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32057",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32057#event-423872"
}
```



---

archive/issue_events_423873.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2021-06-27T20:57:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32057",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32057#event-423873"
}
```



---

archive/issue_comments_519206.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nThanks!",
    "created_at": "2021-06-27T20:58:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32057#issuecomment-519206",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:10'>Comment 10:</a>
Thanks!



---

archive/issue_events_423874.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-07-06T21:29:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32057",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32057#event-423874"
}
```



---

archive/issue_events_423875.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "d439c06f062b3fc35aa211d5bda57125bcf1fee8",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-07-06T21:29:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32057",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32057#event-423875"
}
```



---

archive/issue_comments_519207.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/sage_setup_setenv__only_set____path_variables_and_ldflags_if_sage_local_differs_from_sage_venv](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/sage_setup_setenv__only_set____path_variables_and_ldflags_if_sage_local_differs_from_sage_venv)** to **[0867811](https://github.com/sagemath/sagetrac-mirror/commit/08678114db533b7f6da8529a81cae556ce60461a)**",
    "created_at": "2021-07-06T21:29:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32057",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32057#issuecomment-519207",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/sage_setup_setenv__only_set____path_variables_and_ldflags_if_sage_local_differs_from_sage_venv](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/sage_setup_setenv__only_set____path_variables_and_ldflags_if_sage_local_differs_from_sage_venv)** to **[0867811](https://github.com/sagemath/sagetrac-mirror/commit/08678114db533b7f6da8529a81cae556ce60461a)**
