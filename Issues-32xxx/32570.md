# Issue 32570: tox.ini, GH Actions: Update macOS environments to macOS 11, 12

archive/issues_032333.json:
```json
{
    "assignees": [],
    "body": "The macOS-10.15 environment is deprecated and will be unsupported on 2022-12-01. Homebrew packages are already not available any more.\n\n- https://github.com/actions/virtual-environments\n- https://github.com/actions/virtual-environments/blob/main/images/macos/macos-12-Readme.md\n- https://github.com/actions/virtual-environments/blob/main/images/macos/macos-11-Readme.md:\n- https://github.com/actions/virtual-environments/blob/main/images/macos/macos-10.15-Readme.md:\n\nThere's no macOS 13 environment yet.\n\nPossible follow-up:\n- Add variants that test with an actual XCode installation (not XCode CLT) because the SDK paths are different, and there may be other subtle differences. GH Actions install location: see https://github.com/actions/virtual-environments/blob/main/images/macos/macos-11-Readme.md#xcode\n\n\n\n\nDepends on #33788\n\nCC:  @culler @dimpase @vbraun\n\nBranch/Commit: **[f185d68](https://github.com/sagemath/sagetrac-mirror/commit/f185d689989a661cf77e22471a56277f61a18dd5)**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32570_\n\n",
    "closed_at": "2022-11-15T23:43:12Z",
    "created_at": "2021-09-26T22:55:48Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "tox.ini, GH Actions: Update macOS environments to macOS 11, 12",
    "type": "issue",
    "updated_at": "2022-11-15T23:43:12Z",
    "url": "https://github.com/sagemath/sage/issues/32570",
    "user": "https://github.com/mkoeppe"
}
```
The macOS-10.15 environment is deprecated and will be unsupported on 2022-12-01. Homebrew packages are already not available any more.

- https://github.com/actions/virtual-environments
- https://github.com/actions/virtual-environments/blob/main/images/macos/macos-12-Readme.md
- https://github.com/actions/virtual-environments/blob/main/images/macos/macos-11-Readme.md:
- https://github.com/actions/virtual-environments/blob/main/images/macos/macos-10.15-Readme.md:

There's no macOS 13 environment yet.

Possible follow-up:
- Add variants that test with an actual XCode installation (not XCode CLT) because the SDK paths are different, and there may be other subtle differences. GH Actions install location: see https://github.com/actions/virtual-environments/blob/main/images/macos/macos-11-Readme.md#xcode




Depends on #33788

CC:  @culler @dimpase @vbraun

Branch/Commit: **[f185d68](https://github.com/sagemath/sagetrac-mirror/commit/f185d689989a661cf77e22471a56277f61a18dd5)**

Reviewer: **Dima Pasechnik**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/32570_





---

archive/issue_events_430558.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-26T22:55:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32570#event-430558"
}
```



---

archive/issue_events_430559.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-26T22:55:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting",
    "label_color": "000080",
    "label_name": "component: porting",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32570#event-430559"
}
```



---

archive/issue_events_430560.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-26T22:55:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32570#event-430560"
}
```



---

archive/issue_events_430561.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-26T22:55:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32570#event-430561"
}
```



---

archive/issue_comments_528187.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,5 +2,7 @@\n - https://github.com/actions/virtual-environments/blob/main/images/macos/macos-11-Readme.md:\n - https://github.com/actions/virtual-environments/blob/main/images/macos/macos-10.15-Readme.md:\n \n+We should also add variants that test with an actual XCode installation (not XCode CLT) because the SDK paths are different, and there may be other subtle differences. GH Actions install location: see https://github.com/actions/virtual-environments/blob/main/images/macos/macos-11-Readme.md#xcode\n \n \n+\n``````\n",
    "created_at": "2021-10-02T16:46:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32570#issuecomment-528187",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,5 +2,7 @@
 - https://github.com/actions/virtual-environments/blob/main/images/macos/macos-11-Readme.md:
 - https://github.com/actions/virtual-environments/blob/main/images/macos/macos-10.15-Readme.md:
 
+We should also add variants that test with an actual XCode installation (not XCode CLT) because the SDK paths are different, and there may be other subtle differences. GH Actions install location: see https://github.com/actions/virtual-environments/blob/main/images/macos/macos-11-Readme.md#xcode
 
 
+
``````




---

archive/issue_comments_528188.json:
```json
{
    "body": "Dependencies: **#32703**",
    "created_at": "2021-10-23T05:35:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32570#issuecomment-528188",
    "user": "https://github.com/mkoeppe"
}
```

Dependencies: **#32703**



---

archive/issue_comments_528189.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nUpdate for Sage 9.5 done in #32703. Let's keep this ticket for the next update",
    "created_at": "2021-12-21T05:23:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32570#issuecomment-528189",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:3" align="right">Comment 3</div>

Update for Sage 9.5 done in #32703. Let's keep this ticket for the next update



---

archive/issue_events_430562.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-21T05:23:06Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32570#event-430562"
}
```



---

archive/issue_events_430563.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-21T05:23:06Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32570#event-430563"
}
```



---

archive/issue_events_430564.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T01:55:27Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32570#event-430564"
}
```



---

archive/issue_events_430565.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T01:55:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32570#event-430565"
}
```



---

archive/issue_comments_528190.json:
```json
{
    "body": "Changed dependencies from **#32703** to **#33788**",
    "created_at": "2022-07-02T19:46:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32570#issuecomment-528190",
    "user": "https://github.com/mkoeppe"
}
```

Changed dependencies from **#32703** to **#33788**



---

archive/issue_comments_528191.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,5 @@\n ... according to:\n+- https://github.com/actions/virtual-environments/blob/main/images/macos/macos-12-Readme.md\n - https://github.com/actions/virtual-environments/blob/main/images/macos/macos-11-Readme.md:\n - https://github.com/actions/virtual-environments/blob/main/images/macos/macos-10.15-Readme.md:\n \n``````\n",
    "created_at": "2022-07-02T19:47:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32570#issuecomment-528191",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,5 @@
 ... according to:
+- https://github.com/actions/virtual-environments/blob/main/images/macos/macos-12-Readme.md
 - https://github.com/actions/virtual-environments/blob/main/images/macos/macos-11-Readme.md:
 - https://github.com/actions/virtual-environments/blob/main/images/macos/macos-10.15-Readme.md:
 
``````




---

archive/issue_comments_528192.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,5 @@\n ... according to:\n+- https://github.com/actions/virtual-environments\n - https://github.com/actions/virtual-environments/blob/main/images/macos/macos-12-Readme.md\n - https://github.com/actions/virtual-environments/blob/main/images/macos/macos-11-Readme.md:\n - https://github.com/actions/virtual-environments/blob/main/images/macos/macos-10.15-Readme.md:\n``````\n",
    "created_at": "2022-07-08T04:45:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32570#issuecomment-528192",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,5 @@
 ... according to:
+- https://github.com/actions/virtual-environments
 - https://github.com/actions/virtual-environments/blob/main/images/macos/macos-12-Readme.md
 - https://github.com/actions/virtual-environments/blob/main/images/macos/macos-11-Readme.md:
 - https://github.com/actions/virtual-environments/blob/main/images/macos/macos-10.15-Readme.md:
``````




---

archive/issue_comments_528193.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -4,6 +4,9 @@\n - https://github.com/actions/virtual-environments/blob/main/images/macos/macos-11-Readme.md:\n - https://github.com/actions/virtual-environments/blob/main/images/macos/macos-10.15-Readme.md:\n \n+The macOS-10.15 environment is deprecated and will be removed on August 30th, 2022\n+\n+\n We should also add variants that test with an actual XCode installation (not XCode CLT) because the SDK paths are different, and there may be other subtle differences. GH Actions install location: see https://github.com/actions/virtual-environments/blob/main/images/macos/macos-11-Readme.md#xcode\n \n \n``````\n",
    "created_at": "2022-08-03T08:00:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32570#issuecomment-528193",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -4,6 +4,9 @@
 - https://github.com/actions/virtual-environments/blob/main/images/macos/macos-11-Readme.md:
 - https://github.com/actions/virtual-environments/blob/main/images/macos/macos-10.15-Readme.md:
 
+The macOS-10.15 environment is deprecated and will be removed on August 30th, 2022
+
+
 We should also add variants that test with an actual XCode installation (not XCode CLT) because the SDK paths are different, and there may be other subtle differences. GH Actions install location: see https://github.com/actions/virtual-environments/blob/main/images/macos/macos-11-Readme.md#xcode
 
 
``````




---

archive/issue_events_430566.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32570#event-430566"
}
```



---

archive/issue_events_430567.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32570#event-430567"
}
```



---

archive/issue_comments_528194.json:
```json
{
    "body": "Branch: **[u/mkoeppe/tox_ini__gh_actions__update_macos_environments](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/tox_ini__gh_actions__update_macos_environments)**",
    "created_at": "2022-11-11T21:48:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32570#issuecomment-528194",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/tox_ini__gh_actions__update_macos_environments](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/tox_ini__gh_actions__update_macos_environments)**



---

archive/issue_comments_528195.json:
```json
{
    "body": "Commit: **[37ac84a](https://github.com/sagemath/sagetrac-mirror/commit/37ac84aed05a028e3411c201f4b8db35e0e0b2f9)**",
    "created_at": "2022-11-11T21:51:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32570#issuecomment-528195",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[37ac84a](https://github.com/sagemath/sagetrac-mirror/commit/37ac84aed05a028e3411c201f4b8db35e0e0b2f9)**



---

archive/issue_events_430568.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-11-11T21:51:04Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "title_is": "tox.ini, GH Actions: Update macOS environments to macOS 11, 12",
    "title_was": "tox.ini, GH Actions: Update macOS environments",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32570#event-430568"
}
```



---

archive/issue_comments_528196.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2022-11-11T21:51:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32570#issuecomment-528196",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_comments_528197.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,13 +1,14 @@\n-... according to:\n+The macOS-10.15 environment is deprecated and will be unsupported on 2022-12-01. Homebrew packages are already not available any more.\n+\n - https://github.com/actions/virtual-environments\n - https://github.com/actions/virtual-environments/blob/main/images/macos/macos-12-Readme.md\n - https://github.com/actions/virtual-environments/blob/main/images/macos/macos-11-Readme.md:\n - https://github.com/actions/virtual-environments/blob/main/images/macos/macos-10.15-Readme.md:\n \n-The macOS-10.15 environment is deprecated and will be removed on August 30th, 2022\n+There's no macOS 13 environment yet.\n \n-\n-We should also add variants that test with an actual XCode installation (not XCode CLT) because the SDK paths are different, and there may be other subtle differences. GH Actions install location: see https://github.com/actions/virtual-environments/blob/main/images/macos/macos-11-Readme.md#xcode\n+Possible follow-up:\n+- Add variants that test with an actual XCode installation (not XCode CLT) because the SDK paths are different, and there may be other subtle differences. GH Actions install location: see https://github.com/actions/virtual-environments/blob/main/images/macos/macos-11-Readme.md#xcode\n \n \n \n``````\n",
    "created_at": "2022-11-11T21:51:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32570#issuecomment-528197",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,13 +1,14 @@
-... according to:
+The macOS-10.15 environment is deprecated and will be unsupported on 2022-12-01. Homebrew packages are already not available any more.
+
 - https://github.com/actions/virtual-environments
 - https://github.com/actions/virtual-environments/blob/main/images/macos/macos-12-Readme.md
 - https://github.com/actions/virtual-environments/blob/main/images/macos/macos-11-Readme.md:
 - https://github.com/actions/virtual-environments/blob/main/images/macos/macos-10.15-Readme.md:
 
-The macOS-10.15 environment is deprecated and will be removed on August 30th, 2022
+There's no macOS 13 environment yet.
 
-
-We should also add variants that test with an actual XCode installation (not XCode CLT) because the SDK paths are different, and there may be other subtle differences. GH Actions install location: see https://github.com/actions/virtual-environments/blob/main/images/macos/macos-11-Readme.md#xcode
+Possible follow-up:
+- Add variants that test with an actual XCode installation (not XCode CLT) because the SDK paths are different, and there may be other subtle differences. GH Actions install location: see https://github.com/actions/virtual-environments/blob/main/images/macos/macos-11-Readme.md#xcode
 
 
 
``````




---

archive/issue_comments_528198.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b5c5e44f0d780ff34819a9f8a6eae195c589e622\">b5c5e44</a></td><td><code>Merge #33788</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/37ac84aed05a028e3411c201f4b8db35e0e0b2f9\">37ac84a</a></td><td><code>.github/workflows/ci-macos.yml: Update tested macos versions, remove test of outdated python from python.org</code></td></tr></table>\n",
    "created_at": "2022-11-11T21:51:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32570#issuecomment-528198",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:11" align="right">Comment 11</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b5c5e44f0d780ff34819a9f8a6eae195c589e622">b5c5e44</a></td><td><code>Merge #33788</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/37ac84aed05a028e3411c201f4b8db35e0e0b2f9">37ac84a</a></td><td><code>.github/workflows/ci-macos.yml: Update tested macos versions, remove test of outdated python from python.org</code></td></tr></table>




---

archive/issue_comments_528199.json:
```json
{
    "body": "Changed commit from **[37ac84a](https://github.com/sagemath/sagetrac-mirror/commit/37ac84aed05a028e3411c201f4b8db35e0e0b2f9)** to **[f185d68](https://github.com/sagemath/sagetrac-mirror/commit/f185d689989a661cf77e22471a56277f61a18dd5)**",
    "created_at": "2022-11-11T21:51:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32570#issuecomment-528199",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[37ac84a](https://github.com/sagemath/sagetrac-mirror/commit/37ac84aed05a028e3411c201f4b8db35e0e0b2f9)** to **[f185d68](https://github.com/sagemath/sagetrac-mirror/commit/f185d689989a661cf77e22471a56277f61a18dd5)**



---

archive/issue_comments_528200.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f185d689989a661cf77e22471a56277f61a18dd5\">f185d68</a></td><td><code>.github/workflows/ci-macos.yml: Update tested macos versions, remove test of outdated python from python.org</code></td></tr></table>\n",
    "created_at": "2022-11-11T21:51:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32570#issuecomment-528200",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:12" align="right">Comment 12</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f185d689989a661cf77e22471a56277f61a18dd5">f185d68</a></td><td><code>.github/workflows/ci-macos.yml: Update tested macos versions, remove test of outdated python from python.org</code></td></tr></table>




---

archive/issue_comments_528201.json:
```json
{
    "body": "Reviewer: **https://github.com/mkoeppe/sage/actions/runs/3448209170**",
    "created_at": "2022-11-11T22:03:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32570#issuecomment-528201",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **https://github.com/mkoeppe/sage/actions/runs/3448209170**



---

archive/issue_events_430569.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-11-11T22:03:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32570#event-430569"
}
```



---

archive/issue_comments_528202.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">Comment 15</div>\n\nno `macos-13` ? Is it on purpose, or due to [GitHub](../wiki/GitHub) not having it yet?",
    "created_at": "2022-11-13T10:10:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32570#issuecomment-528202",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:15" align="right">Comment 15</div>

no `macos-13` ? Is it on purpose, or due to [GitHub](../wiki/GitHub) not having it yet?



---

archive/issue_events_430570.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2022-11-13T10:23:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32570#event-430570"
}
```



---

archive/issue_events_430571.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2022-11-13T10:23:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32570#event-430571"
}
```



---

archive/issue_comments_528203.json:
```json
{
    "body": "Changed reviewer from **https://github.com/mkoeppe/sage/actions/runs/3448209170** to **Dima Pasechnik**",
    "created_at": "2022-11-13T10:23:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32570#issuecomment-528203",
    "user": "https://github.com/dimpase"
}
```

Changed reviewer from **https://github.com/mkoeppe/sage/actions/runs/3448209170** to **Dima Pasechnik**



---

archive/issue_comments_528204.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\nah, OK",
    "created_at": "2022-11-13T10:23:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32570#issuecomment-528204",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:16" align="right">Comment 16</div>

ah, OK



---

archive/issue_comments_528205.json:
```json
{
    "body": "<div id=\"comment:17\" align=\"right\">Comment 17</div>\n\nThanks!",
    "created_at": "2022-11-13T18:11:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32570#issuecomment-528205",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:17" align="right">Comment 17</div>

Thanks!



---

archive/issue_comments_528206.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/tox_ini__gh_actions__update_macos_environments](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/tox_ini__gh_actions__update_macos_environments)** to **[f185d68](https://github.com/sagemath/sagetrac-mirror/commit/f185d689989a661cf77e22471a56277f61a18dd5)**",
    "created_at": "2022-11-15T23:43:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32570#issuecomment-528206",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/tox_ini__gh_actions__update_macos_environments](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/tox_ini__gh_actions__update_macos_environments)** to **[f185d68](https://github.com/sagemath/sagetrac-mirror/commit/f185d689989a661cf77e22471a56277f61a18dd5)**



---

archive/issue_events_430572.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-11-15T23:43:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32570#event-430572"
}
```



---

archive/issue_events_430573.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "cf0b066c0d93256f206892912cd3b9a27c992230",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-11-15T23:43:12Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32570",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32570#event-430573"
}
```
