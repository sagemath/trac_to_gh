# Issue 32395: bliss's canonical_form should guard against incorrect partition

archive/issues_032158.json:
```json
{
    "assignees": [],
    "body": "make sure bliss is installed, then\n\n```\nsage: g = Graph(); g.add_edges([[0, 2]]); # 1 is not a vertex!\nsage: g.canonical_label(partition=[[0], [1], [2]], algorithm=\"bliss\") \n```\nleads to segfault.\n\nAs reported on https://groups.google.com/d/msgid/sage-devel/22ad7e6d-2175-4c7c-af1f-b9d9e09a44d6n%40googlegroups.com\n\n\nCC:  @dcoudert\n\nBranch/Commit: **[c32ed72](https://github.com/sagemath/sagetrac-mirror/commit/c32ed72483533ae36ee40a4724e6123a72a8826e)**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32395_\n\n",
    "closed_at": "2021-09-13T22:22:06Z",
    "created_at": "2021-08-18T06:08:24Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "bliss's canonical_form should guard against incorrect partition",
    "type": "issue",
    "updated_at": "2021-09-13T22:22:06Z",
    "url": "https://github.com/sagemath/sage/issues/32395",
    "user": "https://github.com/dimpase"
}
```
make sure bliss is installed, then

```
sage: g = Graph(); g.add_edges([[0, 2]]); # 1 is not a vertex!
sage: g.canonical_label(partition=[[0], [1], [2]], algorithm="bliss") 
```
leads to segfault.

As reported on https://groups.google.com/d/msgid/sage-devel/22ad7e6d-2175-4c7c-af1f-b9d9e09a44d6n%40googlegroups.com


CC:  @dcoudert

Branch/Commit: **[c32ed72](https://github.com/sagemath/sagetrac-mirror/commit/c32ed72483533ae36ee40a4724e6123a72a8826e)**

Reviewer: **Dima Pasechnik**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/32395_





---

archive/issue_events_428557.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-08-18T06:08:24Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32395",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32395#event-428557"
}
```



---

archive/issue_events_428558.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-08-18T06:08:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32395",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32395#event-428558"
}
```



---

archive/issue_events_428559.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-08-18T06:08:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32395",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32395#event-428559"
}
```



---

archive/issue_events_428560.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-08-18T06:08:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32395",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32395#event-428560"
}
```



---

archive/issue_comments_525281.json:
```json
{
    "body": "Commit: **[c32ed72](https://github.com/sagemath/sagetrac-mirror/commit/c32ed72483533ae36ee40a4724e6123a72a8826e)**",
    "created_at": "2021-08-18T16:18:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32395",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32395#issuecomment-525281",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[c32ed72](https://github.com/sagemath/sagetrac-mirror/commit/c32ed72483533ae36ee40a4724e6123a72a8826e)**



---

archive/issue_comments_525282.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nThis should do the job.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c32ed72483533ae36ee40a4724e6123a72a8826e\">c32ed72</a></td><td><code>trac #32395: guards against incorrect partitions</code></td></tr></table>\n",
    "created_at": "2021-08-18T16:18:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32395",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32395#issuecomment-525282",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:1'>Comment 1:</a>
This should do the job.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c32ed72483533ae36ee40a4724e6123a72a8826e">c32ed72</a></td><td><code>trac #32395: guards against incorrect partitions</code></td></tr></table>




---

archive/issue_comments_525283.json:
```json
{
    "body": "Branch: **[public/graphs/32395_bliss](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/32395_bliss)**",
    "created_at": "2021-08-18T16:18:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32395",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32395#issuecomment-525283",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[public/graphs/32395_bliss](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/32395_bliss)**



---

archive/issue_events_428561.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2021-08-18T16:18:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32395",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32395#event-428561"
}
```



---

archive/issue_comments_525284.json:
```json
{
    "body": "Author: **David Coudert**",
    "created_at": "2021-08-18T16:18:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32395",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32395#issuecomment-525284",
    "user": "https://github.com/dcoudert"
}
```

Author: **David Coudert**



---

archive/issue_events_428562.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-08-18T19:12:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32395",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32395#event-428562"
}
```



---

archive/issue_events_428563.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-08-18T19:12:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32395",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32395#event-428563"
}
```



---

archive/issue_comments_525285.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nThanks, looks good to me.",
    "created_at": "2021-08-18T19:12:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32395",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32395#issuecomment-525285",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:2'>Comment 2:</a>
Thanks, looks good to me.



---

archive/issue_comments_525286.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2021-08-18T19:12:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32395",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32395#issuecomment-525286",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_events_428564.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-09-13T22:22:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32395",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32395#event-428564"
}
```



---

archive/issue_events_428565.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "9d5680932ff89797a775b21e2ec03d45407444ab",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-09-13T22:22:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32395",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32395#event-428565"
}
```



---

archive/issue_comments_525287.json:
```json
{
    "body": "Changed branch from **[public/graphs/32395_bliss](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/32395_bliss)** to **[c32ed72](https://github.com/sagemath/sagetrac-mirror/commit/c32ed72483533ae36ee40a4724e6123a72a8826e)**",
    "created_at": "2021-09-13T22:22:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32395",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32395#issuecomment-525287",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/graphs/32395_bliss](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/32395_bliss)** to **[c32ed72](https://github.com/sagemath/sagetrac-mirror/commit/c32ed72483533ae36ee40a4724e6123a72a8826e)**
