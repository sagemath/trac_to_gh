# Issue 32411: Remove unnecessary uses of SR and symbolic functions in sage.algebras, sage.combinat

archive/issues_032174.json:
```json
{
    "assignees": [],
    "body": "`git grep -E '[.](symbolic|calculus|functions).*import'` reveals a number of calls to the symbolic calculus subsystem of Sage that can be avoided.\n\nExamples:\n\n- `ArikiKoikeAlgebra._BasesCategory.ParentMethods.dimension` uses `sage.functions.other.factorial` although it only needs `ZZ.factorial`\n\n- Likewise, `sage.categories.finite_dimensional_lie_algebras_with_basis` uses symbolic `binomial` but only needs `ZZ.binomial`\n\n- `ClassicalCrystals.ParentMethods.demazure_character` could be generalized to take a \"variables\" parameter so that users can obtain the result as an element of a polynomial ring instead of SR; or as a sympy expression\n\n- Many more examples like this in `sage.combinat` whenever counting formulas or similar are produced\n\n\nThis is part of Meta-ticket #29705 (modularization) - to remove unnecessary dependencies on the distribution `sagemath-symbolics` (#31695)\n\n\nCC:  @tscrim\n\nBranch/Commit: **[ec09c4e](https://github.com/sagemath/sagetrac-mirror/commit/ec09c4e9523c39fa6244861a3596f9af5919bac3)**\n\nReviewer: **Matthias Koeppe**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32411_\n\n",
    "closed_at": "2021-09-13T22:18:39Z",
    "created_at": "2021-08-24T06:18:04Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/refactoring",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove unnecessary uses of SR and symbolic functions in sage.algebras, sage.combinat",
    "type": "issue",
    "updated_at": "2021-09-13T22:18:39Z",
    "url": "https://github.com/sagemath/sage/issues/32411",
    "user": "https://github.com/mkoeppe"
}
```
`git grep -E '[.](symbolic|calculus|functions).*import'` reveals a number of calls to the symbolic calculus subsystem of Sage that can be avoided.

Examples:

- `ArikiKoikeAlgebra._BasesCategory.ParentMethods.dimension` uses `sage.functions.other.factorial` although it only needs `ZZ.factorial`

- Likewise, `sage.categories.finite_dimensional_lie_algebras_with_basis` uses symbolic `binomial` but only needs `ZZ.binomial`

- `ClassicalCrystals.ParentMethods.demazure_character` could be generalized to take a "variables" parameter so that users can obtain the result as an element of a polynomial ring instead of SR; or as a sympy expression

- Many more examples like this in `sage.combinat` whenever counting formulas or similar are produced


This is part of Meta-ticket #29705 (modularization) - to remove unnecessary dependencies on the distribution `sagemath-symbolics` (#31695)


CC:  @tscrim

Branch/Commit: **[ec09c4e](https://github.com/sagemath/sagetrac-mirror/commit/ec09c4e9523c39fa6244861a3596f9af5919bac3)**

Reviewer: **Matthias Koeppe**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/32411_





---

archive/issue_events_428726.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-24T06:18:04Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32411",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32411#event-428726"
}
```



---

archive/issue_events_428727.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-24T06:18:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32411",
    "label": "https://github.com/sagemath/sage/labels/refactoring",
    "label_color": "696969",
    "label_name": "refactoring",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32411#event-428727"
}
```



---

archive/issue_events_428728.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-24T06:18:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32411",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32411#event-428728"
}
```



---

archive/issue_events_428729.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-24T06:18:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32411",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32411#event-428729"
}
```



---

archive/issue_events_428730.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-08-24T07:31:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32411",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32411#event-428730"
}
```



---

archive/issue_comments_525512.json:
```json
{
    "body": "Branch: **[public/ticket/32411](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/32411)**",
    "created_at": "2021-08-24T07:31:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32411#issuecomment-525512",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[public/ticket/32411](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/32411)**



---

archive/issue_comments_525513.json:
```json
{
    "body": "Commit: **[4a95d8a](https://github.com/sagemath/sagetrac-mirror/commit/4a95d8a2c1e5422326ffc9ead60d228a789c8e07)**",
    "created_at": "2021-08-24T07:31:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32411#issuecomment-525513",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[4a95d8a](https://github.com/sagemath/sagetrac-mirror/commit/4a95d8a2c1e5422326ffc9ead60d228a789c8e07)**



---

archive/issue_comments_525514.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4a95d8a2c1e5422326ffc9ead60d228a789c8e07\">4a95d8a</a></td><td><code>changing imports of binomial, factorial and floor in algebras and combinat</code></td></tr></table>\n",
    "created_at": "2021-08-24T07:31:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32411#issuecomment-525514",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4a95d8a2c1e5422326ffc9ead60d228a789c8e07">4a95d8a</a></td><td><code>changing imports of binomial, factorial and floor in algebras and combinat</code></td></tr></table>




---

archive/issue_comments_525515.json:
```json
{
    "body": "Changed commit from **[4a95d8a](https://github.com/sagemath/sagetrac-mirror/commit/4a95d8a2c1e5422326ffc9ead60d228a789c8e07)** to **[c1485e5](https://github.com/sagemath/sagetrac-mirror/commit/c1485e5ee14d9cbe5365dde32c4c3f9a114d5753)**",
    "created_at": "2021-08-24T08:01:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32411#issuecomment-525515",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[4a95d8a](https://github.com/sagemath/sagetrac-mirror/commit/4a95d8a2c1e5422326ffc9ead60d228a789c8e07)** to **[c1485e5](https://github.com/sagemath/sagetrac-mirror/commit/c1485e5ee14d9cbe5365dde32c4c3f9a114d5753)**



---

archive/issue_comments_525516.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c1485e5ee14d9cbe5365dde32c4c3f9a114d5753\">c1485e5</a></td><td><code>remove one import of SR in combinat</code></td></tr></table>\n",
    "created_at": "2021-08-24T08:01:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32411#issuecomment-525516",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c1485e5ee14d9cbe5365dde32c4c3f9a114d5753">c1485e5</a></td><td><code>remove one import of SR in combinat</code></td></tr></table>




---

archive/issue_comments_525517.json:
```json
{
    "body": "Changed commit from **[c1485e5](https://github.com/sagemath/sagetrac-mirror/commit/c1485e5ee14d9cbe5365dde32c4c3f9a114d5753)** to **[ad0b243](https://github.com/sagemath/sagetrac-mirror/commit/ad0b243da7b31da1f2b99506b81d0f3436f32e12)**",
    "created_at": "2021-08-24T11:21:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32411#issuecomment-525517",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[c1485e5](https://github.com/sagemath/sagetrac-mirror/commit/c1485e5ee14d9cbe5365dde32c4c3f9a114d5753)** to **[ad0b243](https://github.com/sagemath/sagetrac-mirror/commit/ad0b243da7b31da1f2b99506b81d0f3436f32e12)**



---

archive/issue_comments_525518.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ad0b243da7b31da1f2b99506b81d0f3436f32e12\">ad0b243</a></td><td><code>fix typo</code></td></tr></table>\n",
    "created_at": "2021-08-24T11:21:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32411#issuecomment-525518",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ad0b243da7b31da1f2b99506b81d0f3436f32e12">ad0b243</a></td><td><code>fix typo</code></td></tr></table>




---

archive/issue_comments_525519.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ec09c4e9523c39fa6244861a3596f9af5919bac3\">ec09c4e</a></td><td><code>fix in cluster</code></td></tr></table>\n",
    "created_at": "2021-08-24T14:39:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32411#issuecomment-525519",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:5" align="right">Comment 5</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ec09c4e9523c39fa6244861a3596f9af5919bac3">ec09c4e</a></td><td><code>fix in cluster</code></td></tr></table>




---

archive/issue_comments_525520.json:
```json
{
    "body": "Changed commit from **[ad0b243](https://github.com/sagemath/sagetrac-mirror/commit/ad0b243da7b31da1f2b99506b81d0f3436f32e12)** to **[ec09c4e](https://github.com/sagemath/sagetrac-mirror/commit/ec09c4e9523c39fa6244861a3596f9af5919bac3)**",
    "created_at": "2021-08-24T14:39:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32411#issuecomment-525520",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[ad0b243](https://github.com/sagemath/sagetrac-mirror/commit/ad0b243da7b31da1f2b99506b81d0f3436f32e12)** to **[ec09c4e](https://github.com/sagemath/sagetrac-mirror/commit/ec09c4e9523c39fa6244861a3596f9af5919bac3)**



---

archive/issue_comments_525521.json:
```json
{
    "body": "Author: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2021-08-24T17:56:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32411#issuecomment-525521",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Frédéric Chapoton**



---

archive/issue_events_428731.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-24T17:57:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32411",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32411#event-428731"
}
```



---

archive/issue_events_428732.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-24T17:57:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32411",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32411#event-428732"
}
```



---

archive/issue_comments_525522.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nThanks a lot, LGTM.",
    "created_at": "2021-08-24T17:57:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32411#issuecomment-525522",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:7" align="right">Comment 7</div>

Thanks a lot, LGTM.



---

archive/issue_events_428733.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-24T17:58:57Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/32411",
    "title_is": "Remove unnecessary uses of SR and symbolic functions in sage.algebras, sage.combinat",
    "title_was": "Remove unnecessary uses of SR and symbolic functions",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32411#event-428733"
}
```



---

archive/issue_comments_525523.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2021-08-24T17:59:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32411#issuecomment-525523",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_comments_525524.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -11,5 +11,5 @@\n - Many more examples like this in `sage.combinat` whenever counting formulas or similar are produced\n \n \n-This is part of Meta-ticket #29705 (modularization) - to remove unnecessary dependencies on the distribution `sagemath-symbolics`\n+This is part of Meta-ticket #29705 (modularization) - to remove unnecessary dependencies on the distribution `sagemath-symbolics` (#31695)\n \n``````\n",
    "created_at": "2021-08-24T18:01:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32411#issuecomment-525524",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -11,5 +11,5 @@
 - Many more examples like this in `sage.combinat` whenever counting formulas or similar are produced
 
 
-This is part of Meta-ticket #29705 (modularization) - to remove unnecessary dependencies on the distribution `sagemath-symbolics`
+This is part of Meta-ticket #29705 (modularization) - to remove unnecessary dependencies on the distribution `sagemath-symbolics` (#31695)
 
``````




---

archive/issue_comments_525525.json:
```json
{
    "body": "Changed branch from **[public/ticket/32411](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/32411)** to **[ec09c4e](https://github.com/sagemath/sagetrac-mirror/commit/ec09c4e9523c39fa6244861a3596f9af5919bac3)**",
    "created_at": "2021-09-13T22:18:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32411#issuecomment-525525",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/ticket/32411](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/32411)** to **[ec09c4e](https://github.com/sagemath/sagetrac-mirror/commit/ec09c4e9523c39fa6244861a3596f9af5919bac3)**



---

archive/issue_events_428734.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-09-13T22:18:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32411",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32411#event-428734"
}
```



---

archive/issue_events_428735.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "48872d39acbfcafe98bc4bf11618448d3642907a",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-09-13T22:18:39Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32411",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32411#event-428735"
}
```
