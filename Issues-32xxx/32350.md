# Issue 32350: Fix unstable numeric doctest of LU decomposition

archive/issues_032113.json:
```json
{
    "assignees": [],
    "body": "This doctest can fail due to floating point precision problems:\n\n```\nFile \"src/sage/matrix/matrix_double_dense.pyx\", line 1112, in sage.matrix.matrix_double_dense.Matrix_double_dense.LU\nFailed example:\n    m == P*L*U\nExpected:\n    True\nGot:\n    False\n```\n\n\nCC:  @DavidAyotte\n\nBranch/Commit: **[da32e65](https://github.com/sagemath/sagetrac-mirror/commit/da32e650460ca5f799a51c82b2eb19ae154e4dbb)**\n\nReviewer: **David Ayotte**\n\nAuthor: **Markus Wageringel**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32350_\n\n",
    "closed_at": "2021-09-07T20:52:19Z",
    "created_at": "2021-08-09T18:53:08Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20numerical",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix unstable numeric doctest of LU decomposition",
    "type": "issue",
    "updated_at": "2021-09-07T20:52:19Z",
    "url": "https://github.com/sagemath/sage/issues/32350",
    "user": "https://github.com/mwageringel"
}
```
This doctest can fail due to floating point precision problems:

```
File "src/sage/matrix/matrix_double_dense.pyx", line 1112, in sage.matrix.matrix_double_dense.Matrix_double_dense.LU
Failed example:
    m == P*L*U
Expected:
    True
Got:
    False
```


CC:  @DavidAyotte

Branch/Commit: **[da32e65](https://github.com/sagemath/sagetrac-mirror/commit/da32e650460ca5f799a51c82b2eb19ae154e4dbb)**

Reviewer: **David Ayotte**

Author: **Markus Wageringel**

_Issue created by migration from https://trac.sagemath.org/ticket/32350_





---

archive/issue_events_427946.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2021-08-09T18:53:08Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32350",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32350#event-427946"
}
```



---

archive/issue_events_427947.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2021-08-09T18:53:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32350",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20numerical",
    "label_color": "0000ff",
    "label_name": "component: numerical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32350#event-427947"
}
```



---

archive/issue_events_427948.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2021-08-09T18:53:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32350",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32350#event-427948"
}
```



---

archive/issue_events_427949.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2021-08-09T18:53:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32350",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32350#event-427949"
}
```



---

archive/issue_comments_524220.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/da32e650460ca5f799a51c82b2eb19ae154e4dbb\">da32e65</a></td><td><code>fix unstable numeric doctest of LU decomposition</code></td></tr></table>\n",
    "created_at": "2021-08-09T18:54:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32350#issuecomment-524220",
    "user": "https://github.com/mwageringel"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/da32e650460ca5f799a51c82b2eb19ae154e4dbb">da32e65</a></td><td><code>fix unstable numeric doctest of LU decomposition</code></td></tr></table>




---

archive/issue_comments_524221.json:
```json
{
    "body": "Author: **Markus Wageringel**",
    "created_at": "2021-08-09T18:54:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32350#issuecomment-524221",
    "user": "https://github.com/mwageringel"
}
```

Author: **Markus Wageringel**



---

archive/issue_comments_524222.json:
```json
{
    "body": "Branch: **[u/gh-mwageringel/32350](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/32350)**",
    "created_at": "2021-08-09T18:54:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32350#issuecomment-524222",
    "user": "https://github.com/mwageringel"
}
```

Branch: **[u/gh-mwageringel/32350](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/32350)**



---

archive/issue_events_427950.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2021-08-09T18:54:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32350",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32350#event-427950"
}
```



---

archive/issue_comments_524223.json:
```json
{
    "body": "Commit: **[da32e65](https://github.com/sagemath/sagetrac-mirror/commit/da32e650460ca5f799a51c82b2eb19ae154e4dbb)**",
    "created_at": "2021-08-09T18:54:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32350#issuecomment-524223",
    "user": "https://github.com/mwageringel"
}
```

Commit: **[da32e65](https://github.com/sagemath/sagetrac-mirror/commit/da32e650460ca5f799a51c82b2eb19ae154e4dbb)**



---

archive/issue_events_427951.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2021-08-09T18:54:27Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/32350",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32350#event-427951"
}
```



---

archive/issue_events_427952.json:
```json
{
    "actor": "https://github.com/mwageringel",
    "created_at": "2021-08-09T18:54:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32350",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32350#event-427952"
}
```



---

archive/issue_events_427953.json:
```json
{
    "actor": "https://github.com/DavidAyotte",
    "created_at": "2021-08-10T17:35:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32350",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32350#event-427953"
}
```



---

archive/issue_events_427954.json:
```json
{
    "actor": "https://github.com/DavidAyotte",
    "created_at": "2021-08-10T17:35:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32350",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32350#event-427954"
}
```



---

archive/issue_comments_524224.json:
```json
{
    "body": "Reviewer: **David Ayotte**",
    "created_at": "2021-08-10T17:35:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32350#issuecomment-524224",
    "user": "https://github.com/DavidAyotte"
}
```

Reviewer: **David Ayotte**



---

archive/issue_comments_524225.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nLGTM.",
    "created_at": "2021-08-10T17:35:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32350#issuecomment-524225",
    "user": "https://github.com/DavidAyotte"
}
```

<a id='comment:2'>Comment 2:</a>
LGTM.



---

archive/issue_comments_524226.json:
```json
{
    "body": "Changed branch from **[u/gh-mwageringel/32350](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/32350)** to **[da32e65](https://github.com/sagemath/sagetrac-mirror/commit/da32e650460ca5f799a51c82b2eb19ae154e4dbb)**",
    "created_at": "2021-09-07T20:52:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32350",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32350#issuecomment-524226",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/gh-mwageringel/32350](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mwageringel/32350)** to **[da32e65](https://github.com/sagemath/sagetrac-mirror/commit/da32e650460ca5f799a51c82b2eb19ae154e4dbb)**



---

archive/issue_events_427955.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-09-07T20:52:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32350",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32350#event-427955"
}
```



---

archive/issue_events_427956.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "b24543d286a432749f6909dc1f3e957e5bd72fff",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-09-07T20:52:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32350",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32350#event-427956"
}
```
