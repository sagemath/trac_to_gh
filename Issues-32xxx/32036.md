...
```json
{
    "body": "There are a few remaining uses of `SAGE_SHARE` in `src/sage` that will have to be eliminated though.",
...
```json
{
    "body": "(see #30819 `sage.databases`: Use `sage.feature` to define database file system location)",
...
```json
{
    "body": "Perhaps now is also the time that we do #30914 (Meta-ticket: Create upstream repositories, pip-installable packages for database packages)?",
...
```json
{
    "body": "No, `SAGE_ROOT` is not the right choice there.",
...
```json
{
    "body": "Perhaps someone who knows more about GAP workspaces can comment whether this change makes sense.",
...
```json
{
    "body": "I am a bit confused by what `dist.py` does. If it is doing an installation task, should it live in `sage_setup`?",
...
```json
{
    "body": "For the remaining items we have separate open tickets listed in #30818",
...
```json
{
    "body": "Should I still provide a commit for `lie` here or add a new ticket in #30818?",
...
```json
{
    "body": "And here is the LiE stuff. \n\n---\nNew commits:",
...
```json
{
    "body": "`CONWAY_POLYNOMIALS_DATA_DIR` can just be set to the correct location in `sage_conf`",
...
```json
{
    "body": "I have opened #32055 for the task of adding an `spkg-configure.m4` for `lie`. I don't think a fallback is needed",
...
```json
{
    "body": "I'll add a feature `DatabaseConwayPolynomials`",
...
```json
{
    "body": "I don't get sci-mathematics/lie on my Gentoo box, is it not yet in Gentoo?",
...
```json
{
    "body": "Do you set `SAGE_LOCAL` in your distribution packaging?",
...
```json
{
    "body": "I have removed the initial commit, so the fallback is back. The other changes made here have independent value, so let's get them in.",
...
```json
{
    "body": "I am otherwise quite happy with the state of the ticket. I would have prefered the initial commit to stay in but we can do that later.",
...
```json
{
    "body": "I see, we forgot to replace `sage_local_lib` which was defined as `os.path.join(SAGE_LOCAL, 'lib')` by expressions based on the new `libdir` in the darwin section. That was a bit careless of me not to notice.",
...
```json
{
    "body": "Yes, looks like I fell asleep in the middle of rewriting this function.",
    "created_at": "2021-07-06T13:14:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32036",
    "type": "issue_comment",
...
```json
{
    "body": "Didn't we take care of this in #32057?",
...
```json
{
    "body": "Right, thanks. So, as discussed in #31338#comment:18, we're back to having to solve the same problem as #31578.",
...
```json
{
    "body": "I think I will just adopt the commit you removed for Antonio. `sage_conf` is not really designed to set a variable to `None`, that cannot work and it is even probably undesirable.",
