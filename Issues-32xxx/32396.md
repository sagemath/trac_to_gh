# Issue 32396: Absolute value function for scalar fields

archive/issues_032159.json:
```json
{
    "assignees": [],
    "body": "Currently, the `abs` operator cannot be applied to scalar fields:\n\n```\nsage: M = Manifold(2, 'M', structure='topological')\nsage: X.<x,y> = M.chart()\nsage: f = M.scalar_field({X: x*y}, name='f', latex_name=r\"\\Phi\")\nsage: abs(f)\nTraceback (most recent call last)\n...\nTypeError: bad operand type for abs(): 'ScalarFieldAlgebra_with_category.element_class'\n```\n\nIn this ticket, we add this feature.\n\n\nCC:  @egourgoulhon @tscrim\n\nBranch/Commit: **[2095de3](https://github.com/sagemath/sagetrac-mirror/commit/2095de3117eb3a6cee9f4a928d0b8fd2e4496e85)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Michael Jung**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32396_\n\n",
    "closed_at": "2021-09-13T22:22:04Z",
    "created_at": "2021-08-18T13:01:02Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20manifolds",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Absolute value function for scalar fields",
    "type": "issue",
    "updated_at": "2021-09-13T22:22:04Z",
    "url": "https://github.com/sagemath/sage/issues/32396",
    "user": "https://github.com/mjungmath"
}
```
Currently, the `abs` operator cannot be applied to scalar fields:

```
sage: M = Manifold(2, 'M', structure='topological')
sage: X.<x,y> = M.chart()
sage: f = M.scalar_field({X: x*y}, name='f', latex_name=r"\Phi")
sage: abs(f)
Traceback (most recent call last)
...
TypeError: bad operand type for abs(): 'ScalarFieldAlgebra_with_category.element_class'
```

In this ticket, we add this feature.


CC:  @egourgoulhon @tscrim

Branch/Commit: **[2095de3](https://github.com/sagemath/sagetrac-mirror/commit/2095de3117eb3a6cee9f4a928d0b8fd2e4496e85)**

Reviewer: **Travis Scrimshaw**

Author: **Michael Jung**

_Issue created by migration from https://trac.sagemath.org/ticket/32396_





---

archive/issue_events_428570.json:
```json
{
    "actor": "https://github.com/mjungmath",
    "created_at": "2021-08-18T13:01:02Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32396#event-428570"
}
```



---

archive/issue_events_428571.json:
```json
{
    "actor": "https://github.com/mjungmath",
    "created_at": "2021-08-18T13:01:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20manifolds",
    "label_color": "0000ff",
    "label_name": "component: manifolds",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32396#event-428571"
}
```



---

archive/issue_events_428572.json:
```json
{
    "actor": "https://github.com/mjungmath",
    "created_at": "2021-08-18T13:01:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32396#event-428572"
}
```



---

archive/issue_events_428573.json:
```json
{
    "actor": "https://github.com/mjungmath",
    "created_at": "2021-08-18T13:01:02Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32396#event-428573"
}
```



---

archive/issue_comments_525286.json:
```json
{
    "body": "Branch: **[u/gh-mjungmath/abs_function_for_scalar_fields](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mjungmath/abs_function_for_scalar_fields)**",
    "created_at": "2021-08-18T13:02:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32396#issuecomment-525286",
    "user": "https://github.com/mjungmath"
}
```

Branch: **[u/gh-mjungmath/abs_function_for_scalar_fields](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mjungmath/abs_function_for_scalar_fields)**



---

archive/issue_comments_525287.json:
```json
{
    "body": "Commit: **[fcd64c9](https://github.com/sagemath/sagetrac-mirror/commit/fcd64c9c1ecc5e68ec5e42125f7d403138fe7b0a)**",
    "created_at": "2021-08-18T13:02:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32396#issuecomment-525287",
    "user": "https://github.com/mjungmath"
}
```

Commit: **[fcd64c9](https://github.com/sagemath/sagetrac-mirror/commit/fcd64c9c1ecc5e68ec5e42125f7d403138fe7b0a)**



---

archive/issue_comments_525288.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/fcd64c9c1ecc5e68ec5e42125f7d403138fe7b0a\">fcd64c9</a></td><td><code>Trac #32396: add `__abs__` to chart_func and scalarfield</code></td></tr></table>\n",
    "created_at": "2021-08-18T13:02:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32396#issuecomment-525288",
    "user": "https://github.com/mjungmath"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/fcd64c9c1ecc5e68ec5e42125f7d403138fe7b0a">fcd64c9</a></td><td><code>Trac #32396: add `__abs__` to chart_func and scalarfield</code></td></tr></table>




---

archive/issue_events_428574.json:
```json
{
    "actor": "https://github.com/mjungmath",
    "created_at": "2021-08-18T13:02:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32396#event-428574"
}
```



---

archive/issue_comments_525289.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,10 +1,10 @@\n Currently, the `abs` operator cannot be applied to scalar fields:\n \n ```\n-sage:             sage: M = Manifold(2, 'M', structure='topological')\n-....:             sage: X.<x,y> = M.chart()\n-....:             sage: f = M.scalar_field({X: x*y}, name='f', latex_name=r\"\\Phi\")\n-....: abs(f)\n+sage: M = Manifold(2, 'M', structure='topological')\n+....: X.<x,y> = M.chart()\n+....: f = M.scalar_field({X: x*y}, name='f', latex_name=r\"\\Phi\")\n+sage: abs(f)\n Traceback (most recent call last)\n ...\n TypeError: bad operand type for abs(): 'ScalarFieldAlgebra_with_category.element_class'\n``````\n",
    "created_at": "2021-08-18T13:03:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32396#issuecomment-525289",
    "user": "https://github.com/mjungmath"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,10 +1,10 @@
 Currently, the `abs` operator cannot be applied to scalar fields:
 
 ```
-sage:             sage: M = Manifold(2, 'M', structure='topological')
-....:             sage: X.<x,y> = M.chart()
-....:             sage: f = M.scalar_field({X: x*y}, name='f', latex_name=r"\Phi")
-....: abs(f)
+sage: M = Manifold(2, 'M', structure='topological')
+....: X.<x,y> = M.chart()
+....: f = M.scalar_field({X: x*y}, name='f', latex_name=r"\Phi")
+sage: abs(f)
 Traceback (most recent call last)
 ...
 TypeError: bad operand type for abs(): 'ScalarFieldAlgebra_with_category.element_class'
``````




---

archive/issue_comments_525290.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,8 +2,8 @@\n \n ```\n sage: M = Manifold(2, 'M', structure='topological')\n-....: X.<x,y> = M.chart()\n-....: f = M.scalar_field({X: x*y}, name='f', latex_name=r\"\\Phi\")\n+sage: X.<x,y> = M.chart()\n+sage: f = M.scalar_field({X: x*y}, name='f', latex_name=r\"\\Phi\")\n sage: abs(f)\n Traceback (most recent call last)\n ...\n``````\n",
    "created_at": "2021-08-18T13:03:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32396#issuecomment-525290",
    "user": "https://github.com/mjungmath"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,8 +2,8 @@
 
 ```
 sage: M = Manifold(2, 'M', structure='topological')
-....: X.<x,y> = M.chart()
-....: f = M.scalar_field({X: x*y}, name='f', latex_name=r"\Phi")
+sage: X.<x,y> = M.chart()
+sage: f = M.scalar_field({X: x*y}, name='f', latex_name=r"\Phi")
 sage: abs(f)
 Traceback (most recent call last)
 ...
``````




---

archive/issue_comments_525291.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2021-08-19T08:11:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32396#issuecomment-525291",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_525292.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nLGTM.",
    "created_at": "2021-08-19T08:11:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32396#issuecomment-525292",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:5" align="right">Comment 5</div>

LGTM.



---

archive/issue_events_428575.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-08-19T08:11:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32396#event-428575"
}
```



---

archive/issue_events_428576.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-08-19T08:11:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32396#event-428576"
}
```



---

archive/issue_comments_525293.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nThank you for the review!",
    "created_at": "2021-08-19T08:36:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32396#issuecomment-525293",
    "user": "https://github.com/mjungmath"
}
```

<div id="comment:6" align="right">Comment 6</div>

Thank you for the review!



---

archive/issue_comments_525294.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nThanks for implementing this. Any reason for the choice of `Abs` rather than `abs` for the function name?",
    "created_at": "2021-08-19T08:46:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32396#issuecomment-525294",
    "user": "https://github.com/egourgoulhon"
}
```

<div id="comment:7" align="right">Comment 7</div>

Thanks for implementing this. Any reason for the choice of `Abs` rather than `abs` for the function name?



---

archive/issue_comments_525295.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nNo particular reason. Do you prefer `abs` or maybe `|.|`?",
    "created_at": "2021-08-19T08:49:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32396#issuecomment-525295",
    "user": "https://github.com/mjungmath"
}
```

<div id="comment:8" align="right">Comment 8</div>

No particular reason. Do you prefer `abs` or maybe `|.|`?



---

archive/issue_comments_525296.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nReplying to [@mjungmath](#comment%3A8):\n> No particular reason. Do you prefer `abs` or maybe `|.|`?\n\nI don't have any strong preference. `abs` sounds more inline with other function names in Sage, while `|.|` is closer to mathematical notation, so I would slightly prefer the latter. Travis, what do you think?",
    "created_at": "2021-08-19T09:47:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32396#issuecomment-525296",
    "user": "https://github.com/egourgoulhon"
}
```

<div id="comment:9" align="right">Comment 9</div>

Replying to [@mjungmath](#comment%3A8):
> No particular reason. Do you prefer `abs` or maybe `|.|`?

I don't have any strong preference. `abs` sounds more inline with other function names in Sage, while `|.|` is closer to mathematical notation, so I would slightly prefer the latter. Travis, what do you think?



---

archive/issue_comments_525297.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nI am slightly worried about possible ambiguity with `|.|`. I might go with `abs` over `Abs` just because of `\\lvert` compared to `\\lVert` and to match Sages's `abs`. However, I didn't (and still don't) really care for which one as it is clear documentation-wise what it is doing (and hidden from the causal user). So I am not the person to ask for a real opinion on this.",
    "created_at": "2021-08-19T10:00:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32396#issuecomment-525297",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:10" align="right">Comment 10</div>

I am slightly worried about possible ambiguity with `|.|`. I might go with `abs` over `Abs` just because of `\lvert` compared to `\lVert` and to match Sages's `abs`. However, I didn't (and still don't) really care for which one as it is clear documentation-wise what it is doing (and hidden from the causal user). So I am not the person to ask for a real opinion on this.



---

archive/issue_comments_525298.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nBranch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2095de3117eb3a6cee9f4a928d0b8fd2e4496e85\">2095de3</a></td><td><code>Trac #32396: adjust abs function name to general Sage convention</code></td></tr></table>\n",
    "created_at": "2021-08-19T21:46:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32396#issuecomment-525298",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:11" align="right">Comment 11</div>

Branch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2095de3117eb3a6cee9f4a928d0b8fd2e4496e85">2095de3</a></td><td><code>Trac #32396: adjust abs function name to general Sage convention</code></td></tr></table>




---

archive/issue_events_428577.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2021-08-19T21:46:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32396#event-428577"
}
```



---

archive/issue_events_428578.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2021-08-19T21:46:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32396#event-428578"
}
```



---

archive/issue_comments_525299.json:
```json
{
    "body": "Changed commit from **[fcd64c9](https://github.com/sagemath/sagetrac-mirror/commit/fcd64c9c1ecc5e68ec5e42125f7d403138fe7b0a)** to **[2095de3](https://github.com/sagemath/sagetrac-mirror/commit/2095de3117eb3a6cee9f4a928d0b8fd2e4496e85)**",
    "created_at": "2021-08-19T21:46:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32396#issuecomment-525299",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[fcd64c9](https://github.com/sagemath/sagetrac-mirror/commit/fcd64c9c1ecc5e68ec5e42125f7d403138fe7b0a)** to **[2095de3](https://github.com/sagemath/sagetrac-mirror/commit/2095de3117eb3a6cee9f4a928d0b8fd2e4496e85)**



---

archive/issue_comments_525300.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nAlright, I changed it to `abs` with small letters. I find it a strong argument to say that this follows general Sage convention.",
    "created_at": "2021-08-19T21:46:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32396#issuecomment-525300",
    "user": "https://github.com/mjungmath"
}
```

<div id="comment:12" align="right">Comment 12</div>

Alright, I changed it to `abs` with small letters. I find it a strong argument to say that this follows general Sage convention.



---

archive/issue_events_428579.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-08-19T23:22:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32396#event-428579"
}
```



---

archive/issue_events_428580.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-08-19T23:22:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32396#event-428580"
}
```



---

archive/issue_comments_525301.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nThanks. I am setting back to a positive review.",
    "created_at": "2021-08-19T23:22:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32396#issuecomment-525301",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:13" align="right">Comment 13</div>

Thanks. I am setting back to a positive review.



---

archive/issue_events_428581.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-20T03:26:59Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "title_is": "Absolute value function for scalar fields",
    "title_was": "Abolute value function for scalar fields",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32396#event-428581"
}
```



---

archive/issue_comments_525302.json:
```json
{
    "body": "Changed branch from **[u/gh-mjungmath/abs_function_for_scalar_fields](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mjungmath/abs_function_for_scalar_fields)** to **[2095de3](https://github.com/sagemath/sagetrac-mirror/commit/2095de3117eb3a6cee9f4a928d0b8fd2e4496e85)**",
    "created_at": "2021-09-13T22:22:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32396#issuecomment-525302",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/gh-mjungmath/abs_function_for_scalar_fields](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-mjungmath/abs_function_for_scalar_fields)** to **[2095de3](https://github.com/sagemath/sagetrac-mirror/commit/2095de3117eb3a6cee9f4a928d0b8fd2e4496e85)**



---

archive/issue_events_428582.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-09-13T22:22:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32396#event-428582"
}
```



---

archive/issue_events_428583.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "4499a675d9ed4acce567818cfdbce3a5ae68b838",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-09-13T22:22:04Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32396",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32396#event-428583"
}
```
