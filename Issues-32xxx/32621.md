# Issue 32621: Replace imports from sage.misc.all by more specific imports

archive/issues_032384.json:
```json
{
    "assignees": [],
    "body": "(cherry-picked from #32432)\n\nCC:  @tscrim @kliem\n\nBranch/Commit: **[43ed0ca](https://github.com/sagemath/sagetrac-mirror/commit/43ed0cadadf02c48c31990b62817ef53d4eef15d)**\n\nReviewer: **Jonathan Kliem**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32621_\n\n",
    "closed_at": "2021-10-13T22:49:59Z",
    "created_at": "2021-10-04T06:25:55Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/refactoring",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Replace imports from sage.misc.all by more specific imports",
    "type": "issue",
    "updated_at": "2021-10-13T22:49:59Z",
    "url": "https://github.com/sagemath/sage/issues/32621",
    "user": "https://github.com/mkoeppe"
}
```
(cherry-picked from #32432)

CC:  @tscrim @kliem

Branch/Commit: **[43ed0ca](https://github.com/sagemath/sagetrac-mirror/commit/43ed0cadadf02c48c31990b62817ef53d4eef15d)**

Reviewer: **Jonathan Kliem**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/32621_





---

archive/issue_events_431165.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T06:25:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32621",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32621#event-431165"
}
```



---

archive/issue_events_431166.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T06:25:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32621",
    "label": "https://github.com/sagemath/sage/labels/refactoring",
    "label_color": "696969",
    "label_name": "refactoring",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32621#event-431166"
}
```



---

archive/issue_events_431167.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T06:25:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32621",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32621#event-431167"
}
```



---

archive/issue_events_431168.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T06:25:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32621",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32621#event-431168"
}
```



---

archive/issue_comments_529144.json:
```json
{
    "body": "Branch: **[u/mkoeppe/replace_imports_from_sage_misc_all_by_more_specific_imports](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/replace_imports_from_sage_misc_all_by_more_specific_imports)**",
    "created_at": "2021-10-04T06:28:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32621#issuecomment-529144",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/replace_imports_from_sage_misc_all_by_more_specific_imports](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/replace_imports_from_sage_misc_all_by_more_specific_imports)**



---

archive/issue_comments_529145.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0b06521bbe9ff8075d7158f7266657785a02da93\">0b06521</a></td><td><code>src/sage/sets/disjoint_union_enumerated_sets.py: Remove import from sage.misc.all</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/43ed0cadadf02c48c31990b62817ef53d4eef15d\">43ed0ca</a></td><td><code>git grep -l 'all import cputime' src/sage | xargs sed -E -i.bak $'s/^( *)from sage.*all import (cputime) *$/\\1from sage.misc.misc import \\2/'</code></td></tr></table>\n",
    "created_at": "2021-10-04T06:28:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32621#issuecomment-529145",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0b06521bbe9ff8075d7158f7266657785a02da93">0b06521</a></td><td><code>src/sage/sets/disjoint_union_enumerated_sets.py: Remove import from sage.misc.all</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/43ed0cadadf02c48c31990b62817ef53d4eef15d">43ed0ca</a></td><td><code>git grep -l 'all import cputime' src/sage | xargs sed -E -i.bak $'s/^( *)from sage.*all import (cputime) *$/\1from sage.misc.misc import \2/'</code></td></tr></table>




---

archive/issue_events_431169.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T06:28:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32621",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32621#event-431169"
}
```



---

archive/issue_comments_529146.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2021-10-04T06:28:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32621#issuecomment-529146",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_comments_529147.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-\n+(cherry-picked from #32432)\n``````\n",
    "created_at": "2021-10-04T06:28:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32621#issuecomment-529147",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-
+(cherry-picked from #32432)
``````




---

archive/issue_comments_529148.json:
```json
{
    "body": "Commit: **[43ed0ca](https://github.com/sagemath/sagetrac-mirror/commit/43ed0cadadf02c48c31990b62817ef53d4eef15d)**",
    "created_at": "2021-10-04T06:28:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32621#issuecomment-529148",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[43ed0ca](https://github.com/sagemath/sagetrac-mirror/commit/43ed0cadadf02c48c31990b62817ef53d4eef15d)**



---

archive/issue_comments_529149.json:
```json
{
    "body": "Reviewer: **Jonathan Kliem**",
    "created_at": "2021-10-04T16:33:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32621#issuecomment-529149",
    "user": "https://github.com/kliem"
}
```

Reviewer: **Jonathan Kliem**



---

archive/issue_events_431170.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2021-10-04T16:33:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32621",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32621#event-431170"
}
```



---

archive/issue_events_431171.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2021-10-04T16:33:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32621",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32621#event-431171"
}
```



---

archive/issue_comments_529150.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nLGTM.",
    "created_at": "2021-10-04T16:33:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32621#issuecomment-529150",
    "user": "https://github.com/kliem"
}
```

<div id="comment:4" align="right">Comment 4</div>

LGTM.



---

archive/issue_comments_529151.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThanks!",
    "created_at": "2021-10-04T19:32:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32621#issuecomment-529151",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:5" align="right">Comment 5</div>

Thanks!



---

archive/issue_events_431172.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-10-13T22:49:59Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32621",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32621#event-431172"
}
```



---

archive/issue_events_431173.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "4b50b849c0f3df46ecc67cc8394e42e582dbaa95",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-10-13T22:49:59Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32621",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32621#event-431173"
}
```



---

archive/issue_comments_529152.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/replace_imports_from_sage_misc_all_by_more_specific_imports](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/replace_imports_from_sage_misc_all_by_more_specific_imports)** to **[43ed0ca](https://github.com/sagemath/sagetrac-mirror/commit/43ed0cadadf02c48c31990b62817ef53d4eef15d)**",
    "created_at": "2021-10-13T22:49:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32621",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32621#issuecomment-529152",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/replace_imports_from_sage_misc_all_by_more_specific_imports](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/replace_imports_from_sage_misc_all_by_more_specific_imports)** to **[43ed0ca](https://github.com/sagemath/sagetrac-mirror/commit/43ed0cadadf02c48c31990b62817ef53d4eef15d)**
