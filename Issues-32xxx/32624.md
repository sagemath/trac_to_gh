# Issue 32624: Update imports of Family

archive/issues_032387.json:
```json
{
    "assignees": [],
    "body": "These should all import from `sage.sets.family`:\n\n```\n$ git grep sage[.]combinat[.]family\nsrc/sage/algebras/iwahori_hecke_algebra.py:from sage.combinat.family import Family\nsrc/sage/categories/modules_with_basis.py:            from sage.combinat.family import Family\nsrc/sage/categories/modules_with_basis.py:                    from sage.combinat.family import Family\nsrc/sage/combinat/chas/fsym.py:            from sage.combinat.family import Family\nsrc/sage/combinat/fqsym.py:            from sage.combinat.family import Family\nsrc/sage/combinat/root_system/type_E.py:from sage.combinat.family import Family\nsrc/sage/combinat/root_system/type_F.py:from sage.combinat.family import Family\nsrc/sage/combinat/root_system/weight_lattice_realizations.py:from sage.combinat.family import Family\nsrc/sage/libs/coxeter3/coxeter_group.py:        from sage.combinat.family import Family\n```\n\n\nCC:  @tscrim\n\nBranch/Commit: **[efb139a](https://github.com/sagemath/sagetrac-mirror/commit/efb139a95a9331c08708ec3b828374c7a6fe89ac)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32624_\n\n",
    "closed_at": "2021-10-19T20:32:09Z",
    "created_at": "2021-10-04T16:35:51Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/refactoring",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Update imports of Family",
    "type": "issue",
    "updated_at": "2021-10-19T20:32:09Z",
    "url": "https://github.com/sagemath/sage/issues/32624",
    "user": "https://github.com/mkoeppe"
}
```
These should all import from `sage.sets.family`:

```
$ git grep sage[.]combinat[.]family
src/sage/algebras/iwahori_hecke_algebra.py:from sage.combinat.family import Family
src/sage/categories/modules_with_basis.py:            from sage.combinat.family import Family
src/sage/categories/modules_with_basis.py:                    from sage.combinat.family import Family
src/sage/combinat/chas/fsym.py:            from sage.combinat.family import Family
src/sage/combinat/fqsym.py:            from sage.combinat.family import Family
src/sage/combinat/root_system/type_E.py:from sage.combinat.family import Family
src/sage/combinat/root_system/type_F.py:from sage.combinat.family import Family
src/sage/combinat/root_system/weight_lattice_realizations.py:from sage.combinat.family import Family
src/sage/libs/coxeter3/coxeter_group.py:        from sage.combinat.family import Family
```


CC:  @tscrim

Branch/Commit: **[efb139a](https://github.com/sagemath/sagetrac-mirror/commit/efb139a95a9331c08708ec3b828374c7a6fe89ac)**

Reviewer: **Frédéric Chapoton**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/32624_





---

archive/issue_events_431189.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T16:35:51Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32624",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32624#event-431189"
}
```



---

archive/issue_events_431190.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T16:35:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32624",
    "label": "https://github.com/sagemath/sage/labels/refactoring",
    "label_color": "696969",
    "label_name": "refactoring",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32624#event-431190"
}
```



---

archive/issue_events_431191.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T16:35:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32624",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32624#event-431191"
}
```



---

archive/issue_events_431192.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-04T16:35:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32624",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32624#event-431192"
}
```



---

archive/issue_comments_529177.json:
```json
{
    "body": "Branch: **[public/refactoring/family_import-32624](https://github.com/sagemath/sagetrac-mirror/tree/public/refactoring/family_import-32624)**",
    "created_at": "2021-10-05T03:29:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32624",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32624#issuecomment-529177",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/refactoring/family_import-32624](https://github.com/sagemath/sagetrac-mirror/tree/public/refactoring/family_import-32624)**



---

archive/issue_comments_529178.json:
```json
{
    "body": "Commit: **[29f5a7e](https://github.com/sagemath/sagetrac-mirror/commit/29f5a7ee02dc0064af63c2f58a262bb431b077d3)**",
    "created_at": "2021-10-05T03:29:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32624",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32624#issuecomment-529178",
    "user": "https://github.com/tscrim"
}
```

Commit: **[29f5a7e](https://github.com/sagemath/sagetrac-mirror/commit/29f5a7ee02dc0064af63c2f58a262bb431b077d3)**



---

archive/issue_comments_529179.json:
```json
{
    "body": "Author: **Travis Scrimshaw**",
    "created_at": "2021-10-05T03:29:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32624",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32624#issuecomment-529179",
    "user": "https://github.com/tscrim"
}
```

Author: **Travis Scrimshaw**



---

archive/issue_events_431193.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-10-05T03:29:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32624",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32624#event-431193"
}
```



---

archive/issue_comments_529180.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/29f5a7ee02dc0064af63c2f58a262bb431b077d3\">29f5a7e</a></td><td><code>Replacing all sage.combinat.family import with sage.sets.family.</code></td></tr></table>\n",
    "created_at": "2021-10-05T03:29:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32624",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32624#issuecomment-529180",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/29f5a7ee02dc0064af63c2f58a262bb431b077d3">29f5a7e</a></td><td><code>Replacing all sage.combinat.family import with sage.sets.family.</code></td></tr></table>




---

archive/issue_comments_529181.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2021-10-05T06:43:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32624",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32624#issuecomment-529181",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_529182.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nif green bot, then positive review",
    "created_at": "2021-10-05T06:43:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32624",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32624#issuecomment-529182",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:2'>Comment 2:</a>
if green bot, then positive review



---

archive/issue_events_431194.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-10-05T08:49:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32624",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32624#event-431194"
}
```



---

archive/issue_events_431195.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-10-05T08:49:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32624",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32624#event-431195"
}
```



---

archive/issue_comments_529183.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nThanks for fixing this!",
    "created_at": "2021-10-05T16:28:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32624",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32624#issuecomment-529183",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:4'>Comment 4:</a>
Thanks for fixing this!



---

archive/issue_events_431196.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-10-13T19:33:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32624",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32624#event-431196"
}
```



---

archive/issue_events_431197.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-10-13T19:33:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32624",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32624#event-431197"
}
```



---

archive/issue_comments_529184.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nmerge conflict",
    "created_at": "2021-10-13T19:33:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32624",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32624#issuecomment-529184",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:5'>Comment 5:</a>
merge conflict



---

archive/issue_comments_529185.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/efb139a95a9331c08708ec3b828374c7a6fe89ac\">efb139a</a></td><td><code>Merge tag '9.5.beta3' into t/32624/public/refactoring/family_import-32624</code></td></tr></table>\n",
    "created_at": "2021-10-13T19:41:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32624",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32624#issuecomment-529185",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:6'>Comment 6:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/efb139a95a9331c08708ec3b828374c7a6fe89ac">efb139a</a></td><td><code>Merge tag '9.5.beta3' into t/32624/public/refactoring/family_import-32624</code></td></tr></table>




---

archive/issue_comments_529186.json:
```json
{
    "body": "Changed commit from **[29f5a7e](https://github.com/sagemath/sagetrac-mirror/commit/29f5a7ee02dc0064af63c2f58a262bb431b077d3)** to **[efb139a](https://github.com/sagemath/sagetrac-mirror/commit/efb139a95a9331c08708ec3b828374c7a6fe89ac)**",
    "created_at": "2021-10-13T19:41:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32624",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32624#issuecomment-529186",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[29f5a7e](https://github.com/sagemath/sagetrac-mirror/commit/29f5a7ee02dc0064af63c2f58a262bb431b077d3)** to **[efb139a](https://github.com/sagemath/sagetrac-mirror/commit/efb139a95a9331c08708ec3b828374c7a6fe89ac)**



---

archive/issue_events_431198.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-13T19:42:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32624",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32624#event-431198"
}
```



---

archive/issue_events_431199.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-13T19:42:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32624",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32624#event-431199"
}
```



---

archive/issue_events_431200.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-10-19T20:32:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32624",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32624#event-431200"
}
```



---

archive/issue_events_431201.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "873a7b2e711e371048a3bfc7c58486cb303144a5",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-10-19T20:32:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32624",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32624#event-431201"
}
```



---

archive/issue_comments_529187.json:
```json
{
    "body": "Changed branch from **[public/refactoring/family_import-32624](https://github.com/sagemath/sagetrac-mirror/tree/public/refactoring/family_import-32624)** to **[efb139a](https://github.com/sagemath/sagetrac-mirror/commit/efb139a95a9331c08708ec3b828374c7a6fe89ac)**",
    "created_at": "2021-10-19T20:32:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32624",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32624#issuecomment-529187",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/refactoring/family_import-32624](https://github.com/sagemath/sagetrac-mirror/tree/public/refactoring/family_import-32624)** to **[efb139a](https://github.com/sagemath/sagetrac-mirror/commit/efb139a95a9331c08708ec3b828374c7a6fe89ac)**
