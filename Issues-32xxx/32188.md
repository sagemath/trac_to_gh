...
<a id='comment:7'></a>To answer my own question from[comment:3](#comment%3A3), it looks like `Chan(c)` does something that eliminates the `ZeroDivisionError`, leading to the `DecodingError` that we expect:
