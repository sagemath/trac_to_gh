# Issue 32764: SAGE_ROOT/tox.ini: Make tox -p auto and tox --listenvs useful again

archive/issues_032527.json:
```json
{
    "assignees": [],
    "body": "(from #29146)\n\ntox does not actually understand the nested braces used in `envlist`.\n\nhttps://groups.google.com/g/sage-devel/c/NU-xlUTIIdE/m/Q14HiKvNAgAJ\n\nCC:  @jhpalmieri @kliem\n\nBranch/Commit: **[47a2b42](https://github.com/sagemath/sagetrac-mirror/commit/47a2b421fb70095a0971d37c39689460c0b16d4c)**\n\nReviewer: **John Palmieri**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32764_\n\n",
    "closed_at": "2021-11-14T17:06:23Z",
    "created_at": "2021-10-25T15:44:40Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "SAGE_ROOT/tox.ini: Make tox -p auto and tox --listenvs useful again",
    "type": "issue",
    "updated_at": "2021-11-14T17:06:23Z",
    "url": "https://github.com/sagemath/sage/issues/32764",
    "user": "https://github.com/mkoeppe"
}
```
(from #29146)

tox does not actually understand the nested braces used in `envlist`.

https://groups.google.com/g/sage-devel/c/NU-xlUTIIdE/m/Q14HiKvNAgAJ

CC:  @jhpalmieri @kliem

Branch/Commit: **[47a2b42](https://github.com/sagemath/sagetrac-mirror/commit/47a2b421fb70095a0971d37c39689460c0b16d4c)**

Reviewer: **John Palmieri**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/32764_





---

archive/issue_events_432925.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-25T15:44:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32764",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32764#event-432925"
}
```



---

archive/issue_events_432926.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-25T15:44:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32764",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting",
    "label_color": "000080",
    "label_name": "component: porting",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32764#event-432926"
}
```



---

archive/issue_events_432927.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-25T15:44:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32764",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32764#event-432927"
}
```



---

archive/issue_events_432928.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-25T15:44:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32764",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32764#event-432928"
}
```



---

archive/issue_comments_531924.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,4 +2,4 @@\n \n tox does not actually understand the nested braces used in `envlist`.\n \n-\n+https://groups.google.com/g/sage-devel/c/NU-xlUTIIdE/m/Q14HiKvNAgAJ\n``````\n",
    "created_at": "2021-10-25T15:51:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32764#issuecomment-531924",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,4 +2,4 @@
 
 tox does not actually understand the nested braces used in `envlist`.
 
-
+https://groups.google.com/g/sage-devel/c/NU-xlUTIIdE/m/Q14HiKvNAgAJ
``````




---

archive/issue_comments_531925.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2021-10-25T15:56:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32764#issuecomment-531925",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_comments_531926.json:
```json
{
    "body": "Branch: **[u/mkoeppe/sage_root_tox_ini__make_tox__p_auto_and_tox___listenvs_useful_again](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/sage_root_tox_ini__make_tox__p_auto_and_tox___listenvs_useful_again)**",
    "created_at": "2021-10-25T16:25:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32764#issuecomment-531926",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/sage_root_tox_ini__make_tox__p_auto_and_tox___listenvs_useful_again](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/sage_root_tox_ini__make_tox__p_auto_and_tox___listenvs_useful_again)**



---

archive/issue_comments_531927.json:
```json
{
    "body": "Commit: **[5392cd7](https://github.com/sagemath/sagetrac-mirror/commit/5392cd7925de2eb58b349b8fcc26b5cf71d46eb3)**",
    "created_at": "2021-10-25T16:35:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32764#issuecomment-531927",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[5392cd7](https://github.com/sagemath/sagetrac-mirror/commit/5392cd7925de2eb58b349b8fcc26b5cf71d46eb3)**



---

archive/issue_comments_531928.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5392cd7925de2eb58b349b8fcc26b5cf71d46eb3\">5392cd7</a></td><td><code>tox.ini: Do not include the environments delegated to src/tox.ini in 'tox -p auto'</code></td></tr></table>\n",
    "created_at": "2021-10-25T16:35:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32764#issuecomment-531928",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5392cd7925de2eb58b349b8fcc26b5cf71d46eb3">5392cd7</a></td><td><code>tox.ini: Do not include the environments delegated to src/tox.ini in 'tox -p auto'</code></td></tr></table>




---

archive/issue_comments_531929.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nHere's an attempt at improving the `tox -p auto` situation and documentation.\nPlease feel free to push further changes to this ticket",
    "created_at": "2021-10-25T16:41:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32764#issuecomment-531929",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:5'>Comment 5:</a>
Here's an attempt at improving the `tox -p auto` situation and documentation.
Please feel free to push further changes to this ticket



---

archive/issue_comments_531930.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/47a2b421fb70095a0971d37c39689460c0b16d4c\">47a2b42</a></td><td><code>tox.ini: Add more examples</code></td></tr></table>\n",
    "created_at": "2021-10-25T17:32:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32764#issuecomment-531930",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:6'>Comment 6:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/47a2b421fb70095a0971d37c39689460c0b16d4c">47a2b42</a></td><td><code>tox.ini: Add more examples</code></td></tr></table>




---

archive/issue_comments_531931.json:
```json
{
    "body": "Changed commit from **[5392cd7](https://github.com/sagemath/sagetrac-mirror/commit/5392cd7925de2eb58b349b8fcc26b5cf71d46eb3)** to **[47a2b42](https://github.com/sagemath/sagetrac-mirror/commit/47a2b421fb70095a0971d37c39689460c0b16d4c)**",
    "created_at": "2021-10-25T17:32:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32764#issuecomment-531931",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[5392cd7](https://github.com/sagemath/sagetrac-mirror/commit/5392cd7925de2eb58b349b8fcc26b5cf71d46eb3)** to **[47a2b42](https://github.com/sagemath/sagetrac-mirror/commit/47a2b421fb70095a0971d37c39689460c0b16d4c)**



---

archive/issue_events_432929.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-26T01:05:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32764",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32764#event-432929"
}
```



---

archive/issue_comments_531932.json:
```json
{
    "body": "<a id='comment:9'>Comment 9:</a>\nWas the hope that the nested braces would allow testing all of the various combinations via `tox -p auto`?",
    "created_at": "2021-11-04T02:15:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32764#issuecomment-531932",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:9'>Comment 9:</a>
Was the hope that the nested braces would allow testing all of the various combinations via `tox -p auto`?



---

archive/issue_comments_531933.json:
```json
{
    "body": "<a id='comment:10'>Comment 10:</a>\nYes, and some early version of it did something like that. https://tox.wiki/en/latest/config.html#generative-section-names\nNot sure if nesting ever worked.",
    "created_at": "2021-11-04T02:19:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32764#issuecomment-531933",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:10'>Comment 10:</a>
Yes, and some early version of it did something like that. https://tox.wiki/en/latest/config.html#generative-section-names
Not sure if nesting ever worked.



---

archive/issue_comments_531934.json:
```json
{
    "body": "<a id='comment:11'>Comment 11:</a>\nI could imagine expanding that environment list (since it's much shorter now than the original intention), but that can wait.",
    "created_at": "2021-11-04T03:07:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32764#issuecomment-531934",
    "user": "https://github.com/jhpalmieri"
}
```

<a id='comment:11'>Comment 11:</a>
I could imagine expanding that environment list (since it's much shorter now than the original intention), but that can wait.



---

archive/issue_comments_531935.json:
```json
{
    "body": "Reviewer: **John Palmieri**",
    "created_at": "2021-11-04T03:07:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32764#issuecomment-531935",
    "user": "https://github.com/jhpalmieri"
}
```

Reviewer: **John Palmieri**



---

archive/issue_events_432930.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2021-11-04T03:07:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32764",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32764#event-432930"
}
```



---

archive/issue_events_432931.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2021-11-04T03:07:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32764",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32764#event-432931"
}
```



---

archive/issue_comments_531936.json:
```json
{
    "body": "<a id='comment:12'>Comment 12:</a>\nThanks!",
    "created_at": "2021-11-04T05:42:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32764#issuecomment-531936",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:12'>Comment 12:</a>
Thanks!



---

archive/issue_comments_531937.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/sage_root_tox_ini__make_tox__p_auto_and_tox___listenvs_useful_again](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/sage_root_tox_ini__make_tox__p_auto_and_tox___listenvs_useful_again)** to **[47a2b42](https://github.com/sagemath/sagetrac-mirror/commit/47a2b421fb70095a0971d37c39689460c0b16d4c)**",
    "created_at": "2021-11-14T17:06:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32764",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32764#issuecomment-531937",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/sage_root_tox_ini__make_tox__p_auto_and_tox___listenvs_useful_again](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/sage_root_tox_ini__make_tox__p_auto_and_tox___listenvs_useful_again)** to **[47a2b42](https://github.com/sagemath/sagetrac-mirror/commit/47a2b421fb70095a0971d37c39689460c0b16d4c)**



---

archive/issue_events_432932.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-11-14T17:06:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32764",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32764#event-432932"
}
```



---

archive/issue_events_432933.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "23ff9d435fedb188b0069f59bffad58c009e2fbe",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-11-14T17:06:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32764",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32764#event-432933"
}
```
