# Issue 32922: Change parameter name

archive/issues_032685.json:
```json
{
    "assignees": [],
    "body": "Change parameter name to be more descriptive.\n\nDepends on #32278\n\nCC:  @behackl @cheuberg @dkrenn\n\nKeywords: **asymptotics**\n\nBranch/Commit: **[bca6e35](https://github.com/sagemath/sagetrac-mirror/commit/bca6e359aee3b89fb76281420f0f9a5aa280ec7d)**\n\nReviewer: **Benjamin Hackl**\n\nAuthor: **Thomas Hagelmayer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32922_\n\n",
    "closed_at": "2022-03-03T22:19:13Z",
    "created_at": "2021-11-22T18:13:39Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20asymptotic%20expansions",
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Change parameter name",
    "type": "issue",
    "updated_at": "2022-03-03T22:19:13Z",
    "url": "https://github.com/sagemath/sage/issues/32922",
    "user": "https://github.com/thhagelmayer"
}
```
Change parameter name to be more descriptive.

Depends on #32278

CC:  @behackl @cheuberg @dkrenn

Keywords: **asymptotics**

Branch/Commit: **[bca6e35](https://github.com/sagemath/sagetrac-mirror/commit/bca6e359aee3b89fb76281420f0f9a5aa280ec7d)**

Reviewer: **Benjamin Hackl**

Author: **Thomas Hagelmayer**

_Issue created by migration from https://trac.sagemath.org/ticket/32922_





---

archive/issue_events_434848.json:
```json
{
    "actor": "https://github.com/thhagelmayer",
    "created_at": "2021-11-22T18:13:39Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32922",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32922#event-434848"
}
```



---

archive/issue_events_434849.json:
```json
{
    "actor": "https://github.com/thhagelmayer",
    "created_at": "2021-11-22T18:13:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32922",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20asymptotic%20expansions",
    "label_color": "0000ff",
    "label_name": "component: asymptotic expansions",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32922#event-434849"
}
```



---

archive/issue_events_434850.json:
```json
{
    "actor": "https://github.com/thhagelmayer",
    "created_at": "2021-11-22T18:13:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32922",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32922#event-434850"
}
```



---

archive/issue_events_434851.json:
```json
{
    "actor": "https://github.com/thhagelmayer",
    "created_at": "2021-11-22T18:13:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32922",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32922#event-434851"
}
```



---

archive/issue_comments_534758.json:
```json
{
    "body": "Branch: **[u/gh-thhagelmayer/change_parameter_name](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-thhagelmayer/change_parameter_name)**",
    "created_at": "2021-11-22T18:26:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32922",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32922#issuecomment-534758",
    "user": "https://github.com/thhagelmayer"
}
```

Branch: **[u/gh-thhagelmayer/change_parameter_name](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-thhagelmayer/change_parameter_name)**



---

archive/issue_events_434852.json:
```json
{
    "actor": "https://github.com/thhagelmayer",
    "created_at": "2021-11-22T18:28:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32922",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32922#event-434852"
}
```



---

archive/issue_comments_534759.json:
```json
{
    "body": "Commit: **[bca6e35](https://github.com/sagemath/sagetrac-mirror/commit/bca6e359aee3b89fb76281420f0f9a5aa280ec7d)**",
    "created_at": "2021-11-22T18:28:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32922",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32922#issuecomment-534759",
    "user": "https://github.com/thhagelmayer"
}
```

Commit: **[bca6e35](https://github.com/sagemath/sagetrac-mirror/commit/bca6e359aee3b89fb76281420f0f9a5aa280ec7d)**



---

archive/issue_comments_534760.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nI changed the parameter `self` to `expression` in:\n\n\n```\n@staticmethod\n    def B(expression, valid_from=0):\n    ...\n    return expression.B(valid_from)\n```\n\n---\nLast 10 new commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f7c1823d67bf01936a38bcd01ff4d0102e8d85b6\">f7c1823</a></td><td><code>Merge branch 't/32214/term-monoids-unify-repr' into t/32215/refactor-element-construction-term-monoids</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ba8efe684bd13c2122d5f72c4cae554d5e70928a\">ba8efe6</a></td><td><code>Merge branch 'u/gh-thhagelmayer/refactor-element-construction-term-monoids' of git://trac.sagemath.org/sage into t/32215/refactor-element-construction-term-monoids</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/51e035f56d430e6acb0417a91899ed9d869b5bb6\">51e035f</a></td><td><code>Trac #32215: fix doctests (due to merge of #32214)</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d763d663355c4519128a616fb5f6ce7ebba2cfee\">d763d66</a></td><td><code>Trac #32215: full coverage of _element_constructor_</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/540d08880bdcc5fbda1b2487bce9c74d3d35f27c\">540d088</a></td><td><code>Merge commit 'c11b2f4' into t/32215/refactor-element-construction-term-monoids</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/62cae10d33ab58abe5b2bbb2388adce8e01d1487\">62cae10</a></td><td><code>Trac #32215: fix doctest after merge</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a84a2f2750ae18cd56695cf967fc9d4886f53134\">a84a2f2</a></td><td><code>Merge branch 'u/dkrenn/refactor-element-construction-term-monoids' of trac.sagemath.org:sage into t/32229/bterms_conversion</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/84ad7a3efb761d10cbc0b02c4dd8145e8c991f04\">84ad7a3</a></td><td><code>Trac #32229: take absolute value of coefficient of BTerm</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/24748702f01276cdc678c969ff74482c3c8d347b\">2474870</a></td><td><code>Merge branch 'u/behackl/bterms_conversion' of trac.sagemath.org:sage into t/32278/add_support_for_b_terms_in_asymptotic_ring</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bca6e359aee3b89fb76281420f0f9a5aa280ec7d\">bca6e35</a></td><td><code>Trac #32922: Change parameter name to be more descriptive.</code></td></tr></table>\n",
    "created_at": "2021-11-22T18:28:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32922",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32922#issuecomment-534760",
    "user": "https://github.com/thhagelmayer"
}
```

<a id='comment:2'>Comment 2:</a>
I changed the parameter `self` to `expression` in:


```
@staticmethod
    def B(expression, valid_from=0):
    ...
    return expression.B(valid_from)
```

---
Last 10 new commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f7c1823d67bf01936a38bcd01ff4d0102e8d85b6">f7c1823</a></td><td><code>Merge branch 't/32214/term-monoids-unify-repr' into t/32215/refactor-element-construction-term-monoids</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ba8efe684bd13c2122d5f72c4cae554d5e70928a">ba8efe6</a></td><td><code>Merge branch 'u/gh-thhagelmayer/refactor-element-construction-term-monoids' of git://trac.sagemath.org/sage into t/32215/refactor-element-construction-term-monoids</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/51e035f56d430e6acb0417a91899ed9d869b5bb6">51e035f</a></td><td><code>Trac #32215: fix doctests (due to merge of #32214)</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d763d663355c4519128a616fb5f6ce7ebba2cfee">d763d66</a></td><td><code>Trac #32215: full coverage of _element_constructor_</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/540d08880bdcc5fbda1b2487bce9c74d3d35f27c">540d088</a></td><td><code>Merge commit 'c11b2f4' into t/32215/refactor-element-construction-term-monoids</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/62cae10d33ab58abe5b2bbb2388adce8e01d1487">62cae10</a></td><td><code>Trac #32215: fix doctest after merge</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a84a2f2750ae18cd56695cf967fc9d4886f53134">a84a2f2</a></td><td><code>Merge branch 'u/dkrenn/refactor-element-construction-term-monoids' of trac.sagemath.org:sage into t/32229/bterms_conversion</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/84ad7a3efb761d10cbc0b02c4dd8145e8c991f04">84ad7a3</a></td><td><code>Trac #32229: take absolute value of coefficient of BTerm</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/24748702f01276cdc678c969ff74482c3c8d347b">2474870</a></td><td><code>Merge branch 'u/behackl/bterms_conversion' of trac.sagemath.org:sage into t/32278/add_support_for_b_terms_in_asymptotic_ring</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bca6e359aee3b89fb76281420f0f9a5aa280ec7d">bca6e35</a></td><td><code>Trac #32922: Change parameter name to be more descriptive.</code></td></tr></table>




---

archive/issue_events_434853.json:
```json
{
    "actor": "https://github.com/thhagelmayer",
    "created_at": "2021-11-22T18:30:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32922",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32922#event-434853"
}
```



---

archive/issue_events_434854.json:
```json
{
    "actor": "https://github.com/thhagelmayer",
    "created_at": "2021-11-22T18:30:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32922",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32922#event-434854"
}
```



---

archive/issue_comments_534761.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nSemantically, this makes sense. LGTM.",
    "created_at": "2021-11-23T09:12:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32922",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32922#issuecomment-534761",
    "user": "https://github.com/behackl"
}
```

<a id='comment:4'>Comment 4:</a>
Semantically, this makes sense. LGTM.



---

archive/issue_comments_534762.json:
```json
{
    "body": "Reviewer: **Benjamin Hackl**",
    "created_at": "2021-11-23T09:12:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32922",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32922#issuecomment-534762",
    "user": "https://github.com/behackl"
}
```

Reviewer: **Benjamin Hackl**



---

archive/issue_events_434855.json:
```json
{
    "actor": "https://github.com/behackl",
    "created_at": "2021-11-23T09:12:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32922",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32922#event-434855"
}
```



---

archive/issue_events_434856.json:
```json
{
    "actor": "https://github.com/behackl",
    "created_at": "2021-11-23T09:12:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32922",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32922#event-434856"
}
```



---

archive/issue_comments_534763.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nSetting milestone to 9.6 now that 9.5 is out.",
    "created_at": "2022-01-30T15:39:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32922",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32922#issuecomment-534763",
    "user": "https://github.com/slel"
}
```

<a id='comment:5'>Comment 5:</a>
Setting milestone to 9.6 now that 9.5 is out.



---

archive/issue_events_434857.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2022-01-30T15:39:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/32922",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32922#event-434857"
}
```



---

archive/issue_events_434858.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2022-01-30T15:39:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32922",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32922#event-434858"
}
```



---

archive/issue_events_434859.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-03-03T22:19:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32922",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32922#event-434859"
}
```



---

archive/issue_events_434860.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "9a0c9833034368fb81e518c55f4ede500ffd01de",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-03-03T22:19:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32922",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32922#event-434860"
}
```



---

archive/issue_comments_534764.json:
```json
{
    "body": "Changed branch from **[u/gh-thhagelmayer/change_parameter_name](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-thhagelmayer/change_parameter_name)** to **[bca6e35](https://github.com/sagemath/sagetrac-mirror/commit/bca6e359aee3b89fb76281420f0f9a5aa280ec7d)**",
    "created_at": "2022-03-03T22:19:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32922",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32922#issuecomment-534764",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/gh-thhagelmayer/change_parameter_name](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-thhagelmayer/change_parameter_name)** to **[bca6e35](https://github.com/sagemath/sagetrac-mirror/commit/bca6e359aee3b89fb76281420f0f9a5aa280ec7d)**
