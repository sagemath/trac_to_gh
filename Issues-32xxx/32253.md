# Issue 32253: csdp feature-not-present information references old url

archive/issues_032016.json:
```json
{
    "assignees": [],
    "body": "Prompted by me replicating https://groups.google.com/g/sage-support/c/2ufrP1M4mRs/m/bpMfD-oKBwAJ \n\nIf the optional csdp package is not installed, the following error message is shown.\n\n```\nFeatureNotPresentError: CSDP is not available.\nExecutable 'theta' not found on PATH.\nNo equivalent system packages for debian are known to Sage.\nTo install CSDP using the Sage package manager, you can try to run:\n  !sage -i csdp\nNo equivalent system packages for pip are known to Sage.\nFurther installation instructions might be available at http://github.org/dimpase/csdp.\n```\n\nHowever, http://github.org/dimpase/csdp doesn't exist. Perhaps it is appropriate to replace this with https://github.com/dimpase/csdp ? I note that the last commit there was 6 years ago, and I think the instructions there are perhaps not what I would call \"further installation instructions\" now.\n\nNote that #32244 also concerns csdp.\n\nCC:  @dimpase\n\nKeywords: **csdp**\n\nReviewer: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32253_\n\n",
    "closed_at": "2021-08-11T12:16:43Z",
    "created_at": "2021-07-20T15:34:46Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "csdp feature-not-present information references old url",
    "type": "issue",
    "updated_at": "2021-08-11T12:16:43Z",
    "url": "https://github.com/sagemath/sage/issues/32253",
    "user": "https://github.com/davidlowryduda"
}
```
Prompted by me replicating https://groups.google.com/g/sage-support/c/2ufrP1M4mRs/m/bpMfD-oKBwAJ 

If the optional csdp package is not installed, the following error message is shown.

```
FeatureNotPresentError: CSDP is not available.
Executable 'theta' not found on PATH.
No equivalent system packages for debian are known to Sage.
To install CSDP using the Sage package manager, you can try to run:
  !sage -i csdp
No equivalent system packages for pip are known to Sage.
Further installation instructions might be available at http://github.org/dimpase/csdp.
```

However, http://github.org/dimpase/csdp doesn't exist. Perhaps it is appropriate to replace this with https://github.com/dimpase/csdp ? I note that the last commit there was 6 years ago, and I think the instructions there are perhaps not what I would call "further installation instructions" now.

Note that #32244 also concerns csdp.

CC:  @dimpase

Keywords: **csdp**

Reviewer: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/32253_





---

archive/issue_events_426609.json:
```json
{
    "actor": "https://github.com/davidlowryduda",
    "created_at": "2021-07-20T15:34:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32253",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32253#event-426609"
}
```



---

archive/issue_events_426610.json:
```json
{
    "actor": "https://github.com/davidlowryduda",
    "created_at": "2021-07-20T15:34:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32253",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
    "label_color": "000080",
    "label_name": "component: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32253#event-426610"
}
```



---

archive/issue_events_426611.json:
```json
{
    "actor": "https://github.com/davidlowryduda",
    "created_at": "2021-07-20T15:34:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32253",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32253#event-426611"
}
```



---

archive/issue_events_426612.json:
```json
{
    "actor": "https://github.com/davidlowryduda",
    "created_at": "2021-07-20T15:34:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32253",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32253#event-426612"
}
```



---

archive/issue_comments_522602.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nok, let me fix this on #32244.",
    "created_at": "2021-07-20T15:39:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32253#issuecomment-522602",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:1" align="right">Comment 1</div>

ok, let me fix this on #32244.



---

archive/issue_comments_522603.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nI've fixed the URL on #32244.",
    "created_at": "2021-07-20T15:45:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32253#issuecomment-522603",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:2" align="right">Comment 2</div>

I've fixed the URL on #32244.



---

archive/issue_comments_522604.json:
```json
{
    "body": "Author: **Dima Pasechnik**",
    "created_at": "2021-07-20T15:45:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32253#issuecomment-522604",
    "user": "https://github.com/dimpase"
}
```

Author: **Dima Pasechnik**



---

archive/issue_events_426613.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-07-20T15:45:18Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/32253",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32253#event-426613"
}
```



---

archive/issue_events_426614.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-07-20T15:45:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32253",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32253#event-426614"
}
```



---

archive/issue_events_426615.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-07-20T15:45:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32253",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32253#event-426615"
}
```



---

archive/issue_events_426616.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-07-20T15:45:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32253",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32253#event-426616"
}
```



---

archive/issue_events_426617.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-20T16:00:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32253",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32253#event-426617"
}
```



---

archive/issue_events_426618.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-20T16:00:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32253",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32253#event-426618"
}
```



---

archive/issue_comments_522605.json:
```json
{
    "body": "Changed author from **Dima Pasechnik** to none",
    "created_at": "2021-07-20T16:00:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32253#issuecomment-522605",
    "user": "https://github.com/mkoeppe"
}
```

Changed author from **Dima Pasechnik** to none



---

archive/issue_comments_522606.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2021-07-20T16:00:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32253",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32253#issuecomment-522606",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_events_426619.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-08-11T12:16:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32253",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32253#event-426619"
}
```



---

archive/issue_events_426620.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-08-11T12:16:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32253",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32253#event-426620"
}
```



---

archive/issue_events_426621.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-08-11T12:16:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32253",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32253#event-426621"
}
```
