...
```json
{
    "body": "For macOS the binascii module mysteriously failed to be compiled after running\n\n```\ngit switch develop\nmake\n```\n\nbut the problem went away after running\n\n```\nmake distclean\n./configure\nmake\n```",
...
```json
{
    "body": "... that's unfortunately not specific enough. Any chance you can restore the broken state with TimeMachine so that we can diagnose the issue?",
...
```json
{
    "body": "OK - although I don't have an easy way to reproduce the bug, so the review is based on reading only.",
...
```json
{
    "body": "Then, I checked out 9.4.beta3, run make, and was able to reproduce the error with python 3.9.5. Then, I pulled the current branch of this ticket and then `python3.9.5` was built ok after a make. So I feel this branch is fixing the issue.\n\nBut, I now get an error with cython (forgot to copy paste the error message, sorry). Then, I did make distclean and make and I am waiting for the result.",
...
```json
{
    "body": "I just noticed that the cysignals spkg also failed to install with the same exception about importing gen from tornado.  I was using make -j4, and evidently I got both errors simultaneously.",
...
```json
{
    "body": "That's an unrelated problem fixed in #32022",
