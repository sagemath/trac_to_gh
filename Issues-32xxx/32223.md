# Issue 32223: Fix use of MixedIntegerLinearProgram in cutwidth

archive/issues_031986.json:
```json
{
    "assignees": [],
    "body": "Part of #32191 (Meta-ticket: Audit/fix all uses of MixedIntegerLinearProgram? in sage.combinat, sage.graphs).\n\nDepends on #32197\n\nCC:  @mkoeppe @tscrim @dimpase\n\nBranch/Commit: **[9223c66](https://github.com/sagemath/sagetrac-mirror/commit/9223c66f81ea5c94fb52ea7eb30ffdd426df1f48)**\n\nReviewer: **Matthias Koeppe**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32223_\n\n",
    "closed_at": "2021-09-01T20:55:14Z",
    "created_at": "2021-07-17T11:38:29Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix use of MixedIntegerLinearProgram in cutwidth",
    "type": "issue",
    "updated_at": "2021-09-01T20:55:14Z",
    "url": "https://github.com/sagemath/sage/issues/32223",
    "user": "https://github.com/dcoudert"
}
```
Part of #32191 (Meta-ticket: Audit/fix all uses of MixedIntegerLinearProgram? in sage.combinat, sage.graphs).

Depends on #32197

CC:  @mkoeppe @tscrim @dimpase

Branch/Commit: **[9223c66](https://github.com/sagemath/sagetrac-mirror/commit/9223c66f81ea5c94fb52ea7eb30ffdd426df1f48)**

Reviewer: **Matthias Koeppe**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/32223_





---

archive/issue_events_426218.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2021-07-17T11:38:29Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32223",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32223#event-426218"
}
```



---

archive/issue_events_426219.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2021-07-17T11:38:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32223",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32223#event-426219"
}
```



---

archive/issue_events_426220.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2021-07-17T11:38:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32223",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32223#event-426220"
}
```



---

archive/issue_events_426221.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2021-07-17T11:38:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32223",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32223#event-426221"
}
```



---

archive/issue_events_426222.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2021-07-17T11:40:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32223",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32223#event-426222"
}
```



---

archive/issue_comments_522153.json:
```json
{
    "body": "Branch: **[public/graphs/32223_cutwidth](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/32223_cutwidth)**",
    "created_at": "2021-07-17T11:40:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32223",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32223#issuecomment-522153",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[public/graphs/32223_cutwidth](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/32223_cutwidth)**



---

archive/issue_comments_522154.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nLast 10 new commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3dca4e940cfdbe84b9a301a71f263b36a01d441b\">3dca4e9</a></td><td><code>MixedIntegerLinearProgram.get_values: If convert=True, convert real variables from float to base_ring</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9ad3fa80afd83753fe5b6445eb8338f8f885ddcc\">9ad3fa8</a></td><td><code>MixedIntegerLinearProgram.get_converted_values; subsumed by get_values</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f62a7777b342bc46ecd5e4ccb1819d12bdb5db8a\">f62a777</a></td><td><code>src/sage/numerical/mip.pyx: Remove unused imports</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cfd8c7becb47cbe3ad9ad5cf9b6fb2b12122657e\">cfd8c7b</a></td><td><code>trac #32197: remove another useless import</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cf16293c1131db37f0ed1274eded6717c819989c\">cf16293</a></td><td><code>MixedIntegerLinearProgram.get_values: Fix up for tolerance=None</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5a74113233bba7b0ac689f576849c9c36dd6cb39\">5a74113</a></td><td><code>MixedIntegerLinearProgram._backend_variable_value: Add docstring</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/aa4eed5843bce5bbb022ef79577e266d2ef41bfc\">aa4eed5</a></td><td><code>MixedIntegerLinearProgram._backend_variable_value*: Add docstrings, examples</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/00b34aaedbefbdfa7d431868f20055fd6d3572b6\">00b34aa</a></td><td><code>trac #32197: review commit</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/db83853c6df0ed792b4b011b842cf5fd01444765\">db83853</a></td><td><code>trac #32197: another review commit</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1a1f671b1e543d478d19945ed83118f4f2e8322f\">1a1f671</a></td><td><code>trac 32223: Fix use of MixedIntegerLinearProgram in cutwidth</code></td></tr></table>\n",
    "created_at": "2021-07-17T11:40:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32223",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32223#issuecomment-522154",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:1'>Comment 1:</a>
Last 10 new commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3dca4e940cfdbe84b9a301a71f263b36a01d441b">3dca4e9</a></td><td><code>MixedIntegerLinearProgram.get_values: If convert=True, convert real variables from float to base_ring</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9ad3fa80afd83753fe5b6445eb8338f8f885ddcc">9ad3fa8</a></td><td><code>MixedIntegerLinearProgram.get_converted_values; subsumed by get_values</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f62a7777b342bc46ecd5e4ccb1819d12bdb5db8a">f62a777</a></td><td><code>src/sage/numerical/mip.pyx: Remove unused imports</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cfd8c7becb47cbe3ad9ad5cf9b6fb2b12122657e">cfd8c7b</a></td><td><code>trac #32197: remove another useless import</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cf16293c1131db37f0ed1274eded6717c819989c">cf16293</a></td><td><code>MixedIntegerLinearProgram.get_values: Fix up for tolerance=None</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5a74113233bba7b0ac689f576849c9c36dd6cb39">5a74113</a></td><td><code>MixedIntegerLinearProgram._backend_variable_value: Add docstring</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/aa4eed5843bce5bbb022ef79577e266d2ef41bfc">aa4eed5</a></td><td><code>MixedIntegerLinearProgram._backend_variable_value*: Add docstrings, examples</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/00b34aaedbefbdfa7d431868f20055fd6d3572b6">00b34aa</a></td><td><code>trac #32197: review commit</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/db83853c6df0ed792b4b011b842cf5fd01444765">db83853</a></td><td><code>trac #32197: another review commit</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1a1f671b1e543d478d19945ed83118f4f2e8322f">1a1f671</a></td><td><code>trac 32223: Fix use of MixedIntegerLinearProgram in cutwidth</code></td></tr></table>




---

archive/issue_comments_522155.json:
```json
{
    "body": "Commit: **[1a1f671](https://github.com/sagemath/sagetrac-mirror/commit/1a1f671b1e543d478d19945ed83118f4f2e8322f)**",
    "created_at": "2021-07-17T11:40:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32223",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32223#issuecomment-522155",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[1a1f671](https://github.com/sagemath/sagetrac-mirror/commit/1a1f671b1e543d478d19945ed83118f4f2e8322f)**



---

archive/issue_comments_522156.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2021-07-18T19:13:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32223",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32223#issuecomment-522156",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_events_426223.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-18T19:13:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32223",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32223#event-426223"
}
```



---

archive/issue_events_426224.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-18T19:13:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32223",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32223#event-426224"
}
```



---

archive/issue_comments_522157.json:
```json
{
    "body": "Dependencies: **#32197**",
    "created_at": "2021-07-19T12:34:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32223",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32223#issuecomment-522157",
    "user": "https://github.com/dcoudert"
}
```

Dependencies: **#32197**



---

archive/issue_events_426225.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-07-24T23:29:25Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32223",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32223#event-426225"
}
```



---

archive/issue_events_426226.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-07-24T23:29:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32223",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32223#event-426226"
}
```



---

archive/issue_comments_522158.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nMerge conflict",
    "created_at": "2021-07-24T23:29:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32223",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32223#issuecomment-522158",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:4'>Comment 4:</a>
Merge conflict



---

archive/issue_comments_522159.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9223c66f81ea5c94fb52ea7eb30ffdd426df1f48\">9223c66</a></td><td><code>Merge tag '9.4.beta6' into t/32223/public/graphs/32223_cutwidth</code></td></tr></table>\n",
    "created_at": "2021-07-25T21:11:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32223",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32223#issuecomment-522159",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:5'>Comment 5:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9223c66f81ea5c94fb52ea7eb30ffdd426df1f48">9223c66</a></td><td><code>Merge tag '9.4.beta6' into t/32223/public/graphs/32223_cutwidth</code></td></tr></table>




---

archive/issue_comments_522160.json:
```json
{
    "body": "Changed commit from **[1a1f671](https://github.com/sagemath/sagetrac-mirror/commit/1a1f671b1e543d478d19945ed83118f4f2e8322f)** to **[9223c66](https://github.com/sagemath/sagetrac-mirror/commit/9223c66f81ea5c94fb52ea7eb30ffdd426df1f48)**",
    "created_at": "2021-07-25T21:11:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32223",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32223#issuecomment-522160",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[1a1f671](https://github.com/sagemath/sagetrac-mirror/commit/1a1f671b1e543d478d19945ed83118f4f2e8322f)** to **[9223c66](https://github.com/sagemath/sagetrac-mirror/commit/9223c66f81ea5c94fb52ea7eb30ffdd426df1f48)**



---

archive/issue_events_426227.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-25T21:11:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32223",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32223#event-426227"
}
```



---

archive/issue_events_426228.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-25T21:11:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32223",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32223#event-426228"
}
```



---

archive/issue_comments_522161.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nThanks.",
    "created_at": "2021-07-26T09:50:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32223",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32223#issuecomment-522161",
    "user": "https://github.com/dcoudert"
}
```

<a id='comment:7'>Comment 7:</a>
Thanks.



---

archive/issue_events_426229.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-22T20:35:10Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/32223",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32223#event-426229"
}
```



---

archive/issue_events_426230.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-22T20:35:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32223",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32223#event-426230"
}
```



---

archive/issue_comments_522162.json:
```json
{
    "body": "Changed branch from **[public/graphs/32223_cutwidth](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/32223_cutwidth)** to **[9223c66](https://github.com/sagemath/sagetrac-mirror/commit/9223c66f81ea5c94fb52ea7eb30ffdd426df1f48)**",
    "created_at": "2021-09-01T20:55:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32223",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32223#issuecomment-522162",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/graphs/32223_cutwidth](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/32223_cutwidth)** to **[9223c66](https://github.com/sagemath/sagetrac-mirror/commit/9223c66f81ea5c94fb52ea7eb30ffdd426df1f48)**



---

archive/issue_events_426231.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-09-01T20:55:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32223",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32223#event-426231"
}
```



---

archive/issue_events_426232.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "7f986f51d3a4dcb845b1369e7f4b9660edccfd2e",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-09-01T20:55:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32223",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32223#event-426232"
}
```
