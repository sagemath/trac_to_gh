# Issue 32708: sage.tensor.modules: remove dependency on sage.manifolds

archive/issues_032471.json:
```json
{
    "assignees": [],
    "body": "In view of Sage modularization, this ticket removes the only import from `sage.manifolds` in the module `sage.tensor.modules`. \nThis was an import of `Chart` in `FreeModuleTensor._preparse_display`. This is avoided by reimplementing `_preparse_display` in `TensorFieldParal`. \n\nCC:  @mkoeppe @tscrim\n\nKeywords: **tensor display**\n\nBranch/Commit: **[1635bd3](https://github.com/sagemath/sagetrac-mirror/commit/1635bd3713a426939202e098e515ed5258485ab5)**\n\nReviewer: **Matthias Koeppe**\n\nAuthor: **Eric Gourgoulhon**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32708_\n\n",
    "closed_at": "2021-10-20T23:00:42Z",
    "created_at": "2021-10-17T19:08:25Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20manifolds",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage.tensor.modules: remove dependency on sage.manifolds",
    "type": "issue",
    "updated_at": "2021-10-20T23:00:42Z",
    "url": "https://github.com/sagemath/sage/issues/32708",
    "user": "https://github.com/egourgoulhon"
}
```
In view of Sage modularization, this ticket removes the only import from `sage.manifolds` in the module `sage.tensor.modules`. 
This was an import of `Chart` in `FreeModuleTensor._preparse_display`. This is avoided by reimplementing `_preparse_display` in `TensorFieldParal`. 

CC:  @mkoeppe @tscrim

Keywords: **tensor display**

Branch/Commit: **[1635bd3](https://github.com/sagemath/sagetrac-mirror/commit/1635bd3713a426939202e098e515ed5258485ab5)**

Reviewer: **Matthias Koeppe**

Author: **Eric Gourgoulhon**

_Issue created by migration from https://trac.sagemath.org/ticket/32708_





---

archive/issue_events_417939.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2021-10-17T19:08:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32708",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32708#event-417939"
}
```



---

archive/issue_events_417940.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2021-10-17T19:08:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32708",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20manifolds",
    "label_color": "0000ff",
    "label_name": "component: manifolds",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32708#event-417940"
}
```



---

archive/issue_events_417941.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2021-10-17T19:08:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32708",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32708#event-417941"
}
```



---

archive/issue_events_417942.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2021-10-17T19:08:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32708",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32708#event-417942"
}
```



---

archive/issue_comments_530740.json:
```json
{
    "body": "Commit: **[1635bd3](https://github.com/sagemath/sagetrac-mirror/commit/1635bd3713a426939202e098e515ed5258485ab5)**",
    "created_at": "2021-10-17T19:12:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32708",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32708#issuecomment-530740",
    "user": "https://github.com/egourgoulhon"
}
```

Commit: **[1635bd3](https://github.com/sagemath/sagetrac-mirror/commit/1635bd3713a426939202e098e515ed5258485ab5)**



---

archive/issue_comments_530741.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1635bd3713a426939202e098e515ed5258485ab5\">1635bd3</a></td><td><code>Remove import of Chart in sage.tensor.modules (#32708)</code></td></tr></table>\n",
    "created_at": "2021-10-17T19:12:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32708",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32708#issuecomment-530741",
    "user": "https://github.com/egourgoulhon"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1635bd3713a426939202e098e515ed5258485ab5">1635bd3</a></td><td><code>Remove import of Chart in sage.tensor.modules (#32708)</code></td></tr></table>




---

archive/issue_events_417943.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2021-10-17T19:12:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32708",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32708#event-417943"
}
```



---

archive/issue_comments_530742.json:
```json
{
    "body": "Branch: **[public/manifolds/no_chart_import_in_tensor_modules-32708](https://github.com/sagemath/sagetrac-mirror/tree/public/manifolds/no_chart_import_in_tensor_modules-32708)**",
    "created_at": "2021-10-17T19:12:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32708",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32708#issuecomment-530742",
    "user": "https://github.com/egourgoulhon"
}
```

Branch: **[public/manifolds/no_chart_import_in_tensor_modules-32708](https://github.com/sagemath/sagetrac-mirror/tree/public/manifolds/no_chart_import_in_tensor_modules-32708)**



---

archive/issue_events_417944.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-18T16:49:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32708",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32708#event-417944"
}
```



---

archive/issue_events_417945.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-18T16:49:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32708",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32708#event-417945"
}
```



---

archive/issue_comments_530743.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2021-10-18T16:49:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32708",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32708#issuecomment-530743",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_comments_530744.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nThank you for the review!",
    "created_at": "2021-10-18T19:55:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32708",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32708#issuecomment-530744",
    "user": "https://github.com/egourgoulhon"
}
```

<a id='comment:3'>Comment 3:</a>
Thank you for the review!



---

archive/issue_events_417946.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-10-20T23:00:42Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32708",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32708#event-417946"
}
```



---

archive/issue_events_417947.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "11684c278f4958d4baf321bbf7ea6de8b4ab34c6",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-10-20T23:00:42Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32708",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32708#event-417947"
}
```



---

archive/issue_comments_530745.json:
```json
{
    "body": "Changed branch from **[public/manifolds/no_chart_import_in_tensor_modules-32708](https://github.com/sagemath/sagetrac-mirror/tree/public/manifolds/no_chart_import_in_tensor_modules-32708)** to **[1635bd3](https://github.com/sagemath/sagetrac-mirror/commit/1635bd3713a426939202e098e515ed5258485ab5)**",
    "created_at": "2021-10-20T23:00:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32708",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32708#issuecomment-530745",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/manifolds/no_chart_import_in_tensor_modules-32708](https://github.com/sagemath/sagetrac-mirror/tree/public/manifolds/no_chart_import_in_tensor_modules-32708)** to **[1635bd3](https://github.com/sagemath/sagetrac-mirror/commit/1635bd3713a426939202e098e515ed5258485ab5)**
