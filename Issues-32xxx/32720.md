# Issue 32720: Coercion of booleans from magma

archive/issues_032483.json:
```json
{
    "assignees": [],
    "body": "The following causes an error:\n\n```\nsage: bool(magma(True))\n```\n\nBranch/Commit: **[u/roed/magma_bool](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/magma_bool) @ [473cd41](https://github.com/sagemath/sagetrac-mirror/commit/473cd41f19ec23df7e207391cfb0cf41c7c4ef46)**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32720_\n\n",
    "closed_at": "2021-10-19T06:32:08Z",
    "created_at": "2021-10-19T04:43:16Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20interfaces%3A%20optional",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Coercion of booleans from magma",
    "type": "issue",
    "updated_at": "2021-10-19T06:32:08Z",
    "url": "https://github.com/sagemath/sage/issues/32720",
    "user": "https://github.com/roed314"
}
```
The following causes an error:

```
sage: bool(magma(True))
```

Branch/Commit: **[u/roed/magma_bool](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/magma_bool) @ [473cd41](https://github.com/sagemath/sagetrac-mirror/commit/473cd41f19ec23df7e207391cfb0cf41c7c4ef46)**

_Issue created by migration from https://trac.sagemath.org/ticket/32720_





---

archive/issue_events_432393.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2021-10-19T04:43:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32720",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32720#event-432393"
}
```



---

archive/issue_events_432394.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2021-10-19T04:43:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32720",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces%3A%20optional",
    "label_color": "0000ff",
    "label_name": "component: interfaces: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32720#event-432394"
}
```



---

archive/issue_events_432395.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2021-10-19T04:43:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32720",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32720#event-432395"
}
```



---

archive/issue_events_432396.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2021-10-19T04:43:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32720",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32720#event-432396"
}
```



---

archive/issue_events_432397.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2021-10-19T04:46:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32720",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32720#event-432397"
}
```



---

archive/issue_comments_531011.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nDupe of #32602 ?",
    "created_at": "2021-10-19T04:46:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32720",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32720#issuecomment-531011",
    "user": "https://github.com/nbruin"
}
```

<div id="comment:1" align="right">Comment 1</div>

Dupe of #32602 ?



---

archive/issue_events_432398.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2021-10-19T04:46:33Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/32720",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32720#event-432398"
}
```



---

archive/issue_events_432399.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2021-10-19T04:46:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32720",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32720#event-432399"
}
```



---

archive/issue_events_432400.json:
```json
{
    "actor": "https://github.com/nbruin",
    "created_at": "2021-10-19T04:46:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32720",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32720#event-432400"
}
```



---

archive/issue_comments_531012.json:
```json
{
    "body": "Branch: **[u/roed/magma_bool](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/magma_bool)**",
    "created_at": "2021-10-19T04:57:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32720",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32720#issuecomment-531012",
    "user": "https://github.com/roed314"
}
```

Branch: **[u/roed/magma_bool](https://github.com/sagemath/sagetrac-mirror/tree/u/roed/magma_bool)**



---

archive/issue_comments_531013.json:
```json
{
    "body": "Commit: **[473cd41](https://github.com/sagemath/sagetrac-mirror/commit/473cd41f19ec23df7e207391cfb0cf41c7c4ef46)**",
    "created_at": "2021-10-19T05:00:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32720",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32720#issuecomment-531013",
    "user": "https://github.com/roed314"
}
```

Commit: **[473cd41](https://github.com/sagemath/sagetrac-mirror/commit/473cd41f19ec23df7e207391cfb0cf41c7c4ef46)**



---

archive/issue_comments_531014.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nYep.  I searched \"interfaces: optional\" not \"interfaces\".",
    "created_at": "2021-10-19T05:00:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32720",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32720#issuecomment-531014",
    "user": "https://github.com/roed314"
}
```

<div id="comment:3" align="right">Comment 3</div>

Yep.  I searched "interfaces: optional" not "interfaces".



---

archive/issue_events_432401.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2021-10-19T05:01:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32720",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32720#event-432401"
}
```



---

archive/issue_events_432402.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2021-10-19T05:01:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32720",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32720#event-432402"
}
```



---

archive/issue_events_432403.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-10-19T06:32:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32720",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32720#event-432403"
}
```



---

archive/issue_events_432404.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-10-19T06:32:08Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32720",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32720#event-432404"
}
```



---

archive/issue_events_432405.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-10-19T06:32:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32720",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32720#event-432405"
}
```
