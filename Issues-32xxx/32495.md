# Issue 32495: EllipticCurveIsogeny._richcmp_ does not use the kernel polynomial

archive/issues_032258.json:
```json
{
    "assignees": [],
    "body": "Back in the days, `EllipticCurveIsogeny._richcmp_` used to compare only the kernel polynomials. This was incorrect and is fixed since #11327, but traces of the old implementation remain:\n\n* Unnecessary `self.__init_kernel_polynomial()` in `_richcmp_`.\n* Docstring: \"This function works by comparing the underlying kernel objects\".\n\nThis (tiny) patch corrects these two things.\n\nCC:  @JohnCremona @categorie @defeo\n\nBranch/Commit: **[0bdb8f1](https://github.com/sagemath/sagetrac-mirror/commit/0bdb8f13163501d6c82e392e02df0f29a953bdd4)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Lorenz Panny**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32495_\n\n",
    "closed_at": "2021-10-23T22:58:31Z",
    "created_at": "2021-09-10T03:00:45Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20elliptic%20curves",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "EllipticCurveIsogeny._richcmp_ does not use the kernel polynomial",
    "type": "issue",
    "updated_at": "2021-10-23T22:58:31Z",
    "url": "https://github.com/sagemath/sage/issues/32495",
    "user": "https://github.com/yyyyx4"
}
```
Back in the days, `EllipticCurveIsogeny._richcmp_` used to compare only the kernel polynomials. This was incorrect and is fixed since #11327, but traces of the old implementation remain:

* Unnecessary `self.__init_kernel_polynomial()` in `_richcmp_`.
* Docstring: "This function works by comparing the underlying kernel objects".

This (tiny) patch corrects these two things.

CC:  @JohnCremona @categorie @defeo

Branch/Commit: **[0bdb8f1](https://github.com/sagemath/sagetrac-mirror/commit/0bdb8f13163501d6c82e392e02df0f29a953bdd4)**

Reviewer: **Travis Scrimshaw**

Author: **Lorenz Panny**

_Issue created by migration from https://trac.sagemath.org/ticket/32495_





---

archive/issue_events_429736.json:
```json
{
    "actor": "https://github.com/yyyyx4",
    "created_at": "2021-09-10T03:00:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32495",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32495#event-429736"
}
```



---

archive/issue_events_429737.json:
```json
{
    "actor": "https://github.com/yyyyx4",
    "created_at": "2021-09-10T03:00:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32495",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20elliptic%20curves",
    "label_color": "0000ff",
    "label_name": "component: elliptic curves",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32495#event-429737"
}
```



---

archive/issue_events_429738.json:
```json
{
    "actor": "https://github.com/yyyyx4",
    "created_at": "2021-09-10T03:00:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32495",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32495#event-429738"
}
```



---

archive/issue_events_429739.json:
```json
{
    "actor": "https://github.com/yyyyx4",
    "created_at": "2021-09-10T03:00:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32495",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32495#event-429739"
}
```



---

archive/issue_events_429740.json:
```json
{
    "actor": "https://github.com/yyyyx4",
    "created_at": "2021-09-10T03:07:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32495",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32495#event-429740"
}
```



---

archive/issue_comments_527130.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0bdb8f13163501d6c82e392e02df0f29a953bdd4\">0bdb8f1</a></td><td><code>kernel polynomial is not used anymore in _richcmp_</code></td></tr></table>\n",
    "created_at": "2021-09-10T03:07:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32495",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32495#issuecomment-527130",
    "user": "https://github.com/yyyyx4"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0bdb8f13163501d6c82e392e02df0f29a953bdd4">0bdb8f1</a></td><td><code>kernel polynomial is not used anymore in _richcmp_</code></td></tr></table>




---

archive/issue_comments_527131.json:
```json
{
    "body": "Author: **Lorenz Panny**",
    "created_at": "2021-09-10T03:07:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32495",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32495#issuecomment-527131",
    "user": "https://github.com/yyyyx4"
}
```

Author: **Lorenz Panny**



---

archive/issue_comments_527132.json:
```json
{
    "body": "Branch: **[public/32495](https://github.com/sagemath/sagetrac-mirror/tree/public/32495)**",
    "created_at": "2021-09-10T03:07:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32495",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32495#issuecomment-527132",
    "user": "https://github.com/yyyyx4"
}
```

Branch: **[public/32495](https://github.com/sagemath/sagetrac-mirror/tree/public/32495)**



---

archive/issue_comments_527133.json:
```json
{
    "body": "Commit: **[0bdb8f1](https://github.com/sagemath/sagetrac-mirror/commit/0bdb8f13163501d6c82e392e02df0f29a953bdd4)**",
    "created_at": "2021-09-10T03:07:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32495",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32495#issuecomment-527133",
    "user": "https://github.com/yyyyx4"
}
```

Commit: **[0bdb8f1](https://github.com/sagemath/sagetrac-mirror/commit/0bdb8f13163501d6c82e392e02df0f29a953bdd4)**



---

archive/issue_comments_527134.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -3,3 +3,4 @@\n * Unnecessary `self.__init_kernel_polynomial()` in `_richcmp_`.\n * Docstring: \"This function works by comparing the underlying kernel objects\".\n \n+This (tiny) patch corrects these two things.\n``````\n",
    "created_at": "2021-09-10T03:07:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32495",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32495#issuecomment-527134",
    "user": "https://github.com/yyyyx4"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -3,3 +3,4 @@
 * Unnecessary `self.__init_kernel_polynomial()` in `_richcmp_`.
 * Docstring: "This function works by comparing the underlying kernel objects".
 
+This (tiny) patch corrects these two things.
``````




---

archive/issue_comments_527135.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2021-09-13T01:20:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32495",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32495#issuecomment-527135",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_429741.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-09-13T01:20:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32495",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32495#event-429741"
}
```



---

archive/issue_events_429742.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2021-09-13T01:20:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32495",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32495#event-429742"
}
```



---

archive/issue_comments_527136.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nLGTM.",
    "created_at": "2021-09-13T01:20:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32495",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32495#issuecomment-527136",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:2" align="right">Comment 2</div>

LGTM.



---

archive/issue_events_429743.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-10-23T22:58:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32495",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32495#event-429743"
}
```



---

archive/issue_events_429744.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "ec536c7872dd6deb758c383bdda10bf957b2f9aa",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-10-23T22:58:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32495",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32495#event-429744"
}
```



---

archive/issue_comments_527137.json:
```json
{
    "body": "Changed branch from **[public/32495](https://github.com/sagemath/sagetrac-mirror/tree/public/32495)** to **[0bdb8f1](https://github.com/sagemath/sagetrac-mirror/commit/0bdb8f13163501d6c82e392e02df0f29a953bdd4)**",
    "created_at": "2021-10-23T22:58:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32495",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32495#issuecomment-527137",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/32495](https://github.com/sagemath/sagetrac-mirror/tree/public/32495)** to **[0bdb8f1](https://github.com/sagemath/sagetrac-mirror/commit/0bdb8f13163501d6c82e392e02df0f29a953bdd4)**
