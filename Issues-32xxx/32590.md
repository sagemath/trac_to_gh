# Issue 32590: Update memory_allocator to 0.1.1 to fix self-tests on 32bit systems

archive/issues_032353.json:
```json
{
    "assignees": [],
    "body": "memory_allocator 0.1.0 does not pass self-tests on 32bit systems, see the attached log.\n\nCC:  @kliem @embray\n\nKeywords: **sdl**\n\nBranch/Commit: **[27bc041](https://github.com/sagemath/sagetrac-mirror/commit/27bc041532bdeadd69f5285e0991fecad9da5022)**\n\nUpstream: **Completely fixed; Fix reported upstream**\n\nReviewer: **Thierry Monteil**\n\nAuthor: **Jonathan Kliem**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32590_\n\n",
    "closed_at": "2021-10-13T22:50:49Z",
    "created_at": "2021-09-30T00:09:41Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Update memory_allocator to 0.1.1 to fix self-tests on 32bit systems",
    "type": "issue",
    "updated_at": "2021-10-13T22:50:49Z",
    "url": "https://github.com/sagemath/sage/issues/32590",
    "user": "https://github.com/sagetrac-tmonteil"
}
```
memory_allocator 0.1.0 does not pass self-tests on 32bit systems, see the attached log.

CC:  @kliem @embray

Keywords: **sdl**

Branch/Commit: **[27bc041](https://github.com/sagemath/sagetrac-mirror/commit/27bc041532bdeadd69f5285e0991fecad9da5022)**

Upstream: **Completely fixed; Fix reported upstream**

Reviewer: **Thierry Monteil**

Author: **Jonathan Kliem**

_Issue created by migration from https://trac.sagemath.org/ticket/32590_





---

archive/issue_events_430799.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2021-09-30T00:09:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32590",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32590#event-430799"
}
```



---

archive/issue_events_430800.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2021-09-30T00:09:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32590",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32590#event-430800"
}
```



---

archive/issue_events_430801.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2021-09-30T00:09:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32590",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32590#event-430801"
}
```



---

archive/issue_events_430802.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2021-09-30T00:09:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32590",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32590#event-430802"
}
```



---

archive/issue_comments_528480.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nAttachment: **[memory_allocator-0.1.0.log](https://github.com/sagemath/sage/files/ticket32590/memory_allocator-0.1.0.log)**",
    "created_at": "2021-09-30T00:11:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32590",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32590#issuecomment-528480",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

<div id="comment:1" align="right">Comment 1</div>

Attachment: **[memory_allocator-0.1.0.log](https://github.com/sagemath/sage/files/ticket32590/memory_allocator-0.1.0.log)**



---

archive/issue_comments_528481.json:
```json
{
    "body": "Author: **Jonathan Kliem**",
    "created_at": "2021-09-30T09:34:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32590",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32590#issuecomment-528481",
    "user": "https://github.com/kliem"
}
```

Author: **Jonathan Kliem**



---

archive/issue_comments_528482.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nThanks for the report, I'll fix it.\n\n(This really is just an issue of the tests.)",
    "created_at": "2021-09-30T09:34:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32590",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32590#issuecomment-528482",
    "user": "https://github.com/kliem"
}
```

<div id="comment:2" align="right">Comment 2</div>

Thanks for the report, I'll fix it.

(This really is just an issue of the tests.)



---

archive/issue_comments_528483.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nApparently https://github.com/sagemath/memory_allocator/pull/1 tests and fixes the issue.\n\nThat it is indeed tested, can be witnessed here:\n\nhttps://github.com/sagemath/memory_allocator/actions/runs/1290649550\n\nWhat is not nice, is that I didn't find a good way to have `python test.py` fail the github workflow except for `python test.py || exit 1`. This works, because `exit 1` is incorrect syntax (so not nice, but will do exact the desired thing).",
    "created_at": "2021-09-30T11:15:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32590",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32590#issuecomment-528483",
    "user": "https://github.com/kliem"
}
```

<div id="comment:3" align="right">Comment 3</div>

Apparently https://github.com/sagemath/memory_allocator/pull/1 tests and fixes the issue.

That it is indeed tested, can be witnessed here:

https://github.com/sagemath/memory_allocator/actions/runs/1290649550

What is not nice, is that I didn't find a good way to have `python test.py` fail the github workflow except for `python test.py || exit 1`. This works, because `exit 1` is incorrect syntax (so not nice, but will do exact the desired thing).



---

archive/issue_comments_528484.json:
```json
{
    "body": "Upstream: **Fixed upstream, but not in a stable release.**",
    "created_at": "2021-09-30T11:21:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32590",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32590#issuecomment-528484",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Upstream: **Fixed upstream, but not in a stable release.**



---

archive/issue_comments_528485.json:
```json
{
    "body": "Changed upstream from **Fixed upstream, but not in a stable release.** to **Completely fixed; Fix reported upstream**",
    "created_at": "2021-09-30T13:50:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32590",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32590#issuecomment-528485",
    "user": "https://github.com/kliem"
}
```

Changed upstream from **Fixed upstream, but not in a stable release.** to **Completely fixed; Fix reported upstream**



---

archive/issue_comments_528486.json:
```json
{
    "body": "Branch: **[public/32590](https://github.com/sagemath/sagetrac-mirror/tree/public/32590)**",
    "created_at": "2021-09-30T13:50:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32590",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32590#issuecomment-528486",
    "user": "https://github.com/kliem"
}
```

Branch: **[public/32590](https://github.com/sagemath/sagetrac-mirror/tree/public/32590)**



---

archive/issue_comments_528487.json:
```json
{
    "body": "Commit: **[27bc041](https://github.com/sagemath/sagetrac-mirror/commit/27bc041532bdeadd69f5285e0991fecad9da5022)**",
    "created_at": "2021-09-30T13:50:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32590",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32590#issuecomment-528487",
    "user": "https://github.com/kliem"
}
```

Commit: **[27bc041](https://github.com/sagemath/sagetrac-mirror/commit/27bc041532bdeadd69f5285e0991fecad9da5022)**



---

archive/issue_events_430803.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2021-09-30T13:50:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32590",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32590#event-430803"
}
```



---

archive/issue_events_430804.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2021-09-30T13:50:59Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/32590",
    "title_is": "Update memory_allocator to 0.1.1 to fix self-tests on 32bit systems",
    "title_was": "memory_allocator 0.1.0 does not pass self-tests on 32bit systems",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32590#event-430804"
}
```



---

archive/issue_comments_528488.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/27bc041532bdeadd69f5285e0991fecad9da5022\">27bc041</a></td><td><code>update memory_allocator to fix test failures on 32bit</code></td></tr></table>\n",
    "created_at": "2021-09-30T13:50:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32590",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32590#issuecomment-528488",
    "user": "https://github.com/kliem"
}
```

<div id="comment:5" align="right">Comment 5</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/27bc041532bdeadd69f5285e0991fecad9da5022">27bc041</a></td><td><code>update memory_allocator to fix test failures on 32bit</code></td></tr></table>




---

archive/issue_comments_528489.json:
```json
{
    "body": "Reviewer: **Thierry Monteil**",
    "created_at": "2021-09-30T15:34:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32590",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32590#issuecomment-528489",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Reviewer: **Thierry Monteil**



---

archive/issue_events_430805.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2021-09-30T15:34:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32590",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32590#event-430805"
}
```



---

archive/issue_events_430806.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2021-09-30T15:34:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32590",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32590#event-430806"
}
```



---

archive/issue_comments_528490.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nThis works now, tested on both on 32bit and 64bit systems, thanks for fixing !",
    "created_at": "2021-09-30T15:34:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32590",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32590#issuecomment-528490",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

<div id="comment:6" align="right">Comment 6</div>

This works now, tested on both on 32bit and 64bit systems, thanks for fixing !



---

archive/issue_comments_528491.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nThanks for reviewing and testing.",
    "created_at": "2021-10-01T06:33:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32590",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32590#issuecomment-528491",
    "user": "https://github.com/kliem"
}
```

<div id="comment:7" align="right">Comment 7</div>

Thanks for reviewing and testing.



---

archive/issue_comments_528492.json:
```json
{
    "body": "Changed branch from **[public/32590](https://github.com/sagemath/sagetrac-mirror/tree/public/32590)** to **[27bc041](https://github.com/sagemath/sagetrac-mirror/commit/27bc041532bdeadd69f5285e0991fecad9da5022)**",
    "created_at": "2021-10-13T22:50:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32590",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32590#issuecomment-528492",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/32590](https://github.com/sagemath/sagetrac-mirror/tree/public/32590)** to **[27bc041](https://github.com/sagemath/sagetrac-mirror/commit/27bc041532bdeadd69f5285e0991fecad9da5022)**



---

archive/issue_events_430807.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-10-13T22:50:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32590",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32590#event-430807"
}
```



---

archive/issue_events_430808.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "f4d99de8b2f3bd77c9c36e454a6d149db5458222",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-10-13T22:50:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32590",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32590#event-430808"
}
```
