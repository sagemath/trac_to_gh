# Issue 32303: numpy needs Python >= 3.7, but Sage only installs 3.6.13

archive/issues_032066.json:
```json
{
    "body": "Make-building Sage 9.4.rc0 on Cygwin fails shortly after the beginning of numpy installation:\n\n```\n[numpy-1.20.3] Supported LTO compression algorithms: zlib zstd\n[numpy-1.20.3] gcc version 10.2.0 (GCC)\n[numpy-1.20.3] ****************************************************\n[numpy-1.20.3] Uninstalling 'numpy' with legacy uninstaller\n[numpy-1.20.3] Package 'numpy' is currently not installed\n[numpy-1.20.3] Traceback (most recent call last):\n[numpy-1.20.3]   File \"setup.py\", line 30, in <module>\n[numpy-1.20.3]     raise RuntimeError(\"Python version >= 3.7 required.\")\n[numpy-1.20.3] RuntimeError: Python version >= 3.7 required.\n[numpy-1.20.3] **********************************************************************************************************************************************************************************************\n[numpy-1.20.3] Error building a wheel for numpy-1.20.3\n[numpy-1.20.3] **********************************************************************************************************************************************************************************************\n[numpy-1.20.3]\n[numpy-1.20.3] real     0m0.783s\n[numpy-1.20.3] user     0m0.320s\n[numpy-1.20.3] sys      0m0.546s\n[numpy-1.20.3] ************************************************************************\n[numpy-1.20.3] Error installing package numpy-1.20.3\n[numpy-1.20.3] ************************************************************************\n```\n\nCC:  @mkoeppe\n\nKeywords: install, dependencies, numpy, python\n\nResolution: invalid\n\nIssue created by migration from https://trac.sagemath.org/ticket/32303\n\n",
    "closed_at": "2021-08-02T14:55:28Z",
    "created_at": "2021-07-29T15:07:11Z",
    "labels": [
        "component: python3",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "numpy needs Python >= 3.7, but Sage only installs 3.6.13",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/32303",
    "user": "https://github.com/darijgr"
}
```
Make-building Sage 9.4.rc0 on Cygwin fails shortly after the beginning of numpy installation:

```
[numpy-1.20.3] Supported LTO compression algorithms: zlib zstd
[numpy-1.20.3] gcc version 10.2.0 (GCC)
[numpy-1.20.3] ****************************************************
[numpy-1.20.3] Uninstalling 'numpy' with legacy uninstaller
[numpy-1.20.3] Package 'numpy' is currently not installed
[numpy-1.20.3] Traceback (most recent call last):
[numpy-1.20.3]   File "setup.py", line 30, in <module>
[numpy-1.20.3]     raise RuntimeError("Python version >= 3.7 required.")
[numpy-1.20.3] RuntimeError: Python version >= 3.7 required.
[numpy-1.20.3] **********************************************************************************************************************************************************************************************
[numpy-1.20.3] Error building a wheel for numpy-1.20.3
[numpy-1.20.3] **********************************************************************************************************************************************************************************************
[numpy-1.20.3]
[numpy-1.20.3] real     0m0.783s
[numpy-1.20.3] user     0m0.320s
[numpy-1.20.3] sys      0m0.546s
[numpy-1.20.3] ************************************************************************
[numpy-1.20.3] Error installing package numpy-1.20.3
[numpy-1.20.3] ************************************************************************
```

CC:  @mkoeppe

Keywords: install, dependencies, numpy, python

Resolution: invalid

Issue created by migration from https://trac.sagemath.org/ticket/32303





---

archive/issue_comments_488829.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2021-08-01T19:34:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32303",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32303#issuecomment-488829",
    "user": "https://github.com/mkoeppe"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_488830.json:
```json
{
    "body": "<a id='comment:1'></a>Sage definitely does not install Python 3.6.13. You are trying to do an incremental build using a system python version that is no longer supported. Just rebuild from scratch.\n\n(See #29708 for the task of catching this in the configuration phase.)",
    "created_at": "2021-08-01T19:34:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32303",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32303#issuecomment-488830",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:1'></a>Sage definitely does not install Python 3.6.13. You are trying to do an incremental build using a system python version that is no longer supported. Just rebuild from scratch.

(See #29708 for the task of catching this in the configuration phase.)



---

archive/issue_events_085667.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-01T19:34:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/32303",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/32303#event-85667"
}
```



---

archive/issue_comments_488831.json:
```json
{
    "body": "<a id='comment:2'></a>Ah! I was thinking Sage would automatically update all packages, or, if a system package was used that is now out of date, automatically switch to its own version. But that's probably harder than it sounds...",
    "created_at": "2021-08-01T19:49:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32303",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32303#issuecomment-488831",
    "user": "https://github.com/darijgr"
}
```

<a id='comment:2'></a>Ah! I was thinking Sage would automatically update all packages, or, if a system package was used that is now out of date, automatically switch to its own version. But that's probably harder than it sounds...



---

archive/issue_events_085668.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-08-02T14:55:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/32303",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/32303#event-85668"
}
```



---

archive/issue_comments_488832.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2021-08-02T14:55:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32303",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32303#issuecomment-488832",
    "user": "https://github.com/fchapoton"
}
```

Resolution: invalid
