...
<a id='comment:46'></a>Replying to [mantepse](#comment%3A44):
> Trying to get back to this ticket :-)
...
<a id='comment:84'></a>Replying to [mantepse](#comment%3A79):
> Do you know whether it is correct that `1/(1-z)` calls `_div_`, and `(1-z)^-1` calls `_pow_` rather than `_invert_`?  Would it make sense to redirect these two?
...
```json
{
    "body": "<a id='comment:86'></a>comment:81,[comment:82](#comment%3A82): Yes, I think this is all because we didn't have the ABC of `LazySeries` when we first did things and never migrated things over. This likely also applies to[comment:83](#comment%3A83) too.",
...
<a id='comment:86'></a>comment:81,[comment:82](#comment%3A82): Yes, I think this is all because we didn't have the ABC of `LazySeries` when we first did things and never migrated things over. This likely also applies to[comment:83](#comment%3A83) too.
...
<a id='comment:94'></a>Replying to [tscrim](#comment%3A88):
> In the Frobenius character of unlabeled trees test, I need to explicitly declare the `valuation = 1` when I add in a test for composing that the valuation is positive.
...
<a id='comment:104'></a>Replying to [mantepse](#comment%3A103):
> It might be interesting to compare with fricas speed wise.
