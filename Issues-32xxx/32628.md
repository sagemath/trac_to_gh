# Issue 32628: ConvexSet_base, Polyhedron_base: In _test methods, do not fail if some modules are unavailable

archive/issues_032391.json:
```json
{
    "assignees": [],
    "body": "(cherry-picked from #32432)\n\n\nCC:  @kliem\n\nBranch/Commit: **[52ac1da](https://github.com/sagemath/sagetrac-mirror/commit/52ac1dac69cfc8b869459e08408ff94ff2f81611)**\n\nReviewer: **Jonathan Kliem**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32628_\n\n",
    "closed_at": "2021-10-19T20:32:07Z",
    "created_at": "2021-10-05T06:49:33Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/refactoring",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "ConvexSet_base, Polyhedron_base: In _test methods, do not fail if some modules are unavailable",
    "type": "issue",
    "updated_at": "2021-10-19T20:32:07Z",
    "url": "https://github.com/sagemath/sage/issues/32628",
    "user": "https://github.com/mkoeppe"
}
```
(cherry-picked from #32432)


CC:  @kliem

Branch/Commit: **[52ac1da](https://github.com/sagemath/sagetrac-mirror/commit/52ac1dac69cfc8b869459e08408ff94ff2f81611)**

Reviewer: **Jonathan Kliem**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/32628_





---

archive/issue_events_431241.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-05T06:49:33Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32628",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32628#event-431241"
}
```



---

archive/issue_events_431242.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-05T06:49:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32628",
    "label": "https://github.com/sagemath/sage/labels/refactoring",
    "label_color": "696969",
    "label_name": "refactoring",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32628#event-431242"
}
```



---

archive/issue_events_431243.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-05T06:49:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32628",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32628#event-431243"
}
```



---

archive/issue_events_431244.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-05T06:49:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32628",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32628#event-431244"
}
```



---

archive/issue_comments_529243.json:
```json
{
    "body": "Branch: **[u/mkoeppe/convexset_base__polyhedron_base__in__test_methods__do_not_fail_if_some_modules_are_unavailable](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/convexset_base__polyhedron_base__in__test_methods__do_not_fail_if_some_modules_are_unavailable)**",
    "created_at": "2021-10-05T06:56:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32628",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32628#issuecomment-529243",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/convexset_base__polyhedron_base__in__test_methods__do_not_fail_if_some_modules_are_unavailable](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/convexset_base__polyhedron_base__in__test_methods__do_not_fail_if_some_modules_are_unavailable)**



---

archive/issue_comments_529244.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/28dc817c7b8a97807e5514d28a66a178652d3ed4\">28dc817</a></td><td><code>Polyhedron._test...: Avoid testing using unavailable functionality</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/52ac1dac69cfc8b869459e08408ff94ff2f81611\">52ac1da</a></td><td><code>Polyhedron_base._test_gale_transform: Do not test is_combinatorially_isomorphic if sage.graphs.graphs not available</code></td></tr></table>\n",
    "created_at": "2021-10-05T06:57:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32628",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32628#issuecomment-529244",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/28dc817c7b8a97807e5514d28a66a178652d3ed4">28dc817</a></td><td><code>Polyhedron._test...: Avoid testing using unavailable functionality</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/52ac1dac69cfc8b869459e08408ff94ff2f81611">52ac1da</a></td><td><code>Polyhedron_base._test_gale_transform: Do not test is_combinatorially_isomorphic if sage.graphs.graphs not available</code></td></tr></table>




---

archive/issue_comments_529245.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2021-10-05T06:57:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32628",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32628#issuecomment-529245",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_events_431245.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-05T06:57:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32628",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32628#event-431245"
}
```



---

archive/issue_comments_529246.json:
```json
{
    "body": "Commit: **[52ac1da](https://github.com/sagemath/sagetrac-mirror/commit/52ac1dac69cfc8b869459e08408ff94ff2f81611)**",
    "created_at": "2021-10-05T06:57:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32628",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32628#issuecomment-529246",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[52ac1da](https://github.com/sagemath/sagetrac-mirror/commit/52ac1dac69cfc8b869459e08408ff94ff2f81611)**



---

archive/issue_comments_529247.json:
```json
{
    "body": "Reviewer: **Jonathan Kliem**",
    "created_at": "2021-10-05T07:16:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32628",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32628#issuecomment-529247",
    "user": "https://github.com/kliem"
}
```

Reviewer: **Jonathan Kliem**



---

archive/issue_events_431246.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2021-10-05T07:16:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32628",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32628#event-431246"
}
```



---

archive/issue_events_431247.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2021-10-05T07:16:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32628",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32628#event-431247"
}
```



---

archive/issue_comments_529248.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLGTM.",
    "created_at": "2021-10-05T07:16:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32628",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32628#issuecomment-529248",
    "user": "https://github.com/kliem"
}
```

<div id="comment:3" align="right">Comment 3</div>

LGTM.



---

archive/issue_comments_529249.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThanks!",
    "created_at": "2021-10-05T16:01:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32628",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32628#issuecomment-529249",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:4" align="right">Comment 4</div>

Thanks!



---

archive/issue_comments_529250.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/convexset_base__polyhedron_base__in__test_methods__do_not_fail_if_some_modules_are_unavailable](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/convexset_base__polyhedron_base__in__test_methods__do_not_fail_if_some_modules_are_unavailable)** to **[52ac1da](https://github.com/sagemath/sagetrac-mirror/commit/52ac1dac69cfc8b869459e08408ff94ff2f81611)**",
    "created_at": "2021-10-19T20:32:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32628",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32628#issuecomment-529250",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/convexset_base__polyhedron_base__in__test_methods__do_not_fail_if_some_modules_are_unavailable](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/convexset_base__polyhedron_base__in__test_methods__do_not_fail_if_some_modules_are_unavailable)** to **[52ac1da](https://github.com/sagemath/sagetrac-mirror/commit/52ac1dac69cfc8b869459e08408ff94ff2f81611)**



---

archive/issue_events_431248.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-10-19T20:32:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32628",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32628#event-431248"
}
```



---

archive/issue_events_431249.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "93abcdb4cec51222d119bee3c989f83a833783a9",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-10-19T20:32:07Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32628",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32628#event-431249"
}
```
