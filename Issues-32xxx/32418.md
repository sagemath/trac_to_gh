# Issue 32418: archlinux-latest: Trivial doctest failure

archive/issues_032181.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage -t --random-seed=0 src/sage/interfaces/quit.py\n**********************************************************************\nFile \"src/sage/interfaces/quit.py\", line 30, in sage.interfaces.quit.expect_quitall\nFailed example:\n    sage.interfaces.quit.expect_quitall(verbose=True)\nExpected:\n    Exiting PARI/GP interpreter with PID ... running .../bin/gp --fast --emacs --quiet --stacksize 10000000\nGot:\n    Exiting PARI/GP interpreter with PID 153457 running /usr/sbin/gp --fast --emacs --quiet --stacksize 10000000\n```\n(seen in https://github.com/sympy/sympy/pull/21941/checks?check_run_id=3417685771)\n\nCC:  @antonio-rojas\n\nBranch/Commit: **[21da195](https://github.com/sagemath/sagetrac-mirror/commit/21da195aa9a0f40dedc88eca249a10ef516e93b6)**\n\nReviewer: **Matthias Koeppe**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32418_\n\n",
    "closed_at": "2021-09-19T09:59:01Z",
    "created_at": "2021-08-25T05:39:40Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "archlinux-latest: Trivial doctest failure",
    "type": "issue",
    "updated_at": "2021-09-19T09:59:01Z",
    "url": "https://github.com/sagemath/sage/issues/32418",
    "user": "https://github.com/mkoeppe"
}
```

```
sage -t --random-seed=0 src/sage/interfaces/quit.py
**********************************************************************
File "src/sage/interfaces/quit.py", line 30, in sage.interfaces.quit.expect_quitall
Failed example:
    sage.interfaces.quit.expect_quitall(verbose=True)
Expected:
    Exiting PARI/GP interpreter with PID ... running .../bin/gp --fast --emacs --quiet --stacksize 10000000
Got:
    Exiting PARI/GP interpreter with PID 153457 running /usr/sbin/gp --fast --emacs --quiet --stacksize 10000000
```
(seen in https://github.com/sympy/sympy/pull/21941/checks?check_run_id=3417685771)

CC:  @antonio-rojas

Branch/Commit: **[21da195](https://github.com/sagemath/sagetrac-mirror/commit/21da195aa9a0f40dedc88eca249a10ef516e93b6)**

Reviewer: **Matthias Koeppe**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/32418_





---

archive/issue_events_428806.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-25T05:39:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32418",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32418#event-428806"
}
```



---

archive/issue_events_428807.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-25T05:39:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32418",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting",
    "label_color": "000080",
    "label_name": "component: porting",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32418#event-428807"
}
```



---

archive/issue_events_428808.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-25T05:39:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32418",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32418#event-428808"
}
```



---

archive/issue_events_428809.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-25T05:39:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32418",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32418#event-428809"
}
```



---

archive/issue_comments_525652.json:
```json
{
    "body": "Commit: **[21da195](https://github.com/sagemath/sagetrac-mirror/commit/21da195aa9a0f40dedc88eca249a10ef516e93b6)**",
    "created_at": "2021-08-25T15:29:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32418",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32418#issuecomment-525652",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[21da195](https://github.com/sagemath/sagetrac-mirror/commit/21da195aa9a0f40dedc88eca249a10ef516e93b6)**



---

archive/issue_comments_525653.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nhere is a branch\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/21da195aa9a0f40dedc88eca249a10ef516e93b6\">21da195</a></td><td><code>fix one doctest for archlinux</code></td></tr></table>\n",
    "created_at": "2021-08-25T15:29:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32418",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32418#issuecomment-525653",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:1" align="right">Comment 1</div>

here is a branch

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/21da195aa9a0f40dedc88eca249a10ef516e93b6">21da195</a></td><td><code>fix one doctest for archlinux</code></td></tr></table>




---

archive/issue_events_428810.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-08-25T15:29:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32418",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32418#event-428810"
}
```



---

archive/issue_comments_525654.json:
```json
{
    "body": "Branch: **[u/chapoton/32418](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/32418)**",
    "created_at": "2021-08-25T15:29:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32418",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32418#issuecomment-525654",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/32418](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/32418)**



---

archive/issue_comments_525655.json:
```json
{
    "body": "Author: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2021-08-25T15:29:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32418",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32418#issuecomment-525655",
    "user": "https://github.com/fchapoton"
}
```

Author: **Frédéric Chapoton**



---

archive/issue_events_428811.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-25T16:05:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32418",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32418#event-428811"
}
```



---

archive/issue_events_428812.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-25T16:05:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32418",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32418#event-428812"
}
```



---

archive/issue_comments_525656.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2021-08-25T16:05:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32418",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32418#issuecomment-525656",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_comments_525657.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nIncreasing priority for the benefit of the sympy-sage CI.",
    "created_at": "2021-09-02T21:02:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32418",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32418#issuecomment-525657",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:3" align="right">Comment 3</div>

Increasing priority for the benefit of the sympy-sage CI.



---

archive/issue_events_428813.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-02T21:02:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32418",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32418#event-428813"
}
```



---

archive/issue_events_428814.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-09-02T21:02:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32418",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32418#event-428814"
}
```



---

archive/issue_events_428815.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-09-19T09:59:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32418",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32418#event-428815"
}
```



---

archive/issue_events_428816.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "c8dc0bcc6bf37db8f3dee4050ca2067a062aa436",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-09-19T09:59:01Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32418",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32418#event-428816"
}
```



---

archive/issue_comments_525658.json:
```json
{
    "body": "Changed branch from **[u/chapoton/32418](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/32418)** to **[21da195](https://github.com/sagemath/sagetrac-mirror/commit/21da195aa9a0f40dedc88eca249a10ef516e93b6)**",
    "created_at": "2021-09-19T09:59:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32418",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32418#issuecomment-525658",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/32418](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/32418)** to **[21da195](https://github.com/sagemath/sagetrac-mirror/commit/21da195aa9a0f40dedc88eca249a10ef516e93b6)**
