# Issue 32252: fix colours of tiles in TilingSolver

archive/issues_032015.json:
```json
{
    "assignees": [],
    "body": "Here is an example illustrating the problem. We compute the tilings of a 2x2 square using vertical (blue) and horizontal dominos (red).\n\n```\nv = Polyomino([(0, 0), (0, 1)], color=\"blue\")\nh = Polyomino([(0, 0), (1, 0)], color=\"red\")\nT = TilingSolver(pieces=[v, h], box=(2, 2), rotation=False, reflection=False, reusable=True)\nfor soln in T.solve():\n    show(sum([p.show2d() for p in soln]))\n```\n\nWe should get two tilings, one with only red dominos and one with blue dominos, but instead both tilings use red dominos.\n\nThe problem is in the method `T.row_to_polyomino`, which miscalculates the colour (the default value is -1 and so the colour of the last piece is always used).\n\nCC:  @seblabbe\n\nKeywords: **tilings, TilingSolver**\n\nBranch/Commit: **[9be13da](https://github.com/sagemath/sagetrac-mirror/commit/9be13daf462d55061728d9f369e4358b407630da)**\n\nReviewer: **Franco Saliola**\n\nAuthor: **S\u00e9bastien Labb\u00e9**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32252_\n\n",
    "closed_at": "2022-01-31T23:31:30Z",
    "created_at": "2021-07-20T15:22:25Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix colours of tiles in TilingSolver",
    "type": "issue",
    "updated_at": "2022-01-31T23:31:30Z",
    "url": "https://github.com/sagemath/sage/issues/32252",
    "user": "https://github.com/saliola"
}
```
Here is an example illustrating the problem. We compute the tilings of a 2x2 square using vertical (blue) and horizontal dominos (red).

```
v = Polyomino([(0, 0), (0, 1)], color="blue")
h = Polyomino([(0, 0), (1, 0)], color="red")
T = TilingSolver(pieces=[v, h], box=(2, 2), rotation=False, reflection=False, reusable=True)
for soln in T.solve():
    show(sum([p.show2d() for p in soln]))
```

We should get two tilings, one with only red dominos and one with blue dominos, but instead both tilings use red dominos.

The problem is in the method `T.row_to_polyomino`, which miscalculates the colour (the default value is -1 and so the colour of the last piece is always used).

CC:  @seblabbe

Keywords: **tilings, TilingSolver**

Branch/Commit: **[9be13da](https://github.com/sagemath/sagetrac-mirror/commit/9be13daf462d55061728d9f369e4358b407630da)**

Reviewer: **Franco Saliola**

Author: **Sébastien Labbé**

_Issue created by migration from https://trac.sagemath.org/ticket/32252_





---

archive/issue_events_426596.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2021-07-20T15:22:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32252",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32252#event-426596"
}
```



---

archive/issue_events_426597.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2021-07-20T15:22:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32252",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32252#event-426597"
}
```



---

archive/issue_events_426598.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2021-07-20T15:22:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32252",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32252#event-426598"
}
```



---

archive/issue_events_426599.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2021-07-20T15:22:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32252",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32252#event-426599"
}
```



---

archive/issue_events_426600.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-22T20:35:10Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/32252",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32252#event-426600"
}
```



---

archive/issue_events_426601.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-22T20:35:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32252",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32252#event-426601"
}
```



---

archive/issue_events_426602.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T20:12:32Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/32252",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32252#event-426602"
}
```



---

archive/issue_events_426603.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T20:12:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32252",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32252#event-426603"
}
```



---

archive/issue_events_426604.json:
```json
{
    "actor": "https://github.com/seblabbe",
    "created_at": "2022-01-04T15:49:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32252",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32252#event-426604"
}
```



---

archive/issue_comments_522594.json:
```json
{
    "body": "Commit: **[9be13da](https://github.com/sagemath/sagetrac-mirror/commit/9be13daf462d55061728d9f369e4358b407630da)**",
    "created_at": "2022-01-04T15:49:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32252#issuecomment-522594",
    "user": "https://github.com/seblabbe"
}
```

Commit: **[9be13da](https://github.com/sagemath/sagetrac-mirror/commit/9be13daf462d55061728d9f369e4358b407630da)**



---

archive/issue_comments_522595.json:
```json
{
    "body": "Branch: **[u/slabbe/32252](https://github.com/sagemath/sagetrac-mirror/tree/u/slabbe/32252)**",
    "created_at": "2022-01-04T15:49:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32252#issuecomment-522595",
    "user": "https://github.com/seblabbe"
}
```

Branch: **[u/slabbe/32252](https://github.com/sagemath/sagetrac-mirror/tree/u/slabbe/32252)**



---

archive/issue_comments_522596.json:
```json
{
    "body": "Author: **S\u00e9bastien Labb\u00e9**",
    "created_at": "2022-01-04T15:49:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32252#issuecomment-522596",
    "user": "https://github.com/seblabbe"
}
```

Author: **Sébastien Labbé**



---

archive/issue_comments_522597.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nMerci Franco d'avoir trouv\u00e9 ce bogue. Je viens de le corriger. Needs review et bonne ann\u00e9e!\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9be13daf462d55061728d9f369e4358b407630da\">9be13da</a></td><td><code>32252: fixing color in row_to_polyomino method</code></td></tr></table>\n",
    "created_at": "2022-01-04T15:49:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32252#issuecomment-522597",
    "user": "https://github.com/seblabbe"
}
```

<div id="comment:3" align="right">Comment 3</div>

Merci Franco d'avoir trouvé ce bogue. Je viens de le corriger. Needs review et bonne année!

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9be13daf462d55061728d9f369e4358b407630da">9be13da</a></td><td><code>32252: fixing color in row_to_polyomino method</code></td></tr></table>




---

archive/issue_comments_522598.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nlgtm",
    "created_at": "2022-01-05T20:48:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32252#issuecomment-522598",
    "user": "https://github.com/saliola"
}
```

<div id="comment:4" align="right">Comment 4</div>

lgtm



---

archive/issue_events_426605.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2022-01-05T20:48:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32252",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32252#event-426605"
}
```



---

archive/issue_events_426606.json:
```json
{
    "actor": "https://github.com/saliola",
    "created_at": "2022-01-05T20:48:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32252",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32252#event-426606"
}
```



---

archive/issue_comments_522599.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nMerci Franco!",
    "created_at": "2022-01-05T21:14:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32252#issuecomment-522599",
    "user": "https://github.com/seblabbe"
}
```

<div id="comment:5" align="right">Comment 5</div>

Merci Franco!



---

archive/issue_comments_522600.json:
```json
{
    "body": "Reviewer: **Franco Saliola**",
    "created_at": "2022-01-05T21:14:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32252#issuecomment-522600",
    "user": "https://github.com/seblabbe"
}
```

Reviewer: **Franco Saliola**



---

archive/issue_comments_522601.json:
```json
{
    "body": "Changed branch from **[u/slabbe/32252](https://github.com/sagemath/sagetrac-mirror/tree/u/slabbe/32252)** to **[9be13da](https://github.com/sagemath/sagetrac-mirror/commit/9be13daf462d55061728d9f369e4358b407630da)**",
    "created_at": "2022-01-31T23:31:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32252",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32252#issuecomment-522601",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/slabbe/32252](https://github.com/sagemath/sagetrac-mirror/tree/u/slabbe/32252)** to **[9be13da](https://github.com/sagemath/sagetrac-mirror/commit/9be13daf462d55061728d9f369e4358b407630da)**



---

archive/issue_events_426607.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2022-01-31T23:31:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32252",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32252#event-426607"
}
```



---

archive/issue_events_426608.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "742c77c14b2a2cb48da1323e50d44e21d98aa4f9",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2022-01-31T23:31:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32252",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32252#event-426608"
}
```
