# Issue 32374: Remove pip package "pyopenssl"

archive/issues_032137.json:
```json
{
    "assignees": [],
    "body": "It is not needed for anything.\n\nWith the update of `requests` in #32372, it is not used for anything any more even if it is installed. https://docs.python-requests.org/en/master/community/updates/#id4\n\n\nDepends on #32372\n\nCC:  @slel @NathanDunfield @sagetrac-tmonteil @dimpase\n\nBranch/Commit: **[a9b2bd6](https://github.com/sagemath/sagetrac-mirror/commit/a9b2bd6c52f0e49bd877829c2b7922510bc3e153)**\n\nReviewer: **Thierry Monteil**\n\nAuthor: **Matthias Koeppe**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32374_\n\n",
    "closed_at": "2021-10-09T11:09:52Z",
    "created_at": "2021-08-13T02:13:40Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove pip package \"pyopenssl\"",
    "type": "issue",
    "updated_at": "2021-10-09T11:09:52Z",
    "url": "https://github.com/sagemath/sage/issues/32374",
    "user": "https://github.com/mkoeppe"
}
```
It is not needed for anything.

With the update of `requests` in #32372, it is not used for anything any more even if it is installed. https://docs.python-requests.org/en/master/community/updates/#id4


Depends on #32372

CC:  @slel @NathanDunfield @sagetrac-tmonteil @dimpase

Branch/Commit: **[a9b2bd6](https://github.com/sagemath/sagetrac-mirror/commit/a9b2bd6c52f0e49bd877829c2b7922510bc3e153)**

Reviewer: **Thierry Monteil**

Author: **Matthias Koeppe**

_Issue created by migration from https://trac.sagemath.org/ticket/32374_





---

archive/issue_events_428260.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-13T02:13:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32374",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32374#event-428260"
}
```



---

archive/issue_events_428261.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-13T02:13:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32374",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
    "label_color": "000080",
    "label_name": "component: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32374#event-428261"
}
```



---

archive/issue_events_428262.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-13T02:13:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32374",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32374#event-428262"
}
```



---

archive/issue_events_428263.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-13T02:13:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32374",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32374#event-428263"
}
```



---

archive/issue_comments_524772.json:
```json
{
    "body": "Branch: **[u/mkoeppe/remove_pip_package__pyopenssl_](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/remove_pip_package__pyopenssl_)**",
    "created_at": "2021-08-13T03:34:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32374#issuecomment-524772",
    "user": "https://github.com/mkoeppe"
}
```

Branch: **[u/mkoeppe/remove_pip_package__pyopenssl_](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/remove_pip_package__pyopenssl_)**



---

archive/issue_events_428264.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-13T03:34:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32374",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32374#event-428264"
}
```



---

archive/issue_comments_524773.json:
```json
{
    "body": "Commit: **[a9b2bd6](https://github.com/sagemath/sagetrac-mirror/commit/a9b2bd6c52f0e49bd877829c2b7922510bc3e153)**",
    "created_at": "2021-08-13T03:34:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32374#issuecomment-524773",
    "user": "https://github.com/mkoeppe"
}
```

Commit: **[a9b2bd6](https://github.com/sagemath/sagetrac-mirror/commit/a9b2bd6c52f0e49bd877829c2b7922510bc3e153)**



---

archive/issue_comments_524774.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a9b2bd6c52f0e49bd877829c2b7922510bc3e153\">a9b2bd6</a></td><td><code>build/pkgs/pyopenssl: Remove</code></td></tr></table>\n",
    "created_at": "2021-08-13T03:34:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32374#issuecomment-524774",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a9b2bd6c52f0e49bd877829c2b7922510bc3e153">a9b2bd6</a></td><td><code>build/pkgs/pyopenssl: Remove</code></td></tr></table>




---

archive/issue_comments_524775.json:
```json
{
    "body": "Author: **Matthias Koeppe**",
    "created_at": "2021-08-13T04:05:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32374#issuecomment-524775",
    "user": "https://github.com/mkoeppe"
}
```

Author: **Matthias Koeppe**



---

archive/issue_comments_524776.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nLGTM.",
    "created_at": "2021-09-23T16:18:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32374#issuecomment-524776",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

<div id="comment:6" align="right">Comment 6</div>

LGTM.



---

archive/issue_comments_524777.json:
```json
{
    "body": "Reviewer: **Thierry Monteil**",
    "created_at": "2021-09-23T16:18:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32374#issuecomment-524777",
    "user": "https://github.com/sagetrac-tmonteil"
}
```

Reviewer: **Thierry Monteil**



---

archive/issue_events_428265.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2021-09-23T16:18:07Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32374",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32374#event-428265"
}
```



---

archive/issue_events_428266.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2021-09-23T16:18:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32374",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32374#event-428266"
}
```



---

archive/issue_comments_524778.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nThanks!",
    "created_at": "2021-09-23T16:31:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32374#issuecomment-524778",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:7" align="right">Comment 7</div>

Thanks!



---

archive/issue_events_428267.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-10-09T11:09:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32374",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32374#event-428267"
}
```



---

archive/issue_events_428268.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "658012052fa810fe73cabb548ccfd33020f22456",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-10-09T11:09:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32374",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32374#event-428268"
}
```



---

archive/issue_comments_524779.json:
```json
{
    "body": "Changed branch from **[u/mkoeppe/remove_pip_package__pyopenssl_](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/remove_pip_package__pyopenssl_)** to **[a9b2bd6](https://github.com/sagemath/sagetrac-mirror/commit/a9b2bd6c52f0e49bd877829c2b7922510bc3e153)**",
    "created_at": "2021-10-09T11:09:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32374#issuecomment-524779",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mkoeppe/remove_pip_package__pyopenssl_](https://github.com/sagemath/sagetrac-mirror/tree/u/mkoeppe/remove_pip_package__pyopenssl_)** to **[a9b2bd6](https://github.com/sagemath/sagetrac-mirror/commit/a9b2bd6c52f0e49bd877829c2b7922510bc3e153)**
