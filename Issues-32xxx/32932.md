# Issue 32932: Fix 4ti2 distro info: cygwin

archive/issues_032695.json:
```json
{
    "assignees": [],
    "body": "The 4ti2 cygwin distro file only has the -devel\npackage but the non-devel one is needed too.\n\nWe fix that.\n\nCC:  @alexjbest @dimpase @embray @jengelh @kliem @mkoeppe @slel\n\nKeywords: **4ti2, distro, cygwin**\n\nBranch/Commit: **[82f27df](https://github.com/sagemath/sagetrac-mirror/commit/82f27dfd8450b20b7ab79ebdbfdd2b6efc82e0f0)**\n\nReviewer: **Dima Pasechnik**\n\nAuthor: **Samuel Leli\u00e8vre**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32932_\n\n",
    "closed_at": "2021-12-23T20:25:52Z",
    "created_at": "2021-11-25T18:24:57Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build%3A%20configure",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix 4ti2 distro info: cygwin",
    "type": "issue",
    "updated_at": "2021-12-23T20:25:52Z",
    "url": "https://github.com/sagemath/sage/issues/32932",
    "user": "https://github.com/slel"
}
```
The 4ti2 cygwin distro file only has the -devel
package but the non-devel one is needed too.

We fix that.

CC:  @alexjbest @dimpase @embray @jengelh @kliem @mkoeppe @slel

Keywords: **4ti2, distro, cygwin**

Branch/Commit: **[82f27df](https://github.com/sagemath/sagetrac-mirror/commit/82f27dfd8450b20b7ab79ebdbfdd2b6efc82e0f0)**

Reviewer: **Dima Pasechnik**

Author: **Samuel Lelièvre**

_Issue created by migration from https://trac.sagemath.org/ticket/32932_





---

archive/issue_events_434995.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-11-25T18:24:57Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32932",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32932#event-434995"
}
```



---

archive/issue_events_434996.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-11-25T18:24:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32932",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build%3A%20configure",
    "label_color": "000080",
    "label_name": "component: build: configure",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32932#event-434996"
}
```



---

archive/issue_events_434997.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-11-25T18:24:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32932",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32932#event-434997"
}
```



---

archive/issue_events_434998.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-11-25T18:24:57Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32932",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32932#event-434998"
}
```



---

archive/issue_comments_534949.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nAn oversight in #30619 and #30887.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a2f0bcff3b312723212b3eb6543329871faebc0d\">a2f0bcf</a></td><td><code>32932: Fix 4ti2 distro info</code></td></tr></table>\n",
    "created_at": "2021-11-25T18:27:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32932#issuecomment-534949",
    "user": "https://github.com/slel"
}
```

<div id="comment:1" align="right">Comment 1</div>

An oversight in #30619 and #30887.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a2f0bcff3b312723212b3eb6543329871faebc0d">a2f0bcf</a></td><td><code>32932: Fix 4ti2 distro info</code></td></tr></table>




---

archive/issue_comments_534950.json:
```json
{
    "body": "Branch: **[public/32932](https://github.com/sagemath/sagetrac-mirror/tree/public/32932)**",
    "created_at": "2021-11-25T18:27:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32932#issuecomment-534950",
    "user": "https://github.com/slel"
}
```

Branch: **[public/32932](https://github.com/sagemath/sagetrac-mirror/tree/public/32932)**



---

archive/issue_comments_534951.json:
```json
{
    "body": "Author: **Samuel Leli\u00e8vre**",
    "created_at": "2021-11-25T18:27:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32932#issuecomment-534951",
    "user": "https://github.com/slel"
}
```

Author: **Samuel Lelièvre**



---

archive/issue_events_434999.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-11-25T18:27:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32932",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32932#event-434999"
}
```



---

archive/issue_comments_534952.json:
```json
{
    "body": "Commit: **[a2f0bcf](https://github.com/sagemath/sagetrac-mirror/commit/a2f0bcff3b312723212b3eb6543329871faebc0d)**",
    "created_at": "2021-11-25T18:27:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32932#issuecomment-534952",
    "user": "https://github.com/slel"
}
```

Commit: **[a2f0bcf](https://github.com/sagemath/sagetrac-mirror/commit/a2f0bcff3b312723212b3eb6543329871faebc0d)**



---

archive/issue_comments_534953.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nlatte needs 4ti2 headers",
    "created_at": "2021-11-25T19:40:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32932#issuecomment-534953",
    "user": "https://github.com/jengelh"
}
```

<div id="comment:3" align="right">Comment 3</div>

latte needs 4ti2 headers



---

archive/issue_events_435000.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-11-25T20:13:31Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/32932",
    "title_is": "Fix 4ti2 distro info: cygwin",
    "title_was": "Fix 4ti2 distro info: cygwin, opensuse",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32932#event-435000"
}
```



---

archive/issue_comments_534954.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-The 4ti2 cygwin and opensuse distro files\n-incorrectly refer to the -devel version.\n+The 4ti2 cygwin distro file only has the -devel\n+package but the non-devel one is needed too.\n \n We fix that.\n``````\n",
    "created_at": "2021-11-25T20:13:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32932#issuecomment-534954",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-The 4ti2 cygwin and opensuse distro files
-incorrectly refer to the -devel version.
+The 4ti2 cygwin distro file only has the -devel
+package but the non-devel one is needed too.
 
 We fix that.
``````




---

archive/issue_comments_534955.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nOh, so let me change the ticket summary and description.",
    "created_at": "2021-11-25T20:13:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32932#issuecomment-534955",
    "user": "https://github.com/slel"
}
```

<div id="comment:4" align="right">Comment 4</div>

Oh, so let me change the ticket summary and description.



---

archive/issue_events_435001.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-11-25T20:13:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32932",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32932#event-435001"
}
```



---

archive/issue_events_435002.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-11-25T20:13:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32932",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32932#event-435002"
}
```



---

archive/issue_events_435003.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-11-25T20:14:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32932",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32932#event-435003"
}
```



---

archive/issue_events_435004.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2021-11-25T20:14:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32932",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32932#event-435004"
}
```



---

archive/issue_comments_534956.json:
```json
{
    "body": "Changed commit from **[a2f0bcf](https://github.com/sagemath/sagetrac-mirror/commit/a2f0bcff3b312723212b3eb6543329871faebc0d)** to **[82f27df](https://github.com/sagemath/sagetrac-mirror/commit/82f27dfd8450b20b7ab79ebdbfdd2b6efc82e0f0)**",
    "created_at": "2021-11-25T20:14:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32932#issuecomment-534956",
    "user": "https://github.com/slel"
}
```

Changed commit from **[a2f0bcf](https://github.com/sagemath/sagetrac-mirror/commit/a2f0bcff3b312723212b3eb6543329871faebc0d)** to **[82f27df](https://github.com/sagemath/sagetrac-mirror/commit/82f27dfd8450b20b7ab79ebdbfdd2b6efc82e0f0)**



---

archive/issue_comments_534957.json:
```json
{
    "body": "Changed keywords from **4ti2, distro, cygwin, opensuse** to **4ti2, distro, cygwin**",
    "created_at": "2021-11-25T20:14:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32932#issuecomment-534957",
    "user": "https://github.com/slel"
}
```

Changed keywords from **4ti2, distro, cygwin, opensuse** to **4ti2, distro, cygwin**



---

archive/issue_comments_534958.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nHere is a branch only fixing the Cygwin distro file.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/82f27dfd8450b20b7ab79ebdbfdd2b6efc82e0f0\">82f27df</a></td><td><code>32932: Fix 4ti2 cygwin distro info</code></td></tr></table>\n",
    "created_at": "2021-11-25T20:14:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32932#issuecomment-534958",
    "user": "https://github.com/slel"
}
```

<div id="comment:5" align="right">Comment 5</div>

Here is a branch only fixing the Cygwin distro file.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/82f27dfd8450b20b7ab79ebdbfdd2b6efc82e0f0">82f27df</a></td><td><code>32932: Fix 4ti2 cygwin distro info</code></td></tr></table>




---

archive/issue_comments_534959.json:
```json
{
    "body": "Changed branch from **[public/32932](https://github.com/sagemath/sagetrac-mirror/tree/public/32932)** to **[u/slelievre/32932](https://github.com/sagemath/sagetrac-mirror/tree/u/slelievre/32932)**",
    "created_at": "2021-11-25T20:14:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32932#issuecomment-534959",
    "user": "https://github.com/slel"
}
```

Changed branch from **[public/32932](https://github.com/sagemath/sagetrac-mirror/tree/public/32932)** to **[u/slelievre/32932](https://github.com/sagemath/sagetrac-mirror/tree/u/slelievre/32932)**



---

archive/issue_events_435005.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-11-27T19:09:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32932",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32932#event-435005"
}
```



---

archive/issue_events_435006.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2021-11-27T19:09:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32932",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32932#event-435006"
}
```



---

archive/issue_comments_534960.json:
```json
{
    "body": "Reviewer: **Dima Pasechnik**",
    "created_at": "2021-11-27T19:09:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32932#issuecomment-534960",
    "user": "https://github.com/dimpase"
}
```

Reviewer: **Dima Pasechnik**



---

archive/issue_comments_534961.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nOK, I assume you put in the correct package name",
    "created_at": "2021-11-27T19:09:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32932#issuecomment-534961",
    "user": "https://github.com/dimpase"
}
```

<div id="comment:7" align="right">Comment 7</div>

OK, I assume you put in the correct package name



---

archive/issue_comments_534962.json:
```json
{
    "body": "Changed branch from **[u/slelievre/32932](https://github.com/sagemath/sagetrac-mirror/tree/u/slelievre/32932)** to **[82f27df](https://github.com/sagemath/sagetrac-mirror/commit/82f27dfd8450b20b7ab79ebdbfdd2b6efc82e0f0)**",
    "created_at": "2021-12-23T20:25:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32932",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32932#issuecomment-534962",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/slelievre/32932](https://github.com/sagemath/sagetrac-mirror/tree/u/slelievre/32932)** to **[82f27df](https://github.com/sagemath/sagetrac-mirror/commit/82f27dfd8450b20b7ab79ebdbfdd2b6efc82e0f0)**



---

archive/issue_events_435007.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-12-23T20:25:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32932",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32932#event-435007"
}
```



---

archive/issue_events_435008.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "bd8d812e98e8757de7b43b784d5c94e039c446b8",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-12-23T20:25:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32932",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32932#event-435008"
}
```
