# Issue 32355: Bug in multiple tensor contractions with scalar result

archive/issues_032118.json:
```json
{
    "body": "Tensor contractions on multiple indices give wrong results when the outcome is a scalar and the index positions in each tensor do not have the same order. For instance, let us consider\n\n```\nsage: M = Manifold(2, 'M')                                                                          \nsage: X.<x,y> = M.chart()                                                                           \nsage: a = M.multivector_field(2)                                                                    \nsage: a[0,1] = 1                                                                                    \nsage: b = M.diff_form(2)                                                                            \nsage: b[0,1] = 1                                                                                    \n```\nThe double contraction a<sup>ij</sup> b<sub>ij</sub> is obtained via\n\n```\nsage: a.contract(0, 1, b, 0, 1).expr()                                                              \n2\nsage: (a['^ij']*b['_ij']).expr()  # equivalent to above                                                                 \n2\n```\nwhich is correct. However, asking for the double contraction a<sup>ij</sup> b<sub>ji</sub> \nleads to \n\n```\nsage: a.contract(0, 1, b, 1, 0).expr()  \n2\nsage: (a['^ij']*b['_ji']).expr()  # equivalent to above                                                                     \n2\n```                                                          \nwhich is obviously wrong: the result should be -2, since b is antisymmetric.\n\nIf the outcome is not a scalar field, there is no issue:\n\n```\nsage: c = M.tensor_field(3, 0, antisym=(0,1))                                                       \nsage: c[0,1,0] = 1                                                                                  \nsage: c.contract(0, 1, b, 0, 1).display()  # correct result                                                        \n2 \u2202/\u2202x\nsage: c.contract(0, 1, b, 1, 0).display()  # correct result                                                           \n-2 \u2202/\u2202x\n```\nThis bug has been reported in https://ask.sagemath.org/question/58379.\n\nCC:  @tscrim @mjungmath @mkoeppe\n\nKeywords: tensor contraction\n\nBranch/Commit: 48f5523074fbbb835fa41be1d567d244e9244ae4\n\nReviewer: Michael Jung\n\nAuthor: Eric Gourgoulhon\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/32355\n\n",
    "closed_at": "2021-09-07T20:52:15Z",
    "created_at": "2021-08-10T08:40:41Z",
    "labels": [
        "component: manifolds",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.5",
    "title": "Bug in multiple tensor contractions with scalar result",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/32355",
    "user": "https://github.com/egourgoulhon"
}
```
Tensor contractions on multiple indices give wrong results when the outcome is a scalar and the index positions in each tensor do not have the same order. For instance, let us consider

```
sage: M = Manifold(2, 'M')                                                                          
sage: X.<x,y> = M.chart()                                                                           
sage: a = M.multivector_field(2)                                                                    
sage: a[0,1] = 1                                                                                    
sage: b = M.diff_form(2)                                                                            
sage: b[0,1] = 1                                                                                    
```
The double contraction a<sup>ij</sup> b<sub>ij</sub> is obtained via

```
sage: a.contract(0, 1, b, 0, 1).expr()                                                              
2
sage: (a['^ij']*b['_ij']).expr()  # equivalent to above                                                                 
2
```
which is correct. However, asking for the double contraction a<sup>ij</sup> b<sub>ji</sub> 
leads to 

```
sage: a.contract(0, 1, b, 1, 0).expr()  
2
sage: (a['^ij']*b['_ji']).expr()  # equivalent to above                                                                     
2
```                                                          
which is obviously wrong: the result should be -2, since b is antisymmetric.

If the outcome is not a scalar field, there is no issue:

```
sage: c = M.tensor_field(3, 0, antisym=(0,1))                                                       
sage: c[0,1,0] = 1                                                                                  
sage: c.contract(0, 1, b, 0, 1).display()  # correct result                                                        
2 ∂/∂x
sage: c.contract(0, 1, b, 1, 0).display()  # correct result                                                           
-2 ∂/∂x
```
This bug has been reported in https://ask.sagemath.org/question/58379.

CC:  @tscrim @mjungmath @mkoeppe

Keywords: tensor contraction

Branch/Commit: 48f5523074fbbb835fa41be1d567d244e9244ae4

Reviewer: Michael Jung

Author: Eric Gourgoulhon

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/32355





---

archive/issue_comments_644170.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-Tensor contractions on multiple indices give wrong results when the index positions in each tensor do not have the same order. For instance, let us consider\n+Tensor contractions on multiple indices give wrong results when the outcome is a scalar and the index positions in each tensor do not have the same order. For instance, let us consider\n \n ```\n sage: M = Manifold(2, 'M')                                                                          \n@@ -27,4 +27,14 @@\n ```                                                          \n which is obviously wrong: the result should be -2, since b is antisymmetric.\n \n+If the outcome is not a scalar field, there is no issue:\n+\n+```\n+sage: c = M.tensor_field(3, 0, antisym=(0,1))                                                       \n+sage: c[0,1,0] = 1                                                                                  \n+sage: c.contract(0, 1, b, 0, 1).display()  # correct result                                                        \n+2 \u2202/\u2202x\n+sage: c.contract(0, 1, b, 1, 0).display()  # correct result                                                           \n+-2 \u2202/\u2202x\n+```\n This bug has been reported in https://ask.sagemath.org/question/58379.\n``````\n",
    "created_at": "2021-08-10T08:59:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32355#issuecomment-644170",
    "user": "https://github.com/egourgoulhon"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-Tensor contractions on multiple indices give wrong results when the index positions in each tensor do not have the same order. For instance, let us consider
+Tensor contractions on multiple indices give wrong results when the outcome is a scalar and the index positions in each tensor do not have the same order. For instance, let us consider
 
 ```
 sage: M = Manifold(2, 'M')                                                                          
@@ -27,4 +27,14 @@
 ```                                                          
 which is obviously wrong: the result should be -2, since b is antisymmetric.
 
+If the outcome is not a scalar field, there is no issue:
+
+```
+sage: c = M.tensor_field(3, 0, antisym=(0,1))                                                       
+sage: c[0,1,0] = 1                                                                                  
+sage: c.contract(0, 1, b, 0, 1).display()  # correct result                                                        
+2 ∂/∂x
+sage: c.contract(0, 1, b, 1, 0).display()  # correct result                                                           
+-2 ∂/∂x
+```
 This bug has been reported in https://ask.sagemath.org/question/58379.
``````




---

archive/issue_comments_644171.json:
```json
{
    "body": "<a id='comment:2'></a>\nThe culprit is line 2187 of `src/sage/tensor/modules/comp.py`:\n\n```\n    for ind in comp_for_contr.index_generator():\n        res += self[[ind]] * other[[ind]]\n```\n`ind` should be reordered before being passed to `other[This is the Trac macro *...* that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#...-macro)`.",
    "created_at": "2021-08-10T09:09:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32355#issuecomment-644171",
    "user": "https://github.com/egourgoulhon"
}
```

<a id='comment:2'></a>
The culprit is line 2187 of `src/sage/tensor/modules/comp.py`:

```
    for ind in comp_for_contr.index_generator():
        res += self[[ind]] * other[[ind]]
```
`ind` should be reordered before being passed to `other[This is the Trac macro *...* that was inherited from the migration](https://trac.sagemath.org/wiki/WikiMacros#...-macro)`.



---

archive/issue_comments_644172.json:
```json
{
    "body": "<a id='comment:3'></a>\nThe bug is fixed in the above branch. Some slight clean up of the code for parallelized contractions has also been performed.\n\n---\nNew commits:\n|                                                                                                                                          |                                                |\n|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------|\n|[26ba8f7](https://git.sagemath.org/sage.git/commit?id=26ba8f76d35623c499ce7c70a0b6a2afbb724929)|`Fix bug in Components.contract() (Trac #32355)`|",
    "created_at": "2021-08-12T14:00:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32355#issuecomment-644172",
    "user": "https://github.com/egourgoulhon"
}
```

<a id='comment:3'></a>
The bug is fixed in the above branch. Some slight clean up of the code for parallelized contractions has also been performed.

---
New commits:
|                                                                                                                                          |                                                |
|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------|
|[26ba8f7](https://git.sagemath.org/sage.git/commit?id=26ba8f76d35623c499ce7c70a0b6a2afbb724929)|`Fix bug in Components.contract() (Trac #32355)`|



---

archive/issue_comments_644173.json:
```json
{
    "body": "Changing branch from \"\" to \"public/manifolds/contraction_bug-32355\"",
    "created_at": "2021-08-12T14:00:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32355#issuecomment-644173",
    "user": "https://github.com/egourgoulhon"
}
```

Changing branch from "" to "public/manifolds/contraction_bug-32355"



---

archive/issue_comments_644174.json:
```json
{
    "body": "Changing commit from \"\" to \"26ba8f76d35623c499ce7c70a0b6a2afbb724929\"",
    "created_at": "2021-08-12T14:00:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32355#issuecomment-644174",
    "user": "https://github.com/egourgoulhon"
}
```

Changing commit from "" to "26ba8f76d35623c499ce7c70a0b6a2afbb724929"



---

archive/issue_comments_644175.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2021-08-12T14:00:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32355#issuecomment-644175",
    "user": "https://github.com/egourgoulhon"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_644176.json:
```json
{
    "body": "Changing author from \"\" to \"Eric Gourgoulhon\"",
    "created_at": "2021-08-12T14:02:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32355#issuecomment-644176",
    "user": "https://github.com/egourgoulhon"
}
```

Changing author from "" to "Eric Gourgoulhon"



---

archive/issue_comments_644177.json:
```json
{
    "body": "<a id='comment:5'></a>\nHere are some comments.\n\n- I think you don't need to initialize a new component in\n\n   {{{#!python\n             comp_for_contr = Components(self._ring, self._frame, ncontr,\n                                         start_index=self._sindex)\n   }}}\n\n   and you can use `self.index_generator()` right away, no?\n\n- What about this?\n\n   {{{#!diff\n-            for ind_s in comp_for_contr.index_generator():\n-                ind_o = [None for i in range(ncontr)]\n-                for pos_s, pos_o in contractions:\n-                    ind_o[pos_o] = ind_s[pos_s]\n-                ind_pairs.append((ind_s, ind_o))\n+            for ind_s in self.index_generator():\n+                ind_o = [ind_s[pos_s] for pos_s, _ in sorted(contractions, key=itemgetter(1))]\n+                ind_pairs.append((ind_s, ind_o))\n   }}}\n\n   You could also turn the outer for-loop into a list comprehension which would be faster but I reckon less well readable.",
    "created_at": "2021-08-12T18:06:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32355#issuecomment-644177",
    "user": "https://github.com/mjungmath"
}
```

<a id='comment:5'></a>
Here are some comments.

- I think you don't need to initialize a new component in

   {{{#!python
             comp_for_contr = Components(self._ring, self._frame, ncontr,
                                         start_index=self._sindex)
   }}}

   and you can use `self.index_generator()` right away, no?

- What about this?

   {{{#!diff
-            for ind_s in comp_for_contr.index_generator():
-                ind_o = [None for i in range(ncontr)]
-                for pos_s, pos_o in contractions:
-                    ind_o[pos_o] = ind_s[pos_s]
-                ind_pairs.append((ind_s, ind_o))
+            for ind_s in self.index_generator():
+                ind_o = [ind_s[pos_s] for pos_s, _ in sorted(contractions, key=itemgetter(1))]
+                ind_pairs.append((ind_s, ind_o))
   }}}

   You could also turn the outer for-loop into a list comprehension which would be faster but I reckon less well readable.



---

archive/issue_comments_644178.json:
```json
{
    "body": "Changing commit from \"26ba8f76d35623c499ce7c70a0b6a2afbb724929\" to \"3e5ac7d48bc15509694e0b904b8595b38caa4e8a\"",
    "created_at": "2021-08-13T12:39:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32355#issuecomment-644178",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "26ba8f76d35623c499ce7c70a0b6a2afbb724929" to "3e5ac7d48bc15509694e0b904b8595b38caa4e8a"



---

archive/issue_comments_644179.json:
```json
{
    "body": "<a id='comment:6'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                   |\n|-------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------|\n|[3e5ac7d](https://git.sagemath.org/sage.git/commit/?id=3e5ac7d48bc15509694e0b904b8595b38caa4e8a)|`Improvement in the fix for #32355`|",
    "created_at": "2021-08-13T12:39:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32355#issuecomment-644179",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:6'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                   |
|-------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------|
|[3e5ac7d](https://git.sagemath.org/sage.git/commit/?id=3e5ac7d48bc15509694e0b904b8595b38caa4e8a)|`Improvement in the fix for #32355`|



---

archive/issue_comments_644180.json:
```json
{
    "body": "<a id='comment:7'></a>\nReplying to [gh-mjungmath](#comment%3A5):\n> Here are some comments.\n\n\nThanks for taking a look into this.\n\n> \n> - I think you don't need to initialize a new component in\n> \n>    {{{#!python\n>              comp_for_contr = Components(self._ring, self._frame, ncontr,\n>                                          start_index=self._sindex)\n>    }}}\n> \n>    and you can use `self.index_generator()` right away, no?\n> \n\n\nYes indeed! (this is needed for the generic case, but not for the specific case of a scalar output). The latest commit implement this change.\n\n> - What about this?\n> \n>    {{{#!diff\n> -            for ind_s in comp_for_contr.index_generator():\n> -                ind_o = [None for i in range(ncontr)]\n> -                for pos_s, pos_o in contractions:\n> -                    ind_o[pos_o] = ind_s[pos_s]\n> -                ind_pairs.append((ind_s, ind_o))\n> +            for ind_s in self.index_generator():\n> +                ind_o = [ind_s[pos_s] for pos_s, _ in sorted(contractions, key=itemgetter(1))]\n> +                ind_pairs.append((ind_s, ind_o))\n>    }}}\n> \n\nI would prefer to keep the original version, since it is more efficient from a computational point of view: there is no need to sort a list. Granted: in most use cases, the list contains only a few elements and sorting shall be fast. So it is more a matter of taste...",
    "created_at": "2021-08-13T12:48:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32355#issuecomment-644180",
    "user": "https://github.com/egourgoulhon"
}
```

<a id='comment:7'></a>
Replying to [gh-mjungmath](#comment%3A5):
> Here are some comments.


Thanks for taking a look into this.

> 
> - I think you don't need to initialize a new component in
> 
>    {{{#!python
>              comp_for_contr = Components(self._ring, self._frame, ncontr,
>                                          start_index=self._sindex)
>    }}}
> 
>    and you can use `self.index_generator()` right away, no?
> 


Yes indeed! (this is needed for the generic case, but not for the specific case of a scalar output). The latest commit implement this change.

> - What about this?
> 
>    {{{#!diff
> -            for ind_s in comp_for_contr.index_generator():
> -                ind_o = [None for i in range(ncontr)]
> -                for pos_s, pos_o in contractions:
> -                    ind_o[pos_o] = ind_s[pos_s]
> -                ind_pairs.append((ind_s, ind_o))
> +            for ind_s in self.index_generator():
> +                ind_o = [ind_s[pos_s] for pos_s, _ in sorted(contractions, key=itemgetter(1))]
> +                ind_pairs.append((ind_s, ind_o))
>    }}}
> 

I would prefer to keep the original version, since it is more efficient from a computational point of view: there is no need to sort a list. Granted: in most use cases, the list contains only a few elements and sorting shall be fast. So it is more a matter of taste...



---

archive/issue_comments_644181.json:
```json
{
    "body": "<a id='comment:8'></a>\nReplying to [egourgoulhon](#comment%3A7):\n> I would prefer to keep the original version, since it is more efficient from a computational point of view: there is no need to sort a list. Granted: in most use cases, the list contains only a few elements and sorting shall be fast. So it is more a matter of taste...\n\n\nYou're right, this is faster! Then I still have one last comment:\n\n```diff\n-                ind_o = [None for i in range(ncontr)]\n+                ind_o = [None] * ncontr\n```\n\nThis is indeed faster.\n\nIf that is done, and patchbot is happy -> positive review.",
    "created_at": "2021-08-13T13:20:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32355#issuecomment-644181",
    "user": "https://github.com/mjungmath"
}
```

<a id='comment:8'></a>
Replying to [egourgoulhon](#comment%3A7):
> I would prefer to keep the original version, since it is more efficient from a computational point of view: there is no need to sort a list. Granted: in most use cases, the list contains only a few elements and sorting shall be fast. So it is more a matter of taste...


You're right, this is faster! Then I still have one last comment:

```diff
-                ind_o = [None for i in range(ncontr)]
+                ind_o = [None] * ncontr
```

This is indeed faster.

If that is done, and patchbot is happy -> positive review.



---

archive/issue_comments_644182.json:
```json
{
    "body": "<a id='comment:9'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                               |\n|-------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------|\n|[48f5523](https://git.sagemath.org/sage.git/commit/?id=48f5523074fbbb835fa41be1d567d244e9244ae4)|`Yet another improvement in the fix for #32355`|",
    "created_at": "2021-08-14T08:38:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32355#issuecomment-644182",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:9'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                               |
|-------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------|
|[48f5523](https://git.sagemath.org/sage.git/commit/?id=48f5523074fbbb835fa41be1d567d244e9244ae4)|`Yet another improvement in the fix for #32355`|



---

archive/issue_comments_644183.json:
```json
{
    "body": "Changing commit from \"3e5ac7d48bc15509694e0b904b8595b38caa4e8a\" to \"48f5523074fbbb835fa41be1d567d244e9244ae4\"",
    "created_at": "2021-08-14T08:38:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32355#issuecomment-644183",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "3e5ac7d48bc15509694e0b904b8595b38caa4e8a" to "48f5523074fbbb835fa41be1d567d244e9244ae4"



---

archive/issue_comments_644184.json:
```json
{
    "body": "<a id='comment:10'></a>\nReplying to [gh-mjungmath](#comment%3A8):\nThen I still have one last comment:\n> \n> \n> ```diff\n> -                ind_o = [None for i in range(ncontr)]\n> +                ind_o = [None] * ncontr\n> ```\n> \n\nDone in the above commit.",
    "created_at": "2021-08-14T08:40:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32355#issuecomment-644184",
    "user": "https://github.com/egourgoulhon"
}
```

<a id='comment:10'></a>
Replying to [gh-mjungmath](#comment%3A8):
Then I still have one last comment:
> 
> 
> ```diff
> -                ind_o = [None for i in range(ncontr)]
> +                ind_o = [None] * ncontr
> ```
> 

Done in the above commit.



---

archive/issue_comments_644185.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Michael Jung\"",
    "created_at": "2021-08-15T21:51:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32355#issuecomment-644185",
    "user": "https://github.com/mjungmath"
}
```

Changing reviewer from "" to "Michael Jung"



---

archive/issue_comments_644186.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2021-08-15T21:51:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32355#issuecomment-644186",
    "user": "https://github.com/mjungmath"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_644187.json:
```json
{
    "body": "<a id='comment:12'></a>\nThank you for the review!",
    "created_at": "2021-08-16T14:45:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32355#issuecomment-644187",
    "user": "https://github.com/egourgoulhon"
}
```

<a id='comment:12'></a>
Thank you for the review!



---

archive/issue_events_085853.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-22T20:35:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/32355",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/32355#event-85853"
}
```



---

archive/issue_comments_644188.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2021-09-07T20:52:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32355#issuecomment-644188",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_events_085854.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-09-07T20:52:15Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/32355",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/32355#event-85854"
}
```



---

archive/issue_comments_644189.json:
```json
{
    "body": "Changing branch from \"public/manifolds/contraction_bug-32355\" to \"48f5523074fbbb835fa41be1d567d244e9244ae4\"",
    "created_at": "2021-09-07T20:52:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/32355",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/32355#issuecomment-644189",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "public/manifolds/contraction_bug-32355" to "48f5523074fbbb835fa41be1d567d244e9244ae4"
