# Issue 32439: sage.misc.prandom misses several functions added in Python 3

archive/issues_032202.json:
```json
{
    "assignees": [],
    "body": "According to https://docs.python.org/3/library/random.html, we are missing the following random generators\n\n- [choices](https://docs.python.org/3/library/random.html#random.choices) (added in Python 3.6)\n- [randbytes](https://docs.python.org/3/library/random.html#random.randbytes) (added in Python 3.9)\n- [triangular](https://docs.python.org/3/library/random.html#random.triangular)\n\nAs well as the classes to get generators without shared states\n\n- [Random class](https://docs.python.org/3/library/random.html#random.Random)\n- [SystemRandom class](https://docs.python.org/3/library/random.html#random.Random)\n\nAnd the getter/stter\n\n- [seed](https://docs.python.org/3/library/random.html#random.seed)\n- [getstate](https://docs.python.org/3/library/random.html#random.getstate)\n- [setstate](https://docs.python.org/3/library/random.html#random.setstate)\n\n\nCC:  @videlec\n\nBranch/Commit: **[u/gh-Sandstorm831/32439](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-Sandstorm831/32439) @ [1a42056](https://github.com/sagemath/sagetrac-mirror/commit/1a42056f8ad687f595a3162cf151c4b78db9641e)**\n\nComponent: **misc**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32439_\n\n",
    "created_at": "2021-08-31T00:05:41Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "sage.misc.prandom misses several functions added in Python 3",
    "type": "issue",
    "updated_at": "2023-01-09T16:48:24Z",
    "url": "https://github.com/sagemath/sage/issues/32439",
    "user": "https://github.com/mkoeppe"
}
```
According to https://docs.python.org/3/library/random.html, we are missing the following random generators

- [choices](https://docs.python.org/3/library/random.html#random.choices) (added in Python 3.6)
- [randbytes](https://docs.python.org/3/library/random.html#random.randbytes) (added in Python 3.9)
- [triangular](https://docs.python.org/3/library/random.html#random.triangular)

As well as the classes to get generators without shared states

- [Random class](https://docs.python.org/3/library/random.html#random.Random)
- [SystemRandom class](https://docs.python.org/3/library/random.html#random.Random)

And the getter/stter

- [seed](https://docs.python.org/3/library/random.html#random.seed)
- [getstate](https://docs.python.org/3/library/random.html#random.getstate)
- [setstate](https://docs.python.org/3/library/random.html#random.setstate)


CC:  @videlec

Branch/Commit: **[u/gh-Sandstorm831/32439](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-Sandstorm831/32439) @ [1a42056](https://github.com/sagemath/sagetrac-mirror/commit/1a42056f8ad687f595a3162cf151c4b78db9641e)**

Component: **misc**

_Issue created by migration from https://trac.sagemath.org/ticket/32439_





---

archive/issue_events_447395.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-31T00:05:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32439",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32439#event-447395"
}
```



---

archive/issue_events_447396.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-31T00:05:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32439",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32439#event-447396"
}
```



---

archive/issue_events_447397.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-31T00:05:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32439",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32439#event-447397"
}
```



---

archive/issue_events_447398.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/32439",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32439#event-447398"
}
```



---

archive/issue_events_447399.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32439",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32439#event-447399"
}
```



---

archive/issue_comments_523502.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\n\n```\nsage: import random\nsage: import sage.misc.prandom\nsage: sorted(name for name in set(dir(random)).difference(dir(sage.misc.prandom)) if not name.startswith('_') and callable(getattr(random, name)))\n['Random',\n 'SystemRandom',\n 'choices',\n 'getstate',\n 'randbytes',\n 'seed',\n 'setstate',\n 'triangular']\n```",
    "created_at": "2022-03-05T12:59:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32439#issuecomment-523502",
    "user": "https://github.com/videlec"
}
```

<div id="comment:3" align="right">comment:3</div>


```
sage: import random
sage: import sage.misc.prandom
sage: sorted(name for name in set(dir(random)).difference(dir(sage.misc.prandom)) if not name.startswith('_') and callable(getattr(random, name)))
['Random',
 'SystemRandom',
 'choices',
 'getstate',
 'randbytes',
 'seed',
 'setstate',
 'triangular']
```



---

archive/issue_comments_523503.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,17 @@\n-https://docs.python.org/3/library/random.html\n+According to https://docs.python.org/3/library/random.html, we are missing the following random generators\n \n-For example, there is `random.triangular` but no `sage.misc.prandom.triangular`.\n+- [choices](https://docs.python.org/3/library/random.html#random.choices) (added in Python 3.6)\n+- [randbytes](https://docs.python.org/3/library/random.html#random.randbytes) (added in Python 3.9)\n+- [triangular](https://docs.python.org/3/library/random.html#random.triangular)\n+\n+As well as the classes to get generators without shared states\n+\n+- [Random class](https://docs.python.org/3/library/random.html#random.Random)\n+- [SystemRandom class](https://docs.python.org/3/library/random.html#random.Random)\n+\n+And the getter/stter\n+\n+- [seed](https://docs.python.org/3/library/random.html#random.seed)\n+- [getstate](https://docs.python.org/3/library/random.html#random.getstate)\n+- [setstate](https://docs.python.org/3/library/random.html#random.setstate)\n+\n``````\n",
    "created_at": "2022-03-05T13:04:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32439#issuecomment-523503",
    "user": "https://github.com/videlec"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,17 @@
-https://docs.python.org/3/library/random.html
+According to https://docs.python.org/3/library/random.html, we are missing the following random generators
 
-For example, there is `random.triangular` but no `sage.misc.prandom.triangular`.
+- [choices](https://docs.python.org/3/library/random.html#random.choices) (added in Python 3.6)
+- [randbytes](https://docs.python.org/3/library/random.html#random.randbytes) (added in Python 3.9)
+- [triangular](https://docs.python.org/3/library/random.html#random.triangular)
+
+As well as the classes to get generators without shared states
+
+- [Random class](https://docs.python.org/3/library/random.html#random.Random)
+- [SystemRandom class](https://docs.python.org/3/library/random.html#random.Random)
+
+And the getter/stter
+
+- [seed](https://docs.python.org/3/library/random.html#random.seed)
+- [getstate](https://docs.python.org/3/library/random.html#random.getstate)
+- [setstate](https://docs.python.org/3/library/random.html#random.setstate)
+
``````




---

archive/issue_events_447400.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-02T20:45:28Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/32439",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32439#event-447400"
}
```



---

archive/issue_events_447401.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-02T20:45:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32439",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32439#event-447401"
}
```



---

archive/issue_events_447402.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/32439",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32439#event-447402"
}
```



---

archive/issue_events_447403.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32439",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32439#event-447403"
}
```



---

archive/issue_events_447404.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T21:39:40Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/32439",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32439#event-447404"
}
```



---

archive/issue_events_447405.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T21:39:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32439",
    "milestone_number": null,
    "milestone_title": "sage-9.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32439#event-447405"
}
```



---

archive/issue_comments_523504.json:
```json
{
    "body": "Commit: **[1a42056](https://github.com/sagemath/sagetrac-mirror/commit/1a42056f8ad687f595a3162cf151c4b78db9641e)**",
    "created_at": "2023-01-09T16:35:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32439#issuecomment-523504",
    "user": "https://github.com/Sandstorm831"
}
```

Commit: **[1a42056](https://github.com/sagemath/sagetrac-mirror/commit/1a42056f8ad687f595a3162cf151c4b78db9641e)**



---

archive/issue_comments_523505.json:
```json
{
    "body": "Branch: **[u/gh-Sandstorm831/32439](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-Sandstorm831/32439)**",
    "created_at": "2023-01-09T16:35:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32439#issuecomment-523505",
    "user": "https://github.com/Sandstorm831"
}
```

Branch: **[u/gh-Sandstorm831/32439](https://github.com/sagemath/sagetrac-mirror/tree/u/gh-Sandstorm831/32439)**



---

archive/issue_comments_523506.json:
```json
{
    "body": "<div id=\"comment:8\"></div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f0f4fc65c4aed652b2d4d7189247b8258d55fb94\">f0f4fc6</a></td><td><code>minor documentation addition</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7175d2f75c73a8db254e38b201991cb4deb0017e\">7175d2f</a></td><td><code>Revert \"minor documentation addition\"</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/81c4314f73cc82f928a1e465d19c53f3bee14a67\">81c4314</a></td><td><code>implemented choices, randbytes and triangular functions</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1a42056f8ad687f595a3162cf151c4b78db9641e\">1a42056</a></td><td><code>adding examples</code></td></tr></table>\n\nI have added the functions along the examples, I wanted to know that do we have to write the complete Random class and SystemRandom class? Also getter, setter and seed have to be added in the both classes or seperately ?",
    "created_at": "2023-01-09T16:35:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32439",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32439#issuecomment-523506",
    "user": "https://github.com/Sandstorm831"
}
```

<div id="comment:8"></div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f0f4fc65c4aed652b2d4d7189247b8258d55fb94">f0f4fc6</a></td><td><code>minor documentation addition</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7175d2f75c73a8db254e38b201991cb4deb0017e">7175d2f</a></td><td><code>Revert "minor documentation addition"</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/81c4314f73cc82f928a1e465d19c53f3bee14a67">81c4314</a></td><td><code>implemented choices, randbytes and triangular functions</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1a42056f8ad687f595a3162cf151c4b78db9641e">1a42056</a></td><td><code>adding examples</code></td></tr></table>

I have added the functions along the examples, I wanted to know that do we have to write the complete Random class and SystemRandom class? Also getter, setter and seed have to be added in the both classes or seperately ?



---

archive/issue_events_447406.json:
```json
{
    "actor": "https://github.com/Sandstorm831",
    "created_at": "2023-01-09T16:35:14Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/32439",
    "milestone_number": null,
    "milestone_title": "sage-9.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32439#event-447406"
}
```



---

archive/issue_events_447407.json:
```json
{
    "actor": "https://github.com/Sandstorm831",
    "created_at": "2023-01-09T16:35:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32439",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32439#event-447407"
}
```
