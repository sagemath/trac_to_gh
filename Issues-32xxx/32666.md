# Issue 32666: Polyhedron_base.vertex_adjacency_matrix: Do not use face_lattice

archive/issues_032429.json:
```json
{
    "assignees": [],
    "body": "instead just go through `self.combinatorial_polyhedron().edges()`\n\nThis should be faster - and only avoids an indirect dependency of `sage.geometry.polyhedron.plot` on `sage.combinat`\n\nCC:  @kliem\n\nBranch/Commit: **[2cd253c](https://github.com/sagemath/sagetrac-mirror/commit/2cd253c1b7d2b6cb6b8ca0254087565652b9d723)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton, Matthias Koeppe**\n\nAuthor: **Jonathan Kliem**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32666_\n\n",
    "closed_at": "2021-10-19T20:31:31Z",
    "created_at": "2021-10-11T03:27:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Polyhedron_base.vertex_adjacency_matrix: Do not use face_lattice",
    "type": "issue",
    "updated_at": "2021-10-19T20:31:31Z",
    "url": "https://github.com/sagemath/sage/issues/32666",
    "user": "https://github.com/mkoeppe"
}
```
instead just go through `self.combinatorial_polyhedron().edges()`

This should be faster - and only avoids an indirect dependency of `sage.geometry.polyhedron.plot` on `sage.combinat`

CC:  @kliem

Branch/Commit: **[2cd253c](https://github.com/sagemath/sagetrac-mirror/commit/2cd253c1b7d2b6cb6b8ca0254087565652b9d723)**

Reviewer: **Frédéric Chapoton, Matthias Koeppe**

Author: **Jonathan Kliem**

_Issue created by migration from https://trac.sagemath.org/ticket/32666_





---

archive/issue_events_431674.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-11T03:27:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32666",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32666#event-431674"
}
```



---

archive/issue_events_431675.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-11T03:27:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32666",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "component: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32666#event-431675"
}
```



---

archive/issue_events_431676.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-11T03:27:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32666",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32666#event-431676"
}
```



---

archive/issue_events_431677.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-11T03:27:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32666",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32666#event-431677"
}
```



---

archive/issue_comments_529926.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7db923eba8e39a66672c58be42ecc727043c6581\">7db923e</a></td><td><code>compute vertex adjacency matrix from edges</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/21a6c945088d0b4467e2a02d0028743f2ca402c2\">21a6c94</a></td><td><code>remove code duplications</code></td></tr></table>\n",
    "created_at": "2021-10-11T07:30:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32666",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32666#issuecomment-529926",
    "user": "https://github.com/kliem"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7db923eba8e39a66672c58be42ecc727043c6581">7db923e</a></td><td><code>compute vertex adjacency matrix from edges</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/21a6c945088d0b4467e2a02d0028743f2ca402c2">21a6c94</a></td><td><code>remove code duplications</code></td></tr></table>




---

archive/issue_events_431678.json:
```json
{
    "actor": "https://github.com/kliem",
    "created_at": "2021-10-11T07:30:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32666",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32666#event-431678"
}
```



---

archive/issue_comments_529927.json:
```json
{
    "body": "Author: **Jonathan Kliem**",
    "created_at": "2021-10-11T07:30:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32666",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32666#issuecomment-529927",
    "user": "https://github.com/kliem"
}
```

Author: **Jonathan Kliem**



---

archive/issue_comments_529928.json:
```json
{
    "body": "Commit: **[21a6c94](https://github.com/sagemath/sagetrac-mirror/commit/21a6c945088d0b4467e2a02d0028743f2ca402c2)**",
    "created_at": "2021-10-11T07:30:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32666",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32666#issuecomment-529928",
    "user": "https://github.com/kliem"
}
```

Commit: **[21a6c94](https://github.com/sagemath/sagetrac-mirror/commit/21a6c945088d0b4467e2a02d0028743f2ca402c2)**



---

archive/issue_comments_529929.json:
```json
{
    "body": "Branch: **[public/32666](https://github.com/sagemath/sagetrac-mirror/tree/public/32666)**",
    "created_at": "2021-10-11T07:30:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32666",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32666#issuecomment-529929",
    "user": "https://github.com/kliem"
}
```

Branch: **[public/32666](https://github.com/sagemath/sagetrac-mirror/tree/public/32666)**



---

archive/issue_comments_529930.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2021-10-11T15:39:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32666",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32666#issuecomment-529930",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_529931.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nok, looks good",
    "created_at": "2021-10-11T15:39:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32666",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32666#issuecomment-529931",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

ok, looks good



---

archive/issue_events_431679.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-10-11T15:39:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32666",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32666#event-431679"
}
```



---

archive/issue_events_431680.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2021-10-11T15:39:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32666",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32666#event-431680"
}
```



---

archive/issue_comments_529932.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nThanks for working on this!",
    "created_at": "2021-10-11T17:16:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32666",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32666#issuecomment-529932",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:3" align="right">Comment 3</div>

Thanks for working on this!



---

archive/issue_comments_529933.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThanks for the review.\nThis change turned out to be surprisingly uncomplicated.",
    "created_at": "2021-10-11T19:06:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32666",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32666#issuecomment-529933",
    "user": "https://github.com/kliem"
}
```

<div id="comment:4" align="right">Comment 4</div>

Thanks for the review.
This change turned out to be surprisingly uncomplicated.



---

archive/issue_comments_529934.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nhow about\n\n```\n--- a/src/sage/geometry/polyhedron/combinatorial_polyhedron/base.pyx\n+++ b/src/sage/geometry/polyhedron/combinatorial_polyhedron/base.pyx\n@@ -1276,7 +1276,7 @@ cdef class CombinatorialPolyhedron(SageObject):\n             sage: polytopes.cube().vertex_adjacency_matrix().is_immutable()\n             True\n         \"\"\"\n-        from sage.rings.all import ZZ\n+        from sage.rings.integer_ring import ZZ\n         from sage.matrix.constructor import matrix\n```",
    "created_at": "2021-10-11T19:56:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32666",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32666#issuecomment-529934",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:5" align="right">Comment 5</div>

how about

```
--- a/src/sage/geometry/polyhedron/combinatorial_polyhedron/base.pyx
+++ b/src/sage/geometry/polyhedron/combinatorial_polyhedron/base.pyx
@@ -1276,7 +1276,7 @@ cdef class CombinatorialPolyhedron(SageObject):
             sage: polytopes.cube().vertex_adjacency_matrix().is_immutable()
             True
         """
-        from sage.rings.all import ZZ
+        from sage.rings.integer_ring import ZZ
         from sage.matrix.constructor import matrix
```



---

archive/issue_comments_529935.json:
```json
{
    "body": "Changed commit from **[21a6c94](https://github.com/sagemath/sagetrac-mirror/commit/21a6c945088d0b4467e2a02d0028743f2ca402c2)** to **[2cd253c](https://github.com/sagemath/sagetrac-mirror/commit/2cd253c1b7d2b6cb6b8ca0254087565652b9d723)**",
    "created_at": "2021-10-11T19:58:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32666",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32666#issuecomment-529935",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[21a6c94](https://github.com/sagemath/sagetrac-mirror/commit/21a6c945088d0b4467e2a02d0028743f2ca402c2)** to **[2cd253c](https://github.com/sagemath/sagetrac-mirror/commit/2cd253c1b7d2b6cb6b8ca0254087565652b9d723)**



---

archive/issue_events_431681.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2021-10-11T19:58:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32666",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32666#event-431681"
}
```



---

archive/issue_events_431682.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2021-10-11T19:58:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32666",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32666#event-431682"
}
```



---

archive/issue_comments_529936.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2cd253c1b7d2b6cb6b8ca0254087565652b9d723\">2cd253c</a></td><td><code>more specific import</code></td></tr></table>\n",
    "created_at": "2021-10-11T19:58:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32666",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32666#issuecomment-529936",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2cd253c1b7d2b6cb6b8ca0254087565652b9d723">2cd253c</a></td><td><code>more specific import</code></td></tr></table>




---

archive/issue_comments_529937.json:
```json
{
    "body": "Changed reviewer from **Fr\u00e9d\u00e9ric Chapoton** to **Fr\u00e9d\u00e9ric Chapoton, Matthias Koeppe**",
    "created_at": "2021-10-11T21:08:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32666",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32666#issuecomment-529937",
    "user": "https://github.com/mkoeppe"
}
```

Changed reviewer from **Frédéric Chapoton** to **Frédéric Chapoton, Matthias Koeppe**



---

archive/issue_comments_529938.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nThanks!",
    "created_at": "2021-10-11T21:08:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32666",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32666#issuecomment-529938",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:7" align="right">Comment 7</div>

Thanks!



---

archive/issue_events_431683.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-11T21:08:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32666",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32666#event-431683"
}
```



---

archive/issue_events_431684.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-10-11T21:08:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32666",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32666#event-431684"
}
```



---

archive/issue_comments_529939.json:
```json
{
    "body": "Changed branch from **[public/32666](https://github.com/sagemath/sagetrac-mirror/tree/public/32666)** to **[2cd253c](https://github.com/sagemath/sagetrac-mirror/commit/2cd253c1b7d2b6cb6b8ca0254087565652b9d723)**",
    "created_at": "2021-10-19T20:31:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32666",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32666#issuecomment-529939",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/32666](https://github.com/sagemath/sagetrac-mirror/tree/public/32666)** to **[2cd253c](https://github.com/sagemath/sagetrac-mirror/commit/2cd253c1b7d2b6cb6b8ca0254087565652b9d723)**



---

archive/issue_events_431685.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-10-19T20:31:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32666",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32666#event-431685"
}
```



---

archive/issue_events_431686.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "ba408efdabd45d397edf1f2e54100abd42fbbeb1",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-10-19T20:31:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32666",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32666#event-431686"
}
```
