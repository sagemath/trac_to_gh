# Issue 32237: Fix use of MixedIntegerLinearProgram in sage.graphs.graph - part 1

archive/issues_032000.json:
```json
{
    "assignees": [],
    "body": "Part of #32191 (Meta-ticket: Audit/fix all uses of `MixedIntegerLinearProgram` in `sage.combinat`, `sage.graphs`).\n\nWe update the following methods in `graph.py` and `bipartite_graph.py`:\n- `degree_constrained_subgraph `\n- `minimum_outdegree_orientation `\n- `matching `\n- `has_perfect_matching `\n\nDepends on #32197\n\nCC:  @mkoeppe @tscrim @dimpase\n\nBranch/Commit: **[b7e9915](https://github.com/sagemath/sagetrac-mirror/commit/b7e991572234f6080305e38227c4aae4aca39a6d)**\n\nReviewer: **Matthias Koeppe**\n\nAuthor: **David Coudert**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/32237_\n\n",
    "closed_at": "2021-07-25T13:26:18Z",
    "created_at": "2021-07-19T11:59:43Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix use of MixedIntegerLinearProgram in sage.graphs.graph - part 1",
    "type": "issue",
    "updated_at": "2021-07-25T13:26:18Z",
    "url": "https://github.com/sagemath/sage/issues/32237",
    "user": "https://github.com/dcoudert"
}
```
Part of #32191 (Meta-ticket: Audit/fix all uses of `MixedIntegerLinearProgram` in `sage.combinat`, `sage.graphs`).

We update the following methods in `graph.py` and `bipartite_graph.py`:
- `degree_constrained_subgraph `
- `minimum_outdegree_orientation `
- `matching `
- `has_perfect_matching `

Depends on #32197

CC:  @mkoeppe @tscrim @dimpase

Branch/Commit: **[b7e9915](https://github.com/sagemath/sagetrac-mirror/commit/b7e991572234f6080305e38227c4aae4aca39a6d)**

Reviewer: **Matthias Koeppe**

Author: **David Coudert**

_Issue created by migration from https://trac.sagemath.org/ticket/32237_





---

archive/issue_events_426420.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2021-07-19T11:59:43Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/32237",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32237#event-426420"
}
```



---

archive/issue_events_426421.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2021-07-19T11:59:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32237",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32237#event-426421"
}
```



---

archive/issue_events_426422.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2021-07-19T11:59:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32237",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32237#event-426422"
}
```



---

archive/issue_events_426423.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2021-07-19T11:59:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32237",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32237#event-426423"
}
```



---

archive/issue_comments_522384.json:
```json
{
    "body": "Branch: **[oublic/graphs/32237_graph_1](https://github.com/sagemath/sagetrac-mirror/tree/oublic/graphs/32237_graph_1)**",
    "created_at": "2021-07-19T12:36:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32237",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32237#issuecomment-522384",
    "user": "https://github.com/dcoudert"
}
```

Branch: **[oublic/graphs/32237_graph_1](https://github.com/sagemath/sagetrac-mirror/tree/oublic/graphs/32237_graph_1)**



---

archive/issue_comments_522385.json:
```json
{
    "body": "Dependencies: **#32197**",
    "created_at": "2021-07-19T12:36:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32237",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32237#issuecomment-522385",
    "user": "https://github.com/dcoudert"
}
```

Dependencies: **#32197**



---

archive/issue_events_426424.json:
```json
{
    "actor": "https://github.com/dcoudert",
    "created_at": "2021-07-19T12:36:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32237",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32237#event-426424"
}
```



---

archive/issue_comments_522386.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nLast 10 new commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3dca4e940cfdbe84b9a301a71f263b36a01d441b\">3dca4e9</a></td><td><code>MixedIntegerLinearProgram.get_values: If convert=True, convert real variables from float to base_ring</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9ad3fa80afd83753fe5b6445eb8338f8f885ddcc\">9ad3fa8</a></td><td><code>MixedIntegerLinearProgram.get_converted_values; subsumed by get_values</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f62a7777b342bc46ecd5e4ccb1819d12bdb5db8a\">f62a777</a></td><td><code>src/sage/numerical/mip.pyx: Remove unused imports</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cfd8c7becb47cbe3ad9ad5cf9b6fb2b12122657e\">cfd8c7b</a></td><td><code>trac #32197: remove another useless import</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cf16293c1131db37f0ed1274eded6717c819989c\">cf16293</a></td><td><code>MixedIntegerLinearProgram.get_values: Fix up for tolerance=None</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/5a74113233bba7b0ac689f576849c9c36dd6cb39\">5a74113</a></td><td><code>MixedIntegerLinearProgram._backend_variable_value: Add docstring</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/aa4eed5843bce5bbb022ef79577e266d2ef41bfc\">aa4eed5</a></td><td><code>MixedIntegerLinearProgram._backend_variable_value*: Add docstrings, examples</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/00b34aaedbefbdfa7d431868f20055fd6d3572b6\">00b34aa</a></td><td><code>trac #32197: review commit</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/db83853c6df0ed792b4b011b842cf5fd01444765\">db83853</a></td><td><code>trac #32197: another review commit</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/231e9f86ea66aefb8ba6c33e1a03980cdc319263\">231e9f8</a></td><td><code>trac #32237: graph part 1</code></td></tr></table>\n",
    "created_at": "2021-07-19T12:37:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32237",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32237#issuecomment-522386",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:2" align="right">Comment 2</div>

Last 10 new commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3dca4e940cfdbe84b9a301a71f263b36a01d441b">3dca4e9</a></td><td><code>MixedIntegerLinearProgram.get_values: If convert=True, convert real variables from float to base_ring</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9ad3fa80afd83753fe5b6445eb8338f8f885ddcc">9ad3fa8</a></td><td><code>MixedIntegerLinearProgram.get_converted_values; subsumed by get_values</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f62a7777b342bc46ecd5e4ccb1819d12bdb5db8a">f62a777</a></td><td><code>src/sage/numerical/mip.pyx: Remove unused imports</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cfd8c7becb47cbe3ad9ad5cf9b6fb2b12122657e">cfd8c7b</a></td><td><code>trac #32197: remove another useless import</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cf16293c1131db37f0ed1274eded6717c819989c">cf16293</a></td><td><code>MixedIntegerLinearProgram.get_values: Fix up for tolerance=None</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/5a74113233bba7b0ac689f576849c9c36dd6cb39">5a74113</a></td><td><code>MixedIntegerLinearProgram._backend_variable_value: Add docstring</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/aa4eed5843bce5bbb022ef79577e266d2ef41bfc">aa4eed5</a></td><td><code>MixedIntegerLinearProgram._backend_variable_value*: Add docstrings, examples</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/00b34aaedbefbdfa7d431868f20055fd6d3572b6">00b34aa</a></td><td><code>trac #32197: review commit</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/db83853c6df0ed792b4b011b842cf5fd01444765">db83853</a></td><td><code>trac #32197: another review commit</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/231e9f86ea66aefb8ba6c33e1a03980cdc319263">231e9f8</a></td><td><code>trac #32237: graph part 1</code></td></tr></table>




---

archive/issue_comments_522387.json:
```json
{
    "body": "Changed branch from **[oublic/graphs/32237_graph_1](https://github.com/sagemath/sagetrac-mirror/tree/oublic/graphs/32237_graph_1)** to **[public/graphs/32237_graph_1](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/32237_graph_1)**",
    "created_at": "2021-07-19T12:37:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32237",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32237#issuecomment-522387",
    "user": "https://github.com/dcoudert"
}
```

Changed branch from **[oublic/graphs/32237_graph_1](https://github.com/sagemath/sagetrac-mirror/tree/oublic/graphs/32237_graph_1)** to **[public/graphs/32237_graph_1](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/32237_graph_1)**



---

archive/issue_comments_522388.json:
```json
{
    "body": "Commit: **[231e9f8](https://github.com/sagemath/sagetrac-mirror/commit/231e9f86ea66aefb8ba6c33e1a03980cdc319263)**",
    "created_at": "2021-07-19T12:37:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32237",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32237#issuecomment-522388",
    "user": "https://github.com/dcoudert"
}
```

Commit: **[231e9f8](https://github.com/sagemath/sagetrac-mirror/commit/231e9f86ea66aefb8ba6c33e1a03980cdc319263)**



---

archive/issue_comments_522389.json:
```json
{
    "body": "Reviewer: **Matthias Koeppe**",
    "created_at": "2021-07-19T13:53:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32237",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32237#issuecomment-522389",
    "user": "https://github.com/mkoeppe"
}
```

Reviewer: **Matthias Koeppe**



---

archive/issue_comments_522390.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLGTM. Green patchbot => positive review",
    "created_at": "2021-07-19T13:53:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32237",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32237#issuecomment-522390",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:3" align="right">Comment 3</div>

LGTM. Green patchbot => positive review



---

archive/issue_comments_522391.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b7e991572234f6080305e38227c4aae4aca39a6d\">b7e9915</a></td><td><code>trac #32237: add paramter to method matching in bipartite_graph</code></td></tr></table>\n",
    "created_at": "2021-07-19T15:08:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32237",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32237#issuecomment-522391",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b7e991572234f6080305e38227c4aae4aca39a6d">b7e9915</a></td><td><code>trac #32237: add paramter to method matching in bipartite_graph</code></td></tr></table>




---

archive/issue_comments_522392.json:
```json
{
    "body": "Changed commit from **[231e9f8](https://github.com/sagemath/sagetrac-mirror/commit/231e9f86ea66aefb8ba6c33e1a03980cdc319263)** to **[b7e9915](https://github.com/sagemath/sagetrac-mirror/commit/b7e991572234f6080305e38227c4aae4aca39a6d)**",
    "created_at": "2021-07-19T15:08:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32237",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32237#issuecomment-522392",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[231e9f8](https://github.com/sagemath/sagetrac-mirror/commit/231e9f86ea66aefb8ba6c33e1a03980cdc319263)** to **[b7e9915](https://github.com/sagemath/sagetrac-mirror/commit/b7e991572234f6080305e38227c4aae4aca39a6d)**



---

archive/issue_comments_522393.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,7 @@\n Part of #32191 (Meta-ticket: Audit/fix all uses of `MixedIntegerLinearProgram` in `sage.combinat`, `sage.graphs`).\n+\n+We update the following methods in `graph.py` and `bipartite_graph.py`:\n+- `degree_constrained_subgraph `\n+- `minimum_outdegree_orientation `\n+- `matching `\n+- `has_perfect_matching `\n``````\n",
    "created_at": "2021-07-19T15:10:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32237",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32237#issuecomment-522393",
    "user": "https://github.com/dcoudert"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,7 @@
 Part of #32191 (Meta-ticket: Audit/fix all uses of `MixedIntegerLinearProgram` in `sage.combinat`, `sage.graphs`).
+
+We update the following methods in `graph.py` and `bipartite_graph.py`:
+- `degree_constrained_subgraph `
+- `minimum_outdegree_orientation `
+- `matching `
+- `has_perfect_matching `
``````




---

archive/issue_comments_522394.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nI forgot to modify method `matching` in `bipartite_graph.py`.",
    "created_at": "2021-07-19T15:10:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32237",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32237#issuecomment-522394",
    "user": "https://github.com/dcoudert"
}
```

<div id="comment:5" align="right">Comment 5</div>

I forgot to modify method `matching` in `bipartite_graph.py`.



---

archive/issue_events_426425.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-20T06:25:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32237",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32237#event-426425"
}
```



---

archive/issue_events_426426.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-20T06:25:21Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/32237",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32237#event-426426"
}
```



---

archive/issue_comments_522395.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nI've tested it locally",
    "created_at": "2021-07-20T06:25:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32237",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32237#issuecomment-522395",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:6" align="right">Comment 6</div>

I've tested it locally



---

archive/issue_events_426427.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-07-25T13:26:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/32237",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32237#event-426427"
}
```



---

archive/issue_events_426428.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "2551a1ac24c6142a419f2f2f3495e0827265375b",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-07-25T13:26:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/32237",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/32237#event-426428"
}
```



---

archive/issue_comments_522396.json:
```json
{
    "body": "Changed branch from **[public/graphs/32237_graph_1](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/32237_graph_1)** to **[b7e9915](https://github.com/sagemath/sagetrac-mirror/commit/b7e991572234f6080305e38227c4aae4aca39a6d)**",
    "created_at": "2021-07-25T13:26:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/32237",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/32237#issuecomment-522396",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/graphs/32237_graph_1](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/32237_graph_1)** to **[b7e9915](https://github.com/sagemath/sagetrac-mirror/commit/b7e991572234f6080305e38227c4aae4aca39a6d)**
