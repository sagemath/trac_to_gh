# Issue 11552: only print warnings when needed

archive/issues_011552.json:
```json
{
    "body": "currently building sage, or installing spkgs or running other scripts which source sage-env might sometimes print \n\nWarning: Attempted to overwrite SAGE_ROOT environment variable\n\nwhile it's not overwriting or attempting to overwrite SAGE_ROOT\n\nIssue created by migration from https://trac.sagemath.org/ticket/11724\n\n",
    "created_at": "2011-08-23T20:59:50Z",
    "labels": [
        "scripts",
        "minor",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "only print warnings when needed",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/11552",
    "user": "mderickx"
}
```
currently building sage, or installing spkgs or running other scripts which source sage-env might sometimes print 

Warning: Attempted to overwrite SAGE_ROOT environment variable

while it's not overwriting or attempting to overwrite SAGE_ROOT

Issue created by migration from https://trac.sagemath.org/ticket/11724





---

archive/issue_comments_129887.json:
```json
{
    "body": "apply to scripts repository",
    "created_at": "2011-08-23T21:02:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11552",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11552#issuecomment-129887",
    "user": "mderickx"
}
```

apply to scripts repository



---

archive/issue_comments_129888.json:
```json
{
    "body": "Attachment [11724-dont-print-unneeded-waringin-message.patch](tarball://root/attachments/some-uuid/ticket11724/11724-dont-print-unneeded-waringin-message.patch) by mderickx created at 2011-08-23 21:03:31\n\nnote that sage-env is not executable so you need to do \n \n   source sage-env\n\nto test it.",
    "created_at": "2011-08-23T21:03:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11552",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11552#issuecomment-129888",
    "user": "mderickx"
}
```

Attachment [11724-dont-print-unneeded-waringin-message.patch](tarball://root/attachments/some-uuid/ticket11724/11724-dont-print-unneeded-waringin-message.patch) by mderickx created at 2011-08-23 21:03:31

note that sage-env is not executable so you need to do 
 
   source sage-env

to test it.



---

archive/issue_comments_129889.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2011-08-23T21:03:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11552",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11552#issuecomment-129889",
    "user": "mderickx"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_129890.json:
```json
{
    "body": "This is also fixed by #5852, where the `SAGE_ROOT` detection is completely changed.\n\nSo personally, I prefer closing this as \"duplicate\".",
    "created_at": "2011-10-31T12:15:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11552",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11552#issuecomment-129890",
    "user": "jdemeyer"
}
```

This is also fixed by #5852, where the `SAGE_ROOT` detection is completely changed.

So personally, I prefer closing this as "duplicate".



---

archive/issue_comments_129891.json:
```json
{
    "body": "that's ok with me",
    "created_at": "2011-10-31T13:05:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11552",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11552#issuecomment-129891",
    "user": "mderickx"
}
```

that's ok with me



---

archive/issue_comments_129892.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2011-12-17T19:51:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11552",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11552#issuecomment-129892",
    "user": "mderickx"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_129893.json:
```json
{
    "body": "Hej Jeroen,\n\nI was just walking trough the open tickets I participated in. Are you still going to close this one as duplicate?",
    "created_at": "2011-12-17T19:51:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11552",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11552#issuecomment-129893",
    "user": "mderickx"
}
```

Hej Jeroen,

I was just walking trough the open tickets I participated in. Are you still going to close this one as duplicate?



---

archive/issue_comments_129894.json:
```json
{
    "body": "Resolution: duplicate",
    "created_at": "2012-01-05T13:32:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11552",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11552#issuecomment-129894",
    "user": "jdemeyer"
}
```

Resolution: duplicate
