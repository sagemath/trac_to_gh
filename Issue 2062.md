# Issue 2062: [with patch] add support for binary vectors (sd7 test ticket)

archive/issues_002062.json:
```json
{
    "body": "Assignee: @mwhansen\n\nCC:  sage-combinat\n\nblah blah\n\nIssue created by migration from https://trac.sagemath.org/ticket/2062\n\n",
    "created_at": "2008-02-05T18:55:33Z",
    "labels": [
        "component: combinatorics"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-2.10.2",
    "title": "[with patch] add support for binary vectors (sd7 test ticket)",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/2062",
    "user": "https://github.com/mwhansen"
}
```
Assignee: @mwhansen

CC:  sage-combinat

blah blah

Issue created by migration from https://trac.sagemath.org/ticket/2062





---

archive/issue_comments_013320.json:
```json
{
    "body": "Attachment [bv.patch](tarball://root/attachments/some-uuid/ticket2062/bv.patch) by @mwhansen created at 2008-02-05 18:56:56",
    "created_at": "2008-02-05T18:56:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2062",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2062#issuecomment-13320",
    "user": "https://github.com/mwhansen"
}
```

Attachment [bv.patch](tarball://root/attachments/some-uuid/ticket2062/bv.patch) by @mwhansen created at 2008-02-05 18:56:56



---

archive/issue_comments_013321.json:
```json
{
    "body": "Looks good from the back of the room!",
    "created_at": "2008-02-05T18:57:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2062",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2062#issuecomment-13321",
    "user": "https://github.com/rlmill"
}
```

Looks good from the back of the room!



---

archive/issue_comments_013322.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2008-02-05T19:52:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/2062",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/2062#issuecomment-13322",
    "user": "https://github.com/mwhansen"
}
```

Resolution: invalid



---

archive/issue_events_002223.json:
```json
{
    "actor": "https://github.com/mwhansen",
    "created_at": "2008-02-05T19:52:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/2062",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/2062#event-2223"
}
```
