# Issue 22997: Parse unevaluated integral from giac

archive/issues_022760.json:
```json
{
    "body": "If `integrate(..., algorithm='giac')` does not find a solution, Sage should return an unevaluated expression. \n\nFor example, consider `integrate(exp(-x^2)*log(x), x, algorithm='giac')`.\n\nIt returns either\n\n```\nTraceback (most recent call last):\n...\nNotImplementedError: Unable to parse Giac output: integrate(ln(x)*exp(-x^2),x)\n```\n\nor\n\n```\nintegration(e^(-x^2)*ln(x), x)\n```\n\ndepending on the user's locale settings (EN and FR respectively). \n\nOTOH, both Maxima and SymPy return `integrate(e<sup>(-x</sup>2)*log(x), x)`.\n\nKeywords: integrate, interfaces, giac\n\nBranch/Commit: 0e5bdf047579f58f169429ea00f8519d4bdb80b4\n\nReviewer: Steven Trogdon\n\nAuthor: Marcelo Forets\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/22997\n\n",
    "closed_at": "2017-05-20T20:07:45Z",
    "created_at": "2017-05-14T08:26:34Z",
    "labels": [
        "component: calculus"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.0",
    "title": "Parse unevaluated integral from giac",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/22997",
    "user": "https://github.com/mforets"
}
```
If `integrate(..., algorithm='giac')` does not find a solution, Sage should return an unevaluated expression. 

For example, consider `integrate(exp(-x^2)*log(x), x, algorithm='giac')`.

It returns either

```
Traceback (most recent call last):
...
NotImplementedError: Unable to parse Giac output: integrate(ln(x)*exp(-x^2),x)
```

or

```
integration(e^(-x^2)*ln(x), x)
```

depending on the user's locale settings (EN and FR respectively). 

OTOH, both Maxima and SymPy return `integrate(e<sup>(-x</sup>2)*log(x), x)`.

Keywords: integrate, interfaces, giac

Branch/Commit: 0e5bdf047579f58f169429ea00f8519d4bdb80b4

Reviewer: Steven Trogdon

Author: Marcelo Forets

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/22997





---

archive/issue_comments_430635.json:
```json
{
    "body": "Changing commit from \"\" to \"f3c9d15f97ebe44a4fa6fe83f3b56d62a34689ef\"",
    "created_at": "2017-05-14T09:31:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22997#issuecomment-430635",
    "user": "https://github.com/mforets"
}
```

Changing commit from "" to "f3c9d15f97ebe44a4fa6fe83f3b56d62a34689ef"



---

archive/issue_comments_430636.json:
```json
{
    "body": "Changing branch from \"\" to \"u/mforets/22997\"",
    "created_at": "2017-05-14T09:31:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22997#issuecomment-430636",
    "user": "https://github.com/mforets"
}
```

Changing branch from "" to "u/mforets/22997"



---

archive/issue_comments_430637.json:
```json
{
    "body": "<a id='comment:1'></a>does it correspond at the level or `giac_integrator` (see `symbolic/integration/external.py`)? or at the level of the giac interface? (see `def _sage_(self, locals={})`).\n\nin the former case, is this correct?\n\n```\n...\nif 'integrate' in format(result) or 'integration' in format(result):\n    return integrate(expr, v, a, b, hold=True) # ???\nelse:\n    return result._sage_()\n```\n\n---\nNew commits:",
    "created_at": "2017-05-14T09:31:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22997#issuecomment-430637",
    "user": "https://github.com/mforets"
}
```

<a id='comment:1'></a>does it correspond at the level or `giac_integrator` (see `symbolic/integration/external.py`)? or at the level of the giac interface? (see `def _sage_(self, locals={})`).

in the former case, is this correct?

```
...
if 'integrate' in format(result) or 'integration' in format(result):
    return integrate(expr, v, a, b, hold=True) # ???
else:
    return result._sage_()
```

---
New commits:



---

archive/issue_comments_430638.json:
```json
{
    "body": "Changing commit from \"f3c9d15f97ebe44a4fa6fe83f3b56d62a34689ef\" to \"2bba41df3121f7c1b14f9bc072aae8893e8494c9\"",
    "created_at": "2017-05-14T09:47:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22997#issuecomment-430638",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "f3c9d15f97ebe44a4fa6fe83f3b56d62a34689ef" to "2bba41df3121f7c1b14f9bc072aae8893e8494c9"



---

archive/issue_comments_430639.json:
```json
{
    "body": "<a id='comment:2'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2017-05-14T09:47:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22997#issuecomment-430639",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:2'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_430640.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2017-05-14T09:48:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22997#issuecomment-430640",
    "user": "https://github.com/mforets"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_430641.json:
```json
{
    "body": "<a id='comment:4'></a>Perhaps this exposes a bug in `sympy_integrator`. These work\n\n```\nsage: integrate(e^(-x^2)*log(x), x, algorithm='giac')\nintegrate(e^(-x^2)*log(x), x)\nsage: integrate(e^(-x^2)*log(x), x, algorithm='maxima')\nintegrate(e^(-x^2)*log(x), x)\n```\nalthough `algorithm='maxima'` was not doctested, but\n\n```\nintegrate(e^(-x^2)*log(x), x, algorithm='sympy')\n```\njust hangs.",
    "created_at": "2017-05-18T04:50:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22997#issuecomment-430641",
    "user": "https://github.com/strogdon"
}
```

<a id='comment:4'></a>Perhaps this exposes a bug in `sympy_integrator`. These work

```
sage: integrate(e^(-x^2)*log(x), x, algorithm='giac')
integrate(e^(-x^2)*log(x), x)
sage: integrate(e^(-x^2)*log(x), x, algorithm='maxima')
integrate(e^(-x^2)*log(x), x)
```
although `algorithm='maxima'` was not doctested, but

```
integrate(e^(-x^2)*log(x), x, algorithm='sympy')
```
just hangs.



---

archive/issue_comments_430642.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2017-05-18T04:50:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22997#issuecomment-430642",
    "user": "https://github.com/strogdon"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_430643.json:
```json
{
    "body": "<a id='comment:5'></a>here it works only after a looong time:\n\n```\nsage: integrate(e^(-x^2)*log(x), x, algorithm='sympy') # 15-20 minutes, processor intensive\nintegrate(e^(-x^2)*log(x), x)\n```",
    "created_at": "2017-05-18T05:13:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22997#issuecomment-430643",
    "user": "https://github.com/mforets"
}
```

<a id='comment:5'></a>here it works only after a looong time:

```
sage: integrate(e^(-x^2)*log(x), x, algorithm='sympy') # 15-20 minutes, processor intensive
integrate(e^(-x^2)*log(x), x)
```



---

archive/issue_comments_430644.json:
```json
{
    "body": "Changing status from needs_work to positive_review.",
    "created_at": "2017-05-18T14:03:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22997#issuecomment-430644",
    "user": "https://github.com/strogdon"
}
```

Changing status from needs_work to positive_review.



---

archive/issue_comments_430645.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Steven Trogdon\"",
    "created_at": "2017-05-18T14:03:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22997#issuecomment-430645",
    "user": "https://github.com/strogdon"
}
```

Changing reviewer from "" to "Steven Trogdon"



---

archive/issue_comments_430646.json:
```json
{
    "body": "<a id='comment:6'></a>OK, I was impatient. It took 30+ minutes here.",
    "created_at": "2017-05-18T14:03:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22997#issuecomment-430646",
    "user": "https://github.com/strogdon"
}
```

<a id='comment:6'></a>OK, I was impatient. It took 30+ minutes here.



---

archive/issue_comments_430647.json:
```json
{
    "body": "Changing author from \"\" to \"Marcelo Forets\"",
    "created_at": "2017-05-18T14:03:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22997#issuecomment-430647",
    "user": "https://github.com/strogdon"
}
```

Changing author from "" to "Marcelo Forets"



---

archive/issue_comments_430648.json:
```json
{
    "body": "<a id='comment:7'></a>Replying to [strogdon](#comment%3A6):\n> OK, I was impatient. It took 30+ minutes here.\n\n\nThanks for the review!",
    "created_at": "2017-05-18T14:41:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22997#issuecomment-430648",
    "user": "https://github.com/mforets"
}
```

<a id='comment:7'></a>Replying to [strogdon](#comment%3A6):
> OK, I was impatient. It took 30+ minutes here.


Thanks for the review!



---

archive/issue_comments_430649.json:
```json
{
    "body": "<a id='comment:8'></a>Merge conflict",
    "created_at": "2017-05-18T19:52:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22997#issuecomment-430649",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:8'></a>Merge conflict



---

archive/issue_comments_430650.json:
```json
{
    "body": "Changing status from positive_review to needs_work.",
    "created_at": "2017-05-18T19:52:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22997#issuecomment-430650",
    "user": "https://github.com/vbraun"
}
```

Changing status from positive_review to needs_work.



---

archive/issue_comments_430651.json:
```json
{
    "body": "<a id='comment:9'></a>This really should have depended on #22833 since `src/sage/symbolic/integration/external.py` was touched there. I though I had that ticket merged when I tested things here, but apparently not, or else I would have noticed the conflict. #22833 is now in `8.0.beta7` and so this will have to be rebased. Just change it back to `positive_review` when that is done.",
    "created_at": "2017-05-19T00:52:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22997#issuecomment-430651",
    "user": "https://github.com/strogdon"
}
```

<a id='comment:9'></a>This really should have depended on #22833 since `src/sage/symbolic/integration/external.py` was touched there. I though I had that ticket merged when I tested things here, but apparently not, or else I would have noticed the conflict. #22833 is now in `8.0.beta7` and so this will have to be rebased. Just change it back to `positive_review` when that is done.



---

archive/issue_comments_430652.json:
```json
{
    "body": "<a id='comment:10'></a>Replying to [strogdon](#comment%3A9):\n> This really should have depended on #22833 since `src/sage/symbolic/integration/external.py` was touched there. \n\n\nmy fault, thanks for pointing it out. \n\n> I though I had that ticket merged when I tested things here, but apparently not, or else I would have noticed the conflict. #22833 is now in `8.0.beta7` and so this will have to be rebased. \n\n\nthere's something that i don't understand. i do:\n\n```\n(in t/mforets/22997) $ git rebase develop\n(... solve the merge conflict manually ...)\n(in t/mforets/22997) $ git add src/sage/symbolic/integration/external.py\n(in t/mforets/22997) $ git status\nrebase in progress; onto 7a36941\nYou are currently rebasing branch 't/22997/22997' on '7a36941'.\n  (all conflicts fixed: run \"git rebase --continue\")\n\nChanges to be committed:\n  (use \"git reset HEAD <file>...\" to unstage)\n\n\tmodified:   src/sage/symbolic/integration/external.py\n(in t/mforets/22997) $ git rebase --continue\nApplying: parse result in giac_integrator\n```\n\nup to now everything seems to work fine, locally. however i try to push to the remote branch and it breaks:\n\n```\n(in t/mforets/22997) $ git push trac HEAD:u/mforets/22997\nTo trac.sagemath.org:sage.git\n ! [rejected]        HEAD -> u/mforets/22997 (non-fast-forward)\nerror: failed to push some refs to 'git@trac.sagemath.org:sage.git'\nhint: Updates were rejected because a pushed branch tip is behind its remote\nhint: counterpart. Check out this branch and integrate the remote changes\nhint: (e.g. 'git pull ...') before pushing again.\nhint: See the 'Note about fast-forwards' in 'git push --help' for details.\n```\n\nit also breaks if i use the automatic command:\n\n```\n(in t/mforets/22997) $ git trac push\nPushing to Trac #22997...\nGuessed remote branch: u/mforets/22997\nTraceback (most recent call last):\n  File \"/usr/local/bin/git-trac\", line 18, in <module>\n    cmdline.launch()\n  File \"/Users/forets/sage-src/sage/git-trac-command/git_trac/cmdline.py\", line 223, in launch\n    app.push(ticket_number, remote=args.remote, force=args.force)\n  File \"/Users/forets/sage-src/sage/git-trac-command/git_trac/app.py\", line 216, in push\n    self.repo.push(remote, force)\n  File \"/Users/forets/sage-src/sage/git-trac-command/git_trac/git_repository.py\", line 197, in push\n    self.git.echo.push('trac', refspec)\n  File \"/Users/forets/sage-src/sage/git-trac-command/git_trac/git_interface.py\", line 341, in meth\n    return self.execute(git_cmd, *args, **kwds)\n  File \"/Users/forets/sage-src/sage/git-trac-command/git_trac/git_interface.py\", line 98, in execute\n    popen_stderr=subprocess.PIPE)\n  File \"/Users/forets/sage-src/sage/git-trac-command/git_trac/git_interface.py\", line 263, in _run\n    raise GitError(result)\ngit_trac.git_error.GitError: git returned with non-zero exit code (1) when executing \"git push trac HEAD:refs/heads/u/mforets/22997\"\n    STDERR: To trac.sagemath.org:sage.git\n    STDERR:  ! [rejected]        HEAD -> u/mforets/22997 (non-fast-forward)\n    STDERR: error: failed to push some refs to 'git@trac.sagemath.org:sage.git'\n    STDERR: hint: Updates were rejected because a pushed branch tip is behind its remote\n    STDERR: hint: counterpart. Check out this branch and integrate the remote changes\n    STDERR: hint: (e.g. 'git pull ...') before pushing again.\n    STDERR: hint: See the 'Note about fast-forwards' in 'git push --help' for details.\n```",
    "created_at": "2017-05-19T07:56:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22997#issuecomment-430652",
    "user": "https://github.com/mforets"
}
```

<a id='comment:10'></a>Replying to [strogdon](#comment%3A9):
> This really should have depended on #22833 since `src/sage/symbolic/integration/external.py` was touched there. 


my fault, thanks for pointing it out. 

> I though I had that ticket merged when I tested things here, but apparently not, or else I would have noticed the conflict. #22833 is now in `8.0.beta7` and so this will have to be rebased. 


there's something that i don't understand. i do:

```
(in t/mforets/22997) $ git rebase develop
(... solve the merge conflict manually ...)
(in t/mforets/22997) $ git add src/sage/symbolic/integration/external.py
(in t/mforets/22997) $ git status
rebase in progress; onto 7a36941
You are currently rebasing branch 't/22997/22997' on '7a36941'.
  (all conflicts fixed: run "git rebase --continue")

Changes to be committed:
  (use "git reset HEAD <file>..." to unstage)

	modified:   src/sage/symbolic/integration/external.py
(in t/mforets/22997) $ git rebase --continue
Applying: parse result in giac_integrator
```

up to now everything seems to work fine, locally. however i try to push to the remote branch and it breaks:

```
(in t/mforets/22997) $ git push trac HEAD:u/mforets/22997
To trac.sagemath.org:sage.git
 ! [rejected]        HEAD -> u/mforets/22997 (non-fast-forward)
error: failed to push some refs to 'git@trac.sagemath.org:sage.git'
hint: Updates were rejected because a pushed branch tip is behind its remote
hint: counterpart. Check out this branch and integrate the remote changes
hint: (e.g. 'git pull ...') before pushing again.
hint: See the 'Note about fast-forwards' in 'git push --help' for details.
```

it also breaks if i use the automatic command:

```
(in t/mforets/22997) $ git trac push
Pushing to Trac #22997...
Guessed remote branch: u/mforets/22997
Traceback (most recent call last):
  File "/usr/local/bin/git-trac", line 18, in <module>
    cmdline.launch()
  File "/Users/forets/sage-src/sage/git-trac-command/git_trac/cmdline.py", line 223, in launch
    app.push(ticket_number, remote=args.remote, force=args.force)
  File "/Users/forets/sage-src/sage/git-trac-command/git_trac/app.py", line 216, in push
    self.repo.push(remote, force)
  File "/Users/forets/sage-src/sage/git-trac-command/git_trac/git_repository.py", line 197, in push
    self.git.echo.push('trac', refspec)
  File "/Users/forets/sage-src/sage/git-trac-command/git_trac/git_interface.py", line 341, in meth
    return self.execute(git_cmd, *args, **kwds)
  File "/Users/forets/sage-src/sage/git-trac-command/git_trac/git_interface.py", line 98, in execute
    popen_stderr=subprocess.PIPE)
  File "/Users/forets/sage-src/sage/git-trac-command/git_trac/git_interface.py", line 263, in _run
    raise GitError(result)
git_trac.git_error.GitError: git returned with non-zero exit code (1) when executing "git push trac HEAD:refs/heads/u/mforets/22997"
    STDERR: To trac.sagemath.org:sage.git
    STDERR:  ! [rejected]        HEAD -> u/mforets/22997 (non-fast-forward)
    STDERR: error: failed to push some refs to 'git@trac.sagemath.org:sage.git'
    STDERR: hint: Updates were rejected because a pushed branch tip is behind its remote
    STDERR: hint: counterpart. Check out this branch and integrate the remote changes
    STDERR: hint: (e.g. 'git pull ...') before pushing again.
    STDERR: hint: See the 'Note about fast-forwards' in 'git push --help' for details.
```



---

archive/issue_comments_430653.json:
```json
{
    "body": "<a id='comment:11'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2017-05-19T13:36:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22997#issuecomment-430653",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:11'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_430654.json:
```json
{
    "body": "Changing commit from \"2bba41df3121f7c1b14f9bc072aae8893e8494c9\" to \"0e5bdf047579f58f169429ea00f8519d4bdb80b4\"",
    "created_at": "2017-05-19T13:36:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22997#issuecomment-430654",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "2bba41df3121f7c1b14f9bc072aae8893e8494c9" to "0e5bdf047579f58f169429ea00f8519d4bdb80b4"



---

archive/issue_comments_430655.json:
```json
{
    "body": "<a id='comment:12'></a>(merged into v.8.0.beta7)",
    "created_at": "2017-05-19T13:37:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22997#issuecomment-430655",
    "user": "https://github.com/mforets"
}
```

<a id='comment:12'></a>(merged into v.8.0.beta7)



---

archive/issue_comments_430656.json:
```json
{
    "body": "Changing status from needs_work to positive_review.",
    "created_at": "2017-05-19T13:37:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22997#issuecomment-430656",
    "user": "https://github.com/mforets"
}
```

Changing status from needs_work to positive_review.



---

archive/issue_comments_430657.json:
```json
{
    "body": "Changing status from positive_review to needs_work.",
    "created_at": "2017-05-19T14:47:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22997#issuecomment-430657",
    "user": "https://github.com/fchapoton"
}
```

Changing status from positive_review to needs_work.



---

archive/issue_comments_430658.json:
```json
{
    "body": "<a id='comment:13'></a>The doctest is going to fail with the French locale:\n\n```\nsage: from sage.symbolic.integration.external import *\nsage:  giac_integrator(e^(-x^2)*log(x), x)\nintegration(e^(-x^2)*ln(x), x)\n```",
    "created_at": "2017-05-19T14:47:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22997#issuecomment-430658",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:13'></a>The doctest is going to fail with the French locale:

```
sage: from sage.symbolic.integration.external import *
sage:  giac_integrator(e^(-x^2)*log(x), x)
integration(e^(-x^2)*ln(x), x)
```



---

archive/issue_comments_430659.json:
```json
{
    "body": "<a id='comment:14'></a>oops, sorry.",
    "created_at": "2017-05-19T14:47:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22997#issuecomment-430659",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:14'></a>oops, sorry.



---

archive/issue_comments_430660.json:
```json
{
    "body": "Changing status from needs_work to positive_review.",
    "created_at": "2017-05-19T14:47:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22997#issuecomment-430660",
    "user": "https://github.com/fchapoton"
}
```

Changing status from needs_work to positive_review.



---

archive/issue_events_059585.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-05-20T20:07:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/22997#event-59585"
}
```



---

archive/issue_comments_430661.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2017-05-20T20:07:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22997#issuecomment-430661",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_comments_430662.json:
```json
{
    "body": "Changing branch from \"u/mforets/22997\" to \"0e5bdf047579f58f169429ea00f8519d4bdb80b4\"",
    "created_at": "2017-05-20T20:07:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22997",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22997#issuecomment-430662",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/mforets/22997" to "0e5bdf047579f58f169429ea00f8519d4bdb80b4"
