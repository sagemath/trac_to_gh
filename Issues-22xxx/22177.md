# Issue 22177: Implement SVG plotting in pari_jupyter

archive/issues_021940.json:
```json
{
    "body": "This requires various patches to upstream PARI:\n\n- `pari_str_1.patch` and `pari_str_2.patch`: implement new `pari_str` API. Patch taken from upstream master.\n\n- `plot_libpari.patch`: move plotting interface from gp to libPARI. Being discussed with upstream.\n\n- `plot_svg.patch`: implement SVG plotting in libPARI. Applied upstream with significant changes.\n\nThe `pari_jupyter` package also needs to be updated to support SVG plotting:\n\n**Demo**: https://github.com/OpenDreamKit/OpenDreamKit/blob/master/WP4/D4.4/demo-pari.ipynb\n\n**Tarball**: https://pypi.python.org/packages/6d/a7/9186eeacf76f8b723915b9c745ab42a73b987e5b5296fdd4a0a7c6626800/pari_jupyter-1.2.2.tar.bz2\n\nCC:  @dimpase\n\nBranch/Commit: 7df14aa9f0cd32a43f2e317c8fe43b4336ec48f0\n\nUpstream: Reported upstream. Developers acknowledge bug.\n\nReviewer: Dima Pasechnik\n\nAuthor: Jeroen Demeyer\n\nDependencies: #22276\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/22177\n\n",
    "closed_at": "2017-02-24T14:49:06Z",
    "created_at": "2017-01-12T10:12:35Z",
    "labels": [
        "component: packages: standard"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-7.6",
    "title": "Implement SVG plotting in pari_jupyter",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/22177",
    "user": "https://github.com/jdemeyer"
}
```
This requires various patches to upstream PARI:

- `pari_str_1.patch` and `pari_str_2.patch`: implement new `pari_str` API. Patch taken from upstream master.

- `plot_libpari.patch`: move plotting interface from gp to libPARI. Being discussed with upstream.

- `plot_svg.patch`: implement SVG plotting in libPARI. Applied upstream with significant changes.

The `pari_jupyter` package also needs to be updated to support SVG plotting:

**Demo**: https://github.com/OpenDreamKit/OpenDreamKit/blob/master/WP4/D4.4/demo-pari.ipynb

**Tarball**: https://pypi.python.org/packages/6d/a7/9186eeacf76f8b723915b9c745ab42a73b987e5b5296fdd4a0a7c6626800/pari_jupyter-1.2.2.tar.bz2

CC:  @dimpase

Branch/Commit: 7df14aa9f0cd32a43f2e317c8fe43b4336ec48f0

Upstream: Reported upstream. Developers acknowledge bug.

Reviewer: Dima Pasechnik

Author: Jeroen Demeyer

Dependencies: #22276

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/22177





---

archive/issue_comments_410877.json:
```json
{
    "body": "Changing component from packages: optional to packages: standard.",
    "created_at": "2017-01-12T15:58:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22177#issuecomment-410877",
    "user": "https://github.com/jdemeyer"
}
```

Changing component from packages: optional to packages: standard.



---

archive/issue_events_058320.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-01-12T15:58:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/22177",
    "milestone": "sage-7.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/22177#event-58320"
}
```



---

archive/issue_comments_410878.json:
```json
{
    "body": "Changing dependencies from \"#21756\" to \"#21756, #22180\"",
    "created_at": "2017-01-12T15:58:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22177#issuecomment-410878",
    "user": "https://github.com/jdemeyer"
}
```

Changing dependencies from "#21756" to "#21756, #22180"



---

archive/issue_comments_410879.json:
```json
{
    "body": "Changing dependencies from \"#21756, #22180\" to \"#22180, #22276\"",
    "created_at": "2017-02-13T10:30:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22177#issuecomment-410879",
    "user": "https://github.com/jdemeyer"
}
```

Changing dependencies from "#21756, #22180" to "#22180, #22276"



---

archive/issue_comments_410880.json:
```json
{
    "body": "Changing branch from \"\" to \"u/jdemeyer/implement_svg_plotting_in_pari_jupyter\"",
    "created_at": "2017-02-13T14:07:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22177#issuecomment-410880",
    "user": "https://github.com/jdemeyer"
}
```

Changing branch from "" to "u/jdemeyer/implement_svg_plotting_in_pari_jupyter"



---

archive/issue_comments_410881.json:
```json
{
    "body": "<a id='comment:4'></a>\nNew commits:\n|                                                                                                                                          |                          |\n|------------------------------------------------------------------------------------------------------------------------------------------|--------------------------|\n|[a70bf47](https://git.sagemath.org/sage.git/commit?id=a70bf47d75dbcdd6697b8fd78a823cfc7ac4dd67)|`Fix and add PARI patches`|\n|[72c15e4](https://git.sagemath.org/sage.git/commit?id=72c15e45423801e8a44a3b52fb072d0535a86aa0)|`Add patches to PARI for SVG plotting`|",
    "created_at": "2017-02-13T14:24:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22177#issuecomment-410881",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:4'></a>
New commits:
|                                                                                                                                          |                          |
|------------------------------------------------------------------------------------------------------------------------------------------|--------------------------|
|[a70bf47](https://git.sagemath.org/sage.git/commit?id=a70bf47d75dbcdd6697b8fd78a823cfc7ac4dd67)|`Fix and add PARI patches`|
|[72c15e4](https://git.sagemath.org/sage.git/commit?id=72c15e45423801e8a44a3b52fb072d0535a86aa0)|`Add patches to PARI for SVG plotting`|



---

archive/issue_comments_410882.json:
```json
{
    "body": "Changing commit from \"\" to \"72c15e45423801e8a44a3b52fb072d0535a86aa0\"",
    "created_at": "2017-02-13T14:24:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22177#issuecomment-410882",
    "user": "https://github.com/jdemeyer"
}
```

Changing commit from "" to "72c15e45423801e8a44a3b52fb072d0535a86aa0"



---

archive/issue_comments_410883.json:
```json
{
    "body": "Changing dependencies from \"#22180, #22276\" to \"#22276\"",
    "created_at": "2017-02-13T14:24:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22177#issuecomment-410883",
    "user": "https://github.com/jdemeyer"
}
```

Changing dependencies from "#22180, #22276" to "#22276"



---

archive/issue_comments_410884.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,11 @@\n+This adds various patches to upstream PARI:\n \n+- `pari_str_1.patch` and `pari_str_2.patch`: implement new `pari_str` API. Patch taken from upstream master.\n+\n+- `plot_libpari.patch`: move plotting interface from gp to libPARI. Being discussed with upstream.\n+\n+- `plot_svg.patch`: implement SVG plotting libPARI. Applied upstream with significant changes.\n+\n+The `pari_jupyter` package also needs to be updated to support SVG plotting:\n+\n+**Tarball**: https://pypi.python.org/packages/6d/a7/9186eeacf76f8b723915b9c745ab42a73b987e5b5296fdd4a0a7c6626800/pari_jupyter-1.2.2.tar.bz2\n``````\n",
    "created_at": "2017-02-13T14:32:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22177#issuecomment-410884",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,11 @@
+This adds various patches to upstream PARI:
 
+- `pari_str_1.patch` and `pari_str_2.patch`: implement new `pari_str` API. Patch taken from upstream master.
+
+- `plot_libpari.patch`: move plotting interface from gp to libPARI. Being discussed with upstream.
+
+- `plot_svg.patch`: implement SVG plotting libPARI. Applied upstream with significant changes.
+
+The `pari_jupyter` package also needs to be updated to support SVG plotting:
+
+**Tarball**: https://pypi.python.org/packages/6d/a7/9186eeacf76f8b723915b9c745ab42a73b987e5b5296fdd4a0a7c6626800/pari_jupyter-1.2.2.tar.bz2
``````




---

archive/issue_comments_410885.json:
```json
{
    "body": "<a id='comment:6'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                      |\n|-------------------------------------------------------------------------------------------------------------------------------------------|----------------------|\n|[7df14aa](https://git.sagemath.org/sage.git/commit/?id=7df14aa9f0cd32a43f2e317c8fe43b4336ec48f0)|`Upgrade pari_jupyter`|",
    "created_at": "2017-02-13T14:32:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22177#issuecomment-410885",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:6'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                      |
|-------------------------------------------------------------------------------------------------------------------------------------------|----------------------|
|[7df14aa](https://git.sagemath.org/sage.git/commit/?id=7df14aa9f0cd32a43f2e317c8fe43b4336ec48f0)|`Upgrade pari_jupyter`|



---

archive/issue_comments_410886.json:
```json
{
    "body": "Changing commit from \"72c15e45423801e8a44a3b52fb072d0535a86aa0\" to \"7df14aa9f0cd32a43f2e317c8fe43b4336ec48f0\"",
    "created_at": "2017-02-13T14:32:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22177#issuecomment-410886",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "72c15e45423801e8a44a3b52fb072d0535a86aa0" to "7df14aa9f0cd32a43f2e317c8fe43b4336ec48f0"



---

archive/issue_comments_410887.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2017-02-13T14:36:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22177#issuecomment-410887",
    "user": "https://github.com/jdemeyer"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_410888.json:
```json
{
    "body": "<a id='comment:8'></a>\nnot sure I am really qualified to review this; could you at least provide a jupyter notebook with the graphics in question, to test if it works?",
    "created_at": "2017-02-14T05:45:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22177#issuecomment-410888",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:8'></a>
not sure I am really qualified to review this; could you at least provide a jupyter notebook with the graphics in question, to test if it works?



---

archive/issue_comments_410889.json:
```json
{
    "body": "Changing upstream from \"N/A\" to \"Reported upstream. Developers acknowledge bug.\"",
    "created_at": "2017-02-14T06:58:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22177#issuecomment-410889",
    "user": "https://github.com/jdemeyer"
}
```

Changing upstream from "N/A" to "Reported upstream. Developers acknowledge bug."



---

archive/issue_comments_410890.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,10 +1,10 @@\n-This adds various patches to upstream PARI:\n+This requires various patches to upstream PARI:\n \n - `pari_str_1.patch` and `pari_str_2.patch`: implement new `pari_str` API. Patch taken from upstream master.\n \n - `plot_libpari.patch`: move plotting interface from gp to libPARI. Being discussed with upstream.\n \n-- `plot_svg.patch`: implement SVG plotting libPARI. Applied upstream with significant changes.\n+- `plot_svg.patch`: implement SVG plotting in libPARI. Applied upstream with significant changes.\n \n The `pari_jupyter` package also needs to be updated to support SVG plotting:\n \n``````\n",
    "created_at": "2017-02-14T06:58:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22177#issuecomment-410890",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,10 +1,10 @@
-This adds various patches to upstream PARI:
+This requires various patches to upstream PARI:
 
 - `pari_str_1.patch` and `pari_str_2.patch`: implement new `pari_str` API. Patch taken from upstream master.
 
 - `plot_libpari.patch`: move plotting interface from gp to libPARI. Being discussed with upstream.
 
-- `plot_svg.patch`: implement SVG plotting libPARI. Applied upstream with significant changes.
+- `plot_svg.patch`: implement SVG plotting in libPARI. Applied upstream with significant changes.
 
 The `pari_jupyter` package also needs to be updated to support SVG plotting:
 
``````




---

archive/issue_comments_410891.json:
```json
{
    "body": "<a id='comment:10'></a>\nReplying to [dimpase](#comment%3A8):\n> not sure I am really qualified to review this; could you at least provide a jupyter notebook with the graphics in question, to test if it works?\n\n\nSee https://github.com/OpenDreamKit/OpenDreamKit/blob/master/WP4/D4.4/demo-pari.ipynb",
    "created_at": "2017-02-21T11:02:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22177#issuecomment-410891",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:10'></a>
Replying to [dimpase](#comment%3A8):
> not sure I am really qualified to review this; could you at least provide a jupyter notebook with the graphics in question, to test if it works?


See https://github.com/OpenDreamKit/OpenDreamKit/blob/master/WP4/D4.4/demo-pari.ipynb



---

archive/issue_comments_410892.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -8,4 +8,6 @@\n \n The `pari_jupyter` package also needs to be updated to support SVG plotting:\n \n+**Demo**: https://github.com/OpenDreamKit/OpenDreamKit/blob/master/WP4/D4.4/demo-pari.ipynb\n+\n **Tarball**: https://pypi.python.org/packages/6d/a7/9186eeacf76f8b723915b9c745ab42a73b987e5b5296fdd4a0a7c6626800/pari_jupyter-1.2.2.tar.bz2\n``````\n",
    "created_at": "2017-02-21T11:02:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22177#issuecomment-410892",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -8,4 +8,6 @@
 
 The `pari_jupyter` package also needs to be updated to support SVG plotting:
 
+**Demo**: https://github.com/OpenDreamKit/OpenDreamKit/blob/master/WP4/D4.4/demo-pari.ipynb
+
 **Tarball**: https://pypi.python.org/packages/6d/a7/9186eeacf76f8b723915b9c745ab42a73b987e5b5296fdd4a0a7c6626800/pari_jupyter-1.2.2.tar.bz2
``````




---

archive/issue_comments_410893.json:
```json
{
    "body": "<a id='comment:12'></a>\nthe notebook looks good, besides `ellinit(\"10a1\")` needs some extra data I don't have installed\n\n```\n\n  ***   at top-level: ellinit(\"10a1\")\n  ***                 ^---------------\n  *** ellinit: error opening elldata file: `/home/dima/Sage/sage-dev/local/share/pari/elldata/ell0'.\n```\n(well, this is not relevant to this ticket).\n\nTonight I'll see how this goes on OSX, and hopefully it's good to go then.",
    "created_at": "2017-02-21T15:14:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22177#issuecomment-410893",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:12'></a>
the notebook looks good, besides `ellinit("10a1")` needs some extra data I don't have installed

```

  ***   at top-level: ellinit("10a1")
  ***                 ^---------------
  *** ellinit: error opening elldata file: `/home/dima/Sage/sage-dev/local/share/pari/elldata/ell0'.
```
(well, this is not relevant to this ticket).

Tonight I'll see how this goes on OSX, and hopefully it's good to go then.



---

archive/issue_comments_410894.json:
```json
{
    "body": "<a id='comment:13'></a>\nReplying to [dimpase](#comment%3A12):\n> the notebook looks good, besides `ellinit(\"10a1\")` needs some extra data I don't have installed\n\n\nAs documented in the demo notebook, that's on purpose, to show that error handling works fine. There is no curve called `10a1`, so installing a database won't help.",
    "created_at": "2017-02-21T21:09:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22177#issuecomment-410894",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:13'></a>
Replying to [dimpase](#comment%3A12):
> the notebook looks good, besides `ellinit("10a1")` needs some extra data I don't have installed


As documented in the demo notebook, that's on purpose, to show that error handling works fine. There is no curve called `10a1`, so installing a database won't help.



---

archive/issue_comments_410895.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2017-02-21T23:44:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22177#issuecomment-410895",
    "user": "https://github.com/dimpase"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_410896.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Dima Pasechnik\"",
    "created_at": "2017-02-21T23:44:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22177#issuecomment-410896",
    "user": "https://github.com/dimpase"
}
```

Changing reviewer from "" to "Dima Pasechnik"



---

archive/issue_comments_410897.json:
```json
{
    "body": "<a id='comment:14'></a>\nlooks good on OSX too.",
    "created_at": "2017-02-21T23:44:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22177#issuecomment-410897",
    "user": "https://github.com/dimpase"
}
```

<a id='comment:14'></a>
looks good on OSX too.



---

archive/issue_events_058321.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-02-24T14:49:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/22177",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/22177#event-58321"
}
```



---

archive/issue_comments_410898.json:
```json
{
    "body": "Changing branch from \"u/jdemeyer/implement_svg_plotting_in_pari_jupyter\" to \"7df14aa9f0cd32a43f2e317c8fe43b4336ec48f0\"",
    "created_at": "2017-02-24T14:49:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22177#issuecomment-410898",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/jdemeyer/implement_svg_plotting_in_pari_jupyter" to "7df14aa9f0cd32a43f2e317c8fe43b4336ec48f0"



---

archive/issue_comments_410899.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2017-02-24T14:49:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22177#issuecomment-410899",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
