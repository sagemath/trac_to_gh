# Issue 22129: Remove gens argument from Group.__init__

archive/issues_021892.json:
```json
{
    "assignees": [],
    "body": "It only passes it to `Parent.__init__`, where it is not a valid keyword, and does not seem to be used anywhere.\n\nCC:  @pjbruin\n\nBranch/Commit: **[53422d9](https://github.com/sagemath/sagetrac-mirror/commit/53422d901edfbe525bf1345650042e1806d3e9b7)**\n\nReviewer: **Peter Bruin**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/22129_\n\n",
    "closed_at": "2017-01-25T22:40:38Z",
    "created_at": "2017-01-03T13:14:52Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20group%20theory",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove gens argument from Group.__init__",
    "type": "issue",
    "updated_at": "2017-01-25T22:40:38Z",
    "url": "https://github.com/sagemath/sage/issues/22129",
    "user": "https://github.com/tscrim"
}
```
It only passes it to `Parent.__init__`, where it is not a valid keyword, and does not seem to be used anywhere.

CC:  @pjbruin

Branch/Commit: **[53422d9](https://github.com/sagemath/sagetrac-mirror/commit/53422d901edfbe525bf1345650042e1806d3e9b7)**

Reviewer: **Peter Bruin**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/22129_





---

archive/issue_events_291904.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-01-03T13:14:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/22129",
    "milestone_number": null,
    "milestone_title": "sage-7.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22129#event-291904"
}
```



---

archive/issue_events_291905.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-01-03T13:14:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22129",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20group%20theory",
    "label_color": "0000ff",
    "label_name": "component: group theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22129#event-291905"
}
```



---

archive/issue_events_291906.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-01-03T13:14:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22129",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22129#event-291906"
}
```



---

archive/issue_events_291907.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-01-03T13:14:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22129",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22129#event-291907"
}
```



---

archive/issue_comments_329078.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/53422d901edfbe525bf1345650042e1806d3e9b7\">53422d9</a></td><td><code>Remove passing gens from Group and deprecate.</code></td></tr></table>\n",
    "created_at": "2017-01-03T13:17:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22129#issuecomment-329078",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/53422d901edfbe525bf1345650042e1806d3e9b7">53422d9</a></td><td><code>Remove passing gens from Group and deprecate.</code></td></tr></table>




---

archive/issue_comments_329079.json:
```json
{
    "body": "Commit: **[53422d9](https://github.com/sagemath/sagetrac-mirror/commit/53422d901edfbe525bf1345650042e1806d3e9b7)**",
    "created_at": "2017-01-03T13:17:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22129#issuecomment-329079",
    "user": "https://github.com/tscrim"
}
```

Commit: **[53422d9](https://github.com/sagemath/sagetrac-mirror/commit/53422d901edfbe525bf1345650042e1806d3e9b7)**



---

archive/issue_events_291908.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-01-03T13:17:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22129",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22129#event-291908"
}
```



---

archive/issue_comments_329080.json:
```json
{
    "body": "Branch: **[public/groups/remove_gens_input-22129](https://github.com/sagemath/sagetrac-mirror/tree/public/groups/remove_gens_input-22129)**",
    "created_at": "2017-01-03T13:17:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22129#issuecomment-329080",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/groups/remove_gens_input-22129](https://github.com/sagemath/sagetrac-mirror/tree/public/groups/remove_gens_input-22129)**



---

archive/issue_events_291909.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-01-03T13:17:59Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/22129",
    "title_is": "Remove gens argument from Group.__init__",
    "title_was": "Remove gens from Group",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22129#event-291909"
}
```



---

archive/issue_comments_329081.json:
```json
{
    "body": "Reviewer: **Peter Bruin**",
    "created_at": "2017-01-03T15:23:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22129#issuecomment-329081",
    "user": "https://github.com/pjbruin"
}
```

Reviewer: **Peter Bruin**



---

archive/issue_comments_329082.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nLooks good and passes doctests.",
    "created_at": "2017-01-03T15:23:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22129#issuecomment-329082",
    "user": "https://github.com/pjbruin"
}
```

<div id="comment:4" align="right">Comment 4</div>

Looks good and passes doctests.



---

archive/issue_events_291910.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2017-01-03T15:23:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/22129",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22129#event-291910"
}
```



---

archive/issue_events_291911.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2017-01-03T15:23:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22129",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22129#event-291911"
}
```



---

archive/issue_comments_329083.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThank you.",
    "created_at": "2017-01-03T15:23:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22129#issuecomment-329083",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:5" align="right">Comment 5</div>

Thank you.



---

archive/issue_events_291912.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-01-25T22:40:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/22129",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22129#event-291912"
}
```



---

archive/issue_events_291913.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "bcfc6d95ca47be11454f4f07035160d95c02930b",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-01-25T22:40:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/22129",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22129#event-291913"
}
```



---

archive/issue_comments_329084.json:
```json
{
    "body": "Changed branch from **[public/groups/remove_gens_input-22129](https://github.com/sagemath/sagetrac-mirror/tree/public/groups/remove_gens_input-22129)** to **[53422d9](https://github.com/sagemath/sagetrac-mirror/commit/53422d901edfbe525bf1345650042e1806d3e9b7)**",
    "created_at": "2017-01-25T22:40:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22129",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22129#issuecomment-329084",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/groups/remove_gens_input-22129](https://github.com/sagemath/sagetrac-mirror/tree/public/groups/remove_gens_input-22129)** to **[53422d9](https://github.com/sagemath/sagetrac-mirror/commit/53422d901edfbe525bf1345650042e1806d3e9b7)**
