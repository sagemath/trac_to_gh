# Issue 22435: Random build failure in pynac

archive/issues_022198.json:
```json
{
    "body": "```\nMaking install in ginac\nmake[4]: Entering directory `/home/buildbot/slave/sage_git/build/local/var/tmp/sage/build/pynac-0.7.4/src/ginac'\nmake[5]: Entering directory `/home/buildbot/slave/sage_git/build/local/var/tmp/sage/build/pynac-0.7.4/src/ginac'\n /bin/mkdir -p '/home/buildbot/slave/sage_git/build/local/include/pynac'\n/bin/sh: 3:   if test -f $p; then \\: not found\n/bin/sh: 4:     list2=\"$list2 $p\"; \\: not found\n/bin/sh: 5:   else :; fi; \\: not found\n/bin/sh: 6: done; \\: not found\n/bin/sh: 7: test -z \"$list2\" || { \\: not found\n/bin/sh: 8:   echo \" /bin/mkdir -p '/home/buildbot/slave/sage_git/build/local/lib'\"; \\: not found\n/bin/sh: 9:   /bin/mkdir -p \"/home/buildbot/slave/sage_git/build/local/lib\" || exit 1; \\: not found\n/bin/sh: 10:   echo \" /bin/bash ../libtool   --mode=install /usr/bin/install -c  $list2 '/home/buildbot/slave/sage_git/build/local/lib'\"; \\: not found\n/bin/sh: 11:   /bin/bash ../libtool   --mode=install /usr/bin/install -c  $list2 \"/home/buildbot/slave/sage_git/build/local/lib\"; \\: not found\n/bin/sh: 12: }: not found\nmake[5]: *** [install-libLTLIBRARIES] Error 127\nmake[5]: Target `install-exec-am' not remade because of errors.\n /usr/bin/install -c -m 644 ginac.h py_funcs.h add.h archive.h assertion.h basic.h class_info.h clifford.h constant.h infinity.h container.h ex.h expair.h expairseq.h exprseq.h fail.h fderivative.h flags.h function.h idx.h indexed.h inifcns.h integral.h lst.h matrix.h mpoly.h mul.h ncmul.h normal.h numeric.h operators.h power.h print.h pseries.h ptr.h registrar.h relational.h extern_templates.h symbol.h symmetry.h '/home/buildbot/slave/sage_git/build/local/include/pynac'\n /usr/bin/install -c -m 644 tensor.h version.h wildcard.h order.h templates.h infoflagbase.h assume.h upoly.h useries.h useries-flint.h sum.h '/home/buildbot/slave/sage_git/build/local/include/pynac'\nmake[5]: Leaving directory `/home/buildbot/slave/sage_git/build/local/var/tmp/sage/build/pynac-0.7.4/src/ginac'\nmake[4]: *** [install-am] Error 2\nmake[4]: Target `install' not remade because of errors.\n```\n\nCC:  @rwst @slel\n\nKeywords: random_fail\n\nReviewer: Samuel Leli\u00e8vre\n\nResolution: invalid\n\nIssue created by migration from https://trac.sagemath.org/ticket/22435\n\n",
    "closed_at": "2021-08-26T02:08:43Z",
    "created_at": "2017-02-24T14:46:49Z",
    "labels": [
        "component: symbolics",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "Random build failure in pynac",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/22435",
    "user": "https://github.com/vbraun"
}
```
```
Making install in ginac
make[4]: Entering directory `/home/buildbot/slave/sage_git/build/local/var/tmp/sage/build/pynac-0.7.4/src/ginac'
make[5]: Entering directory `/home/buildbot/slave/sage_git/build/local/var/tmp/sage/build/pynac-0.7.4/src/ginac'
 /bin/mkdir -p '/home/buildbot/slave/sage_git/build/local/include/pynac'
/bin/sh: 3:   if test -f $p; then \: not found
/bin/sh: 4:     list2="$list2 $p"; \: not found
/bin/sh: 5:   else :; fi; \: not found
/bin/sh: 6: done; \: not found
/bin/sh: 7: test -z "$list2" || { \: not found
/bin/sh: 8:   echo " /bin/mkdir -p '/home/buildbot/slave/sage_git/build/local/lib'"; \: not found
/bin/sh: 9:   /bin/mkdir -p "/home/buildbot/slave/sage_git/build/local/lib" || exit 1; \: not found
/bin/sh: 10:   echo " /bin/bash ../libtool   --mode=install /usr/bin/install -c  $list2 '/home/buildbot/slave/sage_git/build/local/lib'"; \: not found
/bin/sh: 11:   /bin/bash ../libtool   --mode=install /usr/bin/install -c  $list2 "/home/buildbot/slave/sage_git/build/local/lib"; \: not found
/bin/sh: 12: }: not found
make[5]: *** [install-libLTLIBRARIES] Error 127
make[5]: Target `install-exec-am' not remade because of errors.
 /usr/bin/install -c -m 644 ginac.h py_funcs.h add.h archive.h assertion.h basic.h class_info.h clifford.h constant.h infinity.h container.h ex.h expair.h expairseq.h exprseq.h fail.h fderivative.h flags.h function.h idx.h indexed.h inifcns.h integral.h lst.h matrix.h mpoly.h mul.h ncmul.h normal.h numeric.h operators.h power.h print.h pseries.h ptr.h registrar.h relational.h extern_templates.h symbol.h symmetry.h '/home/buildbot/slave/sage_git/build/local/include/pynac'
 /usr/bin/install -c -m 644 tensor.h version.h wildcard.h order.h templates.h infoflagbase.h assume.h upoly.h useries.h useries-flint.h sum.h '/home/buildbot/slave/sage_git/build/local/include/pynac'
make[5]: Leaving directory `/home/buildbot/slave/sage_git/build/local/var/tmp/sage/build/pynac-0.7.4/src/ginac'
make[4]: *** [install-am] Error 2
make[4]: Target `install' not remade because of errors.
```

CC:  @rwst @slel

Keywords: random_fail

Reviewer: Samuel Lelièvre

Resolution: invalid

Issue created by migration from https://trac.sagemath.org/ticket/22435





---

archive/issue_comments_416342.json:
```json
{
    "body": "Changing keywords from \"\" to \"random_fail\".",
    "created_at": "2017-02-24T14:46:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22435#issuecomment-416342",
    "user": "https://github.com/vbraun"
}
```

Changing keywords from "" to "random_fail".



---

archive/issue_comments_416343.json:
```json
{
    "body": "<a id='comment:2'></a>The lines with `: not found` at the end are where in a normal install I can see this line: ` /bin/sh ../libtool   --mode=install /usr/bin/install -c   libpynac.la '/home/ralf/sage/local/lib'`, so presumably this has something to do with `libtool`?",
    "created_at": "2017-02-25T06:48:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22435#issuecomment-416343",
    "user": "https://github.com/rwst"
}
```

<a id='comment:2'></a>The lines with `: not found` at the end are where in a normal install I can see this line: ` /bin/sh ../libtool   --mode=install /usr/bin/install -c   libpynac.la '/home/ralf/sage/local/lib'`, so presumably this has something to do with `libtool`?



---

archive/issue_comments_416344.json:
```json
{
    "body": "<a id='comment:3'></a>I think the `: not found` comes from the `find` command getting its argument(s) from a backtick expansion, but that the expansion does not yield file names but lines of a script. The script looks like the content of the `install-libLTLIBRARIES` target code starting from its 3rd line. Something like this should not happen(tm), and even more, not sporadically.",
    "created_at": "2017-02-25T07:12:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22435#issuecomment-416344",
    "user": "https://github.com/rwst"
}
```

<a id='comment:3'></a>I think the `: not found` comes from the `find` command getting its argument(s) from a backtick expansion, but that the expansion does not yield file names but lines of a script. The script looks like the content of the `install-libLTLIBRARIES` target code starting from its 3rd line. Something like this should not happen(tm), and even more, not sporadically.



---

archive/issue_comments_416345.json:
```json
{
    "body": "<a id='comment:4'></a>In summary I think this is a rare problem with GNU make. At least there were no recent Pynac changes that can even potentially trigger such behaviour.",
    "created_at": "2017-02-25T07:15:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22435#issuecomment-416345",
    "user": "https://github.com/rwst"
}
```

<a id='comment:4'></a>In summary I think this is a rare problem with GNU make. At least there were no recent Pynac changes that can even potentially trigger such behaviour.



---

archive/issue_comments_416346.json:
```json
{
    "body": "<a id='comment:5'></a>Does this happen on one particular system or several systems?\n\nIt seems to me indeed a strange problem with either `make` or `sh`. Somehow the shell commands are totally misparsed. First of all, since we get an error message for every individual line, the backslash-newline escaping doesn't work. Then it also seems like the whole line is interpreted as 1 command: the shell is literally trying to run the single program `if test -f $p; then \\` instead of parsing it as code.",
    "created_at": "2017-02-25T07:21:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22435#issuecomment-416346",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:5'></a>Does this happen on one particular system or several systems?

It seems to me indeed a strange problem with either `make` or `sh`. Somehow the shell commands are totally misparsed. First of all, since we get an error message for every individual line, the backslash-newline escaping doesn't work. Then it also seems like the whole line is interpreted as 1 command: the shell is literally trying to run the single program `if test -f $p; then \` instead of parsing it as code.



---

archive/issue_comments_416347.json:
```json
{
    "body": "<a id='comment:6'></a>So far I've only observed it once.\n\nWhat about the \n\n```\nmake[4]: Entering directory `/home/buildbot/slave/sage_git/build/local/var/tmp/sage/build/pynac-0.7.4/src/ginac'\nmake[5]: Entering directory `/home/buildbot/slave/sage_git/build/local/var/tmp/sage/build/pynac-0.7.4/src/ginac'\n```\nIt seems that there are two parallel make processes in the ginac subdirectory; If one is writing the install-libLTLIBRARIES script while the other is executing it the observed error  could easily happen.",
    "created_at": "2017-02-25T12:25:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22435#issuecomment-416347",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:6'></a>So far I've only observed it once.

What about the 

```
make[4]: Entering directory `/home/buildbot/slave/sage_git/build/local/var/tmp/sage/build/pynac-0.7.4/src/ginac'
make[5]: Entering directory `/home/buildbot/slave/sage_git/build/local/var/tmp/sage/build/pynac-0.7.4/src/ginac'
```
It seems that there are two parallel make processes in the ginac subdirectory; If one is writing the install-libLTLIBRARIES script while the other is executing it the observed error  could easily happen.



---

archive/issue_comments_416348.json:
```json
{
    "body": "<a id='comment:7'></a>What normally seems to happen is that make \"puts a child on the chain\" to remake the target (output snippet from `make -d`):\n\n```\n   Finished prerequisites of target file 'install-libLTLIBRARIES'.\n  Must remake target 'install-libLTLIBRARIES'.\nmake[2]: Entering directory '/home/ralf/sage/pynac/ginac'\nPutting child 0x23c1f50 (install-libLTLIBRARIES) PID 29581 on the chain.\nLive child 0x23c1f50 (install-libLTLIBRARIES) PID 29581 \n /usr/bin/mkdir -p '/usr/local/lib64'\n /bin/sh ../libtool   --mode=install /usr/bin/install -c   libpynac.la '/usr/local/lib64'\n```",
    "created_at": "2017-02-26T07:18:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22435#issuecomment-416348",
    "user": "https://github.com/rwst"
}
```

<a id='comment:7'></a>What normally seems to happen is that make "puts a child on the chain" to remake the target (output snippet from `make -d`):

```
   Finished prerequisites of target file 'install-libLTLIBRARIES'.
  Must remake target 'install-libLTLIBRARIES'.
make[2]: Entering directory '/home/ralf/sage/pynac/ginac'
Putting child 0x23c1f50 (install-libLTLIBRARIES) PID 29581 on the chain.
Live child 0x23c1f50 (install-libLTLIBRARIES) PID 29581 
 /usr/bin/mkdir -p '/usr/local/lib64'
 /bin/sh ../libtool   --mode=install /usr/bin/install -c   libpynac.la '/usr/local/lib64'
```



---

archive/issue_comments_416349.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2021-08-16T22:03:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22435#issuecomment-416349",
    "user": "https://github.com/mkoeppe"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_416350.json:
```json
{
    "body": "<a id='comment:8'></a>Outdated, should close",
    "created_at": "2021-08-16T22:03:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22435#issuecomment-416350",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:8'></a>Outdated, should close



---

archive/issue_events_058697.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-16T22:03:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/22435",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/22435#event-58697"
}
```



---

archive/issue_comments_416351.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2021-08-19T22:46:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22435#issuecomment-416351",
    "user": "https://github.com/slel"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_416352.json:
```json
{
    "body": "<a id='comment:9'></a>Let us close this.",
    "created_at": "2021-08-19T22:46:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22435#issuecomment-416352",
    "user": "https://github.com/slel"
}
```

<a id='comment:9'></a>Let us close this.



---

archive/issue_comments_416353.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Samuel Leli\u00e8vre\"",
    "created_at": "2021-08-19T22:46:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22435#issuecomment-416353",
    "user": "https://github.com/slel"
}
```

Changing reviewer from "" to "Samuel Lelièvre"



---

archive/issue_comments_416354.json:
```json
{
    "body": "Resolution: invalid",
    "created_at": "2021-08-26T02:08:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22435",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22435#issuecomment-416354",
    "user": "https://github.com/mkoeppe"
}
```

Resolution: invalid



---

archive/issue_events_058698.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-26T02:08:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/22435",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/22435#event-58698"
}
```
