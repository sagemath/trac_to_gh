# Issue 22423: Random failure in sagenb/storage/filesystem_storage.py

archive/issues_022186.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage -t --long local/lib/python2.7/site-packages/sagenb/storage/filesystem_storage.py\n**********************************************************************\nFile \"local/lib/python2.7/site-packages/sagenb/storage/filesystem_storage.py\", line 183, in sagenb.storage.filesystem_storage.FilesystemDatastore._save\nFailed example:\n    try:  # long time\n        alarm(1)\n        while True:\n            D._save(s, fn)\n    except (AlarmInterrupt, OSError):\n        # OSError could happen due to a double close() in\n        # Python's tempfile module.\n        pass\nExpected nothing\nGot:\n    Exception AttributeError: \"'file' object has no attribute 'close_called'\" in <bound method _TemporaryFileWrapper.__del__ of <open file '<fdopen>', mode 'w+b' at 0x272406c90>> ignored\n    close failed in file object destructor:\n    IOError: [Errno 9] Bad file descriptor\n**********************************************************************\n1 item had failures:\n   1 of   8 in sagenb.storage.filesystem_storage.FilesystemDatastore._save\n    [46 tests, 1 failure, 2.07 s]\n```\nClearly the signal arrived during the TemporaryFile constructor...\n\nKeywords: **random_fail**\n\nAuthor: **Jeroen Demeyer**\n\nComponent: **notebook**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/22423_\n\n",
    "closed_at": "2017-03-02T17:00:48Z",
    "created_at": "2017-02-23T11:49:16Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Random failure in sagenb/storage/filesystem_storage.py",
    "type": "issue",
    "updated_at": "2017-03-02T17:00:48Z",
    "url": "https://github.com/sagemath/sage/issues/22423",
    "user": "https://github.com/vbraun"
}
```

```
sage -t --long local/lib/python2.7/site-packages/sagenb/storage/filesystem_storage.py
**********************************************************************
File "local/lib/python2.7/site-packages/sagenb/storage/filesystem_storage.py", line 183, in sagenb.storage.filesystem_storage.FilesystemDatastore._save
Failed example:
    try:  # long time
        alarm(1)
        while True:
            D._save(s, fn)
    except (AlarmInterrupt, OSError):
        # OSError could happen due to a double close() in
        # Python's tempfile module.
        pass
Expected nothing
Got:
    Exception AttributeError: "'file' object has no attribute 'close_called'" in <bound method _TemporaryFileWrapper.__del__ of <open file '<fdopen>', mode 'w+b' at 0x272406c90>> ignored
    close failed in file object destructor:
    IOError: [Errno 9] Bad file descriptor
**********************************************************************
1 item had failures:
   1 of   8 in sagenb.storage.filesystem_storage.FilesystemDatastore._save
    [46 tests, 1 failure, 2.07 s]
```
Clearly the signal arrived during the TemporaryFile constructor...

Keywords: **random_fail**

Author: **Jeroen Demeyer**

Component: **notebook**

_Issue created by migration from https://trac.sagemath.org/ticket/22423_





---

archive/issue_events_313473.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-02-23T11:49:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/22423",
    "milestone_number": null,
    "milestone_title": "sage-7.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22423#event-313473"
}
```



---

archive/issue_events_313474.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-02-23T11:49:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22423",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "c: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22423#event-313474"
}
```



---

archive/issue_events_313475.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-02-23T11:49:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22423",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22423#event-313475"
}
```



---

archive/issue_events_313476.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-02-23T11:49:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22423",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22423#event-313476"
}
```



---

archive/issue_comments_331833.json:
```json
{
    "body": "Author: **Jeroen Demeyer**",
    "created_at": "2017-03-02T17:00:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22423",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22423#issuecomment-331833",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Jeroen Demeyer**



---

archive/issue_events_313477.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-03-02T17:00:48Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/22423",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22423#event-313477"
}
```



---

archive/issue_comments_331834.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nSee https://github.com/sagemath/sagenb/commit/f987c9c0e1003916bb37a7791936a14bfe75beff",
    "created_at": "2017-03-02T17:00:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22423",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22423#issuecomment-331834",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:1" align="right">comment:1</div>

See https://github.com/sagemath/sagenb/commit/f987c9c0e1003916bb37a7791936a14bfe75beff



---

archive/issue_events_313478.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-03-02T17:00:48Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/22423",
    "milestone_number": null,
    "milestone_title": "sage-7.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22423#event-313478"
}
```



---

archive/issue_events_313479.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-03-02T17:00:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/22423",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22423#event-313479"
}
```



---

archive/issue_events_313480.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-03-02T17:00:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22423",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22423#event-313480"
}
```
