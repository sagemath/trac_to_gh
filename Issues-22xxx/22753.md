# Issue 22753: Don't declare functions/methods as "cdef inline"

archive/issues_022516.json:
```json
{
    "assignees": [],
    "body": "Declaring (as opposed to implementing) functions/methods as inline makes no sense. Cython currently allows it, but it is known to have issues (including generating invalid C code and breaking with Cython updates).\n\nBranch/Commit: **[94302ca](https://github.com/sagemath/sagetrac-mirror/commit/94302cabcce3c932af9660f709911fd20dab12e5)**\n\nReviewer: **Vincent Delecroix**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/22753_\n\n",
    "closed_at": "2017-04-07T22:23:46Z",
    "created_at": "2017-04-04T13:25:25Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20cython",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Don't declare functions/methods as \"cdef inline\"",
    "type": "issue",
    "updated_at": "2017-04-07T22:23:46Z",
    "url": "https://github.com/sagemath/sage/issues/22753",
    "user": "https://github.com/jdemeyer"
}
```
Declaring (as opposed to implementing) functions/methods as inline makes no sense. Cython currently allows it, but it is known to have issues (including generating invalid C code and breaking with Cython updates).

Branch/Commit: **[94302ca](https://github.com/sagemath/sagetrac-mirror/commit/94302cabcce3c932af9660f709911fd20dab12e5)**

Reviewer: **Vincent Delecroix**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/22753_





---

archive/issue_events_299641.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-04T13:25:25Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/22753",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22753#event-299641"
}
```



---

archive/issue_events_299642.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-04T13:25:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22753",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20cython",
    "label_color": "000080",
    "label_name": "component: cython",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22753#event-299642"
}
```



---

archive/issue_events_299643.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-04T13:25:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22753",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22753#event-299643"
}
```



---

archive/issue_events_299644.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-04T13:25:25Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22753",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22753#event-299644"
}
```



---

archive/issue_comments_341338.json:
```json
{
    "body": "Branch: **[u/jdemeyer/don_t_declare_functions_methods_as__cdef_inline_](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/don_t_declare_functions_methods_as__cdef_inline_)**",
    "created_at": "2017-04-04T13:26:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22753",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22753#issuecomment-341338",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/don_t_declare_functions_methods_as__cdef_inline_](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/don_t_declare_functions_methods_as__cdef_inline_)**



---

archive/issue_comments_341339.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/94302cabcce3c932af9660f709911fd20dab12e5\">94302ca</a></td><td><code>Do not declare functions/methods as \"cdef inline\"</code></td></tr></table>\n",
    "created_at": "2017-04-04T13:26:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22753",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22753#issuecomment-341339",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/94302cabcce3c932af9660f709911fd20dab12e5">94302ca</a></td><td><code>Do not declare functions/methods as "cdef inline"</code></td></tr></table>




---

archive/issue_events_299645.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-04T13:26:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22753",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22753#event-299645"
}
```



---

archive/issue_comments_341340.json:
```json
{
    "body": "Commit: **[94302ca](https://github.com/sagemath/sagetrac-mirror/commit/94302cabcce3c932af9660f709911fd20dab12e5)**",
    "created_at": "2017-04-04T13:26:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22753",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22753#issuecomment-341340",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[94302ca](https://github.com/sagemath/sagetrac-mirror/commit/94302cabcce3c932af9660f709911fd20dab12e5)**



---

archive/issue_events_299646.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2017-04-04T21:32:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/22753",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22753#event-299646"
}
```



---

archive/issue_events_299647.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2017-04-04T21:32:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22753",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22753#event-299647"
}
```



---

archive/issue_comments_341341.json:
```json
{
    "body": "Reviewer: **Vincent Delecroix**",
    "created_at": "2017-04-04T21:32:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22753",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22753#issuecomment-341341",
    "user": "https://github.com/videlec"
}
```

Reviewer: **Vincent Delecroix**



---

archive/issue_events_299648.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-04-07T22:23:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/22753",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22753#event-299648"
}
```



---

archive/issue_events_299649.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "10286a67d4102cd1cdb5226cf7edda49b166fa22",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-04-07T22:23:46Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/22753",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22753#event-299649"
}
```



---

archive/issue_comments_341342.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/don_t_declare_functions_methods_as__cdef_inline_](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/don_t_declare_functions_methods_as__cdef_inline_)** to **[94302ca](https://github.com/sagemath/sagetrac-mirror/commit/94302cabcce3c932af9660f709911fd20dab12e5)**",
    "created_at": "2017-04-07T22:23:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22753",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22753#issuecomment-341342",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/don_t_declare_functions_methods_as__cdef_inline_](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/don_t_declare_functions_methods_as__cdef_inline_)** to **[94302ca](https://github.com/sagemath/sagetrac-mirror/commit/94302cabcce3c932af9660f709911fd20dab12e5)**
