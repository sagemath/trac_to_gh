# Issue 22780: Hecke operators on spaces of modular forms of level 1 are wrong

archive/issues_022543.json:
```json
{
    "assignees": [],
    "body": "The following bug was reported to me by Sander Dahmen and Joey van Langen:\n\n```\nsage: M = ModularForms(SL2Z, 12)\nsage: Delta = M.basis()[0]; Delta\nq - 24*q^2 + 252*q^3 - 1472*q^4 + 4830*q^5 + O(q^6)\nsage: T2 = M.hecke_operator(2)\nsage: T2(Delta) == -24*Delta\nFalse\n```\nThis happens because (apparently since #9749) the Hecke matrix is computed with respect to a different basis than `M.q_expansion_basis()`:\n\n```\nsage: M._compute_hecke_matrix(2)\n[  2049 196560]\n[     0    -24]\n```\nThe correct answer is\n\n```\n[ -24    0]\n[   0 2049]\n```\nCompare the following:\n\n```\nsage: M.q_expansion_basis()\n[\nq - 24*q^2 + 252*q^3 - 1472*q^4 + 4830*q^5 + O(q^6),\n1 + 65520/691*q + 134250480/691*q^2 + 11606736960/691*q^3 + 274945048560/691*q^4 + 3199218815520/691*q^5 + O(q^6)\n]\nsage: hecke_operator_on_basis(M.q_expansion_basis(), 2, 12)\n[ -24    0]\n[   0 2049]\n```\nvs.\n\n```\nsage: victor_miller_basis(12)\n[\n1 + 196560*q^2 + 16773120*q^3 + 398034000*q^4 + 4629381120*q^5 + 34417656000*q^6 + 187489935360*q^7 + 814879774800*q^8 + 2975551488000*q^9 + O(q^10),\nq - 24*q^2 + 252*q^3 - 1472*q^4 + 4830*q^5 - 6048*q^6 - 16744*q^7 + 84480*q^8 - 113643*q^9 + O(q^10)\n]\nsage: hecke_operator_on_basis(victor_miller_basis(12), 2, 12)\n[  2049 196560]\n[     0    -24]\n```\n\n\nCC:  @aghitza @williamstein\n\nKeywords: **hecke operator**\n\nBranch/Commit: **[c68af72](https://github.com/sagemath/sagetrac-mirror/commit/c68af72470940d22d359dc551e4cc832da1af082)**\n\nReviewer: **Peter Bruin**\n\nAuthor: **David Loeffler**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/22780_\n\n",
    "closed_at": "2017-12-11T01:03:52Z",
    "created_at": "2017-04-07T17:12:45Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20modular%20forms",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Hecke operators on spaces of modular forms of level 1 are wrong",
    "type": "issue",
    "updated_at": "2017-12-11T01:03:52Z",
    "url": "https://github.com/sagemath/sage/issues/22780",
    "user": "https://github.com/pjbruin"
}
```
The following bug was reported to me by Sander Dahmen and Joey van Langen:

```
sage: M = ModularForms(SL2Z, 12)
sage: Delta = M.basis()[0]; Delta
q - 24*q^2 + 252*q^3 - 1472*q^4 + 4830*q^5 + O(q^6)
sage: T2 = M.hecke_operator(2)
sage: T2(Delta) == -24*Delta
False
```
This happens because (apparently since #9749) the Hecke matrix is computed with respect to a different basis than `M.q_expansion_basis()`:

```
sage: M._compute_hecke_matrix(2)
[  2049 196560]
[     0    -24]
```
The correct answer is

```
[ -24    0]
[   0 2049]
```
Compare the following:

```
sage: M.q_expansion_basis()
[
q - 24*q^2 + 252*q^3 - 1472*q^4 + 4830*q^5 + O(q^6),
1 + 65520/691*q + 134250480/691*q^2 + 11606736960/691*q^3 + 274945048560/691*q^4 + 3199218815520/691*q^5 + O(q^6)
]
sage: hecke_operator_on_basis(M.q_expansion_basis(), 2, 12)
[ -24    0]
[   0 2049]
```
vs.

```
sage: victor_miller_basis(12)
[
1 + 196560*q^2 + 16773120*q^3 + 398034000*q^4 + 4629381120*q^5 + 34417656000*q^6 + 187489935360*q^7 + 814879774800*q^8 + 2975551488000*q^9 + O(q^10),
q - 24*q^2 + 252*q^3 - 1472*q^4 + 4830*q^5 - 6048*q^6 - 16744*q^7 + 84480*q^8 - 113643*q^9 + O(q^10)
]
sage: hecke_operator_on_basis(victor_miller_basis(12), 2, 12)
[  2049 196560]
[     0    -24]
```


CC:  @aghitza @williamstein

Keywords: **hecke operator**

Branch/Commit: **[c68af72](https://github.com/sagemath/sagetrac-mirror/commit/c68af72470940d22d359dc551e4cc832da1af082)**

Reviewer: **Peter Bruin**

Author: **David Loeffler**

_Issue created by migration from https://trac.sagemath.org/ticket/22780_





---

archive/issue_events_300037.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2017-04-07T17:12:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/22780",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22780#event-300037"
}
```



---

archive/issue_events_300038.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2017-04-07T17:12:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22780",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20modular%20forms",
    "label_color": "0000ff",
    "label_name": "component: modular forms",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22780#event-300038"
}
```



---

archive/issue_events_300039.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2017-04-07T17:12:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22780",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22780#event-300039"
}
```



---

archive/issue_events_300040.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2017-04-07T17:12:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22780",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22780#event-300040"
}
```



---

archive/issue_comments_341911.json:
```json
{
    "body": "Branch: **[public/22780](https://github.com/sagemath/sagetrac-mirror/tree/public/22780)**",
    "created_at": "2017-11-20T18:41:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22780#issuecomment-341911",
    "user": "https://github.com/loefflerd"
}
```

Branch: **[public/22780](https://github.com/sagemath/sagetrac-mirror/tree/public/22780)**



---

archive/issue_comments_341912.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2441d5a3d8d6b23af2fbcd25fb7ba0604dd0981e\">2441d5a</a></td><td><code>Trac 22780: hecke operators in level 1 use wrong basis</code></td></tr></table>\n",
    "created_at": "2017-11-20T18:42:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22780#issuecomment-341912",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2441d5a3d8d6b23af2fbcd25fb7ba0604dd0981e">2441d5a</a></td><td><code>Trac 22780: hecke operators in level 1 use wrong basis</code></td></tr></table>




---

archive/issue_comments_341913.json:
```json
{
    "body": "Commit: **[2441d5a](https://github.com/sagemath/sagetrac-mirror/commit/2441d5a3d8d6b23af2fbcd25fb7ba0604dd0981e)**",
    "created_at": "2017-11-20T18:42:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22780#issuecomment-341913",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[2441d5a](https://github.com/sagemath/sagetrac-mirror/commit/2441d5a3d8d6b23af2fbcd25fb7ba0604dd0981e)**



---

archive/issue_events_300041.json:
```json
{
    "actor": "https://github.com/loefflerd",
    "created_at": "2017-11-20T18:43:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22780",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22780#event-300041"
}
```



---

archive/issue_comments_341914.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nHere is a fix (which is also a tiny bit faster than the old code).",
    "created_at": "2017-11-20T18:43:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22780#issuecomment-341914",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:3" align="right">Comment 3</div>

Here is a fix (which is also a tiny bit faster than the old code).



---

archive/issue_comments_341915.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8bcd67431c978813f671e540aee4a71009af7266\">8bcd674</a></td><td><code>22780: handle 0-dimensional spaces correctly</code></td></tr></table>\n",
    "created_at": "2017-11-20T18:47:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22780#issuecomment-341915",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8bcd67431c978813f671e540aee4a71009af7266">8bcd674</a></td><td><code>22780: handle 0-dimensional spaces correctly</code></td></tr></table>




---

archive/issue_comments_341916.json:
```json
{
    "body": "Changed commit from **[2441d5a](https://github.com/sagemath/sagetrac-mirror/commit/2441d5a3d8d6b23af2fbcd25fb7ba0604dd0981e)** to **[8bcd674](https://github.com/sagemath/sagetrac-mirror/commit/8bcd67431c978813f671e540aee4a71009af7266)**",
    "created_at": "2017-11-20T18:47:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22780#issuecomment-341916",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[2441d5a](https://github.com/sagemath/sagetrac-mirror/commit/2441d5a3d8d6b23af2fbcd25fb7ba0604dd0981e)** to **[8bcd674](https://github.com/sagemath/sagetrac-mirror/commit/8bcd67431c978813f671e540aee4a71009af7266)**



---

archive/issue_comments_341917.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nOK, now zero-dimensional spaces should work as well.",
    "created_at": "2017-11-20T18:49:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22780#issuecomment-341917",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:5" align="right">Comment 5</div>

OK, now zero-dimensional spaces should work as well.



---

archive/issue_comments_341918.json:
```json
{
    "body": "Author: **David Loeffler**",
    "created_at": "2017-11-20T21:28:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22780#issuecomment-341918",
    "user": "https://github.com/loefflerd"
}
```

Author: **David Loeffler**



---

archive/issue_comments_341919.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c68af72470940d22d359dc551e4cc832da1af082\">c68af72</a></td><td><code>Trac 22780: fix failing doctests in sage/modular/hecke</code></td></tr></table>\n",
    "created_at": "2017-11-21T08:45:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22780#issuecomment-341919",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:7" align="right">Comment 7</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c68af72470940d22d359dc551e4cc832da1af082">c68af72</a></td><td><code>Trac 22780: fix failing doctests in sage/modular/hecke</code></td></tr></table>




---

archive/issue_comments_341920.json:
```json
{
    "body": "Changed commit from **[8bcd674](https://github.com/sagemath/sagetrac-mirror/commit/8bcd67431c978813f671e540aee4a71009af7266)** to **[c68af72](https://github.com/sagemath/sagetrac-mirror/commit/c68af72470940d22d359dc551e4cc832da1af082)**",
    "created_at": "2017-11-21T08:45:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22780#issuecomment-341920",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[8bcd674](https://github.com/sagemath/sagetrac-mirror/commit/8bcd67431c978813f671e540aee4a71009af7266)** to **[c68af72](https://github.com/sagemath/sagetrac-mirror/commit/c68af72470940d22d359dc551e4cc832da1af082)**



---

archive/issue_comments_341921.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nPatchbot picked up some failing doctests. I had run all tests in sage/modular/modform, but I didn't realise there were tests in sage/modular/hecke that also depend on this code.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c68af72470940d22d359dc551e4cc832da1af082\">c68af72</a></td><td><code>Trac 22780: fix failing doctests in sage/modular/hecke</code></td></tr></table>\n",
    "created_at": "2017-11-21T08:46:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22780#issuecomment-341921",
    "user": "https://github.com/loefflerd"
}
```

<div id="comment:8" align="right">Comment 8</div>

Patchbot picked up some failing doctests. I had run all tests in sage/modular/modform, but I didn't realise there were tests in sage/modular/hecke that also depend on this code.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c68af72470940d22d359dc551e4cc832da1af082">c68af72</a></td><td><code>Trac 22780: fix failing doctests in sage/modular/hecke</code></td></tr></table>




---

archive/issue_events_300042.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2017-11-22T08:03:47Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/22780",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22780#event-300042"
}
```



---

archive/issue_events_300043.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2017-11-22T08:03:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22780",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22780#event-300043"
}
```



---

archive/issue_comments_341922.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nLooks good to me and tests pass.  Thanks for fixing this!",
    "created_at": "2017-11-22T08:03:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22780#issuecomment-341922",
    "user": "https://github.com/pjbruin"
}
```

<div id="comment:9" align="right">Comment 9</div>

Looks good to me and tests pass.  Thanks for fixing this!



---

archive/issue_comments_341923.json:
```json
{
    "body": "Reviewer: **Peter Bruin**",
    "created_at": "2017-11-22T08:03:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22780#issuecomment-341923",
    "user": "https://github.com/pjbruin"
}
```

Reviewer: **Peter Bruin**



---

archive/issue_events_300044.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-11-30T14:49:26Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/22780",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22780#event-300044"
}
```



---

archive/issue_events_300045.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-11-30T14:49:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/22780",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22780#event-300045"
}
```



---

archive/issue_comments_341924.json:
```json
{
    "body": "Changed branch from **[public/22780](https://github.com/sagemath/sagetrac-mirror/tree/public/22780)** to **[c68af72](https://github.com/sagemath/sagetrac-mirror/commit/c68af72470940d22d359dc551e4cc832da1af082)**",
    "created_at": "2017-12-11T01:03:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22780",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22780#issuecomment-341924",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/22780](https://github.com/sagemath/sagetrac-mirror/tree/public/22780)** to **[c68af72](https://github.com/sagemath/sagetrac-mirror/commit/c68af72470940d22d359dc551e4cc832da1af082)**



---

archive/issue_events_300046.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-12-11T01:03:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/22780",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22780#event-300046"
}
```



---

archive/issue_events_300047.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "ae4af73bcdd46cdc2be228665adcc912176998c5",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-12-11T01:03:52Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/22780",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22780#event-300047"
}
```
