# Issue 22706: Add more conversions from giac

archive/issues_022469.json:
```json
{
    "body": "A \"conversion\" is a dictionary entry which maps object names in an interface (e.g. Maxima, Mathematica, Giac, ...) to corresponding objects with a different name in Sage.\n\nSome conversions relevant to the Giac interface are missing:\n\n- Derivatives of Dirac delta, as in `Dirac(t, 1)`.\n- Special function names at `/functions/orthogonal_polys.py`.\n\nSee also: \n\n- In the file: `local/share/giac/doc/aide_cas`, lines starting by # are keywords followed by synonyms, next lines are for small doc and see also. Full doc can be found in: [Symbolic algebra and Mathematics with Xcas](http://www-fourier.ujf-grenoble.fr/~parisse/giac/doc/en/cascmd_en/cascmd_en.html)\n\n- `sage.libs.pynac.pynac.register_symbol` can be used to register new symbols in the `symbols_table`, as in `register_symbol(heaviside,{'giac':'Heaviside'})`.\n\n- for `BuiltIn` functions which exist in giac with a different name, it is sufficient to update the `conversions` dictionary for the corresponding `__init__` method. \n\nCC:  @paulmasson @frederichan-IMJPRG\n\nKeywords: giac, special functions\n\nBranch/Commit: 350316fe0bc69cb8c1fdebbd3363d3437661f28f\n\nReviewer: Fr\u00e9d\u00e9ric Chapoton\n\nAuthor: Paul Masson, Marcelo Forets\n\nDependencies: #22422\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/22706\n\n",
    "closed_at": "2017-06-04T10:08:54Z",
    "created_at": "2017-03-29T08:19:56Z",
    "labels": [
        "component: interfaces"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.0",
    "title": "Add more conversions from giac",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/22706",
    "user": "https://github.com/mforets"
}
```
A "conversion" is a dictionary entry which maps object names in an interface (e.g. Maxima, Mathematica, Giac, ...) to corresponding objects with a different name in Sage.

Some conversions relevant to the Giac interface are missing:

- Derivatives of Dirac delta, as in `Dirac(t, 1)`.
- Special function names at `/functions/orthogonal_polys.py`.

See also: 

- In the file: `local/share/giac/doc/aide_cas`, lines starting by # are keywords followed by synonyms, next lines are for small doc and see also. Full doc can be found in: [Symbolic algebra and Mathematics with Xcas](http://www-fourier.ujf-grenoble.fr/~parisse/giac/doc/en/cascmd_en/cascmd_en.html)

- `sage.libs.pynac.pynac.register_symbol` can be used to register new symbols in the `symbols_table`, as in `register_symbol(heaviside,{'giac':'Heaviside'})`.

- for `BuiltIn` functions which exist in giac with a different name, it is sufficient to update the `conversions` dictionary for the corresponding `__init__` method. 

CC:  @paulmasson @frederichan-IMJPRG

Keywords: giac, special functions

Branch/Commit: 350316fe0bc69cb8c1fdebbd3363d3437661f28f

Reviewer: Frédéric Chapoton

Author: Paul Masson, Marcelo Forets

Dependencies: #22422

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/22706





---

archive/issue_comments_423189.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,7 +2,7 @@\n \n Some conversions relevant to the Giac interface are missing:\n \n-- Dirac delta with support different from 0, as in `Dirac(t, 1)`.\n+- Derivatives of Dirac delta, as in `Dirac(t, 1)`.\n - Special function names at `/functions/orthogonal_polys.py`.\n \n See also: \n``````\n",
    "created_at": "2017-03-29T08:27:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423189",
    "user": "https://github.com/mforets"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,7 +2,7 @@
 
 Some conversions relevant to the Giac interface are missing:
 
-- Dirac delta with support different from 0, as in `Dirac(t, 1)`.
+- Derivatives of Dirac delta, as in `Dirac(t, 1)`.
 - Special function names at `/functions/orthogonal_polys.py`.
 
 See also: 
``````




---

archive/issue_comments_423190.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -7,6 +7,6 @@\n \n See also: \n \n-- In the file: `local/share/giac/doc/aide_cas`, lines starting by # are keywords followed by synonyms, next lines are for small doc and see also. Full doc it is here: [Symbolic algebra and Mathematics with Xcas](http://www-fourier.ujf-grenoble.fr/~parisse/giac/doc/en/cascmd_en/cascmd_en.html)\n+- In the file: `local/share/giac/doc/aide_cas`, lines starting by # are keywords followed by synonyms, next lines are for small doc and see also. Full doc can be found in: [Symbolic algebra and Mathematics with Xcas](http://www-fourier.ujf-grenoble.fr/~parisse/giac/doc/en/cascmd_en/cascmd_en.html)\n \n - `sage.libs.pynac.pynac.register_symbol` can be used to register new symbols in the `symbols_table`, as in `register_symbol(heaviside,{'giac':'Heaviside'})`.\n``````\n",
    "created_at": "2017-03-29T08:47:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423190",
    "user": "https://github.com/mforets"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -7,6 +7,6 @@
 
 See also: 
 
-- In the file: `local/share/giac/doc/aide_cas`, lines starting by # are keywords followed by synonyms, next lines are for small doc and see also. Full doc it is here: [Symbolic algebra and Mathematics with Xcas](http://www-fourier.ujf-grenoble.fr/~parisse/giac/doc/en/cascmd_en/cascmd_en.html)
+- In the file: `local/share/giac/doc/aide_cas`, lines starting by # are keywords followed by synonyms, next lines are for small doc and see also. Full doc can be found in: [Symbolic algebra and Mathematics with Xcas](http://www-fourier.ujf-grenoble.fr/~parisse/giac/doc/en/cascmd_en/cascmd_en.html)
 
 - `sage.libs.pynac.pynac.register_symbol` can be used to register new symbols in the `symbols_table`, as in `register_symbol(heaviside,{'giac':'Heaviside'})`.
``````




---

archive/issue_comments_423191.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -10,3 +10,5 @@\n - In the file: `local/share/giac/doc/aide_cas`, lines starting by # are keywords followed by synonyms, next lines are for small doc and see also. Full doc can be found in: [Symbolic algebra and Mathematics with Xcas](http://www-fourier.ujf-grenoble.fr/~parisse/giac/doc/en/cascmd_en/cascmd_en.html)\n \n - `sage.libs.pynac.pynac.register_symbol` can be used to register new symbols in the `symbols_table`, as in `register_symbol(heaviside,{'giac':'Heaviside'})`.\n+\n+- for BuiltIn functions which exist in giac with a different name, it is sufficient to update the `conversions` dictionary from the corresponding `__init__` method. \n``````\n",
    "created_at": "2017-03-29T08:55:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423191",
    "user": "https://github.com/mforets"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -10,3 +10,5 @@
 - In the file: `local/share/giac/doc/aide_cas`, lines starting by # are keywords followed by synonyms, next lines are for small doc and see also. Full doc can be found in: [Symbolic algebra and Mathematics with Xcas](http://www-fourier.ujf-grenoble.fr/~parisse/giac/doc/en/cascmd_en/cascmd_en.html)
 
 - `sage.libs.pynac.pynac.register_symbol` can be used to register new symbols in the `symbols_table`, as in `register_symbol(heaviside,{'giac':'Heaviside'})`.
+
+- for BuiltIn functions which exist in giac with a different name, it is sufficient to update the `conversions` dictionary from the corresponding `__init__` method. 
``````




---

archive/issue_comments_423192.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -11,4 +11,4 @@\n \n - `sage.libs.pynac.pynac.register_symbol` can be used to register new symbols in the `symbols_table`, as in `register_symbol(heaviside,{'giac':'Heaviside'})`.\n \n-- for BuiltIn functions which exist in giac with a different name, it is sufficient to update the `conversions` dictionary from the corresponding `__init__` method. \n+- for `BuiltIn` functions which exist in giac with a different name, it is sufficient to update the `conversions` dictionary for the corresponding `__init__` method. \n``````\n",
    "created_at": "2017-03-29T08:56:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423192",
    "user": "https://github.com/mforets"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -11,4 +11,4 @@
 
 - `sage.libs.pynac.pynac.register_symbol` can be used to register new symbols in the `symbols_table`, as in `register_symbol(heaviside,{'giac':'Heaviside'})`.
 
-- for BuiltIn functions which exist in giac with a different name, it is sufficient to update the `conversions` dictionary from the corresponding `__init__` method. 
+- for `BuiltIn` functions which exist in giac with a different name, it is sufficient to update the `conversions` dictionary for the corresponding `__init__` method. 
``````




---

archive/issue_comments_423193.json:
```json
{
    "body": "<a id='comment:5'></a>\nThis one doesn't look easy. Any idea to deal with rootof?\n\n```\nsage: giac.simplify((exp(2*i*pi/3)+exp(2*i*pi/15))^3)\n((-768*i)*(-1)^(2/15)*sqrt(3)-768*(-1)^(2/15)+768*i*(-1)^(4/15)*sqrt(3)-768*(-1)^(4/15)+rootof([[16-16*i,-16+16*i,-400+528*i,144+240*i],[1,0,-30,-40,5]]))/512\n```\ndo we need to chose a name for the variable in the extention field? (if yes  then how? (or what would be a good name))",
    "created_at": "2017-03-29T09:50:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423193",
    "user": "https://github.com/frederichan-IMJPRG"
}
```

<a id='comment:5'></a>
This one doesn't look easy. Any idea to deal with rootof?

```
sage: giac.simplify((exp(2*i*pi/3)+exp(2*i*pi/15))^3)
((-768*i)*(-1)^(2/15)*sqrt(3)-768*(-1)^(2/15)+768*i*(-1)^(4/15)*sqrt(3)-768*(-1)^(4/15)+rootof([[16-16*i,-16+16*i,-400+528*i,144+240*i],[1,0,-30,-40,5]]))/512
```
do we need to chose a name for the variable in the extention field? (if yes  then how? (or what would be a good name))



---

archive/issue_comments_423194.json:
```json
{
    "body": "Changing dependencies from \"\" to \"#22422\"",
    "created_at": "2017-04-01T12:33:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423194",
    "user": "https://github.com/mforets"
}
```

Changing dependencies from "" to "#22422"



---

archive/issue_comments_423195.json:
```json
{
    "body": "Changing branch from \"\" to \"u/paulmasson/22706\"",
    "created_at": "2017-04-02T23:29:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423195",
    "user": "https://github.com/paulmasson"
}
```

Changing branch from "" to "u/paulmasson/22706"



---

archive/issue_comments_423196.json:
```json
{
    "body": "Changing commit from \"\" to \"bdc2807934ce570c7a29547f3ff3fdff5a387beb\"",
    "created_at": "2017-04-02T23:38:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423196",
    "user": "https://github.com/paulmasson"
}
```

Changing commit from "" to "bdc2807934ce570c7a29547f3ff3fdff5a387beb"



---

archive/issue_comments_423197.json:
```json
{
    "body": "Changing author from \"\" to \"Paul Masson\"",
    "created_at": "2017-04-02T23:38:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423197",
    "user": "https://github.com/paulmasson"
}
```

Changing author from "" to "Paul Masson"



---

archive/issue_comments_423198.json:
```json
{
    "body": "<a id='comment:9'></a>\nI went through the files in `sage/functions` to add as many conversions for special functions as possible, but could only find ten that were different from Sage.\n\nThe Giac exponential integrals `Ei`, `Si` and `Ci` have existing Sage aliases, so there shouldn't be a problem with those.\n\nThe orthogonal polynomials `hermite` and `laguerre` have the same names in Giac and Sage. Same for `erf` and a pending `erfc`, as well as `binomial` and `factorial`.\n\nThe Giac function `Psi` has is a different name from Sage `psi`, but the order of the arguments is reversed so I didn't add it now.\n\n---\nNew commits:\n|                                                                                                                                          |                                  |\n|------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------|\n|[0e337ef](https://git.sagemath.org/sage.git/commit?id=0e337ef83b79610a4bd0df64b9e7c88450f3e7b4)|`Add Giac conversions and cleanup`|\n|[ffba36a](https://git.sagemath.org/sage.git/commit?id=ffba36a98cdf3a5a1de6480ded01711f034adc8d)|`Add Giac conversions and cleanup`|\n|[e1a63ed](https://git.sagemath.org/sage.git/commit?id=e1a63edf85af5280aa74d9001bed6ce48328b34a)|`Add Giac conversions and cleanup`|\n|[3a3d079](https://git.sagemath.org/sage.git/commit?id=3a3d0792d3a10a4b067231c3d1279c82fcafbb40)|`Add Giac conversion`|\n|[bdc2807](https://git.sagemath.org/sage.git/commit?id=bdc2807934ce570c7a29547f3ff3fdff5a387beb)|`Add Giac conversions and cleanup`|",
    "created_at": "2017-04-02T23:38:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423198",
    "user": "https://github.com/paulmasson"
}
```

<a id='comment:9'></a>
I went through the files in `sage/functions` to add as many conversions for special functions as possible, but could only find ten that were different from Sage.

The Giac exponential integrals `Ei`, `Si` and `Ci` have existing Sage aliases, so there shouldn't be a problem with those.

The orthogonal polynomials `hermite` and `laguerre` have the same names in Giac and Sage. Same for `erf` and a pending `erfc`, as well as `binomial` and `factorial`.

The Giac function `Psi` has is a different name from Sage `psi`, but the order of the arguments is reversed so I didn't add it now.

---
New commits:
|                                                                                                                                          |                                  |
|------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------|
|[0e337ef](https://git.sagemath.org/sage.git/commit?id=0e337ef83b79610a4bd0df64b9e7c88450f3e7b4)|`Add Giac conversions and cleanup`|
|[ffba36a](https://git.sagemath.org/sage.git/commit?id=ffba36a98cdf3a5a1de6480ded01711f034adc8d)|`Add Giac conversions and cleanup`|
|[e1a63ed](https://git.sagemath.org/sage.git/commit?id=e1a63edf85af5280aa74d9001bed6ce48328b34a)|`Add Giac conversions and cleanup`|
|[3a3d079](https://git.sagemath.org/sage.git/commit?id=3a3d0792d3a10a4b067231c3d1279c82fcafbb40)|`Add Giac conversion`|
|[bdc2807](https://git.sagemath.org/sage.git/commit?id=bdc2807934ce570c7a29547f3ff3fdff5a387beb)|`Add Giac conversions and cleanup`|



---

archive/issue_comments_423199.json:
```json
{
    "body": "<a id='comment:10'></a>\nConsidering the derivatives of delta functions, I think we'll have to do a string replacement using a regular expression similar to how Maxima conversions are handled starting [here](https://github.com/sagemath/sage/blob/master/src/sage/calculus/calculus.py#L1702). Perhaps that should be a separate ticket after #22422 is merged.\n\nThe conversions I've entered are not dependent on #22422 and can be merged at any time. If either of you can identify additional functions that can be added easily with this ticket, let me know.",
    "created_at": "2017-04-03T20:57:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423199",
    "user": "https://github.com/paulmasson"
}
```

<a id='comment:10'></a>
Considering the derivatives of delta functions, I think we'll have to do a string replacement using a regular expression similar to how Maxima conversions are handled starting [here](https://github.com/sagemath/sage/blob/master/src/sage/calculus/calculus.py#L1702). Perhaps that should be a separate ticket after #22422 is merged.

The conversions I've entered are not dependent on #22422 and can be merged at any time. If either of you can identify additional functions that can be added easily with this ticket, let me know.



---

archive/issue_comments_423200.json:
```json
{
    "body": "<a id='comment:11'></a>\nReplying to [frederichan](#comment%3A5):\n> This one doesn't look easy. Any idea to deal with rootof?\n> \n> ```\n> sage: giac.simplify((exp(2*i*pi/3)+exp(2*i*pi/15))^3)\n> ((-768*i)*(-1)^(2/15)*sqrt(3)-768*(-1)^(2/15)+768*i*(-1)^(4/15)*sqrt(3)-768*(-1)^(4/15)+rootof([[16-16*i,-16+16*i,-400+528*i,144+240*i],[1,0,-30,-40,5]]))/512\n> ```\n> do we need to chose a name for the variable in the extention field? (if yes  then how? (or what would be a good name))\n\n\nThis appears to be #22024.",
    "created_at": "2017-04-05T15:06:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423200",
    "user": "https://github.com/rwst"
}
```

<a id='comment:11'></a>
Replying to [frederichan](#comment%3A5):
> This one doesn't look easy. Any idea to deal with rootof?
> 
> ```
> sage: giac.simplify((exp(2*i*pi/3)+exp(2*i*pi/15))^3)
> ((-768*i)*(-1)^(2/15)*sqrt(3)-768*(-1)^(2/15)+768*i*(-1)^(4/15)*sqrt(3)-768*(-1)^(4/15)+rootof([[16-16*i,-16+16*i,-400+528*i,144+240*i],[1,0,-30,-40,5]]))/512
> ```
> do we need to chose a name for the variable in the extention field? (if yes  then how? (or what would be a good name))


This appears to be #22024.



---

archive/issue_comments_423201.json:
```json
{
    "body": "<a id='comment:12'></a>\nReplying to [paulmasson](#comment%3A9):\n> The Giac function `Psi` has is a different name from Sage `psi`, but the order of the arguments is reversed so I didn't add it now.\n\n\ntrue and they agree if just 1 argument is passed.\n\nalso:\n\n- should Giac's `abs` be mapped so `abs_symbolic`? notice that asking for `abs_symbolic??`, there is already some: \n\n```\n\"\"\"GinacFunction.__init__(self, \"abs\", latex_name=r\"\\mathrm{abs}\",\n                               conversions=dict(sympy='Abs'))\n```\n\n- same for `binomial`, `arg`, `ceil`, `conjugage` (and `conj`), `factorial`, `frac`, `imag` (and `imag_part`), `real` (and `real_part`).",
    "created_at": "2017-04-06T11:58:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423201",
    "user": "https://github.com/mforets"
}
```

<a id='comment:12'></a>
Replying to [paulmasson](#comment%3A9):
> The Giac function `Psi` has is a different name from Sage `psi`, but the order of the arguments is reversed so I didn't add it now.


true and they agree if just 1 argument is passed.

also:

- should Giac's `abs` be mapped so `abs_symbolic`? notice that asking for `abs_symbolic??`, there is already some: 

```
"""GinacFunction.__init__(self, "abs", latex_name=r"\mathrm{abs}",
                               conversions=dict(sympy='Abs'))
```

- same for `binomial`, `arg`, `ceil`, `conjugage` (and `conj`), `factorial`, `frac`, `imag` (and `imag_part`), `real` (and `real_part`).



---

archive/issue_comments_423202.json:
```json
{
    "body": "<a id='comment:13'></a>\nReplying to [paulmasson](#comment%3A10):\n> Considering the derivatives of delta functions, I think we'll have to do a string replacement using a regular expression similar to how Maxima conversions are handled starting [here](https://github.com/sagemath/sage/blob/master/src/sage/calculus/calculus.py#L1702). Perhaps that should be a separate ticket after #22422 is merged.\n\n\n+1. moreover, it should also fix integration:\n\n```\nsage: integrate(dirac_delta(x-2)*sin(x), x, 0, 4)    # unevaluated??\nintegrate(dirac_delta(x-2)*sin(x), x, 0, 4)\nsage: integrate(dirac_delta(x-2)*sin(x), x, 0, 4, algorithm='sympy')    # ok\nsin(2)\n```",
    "created_at": "2017-04-08T13:30:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423202",
    "user": "https://github.com/mforets"
}
```

<a id='comment:13'></a>
Replying to [paulmasson](#comment%3A10):
> Considering the derivatives of delta functions, I think we'll have to do a string replacement using a regular expression similar to how Maxima conversions are handled starting [here](https://github.com/sagemath/sage/blob/master/src/sage/calculus/calculus.py#L1702). Perhaps that should be a separate ticket after #22422 is merged.


+1. moreover, it should also fix integration:

```
sage: integrate(dirac_delta(x-2)*sin(x), x, 0, 4)    # unevaluated??
integrate(dirac_delta(x-2)*sin(x), x, 0, 4)
sage: integrate(dirac_delta(x-2)*sin(x), x, 0, 4, algorithm='sympy')    # ok
sin(2)
```



---

archive/issue_comments_423203.json:
```json
{
    "body": "<a id='comment:14'></a>\nafter this one is done, we could go for \"Add giac interface to integrate\". what do you think?",
    "created_at": "2017-04-18T11:31:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423203",
    "user": "https://github.com/mforets"
}
```

<a id='comment:14'></a>
after this one is done, we could go for "Add giac interface to integrate". what do you think?



---

archive/issue_comments_423204.json:
```json
{
    "body": "Changing branch from \"u/paulmasson/22706\" to \"u/mforets/22706\"",
    "created_at": "2017-04-27T11:47:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423204",
    "user": "https://github.com/mforets"
}
```

Changing branch from "u/paulmasson/22706" to "u/mforets/22706"



---

archive/issue_comments_423205.json:
```json
{
    "body": "<a id='comment:16'></a>\nhi,\ni've added some simple conversions. \n\nremaining:\n\n- rootof\n- psi function\n- derivative of delta function. \n\nfor the last item is not clear to me if it is required to write a `_derivative_` method for `FunctionDiracDelta`, or to write a new a class `FunctionDiracDeltaDerivative`, or if it's about passing a dictionary element with some preprocessing (some arguments) so that say `Dirac(x, 2)` (giac) changes to `diff(dirac_delta(x), x, x)` in sage and conversely.\n\n---\nNew commits:\n|                                                                                                                                          |                                 |\n|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------|\n|[9143bd2](https://git.sagemath.org/sage.git/commit?id=9143bd2c7f60e364f3864e862d0cf8d563bc6a60)|`add more /functions conversions`|",
    "created_at": "2017-04-27T12:04:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423205",
    "user": "https://github.com/mforets"
}
```

<a id='comment:16'></a>
hi,
i've added some simple conversions. 

remaining:

- rootof
- psi function
- derivative of delta function. 

for the last item is not clear to me if it is required to write a `_derivative_` method for `FunctionDiracDelta`, or to write a new a class `FunctionDiracDeltaDerivative`, or if it's about passing a dictionary element with some preprocessing (some arguments) so that say `Dirac(x, 2)` (giac) changes to `diff(dirac_delta(x), x, x)` in sage and conversely.

---
New commits:
|                                                                                                                                          |                                 |
|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------|
|[9143bd2](https://git.sagemath.org/sage.git/commit?id=9143bd2c7f60e364f3864e862d0cf8d563bc6a60)|`add more /functions conversions`|



---

archive/issue_comments_423206.json:
```json
{
    "body": "Changing commit from \"bdc2807934ce570c7a29547f3ff3fdff5a387beb\" to \"9143bd2c7f60e364f3864e862d0cf8d563bc6a60\"",
    "created_at": "2017-04-27T12:04:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423206",
    "user": "https://github.com/mforets"
}
```

Changing commit from "bdc2807934ce570c7a29547f3ff3fdff5a387beb" to "9143bd2c7f60e364f3864e862d0cf8d563bc6a60"



---

archive/issue_comments_423207.json:
```json
{
    "body": "Changing author from \"Paul Masson\" to \"Paul Masson, Marcelo Forets\"",
    "created_at": "2017-04-27T12:04:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423207",
    "user": "https://github.com/mforets"
}
```

Changing author from "Paul Masson" to "Paul Masson, Marcelo Forets"



---

archive/issue_comments_423208.json:
```json
{
    "body": "<a id='comment:17'></a>\nsee #22891 for \"add giac interface to integrate\"",
    "created_at": "2017-04-27T20:52:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423208",
    "user": "https://github.com/mforets"
}
```

<a id='comment:17'></a>
see #22891 for "add giac interface to integrate"



---

archive/issue_comments_423209.json:
```json
{
    "body": "<a id='comment:18'></a>\nsomething related:\n\n```\nsage: integrate(exp(x^3), (x, 1, 2), algorithm='giac') \n1/3*igamma(1/3, -1) - 1/3*igamma(1/3, -8)\n```\nwhere `igamma` is giac's name for the incomplete gamma function.",
    "created_at": "2017-05-15T20:02:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423209",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:18'></a>
something related:

```
sage: integrate(exp(x^3), (x, 1, 2), algorithm='giac') 
1/3*igamma(1/3, -1) - 1/3*igamma(1/3, -8)
```
where `igamma` is giac's name for the incomplete gamma function.



---

archive/issue_comments_423210.json:
```json
{
    "body": "<a id='comment:19'></a>\nReplying to [chapoton](#comment%3A18):\n> something related:\n> \n> ```\n> sage: integrate(exp(x^3), (x, 1, 2), algorithm='giac') \n> 1/3*igamma(1/3, -1) - 1/3*igamma(1/3, -8)\n> ```\n> where `igamma` is giac's name for the incomplete gamma function.\n\n\ninteresting. it seems that giac's `igamma` corresponds to `gamma_inc_lower` in Sage (lower incomplete Gamma). moreover, giac's `ugamma` corresponds to `gamma_inc` in Sage (upper incomplete Gamma). i'll add these conversions asap.",
    "created_at": "2017-05-16T06:13:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423210",
    "user": "https://github.com/mforets"
}
```

<a id='comment:19'></a>
Replying to [chapoton](#comment%3A18):
> something related:
> 
> ```
> sage: integrate(exp(x^3), (x, 1, 2), algorithm='giac') 
> 1/3*igamma(1/3, -1) - 1/3*igamma(1/3, -8)
> ```
> where `igamma` is giac's name for the incomplete gamma function.


interesting. it seems that giac's `igamma` corresponds to `gamma_inc_lower` in Sage (lower incomplete Gamma). moreover, giac's `ugamma` corresponds to `gamma_inc` in Sage (upper incomplete Gamma). i'll add these conversions asap.



---

archive/issue_comments_423211.json:
```json
{
    "body": "<a id='comment:20'></a>\nin this case there is also a difference in their numerical evaluations:\n\nactual value of the integral:\n\n```\nsage: numerical_integral(exp(x^3), 1, 2)\n(275.51098376331174, 3.0587863771115636e-12)\n```\n\ngiac:\n\n```\nsage: giac('1/3*igamma(1./3, -1) - 1/3*igamma(1./3, -8)')\n275.510983763\nsage: giac('igamma(1./3, -1)')\n-4.02571325393\nsage: giac('igamma(1./3, -8)') \n-830.558664544\nsage: giac('igamma(1./3, 1)') \n2.42253354642\nsage: giac('igamma(1./3, 8)') \n2.67886054288\n```\n\nsage:\n\n```\nsage: 1/3*gamma_inc_lower(1./3, -1) - 1/3*gamma_inc_lower(1./3, -8)\n-137.755491881656 - 238.599510960670*I\nsage: gamma_inc_lower(1./3, -1)    # values don't match. imag part => t^(-2/3) ? \n2.01285662696613 + 3.48636994625705*I\nsage: gamma_inc_lower(1./3, -8)    # values don't match\n415.279332271934 + 719.284902828267*I    \nsage: gamma_inc_lower(1./3, 1)    # values match\n2.42253354641901\nsage: gamma_inc_lower(1./3, 8)    # values match\n2.67886054288163\n```",
    "created_at": "2017-05-16T07:16:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423211",
    "user": "https://github.com/mforets"
}
```

<a id='comment:20'></a>
in this case there is also a difference in their numerical evaluations:

actual value of the integral:

```
sage: numerical_integral(exp(x^3), 1, 2)
(275.51098376331174, 3.0587863771115636e-12)
```

giac:

```
sage: giac('1/3*igamma(1./3, -1) - 1/3*igamma(1./3, -8)')
275.510983763
sage: giac('igamma(1./3, -1)')
-4.02571325393
sage: giac('igamma(1./3, -8)') 
-830.558664544
sage: giac('igamma(1./3, 1)') 
2.42253354642
sage: giac('igamma(1./3, 8)') 
2.67886054288
```

sage:

```
sage: 1/3*gamma_inc_lower(1./3, -1) - 1/3*gamma_inc_lower(1./3, -8)
-137.755491881656 - 238.599510960670*I
sage: gamma_inc_lower(1./3, -1)    # values don't match. imag part => t^(-2/3) ? 
2.01285662696613 + 3.48636994625705*I
sage: gamma_inc_lower(1./3, -8)    # values don't match
415.279332271934 + 719.284902828267*I    
sage: gamma_inc_lower(1./3, 1)    # values match
2.42253354641901
sage: gamma_inc_lower(1./3, 8)    # values match
2.67886054288163
```



---

archive/issue_comments_423212.json:
```json
{
    "body": "<a id='comment:21'></a>\nhowever, \n\n```\nsage: integrate(exp(x^3), x, 1, 2, algorithm='maxima') # OK\n-1/3*(-1)^(2/3)*gamma(1/3, -1) + 1/3*(-1)^(2/3)*gamma(1/3, -8)\nsage: _.n()\n275.510983763312 - 7.21644966006352e-14*I\nsage: giac('integrate(exp(x^3), x, 1, 2)')  # OK\n1/3*igamma(1/3,-1)-1/3*igamma(1/3,-8)\n```\n\nwhat's going on? is there a problem on the numerical eval of `gamma_inc_lower` or the conversion is not accurate?",
    "created_at": "2017-05-16T10:53:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423212",
    "user": "https://github.com/mforets"
}
```

<a id='comment:21'></a>
however, 

```
sage: integrate(exp(x^3), x, 1, 2, algorithm='maxima') # OK
-1/3*(-1)^(2/3)*gamma(1/3, -1) + 1/3*(-1)^(2/3)*gamma(1/3, -8)
sage: _.n()
275.510983763312 - 7.21644966006352e-14*I
sage: giac('integrate(exp(x^3), x, 1, 2)')  # OK
1/3*igamma(1/3,-1)-1/3*igamma(1/3,-8)
```

what's going on? is there a problem on the numerical eval of `gamma_inc_lower` or the conversion is not accurate?



---

archive/issue_comments_423213.json:
```json
{
    "body": "<a id='comment:22'></a>\nbranch looks good to me and bot is green, but is this in \"needs review\" state ?",
    "created_at": "2017-05-16T19:22:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423213",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:22'></a>
branch looks good to me and bot is green, but is this in "needs review" state ?



---

archive/issue_comments_423214.json:
```json
{
    "body": "<a id='comment:23'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                             |\n|-------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------|\n|[4b9bfda](https://git.sagemath.org/sage.git/commit/?id=4b9bfda2db0dede52b8639336fdd984619945267)|`Merge branch 'develop' into t/mforets/22706`|\n|[6e5e3e2](https://git.sagemath.org/sage.git/commit/?id=6e5e3e2af7f55105b5b85b5940dcbe916c3a35d9)|`incomplete gamma conversions`|",
    "created_at": "2017-05-17T06:20:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423214",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:23'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                             |
|-------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------|
|[4b9bfda](https://git.sagemath.org/sage.git/commit/?id=4b9bfda2db0dede52b8639336fdd984619945267)|`Merge branch 'develop' into t/mforets/22706`|
|[6e5e3e2](https://git.sagemath.org/sage.git/commit/?id=6e5e3e2af7f55105b5b85b5940dcbe916c3a35d9)|`incomplete gamma conversions`|



---

archive/issue_comments_423215.json:
```json
{
    "body": "Changing commit from \"9143bd2c7f60e364f3864e862d0cf8d563bc6a60\" to \"6e5e3e2af7f55105b5b85b5940dcbe916c3a35d9\"",
    "created_at": "2017-05-17T06:20:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423215",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "9143bd2c7f60e364f3864e862d0cf8d563bc6a60" to "6e5e3e2af7f55105b5b85b5940dcbe916c3a35d9"



---

archive/issue_comments_423216.json:
```json
{
    "body": "<a id='comment:24'></a>\nReplying to [chapoton](#comment%3A22):\n> branch looks good to me and bot is green, but is this in \"needs review\" state ?\n\n\nThanks for the feedback -- not yet but close! I'd like to handle the `psi` function conversion. Have to figure out how to reverse the arguments in the function call. I haven't seen examples of that sort.. \n\nFor the other tasks discussed in this thread, i've separated them into smaller chunks, see  [giac interface wiki section](https://trac.sagemath.org/wiki/symbolics#Giacinterface).",
    "created_at": "2017-05-17T06:55:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423216",
    "user": "https://github.com/mforets"
}
```

<a id='comment:24'></a>
Replying to [chapoton](#comment%3A22):
> branch looks good to me and bot is green, but is this in "needs review" state ?


Thanks for the feedback -- not yet but close! I'd like to handle the `psi` function conversion. Have to figure out how to reverse the arguments in the function call. I haven't seen examples of that sort.. 

For the other tasks discussed in this thread, i've separated them into smaller chunks, see  [giac interface wiki section](https://trac.sagemath.org/wiki/symbolics#Giacinterface).



---

archive/issue_comments_423217.json:
```json
{
    "body": "<a id='comment:25'></a>\nbranch does not apply",
    "created_at": "2017-05-21T20:14:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423217",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:25'></a>
branch does not apply



---

archive/issue_comments_423218.json:
```json
{
    "body": "Changing commit from \"6e5e3e2af7f55105b5b85b5940dcbe916c3a35d9\" to \"71bc33610a526043a93ff68f45964ed5b4b93660\"",
    "created_at": "2017-05-25T21:08:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423218",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "6e5e3e2af7f55105b5b85b5940dcbe916c3a35d9" to "71bc33610a526043a93ff68f45964ed5b4b93660"



---

archive/issue_comments_423219.json:
```json
{
    "body": "<a id='comment:26'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                                           |\n|-------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------|\n|[666c2f3](https://git.sagemath.org/sage.git/commit/?id=666c2f36fd23ae9918f1a5e3cc2501017635dd9e)|`resolve merge conflicts  with v.8.0.beta8`|\n|[71bc336](https://git.sagemath.org/sage.git/commit/?id=71bc33610a526043a93ff68f45964ed5b4b93660)|`add error functions conversions`|",
    "created_at": "2017-05-25T21:08:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423219",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:26'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                                           |
|-------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------|
|[666c2f3](https://git.sagemath.org/sage.git/commit/?id=666c2f36fd23ae9918f1a5e3cc2501017635dd9e)|`resolve merge conflicts  with v.8.0.beta8`|
|[71bc336](https://git.sagemath.org/sage.git/commit/?id=71bc33610a526043a93ff68f45964ed5b4b93660)|`add error functions conversions`|



---

archive/issue_comments_423220.json:
```json
{
    "body": "<a id='comment:27'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                            |\n|-------------------------------------------------------------------------------------------------------------------------------------------|----------------------------|\n|[b2f4261](https://git.sagemath.org/sage.git/commit/?id=b2f426184bbd24ddc9b8d710ed937ff7b24eef0c)|`fix missing ) in giac erfc`|",
    "created_at": "2017-05-25T22:05:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423220",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:27'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                            |
|-------------------------------------------------------------------------------------------------------------------------------------------|----------------------------|
|[b2f4261](https://git.sagemath.org/sage.git/commit/?id=b2f426184bbd24ddc9b8d710ed937ff7b24eef0c)|`fix missing ) in giac erfc`|



---

archive/issue_comments_423221.json:
```json
{
    "body": "Changing commit from \"71bc33610a526043a93ff68f45964ed5b4b93660\" to \"b2f426184bbd24ddc9b8d710ed937ff7b24eef0c\"",
    "created_at": "2017-05-25T22:05:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423221",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "71bc33610a526043a93ff68f45964ed5b4b93660" to "b2f426184bbd24ddc9b8d710ed937ff7b24eef0c"



---

archive/issue_comments_423222.json:
```json
{
    "body": "<a id='comment:28'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                   |\n|-------------------------------------------------------------------------------------------------------------------------------------------|-------------------|\n|[6592c61](https://git.sagemath.org/sage.git/commit/?id=6592c6197fbdb40307c6bad3e01e97116af448a3)|`register swap psi`|",
    "created_at": "2017-05-26T05:56:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423222",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:28'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                   |
|-------------------------------------------------------------------------------------------------------------------------------------------|-------------------|
|[6592c61](https://git.sagemath.org/sage.git/commit/?id=6592c6197fbdb40307c6bad3e01e97116af448a3)|`register swap psi`|



---

archive/issue_comments_423223.json:
```json
{
    "body": "Changing commit from \"b2f426184bbd24ddc9b8d710ed937ff7b24eef0c\" to \"6592c6197fbdb40307c6bad3e01e97116af448a3\"",
    "created_at": "2017-05-26T05:56:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423223",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "b2f426184bbd24ddc9b8d710ed937ff7b24eef0c" to "6592c6197fbdb40307c6bad3e01e97116af448a3"



---

archive/issue_comments_423224.json:
```json
{
    "body": "<a id='comment:29'></a>\npartial progress: with the last commit one has\n\n```\nsage:\u00a0giac('Psi(x,1)')._sage_()\npsi(1,\u00a0x)\n```\n\nas desired. however the other way round doesn't work get swapped:\n\n```\nsage: psi(1, x)._giac_() \npsi(1, x)   # Psi(x, 1) is expected\n```",
    "created_at": "2017-05-26T06:01:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423224",
    "user": "https://github.com/mforets"
}
```

<a id='comment:29'></a>
partial progress: with the last commit one has

```
sage: giac('Psi(x,1)')._sage_()
psi(1, x)
```

as desired. however the other way round doesn't work get swapped:

```
sage: psi(1, x)._giac_() 
psi(1, x)   # Psi(x, 1) is expected
```



---

archive/issue_comments_423225.json:
```json
{
    "body": "<a id='comment:30'></a>\nIt works if you give a giac conversion in the psi2 init as well.",
    "created_at": "2017-05-26T06:48:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423225",
    "user": "https://github.com/rwst"
}
```

<a id='comment:30'></a>
It works if you give a giac conversion in the psi2 init as well.



---

archive/issue_comments_423226.json:
```json
{
    "body": "<a id='comment:31'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                           |                            |\n|-------------------------------------------------------------------------------------------------------------------------------------------|----------------------------|\n|[350316f](https://git.sagemath.org/sage.git/commit/?id=350316fe0bc69cb8c1fdebbd3363d3437661f28f)|`add Psi conversion in psi2`|",
    "created_at": "2017-05-26T08:08:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423226",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:31'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                           |                            |
|-------------------------------------------------------------------------------------------------------------------------------------------|----------------------------|
|[350316f](https://git.sagemath.org/sage.git/commit/?id=350316fe0bc69cb8c1fdebbd3363d3437661f28f)|`add Psi conversion in psi2`|



---

archive/issue_comments_423227.json:
```json
{
    "body": "Changing commit from \"6592c6197fbdb40307c6bad3e01e97116af448a3\" to \"350316fe0bc69cb8c1fdebbd3363d3437661f28f\"",
    "created_at": "2017-05-26T08:08:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423227",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "6592c6197fbdb40307c6bad3e01e97116af448a3" to "350316fe0bc69cb8c1fdebbd3363d3437661f28f"



---

archive/issue_comments_423228.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2017-05-26T08:09:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423228",
    "user": "https://github.com/mforets"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_423229.json:
```json
{
    "body": "<a id='comment:33'></a>\nReplying to [rws](#comment%3A30):\n> It works if you give a giac conversion in the psi2 init as well.\n\nIt does!! Thanks.",
    "created_at": "2017-05-26T08:14:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423229",
    "user": "https://github.com/mforets"
}
```

<a id='comment:33'></a>
Replying to [rws](#comment%3A30):
> It works if you give a giac conversion in the psi2 init as well.

It does!! Thanks.



---

archive/issue_comments_423230.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Fr\u00e9d\u00e9ric Chapoton\"",
    "created_at": "2017-06-02T13:54:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423230",
    "user": "https://github.com/fchapoton"
}
```

Changing reviewer from "" to "Frédéric Chapoton"



---

archive/issue_comments_423231.json:
```json
{
    "body": "<a id='comment:34'></a>\nok, let it be",
    "created_at": "2017-06-02T13:54:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423231",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:34'></a>
ok, let it be



---

archive/issue_comments_423232.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2017-06-02T13:54:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423232",
    "user": "https://github.com/fchapoton"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_423233.json:
```json
{
    "body": "<a id='comment:35'></a>\nReplying to [chapoton](#comment%3A34):\n> ok, let it be\n\n\ngreat, thanks for the review and to all participants of this ticket! \n\nw.r.t [giac symbolics tickets](https://trac.sagemath.org/wiki/symbolics#Giacinterface) i think i'll go on with #23016...",
    "created_at": "2017-06-04T08:34:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423233",
    "user": "https://github.com/mforets"
}
```

<a id='comment:35'></a>
Replying to [chapoton](#comment%3A34):
> ok, let it be


great, thanks for the review and to all participants of this ticket! 

w.r.t [giac symbolics tickets](https://trac.sagemath.org/wiki/symbolics#Giacinterface) i think i'll go on with #23016...



---

archive/issue_comments_423234.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2017-06-04T10:08:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423234",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_comments_423235.json:
```json
{
    "body": "Changing branch from \"u/mforets/22706\" to \"350316fe0bc69cb8c1fdebbd3363d3437661f28f\"",
    "created_at": "2017-06-04T10:08:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22706#issuecomment-423235",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/mforets/22706" to "350316fe0bc69cb8c1fdebbd3363d3437661f28f"



---

archive/issue_events_059169.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-06-04T10:08:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/22706",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/22706#event-59169"
}
```
