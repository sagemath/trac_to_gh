# Issue 22809: Pass number of variables to polygens

archive/issues_022572.json:
```json
{
    "body": "The number of generators of a polynomial can be specified as a number:\n\n```\nsage: PolynomialRing(QQ, 'x', 4).gens()\n(x0, x1, x2, x3)\n```\n\nThis ticket should allow doing: \n\n```\nsage: polygens(QQ, 'x', 4)\n(x0, x1, x2, x3)\n```\n\nCurrently it returns `TypeError: polygens() takes at most 2 arguments (3 given)`.\n\nCC:  @nbruin @tscrim @videlec\n\nKeywords: polynomial vector\n\nBranch/Commit: 5f1a4da2392e198878f33f3bc8fc118178dd6218\n\nReviewer: Simon King\n\nAuthor: Marcelo Forets\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/22809\n\n",
    "closed_at": "2019-07-27T19:51:26Z",
    "created_at": "2017-04-14T09:37:29Z",
    "labels": [
        "component: commutative algebra"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.0",
    "title": "Pass number of variables to polygens",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/22809",
    "user": "https://github.com/mforets"
}
```
The number of generators of a polynomial can be specified as a number:

```
sage: PolynomialRing(QQ, 'x', 4).gens()
(x0, x1, x2, x3)
```

This ticket should allow doing: 

```
sage: polygens(QQ, 'x', 4)
(x0, x1, x2, x3)
```

Currently it returns `TypeError: polygens() takes at most 2 arguments (3 given)`.

CC:  @nbruin @tscrim @videlec

Keywords: polynomial vector

Branch/Commit: 5f1a4da2392e198878f33f3bc8fc118178dd6218

Reviewer: Simon King

Author: Marcelo Forets

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/22809





---

archive/issue_comments_425829.json:
```json
{
    "body": "Changing branch from \"\" to \"u/mforets/pass_number_of_variables_to_polygens\"",
    "created_at": "2017-04-14T11:43:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425829",
    "user": "https://github.com/mforets"
}
```

Changing branch from "" to "u/mforets/pass_number_of_variables_to_polygens"



---

archive/issue_comments_425830.json:
```json
{
    "body": "Changing commit from \"\" to \"df2e39f6096e63e18a7014717e44a2013450cb36\"",
    "created_at": "2017-04-14T11:51:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425830",
    "user": "https://github.com/mforets"
}
```

Changing commit from "" to "df2e39f6096e63e18a7014717e44a2013450cb36"



---

archive/issue_comments_425831.json:
```json
{
    "body": "<a id='comment:2'></a>apart from that, i'm tempted to propose making this possible too:\n\n```\nsage: polygens(QQ, 'x', 4, start_at=1)\n(x1, x2, x3, x4)\n```\n\n---\nNew commits:",
    "created_at": "2017-04-14T11:51:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425831",
    "user": "https://github.com/mforets"
}
```

<a id='comment:2'></a>apart from that, i'm tempted to propose making this possible too:

```
sage: polygens(QQ, 'x', 4, start_at=1)
(x1, x2, x3, x4)
```

---
New commits:



---

archive/issue_comments_425832.json:
```json
{
    "body": "<a id='comment:3'></a>Hmm, I wasn't aware of this other functionality.  I have no opinion about it per se, but if you do it, I wonder if you could experiment with what might happen if one did\n\n```\nL = polygens(QQ, 'x', 4)\nfor l in L:\n    SR(l).inject_variable()\n```\nor whatever the right commands would be for that.  Or even\n\n```\npolygens(SR, 'x', 4)\n```\nthough I assume that wouldn't work right.  See #11576 for motivation.  (Or forget this comment and just be aware of #11576, which had a flurry of activity four years ago but hasn't been looked at since.)",
    "created_at": "2017-04-14T14:36:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425832",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:3'></a>Hmm, I wasn't aware of this other functionality.  I have no opinion about it per se, but if you do it, I wonder if you could experiment with what might happen if one did

```
L = polygens(QQ, 'x', 4)
for l in L:
    SR(l).inject_variable()
```
or whatever the right commands would be for that.  Or even

```
polygens(SR, 'x', 4)
```
though I assume that wouldn't work right.  See #11576 for motivation.  (Or forget this comment and just be aware of #11576, which had a flurry of activity four years ago but hasn't been looked at since.)



---

archive/issue_comments_425833.json:
```json
{
    "body": "<a id='comment:4'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2017-04-14T17:01:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425833",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:4'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_425834.json:
```json
{
    "body": "Changing commit from \"df2e39f6096e63e18a7014717e44a2013450cb36\" to \"6d2082c66faeadc4d569afbc02a5b2fb52bc741b\"",
    "created_at": "2017-04-14T17:01:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425834",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "df2e39f6096e63e18a7014717e44a2013450cb36" to "6d2082c66faeadc4d569afbc02a5b2fb52bc741b"



---

archive/issue_comments_425835.json:
```json
{
    "body": "<a id='comment:5'></a>Replying to [kcrisman](#comment%3A3):\n> Hmm, I wasn't aware of this other functionality.  I have no opinion about it per se, but if you do it, I wonder if you could experiment with what might happen if one did\n> \n> ```\n> L = polygens(QQ, 'x', 4)\n> for l in L:\n>     SR(l).inject_variable()\n> ```\n> or whatever the right commands would be for that.  Or even\n> \n> ```\n> polygens(SR, 'x', 4)\n> ```\n> though I assume that wouldn't work right.  \n\n\ngreat idea! it is very useful that these variables 'exist' in the calling method.\nas you can see in revised patch i used `get_main_globals` to get around the issue.\n\nhow does it look? does someone suggest other examples?\n\n> See #11576 for motivation.  (Or forget this comment and just be aware of #11576, which had a flurry of activity four years ago but hasn't been looked at since.)\n\n\nok, thanks for pointing that out! although it seems to me that #11576 is an advanced version of the simpler (but yet lacking) uses `var('x', 4)` and `var('x', 4, 4)` for vectors and matrices respectively. or i'm missing some basic reason why they don't exist already?",
    "created_at": "2017-04-14T17:10:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425835",
    "user": "https://github.com/mforets"
}
```

<a id='comment:5'></a>Replying to [kcrisman](#comment%3A3):
> Hmm, I wasn't aware of this other functionality.  I have no opinion about it per se, but if you do it, I wonder if you could experiment with what might happen if one did
> 
> ```
> L = polygens(QQ, 'x', 4)
> for l in L:
>     SR(l).inject_variable()
> ```
> or whatever the right commands would be for that.  Or even
> 
> ```
> polygens(SR, 'x', 4)
> ```
> though I assume that wouldn't work right.  


great idea! it is very useful that these variables 'exist' in the calling method.
as you can see in revised patch i used `get_main_globals` to get around the issue.

how does it look? does someone suggest other examples?

> See #11576 for motivation.  (Or forget this comment and just be aware of #11576, which had a flurry of activity four years ago but hasn't been looked at since.)


ok, thanks for pointing that out! although it seems to me that #11576 is an advanced version of the simpler (but yet lacking) uses `var('x', 4)` and `var('x', 4, 4)` for vectors and matrices respectively. or i'm missing some basic reason why they don't exist already?



---

archive/issue_comments_425836.json:
```json
{
    "body": "<a id='comment:6'></a>Don't both return a result and inject bindings (and certainly don't go grabbing the global scope!). It's pretty clear that the behaviour of `var` to both inject and return variables is a mistake once you see the confusion it causes.\n\nInjection of polynomial variables is quite efficiently done with `PolynomialRing(R,'x',4).inject_variables()`, which prints a message and returns `None` on purpose.\n\n`polygen(*args)` is just a short form of `PolynomialRing(*args).gens()` which helps people who know what a polynomial variable is but not what a polynomial ring is. I think it's a bad design decision to muddle this with other features.\n\nI'm also against providing options to modify the numbering. There's a good reason why `RX=PolynomialRing(R,'x',n)` is allowed: it's clearly desirable to make \"a polynomial ring in n variables\" in various settings. Sage requires names for these variables, but specifying n names is bothersome if you don't care about the names. Hence the indexing. which agrees with what `RX.gen(i)` does. If you do care what your variable names are called, it's easy to specify a list of names. All kinds of possibilities exist: `[\"x_%s\"%s for i in range(n)]`, `[\"x^(%s)\"%s for i in range(1,n+1)]` etc. It's much better to teach people how to get full customizibility than to give them one more option to discover, which will probably not meet their needs if they develop further.",
    "created_at": "2017-04-14T18:02:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425836",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:6'></a>Don't both return a result and inject bindings (and certainly don't go grabbing the global scope!). It's pretty clear that the behaviour of `var` to both inject and return variables is a mistake once you see the confusion it causes.

Injection of polynomial variables is quite efficiently done with `PolynomialRing(R,'x',4).inject_variables()`, which prints a message and returns `None` on purpose.

`polygen(*args)` is just a short form of `PolynomialRing(*args).gens()` which helps people who know what a polynomial variable is but not what a polynomial ring is. I think it's a bad design decision to muddle this with other features.

I'm also against providing options to modify the numbering. There's a good reason why `RX=PolynomialRing(R,'x',n)` is allowed: it's clearly desirable to make "a polynomial ring in n variables" in various settings. Sage requires names for these variables, but specifying n names is bothersome if you don't care about the names. Hence the indexing. which agrees with what `RX.gen(i)` does. If you do care what your variable names are called, it's easy to specify a list of names. All kinds of possibilities exist: `["x_%s"%s for i in range(n)]`, `["x^(%s)"%s for i in range(1,n+1)]` etc. It's much better to teach people how to get full customizibility than to give them one more option to discover, which will probably not meet their needs if they develop further.



---

archive/issue_comments_425837.json:
```json
{
    "body": "<a id='comment:7'></a>`@`nbruin : thanks for the prompt feedback :) but are you ok with df2e39f or not? notice that `*args` was missing there on the first place.\n\n> Don't both return a result and inject bindings (and certainly don't go grabbing the global scope!). \n\n\nis there an alternative to `global_scope()`? i tried with just `R.inject_variables()` and it didn't work. i suppose it works via `return R.injected_variables()`, but then you don't have access to your vector of variables, right? for multiplication and so on, it is useful to have it.\n\n> It's pretty clear that the behaviour of `var` to both inject and return variables is a mistake once you see the confusion it causes.\n\n\ncould you point me to some threads on this issue, so that i can catch up? for me there are no surprises with `var`. moreover, i find it practical that the function returns you the variables and/or allows you to use them in the console.\n\n> Injection of polynomial variables is quite efficiently done with `PolynomialRing(R,'x',4).inject_variables()`, which prints a message and returns `None` on purpose.\n\n\nok, but let's agree it is a rather long construction for people with bad memory (i'm in that group).\n\n> `polygen(*args)` is just a short form of `PolynomialRing(*args).gens()` which helps people who know what a polynomial variable is but not what a polynomial ring is. I think it's a bad design decision to muddle this with other features.\n\n\nyes (essentially i'm on that group, too). that's arguable: why should a feature bother at all? so long as it is consistent with the rest & well documented.. (more on this below)\n\n> If you do care what your variable names are called, it's easy to specify a list of names. All kinds of possibilities exist: `[\"x_%s\"%s for i in range(n)]`, `[\"x^(%s)\"%s for i in range(1,n+1)]` etc.\n\n\nthat's definitely not easy. \n\n> It's much better to teach people how to get full customizibility than to give them one more option to discover, which will probably not meet their needs if they develop further.\n\n\ni get the point about \"teaching a man to fish\", 100%. at the same time, i'm with the lazy people here: if someone thought the solution and shows you a little example in command help, then i prefer using it rather than working it out myself (saves time, and is less error-prone). \n\nfor the point on features: what about LaTeX? in general people are happy with lots and lots of customizations ready to be discovered.",
    "created_at": "2017-04-14T18:53:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425837",
    "user": "https://github.com/mforets"
}
```

<a id='comment:7'></a>`@`nbruin : thanks for the prompt feedback :) but are you ok with df2e39f or not? notice that `*args` was missing there on the first place.

> Don't both return a result and inject bindings (and certainly don't go grabbing the global scope!). 


is there an alternative to `global_scope()`? i tried with just `R.inject_variables()` and it didn't work. i suppose it works via `return R.injected_variables()`, but then you don't have access to your vector of variables, right? for multiplication and so on, it is useful to have it.

> It's pretty clear that the behaviour of `var` to both inject and return variables is a mistake once you see the confusion it causes.


could you point me to some threads on this issue, so that i can catch up? for me there are no surprises with `var`. moreover, i find it practical that the function returns you the variables and/or allows you to use them in the console.

> Injection of polynomial variables is quite efficiently done with `PolynomialRing(R,'x',4).inject_variables()`, which prints a message and returns `None` on purpose.


ok, but let's agree it is a rather long construction for people with bad memory (i'm in that group).

> `polygen(*args)` is just a short form of `PolynomialRing(*args).gens()` which helps people who know what a polynomial variable is but not what a polynomial ring is. I think it's a bad design decision to muddle this with other features.


yes (essentially i'm on that group, too). that's arguable: why should a feature bother at all? so long as it is consistent with the rest & well documented.. (more on this below)

> If you do care what your variable names are called, it's easy to specify a list of names. All kinds of possibilities exist: `["x_%s"%s for i in range(n)]`, `["x^(%s)"%s for i in range(1,n+1)]` etc.


that's definitely not easy. 

> It's much better to teach people how to get full customizibility than to give them one more option to discover, which will probably not meet their needs if they develop further.


i get the point about "teaching a man to fish", 100%. at the same time, i'm with the lazy people here: if someone thought the solution and shows you a little example in command help, then i prefer using it rather than working it out myself (saves time, and is less error-prone). 

for the point on features: what about LaTeX? in general people are happy with lots and lots of customizations ready to be discovered.



---

archive/issue_comments_425838.json:
```json
{
    "body": "<a id='comment:8'></a>Replying to [mforets](#comment%3A7):\n> `@`nbruin : thanks for the prompt feedback :) but are you ok with df2e39f or not? notice that `*args` was missing there on the first place.\n\n\nYes, that looks pretty straightforward. It might be that passing `*args` on wholesale is problematic (it causes erroneous parameters to be reported with a deeper traceback), but I don't see a big problem immediately. That's also something that is easy to fix later.\n\nConcerning your other queries:\n- in python it is usual that routines that are called because of their side-effects don't return anything. Compare `L.sort()` versus `sorted(L)`\n- just google or search the documentation for the number of examples of `y=var('y')` etc.\n- I don't think you will find many people who would give LaTeX as an example of a well-designed interface.\n- implementing extra features adds bloat and other maintenance burden, and makes it more confusing for new people, because now they will be confronted with multiple ways of doing things. See \"The zen of python\". Not all of it applies to math software, but generally the ideas there have worked well to keep python a pleasant and easy to learn language.\n\nThat said, nothing prevents you from making your own convenience library functions and maintaining them in your own installation. Once they have been sitting around for a year or two and you're finding that you use them often, you'll have a nice body of code to demonstrate in exactly what way they are more convenient. Practice learns that what seems convenient when you're writing/designing something is often not used much in actual use.\n\nSage is getting large and mature enough that if there is a reasonable way of doing something, we should be hesitant about adding alternative constructions.",
    "created_at": "2017-04-14T19:55:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425838",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:8'></a>Replying to [mforets](#comment%3A7):
> `@`nbruin : thanks for the prompt feedback :) but are you ok with df2e39f or not? notice that `*args` was missing there on the first place.


Yes, that looks pretty straightforward. It might be that passing `*args` on wholesale is problematic (it causes erroneous parameters to be reported with a deeper traceback), but I don't see a big problem immediately. That's also something that is easy to fix later.

Concerning your other queries:
- in python it is usual that routines that are called because of their side-effects don't return anything. Compare `L.sort()` versus `sorted(L)`
- just google or search the documentation for the number of examples of `y=var('y')` etc.
- I don't think you will find many people who would give LaTeX as an example of a well-designed interface.
- implementing extra features adds bloat and other maintenance burden, and makes it more confusing for new people, because now they will be confronted with multiple ways of doing things. See "The zen of python". Not all of it applies to math software, but generally the ideas there have worked well to keep python a pleasant and easy to learn language.

That said, nothing prevents you from making your own convenience library functions and maintaining them in your own installation. Once they have been sitting around for a year or two and you're finding that you use them often, you'll have a nice body of code to demonstrate in exactly what way they are more convenient. Practice learns that what seems convenient when you're writing/designing something is often not used much in actual use.

Sage is getting large and mature enough that if there is a reasonable way of doing something, we should be hesitant about adding alternative constructions.



---

archive/issue_comments_425839.json:
```json
{
    "body": "<a id='comment:9'></a>> ok, thanks for pointing that out! although it seems to me that #11576 is an advanced version of the simpler (but yet lacking) uses var('x', 4) and var('x', 4, 4) for vectors and matrices respectively. or i'm missing some basic reason why they don't exist already?\n\n\nBecause nobody has both implemented and reviewed them.  Lots of [bikeshedding](https://en.wikipedia.org/wiki/Law_of_triviality) around exactly what the defaults and syntax should be.",
    "created_at": "2017-04-14T20:10:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425839",
    "user": "https://github.com/kcrisman"
}
```

<a id='comment:9'></a>> ok, thanks for pointing that out! although it seems to me that #11576 is an advanced version of the simpler (but yet lacking) uses var('x', 4) and var('x', 4, 4) for vectors and matrices respectively. or i'm missing some basic reason why they don't exist already?


Because nobody has both implemented and reviewed them.  Lots of [bikeshedding](https://en.wikipedia.org/wiki/Law_of_triviality) around exactly what the defaults and syntax should be.



---

archive/issue_comments_425840.json:
```json
{
    "body": "<a id='comment:10'></a>Replying to [nbruin](#comment%3A8):\n> Replying to [mforets](#comment%3A7):\n> > `@`nbruin : thanks for the prompt feedback :) but are you ok with df2e39f or not? notice that `*args` was missing there on the first place.\n\n> \n> Yes, that looks pretty straightforward. It might be that passing `*args` on wholesale is problematic (it causes erroneous parameters to be reported with a deeper traceback), but I don't see a big problem immediately. That's also something that is easy to fix later.\n\n\ngot it. df2e39f does solve the ticket's requirement. yet i suggest to leave it in the current status for 1 week in case someone else want to chime in.\n\nBTW I'm willing to learn and apply the more robust solution if you have time now (or in the future).\n \n> That said, nothing prevents you from making your own convenience library functions and maintaining them in your own installation. Once they have been sitting around for a year or two and you're finding that you use them often, you'll have a nice body of code to demonstrate in exactly what way they are more convenient. Practice learns that what seems convenient when you're writing/designing something is often not used much in actual use.\n\n\nYes, I need a year or two (more than just time, certainly!) to just grasp what is convenient or not in terms of good design from an expert Python / Sage developer's standpoint. \n\nNo, I don't need a year or two to know that any of the possibilities that you suggest `[\"x_%s\"%s for i in range(n)]`, `[\"x^(%s)\"%s for i in range(1,n+1)]` are not easy. Just because of this:\n\n- Maple (born 1982) -- `Vector(n, symbol = x)`\n\n- Matlab\u00a0(born 1984) -- `sym('x', [n 1])`\n\n- Mathematica (born 1988) -- `Array[x, n]`\n\n- SymPy (born 2007) -- `symbols('x:n')`. \n\nMatrices are constructed in a similar way.",
    "created_at": "2017-04-14T21:40:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425840",
    "user": "https://github.com/mforets"
}
```

<a id='comment:10'></a>Replying to [nbruin](#comment%3A8):
> Replying to [mforets](#comment%3A7):
> > `@`nbruin : thanks for the prompt feedback :) but are you ok with df2e39f or not? notice that `*args` was missing there on the first place.

> 
> Yes, that looks pretty straightforward. It might be that passing `*args` on wholesale is problematic (it causes erroneous parameters to be reported with a deeper traceback), but I don't see a big problem immediately. That's also something that is easy to fix later.


got it. df2e39f does solve the ticket's requirement. yet i suggest to leave it in the current status for 1 week in case someone else want to chime in.

BTW I'm willing to learn and apply the more robust solution if you have time now (or in the future).
 
> That said, nothing prevents you from making your own convenience library functions and maintaining them in your own installation. Once they have been sitting around for a year or two and you're finding that you use them often, you'll have a nice body of code to demonstrate in exactly what way they are more convenient. Practice learns that what seems convenient when you're writing/designing something is often not used much in actual use.


Yes, I need a year or two (more than just time, certainly!) to just grasp what is convenient or not in terms of good design from an expert Python / Sage developer's standpoint. 

No, I don't need a year or two to know that any of the possibilities that you suggest `["x_%s"%s for i in range(n)]`, `["x^(%s)"%s for i in range(1,n+1)]` are not easy. Just because of this:

- Maple (born 1982) -- `Vector(n, symbol = x)`

- Matlab (born 1984) -- `sym('x', [n 1])`

- Mathematica (born 1988) -- `Array[x, n]`

- SymPy (born 2007) -- `symbols('x:n')`. 

Matrices are constructed in a similar way.



---

archive/issue_comments_425841.json:
```json
{
    "body": "<a id='comment:11'></a>Replying to [mforets](#comment%3A10):\n> - Maple (born 1982) -- `Vector(n, symbol = x)`\n> \n> - Matlab\u00a0(born 1984) -- `sym('x', [n 1])`\n> \n> - Mathematica (born 1988) -- `Array[x, n]`\n> \n> - SymPy (born 2007) -- `symbols('x:n')`. \n> \n> Matrices are constructed in a similar way.\n\n\nDoesn't `vector(polygen(QQ,'x',n))` and `matrix(n,m,polygen(QQ,'x',n*m))` fit the bill for that? We need a little more symbols, but that's because sage is more explicit about the ring over which one works.",
    "created_at": "2017-04-14T23:29:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425841",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:11'></a>Replying to [mforets](#comment%3A10):
> - Maple (born 1982) -- `Vector(n, symbol = x)`
> 
> - Matlab (born 1984) -- `sym('x', [n 1])`
> 
> - Mathematica (born 1988) -- `Array[x, n]`
> 
> - SymPy (born 2007) -- `symbols('x:n')`. 
> 
> Matrices are constructed in a similar way.


Doesn't `vector(polygen(QQ,'x',n))` and `matrix(n,m,polygen(QQ,'x',n*m))` fit the bill for that? We need a little more symbols, but that's because sage is more explicit about the ring over which one works.



---

archive/issue_comments_425842.json:
```json
{
    "body": "<a id='comment:12'></a>Replying to [nbruin](#comment%3A11):\n> Replying to [mforets](#comment%3A10):\n> > - Maple (born 1982) -- `Vector(n, symbol = x)`\n> > \n> > - Matlab\u00a0(born 1984) -- `sym('x', [n 1])`\n> > \n> > - Mathematica (born 1988) -- `Array[x, n]`\n> > \n> > - SymPy (born 2007) -- `symbols('x:n')`. \n> > \n> > Matrices are constructed in a similar way.\n\n> \n> Doesn't `vector(polygen(QQ,'x',n))` and `matrix(n,m,polygen(QQ,'x',n*m))` fit the bill for that? We need a little more symbols, but that's because sage is more explicit about the ring over which one works.\n\n\nOh yes, that's a nice construction for `\\mathcal{M}_{n\\times m}(\\mathbb{K}[x])` (minor: it is the plural, `polygens`). \n\nAnother thing that the commercial software have in common is that they follow mainstream Linear Algebra convention about variable names. That construction doesn't, especially:\n\n```\nsage: matrix(2, 4, polygens(QQ, 'x', 8))\n\n[x0 x1 x2 x3]\n[x4 x5 x6 x7]\n```\n\nWhat do you people think about the following. There is an open ticket #13703 which targets common matrix constructors. In the same spirit as the polytope library (type `polytopes.?`), with some work we can have a comprehensive matrix library, `matrices.?`, that features in particular:\n\n```\nsage: matrices.symbolic(2)\n\n[a11 a12]\n[a21 a22]\n```",
    "created_at": "2017-04-15T08:08:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425842",
    "user": "https://github.com/mforets"
}
```

<a id='comment:12'></a>Replying to [nbruin](#comment%3A11):
> Replying to [mforets](#comment%3A10):
> > - Maple (born 1982) -- `Vector(n, symbol = x)`
> > 
> > - Matlab (born 1984) -- `sym('x', [n 1])`
> > 
> > - Mathematica (born 1988) -- `Array[x, n]`
> > 
> > - SymPy (born 2007) -- `symbols('x:n')`. 
> > 
> > Matrices are constructed in a similar way.

> 
> Doesn't `vector(polygen(QQ,'x',n))` and `matrix(n,m,polygen(QQ,'x',n*m))` fit the bill for that? We need a little more symbols, but that's because sage is more explicit about the ring over which one works.


Oh yes, that's a nice construction for `\mathcal{M}_{n\times m}(\mathbb{K}[x])` (minor: it is the plural, `polygens`). 

Another thing that the commercial software have in common is that they follow mainstream Linear Algebra convention about variable names. That construction doesn't, especially:

```
sage: matrix(2, 4, polygens(QQ, 'x', 8))

[x0 x1 x2 x3]
[x4 x5 x6 x7]
```

What do you people think about the following. There is an open ticket #13703 which targets common matrix constructors. In the same spirit as the polytope library (type `polytopes.?`), with some work we can have a comprehensive matrix library, `matrices.?`, that features in particular:

```
sage: matrices.symbolic(2)

[a11 a12]
[a21 a22]
```



---

archive/issue_comments_425843.json:
```json
{
    "body": "<a id='comment:13'></a>Replying to [mforets](#comment%3A12):\n> What do you people think about the following. There is an open ticket #13703 which targets common matrix constructors. In the same spirit as the polytope library (type `polytopes.?`), with some work we can have a comprehensive matrix library, `matrices.?`, that features in particular:\n> \n> \n> ```\n> sage: matrices.symbolic(2)\n> \n> [a11 a12]\n> [a21 a22]\n> ```\n\n\nWhat would it be used for? If the matrix is of any significant size, there is not much computation you can do with these things. And if the matrix is small you might as well construct it manually. In any case:\n- you'd want to be able to specify what the base ring is (or specify you want elements of SR as entries)\n- once you're making these \"universal\" matrices, you should probably be able to specify the form (symmetric, antisymmetric, upper triangular, etc.)\n- The labelling should be 0-based. You'd want `M[1,2]` to be `a12`.",
    "created_at": "2017-04-15T17:02:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425843",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:13'></a>Replying to [mforets](#comment%3A12):
> What do you people think about the following. There is an open ticket #13703 which targets common matrix constructors. In the same spirit as the polytope library (type `polytopes.?`), with some work we can have a comprehensive matrix library, `matrices.?`, that features in particular:
> 
> 
> ```
> sage: matrices.symbolic(2)
> 
> [a11 a12]
> [a21 a22]
> ```


What would it be used for? If the matrix is of any significant size, there is not much computation you can do with these things. And if the matrix is small you might as well construct it manually. In any case:
- you'd want to be able to specify what the base ring is (or specify you want elements of SR as entries)
- once you're making these "universal" matrices, you should probably be able to specify the form (symmetric, antisymmetric, upper triangular, etc.)
- The labelling should be 0-based. You'd want `M[1,2]` to be `a12`.



---

archive/issue_comments_425844.json:
```json
{
    "body": "<a id='comment:14'></a>Replying to [nbruin](#comment%3A13):\n> Replying to [mforets](#comment%3A12):\n> > What do you people think about the following. There is an open ticket #13703 which targets common matrix constructors. In the same spirit as the polytope library (type `polytopes.?`), with some work we can have a comprehensive matrix library, `matrices.?`, that features in particular:\n> > \n> > \n> > ```\n> > sage: matrices.symbolic(2)\n> > \n> > [a11 a12]\n> > [a21 a22]\n> > ```\n\n> \n> What would it be used for? \n\n\n- Teaching : undergraduate level or before using [SageMath](SageMath). \n- Mathematical Modeling : to represent an optimization variable in a semidefinite program.  \n\nI'd be delighted to go deeper into any of these use cases, or to further exchange our ideas and ideals about less concrete, open-ended issues that innocent #22809 has raised, such as why this very basic construct exists in all the software cited before, while we shouldn't expect a useful answer for the inverse of a big matrix whose coefficients are variables. \n\nBut, is this relevant at all to #22809? Maybe in Sage-devel?",
    "created_at": "2017-04-15T18:46:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425844",
    "user": "https://github.com/mforets"
}
```

<a id='comment:14'></a>Replying to [nbruin](#comment%3A13):
> Replying to [mforets](#comment%3A12):
> > What do you people think about the following. There is an open ticket #13703 which targets common matrix constructors. In the same spirit as the polytope library (type `polytopes.?`), with some work we can have a comprehensive matrix library, `matrices.?`, that features in particular:
> > 
> > 
> > ```
> > sage: matrices.symbolic(2)
> > 
> > [a11 a12]
> > [a21 a22]
> > ```

> 
> What would it be used for? 


- Teaching : undergraduate level or before using [SageMath](SageMath). 
- Mathematical Modeling : to represent an optimization variable in a semidefinite program.  

I'd be delighted to go deeper into any of these use cases, or to further exchange our ideas and ideals about less concrete, open-ended issues that innocent #22809 has raised, such as why this very basic construct exists in all the software cited before, while we shouldn't expect a useful answer for the inverse of a big matrix whose coefficients are variables. 

But, is this relevant at all to #22809? Maybe in Sage-devel?



---

archive/issue_comments_425845.json:
```json
{
    "body": "<a id='comment:15'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2017-04-15T19:41:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425845",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:15'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_425846.json:
```json
{
    "body": "Changing commit from \"6d2082c66faeadc4d569afbc02a5b2fb52bc741b\" to \"7a09c12b7f8b1ff03fdf2093d1f57d41bd622974\"",
    "created_at": "2017-04-15T19:41:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425846",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "6d2082c66faeadc4d569afbc02a5b2fb52bc741b" to "7a09c12b7f8b1ff03fdf2093d1f57d41bd622974"



---

archive/issue_comments_425847.json:
```json
{
    "body": "<a id='comment:16'></a>fair enough about naming conventions or handy constructions. as far as i'm concerned, this ticket is ready for peer-review!\n\nside note: i've used `git revert` as suggested [here](http://stackoverflow.com/questions/4114095/how-to-revert-git-repository-to-a-previous-commit) trying to avoid messing up history (*). please let me know if this is not the correct way in this context.\n\n(*) for the record: checked chapter 1 of the Guide, but didn't find this use case.",
    "created_at": "2017-04-15T19:43:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425847",
    "user": "https://github.com/mforets"
}
```

<a id='comment:16'></a>fair enough about naming conventions or handy constructions. as far as i'm concerned, this ticket is ready for peer-review!

side note: i've used `git revert` as suggested [here](http://stackoverflow.com/questions/4114095/how-to-revert-git-repository-to-a-previous-commit) trying to avoid messing up history (*). please let me know if this is not the correct way in this context.

(*) for the record: checked chapter 1 of the Guide, but didn't find this use case.



---

archive/issue_comments_425848.json:
```json
{
    "body": "<a id='comment:17'></a>ps: sorry, next time i'll write the updates message in this place so that you don't receive 2 emails..",
    "created_at": "2017-04-15T19:45:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425848",
    "user": "https://github.com/mforets"
}
```

<a id='comment:17'></a>ps: sorry, next time i'll write the updates message in this place so that you don't receive 2 emails..



---

archive/issue_comments_425849.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2017-04-15T19:45:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425849",
    "user": "https://github.com/mforets"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_425850.json:
```json
{
    "body": "<a id='comment:18'></a>Replying to [mforets](#comment%3A16):\n> side note: i've used `git revert` as suggested [here](http://stackoverflow.com/questions/4114095/how-to-revert-git-repository-to-a-previous-commit) trying to avoid messing up history (*). please let me know if this is not the correct way in this context.\n\n\nYou can do that if you care about preserving in the history that a change was made and rolled back. In a case like this I would just go with the branch that does NOT have the changes  that were rolled back (rebase, or otherwise rebuild your branch into the state you want directly). Rebasing becomes a real problem if other people have branches based on your branch. I don't think that's an issue here.",
    "created_at": "2017-04-15T22:57:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425850",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:18'></a>Replying to [mforets](#comment%3A16):
> side note: i've used `git revert` as suggested [here](http://stackoverflow.com/questions/4114095/how-to-revert-git-repository-to-a-previous-commit) trying to avoid messing up history (*). please let me know if this is not the correct way in this context.


You can do that if you care about preserving in the history that a change was made and rolled back. In a case like this I would just go with the branch that does NOT have the changes  that were rolled back (rebase, or otherwise rebuild your branch into the state you want directly). Rebasing becomes a real problem if other people have branches based on your branch. I don't think that's an issue here.



---

archive/issue_comments_425851.json:
```json
{
    "body": "<a id='comment:19'></a>Replying to [nbruin](#comment%3A18):\n> Replying to [mforets](#comment%3A16):\n> > side note: i've used `git revert` as suggested [here](http://stackoverflow.com/questions/4114095/how-to-revert-git-repository-to-a-previous-commit) trying to avoid messing up history (*). please let me know if this is not the correct way in this context.\n\n> \n> You can do that if you care about preserving in the history that a change was made and rolled back. In a case like this I would just go with the branch that does NOT have the changes  that were rolled back (rebase, or otherwise rebuild your branch into the state you want directly). Rebasing becomes a real problem if other people have branches based on your branch. I don't think that's an issue here.\n\n\nok. may i ask for some extra help here please, like an example? the one in page 19 of the Guide doesn't seem to treat this case. (i did some wrong stuff with git in days84, so i pretend to be more cautious!.)",
    "created_at": "2017-04-16T10:29:05Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425851",
    "user": "https://github.com/mforets"
}
```

<a id='comment:19'></a>Replying to [nbruin](#comment%3A18):
> Replying to [mforets](#comment%3A16):
> > side note: i've used `git revert` as suggested [here](http://stackoverflow.com/questions/4114095/how-to-revert-git-repository-to-a-previous-commit) trying to avoid messing up history (*). please let me know if this is not the correct way in this context.

> 
> You can do that if you care about preserving in the history that a change was made and rolled back. In a case like this I would just go with the branch that does NOT have the changes  that were rolled back (rebase, or otherwise rebuild your branch into the state you want directly). Rebasing becomes a real problem if other people have branches based on your branch. I don't think that's an issue here.


ok. may i ask for some extra help here please, like an example? the one in page 19 of the Guide doesn't seem to treat this case. (i did some wrong stuff with git in days84, so i pretend to be more cautious!.)



---

archive/issue_comments_425852.json:
```json
{
    "body": "<a id='comment:20'></a>Replying to [mforets](#comment%3A19):\n> Replying to [nbruin](#comment%3A18):\n> > Replying to [mforets](#comment%3A16):\n> > > side note: i've used `git revert` as suggested [here](http://stackoverflow.com/questions/4114095/how-to-revert-git-repository-to-a-previous-commit) trying to avoid messing up history (*). please let me know if this is not the correct way in this context.\n\n> > \n> > You can do that if you care about preserving in the history that a change was made and rolled back. In a case like this I would just go with the branch that does NOT have the changes  that were rolled back (rebase, or otherwise rebuild your branch into the state you want directly). Rebasing becomes a real problem if other people have branches based on your branch. I don't think that's an issue here.\n\n> \n> ok. may i ask for some extra help here please, like an example? the one in page 19 of the Guide doesn't seem to treat this case. (i did some wrong stuff with git in days84, so i pretend to be more cautious!.)\n\n\nIn this case it would seem to be that a hard reset (as described on the same page) and a forced push to the branch on this ticket would be fine. Anyone who has work based on your branch will be seriously inconvenienced, but I expect there to be no-one in that group here. Note that your actual change is (-2/+7) lines and that with the revert, you'd be archiving the adding and removal of an additional 42 lines.\n\nAs is described in many places, as soon as you publish a git branch you should be careful  rewriting its history, because you could affect people quite badly. However, in this case, with changes so small and localized, any rebase merge would be really simple (and probably handled automatically), so I'd opt for publishing a cleaner history. People shouldn't really be basing work off branches for tickets that are still developing, unless there's a good reason for it.",
    "created_at": "2017-04-17T18:48:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425852",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:20'></a>Replying to [mforets](#comment%3A19):
> Replying to [nbruin](#comment%3A18):
> > Replying to [mforets](#comment%3A16):
> > > side note: i've used `git revert` as suggested [here](http://stackoverflow.com/questions/4114095/how-to-revert-git-repository-to-a-previous-commit) trying to avoid messing up history (*). please let me know if this is not the correct way in this context.

> > 
> > You can do that if you care about preserving in the history that a change was made and rolled back. In a case like this I would just go with the branch that does NOT have the changes  that were rolled back (rebase, or otherwise rebuild your branch into the state you want directly). Rebasing becomes a real problem if other people have branches based on your branch. I don't think that's an issue here.

> 
> ok. may i ask for some extra help here please, like an example? the one in page 19 of the Guide doesn't seem to treat this case. (i did some wrong stuff with git in days84, so i pretend to be more cautious!.)


In this case it would seem to be that a hard reset (as described on the same page) and a forced push to the branch on this ticket would be fine. Anyone who has work based on your branch will be seriously inconvenienced, but I expect there to be no-one in that group here. Note that your actual change is (-2/+7) lines and that with the revert, you'd be archiving the adding and removal of an additional 42 lines.

As is described in many places, as soon as you publish a git branch you should be careful  rewriting its history, because you could affect people quite badly. However, in this case, with changes so small and localized, any rebase merge would be really simple (and probably handled automatically), so I'd opt for publishing a cleaner history. People shouldn't really be basing work off branches for tickets that are still developing, unless there's a good reason for it.



---

archive/issue_comments_425853.json:
```json
{
    "body": "<a id='comment:21'></a>Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:",
    "created_at": "2017-04-17T21:04:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425853",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:21'></a>Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:



---

archive/issue_comments_425854.json:
```json
{
    "body": "Changing commit from \"7a09c12b7f8b1ff03fdf2093d1f57d41bd622974\" to \"5f1a4da2392e198878f33f3bc8fc118178dd6218\"",
    "created_at": "2017-04-17T21:04:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425854",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "7a09c12b7f8b1ff03fdf2093d1f57d41bd622974" to "5f1a4da2392e198878f33f3bc8fc118178dd6218"



---

archive/issue_comments_425855.json:
```json
{
    "body": "Changing author from \"\" to \"Marcelo Forets\"",
    "created_at": "2017-04-18T16:01:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425855",
    "user": "https://github.com/mforets"
}
```

Changing author from "" to "Marcelo Forets"



---

archive/issue_comments_425856.json:
```json
{
    "body": "<a id='comment:23'></a>`@`nbruin : thanks for your detailed answer concerning git reset. i've done what you suggested.\n\nAll: do you have any further inputs with respect to this ticket? thanks.",
    "created_at": "2017-05-05T12:49:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425856",
    "user": "https://github.com/mforets"
}
```

<a id='comment:23'></a>`@`nbruin : thanks for your detailed answer concerning git reset. i've done what you suggested.

All: do you have any further inputs with respect to this ticket? thanks.



---

archive/issue_comments_425857.json:
```json
{
    "body": "Changing component from calculus to commutative algebra.",
    "created_at": "2017-05-12T07:57:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425857",
    "user": "https://github.com/rwst"
}
```

Changing component from calculus to commutative algebra.



---

archive/issue_comments_425858.json:
```json
{
    "body": "<a id='comment:25'></a>friendly reminder for ticket review -- [seemingly green light from Nils five weeks ago](https://trac.sagemath.org/ticket/22809#comment:8). \n\nthanks",
    "created_at": "2017-05-22T05:39:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425858",
    "user": "https://github.com/mforets"
}
```

<a id='comment:25'></a>friendly reminder for ticket review -- [seemingly green light from Nils five weeks ago](https://trac.sagemath.org/ticket/22809#comment:8). 

thanks



---

archive/issue_comments_425859.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Simon King\"",
    "created_at": "2019-07-26T07:52:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425859",
    "user": "https://github.com/simon-king-jena"
}
```

Changing reviewer from "" to "Simon King"



---

archive/issue_comments_425860.json:
```json
{
    "body": "<a id='comment:26'></a>Although I have never used `polygens`, I'd say it is an obviously reasonable idea to allow the same optional arguments that are available for the polynomial ring constructor. It doesn't break anything, the patchbot is happy, and it seems to me that 2 years ago people just forgot to set it to positive review. Doing it now. If Nils want to have his name as a reviewer added, please go ahead.",
    "created_at": "2019-07-26T07:52:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425860",
    "user": "https://github.com/simon-king-jena"
}
```

<a id='comment:26'></a>Although I have never used `polygens`, I'd say it is an obviously reasonable idea to allow the same optional arguments that are available for the polynomial ring constructor. It doesn't break anything, the patchbot is happy, and it seems to me that 2 years ago people just forgot to set it to positive review. Doing it now. If Nils want to have his name as a reviewer added, please go ahead.



---

archive/issue_comments_425861.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2019-07-26T07:52:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425861",
    "user": "https://github.com/simon-king-jena"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_425862.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2019-07-27T19:51:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425862",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_events_059307.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-07-27T19:51:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/22809#event-59307"
}
```



---

archive/issue_comments_425863.json:
```json
{
    "body": "Changing branch from \"u/mforets/pass_number_of_variables_to_polygens\" to \"5f1a4da2392e198878f33f3bc8fc118178dd6218\"",
    "created_at": "2019-07-27T19:51:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22809",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22809#issuecomment-425863",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/mforets/pass_number_of_variables_to_polygens" to "5f1a4da2392e198878f33f3bc8fc118178dd6218"
