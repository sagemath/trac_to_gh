# Issue 22069: Coefficients of truncated series bug

archive/issues_021832.json:
```json
{
    "assignees": [],
    "body": "This has been reported on \n[AskSM](https://ask.sagemath.org/question/35774/coefficients-of-series/).\n\nAt least in some cases coefficients() does not return the leading coefficient. \n\nExample:\n    g = lambda x, n: (1 + x<sup>2</sup>)<sup>n</sup> + n*x*(1 + x<sup>2</sup>)^(n - 1)^\n\n    g(x,4).series(x,12).coefficients()\nReturns:\n    [ [1, 0], [4, 1], [4, 2], [12, 3], [6, 4], [12, 5], [4, 6], [4, 7] ]\nCompare with the deprecated coeffs() which returns correct values:\n    [ [1, 0], [4, 1], [4, 2], [12, 3], [6, 4], [12, 5], [4, 6], [4, 7], [1, 8] ]\n\n\n\n\n\n\nCC:  @slel\n\nKeywords: **coefficients**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/22069_\n\n",
    "created_at": "2016-12-16T10:57:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Coefficients of truncated series bug",
    "type": "issue",
    "updated_at": "2022-12-29T01:37:47Z",
    "url": "https://github.com/sagemath/sage/issues/22069",
    "user": "https://github.com/PeterLuschny"
}
```
This has been reported on 
[AskSM](https://ask.sagemath.org/question/35774/coefficients-of-series/).

At least in some cases coefficients() does not return the leading coefficient. 

Example:
    g = lambda x, n: (1 + x<sup>2</sup>)<sup>n</sup> + n*x*(1 + x<sup>2</sup>)^(n - 1)^

    g(x,4).series(x,12).coefficients()
Returns:
    [ [1, 0], [4, 1], [4, 2], [12, 3], [6, 4], [12, 5], [4, 6], [4, 7] ]
Compare with the deprecated coeffs() which returns correct values:
    [ [1, 0], [4, 1], [4, 2], [12, 3], [6, 4], [12, 5], [4, 6], [4, 7], [1, 8] ]






CC:  @slel

Keywords: **coefficients**

_Issue created by migration from https://trac.sagemath.org/ticket/22069_





---

archive/issue_events_291102.json:
```json
{
    "actor": "https://github.com/PeterLuschny",
    "created_at": "2016-12-16T10:57:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/22069",
    "milestone_number": null,
    "milestone_title": "sage-7.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22069#event-291102"
}
```



---

archive/issue_events_291103.json:
```json
{
    "actor": "https://github.com/PeterLuschny",
    "created_at": "2016-12-16T10:57:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22069",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22069#event-291103"
}
```



---

archive/issue_events_291104.json:
```json
{
    "actor": "https://github.com/PeterLuschny",
    "created_at": "2016-12-16T10:57:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22069",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22069#event-291104"
}
```



---

archive/issue_events_291105.json:
```json
{
    "actor": "https://github.com/PeterLuschny",
    "created_at": "2016-12-16T10:57:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22069",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22069#event-291105"
}
```



---

archive/issue_comments_328069.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nJust a hint, to expand expressions as `g(x,4)` you better use `expand` instead of `series`. The bug seems associated with truncated series so this is also a workaround.",
    "created_at": "2017-01-19T07:01:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22069",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22069#issuecomment-328069",
    "user": "https://github.com/rwst"
}
```

<a id='comment:1'>Comment 1:</a>
Just a hint, to expand expressions as `g(x,4)` you better use `expand` instead of `series`. The bug seems associated with truncated series so this is also a workaround.



---

archive/issue_events_291106.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-01-19T07:01:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/22069",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebra",
    "label_color": "0000ff",
    "label_name": "component: algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22069#event-291106"
}
```



---

archive/issue_events_291107.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-01-19T07:01:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22069",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22069#event-291107"
}
```



---

archive/issue_events_291108.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-01-19T07:25:29Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/22069",
    "title_is": "Coefficients of truncated series bug",
    "title_was": "Coefficients",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22069#event-291108"
}
```



---

archive/issue_events_291109.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:37:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/22069",
    "milestone_number": null,
    "milestone_title": "sage-7.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22069#event-291109"
}
```
