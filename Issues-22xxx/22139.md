# Issue 22139: Poset: Add certificate to is_[meet|join]_semilattice

archive/issues_021902.json:
```json
{
    "assignees": [],
    "body": "This patch will add a `certificate`-option to functions checking finite poset is a semilattice.\n\nI also suggest removing deprecated `meet_matrix()` and `join_matrix()` for posets (they are not at lattices).\n\n\nCC:  @tscrim\n\nBranch/Commit: **[5364368](https://github.com/sagemath/sagetrac-mirror/commit/53643683480fb5d46e0d29db21631b04f299646b)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Jori M\u00e4ntysalo**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/22139_\n\n",
    "closed_at": "2017-04-05T19:38:00Z",
    "created_at": "2017-01-05T11:59:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Poset: Add certificate to is_[meet|join]_semilattice",
    "type": "issue",
    "updated_at": "2017-04-05T19:38:00Z",
    "url": "https://github.com/sagemath/sage/issues/22139",
    "user": "https://github.com/jm58660"
}
```
This patch will add a `certificate`-option to functions checking finite poset is a semilattice.

I also suggest removing deprecated `meet_matrix()` and `join_matrix()` for posets (they are not at lattices).


CC:  @tscrim

Branch/Commit: **[5364368](https://github.com/sagemath/sagetrac-mirror/commit/53643683480fb5d46e0d29db21631b04f299646b)**

Reviewer: **Travis Scrimshaw**

Author: **Jori Mäntysalo**

_Issue created by migration from https://trac.sagemath.org/ticket/22139_





---

archive/issue_events_292010.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2017-01-05T11:59:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/22139",
    "milestone_number": null,
    "milestone_title": "sage-7.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22139#event-292010"
}
```



---

archive/issue_events_292011.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2017-01-05T11:59:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22139",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22139#event-292011"
}
```



---

archive/issue_events_292012.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2017-01-05T11:59:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22139",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22139#event-292012"
}
```



---

archive/issue_events_292013.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2017-01-05T11:59:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22139",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22139#event-292013"
}
```



---

archive/issue_comments_329164.json:
```json
{
    "body": "Branch: **[u/jmantysalo/semilattice-cert](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/semilattice-cert)**",
    "created_at": "2017-01-05T12:00:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22139#issuecomment-329164",
    "user": "https://github.com/jm58660"
}
```

Branch: **[u/jmantysalo/semilattice-cert](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/semilattice-cert)**



---

archive/issue_events_292014.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2017-01-05T12:11:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22139",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22139#event-292014"
}
```



---

archive/issue_comments_329165.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nThis is kind of needs-review, but if this is OK, I must made the \"dual changes\" meet <-> join.\n\nThere was also a slight problem. Code like\n\n```\ntry:\n    make_some_matrix()\nexcept ValueError:\n    return False\nreturn True\n```\n\nmight -- at least in theory -- return `False` when there is some other error at Matrix creation (out of memory?).\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/db04bf55c6cb339ecc8f1db8ca87d748b827b0ec\">db04bf5</a></td><td><code>Certificate for is_meet_semilattice.</code></td></tr></table>\n",
    "created_at": "2017-01-05T12:11:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22139#issuecomment-329165",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:2" align="right">Comment 2</div>

This is kind of needs-review, but if this is OK, I must made the "dual changes" meet <-> join.

There was also a slight problem. Code like

```
try:
    make_some_matrix()
except ValueError:
    return False
return True
```

might -- at least in theory -- return `False` when there is some other error at Matrix creation (out of memory?).

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/db04bf55c6cb339ecc8f1db8ca87d748b827b0ec">db04bf5</a></td><td><code>Certificate for is_meet_semilattice.</code></td></tr></table>




---

archive/issue_comments_329166.json:
```json
{
    "body": "Commit: **[db04bf5](https://github.com/sagemath/sagetrac-mirror/commit/db04bf55c6cb339ecc8f1db8ca87d748b827b0ec)**",
    "created_at": "2017-01-05T12:11:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22139#issuecomment-329166",
    "user": "https://github.com/jm58660"
}
```

Commit: **[db04bf5](https://github.com/sagemath/sagetrac-mirror/commit/db04bf55c6cb339ecc8f1db8ca87d748b827b0ec)**



---

archive/issue_comments_329167.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nReplying to [@jm58660](#comment%3A2):\n> This is kind of needs-review, but if this is OK, I must made the \"dual changes\" meet <-> join.\n\nTravis, want still another \"add certificate\" -ticket?",
    "created_at": "2017-01-24T04:58:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22139#issuecomment-329167",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:3" align="right">Comment 3</div>

Replying to [@jm58660](#comment%3A2):
> This is kind of needs-review, but if this is OK, I must made the "dual changes" meet <-> join.

Travis, want still another "add certificate" -ticket?



---

archive/issue_comments_329168.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2a09fc71043a0c71a7351e3d9de493a216136c87\">2a09fc7</a></td><td><code>Merge branch 'develop' into t/22139/semilattice-cert</code></td></tr></table>\n",
    "created_at": "2017-04-03T10:51:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22139#issuecomment-329168",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2a09fc71043a0c71a7351e3d9de493a216136c87">2a09fc7</a></td><td><code>Merge branch 'develop' into t/22139/semilattice-cert</code></td></tr></table>




---

archive/issue_comments_329169.json:
```json
{
    "body": "Changed commit from **[db04bf5](https://github.com/sagemath/sagetrac-mirror/commit/db04bf55c6cb339ecc8f1db8ca87d748b827b0ec)** to **[2a09fc7](https://github.com/sagemath/sagetrac-mirror/commit/2a09fc71043a0c71a7351e3d9de493a216136c87)**",
    "created_at": "2017-04-03T10:51:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22139#issuecomment-329169",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[db04bf5](https://github.com/sagemath/sagetrac-mirror/commit/db04bf55c6cb339ecc8f1db8ca87d748b827b0ec)** to **[2a09fc7](https://github.com/sagemath/sagetrac-mirror/commit/2a09fc71043a0c71a7351e3d9de493a216136c87)**



---

archive/issue_events_292015.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2017-04-03T10:52:44Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/22139",
    "milestone_number": null,
    "milestone_title": "sage-7.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22139#event-292015"
}
```



---

archive/issue_events_292016.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2017-04-03T10:52:44Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/22139",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22139#event-292016"
}
```



---

archive/issue_comments_329170.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nI think this should be okay. Just implement the dual version, and we will be good to go.",
    "created_at": "2017-04-03T14:52:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22139#issuecomment-329170",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:6" align="right">Comment 6</div>

I think this should be okay. Just implement the dual version, and we will be good to go.



---

archive/issue_comments_329171.json:
```json
{
    "body": "Changed commit from **[2a09fc7](https://github.com/sagemath/sagetrac-mirror/commit/2a09fc71043a0c71a7351e3d9de493a216136c87)** to **[5364368](https://github.com/sagemath/sagetrac-mirror/commit/53643683480fb5d46e0d29db21631b04f299646b)**",
    "created_at": "2017-04-03T19:11:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22139#issuecomment-329171",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[2a09fc7](https://github.com/sagemath/sagetrac-mirror/commit/2a09fc71043a0c71a7351e3d9de493a216136c87)** to **[5364368](https://github.com/sagemath/sagetrac-mirror/commit/53643683480fb5d46e0d29db21631b04f299646b)**



---

archive/issue_comments_329172.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/53643683480fb5d46e0d29db21631b04f299646b\">5364368</a></td><td><code>Add certificate to is_join_semilattice.</code></td></tr></table>\n",
    "created_at": "2017-04-03T19:11:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22139#issuecomment-329172",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:7" align="right">Comment 7</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/53643683480fb5d46e0d29db21631b04f299646b">5364368</a></td><td><code>Add certificate to is_join_semilattice.</code></td></tr></table>




---

archive/issue_events_292017.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2017-04-03T19:21:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/22139",
    "label": "https://github.com/sagemath/sage/labels/needs%20info",
    "label_color": "ffff00",
    "label_name": "needs info",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22139#event-292017"
}
```



---

archive/issue_events_292018.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2017-04-03T19:21:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22139",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22139#event-292018"
}
```



---

archive/issue_comments_329173.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nReplying to [@tscrim](#comment%3A6):\n> I think this should be okay. Just implement the dual version, and we will be good to go.\n\nDone that.",
    "created_at": "2017-04-03T19:21:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22139#issuecomment-329173",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:8" align="right">Comment 8</div>

Replying to [@tscrim](#comment%3A6):
> I think this should be okay. Just implement the dual version, and we will be good to go.

Done that.



---

archive/issue_events_292019.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-04-03T19:23:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/22139",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22139#event-292019"
}
```



---

archive/issue_events_292020.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-04-03T19:23:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22139",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22139#event-292020"
}
```



---

archive/issue_comments_329174.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2017-04-03T19:23:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22139#issuecomment-329174",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_329175.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nLGTM. Thanks.",
    "created_at": "2017-04-03T19:23:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22139#issuecomment-329175",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:9" align="right">Comment 9</div>

LGTM. Thanks.



---

archive/issue_events_292021.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-04-05T19:38:00Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/22139",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22139#event-292021"
}
```



---

archive/issue_events_292022.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "de87bba583ab79bf4289d7a54e9b77dd7e484473",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-04-05T19:38:00Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/22139",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22139#event-292022"
}
```



---

archive/issue_comments_329176.json:
```json
{
    "body": "Changed branch from **[u/jmantysalo/semilattice-cert](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/semilattice-cert)** to **[5364368](https://github.com/sagemath/sagetrac-mirror/commit/53643683480fb5d46e0d29db21631b04f299646b)**",
    "created_at": "2017-04-05T19:38:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22139",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22139#issuecomment-329176",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jmantysalo/semilattice-cert](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/semilattice-cert)** to **[5364368](https://github.com/sagemath/sagetrac-mirror/commit/53643683480fb5d46e0d29db21631b04f299646b)**
