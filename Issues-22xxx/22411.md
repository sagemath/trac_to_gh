# Issue 22411: Improve handling of MtxLibDir

archive/issues_022174.json:
```json
{
    "assignees": [],
    "body": "\n\nCC:  @simon-king-jena\n\nBranch/Commit: **[1a9ac6f](https://github.com/sagemath/sagetrac-mirror/commit/1a9ac6f1eb7b4c1da8e14864beae802b75636c2c)**\n\nReviewer: **Simon King, Travis Scrimshaw**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/22411_\n\n",
    "closed_at": "2017-05-21T21:51:54Z",
    "created_at": "2017-02-22T09:26:41Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Improve handling of MtxLibDir",
    "type": "issue",
    "updated_at": "2017-05-21T21:51:54Z",
    "url": "https://github.com/sagemath/sage/issues/22411",
    "user": "https://github.com/jdemeyer"
}
```


CC:  @simon-king-jena

Branch/Commit: **[1a9ac6f](https://github.com/sagemath/sagetrac-mirror/commit/1a9ac6f1eb7b4c1da8e14864beae802b75636c2c)**

Reviewer: **Simon King, Travis Scrimshaw**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/22411_





---

archive/issue_events_295386.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-02-22T09:26:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/22411",
    "milestone_number": null,
    "milestone_title": "sage-7.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22411#event-295386"
}
```



---

archive/issue_events_295387.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-02-22T09:26:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22411",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
    "label_color": "000080",
    "label_name": "component: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22411#event-295387"
}
```



---

archive/issue_events_295388.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-02-22T09:26:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22411",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22411#event-295388"
}
```



---

archive/issue_events_295389.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-02-22T09:26:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22411",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22411#event-295389"
}
```



---

archive/issue_comments_334066.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nIsn't that part of #20136 or #21437?",
    "created_at": "2017-02-22T09:55:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22411#issuecomment-334066",
    "user": "https://github.com/simon-king-jena"
}
```

<div id="comment:1" align="right">Comment 1</div>

Isn't that part of #20136 or #21437?



---

archive/issue_comments_334067.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nReplying to [@simon-king-jena](#comment%3A1):\n> Isn't that part of #20136 or #21437?\n\nMaybe it could be made part of those. It will certainly conflict with #21437.\n\nSince I'm already working on this anyway, let me continue and then look at what should happen.",
    "created_at": "2017-02-22T10:00:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22411#issuecomment-334067",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

Replying to [@simon-king-jena](#comment%3A1):
> Isn't that part of #20136 or #21437?

Maybe it could be made part of those. It will certainly conflict with #21437.

Since I'm already working on this anyway, let me continue and then look at what should happen.



---

archive/issue_comments_334068.json:
```json
{
    "body": "Branch: **[u/jdemeyer/improve_handling_of_mtxlibdir](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/improve_handling_of_mtxlibdir)**",
    "created_at": "2017-02-22T10:10:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22411#issuecomment-334068",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/improve_handling_of_mtxlibdir](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/improve_handling_of_mtxlibdir)**



---

archive/issue_comments_334069.json:
```json
{
    "body": "Commit: **[1a9ac6f](https://github.com/sagemath/sagetrac-mirror/commit/1a9ac6f1eb7b4c1da8e14864beae802b75636c2c)**",
    "created_at": "2017-02-22T10:16:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22411#issuecomment-334069",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[1a9ac6f](https://github.com/sagemath/sagetrac-mirror/commit/1a9ac6f1eb7b4c1da8e14864beae802b75636c2c)**



---

archive/issue_events_295390.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-02-22T10:16:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22411",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22411#event-295390"
}
```



---

archive/issue_comments_334070.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nI'm adding a simple patch here which makes sense by itself. Further changes will be for other tickets.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1a9ac6f1eb7b4c1da8e14864beae802b75636c2c\">1a9ac6f</a></td><td><code>Declare MtxLibDir size in header file + increase to 1024 bytes</code></td></tr></table>\n",
    "created_at": "2017-02-22T10:16:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22411#issuecomment-334070",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:4" align="right">Comment 4</div>

I'm adding a simple patch here which makes sense by itself. Further changes will be for other tickets.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1a9ac6f1eb7b4c1da8e14864beae802b75636c2c">1a9ac6f</a></td><td><code>Declare MtxLibDir size in header file + increase to 1024 bytes</code></td></tr></table>




---

archive/issue_comments_334071.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nLGTM. But would the patchbots test this with the optional meataxe package installed?",
    "created_at": "2017-02-22T14:45:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22411#issuecomment-334071",
    "user": "https://github.com/simon-king-jena"
}
```

<div id="comment:5" align="right">Comment 5</div>

LGTM. But would the patchbots test this with the optional meataxe package installed?



---

archive/issue_comments_334072.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nReplying to [@simon-king-jena](#comment%3A5):\n> LGTM. But would the patchbots test this with the optional meataxe package installed?\n\nOnly the patchbots which have meataxe installed. I believe that `arando` does at least.",
    "created_at": "2017-02-23T07:29:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22411#issuecomment-334072",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:6" align="right">Comment 6</div>

Replying to [@simon-king-jena](#comment%3A5):
> LGTM. But would the patchbots test this with the optional meataxe package installed?

Only the patchbots which have meataxe installed. I believe that `arando` does at least.



---

archive/issue_comments_334073.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nTested and everything seems to work.",
    "created_at": "2017-05-21T02:23:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22411#issuecomment-334073",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:7" align="right">Comment 7</div>

Tested and everything seems to work.



---

archive/issue_comments_334074.json:
```json
{
    "body": "Reviewer: **Simon King, Travis Scrimshaw**",
    "created_at": "2017-05-21T02:23:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22411#issuecomment-334074",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Simon King, Travis Scrimshaw**



---

archive/issue_events_295391.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-05-21T02:23:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/22411",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22411#event-295391"
}
```



---

archive/issue_events_295392.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-05-21T02:23:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22411",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22411#event-295392"
}
```



---

archive/issue_events_295393.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-05-21T02:23:50Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/22411",
    "milestone_number": null,
    "milestone_title": "sage-7.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22411#event-295393"
}
```



---

archive/issue_events_295394.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-05-21T02:23:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/22411",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22411#event-295394"
}
```



---

archive/issue_events_295395.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-05-21T21:51:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/22411",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22411#event-295395"
}
```



---

archive/issue_events_295396.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "77bb055d60f374993e5419f5b7c0c2e691bfb1ac",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-05-21T21:51:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/22411",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22411#event-295396"
}
```



---

archive/issue_comments_334075.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/improve_handling_of_mtxlibdir](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/improve_handling_of_mtxlibdir)** to **[1a9ac6f](https://github.com/sagemath/sagetrac-mirror/commit/1a9ac6f1eb7b4c1da8e14864beae802b75636c2c)**",
    "created_at": "2017-05-21T21:51:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22411",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22411#issuecomment-334075",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/improve_handling_of_mtxlibdir](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/improve_handling_of_mtxlibdir)** to **[1a9ac6f](https://github.com/sagemath/sagetrac-mirror/commit/1a9ac6f1eb7b4c1da8e14864beae802b75636c2c)**
