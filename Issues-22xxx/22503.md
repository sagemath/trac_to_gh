# Issue 22503: Fix treatment of symbolic derivatives in simplification of coordinate functions

archive/issues_022266.json:
```json
{
    "assignees": [],
    "body": "This ticket fixes a bug that appeared in `src/sage/manifolds/utilities/simplify_sqrt_real` after the introduction of `diff(...)` instead of `D[...]` to denote derivatives of symbolic functions. This bug has been reported at https://ask.sagemath.org/question/36777/covariant-derivative-gives-error-why-sage-751/ and the `diff(...)` issue is discussed at https://groups.google.com/d/msg/sage-support/lZ4AjbmvvQE/4ZZddjLTBAAJ.\n\nKeywords: **manifolds, coordinate functions**\n\nBranch/Commit: **[2207350](https://github.com/sagemath/sagetrac-mirror/commit/220735012b437440115779f9a06b5f6b2bcf32fb)**\n\nReviewer: **Micha\u0142\u00a0Bejger**\n\nAuthor: **Eric Gourgoulhon**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/22503_\n\n",
    "closed_at": "2017-03-06T23:35:55Z",
    "created_at": "2017-03-02T21:00:10Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix treatment of symbolic derivatives in simplification of coordinate functions",
    "type": "issue",
    "updated_at": "2017-03-06T23:35:55Z",
    "url": "https://github.com/sagemath/sage/issues/22503",
    "user": "https://github.com/egourgoulhon"
}
```
This ticket fixes a bug that appeared in `src/sage/manifolds/utilities/simplify_sqrt_real` after the introduction of `diff(...)` instead of `D[...]` to denote derivatives of symbolic functions. This bug has been reported at https://ask.sagemath.org/question/36777/covariant-derivative-gives-error-why-sage-751/ and the `diff(...)` issue is discussed at https://groups.google.com/d/msg/sage-support/lZ4AjbmvvQE/4ZZddjLTBAAJ.

Keywords: **manifolds, coordinate functions**

Branch/Commit: **[2207350](https://github.com/sagemath/sagetrac-mirror/commit/220735012b437440115779f9a06b5f6b2bcf32fb)**

Reviewer: **Michał Bejger**

Author: **Eric Gourgoulhon**

_Issue created by migration from https://trac.sagemath.org/ticket/22503_





---

archive/issue_events_296554.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2017-03-02T21:00:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/22503",
    "milestone_number": null,
    "milestone_title": "sage-7.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22503#event-296554"
}
```



---

archive/issue_events_296555.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2017-03-02T21:00:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22503",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "component: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22503#event-296555"
}
```



---

archive/issue_events_296556.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2017-03-02T21:00:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22503",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22503#event-296556"
}
```



---

archive/issue_events_296557.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2017-03-02T21:00:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22503",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22503#event-296557"
}
```



---

archive/issue_comments_335973.json:
```json
{
    "body": "Commit: **[2207350](https://github.com/sagemath/sagetrac-mirror/commit/220735012b437440115779f9a06b5f6b2bcf32fb)**",
    "created_at": "2017-03-02T21:08:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22503",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22503#issuecomment-335973",
    "user": "https://github.com/egourgoulhon"
}
```

Commit: **[2207350](https://github.com/sagemath/sagetrac-mirror/commit/220735012b437440115779f9a06b5f6b2bcf32fb)**



---

archive/issue_comments_335974.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/220735012b437440115779f9a06b5f6b2bcf32fb\">2207350</a></td><td><code>Fix bug with diff(...) in simplify_sqrt_real</code></td></tr></table>\n",
    "created_at": "2017-03-02T21:08:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22503",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22503#issuecomment-335974",
    "user": "https://github.com/egourgoulhon"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/220735012b437440115779f9a06b5f6b2bcf32fb">2207350</a></td><td><code>Fix bug with diff(...) in simplify_sqrt_real</code></td></tr></table>




---

archive/issue_comments_335975.json:
```json
{
    "body": "Branch: **[public/manifolds/bug-22503](https://github.com/sagemath/sagetrac-mirror/tree/public/manifolds/bug-22503)**",
    "created_at": "2017-03-02T21:08:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22503",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22503#issuecomment-335975",
    "user": "https://github.com/egourgoulhon"
}
```

Branch: **[public/manifolds/bug-22503](https://github.com/sagemath/sagetrac-mirror/tree/public/manifolds/bug-22503)**



---

archive/issue_events_296558.json:
```json
{
    "actor": "https://github.com/egourgoulhon",
    "created_at": "2017-03-02T21:10:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22503",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22503#event-296558"
}
```



---

archive/issue_comments_335976.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nProposed fix handles the problem.",
    "created_at": "2017-03-05T15:05:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22503",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22503#issuecomment-335976",
    "user": "https://github.com/sagetrac-mbejger"
}
```

<div id="comment:3" align="right">Comment 3</div>

Proposed fix handles the problem.



---

archive/issue_events_296559.json:
```json
{
    "actor": "https://github.com/sagetrac-mbejger",
    "created_at": "2017-03-05T15:05:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/22503",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22503#event-296559"
}
```



---

archive/issue_events_296560.json:
```json
{
    "actor": "https://github.com/sagetrac-mbejger",
    "created_at": "2017-03-05T15:05:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22503",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22503#event-296560"
}
```



---

archive/issue_comments_335977.json:
```json
{
    "body": "Reviewer: **Micha\u0142\u00a0Bejger**",
    "created_at": "2017-03-05T15:23:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22503",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22503#issuecomment-335977",
    "user": "https://github.com/sagetrac-mbejger"
}
```

Reviewer: **Michał Bejger**



---

archive/issue_comments_335978.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThank you Michal for the review!",
    "created_at": "2017-03-05T15:49:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22503",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22503#issuecomment-335978",
    "user": "https://github.com/egourgoulhon"
}
```

<div id="comment:5" align="right">Comment 5</div>

Thank you Michal for the review!



---

archive/issue_events_296561.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-03-06T23:35:55Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/22503",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22503#event-296561"
}
```



---

archive/issue_events_296562.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "0079d2771c1b8290b734234e5211a18a4a46ec10",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-03-06T23:35:55Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/22503",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22503#event-296562"
}
```



---

archive/issue_comments_335979.json:
```json
{
    "body": "Changed branch from **[public/manifolds/bug-22503](https://github.com/sagemath/sagetrac-mirror/tree/public/manifolds/bug-22503)** to **[2207350](https://github.com/sagemath/sagetrac-mirror/commit/220735012b437440115779f9a06b5f6b2bcf32fb)**",
    "created_at": "2017-03-06T23:35:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22503",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22503#issuecomment-335979",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/manifolds/bug-22503](https://github.com/sagemath/sagetrac-mirror/tree/public/manifolds/bug-22503)** to **[2207350](https://github.com/sagemath/sagetrac-mirror/commit/220735012b437440115779f9a06b5f6b2bcf32fb)**
