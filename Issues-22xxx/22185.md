# Issue 22185: Rename gen -> Gen

archive/issues_021948.json:
```json
{
    "assignees": [],
    "body": "For consistency, use a capital for the class name.\n\nThis will also allow `from .gen import Gen` in `__init__.py` (which doesn't work if the class has the same name as the module).\n\nDepends on #21756\nDepends on #21806\nDepends on #21807\nDepends on #21808\nDepends on #21809\nDepends on #21810\nDepends on #22183\n\nCC:  @defeo @videlec\n\nKeywords: **atelierpari2017**\n\nBranch/Commit: **[0c4433c](https://github.com/sagemath/sagetrac-mirror/commit/0c4433cf66fb6d9f5fcb5b4429d9f25978008f36)**\n\nReviewer: **Vincent Delecroix**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/22185_\n\n",
    "closed_at": "2017-01-23T22:52:36Z",
    "created_at": "2017-01-13T10:38:39Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Rename gen -> Gen",
    "type": "issue",
    "updated_at": "2017-01-23T22:52:36Z",
    "url": "https://github.com/sagemath/sage/issues/22185",
    "user": "https://github.com/jdemeyer"
}
```
For consistency, use a capital for the class name.

This will also allow `from .gen import Gen` in `__init__.py` (which doesn't work if the class has the same name as the module).

Depends on #21756
Depends on #21806
Depends on #21807
Depends on #21808
Depends on #21809
Depends on #21810
Depends on #22183

CC:  @defeo @videlec

Keywords: **atelierpari2017**

Branch/Commit: **[0c4433c](https://github.com/sagemath/sagetrac-mirror/commit/0c4433cf66fb6d9f5fcb5b4429d9f25978008f36)**

Reviewer: **Vincent Delecroix**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/22185_





---

archive/issue_events_292675.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-01-13T10:38:39Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/22185",
    "milestone_number": null,
    "milestone_title": "sage-7.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22185#event-292675"
}
```



---

archive/issue_events_292676.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-01-13T10:38:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22185",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20interfaces",
    "label_color": "0000ff",
    "label_name": "component: interfaces",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22185#event-292676"
}
```



---

archive/issue_events_292677.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-01-13T10:38:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22185",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22185#event-292677"
}
```



---

archive/issue_events_292678.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-01-13T10:38:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22185",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22185#event-292678"
}
```



---

archive/issue_comments_330304.json:
```json
{
    "body": "Changed keywords from **** to **atelierpari2017**",
    "created_at": "2017-01-13T11:03:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22185#issuecomment-330304",
    "user": "https://github.com/jdemeyer"
}
```

Changed keywords from **** to **atelierpari2017**



---

archive/issue_comments_330305.json:
```json
{
    "body": "Branch: **[u/jdemeyer/ticket/22185](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/22185)**",
    "created_at": "2017-01-13T11:05:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22185#issuecomment-330305",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/ticket/22185](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/22185)**



---

archive/issue_comments_330306.json:
```json
{
    "body": "Commit: **[0c4433c](https://github.com/sagemath/sagetrac-mirror/commit/0c4433cf66fb6d9f5fcb5b4429d9f25978008f36)**",
    "created_at": "2017-01-13T22:59:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22185#issuecomment-330306",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[0c4433c](https://github.com/sagemath/sagetrac-mirror/commit/0c4433cf66fb6d9f5fcb5b4429d9f25978008f36)**



---

archive/issue_comments_330307.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0c4433cf66fb6d9f5fcb5b4429d9f25978008f36\">0c4433c</a></td><td><code>Fix documentation</code></td></tr></table>\n",
    "created_at": "2017-01-13T22:59:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22185#issuecomment-330307",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0c4433cf66fb6d9f5fcb5b4429d9f25978008f36">0c4433c</a></td><td><code>Fix documentation</code></td></tr></table>




---

archive/issue_events_292679.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-01-13T23:00:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22185",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22185#event-292679"
}
```



---

archive/issue_events_292680.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2017-01-18T00:25:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/22185",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22185#event-292680"
}
```



---

archive/issue_events_292681.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2017-01-18T00:25:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22185",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22185#event-292681"
}
```



---

archive/issue_comments_330308.json:
```json
{
    "body": "Reviewer: **Vincent Delecroix**",
    "created_at": "2017-01-18T00:25:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22185#issuecomment-330308",
    "user": "https://github.com/videlec"
}
```

Reviewer: **Vincent Delecroix**



---

archive/issue_comments_330309.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nThanks!",
    "created_at": "2017-01-18T10:09:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22185#issuecomment-330309",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:7'>Comment 7:</a>
Thanks!



---

archive/issue_events_292682.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-01-23T22:52:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/22185",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22185#event-292682"
}
```



---

archive/issue_events_292683.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "248190108f55df98ef9ca2b2b1f7d927faa806c0",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-01-23T22:52:36Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/22185",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22185#event-292683"
}
```



---

archive/issue_comments_330310.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/ticket/22185](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/22185)** to **[0c4433c](https://github.com/sagemath/sagetrac-mirror/commit/0c4433cf66fb6d9f5fcb5b4429d9f25978008f36)**",
    "created_at": "2017-01-23T22:52:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22185",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22185#issuecomment-330310",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/ticket/22185](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/22185)** to **[0c4433c](https://github.com/sagemath/sagetrac-mirror/commit/0c4433cf66fb6d9f5fcb5b4429d9f25978008f36)**
