...
```json
{
    "body": "No dependency on #22886 anymore (it is needed for cylinbox but it is fine for this ticket).\n\nI cleaned the flint interface a bit more than what you asked for.",
...
```json
{
    "body": "I was wrong (and you were right). There is still `def _poly_linbox(self, typ, var='x'):` as a method of `Matrix_integer_dense`.",
...
```json
{
    "body": "All right: `Matrix_integer_dense` cleaned and interface fixed on corner cases with commit `fde1353`.",
...
```json
{
    "body": "And patchbot is happy (excepted some timeout I am not responsible for).",
...
```json
{
    "body": "In `linbox_flint_interface.pyx` lines 84 and 157, you seem to use the method `size` of `givvector` to get the degree of the corresponding polynomial. While this is very likely to be correct in the current code, the specification do not require that the size of the vector storing the polynomial is ajusted to its degree. One should rather use `Givaro::Poly1Dom::degree()` instead.",
...
```json
{
    "body": "I implemented your remarks. Though, I do not think that `6b45893` is reasonable.",
...
```json
{
    "body": "Also, I see some new code not directly related to the subject of the ticket (e.g. in `_rational_kernel_flint`, `rational_kernel_iml`), but this is rather ok for me as it is documentation improvement on routines related to the ticket).",
...
```json
{
    "body": "Tell me if there are other improvements to be done on this ticket.",
...
```json
{
    "body": "Ok, the ticket is now fine for me. -> positive review.",
    "created_at": "2017-05-10T09:33:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22924",
    "type": "issue_comment",
...
```json
{
    "body": "Sorry, two minor documentation typos to be fixed in `src/sage/libs/linbox/linbox_flint_interface.pyx`: \n* line 10 \"linbox-Sage.C\" -> \"linbox-sage.C\"\n* line 128 \"preformed\" -> \"performed\"\nBack in needs work.",
    "created_at": "2017-05-10T12:53:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22924",
    "type": "issue_comment",
...
```json
{
    "body": "Fine then, back to positive review.",
