# Issue 22465: Doctest continuation marker / several files / 3

archive/issues_022228.json:
```json
{
    "assignees": [],
    "body": "This is the last patch about doctest continuation markers at `src/sage`. (There are some left at `src/doc`.)\n\nIn few cases this corrects a bug where a doctest does not really check for right output.\n\n\nCC:  @tscrim\n\nKeywords: **days85**\n\nBranch/Commit: **[4e7d27a](https://github.com/sagemath/sagetrac-mirror/commit/4e7d27a4cfcf1b0a8d591fbd46c98e66ec8bf2ce)**\n\nReviewer: **Julian R\u00fcth**\n\nAuthor: **Jori M\u00e4ntysalo**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/22465_\n\n",
    "closed_at": "2017-04-03T20:59:09Z",
    "created_at": "2017-02-27T18:48:12Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Doctest continuation marker / several files / 3",
    "type": "issue",
    "updated_at": "2017-04-03T20:59:09Z",
    "url": "https://github.com/sagemath/sage/issues/22465",
    "user": "https://github.com/jm58660"
}
```
This is the last patch about doctest continuation markers at `src/sage`. (There are some left at `src/doc`.)

In few cases this corrects a bug where a doctest does not really check for right output.


CC:  @tscrim

Keywords: **days85**

Branch/Commit: **[4e7d27a](https://github.com/sagemath/sagetrac-mirror/commit/4e7d27a4cfcf1b0a8d591fbd46c98e66ec8bf2ce)**

Reviewer: **Julian Rüth**

Author: **Jori Mäntysalo**

_Issue created by migration from https://trac.sagemath.org/ticket/22465_





---

archive/issue_events_296102.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2017-02-27T18:48:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/22465",
    "milestone_number": null,
    "milestone_title": "sage-7.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22465#event-296102"
}
```



---

archive/issue_events_296103.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2017-02-27T18:48:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22465",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22465#event-296103"
}
```



---

archive/issue_events_296104.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2017-02-27T18:48:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22465",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22465#event-296104"
}
```



---

archive/issue_events_296105.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2017-02-27T18:48:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22465",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22465#event-296105"
}
```



---

archive/issue_comments_335304.json:
```json
{
    "body": "Branch: **[u/jmantysalo/doctest-continuation-last](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/doctest-continuation-last)**",
    "created_at": "2017-02-27T18:48:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22465",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22465#issuecomment-335304",
    "user": "https://github.com/jm58660"
}
```

Branch: **[u/jmantysalo/doctest-continuation-last](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/doctest-continuation-last)**



---

archive/issue_comments_335305.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nIs it possible to forbid old-style continuation markers? That would prevent some doctest errors.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4db20c0eb91d44268ace513ee2ba4071d80fb5b1\">4db20c0</a></td><td><code>Last doctest continuation marker change on src/sage.</code></td></tr></table>\n",
    "created_at": "2017-02-27T18:53:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22465",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22465#issuecomment-335305",
    "user": "https://github.com/jm58660"
}
```

<a id='comment:2'>Comment 2:</a>
Is it possible to forbid old-style continuation markers? That would prevent some doctest errors.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4db20c0eb91d44268ace513ee2ba4071d80fb5b1">4db20c0</a></td><td><code>Last doctest continuation marker change on src/sage.</code></td></tr></table>




---

archive/issue_comments_335306.json:
```json
{
    "body": "Commit: **[4db20c0](https://github.com/sagemath/sagetrac-mirror/commit/4db20c0eb91d44268ace513ee2ba4071d80fb5b1)**",
    "created_at": "2017-02-27T18:53:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22465",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22465#issuecomment-335306",
    "user": "https://github.com/jm58660"
}
```

Commit: **[4db20c0](https://github.com/sagemath/sagetrac-mirror/commit/4db20c0eb91d44268ace513ee2ba4071d80fb5b1)**



---

archive/issue_events_296106.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2017-02-27T18:53:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22465",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22465#event-296106"
}
```



---

archive/issue_comments_335307.json:
```json
{
    "body": "Changed commit from **[4db20c0](https://github.com/sagemath/sagetrac-mirror/commit/4db20c0eb91d44268ace513ee2ba4071d80fb5b1)** to **[4e7d27a](https://github.com/sagemath/sagetrac-mirror/commit/4e7d27a4cfcf1b0a8d591fbd46c98e66ec8bf2ce)**",
    "created_at": "2017-03-14T07:12:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22465",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22465#issuecomment-335307",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[4db20c0](https://github.com/sagemath/sagetrac-mirror/commit/4db20c0eb91d44268ace513ee2ba4071d80fb5b1)** to **[4e7d27a](https://github.com/sagemath/sagetrac-mirror/commit/4e7d27a4cfcf1b0a8d591fbd46c98e66ec8bf2ce)**



---

archive/issue_comments_335308.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4e7d27a4cfcf1b0a8d591fbd46c98e66ec8bf2ce\">4e7d27a</a></td><td><code>Merge branch 'develop' into t/22465/doctest-continuation-last</code></td></tr></table>\n",
    "created_at": "2017-03-14T07:12:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22465",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22465#issuecomment-335308",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:3'>Comment 3:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4e7d27a4cfcf1b0a8d591fbd46c98e66ec8bf2ce">4e7d27a</a></td><td><code>Merge branch 'develop' into t/22465/doctest-continuation-last</code></td></tr></table>




---

archive/issue_comments_335309.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nI am happy with this, assuming that the patchbots do not complain.",
    "created_at": "2017-03-14T15:26:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22465",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22465#issuecomment-335309",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:4'>Comment 4:</a>
I am happy with this, assuming that the patchbots do not complain.



---

archive/issue_events_296107.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-03-14T15:26:32Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/22465",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22465#event-296107"
}
```



---

archive/issue_events_296108.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2017-03-14T15:26:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22465",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22465#event-296108"
}
```



---

archive/issue_comments_335310.json:
```json
{
    "body": "Reviewer: **Julian R\u00fcth**",
    "created_at": "2017-03-14T15:26:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22465",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22465#issuecomment-335310",
    "user": "https://github.com/saraedum"
}
```

Reviewer: **Julian Rüth**



---

archive/issue_comments_335311.json:
```json
{
    "body": "Changed keywords from **** to **days85**",
    "created_at": "2017-03-14T16:11:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22465",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22465#issuecomment-335311",
    "user": "https://github.com/saraedum"
}
```

Changed keywords from **** to **days85**



---

archive/issue_comments_335312.json:
```json
{
    "body": "Changed branch from **[u/jmantysalo/doctest-continuation-last](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/doctest-continuation-last)** to **[4e7d27a](https://github.com/sagemath/sagetrac-mirror/commit/4e7d27a4cfcf1b0a8d591fbd46c98e66ec8bf2ce)**",
    "created_at": "2017-04-03T20:59:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22465",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22465#issuecomment-335312",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jmantysalo/doctest-continuation-last](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/doctest-continuation-last)** to **[4e7d27a](https://github.com/sagemath/sagetrac-mirror/commit/4e7d27a4cfcf1b0a8d591fbd46c98e66ec8bf2ce)**



---

archive/issue_events_296109.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-04-03T20:59:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/22465",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22465#event-296109"
}
```



---

archive/issue_events_296110.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "2b13d2c47b17dca6b2d5c801504e56d35c52b424",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-04-03T20:59:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/22465",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22465#event-296110"
}
```
