# Issue 22692: LONG_LONG_MAX is obsolete

archive/issues_022455.json:
```json
{
    "assignees": [],
    "body": "LONG_LONG_MAX/MIN are [obsolete](https://www.gnu.org/software/libc/manual/html_node/Range-of-Type.html) (and lead to impossibility to build Sage with clang 3.8 on FreeBSD).\n\nLLONG_MAX/MIN is understood by gcc 4 or better, so we can safely fix this.\n\nCC:  @kiwifb\n\nBranch/Commit: **[0b2b060](https://github.com/sagemath/sagetrac-mirror/commit/0b2b060cfa1b4658628168a19b41db65a85a9253)**\n\nReviewer: **Fran\u00e7ois Bissey**\n\nAuthor: **Dima Pasechnik**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/22692_\n\n",
    "closed_at": "2017-04-03T20:59:36Z",
    "created_at": "2017-03-27T21:19:46Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20porting",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "LONG_LONG_MAX is obsolete",
    "type": "issue",
    "updated_at": "2017-04-03T20:59:36Z",
    "url": "https://github.com/sagemath/sage/issues/22692",
    "user": "https://github.com/dimpase"
}
```
LONG_LONG_MAX/MIN are [obsolete](https://www.gnu.org/software/libc/manual/html_node/Range-of-Type.html) (and lead to impossibility to build Sage with clang 3.8 on FreeBSD).

LLONG_MAX/MIN is understood by gcc 4 or better, so we can safely fix this.

CC:  @kiwifb

Branch/Commit: **[0b2b060](https://github.com/sagemath/sagetrac-mirror/commit/0b2b060cfa1b4658628168a19b41db65a85a9253)**

Reviewer: **François Bissey**

Author: **Dima Pasechnik**

_Issue created by migration from https://trac.sagemath.org/ticket/22692_





---

archive/issue_events_298933.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2017-03-27T21:19:46Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/22692",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22692#event-298933"
}
```



---

archive/issue_events_298934.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2017-03-27T21:19:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22692",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20porting",
    "label_color": "000080",
    "label_name": "component: porting",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22692#event-298934"
}
```



---

archive/issue_events_298935.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2017-03-27T21:19:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22692",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22692#event-298935"
}
```



---

archive/issue_events_298936.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2017-03-27T21:19:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22692",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22692#event-298936"
}
```



---

archive/issue_events_298937.json:
```json
{
    "actor": "https://github.com/dimpase",
    "created_at": "2017-03-27T21:20:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22692",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22692#event-298937"
}
```



---

archive/issue_comments_340155.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nLooks good to me.",
    "created_at": "2017-03-27T21:26:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22692",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22692#issuecomment-340155",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:2" align="right">Comment 2</div>

Looks good to me.



---

archive/issue_events_298938.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2017-03-27T21:26:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/22692",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22692#event-298938"
}
```



---

archive/issue_events_298939.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2017-03-27T21:26:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22692",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22692#event-298939"
}
```



---

archive/issue_comments_340156.json:
```json
{
    "body": "Reviewer: **Fran\u00e7ois Bissey**",
    "created_at": "2017-03-27T21:26:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22692",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22692#issuecomment-340156",
    "user": "https://github.com/kiwifb"
}
```

Reviewer: **François Bissey**



---

archive/issue_events_298940.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-04-03T20:59:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/22692",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22692#event-298940"
}
```



---

archive/issue_events_298941.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-04-03T20:59:36Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/22692",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22692#event-298941"
}
```



---

archive/issue_comments_340157.json:
```json
{
    "body": "Changed branch from **[u/dimpase/llongfix](https://github.com/sagemath/sagetrac-mirror/tree/u/dimpase/llongfix)** to **[0b2b060](https://github.com/sagemath/sagetrac-mirror/commit/0b2b060cfa1b4658628168a19b41db65a85a9253)**",
    "created_at": "2017-04-03T20:59:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22692",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22692#issuecomment-340157",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/dimpase/llongfix](https://github.com/sagemath/sagetrac-mirror/tree/u/dimpase/llongfix)** to **[0b2b060](https://github.com/sagemath/sagetrac-mirror/commit/0b2b060cfa1b4658628168a19b41db65a85a9253)**
