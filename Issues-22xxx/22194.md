...
```json
{
    "body": "<a id='comment:1'></a>Yep, something is going wrong with either the coercion or in `_element_constructor_` as these are almost instantaneous:\n\n```\nsage: phi = R2.coerce_map_from(R1)\nsage: R2._element_constructor_??\nsage: phi(x)\n1\nsage: phi(x) * y\nx0\nsage: R2(x) * y\nx0\n```",
...
<a id='comment:1'></a>Yep, something is going wrong with either the coercion or in `_element_constructor_` as these are almost instantaneous:
...
```json
{
    "body": "<a id='comment:2'></a>My guess is probably in the `_element_constructor_`.",
...
<a id='comment:2'></a>My guess is probably in the `_element_constructor_`.
...
```json
{
    "body": "<a id='comment:3'></a>Duplicate of #23906.",
    "created_at": "2018-03-08T10:14:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22194",
    "type": "issue_comment",
...
<a id='comment:3'></a>Duplicate of #23906.
