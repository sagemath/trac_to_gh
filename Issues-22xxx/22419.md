# Issue 22419: Allow custom CXXFLAGS for pynac

archive/issues_022182.json:
```json
{
    "assignees": [],
    "body": "The `spkg-install` file of Pynac overrides `CXXFLAGS=\"-O2 -g\"`, ignoring any flags which might be set by the user.\n\nCC:  @rwst\n\nBranch/Commit: **[4c2a96a](https://github.com/sagemath/sagetrac-mirror/commit/4c2a96ac5168b1854117b61130df19e81c964207)**\n\nReviewer: **Ralf Stephan**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/22419_\n\n",
    "closed_at": "2017-02-26T15:33:54Z",
    "created_at": "2017-02-23T09:20:18Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Allow custom CXXFLAGS for pynac",
    "type": "issue",
    "updated_at": "2017-02-26T15:33:54Z",
    "url": "https://github.com/sagemath/sage/issues/22419",
    "user": "https://github.com/jdemeyer"
}
```
The `spkg-install` file of Pynac overrides `CXXFLAGS="-O2 -g"`, ignoring any flags which might be set by the user.

CC:  @rwst

Branch/Commit: **[4c2a96a](https://github.com/sagemath/sagetrac-mirror/commit/4c2a96ac5168b1854117b61130df19e81c964207)**

Reviewer: **Ralf Stephan**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/22419_





---

archive/issue_events_295461.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-02-23T09:20:18Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/22419",
    "milestone_number": null,
    "milestone_title": "sage-7.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22419#event-295461"
}
```



---

archive/issue_events_295462.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-02-23T09:20:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22419",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22419#event-295462"
}
```



---

archive/issue_events_295463.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-02-23T09:20:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22419",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22419#event-295463"
}
```



---

archive/issue_events_295464.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-02-23T09:20:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22419",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22419#event-295464"
}
```



---

archive/issue_comments_334182.json:
```json
{
    "body": "Branch: **[u/jdemeyer/allow_custom_cxxflags_for_pynac](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/allow_custom_cxxflags_for_pynac)**",
    "created_at": "2017-02-23T09:21:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22419",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22419#issuecomment-334182",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/allow_custom_cxxflags_for_pynac](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/allow_custom_cxxflags_for_pynac)**



---

archive/issue_comments_334183.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4c2a96ac5168b1854117b61130df19e81c964207\">4c2a96a</a></td><td><code>Allow custom CXXFLAGS for pynac</code></td></tr></table>\n",
    "created_at": "2017-02-23T09:22:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22419",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22419#issuecomment-334183",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4c2a96ac5168b1854117b61130df19e81c964207">4c2a96a</a></td><td><code>Allow custom CXXFLAGS for pynac</code></td></tr></table>




---

archive/issue_comments_334184.json:
```json
{
    "body": "Commit: **[4c2a96a](https://github.com/sagemath/sagetrac-mirror/commit/4c2a96ac5168b1854117b61130df19e81c964207)**",
    "created_at": "2017-02-23T09:22:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22419",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22419#issuecomment-334184",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[4c2a96a](https://github.com/sagemath/sagetrac-mirror/commit/4c2a96ac5168b1854117b61130df19e81c964207)**



---

archive/issue_events_295465.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-02-23T09:31:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22419",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22419#event-295465"
}
```



---

archive/issue_events_295466.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-02-23T15:59:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/22419",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22419#event-295466"
}
```



---

archive/issue_events_295467.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-02-23T15:59:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22419",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22419#event-295467"
}
```



---

archive/issue_comments_334185.json:
```json
{
    "body": "Reviewer: **Ralf Stephan**",
    "created_at": "2017-02-23T15:59:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22419",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22419#issuecomment-334185",
    "user": "https://github.com/rwst"
}
```

Reviewer: **Ralf Stephan**



---

archive/issue_events_295468.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-02-26T15:33:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/22419",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22419#event-295468"
}
```



---

archive/issue_events_295469.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "7a1a079b66306dcb4b1bc1d6bea94a4b2a77dc98",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-02-26T15:33:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/22419",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22419#event-295469"
}
```



---

archive/issue_comments_334186.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/allow_custom_cxxflags_for_pynac](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/allow_custom_cxxflags_for_pynac)** to **[4c2a96a](https://github.com/sagemath/sagetrac-mirror/commit/4c2a96ac5168b1854117b61130df19e81c964207)**",
    "created_at": "2017-02-26T15:33:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22419",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22419#issuecomment-334186",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/allow_custom_cxxflags_for_pynac](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/allow_custom_cxxflags_for_pynac)** to **[4c2a96a](https://github.com/sagemath/sagetrac-mirror/commit/4c2a96ac5168b1854117b61130df19e81c964207)**
