# Issue 22843: getattr debugger

archive/issues_022606.json:
```json
{
    "assignees": [],
    "body": "When getting an attribute of some object, there is a lot of stuff going on.\n\nWhen working on #22747, I wrote a debugging version of `getattr()` which I think is useful enough to add to Sage.\n\nBranch/Commit: **[db180c7](https://github.com/sagemath/sagetrac-mirror/commit/db180c742fba80a93b9d37b89e9fb6f23876c89d)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/22843_\n\n",
    "closed_at": "2017-04-23T12:57:15Z",
    "created_at": "2017-04-20T15:25:03Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.0",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "getattr debugger",
    "type": "issue",
    "updated_at": "2017-04-23T12:57:15Z",
    "url": "https://github.com/sagemath/sage/issues/22843",
    "user": "https://github.com/jdemeyer"
}
```
When getting an attribute of some object, there is a lot of stuff going on.

When working on #22747, I wrote a debugging version of `getattr()` which I think is useful enough to add to Sage.

Branch/Commit: **[db180c7](https://github.com/sagemath/sagetrac-mirror/commit/db180c742fba80a93b9d37b89e9fb6f23876c89d)**

Reviewer: **Travis Scrimshaw**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/22843_





---

archive/issue_events_300803.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-20T15:25:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/22843",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22843#event-300803"
}
```



---

archive/issue_events_300804.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-20T15:25:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22843",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22843#event-300804"
}
```



---

archive/issue_events_300805.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-20T15:25:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22843",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22843#event-300805"
}
```



---

archive/issue_comments_343351.json:
```json
{
    "body": "Branch: **[u/jdemeyer/getattr_debugger](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/getattr_debugger)**",
    "created_at": "2017-04-21T08:45:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22843#issuecomment-343351",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/getattr_debugger](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/getattr_debugger)**



---

archive/issue_comments_343352.json:
```json
{
    "body": "Commit: **[24e07f0](https://github.com/sagemath/sagetrac-mirror/commit/24e07f0ad08963a5ea21d5c556b612b8e6f8fb08)**",
    "created_at": "2017-04-21T08:45:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22843#issuecomment-343352",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[24e07f0](https://github.com/sagemath/sagetrac-mirror/commit/24e07f0ad08963a5ea21d5c556b612b8e6f8fb08)**



---

archive/issue_events_300806.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-21T08:45:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22843",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22843#event-300806"
}
```



---

archive/issue_comments_343353.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/24e07f0ad08963a5ea21d5c556b612b8e6f8fb08\">24e07f0</a></td><td><code>getattr() debugger</code></td></tr></table>\n",
    "created_at": "2017-04-21T08:45:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22843#issuecomment-343353",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/24e07f0ad08963a5ea21d5c556b612b8e6f8fb08">24e07f0</a></td><td><code>getattr() debugger</code></td></tr></table>




---

archive/issue_comments_343354.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNitpick: `Returns` -> `Return`. Otherwise LGTM.",
    "created_at": "2017-04-21T15:42:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22843#issuecomment-343354",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:3" align="right">Comment 3</div>

Nitpick: `Returns` -> `Return`. Otherwise LGTM.



---

archive/issue_comments_343355.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2017-04-21T15:42:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22843#issuecomment-343355",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_343356.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/db180c742fba80a93b9d37b89e9fb6f23876c89d\">db180c7</a></td><td><code>getattr() debugger</code></td></tr></table>\n",
    "created_at": "2017-04-21T17:01:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22843#issuecomment-343356",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/db180c742fba80a93b9d37b89e9fb6f23876c89d">db180c7</a></td><td><code>getattr() debugger</code></td></tr></table>




---

archive/issue_comments_343357.json:
```json
{
    "body": "Changed commit from **[24e07f0](https://github.com/sagemath/sagetrac-mirror/commit/24e07f0ad08963a5ea21d5c556b612b8e6f8fb08)** to **[db180c7](https://github.com/sagemath/sagetrac-mirror/commit/db180c742fba80a93b9d37b89e9fb6f23876c89d)**",
    "created_at": "2017-04-21T17:01:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22843#issuecomment-343357",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[24e07f0](https://github.com/sagemath/sagetrac-mirror/commit/24e07f0ad08963a5ea21d5c556b612b8e6f8fb08)** to **[db180c7](https://github.com/sagemath/sagetrac-mirror/commit/db180c742fba80a93b9d37b89e9fb6f23876c89d)**



---

archive/issue_events_300807.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-21T17:04:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/22843",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22843#event-300807"
}
```



---

archive/issue_events_300808.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-21T17:04:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22843",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22843#event-300808"
}
```



---

archive/issue_events_300809.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-04-23T12:57:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/22843",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22843#event-300809"
}
```



---

archive/issue_events_300810.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "3a057130ab5eec7f6d038a7eb8cf8e09eb440dbb",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-04-23T12:57:15Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/22843",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22843#event-300810"
}
```



---

archive/issue_comments_343358.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/getattr_debugger](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/getattr_debugger)** to **[db180c7](https://github.com/sagemath/sagetrac-mirror/commit/db180c742fba80a93b9d37b89e9fb6f23876c89d)**",
    "created_at": "2017-04-23T12:57:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22843",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22843#issuecomment-343358",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/getattr_debugger](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/getattr_debugger)** to **[db180c7](https://github.com/sagemath/sagetrac-mirror/commit/db180c742fba80a93b9d37b89e9fb6f23876c89d)**
