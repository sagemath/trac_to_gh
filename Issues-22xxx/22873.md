# Issue 22873: No evaluation with gamma of ball/interval field arguments

archive/issues_022636.json:
```json
{
    "assignees": [],
    "body": "Incomplete gamma doesn't do anything given ball arguments\n\n```\nsage: gamma(2, RBF(1/3))\ngamma(2, [0.3333333333333333 +/- 7.04e-17])\nsage: gamma(2., RBF(1/3))\ngamma(2.00000000000000, [0.3333333333333333 +/- 7.04e-17])\nsage: gamma(2, RIF(1/3))\ngamma(2, 0.3333333333333334?)\n```\n\n\n_Issue created by migration from https://trac.sagemath.org/ticket/22873_\n\n",
    "created_at": "2017-04-26T07:20:01Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "No evaluation with gamma of ball/interval field arguments",
    "type": "issue",
    "updated_at": "2022-12-29T01:40:48Z",
    "url": "https://github.com/sagemath/sage/issues/22873",
    "user": "https://github.com/rwst"
}
```
Incomplete gamma doesn't do anything given ball arguments

```
sage: gamma(2, RBF(1/3))
gamma(2, [0.3333333333333333 +/- 7.04e-17])
sage: gamma(2., RBF(1/3))
gamma(2.00000000000000, [0.3333333333333333 +/- 7.04e-17])
sage: gamma(2, RIF(1/3))
gamma(2, 0.3333333333333334?)
```


_Issue created by migration from https://trac.sagemath.org/ticket/22873_





---

archive/issue_events_301159.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-04-26T07:20:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/22873",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22873#event-301159"
}
```



---

archive/issue_events_301160.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-04-26T07:20:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22873",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22873#event-301160"
}
```



---

archive/issue_events_301161.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-04-26T07:20:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22873",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22873#event-301161"
}
```



---

archive/issue_events_301162.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-04-26T07:20:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22873",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22873#event-301162"
}
```



---

archive/issue_comments_343827.json:
```json
{
    "body": "Branch: **[u/rws/no_evaluation_with_gamma_of_ball_arguments](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/no_evaluation_with_gamma_of_ball_arguments)**",
    "created_at": "2017-05-14T15:17:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22873#issuecomment-343827",
    "user": "https://github.com/rwst"
}
```

Branch: **[u/rws/no_evaluation_with_gamma_of_ball_arguments](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/no_evaluation_with_gamma_of_ball_arguments)**



---

archive/issue_comments_343828.json:
```json
{
    "body": "Commit: **[270ade9](https://github.com/sagemath/sagetrac-mirror/commit/270ade9eb48bfc0fad793e34c0d0ea2b1dc2dad2)**",
    "created_at": "2017-12-21T07:29:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22873#issuecomment-343828",
    "user": "https://github.com/rwst"
}
```

Commit: **[270ade9](https://github.com/sagemath/sagetrac-mirror/commit/270ade9eb48bfc0fad793e34c0d0ea2b1dc2dad2)**



---

archive/issue_comments_343829.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -5,5 +5,7 @@\n gamma(2, [0.3333333333333333 +/- 7.04e-17])\n sage: gamma(2., RBF(1/3))\n gamma(2.00000000000000, [0.3333333333333333 +/- 7.04e-17])\n+sage: gamma(2, RIF(1/3))\n+gamma(2, 0.3333333333333334?)\n ```\n \n``````\n",
    "created_at": "2017-12-21T07:29:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22873#issuecomment-343829",
    "user": "https://github.com/rwst"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -5,5 +5,7 @@
 gamma(2, [0.3333333333333333 +/- 7.04e-17])
 sage: gamma(2., RBF(1/3))
 gamma(2.00000000000000, [0.3333333333333333 +/- 7.04e-17])
+sage: gamma(2, RIF(1/3))
+gamma(2, 0.3333333333333334?)
 ```
 
``````




---

archive/issue_events_301163.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-12-21T07:29:45Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/22873",
    "title_is": "No evaluation with gamma of ball/interval field arguments",
    "title_was": "No evaluation with gamma of ball arguments",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22873#event-301163"
}
```



---

archive/issue_comments_343830.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/270ade9eb48bfc0fad793e34c0d0ea2b1dc2dad2\">270ade9</a></td><td><code>22873: first attempt</code></td></tr></table>\n",
    "created_at": "2017-12-21T07:29:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22873#issuecomment-343830",
    "user": "https://github.com/rwst"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/270ade9eb48bfc0fad793e34c0d0ea2b1dc2dad2">270ade9</a></td><td><code>22873: first attempt</code></td></tr></table>




---

archive/issue_events_301164.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-12-21T07:29:45Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/22873",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22873#event-301164"
}
```



---

archive/issue_events_301165.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-12-21T07:29:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/22873",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22873#event-301165"
}
```



---

archive/issue_comments_343831.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nIt's not use making a `GinacFunction` out of this so a Python solution should do.",
    "created_at": "2018-02-03T07:47:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22873#issuecomment-343831",
    "user": "https://github.com/rwst"
}
```

<div id="comment:3" align="right">Comment 3</div>

It's not use making a `GinacFunction` out of this so a Python solution should do.



---

archive/issue_comments_343832.json:
```json
{
    "body": "Changed commit from **[270ade9](https://github.com/sagemath/sagetrac-mirror/commit/270ade9eb48bfc0fad793e34c0d0ea2b1dc2dad2)** to none",
    "created_at": "2018-02-24T07:06:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22873#issuecomment-343832",
    "user": "https://github.com/rwst"
}
```

Changed commit from **[270ade9](https://github.com/sagemath/sagetrac-mirror/commit/270ade9eb48bfc0fad793e34c0d0ea2b1dc2dad2)** to none



---

archive/issue_comments_343833.json:
```json
{
    "body": "Changed branch from **[u/rws/no_evaluation_with_gamma_of_ball_arguments](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/no_evaluation_with_gamma_of_ball_arguments)** to none",
    "created_at": "2018-02-24T07:06:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22873",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22873#issuecomment-343833",
    "user": "https://github.com/rwst"
}
```

Changed branch from **[u/rws/no_evaluation_with_gamma_of_ball_arguments](https://github.com/sagemath/sagetrac-mirror/tree/u/rws/no_evaluation_with_gamma_of_ball_arguments)** to none



---

archive/issue_events_301166.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:40:48Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/22873",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22873#event-301166"
}
```
