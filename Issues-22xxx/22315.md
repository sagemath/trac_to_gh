# Issue 22315: make install fails on Cygwin for giac-1.2.2.103

archive/issues_022078.json:
```json
{
    "body": "The build succeeds, but `make install` fails at:\n\n```\n[giac-1.2.2.103] mv //home/embray/src/sagemath/sage-cygwin/local/bin/aide.exe /home/embray/src/sagemath/sage-cygwin/local/bin/cas_help\n[giac-1.2.2.103] mv: cannot stat '//home/embray/src/sagemath/sage-cygwin/local/bin/aide.exe': No such file or directory\n```\n\nIt seems the main problem is it doesn't like the double `//` at the beginning of the path.  It probably should also end the filename with `.exe` though technically it doesn't matter so much.\n\nI wonder why it installs this file under one name, and then renames it in the first place...\n\nCC:  @frederichan-IMJPRG parisse\n\nBranch/Commit: 36f4c0a65f74b7e86dc535cbfe6b78c255f82c55\n\nUpstream: Fixed upstream, but not in a stable release.\n\nReviewer: Travis Scrimshaw\n\nAuthor: Erik Bray\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/22315\n\n",
    "closed_at": "2017-02-11T10:22:30Z",
    "created_at": "2017-02-06T14:39:17Z",
    "labels": [
        "component: porting: cygwin",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-7.6",
    "title": "make install fails on Cygwin for giac-1.2.2.103",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/22315",
    "user": "https://github.com/embray"
}
```
The build succeeds, but `make install` fails at:

```
[giac-1.2.2.103] mv //home/embray/src/sagemath/sage-cygwin/local/bin/aide.exe /home/embray/src/sagemath/sage-cygwin/local/bin/cas_help
[giac-1.2.2.103] mv: cannot stat '//home/embray/src/sagemath/sage-cygwin/local/bin/aide.exe': No such file or directory
```

It seems the main problem is it doesn't like the double `//` at the beginning of the path.  It probably should also end the filename with `.exe` though technically it doesn't matter so much.

I wonder why it installs this file under one name, and then renames it in the first place...

CC:  @frederichan-IMJPRG parisse

Branch/Commit: 36f4c0a65f74b7e86dc535cbfe6b78c255f82c55

Upstream: Fixed upstream, but not in a stable release.

Reviewer: Travis Scrimshaw

Author: Erik Bray

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/22315





---

archive/issue_comments_413692.json:
```json
{
    "body": "Changing commit from \"\" to \"36f4c0a65f74b7e86dc535cbfe6b78c255f82c55\"",
    "created_at": "2017-02-06T15:20:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22315#issuecomment-413692",
    "user": "https://github.com/embray"
}
```

Changing commit from "" to "36f4c0a65f74b7e86dc535cbfe6b78c255f82c55"



---

archive/issue_comments_413693.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2017-02-06T15:20:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22315#issuecomment-413693",
    "user": "https://github.com/embray"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_413694.json:
```json
{
    "body": "Changing upstream from \"N/A\" to \"Not yet reported upstream; Will do shortly.\"",
    "created_at": "2017-02-06T15:20:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22315#issuecomment-413694",
    "user": "https://github.com/embray"
}
```

Changing upstream from "N/A" to "Not yet reported upstream; Will do shortly."



---

archive/issue_comments_413695.json:
```json
{
    "body": "Changing author from \"\" to \"Erik Bray\"",
    "created_at": "2017-02-06T15:20:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22315#issuecomment-413695",
    "user": "https://github.com/embray"
}
```

Changing author from "" to "Erik Bray"



---

archive/issue_comments_413696.json:
```json
{
    "body": "<a id='comment:1'></a>\nThis patch fixes it.\n\nI added pre- and post-autotools version of the patch.  Don't recall exactly what the convention is for this.\n\n---\nNew commits:\n|                                                                                                                                          |                                                                                               |\n|------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------|\n|[36f4c0a](https://git.sagemath.org/sage.git/commit?id=36f4c0a65f74b7e86dc535cbfe6b78c255f82c55)|`Fix superfluous slashes in some paths in giac's makefile, which broke some commands on Cygwin`|",
    "created_at": "2017-02-06T15:20:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22315#issuecomment-413696",
    "user": "https://github.com/embray"
}
```

<a id='comment:1'></a>
This patch fixes it.

I added pre- and post-autotools version of the patch.  Don't recall exactly what the convention is for this.

---
New commits:
|                                                                                                                                          |                                                                                               |
|------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------|
|[36f4c0a](https://git.sagemath.org/sage.git/commit?id=36f4c0a65f74b7e86dc535cbfe6b78c255f82c55)|`Fix superfluous slashes in some paths in giac's makefile, which broke some commands on Cygwin`|



---

archive/issue_comments_413697.json:
```json
{
    "body": "Changing branch from \"\" to \"u/embray/cygwin/ticket-22315\"",
    "created_at": "2017-02-06T15:20:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22315#issuecomment-413697",
    "user": "https://github.com/embray"
}
```

Changing branch from "" to "u/embray/cygwin/ticket-22315"



---

archive/issue_comments_413698.json:
```json
{
    "body": "<a id='comment:2'></a>\nAnyone know how best to report upstream issues in Giac?",
    "created_at": "2017-02-06T15:24:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22315#issuecomment-413698",
    "user": "https://github.com/embray"
}
```

<a id='comment:2'></a>
Anyone know how best to report upstream issues in Giac?



---

archive/issue_comments_413699.json:
```json
{
    "body": "<a id='comment:3'></a>\nI'm cc-ing Frederic Han as a starting point.",
    "created_at": "2017-02-06T16:18:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22315#issuecomment-413699",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:3'></a>
I'm cc-ing Frederic Han as a starting point.



---

archive/issue_comments_413700.json:
```json
{
    "body": "<a id='comment:4'></a>\nSorry I have no experience with cygwin, (I only use mingw from fedora cross tool chain) but parisse uses cygwin for xcas so I am CC him. You can also report about giac on \nxcas-english forum:\n\nhttp://xcas.e.ujf-grenoble.fr/XCAS/",
    "created_at": "2017-02-06T21:50:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22315#issuecomment-413700",
    "user": "https://github.com/frederichan-IMJPRG"
}
```

<a id='comment:4'></a>
Sorry I have no experience with cygwin, (I only use mingw from fedora cross tool chain) but parisse uses cygwin for xcas so I am CC him. You can also report about giac on 
xcas-english forum:

http://xcas.e.ujf-grenoble.fr/XCAS/



---

archive/issue_comments_413701.json:
```json
{
    "body": "<a id='comment:5'></a>\nTo be clear, nothing here is very particular to cygwin.  The Makefile.am is generating incorrect paths on any platform (starting with `//`).  On most unices the superfluous slash will be ignored, but on Cygwin paths starting with `//` may be interpreted as a path on Windows network drive so it breaks.  Regardless, while the symptom is particular to Cygwin, the cause and the fix are not.",
    "created_at": "2017-02-07T10:01:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22315#issuecomment-413701",
    "user": "https://github.com/embray"
}
```

<a id='comment:5'></a>
To be clear, nothing here is very particular to cygwin.  The Makefile.am is generating incorrect paths on any platform (starting with `//`).  On most unices the superfluous slash will be ignored, but on Cygwin paths starting with `//` may be interpreted as a path on Windows network drive so it breaks.  Regardless, while the symptom is particular to Cygwin, the cause and the fix are not.



---

archive/issue_comments_413702.json:
```json
{
    "body": "<a id='comment:6'></a>\nI will make the following changes in Makefile.am and src/Makefile.am starting from version 1.2.3-27:\\\\\nreplace $(DESTDIR)/$(prefix) by $(DESTDIR)$(prefix)",
    "created_at": "2017-02-08T10:08:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22315#issuecomment-413702",
    "user": "https://trac.sagemath.org/admin/accounts/users/parisse"
}
```

<a id='comment:6'></a>
I will make the following changes in Makefile.am and src/Makefile.am starting from version 1.2.3-27:\\
replace $(DESTDIR)/$(prefix) by $(DESTDIR)$(prefix)



---

archive/issue_comments_413703.json:
```json
{
    "body": "<a id='comment:7'></a>\nReplying to [parisse](#comment%3A6):\n> I will make the following changes in Makefile.am and src/Makefile.am starting from version 1.2.3-27:\\\\\n> replace $(DESTDIR)/$(prefix) by $(DESTDIR)$(prefix)\n\n\nYes, that sounds right--that's all this patch is doing too.",
    "created_at": "2017-02-09T13:06:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22315#issuecomment-413703",
    "user": "https://github.com/embray"
}
```

<a id='comment:7'></a>
Replying to [parisse](#comment%3A6):
> I will make the following changes in Makefile.am and src/Makefile.am starting from version 1.2.3-27:\\
> replace $(DESTDIR)/$(prefix) by $(DESTDIR)$(prefix)


Yes, that sounds right--that's all this patch is doing too.



---

archive/issue_comments_413704.json:
```json
{
    "body": "Changing upstream from \"Not yet reported upstream; Will do shortly.\" to \"Fixed upstream, but not in a stable release.\"",
    "created_at": "2017-02-09T13:07:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22315#issuecomment-413704",
    "user": "https://github.com/embray"
}
```

Changing upstream from "Not yet reported upstream; Will do shortly." to "Fixed upstream, but not in a stable release."



---

archive/issue_comments_413705.json:
```json
{
    "body": "<a id='comment:9'></a>\nGood for upstream, then good for Sage.",
    "created_at": "2017-02-09T15:02:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22315#issuecomment-413705",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:9'></a>
Good for upstream, then good for Sage.



---

archive/issue_comments_413706.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Travis Scrimshaw\"",
    "created_at": "2017-02-09T15:02:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22315#issuecomment-413706",
    "user": "https://github.com/tscrim"
}
```

Changing reviewer from "" to "Travis Scrimshaw"



---

archive/issue_comments_413707.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2017-02-09T15:02:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22315#issuecomment-413707",
    "user": "https://github.com/tscrim"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_events_058507.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-02-11T10:22:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/22315",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/22315#event-58507"
}
```



---

archive/issue_comments_413708.json:
```json
{
    "body": "Changing branch from \"u/embray/cygwin/ticket-22315\" to \"36f4c0a65f74b7e86dc535cbfe6b78c255f82c55\"",
    "created_at": "2017-02-11T10:22:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22315#issuecomment-413708",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/embray/cygwin/ticket-22315" to "36f4c0a65f74b7e86dc535cbfe6b78c255f82c55"



---

archive/issue_comments_413709.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2017-02-11T10:22:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/22315",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/22315#issuecomment-413709",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed
