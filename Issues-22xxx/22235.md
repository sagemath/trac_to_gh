...
```
Hello, over here in SMC-world, we have a global [SageMath](../wikis/SageMath) install. One thing users like to do is to run ` sage -pip install --user ... }}. Problem is, with version 7.5 there is this {{{pip-lock` script in place, which prevents them from doing that. The problem is:
