# Issue 22825: Symbolic constant's interactive docs are unhelpful

archive/issues_022588.json:
```json
{
    "assignees": [],
    "body": "Symbolic constants become expressions. That's probably why this happens:\n\n```\nsage: NaN?\nType:           Expression\nString form:    NaN\nLength:         0\nFile:           ~/sage/local/lib/python2.7/site-packages/sage/symbolic/expression.pyx\nDocstring:     \n   Nearly all expressions are created by calling\n...\n```\nIt should be possible to get the docstring for the embedded Pyobject.\n\n_Issue created by migration from https://trac.sagemath.org/ticket/22825_\n\n",
    "closed_at": "2017-04-18T15:51:16Z",
    "created_at": "2017-04-18T13:07:05Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Symbolic constant's interactive docs are unhelpful",
    "type": "issue",
    "updated_at": "2017-04-18T15:51:16Z",
    "url": "https://github.com/sagemath/sage/issues/22825",
    "user": "https://github.com/rwst"
}
```
Symbolic constants become expressions. That's probably why this happens:

```
sage: NaN?
Type:           Expression
String form:    NaN
Length:         0
File:           ~/sage/local/lib/python2.7/site-packages/sage/symbolic/expression.pyx
Docstring:     
   Nearly all expressions are created by calling
...
```
It should be possible to get the docstring for the embedded Pyobject.

_Issue created by migration from https://trac.sagemath.org/ticket/22825_





---

archive/issue_events_300585.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-04-18T13:07:05Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/22825",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22825#event-300585"
}
```



---

archive/issue_events_300586.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-04-18T13:07:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22825",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22825#event-300586"
}
```



---

archive/issue_events_300587.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-04-18T13:07:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22825",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22825#event-300587"
}
```



---

archive/issue_events_300588.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2017-04-18T13:07:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22825",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22825#event-300588"
}
```



---

archive/issue_events_300589.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-18T15:51:16Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/22825",
    "milestone_number": null,
    "milestone_title": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22825#event-300589"
}
```



---

archive/issue_events_300590.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-18T15:51:16Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/22825",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22825#event-300590"
}
```



---

archive/issue_events_300591.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-18T15:51:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22825",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22825#event-300591"
}
```



---

archive/issue_events_300592.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-18T15:51:16Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/22825",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22825#event-300592"
}
```



---

archive/issue_comments_342997.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nDuplicate of #18077",
    "created_at": "2017-04-18T15:51:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22825",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22825#issuecomment-342997",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:1" align="right">Comment 1</div>

Duplicate of #18077



---

archive/issue_events_300593.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-18T15:51:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22825",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22825#event-300593"
}
```
