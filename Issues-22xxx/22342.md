# Issue 22342: Remove superfluous quotes from bin_op exception messages

archive/issues_022105.json:
```json
{
    "assignees": [],
    "body": "Replace\n\n```\nTypeError: unsupported operand parent(s) for '+': ....\n```\nby\n\n```\nTypeError: unsupported operand parent(s) for +: ....\n```\nto be more consistent with Python:\n\n```\n>>> 1 + \"foo\"\nTypeError: unsupported operand type(s) for +: 'int' and 'str'\n```\n\nBranch/Commit: **[14f4c72](https://github.com/sagemath/sagetrac-mirror/commit/14f4c72e3295257f4560ee34a3b1697ac574178d)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/22342_\n\n",
    "closed_at": "2017-02-12T22:08:54Z",
    "created_at": "2017-02-10T14:02:31Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20coercion",
        "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Remove superfluous quotes from bin_op exception messages",
    "type": "issue",
    "updated_at": "2017-02-12T22:08:54Z",
    "url": "https://github.com/sagemath/sage/issues/22342",
    "user": "https://github.com/jdemeyer"
}
```
Replace

```
TypeError: unsupported operand parent(s) for '+': ....
```
by

```
TypeError: unsupported operand parent(s) for +: ....
```
to be more consistent with Python:

```
>>> 1 + "foo"
TypeError: unsupported operand type(s) for +: 'int' and 'str'
```

Branch/Commit: **[14f4c72](https://github.com/sagemath/sagetrac-mirror/commit/14f4c72e3295257f4560ee34a3b1697ac574178d)**

Reviewer: **Travis Scrimshaw**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/22342_





---

archive/issue_events_294556.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-02-10T14:02:31Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/22342",
    "milestone_number": null,
    "milestone_title": "sage-7.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22342#event-294556"
}
```



---

archive/issue_events_294557.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-02-10T14:02:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22342",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20coercion",
    "label_color": "0000ff",
    "label_name": "component: coercion",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22342#event-294557"
}
```



---

archive/issue_events_294558.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-02-10T14:02:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22342",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22342#event-294558"
}
```



---

archive/issue_events_294559.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-02-10T14:02:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22342",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22342#event-294559"
}
```



---

archive/issue_comments_332874.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,16 @@\n+Replace\n \n+```\n+TypeError: unsupported operand parent(s) for '+': ....\n+```\n+by\n+\n+```\n+TypeError: unsupported operand parent(s) for +: ....\n+```\n+to be more consistent with Python:\n+\n+```\n+>>> 1 + \"foo\"\n+TypeError: unsupported operand type(s) for +: 'int' and 'str'\n+```\n``````\n",
    "created_at": "2017-02-10T14:04:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22342",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22342#issuecomment-332874",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,16 @@
+Replace
 
+```
+TypeError: unsupported operand parent(s) for '+': ....
+```
+by
+
+```
+TypeError: unsupported operand parent(s) for +: ....
+```
+to be more consistent with Python:
+
+```
+>>> 1 + "foo"
+TypeError: unsupported operand type(s) for +: 'int' and 'str'
+```
``````




---

archive/issue_comments_332875.json:
```json
{
    "body": "Branch: **[u/jdemeyer/remove_superfluous_quotes_from_bin_op_exception_messages](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/remove_superfluous_quotes_from_bin_op_exception_messages)**",
    "created_at": "2017-02-10T15:54:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22342",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22342#issuecomment-332875",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/remove_superfluous_quotes_from_bin_op_exception_messages](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/remove_superfluous_quotes_from_bin_op_exception_messages)**



---

archive/issue_comments_332876.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/14f4c72e3295257f4560ee34a3b1697ac574178d\">14f4c72</a></td><td><code>Remove superfluous quotes from bin_op exception messages</code></td></tr></table>\n",
    "created_at": "2017-02-10T15:54:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22342",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22342#issuecomment-332876",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:3'>Comment 3:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/14f4c72e3295257f4560ee34a3b1697ac574178d">14f4c72</a></td><td><code>Remove superfluous quotes from bin_op exception messages</code></td></tr></table>




---

archive/issue_comments_332877.json:
```json
{
    "body": "Commit: **[14f4c72](https://github.com/sagemath/sagetrac-mirror/commit/14f4c72e3295257f4560ee34a3b1697ac574178d)**",
    "created_at": "2017-02-10T15:54:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22342",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22342#issuecomment-332877",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[14f4c72](https://github.com/sagemath/sagetrac-mirror/commit/14f4c72e3295257f4560ee34a3b1697ac574178d)**



---

archive/issue_events_294560.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-02-10T15:54:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22342",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22342#event-294560"
}
```



---

archive/issue_comments_332878.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2017-02-10T15:57:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22342",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22342#issuecomment-332878",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_332879.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nPositive review if the patchbot comes back green or you have run all of the tests.",
    "created_at": "2017-02-10T15:57:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22342",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22342#issuecomment-332879",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:4'>Comment 4:</a>
Positive review if the patchbot comes back green or you have run all of the tests.



---

archive/issue_comments_332880.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nI did not run all tests, so let's wait for the patchbot.",
    "created_at": "2017-02-10T22:05:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22342",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22342#issuecomment-332880",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:5'>Comment 5:</a>
I did not run all tests, so let's wait for the patchbot.



---

archive/issue_events_294561.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-02-11T08:10:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/22342",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22342#event-294561"
}
```



---

archive/issue_events_294562.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-02-11T08:10:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/22342",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22342#event-294562"
}
```



---

archive/issue_comments_332881.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/remove_superfluous_quotes_from_bin_op_exception_messages](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/remove_superfluous_quotes_from_bin_op_exception_messages)** to **[14f4c72](https://github.com/sagemath/sagetrac-mirror/commit/14f4c72e3295257f4560ee34a3b1697ac574178d)**",
    "created_at": "2017-02-12T22:08:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/22342",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/22342#issuecomment-332881",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/remove_superfluous_quotes_from_bin_op_exception_messages](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/remove_superfluous_quotes_from_bin_op_exception_messages)** to **[14f4c72](https://github.com/sagemath/sagetrac-mirror/commit/14f4c72e3295257f4560ee34a3b1697ac574178d)**



---

archive/issue_events_294563.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-02-12T22:08:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/22342",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22342#event-294563"
}
```



---

archive/issue_events_294564.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "fd64028bb66866652c6cf33e04a27648ed1f8367",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-02-12T22:08:54Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/22342",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/22342#event-294564"
}
```
