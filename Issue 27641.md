...
Original creation time: 2019-05-27 23:59:56
...
Comment by fbissey created at 2019-05-28 01:11:58
...
Comment by strogdon created at 2019-05-28 04:09:42
...
Comment by dimpase created at 2019-05-28 09:09:06
...
Comment by embray created at 2019-05-31 15:41:51

I don't quite understand the intention behind this change.  When using the pexpect interface to GAP, which much of Sage still (sadly) relies on (I am making progress fixing that) we _want_ the `-r` flag because we _don't_ want random stuff a user might have in their `~/.gap` interfering with Sage functionality.
...
Comment by embray created at 2019-05-31 15:48:14

ISTM the problem is not that we pass `-r` in the pexpect interface, but that we _aren't_ passing `-r` to the libgap interface.


---

Comment by fbissey created at 2019-05-31 19:58:22
...
Comment by nthiery created at 2019-06-01 05:05:41
...
Comment by vbraun created at 2019-06-02 22:04:28
...
Comment by vbraun created at 2019-06-02 22:05:27
...
Comment by vbraun created at 2019-06-02 22:05:35
...
Comment by embray created at 2019-06-03 06:30:41
...
Comment by embray created at 2019-06-03 06:32:53

In any case, while this clearly needs to be discussed further the right answer is definitely _not_ to just disable use of `-r` completely, as that is only likely to break more things when testing.

I agree with nthiery that one way or other ignoring the user's `.gap` should _definitely_ be the default when running the test suite.  The rest is debatable.


---

Comment by nthiery created at 2019-06-03 22:53:04
...
Comment by fbissey created at 2019-06-03 22:59:19
...
Comment by nthiery created at 2019-06-03 23:28:08
...
Comment by nthiery created at 2019-06-04 19:45:03
...
Comment by fbissey created at 2019-06-04 20:00:16
...
Comment by fbissey created at 2019-06-04 23:54:24
...
Comment by nthiery created at 2019-06-05 08:05:48
...
Comment by fbissey created at 2019-06-05 08:24:15
...
Comment by embray created at 2019-06-07 08:58:52
...
This principle applies to A and B, but emphatically _not_ for C (even if there are ostensibly benefits to doing so, such as packages that provide alternative implementations of some algorithms that happen to be used by Sage; if that capability exists it should be enabled explicitly through Sage).
...
Comment by embray created at 2019-06-07 09:00:03

If I had my way I would also be more aggressive about controlling exactly what GAP packages are loaded by the GAP interpreter used by Sage, because my experience has been that random GAP packages being loaded can lead to _very_ difficult to trace bugs or differences in results.


---

Comment by embray created at 2019-06-07 09:03:00
...
Comment by embray created at 2019-06-07 09:12:12
...
I'm also not sure about the name `no_dotgap`.  I've been trying (and sometimes it's hard) to avoid predicates with a negative proposition, which leads to confusing double-negatives (`no_dotgap=False` to _enable_ the `.gap` directory).  I would instead call it something like `enable_dotgap=True` (and enable it by default).  Therefore any `Gap()` initialized by a user _would_ use `.gap` which I absolutely agree is the desirable _default_ behavior.

Only the GAP interface used internally for Sage would set `enable_dotgap=False`).

For libgap we're in a bit of a bind, but I _think_ that the most important use case for the libgap interface in Sage is for interfacing with GAP internally (to replace the pexpect interface as much as possible), so this should disable .gap by default.


---

Comment by fbissey created at 2019-06-07 09:13:35
...
Comment by fbissey created at 2019-06-07 09:17:06
...
Comment by fbissey created at 2019-06-07 09:20:48
...
Comment by embray created at 2019-06-07 09:33:17
...
Comment by embray created at 2019-06-07 09:33:59
...
Comment by embray created at 2019-06-07 09:37:45
...
Comment by embray created at 2019-06-07 09:51:43
...
Comment by embray created at 2019-06-07 13:59:54

Replying to [comment:22 embray]:
> > Principle of less surprise: keep GAP in Sage as similar as possible to GAP outside of Sage.
> I think that the `gap` global made available to users in Sage should not be the same as that used internally by Sage.  I think a lot of problems like this can be resolved cleanly by not crossing these use-cases where it isn't necessary to.

I see now, after experimenting with this, that there is a technical challenge to it (I still think it is the right approach in principle).  The problem is that although it is possible to have multiple Gap interpreter instances, much of the code in Sage makes inconsistent assumptions that you are _only_ using the "main" instance, currently the global variable named `gap`.

For example, SageObjects can have both a `_gap_` method, and a `_gap_init_` method.  The former returns the object's GAP representation wrapped in a `GapElement`.  It can take an _optional_ `Gap` instance as an argument: The GAP interpreter instance in which to create the element.  The latter, `_gap_init_`, only returns a string of GAP code to be evaluated in the GAP interpreter to instantiate the object.  This is consistent with how other interfaces work.

The problem (or one of them) is that `_gap_init_` does _not_ take an optional `Gap` instance, and just always assumes you're using the "main" `gap` instance.  This can lead to all kinds of inconsistencies, as sometimes the `_gap_init_` methods themselves query the Gap instance (e.g. what is the current `$sage` variable number) for details on how to evaluate the object.  So if you use one Gap instance in the `_gap_init_` call, but then pass the result of that call to a different `Gap` instance it is not guaranteed to work.

For this reason, among others, I would propose that we keep the current status quo w.r.t. disabling `.gap` by default _for now_, but add the `enable_dotgap` option, so that it's easy to create a new `Gap` instance that uses it.  Then change the default behavior only once further refactoring is done to maintain better separation between the (B) and (C) use cases.


---

Comment by fbissey created at 2019-06-10 23:41:26
...
Comment by nthiery created at 2019-06-11 08:14:05
...
Comment by dimpase created at 2019-06-11 08:26:23
...
Comment by nthiery created at 2019-06-11 21:22:39
...
Comment by dimpase created at 2019-06-11 22:01:52
...
Comment by fbissey created at 2019-07-23 02:11:48
