# Issue 11529: Add spkg-check to Mercurial

archive/issues_011529.json:
```json
{
    "body": "Assignee: tbd\n\nCC:  @jasongrout ryan @kini @jhpalmieri @nexttime\n\nMercurial was recently upgraded at #10594.  It was mentioned there that it has self-tests.  We should enable those to be checked with `SAGE_CHECK` and an spkg-check script.\n\nIssue created by migration from https://trac.sagemath.org/ticket/11701\n\n",
    "created_at": "2011-08-18T15:05:16Z",
    "labels": [
        "component: packages: standard",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-duplicate/invalid/wontfix",
    "title": "Add spkg-check to Mercurial",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/11529",
    "user": "https://github.com/kcrisman"
}
```
Assignee: tbd

CC:  @jasongrout ryan @kini @jhpalmieri @nexttime

Mercurial was recently upgraded at #10594.  It was mentioned there that it has self-tests.  We should enable those to be checked with `SAGE_CHECK` and an spkg-check script.

Issue created by migration from https://trac.sagemath.org/ticket/11701





---

archive/issue_comments_129106.json:
```json
{
    "body": "For the record: The new Mercurial spkg didn't make it into Sage 4.7.**1**.",
    "created_at": "2011-08-18T15:34:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11529",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11529#issuecomment-129106",
    "user": "https://github.com/nexttime"
}
```

For the record: The new Mercurial spkg didn't make it into Sage 4.7.**1**.



---

archive/issue_comments_129107.json:
```json
{
    "body": "Replying to [comment:2 leif]:\n> For the record: The new Mercurial spkg didn't make it into Sage 4.7.**1**.\nCorrect, and I didn't mean to imply that.   In theory, this wouldn't even necessarily depend on that, if the invocation for the self-tests are the same in both versions.  I'll put that in dependencies nonetheless.",
    "created_at": "2011-08-18T15:39:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11529",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11529#issuecomment-129107",
    "user": "https://github.com/kcrisman"
}
```

Replying to [comment:2 leif]:
> For the record: The new Mercurial spkg didn't make it into Sage 4.7.**1**.
Correct, and I didn't mean to imply that.   In theory, this wouldn't even necessarily depend on that, if the invocation for the self-tests are the same in both versions.  I'll put that in dependencies nonetheless.



---

archive/issue_comments_129108.json:
```json
{
    "body": "P.S.:\n\nWe might actually (always) run the test suite from `spkg-install` instead (*before* we install Mercurial), since it is a central component and -- more important -- installing it, afterwards realizing some tests fail, doesn't make much sense.",
    "created_at": "2011-08-18T15:43:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11529",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11529#issuecomment-129108",
    "user": "https://github.com/nexttime"
}
```

P.S.:

We might actually (always) run the test suite from `spkg-install` instead (*before* we install Mercurial), since it is a central component and -- more important -- installing it, afterwards realizing some tests fail, doesn't make much sense.



---

archive/issue_comments_129109.json:
```json
{
    "body": "> We might actually (always) run the test suite from `spkg-install` instead (*before* we install Mercurial), since it is a central component and -- more important -- installing it, afterwards realizing some tests fail, doesn't make much sense.\n\nI think we do this for a few packages, right?  Either one would be a fine solution to this ticket.",
    "created_at": "2011-08-18T15:47:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11529",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11529#issuecomment-129109",
    "user": "https://github.com/kcrisman"
}
```

> We might actually (always) run the test suite from `spkg-install` instead (*before* we install Mercurial), since it is a central component and -- more important -- installing it, afterwards realizing some tests fail, doesn't make much sense.

I think we do this for a few packages, right?  Either one would be a fine solution to this ticket.



---

archive/issue_comments_129110.json:
```json
{
    "body": "Replying to [comment:4 leif]:\n> We might actually (always) run the test suite from `spkg-install` instead (*before* we install Mercurial), since it is a central component and -- more important -- installing it, afterwards realizing some tests fail, doesn't make much sense.\n\n... since the user would probably end up with a broken, perhaps broadly unusable Sage installation.\n\n(Of course he/she should still be able to manually reinstall a previous Mercurial version though, but I think avoiding the need for that is desirable.)",
    "created_at": "2011-08-18T15:49:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11529",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11529#issuecomment-129110",
    "user": "https://github.com/nexttime"
}
```

Replying to [comment:4 leif]:
> We might actually (always) run the test suite from `spkg-install` instead (*before* we install Mercurial), since it is a central component and -- more important -- installing it, afterwards realizing some tests fail, doesn't make much sense.

... since the user would probably end up with a broken, perhaps broadly unusable Sage installation.

(Of course he/she should still be able to manually reinstall a previous Mercurial version though, but I think avoiding the need for that is desirable.)



---

archive/issue_comments_129111.json:
```json
{
    "body": "Replying to [comment:5 kcrisman]:\n> I think we do this for a few packages, right?  Either one would be a fine solution to this ticket.\n\nThe overall design w.r.t. `SAGE_CHECK` and `spkg-check` is a bit suboptimal. The only spkg where we unconditionally run the test suite from `spkg-install` I'm currently aware of is MPFR; there might be very few others, since we changed that in a couple of spkgs a while ago.\n\n(I'm not sure what you mean by *either one*; just adding an `spkg-check` vs. running the test suite from `spkg-install`?)",
    "created_at": "2011-08-18T15:56:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11529",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11529#issuecomment-129111",
    "user": "https://github.com/nexttime"
}
```

Replying to [comment:5 kcrisman]:
> I think we do this for a few packages, right?  Either one would be a fine solution to this ticket.

The overall design w.r.t. `SAGE_CHECK` and `spkg-check` is a bit suboptimal. The only spkg where we unconditionally run the test suite from `spkg-install` I'm currently aware of is MPFR; there might be very few others, since we changed that in a couple of spkgs a while ago.

(I'm not sure what you mean by *either one*; just adding an `spkg-check` vs. running the test suite from `spkg-install`?)



---

archive/issue_comments_129112.json:
```json
{
    "body": "Replying to [comment:7 leif]:\n> Replying to [comment:5 kcrisman]:\n> > I think we do this for a few packages, right?  Either one would be a fine solution to this ticket.\n> \n> The overall design w.r.t. `SAGE_CHECK` and `spkg-check` is a bit suboptimal. The only spkg where we unconditionally run the test suite from `spkg-install` I'm currently aware of is MPFR; there might be very few others, since we changed that in a couple of spkgs a while ago.\n> \n> (I'm not sure what you mean by *either one*; just adding an `spkg-check` vs. running the test suite from `spkg-install`?)\nYes, either of those would be fine, unless the spkg-install version added significantly to the Sage build time.",
    "created_at": "2011-08-18T16:01:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11529",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11529#issuecomment-129112",
    "user": "https://github.com/kcrisman"
}
```

Replying to [comment:7 leif]:
> Replying to [comment:5 kcrisman]:
> > I think we do this for a few packages, right?  Either one would be a fine solution to this ticket.
> 
> The overall design w.r.t. `SAGE_CHECK` and `spkg-check` is a bit suboptimal. The only spkg where we unconditionally run the test suite from `spkg-install` I'm currently aware of is MPFR; there might be very few others, since we changed that in a couple of spkgs a while ago.
> 
> (I'm not sure what you mean by *either one*; just adding an `spkg-check` vs. running the test suite from `spkg-install`?)
Yes, either of those would be fine, unless the spkg-install version added significantly to the Sage build time.



---

archive/issue_comments_129113.json:
```json
{
    "body": "From the src directory, I ran \"make\" and then \"time make tests\".  On an OS X box:\n\n```\n# Ran 407 tests, 35 skipped, 32 failed.\nmake: *** [tests] Error 1\n\nreal\t22m55.609s\nuser\t10m16.167s\nsys\t8m57.330s\n```\n\nOn sage.math:\n\n```\n# Ran 407 tests, 25 skipped, 2 failed.\nmake: *** [tests] Error 1\n\nreal\t13m28.549s\nuser\t7m48.570s\nsys\t2m27.840s\n```\n\nI haven't looked at the output to try to figure out what the failures were, but in any case, this testing process takes *much* longer than the build process.  So I think that we should put this into an `spkg-check` file, not into `spkg-install`.  It would be then be annoying to have another spkg (along with python) which is known to fail self-tests on standard platforms.",
    "created_at": "2011-08-19T01:07:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11529",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11529#issuecomment-129113",
    "user": "https://github.com/jhpalmieri"
}
```

From the src directory, I ran "make" and then "time make tests".  On an OS X box:

```
# Ran 407 tests, 35 skipped, 32 failed.
make: *** [tests] Error 1

real	22m55.609s
user	10m16.167s
sys	8m57.330s
```

On sage.math:

```
# Ran 407 tests, 25 skipped, 2 failed.
make: *** [tests] Error 1

real	13m28.549s
user	7m48.570s
sys	2m27.840s
```

I haven't looked at the output to try to figure out what the failures were, but in any case, this testing process takes *much* longer than the build process.  So I think that we should put this into an `spkg-check` file, not into `spkg-install`.  It would be then be annoying to have another spkg (along with python) which is known to fail self-tests on standard platforms.



---

archive/issue_comments_129114.json:
```json
{
    "body": "Agreed - definitely let's not make this standard.",
    "created_at": "2011-08-19T01:21:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11529",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11529#issuecomment-129114",
    "user": "https://github.com/kcrisman"
}
```

Agreed - definitely let's not make this standard.



---

archive/issue_comments_129115.json:
```json
{
    "body": "Replying to [comment:10 kcrisman]:\n> Agreed - definitely let's not make this standard.\n\n... or modify the tests run \"by default\" by patching the Makefile; this of course requires sorting out which tests fail where (and probably why), so room for a follow-up I guess.",
    "created_at": "2011-08-19T02:31:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11529",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11529#issuecomment-129115",
    "user": "https://github.com/nexttime"
}
```

Replying to [comment:10 kcrisman]:
> Agreed - definitely let's not make this standard.

... or modify the tests run "by default" by patching the Makefile; this of course requires sorting out which tests fail where (and probably why), so room for a follow-up I guess.



---

archive/issue_comments_129116.json:
```json
{
    "body": "Replying to [comment:11 leif]:\n> Replying to [comment:10 kcrisman]:\n> > Agreed - definitely let's not make this standard.\n> \n> ... or modify the tests run \"by default\" by patching the Makefile; this of course requires sorting out which tests fail where (and probably why), so room for a follow-up I guess.\n\nMy point for making it non-standard wasn't the failures, it was the time required.  To install the spkg takes under 5 seconds on my OS X box, whereas running self-tests takes 23 minutes.  We can't add this much time to the default Sage installation, especially since Mercurial seems to function just fine -- the failures don't seem to be too important.  In my opinion, if we want to make some tests standard, we have to just run a few absolutely crucial ones so we don't add too much time; the full test suite is way too large to include every time.\n\nIf we can identify which tests fail and why, and if those particular tests aren't important for our uses, then I agree that (probably on a follow-up) we should document those failures, so people know they may not be getting a fully functional Mercurial installation, and skip those tests.  (It would be nice to do the same for the Python spkg...)",
    "created_at": "2011-08-19T16:20:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11529",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11529#issuecomment-129116",
    "user": "https://github.com/jhpalmieri"
}
```

Replying to [comment:11 leif]:
> Replying to [comment:10 kcrisman]:
> > Agreed - definitely let's not make this standard.
> 
> ... or modify the tests run "by default" by patching the Makefile; this of course requires sorting out which tests fail where (and probably why), so room for a follow-up I guess.

My point for making it non-standard wasn't the failures, it was the time required.  To install the spkg takes under 5 seconds on my OS X box, whereas running self-tests takes 23 minutes.  We can't add this much time to the default Sage installation, especially since Mercurial seems to function just fine -- the failures don't seem to be too important.  In my opinion, if we want to make some tests standard, we have to just run a few absolutely crucial ones so we don't add too much time; the full test suite is way too large to include every time.

If we can identify which tests fail and why, and if those particular tests aren't important for our uses, then I agree that (probably on a follow-up) we should document those failures, so people know they may not be getting a fully functional Mercurial installation, and skip those tests.  (It would be nice to do the same for the Python spkg...)



---

archive/issue_comments_129117.json:
```json
{
    "body": "Actually, rather than editing the Makefile, we can do `make test-foo` for various values of `foo`.",
    "created_at": "2011-08-19T16:28:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11529",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11529#issuecomment-129117",
    "user": "https://github.com/jhpalmieri"
}
```

Actually, rather than editing the Makefile, we can do `make test-foo` for various values of `foo`.



---

archive/issue_events_030750.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2013-08-13T15:35:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11529",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11529#event-30750"
}
```



---

archive/issue_events_030751.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11529",
    "milestone": "sage-5.12",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11529#event-30751"
}
```



---

archive/issue_events_030752.json:
```json
{
    "actor": "https://trac.sagemath.org/admin/accounts/users/vbraun_spam",
    "created_at": "2014-01-30T21:20:52Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11529",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11529#event-30752"
}
```



---

archive/issue_comments_129118.json:
```json
{
    "body": "Let's close this, since we no longer distribute Mercurial.",
    "created_at": "2014-01-31T19:36:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11529",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11529#issuecomment-129118",
    "user": "https://github.com/jhpalmieri"
}
```

Let's close this, since we no longer distribute Mercurial.



---

archive/issue_comments_129119.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2014-01-31T19:36:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11529",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11529#issuecomment-129119",
    "user": "https://github.com/jhpalmieri"
}
```

Changing status from new to needs_review.



---

archive/issue_events_030753.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2014-01-31T19:36:16Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11529",
    "milestone": "sage-6.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11529#event-30753"
}
```



---

archive/issue_events_030754.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2014-01-31T19:36:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/11529",
    "milestone": "sage-duplicate/invalid/wontfix",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11529#event-30754"
}
```



---

archive/issue_comments_129120.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2014-01-31T19:36:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11529",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11529#issuecomment-129120",
    "user": "https://github.com/jhpalmieri"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_129121.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2014-02-01T21:35:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11529",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11529#issuecomment-129121",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_events_030755.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2014-02-01T21:35:10Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/11529",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/11529#event-30755"
}
```



---

archive/issue_comments_129122.json:
```json
{
    "body": "Resolution changed from fixed to wontfix",
    "created_at": "2014-02-01T21:35:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/11529",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/11529#issuecomment-129122",
    "user": "https://github.com/vbraun"
}
```

Resolution changed from fixed to wontfix
