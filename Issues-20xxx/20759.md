# Issue 20759: Lazier computation of generators of relative number fields

archive/issues_020522.json:
```json
{
    "assignees": [],
    "body": "Currently, the generators of a relative number field (including images of generators of the base field) are computed on initialisation.  This causes a slowdown when applying #20749; see [#20749 comment:5](https://github.com/sagemath/sage/issues/20749#comment:5).\n\nThe slow initalisation can be solved using \"lazier\" implementations of the methods `gen()`, `gens()`, `ngens()` and `_first_ngens()` of `NumberField_relative`.  In combination with #20749, this means that the somewhat expensive method `_nf_nfzk()` will only be called the first time an element from the base field is mapped to the relative field.  This is probably the most reasonable point at which to call `_nf_nfzk()`.\n\nBranch/Commit: **[5dfaa28](https://github.com/sagemath/sagetrac-mirror/commit/5dfaa2804b311214789db5e6077d40874825f937)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Peter Bruin**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20759_\n\n",
    "closed_at": "2016-06-09T20:28:18Z",
    "created_at": "2016-06-02T11:02:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Lazier computation of generators of relative number fields",
    "type": "issue",
    "updated_at": "2016-06-09T20:28:18Z",
    "url": "https://github.com/sagemath/sage/issues/20759",
    "user": "https://github.com/pjbruin"
}
```
Currently, the generators of a relative number field (including images of generators of the base field) are computed on initialisation.  This causes a slowdown when applying #20749; see [#20749 comment:5](https://github.com/sagemath/sage/issues/20749#comment:5).

The slow initalisation can be solved using "lazier" implementations of the methods `gen()`, `gens()`, `ngens()` and `_first_ngens()` of `NumberField_relative`.  In combination with #20749, this means that the somewhat expensive method `_nf_nfzk()` will only be called the first time an element from the base field is mapped to the relative field.  This is probably the most reasonable point at which to call `_nf_nfzk()`.

Branch/Commit: **[5dfaa28](https://github.com/sagemath/sagetrac-mirror/commit/5dfaa2804b311214789db5e6077d40874825f937)**

Reviewer: **Travis Scrimshaw**

Author: **Peter Bruin**

_Issue created by migration from https://trac.sagemath.org/ticket/20759_





---

archive/issue_events_274367.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2016-06-02T11:02:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20759",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20759#event-274367"
}
```



---

archive/issue_events_274368.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2016-06-02T11:02:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20759",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
    "label_color": "0000ff",
    "label_name": "component: number fields",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20759#event-274368"
}
```



---

archive/issue_events_274369.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2016-06-02T11:02:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20759",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20759#event-274369"
}
```



---

archive/issue_events_274370.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2016-06-02T11:02:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20759",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20759#event-274370"
}
```



---

archive/issue_comments_303603.json:
```json
{
    "body": "Branch: **[u/pbruin/20759-NumberField_relative_gens](https://github.com/sagemath/sagetrac-mirror/tree/u/pbruin/20759-NumberField_relative_gens)**",
    "created_at": "2016-06-02T11:04:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20759",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20759#issuecomment-303603",
    "user": "https://github.com/pjbruin"
}
```

Branch: **[u/pbruin/20759-NumberField_relative_gens](https://github.com/sagemath/sagetrac-mirror/tree/u/pbruin/20759-NumberField_relative_gens)**



---

archive/issue_events_274371.json:
```json
{
    "actor": "https://github.com/pjbruin",
    "created_at": "2016-06-02T11:04:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20759",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20759#event-274371"
}
```



---

archive/issue_comments_303604.json:
```json
{
    "body": "Commit: **[5dfaa28](https://github.com/sagemath/sagetrac-mirror/commit/5dfaa2804b311214789db5e6077d40874825f937)**",
    "created_at": "2016-06-02T11:04:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20759",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20759#issuecomment-303604",
    "user": "https://github.com/pjbruin"
}
```

Commit: **[5dfaa28](https://github.com/sagemath/sagetrac-mirror/commit/5dfaa2804b311214789db5e6077d40874825f937)**



---

archive/issue_events_274372.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-06-09T03:18:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20759",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20759#event-274372"
}
```



---

archive/issue_events_274373.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-06-09T03:18:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20759",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20759#event-274373"
}
```



---

archive/issue_comments_303605.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nLGTM.",
    "created_at": "2016-06-09T03:18:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20759",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20759#issuecomment-303605",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:2" align="right">Comment 2</div>

LGTM.



---

archive/issue_comments_303606.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2016-06-09T03:18:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20759",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20759#issuecomment-303606",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_303607.json:
```json
{
    "body": "Changed branch from **[u/pbruin/20759-NumberField_relative_gens](https://github.com/sagemath/sagetrac-mirror/tree/u/pbruin/20759-NumberField_relative_gens)** to **[5dfaa28](https://github.com/sagemath/sagetrac-mirror/commit/5dfaa2804b311214789db5e6077d40874825f937)**",
    "created_at": "2016-06-09T20:28:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20759",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20759#issuecomment-303607",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/pbruin/20759-NumberField_relative_gens](https://github.com/sagemath/sagetrac-mirror/tree/u/pbruin/20759-NumberField_relative_gens)** to **[5dfaa28](https://github.com/sagemath/sagetrac-mirror/commit/5dfaa2804b311214789db5e6077d40874825f937)**



---

archive/issue_events_274374.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-06-09T20:28:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20759",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20759#event-274374"
}
```



---

archive/issue_events_274375.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "7adfc9419f768d75d941e2d9c4bf0cedbabec907",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-06-09T20:28:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20759",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20759#event-274375"
}
```
