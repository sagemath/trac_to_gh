...
```json
{
    "body": "It seems like the culprit is that someone added a six.py file in misc so that the import is now importing from the local six.py file instead of the actual six file!",
...
```json
{
    "body": "Seems like it is already fixed in #20797 so this ticket can be closed.",
