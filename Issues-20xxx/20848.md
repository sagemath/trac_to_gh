# Issue 20848: Degree and arithmetic genus for projective curves

archive/issues_020611.json:
```json
{
    "assignees": [],
    "body": "Implement a function to compute the degree of a projective curve, and generalize the existing arithmetic genus computation for projective plane curves to work for arbitrary projective curves.\n\nCC:  @bhutz @miguelmarco\n\nKeywords: **gsoc2016**\n\nBranch/Commit: **[3427412](https://github.com/sagemath/sagetrac-mirror/commit/3427412f972f676039d7e9a6075f4855e999945f)**\n\nReviewer: **Ben Hutz**\n\nAuthor: **Grayson Jorgenson**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20848_\n\n",
    "closed_at": "2016-06-29T08:37:35Z",
    "created_at": "2016-06-20T05:44:01Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Degree and arithmetic genus for projective curves",
    "type": "issue",
    "updated_at": "2016-06-29T08:37:35Z",
    "url": "https://github.com/sagemath/sage/issues/20848",
    "user": "https://github.com/sagetrac-gjorgenson"
}
```
Implement a function to compute the degree of a projective curve, and generalize the existing arithmetic genus computation for projective plane curves to work for arbitrary projective curves.

CC:  @bhutz @miguelmarco

Keywords: **gsoc2016**

Branch/Commit: **[3427412](https://github.com/sagemath/sagetrac-mirror/commit/3427412f972f676039d7e9a6075f4855e999945f)**

Reviewer: **Ben Hutz**

Author: **Grayson Jorgenson**

_Issue created by migration from https://trac.sagemath.org/ticket/20848_





---

archive/issue_events_275463.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2016-06-20T05:44:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20848#event-275463"
}
```



---

archive/issue_events_275464.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2016-06-20T05:44:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
    "label_color": "0000ff",
    "label_name": "component: algebraic geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20848#event-275464"
}
```



---

archive/issue_events_275465.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2016-06-20T05:44:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20848#event-275465"
}
```



---

archive/issue_events_275466.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2016-06-20T05:44:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20848#event-275466"
}
```



---

archive/issue_comments_304925.json:
```json
{
    "body": "Branch: **[u/gjorgenson/ticket/20848](https://github.com/sagemath/sagetrac-mirror/tree/u/gjorgenson/ticket/20848)**",
    "created_at": "2016-06-21T09:46:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20848#issuecomment-304925",
    "user": "https://github.com/sagetrac-gjorgenson"
}
```

Branch: **[u/gjorgenson/ticket/20848](https://github.com/sagemath/sagetrac-mirror/tree/u/gjorgenson/ticket/20848)**



---

archive/issue_comments_304926.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0f962116681a5683ad2db4201dbfe13a4b8540cf\">0f96211</a></td><td><code>20848: first implementation attempt.</code></td></tr></table>\n",
    "created_at": "2016-06-21T09:46:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20848#issuecomment-304926",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0f962116681a5683ad2db4201dbfe13a4b8540cf">0f96211</a></td><td><code>20848: first implementation attempt.</code></td></tr></table>




---

archive/issue_comments_304927.json:
```json
{
    "body": "Commit: **[0f96211](https://github.com/sagemath/sagetrac-mirror/commit/0f962116681a5683ad2db4201dbfe13a4b8540cf)**",
    "created_at": "2016-06-21T09:46:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20848#issuecomment-304927",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[0f96211](https://github.com/sagemath/sagetrac-mirror/commit/0f962116681a5683ad2db4201dbfe13a4b8540cf)**



---

archive/issue_events_275467.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2016-06-21T09:46:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20848#event-275467"
}
```



---

archive/issue_comments_304928.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nIn the case of plane curves, which genus do we want to assignt to non-reduced curves?",
    "created_at": "2016-06-21T09:55:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20848#issuecomment-304928",
    "user": "https://github.com/miguelmarco"
}
```

<div id="comment:4" align="right">Comment 4</div>

In the case of plane curves, which genus do we want to assignt to non-reduced curves?



---

archive/issue_comments_304929.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nOops, I was under the impression that these definitions agreed for arbitrary plane curves. Is the Hilbert polynomial definition still okay for reducible/non-reduced curves? Would it be better to restrict the genus computations to just irreducible curves?",
    "created_at": "2016-06-21T19:25:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20848#issuecomment-304929",
    "user": "https://github.com/sagetrac-gjorgenson"
}
```

<div id="comment:5" align="right">Comment 5</div>

Oops, I was under the impression that these definitions agreed for arbitrary plane curves. Is the Hilbert polynomial definition still okay for reducible/non-reduced curves? Would it be better to restrict the genus computations to just irreducible curves?



---

archive/issue_comments_304930.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nI think genus should be restricted to irreducible. The problem is not the difference between plane and not plane curves. You could have a curve with multiple components of different genus\n\neg: (y-x)(y<sup>2-x</sup>2-x-1)",
    "created_at": "2016-06-22T20:02:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20848#issuecomment-304930",
    "user": "https://github.com/bhutz"
}
```

<div id="comment:6" align="right">Comment 6</div>

I think genus should be restricted to irreducible. The problem is not the difference between plane and not plane curves. You could have a curve with multiple components of different genus

eg: (y-x)(y<sup>2-x</sup>2-x-1)



---

archive/issue_comments_304931.json:
```json
{
    "body": "Changed commit from **[0f96211](https://github.com/sagemath/sagetrac-mirror/commit/0f962116681a5683ad2db4201dbfe13a4b8540cf)** to **[07e6b72](https://github.com/sagemath/sagetrac-mirror/commit/07e6b720f2cf15bdf9b3ac814e987925c3937d4b)**",
    "created_at": "2016-06-23T06:55:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20848#issuecomment-304931",
    "user": "https://github.com/sagetrac-gjorgenson"
}
```

Changed commit from **[0f96211](https://github.com/sagemath/sagetrac-mirror/commit/0f962116681a5683ad2db4201dbfe13a4b8540cf)** to **[07e6b72](https://github.com/sagemath/sagetrac-mirror/commit/07e6b720f2cf15bdf9b3ac814e987925c3937d4b)**



---

archive/issue_comments_304932.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nI added an is_irreducible check which checks irreducibility by just counting irreducible components.\n\nThe new commit shows up in the log for this ticket but no notification seems to have been generated (I think because of the trac server change).\n\nEDIT: actually the new commit shows up as part of this comment now\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/07e6b720f2cf15bdf9b3ac814e987925c3937d4b\">07e6b72</a></td><td><code>20848: added is_irreducible check</code></td></tr></table>\n",
    "created_at": "2016-06-23T06:55:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20848#issuecomment-304932",
    "user": "https://github.com/sagetrac-gjorgenson"
}
```

<div id="comment:7" align="right">Comment 7</div>

I added an is_irreducible check which checks irreducibility by just counting irreducible components.

The new commit shows up in the log for this ticket but no notification seems to have been generated (I think because of the trac server change).

EDIT: actually the new commit shows up as part of this comment now

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/07e6b720f2cf15bdf9b3ac814e987925c3937d4b">07e6b72</a></td><td><code>20848: added is_irreducible check</code></td></tr></table>




---

archive/issue_comments_304933.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\n- Just checking the number of components for is_irreducible is not good enough. You're better off checking if the defining ideal is prime.\n\n```\nP.<x,y>=ProjectiveSpace(QQ,1)\nX=P.subscheme([x^2])\nX.is_irreducible(),X.defining_ideal().is_prime()\n```\n\n- since the degree is just the dim! times the leading coefficient of HP, why not do it for projective subschemes instead of just curves?\n\n- Adding an actual REF for Hartshorne would be nice.",
    "created_at": "2016-06-23T11:56:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20848#issuecomment-304933",
    "user": "https://github.com/bhutz"
}
```

<div id="comment:8" align="right">Comment 8</div>

- Just checking the number of components for is_irreducible is not good enough. You're better off checking if the defining ideal is prime.

```
P.<x,y>=ProjectiveSpace(QQ,1)
X=P.subscheme([x^2])
X.is_irreducible(),X.defining_ideal().is_prime()
```

- since the degree is just the dim! times the leading coefficient of HP, why not do it for projective subschemes instead of just curves?

- Adding an actual REF for Hartshorne would be nice.



---

archive/issue_comments_304934.json:
```json
{
    "body": "Reviewer: **Ben Hutz**",
    "created_at": "2016-06-23T11:56:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20848#issuecomment-304934",
    "user": "https://github.com/bhutz"
}
```

Reviewer: **Ben Hutz**



---

archive/issue_events_275468.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2016-06-23T11:56:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20848#event-275468"
}
```



---

archive/issue_events_275469.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2016-06-23T11:56:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20848#event-275469"
}
```



---

archive/issue_comments_304935.json:
```json
{
    "body": "Changed commit from **[07e6b72](https://github.com/sagemath/sagetrac-mirror/commit/07e6b720f2cf15bdf9b3ac814e987925c3937d4b)** to **[f4709f5](https://github.com/sagemath/sagetrac-mirror/commit/f4709f535b1c9b1e8233f73353a8f2e28d1ca1fb)**",
    "created_at": "2016-06-24T00:44:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20848#issuecomment-304935",
    "user": "https://github.com/sagetrac-gjorgenson"
}
```

Changed commit from **[07e6b72](https://github.com/sagemath/sagetrac-mirror/commit/07e6b720f2cf15bdf9b3ac814e987925c3937d4b)** to **[f4709f5](https://github.com/sagemath/sagetrac-mirror/commit/f4709f535b1c9b1e8233f73353a8f2e28d1ca1fb)**



---

archive/issue_comments_304936.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nThanks, made the changes.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f4709f535b1c9b1e8233f73353a8f2e28d1ca1fb\">f4709f5</a></td><td><code>20848: generalized degree computation, and improved is_irreducible</code></td></tr></table>\n",
    "created_at": "2016-06-24T00:44:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20848#issuecomment-304936",
    "user": "https://github.com/sagetrac-gjorgenson"
}
```

<div id="comment:9" align="right">Comment 9</div>

Thanks, made the changes.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f4709f535b1c9b1e8233f73353a8f2e28d1ca1fb">f4709f5</a></td><td><code>20848: generalized degree computation, and improved is_irreducible</code></td></tr></table>




---

archive/issue_events_275470.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2016-06-24T00:44:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20848#event-275470"
}
```



---

archive/issue_events_275471.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2016-06-24T00:44:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20848#event-275471"
}
```



---

archive/issue_events_275472.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2016-06-24T13:31:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20848#event-275472"
}
```



---

archive/issue_events_275473.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2016-06-24T13:31:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20848#event-275473"
}
```



---

archive/issue_comments_304937.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nMerge conflict",
    "created_at": "2016-06-24T22:18:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20848#issuecomment-304937",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:11" align="right">Comment 11</div>

Merge conflict



---

archive/issue_events_275474.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-06-24T22:18:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20848#event-275474"
}
```



---

archive/issue_events_275475.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-06-24T22:18:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20848#event-275475"
}
```



---

archive/issue_comments_304938.json:
```json
{
    "body": "Changed commit from **[f4709f5](https://github.com/sagemath/sagetrac-mirror/commit/f4709f535b1c9b1e8233f73353a8f2e28d1ca1fb)** to **[df1faa6](https://github.com/sagemath/sagetrac-mirror/commit/df1faa685c2092d9b04710a3f2fbe23ed423bd3f)**",
    "created_at": "2016-06-25T00:02:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20848#issuecomment-304938",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[f4709f5](https://github.com/sagemath/sagetrac-mirror/commit/f4709f535b1c9b1e8233f73353a8f2e28d1ca1fb)** to **[df1faa6](https://github.com/sagemath/sagetrac-mirror/commit/df1faa685c2092d9b04710a3f2fbe23ed423bd3f)**



---

archive/issue_comments_304939.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/df1faa685c2092d9b04710a3f2fbe23ed423bd3f\">df1faa6</a></td><td><code>20848: merge with 7.3 beta5.</code></td></tr></table>\n",
    "created_at": "2016-06-25T00:02:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20848#issuecomment-304939",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:12" align="right">Comment 12</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/df1faa685c2092d9b04710a3f2fbe23ed423bd3f">df1faa6</a></td><td><code>20848: merge with 7.3 beta5.</code></td></tr></table>




---

archive/issue_events_275476.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2016-06-25T00:03:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20848#event-275476"
}
```



---

archive/issue_events_275477.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2016-06-25T00:03:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20848#event-275477"
}
```



---

archive/issue_events_275478.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2016-06-27T13:39:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20848#event-275478"
}
```



---

archive/issue_events_275479.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2016-06-27T13:39:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20848#event-275479"
}
```



---

archive/issue_comments_304940.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">Comment 15</div>\n\nMerge conflict #20839",
    "created_at": "2016-06-27T16:27:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20848#issuecomment-304940",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:15" align="right">Comment 15</div>

Merge conflict #20839



---

archive/issue_events_275480.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-06-27T16:27:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20848#event-275480"
}
```



---

archive/issue_events_275481.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-06-27T16:27:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20848#event-275481"
}
```



---

archive/issue_comments_304941.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f775a0ffaa6a969b5c3c4dc7a03862f0e71fdc58\">f775a0f</a></td><td><code>20839: first implementation attempt.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e0188ce3e3283fd7d8234dfa115d6d7ec1fa3b87\">e0188ce</a></td><td><code>20839: some changes from review</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6564724eb1167aec8a30d966257fb75ea636473a\">6564724</a></td><td><code>20839: merge with ticket 20774</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/09eea02208e31ac7a3d829fd307c01d133ed0c08\">09eea02</a></td><td><code>20839: some remaining changes from review</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/4b9ab0a8d1744f35fa507ff46da2cf032d7405d7\">4b9ab0a</a></td><td><code>20839: implemented Serre intersection multiplicity for affine/projective subschemes</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cae16fe73a6db14d63156e509c48b160ae17094e\">cae16fe</a></td><td><code>20839: improved is_transverse</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3427412f972f676039d7e9a6075f4855e999945f\">3427412</a></td><td><code>20848: merge with ticket 20839</code></td></tr></table>\n",
    "created_at": "2016-06-27T19:38:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20848#issuecomment-304941",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:16" align="right">Comment 16</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f775a0ffaa6a969b5c3c4dc7a03862f0e71fdc58">f775a0f</a></td><td><code>20839: first implementation attempt.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e0188ce3e3283fd7d8234dfa115d6d7ec1fa3b87">e0188ce</a></td><td><code>20839: some changes from review</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6564724eb1167aec8a30d966257fb75ea636473a">6564724</a></td><td><code>20839: merge with ticket 20774</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/09eea02208e31ac7a3d829fd307c01d133ed0c08">09eea02</a></td><td><code>20839: some remaining changes from review</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/4b9ab0a8d1744f35fa507ff46da2cf032d7405d7">4b9ab0a</a></td><td><code>20839: implemented Serre intersection multiplicity for affine/projective subschemes</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cae16fe73a6db14d63156e509c48b160ae17094e">cae16fe</a></td><td><code>20839: improved is_transverse</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3427412f972f676039d7e9a6075f4855e999945f">3427412</a></td><td><code>20848: merge with ticket 20839</code></td></tr></table>




---

archive/issue_comments_304942.json:
```json
{
    "body": "Changed commit from **[df1faa6](https://github.com/sagemath/sagetrac-mirror/commit/df1faa685c2092d9b04710a3f2fbe23ed423bd3f)** to **[3427412](https://github.com/sagemath/sagetrac-mirror/commit/3427412f972f676039d7e9a6075f4855e999945f)**",
    "created_at": "2016-06-27T19:38:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20848#issuecomment-304942",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[df1faa6](https://github.com/sagemath/sagetrac-mirror/commit/df1faa685c2092d9b04710a3f2fbe23ed423bd3f)** to **[3427412](https://github.com/sagemath/sagetrac-mirror/commit/3427412f972f676039d7e9a6075f4855e999945f)**



---

archive/issue_events_275482.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2016-06-27T19:40:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20848#event-275482"
}
```



---

archive/issue_events_275483.json:
```json
{
    "actor": "https://github.com/sagetrac-gjorgenson",
    "created_at": "2016-06-27T19:40:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20848#event-275483"
}
```



---

archive/issue_comments_304943.json:
```json
{
    "body": "<div id=\"comment:17\" align=\"right\">Comment 17</div>\n\nOkay I think the conflict with #20839 is fixed now.",
    "created_at": "2016-06-27T19:40:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20848#issuecomment-304943",
    "user": "https://github.com/sagetrac-gjorgenson"
}
```

<div id="comment:17" align="right">Comment 17</div>

Okay I think the conflict with #20839 is fixed now.



---

archive/issue_comments_304944.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">Comment 18</div>\n\nstill looks fine for me.",
    "created_at": "2016-06-27T22:20:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20848#issuecomment-304944",
    "user": "https://github.com/bhutz"
}
```

<div id="comment:18" align="right">Comment 18</div>

still looks fine for me.



---

archive/issue_events_275484.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2016-06-27T22:20:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20848#event-275484"
}
```



---

archive/issue_events_275485.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2016-06-27T22:20:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20848#event-275485"
}
```



---

archive/issue_events_275486.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-06-29T08:37:35Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20848#event-275486"
}
```



---

archive/issue_events_275487.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "6c27608623ba01fa4e6eb9afc33415a5ad091e59",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-06-29T08:37:35Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20848#event-275487"
}
```



---

archive/issue_comments_304945.json:
```json
{
    "body": "Changed branch from **[u/gjorgenson/ticket/20848](https://github.com/sagemath/sagetrac-mirror/tree/u/gjorgenson/ticket/20848)** to **[3427412](https://github.com/sagemath/sagetrac-mirror/commit/3427412f972f676039d7e9a6075f4855e999945f)**",
    "created_at": "2016-06-29T08:37:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20848",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20848#issuecomment-304945",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/gjorgenson/ticket/20848](https://github.com/sagemath/sagetrac-mirror/tree/u/gjorgenson/ticket/20848)** to **[3427412](https://github.com/sagemath/sagetrac-mirror/commit/3427412f972f676039d7e9a6075f4855e999945f)**
