# Issue 20028: sorting of number field elements

archive/issues_019791.json:
```json
{
    "body": "See the discussion at https://groups.google.com/forum/#!topic/sage-devel/aP5LP09sADY\n\n```\nsage: K.<a> = NumberField(x^3+x+1)\nsage: K(1) > K(2), K(2) > K(1)\n(True, True)\nsage: K(1) < K(2), K(2) < K(1)\n(False, False)\n```\n\n\nCC:  gjorgens@math.fsu.edu jakobkroeker @kliem @videlec @fchapoton @antonio-rojas\n\nKeywords: sort number field elements\n\nBranch: u/cremona/20028\n\nStatus: new\n\nDependencies: #20064\n\nCommit: 2016dd34852bbdae2bac70e66451bed830753f10\n\nIssue created by migration from https://trac.sagemath.org/ticket/20028\n\n",
    "created_at": "2016-02-09T16:47:37Z",
    "labels": [
        "component: number fields",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.8",
    "title": "sorting of number field elements",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/20028",
    "user": "https://github.com/JohnCremona"
}
```
See the discussion at https://groups.google.com/forum/#!topic/sage-devel/aP5LP09sADY

```
sage: K.<a> = NumberField(x^3+x+1)
sage: K(1) > K(2), K(2) > K(1)
(True, True)
sage: K(1) < K(2), K(2) < K(1)
(False, False)
```


CC:  gjorgens@math.fsu.edu jakobkroeker @kliem @videlec @fchapoton @antonio-rojas

Keywords: sort number field elements

Branch: u/cremona/20028

Status: new

Dependencies: #20064

Commit: 2016dd34852bbdae2bac70e66451bed830753f10

Issue created by migration from https://trac.sagemath.org/ticket/20028





---

archive/issue_comments_361292.json:
```json
{
    "body": "<a id='comment:1'></a>I guess the first step for this is to ensure that order comparisons on number field elements by default raise an error. I think that means implementing `_richcmp_` and nuking the `_cmp_` on NumberFieldElement. Then we can see where that raises errors (all those places are bugs given what `_cmp_` does for order comparisons) and, where necessary, we can put in \"sort\" commands with a suitable sort key (and make convenient key implementations available).",
    "created_at": "2016-02-10T02:28:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361292",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:1'></a>I guess the first step for this is to ensure that order comparisons on number field elements by default raise an error. I think that means implementing `_richcmp_` and nuking the `_cmp_` on NumberFieldElement. Then we can see where that raises errors (all those places are bugs given what `_cmp_` does for order comparisons) and, where necessary, we can put in "sort" commands with a suitable sort key (and make convenient key implementations available).



---

archive/issue_comments_361293.json:
```json
{
    "body": "Changing branch from \"\" to \"u/nbruin/sorting_of_number_field_elements\"",
    "created_at": "2016-02-10T04:03:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361293",
    "user": "https://github.com/nbruin"
}
```

Changing branch from "" to "u/nbruin/sorting_of_number_field_elements"



---

archive/issue_comments_361294.json:
```json
{
    "body": "Changing commit from \"\" to \"7aca11920ce4e5f5b02f6c89adc0ae59213940fd\"",
    "created_at": "2016-02-10T04:09:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361294",
    "user": "https://github.com/nbruin"
}
```

Changing commit from "" to "7aca11920ce4e5f5b02f6c89adc0ae59213940fd"



---

archive/issue_comments_361295.json:
```json
{
    "body": "<a id='comment:3'></a>Failures are pretty bad. We'll need a lot of fixing (and these things are basically all bugs!)\n\nOne of the doctest failures is actually just documenting that comparison is bogus: [numberfield.py:394](https://github.com/sagemath/sagetrac-mirror/blob/master/src/sage/rings/number_field/number_field.py?id=7aca11920ce4e5f5b02f6c89adc0ae59213940fd#n394) so that one is not so bad.\n\n---\nNew commits:",
    "created_at": "2016-02-10T04:09:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361295",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:3'></a>Failures are pretty bad. We'll need a lot of fixing (and these things are basically all bugs!)

One of the doctest failures is actually just documenting that comparison is bogus: [numberfield.py:394](https://github.com/sagemath/sagetrac-mirror/blob/master/src/sage/rings/number_field/number_field.py?id=7aca11920ce4e5f5b02f6c89adc0ae59213940fd#n394) so that one is not so bad.

---
New commits:



---

archive/issue_comments_361296.json:
```json
{
    "body": "<a id='comment:4'></a>Thanks for geting started on this.  I have built the branch and will see what \"make testlong\" reveals.  It may be a good idea -- surely -- to not put this forward for merging until there is at least a default sorting function in place, which is quick such as by comparing strings?",
    "created_at": "2016-02-10T11:39:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361296",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:4'></a>Thanks for geting started on this.  I have built the branch and will see what "make testlong" reveals.  It may be a good idea -- surely -- to not put this forward for merging until there is at least a default sorting function in place, which is quick such as by comparing strings?



---

archive/issue_comments_361297.json:
```json
{
    "body": "<a id='comment:5'></a>Replying to [cremona](#comment%3A4):\n> Thanks for geting started on this.  I have built the branch and will see what \"make testlong\" reveals.  It may be a good idea -- surely -- to not put this forward for merging until there is at least a default sorting function in place, which is quick such as by comparing strings?\n\n\nIsn't the whole idea that there is *no* default sorting function? If you mean making convenient key functions readily available, then: yes. I expect we'll need them to fix doctests.\n\nComparing strings is not cheap at all because constructing the string representation is slow. It's better to stick with binary objects. Coefficient lists are much faster:\n\n```\nsage: L=[a^j for j in [1..100]]\nsage: %timeit [str(l) for l in L]\n100 loops, best of 3: 5.89 ms per loop\nsage: %timeit [l.list() for l in L]\n1000 loops, best of 3: 386 \u00b5s per loop\nsage: %timeit [list(l) for l in L]\n100 loops, best of 3: 9.92 ms per loop\nsage: k=lambda l:l.list()\nsage: %timeit [k(l) for l in L]\n1000 loops, best of 3: 398 \u00b5s per loop\n```\n\nunfortunately, using \"list(l)\" is slow: it might construct an iterator out of l first, which would have much more overhead to get the whole list of coefficients than getting the list immediately. So either we optimize \"list\" so that `sorted(L,key=list)` works or we make `k` above available (possibly in a lower overhead version). Note that coefficient lists already come in the right order for power basis rep: integers look like `[*,0,...,0]`.",
    "created_at": "2016-02-10T16:55:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361297",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:5'></a>Replying to [cremona](#comment%3A4):
> Thanks for geting started on this.  I have built the branch and will see what "make testlong" reveals.  It may be a good idea -- surely -- to not put this forward for merging until there is at least a default sorting function in place, which is quick such as by comparing strings?


Isn't the whole idea that there is *no* default sorting function? If you mean making convenient key functions readily available, then: yes. I expect we'll need them to fix doctests.

Comparing strings is not cheap at all because constructing the string representation is slow. It's better to stick with binary objects. Coefficient lists are much faster:

```
sage: L=[a^j for j in [1..100]]
sage: %timeit [str(l) for l in L]
100 loops, best of 3: 5.89 ms per loop
sage: %timeit [l.list() for l in L]
1000 loops, best of 3: 386 µs per loop
sage: %timeit [list(l) for l in L]
100 loops, best of 3: 9.92 ms per loop
sage: k=lambda l:l.list()
sage: %timeit [k(l) for l in L]
1000 loops, best of 3: 398 µs per loop
```

unfortunately, using "list(l)" is slow: it might construct an iterator out of l first, which would have much more overhead to get the whole list of coefficients than getting the list immediately. So either we optimize "list" so that `sorted(L,key=list)` works or we make `k` above available (possibly in a lower overhead version). Note that coefficient lists already come in the right order for power basis rep: integers look like `[*,0,...,0]`.



---

archive/issue_comments_361298.json:
```json
{
    "body": "<a id='comment:6'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2016-02-10T19:49:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361298",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:6'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_361299.json:
```json
{
    "body": "Changing commit from \"7aca11920ce4e5f5b02f6c89adc0ae59213940fd\" to \"d664262eefc1e4f2d03760409527f5ee5527229f\"",
    "created_at": "2016-02-10T19:49:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361299",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "7aca11920ce4e5f5b02f6c89adc0ae59213940fd" to "d664262eefc1e4f2d03760409527f5ee5527229f"



---

archive/issue_comments_361300.json:
```json
{
    "body": "<a id='comment:7'></a>First list of changes. For me, `number_field_element.pyx` now passes. I suspect that a lot of changes will be along the lines of the ones done here.",
    "created_at": "2016-02-10T19:51:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361300",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:7'></a>First list of changes. For me, `number_field_element.pyx` now passes. I suspect that a lot of changes will be along the lines of the ones done here.



---

archive/issue_comments_361301.json:
```json
{
    "body": "Changing commit from \"d664262eefc1e4f2d03760409527f5ee5527229f\" to \"77618de1e986cc52605e071be2542e43d767bbc0\"",
    "created_at": "2016-02-11T05:31:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361301",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "d664262eefc1e4f2d03760409527f5ee5527229f" to "77618de1e986cc52605e071be2542e43d767bbc0"



---

archive/issue_comments_361302.json:
```json
{
    "body": "<a id='comment:8'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2016-02-11T05:31:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361302",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:8'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_361303.json:
```json
{
    "body": "<a id='comment:9'></a>Getting \"rings\" to pass was easy. In elliptic curves there seem to be some more assumptions about things being sortable that are not sortable. Some preliminary fixes put in place. Comments welcome.\n\nIn particular, please take a look at isogeny_small_degree. The approach I took for the first bit can most certainly be extended, but if you have strong opinions in another direction, then feel free to implement that.",
    "created_at": "2016-02-11T05:36:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361303",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:9'></a>Getting "rings" to pass was easy. In elliptic curves there seem to be some more assumptions about things being sortable that are not sortable. Some preliminary fixes put in place. Comments welcome.

In particular, please take a look at isogeny_small_degree. The approach I took for the first bit can most certainly be extended, but if you have strong opinions in another direction, then feel free to implement that.



---

archive/issue_comments_361304.json:
```json
{
    "body": "<a id='comment:10'></a>Nils -- I am willing & happy to do my bit here but then we need to divide up the task.  I'll look at the elliptic curves section first, but we need to split up the others too.",
    "created_at": "2016-02-11T09:19:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361304",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:10'></a>Nils -- I am willing & happy to do my bit here but then we need to divide up the task.  I'll look at the elliptic curves section first, but we need to split up the others too.



---

archive/issue_comments_361305.json:
```json
{
    "body": "<a id='comment:11'></a>Question:  when doing something like sorted(v,key=list) where v is a list of nf elements in a *relative* number field, I suppose that the coefficient list elements are themselves nf elements which must be sorted somehow?  What is happening then?",
    "created_at": "2016-02-11T09:52:06Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361305",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:11'></a>Question:  when doing something like sorted(v,key=list) where v is a list of nf elements in a *relative* number field, I suppose that the coefficient list elements are themselves nf elements which must be sorted somehow?  What is happening then?



---

archive/issue_comments_361306.json:
```json
{
    "body": "<a id='comment:12'></a>Replying to [nbruin](#comment%3A9):\n> Getting \"rings\" to pass was easy. In elliptic curves there seem to be some more assumptions about things being sortable that are not sortable. Some preliminary fixes put in place. Comments welcome.\n> \n> In particular, please take a look at isogeny_small_degree. The approach I took for the first bit can most certainly be extended, but if you have strong opinions in another direction, then feel free to implement that.\n\n\nAs you know from the original thread, one of my aims in starting this was to get the isogenies in a fixed order.  Partly that is just so that we can doctest, since there is no intrinsic mathematical ordering on the set of isogenies from a given curve.  For this it would suffice to arrange for the doctest outputs not to be order-sensitive which would be rather tedious but a Godd Thing in the long run (as I think Volker would agree).\n\nIn situations were I do care about the order of the curves in an isogeny class, that is something which can be achieved elsewhere, in the code for the isogeny class itself;  the class could have one or more methods for sorting the curves in it, to be use by people who care (such as me).\n\nI will do some work on this right now, just in sage/schemes/elliptic_curves, building on commit 77618de1e986c",
    "created_at": "2016-02-11T10:23:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361306",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:12'></a>Replying to [nbruin](#comment%3A9):
> Getting "rings" to pass was easy. In elliptic curves there seem to be some more assumptions about things being sortable that are not sortable. Some preliminary fixes put in place. Comments welcome.
> 
> In particular, please take a look at isogeny_small_degree. The approach I took for the first bit can most certainly be extended, but if you have strong opinions in another direction, then feel free to implement that.


As you know from the original thread, one of my aims in starting this was to get the isogenies in a fixed order.  Partly that is just so that we can doctest, since there is no intrinsic mathematical ordering on the set of isogenies from a given curve.  For this it would suffice to arrange for the doctest outputs not to be order-sensitive which would be rather tedious but a Godd Thing in the long run (as I think Volker would agree).

In situations were I do care about the order of the curves in an isogeny class, that is something which can be achieved elsewhere, in the code for the isogeny class itself;  the class could have one or more methods for sorting the curves in it, to be use by people who care (such as me).

I will do some work on this right now, just in sage/schemes/elliptic_curves, building on commit 77618de1e986c



---

archive/issue_comments_361307.json:
```json
{
    "body": "<a id='comment:13'></a>I finished with the elliptic curves section, and uploaded the modified branch as u/cremona/20028.  \n\nThe principles I followed are:\n\n1. Where there was sorting within the code, it is now always wrapped in a try/except block which result in no sorting when a NotImplementedError is raised.  For cases where sorting is available there will be no change.\n\n   2. For the doctest outputs relating to isogenies, the outputs are now sorted using key=str except when the base field is Q, or where the list has at most one entry anyway.  This required changing some doctest outputs, mostly just the same but in a different order.  I may have overdone this in that over a finite prime field there was possibly sorting already in place making the sort with key=str unnecessary, but this is only doctest output anyway.\n\n   3. A couple of other places requred additions of the try/except wrapping around sort operations.  This includes a place in the isogeny class code, on which I will do further work;  interestingly the only doctests which fail without that particular change is the one which caused this in the first place (CM curves w.r.t an order of class number 6).",
    "created_at": "2016-02-11T12:17:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361307",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:13'></a>I finished with the elliptic curves section, and uploaded the modified branch as u/cremona/20028.  

The principles I followed are:

1. Where there was sorting within the code, it is now always wrapped in a try/except block which result in no sorting when a NotImplementedError is raised.  For cases where sorting is available there will be no change.

   2. For the doctest outputs relating to isogenies, the outputs are now sorted using key=str except when the base field is Q, or where the list has at most one entry anyway.  This required changing some doctest outputs, mostly just the same but in a different order.  I may have overdone this in that over a finite prime field there was possibly sorting already in place making the sort with key=str unnecessary, but this is only doctest output anyway.

   3. A couple of other places requred additions of the try/except wrapping around sort operations.  This includes a place in the isogeny class code, on which I will do further work;  interestingly the only doctests which fail without that particular change is the one which caused this in the first place (CM curves w.r.t an order of class number 6).



---

archive/issue_comments_361308.json:
```json
{
    "body": "Changing branch from \"u/nbruin/sorting_of_number_field_elements\" to \"u/cremona/20028\"",
    "created_at": "2016-02-11T12:17:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361308",
    "user": "https://github.com/JohnCremona"
}
```

Changing branch from "u/nbruin/sorting_of_number_field_elements" to "u/cremona/20028"



---

archive/issue_comments_361309.json:
```json
{
    "body": "Changing commit from \"77618de1e986cc52605e071be2542e43d767bbc0\" to \"ecde78e4dd46a8be1125bd9d974f6f98698c989d\"",
    "created_at": "2016-02-11T12:17:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361309",
    "user": "https://github.com/JohnCremona"
}
```

Changing commit from "77618de1e986cc52605e071be2542e43d767bbc0" to "ecde78e4dd46a8be1125bd9d974f6f98698c989d"



---

archive/issue_comments_361310.json:
```json
{
    "body": "<a id='comment:14'></a>A full doctest now shows only the following:\n\n```\nsage -t --long src/sage/schemes/projective/projective_morphism.py  # 1 doctest failed\n\n-- caused by a sort in sage/schemes/affine/affine_homset.py, similarly for projective.  There is some badly broken code in lines 3003-3011 of sage/schemes/projective/projective_morphism.py which I have not fixed!\n\nsage -t --long src/sage/modular/modform/constructor.py  # 2 doctests failed\n\n-- caused by a sort() in sage/matrix/matrix2.pyx (line 4633) where a polynomial factorization was being sorted).\n\nsage -t --long src/sage/modular/cusps_nf.py  # 2 doctests failed\n\n-- comparison by < of number field cusps is done by comparing the underlying nf elements.  I just marked the tests # not tested.\n\nsage -t --long src/sage/schemes/projective/projective_rational_point.py  # 1 doctest failed\n\n-- a list of points over a number field was being sorted.\n\n```\nwhich I have sorted -- sorry, fixed.  Testing again.",
    "created_at": "2016-02-11T12:29:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361310",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:14'></a>A full doctest now shows only the following:

```
sage -t --long src/sage/schemes/projective/projective_morphism.py  # 1 doctest failed

-- caused by a sort in sage/schemes/affine/affine_homset.py, similarly for projective.  There is some badly broken code in lines 3003-3011 of sage/schemes/projective/projective_morphism.py which I have not fixed!

sage -t --long src/sage/modular/modform/constructor.py  # 2 doctests failed

-- caused by a sort() in sage/matrix/matrix2.pyx (line 4633) where a polynomial factorization was being sorted).

sage -t --long src/sage/modular/cusps_nf.py  # 2 doctests failed

-- comparison by < of number field cusps is done by comparing the underlying nf elements.  I just marked the tests # not tested.

sage -t --long src/sage/schemes/projective/projective_rational_point.py  # 1 doctest failed

-- a list of points over a number field was being sorted.

```
which I have sorted -- sorry, fixed.  Testing again.



---

archive/issue_comments_361311.json:
```json
{
    "body": "Changing commit from \"ecde78e4dd46a8be1125bd9d974f6f98698c989d\" to \"a8a0714ceb09eef1c5a9e1607b2dce5183d8763d\"",
    "created_at": "2016-02-11T13:37:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361311",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "ecde78e4dd46a8be1125bd9d974f6f98698c989d" to "a8a0714ceb09eef1c5a9e1607b2dce5183d8763d"



---

archive/issue_comments_361312.json:
```json
{
    "body": "<a id='comment:15'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2016-02-11T13:37:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361312",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:15'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_361313.json:
```json
{
    "body": "<a id='comment:16'></a>OK, so this commit fixes all remaining doctest failues with one exception, in src/sage/schemes/projective/projective_morphism.py.  I am not sure how this one crept in.  The main reason I have not fixed it is that around this place in the code (see COmment 14 above) there is some very badly written (wrong) code which should probably be fixed while we are looking at it.\n\ngit blame tells me that the wrong code was written by Grayson Jorgenson in March 2015 so I have CC'd him on this ticket.\n\nNow would perhaps be a good time to get some other people to look at what we have been doing!",
    "created_at": "2016-02-11T13:43:10Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361313",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:16'></a>OK, so this commit fixes all remaining doctest failues with one exception, in src/sage/schemes/projective/projective_morphism.py.  I am not sure how this one crept in.  The main reason I have not fixed it is that around this place in the code (see COmment 14 above) there is some very badly written (wrong) code which should probably be fixed while we are looking at it.

git blame tells me that the wrong code was written by Grayson Jorgenson in March 2015 so I have CC'd him on this ticket.

Now would perhaps be a good time to get some other people to look at what we have been doing!



---

archive/issue_comments_361314.json:
```json
{
    "body": "<a id='comment:17'></a>Thanks! Personally, I dislike the `try: L.sort() except NotImplementedError: pass` blocks. Those sorts shouldn't have been there in the first place. I think people should just deal with the fact that some objects do not come in any particular order. It does seem the easiest way to ensure that most doctests do not change, which is the easier way of getting this stuff in. So I think we'll have to live with it.\n\nIt's sad that python first had `__cmp__` which made it impossible to have eq/ne without pretending to have ordering.",
    "created_at": "2016-02-11T17:09:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361314",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:17'></a>Thanks! Personally, I dislike the `try: L.sort() except NotImplementedError: pass` blocks. Those sorts shouldn't have been there in the first place. I think people should just deal with the fact that some objects do not come in any particular order. It does seem the easiest way to ensure that most doctests do not change, which is the easier way of getting this stuff in. So I think we'll have to live with it.

It's sad that python first had `__cmp__` which made it impossible to have eq/ne without pretending to have ordering.



---

archive/issue_comments_361315.json:
```json
{
    "body": "<a id='comment:18'></a>Hi!\n\nI think the failure in projective_morphism.py comes from projective_homset.py, where a set is now unexpectedly returned instead of a list in the points function for fields. The try block that was added in projective_homset.py prevents `sorted(points)` from executing in line 138 and `sorted` changes the type of points from a set to a list. Changing `return points` to `return list(points)` in line 141 seems to address the problem.\n\nFor the code in projective_morphism.py (lines 3003-3011) in the first line a list is declared but is never used (I think it is an artifact from a previous implementation attempt that wasn't caught). As far as I can tell, the rest of the code is working properly. Is there a problem with its functionality that I've missed?",
    "created_at": "2016-02-12T03:08:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361315",
    "user": "https://trac.sagemath.org/admin/accounts/users/gjorgenson"
}
```

<a id='comment:18'></a>Hi!

I think the failure in projective_morphism.py comes from projective_homset.py, where a set is now unexpectedly returned instead of a list in the points function for fields. The try block that was added in projective_homset.py prevents `sorted(points)` from executing in line 138 and `sorted` changes the type of points from a set to a list. Changing `return points` to `return list(points)` in line 141 seems to address the problem.

For the code in projective_morphism.py (lines 3003-3011) in the first line a list is declared but is never used (I think it is an artifact from a previous implementation attempt that wasn't caught). As far as I can tell, the rest of the code is working properly. Is there a problem with its functionality that I've missed?



---

archive/issue_comments_361316.json:
```json
{
    "body": "<a id='comment:19'></a>Replying to [gjorgenson](#comment%3A18):\n> Hi!\n\n\nThanks a lot for responding.\n\n> \n> I think the failure in projective_morphism.py comes from projective_homset.py, where a set is now unexpectedly returned instead of a list in the points function for fields. The try block that was added in projective_homset.py prevents `sorted(points)` from executing in line 138 and `sorted` changes the type of points from a set to a list. Changing `return points` to `return list(points)` in line 141 seems to address the problem.\n\n\nOK, we'll do that.  (It is odd to me that sorted(X) has the side effect of changing a set to a list, but now that we know that we just make it into a list regardless of whether we are able to sort it.)\n\nI'll update the branch on this ticket accordingly.\n> \n> For the code in projective_morphism.py (lines 3003-3011) in the first line a list is declared but is never used (I think it is an artifact from a previous implementation attempt that wasn't caught). As far as I can tell, the rest of the code is working properly. Is there a problem with its functionality that I've missed?\n\n\nSure, rem_indices is not used.  But also: the line P=self(P) is executed man ytimes (once for each j) and in the next line should it not be if p++points[j]?  Otherwise j is not used.  I am also worried about the effect of popping points[i] while in the i-loop, but perhaps that's OK given that the i loop goes backwards.  A comment to explain what this block of code is doing would help, since I suspect that someone might come up with a slicker solution.  (If we only wanted to remove duplicates and did not care about the order then something like points=list(set(points)) would work.)\n\nIt's a nuisance that any fix to this will conflict with our sorting changes, but we'll have to live with that.",
    "created_at": "2016-02-12T09:13:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361316",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:19'></a>Replying to [gjorgenson](#comment%3A18):
> Hi!


Thanks a lot for responding.

> 
> I think the failure in projective_morphism.py comes from projective_homset.py, where a set is now unexpectedly returned instead of a list in the points function for fields. The try block that was added in projective_homset.py prevents `sorted(points)` from executing in line 138 and `sorted` changes the type of points from a set to a list. Changing `return points` to `return list(points)` in line 141 seems to address the problem.


OK, we'll do that.  (It is odd to me that sorted(X) has the side effect of changing a set to a list, but now that we know that we just make it into a list regardless of whether we are able to sort it.)

I'll update the branch on this ticket accordingly.
> 
> For the code in projective_morphism.py (lines 3003-3011) in the first line a list is declared but is never used (I think it is an artifact from a previous implementation attempt that wasn't caught). As far as I can tell, the rest of the code is working properly. Is there a problem with its functionality that I've missed?


Sure, rem_indices is not used.  But also: the line P=self(P) is executed man ytimes (once for each j) and in the next line should it not be if p++points[j]?  Otherwise j is not used.  I am also worried about the effect of popping points[i] while in the i-loop, but perhaps that's OK given that the i loop goes backwards.  A comment to explain what this block of code is doing would help, since I suspect that someone might come up with a slicker solution.  (If we only wanted to remove duplicates and did not care about the order then something like points=list(set(points)) would work.)

It's a nuisance that any fix to this will conflict with our sorting changes, but we'll have to live with that.



---

archive/issue_comments_361317.json:
```json
{
    "body": "<a id='comment:20'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2016-02-12T11:34:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361317",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:20'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_361318.json:
```json
{
    "body": "Changing commit from \"a8a0714ceb09eef1c5a9e1607b2dce5183d8763d\" to \"f9f6b99b09fc6c5183d2fe7a82e4f27e957f059c\"",
    "created_at": "2016-02-12T11:34:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361318",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "a8a0714ceb09eef1c5a9e1607b2dce5183d8763d" to "f9f6b99b09fc6c5183d2fe7a82e4f27e957f059c"



---

archive/issue_comments_361319.json:
```json
{
    "body": "<a id='comment:21'></a>Replying to [nbruin](#comment%3A17):\n> Thanks! Personally, I dislike the `try: L.sort() except NotImplementedError: pass` blocks. Those sorts shouldn't have been there in the first place. I think people should just deal with the fact that some objects do not come in any particular order. It does seem the easiest way to ensure that most doctests do not change, which is the easier way of getting this stuff in. So I think we'll have to live with it.\n\n\nI agree that these are ugly.  We can either try to get the current patch reviewed, or look to see how bad things would be if we were to remove those.  I will start on that for elliptic curves, but do not want to delay progress.\n\nAfter removing these try/except blocks in sage/schemes/elliptic_curves:\n\n```\nsage -t src/sage/schemes/elliptic_curves/cm.py  # 2 doctests failed\nsage -t src/sage/schemes/elliptic_curves/ell_torsion.py  # 2 doctests failed\nsage -t src/sage/schemes/elliptic_curves/isogeny_small_degree.py  # 3 doctests failed\nsage -t src/sage/schemes/elliptic_curves/ell_modular_symbols.py  # 3 doctests failed\nsage -t src/sage/schemes/elliptic_curves/ell_field.py  # 8 doctests failed\nsage -t src/sage/schemes/elliptic_curves/ell_generic.py  # 2 doctests failed\nsage -t src/sage/schemes/elliptic_curves/ell_point.py  # 5 doctests failed\nsage -t src/sage/schemes/elliptic_curves/isogeny_class.py  # 41 doctests failed\nsage -t src/sage/schemes/elliptic_curves/ell_number_field.py  # 32 doctests failed\nsage -t src/sage/schemes/elliptic_curves/ell_rational_field.py  # 6 doctests failed\n```\nwhich I hope can be resolved easily....nearly done... the files with lots of failures are becuase I deleted one line too many (forgot to set self.curves in the isogeny class code over number fields) so in fact it is not so bad at all).\n> \n> It's sad that python first had `__cmp__` which made it impossible to have eq/ne without pretending to have ordering.",
    "created_at": "2016-02-12T11:36:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361319",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:21'></a>Replying to [nbruin](#comment%3A17):
> Thanks! Personally, I dislike the `try: L.sort() except NotImplementedError: pass` blocks. Those sorts shouldn't have been there in the first place. I think people should just deal with the fact that some objects do not come in any particular order. It does seem the easiest way to ensure that most doctests do not change, which is the easier way of getting this stuff in. So I think we'll have to live with it.


I agree that these are ugly.  We can either try to get the current patch reviewed, or look to see how bad things would be if we were to remove those.  I will start on that for elliptic curves, but do not want to delay progress.

After removing these try/except blocks in sage/schemes/elliptic_curves:

```
sage -t src/sage/schemes/elliptic_curves/cm.py  # 2 doctests failed
sage -t src/sage/schemes/elliptic_curves/ell_torsion.py  # 2 doctests failed
sage -t src/sage/schemes/elliptic_curves/isogeny_small_degree.py  # 3 doctests failed
sage -t src/sage/schemes/elliptic_curves/ell_modular_symbols.py  # 3 doctests failed
sage -t src/sage/schemes/elliptic_curves/ell_field.py  # 8 doctests failed
sage -t src/sage/schemes/elliptic_curves/ell_generic.py  # 2 doctests failed
sage -t src/sage/schemes/elliptic_curves/ell_point.py  # 5 doctests failed
sage -t src/sage/schemes/elliptic_curves/isogeny_class.py  # 41 doctests failed
sage -t src/sage/schemes/elliptic_curves/ell_number_field.py  # 32 doctests failed
sage -t src/sage/schemes/elliptic_curves/ell_rational_field.py  # 6 doctests failed
```
which I hope can be resolved easily....nearly done... the files with lots of failures are becuase I deleted one line too many (forgot to set self.curves in the isogeny class code over number fields) so in fact it is not so bad at all).
> 
> It's sad that python first had `__cmp__` which made it impossible to have eq/ne without pretending to have ordering.



---

archive/issue_comments_361320.json:
```json
{
    "body": "<a id='comment:22'></a>Replying to [cremona](#comment%3A19):\n> It is odd to me that sorted(X) has the side effect of changing a set to a list\n\n\nThe wonders of python's ducktyping: \"sorted\" accepts an iterable and returns the contents of the iterable as a sorted list. A finite set is an iterable. It would certainly be useless if it returned a set, since then the sorting work was all for naught (sets are hash tables underneath in python, so they really don't preserve any kind of order).\n\n> but now that we know that we just make it into a list regardless of whether we are able to sort it.\n\n\nUhm. I don't have a problem returning things in a list in no particular order, but actively turning something that comes naturally in a set into a list just for the sake of it sounds wrong. If the routine you're getting your data from returns a set, perhaps you should too? or perhaps that routine benefits from returning a list instead? This doesn't necessarily need to be resolved here, but bear in mind that postponing a cleaner resolution is the process through which cruft builds up in sage.\n\n**EDIT:** OK, I see now what's going on. There's a lot unnecessary work: Once you've found the points on one affine patch, you should determine the points on the hyperplane at infinity (and repeat the process there). Then there's no reason for \"points\" to be a set in the first place, because you'll be finding them without duplication.",
    "created_at": "2016-02-12T17:11:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361320",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:22'></a>Replying to [cremona](#comment%3A19):
> It is odd to me that sorted(X) has the side effect of changing a set to a list


The wonders of python's ducktyping: "sorted" accepts an iterable and returns the contents of the iterable as a sorted list. A finite set is an iterable. It would certainly be useless if it returned a set, since then the sorting work was all for naught (sets are hash tables underneath in python, so they really don't preserve any kind of order).

> but now that we know that we just make it into a list regardless of whether we are able to sort it.


Uhm. I don't have a problem returning things in a list in no particular order, but actively turning something that comes naturally in a set into a list just for the sake of it sounds wrong. If the routine you're getting your data from returns a set, perhaps you should too? or perhaps that routine benefits from returning a list instead? This doesn't necessarily need to be resolved here, but bear in mind that postponing a cleaner resolution is the process through which cruft builds up in sage.

**EDIT:** OK, I see now what's going on. There's a lot unnecessary work: Once you've found the points on one affine patch, you should determine the points on the hyperplane at infinity (and repeat the process there). Then there's no reason for "points" to be a set in the first place, because you'll be finding them without duplication.



---

archive/issue_comments_361321.json:
```json
{
    "body": "<a id='comment:23'></a>Replying to [cremona](#comment%3A19):\n\n> Sure, rem_indices is not used.  But also: the line P=self(P) is executed man ytimes (once for each j) and in the next line should it not be if p++points[j]?  Otherwise j is not used.  I am also worried about the effect of popping points[i] while in the i-loop, but perhaps that's OK given that the i loop goes backwards.  A comment to explain what this block of code is doing would help, since I suspect that someone might come up with a slicker solution.  (If we only wanted to remove duplicates and did not care about the order then something like points=list(set(points)) would work.)\n\n\nThat code doesn't try to remove duplicates (the list of points returned by rational_points in line 2998 is assumed to contain no duplicates already), but is trying to weed out periodic points of the map `self` that have periods smaller than n so that only those with minimal period n are returned. This is done by iterating each point in the list by `self` (in the j-loop) n-1 times and checking whether any of those iterates is the same point as the 0th iterate (points[i]). j is not used for indexing but is needed to keep track of how many times to iterate each point before it can be concluded that the point has minimal period n.\n\nThe problems associated with popping elements from the list are avoided since the i-loop moves backwards. I'm not sure of a way to do this more efficiently, but it would definitely be good to at least add some more documentation to the code. Would that be appropriate to do in this ticket, or would it be cleaner to leave it to a separate ticket?",
    "created_at": "2016-02-12T17:34:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361321",
    "user": "https://trac.sagemath.org/admin/accounts/users/gjorgenson"
}
```

<a id='comment:23'></a>Replying to [cremona](#comment%3A19):

> Sure, rem_indices is not used.  But also: the line P=self(P) is executed man ytimes (once for each j) and in the next line should it not be if p++points[j]?  Otherwise j is not used.  I am also worried about the effect of popping points[i] while in the i-loop, but perhaps that's OK given that the i loop goes backwards.  A comment to explain what this block of code is doing would help, since I suspect that someone might come up with a slicker solution.  (If we only wanted to remove duplicates and did not care about the order then something like points=list(set(points)) would work.)


That code doesn't try to remove duplicates (the list of points returned by rational_points in line 2998 is assumed to contain no duplicates already), but is trying to weed out periodic points of the map `self` that have periods smaller than n so that only those with minimal period n are returned. This is done by iterating each point in the list by `self` (in the j-loop) n-1 times and checking whether any of those iterates is the same point as the 0th iterate (points[i]). j is not used for indexing but is needed to keep track of how many times to iterate each point before it can be concluded that the point has minimal period n.

The problems associated with popping elements from the list are avoided since the i-loop moves backwards. I'm not sure of a way to do this more efficiently, but it would definitely be good to at least add some more documentation to the code. Would that be appropriate to do in this ticket, or would it be cleaner to leave it to a separate ticket?



---

archive/issue_comments_361322.json:
```json
{
    "body": "<a id='comment:24'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2016-02-15T14:16:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361322",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:24'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_361323.json:
```json
{
    "body": "Changing commit from \"f9f6b99b09fc6c5183d2fe7a82e4f27e957f059c\" to \"ae87261971f6ddc93d1d40c24c3a141be58457bd\"",
    "created_at": "2016-02-15T14:16:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361323",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "f9f6b99b09fc6c5183d2fe7a82e4f27e957f059c" to "ae87261971f6ddc93d1d40c24c3a141be58457bd"



---

archive/issue_comments_361324.json:
```json
{
    "body": "<a id='comment:25'></a>The last version of u/cremona/20028 (commit ae87261) removes the try/except sorting blocks and deals with the issues which that creates in the elliptic curves directory.  Mostly this involves adding sorted() to some doctests, and reordering the expected output.  In the case of isogeny classes I used the existing facility for reordering an isogeny class based on a new ordering for the curve in it;  this is a stop-gap until we have better sorting methods implemented for isogenies (and for n.f. elements).\n\nI am now looking at the pther parts of Sage to make the necessary changes there.",
    "created_at": "2016-02-15T14:20:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361324",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:25'></a>The last version of u/cremona/20028 (commit ae87261) removes the try/except sorting blocks and deals with the issues which that creates in the elliptic curves directory.  Mostly this involves adding sorted() to some doctests, and reordering the expected output.  In the case of isogeny classes I used the existing facility for reordering an isogeny class based on a new ordering for the curve in it;  this is a stop-gap until we have better sorting methods implemented for isogenies (and for n.f. elements).

I am now looking at the pther parts of Sage to make the necessary changes there.



---

archive/issue_comments_361325.json:
```json
{
    "body": "<a id='comment:26'></a>Also commenting on the projective_morphism code line 3002-3014 as I helped review that: Except for the unused list variable rem_indices this code is doing exactly what is it supposed to:\n\nGiven a list of periodic points get rid of any that have period less than n, i.e., if you want the minimal 4 periodic points, go through and remove the fixed points and the 2 periodic points.\n\nHence the variable j is walking through the iterates (up to n) of each point indexed by i. Since the list is traversed in reverse order, the pop doesn't break it. I'm not saying that there isn't a better way to do this, but the code does do its intended task. Perhaps some additional code comments are in order there so that people can more easily decipher it.",
    "created_at": "2016-02-15T17:07:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361325",
    "user": "https://github.com/bhutz"
}
```

<a id='comment:26'></a>Also commenting on the projective_morphism code line 3002-3014 as I helped review that: Except for the unused list variable rem_indices this code is doing exactly what is it supposed to:

Given a list of periodic points get rid of any that have period less than n, i.e., if you want the minimal 4 periodic points, go through and remove the fixed points and the 2 periodic points.

Hence the variable j is walking through the iterates (up to n) of each point indexed by i. Since the list is traversed in reverse order, the pop doesn't break it. I'm not saying that there isn't a better way to do this, but the code does do its intended task. Perhaps some additional code comments are in order there so that people can more easily decipher it.



---

archive/issue_comments_361326.json:
```json
{
    "body": "<a id='comment:27'></a>Replying to [bhutz](#comment%3A26):\n> Also commenting on the projective_morphism code line 3002-3014 as I helped review that: Except for the unused list variable rem_indices this code is doing exactly what is it supposed to:\n> \n> Given a list of periodic points get rid of any that have period less than n, i.e., if you want the minimal 4 periodic points, go through and remove the fixed points and the 2 periodic points.\n> \n> Hence the variable j is walking through the iterates (up to n) of each point indexed by i. Since the list is traversed in reverse order, the pop doesn't break it. I'm not saying that there isn't a better way to do this, but the code does do its intended task. Perhaps some additional code comments are in order there so that people can more easily decipher it.\n\n\nBen, I can only apologise again for doubting the code hen I just misunderstood it (and specifically what self(P) did).  At some point when this ticket is finished, perhaps adding some comments and removing the redundant line would be a good idea, but there is no hurry.  (Feel free to open a ticket for that already.)",
    "created_at": "2016-02-15T19:50:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361326",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:27'></a>Replying to [bhutz](#comment%3A26):
> Also commenting on the projective_morphism code line 3002-3014 as I helped review that: Except for the unused list variable rem_indices this code is doing exactly what is it supposed to:
> 
> Given a list of periodic points get rid of any that have period less than n, i.e., if you want the minimal 4 periodic points, go through and remove the fixed points and the 2 periodic points.
> 
> Hence the variable j is walking through the iterates (up to n) of each point indexed by i. Since the list is traversed in reverse order, the pop doesn't break it. I'm not saying that there isn't a better way to do this, but the code does do its intended task. Perhaps some additional code comments are in order there so that people can more easily decipher it.


Ben, I can only apologise again for doubting the code hen I just misunderstood it (and specifically what self(P) did).  At some point when this ticket is finished, perhaps adding some comments and removing the redundant line would be a good idea, but there is no hurry.  (Feel free to open a ticket for that already.)



---

archive/issue_comments_361327.json:
```json
{
    "body": "<a id='comment:28'></a>Nils, can you explain the following?  Starting with the most recent commit above, if you make one change to sage/structure/factorization.py\n\n```\n@@ -319,10 +319,7 @@ class Factorization(SageObject):\n         self.__unit = unit\n         self.__cr = cr\n         if sort and self.is_commutative():\n-            try:\n-                self.sort()\n-            except NotImplementedError:\n-                pass\n+            self.sort()\n```\nthen there are many failures, for example\n\n```\nsage: pol26 = hilbert_class_polynomial(-4*26)\nsage: K = NumberField(pol26,'a')                                                                                \nsage: K.optimized_representation()\n<boom>\nNotImplementedError: root finding for this polynomial not implemented\n```",
    "created_at": "2016-02-15T19:54:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361327",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:28'></a>Nils, can you explain the following?  Starting with the most recent commit above, if you make one change to sage/structure/factorization.py

```
@@ -319,10 +319,7 @@ class Factorization(SageObject):
         self.__unit = unit
         self.__cr = cr
         if sort and self.is_commutative():
-            try:
-                self.sort()
-            except NotImplementedError:
-                pass
+            self.sort()
```
then there are many failures, for example

```
sage: pol26 = hilbert_class_polynomial(-4*26)
sage: K = NumberField(pol26,'a')                                                                                
sage: K.optimized_representation()
<boom>
NotImplementedError: root finding for this polynomial not implemented
```



---

archive/issue_comments_361328.json:
```json
{
    "body": "<a id='comment:29'></a>no problem. projective morphism issue opened as #20059.",
    "created_at": "2016-02-15T20:15:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361328",
    "user": "https://github.com/bhutz"
}
```

<a id='comment:29'></a>no problem. projective morphism issue opened as #20059.



---

archive/issue_comments_361329.json:
```json
{
    "body": "<a id='comment:30'></a>Replying to [cremona](#comment%3A28):\n> Nils, can you explain the following?\n\n>...\n> then there are many failures, for example\n> \n> ```\n> sage: pol26 = hilbert_class_polynomial(-4*26)\n> sage: K = NumberField(pol26,'a')         \n> sage: K.optimized_representation()\n> <boom>\n> NotImplementedError: root finding for this polynomial not implemented\n> ```\n\nI am not quite sure what you mean by \"explain\". Do you want me to give my interpretation of the traceback that shows where the failure occurs? In that case:\n\nThe error happens in `sage.rings.polynomial.polynomial_element.pyx:6533`, in `Polynomial.roots`. The relevant branches are:\n\n```\n        try:\n            if K.is_integral_domain():\n                ...\n                return self._roots_from_factorization(self.factor(), multiplicities)\n            else:\n                raise NotImplementedError\n        except NotImplementedError:\n            ...\n            raise NotImplementedError(\"root finding for this polynomial not implemented\")\n```\nWith the change, \"self.factor()\" indirectly raises `NotImplementedError` from the `sort`. The really appropriate solution would be to just not sort in `self.factor` at all, because in general there's no way to sort. But I suspect the number of doctests that need to be adjusted would be very unpleasant. So I'd go with the \"best effort sorting attempt\" of a `try` here.\n\nYou can see two things here:\n- Python's try/except invites a rather coarse-grained error selection here. Python advocates EAFP over LBYL (ask forgiveness rather than look before you leap) and it has advantages in systems programming, where error conditions can be transient, but I think it's a poor match for mathematics programming, exactly for this kind of poorly matched interactions.\n- If you raise an error in an except, you should use a bare `raise` unless you have a very good reason not to. Getting the original error here is way more informative (and cheaper!) than this newly constructed error.\n\nOne option for `factor` would be to make the `sort` keyword tri-state: `True/False/Try`, where False is no sort, True is sort (and error if that doesn't work) and Try (which would be the default for backwards compatibility) to try and be quiet about failure to sort. Then people can ask for `sort=True` if they want to rely on the answer being in a particular order. The `sort=Try` option would have no value other than giving a little nicer output in some cases. This may well be overengineering and it's clear what would happen: everybody would just use the default \"Try\" and rely on the order being the same every time (and be surprised if it's not).",
    "created_at": "2016-02-15T22:13:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361329",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:30'></a>Replying to [cremona](#comment%3A28):
> Nils, can you explain the following?

>...
> then there are many failures, for example
> 
> ```
> sage: pol26 = hilbert_class_polynomial(-4*26)
> sage: K = NumberField(pol26,'a')         
> sage: K.optimized_representation()
> <boom>
> NotImplementedError: root finding for this polynomial not implemented
> ```

I am not quite sure what you mean by "explain". Do you want me to give my interpretation of the traceback that shows where the failure occurs? In that case:

The error happens in `sage.rings.polynomial.polynomial_element.pyx:6533`, in `Polynomial.roots`. The relevant branches are:

```
        try:
            if K.is_integral_domain():
                ...
                return self._roots_from_factorization(self.factor(), multiplicities)
            else:
                raise NotImplementedError
        except NotImplementedError:
            ...
            raise NotImplementedError("root finding for this polynomial not implemented")
```
With the change, "self.factor()" indirectly raises `NotImplementedError` from the `sort`. The really appropriate solution would be to just not sort in `self.factor` at all, because in general there's no way to sort. But I suspect the number of doctests that need to be adjusted would be very unpleasant. So I'd go with the "best effort sorting attempt" of a `try` here.

You can see two things here:
- Python's try/except invites a rather coarse-grained error selection here. Python advocates EAFP over LBYL (ask forgiveness rather than look before you leap) and it has advantages in systems programming, where error conditions can be transient, but I think it's a poor match for mathematics programming, exactly for this kind of poorly matched interactions.
- If you raise an error in an except, you should use a bare `raise` unless you have a very good reason not to. Getting the original error here is way more informative (and cheaper!) than this newly constructed error.

One option for `factor` would be to make the `sort` keyword tri-state: `True/False/Try`, where False is no sort, True is sort (and error if that doesn't work) and Try (which would be the default for backwards compatibility) to try and be quiet about failure to sort. Then people can ask for `sort=True` if they want to rely on the answer being in a particular order. The `sort=Try` option would have no value other than giving a little nicer output in some cases. This may well be overengineering and it's clear what would happen: everybody would just use the default "Try" and rely on the order being the same every time (and be surprised if it's not).



---

archive/issue_comments_361330.json:
```json
{
    "body": "<a id='comment:31'></a>Thanks, that does help clarify what is happening.  I'll keep in the try/except here since otherwise (as you say) a vast number of pther changes would be needed.  I think that the half dozen other palces where we added try/except around sort attempts will not be so problematical.\n\nI hope that after this work is done we'll be able to convince others that the change is worth doing!",
    "created_at": "2016-02-16T09:06:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361330",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:31'></a>Thanks, that does help clarify what is happening.  I'll keep in the try/except here since otherwise (as you say) a vast number of pther changes would be needed.  I think that the half dozen other palces where we added try/except around sort attempts will not be so problematical.

I hope that after this work is done we'll be able to convince others that the change is worth doing!



---

archive/issue_comments_361331.json:
```json
{
    "body": "<a id='comment:32'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2016-02-16T14:17:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361331",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:32'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_361332.json:
```json
{
    "body": "Changing commit from \"ae87261971f6ddc93d1d40c24c3a141be58457bd\" to \"2016dd34852bbdae2bac70e66451bed830753f10\"",
    "created_at": "2016-02-16T14:17:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361332",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "ae87261971f6ddc93d1d40c24c3a141be58457bd" to "2016dd34852bbdae2bac70e66451bed830753f10"



---

archive/issue_comments_361333.json:
```json
{
    "body": "<a id='comment:33'></a>Replying to [cremona](#comment%3A31):\n> Thanks, that does help clarify what is happening.  I'll keep in the try/except here since otherwise (as you say) a vast number of other changes would be needed.  I think that the half dozen other places where we added try/except around sort attempts will not be so problematical.\n\n\nI have almost finished this, and it was not so hard.  A few doctests relied on the list of solutions to some equations and these now need sorted() in the doctest since thesolutions are not automatically sorted.  Apart from that, there were several places where lack of sorting of real & complex roots of a polynomial led to permuting the real and complex embeddings of a number field; this was easy to fix since now the functions K.real_mebeddings() and K.complex_embeddings() use the sorting in RR and CC to sort themselves, which dealt with all such problems.  (In the elliptic curve period lattice code which has a ridiculous number of doctests, by me, I made the test more consistent by always using the above functions instead of sometimes using K.embeddings(RR) and K.embeddings(CC), since K.embeddingss() does not sort).\n\nTwo problems remain, only.  (1)  the file src/sage/tests/french_book/mpoly.py where I was not sure of the etiquette of changing doctest input/output since presumably the book text might need changng too. (2) one failing test in src/sage/schemes/elliptic_curves/ell_point.py and two in src/sage/schemes/elliptic_curves/period_lattice.py are causd by a seprate bug which previously struck me in ticket #18336 *which is still not resolved* (see there for what I am talking about).  If we can fix that then both this ticket and that one could be finished off.\n\nThe new commit is uploaded.\n\n> \n> I hope that after this work is done we'll be able to convince others that the change is worth doing!\n\n----\nNew commits:",
    "created_at": "2016-02-16T14:17:59Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361333",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:33'></a>Replying to [cremona](#comment%3A31):
> Thanks, that does help clarify what is happening.  I'll keep in the try/except here since otherwise (as you say) a vast number of other changes would be needed.  I think that the half dozen other places where we added try/except around sort attempts will not be so problematical.


I have almost finished this, and it was not so hard.  A few doctests relied on the list of solutions to some equations and these now need sorted() in the doctest since thesolutions are not automatically sorted.  Apart from that, there were several places where lack of sorting of real & complex roots of a polynomial led to permuting the real and complex embeddings of a number field; this was easy to fix since now the functions K.real_mebeddings() and K.complex_embeddings() use the sorting in RR and CC to sort themselves, which dealt with all such problems.  (In the elliptic curve period lattice code which has a ridiculous number of doctests, by me, I made the test more consistent by always using the above functions instead of sometimes using K.embeddings(RR) and K.embeddings(CC), since K.embeddingss() does not sort).

Two problems remain, only.  (1)  the file src/sage/tests/french_book/mpoly.py where I was not sure of the etiquette of changing doctest input/output since presumably the book text might need changng too. (2) one failing test in src/sage/schemes/elliptic_curves/ell_point.py and two in src/sage/schemes/elliptic_curves/period_lattice.py are causd by a seprate bug which previously struck me in ticket #18336 *which is still not resolved* (see there for what I am talking about).  If we can fix that then both this ticket and that one could be finished off.

The new commit is uploaded.

> 
> I hope that after this work is done we'll be able to convince others that the change is worth doing!

----
New commits:



---

archive/issue_comments_361334.json:
```json
{
    "body": "Changing dependencies from \"\" to \"#20064\"",
    "created_at": "2016-02-16T14:28:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361334",
    "user": "https://github.com/JohnCremona"
}
```

Changing dependencies from "" to "#20064"



---

archive/issue_comments_361335.json:
```json
{
    "body": "<a id='comment:34'></a>I made a separate ticket for the bug in QQbar.sqrt: #20064.\n\nUntil that is resolved we cannot set this ticket to needs_review, but I would like to get it reviewed!",
    "created_at": "2016-02-16T14:28:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361335",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:34'></a>I made a separate ticket for the bug in QQbar.sqrt: #20064.

Until that is resolved we cannot set this ticket to needs_review, but I would like to get it reviewed!



---

archive/issue_comments_361336.json:
```json
{
    "body": "<a id='comment:35'></a>Replying to [cremona](#comment%3A11):\n> Question:  when doing something like sorted(v,key=list) where v is a list of nf elements in a *relative* number field, I suppose that the coefficient list elements are themselves nf elements which must be sorted somehow?  What is happening then?\n\n\nI don't think `key=list` is a valid way of sorting for relative number fields (unless the base field is embedded and inherits an ordering from that).\n\nAre you wondering whether we should have a \"generic\" key function other than \"str\"? I think that would require another protocol, where objects are required to have a `__default_sorting_key__` method or something similar, and probably a access function `default_sorting_key` that would recursively walk tuples etc (but not sets unless we'd have a way to symmetrize the keys) and call this method on relevant objects.\n\nMy feeling is that the need for such a function is not so pressing just yet, and that we're better off with people having to think about what key they use every time they need one (and in the process perhaps recognize when they don't need sorting).",
    "created_at": "2016-02-18T21:20:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361336",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:35'></a>Replying to [cremona](#comment%3A11):
> Question:  when doing something like sorted(v,key=list) where v is a list of nf elements in a *relative* number field, I suppose that the coefficient list elements are themselves nf elements which must be sorted somehow?  What is happening then?


I don't think `key=list` is a valid way of sorting for relative number fields (unless the base field is embedded and inherits an ordering from that).

Are you wondering whether we should have a "generic" key function other than "str"? I think that would require another protocol, where objects are required to have a `__default_sorting_key__` method or something similar, and probably a access function `default_sorting_key` that would recursively walk tuples etc (but not sets unless we'd have a way to symmetrize the keys) and call this method on relevant objects.

My feeling is that the need for such a function is not so pressing just yet, and that we're better off with people having to think about what key they use every time they need one (and in the process perhaps recognize when they don't need sorting).



---

archive/issue_comments_361337.json:
```json
{
    "body": "<a id='comment:36'></a>Yes, I agree with that.  It has been quite instructive to see that for most use cases the only reason we had been sorting at all was for doctest consistency, and for those, using key=str sufficed.  The exceptions all (I think) are for the cases I care about a lot for the purpose of cataloguing and labelling elliptic curves, and here I'll be writing custom (and properly documented) sorting functions.\n\nSo we don't need default sorting method(s) for number field elements.  I hope others agree.",
    "created_at": "2016-02-19T09:04:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361337",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:36'></a>Yes, I agree with that.  It has been quite instructive to see that for most use cases the only reason we had been sorting at all was for doctest consistency, and for those, using key=str sufficed.  The exceptions all (I think) are for the cases I care about a lot for the purpose of cataloguing and labelling elliptic curves, and here I'll be writing custom (and properly documented) sorting functions.

So we don't need default sorting method(s) for number field elements.  I hope others agree.



---

archive/issue_comments_361338.json:
```json
{
    "body": "<a id='comment:37'></a>I see this ticket hasn't received much attention lately. Should we move this along?\n\nAlso, I've noticed that `_get_embedding_approx(i)` gives an precision of 53*2<sup>i</sup> bits, so the original code (starting i at one and incrementing i) was correct. It's already doubling precision.",
    "created_at": "2016-03-24T16:38:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361338",
    "user": "https://github.com/nbruin"
}
```

<a id='comment:37'></a>I see this ticket hasn't received much attention lately. Should we move this along?

Also, I've noticed that `_get_embedding_approx(i)` gives an precision of 53*2<sup>i</sup> bits, so the original code (starting i at one and incrementing i) was correct. It's already doubling precision.



---

archive/issue_comments_361339.json:
```json
{
    "body": "<a id='comment:38'></a>OK, so a few months have passed and all the work we did for this has so far not come to anything.  Meanwhile I started using sort(..., key=lambda a: a.list()) for my own use (with elements of number fields, where I am happy that I will always use the same generating polynomial for each field so this will be consistent).\nBecause of the above it has been less important for me to have Sage-wide sorting of nf elements fixed, but of course it should be.",
    "created_at": "2016-06-13T08:16:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361339",
    "user": "https://github.com/JohnCremona"
}
```

<a id='comment:38'></a>OK, so a few months have passed and all the work we did for this has so far not come to anything.  Meanwhile I started using sort(..., key=lambda a: a.list()) for my own use (with elements of number fields, where I am happy that I will always use the same generating polynomial for each field so this will be consistent).
Because of the above it has been less important for me to have Sage-wide sorting of nf elements fixed, but of course it should be.



---

archive/issue_comments_361340.json:
```json
{
    "body": "<a id='comment:40'></a>Now after the switch to python3, this ticket should be revisited.",
    "created_at": "2020-07-18T19:44:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361340",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:40'></a>Now after the switch to python3, this ticket should be revisited.



---

archive/issue_events_054586.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-07-18T19:44:57Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20028#event-54586"
}
```



---

archive/issue_events_054587.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20028#event-54587"
}
```



---

archive/issue_events_054588.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20028#event-54588"
}
```



---

archive/issue_comments_361341.json:
```json
{
    "body": "<a id='comment:42'></a>Revisiting this would be nice, indeed.",
    "created_at": "2021-03-17T18:15:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361341",
    "user": "https://github.com/mwageringel"
}
```

<a id='comment:42'></a>Revisiting this would be nice, indeed.



---

archive/issue_comments_361342.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,10 @@\n See the discussion at https://groups.google.com/forum/#!topic/sage-devel/aP5LP09sADY\n+\n+```\n+sage: K.<a> = NumberField(x^3+x+1)\n+sage: K(1) > K(2), K(2) > K(1)\n+(True, True)\n+sage: K(1) < K(2), K(2) < K(1)\n+(False, False)\n+```\n+\n``````\n",
    "created_at": "2021-03-17T18:15:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361342",
    "user": "https://github.com/mwageringel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,10 @@
 See the discussion at https://groups.google.com/forum/#!topic/sage-devel/aP5LP09sADY
+
+```
+sage: K.<a> = NumberField(x^3+x+1)
+sage: K(1) > K(2), K(2) > K(1)
+(True, True)
+sage: K(1) < K(2), K(2) < K(1)
+(False, False)
+```
+
``````




---

archive/issue_events_054589.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-05-10T17:42:09Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20028#event-54589"
}
```



---

archive/issue_events_054590.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-05-10T17:42:09Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20028#event-54590"
}
```



---

archive/issue_comments_361343.json:
```json
{
    "body": "<a id='comment:43'></a>Moving to 9.4, as 9.3 has been released.",
    "created_at": "2021-05-10T17:42:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361343",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:43'></a>Moving to 9.4, as 9.3 has been released.



---

archive/issue_events_054591.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-22T20:35:10Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20028#event-54591"
}
```



---

archive/issue_events_054592.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-08-22T20:35:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20028#event-54592"
}
```



---

archive/issue_events_054593.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:11:26Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20028#event-54593"
}
```



---

archive/issue_events_054594.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:11:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20028#event-54594"
}
```



---

archive/issue_comments_361344.json:
```json
{
    "body": "<a id='comment:46'></a>Can we revive this ticket? \nRelated: #33417 - for `CC`, `CDF`.",
    "created_at": "2022-02-25T18:25:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20028#issuecomment-361344",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:46'></a>Can we revive this ticket? 
Related: #33417 - for `CC`, `CDF`.



---

archive/issue_events_054595.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-02T20:45:28Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20028#event-54595"
}
```



---

archive/issue_events_054596.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-02T20:45:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20028#event-54596"
}
```



---

archive/issue_events_054597.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T16:33:45Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20028#event-54597"
}
```



---

archive/issue_events_054598.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T16:33:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20028",
    "milestone": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20028#event-54598"
}
```
