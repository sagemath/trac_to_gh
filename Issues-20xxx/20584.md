# Issue 20584: Regular partitions: 1-regular partitions are mishandled on occasion

archive/issues_020347.json:
```json
{
    "assignees": [],
    "body": "1-regular partitions exist (although there is just one of them -- the empty one). Not all of the code behaves well at \\ell=1, though; e.g., the iterator runs infinite loops once it's past [].\n\nCC:  @tscrim\n\nKeywords: **partition, regular partition, border case**\n\nBranch/Commit: **[7ba506c](https://github.com/sagemath/sagetrac-mirror/commit/7ba506c69802fb3a259020d7b0cf41767c5cb248)**\n\nReviewer: **Darij Grinberg**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20584_\n\n",
    "closed_at": "2017-12-16T09:49:19Z",
    "created_at": "2016-05-10T20:23:42Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Regular partitions: 1-regular partitions are mishandled on occasion",
    "type": "issue",
    "updated_at": "2017-12-16T09:49:19Z",
    "url": "https://github.com/sagemath/sage/issues/20584",
    "user": "https://github.com/darijgr"
}
```
1-regular partitions exist (although there is just one of them -- the empty one). Not all of the code behaves well at \ell=1, though; e.g., the iterator runs infinite loops once it's past [].

CC:  @tscrim

Keywords: **partition, regular partition, border case**

Branch/Commit: **[7ba506c](https://github.com/sagemath/sagetrac-mirror/commit/7ba506c69802fb3a259020d7b0cf41767c5cb248)**

Reviewer: **Darij Grinberg**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/20584_





---

archive/issue_events_272084.json:
```json
{
    "actor": "https://github.com/darijgr",
    "created_at": "2016-05-10T20:23:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20584",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20584#event-272084"
}
```



---

archive/issue_events_272085.json:
```json
{
    "actor": "https://github.com/darijgr",
    "created_at": "2016-05-10T20:23:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20584",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20584#event-272085"
}
```



---

archive/issue_events_272086.json:
```json
{
    "actor": "https://github.com/darijgr",
    "created_at": "2016-05-10T20:23:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20584",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20584#event-272086"
}
```



---

archive/issue_comments_300167.json:
```json
{
    "body": "Branch: **[public/combinat/fix_1_regular_partitions-20584](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/fix_1_regular_partitions-20584)**",
    "created_at": "2017-12-11T08:28:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20584",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20584#issuecomment-300167",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/combinat/fix_1_regular_partitions-20584](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/fix_1_regular_partitions-20584)**



---

archive/issue_comments_300168.json:
```json
{
    "body": "Commit: **[2f0f912](https://github.com/sagemath/sagetrac-mirror/commit/2f0f912920687460bda5ae5c246dea1981d98fed)**",
    "created_at": "2017-12-11T08:28:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20584",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20584#issuecomment-300168",
    "user": "https://github.com/tscrim"
}
```

Commit: **[2f0f912](https://github.com/sagemath/sagetrac-mirror/commit/2f0f912920687460bda5ae5c246dea1981d98fed)**



---

archive/issue_events_272087.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-12-11T08:28:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/20584",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20584#event-272087"
}
```



---

archive/issue_events_272088.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-12-11T08:28:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20584",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20584#event-272088"
}
```



---

archive/issue_comments_300169.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nHey, I finally remembered to fix this. `^^;;`\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2f0f912920687460bda5ae5c246dea1981d98fed\">2f0f912</a></td><td><code>Better handle 1-regular partitions.</code></td></tr></table>\n",
    "created_at": "2017-12-11T08:28:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20584",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20584#issuecomment-300169",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:1" align="right">Comment 1</div>

Hey, I finally remembered to fix this. `^^;;`

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2f0f912920687460bda5ae5c246dea1981d98fed">2f0f912</a></td><td><code>Better handle 1-regular partitions.</code></td></tr></table>




---

archive/issue_comments_300170.json:
```json
{
    "body": "Author: **Travis Scrimshaw**",
    "created_at": "2017-12-11T08:28:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20584",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20584#issuecomment-300170",
    "user": "https://github.com/tscrim"
}
```

Author: **Travis Scrimshaw**



---

archive/issue_events_272089.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-12-11T08:28:47Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20584",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20584#event-272089"
}
```



---

archive/issue_events_272090.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-12-11T08:29:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20584",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20584#event-272090"
}
```



---

archive/issue_comments_300171.json:
```json
{
    "body": "Changed commit from **[2f0f912](https://github.com/sagemath/sagetrac-mirror/commit/2f0f912920687460bda5ae5c246dea1981d98fed)** to **[7ba506c](https://github.com/sagemath/sagetrac-mirror/commit/7ba506c69802fb3a259020d7b0cf41767c5cb248)**",
    "created_at": "2017-12-11T23:25:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20584",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20584#issuecomment-300171",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[2f0f912](https://github.com/sagemath/sagetrac-mirror/commit/2f0f912920687460bda5ae5c246dea1981d98fed)** to **[7ba506c](https://github.com/sagemath/sagetrac-mirror/commit/7ba506c69802fb3a259020d7b0cf41767c5cb248)**



---

archive/issue_comments_300172.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9fb5274d947cf6589dc6c6c3978b14f9eda4e910\">9fb5274</a></td><td><code>Merge branch 'public/combinat/fix_1_regular_partitions-20584' of git://trac.sagemath.org/sage into reg</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7ba506c69802fb3a259020d7b0cf41767c5cb248\">7ba506c</a></td><td><code>minor corrections</code></td></tr></table>\n",
    "created_at": "2017-12-11T23:25:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20584",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20584#issuecomment-300172",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9fb5274d947cf6589dc6c6c3978b14f9eda4e910">9fb5274</a></td><td><code>Merge branch 'public/combinat/fix_1_regular_partitions-20584' of git://trac.sagemath.org/sage into reg</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7ba506c69802fb3a259020d7b0cf41767c5cb248">7ba506c</a></td><td><code>minor corrections</code></td></tr></table>




---

archive/issue_comments_300173.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nFixed a couple little things, one of which predated this ticket.\n\nI don't know if there is an established way of guaranteeing that any function/method that takes a regular partition will still work with a 1-regular partition. Barring that, the only criterion are the doctests, right? Everything else LGTM.",
    "created_at": "2017-12-11T23:27:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20584",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20584#issuecomment-300173",
    "user": "https://github.com/darijgr"
}
```

<div id="comment:4" align="right">Comment 4</div>

Fixed a couple little things, one of which predated this ticket.

I don't know if there is an established way of guaranteeing that any function/method that takes a regular partition will still work with a 1-regular partition. Barring that, the only criterion are the doctests, right? Everything else LGTM.



---

archive/issue_events_272091.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-12-11T23:31:40Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20584",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20584#event-272091"
}
```



---

archive/issue_events_272092.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-12-11T23:31:40Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20584",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20584#event-272092"
}
```



---

archive/issue_comments_300174.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nReplying to [@darijgr](#comment%3A4):\n> Fixed a couple little things, one of which predated this ticket.\n\nThanks.\n\n> I don't know if there is an established way of guaranteeing that any function/method that takes a regular partition will still work with a 1-regular partition. Barring that, the only criterion are the doctests, right? Everything else LGTM.\n\nNo, I don't think there is such a way other than those functions/methods having explicit checks. This is a degenerate case in terms of my applications (there is not really an [affine] sl<sub>1</sub>), so I'm not really worried.",
    "created_at": "2017-12-11T23:31:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20584",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20584#issuecomment-300174",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:5" align="right">Comment 5</div>

Replying to [@darijgr](#comment%3A4):
> Fixed a couple little things, one of which predated this ticket.

Thanks.

> I don't know if there is an established way of guaranteeing that any function/method that takes a regular partition will still work with a 1-regular partition. Barring that, the only criterion are the doctests, right? Everything else LGTM.

No, I don't think there is such a way other than those functions/methods having explicit checks. This is a degenerate case in terms of my applications (there is not really an [affine] sl<sub>1</sub>), so I'm not really worried.



---

archive/issue_comments_300175.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2017-12-11T23:31:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20584",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20584#issuecomment-300175",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_300176.json:
```json
{
    "body": "Changed reviewer from **Travis Scrimshaw** to **Darij Grinberg**",
    "created_at": "2017-12-11T23:32:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20584",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20584#issuecomment-300176",
    "user": "https://github.com/tscrim"
}
```

Changed reviewer from **Travis Scrimshaw** to **Darij Grinberg**



---

archive/issue_events_272093.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-12-16T09:49:19Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20584",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20584#event-272093"
}
```



---

archive/issue_events_272094.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "fe7733927854d437b1aeca0bd85e254b37d2b301",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-12-16T09:49:19Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20584",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20584#event-272094"
}
```



---

archive/issue_comments_300177.json:
```json
{
    "body": "Changed branch from **[public/combinat/fix_1_regular_partitions-20584](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/fix_1_regular_partitions-20584)** to **[7ba506c](https://github.com/sagemath/sagetrac-mirror/commit/7ba506c69802fb3a259020d7b0cf41767c5cb248)**",
    "created_at": "2017-12-16T09:49:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20584",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20584#issuecomment-300177",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/combinat/fix_1_regular_partitions-20584](https://github.com/sagemath/sagetrac-mirror/tree/public/combinat/fix_1_regular_partitions-20584)** to **[7ba506c](https://github.com/sagemath/sagetrac-mirror/commit/7ba506c69802fb3a259020d7b0cf41767c5cb248)**
