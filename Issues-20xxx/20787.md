...
<a id='comment:5'></a>Replying to [danielaugot](#comment%3A4):
> Hi everyone,
> 
> there is no `codes.GolayCode` in my namespace. As a consequence ./sage -t src/codes/coding vastly fails.
> 
...
<a id='comment:9'></a>Thanks for doing this Daniel!
...
<a id='comment:14'></a>there is a misprint (missing `r`) in `coding/code_constructions.py`
...
```json
{
    "body": "<a id='comment:16'></a>doctest failures in patchbot?",
...
<a id='comment:16'></a>doctest failures in patchbot?
...
<a id='comment:17'></a>indeed. One is easy to fix:
...
<a id='comment:18'></a>The following tests are passed. For the binary case, let us try the manual extension
...
<a id='comment:20'></a>Almost OK, but the test coverage
...
```json
{
    "body": "<a id='comment:21'></a>Are you debugging this?",
    "created_at": "2017-02-07T17:43:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20787",
    "type": "issue_comment",
...
<a id='comment:21'></a>Are you debugging this?
...
<a id='comment:22'></a>I was trying to check `golay_code.py`, and also adding some tests.
...
<a id='comment:23'></a>The bug is in `zeta_polynomial()`, as should be clear from the log:
...
<a id='comment:24'></a>And digging further, one sees that the weight enumerator of `codes.GolayCode(GF(3), False)` is inconsistent with the Sage's convention that it is homogeneous bivariate polynomial rather than the univariate one:
...
```json
{
    "body": "<a id='comment:27'></a>removed the useless (the one from the parent class is just as fast) `weight_enumerator()`. And fixed typos in weight distributions data (yes, indeed, do you guys ever check the code you write? :-( ).",
...
<a id='comment:27'></a>removed the useless (the one from the parent class is just as fast) `weight_enumerator()`. And fixed typos in weight distributions data (yes, indeed, do you guys ever check the code you write? :-( ).
...
```json
{
    "body": "<a id='comment:29'></a>All tests pass, with the option `--long`.",
...
<a id='comment:29'></a>All tests pass, with the option `--long`.
...
```json
{
    "body": "<a id='comment:33'></a>IMHO we deserve co-authorship on this :-)",
    "created_at": "2017-02-10T00:03:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20787",
    "type": "issue_comment",
...
<a id='comment:33'></a>IMHO we deserve co-authorship on this :-)
...
```json
{
    "body": "<a id='comment:34'></a>I could build the documentation after a `make doc-clean`.",
...
<a id='comment:34'></a>I could build the documentation after a `make doc-clean`.
...
<a id='comment:37'></a>the 2nd error comes from #20908 - this ticket has nothing to do with it...
...
```json
{
    "body": "<a id='comment:39'></a>Daniel merged #20908 into this branch, so you you need to fix both #20908 and this branch.",
    "created_at": "2017-02-12T12:35:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20787",
    "type": "issue_comment",
...
<a id='comment:39'></a>Daniel merged #20908 into this branch, so you you need to fix both #20908 and this branch.
...
```json
{
    "body": "<a id='comment:41'></a>all right, all right...",
...
<a id='comment:41'></a>all right, all right...
...
```json
{
    "body": "<a id='comment:43'></a>Thanks a lot, Dima, for so much help. Daniel",
...
<a id='comment:43'></a>Thanks a lot, Dima, for so much help. Daniel
