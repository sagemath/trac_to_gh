# Issue 20299: Binary tarball sanity check when running make

archive/issues_020062.json:
```json
{
    "assignees": [],
    "body": "I don't understand why anybody would download a binary tarball and\nthen run \"make\" as the first command, but a steady trickle of support\nrequests proves me wrong. So just execute ./relocate-once.py if it\nexists, and nobody gets harmed.\n\n\nBranch/Commit: **[ff163cd](https://github.com/sagemath/sagetrac-mirror/commit/ff163cd81a4355c102de003cabdcc5ddcc599290)**\n\nReviewer: **Volker Braun**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20299_\n\n",
    "closed_at": "2016-03-27T07:44:23Z",
    "created_at": "2016-03-26T11:44:07Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Binary tarball sanity check when running make",
    "type": "issue",
    "updated_at": "2016-03-27T07:44:23Z",
    "url": "https://github.com/sagemath/sage/issues/20299",
    "user": "https://github.com/vbraun"
}
```
I don't understand why anybody would download a binary tarball and
then run "make" as the first command, but a steady trickle of support
requests proves me wrong. So just execute ./relocate-once.py if it
exists, and nobody gets harmed.


Branch/Commit: **[ff163cd](https://github.com/sagemath/sagetrac-mirror/commit/ff163cd81a4355c102de003cabdcc5ddcc599290)**

Reviewer: **Volker Braun**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/20299_





---

archive/issue_events_268613.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-03-26T11:44:07Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20299",
    "milestone_number": null,
    "milestone_title": "sage-7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20299#event-268613"
}
```



---

archive/issue_events_268614.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-03-26T11:44:07Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20299",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20299#event-268614"
}
```



---

archive/issue_comments_294971.json:
```json
{
    "body": "Branch: **[u/vbraun/binary_tarball_sanity_check_when_running_make](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/binary_tarball_sanity_check_when_running_make)**",
    "created_at": "2016-03-26T11:56:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20299",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20299#issuecomment-294971",
    "user": "https://github.com/vbraun"
}
```

Branch: **[u/vbraun/binary_tarball_sanity_check_when_running_make](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/binary_tarball_sanity_check_when_running_make)**



---

archive/issue_events_268615.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-03-26T11:58:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20299",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20299#event-268615"
}
```



---

archive/issue_events_268616.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-03-26T11:58:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20299",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20299#event-268616"
}
```



---

archive/issue_comments_294972.json:
```json
{
    "body": "Author: **Volker Braun**",
    "created_at": "2016-03-26T11:58:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20299",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20299#issuecomment-294972",
    "user": "https://github.com/vbraun"
}
```

Author: **Volker Braun**



---

archive/issue_comments_294973.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/92737a99568c015e84f6921a50b06a5bf71c794d\">92737a9</a></td><td><code>Support running make in a fresh binary tarball</code></td></tr></table>\n",
    "created_at": "2016-03-26T11:58:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20299",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20299#issuecomment-294973",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/92737a99568c015e84f6921a50b06a5bf71c794d">92737a9</a></td><td><code>Support running make in a fresh binary tarball</code></td></tr></table>




---

archive/issue_comments_294974.json:
```json
{
    "body": "Commit: **[92737a9](https://github.com/sagemath/sagetrac-mirror/commit/92737a99568c015e84f6921a50b06a5bf71c794d)**",
    "created_at": "2016-03-26T11:58:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20299",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20299#issuecomment-294974",
    "user": "https://github.com/vbraun"
}
```

Commit: **[92737a9](https://github.com/sagemath/sagetrac-mirror/commit/92737a99568c015e84f6921a50b06a5bf71c794d)**



---

archive/issue_comments_294975.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,5 @@\n+I don't understand why anybody would download a binary tarball and\n+then run \"make\" as the first command, but a steady trickle of support\n+requests proves me wrong. So just execute ./relocate-once.py if it\n+exists, and nobody gets harmed.\n \n``````\n",
    "created_at": "2016-03-26T11:58:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20299",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20299#issuecomment-294975",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,5 @@
+I don't understand why anybody would download a binary tarball and
+then run "make" as the first command, but a steady trickle of support
+requests proves me wrong. So just execute ./relocate-once.py if it
+exists, and nobody gets harmed.
 
``````




---

archive/issue_events_268617.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-03-26T11:58:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20299",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20299#event-268617"
}
```



---

archive/issue_comments_294976.json:
```json
{
    "body": "Changed branch from **[u/vbraun/binary_tarball_sanity_check_when_running_make](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/binary_tarball_sanity_check_when_running_make)** to **[u/jdemeyer/binary_tarball_sanity_check_when_running_make](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/binary_tarball_sanity_check_when_running_make)**",
    "created_at": "2016-03-26T12:34:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20299",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20299#issuecomment-294976",
    "user": "https://github.com/jdemeyer"
}
```

Changed branch from **[u/vbraun/binary_tarball_sanity_check_when_running_make](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/binary_tarball_sanity_check_when_running_make)** to **[u/jdemeyer/binary_tarball_sanity_check_when_running_make](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/binary_tarball_sanity_check_when_running_make)**



---

archive/issue_comments_294977.json:
```json
{
    "body": "Changed commit from **[92737a9](https://github.com/sagemath/sagetrac-mirror/commit/92737a99568c015e84f6921a50b06a5bf71c794d)** to **[ff163cd](https://github.com/sagemath/sagetrac-mirror/commit/ff163cd81a4355c102de003cabdcc5ddcc599290)**",
    "created_at": "2016-03-26T12:36:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20299",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20299#issuecomment-294977",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[92737a9](https://github.com/sagemath/sagetrac-mirror/commit/92737a99568c015e84f6921a50b06a5bf71c794d)** to **[ff163cd](https://github.com/sagemath/sagetrac-mirror/commit/ff163cd81a4355c102de003cabdcc5ddcc599290)**



---

archive/issue_comments_294978.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ff163cd81a4355c102de003cabdcc5ddcc599290\">ff163cd</a></td><td><code>Support running make in a fresh binary tarball</code></td></tr></table>\n",
    "created_at": "2016-03-26T12:36:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20299",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20299#issuecomment-294978",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ff163cd81a4355c102de003cabdcc5ddcc599290">ff163cd</a></td><td><code>Support running make in a fresh binary tarball</code></td></tr></table>




---

archive/issue_comments_294979.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nI think this way is much better, since it also supports running `sage -i pkg`, which in turn runs `$MAKE all-toolchain` and `$MAKE pkg`.",
    "created_at": "2016-03-26T12:36:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20299",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20299#issuecomment-294979",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:5'>Comment 5:</a>
I think this way is much better, since it also supports running `sage -i pkg`, which in turn runs `$MAKE all-toolchain` and `$MAKE pkg`.



---

archive/issue_comments_294980.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2016-03-26T14:15:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20299",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20299#issuecomment-294980",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_comments_294981.json:
```json
{
    "body": "Changed author from **Volker Braun** to **Jeroen Demeyer**",
    "created_at": "2016-03-26T14:15:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20299",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20299#issuecomment-294981",
    "user": "https://github.com/vbraun"
}
```

Changed author from **Volker Braun** to **Jeroen Demeyer**



---

archive/issue_events_268618.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-03-26T14:15:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20299",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20299#event-268618"
}
```



---

archive/issue_events_268619.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-03-26T14:15:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20299",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20299#event-268619"
}
```



---

archive/issue_events_268620.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-03-27T07:44:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20299",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20299#event-268620"
}
```



---

archive/issue_events_268621.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "da39d48d0cb979413047d7fefcd8c86a18b3e6fa",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-03-27T07:44:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20299",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20299#event-268621"
}
```



---

archive/issue_comments_294982.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/binary_tarball_sanity_check_when_running_make](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/binary_tarball_sanity_check_when_running_make)** to **[ff163cd](https://github.com/sagemath/sagetrac-mirror/commit/ff163cd81a4355c102de003cabdcc5ddcc599290)**",
    "created_at": "2016-03-27T07:44:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20299",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20299#issuecomment-294982",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/binary_tarball_sanity_check_when_running_make](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/binary_tarball_sanity_check_when_running_make)** to **[ff163cd](https://github.com/sagemath/sagetrac-mirror/commit/ff163cd81a4355c102de003cabdcc5ddcc599290)**
