# Issue 20728: Gracefully downgrade Singular 4 -> 3

archive/issues_020491.json:
```json
{
    "assignees": [],
    "body": "After testing #17254, installing Singular 3 no longer works due to a conflicting Singular 4 omalloc library.\n\nCC:  @sagetrac-jakobkroeker\n\nKeywords: **days74**\n\nBranch/Commit: **[df3b063](https://github.com/sagemath/sagetrac-mirror/commit/df3b063e4bdcf8cce119993e80cf5052a320d6b1)**\n\nReviewer: **Volker Braun**\n\nAuthor: **Jeroen Demeyer**\n\nComponent: **packages: standard**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20728_\n\n",
    "closed_at": "2016-06-01T20:43:06Z",
    "created_at": "2016-05-31T08:40:34Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Gracefully downgrade Singular 4 -> 3",
    "type": "issue",
    "updated_at": "2016-06-01T20:43:06Z",
    "url": "https://github.com/sagemath/sage/issues/20728",
    "user": "https://github.com/jdemeyer"
}
```
After testing #17254, installing Singular 3 no longer works due to a conflicting Singular 4 omalloc library.

CC:  @sagetrac-jakobkroeker

Keywords: **days74**

Branch/Commit: **[df3b063](https://github.com/sagemath/sagetrac-mirror/commit/df3b063e4bdcf8cce119993e80cf5052a320d6b1)**

Reviewer: **Volker Braun**

Author: **Jeroen Demeyer**

Component: **packages: standard**

_Issue created by migration from https://trac.sagemath.org/ticket/20728_





---

archive/issue_events_291846.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-05-31T08:40:34Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20728",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20728#event-291846"
}
```



---

archive/issue_events_291847.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-05-31T08:40:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20728",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "c: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20728#event-291847"
}
```



---

archive/issue_events_291848.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-05-31T08:40:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20728",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20728#event-291848"
}
```



---

archive/issue_events_291849.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-05-31T08:40:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20728",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20728#event-291849"
}
```



---

archive/issue_comments_300197.json:
```json
{
    "body": "Branch: **[u/jdemeyer/gracefully_downgrade_singular_4____3](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/gracefully_downgrade_singular_4____3)**",
    "created_at": "2016-05-31T09:11:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20728",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20728#issuecomment-300197",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/gracefully_downgrade_singular_4____3](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/gracefully_downgrade_singular_4____3)**



---

archive/issue_comments_300198.json:
```json
{
    "body": "Commit: **[df3b063](https://github.com/sagemath/sagetrac-mirror/commit/df3b063e4bdcf8cce119993e80cf5052a320d6b1)**",
    "created_at": "2016-05-31T09:43:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20728",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20728#issuecomment-300198",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[df3b063](https://github.com/sagemath/sagetrac-mirror/commit/df3b063e4bdcf8cce119993e80cf5052a320d6b1)**



---

archive/issue_comments_300199.json:
```json
{
    "body": "<div id=\"comment:2\"></div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/df3b063e4bdcf8cce119993e80cf5052a320d6b1\">df3b063</a></td><td><code>Delete previous omalloc installations</code></td></tr></table>\n",
    "created_at": "2016-05-31T09:43:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20728",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20728#issuecomment-300199",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2"></div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/df3b063e4bdcf8cce119993e80cf5052a320d6b1">df3b063</a></td><td><code>Delete previous omalloc installations</code></td></tr></table>




---

archive/issue_events_291850.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-05-31T09:47:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20728",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20728#event-291850"
}
```



---

archive/issue_comments_300200.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2016-05-31T13:53:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20728",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20728#issuecomment-300200",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_events_291851.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-31T13:53:04Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20728",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20728#event-291851"
}
```



---

archive/issue_events_291852.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-31T13:53:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20728",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20728#event-291852"
}
```



---

archive/issue_comments_300201.json:
```json
{
    "body": "Changed keywords from none to **days74**",
    "created_at": "2016-06-01T05:46:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20728",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20728#issuecomment-300201",
    "user": "https://github.com/tscrim"
}
```

Changed keywords from none to **days74**



---

archive/issue_comments_300202.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/gracefully_downgrade_singular_4____3](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/gracefully_downgrade_singular_4____3)** to **[df3b063](https://github.com/sagemath/sagetrac-mirror/commit/df3b063e4bdcf8cce119993e80cf5052a320d6b1)**",
    "created_at": "2016-06-01T20:43:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20728",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20728#issuecomment-300202",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/gracefully_downgrade_singular_4____3](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/gracefully_downgrade_singular_4____3)** to **[df3b063](https://github.com/sagemath/sagetrac-mirror/commit/df3b063e4bdcf8cce119993e80cf5052a320d6b1)**



---

archive/issue_events_291853.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-06-01T20:43:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20728",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20728#event-291853"
}
```



---

archive/issue_events_291854.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "a544de6f29becfe98cad0ddc167688149757bfd2",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-06-01T20:43:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20728",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20728#event-291854"
}
```
