# Issue 20005: Interpreters are cythonized twice

archive/issues_019768.json:
```json
{
    "assignees": [],
    "body": "There is duplication of interpreters modules, leading to potential build failures:\n\n```\n[ 1/11] Cythonizing sage/ext/interpreters/wrapper_cdf.pyx\n[ 2/11] Cythonizing sage/ext/interpreters/wrapper_cdf.pyx\n[ 3/11] Cythonizing sage/ext/interpreters/wrapper_el.pyx\n[ 4/11] Cythonizing sage/ext/interpreters/wrapper_el.pyx\n[ 5/11] Cythonizing sage/ext/interpreters/wrapper_py.pyx\n[ 6/11] Cythonizing sage/ext/interpreters/wrapper_py.pyx\n[ 7/11] Cythonizing sage/ext/interpreters/wrapper_rdf.pyx\n[ 8/11] Cythonizing sage/ext/interpreters/wrapper_rdf.pyx\n[ 9/11] Cythonizing sage/ext/interpreters/wrapper_rr.pyx\n[10/11] Cythonizing sage/ext/interpreters/wrapper_rr.pyx\n```\n\nThe reason is that these modules appear both in the glob `Extension('*', ['sage/ext/**/*.pyx']),` and in the glob `Extension('*', ['sage/ext/interpreters/*.pyx'])`.\n\n\n\nBranch/Commit: **[03fd930](https://github.com/sagemath/sagetrac-mirror/commit/03fd93074afc2cf1522f51def6ef91b4d2dc7871)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20005_\n\n",
    "closed_at": "2016-02-05T19:33:08Z",
    "created_at": "2016-02-03T14:20:41Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20cython",
        "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Interpreters are cythonized twice",
    "type": "issue",
    "updated_at": "2016-02-05T19:33:08Z",
    "url": "https://github.com/sagemath/sage/issues/20005",
    "user": "https://github.com/jdemeyer"
}
```
There is duplication of interpreters modules, leading to potential build failures:

```
[ 1/11] Cythonizing sage/ext/interpreters/wrapper_cdf.pyx
[ 2/11] Cythonizing sage/ext/interpreters/wrapper_cdf.pyx
[ 3/11] Cythonizing sage/ext/interpreters/wrapper_el.pyx
[ 4/11] Cythonizing sage/ext/interpreters/wrapper_el.pyx
[ 5/11] Cythonizing sage/ext/interpreters/wrapper_py.pyx
[ 6/11] Cythonizing sage/ext/interpreters/wrapper_py.pyx
[ 7/11] Cythonizing sage/ext/interpreters/wrapper_rdf.pyx
[ 8/11] Cythonizing sage/ext/interpreters/wrapper_rdf.pyx
[ 9/11] Cythonizing sage/ext/interpreters/wrapper_rr.pyx
[10/11] Cythonizing sage/ext/interpreters/wrapper_rr.pyx
```

The reason is that these modules appear both in the glob `Extension('*', ['sage/ext/**/*.pyx']),` and in the glob `Extension('*', ['sage/ext/interpreters/*.pyx'])`.



Branch/Commit: **[03fd930](https://github.com/sagemath/sagetrac-mirror/commit/03fd93074afc2cf1522f51def6ef91b4d2dc7871)**

Reviewer: **Travis Scrimshaw**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/20005_





---

archive/issue_events_264793.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-02-03T14:20:41Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20005",
    "milestone_number": null,
    "milestone_title": "sage-7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20005#event-264793"
}
```



---

archive/issue_events_264794.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-02-03T14:20:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20005",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20cython",
    "label_color": "000080",
    "label_name": "component: cython",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20005#event-264794"
}
```



---

archive/issue_events_264795.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-02-03T14:20:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20005",
    "label": "https://github.com/sagemath/sage/labels/p2%20%E2%80%93%20critical",
    "label_color": "ff7700",
    "label_name": "p2 \u2013 critical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20005#event-264795"
}
```



---

archive/issue_events_264796.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-02-03T14:20:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20005",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20005#event-264796"
}
```



---

archive/issue_comments_289126.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -11,5 +11,8 @@\n [ 8/11] Cythonizing sage/ext/interpreters/wrapper_rdf.pyx\n [ 9/11] Cythonizing sage/ext/interpreters/wrapper_rr.pyx\n [10/11] Cythonizing sage/ext/interpreters/wrapper_rr.pyx\n-[11/11] Cythonizing sage/plot/plot3d/parametric_surface.pyx\n ```\n+\n+The reason is that these modules appear both in the glob `Extension('*', ['sage/ext/**/*.pyx']),` and in the glob `Extension('*', ['sage/ext/interpreters/*.pyx'])`.\n+\n+\n``````\n",
    "created_at": "2016-02-03T14:26:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20005",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20005#issuecomment-289126",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -11,5 +11,8 @@
 [ 8/11] Cythonizing sage/ext/interpreters/wrapper_rdf.pyx
 [ 9/11] Cythonizing sage/ext/interpreters/wrapper_rr.pyx
 [10/11] Cythonizing sage/ext/interpreters/wrapper_rr.pyx
-[11/11] Cythonizing sage/plot/plot3d/parametric_surface.pyx
 ```
+
+The reason is that these modules appear both in the glob `Extension('*', ['sage/ext/**/*.pyx']),` and in the glob `Extension('*', ['sage/ext/interpreters/*.pyx'])`.
+
+
``````




---

archive/issue_comments_289127.json:
```json
{
    "body": "Branch: **[u/jdemeyer/interpreters_are_cythonized_twice](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/interpreters_are_cythonized_twice)**",
    "created_at": "2016-02-03T17:11:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20005",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20005#issuecomment-289127",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/interpreters_are_cythonized_twice](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/interpreters_are_cythonized_twice)**



---

archive/issue_comments_289128.json:
```json
{
    "body": "Author: **Jeroen Demeyer**",
    "created_at": "2016-02-03T17:12:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20005",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20005#issuecomment-289128",
    "user": "https://github.com/jdemeyer"
}
```

Author: **Jeroen Demeyer**



---

archive/issue_comments_289129.json:
```json
{
    "body": "Commit: **[03fd930](https://github.com/sagemath/sagetrac-mirror/commit/03fd93074afc2cf1522f51def6ef91b4d2dc7871)**",
    "created_at": "2016-02-03T17:12:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20005",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20005#issuecomment-289129",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[03fd930](https://github.com/sagemath/sagetrac-mirror/commit/03fd93074afc2cf1522f51def6ef91b4d2dc7871)**



---

archive/issue_comments_289130.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/03fd93074afc2cf1522f51def6ef91b4d2dc7871\">03fd930</a></td><td><code>Don't compile ext/interpreters twice</code></td></tr></table>\n",
    "created_at": "2016-02-03T17:12:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20005",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20005#issuecomment-289130",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:3" align="right">Comment 3</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/03fd93074afc2cf1522f51def6ef91b4d2dc7871">03fd930</a></td><td><code>Don't compile ext/interpreters twice</code></td></tr></table>




---

archive/issue_events_264797.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-02-03T17:12:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20005",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20005#event-264797"
}
```



---

archive/issue_events_264798.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-02-04T00:46:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20005",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20005#event-264798"
}
```



---

archive/issue_events_264799.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-02-04T00:46:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20005",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20005#event-264799"
}
```



---

archive/issue_comments_289131.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nLGTM.",
    "created_at": "2016-02-04T00:46:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20005",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20005#issuecomment-289131",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:4" align="right">Comment 4</div>

LGTM.



---

archive/issue_comments_289132.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2016-02-04T00:46:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20005",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20005#issuecomment-289132",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_264800.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-02-05T19:33:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20005",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20005#event-264800"
}
```



---

archive/issue_events_264801.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "f0ad6c5cadb625821dade79aa737edd481b6dbb2",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-02-05T19:33:08Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20005",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20005#event-264801"
}
```



---

archive/issue_comments_289133.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/interpreters_are_cythonized_twice](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/interpreters_are_cythonized_twice)** to **[03fd930](https://github.com/sagemath/sagetrac-mirror/commit/03fd93074afc2cf1522f51def6ef91b4d2dc7871)**",
    "created_at": "2016-02-05T19:33:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20005",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20005#issuecomment-289133",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/interpreters_are_cythonized_twice](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/interpreters_are_cythonized_twice)** to **[03fd930](https://github.com/sagemath/sagetrac-mirror/commit/03fd93074afc2cf1522f51def6ef91b4d2dc7871)**
