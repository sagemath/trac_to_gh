# Issue 20092: Inconsistent doc for Graph.matching() and fixing Edmonds algorithm with labeled graphs

archive/issues_019855.json:
```json
{
    "assignees": [],
    "body": "In the method Graph.matching(), the doc says:\n\n```\n- ``use_edge_labels`` -- boolean (default: ``False``)\n```\nBut in fact `use_edge_labels` is set to `True` by default. This ticket changes the default behavior to `False` to match the documentation.\n\nWe also fix a bug with Edmonds algorithm always using edge labels:\n\n```\nsage: gw = Graph([(0,1,1),(1,2,99),(2,3,1)])\nsage: gw.matching(use_edge_labels=False)\n[(1, 2, 99)]\n```\n\n\nCC:  @tscrim @nathanncohen\n\nKeywords: **matching, graph**\n\nBranch/Commit: **[523f40f](https://github.com/sagemath/sagetrac-mirror/commit/523f40fd94e98776193877af2464d2fb5167c900)**\n\nReviewer: **Travis Scrimshaw, Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Emily Gunawan**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20092_\n\n",
    "closed_at": "2017-01-18T20:39:58Z",
    "created_at": "2016-02-21T17:23:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.6",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Inconsistent doc for Graph.matching() and fixing Edmonds algorithm with labeled graphs",
    "type": "issue",
    "updated_at": "2017-01-18T20:39:58Z",
    "url": "https://github.com/sagemath/sage/issues/20092",
    "user": "https://github.com/egunawan"
}
```
In the method Graph.matching(), the doc says:

```
- ``use_edge_labels`` -- boolean (default: ``False``)
```
But in fact `use_edge_labels` is set to `True` by default. This ticket changes the default behavior to `False` to match the documentation.

We also fix a bug with Edmonds algorithm always using edge labels:

```
sage: gw = Graph([(0,1,1),(1,2,99),(2,3,1)])
sage: gw.matching(use_edge_labels=False)
[(1, 2, 99)]
```


CC:  @tscrim @nathanncohen

Keywords: **matching, graph**

Branch/Commit: **[523f40f](https://github.com/sagemath/sagetrac-mirror/commit/523f40fd94e98776193877af2464d2fb5167c900)**

Reviewer: **Travis Scrimshaw, Frédéric Chapoton**

Author: **Emily Gunawan**

_Issue created by migration from https://trac.sagemath.org/ticket/20092_





---

archive/issue_events_265893.json:
```json
{
    "actor": "https://github.com/egunawan",
    "created_at": "2016-02-21T17:23:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "milestone_number": null,
    "milestone_title": "sage-7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20092#event-265893"
}
```



---

archive/issue_events_265894.json:
```json
{
    "actor": "https://github.com/egunawan",
    "created_at": "2016-02-21T17:23:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20graph%20theory",
    "label_color": "0000ff",
    "label_name": "component: graph theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20092#event-265894"
}
```



---

archive/issue_events_265895.json:
```json
{
    "actor": "https://github.com/egunawan",
    "created_at": "2016-02-21T17:23:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20092#event-265895"
}
```



---

archive/issue_comments_290677.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nTo me, having it fixed in any way is good for as long as it is fixed. I would personally enforce in the code what the doc says, for I expect graph functions to not try to understand the labels unless I explicitly want it to. I'm likely to have my edges labelled with things that make sense to me (color classes for example) and that would not make any sense if understood as a weight.\n\nThough that's only me.\n\nNathann",
    "created_at": "2016-02-21T18:02:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20092#issuecomment-290677",
    "user": "https://github.com/nathanncohen"
}
```

<div id="comment:1" align="right">Comment 1</div>

To me, having it fixed in any way is good for as long as it is fixed. I would personally enforce in the code what the doc says, for I expect graph functions to not try to understand the labels unless I explicitly want it to. I'm likely to have my edges labelled with things that make sense to me (color classes for example) and that would not make any sense if understood as a weight.

Though that's only me.

Nathann



---

archive/issue_comments_290678.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nIMO the least surprising thing (i.e., least likely to break 3rd party code) is to change the default listed in doc so that it matches the code. However, given the extra data that needs to be processed and Nathann's comment (plus my own personal use cases), I think the best thing to do is just change the default value to `False`.",
    "created_at": "2016-02-23T23:05:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20092#issuecomment-290678",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:2" align="right">Comment 2</div>

IMO the least surprising thing (i.e., least likely to break 3rd party code) is to change the default listed in doc so that it matches the code. However, given the extra data that needs to be processed and Nathann's comment (plus my own personal use cases), I think the best thing to do is just change the default value to `False`.



---

archive/issue_comments_290679.json:
```json
{
    "body": "Author: **Emily Gunawan**",
    "created_at": "2016-02-29T00:30:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20092#issuecomment-290679",
    "user": "https://github.com/egunawan"
}
```

Author: **Emily Gunawan**



---

archive/issue_comments_290680.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e8d3946327c040f0133a501e3d877fc8634539d1\">e8d3946</a></td><td><code>In .matching(): (1) set default use_edge_labels to False and (2) in Edmonds algorithm, fix bug so that edge weight is not taken into account when computing max matching if use_edge_labels is set to False. Not done for LP algorithm yet.</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c551f0876b64c1a45107446b968634f4345ab816\">c551f08</a></td><td><code>Fix .matching() bug so that if user inputs algorithm=LP and use_edge_labels=False (default) the matching will ignore edge labels.</code></td></tr></table>\n",
    "created_at": "2016-02-29T00:30:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20092#issuecomment-290680",
    "user": "https://github.com/egunawan"
}
```

<div id="comment:3" align="right">Comment 3</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e8d3946327c040f0133a501e3d877fc8634539d1">e8d3946</a></td><td><code>In .matching(): (1) set default use_edge_labels to False and (2) in Edmonds algorithm, fix bug so that edge weight is not taken into account when computing max matching if use_edge_labels is set to False. Not done for LP algorithm yet.</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c551f0876b64c1a45107446b968634f4345ab816">c551f08</a></td><td><code>Fix .matching() bug so that if user inputs algorithm=LP and use_edge_labels=False (default) the matching will ignore edge labels.</code></td></tr></table>




---

archive/issue_comments_290681.json:
```json
{
    "body": "Commit: **[c551f08](https://github.com/sagemath/sagetrac-mirror/commit/c551f0876b64c1a45107446b968634f4345ab816)**",
    "created_at": "2016-02-29T00:30:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20092#issuecomment-290681",
    "user": "https://github.com/egunawan"
}
```

Commit: **[c551f08](https://github.com/sagemath/sagetrac-mirror/commit/c551f0876b64c1a45107446b968634f4345ab816)**



---

archive/issue_comments_290682.json:
```json
{
    "body": "Branch: **[u/egunawan/20092](https://github.com/sagemath/sagetrac-mirror/tree/u/egunawan/20092)**",
    "created_at": "2016-02-29T00:30:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20092#issuecomment-290682",
    "user": "https://github.com/egunawan"
}
```

Branch: **[u/egunawan/20092](https://github.com/sagemath/sagetrac-mirror/tree/u/egunawan/20092)**



---

archive/issue_events_265896.json:
```json
{
    "actor": "https://github.com/egunawan",
    "created_at": "2016-02-29T00:30:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20092#event-265896"
}
```



---

archive/issue_comments_290683.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nCan you explain what the bug was (including the code which resulted in the bug)? Also could you explain this change to me? I don't see the point in creating a new graph and if this is necessary, why don't you cal `copy`?\n\n```diff\n-                g = self.networkx_graph(copy=False)\n+                g = Graph(self.edges(labels=False))\n+                g = g.networkx_graph(copy=False)\n```\nAlso I don't think this change is good\n\n```diff\n-                    return Integer(len(d)/2)\n+                    return len(d)/2\n```\nas now the output is not guaranteed to be a Sage integer. We try to make sure python `int`s don't escape to the user.",
    "created_at": "2016-02-29T06:20:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20092#issuecomment-290683",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:4" align="right">Comment 4</div>

Can you explain what the bug was (including the code which resulted in the bug)? Also could you explain this change to me? I don't see the point in creating a new graph and if this is necessary, why don't you cal `copy`?

```diff
-                g = self.networkx_graph(copy=False)
+                g = Graph(self.edges(labels=False))
+                g = g.networkx_graph(copy=False)
```
Also I don't think this change is good

```diff
-                    return Integer(len(d)/2)
+                    return len(d)/2
```
as now the output is not guaranteed to be a Sage integer. We try to make sure python `int`s don't escape to the user.



---

archive/issue_comments_290684.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThis is the bug: currently the method takes into account the weights of the edges, if `use_edge_labels=False`:\n\n```\nsage: g=Graph([(0,1),(1,2),(2,3)])\nsage: g.matching()\n[(0, 1, None), (2, 3, None)]\n\nsage: gw=Graph([(0,1,1),(1,2,99),(2,3,1)])\nsage: gw.matching(use_edge_labels=False)\n[(1, 2, 99)]\n```",
    "created_at": "2016-02-29T06:58:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20092#issuecomment-290684",
    "user": "https://github.com/egunawan"
}
```

<div id="comment:5" align="right">Comment 5</div>

This is the bug: currently the method takes into account the weights of the edges, if `use_edge_labels=False`:

```
sage: g=Graph([(0,1),(1,2),(2,3)])
sage: g.matching()
[(0, 1, None), (2, 3, None)]

sage: gw=Graph([(0,1,1),(1,2,99),(2,3,1)])
sage: gw.matching(use_edge_labels=False)
[(1, 2, 99)]
```



---

archive/issue_events_265897.json:
```json
{
    "actor": "https://github.com/egunawan",
    "created_at": "2016-02-29T06:58:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20092#event-265897"
}
```



---

archive/issue_events_265898.json:
```json
{
    "actor": "https://github.com/egunawan",
    "created_at": "2016-02-29T06:58:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20092#event-265898"
}
```



---

archive/issue_comments_290685.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/81810fed21f60f99d2c7817bbe59e7f58cc624d0\">81810fe</a></td><td><code>revert to line int(len(d)/2)</code></td></tr></table>\n",
    "created_at": "2016-02-29T07:08:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20092#issuecomment-290685",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:7" align="right">Comment 7</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/81810fed21f60f99d2c7817bbe59e7f58cc624d0">81810fe</a></td><td><code>revert to line int(len(d)/2)</code></td></tr></table>




---

archive/issue_comments_290686.json:
```json
{
    "body": "Changed commit from **[c551f08](https://github.com/sagemath/sagetrac-mirror/commit/c551f0876b64c1a45107446b968634f4345ab816)** to **[81810fe](https://github.com/sagemath/sagetrac-mirror/commit/81810fed21f60f99d2c7817bbe59e7f58cc624d0)**",
    "created_at": "2016-02-29T07:08:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20092#issuecomment-290686",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[c551f08](https://github.com/sagemath/sagetrac-mirror/commit/c551f0876b64c1a45107446b968634f4345ab816)** to **[81810fe](https://github.com/sagemath/sagetrac-mirror/commit/81810fed21f60f99d2c7817bbe59e7f58cc624d0)**



---

archive/issue_comments_290687.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2016-03-02T15:11:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20092#issuecomment-290687",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_265899.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-03-02T15:11:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20092#event-265899"
}
```



---

archive/issue_comments_290688.json:
```json
{
    "body": "Changed commit from **[81810fe](https://github.com/sagemath/sagetrac-mirror/commit/81810fed21f60f99d2c7817bbe59e7f58cc624d0)** to **[523f40f](https://github.com/sagemath/sagetrac-mirror/commit/523f40fd94e98776193877af2464d2fb5167c900)**",
    "created_at": "2016-03-02T15:11:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20092#issuecomment-290688",
    "user": "https://github.com/tscrim"
}
```

Changed commit from **[81810fe](https://github.com/sagemath/sagetrac-mirror/commit/81810fed21f60f99d2c7817bbe59e7f58cc624d0)** to **[523f40f](https://github.com/sagemath/sagetrac-mirror/commit/523f40fd94e98776193877af2464d2fb5167c900)**



---

archive/issue_comments_290689.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nI made some changes (and rebased to 7.1.beta5) to improve the speed/memory usage, standardize the doc, and make the output types more consistent/compatible. If you agree with my changes, then you can set a positive review.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b8b18479838d9428035725737fca77f5f3c8e289\">b8b1847</a></td><td><code>Merge branch 'u/egunawan/20092' of trac.sagemath.org:sage into public/graphs/matching_doc-20092</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/523f40fd94e98776193877af2464d2fb5167c900\">523f40f</a></td><td><code>Reviewer changes to improve the code and output formats.</code></td></tr></table>\n",
    "created_at": "2016-03-02T15:11:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20092#issuecomment-290689",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:8" align="right">Comment 8</div>

I made some changes (and rebased to 7.1.beta5) to improve the speed/memory usage, standardize the doc, and make the output types more consistent/compatible. If you agree with my changes, then you can set a positive review.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b8b18479838d9428035725737fca77f5f3c8e289">b8b1847</a></td><td><code>Merge branch 'u/egunawan/20092' of trac.sagemath.org:sage into public/graphs/matching_doc-20092</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/523f40fd94e98776193877af2464d2fb5167c900">523f40f</a></td><td><code>Reviewer changes to improve the code and output formats.</code></td></tr></table>




---

archive/issue_events_265900.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-03-02T15:11:29Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "title_is": "Inconsistent doc for Graph.matching() and fixing Edmonds algorithm with labeled graphs",
    "title_was": "Inconsistent doc for Graph.matching()",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20092#event-265900"
}
```



---

archive/issue_comments_290690.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -3,8 +3,13 @@\n ```\n - ``use_edge_labels`` -- boolean (default: ``False``)\n ```\n-But in fact `use_edge_labels` is set to True by default.\n+But in fact `use_edge_labels` is set to `True` by default. This ticket changes the default behavior to `False` to match the documentation.\n \n-Question: Should the doc be changed or should the default value be changed?\n+We also fix a bug with Edmonds algorithm always using edge labels:\n \n-Currently working with Sage version 7.1.beta3\n+```\n+sage: gw = Graph([(0,1,1),(1,2,99),(2,3,1)])\n+sage: gw.matching(use_edge_labels=False)\n+[(1, 2, 99)]\n+```\n+\n``````\n",
    "created_at": "2016-03-02T15:11:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20092#issuecomment-290690",
    "user": "https://github.com/tscrim"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -3,8 +3,13 @@
 ```
 - ``use_edge_labels`` -- boolean (default: ``False``)
 ```
-But in fact `use_edge_labels` is set to True by default.
+But in fact `use_edge_labels` is set to `True` by default. This ticket changes the default behavior to `False` to match the documentation.
 
-Question: Should the doc be changed or should the default value be changed?
+We also fix a bug with Edmonds algorithm always using edge labels:
 
-Currently working with Sage version 7.1.beta3
+```
+sage: gw = Graph([(0,1,1),(1,2,99),(2,3,1)])
+sage: gw.matching(use_edge_labels=False)
+[(1, 2, 99)]
+```
+
``````




---

archive/issue_comments_290691.json:
```json
{
    "body": "Changed branch from **[u/egunawan/20092](https://github.com/sagemath/sagetrac-mirror/tree/u/egunawan/20092)** to **[public/graphs/matching_doc-20092](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/matching_doc-20092)**",
    "created_at": "2016-03-02T15:11:29Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20092#issuecomment-290691",
    "user": "https://github.com/tscrim"
}
```

Changed branch from **[u/egunawan/20092](https://github.com/sagemath/sagetrac-mirror/tree/u/egunawan/20092)** to **[public/graphs/matching_doc-20092](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/matching_doc-20092)**



---

archive/issue_events_265901.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-03-02T15:11:29Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20092#event-265901"
}
```



---

archive/issue_events_265902.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-03-02T15:11:29Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20092#event-265902"
}
```



---

archive/issue_comments_290692.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nok, let it be",
    "created_at": "2017-01-05T12:49:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20092#issuecomment-290692",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:9" align="right">Comment 9</div>

ok, let it be



---

archive/issue_comments_290693.json:
```json
{
    "body": "Changed reviewer from **Travis Scrimshaw** to **Travis Scrimshaw, Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2017-01-05T12:49:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20092#issuecomment-290693",
    "user": "https://github.com/fchapoton"
}
```

Changed reviewer from **Travis Scrimshaw** to **Travis Scrimshaw, Frédéric Chapoton**



---

archive/issue_events_265903.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-01-05T12:49:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20092#event-265903"
}
```



---

archive/issue_events_265904.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-01-05T12:49:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20092#event-265904"
}
```



---

archive/issue_events_265905.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-01-05T16:14:26Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "milestone_number": null,
    "milestone_title": "sage-7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20092#event-265905"
}
```



---

archive/issue_events_265906.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2017-01-05T16:14:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "milestone_number": null,
    "milestone_title": "sage-7.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20092#event-265906"
}
```



---

archive/issue_events_265907.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-01-18T20:39:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20092#event-265907"
}
```



---

archive/issue_events_265908.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "732c9535bd7b0a3b2713b1e9ae11433cf3b7a8d6",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-01-18T20:39:58Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20092#event-265908"
}
```



---

archive/issue_comments_290694.json:
```json
{
    "body": "Changed branch from **[public/graphs/matching_doc-20092](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/matching_doc-20092)** to **[523f40f](https://github.com/sagemath/sagetrac-mirror/commit/523f40fd94e98776193877af2464d2fb5167c900)**",
    "created_at": "2017-01-18T20:39:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20092",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20092#issuecomment-290694",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/graphs/matching_doc-20092](https://github.com/sagemath/sagetrac-mirror/tree/public/graphs/matching_doc-20092)** to **[523f40f](https://github.com/sagemath/sagetrac-mirror/commit/523f40fd94e98776193877af2464d2fb5167c900)**
