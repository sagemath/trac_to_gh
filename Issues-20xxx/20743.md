...
```json
{
    "body": "-1 to returning a tuple. `foo.list()` should return the same as `list(foo)`, which is always a list, and `foo.tuple()` should return the same as `tuple(foo)`, which is always a tuple. And for that matter, `foo.str()` returns the same as `str(foo)` which is always a string, et cetera.",
...
```json
{
    "body": "Definitely +1 on returning something immutable, and a tuple is the natural choice; I have been desiring it for a while. The naming issue is somewhat annoying, but that's life, I guess. The only thing that worries me is that this is going to break backward compatibility, and I am not sure there is anything we can do ...",
...
```json
{
    "body": "Alternative idea: create a new type `immutable_list` inheriting from `list`. This has the advantage of being backwards compatible: `isinstance(foo, list)` would still be true and the printing would be like a list. This works and behaves like a `list`:\n\n```\ncdef class immutable_list(list):\n    def __setitem__(self, i, value):\n        raise TypeError\n```\nOf course, you also need to disallow `.append()`, `+=`, `__delitem__`, ... but the basic idea works.",
...
```json
{
    "body": "Trying to fix the bug, I did something else in #20902, which also fixes somehow the bug in the current ticket. Please check.",
...
```json
{
    "body": "Perhaps I need to clarify how #20902 fixes the issue. With the patch,\n\n```\nsage: F=GF(5)\nsage: F.list()\n[0, 1, 2, 3, 4]\nsage: F.list().remove(0)\nsage: F.list()\n[0, 1, 2, 3, 4]\nsage: F.list.__module__\n'sage.categories.finite_enumerated_sets' \n```",
...
```json
{
    "body": "We can just keep this around for right now to note the discussion and the issue.",
