...
```json
{
    "body": "<a id='comment:2'></a>I tried and failed so far. Help is required, please.\n\n---\nNew commits:",
...
<a id='comment:2'></a>I tried and failed so far. Help is required, please.
...
<a id='comment:3'></a>Not related to installation problem, but in the `dependencies` file you should put
...
<a id='comment:5'></a>BTW, when building a source package of a Python project, use `./setup.py sdist` to make the tarball (also make sure the tarball contains all the files it should contain, and none that it shouldn't contain--you might have to edit a `MANIFEST.in` file to adjust it a la https://docs.python.org/2/distutils/sourcedist.html#specifying-the-files-to-distribute)
...
```json
{
    "body": "<a id='comment:8'></a>I am making some slow progress.",
...
<a id='comment:8'></a>I am making some slow progress.
...
<a id='comment:10'></a>There is a fundamental problem here, mentioned also at https://github.com/robertwb/sage-patchbot/issues/79
...
```json
{
    "body": "<a id='comment:12'></a>So it's a build dep for Sage and should be installed \"the usual way\" using the system Python.",
...
<a id='comment:12'></a>So it's a build dep for Sage and should be installed "the usual way" using the system Python.
...
```json
{
    "body": "<a id='comment:14'></a>I changed the name of the package back to sage-patchbot. But I am little bit stuck here.\n\nIt kind of worked with the package name \"patchbot\", but it was installed in SAGE_LOCAL.",
...
<a id='comment:14'></a>I changed the name of the package back to sage-patchbot. But I am little bit stuck here.
...
<a id='comment:18'></a>This is becoming urgent, to cope with https trac.
...
<a id='comment:20'></a>I am trying a new way:
...
```json
{
    "body": "<a id='comment:23'></a>Can you please update the patchbot to *only* deal with the https thing and not change the whole install procedure?",
...
<a id='comment:23'></a>Can you please update the patchbot to *only* deal with the https thing and not change the whole install procedure?
...
<a id='comment:24'></a>I am doing what I can, but this is a bit too complicated for me to handle easily.
...
<a id='comment:29'></a>Replying to [chapoton](#comment%3A26):
> Is there any way to install somewhere inside sage a python package using the system python ? Is there any existing case ? Where should it go ? How should I do that ?
...
```json
{
    "body": "<a id='comment:30'></a>If the `patchbot` becomes a system package, it should be removed as Sage package. I.e. remove the whole directory `build/pkgs/patchbot`.",
...
<a id='comment:30'></a>If the `patchbot` becomes a system package, it should be removed as Sage package. I.e. remove the whole directory `build/pkgs/patchbot`.
...
<a id='comment:33'></a>If installed correctly there should be no `sage/local/bin/patchbot/patchbot.py`
...
<a id='comment:34'></a>Replying to [jdemeyer](#comment%3A22):
> Replying to [ticket:20736 chapoton]:
> > usage: sudo pip install sage-patchbot-2.6.0.tar.gz
...
```json
{
    "body": "<a id='comment:35'></a>The 2.6.0 seems to work well enough on sage4, although at some point it tested closed tickets.\n\nOn the other hand, arando is uselessly running 2.5.7.",
    "created_at": "2016-06-24T08:32:47Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20736",
    "type": "issue_comment",
...
<a id='comment:35'></a>The 2.6.0 seems to work well enough on sage4, although at some point it tested closed tickets.
...
```json
{
    "body": "<a id='comment:38'></a>I don't know why you would run `./sage --patchbot` at all, as opposed to just `patchbot`.  Why is it necessary to use sage to run the thing that's supposed to be testing sage?",
...
<a id='comment:38'></a>I don't know why you would run `./sage --patchbot` at all, as opposed to just `patchbot`.  Why is it necessary to use sage to run the thing that's supposed to be testing sage?
...
<a id='comment:39'></a>I propose the following "modus operandi" for patchbot installation and use
...
```json
{
    "body": "<a id='comment:40'></a>much more annoying is the fact that the patchbot persist to test closed tickets..",
...
<a id='comment:40'></a>much more annoying is the fact that the patchbot persist to test closed tickets..
...
<a id='comment:41'></a>> get the latest tarball.tar.gz from somewhere (git or my web page?)
...
<a id='comment:42'></a>Let us try to move forward, please. This works, as far as I can tell.
...
<a id='comment:43'></a>If I install the package on my system Python, then doing
...
<a id='comment:44'></a>All right, the command *must* specify the sage root as
...
```json
{
    "body": "<a id='comment:45'></a>(I launch the package version of the patchbot on librae)",
...
<a id='comment:45'></a>(I launch the package version of the patchbot on librae)
...
```json
{
    "body": "<a id='comment:46'></a>I guess that the log should not go under `SAGE_ROOT/logs/patchbot` anymore now. The path should be part of the configuration file. What do you think?",
...
<a id='comment:46'></a>I guess that the log should not go under `SAGE_ROOT/logs/patchbot` anymore now. The path should be part of the configuration file. What do you think?
...
```json
{
    "body": "<a id='comment:47'></a>What about switching to [logging Python module](https://docs.python.org/2/library/logging.html)?",
...
<a id='comment:47'></a>What about switching to [logging Python module](https://docs.python.org/2/library/logging.html)?
...
<a id='comment:48'></a>Replying to [vdelecroix](#comment%3A43):
> If I install the package on my system Python, then doing
> 
> ```
> $ python -m sage_patchbot.patchbot
...
<a id='comment:49'></a>Replying to [vdelecroix](#comment%3A44):
> I don't understand why you do not want to put it on PyPI. I can do it myself if you do not want to investigate. But it really takes less than a minute and greatly simplify the installation, version upgrade, online documentation, ...
...
<a id='comment:50'></a>Replying to [chapoton](#comment%3A48):
> Concerning pypi, it may be simpler once done, but I am not really ready to add yet another layer of complexity.
...
<a id='comment:51'></a>Replying to [chapoton](#comment%3A48):
> Replying to [vdelecroix](#comment%3A43):
> > If I install the package on my system Python, then doing
> > 
> > ```
...
<a id='comment:52'></a>The patchbot librae is hanging on #19092 since more than a day
...
```json
{
    "body": "<a id='comment:53'></a>Maybe due to swap (I need to check)...",
...
<a id='comment:53'></a>Maybe due to swap (I need to check)...
...
```json
{
    "body": "<a id='comment:55'></a>Please update the ticket description with the recommended way to install and run the patchbot. Then I will test it.",
    "created_at": "2016-07-25T09:06:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20736",
    "type": "issue_comment",
...
<a id='comment:55'></a>Please update the ticket description with the recommended way to install and run the patchbot. Then I will test it.
...
```json
{
    "body": "<a id='comment:56'></a>You should also decide whether you want to support `./sage --patchbot` or not. If you do, it should actually work (see[comment:37](#comment%3A37)). If you don't, remove the command (or better: make it print some message of which commands should actually be run).",
...
<a id='comment:56'></a>You should also decide whether you want to support `./sage --patchbot` or not. If you do, it should actually work (see[comment:37](#comment%3A37)). If you don't, remove the command (or better: make it print some message of which commands should actually be run).
...
```json
{
    "body": "<a id='comment:58'></a>Like I said before, `./sage --patchbot` does not actually work.",
...
<a id='comment:58'></a>Like I said before, `./sage --patchbot` does not actually work.
...
```json
{
    "body": "<a id='comment:59'></a>See the last line of [comment:37]",
...
<a id='comment:59'></a>See the last line of [comment:37]
...
```json
{
    "body": "<a id='comment:60'></a>Indeed the command `./sage --patchbot` has no clue where the patchbot is actually installed. It might be on system python, in Sage python, in a virtualenv, or something else... I think it is much safer to disable it completely with an error message (as proposed in [comment:56](#comment%3A56)).",
...
<a id='comment:60'></a>Indeed the command `./sage --patchbot` has no clue where the patchbot is actually installed. It might be on system python, in Sage python, in a virtualenv, or something else... I think it is much safer to disable it completely with an error message (as proposed in [comment:56](#comment%3A56)).
...
```json
{
    "body": "<a id='comment:61'></a>I think it makes sense to assume that, if you run `./sage --patchbot`, you want to run the patchbot which is installed from a Sage shell. That would just require some part of `sage-env` to be executed before running the patchbot.",
...
<a id='comment:61'></a>I think it makes sense to assume that, if you run `./sage --patchbot`, you want to run the patchbot which is installed from a Sage shell. That would just require some part of `sage-env` to be executed before running the patchbot.
...
<a id='comment:63'></a>It seems that we are not making progress here. Jeroen, why do you insist so strongly on installing the patchbot package inside the sage shell ? It makes not much sense to me. I think one can reasonably expect anybody wanting to run a patchbot to be able to install pip first. There is no need to call "sage -pip".
...
<a id='comment:64'></a>> Jeroen, why do you insist so strongly on installing the patchbot package inside the sage shell ?
...
```json
{
    "body": "<a id='comment:65'></a>To turn the question around: why do you keep insisting on not supporting a patchbot installed within Sage?",
...
<a id='comment:65'></a>To turn the question around: why do you keep insisting on not supporting a patchbot installed within Sage?
...
<a id='comment:66'></a>I do not insist on that either, I just do not know how to do it,
and I am happy enough with the current state of things. I would prefer to stop spending
my time on this ticket, and to get more momentum towards python 3 instead.
...
<a id='comment:67'></a>It is supported with
...
```json
{
    "body": "<a id='comment:68'></a>Jeroen, do you also prefer (1) ?",
...
<a id='comment:68'></a>Jeroen, do you also prefer (1) ?
...
<a id='comment:71'></a>Replying to [chapoton](#comment%3A66):
> I do not insist on that either, I just do not know how to do it
...
```json
{
    "body": "<a id='comment:73'></a>First attempt to support a patchbot installed within Sage, not fully tested yet.",
...
<a id='comment:73'></a>First attempt to support a patchbot installed within Sage, not fully tested yet.
...
<a id='comment:74'></a>Replying to [vdelecroix](#comment%3A70):
> Why is that? It (seems to) work for me.
...
```json
{
    "body": "<a id='comment:75'></a>Hang on, I have a better solution in mind to run the patchbot.",
...
<a id='comment:75'></a>Hang on, I have a better solution in mind to run the patchbot.
...
```json
{
    "body": "<a id='comment:77'></a>Currently testing...",
...
<a id='comment:77'></a>Currently testing...
...
```json
{
    "body": "<a id='comment:78'></a>This is good for me. Can anybody review my commit?",
...
<a id='comment:78'></a>This is good for me. Can anybody review my commit?
...
<a id='comment:79'></a>does not work for me:
...
```json
{
    "body": "<a id='comment:80'></a>Instead of a sloppy script, I would be very much happier with `sage -patchbot` returning an informative error message.",
...
<a id='comment:80'></a>Instead of a sloppy script, I would be very much happier with `sage -patchbot` returning an informative error message.
...
```json
{
    "body": "<a id='comment:82'></a>So what exactly is necessary to make this branch work, i.e. to run `sage -patchbot` successfully?",
...
<a id='comment:82'></a>So what exactly is necessary to make this branch work, i.e. to run `sage -patchbot` successfully?
...
```json
{
    "body": "<a id='comment:83'></a>Answer: `./sage -pip install sage-patchbot` or the downloaded 2.6.1 tarball as argument. This means the wiki instructions need to change which I just did, please check.",
...
<a id='comment:83'></a>Answer: `./sage -pip install sage-patchbot` or the downloaded 2.6.1 tarball as argument. This means the wiki instructions need to change which I just did, please check.
...
```json
{
    "body": "<a id='comment:84'></a>Please do not interfere with the wiki instructions until this ticket is closed. Revert your changes there, please.",
...
<a id='comment:84'></a>Please do not interfere with the wiki instructions until this ticket is closed. Revert your changes there, please.
...
```json
{
    "body": "<a id='comment:85'></a>Thanks. The current branch should be considered as broken. The wiki instructions were\nperfectly working until Jeroen made his new branch.\n\n**EDIT**: a working branch is at **u/chapoton/20736**",
...
<a id='comment:85'></a>Thanks. The current branch should be considered as broken. The wiki instructions were
perfectly working until Jeroen made his new branch.
...
<a id='comment:86'></a>Replying to [rws](#comment%3A82):
> So what exactly is necessary to make this branch work, i.e. to run `sage -patchbot` successfully?
...
<a id='comment:87'></a>Proposed solution:
...
```json
{
    "body": "<a id='comment:88'></a>My proposal would be to fix `./sage --patchbot` to work both with a patchbot installed in Sage as well as a patchbot installed system-wide. This is easy to do, but it would result in what Vincent calls \"a sloppy script\".",
...
<a id='comment:88'></a>My proposal would be to fix `./sage --patchbot` to work both with a patchbot installed in Sage as well as a patchbot installed system-wide. This is easy to do, but it would result in what Vincent calls "a sloppy script".
...
```json
{
    "body": "<a id='comment:89'></a>I think the goal should be to make it as easy as possible to install and run the patchbot. We could even make it such that `./sage -i patchbot && ./sage --patchbot` still works as before.",
...
<a id='comment:89'></a>I think the goal should be to make it as easy as possible to install and run the patchbot. We could even make it such that `./sage -i patchbot && ./sage --patchbot` still works as before.
...
<a id='comment:90'></a>I think it is more simple to propose a unique way to install and run the patchbot
...
<a id='comment:91'></a>Replying to [chapoton](#comment%3A90):
>  And this is still only two command lines.
...
```json
{
    "body": "<a id='comment:92'></a>Meaning no offense, this comment is slightly small-minded, IMHO",
...
<a id='comment:92'></a>Meaning no offense, this comment is slightly small-minded, IMHO
...
```json
{
    "body": "<a id='comment:93'></a>Let's assume for a second that users are intelligent enough to run commands on their system that are not wrapped in Sage, and that they have working systems with working Python installations...\n\nWhat advantage is there to\n\n```\n$ ./sage -patchbot\n```\nover \n\n```\n$ patchbot\n```",
...
<a id='comment:93'></a>Let's assume for a second that users are intelligent enough to run commands on their system that are not wrapped in Sage, and that they have working systems with working Python installations...
...
<a id='comment:97'></a>The `sage4` patchbot is broken:
...
```json
{
    "body": "<a id='comment:98'></a>There is something very wrong in how the patchbot gets its tickets from the server...",
...
<a id='comment:98'></a>There is something very wrong in how the patchbot gets its tickets from the server...
...
<a id='comment:100'></a>Replying to [chapoton](#comment%3A99):
> But your specific problem with sage4 is also that it has already recently given a report on every available ticket with "BuildFailed" status.
...
```json
{
    "body": "<a id='comment:2'></a>Rebased over #21430, partially squashed, fixed installations in system Python.",
...
<a id='comment:2'></a>Rebased over #21430, partially squashed, fixed installations in system Python.
...
```json
{
    "body": "<a id='comment:3'></a>I am currently running the patchbot on `sage4` and `arando` with this branch.",
...
<a id='comment:3'></a>I am currently running the patchbot on `sage4` and `arando` with this branch.
...
<a id='comment:4'></a>If you guys are not on agreement, then I am +1 on removing or deprecating `sage --patchbot`. As a recent volunteer to run a patchbot, I thought it is illogical to invoke Sage to run a patchbot whose mission is to test Sage. Installing patchbot by "pip ..." and then running "python -m ..." is straightforward and simple. Why do we need an alternate and conceptually confusing way to launch a patchbot? 
...
<a id='comment:105'></a>Replying to [klee](#comment%3A104):
> If you guys are not on agreement, then I am +1 on removing or deprecating `sage --patchbot`. As a recent volunteer to run a patchbot, I thought it is illogical to invoke Sage to run a patchbot whose mission is to test Sage.
...
<a id='comment:106'></a>> I don't see it as invoking Sage. I consider `sage --patchbot` just a simple convenience command. 
...
<a id='comment:107'></a>Replying to [klee](#comment%3A106):
> the command gives the impression that I am about to run a patchbot installed inside Sage. 
...
<a id='comment:108'></a>Replying to [klee](#comment%3A106):
> > I don't see it as invoking Sage. I consider `sage --patchbot` just a simple convenience command. 
...
<a id='comment:109'></a>Replying to [embray](#comment%3A108):
> `sage --make`
...
```json
{
    "body": "<a id='comment:0'></a>Personally I see no problem with supporting `sage --patchbot`.  Just that it should not try to run a patchbot module that's installed *outside* the sage environment.  In other words, it should run the patchbot that's installed in sage, or do nothing at all (or at most give an error).",
...
<a id='comment:0'></a>Personally I see no problem with supporting `sage --patchbot`.  Just that it should not try to run a patchbot module that's installed *outside* the sage environment.  In other words, it should run the patchbot that's installed in sage, or do nothing at all (or at most give an error).
...
<a id='comment:111'></a>Replying to [embray](#comment%3A110):
> Personally I see no problem with supporting `sage --patchbot`.  Just that it should not try to run a patchbot module that's installed *outside* the sage environment.  In other words, it should run the patchbot that's installed in sage, or do nothing at all (or at most give an error).
...
```json
{
    "body": "<a id='comment:2'></a>Whatever way this ticket is closed, I do not care. If you manage to agree on something, go on.",
...
<a id='comment:2'></a>Whatever way this ticket is closed, I do not care. If you manage to agree on something, go on.
...
```json
{
    "body": "<a id='comment:3'></a>So Fr\u00e9d\u00e9ric, do you retract comment [comment:85]? Because then the easiest would be to go back to that branch (i.e. my branch from [comment:76]). Judging from Erik's comment, he should agree on that.",
...
<a id='comment:3'></a>So Frédéric, do you retract comment [comment:85]? Because then the easiest would be to go back to that branch (i.e. my branch from [comment:76]). Judging from Erik's comment, he should agree on that.
...
<a id='comment:5'></a>Uhh, the branch at https://git.sagemath.org/sage.git/commit/?id=6d7990e88c9f194f28309bd0015c40ecc09080bf is what I'm objecting to.  It attempts to run `sage_patchbot` with the system python as well, which I'm saying it shouldn't do.  `sage --whatever` shouldn't *touch* the system Python except when building sage itself.
...
```json
{
    "body": "<a id='comment:6'></a>So it seems that we cannot agree on anything then...",
...
<a id='comment:6'></a>So it seems that we cannot agree on anything then...
...
<a id='comment:7'></a>I thought we did... please re-read #20736#comment:110
...
<a id='comment:118'></a>Replying to [jdemeyer](#comment%3A109):
> You need good arguments to *explicitly not support* something.
...
<a id='comment:119'></a>Replying to [embray](#comment%3A117):
> I thought we did... please re-read #20736#comment:110
...
<a id='comment:0'></a>Okay, but when we say "the patchbot installed within Sage" you mean there's a copy of the `sage_patchbot` package in `local/lib/python2.7/site-packages`, right?
...
<a id='comment:1'></a>I haven't followed the full discussion, nor have I ever run a patchbot, but here's my 2 cents.
...
```json
{
    "body": "<a id='comment:122'></a>Replying to [jdemeyer](#comment%3A119):\n> You cannot use the Python-in-Sage to run the patchbot because that one might be reinstalled: the patchbot builds Sage so it might rebuild the Python-in-Sage too. And reinstalling a program that you are running is a recipe for trouble.\n\n\nReinstalled by what, a Sage upgrade or a patchbot run?",
...
<a id='comment:122'></a>Replying to [jdemeyer](#comment%3A119):
> You cannot use the Python-in-Sage to run the patchbot because that one might be reinstalled: the patchbot builds Sage so it might rebuild the Python-in-Sage too. And reinstalling a program that you are running is a recipe for trouble.
...
<a id='comment:123'></a>Replying to [rws](#comment%3A122):
> Replying to [jdemeyer](#comment%3A119):
> > You cannot use the Python-in-Sage to run the patchbot because that one might be reinstalled: the patchbot builds Sage so it might rebuild the Python-in-Sage too. And reinstalling a program that you are running is a recipe for trouble.
...
<a id='comment:124'></a>Replying to [embray](#comment%3A120):
> TL;DR I don't see what you gain from that.
...
<a id='comment:125'></a>Replying to [jdemeyer](#comment%3A123):
> Replying to [rws](#comment%3A122):
> > Replying to [jdemeyer](#comment%3A119):
> > > You cannot use the Python-in-Sage to run the patchbot because that one might be reinstalled: the patchbot builds Sage so it might rebuild the Python-in-Sage too. And reinstalling a program that you are running is a recipe for trouble.
...
```json
{
    "body": "<a id='comment:6'></a>But wait, a Python rebuild happens potentially only with unsafe tickets, as well.",
...
<a id='comment:6'></a>But wait, a Python rebuild happens potentially only with unsafe tickets, as well.
...
```json
{
    "body": "<a id='comment:7'></a>So your precaution doesn't matter because Python rebuilds are done in /tmp, not in the Sage installation that runs the patchbot.",
...
<a id='comment:7'></a>So your precaution doesn't matter because Python rebuilds are done in /tmp, not in the Sage installation that runs the patchbot.
...
<a id='comment:128'></a>Replying to [rws](#comment%3A126):
> But wait, a Python rebuild happens potentially only with unsafe tickets, as well.
...
```json
{
    "body": "<a id='comment:9'></a>By the way, I was unaware that \"unsafe\" tickets were never tested. I never realized that.",
...
<a id='comment:9'></a>By the way, I was unaware that "unsafe" tickets were never tested. I never realized that.
...
```json
{
    "body": "<a id='comment:0'></a>You can test them but with some runs there were big problems. That's why it's discouraged to not use `--safe-only`.",
...
<a id='comment:0'></a>You can test them but with some runs there were big problems. That's why it's discouraged to not use `--safe-only`.
...
<a id='comment:131'></a>Replying to [mkoeppe](#comment%3A121):
> I think it's inviting trouble to have the testing framework depend in any way on scripts of the software to be tested. 
...
<a id='comment:2'></a>This ticket seems stalled because jdemeyer insists to support running patchbot installed inside Sage by the system python using Sage interface, whose idea is objected by some others.
...
<a id='comment:133'></a>Replying to [klee](#comment%3A132):
> This objective is in direct conflict with the jdemeyer's idea.
...
<a id='comment:5'></a>>The option sage-root should be added as a config file parameter. 
...
```json
{
    "body": "<a id='comment:6'></a>IMHO, it's obvious that the `--sage-root` when running the patchbot is the Sage installation that you want to test. I don't see the point of using a more obscure name like `--sage-testing-root` for the option.",
...
<a id='comment:6'></a>IMHO, it's obvious that the `--sage-root` when running the patchbot is the Sage installation that you want to test. I don't see the point of using a more obscure name like `--sage-testing-root` for the option.
...
<a id='comment:137'></a>Replying to [jdemeyer](#comment%3A136):
...
```json
{
    "body": "<a id='comment:8'></a>And the `'sage -patchbot'` command should print a warning that `'sage -patchbot'` is no longer the preferred way to use patchbot and print the preferred way to use the patchbot or link to the wiki (https://wiki.sagemath.org/buildbot/details)",
...
<a id='comment:8'></a>And the `'sage -patchbot'` command should print a warning that `'sage -patchbot'` is no longer the preferred way to use patchbot and print the preferred way to use the patchbot or link to the wiki (https://wiki.sagemath.org/buildbot/details)
...
<a id='comment:139'></a>Replying to [jdemeyer](#comment%3A136):
> IMHO, it's obvious that the `--sage-root` when running the patchbot is the Sage installation that you want to test. I don't see the point of using a more obscure name like `--sage-testing-root` for the option.
...
<a id='comment:140'></a>Replying to [embray](#comment%3A139):
> doesn't serve any purpose
...
<a id='comment:1'></a>I'm not sure why you need to run `python -m sage_patchbot.patchbot`.  Although that's not bad practice in cases where you want to make sure you're running the right `sage_patchbot` with the right `python`, in most cases (I think?) it suffices to just run `patchbot`.
...
```json
{
    "body": "<a id='comment:3'></a>Rebased to Sage-7.5.rc1",
...
<a id='comment:3'></a>Rebased to Sage-7.5.rc1
...
<a id='comment:4'></a>Since this ticket seems not to reach a consensus, what about splitting it into two part:
...
<a id='comment:145'></a>Replying to [tmonteil](#comment%3A144):
> * remove patchbot optional spkg, and deprecate the related `sage -patchbot` command (this can be done for next beta)
> * discuss and write a convenient alternative command or documentation to run the patchbot (this seems the blocking point and might require more time)
...
<a id='comment:146'></a>OK, see #22473.
...
<a id='comment:147'></a>Replying to [tmonteil](#comment%3A146):
> It is just a suggestion until the current ticket get fixed
...
<a id='comment:148'></a>Replying to [tmonteil](#comment%3A146):
> https://wiki.sagemath.org/buildbot#Installing_the_patchbot (which are the most up-to-date instructions). It is just a suggestion until the current ticket get fixed
...
```json
{
    "body": "<a id='comment:0'></a>I think IIRC I was fine with keeping `sage -patchbot` as a shortcut for \"run `patchbot` with the `--sage-root` option set from this sage).  I think that makes sense, and I think that's what Jeroen was trying to do originally.  I just thought the solution was overcomplicated.\n\nI don't know why we're talking about `python -m` anything.  When you install the `sage-patchbot` Python package (which Fr\u00e9d\u00e9ric did a great job putting together at my urging) it installs a `patchbot` script.  If you run that it just works, you just have to supply the correct `--sage-root` argument.",
...
<a id='comment:0'></a>I think IIRC I was fine with keeping `sage -patchbot` as a shortcut for "run `patchbot` with the `--sage-root` option set from this sage).  I think that makes sense, and I think that's what Jeroen was trying to do originally.  I just thought the solution was overcomplicated.
...
<a id='comment:1'></a>What I want in particular is to install the patchbot within the Sage environment and have `sage --patchbot` recognize that.
...
```json
{
    "body": "<a id='comment:2'></a>Was there some reason that wouldn't work? That is, if you ran `sage -pip install sage-patchbot`?  Then, in principle, it would work if you ran `sage -sh -c patchbot` though I guess if `$SAGE_LOCAL/bin/patchbot` exists then `sage -patchbot` can be a shortcut for that.  I'm just not sure why you'd need or want to do that but I don't oppose it.",
...
<a id='comment:2'></a>Was there some reason that wouldn't work? That is, if you ran `sage -pip install sage-patchbot`?  Then, in principle, it would work if you ran `sage -sh -c patchbot` though I guess if `$SAGE_LOCAL/bin/patchbot` exists then `sage -patchbot` can be a shortcut for that.  I'm just not sure why you'd need or want to do that but I don't oppose it.
...
<a id='comment:154'></a>Replying to [embray](#comment%3A150):
> I don't know why we're talking about `python -m` anything.  When you install the `sage-patchbot` Python package (which Frédéric did a great job putting together at my urging) it installs a `patchbot` script.
...
```json
{
    "body": "<a id='comment:6'></a>Rebased to #22473.",
...
<a id='comment:6'></a>Rebased to #22473.
...
```json
{
    "body": "<a id='comment:8'></a>Let's start over...",
...
<a id='comment:8'></a>Let's start over...
...
<a id='comment:159'></a>Replying to [jdemeyer](#comment%3A154):
> Replying to [embray](#comment%3A150):
> > I don't know why we're talking about `python -m` anything.  When you install the `sage-patchbot` Python package (which Frédéric did a great job putting together at my urging) it installs a `patchbot` script.
...
<a id='comment:1'></a>I wonder what made you change your mind...
...
```json
{
    "body": "<a id='comment:2'></a>My feeling at this point is along the lines of \"I'm not going to use it, but if it suits your needs then fine.\" :)",
...
<a id='comment:2'></a>My feeling at this point is along the lines of "I'm not going to use it, but if it suits your needs then fine." :)
