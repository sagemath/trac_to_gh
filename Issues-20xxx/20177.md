# Issue 20177: Better checks in GRS decoders

archive/issues_019940.json:
```json
{
    "body": "Some checks on the output on `decode_to_code` for Gao, Berlekamp-Welch and Key-Equation Syndrome decoders are actually useless.\n\nI also noticed that the nature of the input code of GRS decoders is never checked: one can perform the following:\n\n```\nC = codes.RandomLinearCode(10, 4, GF(7))\nD = codes.decoders.GRSGaoDecoder(C)\n```\n\nand actually construct the decoder... Whose decoding methods will utterly fail with a terrible error message!\n\n\n\nBranch/Commit: 8f82e4eed97fe685862404856007c92a84746032\n\nReviewer: Johan Sebastian Rosenkilde Nielsen\n\nAuthor: David Lucas\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/20177\n\n",
    "closed_at": "2016-03-22T15:23:34Z",
    "created_at": "2016-03-08T12:13:15Z",
    "labels": [
        "component: coding theory"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-7.1",
    "title": "Better checks in GRS decoders",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/20177",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```
Some checks on the output on `decode_to_code` for Gao, Berlekamp-Welch and Key-Equation Syndrome decoders are actually useless.

I also noticed that the nature of the input code of GRS decoders is never checked: one can perform the following:

```
C = codes.RandomLinearCode(10, 4, GF(7))
D = codes.decoders.GRSGaoDecoder(C)
```

and actually construct the decoder... Whose decoding methods will utterly fail with a terrible error message!



Branch/Commit: 8f82e4eed97fe685862404856007c92a84746032

Reviewer: Johan Sebastian Rosenkilde Nielsen

Author: David Lucas

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/20177





---

archive/issue_comments_364971.json:
```json
{
    "body": "Description changed:\n``````diff\n\n``````\n",
    "created_at": "2016-03-08T12:13:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20177#issuecomment-364971",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

Description changed:
``````diff

``````




---

archive/issue_comments_364972.json:
```json
{
    "body": "Changing branch from \"\" to \"u/dlucas/checks_in_grs_decoders\"",
    "created_at": "2016-03-08T12:13:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20177#issuecomment-364972",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

Changing branch from "" to "u/dlucas/checks_in_grs_decoders"



---

archive/issue_comments_364973.json:
```json
{
    "body": "Changing commit from \"\" to \"d36fccfbef98ba0513bab1a5140c8667f8409d9c\"",
    "created_at": "2016-03-08T12:14:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20177#issuecomment-364973",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

Changing commit from "" to "d36fccfbef98ba0513bab1a5140c8667f8409d9c"



---

archive/issue_comments_364974.json:
```json
{
    "body": "<a id='comment:3'></a>\nI removed one useless check, fixed the bizarre ones, and added input sanitization on `__init__` in every GRS decoder.\n\nThis is now open for review.\n\n---\nNew commits:\n|                                                                                                                                          |                                          |\n|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------|\n|[2d73b3f](http://git.sagemath.org/sage.git/commit/?id=2d73b3f3e1f2bf182f1dc56db4efbee0edab264e)|`Added input sanitization on GRS decoders`|\n|[d36fccf](http://git.sagemath.org/sage.git/commit/?id=d36fccfbef98ba0513bab1a5140c8667f8409d9c)|`Changed checks on the output of decode_to_code methods, added input`|",
    "created_at": "2016-03-08T12:14:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20177#issuecomment-364974",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

<a id='comment:3'></a>
I removed one useless check, fixed the bizarre ones, and added input sanitization on `__init__` in every GRS decoder.

This is now open for review.

---
New commits:
|                                                                                                                                          |                                          |
|------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------|
|[2d73b3f](http://git.sagemath.org/sage.git/commit/?id=2d73b3f3e1f2bf182f1dc56db4efbee0edab264e)|`Added input sanitization on GRS decoders`|
|[d36fccf](http://git.sagemath.org/sage.git/commit/?id=d36fccfbef98ba0513bab1a5140c8667f8409d9c)|`Changed checks on the output of decode_to_code methods, added input`|



---

archive/issue_comments_364975.json:
```json
{
    "body": "Changing author from \"\" to \"David Lucas\"",
    "created_at": "2016-03-08T12:14:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20177#issuecomment-364975",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

Changing author from "" to "David Lucas"



---

archive/issue_comments_364976.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2016-03-08T12:14:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20177#issuecomment-364976",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_364977.json:
```json
{
    "body": "<a id='comment:4'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                          |                                            |\n|------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------|\n|[9f5f20d](http://git.sagemath.org/sage.git/commit/?id=9f5f20d505adc41b34c03135eaef05ed0eb2ce5c)|`Slight optimization in Gao and BW decoders`|",
    "created_at": "2016-03-08T13:16:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20177#issuecomment-364977",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:4'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                          |                                            |
|------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------|
|[9f5f20d](http://git.sagemath.org/sage.git/commit/?id=9f5f20d505adc41b34c03135eaef05ed0eb2ce5c)|`Slight optimization in Gao and BW decoders`|



---

archive/issue_comments_364978.json:
```json
{
    "body": "Changing commit from \"d36fccfbef98ba0513bab1a5140c8667f8409d9c\" to \"9f5f20d505adc41b34c03135eaef05ed0eb2ce5c\"",
    "created_at": "2016-03-08T13:16:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20177#issuecomment-364978",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "d36fccfbef98ba0513bab1a5140c8667f8409d9c" to "9f5f20d505adc41b34c03135eaef05ed0eb2ce5c"



---

archive/issue_comments_364979.json:
```json
{
    "body": "<a id='comment:5'></a>\nI regrouped the checks in a private method `_decode_to_code_and_message` which returns both the message and the corresponding codeword.\n\nThat way, if one does `D.decode_to_code(r)`, it wont compute the codeword twice.\n\nIt's still open for review.",
    "created_at": "2016-03-08T13:18:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20177#issuecomment-364979",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

<a id='comment:5'></a>
I regrouped the checks in a private method `_decode_to_code_and_message` which returns both the message and the corresponding codeword.

That way, if one does `D.decode_to_code(r)`, it wont compute the codeword twice.

It's still open for review.



---

archive/issue_comments_364980.json:
```json
{
    "body": "Changing branch from \"u/dlucas/checks_in_grs_decoders\" to \"u/jsrn/checks_in_grs_decoders\"",
    "created_at": "2016-03-10T12:15:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20177#issuecomment-364980",
    "user": "https://github.com/johanrosenkilde"
}
```

Changing branch from "u/dlucas/checks_in_grs_decoders" to "u/jsrn/checks_in_grs_decoders"



---

archive/issue_comments_364981.json:
```json
{
    "body": "<a id='comment:7'></a>\nIt looks good. I modified the tests that showed failure behaviour under too many errors: \"too many errors\" is of course not well-defined (since the receiver doesn't know the sent codeword); the relevant measure is the distance to *any* codeword. Your test generated random errors of weight `t+1`, where `t` is the decoding radius, and that would still decode into some other codeword sometimes (albeit with extremely low probability). I now changed it to a fixed example.\n\nIf you can accept this, please set it to positive review.\n\n---\nNew commits:\n|                                                                                                                                          |                                                   |\n|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------|\n|[8f82e4e](http://git.sagemath.org/sage.git/commit/?id=8f82e4eed97fe685862404856007c92a84746032)|`Clarification and non-randomization of some tests`|",
    "created_at": "2016-03-10T12:19:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20177#issuecomment-364981",
    "user": "https://github.com/johanrosenkilde"
}
```

<a id='comment:7'></a>
It looks good. I modified the tests that showed failure behaviour under too many errors: "too many errors" is of course not well-defined (since the receiver doesn't know the sent codeword); the relevant measure is the distance to *any* codeword. Your test generated random errors of weight `t+1`, where `t` is the decoding radius, and that would still decode into some other codeword sometimes (albeit with extremely low probability). I now changed it to a fixed example.

If you can accept this, please set it to positive review.

---
New commits:
|                                                                                                                                          |                                                   |
|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------|
|[8f82e4e](http://git.sagemath.org/sage.git/commit/?id=8f82e4eed97fe685862404856007c92a84746032)|`Clarification and non-randomization of some tests`|



---

archive/issue_comments_364982.json:
```json
{
    "body": "Changing commit from \"9f5f20d505adc41b34c03135eaef05ed0eb2ce5c\" to \"8f82e4eed97fe685862404856007c92a84746032\"",
    "created_at": "2016-03-10T12:19:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20177#issuecomment-364982",
    "user": "https://github.com/johanrosenkilde"
}
```

Changing commit from "9f5f20d505adc41b34c03135eaef05ed0eb2ce5c" to "8f82e4eed97fe685862404856007c92a84746032"



---

archive/issue_comments_364983.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Johan Sebastian Rosenkilde Nielsen\"",
    "created_at": "2016-03-10T12:19:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20177#issuecomment-364983",
    "user": "https://github.com/johanrosenkilde"
}
```

Changing reviewer from "" to "Johan Sebastian Rosenkilde Nielsen"



---

archive/issue_comments_364984.json:
```json
{
    "body": "<a id='comment:8'></a>\nI agree with your changes: giving positive review!\n\nDavid",
    "created_at": "2016-03-11T08:02:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20177#issuecomment-364984",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

<a id='comment:8'></a>
I agree with your changes: giving positive review!

David



---

archive/issue_comments_364985.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2016-03-11T08:02:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20177#issuecomment-364985",
    "user": "https://trac.sagemath.org/admin/accounts/users/dlucas"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_364986.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2016-03-22T15:23:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20177#issuecomment-364986",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_events_054832.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-03-22T15:23:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/20177",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20177#event-54832"
}
```



---

archive/issue_comments_364987.json:
```json
{
    "body": "Changing branch from \"u/jsrn/checks_in_grs_decoders\" to \"8f82e4eed97fe685862404856007c92a84746032\"",
    "created_at": "2016-03-22T15:23:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20177",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20177#issuecomment-364987",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/jsrn/checks_in_grs_decoders" to "8f82e4eed97fe685862404856007c92a84746032"
