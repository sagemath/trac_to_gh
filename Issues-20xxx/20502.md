# Issue 20502: fix is_zero for RR('nan')

archive/issues_020265.json:
```json
{
    "assignees": [],
    "body": "We have\n\n```\nsage: RR('nan').is_zero()\nTrue\n```\nSee [https://groups.google.com/forum/#!topic/sage-support/sNjVNSHO5b4 this sage-support thread](https://groups.google.com/forum/#!topic/sage-support/sNjVNSHO5b4 this sage-support thread).\n\nCC:  @williamstein @vbraun\n\nBranch/Commit: **[cd4fde4](https://github.com/sagemath/sagetrac-mirror/commit/cd4fde41d0ef7302d04f6c12c47bc4d2cab85eb8)**\n\nReviewer: **Volker Braun**\n\nAuthor: **Vincent Delecroix**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20502_\n\n",
    "closed_at": "2016-04-26T21:55:13Z",
    "created_at": "2016-04-26T12:04:55Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "fix is_zero for RR('nan')",
    "type": "issue",
    "updated_at": "2016-04-26T21:55:13Z",
    "url": "https://github.com/sagemath/sage/issues/20502",
    "user": "https://github.com/videlec"
}
```
We have

```
sage: RR('nan').is_zero()
True
```
See [https://groups.google.com/forum/#!topic/sage-support/sNjVNSHO5b4 this sage-support thread](https://groups.google.com/forum/#!topic/sage-support/sNjVNSHO5b4 this sage-support thread).

CC:  @williamstein @vbraun

Branch/Commit: **[cd4fde4](https://github.com/sagemath/sagetrac-mirror/commit/cd4fde41d0ef7302d04f6c12c47bc4d2cab85eb8)**

Reviewer: **Volker Braun**

Author: **Vincent Delecroix**

_Issue created by migration from https://trac.sagemath.org/ticket/20502_





---

archive/issue_events_271066.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2016-04-26T12:04:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20502",
    "milestone_number": null,
    "milestone_title": "sage-7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20502#event-271066"
}
```



---

archive/issue_events_271067.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2016-04-26T12:04:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20502",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20basic%20arithmetic",
    "label_color": "0000ff",
    "label_name": "component: basic arithmetic",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20502#event-271067"
}
```



---

archive/issue_events_271068.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2016-04-26T12:04:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20502",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20502#event-271068"
}
```



---

archive/issue_events_271069.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2016-04-26T12:04:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20502",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20502#event-271069"
}
```



---

archive/issue_comments_298571.json:
```json
{
    "body": "Branch: **[u/vdelecroix/20502](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/20502)**",
    "created_at": "2016-04-26T12:07:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20502",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20502#issuecomment-298571",
    "user": "https://github.com/videlec"
}
```

Branch: **[u/vdelecroix/20502](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/20502)**



---

archive/issue_events_271070.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2016-04-26T12:07:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20502",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20502#event-271070"
}
```



---

archive/issue_comments_298572.json:
```json
{
    "body": "Commit: **[cd4fde4](https://github.com/sagemath/sagetrac-mirror/commit/cd4fde41d0ef7302d04f6c12c47bc4d2cab85eb8)**",
    "created_at": "2016-04-26T12:07:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20502",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20502#issuecomment-298572",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[cd4fde4](https://github.com/sagemath/sagetrac-mirror/commit/cd4fde41d0ef7302d04f6c12c47bc4d2cab85eb8)**



---

archive/issue_comments_298573.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/cd4fde41d0ef7302d04f6c12c47bc4d2cab85eb8\">cd4fde4</a></td><td><code>Trac 20502: fix is_zero for RR('nan')</code></td></tr></table>\n",
    "created_at": "2016-04-26T12:07:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20502",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20502#issuecomment-298573",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/cd4fde41d0ef7302d04f6c12c47bc4d2cab85eb8">cd4fde4</a></td><td><code>Trac 20502: fix is_zero for RR('nan')</code></td></tr></table>




---

archive/issue_events_271071.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-04-26T12:55:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20502",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20502#event-271071"
}
```



---

archive/issue_events_271072.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-04-26T12:55:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20502",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20502#event-271072"
}
```



---

archive/issue_comments_298574.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2016-04-26T12:55:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20502",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20502#issuecomment-298574",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_events_271073.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-04-26T21:55:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20502",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20502#event-271073"
}
```



---

archive/issue_events_271074.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "13d6629e92fb70f6e7d803b68a69911803cd7ec4",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-04-26T21:55:13Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20502",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20502#event-271074"
}
```



---

archive/issue_comments_298575.json:
```json
{
    "body": "Changed branch from **[u/vdelecroix/20502](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/20502)** to **[cd4fde4](https://github.com/sagemath/sagetrac-mirror/commit/cd4fde41d0ef7302d04f6c12c47bc4d2cab85eb8)**",
    "created_at": "2016-04-26T21:55:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20502",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20502#issuecomment-298575",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/vdelecroix/20502](https://github.com/sagemath/sagetrac-mirror/tree/u/vdelecroix/20502)** to **[cd4fde4](https://github.com/sagemath/sagetrac-mirror/commit/cd4fde41d0ef7302d04f6c12c47bc4d2cab85eb8)**
