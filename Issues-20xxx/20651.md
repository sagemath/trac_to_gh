# Issue 20651: IPython depends on shutil_get_terminal_size

archive/issues_020414.json:
```json
{
    "assignees": [],
    "body": "Building the documentation imports `IPython`. This can fail if `backports_shutil_get_terminal_size` was not installed.\n\nBranch/Commit: **[2388da1](https://github.com/sagemath/sagetrac-mirror/commit/2388da16d459184f831cc0c1c59f9efd814fca36)**\n\nReviewer: **Volker Braun**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20651_\n\n",
    "closed_at": "2016-05-23T22:23:44Z",
    "created_at": "2016-05-22T09:40:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "IPython depends on shutil_get_terminal_size",
    "type": "issue",
    "updated_at": "2016-05-23T22:23:44Z",
    "url": "https://github.com/sagemath/sage/issues/20651",
    "user": "https://github.com/jdemeyer"
}
```
Building the documentation imports `IPython`. This can fail if `backports_shutil_get_terminal_size` was not installed.

Branch/Commit: **[2388da1](https://github.com/sagemath/sagetrac-mirror/commit/2388da16d459184f831cc0c1c59f9efd814fca36)**

Reviewer: **Volker Braun**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/20651_





---

archive/issue_events_272899.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-05-22T09:40:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20651",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20651#event-272899"
}
```



---

archive/issue_events_272900.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-05-22T09:40:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20651",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20651#event-272900"
}
```



---

archive/issue_events_272901.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-05-22T09:40:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20651",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20651#event-272901"
}
```



---

archive/issue_events_272902.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-05-22T09:40:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20651",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20651#event-272902"
}
```



---

archive/issue_comments_301210.json:
```json
{
    "body": "Branch: **[u/jdemeyer/ipython_depends_on_shutil_get_terminal_size](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ipython_depends_on_shutil_get_terminal_size)**",
    "created_at": "2016-05-22T10:46:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20651#issuecomment-301210",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/ipython_depends_on_shutil_get_terminal_size](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ipython_depends_on_shutil_get_terminal_size)**



---

archive/issue_comments_301211.json:
```json
{
    "body": "Commit: **[2388da1](https://github.com/sagemath/sagetrac-mirror/commit/2388da16d459184f831cc0c1c59f9efd814fca36)**",
    "created_at": "2016-05-22T10:47:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20651#issuecomment-301211",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[2388da1](https://github.com/sagemath/sagetrac-mirror/commit/2388da16d459184f831cc0c1c59f9efd814fca36)**



---

archive/issue_events_272903.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-05-22T10:47:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20651",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20651#event-272903"
}
```



---

archive/issue_comments_301212.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2388da16d459184f831cc0c1c59f9efd814fca36\">2388da1</a></td><td><code>IPython depends on shutil_get_terminal_size</code></td></tr></table>\n",
    "created_at": "2016-05-22T10:47:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20651#issuecomment-301212",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2388da16d459184f831cc0c1c59f9efd814fca36">2388da1</a></td><td><code>IPython depends on shutil_get_terminal_size</code></td></tr></table>




---

archive/issue_comments_301213.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nThere is no backports_shutil_get_terminal_size package, is there?",
    "created_at": "2016-05-22T11:13:42Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20651#issuecomment-301213",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:3" align="right">Comment 3</div>

There is no backports_shutil_get_terminal_size package, is there?



---

archive/issue_comments_301214.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2016-05-22T13:53:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20651#issuecomment-301214",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_comments_301215.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nok got it",
    "created_at": "2016-05-22T13:53:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20651#issuecomment-301215",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:4" align="right">Comment 4</div>

ok got it



---

archive/issue_events_272904.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-22T13:53:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20651",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20651#event-272904"
}
```



---

archive/issue_events_272905.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-22T13:53:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20651",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20651#event-272905"
}
```



---

archive/issue_comments_301216.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-\n+Building the documentation imports `IPython`. This can fail if `backports_shutil_get_terminal_size` was not installed.\n``````\n",
    "created_at": "2016-05-23T08:25:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20651#issuecomment-301216",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-
+Building the documentation imports `IPython`. This can fail if `backports_shutil_get_terminal_size` was not installed.
``````




---

archive/issue_comments_301217.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nI see, the pipe separates normal dependencies from order-only dependencies, as explained in\nhttp://doc.sagemath.org/html/en/developer/packaging.html#package-dependencies\n\nThanks for the fix!",
    "created_at": "2016-05-23T08:39:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20651#issuecomment-301217",
    "user": "https://github.com/slel"
}
```

<div id="comment:6" align="right">Comment 6</div>

I see, the pipe separates normal dependencies from order-only dependencies, as explained in
http://doc.sagemath.org/html/en/developer/packaging.html#package-dependencies

Thanks for the fix!



---

archive/issue_comments_301218.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/ipython_depends_on_shutil_get_terminal_size](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ipython_depends_on_shutil_get_terminal_size)** to **[2388da1](https://github.com/sagemath/sagetrac-mirror/commit/2388da16d459184f831cc0c1c59f9efd814fca36)**",
    "created_at": "2016-05-23T22:23:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20651",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20651#issuecomment-301218",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/ipython_depends_on_shutil_get_terminal_size](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ipython_depends_on_shutil_get_terminal_size)** to **[2388da1](https://github.com/sagemath/sagetrac-mirror/commit/2388da16d459184f831cc0c1c59f9efd814fca36)**



---

archive/issue_events_272906.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-23T22:23:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20651",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20651#event-272906"
}
```



---

archive/issue_events_272907.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "18768c2ebded30a3a561c2e99d485a43b188b767",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-05-23T22:23:44Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20651",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20651#event-272907"
}
```
