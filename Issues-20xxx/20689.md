# Issue 20689: Add certificate option to has_minor

archive/issues_020452.json:
```json
{
    "assignees": [],
    "body": "Add an option to the \"has_minor\" method of matroids to return a witness in case a minor is found.\n\nCC:  @sagetrac-Stefan @sagetrac-yomcat\n\nBranch/Commit: **[e8c5b4e](https://github.com/sagemath/sagetrac-mirror/commit/e8c5b4e1fdb1c4708f300d361daee941ef490c33)**\n\nReviewer: **Michael Welsh, Stefan van Zwam**\n\nAuthor: **Tara Fife**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20689_\n\n",
    "closed_at": "2016-08-19T22:44:44Z",
    "created_at": "2016-05-27T01:11:49Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20matroid%20theory",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add certificate option to has_minor",
    "type": "issue",
    "updated_at": "2016-08-19T22:44:44Z",
    "url": "https://github.com/sagemath/sage/issues/20689",
    "user": "https://github.com/TaraFife"
}
```
Add an option to the "has_minor" method of matroids to return a witness in case a minor is found.

CC:  @sagetrac-Stefan @sagetrac-yomcat

Branch/Commit: **[e8c5b4e](https://github.com/sagemath/sagetrac-mirror/commit/e8c5b4e1fdb1c4708f300d361daee941ef490c33)**

Reviewer: **Michael Welsh, Stefan van Zwam**

Author: **Tara Fife**

_Issue created by migration from https://trac.sagemath.org/ticket/20689_





---

archive/issue_events_273379.json:
```json
{
    "actor": "https://github.com/TaraFife",
    "created_at": "2016-05-27T01:11:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20689#event-273379"
}
```



---

archive/issue_events_273380.json:
```json
{
    "actor": "https://github.com/TaraFife",
    "created_at": "2016-05-27T01:11:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20matroid%20theory",
    "label_color": "0000ff",
    "label_name": "component: matroid theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20689#event-273380"
}
```



---

archive/issue_events_273381.json:
```json
{
    "actor": "https://github.com/TaraFife",
    "created_at": "2016-05-27T01:11:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20689#event-273381"
}
```



---

archive/issue_events_273382.json:
```json
{
    "actor": "https://github.com/TaraFife",
    "created_at": "2016-05-27T01:11:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20689#event-273382"
}
```



---

archive/issue_comments_301837.json:
```json
{
    "body": "Branch: **[public/ticket/20689](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/20689)**",
    "created_at": "2016-05-27T02:12:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301837",
    "user": "https://github.com/TaraFife"
}
```

Branch: **[public/ticket/20689](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/20689)**



---

archive/issue_comments_301838.json:
```json
{
    "body": "Commit: **[ca19af1](https://github.com/sagemath/sagetrac-mirror/commit/ca19af1eabcaa84698633fb8e2f2dcbf23cd32a7)**",
    "created_at": "2016-05-27T02:22:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301838",
    "user": "https://github.com/TaraFife"
}
```

Commit: **[ca19af1](https://github.com/sagemath/sagetrac-mirror/commit/ca19af1eabcaa84698633fb8e2f2dcbf23cd32a7)**



---

archive/issue_comments_301839.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ca19af1eabcaa84698633fb8e2f2dcbf23cd32a7\">ca19af1</a></td><td><code>Added the option to get the sets `X` and `Y`, where `N` is `M/X\\Y`.</code></td></tr></table>\n",
    "created_at": "2016-05-27T02:22:16Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301839",
    "user": "https://github.com/TaraFife"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ca19af1eabcaa84698633fb8e2f2dcbf23cd32a7">ca19af1</a></td><td><code>Added the option to get the sets `X` and `Y`, where `N` is `M/X\Y`.</code></td></tr></table>




---

archive/issue_events_273383.json:
```json
{
    "actor": "https://github.com/TaraFife",
    "created_at": "2016-05-27T02:22:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20689#event-273383"
}
```



---

archive/issue_comments_301840.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\n* Line 88 of `matroid.pyx` has been messed up\n* At line 1110, you're missing an enter.\n* Line 3893 has bonus whitespace.\n* As on the `is_isomorphic()` ticket, you should explicitly call the option in the doctests.",
    "created_at": "2016-05-27T21:31:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301840",
    "user": "https://github.com/sagetrac-yomcat"
}
```

<div id="comment:3" align="right">Comment 3</div>

* Line 88 of `matroid.pyx` has been messed up
* At line 1110, you're missing an enter.
* Line 3893 has bonus whitespace.
* As on the `is_isomorphic()` ticket, you should explicitly call the option in the doctests.



---

archive/issue_events_273384.json:
```json
{
    "actor": "https://github.com/sagetrac-yomcat",
    "created_at": "2016-05-27T21:31:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20689#event-273384"
}
```



---

archive/issue_events_273385.json:
```json
{
    "actor": "https://github.com/sagetrac-yomcat",
    "created_at": "2016-05-27T21:31:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20689#event-273385"
}
```



---

archive/issue_comments_301841.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/85a6317e97c5f889736f1677d1afb15901d23810\">85a6317</a></td><td><code>Fixed errors</code></td></tr></table>\n",
    "created_at": "2016-05-28T20:02:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301841",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/85a6317e97c5f889736f1677d1afb15901d23810">85a6317</a></td><td><code>Fixed errors</code></td></tr></table>




---

archive/issue_comments_301842.json:
```json
{
    "body": "Changed commit from **[ca19af1](https://github.com/sagemath/sagetrac-mirror/commit/ca19af1eabcaa84698633fb8e2f2dcbf23cd32a7)** to **[85a6317](https://github.com/sagemath/sagetrac-mirror/commit/85a6317e97c5f889736f1677d1afb15901d23810)**",
    "created_at": "2016-05-28T20:02:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301842",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[ca19af1](https://github.com/sagemath/sagetrac-mirror/commit/ca19af1eabcaa84698633fb8e2f2dcbf23cd32a7)** to **[85a6317](https://github.com/sagemath/sagetrac-mirror/commit/85a6317e97c5f889736f1677d1afb15901d23810)**



---

archive/issue_comments_301843.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nAlmost done (I feel like a grumpy old man...)\n\nYou've got repeated doctests now.",
    "created_at": "2016-05-28T20:47:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301843",
    "user": "https://github.com/sagetrac-yomcat"
}
```

<div id="comment:5" align="right">Comment 5</div>

Almost done (I feel like a grumpy old man...)

You've got repeated doctests now.



---

archive/issue_comments_301844.json:
```json
{
    "body": "Changed author from **Tara** to **Tara Fife**",
    "created_at": "2016-05-28T20:47:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301844",
    "user": "https://github.com/sagetrac-yomcat"
}
```

Changed author from **Tara** to **Tara Fife**



---

archive/issue_comments_301845.json:
```json
{
    "body": "Reviewer: **Michael Welsh**",
    "created_at": "2016-05-28T20:47:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301845",
    "user": "https://github.com/sagetrac-yomcat"
}
```

Reviewer: **Michael Welsh**



---

archive/issue_comments_301846.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6fc388d3bc08caa5625272453d95723ca0e8055d\">6fc388d</a></td><td><code>Removed repeated doctest</code></td></tr></table>\n",
    "created_at": "2016-05-29T01:29:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301846",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6fc388d3bc08caa5625272453d95723ca0e8055d">6fc388d</a></td><td><code>Removed repeated doctest</code></td></tr></table>




---

archive/issue_comments_301847.json:
```json
{
    "body": "Changed commit from **[85a6317](https://github.com/sagemath/sagetrac-mirror/commit/85a6317e97c5f889736f1677d1afb15901d23810)** to **[6fc388d](https://github.com/sagemath/sagetrac-mirror/commit/6fc388d3bc08caa5625272453d95723ca0e8055d)**",
    "created_at": "2016-05-29T01:29:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301847",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[85a6317](https://github.com/sagemath/sagetrac-mirror/commit/85a6317e97c5f889736f1677d1afb15901d23810)** to **[6fc388d](https://github.com/sagemath/sagetrac-mirror/commit/6fc388d3bc08caa5625272453d95723ca0e8055d)**



---

archive/issue_comments_301848.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nShould this ticket be in \"needs review\"?",
    "created_at": "2016-05-30T16:11:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301848",
    "user": "https://github.com/sagetrac-Stefan"
}
```

<div id="comment:7" align="right">Comment 7</div>

Should this ticket be in "needs review"?



---

archive/issue_comments_301849.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nDocumentation needs to be in standard form. So\n\n```\n    - ``certificate`` -- (default: ``False``) If true, blablabla\n```\nThe double backquotes are for code variables, the single backquotes are for math. Also note the way defaults are reported.\n\nDocumentation can be previewed by running\n\n```\nmake doc\n```\nand then inspecting (starting from your base Sage directory)\n\n```\nlocal/share/doc/sage/html/en/reference/matroids/index.html\n```",
    "created_at": "2016-05-30T20:13:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301849",
    "user": "https://github.com/sagetrac-Stefan"
}
```

<div id="comment:8" align="right">Comment 8</div>

Documentation needs to be in standard form. So

```
    - ``certificate`` -- (default: ``False``) If true, blablabla
```
The double backquotes are for code variables, the single backquotes are for math. Also note the way defaults are reported.

Documentation can be previewed by running

```
make doc
```
and then inspecting (starting from your base Sage directory)

```
local/share/doc/sage/html/en/reference/matroids/index.html
```



---

archive/issue_comments_301850.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1a33e6340048cb10168d5e061be89e9bd558ac24\">1a33e63</a></td><td><code>Eddited documentation</code></td></tr></table>\n",
    "created_at": "2016-05-31T16:48:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301850",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:9" align="right">Comment 9</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1a33e6340048cb10168d5e061be89e9bd558ac24">1a33e63</a></td><td><code>Eddited documentation</code></td></tr></table>




---

archive/issue_comments_301851.json:
```json
{
    "body": "Changed commit from **[6fc388d](https://github.com/sagemath/sagetrac-mirror/commit/6fc388d3bc08caa5625272453d95723ca0e8055d)** to **[1a33e63](https://github.com/sagemath/sagetrac-mirror/commit/1a33e6340048cb10168d5e061be89e9bd558ac24)**",
    "created_at": "2016-05-31T16:48:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301851",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[6fc388d](https://github.com/sagemath/sagetrac-mirror/commit/6fc388d3bc08caa5625272453d95723ca0e8055d)** to **[1a33e63](https://github.com/sagemath/sagetrac-mirror/commit/1a33e6340048cb10168d5e061be89e9bd558ac24)**



---

archive/issue_comments_301852.json:
```json
{
    "body": "<div id=\"comment:10\" align=\"right\">Comment 10</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/19826d187f2173348a4f5d3ec2fe9b00f647f8f8\">19826d1</a></td><td><code>Eddited errors in documentation</code></td></tr></table>\n",
    "created_at": "2016-05-31T16:57:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301852",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:10" align="right">Comment 10</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/19826d187f2173348a4f5d3ec2fe9b00f647f8f8">19826d1</a></td><td><code>Eddited errors in documentation</code></td></tr></table>




---

archive/issue_comments_301853.json:
```json
{
    "body": "Changed commit from **[1a33e63](https://github.com/sagemath/sagetrac-mirror/commit/1a33e6340048cb10168d5e061be89e9bd558ac24)** to **[19826d1](https://github.com/sagemath/sagetrac-mirror/commit/19826d187f2173348a4f5d3ec2fe9b00f647f8f8)**",
    "created_at": "2016-05-31T16:57:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301853",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[1a33e63](https://github.com/sagemath/sagetrac-mirror/commit/1a33e6340048cb10168d5e061be89e9bd558ac24)** to **[19826d1](https://github.com/sagemath/sagetrac-mirror/commit/19826d187f2173348a4f5d3ec2fe9b00f647f8f8)**



---

archive/issue_comments_301854.json:
```json
{
    "body": "Changed commit from **[19826d1](https://github.com/sagemath/sagetrac-mirror/commit/19826d187f2173348a4f5d3ec2fe9b00f647f8f8)** to **[d05d6d2](https://github.com/sagemath/sagetrac-mirror/commit/d05d6d24cfde51e83a973e2044394747f260e49c)**",
    "created_at": "2016-06-05T12:48:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301854",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[19826d1](https://github.com/sagemath/sagetrac-mirror/commit/19826d187f2173348a4f5d3ec2fe9b00f647f8f8)** to **[d05d6d2](https://github.com/sagemath/sagetrac-mirror/commit/d05d6d24cfde51e83a973e2044394747f260e49c)**



---

archive/issue_comments_301855.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/0b86a6bec61f3121d872aee6e7decbf1dae42d8d\">0b86a6b</a></td><td><code>Eddited documentation</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d05d6d24cfde51e83a973e2044394747f260e49c\">d05d6d2</a></td><td><code>Fixed error in documentation</code></td></tr></table>\n",
    "created_at": "2016-06-05T12:48:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301855",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:11" align="right">Comment 11</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/0b86a6bec61f3121d872aee6e7decbf1dae42d8d">0b86a6b</a></td><td><code>Eddited documentation</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d05d6d24cfde51e83a973e2044394747f260e49c">d05d6d2</a></td><td><code>Fixed error in documentation</code></td></tr></table>




---

archive/issue_comments_301856.json:
```json
{
    "body": "Changed commit from **[d05d6d2](https://github.com/sagemath/sagetrac-mirror/commit/d05d6d24cfde51e83a973e2044394747f260e49c)** to **[15131de](https://github.com/sagemath/sagetrac-mirror/commit/15131de5207f9d2b9eabc24e6b11e97506cecfc9)**",
    "created_at": "2016-06-05T12:59:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301856",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[d05d6d2](https://github.com/sagemath/sagetrac-mirror/commit/d05d6d24cfde51e83a973e2044394747f260e49c)** to **[15131de](https://github.com/sagemath/sagetrac-mirror/commit/15131de5207f9d2b9eabc24e6b11e97506cecfc9)**



---

archive/issue_comments_301857.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/15131de5207f9d2b9eabc24e6b11e97506cecfc9\">15131de</a></td><td><code>Fixed spacing error</code></td></tr></table>\n",
    "created_at": "2016-06-05T12:59:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301857",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:12" align="right">Comment 12</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/15131de5207f9d2b9eabc24e6b11e97506cecfc9">15131de</a></td><td><code>Fixed spacing error</code></td></tr></table>




---

archive/issue_comments_301858.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/87533057d70e226d533fce0e853cc8a700c74a40\">8753305</a></td><td><code>Fixed doctest</code></td></tr></table>\n",
    "created_at": "2016-06-07T22:02:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301858",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:13" align="right">Comment 13</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/87533057d70e226d533fce0e853cc8a700c74a40">8753305</a></td><td><code>Fixed doctest</code></td></tr></table>




---

archive/issue_comments_301859.json:
```json
{
    "body": "Changed commit from **[15131de](https://github.com/sagemath/sagetrac-mirror/commit/15131de5207f9d2b9eabc24e6b11e97506cecfc9)** to **[8753305](https://github.com/sagemath/sagetrac-mirror/commit/87533057d70e226d533fce0e853cc8a700c74a40)**",
    "created_at": "2016-06-07T22:02:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301859",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[15131de](https://github.com/sagemath/sagetrac-mirror/commit/15131de5207f9d2b9eabc24e6b11e97506cecfc9)** to **[8753305](https://github.com/sagemath/sagetrac-mirror/commit/87533057d70e226d533fce0e853cc8a700c74a40)**



---

archive/issue_events_273386.json:
```json
{
    "actor": "https://github.com/TaraFife",
    "created_at": "2016-06-07T22:04:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20689#event-273386"
}
```



---

archive/issue_events_273387.json:
```json
{
    "actor": "https://github.com/TaraFife",
    "created_at": "2016-06-07T22:04:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20689#event-273387"
}
```



---

archive/issue_comments_301860.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">Comment 15</div>\n\nEach ticket needs a proper description (at least a line saying what is the goal). For example, there is no way to guess that all this is about matroids!!",
    "created_at": "2016-06-14T17:54:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301860",
    "user": "https://github.com/videlec"
}
```

<div id="comment:15" align="right">Comment 15</div>

Each ticket needs a proper description (at least a line saying what is the goal). For example, there is no way to guess that all this is about matroids!!



---

archive/issue_events_273388.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2016-06-14T17:54:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20689#event-273388"
}
```



---

archive/issue_events_273389.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2016-06-14T17:54:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20689#event-273389"
}
```



---

archive/issue_events_273390.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2016-06-20T15:52:46Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20689#event-273390"
}
```



---

archive/issue_events_273391.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2016-06-20T15:52:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20689#event-273391"
}
```



---

archive/issue_comments_301861.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-\n+Add an option to the \"has_minor\" method of matroids to return a witness in case a minor is found.\n``````\n",
    "created_at": "2016-06-20T15:52:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301861",
    "user": "https://github.com/sagetrac-Stefan"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-
+Add an option to the "has_minor" method of matroids to return a witness in case a minor is found.
``````




---

archive/issue_comments_301862.json:
```json
{
    "body": "<div id=\"comment:17\" align=\"right\">Comment 17</div>\n\nIn the clause 'if N is self' you should still return the tuple (X,Y,dic), where X = Y = frozenset() and `dic = {x: x for x in self.groundset()} `",
    "created_at": "2016-06-20T17:15:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301862",
    "user": "https://github.com/sagetrac-Stefan"
}
```

<div id="comment:17" align="right">Comment 17</div>

In the clause 'if N is self' you should still return the tuple (X,Y,dic), where X = Y = frozenset() and `dic = {x: x for x in self.groundset()} `



---

archive/issue_events_273392.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2016-06-20T17:15:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20689#event-273392"
}
```



---

archive/issue_events_273393.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2016-06-20T17:15:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20689#event-273393"
}
```



---

archive/issue_comments_301863.json:
```json
{
    "body": "<div id=\"comment:19\" align=\"right\">Comment 19</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e8c5b4e1fdb1c4708f300d361daee941ef490c33\">e8c5b4e</a></td><td><code>Fixed return when we test if a matroid has itself as a minor.</code></td></tr></table>\n",
    "created_at": "2016-07-14T22:36:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301863",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:19" align="right">Comment 19</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e8c5b4e1fdb1c4708f300d361daee941ef490c33">e8c5b4e</a></td><td><code>Fixed return when we test if a matroid has itself as a minor.</code></td></tr></table>




---

archive/issue_comments_301864.json:
```json
{
    "body": "Changed commit from **[8753305](https://github.com/sagemath/sagetrac-mirror/commit/87533057d70e226d533fce0e853cc8a700c74a40)** to **[e8c5b4e](https://github.com/sagemath/sagetrac-mirror/commit/e8c5b4e1fdb1c4708f300d361daee941ef490c33)**",
    "created_at": "2016-07-14T22:36:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301864",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[8753305](https://github.com/sagemath/sagetrac-mirror/commit/87533057d70e226d533fce0e853cc8a700c74a40)** to **[e8c5b4e](https://github.com/sagemath/sagetrac-mirror/commit/e8c5b4e1fdb1c4708f300d361daee941ef490c33)**



---

archive/issue_events_273394.json:
```json
{
    "actor": "https://github.com/TaraFife",
    "created_at": "2016-07-14T22:38:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20689#event-273394"
}
```



---

archive/issue_events_273395.json:
```json
{
    "actor": "https://github.com/TaraFife",
    "created_at": "2016-07-14T22:38:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20689#event-273395"
}
```



---

archive/issue_comments_301865.json:
```json
{
    "body": "<div id=\"comment:21\" align=\"right\">Comment 21</div>\n\nI'm happy now",
    "created_at": "2016-08-17T18:46:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301865",
    "user": "https://github.com/sagetrac-Stefan"
}
```

<div id="comment:21" align="right">Comment 21</div>

I'm happy now



---

archive/issue_events_273396.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2016-08-17T18:46:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20689#event-273396"
}
```



---

archive/issue_events_273397.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2016-08-17T18:46:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20689#event-273397"
}
```



---

archive/issue_comments_301866.json:
```json
{
    "body": "Changed reviewer from **Michael Welsh** to **Michael Welsh, Stefan van Zwam**",
    "created_at": "2016-08-17T18:46:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301866",
    "user": "https://github.com/sagetrac-Stefan"
}
```

Changed reviewer from **Michael Welsh** to **Michael Welsh, Stefan van Zwam**



---

archive/issue_comments_301867.json:
```json
{
    "body": "Changed branch from **[public/ticket/20689](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/20689)** to **[e8c5b4e](https://github.com/sagemath/sagetrac-mirror/commit/e8c5b4e1fdb1c4708f300d361daee941ef490c33)**",
    "created_at": "2016-08-19T22:44:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20689#issuecomment-301867",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/ticket/20689](https://github.com/sagemath/sagetrac-mirror/tree/public/ticket/20689)** to **[e8c5b4e](https://github.com/sagemath/sagetrac-mirror/commit/e8c5b4e1fdb1c4708f300d361daee941ef490c33)**



---

archive/issue_events_273398.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-08-19T22:44:44Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20689#event-273398"
}
```



---

archive/issue_events_273399.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "27ef4a5ae67291105e254a15609e74d66edbf407",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-08-19T22:44:44Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20689",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20689#event-273399"
}
```
