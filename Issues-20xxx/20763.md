# Issue 20763: Euler numbers via flint

archive/issues_020526.json:
```json
{
    "assignees": [],
    "body": "euler numbers currently use the slow maxima pexpect interface\n\nthey can be much faster using flint\n\nfor a possible follow-up, see #17770\n\nCC:  @tscrim @jm58660\n\nBranch/Commit: **[510a91d](https://github.com/sagemath/sagetrac-mirror/commit/510a91dcb68a46563e8776f4c2a1b2d99c93bcad)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20763_\n\n",
    "closed_at": "2016-06-05T01:10:34Z",
    "created_at": "2016-06-02T18:44:37Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Euler numbers via flint",
    "type": "issue",
    "updated_at": "2016-06-05T01:10:34Z",
    "url": "https://github.com/sagemath/sage/issues/20763",
    "user": "https://github.com/fchapoton"
}
```
euler numbers currently use the slow maxima pexpect interface

they can be much faster using flint

for a possible follow-up, see #17770

CC:  @tscrim @jm58660

Branch/Commit: **[510a91d](https://github.com/sagemath/sagetrac-mirror/commit/510a91dcb68a46563e8776f4c2a1b2d99c93bcad)**

Reviewer: **Travis Scrimshaw**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/20763_





---

archive/issue_events_264543.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-06-02T18:44:37Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20763",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20763#event-264543"
}
```



---

archive/issue_events_264544.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-06-02T18:44:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20763",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20763#event-264544"
}
```



---

archive/issue_events_264545.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-06-02T18:44:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20763",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20763#event-264545"
}
```



---

archive/issue_events_264546.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-06-02T18:44:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20763",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20763#event-264546"
}
```



---

archive/issue_comments_303658.json:
```json
{
    "body": "Commit: **[510a91d](https://github.com/sagemath/sagetrac-mirror/commit/510a91dcb68a46563e8776f4c2a1b2d99c93bcad)**",
    "created_at": "2016-06-02T18:45:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20763#issuecomment-303658",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[510a91d](https://github.com/sagemath/sagetrac-mirror/commit/510a91dcb68a46563e8776f4c2a1b2d99c93bcad)**



---

archive/issue_events_264547.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-06-02T18:45:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20763",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20763#event-264547"
}
```



---

archive/issue_comments_303659.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/510a91dcb68a46563e8776f4c2a1b2d99c93bcad\">510a91d</a></td><td><code>use flint for euler numbers</code></td></tr></table>\n",
    "created_at": "2016-06-02T18:45:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20763#issuecomment-303659",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/510a91dcb68a46563e8776f4c2a1b2d99c93bcad">510a91d</a></td><td><code>use flint for euler numbers</code></td></tr></table>




---

archive/issue_comments_303660.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,5 @@\n euler numbers currently use the slow maxima pexpect interface\n \n they can be much faster using flint\n+\n+for a possible follow-up, see #17770\n``````\n",
    "created_at": "2016-06-02T18:45:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20763#issuecomment-303660",
    "user": "https://github.com/fchapoton"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,5 @@
 euler numbers currently use the slow maxima pexpect interface
 
 they can be much faster using flint
+
+for a possible follow-up, see #17770
``````




---

archive/issue_comments_303661.json:
```json
{
    "body": "Branch: **[u/chapoton/20763](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/20763)**",
    "created_at": "2016-06-02T18:45:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20763#issuecomment-303661",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[u/chapoton/20763](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/20763)**



---

archive/issue_comments_303662.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nCan you give some reasoning why it is 1000 for the cut-off for `sig_on`/`sig_off`?",
    "created_at": "2016-06-02T18:55:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20763#issuecomment-303662",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:2'>Comment 2:</a>
Can you give some reasoning why it is 1000 for the cut-off for `sig_on`/`sig_off`?



---

archive/issue_comments_303663.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nI just copied that from the code of another function in flint/arith.",
    "created_at": "2016-06-02T18:56:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20763#issuecomment-303663",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:3'>Comment 3:</a>
I just copied that from the code of another function in flint/arith.



---

archive/issue_comments_303664.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nGuess they consider that a good cutoff bound for speed for small values. Nor can I fault it:\n\n```\nsage: %time euler_number(10000)\nCPU times: user 46.3 ms, sys: 0 ns, total: 46.3 ms\nWall time: 46.2 ms\n# Too long to put here\n```",
    "created_at": "2016-06-02T22:49:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20763#issuecomment-303664",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:4'>Comment 4:</a>
Guess they consider that a good cutoff bound for speed for small values. Nor can I fault it:

```
sage: %time euler_number(10000)
CPU times: user 46.3 ms, sys: 0 ns, total: 46.3 ms
Wall time: 46.2 ms
# Too long to put here
```



---

archive/issue_comments_303665.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2016-06-02T22:49:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20763#issuecomment-303665",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_264548.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-06-02T22:49:41Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20763",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20763#event-264548"
}
```



---

archive/issue_events_264549.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-06-02T22:49:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20763",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20763#event-264549"
}
```



---

archive/issue_events_264550.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-06-05T01:10:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20763",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20763#event-264550"
}
```



---

archive/issue_events_264551.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "65e82cf3259dad22021155e1beb33e3295d3d668",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-06-05T01:10:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20763",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20763#event-264551"
}
```



---

archive/issue_comments_303666.json:
```json
{
    "body": "Changed branch from **[u/chapoton/20763](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/20763)** to **[510a91d](https://github.com/sagemath/sagetrac-mirror/commit/510a91dcb68a46563e8776f4c2a1b2d99c93bcad)**",
    "created_at": "2016-06-05T01:10:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20763",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20763#issuecomment-303666",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/20763](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/20763)** to **[510a91d](https://github.com/sagemath/sagetrac-mirror/commit/510a91dcb68a46563e8776f4c2a1b2d99c93bcad)**
