# Issue 20214: Type inconsistencies in polynomial factorization

archive/issues_019977.json:
```json
{
    "body": "CC:  @slel\n\nKeywords: factor, polynomial, Laurent polynomial\n\nThis ticket fixes a lot of inconsistencies with factorization (`structure/factorization.py`). For example the unit was not forced to belong to the universe of the factorization\n\n```\nsage: R.<x> = ZZ[]\nsage: parent((x+1).factor().unit())\nUnivariate Polynomial Ring in x over Integer Ring\nsage: parent(R.one().factor().unit())\nInteger Ring\n```\n\nAlso fixes #20607\n\nIssue created by migration from https://trac.sagemath.org/ticket/20214\n\n",
    "created_at": "2016-03-15T14:03:36Z",
    "labels": [
        "component: algebra",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-9.8",
    "title": "Type inconsistencies in polynomial factorization",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/20214",
    "user": "https://github.com/videlec"
}
```
CC:  @slel

Keywords: factor, polynomial, Laurent polynomial

This ticket fixes a lot of inconsistencies with factorization (`structure/factorization.py`). For example the unit was not forced to belong to the universe of the factorization

```
sage: R.<x> = ZZ[]
sage: parent((x+1).factor().unit())
Univariate Polynomial Ring in x over Integer Ring
sage: parent(R.one().factor().unit())
Integer Ring
```

Also fixes #20607

Issue created by migration from https://trac.sagemath.org/ticket/20214





---

archive/issue_comments_274639.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2016-03-29T08:58:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274639",
    "user": "https://github.com/behackl"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_274640.json:
```json
{
    "body": "I've fixed this to return the unit as an element of the base ring, as it is done for the rationals or by the factorization with Pari.\n\n---\nNew commits:",
    "created_at": "2016-03-29T08:58:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274640",
    "user": "https://github.com/behackl"
}
```

I've fixed this to return the unit as an element of the base ring, as it is done for the rationals or by the factorization with Pari.

---
New commits:



---

archive/issue_comments_274641.json:
```json
{
    "body": "... the doctest failures show that fixing this does require more effort than that. ;-)",
    "created_at": "2016-03-29T13:16:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274641",
    "user": "https://github.com/behackl"
}
```

... the doctest failures show that fixing this does require more effort than that. ;-)



---

archive/issue_comments_274642.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2016-03-29T13:43:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274642",
    "user": "https://github.com/videlec"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_274643.json:
```json
{
    "body": "Factorization is aimed to be a generic class, not only polynomials. I am a little bit worried about your one line change (and patchbot as well). For example the free group has no base ring and matrix groups (`GL` or `SL`) have no multiplication defined with element of the base ring.\n\nNote that some of the patchbot issue is just because `_repr_` of `Factorization` is bad. I will add a commit for that.",
    "created_at": "2016-03-29T13:43:00Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274643",
    "user": "https://github.com/videlec"
}
```

Factorization is aimed to be a generic class, not only polynomials. I am a little bit worried about your one line change (and patchbot as well). For example the free group has no base ring and matrix groups (`GL` or `SL`) have no multiplication defined with element of the base ring.

Note that some of the patchbot issue is just because `_repr_` of `Factorization` is bad. I will add a commit for that.



---

archive/issue_comments_274644.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2016-03-29T13:44:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274644",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_274645.json:
```json
{
    "body": "The biggest issue seems to be that the matrix group code was relying on the current behavior for its factorization. Although I am somewhat more inclined to think the current behavior is the correct way to go because the units do not have to live in the base ring in general. Also from looking at the code, I don't see this fixing things if you work over `QQ[]`.\n\nBTW, you should use `.one()`. It is usually cached, making construction of units in more non-trivial rings much faster and doesn't have to use the coercion framework.",
    "created_at": "2016-03-29T13:45:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274645",
    "user": "https://github.com/tscrim"
}
```

The biggest issue seems to be that the matrix group code was relying on the current behavior for its factorization. Although I am somewhat more inclined to think the current behavior is the correct way to go because the units do not have to live in the base ring in general. Also from looking at the code, I don't see this fixing things if you work over `QQ[]`.

BTW, you should use `.one()`. It is usually cached, making construction of units in more non-trivial rings much faster and doesn't have to use the coercion framework.



---

archive/issue_comments_274646.json:
```json
{
    "body": "With the changes to the representation string and by fixing a silly error I made earlier (pushed one commit), there's just one failure remaining. It seems that the action from a finite field onto the general linear group is not implemented; I think that should be possible. E.g.:\n\n```\nsage: G = GL(3,5); F = G.base_ring()\nsage: F(2) * G.one()\nTraceback (most recent call last):\n...\nTypeError: unsupported operand parent(s) for '*': 'Finite Field of size 5' and 'General Linear Group of degree 3 over Finite Field of size 5'\n```\n\nShould this be handled on this ticket as well or should we open a separate ticket? (Personally, I'm for a separate ticket...)\n\n---\nNew commits:",
    "created_at": "2016-03-29T14:48:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274646",
    "user": "https://github.com/behackl"
}
```

With the changes to the representation string and by fixing a silly error I made earlier (pushed one commit), there's just one failure remaining. It seems that the action from a finite field onto the general linear group is not implemented; I think that should be possible. E.g.:

```
sage: G = GL(3,5); F = G.base_ring()
sage: F(2) * G.one()
Traceback (most recent call last):
...
TypeError: unsupported operand parent(s) for '*': 'Finite Field of size 5' and 'General Linear Group of degree 3 over Finite Field of size 5'
```

Should this be handled on this ticket as well or should we open a separate ticket? (Personally, I'm for a separate ticket...)

---
New commits:



---

archive/issue_comments_274647.json:
```json
{
    "body": "I think that is a separate bug that does need to be fixed. +1 to a separate ticket (but it might be needed as a dependency).",
    "created_at": "2016-03-29T14:51:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274647",
    "user": "https://github.com/tscrim"
}
```

I think that is a separate bug that does need to be fixed. +1 to a separate ticket (but it might be needed as a dependency).



---

archive/issue_comments_274648.json:
```json
{
    "body": "It is not that trivial to fix. The thing is that you can not multiply by `0` in `GL` and there are much more restriction in `SL`... So the action should not go through a coercion.",
    "created_at": "2016-03-29T15:17:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274648",
    "user": "https://github.com/videlec"
}
```

It is not that trivial to fix. The thing is that you can not multiply by `0` in `GL` and there are much more restriction in `SL`... So the action should not go through a coercion.



---

archive/issue_comments_274649.json:
```json
{
    "body": "And still the free group has no base ring... you would better do\n\n```\ntry:\n    unit = self.__universe.base_ring().one()\nexcept XXX:\n    try:\n        unit = self.__universe.one()\n    except XXX\n        unit = Integer(1)\n```\nAnd as Travis mentionned in [comment:6 comment:6] it is not clear to me either if this is the way to go.",
    "created_at": "2016-03-29T15:28:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274649",
    "user": "https://github.com/videlec"
}
```

And still the free group has no base ring... you would better do

```
try:
    unit = self.__universe.base_ring().one()
except XXX:
    try:
        unit = self.__universe.one()
    except XXX
        unit = Integer(1)
```
And as Travis mentionned in [comment:6 comment:6] it is not clear to me either if this is the way to go.



---

archive/issue_comments_274650.json:
```json
{
    "body": "Replying to [comment:9 vdelecroix]:\n> It is not that trivial to fix. The thing is that you can not multiply by `0` in `GL` and there are much more restriction in `SL`... So the action should not go through a coercion.\n\n\nI'd propose to implement `_lmul_` and `_rmul_`; I agree that embedding via coercion is not practicable.\n\nRegarding the `base_ring` and the fact that there might not be one: yes, that has to be taken care of. I haven't thought of that because I was implementing the original version with #20086 in mind. I can't really contribute to the discussion which approach is the most suitable; it just has to be consistent.",
    "created_at": "2016-03-29T16:27:22Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274650",
    "user": "https://github.com/behackl"
}
```

Replying to [comment:9 vdelecroix]:
> It is not that trivial to fix. The thing is that you can not multiply by `0` in `GL` and there are much more restriction in `SL`... So the action should not go through a coercion.


I'd propose to implement `_lmul_` and `_rmul_`; I agree that embedding via coercion is not practicable.

Regarding the `base_ring` and the fact that there might not be one: yes, that has to be taken care of. I haven't thought of that because I was implementing the original version with #20086 in mind. I can't really contribute to the discussion which approach is the most suitable; it just has to be consistent.



---

archive/issue_comments_274651.json:
```json
{
    "body": "My two cents: factoring makes sense in rings (more precisely, in UFD's), so the factors and the unit should be in the corresponding ring. \n\nSo, I think that the correct behaviour should be the opposite of what you propose. That is, we should have:\n\n```\nsage: R.<x> = ZZ[]\nsage: parent((x+1).factor().unit())\nUnivariate Polynomial Ring in x over Integer Ring\nsage: parent(R.one().factor().unit())\nUnivariate Polynomial Ring in x over Integer Ring\n```\n\nThere might be rings (e.g. Laurent polynomials) with units that are not in the rings base ring. So the only possible consistent behaviour is to have the units in the ring (and not in its base ring).",
    "created_at": "2016-05-14T23:09:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274651",
    "user": "https://github.com/miguelmarco"
}
```

My two cents: factoring makes sense in rings (more precisely, in UFD's), so the factors and the unit should be in the corresponding ring. 

So, I think that the correct behaviour should be the opposite of what you propose. That is, we should have:

```
sage: R.<x> = ZZ[]
sage: parent((x+1).factor().unit())
Univariate Polynomial Ring in x over Integer Ring
sage: parent(R.one().factor().unit())
Univariate Polynomial Ring in x over Integer Ring
```

There might be rings (e.g. Laurent polynomials) with units that are not in the rings base ring. So the only possible consistent behaviour is to have the units in the ring (and not in its base ring).



---

archive/issue_comments_274652.json:
```json
{
    "body": "Of course. But in some cases, the units live in a well identified subgring. And it makes sense to keep them here. For me it would still be acceptable if the parent of the unit only had a coercion to the main ring. But of course this should be done consistently for a given ring. Hence the creation of this ticket.\n\nI agree that it is much simpler to enforce the unit to belong in the same ring (or more generally multiplicative semigroup).",
    "created_at": "2016-05-17T02:44:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274652",
    "user": "https://github.com/videlec"
}
```

Of course. But in some cases, the units live in a well identified subgring. And it makes sense to keep them here. For me it would still be acceptable if the parent of the unit only had a coercion to the main ring. But of course this should be done consistently for a given ring. Hence the creation of this ticket.

I agree that it is much simpler to enforce the unit to belong in the same ring (or more generally multiplicative semigroup).



---

archive/issue_events_054903.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2016-06-26T07:02:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "milestone": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20214#event-54903"
}
```



---

archive/issue_comments_274653.json:
```json
{
    "body": "New commits:",
    "created_at": "2016-06-26T07:02:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274653",
    "user": "https://github.com/videlec"
}
```

New commits:



---

archive/issue_comments_274654.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2016-06-26T07:03:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274654",
    "user": "https://github.com/videlec"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_274655.json:
```json
{
    "body": "Some questions after a first quick look.\n\nWhy in this part of the code, \n\n```\n        elif universe is not None:\n            x = [(universe(i),j) for i,j in x]\n            if unit is None:\n                try:\n                    unit = universe(1)\n                except (ValueError,TypeError):\n                    pass\n```\n\nwe don't include this first attempt?:\n\n```\n                 try:\n                    unit = universe.one()\n```\n.\n\n\nIn the neg method we only allow negation of factorizations with unit (from what I have seen, it could happen in parents with no one method, and that doesn't allow to convert 1, not sure if they exist).\n\nOtherwise it looks ok, but I still have to test it a bit more deeply.",
    "created_at": "2016-06-26T09:41:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274655",
    "user": "https://github.com/miguelmarco"
}
```

Some questions after a first quick look.

Why in this part of the code, 

```
        elif universe is not None:
            x = [(universe(i),j) for i,j in x]
            if unit is None:
                try:
                    unit = universe(1)
                except (ValueError,TypeError):
                    pass
```

we don't include this first attempt?:

```
                 try:
                    unit = universe.one()
```
.


In the neg method we only allow negation of factorizations with unit (from what I have seen, it could happen in parents with no one method, and that doesn't allow to convert 1, not sure if they exist).

Otherwise it looks ok, but I still have to test it a bit more deeply.



---

archive/issue_comments_274656.json:
```json
{
    "body": "Replying to [comment:17 mmarco]:\n> Some questions after a first quick look.\n> \n> Why in this part of the code, \n> \n> `...`\n\n>\n> we don't include this first attempt?:\n> \n> `...`\n> \n\n\nRight. It can be simplified a bit.\n\n> In the neg method we only allow negation of factorizations with unit (from what I have seen, it could happen in parents with no one method, and that doesn't allow to convert 1, not sure if they exist).\n\n\nRight, there is a problem here. What should we do for `-fac` when there is no unit? Raise an error?\n\n> Otherwise it looks ok, but I still have to test it a bit more deeply.\n\n\nThanks for looking at it!",
    "created_at": "2016-06-26T10:22:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274656",
    "user": "https://github.com/videlec"
}
```

Replying to [comment:17 mmarco]:
> Some questions after a first quick look.
> 
> Why in this part of the code, 
> 
> `...`

>
> we don't include this first attempt?:
> 
> `...`
> 


Right. It can be simplified a bit.

> In the neg method we only allow negation of factorizations with unit (from what I have seen, it could happen in parents with no one method, and that doesn't allow to convert 1, not sure if they exist).


Right, there is a problem here. What should we do for `-fac` when there is no unit? Raise an error?

> Otherwise it looks ok, but I still have to test it a bit more deeply.


Thanks for looking at it!



---

archive/issue_comments_274657.json:
```json
{
    "body": "Maybe for the -fac problem we could just change the sign to the first factor if there is no unit?",
    "created_at": "2016-06-26T10:28:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274657",
    "user": "https://github.com/miguelmarco"
}
```

Maybe for the -fac problem we could just change the sign to the first factor if there is no unit?



---

archive/issue_comments_274658.json:
```json
{
    "body": "Do you have concrete examples where there is no unit available but `x -> -x` is well defined (inside Sage of course)? I would rather raise a `ValueError`.",
    "created_at": "2016-06-26T10:47:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274658",
    "user": "https://github.com/videlec"
}
```

Do you have concrete examples where there is no unit available but `x -> -x` is well defined (inside Sage of course)? I would rather raise a `ValueError`.



---

archive/issue_comments_274659.json:
```json
{
    "body": "Fair enough. Raise error it is.",
    "created_at": "2016-06-26T11:56:21Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274659",
    "user": "https://github.com/miguelmarco"
}
```

Fair enough. Raise error it is.



---

archive/issue_comments_274660.json:
```json
{
    "body": "bad trac link, see patchbot report:\n\n```\n+inside file:  b/src/sage/structure/factorization.py\n+@@ -1054,21 +1067,24 @@ class Factorization(SageObject):\n++        Check that it only depends on the universe (trac:`20214`)::\n```",
    "created_at": "2016-06-26T18:35:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274660",
    "user": "https://github.com/fchapoton"
}
```

bad trac link, see patchbot report:

```
+inside file:  b/src/sage/structure/factorization.py
+@@ -1054,21 +1067,24 @@ class Factorization(SageObject):
++        Check that it only depends on the universe (trac:`20214`)::
```



---

archive/issue_comments_274661.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2016-06-26T20:51:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274661",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_274662.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2016-06-27T08:55:19Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274662",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_274663.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2016-06-27T13:06:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274663",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_274664.json:
```json
{
    "body": "one failing doctest\n\n```\nFile \"src/sage/tests/french_book/polynomes.py\", line 144, in sage.tests.french_book.polynomes\nFailed example:\n    for A in [QQ, ComplexField(16), GF(5), QQ[sqrt(2)]]:\n        print(\"{} :\".format(A)); print(A['x'](p).factor())\n```",
    "created_at": "2016-06-28T06:04:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274664",
    "user": "https://github.com/fchapoton"
}
```

one failing doctest

```
File "src/sage/tests/french_book/polynomes.py", line 144, in sage.tests.french_book.polynomes
Failed example:
    for A in [QQ, ComplexField(16), GF(5), QQ[sqrt(2)]]:
        print("{} :".format(A)); print(A['x'](p).factor())
```



---

archive/issue_comments_274665.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2016-06-28T06:14:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274665",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_274666.json:
```json
{
    "body": "ping?",
    "created_at": "2016-07-06T14:38:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274666",
    "user": "https://github.com/videlec"
}
```

ping?



---

archive/issue_comments_274667.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2016-11-05T07:28:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274667",
    "user": "https://github.com/fchapoton"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_274668.json:
```json
{
    "body": "no longer applies",
    "created_at": "2016-11-05T07:28:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274668",
    "user": "https://github.com/fchapoton"
}
```

no longer applies



---

archive/issue_comments_274669.json:
```json
{
    "body": "This came up again at\n\n- [Ask Sage question 44764: LaurentPolynomial can't factor constants](https://ask.sagemath.org/question/44764)\n\nCan the work done here be rebased to apply on the latest release?",
    "created_at": "2018-12-26T09:34:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274669",
    "user": "https://github.com/slel"
}
```

This came up again at

- [Ask Sage question 44764: LaurentPolynomial can't factor constants](https://ask.sagemath.org/question/44764)

Can the work done here be rebased to apply on the latest release?



---

archive/issue_events_054904.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2018-12-26T09:38:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "milestone": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20214#event-54904"
}
```



---

archive/issue_events_054905.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2018-12-26T09:38:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "milestone": "sage-8.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20214#event-54905"
}
```



---

archive/issue_comments_274670.json:
```json
{
    "body": "Changing keywords from \"\" to \"factor, polynomial, Laurent polynomial\".",
    "created_at": "2018-12-26T09:38:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274670",
    "user": "https://github.com/slel"
}
```

Changing keywords from "" to "factor, polynomial, Laurent polynomial".



---

archive/issue_comments_274671.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:",
    "created_at": "2018-12-31T13:12:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274671",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:



---

archive/issue_comments_274672.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2018-12-31T13:14:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274672",
    "user": "https://github.com/videlec"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_events_054906.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-12-31T13:14:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "milestone": "sage-8.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20214#event-54906"
}
```



---

archive/issue_events_054907.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-12-31T13:14:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "milestone": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20214#event-54907"
}
```



---

archive/issue_comments_274673.json:
```json
{
    "body": "Replying to [comment:30 slelievre]:\n> This came up again at\n> \n> - [Ask Sage question 44764: LaurentPolynomial can't factor constants](https://ask.sagemath.org/question/44764)\n> \n> Can the work done here be rebased to apply on the latest release?\n\n\nCan you do the review now that it is rebased on your demand?",
    "created_at": "2019-01-03T09:53:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274673",
    "user": "https://github.com/videlec"
}
```

Replying to [comment:30 slelievre]:
> This came up again at
> 
> - [Ask Sage question 44764: LaurentPolynomial can't factor constants](https://ask.sagemath.org/question/44764)
> 
> Can the work done here be rebased to apply on the latest release?


Can you do the review now that it is rebased on your demand?



---

archive/issue_comments_274674.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2019-01-03T09:58:12Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274674",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_274675.json:
```json
{
    "body": "Replying to [comment:34 vdelecroix]:\n> Replying to [comment:30 slelievre]:\n> > This came up again at\n> > \n> > - [Ask Sage question 44764: LaurentPolynomial can't factor constants](https://ask.sagemath.org/question/44764)\n> > \n> > Can the work done here be rebased to apply on the latest release?\n\n> \n> Can you do the review now that it is rebased on your demand?\n\n\nHow much time do you need Samuel for the review?",
    "created_at": "2019-02-05T08:00:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274675",
    "user": "https://github.com/videlec"
}
```

Replying to [comment:34 vdelecroix]:
> Replying to [comment:30 slelievre]:
> > This came up again at
> > 
> > - [Ask Sage question 44764: LaurentPolynomial can't factor constants](https://ask.sagemath.org/question/44764)
> > 
> > Can the work done here be rebased to apply on the latest release?

> 
> Can you do the review now that it is rebased on your demand?


How much time do you need Samuel for the review?



---

archive/issue_comments_274676.json:
```json
{
    "body": "red branch => needs work",
    "created_at": "2019-02-16T10:25:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274676",
    "user": "https://github.com/fchapoton"
}
```

red branch => needs work



---

archive/issue_comments_274677.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2019-02-16T10:25:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274677",
    "user": "https://github.com/fchapoton"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_events_054908.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-03-25T10:56:15Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "milestone": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20214#event-54908"
}
```



---

archive/issue_events_054909.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-03-25T10:56:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "milestone": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20214#event-54909"
}
```



---

archive/issue_comments_274678.json:
```json
{
    "body": "Ticket retargeted after milestone closed (if you don't believe this ticket is appropriate for the Sage 8.8 release please retarget manually)",
    "created_at": "2019-03-25T10:56:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274678",
    "user": "https://github.com/embray"
}
```

Ticket retargeted after milestone closed (if you don't believe this ticket is appropriate for the Sage 8.8 release please retarget manually)



---

archive/issue_comments_274679.json:
```json
{
    "body": "Tickets still needing working or clarification should be moved to the next release milestone at the soonest (please feel free to revert if you think the ticket is close to being resolved).",
    "created_at": "2019-06-14T14:50:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274679",
    "user": "https://github.com/embray"
}
```

Tickets still needing working or clarification should be moved to the next release milestone at the soonest (please feel free to revert if you think the ticket is close to being resolved).



---

archive/issue_events_054910.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-06-14T14:50:27Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "milestone": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20214#event-54910"
}
```



---

archive/issue_events_054911.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-06-14T14:50:27Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "milestone": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20214#event-54911"
}
```



---

archive/issue_comments_274680.json:
```json
{
    "body": "Ticket retargeted after milestone closed",
    "created_at": "2019-12-30T14:48:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274680",
    "user": "https://github.com/embray"
}
```

Ticket retargeted after milestone closed



---

archive/issue_events_054912.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "milestone": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20214#event-54912"
}
```



---

archive/issue_events_054913.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-12-30T14:48:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "milestone": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20214#event-54913"
}
```



---

archive/issue_comments_274681.json:
```json
{
    "body": "Possibly related: #29076",
    "created_at": "2020-01-25T02:50:46Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274681",
    "user": "https://github.com/mkoeppe"
}
```

Possibly related: #29076



---

archive/issue_comments_274682.json:
```json
{
    "body": "Replying to [comment:42 mkoeppe]:\n> Possibly related: #29076\n\n\nit is not.",
    "created_at": "2020-01-25T16:29:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274682",
    "user": "https://github.com/videlec"
}
```

Replying to [comment:42 mkoeppe]:
> Possibly related: #29076


it is not.



---

archive/issue_comments_274683.json:
```json
{
    "body": "Moving tickets to milestone sage-9.2 based on a review of last modification date, branch status, and severity.",
    "created_at": "2020-05-01T04:28:42Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274683",
    "user": "https://github.com/mkoeppe"
}
```

Moving tickets to milestone sage-9.2 based on a review of last modification date, branch status, and severity.



---

archive/issue_events_054914.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-05-01T04:28:42Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "milestone": "sage-9.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20214#event-54914"
}
```



---

archive/issue_events_054915.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-05-01T04:28:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20214#event-54915"
}
```



---

archive/issue_events_054916.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "milestone": "sage-9.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20214#event-54916"
}
```



---

archive/issue_events_054917.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2020-10-24T20:15:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20214#event-54917"
}
```



---

archive/issue_comments_274684.json:
```json
{
    "body": "Setting new milestone based on a cursory review of ticket status, priority, and last modification date.",
    "created_at": "2021-02-13T20:51:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274684",
    "user": "https://github.com/mkoeppe"
}
```

Setting new milestone based on a cursory review of ticket status, priority, and last modification date.



---

archive/issue_events_054918.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "milestone": "sage-9.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20214#event-54918"
}
```



---

archive/issue_events_054919.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-02-13T20:51:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20214#event-54919"
}
```



---

archive/issue_comments_274685.json:
```json
{
    "body": "Setting a new milestone for this ticket based on a cursory review.",
    "created_at": "2021-07-19T00:44:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20214#issuecomment-274685",
    "user": "https://github.com/mkoeppe"
}
```

Setting a new milestone for this ticket based on a cursory review.



---

archive/issue_events_054920.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T00:44:56Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "milestone": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20214#event-54920"
}
```



---

archive/issue_events_054921.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T00:44:56Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20214#event-54921"
}
```



---

archive/issue_events_054922.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:24:17Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "milestone": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20214#event-54922"
}
```



---

archive/issue_events_054923.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-18T19:24:17Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20214#event-54923"
}
```



---

archive/issue_events_054924.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-07T00:07:21Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "milestone": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20214#event-54924"
}
```



---

archive/issue_events_054925.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-04-07T00:07:21Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20214#event-54925"
}
```



---

archive/issue_events_054926.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "milestone": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20214#event-54926"
}
```



---

archive/issue_events_054927.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-09-19T18:58:47Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20214",
    "milestone": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20214#event-54927"
}
```
