# Issue 20366: bogus dependencies in m4rie

archive/issues_020129.json:
```json
{
    "assignees": [],
    "body": "For some reasons `m4rie` depends on `givaro` and `ntl`. Those dependency are bogus\n\n```\nreadelf -d /usr/lib64/libm4rie-0.0.20150908.so\n\nDynamic section at offset 0x8fdd0 contains 27 entries:\n  Tag        Type                         Name/Value\n 0x0000000000000001 (NEEDED)             Shared library: [libm4ri-0.0.20140914.so]\n 0x0000000000000001 (NEEDED)             Shared library: [libpthread.so.0]\n 0x0000000000000001 (NEEDED)             Shared library: [libc.so.6]\n 0x000000000000000e (SONAME)             Library soname: [libm4rie-0.0.20150908.so]\n```\nand uses no headers from either `givaro` or `ntl`.\n\nCC:  @jdemeyer\n\nBranch/Commit: **[d0ce97e](https://github.com/sagemath/sagetrac-mirror/commit/d0ce97ec8acb314a09a86ca64c2b1616b5dca0ee)**\n\nReviewer: **Jeroen Demeyer**\n\nAuthor: **Fran\u00e7ois Bissey**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20366_\n\n",
    "closed_at": "2016-04-08T00:25:39Z",
    "created_at": "2016-04-06T10:07:30Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "bogus dependencies in m4rie",
    "type": "issue",
    "updated_at": "2016-04-08T00:25:39Z",
    "url": "https://github.com/sagemath/sage/issues/20366",
    "user": "https://github.com/kiwifb"
}
```
For some reasons `m4rie` depends on `givaro` and `ntl`. Those dependency are bogus

```
readelf -d /usr/lib64/libm4rie-0.0.20150908.so

Dynamic section at offset 0x8fdd0 contains 27 entries:
  Tag        Type                         Name/Value
 0x0000000000000001 (NEEDED)             Shared library: [libm4ri-0.0.20140914.so]
 0x0000000000000001 (NEEDED)             Shared library: [libpthread.so.0]
 0x0000000000000001 (NEEDED)             Shared library: [libc.so.6]
 0x000000000000000e (SONAME)             Library soname: [libm4rie-0.0.20150908.so]
```
and uses no headers from either `givaro` or `ntl`.

CC:  @jdemeyer

Branch/Commit: **[d0ce97e](https://github.com/sagemath/sagetrac-mirror/commit/d0ce97ec8acb314a09a86ca64c2b1616b5dca0ee)**

Reviewer: **Jeroen Demeyer**

Author: **François Bissey**

_Issue created by migration from https://trac.sagemath.org/ticket/20366_





---

archive/issue_events_269498.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-04-06T10:07:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20366",
    "milestone_number": null,
    "milestone_title": "sage-7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20366#event-269498"
}
```



---

archive/issue_events_269499.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-04-06T10:07:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20366",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20366#event-269499"
}
```



---

archive/issue_events_269500.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-04-06T10:07:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20366",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20366#event-269500"
}
```



---

archive/issue_events_269501.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-04-06T10:07:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20366",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20366#event-269501"
}
```



---

archive/issue_events_269502.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-04-06T10:07:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20366",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20366#event-269502"
}
```



---

archive/issue_comments_296345.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2016-04-06T15:04:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20366",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20366#issuecomment-296345",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_comments_296346.json:
```json
{
    "body": "Author: **Fran\u00e7ois Bissey**",
    "created_at": "2016-04-06T15:23:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20366",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20366#issuecomment-296346",
    "user": "https://github.com/jdemeyer"
}
```

Author: **François Bissey**



---

archive/issue_events_269503.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-06T15:23:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20366",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20366#event-269503"
}
```



---

archive/issue_events_269504.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-06T15:23:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20366",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20366#event-269504"
}
```



---

archive/issue_events_269505.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-04-08T00:25:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20366",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20366#event-269505"
}
```



---

archive/issue_events_269506.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-04-08T00:25:39Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20366",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20366#event-269506"
}
```



---

archive/issue_comments_296347.json:
```json
{
    "body": "Changed branch from **[u/fbissey/m4rie](https://github.com/sagemath/sagetrac-mirror/tree/u/fbissey/m4rie)** to **[d0ce97e](https://github.com/sagemath/sagetrac-mirror/commit/d0ce97ec8acb314a09a86ca64c2b1616b5dca0ee)**",
    "created_at": "2016-04-08T00:25:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20366",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20366#issuecomment-296347",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/fbissey/m4rie](https://github.com/sagemath/sagetrac-mirror/tree/u/fbissey/m4rie)** to **[d0ce97e](https://github.com/sagemath/sagetrac-mirror/commit/d0ce97ec8acb314a09a86ca64c2b1616b5dca0ee)**
