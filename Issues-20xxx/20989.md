# Issue 20989: Fix some ambiguous references in the docs

archive/issues_020752.json:
```json
{
    "assignees": [],
    "body": "Sometimes this can cause building the docs to fail, espcially when\nbuilding in a single process.  See #20639\n\nThis doesn't fix the underlying issue, but does work around it in\nseveral cases that were affected, simply by using more explicit\nreferences.  From my readings of the docs, I'm pretty sure I got the\nupdated references right, but someone who knows that code\n(`sage.graphs` and `sage.categories`) might want to check.\n\nBranch/Commit: **[u/embray/ambiguous-doc-refs](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/ambiguous-doc-refs) @ [9152588](https://github.com/sagemath/sagetrac-mirror/commit/91525886d460c9077745251497a0efab28bf23bc)**\n\nAuthor: **Erik Bray**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20989_\n\n",
    "closed_at": "2016-11-23T16:43:09Z",
    "created_at": "2016-07-08T14:51:03Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/invalid",
        "https://github.com/sagemath/sage/labels/worksforme"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix some ambiguous references in the docs",
    "type": "issue",
    "updated_at": "2016-11-23T17:56:01Z",
    "url": "https://github.com/sagemath/sage/issues/20989",
    "user": "https://github.com/embray"
}
```
Sometimes this can cause building the docs to fail, espcially when
building in a single process.  See #20639

This doesn't fix the underlying issue, but does work around it in
several cases that were affected, simply by using more explicit
references.  From my readings of the docs, I'm pretty sure I got the
updated references right, but someone who knows that code
(`sage.graphs` and `sage.categories`) might want to check.

Branch/Commit: **[u/embray/ambiguous-doc-refs](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/ambiguous-doc-refs) @ [9152588](https://github.com/sagemath/sagetrac-mirror/commit/91525886d460c9077745251497a0efab28bf23bc)**

Author: **Erik Bray**

_Issue created by migration from https://trac.sagemath.org/ticket/20989_





---

archive/issue_events_277331.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-07-08T14:51:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20989",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20989#event-277331"
}
```



---

archive/issue_events_277332.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-07-08T14:51:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20989",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20989#event-277332"
}
```



---

archive/issue_events_277333.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-07-08T14:51:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20989",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20989#event-277333"
}
```



---

archive/issue_events_277334.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-07-08T14:51:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20989",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20989#event-277334"
}
```



---

archive/issue_events_277335.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-07-08T14:51:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20989",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20989#event-277335"
}
```



---

archive/issue_events_277336.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-07-08T14:56:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20989",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20989#event-277336"
}
```



---

archive/issue_events_277337.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-07-08T14:56:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20989",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20989#event-277337"
}
```



---

archive/issue_comments_307527.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nAt least one of these updated references doesn't resolve into a link when I build the docs, it turns out. So maybe it needs to be updated to be even more explicit.",
    "created_at": "2016-07-08T14:56:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20989",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20989#issuecomment-307527",
    "user": "https://github.com/embray"
}
```

<div id="comment:2" align="right">Comment 2</div>

At least one of these updated references doesn't resolve into a link when I build the docs, it turns out. So maybe it needs to be updated to be even more explicit.



---

archive/issue_comments_307528.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nTurns out the issues I was trying to fix here were actually a side-effect of the issue I fixed in #21044, so this ticket can be closed.  Will reopen if this crops up again somehow.",
    "created_at": "2016-11-23T16:43:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20989",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20989#issuecomment-307528",
    "user": "https://github.com/embray"
}
```

<div id="comment:3" align="right">Comment 3</div>

Turns out the issues I was trying to fix here were actually a side-effect of the issue I fixed in #21044, so this ticket can be closed.  Will reopen if this crops up again somehow.



---

archive/issue_events_277338.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-11-23T16:43:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20989",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20989#event-277338"
}
```



---

archive/issue_events_277339.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-11-23T16:43:09Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20989",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20989#event-277339"
}
```



---

archive/issue_events_277340.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-11-23T16:43:09Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/20989",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20989#event-277340"
}
```



---

archive/issue_events_277341.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-11-23T16:43:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20989",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20989#event-277341"
}
```



---

archive/issue_events_277342.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-11-23T16:43:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20989",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20989#event-277342"
}
```



---

archive/issue_events_277343.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-11-23T16:43:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20989",
    "label": "https://github.com/sagemath/sage/labels/worksforme",
    "label_color": "a6a6a6",
    "label_name": "worksforme",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20989#event-277343"
}
```



---

archive/issue_comments_307529.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nThis is also supposed to be fixed by #21454.",
    "created_at": "2016-11-23T17:56:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20989",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20989#issuecomment-307529",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:4" align="right">Comment 4</div>

This is also supposed to be fixed by #21454.
