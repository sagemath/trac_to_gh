# Issue 20644: Disable fragile sagedev tests

archive/issues_020407.json:
```json
{
    "assignees": [],
    "body": "Various sage.dev tests are fragile and depend on details of the git version and/or configuration. A number of buildbots are now failing with\n\n```\nsage -t --long src/sage/dev/git_interface.py  # 8 doctests failed\nsage -t --long src/sage/dev/patch.py  # 2 doctests failed\nsage -t --long src/sage/dev/sagedev.py  # 1 doctest failed\n```\n\nAlternative ticket: #20645 \n\nBranch/Commit: **[u/vbraun/disable_fragile_sagedev_tests](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/disable_fragile_sagedev_tests) @ [36a00ea](https://github.com/sagemath/sagetrac-mirror/commit/36a00ea716f50fc98b2e6437e5927f2fa8973252)**\n\nAuthor: **Volker Braun**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20644_\n\n",
    "closed_at": "2016-05-21T23:06:14Z",
    "created_at": "2016-05-21T10:23:50Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Disable fragile sagedev tests",
    "type": "issue",
    "updated_at": "2016-05-21T23:06:14Z",
    "url": "https://github.com/sagemath/sage/issues/20644",
    "user": "https://github.com/vbraun"
}
```
Various sage.dev tests are fragile and depend on details of the git version and/or configuration. A number of buildbots are now failing with

```
sage -t --long src/sage/dev/git_interface.py  # 8 doctests failed
sage -t --long src/sage/dev/patch.py  # 2 doctests failed
sage -t --long src/sage/dev/sagedev.py  # 1 doctest failed
```

Alternative ticket: #20645 

Branch/Commit: **[u/vbraun/disable_fragile_sagedev_tests](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/disable_fragile_sagedev_tests) @ [36a00ea](https://github.com/sagemath/sagetrac-mirror/commit/36a00ea716f50fc98b2e6437e5927f2fa8973252)**

Author: **Volker Braun**

_Issue created by migration from https://trac.sagemath.org/ticket/20644_





---

archive/issue_events_272806.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-21T10:23:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20644",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20644#event-272806"
}
```



---

archive/issue_events_272807.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-21T10:23:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20644",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20644#event-272807"
}
```



---

archive/issue_events_272808.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-21T10:23:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20644",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20644#event-272808"
}
```



---

archive/issue_events_272809.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-21T10:23:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20644",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20644#event-272809"
}
```



---

archive/issue_comments_301099.json:
```json
{
    "body": "Branch: **[u/vbraun/disable_fragile_sagedev_tests](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/disable_fragile_sagedev_tests)**",
    "created_at": "2016-05-21T10:37:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20644#issuecomment-301099",
    "user": "https://github.com/vbraun"
}
```

Branch: **[u/vbraun/disable_fragile_sagedev_tests](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/disable_fragile_sagedev_tests)**



---

archive/issue_comments_301100.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/36a00ea716f50fc98b2e6437e5927f2fa8973252\">36a00ea</a></td><td><code>Disable fragile sagedev doctests</code></td></tr></table>\n",
    "created_at": "2016-05-21T10:37:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20644#issuecomment-301100",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/36a00ea716f50fc98b2e6437e5927f2fa8973252">36a00ea</a></td><td><code>Disable fragile sagedev doctests</code></td></tr></table>




---

archive/issue_events_272810.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-21T10:37:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20644",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20644#event-272810"
}
```



---

archive/issue_comments_301101.json:
```json
{
    "body": "Author: **Volker Braun**",
    "created_at": "2016-05-21T10:37:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20644#issuecomment-301101",
    "user": "https://github.com/vbraun"
}
```

Author: **Volker Braun**



---

archive/issue_comments_301102.json:
```json
{
    "body": "Commit: **[36a00ea](https://github.com/sagemath/sagetrac-mirror/commit/36a00ea716f50fc98b2e6437e5927f2fa8973252)**",
    "created_at": "2016-05-21T10:37:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20644#issuecomment-301102",
    "user": "https://github.com/vbraun"
}
```

Commit: **[36a00ea](https://github.com/sagemath/sagetrac-mirror/commit/36a00ea716f50fc98b2e6437e5927f2fa8973252)**



---

archive/issue_events_272811.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-21T10:37:17Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20644",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20644#event-272811"
}
```



---

archive/issue_events_272812.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-21T10:37:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20644",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20644#event-272812"
}
```



---

archive/issue_comments_301103.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -5,3 +5,5 @@\n sage -t --long src/sage/dev/patch.py  # 2 doctests failed\n sage -t --long src/sage/dev/sagedev.py  # 1 doctest failed\n ```\n+\n+Alternative ticket: #20645 \n``````\n",
    "created_at": "2016-05-21T10:47:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20644",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20644#issuecomment-301103",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -5,3 +5,5 @@
 sage -t --long src/sage/dev/patch.py  # 2 doctests failed
 sage -t --long src/sage/dev/sagedev.py  # 1 doctest failed
 ```
+
+Alternative ticket: #20645 
``````




---

archive/issue_events_272813.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2016-05-21T11:04:20Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/20644",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20644#event-272813"
}
```



---

archive/issue_events_272814.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2016-05-21T11:04:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20644",
    "label": "https://github.com/sagemath/sage/labels/p1%20%E2%80%93%20blocker",
    "label_color": "ff0000",
    "label_name": "p1 \u2013 blocker",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20644#event-272814"
}
```



---

archive/issue_events_272815.json:
```json
{
    "actor": "https://github.com/sagetrac-tmonteil",
    "created_at": "2016-05-21T11:04:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20644",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20644#event-272815"
}
```



---

archive/issue_events_272816.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-21T23:06:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20644",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20644#event-272816"
}
```



---

archive/issue_events_272817.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-21T23:06:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20644",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20644#event-272817"
}
```



---

archive/issue_events_272818.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-21T23:06:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20644",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20644#event-272818"
}
```
