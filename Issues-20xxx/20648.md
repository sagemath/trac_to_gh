# Issue 20648: functools32 is not py3 compatible

archive/issues_020411.json:
```json
{
    "assignees": [],
    "body": "Thats kind of ok because its just a backport, but we need a way to only conditionally build it. Right now\n\n```\nSAGE_PYTHON3=yes make\n[...]\nThis backport is for Python 2.7 only.\nError installing functools32 ... exiting\n```\n\nCC:  @jdemeyer @fchapoton\n\nBranch/Commit: **[6109459](https://github.com/sagemath/sagetrac-mirror/commit/6109459c84fec249fe1377378c77b0de9149e93e)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Volker Braun**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20648_\n\n",
    "closed_at": "2016-08-07T20:01:37Z",
    "created_at": "2016-05-21T16:39:12Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "functools32 is not py3 compatible",
    "type": "issue",
    "updated_at": "2016-08-07T20:01:37Z",
    "url": "https://github.com/sagemath/sage/issues/20648",
    "user": "https://github.com/vbraun"
}
```
Thats kind of ok because its just a backport, but we need a way to only conditionally build it. Right now

```
SAGE_PYTHON3=yes make
[...]
This backport is for Python 2.7 only.
Error installing functools32 ... exiting
```

CC:  @jdemeyer @fchapoton

Branch/Commit: **[6109459](https://github.com/sagemath/sagetrac-mirror/commit/6109459c84fec249fe1377378c77b0de9149e93e)**

Reviewer: **Frédéric Chapoton**

Author: **Volker Braun**

_Issue created by migration from https://trac.sagemath.org/ticket/20648_





---

archive/issue_events_272859.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-21T16:39:12Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20648",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20648#event-272859"
}
```



---

archive/issue_events_272860.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-21T16:39:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20648",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20648#event-272860"
}
```



---

archive/issue_events_272861.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-21T16:39:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20648",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20648#event-272861"
}
```



---

archive/issue_events_272862.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-21T16:39:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20648",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20648#event-272862"
}
```



---

archive/issue_comments_301171.json:
```json
{
    "body": "Branch: **[u/vbraun/functools32_is_not_py3_compatible](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/functools32_is_not_py3_compatible)**",
    "created_at": "2016-08-01T19:22:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20648",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20648#issuecomment-301171",
    "user": "https://github.com/vbraun"
}
```

Branch: **[u/vbraun/functools32_is_not_py3_compatible](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/functools32_is_not_py3_compatible)**



---

archive/issue_comments_301172.json:
```json
{
    "body": "Commit: **[6109459](https://github.com/sagemath/sagetrac-mirror/commit/6109459c84fec249fe1377378c77b0de9149e93e)**",
    "created_at": "2016-08-01T19:32:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20648",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20648#issuecomment-301172",
    "user": "https://github.com/vbraun"
}
```

Commit: **[6109459](https://github.com/sagemath/sagetrac-mirror/commit/6109459c84fec249fe1377378c77b0de9149e93e)**



---

archive/issue_events_272863.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-08-01T19:32:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20648",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20648#event-272863"
}
```



---

archive/issue_comments_301173.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6109459c84fec249fe1377378c77b0de9149e93e\">6109459</a></td><td><code>Skip building functools32 on Python 3.x</code></td></tr></table>\n",
    "created_at": "2016-08-01T19:32:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20648",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20648#issuecomment-301173",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6109459c84fec249fe1377378c77b0de9149e93e">6109459</a></td><td><code>Skip building functools32 on Python 3.x</code></td></tr></table>




---

archive/issue_comments_301174.json:
```json
{
    "body": "Author: **Volker Braun**",
    "created_at": "2016-08-01T19:32:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20648",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20648#issuecomment-301174",
    "user": "https://github.com/vbraun"
}
```

Author: **Volker Braun**



---

archive/issue_events_272864.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-08-03T18:28:56Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20648",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20648#event-272864"
}
```



---

archive/issue_events_272865.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-08-03T18:28:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20648",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20648#event-272865"
}
```



---

archive/issue_comments_301175.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nok, looks good enough",
    "created_at": "2016-08-03T18:28:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20648",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20648#issuecomment-301175",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">Comment 3</div>

ok, looks good enough



---

archive/issue_comments_301176.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2016-08-03T18:28:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20648",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20648#issuecomment-301176",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_events_272866.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-08-07T20:01:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20648",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20648#event-272866"
}
```



---

archive/issue_events_272867.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "1d3a7804475b23100759405fa6c116b094e154d8",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-08-07T20:01:37Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20648",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20648#event-272867"
}
```



---

archive/issue_comments_301177.json:
```json
{
    "body": "Changed branch from **[u/vbraun/functools32_is_not_py3_compatible](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/functools32_is_not_py3_compatible)** to **[6109459](https://github.com/sagemath/sagetrac-mirror/commit/6109459c84fec249fe1377378c77b0de9149e93e)**",
    "created_at": "2016-08-07T20:01:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20648",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20648#issuecomment-301177",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/vbraun/functools32_is_not_py3_compatible](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/functools32_is_not_py3_compatible)** to **[6109459](https://github.com/sagemath/sagetrac-mirror/commit/6109459c84fec249fe1377378c77b0de9149e93e)**
