# Issue 20909: not using string.join any longer

archive/issues_020672.json:
```json
{
    "assignees": [],
    "body": "as a step towards python3\n\nlet us remove the few existing instances of \"from string import join\"\n\none is in a script that is run early in sage installation\n\nCC:  @embray @tscrim @jdemeyer @vbraun\n\nBranch/Commit: **[b270f8c](https://github.com/sagemath/sagetrac-mirror/commit/b270f8c96088c5679f62f79c0b7706230618ed42)**\n\nReviewer: **Erik Bray**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20909_\n\n",
    "closed_at": "2016-07-01T16:57:03Z",
    "created_at": "2016-06-30T11:42:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20python3",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "not using string.join any longer",
    "type": "issue",
    "updated_at": "2016-07-01T16:57:03Z",
    "url": "https://github.com/sagemath/sage/issues/20909",
    "user": "https://github.com/fchapoton"
}
```
as a step towards python3

let us remove the few existing instances of "from string import join"

one is in a script that is run early in sage installation

CC:  @embray @tscrim @jdemeyer @vbraun

Branch/Commit: **[b270f8c](https://github.com/sagemath/sagetrac-mirror/commit/b270f8c96088c5679f62f79c0b7706230618ed42)**

Reviewer: **Erik Bray**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/20909_





---

archive/issue_events_276336.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-06-30T11:42:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20909",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20909#event-276336"
}
```



---

archive/issue_events_276337.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-06-30T11:42:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20909",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20python3",
    "label_color": "000080",
    "label_name": "component: python3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20909#event-276337"
}
```



---

archive/issue_events_276338.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-06-30T11:42:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20909",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20909#event-276338"
}
```



---

archive/issue_events_276339.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-06-30T11:42:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20909",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20909#event-276339"
}
```



---

archive/issue_comments_306242.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7058f361bd60c6bf742e8dd77df339c1f1102fd6\">7058f36</a></td><td><code>getting rid of from string import join</code></td></tr></table>\n",
    "created_at": "2016-06-30T11:42:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20909#issuecomment-306242",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:1'>Comment 1:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7058f361bd60c6bf742e8dd77df339c1f1102fd6">7058f36</a></td><td><code>getting rid of from string import join</code></td></tr></table>




---

archive/issue_comments_306243.json:
```json
{
    "body": "Branch: **[public/20909](https://github.com/sagemath/sagetrac-mirror/tree/public/20909)**",
    "created_at": "2016-06-30T11:42:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20909#issuecomment-306243",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[public/20909](https://github.com/sagemath/sagetrac-mirror/tree/public/20909)**



---

archive/issue_comments_306244.json:
```json
{
    "body": "Commit: **[7058f36](https://github.com/sagemath/sagetrac-mirror/commit/7058f361bd60c6bf742e8dd77df339c1f1102fd6)**",
    "created_at": "2016-06-30T11:42:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20909#issuecomment-306244",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[7058f36](https://github.com/sagemath/sagetrac-mirror/commit/7058f361bd60c6bf742e8dd77df339c1f1102fd6)**



---

archive/issue_events_276340.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-06-30T11:42:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20909",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20909#event-276340"
}
```



---

archive/issue_comments_306245.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a28c594d659445c663abe57958eefbb9b7db5963\">a28c594</a></td><td><code>one more string.join removed</code></td></tr></table>\n",
    "created_at": "2016-06-30T11:47:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20909#issuecomment-306245",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:2'>Comment 2:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a28c594d659445c663abe57958eefbb9b7db5963">a28c594</a></td><td><code>one more string.join removed</code></td></tr></table>




---

archive/issue_comments_306246.json:
```json
{
    "body": "Changed commit from **[7058f36](https://github.com/sagemath/sagetrac-mirror/commit/7058f361bd60c6bf742e8dd77df339c1f1102fd6)** to **[a28c594](https://github.com/sagemath/sagetrac-mirror/commit/a28c594d659445c663abe57958eefbb9b7db5963)**",
    "created_at": "2016-06-30T11:47:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20909#issuecomment-306246",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[7058f36](https://github.com/sagemath/sagetrac-mirror/commit/7058f361bd60c6bf742e8dd77df339c1f1102fd6)** to **[a28c594](https://github.com/sagemath/sagetrac-mirror/commit/a28c594d659445c663abe57958eefbb9b7db5963)**



---

archive/issue_comments_306247.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b270f8c96088c5679f62f79c0b7706230618ed42\">b270f8c</a></td><td><code>one more import join removed</code></td></tr></table>\n",
    "created_at": "2016-06-30T12:17:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20909#issuecomment-306247",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b270f8c96088c5679f62f79c0b7706230618ed42">b270f8c</a></td><td><code>one more import join removed</code></td></tr></table>




---

archive/issue_comments_306248.json:
```json
{
    "body": "Changed commit from **[a28c594](https://github.com/sagemath/sagetrac-mirror/commit/a28c594d659445c663abe57958eefbb9b7db5963)** to **[b270f8c](https://github.com/sagemath/sagetrac-mirror/commit/b270f8c96088c5679f62f79c0b7706230618ed42)**",
    "created_at": "2016-06-30T12:17:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20909#issuecomment-306248",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[a28c594](https://github.com/sagemath/sagetrac-mirror/commit/a28c594d659445c663abe57958eefbb9b7db5963)** to **[b270f8c](https://github.com/sagemath/sagetrac-mirror/commit/b270f8c96088c5679f62f79c0b7706230618ed42)**



---

archive/issue_comments_306249.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,4 +2,4 @@\n \n let us remove the few existing instances of \"from string import join\"\n \n-\n+one is in a script that is run early in sage installation\n``````\n",
    "created_at": "2016-06-30T12:38:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20909#issuecomment-306249",
    "user": "https://github.com/fchapoton"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,4 +2,4 @@
 
 let us remove the few existing instances of "from string import join"
 
-
+one is in a script that is run early in sage installation
``````




---

archive/issue_comments_306250.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nI think some of the code you updated was questionable in the first place.  But best for this to just focus on the `string.join` > `str.join` conversion and not tinker with anything else.",
    "created_at": "2016-06-30T14:55:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20909#issuecomment-306250",
    "user": "https://github.com/embray"
}
```

<a id='comment:6'>Comment 6:</a>
I think some of the code you updated was questionable in the first place.  But best for this to just focus on the `string.join` > `str.join` conversion and not tinker with anything else.



---

archive/issue_events_276341.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-06-30T14:55:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20909",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20909#event-276341"
}
```



---

archive/issue_events_276342.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2016-06-30T14:55:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20909",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20909#event-276342"
}
```



---

archive/issue_events_276343.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-06-30T18:08:34Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20909",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20909#event-276343"
}
```



---

archive/issue_events_276344.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-06-30T18:08:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20909",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20909#event-276344"
}
```



---

archive/issue_comments_306251.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nReviewer name missing",
    "created_at": "2016-06-30T18:08:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20909#issuecomment-306251",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:7'>Comment 7:</a>
Reviewer name missing



---

archive/issue_comments_306252.json:
```json
{
    "body": "Reviewer: **Erik Bray**",
    "created_at": "2016-06-30T19:34:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20909#issuecomment-306252",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Erik Bray**



---

archive/issue_events_276345.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-06-30T19:34:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20909",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20909#event-276345"
}
```



---

archive/issue_events_276346.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-06-30T19:34:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20909",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20909#event-276346"
}
```



---

archive/issue_comments_306253.json:
```json
{
    "body": "Changed branch from **[public/20909](https://github.com/sagemath/sagetrac-mirror/tree/public/20909)** to **[b270f8c](https://github.com/sagemath/sagetrac-mirror/commit/b270f8c96088c5679f62f79c0b7706230618ed42)**",
    "created_at": "2016-07-01T16:57:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20909",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20909#issuecomment-306253",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/20909](https://github.com/sagemath/sagetrac-mirror/tree/public/20909)** to **[b270f8c](https://github.com/sagemath/sagetrac-mirror/commit/b270f8c96088c5679f62f79c0b7706230618ed42)**



---

archive/issue_events_276347.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-07-01T16:57:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20909",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20909#event-276347"
}
```



---

archive/issue_events_276348.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "5ee9a22f69ae9de13e7bbd1714cdc6eba99bb698",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-07-01T16:57:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20909",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20909#event-276348"
}
```
