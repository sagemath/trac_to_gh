# Issue 20066: asymptotic expansion generators: fix precision parameter

archive/issues_019829.json:
```json
{
    "assignees": [],
    "body": "this should not happen:\n\n```\nsage: asymptotic_expansions.Binomial_kn_over_n('n', 2)\nTraceback (most recent call last):\n...\nTypeError: unsupported operand type(s) for -: 'NoneType' and 'int'\n```\n\nand\n\n```\nsage: asymptotic_expansions.Stirling('n')\nTraceback (most recent call last):\n...\nValueError: precision must be at least 3\n```\n\nCC:  @cheuberg @dkrenn\n\nBranch: **[884fa7f](https://github.com/sagemath/sagetrac-mirror/commit/884fa7f86a81e75704f18d7211b6efd23e82c58f)**\n\nReviewer: **Marc Mezzarobba**\n\nAuthor: **Benjamin Hackl**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20066_\n\n",
    "closed_at": "2021-12-23T20:26:14Z",
    "created_at": "2016-02-16T16:16:42Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20asymptotic%20expansions",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.5",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "asymptotic expansion generators: fix precision parameter",
    "type": "issue",
    "updated_at": "2021-12-24T00:17:54Z",
    "url": "https://github.com/sagemath/sage/issues/20066",
    "user": "https://github.com/behackl"
}
```
this should not happen:

```
sage: asymptotic_expansions.Binomial_kn_over_n('n', 2)
Traceback (most recent call last):
...
TypeError: unsupported operand type(s) for -: 'NoneType' and 'int'
```

and

```
sage: asymptotic_expansions.Stirling('n')
Traceback (most recent call last):
...
ValueError: precision must be at least 3
```

CC:  @cheuberg @dkrenn

Branch: **[884fa7f](https://github.com/sagemath/sagetrac-mirror/commit/884fa7f86a81e75704f18d7211b6efd23e82c58f)**

Reviewer: **Marc Mezzarobba**

Author: **Benjamin Hackl**

_Issue created by migration from https://trac.sagemath.org/ticket/20066_





---

archive/issue_events_265541.json:
```json
{
    "actor": "https://github.com/behackl",
    "created_at": "2016-02-16T16:16:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "milestone_number": null,
    "milestone_title": "sage-7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20066#event-265541"
}
```



---

archive/issue_events_265542.json:
```json
{
    "actor": "https://github.com/behackl",
    "created_at": "2016-02-16T16:16:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20asymptotic%20expansions",
    "label_color": "0000ff",
    "label_name": "component: asymptotic expansions",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20066#event-265542"
}
```



---

archive/issue_events_265543.json:
```json
{
    "actor": "https://github.com/behackl",
    "created_at": "2016-02-16T16:16:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20066#event-265543"
}
```



---

archive/issue_events_265544.json:
```json
{
    "actor": "https://github.com/behackl",
    "created_at": "2016-02-16T16:16:42Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20066#event-265544"
}
```



---

archive/issue_comments_290196.json:
```json
{
    "body": "Branch: **[u/behackl/asy/generators/precision-fix](https://github.com/sagemath/sagetrac-mirror/tree/u/behackl/asy/generators/precision-fix)**",
    "created_at": "2016-02-16T16:54:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20066#issuecomment-290196",
    "user": "https://github.com/behackl"
}
```

Branch: **[u/behackl/asy/generators/precision-fix](https://github.com/sagemath/sagetrac-mirror/tree/u/behackl/asy/generators/precision-fix)**



---

archive/issue_events_265545.json:
```json
{
    "actor": "https://github.com/behackl",
    "created_at": "2016-02-16T16:54:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20066#event-265545"
}
```



---

archive/issue_comments_290197.json:
```json
{
    "body": "Commit: **[e91eb65](https://github.com/sagemath/sagetrac-mirror/commit/e91eb655ffd3f6a11e6826f17e101b4512ae0816)**",
    "created_at": "2016-02-16T16:54:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20066#issuecomment-290197",
    "user": "https://github.com/behackl"
}
```

Commit: **[e91eb65](https://github.com/sagemath/sagetrac-mirror/commit/e91eb655ffd3f6a11e6826f17e101b4512ae0816)**



---

archive/issue_comments_290198.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nFor the Stirling generator, I couldn't get something along the lines of\n\n```\nif precision is not None and precision < 3:\n    raise ...\n```\nto work: while it works in Sage on the console, the doctest-framework wasn't happy with my tests. Suggestions are welcome.\n\n---\nLast 10 new commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/197fe4327389d8d7775bffec841bde9f98988662\">197fe43</a></td><td><code>Trac #20053: Merge #20056</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bbbfab97ac448dbf2cefac0d6c0b03cb841b3702\">bbbfab9</a></td><td><code>Trac #20053: Also rename method here</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/071a595899a62abcdd9069e2504c532e02fdde46\">071a595</a></td><td><code>Trac #20053: Refactor: new method AsymptoticRing._singularity_analysis_</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/2b2e94de2a541ccdd05132dad603260b70077a72\">2b2e94d</a></td><td><code>Trac #20053: minor cleanup in coefficients_of_generating_function</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7f8c745479c0a5fd7372f5bd9c6c05c5a644f648\">7f8c745</a></td><td><code>Trac #20053: add warning to coefficients_of_generating_function</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7acc4879e8992b286119e0ac6c0caf365032b9e5\">7acc487</a></td><td><code>Trac #20053: Merge branch #19540 to resolve merge conflict</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/1949e309c332aa8e4d99303442cdb40228528e1b\">1949e30</a></td><td><code>Merge branch 'asy/allow-singular-expansion' into t/17601/public/asy/trunk</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e8b7f0faad3dd23ff79c1ab6d12b46f8906cf67b\">e8b7f0f</a></td><td><code>fix precision for stirling generator</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3ba62583a488fe1e35b7909e420f4cc6b0d31425\">3ba6258</a></td><td><code>fix precision for binomial-generators</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e91eb655ffd3f6a11e6826f17e101b4512ae0816\">e91eb65</a></td><td><code>add doctests</code></td></tr></table>\n",
    "created_at": "2016-02-16T16:54:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20066#issuecomment-290198",
    "user": "https://github.com/behackl"
}
```

<a id='comment:1'>Comment 1:</a>
For the Stirling generator, I couldn't get something along the lines of

```
if precision is not None and precision < 3:
    raise ...
```
to work: while it works in Sage on the console, the doctest-framework wasn't happy with my tests. Suggestions are welcome.

---
Last 10 new commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/197fe4327389d8d7775bffec841bde9f98988662">197fe43</a></td><td><code>Trac #20053: Merge #20056</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bbbfab97ac448dbf2cefac0d6c0b03cb841b3702">bbbfab9</a></td><td><code>Trac #20053: Also rename method here</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/071a595899a62abcdd9069e2504c532e02fdde46">071a595</a></td><td><code>Trac #20053: Refactor: new method AsymptoticRing._singularity_analysis_</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/2b2e94de2a541ccdd05132dad603260b70077a72">2b2e94d</a></td><td><code>Trac #20053: minor cleanup in coefficients_of_generating_function</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7f8c745479c0a5fd7372f5bd9c6c05c5a644f648">7f8c745</a></td><td><code>Trac #20053: add warning to coefficients_of_generating_function</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7acc4879e8992b286119e0ac6c0caf365032b9e5">7acc487</a></td><td><code>Trac #20053: Merge branch #19540 to resolve merge conflict</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/1949e309c332aa8e4d99303442cdb40228528e1b">1949e30</a></td><td><code>Merge branch 'asy/allow-singular-expansion' into t/17601/public/asy/trunk</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e8b7f0faad3dd23ff79c1ab6d12b46f8906cf67b">e8b7f0f</a></td><td><code>fix precision for stirling generator</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3ba62583a488fe1e35b7909e420f4cc6b0d31425">3ba6258</a></td><td><code>fix precision for binomial-generators</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e91eb655ffd3f6a11e6826f17e101b4512ae0816">e91eb65</a></td><td><code>add doctests</code></td></tr></table>




---

archive/issue_comments_290199.json:
```json
{
    "body": "Author: **Benjamin Hackl**",
    "created_at": "2016-02-16T16:54:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20066#issuecomment-290199",
    "user": "https://github.com/behackl"
}
```

Author: **Benjamin Hackl**



---

archive/issue_comments_290200.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nI've mistakenly started this from `asy/trunk`, this has to be rebased to something else.",
    "created_at": "2016-02-17T11:13:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20066#issuecomment-290200",
    "user": "https://github.com/behackl"
}
```

<a id='comment:2'>Comment 2:</a>
I've mistakenly started this from `asy/trunk`, this has to be rebased to something else.



---

archive/issue_comments_290201.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\ndoes not merge (cf. patchbot).",
    "created_at": "2017-01-05T12:10:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20066#issuecomment-290201",
    "user": "https://github.com/cheuberg"
}
```

<a id='comment:3'>Comment 3:</a>
does not merge (cf. patchbot).



---

archive/issue_events_265546.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2017-01-05T12:10:26Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20066#event-265546"
}
```



---

archive/issue_events_265547.json:
```json
{
    "actor": "https://github.com/cheuberg",
    "created_at": "2017-01-05T12:10:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20066#event-265547"
}
```



---

archive/issue_comments_290202.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nPing ;)",
    "created_at": "2019-03-27T14:23:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20066#issuecomment-290202",
    "user": "https://github.com/dkrenn"
}
```

<a id='comment:4'>Comment 4:</a>
Ping ;)



---

archive/issue_comments_290203.json:
```json
{
    "body": "Changed commit from **[e91eb65](https://github.com/sagemath/sagetrac-mirror/commit/e91eb655ffd3f6a11e6826f17e101b4512ae0816)** to **[884fa7f](https://github.com/sagemath/sagetrac-mirror/commit/884fa7f86a81e75704f18d7211b6efd23e82c58f)**",
    "created_at": "2021-12-08T15:32:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20066#issuecomment-290203",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[e91eb65](https://github.com/sagemath/sagetrac-mirror/commit/e91eb655ffd3f6a11e6826f17e101b4512ae0816)** to **[884fa7f](https://github.com/sagemath/sagetrac-mirror/commit/884fa7f86a81e75704f18d7211b6efd23e82c58f)**



---

archive/issue_comments_290204.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/33ba3480b52ac79a15006bd746a5c2c588453437\">33ba348</a></td><td><code>make sure to handle the case where precision is None</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/884fa7f86a81e75704f18d7211b6efd23e82c58f\">884fa7f</a></td><td><code>added doctests</code></td></tr></table>\n",
    "created_at": "2021-12-08T15:32:07Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20066#issuecomment-290204",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:5'>Comment 5:</a>
Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/33ba3480b52ac79a15006bd746a5c2c588453437">33ba348</a></td><td><code>make sure to handle the case where precision is None</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/884fa7f86a81e75704f18d7211b6efd23e82c58f">884fa7f</a></td><td><code>added doctests</code></td></tr></table>




---

archive/issue_comments_290205.json:
```json
{
    "body": "<a id='comment:6'>Comment 6:</a>\nSeems like this was open for a while. :-)\n\nThrew away the old branch and redid this in a clean way. Should be rather easy to review.",
    "created_at": "2021-12-08T15:41:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20066#issuecomment-290205",
    "user": "https://github.com/behackl"
}
```

<a id='comment:6'>Comment 6:</a>
Seems like this was open for a while. :-)

Threw away the old branch and redid this in a clean way. Should be rather easy to review.



---

archive/issue_events_265548.json:
```json
{
    "actor": "https://github.com/behackl",
    "created_at": "2021-12-08T15:41:36Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "milestone_number": null,
    "milestone_title": "sage-7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20066#event-265548"
}
```



---

archive/issue_events_265549.json:
```json
{
    "actor": "https://github.com/behackl",
    "created_at": "2021-12-08T15:41:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20066#event-265549"
}
```



---

archive/issue_events_265550.json:
```json
{
    "actor": "https://github.com/behackl",
    "created_at": "2021-12-08T15:41:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20066#event-265550"
}
```



---

archive/issue_events_265551.json:
```json
{
    "actor": "https://github.com/behackl",
    "created_at": "2021-12-08T15:41:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20066#event-265551"
}
```



---

archive/issue_comments_290206.json:
```json
{
    "body": "<a id='comment:7'>Comment 7:</a>\nlgtm!",
    "created_at": "2021-12-15T06:50:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20066#issuecomment-290206",
    "user": "https://github.com/mezzarobba"
}
```

<a id='comment:7'>Comment 7:</a>
lgtm!



---

archive/issue_events_265552.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2021-12-15T06:50:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20066#event-265552"
}
```



---

archive/issue_events_265553.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2021-12-15T06:50:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20066#event-265553"
}
```



---

archive/issue_comments_290207.json:
```json
{
    "body": "Reviewer: **Marc Mezzarobba**",
    "created_at": "2021-12-15T06:50:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20066#issuecomment-290207",
    "user": "https://github.com/mezzarobba"
}
```

Reviewer: **Marc Mezzarobba**



---

archive/issue_events_265554.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2021-12-23T20:26:14Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20066#event-265554"
}
```



---

archive/issue_events_265555.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "576bdc76f6b9ac6c92cfe95ae532801f7fe837d1",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2021-12-23T20:26:14Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20066#event-265555"
}
```



---

archive/issue_comments_290208.json:
```json
{
    "body": "Changed branch from **[u/behackl/asy/generators/precision-fix](https://github.com/sagemath/sagetrac-mirror/tree/u/behackl/asy/generators/precision-fix)** to **[884fa7f](https://github.com/sagemath/sagetrac-mirror/commit/884fa7f86a81e75704f18d7211b6efd23e82c58f)**",
    "created_at": "2021-12-23T20:26:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20066#issuecomment-290208",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/behackl/asy/generators/precision-fix](https://github.com/sagemath/sagetrac-mirror/tree/u/behackl/asy/generators/precision-fix)** to **[884fa7f](https://github.com/sagemath/sagetrac-mirror/commit/884fa7f86a81e75704f18d7211b6efd23e82c58f)**



---

archive/issue_events_265556.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-24T00:17:54Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20066#event-265556"
}
```



---

archive/issue_events_265557.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-24T00:17:54Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20066#event-265557"
}
```



---

archive/issue_comments_290209.json:
```json
{
    "body": "Changed commit from **[884fa7f](https://github.com/sagemath/sagetrac-mirror/commit/884fa7f86a81e75704f18d7211b6efd23e82c58f)** to none",
    "created_at": "2021-12-24T00:17:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20066",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20066#issuecomment-290209",
    "user": "https://github.com/mkoeppe"
}
```

Changed commit from **[884fa7f](https://github.com/sagemath/sagetrac-mirror/commit/884fa7f86a81e75704f18d7211b6efd23e82c58f)** to none
