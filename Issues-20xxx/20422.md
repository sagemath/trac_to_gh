# Issue 20422: Fix conversions matrix -> AffineGroup

archive/issues_020185.json:
```json
{
    "assignees": [],
    "body": "Conversion from a matrix representing an affine transformation to an affine group element only works in dimension 2 (due to a hard-coded `2` in `__init__`). Conversion from a matrix group element to an affine group element also does not work.\n\nCC:  @vbraun\n\nBranch/Commit: **[d9f8bde](https://github.com/sagemath/sagetrac-mirror/commit/d9f8bdee1447c80a0ecda4e31ee7675d3832775e)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20422_\n\n",
    "closed_at": "2016-04-15T07:15:43Z",
    "created_at": "2016-04-11T13:27:19Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20group%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix conversions matrix -> AffineGroup",
    "type": "issue",
    "updated_at": "2016-04-15T07:15:43Z",
    "url": "https://github.com/sagemath/sage/issues/20422",
    "user": "https://github.com/jdemeyer"
}
```
Conversion from a matrix representing an affine transformation to an affine group element only works in dimension 2 (due to a hard-coded `2` in `__init__`). Conversion from a matrix group element to an affine group element also does not work.

CC:  @vbraun

Branch/Commit: **[d9f8bde](https://github.com/sagemath/sagetrac-mirror/commit/d9f8bdee1447c80a0ecda4e31ee7675d3832775e)**

Reviewer: **Frédéric Chapoton**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/20422_





---

archive/issue_events_270120.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-11T13:27:19Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20422",
    "milestone_number": null,
    "milestone_title": "sage-7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20422#event-270120"
}
```



---

archive/issue_events_270121.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-11T13:27:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20422",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20group%20theory",
    "label_color": "0000ff",
    "label_name": "component: group theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20422#event-270121"
}
```



---

archive/issue_events_270122.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-11T13:27:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20422",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20422#event-270122"
}
```



---

archive/issue_events_270123.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-11T13:27:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20422",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20422#event-270123"
}
```



---

archive/issue_comments_297463.json:
```json
{
    "body": "Branch: **[u/jdemeyer/ticket/20422](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/20422)**",
    "created_at": "2016-04-11T13:39:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20422",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20422#issuecomment-297463",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/ticket/20422](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/20422)**



---

archive/issue_comments_297464.json:
```json
{
    "body": "Commit: **[d9f8bde](https://github.com/sagemath/sagetrac-mirror/commit/d9f8bdee1447c80a0ecda4e31ee7675d3832775e)**",
    "created_at": "2016-04-11T13:49:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20422",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20422#issuecomment-297464",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[d9f8bde](https://github.com/sagemath/sagetrac-mirror/commit/d9f8bdee1447c80a0ecda4e31ee7675d3832775e)**



---

archive/issue_comments_297465.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-\n+Conversion from a matrix representing an affine transformation to an affine group element only works in dimension 2 (due to a hard-coded `2` in `__init__`). Conversion from a matrix group element to an affine group element also does not work.\n``````\n",
    "created_at": "2016-04-11T13:49:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20422",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20422#issuecomment-297465",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-
+Conversion from a matrix representing an affine transformation to an affine group element only works in dimension 2 (due to a hard-coded `2` in `__init__`). Conversion from a matrix group element to an affine group element also does not work.
``````




---

archive/issue_comments_297466.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d9f8bdee1447c80a0ecda4e31ee7675d3832775e\">d9f8bde</a></td><td><code>Fix conversions matrix -> AffineGroup</code></td></tr></table>\n",
    "created_at": "2016-04-11T13:49:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20422",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20422#issuecomment-297466",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d9f8bdee1447c80a0ecda4e31ee7675d3832775e">d9f8bde</a></td><td><code>Fix conversions matrix -> AffineGroup</code></td></tr></table>




---

archive/issue_events_270124.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-11T13:49:56Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20422",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20422#event-270124"
}
```



---

archive/issue_events_270125.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-04-13T18:40:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20422",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20422#event-270125"
}
```



---

archive/issue_events_270126.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-04-13T18:40:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20422",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20422#event-270126"
}
```



---

archive/issue_comments_297467.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nok, looks good to me",
    "created_at": "2016-04-13T18:40:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20422",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20422#issuecomment-297467",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:4" align="right">Comment 4</div>

ok, looks good to me



---

archive/issue_comments_297468.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2016-04-13T18:40:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20422",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20422#issuecomment-297468",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_297469.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nThanks!",
    "created_at": "2016-04-13T18:53:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20422",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20422#issuecomment-297469",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:6" align="right">Comment 6</div>

Thanks!



---

archive/issue_comments_297470.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/ticket/20422](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/20422)** to **[d9f8bde](https://github.com/sagemath/sagetrac-mirror/commit/d9f8bdee1447c80a0ecda4e31ee7675d3832775e)**",
    "created_at": "2016-04-15T07:15:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20422",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20422#issuecomment-297470",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/ticket/20422](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/20422)** to **[d9f8bde](https://github.com/sagemath/sagetrac-mirror/commit/d9f8bdee1447c80a0ecda4e31ee7675d3832775e)**



---

archive/issue_events_270127.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-04-15T07:15:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20422",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20422#event-270127"
}
```



---

archive/issue_events_270128.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "1c95a3647dd5680f31524070c212ec18d3cc189b",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-04-15T07:15:43Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20422",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20422#event-270128"
}
```
