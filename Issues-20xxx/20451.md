# Issue 20451: error in projective canonical height

archive/issues_020214.json:
```json
{
    "assignees": [],
    "body": "The error bound for canonical height is not working in all cases.\n\n```\nPS.<x,y>=ProjectiveSpace(1,QQ)\nH=Hom(PS,PS)\nf=H([16*x^2 -29*y^2,16*y^2]) #Poonen 3-cycle\nQ=PS([-1/4,1])\nQ.is_preperiodic(f)\n```\n\nis using error_bound = 0.01 and getting an error larger than 0.01\n\nAssignee: **@bhutz**\n\nCC:  @sagetrac-rlmiller\n\nBranch/Commit: **[b3212fa](https://github.com/sagemath/sagetrac-mirror/commit/b3212fa80172a6ef3b6b1c0cebe0cbe0c9a1737b)**\n\nReviewer: **Rebecca Lauren Miller**\n\nAuthor: **Ben Hutz**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20451_\n\n",
    "closed_at": "2016-05-01T16:30:21Z",
    "created_at": "2016-04-16T15:48:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "error in projective canonical height",
    "type": "issue",
    "updated_at": "2016-05-01T16:30:21Z",
    "url": "https://github.com/sagemath/sage/issues/20451",
    "user": "https://github.com/bhutz"
}
```
The error bound for canonical height is not working in all cases.

```
PS.<x,y>=ProjectiveSpace(1,QQ)
H=Hom(PS,PS)
f=H([16*x^2 -29*y^2,16*y^2]) #Poonen 3-cycle
Q=PS([-1/4,1])
Q.is_preperiodic(f)
```

is using error_bound = 0.01 and getting an error larger than 0.01

Assignee: **@bhutz**

CC:  @sagetrac-rlmiller

Branch/Commit: **[b3212fa](https://github.com/sagemath/sagetrac-mirror/commit/b3212fa80172a6ef3b6b1c0cebe0cbe0c9a1737b)**

Reviewer: **Rebecca Lauren Miller**

Author: **Ben Hutz**

_Issue created by migration from https://trac.sagemath.org/ticket/20451_





---

archive/issue_events_270436.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2016-04-16T15:48:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20451",
    "milestone_number": null,
    "milestone_title": "sage-7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20451#event-270436"
}
```



---

archive/issue_events_270437.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2016-04-16T15:48:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20451",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20algebraic%20geometry",
    "label_color": "0000ff",
    "label_name": "component: algebraic geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20451#event-270437"
}
```



---

archive/issue_events_270438.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2016-04-16T15:48:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20451",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20451#event-270438"
}
```



---

archive/issue_events_270439.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2016-04-16T15:48:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20451",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20451#event-270439"
}
```



---

archive/issue_comments_297857.json:
```json
{
    "body": "Branch: **[u/bhutz/ticket/20451](https://github.com/sagemath/sagetrac-mirror/tree/u/bhutz/ticket/20451)**",
    "created_at": "2016-04-16T15:52:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20451#issuecomment-297857",
    "user": "https://github.com/bhutz"
}
```

Branch: **[u/bhutz/ticket/20451](https://github.com/sagemath/sagetrac-mirror/tree/u/bhutz/ticket/20451)**



---

archive/issue_comments_297858.json:
```json
{
    "body": "Commit: **[b3212fa](https://github.com/sagemath/sagetrac-mirror/commit/b3212fa80172a6ef3b6b1c0cebe0cbe0c9a1737b)**",
    "created_at": "2016-04-16T15:52:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20451#issuecomment-297858",
    "user": "https://github.com/bhutz"
}
```

Commit: **[b3212fa](https://github.com/sagemath/sagetrac-mirror/commit/b3212fa80172a6ef3b6b1c0cebe0cbe0c9a1737b)**



---

archive/issue_comments_297859.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nthere were errors in two places. One with how resultant was called (and in resultant itself) with respect to normalization, and a problem with how the error bound was calculated.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b3212fa80172a6ef3b6b1c0cebe0cbe0c9a1737b\">b3212fa</a></td><td><code>20451: fix error_bound for projective canonical height</code></td></tr></table>\n",
    "created_at": "2016-04-16T15:52:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20451#issuecomment-297859",
    "user": "https://github.com/bhutz"
}
```

<div id="comment:1" align="right">Comment 1</div>

there were errors in two places. One with how resultant was called (and in resultant itself) with respect to normalization, and a problem with how the error bound was calculated.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b3212fa80172a6ef3b6b1c0cebe0cbe0c9a1737b">b3212fa</a></td><td><code>20451: fix error_bound for projective canonical height</code></td></tr></table>




---

archive/issue_events_270440.json:
```json
{
    "actor": "https://github.com/bhutz",
    "created_at": "2016-04-16T15:52:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20451",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20451#event-270440"
}
```



---

archive/issue_comments_297860.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLooks good to me!",
    "created_at": "2016-04-28T15:51:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20451#issuecomment-297860",
    "user": "https://github.com/sagetrac-rlmiller"
}
```

<div id="comment:3" align="right">Comment 3</div>

Looks good to me!



---

archive/issue_events_270441.json:
```json
{
    "actor": "https://github.com/sagetrac-rlmiller",
    "created_at": "2016-04-28T15:51:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20451",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20451#event-270441"
}
```



---

archive/issue_events_270442.json:
```json
{
    "actor": "https://github.com/sagetrac-rlmiller",
    "created_at": "2016-04-28T15:51:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20451",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20451#event-270442"
}
```



---

archive/issue_comments_297861.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nReviewer name is missing",
    "created_at": "2016-04-28T22:54:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20451#issuecomment-297861",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:4" align="right">Comment 4</div>

Reviewer name is missing



---

archive/issue_events_270443.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-04-28T22:54:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20451",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20451#event-270443"
}
```



---

archive/issue_events_270444.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-04-28T22:54:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20451",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20451#event-270444"
}
```



---

archive/issue_comments_297862.json:
```json
{
    "body": "Reviewer: **Rebecca Lauren Miller**",
    "created_at": "2016-04-29T17:35:44Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20451#issuecomment-297862",
    "user": "https://github.com/sagetrac-rlmiller"
}
```

Reviewer: **Rebecca Lauren Miller**



---

archive/issue_events_270445.json:
```json
{
    "actor": "https://github.com/sagetrac-rlmiller",
    "created_at": "2016-04-29T17:35:52Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20451",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20451#event-270445"
}
```



---

archive/issue_events_270446.json:
```json
{
    "actor": "https://github.com/sagetrac-rlmiller",
    "created_at": "2016-04-29T17:35:52Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20451",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20451#event-270446"
}
```



---

archive/issue_comments_297863.json:
```json
{
    "body": "Changed branch from **[u/bhutz/ticket/20451](https://github.com/sagemath/sagetrac-mirror/tree/u/bhutz/ticket/20451)** to **[b3212fa](https://github.com/sagemath/sagetrac-mirror/commit/b3212fa80172a6ef3b6b1c0cebe0cbe0c9a1737b)**",
    "created_at": "2016-05-01T16:30:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20451",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20451#issuecomment-297863",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/bhutz/ticket/20451](https://github.com/sagemath/sagetrac-mirror/tree/u/bhutz/ticket/20451)** to **[b3212fa](https://github.com/sagemath/sagetrac-mirror/commit/b3212fa80172a6ef3b6b1c0cebe0cbe0c9a1737b)**



---

archive/issue_events_270447.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-01T16:30:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20451",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20451#event-270447"
}
```



---

archive/issue_events_270448.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "328091486d9a43c5b9df493fe1a1efa42b87df31",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-05-01T16:30:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20451",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20451#event-270448"
}
```
