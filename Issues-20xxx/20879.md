# Issue 20879: Expose PPL's Not-Necessarily-Closed Polyhedra and Normaliz' Polyhedra with Open Facets using Sage Polyhedra

archive/issues_020642.json:
```json
{
    "assignees": [],
    "body": "PPL has NNC_Polyhedron.\nShould expose using Sage's Polyhedra classes.\n\nIn the context of #20877, NNC polyhedra could play the same role as half-open intervals do in the context of the new piecewise functions.\n\nNormaliz also has the feature of \"open facets\". \n\nDepends on #32089\nDepends on #32159\n\nCC:  @kcrisman @kliem @jplab @yuan-zhou\n\nComponent: **geometry**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20879_\n\n",
    "created_at": "2016-06-24T23:54:50Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/c%3A%20geometry",
        "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-9.8",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Expose PPL's Not-Necessarily-Closed Polyhedra and Normaliz' Polyhedra with Open Facets using Sage Polyhedra",
    "type": "issue",
    "updated_at": "2022-08-31T02:51:13Z",
    "url": "https://github.com/sagemath/sage/issues/20879",
    "user": "https://github.com/mkoeppe"
}
```
PPL has NNC_Polyhedron.
Should expose using Sage's Polyhedra classes.

In the context of #20877, NNC polyhedra could play the same role as half-open intervals do in the context of the new piecewise functions.

Normaliz also has the feature of "open facets". 

Depends on #32089
Depends on #32159

CC:  @kcrisman @kliem @jplab @yuan-zhou

Component: **geometry**

_Issue created by migration from https://trac.sagemath.org/ticket/20879_





---

archive/issue_events_295171.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2016-06-24T23:54:50Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20879",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20879#event-295171"
}
```



---

archive/issue_events_295172.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2016-06-24T23:54:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20879",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20879#event-295172"
}
```



---

archive/issue_events_295173.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2016-06-24T23:55:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20879",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20879#event-295173"
}
```



---

archive/issue_events_295174.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2016-06-24T23:55:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20879",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20geometry",
    "label_color": "0000ff",
    "label_name": "c: geometry",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20879#event-295174"
}
```



---

archive/issue_comments_302710.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,2 +1,4 @@\n PPL has NNC_Polyhedron.\n Should expose using Sage's Polyhedra classes.\n+\n+In the context of #20877, NNC polyhedra could play the same role as half-open intervals do in the context of the new piecewise functions.\n``````\n",
    "created_at": "2016-06-25T17:07:14Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20879#issuecomment-302710",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,2 +1,4 @@
 PPL has NNC_Polyhedron.
 Should expose using Sage's Polyhedra classes.
+
+In the context of #20877, NNC polyhedra could play the same role as half-open intervals do in the context of the new piecewise functions.
``````




---

archive/issue_comments_302711.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nAn alternative is #31743",
    "created_at": "2021-05-29T18:27:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20879#issuecomment-302711",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:3" align="right">comment:3</div>

An alternative is #31743



---

archive/issue_events_295175.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-07T20:56:40Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/20879",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20879#event-295175"
}
```



---

archive/issue_events_295176.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-07T20:56:40Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20879",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20879#event-295176"
}
```



---

archive/issue_comments_302712.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,3 +2,5 @@\n Should expose using Sage's Polyhedra classes.\n \n In the context of #20877, NNC polyhedra could play the same role as half-open intervals do in the context of the new piecewise functions.\n+\n+Normaliz also has the feature of \"open facets\". \n``````\n",
    "created_at": "2021-07-07T20:56:40Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20879#issuecomment-302712",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,3 +2,5 @@
 Should expose using Sage's Polyhedra classes.
 
 In the context of #20877, NNC polyhedra could play the same role as half-open intervals do in the context of the new piecewise functions.
+
+Normaliz also has the feature of "open facets". 
``````




---

archive/issue_events_295177.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-07T21:05:09Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/20879",
    "title_is": "Expose PPL's Not-Necessarily-Closed Polyhedra and Normaliz' Polyhedra with Open Facets using Sage Polyhedra",
    "title_was": "Expose PPL's Not-Necessarily-Closed Polyhedra using Sage Polyhedra",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20879#event-295177"
}
```



---

archive/issue_comments_302713.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">comment:6</div>\n\nA new Sage class somewhere between `ConvexSet_base + ConditionSet` and `Polyhedron` that is able to store convex sets given by finitely many, strict or non-strict linear constraints (not necessarily minimal) would be valuable. In contrast to `Polyhedron`, it would not insist on eagerly computing a minimal description by facet defining inequalities.",
    "created_at": "2021-07-07T21:10:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20879#issuecomment-302713",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:6" align="right">comment:6</div>

A new Sage class somewhere between `ConvexSet_base + ConditionSet` and `Polyhedron` that is able to store convex sets given by finitely many, strict or non-strict linear constraints (not necessarily minimal) would be valuable. In contrast to `Polyhedron`, it would not insist on eagerly computing a minimal description by facet defining inequalities.



---

archive/issue_comments_302714.json:
```json
{
    "body": "Dependencies: **#32089**",
    "created_at": "2021-07-07T21:10:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20879#issuecomment-302714",
    "user": "https://github.com/mkoeppe"
}
```

Dependencies: **#32089**



---

archive/issue_comments_302715.json:
```json
{
    "body": "Changed dependencies from **#32089** to **#32089, #32159**",
    "created_at": "2021-07-08T20:26:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20879",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20879#issuecomment-302715",
    "user": "https://github.com/mkoeppe"
}
```

Changed dependencies from **#32089** to **#32089, #32159**



---

archive/issue_events_295178.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:16:42Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/20879",
    "milestone_number": null,
    "milestone_title": "sage-9.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20879#event-295178"
}
```



---

archive/issue_events_295179.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-07-19T01:16:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20879",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20879#event-295179"
}
```



---

archive/issue_events_295180.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/20879",
    "milestone_number": null,
    "milestone_title": "sage-9.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20879#event-295180"
}
```



---

archive/issue_events_295181.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2021-12-14T02:04:49Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20879",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20879#event-295181"
}
```



---

archive/issue_events_295182.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T00:06:20Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/20879",
    "milestone_number": null,
    "milestone_title": "sage-9.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20879#event-295182"
}
```



---

archive/issue_events_295183.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-03-05T00:06:20Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20879",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20879#event-295183"
}
```



---

archive/issue_events_295184.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/20879",
    "milestone_number": null,
    "milestone_title": "sage-9.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20879#event-295184"
}
```



---

archive/issue_events_295185.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-08-31T02:51:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20879",
    "milestone_number": null,
    "milestone_title": "sage-9.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20879#event-295185"
}
```
