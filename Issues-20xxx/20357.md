# Issue 20357: Expose matroid union functionality to user

archive/issues_020120.json:
```json
{
    "assignees": [],
    "body": "Matroid union was implemented in `sage.matroids.union_matroid`. It would be nice to expose this functionality to users:\n\n1) as a method `Matroid.union()`\n2) by importing `MatroidUnion` in `sage.matroids.advanced`\n\nReviewer: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20357_\n\n",
    "closed_at": "2017-07-13T07:54:31Z",
    "created_at": "2016-04-04T19:11:44Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20matroid%20theory",
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Expose matroid union functionality to user",
    "type": "issue",
    "updated_at": "2017-07-13T07:54:31Z",
    "url": "https://github.com/sagemath/sage/issues/20357",
    "user": "https://github.com/sagetrac-Stefan"
}
```
Matroid union was implemented in `sage.matroids.union_matroid`. It would be nice to expose this functionality to users:

1) as a method `Matroid.union()`
2) by importing `MatroidUnion` in `sage.matroids.advanced`

Reviewer: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/20357_





---

archive/issue_events_269426.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2016-04-04T19:11:44Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20357",
    "milestone_number": null,
    "milestone_title": "sage-7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20357#event-269426"
}
```



---

archive/issue_events_269427.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2016-04-04T19:11:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20357",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20matroid%20theory",
    "label_color": "0000ff",
    "label_name": "component: matroid theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20357#event-269427"
}
```



---

archive/issue_events_269428.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2016-04-04T19:11:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20357",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20357#event-269428"
}
```



---

archive/issue_events_269429.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2016-04-04T19:11:44Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20357",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20357#event-269429"
}
```



---

archive/issue_comments_296296.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,4 @@\n-Matroid union was implemented in sage.matroids.union_matroid . It would be nice to expose this functionality to users as:\n+Matroid union was implemented in `sage.matroids.union_matroid`. It would be nice to expose this functionality to users:\n \n-1) a method Matroid.union()\n-2) By importing MatroidUnion in sage.matroids.advanced\n+1) as a method `Matroid.union()`\n+2) by importing `MatroidUnion` in `sage.matroids.advanced`\n``````\n",
    "created_at": "2016-04-04T22:26:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20357#issuecomment-296296",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,4 +1,4 @@
-Matroid union was implemented in sage.matroids.union_matroid . It would be nice to expose this functionality to users as:
+Matroid union was implemented in `sage.matroids.union_matroid`. It would be nice to expose this functionality to users:
 
-1) a method Matroid.union()
-2) By importing MatroidUnion in sage.matroids.advanced
+1) as a method `Matroid.union()`
+2) by importing `MatroidUnion` in `sage.matroids.advanced`
``````




---

archive/issue_comments_296297.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nThis ticket appears to overlap with #19069 \n\nThere is some discussion there about how to best categorize this and related functions.",
    "created_at": "2017-03-11T00:02:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20357#issuecomment-296297",
    "user": "https://github.com/sagetrac-zgershkoff"
}
```

<div id="comment:2" align="right">Comment 2</div>

This ticket appears to overlap with #19069 

There is some discussion there about how to best categorize this and related functions.



---

archive/issue_comments_296298.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nYou're right, this is a duplicate.",
    "created_at": "2017-03-16T02:38:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20357#issuecomment-296298",
    "user": "https://github.com/sagetrac-Stefan"
}
```

<div id="comment:3" align="right">Comment 3</div>

You're right, this is a duplicate.



---

archive/issue_events_269430.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2017-03-16T02:38:12Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/20357",
    "milestone_number": null,
    "milestone_title": "sage-7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20357#event-269430"
}
```



---

archive/issue_events_269431.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2017-03-16T02:38:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20357",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20357#event-269431"
}
```



---

archive/issue_events_269432.json:
```json
{
    "actor": "https://github.com/sagetrac-Stefan",
    "created_at": "2017-03-16T02:38:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20357",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20357#event-269432"
}
```



---

archive/issue_events_269433.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-03-16T14:09:41Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20357",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20357#event-269433"
}
```



---

archive/issue_comments_296299.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2017-03-16T14:09:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20357#issuecomment-296299",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_269434.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-03-16T14:09:54Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20357",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20357#event-269434"
}
```



---

archive/issue_events_269435.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-03-16T14:09:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20357",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20357#event-269435"
}
```



---

archive/issue_comments_296300.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nConcur.",
    "created_at": "2017-03-16T14:09:54Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20357#issuecomment-296300",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:5" align="right">Comment 5</div>

Concur.



---

archive/issue_comments_296301.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nClosing tickets in the sage-duplicate/invalid/wontfix module with positive_review (i.e. someone has confirmed they should be closed).",
    "created_at": "2017-07-13T07:54:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20357",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20357#issuecomment-296301",
    "user": "https://github.com/embray"
}
```

<div id="comment:6" align="right">Comment 6</div>

Closing tickets in the sage-duplicate/invalid/wontfix module with positive_review (i.e. someone has confirmed they should be closed).



---

archive/issue_events_269436.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-07-13T07:54:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20357",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20357#event-269436"
}
```



---

archive/issue_events_269437.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-07-13T07:54:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20357",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20357#event-269437"
}
```



---

archive/issue_events_269438.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2017-07-13T07:54:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20357",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20357#event-269438"
}
```
