# Issue 20029: Implement quantum matrix coordinate algebras

archive/issues_019792.json:
```json
{
    "body": "Assignee: @tscrim\n\nCC:  @dwbump buciumas\n\nKeywords: quantum, coordinate ring\n\nWe provide a basic implementation of quantum matrix coordinate algebras.\n\nIssue created by migration from https://trac.sagemath.org/ticket/20029\n\n",
    "closed_at": "2017-06-22T07:24:48Z",
    "created_at": "2016-02-09T21:58:59Z",
    "labels": [
        "component: algebra"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.0",
    "title": "Implement quantum matrix coordinate algebras",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/20029",
    "user": "https://github.com/tscrim"
}
```
Assignee: @tscrim

CC:  @dwbump buciumas

Keywords: quantum, coordinate ring

We provide a basic implementation of quantum matrix coordinate algebras.

Issue created by migration from https://trac.sagemath.org/ticket/20029





---

archive/issue_comments_271723.json:
```json
{
    "body": "New commits:",
    "created_at": "2016-02-09T21:59:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271723",
    "user": "https://github.com/tscrim"
}
```

New commits:



---

archive/issue_comments_271724.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2016-02-09T21:59:33Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271724",
    "user": "https://github.com/tscrim"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_271725.json:
```json
{
    "body": "This bialgebra (if m=n) has a natural comodule, namely the quantum affine space.\n\nWould it be hard to implement that, too? It seems plausible that this could be useful.",
    "created_at": "2016-02-23T00:57:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271725",
    "user": "https://github.com/dwbump"
}
```

This bialgebra (if m=n) has a natural comodule, namely the quantum affine space.

Would it be hard to implement that, too? It seems plausible that this could be useful.



---

archive/issue_comments_271726.json:
```json
{
    "body": "Perhaps it would be good to add a reference to Faddeev, Reshetikhin and Takhtajan, Quantization of Lie Groups and Lie Algebras, Leningrad Math. J. vol. 1 no. 1 (1990).\n\nThe quantum determinant is described in this paper and there are also bialgebras/Hopf algebras constructed for the other classical types.",
    "created_at": "2016-02-23T01:15:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271726",
    "user": "https://github.com/dwbump"
}
```

Perhaps it would be good to add a reference to Faddeev, Reshetikhin and Takhtajan, Quantization of Lie Groups and Lie Algebras, Leningrad Math. J. vol. 1 no. 1 (1990).

The quantum determinant is described in this paper and there are also bialgebras/Hopf algebras constructed for the other classical types.



---

archive/issue_comments_271727.json:
```json
{
    "body": "I looked over the document and it looks good!\n\nI would suggest adding a note saying what q-convention you are using. For example the q squared in your document would be just a q in Chari and Pressley or in Kassel (or FRT for that matter). I think this would save some confusion for people that don't now about this.\n\nCertain things that could be implemented going forward could be an antipode map for GL_q(n) and maybe the same construction for other classical types (see FRT). The quantum determinant is then replaced by the quantum similitude.",
    "created_at": "2016-02-23T02:03:37Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271727",
    "user": "https://trac.sagemath.org/admin/accounts/users/buciumas"
}
```

I looked over the document and it looks good!

I would suggest adding a note saying what q-convention you are using. For example the q squared in your document would be just a q in Chari and Pressley or in Kassel (or FRT for that matter). I think this would save some confusion for people that don't now about this.

Certain things that could be implemented going forward could be an antipode map for GL_q(n) and maybe the same construction for other classical types (see FRT). The quantum determinant is then replaced by the quantum similitude.



---

archive/issue_comments_271728.json:
```json
{
    "body": "Thank you both for looking at this!\n\nIt's not hard to implement the quantum affine space, but I don't think we currently have a framework for comodules. We do have the basic infrastructure to do this though. However, I agree that this would be a good thing to implement; perhaps on a follow up ticket.\n\nI will add the reference and a note about the q-convention. Does the antipode extend to GL<sub>q</sub>(n)? From FRT (well, at least [Ram's version](http://www.ms.unimelb.edu.au/~ram/Resources/Reshetikhin/QuantizationOfLieGroupsAndLieAlgebras.html)), it is only given for SL<sub>q</sub>(n). Just to let you know, one of my main motivations for implementing this was to look at the crystal basis (see http://arxiv.org/abs/math/0509651).",
    "created_at": "2016-02-23T06:11:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271728",
    "user": "https://github.com/tscrim"
}
```

Thank you both for looking at this!

It's not hard to implement the quantum affine space, but I don't think we currently have a framework for comodules. We do have the basic infrastructure to do this though. However, I agree that this would be a good thing to implement; perhaps on a follow up ticket.

I will add the reference and a note about the q-convention. Does the antipode extend to GL<sub>q</sub>(n)? From FRT (well, at least [Ram's version](http://www.ms.unimelb.edu.au/~ram/Resources/Reshetikhin/QuantizationOfLieGroupsAndLieAlgebras.html)), it is only given for SL<sub>q</sub>(n). Just to let you know, one of my main motivations for implementing this was to look at the crystal basis (see http://arxiv.org/abs/math/0509651).



---

archive/issue_comments_271729.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2016-02-27T06:18:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271729",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_271730.json:
```json
{
    "body": "I've added the FRT reference and the note about the q convention, and I also had come across a small bug with how I was implementing the unique representation behavior that I fixed (and now it takes more general input too). I also added commented code for the antipode once we implement SL<sub>q</sub>(n) (well, really, quantum Gr\u00f6bner bases). However, considering the antipode is coming from \"q-inverting the matrix\", I don't think we get an antipode map for generic q and the quantum determinant is not an invertible element in the algebra (in general). Or is there a different Hopf structure/antipode?",
    "created_at": "2016-02-27T06:23:54Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271730",
    "user": "https://github.com/tscrim"
}
```

I've added the FRT reference and the note about the q convention, and I also had come across a small bug with how I was implementing the unique representation behavior that I fixed (and now it takes more general input too). I also added commented code for the antipode once we implement SL<sub>q</sub>(n) (well, really, quantum Gröbner bases). However, considering the antipode is coming from "q-inverting the matrix", I don't think we get an antipode map for generic q and the quantum determinant is not an invertible element in the algebra (in general). Or is there a different Hopf structure/antipode?



---

archive/issue_comments_271731.json:
```json
{
    "body": "The FRT version at the above web page does not seem to be the whole paper. Both `SL_q(n)` and `GL_q(n)` have antipodes. For `GL_q(n)` the q-determinant (needed for the antipode) is invertible, and for `SL_q(n)` the q-determinant is 1.",
    "created_at": "2016-03-03T00:43:55Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271731",
    "user": "https://github.com/dwbump"
}
```

The FRT version at the above web page does not seem to be the whole paper. Both `SL_q(n)` and `GL_q(n)` have antipodes. For `GL_q(n)` the q-determinant (needed for the antipode) is invertible, and for `SL_q(n)` the q-determinant is 1.



---

archive/issue_comments_271732.json:
```json
{
    "body": "Can you tell me a formula for the inverse of the q-determinant for `GL_q(n)` is or point me to a reference which has it?\n\nAddendum - At least the FRT article is not on MathSciNet; just a description/reference information.",
    "created_at": "2016-03-03T00:55:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271732",
    "user": "https://github.com/tscrim"
}
```

Can you tell me a formula for the inverse of the q-determinant for `GL_q(n)` is or point me to a reference which has it?

Addendum - At least the FRT article is not on MathSciNet; just a description/reference information.



---

archive/issue_comments_271733.json:
```json
{
    "body": "I don't think there is a formula for the inverse of the q-determinant in GL_q(n). I think GL_q(n) is defined as the bialgebra generated by the regular generators plus a generator c that is central mod the relation c * qdet = 1. So you can write the antipode in GL_q(n) as a sum of products of the x_{ij}'s and multiply everything by the inverse of the qdet. In the case of SL_q(n) qdet is set to 1. In the case of the quantum coordinate ring which you implemented you can't write a formula for the antipode because qdet is not invertible or equal to 1. \n\nFormulas for the antipode and qdet (which I think you already know) for SL_q(n) can be found (among many places) here : http://arxiv.org/abs/1602.04262 (page 16 last formula and page 17 first formula).",
    "created_at": "2016-03-03T02:31:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271733",
    "user": "https://trac.sagemath.org/admin/accounts/users/buciumas"
}
```

I don't think there is a formula for the inverse of the q-determinant in GL_q(n). I think GL_q(n) is defined as the bialgebra generated by the regular generators plus a generator c that is central mod the relation c * qdet = 1. So you can write the antipode in GL_q(n) as a sum of products of the x_{ij}'s and multiply everything by the inverse of the qdet. In the case of SL_q(n) qdet is set to 1. In the case of the quantum coordinate ring which you implemented you can't write a formula for the antipode because qdet is not invertible or equal to 1. 

Formulas for the antipode and qdet (which I think you already know) for SL_q(n) can be found (among many places) here : http://arxiv.org/abs/1602.04262 (page 16 last formula and page 17 first formula).



---

archive/issue_comments_271734.json:
```json
{
    "body": "Thank you. I will see if I can implement this over the weekend (I'm flying to Seoul tomorrow).",
    "created_at": "2016-03-03T07:03:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271734",
    "user": "https://github.com/tscrim"
}
```

Thank you. I will see if I can implement this over the weekend (I'm flying to Seoul tomorrow).



---

archive/issue_comments_271735.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2016-03-06T07:50:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271735",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_271736.json:
```json
{
    "body": "I've added a class for GL<sub>q</sub>(n), but it currently fails the antipode test. I've also changed the convention from `q^2` to `q`. I just ran another test and it says that it is failing associativity, so the bug is probably in the multiplication code.\n\nHowever, can you check to make sure the following data is correct?\n\n```\nsage: O = algebras.QuantumGL(2)\nsage: O.quantum_determinant()\nx[1,1]*x[2,2] - q*x[1,2]*x[2,1]\nsage: for g in O.algebra_generators():\n....:     print \"{:>8}{:>33}{:>35}{:>2}\".format(g, g.antipode(),\n....:                                           g.coproduct(), g.counit())\n  x[1,1]                         c*x[2,2]  x[1,1] # x[1,1] + x[1,2] # x[2,1] 1\n  x[1,2]                 -(q^-1)*c*x[2,1]  x[1,1] # x[1,2] + x[1,2] # x[2,2] 0\n  x[2,1]                      -q*c*x[1,2]  x[2,1] # x[1,1] + x[2,2] # x[2,1] 0\n  x[2,2]                         c*x[1,1]  x[2,1] # x[1,2] + x[2,2] # x[2,2] 1\n       c  x[1,1]*x[2,2] - q*x[1,2]*x[2,1]                              c # c 1\n```\nThis agrees with Valentin's paper if `c = 1`.",
    "created_at": "2016-03-06T07:59:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271736",
    "user": "https://github.com/tscrim"
}
```

I've added a class for GL<sub>q</sub>(n), but it currently fails the antipode test. I've also changed the convention from `q^2` to `q`. I just ran another test and it says that it is failing associativity, so the bug is probably in the multiplication code.

However, can you check to make sure the following data is correct?

```
sage: O = algebras.QuantumGL(2)
sage: O.quantum_determinant()
x[1,1]*x[2,2] - q*x[1,2]*x[2,1]
sage: for g in O.algebra_generators():
....:     print "{:>8}{:>33}{:>35}{:>2}".format(g, g.antipode(),
....:                                           g.coproduct(), g.counit())
  x[1,1]                         c*x[2,2]  x[1,1] # x[1,1] + x[1,2] # x[2,1] 1
  x[1,2]                 -(q^-1)*c*x[2,1]  x[1,1] # x[1,2] + x[1,2] # x[2,2] 0
  x[2,1]                      -q*c*x[1,2]  x[2,1] # x[1,1] + x[2,2] # x[2,1] 0
  x[2,2]                         c*x[1,1]  x[2,1] # x[1,2] + x[2,2] # x[2,2] 1
       c  x[1,1]*x[2,2] - q*x[1,2]*x[2,1]                              c # c 1
```
This agrees with Valentin's paper if `c = 1`.



---

archive/issue_comments_271737.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2016-03-06T07:59:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271737",
    "user": "https://github.com/tscrim"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_271738.json:
```json
{
    "body": "I know what the problem is: I need to do some commuting in order to do the cancellation I want to do.",
    "created_at": "2016-03-06T08:13:13Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271738",
    "user": "https://github.com/tscrim"
}
```

I know what the problem is: I need to do some commuting in order to do the cancellation I want to do.



---

archive/issue_comments_271739.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2016-03-06T09:12:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271739",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_271740.json:
```json
{
    "body": "Okay, I fixed the issue with multiplication not be associative by a slightly hacky Gr\u00f6bner basis like process, but I left a little outline/proof of the method as a comment. I also forgot that it is the *transpose* of the cofactor matrix when doing the inverse (and hence the antipode).\n\nNow just to do a little bit of cleanup and get back to full doctest coverage.",
    "created_at": "2016-03-06T09:15:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271740",
    "user": "https://github.com/tscrim"
}
```

Okay, I fixed the issue with multiplication not be associative by a slightly hacky Gröbner basis like process, but I left a little outline/proof of the method as a comment. I also forgot that it is the *transpose* of the cofactor matrix when doing the inverse (and hence the antipode).

Now just to do a little bit of cleanup and get back to full doctest coverage.



---

archive/issue_comments_271741.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2016-03-06T11:34:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271741",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_271742.json:
```json
{
    "body": "Back to needing review.",
    "created_at": "2016-03-06T11:35:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271742",
    "user": "https://github.com/tscrim"
}
```

Back to needing review.



---

archive/issue_comments_271743.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2016-03-06T11:35:57Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271743",
    "user": "https://github.com/tscrim"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_271744.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2016-03-11T00:15:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271744",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_271745.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2016-04-29T18:46:14Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271745",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_271746.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2016-11-20T19:52:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271746",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_271747.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2016-12-14T20:18:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271747",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_271748.json:
```json
{
    "body": "doctest failures (cf. patchbot).",
    "created_at": "2017-01-05T14:15:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271748",
    "user": "https://github.com/cheuberg"
}
```

doctest failures (cf. patchbot).



---

archive/issue_comments_271749.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2017-01-05T14:15:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271749",
    "user": "https://github.com/cheuberg"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_271750.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2017-01-08T05:40:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271750",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_events_054599.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-01-08T05:41:45Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "milestone": "sage-7.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20029#event-54599"
}
```



---

archive/issue_comments_271751.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2017-01-08T05:41:45Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271751",
    "user": "https://github.com/tscrim"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_271752.json:
```json
{
    "body": "could you remove gcmp, which seems not to be used anywhere ?",
    "created_at": "2017-01-09T09:16:53Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271752",
    "user": "https://github.com/fchapoton"
}
```

could you remove gcmp, which seems not to be used anywhere ?



---

archive/issue_comments_271753.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2017-01-10T07:52:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271753",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_271754.json:
```json
{
    "body": "I've removed it, and instead I replaced it with a key for sorting since I don't think there is a guaranteed sorting order between strings and tuples.",
    "created_at": "2017-01-10T07:53:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271754",
    "user": "https://github.com/tscrim"
}
```

I've removed it, and instead I replaced it with a key for sorting since I don't think there is a guaranteed sorting order between strings and tuples.



---

archive/issue_comments_271755.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2017-01-26T18:15:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271755",
    "user": "https://github.com/fchapoton"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_271756.json:
```json
{
    "body": "does not apply",
    "created_at": "2017-01-26T18:15:20Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271756",
    "user": "https://github.com/fchapoton"
}
```

does not apply



---

archive/issue_comments_271757.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2017-01-26T18:43:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271757",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_271758.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2017-01-26T18:43:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271758",
    "user": "https://github.com/tscrim"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_271759.json:
```json
{
    "body": "Changing status from needs_review to needs_work.",
    "created_at": "2017-06-17T05:59:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271759",
    "user": "https://github.com/fchapoton"
}
```

Changing status from needs_review to needs_work.



---

archive/issue_comments_271760.json:
```json
{
    "body": "does not apply",
    "created_at": "2017-06-17T05:59:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271760",
    "user": "https://github.com/fchapoton"
}
```

does not apply



---

archive/issue_comments_271761.json:
```json
{
    "body": "Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2017-06-17T23:04:29Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271761",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_271762.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2017-06-17T23:05:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271762",
    "user": "https://github.com/tscrim"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_events_054600.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-06-17T23:05:07Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "milestone": "sage-7.6",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20029#event-54600"
}
```



---

archive/issue_events_054601.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2017-06-17T23:05:07Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "milestone": "sage-8.0",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20029#event-54601"
}
```



---

archive/issue_comments_271763.json:
```json
{
    "body": "ok, let it be",
    "created_at": "2017-06-18T14:32:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271763",
    "user": "https://github.com/fchapoton"
}
```

ok, let it be



---

archive/issue_comments_271764.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2017-06-18T14:32:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271764",
    "user": "https://github.com/fchapoton"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_271765.json:
```json
{
    "body": "Thank you.",
    "created_at": "2017-06-18T14:44:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271765",
    "user": "https://github.com/tscrim"
}
```

Thank you.



---

archive/issue_comments_271766.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2017-06-22T07:24:48Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20029#issuecomment-271766",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_events_054602.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-06-22T07:24:48Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/20029",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20029#event-54602"
}
```
