# Issue 20574: Reproducible documentation build (2)

archive/issues_020337.json:
```json
{
    "assignees": [],
    "body": "Since recently, we are using more of the upstream version of `autodoc`. This also has some code to remove hexadecimal address (see #19858) but the regular expression is too strict. This can easily be fixed, see https://github.com/sphinx-doc/sphinx/pull/2534.\n\nCC:  @hivert\n\nBranch/Commit: **[d01305a](https://github.com/sagemath/sagetrac-mirror/commit/d01305a13b795f749c2c4902db72d65e8b23e20f)**\n\nUpstream: **Fixed upstream, but not in a stable release.**\n\nReviewer: **Fran\u00e7ois Bissey**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20574_\n\n",
    "closed_at": "2016-06-11T22:28:22Z",
    "created_at": "2016-05-09T12:25:09Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Reproducible documentation build (2)",
    "type": "issue",
    "updated_at": "2016-06-11T22:28:22Z",
    "url": "https://github.com/sagemath/sage/issues/20574",
    "user": "https://github.com/jdemeyer"
}
```
Since recently, we are using more of the upstream version of `autodoc`. This also has some code to remove hexadecimal address (see #19858) but the regular expression is too strict. This can easily be fixed, see https://github.com/sphinx-doc/sphinx/pull/2534.

CC:  @hivert

Branch/Commit: **[d01305a](https://github.com/sagemath/sagetrac-mirror/commit/d01305a13b795f749c2c4902db72d65e8b23e20f)**

Upstream: **Fixed upstream, but not in a stable release.**

Reviewer: **François Bissey**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/20574_





---

archive/issue_events_271973.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-05-09T12:25:09Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20574",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20574#event-271973"
}
```



---

archive/issue_events_271974.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-05-09T12:25:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20574",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20574#event-271974"
}
```



---

archive/issue_events_271975.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-05-09T12:25:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20574",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20574#event-271975"
}
```



---

archive/issue_events_271976.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-05-09T12:25:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20574",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20574#event-271976"
}
```



---

archive/issue_comments_300042.json:
```json
{
    "body": "Branch: **[u/jdemeyer/ticket/20574](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/20574)**",
    "created_at": "2016-05-09T12:32:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20574",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20574#issuecomment-300042",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/ticket/20574](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/20574)**



---

archive/issue_comments_300043.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/d01305a13b795f749c2c4902db72d65e8b23e20f\">d01305a</a></td><td><code>Remove all memory addresses from built documentation</code></td></tr></table>\n",
    "created_at": "2016-05-09T12:34:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20574",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20574#issuecomment-300043",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/d01305a13b795f749c2c4902db72d65e8b23e20f">d01305a</a></td><td><code>Remove all memory addresses from built documentation</code></td></tr></table>




---

archive/issue_comments_300044.json:
```json
{
    "body": "Commit: **[d01305a](https://github.com/sagemath/sagetrac-mirror/commit/d01305a13b795f749c2c4902db72d65e8b23e20f)**",
    "created_at": "2016-05-09T12:34:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20574",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20574#issuecomment-300044",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[d01305a](https://github.com/sagemath/sagetrac-mirror/commit/d01305a13b795f749c2c4902db72d65e8b23e20f)**



---

archive/issue_events_271977.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-05-09T12:34:17Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20574",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20574#event-271977"
}
```



---

archive/issue_comments_300045.json:
```json
{
    "body": "Changed upstream from **Reported upstream. No feedback yet.** to **Fixed upstream, but not in a stable release.**",
    "created_at": "2016-06-05T15:00:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20574",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20574#issuecomment-300045",
    "user": "https://github.com/jdemeyer"
}
```

Changed upstream from **Reported upstream. No feedback yet.** to **Fixed upstream, but not in a stable release.**



---

archive/issue_comments_300046.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nIf it is good enough for upstream it is good enough for sage.",
    "created_at": "2016-06-07T23:26:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20574",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20574#issuecomment-300046",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:4" align="right">Comment 4</div>

If it is good enough for upstream it is good enough for sage.



---

archive/issue_comments_300047.json:
```json
{
    "body": "Reviewer: **Fran\u00e7ois Bissey**",
    "created_at": "2016-06-07T23:26:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20574",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20574#issuecomment-300047",
    "user": "https://github.com/kiwifb"
}
```

Reviewer: **François Bissey**



---

archive/issue_comments_300048.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nFor the record - no `sphinx` update to 1.4.2 or 1.4.3, some changes have been made to latex generation upstream and it is a bit of a mess. I and someone from scikit-learn are involved in trying to fix it. On the positive side we should be able to drop a patch after the next release.",
    "created_at": "2016-06-09T01:54:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20574",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20574#issuecomment-300048",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:5" align="right">Comment 5</div>

For the record - no `sphinx` update to 1.4.2 or 1.4.3, some changes have been made to latex generation upstream and it is a bit of a mess. I and someone from scikit-learn are involved in trying to fix it. On the positive side we should be able to drop a patch after the next release.



---

archive/issue_comments_300049.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nReplying to [@kiwifb](#comment%3A4):\n> If it is good enough for upstream it is good enough for sage.\n\nWouldn't that imply positive_review? :-)",
    "created_at": "2016-06-10T18:52:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20574",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20574#issuecomment-300049",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:6" align="right">Comment 6</div>

Replying to [@kiwifb](#comment%3A4):
> If it is good enough for upstream it is good enough for sage.

Wouldn't that imply positive_review? :-)



---

archive/issue_events_271978.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-06-10T19:50:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20574",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20574#event-271978"
}
```



---

archive/issue_events_271979.json:
```json
{
    "actor": "https://github.com/kiwifb",
    "created_at": "2016-06-10T19:50:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20574",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20574#event-271979"
}
```



---

archive/issue_comments_300050.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nI was sure I had done it.",
    "created_at": "2016-06-10T19:50:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20574",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20574#issuecomment-300050",
    "user": "https://github.com/kiwifb"
}
```

<div id="comment:7" align="right">Comment 7</div>

I was sure I had done it.



---

archive/issue_events_271980.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-06-11T22:28:22Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20574",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20574#event-271980"
}
```



---

archive/issue_events_271981.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "16bd070b7c436a6a31eedc00627fa5182f648aed",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-06-11T22:28:22Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20574",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20574#event-271981"
}
```



---

archive/issue_comments_300051.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/ticket/20574](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/20574)** to **[d01305a](https://github.com/sagemath/sagetrac-mirror/commit/d01305a13b795f749c2c4902db72d65e8b23e20f)**",
    "created_at": "2016-06-11T22:28:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20574",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20574#issuecomment-300051",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/ticket/20574](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/20574)** to **[d01305a](https://github.com/sagemath/sagetrac-mirror/commit/d01305a13b795f749c2c4902db72d65e8b23e20f)**
