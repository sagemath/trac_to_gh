# Issue 20838: Better error message if libhomfly not installed

archive/issues_020601.json:
```json
{
    "assignees": [],
    "body": "When I do \n\n```\nL.homfly_polynomial()\n```\nI just get \n\n```\nImportError: No module named homfly\n ```\nThat should suggest to install the optional package, I think.\n\nCC:  @miguelmarco @tscrim @slel\n\nReviewer: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20838_\n\n",
    "closed_at": "2020-09-28T22:38:03Z",
    "created_at": "2016-06-16T17:41:35Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
        "https://github.com/sagemath/sage/labels/invalid"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Better error message if libhomfly not installed",
    "type": "issue",
    "updated_at": "2020-09-28T22:38:03Z",
    "url": "https://github.com/sagemath/sage/issues/20838",
    "user": "https://github.com/kcrisman"
}
```
When I do 

```
L.homfly_polynomial()
```
I just get 

```
ImportError: No module named homfly
 ```
That should suggest to install the optional package, I think.

CC:  @miguelmarco @tscrim @slel

Reviewer: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/20838_





---

archive/issue_events_275361.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2016-06-16T17:41:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20838",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20838#event-275361"
}
```



---

archive/issue_events_275362.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2016-06-16T17:41:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20838",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20optional",
    "label_color": "000080",
    "label_name": "component: packages: optional",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20838#event-275362"
}
```



---

archive/issue_events_275363.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2016-06-16T17:41:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20838",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20838#event-275363"
}
```



---

archive/issue_events_275364.json:
```json
{
    "actor": "https://github.com/kcrisman",
    "created_at": "2016-06-16T17:41:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20838",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20838#event-275364"
}
```



---

archive/issue_comments_304835.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nFixed by making libhomfly a standard package in\n\n- #25705: Make libhomfly and libbraiding standard packages\n\n(merged in Sage 8.4.beta0).\n\nProposing to close as invalid.",
    "created_at": "2020-09-28T13:50:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20838",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20838#issuecomment-304835",
    "user": "https://github.com/slel"
}
```

<a id='comment:2'>Comment 2:</a>
Fixed by making libhomfly a standard package in

- #25705: Make libhomfly and libbraiding standard packages

(merged in Sage 8.4.beta0).

Proposing to close as invalid.



---

archive/issue_comments_304836.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -6,6 +6,6 @@\n I just get \n \n ```\n-ImportError: No module named homily\n+ImportError: No module named homfly\n  ```\n-That should give me a message to do the optional package instead, I think.\n+That should suggest to install the optional package, I think.\n``````\n",
    "created_at": "2020-09-28T13:50:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20838",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20838#issuecomment-304836",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -6,6 +6,6 @@
 I just get 
 
 ```
-ImportError: No module named homily
+ImportError: No module named homfly
  ```
-That should give me a message to do the optional package instead, I think.
+That should suggest to install the optional package, I think.
``````




---

archive/issue_events_275365.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-09-28T13:50:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20838",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20838#event-275365"
}
```



---

archive/issue_events_275366.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-09-28T13:50:48Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/20838",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20838#event-275366"
}
```



---

archive/issue_events_275367.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-09-28T13:50:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20838",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20838#event-275367"
}
```



---

archive/issue_events_275368.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-09-28T13:50:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20838",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20838#event-275368"
}
```



---

archive/issue_comments_304837.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2020-09-28T22:27:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20838",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20838#issuecomment-304837",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_304838.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nAgreed.",
    "created_at": "2020-09-28T22:27:20Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20838",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20838#issuecomment-304838",
    "user": "https://github.com/tscrim"
}
```

<a id='comment:3'>Comment 3:</a>
Agreed.



---

archive/issue_events_275369.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-09-28T22:27:20Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20838",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20838#event-275369"
}
```



---

archive/issue_events_275370.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2020-09-28T22:27:20Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20838",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20838#event-275370"
}
```



---

archive/issue_events_275371.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-09-28T22:38:03Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20838",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20838#event-275371"
}
```



---

archive/issue_events_275372.json:
```json
{
    "actor": "https://github.com/slel",
    "created_at": "2020-09-28T22:38:03Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20838",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20838#event-275372"
}
```
