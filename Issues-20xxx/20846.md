# Issue 20846: Maxima does not handle symbolic series

archive/issues_020609.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: ((1/6*pi^2).series(x)).simplify_full()\n_SAGE_VAR_(1/6*pi^2)\n```\n\nSee also #17400\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20846_\n\n",
    "created_at": "2016-06-19T09:45:22Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Maxima does not handle symbolic series",
    "type": "issue",
    "updated_at": "2022-12-29T01:36:35Z",
    "url": "https://github.com/sagemath/sage/issues/20846",
    "user": "https://github.com/mezzarobba"
}
```

```
sage: ((1/6*pi^2).series(x)).simplify_full()
_SAGE_VAR_(1/6*pi^2)
```

See also #17400

_Issue created by migration from https://trac.sagemath.org/ticket/20846_





---

archive/issue_events_275451.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2016-06-19T09:45:22Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20846",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20846#event-275451"
}
```



---

archive/issue_events_275452.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2016-06-19T09:45:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20846",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20846#event-275452"
}
```



---

archive/issue_events_275453.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2016-06-19T09:45:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20846",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20846#event-275453"
}
```



---

archive/issue_events_275454.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2016-06-19T09:45:22Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20846",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20846#event-275454"
}
```



---

archive/issue_comments_304923.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nGiNaC/Pynac series are not supported by the Maxima interface at all, atm.",
    "created_at": "2016-06-21T08:16:22Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20846",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20846#issuecomment-304923",
    "user": "https://github.com/rwst"
}
```

<div id="comment:1" align="right">Comment 1</div>

GiNaC/Pynac series are not supported by the Maxima interface at all, atm.



---

archive/issue_events_275455.json:
```json
{
    "actor": "https://github.com/rwst",
    "created_at": "2016-06-21T08:16:22Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/20846",
    "title_is": "Maxima does not handle symbolic series",
    "title_was": "_SAGE_VAR_ escapes from simplify_full() of series",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20846#event-275455"
}
```



---

archive/issue_comments_304924.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -3,3 +3,5 @@\n sage: ((1/6*pi^2).series(x)).simplify_full()\n _SAGE_VAR_(1/6*pi^2)\n ```\n+\n+See also #17400\n``````\n",
    "created_at": "2016-06-21T08:20:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20846",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20846#issuecomment-304924",
    "user": "https://github.com/rwst"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -3,3 +3,5 @@
 sage: ((1/6*pi^2).series(x)).simplify_full()
 _SAGE_VAR_(1/6*pi^2)
 ```
+
+See also #17400
``````




---

archive/issue_events_275456.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:36:35Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/20846",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20846#event-275456"
}
```
