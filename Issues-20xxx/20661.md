# Issue 20661: use python3 syntax for raise also in cython files (step1)

archive/issues_020424.json:
```json
{
    "assignees": [],
    "body": "Let us use the syntax\n\n```\nraise Error(\"msg\")\n```\nin all files, including cython.\n\nHere, done in some cython files. A small bunch, to be followed by one or several similar tickets for the rest.\n\nCC:  @embray @jdemeyer\n\nBranch/Commit: **[58e271c](https://github.com/sagemath/sagetrac-mirror/commit/58e271c73f1f67d7df58eaa94eb7432ec0ebdfd5)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20661_\n\n",
    "closed_at": "2016-05-28T14:19:49Z",
    "created_at": "2016-05-23T17:38:32Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20python3",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "use python3 syntax for raise also in cython files (step1)",
    "type": "issue",
    "updated_at": "2016-05-28T14:19:49Z",
    "url": "https://github.com/sagemath/sage/issues/20661",
    "user": "https://github.com/fchapoton"
}
```
Let us use the syntax

```
raise Error("msg")
```
in all files, including cython.

Here, done in some cython files. A small bunch, to be followed by one or several similar tickets for the rest.

CC:  @embray @jdemeyer

Branch/Commit: **[58e271c](https://github.com/sagemath/sagetrac-mirror/commit/58e271c73f1f67d7df58eaa94eb7432ec0ebdfd5)**

Reviewer: **Travis Scrimshaw**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/20661_





---

archive/issue_events_273005.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-05-23T17:38:32Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20661",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20661#event-273005"
}
```



---

archive/issue_events_273006.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-05-23T17:38:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20661",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20python3",
    "label_color": "000080",
    "label_name": "component: python3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20661#event-273006"
}
```



---

archive/issue_events_273007.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-05-23T17:38:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20661",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20661#event-273007"
}
```



---

archive/issue_events_273008.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-05-23T17:38:32Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20661",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20661#event-273008"
}
```



---

archive/issue_comments_301315.json:
```json
{
    "body": "Branch: **[public/20661](https://github.com/sagemath/sagetrac-mirror/tree/public/20661)**",
    "created_at": "2016-05-23T17:39:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20661",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20661#issuecomment-301315",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[public/20661](https://github.com/sagemath/sagetrac-mirror/tree/public/20661)**



---

archive/issue_comments_301316.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/58e271c73f1f67d7df58eaa94eb7432ec0ebdfd5\">58e271c</a></td><td><code>turning raise to python3 syntax in some cython files</code></td></tr></table>\n",
    "created_at": "2016-05-23T17:39:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20661",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20661#issuecomment-301316",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/58e271c73f1f67d7df58eaa94eb7432ec0ebdfd5">58e271c</a></td><td><code>turning raise to python3 syntax in some cython files</code></td></tr></table>




---

archive/issue_events_273009.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-05-23T17:39:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20661",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20661#event-273009"
}
```



---

archive/issue_comments_301317.json:
```json
{
    "body": "Commit: **[58e271c](https://github.com/sagemath/sagetrac-mirror/commit/58e271c73f1f67d7df58eaa94eb7432ec0ebdfd5)**",
    "created_at": "2016-05-23T17:39:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20661",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20661#issuecomment-301317",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[58e271c](https://github.com/sagemath/sagetrac-mirror/commit/58e271c73f1f67d7df58eaa94eb7432ec0ebdfd5)**



---

archive/issue_comments_301318.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nping ?",
    "created_at": "2016-05-28T06:47:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20661",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20661#issuecomment-301318",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

ping ?



---

archive/issue_comments_301319.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2016-05-28T07:31:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20661",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20661#issuecomment-301319",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_273010.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-05-28T07:31:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20661",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20661#event-273010"
}
```



---

archive/issue_events_273011.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-05-28T07:31:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20661",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20661#event-273011"
}
```



---

archive/issue_comments_301320.json:
```json
{
    "body": "Changed branch from **[public/20661](https://github.com/sagemath/sagetrac-mirror/tree/public/20661)** to **[58e271c](https://github.com/sagemath/sagetrac-mirror/commit/58e271c73f1f67d7df58eaa94eb7432ec0ebdfd5)**",
    "created_at": "2016-05-28T14:19:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20661",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20661#issuecomment-301320",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/20661](https://github.com/sagemath/sagetrac-mirror/tree/public/20661)** to **[58e271c](https://github.com/sagemath/sagetrac-mirror/commit/58e271c73f1f67d7df58eaa94eb7432ec0ebdfd5)**



---

archive/issue_events_273012.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-28T14:19:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20661",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20661#event-273012"
}
```



---

archive/issue_events_273013.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "a384eabc681655f231aed264da4286bd5440d0c5",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-05-28T14:19:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20661",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20661#event-273013"
}
```
