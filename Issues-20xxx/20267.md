# Issue 20267: Comparison with EmptyLetter fails

archive/issues_020030.json:
```json
{
    "assignees": [],
    "body": "We currently have:\n\n```\nsage: C = crystals.Letters(['C',4])\nsage: C(2) < C('E')\nFalse\nsage: C('E') < C(2)\n# Boom\n```\nThis is because we do not have a `_richcmp_` for `EmptyLetter`. This prevents the digraph code from drawing crystals with the empty letter (specifically KR tableaux).\n\nAssignee: **@sagetrac-sage-combinat**\n\nCC:  @sagetrac-sage-combinat @anneschilling @bsalisbury1\n\nKeywords: **crystals**\n\nBranch/Commit: **[831d6a5](https://github.com/sagemath/sagetrac-mirror/commit/831d6a5a85c0445734983a84aa826b88246aaae0)**\n\nReviewer: **Anne Schilling**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20267_\n\n",
    "closed_at": "2016-03-26T11:30:48Z",
    "created_at": "2016-03-23T19:03:36Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Comparison with EmptyLetter fails",
    "type": "issue",
    "updated_at": "2016-03-26T11:30:48Z",
    "url": "https://github.com/sagemath/sage/issues/20267",
    "user": "https://github.com/tscrim"
}
```
We currently have:

```
sage: C = crystals.Letters(['C',4])
sage: C(2) < C('E')
False
sage: C('E') < C(2)
# Boom
```
This is because we do not have a `_richcmp_` for `EmptyLetter`. This prevents the digraph code from drawing crystals with the empty letter (specifically KR tableaux).

Assignee: **@sagetrac-sage-combinat**

CC:  @sagetrac-sage-combinat @anneschilling @bsalisbury1

Keywords: **crystals**

Branch/Commit: **[831d6a5](https://github.com/sagemath/sagetrac-mirror/commit/831d6a5a85c0445734983a84aa826b88246aaae0)**

Reviewer: **Anne Schilling**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/20267_





---

archive/issue_events_268269.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-03-23T19:03:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20267",
    "milestone_number": null,
    "milestone_title": "sage-7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20267#event-268269"
}
```



---

archive/issue_events_268270.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-03-23T19:03:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20267",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20267#event-268270"
}
```



---

archive/issue_events_268271.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-03-23T19:03:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20267",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20267#event-268271"
}
```



---

archive/issue_events_268272.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-03-23T19:03:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20267",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20267#event-268272"
}
```



---

archive/issue_comments_294550.json:
```json
{
    "body": "Branch: **[public/crystals/fix_comparison_empty_letter-20267](https://github.com/sagemath/sagetrac-mirror/tree/public/crystals/fix_comparison_empty_letter-20267)**",
    "created_at": "2016-03-23T19:04:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20267#issuecomment-294550",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/crystals/fix_comparison_empty_letter-20267](https://github.com/sagemath/sagetrac-mirror/tree/public/crystals/fix_comparison_empty_letter-20267)**



---

archive/issue_events_268273.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-03-23T19:04:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20267",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20267#event-268273"
}
```



---

archive/issue_comments_294551.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/e1a75554d7384662503f8a030edd4e1eb834a2be\">e1a7555</a></td><td><code>Implementing comparisons for EmptyLetter.</code></td></tr></table>\n",
    "created_at": "2016-03-23T19:04:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20267#issuecomment-294551",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/e1a75554d7384662503f8a030edd4e1eb834a2be">e1a7555</a></td><td><code>Implementing comparisons for EmptyLetter.</code></td></tr></table>




---

archive/issue_comments_294552.json:
```json
{
    "body": "Commit: **[e1a7555](https://github.com/sagemath/sagetrac-mirror/commit/e1a75554d7384662503f8a030edd4e1eb834a2be)**",
    "created_at": "2016-03-23T19:04:09Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20267#issuecomment-294552",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[e1a7555](https://github.com/sagemath/sagetrac-mirror/commit/e1a75554d7384662503f8a030edd4e1eb834a2be)**



---

archive/issue_comments_294553.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nYou seem to have added the test\n\n```\nsage: C('E') != C(2)\nTrue\n```\ntwice.",
    "created_at": "2016-03-24T00:27:00Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20267#issuecomment-294553",
    "user": "https://github.com/anneschilling"
}
```

<div id="comment:3" align="right">Comment 3</div>

You seem to have added the test

```
sage: C('E') != C(2)
True
```
twice.



---

archive/issue_comments_294554.json:
```json
{
    "body": "Changed commit from **[e1a7555](https://github.com/sagemath/sagetrac-mirror/commit/e1a75554d7384662503f8a030edd4e1eb834a2be)** to **[831d6a5](https://github.com/sagemath/sagetrac-mirror/commit/831d6a5a85c0445734983a84aa826b88246aaae0)**",
    "created_at": "2016-03-24T18:39:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20267#issuecomment-294554",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[e1a7555](https://github.com/sagemath/sagetrac-mirror/commit/e1a75554d7384662503f8a030edd4e1eb834a2be)** to **[831d6a5](https://github.com/sagemath/sagetrac-mirror/commit/831d6a5a85c0445734983a84aa826b88246aaae0)**



---

archive/issue_comments_294555.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/831d6a5a85c0445734983a84aa826b88246aaae0\">831d6a5</a></td><td><code>Improve the testing (and don't do the same test twice).</code></td></tr></table>\n",
    "created_at": "2016-03-24T18:39:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20267#issuecomment-294555",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/831d6a5a85c0445734983a84aa826b88246aaae0">831d6a5</a></td><td><code>Improve the testing (and don't do the same test twice).</code></td></tr></table>




---

archive/issue_comments_294556.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nWhoops. Fixed.",
    "created_at": "2016-03-24T18:40:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20267#issuecomment-294556",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:5" align="right">Comment 5</div>

Whoops. Fixed.



---

archive/issue_comments_294557.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nReplying to [@tscrim](#comment%3A5):\n> Whoops. Fixed.\n\nThanks. Could you please post the code for KR crystals that currently fails? I drew a KR crystals of type C and it worked fine.",
    "created_at": "2016-03-25T04:26:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20267#issuecomment-294557",
    "user": "https://github.com/anneschilling"
}
```

<div id="comment:6" align="right">Comment 6</div>

Replying to [@tscrim](#comment%3A5):
> Whoops. Fixed.

Thanks. Could you please post the code for KR crystals that currently fails? I drew a KR crystals of type C and it worked fine.



---

archive/issue_comments_294558.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nYou need a KR crystal with an empty letter, so it needs to be a KR tableaux which can have them, such as type D<sub>n+1</sub><sup>(2)</sup>. These all generate errors for me:\n\n```\nsage: K = crystals.KirillovReshetikhin(['D',4,2], 1,2, 'KR')\nsage: view(K, tightpage=True)\nsage: K = crystals.KirillovReshetikhin(['A',4,2], 1,1, 'KR')\nsage: view(K, tightpage=True)\nsage: K = crystals.KirillovReshetikhin(['D',4,3], 1,1, 'KR')\nsage: view(K, tightpage=True)\n```",
    "created_at": "2016-03-25T05:22:25Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20267#issuecomment-294558",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:7" align="right">Comment 7</div>

You need a KR crystal with an empty letter, so it needs to be a KR tableaux which can have them, such as type D<sub>n+1</sub><sup>(2)</sup>. These all generate errors for me:

```
sage: K = crystals.KirillovReshetikhin(['D',4,2], 1,2, 'KR')
sage: view(K, tightpage=True)
sage: K = crystals.KirillovReshetikhin(['A',4,2], 1,1, 'KR')
sage: view(K, tightpage=True)
sage: K = crystals.KirillovReshetikhin(['D',4,3], 1,1, 'KR')
sage: view(K, tightpage=True)
```



---

archive/issue_events_268274.json:
```json
{
    "actor": "https://github.com/anneschilling",
    "created_at": "2016-03-25T05:35:53Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20267",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20267#event-268274"
}
```



---

archive/issue_events_268275.json:
```json
{
    "actor": "https://github.com/anneschilling",
    "created_at": "2016-03-25T05:35:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20267",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20267#event-268275"
}
```



---

archive/issue_comments_294559.json:
```json
{
    "body": "Reviewer: **Anne Schilling**",
    "created_at": "2016-03-25T05:35:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20267#issuecomment-294559",
    "user": "https://github.com/anneschilling"
}
```

Reviewer: **Anne Schilling**



---

archive/issue_comments_294560.json:
```json
{
    "body": "Changed branch from **[public/crystals/fix_comparison_empty_letter-20267](https://github.com/sagemath/sagetrac-mirror/tree/public/crystals/fix_comparison_empty_letter-20267)** to **[831d6a5](https://github.com/sagemath/sagetrac-mirror/commit/831d6a5a85c0445734983a84aa826b88246aaae0)**",
    "created_at": "2016-03-26T11:30:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20267",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20267#issuecomment-294560",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/crystals/fix_comparison_empty_letter-20267](https://github.com/sagemath/sagetrac-mirror/tree/public/crystals/fix_comparison_empty_letter-20267)** to **[831d6a5](https://github.com/sagemath/sagetrac-mirror/commit/831d6a5a85c0445734983a84aa826b88246aaae0)**



---

archive/issue_events_268276.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-03-26T11:30:48Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20267",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20267#event-268276"
}
```



---

archive/issue_events_268277.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "b117b30fcb73197eb36ffb2a6042c5b8aae044dc",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-03-26T11:30:48Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20267",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20267#event-268277"
}
```
