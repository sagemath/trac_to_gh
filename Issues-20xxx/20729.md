...
```json
{
    "body": "Maybe we should just never load the module, i.e. behave as if `in_lib` is always `True`?",
...
```json
{
    "body": "What do you think of [comment:3]? Load a file if and only if it is *not* part of a package (i.e. has no `__init__.*` file)\n\nThat would be very simple to implement and would probably do the right thing in most use cases.",
