# Issue 20392: Add a list of packages to download for sdist in Makefile

archive/issues_020155.json:
```json
{
    "assignees": [],
    "body": "Change this from `build/make/deps`\n\n```\ndownload-for-sdist: base\n    env SAGE_INSTALL_FETCH_ONLY=yes $(MAKE) -B SAGERUNTIME= \\\n        $(STANDARD_PACKAGES) gcc mpir python2\n```\nto use a new variable `$(SDIST_PACKAGES)` such that we can change `$(STANDARD_PACKAGES)` without affecting the sdist tarball.\n\nKeywords: **days77**\n\nBranch/Commit: **[c0e42dd](https://github.com/sagemath/sagetrac-mirror/commit/c0e42dd93df5634d4b53bff4e1e7beeda881dca7)**\n\nReviewer: **Volker Braun**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20392_\n\n",
    "closed_at": "2016-05-03T14:08:37Z",
    "created_at": "2016-04-08T21:02:18Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20distribution",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Add a list of packages to download for sdist in Makefile",
    "type": "issue",
    "updated_at": "2016-05-03T14:08:37Z",
    "url": "https://github.com/sagemath/sage/issues/20392",
    "user": "https://github.com/jdemeyer"
}
```
Change this from `build/make/deps`

```
download-for-sdist: base
    env SAGE_INSTALL_FETCH_ONLY=yes $(MAKE) -B SAGERUNTIME= \
        $(STANDARD_PACKAGES) gcc mpir python2
```
to use a new variable `$(SDIST_PACKAGES)` such that we can change `$(STANDARD_PACKAGES)` without affecting the sdist tarball.

Keywords: **days77**

Branch/Commit: **[c0e42dd](https://github.com/sagemath/sagetrac-mirror/commit/c0e42dd93df5634d4b53bff4e1e7beeda881dca7)**

Reviewer: **Volker Braun**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/20392_





---

archive/issue_events_269817.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-08T21:02:18Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20392",
    "milestone_number": null,
    "milestone_title": "sage-7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20392#event-269817"
}
```



---

archive/issue_events_269818.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-08T21:02:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20392",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20distribution",
    "label_color": "000080",
    "label_name": "component: distribution",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20392#event-269818"
}
```



---

archive/issue_events_269819.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-08T21:02:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20392",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20392#event-269819"
}
```



---

archive/issue_events_269820.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-08T21:02:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20392",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20392#event-269820"
}
```



---

archive/issue_comments_297053.json:
```json
{
    "body": "Branch: **[u/jdemeyer/add_a_list_of_packages_to_download_for_sdist_in_makefile](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/add_a_list_of_packages_to_download_for_sdist_in_makefile)**",
    "created_at": "2016-04-08T21:28:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20392#issuecomment-297053",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/add_a_list_of_packages_to_download_for_sdist_in_makefile](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/add_a_list_of_packages_to_download_for_sdist_in_makefile)**



---

archive/issue_events_269821.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-08T21:34:55Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20392",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20392#event-269821"
}
```



---

archive/issue_comments_297054.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/c0e42dd93df5634d4b53bff4e1e7beeda881dca7\">c0e42dd</a></td><td><code>New SDIST_PACKAGES make variable for packages to appear in sdist</code></td></tr></table>\n",
    "created_at": "2016-04-08T21:34:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20392#issuecomment-297054",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/c0e42dd93df5634d4b53bff4e1e7beeda881dca7">c0e42dd</a></td><td><code>New SDIST_PACKAGES make variable for packages to appear in sdist</code></td></tr></table>




---

archive/issue_comments_297055.json:
```json
{
    "body": "Commit: **[c0e42dd](https://github.com/sagemath/sagetrac-mirror/commit/c0e42dd93df5634d4b53bff4e1e7beeda881dca7)**",
    "created_at": "2016-04-08T21:34:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20392#issuecomment-297055",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[c0e42dd](https://github.com/sagemath/sagetrac-mirror/commit/c0e42dd93df5634d4b53bff4e1e7beeda881dca7)**



---

archive/issue_events_269822.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-01T22:49:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20392",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20392#event-269822"
}
```



---

archive/issue_events_269823.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-01T22:49:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20392",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20392#event-269823"
}
```



---

archive/issue_comments_297056.json:
```json
{
    "body": "Reviewer: **Volker Braun**",
    "created_at": "2016-05-01T22:49:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20392#issuecomment-297056",
    "user": "https://github.com/vbraun"
}
```

Reviewer: **Volker Braun**



---

archive/issue_comments_297057.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/add_a_list_of_packages_to_download_for_sdist_in_makefile](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/add_a_list_of_packages_to_download_for_sdist_in_makefile)** to **[c0e42dd](https://github.com/sagemath/sagetrac-mirror/commit/c0e42dd93df5634d4b53bff4e1e7beeda881dca7)**",
    "created_at": "2016-05-03T14:08:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20392",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20392#issuecomment-297057",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/add_a_list_of_packages_to_download_for_sdist_in_makefile](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/add_a_list_of_packages_to_download_for_sdist_in_makefile)** to **[c0e42dd](https://github.com/sagemath/sagetrac-mirror/commit/c0e42dd93df5634d4b53bff4e1e7beeda881dca7)**



---

archive/issue_events_269824.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-03T14:08:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20392",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20392#event-269824"
}
```



---

archive/issue_events_269825.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "b8a0e9a2add8695cecfc1955ef03bf5354301e90",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-05-03T14:08:37Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20392",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20392#event-269825"
}
```
