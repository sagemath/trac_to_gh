# Issue 20662: use python3 syntax for raise also in cython files (step2, rings part1)

archive/issues_020425.json:
```json
{
    "assignees": [],
    "body": "now conversion to `raise Error(\"msg\")` in rings, first half\n\nCC:  @embray @jdemeyer\n\nBranch/Commit: **[b903f5d](https://github.com/sagemath/sagetrac-mirror/commit/b903f5d3b38009471093bc90172de43ada8a6cf5)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20662_\n\n",
    "closed_at": "2016-05-28T12:13:39Z",
    "created_at": "2016-05-23T19:15:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20python3",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "use python3 syntax for raise also in cython files (step2, rings part1)",
    "type": "issue",
    "updated_at": "2016-05-28T12:13:39Z",
    "url": "https://github.com/sagemath/sage/issues/20662",
    "user": "https://github.com/fchapoton"
}
```
now conversion to `raise Error("msg")` in rings, first half

CC:  @embray @jdemeyer

Branch/Commit: **[b903f5d](https://github.com/sagemath/sagetrac-mirror/commit/b903f5d3b38009471093bc90172de43ada8a6cf5)**

Reviewer: **Travis Scrimshaw**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/20662_





---

archive/issue_events_273014.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-05-23T19:15:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20662",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20662#event-273014"
}
```



---

archive/issue_events_273015.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-05-23T19:15:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20662",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20python3",
    "label_color": "000080",
    "label_name": "component: python3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20662#event-273015"
}
```



---

archive/issue_events_273016.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-05-23T19:15:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20662",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20662#event-273016"
}
```



---

archive/issue_events_273017.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-05-23T19:15:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20662",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20662#event-273017"
}
```



---

archive/issue_comments_301321.json:
```json
{
    "body": "Branch: **[public/20662](https://github.com/sagemath/sagetrac-mirror/tree/public/20662)**",
    "created_at": "2016-05-23T19:16:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20662#issuecomment-301321",
    "user": "https://github.com/fchapoton"
}
```

Branch: **[public/20662](https://github.com/sagemath/sagetrac-mirror/tree/public/20662)**



---

archive/issue_comments_301322.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7f9165eaff22a45ed25047d93aafee9ed56134e5\">7f9165e</a></td><td><code>raise to py3 syntax in cython (rings, part 1)</code></td></tr></table>\n",
    "created_at": "2016-05-23T19:16:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20662#issuecomment-301322",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7f9165eaff22a45ed25047d93aafee9ed56134e5">7f9165e</a></td><td><code>raise to py3 syntax in cython (rings, part 1)</code></td></tr></table>




---

archive/issue_comments_301323.json:
```json
{
    "body": "Commit: **[7f9165e](https://github.com/sagemath/sagetrac-mirror/commit/7f9165eaff22a45ed25047d93aafee9ed56134e5)**",
    "created_at": "2016-05-23T19:16:04Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20662#issuecomment-301323",
    "user": "https://github.com/fchapoton"
}
```

Commit: **[7f9165e](https://github.com/sagemath/sagetrac-mirror/commit/7f9165eaff22a45ed25047d93aafee9ed56134e5)**



---

archive/issue_events_273018.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-05-23T19:16:04Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20662",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20662#event-273018"
}
```



---

archive/issue_comments_301324.json:
```json
{
    "body": "Changed commit from **[7f9165e](https://github.com/sagemath/sagetrac-mirror/commit/7f9165eaff22a45ed25047d93aafee9ed56134e5)** to **[b903f5d](https://github.com/sagemath/sagetrac-mirror/commit/b903f5d3b38009471093bc90172de43ada8a6cf5)**",
    "created_at": "2016-05-26T18:42:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20662#issuecomment-301324",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[7f9165e](https://github.com/sagemath/sagetrac-mirror/commit/7f9165eaff22a45ed25047d93aafee9ed56134e5)** to **[b903f5d](https://github.com/sagemath/sagetrac-mirror/commit/b903f5d3b38009471093bc90172de43ada8a6cf5)**



---

archive/issue_comments_301325.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b903f5d3b38009471093bc90172de43ada8a6cf5\">b903f5d</a></td><td><code>Merge branch 'public/20662' into 7.3.b1</code></td></tr></table>\n",
    "created_at": "2016-05-26T18:42:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20662#issuecomment-301325",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:2" align="right">Comment 2</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b903f5d3b38009471093bc90172de43ada8a6cf5">b903f5d</a></td><td><code>Merge branch 'public/20662' into 7.3.b1</code></td></tr></table>




---

archive/issue_comments_301326.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nLGTM.",
    "created_at": "2016-05-26T20:18:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20662#issuecomment-301326",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:3" align="right">Comment 3</div>

LGTM.



---

archive/issue_comments_301327.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2016-05-26T20:18:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20662#issuecomment-301327",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_events_273019.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-05-26T20:18:33Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20662",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20662#event-273019"
}
```



---

archive/issue_events_273020.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-05-26T20:18:33Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20662",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20662#event-273020"
}
```



---

archive/issue_events_273021.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-28T12:13:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20662",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20662#event-273021"
}
```



---

archive/issue_events_273022.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "7bddc8fc8c92198323602e8dae20acef8a2ba6a4",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-05-28T12:13:39Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20662",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20662#event-273022"
}
```



---

archive/issue_comments_301328.json:
```json
{
    "body": "Changed branch from **[public/20662](https://github.com/sagemath/sagetrac-mirror/tree/public/20662)** to **[b903f5d](https://github.com/sagemath/sagetrac-mirror/commit/b903f5d3b38009471093bc90172de43ada8a6cf5)**",
    "created_at": "2016-05-28T12:13:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20662",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20662#issuecomment-301328",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/20662](https://github.com/sagemath/sagetrac-mirror/tree/public/20662)** to **[b903f5d](https://github.com/sagemath/sagetrac-mirror/commit/b903f5d3b38009471093bc90172de43ada8a6cf5)**
