# Issue 20711: Do not iterate in pretty_print

archive/issues_020474.json:
```json
{
    "assignees": [],
    "body": "We want\n\n```\nsage: pretty_print(GF(2))\n\\newcommand{\\Bold}[1]{\\mathbf{#1}}\\Bold{F}_{2}\n```\ninstead of iterating over the elements of GF(2)\n\nCC:  @dkrenn\n\nKeywords: **sd74**\n\nBranch/Commit: **[6fafd06](https://github.com/sagemath/sagetrac-mirror/commit/6fafd06f6adc8621e064d31b948b4a8b744f699b)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Volker Braun**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20711_\n\n",
    "closed_at": "2016-05-31T22:53:49Z",
    "created_at": "2016-05-30T09:09:10Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Do not iterate in pretty_print",
    "type": "issue",
    "updated_at": "2016-05-31T22:53:49Z",
    "url": "https://github.com/sagemath/sage/issues/20711",
    "user": "https://github.com/sagetrac-Braun"
}
```
We want

```
sage: pretty_print(GF(2))
\newcommand{\Bold}[1]{\mathbf{#1}}\Bold{F}_{2}
```
instead of iterating over the elements of GF(2)

CC:  @dkrenn

Keywords: **sd74**

Branch/Commit: **[6fafd06](https://github.com/sagemath/sagetrac-mirror/commit/6fafd06f6adc8621e064d31b948b4a8b744f699b)**

Reviewer: **Frédéric Chapoton**

Author: **Volker Braun**

_Issue created by migration from https://trac.sagemath.org/ticket/20711_





---

archive/issue_events_273722.json:
```json
{
    "actor": "https://github.com/sagetrac-Braun",
    "created_at": "2016-05-30T09:09:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20711",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20711#event-273722"
}
```



---

archive/issue_events_273723.json:
```json
{
    "actor": "https://github.com/sagetrac-Braun",
    "created_at": "2016-05-30T09:09:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20711",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20711#event-273723"
}
```



---

archive/issue_comments_302468.json:
```json
{
    "body": "Branch: **[u/vbraun/do_not_iterate_in_pretty_print](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/do_not_iterate_in_pretty_print)**",
    "created_at": "2016-05-30T12:34:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20711#issuecomment-302468",
    "user": "https://github.com/vbraun"
}
```

Branch: **[u/vbraun/do_not_iterate_in_pretty_print](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/do_not_iterate_in_pretty_print)**



---

archive/issue_events_273724.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-30T12:35:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20711",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20711#event-273724"
}
```



---

archive/issue_comments_302469.json:
```json
{
    "body": "Changed reporter from **vbraun** to **Braun**",
    "created_at": "2016-05-30T12:35:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20711#issuecomment-302469",
    "user": "https://github.com/vbraun"
}
```

Changed reporter from **vbraun** to **Braun**



---

archive/issue_events_273725.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-30T12:35:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20711",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20711#event-273725"
}
```



---

archive/issue_comments_302470.json:
```json
{
    "body": "Author: **Volker Braun**",
    "created_at": "2016-05-30T12:35:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20711#issuecomment-302470",
    "user": "https://github.com/vbraun"
}
```

Author: **Volker Braun**



---

archive/issue_comments_302471.json:
```json
{
    "body": "Changed keywords from **** to **sd74**",
    "created_at": "2016-05-30T12:35:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20711#issuecomment-302471",
    "user": "https://github.com/vbraun"
}
```

Changed keywords from **** to **sd74**



---

archive/issue_comments_302472.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,7 @@\n+We want\n \n+```\n+sage: pretty_print(GF(2))\n+\\newcommand{\\Bold}[1]{\\mathbf{#1}}\\Bold{F}_{2}\n+```\n+instead of iterating over the elements of GF(2)\n``````\n",
    "created_at": "2016-05-30T12:35:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20711#issuecomment-302472",
    "user": "https://github.com/vbraun"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,7 @@
+We want
 
+```
+sage: pretty_print(GF(2))
+\newcommand{\Bold}[1]{\mathbf{#1}}\Bold{F}_{2}
+```
+instead of iterating over the elements of GF(2)
``````




---

archive/issue_events_273726.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-30T12:35:34Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20711",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20user%20interface",
    "label_color": "000080",
    "label_name": "component: user interface",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20711#event-273726"
}
```



---

archive/issue_comments_302473.json:
```json
{
    "body": "Commit: **[64dc304](https://github.com/sagemath/sagetrac-mirror/commit/64dc304ed5d3bf9d417e28ab2f406f72ec9cd20a)**",
    "created_at": "2016-05-30T12:35:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20711#issuecomment-302473",
    "user": "https://github.com/vbraun"
}
```

Commit: **[64dc304](https://github.com/sagemath/sagetrac-mirror/commit/64dc304ed5d3bf9d417e28ab2f406f72ec9cd20a)**



---

archive/issue_comments_302474.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/64dc304ed5d3bf9d417e28ab2f406f72ec9cd20a\">64dc304</a></td><td><code>Do not unwrap iterators/generators in pretty_print</code></td></tr></table>\n",
    "created_at": "2016-05-30T12:35:34Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20711#issuecomment-302474",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/64dc304ed5d3bf9d417e28ab2f406f72ec9cd20a">64dc304</a></td><td><code>Do not unwrap iterators/generators in pretty_print</code></td></tr></table>




---

archive/issue_comments_302475.json:
```json
{
    "body": "<a id='comment:4'>Comment 4:</a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/6fafd06f6adc8621e064d31b948b4a8b744f699b\">6fafd06</a></td><td><code>Fix failing doctest</code></td></tr></table>\n",
    "created_at": "2016-05-30T16:11:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20711#issuecomment-302475",
    "user": "https://github.com/sagetrac-git"
}
```

<a id='comment:4'>Comment 4:</a>
Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/6fafd06f6adc8621e064d31b948b4a8b744f699b">6fafd06</a></td><td><code>Fix failing doctest</code></td></tr></table>




---

archive/issue_comments_302476.json:
```json
{
    "body": "Changed commit from **[64dc304](https://github.com/sagemath/sagetrac-mirror/commit/64dc304ed5d3bf9d417e28ab2f406f72ec9cd20a)** to **[6fafd06](https://github.com/sagemath/sagetrac-mirror/commit/6fafd06f6adc8621e064d31b948b4a8b744f699b)**",
    "created_at": "2016-05-30T16:11:32Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20711#issuecomment-302476",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[64dc304](https://github.com/sagemath/sagetrac-mirror/commit/64dc304ed5d3bf9d417e28ab2f406f72ec9cd20a)** to **[6fafd06](https://github.com/sagemath/sagetrac-mirror/commit/6fafd06f6adc8621e064d31b948b4a8b744f699b)**



---

archive/issue_events_273727.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-05-30T19:07:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20711",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20711#event-273727"
}
```



---

archive/issue_events_273728.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-05-30T19:07:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20711",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20711#event-273728"
}
```



---

archive/issue_comments_302477.json:
```json
{
    "body": "<a id='comment:5'>Comment 5:</a>\nok, lgtm",
    "created_at": "2016-05-30T19:07:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20711#issuecomment-302477",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:5'>Comment 5:</a>
ok, lgtm



---

archive/issue_comments_302478.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2016-05-30T19:07:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20711#issuecomment-302478",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_302479.json:
```json
{
    "body": "Changed branch from **[u/vbraun/do_not_iterate_in_pretty_print](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/do_not_iterate_in_pretty_print)** to **[6fafd06](https://github.com/sagemath/sagetrac-mirror/commit/6fafd06f6adc8621e064d31b948b4a8b744f699b)**",
    "created_at": "2016-05-31T22:53:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20711",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20711#issuecomment-302479",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/vbraun/do_not_iterate_in_pretty_print](https://github.com/sagemath/sagetrac-mirror/tree/u/vbraun/do_not_iterate_in_pretty_print)** to **[6fafd06](https://github.com/sagemath/sagetrac-mirror/commit/6fafd06f6adc8621e064d31b948b4a8b744f699b)**



---

archive/issue_events_273729.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-31T22:53:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20711",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20711#event-273729"
}
```



---

archive/issue_events_273730.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "70a213cb2dfc4d9f2d47bedc3e1e67922d50be6c",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-05-31T22:53:49Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20711",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20711#event-273730"
}
```
