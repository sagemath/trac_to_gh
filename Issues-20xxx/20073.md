# Issue 20073: p-adic extensions with non-monic polynomials

archive/issues_019836.json:
```json
{
    "assignees": [],
    "body": "\n```\nsage: A = Qp(3,5)\nsage: Po.<X> = A[]\nsage: f = Po([3,0,-1])\nsage: K.<a> = A.ext(f)\nTraceback (most recent call last):\n...\nUnboundLocalError: local variable 'preseed' referenced before assignment\n```\n\n\n\nKeywords: **sd87**\n\nBranch/Commit: **[48be601](https://github.com/sagemath/sagetrac-mirror/commit/48be601fe35d7d56ec6cbbb6d21bb6921230b7eb)**\n\nReviewer: **GaYee Park**\n\nAuthor: **Adele Bourgeois**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20073_\n\n",
    "closed_at": "2017-07-26T22:13:38Z",
    "created_at": "2016-02-16T20:54:35Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20padics",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.1",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "p-adic extensions with non-monic polynomials",
    "type": "issue",
    "updated_at": "2017-07-26T22:13:38Z",
    "url": "https://github.com/sagemath/sage/issues/20073",
    "user": "https://github.com/roed314"
}
```

```
sage: A = Qp(3,5)
sage: Po.<X> = A[]
sage: f = Po([3,0,-1])
sage: K.<a> = A.ext(f)
Traceback (most recent call last):
...
UnboundLocalError: local variable 'preseed' referenced before assignment
```



Keywords: **sd87**

Branch/Commit: **[48be601](https://github.com/sagemath/sagetrac-mirror/commit/48be601fe35d7d56ec6cbbb6d21bb6921230b7eb)**

Reviewer: **GaYee Park**

Author: **Adele Bourgeois**

_Issue created by migration from https://trac.sagemath.org/ticket/20073_





---

archive/issue_events_265618.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2016-02-16T20:54:35Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "milestone_number": null,
    "milestone_title": "sage-7.1",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20073#event-265618"
}
```



---

archive/issue_events_265619.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2016-02-16T20:54:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20padics",
    "label_color": "0000ff",
    "label_name": "component: padics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20073#event-265619"
}
```



---

archive/issue_events_265620.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2016-02-16T20:54:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20073#event-265620"
}
```



---

archive/issue_events_265621.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2016-02-16T20:54:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20073#event-265621"
}
```



---

archive/issue_comments_290263.json:
```json
{
    "body": "Branch: **[u/asteele/ticket/20073](https://github.com/sagemath/sagetrac-mirror/tree/u/asteele/ticket/20073)**",
    "created_at": "2016-03-25T20:28:56Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20073#issuecomment-290263",
    "user": "https://github.com/andersteele"
}
```

Branch: **[u/asteele/ticket/20073](https://github.com/sagemath/sagetrac-mirror/tree/u/asteele/ticket/20073)**



---

archive/issue_events_265622.json:
```json
{
    "actor": "https://github.com/andersteele",
    "created_at": "2016-03-25T20:32:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20073#event-265622"
}
```



---

archive/issue_comments_290264.json:
```json
{
    "body": "Commit: **[7f96697](https://github.com/sagemath/sagetrac-mirror/commit/7f966970ce066669c13ec27ef5d6c13eaf6441dd)**",
    "created_at": "2016-03-25T20:32:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20073#issuecomment-290264",
    "user": "https://github.com/andersteele"
}
```

Commit: **[7f96697](https://github.com/sagemath/sagetrac-mirror/commit/7f966970ce066669c13ec27ef5d6c13eaf6441dd)**



---

archive/issue_comments_290265.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/7f966970ce066669c13ec27ef5d6c13eaf6441dd\">7f96697</a></td><td><code>Fixed typo with variable assignment</code></td></tr></table>\n",
    "created_at": "2016-03-25T20:32:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20073#issuecomment-290265",
    "user": "https://github.com/andersteele"
}
```

<div id="comment:2" align="right">Comment 2</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/7f966970ce066669c13ec27ef5d6c13eaf6441dd">7f96697</a></td><td><code>Fixed typo with variable assignment</code></td></tr></table>




---

archive/issue_comments_290266.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nSo, whenever you fix a bug you also need to add doctests to show that the bug has been fixed.  In this case, try creating some extensions defined by non-monic polynomials and playing around with them.  Do they behave as you would want?",
    "created_at": "2016-03-25T21:30:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20073#issuecomment-290266",
    "user": "https://github.com/roed314"
}
```

<div id="comment:3" align="right">Comment 3</div>

So, whenever you fix a bug you also need to add doctests to show that the bug has been fixed.  In this case, try creating some extensions defined by non-monic polynomials and playing around with them.  Do they behave as you would want?



---

archive/issue_events_265623.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2016-03-25T21:30:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20073#event-265623"
}
```



---

archive/issue_events_265624.json:
```json
{
    "actor": "https://github.com/roed314",
    "created_at": "2016-03-25T21:30:18Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20073#event-265624"
}
```



---

archive/issue_comments_290267.json:
```json
{
    "body": "Changed commit from **[7f96697](https://github.com/sagemath/sagetrac-mirror/commit/7f966970ce066669c13ec27ef5d6c13eaf6441dd)** to **[bbb7268](https://github.com/sagemath/sagetrac-mirror/commit/bbb7268c1692a0068a9d4bd5ae7624eadf321454)**",
    "created_at": "2016-03-27T04:50:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20073#issuecomment-290267",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[7f96697](https://github.com/sagemath/sagetrac-mirror/commit/7f966970ce066669c13ec27ef5d6c13eaf6441dd)** to **[bbb7268](https://github.com/sagemath/sagetrac-mirror/commit/bbb7268c1692a0068a9d4bd5ae7624eadf321454)**



---

archive/issue_comments_290268.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bbb7268c1692a0068a9d4bd5ae7624eadf321454\">bbb7268</a></td><td><code>added doctests</code></td></tr></table>\n",
    "created_at": "2016-03-27T04:50:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20073#issuecomment-290268",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bbb7268c1692a0068a9d4bd5ae7624eadf321454">bbb7268</a></td><td><code>added doctests</code></td></tr></table>




---

archive/issue_comments_290269.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nYour branch doesn't merge against develop.  I think the following will work, if executed from within the sage folder:\n\n```\n~/sage$ git fetch trac develop:develop\n~/sage$ git pull develop\n```",
    "created_at": "2016-03-27T22:28:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20073#issuecomment-290269",
    "user": "https://github.com/roed314"
}
```

<div id="comment:5" align="right">Comment 5</div>

Your branch doesn't merge against develop.  I think the following will work, if executed from within the sage folder:

```
~/sage$ git fetch trac develop:develop
~/sage$ git pull develop
```



---

archive/issue_comments_290270.json:
```json
{
    "body": "Changed commit from **[bbb7268](https://github.com/sagemath/sagetrac-mirror/commit/bbb7268c1692a0068a9d4bd5ae7624eadf321454)** to **[8081eab](https://github.com/sagemath/sagetrac-mirror/commit/8081eab7fac81a3cef7d40133937bc2ca01baa38)**",
    "created_at": "2016-03-27T22:54:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20073#issuecomment-290270",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[bbb7268](https://github.com/sagemath/sagetrac-mirror/commit/bbb7268c1692a0068a9d4bd5ae7624eadf321454)** to **[8081eab](https://github.com/sagemath/sagetrac-mirror/commit/8081eab7fac81a3cef7d40133937bc2ca01baa38)**



---

archive/issue_comments_290271.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ee461b61afa23c587307df64a64df219deb0b1c9\">ee461b6</a></td><td><code>merge with develop</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/8081eab7fac81a3cef7d40133937bc2ca01baa38\">8081eab</a></td><td><code>added doctests after fixing conflicts</code></td></tr></table>\n",
    "created_at": "2016-03-27T22:54:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20073#issuecomment-290271",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ee461b61afa23c587307df64a64df219deb0b1c9">ee461b6</a></td><td><code>merge with develop</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/8081eab7fac81a3cef7d40133937bc2ca01baa38">8081eab</a></td><td><code>added doctests after fixing conflicts</code></td></tr></table>




---

archive/issue_comments_290272.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nThanks for the help! Should merge cleanly now.",
    "created_at": "2016-03-27T22:54:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20073#issuecomment-290272",
    "user": "https://github.com/andersteele"
}
```

<div id="comment:7" align="right">Comment 7</div>

Thanks for the help! Should merge cleanly now.



---

archive/issue_comments_290273.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nCool.  You should also add some tests showing that the resulting extensions have the right defining polynomial.",
    "created_at": "2016-03-27T22:57:53Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20073#issuecomment-290273",
    "user": "https://github.com/roed314"
}
```

<div id="comment:8" align="right">Comment 8</div>

Cool.  You should also add some tests showing that the resulting extensions have the right defining polynomial.



---

archive/issue_comments_290274.json:
```json
{
    "body": "Changed keywords from none to **sd87**",
    "created_at": "2017-07-19T15:40:02Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20073#issuecomment-290274",
    "user": "https://github.com/adeines"
}
```

Changed keywords from none to **sd87**



---

archive/issue_comments_290275.json:
```json
{
    "body": "Changed branch from **[u/asteele/ticket/20073](https://github.com/sagemath/sagetrac-mirror/tree/u/asteele/ticket/20073)** to **[u/abourgeois/ticket/20073](https://github.com/sagemath/sagetrac-mirror/tree/u/abourgeois/ticket/20073)**",
    "created_at": "2017-07-19T19:23:57Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20073#issuecomment-290275",
    "user": "https://github.com/sagetrac-abourgeois"
}
```

Changed branch from **[u/asteele/ticket/20073](https://github.com/sagemath/sagetrac-mirror/tree/u/asteele/ticket/20073)** to **[u/abourgeois/ticket/20073](https://github.com/sagemath/sagetrac-mirror/tree/u/abourgeois/ticket/20073)**



---

archive/issue_events_265625.json:
```json
{
    "actor": "https://github.com/sagetrac-abourgeois",
    "created_at": "2017-07-19T19:28:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20073#event-265625"
}
```



---

archive/issue_events_265626.json:
```json
{
    "actor": "https://github.com/sagetrac-abourgeois",
    "created_at": "2017-07-19T19:28:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20073#event-265626"
}
```



---

archive/issue_comments_290276.json:
```json
{
    "body": "Changed commit from **[8081eab](https://github.com/sagemath/sagetrac-mirror/commit/8081eab7fac81a3cef7d40133937bc2ca01baa38)** to **[48be601](https://github.com/sagemath/sagetrac-mirror/commit/48be601fe35d7d56ec6cbbb6d21bb6921230b7eb)**",
    "created_at": "2017-07-19T19:28:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20073#issuecomment-290276",
    "user": "https://github.com/sagetrac-abourgeois"
}
```

Changed commit from **[8081eab](https://github.com/sagemath/sagetrac-mirror/commit/8081eab7fac81a3cef7d40133937bc2ca01baa38)** to **[48be601](https://github.com/sagemath/sagetrac-mirror/commit/48be601fe35d7d56ec6cbbb6d21bb6921230b7eb)**



---

archive/issue_comments_290277.json:
```json
{
    "body": "Author: **Adele Bourgeois**",
    "created_at": "2017-07-19T19:28:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20073#issuecomment-290277",
    "user": "https://github.com/sagetrac-abourgeois"
}
```

Author: **Adele Bourgeois**



---

archive/issue_comments_290278.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/063b58cfb2a52bfb3bd4917e5a400a37ed03a0c2\">063b58c</a></td><td><code>Merge branch 'develop' into t/20073/ticket/20073</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/48be601fe35d7d56ec6cbbb6d21bb6921230b7eb\">48be601</a></td><td><code>Added documentation to verify that the extension has the right defining polynomial</code></td></tr></table>\n",
    "created_at": "2017-07-19T19:28:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20073#issuecomment-290278",
    "user": "https://github.com/sagetrac-abourgeois"
}
```

<div id="comment:11" align="right">Comment 11</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/063b58cfb2a52bfb3bd4917e5a400a37ed03a0c2">063b58c</a></td><td><code>Merge branch 'develop' into t/20073/ticket/20073</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/48be601fe35d7d56ec6cbbb6d21bb6921230b7eb">48be601</a></td><td><code>Added documentation to verify that the extension has the right defining polynomial</code></td></tr></table>




---

archive/issue_comments_290279.json:
```json
{
    "body": "Reviewer: **GaYee Park**",
    "created_at": "2017-07-19T19:38:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20073#issuecomment-290279",
    "user": "https://github.com/sagetrac-gpark"
}
```

Reviewer: **GaYee Park**



---

archive/issue_events_265627.json:
```json
{
    "actor": "https://github.com/sagetrac-gpark",
    "created_at": "2017-07-19T19:38:50Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20073#event-265627"
}
```



---

archive/issue_events_265628.json:
```json
{
    "actor": "https://github.com/sagetrac-gpark",
    "created_at": "2017-07-19T19:38:50Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20073#event-265628"
}
```



---

archive/issue_comments_290280.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nAll test passed",
    "created_at": "2017-07-19T19:38:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20073#issuecomment-290280",
    "user": "https://github.com/sagetrac-gpark"
}
```

<div id="comment:12" align="right">Comment 12</div>

All test passed



---

archive/issue_comments_290281.json:
```json
{
    "body": "Changed branch from **[u/abourgeois/ticket/20073](https://github.com/sagemath/sagetrac-mirror/tree/u/abourgeois/ticket/20073)** to **[48be601](https://github.com/sagemath/sagetrac-mirror/commit/48be601fe35d7d56ec6cbbb6d21bb6921230b7eb)**",
    "created_at": "2017-07-26T22:13:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20073#issuecomment-290281",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/abourgeois/ticket/20073](https://github.com/sagemath/sagetrac-mirror/tree/u/abourgeois/ticket/20073)** to **[48be601](https://github.com/sagemath/sagetrac-mirror/commit/48be601fe35d7d56ec6cbbb6d21bb6921230b7eb)**



---

archive/issue_events_265629.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2017-07-26T22:13:38Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20073#event-265629"
}
```



---

archive/issue_events_265630.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "f647e6c0123a82d235124d969cb1db73d2749580",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2017-07-26T22:13:38Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20073",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20073#event-265630"
}
```
