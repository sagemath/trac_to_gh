...
```json
{
    "body": "What I am doing is iterating through the group and then doing multiplication. The groups are not so big that I couldn't store what I need in a `dict` (which sent a 30s computation to 30ms!), but it would be good for doing testing on random elements for, e.g., G<sub>32</sub>.",
...
```json
{
    "body": "https://groups.google.com/d/topic/sage-support/Bl9hRMRauFo/discussion",
...
```json
{
    "body": "Travis, could you send me some example of what you do and where you get the `30s vs. 30ms` timings?",
...
```json
{
    "body": "I am looking at lengths of coset representatives, so I am doing a multiplication and then looking at the length. What instead I did was loop over the group and created a dictionary `{element: length}` (essentially I cached the results which are known from the iteration).",
