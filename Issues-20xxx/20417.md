# Issue 20417: Improve speed of CoxeterGroup as a matrix group

archive/issues_020180.json:
```json
{
    "assignees": [],
    "body": "We only need to check for the first non-zero entry to determine if there is a descent in position `i`.\n\nAssignee: **@sagetrac-sage-combinat**\n\nCC:  @sagetrac-sage-combinat @sagetrac-stumpc5 @fchapoton @nthiery\n\nBranch/Commit: **[bf7164a](https://github.com/sagemath/sagetrac-mirror/commit/bf7164aa677727c22d8cca72ef10f563e2d78745)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20417_\n\n",
    "closed_at": "2016-04-12T16:30:31Z",
    "created_at": "2016-04-11T09:21:23Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20group%20theory",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Improve speed of CoxeterGroup as a matrix group",
    "type": "issue",
    "updated_at": "2016-04-12T16:30:31Z",
    "url": "https://github.com/sagemath/sage/issues/20417",
    "user": "https://github.com/tscrim"
}
```
We only need to check for the first non-zero entry to determine if there is a descent in position `i`.

Assignee: **@sagetrac-sage-combinat**

CC:  @sagetrac-sage-combinat @sagetrac-stumpc5 @fchapoton @nthiery

Branch/Commit: **[bf7164a](https://github.com/sagemath/sagetrac-mirror/commit/bf7164aa677727c22d8cca72ef10f563e2d78745)**

Reviewer: **Frédéric Chapoton**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/20417_





---

archive/issue_events_270083.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-04-11T09:21:23Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20417",
    "milestone_number": null,
    "milestone_title": "sage-7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20417#event-270083"
}
```



---

archive/issue_events_270084.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-04-11T09:21:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20417",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20group%20theory",
    "label_color": "0000ff",
    "label_name": "component: group theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20417#event-270084"
}
```



---

archive/issue_events_270085.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-04-11T09:21:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20417",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20417#event-270085"
}
```



---

archive/issue_events_270086.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-04-11T09:21:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20417",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20417#event-270086"
}
```



---

archive/issue_comments_297426.json:
```json
{
    "body": "Branch: **[public/groups/speedup_descents_coxeter_groups-20417](https://github.com/sagemath/sagetrac-mirror/tree/public/groups/speedup_descents_coxeter_groups-20417)**",
    "created_at": "2016-04-11T09:21:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20417",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20417#issuecomment-297426",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/groups/speedup_descents_coxeter_groups-20417](https://github.com/sagemath/sagetrac-mirror/tree/public/groups/speedup_descents_coxeter_groups-20417)**



---

archive/issue_events_270087.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2016-04-11T09:21:59Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20417",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20417#event-270087"
}
```



---

archive/issue_comments_297427.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/ebb3d12809a2cbb523b7a15b4255abdb50dc603f\">ebb3d12</a></td><td><code>Increasing speed for matrix group version of Coxeter groups.</code></td></tr></table>\n",
    "created_at": "2016-04-11T09:21:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20417",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20417#issuecomment-297427",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:1" align="right">Comment 1</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/ebb3d12809a2cbb523b7a15b4255abdb50dc603f">ebb3d12</a></td><td><code>Increasing speed for matrix group version of Coxeter groups.</code></td></tr></table>




---

archive/issue_comments_297428.json:
```json
{
    "body": "Commit: **[ebb3d12](https://github.com/sagemath/sagetrac-mirror/commit/ebb3d12809a2cbb523b7a15b4255abdb50dc603f)**",
    "created_at": "2016-04-11T09:21:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20417",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20417#issuecomment-297428",
    "user": "https://github.com/tscrim"
}
```

Commit: **[ebb3d12](https://github.com/sagemath/sagetrac-mirror/commit/ebb3d12809a2cbb523b7a15b4255abdb50dc603f)**



---

archive/issue_comments_297429.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\n`_matrix_test_right_descent(M, i, n, zero)` should raise an error if the for loop ends without finding an element.",
    "created_at": "2016-04-11T09:28:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20417",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20417#issuecomment-297429",
    "user": "https://github.com/sagetrac-stumpc5"
}
```

<div id="comment:2" align="right">Comment 2</div>

`_matrix_test_right_descent(M, i, n, zero)` should raise an error if the for loop ends without finding an element.



---

archive/issue_comments_297430.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/bf7164aa677727c22d8cca72ef10f563e2d78745\">bf7164a</a></td><td><code>trac #20417 assertion error</code></td></tr></table>\n",
    "created_at": "2016-04-11T09:34:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20417",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20417#issuecomment-297430",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/bf7164aa677727c22d8cca72ef10f563e2d78745">bf7164a</a></td><td><code>trac #20417 assertion error</code></td></tr></table>




---

archive/issue_comments_297431.json:
```json
{
    "body": "Changed commit from **[ebb3d12](https://github.com/sagemath/sagetrac-mirror/commit/ebb3d12809a2cbb523b7a15b4255abdb50dc603f)** to **[bf7164a](https://github.com/sagemath/sagetrac-mirror/commit/bf7164aa677727c22d8cca72ef10f563e2d78745)**",
    "created_at": "2016-04-11T09:34:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20417",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20417#issuecomment-297431",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[ebb3d12](https://github.com/sagemath/sagetrac-mirror/commit/ebb3d12809a2cbb523b7a15b4255abdb50dc603f)** to **[bf7164a](https://github.com/sagemath/sagetrac-mirror/commit/bf7164aa677727c22d8cca72ef10f563e2d78745)**



---

archive/issue_events_270088.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-04-11T11:21:49Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20417",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20417#event-270088"
}
```



---

archive/issue_events_270089.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2016-04-11T11:21:49Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20417",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20417#event-270089"
}
```



---

archive/issue_comments_297432.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nlooks good to me",
    "created_at": "2016-04-11T11:21:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20417",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20417#issuecomment-297432",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:4" align="right">Comment 4</div>

looks good to me



---

archive/issue_comments_297433.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2016-04-11T11:21:49Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20417",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20417#issuecomment-297433",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_297434.json:
```json
{
    "body": "Changed branch from **[public/groups/speedup_descents_coxeter_groups-20417](https://github.com/sagemath/sagetrac-mirror/tree/public/groups/speedup_descents_coxeter_groups-20417)** to **[bf7164a](https://github.com/sagemath/sagetrac-mirror/commit/bf7164aa677727c22d8cca72ef10f563e2d78745)**",
    "created_at": "2016-04-12T16:30:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20417",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20417#issuecomment-297434",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/groups/speedup_descents_coxeter_groups-20417](https://github.com/sagemath/sagetrac-mirror/tree/public/groups/speedup_descents_coxeter_groups-20417)** to **[bf7164a](https://github.com/sagemath/sagetrac-mirror/commit/bf7164aa677727c22d8cca72ef10f563e2d78745)**



---

archive/issue_events_270090.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-04-12T16:30:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20417",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20417#event-270090"
}
```



---

archive/issue_events_270091.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "71f392a8f3626a6634d15664b78d9e7bf9657002",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-04-12T16:30:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20417",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20417#event-270091"
}
```
