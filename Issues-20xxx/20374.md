# Issue 20374: Properly package sagenb

archive/issues_020137.json:
```json
{
    "assignees": [],
    "body": "\n\nCC:  @kcrisman\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20374_\n\n",
    "closed_at": "2016-04-06T18:47:31Z",
    "created_at": "2016-04-06T18:18:16Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Properly package sagenb",
    "type": "issue",
    "updated_at": "2016-05-13T09:54:26Z",
    "url": "https://github.com/sagemath/sage/issues/20374",
    "user": "https://github.com/jdemeyer"
}
```


CC:  @kcrisman

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/20374_





---

archive/issue_events_269582.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-06T18:18:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20374",
    "milestone_number": null,
    "milestone_title": "sage-7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20374#event-269582"
}
```



---

archive/issue_events_269583.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-06T18:18:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20374",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20packages%3A%20standard",
    "label_color": "000080",
    "label_name": "component: packages: standard",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20374#event-269583"
}
```



---

archive/issue_events_269584.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-06T18:18:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20374",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20374#event-269584"
}
```



---

archive/issue_events_269585.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-06T18:18:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20374",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20374#event-269585"
}
```



---

archive/issue_comments_296439.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n **Tarballs**:\n-* [\u200bhttps://pypi.python.org/packages/source/B/Babel/Babel-2.2.0.tar.gz]\n-* [https://pypi.python.org/packages/source/p/pytz/pytz-2016.3.tar.bz2](https://pypi.python.org/packages/source/p/pytz/pytz-2016.3.tar.bz2)\n+* https://pypi.python.org/packages/source/B/Babel/Babel-2.2.0.tar.gz\n+* https://pypi.python.org/packages/source/p/pytz/pytz-2016.3.tar.bz2\n``````\n",
    "created_at": "2016-04-06T18:20:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20374#issuecomment-296439",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
 **Tarballs**:
-* [​https://pypi.python.org/packages/source/B/Babel/Babel-2.2.0.tar.gz]
-* [https://pypi.python.org/packages/source/p/pytz/pytz-2016.3.tar.bz2](https://pypi.python.org/packages/source/p/pytz/pytz-2016.3.tar.bz2)
+* https://pypi.python.org/packages/source/B/Babel/Babel-2.2.0.tar.gz
+* https://pypi.python.org/packages/source/p/pytz/pytz-2016.3.tar.bz2
``````




---

archive/issue_events_269586.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-06T18:35:45Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/20374",
    "title_is": "Properly package sagenb",
    "title_was": "Pull babel and pytz out of sagenb",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20374#event-269586"
}
```



---

archive/issue_events_269587.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-06T18:47:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20374",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20374#event-269587"
}
```



---

archive/issue_events_269588.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-06T18:47:31Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20374",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20374#event-269588"
}
```



---

archive/issue_comments_296440.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nSee #14840",
    "created_at": "2016-04-06T18:47:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20374#issuecomment-296440",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:3" align="right">Comment 3</div>

See #14840



---

archive/issue_events_269589.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-04-06T18:47:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20374",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20374#event-269589"
}
```



---

archive/issue_comments_296441.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1 @@\n-**Tarballs**:\n-* https://pypi.python.org/packages/source/B/Babel/Babel-2.2.0.tar.gz\n-* https://pypi.python.org/packages/source/p/pytz/pytz-2016.3.tar.bz2\n+\n``````\n",
    "created_at": "2016-05-13T09:54:26Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20374",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20374#issuecomment-296441",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1 @@
-**Tarballs**:
-* https://pypi.python.org/packages/source/B/Babel/Babel-2.2.0.tar.gz
-* https://pypi.python.org/packages/source/p/pytz/pytz-2016.3.tar.bz2
+
``````

