# Issue 20640: sage-logger: show log(s) each line is going to on stdout

archive/issues_020403.json:
```json
{
    "body": "A minor enhancement to `sage-logger` that I've found incredibly helpful.  Every line output to stdout is prefixed with the name(s) of the log file(s) they are being logged to.\n\nIn particular this helps sort out stdout when there are large number of make processes running in parallel.  It helps to reason about what lines of output are associated with which packages.  I know each package build is logged individually as well, but this still helps understand what's going on at a glance.  It also helps quickly determine which log file to look at if I see something scroll past that I want to investigate further.\n\nReviewer: Volker Braun\n\nAuthor: Erik Bray\n\nBranch: bc0f3daa7ca41f310ee5b21c0bc31e53b3fd3bf9\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/20640\n\n",
    "closed_at": "2016-05-28T12:13:26Z",
    "created_at": "2016-05-20T12:09:41Z",
    "labels": [
        "component: build",
        "minor"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-7.3",
    "title": "sage-logger: show log(s) each line is going to on stdout",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/20640",
    "user": "https://github.com/embray"
}
```
A minor enhancement to `sage-logger` that I've found incredibly helpful.  Every line output to stdout is prefixed with the name(s) of the log file(s) they are being logged to.

In particular this helps sort out stdout when there are large number of make processes running in parallel.  It helps to reason about what lines of output are associated with which packages.  I know each package build is logged individually as well, but this still helps understand what's going on at a glance.  It also helps quickly determine which log file to look at if I see something scroll past that I want to investigate further.

Reviewer: Volker Braun

Author: Erik Bray

Branch: bc0f3daa7ca41f310ee5b21c0bc31e53b3fd3bf9

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/20640





---

archive/issue_comments_375365.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2016-05-20T12:11:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20640",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20640#issuecomment-375365",
    "user": "https://github.com/embray"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_375366.json:
```json
{
    "body": "Changing author from \"\" to \"Erik Bray\"",
    "created_at": "2016-05-20T12:11:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20640",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20640#issuecomment-375366",
    "user": "https://github.com/embray"
}
```

Changing author from "" to "Erik Bray"



---

archive/issue_comments_375367.json:
```json
{
    "body": "<a id='comment:2'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                          |                                |\n|------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------|\n|[c9d0d7c](http://git.sagemath.org/sage.git/commit/?id=c9d0d7cf129b3ad5e66abde2e43273f410584072)|`echo'd string should be quoted`|",
    "created_at": "2016-05-20T13:23:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20640",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20640#issuecomment-375367",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:2'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                          |                                |
|------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------|
|[c9d0d7c](http://git.sagemath.org/sage.git/commit/?id=c9d0d7cf129b3ad5e66abde2e43273f410584072)|`echo'd string should be quoted`|



---

archive/issue_comments_375368.json:
```json
{
    "body": "Changing commit from \"632a6b36f66e5c1250467bbeffc0c40733bd4af0\" to \"c9d0d7cf129b3ad5e66abde2e43273f410584072\"",
    "created_at": "2016-05-20T13:23:52Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20640",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20640#issuecomment-375368",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "632a6b36f66e5c1250467bbeffc0c40733bd4af0" to "c9d0d7cf129b3ad5e66abde2e43273f410584072"



---

archive/issue_comments_375369.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Volker Braun\"",
    "created_at": "2016-05-21T23:03:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20640",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20640#issuecomment-375369",
    "user": "https://github.com/vbraun"
}
```

Changing reviewer from "" to "Volker Braun"



---

archive/issue_comments_375370.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2016-05-21T23:03:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20640",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20640#issuecomment-375370",
    "user": "https://github.com/vbraun"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_comments_375371.json:
```json
{
    "body": "Changing status from positive_review to needs_work.",
    "created_at": "2016-05-22T08:54:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20640",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20640#issuecomment-375371",
    "user": "https://github.com/vbraun"
}
```

Changing status from positive_review to needs_work.



---

archive/issue_comments_375372.json:
```json
{
    "body": "<a id='comment:4'></a>\n\n```\nsage -t --long src/sage/tests/cmdline.py\n**********************************************************************\nFile \"src/sage/tests/cmdline.py\", line 219, in sage.tests.cmdline.test_executable\nFailed example:\n    print(out)\nExpected:\n    Found local metadata for sqlite-...\n    = SQLite =\n    ...\n    SQLite is a software library that implements a self-contained,\n    serverless, zero-configuration, transactional SQL database engine.\n    ...\nGot:\n    [sqlite] Found local metadata for sqlite-autoconf-3130000\n    [sqlite] = SQLite =\n    [sqlite] \n    [sqlite] == Description ==\n    [sqlite] \n    [sqlite] SQLite is a software library that implements a self-contained,\n    [sqlite] serverless, zero-configuration, transactional SQL database engine.\n    [sqlite] \n    [sqlite] == License ==\n    [sqlite] \n    [sqlite] Public Domain\n    [sqlite] \n    [sqlite] == Upstream contact ==\n    [sqlite] \n    [sqlite] * http://www.sqlite.org\n    [sqlite] \n    [sqlite] == Dependencies ==\n    [sqlite] \n    [sqlite] * readline\n    [sqlite] \n    [sqlite] == Special Update/Build Instructions ==\n    [sqlite] \n    [sqlite] * Use the autoconf version of sqlite.\n    <BLANKLINE>\n**********************************************************************\n1 item had failures:\n   1 of 235 in sage.tests.cmdline.test_executable\n    [234 tests, 1 failure, 70.24 s]\n```",
    "created_at": "2016-05-22T08:54:50Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20640",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20640#issuecomment-375372",
    "user": "https://github.com/vbraun"
}
```

<a id='comment:4'></a>

```
sage -t --long src/sage/tests/cmdline.py
**********************************************************************
File "src/sage/tests/cmdline.py", line 219, in sage.tests.cmdline.test_executable
Failed example:
    print(out)
Expected:
    Found local metadata for sqlite-...
    = SQLite =
    ...
    SQLite is a software library that implements a self-contained,
    serverless, zero-configuration, transactional SQL database engine.
    ...
Got:
    [sqlite] Found local metadata for sqlite-autoconf-3130000
    [sqlite] = SQLite =
    [sqlite] 
    [sqlite] == Description ==
    [sqlite] 
    [sqlite] SQLite is a software library that implements a self-contained,
    [sqlite] serverless, zero-configuration, transactional SQL database engine.
    [sqlite] 
    [sqlite] == License ==
    [sqlite] 
    [sqlite] Public Domain
    [sqlite] 
    [sqlite] == Upstream contact ==
    [sqlite] 
    [sqlite] * http://www.sqlite.org
    [sqlite] 
    [sqlite] == Dependencies ==
    [sqlite] 
    [sqlite] * readline
    [sqlite] 
    [sqlite] == Special Update/Build Instructions ==
    [sqlite] 
    [sqlite] * Use the autoconf version of sqlite.
    <BLANKLINE>
**********************************************************************
1 item had failures:
   1 of 235 in sage.tests.cmdline.test_executable
    [234 tests, 1 failure, 70.24 s]
```



---

archive/issue_comments_375373.json:
```json
{
    "body": "<a id='comment:5'></a>\nWas that the only test that changed as a result?  Should be easy enough to update.",
    "created_at": "2016-05-23T12:14:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20640",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20640#issuecomment-375373",
    "user": "https://github.com/embray"
}
```

<a id='comment:5'></a>
Was that the only test that changed as a result?  Should be easy enough to update.



---

archive/issue_comments_375374.json:
```json
{
    "body": "<a id='comment:6'></a>\nSince it seems that `sage-logger` is used in a few places besides just building, maybe this behavior should be enabled only via a switch.",
    "created_at": "2016-05-23T12:15:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20640",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20640#issuecomment-375374",
    "user": "https://github.com/embray"
}
```

<a id='comment:6'></a>
Since it seems that `sage-logger` is used in a few places besides just building, maybe this behavior should be enabled only via a switch.



---

archive/issue_comments_375375.json:
```json
{
    "body": "<a id='comment:7'></a>\nBranch pushed to git repo; I updated commit sha1. New commits:\n|                                                                                                                                          |                                                                                                   |\n|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------|\n|[bc0f3da](http://git.sagemath.org/sage.git/commit/?id=bc0f3daa7ca41f310ee5b21c0bc31e53b3fd3bf9)|`Only prefix log messages if -p is given to sage-logger, but use this throughout the build system.`|",
    "created_at": "2016-05-26T14:32:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20640",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20640#issuecomment-375375",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:7'></a>
Branch pushed to git repo; I updated commit sha1. New commits:
|                                                                                                                                          |                                                                                                   |
|------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------|
|[bc0f3da](http://git.sagemath.org/sage.git/commit/?id=bc0f3daa7ca41f310ee5b21c0bc31e53b3fd3bf9)|`Only prefix log messages if -p is given to sage-logger, but use this throughout the build system.`|



---

archive/issue_comments_375376.json:
```json
{
    "body": "Changing commit from \"c9d0d7cf129b3ad5e66abde2e43273f410584072\" to \"bc0f3daa7ca41f310ee5b21c0bc31e53b3fd3bf9\"",
    "created_at": "2016-05-26T14:32:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20640",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20640#issuecomment-375376",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "c9d0d7cf129b3ad5e66abde2e43273f410584072" to "bc0f3daa7ca41f310ee5b21c0bc31e53b3fd3bf9"



---

archive/issue_comments_375377.json:
```json
{
    "body": "<a id='comment:8'></a>\nI added an optional `-p` flag for applying the line prefix.  This way the previous default behavior is preserved, but the `-p` flag is used throughout the build system where it's most useful.",
    "created_at": "2016-05-26T14:33:03Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20640",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20640#issuecomment-375377",
    "user": "https://github.com/embray"
}
```

<a id='comment:8'></a>
I added an optional `-p` flag for applying the line prefix.  This way the previous default behavior is preserved, but the `-p` flag is used throughout the build system where it's most useful.



---

archive/issue_comments_375378.json:
```json
{
    "body": "Changing status from needs_work to needs_review.",
    "created_at": "2016-05-26T15:24:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20640",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20640#issuecomment-375378",
    "user": "https://github.com/embray"
}
```

Changing status from needs_work to needs_review.



---

archive/issue_comments_375379.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2016-05-27T22:43:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20640",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20640#issuecomment-375379",
    "user": "https://github.com/vbraun"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_events_055581.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-05-28T12:13:26Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/20640",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20640#event-55581"
}
```



---

archive/issue_comments_375380.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2016-05-28T12:13:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20640",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20640#issuecomment-375380",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_comments_375381.json:
```json
{
    "body": "Changing branch from \"u/embray/sage-logger\" to \"bc0f3daa7ca41f310ee5b21c0bc31e53b3fd3bf9\"",
    "created_at": "2016-05-28T12:13:26Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20640",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20640#issuecomment-375381",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/embray/sage-logger" to "bc0f3daa7ca41f310ee5b21c0bc31e53b3fd3bf9"



---

archive/issue_comments_375382.json:
```json
{
    "body": "<a id='comment:12'></a>\nThis causes every single line in the output of `make` to have `[install] ` prepended. Surely, that's not useful and we shouldn't use the `-p` flag for the top-level `sage-logger`.",
    "created_at": "2016-05-29T18:32:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20640",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20640#issuecomment-375382",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:12'></a>
This causes every single line in the output of `make` to have `[install] ` prepended. Surely, that's not useful and we shouldn't use the `-p` flag for the top-level `sage-logger`.



---

archive/issue_comments_375383.json:
```json
{
    "body": "Changing commit from \"bc0f3daa7ca41f310ee5b21c0bc31e53b3fd3bf9\" to \"\"",
    "created_at": "2016-05-29T18:32:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20640",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20640#issuecomment-375383",
    "user": "https://github.com/jdemeyer"
}
```

Changing commit from "bc0f3daa7ca41f310ee5b21c0bc31e53b3fd3bf9" to ""



---

archive/issue_comments_375384.json:
```json
{
    "body": "<a id='comment:13'></a>\nAlso: this causes leading whitespace to be dropped from the logfile, which messes up ASCII-art-style error messages from GCC for example. See #20708.",
    "created_at": "2016-05-29T18:41:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20640",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20640#issuecomment-375384",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:13'></a>
Also: this causes leading whitespace to be dropped from the logfile, which messes up ASCII-art-style error messages from GCC for example. See #20708.
