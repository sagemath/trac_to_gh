...
```json
{
    ...
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366844",
    "user": "https://github.com/saraedum"
}
```
...
```json
{
    "body": "Changing dependencies from \"\" to \"#16342\"",
    "created_at": "2016-03-22T23:39:35Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366845",
    "user": "https://github.com/saraedum"
}
```

Changing dependencies from "" to "#16342"
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366846",
    "user": "https://github.com/saraedum"
}
```
...
```json
{
    "body": "Changing branch from \"\" to \"u/saraedum/use__with_strict_equality_true___to_work_around_hashing_of_p_adics\"",
    "created_at": "2016-03-23T02:39:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366847",
    "user": "https://github.com/saraedum"
}
```

Changing branch from "" to "u/saraedum/use__with_strict_equality_true___to_work_around_hashing_of_p_adics"
...
```json
{
    "body": "Changing branch from \"u/saraedum/use__with_strict_equality_true___to_work_around_hashing_of_p_adics\" to \"\"",
    "created_at": "2016-03-23T02:50:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366848",
    "user": "https://github.com/saraedum"
}
```

Changing branch from "u/saraedum/use__with_strict_equality_true___to_work_around_hashing_of_p_adics" to ""
...
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,4 +1,48 @@\n+Only fixed modulus p-adic numbers can implement an obvious hash functions. Currently some other p-adic elements do which causes horrible bugs:\n \n+```\n+sage: @cached_function\n+....: def is_one(x):\n+....:     return x==1\n+\n+sage: R = Zp(3, 70)\n+sage: is_one(R(1,1))\n+True\n+sage: is_one(R(2^64))\n+True\n+sage: R(2^64)\n+1 + 2*3 + 2*3^2 + 3^3 + 3^4 + 2*3^5 + 3^7 + 2*3^8 + 3^10 + 2*3^11 + 2*3^13 + 3^14 + 2*3^16 + 3^18 + 3^19 + 2*3^20 + 3^21 + 3^23 + 2*3^25 + 3^26 + 2*3^27 + 2*3^28 + 3^29 + 2*3^30 + 2*3^31 + 2*3^34 + 2*3^35 + 2*3^36 + 3^37 + 3^38 + 3^39 + 3^40 + O(3^70)\n+```\n+\n+The problem here is that `==` has been changed so that two numbers are equal if they are equal to the least common precision. In this example, the two elements are equal to precision one and they have the same hash value (at least on my machine).\n+\n+The proposal of this ticket is to a global state which indicates whether we are currently using \"strict precision\", i.e., only considering elements as equal if they are indistinguishable in computations. This is set by wrapping code in `with strict_precision(True):` and should be used when caching takes place. If this is not set, p-adics are unhashable.\n+\n+Here are some alternatives that we're discussed as part of #11895:\n+\n+* Keep it the way it is. (I do not think that this is a good idea. If you do intensive p-adic computations then the birthday paradox will eventually hit you.)\n+* Mark p-adics in a special way so their `==` is not used by `cached_method` and friends. (Not an option, because p-adics may be wrapped inside other objects which will still use their operator `==`.)\n+* Change `==` for p-adics to say whether two numbers are \"really\" equal. (Bad idea. Say you are in a p-adic field. Some algorithm wants to know whether `x` is invertible and says `x==0`. Now this will be false for most `p`-adic zeros.)\n+* Make `__hash__` return a constant. (This would work but will give horrible performance.)\n+* Make p-adics entirely unhashable but add a special `_cache_key` which the caching framework is aware of. (This is how it is currently implemented for p-adics in unramified extensions.) The problem with this solution is that you have to add these `_cache_key` methods to all classes whose `__hash__` might rely on a `p`-adic number somehow (matrices, polynomials, ...) which is ugly.\n+\n+Here are some timings. In all these examples, `X = toric_varieties.dP8()` and we consider `%timeit [X.cohomology_basis(1) for i in range(1000)]`.\n+\n+The current `cachefunc.pyx` (using `_cache_key` in a try-finally-block):\n+\n+```\n+1000 loops, best of 3: 419 \u00b5s per loop\n+```\n+The old version which does not handle unhashable objects at all:\n+\n+```\n+1000 loops, best of 3: 387 \u00b5s per loop\n+```\n+The proposed implementation:\n+\n+```\n+1000 loops, best of 3: 395 \u00b5s per loop\n+```\n \n Summary: Use with strict_equality to work around hashing of p-adics\n \n``````\n",
    "created_at": "2016-03-23T02:50:58Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366849",
    "user": "https://github.com/saraedum"
}
```
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366850",
    "user": "https://github.com/saraedum"
}
```
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366851",
...
```json
{
    "body": "Changing commit from \"\" to \"2ccf5c0ff77a6bf40c403b42c52a18891ae03a4c\"",
    "created_at": "2016-03-24T00:26:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366852",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "" to "2ccf5c0ff77a6bf40c403b42c52a18891ae03a4c"
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366853",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```
...
```json
{
    "body": "Changing commit from \"2ccf5c0ff77a6bf40c403b42c52a18891ae03a4c\" to \"1c93ea34acd15d86717045cf61b71b9db0984182\"",
    "created_at": "2016-03-24T00:26:44Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366854",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "2ccf5c0ff77a6bf40c403b42c52a18891ae03a4c" to "1c93ea34acd15d86717045cf61b71b9db0984182"
...
```json
{
    "body": "Changing commit from \"1c93ea34acd15d86717045cf61b71b9db0984182\" to \"305c28e6f5bbc9f5f5e02e6b2fcde14a56500a98\"",
    "created_at": "2016-03-24T02:17:41Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366855",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "1c93ea34acd15d86717045cf61b71b9db0984182" to "305c28e6f5bbc9f5f5e02e6b2fcde14a56500a98"
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366856",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366857",
    "user": "https://github.com/saraedum"
}
```
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366858",
    "user": "https://github.com/saraedum"
}
```
...
```json
{
    "body": "<a id='comment:13'></a>We should remove the changes to fraction field elements again and move them to #20271.",
    "created_at": "2016-03-24T03:13:02Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366859",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:13'></a>We should remove the changes to fraction field elements again and move them to #20271.
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366860",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```
...
```json
{
    "body": "Changing commit from \"305c28e6f5bbc9f5f5e02e6b2fcde14a56500a98\" to \"ae3e87b827b3d14e899895affae83ee8f592c16f\"",
    "created_at": "2016-03-25T00:06:43Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366861",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "305c28e6f5bbc9f5f5e02e6b2fcde14a56500a98" to "ae3e87b827b3d14e899895affae83ee8f592c16f"
...
```json
{
    "body": "<a id='comment:15'></a>The branch seems to not merge against develop.",
    "created_at": "2016-03-25T00:36:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366862",
    "user": "https://github.com/roed314"
}
```

<a id='comment:15'></a>The branch seems to not merge against develop.
...
```json
{
    "body": "Changing commit from \"ae3e87b827b3d14e899895affae83ee8f592c16f\" to \"f4c5a9db729ccf911517039fdced282588210133\"",
    "created_at": "2016-03-25T10:35:17Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366863",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "ae3e87b827b3d14e899895affae83ee8f592c16f" to "f4c5a9db729ccf911517039fdced282588210133"
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366864",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```
...
```json
{
    "body": "<a id='comment:17'></a>Ok. Fixed.",
    "created_at": "2016-03-25T10:35:56Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366865",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:17'></a>Ok. Fixed.
...
```json
{
    "body": "<a id='comment:18'></a>So, how close is this to Needs Review?",
    "created_at": "2016-03-25T10:51:27Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366866",
    "user": "https://github.com/roed314"
}
```

<a id='comment:18'></a>So, how close is this to Needs Review?
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366867",
    "user": "https://github.com/saraedum"
}
```
...
```json
{
    "body": "<a id='comment:19'></a>You can have a look. A few doctests fail. Let's have the patchbot run it. Most seem to be in the projective minimal model we looked at which seems to be incorrect (i.e., dependent on input formatting.)",
    "created_at": "2016-03-25T13:35:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366868",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:19'></a>You can have a look. A few doctests fail. Let's have the patchbot run it. Most seem to be in the projective minimal model we looked at which seems to be incorrect (i.e., dependent on input formatting.)
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366869",
    "user": "https://github.com/saraedum"
}
```
...
```json
{
    "body": "<a id='comment:20'></a>It seems that I missed quite a few doctests. Mostly trivial changes in printing it seems.",
    "created_at": "2016-03-26T02:20:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366870",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:20'></a>It seems that I missed quite a few doctests. Mostly trivial changes in printing it seems.
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366872",
    "user": "https://github.com/roed314"
}
```
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366874",
    "user": "https://github.com/saraedum"
}
```
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366879",
    "user": "https://github.com/saraedum"
}
```
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366880",
    "user": "https://github.com/nbruin"
}
```
...
```json
{
    "body": "<a id='comment:31'></a>I am slightly confused now. So the scenario you described about weak references does not happen with `weak_dict`, right?\nI do not understand what you mean with \"your callback code\". Are we just talking about weakref callback code?",
    "created_at": "2016-03-31T23:54:15Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366881",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:31'></a>I am slightly confused now. So the scenario you described about weak references does not happen with `weak_dict`, right?
I do not understand what you mean with "your callback code". Are we just talking about weakref callback code?
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366882",
    "user": "https://github.com/nbruin"
}
```
...
```json
{
    "body": "<a id='comment:33'></a>So is there any callback code other than the weakrefs in sage? Right now this seems somewhat artificial to me.",
    "created_at": "2016-04-01T02:35:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366883",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:33'></a>So is there any callback code other than the weakrefs in sage? Right now this seems somewhat artificial to me.
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366885",
    "user": "https://github.com/saraedum"
}
```
...
```json
{
    ...
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20246#event-54987"
}
```
...
```json
{
    "body": "<a id='comment:35'></a>Ok. I am convinced that this is not the way to go. I am essentially out of ideas on how to solve our `==` mess\u2026",
    "created_at": "2016-12-20T21:53:16Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366886",
    "user": "https://github.com/saraedum"
}
```

<a id='comment:35'></a>Ok. I am convinced that this is not the way to go. I am essentially out of ideas on how to solve our `==` mess…
...
```json
{
    "body": "<a id='comment:36'></a>Well, fixed mod and floating point p-adics can be hashed reasonably.  Maybe disable hashing for interval-type p-adics (capped-rel and capped-abs) and add a cache_key?\n\nOn a related note, real intervals in Sage are currently hashable with similar consequences.",
    "created_at": "2016-12-23T07:20:32Z",
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366887",
    "user": "https://github.com/roed314"
}
```

<a id='comment:36'></a>Well, fixed mod and floating point p-adics can be hashed reasonably.  Maybe disable hashing for interval-type p-adics (capped-rel and capped-abs) and add a cache_key?
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/20246#event-54988"
}
```
...
```json
{
    ...
    "issue": "https://github.com/sagemath/sagetest/issues/20246",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/20246#issuecomment-366888",
