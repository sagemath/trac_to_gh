# Issue 20441: Fast conversion  ℚ[√a] → arb balls

archive/issues_020204.json:
```json
{
    "assignees": [],
    "body": "Before:\n\n```\nsage: QQi.<i> = QuadraticField(-1)\nsage: %timeit CBF(i)\nThe slowest run took 5.04 times longer than the fastest. This could mean that an intermediate result is being cached.\n100 loops, best of 3: 2.45 ms per loop\nsage: NF.<a> = QuadraticField(2)\nsage: RBF((a-1)^100)\n[+/- 3.78e+22]\n```\nAfter:\n\n```\nsage: QQi.<i> = QuadraticField(-1)\nsage: %timeit CBF(i)\nThe slowest run took 1175.13 times longer than the fastest. This could mean that an intermediate result is being cached.\n1000000 loops, best of 3: 791 ns per loop\nsage: NF.<a> = QuadraticField(2)\nsage: RBF((a-1)^100)\n[5.27753918069144e-39 +/- 2.15e-54]\n```\n\nBranch/Commit: **[f8e16b6](https://github.com/sagemath/sagetrac-mirror/commit/f8e16b68014544b96698dc11c8fa90e4a187e3d3)**\n\nReviewer: **Fredrik Johansson**\n\nAuthor: **Marc Mezzarobba**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20441_\n\n",
    "closed_at": "2016-04-15T21:15:06Z",
    "created_at": "2016-04-14T07:50:01Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-7.2",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fast conversion  \u211a[\u221aa] \u2192 arb balls",
    "type": "issue",
    "updated_at": "2016-04-15T21:15:06Z",
    "url": "https://github.com/sagemath/sage/issues/20441",
    "user": "https://github.com/mezzarobba"
}
```
Before:

```
sage: QQi.<i> = QuadraticField(-1)
sage: %timeit CBF(i)
The slowest run took 5.04 times longer than the fastest. This could mean that an intermediate result is being cached.
100 loops, best of 3: 2.45 ms per loop
sage: NF.<a> = QuadraticField(2)
sage: RBF((a-1)^100)
[+/- 3.78e+22]
```
After:

```
sage: QQi.<i> = QuadraticField(-1)
sage: %timeit CBF(i)
The slowest run took 1175.13 times longer than the fastest. This could mean that an intermediate result is being cached.
1000000 loops, best of 3: 791 ns per loop
sage: NF.<a> = QuadraticField(2)
sage: RBF((a-1)^100)
[5.27753918069144e-39 +/- 2.15e-54]
```

Branch/Commit: **[f8e16b6](https://github.com/sagemath/sagetrac-mirror/commit/f8e16b68014544b96698dc11c8fa90e4a187e3d3)**

Reviewer: **Fredrik Johansson**

Author: **Marc Mezzarobba**

_Issue created by migration from https://trac.sagemath.org/ticket/20441_





---

archive/issue_events_270321.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2016-04-14T07:50:01Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20441",
    "milestone_number": null,
    "milestone_title": "sage-7.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20441#event-270321"
}
```



---

archive/issue_events_270322.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2016-04-14T07:50:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20441",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20numerical",
    "label_color": "0000ff",
    "label_name": "component: numerical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20441#event-270322"
}
```



---

archive/issue_events_270323.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2016-04-14T07:50:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20441",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20441#event-270323"
}
```



---

archive/issue_events_270324.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2016-04-14T07:50:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20441",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20441#event-270324"
}
```



---

archive/issue_events_270325.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2016-04-14T07:53:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20441",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20441#event-270325"
}
```



---

archive/issue_events_270326.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2016-04-14T09:36:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20441",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20numerical",
    "label_color": "0000ff",
    "label_name": "component: numerical",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20441#event-270326"
}
```



---

archive/issue_events_270327.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2016-04-14T09:36:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20441",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20fields",
    "label_color": "0000ff",
    "label_name": "component: number fields",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20441#event-270327"
}
```



---

archive/issue_comments_297684.json:
```json
{
    "body": "Reviewer: **Fredrik Johansson**",
    "created_at": "2016-04-14T23:46:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20441",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20441#issuecomment-297684",
    "user": "https://github.com/fredrik-johansson"
}
```

Reviewer: **Fredrik Johansson**



---

archive/issue_comments_297685.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nNothing to complain about.",
    "created_at": "2016-04-14T23:46:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20441",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20441#issuecomment-297685",
    "user": "https://github.com/fredrik-johansson"
}
```

<div id="comment:3" align="right">Comment 3</div>

Nothing to complain about.



---

archive/issue_events_270328.json:
```json
{
    "actor": "https://github.com/fredrik-johansson",
    "created_at": "2016-04-14T23:46:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20441",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20441#event-270328"
}
```



---

archive/issue_events_270329.json:
```json
{
    "actor": "https://github.com/fredrik-johansson",
    "created_at": "2016-04-14T23:46:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20441",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20441#event-270329"
}
```



---

archive/issue_comments_297686.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nNice commit message ;-)",
    "created_at": "2016-04-15T07:32:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20441",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20441#issuecomment-297686",
    "user": "https://github.com/vbraun"
}
```

<div id="comment:4" align="right">Comment 4</div>

Nice commit message ;-)



---

archive/issue_comments_297687.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThanks for the review!",
    "created_at": "2016-04-15T07:38:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20441",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20441#issuecomment-297687",
    "user": "https://github.com/mezzarobba"
}
```

<div id="comment:5" align="right">Comment 5</div>

Thanks for the review!



---

archive/issue_events_270330.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2016-04-15T07:43:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20441",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20441#event-270330"
}
```



---

archive/issue_events_270331.json:
```json
{
    "actor": "https://github.com/sagetrac-git",
    "created_at": "2016-04-15T07:43:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20441",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20441#event-270331"
}
```



---

archive/issue_comments_297688.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nBranch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f8e16b68014544b96698dc11c8fa90e4a187e3d3\">f8e16b6</a></td><td><code>misplaced fmpz_clear</code></td></tr></table>\n",
    "created_at": "2016-04-15T07:43:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20441",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20441#issuecomment-297688",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:6" align="right">Comment 6</div>

Branch pushed to git repo; I updated commit sha1 and set ticket back to needs_review. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f8e16b68014544b96698dc11c8fa90e4a187e3d3">f8e16b6</a></td><td><code>misplaced fmpz_clear</code></td></tr></table>




---

archive/issue_comments_297689.json:
```json
{
    "body": "Changed commit from **[ba857dd](https://github.com/sagemath/sagetrac-mirror/commit/ba857dd9802f7712838cbd15896a8ac755a0a503)** to **[f8e16b6](https://github.com/sagemath/sagetrac-mirror/commit/f8e16b68014544b96698dc11c8fa90e4a187e3d3)**",
    "created_at": "2016-04-15T07:43:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20441",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20441#issuecomment-297689",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[ba857dd](https://github.com/sagemath/sagetrac-mirror/commit/ba857dd9802f7712838cbd15896a8ac755a0a503)** to **[f8e16b6](https://github.com/sagemath/sagetrac-mirror/commit/f8e16b68014544b96698dc11c8fa90e4a187e3d3)**



---

archive/issue_comments_297690.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nOops, I just noticed that I used a flint integer after clearing it. Since the tests still pass, I am taking the liberty to set the ticket back to `positive_review`.",
    "created_at": "2016-04-15T07:47:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20441",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20441#issuecomment-297690",
    "user": "https://github.com/mezzarobba"
}
```

<div id="comment:7" align="right">Comment 7</div>

Oops, I just noticed that I used a flint integer after clearing it. Since the tests still pass, I am taking the liberty to set the ticket back to `positive_review`.



---

archive/issue_events_270332.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2016-04-15T07:47:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20441",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20441#event-270332"
}
```



---

archive/issue_events_270333.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2016-04-15T07:47:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20441",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20441#event-270333"
}
```



---

archive/issue_comments_297691.json:
```json
{
    "body": "Changed branch from **[u/mmezzarobba/QNFelt_to_ball](https://github.com/sagemath/sagetrac-mirror/tree/u/mmezzarobba/QNFelt_to_ball)** to **[f8e16b6](https://github.com/sagemath/sagetrac-mirror/commit/f8e16b68014544b96698dc11c8fa90e4a187e3d3)**",
    "created_at": "2016-04-15T21:15:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20441",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20441#issuecomment-297691",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/mmezzarobba/QNFelt_to_ball](https://github.com/sagemath/sagetrac-mirror/tree/u/mmezzarobba/QNFelt_to_ball)** to **[f8e16b6](https://github.com/sagemath/sagetrac-mirror/commit/f8e16b68014544b96698dc11c8fa90e4a187e3d3)**



---

archive/issue_events_270334.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2016-04-15T21:15:06Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20441",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20441#event-270334"
}
```



---

archive/issue_events_270335.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "3e34a0c30fef3fde8ac48831086e22889af04150",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2016-04-15T21:15:06Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20441",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20441#event-270335"
}
```
