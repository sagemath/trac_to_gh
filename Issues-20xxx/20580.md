# Issue 20580: Make __doc__ a descriptor instead of using _sage_doc_()

archive/issues_020343.json:
```json
{
    "assignees": [],
    "body": "Instead of supporting a custom `_sage_doc_()` method, we should just use standard Python tricks to change `__doc__`.\n\nThis actually breaks in Cython due to some Cython bug(?) with decorators. While trying to fix that bug, I broke `@staticmethod` because of an unrelated bug http://trac.cython.org/ticket/880\n\nReviewer: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/20580_\n\n",
    "closed_at": "2017-04-18T15:52:51Z",
    "created_at": "2016-05-09T16:38:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/duplicate",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Make __doc__ a descriptor instead of using _sage_doc_()",
    "type": "issue",
    "updated_at": "2017-04-18T15:52:51Z",
    "url": "https://github.com/sagemath/sage/issues/20580",
    "user": "https://github.com/jdemeyer"
}
```
Instead of supporting a custom `_sage_doc_()` method, we should just use standard Python tricks to change `__doc__`.

This actually breaks in Cython due to some Cython bug(?) with decorators. While trying to fix that bug, I broke `@staticmethod` because of an unrelated bug http://trac.cython.org/ticket/880

Reviewer: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/20580_





---

archive/issue_events_272029.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-05-09T16:38:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/20580",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20580#event-272029"
}
```



---

archive/issue_events_272030.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-05-09T16:38:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20580",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20580#event-272030"
}
```



---

archive/issue_events_272031.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-05-09T16:38:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20580",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20580#event-272031"
}
```



---

archive/issue_events_272032.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-05-09T16:38:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20580",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20580#event-272032"
}
```



---

archive/issue_events_272033.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2016-05-09T16:38:40Z",
    "event": "renamed",
    "issue": "https://github.com/sagemath/sage/issues/20580",
    "title_is": "Make __doc__ a descriptor instead of using _sage_doc_()",
    "title_was": "Make __doc__ a descriptor instead of _sage_doc_",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20580#event-272033"
}
```



---

archive/issue_comments_300112.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1,3 @@\n Instead of supporting a custom `_sage_doc_()` method, we should just use standard Python tricks to change `__doc__`.\n+\n+This actually breaks in Cython due to some Cython bug(?) with decorators. While trying to fix that bug, I broke `@staticmethod` because of an unrelated bug http://trac.cython.org/ticket/880\n``````\n",
    "created_at": "2016-05-12T15:10:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20580",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20580#issuecomment-300112",
    "user": "https://github.com/jdemeyer"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1,3 @@
 Instead of supporting a custom `_sage_doc_()` method, we should just use standard Python tricks to change `__doc__`.
+
+This actually breaks in Cython due to some Cython bug(?) with decorators. While trying to fix that bug, I broke `@staticmethod` because of an unrelated bug http://trac.cython.org/ticket/880
``````




---

archive/issue_events_272034.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-18T15:52:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/20580",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20580#event-272034"
}
```



---

archive/issue_comments_300113.json:
```json
{
    "body": "Reviewer: **Jeroen Demeyer**",
    "created_at": "2017-04-18T15:52:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20580",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20580#issuecomment-300113",
    "user": "https://github.com/jdemeyer"
}
```

Reviewer: **Jeroen Demeyer**



---

archive/issue_events_272035.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-18T15:52:51Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/20580",
    "milestone_number": null,
    "milestone_title": "sage-7.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20580#event-272035"
}
```



---

archive/issue_events_272036.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-18T15:52:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/20580",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20580#event-272036"
}
```



---

archive/issue_events_272037.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-18T15:52:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20580",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20580#event-272037"
}
```



---

archive/issue_comments_300114.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nDuplicate of #22611",
    "created_at": "2017-04-18T15:52:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20580",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20580#issuecomment-300114",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:3'>Comment 3:</a>
Duplicate of #22611



---

archive/issue_events_272038.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2017-04-18T15:52:51Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/20580",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/20580#event-272038"
}
```



---

archive/issue_comments_300115.json:
```json
{
    "body": "Changed author from **Jeroen Demeyer** to none",
    "created_at": "2017-04-18T15:52:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/20580",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/20580#issuecomment-300115",
    "user": "https://github.com/jdemeyer"
}
```

Changed author from **Jeroen Demeyer** to none
