# Issue 25893: some care for sphinx warnings

archive/issues_025656.json:
```json
{
    "assignees": [],
    "body": "but not all of them..\n\nCC:  @tscrim @jdemeyer @embray\n\nBranch/Commit: **[503c1e8](https://github.com/sagemath/sagetrac-mirror/commit/503c1e837fe4f13b9856115a614f14d9b432584e)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Fr\u00e9d\u00e9ric Chapoton**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25893_\n\n",
    "closed_at": "2018-08-05T08:15:51Z",
    "created_at": "2018-07-21T11:31:10Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.4",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "some care for sphinx warnings",
    "type": "issue",
    "updated_at": "2018-08-05T08:15:51Z",
    "url": "https://github.com/sagemath/sage/issues/25893",
    "user": "https://github.com/fchapoton"
}
```
but not all of them..

CC:  @tscrim @jdemeyer @embray

Branch/Commit: **[503c1e8](https://github.com/sagemath/sagetrac-mirror/commit/503c1e837fe4f13b9856115a614f14d9b432584e)**

Reviewer: **Travis Scrimshaw**

Author: **Frédéric Chapoton**

_Issue created by migration from https://trac.sagemath.org/ticket/25893_





---

archive/issue_events_339803.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-07-21T11:31:10Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25893",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25893#event-339803"
}
```



---

archive/issue_events_339804.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-07-21T11:31:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25893",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25893#event-339804"
}
```



---

archive/issue_events_339805.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-07-21T11:31:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25893",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25893#event-339805"
}
```



---

archive/issue_events_339806.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-07-21T11:31:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25893",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25893#event-339806"
}
```



---

archive/issue_events_339807.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-07-21T12:52:19Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25893",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25893#event-339807"
}
```



---

archive/issue_comments_400860.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nan improvement, even if only partial",
    "created_at": "2018-07-21T12:52:19Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25893",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25893#issuecomment-400860",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:1" align="right">Comment 1</div>

an improvement, even if only partial



---

archive/issue_comments_400861.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nWhat do you mean with \"sphinx warnings\"?",
    "created_at": "2018-07-21T14:41:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25893",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25893#issuecomment-400861",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:2" align="right">Comment 2</div>

What do you mean with "sphinx warnings"?



---

archive/issue_comments_400862.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nThere are currently around 27 warnings similar to\n\n```\n[dochtml] [manifolds] /home/u1/chapoton/sage/src/doc/en/reference/manifolds/index.rst:4: WARNING: undefined label: tensors-on-free-modules (if the link has no caption the label must precede a section header)\n```\nwhen building the html doc. This ticket reduces their number to 16.\n\nhttps://patchbot.sagemath.org/log/0/Ubuntu/16.04/x86_64/4.4.0-130-generic/atlas/2018-07-21%2012:06:19?plugin=docbuild",
    "created_at": "2018-07-21T16:02:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25893",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25893#issuecomment-400862",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">Comment 3</div>

There are currently around 27 warnings similar to

```
[dochtml] [manifolds] /home/u1/chapoton/sage/src/doc/en/reference/manifolds/index.rst:4: WARNING: undefined label: tensors-on-free-modules (if the link has no caption the label must precede a section header)
```
when building the html doc. This ticket reduces their number to 16.

https://patchbot.sagemath.org/log/0/Ubuntu/16.04/x86_64/4.4.0-130-generic/atlas/2018-07-21%2012:06:19?plugin=docbuild



---

archive/issue_events_339808.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-07-22T18:16:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25893",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25893#event-339808"
}
```



---

archive/issue_events_339809.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-07-22T18:16:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25893",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25893#event-339809"
}
```



---

archive/issue_comments_400863.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2018-07-22T18:16:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25893",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25893#issuecomment-400863",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_400864.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nLGTM.",
    "created_at": "2018-07-22T18:16:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25893",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25893#issuecomment-400864",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:4" align="right">Comment 4</div>

LGTM.



---

archive/issue_events_339810.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-08-05T08:15:51Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25893",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25893#event-339810"
}
```



---

archive/issue_events_339811.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-08-05T08:15:51Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25893",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25893#event-339811"
}
```



---

archive/issue_comments_400865.json:
```json
{
    "body": "Changed branch from **[u/chapoton/sphinx1](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/sphinx1)** to **[503c1e8](https://github.com/sagemath/sagetrac-mirror/commit/503c1e837fe4f13b9856115a614f14d9b432584e)**",
    "created_at": "2018-08-05T08:15:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25893",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25893#issuecomment-400865",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/chapoton/sphinx1](https://github.com/sagemath/sagetrac-mirror/tree/u/chapoton/sphinx1)** to **[503c1e8](https://github.com/sagemath/sagetrac-mirror/commit/503c1e837fe4f13b9856115a614f14d9b432584e)**
