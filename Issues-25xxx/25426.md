...
```json
{
    "body": "I have my suspicions that it is not in `Permutation` as elements of `PermutationGroup` are stored in GAP as lists and converted to cycles for their print representation. So my guess is the creation of the permutation group elements is going wrong somehow.",
...
```json
{
    "body": "well, should Permutation throw an error if it encountered wrong order of elements in a cycle?\n\nspecifically, you see that the group is fine, while convering its elements into lists goes nuts...",
...
```json
{
    "body": "it is a typical GAP thing, unsorted lists output as orbits.",
...
```json
{
    "body": "According to Travis, this fixes the bug (and it indeed does), I haven't really gotten into the hack, though...\n\n---\nNew commits:",
...
```json
{
    "body": "This gets around the domain issue I noted above. Dima, is this change is okay with you?",
