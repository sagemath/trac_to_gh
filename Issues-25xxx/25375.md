# Issue 25375: One-off doctest failures with matplotlib again

archive/issues_025138.json:
```json
{
    "assignees": [],
    "body": "With Sage 8.3.beta1, I have seen the same failures as reported in #20222:\n\n```\nsage -t --long --warn-long 224.1 src/sage/plot/hyperbolic_regular_polygon.py  # 1 doctest failed\nsage -t --long --warn-long 224.1 src/sage/repl/rich_output/backend_sagenb.py  # 1 doctest failed\nsage -t --long --warn-long 224.1 src/sage/combinat/crystals/crystals.py  # 1 doctest failed\nsage -t --long --warn-long 224.1 src/sage/game_theory/matching_game.py  # 1 doctest failed\n```\nwhere a typical example is\n\n```\nFile \"src/sage/plot/hyperbolic_regular_polygon.py\", line 244, in sage.plot.hyperbolic_regular_polygon.hyperbolic_regular_polygon\nFailed example:\n    g.plot()\nExpected:\n    Graphics object consisting of 1 graphics primitive\nGot:\n    doctest:warning\n      File \"/Users/jpalmier/Desktop/Sage_stuff/sage_builds/VANILLA/sage-8.3.beta1/local/lib/python2.7/threading.py\", line 774, in __bootstrap\n        self.__bootstrap_inner()\n      File \"/Users/jpalmier/Desktop/Sage_stuff/sage_builds/VANILLA/sage-8.3.beta1/local/lib/python2.7/threading.py\", line 801, in __bootstrap_inner\n        self.run()\n      File \"/Users/jpalmier/Desktop/Sage_stuff/sage_builds/VANILLA/sage-8.3.beta1/local/lib/python2.7/threading.py\", line 1073, in run\n        self.function(*self.args, **self.kwargs)\n      File \"/Users/jpalmier/Desktop/Sage_stuff/sage_builds/VANILLA/sage-8.3.beta1/local/lib/python2.7/site-packages/matplotlib/font_manager.py\", line 279, in <lambda>\n        'Matplotlib is building the font cache using fc-list. '\n    :\n    UserWarning: Matplotlib is building the font cache using fc-list. This may take a moment.\n    Graphics object consisting of 1 graphics primitive\n```\n\nComponent: **doctest framework**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25375_\n\n",
    "closed_at": "2018-09-04T18:45:45Z",
    "created_at": "2018-05-16T18:28:14Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/c%3A%20doctest%20framework",
        "https://github.com/sagemath/sage/labels/worksforme"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "One-off doctest failures with matplotlib again",
    "type": "issue",
    "updated_at": "2018-09-04T18:45:45Z",
    "url": "https://github.com/sagemath/sage/issues/25375",
    "user": "https://github.com/jhpalmieri"
}
```
With Sage 8.3.beta1, I have seen the same failures as reported in #20222:

```
sage -t --long --warn-long 224.1 src/sage/plot/hyperbolic_regular_polygon.py  # 1 doctest failed
sage -t --long --warn-long 224.1 src/sage/repl/rich_output/backend_sagenb.py  # 1 doctest failed
sage -t --long --warn-long 224.1 src/sage/combinat/crystals/crystals.py  # 1 doctest failed
sage -t --long --warn-long 224.1 src/sage/game_theory/matching_game.py  # 1 doctest failed
```
where a typical example is

```
File "src/sage/plot/hyperbolic_regular_polygon.py", line 244, in sage.plot.hyperbolic_regular_polygon.hyperbolic_regular_polygon
Failed example:
    g.plot()
Expected:
    Graphics object consisting of 1 graphics primitive
Got:
    doctest:warning
      File "/Users/jpalmier/Desktop/Sage_stuff/sage_builds/VANILLA/sage-8.3.beta1/local/lib/python2.7/threading.py", line 774, in __bootstrap
        self.__bootstrap_inner()
      File "/Users/jpalmier/Desktop/Sage_stuff/sage_builds/VANILLA/sage-8.3.beta1/local/lib/python2.7/threading.py", line 801, in __bootstrap_inner
        self.run()
      File "/Users/jpalmier/Desktop/Sage_stuff/sage_builds/VANILLA/sage-8.3.beta1/local/lib/python2.7/threading.py", line 1073, in run
        self.function(*self.args, **self.kwargs)
      File "/Users/jpalmier/Desktop/Sage_stuff/sage_builds/VANILLA/sage-8.3.beta1/local/lib/python2.7/site-packages/matplotlib/font_manager.py", line 279, in <lambda>
        'Matplotlib is building the font cache using fc-list. '
    :
    UserWarning: Matplotlib is building the font cache using fc-list. This may take a moment.
    Graphics object consisting of 1 graphics primitive
```

Component: **doctest framework**

_Issue created by migration from https://trac.sagemath.org/ticket/25375_





---

archive/issue_events_351147.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2018-05-16T18:28:14Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25375",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25375#event-351147"
}
```



---

archive/issue_events_351148.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2018-05-16T18:28:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25375",
    "label": "https://github.com/sagemath/sage/labels/c%3A%20doctest%20framework",
    "label_color": "000080",
    "label_name": "c: doctest framework",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25375#event-351148"
}
```



---

archive/issue_events_351149.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2018-05-16T18:28:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25375",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25375#event-351149"
}
```



---

archive/issue_events_351150.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2018-05-16T18:28:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25375",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25375#event-351150"
}
```



---

archive/issue_comments_388396.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">comment:1</div>\n\nNaturally I can't figure out how to reproduce this. The only time it happened, I built from a fresh 8.3.beta1 tarball on a machine on which I have built and run Sage many times. The file `.sage/matplotlib-1.5.1/fontList.json` has a recent modification time. I tried rebuilding from scratch and I tried deleting the directory `./sage/matplotlib-1.5.1/`, but I can not reproduce the error.",
    "created_at": "2018-05-17T20:59:48Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25375",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25375#issuecomment-388396",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:1" align="right">comment:1</div>

Naturally I can't figure out how to reproduce this. The only time it happened, I built from a fresh 8.3.beta1 tarball on a machine on which I have built and run Sage many times. The file `.sage/matplotlib-1.5.1/fontList.json` has a recent modification time. I tried rebuilding from scratch and I tried deleting the directory `./sage/matplotlib-1.5.1/`, but I can not reproduce the error.



---

archive/issue_events_351151.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-08-03T19:20:18Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25375",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25375#event-351151"
}
```



---

archive/issue_events_351152.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-08-03T19:20:18Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25375",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25375#event-351152"
}
```



---

archive/issue_comments_388397.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">comment:2</div>\n\nupdate milestone 8.3 -> 8.4",
    "created_at": "2018-08-03T19:20:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25375",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25375#issuecomment-388397",
    "user": "https://github.com/videlec"
}
```

<div id="comment:2" align="right">comment:2</div>

update milestone 8.3 -> 8.4



---

archive/issue_comments_388398.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">comment:3</div>\n\nNot sure what went wrong. I haven't seen this error since then, so I rather close this as \"cosmic ray\" or whatever.",
    "created_at": "2018-08-07T10:41:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25375",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25375#issuecomment-388398",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:3" align="right">comment:3</div>

Not sure what went wrong. I haven't seen this error since then, so I rather close this as "cosmic ray" or whatever.



---

archive/issue_events_351153.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2018-08-07T15:25:58Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25375",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25375#event-351153"
}
```



---

archive/issue_events_351154.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2018-08-07T15:25:58Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25375",
    "label": "https://github.com/sagemath/sage/labels/p%3A%203%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p: 3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25375#event-351154"
}
```



---

archive/issue_events_351155.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2018-08-07T15:25:58Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25375",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25375#event-351155"
}
```



---

archive/issue_comments_388399.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">comment:4</div>\n\nSure, let's close it, since I can't reproduce it, either.",
    "created_at": "2018-08-07T15:25:58Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25375",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25375#issuecomment-388399",
    "user": "https://github.com/jhpalmieri"
}
```

<div id="comment:4" align="right">comment:4</div>

Sure, let's close it, since I can't reproduce it, either.



---

archive/issue_events_351156.json:
```json
{
    "actor": "https://github.com/jhpalmieri",
    "created_at": "2018-08-07T15:26:14Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25375",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25375#event-351156"
}
```



---

archive/issue_events_351157.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-09-04T18:45:45Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25375",
    "label": "https://github.com/sagemath/sage/labels/worksforme",
    "label_color": "a6a6a6",
    "label_name": "worksforme",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25375#event-351157"
}
```



---

archive/issue_events_351158.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-09-04T18:45:45Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25375",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25375#event-351158"
}
```



---

archive/issue_events_351159.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-09-04T18:45:45Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25375",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25375#event-351159"
}
```
