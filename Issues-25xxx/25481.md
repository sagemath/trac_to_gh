# Issue 25481: Broken link in documentation

archive/issues_025244.json:
```json
{
    "assignees": [],
    "body": "The link to the first reference on LLT symmetric functions does not work\n\nhttp://doc.sagemath.org/html/en/reference/combinat/sage/combinat/sf/llt.html\n\nThe correct link is arxiv:q-alg/9512031\n\nIf I knew how to correct the documentation I would do so.\n\nAuthor: **Bruce**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25481_\n\n",
    "closed_at": "2018-11-08T16:03:23Z",
    "created_at": "2018-05-31T12:48:16Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/duplicate"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Broken link in documentation",
    "type": "issue",
    "updated_at": "2018-11-08T16:03:23Z",
    "url": "https://github.com/sagemath/sage/issues/25481",
    "user": "https://github.com/BruceWestbury"
}
```
The link to the first reference on LLT symmetric functions does not work

http://doc.sagemath.org/html/en/reference/combinat/sage/combinat/sf/llt.html

The correct link is arxiv:q-alg/9512031

If I knew how to correct the documentation I would do so.

Author: **Bruce**

_Issue created by migration from https://trac.sagemath.org/ticket/25481_





---

archive/issue_events_334546.json:
```json
{
    "actor": "https://github.com/BruceWestbury",
    "created_at": "2018-05-31T12:48:16Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25481",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25481#event-334546"
}
```



---

archive/issue_events_334547.json:
```json
{
    "actor": "https://github.com/BruceWestbury",
    "created_at": "2018-05-31T12:48:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25481",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25481#event-334547"
}
```



---

archive/issue_events_334548.json:
```json
{
    "actor": "https://github.com/BruceWestbury",
    "created_at": "2018-05-31T12:48:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25481",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25481#event-334548"
}
```



---

archive/issue_events_334549.json:
```json
{
    "actor": "https://github.com/BruceWestbury",
    "created_at": "2018-05-31T12:48:16Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25481",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25481#event-334549"
}
```



---

archive/issue_comments_393459.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\nAre you familiar with git ? Have you read the developer guide ?\n\nhttps://doc.sagemath.org/html/en/developer/\n\nShort instructions: \n\n#(1) clone the git repo (git clone)\n\n#(2) modify the file and commit (git commit -a)\n\n#(3) push the git branch here (git push trac HEAD:u/gh-BruceWestbury/branchname)",
    "created_at": "2018-05-31T18:14:52Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25481#issuecomment-393459",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:1" align="right">Comment 1</div>

Are you familiar with git ? Have you read the developer guide ?

https://doc.sagemath.org/html/en/developer/

Short instructions: 

#(1) clone the git repo (git clone)

#(2) modify the file and commit (git commit -a)

#(3) push the git branch here (git push trac HEAD:u/gh-BruceWestbury/branchname)



---

archive/issue_comments_393460.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nOK. I don't know which file to modify.",
    "created_at": "2018-05-31T18:48:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25481#issuecomment-393460",
    "user": "https://github.com/BruceWestbury"
}
```

<div id="comment:2" align="right">Comment 2</div>

OK. I don't know which file to modify.



---

archive/issue_comments_393461.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nthe documentation is build from the source files in src/sage, in this case : \n\n```\nsrc/sage/combinat/sf/llt.py\n```",
    "created_at": "2018-05-31T19:16:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25481#issuecomment-393461",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">Comment 3</div>

the documentation is build from the source files in src/sage, in this case : 

```
src/sage/combinat/sf/llt.py
```



---

archive/issue_events_334550.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-06-07T06:25:01Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25481",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25481#event-334550"
}
```



---

archive/issue_events_334551.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-06-07T06:25:01Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25481",
    "label": "https://github.com/sagemath/sage/labels/p5%20%E2%80%93%20trivial",
    "label_color": "fff9e5",
    "label_name": "p5 \u2013 trivial",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25481#event-334551"
}
```



---

archive/issue_events_334552.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-06-07T06:25:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25481",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25481#event-334552"
}
```



---

archive/issue_events_334553.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-06-07T06:25:01Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25481",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25481#event-334553"
}
```



---

archive/issue_comments_393462.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nduplicate of #25507",
    "created_at": "2018-06-07T06:25:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25481",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25481#issuecomment-393462",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:4" align="right">Comment 4</div>

duplicate of #25507



---

archive/issue_events_334554.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-06-07T06:25:09Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25481",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25481#event-334554"
}
```



---

archive/issue_events_334555.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-06-07T06:25:09Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25481",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25481#event-334555"
}
```



---

archive/issue_events_334556.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-11-08T16:03:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25481",
    "label": "https://github.com/sagemath/sage/labels/duplicate",
    "label_color": "a6a6a6",
    "label_name": "duplicate",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25481#event-334556"
}
```



---

archive/issue_events_334557.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-11-08T16:03:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25481",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25481#event-334557"
}
```



---

archive/issue_events_334558.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-11-08T16:03:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25481",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25481#event-334558"
}
```
