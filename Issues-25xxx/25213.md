# Issue 25213: Sort terms in DifferentialForm repr/latex by coordinate index

archive/issues_024976.json:
```json
{
    "assignees": [],
    "body": "This update is to code that is deprecated since #24444.  That said, this seems like an obvious thing to do and is a better deal for anyone still using this code.\n\nOr, if nothing else, this fix is needed for the tests to pass on Python 3.\n\nCC:  @slel\n\nBranch/Commit: **[065effd](https://github.com/sagemath/sagetrac-mirror/commit/065effd37caf4b80ff4f1abcdc86400dc4a8313c)**\n\nReviewer: **Travis Scrimshaw**\n\nAuthor: **Erik Bray**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25213_\n\n",
    "closed_at": "2018-05-18T17:49:18Z",
    "created_at": "2018-04-19T09:24:03Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20python3",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Sort terms in DifferentialForm repr/latex by coordinate index",
    "type": "issue",
    "updated_at": "2018-05-18T17:49:18Z",
    "url": "https://github.com/sagemath/sage/issues/25213",
    "user": "https://github.com/embray"
}
```
This update is to code that is deprecated since #24444.  That said, this seems like an obvious thing to do and is a better deal for anyone still using this code.

Or, if nothing else, this fix is needed for the tests to pass on Python 3.

CC:  @slel

Branch/Commit: **[065effd](https://github.com/sagemath/sagetrac-mirror/commit/065effd37caf4b80ff4f1abcdc86400dc4a8313c)**

Reviewer: **Travis Scrimshaw**

Author: **Erik Bray**

_Issue created by migration from https://trac.sagemath.org/ticket/25213_





---

archive/issue_events_330947.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-19T09:24:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25213",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25213#event-330947"
}
```



---

archive/issue_events_330948.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-19T09:24:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25213",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20python3",
    "label_color": "000080",
    "label_name": "component: python3",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25213#event-330948"
}
```



---

archive/issue_events_330949.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-19T09:24:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25213",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25213#event-330949"
}
```



---

archive/issue_events_330950.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-19T09:24:03Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25213",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25213#event-330950"
}
```



---

archive/issue_events_330951.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-19T09:24:35Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25213",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25213#event-330951"
}
```



---

archive/issue_comments_388212.json:
```json
{
    "body": "Reviewer: **Travis Scrimshaw**",
    "created_at": "2018-04-19T21:55:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25213",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25213#issuecomment-388212",
    "user": "https://github.com/tscrim"
}
```

Reviewer: **Travis Scrimshaw**



---

archive/issue_comments_388213.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nLGTM.",
    "created_at": "2018-04-19T21:55:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25213",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25213#issuecomment-388213",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:2" align="right">Comment 2</div>

LGTM.



---

archive/issue_events_330952.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-04-19T21:55:13Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25213",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25213#event-330952"
}
```



---

archive/issue_events_330953.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-04-19T21:55:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25213",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25213#event-330953"
}
```



---

archive/issue_comments_388214.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1,3 +1,3 @@\n-This updated is to code that is deprecated since #24444.  That said, this seems like an obvious thing to do and is a better deal for anyone still using this code.\n+This update is to code that is deprecated since #24444.  That said, this seems like an obvious thing to do and is a better deal for anyone still using this code.\n \n Or, if nothing else, this fix is needed for the tests to pass on Python 3.\n``````\n",
    "created_at": "2018-04-20T10:53:55Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25213",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25213#issuecomment-388214",
    "user": "https://github.com/slel"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1,3 +1,3 @@
-This updated is to code that is deprecated since #24444.  That said, this seems like an obvious thing to do and is a better deal for anyone still using this code.
+This update is to code that is deprecated since #24444.  That said, this seems like an obvious thing to do and is a better deal for anyone still using this code.
 
 Or, if nothing else, this fix is needed for the tests to pass on Python 3.
``````




---

archive/issue_events_330954.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-05-18T17:49:18Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25213",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25213#event-330954"
}
```



---

archive/issue_events_330955.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-05-18T17:49:18Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25213",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25213#event-330955"
}
```



---

archive/issue_comments_388215.json:
```json
{
    "body": "Changed branch from **[u/embray/tensor/order-diff-form-output](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/tensor/order-diff-form-output)** to **[065effd](https://github.com/sagemath/sagetrac-mirror/commit/065effd37caf4b80ff4f1abcdc86400dc4a8313c)**",
    "created_at": "2018-05-18T17:49:18Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25213",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25213#issuecomment-388215",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/embray/tensor/order-diff-form-output](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/tensor/order-diff-form-output)** to **[065effd](https://github.com/sagemath/sagetrac-mirror/commit/065effd37caf4b80ff4f1abcdc86400dc4a8313c)**
