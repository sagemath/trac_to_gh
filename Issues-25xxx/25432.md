# Issue 25432: conversion SR -> fraction fields

archive/issues_025195.json:
```json
{
    "assignees": [],
    "body": "This should work\n\n```\nsage: x = SR.var('x')\nsage: QQ['x'].fraction_field()(1/x)\n1/x\nsage: QQbar['x'].fraction_field()(1/x)\nTraceback (most recent last):\n...\nTypeError: cannot convert 1/x/1 to an element of Fraction Field of\nUnivariate Polynomial Ring in x over Algebraic Field\n```\n\nKeywords: **MathExp2018**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25432_\n\n",
    "created_at": "2018-05-23T17:06:28Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
        "https://github.com/sagemath/sage/labels/invalid",
        "https://github.com/sagemath/sage/labels/needs%20review"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "conversion SR -> fraction fields",
    "type": "issue",
    "updated_at": "2023-01-07T01:38:10Z",
    "url": "https://github.com/sagemath/sage/issues/25432",
    "user": "https://github.com/videlec"
}
```
This should work

```
sage: x = SR.var('x')
sage: QQ['x'].fraction_field()(1/x)
1/x
sage: QQbar['x'].fraction_field()(1/x)
Traceback (most recent last):
...
TypeError: cannot convert 1/x/1 to an element of Fraction Field of
Univariate Polynomial Ring in x over Algebraic Field
```

Keywords: **MathExp2018**

_Issue created by migration from https://trac.sagemath.org/ticket/25432_





---

archive/issue_events_333844.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-23T17:06:28Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25432",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25432#event-333844"
}
```



---

archive/issue_events_333845.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-23T17:06:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25432",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20symbolics",
    "label_color": "0000ff",
    "label_name": "component: symbolics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25432#event-333845"
}
```



---

archive/issue_events_333846.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-23T17:06:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25432",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25432#event-333846"
}
```



---

archive/issue_events_333847.json:
```json
{
    "actor": "https://github.com/videlec",
    "created_at": "2018-05-23T17:06:28Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25432",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25432#event-333847"
}
```



---

archive/issue_events_333848.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:41:45Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25432",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25432#event-333848"
}
```



---

archive/issue_comments_392053.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -2,9 +2,9 @@\n \n ```\n sage: x = SR.var('x')\n-sage: QQ['x'].fraction_field(1/x)\n+sage: QQ['x'].fraction_field()(1/x)\n 1/x\n-sage: QQbar['x'].fraction_field(1/x)\n+sage: QQbar['x'].fraction_field()(1/x)\n Traceback (most recent last):\n ...\n TypeError: cannot convert 1/x/1 to an element of Fraction Field of\n``````\n",
    "created_at": "2023-01-07T01:37:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25432#issuecomment-392053",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -2,9 +2,9 @@
 
 ```
 sage: x = SR.var('x')
-sage: QQ['x'].fraction_field(1/x)
+sage: QQ['x'].fraction_field()(1/x)
 1/x
-sage: QQbar['x'].fraction_field(1/x)
+sage: QQbar['x'].fraction_field()(1/x)
 Traceback (most recent last):
 ...
 TypeError: cannot convert 1/x/1 to an element of Fraction Field of
``````




---

archive/issue_comments_392054.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nFixed typos in the example",
    "created_at": "2023-01-07T01:37:47Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25432#issuecomment-392054",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:2" align="right">Comment 2</div>

Fixed typos in the example



---

archive/issue_events_333849.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T01:38:10Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25432",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25432#event-333849"
}
```



---

archive/issue_events_333850.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T01:38:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25432",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25432#event-333850"
}
```



---

archive/issue_events_333851.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2023-01-07T01:38:10Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25432",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25432#event-333851"
}
```



---

archive/issue_comments_392055.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nWorks OK in 9.8.beta6",
    "created_at": "2023-01-07T01:38:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25432",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25432#issuecomment-392055",
    "user": "https://github.com/mkoeppe"
}
```

<div id="comment:3" align="right">Comment 3</div>

Works OK in 9.8.beta6
