# Issue 25302: Update developer docs to mention GitHub login to Trac

archive/issues_025065.json:
```json
{
    "assignees": [],
    "body": "A lot of people still seem to be missing the fact that you can do this, probably because the developer docs still say that manually requesting an account is the only way to use the Trac site.\n\nThe fact that it can't be used with `git trac` is unfortunate--this is simply due to the fact that the `git trac` plugin uses a user's Trac username and password to authenticate, and when using GitHub login they have no \"password\".\n\nIn the future I might fix this by extending the git-trac plugin to provide generated passwords/authentication tokens for this purpose.\n\nBranch: **[1bdc96b](https://github.com/sagemath/sagetrac-mirror/commit/1bdc96bd05aa435c291931fcc58ded3066e04d38)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Erik Bray**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25302_\n\n",
    "closed_at": "2018-05-15T22:33:30Z",
    "created_at": "2018-05-07T14:26:11Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20documentation",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Update developer docs to mention GitHub login to Trac",
    "type": "issue",
    "updated_at": "2018-07-16T10:33:41Z",
    "url": "https://github.com/sagemath/sage/issues/25302",
    "user": "https://github.com/embray"
}
```
A lot of people still seem to be missing the fact that you can do this, probably because the developer docs still say that manually requesting an account is the only way to use the Trac site.

The fact that it can't be used with `git trac` is unfortunate--this is simply due to the fact that the `git trac` plugin uses a user's Trac username and password to authenticate, and when using GitHub login they have no "password".

In the future I might fix this by extending the git-trac plugin to provide generated passwords/authentication tokens for this purpose.

Branch: **[1bdc96b](https://github.com/sagemath/sagetrac-mirror/commit/1bdc96bd05aa435c291931fcc58ded3066e04d38)**

Reviewer: **Frédéric Chapoton**

Author: **Erik Bray**

_Issue created by migration from https://trac.sagemath.org/ticket/25302_





---

archive/issue_events_332020.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-05-07T14:26:11Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25302",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25302#event-332020"
}
```



---

archive/issue_events_332021.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-05-07T14:26:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25302",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20documentation",
    "label_color": "0075ca",
    "label_name": "component: documentation",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25302#event-332021"
}
```



---

archive/issue_events_332022.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-05-07T14:26:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25302",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25302#event-332022"
}
```



---

archive/issue_events_332023.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-05-07T14:26:11Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25302",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25302#event-332023"
}
```



---

archive/issue_events_332024.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-05-07T14:26:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25302",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25302#event-332024"
}
```



---

archive/issue_events_332025.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-05-07T17:18:36Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25302",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25302#event-332025"
}
```



---

archive/issue_events_332026.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-05-07T17:18:36Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25302",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25302#event-332026"
}
```



---

archive/issue_comments_389608.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nok",
    "created_at": "2018-05-07T17:18:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25302",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25302#issuecomment-389608",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:2" align="right">Comment 2</div>

ok



---

archive/issue_comments_389609.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2018-05-07T17:18:36Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25302",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25302#issuecomment-389609",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_389610.json:
```json
{
    "body": "Changed branch from **[u/embray/docs/account-request](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/docs/account-request)** to **[1bdc96b](https://github.com/sagemath/sagetrac-mirror/commit/1bdc96bd05aa435c291931fcc58ded3066e04d38)**",
    "created_at": "2018-05-15T22:33:30Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25302",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25302#issuecomment-389610",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/embray/docs/account-request](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/docs/account-request)** to **[1bdc96b](https://github.com/sagemath/sagetrac-mirror/commit/1bdc96bd05aa435c291931fcc58ded3066e04d38)**



---

archive/issue_events_332027.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-05-15T22:33:30Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25302",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25302#event-332027"
}
```



---

archive/issue_events_332028.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "bb34ff79ce42927e3c2e6d27cd83722ee78744ef",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-05-15T22:33:30Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25302",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25302#event-332028"
}
```



---

archive/issue_comments_389611.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\n> In the future I might fix this by extending the git-trac plugin to provide generated passwords/authentication tokens for this purpose.\n\nFWIW this has been done, but https://github.com/sagemath/git-trac-command/pull/31 needs to be merged.",
    "created_at": "2018-07-16T10:33:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25302",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25302#issuecomment-389611",
    "user": "https://github.com/embray"
}
```

<div id="comment:4" align="right">Comment 4</div>

> In the future I might fix this by extending the git-trac plugin to provide generated passwords/authentication tokens for this purpose.

FWIW this has been done, but https://github.com/sagemath/git-trac-command/pull/31 needs to be merged.



---

archive/issue_comments_389612.json:
```json
{
    "body": "Changed commit from **[1bdc96b](https://github.com/sagemath/sagetrac-mirror/commit/1bdc96bd05aa435c291931fcc58ded3066e04d38)** to none",
    "created_at": "2018-07-16T10:33:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25302",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25302#issuecomment-389612",
    "user": "https://github.com/embray"
}
```

Changed commit from **[1bdc96b](https://github.com/sagemath/sagetrac-mirror/commit/1bdc96bd05aa435c291931fcc58ded3066e04d38)** to none
