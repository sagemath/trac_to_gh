...
```json
{
    "body": "Minor change. Needs review.",
...
```json
{
    "body": "User gh-canozanoguz added.",
...
```json
{
    "body": "From what you say, Nicolas, I gather this means that one way I could successsfully *not* provide `central_orthogonal_idempotents` in the bad cases is by moving my definition (see the one-line change in commit:\u200b78de7b5) to the end of `__init__`, wrapped in an if statement?:\n\n```\nif self in Algebras.Semisimple:\n    self.central_orthogonal_idempotents = self.cpis\n```",
...
```json
{
    "body": "(with presumably a cache on xxx; maybe on central_orthogonal_idempotents)",
...
```json
{
    "body": "I don't see why we need to deprecate `cpi/s` and not just keep them as an alias.\n\nAlso, +1 to returning a `Family` indexed by `Partitions(n)`.",
...
```json
{
    "body": "I am assuming I should take comment:41 as a positive review. Thank you all.",
...
```json
{
    "body": "Various test failures (run `make ptestlong`)",
...
```json
{
    "body": "My fault. Stupid typo in the variable name. Fixed and now all tests pass.",
    "created_at": "2018-09-08T10:58:08Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25942",
    "type": "issue_comment",
