# Issue 25459: Faster completion_by_cuts()

archive/issues_025222.json:
```json
{
    "assignees": [],
    "body": "With `P = posets.StandardExample(9)` before this ticket it takes 164 ms to run `P.completion_by_cuts()`. After the ticket it is 93 ms.\n\nCC:  @tscrim @fchapoton\n\nBranch/Commit: **[9f2b22e](https://github.com/sagemath/sagetrac-mirror/commit/9f2b22e90fc43d7dce92316928e860c35337d233)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Jori M\u00e4ntysalo**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25459_\n\n",
    "closed_at": "2018-05-30T19:19:28Z",
    "created_at": "2018-05-29T12:31:53Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Faster completion_by_cuts()",
    "type": "issue",
    "updated_at": "2018-05-30T19:19:28Z",
    "url": "https://github.com/sagemath/sage/issues/25459",
    "user": "https://github.com/jm58660"
}
```
With `P = posets.StandardExample(9)` before this ticket it takes 164 ms to run `P.completion_by_cuts()`. After the ticket it is 93 ms.

CC:  @tscrim @fchapoton

Branch/Commit: **[9f2b22e](https://github.com/sagemath/sagetrac-mirror/commit/9f2b22e90fc43d7dce92316928e860c35337d233)**

Reviewer: **Frédéric Chapoton**

Author: **Jori Mäntysalo**

_Issue created by migration from https://trac.sagemath.org/ticket/25459_





---

archive/issue_events_334216.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-05-29T12:31:53Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25459",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25459#event-334216"
}
```



---

archive/issue_events_334217.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-05-29T12:31:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25459",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25459#event-334217"
}
```



---

archive/issue_events_334218.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-05-29T12:31:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25459",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25459#event-334218"
}
```



---

archive/issue_events_334219.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-05-29T12:31:53Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25459",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25459#event-334219"
}
```



---

archive/issue_comments_392731.json:
```json
{
    "body": "Branch: **[u/jmantysalo/faster-dedekind](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/faster-dedekind)**",
    "created_at": "2018-05-29T12:32:33Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25459",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25459#issuecomment-392731",
    "user": "https://github.com/jm58660"
}
```

Branch: **[u/jmantysalo/faster-dedekind](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/faster-dedekind)**



---

archive/issue_comments_392732.json:
```json
{
    "body": "Commit: **[9f2b22e](https://github.com/sagemath/sagetrac-mirror/commit/9f2b22e90fc43d7dce92316928e860c35337d233)**",
    "created_at": "2018-05-29T12:33:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25459",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25459#issuecomment-392732",
    "user": "https://github.com/jm58660"
}
```

Commit: **[9f2b22e](https://github.com/sagemath/sagetrac-mirror/commit/9f2b22e90fc43d7dce92316928e860c35337d233)**



---

archive/issue_events_334220.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-05-29T12:33:06Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25459",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25459#event-334220"
}
```



---

archive/issue_comments_392733.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nAn oneliner to review.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9f2b22e90fc43d7dce92316928e860c35337d233\">9f2b22e</a></td><td><code>Use lambda.</code></td></tr></table>\n",
    "created_at": "2018-05-29T12:33:06Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25459",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25459#issuecomment-392733",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:2" align="right">Comment 2</div>

An oneliner to review.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9f2b22e90fc43d7dce92316928e860c35337d233">9f2b22e</a></td><td><code>Use lambda.</code></td></tr></table>




---

archive/issue_comments_392734.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nok",
    "created_at": "2018-05-29T13:50:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25459",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25459#issuecomment-392734",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">Comment 3</div>

ok



---

archive/issue_comments_392735.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2018-05-29T13:50:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25459",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25459#issuecomment-392735",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_events_334221.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-05-29T13:50:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25459",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25459#event-334221"
}
```



---

archive/issue_events_334222.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-05-29T13:50:05Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25459",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25459#event-334222"
}
```



---

archive/issue_comments_392736.json:
```json
{
    "body": "Changed branch from **[u/jmantysalo/faster-dedekind](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/faster-dedekind)** to **[9f2b22e](https://github.com/sagemath/sagetrac-mirror/commit/9f2b22e90fc43d7dce92316928e860c35337d233)**",
    "created_at": "2018-05-30T19:19:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25459",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25459#issuecomment-392736",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jmantysalo/faster-dedekind](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/faster-dedekind)** to **[9f2b22e](https://github.com/sagemath/sagetrac-mirror/commit/9f2b22e90fc43d7dce92316928e860c35337d233)**



---

archive/issue_events_334223.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-05-30T19:19:28Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25459",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25459#event-334223"
}
```



---

archive/issue_events_334224.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "316315df90e6de590fdb46497813a378a71c4f14",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-05-30T19:19:28Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25459",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25459#event-334224"
}
```
