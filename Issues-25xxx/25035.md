# Issue 25035: Use sage-dist-helpers + add DESTDIR support for flint and arb

archive/issues_024798.json:
```json
{
    "assignees": [],
    "body": "This is implementing #24024 for the flint and arb packages which use basically the same \"build system\".  \n\nThis shouldn't change much anything in the end result of installing these packages, but I bumped the version numbers just to ensure testing on the buildbots.\n\nDepends on #23733\n\nKeywords: **destdir flint arb**\n\nBranch/Commit: **[a9971d7](https://github.com/sagemath/sagetrac-mirror/commit/a9971d7a283398ee107793f3ddd6796262f2d54f)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Erik Bray**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25035_\n\n",
    "closed_at": "2019-03-08T17:22:31Z",
    "created_at": "2018-03-27T09:24:13Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20build",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.7",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Use sage-dist-helpers + add DESTDIR support for flint and arb",
    "type": "issue",
    "updated_at": "2019-03-08T17:22:31Z",
    "url": "https://github.com/sagemath/sage/issues/25035",
    "user": "https://github.com/embray"
}
```
This is implementing #24024 for the flint and arb packages which use basically the same "build system".  

This shouldn't change much anything in the end result of installing these packages, but I bumped the version numbers just to ensure testing on the buildbots.

Depends on #23733

Keywords: **destdir flint arb**

Branch/Commit: **[a9971d7](https://github.com/sagemath/sagetrac-mirror/commit/a9971d7a283398ee107793f3ddd6796262f2d54f)**

Reviewer: **Frédéric Chapoton**

Author: **Erik Bray**

_Issue created by migration from https://trac.sagemath.org/ticket/25035_





---

archive/issue_events_328208.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-03-27T09:24:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25035#event-328208"
}
```



---

archive/issue_events_328209.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-03-27T09:24:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20build",
    "label_color": "000080",
    "label_name": "component: build",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25035#event-328209"
}
```



---

archive/issue_events_328210.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-03-27T09:24:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25035#event-328210"
}
```



---

archive/issue_events_328211.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-03-27T09:24:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25035#event-328211"
}
```



---

archive/issue_events_328212.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-03-27T09:24:54Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25035#event-328212"
}
```



---

archive/issue_comments_384160.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nTwo of the failing patchbots seem to be...really broken, and in different ways.  I don't think it has anything directly to do with this ticket...",
    "created_at": "2018-03-28T12:31:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25035#issuecomment-384160",
    "user": "https://github.com/embray"
}
```

<div id="comment:2" align="right">Comment 2</div>

Two of the failing patchbots seem to be...really broken, and in different ways.  I don't think it has anything directly to do with this ticket...



---

archive/issue_comments_384161.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b4ecda96068fe21d37277e36d631af3b4ee20ea0\">b4ecda9</a></td><td><code>trac 23733: deprecate SAGE64 and CFLAG64</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/905e4d44a3da6d0265c9d2ae24d19691f40c0458\">905e4d4</a></td><td><code>Stop supporting SAGE64 except in Numpy</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/eecf581b827a7decd0eb0ddd345aef8dd1c9d54e\">eecf581</a></td><td><code>Merge branch 'u/jdemeyer/no-sage64' into u/embray/build/destdir-arb-flint</code></td></tr></table>\n",
    "created_at": "2018-04-04T08:05:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25035#issuecomment-384161",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:3" align="right">Comment 3</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b4ecda96068fe21d37277e36d631af3b4ee20ea0">b4ecda9</a></td><td><code>trac 23733: deprecate SAGE64 and CFLAG64</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/905e4d44a3da6d0265c9d2ae24d19691f40c0458">905e4d4</a></td><td><code>Stop supporting SAGE64 except in Numpy</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/eecf581b827a7decd0eb0ddd345aef8dd1c9d54e">eecf581</a></td><td><code>Merge branch 'u/jdemeyer/no-sage64' into u/embray/build/destdir-arb-flint</code></td></tr></table>




---

archive/issue_comments_384162.json:
```json
{
    "body": "Changed commit from **[f63449a](https://github.com/sagemath/sagetrac-mirror/commit/f63449aa0329614b811de045f0d769c88b21f8bd)** to **[eecf581](https://github.com/sagemath/sagetrac-mirror/commit/eecf581b827a7decd0eb0ddd345aef8dd1c9d54e)**",
    "created_at": "2018-04-04T08:05:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25035#issuecomment-384162",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[f63449a](https://github.com/sagemath/sagetrac-mirror/commit/f63449aa0329614b811de045f0d769c88b21f8bd)** to **[eecf581](https://github.com/sagemath/sagetrac-mirror/commit/eecf581b827a7decd0eb0ddd345aef8dd1c9d54e)**



---

archive/issue_comments_384163.json:
```json
{
    "body": "Dependencies: **#23733**",
    "created_at": "2018-04-04T08:05:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25035#issuecomment-384163",
    "user": "https://github.com/embray"
}
```

Dependencies: **#23733**



---

archive/issue_comments_384164.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/b4ecda96068fe21d37277e36d631af3b4ee20ea0\">b4ecda9</a></td><td><code>trac 23733: deprecate SAGE64 and CFLAG64</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/905e4d44a3da6d0265c9d2ae24d19691f40c0458\">905e4d4</a></td><td><code>Stop supporting SAGE64 except in Numpy</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/eecf581b827a7decd0eb0ddd345aef8dd1c9d54e\">eecf581</a></td><td><code>Merge branch 'u/jdemeyer/no-sage64' into u/embray/build/destdir-arb-flint</code></td></tr></table>\n",
    "created_at": "2018-04-04T08:05:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25035#issuecomment-384164",
    "user": "https://github.com/embray"
}
```

<div id="comment:4" align="right">Comment 4</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/b4ecda96068fe21d37277e36d631af3b4ee20ea0">b4ecda9</a></td><td><code>trac 23733: deprecate SAGE64 and CFLAG64</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/905e4d44a3da6d0265c9d2ae24d19691f40c0458">905e4d4</a></td><td><code>Stop supporting SAGE64 except in Numpy</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/eecf581b827a7decd0eb0ddd345aef8dd1c9d54e">eecf581</a></td><td><code>Merge branch 'u/jdemeyer/no-sage64' into u/embray/build/destdir-arb-flint</code></td></tr></table>




---

archive/issue_events_328213.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-26T12:13:55Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25035#event-328213"
}
```



---

archive/issue_events_328214.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-04-26T12:13:55Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25035#event-328214"
}
```



---

archive/issue_comments_384165.json:
```json
{
    "body": "Work Issues: **merge conflicts**",
    "created_at": "2018-07-17T15:42:12Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25035#issuecomment-384165",
    "user": "https://github.com/saraedum"
}
```

Work Issues: **merge conflicts**



---

archive/issue_events_328215.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-07-17T15:42:12Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25035#event-328215"
}
```



---

archive/issue_events_328216.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-07-17T15:42:12Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25035#event-328216"
}
```



---

archive/issue_comments_384166.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/73372910bcf96a04c2220153e2a48570cecd5a93\">7337291</a></td><td><code>Convert the arb and flint packages to use sage-dist-helpers where possible</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/067ebacc262a4712e85b2aeae85dc85ce0e231db\">067ebac</a></td><td><code>bump package versions just to ensure this is tested on the buildbots</code></td></tr></table>\n",
    "created_at": "2018-07-17T18:05:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25035#issuecomment-384166",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:7" align="right">Comment 7</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/73372910bcf96a04c2220153e2a48570cecd5a93">7337291</a></td><td><code>Convert the arb and flint packages to use sage-dist-helpers where possible</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/067ebacc262a4712e85b2aeae85dc85ce0e231db">067ebac</a></td><td><code>bump package versions just to ensure this is tested on the buildbots</code></td></tr></table>




---

archive/issue_comments_384167.json:
```json
{
    "body": "Changed commit from **[eecf581](https://github.com/sagemath/sagetrac-mirror/commit/eecf581b827a7decd0eb0ddd345aef8dd1c9d54e)** to **[067ebac](https://github.com/sagemath/sagetrac-mirror/commit/067ebacc262a4712e85b2aeae85dc85ce0e231db)**",
    "created_at": "2018-07-17T18:05:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25035#issuecomment-384167",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[eecf581](https://github.com/sagemath/sagetrac-mirror/commit/eecf581b827a7decd0eb0ddd345aef8dd1c9d54e)** to **[067ebac](https://github.com/sagemath/sagetrac-mirror/commit/067ebacc262a4712e85b2aeae85dc85ce0e231db)**



---

archive/issue_comments_384168.json:
```json
{
    "body": "Changed work issues from **merge conflicts** to none",
    "created_at": "2018-07-17T18:05:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25035#issuecomment-384168",
    "user": "https://github.com/embray"
}
```

Changed work issues from **merge conflicts** to none



---

archive/issue_events_328217.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-07-17T18:05:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25035#event-328217"
}
```



---

archive/issue_events_328218.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-07-17T18:05:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25035#event-328218"
}
```



---

archive/issue_comments_384169.json:
```json
{
    "body": "<div id=\"comment:8\" align=\"right\">Comment 8</div>\n\nSeems fine now.",
    "created_at": "2018-07-17T18:05:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25035#issuecomment-384169",
    "user": "https://github.com/embray"
}
```

<div id="comment:8" align="right">Comment 8</div>

Seems fine now.



---

archive/issue_events_328219.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-07-18T11:47:03Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25035#event-328219"
}
```



---

archive/issue_events_328220.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-07-18T11:47:03Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25035#event-328220"
}
```



---

archive/issue_comments_384170.json:
```json
{
    "body": "<div id=\"comment:9\" align=\"right\">Comment 9</div>\n\nI believe this issue can reasonably be addressed for Sage 8.4.",
    "created_at": "2018-07-18T11:47:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25035#issuecomment-384170",
    "user": "https://github.com/embray"
}
```

<div id="comment:9" align="right">Comment 9</div>

I believe this issue can reasonably be addressed for Sage 8.4.



---

archive/issue_events_328221.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-10-28T11:41:42Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "milestone_number": null,
    "milestone_title": "sage-8.4",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25035#event-328221"
}
```



---

archive/issue_events_328222.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-10-28T11:41:42Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25035#event-328222"
}
```



---

archive/issue_events_328223.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-12-28T14:10:15Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "milestone_number": null,
    "milestone_title": "sage-8.5",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25035#event-328223"
}
```



---

archive/issue_events_328224.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-12-28T14:10:15Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "milestone_number": null,
    "milestone_title": "sage-8.7",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25035#event-328224"
}
```



---

archive/issue_comments_384171.json:
```json
{
    "body": "<div id=\"comment:11\" align=\"right\">Comment 11</div>\n\nRetargeting some of my tickets (somewhat optimistically for now).",
    "created_at": "2018-12-28T14:10:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25035#issuecomment-384171",
    "user": "https://github.com/embray"
}
```

<div id="comment:11" align="right">Comment 11</div>

Retargeting some of my tickets (somewhat optimistically for now).



---

archive/issue_events_328225.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-03-02T18:04:43Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25035#event-328225"
}
```



---

archive/issue_events_328226.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-03-02T18:04:43Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25035#event-328226"
}
```



---

archive/issue_comments_384172.json:
```json
{
    "body": "<div id=\"comment:12\" align=\"right\">Comment 12</div>\n\nred branch",
    "created_at": "2019-03-02T18:04:43Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25035#issuecomment-384172",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:12" align="right">Comment 12</div>

red branch



---

archive/issue_comments_384173.json:
```json
{
    "body": "<div id=\"comment:13\" align=\"right\">Comment 13</div>\n\nCool. I don't know why this one keeps getting delayed.  It's pretty straightforward.  Fr\u00e9d\u00e9ric, now that you have some experience with this on plantri perhaps you can look at it after I rebase again.  It's really straightforward.",
    "created_at": "2019-03-05T16:53:28Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25035#issuecomment-384173",
    "user": "https://github.com/embray"
}
```

<div id="comment:13" align="right">Comment 13</div>

Cool. I don't know why this one keeps getting delayed.  It's pretty straightforward.  Frédéric, now that you have some experience with this on plantri perhaps you can look at it after I rebase again.  It's really straightforward.



---

archive/issue_comments_384174.json:
```json
{
    "body": "<div id=\"comment:14\" align=\"right\">Comment 14</div>\n\nred branch, still",
    "created_at": "2019-03-05T17:03:35Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25035#issuecomment-384174",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:14" align="right">Comment 14</div>

red branch, still



---

archive/issue_comments_384175.json:
```json
{
    "body": "<div id=\"comment:15\" align=\"right\">Comment 15</div>\n\nHence \"after I rebase again\", which is to say \"when it is done\" and not \"I have done it\".",
    "created_at": "2019-03-05T17:11:17Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25035#issuecomment-384175",
    "user": "https://github.com/embray"
}
```

<div id="comment:15" align="right">Comment 15</div>

Hence "after I rebase again", which is to say "when it is done" and not "I have done it".



---

archive/issue_comments_384176.json:
```json
{
    "body": "Changed commit from **[067ebac](https://github.com/sagemath/sagetrac-mirror/commit/067ebacc262a4712e85b2aeae85dc85ce0e231db)** to **[a9971d7](https://github.com/sagemath/sagetrac-mirror/commit/a9971d7a283398ee107793f3ddd6796262f2d54f)**",
    "created_at": "2019-03-07T13:24:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25035#issuecomment-384176",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[067ebac](https://github.com/sagemath/sagetrac-mirror/commit/067ebacc262a4712e85b2aeae85dc85ce0e231db)** to **[a9971d7](https://github.com/sagemath/sagetrac-mirror/commit/a9971d7a283398ee107793f3ddd6796262f2d54f)**



---

archive/issue_comments_384177.json:
```json
{
    "body": "<div id=\"comment:16\" align=\"right\">Comment 16</div>\n\nBranch pushed to git repo; I updated commit sha1. This was a forced push. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a04d50a9017d0a176d8166e81ea39f3d48e5cdad\">a04d50a</a></td><td><code>Convert the arb and flint packages to use sage-dist-helpers where possible</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/a9971d7a283398ee107793f3ddd6796262f2d54f\">a9971d7</a></td><td><code>bump package versions just to ensure this is tested on the buildbots</code></td></tr></table>\n",
    "created_at": "2019-03-07T13:24:01Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25035#issuecomment-384177",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:16" align="right">Comment 16</div>

Branch pushed to git repo; I updated commit sha1. This was a forced push. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a04d50a9017d0a176d8166e81ea39f3d48e5cdad">a04d50a</a></td><td><code>Convert the arb and flint packages to use sage-dist-helpers where possible</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/a9971d7a283398ee107793f3ddd6796262f2d54f">a9971d7</a></td><td><code>bump package versions just to ensure this is tested on the buildbots</code></td></tr></table>




---

archive/issue_comments_384178.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2019-03-07T13:46:08Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25035#issuecomment-384178",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_events_328227.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-03-07T13:46:08Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "label": "https://github.com/sagemath/sage/labels/needs%20work",
    "label_color": "ffff00",
    "label_name": "needs work",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25035#event-328227"
}
```



---

archive/issue_events_328228.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2019-03-07T13:46:08Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25035#event-328228"
}
```



---

archive/issue_comments_384179.json:
```json
{
    "body": "<div id=\"comment:18\" align=\"right\">Comment 18</div>\n\nThanks!  I'm still re-testing this but I don't anticipate any problems.",
    "created_at": "2019-03-07T13:48:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25035#issuecomment-384179",
    "user": "https://github.com/embray"
}
```

<div id="comment:18" align="right">Comment 18</div>

Thanks!  I'm still re-testing this but I don't anticipate any problems.



---

archive/issue_events_328229.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-03-08T17:22:31Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25035#event-328229"
}
```



---

archive/issue_events_328230.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "5724e3ba92ff61f223dbc5697366818c6949b3bb",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2019-03-08T17:22:31Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25035#event-328230"
}
```



---

archive/issue_comments_384180.json:
```json
{
    "body": "Changed branch from **[u/embray/build/destdir-arb-flint](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/build/destdir-arb-flint)** to **[a9971d7](https://github.com/sagemath/sagetrac-mirror/commit/a9971d7a283398ee107793f3ddd6796262f2d54f)**",
    "created_at": "2019-03-08T17:22:31Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25035",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25035#issuecomment-384180",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/embray/build/destdir-arb-flint](https://github.com/sagemath/sagetrac-mirror/tree/u/embray/build/destdir-arb-flint)** to **[a9971d7](https://github.com/sagemath/sagetrac-mirror/commit/a9971d7a283398ee107793f3ddd6796262f2d54f)**
