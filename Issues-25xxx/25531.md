# Issue 25531: Fix cysignals include

archive/issues_025294.json:
```json
{
    "assignees": [],
    "body": "This (deprecated) include is causing problems with non-standard Sage installations. Since `cython()` adds the cysignals include path, this should fix it.\n\nCC:  @timokau\n\nBranch/Commit: **[28778bd](https://github.com/sagemath/sagetrac-mirror/commit/28778bd25a37c80884d2b24e0683fb2989300cef)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25531_\n\n",
    "closed_at": "2018-06-09T17:49:15Z",
    "created_at": "2018-06-07T15:11:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20cython",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/bug"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Fix cysignals include",
    "type": "issue",
    "updated_at": "2018-06-09T17:49:15Z",
    "url": "https://github.com/sagemath/sage/issues/25531",
    "user": "https://github.com/jdemeyer"
}
```
This (deprecated) include is causing problems with non-standard Sage installations. Since `cython()` adds the cysignals include path, this should fix it.

CC:  @timokau

Branch/Commit: **[28778bd](https://github.com/sagemath/sagetrac-mirror/commit/28778bd25a37c80884d2b24e0683fb2989300cef)**

Reviewer: **Frédéric Chapoton**

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/25531_





---

archive/issue_events_335227.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-06-07T15:11:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25531",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25531#event-335227"
}
```



---

archive/issue_events_335228.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-06-07T15:11:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25531",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20cython",
    "label_color": "000080",
    "label_name": "component: cython",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25531#event-335228"
}
```



---

archive/issue_events_335229.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-06-07T15:11:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25531",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25531#event-335229"
}
```



---

archive/issue_events_335230.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-06-07T15:11:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25531",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25531#event-335230"
}
```



---

archive/issue_comments_394365.json:
```json
{
    "body": "Branch: **[u/jdemeyer/ticket/25531](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/25531)**",
    "created_at": "2018-06-07T15:11:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25531",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25531#issuecomment-394365",
    "user": "https://github.com/jdemeyer"
}
```

Branch: **[u/jdemeyer/ticket/25531](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/25531)**



---

archive/issue_events_335231.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-06-07T15:18:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25531",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25531#event-335231"
}
```



---

archive/issue_comments_394366.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/28778bd25a37c80884d2b24e0683fb2989300cef\">28778bd</a></td><td><code>Fix cysignals include</code></td></tr></table>\n",
    "created_at": "2018-06-07T15:18:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25531",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25531#issuecomment-394366",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:2'>Comment 2:</a>
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/28778bd25a37c80884d2b24e0683fb2989300cef">28778bd</a></td><td><code>Fix cysignals include</code></td></tr></table>




---

archive/issue_comments_394367.json:
```json
{
    "body": "Commit: **[28778bd](https://github.com/sagemath/sagetrac-mirror/commit/28778bd25a37c80884d2b24e0683fb2989300cef)**",
    "created_at": "2018-06-07T15:18:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25531",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25531#issuecomment-394367",
    "user": "https://github.com/jdemeyer"
}
```

Commit: **[28778bd](https://github.com/sagemath/sagetrac-mirror/commit/28778bd25a37c80884d2b24e0683fb2989300cef)**



---

archive/issue_events_335232.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-06-07T17:34:27Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25531",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25531#event-335232"
}
```



---

archive/issue_events_335233.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-06-07T17:34:27Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25531",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25531#event-335233"
}
```



---

archive/issue_comments_394368.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2018-06-07T17:34:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25531",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25531#issuecomment-394368",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_394369.json:
```json
{
    "body": "<a id='comment:3'>Comment 3:</a>\nok",
    "created_at": "2018-06-07T17:34:27Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25531",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25531#issuecomment-394369",
    "user": "https://github.com/fchapoton"
}
```

<a id='comment:3'>Comment 3:</a>
ok



---

archive/issue_comments_394370.json:
```json
{
    "body": "Changed branch from **[u/jdemeyer/ticket/25531](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/25531)** to **[28778bd](https://github.com/sagemath/sagetrac-mirror/commit/28778bd25a37c80884d2b24e0683fb2989300cef)**",
    "created_at": "2018-06-09T17:49:15Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25531",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25531#issuecomment-394370",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jdemeyer/ticket/25531](https://github.com/sagemath/sagetrac-mirror/tree/u/jdemeyer/ticket/25531)** to **[28778bd](https://github.com/sagemath/sagetrac-mirror/commit/28778bd25a37c80884d2b24e0683fb2989300cef)**



---

archive/issue_events_335234.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-06-09T17:49:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25531",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25531#event-335234"
}
```



---

archive/issue_events_335235.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "fdf3f40556226f747fc8a9b0e43245d31bb75129",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-06-09T17:49:15Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25531",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25531#event-335235"
}
```
