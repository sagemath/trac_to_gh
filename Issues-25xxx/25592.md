...
```json
{
    "body": "This looks fine. Although I made a slight change to be more memory efficient. We don't need to create and store the entire list of points in the rational case. I think that is worth the slight penalty of the 'if' we must check once per dimension.\n\nIf you think new version is fine, I'll mark this positive.\n\n---\nNew commits:",
...
```json
{
    "body": "Sir, but isn't that calling range_by_height function every time would have some time penalty? (it calculates `gcd` for `H^2` times each of which would have a complexity of `log(m) + log(n)`).\n\nI think there would be a trade off between memory and time, which one would be significant would be deciding.",
...
```json
{
    "body": "Yes, but I think the memory penalty is worse for storing the whole list.",
