# Issue 25228: Use coercion maps when initializing polynomials in ℤ[x]

archive/issues_024991.json:
```json
{
    "assignees": [],
    "body": "In #22956 we are having trouble creating some integer polynomials:\n\n```\nTypeError: unable to coerce <class 'henselization.base_element.BaseElement_Ring_with_category'> to an integer\n```\n\nThe proposed patch fixes this problem.\n\nBranch/Commit: **[u/saraedum/25228](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/25228) @ [9ee5525](https://github.com/sagemath/sagetrac-mirror/commit/9ee5525edeaf9833868a329d931900fa89e47075)**\n\nReviewer: **Julian R\u00fcth**\n\nAuthor: **Julian R\u00fcth**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25228_\n\n",
    "closed_at": "2018-11-08T16:07:23Z",
    "created_at": "2018-04-23T10:33:48Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/bug",
        "https://github.com/sagemath/sage/labels/component%3A%20number%20theory",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Use coercion maps when initializing polynomials in \u2124[x]",
    "type": "issue",
    "updated_at": "2018-11-08T16:07:23Z",
    "url": "https://github.com/sagemath/sage/issues/25228",
    "user": "https://github.com/saraedum"
}
```
In #22956 we are having trouble creating some integer polynomials:

```
TypeError: unable to coerce <class 'henselization.base_element.BaseElement_Ring_with_category'> to an integer
```

The proposed patch fixes this problem.

Branch/Commit: **[u/saraedum/25228](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/25228) @ [9ee5525](https://github.com/sagemath/sagetrac-mirror/commit/9ee5525edeaf9833868a329d931900fa89e47075)**

Reviewer: **Julian Rüth**

Author: **Julian Rüth**

_Issue created by migration from https://trac.sagemath.org/ticket/25228_





---

archive/issue_events_331119.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-04-23T10:33:48Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25228",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25228#event-331119"
}
```



---

archive/issue_events_331120.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-04-23T10:33:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25228",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20number%20theory",
    "label_color": "0000ff",
    "label_name": "component: number theory",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25228#event-331120"
}
```



---

archive/issue_events_331121.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-04-23T10:33:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25228",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25228#event-331121"
}
```



---

archive/issue_events_331122.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-04-23T10:33:48Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25228",
    "label": "https://github.com/sagemath/sage/labels/bug",
    "label_color": "d73a4a",
    "label_name": "bug",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25228#event-331122"
}
```



---

archive/issue_comments_388426.json:
```json
{
    "body": "Branch: **[u/saraedum/flint_poly_init](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/flint_poly_init)**",
    "created_at": "2018-04-23T10:34:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25228",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25228#issuecomment-388426",
    "user": "https://github.com/saraedum"
}
```

Branch: **[u/saraedum/flint_poly_init](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/flint_poly_init)**



---

archive/issue_comments_388427.json:
```json
{
    "body": "Changed branch from **[u/saraedum/flint_poly_init](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/flint_poly_init)** to none",
    "created_at": "2018-05-29T02:14:11Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25228",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25228#issuecomment-388427",
    "user": "https://github.com/saraedum"
}
```

Changed branch from **[u/saraedum/flint_poly_init](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/flint_poly_init)** to none



---

archive/issue_comments_388428.json:
```json
{
    "body": "Branch: **[u/saraedum/25228](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/25228)**",
    "created_at": "2018-05-29T02:14:59Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25228",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25228#issuecomment-388428",
    "user": "https://github.com/saraedum"
}
```

Branch: **[u/saraedum/25228](https://github.com/sagemath/sagetrac-mirror/tree/u/saraedum/25228)**



---

archive/issue_events_331123.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-05-29T14:22:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25228",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25228#event-331123"
}
```



---

archive/issue_comments_388429.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/10b0144a9a918b1ce0cee36984969defbe1fdef3\">10b0144</a></td><td><code>Use default coercion/conversion maps when generating integer polynomials</code></td></tr><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/9ee5525edeaf9833868a329d931900fa89e47075\">9ee5525</a></td><td><code>Merge remote-tracking branch 'trac/u/saraedum/flint_poly_init' into HEAD</code></td></tr></table>\n",
    "created_at": "2018-05-29T14:22:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25228",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25228#issuecomment-388429",
    "user": "https://github.com/saraedum"
}
```

<div id="comment:4" align="right">Comment 4</div>

New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/10b0144a9a918b1ce0cee36984969defbe1fdef3">10b0144</a></td><td><code>Use default coercion/conversion maps when generating integer polynomials</code></td></tr><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/9ee5525edeaf9833868a329d931900fa89e47075">9ee5525</a></td><td><code>Merge remote-tracking branch 'trac/u/saraedum/flint_poly_init' into HEAD</code></td></tr></table>




---

archive/issue_comments_388430.json:
```json
{
    "body": "Commit: **[9ee5525](https://github.com/sagemath/sagetrac-mirror/commit/9ee5525edeaf9833868a329d931900fa89e47075)**",
    "created_at": "2018-05-29T14:22:39Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25228",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25228#issuecomment-388430",
    "user": "https://github.com/saraedum"
}
```

Commit: **[9ee5525](https://github.com/sagemath/sagetrac-mirror/commit/9ee5525edeaf9833868a329d931900fa89e47075)**



---

archive/issue_events_331124.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-05-29T14:22:39Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25228",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25228#event-331124"
}
```



---

archive/issue_events_331125.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-05-29T14:22:39Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25228",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25228#event-331125"
}
```



---

archive/issue_events_331126.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-05-29T14:22:39Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25228",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25228#event-331126"
}
```



---

archive/issue_comments_388431.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nThis is sort-of-documented feature. Elements must implement `_rational_` and `_integer_` to make this work. Conversion maps are often ignored here.",
    "created_at": "2018-05-29T14:23:10Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25228",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25228#issuecomment-388431",
    "user": "https://github.com/saraedum"
}
```

<div id="comment:5" align="right">Comment 5</div>

This is sort-of-documented feature. Elements must implement `_rational_` and `_integer_` to make this work. Conversion maps are often ignored here.



---

archive/issue_events_331127.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-05-29T14:23:15Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25228",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25228#event-331127"
}
```



---

archive/issue_events_331128.json:
```json
{
    "actor": "https://github.com/saraedum",
    "created_at": "2018-05-29T14:23:15Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25228",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25228#event-331128"
}
```



---

archive/issue_comments_388432.json:
```json
{
    "body": "<div id=\"comment:7\" align=\"right\">Comment 7</div>\n\n*cough* Reviewer name.",
    "created_at": "2018-05-30T06:04:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25228",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25228#issuecomment-388432",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:7" align="right">Comment 7</div>

*cough* Reviewer name.



---

archive/issue_comments_388433.json:
```json
{
    "body": "Reviewer: **Julian R\u00fcth**",
    "created_at": "2018-07-04T17:04:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25228",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25228#issuecomment-388433",
    "user": "https://github.com/saraedum"
}
```

Reviewer: **Julian Rüth**



---

archive/issue_events_331129.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-11-08T16:07:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25228",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25228#event-331129"
}
```



---

archive/issue_events_331130.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-11-08T16:07:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25228",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25228#event-331130"
}
```



---

archive/issue_events_331131.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2018-11-08T16:07:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25228",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25228#event-331131"
}
```
