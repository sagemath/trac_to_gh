# Issue 25135: enumerating short vectors of a lattice

archive/issues_024898.json:
```json
{
    "assignees": [],
    "body": "in Magma there is a function ShortVectors to evaluate all vectors of a lattice of norm less than a given bound. It would be nice to have such a function in Sage (I did not find it).\n\nFor example in Magma:\n\n```\n> w := RMatrixSpace(IntegerRing(), 2,2) ! [1,2,3,5];\n> L:=Lattice(w);\n> ShortVectors(L,1);\n[\n    <(1 0), 1>,\n    <(0 1), 1>\n]\n```\n\nKeywords: **LLL BKZ**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25135_\n\n",
    "created_at": "2018-04-10T11:47:13Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "enumerating short vectors of a lattice",
    "type": "issue",
    "updated_at": "2022-12-29T01:40:48Z",
    "url": "https://github.com/sagemath/sage/issues/25135",
    "user": "https://github.com/zimmermann6"
}
```
in Magma there is a function ShortVectors to evaluate all vectors of a lattice of norm less than a given bound. It would be nice to have such a function in Sage (I did not find it).

For example in Magma:

```
> w := RMatrixSpace(IntegerRing(), 2,2) ! [1,2,3,5];
> L:=Lattice(w);
> ShortVectors(L,1);
[
    <(1 0), 1>,
    <(0 1), 1>
]
```

Keywords: **LLL BKZ**

_Issue created by migration from https://trac.sagemath.org/ticket/25135_





---

archive/issue_events_329731.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2018-04-10T11:47:13Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25135",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25135#event-329731"
}
```



---

archive/issue_events_329732.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2018-04-10T11:47:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25135",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20linear%20algebra",
    "label_color": "0000ff",
    "label_name": "component: linear algebra",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25135#event-329732"
}
```



---

archive/issue_events_329733.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2018-04-10T11:47:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25135",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25135#event-329733"
}
```



---

archive/issue_events_329734.json:
```json
{
    "actor": "https://github.com/zimmermann6",
    "created_at": "2018-04-10T11:47:13Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25135",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25135#event-329734"
}
```



---

archive/issue_comments_386451.json:
```json
{
    "body": "<a id='comment:1'>Comment 1:</a>\nA starting point might be the `LLL` and `BKZ` methods of matrices over `ZZ`:\n\n- https://doc.sagemath.org/html/en/reference/matrices/sage/matrix/matrix_integer_dense.html",
    "created_at": "2018-04-10T12:56:51Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25135",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25135#issuecomment-386451",
    "user": "https://github.com/slel"
}
```

<a id='comment:1'>Comment 1:</a>
A starting point might be the `LLL` and `BKZ` methods of matrices over `ZZ`:

- https://doc.sagemath.org/html/en/reference/matrices/sage/matrix/matrix_integer_dense.html



---

archive/issue_comments_386452.json:
```json
{
    "body": "<a id='comment:2'>Comment 2:</a>\nthank you, but I opened this ticket precisely because the LLL and BKZ functions (which I put in the keywords by the way) are not sufficient for some applications.",
    "created_at": "2018-04-10T13:22:13Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25135",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25135#issuecomment-386452",
    "user": "https://github.com/zimmermann6"
}
```

<a id='comment:2'>Comment 2:</a>
thank you, but I opened this ticket precisely because the LLL and BKZ functions (which I put in the keywords by the way) are not sufficient for some applications.



---

archive/issue_events_329735.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2022-12-29T01:40:48Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25135",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25135#event-329735"
}
```
