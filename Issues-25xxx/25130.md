# Issue 25130: Move sage-dist-helpers from src/bin to build/bin

archive/issues_024893.json:
```json
{
    "body": "This seems to be a script for use within `build/pkgs` scripts, so it should go to `build/bin`.\n\nCC:  @embray @kiwifb @dimpase\n\nBranch/Commit: abc9fa93e25a86eef191ece157158e838d2fb433\n\nReviewer: Erik Bray\n\nAuthor: Matthias Koeppe\n\nResolution: fixed\n\nIssue created by migration from https://trac.sagemath.org/ticket/25130\n\n",
    "closed_at": "2019-07-04T11:09:34Z",
    "created_at": "2018-04-09T15:56:11Z",
    "labels": [
        "component: build",
        "bug"
    ],
    "milestone": "https://github.com/sagemath/sagetest/milestones/sage-8.9",
    "title": "Move sage-dist-helpers from src/bin to build/bin",
    "type": "issue",
    "url": "https://github.com/sagemath/sagetest/issues/25130",
    "user": "https://github.com/mkoeppe"
}
```
This seems to be a script for use within `build/pkgs` scripts, so it should go to `build/bin`.

CC:  @embray @kiwifb @dimpase

Branch/Commit: abc9fa93e25a86eef191ece157158e838d2fb433

Reviewer: Erik Bray

Author: Matthias Koeppe

Resolution: fixed

Issue created by migration from https://trac.sagemath.org/ticket/25130





---

archive/issue_comments_479047.json:
```json
{
    "body": "<a id='comment:1'></a>I go back-and-forth on that.  I agree with your logic completely. But I think we need to talk about where installation-related files go for use \"at runtime\".  Because currently all build-related files live in `build/` but don't get \"installed\" into `$SAGE_LOCAL`.  And yet they are in a sense needed at \"runtime\" since a feature of Sage is the ability to install new packages--particularly optional packages.\n\nSo I think what we really need is overall better organization of what build-related files are needed at runtime and how to \"install\" those files in such a way that installing additional Sage packages doesn't necessarily depend on `$SAGE_ROOT`.\n\nUnder that rubrik I'm not sure the existence of `build/bin` even makes sense since most, if not all of those scripts are needed for optional package installation.",
    "created_at": "2018-04-09T17:16:30Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25130#issuecomment-479047",
    "user": "https://github.com/embray"
}
```

<a id='comment:1'></a>I go back-and-forth on that.  I agree with your logic completely. But I think we need to talk about where installation-related files go for use "at runtime".  Because currently all build-related files live in `build/` but don't get "installed" into `$SAGE_LOCAL`.  And yet they are in a sense needed at "runtime" since a feature of Sage is the ability to install new packages--particularly optional packages.

So I think what we really need is overall better organization of what build-related files are needed at runtime and how to "install" those files in such a way that installing additional Sage packages doesn't necessarily depend on `$SAGE_ROOT`.

Under that rubrik I'm not sure the existence of `build/bin` even makes sense since most, if not all of those scripts are needed for optional package installation.



---

archive/issue_comments_479048.json:
```json
{
    "body": "<a id='comment:2'></a>build is for sage-the-distribution, and therefore for everything related to installing packages.\nsrc is for sagelib.",
    "created_at": "2018-04-09T18:06:49Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25130#issuecomment-479048",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:2'></a>build is for sage-the-distribution, and therefore for everything related to installing packages.
src is for sagelib.



---

archive/issue_comments_479049.json:
```json
{
    "body": "<a id='comment:3'></a>I agree, and yet that ignores everything else I wrote...",
    "created_at": "2018-04-10T07:32:39Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25130#issuecomment-479049",
    "user": "https://github.com/embray"
}
```

<a id='comment:3'></a>I agree, and yet that ignores everything else I wrote...



---

archive/issue_comments_479050.json:
```json
{
    "body": "<a id='comment:4'></a>Replying to [comment:3 embray]:\n> I agree, and yet that ignores everything else I wrote...\n\nSorry, I had to catch a flight.\n\nThe longer answer: I disagree with the notion of installation of optional packages being done \"at runtime\". There is no ticket that describes your goal of being able to install tickets even if $SAGE_ROOT is not present. Right now installation of optional packages does not even reliably work in binary distributions.",
    "created_at": "2018-04-10T09:49:28Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25130#issuecomment-479050",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:4'></a>Replying to [comment:3 embray]:
> I agree, and yet that ignores everything else I wrote...

Sorry, I had to catch a flight.

The longer answer: I disagree with the notion of installation of optional packages being done "at runtime". There is no ticket that describes your goal of being able to install tickets even if $SAGE_ROOT is not present. Right now installation of optional packages does not even reliably work in binary distributions.



---

archive/issue_comments_479051.json:
```json
{
    "body": "<a id='comment:5'></a>I think it should work--at least so long as a compiler is available.  As it stands, in binary distributions the feature is not disabled, and it is built into the command-line `sage` interface.\n\nAlternatively, I have also mentioned that I don't like it in the first place that build-related commands (e.g. `sage -b` and the like) are built into the same runtime interface that users use to start and run Sage and associated commands.  \n\nSo I think there needs to be a clearer vision of what an arbitrary user should expect to be able to do from the `sage` command regardless of the context in which they're running it.",
    "created_at": "2018-04-10T15:07:25Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25130#issuecomment-479051",
    "user": "https://github.com/embray"
}
```

<a id='comment:5'></a>I think it should work--at least so long as a compiler is available.  As it stands, in binary distributions the feature is not disabled, and it is built into the command-line `sage` interface.

Alternatively, I have also mentioned that I don't like it in the first place that build-related commands (e.g. `sage -b` and the like) are built into the same runtime interface that users use to start and run Sage and associated commands.  

So I think there needs to be a clearer vision of what an arbitrary user should expect to be able to do from the `sage` command regardless of the context in which they're running it.



---

archive/issue_comments_479052.json:
```json
{
    "body": "<a id='comment:6'></a>Put more succinctly, if someone wants to move this file around because it's \"build-related\" I have no problem with that.  My point is just that I see it as a distinction without a difference since Sage doesn't clearly separate build-time from run-time in all cases.",
    "created_at": "2018-04-10T15:11:24Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25130#issuecomment-479052",
    "user": "https://github.com/embray"
}
```

<a id='comment:6'></a>Put more succinctly, if someone wants to move this file around because it's "build-related" I have no problem with that.  My point is just that I see it as a distinction without a difference since Sage doesn't clearly separate build-time from run-time in all cases.



---

archive/issue_comments_479053.json:
```json
{
    "body": "<a id='comment:7'></a>I agree with `@`mkoeppe here. Installing Sage packages should not considered to be run-time, but build-time.",
    "created_at": "2018-04-11T07:37:31Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25130#issuecomment-479053",
    "user": "https://github.com/jdemeyer"
}
```

<a id='comment:7'></a>I agree with `@`mkoeppe here. Installing Sage packages should not considered to be run-time, but build-time.



---

archive/issue_comments_479054.json:
```json
{
    "body": "<a id='comment:8'></a>And yet `sage -i <optional-package>` is considered standard functionality of Sage that is expected to work for all users...?",
    "created_at": "2018-04-11T13:10:07Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25130#issuecomment-479054",
    "user": "https://github.com/embray"
}
```

<a id='comment:8'></a>And yet `sage -i <optional-package>` is considered standard functionality of Sage that is expected to work for all users...?



---

archive/issue_comments_479055.json:
```json
{
    "body": "<a id='comment:9'></a>It wouldn't be expected, for example, in a Linux distribution's sage binary; sage's optional packages would probably be supplied by the distribution's package manager instead.\n\nI see the \"sage -i\" as a mere convenience which \"calls out to the SAGE_ROOT\" to install a package.",
    "created_at": "2018-04-11T15:20:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25130#issuecomment-479055",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:9'></a>It wouldn't be expected, for example, in a Linux distribution's sage binary; sage's optional packages would probably be supplied by the distribution's package manager instead.

I see the "sage -i" as a mere convenience which "calls out to the SAGE_ROOT" to install a package.



---

archive/issue_comments_479056.json:
```json
{
    "body": "<a id='comment:10'></a>I'd be curious about the sage-on-gentoo approach to this.  If that is indeed the case (and I'm not necessarily disagreeing) then we should still clearly delineate what features of the command-line `sage` interface should and should not work outside of a Sage source tree, and *clearly* disable if not remove those features in cases where Sage is \"installed\" in another filesystem hierarchy.  This should also be made clear in the documentation both for users, *and* for developers interesting in packaging Sage for different platforms: That they need to think about what optional packages to include, if any, and if and whether users should have means to obtain optional packages.  There's also a question of whether or not \"pip\" should work...\n\nI'm thinking about this because currently installing optional packages is broken in Sage for Windows.  Users really have no way to install them.  If `sage -i` shouldn't work, then there has to be some mechanism whereby users can install packages.  For Windows I have a few different ideas which I'd be fine with, but I would still want to be able to either explicitly disable `sage -i`, or have it print some message as to how users should actually install optional packages (or maybe it could install the package, but would have to go through an alternate system-specific backend to do so).  For example in the case of Windows my preferred approach is to provide binary packages that are downloaded and unpacked.",
    "created_at": "2018-04-11T16:11:11Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25130#issuecomment-479056",
    "user": "https://github.com/embray"
}
```

<a id='comment:10'></a>I'd be curious about the sage-on-gentoo approach to this.  If that is indeed the case (and I'm not necessarily disagreeing) then we should still clearly delineate what features of the command-line `sage` interface should and should not work outside of a Sage source tree, and *clearly* disable if not remove those features in cases where Sage is "installed" in another filesystem hierarchy.  This should also be made clear in the documentation both for users, *and* for developers interesting in packaging Sage for different platforms: That they need to think about what optional packages to include, if any, and if and whether users should have means to obtain optional packages.  There's also a question of whether or not "pip" should work...

I'm thinking about this because currently installing optional packages is broken in Sage for Windows.  Users really have no way to install them.  If `sage -i` shouldn't work, then there has to be some mechanism whereby users can install packages.  For Windows I have a few different ideas which I'd be fine with, but I would still want to be able to either explicitly disable `sage -i`, or have it print some message as to how users should actually install optional packages (or maybe it could install the package, but would have to go through an alternate system-specific backend to do so).  For example in the case of Windows my preferred approach is to provide binary packages that are downloaded and unpacked.



---

archive/issue_comments_479057.json:
```json
{
    "body": "<a id='comment:11'></a>On sage-on-gentoo `sage -i` is not offered. I am pushing slowly but surely for `is_package_installed` to be removed at runtime and in the more distant future at build time. doctesting and `is_package_installed` are also causing me some headaches.\n\nFor information this is sage-on-gentoo `misc/package.py` file [ https://github.com/cschwan/sage-on-gentoo/blob/master/sci-mathematics/sage/files/sage-7.3-package.py].\n\nAnd these are the commands that I ship if you request debugging and doctesting enabled\n\n```\n/usr/bin/math-readline\n/usr/bin/sage\n/usr/bin/sage-cachegrind\n/usr/bin/sage-callgrind\n/usr/bin/sage-cleaner\n/usr/bin/sage-cython\n/usr/bin/sage-eval\n/usr/bin/sage-gdb-commands\n/usr/bin/sage-ipython\n/usr/bin/sage-massif\n/usr/bin/sage-native-execute\n/usr/bin/sage-notebook\n/usr/bin/sage-num-threads.py\n/usr/bin/sage-omega\n/usr/bin/sage-preparse\n/usr/bin/sage-python\n/usr/bin/sage-rst2sws\n/usr/bin/sage-rst2txt\n/usr/bin/sage-run\n/usr/bin/sage-run-cython\n/usr/bin/sage-runtests\n/usr/bin/sage-startuptime.py\n/usr/bin/sage-sws2rst\n/usr/bin/sage-valgrind\n/usr/bin/sage-version.sh\n```\n\nAs for dealing with optional packages. In sage-on-distro we of course expect them to be installed by the distro's package manager. Not all optional packages may be on offer and filling that gap is hard. Debian has a massive amount of packages so they have a better starting point than me on this.\n\nNext there is the problem that there are two categories of optional packages:\n* runtime packages, the functionality will be available if the package is installed and it can be installed after sage. They are easy to deal with - you can just install them whenever, in a standard path, and they should work.\n* build time packages. In the current system if those packages are found when sage is built, specific binding will be build for those packages. Those are the hard ones. If you want to support them they have to be part of your dependency tree.\n\nGentoo has useflags, I offer some built time optional packages through useflags. If you want any of `bliss`, `modular_decomposition`, `libbraiding` or `libhomfly` they can be enabled. `cbc` is hard and I failed to enable it once. \n\nMissing packages in that category are `mcqd`, `tdlib`, `coxeter3`, `fes`, `sirocco`, `meataxe` (not going to happen anytime soon, with all due respect, meataxe packaging sucks), `gurobi` (in its own category, it checks for presence differently from anyone else), `cplex` and `cbc` (the only extension to require lapack).",
    "created_at": "2018-04-11T21:44:09Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25130#issuecomment-479057",
    "user": "https://github.com/kiwifb"
}
```

<a id='comment:11'></a>On sage-on-gentoo `sage -i` is not offered. I am pushing slowly but surely for `is_package_installed` to be removed at runtime and in the more distant future at build time. doctesting and `is_package_installed` are also causing me some headaches.

For information this is sage-on-gentoo `misc/package.py` file [ https://github.com/cschwan/sage-on-gentoo/blob/master/sci-mathematics/sage/files/sage-7.3-package.py].

And these are the commands that I ship if you request debugging and doctesting enabled

```
/usr/bin/math-readline
/usr/bin/sage
/usr/bin/sage-cachegrind
/usr/bin/sage-callgrind
/usr/bin/sage-cleaner
/usr/bin/sage-cython
/usr/bin/sage-eval
/usr/bin/sage-gdb-commands
/usr/bin/sage-ipython
/usr/bin/sage-massif
/usr/bin/sage-native-execute
/usr/bin/sage-notebook
/usr/bin/sage-num-threads.py
/usr/bin/sage-omega
/usr/bin/sage-preparse
/usr/bin/sage-python
/usr/bin/sage-rst2sws
/usr/bin/sage-rst2txt
/usr/bin/sage-run
/usr/bin/sage-run-cython
/usr/bin/sage-runtests
/usr/bin/sage-startuptime.py
/usr/bin/sage-sws2rst
/usr/bin/sage-valgrind
/usr/bin/sage-version.sh
```

As for dealing with optional packages. In sage-on-distro we of course expect them to be installed by the distro's package manager. Not all optional packages may be on offer and filling that gap is hard. Debian has a massive amount of packages so they have a better starting point than me on this.

Next there is the problem that there are two categories of optional packages:
* runtime packages, the functionality will be available if the package is installed and it can be installed after sage. They are easy to deal with - you can just install them whenever, in a standard path, and they should work.
* build time packages. In the current system if those packages are found when sage is built, specific binding will be build for those packages. Those are the hard ones. If you want to support them they have to be part of your dependency tree.

Gentoo has useflags, I offer some built time optional packages through useflags. If you want any of `bliss`, `modular_decomposition`, `libbraiding` or `libhomfly` they can be enabled. `cbc` is hard and I failed to enable it once. 

Missing packages in that category are `mcqd`, `tdlib`, `coxeter3`, `fes`, `sirocco`, `meataxe` (not going to happen anytime soon, with all due respect, meataxe packaging sucks), `gurobi` (in its own category, it checks for presence differently from anyone else), `cplex` and `cbc` (the only extension to require lapack).



---

archive/issue_comments_479058.json:
```json
{
    "body": "<a id='comment:12'></a>Thanks for details.  It's definitely clear to me we need to do some more rethinking of how optional packages are handled.  Anyways, sorry to derail this ticket.  Like I said in the first place I don't mind moving the current location of `sage-dist-helpers`.  It just got me thinking about why I placed it where I did in the first place.  There was certainly motivated reasoning behind it, though in practice it still didn't make sense...",
    "created_at": "2018-04-13T14:08:23Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25130#issuecomment-479058",
    "user": "https://github.com/embray"
}
```

<a id='comment:12'></a>Thanks for details.  It's definitely clear to me we need to do some more rethinking of how optional packages are handled.  Anyways, sorry to derail this ticket.  Like I said in the first place I don't mind moving the current location of `sage-dist-helpers`.  It just got me thinking about why I placed it where I did in the first place.  There was certainly motivated reasoning behind it, though in practice it still didn't make sense...



---

archive/issue_comments_479059.json:
```json
{
    "body": "Changing branch from \"\" to \"u/mkoeppe/move_sage_dist_helpers_from_src_bin_to_build_bin\"",
    "created_at": "2019-05-18T14:47:04Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25130#issuecomment-479059",
    "user": "https://github.com/mkoeppe"
}
```

Changing branch from "" to "u/mkoeppe/move_sage_dist_helpers_from_src_bin_to_build_bin"



---

archive/issue_comments_479060.json:
```json
{
    "body": "Changing author from \"\" to \"Matthias Koeppe\"",
    "created_at": "2019-05-18T14:48:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25130#issuecomment-479060",
    "user": "https://github.com/mkoeppe"
}
```

Changing author from "" to "Matthias Koeppe"



---

archive/issue_comments_479061.json:
```json
{
    "body": "<a id='comment:14'></a>New commits:",
    "created_at": "2019-05-18T14:48:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25130#issuecomment-479061",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:14'></a>New commits:



---

archive/issue_events_063047.json:
```json
{
    "actor": "https://github.com/mkoeppe",
    "created_at": "2019-05-18T14:48:38Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "milestone": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25130#event-63047"
}
```



---

archive/issue_comments_479062.json:
```json
{
    "body": "Changing commit from \"\" to \"7405dd5b05259028e4dadc981b5bd309a124fbfb\"",
    "created_at": "2019-05-18T14:48:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25130#issuecomment-479062",
    "user": "https://github.com/mkoeppe"
}
```

Changing commit from "" to "7405dd5b05259028e4dadc981b5bd309a124fbfb"



---

archive/issue_comments_479063.json:
```json
{
    "body": "Description changed:\n``````diff\n--- \n+++ \n@@ -1 +1 @@\n-This seems to be a script for use within `build/pkgs` scripts, so it should probably go to `build/bin`.\n+This seems to be a script for use within `build/pkgs` scripts, so it should go to `build/bin`.\n``````\n",
    "created_at": "2019-05-18T14:48:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25130#issuecomment-479063",
    "user": "https://github.com/mkoeppe"
}
```

Description changed:
``````diff
--- 
+++ 
@@ -1 +1 @@
-This seems to be a script for use within `build/pkgs` scripts, so it should probably go to `build/bin`.
+This seems to be a script for use within `build/pkgs` scripts, so it should go to `build/bin`.
``````




---

archive/issue_comments_479064.json:
```json
{
    "body": "Changing status from new to needs_review.",
    "created_at": "2019-05-18T14:48:38Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25130#issuecomment-479064",
    "user": "https://github.com/mkoeppe"
}
```

Changing status from new to needs_review.



---

archive/issue_comments_479065.json:
```json
{
    "body": "<a id='comment:16'></a>I'm concerned that this will be broken if `$SAGE_ROOT` contains spaces.  It probably *shouldn't* and I'm not sure that there aren't other things that break if that's the case.\n\nI think it would be fine to just put each path in double-quotes.",
    "created_at": "2019-05-23T12:57:40Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25130#issuecomment-479065",
    "user": "https://github.com/embray"
}
```

<a id='comment:16'></a>I'm concerned that this will be broken if `$SAGE_ROOT` contains spaces.  It probably *shouldn't* and I'm not sure that there aren't other things that break if that's the case.

I think it would be fine to just put each path in double-quotes.



---

archive/issue_comments_479066.json:
```json
{
    "body": "Changing commit from \"7405dd5b05259028e4dadc981b5bd309a124fbfb\" to \"abc9fa93e25a86eef191ece157158e838d2fb433\"",
    "created_at": "2019-05-23T15:06:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25130#issuecomment-479066",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

Changing commit from "7405dd5b05259028e4dadc981b5bd309a124fbfb" to "abc9fa93e25a86eef191ece157158e838d2fb433"



---

archive/issue_comments_479067.json:
```json
{
    "body": "<a id='comment:17'></a>Branch pushed to git repo; I updated commit sha1. New commits:",
    "created_at": "2019-05-23T15:06:51Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25130#issuecomment-479067",
    "user": "https://trac.sagemath.org/admin/accounts/users/git"
}
```

<a id='comment:17'></a>Branch pushed to git repo; I updated commit sha1. New commits:



---

archive/issue_comments_479068.json:
```json
{
    "body": "<a id='comment:18'></a>Needs review.",
    "created_at": "2019-05-30T00:57:18Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25130#issuecomment-479068",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:18'></a>Needs review.



---

archive/issue_comments_479069.json:
```json
{
    "body": "<a id='comment:19'></a>Thank you.  Sorry this took so long.",
    "created_at": "2019-07-01T12:52:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25130#issuecomment-479069",
    "user": "https://github.com/embray"
}
```

<a id='comment:19'></a>Thank you.  Sorry this took so long.



---

archive/issue_comments_479070.json:
```json
{
    "body": "Changing reviewer from \"\" to \"Erik Bray\"",
    "created_at": "2019-07-01T12:52:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25130#issuecomment-479070",
    "user": "https://github.com/embray"
}
```

Changing reviewer from "" to "Erik Bray"



---

archive/issue_comments_479071.json:
```json
{
    "body": "Changing status from needs_review to positive_review.",
    "created_at": "2019-07-01T12:52:36Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25130#issuecomment-479071",
    "user": "https://github.com/embray"
}
```

Changing status from needs_review to positive_review.



---

archive/issue_events_063048.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-07-01T12:52:36Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "milestone": "sage-8.8",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25130#event-63048"
}
```



---

archive/issue_events_063049.json:
```json
{
    "actor": "https://github.com/embray",
    "created_at": "2019-07-01T12:52:36Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "milestone": "sage-8.9",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25130#event-63049"
}
```



---

archive/issue_comments_479072.json:
```json
{
    "body": "<a id='comment:20'></a>Thank you!",
    "created_at": "2019-07-01T18:42:01Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25130#issuecomment-479072",
    "user": "https://github.com/mkoeppe"
}
```

<a id='comment:20'></a>Thank you!



---

archive/issue_events_063050.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2019-07-04T11:09:34Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sagetest/issues/25130#event-63050"
}
```



---

archive/issue_comments_479073.json:
```json
{
    "body": "Resolution: fixed",
    "created_at": "2019-07-04T11:09:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25130#issuecomment-479073",
    "user": "https://github.com/vbraun"
}
```

Resolution: fixed



---

archive/issue_comments_479074.json:
```json
{
    "body": "Changing branch from \"u/mkoeppe/move_sage_dist_helpers_from_src_bin_to_build_bin\" to \"abc9fa93e25a86eef191ece157158e838d2fb433\"",
    "created_at": "2019-07-04T11:09:34Z",
    "issue": "https://github.com/sagemath/sagetest/issues/25130",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sagetest/issues/25130#issuecomment-479074",
    "user": "https://github.com/vbraun"
}
```

Changing branch from "u/mkoeppe/move_sage_dist_helpers_from_src_bin_to_build_bin" to "abc9fa93e25a86eef191ece157158e838d2fb433"
