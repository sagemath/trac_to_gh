# Issue 25184: Replace attrcall by operator.methodcaller

archive/issues_024947.json:
```json
{
    "assignees": [],
    "body": "The functionality provided by `sage.misc.misc.attrcall` is exactly the same as the Python standard function `operator.methodcaller`, so we might as well use the latter.\n\nAuthor: **Jeroen Demeyer**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25184_\n\n",
    "closed_at": "2018-04-16T16:58:23Z",
    "created_at": "2018-04-16T15:50:30Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/enhancement",
        "https://github.com/sagemath/sage/labels/wontfix"
    ],
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Replace attrcall by operator.methodcaller",
    "type": "issue",
    "updated_at": "2018-04-16T16:58:23Z",
    "url": "https://github.com/sagemath/sage/issues/25184",
    "user": "https://github.com/jdemeyer"
}
```
The functionality provided by `sage.misc.misc.attrcall` is exactly the same as the Python standard function `operator.methodcaller`, so we might as well use the latter.

Author: **Jeroen Demeyer**

_Issue created by migration from https://trac.sagemath.org/ticket/25184_





---

archive/issue_events_330523.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-04-16T15:50:30Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25184",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25184#event-330523"
}
```



---

archive/issue_events_330524.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-04-16T15:50:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25184",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25184#event-330524"
}
```



---

archive/issue_events_330525.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-04-16T15:50:30Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25184",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25184#event-330525"
}
```



---

archive/issue_events_330526.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-04-16T16:58:23Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25184",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25184#event-330526"
}
```



---

archive/issue_comments_387749.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\n`operator.methodcaller` is worse: it doesn't support pickling and doesn't support getting the method name.",
    "created_at": "2018-04-16T16:58:23Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25184",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25184#issuecomment-387749",
    "user": "https://github.com/jdemeyer"
}
```

<div id="comment:1" align="right">Comment 1</div>

`operator.methodcaller` is worse: it doesn't support pickling and doesn't support getting the method name.



---

archive/issue_events_330527.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-04-16T16:58:23Z",
    "event": "demilestoned",
    "issue": "https://github.com/sagemath/sage/issues/25184",
    "milestone_number": null,
    "milestone_title": "sage-8.2",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25184#event-330527"
}
```



---

archive/issue_events_330528.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-04-16T16:58:23Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25184",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25184#event-330528"
}
```



---

archive/issue_events_330529.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-04-16T16:58:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25184",
    "label": "https://github.com/sagemath/sage/labels/invalid",
    "label_color": "a6a6a6",
    "label_name": "invalid",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25184#event-330529"
}
```



---

archive/issue_events_330530.json:
```json
{
    "actor": "https://github.com/jdemeyer",
    "created_at": "2018-04-16T16:58:23Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25184",
    "label": "https://github.com/sagemath/sage/labels/wontfix",
    "label_color": "a6a6a6",
    "label_name": "wontfix",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25184#event-330530"
}
```
