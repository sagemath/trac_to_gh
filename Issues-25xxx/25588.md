# Issue 25588: Dimension of poset with certificate

archive/issues_025351.json:
```json
{
    "assignees": [],
    "body": "Currently `.dimension(certificate=True)` returns only realizer, not a pair having a realizer as second element. Compare to, for example, `.height(certificate=True)`. Change this.\n\n\nCC:  @fchapoton @tscrim\n\nBranch/Commit: **[064844b](https://github.com/sagemath/sagetrac-mirror/commit/064844b2efd2b7e25f3405db7509c64ac588374d)**\n\nReviewer: **Fr\u00e9d\u00e9ric Chapoton**\n\nAuthor: **Jori M\u00e4ntysalo**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25588_\n\n",
    "closed_at": "2018-06-19T22:11:05Z",
    "created_at": "2018-06-16T06:12:26Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
        "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Dimension of poset with certificate",
    "type": "issue",
    "updated_at": "2018-06-19T22:11:05Z",
    "url": "https://github.com/sagemath/sage/issues/25588",
    "user": "https://github.com/jm58660"
}
```
Currently `.dimension(certificate=True)` returns only realizer, not a pair having a realizer as second element. Compare to, for example, `.height(certificate=True)`. Change this.


CC:  @fchapoton @tscrim

Branch/Commit: **[064844b](https://github.com/sagemath/sagetrac-mirror/commit/064844b2efd2b7e25f3405db7509c64ac588374d)**

Reviewer: **Frédéric Chapoton**

Author: **Jori Mäntysalo**

_Issue created by migration from https://trac.sagemath.org/ticket/25588_





---

archive/issue_events_335897.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-06-16T06:12:26Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25588",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25588#event-335897"
}
```



---

archive/issue_events_335898.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-06-16T06:12:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25588",
    "label": "https://github.com/sagemath/sage/labels/component%3A%20combinatorics",
    "label_color": "0000ff",
    "label_name": "component: combinatorics",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25588#event-335898"
}
```



---

archive/issue_events_335899.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-06-16T06:12:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25588",
    "label": "https://github.com/sagemath/sage/labels/p4%20%E2%80%93%20minor",
    "label_color": "ffe799",
    "label_name": "p4 \u2013 minor",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25588#event-335899"
}
```



---

archive/issue_events_335900.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-06-16T06:12:26Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25588",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25588#event-335900"
}
```



---

archive/issue_comments_395392.json:
```json
{
    "body": "Branch: **[u/jmantysalo/dimension_of_poset_with_certificate](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/dimension_of_poset_with_certificate)**",
    "created_at": "2018-06-16T08:12:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25588#issuecomment-395392",
    "user": "https://github.com/jm58660"
}
```

Branch: **[u/jmantysalo/dimension_of_poset_with_certificate](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/dimension_of_poset_with_certificate)**



---

archive/issue_comments_395393.json:
```json
{
    "body": "<div id=\"comment:2\" align=\"right\">Comment 2</div>\n\nMore to come with #25562, but this should be the only patch doing a possible unpleasant change to the interface.\n\n---\nNew commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/3b7e5b9bb7daf3d7b134146a9e6ba4c24a1bdf9d\">3b7e5b9</a></td><td><code>Return a pair when certificate requested.</code></td></tr></table>\n",
    "created_at": "2018-06-16T08:14:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25588#issuecomment-395393",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:2" align="right">Comment 2</div>

More to come with #25562, but this should be the only patch doing a possible unpleasant change to the interface.

---
New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/3b7e5b9bb7daf3d7b134146a9e6ba4c24a1bdf9d">3b7e5b9</a></td><td><code>Return a pair when certificate requested.</code></td></tr></table>




---

archive/issue_comments_395394.json:
```json
{
    "body": "Commit: **[3b7e5b9](https://github.com/sagemath/sagetrac-mirror/commit/3b7e5b9bb7daf3d7b134146a9e6ba4c24a1bdf9d)**",
    "created_at": "2018-06-16T08:14:46Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25588#issuecomment-395394",
    "user": "https://github.com/jm58660"
}
```

Commit: **[3b7e5b9](https://github.com/sagemath/sagetrac-mirror/commit/3b7e5b9bb7daf3d7b134146a9e6ba4c24a1bdf9d)**



---

archive/issue_events_335901.json:
```json
{
    "actor": "https://github.com/jm58660",
    "created_at": "2018-06-16T08:14:46Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25588",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25588#event-335901"
}
```



---

archive/issue_comments_395395.json:
```json
{
    "body": "<div id=\"comment:3\" align=\"right\">Comment 3</div>\n\nyou may add a link to the ticket here using `:trac:`",
    "created_at": "2018-06-16T18:07:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25588#issuecomment-395395",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:3" align="right">Comment 3</div>

you may add a link to the ticket here using `:trac:`



---

archive/issue_comments_395396.json:
```json
{
    "body": "Changed commit from **[3b7e5b9](https://github.com/sagemath/sagetrac-mirror/commit/3b7e5b9bb7daf3d7b134146a9e6ba4c24a1bdf9d)** to **[064844b](https://github.com/sagemath/sagetrac-mirror/commit/064844b2efd2b7e25f3405db7509c64ac588374d)**",
    "created_at": "2018-06-16T19:45:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25588#issuecomment-395396",
    "user": "https://github.com/sagetrac-git"
}
```

Changed commit from **[3b7e5b9](https://github.com/sagemath/sagetrac-mirror/commit/3b7e5b9bb7daf3d7b134146a9e6ba4c24a1bdf9d)** to **[064844b](https://github.com/sagemath/sagetrac-mirror/commit/064844b2efd2b7e25f3405db7509c64ac588374d)**



---

archive/issue_comments_395397.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/064844b2efd2b7e25f3405db7509c64ac588374d\">064844b</a></td><td><code>Add trac link.</code></td></tr></table>\n",
    "created_at": "2018-06-16T19:45:45Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25588#issuecomment-395397",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/064844b2efd2b7e25f3405db7509c64ac588374d">064844b</a></td><td><code>Add trac link.</code></td></tr></table>




---

archive/issue_comments_395398.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nReplying to [@fchapoton](#comment%3A3):\n> you may add a link to the ticket here using `:trac:`\n\nLike this?",
    "created_at": "2018-06-16T19:46:03Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25588#issuecomment-395398",
    "user": "https://github.com/jm58660"
}
```

<div id="comment:5" align="right">Comment 5</div>

Replying to [@fchapoton](#comment%3A3):
> you may add a link to the ticket here using `:trac:`

Like this?



---

archive/issue_comments_395399.json:
```json
{
    "body": "Reviewer: **Fr\u00e9d\u00e9ric Chapoton**",
    "created_at": "2018-06-17T07:18:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25588#issuecomment-395399",
    "user": "https://github.com/fchapoton"
}
```

Reviewer: **Frédéric Chapoton**



---

archive/issue_comments_395400.json:
```json
{
    "body": "<div id=\"comment:6\" align=\"right\">Comment 6</div>\n\nok",
    "created_at": "2018-06-17T07:18:37Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25588#issuecomment-395400",
    "user": "https://github.com/fchapoton"
}
```

<div id="comment:6" align="right">Comment 6</div>

ok



---

archive/issue_events_335902.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-06-17T07:18:37Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25588",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25588#event-335902"
}
```



---

archive/issue_events_335903.json:
```json
{
    "actor": "https://github.com/fchapoton",
    "created_at": "2018-06-17T07:18:37Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25588",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25588#event-335903"
}
```



---

archive/issue_comments_395401.json:
```json
{
    "body": "Changed branch from **[u/jmantysalo/dimension_of_poset_with_certificate](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/dimension_of_poset_with_certificate)** to **[064844b](https://github.com/sagemath/sagetrac-mirror/commit/064844b2efd2b7e25f3405db7509c64ac588374d)**",
    "created_at": "2018-06-19T22:11:05Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25588",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25588#issuecomment-395401",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[u/jmantysalo/dimension_of_poset_with_certificate](https://github.com/sagemath/sagetrac-mirror/tree/u/jmantysalo/dimension_of_poset_with_certificate)** to **[064844b](https://github.com/sagemath/sagetrac-mirror/commit/064844b2efd2b7e25f3405db7509c64ac588374d)**



---

archive/issue_events_335904.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-06-19T22:11:05Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25588",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25588#event-335904"
}
```



---

archive/issue_events_335905.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "6bdfb8cd86f0783323a78b8e0c1a076b193a3af7",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-06-19T22:11:05Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25588",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25588#event-335905"
}
```
