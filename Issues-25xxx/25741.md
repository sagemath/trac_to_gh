# Issue 25741: Speedup _mpoly_dict_recursive by Cythonizing it

archive/issues_025504.json:
```json
{
    "assignees": [],
    "body": "This is called in constructing elements of multivariate Laurent polynomial rings, so it is worth speeding it up.\n\nCC:  @tscrim @jdemeyer\n\nKeywords: **days94**\n\nBranch/Commit: **[f70f053](https://github.com/sagemath/sagetrac-mirror/commit/f70f05337b987bf385c844f9f0209a122664abbe)**\n\nReviewer: **Marc Mezzarobba**\n\nAuthor: **Travis Scrimshaw**\n\n_Issue created by migration from https://trac.sagemath.org/ticket/25741_\n\n",
    "closed_at": "2018-08-05T08:17:21Z",
    "created_at": "2018-07-02T14:10:00Z",
    "labels": [
        "https://github.com/sagemath/sage/labels/performance",
        "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
        "https://github.com/sagemath/sage/labels/enhancement"
    ],
    "milestone": "https://github.com/sagemath/sage/milestones/sage-8.3",
    "reactions": [],
    "repository": "https://github.com/sagemath/sage",
    "title": "Speedup _mpoly_dict_recursive by Cythonizing it",
    "type": "issue",
    "updated_at": "2018-08-05T08:17:21Z",
    "url": "https://github.com/sagemath/sage/issues/25741",
    "user": "https://github.com/tscrim"
}
```
This is called in constructing elements of multivariate Laurent polynomial rings, so it is worth speeding it up.

CC:  @tscrim @jdemeyer

Keywords: **days94**

Branch/Commit: **[f70f053](https://github.com/sagemath/sagetrac-mirror/commit/f70f05337b987bf385c844f9f0209a122664abbe)**

Reviewer: **Marc Mezzarobba**

Author: **Travis Scrimshaw**

_Issue created by migration from https://trac.sagemath.org/ticket/25741_





---

archive/issue_events_337843.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-07-02T14:10:00Z",
    "event": "milestoned",
    "issue": "https://github.com/sagemath/sage/issues/25741",
    "milestone_number": null,
    "milestone_title": "sage-8.3",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25741#event-337843"
}
```



---

archive/issue_events_337844.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-07-02T14:10:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25741",
    "label": "https://github.com/sagemath/sage/labels/performance",
    "label_color": "696969",
    "label_name": "performance",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25741#event-337844"
}
```



---

archive/issue_events_337845.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-07-02T14:10:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25741",
    "label": "https://github.com/sagemath/sage/labels/p3%20%E2%80%93%20major",
    "label_color": "ffbb00",
    "label_name": "p3 \u2013 major",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25741#event-337845"
}
```



---

archive/issue_events_337846.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-07-02T14:10:00Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25741",
    "label": "https://github.com/sagemath/sage/labels/enhancement",
    "label_color": "696969",
    "label_name": "enhancement",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25741#event-337846"
}
```



---

archive/issue_comments_398260.json:
```json
{
    "body": "<div id=\"comment:1\" align=\"right\">Comment 1</div>\n\n\n```\nsage: R = Integers(10)['x,y,z']['t,s']\nsage: t,s = R.gens()\nsage: x,y,z = R.base_ring().gens()\nsage: %timeit (x+y+2*z*s+3*t)._mpoly_dict_recursive(('z','t','s'))\nThe slowest run took 8.13 times longer than the fastest. This could mean that an intermediate result is being cached.\n1000 loops, best of 3: 1.38 ms per loop\n```\nversus old\n\n```\nsage: %timeit (x+y+2*z*s+3*t)._mpoly_dict_recursive(('z','t','s'))\nThe slowest run took 115.43 times longer than the fastest. This could mean that an intermediate result is being cached.\n1000 loops, best of 3: 1.47 ms per loop\n```\nSo currently it has a ~10% speedup.",
    "created_at": "2018-07-02T14:11:50Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25741",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25741#issuecomment-398260",
    "user": "https://github.com/tscrim"
}
```

<div id="comment:1" align="right">Comment 1</div>


```
sage: R = Integers(10)['x,y,z']['t,s']
sage: t,s = R.gens()
sage: x,y,z = R.base_ring().gens()
sage: %timeit (x+y+2*z*s+3*t)._mpoly_dict_recursive(('z','t','s'))
The slowest run took 8.13 times longer than the fastest. This could mean that an intermediate result is being cached.
1000 loops, best of 3: 1.38 ms per loop
```
versus old

```
sage: %timeit (x+y+2*z*s+3*t)._mpoly_dict_recursive(('z','t','s'))
The slowest run took 115.43 times longer than the fastest. This could mean that an intermediate result is being cached.
1000 loops, best of 3: 1.47 ms per loop
```
So currently it has a ~10% speedup.



---

archive/issue_comments_398261.json:
```json
{
    "body": "Branch: **[public/performance/improve_mpoly_dict_recursive-25741](https://github.com/sagemath/sagetrac-mirror/tree/public/performance/improve_mpoly_dict_recursive-25741)**",
    "created_at": "2018-07-02T15:19:38Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25741",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25741#issuecomment-398261",
    "user": "https://github.com/tscrim"
}
```

Branch: **[public/performance/improve_mpoly_dict_recursive-25741](https://github.com/sagemath/sagetrac-mirror/tree/public/performance/improve_mpoly_dict_recursive-25741)**



---

archive/issue_events_337847.json:
```json
{
    "actor": "https://github.com/tscrim",
    "created_at": "2018-07-02T15:19:38Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25741",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25741#event-337847"
}
```



---

archive/issue_comments_398262.json:
```json
{
    "body": "Commit: **[f70f053](https://github.com/sagemath/sagetrac-mirror/commit/f70f05337b987bf385c844f9f0209a122664abbe)**",
    "created_at": "2018-07-02T15:19:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25741",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25741#issuecomment-398262",
    "user": "https://github.com/sagetrac-git"
}
```

Commit: **[f70f053](https://github.com/sagemath/sagetrac-mirror/commit/f70f05337b987bf385c844f9f0209a122664abbe)**



---

archive/issue_comments_398263.json:
```json
{
    "body": "<div id=\"comment:4\" align=\"right\">Comment 4</div>\n\nBranch pushed to git repo; I updated commit sha1. New commits:\n<table><tr><td><a href=\"https://github.com/sagemath/sagetrac-mirror/commit/f70f05337b987bf385c844f9f0209a122664abbe\">f70f053</a></td><td><code>Improve the speed of _mpoly_dict_recursive.</code></td></tr></table>\n",
    "created_at": "2018-07-02T15:19:41Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25741",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25741#issuecomment-398263",
    "user": "https://github.com/sagetrac-git"
}
```

<div id="comment:4" align="right">Comment 4</div>

Branch pushed to git repo; I updated commit sha1. New commits:
<table><tr><td><a href="https://github.com/sagemath/sagetrac-mirror/commit/f70f05337b987bf385c844f9f0209a122664abbe">f70f053</a></td><td><code>Improve the speed of _mpoly_dict_recursive.</code></td></tr></table>




---

archive/issue_events_337848.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2018-07-03T08:32:24Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25741",
    "label": "https://github.com/sagemath/sage/labels/needs%20review",
    "label_color": "7fff00",
    "label_name": "needs review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25741#event-337848"
}
```



---

archive/issue_events_337849.json:
```json
{
    "actor": "https://github.com/mezzarobba",
    "created_at": "2018-07-03T08:32:24Z",
    "event": "labeled",
    "issue": "https://github.com/sagemath/sage/issues/25741",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25741#event-337849"
}
```



---

archive/issue_comments_398264.json:
```json
{
    "body": "Reviewer: **Marc Mezzarobba**",
    "created_at": "2018-07-03T08:32:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25741",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25741#issuecomment-398264",
    "user": "https://github.com/mezzarobba"
}
```

Reviewer: **Marc Mezzarobba**



---

archive/issue_comments_398265.json:
```json
{
    "body": "<div id=\"comment:5\" align=\"right\">Comment 5</div>\n\nlgtm!",
    "created_at": "2018-07-03T08:32:24Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25741",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25741#issuecomment-398265",
    "user": "https://github.com/mezzarobba"
}
```

<div id="comment:5" align="right">Comment 5</div>

lgtm!



---

archive/issue_events_337850.json:
```json
{
    "actor": "https://github.com/vbraun",
    "created_at": "2018-08-05T08:17:21Z",
    "event": "unlabeled",
    "issue": "https://github.com/sagemath/sage/issues/25741",
    "label": "https://github.com/sagemath/sage/labels/positive%20review",
    "label_color": "dfffc0",
    "label_name": "positive review",
    "label_text_color": "ffffff",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25741#event-337850"
}
```



---

archive/issue_events_337851.json:
```json
{
    "actor": "https://github.com/vbraun",
    "commit_id": "4f2a218f156ad57cbe65a97bc051083e6949b700",
    "commit_repository": "https://github.com/sagemath/sage",
    "created_at": "2018-08-05T08:17:21Z",
    "event": "closed",
    "issue": "https://github.com/sagemath/sage/issues/25741",
    "type": "issue_event",
    "url": "https://github.com/sagemath/sage/issues/25741#event-337851"
}
```



---

archive/issue_comments_398266.json:
```json
{
    "body": "Changed branch from **[public/performance/improve_mpoly_dict_recursive-25741](https://github.com/sagemath/sagetrac-mirror/tree/public/performance/improve_mpoly_dict_recursive-25741)** to **[f70f053](https://github.com/sagemath/sagetrac-mirror/commit/f70f05337b987bf385c844f9f0209a122664abbe)**",
    "created_at": "2018-08-05T08:17:21Z",
    "formatter": "markdown",
    "issue": "https://github.com/sagemath/sage/issues/25741",
    "type": "issue_comment",
    "url": "https://github.com/sagemath/sage/issues/25741#issuecomment-398266",
    "user": "https://github.com/vbraun"
}
```

Changed branch from **[public/performance/improve_mpoly_dict_recursive-25741](https://github.com/sagemath/sagetrac-mirror/tree/public/performance/improve_mpoly_dict_recursive-25741)** to **[f70f053](https://github.com/sagemath/sagetrac-mirror/commit/f70f05337b987bf385c844f9f0209a122664abbe)**
